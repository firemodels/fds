...
! For MULT/OBST/SHAPE, recompute P1%AREA_ADJUST
...
OBST_SHAPE_IF: IF (OBST_SHAPE_AREA_ADJUST) THEN
   ...
   SHAPE_LOOP_1: DO IW=M%N_EXTERNAL_WALL_CELLS+1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
      ...
      P1=>M%BOUNDARY_PROP1(WC%P1_INDEX)
      II=BC%II
      JJ=BC%JJ
      KK=BC%KK
      IOR=BC%IOR
      ...
      SHAPE_SELECT_1: SELECT CASE(OB%SHAPE_TYPE)
         CASE(OBST_SPHERE_TYPE)
            MR%FDS_AREA(1) = MR%FDS_AREA(1) + P1%AREA
         CASE(OBST_CYLINDER_TYPE)
            ! OB%SHAPE_AREA follows the same pattern as SURF_IDS: top, sides, bottom
            SELECT CASE(IOR)
               CASE(3);         MR%FDS_AREA(1) = MR%FDS_AREA(1) + P1%AREA ! top
               CASE(-1,1,-2,2); MR%FDS_AREA(2) = MR%FDS_AREA(2) + P1%AREA ! side
               CASE(-3);        MR%FDS_AREA(3) = MR%FDS_AREA(3) + P1%AREA ! bottom
            END SELECT
         CASE(OBST_CONE_TYPE)
            SELECT CASE(IOR)
               CASE(-1,1,-2,2,3); MR%FDS_AREA(1) = MR%FDS_AREA(1) + P1%AREA
               CASE(-3);          MR%FDS_AREA(2) = MR%FDS_AREA(2) + P1%AREA
            END SELECT
         CASE(OBST_BOX_TYPE)
            ! Follows sextuplet ordering from SURF_ID6
            SELECT CASE(IOR)
               CASE(-1); MR%FDS_AREA(1) = MR%FDS_AREA(1) + P1%AREA
               CASE( 1); MR%FDS_AREA(2) = MR%FDS_AREA(2) + P1%AREA
               CASE(-2); MR%FDS_AREA(3) = MR%FDS_AREA(3) + P1%AREA
               CASE( 2); MR%FDS_AREA(4) = MR%FDS_AREA(4) + P1%AREA
               CASE(-3); MR%FDS_AREA(5) = MR%FDS_AREA(5) + P1%AREA
               CASE( 3); MR%FDS_AREA(6) = MR%FDS_AREA(6) + P1%AREA
            END SELECT
   ...
   SHAPE_LOOP_2: DO IW=M%N_EXTERNAL_WALL_CELLS+1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
      ...
      IF (OB%SHAPE_TYPE>0) P1%AREA_ADJUST=1._EB
      SHAPE_SELECT_2: SELECT CASE(OB%SHAPE_TYPE)
         CASE(OBST_SPHERE_TYPE)
            P1%AREA_ADJUST = OB%SHAPE_AREA(1)/MR%FDS_AREA(1)
         CASE(OBST_CYLINDER_TYPE)
            SELECT CASE(IOR)
               CASE(3);         P1%AREA_ADJUST = OB%SHAPE_AREA(1)/MR%FDS_AREA(1) ! top
               CASE(-1,1,-2,2); P1%AREA_ADJUST = OB%SHAPE_AREA(2)/MR%FDS_AREA(2) ! side
               CASE(-3);        P1%AREA_ADJUST = OB%SHAPE_AREA(3)/MR%FDS_AREA(3) ! bottom
            END SELECT
         CASE(OBST_CONE_TYPE)
            SELECT CASE(IOR)
               CASE(-1,1,-2,2,3); P1%AREA_ADJUST = OB%SHAPE_AREA(1)/MR%FDS_AREA(1)
               CASE(-3);          P1%AREA_ADJUST = OB%SHAPE_AREA(2)/MR%FDS_AREA(2)
            END SELECT
         CASE(OBST_BOX_TYPE)
            SELECT CASE(IOR)
               CASE(-1); P1%AREA_ADJUST = OB%SHAPE_AREA(1)/MR%FDS_AREA(1)
               CASE( 1); P1%AREA_ADJUST = OB%SHAPE_AREA(2)/MR%FDS_AREA(2)
               CASE(-2); P1%AREA_ADJUST = OB%SHAPE_AREA(3)/MR%FDS_AREA(3)
               CASE( 2); P1%AREA_ADJUST = OB%SHAPE_AREA(1)/MR%FDS_AREA(4)
               CASE(-3); P1%AREA_ADJUST = OB%SHAPE_AREA(2)/MR%FDS_AREA(5)
               CASE( 3); P1%AREA_ADJUST = OB%SHAPE_AREA(3)/MR%FDS_AREA(6)
            END SELECT
      END SELECT SHAPE_SELECT_2
      P1%AREA_ADJUST = P1%AREA_ADJUST*SF%AREA_MULTIPLIER
   ENDDO SHAPE_LOOP_2
...
WALL_LOOP_0: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   P1%PRESSURE_ZONE = M%PRESSURE_ZONE(BC%IIG,BC%JJG,BC%KKG)
   ...
   IF (.NOT.SOLID_CELL) THEN
      IF ( (ABS(P1%U_NORMAL_0)>TWO_EPSILON_EB .OR. ANY(SF%LEAK_PATH>=0)) .AND. WC%OBST_INDEX>0 ) THEN
...
P1 => M%BOUNDARY_PROP1(WC%P1_INDEX)
P2 => M%BOUNDARY_PROP2(WC%P2_INDEX)
...
IF (M%CELL(M%CELL_INDEX(I,J,K))%SOLID .OR. IW<=M%N_EXTERNAL_WALL_CELLS) THEN
   ...
   P1%ZZ_F(1:N_TRACKED_SPECIES)  = M%ZZ(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
...
IF (OBST_INDEX>0) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
   IF (OBX%FDS_AREA(ABS(IOR))>TWO_EPSILON_EB) P1%AREA_ADJUST = SF%AREA_MULTIPLIER*OBX%INPUT_AREA(ABS(IOR))/OBX%FDS_AREA(ABS(IOR))
   IF (P1%AREA_ADJUST<=TWO_EPSILON_EB) P1%AREA_ADJUST = 1._EB
   IF (OBX%MASS>1.E5_EB .AND. OBX%BULK_DENSITY<=0._EB) OBX%MASS = SF%SURFACE_DENSITY*P1%AREA*P1%AREA_ADJUST
ENDIF
...
IF (OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLOW)>=TWO_EPSILON_EB) THEN
   ...
   IF (OBX%FDS_AREA(ABS(IOR))>TWO_EPSILON_EB) &
      P1%U_NORMAL_0 = SF%VOLUME_FLOW*(OBX%INPUT_AREA(ABS(IOR))/OBX%UNDIVIDED_INPUT_AREA(ABS(IOR))) / OBX%FDS_AREA(ABS(IOR))
ENDIF
IF (OBST_INDEX>0 .AND. ABS(SF%MASS_FLUX_TOTAL)>=TWO_EPSILON_EB) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
   P1%U_NORMAL_0 = SF%MASS_FLUX_TOTAL / RHOA * P1%AREA_ADJUST
ENDIF
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   P1%AREA_ADJUST = SF%AREA_MULTIPLIER * VT%INPUT_AREA/VT%FDS_AREA
   IF (P1%AREA_ADJUST<=TWO_EPSILON_EB) P1%AREA_ADJUST = 1._EB
   ...
   IF (ABS(SF%VOLUME_FLOW)>TWO_EPSILON_EB)   THEN
      P1%U_NORMAL_0 = SF%VOLUME_FLOW*(VT%INPUT_AREA/VT%UNDIVIDED_INPUT_AREA)/VT%FDS_AREA
   ENDIF
   IF (ABS(SF%MASS_FLUX_TOTAL)>TWO_EPSILON_EB) P1%U_NORMAL_0 = SF%MASS_FLUX_TOTAL/RHOA*P1%AREA_ADJUST

   IF (SF%CONVERT_VOLUME_TO_MASS) THEN
      IF (ABS(P1%U_NORMAL_0)>TWO_EPSILON_EB) THEN
         ZZ_GET=0._EB
         ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,SF%MASS_FRACTION(1:N_TRACKED_SPECIES))
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
         SF%MASS_FLUX = -RHOA*(RSUM0/RSUM_F)*(TMPA/SF%TMP_FRONT)*SF%MASS_FRACTION*P1%U_NORMAL_0
      ...
      SELECT CASE(ABS(IOR))
         CASE(1)
            IF (VT%RADIUS>0._EB) THEN
               ...
               P1%U_NORMAL_0 = P1%U_NORMAL_0*(VT%RADIUS**2-RR)/VT%RADIUS**2
            ELSE
               ...
               P1%U_NORMAL_0 = P1%U_NORMAL_0*PY*PZ
            ENDIF
         CASE(2)
            IF (VT%RADIUS>0._EB) THEN
            ...
            ELSE
               ...
               PZ = 4._EB*(M%ZC(K)-VT%Z1_ORIG)*(VT%Z2_ORIG-M%ZC(K))/(VT%Z2_ORIG-VT%Z1_ORIG)**2
               P1%U_NORMAL_0 = P1%U_NORMAL_0*PX*PZ
            ENDIF
         CASE(3)
            IF (VT%RADIUS>0._EB) THEN
            ...
            ELSE
               ...
               PY = 4._EB*(M%YC(J)-VT%Y1_ORIG)*(VT%Y2_ORIG-M%YC(J))/(VT%Y2_ORIG-VT%Y1_ORIG)**2
               IF (CYLINDRICAL) THEN
                  P1%U_NORMAL_0 = P1%U_NORMAL_0*PX
               ELSE
                  P1%U_NORMAL_0 = P1%U_NORMAL_0*PX*PY
               ENDIF
            ENDIF
      END SELECT
      IF (ABS(SF%VOLUME_FLOW)>=TWO_EPSILON_EB) THEN   ! Match desired volume flow
         IF (VT%RADIUS>0._EB) THEN
            P1%U_NORMAL_0 = P1%U_NORMAL_0*2._EB
         ELSE
            P1%U_NORMAL_0 = P1%U_NORMAL_0*9._EB/4._EB
         ENDIF
      ENDIF
   ENDIF PARABOLIC_IF
         ...
         CASE(1)
            IF (VT%RADIUS>0._EB) THEN
               ...
               IF (RR>R1 .AND. RR<=VT%RADIUS .AND. DELTA>TWO_EPSILON_EB) THEN
                  P1%U_NORMAL_0 = P1%U_NORMAL_0*(1._EB - ((RR-R1)/DELTA)**2 )
               ENDIF
            ENDIF
         CASE(2)
            IF (VT%RADIUS>0._EB) THEN
   ...
   IF (SF%PROFILE==ATMOSPHERIC_PROFILE) THEN
      IF (M%ZC(K)<GROUND_LEVEL) THEN
         ...
         RETURN
      ENDIF
      P1%U_NORMAL_0 = P1%U_NORMAL_0*((M%ZC(K)-GROUND_LEVEL)/SF%Z0)**SF%PLE
   ENDIF
      ...
      SELECT CASE(ABS(IOR))
         CASE(1)
            IF (SF%RAMP(VELO_PROF_X)%ID/='null') THEN
               ...
               RETURN
            ENDIF
            P1%U_NORMAL_0 = P1%U_NORMAL_0*EVALUATE_RAMP(M%YC(J),SF%RAMP(VELO_PROF_Y)%INDEX,TAU=1._EB)
            P1%U_NORMAL_0 = P1%U_NORMAL_0*EVALUATE_RAMP(M%ZC(K),SF%RAMP(VELO_PROF_Z)%INDEX,TAU=1._EB)
         CASE(2)
            IF (SF%RAMP(VELO_PROF_Y)%ID/='null') THEN
               ...
               RETURN
            ENDIF
            P1%U_NORMAL_0 = P1%U_NORMAL_0*EVALUATE_RAMP(M%XC(I),SF%RAMP(VELO_PROF_X)%INDEX,TAU=1._EB)
            P1%U_NORMAL_0 = P1%U_NORMAL_0*EVALUATE_RAMP(M%ZC(K),SF%RAMP(VELO_PROF_Z)%INDEX,TAU=1._EB)
         CASE(3)
...
IF (SF%TMP_FRONT_INITIAL>0._EB) THEN
   P1%TMP_F = SF%TMP_FRONT_INITIAL
ELSEIF (SF%TMP_FRONT>0._EB) THEN
   P1%TMP_F = M%TMP_0(BC%KK) + EVALUATE_RAMP(TSI,SF%RAMP(TIME_TEMP)%INDEX,TAU=SF%RAMP(TIME_TEMP)%TAU)*(SF%TMP_FRONT-M%TMP_0(BC%KK))
ELSE
   P1%TMP_F = M%TMP_0(BC%KK)
ENDIF

IF (SF%TMP_BACK>0._EB) THEN
   P1%TMP_B = SF%TMP_BACK
ELSE
   P1%TMP_B = SF%TMP_INNER
ENDIF
...
VENT_LOOP: DO N=1,N_VENT
   ...
   ! Find the wall indices (IW) for the vent and set the activation time (P1%T_IGN) for each one
   ...
   DO KK=VT%K1+1,MAX(VT%K1+1,VT%K2)
      DO JJ=VT%J1+1,MAX(VT%J1+1,VT%J2)
         DO II=VT%I1+1,MAX(VT%I1+1,VT%I2)
            ...
            IF (IW==0) CYCLE
            P1 => MESHES(NM)%BOUNDARY_PROP1(MESHES(NM)%WALL(IW)%P1_INDEX)
            BC => MESHES(NM)%BOUNDARY_COORD(MESHES(NM)%WALL(IW)%BC_INDEX)

            IF (ACTIVATE_VENT) THEN
               IF (VT%FIRE_SPREAD_RATE>0._EB) THEN
                  P1%T_IGN = T + SQRT((BC%X-VT%X0)**2 + (BC%Y-VT%Y0)**2 + (BC%Z-VT%Z0)**2)/VT%FIRE_SPREAD_RATE
               ELSE
                  P1%T_IGN = T
               ENDIF
            ELSE
...
IF (REMOVE .AND. SURFACE(WC%SURF_INDEX)%PYROLYSIS_MODEL==PYROLYSIS_SPECIFIED) THEN
   ...
   IF (IW_OLD>0) THEN
      P1 => MESHES(NM)%BOUNDARY_PROP1(WC%P1_INDEX)
      IF (MESHES(NM)%WALL(IW_OLD)%SURF_INDEX==WC%SURF_INDEX) P1%T_IGN = &
         MESHES(NM)%BOUNDARY_PROP1(MESHES(NM)%WALL(IW_OLD)%P1_INDEX)%T_IGN
   ENDIF
ENDIF
...
DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   KK  = BC%KK
   SELECT CASE(IOR)
      CASE( 1) ; P1%U_NORMAL_S = -U(II,JJ,KK)
      CASE(-1) ; P1%U_NORMAL_S =  U(II-1,JJ,KK)
      CASE( 2) ; P1%U_NORMAL_S = -V(II,JJ,KK)
      CASE(-2) ; P1%U_NORMAL_S =  V(II,JJ-1,KK)
      CASE( 3) ; P1%U_NORMAL_S = -W(II,JJ,KK)
      CASE(-3) ; P1%U_NORMAL_S =  W(II,JJ,KK-1)
   END SELECT
