...
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(MAX_DUCTS,MAX_DUCTS),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW, &
            TMP_WGT,TAU_FAN,TAU_VF,CP,ZZ_GET(1:N_TRACKED_SPECIES)
...
IF (LEAK_DUCTS > 0) THEN
   ...
   IF (N_TRACKED_SPECIES > 0) THEN
      ALLOCATE(LEAK_ZZ(0:N_ZONE,0:N_ZONE,1:N_TRACKED_SPECIES,1:NMESHES))
   ...
   ELSE
      ...
      LEAK_CP = Z2CP_C(ITMP)+TMP_WGT*(Z2CP_C(ITMP+1)-Z2CP_C(ITMP))
   ENDIF
ENDIF
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   ...
   IF (DN%LEAKAGE) THEN
      ...
      IF (N_TRACKED_SPECIES > 0) THEN
         ...
         CALL GET_AVERAGE_SPECIFIC_HEAT(DN%ZZ_V,DN%CP,TMPA)      
      ELSE
         ITMP =MIN(4999,INT(TMPA))
         TMP_WGT = TMPA - ITMP
         DN%CP = Z2CP_C(ITMP)+TMP_WGT*(Z2CP_C(ITMP+1)-Z2CP_C(ITMP))
      ENDIF 
      DN%TMP_V  = DN%TMP
      DN%RSUM_V = DN%RSUM
      DN%CP_V   = DN%CP
...
REAL(EB) :: TNOW,MTOT,ETOT,VTOT,ZZTOT(1:N_TRACKED_SPECIES),TGUESS,VFLOW,ZZ_N(1:N_TRACKED_SPECIES)
...
ITER_LOOP: DO
   ...
   NODE_LOOP:DO NN = 1,NE%N_DUCTNODES
      ...
      IF(N_TRACKED_SPECIES > 0) THEN
         DN%ZZ(:)  = ZZTOT/MTOT
         ZZ_N = DN%ZZ
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_N,DN%RSUM)
      ENDIF
      TGUESS = TGUESS / MTOT      
      ETOT = ETOT/ MTOT
      DN%TMP = TGUESS
      CP_LOOP: DO
         IF (N_TRACKED_SPECIES>0) THEN
            CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_N,DN%CP,TGUESS)
         ELSE
...
REAL(EB) :: ZZ_SUM(1:N_TRACKED_SPECIES),ZZ_N(1:N_TRACKED_SPECIES),RHO_SUM,P_SUM,AREA_SUM,AREA,TMP_SUM,H_SUM,H_G,TMP_NEW,CPBAR,TNOW,&
           P_AVE !,VEL2
      ...
      DO JJ = J1,J2
         DO II = I1, I2
            ...
            IF (N_TRACKED_SPECIES > 0) THEN
               ...
               ZZ_N = ZZ(II,JJ,KK,:)
               CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_N,CPBAR,TMP(II,JJ,KK))
...
!            VEL2  = 0.25_EB*( (UP(II,JJ,KK)+UP(II-1,JJ,KK))**2+(VP(II,JJ,KK)+VP(II,JJ-1,KK))**2+(WP(II,JJ,KK)+WP(II,JJ,KK-1))**2)
   ...
   DO
      IF (N_TRACKED_SPECIES > 0) THEN
         ZZ_N = DN%ZZ_V(:)
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_N,CPBAR,TMP_SUM)      
      ELSE
         CALL GET_AVERAGE_SPECIFIC_HEAT_BG(CPBAR,TMP(II,JJ,KK))
      ENDIF
      TMP_NEW = H_SUM / CPBAR
   ...
   IF (N_TRACKED_SPECIES>0) THEN
      ZZ_N = NODE_ZZ(NN,1:N_TRACKED_SPECIES,NM)
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_N,NODE_RSUM(NN,NM))
   ENDIF
ENDDO NODE_LOOP
...
LEAKAGE_BC: IF (LEAK_DUCTS > 0) THEN
   ...
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (N_TRACKED_SPECIES > 0) THEN
         ZZ_N = ZZ(II,JJ,KK,:)
         LEAK_ZZ(IZ1,IZ2,1:N_TRACKED_SPECIES,NM) = LEAK_ZZ(IZ1,IZ2,1:N_TRACKED_SPECIES,NM)+ZZ_N*RHO(II,JJ,KK)*AREA
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_N,CPBAR,TMP(II,JJ,KK))
         H_G = CPBAR * TMP(II,JJ,KK)
      ELSE
         CALL GET_AVERAGE_SPECIFIC_HEAT_BG(CPBAR,TMP(II,JJ,KK))
   ...
   DO IZ1 = 0,N_ZONE
      DO IZ2 = 0,N_ZONE
         ...
         IF (N_TRACKED_SPECIES>0) THEN
            ...
            ZZ_N = LEAK_ZZ(IZ1,IZ2,1:N_TRACKED_SPECIES,NM)
            CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_N,LEAK_RSUM(IZ1,IZ2,NM))
         ENDIF
...
REAL(EB) :: AREA_SUM,AREA,CPBAR,H_G,TMP_SUM,TMP_NEW,ZZ_N(1:N_TRACKED_SPECIES)
TYPE(DUCTNODE_TYPE), POINTER :: DN=>NULL()
...
DO NN=1,N_DUCTNODES
   ...
   LEAKAGE_IF: IF (DN%LEAKAGE) THEN
      ...
      DO NM=1,NMESHES
         ...
         IF (N_TRACKED_SPECIES > 0) THEN
            ZZ_N(1:N_TRACKED_SPECIES) = LEAK_ZZ(IZ1,IZ2,1:N_TRACKED_SPECIES,NM)
            DN%ZZ_V(1:N_TRACKED_SPECIES) = DN%ZZ_V(1:N_TRACKED_SPECIES) + ZZ_N(1:N_TRACKED_SPECIES) * LEAK_RHO(IZ1,IZ2,NM) * AREA
            CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_N,CPBAR,LEAK_TMP(IZ1,IZ2,NM))
      ...
      IF (N_TRACKED_SPECIES>0) THEN
         ...
         ZZ_N = DN%ZZ_V
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_N,DN%RSUM_V)
      ELSE
         DN%RSUM_V = RSUM0
      ENDIF  
   ...
   DO NZ2 = 0, N_ZONE
      IF (LEAK_PATH(NZ1,NZ2) == 1) THEN
         ...
         IF (N_TRACKED_SPECIES > 0) THEN
            ...
            DU%ZZ(1:N_TRACKED_SPECIES) = ZZ_GET
