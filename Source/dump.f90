...
LOGICAL, INTENT(IN) :: CC_INTERP2FACES,CC_CELL_CENTERED
REAL(FB), INTENT(INOUT) :: SLICE_MIN, SLICE_MAX
...
IF (NVERTS>0 .AND. NFACES>0) THEN
   ...
   ENDIF
   IF (SLICE_MIN < SLICE_MAX) THEN
      SLICE_MIN = MIN(SLICE_MIN, VAL_MIN)
      SLICE_MAX = MAX(SLICE_MAX, VAL_MAX)
   ELSE
      SLICE_MIN = VAL_MIN
      SLICE_MAX = VAL_MAX
   ENDIF
   WRITE(FUNIT_BOUNDS,'(ES13.6,1X,ES13.6,1X,ES13.6)') STIME, SLICE_MIN, SLICE_MAX
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   IF (.NOT.PLOT3D) THEN
      ...
      IF (SL%SLICETYPE=='STRUCTURED') THEN ! write out slice file using original slice file format
         ...
         IQ2 = IQ + N_SLCF_MAX
         IF (SL%VAL_MIN < SL%VAL_MAX) THEN
            SL%VAL_MIN = MIN(SL%VAL_MIN, SLICE_MIN)
            SL%VAL_MAX = MAX(SL%VAL_MAX, SLICE_MAX)
         ELSE
            SL%VAL_MIN = SLICE_MIN
            SL%VAL_MAX = SLICE_MAX
         ENDIF             
         OPEN(LU_SLCF(IQ2,NM),FILE=FN_SLCF(IQ2,NM),FORM='FORMATTED',STATUS='REPLACE')
         WRITE(LU_SLCF(IQ2,NM),'(ES13.6,1X,ES13.6,1X,ES13.6)') STIME, SL%VAL_MIN, SL%VAL_MAX
...
FILE_LOOP: DO NF=1,N_BNDF
   ...
   ENDDO PATCH_LOOP
   IF (BF%VAL_MIN < BF%VAL_MAX) THEN
      BF%VAL_MIN = MIN(BF%VAL_MIN, BOUND_MIN)
      BF%VAL_MAX = MAX(BF%VAL_MAX, BOUND_MAX)
   ELSE
      BF%VAL_MIN = BOUND_MIN
      BF%VAL_MAX = BOUND_MAX
   ENDIF
   OPEN(LU_BNDF(NF+N_BNDF,NM),FILE=FN_BNDF(NF+N_BNDF,NM),FORM='FORMATTED',STATUS='REPLACE')
   WRITE(LU_BNDF(NF+N_BNDF,NM),'(ES13.6,1X,ES13.6,1X,ES13.6)')STIME,BF%VAL_MIN,BF%VAL_MAX
