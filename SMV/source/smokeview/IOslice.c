      ...
      if(usetexturebar!=0){
        if(sd->volslice==1){
          ...
          if(sd->slicetype==SLICE_TERRAIN){
            ...
            SNIFF_ERRORS("after drawslice_terrain");
          }
          else{
            drawslice_texture(sd);
            SNIFF_ERRORS("after drawslice_texture");
          }
        }
      }
      else{
        if(sd->volslice==1){
          if(sd->slicetype==SLICE_CENTER){
            if(cellcenter_interp==1){
              ...
              SNIFF_ERRORS("after drawvolslice_cellcenter_interp");
            }
            else{
          ...
          if(sd->slicetype==SLICE_CENTER){
            if(cellcenter_interp==1){
              ...
              SNIFF_ERRORS("after drawslice_cellcenter_interp");
            }
            else{
              drawslice_cellcenter(sd);
              SNIFF_ERRORS("after drawslice_cellcener");
            }
          }
          else{
            drawslice(sd);
  ...
  if(sd->idir==1){
   ...
   for(i=sd->is1+1; i<sd->is2; i++){
     ...
     for(k=sd->ks1 + 1; k<sd->ks2; k++){
       ...
       n++; n2++; 
       if(skip_slice_in_embedded_mesh==1&&iblank_embed!=NULL&&iblank_embed[IJK(i,sd->js1,k)]==0)continue;
       r11 = (float)sd->iqsliceframe[n]/255;
       r31 = (float)sd->iqsliceframe[n2]/255;
       r13 = (float)sd->iqsliceframe[n+1]/255;
       r33 = (float)sd->iqsliceframe[n2+1]/255;
       rmid = (r11+r31+r13+r33)/4.0;
       z1 = (zplt[k]+zplt[k-1])/2.0;
       z3 = (zplt[k+1]+zplt[k])/2.0;
       zmid = (z1+z3)/2.0;
...
void drawvolslice_cellcenter_interp(const slicedata *sd){
  ...
  if(ploty>=0&&visy_all==1){
    ...
    if(ploty<1)ploty=1;
    ploty2=ploty-1;
    constval = (yplt[ploty]+yplt[ploty2])/2.0;
    glBegin(GL_TRIANGLES);
  ...
  if(plotz>=0&&visz_all==1){
   ...
   if(plotz<1)plotz=1;
   plotz2=plotz-1;
   constval = (zplt[plotz]+zplt[plotz2])/2.0;
   glBegin(GL_TRIANGLES);
   for(i=sd->is1+1; i<sd->is1+sd->nslicei; i++){
     float xmid;
...
void drawvslice_cellcenter(const vslicedata *vd){
  slicedata *u, *v, *w,*sd;
  float vrange;
  mesh *meshi;
  float *xplttemp,*yplttemp,*zplttemp;
  ...
  if(sd->idir==1){
   ...
   constval = (xplttemp[sd->is1]+xplttemp[sd->is1+1])/2.0+offset_slice*sd->sliceoffset;
   glLineWidth(vectorlinewidth);
   glBegin(GL_LINES);
   maxj = sd->js2;
   if(sd->js1+1>maxj)maxj=sd->js1+1;
   ...
   glEnd();
   SNIFF_ERRORS("after drawvslice_center:lines dir=1");
   ...
   for(j=sd->js1; j<maxj; j++){
     ...
     yy1 = yplttemp[j];
     yhalf = (yplttemp[j]+yplttemp[j+1])/2.0;
     for(k=sd->ks1; k<sd->ks2; k++){
       int index_v, index_w;
       float *color_v, *color_w;
       float zhalf;
       float z1;
       float dy,dz;
       
       z1 = zplttemp[k];
       zhalf = (zplttemp[k]+zplttemp[k+1])/2.0;
   ...
   glEnd();
   SNIFF_ERRORS("after drawvslice_center:points dir=1");
   ...
   constval = (yplttemp[sd->js1]+yplttemp[sd->js1+1])/2.0+offset_slice*sd->sliceoffset;
   glLineWidth(vectorlinewidth);
   glBegin(GL_LINES);
   ...
   glEnd();
   SNIFF_ERRORS("after drawvslice_center:lines dir=2");
   ...
   for(i=sd->is1; i<sd->is2; i++){
     ...
     x1 = xplttemp[i];
     xhalf = (xplttemp[i]+xplttemp[i+1])/2.0;
     for(k=sd->ks1; k<sd->ks2; k++){
       int index_u, index_w;
       float *color_u, *color_w;
       float zhalf;
       float z1;
       float dx,dz;
   ...
   glEnd();
   SNIFF_ERRORS("after drawvslice_center:points dir=2");
  }
  else if(sd->idir==3){
    float constval;
    int i;
   ...
   glEnd();
   SNIFF_ERRORS("after drawvslice_center:lines dir=3");
   ...
   glEnd();
   SNIFF_ERRORS("after drawvslice_center:points dir=3");
  }
}
