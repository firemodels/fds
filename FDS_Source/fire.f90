...
REAL(EB) :: ZZ_0(1:N_TRACKED_SPECIES),ZZ_NEW(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),&
            ...
            BCOR_SPEC(1:N_TRACKED_SPECIES),BCOR_REAC(1:N_REACTIONS),Q_REAC_OUT(1:N_REACTIONS),TOTAL_MIXED_MASS_0
INTEGER :: SR
...
IF (ANY(REACTION(:)%FAST_CHEMISTRY)) THEN
   DO SR = 0,N_SERIES_REACTIONS
      ...
      CALL BOUNDEDNESS_CORRECTION(BCOR_SPEC,BCOR_REAC,ZZ_0,ZZ_NEW) ! reaction rate boundedness correction
      ZZ_NEW = ZZ_0 + DZZ*BCOR_SPEC ! corrected FE step for all species (5.54)
      ZZ_0 = ZZ_NEW
      Q_REAC_LOC = Q_REAC_LOC + Q_REAC_OUT*BCOR_REAC*TOTAL_MIXED_MASS
   ENDDO
ENDIF
IF (.NOT.ALL(REACTION(:)%FAST_CHEMISTRY)) THEN
   ...
   ZZ_NEW = ZZ_0 + DZZ
   CALL BOUNDEDNESS_CORRECTION(BCOR_SPEC,BCOR_REAC,ZZ_0,ZZ_NEW)
   ZZ_NEW = ZZ_0 + DZZ*BCOR_SPEC
   Q_REAC_LOC = Q_REAC_LOC + Q_REAC_OUT*BCOR_REAC*TOTAL_MIXED_MASS
ENDIF
...
SUBROUTINE BOUNDEDNESS_CORRECTION(BCOR_SPEC,BCOR_REAC,ZZ_0,ZZ_NEW)

! This subroutine finds a correction for reaction rates such that all species remain bounded.

REAL(EB), INTENT(OUT) :: BCOR_SPEC(1:N_TRACKED_SPECIES),BCOR_REAC(1:N_REACTIONS)
REAL(EB), INTENT(IN) :: ZZ_0(1:N_TRACKED_SPECIES),ZZ_NEW(1:N_TRACKED_SPECIES)
REAL(EB) :: BCOR(1:N_TRACKED_SPECIES),DZ_IB,DZ_OB
INTEGER :: NS,NR,NNS
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
DO NS=1,N_TRACKED_SPECIES
   IF (ZZ_NEW(NS)<0._EB) THEN ! FDS Tech Guide (5.55)
      ...
      BCOR(NS) = DZ_IB/MAX(DZ_IB+DZ_OB,TWO_EPSILON_EB)
   ENDIF
...
! Find BCOR_REAC for groups with a common "AIR" species

BCOR_REAC = 1._EB
REAC_LOOP: DO NR=1,N_REACTIONS
   RN => REACTION(NR)
   SPEC_LOOP_1: DO NS=1,N_TRACKED_SPECIES
      IF (NS/=RN%AIR_SMIX_INDEX) CYCLE SPEC_LOOP_1
         SPEC_LOOP_2: DO NNS=1,N_TRACKED_SPECIES
            IF (ABS(RN%NU(NNS))<TWO_EPSILON_EB) CYCLE SPEC_LOOP_2
            BCOR_REAC(NR) = MIN(BCOR(NNS),BCOR_REAC(NR))
         ENDDO SPEC_LOOP_2
   ENDDO SPEC_LOOP_1
ENDDO REAC_LOOP
...
! Now put the MAX BCOR_REAC into the BCOR_SPEC for each species

BCOR_SPEC = 0._EB
REAC_LOOP_2: DO NR=1,N_REACTIONS
   RN => REACTION(NR)
   SPEC_LOOP_3: DO NS=1,N_TRACKED_SPECIES
      IF (ABS(RN%NU(NS))<TWO_EPSILON_EB) CYCLE SPEC_LOOP_3
      BCOR_SPEC(NS) = MAX(BCOR_REAC(NR),BCOR_SPEC(NS))
   ENDDO SPEC_LOOP_3
ENDDO REAC_LOOP_2

!print *,BCOR_SPEC

END SUBROUTINE BOUNDEDNESS_CORRECTION
