...
USE MEMORY_FUNCTIONS, ONLY: COMPUTE_ONE_D_STORAGE_DIMENSIONS,COMPUTE_BOUNDARY_PROPERTY_STORAGE_DIMENSIONS,&
                            COMPUTE_PARTICLE_STORAGE_DIMENSIONS,COMPUTE_WALL_STORAGE_DIMENSIONS,&
                            COMPUTE_CFACE_STORAGE_DIMENSIONS,COMPUTE_BOUNDARY_COORD_STORAGE_DIMENSIONS
...
IF (N_LP_ARRAY_INDICES>0 .AND. .NOT.DO_EVACUATION) THEN
...
IF (.NOT.DO_EVACUATION) THEN
   ...
   IF (N_REACTIONS>0) THEN
      M%CHI_R = MAX(0._EB,RADIATIVE_FRACTION*EVALUATE_RAMP(T_BEGIN,REACTION(1)%RAMP_CHI_R_INDEX))
   ELSE
...
IF (STRATIFICATION .AND. .NOT.DO_EVACUATION) THEN
...
M%QR    = 0._EB
IF (.NOT.DO_EVACUATION) M%UII = 4._EB*SIGMA*TMPA4
M%ANGLE_INC_COUNTER = 0
M%RAD_CALL_COUNTER  = 0
IF (RADIATION .AND. .NOT.DO_EVACUATION) THEN
...
M%WORK5 = 0._EB
M%WORK6 = 0._EB
IF (.NOT.DO_EVACUATION) M%WORK7 = 0._EB
...
CALL COMPUTE_BOUNDARY_PROPERTY_STORAGE_DIMENSIONS
...
NOT_EVAC_IF_1: IF (.NOT.DO_EVACUATION) THEN
...
NOT_EVAC_IF_2: IF (.NOT.DO_EVACUATION) THEN
DO IOR=3,-3,-6
   IF (IOR== 3) K = 0
   IF (IOR==-3) K = KBP1
   DO J=1,JBAR
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS
   K = M%WALL(IW)%BOUNDARY_COORD%KKG
...
WALL_LOOP_0: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.DO_EVACUATION) WC%ONE_D%PRESSURE_ZONE = M%PRESSURE_ZONE(WC%BOUNDARY_COORD%IIG, &
                                                                    WC%BOUNDARY_COORD%JJG,WC%BOUNDARY_COORD%KKG)
   ...
   IF (IW<=M%N_EXTERNAL_WALL_CELLS) THEN
      ...
      ELSE
         IC =  M%CELL_INDEX(WC%BOUNDARY_COORD%II,WC%BOUNDARY_COORD%JJ,WC%BOUNDARY_COORD%KK)
...
CFACE_LOOP_0: DO ICF=1,M%N_EXTERNAL_CFACE_CELLS+M%N_INTERNAL_CFACE_CELLS
   CFA => M%CFACE(ICF)
   CFA%ONE_D%PRESSURE_ZONE = M%PRESSURE_ZONE(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG,CFA%BOUNDARY_COORD%KKG)
ENDDO CFACE_LOOP_0
   ...
   DO NSLICE = 1, M%N_TERRAIN_SLCF
      DO ICF=1,M%N_CUTFACE_MESH
         ...
         IF (CFA%NVEC(KAXIS)>-TWO_EPSILON_EB .AND.  CFA%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
            IF (CFA%BOUNDARY_COORD%KKG > M%K_AGL_SLICE(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG,NSLICE)) THEN
               M%K_AGL_SLICE(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG,NSLICE) = &
                  MIN( M%KBAR , M%K_AGL_SLICE(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG,NSLICE)+CFA%BOUNDARY_COORD%KKG )
            ENDIF
         ENDIF
      ENDDO
   ENDDO
...
NON_EVAC_IF: IF (.NOT.DO_EVACUATION) THEN
...
IF (DO_EVACUATION) N_EDGES_DIM = 4*(IBP1*KBP1+JBP1*KBP1)
DO N=1,M%N_OBST
   OB=>M%OBSTRUCTION(N)
   IPTS = OB%I2-OB%I1
   JPTS = OB%J2-OB%J1
...
INTEGER  :: N,IZERO,IERR,IBP1,JBP1,KBP1,IBAR,JBAR,KBAR,IW,IOR
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS
   ...
   SELECT CASE(M%WALL(IW)%BOUNDARY_COORD%IOR)
      CASE( 1)
         IF (LBC==FISHPAK_BC_NEUMANN_NEUMANN)   LBC = FISHPAK_BC_DIRICHLET_NEUMANN
         IF (LBC==FISHPAK_BC_NEUMANN_DIRICHLET) LBC = FISHPAK_BC_DIRICHLET_DIRICHLET
      CASE(-1)
...
CHECK_MESHES: IF (IW<=M%N_EXTERNAL_WALL_CELLS .AND. .NOT.DO_EVACUATION) THEN
...
WC%BOUNDARY_COORD%II = I
WC%BOUNDARY_COORD%JJ = J
WC%BOUNDARY_COORD%KK = K
WC%BOUNDARY_COORD%IOR = IOR
WC%SURF_INDEX = SURF_INDEX_NEW
WC%OBST_INDEX = OBST_INDEX
WC%BOUNDARY_TYPE = BOUNDARY_TYPE

WC%BOUNDARY_COORD%X = XW
WC%BOUNDARY_COORD%Y = YW
WC%BOUNDARY_COORD%Z = ZW
WC%BOUNDARY_COORD%IIG    = IIG
WC%BOUNDARY_COORD%JJG    = JJG
WC%BOUNDARY_COORD%KKG    = KKG
WC%ONE_D%RDN    = RDN
WC%ONE_D%AREA   = AW
...
IF (.NOT.DO_EVACUATION .AND. (M%SOLID(M%CELL_INDEX(I,J,K)).OR.IW<=M%N_EXTERNAL_WALL_CELLS)) THEN
...
IF (OBST_INDEX>0 .AND. .NOT.DO_EVACUATION) THEN
...
IF (OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLOW)>=TWO_EPSILON_EB .AND. .NOT.DO_EVACUATION) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
   WC%ONE_D%U_NORMAL_0 = SF%VOLUME_FLOW*(OBX%INPUT_AREA(ABS(IOR))/OBX%UNDIVIDED_INPUT_AREA(ABS(IOR))) / OBX%FDS_AREA(ABS(IOR))
ENDIF
IF (OBST_INDEX>0 .AND. ABS(SF%MASS_FLUX_TOTAL)>=TWO_EPSILON_EB .AND. .NOT.DO_EVACUATION) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   ENDIF
   IF (ABS(SF%MASS_FLUX_TOTAL)>TWO_EPSILON_EB .AND. .NOT.DO_EVACUATION) THEN
      WC%ONE_D%U_NORMAL_0 = SF%MASS_FLUX_TOTAL/RHOA*WC%ONE_D%AREA_ADJUST
   ENDIF
      ...
      SELECT CASE(ABS(IOR))
         CASE(1)
            IF (SF%RAMP_V_X/='null') THEN
               ...
               RETURN
            ENDIF
            WC%ONE_D%U_NORMAL_0 = WC%ONE_D%U_NORMAL_0*EVALUATE_RAMP(M%YC(J),SF%RAMP_INDEX(VELO_PROF_Y),TAU=1._EB)
            WC%ONE_D%U_NORMAL_0 = WC%ONE_D%U_NORMAL_0*EVALUATE_RAMP(M%ZC(K),SF%RAMP_INDEX(VELO_PROF_Z),TAU=1._EB)
         CASE(2)
            IF (SF%RAMP_V_Y/='null') THEN
               ...
               RETURN
            ENDIF
            WC%ONE_D%U_NORMAL_0 = WC%ONE_D%U_NORMAL_0*EVALUATE_RAMP(M%XC(I),SF%RAMP_INDEX(VELO_PROF_X),TAU=1._EB)
            WC%ONE_D%U_NORMAL_0 = WC%ONE_D%U_NORMAL_0*EVALUATE_RAMP(M%ZC(K),SF%RAMP_INDEX(VELO_PROF_Z),TAU=1._EB)
         CASE(3)
   ...
   IF (VT%FIRE_SPREAD_RATE>0._EB) THEN
      DIST = SQRT((WC%BOUNDARY_COORD%X-VT%X0)**2 + (WC%BOUNDARY_COORD%Y-VT%Y0)**2 + (WC%BOUNDARY_COORD%Z-VT%Z0)**2)
...
IF (SF%FIRE_SPREAD_RATE>0._EB) THEN
   DIST = SQRT((WC%BOUNDARY_COORD%X-SF%XYZ(1))**2 +(WC%BOUNDARY_COORD%Y-SF%XYZ(2))**2 +(WC%BOUNDARY_COORD%Z-SF%XYZ(3))**2)
...
IF (SF%TMP_FRONT>0._EB) THEN
   WC%ONE_D%TMP_F = M%TMP_0(WC%BOUNDARY_COORD%KK) + &
                    EVALUATE_RAMP(TSI,SF%RAMP_INDEX(TIME_TEMP),TAU=SF%TAU(TIME_TEMP))*(SF%TMP_FRONT-M%TMP_0(WC%BOUNDARY_COORD%KK))
ELSE
   IF (SF%RAMP_T_I_INDEX < 0) WC%ONE_D%TMP_F = M%TMP_0(WC%BOUNDARY_COORD%KK)
ENDIF
      ...
      DO JJ=VT%J1+1,MAX(VT%J1+1,VT%J2)
         DO II=VT%I1+1,MAX(VT%I1+1,VT%I2)
            ...
            IF (ACTIVATE_VENT) THEN
               IF (VT%FIRE_SPREAD_RATE>0._EB) THEN
                  WALL(IW)%ONE_D%T_IGN = T + SQRT((WALL(IW)%BOUNDARY_COORD%X-VT%X0)**2 + &
                                                  (WALL(IW)%BOUNDARY_COORD%Y-VT%Y0)**2 + &
                                                  (WALL(IW)%BOUNDARY_COORD%Z-VT%Z0)**2)/VT%FIRE_SPREAD_RATE
               ELSE
...
DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   IF (WC%OBST_INDEX==0 .AND. WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY .AND. .NOT.MM%SOLID(MM%CELL_INDEX(IIO,JJO,KKO))) THEN
      IC  = CELL_INDEX(WC%BOUNDARY_COORD%II,WC%BOUNDARY_COORD%JJ,WC%BOUNDARY_COORD%KK)
      ICG = CELL_INDEX(WC%BOUNDARY_COORD%IIG,WC%BOUNDARY_COORD%JJG,WC%BOUNDARY_COORD%KKG)
      IF (SOLID(ICG)) CYCLE
      OBST_INDEX=0
...
IF (WC%BOUNDARY_TYPE/=NULL_BOUNDARY) THEN
   ...
   IF (OBST_INDEX_C(IC)>0) THEN
      ...
      WC%SURF_INDEX = OBSTRUCTION(WC%OBST_INDEX)%SURF_INDEX(WC%BOUNDARY_COORD%IOR)
   ...
   CALL INIT_WALL_CELL(NM,WC%BOUNDARY_COORD%II,WC%BOUNDARY_COORD%JJ,WC%BOUNDARY_COORD%KK,WC%OBST_INDEX,IW,&
                       WC%BOUNDARY_COORD%IOR,WC%SURF_INDEX,IERR,T)
   WC => WALL(IW)
