...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   CALL HEAT_TRANSFER_BC(SURF_INDEX,T,WC%TMP_F,WALL_INDEX=IW) 
   WC=>WALL(IW)
...
IF (VIRTUAL_PARTICLES) THEN      
   DO II = 1, NLP         
      ...
      IF (SURF_INDEX > 0) THEN
         CALL HEAT_TRANSFER_BC(SURF_INDEX,T,LP%TMP_F,PARTICLE_INDEX=II)
         IF (SURFACE(SURF_INDEX)%THERMALLY_THICK .AND. CALL_PYROLYSIS) CALL PYROLYSIS(T,DT_BC,PARTICLE_INDEX=II)         
      ENDIF
   ENDDO
ENDIF

IF (CALL_PYROLYSIS) WALL_COUNTER = 0

CONTAINS

SUBROUTINE HEAT_TRANSFER_BC(SURF_INDEX,T,TMP_F,WALL_INDEX,PARTICLE_INDEX)
INTEGER, INTENT(IN) :: SURF_INDEX
INTEGER, INTENT(IN), OPTIONAL :: WALL_INDEX,PARTICLE_INDEX
REAL(EB), INTENT(IN) :: T
REAL(EB), INTENT(INOUT) :: TMP_F
REAL(EB) :: AREA_ADJUST,ARO,FDERIV,QEXTRA,QNET,RAMP_FACTOR,RHO_G,RSUM_W,TMP_G,TMP_OTHER,TSI,UN, &
            ZZ_G_ALL(MAX_SPECIES),RHO_ZZ_F(MAX_SPECIES),ZZ_GET(0:N_TRACKED_SPECIES),TW
LOGICAL :: INFLOW
INTEGER II,JJ,KK,IIG,JJG,KKG,IOR,IIO,JJO,KKO
TYPE(WALL_TYPE), POINTER :: WC=>NULL()
TYPE(LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
...
CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER !Only for wall cells
   ...
   IF (INFLOW) THEN
      IF (WC%VENT_INDEX>0) THEN
         ...
         IF (VT%TMP_EXTERIOR>0._EB) TMP_F = VT%TMP_EXTERIOR
      ELSE
         TMP_F = TMP_0(KK)
      ENDIF
   ...
   TMP(II,JJ,KK) = MAX(TMPMIN,2._EB*TMP_F - TMP(IIG,JJG,KKG))
   IF (N_TRACKED_SPECIES>0) ZZP(II,JJ,KK,1:N_TRACKED_SPECIES) = &
                            MIN(1._EB,MAX(0._EB,2._EB*WC%ZZ_F(1:N_TRACKED_SPECIES)-ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)))
...
CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
   ...
   DTMP = TMP_G - TMP_F
   IF (PRESENT(WALL_INDEX)) THEN
      ONE_D%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,WALL_INDEX=WALL_INDEX)
   ELSE
      ONE_D%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,&
...
CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER !Only for wall cells
   ...
   ENDIF
   ONE_D%QCONF = 0._EB
   TMP_F = 0.5*(TMP(IIG,JJG,KKG)+TMP(II,JJ,KK))
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_AVERAGE_SPECIFIC_HEAT,GET_SPECIFIC_HEAT,GET_SENSIBLE_ENTHALPY
...
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
INTEGER :: I,SURF_INDEX,IIG,JJG,KKG,IW,NS,N
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   CALL CALC_SPECIES_BC(T,WALL_INDEX=IW)
   WC=>WALL(IW)
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. N_TRACKED_SPECIES > 0 .AND. &
       .NOT.SOLID(CELL_INDEX(WC%II,WC%JJ,WC%KK)) .AND. .NOT.SOLID(CELL_INDEX(WC%IIG,WC%JJG,WC%KKG))) &
      ZZP(WC%II,WC%JJ,WC%KK,1:N_TRACKED_SPECIES) = 2._EB*WC%ZZ_F(1:N_TRACKED_SPECIES) - &
                                                   ZZP(WC%IIG,WC%JJG,WC%KKG,1:N_TRACKED_SPECIES)
...
IF (VIRTUAL_PARTICLES) THEN
   ...
   PARTICLE_LOOP: DO I=1,NLP
      ...
      IF (LPC%SURF_INDEX >0) CALL CALC_SPECIES_BC(T,PARTICLE_INDEX=I)
      LP => LAGRANGIAN_PARTICLE(I)
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      ...
      KKG = LP%KKG
      IF (SF%SHRINK) THEN
         RADIUS = SUM(ONE_D%LAYER_THICKNESS)
      ...
      DO NS=0,N_TRACKED_SPECIES
         ...
         ELSE
            MW_RATIO = 1._EB
         ENDIF
         M_DOT_PPP = ONE_D%MASSFLUX(NS)*ONE_D%AREA*RVC
         ZZ_GET=0._EB
         IF (NS>0) ZZ_GET(NS)=1._EB
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR,TMP(IIG,JJG,KKG))
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR2,LP%TMP_F)
         DELTA_H_G = CPBAR2*LP%TMP_F-CPBAR*TMP(IIG,JJG,KKG)
         IF (ENTHALPY_TRANSPORT) THEN
            CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S_B,LP%TMP_F)
            D_LAGRANGIAN(IIG,JJG,KKG) =  D_LAGRANGIAN(IIG,JJG,KKG) + LP%PWT*M_DOT_PPP*H_S_B
         ELSE
            D_LAGRANGIAN(IIG,JJG,KKG) =  D_LAGRANGIAN(IIG,JJG,KKG) + LP%PWT*M_DOT_PPP*(MW_RATIO + DELTA_H_G/H_G)/RHO(IIG,JJG,KKG)
         ENDIF
         RHO_NEW = RHO(IIG,JJG,KKG) + M_DOT_PPP*DT
         IF (NS>0) THEN
            ZZP(IIG,JJG,KKG,NS) = (RHO(IIG,JJG,KKG)*ZZP(IIG,JJG,KKG,NS) + M_DOT_PPP*DT)/RHO_NEW  
         ELSE
            IF (N_TRACKED_SPECIES>0) ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES) = &
                                     RHO(IIG,JJG,KKG)*ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)/RHO_NEW
         ENDIF
      ...
      IF (N_REACTIONS>0) THEN
         IF (ONE_D%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)>0._EB .AND. CORRECTOR) &
            M_DOT(2,NM) = M_DOT(2,NM) + ONE_D%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)*ONE_D%AREA
      ENDIF   
      ...
      CALC_LP_MASS:IF (SF%THERMALLY_THICK) THEN
         ...
         SELECT CASE (SF%GEOMETRY)
            CASE (SURF_CARTESIAN)               
               DO N=1,SF%N_CELLS
                  X2 = LP%ONE_D%X(N)
                  LP%MASS = LP%MASS + SF%LENGTH*SF%WIDTH*(X2-X1)*SUM(LP%ONE_D%RHO(N,:))
                  X1 = X2
               END DO
             CASE (SURF_CYLINDRICAL)
               ...
               DO N=1,SF%N_CELLS
                  X2 = LP%ONE_D%X(N)
                  LP%MASS = LP%MASS + SF%LENGTH*PI*(X2**2-X1**2)*SUM(LP%ONE_D%RHO(N,:))
                  X1 = X2
               END DO      
            CASE (SURF_SPHERICAL)
               X1 = SUM(SF%LAYER_THICKNESS)
               DO N=1,SF%N_CELLS
                  X2 = LP%ONE_D%X(N)
                  LP%MASS = LP%MASS + SF%LENGTH*FOTHPI*(X2**3-X1**3)*SUM(LP%ONE_D%RHO(N,:))
                  X1 = X2
               END DO      
         END SELECT
...
SUBROUTINE CALC_SPECIES_BC(T,WALL_INDEX,PARTICLE_INDEX)
...
REAL(EB) :: T,ZZ_G,UN,DD,MFT,TSI,RADIUS,AREA_SCALING,&
            RVC,RHO_NEW,M_DOT_PPP,CP,CPBAR,MW_RATIO,H_G,DELTA_H_G,ZZ_GET(0:N_TRACKED_SPECIES),CPBAR2,RSUM_F,MPUA_SUM,TW,AREA_ADJUST
...
CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
   ...
   SUM_MASSFLUX_LOOP: DO N=0,N_TRACKED_SPECIES
      IF (SF%MASS_FLUX(N) > 0._EB) THEN  ! Use user-specified ramp-up of mass flux
         IF (ABS(TW-T_BEGIN)< SPACING(WC%TW) .AND. SF%RAMP_INDEX(N)>=1) THEN
            TSI = T + DT
         ELSE
            TSI = T + DT - TW
         ENDIF
   ...
   IF (PRESENT(WALL_INDEX)) THEN
      IF (WC%EW>0._EB) ONE_D%MASSFLUX(:) = ONE_D%MASSFLUX(:)*EXP(-WC%EW)
...
INTEGER, INTENT(IN), OPTIONAL:: WALL_INDEX,PARTICLE_INDEX
REAL(EB) :: DTMP,QNETF,QDXKF,QDXKB,RR,TMP_G,RFACF,RFACB,RFACF2,RFACB2,PPCLAUS,PPSURF, &
...
REAL(EB), POINTER, DIMENSION(:) :: X,LAYER_THICKNESS
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN UNNPACK_WALL_PARTICLE
   ...
   ONE_D%HEAT_TRANS_COEF = &
      HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,PARTICLE_INDEX=PARTICLE_INDEX)
   ONE_D%QCONF = ONE_D%HEAT_TRANS_COEF*DTMP
   Q_WATER_F  = 0._EB   
ELSE UNNPACK_WALL_PARTICLE
  RETURN
ENDIF UNNPACK_WALL_PARTICLE

IF (SF%SHRINK) THEN
   X => ONE_D%X
   LAYER_THICKNESS => ONE_D%LAYER_THICKNESS
   N_LAYER_CELLS => ONE_D%N_LAYER_CELLS
ELSE
   X => SF%X_S
   LAYER_THICKNESS => SF%LAYER_THICKNESS
   N_LAYER_CELLS => SF%N_LAYER_CELLS
ENDIF
...
SELECT CASE(SF%BACKING)
   CASE(VOID)  ! Non-insulated backing to an ambient void
      ...
      ENDIF
      DTMP = TMP_BACK - TMP_B
      HTCB = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,WALL_INDEX=-1)
   ...
   CASE(EXPOSED)
      IF (IWB >= 0) THEN  
         ...
         IF (WALL(IWB)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
            ...
            DTMP = TMP_BACK - TMP_B
            HTCB = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,WALL_INDEX=IWB)
      ...
      ELSE !Particle backside is the same gas cell
         ...
         DTMP = TMP_BACK - TMP_B
         HTCB = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,PARTICLE_INDEX=PARTICLE_INDEX)
...
COMPUTE_GRID: IF (SF%SHRINK) THEN
   NWP = SUM(ONE_D%N_LAYER_CELLS)
   CALL GET_WALL_NODE_WEIGHTS(NWP,SF%N_LAYERS,ONE_D%N_LAYER_CELLS,LAYER_THICKNESS,SF%GEOMETRY, &
      X(0:NWP),SF%LAYER_DIVIDE,DX_S(1:NWP),RDX_S(0:NWP+1),RDXN_S(0:NWP),DX_WGT_S(0:NWP),DXF,DXB,&
...
PYROLYSIS_MATERIAL_IF: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_MATERIAL) THEN
   ...
   IF (SF%SHRINK) X_S_NEW(0:NWP)  = X(0:NWP)
   ...
   POINT_LOOP1: DO I=1,NWP
      ...
      MATERIAL_LOOP1b: DO N=1,SF%N_MATL
         ...
         REACTION_LOOP: DO J=1,ML%N_REACTIONS
            ...
            ONE_D%RHO(I,N) = ONE_D%RHO(I,N) - DT_BC*REACTION_RATE
            ONE_D%RHO(I,N) = MAX(0._EB, ONE_D%RHO(I,N))            
         ...
         IF (VOLSUM<1.0_EB) THEN
            ...
            IF (VOLSUM>0.0_EB) THEN
               MATERIAL_LOOP1c: DO N=1,SF%N_MATL
                  ONE_D%RHO(I,N) = ONE_D%RHO(I,N)/VOLSUM
      ...
      IF (SF%SHRINK) X_S_NEW(I) = X_S_NEW(I-1)+(X(I)-X(I-1))*VOLSUM
   ...
   IF (TW>T) THEN
      IF (SUM(ONE_D%MASSFLUX(:)) > 0._EB) TW = T
   ENDIF
   ...
   RECOMPUTE_GRID: IF (ONE_D%SHRINKING) THEN
      ...
      LAYER_LOOP: DO NL=1,SF%N_LAYERS
         LAYER_THICKNESS(NL) = X_S_NEW(I+ONE_D%N_LAYER_CELLS(NL))-X_S_NEW(I)
         ! Remove very thin layers
         IF (LAYER_THICKNESS(NL) < SF%MINIMUM_LAYER_THICKNESS) THEN 
            X_S_NEW(I+ONE_D%N_LAYER_CELLS(NL):NWP) = X_S_NEW(I+N_LAYER_CELLS(NL):NWP)-LAYER_THICKNESS(NL)
            LAYER_THICKNESS(NL) = 0._EB
            N_LAYER_CELLS_NEW(NL)  = 0
         ELSE
            CALL GET_N_LAYER_CELLS(SF%MIN_DIFFUSIVITY(NL),LAYER_THICKNESS(NL), &
               SF%STRETCH_FACTOR(NL),SF%CELL_SIZE_FACTOR,N_LAYER_CELLS_NEW(NL),SMALLEST_CELL_SIZE(NL))
            NWP_NEW = NWP_NEW + N_LAYER_CELLS_NEW(NL)
         ENDIF
         IF ( N_LAYER_CELLS_NEW(NL) /= N_LAYER_CELLS(NL)) RECOMPUTE = .TRUE.
         THICKNESS = THICKNESS + LAYER_THICKNESS(NL)
         I = I + N_LAYER_CELLS(NL)
      ENDDO LAYER_LOOP      
      ...
      IF (RECOMPUTE) THEN
         ...
         CALL GET_WALL_NODE_WEIGHTS(NWP_NEW,SF%N_LAYERS,N_LAYER_CELLS_NEW,LAYER_THICKNESS,SF%GEOMETRY, &
         ...
         CALL ChkMemErr('WALL:PYROLYSIS','INT_WGT',IZERO)
         CALL GET_INTERPOLATION_WEIGHTS(SF%N_LAYERS,NWP,NWP_NEW,N_LAYER_CELLS,N_LAYER_CELLS_NEW, &
                                    X(0:NWP),X_S_NEW(0:NWP_NEW),INT_WGT)      
         ...
         ENDDO
         DEALLOCATE(INT_WGT)
         ONE_D%N_LAYER_CELLS = N_LAYER_CELLS_NEW(1:SF%N_LAYERS)
         NWP = NWP_NEW
         X(0:NWP) = X_S_NEW(0:NWP)      ! Note: X(NWP+1...) are not set to zero.
      ELSE      
         CALL GET_WALL_NODE_WEIGHTS(NWP,SF%N_LAYERS,N_LAYER_CELLS_NEW,ONE_D%LAYER_THICKNESS,SF%GEOMETRY, &
            X(0:NWP),SF%LAYER_DIVIDE,DX_S(1:NWP),RDX_S(0:NWP+1),RDXN_S(0:NWP),DX_WGT_S(0:NWP),DXF,DXB, &
            LAYER_INDEX(0:NWP+1),MF_FRAC(1:NWP))
      ENDIF
...
CALL ChkMemErr('WALL:PYROLYSIS','TMP_W_NEW',IZERO)
...
REAL(EB) FUNCTION HEAT_TRANSFER_COEFFICIENT(DELTA_TMP,H_FIXED,GEOMETRY,CONV_LENGTH,WALL_INDEX,PARTICLE_INDEX)

USE PHYSICAL_FUNCTIONS, ONLY : GET_CONDUCTIVITY
REAL(EB), INTENT(IN) :: DELTA_TMP,H_FIXED,CONV_LENGTH
INTEGER, INTENT(IN), OPTIONAL :: WALL_INDEX,PARTICLE_INDEX
INTEGER  :: IIG,JJG,KKG,IOR,GEOMETRY
REAL(EB) :: U2=0._EB,V2=0._EB,W2=0._EB,H_NATURAL,H_FORCED,NUSSELT,ZZ_GET(0:N_TRACKED_SPECIES),KP,RDN
...
IF (PRESENT(PARTICLE_INDEX)) THEN
   IIG = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%IIG
   JJG = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%JJG
   KKG = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%KKG
   U2 = 0.25_EB*(UU(IIG,JJG,KKG)+UU(IIG-1,JJG,KKG)-2._EB*LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%U)**2
   V2 = 0.25_EB*(VV(IIG,JJG,KKG)+VV(IIG,JJG-1,KKG)-2._EB*LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%U)**2
   W2 = 0.25_EB*(WW(IIG,JJG,KKG)+WW(IIG,JJG,KKG-1)-2._EB*LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%U)**2   
   IOR = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%IOR
ELSEIF (PRESENT(WALL_INDEX)) THEN
   IF (WALL_INDEX ==-1) THEN !Void
      IIG = -1 
      IOR = 0
   ELSE   
      IIG = WALL(WALL_INDEX)%IIG
      JJG = WALL(WALL_INDEX)%JJG
      KKG = WALL(WALL_INDEX)%KKG
      IOR = WALL(WALL_INDEX)%IOR
...
IF (DNS) THEN
   IF (PRESENT(PARTICLE_INDEX)) THEN
      ...
      ZZ_GET = 0._EB
      IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)        
      CALL GET_CONDUCTIVITY(ZZ_GET,KP,TMP(IIG,JJG,KKG)) 
      HEAT_TRANSFER_COEFFICIENT = 2._EB*KP*WALL(WALL_INDEX)%RDN
   ELSEIF (PRESENT(WALL_INDEX)) THEN
      HEAT_TRANSFER_COEFFICIENT = 2._EB*WALL(WALL_INDEX)%KW*WALL(WALL_INDEX)%RDN
   ENDIF
   RETURN
ENDIF
