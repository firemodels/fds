...
IF (N_SPECIES > 0) THEN
   ...
   CALL GET_MU(YY_GET,MU_N,NINT(0.1_EB*TMPA))
ELSE
   MU_N = Y2MU_C(NINT(0.1_EB*TMPA))
ENDIF
...
IF (PRESSURE_CORRECTION .OR. EMBEDDED_MESH) THEN
      ...
      SELECT_POISSON_SOLVER: SELECT CASE(M%IPS)
         ...
         CASE (0:1) SELECT_POISSON_SOLVER
            IF (.NOT.TWO_D) THEN
               ...
               IF (EMBEDDED_MESH)       CALL H3CZIS(XS,XF,IBAR,  3,YS,YF,JBAR,  3,ZS,ZF,KBAR,  3,M%HX,XLM,ITRN,JTRN,IERR,M%SAVE2)
            ENDIF
...
HRR_COUNT=0._EB
ALLOCATE(MINT(0:N_Y_ARRAY,NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','MINT',IZERO) 
ALLOCATE(MINT_SUM(0:N_Y_ARRAY,NMESHES),STAT=IZERO)
