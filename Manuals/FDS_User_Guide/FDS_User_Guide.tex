...
\label{info:DISPERSION}

For {\ct MASSLESS} tracer particles it is possible to account for the dispersion due to unresolved turbulent fluctuations.  Set {\ct TURBULENT\_DISPERSION=.TRUE.} on the {\ct PART} line.  The tracer particles will undergo a random walk based on the local turbulent diffusivity (based on the viscosity and the Schmidt number, $D = \mu/\mbox{Sc}$).
...
{\ct TURBULENT\_DISPERSION}         & Logical         & Section~\ref{info:DISPERSION}           &           & .FALSE.       \\ \hline
