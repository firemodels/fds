...
\Huge \flushright{Fire Dynamics Simulator (Version 5) \\ Technical Reference Guide \\ (Draft: July 13, 2007)}
...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~1018-5, 80 pages (May 2007) \\
CODEN: NSPUE2 }
...
conceived by Gunnar Heskestad of Factory Mutual. Jason Sutula, Steve
...
%\markboth{FDS Technical Reference Guide}{March 26, 2007 (Draft)}
...
The present version of FDS is 5, first released in March 2007. Changes in
...
There are numerous sources that describe various parts of the model. The basic set of equations solved in
FDS was formulated by Rehm and Baum in the {\em Journal of Research of the National Bureau of Standards}~\cite{Rehm:1}.
The basic hydrodynamic algorithm evolved at NIST through the 1980s and 1990s, incorporating fairly well-known numerical
schemes that are documented in books by
Anderson, Tannehill and Pletcher~\cite{Anderson:1}, Peyret and Taylor~\cite{Peyret:1}, and
Ferziger and Peri\'{c}~\cite{Ferziger:1}. This last book provides a good description of the large eddy
simulation technique and provides references to many current publications on the subject.
Numerical techniques appropriate for combustion systems are described by Oran and Boris~\cite{Oran:1}.
The mixture fraction combustion model is described in a review article by Bilger~\cite{Bilger:AnnRev}. Basic heat transfer
theory is provided by Holman~\cite{Holman:1} and Incropera~\cite{Incropera:1}. Thermal radiation is
described in Siegel and Howell~\cite{Siegel:1}.
...
\be \dod{\rho}{t} + \nabla \cdot \rho \bu  =  0  \label{mass} \ee
...
\be \dod{ }{t}(\rho h) + \nabla \cdot \rho h \bu = \frac{Dp}{Dt}  + \dq'''
...
The force term $\bof$ in the momentum equation represents external forces such as
...
$\dq'''$ is the heat release rate per unit volume from a chemical reaction. The term
...
where
\begin{eqnarray*}
\cal{D}          &=& \frac{\R}{\bW c_p \bp_m} \; \Big( \dq''' - \nabla \cdot \dbq'' \Big) - {\cal P} \, w \rho_m g  \quad + \nonumber \\
                 & & \frac{\bW}{\rho} \sum_\alpha \nabla \cdot \rho D_\alpha \nabla (Y_\alpha/W_\alpha) -
\frac{\R}{\bW c_p \bp_m} \; \sum_\alpha h_\alpha \nabla \cdot \rho D_\alpha \nabla Y_\alpha +
 \frac{1}{\rho} \sum_\alpha \left( \frac{\bW}{W_\alpha} - \frac{h_\alpha}{c_p T} \right) \;
\dm_\alpha''' \\
\cal{P}          &=& \frac{1}{\bp_m} \; \left( \frac{\R}{\bW c_p} - 1 \right)
\end{eqnarray*}
Contributions to the divergence of the flow include the heat release rate of the fire, $\dq'''$, the net heat flux from thermal conduction and
...
sources of vorticity in the absence of any external forces, $\bof$:
\be \frac{d\Gamma}{dt} =
...
\be \rho \, \DoD{\bu}{t} \cdot \bu = \rho  \, \DoD{\left(|\bu|^2/2\right)}{t} = \rho \bof \cdot \bu -
...
Start with a simple combustion reaction:
\be  \hbox{Fuel} +  \mathrm{O_2}  \rightarrow  \hbox{Products} \label{stoich} \ee
The mixture fraction $Z$ can be defined in terms of the mass fraction of fuel (C$_x$H$_y$O$_z$)
and the carbon-carrying products of combustion:
\be Z = Y_\F + \frac{W_\F}{x W_\COTWO} \, Y_\COTWO + \frac{W_\F}{x W_\CO} \, Y_\CO  + \frac{W_\F}{x W_\C} \, Y_\C
\label{Zdef} \ee
The mixture fraction satisfies the conservation equation
\be \rho \DoD{Z}{t} = \nabla \cdot \rho D \nabla Z \label{Zeqn} \ee
obtained from a linear combination of the conservation equations for fuel and the carbon carriers.

If it is assumed that combustion occurs so rapidly that the fuel and oxygen cannot
co-exist, both simultaneously vanish at a flame surface:
...
The physical limitation of the single-step reaction is that it assumes fuel and
...
concentration for which burning can and cannot take place. A derivation of the model, based on the critical flame temperature concept, is given in Appendix~\ref{mowrer_model}.
\begin{figure}[ht]
\begin{minipage}[t]{4.1in}
...
where fuel and oxygen react and form products. Note that the term ``complete'' does not imply that
no soot or CO is formed, but rather that their respective production rates are proportional to the fuel consumption rate.
\begin{eqnarray}  \hbox{Reaction 1:} \quad \quad \hbox{Fuel} + \mathrm{O_2}  &\rightarrow& \hbox{Fuel} +  \mathrm{O_2} \nonumber \\
                  \hbox{Reaction 2:} \quad \quad \hbox{Fuel} + \mathrm{O_2}  &\rightarrow& \hbox{Products} \label{2reac} \end{eqnarray}
For each reaction, a set of state relations exist. Those for Reaction 2 are the same as for the single-step mixture fraction model
described above. Those for Reaction 1 are linear functions describing the mixing of fuel, oxygen and nitrogen with no combustion.
...
\begin{eqnarray}  Z_1 &=& Y_\F    \\*[.1in]
                  Z_2 &=& \frac{W_\F}{x \, W_\COTWO } \,  Y_\COTWO  + \frac{W_\F}{x \, W_\CO } \, Y_\CO  + \frac{W_\F}{x \, W_\C } \, Y_\C
\end{eqnarray}
...
oxygen co-exist, a reaction occurs if conditions are favorable. If a reaction occurs, $Z_1$ is converted to $Z_2$ representing the
...
The mass fraction of any species in the mixture is found via a
linear combination of its state relations for Reaction 1 and Reaction 2. For example, let $Y_{\alpha,1}(Z)$
represent the mass fraction of species $\alpha$ assuming that only the null reaction occurs. Likewise, $Y_{\alpha,2}(Z)$ is the mass fraction of
species $\alpha$ assuming that only the complete reaction occurs.  For a mixture of products, unburned fuel, and unconsumed oxygen, the mass
fraction of species $\alpha$ is
\be  Y_{\alpha}(Z) = (1-c) \, Y_{\alpha,1}(Z) + c \, Y_{\alpha,2}(Z) \label{1step} \ee
The variable $c$ is often referred to as a ``progress variable.'' It is a number between 0 and 1 that indicates the extent to which
the gas mixture is composed of products of the null or complete reaction. If $c=1$, then none of the gases are the result of a
local extinction. The value of $c$ is determined locally from the values of $Z_1$ and $Z_2$.
\be c = \frac{Z-Z_1}{Z-Y_{\F,2}(Z)} = \frac{Z_2}{Z-Y_{\F,2}(Z)}   \ee
The function $Y_{\F,2}(Z)$ is the fuel state relation for complete combustion.

Below is a pictorial rendering of Eq.~(\ref{1step}), demonstrating the linear combination of state relations.
\begin{tabular*}{\textwidth}[t]{lccccr}
\includegraphics[height=1.8in]{FIGURES/State_Relations_1+3_Full}
\raisebox{0.7in}[0pt]{=} &
\raisebox{0.7in}[0pt]{$(1-c)$} &
\includegraphics[height=1.8in]{FIGURES/State_Relations_1_Full} &
\raisebox{0.7in}[0pt]{+ $\quad c$} &
\includegraphics[height=1.8in]{FIGURES/State_Relations_3_Full}
 \end{tabular*}
It is important to note that the definitions of $Z_1$ and $Z_2$, unlike the single parameter model,
...
However, in under-ventilated fires, soot and CO are produced at higher rates, and exist within the fuel-rich flame envelope at higher concentrations,
...
\begin{eqnarray}  \hbox{Reaction 1:} \quad \quad \hbox{Fuel} + \mathrm{O_2}  &\rightarrow& \hbox{Fuel} +  \mathrm{O_2}  \\
                  \hbox{Reaction 2:} \quad \quad \hbox{Fuel} + \mathrm{O_2}  &\rightarrow& \mathrm{CO} + \hbox{Other Products}  \\
                  \hbox{Reaction 3:} \quad \quad \hbox{Fuel} + \mathrm{O_2}  &\rightarrow& \mathrm{CO_2} + \hbox{Other Products}
                  \label{3reac} \end{eqnarray}
These reactions shall be referred to as the ``null,'' ``incomplete,'' and ``complete'' reactions, respectively.
To describe the state of the system, the mixture fraction, $Z$, must be written as the sum of three components:
\begin{eqnarray}  Z_1 &=& Y_\F    \\*[.1in]
                  Z_2 &=& \frac{W_\F}{x \, W_\CO }    \;  Y_\CO \\*[.1in]
                  Z_3 &=& \frac{W_\F}{x \, W_\COTWO } \;  Y_\COTWO  \end{eqnarray}
Here, $x$ is the number of carbon atoms in the fuel molecule that are oxidized (i.e. $x$ does not include carbon that becomes soot).

The decomposition of $Z$ into three components is numerically convenient. However, the recovery of the individual species mass fractions requires some care. As a first step,
define two progress variables. The first, $c_\F$, indicates the extent to which the fuel has reacted with oxygen, and the second, $c_\CO$,
indicates the extent to which CO has been converted to CO$_2$.  To derive formulae for the progress variables, we first must define the local stoichiometric
value of the mixture fraction. The ``incomplete'' and ``complete'' reactions have different stoichiometric values of $Z$. Depending on the local composition of
the gases, the stoichiometric value falls between those of the ``incomplete'' and ``complete'' reactions:
\be  Z_f   = \left( 1+\frac{W_\OTWO}{W_\F}\frac{1}{Y_\OTWO^\infty} \, \Big( (1-c)\, \nu_{O_2,2}+c \, \nu_{O_2,3} \Big) \right)^{-1}   \quad ; \quad  c = \frac {Z_3}{Z_2+Z_3}  \ee
The state relation for fuel depends on the local stoichiometric value:
\be  Y_\F(Z) = \left\{ \begin{array}{l@{\quad}l}  0                    & Z \le Z_f  \\
                                                  \frac{Z-Z_f}{1-Z_f}    & Z >   Z_f  \end{array} \right.  \ee
Now the progress variables are written:
\begin{eqnarray}  c_\F   &=& \frac{Z-Z_1}{Z-Y_{\F}(Z)}    \\ \vspace{0.1in}
                  c_\CO  &=& \frac{1}{c_\F} \, \frac{Z_3}{Z_{3,max}(Z)}   \quad ; \quad Z_{3,\max}(Z)  = \frac{W_\F}{x \, W_\COTWO } Y_{\COTWO,3}  \end{eqnarray}

With the definitions of the progress variables in hand, we are ready to define the state relations. As before, the state relations are essentially linear
combinations of those of the individual reactions.
The ``null'' reaction, and its corresponding progress variable, $c_\F$, is treated as before. However, the second and third reaction
require some care. Consider a combination of products of the ``incomplete'' and ``complete'' reactions. The state relations for this
mixture can be found via a linear combination of the state relations:

\vspace{0.1in}
\noindent
\begin{tabular*}{\textwidth}[t]{lccccr}
\includegraphics[height=1.75in]{FIGURES/State_Relations_2+3} &
\raisebox{0.75in}[0pt]{=} &
\raisebox{0.75in}[0pt]{$(1-c_\CO)$} &
\includegraphics[height=1.75in]{FIGURES/State_Relations_2} &
\raisebox{0.75in}[0pt]{+ $c_\CO$} &
\includegraphics[height=1.75in]{FIGURES/State_Relations_3}
 \end{tabular*}

\vspace{0.1in}
\noindent
The state relations on the left represent a reaction of fuel and oxygen that produce only CO and H$_2$O (soot is neglected here,
not only because methane is the fuel chosen for this example, but also to simplify the discussion). The state relations in the
center represent the complete conversion of fuel and oxygen to CO$_2$ and H$_2$O. The state relations on the right represent a
linear combination of the two (with $c_\CO=0.5$). Note that there is a small overlap of the fuel and oxygen lines between the
stoichiometric values of each respective reaction. It is assumed that if only Reactions 2 and 3 are being considered, there should
be no overlap of fuel and oxgyen. Thus, a correction needs to be made to consume excess oxygen on the fuel-rich side of
the flame ($Z>Z_f$) or to consume excess fuel on the fuel-lean side ($Z<Z_f$), and adjust other products accordingly.
The stoichiometric value of the gas mixture, $Z_f$, falls between the values
for the individual reactions, $Z_{f,2}$ and $Z_{f,3}$, roughly (but not exactly) corresponding to the progress variable
$c_\CO$.

The state relations for the three-reaction system are derived as before for the two-reaction system. Pictorially:

\vspace{0.1in}
\noindent
\begin{tabular*}{\textwidth}[t]{lccccr}
\includegraphics[height=1.75in]{FIGURES/State_Relations_1+2+3} &
\raisebox{0.75in}[0pt]{=} &
\raisebox{0.75in}[0pt]{$(1-c_\F)$} &
\includegraphics[height=1.75in]{FIGURES/State_Relations_1} &
\raisebox{0.75in}[0pt]{+ $\quad c_\F$} &
\includegraphics[height=1.75in]{FIGURES/State_Relations_2+3_Corrected}
\end{tabular*}

\vspace{0.1in}
\noindent
Notice that now fuel and oxygen mix, because Reaction 1 is nothing more than the mixing of fuel and oxgyen without burning.
A summary of the derivation is:
\be  Y_{\alpha}(Z) = (1-c_\F) \, Y_{\alpha,1}(Z) + c_\F \; \left[ (1-c_\CO) \, Y'_{\alpha,2}(Z) + c_\CO \, Y'_{\alpha,3}(Z) \right] \ee
Note that the state relations for Reaction 2 and Reaction 3 require a slight modification:
\be \begin{array}{ll}
     Y'_{\alpha,2}(Z) = Y_{\alpha,2}(Z) \pm (\nu_\alpha W_\alpha/\nu_\OTWO W_\OTWO) \, Y_{\OTWO,2}(Z) \quad & \quad \hbox{if} \quad Z>Z_f  \\*[.1in]
     Y'_{\alpha,3}(Z) = Y_{\alpha,3}(Z) \pm (\nu_\alpha W_\alpha/\nu_\F W_\F)       \, Y_{\F,3}(Z)    \quad & \quad \hbox{if} \quad Z<Z_f  \end{array} \ee
...
equation is written
\be
  \overline{\rho c} \; \dod{T_s}{t} = \frac{1}{r} \, \dod{}{r}
  \left(rk_s \dod{T_s}{r} \right)+\dq_s'''
  \label{1dheatcyl} \quad ; \quad
  \overline{\rho c} \; \dod{T_s}{t} = \frac{1}{r^2} \, \dod{}{r}
...
\be
  \overline{\rho c} \; \dod{T_s}{t} = \dod{}{x} k_s \dod{T_s}{x} +
    \dq_s'''
  \label{1dheat}
\ee
Section~\ref{matcoefs} describes the component-averaged material properties, $k_s$ and
$\overline{\rho c}$.
The source term, $\dq_s'''$, consists of chemical reactions and radiative absorption:
\be
  \dq_s'''=\dq_{s,c}'''+\dq_{s,r}'''
\ee
Section~\ref{pyrosection} describes the term $\dq_{s,c}'''$, which is essentially the pyrolysis models
for different types of solid and liquid fuels.
Section~\ref{inradsection} describes the term $\dq_{s,r}'''$, the
radiative absorption and emission in depth.
...
radiative and convective heat fluxes are applied as
boundary conditions for Eq.~(\ref{1dheat}).
...
The conductivity and thermal inertia of the solid are defined
\be
   k_s = \sum_{\alpha=1}^{N_m} X_\alpha \; k_{s,\alpha} \quad ; \quad
   \overline{\rho c} = \sum_{\alpha=1}^{N_m} \rho_{s,\alpha} \; c_{s,\alpha}
\ee
$N_m$ is the number of material components forming the solid. $\rho_{s,\alpha}$ is the
...
This section describes how the chemical source term in the solid phase heat conduction equation, $\dot{q}_{s,c}'''$,
is modeled. This is what is commonly referred to as the ``pyrolysis model,'' but it actually can represent any number of
reactive processes, including evaporation, charring, and internal heating.


\subsubsection{Specified Heat Release Rate}

Often the intent of a fire simulation is merely to predict the transport of smoke and heat from a {\em specified} fire. In other
words, the heat release rate is a user input, not something the model predicts. In these instances, the desired HRR is translated into
a mass flux for fuel at a given solid surface, which can be thought of as the surface of a burner:
\be \dm_f'' = \frac{ f(t) \; \dq_{\hbox{user}}''}{\Delta H}  \ee
Usually, the user specifies a desired heat release rate per unit area,
$\dq_{\hbox{user}}''$, plus a time ramp, $f(t)$, and the mass
loss rate is computed accordingly.
...
because the volume fractions defined by Eq.~(\ref{volfrac}) need not sum to unity, in which case
the thermal conductivity and absorption coefficient are effectively reduced.}
\end{itemize}
...
These coefficients should satisfy $\nu_s + \nu_f + \nu_w = 1$,
but smaller yields may also be
used to take into account the gaseous products that are not explicitly included in the simulation.
...
\ee
where $r_{\alpha \beta}$ is the rate of reaction $\beta$ in units
...
r_{\alpha \beta} =
    ...
    \; \max \big[0,T_s-T_{ign,\alpha \beta} \big]^{n_{t,\alpha\beta}}
\ee
where $T_{ign,\alpha \beta}$ is an effective ``ignition temperature''
that can be used to dictate that the reaction not occur below a
...
\be
   \dm_f'' = \int \dm_f''' \, dx  \quad ; \quad \dm_w'' = \int\dm_w'''
   \, dx
\ee
The chemical source term of the heat conduction equation consists of the
heats of reaction and the enthalpy changes
\be
\dot{q}_{s,c}''' = \rho_{s0}\;
    \sum_{\alpha=1}^{N_m} \sum_{\beta=1}^{N_{r,\alpha}}
    r_{\alpha \beta} \;
    \left[H_{v,\alpha \beta}-
    \int_{T_0}^T \left(c_{s,\alpha}-\nu_{s,\alpha \beta} \;
    c_{s,\alpha \beta}
        - \nu_{f,\alpha \beta} \; c_{f}
        - \nu_{w,\alpha \beta} \; c_{w}\right) dT \right]
\ee
where $c_{s,\alpha \beta}$ is the specific heat of the residue of the
...
each material and tabulated between $T_0$ and $T_0 \;+\;2000$~K.
During the pyrolysis calculation, integral values are found from a ``look-up'' table.
...
user to pick these various parameters, and in so doing, pick whichever model the user feels is appropriate.
...
For a spray, the force term $\bof$ in Eq.~(\ref{momentum})
represents the momentum transferred from the water droplets to the gas.
It is obtained by summing the force transferred from each droplet
in a grid cell and dividing by the cell volume
\be {\bof} = \ha \frac{\sum \rho C_D \pi  r_d^2 (\bu_d-\bu) |\bu_d-\bu|}
    {\dx \, \dy \, \dz} \ee
...
\subsection{Mass and Energy Transfer from Droplets}

The evaporation of liquid droplets is handled semi-empirically.
Over the course of a time step of the gas phase solver, the droplets in a
given grid cell evaporate as a function of
the droplet equilibrium vapor mass fraction, $Y_d$,
the local gas phase vapor mass fraction, $Y_g$, the (assumed uniform) droplet temperature, $T_d$,
the local gas temperature, $T_g$,
and the droplet velocity relative to the local gas velocity.
The mass and energy transfer between the gas and the droplets can be described by the
following set of equations~\cite{Cheremisinoff:1}
\begin{eqnarray}
\frac{dm_d}{dt} & = & -2 \pi \; r_d \; \SH \; \rho D \; (Y_d - Y_g) \\
\frac{dT_d}{dt} & = & \frac{1}{m_d \; c_w}
     \left( A_d \, h_d \, (T_g-T_d) -  \frac{dm_d}{dt} \; h_v \right) \end{eqnarray}
The subscripts $d$ and $g$ refer to the droplet and gas,
respectively, $m_d$ is the droplet mass, $r_d$ is the droplet radius,
$\rho$ is the gas density, $D$ is the diffusion coefficient for water vapor into air,
and $\SH$ is the droplet Sherwood number, given by
a correlation involving the Reynolds and Schmidt numbers
\be \SH = 2 + 0.6 \; \RE_d^\ha \; \SC^\ot \ee
The Schmidt number is assumed to have a constant value of 0.6.
The vapor mass fraction of the gas, $Y_g$, is obtained from the overall
set of gas phase mass conservation equations, and the droplet equilibrium vapor mass fraction
is obtained from the Clausius-Clapeyron equation
\be X_d = \exp \left[ \frac{h_v \, W_w}{\cal R}
      \left( \frac{1}{T_b}-\frac{1}{T_d} \right) \right]  \quad ; \quad
      Y_d = \frac{X_d}{X_d (1-W_a/W_w) + W_a/W_w}  \ee
where $X_d$ is the droplet water vapor volume fraction,
$h_v$ is the heat of vaporization, $W_w$ is the molecular weight
of water, $W_a$ is the molecular weight of air,
$\cal R$ is the gas constant, $T_b$ is the boiling temperature
of the liquid and $T_d$ is the droplet temperature.
In the energy transfer equation, $c_w$ is the specific heat of the liquid,
$A_d=4\pi r_d^2$ is the surface
area of the droplet, $h_d$ is the heat transfer coefficient, given by
\be h_d = \frac{\NU \; k}{2 r_d} \quad ; \quad
    \NU = 2 + 0.6\; \RE_d^\ha \; \PR^\ot \ee
$\NU$ is the Nusselt number, $k$ is the thermal conductivity of air,
and the Prandtl number, $\PR$, is approximately 0.7 for air. The Sherwood number,
$\SH$, is analogous to the Nusselt number.

Finally, the exchange of mass and energy between the droplets and the
gas results in an additional term that must be added to the expression
for the divergence, Eq.~(\ref{phi})
\be \nabla \cdot \bu = \quad ... \quad + \frac{\cal R}{\gamma \; \bp_m}
   \left( \rho \; \sum (Y_\alpha/W_\alpha) \;
   \dod{T}{t} + \frac{T}{W_\alpha} \dm_\alpha'''  \right)   \ee
where $\dm_\alpha'''$ is the liquid evaporation rate per unit volume. The
liquid droplets are assumed to occupy no volume, simplifying the
analysis. The change in gas temperature is obtained by summing droplets over a single grid cell of volume $V$:
\be
  \rho c_p \dod{T}{t} = \frac{1}{V} \sum \left( m_d c_w \frac{dT}{dt} + h_v \frac{dm_d}{dt} \right)
\ee
...
for water mist systems~\cite{Ravigururajan:1}.
Liquid droplets attenuate thermal radiation through a combination of
scattering and absorption~\cite{Tuntomo:1}.
The radiation-droplet interaction
must therefore be solved for both the accurate prediction of the
radiation field and for the droplet energy balance.
...
the same form as the droplet size distribution, but a different mean.

An accurate computation of the in-scattering integral on the right
hand side of Eq.~(\ref{RTEspray}) would be extremely time consuming. It is
here approximated by dividing the total $4\pi$ solid angle to a
``forward angle'' $\delta\Omega^l$
and ``ambient angle'' $\delta\Omega^*=4\pi - \delta\Omega^l$.
For compatibility with the FVM solver, $\delta\Omega^l$ is set
equal to the control angle given by the angular discretization.
However, it is assumed to be symmetric around the center of the control angle.
Within $\delta\Omega^l$ the intensity is $I_{\la}(\bx,\bs)$ and elsewhere it
is approximated as
\be
U^*(\bx,\la) = \frac{U(\bx,\la) - \delta\Omega^l \, I_{\la}(\bx,\bs)}{\delta\Omega^*}
\ee
where $U(\bx)$ is the total integrated intensity.
The in-scattering integral can now be written as
\be
\frac{\sigma_d(\bx,\la)}{4\pi}\int_{4\pi}\Phi(\bs,\bs') \; I_{\la}(\bx,\bs')
  \; d\Omega' =
\sigma_d(\bx,\la)\left[\chi_f \; I_{\la}(\bx,\bs) + (1-\chi_f)U^*(\bx)\right]
\ee
where $\chi_f = \chi_f(r,\la)$ is a fraction of the
total intensity originally within the solid
angle $\delta\Omega^l$ that is scattered into the same
angle $\delta\Omega^l$.
...
\ee
This equation can be integrated over the spectrum to get the
band specific RTE's. The
procedure is exactly the same as that used for the gas phase RTE.
After the band integrations, the spray RTE for band $n$ becomes
\be
\bs \cdot \nabla I_{n}(\bx,\bs) =
...
scattering phase function are
calculated using the MieV code developed by Wiscombe~\cite{Wiscombe}.
Both $\kappa_d$ and
$\overline{\sigma_d}$ are averaged over the possible droplet radii
and wavelength before the actual simulation.
A constant ``radiation'' temperature, $T_{rad}$, is used in the wavelength averaging.
$T_{rad}$ should be selected to
represent a typical radiating
flame temperature. A value of 1173~K is used by default.
The averaged quantities, now functions of the droplet
mean diameter only, are saved in one-dimensional arrays.
During the simulation, the local properties are
calculated as a table look-up using the local mean droplet diameter~\cite{Hale:1}.
Details of the computation are given in Section~\ref{dropnumericalmethod}.
...
$E$                       \> activation energy \\
$\bof$                    \> external force vector (excluding gravity) \\
...
$\nu_i$                   \> stoichiometric coefficient, species $i$ \\
$\rho$                    \> density \\
...
\chapter{Derivation of the Velocity Divergence \\ \normalsize  Randall McDermott, NIST Postdoctoral Fellow}
...
Let $\rho$ denote the fluid mass density; let $\mathbf{u} = [u,v,w]^T$ denote the fluid mass-average velocity; and let $\dot{m}_b^{\tripleprime}$ denote a bulk source of mass per unit volume (which may come from the evaporation of water droplets, for example). The continuity equation may be rearranged to yield the following divergence constraint on the velocity
\begin{equation}
\label{eqn_divconstraint1}
\Div \mathbf{u} = \frac{1}{\rho} \left( \dot{m}_b^{\tripleprime} -  \frac{\mbox{D} \rho}{\mbox{D} t} \right) \,\mbox{,}
\end{equation}
...
We consider the transport of $n_s$ species mass fractions $Y_\alpha$ for $\alpha = \{1,\ldots,n_s\}$, $n_s-1$ of which are independent.  The molecular weight of a given species is denoted $W_\alpha$ and the molecular weight of the mixture, $\overline{W}$, is given by
\begin{equation}
\label{eqn_mixmolewt}
\overline{W} = \left( \sum_{\alpha} \frac{Y_\alpha}{W_\alpha} \right)^{-1} \,\mbox{,}
\end{equation}
...
\overline{p}_i = \frac{\rho \mathcal{R} T}{\overline{W}} \,\mbox{,}
\end{equation}
...
\begin{equation}
\label{eqn_DEOS1}
\frac{\mbox{D} \overline{p}_i}{\mbox{D} t} = \rho \mathcal{R} T \frac{\mbox{D}}{\mbox{D} t}\left(\frac{1}{\overline{W}}\right) + \frac{\rho \mathcal{R}}{\overline{W}} \frac{\mbox{D} T}{\mbox{D} t} + \frac{\mathcal{R}T}{\overline{W}} \frac{\mbox{D} \rho}{\mbox{D} t} \,\mbox{,}
\end{equation}
which rearranges to
\begin{equation}
\label{eqn_DEOS2}
\frac{\mbox{D} \rho}{\mbox{D} t} = \frac{\overline{W}}{\mathcal{R}T} \frac{\mbox{D}\overline{p}_i}{\mbox{D} t} - \rho \overline{W} \frac{\mbox{D}}{\mbox{D} t}\left(\frac{1}{\overline{W}}\right) - \frac{\rho}{T} \frac{\mbox{D} T}{\mbox{D} t} \,\mbox{.}
\end{equation}
...
\frac{\partial \left(\rho Y_\alpha\right)}{\partial t} + \Div \left(\rho Y_\alpha \mathbf{u}\right)  = - \Div \mathbf{J}_{\alpha} + \dot{m}_\alpha^{\tripleprime} + \dot{m}_{b,\alpha}^{\tripleprime} \,\mbox{,}
\end{equation}
where $\mathbf{J}_{\alpha}$ is the diffusive mass flux vector for species $\alpha$ (relative to the mass-average velocity), $\dot{m}_\alpha^{\tripleprime}$ is the chemical mass production rate of $\alpha$ per unit volume [kg-$\alpha$ produced /(sec m$^3$)], and $\dot{m}_{b,\alpha}^{\tripleprime}$ is the bulk mass source of $\alpha$ per unit volume [kg-$\alpha$ introduced /(sec m$^3$)].  Note that
\begin{equation}
\label{eqn_bulksum}
\sum_\alpha \dot{m}_{b,\alpha}^{\tripleprime} = \dot{m}_b^{\tripleprime} \,\mbox{}
\end{equation}
and
\begin{equation}
\label{eqn_massconservation}
\sum_\alpha \dot{m}_\alpha^{\tripleprime} = 0 \,\mbox{.}
\end{equation}
Additionally, by construction, the $i$th component of the species diffusive fluxes sum to zero,
\begin{equation}
\label{eqn_sumdiffflux}
\sum_\alpha J_{\alpha,i} = 0 \,\mbox{.}
\end{equation}
Thus, as must be the case, summing (\ref{eqn_speciestransport}) over $\alpha$ yields the continuity equation.  We employ the binary form of Fick's law using mixture-averaged diffusivities $D_\alpha$ as a constitutive relation for the diffusive flux,
\begin{equation}
\label{eqn_fickslaw}
\mathbf{J}_{\alpha} = - \rho D_\alpha \nabla Y_\alpha \,\mbox{.}
\end{equation}
Note that summation is not implied over repeated suffixes.  At present it is convenient to leave the diffusive flux in a general form.
Hence, the substitution of (\ref{eqn_fickslaw}) into the divergence constraint is made later.
...
\begin{equationarray}{lcl}
\label{eqn_speciesexpansion}
\rho\frac{\partial Y_\alpha}{\partial t} + Y_\alpha \frac{\partial \rho}{\partial t} + \rho \mathbf{u}\cdot \nabla Y_\alpha + Y_\alpha \Div \left(\rho \mathbf{u}\right) &=& - \Div \mathbf{J}_{\alpha} + \dot{m}_\alpha^{\tripleprime} + \dot{m}_{b,\alpha}^{\tripleprime} \,\mbox{,} \nonumber\vspace{0.3cm}\\
...
\frac{\mbox{D} Y_\alpha}{\mbox{D} t} &=& \frac{1}{\rho}\left(  \dot{m}_\alpha^{\tripleprime} + \dot{m}_{b,\alpha}^{\tripleprime} - Y_\alpha \dot{m}_b^{\tripleprime}  - \Div \mathbf{J}_{\alpha} \right) \,\mbox{,} \nonumber \vspace{0.3cm}\\
&=& \frac{1}{\rho}\left(  \dot{m}_\alpha^{\tripleprime} + \dot{m}_{b}^{\tripleprime}[Y_{b,\alpha} - Y_\alpha] - \Div \mathbf{J}_{\alpha} \right) \,\mbox{,}
\end{equationarray}
where in the second step we use the identity $\dot{m}_{b,\alpha}^{\tripleprime} = Y_\alpha \dot{m}_{b}^{\tripleprime}$ with $Y_{b,\alpha}$ being the mass fraction of $\alpha$ in the bulk prior to its introduction into the fluid mixture.
...
\rho \frac{\mbox{D} h_s }{\mbox{D} t} = -\sum_\alpha \Delta h^0_\alpha \dot{m}_{\alpha}^{\tripleprime} + \frac{\mbox{D} \overline{p}_i }{\mbox{D} t} - \Div \dot{\mathbf{q}}^{\prime\prime} + \dot{m}_{b}^{\tripleprime} ( h_{s,b} - h_s ) \,\mbox{.}
\end{equation}
where $\Delta h^0_\alpha$ is the heat of formation of $\alpha$ at reference temperature $T_0$, $h_{s,b}$ is the specific sensible enthalpy of the bulk mass source, and $\dot{\mathbf{q}}^{\prime\prime}$ is the heat flux vector which contains contributions from conduction, molecular diffusion, and radiation,
\begin{equation}
\label{eqn_heatflux}
\dot{\mathbf{q}}^{\prime\prime} = -k \nabla T + \sum_\alpha h_{s,\alpha} \mathbf{J}_{\alpha} + \dot{\mathbf{q}}^{\prime\prime}_{r} \,\mbox{.}
\end{equation}
...
\begin{equation}
\label{eqn_chainrule}
\frac{\mbox{D} h_s}{\mbox{D} t} = \left(\frac{\partial h_s}{\partial T}\right) \frac{\mbox{D} T }{\mbox{D} t} + \sum_\alpha \left( \frac{\partial h_s}{\partial Y_\alpha} \right) \frac{\mbox{D} Y_\alpha }{\mbox{D} t} \,\mbox{.}
\end{equation}
...
= \sum_\beta h_{\beta} \,\delta_{\alpha \beta} = h_{\alpha} \,\mbox{,}
\end{equation}
where $\delta_{\alpha \beta}$ is the Kronecker delta. Also,
\begin{equation}
\label{eqn_dhdT}
\frac{\partial h_s}{\partial T} = \frac{\partial}{\partial T} \sum_\alpha Y_\alpha h_{\alpha} = \sum_\alpha Y_\alpha \left(\frac{\partial h_{\alpha}}{\partial T}\right) = \sum_\alpha Y_\alpha c_{p,\alpha} \equiv c_p \,\mbox{,}
\end{equation}
...
\begin{equation}
\label{eqn_DTDt1}
\frac{\mbox{D} T}{\mbox{D} t} = \frac{1}{c_p} \left[ \frac{\mbox{D} h_s}{\mbox{D} t} - \sum_\alpha h_{\alpha} \frac{\mbox{D} Y_\alpha}{\mbox{D} t} \right] \,\mbox{.}
\end{equation}
Utilizing (\ref{eqn_matdermassfrac}) and (\ref{eqn_enthalpytransport}) in (\ref{eqn_DTDt1}) yields
\begin{equationarray}{lcrl}
\label{eqn_DTDt2}
\frac{\mbox{D} T}{\mbox{D} t} &=& \frac{1}{\rho c_p} & \left[ \Big\{ -\sum_\alpha \Delta h^0_\alpha \dot{m}_{\alpha}^{\tripleprime} + \frac{\mbox{D} \overline{p}_i }{\mbox{D} t} - \Div\dot{\mathbf{q}}^{\prime\prime} + \dot{m}_{b}^{\tripleprime} ( h_{s,b} - h_s ) \Big\}\right. \nonumber\vspace{0.3cm}\\
& & &- \left. \sum_\alpha h_{s,\alpha} \Big\{  \dot{m}_\alpha^{\tripleprime} + \dot{m}_{b}^{\tripleprime}[Y_{b,\alpha} - Y_\alpha] - \Div \mathbf{J}_{\alpha} \Big\} \right] \,\mbox{.}
\end{equationarray}
...
\begin{equationarray}{lcl}
\label{eqn_DTDt}
\frac{\mbox{D} T}{\mbox{D} t} &=& \frac{1}{\rho c_p} \left[ -\sum_\alpha \Delta h^0_\alpha \dot{m}_{\alpha}^{\tripleprime} + \frac{\mbox{D} \overline{p}_i }{\mbox{D} t} - \Div\dot{\mathbf{q}}^{\prime\prime}  - \sum_\alpha h_{\alpha} \Big\{  \dot{m}_\alpha^{\tripleprime} - \Div \mathbf{J}_{\alpha} \Big\} \right] \,\mbox{.}
\end{equationarray}
...
\Div\mathbf{u} &=& \frac{1}{\rho} \left( \dot{m}_b^{\tripleprime} -  \left[ \frac{\overline{W}}{\mathcal{R}T} \frac{\mbox{D}\overline{p}_i}{\mbox{D} t} - \rho \overline{W} \frac{\mbox{D}}{\mbox{D} t}\left(\frac{1}{\overline{W}}\right) - \frac{\rho}{T} \frac{\mbox{D} T}{\mbox{D} t} \right] \right) \,\mbox{,} \nonumber \vspace{0.3cm} \\
&=& \frac{1}{\rho} \,\dot{m}_b^{\tripleprime} -  \frac{1}{\overline{p}_i} \frac{\mbox{D}\overline{p}_i}{\mbox{D} t} + \overline{W} \frac{\mbox{D}}{\mbox{D} t}\left(\frac{1}{\overline{W}}\right) + \frac{1}{T} \frac{\mbox{D} T}{\mbox{D} t} \,\mbox{,}
\end{equationarray}
...
&+&  \frac{1}{T} \left[ \frac{1}{\rho c_p} \left\{-\sum_\alpha \Delta h^0_\alpha \dot{m}_{\alpha}^{\tripleprime} + \frac{\mbox{D} \overline{p}_i }{\mbox{D} t} - \Div\dot{\mathbf{q}}^{\prime\prime} - \sum_\alpha h_{\alpha} \Big(  \dot{m}_\alpha^{\tripleprime} - \Div \mathbf{J}_{\alpha} \Big) \right\} \right] \,\mbox{.}
\end{equationarray}

Note that $\overline{W} \sum_\alpha (Y_\alpha/W_\alpha) = 1$ and also $\overline{W} \sum_\alpha (Y_{b,\alpha}/W_\alpha) = \overline{W}/\overline{W}_b$, where $\overline{W}_b$ is the molecular weight of the bulk mixture prior to its introduction into the fluid mixture.  Equation (\ref{eqn_divconstraint3}) thus simplifies to
\begin{equationarray}{lcl}
...
&+& \frac{1}{\rho} \left[ \dot{m}_b^{\tripleprime} \frac{\overline{W}}{\overline{W}_b} +  \overline{W}\sum_\alpha \frac{1}{W_\alpha} \left\{  \dot{m}_\alpha^{\tripleprime} - \Div\mathbf{J}_{\alpha} \right\} \right] \nonumber \vspace{0.3cm}\\
&+&  \frac{1}{\rho c_p T} \left[ -\sum_\alpha \Delta h^0_\alpha \dot{m}_{\alpha}^{\tripleprime} - \Div \dot{\mathbf{q}}^{\prime\prime} - \sum_\alpha h_{\alpha} \left\{  \dot{m}_\alpha^{\tripleprime} - \Div \mathbf{J}_{\alpha} \right\} \right] \,\mbox{.}
\end{equationarray}
...
In addition to the substitution of Fick's law (\ref{eqn_fickslaw}) for the diffusive flux, the following relationships are used to rearrange (\ref{eqn_divconstraint_final}) into
...
\begin{equationarray}{lcl}
\label{eqn_fdsD}
\mathcal{D} &=& \frac{\dot{m}_b^\tripleprime}{\rho} \frac{\overline{W}}{\overline{W}_b} - \left( \frac{1}{\rho c_p T} - \frac{1}{\overline{p}_i} \right) w \rho_i g \nonumber \vspace{0.3cm}\\
&+& \frac{\overline{W}}{\rho} \sum_\alpha \frac{1}{W_\alpha} \left( \dot{m}_{\alpha}^\tripleprime + \Div \left[\rho D_\alpha \nabla Y_\alpha\right] \right) \nonumber \vspace{0.3cm}\\
&+& \frac{1}{\rho c_p T} \left[ \dot{q}^\tripleprime - \Div\dot{\mathbf{q}}^{\prime\prime} - \sum_\alpha h_{\alpha} \left( \dot{m}_{\alpha}^\tripleprime + \Div \left[\rho D_\alpha \nabla Y_\alpha \right] \right) \right] \,\mbox{,} \nonumber \vspace{0.3cm}\\
& & \nonumber \vspace{0.3cm}\\
&=& \frac{\dot{m}_b^\tripleprime}{\rho} \frac{\overline{W}}{\overline{W}_b} + \frac{\mathcal{R}}{\overline{W} c_p \overline{p}_i} \left( \dot{q}^\tripleprime - \Div \dot{\mathbf{q}}^{\prime\prime} \right) - \mathcal{P} w \rho_i g \nonumber \vspace{0.3cm}\\
&+& \frac{\overline{W}}{\rho} \sum_\alpha \Div \left( \rho D_\alpha \nabla[Y_\alpha/W_\alpha] \right) - \frac{\mathcal{R}}{\overline{W}c_p \overline{p}_i} \sum_\alpha h_{\alpha} \Div\left( \rho D_\alpha \nabla Y_\alpha \right) + \frac{1}{\rho} \sum_\alpha \left( \frac{\overline{W}}{W_\alpha} - \frac{h_{\alpha}}{c_p T} \right) \dot{m}_{\alpha}^\tripleprime \,\mbox{.} \nonumber \vspace{0.3cm}\\
\end{equationarray}




\chapter{A Simple Model of Fire Suppression \\ \vspace{0.2in} \normalsize Frederick W. Mowrer, Department of Fire Protection Engineering, University of Maryland}
...
by:
\be
   Y_{\OTWO,lim} = \frac{ \overline{c_p} (T_{f,lim}-T_m) }{\Delta H/r_\OTWO} \approx  \frac{ 1.1 \, (1700-T_m) }{ 13100}
\ee
