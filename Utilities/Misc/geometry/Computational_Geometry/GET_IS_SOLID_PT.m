function [IS_SOLID] = GET_IS_SOLID_PT(X1AXIS,X2AXIS,X3AXIS,XY,NVEC,X1PLN)
...
if(abs(NVEC(IAXIS)) > abs(NVEC(JAXIS))) % Do X2 ray
   ...
   DELBIN = BODINT_PLANE.TBAXIS(XAXIS).DELBIN;
   ILO_BIN= max(1,ceil( (XRAY-GEOMEPS-BODINT_PLANE.BOX(LOW_IND,XAXIS))/DELBIN ));
   IHI_BIN= min(BODINT_PLANE.TBAXIS(XAXIS).N_BINS, ...
                ceil( (XRAY+GEOMEPS-BODINT_PLANE.BOX(LOW_IND,XAXIS))/DELBIN ));
   for IBIN=ILO_BIN:IHI_BIN
      if (XRAY < BODINT_PLANE.TBAXIS(XAXIS).TRIBIN(IBIN).X_LOW-GEOMEPS); continue; end
      if (XRAY > BODINT_PLANE.TBAXIS(XAXIS).TRIBIN(IBIN).X_HIGH+GEOMEPS); continue; end
      for IISEG=1:BODINT_PLANE.TBAXIS(XAXIS).TRIBIN(IBIN).NTL
         ISEG = BODINT_PLANE.TBAXIS(XAXIS).TRIBIN(IBIN).TRI_LIST(IISEG);
...
%     for ISEG=1:BODINT_PLANE.NSEGS
         ...
         if ( (abs(DOT1)+abs(DOT2)) == 0. )
            ...
            [ierr2]=INSERT_RAY_CROSS(SVARI,ICRSI,SCRSI,STANI);
            for ICR=2:BODINT_PLANE.NBCROSS(ISEG)-1
                SVARI = X3_1 + BODINT_PLANE.SVAR(ICR,ISEG)*STANI(JAXIS);;
                ICRSI(LOW_IND:HIGH_IND+1) = [ IBM_SOLID, IBM_SOLID, IBM_SOLID];
                SCRSI = ISEG;
                % Insertion sort:
                [ierr2]=INSERT_RAY_CROSS(SVARI,ICRSI,SCRSI,STANI);
...
[ierr]=COLLAPSE_CROSSINGS(X1AXIS,X2AXIS,X3AXIS,XRAY,X1PLN,2);
