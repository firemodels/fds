...
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_600_kW_Middle_Surface_Temp} &
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_750_kW_Middle_Surface_Temp} &
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_900_kW_Middle_Surface_Temp} \\
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_600_kW_Middle-Right_Surface_Temp} &
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_750_kW_Middle-Right_Surface_Temp} &
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_900_kW_Middle-Right_Surface_Temp} \\
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_600_kW_Right_Surface_Temp} &
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_750_kW_Right_Surface_Temp} &
\includegraphics[width=2.1in]{SCRIPT_FIGURES/JIS_Facade/JIS_facade_900_kW_Right_Surface_Temp}
\end{tabular*}
\caption[JIS Facade surface temperatures, JIS A 1310 facade test]{JIS Facade surface temperatures, JIS A 1310 facade test. The top row displays vertical surface temperature profiles along the centerline of the compartment window. The middle row displays the temperatures 23~cm off-center. The bottom row displays the profiles 41~cm off-center.}
...
\end{tabular*}
\caption[NIST FSE experiments, floor and ceiling temperatures, Tests 3, 8]{NIST FSE experiments, floor and ceiling temperatures, Tests 3, 8.}
...
\subsection{SNL Walls}

A description of these experiments is found in Sec.~\ref{SNL_Walls_Description}. Figure~\ref{SNL_Walls_concrete} displays predicted and measured temperature profiles in the mid-section of a heated concrete wall. Figure~\ref{SNL_Walls_TC_locations} indicates the locations of thermocouples inserted into a masonry block. Figure~\ref{SNL_Walls_masonry} displays time histories of various surface and interior points of a masonry block wall. 

\begin{figure}[h]
\centering
\includegraphics[height=2.15in]{SCRIPT_FIGURES/SNL_Walls/concrete_wall}
\caption[SNL Walls, interior concrete wall temperature profiles]{SNL Walls, interior concrete wall temperature profiles.}
\label{SNL_Walls_concrete}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[height=2.15in]{FIGURES/SNL_Walls/TC_Locations}
\caption[SNL Walls, location of thermocouples in the masonry block]{Photograph taken from Ref.~\cite{Luketa:11078} showing the locations of thermocouples in the masonry block. Locations M-F1 through M-F5 are along the front face, M-M1 through M-M3 are along the flanges, and M-R1 through M-R5 are along the back face.}
\label{SNL_Walls_TC_locations}
\end{figure}


\begin{figure}[p]
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
\includegraphics[height=2.15in]{SCRIPT_FIGURES/SNL_Walls/masonry_wall_surface_temp} &
\includegraphics[height=2.15in]{SCRIPT_FIGURES/SNL_Walls/masonry_wall_temp_1} \\
\includegraphics[height=2.15in]{SCRIPT_FIGURES/SNL_Walls/masonry_wall_temp_2} &
\includegraphics[height=2.15in]{SCRIPT_FIGURES/SNL_Walls/masonry_wall_temp_3} \\
\includegraphics[height=2.15in]{SCRIPT_FIGURES/SNL_Walls/masonry_wall_temp_4} &
\includegraphics[height=2.15in]{SCRIPT_FIGURES/SNL_Walls/masonry_wall_temp_5}
\end{tabular*}
\caption[SNL Walls, surface and interior masonry temperatures]{SNL Walls, surface and interior masonry temperatures. The upper left plot displays the front and back surface temperature of the mid-wall masonry block. The other plots display the near front (M-FX), middle (M-MX), and rear (M-RX) thermocouple temperatures. The points labeled M-FX are located 16~mm (5/8~in) inside the front surface of the cinder block wall. The points labeled M-MX are located 10~cm (4~in) from the surface. The points labeled M-RX are located 16~mm (5/8~in) from the rear surface.}
\label{SNL_Walls_masonry}
\end{figure}
...
Figure~\ref{Cable_Failure_Time_Summary_Plot} compares the measured versus predicted time to a ``threshold'' temperature. The threshold temperature is 400~\unit{\degreeCelsius} for thermoset cables and 200~\unit{\degreeCelsius} for thermoplastics.
...
Figure~\ref{NIST_Methanol_1m_pan_TSURF} displays the mean surface temperature for the 1~m NIST methanol pool fire~\cite{Sung:TN2083} and the 30~cm Waterloo methanol pool fire~\cite{Weckman:CF1996}. The predicted surface temperatures are compared to the boiling temperature of methanol, 64.7~\unit{\degreeCelsius}.
