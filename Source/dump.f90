...
IF (T>=PART_CLOCK(M%PART_COUNTER) .AND. PARTICLE_FILE) THEN
   CALL DUMP_PART(T,NM)
   M%PART_COUNTER = M%PART_COUNTER + 1
ENDIF

IF (T>=ISOF_CLOCK(M%ISOF_COUNTER)) THEN
   CALL DUMP_ISOF(T,DT,NM)
   M%ISOF_COUNTER = M%ISOF_COUNTER + 1
ENDIF

IF (T>=SM3D_CLOCK(M%SM3D_COUNTER) .AND. SMOKE3D) THEN
   CALL DUMP_SMOKE3D(T,DT,NM)
   M%SM3D_COUNTER = M%SM3D_COUNTER + 1
ENDIF

IF (T>=SLCF_CLOCK(M%SLCF_COUNTER)) THEN
   CALL DUMP_SLCF(T,DT,NM,0)
   M%SLCF_COUNTER = M%SLCF_COUNTER + 1
ENDIF

IF (T>=SL3D_CLOCK(M%SL3D_COUNTER) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
   CALL DUMP_SLCF(T,DT,NM,2)
   M%SL3D_COUNTER = M%SL3D_COUNTER + 1
ENDIF

IF (T>=BNDF_CLOCK(M%BNDF_COUNTER)) THEN
   CALL DUMP_BNDF(T,DT,NM)
   M%BNDF_COUNTER = M%BNDF_COUNTER + 1
ENDIF

IF (T>=PL3D_CLOCK(M%PL3D_COUNTER) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
   CALL DUMP_SLCF(T,DT,NM,1)
   M%PL3D_COUNTER = M%PL3D_COUNTER + 1
ENDIF

IF (T>=PROF_CLOCK(M%PROF_COUNTER)) THEN
   CALL DUMP_PROF(T,NM)
   M%PROF_COUNTER = M%PROF_COUNTER + 1
ENDIF
...
WRITE(LU_CORE(NM)) T,ICYC,PART_COUNTER,RSRT_CLOCK,SLCF_COUNTER,SM3D_COUNTER,RADF_COUNTER, &
             PL3D_COUNTER,BNDF_COUNTER,GEOM_CLOCK,CTRL_COUNTER,DEVC_COUNTER,HRR_COUNTER,MASS_COUNTER, &
             ISOF_COUNTER,BC_CLOCK,WALL_COUNTER,DT, &
...
READ(LU_RESTART(NM)) T,ICYC,PART_COUNTER,RSRT_CLOCK,SLCF_COUNTER,SM3D_COUNTER,RADF_COUNTER, &
             PL3D_COUNTER,BNDF_COUNTER,GEOM_CLOCK,CTRL_COUNTER,DEVC_COUNTER,HRR_COUNTER,MASS_COUNTER, &
             ISOF_COUNTER,BC_CLOCK,WALL_COUNTER,DT, &
             PBAR,D_PBAR_DT,N_EDGES,RAD_CALL_COUNTER,ANGLE_INC_COUNTER,T_LAST_DUMP_HRR,T_LAST_DUMP_MASS, &
             RTE_SOURCE_CORRECTION_FACTOR,RAD_Q_SUM,KFST4_SUM,ENTHALPY_SUM(NM),&
