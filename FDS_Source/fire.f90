...
REAL(EB) :: ERR_EST,ERR_TOL,ZZ_TEMP(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),&
            ...
            Q_R_SUM(1:N_REACTIONS),CHI_R_SUM,TIME_RAMP_FACTOR
INTEGER :: NR,NS,ITER,TVI,RICH_ITER,TIME_ITER
...
IF (SUM(Q_R_SUM)>0._EB) THEN
   CHI_R_SUM=0._EB
   DO NR=1,N_REACTIONS
      RN=>REACTION(NR)
      TIME_RAMP_FACTOR = EVALUATE_RAMP(T,0._EB,RN%RAMP_CHI_R_INDEX)
      CHI_R_SUM = CHI_R_SUM + Q_R_SUM(NR)*RN%CHI_R*TIME_RAMP_FACTOR
   ENDDO
   CHI_R(I,J,K) = CHI_R_SUM/(SUM(Q_R_SUM))
ENDIF
...
IF (REAC_SOURCE_CHECK) THEN
   ...
   Q_REAC(I,J,K,:) = Q_REAC_TEMP/CELL_VOLUME/DT
ENDIF
...
SUBROUTINE FIRE_FORWARD_EULER(ZZ_OUT,ZETA_OUT,ZZ_IN,ZETA_IN,DT_LOC,TMP_MIXED,TMP_UNMIXED,RHO_HAT,ZZ_UNMIXED,CELL_MASS,TAU_MIX,&
                              PBAR_0,DELTA,VEL_RMS,EXTINCT,Q_REAC_OUT)
USE COMP_FUNCTIONS, ONLY:SHUTDOWN
...
REAL(EB), INTENT(OUT) :: ZZ_OUT(1:N_TRACKED_SPECIES),ZETA_OUT,Q_REAC_OUT(1:N_REACTIONS)
...
REAL(EB) :: ZZ_0(1:N_TRACKED_SPECIES),ZZ_NEW(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),UNMIXED_MASS_0(1:N_TRACKED_SPECIES),&
            ...
            CPBAR_UNMIXED,CPBAR_MIXED,CPBAR_MIXED_0,TMP_MIXED_0,TOTAL_MIXED_MASS_0,Q_REAC_TEMP(1:N_REACTIONS)
INTEGER :: SR,ITER
...
IF (ANY(REACTION(:)%FAST_CHEMISTRY)) THEN
   DO SR = 0,N_SERIES_REACTIONS
      ...
      ZZ_0 = ZZ_NEW
      Q_REAC_OUT = Q_REAC_OUT + Q_REAC_TEMP*BOUNDEDNESS_CORRECTION*TOTAL_MIXED_MASS
   ENDDO
ENDIF
...
REAL(EB), INTENT(OUT) :: DZZ(1:N_TRACKED_SPECIES),Q_REAC_TEMP(1:N_REACTIONS)
REAL(EB), INTENT(IN) :: ZZ_0(1:N_TRACKED_SPECIES),DT_LOC,RHO_0,TMP_0,PBAR_0,DELTA,VEL_RMS
INTEGER, INTENT(IN) :: KINETICS
LOGICAL, INTENT(IN) :: EXTINCT(1:N_REACTIONS)
REAL(EB) :: DZ_F,YY_PRIMITIVE(1:N_SPECIES),DG_RXN,MW,MOLPCM3,DTHETA
INTEGER :: I,NS
INTEGER, PARAMETER :: INFINITELY_FAST=1,FINITE_RATE=2
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
KINETICS_SELECT: SELECT CASE(KINETICS)
   ...
   CASE(INFINITELY_FAST)
      REACTION_LOOP_1: DO I=1,N_REACTIONS
         ...
         DZ_F = ZZ_0(RN%FUEL_SMIX_INDEX)*DTHETA
         Q_REAC_TEMP(I) = Q_REAC_TEMP(I) + RN%HEAT_OF_COMBUSTION*DZ_F
         DZZ = DZZ + RN%NU_MW_O_MW_F*DZ_F
   ...
   CASE(FINITE_RATE) 
      REACTION_LOOP_2: DO I=1,N_REACTIONS
         ...
         ENDDO
         DZ_F = RN%A_PRIME*RHO_0**RN%RHO_EXPONENT*TMP_0**RN%N_T*EXP(-RN%E/(R0*TMP_0)) ! FDS Tech Guide, Eq. (5.49)
         DO NS=1,N_SPECIES
