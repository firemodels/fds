SUBROUTINE FDS2LS_DYNA (CHID,NOMASTER,C_S,TBEG,TEND,TINT,COUNTER,VARIABLE,N,N_AVERAGE,BOUNDARY,CURVEID)
...
INTEGER :: IERR, NMESHES, NM, NOC, I, J, K, L, IOR_LOOP
...
LOOP_FDS2LS_DYNA:DO P=1,COUNTER
!CALL CPU_TIME (tb_fds2ast)
    ...
    PRINT *, OUTFILE
    C_SIZE=C_S
LOOP_LOCATION:DO L=1,2
    BOUNDARY(P*2+L-2,1)=P
    IF (L==1) THEN
      LOCATION=1
      NX=N(P,2)
      NY=N(P,3)
...
IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') THEN
  DO I=1,V-1  
    WRITE (70,'(ES10.3,A,ES10.3)') V_TIME(I), ",", (TAST(I)+273.15)
  ENDDO
ELSEIF (VARIABLE_KIND==' HEAT TRANSFER COEFFICIENT    ') THEN  
  DO I=1,V-1  
    WRITE (70,'(ES10.3,A,ES10.3)') V_TIME(I), ",", TAST(I)
  ENDDO
ENDIF
...
ELSE IF (T_AVERAGE==0) THEN
    ...
    IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') THEN    
      WRITE (70,'(ES10.3,A,ES10.3)') V_TIME(V)+TINT, ",", (MED+273.15)
      WRITE (70,'(ES10.3,A,ES10.3)') 18000.0, ",", (MED+273.15)
    ENDIF
    IF (VARIABLE_KIND==' HEAT TRANSFER COEFFICIENT    ') THEN
      WRITE (70,'(ES10.3,A,ES10.3)') V_TIME(V)+TINT, ",", MED
      WRITE (70,'(ES10.3,A,ES10.3)') 18000.0, ",", MED
    ENDIF
ELSE IF (T_AVERAGE.NE.0) THEN
    ...
    IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') THEN    
      WRITE (70,'(ES10.3,A,ES10.3)') V_TIME(V)+T_AVERAGE, ",", (MED+273.15)
      WRITE (70,'(ES10.3,A,ES10.3)') 18000.0, ",", (MED+273.15)      
    ENDIF
    IF (VARIABLE_KIND==' HEAT TRANSFER COEFFICIENT    ') THEN
      WRITE (70,'(ES10.3,A,ES10.3)') V_TIME(V)+T_AVERAGE, ",", (MED+273.15)
...
!
IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') THEN
  BOUNDARY(P*2+L-2,2)=CURVE 
ELSEIF (VARIABLE_KIND==' HEAT TRANSFER COEFFICIENT    ') THEN
  BOUNDARY(P*2+L-2,3)=CURVE
ENDIF 
CURVE=CURVE+1
!**********************
    IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') THEN
