...
DO
   DO N=1,N_MATL
      ML=>MATERIAL(N)
      ALLOCATE(ML%CHILD_MATL(N_MATL))
      ML%CHILD_MATL = 0
      ML%CHILD_MATL(N) = 1
      IF (ML%CHILD_MATL(N) > 0) THEN
         ML2 => MATERIAL(ML%CHILD_MATL(N))
         IF (ML2%N_REACTIONS > 0) THEN
            DO NR=1,ML2%N_REACTIONS
               DO NN = 1,ML2%N_RESIDUE(NR)
                  IF (ML2%NU_RESIDUE(NR,NN) > 0._EB .AND. ML%CHILD_MATL(ML2%RESIDUE_MATL_INDEX(NR,NN))==0) THEN
                     ML%CHILD_MATL(ML2%RESIDUE_MATL_INDEX(NR,NN)) = 1
                     ITERATE = .TRUE.
                  ENDIF
               ENDDO
            ENDDO
