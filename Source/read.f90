...
USE OUTPUT_DATA
CHARACTER(FORMULA_LENGTH) :: SPECFILE,SPECFILE_NM,TMPFILE,TMPFILE_NM,UVWFILE,UVWFILE_NM
LOGICAL :: PER_MESH
INTEGER :: NM
...
READ_CSVF_LOOP: DO I=1,N_CSVF
   ...
   IF (TRIM(TMPFILE)/='null' .AND. I>NMESHES) THEN
      CALL SHUTDOWN('Problem with CSVF line: TMPFILE must be in order with MESH.') ; RETURN
   ELSE
      CSVFINFO(I)%TMPFILE = TMPFILE
      TMP_RESTART = .TRUE.
   ENDIF

   IF (TRIM(SPECFILE)/='null' .AND. I>NMESHES) THEN
      CALL SHUTDOWN('Problem with CSVF line: SPECFILE must be in order with MESH.') ; RETURN
   ELSE
      CSVFINFO(I)%SPECFILE = SPECFILE
      SPEC_RESTART = .TRUE.
   ENDIF
...
IF (TRIM(TMPFILE)/='null' .AND. PER_MESH) THEN
   DO NM=1,NMESHES
      WRITE(TMPFILE_NM,'(A,I0,A)') TRIM(TMPFILE),NM,'.csv'
...
IF (TRIM(SPECFILE)/='null' .AND. PER_MESH) THEN
   DO NM=1,NMESHES
      WRITE(SPECFILE_NM,'(A,I0,A)') TRIM(SPECFILE),NM,'.csv'
