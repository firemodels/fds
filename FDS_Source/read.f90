...
LOGICAL :: SUCCESS,EX,THICKEN_OBSTRUCTIONS,BAD,SIMPLE_CHEMISTRY=.FALSE.,BACKGROUND_DECLARED=.FALSE.
   ...
   SELECT CASE(N)
      ...
      CASE(2)
         ...
         VOLUME_FRACTION(4) = RN%NU_CO2 + SPECIES_MIXTURE(0)%VOLUME_FRACTION(CO2_INDEX)*RN%NU_O2 / &
                                          SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)
         VOLUME_FRACTION(5) = RN%N      + SPECIES_MIXTURE(0)%VOLUME_FRACTION(N2_INDEX)*RN%NU_O2 / &
                                          SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)
...
REAL(EB) :: H2_YIELD,SOOT_YIELD,CO_YIELD,Y_F_LFL,X_O2_LL,EPUMO2,BOF, &
            ...
            AUTO_IGNITION_TEMPERATURE,THRESHOLD_TEMPERATURE,SOOT_H_FRACTION,HUMIDITY
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   ! Heat of Combustion calculation (IDEAL not hooked up yet)

   IF (RN%HEAT_OF_COMBUSTION<0._EB) THEN
      IF (RN%IDEAL) THEN
         RN%HEAT_OF_COMBUSTION = -RN%EPUMO2*RN%NU_SPECIES(O2_INDEX)*MW_O2/SPECIES(FUEL_INDEX)%MW
      ELSE
         RN%HEAT_OF_COMBUSTION = -RN%EPUMO2*RN%NU_SPECIES(O2_INDEX)*MW_O2/SPECIES(FUEL_INDEX)%MW
      ENDIF
   ENDIF
