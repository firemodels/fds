...
REAL(EB) :: DT_BC,T,TSI,TMP_G,RHO_G,DTMP,TMP_OTHER,RAMP_FACTOR,QNET,FDERIV,TMP_EXTERIOR,UN,ARO,UWO,QEXTRA,RSUM_W, &
            YY_G_ALL(MAX_SPECIES),RHO_YY_F(MAX_SPECIES),YY_GET(1:N_TRACKED_SPECIES),YY_EXTERIOR(MAX_SPECIES)
   ...
   METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
      ...
      CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER 
         ...
         IF (INFLOW) THEN
            ...
            IF (N_TRACKED_SPECIES>0) YY_EXTERIOR(1:N_TRACKED_SPECIES) = YY_AMBIENT(1:N_TRACKED_SPECIES)
            ...
            IF (N_TRACKED_SPECIES>0) YY_F(IW,1:N_TRACKED_SPECIES)=YY_EXTERIOR(1:N_TRACKED_SPECIES)
         ELSE
            TMP_F(IW) = TMP(IIG,JJG,KKG)
            IF (N_TRACKED_SPECIES>0) YY_F(IW,1:N_TRACKED_SPECIES) = YYP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
         ENDIF

         TMP(II,JJ,KK) = TMP_F(IW)
         IF (N_TRACKED_SPECIES>0) YYP(II,JJ,KK,1:N_TRACKED_SPECIES)=YY_F(IW,1:N_TRACKED_SPECIES)
      ...
      CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER
         ...
         IF (PREDICTOR) THEN
            OM_RHOP => OM%RHOS
            IF (N_TRACKED_SPECIES>0) OM_YYP => OM%YYS
         ELSE
            OM_RHOP => OM%RHO
            IF (N_TRACKED_SPECIES>0) OM_YYP => OM%YY
         ENDIF
         MM    => MESHES(NOM)
         RHO_G = RHOP(IIG,JJG,KKG)
         RHO_F(IW) = RHO_G  ! Initialize face value of RHO with RHO_G
         IF (N_TRACKED_SPECIES>0) THEN
            YY_G_ALL(1:N_TRACKED_SPECIES) = YYP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
            RHO_YY_F(1:N_TRACKED_SPECIES) = RHO_G*YY_G_ALL(1:N_TRACKED_SPECIES) ! Initialize face value of RHO_YY with RHO_G*YY_G
         ENDIF
         ...
         DO KKO=IJKW(12,IW),IJKW(15,IW)
            DO JJO=IJKW(11,IW),IJKW(14,IW)
               DO IIO=IJKW(10,IW),IJKW(13,IW)
                  ...
                  IF (N_TRACKED_SPECIES>0) RHO_YY_F(1:N_TRACKED_SPECIES) = RHO_YY_F(1:N_TRACKED_SPECIES) + &
                     0.5_EB*ARO*(OM_RHOP(IIO,JJO,KKO)*OM_YYP(IIO,JJO,KKO,1:N_TRACKED_SPECIES)-RHO_G*YY_G_ALL(1:N_TRACKED_SPECIES))
               ENDDO
            ENDDO
         ENDDO
         ...
         ELSE
            YY_F(IW,1:N_TRACKED_SPECIES)      = RHO_YY_F(1:N_TRACKED_SPECIES)/RHO_F(IW)
            YYP(II,JJ,KK,1:N_TRACKED_SPECIES) = 2._EB*YY_F(IW,1:N_TRACKED_SPECIES) - YY_G_ALL(1:N_TRACKED_SPECIES)
            YY_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,YYP(II,JJ,KK,1:N_TRACKED_SPECIES))
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_HEAT,GET_MASS_FRACTION_ALL,GET_AVERAGE_SPECIFIC_HEAT,&
                              GET_SPECIFIC_HEAT_BG,GET_AVERAGE_SPECIFIC_HEAT_BG
USE MATH_FUNCTIONS, ONLY : EVALUATE_RAMP
REAL(EB) :: T,YY_G,RHO_G,UN,DD,MFT,TSI,RADIUS,AREA_SCALING,&
            RVC,RHO_NEW,M_DOT_PPP,CP,CPBAR,MW_RATIO,H_G,DELTA_H_G,YY_GET(1:N_TRACKED_SPECIES),CPBAR2
INTEGER :: I,IBC,IIG,JJG,KKG,IW,II,JJ,KK,N,NS
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   METHOD_OF_MASS_TRANSFER: SELECT CASE(SF%SPECIES_BC_INDEX)
      ...
      CASE (NO_MASS_FLUX) METHOD_OF_MASS_TRANSFER 
         ...
         IF (.NOT.SOLID(CELL_INDEX(IIG,JJG,KKG))) YY_F(IW,1:N_TRACKED_SPECIES) = YYP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
         IF ( SF%LEAK_PATH(1)>-1 .AND. UWS(IW)<0._EB) THEN
            RHO_G = RHOP(IIG,JJG,KKG)
            UN    = -UWS(IW)
            SPECIES_LOOP_1: DO N=1,N_TRACKED_SPECIES
      ...
      CASE (SPECIFIED_MASS_FRACTION) METHOD_OF_MASS_TRANSFER
         ...
         IF (UWS(IW)<0._EB) THEN
            ...
            ENDDO
         ELSE
            YY_F(IW,1:N_TRACKED_SPECIES) = YYP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
         ENDIF
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. N_TRACKED_SPECIES > 0 .AND. &
      ...
      YYP(II,JJ,KK,1:N_TRACKED_SPECIES) = 2._EB*YY_F(IW,1:N_TRACKED_SPECIES) - YYP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
...
IF (VIRTUAL_PARTICLES .AND. CORRECTOR) THEN
   ...
   DROPLET_LOOP: DO I=1,NLP
      ...
      DO NS=0,N_TRACKED_SPECIES
         ...
         ELSE
            IF (N_TRACKED_SPECIES>0) YYP(II,JJ,KK,1:N_TRACKED_SPECIES) = RHO(II,JJ,KK)*YYP(II,JJ,KK,1:N_TRACKED_SPECIES)/RHO_NEW
         ENDIF
...
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT
REAL(EB) :: YY_GET(1:N_TRACKED_SPECIES),RSUM_F
...
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT
REAL(EB) :: YY_GET(1:N_TRACKED_SPECIES),YY_G(1:N_TRACKED_SPECIES),UN,MFT,DD,RSUM_F
REAL(EB) :: RHO_0,TMP_0,YY_0(1:N_TRACKED_SPECIES),UW_0,YY_ERR
INTEGER, INTENT(IN):: NM
...
IF (PREDICTOR) THEN 
   ...
   WW => W
   RHOP => RHOS
   IF (N_TRACKED_SPECIES > 0) YYP => YYS
   PBAR_P => PBAR_S
ELSE 
   UU => US
   VV => VS
   ...
   DO WHILE (ITER)
      ...
      ELSE SPECIES_IF
         ...
         ELSE
            ...
            IF (VENT_INDEX(IW)>0) THEN
               IF (VENTS(VENT_INDEX(IW))%NODE_INDEX > 0) THEN
                  MASSFLUX(IW,1:N_TRACKED_SPECIES) = -NODE_YY(DN,1:N_TRACKED_SPECIES,NM)*MFT
...
!REAL(EB) :: YY_S,RSUM_S
INTEGER :: IBC,IIG,JJG,KKG,IIB,JJB,KKB,IWB,NWP,I,J,NR,NN,NNN,NL,II,JJ,KK,IW,IOR,N,I_OBST,NS
REAL(EB) :: SMALLEST_CELL_SIZE(MAX_LAYERS),THICKNESS,YY_GET(1:N_TRACKED_SPECIES),ZZ(1:I_Z_MAX - I_Z_MIN + 1)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   SELECT CASE(SF%BACKING)
      ...
      CASE(EXPOSED)  
         ...
         IF (BOUNDARY_TYPE(IWB)==SOLID_BOUNDARY) THEN
            ...
            DTMP = TMP_BACK - TMP_B(IW)
            HTCB = HEAT_TRANSFER_COEFFICIENT(IWB,IIB,JJB,KKB,IOR,TMP_BACK,DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH)
   ...
   ELSE NEW_EVAP_IF !Old routine
      ...
      MATERIAL_LOOP2: DO N=1,SF%N_MATL
         ...
         IF (ML%NU_GAS(I_FUEL,1)>0._EB) MFLUX = MFLUX/ML%NU_GAS(I_FUEL,1)
         ! gas phase 
         ...
         IF ((ABS(MFLUX)<=ZERO_P) .AND. (PPSURF<PPCLAUS)) THEN         
            MFLUX = (ML%INIT_VAPOR_FLUX/(R0*TMPA/P_INF))*REACTION(1)%MW_FUEL/ML%ADJUST_BURN_RATE(I_FUEL,1)
         ENDIF
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_HEAT,GET_VISCOSITY,GET_SPECIFIC_HEAT_BG,GET_VISCOSITY_BG
...
INTEGER  :: IW,IIG,JJG,KKG,IOR,IHTMODEL,GEOMETRY
INTEGER, PARAMETER :: IHEDDY=1,IHLOGLAW=2,ICHILTON=3
REAL(EB) :: U2=0._EB,V2=0._EB,W2=0._EB,VELCON,H_NATURAL,H_FORCED,CP,YY_GET(1:N_TRACKED_SPECIES),NUSSELT,MU_G
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU=>NULL(),VV=>NULL(),WW=>NULL(),RHOP=>NULL()
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: YYP=>NULL()
...
NO_GAS_CELL_IF: IF (IIG>=0) THEN  
   ...
   IF (H_EDDY .OR. H_LOGLAW .OR. H_CHILTON_COLBURN) THEN
      ...
      ELSE
         CALL GET_VISCOSITY_BG(MU_G,TMP_G)
         NU = MU_G/RHOP(IIG,JJG,KKG)
         CALL GET_SPECIFIC_HEAT_BG(CP,TMP_G)
      ENDIF
      DN = 1._EB/RDN(IW)
   ENDIF
...
USE GLOBAL_CONSTANTS, ONLY: EVACUATION_ONLY,SOLID_PHASE_ONLY,SOLID_BOUNDARY,I_PROG_SOOT,N_TRACKED_SPECIES,TUSED
INTEGER, INTENT(IN) :: NM
!REAL(EB), PARAMETER :: CS=1.147_EB,CT=2.18_EB,CM=1.146_EB,KPKG=1._EB,PARTD=2.E-8_EB
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_SOOT,RHOG,YIN(1:N_TRACKED_SPECIES),YDEP,K_AIR,TMP_FILM
