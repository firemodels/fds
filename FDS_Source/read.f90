...
CHARACTER(30) :: LABEL,MB,SIMPLE_CHEMISTRY_FUEL
CHARACTER(100) :: MESSAGE,FYI
CHARACTER(30) :: ID,SURF_DEFAULT,EVAC_SURF_DEFAULT
LOGICAL :: SUCCESS,EX,THICKEN_OBSTRUCTIONS,BAD,SPECIFIED_HUMIDITY=.FALSE.
REAL(EB) :: XB(6),TEXTURE_ORIGIN(3)
REAL(EB) :: PBX,PBY,PBZ,HUMIDITY
REAL(EB) :: MW_MIN,MW_MAX
INTEGER  :: I,J,K,IZERO,IOS,N_SPEC_READ,I_BACKGROUND=0
...
REAL(EB) :: DT,VEL_CHAR
INTEGER :: NM
NAMELIST /TIME/ DT,T_BEGIN,T_END,FYI,WALL_INCREMENT,SYNCHRONIZE, &
...
NAMELIST /DUMP/ RENDER_FILE,SMOKE3D,SMOKE3D_QUANTITY,FLUSH_FILE_BUFFERS,MASS_FILE, &
                ...
                NFRAMES,DT_RESTART,DEBUG,TIMING,COLUMN_DUMP_LIMIT,MAXIMUM_DROPLETS,WRITE_XYZ, &
                PLOT3D_QUANTITY,STATUS_FILES,DEVC_COLUMN_LIMIT,CTRL_COLUMN_LIMIT,PLOT3D_SPEC_ID,SMOKE3D_SPEC_ID, &
                PLOT3D_PART_ID,PLOT3D_VELO_INDEX,VELOCITY_ERROR_FILE,DT_SL3D
...
REAL(EB) :: MASS_FRACTION_0,MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,XVAP,FIC_CONCENTRATION,FLD_LETHAL_DOSE
INTEGER  :: N_SPEC_READ,N_SPEC_REALLOC,SPEC_PTR,N,NN,N_PREDEFINED
LOGICAL  :: ABSORBING,SMIX_COMPONENT_ONLY,BACKGROUND,PREDEFINED
CHARACTER(30) :: FORMULA
TYPE(SPECIES_TYPE), POINTER :: SS=>NULL()
TYPE(SPECIES_TYPE),POINTER,DIMENSION(:):: SPECIES_TEMP
NAMELIST /SPEC/ ABSORBING,BACKGROUND,CONDUCTIVITY,DIFFUSIVITY,EPSILONKLJ,FIC_CONCENTRATION,FLD_LETHAL_DOSE,FORMULA,FYI,HUMIDITY,ID,&
                MASS_EXTINCTION_COEFFICIENT,MASS_FRACTION_0,MW,REFERENCE_TEMPERATURE,SIGMALJ,SMIX_COMPONENT_ONLY,&
                SPECIFIC_ENTHALPY,SPECIFIC_HEAT,VISCOSITY
...
IF (SIMPLE_CHEMISTRY) CALL SET_SIMPLE_CHEMISTRY_SPECIES

N_SPEC_REALLOC = N_SPECIES
N_PREDEFINED = N_SPECIES
...
COUNT_SPEC_LOOP: DO
   ...
   N_SPEC_REALLOC = N_SPEC_REALLOC + 1
   
!IF SIMPLE_CHEMISTRY do not allocate additional array space for predefined species   
   IF (SIMPLE_CHEMISTRY) THEN
      DO NN=1,N_SPECIES
         IF(TRIM(SPECIES(NN)%ID)==TRIM(ID) .OR. TRIM(ID)==TRIM(SPECIES(FUEL_INDEX)%ID)) THEN
            N_SPEC_REALLOC = N_SPEC_REALLOC - 1
            EXIT
         ENDIF
      ENDDO
   ...
   IF (BACKGROUND) THEN 
      ...
      IF (I_BACKGROUND /= 0) THEN
         ...
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      I_BACKGROUND = N_SPEC_READ
   ENDIF
   IF (SMIX_COMPONENT_ONLY .AND. MASS_FRACTION_0 > 0._EB) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Cannot define MASS_FRACTION_0 for a SMIX_COMPONENT_ONLY species'
         CALL SHUTDOWN(MESSAGE)
   ENDIF
...
IF (N_SPEC_REALLOC > N_SPECIES) THEN
   IF (N_SPECIES > 0) THEN
      ALLOCATE(SPECIES_TEMP(1:N_SPECIES),STAT=IZERO)
      CALL ChkMemErr('READ','SPECIES_TEMP',IZERO) 
      SPECIES_TEMP(1:N_SPECIES) = SPECIES(1:N_SPECIES)
      DEALLOCATE(SPECIES)
      ALLOCATE(SPECIES(1:N_SPEC_REALLOC),STAT=IZERO)
      CALL ChkMemErr('READ','SPECIES',IZERO) 
      SPECIES(1:N_SPECIES)=SPECIES_TEMP(1:N_SPECIES)
      DEALLOCATE(SPECIES_TEMP)
   ELSE
      ALLOCATE(SPECIES(1:N_SPEC_REALLOC),STAT=IZERO)
      CALL ChkMemErr('READ','SPECIES',IZERO)   

   ENDIF
   N_SPECIES = N_SPEC_REALLOC
ENDIF

! Read and process species
SPEC_PTR = N_PREDEFINED

SPEC_READ_LOOP: DO N=1,N_SPEC_READ
   ...
   PREDEFINED = .FALSE.
   IF (SIMPLE_CHEMISTRY) THEN      
      DO NN=1,N_PREDEFINED
         IF(TRIM(SPECIES(NN)%ID)==TRIM(ID)) THEN
            SELECT CASE(TRIM(ID))
               CASE('NITROGEN')
                  SS => SPECIES(N2_INDEX)       
                  PREDEFINED = .TRUE.
               CASE('OXYGEN')
                  ...
                  PREDEFINED = .TRUE.
               CASE('CARBON DIOXIDE')
                  SS => SPECIES(CO2_INDEX)       
                  PREDEFINED = .TRUE.
               CASE('CARBON MONOXIDE')
                  SS => SPECIES(CO_INDEX)       
                  PREDEFINED = .TRUE.
               CASE('WATER VAPOR')
                  SS => SPECIES(H2O_INDEX)       
                  PREDEFINED = .TRUE.
               CASE('HYDROGEN')
                  SS => SPECIES(H2_INDEX)       
                  PREDEFINED = .TRUE.
               CASE('SOOT')
                  ...
                  PREDEFINED = .TRUE.
               CASE DEFAULT
                  IF (TRIM(ID)==TRIM(SPECIES(FUEL_INDEX)%ID)) THEN
                     SS => SPECIES(FUEL_INDEX)                      
                     PREDEFINED = .TRUE.
                  ENDIF
            END SELECT
            EXIT
   ...
   IF (PREDEFINED) THEN
      ...
      SS%PRIMITIVE_ONLY = .FALSE.
      IF(TRIM(ID)=='WATER VAPOR') THEN
   ...
   SS%SIG                         = SIGMALJ
   SS%SMIX_COMPONENT_ONLY         = SMIX_COMPONENT_ONLY   
   IF (SMIX_COMPONENT_ONLY) SS%PRIMITIVE_ONLY = .FALSE.
   ...
   IF (TRIM(FORMULA)=='null') WRITE(FORMULA,'(A,I2)') 'SPEC_',N
   SS%RCON = R0/SS%MW
   SS%MODE = GAS_SPECIES
   SELECT CASE (ID)
      CASE('WATER VAPOR')
         H2O_INDEX = N
         IF (.NOT. SMIX_COMPONENT_ONLY) THEN
            IF (MASS_FRACTION_0<0._EB) THEN           
               ! Humidity (40% by default, but limited for high temps)
               IF (HUMIDITY < 0._EB) THEN
                  XVAP = AMBIENT_WATER_VAPOR(40._EB,TMPA)
...
SUBROUTINE SET_SIMPLE_CHEMISTRY_SPECIES
...
INTEGER  :: N
LOGICAL  :: ABSORBING,SMIX_COMPONENT_ONLY,BACKGROUND,PRIMITIVE_ONLY
CHARACTER(30) :: FORMULA,SIMPLE_ID(8)
TYPE(SPECIES_TYPE), POINTER :: SS=>NULL()

N_SPECIES = 8

SIMPLE_ID(1)='ETHYLENE'
SIMPLE_ID(2)='NITROGEN'
SIMPLE_ID(3)='OXYGEN'
SIMPLE_ID(4)='CARBON DIOXIDE'
SIMPLE_ID(5)='CARBON MONOXIDE'
SIMPLE_ID(6)='WATER VAPOR'
SIMPLE_ID(7)='HYDROGEN'
SIMPLE_ID(8)='SOOT'

IF (TRIM(SIMPLE_CHEMISTRY_FUEL) /='null') SIMPLE_ID(1)=SIMPLE_CHEMISTRY_FUEL
FUEL_INDEX = 1
N2_INDEX   = 2
O2_INDEX   = 3
CO2_INDEX  = 4
...
ALLOCATE(SPECIES(8),STAT=IZERO)
CALL ChkMemErr('SET_SIMPLE_CHEMISTRY_SPECIES','SPECIES',IZERO)
...
DO N=1,8
   ...
   SS%EPSK                        = EPSILONKLJ
   SS%FORMULA                     = FORMULA
   SS%FIC_CONCENTRATION           = 0._EB
   SS%FLD_LETHAL_DOSE             = 0._EB
   SS%ID                          = SIMPLE_ID(N)
   SS%MASS_EXTINCTION_COEFFICIENT = MASS_EXTINCTION_COEFFICIENT
   SS%MODE                        = GAS_SPECIES   
   SS%MW                          = MW
   SS%PRIMITIVE_ONLY              = PRIMITIVE_ONLY
   SS%REFERENCE_TEMPERATURE       = REFERENCE_TEMPERATURE + TMPM
   SS%SIG                         = SIGMALJ
   SS%SMIX_COMPONENT_ONLY         = SMIX_COMPONENT_ONLY
   SS%SPECIFIC_HEAT               = SPECIFIC_HEAT*1000._EB
   ...
   CALL GAS_PROPS(SS%ID,SS%SIG,SS%EPSK,SS%MW,SS%ABSORBING,SS%FORMULA)  
   SS%RCON                        = R0/SS%MW
   IF (SIMPLE_ID(N)=='SOOT') THEN
      SS%MODE = AEROSOL_SPECIES  
      SS%MASS_EXTINCTION_COEFFICIENT=8700._EB
   ENDIF
ENDDO

END SUBROUTINE SET_SIMPLE_CHEMISTRY_SPECIES
...
REAL(EB) :: VOLUME_FRACTION(1:MAX_SPECIES),MASS_FRACTION(1:MAX_SPECIES),CONVERSION,MASS_FRACTION_0
INTEGER :: N,NS,NS2,SMIX_OFFSET
LOGICAL :: BACKGROUND
...
COUNT_SMIX_LOOP: DO
   ...
   IF (BACKGROUND) THEN
      ...
      IF (I_BACKGROUND /=0) THEN
         ...
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      I_BACKGROUND = N_LUMPED_SPECIES
      SMIX_OFFSET = 1
   ENDIF
   
   30 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I2)') 'ERROR: Problem with Species MIXture number ',N_LUMPED_SPECIES+1
         CALL SHUTDOWN(MESSAGE)
      ENDIF
ENDDO COUNT_SMIX_LOOP
29 REWIND(LU_INPUT)

N_TRACKED_SPECIES = N_LUMPED_SPECIES - SMIX_OFFSET

IF (N_LUMPED_SPECIES==0) RETURN

ALLOCATE(SPECIES_MIXTURE(0:N_TRACKED_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ_SMIX','SPECIES_MIXTURE',IZERO)

READ_SMIX_LOOP: DO N=1,N_LUMPED_SPECIES   

   SM => SPECIES_MIXTURE(N-SMIX_OFFSET)
   WRITE(SM%FORMULA,'(A,I2)') 'Z',N-SMIX_OFFSET   
   MASS_FRACTION = -1._EB
   ...
   IF (TRIM(ID)=='null') THEN
      IF (N_LUMPED_SPECIES < 10) THEN
         WRITE(ID,'(A,I1)') 'LUMPED SPECIES ',N_LUMPED_SPECIES - SMIX_OFFSET
      ELSE
         WRITE(ID,'(A,I2)') 'LUMPED SPECIES ',N_LUMPED_SPECIES - SMIX_OFFSET
      ENDIF
   ENDIF
   ...
   SM%N_SUB_SPECIES = 0
   DO NS = 1,N_SPECIES
      IF (TRIM(ID)==TRIM(SPECIES(NS)%ID)) THEN
         WRITE(MESSAGE,'(A,I2,A,A,A)') 'ERROR: Problem with Species MIXture number ',N, &
                                    '. The ID ',TRIM(ID),' has already been used for a SPEC input.'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
   ENDDO
   SM%ID = ID
   ...
   ALLOCATE (SM%SPEC_ID(SM%N_SUB_SPECIES),STAT=IZERO)
   CALL ChkMemErr('READ_SMIX','SM%SPEC_ID',IZERO)
   SM%SPEC_ID(1:SM%N_SUB_SPECIES) = SPEC_ID(1:SM%N_SUB_SPECIES)
   ALLOCATE (SM%Y_INDEX(SM%N_SUB_SPECIES),STAT=IZERO)
   CALL ChkMemErr('READ_SMIX','SM%Y_INDEX',IZERO)
   SM%Y_INDEX = -1
   ALLOCATE (SM%VOLUME_FRACTION(SM%N_SUB_SPECIES),STAT=IZERO)
   CALL ChkMemErr('READ_SMIX','SM%VOLUME_FRACTION',IZERO)
   SM%VOLUME_FRACTION(1:SM%N_SUB_SPECIES) = VOLUME_FRACTION(1:SM%N_SUB_SPECIES)
   ALLOCATE (SM%MASS_FRACTION(SM%N_SUB_SPECIES),STAT=IZERO)
   CALL ChkMemErr('READ_SMIX','SM%MASS_FRACTION',IZERO)
   ...
   DO NS = 1,SM%N_SUB_SPECIES
      ...
      FIND_SPEC_ID: DO NS2 = 1,N_SPECIES
         IF (TRIM(SPECIES(NS2)%ID) == TRIM(SPEC_ID(NS))) THEN
            SM%Y_INDEX(NS) = NS2
            SPECIES(NS2)%PRIMITIVE_ONLY = .FALSE.
            EXIT FIND_SPEC_ID
         ENDIF
      ...
      IF (MASS_FRACTION(NS) >= 0._EB) THEN
         CONVERSION = CONVERSION + MASS_FRACTION(NS) / SPECIES(SM%Y_INDEX(NS))%MW
      ELSE
         CONVERSION = CONVERSION + VOLUME_FRACTION(NS) * SPECIES(SM%Y_INDEX(NS))%MW
      ENDIF
   ENDDO
   IF (MASS_FRACTION(1) > 0._EB) THEN
      DO NS = 1,SM%N_SUB_SPECIES
         SM%VOLUME_FRACTION(NS) = MASS_FRACTION(NS) / SPECIES(SM%Y_INDEX(NS))%MW / CONVERSION
      ENDDO
   ELSE
      DO NS = 1,SM%N_SUB_SPECIES
         SM%MASS_FRACTION(NS) = VOLUME_FRACTION(NS) * SPECIES(SM%Y_INDEX(NS))%MW / CONVERSION
      ENDDO
   ENDIF
   ...
   SM%MW = 0._EB
   DO NS = 1,SM%N_SUB_SPECIES
      SM%MASS_EXTINCTION_COEFFICIENT = SM%MASS_FRACTION(NS) * SPECIES(SM%Y_INDEX(NS))%MASS_EXTINCTION_COEFFICIENT
      SM%MW = SM%MW + SM%VOLUME_FRACTION(NS) * SPECIES(SM%Y_INDEX(NS))%MW
   ENDDO     
   SM%RCON = R0/SM%MW
...
IF (I_BACKGROUND<=0) THEN
   ...
   ELSE
      DO N2 = 1,N_SPECIES
         IF (TRIM(SPECIES(N2)%ID)=='AIR') THEN
            SPECIES(N2)%BACKGROUND=.TRUE.
            SPECIES(N2)%SMIX_COMPONENT_ONLY=.FALSE.
            N=N2
            I_BACKGROUND = N2
            EXIT
         ENDIF
      ENDDO
      IF (N==0) THEN
         ALLOCATE(SPEC_ALLOC(1:N_SPECIES),STAT=IZERO)
         CALL ChkMemErr('PROC_SMIX','SPEC_ALLOC',IZERO)
         SPEC_ALLOC(1:N_SPECIES) = SPECIES(1:N_SPECIES)
         DEALLOCATE(SPECIES)
         ALLOCATE(SPECIES(1:N_SPECIES+1),STAT=IZERO)
         CALL ChkMemErr('PROC_SMIX','SPECIES',IZERO)
         SPECIES(1:N_SPECIES) = SPEC_ALLOC(1:N_SPECIES)
   ...
   IF (SET_BACKGROUND) THEN
      ...
      SS%FIC_CONCENTRATION           = 0._EB
      SS%ID                          = 'AIR'
      SS%MASS_EXTINCTION_COEFFICIENT = 0._EB
      SS%MODE                        = GAS_SPECIES   
      SS%MU_USER                     = -1._EB
      SS%MW                          = -1._EB
      SS%REFERENCE_TEMPERATURE       = TMPM
      SS%SIG                         =  -1._EB
      SS%SMIX_COMPONENT_ONLY         = .FALSE.   
      SS%PRIMITIVE_ONLY              = .TRUE.
      SS%SPECIFIC_HEAT               = -1000._EB
      SS%SPECIFIC_ENTHALPY           = -1000._EB
      SS%YY0                         = 0._EB
      CALL GAS_PROPS(SS%ID,SS%SIG,SS%EPSK,SS%MW,SS%ABSORBING,SS%FORMULA)
      SS%RCON = R0/SS%MW
   ENDIF
...
N2 = 0
DO N=1,N_SPECIES
 IF (SPECIES(N)%BACKGROUND .OR. SPECIES(N)%SMIX_COMPONENT_ONLY) CYCLE
  N2 = N2 + 1
ENDDO

IF(ALLOCATED(SPECIES_MIXTURE) .AND. N2>0) THEN
   ALLOCATE(SMIX_ALLOC(0:N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('PROC_SMIX','SMIX_ALLOC',IZERO)
   SMIX_ALLOC(0:N_TRACKED_SPECIES) = SPECIES_MIXTURE(0:N_TRACKED_SPECIES)
   DEALLOCATE(SPECIES_MIXTURE)
   ALLOCATE(SPECIES_MIXTURE(0:N_TRACKED_SPECIES+N2),STAT=IZERO)
   CALL ChkMemErr('PROC_SMIX','SPECIES_MIXTURE',IZERO)
   SPECIES_MIXTURE(0:N_TRACKED_SPECIES) = SMIX_ALLOC(0:N_TRACKED_SPECIES)
   DEALLOCATE(SMIX_ALLOC)
ELSEIF(.NOT. ALLOCATED(SPECIES_MIXTURE)) THEN
   ALLOCATE(SPECIES_MIXTURE(0:N2),STAT=IZERO)
   CALL ChkMemErr('PROC_SMIX','SPECIES_MIXTURE',IZERO)   
ENDIF

NN = 0
DO N=1,N_SPECIES
   IF (SPECIES(N)%SMIX_COMPONENT_ONLY) CYCLE
   IF (SPECIES(N)%BACKGROUND) THEN
      NN = 0
   ELSE
      IF (N>1 .AND. NN == 0) THEN
         NN = N_TRACKED_SPECIES + 1
      ELSE
         NN = NN + 1
      ENDIF
   ENDIF
   SM => SPECIES_MIXTURE(NN)
   WRITE(SM%FORMULA,'(A,I2)') 'Z',NN
   SM%ID = SPECIES(N)%ID
   SM%MASS_EXTINCTION_COEFFICIENT = SPECIES(N)%MASS_EXTINCTION_COEFFICIENT
   ALLOCATE(SM%MASS_FRACTION(1))
   SM%MASS_FRACTION = 1._EB
   SM%MW = SPECIES(N)%MW
   SM%N_SUB_SPECIES = 1
   SM%RCON = SPECIES(N)%RCON
   ALLOCATE(SM%SPEC_ID(1))
   SM%SPEC_ID = SPECIES(N)%ID
   ALLOCATE(SM%Y_INDEX(1))   
   SM%Y_INDEX = N
   ALLOCATE(SM%VOLUME_FRACTION(1))
   SM%VOLUME_FRACTION = 1._EB
   SM%ZZ0 = SPECIES(N)%YY0
ENDDO

N_TRACKED_SPECIES = N_TRACKED_SPECIES + N2

IF (SUM(SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0) > 1._EB) &
  SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0 = SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0/SUM(SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0)
SPECIES_MIXTURE(0)%ZZ0 = 1._EB - SUM(SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0)

! Setup the array to convert the index of the tracked species to the index of all species

IF (N_REACTIONS > 0) CALL GET_RN_NU_SPECIES
...
Z2Y = 0._EB 
ALLOCATE(Z2Y_C(N_SPECIES),STAT=IZERO)
...
DO NN = 1,SM%N_SUB_SPECIES
   Z2Y_C(SM%Y_INDEX(NN))=SM%MASS_FRACTION(NN)
ENDDO

Y_MF_SUM_Y_C = SUM(Z2Y_C)

DO N = 1, N_TRACKED_SPECIES
   SM =>SPECIES_MIXTURE(N)
   DO NN = 1,SM%N_SUB_SPECIES
      Z2Y(SM%Y_INDEX(NN),N)=SM%MASS_FRACTION(NN)-Z2Y_C(SM%Y_INDEX(NN))
   ENDDO
ENDDO

! Compute Y to MW Arrays
ALLOCATE(MW_AVG_Y(N_TRACKED_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ','MW_AVG_Y',IZERO) 

MW_AVG_Y_C = 1._EB/SPECIES_MIXTURE(0)%MW
! Compute the initial value of R0/MW_AVG
IF (N_TRACKED_SPECIES > 0) THEN   
   MW_AVG_Y(1:N_TRACKED_SPECIES) = 1._EB/SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%MW - MW_AVG_Y_C
   ALLOCATE(ZZZ(1:N_TRACKED_SPECIES))
   ZZZ(1:N_TRACKED_SPECIES) = SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0
   CALL GET_SPECIFIC_GAS_CONSTANT(ZZZ,RSUM0)
   DEALLOCATE(ZZZ)
ELSE
   RSUM0 = SPECIES_MIXTURE(0)%RCON
ENDIF
...
ALLOCATE(Z2CP_C(0:5000))
CALL ChkMemErr('READ','Z2CP_C',IZERO)    
Z2CP_C = 0._EB   

ALLOCATE(Z2CPBAR_C(0:5000))   
CALL ChkMemErr('READ','Z2CPBAR_C',IZERO)    
Z2CPBAR_C = 0._EB   

ALLOCATE(Z2K_C(0:5000))
CALL ChkMemErr('READ','Z2K_C',IZERO)    
Z2K_C = 0._EB   

ALLOCATE(Z2MU_C(0:5000))      
CALL ChkMemErr('READ','Z2MU_C',IZERO)    
Z2MU_C = 0._EB   

ALLOCATE(Z2CP(0:5000,N_TRACKED_SPECIES))     
CALL ChkMemErr('READ','Z2CC',IZERO)    
Z2CP = 0._EB   

ALLOCATE(Z2CPBAR(0:5000,N_TRACKED_SPECIES))     
CALL ChkMemErr('READ','Z2CPBAR',IZERO)    
Z2CPBAR = 0._EB   

ALLOCATE(Z2K(0:5000,N_TRACKED_SPECIES))      
CALL ChkMemErr('READ','Z2K',IZERO)    
Z2K = 0._EB   

ALLOCATE(Z2MU(0:5000,N_TRACKED_SPECIES))      
CALL ChkMemErr('READ','Z2MU',IZERO)    
Z2MU = 0._EB   

ALLOCATE(Z2D(0:5000,0:N_TRACKED_SPECIES))      
CALL ChkMemErr('READ','Z2MU',IZERO)    
Z2D = 0._EB   
...
DO J = 1,5000
   DO N = 1,N_SPECIES
      ...
      IF (SS%ISFUEL .AND. FUEL_INDEX <=0) THEN
         FUEL_INDEX = N
         IF (MW_FUEL==-1) MW_FUEL = SPECIES(N)%MW
      ENDIF      
   ENDDO
   Z2CP_C(J)    = SUM(Z2Y_C(:) * CP_TMP(:))
   IF (J>1) THEN
      Z2CPBAR_C(J) = (Z2CPBAR_C(J-1)*(REAL(J,EB)-1._EB)+0.5_EB*(Z2CP_C(J)+Z2CP_C(J-1)))/REAL(J,EB)
   ELSE
      Z2CP_C(0) = Z2CP_C(J)
      Z2CPBAR_C(0) = SUM(Z2Y_C(:) * H_TMP(:))
      Z2CPBAR_C(1) = Z2CPBAR_C(0)+Z2CP_C(1)
   ENDIF
   Z2MU_C(J)    = SUM(Z2Y_C(:) * MU_TMP(:))
   Z2K_C(J)     = SUM(Z2Y_C(:) * K_TMP(:))   
   Z2D(J,0) = SUM(Z2Y_C(:) * D_TMP(:))
   DO N = 1,N_TRACKED_SPECIES
      Z2D(J,N) = SUM((Z2Y(:,N)+Z2Y_C(:)) * D_TMP(:))
      Z2CP(J,N)    = SUM(Z2Y(:,N) * CP_TMP(:))
      IF (J>1) THEN      
         Z2CPBAR(J,N) = (Z2CPBAR(J-1,N)*(REAL(J,EB)-1._EB)+0.5_EB*(Z2CP(J,N)+Z2CP(J-1,N)))/REAL(J,EB)
      ELSE
...
DO N = 1,N_TRACKED_SPECIES
   Z2CP(:,N)=Z2CP(:,N)-Z2CP_C(:)
   Z2CPBAR(:,N)=Z2CPBAR(:,N)-Z2CPBAR_C(:)
ENDDO
...
SUBROUTINE READ_REAC(I_PASS)

USE PHYSICAL_FUNCTIONS, ONLY : AMBIENT_WATER_VAPOR
CHARACTER(30) :: FUEL,SPEC_ID(1:MAX_SPECIES)
LOGICAL :: IDEAL
INTEGER :: NR,NS,NS2
INTEGER, INTENT(IN) :: I_PASS
REAL(EB) :: Y_F_INLET, MASS_REACTANT, MASS_PRODUCT, Y_CO2_INFTY,&
            H2_YIELD,SOOT_YIELD,CO_YIELD, Y_F_LFL,X_O2_LL,EPUMO2,BOF,MASS_EXTINCTION_COEFFICIENT,&
            CRITICAL_FLAME_TEMPERATURE,HEAT_OF_COMBUSTION,NU(0:MAX_SPECIES),E,N_S(0:MAX_SPECIES),C,H,N,O, &
            AUTO_IGNITION_TEMPERATURE,SUM1,THRESHOLD_TEMPERATURE
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM=>NULL()
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,EPUMO2,ID, N_S,EXTINCTION2,&
                ...
                SPEC_ID,THRESHOLD_TEMPERATURE,Y_CO2_INFTY,HUMIDITY,MASS_EXTINCTION_COEFFICIENT
...
COUNT_REAC_LOOP: DO
   ...
   IF ((BOF > 0._EB .OR. E > 0._EB) .AND. (C>ZERO_P .OR. H>ZERO_P)) THEN
      ...
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (SIMPLE_CHEMISTRY .AND. I_BACKGROUND>0) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Cannot define a BACKGROUND SPEC when using simple chemistry'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (SIMPLE_CHEMISTRY .AND. C<=ZERO_P .AND. H<=ZERO_P) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Must specify fuel chemistry using C and/or H when using simple chemistry'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (SIMPLE_CHEMISTRY .AND. N_LUMPED_SPECIES > 0) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Cannot specifiy additional lumped species when using simple chemistry'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (C <=ZERO_P .AND. CO_PRODUCTION) THEN
      WRITE(MESSAGE,'(A)')  'ERROR: Cannot set CO_PRODUCTION=.TRUE. when C=0'
      CALL SHUTDOWN(MESSAGE)
   ENDIF    
   IF ((C <=ZERO_P .OR. SOOT_YIELD<=ZERO_P) .AND. SOOT_DEPOSITION) THEN
      WRITE(MESSAGE,'(A)')  'ERROR: Cannot set SOOT_DEPOSITION=.TRUE. when C=0 or SOOT_YIELD=0'
      CALL SHUTDOWN(MESSAGE)
   ENDIF    
...
IF (I_PASS==1) THEN
   IF (SIMPLE_CHEMISTRY) SIMPLE_CHEMISTRY_FUEL = FUEL
   RETURN
ENDIF

IF (SIMPLE_CHEMISTRY .AND. I_BACKGROUND > 0) THEN
   WRITE(MESSAGE,'(A)') 'ERROR: cannot set BACKGROUND=.TRUE. on SPEC or SMIX when using a simple chemistry REAC'
   CALL SHUTDOWN(MESSAGE)
ENDIF
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   ! Process simple  chemistry inputs
   IF (SIMPLE_CHEMISTRY) THEN      
      CALL SIMPLE_CHEMISTRY_INIT
      EXIT REAC_READ_LOOP
   ENDIF
   
   ! Process inputs for user defined reactions 
   RN => REACTION(NR)
   ...
   ENDDO
   RN%N_SPECIES=NS2
   ALLOCATE(RN%N(RN%N_SPECIES))
   RN%N(1:RN%N_SPECIES) = N_S(1:RN%N_SPECIES)
   ALLOCATE(RN%NU(RN%N_SPECIES))
   RN%NU(1:RN%N_SPECIES)=NU(1:RN%N_SPECIES)
   ...
   DO NS=1,RN%N_SPECIES
      ...
      IF (RN%SPECIES(NS)==-1) THEN
         ...
         ' not specified or is SMIX_COMPONENT_ONLY' 
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      IF (RN%NU(NS) < -ZERO_P) MASS_REACTANT = MASS_REACTANT + RN%NU(NS)*SPECIES_MIXTURE(NS2)%MW
      IF (RN%NU(NS) > ZERO_P)  MASS_PRODUCT = MASS_PRODUCT + RN%NU(NS)*SPECIES_MIXTURE(NS2)%MW
   ENDDO  
   ...
   DO NS=1,RN%N_SPECIES
      IF (TRIM(SPEC_ID(NS))==TRIM(FUEL)) THEN
         ...
         IF (RN%NU(NS)>0._EB) THEN
            WRITE(MESSAGE,'(A,I3,A,A,A)') 'ERROR: Problem with REAC ',NR,'. Fuel species, ',TRIM(FUEL),', has NU > 0.' 
            CALL SHUTDOWN(MESSAGE)
         ENDIF                      
...
SUBROUTINE SIMPLE_CHEMISTRY_INIT
INTEGER :: N_SUB_SPECIES=8
INTEGER :: NR2,NS,NS2
REAL(EB) :: XVAP,NU(1:MAX_SPECIES)=0._EB
CHARACTER(30):: ADD_NAME
!Initialize species mixtures and reactions for simple chemistry, mixing controlled combustion
...
MW_FUEL = C * MW_C + H * MW_H + O * MW_O + N * MW_N
...
DO NS=0,N_TRACKED_SPECIES
   ...
   SM%N_SUB_SPECIES=N_SUB_SPECIES
   ALLOCATE (SM%SPEC_ID(N_SUB_SPECIES))
   ALLOCATE (SM%Y_INDEX(N_SUB_SPECIES))
   DO NS2 = 1,N_SUB_SPECIES
      SM%Y_INDEX(NS2) = NS2
      SM%SPEC_ID(NS2) = SPECIES(NS2)%ID
   ENDDO
   ALLOCATE (SM%MASS_FRACTION(N_SUB_SPECIES))
   SM%MASS_FRACTION = 0._EB
   ALLOCATE (SM%VOLUME_FRACTION(N_SUB_SPECIES))
   SM%VOLUME_FRACTION = 0._EB
   WRITE(SM%ID,'(A,I1)') 'LUMPED SPECIES ',NS
ENDDO
...
!Set Background Lumped Species
!Uses NASA Earth Fact Sheet (updated 2007.01) for O2 and CO2 with N2 making up the balance + User Humidity

SM => SPECIES_MIXTURE(0)
SM%FORMULA='Z0'
IF (SPECIES(O2_INDEX)%YY0 > ZERO_P) THEN
   WRITE(LU_ERR,*) 'WARNING: Specified O2 MASS_FRACTION_0 on SPEC will overide default simple chemistry value.'
   SM%MASS_FRACTION(O2_INDEX)  = SPECIES(O2_INDEX)%YY0  
ELSE
   SM%MASS_FRACTION(O2_INDEX)  = Y_O2_INFTY
ENDIF

IF (SPECIES(CO2_INDEX)%YY0 > ZERO_P) THEN
   WRITE(LU_ERR,*) 'WARNING: Specified CO2 MASS_FRACTION_0 on SPEC will overide default simple chemistry value.'
   SM%MASS_FRACTION(CO2_INDEX)  = SPECIES(CO2_INDEX)%YY0  
ELSE
   SM%MASS_FRACTION(CO2_INDEX)  = Y_CO2_INFTY
ENDIF

IF (SPECIFIED_HUMIDITY) THEN
      WRITE(LU_ERR,*) &
      'WARNING: Specified H2O MASS_FRACITON_0 or HUMIDITITY on SPEC will overide default simple chemistry value of HUMIDTY.'   
      SM%MASS_FRACTION(H2O_INDEX)  = 0._EB  
ELSE
   SPECIES(H2O_INDEX)%YY0=0._EB
   IF (HUMIDITY < 0._EB) THEN
      XVAP = AMBIENT_WATER_VAPOR(40._EB,TMPA)
      IF (XVAP > 0._EB) THEN
         HUMIDITY = 40._EB*MIN(1._EB,AMBIENT_WATER_VAPOR(40._EB,313.15_EB)/XVAP)
...
SM%MASS_FRACTION(O2_INDEX) = SM%MASS_FRACTION(O2_INDEX) * (1._EB-SM%MASS_FRACTION(H2O_INDEX))    
SM%MASS_FRACTION(CO2_INDEX) = SM%MASS_FRACTION(CO2_INDEX) * (1._EB-SM%MASS_FRACTION(H2O_INDEX))
SM%MASS_FRACTION(N2_INDEX) = 1._EB-SUM(SM%MASS_FRACTION)

IF (SM%MASS_FRACTION(N2_INDEX) < 0._EB) THEN
   WRITE(MESSAGE,*) 'ERROR: Computed N2 mass fraction for simple chemistry is less  than zero.'
   CALL SHUTDOWN(MESSAGE)
ENDIF
...
!Set Fuel Lumped Species (I_FUEL)
...
!Compute fuel heat of formation
   ...
   NU(H2O_INDEX) = 0.5_EB * H - NU(H2_INDEX) - 0.5_EB * NU(SOOT_INDEX) * SOOT_H_FRACTION
   ...
   NU(CO_INDEX) = C - NU(SOOT_INDEX) * (1._EB - SOOT_H_FRACTION)
   IF (NU(CO_INDEX) < 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Values for SOOT_YIELD and SOOT_H_FRACTION result in negative CO yield'
      CALL SHUTDOWN(MESSAGE)
   ENDIF

   NU(O2_INDEX)  = 0.5_EB*NU(CO_INDEX) + 0.5_EB*NU(H2O_INDEX) - 0.5_EB * O

   
!Set Incomplete Combustion Lumped Species (I_PROG_F)
   SM => SPECIES_MIXTURE(2) 
   SM%FORMULA='Z2'   
   SM%VOLUME_FRACTION(N2_INDEX) = NU(N2_INDEX) +& !N2 from fuel molecule
                    SPECIES_MIXTURE(0)%VOLUME_FRACTION(N2_INDEX)*NU(O2_INDEX)/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)+&!N2 air
                    SPECIES_MIXTURE(1)%VOLUME_FRACTION(N2_INDEX)/SPECIES_MIXTURE(1)%VOLUME_FRACTION(FUEL_INDEX) !N2 fuel stream
   SM%MASS_FRACTION(N2_INDEX) = SM%VOLUME_FRACTION(N2_INDEX) * MW_N2

   SM%VOLUME_FRACTION(CO2_INDEX) = SPECIES_MIXTURE(0)%VOLUME_FRACTION(CO2_INDEX)*NU(O2_INDEX)/&
                                 SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)!CO2 air
   ...
   SM%VOLUME_FRACTION(H2O_INDEX) = NU(H2O_INDEX)  +&!H2O from fuel
                  SPECIES_MIXTURE(0)%VOLUME_FRACTION(H2O_INDEX)*NU(O2_INDEX)/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)!H2O air 
   ...
   RN%SPEC_ID(1) = 'LUMPED SPECIES 0'   
   RN%SPEC_ID(2) = 'LUMPED SPECIES 1'   
   RN%SPEC_ID(3) = 'LUMPED SPECIES 2'
   RN%SPECIES(1) = 0
   RN%SPECIES(2) = 1
   RN%SPECIES(3) = 2
   RN%NU(1) = -NU(O2_INDEX)/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)*SPECIES_MIXTURE(1)%VOLUME_FRACTION(FUEL_INDEX) 
   RN%NU(2) = -1._EB
   RN%NU(3) = -(RN%NU(1)*SPECIES_MIXTURE(0)%MW+RN%NU(2)*SPECIES_MIXTURE(1)%MW)/SPECIES_MIXTURE(2)%MW
...
! Correct heat of combustion for minor products of combustion
      HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION - (C-NU(CO_INDEX)) * CO2_HEAT_OF_FORMATION * MW_CO2/MW_FUEL &
                                              - (0.5_EB*H-NU(H2O_INDEX)) * H2O_HEAT_OF_FORMATION * MW_H2O/MW_FUEL
   ENDIF
...
!*****
   ...
   RN%THRESHOLD_TEMP = THRESHOLD_TEMPERATURE + TMPM
   RN%MODE = MIXED
   RN%FUEL = 'CARBON MONOXIDE'
   RN%FUEL_INDEX = I_PROG_CO
...
!Set Complete Combustion Lumped Species (I_PROG_CO)
   ...
   SM%VOLUME_FRACTION(CO2_INDEX) = NU(CO2_INDEX) +&!CO2 from fuel
                    SPECIES_MIXTURE(0)%VOLUME_FRACTION(CO2_INDEX)*NU(O2_INDEX)/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)!CO2 air
   SM%MASS_FRACTION(CO2_INDEX) =SM%VOLUME_FRACTION(CO2_INDEX) * MW_CO2
   ...
   RN%SPECIES(2) = 2
   RN%SPECIES(2) = 3
   RN%NU(1) = -0.5_EB/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)/SPECIES_MIXTURE(1)%VOLUME_FRACTION(CO_INDEX)
   RN%NU(2) = -1._EB
   RN%NU(3) = -(RN%NU(1)*SPECIES_MIXTURE(0)%MW+RN%NU(2)*SPECIES_MIXTURE(1)%MW)/SPECIES_MIXTURE(2)%MW
...
!*****
   IF(H > 0) THEN
      RN%BOF = 3.981E14_EB
      RN%E = 167440._EB*1000._EB     
   ELSE
      RN%BOF = 2.53E12_EB
      RN%E = 199547._EB*1000._EB
   ENDIF
   RN%HEAT_OF_COMBUSTION = (CO2_HEAT_OF_FORMATION - CO_HEAT_OF_FORMATION)*1000._EB/MW_CO
   REACTION%HOC_COMPLETE = REACTION(1)%HEAT_OF_COMBUSTION + NU(CO2_INDEX) * MW_CO * RN%HEAT_OF_COMBUSTION/MW_FUEL
...
!Compute fuel heat of formation
   ...
   NU(CO2_INDEX) = C - NU(CO_INDEX) - NU(SOOT_INDEX) * (1._EB - SOOT_H_FRACTION)
   IF (NU(CO2_INDEX) < 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Values for SOOT_YIELD, CO_YIELD, and SOOT_H_FRACTION result in negative CO2 yield'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   ...
   NU(O2_INDEX)  = NU(CO2_INDEX)  + 0.5_EB * NU(CO_INDEX) + 0.5*NU(H2O_INDEX) - 0.5_EB * O
   NU(N2_INDEX) = 0.5_EB * N

!Set Complete Combustion Lumped Species (I_PROG_F)
   SM => SPECIES_MIXTURE(2)
   SM%FORMULA='Z2'
   SM%VOLUME_FRACTION(N2_INDEX) = NU(N2_INDEX) +& !N2 from fuel molecule
                    SPECIES_MIXTURE(0)%VOLUME_FRACTION(N2_INDEX)*NU(O2_INDEX)/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)+&!N2 air
   ...
   IF (SOOT_DEPOSITION) THEN
      ...
      SPECIES_MIXTURE(3)%VOLUME_FRACTION(SOOT_INDEX) = 1._EB
      SPECIES_MIXTURE(3)%MASS_FRACTION(SOOT_INDEX) = 1._EB
      SPECIES_MIXTURE(3)%MW = MW_SOOT
      SPECIES_MIXTURE(3)%RCON = R0/MW_SOOT
   ELSE
      SM%VOLUME_FRACTION(SOOT_INDEX) = NU(SOOT_INDEX)
      SM%MASS_FRACTION(SOOT_INDEX) = SM%VOLUME_FRACTION(SOOT_INDEX) * MW_SOOT      
   ENDIF
   ...
   IF (SOOT_DEPOSITION) THEN
      ...
      RN%NU(1) = -NU(O2_INDEX)/SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)*SPECIES_MIXTURE(1)%VOLUME_FRACTION(FUEL_INDEX)
      RN%NU(2) = -1._EB
      RN%NU(4) = SOOT_YIELD*MW_FUEL/MW_SOOT*SPECIES_MIXTURE(1)%VOLUME_FRACTION(FUEL_INDEX)
      RN%NU(3) = -(RN%NU(1)*SPECIES_MIXTURE(0)%MW+RN%NU(2)*SPECIES_MIXTURE(1)%MW-RN%NU(4)*MW_SOOT)/SPECIES_MIXTURE(2)%MW
   ELSE
      RN%N_SPECIES=3
      ALLOCATE(RN%SPEC_ID(RN%N_SPECIES))
      ALLOCATE(RN%SPECIES(RN%N_SPECIES))   
   ...
   IF (.NOT. RN%IDEAL) THEN
      IF(HEAT_OF_COMBUSTION < 0._EB) HEAT_OF_COMBUSTION = &
                                     -EPUMO2*RN%NU(1)*MW_O2*SPECIES_MIXTURE(0)%VOLUME_FRACTION(O2_INDEX)/&
                                     (SPECIES_MIXTURE(1)%VOLUME_FRACTION(FUEL_INDEX)*MW_FUEL)
   ELSE
! Correct heat of combustion for minor products of combustion
      HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION - NU(SOOT_INDEX)*(1._EB-SOOT_H_FRACTION)*CO2_HEAT_OF_FORMATION/MW_FUEL &
                                              - NU(CO_INDEX)*(CO2_HEAT_OF_FORMATION - CO_HEAT_OF_FORMATION)/MW_FUEL &
                                              - (0.5_EB*H-NU(H2O_INDEX))*H2O_HEAT_OF_FORMATION/MW_FUEL
   ENDIF
   RN%HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION*1000._EB
...
END SUBROUTINE SIMPLE_CHEMISTRY_INIT
...
ID                          = 'null'
IDEAL                       = .FALSE.
MASS_EXTINCTION_COEFFICIENT = 8700._EB  !m**2/kg
N                           = 0._EB
NU                          = 0._EB
N_S                         = -999._EB
O                           = 0._EB
...
SUBROUTINE GET_RN_NU_SPECIES
INTEGER :: NR,NS,NS2,IZERO
TYPE (REACTION_TYPE),POINTER :: RN=>NULL()
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM=>NULL()
...
DO NR=1,N_REACTIONS
   RN => REACTION(NR)
   ALLOCATE(RN%NU_SPECIES(1:N_SPECIES),STAT=IZERO)
   CALL ChkMemErr('GET_RN_NU_SPECIES','RN%NU_SPECIES)',IZERO)
   RN%NU_SPECIES = 0._EB
   DO NS=1,RN%N_SPECIES
      SM => SPECIES_MIXTURE(RN%SPECIES(NS))
      DO NS2 = 1,SM%N_SUB_SPECIES
         RN%NU_SPECIES(SM%Y_INDEX(NS2)) =  RN%NU_SPECIES(SM%Y_INDEX(NS2)) + RN%NU(NS)*SM%VOLUME_FRACTION(NS2)
      ENDDO
   ENDDO
...
INTEGER :: SAMPLING_FACTOR,N,NN,IPC,RGB(3),I_DUMMY,NUMBER_RADIATION_ANGLES
REAL(EB) :: SPECIFIC_HEAT,VAPORIZATION_TEMPERATURE,MELTING_TEMPERATURE,DIAMETER, &
            GAMMA_D,AGE,INITIAL_TEMPERATURE,HEAT_OF_COMBUSTION,HEAT_OF_VAPORIZATION,DENSITY, &
...
NAMELIST /PART/ FYI,SAMPLING_FACTOR,ID,STATIC,MASSLESS,TREE, &
                ...
                AGE,DEVC_ID,CTRL_ID,SPEC_ID,SURF_ID,INITIAL_TEMPERATURE,RGB,QUANTITIES,COLOR, &
...
READ_PART_LOOP: DO N=1,N_PART
   ...
   ENDIF
   IF (SIMPLE_CHEMISTRY) THEN
      IF (TRIM(SPEC_ID)==TRIM(REACTION(1)%FUEL) .OR. TRIM(SPEC_ID)=='FUEL') THEN
         PC%FUEL = .TRUE.
...
PLOOP2: DO IPC=1,N_PART
   ...
   IF (.NOT.PC%MASSLESS) THEN
      ...
      PC%R50 = 0._EB
      ALLOCATE(PC%DMN(0:NDG))
      CALL ChkMemErr('INIT','DMN',IZERO)
      PC%DMN = 0._EB
   ENDIF
ENDDO PLOOP2
   ...
   IF (ANY(PC%QUANTITIES/='null')) THEN
      ...
      QUANTITIES_LOOP: DO NN=1,10
         ...
         CALL GET_QUANTITY_INDEX(PC%SMOKEVIEW_LABEL(PC%N_QUANTITIES),PC%SMOKEVIEW_BAR_LABEL(PC%N_QUANTITIES), &
                                 ...
                                 I_DUMMY,I_DUMMY,I_DUMMY,I_DUMMY,I_DUMMY,'PART', &
...
INTEGER :: N, J, ITMP
REAL(EB) :: H_L,H_V,ZZ_GET(1:N_TRACKED_SPECIES),CPBAR,H_G_S,H_G_S_REF,H_L_REF,TMP_REF,TMP_MELT,TMP_V,TMP_WGT
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC=>NULL()
...
PART_LOOP: DO N=1,N_PART
   ...
   SPEC_ID = SPECIES_MIXTURE(PC%Z_INDEX)%ID
   IF (SIMPLE_CHEMISTRY) THEN
      IF(PC%Z_INDEX==I_FUEL) SPEC_ID = REACTION(1)%FUEL
   ENDIF
   TMP_REF = -1._EB
   TMP_MELT = -1._EB
   TMP_V = -1._EB
   ...
   IF (PC%Z_INDEX>0) THEN
      ...
      CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR,PC%H_V_REFERENCE_TEMPERATURE)      
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
                ...
                P0,PX,PXX,VELOCITY_COMPONENT,SPRAY_PATTERN_SHAPE,SPRAY_PATTERN_MU,SPRAY_PATTERN_BETA
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   IF (PART_ID/='null' .AND. SPRAY_PATTERN_TABLE == 'null' ) THEN
           ALLOCATE(PY%SPRAY_ANGLE_CDF(0:NDC),PY%SPRAY_ANGLE_CDF_THETA(0:NDC))
           IF(SPRAY_PATTERN_MU<0._EB) THEN
              IF(SPRAY_ANGLE(1)>0._EB) THEN
                 SPRAY_PATTERN_MU=0.5_EB*SUM(PY%SPRAY_ANGLE)
              ELSE
                 SPRAY_PATTERN_MU=0._EB
              ENDIF
           ENDIF
           CALL SPRAY_ANGLE_DISTRIBUTION(PY%SPRAY_ANGLE_CDF_THETA,PY%SPRAY_ANGLE_CDF,SPRAY_PATTERN_BETA,SPRAY_PATTERN_MU,MAXVAL(PY%SPRAY_ANGLE),MINVAL(PY%SPRAY_ANGLE),SPRAY_PATTERN_SHAPE,NDC)
   ENDIF
   ...
   IF (PY%SPEC_ID/='null') THEN
      DO I=1,N_SPECIES
         IF (TRIM(SPECIES(I)%ID)==TRIM(PY%SPEC_ID)) THEN
            IF (.NOT. SPECIES(I)%PRIMITIVE_ONLY) PY%Y_INDEX = -I            
            EXIT
         ENDIF
      ENDDO
      IF (PY%Y_INDEX == -999) THEN
         DO I=0,N_TRACKED_SPECIES
            IF (TRIM(SPECIES_MIXTURE(I)%ID)==TRIM(PY%SPEC_ID)) THEN
               PY%Y_INDEX = I
               EXIT
            ENDIF
         ENDDO
      ENDIF
      IF (SIMPLE_CHEMISTRY .AND. (TRIM(PY%SPEC_ID)=='FUEL' .OR. TRIM(PY%SPEC_ID)==TRIM(REACTION(1)%FUEL))) PY%Y_INDEX=I_FUEL
      IF (PY%Y_INDEX==-999) THEN
...
NAMELIST /MATL/ ID,FYI,SPECIFIC_HEAT,CONDUCTIVITY,CONDUCTIVITY_RAMP,SPECIFIC_HEAT_RAMP, &
                ...
                EMISSIVITY,HEATING_RATE,PYROLYSIS_RANGE,HEAT_OF_REACTION,DENSITY,MATL_ID, &
                HEAT_OF_COMBUSTION,A,E,NU_MATL,NU_SPEC,N_S,N_T,N_REACTIONS,&
...
INTEGER :: N,J,NS,NS2,NR,Y_INDEX(MAX_SPECIES,MAX_REACTIONS)
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   DO NR=1,ML%N_REACTIONS
      DO NS=1,MAX_SPECIES
         ...
         IF (SIMPLE_CHEMISTRY) THEN
            IF (TRIM(ML%SPEC_ID(NS,NR))==TRIM(REACTION(1)%FUEL) .OR. TRIM(ML%SPEC_ID(NS,NR))=='FUEL') THEN 
               ML%SPEC_ID(NS,NR) = 'LUMPED SPECIES 1'
            ENDIF
         ENDIF
         ...
         DO NS2=0,N_TRACKED_SPECIES
            IF (TRIM(ML%SPEC_ID(NS,NR))==TRIM(SPECIES_MIXTURE(NS2)%ID)) THEN
               Y_INDEX(NS,NR) = NS2
               ML%NU_GAS(Y_INDEX(NS,NR),NR) = ML%NU_SPEC(NS,NR)
               EXIT
            ENDIF
         ENDDO
         IF (Y_INDEX(NS,NR)==-1) THEN
      ...
      IF (SIMPLE_CHEMISTRY) THEN
         IF (N_REACTIONS > 0) THEN
            ...
            IF (ML%HEAT_OF_COMBUSTION(J)>0._EB .AND. RN%HEAT_OF_COMBUSTION>0._EB)  &
             ML%ADJUST_BURN_RATE(I_FUEL,J) = ML%HEAT_OF_COMBUSTION(J)/(RN%Y_F_INLET*RN%HEAT_OF_COMBUSTION)
         ENDIF
   ...
   IF (ANY(MASS_FLUX>0._EB) .OR. ANY(MASS_FRACTION>0._EB)) THEN
      ...
      ELSE 
         DO NN=1,MAX_SPECIES
            ...
            IF (SIMPLE_CHEMISTRY) THEN
               IF (TRIM(SPEC_ID(NN))=='FUEL' .OR. TRIM(SPEC_ID(NN))==REACTION(1)%FUEL) THEN
                  SF%MASS_FLUX(I_FUEL)    = MASS_FLUX(NN)
                  SF%MASS_FRACTION(I_FUEL)= MASS_FRACTION(NN)
                  SF%TAU(I_FUEL)          = TAU_MF(NN)
                  SF%RAMP_MF(I_FUEL)      = RAMP_MF(NN)
                  CYCLE
               ENDIF
            ENDIF
   ...
   IF (SF%N_LAYERS > 0) THEN
      DO NN=1,SF%N_MATL
         ...
         IF (ML%PYROLYSIS_MODEL/=PYROLYSIS_NONE) THEN
            ...
            IF (I_FUEL >=0) THEN
               IF (ANY(ML%NU_SPEC(I_FUEL,:)>0._EB))  THEN
                  BURNING = .TRUE.
   ...
   BURNING_IF: IF (BURNING .AND. .NOT.ALL(EVACUATION_ONLY)) THEN
      IF (SF%HRRPUA>0._EB) THEN
         ...
         ELSE
            RN => REACTION(1)
         ENDIF
         SF%MASS_FLUX(I_FUEL) = SF%HRRPUA/ (RN%HOC_COMPLETE*SPECIES_MIXTURE(1)%MASS_FRACTION(FUEL_INDEX))
      ENDIF
...
NAMELIST /CLIP/ MINIMUM_DENSITY,MAXIMUM_DENSITY,FYI,MINIMUM_TEMPERATURE,MAXIMUM_TEMPERATURE
...
NAMELIST /OBST/ XB,ID,PROP_ID,SURF_ID,SURF_IDS,SURF_ID6,FYI,BNDF_FACE,BNDF_OBST, &
                ...
                PERMIT_HOLE,ALLOW_VENT,EVACUATION,MESH_ID,REMOVABLE,MULT_ID,BULK_DENSITY,NOTERRAIN
...
REAL(EB) :: SPREAD_RATE,TRANSPARENCY,XYZ(3),TMP_EXTERIOR,DYNAMIC_PRESSURE, &
            REYNOLDS_STRESS(3,3)=0._EB,L_EDDY,VEL_RMS
...
NAMELIST /VENT/ XB,ID,IOR,MB,PBX,PBY,PBZ,SURF_ID,FYI,RGB,TRANSPARENCY,COLOR, &
                ...
                XYZ,EVACUATION,MESH_ID,SPREAD_RATE,TMP_EXTERIOR,DYNAMIC_PRESSURE,PRESSURE_RAMP, &
                REYNOLDS_STRESS,L_EDDY,N_EDDY,VEL_RMS
...
REAL(EB) :: TEMPERATURE,DENSITY,MASS_FRACTION(1:MAX_SPECIES),RR_SUM,ZZ_GET(1:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
...
NAMELIST /INIT/ XB,TEMPERATURE,DENSITY,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,NUMBER_INITIAL_DROPLETS, &
                PART_ID,SHAPE,DT_INSERT,MULT_ID,PROF_ID,ORIGIN,XYZ,UVW,DIAMETER,SPEC_ID,HRRPUV,DEVC_ID,CTRL_ID,UNIFORM 
            ...
            SPEC_INIT_IF:IF (N_TRACKED_SPECIES > 0._EB .AND. ANY(MASS_FRACTION > ZERO_P)) THEN
               ...
               ELSE 
                  DO NS=1,MAX_SPECIES
                     ...
                     IF (SIMPLE_CHEMISTRY) THEN
                        IF (TRIM(SPEC_ID(NN))=='FUEL' .OR. TRIM(SPEC_ID(NN))==REACTION(1)%FUEL) THEN
                           IN%MASS_FRACTION(I_FUEL)=MASS_FRACTION(NS)
                           CYCLE
                        ENDIF
                     ENDIF
                     DO NS2=0,N_TRACKED_SPECIES
                  ...
                  ZZ_GET = IN%MASS_FRACTION(1:N_TRACKED_SPECIES)
...
LOGICAL :: INITIAL_STATE,LATCH,DRY,TIME_AVERAGED,EVACUATION,HIDE_COORDINATES,RELATIVE
...
! Assign Y_INDEX when SPEC_ID is specified
IF (SPEC_ID/='null') THEN
   DO NS=1,N_SPECIES
      IF (TRIM(SPEC_ID)==TRIM(SPECIES(NS)%ID)) THEN
         IF (.NOT. SPECIES(NS)%PRIMITIVE_ONLY) Y_INDEX = NS
         EXIT
      ENDIF
   ENDDO
   IF (Y_INDEX==-999) THEN
...
IF (TRIM(SPEC_ID)/='null' .AND. (Z_INDEX==-999 .AND. Y_INDEX==-999)) THEN
   IF (SIMPLE_CHEMISTRY) THEN
...
DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
   ...
   IF (QUANTITY2==OUTPUT_QUANTITY(ND)%NAME) THEN
      ...
      IF (OUTPUT_QUANTITY(ND)%SPEC_ID_REQUIRED .AND. (Y_INDEX==-999 .AND. Z_INDEX==-999)) THEN
...
QUANTITY_INDEX_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
  ...
  QUANTITY_IF:IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
     ...
     IF (OUTPUT_QUANTITY(ND)%SMIX_COMPONENT_ONLY .AND. .NOT.SIMPLE_CHEMISTRY) THEN
     ...
     IF (( QUANTITY=='RELATIVE HUMIDITY' .OR. QUANTITY=='HUMIDITY').AND. H2O_INDEX==0) THEN
        WRITE(MESSAGE,'(A)')  'ERROR: RELATIVE HUMIDITY and HUMIDITY require SPEC=WATER VAPOR, or SIMPLE_CHEMISTRY'
        CALL SHUTDOWN(MESSAGE)
     END IF
