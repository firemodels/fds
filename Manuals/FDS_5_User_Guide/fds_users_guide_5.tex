...
\Huge
\flushright{Fire Dynamics Simulator (Version 5) \\ User's Guide \\ (Draft, April 30, 2007)}
...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~1019-5, \pageref{LastPage} pages (March 2007) \\
CODEN: NSPUE2 }
...
The operation of FDS is based on a single input text file containing parameters organized into
{\em namelist}\footnote{A {\em namelist} is a Fortran input record.} groups.
This chapter describes,
with examples, the namelist groups and parameters required to define a fire scenario.
When possible, the examples of input statements have been extracted from input files that can be found in the
Verification or Validation Guide and will be referenced by filename.
...
The first step in performing an FDS simulation is to create an ASCII\footnote{American Standard Character II??} text input file
that provides FDS  with all of the necessary information to
...
\subsection{Non-rectangular Geometry}%Bryan
...
\section{Creating Voids: The \texorpdfstring{{\tt HOLE}}{HOLE} Namelist Group}%Bryan
\label{info:HOLE}
...
\section{Designating Vents on Surfaces: The \texorpdfstring{{\tt VENT}}{VENT} Namelist Group}%Bryan
\label{info:VENT}
...
\section{Coloring Obstructions, Vents, Surfaces and Meshes}%Bryan
\label{info:colors}
...
\subsection{Texture Mapping}%Bryan
...
\subsection{Important Issues Related to Combustion}%Simo
...
\clearpage
\section{Devices and Control Logic}
...
\subsection{Special Devices and their Properties: The \texorpdfstring{{\tt PROP}}{PROP} Namelist Group (Table \ref{tbl:PROP})}
\label{info:PROP}
...
specify a {\ct QUANTITY} and {\ct SETPOINT} directly on the {\ct DEVC} line. More about the idea of a {\em setpoint} can be found in
Sections~\ref{info:openclose} and \ref{info:CTRL}.
...
\subsection{Creating or Removing Obstructions; Opening or Closing Vents}
...
\subsection{Animated Boundary Quantities: The \texorpdfstring{{\tt BNDF}}{BNDF} Namelist Group}%Bryan
\label{info:BNDF}
...
\subsection{Animated Isosurfaces: The \texorpdfstring{{\tt ISOF}}{ISOF} Namelist Group}%Bryan
...
\subsection{Plot3D Static Data Dumps}%Bryan
\label{info:PL3D}
...
{\ct LINK TEMPERATURE}                          & See Section~\ref{info:heatdetector}           & $^\circ$C      & D            \\ \hline
...
%{\ct obscuration}                               & See Section~\ref{info:smoke_detector}         & \%/m           & D            \\ \hline
...
FDS will evalute the reactions in the order they are listed in the input file. %Bryan don't know how to do what you ask %Jason add tech man ref
\begin{description}
...
\section{Stack Effect}%Kevin - more information about how it relates to FDS.
\label{stack}
...
\section{Fine-Tuning Radiation Transport: The \texorpdfstring{{\tt RADI}}{RADI} Namelist Group}%Simo
\label{info:RADI}
...
\chapter{Troubleshooting and Support}%Bryan
...
\section{Support Requests and Bug Tracking}%Bryan
...
\chapter{Compiling FDS}%Bryan - Generalize and move details to website.
...
\section{Sample of Color Definitions}%Bryan
\begin{table}[H]
\begin{center}
