...
MAIN_LOOP: DO  
   ...
   IF (SYNCHRONIZE) THEN
      ...
      IF (USE_MPI) CALL MPI_ALLGATHERV(DT_SYNC(DISPLS(MYID)+1),COUNTS(MYID),MPI_DOUBLE_PRECISION,DT_SYNC,COUNTS,DISPLS, &
   ...
   IF (USE_MPI) CALL MPI_ALLGATHERV(ACTIVE_MESH(DISPLS(MYID)+1),COUNTS(MYID),MPI_LOGICAL,ACTIVE_MESH,COUNTS,DISPLS,MPI_LOGICAL, &
                                    MPI_COMM_WORLD,IERR)
...
ENDDO
IF (USE_MPI) CALL MPI_GATHERV(TUSED(1,DISPLS(MYID)+1),COUNTS_TIMERS(MYID),MPI_DOUBLE_PRECISION,TUSED, &
...
PRESSURE_ITERATION_LOOP: DO
   ...
   IF (USE_MPI) THEN
      CALL MPI_ALLGATHERV(VELOCITY_ERROR_MAX(DISPLS(MYID)+1),COUNTS(MYID),MPI_DOUBLE_PRECISION, &
                          VELOCITY_ERROR_MAX,COUNTS,DISPLS,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
      CALL MPI_ALLGATHERV(VELOCITY_ERROR_MAX_I(DISPLS(MYID)+1),COUNTS(MYID),MPI_INTEGER, &
                          VELOCITY_ERROR_MAX_I,COUNTS,DISPLS,MPI_INTEGER,MPI_COMM_WORLD,IERR)
      CALL MPI_ALLGATHERV(VELOCITY_ERROR_MAX_J(DISPLS(MYID)+1),COUNTS(MYID),MPI_INTEGER, &
                          VELOCITY_ERROR_MAX_J,COUNTS,DISPLS,MPI_INTEGER,MPI_COMM_WORLD,IERR)
      CALL MPI_ALLGATHERV(VELOCITY_ERROR_MAX_K(DISPLS(MYID)+1),COUNTS(MYID),MPI_INTEGER, &
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (N_GAS_SPECIES>0) THEN
      ALLOCATE(M%OMESH(NOM)%  YY(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,N_GAS_SPECIES))
      ALLOCATE(M%OMESH(NOM)% YYS(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,N_GAS_SPECIES))
      DO N=1,N_GAS_SPECIES
      M%OMESH(NOM)%YY(:,:,:,N)  = YY_AMBIENT(N)
      M%OMESH(NOM)%YYS(:,:,:,N) = YY_AMBIENT(N)
      ENDDO
   ENDIF
   ...
   ALLOCATE(M%OMESH(NOM)%WALL(0:M2%N_EXTERNAL_WALL_CELLS))
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZATION_IF: IF (CODE==0) THEN
      ...
      IF (NIC(NM,NOM)>0) THEN
         ...
         ALLOCATE(M3%REAL_RECV_PKG1(IJK_SIZE*(3+N_GAS_SPECIES)))
         ALLOCATE(M3%REAL_RECV_PKG2(IJK_SIZE*(5          )))
         ALLOCATE(M3%REAL_RECV_PKG3(IJK_SIZE*(3+N_GAS_SPECIES)))
...
DO NOM=1,NMESHES
   ...
   IF (CODE==6 .AND. EXCHANGE_EVACUATION .AND. MYID==EVAC_PROCESS .AND. .NOT.EVACUATION_ONLY(NOM)) THEN
      ...
      IF (N_GAS_SPECIES>0) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M4%YY(0,0,0,1),IWW*N_GAS_SPECIES,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
...
RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZE_SEND_IF: IF (CODE==0) THEN
      ...
      IF (NIC(NOM,NM)>0 .AND. RNODE/=SNODE) THEN
         NRA = NUMBER_RADIATION_ANGLES
         NSB = NUMBER_SPECTRAL_BANDS
         ALLOCATE(M3%REAL_SEND_PKG1(IJK_SIZE*(3+N_GAS_SPECIES)))
         ALLOCATE(M3%REAL_SEND_PKG2(IJK_SIZE*(5          )))
         ALLOCATE(M3%REAL_SEND_PKG3(IJK_SIZE*(3+N_GAS_SPECIES)))
      ...
      IF (RNODE/=SNODE) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M%IJKW(1,1),15*M%N_EXTERNAL_WALL_CELLS, MPI_INTEGER,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
         M%OMESH(NOM)%IJKW = M4%IJKW(:,1:M4%N_EXTERNAL_WALL_CELLS)
      ENDIF
         ...
         DO KK=KMIN,KMAX
            DO JJ=JMIN,JMAX
               DO II=IMIN,IMAX
                  ...
                  IF (N_GAS_SPECIES>0) M3%REAL_SEND_PKG1(LL+4:LL+3+N_GAS_SPECIES) = M%YYS(II,JJ,KK,1:N_GAS_SPECIES)
                  LL = LL+3+N_GAS_SPECIES
               ENDDO
            ENDDO
         ENDDO
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M3%REAL_SEND_PKG1(1),IJK_SIZE*(3+N_GAS_SPECIES),MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,&
                        REQ(N_REQ),IERR)
      ELSE
         ...
         IF (N_GAS_SPECIES>0) M2%YYS(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_GAS_SPECIES)= &
                               M%YYS(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_GAS_SPECIES)
      ENDIF
   ENDIF
   ...
   IF (CODE==4 .AND. NIC(NOM,NM)>0) THEN
         ...
         DO KK=KMIN,KMAX
            DO JJ=JMIN,JMAX
               DO II=IMIN,IMAX
                  ...
                  IF (N_GAS_SPECIES>0) M3%REAL_SEND_PKG3(LL+4:LL+3+N_GAS_SPECIES) = M%YY(II,JJ,KK,1:N_GAS_SPECIES)
                  LL = LL+3+N_GAS_SPECIES
               ENDDO
            ENDDO
         ENDDO
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),IJK_SIZE*(3+N_GAS_SPECIES),MPI_DOUBLE_PRECISION,SNODE,TAG,&
      ...
      ELSE
         ...
         IF (N_GAS_SPECIES>0) M2%YY(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_GAS_SPECIES)= &
                               M%YY(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_GAS_SPECIES)
      ENDIF
   ENDIF
   ...
   IF (CODE==0 .OR. CODE==6) THEN
      IF (RNODE/=SNODE) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M%BOUNDARY_TYPE(0),M%N_EXTERNAL_WALL_CELLS+1,MPI_INTEGER,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
         M2=>MESHES(NOM)%OMESH(NM)
         M2%BOUNDARY_TYPE(0:M%N_EXTERNAL_WALL_CELLS) = M%BOUNDARY_TYPE(0:M%N_EXTERNAL_WALL_CELLS)
      ENDIF
   ENDIF
...
DO NM=1,NMESHES
   ...
   IF (USE_MPI .AND. CODE==6 .AND. EXCHANGE_EVACUATION .AND. MYID/=EVAC_PROCESS .AND. PROCESS(NM)==MYID .AND. &
      ...
      IF (N_GAS_SPECIES>0) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M%YY(0,0,0,1),IWW*N_GAS_SPECIES,MPI_DOUBLE_PRECISION,EVAC_PROCESS,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      ...
      DO KK=KMIN,KMAX
         DO JJ=JMIN,JMAX
            DO II=IMIN,IMAX
               ...
               IF (N_GAS_SPECIES>0) M2%YYS(II,JJ,KK,1:N_GAS_SPECIES)= M2%REAL_RECV_PKG1(LL+4:LL+3+N_GAS_SPECIES)
               LL = LL+3+N_GAS_SPECIES
            ENDDO
         ENDDO
      ENDDO
      ...
      DO KK=KMIN,KMAX
         DO JJ=JMIN,JMAX
            DO II=IMIN,IMAX
               ...
               IF (N_GAS_SPECIES>0) M2%YY(II,JJ,KK,1:N_GAS_SPECIES)= M2%REAL_RECV_PKG3(LL+4:LL+3+N_GAS_SPECIES)
               LL = LL+3+N_GAS_SPECIES
            ENDDO
         ENDDO
      ENDDO
...
IF_DUMP_HRR: IF (T>=HRR_CLOCK) THEN
   IF (USE_MPI) CALL MPI_ALLGATHERV(HRR_TIME_INTERVAL(DISP),CNT,MPI_DOUBLE_PRECISION,HRR_TIME_INTERVAL,COUNTS,DISPLS, &
                                    MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
...
IF (N_TREES_OUT > 0 .AND. T > VEG_CLOCK) THEN
      ...
      IF (USE_MPI) CALL MPI_GATHERV(TREE_OUTPUT_DATA(1,1,DISPLS(MYID)+1),COUNTS_TREE(MYID),MPI_DOUBLE_PRECISION, &
...
IF_DUMP_MASS: IF (T>=MINT_CLOCK) THEN
   IF (USE_MPI) CALL MPI_ALLGATHERV(MINT_TIME_INTERVAL(DISP),CNT,MPI_DOUBLE_PRECISION,MINT_TIME_INTERVAL,COUNTS,DISPLS, &
                                    MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
   IF (MINVAL(MINT_TIME_INTERVAL,MASK=.NOT.EVACUATION_ONLY)>0.) THEN
      IF (USE_MPI) CALL MPI_GATHERV(MINT_SUM(0,DISP),COUNTS_MASS(MYID),MPI_DOUBLE_PRECISION,MINT_SUM,COUNTS_MASS,DISPLS_MASS, &
                                    MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD, IERR)
...
DO NM=1,NMESHES
   IF (USE_MPI .AND. MYID==EVAC_PROCESS .AND. .NOT.EVACUATION_ONLY(NM)) THEN
      ...
      IF (N_GAS_SPECIES>0) THEN
         ALLOCATE(M%YY(0:M%IBP1,0:M%JBP1,0:M%KBP1,N_GAS_SPECIES),STAT=IZERO)
...
IF (ICYC > 0) THEN ! Syncrhonize evacuation and fire clocks, if both type meshes present
   IF (.NOT.ALL(EVACUATION_ONLY)) THEN
      ...
      IF (USE_MPI) CALL MPI_ALLGATHERV(DT_NEXT_SYNC(DISPLS(MYID)+1),COUNTS(MYID),MPI_DOUBLE_PRECISION, &
...
USE HVAC_ROUTINES, ONLY: NODE_CP,NODE_P,NODE_TMP,NODE_RHO,NODE_RSUM,NODE_YY, &
                         LEAK_CP,LEAK_TMP,LEAK_RHO,LEAK_RSUM,LEAK_YY
...
CALL MPI_GATHERV(NODE_P(1,DISPLS(MYID)+1),COUNTS_HVAC(MYID),MPI_DOUBLE_PRECISION,NODE_P, &
                    COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_GATHERV(NODE_TMP(1,DISPLS(MYID)+1),COUNTS_HVAC(MYID),MPI_DOUBLE_PRECISION,NODE_TMP, &
                    COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_GATHERV(NODE_RHO(1,DISPLS(MYID)+1),COUNTS_HVAC(MYID),MPI_DOUBLE_PRECISION,NODE_RHO, &
                    COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_GATHERV(NODE_CP(1,DISPLS(MYID)+1),COUNTS_HVAC(MYID),MPI_DOUBLE_PRECISION,NODE_CP, &
                    COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_GATHERV(NODE_RSUM(1,DISPLS(MYID)+1),COUNTS_HVAC(MYID),MPI_DOUBLE_PRECISION,NODE_RSUM, &
                    COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_GATHERV(NODE_YY(1,1,DISPLS(MYID)+1),COUNTS_HVAC(MYID)*N_GAS_SPECIES,MPI_DOUBLE_PRECISION,NODE_YY, &
                    COUNTS_HVAC*N_GAS_SPECIES,DISPLS_HVAC*N_GAS_SPECIES,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)

IF (LEAK_DUCTS > 0) THEN
   COUNTS_LEAK = COUNTS * (N_ZONE+1)**2
   DISPLS_LEAK = DISPLS * (N_ZONE+1)**2
   CALL MPI_GATHERV(LEAK_TMP(0,0,DISPLS(MYID)+1),COUNTS_LEAK(MYID),MPI_DOUBLE_PRECISION,LEAK_TMP, &
                     COUNTS_LEAK,DISPLS_LEAK,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
   CALL MPI_GATHERV(LEAK_RHO(0,0,DISPLS(MYID)+1),COUNTS_LEAK(MYID),MPI_DOUBLE_PRECISION,LEAK_RHO, &
                     COUNTS_LEAK,DISPLS_LEAK,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
   CALL MPI_GATHERV(LEAK_CP(0,0,DISPLS(MYID)+1),COUNTS_LEAK(MYID),MPI_DOUBLE_PRECISION,LEAK_CP, &
                     COUNTS_LEAK,DISPLS_LEAK,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
   CALL MPI_GATHERV(LEAK_RSUM(0,0,DISPLS(MYID)+1),COUNTS_LEAK(MYID),MPI_DOUBLE_PRECISION,LEAK_RSUM, &
                     COUNTS_LEAK,DISPLS_LEAK,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
   CALL MPI_GATHERV(FDS_LEAK_AREA(0,0,DISPLS(MYID)+1),COUNTS_LEAK(MYID),MPI_DOUBLE_PRECISION,FDS_LEAK_AREA, &
                     COUNTS_LEAK,DISPLS_LEAK,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)                     
   CALL MPI_GATHERV(LEAK_YY(0,0,1,DISPLS(MYID)+1),COUNTS_LEAK(MYID)*N_GAS_SPECIES,MPI_DOUBLE_PRECISION,LEAK_YY, &
                     COUNTS_LEAK*N_GAS_SPECIES,DISPLS_LEAK*N_GAS_SPECIES,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF
...
USE HVAC_ROUTINES, ONLY: NODE_TMP,NODE_RHO,DUCT_U,NODE_YY
! Exchange information mesh to mesh needed for performing the HVAC computation
...
CALL MPI_SCATTER(NODE_TMP(1,1),SIZE(NODE_TMP),MPI_DOUBLE_PRECISION,NODE_P, &
                    SIZE(NODE_P),MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_SCATTER(NODE_RHO(1,1),SIZE(NODE_RHO),MPI_DOUBLE_PRECISION,NODE_P, &
                    SIZE(NODE_RHO),MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_SCATTER(DUCT_U(1,1),SIZE(DUCT_U),MPI_DOUBLE_PRECISION,DUCT_U, &
                    SIZE(DUCT_U),MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_SCATTER(NODE_YY(1,1,1),SIZE(NODE_P),MPI_DOUBLE_PRECISION,NODE_YY, &
                    SIZE(NODE_YY),MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
