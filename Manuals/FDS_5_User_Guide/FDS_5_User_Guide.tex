...
&OBST ID='BURNER', XB=2.3,4.5,1.3,4.8,0.0,9.2, SURF_IDS='FIRE','INERT','INERT' /
\end{verbatim}
\normalsize

\noindent
puts a fire on top of the obstruction. This is a simple way of
prescribing a burner. Note that the {\ct ID} string helps to identify the obstruction line in cases
where there might be thousands of them. If there is a problem with the syntax of the line, FDS can
print out a message using the {\ct ID} rather than the number.
...
&VENT XB=1.0,2.0,2.0,2.0,1.0,3.0, SURF_ID='hot patch', ID='heater' /
\end{verbatim} \normalsize
...
underlying obstruction. Also note that the {\ct ID} string on the {\ct VENT} line helps to identify it in cases
where there might be many of them. If there is a problem with the syntax of the line, FDS can
print out a message using the {\ct ID} rather than the number.
...
\hline
Species &   Mol.~Wgt.        & $\sigma$  & $k/\epsilon$   & Liquid \\
        &   (g/mol)          & (\AA)     & (K)            &        \\ \hline
\hline
...
to the {\ct PART} line to indicate the number of particles/droplets within the computational domain at the start of the simulation.
...
If there is more than one rectangular volume in which you want to introduce initial droplets or particles, use {\ct INIT} lines of the form:
...
\noindent
If you want to introduce droplets or particles within a given volume periodically in time and not just at the initial time, set {\ct DT\_INSERT} to a positive value
...
{\ct FYI}                 & Character           & Comment String (has no effect)        &    &               \\ \hline
{\ct ID}                  & Character           & ID string for error messages          &    &               \\ \hline
...
{\ct SMOKEVIEW\_PARAMETERS}             & Real Array    & Misc.~parameters for drawing              &                       &           \\ \hline
...
{\ct STATE\_INDEX}                      & Integer       & State code for Smokeview drawing          &                       & 0         \\ \hline
