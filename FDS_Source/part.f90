...
REAL(EB) :: RHO_G,RVC,RDS,RDC,QREL,SFAC,UREL,VREL,WREL,TMP_G,RN,THETA_RN, &
            RD,T,C_DRAG,XI,YJ,ZK,MU_AIR,WE_G,DROP_VOL_FRAC,M_DROP,DROP_DEN, &
...
!$OMP PRIVATE(TMP_G,RHO_G,MU_AIR,C_DRAG,WE_G,T_BU_BAG,T_BU_STRIP,DROP_VOL_FRAC,M_DROP,DROP_DEN) &
...
DROPLET_LOOP: DO I=1,NLP  
   ...
   DRAG_CALC: IF (DR%IOR==0 .AND. DR%RE>0) THEN
      ...
      ! Drag reduction
!      M_DROP        = PC%FTPR*RD**3
!      DROP_DEN      = AVG_DROP_DEN(II,JJ,KK,PC%EVAP_INDEX) + DR%PWT*M_DROP*RVC
