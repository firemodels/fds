...
PRESSURE_ITERATION_LOOP: DO
   ...
   IF (ITERATE_BAROCLINIC_TERM .OR. PRESSURE_ITERATIONS==1 .OR. (CC_IBM .AND. .NOT.CC_STRESS_METHOD)) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      IF (INSEPARABLE_POISSON.AND.(PRES_FLAG==SCARC_FLAG.OR.PRES_FLAG==USCARC_FLAG)) THEN
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      SELECT CASE(PRES_FLAG)
         ...
         CASE (UGLMAT_FLAG,ULMAT_FLAG,USCARC_FLAG)
...
MESH_LOOP_1: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_2: DO NOM=1,NMESHES
      IF (PROCESS(NM)==PROCESS(NOM)) CYCLE MESH_LOOP_2
      M3 => M%OMESH(NOM)
      IF (M3%N_WALL_CELLS_SEND==0 .AND. M3%N_WALL_CELLS_RECV==0) CYCLE MESH_LOOP_2
      IF (M3%N_WALL_CELLS_SEND>0) ALLOCATE(M3%WALL_STORAGE_SEND(N_SURF))
      IF (M3%N_WALL_CELLS_RECV>0) ALLOCATE(M3%WALL_STORAGE_RECV(N_SURF))
      IF (M3%N_WALL_CELLS_SEND>0 .OR. M3%N_WALL_CELLS_RECV>0) CALL ALLOCATE_BOUNDARY_TYPES(NOM,50)
      DO SURF_INDEX=1,N_SURF
         SF => SURFACE(SURF_INDEX)
         ...
         IF (SLOTS>0) THEN
            ...
            DO I=1,M3%N_WALL_CELLS_RECV
               ...
               CALL ALLOCATE_STORAGE(NOM,WALL_INDEX=I,SURF_INDEX=SURF_INDEX)
            ENDDO
         ENDIF
      ENDDO
   ENDDO MESH_LOOP_2
      ...
      DO SURF_INDEX=1,N_SURF
         ...
         IF (SLOTS>0) THEN
            ...
            DO I=1,M3%N_THIN_WALL_CELLS_RECV
               ...
               CALL ALLOCATE_STORAGE(NOM,THIN_WALL_INDEX=I,SURF_INDEX=SURF_INDEX)
            ENDDO
         ENDIF
      ENDDO
   ENDDO MESH_LOOP_2B
...
INTEGER :: NNN,NN1,NN2,IPC,CNT,II1,II2,JJ1,JJ2,KK1,KK2,NQT2,NN,IOR,NRA,NRA_MAX,AIC,OBST_INDEX,IP,SURF_INDEX
      ...
      RECEIVE_BACK_WALL: IF (CODE==6 .AND. SNODE/=RNODE) THEN
         DO SURF_INDEX=1,N_SURF
            ...
            DO I=1,M2%N_WALL_CELLS_RECV
               ...
               II = II + 1
               CALL PACK_WALL(NM,I,SURF_INDEX,II,UNPACK_IT=.TRUE.,NOM=NOM)
            ENDDO
         ENDDO
      ENDIF RECEIVE_BACK_WALL

      RECEIVE_BACK_THIN_WALL: IF (CODE==6 .AND. SNODE/=RNODE) THEN
         DO SURF_INDEX=1,N_SURF
            II = 0
            DO I=1,M2%N_THIN_WALL_CELLS_RECV
               IF (M2%THIN_WALL_SURF_INDICES_RECV(I)/=SURF_INDEX) CYCLE
               II = II + 1
               CALL PACK_THIN_WALL(NM,I,SURF_INDEX,II,UNPACK_IT=.TRUE.,NOM=NOM)
            ENDDO
         ENDDO
