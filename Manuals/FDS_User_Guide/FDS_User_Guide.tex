...
\begin{warning}
\noindent Note that versions of FDS from 5.0 through 5.3 used a slightly different definition of
{\ct REFERENCE\_TEMPERATURE} and {\ct REFERENCE\_RATE}. This will lead to different results when using the
...
\noindent
Notice that in addition to $k$, $\rho$, $c$ and $\epsilon$, there is one reaction specified, the {\em yield} of which is 100~\% fuel gas ({\ct NU\_FUEL=1.}).
...
The gas phase reaction can be described in two ways. By default, a so-called {\em mixture controlled} model is used to account for the evolution of
...
The chemical composition of the fuel is specified using the {\ct REAC} namelist group. For the default mixture fraction
...
\item[{\ct C, H, O, N, OTHER}] The fuel chemical formula. All numbers are positive.  One of either C or H must be specified.
...
can be specified using the parameters {\ct NU\_GAS(j,k)} and {\ct SPEC\_ID(k)}, where {\ct j} refers to the {\ct j}'th reaction
of the material and {\ct k} refers to the {\ct k}'th gas species in {\ct SPEC\_ID(k)}.
...
For consistency, the {\ct HEAT\_OF\_COMBUSTION(j)} can also be specified for each reaction. These values are
used only if the corresponding heats of combustion for the gaseous
species are greater than zero.
...
&MATL ID = 'WOOD'
      ...
      N_S                     = 1.0
      NU_RESIDUE              = 0.18
      SPEC_ID(1:4) = 'PYROLYZATE','OXYGEN','WATER VAPOR','CARBON DIOXIDE'
      NU_GAS(1,1:4)           = 0.82,0,0,0
      HEAT_OF_REACTION        = 430.
      HEAT_OF_COMBUSTION      = 14500.
      RESIDUE                 = 'CHAR' /
\end{verbatim} \normalsize
...
\item[{\ct FUEL}] Character string indicating which of the listed gas species is the fuel.
\item[{\ct NU}] Array containing the stoichiometry of the chemical reaction for each {\ct SPEC} where negative values indicate reactants and positive values indicate products.
\item[{\ct SPEC\_ID}] Array containing the names of the species that participate in the reaction.
\item[{\ct HEAT\_OF\_COMBUSTION}] The effective heat of combustion of the {\ct FUEL} in chemical reaction in units of kJ/kg. (No default)
\end{description}
...
In many simulations there are sets of species that are created and transported in the same manner. In this case, tracking each species individually uses computational resources and time that need not be spent. For example, one could use the {\ct SPEC} namelist group to define the background species of air as a combination of nitrogen, oxygen, water vapor, and carbon dioxide. However, this would require tracking three species each of which would require memory and time. Alternatively, if one could create a lumped species that represented that composition and tracked only that species, then both time and memory could be saved. This can be accomplished with the {\ct SMIX} namelist group.

The following inputs:

\footnotesize
\begin{verbatim}
&SPEC ID = 'NITROGEN',BACKGROUND_ONLY=.TRUE./
&SPEC ID = 'OXYGEN', MASS_FRACTION_0 = 0.23054 /
&SPEC ID = 'WATER VAPOR', MASS_FRACTION = 0.00626/
&SPEC ID = 'CARBON DIOXIDE', MASS_FRACTION = 0.00046 /
\end{verbatim} \normalsize
...
&SPEC ID = 'WATER VAPO'R,LUMPED_SPECIES_ONLY=.TRUE./
&SPEC ID = 'CARBON DIOXIDE',LUMPED_SPECIES_ONLY=.TRUE./

&SMIX ID = 'AIR', BACKGROUND=.TURE.,
      SPEC_ID ='NITROGEN','OXYGEN','WATER VAPOR','CARBON DIOXIDE'
      MASS_FRACTION = 0.7624,0.23054,0.00046,0.76274/
\end{verbatim} \normalsize

The keyword {\ct LUMPED\_SPECIES\_ONLY} on {\ct SPEC} denotes that the species is only present as part of a lumped species.  When this flag is set, FDSwill not allocate space to track that species individually.
...
\item[{\ct ID}] Character string identitfying the name of the species.  If no {\ct ID} is given the background lumped species will be named LUMPED SPECIES 0 and any remaining lumped species will be named LUMPED SPECIES N where N is the position of the the input following any background lumped species.
...
\item[{\ct MASS\_FRACTION}] The mass fractions of the species making up the lumped species.  FDS will normalize the values to 1.  Specify only one of {\ct MASS\_FRACTION} or {\ct VOLUME\_FRACTION}.
\item[{\ct VOLUME\_FRACTION}]  The volume fractions of the species making up the lumped species.  FDS will normalize the values to 1.  Specify only one of {\ct MASS\_FRACTION} or {\ct VOLUME\_FRACTION}.
\end{description}

Any FDS input that requires a {\ct SPEC\_ID} can also use the {\ct ID} of an {\ct SMIX}.
...
A useful option for the {\ct DEVC} line is to add {\ct CHANGE\_ONLY=.TRUE.}, which will indicate that only the change in
...
{\ct CHANGE\_ONLY}     & Logical         & See Section~\ref{info:out:DEVC}                                  &       & {\ct .FALSE.} \\ \hline
...
{\ct DUCT\_ID}         & Character       & Associated {\ct HVAC} line for a duct output                     &       &               \\ \hline
{\ct DEPTH}            & Real            & Depth into surface for internal wall temp                        & m     & 0             \\ \hline
{\ct FLOWRATE}         & Real            & Suction flow rate for an aspiration detector                     & kg/s  & 0             \\ \hline
{\ct FYI}              & Character       & Comment String (has no effect)                                   &       &               \\ \hline
...
{\ct NODE\_ID}         & Character(2)    & Associated {\ct HVAC} line for a node output                     &       &               \\ \hline
...
{\ct PLOT3D\_VELO\_INDEX}           & Integer~Quint & See Section~\ref{info:velocity}                                  &       &  0      \\ \hline
...
{\ct HEAT\_OF\_COMBUSTION(:)} & Real array  & Heats of Combustion for each reaction     & kJ/kg             &        \\ \hline
{\ct HEAT\_OF\_REACTION(:)}   & Real array  & Heats of Reaction for each reaction       & kJ/kg             & 0.     \\ \hline
...
{\ct NU\_WATER}               & Real array  & Water vapor yields                        & kg/kg             & 0.     \\ \hline
{\ct PCR(:)}                  & Logical array & Phase-change reaction (\ref{info:solid_pyrolysis}) &         & {\ct.FALSE.}\\ \hline
...
{\ct SPEC\_ID}          & Character        & {\ct IDs} of gas species in {\ct NU\_GAS}           &                   &        \\ \hline
...
\hline \hline
\endhead
{\ct AUTO\_IGNITION\_TEMPERATURE}         & Real        & Auto ignition temperature                     &   K               &  0  \\ \hline
...
{\ct HRRPUA\_SHEET}                       & Real        & See Section~\ref{info:turbulent_combustion}   & kW/m$^2$          & 0 (LES); 200 (DNS)    \\ \hline
{\ct HRRPUV\_AVERAGE}                     & Real        & See Section~\ref{info:turbulent_combustion}   & kW/m$^3$          & 2500 (LES); 0 (DNS)    \\ \hline
...
{\ct LUMPED\_SPECIES\_ONLY}         & Logical     & Species is only part of an {\ct SMIX}       &                   & {\ct .FALSE.} \\ \hline
...
{\ct VOLUME\_FRACTION}              & Real Array  & Volune fraction of mixture components                             &                   &     \\ \hline
\end{longtable}
