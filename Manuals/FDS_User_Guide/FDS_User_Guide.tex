...
A diagnostic tool for the time integration of chemical reactions is the number of subiterations the ODE solver takes for a given FDS time step. For infinitely fast chemistry the number of {\ct CHEMICAL SUBITERATIONS} should always be 1. For finite rate chemical reactions the number can vary between 1 and the minimum between {\ct MAX\_CHEMISTRY\_ITERATIONS} and the number needed to satisfy the error tolerance of the ODE solver. If the maximum value is reached, the user can increase the number of iterations or decrease reduce the error tolerate constraints of the integrator.
...
{\ct CHEMICAL SUBITERATIONS}                    & Section~\ref{info:chem_integration}               &                & D,S          \\ \hline
