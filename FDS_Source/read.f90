...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,PRESSURE_RELAX_FACTOR,RELAXATION_FACTOR,FYI, &
                ...
                NEW_PARTICLE_METHOD,WFDS,PATCH_VELOCITY,OVERWRITE,ENSEMBLE_MEAN_CLOSURE,C_EMC,UVW_FILE
...
IF (ENSEMBLE_MEAN_CLOSURE) DYNSMAG=.FALSE.
...
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,OXIDIZER,EPUMO2,ID, N_S,&
                ...
                CHECK_CHEMICAL_TIME_SCALE,NEW_MIX_TIME
...
INTEGER :: SAMPLING_FACTOR,DROPLETS_PER_SECOND,N,NN,IPC,RGB(3),I_DUMMY,NUMBER_INITIAL_DROPLETS,IPC_0, &
           NUMBER_INDEPENDENT_DIRECTIONS
...
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC,PC2
TYPE(PROPERTY_TYPE), POINTER :: PY
NAMELIST /PART/ FYI,DROPLETS_PER_SECOND,SAMPLING_FACTOR,ID,STATIC,MASSLESS,FUEL,WATER,TREE, &
                ...
                DENSE_VOLUME_FRACTION,NUMBER_INDEPENDENT_DIRECTIONS,ORIENTATION
...
! Count the number of split particles

COUNT_PART_LOOP2: DO
   CALL CHECKREAD('PART',LU_INPUT,IOS) 
   IF (IOS==1) EXIT COUNT_PART_LOOP2
   READ(LU_INPUT,PART,END=221)
   IF (NUMBER_INDEPENDENT_DIRECTIONS>1) N_PART = N_PART + NUMBER_INDEPENDENT_DIRECTIONS - 1
ENDDO COUNT_PART_LOOP2
221 REWIND(LU_INPUT)
...
READ_PART_LOOP: DO N=1,N_PART
   ...
   NUMBER_INDEPENDENT_DIRECTIONS = 1
   GAMMA_D                  = 2.4_EB
   SIGMA_D                  = -99999.9_EB
   AGE                      = 1.E6_EB      ! s
   ID                       = 'null'
   ...
   PC%ORIENTATION(1:3)   = ORIENTATION(1:3)/SQRT(ORIENTATION(1)**2+ORIENTATION(2)**2+ORIENTATION(3)**2)
   PC%PROP_ID            = PROP_ID
   PC%GAMMA              = GAMMA_D
   ...
   PC%SPLIT_PART_INDEX = IPC
   PC%N_SPLIT                  = NUMBER_INDEPENDENT_DIRECTIONS
   IF (PC%N_SPLIT>1) THEN
      IPC_0 = IPC
      ! Here we need a loop over N_SPLIT. Now assuming N_SPLIT = 2
      IPC = IPC + 1
      PC%SPLIT_PART_INDEX = IPC
      PC2=>PARTICLE_CLASS(IPC)
      PC2 = PC
      PC2%ORIENTATION = -1._EB * PC%ORIENTATION
      PC2%SPLIT_PART_INDEX = IPC_0
   ENDIF
