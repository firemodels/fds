...
DO N=0,N_TRACKED_SPECIES
   ...
   DO NN=1,SM%N_SUB_SPECIES
      Z2Y(SM%Y_INDEX(NN),N) = SM%MASS_FRACTION(NN)
   ENDDO
ENDDO

! Compute effective gas properties of the species mixtures
...
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,EPUMO2,ID, N_S, &
                Y_O2_INFTY,Y_F_INLET, CO_PRODUCTION,SOOT_DEPOSITION,SUPPRESSION, &
...
INTEGER :: N, J, ITMP
REAL(EB) :: H_L,H_V,ZZ_GET(0:N_TRACKED_SPECIES),CPBAR,H_G_S,H_G_S_REF,H_L_REF,TMP_REF,TMP_MELT,TMP_V,TMP_WGT
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC=>NULL()
