...
!    ENDIF
   ...
   IF (HVAC_SOLVE .AND. .NOT.EVACUATION_SKIP(1)) CALL HVAC_CALC(T(1),.TRUE.)
...
!    ENDIF
   ...
   VELOCITY_BC_LOOP_2: DO NM=1,NMESHES
      ...
      CALL VELOCITY_BC(T(NM),NM)
      IF (.NOT.EVACUATION_ONLY(NM)) CALL UPDATE_GLOBAL_OUTPUTS(T(NM),NM)
...
PRESSURE_ITERATION_LOOP: DO
   ...
   DO NM=1,NMESHES
      IF (EVACUATION_ONLY(NM) .OR. EVACUATION_SKIP(NM)) CYCLE
      IF (PROCESS(NM)==MYID) CALL COMPUTE_VELOCITY_ERROR(NM)
   ENDDO
...
DO NM=1,NMESHES
   ...
   IF (EVACUATION_ONLY(NM)) THEN
      IF(EMESH_INDEX(NM)>0) N_TIMERS_TMP = N_TIMERS_EVAC - 3
   END IF
...
ELSE
   ...
   T_TMP = T_EVAC
END IF
IF (.NOT.ANY(EVACUATION_ONLY)) RETURN ! No main evacuation meshes
...
DO NM=1,NMESHES
   IF (MYID/=MAX(0,EVAC_PROCESS).OR..NOT.EVACUATION_SKIP(NM).OR.EMESH_INDEX(NM)==0) CYCLE
   CALL UPDATE_GLOBAL_OUTPUTS(T(NM),NM)      
ENDDO
...
IF (ICYC > 0) THEN ! Syncrhonize evacuation and fire clocks, if both type meshes present
   IF (.NOT.ALL(EVACUATION_ONLY)) THEN
      T_FIRE = MINVAL(T, MASK=(.NOT.EVACUATION_ONLY))
      DO NM=1,NMESHES
      ...
      FIRE_DT = MINVAL(DT_NEXT_SYNC, MASK=(.NOT.EVACUATION_ONLY))
   ...
   DO NM = 1, NMESHES
      IF (EVACUATION_ONLY(NM)) THEN
         ...
         IF (EMESH_INDEX(NM)>0) THEN
            ...
            IF (PROCESS(NM)==MYID .AND. EVACUATION_SKIP(NM)) NTCYC(NM) = NTCYC(NM) + 1
