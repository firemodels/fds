...
In general only one species for a given {\ct ID} can be defined; however, you may wish to model multiple inlet streams of a species and be able to identify how well the streams are mixing.  This can be done by using the character string {\ct ALIAS} on the {\ct SPEC} line.  The {\ct ID} must exist on a previously given {\ct SPEC} input.  For example, the lines:
...
\begin{verbatim}
&DEVC XYZ=..., QUANTITY='MASS FRACTION', SPEC_ID='CO2 1', ID='Device 1'/
&DEVC XYZ=..., QUANTITY='MASS FRACTION', SPEC_ID='CARBON MONOXIDE', ID='Device 2'/
\end{verbatim} \normalsize
