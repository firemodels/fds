...
USE CC_SCALARS_IBM, ONLY : CCREGION_COMPUTE_KRES,CCREGION_COMPUTE_VISCOSITY,CUTFACE_VELOCITIES
REAL(EB), INTENT(IN) :: T
...
USE CC_SCALARS_IBM, ONLY : CCIBM_VELOCITY_FLUX,ROTATED_CUBE_VELOCITY_FLUX,CCIBM_INTERP_FACE_VEL,CUTFACE_VELOCITIES,&
                           CCIBM_VELOCITY_BC
INTEGER, INTENT(IN) :: NM
...
IF (CC_IBM) THEN
   ...
   ELSE
      CALL CCIBM_VELOCITY_BC(T,NM,APPLY_TO_ESTIMATED_VARIABLES,DO_IBEDGES=.FALSE.)
      T_NOW=CURRENT_TIME()
      CALL CUTFACE_VELOCITIES(NM,UU,VV,WW,CUTFACES=.TRUE.)
      T_USED(14) = T_USED(14) + CURRENT_TIME() - T_NOW
   ENDIF
...
IF (CC_IBM) THEN
   ...
   ELSE
      ...
      T_USED(14) = T_USED(14) + CURRENT_TIME() - T_NOW
   ENDIF
   CALL CCIBM_VELOCITY_FLUX(NM,DT,APPLY_TO_ESTIMATED_VARIABLES,RHOP,GX,GY,GZ)
   T_NOW=CURRENT_TIME()
ENDIF
...
NO_SCARC_IF: IF (PRES_FLAG /= SCARC_FLAG .AND. PRES_FLAG /= USCARC_FLAG) THEN
...
USE CC_SCALARS_IBM, ONLY : CCIBM_VELOCITY_NO_GRADH, CCIBM_PROJECT_VELOCITY
...
ELSE FREEZE_VELOCITY_IF
   ...
   IF (CC_IBM) THEN
      T_USED(4)=T_USED(4)+CURRENT_TIME()-T_NOW
      CALL CCIBM_PROJECT_VELOCITY(NM,DT,STORE_FLG=.FALSE.)
      T_NOW=CURRENT_TIME()
   ENDIF
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG,USCARC_FLAG); CALL WALL_VELOCITY_NO_GRADH(DT,.FALSE.)
   END SELECT
   IF (CC_IBM) THEN
      T_USED(4)=T_USED(4)+CURRENT_TIME()-T_NOW
      CALL CCIBM_VELOCITY_NO_GRADH(DT,.FALSE.)
      T_NOW=CURRENT_TIME()
   ENDIF
...
ELSE FREEZE_VELOCITY_IF
   ...
   IF (CC_IBM) THEN
      T_USED(4)=T_USED(4)+CURRENT_TIME()-T_NOW
      CALL CCIBM_PROJECT_VELOCITY(NM,DT,.TRUE.)
      T_NOW=CURRENT_TIME()
   ENDIF
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG,USCARC_FLAG)
         CALL WALL_VELOCITY_NO_GRADH(DT,.TRUE.)                    ! Store U velocities on OBST surfaces.
         IF (CC_IBM) THEN
            T_USED(4)=T_USED(4)+CURRENT_TIME()-T_NOW
            CALL CCIBM_VELOCITY_NO_GRADH(DT,.TRUE.)                ! Store velocities on GEOM SOLID faces.
            T_NOW=CURRENT_TIME()
         ENDIF
   END SELECT
   ...
   IF (CC_IBM) THEN
      T_USED(4)=T_USED(4)+CURRENT_TIME()-T_NOW
      CALL CCIBM_PROJECT_VELOCITY(NM,DT,.FALSE.)
      T_NOW=CURRENT_TIME()
   ENDIF
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG,USCARC_FLAG)
...
EDGE_LOOP: DO IE=1,N_EDGES
   ...
   SIGN_LOOP: DO I_SGN=-1,1,2
      ORIENTATION_LOOP: DO IS=1,3
         ...
         WCM_ONE_D => BOUNDARY_ONE_D(WCM%OD_INDEX)
         WCP_ONE_D => BOUNDARY_ONE_D(WCP%OD_INDEX)
            ...
            ELSE VEL_T_IF
               ...
               IF (HVAC_TANGENTIAL) THEN
                  ...
                  IEC_SELECT: SELECT CASE(IEC) ! edge orientation
                     CASE (1)
                        IF (ICD==1) VEL_T = 0.5_EB*ABS((WCM_ONE_D%U_NORMAL_S+WCP_ONE_D%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(3)
                        IF (ICD==2) VEL_T = 0.5_EB*ABS((WCM_ONE_D%U_NORMAL_S+WCP_ONE_D%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(2)
                     CASE (2)
                        IF (ICD==1) VEL_T = 0.5_EB*ABS((WCM_ONE_D%U_NORMAL_S+WCP_ONE_D%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(1)
                        IF (ICD==2) VEL_T = 0.5_EB*ABS((WCM_ONE_D%U_NORMAL_S+WCP_ONE_D%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(3)
                     CASE (3)
                        IF (ICD==1) VEL_T = 0.5_EB*ABS((WCM_ONE_D%U_NORMAL_S+WCP_ONE_D%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(2)
                        IF (ICD==2) VEL_T = 0.5_EB*ABS((WCM_ONE_D%U_NORMAL_S+WCP_ONE_D%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(1)
                  END SELECT IEC_SELECT
               ELSE
                  IF (SF%PROFILE/=0 .AND. SF%VEL>TWO_EPSILON_EB) &
                     PROFILE_FACTOR = ABS(0.5_EB*(WCM_ONE_D%U_NORMAL_0+WCP_ONE_D%U_NORMAL_0)/SF%VEL)
                  IF (SF%RAMP_INDEX(VELO_PROF_Z)>0) PROFILE_FACTOR = EVALUATE_RAMP(ZC(KK),SF%RAMP_INDEX(VELO_PROF_Z))
                  RAMP_T = EVALUATE_RAMP(TSI,SF%RAMP_INDEX(TIME_VELO),TAU=SF%TAU(TIME_VELO))
...
IF(CC_IBM) CALL CCIBM_VELOCITY_BC(T,NM,APPLY_TO_ESTIMATED_VARIABLES,DO_IBEDGES=.TRUE.)
...
USE CC_SCALARS_IBM, ONLY : CCREGION_MATCH_VELOCITY
...
IF(CC_IBM .AND. CC_UNSTRUCTURED_PROJECTION) THEN
   CALL CCREGION_MATCH_VELOCITY(NM,PREDICTOR,.TRUE.)
   RETURN
ENDIF
...
USE CC_SCALARS_IBM, ONLY : CCREGION_MATCH_VELOCITY_FLUX
INTEGER  :: NOM,II,JJ,KK,IOR,IW,IIO,JJO,KKO
INTEGER, INTENT(IN) :: NM
...
IF(CC_IBM .AND. CC_UNSTRUCTURED_PROJECTION) THEN
   CALL CCREGION_MATCH_VELOCITY_FLUX(NM)
   RETURN
ENDIF
...
USE CC_SCALARS_IBM, ONLY : CHECK_CFLVN_LINKED_CELLS
...
HEAT_TRANSFER_IF: IF (CHECK_HT) THEN
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      JJG = BC%JJG
      KKG = BC%KKG
      UVW = (ABS(ONE_D%Q_CON_F)/ONE_D%RHO_F)**ONTH * 2._EB*ONE_D%RDN
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL=IIG
         JCFL=JJG
