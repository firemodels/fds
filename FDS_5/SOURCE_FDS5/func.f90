...
REAL(EB) :: Z_1,Z_2,Z_3,Y_MF,ZZ,OMYYSUM
...
!                                Z_3 * RN%Y_F_INLET / RN%MW_FUEL * MW_O2 * RN%NU_CO * 0.5_EB
!   CASE(CO_INDEX)
!      IF (CO_PRODUCTION) THEN
...
!   CASE(CO2_INDEX)
!      IF (CO_PRODUCTION) THEN
!         Y_MF = Z_3 * RN%Y_F_INLET / RN%MW_FUEL * MW_CO2 * (RN%NU_CO + RN%NU_CO2)
!      ELSE
...
!IF (ABS(ZZ - Y_MF) > 1.E-10_EB) THEN
!  WRITE(*,*) INDEX,Z_1,Z_2,Z_3
!  WRITE(*,*) ZZ,Y_MF
!ENDIF
...
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: ZZ,Z_1,Z_2,Z_3,Y_MF(9),OMYYSUM
TYPE(REACTION_TYPE), POINTER :: RN
...
!   Y_MF(CO2_INDEX) = Z_3 * RN%Y_F_INLET / RN%MW_FUEL * MW_CO2 * (RN%NU_CO + RN%NU_CO2 )
!ELSE
...
REAL(EB) :: Z_1,Z_2,Z_3,Y_MF(9),W_MF(9),OMYYSUM
TYPE(REACTION_TYPE), POINTER :: RN
...
IF (CO_PRODUCTION) THEN
   W_MF(:) = Z2MW_C+ Z2MW(:,1) * Z_1 + Z2MW(:,2) * Z_2 + Z2MW(:,3) * Z_3
   Y_MF(:) = Z2Y_C + Z2Y(:,1) *  Z_1 + Z2Y(:,2) *  Z_2 + Z2Y(:,3) *  Z_3
ELSE
   W_MF(:) = Z2MW_C+ Z2MW(:,1) * Z_1 + Z2MW(:,2) * Z_3
   Y_MF(:) = Z2Y_C + Z2Y(:,1) *  Z_1 + Z2Y(:,2) *  Z_3
ENDIF
MW_MF = SUM(Y_MF)/SUM(W_MF)
...
!MW_MF = SUM(Y_MF)/(Y_MF(FUEL_INDEX)/RN%MW_FUEL + Y_MF(O2_INDEX)/MW_O2     + Y_MF(N2_INDEX)/MW_N2   + &
...
!OMYYSUM = (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_2 + Y_MF_SUM_Z(3)*Z_3)/ &
!           (MW_AVG_C  + MW_AVG_Z(1) * Z_1 + MW_AVG_Z(2) * Z_2 + MW_AVG_Z(3) * Z_3)
!OMYYSUM = (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_3)/ &
!           (MW_AVG_C  + MW_AVG_Z(1) * Z_1 + MW_AVG_Z(2) * Z_3)
!IF (ABS(MW_MF-OMYYSUM)>=1.E-6_EB) THEN
!   WRITE(*,'(1X,5(E14.7,1X))') Z_1,Z_2,Z_3,MW_MF,OMYYSUM
!   WRITE(*,'(1X,5(E14.7,1X))') Y_MF
!   WRITE(*,'(1X,5(E14.7,1X))') Z2Y_C(:)+ Z2Y(:,1) * Z_1 + Z2Y(:,2) * Z_3
!   WRITE(*,*) 
!ENDIF
...
INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,D_MF,Y_MF(9),OMYYSUM
TYPE(SPECIES_TYPE), POINTER :: SS
...
INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,CP_MF,Y_MF(9),OMYYSUM
TYPE(SPECIES_TYPE), POINTER :: SS
...
INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,K_MF,Y_MF(9),MW_MF,OMYYSUM
TYPE(SPECIES_TYPE), POINTER :: SS
TYPE(REACTION_TYPE), POINTER :: RN
