...
TYPE(BOUNDARY_PROP2_TYPE), POINTER :: P2
      ...
      DO JJG=1,JBAR
         DO IIG=1,IBAR
            ...
            DO IKT=LS_KLO_TERRAIN(IIG,JJG),K_LS(IIG,JJG)
               ! Loop over all CFACEs corresponding to IIG,JJG and set P1%T_IGN and P2%PHI_LS as below
               ...
               DO IW=1,CUT_FACE(ICF)%NFACE ! All CC_INBOUNDARY CFACES on this cell.
                  ...
                  IF (PHI_LS(IIG,JJG)>=0._EB .AND. P1%T_IGN>1.E5_EB) CALL IGNITE_GRID_CELL
                  IF (PHI_LS(IIG,JJG)>=0._EB .AND. TOA(IIG,JJG)<0._EB) TOA(IIG,JJG) = T
                  P2 => BOUNDARY_PROP2(CFA%P2_INDEX)
                  P2%PHI_LS = PHI_LS(IIG,JJG)
               ENDDO
            ENDDO
         ENDDO
      ENDDO
...
ENDIF
P1%BURN_DURATION = SF%BURN_DURATION + CROSSING_DISTANCE/ROS_MAG
P1%AREA_ADJUST = SF%BURN_DURATION/P1%BURN_DURATION
