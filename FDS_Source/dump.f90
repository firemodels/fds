...
INTEGER :: N,I,J,K,NN,IL,III,JJJ,KKK,Y_INDEX,Z_INDEX,PART_INDEX,ITMP,IP,JP,KP,FLOW_INDEX,IW
CHARACTER(100) :: MESSAGE
...
SELECT CASE(IND)
   ...
   CASE(30)  ! SMAGORINSKY COEFFICIENT
      ...
      JJJ = MAX(1,MIN(JJ,JBAR))
      KKK = MAX(1,MIN(KK,KBAR))
      DELTA = LES_FILTER_WIDTH(DX(III),DY(JJJ),DZ(KKK))
...
USE PHYSICAL_FUNCTIONS, ONLY: SURFACE_DENSITY,GET_MASS_FRACTION,GET_VISCOSITY
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
...
REAL(EB) :: CONCORR,U2,V2,W2,HQ2,FLUX,DFLUXDT,SOLID_PHASE_OUTPUT_OLD,VOLSUM,C_S_ADJUST_UNITS,TF_MEAN,&
            MU_DNS,ZZ_GET(1:N_TRACKED_SPECIES)
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(24) ! WALL THICKNESS
      ...
      ELSE
         IF (SURFACE(SURF_INDEX)%THERMALLY_THICK) THEN
            SOLID_PHASE_OUTPUT = MAXVAL(SURFACE(SURF_INDEX)%X_S)
         ELSE
            SOLID_PHASE_OUTPUT = 0._EB
         ENDIF
      ENDIF
   ...
   CASE(33) ! SOLID SPECIFIC HEAT
      ...
      MATERIAL_LOOP_CP: DO NN=1,SF%N_MATL
         IF (ONE_D%RHO(II1,NN)<=ZERO_P) CYCLE MATERIAL_LOOP_CP
   ...
   CASE(34) ! SOLID CONDUCTIVITY 
      ...
      MATERIAL_LOOP_K: DO NN=1,SF%N_MATL
         IF (ONE_D%RHO(II1,NN)<=ZERO_P) CYCLE MATERIAL_LOOP_K
         ML => MATERIAL(SF%MATL_INDEX(NN))
         VOLSUM = VOLSUM + ONE_D%RHO(II1,NN)/ML%RHO_S         
         IF (ML%K_S>0._EB) THEN
   ...
   CASE(35) ! YPLUS (distance from the wall expressed in nondimensional viscous units)
      ...
      SOLID_PHASE_OUTPUT = (0.5_EB/WC%RDN)/( MU_DNS/RHO(IIG,JJG,KKG)/(WC%U_TAU+EPS)+EPS )
