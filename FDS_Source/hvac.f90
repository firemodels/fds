   ...
   SELECT CASE (TYPE_ID)
      ...
      CASE('NODE')
         ...
         IF (DN%N_DUCTS == 0) THEN
            WRITE(MESSAGE,'(A,A)') 'ERROR: No ducts specified for ductnode ID: ',TRIM(DN%ID)
            CALL SHUTDOWN(MESSAGE); RETURN         
         ENDIF
...
SUBROUTINE RHSDUCT(DT,NETWORK_INDEX)
...
SUBROUTINE LHSDUCT(DT,NETWORK_INDEX)
USE GLOBAL_CONSTANTS
REAL(EB), INTENT(IN) :: DT
INTEGER, INTENT(IN)::NETWORK_INDEX
INTEGER :: NN,ND, ARRAYLOC1,ARRAYLOC2
...
SUBROUTINE CONVERGENCE_CHECK(DT,NNE)
INTEGER, INTENT(IN) :: NNE
REAL(EB), INTENT(IN) :: DT
INTEGER :: NN, ND
LOGICAL :: CONVERGED
