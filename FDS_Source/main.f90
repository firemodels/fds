...
! Use the same TAG for all communications  
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (CODE==6 .AND. DROPLET_FILE .AND. (M3%NIC_S>0 .OR. M3%NIC_R>0)) THEN
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
...
RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZE_SEND_IF: IF (CODE==0) THEN
      ...
      IF (DROPLET_FILE .AND.  (M3%NIC_S>0 .OR. M3%NIC_R>0) .AND. RNODE/=SNODE) THEN
...
IF (USE_MPI .AND. CODE/=2 .AND. CODE/=5) CALL MPI_WAITALL(N_REQ,REQ(1:N_REQ),MPI_STATUSES_IGNORE,IERR)
