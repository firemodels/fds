...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE, &
                ...
                LES_FILTER_TYPE,LEVEL_SET_ELLIPSE,LEVEL_SET_MODE,MATL_REAC_RATE_DELTA,&
   ...
   IF (ML%PYROLYSIS_MODEL==PYROLYSIS_LIQUID) THEN
      DO NS=1,N_TRACKED_SPECIES
         IF (ML%NU_GAS(NS,1) > 0._EB) THEN
            ...
            ML%RENODE_DELTA_T = MATL_REAC_RATE_DELTA/(SPECIES(NS)%H_V(INT(ML%TMP_REF(1))*SPECIES(NS)%MW/R0/ML%TMP_REF(1))**2)
            EXIT
         ENDIF
      ENDDO
   ELSE
      ...
      DO NR=1, ML%N_REACTIONS
         IF (ABS(ML%N_T(NR)) < TWO_EPSILON_EB) THEN
            ...
            WRITE(*,*) '*',NR,ML%TMP_REF(NR),ML%E(NR)/(R0*ML%TMP_REF(NR)**2)
            ML%RENODE_DELTA_T = MAX(ML%RENODE_DELTA_T,(ML%E(NR)/(R0*ML%TMP_REF(NR)**2)))
         ELSE
         ...
         ENDIF
      ENDDO
      WRITE(*,*) 'A',ML%RENODE_DELTA_T,MATL_REAC_RATE_DELTA
      ML%RENODE_DELTA_T = MATL_REAC_RATE_DELTA/ML%RENODE_DELTA_T
      WRITE(*,*) 'B',ML%RENODE_DELTA_T
   ENDIF
ENDDO
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            AREA_MULTIPLIER,Z_0,PARTICLE_EXTRACTION_VELOCITY,RENODE_DELTA_T,NEAR_WALL_EDDY_VISCOSITY
