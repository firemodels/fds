SUBROUTINE FDS2AST (CHID,NOMASTER,C_S,TBEG,TEND,TINT,COUNTER,VARIABLE,N,N_AVERAGE,T_AVERAGE,CODE,DEVICE)
...
INTEGER :: IDUM, IFILE, NSAM, NV, MV, CODE
...
CHARACTER(2) :: ANS
CHARACTER(4) :: CHOICE
CHARACTER(256) GRIDFILE,CHID,QFILE,JUNK, BNDF_CHECK
...
LOGICAL :: EXISTS
INTEGER :: BATCHMODE
CHARACTER(16), DIMENSION(500) :: DEVICE 
...
SEARCH_BNDF: DO I=1,500
   ...
   IF (IERR==1) EXIT SEARCH_BNDF
      ...
      READ(11,'(A)') BNDF_CHECK
      IF (CODE==5) THEN 
         IF (TRIM(BNDF_CHECK)==' ADIABATIC SURFACE TEMPERATURE') THEN 
            BNDF_TEXT(I)=BNDF_CHECK
         ELSE
            GOTO 200
         END IF
      ELSE IF (CODE==1 .AND. VARIABLE ==1) THEN 
         IF (TRIM(BNDF_CHECK)==' ADIABATIC SURFACE TEMPERATURE') THEN 
            BNDF_TEXT(I)=BNDF_CHECK
         ELSE
            GOTO 200
         END IF
      ELSE IF (CODE==1 .AND. VARIABLE ==2) THEN
         IF (TRIM(BNDF_CHECK)==' ADIABATIC SURFACE TEMPERATURE' .OR. TRIM(BNDF_CHECK)==' HEAT TRANSFER COEFFICIENT    ') THEN 
            BNDF_TEXT(I)=BNDF_CHECK
         ELSE
            GOTO 200
         END IF
...
!**********************
   ...
   OPEN(70,FILE=TRIM(DEVICE(P))//'.dat',FORM='FORMATTED',STATUS='UNKNOWN')
   IF (T_AVERAGE==0) THEN
      DO I=1,V
         IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') THEN
