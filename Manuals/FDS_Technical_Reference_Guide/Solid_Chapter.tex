...
\be
 -k_s\frac{\partial T_s}{\partial x}(0,t) = \dot{q}_c''+ \dot{q}_r''
\ee
where $\dot{q}_c''$ is the convective and $\dot{q}_r''$ the radiative flux. If the internal radiation is solved for a solid, the radiation boundary condition $\dot{q}_r''$ is not used.

On the back surface, two possible boundary condition types may be specified by the user. (1) If the back surface is assumed to be open either to ambient void or to another part of the computational domain, the back side boundary condition is similar to that of the front side. (2) If the back side is assumed to be perfectly insulated, and adiabatic condition is used
\be
 -k_s\frac{\partial T_s}{\partial x}(L,t) = 0.
\ee
...
direction is
\be
 \frac{1}{2}\frac{\d I^+(x)}{\d x}=\kappa_s\,\left(I_b-I^+(x)\right)
 \label{RInForward}
\ee
...
\be
 \frac{1}{2}\frac{{\d\dq^+_r(x)} }{\d x}=\kappa_s\,
...
\be
  \dq_{s,r}'''(x) = \frac{\d\dq_r^+(x)}{\d x}+\frac{\d\dq_r^-(x)}{\d x}
\ee
...
In a LES calculation, the convective heat flux to the surface is
...
\dq_{\hbox{\scriptsize user}}''}{\Delta H} \ee Usually, the user specifies a
