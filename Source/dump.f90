...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       ...
       DUMP_GEOM,UPDATE_DEVICES_2,WRITE_DEVC_CTRL_LOG,WRITE_STL_FILE,DUMP_AGL_TIMERS
...
IF (WRITE_VTK) THEN
   ...
   ELSE
      ...
      IF (T>=SL3D_VTK_CLOCK(SL3D_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
         IF (NM .EQ. 1) THEN
            CALL WRITE_VTK_SL3D_WRAPPER(T,NMESHES,1)
         ENDIF
      ...
      IF (T>=SLCF_VTK_CLOCK(SLCF_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
         IF (NM .EQ. 1) THEN
            CALL WRITE_VTK_SL3D_WRAPPER(T,NMESHES,0)
         ENDIF
         CALL DUMP_SLCF(T,DT,NM,4)
         DO WHILE(SLCF_VTK_COUNTER(NM)<SIZE(SLCF_VTK_CLOCK)-1)
            SLCF_VTK_COUNTER(NM) = SLCF_VTK_COUNTER(NM) + 1
            IF (SLCF_VTK_CLOCK(SLCF_VTK_COUNTER(NM))>=T) EXIT
         ENDDO
      ...
      IF (T>=SM3D_VTK_CLOCK(SM3D_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
         CALL POINT_TO_MESH(NM)
         IF ((NM .EQ. 1).AND.(N_SMOKE3D > 0).AND.(.NOT.VTK_HDF)) THEN
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   WRITE(MYSTR,'(2I5)') M%N_VENT-N_CVENT+NDV,NDV; CALL ADDSTR
   ...
   DO N=1,NDV
      ...
      WRITE(MYSTR,'(6F14.5,I6,I4)') M%X(IDV1(N)),M%X(IDV2(N)),M%Y(JDV1(N)),M%Y(JDV2(N)),  &
                                    M%Z(KDV1(N)),M%Z(KDV2(N)),M%N_VENT+N,SURF_INDEX; CALL ADDSTR
   ENDDO
   ...
   DO N=1,NDV
      ...
      IF (VRGB(1)<0) THEN
         WRITE(MYSTR,'(8I5)') IDV1(N),IDV2(N),JDV1(N),JDV2(N),KDV1(N),KDV2(N),COLOR_INDEX,TYPE_INDEX; CALL ADDSTR
      ELSE
         WRITE(MYSTR,'(8I5,4F13.5)') IDV1(N),IDV2(N),JDV1(N),JDV2(N),KDV1(N),KDV2(N),COLOR_INDEX,TYPE_INDEX, &
...
IF (AGL_TIMERS(1)) WRITE(LU_CORE(NM)) T_ARR
IF (AGL_TIMERS(2)) WRITE(LU_CORE(NM)) T_RES
...
IF (AGL_TIMERS(1)) READ(LU_RESTART(NM)) T_ARR
IF (AGL_TIMERS(2)) READ(LU_RESTART(NM)) T_RES
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (IFRMT.EQ.1) THEN ; ALLOCATE(COLORS(NPLIM,3),STAT=IZERO) ; CALL ChkMemErr('DUMP','COLOR',IZERO) ; ENDIF
   ...
   LOAD_LOOP: DO IP=1,NLP
      ...
      IF (IFRMT.EQ.1) THEN
         IF (LPC%RGB(1)==-1) THEN
            COLORS(NPP,1:3) = REAL(SURFACE(LPC%SURF_INDEX)%RGB,FB)/255._FB
         ELSE
            COLORS(NPP,1:3) = REAL(LPC%RGB,FB)/255._FB
         ENDIF
      ENDIF
   ENDDO LOAD_LOOP
   ...
   ELSEIF (IFRMT.EQ.1) THEN
      WRITE(FN_PART_VTK(N,NM),'(A,A,A,A,A,I0,A,I8.8,I2.2,A)') "",TRIM(RESULTS_DIR)//TRIM(CHID),'_PART_',TRIM(LPC%ID),'_',&
                               NM,'_',ITM,ITM1,'.vtp'
      IF (VTK_BINARY) THEN
      ...
      VTK_ERROR = A_VTK_FILE%XML_WRITER%W_DATA(DATA_NAME='TAG', X=TA(1:NPP))
      VTK_ERROR = A_VTK_FILE%XML_WRITER%W_DATA(DATA_NAME='COLOR', X=COLORS(1:NPP,1), Y=COLORS(1:NPP,2), Z=COLORS(1:NPP,3))
      DO NN=1,LPC%N_QUANTITIES
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   WRITE(FILENAME,'(A,A,A,A,A,I8.8,I2.2,A)') "",TRIM(RESULTS_DIR)//TRIM(CHID),'_PART_',TRIM(LPC%ID),'_',ITM,ITM1,'.vtkhdf'
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   IF (VTK_OUT) THEN
      ...
      IF (ITM1==100) THEN
         ...
         ITM1 = 0
      ENDIF
      WRITE(FN_SMOKE3D_VTK(NM),'(A,A,A,I0,A,I8.8,I2.2,A)') "",TRIM(RESULTS_DIR)//TRIM(CHID),'_SM3D_',NM,'_',ITM,ITM1,'.vtu'
      I2 = MESHES(NM)%IBAR
      J2 = MESHES(NM)%JBAR
      K2 = MESHES(NM)%KBAR
      NX = I2 + 1 - I1
      ...
      IF (.NOT.VTK_HDF) THEN
         ...
         IF ((N .EQ. 1).AND.(.NOT.VTK_HDF)) THEN
            IF (VTK_BINARY) THEN
...
IF (((VTK_OUT).AND.(N_SMOKE3D > 0)).AND.(.NOT.VTK_HDF)) THEN
   VTK_ERROR = A_VTK_FILE%XML_WRITER%W_DATA(LOCATION='NODE', ACTION='CLOSE')
   VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_PIECE()
   VTK_ERROR = A_VTK_FILE%FINALIZE()
ENDIF
...
SUBROUTINE DUMP_SMOKE3D_VTKHDF(T,DT,NM,IFRMT,FAKEWRITE)
...
INTEGER :: I,J,K,NQT,I1,I2,J1,J2,K1,K2,ITM,ITM1,IQ,IQ2,IQ3,IQQ,IND,IND2,II,II1,II2,JJ1,JJ2,KK1,KK2, &
           IC,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,NOM,IIO,JJO,KKO,I_INC,J_INC,&
           DEBUG,IERROR,IFACT,JFACT,KFACT,NC,NP,NX,NY,NZ,KTS,NTSL,ICO
...
IF (VTK_HDF.AND.VTK3D) THEN
   ...
   DO II=1,MESHES(1)%N_UNIQUE_SLCF
      ...
      !CALL WRITE_VTKHDF_SLICE_DATA_FILE(FILENAME, "FAKENEWS", QQ_PACK)
   ENDDO
ENDIF
...
IF (.NOT.VTK3D) THEN
   QUANTITY_LOOP: DO IQ=1,NQT
      ...
      ELSE
         SL => SLICE(IQ)
         IND  = SL%INDEX
         ! TIME OF ARRIVAL and RESIDENCE TIME slices not updated here
         IF (SL%INDEX==78 .OR. SL%INDEX==79) CYCLE QUANTITY_LOOP
         IND2 = SL%INDEX2
         Y_INDEX = SL%Y_INDEX
         Z_INDEX = SL%Z_INDEX
...
IF (VTK3D) THEN
   ...
   UNIQUE_LOOPA: DO II=1,MESHES(1)%N_UNIQUE_SLCF
      ...
      IF ((.NOT.SLCF3D).AND.(MESHES(1)%UNIQUE_SLICE_IS_SL3D(II))) CYCLE UNIQUE_LOOPA
      IF ((SLCF3D).AND.(.NOT.MESHES(1)%UNIQUE_SLICE_IS_SL3D(II))) CYCLE UNIQUE_LOOPA
      ...
      QUANTITY_LOOPA: DO IQ=1,NQT
         ...
         IF (.NOT.VTK_INITIALIZED) THEN
            WRITE(FN_SL3D_VTK(II,NM),'(A,A,A,A,I0,A,I8.8,I2.2,A)') TRIM(RESULTS_DIR)//TRIM(CHID),'_',&
                                     TRIM(MESHES(1)%UNIQUE_SLICE_NAMES(II)),'_',NM,'_',ITM,ITM1,'.vtu'
            IF (VTK_BINARY) THEN
               VTK_ERROR = A_VTK_FILE%INITIALIZE(FORMAT='raw', FILENAME=FN_SL3D_VTK(II,NM), MESH_TOPOLOGY='UnstructuredGrid')
            ELSE
               VTK_ERROR = A_VTK_FILE%INITIALIZE(FORMAT='ascii', FILENAME=FN_SL3D_VTK(II,NM), MESH_TOPOLOGY='UnstructuredGrid')
            ENDIF ! do not change capitalization on mesh topology
            CALL BUILD_VTK_SLICE_GEOMETRY(NM, SL, NC, NP, X_PTS, Y_PTS, Z_PTS, CONNECT, OFFSETS, VTKC_TYPE)
            VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_PIECE(NP=NP, NC=NC)
            VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_GEO(NP=NP, NC=NC, X=X_PTS, Y=Y_PTS, Z=Z_PTS)
            VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_CONNECTIVITY(NC=NC, CONNECTIVITY=CONNECT, OFFSET=OFFSETS, VTKC_TYPE=VTKC_TYPE)
            VTK_ERROR = A_VTK_FILE%XML_WRITER%W_DATA(LOCATION='NODE', ACTION='OPEN')
         ...
         IF (SL%SLICETYPE=='STRUCTURED') THEN ! write out slice file using original slice file format
            IF (NX*NY*NZ>0) THEN
               ...
               ENDDO
               IF (VTK_HDF) THEN
                  !CALL WRITE_VTKHDF_SLICE_DATA_FILE(FILENAME, TRIM(SL%SMOKEVIEW_LABEL(1:30)), QQ_PACK)
               ELSE
...
IF(FAKEWRITE) THEN
   UNIQUE_LOOPF: DO IQ=1,MESHES(1)%N_UNIQUE_SLCF
      ...
      CALL WRITE_VTKHDF_SLICE_CELL_FILE(FILENAME,SLCFNAME,SL3D,NM,NCELLS,NPOINTS,NCONNECTIONS)
      NPOINTS=0
      NCELLS=0
      NCONNECTIONS=0
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(78)  ! TIME OF ARRIVAL
      GAS_PHASE_OUTPUT_RES = T_ARR(II,JJ)
   CASE(79)  ! RESIDENCE TIME
      GAS_PHASE_OUTPUT_RES = T_RES(II,JJ)
...
ELSEIF ((IFRMT.EQ.1).AND.(MESHES(NM)%N_PATCH > 0)) THEN
   ...
   IF (ITM1==100) THEN
      ...
      ITM1 = 0
   ENDIF
   WRITE(FN_BNDF_VTK(NM),'(A,A,A,I0,A,I8.8,I2.2,A)') "",TRIM(RESULTS_DIR)//TRIM(CHID),'_BNDF_',NM,'_',ITM,ITM1,'.vtu'
   IF (VTK_BINARY) THEN
...
! \brief Write time of arrival and/or residence time AGL_SLICE to a file
!>
!> \param T Current simulation time (s)
!> \param NM Mesh number

SUBROUTINE DUMP_AGL_TIMERS(T,NM)

USE MEMORY_FUNCTIONS, ONLY: ChkMemErr
USE OUTPUT_DATA, ONLY: OUTPUT_QUANTITY
INTEGER, INTENT(IN) :: NM
REAL(EB), INTENT(IN) :: T
INTEGER :: I,J,I1,I2,II2,J1,J2,JJ2,IQ,IQ2,IND,IZERO,NTSL
REAL(EB) :: MASK_VALUE,T_SUM
REAL(EB), POINTER :: T_DATA(:,:)
REAL(FB) :: SLICE_MIN,SLICE_MAX,STIME,T_COUNT
REAL(FB), ALLOCATABLE, DIMENSION(:,:) :: QQ_T
...
QUANTITY_LOOP: DO IQ=1,N_SLCF
   ...
   ! only TIME OF ARRIVAL slice updated here
   IF (SL%INDEX/=78 .AND. SL%INDEX/=79) CYCLE QUANTITY_LOOP
   ...
   ELSE
      ...
      T_DATA => T_RES
   ENDIF
   ALLOCATE(QQ_T(1:II2,1:JJ2),STAT=IZERO); CALL ChkMemErr('DUMP','QQ_T',IZERO)
   QQ_T=REAL(MASK_VALUE,FB)

   ! Average (mask out points where no fire arrival detected)
   DO J=J1,J2
      DO I=I1,I2
         T_SUM=SUM(T_DATA(I:I+1,J:J+1), MASK=ABS(T_DATA(I:I+1,J:J+1)-MASK_VALUE)>TWO_EPSILON_EB)
         T_COUNT=REAL(COUNT(ABS(T_DATA(I:I+1,J:J+1)-MASK_VALUE)>TWO_EPSILON_EB),FB)
         IF (T_COUNT>0) QQ_T(I-J1+1,J-J1+1) = REAL(T_SUM/T_COUNT,FB)
      ENDDO
   ENDDO

   STIME = REAL(T_BEGIN + (T-T_BEGIN)*TIME_SHRINK_FACTOR,FB)

   ! Replace slice file each time it is written
   OPEN(LU_SLCF(IQ,NM),FILE=FN_SLCF(IQ,NM),FORM='UNFORMATTED',STATUS='REPLACE')
   WRITE(LU_SLCF(IQ,NM)) SL%SMOKEVIEW_LABEL(1:30)
   WRITE(LU_SLCF(IQ,NM)) SL%SMOKEVIEW_BAR_LABEL(1:30)
   WRITE(LU_SLCF(IQ,NM)) OUTPUT_QUANTITY(SL%INDEX)%UNITS(1:30)
   WRITE(LU_SLCF(IQ,NM)) I1,I2,J1,J2,K_AGL_SLICE(I1,J1,NTSL),K_AGL_SLICE(I1,J1,NTSL)

   WRITE(LU_SLCF(IQ,NM)) STIME
   WRITE(LU_SLCF(IQ,NM)) ((QQ_T(I,J) ,I=1,II2),J=1,JJ2)
   CLOSE(LU_SLCF(IQ,NM))

   SLICE_MIN = MINVAL(QQ_T)
   SLICE_MAX = MAXVAL(QQ_T,MASK=(QQ_T<1.E10))
...
END SUBROUTINE DUMP_AGL_TIMERS
