...
N_SURF_RESERVED = 6
IF (.NOT. NO_EVACUATION) N_SURF_RESERVED = 7
...
INERT_PARTICLE_SURF_INDEX          = N_SURF + 6
IF (.NOT. NO_EVACUATION) EVACUATION_SURF_INDEX = N_SURF + 7
...
SURFACE(INERT_PARTICLE_SURF_INDEX)%ID = 'INERT PARTICLE'
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
   ...
   ENDIF
   IF (N==INERT_PARTICLE_SURF_INDEX) THEN
...
INTEGER :: SURF_INDEX,N,NL,NWP_MAX,II,IL,NN
...
IF (.NOT.RADIATION) NUMBER_RADIATION_ANGLES = 1
...
REAL(EB) :: TEMPERATURE,DENSITY,MASS_FRACTION(1:MAX_SPECIES),RR_SUM,ZZ_GET(0:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
            MASS_PER_TIME,DT_INSERT,UVW(3),HRRPUV
