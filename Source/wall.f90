...
REAL(EB) :: T_SCALE(MAX_QDOTPP_REF),QDOTPP_REF(MAX_QDOTPP_REF),XVAL,QDOTPP,QDOTPP1,QDOTPP2,DT_SPYRO
INTEGER :: N,NQ,ITER,IIO,JJO,KKO,OTHER_MESH_OBST_INDEX,LL
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (SF%REFERENCE_HEAT_FLUX(1) > 0._EB .AND. B1%T_IGN <=T .AND. PREDICTOR) THEN
         DT_SPYRO = DT * SF%SPYRO_TH_FACTOR
         ...
         B1%Q_IN_SMOOTH_INT = B1%Q_IN_SMOOTH_INT + DT_SPYRO * B1%Q_IN_SMOOTH 
!         WRITE(*,*) 'B1',WALL_INDEX,TRIM(SF%ID),B1%Q_IN_SMOOTH,B1%Q_IN_SMOOTH_INT,B1%Q_CON_F,B1%Q_RAD_IN,DT_SPYRO
      ...
      IF (SF%REFERENCE_HEAT_FLUX(1) > 0._EB .AND. N_REACTIONS>=1 .AND. PREDICTOR .AND. B1%T_IGN <=T) THEN
         DO NQ = 1,SF%N_QDOTPP_REF
            RP     => RAMPS(SF%HRRPUA_INT_INDEX(NQ))
            RP_REF => RAMPS(SF%QREF_INDEX(NQ))
            RP_E2T => RAMPS(SF%E2T_INDEX(NQ))
            CALL INTERPOLATE1D_UNIFORM(0,RP_E2T%INTERPOLATED_DATA(:),B1%Q_IN_SMOOTH_INT*RP_E2T%RDT,T_SCALE(NQ))
            CALL INTERPOLATE1D_UNIFORM(0,RP_REF%INTERPOLATED_DATA(:),T_SCALE(NQ)*RP_REF%RDT,QDOTPP_REF(NQ))
            IF (B1%Q_IN_SMOOTH_INT >= RP_E2T%T_MAX) THEN
               RP => RAMPS(SF%RAMP(TIME_HEAT-NQ+1)%INDEX)
               Q_NEW(NQ) = B1%QDOTPP_INT(NQ) + &
                             DT_SPYRO*RP%INTERPOLATED_DATA(RP%NUMBER_INTERPOLATION_POINTS)*B1%Q_IN_SMOOTH/QDOTPP_REF(NQ)
            ELSE
...
!            WRITE(*,*) 'A',NQ,T_SCALE(NQ),QDOTPP_REF(NQ),Q_NEW(NQ),SF%RAMP(TIME_HEAT-NQ+1)%INDEX,SF%HRRPUA_INT_INDEX(NQ),SF%QREF_INDEX(NQ),SF%E2T_INDEX(NQ)
         ENDDO
         IF (B1%Q_IN_SMOOTH <= QDOTPP_REF(1)) THEN
            QDOTPP = MAX(0._EB,(Q_NEW(1) - B1%QDOTPP_INT(1))/DT_SPYRO)
!            WRITE(*,*) 'A1',1,QDOTPP,B1%QDOTPP_INT(1)
         ELSEIF (B1%Q_IN_SMOOTH > QDOTPP_REF(SF%N_QDOTPP_REF)) THEN
            QDOTPP = MAX(0._EB,(Q_NEW(SF%N_QDOTPP_REF) - B1%QDOTPP_INT(SF%N_QDOTPP_REF))/DT_SPYRO)
!            WRITE(*,*) 'A2',SF%N_QDOTPP_REF,QDOTPP,B1%QDOTPP_INT(SF%N_QDOTPP_REF)
         ELSE
            DO NQ=2,SF%N_QDOTPP_REF
               IF (B1%Q_IN_SMOOTH <= QDOTPP_REF(NQ)) THEN
                  QDOTPP1 = MAX(0._EB,(Q_NEW(NQ-1)- B1%QDOTPP_INT(NQ-1))/DT_SPYRO)
                  QDOTPP2 = MAX(0._EB,(Q_NEW(NQ)  - B1%QDOTPP_INT(NQ))/DT_SPYRO)
                  QDOTPP = QDOTPP1 + (QDOTPP2 - QDOTPP1)*(B1%Q_IN_SMOOTH - QDOTPP_REF(NQ-1))/(QDOTPP_REF(NQ)-QDOTPP_REF(NQ-1))
...
!                     WRITE(*,*) 'C',QDOTPP,SF%MASS_FLUX(N),B1%M_DOT_G_PP_ACTUAL(N)  
                  ELSE
