...
CFACE_LOOP: DO ICF=1,N_CFACE_CELLS
   CF=>CFACE(ICF)
   IF (CF%BOUNDARY_TYPE==NULL_BOUNDARY .AND. .NOT.SOLID_HT3D) CYCLE CFACE_LOOP
   SURF_INDEX = CF%SURF_INDEX
   ONE_D => CF%ONE_D
 ! ONE_D%TMP_G   = 
 ! ONE_D%RHO_G   = 
 ! ONE_D%ZZ_G(:) =
...
TYPE (CFACE_TYPE), POINTER :: CF=>NULL()
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
TYPE (ONE_D_M_AND_E_XFER_TYPE), POINTER :: ONE_D=>NULL()
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
HTC_MODEL_SELECT: SELECT CASE(SF%HEAT_TRANSFER_MODEL)
   CASE DEFAULT
      ...
      RE = ONE_D%RHO_G*ONE_D%U_TANG*SF%CONV_LENGTH/MU_G
      SELECT CASE(SF%GEOMETRY)
         CASE (SURF_CARTESIAN)
      ...
      END SELECT
      H_FORCED  = NUSSELT*K_G/SF%CONV_LENGTH
   CASE(H_LOGLAW)
      H_NATURAL = 0._EB
      CALL GET_VISCOSITY(ZZ_GET,MU_G,TMP_FILM)
      CALL GET_CONDUCTIVITY(ZZ_GET,K_G,TMP_FILM)
   ...
   CASE(H_ABL)
      H_NATURAL = 0._EB
      CALL GET_SPECIFIC_HEAT(ZZ_GET,CP_G,TMP_FILM)
      CALL ABL_HEAT_FLUX_MODEL(H_FORCED,FRICTION_VELOCITY,DN,SF%ROUGHNESS,ONE_D%TMP_G,ONE_D%TMP_F,ONE_D%RHO_G,CP_G)
   CASE(H_CUSTOM)
      CALL GET_VISCOSITY(ZZ_GET,MU_G,TMP_FILM)
      CALL GET_CONDUCTIVITY(ZZ_GET,K_G,TMP_FILM)
      RE = ONE_D%RHO_G*ONE_D%U_TANG*SF%CONV_LENGTH/MU_G
