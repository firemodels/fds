...
INTEGER :: NM,IERR,IPROC,I,N_OBST
INTEGER, PARAMETER :: PADDING = 256
...
DO NM=1,NMESHES
   DO IPROC=1,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC.AND.IPROC==PROCESS(NM)) THEN
         IF (MY_RANK/=0) THEN
            CALL MPI_SEND(MESHES(NM)%N_OBST,1,MPI_INTEGER,0,PROCESS(NM),MPI_COMM_WORLD,IERR)
         ENDIF
      ELSEIF (MY_RANK==0) THEN
         IF (PROCESS(NM)==0) CYCLE
         IF (PROCESS(NM)/=IPROC) CYCLE
         CALL MPI_RECV(N_OBST,1,MPI_INTEGER,IPROC,IPROC,MPI_COMM_WORLD,MPISTATUS,IERR)
         IF (MESHES(NM)%N_OBST > 0) CYCLE
         MESHES(NM)%N_OBST = N_OBST
      ENDIF
   ENDDO
ENDDO
...
DO NM=1,NMESHES
   DO IPROC=0,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC) THEN 
         IF (MY_RANK/=0) THEN
            CALL MPI_RECV(N_OBST,1,MPI_INTEGER,0,0,MPI_COMM_WORLD,MPISTATUS,IERR)
            MESHES(NM)%N_OBST = N_OBST
         ENDIF
      ELSEIF (MY_RANK==0) THEN
         CALL MPI_SEND(MESHES(NM)%N_OBST,1,MPI_INTEGER,IPROC,0,MPI_COMM_WORLD,IERR)
      ENDIF
   ENDDO
ENDDO
