...
IF (PRESENT(LP_INDEX)) THEN
   ...
   CALL PACK_PARTICLE(NM,LP_INDEX,LPC_INDEX,1,UNPACK_IT=.FALSE.,&
                      N_INTEGERS=LP%N_INTEGERS,N_REALS=LP%N_REALS,N_LOGICALS=LP%N_LOGICALS)
...
ELSEIF (PRESENT(WALL_INDEX)) THEN
   ...
   CALL PACK_WALL(NM,WALL_INDEX,SURF_INDEX,1,UNPACK_IT=.FALSE.,N_INTEGERS=WC%N_INTEGERS,N_REALS=WC%N_REALS,N_LOGICALS=WC%N_LOGICALS)
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   CALL PACK_THIN_WALL(NM,THIN_WALL_INDEX,SURF_INDEX,1,UNPACK_IT=.FALSE.,&
                       N_INTEGERS=TW%N_INTEGERS,N_REALS=TW%N_REALS,N_LOGICALS=TW%N_LOGICALS)
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   ...
   CALL PACK_CFACE(NM,CFACE_INDEX,SURF_INDEX,UNPACK_IT=.FALSE.,&
                   N_INTEGERS=CFA%N_INTEGERS,N_REALS=CFA%N_REALS,N_LOGICALS=CFA%N_LOGICALS)
...
INTEGER :: IC,RC,LC,BC_INDEX,OD_INDEX,B1_INDEX,B2_INDEX,BR_INDEX
LOGICAL :: COUNT_ONLY
TYPE(LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP
TYPE(LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%N_REALS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%N_INTEGERS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%N_LOGICALS,UNPACK_IT)
...
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,STORAGE_INDEX,B1_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_PROP2_TYPE) CALL PACK_BOUNDARY_PROP2(NM,IC,RC,OS,1,B2_INDEX,UNPACK_IT,COUNT_ONLY)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%N_REALS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%N_INTEGERS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%N_LOGICALS,UNPACK_IT)
...
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM2,IC,RC,OS,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM2,IC,RC,LC,OS,STORAGE_INDEX,B1_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) CALL PACK_BOUNDARY_PROP2(NM2,IC,RC,OS,STORAGE_INDEX,B2_INDEX,UNPACK_IT,COUNT_ONLY)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%N_REALS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%N_INTEGERS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%N_LOGICALS,UNPACK_IT)
...
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER, OPTIONAL, INTENT(OUT) :: N_INTEGERS,N_REALS,N_LOGICALS
INTEGER :: I1,I2,IC,RC,LC,BC_INDEX,OD_INDEX,B1_INDEX,B2_INDEX,BR_INDEX
LOGICAL :: COUNT_ONLY
TYPE(CFACE_TYPE), POINTER :: CFA
TYPE(STORAGE_TYPE), POINTER :: OS
TYPE(SURFACE_TYPE), POINTER :: SF
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%N_REALS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%N_INTEGERS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%N_LOGICALS,UNPACK_IT)
...
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,1,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,1,B1_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) CALL PACK_BOUNDARY_PROP2(NM,IC,RC,OS,1,B2_INDEX,UNPACK_IT,COUNT_ONLY)
...
SUBROUTINE PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%N_LAYERS,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%N_MATL,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%N_LPC,UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,STORAGE_INDEX,B1_INDEX,UNPACK_IT,COUNT_ONLY)

USE COMP_OPERATORS, ONLY: EQUATE
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,N_SURFACE_DENSITY_SPECIES
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,B1_INDEX
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),B1%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),B1%PRESSURE_ZONE,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),B1%NODE_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),B1%N_SUBSTEPS,UNPACK_IT)

! Logicals

LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),B1%BURNAWAY,UNPACK_IT)

! Reals

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%AREA,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%HEAT_TRANS_COEF,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_CON_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_RAD_IN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_RAD_OUT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%EMISSIVITY,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%AREA_ADJUST,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%T_IGN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%TMP_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%TMP_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%TMP_B,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%U_NORMAL,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%U_NORMAL_S,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%U_NORMAL_0,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%U_TANG,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%RHO_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%RDN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%K_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_DOT_G_PP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_DOT_O2_PP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_CONDENSE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%TMP_F_OLD,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%BURN_DURATION,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%T_SCALE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_SCALE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%T_MATL_PART,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%B_NUMBER,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%M_DOT_PART_ACTUAL,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B1%Q_IN_SMOOTH,UNPACK_IT)

I2 = RC  ! I1 and I2 continue counting reals

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , B1%M_DOT_G_PP_ACTUAL(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , B1%M_DOT_G_PP_ADJUST(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , B1%ZZ_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , B1%RHO_D_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , B1%RHO_D_DZDN_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_SURFACE_DENSITY_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , B1%AWM_AEROSOL(1:I2-I1+1) , UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_PROP2(NM,IC,RC,OS,STORAGE_INDEX,B2_INDEX,UNPACK_IT,COUNT_ONLY)

USE COMP_OPERATORS, ONLY: EQUATE
USE GLOBAL_CONSTANTS, ONLY: N_LP_ARRAY_INDICES
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,B2_INDEX
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),B2%SURF_INDEX,UNPACK_IT)

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B2%U_TAU,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B2%Y_PLUS,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B2%Z_STAR,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B2%PHI_LS,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B2%WORK1,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B2%WORK2,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),B2%K_SUPPRESSION,UNPACK_IT)

I2 = RC

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),B2%A_LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),B2%LP_CPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),B2%LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),B2%LP_TEMP(1:I2-I1+1),UNPACK_IT)
