...
REAL(EB), DIMENSION(1:MAX_REACTIONS) :: A,E,TGA_HEATING_RATE,TGA_PYROLYSIS_RANGE,HEAT_OF_REACTION,NU_FUEL,NU_WATER,NU_RESIDUE, &
...
NAMELIST /MATL/ ID,FYI,SPECIFIC_HEAT,CONDUCTIVITY,CONDUCTIVITY_RAMP,SPECIFIC_HEAT_RAMP, &
                ...
                EMISSIVITY,TGA_HEATING_RATE,TGA_PYROLYSIS_RANGE,HEAT_OF_REACTION,DENSITY,RESIDUE, &
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   ML%TGA_HEATING_RATE(:)  = TGA_HEATING_RATE(:)/60._EB
   ML%TGA_PYROLYSIS_RANGE(:)= TGA_PYROLYSIS_RANGE(:)
   ...
   DO NN=1,ML%N_REACTIONS
      IF (ML%TMP_REF(NN) > 0._EB) THEN
         PEAK_REACTION_RATE = 2._EB*ML%TGA_HEATING_RATE(NN)/ML%TGA_PYROLYSIS_RANGE(NN) ! Triangle approximation of DTA
         PEAK_MASS_FRACTION = 0.4_EB ! Approximate value of Y(T_pyro)
         ML%E(NN) = PEAK_REACTION_RATE*R0*ML%TMP_REF(NN)**2/(ML%TGA_HEATING_RATE(NN)*PEAK_MASS_FRACTION)
         ML%A(NN) = ML%E(NN)/(R0*ML%TMP_REF(NN)**2)*EXP(ML%E(NN)/(R0*ML%TMP_REF(NN)))*ML%TGA_HEATING_RATE(NN)
      ENDIF
   ENDDO
...
TGA_HEATING_RATE       = 5._EB       ! K/min
TGA_PYROLYSIS_RANGE    = 80._EB      ! K or C
