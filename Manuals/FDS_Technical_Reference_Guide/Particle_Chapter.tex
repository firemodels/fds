...
\ee
where $C_{\rm d}$ is the drag coefficient, $A_{\rm p,c}$ is the particle cross-sectional area, $r_{\rm p}$ is the particle radius, $\bu_{\rm p}$ is the particle velocity, $m_{\rm p}$ is the particle mass, $\bu$ is the gas velocity, and $\rho$ is the gas density. The particle acceleration is given by
\be
...
For some applications, it is convenient to use Lagrangian particles to represent stationary vegetation, like grasses or leaves, in which case the force term, $\bof_{\rm b}$, can be written in an alternative form:
\be
    {\bof_{\rm b}} = \ha \frac{A_{\rm p,c}}{A_{\rm p,s}} \, \rho \, C_{\rm d} \, \beta_{\rm b} \, \sigma_{\rm b} \, \bu |\bu|  \quad ; \quad \beta_{\rm b} \, \sigma_{\rm b} = \frac{1}{V} \sum A_{\rm p,s}  \label{part_force_veg}
\ee
Here, $\beta_{\rm b}$ is the ratio of the volume occupied by solid mass to the overall volume, sometimes referred to as the {\em packing ratio}; $A_{\rm p,s}$ is the surface area of a single particle, and $\sigma_{\rm b}$ is the surface area to volume ratio of the particle. 
