...
! Initialize isosurface file
   ...
   DO N=1,N_ISOF
      ...
      ELSE
        ...
        M%N_STRINGS = M%N_STRINGS + 1
        WRITE(M%STRING(M%N_STRINGS),'(A,I6,1X,I6)') 'ISOG',NM,IS%SKIP
        M%N_STRINGS = M%N_STRINGS + 1
...
DO N=1,N_DEVC
   ...
   WRITE(LU_CORE(NM)) DV%T,DV%T_CHANGE,DV%TMP_L,DV%Y_C,DV%CURRENT_STATE,DV%PRIOR_STATE,&
                      DV%LP_TAG,DV%PART_CLASS_INDEX,DV%RMS_VALUE,DV%RMS_VALUE2,DV%COV_VALUE,DV%TIME_INTEGRAL,&
                      DV%TIME_INTEGRAL2,DV%VALUE,DV%SMOOTHED_VALUE,DV%TIME_INTERVAL
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   SELECT CASE (DV%TEMPORAL_STATISTIC)
      ...
      CASE('RUNNING AVERAGE')
         DV%TIME_INTEGRAL  = DV%TIME_INTEGRAL  + DT*DV%INSTANT_VALUE
         WGT = 1._EB/(T - DV%STATISTICS_START + DT)
         DV%TIME_INTERVAL = 1._EB
         DV%VALUE = DV%TIME_INTEGRAL*WGT
      CASE('TIME INTEGRAL')
         DV%TIME_INTEGRAL  = DV%TIME_INTEGRAL  + DT*DV%INSTANT_VALUE
         DV%TIME_INTERVAL = 1._EB
         DV%VALUE = DV%TIME_INTEGRAL
      CASE('MAX')
         DV%TIME_INTERVAL = 1._EB
      ...
      CASE('RMS')
         DV%TIME_INTEGRAL = DV%TIME_INTEGRAL + DT*DV%INSTANT_VALUE
         WGT = 1._EB/(T - DV%STATISTICS_START + DT)
         DV%RMS_VALUE = DV%RMS_VALUE + (DV%INSTANT_VALUE-DV%TIME_INTEGRAL*WGT)**2*DT
         DV%VALUE = SQRT(DV%RMS_VALUE*WGT)
         DV%TIME_INTERVAL = 1._EB
      CASE('COV')
         DV%TIME_INTEGRAL  = DV%TIME_INTEGRAL  + DT*DV%INSTANT_VALUE
         DV%TIME_INTEGRAL2 = DV%TIME_INTEGRAL2 + DT*DV%VALUE_2
         WGT = 1._EB/(T - DV%STATISTICS_START + DT)
         DV%COV_VALUE = DV%COV_VALUE + (DV%INSTANT_VALUE-DV%TIME_INTEGRAL*WGT)*(DV%VALUE_2-DV%TIME_INTEGRAL2*WGT)*DT
      ...
      CASE('CORRCOEF')
         ...
         DV%COV_VALUE  = DV%COV_VALUE + (DV%INSTANT_VALUE-DV%TIME_INTEGRAL*WGT)*(DV%VALUE_2-DV%TIME_INTEGRAL2*WGT)*DT
         DV%RMS_VALUE  = DV%RMS_VALUE + (DV%INSTANT_VALUE-DV%TIME_INTEGRAL*WGT)**2*DT
         DV%RMS_VALUE2 = DV%RMS_VALUE2+ (DV%VALUE_2-DV%TIME_INTEGRAL2*WGT)**2*DT
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,MU_G,DUMMY,&
            ...
            UVW,UODX,VODY,WODZ,XHAT,ZHAT,BBF,RHO2,TMPUP,TMPLOW,ZINT,RHO_S,GAMMA_LOC,VC,TIME_RAMP_FACTOR
...
!       GAS_PHASE_OUTPUT_RES = MEASURE_SCALAR_RESOLUTION(II,JJ,KK,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,NM)
   ...
   CASE(105:108) ! Hot Gas Layer Reduction
      CALL GET_LAYER_HEIGHT(SDV%I1,SDV%J1,SDV%K1,SDV%K2,ZINT,TMPUP,TMPLOW)
...
USE COMPLEX_GEOMETRY, ONLY : GET_PRES_CFACE, GET_UVWGAS_CFACE, GET_MUDNS_CFACE
