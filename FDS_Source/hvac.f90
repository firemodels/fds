...
REAL(EB), ALLOCATABLE, DIMENSION(:,:):: NODE_CP,NODE_P,NODE_TMP,NODE_RHO,NODE_RSUM,DUCT_VF
...
CHARACTER(30) :: TYPE_ID,DEVC_ID,CTRL_ID,DUCT_ID(MAX_DUCTS),NODE_ID(2),VENT_ID,FAN_ID,AIRCOIL_ID,RAMP_ID,ID,FILTER_ID,&
...
NAMELIST /HVAC/ AIRCOIL_ID,AMBIENT,AREA,CLEAN_LOSS,COOLANT_CP,COOLANT_MDOT,COOLANT_TEMPERATURE,CTRL_ID,DAMPER,DEVC_ID,DIAMETER,&
                DUCT_ID,EFFICIENCY,FAN_ID,FILTER_ID,FIXED_Q,ID,LENGTH,LOADING,LOADING_MULTIPLIER,LOSS,MAX_FLOW,MAX_PRESSURE,&
                NODE_ID,RAMP_ID,REVERSE,ROUGHNESS,SPEC_ID,TAU_FAN,TAU_VF,TYPE_ID,VENT_ID,VOLUME_FLOW,XYZ !,ROUND, SQUARE
...
!         ENDIF
      ...
      CASE('AIRCOIL')
         ...
         AIRCOIL(I_AIRCOIL)%COOLANT_CP   = COOLANT_CP*1000._EB
...
ALLOCATE(DUCT_VF(1:N_DUCTS,1:NMESHES))
DUCT_VF(1:N_DUCTS,1:NMESHES) = 0._EB
...
DO NNE = 1, N_NETWORKS
   ...
   IF (NE%N_MATRIX > 0) THEN 
      ...
      DO WHILE (ITER < ITER_MAX)
         !WRITE(*,'(A,1X,I2,1X,E12.5,1X,E12.5,1X,E12.5)') '**** ITER **** ',ITER
         LHS = 0._EB
         RHS = 0._EB
         CALL SET_DONOR(NNE)
...
DO NN = 1,NE%N_DUCTNODES
   ...
   IF (DN%VENT_INDEX > 0 .OR. DN%LEAKAGE) THEN   
      ...
      IF (DU%VEL(NEW)*DN%DIR(1) < -ZERO_P) DN%UPDATED = .TRUE.
   ENDIF
ENDDO
DO ND = 1,NE%N_DUCTS
      ...
      DO ND = 1,DN%N_DUCTS
         ...
         IF (DN%FILTER_INDEX > 0) THEN
            ...
            DO NS = 1,N_TRACKED_SPECIES
               ...
               !IF (STRATIFICATION) ETOT = ETOT - DN%FILTER_LOADING(NS,3)*GVEC(3)*(DN%XYZ(3)-DN2%XYZ(3))
            ENDDO
         ENDIF
      ENDDO
...
NODELOOP : DO NN=1,NE%N_DUCTNODES
  ...
  NODECLASS: IF (DN%FILTER_INDEX > 0) THEN
     ...
     IF (DUCT(DN%DUCT_INDEX(1))%AREA < ZERO_P .OR. DUCT(DN%DUCT_INDEX(2))%AREA < ZERO_P) CYCLE
  ...
  ELSEIF(DN%VENT_INDEX > 0) THEN NODECLASS
     IF (DUCT(DN%DUCT_INDEX(1))%AREA < ZERO_P) CYCLE
     IF(DUCT(DN%DUCT_INDEX(1))%VEL(GUESS)*DN%DIR(1) < 0._EB) THEN
...
DO ND=1,N_DUCTS
   DUCT_VF(ND,:) = DUCT(ND)%VEL(NEW)*DUCT(ND)%AREA
ENDDO
