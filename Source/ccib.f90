...
! =10 Do not Split F in STRESS and IBM forces. Use only Stress Method F computed
...
PUBLIC :: ADD_CUTCELL_D_PBAR_DT, ADD_LINKEDCELL_D_PBAR_DT,ADD_CUTCELL_PSUM,ADD_LINKEDCELL_PSUM,&
          ...
          CCCOMPUTE_RADIATION,CCREGION_DIVERGENCE_PART_1,CC_VEL_FACE_TO_SOLID_NODE,CCIBM_BAROCLINIC_CORRECTION,&
          CCIBM_CHECK_DIVERGENCE,CCIBM_COMPUTE_VELOCITY_ERROR, &
          CCIBM_END_STEP,CCIBM_H_INTERP,CCIBM_NO_FLUX,CCIBM_PROJECT_VELOCITY, &
          CCIBM_RHO0W_INTERP,CCIBM_SET_DATA,CCIBM_VELOCITY_BC,&
          CCIBM_VELOCITY_FLUX,CCREGION_MATCH_VELOCITY,CCREGION_MATCH_VELOCITY_FLUX, &
          CCIBM_VELOCITY_NO_GRADH,CCREGION_DENSITY,CCREGION_COMPUTE_KRES,CCREGION_COMPUTE_VISCOSITY,&
          CCREGION_CUTFACE_VELOCITY,CCREGION_RESTORE_UVW_UNLINKED,&
          ...
          COPY_CC_UNKH_TO_HS, COPY_CC_HS_TO_UNKH, CUTFACE_VELOCITIES, &
          ...
          GET_CC_MATRIXGRAPH_H,GET_CC_IROW,GET_CC_UNKH,GET_CUTCELL_HP, GET_PRES_CFACE_BCS, &
          GET_PRES_CFACE, GET_PRES_CFACE_TEST, GET_UVWGAS_CFACE, GET_MUDNS_CFACE, GET_BOUNDFACE_GEOM_INFO_H, &
          GET_FH_FROM_PRHS_AND_BCS,GET_LINKED_VELOCITIES,GRADH_ON_CARTESIAN, &
          FINISH_CCIBM, INIT_CUTCELL_DATA,MESH_CC_EXCHANGE,NUMBER_UNKH_CUTCELLS,&
...
IF(PRES_FLAG==ULMAT_FLAG) THEN
  ...
  ELSE
     WALL_CELL_LOOP_2 : DO IW=1,N_EXTERNAL_WALL_CELLS
        WC => WALL(IW); BC => BOUNDARY_COORD(WC%BC_INDEX); ICC = CCVAR(BC%II,BC%JJ,BC%KK,IBM_IDCC)
...
! ASSUMES POINT_TO_MESH has been called and CCVAR(I,J,K,IBM_UNKZ) > 0.
...
REAL(EB):: P_BAR_CV
P_BAR_CV = PBAR_K - P_0(K) + P_0_CV(CCVAR(I,J,K,IBM_UNKZ)-UNKZ_IND(NM_START))
...
CC => CUT_CELL(CCVAR(I,J,K,IBM_IDCC))
...
! ASSUMES POINT_TO_MESH has been called and CCVAR(I,J,K,IBM_IDCC) > 0.
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         X1AXIS = IAXIS
         DO ILH=-1,0
            IRC  = FCVAR(I+ILH,J,K,IBM_IDRC,X1AXIS)
            MURDN = MU_TMP/DX(I)
            IF (IRC>0) THEN
         ...
         X1AXIS = JAXIS
         DO ILH=-1,0
            IRC  = FCVAR(I,J+ILH,K,IBM_IDRC,X1AXIS)
         ...
         X1AXIS = KAXIS
         DO ILH=-1,0
            IRC  = FCVAR(I,J,K+ILH,IBM_IDRC,X1AXIS)
            MURDN = MU_TMP/DZ(K)
            IF (IRC>0) THEN
               IF(RC_FACE(IRC)%SHAREDZ) CYCLE
               MURDN = MU_TMP/(RC_FACE(IRC)%XCEN(X1AXIS,HIGH_IND)-RC_FACE(IRC)%XCEN(X1AXIS,LOW_IND))
   ...
   DO JCC=1,CUT_CELL(ICC)%NCELL
      ...
      IFC_LOOP : DO IFC=1,CUT_CELL(ICC)%CCELEM(1,JCC)
         ...
         CASE(IBM_FTYPE_RCGAS) ! REGULAR GASPHASE
            ...
            SELECT CASE(X1AXIS)
            CASE(IAXIS)
               IRC  = FCVAR(I-1+ILH,J,K,IBM_IDRC,X1AXIS)
               MURDN = MU_TMP/DX(I)
               IF (IRC>0) THEN
                  IF(RC_FACE(IRC)%SHAREDZ) CYCLE
                  MURDN = MU_TMP/(RC_FACE(IRC)%XCEN(X1AXIS,HIGH_IND)-RC_FACE(IRC)%XCEN(X1AXIS,LOW_IND))
            ...
            CASE(JAXIS)
               IRC  = FCVAR(I,J-1+ILH,K,IBM_IDRC,X1AXIS)
               MURDN = TWOD_FCT * MU_TMP/DY(J)
               IF (IRC>0) THEN
                  IF(RC_FACE(IRC)%SHAREDZ) CYCLE
                  MURDN=TWOD_FCT*MU_TMP/(RC_FACE(IRC)%XCEN(X1AXIS,HIGH_IND)-RC_FACE(IRC)%XCEN(X1AXIS,LOW_IND))
            ...
            CASE(KAXIS)
               IRC  = FCVAR(I,J,K-1+ILH,IBM_IDRC,X1AXIS)
               MURDN = MU_TMP/DZ(K)
               IF (IRC>0) THEN
                  IF(RC_FACE(IRC)%SHAREDZ) CYCLE
                  MURDN = MU_TMP/(RC_FACE(IRC)%XCEN(X1AXIS,HIGH_IND)-RC_FACE(IRC)%XCEN(X1AXIS,LOW_IND))
         ...
         CASE(IBM_FTYPE_CFINB) ! INBOUNDARY CUT FACE: Note here we add U_NORMAL with Transpiration velocity due to
            ...
            AF      = CUT_FACE(IFC2)%AREA(IFACE2)
            VELN    = CUT_FACE(IFC2)%VELS(IFACE2)
            MURDN   = MU_TMP * BOUNDARY_ONE_D(CFACE(ICFA)%OD_INDEX)%RDN
...
DO_SEPARABLE_IF : IF (DO_SEPARABLE) THEN
   ...
   DO ILH=-1,0
      ...
      IRC  = FCVAR(I+ILH,J,K,IBM_IDRC,X1AXIS)
      IF (IRC>0) THEN
         IF(.NOT.GRADH_ON_CARTESIAN) RDN = 1._EB/(RC_FACE(IRC)%XCEN(X1AXIS,HIGH_IND) - &
                                                  RC_FACE(IRC)%XCEN(X1AXIS,LOW_IND))
      ENDIF
...
ELSE DO_SEPARABLE_IF
   ...
   DO ILH=-1,0
      ...
      IF (FCVAR(I+ILH,J,K,IBM_UNKF,IAXIS)>0) THEN
         FN_B = F_LINK(FCVAR(I+ILH,J,K,IBM_UNKF,IAXIS))
      ELSE
      ...
      IF (IRC>0) THEN
         ...
         ELSE
            ...
            ENDIF
            IF(RC_FACE(IRC)%CELL_LIST(1,LOW_IND)==IBM_FTYPE_CFGAS) THEN
            ...
            ENDIF
            IF(RC_FACE(IRC)%CELL_LIST(1,HIGH_IND)==IBM_FTYPE_CFGAS) THEN
   ...
   DO ILH=-1,0
      ...
      IF (FCVAR(I,J+ILH,K,IBM_UNKF,JAXIS)>0) THEN
         FN_B = F_LINK(FCVAR(I,J+ILH,K,IBM_UNKF,JAXIS))
      ELSE
   ...
   DO ILH=-1,0
      ...
      IF (FCVAR(I,J,K+ILH,IBM_UNKF,KAXIS)>0) THEN
         FN_B = F_LINK(FCVAR(I,J,K+ILH,IBM_UNKF,KAXIS))
      ELSE
      ...
      CASE(IBM_FTYPE_RCGAS) ! REGULAR GASPHASE
         ...
         CASE(IAXIS)
            ...
            IF(LOWHIGH==HIGH_IND) THEN
               ...
               X2   = XC(I+1); IRC  = FCVAR(I,J,K,IBM_IDRC,X1AXIS)
               IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X2=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
            ELSE
               HC1  = HP(I-1,J,K)
               HC2  = PRFCT*CUT_CELL(ICC)%H(JCC) + (1._EB-PRFCT)*CUT_CELL(ICC)%HS(JCC)
               X1   = XC(I-1); IRC  = FCVAR(I-1,J,K,IBM_IDRC,X1AXIS)
               IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X1=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
         ...
         CASE(JAXIS)
            ...
            IF(LOWHIGH==HIGH_IND) THEN
               ...
               X2   = YC(J+1); IRC  = FCVAR(I,J,K,IBM_IDRC,X1AXIS)
               IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X2=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
            ELSE
               HC1  = HP(I,J-1,K)
               HC2  = PRFCT*CUT_CELL(ICC)%H(JCC) + (1._EB-PRFCT)*CUT_CELL(ICC)%HS(JCC)
               X1   = YC(J-1); IRC  = FCVAR(I,J-1,K,IBM_IDRC,X1AXIS)
               IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X1=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
         ...
         CASE(KAXIS)
            ...
            IF(LOWHIGH==HIGH_IND) THEN
               ...
               X2   = ZC(K+1); IRC  = FCVAR(I,J,K,IBM_IDRC,X1AXIS)
               IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X2=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
            ELSE
               HC1  = HP(I,J,K-1)
               HC2  = PRFCT*CUT_CELL(ICC)%H(JCC) + (1._EB-PRFCT)*CUT_CELL(ICC)%HS(JCC)
               X1   = ZC(K-1); IRC  = FCVAR(I,J,K-1,IBM_IDRC,X1AXIS)
               IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X1=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
      ...
      CASE(IBM_FTYPE_RCGAS) ! REGULAR GASPHASE
         ...
         CASE(IAXIS)
            ...
            IF(LOWHIGH==HIGH_IND) THEN
               ...
               X1   = CUT_CELL(ICC)%XYZCEN(X1AXIS,JCC); IF(GRADH_ON_CARTESIAN) X1   = XC(I)
               X2   = XC(I+1); IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X2=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
               RHO2 = RHOP(I+1,J,K)
            ELSE
               P1   = P(I-1,J,K); KR1 = KRES(I-1,J,K)
               X1   = XC(I-1); IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X1=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
               X2   = CUT_CELL(ICC)%XYZCEN(X1AXIS,JCC); IF(GRADH_ON_CARTESIAN) X2   = XC(I)
               RHO1 = RHOP(I-1,J,K)
            ENDIF
         CASE(JAXIS)
            ...
            IF(LOWHIGH==HIGH_IND) THEN
               ...
               X1   = CUT_CELL(ICC)%XYZCEN(X1AXIS,JCC); IF(GRADH_ON_CARTESIAN) X1   = YC(J)
               X2   = YC(J+1); IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X2=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
               RHO2 = RHOP(I,J+1,K)
            ELSE
               P1   = P(I,J-1,K); KR1 = KRES(I,J-1,K)
               X1   = YC(J-1); IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X1=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
               X2   = CUT_CELL(ICC)%XYZCEN(X1AXIS,JCC); IF(GRADH_ON_CARTESIAN) X2   = YC(J)
               RHO1 = RHOP(I,J-1,K)
            ENDIF
         CASE(KAXIS)
            ...
            IF(LOWHIGH==HIGH_IND) THEN
               ...
               X1   = CUT_CELL(ICC)%XYZCEN(X1AXIS,JCC); IF(GRADH_ON_CARTESIAN) X1   = ZC(K)
               X2   = ZC(K+1); IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X2=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
               RHO2 = RHOP(I,J,K+1)
            ELSE
               P1   = P(I,J,K-1); KR1 = KRES(I,J,K-1)
               X1   = ZC(K-1); IF(.NOT.GRADH_ON_CARTESIAN .AND. IRC>0) X1=RC_FACE(IRC)%XCEN(X1AXIS,LOWHIGH)
               X2   = CUT_CELL(ICC)%XYZCEN(X1AXIS,JCC); IF(GRADH_ON_CARTESIAN) X2   = ZC(K)
               RHO1 = RHOP(I,J,K-1)
            ENDIF
         END SELECT
...
CF_LOOP : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
   CF => CUT_FACE(ICF); IF (CF%STATUS/=IBM_GASPHASE) CYCLE CF_LOOP
...
RC_LOOP : DO ICF=1,MESHES(NM)%IBM_NRCFACE_Z
   RCF => RC_FACE(ICF);
...
REAL(EB),INTENT(IN) :: PRFCT
TYPE(IBM_CUTFACE_TYPE), INTENT(IN), POINTER :: CF
...
IF(RC_FACE(ICF)%CELL_LIST(1,LOW_IND)==IBM_FTYPE_CFGAS) THEN
   ICC = RC_FACE(ICF)%CELL_LIST(2,LOW_IND); JCC = RC_FACE(ICF)%CELL_LIST(3,LOW_IND)
   RHO1= PRFCT*CUT_CELL(ICC)%RHO(JCC)+(1._EB-PRFCT)*CUT_CELL(ICC)%RHOS(JCC)
ENDIF
IF(RC_FACE(ICF)%CELL_LIST(1,HIGH_IND)==IBM_FTYPE_CFGAS) THEN
...
DO K=0,KBAR
   DO J=0,JBAR
      DO I=0,IBAR
         ! X face:
         IF (FCVAR(I,J,K,IBM_UNKF,IAXIS)>0) THEN
            FN_B = -(P(I,J,K)*RHOP(I+1,J,K)+P(I+1,J,K)*RHOP(I,J,K))*(1._EB/RHOP(I+1,J,K)-1._EB/RHOP(I,J,K))*RDXN(I)/ &
                             (RHOP(I+1,J,K)+RHOP(I,J,K))
            F_LINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = F_LINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + FN_B*DY(J)*DZ(K)
            A_LINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = A_LINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + DY(J)*DZ(K)
         ENDIF
         ! Y face:
         IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0) THEN
            FN_B = -(P(I,J,K)*RHOP(I,J+1,K)+P(I,J+1,K)*RHOP(I,J,K))*(1._EB/RHOP(I,J+1,K)-1._EB/RHOP(I,J,K))*RDYN(J)/ &
                             (RHOP(I,J+1,K)+RHOP(I,J,K))
            F_LINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = F_LINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + FN_B*DX(I)*DZ(K)
            A_LINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = A_LINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + DX(I)*DZ(K)
         ENDIF
         ! Z face:
         IF (FCVAR(I,J,K,IBM_UNKF,KAXIS)>0) THEN
            FN_B = -(P(I,J,K)*RHOP(I,J,K+1)+P(I,J,K+1)*RHOP(I,J,K))*(1._EB/RHOP(I,J,K+1)-1._EB/RHOP(I,J,K))*RDZN(K)/ &
                             (RHOP(I,J,K+1)+RHOP(I,J,K))
            F_LINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = F_LINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + FN_B*DX(I)*DY(J)
            A_LINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = A_LINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + DX(I)*DY(J)
         ENDIF
      ENDDO
   ENDDO
ENDDO
               ...
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                        IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                        IF(M2%FCVAR(IIO,JJO,KKO,IBM_FGSC,ABS(IOR))==IBM_SOLID) CYCLE

                        ICFO = M2%FCVAR(IIO,JJO,KKO,IBM_IDCF,ABS(IOR))
                        IF (ICFO>0) THEN
            ...
            CASE(-1)
               ...
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                        IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                        IF(M2%FCVAR(IIO-1,JJO,KKO,IBM_FGSC,ABS(IOR))==IBM_SOLID) CYCLE

                        ICFO = M2%FCVAR(IIO-1,JJO,KKO,IBM_IDCF,ABS(IOR))
                        IF (ICFO>0) THEN
                           CF2 =>M2%CUT_FACE(ICFO); A_FACE = 0._EB
                           DO JCFO=1,CF2%NFACE
                              A_FACE   = A_FACE + CF2%AREA(JCFO)
            ...
            CASE(-2)
               ...
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                        IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                        IF(M2%FCVAR(IIO,JJO-1,KKO,IBM_FGSC,ABS(IOR))==IBM_SOLID) CYCLE

                        ICFO = M2%FCVAR(IIO,JJO-1,KKO,IBM_IDCF,ABS(IOR))
                        IF (ICFO>0) THEN
                           CF2 =>M2%CUT_FACE(ICFO); A_FACE = 0._EB
                           DO JCFO=1,CF2%NFACE
                              A_FACE   = A_FACE + CF2%AREA(JCFO)
            ...
            CASE(-3)
               ...
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                        IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                        IF(M2%FCVAR(IIO,JJO,KKO-1,IBM_FGSC,ABS(IOR))==IBM_SOLID) CYCLE

                        ICFO = M2%FCVAR(IIO,JJO,KKO-1,IBM_IDCF,ABS(IOR))
                        IF (ICFO>0) THEN
                           CF2 =>M2%CUT_FACE(ICFO); A_FACE = 0._EB
                           DO JCFO=1,CF2%NFACE
                              A_FACE = A_FACE + CF2%AREA(JCFO)
...
ELSE STORE_IF
   ...
   PRED_CORR_IF : IF (PREDICTOR) THEN
      ...
      DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
         CF => CUT_FACE(ICF); IF(CF%STATUS /= IBM_INBOUNDARY) CYCLE
         ...
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=0,IBAR
                  ...
                  IF (ICF>0) THEN
                     CF => CUT_FACE(ICF)
                     DO JCF=1,CF%NFACE ! NOTE: At this point FV+FB are stored in FN(JCF) similar to FVX,FVY,FVZ.
                        CF%VELS(JCF) = CF%VEL(JCF) - DT*( CF%FN(JCF) + RDXN(I)*(H(I+1,J,K)-H(I,J,K)) )
                     ENDDO
            ...
            DO J=0,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     CF => CUT_FACE(ICF)
                     DO JCF=1,CF%NFACE
                        CF%VELS(JCF) = CF%VEL(JCF) - DT*( CF%FN(JCF) + RDYN(J)*(H(I,J+1,K)-H(I,J,K)) )
                     ENDDO
            ...
            DO J=1,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     CF => CUT_FACE(ICF)
                     DO JCF=1,CF%NFACE
                        CF%VELS(JCF) = CF%VEL(JCF) - DT*( CF%FN(JCF) + RDZN(K)*(H(I,J,K+1)-H(I,J,K)) )
                     ENDDO
                     WS(I,J,K) = DOT_PRODUCT(CF%VELS(1:CF%NFACE),CF%AREA(1:CF%NFACE)) / (DX(I)*DY(J))
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
      ...
      ELSE GRADH_ON_CARTESIAN_IF_1
         ...
         WALL_CELL_LOOP_1 : DO IW=1,N_EXTERNAL_WALL_CELLS
            ...
            CASE( 3); K = BC%KKG-1
            END SELECT
            IRC = FCVAR(I,J,K,IBM_IDRC,ABS(IOR)); IF(IRC < 1) CYCLE WALL_CELL_LOOP_1 ! Case of RC face in the boundary.
   ...
   ELSE PRED_CORR_IF
            ...
            DO J=1,JBAR
               DO I=0,IBAR
                  ...
                  IF (ICF>0) THEN
                     CF => CUT_FACE(ICF)
                     DO JCF=1,CF%NFACE
                        CF%VEL(JCF) = 0.5_EB*( CF%VEL(JCF) +CF%VELS(JCF) - DT*( CF%FN(JCF) + &
                                                            RDXN(I)*(HS(I+1,J,K)-HS(I,J,K))) )
                     ENDDO
            ...
            DO J=1,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     DO JCF=1,CF%NFACE
                        CF%VEL(JCF) = 0.5_EB*( CF%VEL(JCF) +CF%VELS(JCF) - DT*( CF%FN(JCF) + &
                                                            RDZN(K)*(HS(I,J,K+1)-HS(I,J,K))) )
                     ENDDO
                     W(I,J,K) = DOT_PRODUCT(CF%VEL(1:CF%NFACE),CF%AREA(1:CF%NFACE)) / (DX(I)*DY(J))
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
      ...
      ELSE GRADH_ON_CARTESIAN_IF_2
         ...
         WALL_CELL_LOOP_2 : DO IW=1,N_EXTERNAL_WALL_CELLS
            ...
            CASE( 3); K = BC%KKG-1
            END SELECT
            IRC = FCVAR(I,J,K,IBM_IDRC,ABS(IOR)); IF(IRC < 1) CYCLE WALL_CELL_LOOP_2 ! Case of RC face in the boundary.
            IDX = 1._EB/( RC_FACE(IRC)%XCEN(ABS(BC%IOR),HIGH_IND) - RC_FACE(IRC)%XCEN(ABS(BC%IOR),LOW_IND) )
            SELECT CASE (ABS(IOR))
...
TYPE (MESH_TYPE), POINTER :: M2
TYPE (IBM_CUTFACE_TYPE), POINTER :: CFO
...
CMP_FLG_IF : IF (CMP_FLG) THEN
   ...
   PRED_1 : IF (APPLY_TO_ESTIMATED_VARIABLES) THEN
      ...
      DO K=0,KBAR
         DO J=0,JBAR
            DO I=0,IBAR
               IF (FCVAR(I,J,K,IBM_UNKF,IAXIS)>0) THEN ! X face
                  UN_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + US(I,J,K)*DY(J)*DZ(K)
                  AREA_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = AREA_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + DY(J)*DZ(K)
               ENDIF
               IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0) THEN ! Y face
                  UN_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + VS(I,J,K)*DX(I)*DZ(K)
                  AREA_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = AREA_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + DX(I)*DZ(K)
               ENDIF
               IF (FCVAR(I,J,K,IBM_UNKF,KAXIS)>0) THEN ! Z face
                  UN_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + WS(I,J,K)*DX(I)*DY(J)
                  AREA_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = AREA_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + DX(I)*DY(J)
               ENDIF
            ENDDO
         ENDDO
      ENDDO
   ...
   ELSE PRED_1
      ...
      DO K=0,KBAR
         DO J=0,JBAR
            DO I=0,IBAR
               IF (FCVAR(I,J,K,IBM_UNKF,IAXIS)>0) THEN
                  UN_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + U(I,J,K)*DY(J)*DZ(K)
                  AREA_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = AREA_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + DY(J)*DZ(K)
               ENDIF
               IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0) THEN
                  UN_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + V(I,J,K)*DX(I)*DZ(K)
                  AREA_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = AREA_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + DX(I)*DZ(K)
               ENDIF
               IF (FCVAR(I,J,K,IBM_UNKF,KAXIS)>0) THEN
                  UN_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + W(I,J,K)*DX(I)*DY(J)
                  AREA_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = AREA_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + DX(I)*DY(J)
               ENDIF
            ENDDO
         ENDDO
...
ELSE CMP_FLG_IF
   ...
   PRED_2 : IF (APPLY_TO_ESTIMATED_VARIABLES) THEN
      ...
      DO K=0,KBAR
         DO J=0,JBAR
            DO I=0,IBAR
               IF (FCVAR(I,J,K,IBM_UNKF,IAXIS)>0)                  US(I,J,K) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS))
               IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0 .AND. .NOT.TWO_D) VS(I,J,K) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS))
               IF (FCVAR(I,J,K,IBM_UNKF,KAXIS)>0)                  WS(I,J,K) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS))
            ENDDO
         ENDDO
      ENDDO
      DO ICF=1,MESHES(NM)%IBM_NRCFACE_Z
      ...
      EXTERNAL_WALL_LOOP_2: DO IW=1,MESHES(NM)%N_EXTERNAL_WALL_CELLS
         ...
         CASE( 1)
            DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
               DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                  DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                     IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE ! Drop if OMESH cell is solid.
                     IF(M2%FCVAR(IIO,JJO,KKO,IBM_FGSC,ABS(IOR))==IBM_SOLID) CYCLE  ! Drop if solid cart face.
                     ICFO = M2%FCVAR(IIO,JJO,KKO,IBM_IDCF,ABS(IOR))
                     IF(ICFO>0) THEN
   ...
   ELSE PRED_2
      ...
      ! Save original U,V,W and CF%VEL to restore in Predictor call to CCREGION_RESTORE_UVW_UNLINKED(NM):
      IF (FIRST_PASS) CALL CCREGION_RESTORE_UVW_UNLINKED(NM,ASSIGN_UNLINKED_VEL=.FALSE.)

      ! Assign linked velocities to U,V,W:
      DO K=0,KBAR
         DO J=0,JBAR
            DO I=0,IBAR
               IF (FCVAR(I,J,K,IBM_UNKF,IAXIS)>0)                  U(I,J,K) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,IAXIS))
               IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0 .AND. .NOT.TWO_D) V(I,J,K) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,JAXIS))
               IF (FCVAR(I,J,K,IBM_UNKF,KAXIS)>0)                  W(I,J,K) = UN_LNK(FCVAR(I,J,K,IBM_UNKF,KAXIS))
            ENDDO
         ENDDO
      ENDDO
      DO ICF=1,MESHES(NM)%IBM_NRCFACE_Z
...
SUBROUTINE CCREGION_RESTORE_UVW_UNLINKED(NM,ASSIGN_UNLINKED_VEL)
   ...
   DO K=0,KBAR
      DO J=0,JBAR
         DO I=0,IBAR
            ...
            ENDIF
            IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0 .AND. .NOT.TWO_D) THEN
...
END SUBROUTINE CCREGION_RESTORE_UVW_UNLINKED
...
CUTCELL_DO : DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
   CC => CUT_CELL(ICC); I = CC%IJK(IAXIS); J = CC%IJK(JAXIS); K = CC%IJK(KAXIS); IF(SOLID(CELL_INDEX(I,J,K))) CYCLE
   IF (ONE_UNKH_PER_CUTCELL) THEN
      DO JCC=1,CC%NCELL
      ...
      RNODE_SNODE_IF: IF (RNODE/=SNODE) THEN
         ...
         IF (CODE==1 .AND. M2%NICC_R(1)>0) THEN
            ...
            EXTERNAL_WALL_LOOP_1 : DO IW=1,M%N_EXTERNAL_WALL_CELLS
               ...
               IF (M%CCVAR(BC%II,BC%JJ,BC%KK,IBM_CGSC) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
         ...
         EXTERNAL_WALL_LOOP_112A : DO IW=1,M%N_EXTERNAL_WALL_CELLS
            ...
            SELECT CASE(BC%IOR)
            CASE( IAXIS); IF(M%FCVAR(IIO-1,JJO  ,KKO  ,IBM_FGSC,IAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112A
            CASE(-IAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,IAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112A
            CASE( JAXIS); IF(M%FCVAR(IIO  ,JJO-1,KKO  ,IBM_FGSC,JAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112A
            CASE(-JAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,JAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112A
            CASE( KAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO-1,IBM_FGSC,KAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112A
            CASE(-KAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,KAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112A
            END SELECT
            DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
               DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                  DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                   SELECT CASE(-BC%IOR)
                   CASE( IAXIS); ICF=MESHES(NM)%FCVAR(IIO-1,JJO  ,KKO  ,IBM_IDCF,IAXIS)
                   CASE(-IAXIS); ICF=MESHES(NM)%FCVAR(IIO  ,JJO  ,KKO  ,IBM_IDCF,IAXIS)
                   CASE( JAXIS); ICF=MESHES(NM)%FCVAR(IIO  ,JJO-1,KKO  ,IBM_IDCF,JAXIS)
                   CASE(-JAXIS); ICF=MESHES(NM)%FCVAR(IIO  ,JJO  ,KKO  ,IBM_IDCF,JAXIS)
                   CASE( KAXIS); ICF=MESHES(NM)%FCVAR(IIO  ,JJO  ,KKO-1,IBM_IDCF,KAXIS)
                   CASE(-KAXIS); ICF=MESHES(NM)%FCVAR(IIO  ,JJO  ,KKO  ,IBM_IDCF,KAXIS)
                   END SELECT
                   IF (ICF > 0) THEN
         ...
         IF(CODE==3 .AND. M2%NICF_R(1)>0) THEN
            ...
            EXTERNAL_WALL_LOOP_112B : DO IW=1,M%N_EXTERNAL_WALL_CELLS
               ...
               BC=>M%BOUNDARY_COORD(WC%BC_INDEX); IIO = BC%IIG; JJO = BC%JJG; KKO = BC%KKG
               SELECT CASE(BC%IOR)
               CASE( IAXIS); IF(M%FCVAR(IIO-1,JJO  ,KKO  ,IBM_FGSC,IAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112B
               CASE(-IAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,IAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112B
               CASE( JAXIS); IF(M%FCVAR(IIO  ,JJO-1,KKO  ,IBM_FGSC,JAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112B
               CASE(-JAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,JAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112B
               CASE( KAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO-1,IBM_FGSC,KAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112B
               CASE(-KAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,KAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112B
               END SELECT
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
         ...
         IF(CODE==6 .AND. M2%NICF_R(1)>0) THEN
            ...
            EXTERNAL_WALL_LOOP_112C : DO IW=1,M%N_EXTERNAL_WALL_CELLS
               ...
               BC=>M%BOUNDARY_COORD(WC%BC_INDEX); IIO = BC%IIG; JJO = BC%JJG; KKO = BC%KKG
               SELECT CASE(BC%IOR)
               CASE( IAXIS); IF(M%FCVAR(IIO-1,JJO  ,KKO  ,IBM_FGSC,IAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112C
               CASE(-IAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,IAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112C
               CASE( JAXIS); IF(M%FCVAR(IIO  ,JJO-1,KKO  ,IBM_FGSC,JAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112C
               CASE(-JAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,JAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112C
               CASE( KAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO-1,IBM_FGSC,KAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112C
               CASE(-KAXIS); IF(M%FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,KAXIS)/=IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_112C
               END SELECT
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
...
REAL(EB):: PRFCT, VCELL, RHO_CC, TMP_CC, RSUM_CC, D_CC, ZZ_CC(1:N_TOTAL_SCALARS), VOL, VOLR
TYPE (OMESH_TYPE), POINTER :: OM
TYPE(IBM_CUTCELL_TYPE), POINTER :: OCC
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   EXTERNAL_WALL_LOOP : DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
              IF(MESHES(NOM)%SOLID(MESHES(NOM)%CELL_INDEX(IIO,JJO,KKO))) CYCLE
...
SUBROUTINE CCREGION_MATCH_VELOCITY_FLUX(NM)
...
EXTERNAL_WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   SELECT CASE(IOR)
      ...
      CASE( 1)
         ...
         ICF = FCVAR(0,JJ,KK,IBM_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
      ...
      CASE(-1)
         ...
         ICF = FCVAR(IBAR,JJ,KK,IBM_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
      ...
      CASE( 2)
         ...
         ICF = FCVAR(II,0,KK,IBM_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
      ...
      CASE(-2)
         ...
         ICF = FCVAR(II,JBAR,KK,IBM_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
      ...
      CASE( 3)
         ...
         ICF = FCVAR(II,JJ,0,IBM_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
      ...
      CASE(-3)
         ...
         ICF = FCVAR(II,JJ,KBAR,IBM_IDCF,ABS(IOR))
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
...
RETURN
END SUBROUTINE CCREGION_MATCH_VELOCITY_FLUX


! ------------------------------ CCREGION_MATCH_VELOCITY ----------------------------

SUBROUTINE CCREGION_MATCH_VELOCITY(NM,APPLY_TO_ESTIMATED_VARIABLES,DO_U_GHOST)
...
EXTERNAL_WALL_LOOP: DO IW=1,MESHES(NM)%N_EXTERNAL_WALL_CELLS
   ...
   CASE( IAXIS)
      ...
      ICF = FCVAR(0,JJ,KK,IBM_IDCF,ABS(IOR)); JCF = 1;
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX

               IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE ! Drop if OMESH cell is solid.
               IF(M2%FCVAR(IIO,JJO,KKO,IBM_FGSC,ABS(IOR))==IBM_SOLID) CYCLE  ! Drop if cartesian face tagged solid.
   ...
   CASE(-IAXIS)
      ...
      IF (APPLY_TO_ESTIMATED_VARIABLES) THEN; OM_UU => OM%US; ELSE; OM_UU => OM%U; ENDIF
      ICF = FCVAR(IBAR,JJ,KK,IBM_IDCF,ABS(IOR)); JCF = 1;
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
   ...
   CASE( JAXIS)
      ...
      ICF = FCVAR(II,0,KK,IBM_IDCF,ABS(IOR)); JCF = 1
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
   ...
   CASE(-JAXIS)
      ...
      IF (APPLY_TO_ESTIMATED_VARIABLES) THEN; OM_VV => OM%VS; ELSE; OM_VV => OM%V; ENDIF
      ICF = FCVAR(II,JBAR,KK,IBM_IDCF,ABS(IOR)); JCF = 1
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
   ...
   CASE( KAXIS)
      ...
      ICF = FCVAR(II,JJ,0,IBM_IDCF,ABS(IOR)); JCF = 1
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
   ...
   CASE(-KAXIS)
      ...
      IF (APPLY_TO_ESTIMATED_VARIABLES) THEN; OM_WW => OM%WS; ELSE; OM_WW => OM%W; ENDIF
      ICF = FCVAR(II,JJ,KBAR,IBM_IDCF,ABS(IOR)); JCF = 1
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
   ...
   IF (DO_U_GHOST) THEN
      ...
      DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
               AU =1._EB; ICF=M2%FCVAR(IIO  ,JJO,KKO,IBM_IDCF,IAXIS); IF(ICF>0) AU =M2%CUT_FACE(ICF)%ALPHA_CF
               AU1=1._EB; ICF=M2%FCVAR(IIO-1,JJO,KKO,IBM_IDCF,IAXIS); IF(ICF>0) AU1=M2%CUT_FACE(ICF)%ALPHA_CF
               AV =1._EB; ICF=M2%FCVAR(IIO,JJO  ,KKO,IBM_IDCF,JAXIS); IF(ICF>0) AV =M2%CUT_FACE(ICF)%ALPHA_CF
               AV1=1._EB; ICF=M2%FCVAR(IIO,JJO-1,KKO,IBM_IDCF,JAXIS); IF(ICF>0) AV1=M2%CUT_FACE(ICF)%ALPHA_CF
               AW =1._EB; ICF=M2%FCVAR(IIO,JJO,KKO  ,IBM_IDCF,KAXIS); IF(ICF>0) AW =M2%CUT_FACE(ICF)%ALPHA_CF
               AW1=1._EB; ICF=M2%FCVAR(IIO,JJO,KKO-1,IBM_IDCF,KAXIS); IF(ICF>0) AW1=M2%CUT_FACE(ICF)%ALPHA_CF
...
END SUBROUTINE CCREGION_MATCH_VELOCITY
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=0,IBAR
         IF (CC_IBM) THEN
            IF (FCVAR(I,J,K,IBM_FGSC,IAXIS) == IBM_SOLID) CYCLE    ! Stress method.
         ENDIF
         ! Kinematic Viscosity:
...
DO K=0,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         IF (CC_IBM) THEN
            IF (FCVAR(I,J,K,IBM_FGSC,KAXIS) == IBM_SOLID) CYCLE    ! Stress method.
         ENDIF
         ! Kinematic Viscosity:
...
SUBROUTINE CCIBM_VELOCITY_CUTFACES(APPLY_TO_ESTIMATED_VARIABLES)
...
IF((CC_STM_TYPE==10) .OR. CC_STM_TYPE==2) THEN
   ...
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      ...
      DO JCC=1,CC%NCELL
         ...
         DO ICF_CELL=1,CC%CCELEM(1,JCC)
            ...
            IF (CC%FACE_LIST(1,IFACE_CELL) /= IBM_FTYPE_CFINB) CYCLE
...
MESH_LOOP_02 : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      CF => CUT_FACE(ICF); IF(CF%STATUS /= IBM_INBOUNDARY) CYCLE
      I = CF%IJK(IAXIS); J = CF%IJK(JAXIS); K = CF%IJK(KAXIS); IF(SOLID(CELL_INDEX(I,J,K))) CYCLE
      DO IFACE=1,CF%NFACE
         IF (CF%CELL_LIST(1,LOW_IND,IFACE) /= IBM_FTYPE_CFGAS) CYCLE
         ICC = CF%CELL_LIST(2,LOW_IND,IFACE)
         JCC = CF%CELL_LIST(3,LOW_IND,IFACE)
         IROW = CUT_CELL(ICC)%UNKZ(JCC) - UNKZ_IND(NM_START)
         BOUNDARY_ONE_D(CFACE(CF%CFACE_INDEX(IFACE))%OD_INDEX)%RDN = 1._EB/DXN_UNKZ_LOC(IROW)
      ENDDO
   ENDDO
ENDDO MESH_LOOP_02
...
CALL CCIBM_VELOCITY_CUTFACES(PREDICTOR)
...
INTEGER :: INT_NPE_LO,INT_NPE_HI,INPE,VIND,EP,IW,IROW_LOC
...
MESH_LOOP_0 : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (CCVAR(I,J,K,IBM_UNKZ) <= 0) CYCLE
            IROW_LOC     = CCVAR(I,J,K,IBM_UNKZ) - UNKZ_IND(NM_START)
   ...
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      CC => CUT_CELL(ICC); IF(SOLID(CELL_INDEX(CC%IJK(IAXIS),CC%IJK(JAXIS),CC%IJK(KAXIS)))) CYCLE
      DO JCC=1,CC%NCELL
...
MESH_LOOP_1 : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   PERIODIC_TEST_COND : IF (PERIODIC_TEST /= 21 .AND. PERIODIC_TEST /= 22 .AND. PERIODIC_TEST /= 24) THEN
      ...
      DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
         ...
         SELECT CASE(X1AXIS)
         CASE(IAXIS); U(I,J,K) = VEL_CF/(DY(J)*DZ(K))
         CASE(JAXIS); V(I,J,K) = VEL_CF/(DX(I)*DZ(K))
         CASE(KAXIS); W(I,J,K) = VEL_CF/(DX(I)*DY(J))
         END SELECT
      ENDDO
...
! Flux match Cartesian face velocity back to cut-faces:
CALL CCIBM_VELOCITY_CUTFACES(APPLY_TO_ESTIMATED_VARIABLES=.FALSE.)
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            RHO_0_CV( CCVAR(I,J,K,IBM_UNKZ)-UNKZ_IND(NM_START) )*GVEC(KAXIS)*DX(I)*DY(J)*DZ(K)
         ENDDO
      ENDDO
   ENDDO
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
...
INTEGER :: IOR, ICFA
TYPE(BOUNDARY_ONE_D_TYPE), POINTER :: ONE_D
TYPE(IBM_REGFACEZ_TYPE),  POINTER, DIMENSION(:) :: RGF=>NULL()
...
! IAXIS faces:
X1AXIS = IAXIS; RGF => IBM_REGFACE_IAXIS_Z
DO IFACE=1,MESHES(NM)%IBM_NREGFACE_Z(X1AXIS)
...
! JAXIS faces:
X1AXIS = JAXIS; RGF => IBM_REGFACE_JAXIS_Z
DO IFACE=1,MESHES(NM)%IBM_NREGFACE_Z(X1AXIS)
...
! KAXIS faces:
X1AXIS = KAXIS; RGF => IBM_REGFACE_KAXIS_Z
DO IFACE=1,MESHES(NM)%IBM_NREGFACE_Z(X1AXIS)
   ...
   CF_ON_WALL_CELL_IF : IF (IW > 0) THEN
      ...
      DO IFACE=1,CF%NFACE
         ...
         IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN ! Here used the CFA corresponding U_NORMAL.
            FCT  = 1._EB
            ONE_D => BOUNDARY_ONE_D(CFACE(CF%CFACE_INDEX(IFACE))%OD_INDEX)
         ...
         ENDIF
         ! First (rho hs)_i,j,k:
         IF (CF%CELL_LIST(1,ISIDE+2,IFACE) == IBM_FTYPE_CFGAS) THEN
...
DO IFACE=1,MESHES(NM)%IBM_NBBRCFACE_Z
   ...
   SELECT CASE(RC_FACE(IFACE)%CELL_LIST(1,ISIDE+2))
   CASE(IBM_FTYPE_RGGAS) ! Regular cell
   DPVOL(IIG,JJG,KKG)=DPVOL(IIG,JJG,KKG)+FCT*SUM(RC_FACE(IFACE)%H_RHO_D_DZDN(1:N_TOTAL_SCALARS))*AF ! +ve or -ve dot
   DO N=1,N_TOTAL_SCALARS
      DEL_RHO_D_DEL_Z(IIG,JJG,KKG,N)=DEL_RHO_D_DEL_Z(IIG,JJG,KKG,N)+FCT*RC_FACE(IFACE)%RHO_D_DZDN(N)*AF
   ENDDO
   CASE(IBM_FTYPE_CFGAS) ! Cut-cell
   ICC = RC_FACE(IFACE)%CELL_LIST(2,ISIDE+2)
   JCC = RC_FACE(IFACE)%CELL_LIST(3,ISIDE+2)
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               PBAR_K = PBAR_S(K,PRESSURE_ZONE(I,J,K)) - P_0(K) + P_0_CV(CCVAR(I,J,K,IBM_UNKZ)-UNKZ_IND(NM_START))
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM(I,J,K))
               PBAR_K = PBAR(K,PRESSURE_ZONE(I,J,K)) - P_0(K) + P_0_CV(CCVAR(I,J,K,IBM_UNKZ)-UNKZ_IND(NM_START))
...
! ------------------------------ GET_RHOZZ_CCREGION_3D ---------------------------

SUBROUTINE GET_RHOZZ_CCREGION_3D
...
MESH_LOOP : DO NM=1,NMESHES
   ...
   IF (PREDICTOR) THEN
      ...
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         CC => CUT_CELL(ICC)
         IF( SOLID(CELL_INDEX(CC%IJK(IAXIS),CC%IJK(JAXIS),CC%IJK(KAXIS))) ) CYCLE
         DO JCC=1,CC%NCELL
            ...
            IF (DEBUG_CCREGION_SCALAR_TRANSPORT) THEN
               ...
               IF ( (CC%RHOS(JCC)<RHOMIN) .OR. (CC%RHOS(JCC)>RHOMAX) ) THEN
                  WRITE(LU_ERR,*) 'GET_RHOZZ_CCREGION_3D CC Pred:',ICC,JCC,', Vol Fraction=',&
            ...
            IF (DEBUG_CCREGION_SCALAR_TRANSPORT) THEN
               ...
               DO N=1,N_TOTAL_SCALARS
                  IF ( (CC%ZZS(N,JCC)<(0._EB-GEOMEPS)) .OR. (CC%ZZS(N,JCC)>(1._EB+GEOMEPS)) ) THEN
                     WRITE(LU_ERR,*) 'GET_RHOZZ_CCREGION_3D CC Pred:',ICC,JCC,N
         ...
         DO JCC=1,CC%NCELL
            ...
            IF (DEBUG_CCREGION_SCALAR_TRANSPORT) THEN
               ...
               IF ( (CC%RHO(JCC)<RHOMIN) .OR. (CC%RHO(JCC)>RHOMAX) ) THEN
                  WRITE(LU_ERR,*) 'GET_RHOZZ_CCREGION_3D CC Corr:',ICC,JCC,', Vol Fraction=',&
                  CC%VOLUME(JCC)/(DX(CC%IJK(IAXIS))*DY(CC%IJK(JAXIS))*DZ(CC%IJK(KAXIS))),', UNK_Z=',CC%UNKZ(JCC)
            ...
            IF (DEBUG_CCREGION_SCALAR_TRANSPORT) THEN
               ...
               DO N=1,N_TOTAL_SCALARS
                  IF ( (CC%ZZ(N,JCC)<(0._EB-GEOMEPS)) .OR. (CC%ZZ(N,JCC)>(1._EB+GEOMEPS)) ) THEN
                     WRITE(LU_ERR,*) 'GET_RHOZZ_CCREGION_3D CC Corr:',ICC,JCC,N
         ...
         DO I=1,IBAR
            ...
            IF (DEBUG_CCREGION_SCALAR_TRANSPORT) THEN
               IF ((RHOP(I,J,K)<RHOMIN) .OR. (RHOP(I,J,K)>RHOMAX) ) THEN
                  WRITE(LU_ERR,*) 'GET_RHOZZ_CCREGION_3D Cart:',I,J,K
            ...
            IF (DEBUG_CCREGION_SCALAR_TRANSPORT) THEN
               ...
               DO N=1,N_TOTAL_SCALARS
                  IF (ZZP(I,J,K,N)<(0._EB-GEOMEPS) .OR. ZZP(I,J,K,N)>(1._EB+GEOMEPS)) THEN
                     WRITE(LU_ERR,*) 'GET_RHOZZ_CCREGION_3D Cart:',I,J,K,N
...
RETURN
END SUBROUTINE GET_RHOZZ_CCREGION_3D
...
MESH_LOOP_DBND : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   IFACE_LOOP_RCF1: DO IFACE=1,MESHES(NM)%IBM_NRCFACE_Z
...
SUBROUTINE CCIBM_VELOCITY_BC(T,NM,APPLY_TO_ESTIMATED_VARIABLES,DO_IBEDGES)
...
! Note approximation from cartesian face velocities which have been filled in MESH_EXCHANGE before CCIBM_END_STEP.
CUTFACE_LOOP : DO ICF=MESHES(NM)%N_CUTFACE_MESH+1,MESHES(NM)%N_CUTFACE_MESH+MESHES(NM)%N_GCCUTFACE_MESH
   ...
   IF (PREDICTOR) THEN; CF%VELS(1:CF%NFACE) = CF%VEL_CF ! Assign to all cut-faces same velocity.
...
IF (CE%STATUS/=IBM_GASPHASE .OR. TWO_D .OR. CE%IE==0) RETURN
...
SUBROUTINE CCIBM_VELOCITY_FLUX(NM,DT,APPLY_TO_ESTIMATED_VARIABLES,RHOP,GX,GY,GZ)

! Assumes POINT_TO_MESH has been called.

INTEGER, INTENT(IN) :: NM
LOGICAL, INTENT(IN) :: APPLY_TO_ESTIMATED_VARIABLES
REAL(EB),INTENT(IN) :: DT,GX(0:IBAR_MAX),GY(0:IBAR_MAX),GZ(0:IBAR_MAX)
...
CUTFACE_LOOP_0 : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
   CF => CUT_FACE(ICF); IF(CF%STATUS /= IBM_GASPHASE) CYCLE CUTFACE_LOOP_0
   I = CF%IJK(IAXIS); J = CF%IJK(JAXIS); K = CF%IJK(KAXIS); X1AXIS = CF%IJK(KAXIS+1)
...
RC_FACE_LOOP : DO IRC=1,MESHES(NM)%IBM_NRCFACE_Z
   RCF    =>RC_FACE(IRC)
   I      = RCF%IJK(IAXIS); J = RCF%IJK(JAXIS); K = RCF%IJK(KAXIS); X1AXIS = RCF%IJK(KAXIS+1)
   IDX = 1._EB/(RCF%XCEN(X1AXIS,HIGH_IND)-RCF%XCEN(X1AXIS,LOW_IND))
   SELECT CASE(X1AXIS)
   CASE(IAXIS)
      ...
      IF(CCVAR(I  ,J,K,IBM_UNKZ)>0) RHO0V(-1) = RHO_0_CV( CCVAR(I  ,J,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
      IF(CCVAR(I+1,J,K,IBM_UNKZ)>0) RHO0V( 0) = RHO_0_CV( CCVAR(I+1,J,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
      DO ISIDE=-1,0
         SELECT CASE(RCF%CELL_LIST(1,ISIDE+2))
         CASE(IBM_FTYPE_CFGAS) ! Cut-cell
   ...
   CASE(JAXIS)
      ...
      IF(CCVAR(I,J  ,K,IBM_UNKZ)>0) RHO0V(-1) = RHO_0_CV( CCVAR(I,J  ,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
      IF(CCVAR(I,J+1,K,IBM_UNKZ)>0) RHO0V( 0) = RHO_0_CV( CCVAR(I,J+1,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
      DO ISIDE=-1,0
         SELECT CASE(RCF%CELL_LIST(1,ISIDE+2))
         CASE(IBM_FTYPE_CFGAS) ! Cut-cell
            ICC = RCF%CELL_LIST(2,ISIDE+2); JCC = RCF%CELL_LIST(3,ISIDE+2); CC=>CUT_CELL(ICC)
   ...
   CASE(KAXIS)
      ...
      IF(CCVAR(I,J,K  ,IBM_UNKZ)>0) RHO0V(-1) = RHO_0_CV( CCVAR(I,J,K  ,IBM_UNKZ)-UNKZ_IND(NM_START) )
      IF(CCVAR(I,J,K+1,IBM_UNKZ)>0) RHO0V( 0) = RHO_0_CV( CCVAR(I,J,K+1,IBM_UNKZ)-UNKZ_IND(NM_START) )
      DO ISIDE=-1,0
         SELECT CASE(RCF%CELL_LIST(1,ISIDE+2))
         CASE(IBM_FTYPE_CFGAS) ! Cut-cell
            ICC = RCF%CELL_LIST(2,ISIDE+2); JCC = RCF%CELL_LIST(3,ISIDE+2); CC=>CUT_CELL(ICC)
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=0,IBAR
         IF(ANY(CCVAR(I:I+1,J,K,IBM_CGSC) /= IBM_GASPHASE)) CYCLE
         IF(ALL(CCVAR(I:I+1,J,K,IBM_UNKZ) <= 0)) CYCLE
         RHOV(-1:0) = RHOP(I:I+1,J,K); RHO0V(-1:0) = RHO_0(K)
         IF(CCVAR(I  ,J,K,IBM_UNKZ)>0) RHO0V(-1) = RHO_0_CV( CCVAR(I  ,J,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
         IF(CCVAR(I+1,J,K,IBM_UNKZ)>0) RHO0V( 0) = RHO_0_CV( CCVAR(I+1,J,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
         FVCC = (CCM1*RHO0V(-1) + CCP1*RHO0V(0)) / (CCM1*RHOV(-1) + CCP1*RHOV(0))
...
DO K=1,KBAR
   DO J=0,JBAR
      DO I=1,IBAR
         IF(ANY(CCVAR(I,J:J+1,K,IBM_CGSC) /= IBM_GASPHASE)) CYCLE
         IF(ALL(CCVAR(I,J:J+1,K,IBM_UNKZ) <= 0)) CYCLE
         RHOV(-1:0) = RHOP(I,J:J+1,K); RHO0V(-1:0) = RHO_0(K)
         IF(CCVAR(I,J  ,K,IBM_UNKZ)>0) RHO0V(-1) = RHO_0_CV( CCVAR(I,J  ,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
         IF(CCVAR(I,J+1,K,IBM_UNKZ)>0) RHO0V( 0) = RHO_0_CV( CCVAR(I,J+1,K,IBM_UNKZ)-UNKZ_IND(NM_START) )
         FVCC = (CCM1*RHO0V(-1) + CCP1*RHO0V(0)) / (CCM1*RHOV(-1) + CCP1*RHOV(0))
...
DO K=0,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         IF(ANY(CCVAR(I,J,K:K+1,IBM_CGSC) /= IBM_GASPHASE)) CYCLE
         IF(ALL(CCVAR(I,J,K:K+1,IBM_UNKZ) <= 0)) CYCLE
         RHOV(-1:0) = RHOP(I,J,K:K+1); RHO0V(-1:0) = RHO_0(K:K+1)
         IF(CCVAR(I,J,K  ,IBM_UNKZ)>0) RHO0V(-1) = RHO_0_CV( CCVAR(I,J,K  ,IBM_UNKZ)-UNKZ_IND(NM_START) )
         IF(CCVAR(I,J,K+1,IBM_UNKZ)>0) RHO0V( 0) = RHO_0_CV( CCVAR(I,J,K+1,IBM_UNKZ)-UNKZ_IND(NM_START) )
         FVCC = (CCM1*RHO0V(-1) + CCP1*RHO0V(0)) / (CCM1*RHOV(-1) + CCP1*RHOV(0))
...
! Define averaged FV for linked faces and redisribute in FVX, FVY, FVZ, CUT_FACE(ICF)%FN(:), CUT_FACE(ICF)%FV
...
DO K=0,KBAR
   DO J=0,JBAR
      DO I=0,IBAR
         ! X face:
         IF (FCVAR(I,J,K,IBM_UNKF,IAXIS)>0) THEN
            FLINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = FLINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + FVXP(I,J,K)*DY(J)*DZ(K)
            ALINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) = ALINK(FCVAR(I,J,K,IBM_UNKF,IAXIS)) + DY(J)*DZ(K)
         ENDIF
         ! Y face:
         IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0) THEN
            FLINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = FLINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + FVYP(I,J,K)*DX(I)*DZ(K)
            ALINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) = ALINK(FCVAR(I,J,K,IBM_UNKF,JAXIS)) + DX(I)*DZ(K)
         ENDIF
         ! Z face:
         IF (FCVAR(I,J,K,IBM_UNKF,KAXIS)>0) THEN
            FLINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = FLINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + FVZP(I,J,K)*DX(I)*DY(J)
            ALINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) = ALINK(FCVAR(I,J,K,IBM_UNKF,KAXIS)) + DX(I)*DY(J)
         ENDIF
      ENDDO
   ENDDO
ENDDO
...
DO K=0,KBAR
   DO J=0,JBAR
      DO I=0,IBAR
         IF (FCVAR(I,J,K,IBM_UNKF,IAXIS)>0)                  FVXP(I,J,K) = FLINK(FCVAR(I,J,K,IBM_UNKF,IAXIS))
         IF (FCVAR(I,J,K,IBM_UNKF,JAXIS)>0 .AND. .NOT.TWO_D) FVYP(I,J,K) = FLINK(FCVAR(I,J,K,IBM_UNKF,JAXIS))
         IF (FCVAR(I,J,K,IBM_UNKF,KAXIS)>0)                  FVZP(I,J,K) = FLINK(FCVAR(I,J,K,IBM_UNKF,KAXIS))
      ENDDO
   ENDDO
ENDDO
DO ICF=1,MESHES(NM)%IBM_NRCFACE_Z
   ...
   IF (CC_STM_TYPE==0 .OR. CC_STM_TYPE==10) THEN
      IF (PRESSURE_ITERATIONS==1) THEN
         CUTFACE_LOOP_2A : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
            IF ( CUT_FACE(ICF)%STATUS /= IBM_GASPHASE) CYCLE CUTFACE_LOOP_2A
            I      = CUT_FACE(ICF)%IJK(IAXIS)
            J      = CUT_FACE(ICF)%IJK(JAXIS)
            K      = CUT_FACE(ICF)%IJK(KAXIS)
            X1AXIS = CUT_FACE(ICF)%IJK(KAXIS+1)
            SELECT CASE(X1AXIS)
                  CASE(IAXIS)
                     IF (CUT_FACE(ICF)%ALPHA_CF > A_THRESH_FORCING) CYCLE CUTFACE_LOOP_2A
                     IDX = RDXN(I)
                     DO JCF=1,CUT_FACE(ICF)%NFACE
                        IF (PREDICTOR) DUUDT = (U_IBM-CUT_FACE(ICF)%VEL(JCF))/DT
                        IF (CORRECTOR) DUUDT = (2._EB*U_IBM-(CUT_FACE(ICF)%VEL(JCF)+CUT_FACE(ICF)%VELS(JCF)))/DT
                        IF (.NOT.GRADH_ON_CARTESIAN) IDX=1._EB/(CUT_FACE(ICF)%XCENHIGH(X1AXIS,JCF) - &
                                                                CUT_FACE(ICF)%XCENLOW( X1AXIS,JCF))
                        CUT_FACE(ICF)%FN(JCF)   = -IDX*(HP(I+1,J,K)-HP(I,J,K)) - DUUDT
                        CUT_FACE(ICF)%FN_B(JCF) = 0._EB
                     ENDDO
                  CASE(JAXIS)
                     IF (CUT_FACE(ICF)%ALPHA_CF > A_THRESH_FORCING) CYCLE CUTFACE_LOOP_2A
                     IDX = RDYN(J)
                     DO JCF=1,CUT_FACE(ICF)%NFACE
                        IF (PREDICTOR) DVVDT = (V_IBM-CUT_FACE(ICF)%VEL(JCF))/DT
                        IF (CORRECTOR) DVVDT = (2._EB*V_IBM-(CUT_FACE(ICF)%VEL(JCF)+CUT_FACE(ICF)%VELS(JCF)))/DT
                        IF (.NOT.GRADH_ON_CARTESIAN) IDX=1._EB/(CUT_FACE(ICF)%XCENHIGH(X1AXIS,JCF) - &
                                                                CUT_FACE(ICF)%XCENLOW( X1AXIS,JCF))
                        CUT_FACE(ICF)%FN(JCF)   = -IDX*(HP(I,J+1,K)-HP(I,J,K)) - DVVDT
                        CUT_FACE(ICF)%FN_B(JCF) = 0._EB
                     ENDDO
                  CASE(KAXIS)
                     IF (CUT_FACE(ICF)%ALPHA_CF > A_THRESH_FORCING) CYCLE CUTFACE_LOOP_2A
                     IDX = RDZN(K)
                     DO JCF=1,CUT_FACE(ICF)%NFACE
                        IF (PREDICTOR) DWWDT = (W_IBM-CUT_FACE(ICF)%VEL(JCF))/DT
                        IF (CORRECTOR) DWWDT = (2._EB*W_IBM-(CUT_FACE(ICF)%VEL(JCF)+CUT_FACE(ICF)%VELS(JCF)))/DT
                        IF (.NOT.GRADH_ON_CARTESIAN) IDX=1._EB/(CUT_FACE(ICF)%XCENHIGH(X1AXIS,JCF) - &
                                                                CUT_FACE(ICF)%XCENLOW( X1AXIS,JCF))
                        CUT_FACE(ICF)%FN(JCF)   = -IDX*(HP(I,J,K+1)-HP(I,J,K)) - DWWDT
                        CUT_FACE(ICF)%FN_B(JCF) = 0._EB
                     ENDDO
               END SELECT
...
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE(BOUNDARY_ONE_D_TYPE), POINTER :: ONE_D
TYPE(IBM_CUTFACE_TYPE), POINTER :: CF,CF2
...
CHECK_WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   IF (PREDICTOR) THEN
      ...
      CASE(IAXIS)
         IDX = RDXN(I)
         DO JCF=1,CF%NFACE
            IF(.NOT.GRADH_ON_CARTESIAN) IDX = 1._EB/(CF%XCENHIGH(X1AXIS,JCF)-CF%XCENLOW(X1AXIS,JCF))
         ...
         CASE( 1)
            DO KKO=KKO1,KKO2
               DO JJO=JJO1,JJO2
                  DO IIO=IIO1,IIO2
                     IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                     IF(M2%FCVAR(IIO,JJO,KKO,IBM_FGSC,X1AXIS)==IBM_SOLID) CYCLE

                     ICF = M2%FCVAR(IIO,JJO,KKO,IBM_IDCF,X1AXIS)
                     IF (ICF>0) THEN
                        CF2 =>M2%CUT_FACE(ICF); IDX = M2%RDXN(IIO)
                        DO JCF=1,CF2%NFACE
                              DA_OTHER = DA_OTHER  + CF2%AREA(JCF)
                              IF (.NOT.GRADH_ON_CARTESIAN) &
         ...
         CASE(-1)
            DO KKO=KKO1,KKO2
               DO JJO=JJO1,JJO2
                  DO IIO=IIO1,IIO2
                     IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                     IF(M2%FCVAR(IIO-1,JJO,KKO,IBM_FGSC,X1AXIS)==IBM_SOLID) CYCLE

                     ICF = M2%FCVAR(IIO-1,JJO,KKO,IBM_IDCF,X1AXIS)
                     IF (ICF>0) THEN
         ...
         CASE(-2)
            DO KKO=KKO1,KKO2
               DO JJO=JJO1,JJO2
                  DO IIO=IIO1,IIO2
                     IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                     IF(M2%FCVAR(IIO,JJO-1,KKO,IBM_FGSC,X1AXIS)==IBM_SOLID) CYCLE

                     ICF = M2%FCVAR(IIO,JJO-1,KKO,IBM_IDCF,X1AXIS)
                     IF (ICF>0) THEN
         ...
         CASE(-3)
            DO KKO=KKO1,KKO2
               DO JJO=JJO1,JJO2
                  DO IIO=IIO1,IIO2
                     IF(M2%SOLID(M2%CELL_INDEX(IIO,JJO,KKO))) CYCLE
                     IF(M2%FCVAR(IIO,JJO,KKO-1,IBM_FGSC,X1AXIS)==IBM_SOLID) CYCLE

                     ICF = M2%FCVAR(IIO,JJO,KKO-1,IBM_IDCF,X1AXIS)
                     IF (ICF>0) THEN
...
INTEGER :: I,J,K,NCELL,ICC,JCC,IJK(MAX_DIM),ICF,ICF1,IFACE
...
MESHES_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   ! Initialize IBM_IDRC to IBM_FGSC, this is to discard from the onset faces that can not be used in interpolation
   ! stencils (i.e. not fluid points). Faces allowed for interpolation stencils must be type IBM_GASPHASE:
   FCVAR(:,:,:,IBM_IDRC,IAXIS:KAXIS) = FCVAR(:,:,:,IBM_FGSC,IAXIS:KAXIS)
   ...
   CUT_FACE_LOOP : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      ...
      CUTFACE_STATUS_IF : IF(CUT_FACE(ICF)%STATUS == IBM_INBOUNDARY) THEN ! CUTFACE_STATUS_IF
...
IF(VIND > 0) THEN
   IF (MASK_FLG) THEN
      DO KK = KLO,KHI
         DO JJ = JLO,JHI
            DO II = ILO,IHI
               IF(FCVAR(II,JJ,KK,IBM_IDRC,VIND) /= IBM_GASPHASE) CYCLE ! Cycle if facevar is masked by IBM_IDRC.
               NPE_LIST_COUNT = NPE_LIST_COUNT + 1
...
CALL GET_RCEDGE_FACE_LIST ! This routine makes use of RCFACES index defined in ECVAR(:,:,:,IBM_IDRC,:) in the previous.
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO K=0,M%KBAR
      DO J=0,M%JBAR
         DO I=1,M%IBAR
            IRC = M%ECVAR(I,J,K,IBM_IDCE,EAXIS)
            IF (M%ECVAR(I,J,K,IBM_EGSC,EAXIS)/=IBM_GASPHASE .OR. IRC<1) CYCLE
            ALLOCATE(M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,-2:2)); M%IBM_RCEDGE(IRC)%FACE_LIST = IBM_UNDEFINED
            ! Faces -1 and 1 : KAXIS, I,J,K and KAXIS I,J+1,K
            X1AXIS = KAXIS; LOHI_AXIS = JAXIS
            DO ISIDE=0,1
               IF(M%FCVAR(I,J+ISIDE,K,IBM_IDCF,X1AXIS)>0) THEN ! CUT_FACE
                  ICF = M%FCVAR(I,J+ISIDE,K,IBM_IDCF,X1AXIS)
                  ...
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,2*ISIDE-1) = (/ IBM_FTYPE_CFGAS, ICF, JCF /)
               ELSEIF (M%FCVAR(I,J+ISIDE,K,IBM_IDRC,X1AXIS)>0) THEN ! RCFACE
                  ICF = M%FCVAR(I,J+ISIDE,K,IBM_IDRC,X1AXIS)
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,2*ISIDE-1) = (/ IBM_FTYPE_RCGAS, ICF,   0 /)
               ELSE ! Regular gas face. There can be no solid faces adjacent to an RCEDGE.
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,2*ISIDE-1) = (/ IBM_FTYPE_RGGAS,   0,   0 /)
               ENDIF
            ENDDO
            ! Faces -2 and 2 : JAXIS, I,J,K and JAXIS I,J,K+1
            X1AXIS = JAXIS; LOHI_AXIS = KAXIS
            DO ISIDE=0,1
               IF(M%FCVAR(I,J,K+ISIDE,IBM_IDCF,X1AXIS)>0) THEN ! CUT_FACE
                  ICF = M%FCVAR(I,J,K+ISIDE,IBM_IDCF,X1AXIS)
                  ! Find which cut-face in ICF entry has the RCEDGE(IRC) as boundary:
                  HILO=2-ISIDE ! 2=HIGH_IND side reg edge for face; 1=LOW_IND reg side edge.
                  CALL GET_RCEDGE_CUTFACE(NM,LOHI_AXIS,HILO,ICF,JCF)
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,4*ISIDE-2) = (/ IBM_FTYPE_CFGAS, ICF, JCF /)
               ELSEIF (M%FCVAR(I,J,K+ISIDE,IBM_IDRC,X1AXIS)>0) THEN ! RCFACE
                  ICF = M%FCVAR(I,J,K+ISIDE,IBM_IDRC,X1AXIS)
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,4*ISIDE-2) = (/ IBM_FTYPE_RCGAS, ICF,   0 /)
               ELSE ! Regular gas face. There can be no solid faces adjacent to an RCEDGE.
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,4*ISIDE-2) = (/ IBM_FTYPE_RGGAS,   0,   0 /)
               ENDIF
            ENDDO
         ENDDO
      ENDDO
   ...
   DO K=0,M%KBAR
      DO J=1,M%JBAR
         DO I=0,M%IBAR
            ...
            DO ISIDE=0,1
               IF(M%FCVAR(I+ISIDE,J,K,IBM_IDCF,X1AXIS)>0) THEN ! CUT_FACE
                  ICF = M%FCVAR(I+ISIDE,J,K,IBM_IDCF,X1AXIS)
                  ! Find which cut-face in ICF entry has the RCEDGE(IRC) as boundary:
                  HILO=2-ISIDE ! 2=HIGH_IND side reg edge for face; 1=LOW_IND reg side edge.
                  CALL GET_RCEDGE_CUTFACE(NM,LOHI_AXIS,HILO,ICF,JCF)
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,4*ISIDE-2) = (/ IBM_FTYPE_CFGAS, ICF, JCF /)
               ELSEIF (M%FCVAR(I+ISIDE,J,K,IBM_IDRC,X1AXIS)>0) THEN ! RCFACE
                  ICF = M%FCVAR(I+ISIDE,J,K,IBM_IDRC,X1AXIS)
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,4*ISIDE-2) = (/ IBM_FTYPE_RCGAS, ICF,   0 /)
               ELSE ! Regular gas face. There can be no solid faces adjacent to an RCEDGE.
                  M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,4*ISIDE-2) = (/ IBM_FTYPE_RGGAS,   0,   0 /)
               ENDIF
   ...
   ! DO IRC=1,M%IBM_NRCEDGE_Z
   !    WRITE(LU_ERR,*) ' '
   !    WRITE(LU_ERR,*) 'IRC=',IRC,M%IBM_RCEDGE(IRC)%IJK(1:4)
   !    WRITE(LU_ERR,*) '-2 =',M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,-2)
   !    WRITE(LU_ERR,*) '-1 =',M%IBM_RCEDGE(IRC)%FACE_LIST(1:3,-1)
   !    WRITE(LU_ERR,*) ' 1 =',M%IBM_RCEDGE(IRC)%FACE_LIST(1:3, 1)
   !    WRITE(LU_ERR,*) ' 2 =',M%IBM_RCEDGE(IRC)%FACE_LIST(1:3, 2)
   ! ENDDO
...
DO JCF=1,CF%NFACE
   DO IED=2,CF%CEDGES(1,JCF)+1
      ...
      IF(CF%EDGE_LIST(1,IEDGE)/=IBM_ETYPE_RGGAS) CYCLE  ! Regular gas edge, RCEDGES defined like this in
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   FCVAR(:,:,:,IBM_IDRC,:) = 0
   DO ICF=1,MESHES(NM)%IBM_NRCFACE_Z
      ! Add ICF position in FCVAR(I,J,K,IBM_IDRC,X1AXIS):
      I      = RC_FACE(ICF)%IJK(IAXIS)
      J      = RC_FACE(ICF)%IJK(JAXIS)
      K      = RC_FACE(ICF)%IJK(KAXIS)
      X1AXIS = RC_FACE(ICF)%IJK(KAXIS+1)
      FCVAR(I,J,K,IBM_IDRC,X1AXIS) = ICF
      ! Then set if SHARED:
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   DO NOM=1,NMESHES
      ...
      IF (MESHES(NM)%OMESH(NOM)%NICC_R(1)>0) THEN
         ...
         EXTERNAL_WALL_LOOP_1A : DO IW=1,N_EXTERNAL_WALL_CELLS
            ...
            IF ( WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY ) THEN
               ...
               IF (CCVAR(BC%II,BC%JJ,BC%KK,IBM_CGSC) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1A
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                       ICC   = MESHES(NOM)%CCVAR(IIO,JJO,KKO,IBM_IDCC)
            ...
            ELSEIF ( WC%BOUNDARY_TYPE==MIRROR_BOUNDARY ) THEN
               ...
               SELECT CASE(IOR)
               CASE( IAXIS); IF(FCVAR(IIO-1,JJO  ,KKO  ,IBM_FGSC,IAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_1A
               CASE(-IAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,IAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_1A
               CASE( JAXIS); IF(FCVAR(IIO  ,JJO-1,KKO  ,IBM_FGSC,JAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_1A
               CASE(-JAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,JAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_1A
               CASE( KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO-1,IBM_FGSC,KAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_1A
               CASE(-KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,KAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_1A
               END SELECT
               ICC = MESHES(NOM)%CCVAR(IIO,JJO,KKO,IBM_IDCC); IF (ICC<1) CYCLE
               NICC_R = NICC_R + 1
      ...
      DO IW=1,N_EXTERNAL_WALL_CELLS
         ...
         SELECT CASE(BC%IOR)
         CASE( IAXIS); IF(FCVAR(IIO-1,JJO  ,KKO  ,IBM_FGSC,IAXIS) /= IBM_CUTCFE) CYCLE
         CASE(-IAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,IAXIS) /= IBM_CUTCFE) CYCLE
         CASE( JAXIS); IF(FCVAR(IIO  ,JJO-1,KKO  ,IBM_FGSC,JAXIS) /= IBM_CUTCFE) CYCLE
         CASE(-JAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,JAXIS) /= IBM_CUTCFE) CYCLE
         CASE( KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO-1,IBM_FGSC,KAXIS) /= IBM_CUTCFE) CYCLE
         CASE(-KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,KAXIS) /= IBM_CUTCFE) CYCLE
         END SELECT
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                SELECT CASE(-BC%IOR)
                CASE( IAXIS); ICF=MESHES(NOM)%FCVAR(IIO-1,JJO  ,KKO  ,IBM_IDCF,IAXIS)
                CASE(-IAXIS); ICF=MESHES(NOM)%FCVAR(IIO  ,JJO  ,KKO  ,IBM_IDCF,IAXIS)
                CASE( JAXIS); ICF=MESHES(NOM)%FCVAR(IIO  ,JJO-1,KKO  ,IBM_IDCF,JAXIS)
                CASE(-JAXIS); ICF=MESHES(NOM)%FCVAR(IIO  ,JJO  ,KKO  ,IBM_IDCF,JAXIS)
                CASE( KAXIS); ICF=MESHES(NOM)%FCVAR(IIO  ,JJO  ,KKO-1,IBM_IDCF,KAXIS)
                CASE(-KAXIS); ICF=MESHES(NOM)%FCVAR(IIO  ,JJO  ,KKO  ,IBM_IDCF,KAXIS)
                END SELECT
                IF (ICF > 0) THEN
      ...
      IF (MESHES(NM)%OMESH(NOM)%NICF_R(1)>0) THEN
         M3 => MESHES(NM)%OMESH(NOM)
         ALLOCATE(M3%ICF_UFFB_CF_R(M3%NICF_R(1)))
         ! Dump cut-cell indexes on sending mesh NOM, whose info will be received:
         NICF_R = 0
         CALL POINT_TO_MESH(NM)
         ! Loop over cut-cells:
         EXTERNAL_WALL_LOOP_1B : DO IW=1,N_EXTERNAL_WALL_CELLS
            ...
            SELECT CASE(BC%IOR)
            CASE( IAXIS); IF(FCVAR(IIO-1,JJO  ,KKO  ,IBM_FGSC,IAXIS) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1B
            CASE(-IAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,IAXIS) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1B
            CASE( JAXIS); IF(FCVAR(IIO  ,JJO-1,KKO  ,IBM_FGSC,JAXIS) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1B
            CASE(-JAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,JAXIS) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1B
            CASE( KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO-1,IBM_FGSC,KAXIS) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1B
            CASE(-KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,KAXIS) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_1B
            END SELECT
            DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
               DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                  DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                   ...
                   IF (ICF > 0) THEN
                      NICF_R = NICF_R + 1
                      M3%ICF_UFFB_CF_R(NICF_R) = ICF ! Note : This ICF index refers to NOM mesh.
...
DO NM=1,NMESHES
   DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      IF (MESHES(NOM)%OMESH(NM)%NICF_S(1)>0) THEN
         M2 => MESHES(NOM)%OMESH(NM)
         ALLOCATE(M2%ICF_UFFB_CF_S(M2%NICC_S(1)))
      ENDIF
   ENDDO
ENDDO
N_REQ0 = 0
DO NM=1,NMESHES
   DO NOM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      IF (N_MPI_PROCESSES>1 .AND. NM/=NOM .AND. PROCESS(NM)/=MY_RANK .AND. M2%NICF_S(1)>0) THEN
         N_REQ0 = N_REQ0 + 1
         CALL MPI_IRECV(M2%ICF_UFFB_CF_S(1),M2%NICF_S(1),MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
      ENDIF
   ENDDO
ENDDO
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   DO NOM=1,NMESHES
      ...
      IF (PROCESS(NOM)/=MY_RANK) THEN
         N_REQ0 = N_REQ0 + 1
         CALL MPI_ISEND(M3%ICF_UFFB_CF_R(1),M3%NICF_R(1),MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,REQ0(N_REQ0),IERR)
      ELSE
         M2 => MESHES(NOM)%OMESH(NM)
         M2%ICF_UFFB_CF_S(1:M2%NICF_S(1)) = M3%ICF_UFFB_CF_R(1:M3%NICF_R(1))
      ENDIF
   ENDDO
ENDDO
IF ((N_REQ0>0) .AND. (N_MPI_PROCESSES>1)) CALL MPI_WAITALL(N_REQ0,REQ0(1:N_REQ0),MPI_STATUSES_IGNORE,IERR)
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   EXTERNAL_WALL_LOOP_2 : DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      ELSEIF ( WC%BOUNDARY_TYPE==MIRROR_BOUNDARY ) THEN
         ...
         ! CYCLE if OBJECT face is in the Mirror Boundary, normal out into ghost-cell:
         SELECT CASE(BC%IOR)
         CASE( IAXIS); IF(FCVAR(IIO-1,JJO  ,KKO  ,IBM_FGSC,IAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_2
         CASE(-IAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,IAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_2
         CASE( JAXIS); IF(FCVAR(IIO  ,JJO-1,KKO  ,IBM_FGSC,JAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_2
         CASE(-JAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,JAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_2
         CASE( KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO-1,IBM_FGSC,KAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_2
         CASE(-KAXIS); IF(FCVAR(IIO  ,JJO  ,KKO  ,IBM_FGSC,KAXIS) == IBM_SOLID) CYCLE EXTERNAL_WALL_LOOP_2
         END SELECT
         NOM = NM; ICC = MESHES(NOM)%CCVAR(IIO,JJO,KKO,IBM_IDCC)
         IF (ICC > 0) THEN
            DO JCC=1,MESHES(NOM)%CUT_CELL(ICC)%NCELL
               NCC_SV(NOM)=NCC_SV(NOM)+1
               MESHES(NOM)%CUT_CELL(ICC)%UNKZ(JCC) = M%OMESH(NOM)%UNKZ_CC_R(NCC_SV(NOM))
...
MAIN_MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   WALL_CUTFACE_LOOP :  DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (FCVAR(IIF,JJF,KKF,IBM_FGSC,X1AXIS) /= IBM_CUTCFE) CYCLE WALL_CUTFACE_LOOP
      ...
      IF(SOLID(CELL_INDEX(II,JJ,KK))) CYCLE WALL_CUTFACE_LOOP
      ICC = MESHES(NM)%CCVAR(II,JJ,KK,IBM_IDCC)
      DO JCC=1,CUT_CELL(ICC)%NCELL
         ! Loop faces and test:
         DO IFC=1,CUT_CELL(ICC)%CCELEM(1,JCC)
...
DO ICC=1,M%N_CUTCELL_MESH
   ...
   DO JCC=1,CC%NCELL
      ...
      DO IFC=1,CC%CCELEM(1,JCC)
         IFACE = CC%CCELEM(IFC+1,JCC)
         ! If face type in face_list is not IBM_FTYPE_RCGAS, drop:
         IF(CC%FACE_LIST(1,IFACE) /= IBM_FTYPE_RCGAS) CYCLE
         ! Which face?
         LOWHIGH = CC%FACE_LIST(2,IFACE)
         X1AXIS  = CC%FACE_LIST(3,IFACE)
         SELECT CASE(X1AXIS)
         ...
         IF (LOWHIGH == LOW_IND) THEN
            ...
            IF (PRES_FLAG==UGLMAT_FLAG) CELL_UNKH = M%CCVAR(INCELL,JNCELL,KNCELL,IBM_UNKH)
            IF (PRES_FLAG== ULMAT_FLAG) CELL_UNKH = M%MUNKH(INCELL,JNCELL,KNCELL)
            IF ( CELL_UNKH > 0 ) THEN
               IJKFACE(INFACE,JNFACE,KNFACE,X1AXIS) = 1
            ELSEIF ( M%CCVAR(INCELL,JNCELL,KNCELL,IBM_CGSC) == IBM_CUTCFE ) THEN ! Cut-cell.
               IJKFACE(INFACE,JNFACE,KNFACE,X1AXIS) = 1
            ENDIF
         ELSE ! HIGH_IND
            ! Face indexes:
...
GRD_CC_LOOP_1 :  DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   END SELECT
   IF(ALL(CCVAR(IIF+IADD:IIF,JJF+JADD:JJF,KKF+KADD:KKF,IBM_CGSC)==IBM_SOLID)) CYCLE GRD_CC_LOOP_1
   IF( PRES_FLAG==UGLMAT_FLAG .AND. &
      ...
      IF (M%FCVAR(IIF,JJF,KKF,IBM_FGSC,X1AXIS) /= IBM_GASPHASE) CYCLE GRD_CC_LOOP_1
      ! Is this an actual RCF_H laying on the mesh boundary, where the cut-cell is in the guard-cell region?
      IF(.NOT.((M%CCVAR(II,JJ,KK,IBM_CGSC)==IBM_CUTCFE).AND.(M%CCVAR(BC%IIG,BC%JJG,BC%KKG,IBM_CGSC)==IBM_GASPHASE))) &
      CYCLE GRD_CC_LOOP_1
...
M%IBM_NRCFACE_H = IRC ! Same number of regular - cut cell faces as scalars.
ALLOCATE( M%RCF_H(IRC) )
IRC = 0
DO ICC=1,M%N_CUTCELL_MESH
   CC => M%CUT_CELL(ICC); IJK(IAXIS:KAXIS) = CC%IJK(IAXIS:KAXIS)
   DO JCC=1,CC%NCELL
      ...
      DO IFC=1,CC%CCELEM(1,JCC)
         ...
         IF_LOW_HIGH_H : IF (LOWHIGH == LOW_IND) THEN
            ...
            IF ( CELL_UNKH > 0 ) THEN
               ...
               IRC = IRC + 1
               M%RCF_H(IRC) = FCVAR(INFACE,JNFACE,KNFACE,IBM_IDRC,X1AXIS)
...
IF (PRES_FLAG==UGLMAT_FLAG) THEN
   GRD_CC_LOOP_2 :  DO IW=1,M%N_EXTERNAL_WALL_CELLS
      ...
      ! Drop if FACE is not type IBM_GASPHASE
      IF (M%FCVAR(IIF,JJF,KKF,IBM_FGSC,X1AXIS) /= IBM_GASPHASE) CYCLE GRD_CC_LOOP_2

      ! Is this an actual RCF_H laying on the mesh boundary, where the cut-cell is in the guard-cell region?
      IF(.NOT.((M%CCVAR(II,JJ,KK,IBM_CGSC)==IBM_CUTCFE).AND.(M%CCVAR(BC%IIG,BC%JJG,BC%KKG,IBM_UNKH)>0))) &
      CYCLE GRD_CC_LOOP_2

      CC => M%CUT_CELL(M%CCVAR(II,JJ,KK,IBM_IDCC))
      DO JCC=1,CC%NCELL
         ! Loop faces and test:
         DO IFC=1,CC%CCELEM(1,JCC)
            IFACE = CC%CCELEM(IFC+1,JCC)
            ! Which face ?
            LOWHIGH = CC%FACE_LIST(2,IFACE)
            IF ( CC%FACE_LIST(1,IFACE) /= IBM_FTYPE_RCGAS) CYCLE  ! Must Be gasphase RCFACE
            IF ( LOWHIGH                              /= LOWHIGH_TEST) CYCLE ! In same side as EWC from guard-cell
            ...
            IF (LOWHIGH == LOW_IND) THEN ! Face on low side of guard cut-cell
               IRC = IRC + 1
               M%RCF_H(IRC) = M%FCVAR(IIF,JJF,KKF,IBM_IDRC,X1AXIS)
               ! Cell at i-1, i.e. regular GASPHASE:
               M%RC_FACE(M%RCF_H(IRC))%UNKH(LOW_IND)  = M%CCVAR(BC%IIG,BC%JJG,BC%KKG,IBM_UNKH)
               ! Cell at i+1, i.e. cut-cell:
               M%RC_FACE(M%RCF_H(IRC))%UNKH(HIGH_IND) = CC%UNKH(JCC)
            ...
            ELSEIF(LOWHIGH == HIGH_IND) THEN ! Face on high side of guard cut-cell
               ...
               M%RC_FACE(M%RCF_H(IRC))%UNKH(LOW_IND)  = CC%UNKH(JCC)
               ! Cell at i+1, i.e. regular GASPHASE:
               M%RC_FACE(M%RCF_H(IRC))%UNKH(HIGH_IND) = M%CCVAR(BC%IIG,BC%JJG,BC%KKG,IBM_UNKH)
...
MAIN_MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   M%IBM_NREGFACE_Z(IAXIS:KAXIS) = 0
   ...
   IBNDINT_LOOP_X : DO IBNDINT=1,3
      IF(IBNDINT==3) M%IBM_NBBREGFACE_Z(X1AXIS)=IREG ! REG faces in block boundaries.
      SELECT CASE(IBNDINT)
      CASE(1)
   ...
   M%IBM_NREGFACE_Z(X1AXIS) = IREG
   IF(ALLOCATED(M%IBM_REGFACE_IAXIS_Z)) DEALLOCATE(M%IBM_REGFACE_IAXIS_Z)
   ALLOCATE(M%IBM_REGFACE_IAXIS_Z(IREG))
   DO II=1,IREG
      M%IBM_REGFACE_IAXIS_Z(II)%IJK(IAXIS:KAXIS) = IJKBUFFER(IAXIS:KAXIS,II)
      M%IBM_REGFACE_IAXIS_Z(II)%IWC              = IJKBUFFER(KAXIS+1,II)
      M%IBM_REGFACE_IAXIS_Z(II)%DO_LO_IND        = LOHIBUFF(LOW_IND,II)
      M%IBM_REGFACE_IAXIS_Z(II)%DO_HI_IND        = LOHIBUFF(HIGH_IND,II)
   ENDDO
   ...
   M%IBM_NREGFACE_Z(X1AXIS) = IREG
   IF(ALLOCATED(M%IBM_REGFACE_JAXIS_Z)) DEALLOCATE(M%IBM_REGFACE_JAXIS_Z)
   ALLOCATE(M%IBM_REGFACE_JAXIS_Z(IREG))
   DO II=1,IREG
      M%IBM_REGFACE_JAXIS_Z(II)%IJK(IAXIS:KAXIS) = IJKBUFFER(IAXIS:KAXIS,II)
      M%IBM_REGFACE_JAXIS_Z(II)%IWC              = IJKBUFFER(KAXIS+1,II)
      M%IBM_REGFACE_JAXIS_Z(II)%DO_LO_IND        = LOHIBUFF(LOW_IND,II)
      M%IBM_REGFACE_JAXIS_Z(II)%DO_HI_IND        = LOHIBUFF(HIGH_IND,II)
   ENDDO
   ...
   M%IBM_NREGFACE_Z(X1AXIS) = IREG
   IF(ALLOCATED(M%IBM_REGFACE_KAXIS_Z)) DEALLOCATE(M%IBM_REGFACE_KAXIS_Z)
   ALLOCATE(M%IBM_REGFACE_KAXIS_Z(IREG))
   DO II=1,IREG
      M%IBM_REGFACE_KAXIS_Z(II)%IJK(IAXIS:KAXIS) = IJKBUFFER(IAXIS:KAXIS,II)
      M%IBM_REGFACE_KAXIS_Z(II)%IWC              = IJKBUFFER(KAXIS+1,II)
      M%IBM_REGFACE_KAXIS_Z(II)%DO_LO_IND        = LOHIBUFF(LOW_IND,II)
      M%IBM_REGFACE_KAXIS_Z(II)%DO_HI_IND        = LOHIBUFF(HIGH_IND,II)
   ENDDO
   ...
   GUARD_CUT_CELL_LOOP_1A :  DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      DO JCC=1,CUT_CELL(ICC)%NCELL
         ...
         DO IFC=1,CUT_CELL(ICC)%CCELEM(1,JCC)
            ...
            ! Which face ?
            LOWHIGH = CUT_CELL(ICC)%FACE_LIST(2,IFACE)
            IF ( CUT_CELL(ICC)%FACE_LIST(1,IFACE) /= IBM_FTYPE_RCGAS) CYCLE ! Must Be gasphase cut-face
            IF ( LOWHIGH                             /= LOWHIGH_TEST) CYCLE ! In same side as EWC from guard-cell
            IF ( CUT_CELL(ICC)%FACE_LIST(3,IFACE) /= X1AXIS) CYCLE ! Normal to same axis as EWC
   ...
   GUARD_CUT_CELL_LOOP_1B :  DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF_LOW_HIGH_1B : IF (LOWHIGH == LOW_IND) THEN
         ...
         IF (CCVAR(INCELL,JNCELL,KNCELL,IBM_CGSC) == IBM_GASPHASE ) THEN ! next cell is reg-cell:
            ...
            ! Cell at i-1, i.e. regular GASPHASE:
            M%RC_FACE(IRC)%UNKZ(LOW_IND)                     = CCVAR(INCELL,JNCELL,KNCELL,IBM_UNKZ)
            M%RC_FACE(IRC)%XCEN(IAXIS:KAXIS,LOW_IND)         = (/ XCELL(INCELL), YCELL(JNCELL), ZCELL(KNCELL) /)
            M%RC_FACE(IRC)%CELL_LIST(IAXIS:KAXIS+1, LOW_IND) = (/ IBM_FTYPE_RGGAS, INCELL, JNCELL, KNCELL /)
            ...
            M%RC_FACE(IRC)%CELL_LIST(IAXIS:KAXIS+1,HIGH_IND) = (/ IBM_FTYPE_CFGAS, ICC, JCC, IFC /)
      ...
      ELSE ! IF_LOW_HIGH : HIGH_IND
         ...
         IF (CCVAR(INCELL,JNCELL,KNCELL,IBM_CGSC) == IBM_GASPHASE ) THEN
            ...
            M%RC_FACE(IRC)%CELL_LIST(IAXIS:KAXIS+1, LOW_IND) = (/ IBM_FTYPE_CFGAS, ICC, JCC, IFC /)
            ! Modify FACE_LIST for the given cut-cell:
            CUT_CELL(ICC)%FACE_LIST(4,IFACE) = IRC

            ! Cell at i+1, i.e. regular GASPHASE:
            M%RC_FACE(IRC)%UNKZ(HIGH_IND)                    = CCVAR(INCELL,JNCELL,KNCELL,IBM_UNKZ)
            M%RC_FACE(IRC)%XCEN(IAXIS:KAXIS,HIGH_IND)        = (/ XCELL(INCELL), YCELL(JNCELL), ZCELL(KNCELL) /)
            M%RC_FACE(IRC)%CELL_LIST(IAXIS:KAXIS+1,HIGH_IND) = (/ IBM_FTYPE_RGGAS, INCELL, JNCELL, KNCELL /)
   ...
   DO ICC=1,M%N_CUTCELL_MESH
      ...
      DO JCC=1,CC%NCELL
         ...
         IFC_LOOP : DO IFC=1,CC%CCELEM(1,JCC)
            IFACE = CC%CCELEM(IFC+1,JCC)
            ! If face type in face_list is not IBM_FTYPE_RCGAS, drop:
            IF(CC%FACE_LIST(1,IFACE) /= IBM_FTYPE_RCGAS) CYCLE IFC_LOOP
            ...
            IF_LOW_HIGH : IF (LOWHIGH == LOW_IND) THEN
               ...
               ELSEIF(CCVAR(INCELL,JNCELL,KNCELL,IBM_CGSC) == IBM_CUTCFE) THEN ! next cell is cc:
                  ...
                  IF (INLIST) THEN
                     ...
                     M%RC_FACE(IIFC)%CELL_LIST(IAXIS:KAXIS+1,HIGH_IND) = (/ IBM_FTYPE_CFGAS, ICC, JCC, IFC /)
                     ! Modify FACE_LIST for the given cut-cell:
            ...
            ELSE ! IF_LOW_HIGH : HIGH_IND
               ...
               ELSEIF(CCVAR(INCELL,JNCELL,KNCELL,IBM_CGSC) == IBM_CUTCFE) THEN ! next cell is cc:
                  ...
                  IF (INLIST) THEN
                     ...
                     M%RC_FACE(IIFC)%CELL_LIST(IAXIS:KAXIS+1, LOW_IND) = (/ IBM_FTYPE_CFGAS, ICC, JCC, IFC /)
                     ! Modify FACE_LIST for the given cut-cell:
                     CC%FACE_LIST(4,IFACE) = IIFC
                     CYCLE IFC_LOOP
                  ENDIF
   ...
   DO IRC=M%IBM_NBBRCFACE_Z+1,M%IBM_NRCFACE_Z
      RCF => M%RC_FACE(IRC);
      I = RCF%IJK(IAXIS); J = RCF%IJK(JAXIS); K = RCF%IJK(KAXIS); X1AXIS = RCF%IJK(KAXIS+1)
      ...
      CASE(IAXIS)
         ...
         ELSEIF(    SOLID(CELL_INDEX(I,J,K)) .AND. .NOT.SOLID(CELL_INDEX(I+1,J,K))) THEN
            IW = WALL_INDEX(CELL_INDEX(I+1,J,K),-X1AXIS) ! Low face of I+1 cell.
         ELSEIF(.NOT.SOLID(CELL_INDEX(I,J,K)) .AND.     SOLID(CELL_INDEX(I+1,J,K))) THEN
            IW = WALL_INDEX(CELL_INDEX(I  ,J,K), X1AXIS) ! High face of I cell.
         ENDIF
      CASE(JAXIS)
         ...
         ELSEIF(    SOLID(CELL_INDEX(I,J,K)) .AND. .NOT.SOLID(CELL_INDEX(I,J+1,K))) THEN
            IW = WALL_INDEX(CELL_INDEX(I,J+1,K),-X1AXIS) ! Low face of J+1 cell.
         ELSEIF(.NOT.SOLID(CELL_INDEX(I,J,K)) .AND.     SOLID(CELL_INDEX(I,J+1,K))) THEN
            IW = WALL_INDEX(CELL_INDEX(I,J  ,K), X1AXIS) ! High face of J cell.
         ENDIF
      CASE(KAXIS)
         ...
         ELSEIF(    SOLID(CELL_INDEX(I,J,K)) .AND. .NOT.SOLID(CELL_INDEX(I,J,K+1))) THEN
            IW = WALL_INDEX(CELL_INDEX(I,J,K+1),-X1AXIS) ! Low face of K+1 cell.
         ELSEIF(.NOT.SOLID(CELL_INDEX(I,J,K)) .AND.     SOLID(CELL_INDEX(I,J,K+1))) THEN
            IW = WALL_INDEX(CELL_INDEX(I,J,K  ), X1AXIS) ! High face of K cell.
         ENDIF
      END SELECT
      IF(IW>0) RCF%IWC=IW
   ENDDO
...
IF (DEBUG_MATVEC_DATA) THEN
   DBG_MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      WRITE(LU_ERR,*) 'IBM_NBBREGFACE(1:3) : ',M%IBM_NBBREGFACE_Z(IAXIS:KAXIS)
      WRITE(LU_ERR,*) 'IBM_NREGFACE(1:3)   : ',M%IBM_NREGFACE_Z(IAXIS:KAXIS)
      WRITE(LU_ERR,*) 'IBM_NRCFACE_Z, IBM_NBBRCFACE_Z : ', &
                       M%IBM_NRCFACE_Z,M%IBM_NBBRCFACE_Z
   ENDDO DBG_MESH_LOOP
ENDIF
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            ...
            ICC  = CCVAR(I,J,K,IBM_IDCC) ! The following test excludes GC cut-cells from numbering.
            IF (ICC <= MESHES(NM)%N_CUTCELL_MESH .AND. .NOT.SOLID(CELL_INDEX(I,J,K)) ) THEN
               DO JCC=1,CUT_CELL(ICC)%NCELL
            ...
            DO KNGH=K-1,K+1
               ...
               DO JNGH=J-1,J+1
                  ...
                  DO INGH=I-1,I+1
                     ...
                     IF ((CCVAR(INGH,JNGH,KNGH,IBM_CGSC)/=IBM_GASPHASE) .OR. (CCVAR(INGH,JNGH,KNGH,IBM_UNKZ)>0)) CYCLE
   ...
   LINK_LEV_DO : DO ILEV=-1,MESHES(NM)%FINEST_LINK_LEV,-1
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         DO JCC=1,CUT_CELL(ICC)%NCELL
            ...
            CASE(IBM_GASPHASE)
               CUT_CELL(ICC)%UNKZ(JCC) = CCVAR(I,J,K,IBM_UNKZ)
            CASE(IBM_CUTCFE)
               ICC2 = CCVAR(I,J,K,IBM_IDCC); JCC2 = CUT_CELL(ICC)%IJK_LINK(5,JCC)
...
REIND_MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF(CCVAR(I,J,K,IBM_UNKZ) > 0) CELLPUNKZ(CCVAR(I,J,K,IBM_UNKZ)) = CELLPUNKZ(CCVAR(I,J,K,IBM_UNKZ)) + 1
         ENDDO
      ENDDO
   ENDDO
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF(CCVAR(I,J,K,IBM_UNKZ) > 0) CCVAR(I,J,K,IBM_UNKZ) = INDUNKZ(CCVAR(I,J,K,IBM_UNKZ)) ! Condensed value.
         ENDDO
      ENDDO
   ENDDO
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (CCVAR(I,J,K,IBM_CGSC) /= IBM_GASPHASE .OR. CCVAR(I,J,K,IBM_UNKZ) <= 0 ) CYCLE
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      ...
      SELECT CASE(X1AXIS)
      CASE(IAXIS); IF ( SOLID(CELL_INDEX(I,J,K)) .AND. SOLID(CELL_INDEX(I+1,J,K)) ) CF_STATUS = IBM_SOLID
      CASE(JAXIS); IF ( SOLID(CELL_INDEX(I,J,K)) .AND. SOLID(CELL_INDEX(I,J+1,K)) ) CF_STATUS = IBM_SOLID
      CASE(KAXIS); IF ( SOLID(CELL_INDEX(I,J,K)) .AND. SOLID(CELL_INDEX(I,J,K+1)) ) CF_STATUS = IBM_SOLID
      END SELECT
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   FACE_LINK_IF : IF (NO_FACE_LINKING) THEN
      ...
      ICF_LOOP_0 : DO ICF=1,M%N_CUTFACE_MESH
         CF => M%CUT_FACE(ICF); IF(CF%STATUS/=IBM_GASPHASE) CYCLE ICF_LOOP_0
   ...
   ICF_LOOP_1 : DO ICF=1,M%N_CUTFACE_MESH
      CF => M%CUT_FACE(ICF); IF(CF%STATUS/=IBM_GASPHASE) CYCLE ICF_LOOP_1
   ...
   !    DO J=1,M%JBAR
   !       DO I=0,M%IBAR
   !          IF(M%FCVAR(I,J,K,IBM_UNKF,IAXIS)>0) WRITE(LU_ERR,*) 'CRT=',I,J,K,IAXIS,M%FCVAR(I,J,K,IBM_UNKF,X1AXIS)
   !       ENDDO
   !    ENDDO
   ! ENDDO
  ...
  DO K=0,M%KBAR
     DO J=0,M%JBAR
        DO I=0,M%IBAR
           IF (M%FCVAR(I,J,K,IBM_UNKF,IAXIS)>0) COUNT = COUNT+1
           IF (M%FCVAR(I,J,K,IBM_UNKF,JAXIS)>0 .AND. .NOT.TWO_D) COUNT = COUNT+1
           IF (M%FCVAR(I,J,K,IBM_UNKF,KAXIS)>0) COUNT = COUNT+1
        ENDDO
     ENDDO
  ENDDO
  DO ICF=1,M%IBM_NRCFACE_Z
...
ICF_LOOP_2 : DO ICF=1,M%N_CUTFACE_MESH
   CF => M%CUT_FACE(ICF); IF(CF%STATUS/=IBM_GASPHASE) CYCLE ICF_LOOP_2
   I  = CF%IJK(IAXIS); J = CF%IJK(JAXIS); K = CF%IJK(KAXIS); X1AXIS = CF%IJK(KAXIS+1)
   SELECT CASE(X1AXIS)
       ...
       IAXIS_JCF_LOOP : DO JCF=1,CF%NFACE
          IF(CF%UNKF(JCF)<1) THEN ! .AND. CF%AREA(JCF) < CCVOL_LINK*ACRT+TWO_EPSILON_EB) THEN
             ...
             IAXIS_IEC_LOOP : DO IEC=2,CF%CEDGES(1,JCF)+1
                ...
                CASE(IBM_ETYPE_RGGAS) IAXIS_IEC_SELECT ! Cut-faces Regular Gas Edge.
                   ...
                   IF (X2AXIS==JAXIS) THEN ! Edge pointed in X3AXIS=KAXIS direction.
                      ...
                      IERC   = M%ECVAR(II,JJ,KK,IBM_IDCE,X3AXIS); IF(IERC==0) CYCLE IAXIS_IEC_LOOP ! Index in IBM_RCEDGE
                      IF(LOHI==LOW_IND) THEN ! Indexes of other Cartesian face @ J-1
                         IIO=I; JJO=J-1; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3,-2) ! X Face in low J
                      ELSE ! Indexes of other Cartesian face @ J+1
                         IIO=I; JJO=J+1; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3, 2) ! X Face in high J
                      ENDIF
                   ...
                   ELSEIF(X2AXIS==KAXIS) THEN ! Edge pointed in X3AXIS=JAXIS direction.
                      ...
                      IERC   = M%ECVAR(II,JJ,KK,IBM_IDCE,X3AXIS); IF(IERC==0) CYCLE IAXIS_IEC_LOOP
                      IF(LOHI==LOW_IND) THEN ! Indexes of other Cartesian face @ K-1
                         IIO=I; JJO=J; KKO=K-1; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3,-1) ! Face in low K
                      ELSE ! Indexes of other Cartesian face @ K+1
                         IIO=I; JJO=J; KKO=K+1; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3, 1) ! Face in high K
                      ENDIF
                   ...
                   CASE(IBM_FTYPE_RGGAS) IAXIS_OFACE_SELECT
                      OLO_UNKZ = M%CCVAR(IIO  ,JJO  ,KKO  ,IBM_UNKZ)
                      OHI_UNKZ = M%CCVAR(IIO+1,JJO  ,KKO  ,IBM_UNKZ)
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to regular face:
                         IF(M%FCVAR(IIO,JJO,KKO,IBM_UNKF,X1AXIS)<1) THEN
                             M%NUNK_F = M%NUNK_F + 1
                             M%FCVAR(IIO,JJO,KKO,IBM_UNKF,X1AXIS) = M%NUNK_F
                         ENDIF
                         CF%UNKF(JCF)     = M%FCVAR(IIO,JJO,KKO,IBM_UNKF,X1AXIS)
                         CF%LINK_LEV(JCF) = -1
                   ...
                   CASE(IBM_FTYPE_RCGAS) IAXIS_OFACE_SELECT
                      ...
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to RC face:
                         ...
                         CYCLE IAXIS_JCF_LOOP
                      ENDIF
                   CASE(IBM_FTYPE_CFGAS) IAXIS_OFACE_SELECT
                ...
                CASE(IBM_ETYPE_CFGAS) IAXIS_IEC_SELECT ! Gas cut-edge.
       ...
       JAXIS_JCF_LOOP : DO JCF=1,CF%NFACE
          IF(CF%UNKF(JCF)<1) THEN ! .AND. CF%AREA(JCF) < CCVOL_LINK*ACRT+TWO_EPSILON_EB) THEN
             ...
             JAXIS_IEC_LOOP : DO IEC=2,CF%CEDGES(1,JCF)+1
                ...
                CASE(IBM_ETYPE_RGGAS) JAXIS_IEC_SELECT ! Cut-faces Regular Gas Edge.
                   LOHI   = CF%EDGE_LIST(2,IEDGE)
                   X2AXIS = CF%EDGE_LIST(3,IEDGE)
                   IF (X2AXIS==IAXIS) THEN ! Edge pointed in X3AXIS=KAXIS direction.
                      X3AXIS = KAXIS; II=I; JJ=J; KK=K; IF(LOHI==LOW_IND) II=I-1
                      IERC   = M%ECVAR(II,JJ,KK,IBM_IDCE,X3AXIS); IF(IERC==0) CYCLE JAXIS_IEC_LOOP ! Index in IBM_RCEDGE
                      IF(LOHI==LOW_IND) THEN ! Indexes of other Cartesian face @ I-1
                         IIO=I-1; JJO=J; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3,-1) ! Y Face in low I
                      ELSE ! Indexes of other Cartesian face @ I+1
                         IIO=I+1; JJO=J; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3, 1) ! Y Face in high I
                      ENDIF
                   ...
                   ELSEIF(X2AXIS==KAXIS) THEN ! Edge pointed in X3AXIS=IAXIS direction.
                      ...
                      IERC   = M%ECVAR(II,JJ,KK,IBM_IDCE,X3AXIS); IF(IERC==0) CYCLE JAXIS_IEC_LOOP
                      IF(LOHI==LOW_IND) THEN ! Indexes of other Cartesian face @ K-1
                         IIO=I; JJO=J; KKO=K-1; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3,-2) ! Y Face in low K
                      ELSE ! Indexes of other Cartesian face @ K+1
                         IIO=I; JJO=J; KKO=K+1; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3, 2) ! Y Face in high K
                      ENDIF
                      IF(KKO<1 .OR. KKO>M%KBAR) CYCLE JAXIS_IEC_LOOP
                   ENDIF
                   ! Now look for other potential face and link CF%UNKF(JCF):
                   JAXIS_OFACE_SELECT : SELECT CASE (OFACE(1))
                   CASE(IBM_FTYPE_RGGAS) JAXIS_OFACE_SELECT
                      OLO_UNKZ = M%CCVAR(IIO  ,JJO  ,KKO  ,IBM_UNKZ)
                      OHI_UNKZ = M%CCVAR(IIO  ,JJO+1,KKO  ,IBM_UNKZ)
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to regular face:
                         IF(M%FCVAR(IIO,JJO,KKO,IBM_UNKF,X1AXIS)<1) THEN
                             M%NUNK_F = M%NUNK_F + 1
                             M%FCVAR(IIO,JJO,KKO,IBM_UNKF,X1AXIS) = M%NUNK_F
                   ...
                   CASE(IBM_FTYPE_RCGAS) JAXIS_OFACE_SELECT
                      ...
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to RC face:
                         ...
                         CYCLE JAXIS_JCF_LOOP
                      ENDIF
                   CASE(IBM_FTYPE_CFGAS) JAXIS_OFACE_SELECT
                      OICF     = OFACE(2); OJCF     = OFACE(3)
                      OLO_UNKZ = M%CUT_FACE(OICF)%UNKZ(LOW_IND,OJCF)
                      OHI_UNKZ = M%CUT_FACE(OICF)%UNKZ(HIGH_IND,OJCF)
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to other cut-face:
                ...
                CASE(IBM_ETYPE_CFGAS) JAXIS_IEC_SELECT ! Gas cut-edge.
                   IECE = CF%EDGE_LIST(2,IEDGE);  JECE = CF%EDGE_LIST(3,IEDGE)
                   II=M%CUT_EDGE(IECE)%IJK(IAXIS); JJ=M%CUT_EDGE(IECE)%IJK(JAXIS); KK=M%CUT_EDGE(IECE)%IJK(KAXIS);
                   X3AXIS = M%CUT_EDGE(IECE)%IJK(KAXIS+1);
       ...
       KAXIS_JCF_LOOP : DO JCF=1,CF%NFACE
          IF(CF%UNKF(JCF)<1) THEN ! .AND. CF%AREA(JCF) < CCVOL_LINK*ACRT+TWO_EPSILON_EB) THEN
             ...
             KAXIS_IEC_LOOP : DO IEC=2,CF%CEDGES(1,JCF)+1
                ...
                CASE(IBM_ETYPE_RGGAS) KAXIS_IEC_SELECT ! Cut-faces Regular Gas Edge.
                   LOHI   = CF%EDGE_LIST(2,IEDGE)
                   X2AXIS = CF%EDGE_LIST(3,IEDGE)
                   IF (X2AXIS==IAXIS) THEN ! Edge pointed in X3AXIS=JAXIS direction.
                      X3AXIS = JAXIS; II=I; JJ=J; KK=K; IF(LOHI==LOW_IND) II=I-1
                      IERC   = M%ECVAR(II,JJ,KK,IBM_IDCE,X3AXIS); IF(IERC==0) CYCLE KAXIS_IEC_LOOP ! Index in IBM_RCEDGE
                      IF(IERC==0) CYCLE KAXIS_IEC_LOOP
                      !WRITE(LU_ERR,*) 'EDGE=',IERC,II,JJ,KK,X3AXIS,', CF=',I,J,K,X1AXIS
                      IF(LOHI==LOW_IND) THEN ! Indexes of other Cartesian face @ I-1
                         IIO=I-1; JJO=J; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3,-2) ! Z Face in low I
                      ELSE ! Indexes of other Cartesian face @ I+1
                         IIO=I+1; JJO=J; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3, 2) ! Z Face in high I
                      ENDIF
                   ...
                   ELSEIF(X2AXIS==JAXIS) THEN ! Edge pointed in X3AXIS=IAXIS direction.
                      ...
                      IERC   = M%ECVAR(II,JJ,KK,IBM_IDCE,X3AXIS); IF(IERC==0) CYCLE KAXIS_IEC_LOOP
                      IF(LOHI==LOW_IND) THEN ! Indexes of other Cartesian face @ J-1
                         IIO=I; JJO=J-1; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3,-1) ! Z Face in low J
                      ELSE ! Indexes of other Cartesian face @ J+1
                         IIO=I; JJO=J+1; KKO=K; OFACE(1:3) = M%IBM_RCEDGE(IERC)%FACE_LIST(1:3, 1) ! Z Face in high J
                      ENDIF
                      IF(JJO<1 .OR. JJO>M%JBAR) CYCLE KAXIS_IEC_LOOP
                   ENDIF
                   ! Now look for other potential face and link CF%UNKF(JCF):
                   KAXIS_OFACE_SELECT : SELECT CASE (OFACE(1))
                   CASE(IBM_FTYPE_RGGAS) KAXIS_OFACE_SELECT
                      OLO_UNKZ = M%CCVAR(IIO  ,JJO  ,KKO  ,IBM_UNKZ)
                      OHI_UNKZ = M%CCVAR(IIO  ,JJO  ,KKO+1,IBM_UNKZ)
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to regular face:
                         IF(M%FCVAR(IIO,JJO,KKO,IBM_UNKF,X1AXIS)<1) THEN
                             M%NUNK_F = M%NUNK_F + 1
                             M%FCVAR(IIO,JJO,KKO,IBM_UNKF,X1AXIS) = M%NUNK_F
                   ...
                   CASE(IBM_FTYPE_RCGAS) KAXIS_OFACE_SELECT
                      ...
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to RC face:
                         ...
                         CYCLE KAXIS_JCF_LOOP
                      ENDIF
                   CASE(IBM_FTYPE_CFGAS) KAXIS_OFACE_SELECT
                      OICF     = OFACE(2); OJCF     = OFACE(3)
                      OLO_UNKZ = M%CUT_FACE(OICF)%UNKZ(LOW_IND,OJCF)
                      OHI_UNKZ = M%CUT_FACE(OICF)%UNKZ(HIGH_IND,OJCF)
                      IF(LO_UNKZ==OLO_UNKZ .OR. HI_UNKZ==OHI_UNKZ .OR. ALL_FLG) THEN ! Link cut-face to other cut-face:
                ...
                CASE(IBM_ETYPE_CFGAS) KAXIS_IEC_SELECT ! Gas cut-edge.
                   IECE = CF%EDGE_LIST(2,IEDGE);  JECE = CF%EDGE_LIST(3,IEDGE)
                   II=M%CUT_EDGE(IECE)%IJK(IAXIS); JJ=M%CUT_EDGE(IECE)%IJK(JAXIS); KK=M%CUT_EDGE(IECE)%IJK(KAXIS);
                   X3AXIS = M%CUT_EDGE(IECE)%IJK(KAXIS+1);
