...
In FDS, the rate expression is on a fuel basis where only the concentrations of species in the mixed state ({\em hat} symbol) are allowed to react. For a reaction with fuel, $F$, and $\alpha$ species, Eq.~(\ref{eq:generic_rate}) becomes:
\begin{equation}\label{eq:finite_rate}
\frac{\mbox{d}\hat{C}_{F}}{\mbox{d}t}= -k\; \prod \hat{C}_{\alpha}\,^{a_{\alpha}}
\end{equation}
For an Arrhenius reaction the rate constant, $k$, is a function of temperature ($T^n$), the pre-exponential ($A$), and the activation energy ($E$):
\begin{equation}\label{eq:rate_cons}
k = A\;T^n\;e^{-E/RT} 
\end{equation}
In FDS input files, $A$ has units of mol/cm$^3$/s and $E$ has units of J/mol.
FDS solves for and transports mass fractions of lumped species rather for individual species; the only ``species'' that can be consumed or created are the lumped species.  However, any of the primitive species components of the lumped species can participate as a collision species.  Therefore, rather than solve $\mbox{d}\hat{C}_F/\mbox{d}t$,  FDS computes $\mbox{d}\hat{Y}_{F}/\mbox{d}t$. Since it is quicker to determine $\hat{Y}_{\alpha}$ rather than $\hat{C}_{\alpha}$, the relation $\hat{C}_{\alpha}=\hat{Y}_{\alpha} \rho/W_{\alpha}$ is used.  Substituting for $\hat{C}_{\alpha}$ into Eq.~(\ref{eq:finite_rate}) and converting the appropriate units of $A$ (kg/m$^3$/s) yields:
\begin{equation}\label{eq:finite_rate_unit}
\frac{\mbox{d}\hat{Y}_{F}}{\mbox{d}t} = -\frac{1000}{\rho}k \; \prod \left(\frac{\hat{Y}_{\alpha}\rho}{W_{\alpha}}\right)^{a_{\alpha}} \end{equation}
...
A^{\prime} = 1000\;A\;\rho^{\sum(a_{n}-1)}\prod W_{\alpha}\,^{-a_{\alpha}}
\end{equation}
Using $A^{\prime}$, the final form of the rate expression becomes:
\begin{equation}\label{eq:finite_rate_fin}
\frac{\mbox{d}\hat{Y}_{F}}{\mbox{d}t} = -A^{\prime}\;T^n\;e^{-E/RT}\;\prod \hat{Y}_{\alpha}\,^{a_{\alpha}} 
\end{equation}
