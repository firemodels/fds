...
DO K=0,MESHES(NM)%KBP1
   MESHES(NM)%U(:,:,K)   =               0.5_EB*MIXING_LAYER_U0*    TANH(2._EB*MESHES(NM)%ZC(K)/MIXING_LAYER_H0)
   MESHES(NM)%RHO(:,:,K) = RHOA*(1._EB - 0.5_EB*MIXING_LAYER_THETA0*TANH(2._EB*MESHES(NM)%ZC(K)/MIXING_LAYER_H0))
ENDDO
...
EPS_EDDY=-1._EB
CALL RANDOM_NUMBER(RN); IF (RN>0.5) EPS_EDDY(1)=1._EB
CALL RANDOM_NUMBER(RN); IF (RN>0.5) EPS_EDDY(2)=1._EB
CALL RANDOM_NUMBER(RN); IF (RN>0.5) EPS_EDDY(3)=1._EB
