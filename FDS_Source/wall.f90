...
REAL(EB) :: DT_BC,T,TSI,TMP_G,RHO_G,DTMP,TMP_OTHER,RAMP_FACTOR,QNET,FDERIV,TMP_EXTERIOR,UN,ARO,UWO,QEXTRA,RSUM_W, &
            YY_G_ALL(MAX_SPECIES),RHO_YY_F(MAX_SPECIES),YY_GET(1:N_SPECIES),YY_EXTERIOR(MAX_SPECIES)
...
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT
REAL(EB) :: YY_GET(1:N_SPECIES),YY_G(1:N_SPECIES),UN,MFT,TMP_G,RHO_G,DD,RSUM_F
...
WALL_CELL_LOOP: DO IW=1,NWC
   ...
   IF ( (SOLID(CELL_INDEX(II,JJ,KK)) .AND. .NOT.SOLID(CELL_INDEX(IIG,JJG,KKG))) .OR.  &
      ...
      IF (N_SPECIES>0) RSUM(II,JJ,KK)  = 2._EB*RSUM_F - RSUM(IIG,JJG,KKG)
...
REAL(EB) :: U2=0._EB,V2=0._EB,W2=0._EB,VELCON,H_NATURAL,H_FORCED,CP,YY_GET(1:N_SPECIES),MU_G,K_G
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU=>NULL(),VV=>NULL(),WW=>NULL(),RHOP=>NULL()
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: YYP=>NULL()
...
NO_GAS_CELL_IF: IF (IIG>=0) THEN  
   ...
   SELECT CASE (IHTMODEL)
      ...
      CASE DEFAULT
         MU_G = 1.8E-5_EB
         RE   = RHOP(IIG,JJG,KKG)*SQRT(U2+V2+W2)*CONV_LENGTH/MU_G
         K_G  = MU_G*1012._EB/0.7_EB
         SELECT CASE(GEOMETRY)
         CASE (SURF_CARTESIAN)
!            H_FORCED  = C_FORCED*VELCON*RHOP(IIG,JJG,KKG)**0.8_EB
            H_FORCED  =          C_FORCED         *RE**(0.8_EB)*(0.7_EB)**(1._EB/3._EB) * (K_G/CONV_LENGTH)
         CASE (SURF_CYLINDRICAL)
            H_FORCED  =          C_FORCED_CYLINDER*RE**(0.5_EB)*(0.7_EB)**(1._EB/3._EB) * (K_G/CONV_LENGTH)
         CASE (SURF_SPHERICAL)
            H_FORCED  = (2._EB + C_FORCED_SPHERE  *RE**(0.5_EB)*(0.7_EB)**(1._EB/3._EB))* (K_G/CONV_LENGTH)
         END SELECT
   END SELECT
