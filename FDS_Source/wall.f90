...
IF (VIRTUAL_PARTICLES .AND. CORRECTOR) THEN
   ...
   DROPLET_LOOP: DO I=1,NLP
      ...
      DO NS=0,N_TRACKED_SPECIES
         ...
         IF (ENTHALPY_TRANSPORT) E(II,JJ,KK) = E(II,JJ,KK) + DT*DR%PWT*M_DOT_PPP*CPBAR2*TMP_F(IW) 
      ...
      IF (ENTHALPY_TRANSPORT) E(II,JJ,KK) = E(II,JJ,KK) - DT*QCONF(IW)*AW(IW)*RVC * DR%PWT
