...
IF (VTK3D) THEN
   ...
   UNIQUE_LOOPA: DO II=1,MESHES(1)%N_UNIQUE_SLCF
      WRITE(*,*) "SLICE", MESHES(1)%UNIQUE_SLICE_NAMES(II)
      WRITE(*,*) " MESH", NM, "EXISTS", M%EMPTY_UNIQUE_SLICE(II)
      IF (M%EMPTY_UNIQUE_SLICE(II)) CYCLE UNIQUE_LOOPA
      ...
      QUANTITY_LOOPA: DO IQ=1,NQT
         ...
         IF (.NOT.VTK_INITIALIZED) THEN
            WRITE(FN_SL3D_VTK(II,NM),'(A,A,A,A,I0,A,I8.8,I2.2,A)') TRIM(CHID),'_',TRIM(MESHES(1)%UNIQUE_SLICE_NAMES(II)),&
                                     '_',NM,'_',ITM,ITM1,'.vtu'
            WRITE(*,*) "STARTING ON FILE", FN_SL3D_VTK(II,NM)
