...
! ! If gravitation deposition is enabled, transport depositing aerosol via 
! ! WW minus settling velocity. K. Overholt

! USE PHYSICAL_FUNCTIONS, ONLY: GET_VISCOSITY
! REAL(EB) :: TMP_G,MU_G,TWALL,TMP_FILM,MASS_P,KN
! INTEGER :: IOR
...
!          WW_GRAV = GVEC(3)*MASS_P*(1._EB+1.25_EB*KN+0.41_EB*KN*EXP(-0.88_EB/KN))/(3._EB*CHI_D*MU_G*SM%MEAN_DIAMETER)

!          ! Calculate FZ including WW_GRAV effects
!          IF (ABS(WW(I,J,K))>ZERO_P) FZ(I,J,K,N) = FZ(I,J,K,N)*(1._EB - WW_GRAV/WW(I,J,K))
!       ENDDO
!    ENDDO
! ENDDO
