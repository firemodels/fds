...
#ifdef WITH_HDF5
USE VTK_FDS_INTERFACE, ONLY : PARALLEL_INIT_F32,PARALLEL_WRITE_F32,&
                CLOSE_VTKHDF,CLOSE_PART_VTKHDF,PARALLEL_INIT_I32,PARALLEL_WRITE_I32,&
...
! Only write out spreadsheet data if not writing SMV file
...
REAL(EB) :: STIME
INTEGER  :: IPROC, IERR
INTEGER  :: NPP,NPLIM,IP,N,NM,NN,IZERO,ITM,ITM1
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   LPC => LAGRANGIAN_PARTICLE_CLASS(N)
   WRITE(FILENAME,'(A,A,A,A,A,I8.8,I2.2,A)') "",TRIM(VTK_DIR)//TRIM(CHID),&
   ...
   MESH_LOOP1: DO NM=1,NMESHES
      CALL POINT_TO_MESH(NM)
      IF (PROCESS(NM)/=MY_RANK) CYCLE MESH_LOOP1
   ...
   !!!!!!!!!!!!!!!!!!!!!!!
   ! Add MPI call here to pass number of particles in each mesh
   !!!!!!!!!!!!!!!!!!!!!!!
   ! Exchange boundary NCELLS, NPOINTS, and NCONNECTIONS
   MESH_LOOP2: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MY_RANK) THEN
         CALL MPI_RECV(NINFO,1,MPI_INTEGER,PROCESS(NM),PROCESS(NM),MPI_COMM_WORLD,MPISTATUS,IERR)
         NPOINTS(NM) = INT(NINFO(1),IB32)
         CYCLE MESH_LOOP2
      ELSE
         NINFO = INT((/NPOINTS(NM)/))
         DO IPROC=0,N_MPI_PROCESSES-1
            IF (MY_RANK/=IPROC) THEN
               CALL MPI_SEND(NINFO,1,MPI_INTEGER,IPROC,MY_RANK,MPI_COMM_WORLD,IERR)
            ENDIF
         ENDDO
      ENDIF
   ENDDO MESH_LOOP2
   
   NPOINTS_TOTAL = MAX(SUM(NPOINTS),1) ! !+1
   ...
   MESH_LOOP3: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MY_RANK) THEN
         ...
         ! Write empty data if mesh not owned by rank
      ...
      ALLOCATE(TA(NPLIM),STAT=IZERO)                  ; CALL ChkMemErr('DUMP','TA',IZERO)
      ALLOCATE(QP(NPLIM,LPC%N_QUANTITIES),STAT=IZERO) ; CALL ChkMemErr('DUMP','QP',IZERO)
      ALLOCATE(COLORS(3,NPLIM),STAT=IZERO) ; CALL ChkMemErr('DUMP','COLOR',IZERO)
