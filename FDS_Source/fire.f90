...
INTEGRATION_LOOP: DO WHILE (DT_ITER < DT)
   IF (COMBUSTION_ODE /= EDCM_RK2) THEN ! Explicit Euler
      ZETA = ZETA_0*EXP(-DT_SUB/MIX_TIME(I,J,K))
      TOTAL_MIX_MASS(1) = (1-ZETA)*CELL_MASS  
      SPEC_MIX_MASS(:,1) = SPEC_MIX_MASS(:,0) + (ZETA_0-ZETA)*CELL_MASS*ZZ_GET_0
      PHI = SPEC_MIX_MASS(:,1)/(TOTAL_MIX_MASS(1))
   ...
   ! Heat Release
   DO NSS = 0,N_TRACKED_SPECIES
      Q_SUM = Q_SUM - SPECIES_MIXTURE(NSS)%H_F*RHO(I,J,K)*(ZZ_GET(NSS)-ZZ_TEMP(NSS))/SPECIES_MIXTURE(NSS)%MW
   ENDDO
   IF (Q_SUM < ZERO_P) Q_SUM = 0._EB
   IF (Q_CALC + Q_SUM > Q_UPPER*DT_ITER) THEN
      Q_OUT = Q_UPPER
      ZZ_GET = (Q_UPPER*DT_ITER/(Q_CALC + Q_SUM))*ZZ_GET
      EXIT INTEGRATION_LOOP
   ELSE 
      Q_CALC = Q_CALC+Q_SUM    
      Q_OUT = Q_CALC/DT 
...
RECURSIVE SUBROUTINE COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT,ZZ_GET,I,J,K,DT_SUB,ITER)
USE PHYSICAL_FUNCTIONS, ONLY : GET_MASS_FRACTION_ALL
REAL(EB), INTENT(IN) :: ZZ_GET(0:N_TRACKED_SPECIES),DT_SUB
...
IF_SUPPRESSION: IF (SUPPRESSION) THEN
   IF (ITER==0) THEN
      SELECT CASE (EXTINCT_MOD)
         CASE(EXTINCTION_1)
            IF(EXTINCT_1(I,J,K,ZZ_GET)) THEN
               RATE_CONSTANT = 0._EB
               RETURN
            ENDIF
         CASE(EXTINCTION_2)
            IF(EXTINCT_2(I,J,K,ZZ_GET)) THEN
               RATE_CONSTANT = 0._EB
               RETURN
            ENDIF
      END SELECT       
...
REAL(EB):: H_T,H_T_CR,Q_SUM,PHI_STOICH(0:N_TRACKED_SPECIES),PHI_STOICH_SUM,PHI_NORMAL(0:N_TRACKED_SPECIES),PHI_NORMAL_SUM,&
           PHI_RATIO(0:N_TRACKED_SPECIES),D_PHI(0:N_TRACKED_SPECIES)
INTEGER, INTENT(IN) :: I,J,K
INTEGER :: NS,INDEX_LIMITING_REACTANT

EXTINCT_2 = .FALSE.
IF (TMP(I,J,K) < RN%AUTO_IGNITION_TEMPERATURE) THEN
   EXTINCT_2 = .TRUE.
ELSE
   PHI_STOICH_SUM = 0._EB
   PHI_NORMAL_SUM = 0._EB
   PHI_STOICH(:) = 0._EB
   PHI_NORMAL(:) = 0._EB
   PHI_RATIO(:) = 0._EB
   DO NS = 0,N_TRACKED_SPECIES
      IF (.NOT. RN%NU(NS)<-ZERO_P) CYCLE
      PHI_STOICH_SUM = PHI_STOICH_SUM + ABS(RN%NU(NS))*SPECIES_MIXTURE(NS)%MW
      PHI_NORMAL_SUM = PHI_NORMAL_SUM + PHI_IN(NS)
   ENDDO 
       
   DO NS = 0,N_TRACKED_SPECIES
      IF (.NOT. RN%NU(NS)<-ZERO_P) CYCLE
      PHI_STOICH(NS) = ABS(RN%NU(NS))*SPECIES_MIXTURE(NS)%MW/PHI_STOICH_SUM
      PHI_NORMAL(NS) = PHI_IN(NS)/PHI_NORMAL_SUM
   ENDDO

   DO NS = 0,N_TRACKED_SPECIES
      IF (PHI_NORMAL(NS) == 0._EB .OR. PHI_STOICH(NS) == 0._EB) CYCLE
      PHI_RATIO(NS) = PHI_NORMAL(NS)/PHI_STOICH(NS)
   ENDDO

   INDEX_LIMITING_REACTANT = 0
   IF (PHI_RATIO(INDEX_LIMITING_REACTANT) == 0._EB) INDEX_LIMITING_REACTANT = INDEX_LIMITING_REACTANT + 1
   DO NS = 1, N_TRACKED_SPECIES
      IF (.NOT. RN%NU(NS)<-ZERO_P) CYCLE
      IF (PHI_RATIO(NS) < PHI_RATIO(INDEX_LIMITING_REACTANT)) INDEX_LIMITING_REACTANT = NS
   ENDDO
   
   DO NS = 0,N_TRACKED_SPECIES
      D_PHI(NS) = RN%NU(NS)*SPECIES_MIXTURE(NS)%MW/&
              (ABS(RN%NU(INDEX_LIMITING_REACTANT))*SPECIES_MIXTURE(INDEX_LIMITING_REACTANT)%MW)*PHI_IN(INDEX_LIMITING_REACTANT)
   ENDDO    
      
   !See if enough energy is released to raise the temperatures above the critical flame temp
   Q_SUM = 0._EB
   DO NS = 0,N_TRACKED_SPECIES
      Q_SUM = Q_SUM - SPECIES_MIXTURE(NS)%H_F*D_PHI(NS)/SPECIES_MIXTURE(NS)%MW
   ENDDO
 
   CALL GET_SENSIBLE_ENTHALPY(PHI_IN,H_T,TMP(I,J,K)) 
   CALL GET_SENSIBLE_ENTHALPY(PHI_IN,H_T_CR,RN%CRIT_FLAME_TMP) 
   IF (H_T + Q_SUM < H_T_CR) EXTINCT_2 = .TRUE.
