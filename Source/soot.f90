...
USE GLOBAL_CONSTANTS, ONLY: SOLID_PHASE_ONLY,SOLID_BOUNDARY,N_TRACKED_SPECIES,TURBULENT_DEPOSITION, PREDICTOR
INTEGER, INTENT(IN) :: NM
...
SMIX_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY .OR. P1%NODE_INDEX /= 0) CYCLE WALL_CELL_LOOP
...
REAL(EB) :: U_THERM,U_TURB,MU_G,Y_AEROSOL,TMP_G,RHO_G,ZZ_GET(1:N_TRACKED_SPECIES),YDEP,K_G,TMP_FILM,ALPHA,DTMPDX,&
            TAU_PLUS,U_GRAV,D_SOLID,MW_RATIO,KN,KN_FAC,NVEC(3),RSUM_G,ZZ_G(1:N_TRACKED_SPECIES)
INTEGER  :: IIG,JJG,KKG
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   ...
   RHO_G = CFA%RHO_G
   RSUM_G= CFA%RSUM_G
   ZZ_G(1:N_TRACKED_SPECIES) = CFA%ZZ_G(1:N_TRACKED_SPECIES)
ENDIF
...
TMP_FILM = 0.5_EB*(TMP_G+P1%TMP_F)
CALL GET_VISCOSITY(ZZ_GET,MU_G,TMP_FILM)
CALL GET_CONDUCTIVITY(ZZ_GET,K_G,TMP_FILM)
KN_FAC = MU_G*SQRT(0.5_EB*PI/(PBAR(KKG,PRESSURE_ZONE(IIG,JJG,KKG))*RHO_G))
ALPHA = K_G/SM%CONDUCTIVITY_SOLID
DTMPDX = P1%HEAT_TRANS_COEF*(TMP_G-P1%TMP_F)/K_G
IF (THERMOPHORETIC_DEPOSITION) THEN
   KN = KN_FAC/SM%THERMOPHORETIC_DIAMETER
   U_THERM = CS2*(ALPHA+CT*KN)*CUNNINGHAM(KN)/((1._EB+CM3*KN)*(1+2*ALPHA+CT2*KN)) * MU_G/(TMP_G*RHO_G)*DTMPDX
ENDIF
...
IF (TURBULENT_DEPOSITION) THEN
   KN = KN_FAC/SM%MEAN_DIAMETER
   TAU_PLUS = TAU_PLUS_C/MU_G**2*P2%U_TAU**2*RHO_G
   IF (TAU_PLUS < 0.2_EB) THEN ! Diffusion regime
      D_SOLID = K_BOLTZMANN*TMP_G*CUNNINGHAM(KN)/(3._EB*PI*MU_G*SM%MEAN_DIAMETER)
      U_TURB = P2%U_TAU * 0.086_EB*(MU_G/RHO_G/D_SOLID)**(-0.7_EB)
   ELSEIF (TAU_PLUS >= 0.2_EB .AND. TAU_PLUS < 22.9_EB) THEN ! Diffusion-impaction regime
      U_TURB = P2%U_TAU * 3.5E-4_EB * TAU_PLUS**2
   ELSE ! Inertia regime
      U_TURB = P2%U_TAU * 0.17_EB
   ENDIF
ENDIF

IF (PRESENT(WALL_INDEX)) THEN
   WC%V_DEP = MAX(0._EB,U_THERM+U_TURB+U_GRAV+P1%U_NORMAL)
   IF (WC%V_DEP <= TWO_EPSILON_EB) RETURN
   ZZ_GET = ZZ_GET * RHO_G
   Y_AEROSOL = ZZ_GET(N)
   YDEP = Y_AEROSOL*MIN(1._EB,(WC%V_DEP)*DT*P1%RDN)
   ZZ_GET(N) = Y_AEROSOL - YDEP
   IF (SM%AWM_INDEX > 0) P1%AWM_AEROSOL(SM%AWM_INDEX)= P1%AWM_AEROSOL(SM%AWM_INDEX)+YDEP/P1%RDN
   IF (SS%AWM_INDEX > 0) P1%AWM_AEROSOL(SS%AWM_INDEX)= P1%AWM_AEROSOL(SS%AWM_INDEX)+YDEP/P1%RDN
ENDIF
IF (PRESENT(CFACE_INDEX)) THEN
   CFA%V_DEP = MAX(0._EB,U_THERM+U_TURB+U_GRAV+P1%U_NORMAL)
   IF (CFA%V_DEP <= TWO_EPSILON_EB) RETURN
   ZZ_GET = ZZ_GET * RHO_G
   Y_AEROSOL = ZZ_GET(N)
   YDEP = Y_AEROSOL*MIN(1._EB,(CFA%V_DEP)*DT*P1%RDN)
   ZZ_GET(N) = Y_AEROSOL - YDEP
   IF (SM%AWM_INDEX > 0) P1%AWM_AEROSOL(SM%AWM_INDEX)= P1%AWM_AEROSOL(SM%AWM_INDEX)+YDEP/P1%RDN
   IF (SS%AWM_INDEX > 0) P1%AWM_AEROSOL(SS%AWM_INDEX)= P1%AWM_AEROSOL(SS%AWM_INDEX)+YDEP/P1%RDN
ENDIF

D_SOURCE(IIG,JJG,KKG) = D_SOURCE(IIG,JJG,KKG) - MW_RATIO*YDEP / RHO_G / DT
...
WALL_CELL_LOOP: DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (P1%AWM_AEROSOL(SS%AWM_INDEX)<ZZ_MIN_GLOBAL) CYCLE WALL_CELL_LOOP
   M_SOOT = P1%AWM_AEROSOL(SS%AWM_INDEX)*P1%AREA
   IIG = BC%IIG
   JJG = BC%JJG
   KKG = BC%KKG
   ...
   X_O2 = Y_O2*MW/MW_O2
   DZZ = 0._EB
   DMDT = A*M_SOOT*X_O2*EXP(E/P1%TMP_F)
   ...
   IF (SS%AGGLOMERATING) THEN
      ...
      DO NS=AGGLOMERATION_SMIX_INDEX(NS2),AGGLOMERATION_SMIX_INDEX(NS2)+N_PARTICLE_BINS(NS2)-1
         ...
         IF (SM%AWM_INDEX > 0) &
            P1%AWM_AEROSOL(SM%AWM_INDEX) = P1%AWM_AEROSOL(SM%AWM_INDEX) - DM * P1%AWM_AEROSOL(SM%AWM_INDEX)/M_SOOT
      ENDDO
   ENDIF
   P1%AWM_AEROSOL(SS%AWM_INDEX) = P1%AWM_AEROSOL(SS%AWM_INDEX) - DM
   ...
   DO NS=1,N_TRACKED_SPECIES
      ...
      ELSE
         ...
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR2,P1%TMP_F)
         DELTA_H_G = CPBAR2*P1%TMP_F-CPBAR*TMP_G
...
CFACE_LOOP: DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   BC=>BOUNDARY_COORD(CFA%BC_INDEX)
   IF (CFA%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE CFACE_LOOP
   IF (P1%AWM_AEROSOL(SS%AWM_INDEX)<ZZ_MIN_GLOBAL) CYCLE CFACE_LOOP
   M_SOOT = P1%AWM_AEROSOL(SS%AWM_INDEX)*P1%AREA
   IIG = BC%IIG
   JJG = BC%JJG
   KKG = BC%KKG
   RHO_G = CFA%RHO_G
   TMP_G = CFA%TMP_G
   ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,CFA%ZZ_G(1:N_TRACKED_SPECIES)- M_DOT_PPP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)*DT/RHO_G) 
   CALL GET_MASS_FRACTION(ZZ_GET,O2_INDEX,Y_O2)
   IF (Y_O2 < ZZ_MIN_GLOBAL) CYCLE
   CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW)
   X_O2 = Y_O2*MW/MW_O2
