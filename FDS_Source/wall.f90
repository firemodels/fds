...
REAL(EB) :: DT_BC,T,TSI,TMP_G,RHO_G,DTMP,TMP_OTHER,RAMP_FACTOR,QNET,FDERIV,TMP_EXTERIOR,UN,ARO,UWO,QEXTRA,RSUM_W, &
            ZZ_G_ALL(MAX_SPECIES),RHO_ZZ_F(MAX_SPECIES),ZZ_GET(1:N_TRACKED_SPECIES),ZZ_EXTERIOR(MAX_SPECIES)
...
USE MATH_FUNCTIONS, ONLY : EVALUATE_RAMP
REAL(EB) :: T,ZZ_G,RHO_G,UN,DD,MFT,TSI,RADIUS,AREA_SCALING,&
            RVC,RHO_NEW,M_DOT_PPP,CP,CPBAR,MW_RATIO,H_G,DELTA_H_G,ZZ_GET(1:N_TRACKED_SPECIES),CPBAR2
      ...
      CASE (NO_MASS_FLUX) METHOD_OF_MASS_TRANSFER 
         ...
         IF ( SF%LEAK_PATH(1)>-1 .AND. UWS(IW)<0._EB) THEN
            ...
            SPECIES_LOOP_1: DO N=1,N_TRACKED_SPECIES
               ...
               ZZ_F(IW,N) = ( MASSFLUX(IW,N) + ZZ_G*DD ) / (DD + UN*RHO_F(IW))
...
USE HVAC_ROUTINES, ONLY : NODE_TMP,NODE_RHO,DUCT_U,LEAK_PATH,NODE_ZZ
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   IF (I_FUEL >= 0) MFLUX = MASSFLUX_ACTUAL(IW,I_FUEL)
   MASSFLUX(IW,:)       = 0._EB
   ...
   NEW_EVAP_IF: IF (NEW_EVAP) THEN
      MATERIAL_LOOP2B: DO N=1,SF%N_MATL
         ...
         ZZ_GET = MAX(0._EB,ZZ(IIG,JJG,KKG,:))
         CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW_G)
         X_G = ZZ_GET(I_FUEL)/REACTION(1)%MW_FUEL*MW_G
         ...
         IF (DNS) THEN
            ...
            D_AIR = Z2D(ITMP,I_FUEL)+TMP_WGT*(Z2D(ITMP+1,I_FUEL)-Z2D(ITMP+1,I_FUEL))
            H_MASS = 2._EB*D_AIR*RDN(IW)
         ELSE
            MU_AIR = Z2MU_C(MIN(5000,NINT(TMP_G)))*SPECIES_MIXTURE(0)%MW
   ...
   ELSE NEW_EVAP_IF !Old routine
      IF (I_FUEL >=0) MFLUX = MAX(0._EB,MFLUX - MASSFLUX_ACTUAL(IW,I_FUEL)) ! Decrease 
      MATERIAL_LOOP2: DO N=1,SF%N_MATL
         ...
         ! gas phase 
         Z_MF_G = MAX(0._EB,ZZ(IIG,JJG,KKG,I_FUEL))
         RSUM_G = RSUM(IIG,JJG,KKG)                    
         ZPRSUM  = Z_MF_G/RSUM_G
         HVRG    = REACTION(1)%MW_FUEL*ML%H_R(1)/R0
         PPSURF  = MIN(1._EB,(R0/REACTION(1)%MW_FUEL)*ZPRSUM)
...
REAL(EB) :: U2=0._EB,V2=0._EB,W2=0._EB,VELCON,H_NATURAL,H_FORCED,CP,ZZ_GET(1:N_TRACKED_SPECIES),NUSSELT,MU_G
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU=>NULL(),VV=>NULL(),WW=>NULL(),RHOP=>NULL()
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP=>NULL()
...
NO_GAS_CELL_IF: IF (IIG>=0) THEN  
   ...
   IF (H_EDDY .OR. H_LOGLAW .OR. H_CHILTON_COLBURN) THEN
      ...
      IF (N_TRACKED_SPECIES > 0) THEN
         ZZ_GET = MAX(0._EB,ZZP(IIG,JJG,KKG,:))
...
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_SOOT,RHOG,ZIN(1:N_TRACKED_SPECIES),YDEP,K_AIR,TMP_FILM
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   JJG = IJKW(7,IW)
   KKG = IJKW(8,IW)
   ZIN = MAX(0._EB,ZZ(IIG,JJG,KKG,:))
   IF (ZIN(I_PROG_SOOT) < 1.E-14_EB) CYCLE WALL_CELL_LOOP
   TGAS = TMP(IIG,JJG,KKG)
   TWALL = TMP_F(IW)
   TMP_FILM = 0.5_EB*(TGAS+TWALL)
   RHOG=RHO(IIG,JJG,KKG)
   CALL GET_VISCOSITY(ZIN,MUGAS,TMP_FILM)
   CALL GET_CONDUCTIVITY(ZIN,K_AIR,TMP_FILM)
