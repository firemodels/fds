...
PLOOP: DO N=1,5
   CALL GET_QUANTITY_INDEX(PLOT3D_SMOKEVIEW_LABEL(N),PLOT3D_SMOKEVIEW_BAR_LABEL(N),PLOT3D_QUANTITY_INDEX(N), &
                           PLOT3D_SPEC_INDEX(N),I_DUM,'PLOT3D',PLOT3D_QUANTITY(N),PLOT3D_SPEC_ID(N),'null')
ENDDO PLOOP
...
NAMELIST /SPEC/ MASS_FRACTION_0,MW,FYI,ID,SIGMALJ,EPSILONKLJ,CONDUCTIVITY,VISCOSITY,DIFFUSIVITY,ABSORBING, &
                MASS_EXTINCTION_COEFFICIENT,FORMULA
...
SET_MIXTURE_FRACTION: IF (MIXTURE_FRACTION) THEN
   ...
   SET_THREE_PARAMETER: IF (CO_PRODUCTION) THEN !Set reaction variables for three parameter mixture fraction 
      ALLOCATE(Z2Y(N_STATE_SPECIES,3),STAT=IZERO)
      CALL ChkMemErr('READ','Z2Y',IZERO) 
      Z2Y = 0._EB
      ALLOCATE(Z2MW(N_STATE_SPECIES,3),STAT=IZERO)
   ...
   ELSE SET_THREE_PARAMETER !Set reaction variables for two parameter mixture fraction 
      ALLOCATE(Z2Y(N_STATE_SPECIES,2),STAT=IZERO)
      CALL ChkMemErr('READ','Z2Y',IZERO) 
      Z2Y = 0._EB
      ALLOCATE(Z2MW(N_STATE_SPECIES,2),STAT=IZERO)
      CALL ChkMemErr('READ','Z2MW',IZERO) 
      Z2MW = 0._EB
   ...
   T_LOOP: DO J=1,500
      ...
      SELECT CASE (SPECIES(N)%MODE)
         CASE (MIXTURE_FRACTION_SPECIES)
            ...
            CP_MF(N2_INDEX,J)    = CP_N2*1000._EB
            CP_MF(OTHER_INDEX,J) = CP_N2*1000._EB
            CP_MF(H2O_INDEX,J)   = CP_H2O*1000._EB
            CP_MF(CO2_INDEX,J)   = CP_CO2*1000._EB
            CP_MF(CO_INDEX,J)    = CP_CO*1000._EB
            CP_MF(SOOT_INDEX,J)  = 904._EB !cp carbon
...
SPECIES_LOOP_2: DO N=0,N_SPECIES
   ...
   IF (SPECIES(N)%MODE/=GAS_SPECIES) CYCLE SPECIES_LOOP_2
...
MF_DAND_K: IF (MIXTURE_FRACTION) THEN
   ALLOCATE(Z2D_C(500))
   ALLOCATE(Z2K_C(500))
   ALLOCATE(Z2MU(N_STATE_SPECIES,500))      
   IF(CO_PRODUCTION) THEN
      ALLOCATE(Z2D(500,3))
      ALLOCATE(Z2K(500,3))      
   ELSE
   ...
   DO ITMP=1,500
      SUB_SPECIES_LOOP2: DO NN=1,N_STATE_SPECIES
         ...
         K_MF_TMP(NN) = MU_N*CP_MF(NN,ITMP)/PR
         !Diffusivity
   ...
   IF (ANY(PC%QUANTITIES/='null')) THEN
      QUANTITIES_LOOP: DO NN=1,10
         ...
         CALL GET_QUANTITY_INDEX(PC%SMOKEVIEW_LABEL(PC%N_QUANTITIES),PC%SMOKEVIEW_BAR_LABEL(PC%N_QUANTITIES), &
                                 PC%QUANTITIES_INDEX(PC%N_QUANTITIES),I_DUMMY,I_DUMMY,'PART',PC%QUANTITIES(NN),'null','null')
