   ...
   IF_SEND_PARTICLES: IF (CODE==6 .AND. PARTICLE_FILE) THEN 
      ...
      IF (SNODE/=RNODE) THEN
         ...
         DO N=1,MIN(M3%N_DROP_ORPHANS,N_DROP_ADOPT_MAX)
            ...
            M3%REAL_SEND_PKG6((N-1)*NC+10) = M3%PARTICLE(N)%A_X
            M3%REAL_SEND_PKG6((N-1)*NC+11) = M3%PARTICLE(N)%A_Y
            M3%REAL_SEND_PKG6((N-1)*NC+12) = M3%PARTICLE(N)%A_Z
   ...
   IF_RECEIVE_PARTICLES: IF (CODE==6 .AND. PARTICLE_FILE .AND. RNODE/=SNODE) THEN 
      IF_PARTICLES_SENT: IF (M2%N_DROP_ADOPT>0) THEN
         ...
         DO N=M4%NLP+1,M4%NLP+M2%N_DROP_ADOPT
            ...
            M4%PARTICLE(N)%A_X = M2%REAL_RECV_PKG6((NN)*NC+10) 
            M4%PARTICLE(N)%A_Y = M2%REAL_RECV_PKG6((NN)*NC+11) 
            M4%PARTICLE(N)%A_Z = M2%REAL_RECV_PKG6((NN)*NC+12) 
