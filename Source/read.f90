...
REAL(EB) :: MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            ...
            CONVERSION,PR_GAS,CONDUCTIVITY_LIQUID,VISCOSITY_LIQUID,BETA_LIQUID,THERMOPHORETIC_DIAMETER
...
NR2 = 1
FOUND = .FALSE.
PRIMITIVE_LOOP: DO N=1,N_SPECIES - N_COPY_PRIMITIVE
   ...
   IF (SS%LISTED .OR. SS%REFERENCE_ENTHALPY > -1.E21_EB .OR. SS%H_F > -1.E21_EB) SS%EXPLICIT_H_F = .TRUE.
...
SPEC_LOOP: DO N=1,N_SPECIES
   ...
   DO I = 1, I_MAX_TEMP
      CALL CALC_GAS_PROPS(1,N,SS_D(N,I),SS_MU(N,I),SS_K(N,I),SS_CP(N,I),SS_G_F(N,I))
      IF (SS%RAMP_CP_INDEX > 0)     SS_CP(N,I)  = EVALUATE_RAMP(0._EB,SS%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
      IF (SS%RAMP_D_INDEX > 0)      SS_D(N,I)   = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_D_INDEX,TAU=1._EB)
      IF (SS%RAMP_G_F_INDEX > 0)    SS_G_F(N,I) = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_G_F_INDEX,TAU=1._EB)
      IF (SS%RAMP_K_INDEX>0)        SS_K(N,I)   = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_K_INDEX,TAU=1._EB)/SQRT(SS%MW)
      IF (SS%RAMP_MU_INDEX > 0)     SS_MU(N,I)  = EVALUATE_RAMP(REAL(J,EB),SS%RAMP_MU_INDEX,TAU=1._EB)/SQRT(SS%MW)
   ...
   IF (SS%REFERENCE_ENTHALPY > -1.E21_EB) THEN
      ...
      H_REF = SS_H(N,ITMP)*REAL(ITMP,EB) + (SS%REFERENCE_TEMPERATURE-REAL(ITMP,EB)) * (SS_H(N,ITMP+1)-SS_H(N,ITMP))
      SS_H(N,:) = SS_H(N,:) + SS%REFERENCE_ENTHALPY - H_REF
      ITMP = INT(H_F_REFERENCE_TEMPERATURE)
      SS%H_F = SS_H(N,ITMP)*REAL(ITMP,EB) + (H_F_REFERENCE_TEMPERATURE-REAL(ITMP,EB)) * (SS_H(N,ITMP+1)-SS_H(N,ITMP))
   ELSE
      IF (SS%H_F > -1.E21_EB .OR. SS%H_F_LISTED > -1.E21_EB) THEN
      ITMP = INT(H_F_REFERENCE_TEMPERATURE)
      H_REF = SS_H(N,ITMP)*REAL(ITMP,EB) + (H_F_REFERENCE_TEMPERATURE-REAL(ITMP,EB)) * (SS_H(N,ITMP+1)-SS_H(N,ITMP))
   ...
   LIQUID_IF: IF (CP_TEMP > 0._EB .OR. SS%TMP_MELT >= 0._EB .OR. SS%TMP_V > 0._EB .OR. SS%DENSITY_LIQUID > 0._EB .OR. &
      SS%BETA_LIQUID > 0._EB .OR. SS%K_LIQUID > 0._EB .OR. SS%MU_LIQUID > 0._EB .OR. SS%SPECIFIC_HEAT_LIQUID > 0._EB .OR. &
      SS%RAMP_CP_INDEX > 0) THEN
      ...
      DO I=1,I_MAX_TEMP
         ...
         SS%H_L = SS%H_L + 0.5+EB*(SS%C_P_L(I-1) + SS%C_P_L(I))
      ENDDO
...
SPEC_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   ELSEIF (SM%H_F > -1.E21_EB) THEN
      ITMP = H_F_REFERENCE_TEMPERATURE
   ...
   ELSE
      CPBAR_Z(0,N) = SUM(Z2Y(:,N) * SS_H(:,0))
      SM%H_F = SUM(Z2Y(:,N) * SPECIES(:)%H_F)
   ENDIF

   DO I = 1,I_MAX_TEMP
      CPBAR_Z(I,N) = (CPBAR_Z(0,N)+H_SENS_Z(I,N))/REAL(I,EB)
   ENDDO
   ...
   ELSE SIMPLE_CHEMISTRY_IF ! Simple chemistry reaction
      ...
      ELSE HOC_IF ! Heat of combustion not specified, use EPUMO2 or H_F is fuel is listed
         ...
         ELSE LISTED_FUEL_IF ! Listed Fuel
            RN%HEAT_OF_COMBUSTION  = SMF%H_F_HOC+RN%S*SPECIES_MIXTURE(1)%H_F-(1._EB+RN%S)*SPECIES_MIXTURE(RN%PROD_SMIX_INDEX)%H_F
            SM => SPECIES_MIXTURE(1)
            RN%EPUMO2 = RN%HEAT_OF_COMBUSTION*SMF%MW*RN%NU(RN%FUEL_SMIX_INDEX)/(RN%NU(1)*SM%MW*SM%MASS_FRACTION(O2_INDEX))
            IF (RN%N_SIMPLE_CHEMISTRY_REACTIONS==2) THEN
...
IF (ANY(REDEFINE_H_F)) THEN
   DO NS=1,N_TRACKED_SPECIES
      ...
      DO J=1,I_MAX_TEMP
         CPBAR_Z(J,NS) = CPBAR_Z(J,NS) + D_H_F
      ENDDO
...
INTEGER :: N,NN
REAL(EB) :: DENSITY,MASS,VOLUME,R_O,R_I
TYPE(LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC=>NULL()
TYPE(SPECIES_TYPE),POINTER:: SS=>NULL()
TYPE(SURFACE_TYPE),POINTER:: SF=>NULL()
