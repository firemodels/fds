...
If a single droplet having a constant drag coefficient falls through a stationary fluid, its velocity and trajectory can be found analytically. This is the ``terminal velocity'' case. The particle starts from rest and falls under the influence of gravity and drag in 1D. FDS runs until a steady-state solution is reached. The equation governing this motion is
\[
	\dot{u} = K u^2 - g \quad ; \quad K \equiv \frac{\rho_a C_d A}{2 m}
\]
$\rho_a$ is the ambient air density, $C_d$ is the drag coefficient of the droplet, $A$ is the cross-sectional area of the droplet, and $m$ is the mass of the droplet. Defining $\rho_d$ as the density of the droplet and $D_d$ as the diameter of the droplet and solving this ODE with zero ICs, the solution below and equivalent definition of $K$ for a droplet is found.
% D = 10 mm
% Cd = 1.0
% g = -9.8 m/s^2
\be
...
\ee
FDS ran the terminal velocity case over several time steps. The precise configuration of the scenario is detailed in the table below. The error of this terminal velocity and the L$_\infty$ norm of position error are computed and plotted for each time step.
\begin{center}
\begin{tabular}{|r|c|l|}
\hline
...
\end{tabular}
\end{center}
FDS demonstrated second-order convergence in the terminal velocity case for both the velocities and the positions. See figures \ref{fig_terminal_velocity_convergence} and \ref{fig_position_convergence}.
\begin{figure}
   \begin{center}
      \scalebox{.7}{\includegraphics{SCRIPT_FIGURES/terminal_velocity_convergence}}
      \caption{\label{fig_terminal_velocity_convergence} \small FDS exhibits second-order convergence for the particle velocity in the terminal velocity case.}
   \end{center}
\end{figure}
\begin{figure}
   \begin{center}
      \scalebox{.7}{\includegraphics{SCRIPT_FIGURES/position_convergence}}
      \caption{\label{fig_position_convergence} \small FDS exhibits second-order convergence for the particle position in the terminal velocity case.}
   \end{center}
\end{figure}
\section{2D Flat Fire Trajectory (flat\_fire)}
There are no known solutions of the drag equations in higher dimensions. However, ballisticians have developed an approximate solution useful for computing trajectories of projectiles when guns are shot within a certain range of angles. This solution makes use of the ``flat fire'' assumption. In short, if a particle is launched with its velocity in only the $x$-direction and its $y$-velocity is never very significant compared against the $x$-velocity, then throughout the entire trajectory $u$ is a good approximation for $|\mathbf{u}|$.
...
\be
	...
	y = h + \frac{g \text{log}(V_0 K t + 1)}{2 (K V_0)^2} - \frac{g t^2}{4} - \frac{g t}{2 K V_0} \quad ; \quad
	K \equiv \frac{3 \rho_a C_d}{4 \rho_d D_d}
\ee
\be
	u = \frac{d x}{d t} = \frac{V_0}{V_0 K t + 1} \quad ; \quad v = \frac{d y}{d t} = \frac{g}{2 K V_0 (K V_0 t + 1)} - \frac{g t}{2} - \frac{g}{2 K V_0}
\ee
