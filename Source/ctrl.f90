...
USE GLOBAL_CONSTANTS, ONLY : UPDATE_DEVICES_AGAIN,EXTERNAL_CTRL
...
CONTROL_SELECT: SELECT CASE (CF%CONTROL_INDEX)
   ...
   CASE (CORE_DUMP)
      SELECT CASE (CF%INPUT_TYPE(1))
         ...
         CASE (CONTROL_INPUT)
            ...
            T_CHANGE = T
      END SELECT
      IF (STATE2) RSRT_CLOCK(RSRT_COUNTER(1)) = T_CHANGE
   ...
   CASE (CF_STEADY)
      ...
      SELECT CASE (CF%INPUT_TYPE(1))
         CASE (DEVICE_INPUT)
            DV => DEVICE(CF%INPUT(1))
            VALUE1 = DV%PREVIOUS_VALUE
            VALUE2 = DV%SMOOTHED_VALUE
            CF%MESH = DV%MESH
         CASE (CONTROL_INPUT)
            IF (.NOT. CONTROL(CF%INPUT(1))%UPDATED) THEN
               CALL EVALUATE_CONTROL(T,CF%INPUT(1),DT,CTRL_STOP_STATUS)
               CF => CONTROL(ID)
            ENDIF
            VALUE1 = CF%PREVIOUS_VALUE
            VALUE2 = CF%INSTANT_VALUE
            CF%MESH=CONTROL(CF%INPUT(1))%MESH
      END SELECT
