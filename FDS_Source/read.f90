...
SPEC_LOOP: DO N=1,N_SPEC_READ
   ...
   REFERENCE_TEMPERATURE  = 298.15_EB
   CALL CHECKREAD('SPEC',LU_INPUT,IOS)
   ...
   IF ((SPECIFIC_HEAT < 0._EB .AND. SPECIFIC_ENTHALPY > 0._EB).OR.(SPECIFIC_HEAT > 0._EB .AND. SPECIFIC_ENTHALPY < 0._EB)) THEN
      WRITE(MESSAGE,'(A,I2)') 'ERROR: SPECies number',N_SPECIES+1,&
         ' if either SPECIFIC_HEAT or ESPECIFIC_NTHALPY is specified, then both must be specified'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   SS%REFERENCE_TEMPERATURE = REFERENCE_TEMPERATURE 
...
! Loop through temperatures from 1 K to 5000 K to get temperature-specific gas properties
...
T_LOOP_1: DO J=1,5000
   ...
   IF (MIXTURE_FRACTION) THEN
      ...
      CP_TMP(SOOT_INDEX)  =  904._EB ! carbon
   ...
   SPECIES_LOOP_1: DO N = 0,N_SPECIES
      ...
      IF (SS%SPECIFIC_HEAT > 0._EB) THEN
         CP_TMP(SS%INDEX) = SS%SPECIFIC_HEAT
      ELSE
         SELECT CASE(SS%ID)
            CASE DEFAULT
         ...
         END SELECT      
      ENDIF
      IF (SS%SPECIFIC_ENTHALPY > 0._EB) THEN
         H_TMP(SS%INDEX) = SS%SPECIFIC_ENTHALPY - SS%SPECIFIC_HEAT * SS%REFERENCE_TEMPERATURE
      ELSE
         SELECT CASE(SS%ID)
            CASE('AIR')
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(0:MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2), &
            ...
            BOUNDARY_LAYER_THICKNESS,ROUGHNESS
...
IF (LES) ROUGHNESS      = 4.5E-5_EB  ! meters, commercial steel
...
NAMELIST /BNDF/ CELL_CENTERED,QUANTITY,FYI,PROP_ID,SPEC_ID,PART_ID
