...
UNSTRUCTURED_GEOMETRY: IF (N_BNDE>0) THEN
...
WRITE(LU_CORE(NM)) T,ICYC,PART_CLOCK(NM),CORE_CLOCK(NM),SLCF_CLOCK(NM), &
             PL3D_CLOCK(NM),BNDF_CLOCK(NM),BNDE_CLOCK,DEVC_CLOCK,HRR_CLOCK,MINT_CLOCK, &
...
SELECT CASE(IND)
   CASE DEFAULT
      GEOM_OUTPUT = 0._EB
   CASE(400)  ! Face Velocity
      GEOM_OUTPUT = 0._EB
   CASE(401)  ! Face Temperature
      GEOM_OUTPUT = FACET(FACE_INDEX)%TMP_F-273.15_EB
END SELECT   
...
GLOOP: DO N=1,1
   ...
   WRITE(LU_GEOM(N)) N_VERT
   DO I=1,N_VERT
      WRITE(LU_GEOM(N)) REAL(VERTEX(I)%X,FB),REAL(VERTEX(I)%Y,FB),REAL(VERTEX(I)%Z,FB)
   ENDDO
   WRITE(LU_GEOM(N)) N_FACE
   DO I=1,N_FACE
      WRITE(LU_GEOM(N)) FACET(I)%VERTEX,REAL(FACET(I)%NVEC,FB)
   ENDDO  
