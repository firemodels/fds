...
PUBLIC :: INIT_IBM,TRILINEAR,GETU,GETGRAD,GET_VELO_IBM,INIT_FACE,GET_REV_geom, &
          READ_GEOM,READ_VERT,READ_FACE,READ_VOLU,LINKED_LIST_INSERT,WRITE_GEOM,WRITE_GEOM_ALL
...
READ_GEOM_LOOP: DO N=1,N_GEOMETRY
   ...
   N_VERTS=0
   DO I = 1, MAX_VERTS
      IF (ANY(VERTS(3*I-2:3*I).GE.MAX_VAL)) EXIT
      N_VERTS = N_VERTS+1
   END DO
   ...
   N_FACES=0
   DO I = 1, MAX_FACES
      IF (ANY(FACES(3*I-2:3*I).EQ.0)) EXIT
      N_FACES = N_FACES+1
   END DO
   ...
   N_VOLUS=0
   DO I = 1, MAX_VOLUS
      IF (ANY(VOLUS(4*I-3:4*I).EQ.0)) EXIT
      N_VOLUS = N_VOLUS+1
   END DO
   ...
   ZVALS_IF: IF (N_ZVALS.GT.0) THEN
      ...
      ENDIF
      IF (IJK(1).LT.2.OR.IJK(2).LT.2) THEN
...
!    I11   I21      
   ...
   IF (SPHERE_RADIUS.LT.MAX_VAL.AND. &
      SPHERE_ORIGIN(1).LT.MAX_VAL.AND.SPHERE_ORIGIN(2).LT.MAX_VAL.AND.SPHERE_ORIGIN(3).LT.MAX_VAL) THEN
      GEOM_TYPE = 2
      ...
      IF (SPHERE_RADIUS.LT.100.0_EB*TWO_EPSILON_EB) SPHERE_RADIUS = 100.0_EB*TWO_EPSILON_EB

      IF (SPHERE_TYPE.NE.2) SPHERE_TYPE = 1
      IF (N_LEVELS.LT.0.AND.N_LAT.GT.0.AND.N_LONG.GT.0) SPHERE_TYPE = 2
      IF (SPHERE_TYPE.EQ.1) THEN
         IF (N_LEVELS==-1) N_LEVELS = INT(LOG(2.0_EB*PI*SPHERE_RADIUS/(5.0_EB*DX))/LOG(2.0_EB))
         N_LEVELS = MIN(5,MAX(0,N_LEVELS))
      ELSE
         IF (N_LONG.LT.6) N_LONG = MAX(6,INT(2.0_EB*PI*SPHERE_RADIUS/DX)+1)
         IF (N_LAT.LT.3)  N_LAT = MAX(3,INT(PI*SPHERE_RADIUS/DX)+1)
      ENDIF
      
      IF (SPHERE_TYPE.EQ.1) THEN
         CALL INIT_SPHERE(N_LEVELS,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,VERTS,FACES,TFACES)
      ELSE
   ...
   G%HAS_TEXTURE=0
   IF (SURF_INDEX.GE.1.AND.SURF_INDEX.LE.N_SURF) THEN
      IF(SURFACE(SURF_INDEX)%TEXTURE_MAP/='null')G%HAS_TEXTURE=1
   ENDIF
   ...
   IF (N_VOLUS.GT.0) THEN
      ...
      IF (ANY(VOLUS(1:4*N_VOLUS).LT.1.OR.VOLUS(1:4*N_VOLUS).GT.N_VERTS)) THEN
   ...
   IF (N_FACES.GT.0) THEN
      ...
      IF ( ANY(FACES(1:3*N_FACES).LT.1.OR.FACES(1:3*N_FACES).GT.N_VERTS) ) THEN
         CALL SHUTDOWN('ERROR: problem with GEOM, vertex index out of bounds')
      ENDIF
   ...
   IF (ANY(GAXIS(1:3).LT.MAX_VAL).OR.GROTATE.LT.MAX_VAL.OR.GROTATE_DOT.LT.MAX_VAL) THEN
      ...
      IF (GAXIS(2).GT.MAX_VAL) GAXIS(2) = 0.0_EB
      IF (GAXIS(3).GT.MAX_VAL) GAXIS(3) = 0.0_EB
      AZIM = 0.0_EB
      ELEV = 0.0_EB
      AZIM_DOT = 0.0_EB
      ELEV_DOT = 0.0_EB
      
      IF (GROTATE.GT.MAX_VAL) GROTATE = 0.0_EB
...
MAX_VERTS=MAX(MAX_VERTS,100000)
MAX_FACES=MAX(MAX_FACES,100000)
MAX_VOLUS=MAX(MAX_VOLUS,100000)
MAX_IDS=MAX(MAX_IDS,100000)
...
DO WHILE (I.LE.N_VERTS)
   ...
   DO WHILE (J.LE.N_VERTS)
    ...
    !     WHERE ( SPHERE_VOLUS(1:N_VOLUS)==N_VERTS ) SPHERE_VOLUS(1:N_VOLUS)= I
         ...
         DO K = 1, 3*N_FACES
           IF(SPHERE_FACES(K).EQ.J)SPHERE_FACES(K)=I
         END DO
         DO K = 1, 4*N_VOLUS
           IF(SPHERE_VOLUS(K).EQ.J)SPHERE_VOLUS(K)=I
         END DO
         VJ(1:3)=SPHERE_VERTS(3*N_VERTS-2:3*N_VERTS)
         DO K = 1, 3*N_FACES
           IF(SPHERE_FACES(K).EQ.N_VERTS)SPHERE_FACES(K)=J
         END DO
         DO K = 1, 4*N_VOLUS
           IF(SPHERE_VOLUS(K).EQ.N_VERTS)SPHERE_VOLUS(K)=J
         END DO
         N_VERTS=N_VERTS-1
      ENDIF
      J=J+1
...
SUBROUTINE INIT_SPHERE(N_LEVELS,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,SPHERE_VERTS,SPHERE_FACES,SPHERE_TFACES)
...
DO I=0, 4
   TFACE=>SPHERE_TFACES(6*I+1:6*I+6)
...
DO I = 0, 4
   ...
   TFACE=>SPHERE_TFACES(30+12*I+7:30+12*I+12)
...
! adjust texture coordinates when a triangle crosses the "prime meridian"
   ...
   IF(TFACE(1).GT.1.0_EB-EPS_TEXTURE.AND.TFACE(3).LT.EPS_TEXTURE)THEN
      TFACE(3)=TFACE(3)+1.0_EB
   ENDIF
   IF(TFACE(1).GT.1.0_EB-EPS_TEXTURE.AND.TFACE(5).LT.EPS_TEXTURE)THEN
      TFACE(5)=TFACE(5)+1.0_EB
   ENDIF
   
   IF(TFACE(3).GT.1.0_EB-EPS_TEXTURE.AND.TFACE(1).LT.EPS_TEXTURE)THEN
      TFACE(1)=TFACE(1)+1.0_EB
   ENDIF
   IF(TFACE(3).GT.1.0_EB-EPS_TEXTURE.AND.TFACE(5).LT.EPS_TEXTURE)THEN
      TFACE(5)=TFACE(5)+1.0_EB
   ENDIF
   
   IF(TFACE(5).GT.1.0_EB-EPS_TEXTURE.AND.TFACE(1).LT.EPS_TEXTURE)THEN
      TFACE(1)=TFACE(1)+1.0_EB
   ENDIF
   IF(TFACE(5).GT.1.0_EB-EPS_TEXTURE.AND.TFACE(3).LT.EPS_TEXTURE)THEN
      TFACE(3)=TFACE(3)+1.0_EB
   ENDIF
   
   ! make adjustments when face is at a pole
   
   IF(ABS(TFACE(2)-1.0_EB).LT.0.001_EB)THEN
      TFACE(1) = (TFACE(3)+TFACE(5))/2.0_EB
   ENDIF
   IF(ABS(TFACE(4)-1.0_EB).LT.0.001_EB)THEN
      TFACE(3) = (TFACE(1)+TFACE(5))/2.0_EB
   ENDIF
   IF(ABS(TFACE(6)-1.0_EB).LT.0.001_EB)THEN
      TFACE(5) = (TFACE(1)+TFACE(3))/2.0_EB
   ENDIF
   
   IF(ABS(TFACE(2)).LT.0.001_EB)THEN
      TFACE(1) = (TFACE(3)+TFACE(5))/2.0_EB
   ENDIF
   IF(ABS(TFACE(4)).LT.0.001_EB)THEN
      TFACE(3) = (TFACE(1)+TFACE(5))/2.0_EB
   ENDIF
   IF(ABS(TFACE(6)).LT.0.001_EB)THEN
      TFACE(5) = (TFACE(1)+TFACE(3))/2.0_EB
   ENDIF
...
SUBROUTINE INIT_SPHERE2(N_VERTS, N_FACES, NLAT,NLONG,SPHERE_VERTS,SPHERE_FACES,SPHERE_TFACES)
...
REAL(EB), INTENT(OUT), TARGET, DIMENSION(6*(NLAT-1)*NLONG*2*2) :: SPHERE_TFACES
INTEGER, INTENT(OUT) :: N_VERTS, N_FACES
REAL(EB) :: LAT, LONG
INTEGER :: ILONG, ILAT
...
DO ILONG = 1, NLONG
   ...
   I12 = I11 + 1
   I22 = NLONG*(NLAT-2)+2
   IF(ILONG.EQ.NLONG)I12=2+NLONG*(NLAT-3)
...
DO IFACE=NLONG, N_FACES-NLONG-1
   ...
   IF(MOD(IFACE-NLONG,2*NLONG).EQ.0)THEN
      TFACE(1)=0.0_EB
      TFACE(3)=0.0_EB
   ENDIF
   IF(MOD(IFACE-NLONG,2*NLONG).EQ.1) TFACE(3)=0.0_EB
   
   IF(MOD(IFACE-NLONG,2*NLONG).EQ.2*NLONG-2) TFACE(5)=1.0_EB
   IF(MOD(IFACE-NLONG,2*NLONG).EQ.2*NLONG-1)THEN
      TFACE(1)=1.0_EB
      TFACE(5)=1.0_EB
   ENDIF
END DO
...
DO IFACE=N_FACES-NLONG, N_FACES-1
   ...
   IF (IFACE.EQ.N_FACES-NLONG) TFACE(1)=0.0_EB
   TFACE(3)=REAL(IFACE-N_FACES+NLONG,EB)/REAL(NLONG-1,EB)
   IF (IFACE.EQ.N_FACES-1) TFACE(5)=1.0_EB
END DO
END SUBROUTINE INIT_SPHERE2

! ---------------------------- REFINE_FACE ----------------------------------------

RECURSIVE SUBROUTINE REFINE_FACE(N_LEVELS,IFACE,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,SPHERE_VERTS,SPHERE_FACES,SPHERE_TFACES)
USE MATH_FUNCTIONS, ONLY: NORM2
...
INTEGER, INTENT(IN) :: MAX_VERTS, MAX_FACES
REAL(EB), INTENT(INOUT), TARGET :: SPHERE_VERTS(3*MAX_VERTS), SPHERE_TFACES(6*MAX_FACES)
INTEGER, INTENT(INOUT), TARGET :: SPHERE_FACES(3*MAX_FACES)

INTEGER, POINTER, DIMENSION(:) :: FACE1, FACE2, FACE3, FACE4
REAL(EB), POINTER, DIMENSION(:) :: TFACE1, TFACE2, TFACE3, TFACE4
REAL(EB), POINTER, DIMENSION(:) :: V1, V2, V3
REAL(EB), POINTER, DIMENSION(:) :: V12, V13, V23
INTEGER :: N1, N2, N3, N4
REAL(EB), DIMENSION(2) :: TF12, TF13, TF23

IF(N_LEVELS.EQ.0.OR.N_FACES+3.GT.MAX_FACES.OR.N_VERTS+3.GT.MAX_VERTS)RETURN ! prevent memory overwrites
...
TFACE1(1:6)=>SPHERE_TFACES(6*IFACE-5:6*IFACE) ! original tface and 1st new tface
TFACE2(1:6)=>SPHERE_TFACES(6*N_FACES+1:6*N_FACES+6) ! 2nd new tface
TFACE3(1:6)=>SPHERE_TFACES(6*N_FACES+7:6*N_FACES+12) ! 3rd new tface
TFACE4(1:6)=>SPHERE_TFACES(6*N_FACES+13:6*N_FACES+18) ! 4th new tface
...
CALL TEXTURE_AVERAGE(TFACE1(1:2),TFACE1(3:4),TF12(1:2))
CALL TEXTURE_AVERAGE(TFACE1(1:2),TFACE1(5:6),TF13(1:2))
CALL TEXTURE_AVERAGE(TFACE1(3:4),TFACE1(5:6),TF23(1:2))

IF(ABS(TFACE1(2)-1.0_EB).LT.10.0_EB*TWO_EPSILON_EB)THEN
   TF12(1)=TFACE1(3)
   TF13(1)=TFACE1(5)
ENDIF
IF(ABS(TFACE1(2)).LT.10.0_EB*TWO_EPSILON_EB)THEN
   TF13(1)=TFACE1(5)
   TF12(1)=TFACE1(3)
ENDIF

TFACE2(1:2) = TF12
TFACE2(3:4) = TFACE1(3:4)
TFACE2(5:6) = TF23
...
TFACE4(1:2) = TF13
TFACE4(3:4) = TF23
TFACE4(5:6) = TFACE1(5:6)

TFACE1(3:4) = TF12
TFACE1(5:6) = TF13
TFACE1(1:2) = TFACE1(1:2)
...
N_FACES = N_FACES + 3
N_VERTS = N_VERTS + 3
IF (N_LEVELS.EQ.1) RETURN  ! stop recursion

CALL REFINE_FACE(N_LEVELS-1,N1,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,SPHERE_VERTS,SPHERE_FACES,SPHERE_TFACES)
CALL REFINE_FACE(N_LEVELS-1,N2,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,SPHERE_VERTS,SPHERE_FACES,SPHERE_TFACES)
CALL REFINE_FACE(N_LEVELS-1,N3,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,SPHERE_VERTS,SPHERE_FACES,SPHERE_TFACES)
CALL REFINE_FACE(N_LEVELS-1,N4,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,SPHERE_VERTS,SPHERE_FACES,SPHERE_TFACES)
...
ANGLES = (/ATAN2(XYZ(2),XYZ(1)),ASIN(XYZ(3)/NORM2(XYZ))/)
...
SUBROUTINE TEXTURE_AVERAGE(TXY1, TXY2, TXY_AVG)

! compute mid point of two texture coordinates

REAL(EB), DIMENSION(2), INTENT(IN) :: TXY1, TXY2
REAL(EB), DIMENSION(2), INTENT(OUT) :: TXY_AVG
REAL(EB) :: ANGLES1(2), ANGLES2(2), ANGLES_AVG(2)
REAL(EB) :: XYZ1(3), XYZ2(3), XYZ_AVG(3)

! convert texture coordinates (0->1 ) to spherical angles

ANGLES1 = (/-PI+2.0_EB*TXY1(1)*PI,-PI/2.0_EB + PI*TXY1(2) /)
ANGLES2 = (/-PI+2.0_EB*TXY2(1)*PI,-PI/2.0_EB + PI*TXY2(2) /)

! convert spherical angles to cartesian coordinates

XYZ1 = (/ COS(ANGLES1(1))*COS(ANGLES1(2)), SIN(ANGLES1(1))*COS(ANGLES1(2)),SIN(ANGLES1(2))/)
XYZ2 = (/ COS(ANGLES2(1))*COS(ANGLES2(2)), SIN(ANGLES2(1))*COS(ANGLES2(2)),SIN(ANGLES2(2))/)

! average cartesian coordiantes

XYZ_AVG = (XYZ1+XYZ2)/2.0_EB

! convert back to spherical angles

ANGLES_AVG=(/ATAN2(XYZ_AVG(2),XYZ_AVG(1)),ASIN(XYZ_AVG(3))/)

!convert back to texture coordinates

TXY_AVG = (/ 0.5_EB + 0.5_EB*ANGLES_AVG(1)/PI,0.5_EB + ANGLES_AVG(2)/PI /)

END SUBROUTINE TEXTURE_AVERAGE
...
SUBROUTINE MERGE_GEOMS(HAS_SURF,HAS_MATL,HAS_TEXTURE,&
...
INTEGER, INTENT(OUT) :: HAS_SURF,HAS_MATL,HAS_TEXTURE
...
DO I = 0, N_GEOMETRY
   ...
   IF(G%HAS_SURF.EQ.1)HAS_SURF=1
   IF(G%HAS_TEXTURE.EQ.1)HAS_TEXTURE=1
   IF(G%HAS_MATL.EQ.1)HAS_MATL=1
...
INTEGER :: IVERT, IFACE, IVOLUS, J, NSUB_VERTS, NSUB_FACES, NSUB_VOLUS, MATL_INDEX, SURF_INDEX
...
IF (G%N_VERTS_BASE.EQ.0.OR.(G%N_FACES_BASE.EQ.0.AND.G%N_VOLUS_BASE.EQ.0)) RETURN ! nothing to do if GEOM_IDS geometries are empty
...
DO J = 1, G%NSUB_GEOMS
   ...
   IF (NSUB_VERTS.EQ.0.OR.(NSUB_FACES.EQ.0.AND.NSUB_VOLUS.EQ.0)) CYCLE
...
G%N_FACES = IFACE
G%N_VOLUS = IVOLUS
IF (G%HAS_SURF.EQ.1.AND.IFACE>0) THEN
   SURF_INDEX = GET_SURF_INDEX(G%SURF_ID)
   G%SURFS(1:G%N_FACES) = SURF_INDEX
ENDIF
IF (G%HAS_MATL.EQ.1.AND.IVOLUS>0) THEN
   MATL_INDEX = GET_MATL_INDEX(G%MATL_ID)
   G%MATLS(1:G%N_VOLUS) = MATL_INDEX
ENDIF
...
SUBROUTINE CONVERTGEOM(IS_DYNAMIC,TIME)
   ...
   INTEGER :: HAS_SURF, HAS_MATL, HAS_TEXTURE
   ...
   IF (N_VERTS.GT.0.AND.(N_FACES.GT.0.OR.N_VOLUS.GT.0)) THEN
...
SUBROUTINE OUTGEOM(LUNIT,IS_DYNAMIC,TIME)
   ...
   REAL(EB), ALLOCATABLE, DIMENSION(:) :: VERTS, TFACES
   INTEGER :: IZERO
   INTEGER :: HAS_MATL, HAS_SURF, HAS_TEXTURE
   ...
   IF (N_VERTS.GT.0.AND.(N_FACES.GT.0.OR.N_VOLUS.GT.0)) THEN
      CALL MERGE_GEOMS(HAS_SURF,HAS_MATL,HAS_TEXTURE,VERTS,N_VERTS,&
          FACES,TFACES,SURF_IDS,N_FACES,VOLUS,MATL_IDS,N_VOLUS,IS_DYNAMIC)
   ELSE
      HAS_SURF=0
      HAS_MATL=0
      HAS_TEXTURE=0
   ...
   WRITE(LUNIT) HAS_SURF, HAS_MATL, HAS_TEXTURE
   IF (N_VERTS>0) WRITE(LUNIT) (REAL(VERTS(I),FB), I=1,3*N_VERTS)
   IF (N_FACES>0) THEN
      WRITE(LUNIT) (FACES(I), I=1,3*N_FACES)
      IF(HAS_SURF.EQ.1)WRITE(LUNIT) (SURF_IDS(I), I=1,N_FACES)
      IF(HAS_TEXTURE.EQ.1)WRITE(LUNIT) (REAL(TFACES(I),FB), I=1,6*N_FACES)
   ENDIF
   IF (N_VOLUS.GT.0) THEN
      WRITE(LUNIT) (VOLUS(I), I=1,4*N_VOLUS)
      IF(HAS_MATL.EQ.1)WRITE(LUNIT) (MATL_IDS(I), I=1,N_VOLUS)
   ENDIF
