...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (.NOT. RN%SIMPLE_CHEMISTRY) THEN
      IF (RN%FUEL=='null') THEN
         ...
         IF (RN%FUEL_SMIX_INDEX < 0) THEN
            WRITE(MESSAGE,'(A,I0,3A)') 'ERROR(XXX): REAC ',NR,' Has no consumed species.'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
      ELSE
   ...
   DO NS=1,N_TRACKED_SPECIES
      IF (ABS(RN%NU(NS)) > TWO_EPSILON_EB) WRITE(*,*) NR,NS,TRIM(SPECIES_MIXTURE(NS)%ID)," ",TRIM(SPECIES_MIXTURE(NS)%FORMULA),RN%NU(NS),SPECIES_MIXTURE(NS)%MW
      ...
      IF (ABS(RN%NU(NS)) > TWO_EPSILON_EB) WRITE(*,*) MASS_REACTANT,MASS_PRODUCT
   ENDDO
