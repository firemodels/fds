...
INTEGER :: NOM,SURF_INDEX,SLOTS,II
   ...
   MESH_LOOP_2: DO NOM=1,NMESHES
      ...
      DO SURF_INDEX=1,N_SURF
         ...
         IF (SLOTS>0) THEN
            ...
            DO II=1,M3%N_WALL_CELLS_SEND
               CALL ALLOCATE_STORAGE(NOM,WALL_INDEX=II,SURF_INDEX=SURF_INDEX)
            ENDDO
         ENDIF
...
SENDING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (CODE==6 .AND. RNODE/=SNODE .AND. M3%N_WALL_CELLS_SEND>0) THEN
         DO SURF_INDEX=1,N_SURF
            DO II=1,M3%N_WALL_CELLS_SEND
               IF (M3%WALL_SURF_INDICES_SEND(II)/=SURF_INDEX) CYCLE
               IW = M3%WALL_CELL_INDICES_SEND(II)
...
RECV_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      RECEIVE_BACK_WALL: IF (CODE==6 .AND. SNODE/=RNODE) THEN
         DO SURF_INDEX=1,N_SURF
            DO II=1,M2%N_WALL_CELLS_RECV
               IF (M2%WALL_SURF_INDICES_RECV(II)/=SURF_INDEX) CYCLE
               CALL PACK_WALL(NM,II,SURF_INDEX,II,UNPACK_IT=.TRUE.,NOM=NOM)
            ENDDO
         ENDDO
