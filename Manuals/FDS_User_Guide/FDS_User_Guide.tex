...
For Linux and Mac OS targets, and non overlapping meshes at same refinement level, the parameter {\ct GLMAT\_SOLVER} set to {\ct .TRUE.} allows for the solution of the Poisson equation from the global discretization matrix. The solver used in this mode is Intel\textsuperscript{\textregistered} MKL Pardiso for serial targets, and Intel\textsuperscript{\textregistered} MKL Sparse Cluster Solver for MPI targets. Two solution modes are defined by the logical parameter {\ct PRES\_ON\_WHOLE\_DOMAIN}: if {\ct .TRUE.} (default) the $H$ equation is solved in cells covered by obstacles as well as gas phase cells in the spirit of FDS default solver. If {\ct .FALSE.} the pressure is solved only on gas cells, employing solid boundary conditions for pressure in immersed obstacles. This method is more computationally and memory intensive than the default solver with default options, but eliminates velocity penetration errors in obstacles as well as inter-mesh boundaries.
...
The chapter describes techniques for modeling the atmospheric boundary layer. Most of the parameters that describe the atmosphere are entered on a single namelist line called {\ct WIND}.
...
\paragraph{\ct 'RADIANCE'}  This is the term for the radiation intensity at the point, $\bx$, and direction angle, $\bs$, where the summation is over all spectral bands:
\be
...
\end{eqnarray*}
For the quantity {\ct 'NORMALIZED MASS'}, you can add a {\ct MATL\_ID} to get the normalized mass of a single component of the solid material, $m''_\alpha (t) / \sum_\alpha  m''_\alpha (0)$.
...
{\ct MASS FLUX}$^1$                             & Mass flux at solid surface                    & \si{kg/(m$^2$.s)} & B,D       \\ \hline
...
\noindent
\begin{tabbing}
$^1$  \hspace{0.05in} \= Quantity requires the specification of a gas species using {\ct SPEC\_ID}. \\
                      \> Do not use for {\ct MIXTURE FRACTION}. \\
$^2$                  \> Quantity requires the specification of a particle name using {\ct PART\_ID}. \\
...
$^4$                  \> Quantify allows for an optional {\ct MATL\_ID}.
...
{\ct NORMALIZED MASS LOSS RATE}$^1$             & Section~\ref{info:material_components}            & 1/s            & D            \\ \hline
...
$^1$ \hspace{0.05in} \= Quantity requires the specification of a gas species using {\ct SPEC\_ID}. \\
$^2$                 \> Quantity requires the specification of a particle name using {\ct PART\_ID}. \\
$^3$                 \> Quantity requires specification of an additional scalar using {\ct QUANTITY2}. \\
$^4$                 \> Quantity allows for an optional {\ct MATL\_ID}.
\end{tabbing}
...
{\ct GLMAT\_SOLVER}                 & Logical       & Section~\ref{info:PRES}           &               & {\ct .FALSE.}             \\ \hline
{\ct PRES\_ON\_WHOLE\_DOMAIN}       & Logical       & Section~\ref{info:PRES}           &               & {\ct .TRUE.}              \\ \hline
\end{longtable}
...
{\ct DT\_MEAN\_FORCING}                         & Real          & Section~\ref{info:WIND}                       & s             & 1.E10             \\ \hline
...
{\ct MEAN\_FORCING(3)}                          & Logical       & Section~\ref{info:WIND}                       &               & {\ct .FALSE.}     \\ \hline
...
%{\ct SPONGE\_LAYER\_DISTANCE}                   & Real          & Section~\ref{info:WIND}                       &               & 0.                \\ \hline
