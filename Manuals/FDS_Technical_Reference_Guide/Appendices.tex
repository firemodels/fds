...
\end{eqnarray}
\vspace{0.3cm}\newpage
\noindent The temperature equation then becomes
\begin{eqnarray}
\label{eqn_DTDt}
...
Using the requirements that mass fractions sum to unity and that diffusive fluxes sum to zero, for computatinal convenience we make the following rearrangements of the summation terms:
\begin{eqnarray}
\label{eqn_hdiff_1}
...
&=& \sum_{n=1}^{N_z} \left[ \overline{W}(1/W_n - 1/W_0) - \frac{(h_{s,n}-h_{s,0})}{c_p T} \right]  \left(\dot{m}_n^\tripleprime - \Div\mathbf{J}_n \right)
\end{eqnarray}
\vspace{0.3cm}
