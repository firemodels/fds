...
At the interface of two abutting meshes, the following (Dirichlet) boundary condition is applied (in this example, the interface $x=x_{\max}$):
\be \cH_{I+\ha,j,k,m} = \frac{ \cH_{I,j,k,m} + \cH_{1,jo,ko,mo} }{2}  \ee
Note the additional subscripts $m$ and $mo$ that indicate the mesh for which the Poisson equation is to be solved, and the ``other'' or ``opposing'' mesh, respectively. In a
parallel calculation, the values of $\cH$ in both meshes is taken from the previous Poisson solve, half a time step in the past.


\subsubsection{Multiple Mesh Considerations (On-Going Research)}

The procedure for obtaining the pressure that was described in the previous section is fairly stable, and provides a reasonable exchange of momentum mesh to mesh.
However, there is no guarantee that the individual solutions
of the Poisson equation on each mesh conserve volume. That is, $\dot{V}=\int \bu \cdot \dS$ is not necessarily the same at the interfaces between meshes.

The strategy for solving the Poisson equation on multiple meshes is to decompose the pressure term such that the velocity is
updated according to:
\be \dod{\bu}{t} + \bF + \nabla \cH^0 + \nabla \cH' = 0 \label{finepois} \ee
such that
\be \nabla^2 {\cal H}^0 = -\nabla \cdot \bF - \dod{}{t} (\nabla \cdot \bu) \quad ; \quad  \nabla^2 \cH' = 0  \ee
The equation on the left is the same as that described above, with the same boundary conditions, as well. However, $\nabla \cH^0$ only enforces continuity of pressure
across the mesh boundaries. We need the additional perturbation term, $\nabla \cH'$, to enforce volume conservation at mesh boundaries.
