...
ALLOCATE(M%MIX_TIME(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO) ; CALL ChkMemErr('INIT','MIX_TIME',IZERO) ; M%MIX_TIME=0._EB
...
CHARACTER(LABEL_LENGTH) :: PRES_SOLVER
TYPE (MESH_TYPE), POINTER :: M
TYPE (WALL_TYPE), POINTER :: WC
TYPE (BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE (BOUNDARY_PROP1_TYPE), POINTER :: B1
...
IF(CC_IBM .AND. PERIODIC_TEST/=7) THEN
   PRES_SOLVER=PRES_METHOD
   ! Reset solver to specific solver if library is specified:
   IF(PRES_FLAG==ULMAT_FLAG  .AND. ULMAT_SOLVER_LIBRARY ==HYPRE_FLAG)        PRES_SOLVER='ULMAT HYPRE'
   IF(PRES_FLAG==ULMAT_FLAG  .AND. ULMAT_SOLVER_LIBRARY ==MKL_PARDISO_FLAG)  PRES_SOLVER='ULMAT PARDISO'
   IF(PRES_FLAG==UGLMAT_FLAG .AND. UGLMAT_SOLVER_LIBRARY==HYPRE_FLAG)        PRES_SOLVER='UGLMAT HYPRE'
   IF(PRES_FLAG==UGLMAT_FLAG .AND. UGLMAT_SOLVER_LIBRARY==MKL_CPARDISO_FLAG) PRES_SOLVER='UGLMAT PARDISO'
   ! Reset solver to default solver depending on N_ZONE if solver not specified:
   IF (PRES_FLAG/=ULMAT_FLAG .AND. PRES_FLAG/=UGLMAT_FLAG) THEN
      IF(N_ZONE>1) THEN
         PRES_SOLVER = 'ULMAT'
      ELSE
         PRES_SOLVER = 'UGLMAT'
      ENDIF
   ENDIF
   CALL DEFINE_PRES_METHOD(PRES_SOLVER)
ENDIF
...
INTEGER, INTENT(IN) :: NM
INTEGER :: I,IW,IW2,ITW,ITW2,NWP,NWP2,I2,IWA,DM,IOR,NOM,II,JJ,KK,NN,NNN,IC,NL,IW_FOUND,NN_FOUND
...
DO N=1,M%N_NEIGHBORING_MESHES
   ...
   IF (NM==NOM) CYCLE
   COMPLETELY_SOLID = .TRUE.
   DO IC=1,CELL_COUNT(NM)
      IF (.NOT.M%CELL(IC)%SOLID) COMPLETELY_SOLID = .FALSE.
   ENDDO
   IF (COMPLETELY_SOLID) CYCLE
   M4 => MESHES(NOM)
   IF ((M%XS>=M4%XF-TWENTY_EPSILON_EB .OR. M%XF-TWENTY_EPSILON_EB<=M4%XS) .AND. &
       (M%YS>=M4%YF-TWENTY_EPSILON_EB .OR. M%YF-TWENTY_EPSILON_EB<=M4%YS) .AND. &
       (M%ZS>=M4%ZF-TWENTY_EPSILON_EB .OR. M%ZF-TWENTY_EPSILON_EB<=M4%ZS)) CYCLE
   OS => M%OMESH(NOM)%WALL_RECV_BUFFER
   DO IC=1,CELL_COUNT(NOM)
      ...
      DO IOR=-3,3
         ...
         IF (M%XS>=M4%XF-TWENTY_EPSILON_EB .AND. IOR==-1) CYCLE
         IF (M%XF-TWENTY_EPSILON_EB<=M4%XS .AND. IOR== 1) CYCLE
         IF (M%YS>=M4%YF-TWENTY_EPSILON_EB .AND. IOR==-2) CYCLE
         IF (M%YF-TWENTY_EPSILON_EB<=M4%YS .AND. IOR== 2) CYCLE
         IF (M%ZS>=M4%ZF-TWENTY_EPSILON_EB .AND. IOR==-3) CYCLE
         IF (M%ZF-TWENTY_EPSILON_EB<=M4%ZS .AND. IOR== 3) CYCLE
   ...
   IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN
      ...
      IF (OBST_INDEX/=OBST_INDEX_PREVIOUS .AND. OBST_INDEX_PREVIOUS>0 .AND. OBST_INDEX>0) THEN
         ...
         IF (OB%ORDINAL/=OB_PREV%ORDINAL) THEN
            IF ((OB%REMOVABLE .OR. OB_PREV%REMOVABLE) .AND. &
                (OB%DEVC_INDEX/=OB_PREV%DEVC_INDEX .OR. OB%CTRL_INDEX/=OB_PREV%CTRL_INDEX)) THEN
               WRITE(MESSAGE,'(3A)') 'ERROR(380): OBST ',TRIM(OB%ID),' is VARIABLE_THICKNESS or HT3D and cannot be removed.'
               CALL SHUTDOWN(MESSAGE,PROCESS_0_ONLY=.FALSE.) ; RETURN
            ENDIF
