...
DO NOM=1,NMESHES
   IF (M%OMESH(NOM)%N_WALL_CELLS_RECV==0) CYCLE
   ALLOCATE(M%OMESH(NOM)%WALL_CELL_INDICES_RECV(M%OMESH(NOM)%N_WALL_CELLS_RECV))
   ALLOCATE(M%OMESH(NOM)%WALL_SURF_INDICES_RECV(M%OMESH(NOM)%N_WALL_CELLS_RECV))
ENDDO
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (WC%BACK_MESH/=NM .AND. PROCESS(WC%BACK_MESH)/=MY_RANK) THEN
      IW_EXPORT(WC%BACK_MESH) = IW_EXPORT(WC%BACK_MESH) + 1
      M%OMESH(WC%BACK_MESH)%WALL_CELL_INDICES_RECV(IW_EXPORT(WC%BACK_MESH)) = WC%BACK_INDEX
      M%OMESH(WC%BACK_MESH)%WALL_SURF_INDICES_RECV(IW_EXPORT(WC%BACK_MESH)) = WC%SURF_INDEX
...
! Special surfaces that have a variable thickness need to be reintialized

CALL INITIALIZE_VARIABLE_THICKNESS_WALL_CELLS
...
SUBROUTINE INITIALIZE_VARIABLE_THICKNESS_WALL_CELLS

USE GEOMETRY_FUNCTIONS, ONLY: GET_N_LAYER_CELLS,GET_WALL_NODE_COORDINATES
INTEGER :: IW2,NWP,NWP2,I2,IWA,DM,NN,BACK_INDEX
...
REAL(EB) :: X1,X2,Y1,Y2,Z1,Z2,XX1,XX2,YY1,YY2,ZZ1,ZZ2,PRIMARY_VOLUME,OVERLAP_VOLUME,DXX,DYY,DZZ,DUM,WEIGHT_FACTOR,EXTRUDE,&
            SUM_WGT(3)
TYPE(WALL_TYPE), POINTER :: WC2,WCB
