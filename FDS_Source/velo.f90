...
EXTERNAL_WALL_LOOP: DO IW=1,NEWC
 ...
 ! ENDIF
   ...
   END SELECT
   IF (INFLOW) THEN
 !!   RHMK(II,JJ,KK) = 2._EB*P_EXTERNAL - RHMK(IIG,JJG,KKG)
      RHMK(II,JJ,KK) = RHOP(II,JJ,KK)*(2._EB*P_EXTERNAL/RHO_F(IW) - RHMK(IIG,JJG,KKG)/RHOP(IIG,JJG,KKG) &
                                       - KRES(II,JJ,KK) - KRES(IIG,JJG,KKG))
   ELSE
 !!   RHMK(II,JJ,KK) = RHOP(II,JJ,KK)*(2._EB*P_EXTERNAL/RHO_F(IW) - RHMK(IIG,JJG,KKG)/RHOP(IIG,JJG,KKG))
 !!   RHMK(II,JJ,KK) = RHOP(II,JJ,KK)*(2._EB*P_EXTERNAL/RHO_F(IW) - RHMK(IIG,JJG,KKG)/RHOP(IIG,JJG,KKG) &
 !!                                    - KRES(II,JJ,KK) + KRES(IIG,JJG,KKG))
      RHMK(II,JJ,KK) = -RHMK(IIG,JJG,KKG)  ! No baroclinic correction for outflow
   ENDIF
ENDDO EXTERNAL_WALL_LOOP
