...
same amount of each product species should be produced in each case. The following two figures compare the
mass of the three combustion products. The first figure compares the simple chemistry versus the primitive species and the second figure compares the 
simple chemistry versus the lumped species.
\begin{figure}[h]
\begin{tabular*}{\textwidth}{lr}
...
\end{tabular*}
\caption[Yield of combustion products using three alternative definitions of the same reaction.]{The combustion product masses using three alternative definitions of the reaction. (Left) Simple chemistry vs. primitive species. (Right) Simple chemistry vs. lumped species. }
...
This section describes a series of test cases designed to check the reaction rate mechanism for mixing-controlled, finite-rate, and mixed-mode reaction schemes.

\subsection{Mixing-controlled reactions (ReactionRate\_MixCtrl\_)}

The mixing-controlled (mixed is burned) reaction model with local extinction is the default reaction mechanism in FDS. The reaction rate is determined by the eddy dissipation concept (EDC). The EDC assumes that the consumption rate of fuel is determined from the local fuel, oxidizer, and product mass fractions and the turbulent mixing time by the following equation \cite{Poinsot:TNC}:

\begin{equation}\label{eq:EDC}
k = \frac{C}{\tau_{mix}} \, \min \left( Y_F,\frac{Y_\OTWO}{s},\beta\frac{Y_P}{(1+s)} \right)
\end{equation}
where $C$ and $\beta$ are model constants and $s$ is determined from the stoichiometry of the reaction. This model can be considered a deficient model as the consumption rate is found by the limiting species. To determine species evolution for the reaction, a system of ordinary differential equations is solved for each participating species:
\begin{equation}\label{eq:spec_ode}
\frac{dY_{i}}{dt}= \frac{\nu_i \, W_i}{W_F} \, k
\end{equation}
To test this implementation, a one-step methane reaction
\begin{equation}\label{eq:1step_methane}
\mathrm{CH_4 + 2 \, O_2 \rightarrow  CO_2 + 2\, H_2O}
\end{equation}
is considered for both a fuel-limiting and oxygen-limiting case with a fixed mixing time, $\tau_{mix}=0.1$~s.
...
Similar to the one-step reaction, the EDC reaction implementation is tested for a two-step methane reaction:

\begin{eqnarray}\label{eq:2step_methane}
\mathrm{CH_4 + 1.5 \, O_2 \rightarrow  CO + 2 \, H_2O} \\
\nonumber \mathrm{CO + 0.5 \, O_2 \rightarrow CO_2}
\end{eqnarray}
for both fuel-limited and oxygen-limited conditions with a fixed mixing time, $\tau_{mix}=0.1$~s.
...
\end{tabular*}
\caption[Species Evolution in Mixing Controlled Reaction]{Time evolution of species mass fraction for a two-step mixing controlled reaction with a fixed mixing time of $\tau_{mix}=0.1$~s. (Left) Fuel-limited reaction. (Right) Oxygen-limited reaction.}
...
\end{equation}
where A, E, $a_{i}$, and $n_{i}$ are user defined inputs based on the specific reaction. Note that since Equation \ref{eq:FR_Arr} has an exponential term, the integration of Equation Set \ref{eq:spec_ode} is solved using a two-step Runge-Kutta scheme as compared to an explicit Euler scheme used in the mixing-controlled test case.

Here we consider two one-step cases, a zero-order and second-order reaction.  First, a one-step, zero-order test case of carbon monoxide oxidation:
...
\end{tabular*}
\caption[Species Evolution in Mixing Controlled Reaction]{Time evolution of species mass fraction for multi-step Arrhenius finite rate propane reactions. (Left) 2-step. (Right) 3-step.}
\label{fig:MixCtrl_2Step2}
\end{figure}
