...
made of steel and covered on the outside by an insulation material would have to be described with two {\ct SURF} lines like the following: 
...
\noindent
If, in addition, the insulation material is combustible, and its burning properties are specified on the appropriate {\ct MATL} line, then you need
to indicate which side of the column would generate the fuel vapor. In this case, the steel is impermeable; thus you should add the parameter
{\ct LAYER\_DIVIDE=1.0} to the {\ct SURF} line labelled {\ct 'COLUMN EXTERIOR'} to indicate that fuel vapors formed by the heating of the 
first layer ({\ct 'INSULATION'}) are to be driven out of that surface. You need to also specify {\ct LAYER\_DIVIDE=0.0} on the {\ct SURF}
line labelled {\ct 'COLUMN INTERIOR'} to indicate that no fuel vapors are to driven into the interior of the column.   
...
{\ct LAYER\_DIVIDE}                   & Real            & See Section~\ref{info:EXPOSED}    &                     & 0.5                     \\ \hline
