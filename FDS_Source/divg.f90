...
REAL(EB), POINTER, DIMENSION(:,:,:) :: KDTDX,KDTDY,KDTDZ,DP,KP,CP, &
          RHO_D,RHOP,RHOO,H_RHO_D_DZDX,H_RHO_D_DZDY,H_RHO_D_DZDZ,RTRM, &
          U_DOT_DEL_RHO_H_S,RHO_H_S_P,UU,VV,WW,RHO_Z_P,U_DOT_DEL_RHO_Z,RHO_D_TURB,R_H_G
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP,ZZO,RHO_D_DZDX,RHO_D_DZDY,RHO_D_DZDZ
REAL(EB), POINTER, DIMENSION(:,:) :: PBAR_P,PBAR_O
...
DO K=0,KBP1
   DO J=0,JBP1
      DO I=0,IBP1
         ZZ_GET(1:N_TRACKED_SPECIES) = ZZO(I,J,K,1:N_TRACKED_SPECIES)
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_LOC)
         TMP_LOC = PBAR_O(K,PRESSURE_ZONE(I,J,K))/(RSUM_LOC*RHOO(I,J,K))
         CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,TMP_LOC)
         RHO_H_S_P(I,J,K) = RHOO(I,J,K)*H_S
      ENDDO
   ENDDO
ENDDO
!$OMP END DO
...
DO K=0,KBP1
   DO J=0,JBP1
      DO I=0,IBP1
         RHO_Z_P(I,J,K) = RHOO(I,J,K)*ZZO(I,J,K,N)
      ENDDO
   ENDDO
ENDDO
!$OMP END DO
