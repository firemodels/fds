...
The qualitative form of the solution (\ref{eqn_1dsoln}) is illustrated by Figure \ref{fig_soln1d},
where we show the solution at specific points in time corresponding to the initial condition, the peak of the first compression cycle,
and the trough of the first compression cycle.  The initial condition for the density is specified as $\rho(x,0) = 1$ and the amplitude and frequency are set to unity, $B=1$ and $\omega=1$.

\begin{figure}[ht]
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/pulsating_peak_trough.pdf}
   \caption{(Left) Analytical solution for the 1D continuity equation at various points in time. The
domain is periodic on an interval of 2\pi.}
   \label{fig_pulsating_2D_wave_peak_trough}
\end{figure}
...
An example of the solution to (\ref{eqn_2dsoln}) is shown in Figure \ref{fig_pulsating_2D_wave_pi} and Figure \ref{fig_pulsating 2D wave}. The analytical time series of the density at positions $(x,y) = (\pi,\pi)$ and $(x,y) = (3\pi/2, 3\pi/2)$ are shown with black lines. There are two sets of graphs for these two points in the domain to provide more evidence in support of the pulsating 2D continuity equation's second-order convergence. FDS is run with the CHARM flux limiter scheme (as this is a DNS flow field). The solution at successively finer grid resolutions is plotted and compared with the analytical solution, demonstrating convergence of the scheme. On the right side of the figures, we demonstrate second-order convergence of the Surperbee and CHARM limiter schemes in FDS. The initial condition for the density is specified as $\rho(x,0) = 1$ and the amplitude and frequency are set to unity, $B=1$ and $\omega=1$.

\begin{figure}[ht]
   \begin{tabular}{rl}
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/pulsating_time_series_pi.pdf} &
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/pulsating_convergence_pi.pdf}
   \end{tabular}
   \caption{(Left) Time series of $\rho$ at the position $(x,y) = (\pi, \pi)$ for several grid resolutions using the CHARM flux limiter. (Right) Convergence plot for Superbee and CHARM, the scalar transport schemes in FDS.  All schemes are second-order accurate.}
   \label{fig_pulsating_2D_wave_pi}
\end{figure}
...
\begin{figure}[ht]
   ...
   \end{tabular}
         \caption{(Left) Time series of $\rho$ at the position $(x,y) = (3\pi/2, 3\pi/2)$ for several grid resolutions using the CHARM flux limiter. (Right) Convergence plot for Superbee and CHARM, the scalar transport schemes in FDS.  All schemes are second-order accurate.}
