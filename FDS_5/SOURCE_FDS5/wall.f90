...
REAL(EB) T,YY_WALL,YY_G,DENOM,YY_OTHER(MAX_SPECIES),YY_EXTERIOR(MAX_SPECIES),RHO_G,UN,DD,EPSB,MFT,TSI
INTEGER IBC,IIG,JJG,KKG,IOR,IWB,IW, II, JJ, KK, N
TYPE (SURFACE_TYPE), POINTER :: SF
TYPE (VENTS_TYPE), POINTER :: VT
...
! Determine ghost cell value of RSUM=R0*Sum(Y_i/M_i) for mixture fraction case
   ...
   IF (MIXTURE_FRACTION) THEN
      ...
      DO N=1,N_SPECIES
         ...
         ENDIF
      ENDDO
      CALL GET_MOLECULAR_WEIGHT2(YY_W(IW,I_FUEL),Z_2,YY_W(IW,I_PROG_F),Y_SUM_W,RSUM_W(IW))
...
REAL(EB) :: DTMP,QNETF,QDXKF,QDXKB,RR,TMP_G,T,RFACF,RFACB,RFACF2,RFACB2, &
            ...
            HVRG,Z_2,Y_MF_G,Y_MF_W,YY_S,Y_SUM_W, RSUM_W, RSUM_G, RSUM_S, MFLUX, MFLUX_W, VOLSUM, &
            DXF, DXB,HTCB,Q_WATER_F,Q_WATER_B,TMP_F_OLD, DX_GRID, RHO_S0, H_R,DT2_BC,TOLERANCE,C_S_ADJUST_UNITS
INTEGER :: IBC,IIG,JJG,KKG,IIB,JJB,KKB,IWB,NWP,I,J,ITMP,NR,NNN,NL,II,JJ,KK,IW,IOR,N,I_OBST
...
WALL_CELL_LOOP: DO IW=1,NWC
   ...
   MFLUX = MAX(0._EB,MFLUX - ACTUAL_BURN_RATE(IW))
   ...
   MATERIAL_LOOP2: DO N=1,SF%N_MATL
      ...
      ELSEIF (ML%NU_WATER(1)>0._EB) THEN
         IF (I_WATER>0) MFLUX_W = MFLUX_W/ML%NU_WATER(1)
      ENDIF
      ! gas phase 
      ...
      ELSE
         Z_2 = 0._EB
      ENDIF
      CALL GET_MOLECULAR_WEIGHT2(Y_MF_G,Z_2,YY(IIG,JJG,KKG,I_PROG_F),  Y_SUM(IIG,JJG,KKG),RSUM_G)
      ! wall values
      ...
      IF (PPSURF/=PPCLAUS) THEN
         IF (MFLUX==0.AND.PPSURF<PPCLAUS) THEN
            MFLUX = 2.E-5*REACTION(1)%MW_FUEL/ML%ADJUST_BURN_RATE
            TW(IW)=T
         ELSE
