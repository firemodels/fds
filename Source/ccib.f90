...
! =10 Do not Split F in STRESS and IBM forces. IF CC_UNSTRUCTURED_PROJECTION=T, use only Stress Method F computed
!     using cut-face velocities. For Cartesian projection use cartesian face velocities.
...
INTEGER, PARAMETER ::         CC_STM_TYPE = 10
...
PUBLIC :: ADD_INPLACE_NNZ_H_WHLDOM,CALL_FOR_GLMAT,CALL_FROM_GLMAT_SETUP,CCCOMPUTE_RADIATION,&
          ...
          CCREGION_CUTFACE_VELOCITY,ADD_Q_DOT_CUTCELLS,CFACE_THERMAL_GASVARS,&
          CFACE_PREDICT_NORMAL_VELOCITY,CHECK_SPEC_TRANSPORT_CONSERVE,&
          COPY_CC_UNKH_TO_HS, COPY_CC_HS_TO_UNKH, &
          GET_CFACE_OPEN_BC_COEF,GET_FN_DIVERGENCE_CUTCELL,&
...
SUBROUTINE CCIBM_BAROCLINIC_CORRECTION(T,NM)
...
! Local variables:
INTEGER :: ICF,JCF,ICC,JCC,NFACE,I,J,K,X1AXIS,II1,II2,JJ1,JJ2,KK1,KK2
REAL(EB):: PRFCT,WG1,WG2,IDX,X1F,RHO1,RHO2,P1,P2,RHOF,AT,IOR
REAL(EB), POINTER, DIMENSION(:,:,:) :: HP=>NULL()
REAL(EB) :: P_EXTERNAL,TSI,TIME_RAMP_FACTOR,UN
...
! Get Baroclinic torque terms for internal cut-faces:
WG1 = 0.5_EB; WG2 = 0.5_EB
CF_LOOP_1 : DO ICF=MESHES(NM)%N_BBCUTFACE_MESH+1,MESHES(NM)%N_CUTFACE_MESH
   IF (CUT_FACE(ICF)%STATUS/=IBM_GASPHASE) CYCLE CF_LOOP_1
   ...
   II2 = I; JJ2 = J; KK2 = K
   SELECT CASE(X1AXIS)
   CASE(IAXIS); IDX = 1/DXN(I); II2 = II2+1; AT = CUT_FACE(ICF)%ALPHA_CF * (DY(J)*DZ(K))
   CASE(JAXIS); IDX = 1/DYN(J); JJ2 = JJ2+1; AT = CUT_FACE(ICF)%ALPHA_CF * (DZ(K)*DX(I))
   CASE(KAXIS); IDX = 1/DZN(K); KK2 = KK2+1; AT = CUT_FACE(ICF)%ALPHA_CF * (DX(I)*DY(J))
   END SELECT
   DO JCF=1,NFACE
      IF(.NOT.GRADH_ON_CARTESIAN) THEN
         IDX=1._EB/(CUT_FACE(ICF)%XCENHIGH(X1AXIS,JCF)-CUT_FACE(ICF)%XCENLOW(X1AXIS,JCF))
         ! Interpolation weights:
         X1F = CUT_FACE(ICF)%XYZCEN(X1AXIS,JCF)
         WG1 = IDX*(CUT_FACE(ICF)%XCENHIGH(X1AXIS,JCF)-X1F)
         WG2 = IDX*(X1F-CUT_FACE(ICF)%XCENLOW(X1AXIS, JCF))
      ENDIF
      ! Cut-cell pressure on low side:
      ICC = CUT_FACE(ICF)%CELL_LIST(2,LOW_IND,JCF); JCC = CUT_FACE(ICF)%CELL_LIST(3,LOW_IND,JCF);
      ...
      ICC = CUT_FACE(ICF)%CELL_LIST(2,HIGH_IND,JCF); JCC = CUT_FACE(ICF)%CELL_LIST(3,HIGH_IND,JCF);
...
! External CFACEs : Set baroclinic term to zero at outflow boundaries and P_EXTERNAL at inflow boundaries

CF_LOOP_2 : DO ICF=1,MESHES(NM)%N_BBCUTFACE_MESH

   WC=>WALL(CUT_FACE(ICF)%IWC)
   IF(WC%BOUNDARY_TYPE/=OPEN_BOUNDARY) CYCLE CF_LOOP_2
   ...
   DO JCF=1,NFACE
      ...
      UN     = PRFCT*CUT_FACE(ICF)%VEL(JCF) + (1._EB-PRFCT)*CUT_FACE(ICF)%VELS(JCF)
      IF (UN*SIGN(1._EB,REAL(IOR,EB))>TWO_EPSILON_EB) THEN
         IF(IOR>0) P1 = P_EXTERNAL/WG1 - P2*WG2/WG1  ! Pressure at inflow boundary is P_EXTERNAL
         IF(IOR<0) P2 = P_EXTERNAL/WG2 - P1*WG1/WG2
      ELSE
         IF(IOR>0) P1 = - P2*WG2/WG1
         IF(IOR<0) P2 = - P1*WG1/WG2
      ENDIF
...
RETURN
END SUBROUTINE CCIBM_BAROCLINIC_CORRECTION
...
!    SELECT CASE(IND)
!    CASE(6,7,8) ! U,V,W.
!       IF(MESHES(NM)%VERTVAR(I,J,K,IBM_VGSC)==IBM_SOLID) THEN
...
! Note: Routine assumes POINT_TO_MESH has been called and the node I,J,K is s.t VERTVAR(I,J,K,IBM_VGSC)==IBM_SOLID.
INTEGER, INTENT(IN) :: NM,I,J,K
...
! For now set velocity in IBM_SOLID node to zero:
VAL  = 0._EB
...
! --------------------------- CCREGION_CUTFACE_VELOCITY ---------------------------

SUBROUTINE CCREGION_CUTFACE_VELOCITY(NM,UU,VV,WW,CF_VEL_FLAG)
...
END SUBROUTINE CCREGION_CUTFACE_VELOCITY
...
DO JCF=1,CUT_FACE(ICF)%NFACE
   IF(.NOT.GRADH_ON_CARTESIAN) IDX = 1._EB/(CUT_FACE(ICF)%XCENHIGH(X1AXIS,JCF)-CUT_FACE(ICF)%XCENLOW(X1AXIS,JCF))
...
WALL_CELL_LOOP_1: DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   IF (.NOT.PRES_ON_CARTESIAN    .AND. WC%CUT_FACE_INDEX>0) CYCLE
   ! Drop if whole mesh solution and boundary type is NULL.
   ...
   IF_DIRICHLET: IF (WC%PRESSURE_BC_INDEX==DIRICHLET) THEN
      ...
      ! Here case where SOLID and OPEN or interpolated are mixed on a boundary:
      IF( WC%BOUNDARY_TYPE==NULL_BOUNDARY .OR. WC%BOUNDARY_TYPE==SOLID_BOUNDARY) CYCLE
...
REAL(EB):: HN,TSI,VEL_EDDY,TIME_RAMP_FACTOR,P_EXTERNAL
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU,VV,WW,HP
TYPE (VENTS_TYPE), POINTER :: VT
TYPE (WALL_TYPE), POINTER :: WC
TYPE (CFACE_TYPE), POINTER :: CFA
TYPE (BOUNDARY_ONE_D_TYPE), POINTER :: CFA_ONE_D

! Only get CFACES pres bcs if unstructured cut-cell grid solve.
IF(PRES_ON_CARTESIAN) RETURN
...
CFACE_LOOP_1 : DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   EXT_CFACE_IF : IF (ICF<=N_EXTERNAL_CFACE_CELLS) THEN
      ...
      INTERPOLATED_ONLY:  IF(PRES_FLAG==ULMAT_FLAG .AND. CFA%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) THEN
         ! Interpolated values of H have already been computer in PRESSURE_SOLVER_RHS and placed in BXS, BXF, ...
         SELECT CASE(IOR)
         CASE( 1); CFA%PRES_BXN = BXS(J,K)
         CASE(-1); CFA%PRES_BXN = BXF(J,K)
         CASE( 2); CFA%PRES_BXN = BYS(I,K)
         CASE(-2); CFA%PRES_BXN = BYF(I,K)
         CASE( 3); CFA%PRES_BXN = BZS(I,J)
         CASE(-3); CFA%PRES_BXN = BZF(I,J)
         END SELECT
      ...
      OPEN_IF: IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
         ...
         CFA_ONE_D => BOUNDARY_ONE_D(CFA%OD_INDEX)
         SELECT CASE(IOR)
            CASE( 1)
...
PRED_CORR_IF : IF (PREDICTOR) THEN
   ...
   ICC_LOOP_1 : DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      ...
      IF( SOLID(CELL_INDEX(I,J,K)) ) CYCLE ICC_LOOP_1
...
ELSEIF (CORRECTOR) THEN PRED_CORR_IF
   ...
   ICC_LOOP_2 : DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      ...
      J      = CUT_CELL(ICC)%IJK(JAXIS)
      K      = CUT_CELL(ICC)%IJK(KAXIS)
      IF( SOLID(CELL_INDEX(I,J,K)) ) CYCLE ICC_LOOP_2
      IPZ    = PRESSURE_ZONE(I,J,K)
      NCELL  = CUT_CELL(ICC)%NCELL
      DIVVOL = 0._EB; DPCC   = 0._EB; VOL    = 0._EB
      DO JCC=1,NCELL
...
ELSE STORE_IF
   ...
   PRED_CORR_IF : IF (PREDICTOR) THEN
            ...
            DO J=1,JBAR
               DO I=0,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     ENDDO
                     US(I,J,K) = DOT_PRODUCT(CUT_FACE(ICF)%VELS(1:NFACE),CUT_FACE(ICF)%AREA(1:NFACE)) / &
                                 (DY(J)*DZ(K))
                  ENDIF
               ENDDO
            ENDDO
            ...
            DO J=0,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     ENDDO
                     VS(I,J,K) = DOT_PRODUCT(CUT_FACE(ICF)%VELS(1:NFACE),CUT_FACE(ICF)%AREA(1:NFACE)) / &
                                 (DX(I)*DZ(K))
                  ENDIF
               ENDDO
            ENDDO
            ...
            DO J=1,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     ENDDO
                     WS(I,J,K) = DOT_PRODUCT(CUT_FACE(ICF)%VELS(1:NFACE),CUT_FACE(ICF)%AREA(1:NFACE)) / &
                                 (DX(I)*DY(J))
                  ENDIF
               ENDDO
            ENDDO
      ...
      ELSE GRADH_ON_CARTESIAN_IF_1
         ...
         DO IFACE=1,MESHES(NM)%IBM_NRCFACE_H
            ...
            SELECT CASE(X1AXIS)
               CASE(IAXIS)
                  US(I,J,K)= U(I,J,K) - DT*( FVX(I,J,K) + IDX*(H(I+1,J,K)-H(I,J,K)) )
               CASE(JAXIS)
                  VS(I,J,K)= V(I,J,K) - DT*( FVY(I,J,K) + IDX*(H(I,J+1,K)-H(I,J,K)) )
               CASE(KAXIS)
                  WS(I,J,K)= W(I,J,K) - DT*( FVZ(I,J,K) + IDX*(H(I,J,K+1)-H(I,J,K)) )
            END SELECT
         ENDDO

      ENDIF GRADH_ON_CARTESIAN_IF_1

      IF(.NOT.PRES_ON_WHOLE_DOMAIN) THEN
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=0,IBAR
                  IF(FCVAR(I,J,K,IBM_FGSC,IAXIS)==IBM_SOLID) US(I,J,K) = 0._EB
               ENDDO
            ENDDO
         ENDDO
         DO K=1,KBAR
            DO J=0,JBAR
               DO I=1,IBAR
                  IF(FCVAR(I,J,K,IBM_FGSC,JAXIS)==IBM_SOLID) VS(I,J,K) = 0._EB
               ENDDO
            ENDDO
         ENDDO
         DO K=0,KBAR
            DO J=1,JBAR
               DO I=1,IBAR
                  IF(FCVAR(I,J,K,IBM_FGSC,KAXIS)==IBM_SOLID) WS(I,J,K) = 0._EB
               ENDDO
            ENDDO
         ENDDO
      ENDIF
   ...
   ELSE PRED_CORR_IF
            ...
            DO J=1,JBAR
               DO I=0,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     ENDDO
                     U(I,J,K) = DOT_PRODUCT(CUT_FACE(ICF)%VEL(1:NFACE),CUT_FACE(ICF)%AREA(1:NFACE)) / (DY(J)*DZ(K))
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
            ...
            DO J=0,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     ENDDO
                     V(I,J,K) = DOT_PRODUCT(CUT_FACE(ICF)%VEL(1:NFACE),CUT_FACE(ICF)%AREA(1:NFACE)) / (DX(I)*DZ(K))
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
            ...
            DO J=1,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     ENDDO
                     W(I,J,K) = DOT_PRODUCT(CUT_FACE(ICF)%VEL(1:NFACE),CUT_FACE(ICF)%AREA(1:NFACE)) / (DX(I)*DY(J))
                  ENDIF
               ENDDO
            ENDDO
         ENDDO

      ELSE GRADH_ON_CARTESIAN_IF_2
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=0,IBAR
                  ICF = FCVAR(I,J,K,IBM_IDCF,IAXIS)
            ...
            DO J=1,JBAR
               DO I=1,IBAR
                  ...
                  IF (ICF>0) THEN
                     ...
                     ENDDO
                     W(I,J,K) = DOT_PRODUCT(CUT_FACE(ICF)%VEL(1:NFACE),CUT_FACE(ICF)%AREA(1:NFACE)) / (DY(J)*DX(I))
                  ENDIF
               ENDDO
            ENDDO
         ENDDO
      ...
      ENDIF GRADH_ON_CARTESIAN_IF_2

      IF(.NOT.PRES_ON_WHOLE_DOMAIN) THEN
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=0,IBAR
                  IF(FCVAR(I,J,K,IBM_FGSC,IAXIS)==IBM_SOLID) U(I,J,K) = 0._EB
               ENDDO
            ENDDO
         ENDDO
         DO K=1,KBAR
            DO J=0,JBAR
               DO I=1,IBAR
                  IF(FCVAR(I,J,K,IBM_FGSC,JAXIS)==IBM_SOLID) V(I,J,K) = 0._EB
               ENDDO
            ENDDO
         ENDDO
         DO K=0,KBAR
            DO J=1,JBAR
               DO I=1,IBAR
                  IF(FCVAR(I,J,K,IBM_FGSC,KAXIS)==IBM_SOLID) W(I,J,K) = 0._EB
               ENDDO
            ENDDO
         ENDDO
      ENDIF
...
SUBROUTINE CCREGION_COMPUTE_KRES(APPLY_TO_ESTIMATED_VARIABLES,NM)
...
REAL(EB):: AU,AV,AW,AUI,AVI,AWI,UVEL,VVEL,WVEL,FCT,AF,VELN,PRFCT
...
CUTCELL_DO : DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
   ...
   DO JCC=1,CUT_CELL(ICC)%NCELL
      IFC_LOOP : DO IFC=1,CUT_CELL(ICC)%CCELEM(1,JCC)
         ...
         CASE(IBM_FTYPE_CFGAS) ! GASPHASE CUT FACE:
            ...
            SELECT CASE(CUT_FACE(IFC2)%IJK(KAXIS+1)) ! X1AXIS
            CASE(IAXIS); UVEL = UVEL + VELN*AF; AU   = AU + AF
            CASE(JAXIS); VVEL = VVEL + VELN*AF; AV   = AV + AF
            CASE(KAXIS); WVEL = WVEL + VELN*AF; AW   = AW + AF
            END SELECT
         CASE(IBM_FTYPE_CFINB) ! INBOUNDARY CUT FACE: Note here we add U_NORMAL with Transpiration velocity due to
            ...
            AUI  = ABS(CFACE(ICFA)%NVEC(IAXIS)*AF)
            AVI  = ABS(CFACE(ICFA)%NVEC(JAXIS)*AF)
            AWI  = ABS(CFACE(ICFA)%NVEC(KAXIS)*AF)
            UVEL = UVEL - VELN*CFACE(ICFA)%NVEC(IAXIS)*AUI; AU   = AU + AUI
            VVEL = VVEL - VELN*CFACE(ICFA)%NVEC(JAXIS)*AVI; AV   = AV + AVI
            WVEL = WVEL - VELN*CFACE(ICFA)%NVEC(KAXIS)*AWI; AW   = AW + AWI
         END SELECT
      ENDDO IFC_LOOP
   ENDDO
   ...
   ! ! U velocity:
   ! DO IADD=-1,0
   !    ICF=FCVAR(I+IADD,J,K,IBM_IDCF,IAXIS)
   !    UVEL= UVEL + UU(I+IADD,J,K)*DY(J)*DZ(K)
   !    IF(ICF>0) THEN; AU  = AU + SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))
   !    ELSE;           AU  = AU + DY(J)*DZ(K); ENDIF
   !    !IF(ICF>0) THEN; AUI  = SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))
   !    !ELSE;           AUI  = DY(J)*DZ(K); ENDIF
   !    !AU  =   AU + AUI
   !    !UVEL= UVEL + UU(I+IADD,J,K)*AUI
   ! ENDDO
   !
   ! ! V velocity:
   ! DO IADD=-1,0
   !    ICF=FCVAR(I,J+IADD,K,IBM_IDCF,JAXIS)
   !    VVEL= VVEL + VV(I,J+IADD,K)*DX(I)*DZ(K)
   !    IF(ICF>0) THEN; AV  = AV + SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))
   !    ELSE;           AV  = AV + DX(I)*DZ(K); ENDIF
   !    !IF(ICF>0) THEN; AVI  = SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))
   !    !ELSE;           AVI  = DX(I)*DZ(K); ENDIF
   !    !AV  =   AV + AVI
   !    !VVEL= VVEL + VV(I,J+IADD,K)*AVI
   ! ENDDO
   !
   ! ! W velocity:
   ! DO IADD=-1,0
   !    ICF=FCVAR(I,J,K+IADD,IBM_IDCF,KAXIS)
   !    WVEL= WVEL + WW(I,J,K+IADD)*DX(I)*DY(J)
   !    IF(ICF>0) THEN; AW  = AW + SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))
   !    ELSE;           AW  = AW + DX(I)*DY(J); ENDIF
   !    !IF(ICF>0) THEN; AWI  = SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))
   !    !ELSE;           AWI  = DX(I)*DY(J); ENDIF
   !    !AWI  = DX(I)*DY(J);
   !    !AW  =   AW + AWI
   !    !WVEL= WVEL + WW(I,J,K+IADD)*AWI
   ! ENDDO

   UVEL = UVEL/(AU+TWO_EPSILON_EB)
   VVEL = VVEL/(AV+TWO_EPSILON_EB)
   WVEL = WVEL/(AW+TWO_EPSILON_EB)

   ! Finally KRES:
   KRES(I,J,K) = 0.5_EB*(UVEL**2._EB+VVEL**2._EB+WVEL**2._EB)
      ...
      RNODE_SNODE_IF: IF (RNODE/=SNODE) THEN
         ...
         IF (CODE==1 .AND. M2%NICC_R(1)>0) THEN
            ...
            EXTERNAL_WALL_LOOP_1 : DO IW=1,M%N_EXTERNAL_WALL_CELLS
               ...
               IF (M%CCVAR(BC%II,BC%JJ,BC%KK,IBM_CGSC) /= IBM_CUTCFE) &
...
DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   CALL WALL_MODEL(SLIP_FACTOR,BP%U_TAU,BP%Y_PLUS,MU_WALL/RHO_WALL,SF%ROUGHNESS,0.5_EB*DN,U_STRM)
...
PREDICT_NORMALS: IF (PREDICTOR) THEN
      ...
      WALL_CELL_TYPE: SELECT CASE (CFA%BOUNDARY_TYPE)
         ...
         CASE (SOLID_BOUNDARY)
            ...
            IF (SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX .OR. &
                ONE_D%NODE_INDEX > 0                 .OR. &
                ANY(SF%LEAK_PATH>0))                          &
                CYCLE CFACE_LOOP

            IF (ABS(ONE_D%T_IGN-T_BEGIN) < SPACING(ONE_D%T_IGN) .AND. SF%RAMP_INDEX(TIME_VELO)>=1) THEN
               TSI = T + DT
            ELSE
               TSI = T + DT - ONE_D%T_IGN
               IF (TSI<0._EB) THEN
                  ONE_D%U_NORMAL_S = 0._EB
                  CYCLE CFACE_LOOP
               ENDIF
            ...
            IF (ABS(SURFACE(CFA%SURF_INDEX)%MASS_FLUX_TOTAL)>=TWO_EPSILON_EB) ONE_D%U_NORMAL_S = &
                                                                              ONE_D%U_NORMAL_S*RHOA/ONE_D%RHO_F
   ...
   DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      CFA => CFACE(ICF)
      ONE_D => BOUNDARY_ONE_D(CFA%OD_INDEX)
      CFA%DUNDT = RDT*(ONE_D%U_NORMAL_S-ONE_D%U_NORMAL)
   ENDDO
...
ELSE PREDICT_NORMALS
   ...
   DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      CFA => CFACE(ICF)
      ONE_D => BOUNDARY_ONE_D(CFA%OD_INDEX)
      CFA%DUNDT = CFA%DUNDT + 2._EB*RDT*(ONE_D%U_NORMAL-ONE_D%U_NORMAL_S)
   ENDDO
...
TYPE(BOUNDARY_ONE_D_TYPE) :: ONE_D
...
BC%IOR = INT(SIGN(1._EB,NVEC(X1AXIS)))*X1AXIS
...
IF (CFACE_INTERPOLATE) THEN
   ...
   IF (INT_NPE_HI > 0) THEN
      ...
      ! Gas conductivity:
      CALL GET_CCREGION_CELL_CONDUCTIVITY(CFA%ZZ_G(1:N_TRACKED_SPECIES),CFA%MU_G,MU_DNS_G,CFA%TMP_G,K_G)
      ONE_D%K_G = K_G
...
CASE(IBM_FTYPE_CFGAS) ! Cut-cell -> use value from CUT_CELL data struct:
   ...
   ! Gas conductivity:
   CALL GET_CCREGION_CELL_CONDUCTIVITY(CFA%ZZ_G(1:N_TRACKED_SPECIES),MU(I,J,K),MU_DNS(I,J,K),CFA%TMP_G,K_G)
   ONE_D%K_G = K_G
...
REAL(EB) :: AREATOT, VEL_CART, FLX_FCT, FSCU, A_CART
REAL(EB), POINTER, DIMENSION(:,:,:) :: UP,VP,WP
...
MESH_LOOP : DO NM=1,NMESHES
   ...
   CUTFACE_LOOP : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      ...
      CASE(IAXIS)
         VEL_CART = UP(I,J,K)
         A_CART   = DY(J)*DZ(K)
         FLX_FCT  = A_CART/AREATOT  ! This is Area Cartesian / Sum of cut-face areas.
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   ICC_LOOP : DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      ...
      ! Interpolate values of H to corresponding cut-cell centroids:
      IF (GRADH_ON_CARTESIAN .OR. .NOT.CC_UNSTRUCTURED_PROJECTION) THEN
   ...
   CUTFACE_LOOP_1 : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      ...
      IF(INT_N_EXT_PTS==1) THEN
         ...
         IF( CUT_FACE(ICF)%INT_INBFC(1,IFACE)== IBM_FTYPE_CFINB) THEN
            ...
            IF (ICFA>0) THEN
               VELN = -BOUNDARY_ONE_D(CFACE(ICFA)%OD_INDEX)%U_NORMAL
...
IF(CC_STRESS_METHOD) THEN
   ...
   IF (N_MPI_PROCESSES>1) THEN
      CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,CELL_COUNT_CC,COUNTS,DISPLS,MPI_INTEGER,MPI_COMM_WORLD,IERR)
   ENDIF
   DEALLOCATE(COUNTS,DISPLS)
   ! Now reallocate arrays defined by CELL_COUNT to sizes defined by
   ! CELL_COUNT+CELL_COUNT_CC:
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      M => MESHES(NM)
      DO N=1,M%N_NEIGHBORING_MESHES
         ...
         M4 => MESHES(NOM)
         IF(ALLOCATED(M4%SOLID))        DEALLOCATE(M4%SOLID)
         IF(ALLOCATED(M4%WALL_INDEX))   DEALLOCATE(M4%WALL_INDEX)
         IF(ALLOCATED(M4%OBST_INDEX_C)) DEALLOCATE(M4%OBST_INDEX_C)
         ALLOCATE(M4%SOLID(0:CELL_COUNT(NOM)+CELL_COUNT_CC(NOM)));           M4%SOLID = .FALSE.
         ALLOCATE(M4%WALL_INDEX(0:CELL_COUNT(NOM)+CELL_COUNT_CC(NOM),-3:3)); M4%WALL_INDEX = 0
         ALLOCATE(M4%OBST_INDEX_C(0:CELL_COUNT(NOM)+CELL_COUNT_CC(NOM)));    M4%OBST_INDEX_C  = 0
      ENDDO
   ENDDO
   IF (N_MPI_PROCESSES>1) THEN
      DO NM=1,NMESHES
         ...
         M => MESHES(NM)
         CALL MPI_BCAST(M%SOLID,SIZE(M%SOLID),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
         CALL MPI_BCAST(M%WALL_INDEX,SIZE(M%WALL_INDEX),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
         CALL MPI_BCAST(M%OBST_INDEX_C,SIZE(M%OBST_INDEX_C),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
      ENDDO
   ENDIF
ENDIF
   ...
   MESH_LOOP_2 : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
         ...
         DO IFACE=1,CUT_FACE(ICF)%NFACE
            ...
            IROW = CUT_CELL(ICC)%UNKZ(JCC) - UNKZ_IND(NM_START)
            BOUNDARY_ONE_D(CFACE(CFACE_INDEX_LOCAL)%OD_INDEX)%RDN = 1._EB/DXN_UNKZ_LOC(IROW)
         ENDDO
      ENDDO
   ENDDO MESH_LOOP_2
   ...
   MESH_LOOP_3 : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
         ...
         IF(CUT_FACE(ICF1)%INT_XN(1,ICF2) < TWO_EPSILON_EB) THEN
            ONE_D%RDN = 1._EB/DX(1)
         ELSE
            ONE_D%RDN = 0.5_EB/CUT_FACE(ICF1)%INT_XN(1,ICF2)
         ENDIF
      ENDDO
   ENDDO MESH_LOOP_3
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   EXTERNAL_WALL_LOOP : DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      IF (CCVAR(BC%II,BC%JJ,BC%KK,IBM_CGSC) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP
         ...
         DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
            DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
              ...
              ELSEIF(MESHES(NOM)%CCVAR(IIO,JJO,KKO,IBM_CGSC) == IBM_GASPHASE) THEN ! Regular cell:
                 ...
                 RHO_CC  = RHO_CC  + (      PRFCT*RHOS(BC%II,BC%JJ,BC%KK) + &
                                     (1._EB-PRFCT)*RHO(BC%II,BC%JJ,BC%KK))*VCELL
                 TMP_CC  = TMP_CC  + TMP(BC%II,BC%JJ,BC%KK)*VCELL
                 D_CC    = D_CC    + (      PRFCT*D(BC%II,BC%JJ,BC%KK) + &
                                     (1._EB-PRFCT)*DS(BC%II,BC%JJ,BC%KK))*VCELL
                 DO NN=1,N_TOTAL_SCALARS
                    ZZ_CC(NN)=ZZ_CC(NN)+(      PRFCT *ZZS(BC%II,BC%JJ,BC%KK,NN) &
                                       +(1._EB-PRFCT)*ZZ(BC%II,BC%JJ,BC%KK,NN) )*VCELL
                 ENDDO
                 VOL     = VOL + VCELL
              ENDIF
            ENDDO
      ...
      ICC   = CCVAR(BC%II,BC%JJ,BC%KK,IBM_IDCC)
      IF (PREDICTOR) THEN
         DO JCC=1,CUT_CELL(ICC)%NCELL
...
ELSE
   IF (CORRECTOR .AND. (DIAGNOSTICS .OR. STORE_CUTCELL_DIVERGENCE)) CALL CCIBM_CHECK_DIVERGENCE(T,DT,.FALSE.)
ENDIF
...
INTEGER :: INT_NPE_LO,INT_NPE_HI,INPE,VIND,EP,IW
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_CUTFACE_LOOP: DO ICF = 1,MESHES(NM)%N_CUTFACE_MESH
      ...
      K = CUT_FACE(ICF)%IJK(KAXIS)
      X1AXIS = CUT_FACE(ICF)%IJK(KAXIS+1)
      CUT_FACE_STATUS_IF: IF ( CUT_FACE(ICF)%STATUS == IBM_GASPHASE ) THEN
      ...
      ELSEIF( CUT_FACE(ICF)%STATUS == IBM_INBOUNDARY ) THEN CUT_FACE_STATUS_IF

         DO IFACE=1,CUT_FACE(ICF)%NFACE

            ! Approximate external point velocities:
            INT_NPE_LO  = CUT_FACE(ICF)%INT_NPE( LOW_IND,IAXIS,EP,IFACE)
            INT_NPE_HI  = CUT_FACE(ICF)%INT_NPE(HIGH_IND,IAXIS,EP,IFACE)
            DO INPE=INT_NPE_LO+1,INT_NPE_LO+INT_NPE_HI
               CUT_FACE(ICF)%INT_FVARS(INT_VEL_IND,INPE) = U(I,J,K)
            ENDDO
            INT_NPE_LO  = CUT_FACE(ICF)%INT_NPE( LOW_IND,JAXIS,EP,IFACE)
            INT_NPE_HI  = CUT_FACE(ICF)%INT_NPE(HIGH_IND,JAXIS,EP,IFACE)
            DO INPE=INT_NPE_LO+1,INT_NPE_LO+INT_NPE_HI
               CUT_FACE(ICF)%INT_FVARS(INT_VEL_IND,INPE) = V(I,J,K)
            ENDDO
            INT_NPE_LO  = CUT_FACE(ICF)%INT_NPE( LOW_IND,KAXIS,EP,IFACE)
            INT_NPE_HI  = CUT_FACE(ICF)%INT_NPE(HIGH_IND,KAXIS,EP,IFACE)
            DO INPE=INT_NPE_LO+1,INT_NPE_LO+INT_NPE_HI
               CUT_FACE(ICF)%INT_FVARS(INT_VEL_IND,INPE) = W(I,J,K)
            ENDDO

            ! Then cell centered ZZ, TMP, etc.:
            INT_NPE_LO  = CUT_FACE(ICF)%INT_NPE( LOW_IND,VIND,EP,IFACE)
            INT_NPE_HI  = CUT_FACE(ICF)%INT_NPE(HIGH_IND,VIND,EP,IFACE)
            DO INPE=INT_NPE_LO+1,INT_NPE_LO+INT_NPE_HI
               CUT_FACE(ICF)%INT_CVARS(    INT_H_IND,INPE)= H(I,J,K)
               CUT_FACE(ICF)%INT_CVARS(  INT_RHO_IND,INPE)= RHO(I,J,K)
               CUT_FACE(ICF)%INT_CVARS(  INT_TMP_IND,INPE)= TMP(I,J,K)
               CUT_FACE(ICF)%INT_CVARS( INT_RSUM_IND,INPE)= RSUM(I,J,K)
               CUT_FACE(ICF)%INT_CVARS(   INT_MU_IND,INPE)= MU(I,J,K)
               CUT_FACE(ICF)%INT_CVARS(INT_MUDNS_IND,INPE)= MU_DNS(I,J,K)
               CUT_FACE(ICF)%INT_CVARS(    INT_P_IND,INPE)= RHOS(I,J,K)*(H(I,J,K)-KRES(I,J,K))
               CUT_FACE(ICF)%INT_CVARS(INT_P_IND+1:INT_P_IND+N_TOTAL_SCALARS,INPE)= ZZ(I,J,K,1:N_TOTAL_SCALARS)
            ENDDO
         ENDDO

      ENDIF CUT_FACE_STATUS_IF

   ENDDO MESH_CUTFACE_LOOP

   ! External mesh CFACEs initialize ONE_D BCs:
   DO ICF=1,N_EXTERNAL_CFACE_CELLS
      CFA  => CFACE(ICF)
   ...
   ! Geometry boundary CFACES initialize ONE_D BCs:
   DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      CFA  => CFACE(ICF)
...
AXIS_DO : DO X1AXIS = IAXIS,KAXIS
   ...
   IFACE_DO : DO IFACE=1,MESHES(NM)%IBM_NREGFACE_Z(X1AXIS)
      ...
      IF (REGFACE_Z(IFACE)%IWC > 0) THEN
         ...
         REGFACE_Z(IFACE)%FN_ZZ(N)=ONE_D%RHO_F*ONE_D%ZZ_F(N)
      ELSE
         SELECT CASE(X1AXIS)
         CASE(IAXIS)
...
REAL(EB) :: VCELL, VCCELL, DIVVOL, DUMMY, RTRMVOL, DIVVOL_BC, CCVOL
...
DO IFACE=1,MESHES(NM)%IBM_NBBREGFACE_Z(X1AXIS)
   ...
   RHO_Z_PV(ISIDE) = RHOPV(ISIDE)*ZZP(I+1+ISIDE,J,K,N)
   FN_ZZ           = ONE_D%RHO_F*ONE_D%ZZ_F(N)
   SELECT CASE(WC%BOUNDARY_TYPE)
      CASE DEFAULT
         VELC = UU(I,J,K)
      CASE(SOLID_BOUNDARY)
         IF (PREDICTOR) VELC = -SIGN(1._EB,REAL(IOR,EB))*ONE_D%U_NORMAL_S
         IF (CORRECTOR) VELC = -SIGN(1._EB,REAL(IOR,EB))*ONE_D%U_NORMAL
   END SELECT
   ! Add: -(bar{rho*zz} u dot n - (rho*zz) u dot n) to corresponding cell DP:
   AF = DY(J)*DZ(K)
   U_DOT_DEL_RHO_Z_VOL(I+1+ISIDE,J,K) = U_DOT_DEL_RHO_Z_VOL(I+1+ISIDE,J,K) - &
...
DO ICF = 1,MESHES(NM)%N_BBCUTFACE_MESH
   ...
   DO IFACE=1,CUT_FACE(ICF)%NFACE
      ...
      IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN ! Here used the CFA corresponding U_NORMAL.
         FCT  = 1._EB
         ONE_D => BOUNDARY_ONE_D(CFACE(CUT_FACE(ICF)%CFACE_INDEX(IFACE))%OD_INDEX)
         VELC =         PRFCT*ONE_D%U_NORMAL + (1._EB-PRFCT)*ONE_D%U_NORMAL_S
      ELSE
...
ISIDE=-1
CFACE_LOOP : DO ICFA=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   CFA => CFACE(ICFA)
   ONE_D => BOUNDARY_ONE_D(CFA%OD_INDEX)
...
DO IFACE=1,MESHES(NM)%IBM_NBBREGFACE_Z(X1AXIS)
   ...
   VELC      = UU(I,J,K)
   VELC2     = PRFCT*ONE_D%U_NORMAL + (1._EB-PRFCT)*ONE_D%U_NORMAL_S
   TMP_F_GAS = ONE_D%TMP_F
   ...
   ZZ_GET(1:N_TRACKED_SPECIES) = ONE_D%ZZ_F(1:N_TRACKED_SPECIES)
   CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,TMP_F_GAS)
   FN_H_S = ONE_D%RHO_F*H_S ! bar{rho*hs}
   ! Add: -(bar{rho*hs} u dot n - (rho*hs) u dot n) to corresponding cell DP:
   AF = DY(J)*DZ(K)
...
CFACE_LOOP : DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   ! H_RHO_D_DZDN
   UN_P =  PRFCT*ONE_D%U_NORMAL + (1._EB-PRFCT)*ONE_D%U_NORMAL_S
   TMP_G = ONE_D%TMP_F
   IF (UN_P>0._EB) TMP_G = CFA%TMP_G
   DO N=1,N_TOTAL_SCALARS
      CALL GET_SENSIBLE_ENTHALPY_Z(N,TMP_G,H_S)
      CUT_FACE(IND1)%H_RHO_D_DZDN(N,IND2) = H_S*ONE_D%RHO_D_DZDN_F(N)
   ENDDO
   ...
   CUT_CELL(ICC)%DEL_RHO_D_DEL_Z_VOL(1:N_TOTAL_SCALARS,JCC) = &
   CUT_CELL(ICC)%DEL_RHO_D_DEL_Z_VOL(1:N_TOTAL_SCALARS,JCC) - ONE_D%RHO_D_DZDN_F(1:N_TOTAL_SCALARS)*CFA%AREA
ENDDO CFACE_LOOP
...
DO IFACE=1,MESHES(NM)%IBM_NBBREGFACE_Z(X1AXIS)
   ...
   DPVOL(IIG,JJG,KKG) = DPVOL(IIG,JJG,KKG) - ( ONE_D%Q_CON_F ) * AF  + WC%Q_LEAK * (DX(IIG)*DY(JJG)*DZ(KKG))
ENDDO
...
DO IFACE=1,MESHES(NM)%IBM_NBBRCFACE_Z
   ...
   CASE(IBM_FTYPE_CFGAS) ! Cut-cell -> use Temperature value from CUT_CELL data struct:
      ...
      CUT_CELL(ICC)%DVOL(JCC) - ( ONE_D%Q_CON_F ) * AF + WC%Q_LEAK * CUT_CELL(ICC)%VOLUME(JCC) ! Qconf +ve sign is
   ...
   ELSE
      ...
      DO IFACE=1,CUT_FACE(ICF)%NFACE
         ...
         CASE(IBM_FTYPE_CFGAS) ! Cut-cell -> use Temperature value from CUT_CELL data struct:
            ...
            JCC = CUT_FACE(ICF)%CELL_LIST(3,ISIDE+2,IFACE)
            CUT_CELL(ICC)%DVOL(JCC) = &
            CUT_CELL(ICC)%DVOL(JCC) - ( ONE_D%Q_CON_F ) * AF + WC%Q_LEAK * CUT_CELL(ICC)%VOLUME(JCC) ! Qconf +ve sign
...
IF (PREDICTOR) THEN
  DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
     ...
     IND1 = CFA%CUT_FACE_IND1;                         IND2 = CFA%CUT_FACE_IND2
     ICC  = CUT_FACE(IND1)%CELL_LIST(2,LOW_IND,IND2);  JCC  = CUT_FACE(IND1)%CELL_LIST(3,LOW_IND,IND2)
     CUT_CELL(ICC)%DVOL(JCC)=CUT_CELL(ICC)%DVOL(JCC)-( ONE_D%Q_CON_F ) * CUT_FACE(IND1)%AREA(IND2) ! QCONF(+) into solid.
  ENDDO
ELSE
  DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
     CFA  => CFACE(ICF)
...
ALLOCATE(N_ZZ_MAX_V(N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS))
DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   CFA => CFACE(ICF)
   ONE_D => BOUNDARY_ONE_D(CFA%OD_INDEX)
   N_ZZ_MAX_V(ICF)=MAXLOC(ONE_D%ZZ_F(1:N_TRACKED_SPECIES),1)
ENDDO
...
DIFFUSIVE_FLUX_LOOP: DO N=1,N_TOTAL_SCALARS
   ...
   CFACE_LOOP : DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      ...
      RHO_D_DZDN = 2._EB*ONE_D%RHO_D_F(N)*(CFA%ZZ_G(N)-ONE_D%ZZ_F(N))*ONE_D%RDN
      IF (N==N_ZZ_MAX_V(ICF)) THEN
         ZZ_GET(1:N_TRACKED_SPECIES) = CFA%ZZ_G(1:N_TRACKED_SPECIES)
         RHO_D_DZDN_GET(1:N_TRACKED_SPECIES) = &
         2._EB*ONE_D%RHO_D_F(1:N_TRACKED_SPECIES)*( ZZ_GET(1:N_TRACKED_SPECIES) - ONE_D%ZZ_F(1:N_TRACKED_SPECIES))*ONE_D%RDN
      ...
      CUT_FACE(CFA%CUT_FACE_IND1)%ZZ_FACE(N,   CFA%CUT_FACE_IND2) = ONE_D%ZZ_F(N)
      CUT_FACE(CFA%CUT_FACE_IND1)%TMP_FACE(    CFA%CUT_FACE_IND2) = ONE_D%TMP_F
   ENDDO CFACE_LOOP
...
KKG = BC%KKG
IOR = BC%IOR
N_ZZ_MAX = MAXLOC(ONE_D%ZZ_F(1:N_TRACKED_SPECIES),1)
RHO_D_DZDN = 2._EB*ONE_D%RHO_D_F(N)*(ZZP(IIG,JJG,KKG,N)-ONE_D%ZZ_F(N))*ONE_D%RDN
IF (N==N_ZZ_MAX) THEN
   RHO_D_DZDN_GET = 2._EB*ONE_D%RHO_D_F(:)*(ZZP(IIG,JJG,KKG,:)-ONE_D%ZZ_F(:))*ONE_D%RDN
...
N_ZZ_MAX = MAXLOC(ONE_D%ZZ_F(1:N_TRACKED_SPECIES),1)
RHO_D_DZDN = ONE_D%RHO_D_F(N)*(ZZ_G-ONE_D%ZZ_F(N))*IDX
IF (N==N_ZZ_MAX) THEN
   RHO_D_DZDN_GET = ONE_D%RHO_D_F(:)*(ZZ_GV(:)-ONE_D%ZZ_F(:))*IDX
   RHO_D_DZDN = -(SUM(RHO_D_DZDN_GET(:))-RHO_D_DZDN)
ENDIF

IF (IOR < 0) RHO_D_DZDN = -RHO_D_DZDN ! This is to switch the sign of the spatial derivative in high side boundaries.
DIFF_FACE = ONE_D%RHO_D_F(N)/ONE_D%RHO_F
ZZ_FACE   = ONE_D%ZZ_F(N)
TMP_FACE  = ONE_D%TMP_F
...
MESH_LOOP_DBND : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   IFACE_LOOP_RCF1: DO IFACE=1,MESHES(NM)%IBM_NBBRCFACE_Z
      ...
      CASE(KAXIS)
         ...
         VELC = WP(I,J,K)
      END SELECT
      FN_ZZ           = ONE_D%RHO_F*ONE_D%ZZ_F(N) ! These have been Flux limited in wall.f90
      SELECT CASE(WC%BOUNDARY_TYPE)
         CASE DEFAULT
            ! Already filled in previous X1AXIS select case.
         CASE(SOLID_BOUNDARY)
      ...
      IEC_SELECT: SELECT CASE(IEC)
         CASE(IAXIS)
            ...
            ELSE ! IF(FAXIS==KAXIS) THEN
                IF(CC_DO_CUTFACE_VELOCITIES) &
                AFCT = MIN(THRESF,DXX(1)/(XB_IB(ICD_SGN)+TWO_EPSILON_EB)) ! Rescale to CF veloc.
...
DO IOE = 1,2
   IOEFLG = .TRUE.
   IF (IBM_EDGE%SURF_INDEX(-IOE)>-1) THEN
       IF(SURFACE(IBM_EDGE%SURF_INDEX(-IOE))%VELOCITY_BC_INDEX==FREE_SLIP_BC) THEN
          IBM_EDGE%DUIDXJ((/-IOE,IOE/))    = 0._EB
          IOEFLG = .FALSE.
       ENDIF
   ENDIF
   IF (IBM_EDGE%SURF_INDEX(IOE)>-1) THEN
      ...
      IEC_SELECT: SELECT CASE(IEC)
         CASE(IAXIS) IEC_SELECT
            ...
            IF (FAXIS==JAXIS) THEN
               ...
               DF     = DXX(2) - ABS(XB_IB)
               DE     = DXN_STRM_UB
               IF(CC_DO_CUTFACE_VELOCITIES) AFCT = MIN(THRESF,DE/(DF+TWO_EPSILON_EB))
...
SUBROUTINE CCIBM_VELOCITY_FLUX(NM,DT)

INTEGER, INTENT(IN) :: NM
REAL(EB),INTENT(IN) :: DT

! Local Variables:
INTEGER  :: ICF !,I,J,K,IOR,IW
REAL(EB) :: TNOW, TNOW2
...
CALL CCIBM_STORE_FACE_FV(NM,SUBSTRACT_BAROCLINIC=.FALSE.)
         ...
         INT_N_EXT_PTS_IF: IF(INT_N_EXT_PTS==1) THEN
            ...
            IF( CUT_FACE(ICF)%INT_INBFC(1,IFACE)== IBM_FTYPE_CFINB) THEN
               ...
               IF (ICFA>0) THEN
                  ...
                  ONE_D => BOUNDARY_ONE_D(CFA%OD_INDEX)
                  VELN = -ONE_D%U_NORMAL
                  SRGH = SURFACE(CFA%SURF_INDEX)%ROUGHNESS
               ENDIF
            ENDIF
            NN(IAXIS:KAXIS) = CUT_FACE(ICF)%INT_NOUT(IAXIS:KAXIS,IFACE)
            TT=0._EB
...
REAL(EB):: U_IBM,V_IBM,W_IBM,DUUDT,DVVDT,DWWDT,RFODT,TNOW,FCT,UN,AFCT,IDX
INTEGER :: I,J,K,II,JJ,KK,IOR,IW,ICF,JCF,X1AXIS,NOM,IFACE,JFACE,NFACE
TYPE(WALL_TYPE), POINTER :: WC
TYPE(EXTERNAL_WALL_TYPE), POINTER :: EWC
TYPE(CFACE_TYPE), POINTER :: CFA
            ...
            CASE(IAXIS)
               ...
               ELSE
                  IF(GRADH_ON_CARTESIAN) THEN
                     FVX(I,J,K) = -RDXN(I)*(HP(I+1,J,K)-HP(I,J,K)) - DUUDT/CUT_FACE(ICF)%ALPHA_CF
                  ELSE
                     ! Area average IDX:
                     IDX=0._EB
                     DO JCF=1,NFACE
                        IDX = IDX + &
                        CUT_FACE(ICF)%AREA(JCF)/(CUT_FACE(ICF)%XCENHIGH(X1AXIS,JCF)-CUT_FACE(ICF)%XCENLOW(X1AXIS,JCF))
                     ENDDO
                     IDX = IDX/SUM(CUT_FACE(ICF)%AREA(1:NFACE))
                     FVX(I,J,K) = -IDX*(HP(I+1,J,K)-HP(I,J,K)) - DUUDT/CUT_FACE(ICF)%ALPHA_CF
                  ENDIF
               ENDIF
            CASE(JAXIS)
               ...
               ELSE
                  IF(GRADH_ON_CARTESIAN) THEN
                     FVY(I,J,K) = -RDYN(J)*(HP(I,J+1,K)-HP(I,J,K)) - DVVDT/CUT_FACE(ICF)%ALPHA_CF
                  ELSE
                     ! Area average IDX:
                     IDX=0._EB
                     DO JCF=1,NFACE
                     ...
                     ENDDO
                     IDX = IDX/SUM(CUT_FACE(ICF)%AREA(1:NFACE))
                     FVY(I,J,K) = -IDX*(HP(I,J+1,K)-HP(I,J,K)) - DVVDT/CUT_FACE(ICF)%ALPHA_CF
                  ENDIF
               ENDIF
            CASE(KAXIS)
               ...
               ELSE
                  IF(GRADH_ON_CARTESIAN) THEN
                     FVZ(I,J,K) = -RDZN(K)*(HP(I,J,K+1)-HP(I,J,K)) - DWWDT/CUT_FACE(ICF)%ALPHA_CF
                  ELSE
                     ! Area average IDX:
                     IDX=0._EB
                     DO JCF=1,NFACE
                     ...
                     ENDDO
                     IDX = IDX/SUM(CUT_FACE(ICF)%AREA(1:NFACE))
                     FVZ(I,J,K) = -IDX*(HP(I,J,K+1)-HP(I,J,K)) - DWWDT/CUT_FACE(ICF)%ALPHA_CF
                  ENDIF
               ENDIF
         END SELECT
   ENDDO CUTFACE_LOOP
   ! Store the Force vector in gasphase CUT_FACE(ICF) - FV, FN(1:NFACE):
   CALL CCIBM_STORE_FACE_FV(NM,SUBSTRACT_BAROCLINIC=.FALSE.)
   DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      IF ( CUT_FACE(ICF)%STATUS /= IBM_GASPHASE) CYCLE
      CUT_FACE(ICF)%FN(1:CUT_FACE(ICF)%NFACE) = CUT_FACE(ICF)%FV
...
IF(CC_STRESS_METHOD) THEN
   ...
   IF (CC_STM_TYPE==0 .OR. CC_STM_TYPE==10) THEN
      CUTFACE_LOOP_2 : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
         IF ( CUT_FACE(ICF)%STATUS /= IBM_GASPHASE) CYCLE CUTFACE_LOOP_2
         I      = CUT_FACE(ICF)%IJK(IAXIS)
         J      = CUT_FACE(ICF)%IJK(JAXIS)
         ...
         SELECT CASE(X1AXIS)
               CASE(IAXIS)
                  IF (CUT_FACE(ICF)%ALPHA_CF > A_THRESH_FORCING) CYCLE CUTFACE_LOOP_2
                  IF (PREDICTOR) DUUDT = (U_IBM-U(I,J,K))/DT
                  IF (CORRECTOR) DUUDT = (2._EB*U_IBM-(U(I,J,K)+US(I,J,K)))/DT
                  FVX(I,J,K) = -RDXN(I)*(HP(I+1,J,K)-HP(I,J,K)) - DUUDT
               CASE(JAXIS)
   ...
   ELSEIF(CC_STM_TYPE==1 .OR. CC_STM_TYPE==2) THEN
      IF(PRESSURE_ITERATIONS==1) CALL CCIBM_STORE_FACE_FV(NM,SUBSTRACT_BAROCLINIC=.TRUE.)
      ...
      CUTFACE_LOOP_3 : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
         ...
         SELECT CASE(X1AXIS)
               CASE(IAXIS)
                  FCT=CUT_FACE(ICF)%ALPHA_CF
                  IF (PREDICTOR) DUUDT = (U_IBM-U(I,J,K))/DT
                  IF (CORRECTOR) DUUDT = (2._EB*U_IBM-(U(I,J,K)+US(I,J,K)))/DT
                  FVX(I,J,K) = FCT*CUT_FACE(ICF)%FV + (1._EB-FCT)*(-RDXN(I)*(HP(I+1,J,K)-HP(I,J,K))- DUUDT)
               CASE(JAXIS)
                  FCT=CUT_FACE(ICF)%ALPHA_CF
                  IF (PREDICTOR) DVVDT = (V_IBM-V(I,J,K))/DT
                  IF (CORRECTOR) DVVDT = (2._EB*V_IBM-(V(I,J,K)+VS(I,J,K)))/DT
                  FVY(I,J,K) = FCT*CUT_FACE(ICF)%FV + (1._EB-FCT)*(-RDYN(J)*(HP(I,J+1,K)-HP(I,J,K))- DVVDT)
               CASE(KAXIS)
                  FCT=CUT_FACE(ICF)%ALPHA_CF
                  IF (PREDICTOR) DWWDT = (W_IBM-W(I,J,K))/DT
                  IF (CORRECTOR) DWWDT = (2._EB*W_IBM-(W(I,J,K)+WS(I,J,K)))/DT
                  FVZ(I,J,K) = FCT*CUT_FACE(ICF)%FV + (1._EB-FCT)*(-RDZN(K)*(HP(I,J,K+1)-HP(I,J,K))- DWWDT)
            END SELECT
...
CC_UNSTRUCTURED_PROJ_COND : IF (CC_UNSTRUCTURED_PROJECTION) THEN
   CFACE_LOOP : DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      CFA  =>CFACE(ICF)
      IF (CFA%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY .OR. CFA%BOUNDARY_TYPE==OPEN_BOUNDARY) CYCLE CFACE_LOOP
      ...
      IF (NOM/=0 .OR. CFA%BOUNDARY_TYPE==SOLID_BOUNDARY .OR. CFA%BOUNDARY_TYPE==NULL_BOUNDARY) THEN
         AFCT = 1._EB
         IF(ICF<=N_EXTERNAL_CFACE_CELLS) AFCT = -SIGN(1._EB,REAL(IOR,EB))
         IF (PREDICTOR) THEN
            UN    = AFCT*ONE_D%U_NORMAL_S
            DUUDT = RFODT*(UN-CUT_FACE(IFACE)%VEL(JFACE))
         ELSE
            UN = AFCT*ONE_D%U_NORMAL
...
ENDIF CC_UNSTRUCTURED_PROJ_COND
...
SUBROUTINE CCIBM_STORE_FACE_FV(NM,SUBSTRACT_BAROCLINIC)
...
ELSE
   ...
   CF_LOOP_2 : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      IF ( CUT_FACE(ICF)%STATUS /= IBM_GASPHASE) CYCLE CF_LOOP_2
      CUT_FACE(ICF)%FV = 0._EB
...
IF (PREDICTOR) THEN
   VELX1 = -ONE_D%U_NORMAL * CFACE(ICF)%NVEC(X1AXIS)
ELSE
   VELX1 = -ONE_D%U_NORMAL_S* CFACE(ICF)%NVEC(X1AXIS)
ENDIF
...
MESHES_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   IF(.NOT.PRES_ON_WHOLE_DOMAIN .AND. STORE_CARTESIAN_DIVERGENCE) THEN
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               IF (SOLID(CELL_INDEX(I,J,K)) .OR. CCVAR(I,J,K,IBM_CGSC)==IBM_SOLID) CARTVELDIV(I,J,K) = 0._EB
            ENDDO
         ENDDO
      ENDDO
   ENDIF
...
SUBROUTINE GET_FN_DIVERGENCE_CUTCELL(ICC,JCC,DIV_FN,WITH_BAROCLINIC)
...
LOGICAL, INTENT(IN) :: WITH_BAROCLINIC

! Local Variables:
INTEGER :: IFACE, IFC, LOWHIGH, X1AXIS, ILH, IFC2, IFACE2, ICFA, I, J, K
REAL(EB):: AF, FN, FCT, REG_FACTOR, CF_FACTOR


IF (WITH_BAROCLINIC) THEN
   REG_FACTOR = 0._EB
   CF_FACTOR  = 1._EB
ELSE
   CF_FACTOR  = 0._EB
...
IFC_LOOP : DO IFC=1,CUT_CELL(ICC)%CCELEM(1,JCC)
   ...
   CASE(IBM_FTYPE_RGGAS) ! REGULAR GASPHASE
      ...
      CASE(IAXIS)
         AF   = DY(J)*DZ(K)
         FN   = FCT*(FVX(I-1+ILH,J,K) - REG_FACTOR*FVX_B(I-1+ILH,J,K))
      CASE(JAXIS)
         AF   = DX(I)*DZ(K)
         FN   = FCT*(FVY(I,J-1+ILH,K) - REG_FACTOR*FVY_B(I,J-1+ILH,K))
      CASE(KAXIS)
         AF   = DX(I)*DY(J)
         FN   = FCT*(FVZ(I,J,K-1+ILH) - REG_FACTOR*FVZ_B(I,J,K-1+ILH))
      END SELECT
   CASE(IBM_FTYPE_CFGAS) ! GASPHASE CUT FACE:
      LOWHIGH = CUT_CELL(ICC)%FACE_LIST(2,IFACE)
      FCT     = REAL(2*LOWHIGH - 3, EB)
      ...
      IFACE2  = CUT_CELL(ICC)%FACE_LIST(5,IFACE)
      AF      = CUT_FACE(IFC2)%AREA(IFACE2)
      FN      = FCT*(CUT_FACE(IFC2)%FN( IFACE2) + CF_FACTOR*CUT_FACE(IFC2)%FN_B( IFACE2))
   CASE(IBM_FTYPE_CFINB) ! INBOUNDARY CUT FACE
      FCT     = 1._EB    ! Normal vector defined into the body.
      IFC2    = CUT_CELL(ICC)%FACE_LIST(4,IFACE)
      IFACE2  = CUT_CELL(ICC)%FACE_LIST(5,IFACE)
   ...
   EWALL_LOOP : DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      ELSE ! WC%BOUNDARY_TYPE/=OPEN_BOUNDARY
         ...
         SPECIES_LOOP: DO N=1,N_TOTAL_SCALARS
            ...
            ! ONE_D%ZZ_F(1:N_TRACKED_SPECIES), but loop  N=1,N_TOTAL_SCALARS
            RHO_D_DZDN(N) = 2._EB*ONE_D%RHO_D_F(N)*(ZZP(IIG,JJG,KKG,N)-ONE_D%ZZ_F(N))*ONE_D%RDN
            IF (N==N_ZZ_MAX) THEN
               RHO_D_DZDN_GET(1:N_TRACKED_SPECIES) = &
                 2._EB*ONE_D%RHO_D_F(1:N_TRACKED_SPECIES)*  &
               (ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)-ONE_D%ZZ_F(1:N_TRACKED_SPECIES))*ONE_D%RDN
      ...
      IF (MESHES(NM)%OMESH(NOM)%NICC_R(1)>0) THEN
         ...
         EXTERNAL_WALL_LOOP_1 : DO IW=1,N_EXTERNAL_WALL_CELLS
            ...
            IF ( WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY ) THEN
               ...
               IF (CCVAR(BC%II,BC%JJ,BC%KK,IBM_CGSC) /= IBM_CUTCFE) &
                  CYCLE EXTERNAL_WALL_LOOP_1
               DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
                  DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   EXTERNAL_WALL_LOOP_2 : DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      IF ( WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY ) THEN
         IF (CCVAR(BC%II,BC%JJ,BC%KK,IBM_CGSC) /= IBM_CUTCFE) CYCLE EXTERNAL_WALL_LOOP_2
         NOM = EWC%NOM
         DO KKO=EWC%KKO_MIN,EWC%KKO_MAX
            DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
               DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
...
IF (N_MPI_PROCESSES>1) THEN
   DO NM=1,NMESHES
      ...
      CALL MPI_BCAST(M%CCVAR(ISTR,JSTR,KSTR,IBM_UNKZ),N_INT,MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
...
ELSEIF (PRES_ON_CARTESIAN .AND. .NOT.PRES_ON_WHOLE_DOMAIN ) THEN
   ...
   CUTCELL_LOOP_B : DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
...
DO IFACE=1,MESHES(NM)%IBM_NRCFACE_H
   ...
   IF(.NOT.GRADH_ON_CARTESIAN) IDX = 1._EB / ( MESHES(NM)%IBM_RCFACE_H(IFACE)%XCEN(X1AXIS,HIGH_IND) - &
                                               MESHES(NM)%IBM_RCFACE_H(IFACE)%XCEN(X1AXIS,LOW_IND) )
...
FLAG12_COND : IF (FLAG12) THEN
   ...
   IF(PRES_ON_CARTESIAN) THEN ! Use Underlying Cartesian cells: Method 2.
      ...
      ENDDO
   ELSE ! Unstructured pressure solve: Method 1. Note: All cut-cells in a cartesian cell take same pressure value.
      DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
         NCELL = CUT_CELL(ICC)%NCELL
         NUNKH_LC(NM) = NUNKH_LC(NM) + 1
