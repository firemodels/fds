...
MESH_LOOP: DO NM=1,NMESHES
   ...
   DO N=1,N_SMOKE3D
      ...
      IF (RESTART) LU_SMOKE3D(N+N_SMOKE3D,NM) = ABS(LU_SMOKE3D(N+N_SMOKE3D,NM))
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D) THEN
   ...
   DO N=1,N_SMOKE3D
      ...
      IF (APPEND) THEN
         ...
         OPEN(ABS(LU_SMOKE3D(N+N_SMOKE3D,NM)),FILE=FN_SMOKE3D(N+N_SMOKE3D,NM),FORM='FORMATTED',  STATUS='OLD',POSITION='APPEND')
      ELSE
         IF (SMOKE3D_FILE(N)%Y_INDEX > 0) THEN
            MASS_EXT_COEF = REAL(SPECIES(SMOKE3D_FILE(N)%Y_INDEX)%MASS_EXTINCTION_COEFFICIENT,FB)
         ELSEIF (SMOKE3D_FILE(N)%Z_INDEX >= 0) THEN
            MASS_EXT_COEF = REAL(SPECIES_MIXTURE(SMOKE3D_FILE(N)%Z_INDEX)%MASS_EXTINCTION_COEFFICIENT,FB)
         ELSE
            MASS_EXT_COEF = 0.0_FB
         ENDIF    
         OPEN(ABS(LU_SMOKE3D(N,NM)),  FILE=FN_SMOKE3D(N,NM),  FORM='UNFORMATTED',STATUS='REPLACE')
         OPEN(ABS(LU_SMOKE3D(N+N_SMOKE3D,NM)),FILE=FN_SMOKE3D(N+N_SMOKE3D,NM),FORM='FORMATTED',STATUS='REPLACE')
         IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A,I6,F11.3)') 'SMOKF3D',NM,MASS_EXT_COEF
         M%N_STRINGS = M%N_STRINGS + 1
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   IF (SMOKE3D_FILE(N)%QUANTITY_INDEX==0) CYCLE DATA_FILE_LOOP
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            FF(I,J,K)=GAS_PHASE_OUTPUT(T,DT,NM,I,J,K,SMOKE3D_FILE(N)%QUANTITY_INDEX,0,&
                                       SMOKE3D_FILE(N)%Y_INDEX,SMOKE3D_FILE(N)%Z_INDEX,0,0,0,0,0,0)
         ENDDO
      ENDDO
   ENDDO
   ...
   CALL SMOKE3D_TO_FILE(LU_SMOKE3D(N,NM),LU_SMOKE3D(N+N_SMOKE3D,NM),STIME,DXX,MASS_EXT_COEF,N,QQ_PACK,IBP1,JBP1,KBP1,&
