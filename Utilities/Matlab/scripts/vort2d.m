...
%-------------------------------------------------------------------------%
    ...
    DT = 0.0000088914285714;
    
    % Set the flow-through time
        % The time period required for the vortex to
        % pass through the entire computational domain
        % and return to where it was initialized
...
%-------------------------------------------------------------------------%
    ...
    for m = 1:4
        ...
        MatU{m}  = csvread([dir,'vort2d_',meshname,'_devc.csv'],2,0,...
                           [2,0,endrow,endcol]);
    end
...
%-------------------------------------------------------------------------%
    ...
    %-----------------------------------------------------------------%
        ...
        % Save plot to file
        print(gcf,'-dpdf',[dir,'vort2d_',meshname,'_uxgraph']);
    ...
    %-----------------------------------------------------------------%
        ...
        % Save plot to file
        print(gcf,'-dpdf',[dir,'vort2d_',meshname,'_uzgraph']);
    ...
    %-----------------------------------------------------------------%
        ...
        % Save plot to file
        print(gcf,'-dpdf',[dir,'vort2d_',meshname,'_upgraph']);
...
%-------------------------------------------------------------------------%
    ...
    % Plot first-order error reference line
    DXGRAPH=loglog(dx,8*dx,'--');
    
    % Hold first-order error plot to be included in subsequent plots
    hold on
    
    % Plot second-order error reference line
    DX2GRAPH=loglog(dx,120*dx2);
    
    % Define plot colors and line thickness
    set(DXGRAPH,'Color',[0 0 0],'LineWidth',1.6);
    set(DX2GRAPH,'Color',[0.5 0.5 0.5],'LineWidth',1.6);
    
    for t = 1:TIMESTEPS
        
        % Plot error values
        ERRORGRAPH=loglog(dx,ERROR(:,t),'-*');
        ...
        % Define plot colors
        set(ERRORGRAPH,'Color',[CFRed CFGreen 0]);
    end

    % Specify axes for above plots
    axis([0.0009 0.0090 0.00005 0.1]);
    
    % Specify legend for above plots
    legend('dx','dx^2','Error','Location','SouthEast');
    
    % Plot all plots together (from hold on)
    hold off
    
    % Save plot to file
    print(gcf,'-dpdf',[dir,'vort2d_errorgraph']);

    % Open data file for reference from FDS_verification_script.m
    RMSFile = fopen([dir,'vort2d_error.csv'],'wt');
