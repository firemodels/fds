...
REAL(EB):: ZZ_GET(0:N_TRACKED_SPECIES),ZZ_MIN=1.E-10_EB,DZZ(0:N_TRACKED_SPECIES),CP_TMP,MW_BAR,RMW_0,RMW_N,H_S_0,H_S_N
   ...
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         ...
         IF (ABS(Q(I,J,K)) > ZERO_P) THEN
            ...
            CALL GET_SPECIFIC_HEAT(ZZ_GET,CP_TMP,TMP(I,J,K))
            CP_TMP = CP_TMP*TMP(I,J,K)            
            CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW_BAR)
            RMW_0 = 1._EB/SPECIES_MIXTURE(0)%MW
            ITMP = MIN(4999,NINT(TMP(I,J,K)))
            H_S_0 = CPBAR_Z(ITMP,0)*TMP(I,J,K)
            DO N=1,N_TRACKED_SPECIES
               RMW_N = 1._EB/SPECIES_MIXTURE(N)%MW
               H_S_N = CPBAR_Z(ITMP,N)*TMP(I,J,K)
               D_REACTION(I,J,K) = D_REACTION(I,J,K) + ( MW_BAR*(RMW_N-RMW_0) - (H_S_N-H_S_0)/CP_TMP )*DZZ(N)/DT
            ENDDO
         ENDIF
