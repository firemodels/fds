...
USE GLOBAL_CONSTANTS, ONLY : LU_ERR,SOLID_BOUNDARY,MIRROR_BOUNDARY,PERIODIC_BOUNDARY,OPEN_BOUNDARY,INTERPOLATED_BOUNDARY,&
                             ...
                             LOWER_MESH_INDEX,FISHPAK_BC,FISHPAK_BC_PERIODIC
#else
USE GLOBAL_CONSTANTS
#endif
USE MESH_VARIABLES
...
INTEGER :: INNZ
#ifdef WITH_MKL
INTEGER :: PHASE, PERM(1), IROW, JCOL
INTEGER :: I
#endif
#ifdef WITH_PETSC
INTEGER :: IROW, JCOL
...
CALL MATSETFROMOPTIONS(ZM%PETSC_MZ%A_H,PETSC_IERR)
DO IROW=1,ZM%NUNKH
   DO JCOL=1,NNZ_H_MAT(IROW)
...
CALL VECSETFROMOPTIONS(ZM%PETSC_MZ%F_H,PETSC_IERR)
...
USE GLOBAL_CONSTANTS, ONLY : LU_ERR,SOLID_BOUNDARY,MIRROR_BOUNDARY,PERIODIC_BOUNDARY,OPEN_BOUNDARY,INTERPOLATED_BOUNDARY,&
                             ...
                             GLMAT_FLAG,STOP_STATUS,SETUP_STOP,PROCESS
#else
USE GLOBAL_CONSTANTS
#endif
USE MESH_VARIABLES
...
#ifdef WITH_PETSC
ALLOCATE(VECIND(0:ZSL%NUNKH_LOCAL-1));
VECIND(0:ZSL%NUNKH_LOCAL-1) =  (/ (ZSL%UNKH_IND(NM_START)+IROW, IROW=0,ZSL%NUNKH_LOCAL-1) /)
CALL VECSETVALUES(ZSL%PETSC_ZS%F_H,ZSL%NUNKH_LOCAL,VECIND(0:ZSL%NUNKH_LOCAL-1),ZSL%F_H(1:ZSL%NUNKH_LOCAL),&
...
CALL VECGETVALUES(ZSL%PETSC_ZS%X_H,ZSL%NUNKH_LOCAL,VECIND(0:ZSL%NUNKH_LOCAL-1),ZSL%X_H(1:ZSL%NUNKH_LOCAL),PETSC_IERR)
DEALLOCATE(VECIND)
#else
#ifdef WITH_MKL
...
! Create PETSc RHS and unknown arrays:
CALL VECCREATE(MPI_COMM_WORLD,ZSL%PETSC_ZS%F_H,PETSC_IERR)                    !
CALL VECSETSIZES(ZSL%PETSC_ZS%F_H,ZSL%NUNKH_LOCAL,ZSL%NUNKH_TOTAL,PETSC_IERR)
CALL VECSETFROMOPTIONS(ZSL%PETSC_ZS%F_H,PETSC_IERR)
CALL VECDUPLICATE(ZSL%PETSC_ZS%F_H,ZSL%PETSC_ZS%X_H,PETSC_IERR)

! Create PETSc linear solver context:
CALL KSPCREATE(MPI_COMM_WORLD,ZSL%PETSC_ZS%LS,PETSC_IERR)
