   ...
   CASE(:1) 
      ...
      IF (.NOT.TWO_D) THEN
         SELECT CASE(SCARC_METHOD)
            ...
            CASE(1)
               CALL SCARC_CG3D(NM,ITRN,POIS_PTB,SAVE1,WORK,HX)
            CASE(2)
               !CALL SCARC_MG3D(NM,ITRN,POIS_PTB,SAVE1,WORK,HX)
         END SELECT
      ENDIF
      ...
      IF (TWO_D .AND. .NOT. CYLINDRICAL) THEN
         SELECT CASE(SCARC_METHOD)
            ...
            CASE(1)
               !CALL SCARC_CG2D(NM,ITRN,POIS_PTB,SAVE1,WORK,HX)
               CALL SCARC_CG2D(BXS,BXF,BZS,BZF,ITRN,PRHS,NM,POIS_PTB,SAVE1,WORK,HX)
            CASE(2)
               !CALL SCARC_MG2D(NM,ITRN,POIS_PTB,SAVE1,WORK,HX)
         ENDIF
      ENDIF
...
! Perform an update of local solutions

IF (SCARC_METHOD>0) CALL SCARC_UPDATE(MYID,NUPDATE_H)
