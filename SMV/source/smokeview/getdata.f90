...
USE BOXTETRA_ROUTINES
IMPLICIT NONE
REAL(FB), INTENT(IN), DIMENSION(2) :: VERT, V1, V2, V3
INTEGER, INTENT(OUT) :: FLAG

REAL(EB), DIMENSION(2) :: VVERT, VV1, VV2, VV3
...
VV2 = REAL(V2,EB)
VV3 = REAL(V3,EB)
IF(IN_TRIANGLE(VVERT,VV1,VV2,VV3,0))FLAG = 1

END SUBROUTINE GET_IN_TRIANGLE

!  ------------------ GET_IS_ANGLE_GE_180 ------------------------

SUBROUTINE GET_IS_ANGLE_GE_180(V1,V2,V3, FLAG)
USE PRECISION_PARAMETERS
USE BOXTETRA_ROUTINES
IMPLICIT NONE
REAL(FB), INTENT(IN), DIMENSION(2) :: V1, V2, V3
INTEGER, INTENT(OUT) :: FLAG

REAL(EB), DIMENSION(2) :: VV1, VV2, VV3
...
FLAG = 0
IF(IS_ANGLE_GE_180(VV1,VV2,VV3))FLAG = 1
END SUBROUTINE GET_IS_ANGLE_GE_180

!  ------------------ FPOLY2TRI ------------------------

SUBROUTINE FPOLY2TRI(VERTS,NVERTS,POLY,NPOLY,TRIS,NTRIS)
USE PRECISION_PARAMETERS
USE BOXTETRA_ROUTINES
IMPLICIT NONE
INTEGER, INTENT(IN) :: NVERTS, NPOLY
REAL(FB), INTENT(IN), DIMENSION(2*NVERTS) :: VERTS
INTEGER, INTENT(IN), DIMENSION(NPOLY) :: POLY
INTEGER, INTENT(OUT) :: NTRIS
INTEGER, INTENT(OUT), DIMENSION(3*(NPOLY-2)) :: TRIS

REAL(EB), DIMENSION(2*NVERTS) :: EVERTS

EVERTS(1:2*NVERTS) = REAL(VERTS(1:2*NVERTS),EB)
CALL POLY2TRI(EVERTS,NVERTS,POLY,NPOLY,TRIS,NTRIS)
