...
PARTICLE_NON_STATIC_IF: IF (.NOT.LPC%STATIC .OR. LP%EMBER) THEN ! Move airborne, non-stationary particles
   ...
   IF (BETA>TWO_EPSILON_EB) THEN
      ...
      LP%ACCEL_X = LP%ACCEL_X + MPOM*(LP%MASS*((U_OLD-LP%U)/DT+GX_LOC) + M_DOT*UREL)
      LP%ACCEL_Y = LP%ACCEL_Y + MPOM*(LP%MASS*((V_OLD-LP%V)/DT+GY_LOC) + M_DOT*VREL)
      LP%ACCEL_Z = LP%ACCEL_Z + MPOM*(LP%MASS*((W_OLD-LP%W)/DT+GZ_LOC) + M_DOT*WREL)
   ELSE
      LP%ACCEL_X  = 0._EB
      LP%ACCEL_Y  = 0._EB
      LP%ACCEL_Z  = 0._EB
