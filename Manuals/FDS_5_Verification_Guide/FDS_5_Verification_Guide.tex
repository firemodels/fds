...
Consider a solid material that undergoes multiple reactions during its thermal decomposition, as evidenced by, for example,
thermal gravimetric analysis (TGA). From this measurement alone, it is not possible to determine the chemical make-up of the solid. However,
if it is assumed that the number of material components making up the solid is equal to the assumed number of reactions, as determined from the TGA
results, the mass fraction of any component, $Y_i(t)$, changes in time according to the equation:
\be
   \frac{dY_i}{dt} = -A_i \, Y_i \, \exp \left( -\frac{E_i}{RT} \right)    \quad ; \quad Y_i(0) = Y_{0,i}  \label{dYdt}
\ee
...
\ee
Note that the formulae for $A_i$ and $E_i$ are appropriate for multiple-step reactions, where the number of peaks in the reaction
...
For each peak, the value of $T_{p,i}$ is readily obtained from the plot.
For TGA data, the values of $r_{p,i}$ can be obtained directly from inspection of the reaction
rate curve, assuming the units are s$^{-1}$. The values of $Y_{0,i}$ can be estimated based on the relative area underneath
the peak.
