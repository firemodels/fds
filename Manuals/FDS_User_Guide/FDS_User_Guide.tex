...
Arrhenius reaction parameters are typically found for a particular range of equivalence ratios, however equivalence ratios within a simulation may exist in multiple ranges. In these cases a piecewise definition of Arrhenius parameters based on equivalence ratio can be used to define the pre-exponential, {\ct A} and activation energy {\ct E} using the {\ct RAMP} function with the defined chemical reaction like this:
...
\end{verbatim}
\normalsize
Here {\ct A\_RAMP} and {\ct E\_RAMP} are the flags on the {\ct REAC} line to indicate that a {\ct RAMP} is being used. Unlike other {\ct RAMP} functions, {\ct T} is the equivalence ratio, {\em not} time or temperature. For Arrhenius ramps, {\ct F} is a fraction of the the quantity {\ct A} or {\ct E}. In this example for equivalence ratios of a 1 or less, the pre-exponential is large and the activation is energy is 0, indicating that reaction is fast. For equivalence ration greater than 1, the reaction parameters are equal to those defined on the {\ct REAC} line.
