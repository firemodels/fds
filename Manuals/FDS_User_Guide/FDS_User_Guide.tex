...
\item[{\ct U0, V0, W0}] Computationally, these are the initial values of the gas velocity in each of the coordinate directions. Normally, these are all 0~m/s, but there are a few applications where it is convenient to start the flow immediately, like in an outdoor simulation involving wind.  Physically, these values should be thought of a ``far-field'' values of the velocity components.  Setting these values affects the pressure boundary condition for {\ct OPEN} vents.  See the FDS Tech Guide \cite{FDS_Math_Guide}.
\end{description}
...
By default, FDS will suspend the pressure iterations if the error is not reduced by at least 25~\% at each step. If you do not want FDS to suspend the pressure iteration, add {\ct SUSPEND\_PRESSURE\_ITERATIONS=.FALSE.} to the {\ct PRES} line. This will be done automatically if you change the default values of {\ct VELOCITY\_TOLERANCE} or {\ct MAX\_PRESSURE\_ITERATIONS}.
...
A common application of FDS is tunnel fires, but simulations of fires in long, relatively tight tunnels can have spurious fluctuations in the pressure field that can lead to numerical instabilities. The root cause of the problem is the way that FDS solves the equation for the perturbation pressure, $\tp$. In the momentum equation, the pressure term is decomposed as follows:
\be
   \frac{1}{\rho} \nabla \tp = \nabla \left( \frac{\tp}{\rho}\right) - \tp \, \nabla \left(\frac{1}{\rho} \right) \label{p_decomp}
\ee
The equation for the pressure perturbation is formed by taking the divergence of the momentum equation. The two terms on the right hand side of Eq.~(\ref{p_decomp}) are treated differently in the pressure equation in that the pressure in the second term (referred to as the baroclinic torque) is taken from the previous time step and the pressure in the first term is the variable that is solved for. The reason for doing this is that the fast Poisson solver used in FDS only works with \emph{separable} equations; that is, equations of the form:
\be
   \nabla^2 \left( \frac{\tp^n}{\rho^n} \right) = \tp^{n-1} \, \nabla \left(\frac{1}{\rho^n} \right) + \; \cdots
\ee
To alleviate numerical problems related to a mismatch between the old and new pressure fields, the pressure equation is solved iteratively until the change in the baroclinic term drops below a specified tolerance. By default, the {\ct PRESSURE\_TOLERANCE} is $20/\dx^2$ where $\dx$ is the characteristic grid cell size. If a numerical instability occurs in a simulation involving a tunnel, you could try reducing this value to alleviate the mismatch between old and new pressure fields.
...
Figure~\ref{pressure_rise} shows the results of this calculation.  The density matches exactly showing
...
\label{zone_shape_example}

This example is similar to the ones in the previous section, except in this case, the pressure zone is L-shaped and split across two meshes. The objective is simply to ensure that the specification of the pressure zone is properly accounted for in the model. Figure~\ref{zone_shape} compares the predicted pressure in the compartment compared to an exact solution. Air is injected into the compartment for 5~s, after which the compartment is opened to a smaller compartment. At 15~s, the smaller compartment is opened to the outside.
...
Diffusivity is assumed to be the binary diffusion coefficient between the given species and the background species. Diffusivty can be specified in one of three ways:  it can be defined as a constant using {\ct DIFFUSIVITY} (m$^2$/s), it can be defined as a temperature vs. diffusivity ramp  using {\ct RAMP\_D}, or it can be computed by FDS using {\ct MW} and the Lennard-Jones potential parameters $\sigma$ ({\ct SIGMALJ}) and $\epsilon/k$ ({\ct EPSILONKLJ}).  If no inputs are specified, FDS will compute the diffusivity using {\ct MW} and the Lennard-Jones parameters for air.
...
The dynamic viscosity of the gas species can be specified in one of three ways:  it can be defined as a constant using {\ct VISCOSITY}, it can be defined as a temperature vs. viscosity ramp  using {\ct RAMP\_MU}, or it can be computed  using the Lennard-Jones potential parameters $\sigma$ ({\ct SIGMALJ}) and $\epsilon/k$ ({\ct EPSILONKLJ}).  If no viscosity inputs are provided, FDS will use the Lennard-Jones values for air.
...
fire as thermal radiation, commonly referred to as the {\em radiative fraction}, symbolically denoted $\chi_{\rm r}$. It is a function of both the flame temperature
...
As a practical alternative, the parameter {\ct RADIATIVE\_FRACTION} on the {\ct REAC} line allows you to specify explicitly the
...
on the gas temperature and the chemical composition. By default, the {\ct RADIATIVE\_FRACTION} is based on the reaction's {\ct FUEL} for an LES calculation (see \ref{tab:chi_r}), and zero for DNS.
...
\ee
If the heat flux gauge used in an experiment has a temperature other than ambient or an emissivity other than that of the surrounding surface, set {\ct GAUGE\_TEMPERATURE} ($T_{\rm G}$) and {\ct GAUGE\_EMISSIVITY} ($\epsilon$) on the {\ct PROP} line associated with the device.  When comparing against a radiometer measurement, use {\ct RADIOMETER}:
\be
...
measurement are defined using an associated {\ct PROP} line.
...
%{\ct RAMP\_U0}                                  & Character     & Section~\ref{info:mean_forcing}                       &               & 'null'            \\ \hline
%{\ct RAMP\_V0}                                  & Character     & Section~\ref{info:mean_forcing}                       &               & 'null'            \\ \hline
%{\ct RAMP\_W0}                                  & Character     & Section~\ref{info:mean_forcing}                       &               & 'null'            \\ \hline
%{\ct RAMP\_U0\_Z}                               & Character     & Section~\ref{info:mean_forcing}                       &               & 'null'            \\ \hline
%{\ct RAMP\_V0\_Z}                               & Character     & Section~\ref{info:mean_forcing}                       &               & 'null'            \\ \hline
%{\ct RAMP\_W0\_Z}                               & Character     & Section~\ref{info:mean_forcing}                       &               & 'null'            \\ \hline
...
{\ct \footnotesize SECOND\_ORDER\_INTERPOLATED\_BOUNDARY}     & Logical       & Section~\ref{info:flux_limiters}                      &               & {\ct .FALSE.}     \\ \hline
...
{\ct \footnotesize BEAD\_HEAT\_TRANSFER\_COEFFICIENT} & Real          & Section~\ref{info:THERMOCOUPLE}           & \si{W/(m$^2$.K)}      &           \\ \hline
