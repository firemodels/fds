...
SELECT CASE (FA%FAN_TYPE)
   ...
   CASE(4) ! System curve-based quadratic fan BETA
         ...
         FUNC = FA%MAX_PRES/((DU%VEL_SYSTEM(2,DU%QFAN_N,2)-DU%VEL_SYSTEM(1,DU%QFAN_N,2))*DU%AREA)**2
         FUNC = FUNC * (FLOW1-DU%VEL_SYSTEM(1,DU%QFAN_N,2)*DU%AREA)**2
         FUNC = FUNC - FA%MAX_PRES + FA%MAX_PRES*(FLOW1/FA%MAX_FLOW)**2
         IF (FUNC > 0) THEN
            FLOW2 = FLOWGUESS
         ELSE
            FLOW1 = FLOWGUESS
