...
HEAT_FLUX_LOOP: DO IW=1,NWC
   ...
   IF (SF%THERMAL_BC_INDEX /= NO_CONVECTION .AND. SF%THERMAL_BC_INDEX /= INTERPOLATED_BC) THEN
      ITMP = MIN(500,NINT(0.05_EB*(TMP(IIG,JJG,KKG)+TMP_W(IW))))
      IF (N_SPECIES > 0) THEN
...
! Determine ghost cell value of RSUM=R0*Sum(Y_i/M_i) 
   IF (N_SPECIES>0) THEN
      YY_GET = YY_W(IW,:)
      CALL GET_MOLECULAR_WEIGHT(YY_GET,RSUM_W(IW))
...
REAL(EB) :: SMALLEST_CELL_SIZE(MAX_LAYERS),THICKNESS,LAYER_THICKNESS_NEW(MAX_LAYERS),YY_G(1:N_SPECIES),YY_GET(1:N_SPECIES)
