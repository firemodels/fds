...
INTEGER :: ITER,INDX,N,I,J,K,NN,IL,III,JJJ,KKK,IPC
...
SELECT CASE(IND)
   ...
   CASE(231:239) ! {PART_ID}_MEDIAN_DIAMETER
      DLOOP: DO I=1,NLP
         DR=>DROPLET(I)
         IPC=DR%CLASS
         PC=>PARTICLE_CLASS(IPC)
         IF (PC%EVAP_INDEX/=IND-230) CYCLE DLOOP
         IF (DR%X<X(II-1) .OR. DR%X>X(II) .OR. DR%Y<Y(JJ-1) .OR. DR%Y>Y(JJ) .OR. DR%Z<Z(KK-1) .OR. DR%Z>Z(KK)) CYCLE DLOOP
         DV%SUM_1 = DV%SUM_1 + DR%PWT*DR%R**4
         DV%SUM_2 = DV%SUM_2 + DR%PWT*DR%R**3
      ENDDO DLOOP
      IF (DV%SUM_2>0._EB) THEN
         GAS_PHASE_OUTPUT = 2.E6_EB*DV%SUM_1/DV%SUM_2
      ELSE
         GAS_PHASE_OUTPUT = 0._EB
      ENDIF
