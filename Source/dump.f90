...
! Initialize isosurface file
   ...
   DO N=1,N_ISOF
      ...
      ELSE
        ...
        ELSE
           WRITE(M%STRING(M%N_STRINGS),'(A,I6,1X,I6)') 'ISOV',NM,IS%SKIP
        ENDIF
        M%N_STRINGS = M%N_STRINGS + 1
        WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(FN_ISOF2(N,NM))
        M%N_STRINGS = M%N_STRINGS + 1
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   CALL ISO_TO_FILE(LU_ISOF(N,NM),LU_ISOF2(N,NM),NM,STIME,QQ,QQ2,HAVE_ISO2,&
