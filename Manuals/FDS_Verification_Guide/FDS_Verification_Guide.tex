...
In FDS, the numerical stability of the CFD program is of utmost importance. In this section we present an analysis of the stability a two-dimensional vortex. We set up an analytically stable flow pattern consisting of a single stable vortex in a constant flow field and attempt to simulate it using FDS. We tested the stability of the regime under a range of square, two-dimensional meshes, 40, 80, 160, and finally 320 cells on a side. This allowed us to determine the numerical stability of our CFD program as a function of mesh size.

\begin{figure}[h]
   \begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/vort2d_40_ugraph2}  &
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/vort2d_80_ugraph2} \\
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/vort2d_160_ugraph2} &
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/vort2d_320_ugraph2}
   \end{tabular*}
   \caption{Contour plots of the u-velocity field of the vortex after one flow-through time at each grid resolution.}
   \label{fig_vort2d_main}
\end{figure}

Analytically, our flow pattern was governed by the equation
\begin{equation}
\label{eqn_vort2d}
\Psi_{0} (x,z) = \Gamma \ Exp \left[ - \frac{x^{2} + z^{2}}{2 \ R_{c}^{2}} \right],
\end{equation}
where $\Gamma$ is a parameter that determines the intensity of the vortices and $R_{c}$ sets the characteristic size of the vortices. This flow pattern is defined such that the point at the center of the two vortices is at $(x,y) = (0,0)$.

The velocity-component fields may be determined by taking the partial derivative of this governing equation with respect to each direction, yielding
\begin{eqnarray}
\label{eqn_derivatives}
\frac{\partial}{\partial x} \Psi_{0} &=& - \frac{\Gamma \ x}{R_{c}^{2}} \ Exp \left[ - \frac{x^{2} + z^{2}}{2 \ R_{c}^{2}} \right], \\
\frac{\partial}{\partial z} \Psi_{0} &=& - \frac{\Gamma \ z}{R_{c}^{2}} \ Exp \left[ - \frac{x^{2} + z^{2}}{2 \ R_{c}^{2}} \right].
\end{eqnarray}

Taking into account the constant flow field, $U_{0}$, the individual velocity-component fields may be stated as
\begin{eqnarray}
\label{eqn_uvel}
u (x,z) &=& U_{0} + \frac{\partial}{\partial z} \Psi_{0} \notag \\
&=& U_{0} - \frac{\Gamma \ z}{R_{c}^{2}} \ Exp \left[ - \frac{x^{2} + z^{2}}{2 \ R_{c}^{2}} \right],
\end{eqnarray}
\begin{eqnarray}
\label{eqn_wvel}
w (x,z) &=& - \frac{\partial}{\partial x} \Psi_{0} \notag \\
&=& \frac{\Gamma \ x}{R_{c}^{2}} \ Exp \left[ - \frac{x^{2} + z^{2}}{2 \ R_{c}^{2}} \right],
\end{eqnarray} where $u$ and $v$ refer to velocity in the $x$- and $y$-directions respectively. For our purposes we need only analyze one component of the velocity field. We will focus our attention on the u-component of velocity.

\begin{figure}[t]
   \begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/vort2d_80_ugraph4} &
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/vort2d_160_ugraph4}
   \end{tabular*}
   \caption{Contour plot of u-velocity field after three flow-through times at grid resolutions of 6400 (Left) and 25600 (Right).}
   \label{fig_vort2d_velocitycontours}
\end{figure}

We define the computational domain as a two-dimensional square region, $L=0.3112$ m on a side, with periodic boundary conditions. As previously stated, this domain is discretized upon meshes of several different sizes. Each square mesh divides the computational domain equally across 1600, 6400, 25600, or 102400 grid cells, corresponding to meshes that are 40, 80, 160, or 320 cells on a side respectively. For the purposes of this test, we set the flow parameters as
$$
\begin{array}{lll}
U_{0} &&= 35.0 \ \mathrm{m \ s^{-1}}, \\
R_{c} &= L / 20 &= 1.55600 \times 10^{-2} \ \mathrm{m}, \\
\Gamma &= 0.04 \ U_{0} \ R_{c} \ \sqrt{e} &= 3.59157 \times 10^{-2}.
\end{array}
$$

FDS simulates the associated pressure field for us, but in order to do so, we must specify a few other standard parameters,
\begin{eqnarray*}
T_{0} &=& 300 \ \mathrm{K}, \\
P_{0} &=& 1.01300 \times 10^{5} \ \mathrm{Pa}, \\
\rho_{0} &=& 1.17170 \ \mathrm{kg \ m^{-3}}.
\end{eqnarray*}
...
\begin{eqnarray*}
t_{f} &=& L / U_{0} \\
&\approx& 8.8914 \times 10^{-3} \ \mathrm{s}.
\end{eqnarray*}
...
A contour plot of the u-velocity field in the region of the vortex provides a simple characterization of the vortex's geometry. The extent to which this geometry changes over time provides a qualitative measure of the stability of the vortex. Figure 3.5 demonstrates the effectiveness of doubling the grid resolution by portraying the relative distortion of the vortex geometry after two flow-through times.

A more quantitative measure of the stability of the vortex may be found by plotting the u-velocity values just along the z-axis. Figure 3.6 displays such plots for two different grid resolutions. Each line represents a plot taken for a different number of flow-through times such that the red lines represent the vortex after it has undergone the most loops through the computational domain while the green lines represent the vortex in the initial first couple of loops.
...
\begin{figure}[t]
   ...
   \end{tabular*}
   \caption{u-velocity along the z-axis at $x=0$ plotted for each of the vortex's first four loops through the computational domain. (Left) Grid resolution of 6400. (Right) Grid resolution of 25600.}
...
The rate with which the simulated profiles converge to the analytical ones by means of grid refinement describes the order of error experienced as a result of our numerical methods. In Figure 3.7 we plot the rms error resulting from differences between the simulated and analytical profiles for each grid resolution.

\begin{center}
   \begin{figure}[t]
         \includegraphics[width=\textwidth]{SCRIPT_FIGURES/vort2d_error}
...
\begin{figure}[t]
   ...
   \end{tabular*}
   \caption{Simulated and analytical values of u-velocity at a point plotted as a function of time over a time period equal to four times the flow-through time. (Left) Grid resolution of 6400. (Right) Grid resolution of 25600.}
