...
INTEGER :: NR,NS,NS2,NFR,PRIORITY,INT_PROD_COUNTER
...
COUNT_REAC_LOOP: DO
   ...
   IF (A < 0._EB .AND. E < 0._EB .AND. TRIM(SPEC_ID_NU(1))=='null' .AND. TRIM(EQUATION)=='null') THEN
      ...
      IF (N_SIMPLE_CHEMISTRY_REACTIONS(N_REACTIONS)==0) N_SIMPLE_CHEMISTRY_REACTIONS(N_REACTIONS)==1
      IF (N_SIMPLE_CHEMISTRY_REACTIONS(N_REACTIONS)==2) THEN
         N_REACTIONS = N_REACTIONS + 1
         N_SIMPLE_CHEMISTRY_REACTIONS(N_REACTIONS)==-2
      ENDIF
   ELSE
      N_SIMPLE_CHEMISTRY_REACTIONS(N_REACTIONS)==0
   ENDIF
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (N_SIMPLE_CHEMISTRY_REACTIONS(NR)==-2) THEN
      CALL SET_REAC_DEFAULTS
      REACTION(NR)%SIMPLE_CHEMISTRY = .TRUE.
      INT_PROD_COUNTER = INT_PROD_COUNTER + 1
      IF (INT_PROD_COUNTER==1) THEN
         FUEL = 'INTERMEDIATE PRODUCTS'
      ELSE
         WRITE(FUEL,'(A,I0))') FUEL = 'INTERMEDIATE PRODUCTS ',INT_PROD_COUNTER
      ENDIF
      PRIORITY = 2
   ELSE
      CALL CHECKREAD('REAC',LU_INPUT,IOS)  ; IF (STOP_STATUS==SETUP_STOP) RETURN
      ...
      CALL SET_REAC_DEFAULTS
      READ(LU_INPUT,REAC)
      IF (N_SIMPLE_CHEMISTRY_REACTIONS(NR)>0) REACTION(NR)%SIMPLE_CHEMISTRY = .TRUE.
   ENDIF
