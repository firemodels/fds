...
!    I11   I21      
   ...
   IF (N_VERTS.GT.0) THEN
      ...
      CALL ChkMemErr('READ_GEOM','G%TVERTS',IZERO)
   ENDIF
...
MAX_ZVALS=MAX(MAX_ZVALS,1000)
MAX_VERTS=MAX(MAX_VERTS,1000)
MAX_FACES=MAX(MAX_FACES,1000)
MAX_VOLUS=MAX(MAX_VOLUS,1000)
...
SUBROUTINE GEOM2TEXTURE
   INTEGER :: I,J,K,JJ
   TYPE(GEOMETRY_TYPE), POINTER :: G=>NULL()
   REAL(EB), POINTER, DIMENSION(:) :: XYZ, TVERT
      ...
      IF (G%TEXTURE_MAPPING .EQ. 'RECTANGULAR') THEN
         DO J = 1, G%N_FACES
            ...
            DO K = 1, 3
               ...
               TVERT(1:2) => G%TVERTS(2*JJ-1:2*JJ)
               TVERT(1:2) = (XYZ(1:2) - G%TEXTURE_ORIGIN(1:2))/G%TEXTURE_SCALE(1:2)
            END DO
         END DO
      ELSE
         DO J = 1, G%N_FACES
            ...
            DO K = 1, 3
               ...
               TVERT(1) = 0.5_EB + 0.5_EB*AZ/PI ! scale from (-PI,PI) to (0.0,1.0)

               ELEV = ATAN2(DXYZ(3),XYNORM) ! ranges from -PI/2 to +PI/2 (since XYNORM>=0)
               TVERT(2) = 0.5_EB + ELEV/PI  ! scale from (-PI/2,PI/2) to (0.0,1.0)
            END DO
         END DO
      ENDIF
   END DO
...
SUBROUTINE MERGE_GEOMS(VERTS,TVERTS,N_VERTS,FACES,SURF_IDS,N_FACES,VOLUS,MATL_IDS,N_VOLUS,IS_DYNAMIC)
...
LOGICAL, INTENT(IN) :: IS_DYNAMIC
REAL(EB), DIMENSION(:), INTENT(OUT) :: VERTS(3*N_VERTS), TVERTS(2*N_VERTS)
INTEGER, DIMENSION(:), INTENT(OUT) :: FACES(3*N_FACES), VOLUS(4*N_VOLUS), MATL_IDS(N_VOLUS), SURF_IDS(N_FACES)

INTEGER :: I
TYPE(GEOMETRY_TYPE), POINTER :: G=>NULL()
INTEGER :: IVERT, ITVERT, IFACE, IVOLUS, IMATL, ISURF, OFFSET
...
DO I = 0, N_GEOMETRY
   ...
   IF (.NOT.G%IS_DYNAMIC.AND.IS_DYNAMIC) CYCLE
    ...
    IF (G%N_VERTS>0) THEN
      ...
      TVERTS(1+ITVERT:2*G%N_VERTS+ITVERT) = G%TVERTS(1:2*G%N_VERTS)
      IVERT = IVERT + 3*G%N_VERTS
      ITVERT = ITVERT + 2*G%N_VERTS
   ENDIF
...
SUBROUTINE CONVERTGEOM(IS_DYNAMIC,TIME)
   ...
   REAL(EB), ALLOCATABLE, DIMENSION(:) :: VERTS, TVERTS
   INTEGER :: IZERO
   LOGICAL :: EX
   INTEGER :: NS, NNN
   ...
   IF (N_VERTS.GT.0.AND.(N_FACES.GT.0.OR.N_VOLUS.GT.0)) THEN
      ...
      ALLOCATE(TVERTS(2*N_VERTS),STAT=IZERO)   ! create arrays to contain all vertices and faces
      ...
      CALL MERGE_GEOMS(VERTS,TVERTS,N_VERTS,FACES,SURF_IDS,N_FACES,VOLUS,MATL_IDS,N_VOLUS,IS_DYNAMIC)
   ENDIF
...
SUBROUTINE OUTGEOM(LUNIT,IS_DYNAMIC,TIME)
   ...
   ALLOCATE(TVERTS(MAX(1,2*N_VERTS)),STAT=IZERO)
   CALL ChkMemErr('OUTGEOM','VERTS',IZERO)
   ...
   IF (N_VERTS>0) THEN
      ...
      WRITE(LUNIT) (REAL(TVERTS(I),FB), I=1,2*N_VERTS)
   ENDIF
   IF (N_FACES>0) THEN
