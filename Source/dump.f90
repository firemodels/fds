...
CALL GET_GEOMSIZES(SLICETYPE,I1,I2,J1,J2,K1,K2,NVERTS,NVERTS_CUTCELLS,NFACES,NFACES_CUTCELLS)
IF (NVERTS>0 .AND. NFACES>0) THEN
! remove !x comments to test node centered geometry slice data output
!x   IF (CC_CELL_CENTERED) THEN
     NVALS = NFACES
...
!x   ELSE
!x     NVALS = NVERTS
!x     ALLOCATE(VALS(NVERTS))
!x     ! use dummy values until GETGEOMVALS or equivalent is working for this case
!x     DO I = 1, NVALS
!x        VALS(I) = REAL(I,FB)/REAL(NVERTS, FB)
!x     ENDDO
!x!     CALL GET_GEOMVALS(CC_FACE_CENTERED,CC_CELL_CENTERED,SLICETYPE,&
!x!                       I1,I2,J1,J2,K1,K2,NFACES,NFACES_CUTCELLS,VALS,&
!x!                       IND,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,MATL_INDEX,T,DT,NM)
!x   ENDIF
ELSE
   NVERTS=0
   NFACES=0
...
!xIF (CC_CELL_CENTERED) THEN
   WRITE(FUNIT_DATA) INTEGER_ZERO, INTEGER_ZERO, INTEGER_ZERO, NVALS
!xELSE
!x   WRITE(FUNIT_DATA) INTEGER_ZERO, INTEGER_ZERO, NVALS,        INTEGER_ZERO
!xENDIF
IF (NVERTS>0 .AND. NFACES>0) THEN
   IF (DEBUG .EQ. 0) THEN
