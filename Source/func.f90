...
!> \brief Determines the size of the BOUNDARY_RADIA derived type
...
INTEGER :: N_NEW_STORAGE_SLOTS,OD_INDEX,BP_INDEX,BR_INDEX,BC_INDEX
...
IF (MINVAL(M%BOUNDARY_RADIA_OCCUPANCY)==1) THEN  ! There are no open slots for boundary coordinates
   N_NEW_STORAGE_SLOTS = 1000
...
IF (BR_INDEX==0) BR_INDEX = MINLOC(M%BOUNDARY_RADIA_OCCUPANCY,DIM=1) ; M%BOUNDARY_RADIA_OCCUPANCY(BR_INDEX) = 1

BR => M%BOUNDARY_RADIA(BR_INDEX)

IF (ALLOCATED(BR%BAND)) DEALLOCATE(BR%BAND)
ALLOCATE(BR%BAND(NUMBER_SPECTRAL_BANDS))
DO NN=1,NUMBER_SPECTRAL_BANDS
   IF (ALLOCATED(BR%BAND(NN)%ILW)) DEALLOCATE(BR%BAND(NN)%ILW)
   ALLOCATE(BR%BAND(NN)%ILW(NUMBER_RADIATION_ANGLES)) ; BR%BAND(NN)%ILW = 0._EB
ENDDO
IF (ALLOCATED(BR%IL)) DEALLOCATE(BR%IL)
ALLOCATE(BR%IL(BOUNDARY_RADIA_REALS_ARRAY_SIZE(2))) ; BR%IL = SIGMA*TMPA4/PI
