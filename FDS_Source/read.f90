...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_DROPLETS,ALLOW_UNDERSIDE_DROPLETS,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                ...
                DEARDORFF,DNS,DYNSMAG,ENTHALPY_TRANSPORT,&
...
READ_SMIX_LOOP: DO N=0,N_TRACKED_SPECIES  
   ...
   DO NS = 1,N_SPECIES
      ...
      SM%MW = SM%MW + SM%VOLUME_FRACTION(NS) * SPECIES(NS)%MW * SM%ADJUST_NU
      IF (SPECIES(NS)%FORMULA(1:5)=='SPEC_') SM%VALID_ATOMS=.FALSE.
      SM%ATOMS = SM%ATOMS + SM%VOLUME_FRACTION(NS)*SPECIES(NS)%ATOMS*SM%ADJUST_NU
   ENDDO     
   SM%RCON = R0/SM%MW
