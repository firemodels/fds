...
EVAC_ONLY5: IF (ANY(EVACUATION_GRID).AND.MYID==MAX(0,EVAC_PROCESS)) THEN
   ...
   DO N=1,N_ENTRYS
      ...
      ELSE
         WRITE(LU_SMV,'(3F6.2,3I6)') EVAC_ENTRYS(N)%HEIGHT, 0.05, EVAC_ENTRYS(N)%WIDTH, EVAC_ENTRYS(N)%RGB(1:3)
      ENDIF
   ENDDO
   DO N=1,N_SSTANDS
...
SELECT CASE(INDX)
   ...
   CASE( 8) ! HRRPUM
      SOLID_PHASE_OUTPUT = MASSFLUX_ACTUAL(IWX,I_FUEL)*0.001*REACTION(1)%HEAT_OF_COMBUSTION/SURFACE(IJKW(5,IWX))%SURFACE_DENSITY
