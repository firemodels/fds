...
\end{lstlisting}
which means that the first three materials of layer 1 have mass fractions of 0.5, 0.4, and 0.1, respectively. The notation {\ct 1:3} means array element 1 through 3, inclusive.
...
\item {\ct TITLE} is a string of 60 characters or less that describes the simulation. It is simply descriptive text that is passed to various output files.
\end{description}
...
{\ct MIRROR} is exactly reversed\footnote{Note that the mirror image of a scene is {\em not} shown in Smokeview.}. From a numerical point of view, a {\ct MIRROR} is a no-flux, free-slip boundary. As with {\ct OPEN}, a {\ct MIRROR} can only be prescribed at an exterior boundary of the computational domain. Often, {\ct OPEN} or {\ct MIRROR} {\ct VENT}s are prescribed along an
...
\be \dq_c'' = h \; \Delta T
    ...
    \max \; \left[ \; C \, |\Delta T|^\ot \; , \;
            ...
            \right]  \quad
    \si{W/(m^2.K)} \ee
where $\Delta T$ is the difference between the wall and the gas temperature, $C$ is the coefficient for natural convection (1.52 for a horizontal surface
...
\be \NU = \left\{ \begin{array}{ll} 0.037 \; \RE^\fofi \; \PR^\ot & \hbox{Cartesian} \\
                                    C_{\rm cyl}\; \RE^{n_{\rm cyl}} \; \PR^\ot & \hbox{Cylindrical} \\
                                    2 + 0.6 \; \RE^\ha \; \PR^\ot & \hbox{Spherical}  \\
                                    C_1 + C_2 \; \RE^{C_3} \; \PR^{C_4} & \hbox{Custom} \end{array} \right. \ee
where $C_{\rm cyl}$ and $n_{\rm cyl}$ are given in Table~\ref{tbl:cylheat}.
\begin{table}[ht]
\begin{center}
\caption{Cylinder Heat Transfer Constants}
\label{tbl:cylheat}
\begin{tabular}{|c|c|c|}
\hline
$C_{\rm cyl}$ & $n_{\rm cyl}$ & Range  \\ \hline
...
\be
  \dod{Y_{{\rm s},i}}{t} = - \sum_{j=1}^{N_{{\rm r},i}} r_{ij} + \sum_{i'=1}^{N_{\rm m}} \sum_{j=1}^{N_{{\rm r},i'}} \nu_{{\rm s},i'j} \; r_{i'j} \quad (i' \neq i)  \quad ; \quad
  r_{ij} = A_{ij} \; Y_{{\rm s},i}^{n_{{\rm s},ij}} \; \exp \left(-\frac{E_{ij}}{R \, T_{\rm s}} \right) \quad ; \quad Y_{{\rm s},i} = \left( \frac{\rho_{{\rm s},i}}{\rho_{\rm s}(0)} \right)
...
\be
   E = \frac{{\rm e} \, r_{\rm p}}{Y_0} \, \frac{R \, T_{\rm p}^2}{\dot{T}} \quad ; \quad
   A = \frac{{\rm e} \, r_{\rm p}}{Y_0} \, {\rm e}^{E/R \, T_{\rm p}} \label{AandE}
\ee
...
undergoing heating at a rate of 5~K/min. The green curve represents the reaction rate, $-\d Y/\d t$. The system of ordinary differential equations that describe the transformation is shown at right. Note that the parameters $T_{\rm p}$, $r_{\rm p}$, and $\nu_{\rm s}$ represent the ``reference'' temperature,
...
\be
   \frac{r_{\rm p}}{Y_0} = \frac{2 \, \dot{T}} {\Delta T} \, (1-\nu_{\rm s}) \label{r_p}
\ee
...
\be t_{\rm b} = \frac{\rho_{\rm s} \, \delta_{\rm s}}{\dm''} \ee
...
\be t_{\rm b} = \frac{\rho_{\rm s} \, \delta_{\rm s} \, \Delta H}{\dq''_{\rm f}} \ee
...
\be T_{\rm fluid,out}=\frac{c_{p,{\rm gas}} u_{\rm duct} A_{\rm duct} \rho_{\rm duct} T_{\rm duct,in} + c_{p,{\rm fluid}} \dm_{\rm fluid} T_{\rm fluid,in}}{c_{p,{\rm gas}} u_{\rm duct} A_{\rm duct} \rho_{\rm duct} + c_{p,{\rm fluid}} \dm_{\rm fluid}}
\ee
...
\be  \dot{V}_{\rm leak} = A_L \; \hbox{sign} (\Delta p) \; \sqrt{2 \frac{ | \Delta p |}{\rho_\infty}} \ee
...
\ee
where $\cal R$ is the universal gas constant, $g$ is the acceleration of gravity, and $\overline{W}$ is the average molecular weight of the air, $z$ is the height above the {\ct GROUND\_LEVEL}, and $T_0$ is the ambient temperature. Applying this formula to the external and internal locations at the lower and upper vents results in densities of 1.2412, 1.1989, 1.2272, and 1.1858~kg/m$^3$, respectively.
...
\be
   \Delta p = \frac{\overline{W} \, p_0(z) \, g \, \Delta z} {\cal R} \left( \frac{1}{T_\infty} - \frac{1}{T_{\rm b}} \right)
\ee
