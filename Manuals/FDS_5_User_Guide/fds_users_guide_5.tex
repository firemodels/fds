...
\Huge
\flushright{Fire Dynamics Simulator (Version 5) \\ User's Guide \\ (Draft, May 22, 2007)}
...
500~$^\circ$. Burning shall continue for 5~min, and then ramp-down 
in 10~s. Note that the time {\ct T} in the {\ct RAMP} means time from ignition.

\begin{warning}
Be aware of the distinction between {\ct IGNITION\_TEMPERATURE} and
{\ct REFERENCE\_TEMPERATURE}. Use the former {\em only} if you want to
add the extra term to Eq.~(\ref{rr}) {\em or} to specify a temperature 
...
production of gaseous fuel, the heat of reaction can be specified for
the material. For example, when using the lines below, the net heat
flux at the material surface is reduced by a factor corresponding to a
product of burning rate and heat of reaction 1000.0 kJ/kg. 
...
{\ct MATL} line. The thermal properties are assumed to be the same as
for a solid material. The evaporation rate of the fuel 
...
the model. The {\ct HEAT\_OF\_REACTION} in this case is the latent
...
{\ct ID     }                 & Character   & IDentifier                  &                     &        \\ \hline
{\ct IGNITION\_TEMPERATURE}   & Real        & Ignition temperature & $^\circ$C           &        \\ \hline
...
{\ct REFERENCE\_RATE}         & Real        & Reaction rate at
	reference temperature       & kg/m$^3$s           & 0.10   \\ \hline
