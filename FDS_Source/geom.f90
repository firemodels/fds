...
READ_FACE_LOOP: DO I=1,N_FACE
   ...
   ! Allocate 1D array for diffusivity
...
GEOC_SURF_LOOP: DO N=1,N_SURF
   IF (TRIM(SURFACE(N)%GEOC_FILENAME)=='null') CYCLE
   
   IF (ABS(T-T_BEGIN)<ZERO_P .AND. LU_GEOC<0) THEN

      FN = TRIM(SURFACE(N)%GEOC_FILENAME)
...
BNDC_SURF_LOOP: DO N=1,N_SURF
   IF (TRIM(SURFACE(N)%BNDC_FILENAME)=='null') CYCLE
   
   IF (ABS(T-T_BEGIN)<ZERO_P .AND. LU_BNDC<0) THEN

      FN = TRIM(SURFACE(N)%BNDC_FILENAME)
      INQUIRE(FILE=FN,EXIST=EX,OPENED=OP,NUMBER=LU)
