...
In this section, a series of liquid pool fires with different fuels is considered. The pool fires are assumed to burn in a 1~m by 1~m pool configuration, and the fuel layer is 10~cm thick. The burning rate of liquid hydrocarbon fuels has been found to correlate well with~\cite{SFPE}:
\begin{equation}
\dot{m}^{\prime\prime}=1\times10^{-3}\frac{\Delta h_c}{\Delta h_{v,sen}} ; \quad \Delta h_{v,sen} = \Delta h_v + \int_{T_0}^{T_b} c_p dT
\end{equation} 
where $\Delta h_c$ is the heat of combustion, $\Delta h_v$ is the sensible heat of vaporization, $T_0$ is the initial temperature, $T_b$ is the boiling point temperature, and $c_p$ is the specific heat of the liquid fuel. The sensible heat of vaporization $\Delta h_{v,sen}$ is the amount of energy required to raise the fuel from its initial temperature to its boiling temperature and evaporate it. Figure~\ref{POOL_MLR} shows a comparison of predicted burning rates with data taken from~\cite{SFPE}. 
...
\end{tabular}
\caption{Fuel properties used in liquid pool fires.}
\end{center}
\end{table}
...
\end{tabular*}
\caption[Mass loss rate of poly(butylene terephtalate) (PBT).]{Comparison of burning rates for various liquid pool fires.}
\label{POOL_MLR}
\end{figure}
