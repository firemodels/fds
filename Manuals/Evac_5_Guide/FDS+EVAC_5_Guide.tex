\documentclass[12pt,a4paper]{stylevk}
...
% Abstract: Arial 18pt bold
% Acknowledgements: Arial 18pt bold
%
...
\begin{titlepage}
  ...
  \begin{center}
    {\bf \huge Fire Dynamics Simulator with Evacuation}\\[0.2em]

    {\bf \huge FDS+Evac, version 5.}\\[0.2em]

    {\bf \huge Technical Reference and User's Guide}\\[0.2em]

    {\large (FDS 5.2.0, Evac 2.0.0, SVN Revision 2186)}
  \end{center}
  \vspace{30pt}
  \begin{center}
...
This document describes, how to simulate human egress using the Fire
Dynamics Simulator (FDS)~\cite{FDS_Manual, FDS_UserGuide} developed at
National Institute of Standards and Technology (NIST), and in
particular, the evacuation module~\cite{Korhonen05, Korhonen07a,
Korhonen07b, Korhonen08a, Korhonen08b} developed at VTT Technical
Research Centre of Finland.  This combined fire and evacuation
simulation program is called FDS+Evac in this document.  FDS+Evac
allows simultaneous simulation of the fire and evacuation processes.
FDS+Evac can also be used to simulate only the human egress process
without any fire effects.

FDS+Evac treats each evacuee as a separate entity, or 'agent', which
has its own personal properties and escape strategies.  The movement
of the agents is simulated using two-dimensional planes representing the
floors of buildings.  The basic algorithm behind the egress movement
solves an equation of motion for each agent in a continuous 2D space
and time, \emph{i.e.}, doing some kind of an artificial molecular
dynamics for the agents.  The forces acting on the agents consist of both
physical forces, such as contact forces and gravity, and psycholigical
forces exerted by the environment and other agents.  The model behind
the movement algorithm is the social force model introduced by
Helbing's group~\cite{Helbing95, Helbing00, Helbing02, Werner03}. 
A modification of the model to a better describe a human body
was introduced by Langston \em et al. \em ~\cite{Langston06}.
...
results.  Chapters 7--11 contain the user manual, where the
...
theorethical method is needed in order to build up the model
correctly.  It also helps making the judgement on the applicability
and reliability of the programme in the problem in question.  Because
the evacuation calculation is implemented as a subroutine of FDS, the
reader is recommended to read first the manual of FDS and learn how to
do fire simulations.  Even if the user wants to run the programme
without fire, she/he should be acquainted with the FDS in order to
build up the simulation geometry.


\section{Getting Started}\label{Sec_GetStart}

The evacuation module is implemented as
subrutines of FDS.  Thus, the running of FDS+Evac evacuation
simulation is done similarly as an ordinary FDS fire simulation. See
the FDS User's Guide~\cite{FDS_UserGuide} for details.  FDS+Evac
results can be visualized using the Smokeview~\cite{SV_UserGuide}
...
resulting FDS executables are also the executables of the evacuation
calculation module.  The source codes can also be obtained via the web
pages.  See the document cover page for detailed version information.
...
\end{verbatim}
This page contains the combined Technical Reference and User's
...
\section{Limitations and Special Features to Consider}\label{Sec_SpecFeatures}

All the intended features of FDS+Evac are not yet fully functional.
For now, FDS+Evac is best suited for doing calculations in buildings, whose
floors are horizontal.  Sports halls with spectator stands or concert
halls can also be modelled if their geometry is not too complicated,
but the user should note that simulations in inclined geometries have
not been validated.  It is also assumed that the different levels of
the building are separated from each other, \emph{i.e.}, they are
connected to each other by stairs, escalators, doors, and similar
objects.  Note, that FDS+Evac does not yet support the use of
elevators during evacuation process.  Wide stairs or inclines can also
be used to connect different floors, but this is not as
...
usually very practical way of doing things.
...
meshes and FDS+Evac can be run in an ``evacuation drill'' mode.  One
...
as is done for the standard FDS fire simulation geometrical objects,
\emph{e.g.} the namelists \Timtt{OBST}, \Timtt{HOLE}, and
\Timtt{VENT}.  Evacuation meshes should not be too fine, because then
one might face problems with the evacuation flow fields, which are
used to guide human movement towards the exits.  This dificulty can be
addressed by an experience user, but an easier way is always to use
evacuation meshes, which are not too fine.  Usually mesh cell sizes
0.25~m or larger can be used without any problems.  For example, if
you have 1.2~m wide doors you could use 0.3~m, 0.6~m, or 1.2~m cell
spacing depending on how detailed geometry is needed.  Note, that
Smokeview does not yet fully support the evacuation calculation, \em
e.g.\em, the positions of doors, stairs and other evacuation objects
are not shown by Smokeview.  Note also that the spaces, where humans
are allowed to move, should be at least about 0.7~m wide, because
FDS+Evac is not able to move humans in narrower exit paths correctly.
User should check the evacuation geometry by using the Smokeview
programme and make sure that the ``holes'' in the geometry are not too
narrow. 

Note that while the FDS fire simulations can use time dependent
geometry elements, such ase obstacles and holes which are created and
removed by special control devices, the evacuation geometry does 
not support the time dependent geometries.  For evacuation, the
initial geometry is always used for the duration of the
simulation. 

A door should be defined such that it is slightly inside a wall,
see Fig.~\ref{Fig_DoorGeom}.  This way the door casings will produce
forces on humans.  One should define a small hole in the wall, where
an exit (or a door) is placed.  Note, that the width of the exit will
depend on the mesh resolution.  One should define the exit wide enough
to cover the whole width of the hole.  Because the obstacles are
aligned with the mesh but the exits (or doors) are not, be sure that
exits are wide enough.  (Use Smokeview and activate mesh lines.)  It
does not matter if the end points of the exits (or doors) are inside
solid obstacles.

%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,
    width=80mm]{FIGURES/door_geometry2.eps}}
  \caption{How to make an exit or a door.}\label{Fig_DoorGeom} 
\end{figure}
%

For now, the number of humans placed in the same main evacuation mesh
is limited. The program stops and writes out an error message if more
than 10000 humans are tried to place on the same evacuation mesh.
Usually, one main evacuation mesh extends over a whole floor of the
building.  Several main evacuation meshes may co-exist, \emph{e.g.},
the different floors of the building, and the total number of humans
is not restricted by the program. The available computer memory is the
only factor limiting the total number of humans.  However,
the calculation is going to be very CPU expensive if there are more
than a few thousands humans in the same evacuation mesh.

The initial density of humans cannot be much larger than 4 persons
per square metre.  In cases of high human density, the simulation may
require a couple of initialization trials ecause the initial positions
of humans are generated randomly. If FDS+Evac cannot place
humans on their initial positions, an error message ``ERROR: FDS
...
and gas consentrations from the fire calculation affect the movement
...
index~\cite{Purser95}, indicating the human incapasitation.  Smoke
density is used both to slow down the walking speeds of humans and to
affect the exit selection algorithm of humans.  Smoke density can also
be used to detect the fire, but is should be kept in mind that human
nose is a very delicate instrument and the levels of smoke
consentration needed for a smoke sensing may be below the accuracy of
the current FDS predictions.  Note that the effects of radiation and
gas temperature on humans are not yet implemented in the programme, so
humans do not try to avoid a fire if the user does not explicitely define
the evacuation geometry to take this in to account.
...
of humans. In addition, there is a small random force on each human's
...
should always do a dozen or so egress simulations to see the variation
of the results. To speed up this process, several egress calculation
can be done per one fire simulation and the calculation of the
...
evacuation part.  In later versions of the program, the memory usage
will be optimized.  Currently, an efficient use of the computer memory
...
if FDS+Evac calculation takes too much memory, the user must
increase the amount of available memory. This may require
swicthing from a 32 bit to a 64 bit operating system.
...
This chapter provides a short description of the evacuation module of
the Fire Dynamics Simulator (FDS).  Detailed information about FDS can
be found in its documentation~\cite{FDS_Manual, FDS_UserGuide} and
more detailed information about the evacuation algorithm and its
features can be found in the next chapters.
...
FDS and these subroutines together with the FDS are called as
...
present version of FDS+Evac is build on the FDS version 5.2.0 and it
is the second version of the evacuation module and, thus, its version
number is 2.0.0, \emph{i.e.}, FDS5.2.0+Evac2.0.0.  Changes in the
...
on the web site http://fire.nist.gov/fds/.  The documentation on the
evacuation part of FDS+Evac can be found on the web page
http://www.vtt.fi/fdsevac/.
...
and time, but the building geometry is fitted to the underlaying
...
of Standards and Technology (NIST).  Additional contributors and
...
parameters are neglected by the FDS fire calculation.  Same is true
for the other way also, \emph{i.e.}, by commenting out the fire meshes
one can make easily test runs to see that the construction of the
evacuation calculation part is done correctly.
...
using the Smokeview programme~\cite{SV_UserGuide}.  The number of
agents gone through different parts of the evacuation scenario as well
as the number of agents in different evacuation nodes are saved in
simple, comma-delimeted text file, that can be plotted using a
spreadsheet program.  Some additional, quite detailed, information are
written to the diagnostic evacuation text file including the initial
positions and properties of the agents.
...
programme and the geometry of the building is discretized,
\emph{e.g.}, the widths of the doors are integral multiples of the
grid cell sizes.
...
the results of a recent experiment by Frantzich and
Nilsson~\cite{Frantzich03}, where larger smoke consentration were used
than in the experiments by Jin~\cite{Jin78}.  There are large scatter
in the experimental results and more experimental results are more
than welcome.  Note also that there is a large scatter between
different agents but the present version of the programme uses average
values for each agenr.  The default model for stairs does not include
the option for agents to turn back when the smoke consentration
becomes too high.
...
the user input.  Note, that the effect of HCN is not modelled and that
...
full control on the doors which different agents will be using.  In
some applications this can be listed as a benefit of the model, but
...
present version one floor can contain at most 10000 humans.  The
...
Some of the memory limits can be avoided by changing from 32 bit
...
This chapter describes the theoretical model behind the FDS+Evac
evacuation simulations.  The model uses autonomous agents to treat the
escaping persons and their individual properties and escape
strategies.  The movement of the agents is is modelled on continuous
space and time.  The building geometry is modelled as two-dimensional
floors, which are connected to each others by staircases and
inclines.  The geometry of the building is treated using rectilinear
underlaying mesh, similarly as in the FDS fire simulations.

\section{Introduction}

FDS+Evac treats each escaping person as an individual agent, whose
...
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,
    width=30mm]{FIGURES/body_shape.eps}}
...
body, see Fig.~\ref{Fig_HumanBody}, just like in the Simulex
program~\cite{Simulex96, Thompson95a, Thompson95b, Thompson03} and in
the MASSEgress program~\cite{Pan06}.  The body dimensions and the
unimpeded moving speeds of the default population types are shown in
Table~\ref{Table_DefaultHumans}.  The body diameters and walking
speeds are by default drawn randomly for each generated agent from
uniform distributions, whose widths are also given in the table.  The
body diameter and moving speed distributions are taken to be same as
in the Simulex program~\cite{Simulex96, Thompson95a, Thompson95b,
...
calculation would allow the evacuation calculation to change the flow
of the fire calculation, e.g., by allowing the evacuees to open or
...
agents, see Fig.~\ref{Fig_SocForce}.  This model is shortly described
...
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,
    width=120mm]{FIGURES/voimakuvat_Poster.eps}}
...
FDS+Evac treats agents as combination of three elastic circles moving
...
model~\cite{Pan06}, see Fig.~\ref{Fig_HumanBody}.  The default body
dimensions and the unimpeded walking speeds of different predefined
agent types of FDS+Evac are listed in Table~\ref{Table_DefaultHumans}.
...
$A_i$, $B_i$ describe the strength and spatial extent of the force,
...
can be used to describe attraction (or repulsion) between agents, like
a herding behaviour or an adult--children interaction.  It could also
be used to form pairs of agents, e.g., describing a fire fighter pair
...
torques on agents and these are given by the formula
%
...
just follow the egress flow to the outside of the building without
much control on the process.

%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=120mm]{FIGURES/evac_ffield.eps}}
...
minimum walking spees of an agent is $ v^0_{i,\mathrm{min}} = 0.1
\cdot v^0_i$, i.e., the agents are not stopping due to a thick smoke,
they continue to move with a slow speed until they are incapacitated
by the toxic effects of the fire products.  The standard deviations
are reported to be $\sigma_\alpha = 0.069~\mathrm{ \textrm{m} \,
  \textrm{s}^\textrm{-1} } $ and $\sigma_\beta = 0.015~ \mathrm{
  \textrm{m}^\textrm{2} \textrm{s}^\textrm{-1} } $, but only the mean
values are used in FDS+Evac, \emph{i.e.}, there is no variation
between the agents.
...
FDS+Evac uses game theoretic reaction functions and best response
...
can not select these exits.  The last colum shows the colors used in
...
the maximum forces excerted on agents.  The minimum time step varies
...
0.5*distance to the door, where local visibility = 3/extinction
coefficient.  If there is no line-of-sight to the door, then local
concentrations at the position of the agent are used and the distance
to the door is calculated along a bee line (this is a crude
...
In the verification test cases the default parameter values of
FDS+Evac for the different pre-defined agent types were used unless
...
modelling program, \emph{i.e.}, it uses stochastic distributions to
...
eyes'' that the program is working as it should.
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/FED_Test.eps}}
...
\item Unimpeded walking speed vs smoke density: Smoke reduces the
  ...
  density was increased by 114.9~mg/$\mathrm{m^3}$ after every 10
  seconds.  The length of the simulation was 100~s and the unimpeded
  walking velocity for a smokeclear environment was set to 1.0~m/s.
  The result of this test is shown in Fig.~\ref{Fig_SmokeSpeedTest}.
  In the figure, the line labelled as ``Theory'' is the experimental
  correlation given in Eq.~\ref{Eq_SpeedSmoke}.  The velocities were
  calculated using 8~s periods, like 22~s -- 30~s, because an agent
  needs a little bit time to adjust its speed (the inertia of mass)
  when the smoke density is changed.  The results show that FDS+Evac
  accurately reproduces the anticipated reduction of walking speed.
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/Smoke_vs_speed.eps}}
...
\item IMO 10: Exit route allocation: Populate a cabin corridor section
  ...
  the test.  Note that this geometry needed some more effort to model,
  see the FDS+Evac input file on the FDS+Evac web pages for more
  information.  This is due to the fact that in order to model the
  test geometry rigorously the mesh cell sizes for the evacuation
  calculation meshes were 0.1~m in both the $x$ and $y$ directions.
  This is a little bit too fine mesh to construct the guiding floor
  flow fields for evacuation.  This problem does not arise if one is
  modelling the cabinets to have closed doors and enters the agents in
  to the calculation at the cabin doors.
%
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=60mm]{FIGURES/DoorTestNoSmoke_Doors.eps}
  \includegraphics[clip=true,width=60mm]{FIGURES/DoorTestNoSmokeAlgorithm.eps}}
...
%
\begin{figure}[!bt]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/DoorTestSmokeDens.eps}}
...
\item Decision making model without smoke: The verification of the
  ...
  there are no visible doors, the agents choose the nearest nonvisible
  but known door; see \emph{e.g.} the agents in the bottom left corner
  of the building.  Note however, that in the present version of
  FDS+Evac, the distance to the nonvisible doors is calculated along a
  straight line through the internal walls.  In later versions, the
  algorithm may be changed to calculate the distance along the
  streamlines used to guide the agents towards the doors.
...
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=60mm]{FIGURES/DoorTestSmoke_Doors.eps}
  x\includegraphics[clip=true,width=60mm]{FIGURES/DoorTestSmokeAlgorithm.eps}}
...
The numerical accuracy of the model depends on the time step used to
...
bottlenecs of an evacuation of a building.  This section does not
...
sensivity of the chosen algorithms and numerical methods are examined
...
In principle, the movement algorithm of the agents described in
Sec.~\ref{Sec_BasisModel} does not have any underlaying computational
...
calculation.  The most obvious mesh sensivity issue is that the
...
level of the evacuation calculation.  There are many more factor
...
\begin{figure}[!tb]
  \centerline{ \includegraphics[clip=true,
    width=60mm]{FIGURES/Door_Geom.eps} ~~~~~~~~   
    \includegraphics[clip=true,width=60mm]{FIGURES/CorrGeom2.eps} }
...
The agent movement algorithm of FDS+Evac has many parameters.  Some of
...
Carlo simulations were performed to find the parameters, which have
the greatest effect on specific flows.  Two different geometries were
used in the simulations, see Fig.~\ref{Fig_Geoms}. One of the
geometries was used to study the flow through a narrow door and
through a wide door and the other geometry was used to study flows in
a corridor using densities 1.0 and 2.0 persons per square metre.
There were 100 agents randomly located at the $5 \times 5 ~\mathrm{
  \textrm{m}^\textrm{2} } $ square in the door flow calculations.
Corridor flow calculations had 96 or 192 agents inside the corridor
depending on the density.  Thousand egress simulation with different
random initial properties were performed for each of these four
different cases.  In total eleven model parameters, $A_i$, $B_i$,
$\lambda_i$, $v^0_i$, $\tau_i$, $A_w$, $B_w$, $\lambda_w$,
$\omega^0_i$, $\tau^z$, and $I^z_{i}$ were varied $\pm 20~\%$ about
their means using uniform distributions.  The monitored output
quantity was the specific flow in all cases and the Spearman's rank
correlation coefficients (RCC) were calculated for these four cases
and they are shown in Fig.~\ref{Fig_RCC}.

%
\begin{figure}[!tb]
  \centerline{ \includegraphics[clip=true, width=75mm]{FIGURES/RCC_DoorCorr_new1.eps} ~~
               \includegraphics[clip=true, width=75mm]{FIGURES/RCC_DoorCorr_new2.eps} }
...
%
\begin{figure}[!tb]
  \centerline{ \includegraphics[clip=true, width=50mm]{FIGURES/DoorCorr_Anew.eps} 
  \includegraphics[clip=true, width=50mm]{FIGURES/DoorCorr_Bnew.eps}
  \includegraphics[clip=true, width=50mm]{FIGURES/DoorCorr_Lambdanew.eps} } 
  \centerline{ \includegraphics[clip=true, width=50mm]{FIGURES/DoorCorr_v0new.eps} 
  \includegraphics[clip=true, width=50mm]{FIGURES/DoorCorr_Taunew.eps}
  \includegraphics[clip=true, width=50mm]{FIGURES/DoorCorr_Bwallnew.eps} } 
...
corridor.  The results of these, in total almost 20000 simulations,
are shown in Fig.~\ref{Fig_Door1}, where the markers represent the
...
from a random distribution and there are small random forces in
...
see the actual evacuation geometry and the flow fileds, one should do
...
no smaller than 0.7~m wide ``holes'' in the evacuation geometry,
...
Previous chapters are dealing on the fact that how the implementation
of the model worked as a computer code.  \emph{I.e.} it was tested
that the programme is functioning as planned and it was also cosidered
...
agains experimental data from evacuation experiments and trial
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/CorrFlowResults.eps}}
...
%
\begin{figure}[!bt]
  \centerline{\includegraphics[clip=true,width=60mm]{FIGURES/PorrasKuva.eps}}
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=60mm]{FIGURES/StairsType1Lam0p3.eps}
  \includegraphics[clip=true,width=60mm]{FIGURES/StairsType1Lam0p5.eps}}
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=60mm]{FIGURES/StairsType2Lam0p3.eps}
  \includegraphics[clip=true,width=60mm]{FIGURES/StairsType2Lam0p5.eps}}
...
\end{figure}
  ...
  agents move at reduced speed.  Reducing the walking speed by a
...
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=120mm]{FIGURES/Library_Kuva2.eps}}
...
\item Public library: An observed evacuation experiment of a public
  ...
  movement inside the floor, queuing to the staircase and finally
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/KirjastoTulokset.eps}}
...
standard input parameters for the properties of the agents.  The
premovement times were generated from symmetric triangular
distribution with mean of 31~s and lower and upper limits of 11~s and
71~s, respectively.  A comparison of the simulated and experimentally
observed flows is shown in Fig.~\ref{Fig_KirjastoResults}.  As can be
seen, the predicted flow rates are in very good agreement with the
experiments.  For the west door, the results reflect the goodness or
badness of the assumed premovement time distribution because the flow
rate through the door is so small.  For the north door, the simulation
is very relevant, because the flow rate is mainly determined by the
geometry and the crowd dynamics during the queuing process.
%
%
\end{enumerate}
%
...
The FDS+Evac model is compared here to other evacuation models using
...
project~cite{Hostikka07a}.
%

%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/eeri_n60s15_v1_114s.eps}}
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/EerikkilaResults.eps}}
...
\item Sports hall: FDS+Evac simulations were compared to
  ...
  simulations in a sports hall shown in Fig~\ref{Fig_SportGeom}.  The
  ...
  close to door 3 (the shaded rectangle in Fig.~\ref{Fig_SportGeom})
  ...
  Fig.~\ref{Fig_SportGeom}).  Three different reaction time scenarios
  ...
  having a lognormal distribution (median 75~s, standard deviation of
  the logarithm of reaction time was 0.7).  Actually, the lognormal
  distribution was approximated by two uniform distributions, because
  the version of the Simulex, which was used, does not support
  lognormal distributions for the reaction time.
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/b_ave90s11rnd_d123_initpos.eps}}
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/PankkiTulokset.eps}}
...
\item Open floor office: This test geometry was an open floor office,
  ...
  FDS+Evac simulations, the initial (random) positions of agents do
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=120mm]{FIGURES/Kaytava_fig.eps}}
...
\item Assembly space: The second test case is a large fictitious
  ...
  those of Simulex and buildingExodus~\cite{Exodus3} in
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=80mm]{FIGURES/KaytavaTulokset.eps}}
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=120mm]{FIGURES/kaytava_u1s0p5_65s.eps}}
  \centerline{\includegraphics[clip=true,width=120mm]{FIGURES/kaytava_simulex120s.eps}}
  \caption{A snapshot from (a) FDS+Evac, (b) Simulex
    calculation}\label{Fig_AssemblySnapshots} 
\end{figure}
%
  
Considerable differences are shown between the results of FDS+Evac and
the results of Simulex and buildingExodus codes.  These differences
can be traced back to the motion of the agents in the corridor, see
Fig~\ref{Fig_AssemblySnapshots}.  Simulex and buildingExodus are not
using the whole width of the corridor efficiently, when the
simulations are done using the default values and standard input.  (An
advanced user of these codes might be able to get different results by
using some additional features.)  The results of FDS+Evac model look
more realistic.
  
In Figure~\ref{Fig_AssemblyResults} also shown are the results of the
simulations for a case, where there is no corridor at all,
\emph{i.e.}, there is just one 7.2~m wide exit door located at the wall
of the room.  In this case, the agreement between the different
evacuation programmes is much better.  The calculated specific flows
(1/p/m) are: Simulex 1.48, Exodus 1.95, FDS+Evac 2.15 and FDS+EvacSlow
1.73 (~$\lambda_i =0.3$).
...
%
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,width=120mm]{FIGURES/OviFlowRersults.eps}}
...
be designated as \textbf{CHID.fds}.  In practice, the user should
choose the ID string \Timtt{CHID} so that all of the files associated
with a given calculation will have a common prefix.
...
start with an existing input file, run it as is, and then make the
...
\section{Starting an FDS+Evac Calculation} 

A FDS+Evac simulation is run similarly as a FDS fire simulation, so
...
Assuming that an input file called \textbf{CHID.fds} exists in some
...
programme (fds5\_mpi.exe).  You should first learn how to run parallel
fire calculations by reading the User's Guide of FDS.  The combined
fire and evacuation simulation can be run almost similarly, but you
should be sure that all fire mesh definitions are preceeding all
evacuation mesh definitions, because all the evacuation meshes are
...
the example {\bf config.txt} file given in the User's Guide of FDS
should be modifed as:
\begin{verbatim}
...
as a single thread, \emph{i.e.,} all evacuaton calculation meshes are
...
\item Make a FDS fire input file for your project and do the
  ...
  FDS+Evac only in the ``evacuation trial mode'' then it is best to
  use the serial version of the executable, because all evacuation
  meshes are always calculated as a single thread, \emph{i.e.,}
  evacuation calculation utilizes just one processor.
%
...
%%   change \Timtt{EVACUATION} value to \Timtt{.TRUE.} on these lines.
%%   This may sound like a nuisance, but the way how FDS parses the input
%%   file demand that.
%
%% \item Put a keyword \Timtt{EVACUATION=.FALSE.} to every \Timtt{VENT}
%%   line, which you fire input has.  This may sound like a nuisance, but
%%   the way how FDS parses the input file demand that.
%
\item Add the line to your input file:\newline
  \verb|&SURF ID='OUTFLOW', VEL=+0.000001, TAU_V=0.1 /| 
%
\item Define meshes for the evacuation.  Each floor should have its
  own mesh.  The $z$ co-ordinates for these meshes should be at a
  level, where the most obstacles for the movement are, usually about
  one meter above the floor.  Note, that the smoke and other fire
  related quantities are taken at the middle of each mess cell,
  \emph{i.e.}, the mid $z$ level of the evacuation meshes should be
  about 1.6~m above the floor level.  See Fig.~\ref{Fig_EvacGrid} and
  Sec.~\ref{Sec_EvacGrid}.  Try to make the mesh division such that
  the mesh cell sizes in the $x$ and $y$ are nice round numbers, like
  0.25~m, 0.35~m, 0.5~m, \emph{etc}.
%
\item Put \Timtt{T\_END=0.0} on the \Timtt{TIME} namelist group,
  ...
  comment these things out also, because these do not belong to some
  mesh anymore.
%
...
\item Define evacuation flow field vents (\Timtt{EVACUATION=.TRUE.},
  ...
  out of the evacuation meshes at the places where you want agents to
  go.  These vents should be placed at the doors and exits. Note, that
  in FDS5 vents should be defined on solid surfaces.  If you are using
  ...
  on each flow field.  Remember to define slice output files at your
  evacuation mesh heights, \emph{e.g.}, \\
  \verb|&SLCF PBZ=x.x, QUANTITY='VELOCITY',VECTOR=.TRUE./|
%
\item Remove \Timtt{CHID\_evac.eff} if it exists and run a short
  simulation (\Timtt{T\_END=1.0}).  Check the evacuation flow fields
  by using Smokeview, \emph{i.e.}, open the velocity vector slice
  files of the evacuation meshes.  Check also that your evacuation
  geometry looks fine.  If not, add obstacles or holes to the
  evacuation calculation and remember to remove the
  \Timtt{CHID\_evac.eff} file.  Repeat these steps as long as you are
  satisfied with the geometry.  Note, that FDS+Evac tries to read the
  evacuation flow fields from the disk if there exitst a
  \Timtt{CHID\_evac.eff} file.  This file should be recalculated each
  time when you make changes to the geometry by changing objects like
  \Timtt{OBST}, \Timtt{VENT}, \Timtt{HOLE}, and \Timtt{MESH}.
%
\item Define your person classes, the \Timtt{PERS} namelist groups.
  These define the properties of the agents in the model.  Also the
  detection and reaction time distributions are given here.
%
\item Define your doors, exits, stairs, and entries, the \Timtt{DOOR},
  \Timtt{EXIT}, \Timtt{CORR}, \Timtt{EVSS}, and \Timtt{ENTR}
  namelists.  Activate grid locations in Smokeview to see the actual
  positions of your obstacles in the evacuation meshes, which might be
  a little bit different than the values given in the input file due
  to the fact that FDS moves \Timtt{OBST}s, \Timtt{HOLE}s, and
  \Timtt{VENT}s to match the mesh cell boundaries.  See
...
\item Place the agents inside the building using the \Timtt{EVAC}
  ...
  \Timtt{OBST}s, see Fig.~\ref{Fig_DoorGeom}.
%
\item Once again, run a short simulation.  See, that your agents have
  ...
  \emph{i.e.}, the evacuation flow fields were not recalculated and
  the calculation runs faster than before.
%
...
\item Do a full evacuation calculation and save the results,
  ...
  dozen or so times.  Note, that the results are not exactly similar,
  because agents have random properties and initial positions.  These
  simulations correspond to a fire drill.  After this, activate the
  fire meshes, \emph{i.e.}, put the ampersands back to the fire mesh
  lines, and do a full FDS+Evac simulation.  Now the fire and
  evacuation simulations are done at the same time and a file
  \Timtt{CHID\_evac.fed} is written to the hard drive.  This file can
  be used to run many evacuation simulations per one fire simulation,
  \emph{i.e.}, no need to calculate the same fire for many times.
  Note, that the \Timtt{CHID\_evac.eff} file is (re)calculated always
  when there are active fire meshes.
%
\item If you are doing more than one evacuation calculation per one
  ...
  and evacuation calculations were done at the same time.  Then
  comment the fire meshes out, \emph{i.e.}, the ampersands away once
  again, and rerun the case.  Copy the results
  (\Timtt{CHID\_evac.csv}) to some other name and rerun once again,
  etc. For a given fire, you should run the evacuation part a couple
  of dozen times, because the FDS+Evac is not deternimistic model.
  After the runs, examine the results: calculate averages, variances,
  etc.
%
\end{itemize}
...
\item If the run stops early and the error message ``ERROR: FDS
  improperly set up'' is printed on stderr then the initialization of
...
\item The evacuation input should be matched to the evacuation floor
  ...
  Smokeview.  The evacuation related input, \emph{e.g.}, namelists
...
%
  \item The positions of doors and exits are not checked, \emph{i.e.},
    ...
    solid objects behind the doors.  Thus, it is nice to add
    \Timtt{SLCF} output for velocity vectors at the $z$-level of the
    evacuation meshes and see these vectors in Smokeview.
%
  \item It is good idea to put an exit or a door a little bit inside a
    ...
    agents.
%
  \item Check your \Timtt{EVAC} lines that you are not putting agents
    in areas, where humans should not be.  Check also that agents can
...
%
  \item Check your \Timtt{EVAC} lines that you are not trying to put
    ...
    four persons per a square meter.  If you are trying to populate
    the floor more densly, the program will stop after the
    initialization phase.  Note, that the initial position of agents
    ...
    those agents who were positioned succesfully and see the output on
    stderr to see the position of the agent, which could not be placed
...
\item[\Timts{EVAC\_Z\_OFFSET}] The distance from the mid height of the
  ...
  Smokeview to show the bodies of the agents so that their feet are
  touching the floor (default is 1.0~m).  This needs to be given only
  for the main evacuation meshes.
%
\end{description}
...
agents.  Usually, one main evacuation mesh represents a 'floor'.  You
need more main evacuation meshes on a floor if there exists separate
parts of the floor, where the agents are not going to be using same
...
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,
    width=120mm]{FIGURES/evac_grid2.eps}}
...
Evacuation meshes, which are only used to calculate (additional)
evacuation movement flow fields should have
\Timtt{EVAC\_HUMANS=.FALSE.} or just no \Timtt{EVAC\_HUMANS} keyword
at all.  Note, that these meshes should have exactly the same
...
\item[\Timts{EVAC\_PRESSURE\_ITERATIONS}] is the number of Poisson
  ...
  this.  Too large number makes the flow field calculation to take too
...
\item[\Timts{EVAC\_TIME\_ITERATIONS}] is the number of evacuation flow
  ...
  evacuation flow field, so some iterations are needed.  Too large
...
%
\item[\Timtt{EVACUATION}] Should be \Timtt{.TRUE.} for all evacuation
  flow field vents.  (Default is \Timtt{.FALSE.})
%
...
\Timtt{EVACUATION=.TRUE.} is given.  Note, that in FDS5 VENT must
always be defined on a solid surface or on the outer boundary of the
computational domain.  Thus, the user may need to place additional
evacuation OBSTs behind the VENTs used to generate the evacuation flow
fields.
...
item \Timtt{MESH\_ID} should be given for the evacuation obstacles.
...
%
\begin{table}[!tp]
\caption{Statistical distributions.  The most used ones are: 0) no
  distribution, \Timtt{x\_MEAN} is used; 
1) uniform distribution, \Timtt{x\_LOW} and \Timtt{x\_HIGH} are used;
2) normal distribution, \Timtt{x\_MEAN} is the mean, \Timtt{x\_PARA}
is the std.dev., \Timtt{x\_LOW} and \Timtt{x\_HIGH} are the cut offs,
\emph{i.e.}, the values are within the interval
...
\Timtt{DET}, or \Timtt{PRE}.}\label{Table_StatDists}
\begin{center}
...
Log-normal       & 5 & \_MEAN, \_PARA ($\overline{\ln(x)}$,
$\sigma_{\ln(x)}$)\\  
Beta             & 6 & \_PARA, \_PARA2 ($a$, $b$) \\ 
...
This namelist group is used to define agent types.  Properties like
diameters, walking speeds, pre-evacuation times, and force constants
of the agents are given here.  Some of the values might be given as
distributions.  There are five default agent types defined and they
are 'Adult', 'Male', 'Female', 'Child', and 'Elderly', see
Table~\ref{Table_DefaultHumans} for their properties.  This namelist
is also used to set some miscellaneous parameters.
...
%
\item[\Timtt{DET\_EVAC\_DIST}] The type of the detection time
  distribution, see Table~\ref{Table_StatDists}.
%
\item[\Timtt{PRE\_EVAC\_DIST}] The type of the reaction time
distribution, see Table~\ref{Table_StatDists}.
%
\item[\Timts{DIAMETER\_DIST}] The distribution type used for the size
  of the agents, see Table~\ref{Table_StatDists}.
%
\item[\Timts{VELOCITY\_DIST}] The distribution type used for the
  unimpeded walking speeds of the agents, see
  Table~\ref{Table_StatDists}.
%
\item[\Timts{TAU\_EVAC\_DIST}] The distribution type used for the
  relaxation time of the agents, see Table~\ref{Table_StatDists}.
%
\end{description}
...
\item[\Timts{DIA\_MEAN,DIA\_PARA,DIA\_PARA2,DIA\_LOW,DIA\_HIGH}]
  Diameter of the agent circle ($2R_d$), parameters of the
  distribution, see Fig.~\ref{Fig_HumanBody} and
...
\item[\Timts{D\_TORSO\_MEAN,D\_SHOULDER\_MEAN}] The mean diameters of
  ...
  and Fig.~\ref{Fig_HumanBody}.  The variation of the diameters is
  determined by the variation of the agent circle ($2R_d$).
%
...
%
\item[\Timts{V\_ANGULAR}] Maximum target angular speed of an agent
...
\item[\Timts{SMOKE\_MIN\_SPEED}] This sets the minimum speed of the
  agents when they are moving in smoke, $v^0_{i,\mathrm{min}}$ in
...
\noindent NOTE: \Timts{FAC\_x\_WALL}, \Timts{LAMBDA\_WALL}, \Timts{FC\_DAMPING},
\Timts{V\_ANGULAR}, \Timts{NOISExx},\linebreak[4]
\Timts{COLOR\_METHOD}, \Timts{DEAD\_COLOR}, \Timts{DEAD\_RGB},
\Timts{NOT\_RANDOM}, \Timts{DENS\_INIT},\linebreak[4]
\Timts{TDET\_SMOKE\_DENS}, \Timts{FED\_DOOR\_CRIT} the
last values read from \Timtt{PERS} lines are used.  So, it is nice
practice to give these numbers just in one \Timtt{PERS} group or have
same values in every \Timtt{PERS} group.
...
Places agents in the evacuation meshes, \emph{i.e.}, the initial positions of
the agents.
%
\begin{description}
%
...
%
\item[\Timtt{XB}] where the agents are put, $z$ should belong to a
  main evac mesh.  If \Timtt{XB} belongs to many evacuation meshes
  then the keyword \Timts{MESH\_ID} should be given.
%
...
\item[\Timtt{PERS\_ID}] The \Timtt{ID} string of the \Timtt{PERS}
  namelist, which is used to define the properties of the (randomly)
  generated agents.
%
\item[\Timts{ANGLE}] By default the orientation of agents is random,
  ...
  persons.  Angle 0 means that agents are facing towards +$x$.
%
%\item[\Timts{QUANTITY}] Color of the torso of the agents in Smokeview.
%  The colors which can be used are: black, yellow, blue, red, green,
...
\item[\Timts{FLOW\_FIELD\_ID}] Specifies which evacuation flow field
  the agents are following if there are no known or visible doors
  available.
%
...
Specifies a place where no agents are generated by the \Timtt{EVAC}
lines.
%
\begin{description}
%
\item[\Timtt{ID}] ID string of the hole.  One can refer to this hole
  by its name.
%
\item[\Timtt{XB}] Co-ordinates where the agents are not put, $z$
  should belong to a main evac mesh.  If \Timtt{XB} belongs to many
  evacuation meshes then the keyword \Timts{MESH\_ID} should be given.
%
\item[\Timts{MESH\_ID}] If there are overlapping main evacuation
  meshes then this parameter could be used to specify the mesh where
  this \Timtt{EVHO} line is applied.
%
...
might be used just to count agents, then the keyword
...
\item[\Timtt{XB}] Co-ordinates, should be a line in the $(x,y)$ plane,
  the $z$ should belong to a main evacuation mesh.  If \Timtt{XB}
  belongs to many evacuation meshes then the keyword
...
%
\item[\Timtt{IOR}] Direction of the door, e.g., +1 agents are going
  $+x$ direction, -2 agents are going $-y$ direction (direction means:
...
\item[\Timts{COUNT\_ONLY}] If \Timtt{.TRUE.}, agents are not removed,
  ...
  CHID\_evac.csv file has a column for each \Timtt{EXIT} regardless if
  \Timts{COUNT\_ONLY} is true or false.
%
\item[\Timts{VENT\_FFIELD}] The ID string of the evacuation flow field
  mesh behind this exit door.  The agents are guided to this exit door
  by the specified flow field.  If none is given then the agents,
  whose target this exit is, are following the flow field of the
  current main evacuation mesh, \emph{i.e.,} they are just going where
  the flow goes.  This parameter should be given if the exit selection
  algorithm of FDS+Evac is being used.
%
...
%
\item[\Timts{COLOR}] Color of the agents seen in Smokeview if
  \Timts{COLOR\_METHOD=4}.  See the FDS User's Guide and FDS web page
  for the list of available color names.
%
\item[\Timts{RGB}] Three integers (0--255) specifying a color
  of the agents seen in Smokeview if \Timts{COLOR\_METHOD=4}.  Note,
  that \Timts{COLOR} overrides this option.
%
%\item[\Timts{COLOR\_INDEX}] Color index of the torso of the agent in
%  Smokeview if the \Timtt{COLOR\_METHOD}=4 on \Timtt{PERS} namelist.
%  The colors which can be used are: black (0), yellow(1), blue(2),
...
Defines an entry.  An entry can enter agents to the calculation at a
...
one way door, \emph{i.e.}, agents can only come out from this 'door'. 
...
\item[\Timtt{XB}] coordinates, should be a line in the $(x,y)$ plane,
  the $z$ should be 'main evac mesh' $z$ coordinates.  If \Timtt{XB}
  belongs to many evacuation meshes then the keyword
  \Timts{MESH\_ID} should be given.
%
\item[\Timts{MESH\_ID}] If there are overlapping main evacuation
...
%
\item[\Timtt{IOR}] direction of the entry, \emph{e.g}., +1 agents are
  entering towards $+x$ direction -2 agents are entering towards $-y$
...
\item[\Timts{TIME\_START}] The time (s) when this entry starts adding
  agents to the calculation.  The default is 0~s.
%
...
\item[\Timtt{PERS\_ID}] the properties of agents are generated using
  ...
  other node, \emph{i.e.}, they are 'new' agents.  If not given, default
  values are used.
%
\item[\Timts{FLOW\_FIELD\_ID}] Flow field in the 'room/floor', which
  the aagents are following, \emph{i.e.}, specifies to which door an
  agent tries to go if no known nor visible doors are available.  If
  not given, the flow field of the main evacuation mesh is used.
%
...
%
\item[\Timts{KNOWN\_DOOR\_PROBS}] The probabilities that the exit doors
  are known.  Only values equal to one or zero can be used for
  entrys. (Default values are equal to ones.)
%
\item[\Timts{AVATAR\_COLOR}] Color of the agents seen in Smokeview if
  \Timts{COLOR\_METHOD=0}.  See the FDS User's Guide and FDS web page
  for the list of available color names.
...
\item[\Timts{AVATAR\_RGB}] Three integers (0--255) specifying a color
  ...
  that \Timts{AVATAR\_COLOR} overrides this option.
%
%\item[\Timts{QUANTITY}] color of the bodies of the agents in Smokeview
%  If the agents are coming to this entry form some other node, then
%  their original color is used instead of \Timtt{QUANTITY}.  This is
%  used if the COLOR\_METHOD=0 is specified on (some) PERS line.  See
%  the namelist \Timtt{EVAC} for the possible names of the colors.
%
\end{description}
...
Defines a door.  Similar to \Timtt{EXIT}, but the agents are not
...
\item[\Timts{KEEP\_XY}] saves the information on the position of the
  ...
  placed according to this information.  (Default is \Timtt{.FALSE.})
%
\item[\Timts{VENT\_FFIELD}] The ID string of the evacuation flow field
  mesh behind this door.  The agents are guided to this door by the
  specified flow field.  If none is given then the agents, whose
  target this door is, are following the flow field of the current
  main evacuation mesh, \emph{i.e.,} they are just going where the
  flow goes.  This parameter should be given if the exit selection
  algorithm of FDS+Evac is being used.
%
...
agents from one floor to the next one.  The corridor (actually stairs)
...
density and the movement speed (or specific flow) inside the corridor.
...
\item[\Timtt{XB, XB1, XB2}] Used to specify the points, where the
  smoke and FED data is taken for this corridor/stairs.  If only one
  value is used for the corridor/stairs, give \Timtt{XB}.  If the
...
%
\item[\Timtt{EFF\_LENGTH}] The length of the corridor/stairs, movement
  time = \Timtt{EFF\_LENGTH}/(\Timtt{FAC\_SPEED}$\times v_i^0$).
%
...
constructed using the \Timtt{EVSS} namelists correctly, but this is
not as straight forward as to use \Timtt{CORR} constructions.  See the
example input files on the FDS+Evac web pages.



\section{The \Timtt{EVSS} Namelist Group}

Defines an incline (\emph{e.g.}, stairs or a spectator stand) or an
...
than given on the MESH line of the evacuation floor.  This way
intermediate landings in stairs could be modelled, see the ``stairs''
example case on the FDS+Evac web pages.  The $z$ co-ordinates of the
EVSS constructions are only used to plot the agents in 'correct'
positions when they are viewed by Smokeview.  The agent movement
...
\item[\Timtt{XB}] Co-ordinates, should define a plane in the $(x,y)$
  plane, the $z$ should belong to a main evacuation mesh.  If
  \Timtt{XB} belongs to many evacuation meshes then the keyword
  \Timts{MESH\_ID} should be given.
%
\item[\Timtt{IOR}] Direction of the incline, \emph{e.g.}, +1 means
  that the +$x$ edge of the EVSS plane is a horizontal line at a
  height \Timts{HEIGHT0} above the main evacuation plane defined by
  the $z$ of XB and the -$x$ edge is a horizontal line a height
  \Timts{HEIGHT} above the main evacuation plane.
%
\item[\Timts{MESH\_ID}] If there are overlapping main evacuation
  meshes then this parameter could be used to specify the mesh where
  this EVSS line is applied.
%
\item[\Timts{HEIGHT}] The height of the incline measured from the
  level of the main evacuation plane defined by the $z$ of \Timtt{XB}.
  The other edge (opposite to the \Timtt{IOR} edge) of the incline has
  $z = z_{\scriptscriptstyle XB} + \mathrm{height}$.  \Timts{HEIGTH}
  can have a positive or a negative number.
%
\item[\Timts{HEIGHT0}] The height of the base line of the incline
  ...
  $z$ of \Timts{XB}.  The \Timts{IOR} edge of the incline has $z =
  z_{\scriptscriptstyle XB} + \mathrm{height0}$. \Timts{HEIGTH0} can
  have a positive or a negative value.  If
  \Timts{HEIGHT0}=\Timts{HEIGHT} the EVSS defines a horisontal plane
  and IOR has no effect.
%
\item[\Timts{FAC\_V0\_UP}] The unimpeded speed of an agent upwards the
...
\item[\Timts{FAC\_V0\_HORI}] The unimpeded speed of an agent
  ...
  v_0$.
%
\item[\Timts{ESC\_SPEED}] The speed of an escalator (m/s).  One can
  use EVSS as a simple model of an escalator, agents are all moving
  with the velocity ESC\_SPEED along the escalator, \emph{i.e.}, they
  are not overtaking each others or walking on the escalator.
%
%
\end{description}
%
...
\chapter{Additional FDS+Evac Input Files}\label{Sec_InFiles}
...
calculation.  If this file exist and there are no fire meshes in the
FDS input file, it is read in.  Otherwise it is (re)calculated and
saved on the disk.  This file is useful if one is doing many egress
calculations using exactly the same geometry but with different human
input, \emph{e.g.}, varying the number of humans and the democraphics
of humans.  This is very useful when doing Monte Carlo calculations in
a quite complex buildings.  One do not need to calculate the
...
is tried to read in, if there are no fire meshes specified in the
input file.  If there is at least one 'fire' mesh specified and also
at least one main evacuation mesh (\Timtt{EVAC\_HUMANS=.TRUE.} and
\Timtt{EVACUATION=.TRUE.})  specified, then this file is
(re)calculated and saved on the disk.
...
one, which wrote the files.  Note, that the eff-file does not depend
on the namelists \Timtt{DOOR}, \Timtt{EXIT}, and \Timtt{CORR}, whereas
the fed-file does.



\chapter{FDS+Evac: Output Files}\label{Sec_OutFiles}
...
agents inside the whole building.  Then the number of agents in each
main evacuation meshes (``floors'') and in each corridor/stairs
(\Timtt{CORR} namelists) are given.  After these the number of agents
gone through various exits and doors are reported.  The next colums
are counters for the exit selection algorithm.  They report the number
of agents heading to the different exit doors.  The last three columns
are printed only if the FED is used, \emph{i.e.}, the smoke and toxic
gas information is available.  The first of these columns reports the
number of dead agents, \emph{i.e.}, those whose FED values are larger
...
the last column is the maximum FED value of the agents which are
alive.

Agent movement may be visualized by Smokeview, where 'Evacuation' is
shown on the ``Load/Sav'e'' menu.  The agents are saved on '.prt5'
files.  You can change the appearance of the agents using the
``Show/Hide'' menu and the ``Use Avatar:'' submenu.  The coloring
scheme of the agents can be changed using the ``Show/Hide'' menu and
the submenu ``Humans'' if the chosen \Timtt{COLOR\_METHOD} on the
\Timtt{PERS} namelist allows that.  During the run of FDS+Evac some
evacuation information is printed on the text file
\Timtt{CHID\_evac.out}.

The file \Timtt{CHID\_evac.eff} contains the converged evacuation flow
fields and the file \Timtt{CHID\_evac.fed} contains the smoke and gas
concentration information for the evacuation calculation.  These might
be used later, if more evacuation calculations are done using same
geometrical information, \emph{e.g.}, if one is doing a Monte Carlo
simulation of the egress scenario.
...
\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,
    width=80mm]{FIGURES/evac_example1_EvacGeom.eps}~~\includegraphics[clip=true,
    width=80mm]{FIGURES/evac_example1_FireGeom.eps}}
...
agents from the second floor are using.

\begin{figure}[!tb]
  \centerline{\includegraphics[clip=true,
    width=120mm]{FIGURES/evac_example2_EvacGeom.eps}}
...
The develepment work of FDS+Evac is an undergoing project.  Read
...
also the VTT's project summary report~\cite{Hostikka07a} to get a nice
overview of the development work.

The present status of the FDS+Evac (FDS 5.2.0, Evac 2.0.0) is:
%
\begin{itemize}
%
...
%
\item Inclines, stairs, and escalators can be modelled explicitely by
  using the equations of motion for each person.
%
\item A simple stair algorithm without merging flows is implemented.
  If there are merging flows in staris then these should be modelled
  explicitely, which means some additional work to construct the input
  file containing the landings and stairs.
%
...
The TO DO list of the development work of FDS+Evac is:
%
\begin{itemize}
%
\item Merging flows in stairs.
%
\item A more realistic \Timtt{CORR} object: Record the position of
  each agent inside the corridor/stairs and use the equations of
  motion to model the movement in the corridor/stairs.
%
\item More pre-defined human types, especially those refered in the
...
\item More intelligence in the exit door selection algorithm,
  ...
  heading/queuing for the same exit door.
%
\item Social interactions like herding \emph{etc.}
%
\item Sreading of the information: agent to agent communication.
%
...
\item The evacuation objects, like exits, doors, \emph{etc.}, are not
  visualized on the Smokeview window.
%
\item The user input is not easy to give, \emph{i.e.}, no support for
  ...
  commersial software tools that generate FDS fire input and these
...
manual.  This page contains more verification and validation results
and example cases than this manual.
...
\bibitem{Korhonen08a} Korhonen, T., Hostikka, S., Heli\"ovaara, S.,
  Ehtamo, H., ``FDS+Evac: Modelling Social Interactions in Fire
...
\bibitem{Korhonen08b} Korhonen, T., Hostikka, S., Heli\"ovaara, S.,
  Ehtamo, H., ``FDS+Evac: An Agent Based Fire Evacuation Model,''
...
\bibitem{Heliovaara08} Heli\"ovaara, S., Ehtamo, H., Korhonen, T., and
  ...
  Pedestrian and Evacuation Dynamics, February 27-29, 2008, Wuppertal,
  Germany.
%
...
%
\bibitem{Paloposki02} Paloposki, T., Myllymäki, J., and Weckman, H.,
