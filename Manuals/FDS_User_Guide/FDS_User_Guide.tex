...
By default, the PDPA device output at time $t$ is computed as as a time integral
\be
F(t) = \frac{1}{\min(t,t_e)-t_s} \int_{t_s}^{\min(t,t_e)} f(t) dt
\ee
but instantaneous values can be obtained by setting {\ct PDPA\_INTEGRATE = .FALSE.} on the corresponding {\ct PROP} line. In this case
\be
F(t) = f(t)
\ee
The function $f(t)$ has two forms: 
\be
f_1(t) =  \frac{\sum_i n_i d_i^m x}{\sum_i n_i d_i^n}; \;\;
f_2(t) = \frac{\sum_i n_i x}{V}
\ee
where $n_i$ is the number of real particles repsented by the individual computational particle, $d_i$ 
...
The second form ($f_2(t)$) is used for the computation of mass and energy -related variables that do not include diameter weightings.
