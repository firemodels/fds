...
SELECT CASE(BACKING)
   ...
   CASE(EXPOSED)  ! The backside is exposed to gas in current or adjacent mesh.
      ...
      IF (WC%BACK_MESH/=NM .AND. WC%BACK_MESH>0) THEN  ! Back side is in other mesh.
         ...
         DTMP = TMP_GAS_BACK - B1%TMP_B
         HTCB = HEAT_TRANSFER_COEFFICIENT(WC%BACK_MESH,DTMP,SF%H_FIXED_B,SURF_INDEX_IN=SURF_INDEX,WALL_INDEX_IN=WC%BACK_INDEX)
      ...
      ELSE  ! Back side is in current mesh.
         ...
         IF (PRESENT(WALL_INDEX)) &
            HTCB = HEAT_TRANSFER_COEFFICIENT(NM,DTMP,SF%H_FIXED_B,SURF_INDEX_IN=SURF_INDEX_BACK,WALL_INDEX_IN=WC%BACK_INDEX)
         IF (PRESENT(THIN_WALL_INDEX)) HTCB = 0._EB
         IF (PRESENT(CFACE_INDEX)) &
            HTCB = HEAT_TRANSFER_COEFFICIENT(NM,DTMP,SF%H_FIXED_B,SURF_INDEX_IN=SURF_INDEX_BACK,CFACE_INDEX_IN=CFA%BACK_INDEX)
