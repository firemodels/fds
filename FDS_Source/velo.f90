...
USE TURBULENCE, ONLY: COMPUTE_MEAN_FORCING
...
SELECT CASE(FUNCTION_CODE)
   CASE(1)
      ...
      IF (MEAN_FORCING) CALL COMPUTE_MEAN_FORCING(NM)
   CASE(2)
...
!$OMP DO PRIVATE(IW,II,JJ,KK,IIG,JJG,KKG,ITMP,MU_WALL,YPLUS,DAMPING_FACTOR)
...
REAL(EB) :: T,MUX,MUY,MUZ,UP,UM,VP,VM,WP,WM,VTRM,OMXP,OMXM,OMYP,OMYM,OMZP,OMZM,TXYP,TXYM,TXZP,TXZM,TYZP,TYZM, &
            ...
            DU_FORCING=0._EB,DV_FORCING=0._EB,DW_FORCING=0._EB
REAL(EB) :: VEG_UMAG
...
IF (MEAN_FORCING_U) DU_FORCING = RFAC_FORCING*(U0-UMEAN)/DT
IF (MEAN_FORCING_V) DV_FORCING = RFAC_FORCING*(V0-VMEAN)/DT
IF (MEAN_FORCING_W) DW_FORCING = RFAC_FORCING*(W0-WMEAN)/DT
...
USE TURBULENCE, ONLY: TURBULENT_KINETIC_ENERGY,SUBGRID_SCALAR_VARIANCE,WAVELET_ERROR_ESTIMATOR
...
IF (CHECK_KINETIC_ENERGY .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   CALL TURBULENT_KINETIC_ENERGY(NM)
   !CALL SUBGRID_SCALAR_VARIANCE(NM)
   !CALL WAVELET_ERROR_ESTIMATOR(NM)
ENDIF
...
!$OMP PRIVATE(PROCESS_EDGE,ALTERED_GRADIENT,SLIP_COEF,ICDO_SGN,DUMMY)
