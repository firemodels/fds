...
ALLOCATE(RN_FAST(N_REACTIONS))
RN_FAST = REACTION%FAST_CHEMISTRY

SETUP_FAST: IF (.NOT. ALL(RN_FAST)) THEN

   ALLOCATE(RN_A(N_REACTIONS))
   RN_A = REACTION%A_PRIME
   ALLOCATE(RN_E(N_REACTIONS))
   RN_E = REACTION%E
   ALLOCATE(RN_N_T(N_REACTIONS))
   RN_N_T = REACTION%N_T
   ALLOCATE(RN_C0_EXP(N_REACTIONS))
   RN_C0_EXP = REACTION%C0_EXP
   ALLOCATE(RN_HEAT_OF_COMBUSTION(N_REACTIONS))
   RN_HEAT_OF_COMBUSTION = REACTION%HEAT_OF_COMBUSTION
   ALLOCATE(RN_RHO_E(N_REACTIONS))
   RN_RHO_E = REACTION%RHO_EXPONENT
   ALLOCATE(RN_N_NU(N_REACTIONS))
   RN_N_NU = REACTION%N_SMIX_FR
   ALLOCATE(RN_N_SPEC(N_REACTIONS))
   RN_N_SPEC = REACTION%N_SPEC
   ALLOCATE(RN_N_THIRD(N_REACTIONS))
   RN_N_THIRD = REACTION%N_THIRD
   ALLOCATE(RN_REVERSE(N_REACTIONS))
   RN_REVERSE = REACTION%REVERSE
   ALLOCATE(RN_THIRD(N_REACTIONS))
   RN_THIRD = REACTION%THIRD_BODY
   ALLOCATE(RN_FUEL_SMIX_INDEX(N_REACTIONS))
   RN_FUEL_SMIX_INDEX = REACTION(:)%FUEL_SMIX_INDEX
   ALLOCATE(RN_AIR_SMIX_INDEX(N_REACTIONS))
   RN_AIR_SMIX_INDEX = REACTION(:)%AIR_SMIX_INDEX

   ALLOCATE(RN_NU(MAXVAL(RN_N_NU,1),N_REACTIONS))
   ALLOCATE(RN_NU_I(MAXVAL(RN_N_NU,1),N_REACTIONS))
   RN_NU = 0._EB
   RN_NU_I = -1

   ALLOCATE(RN_N_S(MAXVAL(RN_N_SPEC,1),N_REACTIONS))
   ALLOCATE(RN_N_S_I(MAXVAL(RN_N_SPEC,1),N_REACTIONS))
   RN_N_S = 0._EB
   RN_N_S_I = -1

   ALLOCATE(RN_THIRD_EFF(N_SPECIES,N_REACTIONS))
   RN_THIRD_EFF = 1._EB

   DO NR=1,N_REACTIONS
      IF (REACTION(NR)%FAST_CHEMISTRY) CYCLE
      DO NS = 1,REACTION(NR)%N_SMIX_FR
         RN_NU_I(NS,NR) = REACTION(NR)%NU_INDEX(NS)
         RN_NU(NS,NR) = REACTION(NR)%NU_MW_O_MW_F_FR(NS)
      ENDDO
      DO NS = 1,REACTION(NR)%N_SPEC
         RN_N_S_I(NS,NR) = REACTION(NR)%N_S_INDEX(NS)
         RN_N_S(NS,NR) = REACTION(NR)%N_S(NS)
      ENDDO
      IF (REACTION(NR)%N_THIRD > 0) RN_THIRD_EFF(:,NR) = REACTION(NR)%THIRD_EFF(:)
   ENDDO

   IF (ANY(RN_REVERSE)) THEN
      ALLOCATE(RN_DELTA_G(0:I_MAX_TEMP,N_REACTIONS))
         RN_DELTA_G = 0._EB
      DO NR=1,N_REACTIONS
         IF (REACTION(NR)%REVERSE) RN_DELTA_G(:,NR) = REACTION(NR)%DELTA_G
      ENDDO
   ENDIF
...
PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (LPC%CNF_RAMP_INDEX > 0) THEN
      LPC%MINIMUM_DIAMETER = RAMPS(LPC%CNF_RAMP_INDEX)%INDEPENDENT_DATA(1)*1.E-6_EB
      IF (LPC%MINIMUM_DIAMETER < 0.1E-6_EB) THEN
