...
More general way to specify gaseous yields is to use {\ct NU\_GAS}
keyword, as explained in Section~\ref{nu_gas}.
...
all. Eq.~(\ref{rr}) describes the rate of the reaction as a function
...
\noindent
Note that the {\ct (1)} has been added to the reaction parameters to
...
using the {\ct NU\_FUEL} and {\ct NU\_WATER} keywords at the {\ct
MATL} line. A yield of any gaseous species that is present in the FDS
simulation can be specified using {\ct NU\_GAS} keyword. It is used in
the form {\ct NU\_GAS(j,s)} where {\ct j} refers to the j'th reaction
of the current material, and {\ct s} refers to the s'th species. The
order of the gaseous species is the order of the corresponding {\ct
SPEC} lines. Note howerer, that in many cases FDS will automatically
generate few species for mixture fraction and water vapor. It is best
to check the order and numbering of the species from the CHID.out
file. {\ct NU\_GAS} can also be used to specify the yields of the
mixture fraction and water vapor.

For consistency, {\ct HEAT\_OF\_COMBUSTION(j,s)} can also be specified
separatelly for each reaction and species. These values values are
used only if the corresponding heat of combustion for the gaseous
species is greater than zero.

In the example below, the pyrolysis of wood is modelled in the context
of finite rate reactions. The yield of gaseous fuel is specified for
propane, which in the example is used as a gas phase fuel.
