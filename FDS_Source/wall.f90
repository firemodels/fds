...
REAL(EB) :: T,ZZ_G,RHO_G,UN,DD,MFT,TSI,RADIUS,AREA_SCALING,&
            RVC,RHO_NEW,M_DOT_PPP,CP,CPBAR,MW_RATIO,H_G,DELTA_H_G,ZZ_GET(0:N_TRACKED_SPECIES),CPBAR2
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   NEW_EVAP_IF: IF (NEW_EVAP) THEN
      MATERIAL_LOOP2B: DO N=1,SF%N_MATL
         ...
         X_G = ZZ_GET(I_FUEL)/SPECIES_MIXTURE(I_FUEL)%MW*MW_G
         X_W = MIN(1._EB,EXP(ML%H_R(1)*SPECIES_MIXTURE(I_FUEL)%MW/R0*(1./ML%TMP_BOIL-1./WC%TMP_S(1))))
         IF (DNS) THEN
            ITMP = MIN(4999,INT(TMP(I,J,K)))
            TMP_WGT = TMP(I,J,K) - ITMP
            D_AIR = D_Z(ITMP,I_FUEL)+TMP_WGT*(D_Z(ITMP+1,I_FUEL)-D_Z(ITMP+1,I_FUEL))
         ...
         ENDIF
         MFLUX = MAX(0._EB,SPECIES_MIXTURE(I_FUEL)%MW/R0/WC%TMP_S(1)*H_MASS*LOG((X_G-1._EB)/(X_W-1._EB)))
   ...
   ELSE NEW_EVAP_IF !Old routine
      ...
      MATERIAL_LOOP2: DO N=1,SF%N_MATL
         ...
         HVRG    = SPECIES_MIXTURE(I_FUEL)%MW*ML%H_R(1)/R0
         PPSURF  = MIN(1._EB,(R0/SPECIES_MIXTURE(I_FUEL)%MW)*ZPRSUM)
         PPCLAUS = MIN(1._EB,EXP(HVRG*(1./ML%TMP_BOIL-1./WC%TMP_S(1))))
         ! Make initial guess
         IF ((ABS(MFLUX)<=ZERO_P) .AND. (PPSURF<PPCLAUS)) THEN         
            MFLUX = (ML%INIT_VAPOR_FLUX/(R0*TMPA/P_INF))*SPECIES_MIXTURE(I_FUEL)%MW/ML%ADJUST_BURN_RATE(I_FUEL,1)
         ENDIF
...
REAL(EB) :: U2=0._EB,V2=0._EB,W2=0._EB,VELCON,H_NATURAL,H_FORCED,CP,ZZ_GET(0:N_TRACKED_SPECIES),NUSSELT,MU_G
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU=>NULL(),VV=>NULL(),WW=>NULL(),RHOP=>NULL()
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP=>NULL()
...
NO_GAS_CELL_IF: IF (IIG>=0) THEN  
   ...
   IF (H_EDDY .OR. H_LOGLAW .OR. H_CHILTON_COLBURN) THEN
      ...
      IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES))
...
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_SOOT,RHOG,ZZ_GET(0:N_TRACKED_SPECIES),YDEP,K_AIR,TMP_FILM
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   KKG = IJKW(8,IW)
   ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,ZZ(IIG,JJG,KKG,1:N_TRACKED_SPECIES))
   IF (ZZ_GET(I_PROG_SOOT) < 1.E-14_EB) CYCLE WALL_CELL_LOOP
