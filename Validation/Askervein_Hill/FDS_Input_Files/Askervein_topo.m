...
dump = ['&DUMP NFRAMES=3600,DT_PL3D=1.E10/'];
fprintf(fid,'%s\n',dump);
fprintf(fid,'%s\n','  '); % blank line
...
vent = ['&VENT MB=''XMIN'',SURF_ID=''PERIODIC''/']; fprintf(fid,'%s\n',vent);
vent = ['&VENT MB=''XMAX'',SURF_ID=''PERIODIC''/']; fprintf(fid,'%s\n',vent);
vent = ['&VENT MB=''YMIN'',SURF_ID=''PERIODIC''/']; fprintf(fid,'%s\n',vent);
vent = ['&VENT MB=''YMAX'',SURF_ID=''PERIODIC''/']; fprintf(fid,'%s\n',vent);
...
for i=1:N(1)
    for j=1:N(2)
        ...
        if xf(i)>min_x_data & xf(i)<max_x_data & yf(j)>min_y_data & yf(j)<max_y_data
            ...
            zi = interp2(X,Y,Z_data,xf(i)+dx/2,yf(j)+dy/2);
            if zi>z_min+dz/2
