...
def plot_csv(chid, time, quan, name, tstart, tend, pos):
    ...
    ax.grid(True)
    ax.set_title('Position %s '%pos,fontsize=30)

    ax.set_xlabel('Time [s]',fontsize=16)
    ax.set_ylabel(r'%s' %name,fontsize=16)

    print 'Ranges:', min_total, max_total
    ...
    output = "pictures/%s_%s_ts%.2f_te%.2f.png" % (name, pos, tstart, tend)
    savefig(output)
    #show()
...
chid=[]
if 'middle' in pos:
    #chid.append('fft_middle_1mesh')
    #chid.append('glmat_middle_1mesh')
    #chid.append('glmat_middle_8mesh')
    chid.append('mkl_middle_1mesh')
    chid.append('mkl_middle_8mesh')
elif 'right' in pos:
    #chid.append('fft_right_1mesh')
    #chid.append('glmat_right_1mesh')
    #chid.append('glmat_right_8mesh')
    chid.append('mkl_right_1mesh')
    chid.append('mkl_right_8mesh')
else:
    print "Wrong pos"
    sys.exit()
...
plot_csv(chid, time, uvel, 'pres1', tstart, tend, pos)
plot_csv(chid, time, vvel, 'pres2', tstart, tend, pos)
plot_csv(chid, time, wvel, 'pres3', tstart, tend, pos)
plot_csv(chid, time, pres, 'pres4', tstart, tend, pos)
