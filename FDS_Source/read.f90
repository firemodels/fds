...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,CHECK_GR,CHECK_HT,CDF_CUTOFF,CHECK_VN,CLIP_MASS_FRACTION,CP_FTMP,&
...
INTEGER :: SAMPLING_FACTOR,N,NN,ILPC,IPC,RGB(3),I_DUMMY(6),NUMBER_RADIATION_ANGLES
...
LOGICAL :: MASSLESS,STATIC,TREE,MONODISPERSE,EVAPORATE,BREAKUP,CHECK_DISTRIBUTION,CONSTANT_TEMPERATURE,FUEL,WATER
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CHILD_DIAMETER,BREAKUP_CHILD_GAMMA_D,BREAKUP_CHILD_SIGMA_D,CHECK_DISTRIBUTION,&
                CHILD_CNF_RAMP_ID,CHILD_DISTRIBUTION,CNF_RAMP_ID,COLOR,COMPLEX_REFRACTIVE_INDEX,CONSTANT_TEMPERATURE,&
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (TREE .AND. (SPEC_ID/='null' .OR. SURF_ID/='null')) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Cannot have TREE=.TRUE. with either SURF_ID or SPEC_ID'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   ...
   IF (LPC%SPEC_ID/='null') THEN
      IF (EVAPORATE) THEN
         ...
         ENDIF
      ENDIF
      IF (LPC%SURF_ID/='null') THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Cannot set both SURF_ID and SPEC_ID'     
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      DO NN=0,N_TRACKED_SPECIES
         IF (TRIM(SPECIES_MIXTURE(NN)%ID)==TRIM(LPC%SPEC_ID)) THEN
            LPC%Z_INDEX = NN
            IF (EVAPORATE) SPECIES_MIXTURE(NN)%EVAPORATING = .TRUE.
            EXIT
         ENDIF
      ENDDO
   ...
   IF (DIAMETER > 0._EB) THEN
      ...
      ALLOCATE(LPC%IL_CDF(NSTRATA),STAT=IZERO)
      CALL ChkMemErr('READ','IL_CDF',IZERO)
      ALLOCATE(LPC%IU_CDF(NSTRATA),STAT=IZERO)
      CALL ChkMemErr('READ','IU_CDF',IZERO)
      ALLOCATE(LPC%W_CDF(NSTRATA),STAT=IZERO)
   ...
   IF (BREAKUP) THEN
      ALLOCATE(LPC%CHILD_CDF(0:NDC),STAT=IZERO)
      CALL ChkMemErr('READ','CHILD_CDF',IZERO)
      ALLOCATE(LPC%CHILD_R_CDF(0:NDC),STAT=IZERO)
   ...
   LPC%ID                     = ID
   LPC%BREAKUP                = BREAKUP
   LPC%BREAKUP_CHILD_DIAMETER = BREAKUP_CHILD_DIAMETER
   LPC%BREAKUP_CHILD_GAMMA    = BREAKUP_CHILD_GAMMA_D
   IF ( BREAKUP_CHILD_SIGMA_D > 0._EB ) THEN
      LPC%BREAKUP_CHILD_SIGMA = BREAKUP_CHILD_SIGMA_D
   ELSE
      LPC%BREAKUP_CHILD_SIGMA = 1.15_EB/BREAKUP_CHILD_GAMMA_D
   END IF
   LPC%CTRL_ID                = CTRL_ID
   LPC%DENSE_VOLUME_FRACTION  = DENSE_VOLUME_FRACTION
   LPC%DENSITY                = DENSITY
   LPC%DEVC_ID            = DEVC_ID
   LPC%EVAPORATE          = EVAPORATE
   ...
   LPC%CHILD_DISTRIBUTION = CHILD_DISTRIBUTION
   LPC%CNF_RAMP_ID        = CNF_RAMP_ID
   LPC%CHILD_CNF_RAMP_ID  = CHILD_CNF_RAMP_ID
   ...
   IF(LPC%CHILD_CNF_RAMP_ID/='null') THEN
        CALL GET_RAMP_INDEX(LPC%CHILD_CNF_RAMP_ID,'DIAMETER',LPC%CHILD_CNF_RAMP_INDEX)
   ENDIF
   ...
   LPC%SPEC_ID            = SPEC_ID
   LPC%SURF_ID            = SURF_ID
   LPC%SURF_INDEX         = -1      ! SURFs have not yet been read in
   ...
   LPC%N_SPLIT                  = NUMBER_RADIATION_ANGLES
   IF (LPC%N_SPLIT>1) THEN   
      ALLOCATE(LPC%ORIENTATION(1:LPC%N_SPLIT,1:3),STAT=IZERO)
...
PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   ! Get density if mass but not evaporating
   IF (.NOT. LPC%EVAPORATE .AND. .NOT. LPC%MASSLESS .AND. LPC%Y_INDEX > 0) THEN
   ...
   ! Determine OMESH%LP array sizes
   OMESH_PARTICLES_IF: IF (.NOT. LPC%STATIC) THEN
      IF (NMESHES>0) OMESH_PARTICLES=.TRUE.
      IF (LPC%SURF_INDEX > 0) THEN
         IF (RADIATION) THEN
            OM_LP_ILW_A = NUMBER_RADIATION_ANGLES
            OM_LP_ILW_B = NUMBER_SPECTRAL_BANDS
         ENDIF
         OM_LP_NODES  = MAX(OM_LP_NODES,  SURFACE(LPC%SURF_INDEX)%N_CELLS)
         OM_LP_MATL   = MAX(OM_LP_MATL,   SURFACE(LPC%SURF_INDEX)%N_MATL)
         OM_LP_LAYERS = MAX(OM_LP_LAYERS, SURFACE(LPC%SURF_INDEX)%N_LAYERS)
      ENDIF
   ENDIF OMESH_PARTICLES_IF
   
   
   ! Only process particles or PARTICLEs that have mass and evaporate

   IF (.NOT. LPC%EVAPORATE .OR. LPC%MASSLESS .OR. LPC%SURF_ID/='null' .OR. WFDS_FE) CYCLE PART_LOOP
   
   SURF_OR_SPEC: IF (LPC%Y_INDEX > 0) THEN
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BEAD_DENSITY,BEAD_DIAMETER,BEAD_EMISSIVITY,&
                ...
                CABLE_MASS_PER_LENGTH,CHARACTERISTIC_VELOCITY,CONDUIT_DIAMETER,CONDUIT_THICKNESS,C_FACTOR,PARTICLES_PER_SECOND,&
...
PROP_LOOP: DO N=0,N_PROP
   ...
   IF (PY%PART_ID/='null') THEN
      ...
      ENDIF
      PARTICLE_FILE=.TRUE.
      IF (PY%FLOW_RATE > 0._EB .AND. LPC%SURF_INDEX > 0) THEN
         WRITE(MESSAGE,'(A,A,A)') 'PROP ERROR: PARTicle class ',TRIM(LPC%ID),' cannot be used with FLOW_RATE'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      IF (PY%FLOW_RATE > 0._EB .AND. .NOT. LPC%EVAPORATE .AND. LPC%DENSITY < 0._EB) THEN
...
DO ILPC=1,N_LAGRANGIAN_CLASSES
   LPC=>LAGRANGIAN_PARTICLE_CLASS(ILPC)
   IF (LPC%SURF_ID/='null' .AND. LPC%SURF_INDEX<1) THEN
   ...
   SEARCH_LOOP: DO
      ...
      SELECT CASE(TABLE_TYPE(N))
         ...
         CASE (PART_RADIATIVE_PROPERTY)
            ...
            ENDIF
         CASE (POINTWISE_INSERTION)
            IF (ABS(TABLE_DATA(7)-999._EB)>ZERO_P .AND. TABLE_DATA(7)<0._EB) THEN
               WRITE(MESSAGE,'(A,I5,A,A,A)') 'ERROR: Row ',TA%NUMBER_ROWS,' of ',TRIM(TABLE_ID(N)),' has a bad diameter'
               CALL SHUTDOWN(MESSAGE)  
            ENDIF
            IF (TABLE_DATA(8)+999._EB>0._EB .AND. TABLE_DATA(8)<-273.15) THEN
               WRITE(MESSAGE,'(A,I5,A,A,A)') 'ERROR: Row ',TA%NUMBER_ROWS,' of ',TRIM(TABLE_ID(N)),' has a bad temperature'
               CALL SHUTDOWN(MESSAGE)      
            ENDIF
            IF (TABLE_DATA(9)+999._EB>0._EB .AND. TABLE_DATA(9)<0._EB) THEN
               WRITE(MESSAGE,'(A,I5,A,A,A)') 'ERROR: Row ',TA%NUMBER_ROWS,' of ',TRIM(TABLE_ID(N)),' has a bad mass per time'
               CALL SHUTDOWN(MESSAGE)            
            ENDIF
      END SELECT
...
READ_TABL_LOOP: DO N=1,N_TABLE
   ...
   SEARCH_LOOP2: DO 
      ...
      NN = NN+1
      IF (TABLE_TYPE(N)==POINTWISE_INSERTION) THEN
         TABLE_DATA(7) = TABLE_DATA(7) * 1.E-6_EB
         IF(TABLE_DATA(8)<=-999._EB) THEN
            TABLE_DATA(8) = TMPA
         ELSE
            TABLE_DATA(8) = TABLE_DATA(8) + TMPM
         ENDIF
      ENDIF
...
REAL(EB) :: TEMPERATURE,DENSITY,MASS_FRACTION(1:MAX_SPECIES),RR_SUM,ZZ_GET(0:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
            MASS_PER_TIME,DT_INSERT,UVW(3),DIAMETER,HRRPUV
INTEGER  :: N,NN,NNN,II,JJ,KK,NS,NS2,NUMBER_INITIAL_PARTICLES,N_INIT_NEW,N_INIT_READ
LOGICAL  :: UNIFORM = .FALSE.
CHARACTER(30) :: CTRL_ID,DEVC_ID,PART_ID,TABL_ID,SHAPE,MULT_ID,ORIGIN='CENTER',PROF_ID,SPEC_ID(1:MAX_SPECIES)
TYPE(INITIALIZATION_TYPE), POINTER :: IN=>NULL()
TYPE(MULTIPLIER_TYPE), POINTER :: MR=>NULL()
TYPE(LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC=>NULL()
NAMELIST /INIT/ CTRL_ID,DENSITY,DEVC_ID,DT_INSERT,HRRPUV,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,MULT_ID,&
                NUMBER_INITIAL_PARTICLES,ORIGIN,PART_ID,PROF_ID,SHAPE,SPEC_ID,TABL_ID,TEMPERATURE,UNIFORM,UVW,XB
      ...
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IF (PART_ID/='null') THEN
               ...
               IN%NUMBER_INITIAL_PARTICLES = NUMBER_INITIAL_PARTICLES*LPC%N_SPLIT
            ...
            IF (TABL_ID/='null') CALL GET_TABLE_INDEX(TABL_ID,POINTWISE_INSERTION,IN%TABL_INDEX)
...
INTEGER  :: NN,NM,MESH_NUMBER,N_DEVC_READ,IOR,TRIP_DIRECTION,VELO_INDEX,POINTS,I_POINT,VIRTUAL_INDEX,PIPE_INDEX
...
NAMELIST /DEVC/ BYPASS_FLOWRATE,CONVERSION_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,DUCT_ID,EVACUATION,FLOWRATE,FYI,&
                HIDE_COORDINATES,ID,INITIAL_STATE,IOR,LATCH,MATL_ID,NODE_ID,ORIENTATION,OUTPUT,PART_ID,PIPE_INDEX,POINTS,&
                PROP_ID,QUANTITY,&
...
REAL(EB) :: SETPOINT(2), DELAY, CYCLE_TIME,CONSTANT
CHARACTER(30) :: ID,FUNCTION_TYPE,INPUT_ID(40),RAMP_ID,ON_BOUND
TYPE (CONTROL_TYPE), POINTER :: CF=>NULL()
NAMELIST /CTRL/  CONSTANT,CYCLES,CYCLE_TIME,DELAY,EVACUATION,FUNCTION_TYPE,ID,INITIAL_STATE,INPUT_ID,LATCH,N,ON_BOUND,RAMP_ID,&
                 SETPOINT,TRIP_DIRECTION
