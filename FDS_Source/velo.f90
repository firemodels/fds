...
IF (LES .OR. EVACUATION_ONLY(NM)) THEN
   ...
   !$OMP PRIVATE(A_IJ,AA,JJ,II,B_IJ,BB,NU_EDDY,RHOT,RHOB,DRHODZ,DELTA_G)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
...
!!!$OMP DO PRIVATE(IW,II,JJ,KK,IIG,JJG,KKG,ZZ_GET,ITMP,TMP_WGT,MU_WALL)
...
!$OMP SINGLE PRIVATE(N,OB,K,J,I,IC1,IC2,DUUDT,DVVDT,DWWDT)
