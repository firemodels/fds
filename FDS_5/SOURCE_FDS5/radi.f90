...
IF (GAS_CELL_RAD_FLUX) THEN
   DO N=1, N_GAS_CELL_RAD_DEVC
      ALLOCATE(DEVICE(GAS_CELL_RAD_DEVC(N))%ILW(NRA,NSB),STAT=IZERO)
      CALL ChkMemErr('INIT','DV%ILW',IZERO)
      DEVICE(GAS_CELL_RAD_DEVC(N))%ILW = 0._EB
   ENDDO
ENDIF
...
USE DEVICE_VARIABLES, ONLY : DEVICE_TYPE,DEVICE, GAS_CELL_RAD_FLUX, GAS_CELL_RAD_DEVC, N_GAS_CELL_RAD_DEVC
...
! Interpolate boundary intensities onto other meshes
            ...
            IF (GAS_CELL_RAD_FLUX) THEN
               DEVC_LOOP: DO NN = 1, N_GAS_CELL_RAD_DEVC
                  DV => DEVICE(GAS_CELL_RAD_DEVC(NN))
