...
Methods for visualizing data such as 2D
contours or 3D iso-surfaces are useful for analyzing data quantitatively but are not
as suitable for applications where realism is required such as in a fire fighting simulator.
Realistic smoke visualization methods need to determine opacities and colors from simulation
data using physics based principles.  
This note describes two methods for visualizing smoke realistically both using a form of the radiation transport equation\cite{} to model
smoke opacity and color.
Simplifications used to make these visualizations practical are discussed.

A complete method for visualizing smoke and fire taking into
account light/smoke interactions such as absorption, emission, and scattering (both in and out)
requires the solution of the the volume rendering equation\cite{}
also called the radiation transport equation (RTE)\cite{}.
This equation models how light appears to an observer after interacting with a participating media in this case smoke.
The equation solved here for visualization purposes is the same one used in a fire model such as FDS\cite{}
for modeling radiative heat transfer except that a different wave length of light (visible rather than infrared) is used in the solution.

The
solution of this integro-differential equation requires significant
memory and computational resources.  Approximations
...
Two techniques for visualizing smoke realistically are slice rendered and volume
...
They 
differ in the path over which they are integrated.
The integration path for a slice rendered method is a segment between two adjacent grid planes within a 
3D mesh.  The integration path for a volume rendered method is a segment between the front and back surface of a 3D mesh (across multiple grid planes).
...
one slice plane. As a result, there is only one plane of data drawn.  The video hardware is again exploited, but this time to
