...
REAL(EB) :: RE,H_NATURAL,H_FORCED,NUSSELT,FRICTION_VELOCITY=0._EB,YPLUS=0._EB,ZSTAR,DN,TMP_FILM,MU_G,K_G,CP_G,&
            RA,TMP_G,R_DROP,U_SURF,V_SURF,W_SURF,TMP_2,H_1,H_2,GAMMA,DTDX_W,DUMMY,RHO_G,ZZ_G(1:N_TRACKED_SPECIES),CONV_LENGTH
...
HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
   ...
   CASE(FREE_HORIZONTAL_CYLINDER_HTC_MODEL)
      ...
      CALL GET_CONDUCTIVITY(ZZ_G,K_G,TMP_FILM)
      CALL GET_SPECIFIC_HEAT(ZZ_G,CP_G,TMP_FILM)
      RA = GRAV*ABS(DELTA_TMP)*CONV_LENGTH**3*RHO_G**2*CP_G/(TMP_FILM*MU_G*K_G)
      NUSSELT = (0.6_EB + (0.387_EB*RA**(1._EB/6._EB))/(1._EB+(0.559_EB/PR_AIR)**(9._EB/16._EB))**(8._EB/27._EB))**2
      H_NATURAL = NUSSELT*K_G/CONV_LENGTH
      H_FORCED  = 0._EB
