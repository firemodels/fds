...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,MU_IN,ZETA_INOUT,AIT_IN,PBAR_IN,DELTA,CELL_VOLUME)
USE COMP_FUNCTIONS, ONLY: SHUTDOWN
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE PHYSICAL_FUNCTIONS, ONLY: GET_AVERAGE_SPECIFIC_HEAT,GET_SPECIFIC_GAS_CONSTANT,GET_ENTHALPY
REAL(EB), INTENT(IN) :: T,DT,TMP_IN,RHO_IN,MU_IN,PBAR_IN,AIT_IN,DELTA,CELL_VOLUME
...
REAL(EB) :: ERR_EST,ERR_TOL,A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            PBAR_0,VEL_RMS,ZZ_0(1:N_TRACKED_SPECIES),&
...
INTEGER :: NR,NS,ITER,TVI,RICH_ITER,TIME_ITER,RICH_ITER_MAX,CO_PASS,N_CO_PASS
...
N_CO_PASS = 1
IF (EXTINCT_MOD==EXTINCTION_3) N_CO_PASS = 2

CO_EXTINCT_LOOP: DO CO_PASS = 1,N_CO_PASS
   ...
   INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_ITERATIONS
      ...
      IF (SUPPRESSION) CALL CHECK_AUTO_IGNITION(EXTINCT,TMP_MIXED,AIT_LOC,CO_PASS)
      ...
      INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE_SOLVER)
         ...
         CASE (EXPLICIT_EULER) ! Simple chemistry
            ...
            CALL FIRE_FORWARD_EULER(ZZ_MIXED_NEW,ZZ_MIXED,ZZ_UNMIXED,ZETA,ZETA_0,DT_SUB,TMP_MIXED,TMP_UNMIXED,RHO_HAT,&
                                    CELL_MASS,TAU_MIX,PBAR_0,DELTA,VEL_RMS,Q_REAC_SUB,TIME_ITER,TOTAL_MIXED_MASS,CO_PASS)
            ZETA_0 = ZETA
   ...
   IF (SUPPRESSION) THEN
      SELECT CASE(EXTINCT_MOD)
         ...
         CASE(EXTINCTION_3); CALL EXTINCT_3(EXTINCT,ZZ_0,ZZ_MIXED,TMP_IN,CO_PASS)
      END SELECT
   ENDIF

   IF (.NOT.EXTINCT) THEN
      EXIT CO_EXTINCT_LOOP
   ELSE
      ZZ_GET = ZZ_0
      ZZ_STORE(:,:) = 0._EB
      Q_OUT = 0._EB
...
SUBROUTINE CHECK_AUTO_IGNITION(EXTINCT,TMP_MIXED,AIT_IN,CO_PASS)
...
SELECT CASE (EXTINCT_MOD)
   ...
   CASE(EXTINCTION_3)
      ...
      ELSE
         AIT_LOC = RN%AUTO_IGNITION_TEMPERATURE
      ENDIF
      IF ( TMP_MIXED > AIT_LOC ) EXTINCT = .FALSE.
...
DO NS = 1,N_TRACKED_SPECIES
   IF (NS==R1%FUEL_SMIX_INDEX) THEN
      ZZ_HAT_0(NS) = ZZ_0_IN(NS)             ! FDS Tech Guide (5.15)
      ZZ_HAT(NS)   = ZZ_IN(NS)               ! FDS Tech Guide (5.18)
   ELSEIF (NS==R1%AIR_SMIX_INDEX) THEN
      ZZ_HAT_0(NS) = ZZ_0_IN(NS) - ZZ_IN(NS) ! FDS Tech Guide (5.16)
      ZZ_HAT(NS)   = 0._EB                   ! FDS Tech Guide (5.19)
   ELSE
      ! FDS Tech Guide (5.17)
      ZZ_HAT_0(NS) = ( (ZZ_0_IN(R1%AIR_SMIX_INDEX) - ZZ_IN(R1%AIR_SMIX_INDEX)) / ZZ_0_IN(R1%AIR_SMIX_INDEX) ) * ZZ_0_IN(NS)
      ZZ_HAT(NS)   = ZZ_HAT_0(NS) + ZZ_IN(NS) - ZZ_0_IN(NS) ! FDS Tech Guide (5.20)
   ENDIF
ENDDO
...
IF (H_0 < H_CRIT) EXTINCT = .TRUE. ! FDS Tech Guide (5.21)

END SUBROUTINE EXTINCT_2


SUBROUTINE EXTINCT_3(EXTINCT,ZZ_0_IN,ZZ_IN,TMP_IN,CO_PASS)
USE PHYSICAL_FUNCTIONS,ONLY:GET_ENTHALPY
REAL(EB),INTENT(IN) ::TMP_IN,ZZ_0_IN(1:N_TRACKED_SPECIES),ZZ_IN(1:N_TRACKED_SPECIES)
INTEGER, INTENT(IN) :: CO_PASS
LOGICAL, INTENT(INOUT) :: EXTINCT
REAL(EB) :: ZZ_HAT_0(1:N_TRACKED_SPECIES),ZZ_HAT(1:N_TRACKED_SPECIES),H_0,H_CRIT
INTEGER :: NS
TYPE(REACTION_TYPE), POINTER :: R1=>NULL(),R2=>NULL()
...
DO NS = 1,N_TRACKED_SPECIES
   IF (NS==R1%FUEL_SMIX_INDEX .OR. NS==R2%FUEL_SMIX_INDEX) THEN
...
SUBROUTINE FIRE_FORWARD_EULER(ZZ_OUT,ZZ_IN,ZZ_UNMIXED,ZETA_OUT,ZETA_IN,DT_LOC,TMP_MIXED,TMP_UNMIXED,RHO_HAT,CELL_MASS,TAU_MIX,&
                              PBAR_0,DELTA,VEL_RMS,Q_REAC_LOC,SUB_IT,TOTAL_MIXED_MASS,CO_PASS)
USE COMP_FUNCTIONS, ONLY:SHUTDOWN
...
INTEGER, INTENT(IN) :: SUB_IT,CO_PASS
...
IF (ANY(REACTION(:)%FAST_CHEMISTRY)) THEN
   CALL REACTION_RATE(DZZ,ZZ_0,DT_LOC,RHO_HAT,TMP_MIXED,INFINITELY_FAST,PBAR_0,DELTA,VEL_RMS,Q_REAC_OUT,SUB_IT,CO_PASS)
   ZZ_NEW = ZZ_0 + DZZ
   ZZ_0 = ZZ_NEW
   Q_REAC_LOC = Q_REAC_LOC + Q_REAC_OUT*TOTAL_MIXED_MASS
ENDIF
IF (.NOT.ALL(REACTION(:)%FAST_CHEMISTRY)) THEN
   CALL REACTION_RATE(DZZ,ZZ_0,DT_LOC,RHO_HAT,TMP_MIXED,FINITE_RATE,PBAR_0,DELTA,VEL_RMS,Q_REAC_OUT,SUB_IT,CO_PASS)
   ZZ_NEW = ZZ_0 + DZZ
   Q_REAC_LOC = Q_REAC_LOC + Q_REAC_OUT*TOTAL_MIXED_MASS
ENDIF
...
SUBROUTINE FIRE_RK2(ZZ_OUT,ZZ_IN,ZZ_UNMIXED,ZETA_OUT,ZETA_IN,DT_SUB,N_INC,TMP_MIXED,TMP_UNMIXED,RHO_HAT,CELL_MASS,TAU_MIX,&
                    PBAR_0,DELTA,VEL_RMS,Q_REAC_OUT,SUB_IT,TOTAL_MIXED_MASS_OUT,CO_PASS)
...
REAL(EB), INTENT(INOUT) :: TMP_MIXED
INTEGER, INTENT(IN) :: N_INC,SUB_IT,CO_PASS
...
DO N=1,N_INC
   ...
   CALL FIRE_FORWARD_EULER(ZZ_1,ZZ_0,ZZ_UNMIXED,ZETA_1,ZETA_0,DT_LOC,TMP_MIXED,TMP_UNMIXED,RHO_HAT,CELL_MASS,TAU_MIX,&
                           PBAR_0,DELTA,VEL_RMS,Q_REAC_1,SUB_IT,TOTAL_MIXED_MASS_1,CO_PASS)

   CALL FIRE_FORWARD_EULER(ZZ_2,ZZ_1,ZZ_UNMIXED,ZETA_2,ZETA_1,DT_LOC,TMP_MIXED,TMP_UNMIXED,RHO_HAT,CELL_MASS,TAU_MIX,&
                           PBAR_0,DELTA,VEL_RMS,Q_REAC_2,SUB_IT,TOTAL_MIXED_MASS_2,CO_PASS)
...
INC_LOOP: DO N=1,N_INC
   ...
   CALL FIRE_FORWARD_EULER(ZZ_3,ZZ_2,ZZ_UNMIXED,ZETA_3,ZETA_2, DT_LOC, TMP_MIXED,TMP_UNMIXED,RHO_HAT,CELL_MASS,TAU_MIX,&
                           PBAR_0,DELTA,VEL_RMS,Q_REAC_3,SUB_IT,TOTAL_MIXED_MASS_3,CO_PASS)
...
SUBROUTINE REACTION_RATE(DZZ,ZZ_0,DT_SUB,RHO_0,TMP_0,KINETICS,PBAR_0,DELTA,VEL_RMS,Q_REAC_OUT,SUB_IT,CO_PASS)
USE COMP_FUNCTIONS, ONLY: SHUTDOWN
...
INTEGER, INTENT(IN) :: KINETICS,SUB_IT,CO_PASS
...
KINETICS_SELECT: SELECT CASE(KINETICS)
   ...
   CASE(INFINITELY_FAST)
      ...
      FAST_REAC_LOOP: DO OUTER_IT=1,N_REACTIONS
         IF (CO_PASS==2 .AND. OUTER_IT/=2) CYCLE FAST_REAC_LOOP
         ZZ_TMP = ZZ_NEW
         DZZ = 0._EB
         REACTANTS_PRESENT = .FALSE.
