...
!                     NCSDROP = THFO*AVG_DROP_DEN(I,J,K,EVAP_INDEX)/ (PC%DENSITY*AVG_DROP_RAD(I,J,K,EVAP_INDEX))
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      DO IW = 1,NWC
         IF (BOUNDARY_TYPE(IW)/=SOLID_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=OPEN_BOUNDARY) CYCLE
         IOR = IJKW(4,IW)
      ...
      DO IW=1,NWC
         ...
         ELSEIF (BOUNDARY_TYPE(IW)==OPEN_BOUNDARY) THEN
            QRADIN(IW)  = QRADIN(IW) + E_WALL(IW)*INRAD_W(IW)
         ENDIF
      ENDDO 
