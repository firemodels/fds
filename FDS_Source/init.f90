...
IF (IMMERSED_BOUNDARY_METHOD>=0 .AND. N_GEOM>0) THEN
...
IF (OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLUX)>=TWO_EPSILON_EB .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   IF (ABS(SF%VOLUME_FLUX)    >=TWO_EPSILON_EB)   THEN
