...
\item[{\ct SOOT\_H\_FRACTION}] The fraction of the atoms in the soot that are hydrogen.  Note that this input only applies to simple chemistry {\ct REAC} inputs.  For all other types of {\ct REAC} inputs the species {\ct SOOT} is assumed to be pure carbon unless a {\ct FORMULA} is provided. (default 0.1 - equivalent to the input {\ct FORMULA='C0.9H0.1'} for other {\ct REAC} types)
...
\item[{\ct CHECK\_ATOM\_BALANCE}] If chemical formulas are provide for all {\ct SPEC} that participate in a reaction, then FDS will check the stoichometry to ensure that atoms are conserved.  Setting this flag to {\ct .FALSE.} will bypass this check.  (default is {\ct .TRUE.})
\item{\ct REAC\_ATOM\_ERROR} This is the error tolerance in atoms for the reaction stoichiometry check.  (default is 1.E-5)
\item{\ct REAC\_MASS\_ERROR} This is the relative error tolerance computed as (mass of products - mass of reactants)/(mass of products) for the reaction stoichiometry mass blance check.  (default is 1.E-4)
\end{description}
...
{\ct CHECK\_ATOM\_BALANCE} & Logical        & Check reaction stoichiometry for atom conservation  &                   & {\ct .TRUE.}    \\ \hline
...
{\ct REAC\_ATOM\_ERROR}                   & Real        & Atom conservation error allowed in reaction stoichiometry & atoms             & 1.E-5    \\ \hline
{\ct REAC\_MASS\_ERROR}                   & Real        & Mass conservation error allowed in reaction stoichiometry & kg/kg             & 1.E-4    \\ \hline
