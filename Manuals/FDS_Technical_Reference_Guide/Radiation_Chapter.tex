...
\textbf{Random rotation of anglular directions:}
To mitigate ray effects in the predicted wall radiative heat flux, an option is implemented to randomly rotate the local meridional ($x'$), azimuthal ($y'$), and axial ($z'$) coordinate system at selected time steps. The angular coordinates, $\theta$ and $\phi$, are then evaluated with respect to this rotated coordinate system. Figure~\ref{fig:FVMAngularRotation} illustrates the angular discretization for several rotations of the local coordinate system, where the azimuthal ($y'$) and axial ($z'$) axes are rotated in the $YZ$ plane while the meridional ($x'$) axis remains unchanged. During a simulation with random rotation enabled, the local coordinate system is assigned a new random orientation at each selected time step. As a result, rays originating from a source intersect distant target surfaces from different discrete directions over time, and the time-averaged wall radiative heat flux exhibits substantially reduced ray effects.
...
\includegraphics[height=3in]{FIGURES/RAD_FVM_ANGLES_DEFAULT} &
\includegraphics[height=3in]{FIGURES/RAD_FVM_ANGLES_45DEG} \\[0.3cm]
\includegraphics[height=3in]{FIGURES/RAD_FVM_ANGLES_135DEG} &
\includegraphics[height=3in]{FIGURES/RAD_FVM_ANGLES_225DEG}
\end{tabular*}
