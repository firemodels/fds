...
\end{eqnarray}
Similar to example in Section~\ref{info:Complex_Fuel},we will used the lumped species approach to minimize the number of species that FDS needs to transport. Therefore, the species are defined in the following manner:
...
&SPEC ID = 'SOOT',              SMIX_COMPONENT_ONLY = .TRUE. /
\end{verbatim} \normalsize
Examination of Eq.~(\ref{eq:pu_combust}) and Eq.~(\ref{eq:wood_combust}) show that while Products\_1 and Products\_2 are composed of the same species, the species do not exist in the same proportion or a multiple thereof. As a result, we must construct to separate product lumped species.
\footnotesize \begin{verbatim}
&SMIX ID = 'AIR', SPEC_ID = 'OXYGEN','NITROGEN', VOLUME_FRACTION =1,3.76, BACKGROUND=.TRUE. /
&SMIX ID = 'PRODUCTS_1', SPEC_ID ='CARBON DIOXIDE','CARBON MONOXIDE','WATER VAPOR','SOOT',
           'NITROGEN', VOLUME_FRACTION = 19.7441,0.166587,21,5.0893,103.751 /
&SMIX ID = 'PRODUCTS_2', SPEC_ID ='CARBON DIOXIDE','CARBON MONOXIDE','WATER VAPOR','SOOT',
           'NITROGEN', VOLUME_FRACTION = 0.964384,0.003655,0.85,0.031961,3.84076 /
\end{verbatim} \normalsize
...
&REAC ID = 'wood' 
    ...
    NU=-1,-1.02121,1 /
\end{verbatim} \normalsize
In both of these reactions the {\ct ENTHALPY\_OF\_FORMATION} of the chosen fuels is unknown to FDS so the {\ct HEAT\_OF\_COMBUSTION} is specified for each reaction. Typically in FDS, to set the fire size the Heat Release per Unit Area ({\ct HRRPUA}) is specified on the {\ct SURF} line. The {\ct HRRPUA} parameter cannot currently be used when there is more than one fuel, so the mass flux of fuel must be specified for each fuel. In this example, we want both fuels to flow out of the same burner and ramp up to a 1200~kW fire after 60~s. The first thing is to create a 1~m$^2$ burner by defining a {\ct VENT} line:
\footnotesize \begin{verbatim}
&VENT XB=4.0,5.0,4.0,5.0,0.0,0.0, SURF_ID='FIRE1', COLOR='RED'/
\end{verbatim} \normalsize
This {\ct VENT} points to the {\ct SURF\_ID='FIRE1'} which we can define using both fuels:
\footnotesize \begin{verbatim}
&SURF ID = 'FIRE1', SPEC_ID(1)='POLYURETHANE', MASS_FLUX(1) = 0.241665, RAMP_MF(1)='poly' 
                    SPEC_ID(2)='WOOD',         MASS_FLUX(2) = 0.386074, RAMP_MF(2)='wood' /
\end{verbatim} \normalsize
Note that the {\ct SPEC\_ID}, {\ct MASS\_FLUX}, and {\ct RAMP\_MF} correspond to one another for each fuel. It does not matter which fuel is {\ct (1)}, just that the numbering is consistent. The mass flux values were determined by the desired heat release,the proportion of the total heat release rate each fuel contributes, the burner area, and the heat of combustion of each fuel. We want each fuel to ramp such that fire starts at 0~kW at the initial time, reaches 1200~kW at 100~s, and remains at 1200~kW for the 600~s simulation. 
\footnotesize \begin{verbatim}
...
\end{verbatim} \normalsize
Here, {\ct T} corresponds to the time and {\ct F} is the fraction of the mass flux specified on the {\ct SURF} line.
...
By default, FDS uses a mixing-controlled combustion model, meaning that the reaction rate is infinite and limited by species concentrations. However, FDS can also employ finite-rate reactions using an Arrhenius model. This section describes how to do this.

\begin{enumerate}
\item It is recommended that finite-rate reactions be invoked only
...
\end{enumerate}
First, we consider a single-step reaction mechanism for complete propane combustion:
\begin{equation}\label{eq:propane_1step}
\mathrm{C_3H_8 + 5\,O_{2} \rightarrow 3\,CO_{2} + 4\,H_{2}O}
\end{equation}
In this case we use all primitive species:
\footnotesize
\begin{verbatim}
...
k = AT^{N_{T}}e^{-E/RT}[Y_{\alpha}]^{N_{S}}
\end{equation}

\noindent From Eq.~(\ref{eq:reac_rate}), we can see that the inputs unique to the finite-rate model are given below.
...
\end{description}
If we use Arrhenius parameters found from experiments/literature (for this case Westbrook and Dryer~\cite{Westbrook:1}), Eq.~(\ref{eq:reac_rate}) for the single-step propane reaction becomes:
...
&REAC ID = 'R1'
      ...
      N_S = 0.1,1.65
\end{verbatim} \normalsize
In this example, the {\ct SMIX\_ID}s refer directly to the {\ct SPEC} lines, since all of the species have been defined as primitive. The stoichiometric coefficients ({\ct NU}) can be taken directly from Eq.~(\ref{eq:propane_1step}). If we extend Eq.~(\ref{eq:propane_1step}) to include reactions that account for carbon monoxide production, we end up with Eq.~(\ref{eq:propane_2step})  
...
\end{align}
Again, we will use primitive species for these reactions.
\footnotesize
\begin{verbatim}
&SPEC ID='NITROGEN' BACKGROUND=.TRUE. /
&SPEC ID='METHANE' /
...
\noindent To create the reactions from Eq.~(\ref{eq:propane_2step}), a {\ct REAC} line needs to be defined for each reaction including the reversible reaction. Similar to the one step reaction, the Arrhenius parameters are found from Westbrook and Dryer~\cite{Westbrook:1}:
...
\noindent In {\ct REAC ID = 'R2'}, {\ct WATER VAPOR} concentration impacts the reaction rate as shown by its inclusion on the {\ct SPEC\_ID} and {\ct N\_S} lines, however it does not explicitly participate in the reaction as it is not defined on the {\ct SMIX\_ID} or {\ct NU} lines.
...
\noindent
Particles are introduced into the calculation in several different ways. Liquid droplets are usually introduced via a sprinkler or nozzle. Massless tracer particles are often introduced at a blowing vent or burning surface. Solid particles that represent subgrid-scale objects are introduced randomly throughout a designated volume. Details are found below.

The way to describe particles depends on the type.
If you simply want massless tracers, specify {\ct MASSLESS=.TRUE.} on the {\ct PART} line. If you specify a {\ct SPEC\_ID}, then FDS automatically assumes that you want relatively small, thermally-thin evaporating liquid droplets.
However, it is possible to assign arbitrary surface properties to particles that represent subgrid-scale objects, like office clutter or vegetation. To do this, you add a {\ct SURF\_ID} to the {\ct PART} line. All of these different types of particles are described below.
...
\noindent These lines indicate that the fuel spray nozzles are a separate pipe network from the water sprinklers.  With these inputs, the {\ct PRESSURE\_RAMP} for the water sprinklers would not count any active fuel spray nozzles.
...
({\em e.g.} heat or smoke) this value is taken from the device's {\ct PROP} inputs and need not be specified on the {\ct DEVC} line.
...
The reverse will occur if {\ct ON\_BOUND='UPPER'}, {\em i.e.}, a cooling system.
...
\begin{eqnarray*} (H-z_{int})\; T_u + z_{int} \; T_l = \int_0^H \; T(z) \; dz &=& I_1 \\ [0.1in]
                  (H-z_{int})\; \frac{1}{T_u} + z_{int} \; \frac{1}{T_l} = \int_0^H \; \frac{1}{T(z)} \; dz &=& I_2
\end{eqnarray*}
Solve for $z_{int}$:
...
\be
\mathrm{FED}_\mathrm{CN} = \mathlarger{\int}_0^t \left( \frac{1}{220} \; \exp \left( \frac{C_\mathrm{CN}(t)}{43} \right) - 0.0045 \right) \, dt
\ee
...
repository.  However, only the FDS and Smokeview developers can commit changes to the repository. The current location of the FDS repository is
