...
consumed by the solid phase reaction. Technically it is the enthalpy
difference between the products and reactants at the same
temperature. A positive value indicates that the reaction is {\em
endothermic}; that is, the reaction takes energy out of the
system. Usually the {\ct HEAT\_OF\_REACTION} is accuratelly known only
for simple phase change reactions like the vaporization of water. For
other reactions, it must be determined empirically. The {\ct
HEAT\_OF\_COMBUSTION} ensures that if the fuel vapors burn with a
different $\Delta H$ than that of other materials included in the
simulation, the model will account for this accordingly.
...
of the particle output file used to animate the simulation. The default value is 1, meaning that every particle or droplet will be shown in Smokeview.
...
\noindent
The following parameters are optional, mainly to describe the evaporation of liquid droplets. It is always assumed that non-massless particles are
liquid droplets, but if you specify high values of the vaporization temperature and the heat of vaporization, the effects of evaporation will be minimal.
...
There are several useful attributes that you can assign to particles or droplets, usually via a simple logical parameter.
...
{\ct WATER=.TRUE.} indicates whether the liquid droplets evaporate
into {\ct WATER VAPOR}, which is a separate species that is automatically added to the calculation. By default, {\ct WATER=.FALSE.}, but note that the default
