...
INTEGER :: N,I,J,K,II,JJ,KK,NN,IPTS,JPTS,KPTS,N_EDGES_DIM,N_TOTAL_WALL_CELLS,IW,IWE,IWG,IC,SURF_INDEX,IOR,IOPZ, &
...
DO N=1,N_PART
   ...
   IF (PC%SURF_INDEX>0) THEN
      DO IB=1,N_INIT
         ...
         IF (IN%PART_INDEX==N) M%N_VIRTUAL_WALL_CELLS = M%N_VIRTUAL_WALL_CELLS + IN%NUMBER_INITIAL_PARTICLES
      ENDDO
   ENDIF
ENDDO
...
NOT_EVAC_IF_1: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   DO IW=1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%ZZ_F(N_TRACKED_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%ZZ_F',IZERO)  
      M%WALL(IW)%ZZ_F(1:N_TRACKED_SPECIES) = SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0
   ENDDO
   M%WALL%QRADIN = SIGMA*TMPA4
   M%WALL%QRADOUT = SIGMA*TMPA4
   M%WALL%QCONF = 0._EB
   M%WALL%HEAT_TRANS_COEF = 0._EB
...
NOT_EVAC_IF_2: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   DO IW=1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%MASSFLUX(0:N_TRACKED_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%MASSFLUX',IZERO)  
      M%WALL(IW)%MASSFLUX = 0._EB
      ALLOCATE(M%WALL(IW)%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%MASSFLUX_ACTUAL',IZERO)  
      M%WALL(IW)%MASSFLUX_ACTUAL = 0._EB
   ENDDO
   M%WALL%NPPCW = 1
   M%WALL%BACK_INDEX = 0
   M%WALL%AW = 0._EB
...
M%WALL%PRESSURE_BC_INDEX = NEUMANN
M%WALL%SURF_INDEX_ORIG = 0
M%WALL%BOUNDARY_TYPE = NULL_BOUNDARY
...
IF (N_SURFACE_DENSITY_SPECIES > 0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   M%AWM_AEROSOL = 0._EB
   DO IW=1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%AWM_AEROSOL(N_SURFACE_DENSITY_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%AWM_AEROSOL',IZERO) 
      M%WALL(IW)%AWM_AEROSOL = 0._EB   
   ENDDO
ENDIF
...
IF (ACCUMULATE_WATER .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   DO IW = 1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%A_LP_MPUA(N_EVAP_INDICES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%A_LP_MPUA',IZERO) 
      M%WALL(IW)%A_LP_MPUA = 0._EB  
   ENDDO
ENDIF
...
IF (.NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO IW = 1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%LP_MPUA(N_EVAP_INDICES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%LP_MPUA',IZERO) 
      M%WALL(IW)%LP_MPUA = 0._EB  
      ALLOCATE(M%WALL(IW)%LP_CPUA(N_EVAP_INDICES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%LP_CPUA',IZERO) 
      M%WALL(IW)%LP_CPUA = 0._EB  
   ENDDO
ENDIF
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   CALL INIT_WALL_CELL(NM,I,J,K,0,IW,0,SURF_INDEX,IERR)
ENDDO DEVICE_LOOP
...
INITIALIZATION_LOOP: DO N=1,N_INIT
   ...
   IF (PC%SURF_INDEX>0) THEN
      ...
      DO NN=1,IN%NUMBER_INITIAL_PARTICLES
         IW = IW + 1
         I = 1
         J = 1
         K = 1
...
! Initialize and allocate lagrangian particle/PARTICLE arrays

M%NLP = 0
M%NLPDIM = 1000
IF (PARTICLE_FILE .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ALLOCATE(M%PARTICLE(M%NLPDIM),STAT=IZERO)
...
HRR = 0._EB
ALLOCATE(Q_RADI(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','Q_RADI',IZERO) 
Q_RADI = 0._EB
ALLOCATE(Q_WALL(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','Q_WALL',IZERO) 
Q_WALL = 0._EB
ALLOCATE(Q_OPEN(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','Q_OPEN',IZERO) 
Q_OPEN = 0._EB
ALLOCATE(MLR(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','MLR',IZERO) 
MLR = 0._EB
ALLOCATE(FUEL_PARTICLE_MLR(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','FUEL_PARTICLE_MLR',IZERO) 
FUEL_PARTICLE_MLR = 0._EB
ALLOCATE(HRR_SUM(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','HRR_SUM',IZERO)  
HRR_SUM=0._EB
ALLOCATE(Q_RADI_SUM(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','Q_RADI_SUM',IZERO) 
Q_RADI_SUM=0._EB
ALLOCATE(Q_WALL_SUM(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','Q_WALL_SUM',IZERO) 
Q_WALL_SUM=0._EB
ALLOCATE(Q_OPEN_SUM(NMESHES),STAT=IZERO)
CALL ChkMemErr('INIT','Q_OPEN_SUM',IZERO) 
Q_OPEN_SUM=0._EB
...
INTEGER  :: NM,NOM,SURF_INDEX,IOR,ITER,IIO_MIN,IIO_MAX,JJO_MIN,JJO_MAX,KKO_MIN,KKO_MAX,I_VENT
INTEGER, INTENT(OUT) :: IERR
REAL(EB) :: PX,PY,PZ,X1,X2,Y1,Y2,Z1,Z2,T_ACTIVATE,XIN,YIN,ZIN,DIST
INTEGER  :: N,I_OBST,I,J,K,SURF_INDEXX,IIG,JJG,KKG,IW,IIO,JJO,KKO,IC,ICG,NOM_CHECK(0:1)
INTEGER :: NSLICE
...
IF (ABS(IOR)==1) THEN
   IF (IOR== 1) THEN
      ...
      WC%IIG    = I+1
      WC%RDN    = M%RDXN(I) 
      IF (.NOT.EVACUATION_ONLY(NM)) WC%AW     = M%R(I)*M%DY(J)*M%DZ(K)
      WC%UW     = -U0
   ENDIF
   IF (IOR==-1) THEN
      WC%XW     = M%X(I-1)
      WC%IIG    = I-1
      WC%RDN    = M%RDXN(I-1) 
      IF (.NOT.EVACUATION_ONLY(NM)) WC%AW     = M%R(I-1)*M%DY(J)*M%DZ(K)
      WC%UW     = U0
   ENDIF
   WC%JJG    = J
   WC%KKG    = K   
...
IF (ABS(IOR)==2) THEN
   ...
   WC%XW = M%XC(I)
   WC%ZW = M%ZC(K)
   IF (.NOT.EVACUATION_ONLY(NM)) WC%AW = M%DX(I)*M%DZ(K)
ENDIF
IF (ABS(IOR)==3) THEN
   IF (IOR== 3) THEN
      WC%ZW     = M%Z(K)
   ...
   WC%XW = M%XC(I)
   WC%YW = M%YC(J)
   IF (.NOT.EVACUATION_ONLY(NM)) WC%AW = M%DX(I)*M%RC(I)*M%DY(J)
ENDIF
...
DO NSLICE = 1, M%N_TERRAIN_SLCF
   IF (IOR==3) M%K_AGL_SLICE(I,J,NSLICE) = M%K_AGL_SLICE(I,J,NSLICE) + WC%KKG
ENDDO
...
IF (EVACUATION_ONLY(NM)) WC%UW = 0._EB
...
IF (IW<=M%N_EXTERNAL_WALL_CELLS .AND. I_OBST==0) WC%BOUNDARY_TYPE = SOLID_BOUNDARY
...
IF (M%SOLID(ICG)) WC%BOUNDARY_TYPE = NULL_BOUNDARY

IF (IOR==0) WC%BOUNDARY_TYPE = VIRTUAL_BOUNDARY
...
CHECK_MESHES: IF (IW<=M%N_EXTERNAL_WALL_CELLS .AND.  .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO ITER=0,1
      ...
      IF (ABS(IOR)/=1) XIN = WC%XW + (ITER*0.95_EB-0.475_EB)*(M%X(I)-M%X(I-1))
      IF (ABS(IOR)/=2) YIN = WC%YW + (ITER*0.95_EB-0.475_EB)*(M%Y(J)-M%Y(J-1))
      IF (ABS(IOR)/=3) ZIN = WC%ZW + (ITER*0.95_EB-0.475_EB)*(M%Z(K)-M%Z(K-1))
   ...
   FOUND_OTHER_MESH: IF (NOM>0) THEN 
      ...
      IF (I_OBST==0) THEN
         IF (.NOT.M%SOLID(ICG)) WC%BOUNDARY_TYPE = INTERPOLATED_BOUNDARY
         SURF_INDEXX = INTERPOLATED_SURF_INDEX
         WC%SURF_INDEX = SURF_INDEXX
      ENDIF
      IF (WC%BOUNDARY_TYPE == INTERPOLATED_BOUNDARY) M%SOLID(M%CELL_INDEX(I,J,K)) = .FALSE.
      I_VENT = 0
...
IF (N_TRACKED_SPECIES>0 .AND. WC%BOUNDARY_TYPE/=VIRTUAL_BOUNDARY .AND. &
...
IF (I_OBST>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   WC%AREA_ADJUST = OBX%INPUT_AREA(ABS(IOR))/OBX%FDS_AREA(ABS(IOR))
   IF (WC%AREA_ADJUST<=ZERO_P) WC%AREA_ADJUST = 1._EB
   OBX%MASS = SURFACE(SURF_INDEX)%SURFACE_DENSITY*WC%AW*WC%AREA_ADJUST
...
IF (I_OBST>0 .AND. ABS(SURFACE(SURF_INDEXX)%MASS_FLUX_TOTAL)>=ZERO_P .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   OBX=>M%OBSTRUCTION(I_OBST)
   WC%UW0 = SURFACE(SURF_INDEXX)%MASS_FLUX_TOTAL / RHOA * WC%AREA_ADJUST
ENDIF
...
PROCESS_VENT: IF (I_VENT>0) THEN
   ...
   IF (.NOT.EVACUATION_ONLY(NM)) THEN
   WC%AREA_ADJUST = VT%INPUT_AREA/VT%FDS_AREA
   IF (WC%AREA_ADJUST<=ZERO_P) WC%AREA_ADJUST = 1._EB
   ENDIF
...
IF (WC%BOUNDARY_TYPE/=NULL_BOUNDARY .OR. WC%NOM>0) CALL ALLOCATE_WALL_ARRAYS(NM,IW)
      ...
      DO JJ=VT%J1+1,MAX(VT%J1+1,VT%J2)
         DO II=VT%I1+1,MAX(VT%I1+1,VT%I2)
            ...
            IF (ACTIVATE_VENT) THEN
               IF (VT%X0>-1.E5_EB) THEN
                  WALL(IW)%TW = T + SQRT((WALL(IW)%XW-VT%X0)**2 +(WALL(IW)%YW-VT%Y0)**2 +&
                                         (WALL(IW)%ZW-VT%Z0)**2)/VT%FIRE_SPREAD_RATE
               ELSE
                  WALL(IW)%TW = T        
               ENDIF
            ELSE
               WALL(IW)%TW = 1000000._EB
            ENDIF
         ENDDO
      ENDDO
   ENDDO
...
IF (WALL(IW)%BOUNDARY_TYPE/=NULL_BOUNDARY) CALL ALLOCATE_WALL_ARRAYS(NM,IW)
...
IF (REMOVE .AND. SURFACE(SURF_INDEX)%PYROLYSIS_MODEL==PYROLYSIS_SPECIFIED) THEN
   ...
   IF (IW_OLD>0) THEN
      IF (SURF_INDEX==WALL(IW)%SURF_INDEX) WALL(IW)%TW =  WALL(IW_OLD)%TW
   ENDIF
ENDIF
