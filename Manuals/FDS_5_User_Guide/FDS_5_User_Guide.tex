...
This chapter considers cases involving sprinklers and sprays.
...
Note that in this example there is no need for a separate ignition source. 
The combustion model assumes fuel and oxygen burn on contact, and so the small amount of evaporation at ambient temperature is enough to start the reaction.
...
Example input files for FDS can be found in the FDS-SMV project Subversion repository.  There are a few ways to get a copy of these files, 
two different approaches will be covered here. The first is via the SVN Command Line Client and the second is to Download Individual Files from the Web Interface.  
Note: Some of these files are used for example input throughout this document, especially in Part \ref{part:samples}.

\begin{description}
\item[{\bf Command Line SVN Client}]
There are two methods for using the command line client, both methods have an optional step to keep the files up to date with the latest version in the repository over time.  
The first method using SVN Checkout (co) is useful if you want to able to easily update the files on your local machine.  
The downside of this method is that there will be '.svn' files within each directory that are used to check the local versions when the optional Update command is later used.  
The SVN Export command will make a 'clean' copy the files and folders from the repository to your local computer, where 'clean' in this context 
means that there will not be '.svn' files in the directories.  The disadvantage of this method is that you cannot use the standard 'svn update' command to 
get the latest versions from the repository after check out.  Instead you will have to either force an overwrite of the existing files or remove 
the Verification directory and run the export command again.  Both methods will require that you download and install the \href{http://subversion.tigris.org}{{Subversion}} 
command line client for your Operating System.
