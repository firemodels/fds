...
INTEGER :: NM_MAX
REAL(EB) :: TNOW
LOGICAL :: ITERATE_BAROCLINIC_TERM

PRESSURE_ITERATIONS = 0

DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   IF (EVACUATION_ONLY(NM) .OR. EVACUATION_SKIP(NM)) CYCLE
   CALL COMPUTE_PRESSURE_ERROR(NM)
ENDDO
...
PRESSURE_ITERATION_LOOP: DO
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      IF (EVACUATION_ONLY(NM) .OR. EVACUATION_SKIP(NM)) CYCLE
      IF (ITERATE_BAROCLINIC_TERM) CALL COMPUTE_PRESSURE_ERROR(NM)
   ...
   IF (N_MPI_PROCESSES>1) THEN
      ...
      CALL MPI_GATHERV(INTEGER_BUFFER_1(DISPLS(MYID)+1),COUNTS(MYID),MPI_INTEGER, &
                          VELOCITY_ERROR_MAX_I,COUNTS,DISPLS,MPI_INTEGER,0,MPI_COMM_WORLD,IERR)
      INTEGER_BUFFER_1 = VELOCITY_ERROR_MAX_J
      CALL MPI_GATHERV(INTEGER_BUFFER_1(DISPLS(MYID)+1),COUNTS(MYID),MPI_INTEGER, &
                          VELOCITY_ERROR_MAX_J,COUNTS,DISPLS,MPI_INTEGER,0,MPI_COMM_WORLD,IERR)
      INTEGER_BUFFER_1 = VELOCITY_ERROR_MAX_K
      CALL MPI_GATHERV(INTEGER_BUFFER_1(DISPLS(MYID)+1),COUNTS(MYID),MPI_INTEGER, &
                          VELOCITY_ERROR_MAX_K,COUNTS,DISPLS,MPI_INTEGER,0,MPI_COMM_WORLD,IERR)
      T_USED(11)=T_USED(11) + SECOND() - TNOW
   ENDIF

   IF (MYID==0 .AND. VELOCITY_ERROR_FILE .AND. .NOT.ALL(EVACUATION_ONLY)) THEN
      NM_MAX = MAXLOC(VELOCITY_ERROR_MAX,DIM=1)
      WRITE(LU_VELOCITY_ERROR,'(7(I7,A),E16.8,A,E16.8)') ICYC,', ',PRESSURE_ITERATIONS,', ',NM_MAX,', ', &
         VELOCITY_ERROR_MAX_I(NM_MAX),', ',VELOCITY_ERROR_MAX_J(NM_MAX),', ',VELOCITY_ERROR_MAX_K(NM_MAX),', ', &
         TOTAL_PRESSURE_ITERATIONS,', ',MAXVAL(VELOCITY_ERROR_MAX),', ',MAXVAL(PRESSURE_ERROR_MAX)
   ENDIF
