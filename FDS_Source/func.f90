...
READLOOP: DO
   ...
   TLOOP: DO II=1,255-LEN_TRIM(NAME)
      IF (TEXT(II:II)=='&') FOUND_NAMELIST = .TRUE.
      IF (FOUND_NAMELIST .AND. TEXT(II:II+LEN_TRIM(NAME)-1)==TRIM(NAME)) THEN
         FOUND = .TRUE.
         EXIT READLOOP
      ENDIF
   ENDDO TLOOP
...
REAL(EB) FUNCTION LES_FILTER_WIDTH(DX,DY,DZ)

USE GLOBAL_CONSTANTS, ONLY: TWO_D,MAX_FILTER_WIDTH
REAL(EB), INTENT(IN) :: DX,DY,DZ

IF (TWO_D) THEN
   IF (MAX_FILTER_WIDTH) THEN
      LES_FILTER_WIDTH = MAX(DX,DZ)
   ELSE
      LES_FILTER_WIDTH = SQRT(DX*DZ)
   ENDIF
ELSE
   IF (MAX_FILTER_WIDTH) THEN
      LES_FILTER_WIDTH = MAX(DX,DY,DZ)
   ELSE
      LES_FILTER_WIDTH = (DX*DY*DZ)**ONTH
   ENDIF
ENDIF

END FUNCTION LES_FILTER_WIDTH
