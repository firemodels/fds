...
SPRINKLER_INSERT_LOOP: DO KS=1,N_DEVC  
   ...
   IF (ABS(DV%ORIENTATION(2))<=TWO_EPSILON_EB) THEN
      TRIGT2 = ACOS(1._EB)
   ELSE
...
WALL_INSERT_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   PARTICLE_INSERT_LOOP2: DO I=1,SF%NPPC
      ...
      LP=>MESHES(NM)%LAGRANGIAN_PARTICLE(NLP)      
      SF=>SURFACE(LPC%SURF_INDEX)
      IF (.NOT. LPC%MASSLESS) MASS_SUM = MASS_SUM + LP%PWT*LPC%FTPR*LP%ONE_D%X(SF%N_CELLS_INI)**3
...
SUBROUTINE FACE_PARTICLE_INSERT
USE COMPLEX_GEOMETRY, ONLY: LINKED_LIST_INSERT

TYPE(FACET_TYPE), POINTER :: FC=>NULL()

FACE_INSERT_LOOP: DO N=1,N_FACE

   FC=>FACET(N)
   ILPC = FC%PART_INDEX
   IF (ILPC < 1) CYCLE FACE_INSERT_LOOP
   IF (FC%ALREADY_INSERTED) CYCLE FACE_INSERT_LOOP

   LPC => LAGRANGIAN_PARTICLE_CLASS(ILPC)
   SF=>SURFACE(LPC%SURF_INDEX)

   ALLOCATE(LP_INDEX_LOOKUP(FC%N_PARTICLES))
   LP_INDEX_LOOKUP = 0

   INSERT_PARTICLE_LOOP: DO I=1,FC%N_PARTICLES
      ...
      ! Add particle to FACET PARTICLE linked list

      CALL LINKED_LIST_INSERT(NLP,FC%PARTICLE_LIST)

   ENDDO INSERT_PARTICLE_LOOP

   DEALLOCATE(LP_INDEX_LOOKUP)

   FC%ALREADY_INSERTED = .TRUE.

ENDDO FACE_INSERT_LOOP

END SUBROUTINE FACE_PARTICLE_INSERT
...
INTEGER :: NN, ND, N_INSERT, I1,J1,K1,I2,J2,K2, MOD_N_INSERT
...
PARTICLE_LOOP: DO I=1,NLP  
   ...
   IF (.NOT. LPC%MASSLESS .AND. (RD<=0._EB .OR. (.NOT. LPC%STATIC .AND. LP%MASS<=TWO_EPSILON_EB))) CYCLE PARTICLE_LOOP
   RDS = RD*RD
   RDC = RD*RDS
   ...
   ELSE SOLID_GAS_MOVE
      ...
      IF (LPC%MASSLESS .OR. LP%PWT<=TWO_EPSILON_EB .OR. LPC%STATIC) CYCLE PARTICLE_LOOP   
...
IF (LPC%MASSLESS .OR. LP%PWT<=TWO_EPSILON_EB) THEN
...
INTEGER :: I,II,JJ,KK,IW,N_LPC,NS,N_SUBSTEPS,ITMP,ITMP2,SURF_INDEX,ITCOUNT,Y_INDEX,Z_INDEX,I_BOIL,I_MELT,IIX,JJY,KKZ
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
LOGICAL :: TEMPITER
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
...
SUM_PART_QUANTITIES: IF (N_LP_ARRAY_INDICES > 0) THEN
   ...
   PART_CLASS_SUM_LOOP: DO N_LPC=1,N_LAGRANGIAN_CLASSES
      ...
      IF (LPC%SURF_INDEX/=DROPLET_SURF_INDEX) CYCLE PART_CLASS_SUM_LOOP
   ...
   DO I=1,NLP
      LP=>LAGRANGIAN_PARTICLE(I)       
      IF (LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)%SURF_INDEX/=DROPLET_SURF_INDEX) CYCLE
      II = LP%ONE_D%IIG
      JJ = LP%ONE_D%JJG
      KK = LP%ONE_D%KKG
      RVC = RDX(II)*RRN(II)*RDY(JJ)*RDZ(KK)
...
CASE(1)
   ...
   SUM_MOMENTUM_LOOP_2: DO I=1,NLP
      ...
      IF (LPC%MASSLESS)    CYCLE SUM_MOMENTUM_LOOP_2
      CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XI,YJ,ZK,II,JJ,KK)
...
SUM_MOMENTUM_LOOP2: DO I=1,NLP
   ...
   IF (LPC%MASSLESS)    CYCLE SUM_MOMENTUM_LOOP2
   CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XI,YJ,ZK,II,JJ,KK)
   ...
   ! LOOP over a small neighbourhood of the particle [-1,+1]
