...
\item Clara Cruz, a student at the University of Puerto Rico and Summer Undergraduate Research Fellow at NIST, helped develop useful Matlab scripts to automate
...
\begin{equation}
\label{eqn_colebrook}
\frac{1}{\sqrt{f}} = -2.0 \log_{\scriptsize 10} \left( \frac{s/H}{3.7} + \frac{2.51}{\mbox{Re}_H\sqrt{f}} \right) \,\mbox{,}
\end{equation}
...
The mean streamwise velocity profile at the rib centerline is shown in the upper-right plot of Fig.~\ref{fig_ribbed_channel}.  Here we point out that the setup for this case is basically copied from Loh\'asz et al.~\cite{Lohasz:1}.  Their method of forcing, however, is ambiguous.  Using the FDS {\ct MEAN\_FORCING} feature, we set our bulk velocity to 1 m/s ({\ct U0 = 1.0} on {\ct MISC}) in order to match the mean profile for the finest grid resolution.  All other cases use the same forcing.
...
\be
m \, c_p \frac{\d \overline{T_d}}{\d t} = \dot{Q}_r
\ee
...
This section describes a series of test cases designed to check the reaction rate mechanism using the eddy dissipation concept model for infinitely fast reactions (EDC) and  finite-rate (Arrhenius) reactions. In the most basic sense, if a reaction occurs the rate of the reaction is either infinite or finite-rate. In the infinite-rate case, the rate becomes limited by the species concentrations of the reactants that have mixed. For finite-rate reactions, the rate is dependent on factors such as local species concentration and temperature:

\begin{equation}\label{eq:Arrheniusratemode}
\frac{\d \phi_\F}{\d t} = -A_{mod} \rho^{\sum (a_{\alpha}) -1} \; \prod \left(Y_{\alpha}^{a_{\alpha}} \right) T^n \; e^{-E/RT} \;\;\; A_{mod} = A \prod \left(W_{\alpha}^{-a_{\alpha}} \right)
\end{equation}
More detailed discussion on the combustion routines can be found in the FDS Technical Reference Guide~\cite{FDS_Tech_Guide}.
...
$\alpha$ is defined as [$\mathrm{C_3H_8}$ $\mathrm{O_2}$ $\mathrm{CO}$ $\mathrm{H_2O}$ $\mathrm{CO_2}$].
\end{center}
Equation (\ref{eq:2step_propane_rev}) includes the decomposition of $\mathrm{CO_2}$ into $\mathrm{CO}$ and $\mathrm{O_2}$ whereas the forward-only two-step mechanism, Equation (\ref{eq:2step_propane}), does not. This reverse reaction mechanism is represented by three separate reactions on the {\ct REAC} line in an FDS input file as indicated by rows $\mathrm{a}$, $\mathrm{b}$ and $\mathrm{c}$ in the preceding table. Species evolutions for the 2 step reverse reaction are shown in Figure \ref{fig:Arrhenius_175Order_2stepR}. In the forward CO step, (row $\mathrm{b}$ in the two prior tables), we see that water vapor contributes to the reaction rate based on its local concentration despite not being a participant in the reaction.
\vskip8pt
...
The mixture fraction output quantity is valid for simple one-step reactions of the form A + B $\rightarrow$ C. To test that mixture fraction is correctly implemented we consider a series of test reactors that have been initialized with equivalence ratios from $\phi=0$ to $\phi=1$ to simulate the range of equivalence ratios that exist across a flame sheet. A simple one-step methane reaction is used to govern the combustion:
\begin{equation}\label{eq:methane_ex2}
\underbrace{\mathrm{CH_4}}_{fuel} + \underbrace{\mathrm{2\,(O_2 + 3.76\,N_2)}}_{oxidizer} \rightarrow  \underbrace{\mathrm{CO_2 + 2\, H_2O + 7.52\,N_2}}_{products}
\end{equation}
The simulation results are compared to state relationships which govern temperature and species as a function of mixture fraction \cite{Turns:1996}. The species state relations are governed by two sets of equations based on whether the mixture fraction ($f$) falls inside the flame sheet (fuel rich) or outside the flame sheet (fuel lean).  For fuel rich combustion ($f_{stoic} < f \le 1$):
\begin{align}\label{eq:mix_relate_rich}
Y_{fuel} &= \frac{f-f_{stoich}}{1-f_{stoich}} \\
Y_{ox} &= 0 \\
Y_{prod} &= \frac{1-f}{1-f_{stoich}}
\end{align}
For fuel lean combustion ($0 \le f < f_{stoich}$):
\begin{align}\label{eq:mix_relate_lean}
Y_{fuel} &= 0 \\
Y_{ox} &= 1- \frac{f}{f_{stoich}} \\
Y_{prod} &= \frac{f}{f_{stoich}}
\end{align}
At stoichiometric conditions, the species mass fractions are the same whether approaching from fuel lean or fuel rich: $Y_{fuel} = 0$, $Y_{ox}=0$, and $Y_{prod}=1$. Fig.~\ref{fig:burke_schumann_spec} shows the comparison between the final FDS lumped species concentrations (fuel, oxidizer, and products) and state relation results as a function of mixture fraction.

\begin{figure}[h!]
\begin{center}
\includegraphics[height=2.2in]{SCRIPT_FIGURES/burke_schumann_spec}
\end{center}
\caption[Mixture fraction state relation for species ({\bf burke\_schumann})]{Lumped species mass fractions variation as function of mixture fraction.}
\label{fig:burke_schumann_spec}
\end{figure}

The state relationship for temperature is dependent upon the mixture fraction, the appropriate definition of $Y_{fuel}(f)$, the heat of combustion, and the specific heat. The specific heat, $c_p$, for each species is set to be a constant 1~kJ/kg\,K as defined by Turns \cite{Turns:1996}. The enthalpy of formation of each of the species except for the fuel is set to 0, such that the heat of combustion of the reaction equals the heat of formation of the fuel. To obtain true adiabatic conditions in FDS, these cases were conducted inside a constant-volume reactor, so Eq.~(\ref{eq:mix_relate_temp}) instead needs to be based on $c_v$. We use the  following relationship $c_v = c_p - R/MW$ to determine $c_v$.
\begin{equation}\label{eq:mix_relate_temp}
T = (f-Y_{fuel})\frac{\Delta h_{c}}{c_{v}} + f(T_{fuel,i}-T_{ox,i}) + T_{ox,i}
\end{equation}
The molecular weight of the species were not set to be uniform, so the average molecular weight and $c_v$ will change as a function of the mixture fraction. As a result, the traditional linear temperature profile is not expected.
\begin{figure}[h!]
\begin{center}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/burke_schumann_temp}
\end{center}
\caption[Mixture fraction state relation for temperature ({\bf burke\_schumann})]{Temperature variation as function of mixture fraction.}
...
The calculations described in this section check the smoke detector algorithm that computes the mass fraction of smoke in the sensing chamber of a detector. For this example, the initial soot concentration in a 1~m$^3$ box is set as 100~mg/m$^3$ via the {\ct INIT} line. A vent on the left side injects soot at a velocity of 0.25~m/s and a concentration of 100~mg/m$^3$, and the soot exhausts on the right side of the box via an open vent. By default, if a species is defined as an aerosol by setting {\ct AEROSOL=.TRUE.} on the {\ct SPEC} line, then all of the aerosol deposition mechanisms (gravitational, thermophoretic, and turbulent) will act upon that species. For this example, all deposition mechanisms are disabled by using the follows inputs on the {\ct MISC} line:
...
\ee
Figure~\ref{fig:soot_gravitational_settling} shows a comparison of the analytical and simulated results for the time evolution of the smoke detector chamber obscuration.
...
The calculations described in this section check the conservation of mass for the gravitational settling algorithm. The gravitational settling algorithm involves both the settling of soot particles in the gas phase and the accumulation of soot on the condensed phase. For this example, the initial soot concentration in a 1~m$^3$ box is set as 100~mg/m$^3$ via the {\ct INIT} line, and the soot deposits to the bottom of the box via gravitational settling only. By default, if a species is defined as an aerosol by setting {\ct AEROSOL=.TRUE.} on the {\ct SPEC} line, then all of the aerosol deposition mechanisms (gravitational, thermophoretic, and turbulent) will act upon that species. For this example, gravitational deposition is enabled and the other deposition mechanisms are disabled by using the follows inputs on the {\ct MISC} line:
...
\be
\frac{\d C}{\d t} = -\frac{A}{V} (u_g C)
\label{eq:gravitational_settling}
\ee
where $C$ is the soot concentration (mg/m$^3$), $A/V$ is the surface area to volume ratio (m$^{-1}$), and $u_g$ is the gravitational settling velocity (m/s). Equation~\ref{eq:gravitational_settling} has an analytical solution for the dynamic gas-phase soot concentration of the form
\be
C(t) = C_0 \exp \left[-\frac{A}{V}(u_g t)\right]
\ee
where $C_0$ is the initial gas-phase soot concentration (100~mg/m$^3$). In this example, the relevant surface area is the area of the lower surface where the soot deposits, and thus the surface area to volume ratio ($A/V$) is equal to 1~m$^{-1}$. For a soot particle with a diameter of 1~$\mu$m, the gravitational settling velocity is equal to 1.9747$\times 10^{-4}$~m/s. The governing equations for the gravitational settling velocity are discussed in more detail in the FDS Technical Reference Guide~\cite{FDS_Math_Guide}. Figure~\ref{fig:deposition_mass} shows a comparison of the analytical and simulated results for the time evolution of the gas-phase soot concentration.
...
\end{tabular*}
\caption[The {\bf  surf\_mass\_vent\_char} test cases]{Comparison of analytical and predicted mass change for charring surfaces that are introduced as {\ct VENT}s.}
...
\end{tabular*}
\caption[The {\bf surf\_mass\_vent\_nonchar} test cases]{Comparison of analytical and predicted mass change for non-charring surfaces that are introduced as {\ct VENT}s.}
...
a static matrix structure ({\ct ALLOW\_SHRINKING=.FALSE.} or {\ct ALLOW\_SWELLING=.FALSE.}) the layer thickness should not change. The parameters of the
...
Case     & \multicolumn{2}{|c|}{Reactant}  &  \multicolumn{3}{|c|}{Inert} & Product & \multicolumn{4}{|c|}{Layer} \\ \hline
         & $\rho_1$  & $Y_1$ & $\rho_2$  & $Y_2$ & Allow        & $\rho_3$  & $\rho_s$(0) & $\rho_s$(end) & $\Delta$(0) & $\Delta$(end) \\
...
\be
   \frac{\d T_s}{\d t} = \frac{\dq_{\hbox{\tiny front}}'' + \dq_{\hbox{\tiny back}}''}{c_s \, \rho_s \, \delta}
\ee
...
For material properties, we choose $\rho_s = \rho_l =1,000$~kg/m$^3$,
...
Figure~\ref{water_evaporation_7_plot} displays the energy balance for this case. The heat release rate, {\ct HRR}, of the fire is expected to be 320~kW. The rate at which the water droplets extract energy from the system, {\ct Q\_PART}, is expected to be
\be - \left( 4.184 \; \hbox{kJ/kg/K} \times 80 \; \hbox{K} + 2260 \; \hbox{kJ/kg} \right) \times 1/60 \; \hbox{kg/s} = -43.2 \; \hbox{kW} \ee
The sum of all the terms, {\ct Q\_TOTAL}, is expected to be zero. The three other quantities, {\ct Q\_COND}, {\ct Q\_RADI}, {\ct Q\_CONV}, all have plausible values, but there is no way to determine the exact values.
...
Two cases are set up to test the turbulent dispersion of tracer particles.  Tracer particles are injected into a channel with a steady laminar flow with mean velocity 1 m/s.  In the first case ({\bf random\_walk\_1}), the diffusivity is set to $D=0.0834$ m$^2$/s (this value results from setting the dynamic viscosity to $\mu = 0.1$ kg/m/s with an ambient density of $\rho=1.199$ kg/m$^3$ and a Schmidt number of unity).  In the second case ({\bf random\_walk\_2}), the diffusivity is set to $D=0.00834$ m$^2$/s.  The results are compared with Einstein's classic result for Brownian motion \cite{Gardiner:2004}:
\begin{equation}
...
\caption[Example case {\ct HVAC\_damper}]{Simple duct network with a damper, case {\ct HVAC\_damper}.}
...
\begin{figure}[t]
   ...
   \end{tabular*}
   \caption[Results for the {\ct HVAC\_filter} sample case]{Compartment pressure (upper left), compartment temperature (upper right), duct velocity (lower left), and particulate mass in the compartment and on the filter (lower right).}
   \label{fig_filter}
\end{figure}
...
\end{tabular*}
\caption[Results of the {\ct fan\_test} case]{Pressure and volume flow for the {\ct fan\_test}.}
\label{fan_test}
\end{figure}
