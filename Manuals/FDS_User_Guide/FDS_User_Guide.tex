...
Many of the changes in FDS~6 are improvements to the various sub-models that do not affect the basic structure or parameters of the input file. 

\begin{description}
\item[Species Transport]
The scalar transport solver has been overhauled to prevent the over or under-shooting of scalar values in the region of a steep gradient, for example, the base of the fire plume.
\item[Turbulence]
The default large eddy simulation framework now includes the Deardorff form of the modeled viscosity. Past versions of FDS used the constant coefficient Smagorinsky model. The change increases the dynamic range of the simulation, yielding improved plume dynamics on relatively coarse grids.
\item[Particles]
The functionality of Lagrangian particles has expanded to include the heat transfer and pyrolysis of solid walls. In other words, you can now assign a set of surface properties to planar, cylindrical, or spherical particles much like you would for a solid surface. Liquid droplets are treated in much the same way as past versions.
\item[Pyrolysis] 
The basic 1-D heat transfer and pyrolysis model for solid surfaces remains the same, but there has been a change in several of the input parameters to expand functiontionality and readability of the input file. 
\item[HVAC] The functionality of the HVAC (Heating, Ventilation, Air Conditioning) has expanded to include more flexibility in specifying air coils, dampers, fans, louvers, and other devices. 
\end{description}
...
\noindent Specifying a {\ct DEVC} with a {\ct 'MASS FRACTION'} of soot will output the mass fraction of soot in the gas phase. The quantity {\ct 'VOLUME FRACTION'} of soot will output the volume fraction of soot in the gas phase treating soot as if it were an ideal gas. The quantity {\ct 'AEROSOL VOLUME FRACTION'} will output the volume fraction of soot as if it was a solid particle in computational cell based on the following equation,

\begin{equation}\label{eq:soot_volume_fraction}
f_v = \rho Y_{s} / \rho_{s}
\end{equation}
\noindent where $\rho$ is the local density, $Y_{s}$ is the local mass fraction of soot, and $\rho_{s}$ is density of soot defined using the {\ct SPEC} input {\ct DENSITY\_SOLID} which defaults to 1800~(kg/m$^3$).
