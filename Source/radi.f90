...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   IF (NLP>0 .AND. SOLID_PARTICLES) THEN
      DO IP = 1,NLP
         ...
         KAPPA_PART_SINGLE = 0.25_EB*LP%PWT*P1%AREA*RDX(IID)*RRN(IID)*RDY(JJD)*RDZ(KKD)*P1%EMISSIVITY
         KAPPA_PART(IID,JJD,KKD) = KAPPA_PART(IID,JJD,KKD) + KAPPA_PART_SINGLE
         KFST4_PART(IID,JJD,KKD) = KFST4_PART(IID,JJD,KKD) + KAPPA_PART_SINGLE*FOUR_SIGMA*P1%TMP_F**4
      ENDDO
   ENDIF
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         ELSE
            IF (WIDE_BAND_MODEL) BBF = BLACKBODY_FRACTION(WL_LOW(IBND),WL_HIGH(IBND),P1%TMP_F)
            IF (WSGG_MODEL) THEN
               ...
               BBF = A_WSGG(P1%TMP_F,MOL_RAT,IBND) ! Temperature coefficient for the jth gray gas in the boundary
            ENDIF                                     ! (use information of the cell adjacent to the boundary)
            SF  => SURFACE(WC%SURF_INDEX)
            OUTRAD_W(IW) = BBF*RPI*P1%Q_RAD_OUT
         ENDIF
      ENDDO
      ...
      DO ICF = N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
         ...
         IF (WIDE_BAND_MODEL) BBF = BLACKBODY_FRACTION(WL_LOW(IBND),WL_HIGH(IBND),P1%TMP_F)
         SF => SURFACE(CFA%SURF_INDEX)
         OUTRAD_F(ICF) = BBF*RPI*P1%Q_RAD_OUT
      ENDDO
      ...
      UPDATE_LOOP: DO I_UIID = 1,N_UPDATES
         ...
         ANGLE_LOOP: DO N = NSTART,NEND,NSTEP  ! Sweep through control angles
            ...
            !$OMP PARALLEL DO PRIVATE(WC,BC,BR,P1,IOR,II,JJ,KK,LL,NOM,VT) SCHEDULE(GUIDED)
               ...
               IF (.NOT.TWO_D .OR. ABS(IOR)/=2) THEN
                  SELECT CASE (WC%BOUNDARY_TYPE)
                     ...
                     CASE DEFAULT ! solid wall
                        BR%BAND(IBND)%ILW(N) = OUTRAD_W(IW) + RPI*(1._EB-P1%EMISSIVITY)*INRAD_W(IW)
            ...
            CFACE_LOOP1: DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
               ...
               BR%BAND(IBND)%ILW(N) = OUTRAD_F(ICF) + RPI*(1._EB-P1%EMISSIVITY)*INRAD_F(ICF)
            ENDDO CFACE_LOOP1
      ...
      DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         P1%Q_RAD_IN  = P1%Q_RAD_IN + P1%EMISSIVITY*(INRAD_W(IW)+BBFA*EFLUX)
      ENDDO
      ...
      DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
         ...
         SF => SURFACE(CFA%SURF_INDEX)
         EFLUX = EVALUATE_RAMP(T,SF%RAMP(TIME_EFLUX)%INDEX,TAU=SF%RAMP(TIME_EFLUX)%TAU)*SF%EXTERNAL_FLUX
         P1%Q_RAD_IN  = P1%Q_RAD_IN + P1%EMISSIVITY*(INRAD_F(ICF)+BBFA*EFLUX)
      ENDDO
...
IF (UPDATE_INTENSITY) THEN
   ...
   DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      DO IBND=1,NUMBER_SPECTRAL_BANDS
         P1%Q_RAD_IN  = P1%Q_RAD_IN + SUM(BR%BAND(IBND)%ILW(1:NUMBER_RADIATION_ANGLES))
      ENDDO
   ENDDO
   ...
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN
         ...
         IF (LP%ORIENTATION_INDEX>0) THEN
            ...
            P1%Q_RAD_IN = 0._EB
            DO IBND=1,NUMBER_SPECTRAL_BANDS
               P1%Q_RAD_IN = P1%Q_RAD_IN + P1%EMISSIVITY * (WEIGH_CYL*SUM(BR%BAND(IBND)%ILW(1:NUMBER_RADIATION_ANGLES)) + EFLUX)
            ENDDO
         ELSE
            BC => BOUNDARY_COORD(LP%BC_INDEX)
            P1%Q_RAD_IN = P1%EMISSIVITY*(0.25_EB*UII(BC%IIG,BC%JJG,BC%KKG) + EFLUX)
         ENDIF
      ENDIF
   ENDDO PARTICLE_LOOP
ENDIF
