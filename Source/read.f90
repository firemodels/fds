...
CALL PROC_DUMP    ; CALL CHECK_STOP_STATUS ; IF (STOP_STATUS/=NO_STOP) RETURN
...
NAMELIST /DUMP/ CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEVC_COLUMN_LIMIT,&
                ...
                RAMP_PART,RAMP_PL3D,RAMP_PROF,RAMP_RADF,RAMP_RESTART,RAMP_SLCF,RAMP_SL3D,RAMP_SMOKE3D,RAMP_UVW,&
                RENDER_FILE,SIG_FIGS,SIG_FIGS_EXP, &
                SMOKE3D,SMOKE3D_QUANTITY,SMOKE3D_SPEC_ID,STATUS_FILES,SUPPRESS_DIAGNOSTICS,&
                T_DEVC_LINE_END,VELOCITY_ERROR_FILE,WRITE_XYZ
...
CALL SET_OUTPUT_CLOCK(DT_BNDF   ,RAMP_BNDF_INDEX,BNDF_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_CPU    ,RAMP_CPU_INDEX , CPU_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_CTRL   ,RAMP_CTRL_INDEX,CTRL_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_DEVC   ,RAMP_DEVC_INDEX,DEVC_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_FLUSH  ,RAMP_FLSH_INDEX,FLSH_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_GEOM   ,RAMP_GEOM_INDEX,GEOM_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_HRR    ,RAMP_HRR_INDEX , HRR_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_ISOF   ,RAMP_ISOF_INDEX,ISOF_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_MASS   ,RAMP_MASS_INDEX,MASS_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_PART   ,RAMP_PART_INDEX,PART_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_PL3D   ,RAMP_PL3D_INDEX,PL3D_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_PROF   ,RAMP_PROF_INDEX,PROF_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_RADF   ,RAMP_RADF_INDEX,RADF_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_RESTART,RAMP_RSRT_INDEX,RSRT_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_SLCF   ,RAMP_SLCF_INDEX,SLCF_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_SL3D   ,RAMP_SL3D_INDEX,SL3D_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_SMOKE3D,RAMP_SM3D_INDEX,SM3D_CLOCK)
CALL SET_OUTPUT_CLOCK(DT_UVW    ,RAMP_UVW_INDEX , UVW_CLOCK)
...
SUBROUTINE SET_OUTPUT_CLOCK(DT_OUT,RAMP_INDEX,CLOCK)
...
IF (RAMP_INDEX>0) THEN
   RP => RAMPS(RAMP_INDEX)
   ALLOCATE(CLOCK(0:RP%NUMBER_DATA_POINTS))
   CLOCK(0:RP%NUMBER_DATA_POINTS-1) = RP%INDEPENDENT_DATA(1:RP%NUMBER_DATA_POINTS)
   CLOCK(RP%NUMBER_DATA_POINTS)     = HUGE(EB)
ELSE
