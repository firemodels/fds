...
!!IF (NOISE .OR. PERIODIC_TEST>0) THEN
   ...
   DO NM=1,NMESHES
      ...
      IF (.NOT.ENTHALPY_TRANSPORT) CALL DIVERGENCE_PART_1(T_BEGIN,NM)
      IF (     ENTHALPY_TRANSPORT) CALL DIVERGENCE_PART_1_ET(T_BEGIN,NM)
   ENDDO
!!ENDIF
...
MAIN_LOOP: DO  
   ...
   COMPUTE_FINITE_DIFFERENCES_1: DO NM=1,NMESHES
      ...
      IF (.NOT.ENTHALPY_TRANSPORT) CALL MASS_FINITE_DIFFERENCES(NM)
      IF (     ENTHALPY_TRANSPORT) CALL MASS_FINITE_DIFFERENCES_ET(NM)
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      COMPUTE_DENSITY_LOOP: DO NM=1,NMESHES
         EVACUATION_ZONE: IF (PROCESS(NM)==MYID .AND. N_ZONE>0 .AND. EVACUATION_GRID(NM)) THEN
            USUM(:,NM) = 0._EB; DSUM(:,NM) = 0._EB; PSUM(:,NM) = 0._EB
         END IF EVACUATION_ZONE
...
DO IPZ=1,N_ZONE
   ...
   DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID) CYCLE
      IF(.NOT. EVAC_FDS6 .AND. EVACUATION_ONLY(NM)) CYCLE
      IF(      EVAC_FDS6 .AND. EVACUATION_ONLY(NM) .AND. .NOT. EVACUATION_GRID(NM)) CYCLE
