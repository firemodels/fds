...
! dummy comment to force svn change - Sun Apr 26 21:56:38 EDT 2015
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (TRANSPORT_UNMIXED_FRACTION) THEN
      ALLOCATE(M%OMESH(NOM)%UNMIXED_FRACTION(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX))
      M%OMESH(NOM)%UNMIXED_FRACTION = INITIAL_UNMIXED_FRACTION
   ENDIF
   ...
   OTHER_MESH_LOOP: DO NOM=1,NMESHES
      ...
      INITIALIZATION_IF: IF (CODE==0) THEN
         ...
         IF (M3%NIC_R>0) THEN
            ...
            ALLOCATE(M3%REAL_RECV_PKG3(M3%NIC_R*2*(4+N_TRACKED_SPECIES+N_PASSIVE_SCALARS)))
   ...
   RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
      ...
      INITIALIZE_SEND_IF: IF (CODE==0) THEN
         ...
         IF (M3%NIC_S>0 .AND. RNODE/=SNODE) THEN
            ...
            ALLOCATE(M3%REAL_SEND_PKG3(M3%NIC_S*2*(4+N_TRACKED_SPECIES+N_PASSIVE_SCALARS)))
      ...
      IF (CODE==4 .AND. M3%NIC_S>0) THEN
         IF (RNODE/=SNODE) THEN
            NQT2 = 2*(4+N_TRACKED_SPECIES+N_PASSIVE_SCALARS)
            PACK_REAL_SEND_PKG3: DO LL=1,M3%NIC_S
               ...
               ENDDO
               DO NN=1,N_PASSIVE_SCALARS
                  M3%REAL_SEND_PKG3(NQT2*(LL-1)+8+2*N_TRACKED_SPECIES+2*NN-1) = M%UNMIXED_FRACTION(II1,JJ1,KK1)
                  M3%REAL_SEND_PKG3(NQT2*(LL-1)+8+2*N_TRACKED_SPECIES+2*NN  ) = M%UNMIXED_FRACTION(II2,JJ2,KK2)
               ENDDO
         ...
         ELSE
            ...
            IF (TRANSPORT_UNMIXED_FRACTION) THEN
               M2%UNMIXED_FRACTION(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX) = M%UNMIXED_FRACTION(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX)
            ENDIF
         ENDIF
      ENDIF
   ...
   RECV_MESH_LOOP: DO NM=1,NMESHES
      ...
      IF (CODE==4 .AND. M2%NIC_R>0 .AND. RNODE/=SNODE) THEN
         ...
         UNPACK_REAL_RECV_PKG3: DO LL=1,M2%NIC_R
            ...
            ENDDO
            DO NN=1,N_PASSIVE_SCALARS
               M2%UNMIXED_FRACTION(II1,JJ1,KK1) = M2%REAL_RECV_PKG3(NQT2*(LL-1)+8+2*N_TRACKED_SPECIES+2*NN-1)
               M2%UNMIXED_FRACTION(II2,JJ2,KK2) = M2%REAL_RECV_PKG3(NQT2*(LL-1)+8+2*N_TRACKED_SPECIES+2*NN)
            ENDDO
