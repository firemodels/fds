...
should always be positive and in units of Pa.  If in
the simulation the computed pressure exceeds the specified {\ct MAX\_PRESSURE}, then there will be a backflow in the duct.
...
The burnout time of a reacting solid fuel is calculated automatically based on the layer {\ct THICKNESS}, component {\ct DENSITY}, and the calculated burning rate.




\subsubsection{Testing Your Pyrolysis Model}
...
\begin{warning}
\noindent Note that FDS does not automatically calculate the burnout time for solid fuels with specified heat release rates.
\end{warning}
