...
#grep VDIR $maketable | awk 'BEGIN { FS = "/" } ; { system("./makegitentry.sh  " $2) }'  | sort -n -t '&' -k 4 | awk -F "&" '{ print $1 "&" $2 "&" $3 "\\\\ \\hline"}' >> $OUTPUT_TEX_FILE
CASEFILE=/tmp/temp.out.$$
grep VDIR $maketable | awk 'BEGIN { FS = "/" } ; { print $2 }' > $CASEFILE
while read p; do
  MAKEGITENTRY   $p | sort -n -t '&' -k 4 | awk -F "&" '{ print $1 "&" $2 "&" $3 "\\\\ \\hline"}' >> $OUTPUT_TEX_FILE
done <$CASEFILE
rm $CASEFILE
