...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                ...
                C_FORCED_CYLINDER,C_FORCED_SPHERE,C_HORIZONTAL,C_VERTICAL,DNS,&
...
REAL(EB), DIMENSION(3,1000) :: ORIENTATION
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (LPC%N_ORIENTATION>0) THEN   
      ...
      DO NN=1,LPC%N_ORIENTATION
         ...
         LPC%ORIENTATION(NN,1:3)   = ORIENTATION(1:3,NN)/ NORM2(OR_TEMP)
      ENDDO
   ENDIF
      ...
      DO NN=1,SF%N_CELLS_INI
         SELECT CASE (SF%GEOMETRY)
            ...
            CASE (SURF_CYLINDRICAL)
               ...
               MASS   = MASS   + SF%LENGTH*PI*(R_O**2-R_I**2)*SUM(SF%RHO_0(NN,1:SF%N_MATL))
               VOLUME = VOLUME + SF%LENGTH*PI*(R_O**2-R_I**2)
            CASE (SURF_SPHERICAL)
               R_I = SF%INNER_RADIUS + SF%THICKNESS - SF%X_S(NN)
               R_O = SF%INNER_RADIUS + SF%THICKNESS - SF%X_S(NN-1)
               MASS   = MASS   + FOTHPI*(R_O**3-R_I**3)*SUM(SF%RHO_0(NN,1:SF%N_MATL))
               VOLUME = VOLUME + FOTHPI*(R_O**3-R_I**3)
         END SELECT
      ENDDO
