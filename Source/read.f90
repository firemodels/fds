...
CHARACTER(LABEL_LENGTH) :: ID,MB,ODE_SOLVER,CFT_AGENT
...
INTEGER :: FUEL_SMIX_INDEX,CFT_AGENT_INDEX ! Simple chemistry fuel index
...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                WIND_ONLY,TWO_REAC_EXTINCT_TEST
...
ELSEIF (SIMULATION_MODE=='SVLES') THEN
   ...
   BAROCLINIC = .FALSE.
   CFL_VELOCITY_NORM = 3
   CONSTANT_SPECIFIC_HEAT_RATIO = .TRUE. ! Forces EXTINCTION_MODEL_1
...
NAMELIST /COMB/ BLOCKAGE_FACTOR,CFT_AGENT,CHECK_REALIZABILITY,EXTINCTION_MODEL,FIXED_MIX_TIME,&
                FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,HRRPUV_MAX_SMV,&
                ...
                RICHARDSON_ERROR_TOLERANCE,SUPPRESSION,TAU_CHEM,TAU_FLAME,TEMPERATURE_DEPENDENT_REACTION,TEMP_MAX_SMV
...
USE PROPERTY_DATA, ONLY : ELEMENT,GET_FORMULA_WEIGHT,MAKE_PERIODIC_TABLE,SIMPLE_SPECIES_MW,GAS_PROPS,&
                          LOOKUP_CHI_R,LOOKUP_CRITICAL_FLAME_TEMPERATURE
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (NR==2 .AND. N_SIMPLE_CHEMISTRY_REACTIONS==2) THEN
      ...
      !CRITICAL_FLAME_TEMPERATURE = 859.06_EB
...
IF (N_SIMPLE_CHEMISTRY_REACTIONS==2 .AND. TWO_REAC_EXTINCT_TEST) THEN
   CFT_AGENT_INDEX=-1
   DO NS = 1, N_SPECIES
      IF (TRIM(SPECIES(NS)%ID) == TRIM(CFT_AGENT)) THEN
         CFT_AGENT_INDEX = NS
         EXIT
      ENDIF
   ENDDO
   IF (CFT_AGENT_INDEX == -1) THEN
      WRITE(MESSAGE,'(A)')  'ERROR on COMB: CFT_AGENT is not a defined species'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   CALL CALC_CFT
ENDIF
...
SUBROUTINE CALC_CFT
!Computes CFT for two-step simple chemistry reactions
USE PROPERTY_DATA, ONLY: JANAF_TABLE
LOGICAL :: FUEL
CHARACTER(LABEL_LENGTH) :: GAS_NAME
INTEGER :: N,ITMP
REAL(EB), PARAMETER :: X_O2_AIR = 0.2095_EB
REAL(EB) :: CP,H,G_F,NU_CO2,NU_H2O,NU_O2,NU_AIR,NU_N2,NU_SOOT,NU_H2,NU_CO,NU_AGENT,NU_H2O_2,NU_AIR_2,NU_O2_2
REAL(EB) :: CP_N2,CP_O2,CP_CO2,CP_CO,CP_SOOT,CP_AGENT,CP_H2,CP_H2O
REAL(EB) :: H_FUEL, H_AIR, H_AGENT_R, H_AGENT_P, H_PRODUCTS, H_PRODUCTS_0, H_REACTANTS
REAL(EB) :: DTMP
REAL(EB), ALLOCATABLE, DIMENSION(:) :: H_N2,H_O2,H_CO2,H_CO,H_SOOT,H_AGENT,H_H2,H_H2O


ALLOCATE (H_N2(0:5000))
ALLOCATE (H_O2(0:5000))
ALLOCATE (H_CO(0:5000))
ALLOCATE (H_H2(0:5000))
ALLOCATE (H_CO2(0:5000))
ALLOCATE (H_H2O(0:5000))
ALLOCATE (H_SOOT(0:5000))
ALLOCATE (H_AGENT(0:5000))

! Build Enthalpy tables
DO N = 1, 5000
   IF (N <= 1) THEN
      GAS_NAME = 'NITROGEN'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_N2(0) = H
      H_N2(1) = H+CP
      CP_N2 = CP
      GAS_NAME='OXYGEN'
      ...
      H_H2(1) = H+CP
      CP_H2 = CP
      CALL JANAF_TABLE (N,CP,H,SPECIES(CFT_AGENT_INDEX)%ID,1._EB,FUEL,G_F)
      H_AGENT(0) = H
      H_AGENT(1) = H+CP
      CP_AGENT = CP
   ELSE
      GAS_NAME = 'NITROGEN'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_N2(N) = H_N2(N-1) + 0.5_EB*(CP+CP_N2)
      CP_N2 = CP
      GAS_NAME='OXYGEN'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_O2(N) = H_O2(N-1) + 0.5_EB*(CP+CP_O2)
      CP_O2 = CP
      GAS_NAME = 'CARBON DIOXIDE'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_CO2(N) = H_CO2(N-1) + 0.5_EB*(CP+CP_CO2)
      CP_CO2 = CP
      GAS_NAME = 'CARBON MONOXIDE'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_CO(N) = H_CO(N-1) + 0.5_EB*(CP+CP_CO)
      CP_CO = CP
      GAS_NAME = 'WATER VAPOR'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_H2O(N) = H_H2O(N-1) + 0.5_EB*(CP+CP_H2O)
      CP_H2O = CP
      GAS_NAME = 'SOOT'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_SOOT(N) = H_SOOT(N-1) + 0.5_EB*(CP+CP_N2)
      CP_SOOT = CP
      GAS_NAME = 'HYDROGEN'
      CALL JANAF_TABLE (N,CP,H,GAS_NAME,1._EB,FUEL,G_F)
      H_H2(N) = H_H2(N-1) + 0.5_EB*(CP+CP_H2)
      CP_H2 = CP
      CALL JANAF_TABLE (N,CP,H,SPECIES(CFT_AGENT_INDEX)%ID,1._EB,FUEL,G_F)
      H_AGENT(N) = H_AGENT(N-1) + 0.5_EB*(CP+CP_AGENT)
      CP_AGENT = CP
   ENDIF
ENDDO

!First find MEC given full reaction CFT and extinguishing agent
RN => REACTION(1)
NU_CO2 = RN%C
NU_H2O = RN%H * 0.5_EB
NU_O2  = NU_CO2 + 0.5*(NU_H2O - RN%O)
NU_AIR = NU_O2 / X_O2_AIR
NU_N2  = 0.5_EB*RN%N

!Reactant Enthalpy
H_FUEL = SPECIES_MIXTURE(FUEL_SMIX_INDEX)%MW * &
         (CPBAR_Z(298,FUEL_SMIX_INDEX) + 0.15_EB*(CPBAR_Z(299,FUEL_SMIX_INDEX) - CPBAR_Z(298,FUEL_SMIX_INDEX)))*298.15_EB
H_AIR  = NU_AIR * ( &
         MW_O2 * X_O2_AIR * (H_O2(298) + 0.15_EB*(H_O2(299) - H_O2(298))) + &
         MW_N2 * (1._EB-X_O2_AIR)/X_O2_AIR * (H_N2(298) + 0.15_EB*(H_N2(299) - H_N2(298))) )
H_AGENT_R = H_AGENT(298) + 0.15_EB*(H_AGENT(299) - H_AGENT(298))

!Product Enthalpy
ITMP = INT(RN%CRIT_FLAME_TMP)
DTMP = RN%CRIT_FLAME_TMP - REAL(ITMP,EB)
H_AGENT_P = H_AGENT(ITMP) + DTMP*(H_AGENT(ITMP+1) - H_AGENT(ITMP))
H_PRODUCTS = (NU_N2 + NU_AIR * (1._EB-X_O2_AIR)) * MW_N2 * (H_N2(ITMP) + DTMP*(H_N2(ITMP+1) - H_N2(ITMP))) + &
             NU_CO2 *  MW_CO2 *     (H_CO2(ITMP) +  DTMP*(H_CO2(ITMP+1) -  H_CO2(ITMP))) + &
             NU_H2O *  MW_H2O *     (H_H2O(ITMP) +  DTMP*(H_H2O(ITMP+1) -  H_H2O(ITMP)))

!Moles of diluent
NU_AGENT = (H_FUEL + H_AIR - H_PRODUCTS) / (H_AGENT_P - H_AGENT_R) / SPECIES(CFT_AGENT_INDEX)%MW

!Normalized to 1 mole of air
NU_AGENT = NU_AGENT / NU_AIR

!Second find CFT for first reaction using NU_AGENT as diluent
NU_CO    = RN%C * FUEL_C_TO_CO_FRACTION
NU_H2    = 0.5_EB * RN%H * FUEL_H_TO_H2_FRACTION
NU_SOOT  = RN%C * (1._EB -FUEL_C_TO_CO_FRACTION)/(1._EB-RN%SOOT_H_FRACTION)
NU_H2O   = (RN%H - NU_SOOT * RN%SOOT_H_FRACTION) * 0.5_EB - NU_H2
NU_O2    = 0.5_EB * (NU_CO + NU_H2O - RN%O)
NU_AIR = NU_O2 / X_O2_AIR
...
H_AIR  = NU_AIR * ( &
         MW_O2 * X_O2_AIR * (H_O2(298) + 0.15_EB*(H_O2(299) - H_O2(298))) + &
         MW_N2 * (1._EB-X_O2_AIR)/X_O2_AIR * (H_N2(298) + 0.15_EB*(H_N2(299) - H_N2(298))) )
H_AGENT_R = SPECIES(CFT_AGENT_INDEX)%MW * NU_AGENT * NU_AIR * (H_AGENT(298) + 0.15_EB*(H_AGENT(299) - H_AGENT(298)))
H_REACTANTS = H_AIR + H_AGENT_R + H_FUEL

!Find temperature where products = reactants
DO N = 0,5000
   H_PRODUCTS = (NU_N2 + NU_AIR * (1._EB-X_O2_AIR)) * MW_N2 * H_N2(N) + NU_CO * MW_CO * H_CO(N) + &
                NU_SOOT * RN%MW_SOOT * H_SOOT(N) + NU_H2 * MW_H2 * H_H2(N) + NU_H2O * MW_H2O * H_H2O(N) + &
                SPECIES(CFT_AGENT_INDEX)%MW * NU_AIR * NU_AGENT * H_AGENT(N)
   IF (N > 1 .AND. H_PRODUCTS > H_REACTANTS) THEN
      RN%CRIT_FLAME_TMP = REAL(N-1,EB) + (H_REACTANTS - H_PRODUCTS_0) / (H_PRODUCTS - H_PRODUCTS_0)
      EXIT
   ELSE
      H_PRODUCTS_0 = H_PRODUCTS
   ENDIF
END DO

!Third find CFT for second reaction using NU_AGENT and products from first reaction as diluent
NU_CO2 = RN%C
NU_H2O_2 = RN%H * 0.5_EB
NU_O2_2 = NU_CO2 + 0.5_EB * (NU_H2O_2 - RN%O) - NU_O2
NU_AIR_2 = NU_O2_2 / X_O2_AIR

H_FUEL = NU_CO * MW_CO * (H_CO(298) + 0.15_EB*(H_CO(299) - H_CO(298))) + &
         NU_H2O  * MW_H2O * (H_H2O(298) + 0.15_EB*(H_H2O(299) - H_H2O(298))) + &
         NU_H2  * MW_H2 * (H_H2(298) + 0.15_EB*(H_H2(299) - H_H2(298))) + &
         NU_SOOT * RN%MW_SOOT * (H_SOOT(298) + 0.15_EB*(H_SOOT(299) - H_SOOT(298))) + &
         (NU_N2 + NU_AIR * (1 - X_O2_AIR)) * MW_N2 * (H_N2(298) + 0.15_EB*(H_N2(299) - H_N2(298))) + &
         NU_AIR * NU_AGENT * SPECIES(CFT_AGENT_INDEX)%MW * (H_AGENT(298) + 0.15_EB*(H_AGENT(299) - H_AGENT(298)))
H_AIR  = NU_AIR_2 * ( &
         MW_O2 * X_O2_AIR * (H_O2(298) + 0.15_EB*(H_O2(299) - H_O2(298))) + &
         MW_N2 * (1._EB-X_O2_AIR)/X_O2_AIR * (H_N2(298) + 0.15_EB*(H_N2(299) - H_N2(298))) )
H_AGENT_R = SPECIES(CFT_AGENT_INDEX)%MW * NU_AGENT * NU_AIR_2 * (H_AGENT(298) + 0.15_EB*(H_AGENT(299) - H_AGENT(298)))
H_REACTANTS = H_AIR + H_AGENT_R + H_FUEL
DO N = 0,5000
   H_PRODUCTS = (NU_N2 + NU_AIR * (1 - X_O2_AIR) + NU_AIR_2 * (1 - X_O2_AIR)) * MW_N2 * H_N2(N) + &
                NU_H2O_2 * MW_H2O * H_H2O(N) + NU_CO2 * MW_CO2 * H_CO2(N) + &
                SPECIES(CFT_AGENT_INDEX)%MW * (NU_AIR_2+NU_AIR) * NU_AGENT * H_AGENT(N)
   IF (N > 1 .AND. H_PRODUCTS > H_REACTANTS) THEN
      REACTION(2)%CRIT_FLAME_TMP = REAL(N-1,EB) + (H_REACTANTS - H_PRODUCTS_0) / (H_PRODUCTS - H_PRODUCTS_0)
      EXIT
   ELSE
      H_PRODUCTS_0 = H_PRODUCTS
   ENDIF
...
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,SPECIFIC_HEAT,DENSITY,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE, &
            PEAK_REACTION_RATE,POROSITY,REFRACTIVE_INDEX
...
REAL(EB) :: NU_SPEC(MAX_SPECIES,MAX_REACTIONS),DIFFUSIVITY_SPEC(MAX_SPECIES)
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            AUTO_IGNITION_TEMPERATURE,AREA_ADJUST
