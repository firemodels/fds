...
This \paper suggests a methodology for calculating the mean and standard deviation of this distribution using the following information:
\begin{enumerate}
\item The predicted value,
\item Two parameters resulting from a validation study, the model bias and the relative standard deviation,
\item A few simple empirical relationships between the predicted quantity and the most important input parameters.
\end{enumerate}  
...
the uncertainty of the mathematical model and the uncertainty of the input parameters. Too often the two are lumped together, in which case there is no way to know
...
Before developing the procedure, a few definitions are needed. 
The uncertainty of a model prediction is the result of the uncertainty of its input parameters, known as the {\em parameter uncertainty}, and the
uncertainty of the mathematical model, known as the {\em model uncertainty}. 
...
the heat release rate of something like an electrical cabinet fire can be expressed as a probability distribution, 
and this distribution can be propagated through the model using any number of methods~\cite{SFPE:Notarianni}.
...
Next, consider the model uncertainty. It has been suggested by some that
