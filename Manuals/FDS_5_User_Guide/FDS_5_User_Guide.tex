...
The quality of a particular simulation is most directly tied to grid resolution.  Three output quantities are suggested for measuring errors in the velocity and scalar fields:
\begin{enumerate}
...
&SLCF PBY=0, QUANTITY='SCALAR RESOLUTION', SPEC_ID='HELIUM', CELL_CENTERED=.TRUE. /
&SLCF PBY=0, QUANTITY='WAVELET ERROR', SPEC_ID='FUEL', CELL_CENTERED=.TRUE. /
\end{verbatim} \normalsize
Note that {\tt SPEC\_ID} is required for MSR and WEM.  Also, {\tt CELL\_CENTERED} is optional for any of the three metrics.
...
\end{eqnarray}
where $a$ is the \emph{average} coefficient and $c$ is the \emph{wavelet} coefficient.  The Haar \emph{mother wavelet} (Figure \ref{fig_haar_mother}) is identified as
\begin{eqnarray}
...
{\ct SCALAR RESOLUTION}$^{****}$                &  See Section~\ref{info:meshquality}           &                & D,S          \\ \hline
...
{\ct WAVELET ERROR}$^{****}$                    & See Section~\ref{info:meshquality}            &                & D,S          \\ \hline
\end{longtable}
