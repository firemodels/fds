...
\Huge
\flushright{Fire Dynamics Simulator (Version 5) \\ Technical Reference Guide \\ (Draft: April 30, 2007)}
...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~1018-5, 80 pages (March 2007) \\
CODEN: NSPUE2 }
...
version number. For example, version 5.1.4 would be an updated release of
version 5.1, which in turn is an update within the major release referred to as FDS 5.

Starting with FDS 5, a formal archiving system has been implemented to track changes to the FDS source
code. The open-source program development tools are provided by an Internet-based organization known as SourceForge (sourceforge.net).
More on SourceForge, version control, and related issues can be found in the Verification and Validation Guide~\cite{FDS_VV_Guide_5}.
...
\be h_\alpha = \int_{T_{\hbox{ref}}}^T c_{p,\alpha}(T') \; dT' \ee
...
\be \nabla \cdot \bu = {\cal D} + {\cal P} \; \dod{\bp_m}{t}  \ee
where
\begin{eqnarray}
...
\frac{\R}{\bW c_p \bp_m} \; \sum_\alpha h_\alpha \nabla \cdot \rho D_\alpha \nabla Y_\alpha +
 \frac{1}{\rho} \sum_alpha \left( \frac{\bW}{W_\alpha} - \frac{h_\alpha}{c_p T} \right) \;
\dm_\alpha''' \\
\cal{P}          &=& \frac{1}{\bp_m} \; \left( \frac{\R}{\bW c_p} - 1 \right)   \label{phi}
\end{eqnarray}
...
relative contribution to the creation of vorticity.
When the momentum equation is integrated around a closed loop that moves with
the fluid, in the absence of any external forces, $\bof$, one can readily
identify the sources of vorticity:
\be \frac{d\Gamma}{dt} =
...
Due to the low Mach number approximation being used in the model, the
mass and energy equations are combined through the divergence. The
divergence of the flow field contains
much of the fire-specific source terms described above.
...
The divergence defined in Eq.~(\ref{divdis})
...
Note that this constraint applied to the momentum, mass and energy equations via the
...
A one dimensional heat transfer calculation is
performed at each solid boundary cell for which the user has prescribed thermal
properties. The solid can consist of multiple layers of materials.
Each layer is partitioned into non-uniform cells, clustered near the front and back faces.
The smallest cells are chosen based on the criteria
\be \dx < \sqrt{\frac{k_s}{\rho_s c_s}} \ee
Interior cells double in size. The cell boundaries are located at points $x_i$.
The temperature at the center of the $i$th cell is denoted $T_{s,i}$.
