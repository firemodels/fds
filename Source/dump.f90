...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       ...
       WRITE_SMOKEVIEW_FILE,DUMP_MESH_OUTPUTS,UPDATE_GLOBAL_OUTPUTS,DUMP_DEVICES,DUMP_HRR,DUMP_MASS,DUMP_MOMENTUM,DUMP_CONTROLS,&
...
! Momentum Balance File

IF (MOMENTUM_FILE) THEN
...
! Special output for detailed force balance

IF (MOMENTUM_FILE) THEN
   IF (APPEND) THEN
      OPEN(LU_MOM,FILE=FN_MOM,FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
   ELSE
      OPEN(LU_MOM,FILE=FN_MOM,FORM='FORMATTED',STATUS='REPLACE')
      WRITE(TCFORM,'(A,I4.4,A)') "(",3*N_U_DOT,"(A,','),A)"
      WRITE(LU_MOM,TCFORM) 's','N','N','N','N','N','N','N','N','N'
      WRITE(LU_MOM,TCFORM) 'Time','FVX','FVX','FVX','FVY','FVY','FVY','FVZ','FVZ','FVZ'
   ENDIF
ENDIF
...
WRITE(LU_CORE(NM)) T,ICYC,PART_CLOCK(NM),RESTART_CLOCK,SLCF_CLOCK(NM),RADF_CLOCK(NM), &
             ...
             BC_CLOCK_HT3D,WALL_COUNTER_HT3D,MOM_CLOCK
...
SUBROUTINE UPDATE_MOMENTUM(DT,NM)

! Integrate all the terms of the momentum equation over the entire domain.

! U_DOT(1,1,NM) = int dU/dt dV
! U_DOT(1,2,NM) = int FVX dV
! U_DOT(1,3,NM) = int dH/dx dV

! U_DOT(2,1,NM) = int dV/dt dV
! U_DOT(2,2,NM) = int FVY dV
! U_DOT(2,3,NM) = int dH/dy dV

! U_DOT(3,1,NM) = int dW/dt dV
! U_DOT(3,2,NM) = int FVZ dV
! U_DOT(3,3,NM) = int dH/dz dV
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=0,IBAR
         ...
         ENDIF
         VC = DX(I)*RC(I)*DY(J)*DZ(K)
         U_DOT(1,1,NM) = U_DOT(1,1,NM) + VC*FVX(I,J,K)
         U_DOT(1,2,NM) = U_DOT(1,2,NM) + VC*FVX(I,J,K)
         U_DOT(1,3,NM) = U_DOT(1,3,NM) + VC*FVX(I,J,K)
      ENDDO
   ENDDO
ENDDO
...
DO K=1,KBAR
   DO J=0,JBAR
      DO I=1,IBAR
         ...
         ENDIF
         VC = DX(I)*RC(I)*DY(J)*DZ(K)
         U_DOT(2,1,NM) = U_DOT(2,1,NM) + VC*FVY(I,J,K)
         U_DOT(2,2,NM) = U_DOT(2,2,NM) + VC*FVY(I,J,K)
         U_DOT(2,3,NM) = U_DOT(2,3,NM) + VC*FVY(I,J,K)
      ENDDO
   ENDDO
ENDDO
...
DO K=0,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         ENDIF
         VC = DX(I)*RC(I)*DY(J)*DZ(K)
         U_DOT(3,1,NM) = U_DOT(3,1,NM) + VC*FVZ(I,J,K)
         U_DOT(3,2,NM) = U_DOT(3,2,NM) + VC*FVZ(I,J,K)
         U_DOT(3,3,NM) = U_DOT(3,3,NM) + VC*FVZ(I,J,K)
      ENDDO
   ENDDO
ENDDO

U_DOT_SUM(1,:,NM) = U_DOT_SUM(1,:,NM) + DT*U_DOT(1,:,NM)
U_DOT_SUM(2,:,NM) = U_DOT_SUM(2,:,NM) + DT*U_DOT(2,:,NM)
U_DOT_SUM(3,:,NM) = U_DOT_SUM(3,:,NM) + DT*U_DOT(3,:,NM)

END SUBROUTINE UPDATE_MOMENTUM


SUBROUTINE DUMP_MOMENTUM(T,DT)

REAL(EB), INTENT(IN) :: T,DT
REAL(FB) :: STIME
REAL(EB) :: DUMMY,U_DOT_TOTAL(3,N_U_DOT)
INTEGER :: NM
...
DO NM=1,NMESHES
   U_DOT_TOTAL(1,:) = U_DOT_TOTAL(1,:) + U_DOT_SUM(1,:,NM)/MAX(DT,T-T_LAST_DUMP_MOM)
   U_DOT_TOTAL(2,:) = U_DOT_TOTAL(2,:) + U_DOT_SUM(2,:,NM)/MAX(DT,T-T_LAST_DUMP_MOM)
   U_DOT_TOTAL(3,:) = U_DOT_TOTAL(3,:) + U_DOT_SUM(3,:,NM)/MAX(DT,T-T_LAST_DUMP_MOM)
ENDDO

DUMMY = DT

WRITE(TCFORM,'(A,I4.4,5A)') "(",3*N_U_DOT,"(",FMT_R,",','),",FMT_R,")"
WRITE(LU_MOM,TCFORM) STIME,U_DOT_TOTAL(1,:),U_DOT_TOTAL(2,:),U_DOT_TOTAL(3,:)

END SUBROUTINE DUMP_MOMENTUM
...
IF (MOMENTUM_FILE) THEN
   INQUIRE(UNIT=LU_MOM,OPENED=OPN)
   IF (OPN) FLUSH(LU_MOM)
ENDIF
