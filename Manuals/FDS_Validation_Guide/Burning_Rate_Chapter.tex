...
The thermophysical data is based on several sources. Density values are based on data listed in \cite{Babrauskas:1}. Heat of vaporization, specific heat, and boiling temperature are found fron NIST Chemistry Webbook. Heat of vaporization is evalueated at the liquid boiling point. The radiative fractions and soot yields are from \cite{SFPE:Tewarson}.  Thermal conductivies are based on \cite{CRCHandbook}, except for butane. For butane the thermal conductivity is found from \cite{Webbook:FluidThermo}.
...
%Fuel & $\rho$ (\si{\kg/\m^3}) \cite{Webbook:FluidThermo}& $c_p$ (\si{\kJ/\kg.\K}) \cite{Webbook:FluidThermo} & k (\si{\W/\m\K}) \cite{Webbook:FluidThermo} & $\Delta h_c$ (\si{\kJ/\kg}) \cite{Webbook:Thermo} & $\Delta h_v$ (\si{\kJ/\kg}) \cite{Webbook:Thermo} & $\chi_r$ (-) \cite{SFPE} &  $Y_s$ (\si{\g/\g})  \cite{SFPE} \\ 
Fuel     & $\rho$ \cite{Babrauskas:1}  & $c_p$ \cite{Webbook:HeatCapacity}  & k \cite{CRCHandbook,Webbook:FluidThermo} &  $\Delta h_v$  \cite{Webbook:FluidThermo} & $\chi_r$ \cite{Beyler2:SFPE} &  \cite{SFPE:Tewarson}  $Y_s$ \cite{SFPE:Tewarson} & $T_b$ \cite{Webbook:BoilingPoint} \\
\hline \hline 
