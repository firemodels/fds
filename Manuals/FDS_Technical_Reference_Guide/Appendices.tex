...
As an example, the critical adiabatic flame temperature can be assumed to have a constant value of approximately 1600~K for hydrocarbon diffusion flames, as suggested by Beyler\footnote{C. Beyler, ``Flammability Limits of Premixed and Diffusion Flames,'' {\em SFPE Handbook of Fire Protection Engineering} (3rd Ed.), National Fire Protection Association, Quincy, MA, 2003.}. The combustion products can have an average specific heat of \SI{1.2}{kJ/(kg.K)} which is a value similar to that of nitrogen, the primary component of the products. Then, the relationship between the limiting oxygen mass fraction and the bulk temperature of a control volume is given by:
\be
   \hat{Y}_{\OTWO,lim} = \frac{ \overline{c_p} (T_{CFT}-T_m) }{\Delta H/r_\OTWO} \approx  \frac{ 1.2 \, (1600-T_m) }{ 13100}  \label{extinction_model}
\ee
For a control volume at a temperature of \SI{300}{K}, i.e., near room temperature, the limiting oxygen mass fraction would evaluate to $\hat{Y}_{\OTWO,lim}=0.12$.  This value is consistent with the measurements of Morehart, Zukoski and Kubota\footnote{Morehart, J., Zukoski, E., and Kubota, T., ``Characteristics of Large Diffusion Flames Burning in a Vitiated Atmosphere,'' {\em Third International Symposium on Fire Safety Science}, Elsevier Science Publishers, pp.~575-583, 1991.} who measured the oxygen concentration at extinction of flames by dilution of air with combustion products. They found that flames self-extinguished at oxygen concentrations of 12.4~\% to 14.3~\%. Note that their results are expressed as volume, not mass, fractions. Beyler's chapter in the SFPE Handbook references other researchers who measured oxygen concentrations at extinction ranging from 12~\% to 15~\%.
...
\end{equation}
The term on the right in angled brackets is a conditional mean.  Here it is modeled using a variant of IEM \cite{Dopazo:1974} which we call `interaction by exchange with the mixed mean' or IEMM.  When including chemical reaction, the conditional mean is modeled by
\begin{equation}
\label{eq:iem2}
...
\chapter{Limiting Behavior of the PaSR Model}
...
%\paragraph{Remarks}
%\begin{enumerate}
%%\item It is not clear why $\gamma_\lambda$ should be squared in (\ref{eq:lilleberg_edc}).  In fact, \cite{Chen:1} does not square this quantity.
...
%   
%%   There is clearly a sign error in (\ref{eq:psr}), which may account for unsatisfactory results when implementing kinetics into the PSR models by previous authors \cite{Lilleberg:2010}. {\color{red}[I'm not sure I want to include all this in the paper.  But I cannot seem to reconcile the sign issue.  I would rather be able to simply say our approach compares with Panjwani et al. for constant $\zeta$.]}
%\end{enumerate}
...
\begin{align}
    \frac{\d \mathbf{u}_\text{r}}{\d t} = -\frac{1}{2} \rho C_\text{d} A_\text{p} \left(\frac{1}{m_{\rm p}} + \frac{1}{M_{\rm p}} \right) \mathbf{u}_\text{r} |\mathbf{u}_\text{r}| + \mathbf{g} \,.
\end{align}
The equation above can be written in short as
\begin{align}
    \frac{\d \mathbf{u}_\text{r}}{\d t} = -K_{\rm p} \mathbf{u}_\text{r} |\mathbf{u}_\text{r}| + \mathbf{g} \quad ; \quad K_{\rm p} \equiv \frac{1}{2} \rho C_\text{d} A_\text{p} \left(\frac{1}{m_{\rm p}} + \frac{1}{M_{\rm p}} \right) \,.
\end{align}
...
\begin{align}
    ...
    \mathbf{u}_\text{g} = \mathbf{g} t - \frac{\beta_{\rm p} t^2}{2} \left[\mathbf{u}_{\text{r}}(0) \left(\frac{\mathbf{u}_{\text{r}}(0) \cdot \mathbf{g}}{|\mathbf{u}_{\text{r}}(0)|^2}\right) + \mathbf{g}\right] + \mathit{O}(t^3) \,.
\end{align}
Assembling Eqs. (\ref{ud_exact}) and (\ref{ug_second_order}), the solution for $\mathbf{u}_\text{r}$ is
\begin{align}
    \label{ur_short}
    \mathbf{u}_\text{r} &= \frac{\mathbf{u}_{\text{r}}(0)}{1 + \beta_{\rm p} t} + \mathbf{g} t - \frac{\beta_{\rm p} t^2}{2} \left[\mathbf{u}_{\text{r}}(0) \left(\frac{\mathbf{u}_{\text{r}}(0) \cdot \mathbf{g}}{|\mathbf{u}_{\text{r}}(0)|^2}\right) + \mathbf{g}\right] + \mathit{O}(t^3) \quad ; \quad \beta_{\rm p} \equiv K_{\rm p} |\mathbf{u}_{\text{r}}(0)| \,. %\\
    %\label{ur_expanded}
...
The fluid and particles can gain or lose momentum due to gravity. Drag forces exchange momentum between the fluid and particle, which does not change the total momentum of the system. Summing the equations of motion and integrating in time to find the total momentum at an instant, we find that
\begin{align}
...
\begin{align*}
    \mathbf{U} + \mathbf{u}_\text{d} + \mathbf{u}_\text{g} + \alpha_{\rm p} \mathbf{U} &= \mathbf{u}(0) + \alpha_{\rm p} \mathbf{U}(0) + \mathbf{g} t \,, \\
...
The particle force terms in the fluid momentum equation in the FDS source code are called \texttt{LP\%ACCEL\_X}, \texttt{LP\%ACCEL\_Y}, and \texttt{LP\%ACCEL\_Z}. In the tech.\ guide notation these terms are the components of $\bof_{\rm b} / \rho$. The definition of \texttt{LP\%ACCEL\_$i$} (neglecting mass loss related terms seen in Eq.\ \ref{part_force}) is
\begin{equation}
   \texttt{LP\%ACCEL\_$i$} = \bof_{\rm b} / \rho = \frac{1}{V} \sum \frac{1}{2} C_\text{d} A_\text{p} (\mathbf{u} - \mathbf{U}) |\mathbf{u} - \mathbf{U}| \,,
\end{equation}
where $V$ is the cell volume.

To conserve momentum, rather than using the above result directly, the difference in momentum between the next and current time steps is calculated. This will discretely conserve momentum by construction. It also is often necessary for stability, as using the above result directly can be unstable as it is analogous to Forward Euler integration.

Another way to look at this is to consider rearranging Eq.\ \ref{particle_eom} to solve for the momentum transfered due to drag alone. If the particle phase momentum equation is multiplied by the product of the particle weight (\texttt{LP\%PWT}) multiplied by the reciprocal of the cell volume (\texttt{RVC}) (making a number density, $n$ for short) and divided by the fluid density, the particle force can be found.
\begin{equation}
   -\left(\frac{n m_{\rm p}}{\rho}\right) \frac{\d \mathbf{u}_\text{d}}{\d t} =  n \cdot \frac{1}{2} C_\text{d} A_\text{p} (\mathbf{u}_\text{d} - \mathbf{U}_\text{d}) |\mathbf{u}_\text{d} - \mathbf{U}_\text{d}| = \bof_{\rm b} / \rho \,.
\end{equation}

As $n \equiv (1/V) \sum$, the above expression is equal to the particle force term. Also note that the \text{d} subscript means that this is the velocity field induced solely due to drag, prior to including other terms for effects like gravity. This is necessary as the fluid force term is due solely to gravity.

Implemented in FDS, the time derivative is treated with Forward Euler, which is stable here, as it is merely the difference of momentum between the two time steps previously integrated in a different (stable) fashion. Thus,
\begin{equation}
   \texttt{LP\%ACCEL\_$i$} = \bof_{\rm b} / \rho = \left(\frac{n m_{\rm p}}{\rho}\right) \left(\frac{\mathbf{u}_\text{d}^n - \mathbf{u}_\text{d}^{n+1}}{\Delta t}\right) = \texttt{LP\%PWT*LP\%MASS/(RHO\_G/RVC)} \cdot \left(\frac{\mathbf{u}_\text{d}^n - \mathbf{u}_\text{d}^{n+1}}{\Delta t}\right) \,.
\end{equation}

For static particles, a slightly different approach is used. The fluid momentum equation, Eq.\ \ref{fluid_eom}, is rearranged to solve for the particle force term and this is implemented in FDS:
\begin{equation}
   \texttt{LP\%ACCEL\_$i$} = \bof_{\rm b} / \rho = \left(\frac{\mathbf{U}_\text{d}^{n+1} - \mathbf{U}_\text{d}^{n}}{\Delta t}\right) \,.
\end{equation}
...
\begin{figure}[t]
   ...
   \end{tikzpicture}
    \caption[Solid phase nodes and indexes]{Solid phase nodes and indexes. $r$ is the radius from the back of the material.}
