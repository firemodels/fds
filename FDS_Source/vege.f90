...
INTEGER I,II,I_OUTER_RING,KK,K_BOTTOM_RING
...
TREE_LOOP: DO NCT=1,N_TREES
   ...
   LPC%KILL_RADIUS = 0.5_EB/LPC%VEG_SV !radius bound below which fuel elements are removed
! 
      ...
      DO NXB = 1,IBAR
       DO NYB = 1,JBAR
        ...
        IF (RCELL <= R_TREE) THEN
         NLP  = NLP + 1
         NLP_TREE = NLP_TREE + 1
         CALL ALLOCATE_STORAGE(NM,SURF_INDEX=LAGRANGIAN_PARTICLE_CLASS(IPC)%SURF_INDEX,LPC_INDEX=IPC,LP_INDEX=NLP,TAG=PARTICLE_TAG)
          ...
          IF (XLOC >= XS_RECT_VEG(NCT) .AND. XLOC <= XF_RECT_VEG(NCT)) THEN
           DO NYB = 0,JBAR-1
            ...
            IF (YLOC >= YS_RECT_VEG(NCT) .AND. YLOC <= YF_RECT_VEG(NCT)) THEN
             ...
             CALL ALLOCATE_STORAGE(NM,SURF_INDEX=LAGRANGIAN_PARTICLE_CLASS(IPC)%SURF_INDEX,LPC_INDEX=IPC, &
    ...
    DO NZB=0,KBAR
     DO NXB=0,IBAR
      GRID_LOOP: DO NYB=0,JBAR
       ...
       IF (ABS(REAL(NXB,EB)-LP%X)<ZERO_P .AND. ABS(REAL(NYB,EB)-LP%Y)<ZERO_P .AND. ABS(REAL(NZB,EB)-LP%Z)<ZERO_P) THEN 
        IF(CELL_TAKEN_FLAG(NXB,NYB,NZB)) THEN
         ALLOCATE(LP%ONE_D%X(1))
         LP%ONE_D%X(1) = 0.0001_EB*LPC%KILL_RADIUS
         CYCLE REP_VEG_ELEMS
        ENDIF
        ...
        LP%W = 0.
        ALLOCATE(LP%ONE_D%X(1))
        LP%ONE_D%X(1) =  2./LPC%VEG_SV !cylinder, Porterie
        LP%IOR = 0
        ...
        ALLOCATE(LP%ONE_D%TMP(1))                
        LP%ONE_D%TMP(1) = LPC%VEG_INITIAL_TEMPERATURE
        ...
        ENDIF
        LP%VEG_EMISS = 4.*SIGMA*LP%VEG_KAPPA*LP%ONE_D%TMP(1)**4
        LP%VEG_DIVQR = 0.0_EB
...
PARTICLE_LOOP: DO I=1,NLP
 ...
 IF (N_TREE /= 0) THEN
   ...
   V_CELL = DX(LP%IIG)*DY(LP%JJG)*DZ(LP%KKG)
...
INTEGER :: I,II,JJ,KK,IIX,JJY,KKZ,N_TREE,I_FUEL
INTEGER, INTENT(IN) :: NM
...
PARTICLE_LOOP: DO I=1,NLP
 ...
 IF (.NOT. LPC%TREE) CYCLE PARTICLE_LOOP !Ensure grid cell has vegetation
 IF (LPC%MASSLESS) CYCLE PARTICLE_LOOP   !Skip PARTICLE if massless
 IF (LPC%VEG_STEM) CYCLE PARTICLE_LOOP   !Skip PARTICLE if veg is a tree stem
...
! Vegetation variables
 CHAR_FCTR = 1._EB - LPC%VEG_CHAR_FRACTION
 SV_VEG  = LP%VEG_SV !surface-to-volume ration 1/m
 TMP_VEG = LP%ONE_D%TMP(1)
...
! Determine grid cell quantities of the vegetation fuel element
 CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XI,YJ,ZK,IIX,JJY,KKZ)
 IIX = FLOOR(XI+0.5_EB)
 JJY = FLOOR(YJ+0.5_EB)
 KKZ = FLOOR(ZK+0.5_EB)
 !!******** Where is II,JJ,KK initialized?
...
! Set temperature of inert ignitor elements
 IF(LP%IGNITOR) THEN
  ...
  IF(T > LP%VEG_IGN_TOFF+LP%VEG_IGN_TRAMPOFF) THEN
   LP%ONE_D%X(1) = 0.0001_EB*LPC%KILL_RADIUS !remove ignitor element
   TMP_VEG_NEW = TMP_GAS
  ENDIF
 ENDIF
...
!Handle veg. fuel elements if element mass <= prescribed char mass
     IF (MPV_VEG <= MPV_VEG_MIN) THEN
       IF(LPC%VEG_REMOVE_CHARRED) LP%ONE_D%X(1) = 0.0001_EB*LPC%KILL_RADIUS !fuel element will be removed
     ENDIF
...
!Handle veg. fuel elements if element mass <= prescribed char mass
     IF (MPV_VEG <= MPV_VEG_MIN .AND. .NOT. LPC%VEG_CHAR_OXIDATION) THEN
       IF(LPC%VEG_REMOVE_CHARRED) LP%ONE_D%X(1) = 0.0001_EB*LPC%KILL_RADIUS !fuel element will be removed
...
!combustion)
  IF_CHAR_OXIDATION: IF (LPC%VEG_CHAR_OXIDATION) THEN
   ...
   IF_CHAR_OXIDATION_2: IF (MPV_CHAR > MPV_CHAR_MIN) THEN
     ...
     MPV_CHAR_LOSS = MIN(MPV_CHAR_LOSS,LPC%VEG_CHAR_FRACTION*MPV_VOLIT_MAX) !use specified max
     ...
     IF (MPV_VEG < MPV_VEG_MIN) THEN !charring reduce veg elem size
      LP%VEG_PACKING_RATIO = MPV_CHAR/(LPC%VEG_DENSITY*LPC%VEG_CHAR_FRACTION)
...
! print*,'********************************************'
 ...
 LP%ONE_D%TMP(1) = TMP_VEG_NEW
 LP%VEG_EMISS = 4.*SIGMA*LP%VEG_KAPPA*LP%ONE_D%TMP(1)**4 !used in RTE solver
...
! Add CO2 vapor mass from char oxidation mass to CO2 mass fraction
 IF (I_CO2 /= 0 .AND. LPC%VEG_CHAR_OXIDATION) THEN 
...
! Remove vegetation that has completely burned (i.e., MAXVAL(LP%ONE_D%X) set equal to zero)
...
!   QRADM_INC = WALL(IW)%ONE_D%QRADIN/WALL(IW)%ONE_D%EMISSIVITY + SIGMA*WALL(IW)%TMP_F**4 ! as done in FDS4
