...
\ee
where Kn is the particle Knudsen number given by the ratio of the mean free path of the gas to the particle radius.  Since, the mean free path of a gas is proportional to its temperature, Kn is computed as~\cite{Sippola:1}
\be
...
\be
 \frac{dT}{dx} = \frac{h \left( T_g - T_w \right)}{k_g}
\ee
...
u_{dt} = \left\{ \begin{array}{r@{\quad \quad}c}
         ...
         \end{array} \right.
\ee
where u$_{\tau}$ is the wall friction velocity computed by the Werner-Wengle model and $\tau^+$ is the dimensionless stopping distance given by~\cite{Ludwig_ICONE}
\be
