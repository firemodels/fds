...
REAL :: ACTIVATION_TEMPERATURE,A_C,ALPHA,AREA,A_T,A_V,CHI_R,CONDUIT_DIAMETER,CONDUIT_THICKNESS,CUTOFF_TIME,C_PL,C_CJ, &
        C_S,D,DELTA,DELTA_T_C,DT,H,H_K,H_V,JACKET_THICKNESS,K_S,L,L_F,LEAK_AREA,MASS_PER_LENGTH,M_DOT,P,Q,Q_STAR,Q_STEP, &
        R,RADIATIVE_FRACTION,RHO_AIR,RHO_S,RTI,SCALING_FACTOR,T,t_activation,T_END,T_P,TMP_A,TMP_G,T_CLOCK,U_JET, &
        V,V_DOT,W,W_V
...
LOGICAL :: PROFILE=.FALSE.,ITER=.TRUE.,TIME_OUTPUT=.FALSE.,FLIP_AXIS=.FALSE.
INTEGER :: I,IOS
...
NAMELIST /MOWRER/ TIME_RAMP,Q_RAMP,C_PL,C_CJ,DELTA_T_C,H,R,TMP_A,OUTPUT_FILE
NAMELIST /MILKE/ TIME_RAMP,Q_RAMP,C_PL,C_CJ,DELTA_T_C,H,R,TMP_A,RADIATIVE_FRACTION,OUTPUT_FILE
...
DO I=1,30
   ...
   WRITE(11,'(F6.1,A1,F6.1,A1,F6.1)') TIME_RAMP(I),',',t_activation,',',Q

   IF ((t_activation>0) .AND. (t_activation<=TIME_RAMP(I))) EXIT
ENDDO
