...
By default the mesh cells that fill the computational domain are uniform in size. However, it is possible to specify that the cells be non-uniform in one or two
\begin{figure}[ht]
...
\subsection{Special Topic: Specifying a Force Field}
...
\end{lstlisting}
You may apply a time ramp to {\ct FORCE\_VECTOR(1)}, for example, via {\ct RAMP\_FVX\_T} on {\ct MISC}.
...
\vskip\baselineskip
\noindent
{\ct CFL\_VELOCITY\_NORM=0} (LES default, least restrictive, corresponds to $L_\infty$ norm of velocity vector)
    \begin{equation}
    \frac{\|\mathbf{u}\|}{\delta x} = \max \left(\frac{|u|}{\delta x}, \frac{|v|}{\delta y}, \frac{|w|}{\delta z}\right)
    \end{equation}
{\ct CFL\_VELOCITY\_NORM=1} (DNS default, most restrictive, corresponds to $L_1$ norm of velocity vector)
    \begin{equation}
...
\end{lstlisting}
Also, you must explicitly tell FDS that two non-connected meshes need to share information. To do this, use the parameter {\ct PERIODIC\_MESH\_IDS(1:3)} on the appropriate {\ct MESH} lines. For example, suppose you have three meshes side by side. Mesh~1 and Mesh~3 have periodic boundaries, but they do not physically touch each other. Provide this information as follows:
\begin{lstlisting}
&MESH ID='mesh1', IJK=..., XB=..., PERIODIC_MESH_IDS(1)='mesh3' /
&MESH ID='mesh2', IJK=..., XB=... /
&MESH ID='mesh3', IJK=..., XB=..., PERIODIC_MESH_IDS(1)='mesh1' /
\end{lstlisting}
Note that {\ct PERIODIC\_MESH\_IDS} is an array of three character strings because it is possible to have three periodic boundaries, one in each coordinate direction. In cases where the number of meshes is not large, say a few dozen, you can set {\ct PROCESS\_ALL\_MESHES} to {\ct .TRUE.} on the {\ct MISC} line, in which case FDS will store the geometry of all meshes for all MPI processes, and in which case you need not specify {\ct PERIODIC\_MESH\_IDS}.
...
{\ct HT3D} solves the 3D heat equation within an {\ct {OBST}} region of the domain.  You may connect multiple {\ct {OBST}} regions.  The thermal properties of the material are uniform within a cell and are taken from a {\ct {MATL}} tied to the {\ct {OBST}} line.  If you want layers of material, you must use a different {\ct {OBST}} for each layer, and each layer must be at least one cell thick.  The model uses the underlying gas phase grid resolution and explicit time integration that subcycles if necessary to achieve a stable solution to the heat equation.  The advantage of using the {\ct {OBST}} framework is that things like parallel capabilities and output methods ({\ct {DEVC}} and {\ct {SLCF}}) are automatically available without additional code requirements. {\ct HT3D} is not currently hooked up to the pyrolysis model.

The method is invoked by adding {\ct {HT3D=.TRUE.}} to an {\ct {OBST}} line.  The {\ct {OBST}} line must then also have a {\ct {MATL\_ID}} associated.  If two-way coupling with the gas phase is desired, then the {\ct {SURF}} associated with the {\ct {OBST}} should also have {\ct {HT3D=.TRUE.}}.  Note that if the FDS time step is too large then the accuracy of two-way coupling can be poor.  You may need to set {\ct {DT}} on the {\ct {TIME}} line to be on the same order as the Von Neumann time step restriction based on the thermal diffusivity, $\alpha = k/(\rho c)$, of the solid (see also Sec.~\ref{info:CFL}),
\begin{equation}
\label{eq:vnht3d}
...
experiments, like TGA (thermogravimetric analysis).
...
The first method is the keyword {\ct HVAC\_PRES\_RELAK} on the {\ct MISC} line with a default value of 0.5. At each time step FDS evaluates the average pressure in the FDS gas cells adjacent to nodes connecting the FDS domain to the HVAC network. The node pressure at time step n+1 is taken as:
\be
P_{node}^{n+1}=P_{FDS} \; (1-HVAC\_PRES\_RELAX) \; + \; P_{node}^n \; HVAC\_PRES\_RELAX
\ee
...
Figure~\ref{pressure_rise_fig} shows the results of this calculation.  The density matches exactly showing
...
the ideal values obtained from the first law of thermodynamics.
...
\end{lstlisting}
The value of {\ct F} is the ratio of the temperature in degrees K to {\ct TMPA}.

By default, FDS assumes that the density and pressure decrease with height, regardless of the application or domain size. For most simulations, this effect is negligible, but it can be turned off completely by setting {\ct STRATIFICATION=.FALSE.} on the {\ct MISC} line.
...
If you want to simulate a wind, set {\ct U0}, {\ct V0}, and {\ct W0} to the three components of the prevailing wind field on the {\ct MISC} line. By default, their values are 0~m/s. For example, if the mean wind is 10~m/s in the northeast direction, set both the velocity components {\ct U0} and {\ct V0} to 7.07~m/s ($10 \cos(\pi/4) = 7.07$). Setting these values, however, does not ensure that this flow field will persist. There are several ways to maintain the wind field. One way is to specify the normal component of the wind velocity at one or two exterior boundaries of the computational domain. This is essentially like creating a giant wall of fans on one side of the domain and a passive open boundary on the other. This is not a particularly good strategy to use because the lateral (i.e. tangential to the wind field) boundary conditions are not well-defined.

The better way to specify a wind field is as follows. Suppose there are anemometers located at specific locations within the domain which continuously record wind speed and direction.  The process of steering the solution of the mass, momentum, and energy equations to match the statistics of the data gathered at the weather stations is known as \emph{data assimilation}.  This branch of modeling is early in its development, but very sophisticated (translation: \emph{complicated}) methods already exist \cite{Kalnay:2003} and are employed in operational weather forecasting models. In FDS, you may invoke the most rudimentary of data assimilation techniques, a method called \emph{nudging}, where you add a mean forcing term to the momentum equation to ``nudge'' the solution toward a desired result.  Currently, FDS can only affect the mean flow velocities.  To turn on this capability, specify the mean wind using {\ct U0} and {\ct V0} (rarely would you use {\ct W0}), along with the logical parameter {\ct MEAN\_FORCING(1:2)=.TRUE.} on {\ct MISC}.  This will cause FDS to drive the mean velocity components toward the values of {\ct U0} and {\ct V0}.  For example, to point the wind in the northeast direction (assuming that the positive $x$ axis points eastward) at 10~m/s, set the following parameters on the {\ct MISC} line:
\begin{lstlisting}
&MISC ..., MEAN_FORCING(1:2)=.TRUE., U0=7.07, V0=7.07, DT_MEAN_FORCING=0.1,
      RAMP_U0_T='t-ramp', ... , RAMP_V0_Z='z-ramp' /
\end{lstlisting}
Note that you must also specify a time scale for relaxation, {\ct DT\_MEAN\_FORCING}. The shorter this time scale, the faster the flow field will ``catch up'' to the mean wind, but at the expense of possibly washing out important flow structures. Try several values of this time scale for your particular simulation to determine what is appropriate for your case. There is no firm guidance in the atmospheric modeling literature.

The velocity components {\ct U0}, {\ct V0}, and {\ct W0} can be made functions of time and/or height using the {\ct RAMP} functions: {\ct RAMP\_U0\_T}, {\ct RAMP\_V0\_T}, and {\ct RAMP\_W0\_T} to vary the velocity components with time; and {\ct RAMP\_U0\_Z}, {\ct RAMP\_V0\_Z}, and {\ct RAMP\_W0\_Z} to vary the velocity components with height. For example,
\begin{lstlisting}
...
\end{lstlisting}
The parameter {\ct F} represents the fraction of the specified velocity component as a function of time {\ct T} (s) or height {\ct Z} (m).

For an outdoor flow, all other boundaries (except the ground) should be set to {\ct OPEN}. You may omit a region of the flow domain from forcing using the {\ct HOLE} feature.  If a {\ct HOLE} is used with {\ct MEAN\_FORCING} it is advisable to also set {\ct PROJECTION=.TRUE.} on {\ct MISC}; otherwise the combination of {\ct U0} in the bulk region and zero velocity in the {\ct HOLE} region creates an initial divergence error that will take some time to wash out of the domain.
...
\be
   u(z,t) = 10 \, \cos \left( \frac{2 \pi \, t}{50} \right) \; \left( \frac{z}{5} \right)^{0.2} \quad \hbox{m/s}  \quad ; \quad v(z,t) = 10 \, \sin \left( \frac{2 \pi \, t}{50} \right) \; \left( \frac{z}{5} \right)^{0.2} \quad \hbox{m/s}
\ee
The 360$^\circ$ turn of the wind in 50~s is not realistic, but this case is intended to test the ramp functionality. The plots in Fig.~\ref{wind_example_fig} display some results of the simulation. On the left is a plot showing the mean velocity components of the simulated wind field compared to the specified (ideal) values. Notice that the simulated wind field lags the specified field by roughly 1~s because the relaxation parameter {\ct DT\_MEAN\_FORCING} was set to 1~s. This time scale falls into the denominator of the forcing term that ``nudges'' the simulated wind to follow the specified time history. If it is too small, the ``nudging'' term becomes too dominant. The right hand plot of Fig.~\ref{wind_example_fig} displays the specified wind profile compared to the simulation after the wind has turned full circle. The two profiles should be roughly aligned to indicate that the lateral boundary conditions of the mean forcing scheme are working properly.

\begin{figure}[ht]
\includegraphics[width=3.2in]{SCRIPT_FIGURES/wind_example_time}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/wind_example_prof}
\caption[Results of the {\ct wind\_example} test case]{(Left) Mean velocity components of the wind vs. the specified values. (Right) Simulated vs. specified wind profile.}
\label{wind_example_fig}
\end{figure}

\subsection{Specifying a ``Wall of Wind''}

An alternative to {\ct MEAN\_FORCING} is to specify a power law wind profile at an external boundary of the computational domain. This essentially creates a ``wall of wind'' and for early versions of FDS it was the recommended method for specifying a wind. However, the {\ct MEAN\_FORCING} technique described above is preferable because it handles the lateral boundaries of the computational domain in a more natural way.

To specify a ``wall of wind'', set {\ct PROFILE='ATMOSPHERIC'} on the {\ct SURF} line that is assigned to an exterior lateral boundary of the computational domain. This generates a power law atmospheric wind profile of the form $u=u_0 (z/z_0)^p$ where $z$ is the height above the ground. If an atmospheric profile is prescribed, also prescribe {\ct Z0} for $z_0$  and {\ct PLE} for $p$. {\ct VEL} specifies the reference velocity $u_0$. Note that $z_0$ is not the ground, but rather the height above the ground where the wind speed is measured, like an elevated weather station. It is assumed that the ground is located at 0~m; to change this assumption, set {\ct GROUND\_LEVEL} on the {\ct MISC} line to be the appropriate elevation. Be careful not to apply an atmospheric velocity profile (e.g. negative $z$) below {\ct GROUND\_LEVEL} or FDS will stop with an error.
...
You may also specify {\ct AUTO\_IGNITION\_TEMPERATURE} on an {\ct INIT} region.  This feature is intended to be used as a pilot zone.  For example, you might use something like this for a piloted methane flame:
\begin{lstlisting}
&REAC FUEL='METHANE', AUTO_IGNITION_TEMPERATURE=627. /
&INIT XB=..., AUTO_IGNITION_TEMPERATURE=-273. /
\end{lstlisting}
where the {\ct XB} specifies a region at the base of the burner where AIT is set to achieve ``mixed is burnt''.
...
1\,\underbrace{\mathrm{(C_{25}H_{42}O_{6}N_{2})}}_\text{Fuel} + 27.23436\,\underbrace{\mathrm{(O_2 + 3.76 \; N_2})}_\text{Air} \longrightarrow \\
1\,\underbrace{\mathrm{(19.79113\; CO_2 + 0.166587 \; CO + 20.71987 \; H_2O + 5.60253\; C_{0.9}H_{0.1} + 103.40121 \; N_2)}}_\text{Products\_1}
\end{multline}
...
\end{figure}
Note that when using multiple chemical reactions, you must set {\ct SUPPRESSION=.FALSE.} on the {\ct MISC} line.
...
\section{Special Topic: Chemical Time Integration}
\label{info:chem_integration}
A diagnostic tool for the time integration of chemical reactions is the number of subiterations the ODE solver takes for a given FDS time step. For infinitely fast chemistry the number of {\ct CHEMISTRY SUBITERATIONS} should always be 1. For finite rate chemical reactions the number can vary between 1 and the minimum between {\ct MAX\_CHEMISTRY\_ITERATIONS} and the number needed to satisfy the error tolerance of the ODE solver. If the maximum value is reached, the user can increase the number of iterations or decrease the error tolerance constraint of the integrator.

%\paragraph{Remark}
%A limitation of using finite-rate reactions in an LES calculation is reduced temperature resolution. Finite-rate reactions are temperature dependent and the temperature in a large scale calculation is smeared out over a mesh cell. Setting {\ct TEMPERATURE\_DEPENDENT\_REACTION = .TRUE.} on the {\ct MISC} line will have FDS compute a local cell temperature based on the instantaneous energy release. This temperature is used only as the source temperature in the time integration of the finite-rate reactions. This temperature can be outputted using the {\ct QUANTITY='FLAME TEMPERATURE'}.


\section{Special Topic: Aerosol Deposition}
\label{info:deposition}

It is possible within FDS to model the deposition of smoke and aerosols onto solid surfaces. The aerosol deposition model is invoked by defining a species with the parameter {\ct AEROSOL=.TRUE.} on the {\ct SPEC} line along with the parameters {\ct DENSITY\_SOLID}, {\ct CONDUCTIVITY\_SOLID}, and {\ct MEAN\_DIAMETER}. By default, with {\ct AEROSOL=.TRUE.}, FDS will compute all of the aerosol deposition mechanisms discussed in the Technical Reference Guide~\cite{FDS_Math_Guide}. For diagnostic purposes, each deposition mechanism can be selectively disabled by using the logical parameters {\ct GRAVITATIONAL\_DEPOSITION}, {\ct THERMOPHORETIC\_DEPOSITION}, {\ct TURBULENT\_DEPOSITION}, and {\ct GRAVITATIONAL\_SETTLING} on the {\ct MISC} line. {\ct GRAVITATIONAL\_SETTLING} controls the downward movement of aerosols in the gas phase, whereas {\ct GRAVITATIONAL\_DEPOSITION} controls the deposition of aerosols onto upward-facing surfaces due to gravity.  The deposition velocity at the wall can be output using {\ct QUANTITY='DEPOSITION VELOCITY'} for a wall cell with {\ct DEVC} or {\ct BNDF}.
...
an input on the {\ct REAC} line. This radiated energy completely
...
The absorption and scattering of thermal radiation by Lagrangian
particles is included in the radiation transport equation.
The radiative properties can be given by specifying the components of the material refractive index on the corresponding {\ct PART} line, using keywords
...
The radiative properties of the water and fuel particles (droplets) are determined automatically. For fuel, the properties of heptane are assumed. The heptane values can
be overridden by specifying them on the {\ct PART} line.
...
Controlled fire experiments are often conducted using a spray burner, where a liquid fuel is sprayed out of a nozzle and ignited. In this example ({\ct spray\_burner.fds}), heptane from two nozzles is sprayed downwards into a steel pan.  The flow rate is increased linearly so that the fire grows to 2~MW in 20 s, burns steadily for another 20 s, and then ramps down linearly in 20 s. The key input parameters are given here:
\begin{lstlisting}
...
\section{Using Particles to Represent Vegetation}

Four namelist groups are needed to specify the properties and the location of vegetation using particles: {\ct SURF}, {\ct MATL}, {\ct PART}, and {\ct INIT}. A cone-shape tree crown comprised of needles (i.e., no roundwood) can be defined by the following namelist lines.

The {\ct SURF} and {\ct MATL} namelists specify the properties of the needles. In this example, the {\ct  SURF} namelist specifies that the mass of a needle is 70\% solid and 30\% moisture. The {\ct MATL ID='solid fuel'} namelist contains the properties of the needle's solid fuel and the {\ct MATL ID='moisture'} contains the properties of the needle's moisture. As a given mass of solid fuel undergoes pyrolysis,  74\% ({\ct NU\_SPEC} = 0.74) is converted to fuel gas (pyrolyzate) and 26\% ({\ct NU\_MATL=0.26}) remains as a solid in the form of char. The exothermic process of char oxidation is not modeled. Instead, char remains present as a solid with material properties listed in {\ct MATL ID='char'}.
\begin{lstlisting}
&SURF ID = 'needles'
...
&MATL ID = 'moisture'
      ...
      HEAT_OF_REACTION= 2500. /
\end{lstlisting}
The {\ct PART} namelist references {\ct SURF} for material properties and specifies the drag model. The {\ct INIT} namelist defines the geometry of the volume occupied by the vegetation (in this case a cone) and total mass (dry and moist) of the vegetation in that volume ({\ct MASS\_PER\_VOLUME}).
\begin{lstlisting}
...
\section{Conversion of Vegetation Parameters Commonly Used in Forestry to those Required by FDS}

Users of FDS that work in wildland fire or forestry will be unfamiliar with some of the vegetation material properties in FDS input files. Specifically, the thickness and length of needles or roundwood, for a given vegetation type, are not routinely reported (the surface-to-volume ratio is reported instead). Also, while the bulk density is reported (or can be easily found from vegetation loading and height), it is based on the dry mass. Moisture is reported on a dry weight basis. Thus, pending the creation of new namelists or the addition of new namelist parameters,  commonly reported vegetation properties cannot be directly used in FDS input files. Instead, they must be converted to the parameters needed by FDS. These conversions affect parameters in the {\ct SURF} and {\ct INIT} namelists and are given below.

The forestry literature provides vegetation moisture, $M_{\rm dw}$, on a dry weight basis. The {\ct SURF} namelist parameters for mass fractions of the solid fuel and moisture are then $1 / (1 + M_{\rm dw})$ and $M_{\rm dw} / (1 + M_{\rm dw}$), respectively. In addition, the surface-to-volume ratio of the vegetation, $\sigma_{\rm v}$, is reported instead of the physical dimensions. If we assume a cylindrical shape, with the length of the cylinder being much larger than the radius, then {\ct THICKNESS} = $2/\sigma_{\rm v}$ in the {\ct SURF} namelist (here {\ct THICKNESS} equal the radius of the cylindrical shaped vegetation element).

The {\ct INIT} namelist parameter {\ct MASS\_PER\_VOLUME} = $\rho_{\rm d} (1+M_{\rm dw})$ where $\rho_{\rm d}$ is the bulk density of the dry vegetation.

\section{Thermal Degradation Model for Vegetation}

The working model for the thermal degradation of a thermally-thin vegetative fuel used here is similar to that employed by others (e.g. Porterie et al. 2005; Morvan and Dupuy 2004; Houssami et al. 2016). The solid-phase thermal degradation process has three components: \\

\noindent 1. Endothermic drying
\be
 {\rm Virgin\ Veg} \rightarrow M_{\rm dw} {\rm H_2O} + (1-M_{\rm dw}) {\rm Dry\ Veg} \nonumber
\ee

\noindent 2. Endothermic pyrolysis
\be
 {\rm Dry\ Veg} \rightarrow \chi_{\rm char} {\rm Char} + (1-\chi_{\rm char})({\rm Fuel\ Gas + Soot}) \nonumber
\ee
...
\ee
\noindent
The stoichiometric constants are: $M_{\rm dw}$ = fuel (i.e., vegetation) moisture mass fraction on a dry weight basis;  $\chi_{\rm char}$ = mass fraction of dry fuel that is converted to char during pyrolysis;  $\nu_{\rm 0_2,char}$ = 1.65 [Porterie 2005];  $\chi_{\rm ash}$ = mass fraction of char that is converted to ash during char oxidation.

In the equations that follow, the subscript b denotes bulk quantities that are resolved on the computational grid. For example, the mass of dry vegetation, resolved on the computational grid, is denoted $\rho_{\rm b,dry}$ (kg m$^{-3}$). In the notation of Mell et al. (2009), $\rho_{\rm b,dry}=\langle m^{\prime\prime\prime}_{\rm dry}\rangle_{V_{\rm b}}$ where the angled brackets denote the explicit LES filtering over the grid cell volume $V_{\rm b}$. 

The Arrhenius rate equations for drying, pyrolysis, and char oxidation are:
\begin{eqnarray}
  R_{\rm H_2O} &=& \rho_{\rm b, H_2O}\ A_{\rm H_2O}\ T_{\rm v}^{-\frac{1}{2}} e^{-\frac{E_{\rm H_2O}}{T_{\rm v}}}, \nonumber \\
  R_{\rm pyr} &=& \rho_{\rm b,dry}\ A_{\rm pyr}\ e^{-\frac{E_{\rm pyr}}{T_{\rm v}}}, \nonumber \\
  R_{\rm char} &=& \frac{A_{\rm char}}{\nu_{\rm O_2,char}}\ \rho_{\rm g}\ Y_{\rm O_2}\ \sigma_{\rm v}\ \beta_{\rm v}\ e^{-\frac{E_{\rm char}}{T_{\rm v}}} (1+\beta_{\rm char}\sqrt{Re_{\rm v}}), \nonumber
\end{eqnarray}
where $T_{\rm v}$ is the temperature of the vegetation. The default values of the kinetic constants are given in Table~\ref{tab:veg_kinetics}. The Reynolds number is $Re_{\rm v} = 2\rho_{\rm g} r_{\rm v} / \mu $ with $r_{\rm v} = 2/\sigma_{\rm v}$ for a cylinder. The term containing $Re_{\rm v}$ is included to account for the oxygen blowing effects on char oxidation and the constant $\beta_{\rm char}$ is equal to 0.2 by default, which is the value used by Porterie et al. 2005.

\begin{table}[!ht]
\caption[Default vegetation kinetic constants]{Default vegetation kinetic constants (from Porterie et al., 2005)}
\vspace{0.1in}
...
$E_{\rm H_2O}$ (K)         & 5,800 \\ \hline
$A_{\rm pyr}$ (1/s)           & 36,300 \\ \hline
$E_{\rm pyr}$ (K)            & 7,250  \\ \hline
$A_{\rm char}$ (m/s)          & 430 \\ \hline
$E_{\rm char}$ (K)           & 9,000 \\ \hline
\end{tabular}
\end{center}
\end{table}
...
\rho_{\rm b}\ c_{\rm p,v} \frac{dT_{\rm v}}{dt} = -\Delta{\rm h_{vap}}\ R_{\rm H_2O} - \Delta {\rm h_{pyr}}\ R_{\rm pyr} - \alpha_{\rm char}\ \Delta{\rm h_{char}}\ R_{\rm char} - \langle \nabla \cdot{\mathbf{q}}^{\prime\prime}_{\rm c,b}\rangle_{\rm V_b} - \langle \nabla \cdot{\mathbf{q}}^{\prime\prime}_{\rm r,b}\rangle_{\rm V_b}.
\ee
The first three terms on the right-hand-side are, respectively, endothermic drying, endothermic pyrolysis, exothermic char oxidation. The non-dimensional weighting parameter, $\alpha_{\rm char}$, is the fraction of the heat generated by heterogeneous char oxidation that is deposited in the fuel element and (1-$\alpha_{\rm char}$) in the gas phase. By default, $\alpha_{\rm char}$ = 0.5, which has been used by others (e.g., Porterie et al. 2005). The fourth and fifth terms on the right-hand-side of Eq.~(\ref{eq:Tv_thermally_thin}) are the bulk contributions of convective and radiative heat transfer, respectively (Mell et al. 2009). The heats in Eq.~(\ref{eq:Tv_thermally_thin}) are $\Delta{\rm h_{vap}} = 2.259 \times 10^3$ kJ kg$^{-1}$;  $\Delta{\rm h_{vap}}$ = 418 kJ kg$^{-1}$ [Movan and Dupuy, 2001]; $\Delta{\rm h_{char}} = -32 \times 10^3$  kJ kg$^{-1}$ [Dahale et al. 2013].
...
\be
\rho_{\rm b} = \rho_{\rm b,dry} + \rho_{\rm b,char} + \rho_{\rm b,ash} + \rho_{\rm b,H_2O}, \nonumber
\ee
...
\begin{eqnarray}
c_{\rm p, dry} &=& 10 + 3.7T_{\rm v}~~~ ({\rm Ritchie~et~al.~1997}), \nonumber \\
c_{\rm p, char} &=& 420 + 2.09 T_{\rm v} + 6.85 \times 10^{-4}\ T^2_{\rm v} ~~~({\rm Park~et~al.~ 2010}), \nonumber \\
c_{\rm p, ash} &=& 1244 (T_{\rm v} / 300)^{0.315}~~~({\rm Launtenberger~et~al.~2009}), \nonumber \\
c_{\rm p,H_2O} &=& 4190, \nonumber 
\end{eqnarray}

The rate of change of the total vegetative fuel mass is
\be
\dot{\rho}_{\rm b} = -R_{H_2O} - (1 - \chi_{\rm char})R_{\rm pyr} - (1-\chi_{\rm ash})R_{\rm char}, \nonumber
\ee
where $\dot{(~)}$ denotes $d(~)/dt$ and the rate of change for the components of the vegetative mass during thermal degradation are:

\begin{eqnarray}
\dot{\rho}_{\rm g,H_2O} &=& -R_{\rm H_2O}, \nonumber \\
\dot{\rho}_{\rm b,dry} &=& -R_{\rm dry}, \nonumber \\
\dot{\rho}_{\rm b,char} &=& -\chi_{\rm char}\ R_{\rm pyr} - R_{\rm char}. \nonumber
\end{eqnarray}
Gaseous products created during the thermal degradation process are water vapor (during drying), fuel vapor (during pyrolysis), and CO$_2$ (during char oxidation). The source terms (kg m$^{-3}$ s$^{-1}$) for these species are, respectively, $R_{\rm H_2O}$, $(1-\chi_{\rm char})R_{\rm pyr}$, and  $(1+\nu_{\rm O_2,char} - \chi_{\rm ash})R_{\rm char}$. In addition, the char oxidation process consumes oxygen in the gas phase at a rate of $-\nu_{\rm O_2,char} R_{\rm char}$. 
...
\linebreak[4]{\ct DIFFERENTIAL\_GAIN}; $e(t)$ is the error given by subtracting the {\ct TARGET\_VALUE} from the input; and $u(t)$ is the output.
...
\paragraph{\ct 'NET HEAT FLUX'} The word {\em net} is used here to describe the sum of the emitted and absorbed radiation at a solid surface:
\be
...
\paragraph{\ct 'RADIATIVE HEAT FLUX'}  This is the radiative component of Eq.~(\ref{net_flux}).
\be
   \dq_{\rm r}'' = \epsilon_{\rm s} \, \left( \dq_{\rm inc,rad}'' - \sigma \, T_{\rm s}^4 \right) \label{rad_flux}
\ee

\paragraph{\ct 'CONVECTIVE HEAT FLUX'} This is the convective component of Eq.~(\ref{net_flux}).
\be
   \dq_{\rm c}'' = h \, (T_{\rm gas} - T_{\rm s}) \label{con_flux}
\ee

\paragraph{\ct 'INCIDENT HEAT FLUX'} This is the term $\dq_{\rm inc,rad}''$ in Eq.~(\ref{net_flux}).

\paragraph{\ct 'GAUGE HEAT FLUX'} If you want to compare the heat flux predicted by FDS to a measurement made with a heat flux gauge, use {\ct 'GAUGE HEAT FLUX'}:
\be
...
\paragraph{\ct 'RADIOMETER'}  Similar to a heat flux gauge, a {\ct 'RADIOMETER'} measures only the radiative heat flux:
\be
...
\paragraph{\ct 'RADIATIVE HEAT FLUX GAS'}  All of the above heat flux output quantities are defined at a solid surface. To record the radiative heat flux away from a solid surface, add a device with the following format:
\begin{lstlisting}
&DEVC ID='flux', QUANTITY='RADIATIVE HEAT FLUX GAS', XYZ=..., ORIENTATION=-1,0,0 /
\end{lstlisting}
Note that {\ct XB} and {\ct STATISTICS} are not appropriate for this quantity.  This single line of input is a special shortcut\footnote{This feature maintains backward compatibility with FDS~5.} for the following lines of input that make use of a Lagrangian particle as a surrogate target:
\begin{lstlisting}
...
\paragraph{\ct 'RADIANCE'}  This is the term for the radiation intensity at a point in the gas, $\bx$, with solid angle, $\bs$, where the summation is over all spectral bands:
\be
...
{\ct DIFFUSIVITY}$^{*}$                         & Species diffusivity                               & m$^2$/s        & D,I,P,S      \\ \hline
{\ct EMISSIVITY}                                & Surface emissivity (usually constant)             &                & B,D          \\ \hline
{\ct EXTINCTION}                                & Section~\ref{info:extinct_out}                    &                & D,S          \\ \hline
%{\ct FLAME TEMPERATURE}                         & Section \ref{info:finite}                         & $^\circ$C      & D,I,P,S      \\ \hline
...
{\ct NORMALIZED MASS}                           & Section~\ref{info:material_components}            &                & D            \\ \hline
{\ct NORMALIZED MASS LOSS RATE}$^{*}$           & Section~\ref{info:material_components}            & 1/s            & D            \\ \hline
...
{\ct MULT\_ID }            & Character                     & Section~\ref{info:MULT}                   &    &                  \\ \hline
{\ct PERIODIC\_MESH\_IDS}  & Character Array               & Section~\ref{sec:periodic}                &    &                  \\ \hline
...
%{\ct EVAPORATION}                               & Logical       & Allow evaporation                                     &               & {\ct .TRUE.}      \\ \hline
...
{\ct H\_F\_REFERENCE\_TEMPERATURE}              & Real          & Section~\ref{info:enthalpy}                           &  \si{\degree C} & 25.             \\ \hline
...
{\ct MAX\_CHEMISTRY\_ITERATIONS}                & Integer       & Section~\ref{info:chem_integration}                   &               &  1000             \\ \hline
...
%{\ct N\_FIXED\_CHEMISTRY\_SUBSTEPS}             & Integer       & DT\_SUB = DT/N\_FIXED\_CHEMISTRY\_SUBSTEPS            &               & -1                \\ \hline
%{\ct NEW\_MOMENTUM\_NUDGING}                    & Logical       & Use MOMENTUM\_NUDGING\_2                              &               & {\ct .FALSE.}     \\ \hline
...
%{\ct NORTHANGLE}                                & Real          &                                                       &\si{\degree}   & {\ct -190.0}      \\ \hline
...
{\ct RAMP\_U0\_T}                               & Character     & Section~\ref{info:mean_forcing}                       &               &                   \\ \hline
{\ct RAMP\_V0\_T}                               & Character     & Section~\ref{info:mean_forcing}                       &               &                   \\ \hline
{\ct RAMP\_W0\_T}                               & Character     & Section~\ref{info:mean_forcing}                       &               &                   \\ \hline
{\ct RAMP\_U0\_Z}                               & Character     & Section~\ref{info:mean_forcing}                       &               &                   \\ \hline
{\ct RAMP\_V0\_Z}                               & Character     & Section~\ref{info:mean_forcing}                       &               &                   \\ \hline
{\ct RAMP\_W0\_Z}                               & Character     & Section~\ref{info:mean_forcing}                       &               &                   \\ \hline
...
%{\ct SPONGE\_LAYER\_DISTANCE}                   & Real          & Distance for linear damping of DT_MEAN_FORCING at bc  &               & 0.                \\ \hline
...
%{\ct WIND\_BOUNDARY}                            & Logical       & Use new open boundary condition                       &               & {\ct .FALSE.}     \\ \hline
...
{\ct PRESSURE\_TOLERANCE}           & Real          & Section~\ref{info:PRES}           & m/s$^2$       & 1.                        \\ \hline
...
Both FDS and Smokeview live within a GitHub \emph{organization} called ``Fire Models''.  The current location of the organization is \href{https://github.com/firemodels}{{https://github.com/firemodels}}.  The repositories that are used by the FDS and Smokeview projects are listed below along with a brief description:
...
\noindent The Wiki Pages are particularly useful in describing the details of how you go about working with the repository assets.
...
Table~\ref{tab:sourcefiles} lists the files that make up the FDS source code. Files with the ``.f90'' suffix contain free-form Fortran~90 instructions conforming to the ANSI and ISO standards (2008 edition). A {\ct makefile} is available in the FDS-SMV Repository that contains platform specific options for compilation. Note the following:
\begin{itemize}
\item The source code consists entirely of Fortran statements organized into about 35 files. Some compilers have a standard optimization level, plus various degrees of ``aggressive'' optimization. Be cautious in using the highest levels of optimization.
...
{\ct mpip.f90}   & MPI ``include'' statement for MPI compilation \\ \hline
{\ct mpis.f90}   & ``Dummy'' Fortran/MPI bindings for non-MPI compilation \\ \hline
...
T(1) , -001   , 001    , ... , -001
            ...
            .
\end{lstlisting}
where {\ct N\_CTRL} is the number of controllers, {\ct ID(I)} is the user-defined ID of the {\ct I}th control function, and plus or minus 1's represent the state {\ct -1 = .FALSE. and +1 = .TRUE.} of the {\ct I}th control function at the particular time. The files can be imported into Microsoft Excel or almost any other spreadsheet program. If the number of columns exceeds 256, the file will automatically be split into smaller files.
