...
REAL(EB) :: ZZ_GET(0:N_TRACKED_SPECIES),NU_EDDY,DELTA,KSGS,U2,V2,W2,AA,A_IJ(3,3),BB,B_IJ(3,3),&
            DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,MU_DNS,SLIP_COEF,VEL_GAS,VEL_T,RAMP_T,TSI,TAU_WALL,MU_WALL
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      CASE(SOLID_BOUNDARY)
         ...
         ! Compute effective viscosity based on stress from friction velocity

         IF (LES) THEN
            TAU_WALL = RHOP(IIG,JJG,KKG)*WC%U_TAU**2
            MU_WALL = TAU_WALL*(0.5_EB/WC%RDN)/(ABS(VEL_GAS-VEL_T)+TWO_EPSILON_EB)
            MU(IIG,JJG,KKG) = MAX(MU_DNS,MU_WALL)
         ENDIF
