...
IF (CORRECTOR) THEN
   IF (.NOT. EQUILIBRIUM_MODEL) CALL PARTICLE_MASS_ENERGY_TRANSFER(T,DT,NM)
...
PARTICLE_NON_STATIC_IF: IF (.NOT.LPC%STATIC) THEN ! Move airborne, non-stationary particles
   ...
   ELSE
      ...
      LP%ACCEL_Y  = 0._EB
      LP%ACCEL_Z  = 0._EB
      LP%X = X_OLD + DT*U_OLD + GX_LOC*HALF_DT2
      LP%Y = Y_OLD + DT*V_OLD + GY_LOC*HALF_DT2
      LP%Z = Z_OLD + DT*W_OLD + GZ_LOC*HALF_DT2
   ENDIF
   ...
   SELECT CASE (LPC%DRAG_LAW)
      ...
      CASE (SCREEN_DRAG)
         IF (QREL > 0.015_EB .AND. LPC%FREE_AREA_FRACTION < 1.0_EB ) THEN ! Testing shows below this can have instability
            ...
            DRAG_MAX(1) = LP%ACCEL_X*DT/(-UBAR+TWO_EPSILON_EB)
            DRAG_MAX(2) = LP%ACCEL_Y*DT/(-VBAR+TWO_EPSILON_EB)
            DRAG_MAX(3) = LP%ACCEL_Z*DT/(-WBAR+TWO_EPSILON_EB)
         ELSE
            LP%ACCEL_X = 0._EB
            LP%ACCEL_Y = 0._EB
            LP%ACCEL_Z = 0._EB
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: DROP_AREA=>NULL(),DROP_DEN=>NULL(),DROP_DEN_ALL=>NULL(),DROP_RAD=>NULL(),DROP_TMP=>NULL(),&
                                       ...
                                       QR_LIQUID=>NULL(),T_EQ=>NULL(),TMP_INTERIM=>NULL(),Y_EQ=>NULL()
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZ_INTERIM
REAL(EB), POINTER, DIMENSION(:) :: FILM_THICKNESS=>NULL(),WALL_QCONF=>NULL()
REAL(EB) :: A_DROP,AREA_ADD,BETA,C_DROP,CHI,CP,CP2,D_AIR,DELTA_HG,DEN_ADD,DHOR,EXP_OMEGAT,F,FG_TERM,G,H_G,H_L,H_L_OLD,H_V,H_L_NEW,&
            H_HEAT,H_MASS,&
            ...
            Y_DROP,Y_GAS,ZZ_GET(1:N_TRACKED_SPECIES)
INTEGER :: I,IP,I_FUEL,II,JJ,KK,ITMP,IW,N_LPC,NS,Y_INDEX,Z_INDEX
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
CHARACTER(MESSAGE_LENGTH) :: MESSAGE
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
...
SPECIES_LOOP: DO Z_INDEX = 1,N_TRACKED_SPECIES
   ...
   DO KK=1,KBAR
      DO JJ=1,JBAR
         EQ_LOOP_2: DO II=1,IBAR
            ...
            IF (M_LIQUID(II,JJ,KK)<TWO_EPSILON_EB) CYCLE EQ_LOOP_2
            P_RATIO = P_STP/PBAR(0,PRESSURE_ZONE(II,JJ,KK))
            RVC = RDX(II)*RRN(II)*RDY(JJ)*RDZ(KK)
            ...
            CALL GET_EQUILIBRIUM(Y_EQ(II,JJ,KK),T_EQ(II,JJ,KK),H_G,H_LIQUID(II,JJ,KK),ZZ_GET,M_GAS, &
                                 M_LIQUID(II,JJ,KK),Y_INDEX,Z_INDEX,QR_LIQUID(II,JJ,KK),P_RATIO)
   ...
   PARTICLE_LOOP:DO IP=1,NLP
      ...
      IF (Y_DROP > Y_GAS .AND. OMEGA(II,JJ,KK) > 0._EB) THEN
         SHERWOOD = 2._EB + SH_FAC_GAS*SQRT(LP%RE)
         CALL INTERPOLATE1D_UNIFORM(LBOUND(D_Z(:,Z_INDEX),1),D_Z(:,Z_INDEX),TMP_FILM,D_AIR)
         H_MASS   = SHERWOOD*D_AIR/(2._EB*R_DROP)
         BETA = RHO(II,JJ,KK)*CHI*H_MASS*(Y_GAS - Y_DROP)
         FG_TERM = MAX(0._EB,M_DROP**ONTH+BETA/(3*OMEGA(II,JJ,KK))*(1._EB-EXP(-OMEGA(II,JJ,KK)*0.5_EB*DT)))
         M_DROP_NEW =  MAX(0._EB,M_DROP**ONTH+BETA/(3*OMEGA(II,JJ,KK))*(1._EB-EXP(-OMEGA(II,JJ,KK)*DT)))**3
         M_VAP = (M_DROP - M_DROP_NEW) * LP%PWT
      ...
      IF (M_DROP_NEW > 0._EB) THEN
         EXP_OMEGAT = EXP(-OMEGA(II,JJ,KK)*0.5_EB*DT)
         ...
         G = ((CHI*H_HEAT*(TMP(II,JJ,KK)-T_EQ(II,JJ,KK))+BETA*H_V)*EXP_OMEGAT+CHI*H_HEAT*T_EQ(II,JJ,KK))/&
             (C_DROP*FG_TERM)+QR_LIQUID(II,JJ,KK)/(C_DROP*FG_TERM**3)
         ...
         H_L_NEW = M_DROP_NEW*TMP_DROP_NEW*C_DROP*LP%PWT
      ENDIF
      ...
      IF (SUM(AVG_DROP_AREA(II,JJ,KK,:))>0._EB) Q_D_1 = MIN(H_V*M_DROP,(QR_W(II,JJ,KK)/SUM(AVG_DROP_AREA(II,JJ,KK,:))) * &
                                                        0.25_EB*A_DROP * DT)*LP%PWT
