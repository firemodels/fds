   ...
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         ...
         SELECT CASE (COMBUSTION_ODE)
            ...
            CASE(IMPLICIT_EULER)
               CALL ODE_IMPLICIT_EULER(I,J,K,ZZ_GET,Q(I,J,K))
         END SELECT        
...
SUBROUTINE ODE_IMPLICIT_EULER(I,J,K,ZZ_GET,Q_OUT)
INTEGER,INTENT(IN):: I,J,K
REAL(EB),INTENT(OUT):: Q_OUT
REAL(EB),INTENT(INOUT) :: ZZ_GET(0:N_TRACKED_SPECIES)
REAL(EB) :: ZZ_0(0:N_TRACKED_SPECIES),ZZ_I(0:N_TRACKED_SPECIES),ZZ_N(0:N_TRACKED_SPECIES),DZZDT(0:N_TRACKED_SPECIES),&
            DT_ODE,DT_NEW,RATE_CONSTANT(1:N_REACTIONS),Q_NR(1:N_REACTIONS),Q_NRE(1:N_REACTIONS),Q_SUM,DT_SUM,TOL_CALC,TOL,&
            RATE_CONSTANTE(1:N_REACTIONS),DZZDTE(0:N_TRACKED_SPECIES),ZZ_NE(0:N_TRACKED_SPECIES),DZZDTS(0:N_TRACKED_SPECIES)
INTEGER :: NR,I_TS,NODETS=20,NS
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
...
ODE_LOOP: DO WHILE (DT_SUM < DT)
   ...
   TOLERANCE_LOOP: DO WHILE (TOL_CALC > TOL)
      ...
      REACTION_LOOP: DO NR = 1, N_REACTIONS   
         ...
         IF (RATE_CONSTANT(NR) < ZERO_P) CYCLE REACTION_LOOP
         Q_NR(NR) = RATE_CONSTANT(NR)*RN%HEAT_OF_COMBUSTION*RHO(I,J,K)
         Q_NRE(NR) = RATE_CONSTANTE(NR)*RN%HEAT_OF_COMBUSTION*RHO(I,J,K)
         DZZDT = DZZDT + RN%NU * SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT(NR)
         DZZDTE = DZZDTE + RN%NU * SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANTE(NR)
         DZZDTS=RN%NU * SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT(NR)         
      ...
      ZZ_N = ZZ_0 + 0.5_EB * (DZZDT + DZZDTE) * DT_NEW ! Updates species vector     
      IF (ANY(ZZ_N < 0._EB)) THEN !Shrinks time step if time step results in any negative mass fractions
         DO NS=0,N_TRACKED_SPECIES
           ! IF (ZZ_N(NS) < 0._EB .AND. ABS(DZZDT(NS))>ZERO_P) DT_NEW = MIN(DT_NEW,ABS(ZZ_N(NS)/DZZDT(NS)))
            IF (ZZ_N(NS) < 0._EB .AND. ABS(DZZDT(NS))>ZERO_P) DT_NEW = 0.25_EB*DT_NEW
         ENDDO
      ELSE
         TOL_CALC = MAXVAL(ABS(ZZ_N-ZZ_I)) ! Check tolerance
         ZZ_I = ZZ_N ! Updates guess vector 
      ENDIF 
...
END SUBROUTINE ODE_IMPLICIT_EULER
