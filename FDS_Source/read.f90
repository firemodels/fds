...
NAMELIST /SPEC/ AEROSOL,BACKGROUND,CONDUCTIVITY,CONDUCTIVITY_SOLID, &
                ...
                MASS_EXTINCTION_COEFFICIENT,MASS_FRACTION,MASS_FRACTION_0,MEAN_DIAMETER,MELTING_TEMPERATURE,MW,&
...
COUNT_SPEC_LOOP: DO
   ...
   IF (SPEC_ID(1)=='null' .OR. (SPEC_ID(1)/='null' .AND. SPEC_ID(2)=='null')) THEN
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   ENDIF 
   PROP_ID_S = SS%ID
   IF (SPEC_ID(1)/='null' .AND. SPEC_ID(2)=='null') PROP_ID_S=SPEC_ID(1)
