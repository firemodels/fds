...
REAL(EB) :: DELKDELT,VC,VC1,DTDX,DTDY,DTDZ,TNOW,ZZ_GET(0:N_TRACKED_SPECIES), &
            ...
            TMP_G,TMP_WGT,DIV_DIFF_HEAT_FLUX,H_S,ZZZ(1:4),DU_P,DU_M,UN,RCON_DIFF,MU_DNS
TYPE(SURFACE_TYPE), POINTER :: SF
TYPE(SPECIES_MIXTURE_TYPE), POINTER :: SM,SM0
...
K_DNS_OR_LES: IF (DNS .OR. RESEARCH_MODE) THEN
   ...
   IF (RESEARCH_MODE) THEN
      ! account for molecular and turbulent transport
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
               IF (N_TRACKED_SPECIES > 0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZP(I,J,K,1:N_TRACKED_SPECIES)
               CALL GET_VISCOSITY(ZZ_GET,MU_DNS,TMP(I,J,K))
               KP(I,J,K) = KP(I,J,K) + MAX(0._EB,(MU(I,J,K)-MU_DNS))*CP(I,J,K)*RPR
            ENDDO
         ENDDO
      ENDDO
   ENDIF
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBM1
         ...
         FX(I,J,K) = SCALAR_FACE_VALUE(UU(I,J,K),ZZZ)
      ENDDO
   ENDDO
ENDDO
...
DO K=1,KBAR
   DO J=1,JBM1
      ...
      DO I=1,IBAR
         ...
         FY(I,J,K) = SCALAR_FACE_VALUE(VV(I,J,K),ZZZ)
      ENDDO
   ENDDO
ENDDO
...
DO K=1,KBM1
   ...
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         FZ(I,J,K) = SCALAR_FACE_VALUE(WW(I,J,K),ZZZ)
      ENDDO
   ENDDO
ENDDO
      ...
      OFF_WALL_SELECT_1: SELECT CASE(IOR)
         CASE( 1) OFF_WALL_SELECT_1
            ...
            IF ((UU(II+1,JJ,KK)>0._EB) .AND. .NOT.(WALL_INDEX(CELL_INDEX(II+1,JJ,KK),+1)>0)) THEN
               ...
               FX(II+1,JJ,KK) = SCALAR_FACE_VALUE(UU(II+1,JJ,KK),ZZZ)
            ENDIF
         CASE(-1) OFF_WALL_SELECT_1
            !            FX/UU(II-2)     ghost
            ! ... |  II-2  |  II-1  ///   II   ///
            ...
            IF ((UU(II-2,JJ,KK)<0._EB) .AND. .NOT.(WALL_INDEX(CELL_INDEX(II-1,JJ,KK),-1)>0)) THEN
               ...
               FX(II-2,JJ,KK) = SCALAR_FACE_VALUE(UU(II-2,JJ,KK),ZZZ)
            ENDIF
