...
SUBROUTINE GETVERTS2(BOX_BOUNDS,V0,V1,V2,V3,VERTS,NVERTS,FACES,FACE_ID,NFACES2,NPOLYS,VOLUME,FLAG,BOX_STATE,ERROR,ERR)
...
INTEGER, DIMENSION(0:2) :: FACE
REAL(EB) :: VV1(0:2), VV2(0:2), VV3(0:2)

REAL(EB) :: XMIN, YMIN, ZMIN
REAL(EB) :: XMAX, YMAX, ZMAX
INTEGER :: VERT_STATE

REAL(EB) :: VOL, VOL_TOTAL, VOLUME2
...
CALL GET_VERTS(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB,VERTS_EB,NVERTS,FACES,FACE_ID,NFACES2,NPOLYS,BOX_STATE,VERT_STATE)
DO I = 0, NFACES2-1
   FACE(0:2) = FACES(3*I:3*I+2)
   VV1(0:2) = VERTS_EB(3*FACE(0):3*FACE(0)+2)
   VV2(0:2) = VERTS_EB(3*FACE(1):3*FACE(1)+2)
   VV3(0:2) = VERTS_EB(3*FACE(2):3*FACE(2)+2)
   WRITE(6,*)"FACE:",I
   WRITE(6,*)"VV1:", VV1(0),VV1(1),VV1(2)
   WRITE(6,*)"VV2:", VV2(0),VV2(1),VV2(2)
   WRITE(6,*)"VV3:", VV3(0),VV3(1),VV3(2)
ENDDO

VERTS(0:3*NVERTS-1) = REAL(VERTS_EB(0:3*NVERTS-1),FB)

VOLUME = REAL(GET_TETRABOX_VOLUME(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB),FB)
VOLUME2 = GET_POLYHEDRON_VOLUME(VERTS_EB,NVERTS,FACES,NFACES2)
...
DO K = KBOX_MIN, KBOX_MAX
   FACTORK = REAL(K-KBOX_MIN,EB)/REAL(KBOX_MAX+1-KBOX_MIN,EB)
   FACTORK2 = REAL(K+1-KBOX_MIN,EB)/REAL(KBOX_MAX+1-KBOX_MIN,EB)
   BOX_BOUNDS_IJK_EB(4) = BOX_BOUNDS_EB(4)*(1.0_EB-FACTORK) +   BOX_BOUNDS_EB(5)*FACTORK
   BOX_BOUNDS_IJK_EB(5) = BOX_BOUNDS_EB(4)*(1.0_EB-FACTORK2) +   BOX_BOUNDS_EB(5)*FACTORK2
   DO J = JBOX_MIN, JBOX_MAX
      FACTORJ = REAL(J-JBOX_MIN,EB)/REAL(JBOX_MAX+1-JBOX_MIN,EB)
      FACTORJ2 = REAL(J+1-JBOX_MIN,EB)/REAL(JBOX_MAX+1-JBOX_MIN,EB)
      BOX_BOUNDS_IJK_EB(2) = BOX_BOUNDS_EB(2)*(1.0_EB-FACTORJ) +   BOX_BOUNDS_EB(3)*FACTORJ
      BOX_BOUNDS_IJK_EB(3) = BOX_BOUNDS_EB(2)*(1.0_EB-FACTORJ2) +   BOX_BOUNDS_EB(3)*FACTORJ2
      DO I = IBOX_MIN, IBOX_MAX
         FACTORI = REAL(I-IBOX_MIN,EB)/REAL(IBOX_MAX+1-IBOX_MIN,EB)
         FACTORI2 = REAL(I+1-IBOX_MIN,EB)/REAL(IBOX_MAX+1-IBOX_MIN,EB)
         BOX_BOUNDS_IJK_EB(0) = BOX_BOUNDS_EB(0)*(1.0_EB-FACTORI) +   BOX_BOUNDS_EB(1)*FACTORI
         BOX_BOUNDS_IJK_EB(1) = BOX_BOUNDS_EB(0)*(1.0_EB-FACTORI2) +   BOX_BOUNDS_EB(1)*FACTORI2
         VOL = VOL + GET_TETRABOX_VOLUME(BOX_BOUNDS_IJK_EB,V0_EB,V1_EB,V2_EB,V3_EB)
      END DO
   END DO
END DO
VOL_TOTAL=TETRAHEDRON_VOLUME(V0_EB,V1_EB,V2_EB,V3_EB)
ERR = ABS(VOL-VOL_TOTAL)
IF(ERR.GT.1.0E-10_EB)THEN
  ERROR = 1
ENDIF
IF(FLAG.EQ.0.OR.ERR.GT.1.0E-10_EB)THEN
  WRITE(LU_OUTPUT,*)"box check: ",VOL,VOL_TOTAL,(VOL-VOL_TOTAL)/VOL_TOTAL
ENDIF
...
SUBROUTINE VERIFY_TETRABOX()

! compute box tetrahedron intersection volume for a large number of cases
...
REAL(FB), DIMENSION(0:599) :: VERTS
INTEGER, DIMENSION(0:199) :: FACE_ID
INTEGER, DIMENSION(0:599) :: FACES
INTEGER :: NVERTS,NFACES,NPOLYS
REAL(FB) :: VOLUME
INTEGER :: FLAG, ERROR
REAL(EB) :: ERR,MAX_ERR
REAL(EB) :: XMIN, XMAX, YMIN, YMAX, ZMIN, ZMAX
...
DO J1 = 0, N-1
DO K1 = 0, N-1
   V1(0) = 0.6_EB + REAL(I1,EB)*0.4_EB/REAL(N-1,EB)
   V1(1) = 0.0_EB + REAL(J1,EB)*0.4_EB/REAL(N-1,EB)
   V1(2) = 0.0_EB + REAL(K1,EB)*0.5_EB/REAL(N-1,EB)
DO I2 = 0, N-1
DO J2 = 0, N-1
DO K2 = 0, N-1
V2(0) = 0.0_EB + REAL(I2,EB)*0.5_EB/REAL(N-1,EB)
V2(1) = 0.5_EB + REAL(J2,EB)*0.5_EB/REAL(N-1,EB)
V2(2) = 0.0_EB + REAL(K2,EB)*0.5_EB/REAL(N-1,EB)

V3(0) = (V0(0)+V1(0)+V2(0))/3.0_EB
V3(1) = (V0(1)+V1(1)+V2(1))/3.0_EB
V3(2) = MAX(V0(2),V1(2),V2(2))+1.0_EB

XMIN = MIN(V0(0),V1(0),V2(0),V3(0))-0.5_EB
XMAX = MAX(V0(0),V1(0),V2(0),V3(0))+0.5_EB

YMIN = MIN(V0(1),V1(1),V2(1),V3(1))-0.5_EB
YMAX = MAX(V0(1),V1(1),V2(1),V3(1))+0.5_EB

ZMIN = MIN(V0(2),V1(2),V2(2),V3(2))-0.5_EB
ZMAX = MAX(V0(2),V1(2),V2(2),V3(2))+0.5_EB
...
DO K3 = 0, N-1
DO K4 = 0, N-1
   BOX_BOUNDS(0) =  XMIN +               REAL(I3,EB)*(XMAX-XMIN)/REAL(2*N,EB)
   BOX_BOUNDS(1) =  (XMIN+XMAX)/2.0_EB + REAL(I4,EB)*(XMAX-XMIN)/REAL(2*N,EB)

   BOX_BOUNDS(2) =  YMIN +               REAL(J3,EB)*(YMAX-YMIN)/REAL(2*N,EB)
   BOX_BOUNDS(3) =  (YMIN+YMAX)/2.0_EB + REAL(J4,EB)*(YMAX-YMIN)/REAL(2*N,EB)

   BOX_BOUNDS(4) =  ZMIN +               REAL(K3,EB)*(ZMAX-ZMIN)/REAL(2*N,EB)
   BOX_BOUNDS(5) =  (ZMIN+ZMAX)/2.0_EB + REAL(K4,EB)*(ZMAX-ZMIN)/REAL(2*N,EB)

   CALL GETVERTS2(BOX_BOUNDS,V0,V1,V2,V3,VERTS,NVERTS,FACES,FACE_ID,NFACES,NPOLYS,VOLUME,FLAG,BOX_STATE,ERROR,ERR)
   MAX_ERR = MAX(ERR,MAX_ERR)
   COUNT = COUNT + 1
   ERROR_COUNT = ERROR_COUNT + ERROR
   IF(ERROR.EQ.1)THEN
   WRITE(LU_OUTPUT,*)I1,I2,J1,J2,K1,K2
   WRITE(LU_OUTPUT,*)I3,I4,J3,J4,K3,K4
   WRITE(LU_OUTPUT,*)"VERTS"
   WRITE(LU_OUTPUT,*)V0
   WRITE(LU_OUTPUT,*)V1
   WRITE(LU_OUTPUT,*)V2
   WRITE(LU_OUTPUT,*)V3
   WRITE(LU_OUTPUT,*)"BOX"
   WRITE(LU_OUTPUT,*)BOX_BOUNDS(0),BOX_BOUNDS(1)
   WRITE(LU_OUTPUT,*)BOX_BOUNDS(2),BOX_BOUNDS(3)
   WRITE(LU_OUTPUT,*)BOX_BOUNDS(4),BOX_BOUNDS(5)
   WRITE(LU_OUTPUT,*)""
ENDIF
END DO
END DO
...
END DO
TAFTER=SECOND()
WRITE(LU_OUTPUT,*)"total CPU time=",TAFTER-TBEFORE,"CPU time per calc=",(TAFTER-TBEFORE)/(REAL(COUNT,EB)*4.0_EB**3)
WRITE(LU_OUTPUT,*)"error count=",ERROR_COUNT," total count=",COUNT
WRITE(LU_OUTPUT,*)"max err=",MAX_ERR
RETURN
END SUBROUTINE VERIFY_TETRABOX
