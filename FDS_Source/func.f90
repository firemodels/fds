...
USE GLOBAL_CONSTANTS, ONLY: SIGMA,TMPA4,TMPA,RPI,N_TRACKED_SPECIES,T_BEGIN,DIRICHLET,INITIALIZATION_PHASE
INTEGER, PARAMETER :: N_NEW_STORAGE_SLOTS=1000
...
INTEGER :: STORAGE_INDEX,I1,I2,I,OLD_INDEX,STORAGE_INDEX_OLD,STORAGE_INDEX_OPEN,N_STORAGE_SLOTS_OLD,NAS
LOGICAL :: NEW_SLOT
...
IF (PRESENT(WALL_INDEX)) THEN
   ...
   ! Check if there is enough wall cells
...
ELSEIF(PRESENT(LP_INDEX)) THEN
   ...
   ! Check if there is enough particles
...
WC%BOUNDARY_TYPE      => OS%INTEGERS( 4,STORAGE_INDEX) ; IF (NEW) WC%BOUNDARY_TYPE = NULL_BOUNDARY
WC%SURF_INDEX_ORIG    => OS%INTEGERS( 5,STORAGE_INDEX) ; IF (NEW) WC%SURF_INDEX_ORIG = 0
WC%OBST_INDEX         => OS%INTEGERS( 6,STORAGE_INDEX) ; IF (NEW) WC%OBST_INDEX = 0
WC%PRESSURE_BC_INDEX  => OS%INTEGERS( 7,STORAGE_INDEX) ; IF (NEW) WC%PRESSURE_BC_INDEX = DIRICHLET
WC%PRESSURE_ZONE      => OS%INTEGERS( 8,STORAGE_INDEX) ; IF (NEW) WC%PRESSURE_ZONE = 0
WC%VENT_INDEX         => OS%INTEGERS( 9,STORAGE_INDEX) ; IF (NEW) WC%VENT_INDEX = 0
WC%NOM                => OS%INTEGERS(10,STORAGE_INDEX) ; IF (NEW) WC%NOM = 0
...
I1 = N_WALL_SCALAR_REALS+1 ; I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(1) - 1  ; WC%A_LP_MPUA(1:)       => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(2) - 1  ; WC%LP_CPUA(1:)         => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(3) - 1  ; WC%LP_MPUA(1:)         => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(4) - 1  ; WC%AWM_AEROSOL(1:)     => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(5) - 1  ; WC%RHODW(1:)           => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(6) - 1  ; WC%ZZ_F(1:)            => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(7) - 1  ; WC%VEG_FUELMASS_L(1:)  => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(8) - 1  ; WC%VEG_MOISTMASS_L(1:) => OS%REALS(I1:I2,STORAGE_INDEX)
I1 = I2+1 ;                  I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(9) - 1  ; WC%VEG_TMP_L(0:)       => OS%REALS(I1:I2,STORAGE_INDEX)
...
ONE_D%N_LAYER_CELLS(1:) => OS%INTEGERS(I1:I2,STORAGE_INDEX) 
...
ONE_D%TMP_B           => OS%REALS(RC+11,STORAGE_INDEX) ; IF (NEW) ONE_D%TMP_B           = SF%TMP_BACK
...
ONE_D%MASSFLUX_ACTUAL(0:) => OS%REALS(I1:I2,STORAGE_INDEX) 
IF (NEW) ONE_D%MASSFLUX_ACTUAL(0:SF%ONE_D_REALS_ARRAY_SIZE(1)-1) = 0._EB

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(2) - 1
ONE_D%MASSFLUX(0:) => OS%REALS(I1:I2,STORAGE_INDEX) 
IF (NEW) ONE_D%MASSFLUX(0:SF%ONE_D_REALS_ARRAY_SIZE(1)-1) = 0._EB

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(3) - 1
ONE_D%X(0:) => OS%REALS(I1:I2,STORAGE_INDEX) 
IF (NEW) ONE_D%X(0:) = SF%X_S(0:SF%N_CELLS_INI)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(4) - 1
ONE_D%TMP(0:) => OS%REALS(I1:I2,STORAGE_INDEX) 
IF (NEW) ONE_D%TMP(0:SF%N_CELLS_INI+1) = SF%TMP_INNER(1)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(5) - 1
DUMMY(1:) => OS%REALS(I1:I2,STORAGE_INDEX)
...
ONE_D%LAYER_THICKNESS(1:) => OS%REALS(I1:I2,STORAGE_INDEX) 
...
SUBROUTINE GET_N_LAYER_CELLS(DIFFUSIVITY,LAYER_THICKNESS,STRETCH_FACTOR,CELL_SIZE_FACTOR,N_CELLS,DX_MIN)
...
SEARCH_LOOP: DO
   ...
   THETA = TWOPI*RN
   CALL RANDOM_NUMBER(RN)
   RR = SQRT(RN)*RR0*(1._EB-(ZZ-Z0)/HH0)
   XX = X0 + RR*COS(THETA)
   YY = Y0 + RR*SIN(THETA)
...
ABSTRACT INTERFACE
   FUNCTION FILTER_WIDTH_TYPE(DX,DY,DZ)
      USE GLOBAL_CONSTANTS, ONLY: EB   
      REAL(EB), INTENT(IN):: DX,DY,DZ
      REAL(EB):: FILTER_WIDTH_TYPE
   END FUNCTION FILTER_WIDTH_TYPE
END INTERFACE

PROCEDURE (FILTER_WIDTH_TYPE), POINTER :: LES_FILTER_WIDTH_FUNCTION
...
SUBROUTINE GET_GIBBS_FREE_ENERGY(NU_TMP,G_F_OUT,TMPG)

INTEGER :: ITMP
REAL(EB), INTENT(IN) :: TMPG,NU_TMP(0:N_TRACKED_SPECIES)
REAL(EB), INTENT(OUT) :: G_F_OUT

ITMP = MIN(5000,NINT(TMPG))
G_F_OUT  = DOT_PRODUCT(G_F_Z(ITMP,:),-1._EB*NU_TMP)
...
REAL(EB) FUNCTION LES_FILTER_WIDTH_3D_MEAN(DX,DY,DZ)

REAL(EB), INTENT(IN) :: DX,DY,DZ

LES_FILTER_WIDTH_3D_MEAN = (DX*DY*DZ)**ONTH

END FUNCTION LES_FILTER_WIDTH_3D_MEAN


REAL(EB) FUNCTION LES_FILTER_WIDTH_3D_MAX(DX,DY,DZ)

REAL(EB), INTENT(IN) :: DX,DY,DZ

LES_FILTER_WIDTH_3D_MAX = MAX(DX,DY,DZ)

END FUNCTION LES_FILTER_WIDTH_3D_MAX


REAL(EB) FUNCTION LES_FILTER_WIDTH_3D_MIN(DX,DY,DZ)

REAL(EB), INTENT(IN) :: DX,DY,DZ

LES_FILTER_WIDTH_3D_MIN = MIN(DX,DY,DZ)

END FUNCTION LES_FILTER_WIDTH_3D_MIN


REAL(EB) FUNCTION LES_FILTER_WIDTH_2D_MEAN(DX,DY,DZ)

REAL(EB), INTENT(IN) :: DX,DY,DZ
REAL(EB) :: DUMMY

DUMMY = DY
LES_FILTER_WIDTH_2D_MEAN = SQRT(DX*DZ)

END FUNCTION LES_FILTER_WIDTH_2D_MEAN


REAL(EB) FUNCTION LES_FILTER_WIDTH_2D_MAX(DX,DY,DZ)

REAL(EB), INTENT(IN) :: DX,DY,DZ
REAL(EB) :: DUMMY

DUMMY = DY
LES_FILTER_WIDTH_2D_MAX = MAX(DX,DZ)

END FUNCTION LES_FILTER_WIDTH_2D_MAX


REAL(EB) FUNCTION LES_FILTER_WIDTH_2D_MIN(DX,DY,DZ)

REAL(EB), INTENT(IN) :: DX,DY,DZ
REAL(EB) :: DUMMY

DUMMY = DY
LES_FILTER_WIDTH_2D_MIN = MIN(DX,DZ)

END FUNCTION LES_FILTER_WIDTH_2D_MIN


SUBROUTINE GET_EQUILIBRIUM(Y_EQ,T_EQ,M_EQ,H_G,H_L,H_L_REF,ZZ_G,M_G,M_L,Y_INDEX,Z_INDEX,Q_IN)
REAL(EB), INTENT(IN) :: H_G,H_L,M_G,M_L,ZZ_G(0:N_TRACKED_SPECIES),Q_IN,H_L_REF
INTEGER,INTENT(IN) :: Y_INDEX,Z_INDEX
REAL(EB), INTENT(OUT) :: Y_EQ,T_EQ,M_EQ
REAL(EB) :: MW_G,MW_RATIO,C_P_G,C_P_L,TMP_WGT,H_V,E_TOT,ZZ_NEW(0:N_TRACKED_SPECIES),X_EQ, &
            MW_V,DHOR,M_ZZ_I(0:N_TRACKED_SPECIES),Y_ALL(1:N_SPECIES),TMP_1,TMP_2,E_TOT_1,E_TOT_2,E_TOT_N
INTEGER :: ITMP,ITCOUNT,NS
LOGICAL :: ITER
...
TMP_1 = SS%TMP_MELT
ITMP = INT(TMP_1)
TMP_WGT  = TMP_1- AINT(TMP_1)  
C_P_L    = (SS%C_P_L_BAR(ITMP)+TMP_WGT*(SS%C_P_L_BAR(ITMP+1)-SS%C_P_L_BAR(ITMP)))  
H_V = SS%H_V(ITMP)+TMP_WGT*(SS%H_V(ITMP+1)-SS%H_V(ITMP))
DHOR = H_V*MW_V/R0
X_EQ = MIN(1._EB,EXP(DHOR*(1._EB/SS%TMP_V-1._EB/TMP_1)))
Y_EQ = X_EQ/(MW_RATIO+(1._EB-MW_RATIO)*X_EQ)
IF (Y_EQ >= 1._EB) THEN
   M_EQ = M_L
ELSE
   M_EQ = MAX(0._EB,MIN(M_L,(M_ZZ_I(Z_INDEX) - M_G*Y_EQ)/(Y_EQ-1._EB)))
ENDIF
ZZ_NEW = M_ZZ_I
ZZ_NEW(Z_INDEX) = ZZ_NEW(Z_INDEX) + M_EQ
ZZ_NEW = ZZ_NEW/(M_G+M_EQ)
CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_NEW,C_P_G,TMP_1)
E_TOT_1 = TMP_1*((M_G+M_EQ)*C_P_G+(M_L-M_EQ)*C_P_L)+Q_IN-H_L_REF*(M_L-M_EQ)

TMP_2 = SS%TMP_V
ITMP = INT(TMP_2)
TMP_WGT  = TMP_2- AINT(TMP_2)   
C_P_L    = (SS%C_P_L_BAR(ITMP)+TMP_WGT*(SS%C_P_L_BAR(ITMP+1)-SS%C_P_L_BAR(ITMP)))  
H_V = SS%H_V(ITMP)+TMP_WGT*(SS%H_V(ITMP+1)-SS%H_V(ITMP))
DHOR = H_V*MW_V/R0
X_EQ = MIN(1._EB,EXP(DHOR*(1._EB/SS%TMP_V-1._EB/TMP_2)))
Y_EQ = X_EQ/(MW_RATIO+(1._EB-MW_RATIO)*X_EQ)
IF (Y_EQ >= 1._EB) THEN
   M_EQ = M_L
ELSE
...
E_TOT_2 = TMP_2*((M_G+M_EQ)*C_P_G+(M_L-M_EQ)*C_P_L)+Q_IN-H_L_REF*(M_L-M_EQ)

ITCOUNT = 0
ITER = .TRUE.
DO WHILE (ITCOUNT < 10 .AND. ITER)
   ITCOUNT = ITCOUNT + 1
   T_EQ = MIN(4999._EB,MAX(0._EB,(E_TOT-E_TOT_2)*(TMP_2-TMP_1)/(E_TOT_2-E_TOT_1)+TMP_2))
   ITMP = INT(T_EQ)
   ...
   ZZ_NEW(Z_INDEX) = ZZ_NEW(Z_INDEX) + M_EQ
   ZZ_NEW = ZZ_NEW/(M_G+M_EQ)
   CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_NEW,C_P_G,T_EQ)
   E_TOT_N = T_EQ*((M_G+M_EQ)*C_P_G+(M_L-M_EQ)*C_P_L)+Q_IN-H_L_REF*(M_L-M_EQ)
   ...
   ELSE
      ...
      E_TOT_1 = E_TOT_N
   ENDIF
   IF (ABS(TMP_1 - TMP_2) < 0.1_EB) ITER=.FALSE.
END DO
