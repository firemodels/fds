...
REAL(EB), INTENT(IN) :: T,DT,RHO_IN,MU_IN,AIT_IN,DELTA,CELL_VOLUME
...
REAL(EB) :: ERR_EST,ERR_TOL,A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            ZETA_1,ZETA_2,ZETA_4,AIT_LOC,D_F,TMP_IN,TMP_FLAME
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
   ...
   IF (SUPPRESSION) CALL CHECK_AUTO_IGNITION(EXTINCT,TMP_IN,AIT_LOC,T)
...
SUBROUTINE CHECK_AUTO_IGNITION(EXTINCT,TMP_IN,AIT_IN,T)
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
LOGICAL, INTENT(INOUT) :: EXTINCT
REAL(EB), INTENT(IN) :: TMP_IN,AIT_IN,T
INTEGER :: NR
...
SELECT CASE (EXTINCT_MOD)
   CASE DEFAULT
      ...
      REACTION_LOOP: DO NR=1,N_REACTIONS
         ...
         IF ( TMP_IN > AIT_LOC*TIME_RAMP_FACTOR ) THEN
            EXTINCT = .FALSE.
            EXIT REACTION_LOOP
         ENDIF
      ENDDO REACTION_LOOP
...
SUBROUTINE EXTINCT_1(EXTINCT,ZZ_0,TMP_IN,Q_OUT)
...
REAL(EB), INTENT(IN) :: TMP_IN,Q_OUT,ZZ_0(1:N_TRACKED_SPECIES)
LOGICAL, INTENT(INOUT) :: EXTINCT
REAL(EB) :: Y_O2,Y_O2_LIM,Q_LIM,TMP_FACTOR

EXTINCT = .FALSE.
CALL GET_MASS_FRACTION(ZZ_0,O2_INDEX,Y_O2)
IF (TMP_IN<EXTINCTION_1_TMP_CUTOFF) THEN
