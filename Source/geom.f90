...
ALLOCATE(EXPAND_CC(0:M%IBP1,0:M%JBP1,0:M%KBP1)); EXPAND_CC = .FALSE.
...
CUTFACE_LOOP : DO ICF=1,M%N_CUTFACE_MESH
  ...
  ! First Areas:
  AREA_CF_T = SUM(M%CUT_FACE(ICF)%AREA(1:NFACE))+TWO_EPSILON_EB
  DO JCF=1,NFACE
     FCT = M%CUT_FACE(ICF)%AREA(JCF)/AREA_CF_T
     SELECT CASE(X1AXIS)
        CASE(IAXIS); AREA_CRT = M%DY(J)*M%DZ(K)
        CASE(JAXIS); AREA_CRT = M%DX(I)*M%DZ(K)
        CASE(KAXIS); AREA_CRT = M%DX(I)*M%DY(J)
     END SELECT
     M%CUT_FACE(ICF)%AREA(JCF) = FCT*AREA_CRT
  ENDDO
  ! Then face centroids:
  M%CUT_FACE(ICF)%XYZCEN(IAXIS,1:NFACE) = M%XC(I)
  M%CUT_FACE(ICF)%XYZCEN(JAXIS,1:NFACE) = M%YC(J)
  M%CUT_FACE(ICF)%XYZCEN(KAXIS,1:NFACE) = M%ZC(K)
  SELECT CASE(X1AXIS)
     CASE(IAXIS); M%CUT_FACE(ICF)%XYZCEN(IAXIS,1:NFACE) = M%X(I)
     CASE(JAXIS); M%CUT_FACE(ICF)%XYZCEN(JAXIS,1:NFACE) = M%Y(J)
     CASE(KAXIS); M%CUT_FACE(ICF)%XYZCEN(KAXIS,1:NFACE) = M%Z(K)
  END SELECT
ENDDO CUTFACE_LOOP

CUTCELL_LOOP : DO ICC=1,M%N_CUTCELL_MESH
   ...
   ! First volumes:
   VOL_CC_T = SUM(M%CUT_CELL(ICC)%VOLUME(1:NCELL))+TWO_EPSILON_EB
   DO JCC=1,NCELL
      FCT = M%CUT_CELL(ICC)%VOLUME(JCC)/VOL_CC_T
      M%CUT_CELL(ICC)%VOLUME(JCC) = FCT*M%DX(I)*M%DY(J)*M%DZ(K)
   ENDDO
   ! Then cell centroids:
   M%CUT_CELL(ICC)%XYZCEN(IAXIS,1:NCELL) = M%XC(I)
   M%CUT_CELL(ICC)%XYZCEN(JAXIS,1:NCELL) = M%YC(J)
   M%CUT_CELL(ICC)%XYZCEN(KAXIS,1:NCELL) = M%ZC(K)
ENDDO CUTCELL_LOOP
