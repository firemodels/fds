...
REAL(EB) :: MASS_FRACTION_0,MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,FIC_CONCENTRATION,FLD_LETHAL_DOSE,HUMIDITY,ATOM_COUNTS(118),&
...
LOGICAL  :: ABSORBING,SMIX_COMPONENT_ONLY,BACKGROUND,PREDEFINED(MAX_SPECIES)
CHARACTER(30) :: FORMULA,PREDEFINED_SPEC_ID(MAX_SPECIES),RAMP_CP,RAMP_CP_L,RAMP_K,RAMP_MU,RAMP_D
...
NAMELIST /SPEC/ ABSORBING,BACKGROUND,CONDUCTIVITY,DIFFUSIVITY,EPSILONKLJ,FIC_CONCENTRATION,FLD_LETHAL_DOSE,FORMULA,FYI,HUMIDITY,ID,&
                ...
                SPECIFIC_ENTHALPY,SPECIFIC_HEAT,VISCOSITY,FORMULA,&
                RAMP_CP,RAMP_CP_L,RAMP_K,RAMP_MU,RAMP_D,SPECIFIC_HEAT_LIQUID,&                
                DENSITY_LIQUID,VAPORIZATION_TEMPERATURE,HEAT_OF_VAPORIZATION,MELTING_TEMPERATURE,H_V_REFERENCE_TEMPERATURE
...
IF (SIMPLE_CHEMISTRY) THEN
   ...
   PREDEFINED_SPEC_ID(7) = 'HYDROGEN'
   PREDEFINED_SPEC_ID(8) = 'SOOT'
ELSE
   N_SPECIES = 0
ENDIF
...
SPEC_READ_LOOP: DO N=1,N_SPECIES
   ...
   SS%HEAT_OF_VAPORIZATION        = HEAT_OF_VAPORIZATION
   SS%H_V_REFERENCE_TEMPERATURE   = H_V_REFERENCE_TEMPERATURE
   SS%MELTING_TEMPERATURE         = MELTING_TEMPERATURE
   SS%SPECIFIC_HEAT_LIQUID        = SPECIFIC_HEAT_LIQUID
   SS%VAPORIZATION_TEMPERATURE    = VAPORIZATION_TEMPERATURE

   IF (SPECIFIC_HEAT < 0._EB .AND. SPECIFIC_ENTHALPY > -1.E20_EB .AND. RAMP_CP=='null') THEN
      WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPECies ',N,': Specify both SPECIFIC_HEAT or RAMP_CP and SPECIFIC_ENTHALPY'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (SPECIFIC_HEAT > 0._EB .AND. SPECIFIC_ENTHALPY < -1.E20_EB) THEN
      WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPECies ',N,': Specify both SPECIFIC_HEAT and SPECIFIC_ENTHALPY'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (RAMP_CP/='null' .AND. SPECIFIC_ENTHALPY < -1.E20_EB) THEN
...
IF (SIMPLE_CHEMISTRY .AND. N<=2) THEN
   ...
   SELECT CASE(N)
      ...
      CASE(2)
         ...
         RN%NU_H2           = (SPECIES(FUEL_INDEX)%MW/MW_H2)     *RN%H2_YIELD
         RN%NU_H2O          = 0.5_EB*RN%H - RN%NU_H2 - 0.5_EB*RN%NU_SOOT*RN%SOOT_H_FRACTION
...
DO N=1,N_SPECIES
   ...
   IF(SS%RAMP_CP_INDEX > 0) THEN
      ...
      ENDDO
      SS%SPECIFIC_ENTHALPY = SS%SPECIFIC_ENTHALPY - H1
   ENDIF
END DO
...
INTEGER :: NR,NS,NS2
REAL(EB) :: H2_YIELD,SOOT_YIELD,CO_YIELD,Y_F_LFL,X_O2_LL,EPUMO2,BOF, &
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   RN%Y_O2_LL                   = X_O2_LL*MW_O2/(X_O2_LL*MW_O2+(1._EB-X_O2_LL)*MW_N2)
   RN%Y_F_LFL                   = Y_F_LFL
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   ! Heat of Combustion calculation (IDEAL not hooked up yet) for SIMPLE_CHEMISTRY
   IF (SIMPLE_CHEMISTRY) THEN
      ...
      ELSE
         IF (IDEAL) THEN
            ...
            RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION - RN%NU_CO*(CO2_HEAT_OF_FORMATION - CO_HEAT_OF_FORMATION) &
                                                          ...
                                                          - RN%NU_H2*H2O_HEAT_OF_FORMATION &
         ...
         ENDIF
         RN%EPUMO2 = -RN%HEAT_OF_COMBUSTION*SPECIES(FUEL_INDEX)%MW/(RN%NU_SPECIES(O2_INDEX)*SPECIES(O2_INDEX)%MW)
      ENDIF   
   ENDIF
...
! Adjust the evaporation rate of fuel droplets to account for difference in HoC.
...
! Assign Z_INDEX when SPEC_ID is specified. If no Z_INDEX, look for a Y_INDEX.
...
IF (TRIM(SPEC_ID)/='null' .AND. (Z_INDEX<0 .AND. Y_INDEX<1)) THEN
