...
IF (HVAC_MASS_TRANSPORT) THEN
   DO ND = 1, N_DUCTS
       ...
       DO NN = 1, DU%N_CELLS
           DU%ZZ_C(NN,1:N_TRACKED_SPECIES) = SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0
       ENDDO
       ALLOCATE(DU%RHO_C(DU%N_CELLS))
       ALLOCATE(DU%TMP_C(DU%N_CELLS))
       DO NN = 1, DU%N_CELLS
           DU%RHO_C(NN) = DUCTNODE(DU%NODE_INDEX(1))%RHO + ( ( DUCTNODE(DU%NODE_INDEX(2))%RHO - DUCTNODE(DU%NODE_INDEX(1))%RHO )&
                          / ( DU%N_CELLS + 1._EB )) * NN
           DU%TMP_C(NN) = DUCTNODE(DU%NODE_INDEX(1))%TMP + ( ( DUCTNODE(DU%NODE_INDEX(2))%TMP - DUCTNODE(DU%NODE_INDEX(1))%TMP )&
                          / ( DU%N_CELLS + 1._EB )) * NN
       ENDDO
...
DUCT_LOOP: DO ND = 1,N_DUCTS
   ...
   ELSE
      DUCTNODE(DU%NODE_INDEX(1))%RHO = DU%RHO_C(1)
      DUCTNODE(DU%NODE_INDEX(1))%ZZ(:) = DU%ZZ_C(1,:)
   ENDIF
