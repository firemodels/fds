...
Prof.~Ofodike Ezekoye and Craig Weinschenk of the University of Texas, Austin, helped to formulate and test the thermocouple model within FDS. This work was
based on an earlier analysis by Stephen Welsh and Philip Rubini.
...
For an LES calculation, it can be included by setting {\ct BAROCLINIC=.TRUE.} on the {\ct MISC} line. For DNS, it is included by default.
...
\begin{warning}
\noindent
In versions of FDS prior to 5.5.0, {\ct BAROCLINIC=.FALSE.} was the default for LES.
For FDS~5.5.0 and 5.5.1, however, {\ct BAROCLINIC} was set to {\ct .TRUE.} by default. Its use was responsible for numerical instabilities in a number
of cases. In test versions of FDS~6, the problems do not occur, thus {\ct BAROCLINIC=.TRUE.} for test versions of FDS~6.
\end{warning}
...
discussion of these parameters is given in the FDS Technical Reference Guide~\cite{FDS_Math_Guide}.
...
This construct can also be used for {\ct DENSITY} or {\ct MASS\_FRACTION(N)} where {\ct N} indicates the {\ct N}th species listed in the input file.
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=3in]{SCRIPT_FIGURES/sawtooth}
\end{center}
...
\be \dq_c'' = h \; \Delta T
    ...
    \max \; \left[ \; C_1 \, |\Delta T|^\ot \; , \;
            \frac{k}{L} \; C_2 \; \RE^\fofi \; \PR^\ot \;
            \right]  \quad
    \hbox{W/m$^2$/K} \ee
where $\Delta T$ is the difference between the wall and the gas temperature,
$C_1$ is the coefficient for natural convection (1.52 for a horizontal surface
and 1.31 for a vertical surface, by default) and $C_2$ is the coefficient for forced convection (0.037 by default, see \cite{Holman:1} Eq.~(5-85));
...
calculations. You can change the empirical coefficients using {\ct C\_HORIZONTAL} or {\ct C\_VERTICAL} for $C_1$ and
{\ct C\_FORCED} for $C_2$, all of which are input on the {\ct MISC} line.
...
{\ct TMP\_FRONT} and {\ct EMISSIVITY} appropriately. Note that if you wish there to be only a convective heat flux from a
...
words, you use these parameters to {\em specify} the burning rate.  Note that all pyrolysis models in FDS require the user to explicitly define the gas phase reac \ref {info:REAC}
...
\begin{figure}[ht]
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/thermoplastic_temperature} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/thermoplastic_heat_flux} \\
\includegraphics[width=3.2in]{SCRIPT_FIGURES/thermoplastic_burn_rate} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/thermoplastic_thickness}
\end{tabular*}
...
\begin{figure}[ht]
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/charring_solid_temperature} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/charring_solid_burn_rate}
\end{tabular*}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=3.1in]{SCRIPT_FIGURES/zone_break}
\end{center}
...
can be many types of combustibles in an FDS fire simulation, in the simple chemistry, mixing controlled combustion model there can only be one gaseous fuel. The reason is cost. It is expensive to
...
The gas phase reaction can be described in two ways. By default, a so-called {\em mixing controlled} model is used to account for the evolution of
...
costlier and more complicated approach than the {\em mixing controlled} model. This chapter describes both methods, with an emphasis on the more
commonly used mixing controlled model.


\section{Combustion: The \texorpdfstring{{\tt REAC}}{REAC} Namelist Group}
\label{info:REAC}
...
In both cases, the combustion reaction must be specified.  This is done using the {\ct REAC} namelist group.  The simplest approach to this is treat the fuel as simple mixture of C, H, O, and N that reacts in either one or two steps based using mixing controlled combustion.  A slightly more complex approach is to define a set of gas species and mixing controlled reactions.  The most complex approach is to define a set of gas species and reactions that use temperature dependent kinetics.  For most FDS users, the mixing controlled combustion model with simple chemistry will be the model of choice.

\subsection{Mixing Controlled Reactions with Simple Chemistry}

This approach to combustion considers a single fuel species that is composed primarily of C,H,O, and N that reacts with oxygen in one or two mixing controlled steps to form water vapor and carbon dioxide with the fuel nitrogen becoming nitrogen gas.

The chemical composition of the fuel is specified using the {\ct REAC} namelist group. For the default simple chemistry 
model, only a single {\ct REAC} line is needed. If combustion is desired, a {\ct REAC} line must be specified and there is no default fuel chemistry.  The user is responsible for defined the fuel chemistry and the post-combustion yields of CO and soot.
...
the fuel and the presence of and OTHER species. FDS will use that information internally to
...
\begin{description}
\item[{\ct FUEL}] A character string that identifies fuel species for the reaction. When using simple chemistry, specifying {\ct FUEL} will cause FDS to use the built-in thermophyscial properties for that species when computing quantites such as specific heat or viscosity.  This parameter is independent of the inputs for the fuel chemistry, i.e. {\ct C, H, O, N, OTHER}. Table~\ref{tab:gasspecies} provides a listing of the available species.  By default, FDS uses the gas thermophysical properties of {\ct ETHYLENE} for the fuel.
...
\item[{\ct AUTO\_IGNITION\_TEMPERATURE}] The temperature below which combustion will not occur.  Note that if this parameter is used, then some form of ignition source must be present in order for combution to begin. (Default 0.0 K)
...
Normally when you specify a fire via the {\ct REAC} line the mixing controlled combustion model is applied. A a set of three or four lumped species, see ~\ref{info:SMIX}, represent the air, the fuel (plus an optional diluent), and the products (two product species if CO formatin is being modeled).  The major reactants and products of combustion -- fuel, O$_2$, CO$_2$,
H$_2$O, N$_2$, CO and soot -- are all pre-tabulated functions of
the lumped species. The fuel chemistry listed under the {\ct REAC} namelist
group is used to generate the table associating the mass fractions
with the lumped species. You need not, {\em and should not},
...
This gas can be tracked separately via an additional scalar transport equation\footnote{
...
parameters {\ct MASS\_FRACTION(N)} or {\ct MASS\_FLUX(N)} along with the character array {\ct SPEC\_ID}. The index {\ct N} refers to the position in the {\ct SPEC\_ID} array with the exception that the backgrond species is not listed and is always defined by N=0. Following is a very simple example of how a gas can be introduced into the simulation.
...
There are three ways in which a species can be used in an FDS computation: it can be tracked as a seperate species, it can be the background species, or it can only exist as part of a lumped species defined with {\ct SMIX}.  If a species only exists as part of one or more lumped species, the parameter {\ct LUMPED\_SPECIES\_ONLY=.TRUE.} should be set.  This tells FDS not allocate space for the species in the array of tracked gasses.  If a species is to be used as the background species, the parameter {\ct BACKGROUND=.TRUE.} should be set.  This also tells FDS not allocate space for the species in the array of tracked gasses. A species with {\ct LUMPED\_SPECIES\_ONLY=.TRUE.} cannot be used as an individual species and it cannot be used as the background species; however, the background species and an individual species can also be used as part of one or more lumped species.
...
{\ct SPEC} line with the {\ct BACKGROUND\_SPECIES} and if needed {\ct BACKGROUND\_SPECIES\_ONLY} keywords.  If this species is not listed in Table~\ref{tab:gasspecies}, specify its molecular weight, {\ct MW},
...
\hline
Species &   Mol.~Wgt.        & $\sigma$  & $\epsilon/k$   & Liquid \\
        &   (g/mol)          & (\AA)     & (K)            &        \\ \hline
\hline
{\ct ACETYLENE}          & 24   & 4.033     &231.8           &        \\ \hline
{\ct AIR}                & 29   & 3.711     & 78.6           &    Y   \\ \hline
{\ct ARGON}              & 40   & 3.42      &124.0           &    Y   \\ \hline
{\ct CARBON DIOXIDE}     & 44   & 3.941     &195.2           &        \\ \hline
{\ct CARBON MONOXIDE}    & 28   & 3.690     & 91.7           &    Y   \\ \hline
{\ct ETHANE}             & 30   & 4.443     &215.7           &    Y   \\ \hline
{\ct ETHANOL}            & 46   & 4.530     &362.6           &    Y   \\ \hline
{\ct ETHYLENE}           & 28   & 4.163     &224.7           &    Y   \\ \hline
{\ct HELIUM}             &  4   & 2.551     & 10.22          &    Y   \\ \hline
{\ct HYDROGEN}           &  2   & 2.827     & 59.7           &    Y   \\ \hline
{\ct HYDROGEN BROMIDE}   & 81   & 3.353     &449.0           &    Y   \\ \hline
{\ct HYDROGEN CHLORIDE}  & 36   & 3.339     &344.7           &    Y   \\ \hline
{\ct HYDROGEN CYANIDE}   & 26   & 3.63      &569.1           &    Y   \\ \hline
{\ct HYDROGEN FLOURIDE}  & 20   & 3.148     &330.0           &    Y   \\ \hline
{\ct ISOPROPANOL}        & 60   & 4.549     &576.7           &    Y   \\ \hline
{\ct METHANE}            & 16   & 3.758     &148.6           &    Y   \\ \hline
{\ct METHANOL}           & 32   & 3.626     &481.8           &    Y   \\ \hline
{\ct N-BUTANE}           & 58   & 4.687     &531.4           &    Y   \\ \hline
{\ct N-DECANE}           &142   & 5.233     &226.46          &        \\ \hline
{\ct N-HEPTANE}          &100   & 4.701     &205.75          &    Y   \\ \hline
{\ct N-HEXANE}           & 86   & 5.949     &399.3           &    Y   \\ \hline
{\ct N-OCTANE}           &114   & 4.892     &231.16          &    Y   \\ \hline
{\ct NITROGEN}           & 28   & 3.798     & 71.4           &    Y   \\ \hline
{\ct NITROGEN OXIDE}     & 46   & 3.467     &232.4           &    Y   \\ \hline
{\ct OXYGEN}             & 32   & 3.467     &106.7           &    Y   \\ \hline
{\ct PROPANE}            & 44   & 5.118     &237.1           &    Y   \\ \hline
{\ct PROPYLENE}          & 42   & 4.678     &298.9           &    Y   \\ \hline
{\ct SOOT}               & 12   & 3.798     & 71.4           &        \\ \hline
{\ct SULFUR HEXAFLUORIDE}&146   & 5.128     &146.            &        \\ \hline
{\ct TOLUENE}            & 92   & 5.698     &480.0           &    Y   \\ \hline
{\ct WATER VAPOR}        & 18   & 2.641     &809.1           &    Y   \\ \hline
\end{tabular}
\end{center}
\end{table}
...
The optional parameter {\ct HUMIDTY} can be specified with the species {\ct WATER VAPOR}.  {\ct HUMIDITY} is the relative humidity of water vapor in \% and is 40 \% by default.
...
units of kJ/kmol.
\item[{\ct NU}] Array containing the stoichiometry of the chemical reaction for each {\ct SPEC} where negative values indicate reactants and positive values indicate products.  The background species can be specified as NU(0).
...
\item[{\ct SPEC\_ID}] Array containing the names of the species that participate in the reaction. Note that it is possible that a given {\ct SPEC} can be assigned a value of {\ct N\_S} greater than zero and a value of {\ct NU} equal to zero. In other words,
...
\subsection{Special Topic: Mixing Controlled Combustion}
\label{info:mixing}

In a typical simulation the mixing controlled combustion model is used in conjunction with simple chemistry as described in \ref{info:REAC}.  The mixing controlled combustion model can also be used with user defined species.  The inputs for the mixing controlled combustion model are:
...
In a typical calculation, flame temperatures are not likely to be resolved well enough to allow for a finte rate reaction to be computed.  However, the temperatures in a layer environment are likely to support a finite rate reaction for slow reactions (e.g. carbon monoxide oxidation in a hot layer).  A mixed-mode combustion model can be used for this situation.  To enable this model, specify the finite rate reaction inputs described in the previous section as well as the input parameter {\ct THRESHOLD\_TEMPERATURE} and .  During a simluation, if there is heat release from other reactions, then a mixed-mode reaction will be evaluated as mixing controlled reaction.  If there is no heat release from other reactions and the local temperature exceeds the {\ct THRESHOLD\_TEMPERATURE}, then the mixed-mode reaction will be evaluated as a finte rate reaction.
...
\item[{\ct HEAT\_OF\_COMBUSTION}] The effective heat of combustion of the {\ct FUEL} in chemical reaction in units of kJ/kg. (No default)
\item[{\ct THRESHOLD\_TEMPERATURE}] The temperature at which finite rate combustion will be computed when there is no heat release from other reactions. (Default 0.0 K)
\end{description}

\section{Species Mixtures (Lumped Species): The \texorpdfstring{{\tt SMIX}}{SMIX} Namelist Group}
\label{info:SMIX}
...
\footnotesize
\begin{verbatim}
&SPEC ID='NITROGEN', LUMPED_SPECIES_ONLY=.TRUE. /
&SPEC ID='OXYGEN', LUMPED_SPECIES_ONLY=.TRUE./
&SPEC ID='WATER VAPOR', LUMPED_SPECIES_ONLY=.TRUE. /
&SPEC ID='CARBON DIOXIDE', LUMPED_SPECIES_ONLY=.TRUE. /
...
\noindent
The keyword {\ct LUMPED\_SPECIES\_ONLY} denotes that the species is only present as part of a lumped species.  
...
and any remaining lumped species will be named LUMPED SPECIES N where N is the position of the the input following any background lumped species.
label the {\ct SPEC\_ID} Character array containing the names of the species that make up the lumped species.  This name should not unique over all {\ct SMIX} and {\ct SPEC} inputs.
\item[{\ct MASS\_FRACTION}] The mass fractions of the species making up the lumped species.  FDS will normalize the values to 1.  
Specify only one of {\ct MASS\_FRACTION} or {\ct VOLUME\_FRACTION}.
\item[{\ct VOLUME\_FRACTION}]  The volume fractions of the species making up the lumped species.  FDS will normalize the values to 1.  
Specify only one of {\ct MASS\_FRACTION} or {\ct VOLUME\_FRACTION}.
\end{description}

\noindent Any FDS input that requires a {\ct SPEC\_ID} can also use the {\ct ID} of an {\ct SMIX}.

The addition of the {\ct SMIX} namelist group means that there are three ways in which a simple, mixing controlled reaction could be defined.  The following three sets on inputs are equivalent:


\footnotesize
\begin{verbatim}
&REAC C=1,H=4,HEAT_OF_COMBUSTION=50000./
\end{verbatim} \normalsize
...
&SPEC_ID='METHANE'/
&REAC FUEL='METHANE',SPEC_ID='METHANE','OXYGEN','CARBON DIOXIDE','WATER VAPOR',NU=-1,-2,1,2,HEAT_OF_COMBUSTION=50000./
\end{verbatim} \normalsize
...
&SPEC ID='CARBON DIOXIDE', LUMPED_SPECIES_ONLY=.TRUE. /
&SPEC ID='METHANE'/
&SMIX ID='AIR',BACKGROUND=.TRUE.,SPEC_ID='NITROGEN','OXYGEN','CARBON DIOXIDE','WATER VAPOR', MASS_FRACTION=0.75778,0.21312,0.00577,0.00524/
&SMIX ID='PRODUCTS',SPEC_ID='NITROGEN','CARBON DIOXIDE','WATER VAPOR', MASS_FRACTION=0.71637,0.15572,0.12791/
&REAC FUEL='METHANE',SPEC_ID='METHANE','AIR','PRODUCTS',NU=-1,-9.60825,10.60825,HEAT_OF_COMBUSTION=50000./
\end{verbatim} \normalsize

Note the stoichiometry for the last case.  It is based upon moles of the lumped species and not moles of the primitive species.  Since the lumped species for {\ct AIR} is 21 \% oxygen, taking 21 \% of the 9.60825 moles of {\ct AIR} results in 2 moles of oxygen. 
...
There are three ways of introducing droplets or particles into a simulation. The first is to define a sprinkler or
nozzle using a {\ct PROP} line that includes a {\ct PART\_ID} that specifies the droplet parameters. The second way to
introduce particles or droplets is to
add a {\ct PART\_ID} to a {\ct SURF} line, in which case particles or droplets will be ejected from that surface. Note that
this only works if the surface has a normal velocity pointing into the flow domain. The third way to introduce droplets or
particles is via an {\ct INIT} that defines a volume within the computational domain in which the particles/droplets are
...
Sometimes it is convenient to introduce droplets or particles at the start of the simulation. For this purpose, add {\ct NUMBER\_INITIAL\_DROPLETS}
to the {\ct INIT} line\footnote{Prior to FDS~5.5, it was possible to use the {\ct PART} line to specify {\ct NUMBER\_INITIAL\_DROPLETS}.}
to indicate the number of particles/droplets within the computational domain at the start of the simulation.
...
\noindent
If you want to introduce droplets or particles within a given volume periodically in time and not just at the initial time, set {\ct DT\_INSERT} on the {\ct INIT} line
to a positive value
indicating the time increment (s) for insertion. The parameter {\ct NUMBER\_INITIAL\_DROPLETS} now indicates the number of droplets/particles inserted every
{\ct DT\_INSERT} seconds. If the droplets/particles have mass, use {\ct MASS\_PER\_TIME} (kg/s) instead of {\ct MASS\_PER\_VOLUME} to indicate how much mass is to be
introduced per second.
...
originate at a surface, a sprinkler or nozzle, or a volume. Versions prior to FDS~5.5 had this parameter listed on the
{\ct PART} line. Note that if you introduce particles or droplets via an {\ct INIT} line, you must specify a value for {\ct DT\_INSERT}. Otherwise, particles or
...
\end{description}
Notice that the default {\ct DENSITY} is that of water.  If the drops are specified as {\ct WATER} or the drops are specified asL {\ct FUEL} and the {\ct FUEL} species is shown as a liquid in Table~\ref{tab:gasspecies}, then only the {\ct DENSITY} needs to be provided.
...
If the droplets are defined with {\ct SPEC\_ID='WATER VAPOR'}, then the particles will be assigned the thermophysical properties of water, the radiation absorption properties of water, and will be colored blue in Smokeview.
...
If the droplets evaporate into the {\ct FUEL} identified on {\ct REAC} then they will be colored yellow by default in Smokeview and any resulting fuel vapor will burn according to the combustion model specified on the {\ct REAC} line.  The droplets evaporate into an equivalent amount of fuel vapor such that the resulting heat release rate (assuming complete combustion) is equal to the evaporation rate multiplied by the {\ct HEAT\_OF\_COMBUSTION}.
...
The accumulated water is tracked using a device measuring the accumulated mass per unit area, integrated over the total floor area.
The total mass of accumulated water should increase from zero to 10 kg in 60 s. A comparison of the FDS prediction and this analytical result is
shown in the right side of Figure~\ref{flow_rate}. The small delay of the FDS result is caused by the time it takes from the droplets to fall down on the
floor.
...
\be
f(t) = \frac{1}{\min(t,t_e)-t_s} \int_{t_s}^{\min(t,t_e)} \left( \frac{\sum_i n_i d_i^m x}{\sum_i n_i d_i^n}\right)^{1/(m-n)}  dt
\ee
where $x$ is the quantity to be measured, or just one in case of mean diameters.
The summation goes over all the particles within a sphere with radius {\ct PDPA\_RADIUS} and centered at
...
\label{mtr}
The \emph{measure of turbulence resolution} is defined locally as
\begin{eqnarray}
...
Here we are interested in using the wavelet analysis to say something about the local level of error in grid resolution.  Very simply, we ask what can be discerned from a sample of four data points along a line.  Roughly speaking we might expect to see one of the four scenarios depicted in Figure \ref{fig_wavelet_transforms}.  Within each plot window we also show the results of a Haar wavelet transform for that signal.  Looking first at the two top plots, on the left we have a step function and on the right we have a straight line.  Intuitively, we expect large error for the step function and small error for the line.  The following error measure achieves this goal:
\vskip-.6cm
\begin{eqnarray}
...
Note that you may use the character string {\ct FYI} on any namelist line to make a note or comment.
...
{\ct RECOUNT\_DRIP}     & Logical     & See Section \ref{info:part_output}      &           & {\ct .FALSE.}   \\ \hline
...
{\ct CTRL\_COLUMN\_LIMIT}           & Integer      & No.~columns in {\ct CTRL} file    &           & 254                            \\ \hline
{\ct DEVC\_COLUMN\_LIMIT}           & Integer      & No.~columns in {\ct DEVC} file    &           & 254                            \\ \hline
{\ct DEBUG}                         & Logical      & Flag to monitor MPI calls         &           & {\ct .FALSE.}                  \\ \hline
...
{\ct DT\_SLCF}                      & Real         & 2-D slice dump interval           &  s        & $\Delta t${\ct /NFRAMES}       \\ \hline
{\ct DT\_SL3D}                      & Real         & 3-D slice dump interval           &  s        & $\Delta t${\ct /5}             \\ \hline
{\ct DT\_VEG}                       & Real         & Vegetation dump interval          &  s        & $\Delta t${\ct /NFRAMES}       \\ \hline
...
{\ct TIMING}                        & Logical      & Flag for timing MPI exchanges     &           & {\ct .FALSE.}                  \\ \hline
{\ct VELOCITY\_ERROR\_FILE}         & Logical      & Diagnostic file for pressure      &           & {\ct .FALSE.}                  \\ \hline
...
{\ct SPEC\_ID}                  & Character         & {\ct ID}s of species in {\ct MASS\_FRACTION}          &               &               \\ \hline
...
{\ct BAROCLINIC}                & Logical       & Baroclinic torque correction                  &               & {\ct .TRUE.}     \\ \hline
...
{\ct OVERWRITE}                 & Logical       & Prevevts overwriting of output files               &               & {\ct .TRUE.}      \\ \hline
...
%{\ct PROP\_ID}            & Character           & Name of Property line (if needed)     &    &               \\ \hline
...
{\ct CTRL\_ID}                      & Character       & Name of controller                      &           &               \\ \hline
{\ct DENSITY}                       & Real            & Droplet density                         & kg/m$^3$  & 1000.         \\ \hline
{\ct DEVC\_ID}                      & Character       & Name of controlling device              &           &               \\ \hline
{\ct DIAMETER}                      & Real            & Median Volumetric Diameter              & $\mu$m    & 500.          \\ \hline
{\ct DRAG\_LAW}                     & Character       & Geometry for drag correlation           &           & {\ct 'SPHERE'}\\ \hline
...
{\ct MELTING\_TEMPERATURE}          & Real            & Melting Temperature                     & $^\circ$C &               \\ \hline
{\ct MONODISPERSE}                  & Logical         & Uniform droplet size                    &           & {\ct .FALSE.} \\ \hline
{\ct PROP\_ID}                      & Character       & Name of Property line                   &           &               \\ \hline
...
{\ct SPECIFIC\_HEAT}                & Real            & Droplet specific heat                   & kJ/kg/K   &               \\ \hline
{\ct STATIC}                        & Logical         & Stationary Particles                    &           & {\ct .FALSE.} \\ \hline
%{\ct TREE}                          & Logical         & Particles are leaves                    &           & {\ct .FALSE.} \\ \hline
{\ct USER\_DRAG\_COEFFICIENT}       & Real            & Constant drag coefficient               &           & -1.           \\ \hline
...
{\ct DT\_INSERT}                        & Real          & Time between particle insertions          & s                     & 0.01      \\ \hline
...
{\ct K\_FACTOR}                         & Real          & Flow parameter                            & L/min/bar$^{1/2}$     & 1.        \\ \hline
{\ct LENGTH}                            & Real          & Smoke detector parameter                  &                       & 1.8       \\ \hline
{\ct OFFSET}                            & Real          & Droplet offset distance                   & m                     & 0.05      \\ \hline
{\ct OPERATING\_PRESSURE}               & Real          & Sprinkler pipe pressure                   & bar                   & 1.        \\ \hline
...
{\ct PRESSURE\_RAMP}                    & Character     & Pressure {\ct RAMP} for sprinklers        &                       &           \\ \hline
{\ct QUANTITY}                          & Character     & Name of associated output                 &                       &           \\ \hline
{\ct RTI}                               & Real          & Response Time Index                       & $\sqrt{\hbox{m~s}}$   & 100.      \\ \hline
{\ct SMOKEVIEW\_ID}                     & Char.~Array   & Name(s) of drawn object                   &                       &           \\ \hline
{\ct SMOKEVIEW\_PARAMETERS}             & Char.~Array   & Misc.~parameters for drawing              &                       &           \\ \hline
...
{\ct FUEL}                                & Character   & Name of Fuel (Finite Rate)                    &                   & {\ct ETHYLENE}\\ \hline
...
{\ct BACKGROUND}                    & Logical     & Background species                          &                   & {\ct .FALSE.} \\ \hline
{\ct BACKGROUND\_ONLY}              & Logical     & Species is only background                  &                   & {\ct .FALSE.} \\ \hline
...
{\ct FORMULA }                      & Character   & Smokeview label                             &                   &               \\ \hline
{\ct HUMIDITY}                      & Real        & Relative Humidity                           & \%                & 40.           \\ \hline
{\ct ID }                           & Character   & Name of species                             &                   &               \\ \hline
{\ct LUMPED\_SPECIES\_ONLY}         & Logical     & See Section~\ref{info:SMIX}                 &                   & {\ct .FALSE.} \\ \hline
...
{\ct EMISSIVITY}                      & Real            & Emissivity (front)                &                     & 0.9                     \\ \hline
{\ct EMISSIVITY\_BACK}                & Real            & Emissivity (back)                 &                     &                         \\ \hline
...
{\ct RAMP\_EF}                        & Character       & Ramp {\ct ID} for external flux   &                     &                         \\ \hline
{\ct RAMP\_MF(I)}                     & Character       & Ramp {\ct ID} for species {\ct I} &                     &                         \\ \hline
...
{\ct ROUGHNESS}                       & Real            & Wall roughness height             & m                   & 0.                      \\ \hline
{\ct SPEC\_ID}                        & Character       & Species Identifier                &                     &                         \\ \hline
...
{\ct TAU\_EF}                         & Real            & Ramp time for external flux       & s                   & 1.                      \\ \hline
{\ct TAU\_MF(I)}                      & Real            & Ramp time for species {\ct I}     & s                   & 1.                      \\ \hline
...
{\ct TMP\_INNER(IL)}                  & Real Array      & Initial temperature of layer {\ct IL}& $^\circ$C        & 20.                     \\ \hline
...
\label{tbl:Changes} \\
\hline
FDS 5 Parameter & Namelist  & FDS 6 Parameter & Namelist  & Notes \\
\hline
\endfirsthead
\caption[]{Continued} \\
\hline
...
\hline
\endhead
{\ct MAXIMUM\_MASS\_FRACTION}   & {\ct CLIP}    & Eliminated                                  &                     &                                              \\ \hline
{\ct MINIMUM\_MASS\_FRACTION}   & {\ct CLIP}    & Eliminated                                  &                     &                                              \\ \hline
{\ct NU\_FUEL}                  & {\ct MATL}    & {\ct NU\_SPEC} +  {\ct SPEC\_ID}            &   {\ct MATL}        & See Section~\ref{info:solid_pyrolysis}       \\ \hline
{\ct NU\_GAS}                   & {\ct MATL}    & {\ct NU\_SPEC} +  {\ct SPEC\_ID}            &   {\ct MATL}        & See Section~\ref{info:solid_pyrolysis}       \\ \hline
{\ct NU\_RESIDUE}               & {\ct MATL}    & {\ct NU\_MATL}                              &   {\ct MATL}        & See Section~\ref{info:solid_pyrolysis}       \\ \hline
{\ct NU\_WATER}                 & {\ct MATL}    & {\ct NU\_SPEC} +  {\ct SPEC\_ID}            &   {\ct MATL}        & See Section~\ref{info:solid_pyrolysis}       \\ \hline
{\ct RESIDUE}                   & {\ct MATL}    & {\ct MATL\_ID}                              &   {\ct MATL}        & See Section~\ref{info:solid_pyrolysis}       \\ \hline
{\ct STATE\_FILE}               & {\ct DUMP}    & Eliminated                                  &                     &                                              \\ \hline
\end{longtable}
