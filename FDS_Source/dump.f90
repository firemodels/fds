...
DO N=1,M%N_SLCF
   ...
   ! write out slice file info to the .smv file
      ...
      ENDIF
      IF (SL%GEOMETRY)THEN
         WRITE(M%STRING(M%N_STRINGS),'(A,I6,A)') 'BNDE',NM,TRIM(SLICELABEL)
      ELSE
...
DO NM=1,NMESHES
   ...
   IF (M%Q_MAX>TWO_EPSILON_EB) WRITE(LU_OUTPUT,'(7X,A,E9.2,A)') 'Max HRRPUV: ',M%Q_MAX/1000._EB,  ' kW/m^3'
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IS%INDEX,0,IS%Y_INDEX,IS%Z_INDEX,0,IS%VELO_INDEX,0,0,0,T,NM)
         ENDDO
      ENDDO
   ENDDO
      ...
      CASE(1)
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
                  FF(I,J,K) = RHO(I,J,K)*&
                       GAS_PHASE_OUTPUT(I,J,K,SMOKE3D_QUANTITY_INDEX,0,SMOKE3D_Y_INDEX,SMOKE3D_Z_INDEX,0,0,0,0,0,T,NM)
               ENDDO
            ENDDO
         ENDDO
      CASE(2)
...
SUBROUTINE GETSLICEDIR(I1,I2,J1,J2,K1,K2,DIR,ISLICE)
INTEGER, INTENT(IN) :: I1, I2, J1, J2, K1, K2
INTEGER, INTENT(OUT) :: DIR, ISLICE
...
INTEGER :: NVERTS, NFACES, NI, NJ, NK
REAL(FB), PARAMETER :: ZERO=0.0_FB
...
IF(NVERTS>0.AND.NFACES>0)THEN
   IF(DIR==1)THEN
      WRITE(FUNIT)((( XPLT(I),YPLT(J),ZPLT(K),I=SLICE,SLICE),J=J1,J2),K=K1,K2 )
      WRITE(FUNIT)(( IJK(J,K,NJ),IJK(J+1,K,NJ),IJK(J+1,K+1,NJ),IJK(J,K,NJ),IJK(J+1,K+1,NJ),IJK(J,K+1,NJ),J=1,NJ-1),K=1,NK-1 )
   ELSE IF(DIR==2)THEN
      WRITE(FUNIT)((( XPLT(I),YPLT(J),ZPLT(K),I=I1,I2),J=SLICE,SLICE),K=K1,K2 )
      WRITE(FUNIT)(( IJK(I,K,NI),IJK(I+1,K,NI),IJK(I+1,K+1,NI),IJK(I,K,NI),IJK(I+1,K+1,NI),IJK(I,K+1,NI),I=1,NI-1),K=1,NK-1 )
   ELSE
      WRITE(FUNIT)((( XPLT(I),YPLT(J),ZPLT(K),I=I1,I2),J=J1,J2),K=SLICE,SLICE )
      WRITE(FUNIT)(( IJK(I,J,NI),IJK(I+1,J,NI),IJK(I+1,J+1,NI),IJK(I,J,NI),IJK(I+1,J+1,NI),IJK(I,J+1,NI),I=1,NI-1),J=1,NJ-1 )
   ENDIF
  WRITE(FUNIT) (ZERO_INTEGER,I=1,NFACES)   ! placeholders for now
  WRITE(FUNIT) (ZERO,ZERO,   I=1,3*NFACES) ! placeholders for now
ENDIF
...
SUBROUTINE DUMP_SLICE_GEOM_DATA(FUNIT,HEADER,STIME,I1,I2,J1,J2,K1,K2)
INTEGER, INTENT(IN) :: FUNIT, HEADER, I1, I2, J1, J2, K1, K2
REAL(FB), INTENT(IN) :: STIME

INTEGER :: DIR, SLICE
INTEGER, PARAMETER :: ONE_INTEGER=1, ZERO_INTEGER=0, VERSION=2
INTEGER :: NVERTS, NI, NJ, NK
INTEGER I, J, K
...
ENDIF
WRITE(FUNIT) STIME
WRITE(FUNIT) ZERO_INTEGER, ZERO_INTEGER, NVERTS, ZERO_INTEGER
IF (NVERTS>0) THEN
   IF(DIR==1)THEN
      WRITE(FUNIT) (((QQ(I,J,K,1),I=SLICE,SLICE),J=J1,J2),K=K1,K2)
   ELSE IF(DIR==2)THEN
      WRITE(FUNIT) (((QQ(I,J,K,1),I=I1,I2),J=SLICE,SLICE),K=K1,K2)
   ELSE
      WRITE(FUNIT) (((QQ(I,J,K,1),I=I1,I2),J=J1,J2),K=SLICE,SLICE)
   ENDIF
ENDIF
...
QUANTITY_LOOP: DO IQ=1,NQT
      ...
      DO I=II1,II2
         DO J=JJ1,JJ2
            DO K=KK1,KK2
               ...
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,KTS,IND,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ENDIF
   ...
   IF (.NOT.PLOT3D) THEN
      SL => SLICE(IQ)
      IF (.NOT. SL%GEOMETRY) THEN ! write out slice file using original slice file format
         STIME = T_BEGIN + (T-T_BEGIN)*TIME_SHRINK_FACTOR
         ...
         GAS_STATS_SELECT: SELECT CASE(DV%STATISTICS)
            ...
            CASE('null','RMS','TIME MIN','TIME MAX') GAS_STATS_SELECT
               ...
               VALUE = GAS_PHASE_OUTPUT(DV%I,DV%J,DV%K,DV%OUTPUT_INDEX,0,DV%Y_INDEX,DV%Z_INDEX,DV%PART_INDEX,DV%VELO_INDEX,&
                                        DV%PIPE_INDEX,DV%PROP_INDEX,DV%REAC_INDEX,T,NM)
                  ...
                  J_DEVICE_CELL_LOOP: DO J=DV%J1,DV%J2
                     I_DEVICE_CELL_LOOP: DO I=DV%I1,DV%I2
                        ...
                        VALUE = GAS_PHASE_OUTPUT(I,J,K,DV%OUTPUT_INDEX,0,DV%Y_INDEX,DV%Z_INDEX,& 
                                                 DV%PART_INDEX,DV%VELO_INDEX,DV%PIPE_INDEX,DV%PROP_INDEX,DV%REAC_INDEX,T,NM)
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,MU_G,&
            ...
            UVW,UODX,VODY,WODZ,MW,XHAT,ZHAT
...
!       GAS_PHASE_OUTPUT_RES = MEASURE_SCALAR_RESOLUTION(II,JJ,KK,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,NM)
   ...
   CASE(85)  ! KOLMOGOROV LENGTH SCALE
      SS = GAS_PHASE_OUTPUT(II,JJ,KK,84,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,T,NM)
   ...
   CASE(86)  ! CELL REYNOLDS NUMBER
      ...
      KKK = MAX(1,MIN(KK,KBAR))
      DELTA = LES_FILTER_WIDTH_FUNCTION(DX(III),DY(JJJ),DZ(KKK))
      ETA = GAS_PHASE_OUTPUT(II,JJ,KK,85,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,REAC_INDEX,T,NM)
   ...
   CASE(110)  ! THERMOCOUPLE
      ...
      IF (PY%BEAD_H_FIXED<0._EB) THEN
         ...
         RE_D    = RHO(II,JJ,KK)*SQRT(VEL2)*PY%BEAD_DIAMETER/MU_G
         NUSSELT = 2._EB + 0.6_EB*SQRT(RE_D)*PR_ONTH ! Incropera and DeWitt
         H_TC    = NUSSELT*K_G/PY%BEAD_DIAMETER
      ELSE
...
REAL(EB) :: CONCORR,FLUX,DFLUXDT,SOLID_PHASE_OUTPUT_OLD,VOLSUM,C_S_ADJUST_UNITS,MFT,&
            ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,KSGS,DEPTH,UN,H_S,EMISSIVITY
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(13) ! GAUGE HEAT FLUX
      ...
      IF (PY%GAUGE_EMISSIVITY>=0._EB) THEN
         EMISSIVITY = PY%GAUGE_EMISSIVITY
      ELSE
         EMISSIVITY = ONE_D%EMISSIVITY
      ENDIF
      SOLID_PHASE_OUTPUT = (ONE_D%QRADIN/(EMISSIVITY+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4 + &
   ...
   CASE(22) ! RADIOMETER
      ...
      ELSE
         EMISSIVITY = ONE_D%EMISSIVITY
      ENDIF
      SOLID_PHASE_OUTPUT = ( ONE_D%QRADIN/(EMISSIVITY+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4 )*0.001_EB
...
SS(1) = GAS_PHASE_OUTPUT(MAX(0,II-2),JJ,KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
SS(2) = GAS_PHASE_OUTPUT(MAX(0,II-1),JJ,KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
SS(3) = GAS_PHASE_OUTPUT(II,JJ,KK,                       IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
SS(4) = GAS_PHASE_OUTPUT(MIN(MESHES(NM)%IBP1,II+1),JJ,KK,IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
WAVELET_ERROR_MEASURE = WAVELET_ERROR(SS)

IF (.NOT.TWO_D) THEN
   SS(1) = GAS_PHASE_OUTPUT(II,MAX(0,JJ-2),KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
   SS(2) = GAS_PHASE_OUTPUT(II,MAX(0,JJ-1),KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
   SS(3) = GAS_PHASE_OUTPUT(II,JJ,KK,                       IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
   SS(4) = GAS_PHASE_OUTPUT(II,MIN(MESHES(NM)%JBP1,JJ+1),KK,IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
   WAVELET_ERROR_MEASURE = MAX(WAVELET_ERROR_MEASURE,WAVELET_ERROR(SS))
ENDIF

SS(1) = GAS_PHASE_OUTPUT(II,JJ,MAX(0,KK-2),              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
SS(2) = GAS_PHASE_OUTPUT(II,JJ,MAX(0,KK-1),              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
SS(3) = GAS_PHASE_OUTPUT(II,JJ,KK,                       IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
SS(4) = GAS_PHASE_OUTPUT(II,JJ,MIN(MESHES(NM)%KBP1,KK+1),IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0,0._EB,NM)
WAVELET_ERROR_MEASURE = MAX(WAVELET_ERROR_MEASURE,WAVELET_ERROR(SS))
