...
EVAC_ONLY5: IF (ANY(EVACUATION_GRID)) THEN
   DO N=1,N_DOORS
      ...
      ! ZZ = 0.5_EB*(EVAC_DOORS(N)%Z1+EVAC_DOORS(N)%Z2)
      ZZ = EVAC_DOORS(N)%Z
      WRITE(LU_SMV,'(/A)') 'DEVICE'
      WRITE(LU_SMV,'(A)') 'testdoor'
      WRITE(LU_SMV,'(6F12.5,2I2)') XX,YY,ZZ,EVAC_DOORS(N)%ORIENTATION(1:3),0,6
      WRITE(LU_SMV,'(3F6.2,3F6.3)') EVAC_DOORS(N)%WIDTH, 0.1, EVAC_DOORS(N)%HEIGHT, EVAC_DOORS(N)%RGB(1:3)/255.0_EB
   ENDDO
   DO N=1,N_EXITS
      ...
      ! ZZ = 0.5_EB*(EVAC_EXITS(N)%Z1+EVAC_EXITS(N)%Z2)
      ZZ = EVAC_EXITS(N)%Z
      WRITE(LU_SMV,'(/A)') 'DEVICE'
      WRITE(LU_SMV,'(A)') 'testdoor'
      ! WRITE(LU_SMV,'(A)') 'testexit'
      WRITE(LU_SMV,'(6F12.5,2I2)') XX,YY,ZZ,EVAC_EXITS(N)%ORIENTATION(1:3),0,6
      WRITE(LU_SMV,'(3F6.2,3F6.3)') EVAC_EXITS(N)%WIDTH, 0.1, EVAC_EXITS(N)%HEIGHT, EVAC_EXITS(N)%RGB(1:3)/255.0_EB
   ENDDO
   DO N=1,N_ENTRYS
      ...
      WRITE(LU_SMV,'(/A)') 'DEVICE'
      WRITE(LU_SMV,'(A)') 'testdoor'
      ! WRITE(LU_SMV,'(A)') 'testentr'
      WRITE(LU_SMV,'(6F12.5,2I2)') XX,YY,ZZ,EVAC_ENTRYS(N)%ORIENTATION(1:3),0,6
      WRITE(LU_SMV,'(3F6.2,3F6.3)') EVAC_ENTRYS(N)%WIDTH, 0.1, EVAC_ENTRYS(N)%HEIGHT, EVAC_ENTRYS(N)%RGB(1:3)/255.0_EB
   ENDDO
END IF EVAC_ONLY5
...
!  EVAC_ONLY6: IF (EVACUATION_GRID(NM) .AND. T==T_BEGIN) THEN
   I=0  ! Counter for evacuation devices
   DO N=1,N_DOORS
      IF (.NOT.EVAC_DOORS(N)%SHOW) CYCLE
      I=I+1
