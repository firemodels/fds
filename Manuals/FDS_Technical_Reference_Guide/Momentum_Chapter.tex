...
\be \mu_t = \mu + \rho \, (C_s\, \Delta)^2 \,
   \left(2 \; \overline{\bS}_{ij} : \overline{\bS}_{ij} - \frac{2}{3} (\nabla\!\cdot \overline{\bu})^2 \right)^\ha \label{constant_coef_LES}
\ee
where $\mu$ is the actual molecular viscosity of the fluid, $C_s$ is an empirical constant, and $\Delta$ is a length on the
...
\be 
  \mu_t = \mu + \rho \, C \, \Delta \, \sqrt{k_{\hbox{\tiny sgs}} } \quad ; \quad  
...
\be
   \hat{u} = \frac{u_{ijk}}{2} + \frac{u_{i-1,jk} + u_{i+1,jk} }{4}
\ee
The terms $\hat{v}$ and $\hat{w}$ are defined similarly.
...
The mixing length increases with distance from the wall.  Van Driest (see \cite{Wilcox:1}) proposed the following functionality:
\begin{equation}
\ell_{mix} = \kappa \,y \left[ 1 - e^{-y^+/A} \right]
\end{equation}
where $\kappa = 0.41$ is the von K\'arm\'an constant and $A=26$.

In FDS, the turbulent viscosity of the first off-wall cell is interpolated between $\nu_t(y=0)=0$ at the wall and $\nu_t(y = 3\delta y/2)$ from the second off-wall cell.  The interpolation function is based on the Van Driest mixing length with $y^+ = (\delta y/2)/\delta_\nu$.  Considering that the viscosity scales with $\ell_{mix}^{\,2}$\,, we obtain
\begin{equation}
\nu_t(\delta y/2) = \nu_t(3\delta y/2) \left[ \frac{(1 - e^{-y^+/A}) }{3 ( 1 - e^{-3y^+/A}) } \right]^2
\end{equation}
In practice the exponentials are evaluated using truncated Taylor series.
