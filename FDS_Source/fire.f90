...
INTEGRATION_LOOP: DO TIME_ITER = 1,TIME_ITER_MAX
   ...
   ELSE RK2_IF ! RK2 w/ Richardson
      ...
      RICH_EX_LOOP: DO RICH_ITER =1,RICH_ITER_MAX   
         ...
         ODE_LOOP1: DO NS = 1, SUB_DT1
            ...
            A1 = A1 + DZZDT1_2*DT_SUB 
            A1 = 0.5_EB*(ZZ_0 + A1)
            IF (SUM(A1-ZZ_GET) > TWO_EPSILON_EB) CALL SHUTDOWN('ERROR: Error in A1 Reaction Species')                               
         ENDDO ODE_LOOP1   
         !--------------------
         ! Calculate A2 term
         ! Time step = DT_SUB/2
         ...
         ODE_LOOP2: DO NS = 1, SUB_DT2
            ...
            A2 = 0.5_EB*(ZZ_0 + A2) 
            IF (SUM(A2-ZZ_GET) > TWO_EPSILON_EB) CALL SHUTDOWN('ERROR: Error in A2 Reaction Species')               
            ZZ_0 = A2
         ENDDO ODE_LOOP2     
         !--------------------
         ! Calculate A4 term  
         ...
         ODE_LOOP4: DO NS = 1, SUB_DT4
            ...
            A4 = 0.5_EB*(ZZ_0 + A4) 
            IF (SUM(A4-ZZ_GET) > TWO_EPSILON_EB) CALL SHUTDOWN('ERROR: Error A4 in Reaction Species')             
            ZZ_0 = A4
         ENDDO ODE_LOOP4
   ...
   ENDIF
   ZZ_GET =  ZETA1*ZZ_GET_0 + (1-ZETA1)*ZZ_MIXED
...
IF (RN%A_RAMP /= 'null' .OR. RN%E_RAMP /= 'null') THEN
   EQ_RATIO = 0._EB
   EQ_RATIO = (MASS_OX_STOICH+TWO_EPSILON_EB)/MASS_OX
