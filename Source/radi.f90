...
IF (UPDATE_INTENSITY) THEN
   DO IW=1,N_INTERNAL_WALL_CELLS+N_EXTERNAL_WALL_CELLS
      ...
      WALL(IW)%ONE_D%Q_RAD_OUT = 0._EB
   ENDDO
   DO IP=1,NLP
      LAGRANGIAN_PARTICLE(IP)%ONE_D%Q_RAD_IN = 0._EB
      LAGRANGIAN_PARTICLE(IP)%ONE_D%Q_RAD_OUT = 0._EB
   ENDDO
   DO ICF=1,N_CFACE_CELLS
      CFACE(ICF)%ONE_D%Q_RAD_IN = 0._EB
      CFACE(ICF)%ONE_D%Q_RAD_OUT = 0._EB      
   ENDDO
ENDIF
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         IF (WALL(IW)%BOUNDARY_TYPE == OPEN_BOUNDARY) THEN
            BBF = BBFA
            OUTRAD_W(IW) = BBF*RPI*SIGMA*TMPA**4
         ELSE
      ...
      DO ICF = 1,N_CFACE_CELLS
         ...
         DUMMY = CFACE(ICF)%ONE_D%EMISSIVITY*SIGMA*CFACE(ICF)%ONE_D%TMP_F**4
         IF (.NOT. SF%INTERNAL_RADIATION) CFACE(ICF)%ONE_D%Q_RAD_OUT = CFACE(ICF)%ONE_D%Q_RAD_OUT + BBF*DUMMY
         OUTRAD_F(ICF) = BBF*RPI*DUMMY
      ENDDO
