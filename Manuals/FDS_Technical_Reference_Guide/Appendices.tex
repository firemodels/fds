...
\end{eqnarray}
Hence, utilizing (\ref{eqn_fickslaw}), (\ref{eqn_heatrelease}), and (\ref{eqn_expandp0}), for the $i$th zone we may write the divergence (\ref{eqn_divconstraint_final}) as
\begin{equation}
...
\begin{equation}
\label{eqn_fdsP}
\mathcal{P} = \frac{1}{\overline{p}_i}\left( 1 - \frac{\overline{p}_i}{\rho \,c_p T} \right)
\end{equation}
and
\begin{eqnarray}
\label{eqn_fdsD}
...
&+& \frac{1}{\rho}\sum_{n=1}^{N_z} \left[\overline{W}\left(\frac{1}{W_n} - \frac{1}{W_0}\right) - \frac{(h_{s,n}-h_{s,0})}{c_p T} \right] \Div \rho D_\alpha \nabla Y_\alpha \nonumber\\ [.3cm]
&+& \underbrace{\frac{1}{\rho}\sum_{n=1}^{N_z} \left[\overline{W}\left(\frac{1}{W_n} - \frac{1}{W_0}\right) - \frac{(h_{s,n}-h_{s,0})}{c_p T} \right] \dot{m}_{\alpha}^\tripleprime}_{\mbox{\ct D\_REACTION}} \nonumber\\ [.3cm]
&+& \underbrace{\frac{\dot{m}_b^\tripleprime}{\rho}\frac{\overline{W}}{\overline{W}_b} + \frac{1}{\rho c_p T} \left[ - \dot{q}_b^{\tripleprime} + \dot{m}_b^{\tripleprime} \sum_\alpha Y_{b,\alpha} c_{p,\alpha}(T_b-T) + \frac{\dot{m}_b^{\tripleprime}}{2}\left|\mathbf{u}_b-\mathbf{u}\right|^2  \right]}_{\mbox{\ct D\_LAGRANGIAN}} \nonumber\\ [.3cm]
&+& \mathcal{P} w \rho_i g
\end{eqnarray}
...
A brief remark on the of the sensible enthalpy $h_{s,\alpha}$ versus the [chemical + sensible] enthalpy, $h_\alpha = \Delta h_\alpha^0 + h_{s,\alpha}$.
