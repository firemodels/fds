...
! when complete copy BOXTETRA_ROUTINES module to FDS
...
PUBLIC VERIFY_TETRABOX,GET_TETRABOX_VOLUME, GET_VERTS, VOLUME_VERTS
...
SUBROUTINE GET_VERTS(BOX_BOUNDS,V0,V1,V2,V3,VERTS,NVERTS,FACESTART,FACENUM,NFACES)
...
!  add intersection of tetrahedron edge and box plane (if on box)
   ...
   DO TE = 0, 5 ! tetra edge
      ...
      IF ( PLANE_EDGE_INTERSECTION( BOXVERT,BOXNORMAL,TETRAVERT0,TETRAVERT1,VERT).EQ.1) THEN
         IF (IN_BOX(VERT, BP).EQ.1) THEN
...
REAL(EB), DIMENSION(0:5) :: BOXBOUNDS
INTEGER :: COUNT
...
DO J0 = 0, N-1
DO K0 = 0, N-1
V0(0) = 0.01_EB + REAL(I0,EB)*0.48_EB/REAL(N-1,EB)
V0(1) = 0.01_EB + REAL(J0,EB)*0.48_EB/REAL(N-1,EB)
V0(2) = 0.01_EB + REAL(K0,EB)*0.48_EB/REAL(N-1,EB)
DO I1 = 0, N-1
DO J1 = 0, N-1
DO K1 = 0, N-1
V1(0) = 0.51_EB + REAL(I1,EB)*0.48_EB/REAL(N-1,EB)
V1(1) = 0.01_EB + REAL(J1,EB)*0.48_EB/REAL(N-1,EB)
V1(2) = 0.01_EB + REAL(K1,EB)*0.48_EB/REAL(N-1,EB)
DO I2 = 0, N-1
DO J2 = 0, N-1
DO K2 = 0, N-1
V2(0) = 0.01_EB + REAL(I2,EB)*0.98_EB/REAL(N-1,EB)
V2(1) = 0.51_EB + REAL(J2,EB)*0.48_EB/REAL(N-1,EB)
V2(2) = 0.01_EB + REAL(K2,EB)*0.48_EB/REAL(N-1,EB)
DO I3 = 0, N-1
DO J3 = 0, N-1
DO K3 = 0, N-1
V3(0) = 0.01_EB + REAL(I3,EB)*0.98_EB/REAL(N-1,EB)
V3(1) = 0.01_EB + REAL(J3,EB)*0.98_EB/REAL(N-1,EB)
V3(2) = 0.51_EB + REAL(K3,EB)*0.49_EB/REAL(N-1,EB)
XMIN = MIN(V0(0),V1(0),V2(0),V3(0))-0.1_EB
YMIN = MIN(V0(1),V1(1),V2(1),V3(1))-0.1_EB
ZMIN = MIN(V0(2),V1(2),V2(2),V3(2))-0.1_EB
XMAX = MAX(V0(0),V1(0),V2(0),V3(0))+0.1_EB
YMAX = MAX(V0(1),V1(1),V2(1),V3(1))+0.1_EB
ZMAX = MAX(V0(2),V1(2),V2(2),V3(2))+0.1_EB
VOL_TETRA = ABS(VOLUME_VERTS(V0,V1,V2,V3))
VOL=0.0_EB
DO I = 0, 3
DO J = 0, 3
DO K = 0, 3
BOXBOUNDS(0)=XMIN +   REAL(I,EB)*(XMAX-XMIN)/4.0_EB
BOXBOUNDS(1)=XMIN + REAL(I+1,EB)*(XMAX-XMIN)/4.0_EB
BOXBOUNDS(2)=YMIN +   REAL(J,EB)*(YMAX-YMIN)/4.0_EB
BOXBOUNDS(3)=YMIN + REAL(J+1,EB)*(YMAX-YMIN)/4.0_EB
BOXBOUNDS(4)=ZMIN +   REAL(K,EB)*(ZMAX-ZMIN)/4.0_EB
BOXBOUNDS(5)=ZMIN + REAL(K+1,EB)*(ZMAX-ZMIN)/4.0_EB
VOL = VOL + GET_TETRABOX_VOLUME(BOXBOUNDS,V0,V1,V2,V3)
END DO
END DO
END DO
COUNT=COUNT+1
IF(MOD(COUNT,10000).EQ.0)THEN
WRITE(6,*)I0,J0,K0,VOL_TETRA,VOL
ENDIF
END DO
END DO
END DO
...
END DO
TAFTER=SECOND()
WRITE(6,*)"total CPU time=",TAFTER-TBEFORE,"CPU time per calc=",(TAFTER-TBEFORE)/(REAL(COUNT,EB)*REAL(N)**3)
RETURN
...
! if V1-V0 is parallel to N0 ie (V1-V0).DOT.N0 = 0 then there is no solution
...
SUBROUTINE GETVERTS2(BOX_BOUNDS,V0,V1,V2,V3,VERTS,NVERTS,FACESTART,FACENUM,NFACES,VOLUME)
USE PRECISION_PARAMETERS
...
REAL(EB) :: XXMIN, YYMIN, ZZMIN
REAL(EB) :: XXMAX, YYMAX, ZZMAX
...
XXMIN = XMIN - (XMAX-XMIN)/3.0_EB
XXMAX = XMAX + (XMAX-XMIN)/3.0_EB
YYMIN = YMIN - (YMAX-YMIN)/3.0_EB
YYMAX = YMAX + (YMAX-YMIN)/3.0_EB
ZZMIN = ZMIN - (ZMAX-ZMIN)/3.0_EB
ZZMAX = ZMAX + (ZMAX-ZMIN)/3.0_EB
...
DO J = 0, NY-1
DO I = 0, NX-1
BOX_BOUNDS_EB(0) = XXMIN +   REAL(I,EB)*(XXMAX-XXMIN)/REAL(NX,EB)
BOX_BOUNDS_EB(1) = XXMIN + REAL(I+1,EB)*(XXMAX-XXMIN)/REAL(NX,EB)
BOX_BOUNDS_EB(2) = YYMIN +   REAL(J,EB)*(YYMAX-YYMIN)/REAL(NY,EB)
BOX_BOUNDS_EB(3) = YYMIN + REAL(J+1,EB)*(YYMAX-YYMIN)/REAL(NY,EB)
BOX_BOUNDS_EB(4) = ZZMIN +   REAL(K,EB)*(ZZMAX-ZZMIN)/REAL(NZ,EB)
BOX_BOUNDS_EB(5) = ZZMIN + REAL(K+1,EB)*(ZZMAX-ZZMIN)/REAL(NZ,EB)
VOL_IJK(I) = GET_TETRABOX_VOLUME(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB)
VOL = VOL + VOL_IJK(I)
END DO 
WRITE(6,*)"VV",(VOL_IJK(I),I=0,2)
END DO
END DO
VOL_TOTAL=VOLUME_VERTS(V0_EB,V1_EB,V2_EB,V3_EB)
write(6,*)"check: ",VOL,VOL_TOTAL,(VOL-VOL_TOTAL)/VOL_TOTAL
write(6,*)""
