...
REAL(EB) :: Y_FU_0,Y_P_0,Y_LIMITER,A,ETRM,Y_O2_0,Y_CO_0,DYF,DX_FDT,HFAC_F,DTT,DELTA, & 
            ...
            Y_FU_MAX,TMP_F_MIN,Y_F_CORR,Q_BOUND_1,Q_BOUND_2,ZZ_GET(1:N_TRACKED_SPECIES), &
      ...
      DO I=1,IBAR
         ...
         IF_SUPPRESSION: IF (SUPPRESSION) THEN  ! Get maximum O2 in the current and neighboring cells to see if flame viable
            ...
            IF (EXTINCTION2) THEN
               ...
               ZZ_GET = 0._EB
               ZZ_GET(I_FUEL) = 1._EB
               CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR_F_0,TMP(I,J,K)) !INTENT: IN,OUT,IN
               CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR_F_N,RN%CRIT_FLAME_TMP) !INTENT: IN,OUT,IN
               ZZ_GET = ZZ(I,J,K,:)
               ZZ_GET(I_FUEL) = 0._EB
               ZZ_GET = ZZ_GET / (1._EB - Y_FU_0)
               CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR_G_0,TMP(I,J,K)) !INTENT: IN,OUT,IN
               CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR_G_N,RN%CRIT_FLAME_TMP) !INTENT: IN,OUT,IN
         ...
         !DNS Mix Time
            ...
            TAU_D = Z2D(ITMP,I_FUEL)+TMP_WGT*(Z2D(ITMP+1,I_FUEL)-Z2D(ITMP,I_FUEL))
            TAU_D = DELTA**2/TAU_D
         ...
         Q(I,J,K)  = Q_NEW
         !ZZ_GET = ZZ(I,J,K,:)
         !CALL GET_MASS_FRACTION_ALL(ZZ_GET,Y_SPECIES_OLD)
         !CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW)
         !CALL GET_SPECIFIC_HEAT(ZZ_GET,H_G_0,TMP(I,J,K))         
         ...
         !ZZ_GET = ZZ(I,J,K,:)
         !CALL GET_MASS_FRACTION_ALL(ZZ_GET,Y_SPECIES_DIFF)
         !Y_SPECIES_DIFF = Y_SPECIES_DIFF - Y_SPECIES_OLD
         !ITMP = MIN(4999,INT(TMP(I,J,K)))
         !TMP_WGT = TMP(I,J,K) - REAL(ITMP,EB)
         !DO NS = 1, N_SPECIES
         !   H_G_N = Z2CPBAR_C(ITMP)+Z2CPBAR(ITMP,NS)+TMP_WGT*(Z2CPBAR_C(ITMP+1)+Z2CPBAR(ITMP+1,NS)-&
         !                                                     Z2CPBAR_C(ITMP)-Z2CPBAR(ITMP,NS))
...
REAL(EB):: ETRM1(1:N_REACTIONS),ETRM,X_I(0:N_TRACKED_SPECIES),X_0(0:N_TRACKED_SPECIES), &
           ...
           TMPD,DT_ODE,X_MIN,ZZ_GET(1:N_TRACKED_SPECIES),QFAC
