...
NAMELIST /MESH/ IJK,FYI,ID,SYNCHRONIZE,EVACUATION,EVAC_HUMANS,POISSON_BC, &
                IBAR2,JBAR2,KBAR2,CYLINDRICAL,XB,RGB,COLOR,EVAC_Z_OFFSET,MPI_PROCESS
TYPE (MESH_TYPE), POINTER :: M
...
NAMELIST /TIME/ DT,T_BEGIN,T_END,TWFIN,FYI,WALL_INCREMENT,SYNCHRONIZE, &
                EVAC_DT_FLOWFIELD,EVAC_DT_STEADY_STATE,TIME_SHRINK_FACTOR,LOCK_TIME_STEP
TYPE (MESH_TYPE), POINTER :: M
...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,RELAXATION_FACTOR,FYI, &
                ...
                FLUX_LIMITER,FREEZE_VELOCITY,ADJUST_HOR,CFL_VELOCITY_NORM
...
NAMELIST /DUMP/ RENDER_FILE,SMOKE3D,SMOKE3D_COMPRESSION,SMOKE3D_QUANTITY,FLUSH_FILE_BUFFERS,MASS_FILE,STATE_FILE, &
                ...
                PLOT3D_QUANTITY,UL_PAN_DATA,STATUS_FILES
...
PLOOP: DO N=1,5
   CALL GET_QUANTITY_INDEX(PLOT3D_SMOKEVIEW_LABEL(N),PLOT3D_QUANTITY_INDEX(N),PLOT3D_SPEC_INDEX(N),I_DUM, &
                           'PLOT3D',PLOT3D_QUANTITY(N),PLOT3D_SPEC_ID(N),'null')
ENDDO PLOOP
...
IF (SMOKE3D) THEN
   CALL GET_QUANTITY_INDEX(SMOKE3D_SMOKEVIEW_LABEL,SMOKE3D_QUANTITY_INDEX,SMOKE3D_SPEC_INDEX,I_DUM, &
...
REAL(EB) :: EPSIJ,SIGMA2,AMW,OMEGA,TSTAR,MU_N,K_N,D_N0,TE,EPSK_N,SIG_N,MW_N, &
            ...
            CP_MF_TMP(9),D_MF_TMP(9),K_MF_TMP(9),MW_Z2,MW_Z3
!REAL(EB) :: MU_MF_TMP(9)
INTEGER :: IPC,NN,ITMP,N,REAC_INDEX(1:N_REACTIONS)
...
IF (MIXTURE_FRACTION) THEN
   ...
   T_LOOP2: DO J=1,500
      TE  = MAX(0.01_EB*J,0.301_EB)
      CP_MF_TMP(O2_INDEX) = 0.926844_EB+0.191789_EB*TE-0.0370788_EB*TE**2+0.00299313_EB*TE**3- 0.00686447_EB/TE**2
      CP_MF_TMP(N2_INDEX) = 0.931857_EB+0.293529_EB*TE-0.0705765_EB*TE**2+0.00568836_EB*TE**3+ 0.001589693_EB/TE**2
      CP_MF_TMP(OTHER_INDEX) = CP_MF_TMP(N2_INDEX)
      IF (J<=120) THEN   ! Ethylene
         CP_MF_TMP(FUEL_INDEX)  = -0.2218014_EB + 6.402844_EB*TE - 3.922632_EB*TE**2 + 0.9894420_EB*TE**3 + 0.01095625_EB/TE**2
      ELSE
         CP_MF_TMP(FUEL_INDEX)  = 3.698278_EB+0.4768264_EB*TE-0.0912667_EB*TE**2+0.006062326_EB*TE**3- 0.9078017_EB/TE**2
      ENDIF
      IF (J<=120) THEN
         CP_MF_TMP(CO2_INDEX) = 0.568122_EB+1.25425_EB*TE-0.765713_EB*TE**2+0.180645_EB*TE**3- 0.00310541_EB/TE**2
      ELSE
         CP_MF_TMP(CO2_INDEX) = 1.32196_EB+0.0618199_EB*TE-0.0111884_EB*TE**2+0.00082818_EB*TE**3- 0.146529_EB/TE**2
      ENDIF
      IF (J<=50) THEN
         CP_MF_TMP(H2O_INDEX) = 1.95657565_EB
      ELSEIF (50 < J .AND. J <=170) THEN
         CP_MF_TMP(H2O_INDEX) = 1.67178_EB+0.379583_EB*TE+0.377413_EB*TE**2-0.140804_EB*TE**3+ 0.00456328_EB/TE**2
      ELSE
         CP_MF_TMP(H2O_INDEX) = 2.33135_EB+0.479003_EB*TE-0.00833212_EB*TE**2+0.00545106_EB*TE**3- 0.619869_EB/TE**2
      ENDIF
      IF (J<=130) THEN
         CP_MF_TMP(CO_INDEX) = 0.913128_EB+0.217719_EB*TE+0.144809_EB*TE**2-0.0954036_EB*TE**3+ 0.00467932_EB/TE**2
      ELSE
         CP_MF_TMP(CO_INDEX) = 1.255382_EB+0.046432_EB*TE-0.00735432_EB*TE**2+0.00483928_EB*TE**3- 0.1172421_EB/TE**2
      ENDIF
      IF (J<=100) THEN
         CP_MF_TMP(H2_INDEX) = 16.53309_EB-5.681708_EB*TE+5.716408_EB*TE**2-1.386437_EB*TE**3- 0.079279_EB/TE**2
      ELSEIF (J > 100 .AND. J <= 250) THEN
         CP_MF_TMP(H2_INDEX) = 9.281542_EB+6.1286785_EB*TE-1.429893_EB*TE**2+0.134119_EB*TE**3+ 0.988995_EB/TE**2
      ELSE
      ...
      IF (CO_PRODUCTION) THEN
         ...
         Z2CP(J,3) = SUM(Z2Y(:,3) * CP_MF_TMP(:)) * 1000._EB
      ELSE
         Z2CP(J,1) = SUM(Z2Y(:,1) * CP_MF_TMP(:)) * 1000._EB
         Z2CP(J,2) = SUM(Z2Y(:,2) * CP_MF_TMP(:)) * 1000._EB
      ENDIF    
...
MF_DAND_K: IF (MIXTURE_FRACTION) THEN
   ALLOCATE(Z2D_C(500))
   ALLOCATE(Z2K_C(500))
   ALLOCATE(Z2MU(9,500))      
!   ALLOCATE(Z2MU_C(500))
...
!     STATE_SPECIES(1) = 'ETHYLENE'
   ...
   DO ITMP=1,500
      SUB_SPECIES_LOOP2: DO NN=1,9
         ...
         OMEGA = 1.16145_EB*TSTAR**(-0.14874_EB) + 0.52487_EB*EXP(-0.77320_EB*TSTAR) + 2.16178_EB*EXP(-2.43787_EB*TSTAR)
         MU_N = 26.69E-7_EB*(MW_N*ITMP*10)**0.5_EB/(SIGMA2*OMEGA)
         Z2MU(NN,ITMP) = MU_N/MW_N
         K_MF_TMP(NN) = MU_N*SS%CP_MF2(NN,ITMP)/PR
         !Diffusivity
         SIGMA2 = (0.5_EB*(SIG_N+SIG(0)))**2
         EPSIJ  = SQRT(EPSK_N*EPSK(0))
         AMW    = SQRT( (MW_N+SS0%MW)/(2._EB*MW_N*SS0%MW) )
...
!      Z2MU_C(ITMP) = SUM(Z2MW_C(:) * MU_MF_TMP(:))
      Z2K_C(ITMP)  = SUM(Z2MW_C(:) * K_MF_TMP(:))
      Z2D_C(ITMP)  = SUM(Z2Y_C(:)  * D_MF_TMP(:))
      IF (CO_PRODUCTION) THEN
!         Z2MU(ITMP,1) = SUM(Z2MW(:,1) * MU_MF_TMP(:))
!         Z2MU(ITMP,2) = SUM(Z2MW(:,2) * MU_MF_TMP(:))
!         Z2MU(ITMP,3) = SUM(Z2MW(:,3) * MU_MF_TMP(:))
         Z2K(ITMP,1)  = SUM(Z2MW(:,1) * K_MF_TMP(:))
         Z2K(ITMP,2)  = SUM(Z2MW(:,2) * K_MF_TMP(:))
         Z2K(ITMP,3)  = SUM(Z2MW(:,3) * K_MF_TMP(:))
         Z2D(ITMP,1)  = SUM(Z2Y(:,1)  * D_MF_TMP(:))
         Z2D(ITMP,2)  = SUM(Z2Y(:,2)  * D_MF_TMP(:)) 
         Z2D(ITMP,3)  = SUM(Z2Y(:,3)  * D_MF_TMP(:))
      ELSE
!         Z2MU(ITMP,1) = SUM(Z2MW(:,1) * MU_MF_TMP(:))
!         Z2MU(ITMP,2) = SUM(Z2MW(:,2) * MU_MF_TMP(:))
         Z2K(ITMP,1)  = SUM(Z2MW(:,1) * K_MF_TMP(:))
...
CHARACTER(25) :: COLOR
!rm ->
!orig LOGICAL :: MASSLESS,STATIC,FUEL,WATER,TREE,MONODISPERSE,EVAPORATE
LOGICAL :: MASSLESS,STATIC,FUEL,WATER,MONODISPERSE,EVAPORATE,&
           VEG_REMOVE_CHARRED
!rm <-
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC
TYPE(PROPERTY_TYPE), POINTER :: PY
...
!rm ->
   ...
   VEG_DENSITY              = 540._EB !kg/m^3
   VEG_BULK_DENSITY         = 0.3_EB !kg/m^3
   VEG_BURNING_RATE_MAX     = 0.4_EB !kg/m^2.s
   VEG_DEHYDRATION_RATE_MAX = 0.2_EB !kg/m^2.s
...
!rm ->
   ...
   PC%VEG_BURNING_RATE_MAX     = VEG_BURNING_RATE_MAX !kg/m^2.s
   PC%VEG_DEHYDRATION_RATE_MAX = VEG_DEHYDRATION_RATE_MAX !kg/m^2.s
   PC%VEG_INITIAL_TEMPERATURE  = VEG_INITIAL_TEMPERATURE ! C
...
DO N=1,N_PART
   ...
   IF (ANY(PC%QUANTITIES/='null')) THEN
      QUANTITIES_LOOP: DO NN=1,10
         ...
         CALL GET_QUANTITY_INDEX(PC%SMOKEVIEW_LABEL(PC%N_QUANTITIES),PC%QUANTITIES_INDEX(PC%N_QUANTITIES),I_DUMMY,I_DUMMY, &
                                 'PART',PC%QUANTITIES(NN),'null','null')
...
!            TREE_MESH(NM) = .TRUE. !this does not properly pick up
             !trees
            EXIT MESH_LOOP
         ENDIF
...
REAL(EB) :: ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
            ...
            C_FACTOR,CHARACTERISTIC_VELOCITY, &
...
LOGICAL :: PDPA_U,PDPA_V,PDPA_W,PDPA_COUNT
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
                ...
                PDPA_START,PDPA_END,PDPA_RADIUS,PDPA_U,PDPA_V,PDPA_W,PDPA_COUNT,PDPA_M,PDPA_N
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   DO J = 1,ML%N_REACTIONS
      ...
      DO I = 0,NTEMP
         ...
         ML%Q_ARRAY(J,I) = ML%H_R(J)
         IF (ADJUST_HOR) ML%Q_ARRAY(J,I) = ML%H_R(J) - H_T
      ENDDO
   ENDDO   
...
ALLOCATE(SURFACE(0:N_SURF+N_CABL+3),STAT=IZERO)
...
SURFACE(N_SURF-2:N_SURF)%USER_DEFINED = .FALSE.
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   IF (PROCESS(NM)/=MYID .AND. PROCESS(NM)/=EVAC_PROCESS) CYCLE MESH_LOOP
...
MESH_LOOP_2: DO NM=1,NMESHES
   ...
   IF (PROCESS(NM)/=MYID .AND. PROCESS(NM)/=EVAC_PROCESS) CYCLE MESH_LOOP_2
...
MESH_LOOP_3: DO NM=1,NMESHES
   ...
   IF (PROCESS(NM)/=MYID .AND. PROCESS(NM)/=EVAC_PROCESS) CYCLE MESH_LOOP_3
...
!  IF (PROCESS(NM)/=MYID .AND. PROCESS(NM)/=EVAC_PROCESS) CYCLE MESH_LOOP_1
...
CHARACTER(30) :: QUANTITY,PROP_ID,CTRL_ID,DEVC_ID,SURF_ID,STATISTICS,PART_ID
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%QUANTITY /= 'null') THEN
      CALL GET_QUANTITY_INDEX(DV%SMOKEVIEW_LABEL,QUANTITY_INDEX,DV%SPEC_INDEX,DV%PART_INDEX, &
...
READ_ISOF_LOOP: DO N=1,N_ISOF
   ...
   CALL GET_QUANTITY_INDEX(IS%SMOKEVIEW_LABEL,IS%INDEX,IS%SPEC_INDEX,I_DUM,'ISOF',QUANTITY,SPEC_ID,'null')
   IF (COLOR_QUANTITY/='null') &
      CALL GET_QUANTITY_INDEX(IS%SMOKEVIEW_LABEL,IS%INDEX2,IS%SPEC_INDEX2,I_DUM,'ISOF',COLOR_QUANTITY,COLOR_SPEC_ID,'null')
...
NAMELIST /SLCF/ XB,QUANTITY,FYI,PBX,PBY,PBZ,VECTOR,MESH_NUMBER,RLE,MAXIMUM_VALUE,MINIMUM_VALUE,TWO_BYTE,SPEC_ID,PART_ID
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         CALL GET_QUANTITY_INDEX(SL%SMOKEVIEW_LABEL,SL%INDEX,SL%SPEC_INDEX,SL%PART_INDEX,'SLCF',QUANTITY,SPEC_ID,PART_ID)
      ENDDO VECTORLOOP
...
NAMELIST /BNDF/ QUANTITY,FYI,PROP_ID,SPEC_ID,PART_ID
...
READ_BNDF_LOOP: DO N=1,N_BNDF
   ...
   CALL GET_QUANTITY_INDEX(BF%SMOKEVIEW_LABEL,BF%INDEX,BF%SPEC_INDEX,BF%PART_INDEX,'BNDF',QUANTITY,SPEC_ID,PART_ID)
...
OUTPUT_QUANTITY(23)%NAME = 'TIME STEP'                       
OUTPUT_QUANTITY(23)%UNITS = 's'                 
OUTPUT_QUANTITY(23)%SHORT_NAME = 'dt'
OUTPUT_QUANTITY(23)%SLCF_APPROPRIATE = .FALSE.
OUTPUT_QUANTITY(23)%ISOF_APPROPRIATE = .FALSE.
...
! MIXTURE_FRACTION quantities expressed in terms of mass fraction

OUTPUT_QUANTITY(51)%NAME = 'fuel mass fraction'
OUTPUT_QUANTITY(51)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(51)%SHORT_NAME = 'Y_f'

OUTPUT_QUANTITY(52)%NAME = 'oxygen mass fraction'
OUTPUT_QUANTITY(52)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(52)%SHORT_NAME = 'Y_O2'

OUTPUT_QUANTITY(53)%NAME = 'nitrogen mass fraction'
OUTPUT_QUANTITY(53)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(53)%SHORT_NAME = 'Y_N2'

OUTPUT_QUANTITY(54)%NAME = 'water vapor mass fraction'
OUTPUT_QUANTITY(54)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(54)%SHORT_NAME = 'Y_H2O'

OUTPUT_QUANTITY(55)%NAME = 'carbon dioxide mass fraction'
OUTPUT_QUANTITY(55)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(55)%SHORT_NAME = 'Y_CO2'

OUTPUT_QUANTITY(56)%NAME = 'carbon monoxide mass fraction'
OUTPUT_QUANTITY(56)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(56)%SHORT_NAME = 'Y_CO'

OUTPUT_QUANTITY(57)%NAME = 'hydrogen mass fraction'
OUTPUT_QUANTITY(57)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(57)%SHORT_NAME = 'Y_H2'

OUTPUT_QUANTITY(58)%NAME = 'soot mass fraction'
OUTPUT_QUANTITY(58)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(58)%SHORT_NAME = 'Y_soot'

OUTPUT_QUANTITY(59)%NAME = 'other mass fraction'
OUTPUT_QUANTITY(59)%UNITS = 'kg/kg'
OUTPUT_QUANTITY(59)%SHORT_NAME = 'Y_other'

OUTPUT_QUANTITY(51:59)%MIXTURE_FRACTION_ONLY = .TRUE.
OUTPUT_QUANTITY(51:59)%MASS_FRACTION = .TRUE.

! Soot and related outputs

OUTPUT_QUANTITY(61)%NAME = 'soot density'
OUTPUT_QUANTITY(61)%UNITS = 'mg/m3'
OUTPUT_QUANTITY(61)%SHORT_NAME = 'soot'

OUTPUT_QUANTITY(62)%NAME = 'soot volume fraction'
OUTPUT_QUANTITY(62)%UNITS = ' '
OUTPUT_QUANTITY(62)%SHORT_NAME = 'f_v'

OUTPUT_QUANTITY(63)%NAME = 'extinction coefficient'
OUTPUT_QUANTITY(63)%UNITS = '1/m'
OUTPUT_QUANTITY(63)%SHORT_NAME = 'ext'

OUTPUT_QUANTITY(64)%NAME = 'visibility'
OUTPUT_QUANTITY(64)%UNITS = 'm'
OUTPUT_QUANTITY(64)%SHORT_NAME = 'vis'

OUTPUT_QUANTITY(65)%NAME = 'optical depth'
OUTPUT_QUANTITY(65)%UNITS = '1/m'
OUTPUT_QUANTITY(65)%SHORT_NAME = 'OD'
OUTPUT_QUANTITY(65)%MIXTURE_FRACTION_ONLY = .TRUE.

OUTPUT_QUANTITY(61:65)%MIXTURE_FRACTION_ONLY = .TRUE.

! Species-specific quantities that require a SPEC_ID

OUTPUT_QUANTITY(90)%NAME='MASS_FRACTION'
OUTPUT_QUANTITY(90)%UNITS='kg/kg'
OUTPUT_QUANTITY(90)%MASS_FRACTION = .TRUE.
OUTPUT_QUANTITY(90)%SHORT_NAME = 'Y'

OUTPUT_QUANTITY(91)%NAME = 'MASS_FLUX_X'
OUTPUT_QUANTITY(91)%UNITS = 'kg/s/m2'
OUTPUT_QUANTITY(91)%SHORT_NAME = 'u*rho*Y'
OUTPUT_QUANTITY(91)%CELL_POSITION = CELL_FACE

OUTPUT_QUANTITY(92)%NAME = 'MASS_FLUX_Y'
OUTPUT_QUANTITY(92)%UNITS = 'kg/s/m2'
OUTPUT_QUANTITY(92)%SHORT_NAME = 'v*rho*Y'
OUTPUT_QUANTITY(92)%CELL_POSITION = CELL_FACE

OUTPUT_QUANTITY(93)%NAME = 'MASS_FLUX_Z'
OUTPUT_QUANTITY(93)%UNITS = 'kg/s/m2'
OUTPUT_QUANTITY(93)%SHORT_NAME = 'w*rho*Y'
OUTPUT_QUANTITY(93)%CELL_POSITION = CELL_FACE

OUTPUT_QUANTITY(94)%NAME = 'VOLUME_FRACTION'
...
OUTPUT_QUANTITY(163)%NAME = 'RADIATIVE_FLUX_GAS'
...
OUTPUT_QUANTITY(231)%BNDF_APPROPRIATE = .FALSE.
...
OUTPUT_QUANTITY(-20)%NAME = 'MASS_FLUX'
...
OUTPUT_QUANTITY(-34)%NAME= 'SURFACE DENSITY'
OUTPUT_QUANTITY(-34)%UNITS= 'kg/m2'
OUTPUT_QUANTITY(-34)%SHORT_NAME= 'dens'

! Solid Phase Particle Outputs

OUTPUT_QUANTITY(-50)%NAME = 'MPUA'
OUTPUT_QUANTITY(-50)%UNITS = 'kg/m2'
OUTPUT_QUANTITY(-50)%SHORT_NAME = 'mpua'

OUTPUT_QUANTITY(-60)%NAME = 'CPUA'
OUTPUT_QUANTITY(-60)%UNITS = 'kW/m2'
OUTPUT_QUANTITY(-60)%SHORT_NAME = 'cpua'

OUTPUT_QUANTITY(-70)%NAME = 'AMPUA'
OUTPUT_QUANTITY(-70)%UNITS= 'kg/m2'
OUTPUT_QUANTITY(-70)%SHORT_NAME = 'ampua'
...
SUBROUTINE GET_QUANTITY_INDEX(SMOKEVIEW_LABEL,OUTPUT_INDEX,SPEC_INDEX,PART_INDEX,OUTTYPE,QUANTITY,SPEC_ID,PART_ID)

CHARACTER(*), INTENT(INOUT) :: QUANTITY
CHARACTER(*), INTENT(OUT) :: SMOKEVIEW_LABEL
CHARACTER(*), INTENT(IN) :: SPEC_ID,PART_ID
...
IF (SPEC_ID/='null') THEN
   DO NS=1,N_SPECIES
      IF (SPEC_ID==SPECIES(NS)%NAME) THEN
         SPEC_INDEX = NS
         EXIT
      ENDIF
   ENDDO
...
DO NS=1,N_SPECIES
   IF (QUANTITY==TRIM(SPECIES(NS)%NAME)) THEN
      QUANTITY   = 'MASS_FRACTION'
      SPEC_INDEX = NS
   ENDIF
   IF (QUANTITY==TRIM(SPECIES(NS)%NAME)//'_VF') THEN
      QUANTITY   = 'VOLUME_FRACTION'
      SPEC_INDEX = NS
   ENDIF
   IF (QUANTITY==TRIM(SPECIES(NS)%NAME)//'_FLUX_X') THEN
      QUANTITY   = 'MASS_FLUX_X'
      SPEC_INDEX = NS
   ENDIF
   IF (QUANTITY==TRIM(SPECIES(NS)%NAME)//'_FLUX_Y') THEN
      QUANTITY   = 'MASS_FLUX_Y'
      SPEC_INDEX = NS
   ENDIF
   IF (QUANTITY==TRIM(SPECIES(NS)%NAME)//'_FLUX_Z') THEN
      QUANTITY   = 'MASS_FLUX_Z'
      SPEC_INDEX = NS
   ENDIF
   IF (QUANTITY==TRIM(SPECIES(NS)%NAME)//'_FLUX') THEN
      QUANTITY   = 'MASS_FLUX'
      SPEC_INDEX = NS
   ENDIF
ENDDO
...
DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
  ...
  IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
     ...
     IF (SPEC_INDEX>0) THEN
        SMOKEVIEW_LABEL = TRIM(SPECIES(SPEC_INDEX)%NAME)//' '//TRIM(QUANTITY)
