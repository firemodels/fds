...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            AREA_ADJUST,PARTICLE_SURFACE_DENSITY,MOISTURE_FRACTION,&
         ...
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            PART_ID_IF: IF (PART_ID/='null') THEN
               ...
               IF (IN%MASS_PER_TIME>0._EB .OR. IN%MASS_PER_VOLUME>0._EB) THEN
                  ...
                  ENDIF
                  IF (LPC%SURF_INDEX>0) THEN
                     IF (SURFACE(LPC%SURF_INDEX)%MOISTURE_FRACTION>0._EB) THEN
                        IN%MASS_PER_TIME   = IN%MASS_PER_TIME  *(1._EB+SURFACE(LPC%SURF_INDEX)%MOISTURE_FRACTION)
                        IN%MASS_PER_VOLUME = IN%MASS_PER_VOLUME*(1._EB+SURFACE(LPC%SURF_INDEX)%MOISTURE_FRACTION)
                     ENDIF
                  ENDIF
               ENDIF
