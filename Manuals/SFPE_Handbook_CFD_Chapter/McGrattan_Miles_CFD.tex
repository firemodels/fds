...
Finally, throughout the verification and validation process, the modeler should be skeptical of both the model and the experimental measurements. As discussed previously, all CFD models have limitations, and it is not uncommon to find programming errors, or ``bugs,'' in even the most popular software, especially for applications that are atypical. Similarly, full-scale fire experiments are difficult to perform, and instruments routinely fail under the harsh conditions. Often the failure is obvious, but sometimes the malfunction is less pronounced, as with an instrument that has not been properly calibrated or one that has ``drifted'' over the course of the test series. The experimentalist is hesitant to discount a measurement without some indication as to why it failed, or sometimes it is simply not apparent that there is anything wrong at all. The failure is not detected until years later when the model prediction and the measurement are plotted side by side, and even then, who is to say which is right and which is wrong? As a remedy, modelers should seek out test data from experiments that they have personally observed or that were conducted by people known to them who can relate the many details that often go unreported in the journal article or report of test.
