...
TYPE(MESH_STRUCT) :: MESH_SEND
TYPE(MESH_STRUCT), ALLOCATABLE, DIMENSION(:) ::  MESH_RECV
...
IF (MYID.EQ.0) THEN
   ALLOCATE(MESH_RECV(NMESHES),STAT=IZERO)
END IF

! Gather various values at node 0
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   CALL MPI_GATHER(MESH_SEND, 1, MESH_STRUCT_TYPE, MESH_RECV, &
       1, MESH_STRUCT_TYPE, 0, MPI_COMM_WORLD, IERR)

   DO NOM=1,NMESHES
       IF (PROCESS(NOM)==0 .OR. MYID/=0) CYCLE
       MESHES(NOM)%CFL    = MESH_RECV(PROCESS(NOM))%DBLS(1)
       MESHES(NOM)%DIVMX  = MESH_RECV(PROCESS(NOM))%DBLS(2)
       MESHES(NOM)%DIVMN  = MESH_RECV(PROCESS(NOM))%DBLS(3)
       MESHES(NOM)%RESMAX = MESH_RECV(PROCESS(NOM))%DBLS(4)
       MESHES(NOM)%POIS_PTB = MESH_RECV(PROCESS(NOM))%DBLS(5)
       MESHES(NOM)%POIS_ERR = MESH_RECV(PROCESS(NOM))%DBLS(6)
       MESHES(NOM)%VN = MESH_RECV(PROCESS(NOM))%DBLS(7)
       MESHES(NOM)%ICFL = MESH_RECV(PROCESS(NOM))%INTS(1)
       MESHES(NOM)%JCFL = MESH_RECV(PROCESS(NOM))%INTS(2)
       MESHES(NOM)%KCFL = MESH_RECV(PROCESS(NOM))%INTS(3)
       MESHES(NOM)%IMX = MESH_RECV(PROCESS(NOM))%INTS(4)
       MESHES(NOM)%JMX = MESH_RECV(PROCESS(NOM))%INTS(5)
       MESHES(NOM)%KMX = MESH_RECV(PROCESS(NOM))%INTS(6)
       MESHES(NOM)%IMN = MESH_RECV(PROCESS(NOM))%INTS(7)
       MESHES(NOM)%JMN = MESH_RECV(PROCESS(NOM))%INTS(8)
       MESHES(NOM)%KMN = MESH_RECV(PROCESS(NOM))%INTS(9)
       MESHES(NOM)%IRM = MESH_RECV(PROCESS(NOM))%INTS(10)
       MESHES(NOM)%JRM = MESH_RECV(PROCESS(NOM))%INTS(11)
       MESHES(NOM)%KRM = MESH_RECV(PROCESS(NOM))%INTS(12)
       MESHES(NOM)%I_VN = MESH_RECV(PROCESS(NOM))%INTS(13)
       MESHES(NOM)%J_VN = MESH_RECV(PROCESS(NOM))%INTS(14)
       MESHES(NOM)%K_VN = MESH_RECV(PROCESS(NOM))%INTS(15)
       MESHES(NOM)%NLP = MESH_RECV(PROCESS(NOM))%INTS(16)
   END DO
END DO

IF (MYID.EQ.0) THEN
       DEALLOCATE(MESH_RECV)
END IF
