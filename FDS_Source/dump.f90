...
USE COMP_FUNCTIONS, ONLY : SECOND
USE TURBULENCE, ONLY: DUMP_RAW_DATA
REAL(EB) :: TNOW
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
...
ELSE
   ...
   IF (T>=SLCF_CLOCK(NM)) THEN
      ...
      ENDDO
      IF (RAW_DATA_FILE) CALL DUMP_RAW_DATA(NM)
   ENDIF
...
IF (COLUMN_DUMP_LIMIT) THEN 
   N_DEVC_FILES = N_DEVC / DEVC_COLUMN_LIMIT
   IF (N_DEVC_FILES * DEVC_COLUMN_LIMIT < N_DEVC) N_DEVC_FILES = N_DEVC_FILES + 1
...
DO N=1,N_TREES_OUT 
 ...
 ELSE
    ...
    ELSE
       OPEN(LU_VEG_OUT(N),FILE=FN_VEG_OUT(N),FORM='FORMATTED',STATUS='REPLACE')
       WRITE(LU_VEG_OUT(N),'(A,A,A)')'s,','kg,','kg' 
       WRITE(LU_VEG_OUT(N),'(A,A,A)')'FDS_Vegout_Time,','Tree_Dry_Mass,','Tree_Moist_Mass' 
    ENDIF
 ENDIF
ENDDO
...
IF (.NOT.APPEND .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO N=1,N_ISOF
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') IS%SMOKEVIEW_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') IS%SMOKEVIEW_BAR_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') OUTPUT_QUANTITY(IS%INDEX)%UNITS
      IF (IS%INDEX2/=0) THEN
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A)') IS%SMOKEVIEW_LABEL2
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A)') IS%SMOKEVIEW_BAR_LABEL2
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A)') OUTPUT_QUANTITY(IS%INDEX2)%UNITS
      ENDIF
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D .AND..NOT.EVACUATION_ONLY(NM)) THEN
   ...
   IF (.NOT.APPEND) THEN
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') SMOKE3D_SMOKEVIEW_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') SMOKE3D_SMOKEVIEW_BAR_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
...
DO N=1,M%N_SLCF
   ...
   ELSE
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') SL%SMOKEVIEW_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') SL%SMOKEVIEW_BAR_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
...
BOUNDARY_FILES: DO NF=1,N_BNDF
   ...
   ELSE RESTART
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') BF%SMOKEVIEW_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') BF%SMOKEVIEW_BAR_LABEL
      M%N_STRINGS = M%N_STRINGS + 1
...
!
         NN = N_PART/(N_STRINGS_MAX - M%N_STRINGS)
         DO N = 1,NN ! reallocate if # of PART lines require it
          CALL RE_ALLOCATE_STRINGS(NM)
         ENDDO
!
         DO N=1,N_PART
...
INTEGER :: N,NN,I,J,K,NM,NX,NY,NZ,NIN,NXL,NYL,NZL,NDV,NDVOLD,NDVDIM,INDX,IZERO,EVAC_CODE
...
WRITE(LU_SMV,'(A,A)') ' ',SURFACE(DEFAULT_SURF_INDEX)%ID
 
DO N=0,N_SURF
   SF => SURFACE(N)
   WRITE(LU_SMV,'(/A)') 'SURFACE'
   WRITE(LU_SMV,'(A,A)') ' ',SURFACE(N)%ID
...
EVAC_ONLY3: IF (.NOT. ALL(EVACUATION_ONLY)) THEN
   DO N=1,N_PART
      ...
      IF(PC%SMOKEVIEW_ID.EQ.' ')THEN
         WRITE(LU_SMV,'(A,A)')     ' ',PC%ID      
      ELSE
         WRITE(LU_SMV,'(A,A,A,A)') ' ',PC%ID,'  % ',TRIM(PC%SMOKEVIEW_ID)
      ENDIF
      WRITE(LU_SMV,'(3F13.5)') REAL(PC%RGB,FB)/255._FB
      WRITE(LU_SMV,'(I3)') PC%N_QUANTITIES
      DO NN=1,PC%N_QUANTITIES
         WRITE(LU_SMV,'(A)') PC%SMOKEVIEW_LABEL(NN)
         WRITE(LU_SMV,'(A)') PC%SMOKEVIEW_BAR_LABEL(NN)
         WRITE(LU_SMV,'(A)') OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%UNITS
      ENDDO
      IF(PC%SMOKEVIEW_ID/=' ')WRITE(LU_SMV,'(4F9.3)') PC%DIAMETER,PC%LENGTH,PC%AZIMUTH,PC%ELEVATION
   ENDDO
ENDIF EVAC_ONLY3
...
DO NM=1,NMESHES
WRITE(LU_SMV,'(F13.5)') 0.1E-3_EB*HRRPUA_SHEET/MESHES(NM)%CELL_SIZE
ENDDO
...
DO N=1,N_DEVC
   ...
   N_TEXT = 0
   IF (PY%TEXTURE_MAP/='null') N_TEXT = 1
   WRITE(LU_SMV,'(/A)') 'DEVICE'
   WRITE(LU_SMV,'(A)') TRIM(PY%SMOKEVIEW_ID)
   WRITE(LU_SMV,'(6F12.5,3I3)') DV%X,DV%Y,DV%Z,DV%ORIENTATION(1:3),PY%STATE_INDEX,PY%N_SMOKEVIEW_PARAMETERS,N_TEXT
   IF (PY%N_SMOKEVIEW_PARAMETERS>0) WRITE(LU_SMV,'(6F12.5)') PY%SMOKEVIEW_PARAMETERS(1:PY%N_SMOKEVIEW_PARAMETERS)
   IF (N_TEXT>0) WRITE(LU_SMV,'(A)') PY%TEXTURE_MAP
ENDDO
...
EVAC_ONLY5: IF (ANY(EVACUATION_GRID).AND.MYID==MAX(0,EVAC_PROCESS)) THEN
   DO N=1,N_DOORS
      ...
      WRITE(LU_SMV,'(3F6.2,3F6.3)') EVAC_DOORS(N)%WIDTH, 0.05, EVAC_DOORS(N)%HEIGHT, EVAC_DOORS(N)%RGB(1:3)/255.0_EB
      ! x y z translate for the arrow (cone)
      ! xx: upwards, yy: right, zz: towards the door
      IF (ABS(EVAC_DOORS(N)%IOR) <= 1) THEN
         YY =-EVAC_DOORS(N)%IOR*(EVAC_DOORS(N)%Y - YY)
         XX = EVAC_DOORS(N)%IOR*(EVAC_DOORS(N)%X - XX)
      ELSE
         ZZ = EVAC_DOORS(N)%IOR*(EVAC_DOORS(N)%X - XX)/2.0_EB
         XX = EVAC_DOORS(N)%IOR*(EVAC_DOORS(N)%Y - YY)/2.0_EB
         YY = ZZ
      ENDIF
      ZZ = 0.5_EB*EVAC_DOORS(N)%HEIGHT
      WRITE(LU_SMV,'(3F6.2)') ZZ, 0.0_EB, XX
   ENDDO
   DO N=1,N_EXITS
      ...
      WRITE(LU_SMV,'(3F6.2,3F6.3)') EVAC_EXITS(N)%WIDTH, 0.05, EVAC_EXITS(N)%HEIGHT, EVAC_EXITS(N)%RGB(1:3)/255.0_EB
      ! x y z translate for the arrow (cone)
      ! xx: upwards, yy: right, zz: towards the door
      IF (ABS(EVAC_EXITS(N)%IOR) <= 1) THEN
         YY =-EVAC_EXITS(N)%IOR*(EVAC_EXITS(N)%Y - YY)
         XX = EVAC_EXITS(N)%IOR*(EVAC_EXITS(N)%X - XX)
      ELSE
         ZZ = EVAC_EXITS(N)%IOR*(EVAC_EXITS(N)%X - XX)/2.0_EB
         XX = EVAC_EXITS(N)%IOR*(EVAC_EXITS(N)%Y - YY)/2.0_EB
         YY = ZZ
      ENDIF
      ZZ = 0.5_EB*EVAC_EXITS(N)%HEIGHT
      WRITE(LU_SMV,'(3F6.2)') ZZ, YY, XX
   ENDDO
   DO N=1,N_ENTRYS
      ...
      WRITE(LU_SMV,'(3F6.2,3F6.3)') EVAC_ENTRYS(N)%WIDTH, 0.05, EVAC_ENTRYS(N)%HEIGHT, EVAC_ENTRYS(N)%RGB(1:3)/255.0_EB
   ENDDO
   DO N=1,N_SSTANDS
      ...
      ELSE
         XX = ABS(EVAC_SSTANDS(N)%X2-EVAC_SSTANDS(N)%X1)
         YY = ABS(EVAC_SSTANDS(N)%Y2-EVAC_SSTANDS(N)%Y1)/EVAC_SSTANDS(N)%COS_Y
      ENDIF
      WRITE(LU_SMV,'(3F6.2,3F6.3)') XX, 0.1, YY, EVAC_SSTANDS(N)%RGB(1:3)/255.0_EB
   ENDDO
END IF EVAC_ONLY5
...
SURFLOOP: DO N=0,N_SURF
   ...
   SF => SURFACE(N)
   IF (N==0) THEN
      WRITE(LU_OUTPUT,'(/I4,1X,A,A)')  N,SF%ID,' (DEFAULT)'
   ELSE
      WRITE(LU_OUTPUT,'(/I4,1X,A)')    N,SF%ID
   ENDIF
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IS%INDEX,IS%SPEC_INDEX,0,IS%VELO_INDEX,T,NM)
         ENDDO
      ENDDO
   ENDDO
   ...
   ELSE
      ...
      DO K=0,KBP1
         DO J=0,JBP1
            DO I=0,IBP1
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IS%INDEX2,IS%SPEC_INDEX,0,IS%VELO_INDEX,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ...
   SELECT CASE(DATAFLAG)
      CASE(1)
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
                  FF(I,J,K) = RHO(I,J,K)*GAS_PHASE_OUTPUT(I,J,K,SMOKE3D_QUANTITY_INDEX,SMOKE3D_SPEC_INDEX,0,0,T,NM)
               ENDDO
            ENDDO
         ENDDO
      CASE(2)
...
IF (PLOT3D) THEN  ! Write out information to .smv file
   ...
   DO IQ=1,5
      N_STRINGS = N_STRINGS + 1
      WRITE(STRING(N_STRINGS),'(A)') PLOT3D_SMOKEVIEW_LABEL(IQ)
      N_STRINGS = N_STRINGS + 1
      WRITE(STRING(N_STRINGS),'(A)') PLOT3D_SMOKEVIEW_BAR_LABEL(IQ)
      N_STRINGS = N_STRINGS + 1
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   IF (.NOT. AGL_TERRAIN_SLICE) THEN
      DO K=KK1,KK2
         DO J=JJ1,JJ2
            DO I=II1,II2
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IND,SPEC_INDEX,PART_INDEX,VELO_INDEX,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ELSE
      ...
      DO I=II1,II2
         DO J=JJ1,JJ2
            DO K=KK1,KK2
               ...
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,KTS,IND,SPEC_INDEX,PART_INDEX,VELO_INDEX,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ENDIF
...
REAL(EB) FUNCTION GAS_PHASE_OUTPUT(II,JJ,KK,IND,SPEC_INDEX,PART_INDEX,VELO_INDEX,T,NM)
...
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: II,JJ,KK,IND,NM,VELO_INDEX
REAL(EB) :: FLOW,HMFAC,F1,F2,H_TC,SRAD,TMP_TC,MU_AIR,RE_D,K_AIR,NUSSELT,NU_FAC,PR,AREA,VEL, &
            ...
            DHOR,X_EQUIL,MW_RATIO,Y_EQUIL,TMP_BOIL,EXPON,Y_SPECIES,MEC,Y_SPECIES2,Y_H2O,YY_G(1:N_SPECIES),R_DN,SGN
...
IF (SPEC_INDEX<0) THEN
   ...
   Y_H2O = 0._EB
   IF (DRY) CALL GET_MASS_FRACTION(YY_GET,H2O_INDEX,Y_H2O)
...
SELECT CASE(IND)
   ...
   CASE(94)  ! VOLUME FRACTION
      IF (SPEC_INDEX<0) THEN
         GAS_PHASE_OUTPUT = RCON_MF(-SPEC_INDEX)*Y_SPECIES/RSUM(II,JJ,KK)/(1._EB-RCON_MF(H2O_INDEX)*Y_H2O/RSUM(II,JJ,KK))
      ELSEIF (SPEC_INDEX>0) THEN
...
IF(CENTERED_DEVC_TIME)THEN
  ...
  !
  !   [stime-DT_DEVC, stime]   if t>= T_BEGIN+DT_DEVC,      so center of window is stime - DT_DEVC/2
  !   [T_BEGIN, T_BEGIN+stime] if stime <= T_BEGIN+DT_DEVC, so center of window is T_BEGIN+stime/2 
   IF(STIME.GE.T_BEGIN+DT_DEVC)THEN
...
IF (COLUMN_DUMP_LIMIT) THEN
   DO I=1,N_DEVC_FILES
      WRITE(TCFORM,'(A,I4.4,A)') "(",MIN(DEVC_COLUMN_LIMIT, N_DEVC - DEVC_COLUMN_LIMIT * (I - 1)),"(ES15.7E3,','),ES15.7E3)"
      WRITE(LU_DEVC(I),TCFORM) DEVC_TIME,(DEVICE(N)%VALUE/REAL(DEVICE(N)%COUNT,EB), &
                               N=DEVC_COLUMN_LIMIT*(I-1)+1, MIN(N_DEVC,DEVC_COLUMN_LIMIT*I))
   ENDDO
ELSE
   WRITE(TCFORM,'(A,I4.4,A)') "(",N_DEVC,"(ES15.7E3,','),ES15.7E3)"
   WRITE(LU_DEVC(1),TCFORM) DEVC_TIME,(DEVICE(N)%VALUE/REAL(DEVICE(N)%COUNT,EB), N=1,N_DEVC)
ENDIF
...
WALL_LOOP: DO IW=1,NWC
   ...
   IF (BOUNDARY_TYPE(IW)==OPEN_BOUNDARY .OR. ABS(UW(IW))>ZERO_P) THEN
      E_DIFF = KW(IW)*RDN(IW)*(TMP(IIG,JJG,KKG)-TMP_W(IW))
   ELSE
      E_DIFF = 0._EB
   ENDIF
...
!  MOIST_MASS_TOTAL  = MOIST_MASS_TOTAL  + VEG_TOTAL_MOIST_MASS(NM)
  ...
  DROPLET_LOOP: DO I=1,NLP
   ...
   IF (N_TREE == 0) CYCLE DROPLET_LOOP
   ...
   TREE_OUTPUT_DATA(N_TREE,1) = TREE_OUTPUT_DATA(N_TREE,1) + DR%VEG_FUEL_MASS*V_CELL
   TREE_OUTPUT_DATA(N_TREE,2) = TREE_OUTPUT_DATA(N_TREE,2) + DR%VEG_MOIST_MASS*V_CELL
  ENDDO DROPLET_LOOP
  
ENDDO MESH_LOOP
 
IF (VEG_PRESENT) THEN
!  WRITE(9999,'(3(ES12.4))')T,DRY_MASS_TOTAL,MOIST_MASS_TOTAL
 DO I=1,N_TREES_OUT
  WRITE(LU_VEG_OUT(I),'(3(ES12.4))')T,TREE_OUTPUT_DATA(I,1),TREE_OUTPUT_DATA(I,2)
 ENDDO
ENDIF
...
MINT_TOTAL(:) = 0._EB
DO NM=1,NMESHES
   IF (MINT_COUNT(NM)>0._EB) MINT_TOTAL(0:N_Y_ARRAY) = MINT_TOTAL(0:N_Y_ARRAY) + MINT_SUM(0:N_Y_ARRAY,NM)/MINT_COUNT(NM)
ENDDO
IF (MIXTURE_FRACTION) THEN
