...
IF ( MOD(RAD_CALL_COUNTER,TIME_STEP_INCREMENT)==1 .OR. TIME_STEP_INCREMENT==1 .OR. INITIALIZATION_PHASE) THEN
...
LT1    =   L1 * TEMP/LTSTEP
CALL INTERPOLATE1D_UNIFORM(BBFRAC,LT1,BBFLOW)

LT2    =   L2 * TEMP/LTSTEP
CALL INTERPOLATE1D_UNIFORM(BBFRAC,LT2,BBFHIGH)
...
DO N = 1, N_RADCAL_SPECIES
   ...
   ENDIF
   CALL INTERPOLATE1D_UNIFORM(RADCAL_SPECIES2KAPPA(N,0:N_KAPPA_Y,TYY,IBND),INT_FAC,KAPPA_TEMP)
   GET_KAPPA=GET_KAPPA+KAPPA_TEMP
ENDDO
