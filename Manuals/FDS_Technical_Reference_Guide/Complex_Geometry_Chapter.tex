...
% Use write up from COMP_GEOM manuscript. Can go to an Appendix.
...
%%
%\begin{equation}
% \frac{\partial \rho Y_\alpha}{ \partial t} + \nabla \cdot \left(  \mathbf{u}' (\rho Y_\alpha)   \right) = - \nabla \cdot \left(  \mathbf{J_{d \alpha}'} \right) + \dot{m}_\alpha''' \; , \; \alpha=1,\dots,N \label{eq:bal4}
%\end{equation}
%%
%where
%%
%\begin{eqnarray}
%  \mathbf{u}' &=& \mathbf{u} + \frac{D_\alpha}{\rho} \boldsymbol{\nabla} \rho  \label{eq:vprime} \\
%   \mathbf{J_{d \alpha}'} &=& -D_\alpha \boldsymbol{\nabla}  (\rho Y_\alpha )  \label{eq:jdaprime}
%\end{eqnarray}
%%
%are the sum $\mathbf{u}'$ of the fluid mixture velocity $\mathbf{u}$ and a diffusion driven velocity term for species $\alpha$ in presence of density gradients $D_\alpha \boldsymbol{\nabla} \rho / \rho $, and a constant density diffusive flux term $\mathbf{J_{d \alpha}'}$.
%
...
Consider \texttt{INBOUNDARY} cut-face $k=\mathbf{1}$ in figure~\ref{Fig:FVdiscCC}b. If a Neumann boundary condition is specified in such a face, the term
%
\begin{equation}
...
\end{equation}
%
where the value of $Y_{Ec1}$ is obtained from linear interpolation to location $\mathbf{x}_{Ec1}$, and $h$ takes a value of half the local cell size.


\subsection{CC discretization of Advective term}

We study the discretization of the advective term for the six faces of cell $ii$ in figure~\ref{Fig:FVdiscCC}b:
%
\begin{equation}
...
\subsection*{A. Faces $k=\mathbf{4},\mathbf{5}$ are \textit{regular} \texttt{GASPHASE} faces connecting a cut-cell with a regular cell:}
The treatment of these is as described in the previous section, with the caveat that the advective flux computation for $\rho Y_\alpha$ may now involves a four point stencil ($e=1,...,4$) spatial flux matched interpolation to the face. Consider the situations depicted in figures~\ref{Fig:FVFlxLimCC} for a regular face $RC$ connecting regular cell and cut-cell. I the left figure both extreme interpolation points of the stencil have associated species densities $\rho Y_1$ and $\rho Y_4$, as at least one cut-cell or regular cells are defined at those locations. In the right figure the lower extreme position of the stencil (cell S) lies completely within the \texttt{SOLID} phase, and it takes the value of the inner cell $\rho Y_2$, rendering the interpolation to RC of Godunov type. Note also the loss of accuracy on the flux limited interpolation described in section~\ref{sec_flux_limiters} due to cut-cell centroids making the stencil not uniformly spaced.
...
The treatment of advective terms in boundary cut-faces is similar as described before. Consider a normal velocity $\mathbf{u}_1=v_n \hat{\mathbf{n}}_{ii,1}$ (into solid region) imposed at the boundary in \texttt{INBOUNDARY} cut-face k = 1 in figure~\ref{Fig:FVdiscCC}b. The advective component is in this case:
%
\begin{equation}
...
%  \item Using $\{\mathbf{\rho Y_\alpha}\}^n$, define fluxes $\left( [\rho Y_\alpha \mathbf{u} -\rho D_\alpha \boldsymbol{\nabla} Y_\alpha] \cdot \hat{\mathbf{n}} \right)^n$ at the $\Gamma_{EXIM}$ boundary.
...
%   Forward Euler FE for $EX$ region and Backward Euler BE for $IM$ region. Boundary conditions for the implicit solve are provided by the explicit mass fluxes $\left( [\rho Y_\alpha \mathbf{u} -\rho D_\alpha \boldsymbol{\nabla} Y_\alpha] \cdot \hat{\mathbf{n}} \right)^n$ at the $\Gamma_{EXIM}$ boundary.
%   \begin{eqnarray}
...
%   \end{eqnarray}
%  \item Second Stage: SSPRK2 corrector for $EX$ and $IM$ regions. At the $IM$ region $L'()$ is evaluated implicitly. Boundary conditions for the implicit solve are given by $\left( [\rho Y_\alpha \mathbf{u} -\rho D_\alpha \boldsymbol{\nabla} Y_\alpha] \cdot \hat{\mathbf{n}} \right)^*$.
%   \begin{eqnarray}
...
The next component of our scheme is used to approximate the velocity boundary condition for immersed solid boundaries. In FDS, velocity field components are staggered in cartesian or cut faces. 
As the projection scheme defined above is applied on the cartesian mesh, the treatment of FDS faces immersed completely in the solid region controls fluid penetration into the solid. We force these velocities to approximate the solid velocity using a direct forcing immersed boundary method for the momentum equations~\cite{Fadlun:2000}.

On the other hand, cross velocities and tangent stresses and vorticity emanating from the immersed boundary are defined in cartesian faces being intersected by the geometry surface and surrounding cartesian edges. There are to separate methods to handle these, namely, direct forcing IBM or stress method (STM). 
Before developing both methods, an important concept is described next.
 
 
 \subsection{Equivalence of discrete divergence integrals among cut-cells and underlying Eulerian cells:} \label{sec:discretediveqv}

% figure
\begin{figure}[h]
      \centering
      \includegraphics[trim = 70mm 45mm 60mm 40mm, clip,
       width=0.45\linewidth]{../../../fig/fds/Geometry_Figures/CutCellsIBSketchNormal.png}
       \includegraphics[trim = 70mm 50mm 70mm 50mm, clip,
       width=0.45\linewidth]{../../../fig/fds/Geometry_Figures/CutCellsWallAlignedSketch.png}
      \put(-350,0){(a)}
      \put(-120,0){(b)}
      \caption{(a) Divergence equivalence: For cell (i,j) \texttt{GASPHASE} regular and cut-faces (in red), and underlying Eulerian staggered mesh are shown. (b) Wall stress $\tau_w$ estimation using a wall function for solid boundary crossing through vertex $B$ and aligned with the fluid grid.}
        \label{Fig:DiveqvStrss}
\end{figure}
%
Consider Eulerian cell $i,j$ in figure~\ref{Fig:DiveqvStrss}a, containing a \texttt{GASPHASE} cut-cell $ii$ with centroid $\mathbf{x}_{ii}$. The integral of the Eulerian cell velocity divergence can be divided as:
%
\begin{equation}
...
Referring to figure~\ref{Fig:DiveqvStrss}a, and using the divergence theorem for each of the previous integrals, we have for cell $i,j$
%
\begin{eqnarray}
...
\end{equation} 
%
I this section we are interested in the divergence of deviatoric stress $ \nabla \cdot \tau$ term, its discretization and shear stress values at the boundary.

\subsubsection{Wall modeled shear stress for grid aligned solid}

Consider the grid aligned case of figure~\ref{Fig:DiveqvStrss}b. A velocity component $u_e^n$ is defined in a face adjacent to the solid boundary. 
...
\end{equation}
%
Here $\tau_w$ is the stress at the wall point $B$; $\rho,\nu$ are the density and molecular kinematic viscosity. The quantity $\delta_v$ is the near wall viscous length scale. The functional model used has been described in section~\ref{info:velocity_bc}, and corresponds to a zero pressure gradient equilibrium turbulent boundary layer (\textit{log-law}). As this function is non-linear, $y^+, \tau_w, u_\tau, \delta_v$ are computed to convergence trough fixed point iteration. 

Referring to figure~\ref{Fig:DiveqvStrss}b, and considering the expresison for the wall stress $\tau_w = \rho \nu \left( \frac{\partial u}{\partial y} \right)_{y=0}$ and a known velocity value $u_e^n$ initialize:
%
\begin{itemize}
...
%
\begin{enumerate}
    \item For each RC edge compute the gas finite difference estimation of the cross velocity derivatives $\partial u_i/\partial x_j$ and $\partial u_j/\partial x_i$. In figure~\ref{Fig:STMgraphs}b, $\frac{\partial u}{\partial y} \simeq 2 \frac{u_E-u_F}{\Delta_y + \Delta_{RC}}$, where $u_E=u_{i,j+1,k}$ and  $u_F=u_{i,j,k}$, and $\frac{\partial v}{\partial x} \simeq \frac{v_{i+1,j,k}-v_{i,j,k}}{\Delta_x}$. Note that the distance to the wall $\Delta_{RC}$ is being used to compute the finite difference in the cut-face and $y$ direction. Also interpolating the dynamic viscosity to the RC edge, estimate $\mu_{RC} \frac{\partial u}{\partial y}$ and $\mu_{RC} \frac{\partial v}{\partial x}$. \label{point1}
...
Finally, all RC and IB edges are assigned a vorticity and shear stress using the computed gradients $\partial u_i/\partial x_j$ and components $\mu \partial u_i/\partial x_j$. The computed vorticity and stresses are used in the construction of the force vector $\mathbf{F}^n$ of equation~\eqref{eqn:momfn} and used in the cartesian grid projection of equations~\eqref{it:FSPoisson}-\eqref{it:FSProject}. Cut-face velocities are flux matched from their cartesian counterparts as described in section~\ref{sec:discretediveqv}.

\subsection{Immersed Boundary Method for cross velocities:}

In this section we describe an alternative method based on direct forcing of velocities in cut-face underlaying cartesian faces. Target velocities on these faces are defined by flux matching cut-face velocities obtained using a wall function in the streamwise direction. 

\subsubsection{Tangential and Normal Velocity estimation for wall modeled IBM:} \label{sec:tgnmibm}

Consider the cut-face with centroid $d$ in figure~\ref{Fig:IBMcrossvel}. A normal probe in direction $\mathbf{\hat{n}}$ can be defined passing through point $d$ and intersecting the boundary in point $B$. Then an external point $ex$ is defined at a distance $\delta_{ex}$ of the order of the FDS grid size. The velocity components at $ex$ are interpolated using masked bilinear (trilinear in 3D) interpolation from fluid points $e_i$, $i=1,\dots,4$, obtaining $\mathbf{u}_{ex,abs}$. 
%
\begin{figure}[h]
      \centering
      \includegraphics[trim =70mm 45mm 60mm 30mm, clip,
       width=0.45\linewidth]{../../../fig/fds/Geometry_Figures/CutCellsIBSketchTgtVel.png}
      \caption{Target velocity $\hat{u}_d$ estimation using a wall model and interpolation.}
        \label{Fig:IBMcrossvel}
\end{figure}
%
Having the wall normal direction $\mathbf{\hat{n}}$, an orthonormal direction can be computed from:
%
\begin{equation}
    \mathbf{\hat{t}} = \frac{\mathbf{\hat{n}} \times \mathbf{u}_{ex}}{|| \mathbf{\hat{n}} \times \mathbf{u}_{ex} ||}
\end{equation}
%
where $\mathbf{u}_{ex}=\mathbf{u}_{ex,abs}-\mathbf{u}_{B}$ is the known relative velocity of the fluid respect to the boundary $B$. The streamwise velocity component is defined by the product $ \mathbf{\hat{s}}= \mathbf{\hat{t}} \times  \mathbf{\hat{n}}$ and streamwise and normal velocity components $u_{\mathbf{\hat{s}},ex}=\mathbf{u}_{ex} \cdot \mathbf{\hat{s}}$, $u_{\mathbf{\hat{n}},ex}=\mathbf{u}_{ex} \cdot \mathbf{\hat{n}}$. The local component in the $\mathbf{\hat{t}}$ direction is zero by construction.

The following step requires the definition of the velocity vector at the centroid point $d$. The components in the $\mathbf{\hat{n}}$ and $\mathbf{\hat{t}}$ directions are treated separately:
%
\begin{itemize}
   \item Linearly interpolate the normal component $\mathbf{u}_{B} \cdot \mathbf{\hat{n}}$ from the $B$ boundary, and $u_{\mathbf{\hat{n}},ex}$ at $ex$ point to $d$ location. This is called $u_{\mathbf{\hat{n}},d}$ in figure~\ref{Fig:IBMcrossvel}.
   \item Use wall model to estimate $u_{\mathbf{\hat{s}},d}$ from relative streamwise components at $B$, $ex$, and kinematic viscosity defined at the cell conatining $B$. The relative streamwise component at $B$ is taken to be zero.
\end{itemize} 
%
Finally, the target velocity component $\hat{u}_d$ at $d$ can be obtained projecting $u_{\mathbf{\hat{n}},d},u_{\mathbf{\hat{s}},d}+u_{B} \cdot \mathbf{\hat{s}}$ into the corresponding cartesian direction, and flux matched to the bearing cartesian face ($\hat{u}_E$ in figure~\ref{Fig:IBMcrossvel}). This final cartesian face velocity is used to build the IBM force. in the next section.

\subsubsection{IBM iteration scheme}

%This method uses a combination of finite difference discretization on regular \texttt{GASPHASE} and cut-cell underlying Eulerian cells, where both face velocities and $H$ at cell centers are defined. Transfer of velocities from \texttt{GASPHASE} cut-face centroids to Eulerian faces is done on a flux conserving manner, as explained before. 
The following iteration accounting for the projection is done, which falls naturally within the pressure iteration scheme described in section~\ref{section:pressure_iteration}:
%
\begin{enumerate}
   \item With a known velocity field $\mathbf{u}^{n+1,k-1}$ for the $k-1$ pressure iteration ($\mathbf{u}^{n+1,0}=\mathbf{u}^{n}$), use the method described in the previous section~\ref{sec:tgnmibm}, to define the cartesian face target velocities $\mathbf{\hat{u}}^{n+1,k-1}$. In \texttt{SOLID} cartesian faces inside the stationary geometry $\mathbf{\hat{u}}^{n+1,k-1}=0$.
   \item In IBM forcing faces recompute the force term:
%
\begin{equation}
   \mathbf{F}^{n,k} = - \frac{\mathbf{\hat{u}}^{n+1,k-1}-\mathbf{u}^{n}}{\Delta t} - \boldsymbol{\nabla} H^{n,k-1}
\end{equation}
%
   \item Solve Poisson equation:
%
\begin{equation}
\nabla \cdot \boldsymbol{\nabla} H^{n,k} = - \left[ \frac{\left( \nabla \cdot \mathbf{u}^{n+1} \right)^{th} - \nabla \cdot \mathbf{u}^{n}}{\Delta t} \right] - \nabla \cdot \mathbf{F}^{n,k}   
\end{equation}
%
  \item Obtain final velocity for $k$ sub-step:
%
\begin{equation}
  \mathbf{u}^{n+1,k} = \mathbf{u}^{n} - \Delta t \left[ \mathbf{F}^{n,k} +  \boldsymbol{\nabla} H^{n,k} \right]   
\end{equation}
% 
\end{enumerate}
The iteration in $k$ is conducted until velocities in solid faces differ from the target by less than a certain threshold (velocity error). 
Within the RK2 time integration scheme of FDS, we speedup the calculation process by fixing the target velocities $\mathbf{\hat{u}}^{n+1,k}$ to the value computed from previous step velocities $\mathbf{u}^n$ in the predictor, and to the value computed from the intermediate velocities $\mathbf{u}^*$ in the corrector.
...
%We employ the same diffusive flux definition as was done on the implicit discretization for scalar transport. Therefore:
%\begin{equation}
