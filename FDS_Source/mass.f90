...
CASE(.TRUE.) PREDICTOR_STEP
   ...
   IF (ISOTHERMAL .AND. N_TRACKED_SPECIES>0) THEN
      SM0 => SPECIES_MIXTURE(0)
      SM1 => SPECIES_MIXTURE(1)
      RCON_DIFF = SM1%RCON-SM0%RCON
      RCON_DIFF_ZZ = SM0%RCON
      DO N=2,N_TRACKED_SPECIES
         SM => SPECIES_MIXTURE(N)
         RCON_DIFF_ZZ = RCON_DIFF_ZZ + (SM%RCON-SM0%RCON)*ZZS(I,J,K,N)
      ENDDO
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
...
CASE(.FALSE.) PREDICTOR_STEP
   ...
   IF (ISOTHERMAL .AND. N_TRACKED_SPECIES>0) THEN
      ...
      DO N=2,N_TRACKED_SPECIES
         SM => SPECIES_MIXTURE(N)
         RCON_DIFF_ZZ = RCON_DIFF_ZZ + (SM%RCON-SM0%RCON)*ZZ(I,J,K,N)
      ENDDO
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
