...
ELSE
   ...
   IF (T>=PART_CLOCK(NM) .AND. PARTICLE_FILE) THEN
      CALL DUMP_PART(T,NM)
      DO
         PART_CLOCK(NM) = MIN(PART_CLOCK(NM) + DT_PART, T_END)
   ...
   IF (T>=ISOF_CLOCK(NM)) THEN
      ...
      DO
         ISOF_CLOCK(NM) = MIN(ISOF_CLOCK(NM) + DT_ISOF, T_END)
   ...
   IF (T>=SLCF_CLOCK(NM)) THEN
      ...
      DO
         SLCF_CLOCK(NM) = MIN(SLCF_CLOCK(NM) + DT_SLCF, T_END)
   ...
   IF (T>=SL3D_CLOCK(NM) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
      ...
      DO
         SL3D_CLOCK(NM) = MIN(SL3D_CLOCK(NM) + DT_SL3D, T_END)
   ...
   IF (T>=BNDF_CLOCK(NM)) THEN
      ...
      DO
         BNDF_CLOCK(NM) = MIN(BNDF_CLOCK(NM) + DT_BNDF, T_END)
   ...
   IF (T>=PL3D_CLOCK(NM) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
      ...
      DO
         PL3D_CLOCK(NM) = MIN(PL3D_CLOCK(NM) + DT_PL3D, T_END)
   ...
   IF (T>=PROF_CLOCK(NM)) THEN
      CALL DUMP_PROF(T,NM)
      DO
         PROF_CLOCK(NM) = MIN(PROF_CLOCK(NM) + DT_PROF, T_END)
