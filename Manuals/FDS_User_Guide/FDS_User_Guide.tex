...
\section{Surface Deposition of Aerosols}
\label{info:surface_depo}

FDS contains a model for the deposition of particulate matter, like soot, onto surfaces. Soot deposition can affect smoke detection time or visibility. It can also be used in the evaluation of a specific cause and origin hypothesis by comparing actual and simulated soot patterns following a fire.  Lastly a number of toxicants occur as aerosols (for example radionuclides or chemical or biological agents), and surface deposition is an important phenomena for evaluating dispersion.

Deposition is determined by applying an empirical deposition velocity to the near-wall aerosols.  There are a number of phenomena that cause deposition: thermophoresis (where temperature gradients push the aerosol towards or away from the wall), gravitational settling, diffusive deposition (where the aerosols move along the boundary layer concentration gradient), and turbulent deposition (essentially impact deposition due to a turbulent boundary layer).  Other phenomena such electrical fields can also result in deposition but are not considered in FDS.  The deposition velocity is determined by assuming the phenomena are independent, computing a deposition velocity for each (respectively u$_{th}$, u$_g$, and u$_{dt}$, a combined diffusion-turbulence velocity), and summing them~\cite{Bixler:1} .  By default FDS will compute all of the phenomena, but they can be selectively disabled with the logical flags {\ct GRAVITATIONAL\_DEPOSITION}, {\ct THERMOPHORETIC\_DEPOSITION}, and {\ct TURBULENT\_DEPOSITION}.

The gravitational velocity is given by~\cite{Davies_Charles}
\be
u_g = g m_a \frac{\text{Cn}}{6 \pi \, \chi_d \, \mu \, r_a}
\ee
where $\chi_d$ is a shape factor, r$_a$ is the particle radius, Cn is Cunningham's slip factor given by~\cite{Cunningham:1}, and m$_a$ is the particle mass which is computed using the radius and the {\ct DENSITY\_SOLID} (on {\ct SPEC}).
\be
\text{Cn} = 1 + 1.25 \; \text{Kn} + 0.41 \; \text{Kn} \; e^{-0.88/\text{Kn}}
\ee
where Kn is the particle Knudsen number given by the ratio of the mean free path of the gas to the particle radius ({\ct MEAN\_DIAMETER} on {\ct SPEC}) .  Since, the mean free path of a gas is proportional to its temperature, Kn is computed as~\cite{Sippola:1}
\be
...
\ee
where $\alpha$ is the ratio of the gas conductivity to the particle conductivity ({\ct CONDUCTIVITY\_SOLID} on {\ct SPEC}), C$_m$ is the momentum accommodation coefficient (1.14), C$_t$ is the thermal accommodation coefficient (2.18), and C$_s$ is the thermal slip coefficient (1.17).
...
\ee
A simple example is given in Section~\ref{info:deposition}.
...
This example shows how to define a reaction that invokes an aerosol deposition model in FDS (e.g. soot deposition).  Note, that this model should be considered an experimental feature. The deposition model is invoked by defining a species with the parameter {\ct AEROSOL=.TRUE.}.  For this example propane will be the fuel with a 5 \% soot yield.  The stoichiometric equation is shown below.
...
\nonumber \underbrace{1 \; \mathrm{(18.1182 \; N_2 \, + \, 2.8164 \; CO_2 \,+ \, 4\; H_2O)}}_\text{Products} \,+\,
...
\noindent
Note: The sign of {\ct NU} corresponds to whether that species is consumed (-) or produced (+). Figure~\ref{soot_deposition} shows a plot of the soot density on the wall in units of kg/m$^2$.  This boundary quantity is given by the input below.
...
{\ct N\_PARTICLES} to indicate the number of particles within the computational domain at the start of the simulation.  You may also use {\ct N\_PARTICLES\_PER\_CELL} to indicate the number of particles within each grid cell of the computational domain at the start of the simulation.
...
only a single particle, which is often a handy way of creating a target, you may use the real triplet {\ct XYZ} rather than {\ct XB} to designate the particle's position.  You can give this single particle an initial velocity using the real triplet {\ct UVW}.  When using {\ct N\_PARTICLES\_PER\_CELL}, the particles will be randomly placed within the volume of each cell.  If {\ct CELL\_CENTERED=.TRUE.} is set, then the particles will be placed at the center of each cell.
...
{\ct N\_PARTICLES} or {\ct N\_PARTICLES\_PER\_CELL} on the {\ct INIT} line. The default value of {\ct STATIC} is {\ct .FALSE.}
...
A 2D array of particles can be used to represent the drag given by a window screen.  A window screen is defined on {\ct INIT} by placing particles in a plane using {\ct N\_PARTICLES\_PER\_CELL=1}.  The particles should be defined with a {\ct SURF\_ID} containing the material properties of the screen.  A special drag law for screens is used by setting {\ct DRAG\_LAW} to {\ct 'SCREEN'}, {\ct ORIENTATION}, and {\ct FREE\_AREA\_FRACTION} on {\ct PART}.  {\ct ORIENTATION} is the surface normal to the screen and {\ct FREE\_AREA\_FRACTION} is the fraction of the screen's surface area that is open.  Pressure drop across a screen is given by
...
\be \hbox{K}= 3.44 \times 10^{-9} \; \hbox{FREE\_AREA\_FRACTION}^{1.6}\ee
\be \hbox{Y}= 0.043 \; \hbox{FREE\_AREA\_FRACTION}^{2.13}\ee

In the example below an aluminum screen with a 40 \% free area and an 0.0003 m wire diameter is placed normal to the x-axis.  Note that the {\ct LENGTH} on the {\ct SURF}, when multiplied by the diameter, results in 60 \% of the cell area.
\footnotesize
\begin{verbatim}

&MESH IJK=40,20,20,XB=0,2,0,1,0,1/

&SURF ID='SCREEN',THICKNESS=0.00015, GEOMETRY='CYLINDRICAL',LENGTH=0.5,CONVECTION_LENGTH_SCALE=0.0003,MATL_ID='ALUMINUM'/
&MATL ID='ALUMINUM',DENSITY=2700.,CONDUCTIVITY=200.,SPECIFIC_HEAT=900./
&PART DRAG_LAW='SCREEN',FREE_AREA_FRACTION=0.4,ID='SCREEN',STATIC=.TRUE.,SURF_ID='SCREEN',ORIENTATION=1,0,0/
&INIT N_PARTICLES_PER_CELL=1,CELL_CENTERED=.TRUE.,PART_ID='SCREEN',XB=1.01,1.01,0.0,1.0,0.0,1.0/
...
\section{Assigning Complex Properties to Particles}
...
However, it is possible to assign arbitrary surface properties to particles that represent subgrid-scale objects, like office clutter or vegetation. To do this, you add a {\ct SURF\_ID} to the {\ct PART} line. The
...
The output value of mathematical control functions are available for use by defining a {\ct DEVC} with {\ct QUANTITY='CONTROL'} and setting the {\ct CTRL\_ID} on the {\ct DEVC} input to the {\ct ID} of the control function.
...
{\ct GRAVITATIONAL\_DEPOSITION}                 & Logical       & Section~\ref{info:surface_depo}                       &               & {\ct .TRUE.}    \\ \hline
...
{\ct THERMOPHORETIC\_DEPOSITION}                & Logical       & Section~\ref{info:surface_depo}                       &               & {\ct .TRUE.}    \\ \hline
...
{\ct TURBULENT\_DEPOSITION}                     & Logical       & Section~\ref{info:surface_depo}                       &               & {\ct .TRUE.}    \\ \hline
...
{\ct CONDUCTIVITY\_SOLID}           & Real        & Section~\ref{info:surface_depo}         & W/m/K             &  0.26         \\ \hline
{\ct DENSITY\_LIQUID}               & Real        & Section~\ref{thermal_part_props}        & kg/m$^3$          &               \\ \hline
{\ct DENSITY\_SOLID}                & Real        & Section~\ref{info:surface_depo}         & kg/m$^3$          &  1800.        \\ \hline
...
{\ct MEAN\_DIAMETER}                & Real        & Section~\ref{info:surface_depo}         & m                 & 1.E-6         \\ \hline
