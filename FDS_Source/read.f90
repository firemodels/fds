...
INTEGER  :: N_SPEC_READ,N,NN,NS2,NR,N_SPEC_READ_2,N_SUB_SPECIES,NS,N_BINS
...
TRACKED_SPEC_LOOP_1: DO WHILE (N <= N_TRACKED_SPECIES .OR. .NOT. DEFINED_BACKGROUND)
   ...
   ELSE
      FIND_TRACKED: DO
         ...
         READ(LU_INPUT,NML=SPEC)
         N_SPEC_READ = N_SPEC_READ + 1
         IF (LUMPED_COMPONENT_ONLY .AND. N_PARTICLE_BINS < 0) CYCLE FIND_TRACKED
      ...
      IF (N_PARTICLE_BINS > 0) THEN
         DO NN=1,N_PARTICLE_BINS
            MEAN_DIAMETER = 2._EB*PARTICLE_RADIUS(NN)
            SPEC_ID(1)=SPECIES(AGGLOMERATION_INDEX)%ID         
            WRITE(ID,'(A,A,I0)') TRIM(SPECIES(AGGLOMERATION_INDEX)%ID),'_',NN
