...
SF%ONE_D_REALS_ARRAY_SIZE(3) = SF%N_CELLS+1             ! X
SF%ONE_D_REALS_ARRAY_SIZE(4) = SF%N_CELLS+2             ! TMP
SF%ONE_D_REALS_ARRAY_SIZE(5) = (SF%N_CELLS+2)*SF%N_MATL ! RHO
...
SUBROUTINE ALLOCATE_STORAGE(NM,SURF_INDEX,LPC_INDEX,WALL_INDEX,LP_INDEX,TAG)
...
INTEGER :: ARRAY_INDEX,STORAGE_INDEX,I1,I2,I,OLD_INDEX,REALS_DIMENSION,INTEGERS_DIMENSION,LOGICALS_DIMENSION,&
           STORAGE_INDEX_OLD,STORAGE_INDEX_OPEN
LOGICAL :: NEW_SLOT
...
!if (PRESENT(WALL_INDEX)) write(0,*) 're-allocate wall storage arrays, SURF_INDEX=', SURF_INDEX,' slots=',OS%N_STORAGE_SLOTS
!if (PRESENT(LP_INDEX)) write(0,*) 're-allocate part storage arrays', OS%N_STORAGE_SLOTS,nm
   ...
   ENDIF
   STORAGE_INDEX = OS%N_STORAGE_SLOTS + 1
   OS%N_STORAGE_SLOTS = OS%N_STORAGE_SLOTS+N_NEW_STORAGE_SLOTS
ENDIF

! Assign values to the pointer arrays

IF (PRESENT(WALL_INDEX)) CALL WALL_POINTERS(ARRAY_INDEX,NEW_SLOT)
IF (PRESENT(LP_INDEX)) CALL PARTICLE_POINTERS(ARRAY_INDEX,NEW_SLOT)
...
LP%SURF_INDEX          => OS%INTEGERS( 6,STORAGE_INDEX) ; IF (NEW) LP%SURF_INDEX = 0
LP%WALL_INDEX          => OS%INTEGERS( 7,STORAGE_INDEX) ; IF (NEW) LP%WALL_INDEX = 0
LP%VEG_N_TREE_OUTPUT   => OS%INTEGERS( 8,STORAGE_INDEX) ; IF (NEW) LP%VEG_N_TREE_OUTPUT = 0
...
LP%VEG_KAPPA         => OS%REALS(14,STORAGE_INDEX)
LP%VEG_EMISS         => OS%REALS(15,STORAGE_INDEX)
LP%VEG_DIVQC         => OS%REALS(16,STORAGE_INDEX)
LP%VEG_PACKING_RATIO => OS%REALS(17,STORAGE_INDEX)
LP%VEG_FUEL_MASS     => OS%REALS(18,STORAGE_INDEX)
LP%VEG_MOIST_MASS    => OS%REALS(19,STORAGE_INDEX)
LP%VEG_IGN_TON       => OS%REALS(20,STORAGE_INDEX)
LP%VEG_IGN_TOFF      => OS%REALS(21,STORAGE_INDEX)
LP%VEG_IGN_TRAMPON   => OS%REALS(22,STORAGE_INDEX)
LP%VEG_IGN_TRAMPOFF  => OS%REALS(23,STORAGE_INDEX)
LP%VEG_CHAR_MASS     => OS%REALS(24,STORAGE_INDEX)
LP%VEG_SV            => OS%REALS(25,STORAGE_INDEX)
LP%VEG_MLR           => OS%REALS(26,STORAGE_INDEX)
LP%VEG_VOLFRACTION   => OS%REALS(27,STORAGE_INDEX)
...
I1 = 11 ; I2 = I1 + SF%WALL_INTEGERS_ARRAY_SIZE(1) - 1
...
WC%YW                 => OS%REALS(12,STORAGE_INDEX)
WC%ZW                 => OS%REALS(13,STORAGE_INDEX)
WC%VEG_HEIGHT         => OS%REALS(14,STORAGE_INDEX)

I1 = 15   ; I2 = I1 + SF%WALL_REALS_ARRAY_SIZE(1) - 1  ; WC%A_LP_MPUA(1:)       => OS%REALS(I1:I2,STORAGE_INDEX)
...
I1 = IC+10 ; I2 = I1 + SF%ONE_D_INTEGERS_ARRAY_SIZE(1) - 1
...
ONE_D%SHRINKING       => OS%LOGICALS(LC+2,STORAGE_INDEX) ; IF (NEW) ONE_D%SHRINKING = .FALSE.
...
ONE_D%TMP_F           => OS%REALS(RC+10,STORAGE_INDEX) ; IF (NEW) ONE_D%TMP_F           = TMPA
ONE_D%TMP_B           => OS%REALS(RC+11,STORAGE_INDEX) ; IF (NEW) ONE_D%TMP_B           = TMPA
ONE_D%UW              => OS%REALS(RC+12,STORAGE_INDEX) ; IF (NEW) ONE_D%UW              = 0._EB
ONE_D%UWS             => OS%REALS(RC+13,STORAGE_INDEX) ; IF (NEW) ONE_D%UWS             = 0._EB

I1 = RC+14 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(1) - 1
...
IF (NEW) ONE_D%X(0:) = SF%X_S(0:SF%N_CELLS)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(4) - 1
ONE_D%TMP(0:) => OS%REALS(I1:I2,STORAGE_INDEX) 
IF (NEW) ONE_D%TMP(0:SF%N_CELLS+1) = SF%TMP_INNER(1)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(5) - 1
DUMMY(1:) => OS%REALS(I1:I2,STORAGE_INDEX)
ONE_D%RHO(0:SF%N_CELLS+1,1:SF%N_MATL) => DUMMY(1:I2-I1+1)              
IF (NEW) ONE_D%RHO(0:SF%N_CELLS+1,1:SF%N_MATL) = SF%RHO_0(0:SF%N_CELLS+1,1:SF%N_MATL)
...
!ENDIF
!
!IF (THICKNESS <= SQRT(DIFFUSIVITY)) THEN
!   DX_MIN = THICKNESS
!   N_CELLS = 1
!   RETURN
!ENDIF
!
!IF (ABS(STRETCH_FACTOR-1._EB) <= ZERO_P) THEN
!   N_CELLS = INT(THICKNESS/(CELL_SIZE_FACTOR*SQRT(DIFFUSIVITY)))+1
!   IF (N_CELLS > 999) N_CELLS = 999
!   DX_MIN = THICKNESS / REAL(N_CELLS,EB)
!ELSE
!   DX_MIN = CELL_SIZE_FACTOR*SQRT(DIFFUSIVITY)
!   DDSUM = 0._EB
!   DO I=1,998
!      DDSUM = DDSUM + STRETCH_FACTOR**(I-1)
!      IF (THICKNESS / DDSUM < DX_MIN) EXIT
!   ENDDO
!   DX_MIN = THICKNESS / DDSUM
!   N_CELLS = I
!ENDIF
...
PARTICLE_OR_WALL: IF (PRESENT(WALL_INDEX)) THEN !Wall surface density
   IWX = WALL_INDEX
   ...
   ELSE
      ...
      IF (SURFACE(SURF_INDEX)%SHRINK) THEN
         THICKNESS = SUM(M%WALL(IWX)%ONE_D%LAYER_THICKNESS)
         NWP = SUM(M%WALL(IWX)%ONE_D%N_LAYER_CELLS)
         X_S_NEW(0:NWP) = THICKNESS-M%WALL(IWX)%ONE_D%X(0:NWP)
      ELSE
      ...
      DO II2=1,NWP
         ...
         ENDIF
         SURFACE_DENSITY = SURFACE_DENSITY + SUM(M%WALL(IWX)%ONE_D%RHO(II2,:))*WGT
      ENDDO
   ENDIF

ELSEIF (PRESENT(LAGRANGIAN_PARTICLE_INDEX)) THEN PARTICLE_OR_WALL !Particle surface density
   IPX = LAGRANGIAN_PARTICLE_INDEX
   SURF_INDEX = LAGRANGIAN_PARTICLE_CLASS(M%LAGRANGIAN_PARTICLE(IPX)%CLASS_INDEX)%SURF_INDEX
   IF (.NOT.SURFACE(SURF_INDEX)%THERMALLY_THICK) THEN
      SURFACE_DENSITY = 0._EB
   ELSE
      SELECT CASE(SURFACE(SURF_INDEX)%GEOMETRY)
      ...
      END SELECT
      IF (SURFACE(SURF_INDEX)%SHRINK) THEN
         THICKNESS = SUM(M%LAGRANGIAN_PARTICLE(IPX)%ONE_D%LAYER_THICKNESS)
         NWP = SUM(M%LAGRANGIAN_PARTICLE(IPX)%ONE_D%N_LAYER_CELLS)
         X_S_NEW(0:NWP) = THICKNESS-M%LAGRANGIAN_PARTICLE(IPX)%ONE_D%X(0:NWP)
      ELSE
         THICKNESS = SUM(SURFACE(SURF_INDEX)%LAYER_THICKNESS)
         NWP = SURFACE(SURF_INDEX)%N_CELLS
         X_S_NEW(0:NWP) = THICKNESS-SURFACE(SURF_INDEX)%X_S(0:NWP)
      ...
      DO II2=1,NWP
         ...
         ELSE
            WGT = (X_S_NEW(II2-1)**I_GRAD-X_S_NEW(II2)**I_GRAD)/SURFACE(SURF_INDEX)%THICKNESS**I_GRAD
         ENDIF
         SURFACE_DENSITY = SURFACE_DENSITY + SUM(M%LAGRANGIAN_PARTICLE(IPX)%ONE_D%RHO(II2,:))*WGT
      ENDDO
   ENDIF
   
ENDIF PARTICLE_OR_WALL 
