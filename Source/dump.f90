      ...
      IF (.NOT. APPEND) THEN
         ...
         IF (SMOKE3D_VERSION==2) THEN
            SELECT CASE(SMOKE3D_FILE(N)%DISPLAY_TYPE)
               CASE('GAS')         ; WRITE(LU_SMOKE3D(N,NM)) 0._EB,1._EB
               CASE('HRRPUV')      ; WRITE(LU_SMOKE3D(N,NM)) 0._EB,HRRPUV_MAX_SMV
               CASE('TEMPERATURE') ; WRITE(LU_SMOKE3D(N,NM)) TEMP_MIN_SMV,TEMP_MAX_SMV
            END SELECT
         ENDIF
