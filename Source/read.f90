...
INTEGER :: NR,NR2,NS,NS2,NFR,PRIORITY,INT_PROD_COUNTER, NEW_REAC
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (A < 0._EB .AND. E < 0._EB .AND. TRIM(SPEC_ID_NU(1))=='null' .AND. TRIM(EQUATION)=='null') THEN
      IF (N_REACTIONS > MAX_GAS_REACTIONS) THEN
         WRITE(MESSAGE,'(A,I0,A)') 'ERROR: Too many simple chemistry reactions have been defined.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
IF (ANY(REACTION%N_SIMPLE_CHEMISTRY_REACTIONS==2)) THEN
   ...
   DO NR = 1,N_REACTIONS
      ...
      RN%PAIR_INDEX = NR
      RN%PRIORITY = 2
      IF (NR2 - N_REACTIONS==1) THEN
         RN%FUEL = 'INTERMEDIATE PRODUCTS'
      ELSE
         WRITE(RN%FUEL,'(A,I0)') 'INTERMEDIATE PRODUCTS ',NR2-N_REACTIONS
      ENDIF
   ENDDO
...
! ***** DELETE THIS CHECK ONCE GENERAL ARRAY LOGIC IS WORKING) *****
IF (SIMPLE_CHEMISTRY .AND. (N_REACTIONS > 1 .AND. COUNT(N_SIMPLE_CHEMISTRY_REACTIONS==1)==1) .OR. &
                           (N_REACTIONS > 2 .AND. COUNT(N_SIMPLE_CHEMISTRY_REACTIONS==2)==1)) THEN
...
DO NR=1,N_REACTIONS
   ...
   SELECT CASE(N_SIMPLE_CHEMISTRY_REACTIONS)
      ...
      CASE(2)
         ...
         RN%SPEC_ID_NU_READ(1) = RN%FUEL
         RN%SPEC_ID_NU_READ(2) = 'AIR'
         RN%SPEC_ID_NU_READ(3) = REACTION(NR+1)%FUEL
         RN%NU_READ(1)      = -1._EB
         RN%NU_READ(2)      = -RN%NU_O2/SPECIES_MIXTURE(1)%VOLUME_FRACTION(O2_INDEX)
         RN%NU_READ(3)      = -(RN%NU_READ(1)*SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW+RN%NU_READ(2)*SPECIES_MIXTURE(1)%MW)/ &
      ...
      CASE(-2)
         ...
         RN%SPEC_ID_NU_READ(1) = RN%FUEL
         RN%SPEC_ID_NU_READ(2) = 'AIR'
         IF (RN%FUEL=='INTERMEDIATE PRODUCTS') THEN
            RN%SPEC_ID_NU_READ(3) = 'PRODUCTS'
         ELSE
            RN%SPEC_ID_NU_READ(3) = 'PRODUCTS'//TRIM(RN%FUEL(21:))
         ENDIF
         RN%NU_READ(1)      = -RN2%NU_READ(3)
         RN%NU_READ(2)      = -(RN%NU_O2-RN2%NU_O2)/SPECIES_MIXTURE(1)%VOLUME_FRACTION(O2_INDEX)
         RN%NU_READ(3)      = -(RN2%NU_READ(1)*SPECIES_MIXTURE(RN2%FUEL_SMIX_INDEX)%MW+&
                                 (RN2%NU_READ(2)+RN%NU_READ(2))*SPECIES_MIXTURE(1)%MW)/SPECIES_MIXTURE(RN%PROD_SMIX_INDEX)%MW
         RN%N_SMIX          = 3
         RN%CHI_R           = RN2%CHI_R
   END SELECT
ENDDO
