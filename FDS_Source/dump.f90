   ...
   EDC_IF: IF (.NOT. EDC) THEN
      ...
      SELECT CASE(RN%MODE)
         ...
         CASE (EDDY_DISSIPATION)
            ...
            WRITE(LU_OUTPUT,'(A)') '   Species ID                     Stoich. Coeff.'
            DO NN=0,N_TRACKED_SPECIES
               IF (ABS(RN%NU(NN))>TWO_EPSILON_EB) WRITE(LU_OUTPUT,'(3X,A,1X,F11.5)') SPECIES_MIXTURE(NN)%ID,RN%NU(NN) 
            ENDDO        
            WRITE(LU_OUTPUT,'(/3X,A)') 'Detailed Species'
            WRITE(LU_OUTPUT,'(A)') '   Species ID                     Stoich. Coeff.'
            DO NN=1,N_SPECIES
...
REAL(EB) :: CONCORR,U2,V2,W2,HQ2,FLUX,DFLUXDT,SOLID_PHASE_OUTPUT_OLD,VOLSUM,C_S_ADJUST_UNITS,TF_MEAN
INTEGER :: IOR,II1,II2,II,JJ,KK,IIG,JJG,KKG,NN,NR,IWX,SURF_INDEX,I
LOGICAL :: T_ITERATIVE
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(37) ! PRESSURE COEFFICIENT TEST
      IIG = ONE_D%IIG
      JJG = ONE_D%JJG
      KKG = ONE_D%KKG
