...
INTEGER  :: IIG,JJG,KKG,IC2
REAL(EB) :: RE,H_NATURAL,H_FORCED,FRICTION_VELOCITY=0._EB,YPLUS=0._EB,ZSTAR,DN,TMP_FILM,MU_G,K_G,CP_G,&
            TMP_G,R_DROP,U_SURF,V_SURF,W_SURF,TMP_2,H_1,H_2,GAMMA,DTDX_W,RHO_G,ZZ_G(1:N_TRACKED_SPECIES),CONV_LENGTH
...
HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
   CASE(DEFAULT_HTC_MODEL)
      ...
      CALL FORCED_CONVECTION_MODEL(H_FORCED,RE,K_G,PR_ONTH,CONV_LENGTH,SFX%GEOMETRY)
      CALL NATURAL_CONVECTION_MODEL(H_NATURAL,DELTA_TMP,SURF_INDEX_IN,SFX%GEOMETRY,BCX%IOR,K_G,CONV_LENGTH)
   CASE(BOUNDARY_FUEL_HTC_MODEL)  ! Same as DEFAULT_HTC_MODEL but for a cylindrical object
      RE = RHO_G*ONE_DX%U_TANG*CONV_LENGTH/MU_G
      CALL FORCED_CONVECTION_MODEL(H_FORCED,RE,K_G,PR_ONTH,CONV_LENGTH,SURF_CYLINDRICAL)
      CALL NATURAL_CONVECTION_MODEL(H_NATURAL,DELTA_TMP,SURF_INDEX_IN,SURF_CYLINDRICAL,BCX%IOR,K_G,CONV_LENGTH)
