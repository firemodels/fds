...
\be \nabla \cdot \bu = \frac{1}{\rho} \sum_\alpha \frac{\overline{W}}{W_\alpha} \nabla \cdot \rho D_\alpha \nabla Y_\alpha +
  \frac{1}{\rho \, c_p \, T} \left( \nabla \cdot k \nabla T +
...
\be \nabla \cdot \bu = \frac{1}{\rho \, c_p \, T}
   \sum_\alpha \nabla h_{s,\alpha} \cdot \rho D_\alpha \nabla Y_\alpha = \frac{c_{p,1}-c_{p,2}}{\rho \, c_p \, T} \, \nabla T \cdot \rho D \nabla Y_1
  \label{div3} \ee
...
For most problems involving fire or combustion, \emph{the} most important parameter of a model is the total heat release.  It is therefore extremely important to assure that the correct amount of fuel is injected into the FDS computational domain.  Below we present a series of test cases for the species boundary conditions.
...
\section{Yield of combustion products from a Methane flame (Methane\_flame)}

A simple methane flame with 10 \% CO yield is burned inside a closed cavity for 5 seconds.
The total mass of CO$_2$, H$_2$O and CO is measured.
The same simulations is performed using the three alternative ways
to define the combustion reaction: simple chemistry, primitive species and lumped species. The
same amount of species should be produced in each case. The following two figures compare the
mass of three combustion products. First figure shows the comparison simple vs. primitive and the second for simple vs. lumped.
\begin{figure}[h]
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/Methane_flame_simple_primitive}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/Methane_flame_simple_lumped}
\end{tabular*}
\caption[Yield of combustion products using three alternative definitions of the same chemistry.]{The cumulation of combustion product mass using three alternative definitions of the combustion chemistry. (Left) Simple chemistry vs. primitive species. (Right) Simple chemistry vs. lumped species. }
\label{fig_methane_flame_yields}
\end{figure}

\section{Reaction Rate Cases (ReactionRate\_)}

Similar to the mass flux boundary condition, the gas phase reaction model is important to FDS' ability to determine total heat release. Below are a series of test cases designed to check the reaction rate mechanism for mixing controlled reactions, finite rate reactions, and mixed mode reactions. 

\subsection{Mixing-controlled reactions (ReactionRate\_MixCtrl\_)}

The mixing-controlled (mixed is burned) reaction model with local extinction is the default reaction mechanism in FDS. The reaction rate is determined by the eddy dissipation concept (EDC). The EDC says that the mean local species consumption rate for each reaction is determined from the local mean fuel, oxidizer, and product mass fractions and the turbulent mixing time by the following equation \cite{Poinsot:TNC}:

\begin{equation}\label{eq:EDC}
\bar{k}_{i}(\tilde{Y},\tau_{mix})=C_{mag}\frac{1}{\tau_{mix}}min(\tilde{Y}_{fuel},\frac{\tilde{Y}_{ox}}{s},\beta\frac{\tilde{Y}_{prod}}{(1+s)})
\end{equation}
where $C_{mag}$ and $\beta$ are model constants and $s$ is determined from the stoichiometry of the reaction. This model can be considered a deficient model as the consumption rate is found by the limiting species. To determine species evolution for the reaction, a system of ordinary differential equations is solved for each participating species:

\begin{equation}\label{eq:spec_ode}
\frac{dY_{i}}{dt}=R_{i}  \; \text{for 1 to i tracked species}
\end{equation}
where $R_i=\nu_{i}\frac{W_{i}}{W_{fuel}}\bar{k}_{i}$ is the reaction rate for the $i^{th}$ species. To test this implementation, a 1-step methane reaction

\begin{equation}\label{eq:1step_methane}
CH_4+2O_2 \rightarrow  CO_2+2H_2O
\end{equation}
is considered for both a fuel limiting and oxygen limiting case with a fixed mixing time, $\tau_{mix}=0.1s$.

\begin{figure}[ht]
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_MixCtrl_Flim_1Step} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_MixCtrl_O2lim_1Step}
\end{tabular*}
\caption[Species Evolution in Mixing Controlled Reaction]{Time evolution of species mass fraction for a 1-step mixing controlled reaction with a fixed mixing time of $\tau_{mix}=0.1s$. (Left) Fuel limited reaction. (Right) Oxygen limited reaction.}
\label{fig:MixCtrl_1Step}
\end{figure}

Similar to the 1-step reaction, the EDC reaction implementation was tested for a 2-step methane reaction

\begin{eqnarray}\label{eq:2step_methane}
CH_4+1.5O_2 \rightarrow  CO+2H_2O \\
\nonumber CO+0.5O_2 \rightarrow CO_2
\end{eqnarray}
for both fuel limited and oxygen limited conditions with a fixed mixing time, $\tau_{mix}=0.1s$.

\begin{figure}[ht]
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_MixCtrl_Flim_2Step} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_MixCtrl_O2lim_2Step}
\end{tabular*}
\caption[Species Evolution in Mixing Controlled Reaction]{Time evolution of species mass fraction for a 2-step mixing controlled reaction with a fixed mixing time of $\tau_{mix}=0.1s$. (Left) Fuel limited reaction. (Right) Oxygen limited reaction.}
\label{fig:MixCtrl_2Step}
\end{figure}

\subsection{Finite rate reactions (ReactionRate\_Arrhenius\_)}

The solution process for species evolution in finite rate reactions is similar to mixing-controlled reactions in that species evolution is found by solving a system of ordinary equations (\ref{eq:spec_ode}). The main difference between the mechanisms is that instead of the reaction rate being determined by the turbulent mixing time as part of the EDC, the reaction rate in finite rate reactions is found using an Arrhenius reaction equation:

\begin{equation}\label{eq:FR_Arr}
\bar{k}_{i}=-A\prod(Y_{i}^{a_{i}})T^{n_{i}}\exp(-E/RT)
\end{equation}
where A, E, $a_{i}$, and $n_{i}$ are user defined inputs based on the specific reaction. Note that since Equation \ref{eq:FR_Arr} has an exponential term, the integration of Equation Set \ref{eq:spec_ode} is solved using a Runge-Kutta 2 mechanism as compared to a Explicit Euler scheme used in the mixing controlled test case. 

Here we consider two 1-step cases, a zero-order and second-order reaction.  First, a 1-step, zero-order test case of carbon monoxide oxidation:

\begin{equation}\label{eq:1step_CO}
CO+0.5O_2 \rightarrow  CO_2
\end{equation}

In this case, $E$, $a_{i}$, and $n_{i}$ are set to zero, and $A$ is set such that $\bar{k}_{i} = 1$. The resulting reaction rates simplify to a function of molecular weight and stoichiometric coefficients.

\begin{figure}[ht]
\begin{center}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_Arrhenius_0Order_1Step}
\end{center}
\caption[Species Evolution in Finite Rate Reaction]{Time evolution of species mass fraction for a 1-step zero order Arrhenius finite rate reaction.}
\label{fig:Arrhenius_0Order_1step}
\end{figure} 
The second test case is a 1-step, second-order propane reaction.  

\begin{equation}\label{eq:1step_propane}
C_3H_8+5O_2 \rightarrow  3CO_2+4H_2O
\end{equation}
$E$ and $n_{i}$ are still set to zero, but $a_{i}=[1,1,0,0]$  for propane, oxygen, carbon monoxide, and water vapor respectively, making the reaction second-order ($\mathcal{O}=\sum a_{i}$).

\begin{figure}[ht]
\begin{center}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_Arrhenius_2Order_1Step}
\end{center}
\caption[Species Evolution in Finite Rate Reaction]{Time evolution of species mass fraction for a 1-step second order Arrhenius finite rate reaction.}
\label{fig:Arrhenius_2Order_1step}
\end{figure}

Multi-step Arrhenius finite rate reactions are also tested. A 2-step propane reaction:
\begin{eqnarray}\label{eq:2step_propane}
C_3H_8+3.5O_2 \rightarrow  3CO+4H_2O \\
\nonumber CO+0.5O_2 \rightarrow CO_2
\end{eqnarray}
and a 3-step propane reaction reaction:
\begin{eqnarray}\label{eq:3step_propane}
C_3H_8+3.5O_2 \rightarrow  3CO+4H_2O \\
\nonumber CO+0.5O_2 \leftrightarrow CO_2
\end{eqnarray}
 are considered. Equation \ref{eq:3step_propane} includes the decomposition of $CO_2$ into $CO$ and $O_2$ which the 2-step does not. The reaction order for the first two steps in both cases is 1.75. In the first reaction $a_{i}=[0.1,1.65,0,0]$ for propane, oxygen, carbon monoxide, and water vapor respectively. In the second (forward) reaction $a_{i}=[1,0.25,0,0.5]$ for carbon monoxide, oxygen, carbon dioxide, and water vapor respectively. Note that in the second step, water vapor contributes to the reaction rate based on its local concentration despite not being a participant in the reaction. For the reverse reaction in Equation \ref{eq:3step_propane}, $a_{i}=[1,0,0]$ for carbon dioxide, carbon monoxide, and oxygen respectively.
 
 \begin{figure}[ht]
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_Arrhenius_1-75Order_2Step} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/ReactionRate_Arrhenius_1-75Order_3Step}
\end{tabular*}
\caption[Species Evolution in Mixing Controlled Reaction]{Time evolution of species mass fraction for mulit-step Arrhenius finite rate propane reactions. (Left) 2-step. (Right) 3-step.}
\label{fig:MixCtrl_2Step}
\end{figure}
...
This chapter contains verification cases that test all aspects of particles, droplets, sprays, and so on. Note that in FDS, lagrangian particles are used for
a variety of purposes, not just water droplets.

\section{Momentum Transfer (\texorpdfstring{{\bf particle\_drag}}{particle\_drag})}
\label{particle_drag}
...
\be \left( 4.184 \; \hbox{kJ/kg/K} \times 80 \; \hbox{K} + 2492 \; \hbox{kJ/kg} \right) \times 0.05 \; \hbox{kg/s} = 141.3 \; \hbox{kW} \ee
...
\ee
FDS ran the terminal velocity case over several time steps. The precise configuration of the scenario is detailed in the table below.
The error of this terminal velocity and the L$_\infty$ norm of position error are computed and plotted for each time step.
\begin{center}
\begin{tabular}{|r|c|l|}
\hline
...
\end{tabular*}
\caption[Convergence analysis of terminal velocity test case.]{(Left) FDS exhibits second-order convergence for the particle velocity in the terminal velocity case. (Right) FDS exhibits second-order convergence for the particle position in the terminal velocity case.}
...
There are no known solutions of the drag equations in higher dimensions. However, ballisticians developed an approximate solution useful for computing trajectories of projectiles when guns are shot within a certain range of angles. This solution makes use of the ``flat fire'' assumption. In short, if a particle is launched with its velocity in only the $x$-direction and its $y$-velocity is never very significant compared against the $x$-velocity, then throughout the entire trajectory $u$ is a good approximation for $|\mathbf{u}|$.
...
%\end{align*}
To ensure that the flat fire assumption is valid, the following ICs are used: $x(0) = 0$, $\dot{x}(0) = u(0) = V_0$, $y(0) = h$, $\dot{y}(0) = v(0) = 0$. The exact solution to this system is below.
\be
	x = \frac{\text{log}(V_0 K t + 1)}{K} \quad ; \quad
	y = h + \frac{g \text{log}(V_0 K t + 1)}{2 (V_0 K)^2} - \frac{g t^2}{4} - \frac{g t}{2 V_0 K} \quad ; \quad
	K \equiv \frac{3 \rho_a C_d}{4 \rho_d D_d}
\ee
\be
	u = \frac{d x}{d t} = \frac{V_0}{V_0 K t + 1} \quad ; \quad v = \frac{d y}{d t} = \frac{g}{2 V_0 K (V_0 K t + 1)} - \frac{g t}{2} - \frac{g}{2 V_0 K}
\ee
...
\end{tabular}
\end{center}
As can be seen in figure \ref{flat_fire_plots}, FDS agrees well with this approximate solution. The flat fire solution should have slightly faster $v$-velocities at extended times because the speed of the particle is approximated with $u$, which is lower than the true speed at extended times. This ensures the drag force is lower than it should be, and consequently, the $v$-velocity is too fast. In the lower left plot in \ref{flat_fire_plots} the $v$-velocity from FDS is slightly slower than the flat fire solution.
\begin{figure}[p]
\noindent
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
...
flux related to details of the numerical solver.  The results are shown in Fig.~\ref{leak_test}.
...
Two compartments are defined: A 5 m x 4 m x 3. 2 m compartment containing a second 3.6 m x 2.4 m x 2.4 m compartment, see Figure~\ref{fig_HVAC_leak_fan}.
The interior compartment is connected to the exterior compartment both by a fan (quadratic fan curve with a maximum volumetric flow of 0.005 m$^3$/s and a
maximum pressure of 1000 Pa) and by an 0.0001 m$^2$ leakage path.  The interior compartment is also connected to the ambient by an 0.0001 m$^2$ leakage path.
The fan is defined with a 1 s tanh ramp to reach full power.  When the simulation starts the fan will begin to remove air from the outer compartment and discharge it into the
inner compartment.  As the pressure rises, the leakage flows will slowly increase until the leakage flow is equal to the flow from the fan.
...
\begin{figure}[ht]
   ...
   \end{tabular*}
   \caption{Vector plot of pressure at 10 s with geometry clipped to show compartment interiors (left) and line plot of zone pressures (right)}
