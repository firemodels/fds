...
IF (N_SPECIES > 0) THEN
   ...
   DO N=1,N_SPECIES
      ...
      IF (SPECIES(N)%MODE==MIXTURE_FRACTION_SPECIES) Z_SUM = Z_SUM + SPECIES(N)%YY0   
      IF (SPECIES(N)%MODE==GAS_SPECIES) THEN 
         Y_SUM = Y_SUM + SPECIES(N)%YY0
         RSUM = RSUM + (SPECIES(N)%RCON - SPECIES(0)%RCON)*SPECIES(N)%YY0
      ENDIF
   ENDDO
ENDIF
...
INTEGER N,NN,I_OBST,I,J,K,IBCX,IIG,JJG,KKG,IW,II,JJ,KK,ICG,IL
LOGICAL :: VENT_FOUND
