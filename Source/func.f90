...
!> \param T_OFFSET Offset time (s)
!> \param FUNCTION_TYPE Type of time function

SUBROUTINE SPECIAL_RAMP_FUNCTIONS(SIGMA,TAU,T_OFFSET,FUNCTION_TYPE)
...
REAL(EB), INTENT(IN) :: SIGMA,TAU,T_OFFSET
REAL(EB) :: LCC, DT_THETA
INTEGER :: I
CHARACTER(*), INTENT(IN) :: FUNCTION_TYPE
...
SELECT CASE(FUNCTION_TYPE)
   CASE('RANDOM')
      ...
      ENDDO
   CASE('SIN')
      DO I=1,RRP%NUMBER_DATA_POINTS
         RRP%INDEPENDENT_DATA(I) = T_BEGIN + REAL(I-1,EB)*DT_THETA
         RRP%DEPENDENT_DATA(I)   = SIN(TWOPI*(RRP%INDEPENDENT_DATA(I)-T_OFFSET)/TAU)
      ENDDO
END SELECT

END SUBROUTINE SPECIAL_RAMP_FUNCTIONS
