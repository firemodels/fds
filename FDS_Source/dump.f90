...
!      GAS_PHASE_OUTPUT = SUM(ZZ(II,JJ,KK,I_Z_MIN:I_Z_MAX))
   ...
   CASE(46:48)  ! SPECIFIC ENTHALPY and ENTHALPY and AVERAGE SPECIFIC HEAT
      ...
      IF (IND==47 .AND. .NOT.ENTHALPY_TRANSPORT) GAS_PHASE_OUTPUT = GAS_PHASE_OUTPUT * RHO(II,JJ,KK)
      IF (IND==47 .AND.      ENTHALPY_TRANSPORT) GAS_PHASE_OUTPUT = E(II,JJ,KK)*0.001_EB
   CASE(49)  ! ENTHALPY ERROR
      IF (.NOT.ENTHALPY_TRANSPORT) THEN
         WRITE(MESSAGE,'(A)') "ERROR: Cannot DUMP ENTHALPY ERROR, ENTHALPY_TRANSPORT=.FALSE."
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR,TMP(II,JJ,KK))
      GAS_PHASE_OUTPUT = (E(II,JJ,KK)-RHO(II,JJ,KK)*CPBAR*TMP(II,JJ,KK))*0.001_EB
