...
Equations (\ref{eq:alpha2}) and ({\ref{eq:color}) are equivalent to the recursions presented in Ref.~\cite[Chapter 39]{gpugems} for performing volume rendering.
...
\section{A Solution using Slices}
...
When planes are skipped, (\ref{eq:alphahat}) may be simplified.  In particular, when every 2nd plane is skipped, $\Delta\hat{x}/\Delta x=2$, so that (\ref{eq:alphahat}) simplifies to
...
A general transfer function is used to map data values to color. Several examples are given in Figure \ref{fig:colormaps}.  The black to white colormap could be used to simulate the effect of a thermal imager by using black for cool temperatures and white for warm temperatures.  Colors are chosen in the black body temperature colormap to match the colors emitted by a black body source between the temperatures of 1000~K and 2800~K.  The rainbow colormap produces non-realistic images.  To produce a realistic image, the strategy is to perform color mixing in the same way that light would behave by solving an approximate form of the radiation transport equation.  The approximate RTE solution given in (\ref{eq:rtesoln})  is used to collapse 3D data into 2D images projected onto the sides of the mesh visible to the observer.

\begin{figure}[\figoptions]
\begin{center}
\begin{tabular}{c}
\includegraphics[width=5.0in]{figures/colorbar_blackbody_1000K_2800K}\\
a) black body temperature curve 1000~K to 2800~K\\
...
Figure \ref{fig:volplume_example} illustrates this process showing three images.  The top center is presented from the point of view of the observer.  The two images below show the scene rotated left and right to show more clearly the images projected on the front facing sides of the data mesh.  As the scene is moved through rotations and translations the projected surface images are constantly recomputed and redrawn presenting the illusion that the image is 3D and drawn within (rather than on the surface) of the data mesh.
...
A limitation of the volume rendering procedure is the large file sizes required to store the full precision being used in computations.  As with the slice rendering method, compression procedures are implemented using the ZLIB library~\cite{ZLIB} to reduce this size.
