...
!$    SHARED(KBAR,JBAR,IBAR,SOLID,CELL_INDEX,N_TRACKED_SPECIES,ZZP,MU,TMP,STORE_MU_DNS,MU_DNS, &
!$           LES,EVACUATION_ONLY,NM,CSMAG,TWO_D,DX,DY,DZ,USE_MAX_FILTER_WIDTH,RDX,RDY,RDZ,UU,VV,WW,DYNSMAG,RHOP,CSD2_DYNSMAG, &
!$           N_EXTERNAL_WALL_CELLS,N_INTERNAL_WALL_CELLS,BOUNDARY_TYPE,IJKW,KRES, &
!$           IBP1,JBP1,KBP1,FISHPAK_BC) &
!$    PRIVATE(CS)

!$OMP DO COLLAPSE(3) SCHEDULE(STATIC) &
!$    PRIVATE(K,J,I,ZZ_GET) 
...
IF (LES .OR. EVACUATION_ONLY(NM)) THEN
   ...
   !$OMP DO COLLAPSE(3) SCHEDULE(STATIC) &
   !$    PRIVATE(K,J,I,DELTA,DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,S12,S13,S23,SS)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
...
!$    PRIVATE(IW,II,JJ,KK,IIG,JJG,KKG,ZZ_GET,MU_WALL)
...
!$    SHARED(N_EXTERNAL_WALL_CELLS,N_INTERNAL_WALL_CELLS,IJKW,OMESH,PREDICTOR,MU,KRES,D,DS) &
!$    PRIVATE(IW,II,JJ,KK,NOM,MU_OTHER,DP_OTHER,KRES_OTHER,KKO,JJO,IIO,N_INT_CELLS)
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(KBAR,JBAR,IBAR,RDXN,RDYN,RDZN,UU,VV,WW,OMX,OMY,OMZ,MU,TXY,TXZ,TYZ, &
!$           IMMERSED_BOUNDARY_METHOD,IBM_SAVE1,IBM_SAVE2,IBM_SAVE3,IBM_SAVE4,IBM_SAVE5,IBM_SAVE6, &
!$           SPATIAL_GRAVITY_VARIATION,GX,GY,GZ,T,DUMMY,I_RAMP_GX,I_RAMP_GY,I_RAMP_GZ,GVEC,X, &
!$           CELL_INDEX,EDGE_INDEX,OME_E,TAU_E,RHOP,RHO_0,RDX,RDY,RDZ,DP,FVEC,FVX,FVY,FVZ, &
!$           MEAN_FORCING,SOLID,U_MASK,V_MASK,W_MASK,DX,DY,DZ,DXN,DYN,DZN,INTEGRAL,SUM_VOLUME, &
!$           DU_FORCING,DV_FORCING,DW_FORCING,RFAC_FORCING, &
!$           U0,V0,W0,UMEAN,VMEAN,WMEAN,DT)

! Compute vorticity and stress tensor components

!$OMP DO COLLAPSE(3) SCHEDULE(STATIC) &
!$    PRIVATE(K,J,I,DUDY,DVDX,DUDZ,DWDX,DVDZ,DWDY,MUX,MUY,MUZ)
DO K=0,KBAR
   DO J=0,JBAR
      DO I=0,IBAR
...
MEAN_FORCING_X: IF (MEAN_FORCING(1)) THEN
   ...
   !$OMP END SINGLE
   !$OMP DO COLLAPSE(3) SCHEDULE(STATIC) &
   !$    PRIVATE(K,J,I,IC1,IC2,DVOLUME) REDUCTION(+:INTEGRAL,SUM_VOLUME)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=0,IBAR
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(KBAR,JBAR,IBAR,RDZN,RDXN,UU,WW,OMY,MU,TXZ, &
!$           GX,GZ,XS,T,DUMMY,I_RAMP_GZ,GVEC,I0,J, &
!$           CELL_INDEX,EDGE_INDEX,OME_E,TAU_E,RHOP,RHO_0,RDX,RDZ,DP,R,FVX,FVZ,RRN)

!$OMP DO COLLAPSE(3) SCHEDULE(STATIC) &
!$    PRIVATE(K,J,I,DUDZ,DWDX,MUY)  
DO K=0,KBAR
   DO J=0,JBAR
      DO I=0,IBAR
         DUDZ = RDZN(K)*(UU(I,J,K+1)-UU(I,J,K))
...
!$    PRIVATE(K,I,WP,WM,OMYP,OMYM,TXZP,TXZM,IC,IEYP,IEYM,WOMY,RRHO,AH,DWDZ,TXXP,TXXM,DTXXDX,DTXZDZ,DMUDX,VTRM) 
DO K= 1,KBAR
   DO I=I0,IBAR
      WP    = WW(I,J,K)   + WW(I+1,J,K)
      WM    = WW(I,J,K-1) + WW(I+1,J,K-1)
...
!$OMP DO COLLAPSE(2) SCHEDULE(STATIC) &
!$    PRIVATE(K,I,UP,UM,OMYP,OMYM,TXZP,TXZM,IC,IEYP,IEYM,UOMY,RRHO,AH,DUDX,TZZP,TZZM,DTXZDX,DTZZDZ,VTRM)
DO K=0,KBAR
   DO I=1,IBAR
      UP    = UU(I,J,K)   + UU(I,J,K+1)
      UM    = UU(I-1,J,K) + UU(I-1,J,K+1)
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(PRES_METHOD,N_EXTERNAL_WALL_CELLS,IJKW,PREDICTOR,CORRECTOR,OMESH,H,HS, &
!$           N_OBST,OBSTRUCTION,CELL_INDEX,SOLID,RFODT,U,V,W,US,VS,WS,FVX,FVY,FVZ,RDXN,RDYN,RDZN,HP, &
!$           N_INTERNAL_WALL_CELLS,BOUNDARY_TYPE,UWS,UW)
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(KBAR,IBAR,JBAR,US,VS,WS,U,V,W,DT,FVX,FVY,FVZ,RDXN,RDYN,RDZN,H, &
!$           KRES)
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(KBAR,JBAR,IBAR,U,V,W,US,VS,WS,DT,FVX,FVY,FVZ,RDXN,RDYN,RDZN,HS, &
!$           KRES)
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(TAU_E,OME_E, &
!$           N_EDGES,EDGE_TYPE,EXTERIOR,IJKE,SOLID,UU,VV,WW,DY,DX,DZ,MU, &
!$           WALL_INDEX,I_CELL,J_CELL,K_CELL,BOUNDARY_TYPE,NOM,IJKW,SURFACE,VENT_INDEX,VENTS, &
!$           T_BEGIN,T,ZC,GROUND_LEVEL, &
!$           PERIODIC_TEST,X,Z,CELL_INDEX,TMP,MU_Z,SPECIES_MIXTURE,RHOP,DUMMY,OMESH,PREDICTOR,CORRECTOR, &
!$           EDGE_INTERPOLATION_FACTOR,IBAR,JBAR,KBAR,U_Y,U_Z,V_X,V_Z,W_X,W_Y,UVW_GHOST)
...
!$    PRIVATE(IE,II,JJ,KK,IEC,ICMM,ICPM,ICMP,ICPP,NOM,IIO,JJO,KKO,UUP,UUM,DXX,MUA,I_SGN,IS,IOR,ICD,IVL,ICD_SGN,IBCM,IBCP,ITMP, &
!$            VEL_GAS,VEL_GHOST,IWP,IWM,SF,VELOCITY_BC_INDEX,TSI,PROFILE_FACTOR,RAMP_T,VEL_T,IIGM,JJGM,KKGM,IIGP,JJGP,KKGP, &
!$            RHO_WALL,MU_WALL,OM,VEL_OTHER,WGT,OMW,ICDO,MU_DUIDXJ,DUIDXJ,DUIDXJ_0,MU_DUIDXJ_0,MU_DUIDXJ_USE,DUIDXJ_USE, &
!$            PROCESS_EDGE,ALTERED_GRADIENT,SLIP_COEF,ICDO_SGN,DUMMY,SYNTHETIC_EDDY_METHOD,VT,VEL_EDDY)
...
!$    SHARED(N_EXTERNAL_WALL_CELLS,BOUNDARY_TYPE,IJKW,OMESH,MESHES,PREDICTOR,CORRECTOR, &
!$           D_CORR,DS_CORR,UU,VV,WW,RDX,RDY,RDZ,UVW_SAVE,IBAR,JBAR,KBAR) &
!$    PRIVATE(IW,II,JJ,KK,IOR,NOM,OM,M2,DA_OTHER,OM_UU,OM_VV,OM_WW,KKO,JJO,IIO, &
!$            UU_OTHER,VV_OTHER,WW_OTHER,UU_AVG,VV_AVG,WW_AVG)
...
SELECT_VELOCITY_NORM: SELECT CASE (CFL_VELOCITY_NORM)
   CASE(0)
      P_UVWMAX = UVWMAX
      !$OMP PARALLEL DEFAULT(NONE) & 
      !$    SHARED(UVWMAX,ICFL,JCFL,KCFL,UU,VV,WW,RDXN,RDYN,RDZN,IBAR,JBAR,KBAR,DP) &
      !$    PRIVATE(P_ICFL,P_JCFL,P_KCFL) &
      !$    FIRSTPRIVATE(P_UVWMAX) 
   ...
   CASE(3)
      P_UVWMAX = UVWMAX
      !$OMP PARALLEL DEFAULT(NONE) &
      !$    SHARED(UVWMAX,ICFL,JCFL,KCFL,FVX,FVY,FVZ,RDXN,RDYN,RDZN,IBAR,JBAR,KBAR,DP) &
      !$    PRIVATE(P_ICFL,P_JCFL,P_KCFL) &
      !$    FIRSTPRIVATE(P_UVWMAX) 
...
PARABOLIC_IF: IF (DNS .OR. CHECK_VN) THEN
   ...
   MUP => MU
   !$OMP PARALLEL DEFAULT(NONE) &
   !$    SHARED(KBAR,IBAR,JBAR,SOLID,CELL_INDEX,MUP,RHOP,MU_MAX,I_VN,J_VN,K_VN) &
   !$    PRIVATE(P_I_VN,P_J_VN,P_K_VN,P_MU_TMP) &
   !$    FIRSTPRIVATE(P_MU_MAX)
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(KBP1,JBP1,IBP1,RHMK,RHOP,HP,KRES,RRHO, &
!$           N_EXTERNAL_WALL_CELLS,BOUNDARY_TYPE,VENT_INDEX,VENTS,TW,T_BEGIN,T,DUMMY,IJKW,UU,VV,WW, &
!$           KBAR,JBAR,IBAR,CELL_INDEX,SOLID,FVX,FVY,FVZ,RDXN,RDYN,RDZN,TWO_D)
...
!$OMP DO SCHEDULE(STATIC) &
!$    PRIVATE(IW,VT,TSI,TIME_RAMP_FACTOR,P_EXTERNAL,II,JJ,KK,IOR,IIG,JJG,KKG,INFLOW)
EXTERNAL_WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS
