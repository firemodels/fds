...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   WC => M%WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)
   IF (SF%THERMAL_BC_INDEX/=THERMALLY_THICK .OR. SF%BACKING/=EXPOSED) CYCLE
   ...
   IF (PROCESS(NOM)/=MY_RANK) THEN
      IF (COUNT(WALL_SAVE(NOM)%CELL_INDICES(WALL_SAVE(NOM)%START:WALL_SAVE(NOM)%COUNTER)==ONE_D%BACK_INDEX)>0) THEN
         ...
         DO II=WALL_SAVE(NOM)%START,WALL_SAVE(NOM)%COUNTER
            IF (WALL_SAVE(NOM)%CELL_INDICES(II)==ONE_D%BACK_INDEX) ONE_D%BACK_INDEX = II
         ENDDO
         CYCLE
      ENDIF
      IF (WALL_SAVE(NOM)%COUNTER==WALL_SAVE(NOM)%DIMEN) CALL REALLOCATE_WALL_SAVE
      WALL_SAVE(NOM)%COUNTER = WALL_SAVE(NOM)%COUNTER + 1
      WALL_SAVE(NOM)%CELL_INDICES(WALL_SAVE(NOM)%COUNTER) = ONE_D%BACK_INDEX
      WALL_SAVE(NOM)%SURF_INDICES(WALL_SAVE(NOM)%COUNTER) = ONE_D%BACK_SURF
      ONE_D%BACK_INDEX = WALL_SAVE(NOM)%COUNTER
   ENDIF
ENDDO
...
PRIMARY_WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (SF%HT_DIM==1) CYCLE PRIMARY_WALL_LOOP
