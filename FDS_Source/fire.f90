...
USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_GAS_CONSTANT,GET_MASS_FRACTION_ALL,GET_SPECIFIC_HEAT,GET_MOLECULAR_WEIGHT, &
                              GET_SENSIBLE_ENTHALPY,IS_REALIZABLE
INTEGER :: I,J,K,NS,NR,II,JJ,KK,IIG,JJG,KKG,IW,N
REAL(EB), INTENT(IN) :: T,DT
REAL(EB):: ZZ_GET(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),R_CP_T,CP,H_S_N
LOGICAL :: Q_EXISTS
      ...
      ILOOP: DO I=1,IBAR
         ...
         DZZ_IF: IF ( ANY(ABS(DZZ) > TWO_EPSILON_EB) ) THEN
            IF (ABS(Q(I,J,K)) > TWO_EPSILON_EB) Q_EXISTS = .TRUE.
               ...
               CALL GET_SENSIBLE_ENTHALPY(DZZ,H_S_N,TMP(I,J,K))
               D_SOURCE(I,J,K) = D_SOURCE(I,J,K) + (DOT_PRODUCT(SPECIES_MIXTURE%RCON,DZZ)/RSUM(I,J,K)-H_S_N/(CP*TMP(I,J,K)))/DT
               M_DOT_PPP(I,J,K,:) = M_DOT_PPP(I,J,K,:) + RHO(I,J,K)*DZZ(:)/DT
         ENDIF DZZ_IF
      ENDDO ILOOP
   ENDDO
ENDDO
