...
SPECIES_LOOP: DO N=1,N_GAS_SPECIES
   ...
   !$OMP DO COLLAPSE(3) PRIVATE(K,J,I,TMP_G,HDIFF)
   DO K=0,KBAR
      DO J=0,JBAR
         DO I=0,IBAR
            ! H_RHO_D_DYDX
   ...
   !$OMP SINGLE PRIVATE(IW,IIG,JJG,KKG,IOR,TMP_G,HDIFF,RHO_D_DYDN)
   !!$OMP DO PRIVATE(IW,IIG,JJG,KKG,IOR,TMP_G,HDIFF,RHO_D_DYDN)
   ...
   SPECIES_DIFFUSION_2: IF (SPECIES(Y2SPEC(N))%MODE/=LUMPED_SPECIES) THEN
      !$OMP DO COLLAPSE(3) PRIVATE(K,J,I,HDIFF)
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
...
IF (N_GAS_SPECIES==0 .OR. EVACUATION_ONLY(NM)) THEN
  !$OMP PARALLEL DO PRIVATE(K,J,I,ITMP,TMP_WGT)
   ...
   !$OMP END PARALLEL DO
ELSE
   !$OMP PARALLEL DO PRIVATE(K,J,I,YY_GET)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
