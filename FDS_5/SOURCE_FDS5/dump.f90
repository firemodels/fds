...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS, &
       TIMINGS,FLUSH_BUFFERS,READ_RESTART,WRITE_DIAGNOSTICS, &
...
DO N=0,N_SURF
   ...
   IF (SF%THERMALLY_THICK) THEN
      ...
      WRITE(LU_SMV,'(2F8.1)') TMPM,ML%EMISSIVITY
   ELSE
      WRITE(LU_SMV,'(2F8.1)') 5000.,1.0
   ENDIF
   WRITE(LU_SMV,'(I2,6F8.2)') SF%SURF_TYPE,SF%TEXTURE_WIDTH,SF%TEXTURE_HEIGHT,REAL(SF%RGB,FB)/255._FB,SF%TRANSPARENCY
...
EVAC_ONLY3: IF (.NOT. ALL(EVACUATION_ONLY)) THEN
   DO N=1,N_PART
      ...
      WRITE(LU_SMV,'(3F8.2)') REAL(PC%RGB,FB)/255._FB
...
EVAC_ONLY4: IF (ANY(EVACUATION_GRID)) THEN
   DO N=1,N_EVAC
      ...
      WRITE(LU_SMV,'(3F8.2)') REAL(EVAC_CLASS_RGB(:,N),FB)/255._EB
...
WRITE(LU_SMV,'(6F10.2,I3)') XMIN,YMIN,ZMIN,XMAX,YMIN,ZMIN,N_TICKS
WRITE(LU_SMV,'(6F10.3)') TICK_LENGTH,-2.,-1.,-1.,-1.,TICK_WIDTH
WRITE(LU_SMV,'(A)') 'TICKS'
WRITE(LU_SMV,'(6F10.2,I3)') XMIN,YMIN,ZMIN,XMIN,YMIN,ZMIN,1
WRITE(LU_SMV,'(6F10.3)') XMAX-XMIN,1.,-1.,-1.,-1.,TICK_WIDTH
 
DO I=0,N_TICKS-1
   WRITE(LU_SMV,'(A)') 'LABEL'
   WRITE(LU_SMV,'(8F10.3)') XMIN+I*X_INT,YMIN-2._EB*TICK_LENGTH,ZMIN,-1.,-1.,-1.,0._EB,T_END
   CALL TRIM_LABEL(XMIN+I*X_INT,LABEL)
   WRITE(LU_SMV,'(A)') TRIM(LABEL)
ENDDO
 
WRITE(LU_SMV,'(/A)') 'TICKS'  ! y-axis
WRITE(LU_SMV,'(6F10.2,I3)') XMIN,YMIN,ZMIN,XMIN,YMAX,ZMIN,N_TICKS
WRITE(LU_SMV,'(6F10.3)') TICK_LENGTH,-1.,-1.,-1.,-1.,TICK_WIDTH
WRITE(LU_SMV,'(A)') 'TICKS'
WRITE(LU_SMV,'(6F10.2,I3)') XMIN,YMIN,ZMIN,XMIN,YMIN,ZMIN,1
...
DO I=0,N_TICKS-1
   WRITE(LU_SMV,'(A)') 'LABEL'
   WRITE(LU_SMV,'(8F10.3)') XMIN-2._EB*TICK_LENGTH,YMIN+I*Y_INT,ZMIN,-1.,-1.,-1.,0.,T_END
   CALL TRIM_LABEL(YMIN+I*Y_INT,LABEL)
   WRITE(LU_SMV,'(A)') TRIM(LABEL)
ENDDO
 
WRITE(LU_SMV,'(/A)') 'TICKS'  ! z-axis
WRITE(LU_SMV,'(6F10.2,I3)') XMIN,YMIN,ZMIN,XMIN,YMIN,ZMAX,N_TICKS
WRITE(LU_SMV,'(6F10.3)') TICK_LENGTH,-1.,-1.,-1.,-1.,TICK_WIDTH
WRITE(LU_SMV,'(A)') 'TICKS'
WRITE(LU_SMV,'(6F10.2,I3)') XMIN,YMIN,ZMIN,XMIN,YMIN,ZMIN,1
...
DO I=0,N_TICKS-1
   WRITE(LU_SMV,'(A)') 'LABEL'
   WRITE(LU_SMV,'(8F10.3)') XMIN-2._EB*TICK_LENGTH,YMIN,ZMIN+I*Z_INT, -1.,-1.,-1.,0.,T_END
...
WRITE(LU_SMV,'(3F8.3)') (TEX_ORI(I),I=1,3)
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   WRITE(LU_SMV,'(/A)') 'PDIM'
   WRITE(LU_SMV,'(9F12.5)') M%XS,M%XF,M%YS,M%YF,M%ZS,M%ZF,(REAL(M%RGB(I),FB)/255._FB,I = 1,3)
   ...
   DO N=1,M%N_OBST
      ...
      ELSE
         WRITE(LU_SMV,'(6F12.5,I7,6I4,3F8.2)') OB%X1,OB%X2,OB%Y1,OB%Y2,OB%Z1,OB%Z2,OB%ORDINAL, &
   ...
   DO N=1,M%N_OBST
      OB=>M%OBSTRUCTION(N)
      IF (OB%BCI/=-3) THEN
         WRITE(LU_SMV,'(8I5)') OB%I1,OB%I2,OB%J1,OB%J2,OB%K1,OB%K2,OB%BCI,OB%BTI
      ELSE
         WRITE(LU_SMV,'(8I5,4F6.2)') OB%I1,OB%I2,OB%J1,OB%J2,OB%K1,OB%K2,OB%BCI,OB%BTI, REAL(OB%RGB,FB)/255._FB, OB%TRANSPARENCY
      ENDIF
   ENDDO
   ...
   DO N=1,M%N_VENT
      ...
      ELSE
         WRITE(LU_SMV,'(6F12.5,I6,I4,3F8.2)') VT%X1,VT%X2,VT%Y1,VT%Y2,VT%Z1,VT%Z2,VT%ORDINAL,VT%IBC, &
   ...
   DO N=1,M%N_VENT
      ...
      IF (VT%BOUNDARY_TYPE/=OPEN_BOUNDARY) INDX =  VT%VCI
      IF (VT%RGB(1)<0) THEN
         WRITE(LU_SMV,'(8I5)') VT%I1,VT%I2,VT%J1,VT%J2,VT%K1,VT%K2,INDX,VT%VTI
      ELSE
         WRITE(LU_SMV,'(8I5,4F6.2)') VT%I1,VT%I2,VT%J1,VT%J2,VT%K1,VT%K2,INDX,VT%VTI, REAL(VT%RGB,FB)/255._FB,VT%TRANSPARENCY
      ENDIF
   ENDDO
   DO N=1,NDV
      INDX = 99
...
DO NM=1,NMESHES
WRITE(LU_SMV,'(F8.1)') HRRPUA_SHEET/(7._EB*MESHES(NM)%DXMIN)
ENDDO
...
DO N=1,N_DEVC
   ...
   WRITE(LU_SMV,'(3F12.5,3F6.2)') DV%X,DV%Y,DV%Z,DV%ORIENTATION(1:3)
ENDDO
 
! Flush the .smv file

!!! CALL FLUSH_BUFFER(LU_SMV)
CLOSE(LU_SMV)
OPEN(LU_SMV,FILE=FN_SMV,FORM='FORMATTED', STATUS='OLD',POSITION='APPEND')
...
DO N=1,N_DEVC
   DV => DEVICE(N)
   WRITE(LU_CORE(NM)) DV%INSTANT_VALUE,DV%VALUE,DV%T_CHANGE,DV%TMP_L,DV%Y_C,DV%COUNT,DV%CURRENT_STATE,DV%PRIOR_STATE
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   WRITE(LU_CORE(NM)) M2%TMP,M2%U,M2%V,M2%W,M2%H
...
DO N=1,N_DEVC
   DV => DEVICE(N)
   READ(LU_RESTART(NM)) DV%INSTANT_VALUE,DV%VALUE,DV%T_CHANGE,DV%TMP_L,DV%Y_C,DV%COUNT,DV%CURRENT_STATE,DV%PRIOR_STATE
   IF (DV%QUANTITY=='spot obscuration') THEN
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   IF (NIC(NM,NOM)==0) CYCLE OTHER_MESH_LOOP
   M2=>MESHES(NM)%OMESH(NOM)
   READ(LU_RESTART(NM)) M2%TMP,M2%U,M2%V,M2%W,M2%H
...
DATA_LOOP: DO DATAFLAG=1,2
   ...
   CALL SMOKE3DTOFILE(TRIM(FN_SMOKE3D(DATAFLAG,NM))//CHAR(0), STIME,DXX,EXTCOEF,DATAFLAG, &
...
ELSE
   ...
   WRITE(LU_DEVC(1),TCFORM) T,(DEVICE(N)%VALUE/REAL(DEVICE(N)%COUNT,EB), N=1,N_DEVC)
ENDIF
...
ELSE
   ...
   WRITE(LU_CTRL(1),TCFORM) T,(WRITE_VALUE(N), N=1,N_CTRL)
ENDIF
...
PROF_LOOP: DO N=1,N_PROF
   ...
   IF (PF%QUANTITY == 'TEMPERATURE') THEN
      WRITE(LU_PROF(N),TCFORM) T,NWP+1, &
      (X_S_NEW(I),I=0,NWP),&
...
!!!CALL FLUSH_BUFFER(LU_HRR)
   ...
   OPEN(LU_HRR,FILE=FN_HRR,FORM='FORMATTED', STATUS='OLD',POSITION='APPEND')
IF (MASS_FILE) THEN
 !!! CALL FLUSH_BUFFER(LU_MASS)
   CLOSE(LU_MASS)
   OPEN(LU_MASS,FILE=FN_MASS,FORM='FORMATTED', STATUS='OLD',POSITION='APPEND')
ENDIF

DO N=1,N_PROF
!!!CALL FLUSH_BUFFER(LU_PROF(N))
   CLOSE(LU_PROF(N))
   OPEN(LU_PROF(N),FILE=FN_PROF(N),FORM='FORMATTED', STATUS='OLD',POSITION='APPEND')
ENDDO
...
DO NM=1,NMESHES
 ...
 !!!  CALL FLUSH_BUFFER(LU_SLCF(N,NM))
      CLOSE(LU_SLCF(N,NM))
      OPEN(LU_SLCF(N,NM),FILE=FN_SLCF(N,NM),FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
   ENDDO
   DO N=1,N_BNDF
 !!!  INQUIRE(UNIT=LU_BNDF(N,NM),OPENED=EX)
 !!!  IF (EX) CALL FLUSH_BUFFER(LU_BNDF(N,NM))
      CLOSE(LU_BNDF(N,NM))
      OPEN(LU_BNDF(N,NM),FILE=FN_BNDF(N,NM),FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
   ENDDO
ENDDO

IF (ANY(EVACUATION_GRID)) CALL FLUSH_BUFFER(LU_EVACCSV)
IF (ANY(EVACUATION_GRID)) CALL FLUSH_BUFFER(LU_EVACEFF)
IF (ANY(EVACUATION_GRID)) CALL FLUSH_BUFFER(LU_EVACFED)
