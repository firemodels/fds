...
REAL(EB) :: Y_FU_0,A,ETRM,Y_O2_0,Y_CO_0,DYF,DX_FDT,HFAC_F,DTT,DELTA2,& 
            ...
            ZETA,CS2,SS,KSGS,H_F_0,H_F_N,H_G_0,H_G_N,DYAIR,DELTAH_F
...
INTEGER :: NODETS,I,J,K,II,JJ,KK,IOR,IC,IW,IWA(-3:3),TURBULENT_TIME_SCALE=0,ITMP,ICFT
INTEGER, PARAMETER :: IEDC=1,IEBU=2
...
!$OMP PRIVATE(Y_O2_CORR,Y_F_CORR,DYF,DELTA2,Q_BOUND_1,Q_BOUND_2,Q_NEW,CRIT_FLAME_TMPA,YY_GET,ITMP,DYAIR) &
!$OMP PRIVATE(TURBULENT_TIME_SCALE,KSGS)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         IC = CELL_INDEX(I,J,K)
         ...
         IF_SUPPRESSION: IF (SUPPRESSION) THEN  ! Get maximum O2 in the current and neighboring cells to see if flame viable
            ...
            IF (EXTINCTION2) THEN
               ...
               CALL GET_AVERAGE_SPECIFIC_HEAT(YY_GET,H_F_N,ICFT)
               YY_GET = YY(I,J,K,:)
               YY_GET(I_FUEL) = 0._EB
...
USE PHYSICAL_FUNCTIONS, ONLY : GET_MASS_FRACTION,GET_SPECIFIC_GAS_CONSTANT,GET_AVERAGE_SPECIFIC_HEAT
REAL(EB) :: Y_FU_0,A,ETRM,Y_O2_0,Y_CO_0,DYF,DX_FDT,HFAC_F,DTT,DELTA2,& 
            Y_O2_MAX,TMP_MIN,Y_O2_CORR,Q_NEW,Q_OLD,F_TO_CO,DELTAH_CO,DELTAH_F,DYCO,HFAC_CO,RHOX,H_F_0,H_F_N,H_G_0,H_G_N, &
            X_FU,X_O2,X_FU_0,X_O2_0,X_FU_S,X_O2_S,X_FU_N,X_O2_N,CO_TO_O2, DYAIR, &
            Y_FU_MAX,TMP_F_MIN,Y_F_CORR,Z_2_MIN,Z_2_MIN_FAC,Q_BOUND_1,Q_BOUND_2,YY_GET(1:N_SPECIES)
REAL(EB), PARAMETER :: Y_FU_MIN=1.E-10_EB,Y_O2_MIN=1.E-10_EB,X_O2_MIN=1.E-16_EB,X_FU_MIN=1.E-16_EB,Y_CO_MIN=1.E-10_EB
