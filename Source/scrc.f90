...
#else
         IF (TYPE_MGM_LAPLACE == NSCARC_MGM_LAPLACE_PARDISO) CALL SCARC_ERROR (NSCARC_ERROR_MGM_PARDISO, SCARC_NONE, NSCARC_NONE)
#endif
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   ! Store MGM statistics for chid.out
   ...
   SELECT CASE (NTYPE)
      ...
      ! Termination - Reset to Krylov statistics of Poisson solution for statistics in chid.out

      CASE (-1)
         CAPPA = MGM%CAPPA_POISSON    
         ITE   = MGM%ITE_POISSON
