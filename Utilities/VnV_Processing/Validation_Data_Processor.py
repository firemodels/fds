...
#speciÞed. They are cycled independently and need not be of the same length. It should be noted that the def- 
#inition of mystyle in this example ensures that there is only one instance of the deÞnition of symbolattrs. 
#Putting an explicit deÞnition of symbolattrs in each call to plot would not lead to the desired result 
#because each time a new instance would be created which then starts with the Þrst item in the changelist. 
...
#would like to change between Þlled and open symbols, one can make use of a predeÞned changelist 
#mystyle = graph.style.symbol( 
#graph.style.symbol.changetriangletwice, 
