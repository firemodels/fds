...
\Huge
\flushright{Fire Dynamics Simulator (Version 5) \\ Verification \& Validation Guide \\ \LARGE Volume 1: Verification \\ (Draft, April 30, 2007)}
...
SourceForge.net, an Internet-based development
environment. SourceForge is a widely used service designed to assist
...
access to the FDS repository maintained at SourceForge.net. In
...
CVS control on SourceForge. The figures are either in the form of PDF or jpeg files, depending on whether they are vector or
...
Changes are made to the FDS source code daily, and tracked via CVS
revision control software, a feature of SourceForge. However, these
...
7600~m$^2$/kg results in an optical depth of 0.546~m$^{-1}$.  The compartment has a series of obstructions placed at varying depths that are
multiples of 1~m.  Using the correlation for the output quantity {\ct visibility}, one obtains a visibility distance of 5.5~m.
When viewing the smoke levels with Smokeview, one can just barely see the sixth obstacle which is at a distance of 6~m.
...
Using the optical depth of 0.546~m$^{-1}$ and the path length of 10~m, the expected total obscuration is 99.58~\%, which is the
...
\be
  \mathrm{ 180 \; \frac{L}{min} \times 1 \; \frac{kg}{L} \times \frac{1}{60} \; \frac{min}{s} \times 600 \; s = 1800 \; kg }
\ee
