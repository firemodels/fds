...
T_LOOP_1: DO J=1,500
   ...
   SPECIES_LOOP_1: DO N = 0,N_SPECIES
      SS => SPECIES(N)
      IF(MIXTURE_FRACTION .AND. SS%INDEX <= N_STATE_SPECIES) CYCLE SPECIES_LOOP_1
      SELECT CASE(SS%ID)
         CASE DEFAULT
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_MOLECULAR_WEIGHT
REAL(EB) :: TEMPERATURE,DENSITY,MASS_FRACTION(MAX_SPECIES),RR_SUM
INTEGER  :: N,NN
...
INIT_LOOP: DO N=1,N_INIT
   ...
   CALL GET_MOLECULAR_WEIGHT(IN%MASS_FRACTION,RR_SUM)
   RR_SUM = R0/RR_SUM
