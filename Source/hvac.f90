...
TYPE (BOUNDARY_PROP1_TYPE), POINTER :: P1
...
CFACE_LOOP: DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   CFA => CFACE(ICF)
   P1 => BOUNDARY_PROP1(CFA%P1_INDEX)
...
ZONE_LEAK_IF: IF (ALL(SF%LEAK_PATH < 0)) THEN
   ...
   KK = BC%KKG
   AREA = P1%AREA
   IF (P1%PRESSURE_ZONE /= NODE_ZONE(NODE_INDEX,NM)) THEN
      IF (NODE_ZONE(NODE_INDEX,NM) == 0) THEN
         NODE_ZONE(NODE_INDEX,NM) = P1%PRESSURE_ZONE
      ELSE
   ...
   IF (HVAC_LOCAL_PRESSURE) THEN
      SELECT CASE (IOR)
         CASE DEFAULT
            P_AVE = PBARP(KK,P1%PRESSURE_ZONE)
         CASE (3)
            P_AVE = 0.5_EB*(PBARP(KK-1,P1%PRESSURE_ZONE)+PBARP(KK,P1%PRESSURE_ZONE))
         CASE (-3)
            P_AVE = 0.5_EB*(PBARP(KK,P1%PRESSURE_ZONE)+PBARP(KK+1,P1%PRESSURE_ZONE))
      END SELECT
      ...
      SELECT CASE (IOR)
         CASE (1)
            NODE_P(NODE_INDEX,NM) = NODE_P(NODE_INDEX,NM) + &
                                       (PBARP(KK,P1%PRESSURE_ZONE)-RHO(II,JJ,KK) * &
                                       0.5_EB*(UP(II-1,JJ,KK)+P1%U_NORMAL)**2 * &
                                       SIGN(1._EB,UP(II-1,JJ,KK)+P1%U_NORMAL))* AREA
         CASE(-1)
            NODE_P(NODE_INDEX,NM) = NODE_P(NODE_INDEX,NM) + &
                                       (PBARP(KK,P1%PRESSURE_ZONE)+RHO(II,JJ,KK) * &
                                       0.5_EB*(UP(II,JJ,KK)-P1%U_NORMAL)**2  * &
                                       SIGN(1._EB,UP(II,JJ,KK)-P1%U_NORMAL))*AREA
         CASE (2)
            NODE_P(NODE_INDEX,NM) = NODE_P(NODE_INDEX,NM) + &
                                       (PBARP(KK,P1%PRESSURE_ZONE)-RHO(II,JJ,KK) * &
                                       0.5_EB*(VP(II,JJ-1,KK)+P1%U_NORMAL)**2 * &
                                       SIGN(1._EB,VP(II,JJ-1,KK)+P1%U_NORMAL))*AREA
         CASE(-2)
            NODE_P(NODE_INDEX,NM) = NODE_P(NODE_INDEX,NM) + &
                                       (PBARP(KK,P1%PRESSURE_ZONE)+RHO(II,JJ,KK) * &
                                       0.5_EB*(VP(II,JJ,KK)-P1%U_NORMAL)**2 * &
                                       SIGN(1._EB,VP(II,JJ,KK)-P1%U_NORMAL))*AREA
         CASE (3)
            NODE_P(NODE_INDEX,NM) = NODE_P(NODE_INDEX,NM) + &
                                       (PBARP(KK,P1%PRESSURE_ZONE)-RHO(II,JJ,KK) * &
                                       0.5_EB*(WP(II,JJ,KK-1)+P1%U_NORMAL)**2 * &
                                       SIGN(1._EB,WP(II,JJ,KK-1)+P1%U_NORMAL))*AREA
         CASE (-3)
            NODE_P(NODE_INDEX,NM) = NODE_P(NODE_INDEX,NM) + &
                                       (PBARP(KK,P1%PRESSURE_ZONE)+RHO(II,JJ,KK) * &
                                       0.5_EB*(WP(II,JJ,KK)-P1%U_NORMAL)**2 * &
                                       SIGN(1._EB,WP(II,JJ,KK)-P1%U_NORMAL))*AREA
      END SELECT
   ENDIF

ELSE ZONE_LEAK_IF

   IF (ALL(SF%LEAK_PATH/=P1%PRESSURE_ZONE)) RETURN
   IF (SF%LEAK_PATH(1) == P1%PRESSURE_ZONE) THEN
      IZ1 = SF%LEAK_PATH(1)
      IZ2 = SF%LEAK_PATH(2)
   ...
   ENDIF
   IF (NODE_ZONE(NODE_INDEX,NM) == 0) NODE_ZONE(NODE_INDEX,NM) = P1%PRESSURE_ZONE
   IOR = BC%IOR
   II = BC%IIG
   JJ = BC%JJG
   KK = BC%KKG
