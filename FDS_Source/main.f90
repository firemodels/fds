...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (N_TRACKED_SPECIES>0) THEN
      ALLOCATE(M%OMESH(NOM)%  YY(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,N_TRACKED_SPECIES))
      ALLOCATE(M%OMESH(NOM)% YYS(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,N_TRACKED_SPECIES))
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZATION_IF: IF (CODE==0) THEN
      ...
      IF (M3%NIC_S>0) THEN
         ...
         ALLOCATE(M3%REAL_RECV_PKG1(IJK_SIZE*(3+N_TRACKED_SPECIES)))
         ALLOCATE(M3%REAL_RECV_PKG2(IJK_SIZE*(5          )))
         ALLOCATE(M3%REAL_RECV_PKG3(IJK_SIZE*(3+N_TRACKED_SPECIES)))
...
DO NOM=1,NMESHES
   ...
   IF (CODE==6 .AND. EXCHANGE_EVACUATION .AND. MYID==EVAC_PROCESS .AND. .NOT.EVACUATION_ONLY(NOM)) THEN
      ...
      IF (N_TRACKED_SPECIES>0) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M4%YY(0,0,0,1),IWW*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
...
RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZE_SEND_IF: IF (CODE==0) THEN
      ...
      IF (M3%NIC_R>0 .AND. RNODE/=SNODE) THEN
         NRA = NUMBER_RADIATION_ANGLES
         NSB = NUMBER_SPECTRAL_BANDS
         ALLOCATE(M3%REAL_SEND_PKG1(IJK_SIZE*(3+N_TRACKED_SPECIES)))
         ALLOCATE(M3%REAL_SEND_PKG2(IJK_SIZE*(5          )))
         ALLOCATE(M3%REAL_SEND_PKG3(IJK_SIZE*(3+N_TRACKED_SPECIES)))
         ...
         DO KK=KMIN,KMAX
            DO JJ=JMIN,JMAX
               DO II=IMIN,IMAX
                  ...
                  IF (N_TRACKED_SPECIES>0) M3%REAL_SEND_PKG1(LL+4:LL+3+N_TRACKED_SPECIES) = M%YYS(II,JJ,KK,1:N_TRACKED_SPECIES)
                  LL = LL+3+N_TRACKED_SPECIES
               ENDDO
            ENDDO
         ENDDO
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M3%REAL_SEND_PKG1(1),IJK_SIZE*(3+N_TRACKED_SPECIES),MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,&
                        REQ(N_REQ),IERR)
      ELSE
         ...
         IF (N_TRACKED_SPECIES>0) M2%YYS(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_TRACKED_SPECIES)= &
                               M%YYS(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_TRACKED_SPECIES)
      ENDIF
   ENDIF
   ...
   IF (CODE==4 .AND. M3%NIC_R>0) THEN
         ...
         DO KK=KMIN,KMAX
            DO JJ=JMIN,JMAX
               DO II=IMIN,IMAX
                  ...
                  IF (N_TRACKED_SPECIES>0) M3%REAL_SEND_PKG3(LL+4:LL+3+N_TRACKED_SPECIES) = M%YY(II,JJ,KK,1:N_TRACKED_SPECIES)
                  LL = LL+3+N_TRACKED_SPECIES
               ENDDO
            ENDDO
         ENDDO
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),IJK_SIZE*(3+N_TRACKED_SPECIES),MPI_DOUBLE_PRECISION,SNODE,TAG,&
      ...
      ELSE
         ...
         IF (N_TRACKED_SPECIES>0) M2%YY(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_TRACKED_SPECIES)= &
                               M%YY(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_TRACKED_SPECIES)
      ENDIF
   ENDIF
...
DO NM=1,NMESHES
   ...
   IF (USE_MPI .AND. CODE==6 .AND. EXCHANGE_EVACUATION .AND. MYID/=EVAC_PROCESS .AND. PROCESS(NM)==MYID .AND. &
      ...
      IF (N_TRACKED_SPECIES>0) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M%YY(0,0,0,1),IWW*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,EVAC_PROCESS,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      ...
      DO KK=KMIN,KMAX
         DO JJ=JMIN,JMAX
            DO II=IMIN,IMAX
               ...
               IF (N_TRACKED_SPECIES>0) M2%YYS(II,JJ,KK,1:N_TRACKED_SPECIES)= M2%REAL_RECV_PKG1(LL+4:LL+3+N_TRACKED_SPECIES)
               LL = LL+3+N_TRACKED_SPECIES
            ENDDO
         ENDDO
      ENDDO
      ...
      DO KK=KMIN,KMAX
         DO JJ=JMIN,JMAX
            DO II=IMIN,IMAX
               ...
               IF (N_TRACKED_SPECIES>0) M2%YY(II,JJ,KK,1:N_TRACKED_SPECIES)= M2%REAL_RECV_PKG3(LL+4:LL+3+N_TRACKED_SPECIES)
               LL = LL+3+N_TRACKED_SPECIES
            ENDDO
         ENDDO
      ENDDO
...
DO NM=1,NMESHES
   IF (USE_MPI .AND. MYID==EVAC_PROCESS .AND. .NOT.EVACUATION_ONLY(NM)) THEN
      ...
      IF (N_TRACKED_SPECIES>0) THEN
         ALLOCATE(M%YY(0:M%IBP1,0:M%JBP1,0:M%KBP1,N_TRACKED_SPECIES),STAT=IZERO)
...
CALL MPI_GATHERV(REAL_BUFFER_7(1,1,DISPLS(MYID)+1),COUNTS_HVAC(MYID)*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION, &
                 NODE_YY,COUNTS_HVAC*N_TRACKED_SPECIES,DISPLS_HVAC*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
...
IF (LEAK_DUCTS > 0) THEN
   ...
   CALL MPI_GATHERV(REAL_BUFFER_9(0,0,1,DISPLS(MYID)+1),COUNTS_LEAK(MYID)*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION, &
                    LEAK_YY,COUNTS_LEAK*N_TRACKED_SPECIES,DISPLS_LEAK*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF
