...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      ...
      CASE(SOLID_BOUNDARY)
         ...
         IF (SIM_MODE/=DNS_MODE) THEN
            ...
            SELECT CASE(SF%NEAR_WALL_TURB_MODEL)
               ...
               CASE(CONSMAG) ! Constant Smagorinsky with Van Driest damping
                  VDF = 1._EB-EXP(-P2%Y_PLUS*RAPLUS)
         ...
         CALL WALL_MODEL(SLIP_COEF,P2%U_TAU,P2%Y_PLUS,MU_DNS(IIG,JJG,KKG)/RHO(IIG,JJG,KKG),SF%ROUGHNESS,&
                         0.5_EB/P1%RDN,VEL_GAS-VEL_T)
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (NOM/=0 .OR. WC%BOUNDARY_TYPE==SOLID_BOUNDARY .OR. WC%BOUNDARY_TYPE==NULL_BOUNDARY) THEN
      P1 => BOUNDARY_PROP1(WC%P1_INDEX)
      IF (PREDICTOR) THEN
         UN = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL_S
      ELSE
         UN = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL
      ENDIF
      SELECT CASE(IOR)
         CASE( 1)
            IF (PREDICTOR) THEN
...
TYPE (BOUNDARY_PROP1_TYPE), POINTER :: WCM_P1,WCP_P1
...
EDGE_LOOP: DO IE=1,EDGE_COUNT(NM)
   ...
   SIGN_LOOP: DO I_SGN=-1,1,2
      ORIENTATION_LOOP: DO IS=1,3
         ...
         WCM_P1 => BOUNDARY_PROP1(WCM%P1_INDEX)
         WCP_P1 => BOUNDARY_PROP1(WCP%P1_INDEX)
         ...
         INTERPOLATION_IF: IF (NOM(ICD)==0 .OR. &
            ...
            IF (WCM%VENT_INDEX==WCP%VENT_INDEX .AND. WCP%VENT_INDEX > 0) THEN
               IF(VENTS(WCM%VENT_INDEX)%NODE_INDEX>0 .AND. WCM_P1%U_NORMAL >= 0._EB) VELOCITY_BC_INDEX=FREE_SLIP_BC
            ENDIF
            ...
            ELSE VEL_T_IF
               ...
               IF (HVAC_TANGENTIAL .AND. 0.5_EB*(WCM_P1%U_NORMAL_S+WCP_P1%U_NORMAL_S) > 0._EB) HVAC_TANGENTIAL = .FALSE.
               IF (HVAC_TANGENTIAL) THEN
                  VEL_T = 0._EB
                  IEC_SELECT: SELECT CASE(IEC) ! edge orientation
                     CASE (1)
                        IF (ICD==1) VEL_T = 0.5_EB*ABS((WCM_P1%U_NORMAL_S+WCP_P1%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(3)
                        IF (ICD==2) VEL_T = 0.5_EB*ABS((WCM_P1%U_NORMAL_S+WCP_P1%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(2)
                     CASE (2)
                        IF (ICD==1) VEL_T = 0.5_EB*ABS((WCM_P1%U_NORMAL_S+WCP_P1%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(1)
                        IF (ICD==2) VEL_T = 0.5_EB*ABS((WCM_P1%U_NORMAL_S+WCP_P1%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(3)
                     CASE (3)
                        IF (ICD==1) VEL_T = 0.5_EB*ABS((WCM_P1%U_NORMAL_S+WCP_P1%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(2)
                        IF (ICD==2) VEL_T = 0.5_EB*ABS((WCM_P1%U_NORMAL_S+WCP_P1%U_NORMAL_S)/VT%UVW(ABS(VT%IOR)))*VT%UVW(1)
                  END SELECT IEC_SELECT
               ELSE
                  IF (SF%PROFILE/=0 .AND. SF%VEL>TWO_EPSILON_EB) &
                     PROFILE_FACTOR = ABS(0.5_EB*(WCM_P1%U_NORMAL_0+WCP_P1%U_NORMAL_0)/SF%VEL)
...
HEAT_TRANSFER_IF: IF (CHECK_HT) THEN
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      JJG = BC%JJG
      KKG = BC%KKG
      UVW = (ABS(P1%Q_CON_F)/P1%RHO_F)**ONTH * 2._EB*P1%RDN
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL=IIG
         JCFL=JJG
