...
FDS and Smokeview installation files are built on NIST managed computers with access limited to only those associated with the FDS/Smokeview project. Each bundle file and the bundle itself is scanned for viruses and malware using the open source scanner ClamAV\href{https://github.com/Cisco-Talos/clamav}{\ct{https://github.com/Cisco-Talos/clamav}}. If the scan identifies a suspicious signature, the bundle is not uploaded and made available to the public. A SHA256 hash code is also generated for each bundle file so that one may verify that files have not been altered. 

The installation bundles are hosted at \href{https://github.com/firemodels/fds/releases}{\ct{https://github.com/firemodels/fds/releases}}. A SHA256 hash is generated for each bundle when it is uploaded to Github.  A list of these hash codes and the status of the virus scan is stored in a separate file with the suffix {\tt \_manifest.html} and made available along with the bundle.
