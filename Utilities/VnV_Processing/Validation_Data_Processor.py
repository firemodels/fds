    ...
    for list_item in config_lists:
        if list_item[0] == 'g':
            ...
            if data_counter >= quantity_counter:
                ...
                data_key_name = list_item[1].strip()+"-"+list_item[3].strip()+"-"+list_item[2].strip()+"-"+list_item[6].strip()
                print data_key_name
...
def extract_comp_data(comp_file_info):
    ...
    if exp_column_name[0] == '['
        scatter_data_labels = []
        for col_name in exp_column_name:
            scatter_data_labels.append(comp_file_info['Quantity']+"-"+comp_file_info['Dataname']+"-"+col_name)
    else:
        scatter_data_label = comp_file_info['Quantity']+"-"+comp_file_info['Dataname']+"-"+exp_column_name
    ...
    if mod_column_name[0] == '['
        scatter_data_labels = []
        for col_name in mod_column_name:
            scatter_data_labels.append(comp_file_info['Quantity']+"-"+comp_file_info['Dataname']+"-"+col_name)
    else:
        scatter_data_label = comp_file_info['Quantity']+"-"+comp_file_info['Dataname']+"-"+mod_column_name
    ...
    for exp_list in exp_data_list:
        try:
            ...
            print "Data Conversion Error in Column Name:", exp_list[exp_column_name_row_index].strip()
    ...
    for mod_list in mod_data_list:
        try:
            ...
            print "Data Error in Column Name:", mod_list[mod_column_name_row_index].strip()
    ...
    if scatter_data_label:
        ...
        exp_column_name_list = eval(exp_column_name)
        print exp_column_name_list
        for exp_sub_column_name in exp_column_name_list:
            exp_data_values_comp = exp_data_dict[exp_sub_column_name][exp_comp_ranges[2]:exp_comp_ranges[3]]
        ...
        for mod_sub_column_name in mod_column_name_list:
            exp_data_values_comp = exp_data_dict[exp_sub_column_name][exp_comp_ranges[2]:exp_comp_ranges[3]]
            mod_data_values_comp = mod_data_dict[mod_column_name][mod_comp_ranges[2]:mod_comp_ranges[3]]
...
def comparison_plot(plot_data,exp_data,mod_data):
    ...
    if exp_data is []:
        ...
        for exp_data_item in exp_data:
            g.plot(graph.data.points(exp_data_item, title=exp_key[exp_plot_counter], x=1, y=2),
...
print "There are "+str(len(config_and_data_dicts[0]))+" scatter data groups."
print "There are "+str(len(config_and_data_dicts[1]))+" quantities to process."
print "There are "+str(len(config_and_data_dicts[2]))+" comparison data sets to plot."

# Create comparison plots
for data_record in config_and_data_dicts[2]:
    ...
    comp_data_to_plot = extract_comp_data(config_and_data_dicts[2][data_record])
    print comp_data_to_plot
    ...
    #comparison_plot(config_and_data_dicts[2][data_record],exp_plot_data,mod_plot_data)
# 
...
# for scatter_plot_record in sorted(config_and_data_dicts[1]):
