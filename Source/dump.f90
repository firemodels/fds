         ...
         SOLID_STATS_SELECT: SELECT CASE(DV%SPATIAL_STATISTIC)
            ...
            CASE DEFAULT SOLID_STATS_SELECT
               ...
               WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
                  ...
                  IF (DV%IOR/=0) THEN
                     SELECT CASE(DV%IOR)
                        CASE( 1); EPS_X1 = 1._EB/P1%RDN
                        CASE(-1); EPS_X2 = 1._EB/P1%RDN
                        CASE( 2); EPS_Y1 = 1._EB/P1%RDN
                        CASE(-2); EPS_Y2 = 1._EB/P1%RDN
                        CASE( 3); EPS_Z1 = 1._EB/P1%RDN
                        CASE(-3); EPS_Z2 = 1._EB/P1%RDN
                     END SELECT
                  ENDIF
...
IF (PRESENT(OPT_WALL_INDEX)) THEN
   ...
   IF (WC%P1_INDEX>0) P1    => BOUNDARY_PROP1(WC%P1_INDEX)
   IF (WC%P2_INDEX>0) P2    => BOUNDARY_PROP2(WC%P2_INDEX)
...
ELSEIF (PRESENT(OPT_LP_INDEX)) THEN
   ...
   IF (LP%P1_INDEX>0) P1    => BOUNDARY_PROP1(LP%P1_INDEX)
   IF (LP%P2_INDEX>0) P2    => BOUNDARY_PROP2(LP%P2_INDEX)
...
ELSEIF (PRESENT(OPT_CFACE_INDEX)) THEN
   ...
   IF (CFA%P1_INDEX>0) P1    => BOUNDARY_PROP1(CFA%P1_INDEX)
   IF (CFA%P2_INDEX>0) P2    => BOUNDARY_PROP2(CFA%P2_INDEX)
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   CASE( 1) ! RADIATIVE HEAT FLUX
      SOLID_PHASE_OUTPUT = (P1%Q_RAD_IN-P1%Q_RAD_OUT)*0.001_EB
   CASE( 2) ! CONVECTIVE HEAT FLUX
      SOLID_PHASE_OUTPUT = P1%Q_CON_F*0.001_EB
   CASE( 3) ! NORMAL VELOCITY
      SOLID_PHASE_OUTPUT = P1%U_NORMAL
      IF (ICF>0) THEN
   ...
   CASE( 5) ! WALL TEMPERATURE
      SOLID_PHASE_OUTPUT = P1%TMP_F - TMPM
   ...
   CASE( 7) ! BURNING RATE
      IF (N_REACTIONS>0) THEN
         ...
         DO NR=1,N_REACTIONS
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT+P1%M_DOT_G_PP_ACTUAL(REACTION(NR)%FUEL_SMIX_INDEX)*P1%AREA_ADJUST
         ENDDO
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
   CASE( 8) ! NORMALIZED HEAT RELEASE RATE
      SOLID_PHASE_OUTPUT = 0._EB
      DO NR=1,N_REACTIONS
         SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT+P1%M_DOT_G_PP_ADJUST(REACTION(NR)%FUEL_SMIX_INDEX)*&
                              REACTION(NR)%HEAT_OF_COMBUSTION
      ENDDO
      SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT*0.001_EB/(SF%SURFACE_DENSITY*P1%AREA_ADJUST)
   CASE( 9) ! HRRPUA
      SOLID_PHASE_OUTPUT = 0._EB
      DO NR=1,N_REACTIONS
         SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT+P1%M_DOT_G_PP_ADJUST(REACTION(NR)%FUEL_SMIX_INDEX)*&
   ...
   CASE(10) ! TOTAL HEAT FLUX
      SOLID_PHASE_OUTPUT = (P1%Q_RAD_IN-P1%Q_RAD_OUT+P1%Q_CON_F)*0.001_EB
   ...
   CASE(12) ! BACK WALL TEMPERATURE
      SOLID_PHASE_OUTPUT = P1%TMP_B - TMPM
   CASE(13) ! GAUGE HEAT FLUX
      IF (PY%HEAT_TRANSFER_COEFFICIENT>=0._EB) THEN
         Q_CON = PY%HEAT_TRANSFER_COEFFICIENT*(TMP(BC%IIG,BC%JJG,BC%KKG)-PY%GAUGE_TEMPERATURE)
      ELSE
         Q_CON = P1%Q_CON_F + P1%HEAT_TRANS_COEF*(P1%TMP_F-PY%GAUGE_TEMPERATURE)
      ENDIF
      SOLID_PHASE_OUTPUT = (PY%GAUGE_EMISSIVITY*(P1%Q_RAD_IN/(P1%EMISSIVITY+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4) + &
                            Q_CON)*0.001_EB
   CASE(14) ! NORMALIZED HEATING RATE
      SOLID_PHASE_OUTPUT = P1%Q_CON_F*0.001_EB/SF%SURFACE_DENSITY
   CASE(15,16) ! MASS FLUX, NORMALIZED MASS LOSS RATE
      IF (Z_INDEX >=0) THEN
         SOLID_PHASE_OUTPUT = P1%M_DOT_G_PP_ACTUAL(Z_INDEX)*P1%AREA_ADJUST
      ELSEIF (Y_INDEX > 0) THEN
         SOLID_PHASE_OUTPUT = 0._EB
         MFT = SUM(P1%M_DOT_G_PP_ACTUAL)
         IF (MFT>TWO_EPSILON_EB) THEN
            ZZ_GET = P1%M_DOT_G_PP_ACTUAL/MFT
            CALL GET_MASS_FRACTION(ZZ_GET,Y_INDEX,Y_SPECIES)
            SOLID_PHASE_OUTPUT = Y_SPECIES*MFT*P1%AREA_ADJUST
         ENDIF
      ELSEIF (MATL_ID/='null') THEN
         DO NN=1,SF%N_MATL
            ...
            ENDIF
         ENDDO
         SOLID_PHASE_OUTPUT = ONE_D%M_DOT_S_PP(M_INDEX)*P1%AREA_ADJUST
      ELSE
         SOLID_PHASE_OUTPUT = SUM(P1%M_DOT_G_PP_ACTUAL(:))*P1%AREA_ADJUST
      ENDIF
      IF (INDX==16) SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT/(SF%SURFACE_DENSITY*P1%AREA_ADJUST)
   CASE(17) ! RADIANCE
   ...
   CASE(20) ! INCIDENT HEAT FLUX
      SOLID_PHASE_OUTPUT = ( P1%Q_RAD_IN/(P1%EMISSIVITY+1.0E-10_EB) )*0.001_EB
   CASE(21) ! HEAT TRANSFER COEFFICENT
      SOLID_PHASE_OUTPUT = P1%HEAT_TRANS_COEF
   CASE(22) ! RADIOMETER
      SOLID_PHASE_OUTPUT = PY%GAUGE_EMISSIVITY*(P1%Q_RAD_IN/(P1%EMISSIVITY+1.0E-10_EB)-SIGMA*PY%GAUGE_TEMPERATURE**4)*0.001_EB

   CASE(23) ! ADIABATIC SURFACE TEMPERATURE (Ferrari's Method for Solving the Quartic)
      H_W_EFF = 0._EB
      LTMP = 0._EB
      IF ((PRESENT(OPT_WALL_INDEX).OR.PRESENT(OPT_CFACE_INDEX)) .AND. ASSOCIATED(P2)) THEN  ! Look for evaporating liquid
         IF (ANY(ABS(P2%LP_CPUA)>TWO_EPSILON_EB)) THEN
            ATMP = 0._EB
            CTMP = 0._EB
            DO NN = 1,N_LAGRANGIAN_CLASSES
               LPC => LAGRANGIAN_PARTICLE_CLASS(NN)
               IF (LPC%LIQUID_DROPLET) THEN
                  CTMP = CTMP + P2%LP_CPUA(LPC%ARRAY_INDEX)
                  ATMP = ATMP + ABS(P2%LP_CPUA(LPC%ARRAY_INDEX))
                  LTMP = LTMP + ABS(P2%LP_CPUA(LPC%ARRAY_INDEX))*P2%LP_TEMP(LPC%ARRAY_INDEX)
               ENDIF
            ENDDO
            LTMP = LTMP / (ATMP + TWO_EPSILON_EB)
            H_W_EFF = CTMP / (P1%TMP_F-LTMP+TWO_EPSILON_EB)
            H_W_EFF = MIN(10000._EB,MAX(0._EB,H_W_EFF));
         ENDIF
      ENDIF
      AAA = P1%EMISSIVITY*SIGMA
      IF (P1%HEAT_TRANS_COEF+H_W_EFF>1.E-5_EB .AND. ABS(P1%Q_RAD_IN-AAA*TMP(BC%IIG,BC%JJG,BC%KKG)**4)>5.E-3_EB) THEN
         AAA = P1%EMISSIVITY*SIGMA
         BBB = P1%HEAT_TRANS_COEF + H_W_EFF
         CCC = -P1%Q_RAD_IN - P1%HEAT_TRANS_COEF*TMP(BC%IIG,BC%JJG,BC%KKG) - H_W_EFF*LTMP
      ...
      ELSE
         SOLID_PHASE_OUTPUT = (P1%Q_RAD_IN/(P1%EMISSIVITY*SIGMA+TWO_EPSILON_EB))**0.25 - TMPM
      ENDIF
   ...
   CASE(28) ! EMISSIVITY
      SOLID_PHASE_OUTPUT = P1%EMISSIVITY

   CASE(29) ! SURFACE DEPOSITION
         IF (Z_INDEX>0) SOLID_PHASE_OUTPUT = P1%AWM_AEROSOL(SPECIES_MIXTURE(Z_INDEX)%AWM_INDEX)
         IF (Y_INDEX>0) SOLID_PHASE_OUTPUT = P1%AWM_AEROSOL(SPECIES(Y_INDEX)%AWM_INDEX)

   CASE(30:32) ! MPUA, CPUA, AMPUA
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      IF (ASSOCIATED(P2)) THEN
         SELECT CASE(INDX)
            CASE(30) ; SOLID_PHASE_OUTPUT = P2%LP_MPUA(LPC%ARRAY_INDEX)
            CASE(31) ; SOLID_PHASE_OUTPUT = P2%LP_CPUA(LPC%ARRAY_INDEX)*0.001_EB
            CASE(32) ; SOLID_PHASE_OUTPUT = P2%A_LP_MPUA(LPC%ARRAY_INDEX)
         END SELECT
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
   ...
   CASE(35) ! VISCOUS WALL UNITS (distance from the wall expressed in nondimensional viscous units)
      IF ((PRESENT(OPT_WALL_INDEX).OR.PRESENT(OPT_CFACE_INDEX)) .AND. ASSOCIATED(P2)) THEN
         SOLID_PHASE_OUTPUT = P2%Y_PLUS
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
   ...
   CASE(42:44) ! MPUA_Z, CPUA_Z, AMPUA_Z
      ...
      IF (ASSOCIATED(P2)) THEN
         DO NN = 1,N_LAGRANGIAN_CLASSES
            ...
            IF (LPC%LIQUID_DROPLET .AND. LPC%Y_INDEX==Y_INDEX) THEN
               SELECT CASE(INDX)
                  CASE(42) ; SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + P2%LP_MPUA(LPC%ARRAY_INDEX)
                  CASE(43) ; SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + P2%LP_CPUA(LPC%ARRAY_INDEX)*0.001_EB
                  CASE(44) ; SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + P2%A_LP_MPUA(LPC%ARRAY_INDEX)
               END SELECT
            ENDIF
         ENDDO
      ENDIF
   ...
   CASE(51)  ! ENTHALPY FLUX WALL
      ...
      CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,P1%TMP_F)
      SOLID_PHASE_OUTPUT = (-P1%RHO_F*H_S*P1%U_NORMAL &
                            -2._EB*P1%K_G*(TMP(BC%IIG,BC%JJG,BC%KKG)-P1%TMP_F)*P1%RDN)*0.001_EB
   ...
   CASE(60)  ! MASS FLUX WALL
      ...
      ENDIF
      IF (Z_INDEX > 0) THEN
         Y_SPECIES = P1%ZZ_F(Z_INDEX)
         RHO_D_DYDN = P1%RHO_D_DZDN_F(Z_INDEX)
      ELSEIF (Y_INDEX > 0) THEN
         ZZ_GET(1:N_TRACKED_SPECIES) = P1%ZZ_F(1:N_TRACKED_SPECIES)
         CALL GET_MASS_FRACTION(ZZ_GET,Y_INDEX,Y_SPECIES)
         RHO_D_DYDN = DOT_PRODUCT(Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),P1%RHO_D_DZDN_F(1:N_TRACKED_SPECIES))
      ELSE
         Y_SPECIES = 1._EB
         RHO_D_DYDN = 0._EB
      ENDIF
      ! convention here is: inflow is positive (adds mass to domain), outflow is negative (subtracts mass)
      SOLID_PHASE_OUTPUT = Y_SPECIES*P1%RHO_F*UN - RHO_D_DYDN
   ...
   CASE(66) ! VISCOUS STRESS WALL (takes optional FORCE_DIRECTION vector)
      ...
      IF (PRESENT(OPT_WALL_INDEX) .OR. PRESENT(OPT_CFACE_INDEX)) THEN
         ...
         IF (SF%VELOCITY_BC_INDEX == FREE_SLIP_BC) THEN
            ...
            VEL_WALL = -P1%U_NORMAL*NVEC + ( VEL_CELL - DOT_PRODUCT(VEL_CELL,NVEC)*NVEC )
         ELSE
   ...
   CASE(69) ! WALL ENTHALPY
      ...
      IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
         ...
         DO I=1,NWP
            SELECT CASE (SF%GEOMETRY)
               CASE DEFAULT
                  VOL = P1%AREA*(ONE_D%X(I)-ONE_D%X(I-1))
   ...
   CASE(70) ! SUBSTEPS
      SOLID_PHASE_OUTPUT = REAL(P1%N_SUBSTEPS,EB)

   CASE(71) ! EFFECTIVE HEAT TRANSFER COEFFICIENT
      DTMP = TMP(BC%IIG,BC%JJG,BC%KKG)-0.5_EB*(P1%TMP_F_OLD+P1%TMP_F)
      IF (ABS(DTMP)>TWO_EPSILON_EB .AND. ABS(P1%Q_CON_F)>TWO_EPSILON_EB) THEN
         SOLID_PHASE_OUTPUT = P1%Q_CON_F/DTMP
      ELSE
         SOLID_PHASE_OUTPUT = P1%HEAT_TRANS_COEF
      ENDIF
   CASE(72) ! SCALING HEAT FLUX
      SOLID_PHASE_OUTPUT = P1%Q_IN_SMOOTH*0.001_EB

   CASE(100) ! CONDENSATION HEAT FLUX
      SOLID_PHASE_OUTPUT = P1%Q_CONDENSE * 0.001_EB
...
SELECT CASE(IND)
   ...
   CASE(437)  ! PARTICLE TEMPERATURE
      PARTICLE_OUTPUT = BOUNDARY_PROP1(LP%P1_INDEX)%TMP_F - TMPM
   ...
   CASE(455)  ! PARTICLE HEAT TRANSFER COEFFICIENT
      PARTICLE_OUTPUT = BOUNDARY_PROP1(LP%P1_INDEX)%HEAT_TRANS_COEF
   CASE(456)  ! PARTICLE RADIATIVE HEAT FLUX
      PARTICLE_OUTPUT = (BOUNDARY_PROP1(LP%P1_INDEX)%Q_RAD_IN-BOUNDARY_PROP1(LP%P1_INDEX)%Q_RAD_OUT)*0.001_EB
   CASE(457)  ! PARTICLE CONVECTIVE HEAT FLUX
      PARTICLE_OUTPUT = BOUNDARY_PROP1(LP%P1_INDEX)%Q_CON_F*0.001_EB
   CASE(458)  ! PARTICLE TOTAL HEAT FLUX
      PARTICLE_OUTPUT = ( BOUNDARY_PROP1(LP%P1_INDEX)%Q_RAD_IN-BOUNDARY_PROP1(LP%P1_INDEX)%Q_RAD_OUT &
                        + BOUNDARY_PROP1(LP%P1_INDEX)%Q_CON_F )*0.001_EB
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (N_TRACKED_SPECIES > 1) THEN
      DO N=1,N_TRACKED_SPECIES
         CALL GET_SENSIBLE_ENTHALPY_Z(N,P1%TMP_F,H_S_ALPHA)
         H_S_J_ALPHA = H_S_J_ALPHA + 2._EB*H_S_ALPHA*P1%RHO_D_F(N)*(ZZ(IIG,JJG,KKG,N)-P1%ZZ_F(N))*P1%RDN
      ENDDO
   ENDIF
   AREA_F = P1%AREA
   IF (TWO_D)       AREA_F = AREA_F/DY(BC%JJG)
   IF (CYLINDRICAL) AREA_F = AREA_F*2._EB*PI
   Q_DOT(3,NM) = Q_DOT(3,NM) - U_N*P1%RHO_F*H_S*AREA_F
   Q_DOT(4,NM) = Q_DOT(4,NM) - P1%Q_CON_F*AREA_F
   Q_DOT(5,NM) = Q_DOT(5,NM) - H_S_J_ALPHA*AREA_F
   IF (.NOT. HRR_GAS_ONLY) Q_DOT(1,NM) = Q_DOT(1,NM) + P1%Q_DOT_O2_PP*AREA_F
ENDDO WALL_LOOP
...
CFACE_LOOP : DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   IF (N_TRACKED_SPECIES > 1) THEN
      DO N=1,N_TRACKED_SPECIES
         CALL GET_SENSIBLE_ENTHALPY_Z(N,P1%TMP_F,H_S_ALPHA)
         H_S_J_ALPHA = H_S_J_ALPHA + 2._EB*H_S_ALPHA*P1%RHO_D_F(N)*(CUT_CELL(ICC)%ZZ(N,JCC)-P1%ZZ_F(N))*P1%RDN
      ENDDO
   ENDIF
   AREA_F = P1%AREA
   IF (TWO_D)       AREA_F = AREA_F/DY(BC%JJG)
...
IF (CHAR_OXIDATION .AND. .NOT. HRR_GAS_ONLY) THEN
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      IF (TWO_D)       AREA_F = AREA_F/DY(BC%JJG)
      IF (CYLINDRICAL) AREA_F = AREA_F*2._EB*PI
      Q_DOT(1,NM) = Q_DOT(1,NM) + LP%PWT*P1%Q_DOT_O2_PP*AREA_F
   ENDDO PARTICLE_LOOP
ENDIF
...
WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (CYLINDRICAL) AREA_F = AREA_F*2._EB*PI
   DO N=1,N_TRACKED_SPECIES
      M_DOT(N,NM) = M_DOT(N,NM) + P1%M_DOT_G_PP_ADJUST(N)*AREA_F
   ENDDO
ENDDO WALL_LOOP2
...
CFACE_LOOP_2 : DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   IF (CYLINDRICAL) AREA_F = AREA_F*2._EB*PI
   DO N=1,N_TRACKED_SPECIES
      M_DOT(N,NM) = M_DOT(N,NM) + P1%M_DOT_G_PP_ADJUST(N)*AREA_F*P1%AREA_ADJUST
   ENDDO
ENDDO CFACE_LOOP_2
