...
USE GLOBAL_CONSTANTS, ONLY: PREDICTOR,TWO_D,SOLID_BOUNDARY,OPEN_BOUNDARY,INTERPOLATED_BOUNDARY,CHECK_POISSON
...
EXTERNAL_SOLID_BOUNDARY_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   ! CYCLE if the domain boundary is forced to be pure Dirichlet (combination INTERPOLATED + NEUMANN)

   SELECT CASE(IOR)
      CASE( 1)
         IF (LBC2==1 .OR. LBC2==2) CYCLE EXTERNAL_SOLID_BOUNDARY_LOOP
      CASE(-1)
         IF (LBC2==1 .OR. LBC2==4) CYCLE EXTERNAL_SOLID_BOUNDARY_LOOP
      CASE( 2)
         IF (MBC2==1 .OR. MBC2==2) CYCLE EXTERNAL_SOLID_BOUNDARY_LOOP
      CASE(-2)
         IF (MBC2==1 .OR. MBC2==4) CYCLE EXTERNAL_SOLID_BOUNDARY_LOOP
      CASE( 3)
         IF (NBC2==1 .OR. NBC2==2) CYCLE EXTERNAL_SOLID_BOUNDARY_LOOP
      CASE(-3)
         IF (NBC2==1 .OR. NBC2==4) CYCLE EXTERNAL_SOLID_BOUNDARY_LOOP
   END SELECT
