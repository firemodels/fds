...
DO N=1,N_SPEC_READ
   ...
   DO NN = 1,N-1
      IF (ID==SPEC_ID_READ(NN)) THEN
         WRITE(MESSAGE,'(A,I0,A,I0,A)') 'ERROR: Species ',N,' has the same ID as species ',NN, '.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDDO
   ...
   IF ((HEAT_OF_VAPORIZATION >  0._EB .AND. SPECIFIC_HEAT_LIQUID <= 0._EB .AND. RAMP_CP_L=='null') .OR. &
      ...
      WRITE(MESSAGE,'(A,I0,A)') 'ERROR: SPEC ' ,N, &
...
PREDEF_SPEC_IF: IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
...
WRITE(*,*) 'NS',N_SPECIES , N_PRIMITIVE , N_PREDEFINED , N_COPY_PRIMITIVE , N_SIMPLE_FUEL   

! Determine predefined lumped species
PREDEF_SMIX_ID: IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
   N_PREDEFINED_SMIX = 1
   DO NR=1,N_REACTIONS
      IF (.NOT. REACTION(NR)%SIMPLE_CHEMISTRY .OR. REACTION(NR)%PAIR_INDEX < NR) CYCLE
...
DO N=1,N_SPEC_READ
   IF (ANY(PREDEFINED_SMIX_ID==SPEC_ID_READ(N))) THEN
...
WRITE(*,*) 'NTS',N_TOTAL_SCALARS,N_TRACKED_SPECIES,N_PASSIVE_SCALARS,N_LUMPED,N_COPY,N_TOTAL_BINS,N_PREDEFINED_SMIX,N_CONDENSATION
...
NR2 = 1
FOUND = .FALSE.
DO N=1,N_SPECIES - N_COPY_PRIMITIVE
   IF (N <= N_PREDEFINED) THEN
      CALL SET_SPEC_DEFAULT
      ID = PREDEFINED_SPEC_ID(N)
      LUMPED_COMPONENT_ONLY = .TRUE.
   ELSEIF (N>N_PREDEFINED .AND. N<=N_SPECIES-N_COPY_PRIMITIVE-N_SIMPLE_FUEL) THEN
      KEEP_READING = .FALSE.
      DO WHILE (KEEP_READING)
         CALL SET_SPEC_DEFAULT
         READ(LU_INPUT,NML=SPEC,IOSTAT=IOS)
         IF (TRIM(SPEC_ID(1))/='null') KEEP_READING = .TRUE.
      ENDDO
   ELSE
      CALL SET_SPEC_DEFAULT
      FOUND = .FALSE.
   ...
   IF (SS%H_F > -1.E20_EB .OR. SS%REFERENCE_ENTHALPY > -1.E20_EB) SS%EXPLICIT_H_F=.TRUE.
...
IF (N_COPY_PRIMITIVE > 0) THEN
   DO N=1,N_COPY_PRIMITIVE
      KEEP_READING = .FALSE.
      DO WHILE (KEEP_READING)
         READ(LU_INPUT,NML=SPEC,IOSTAT=IOS)
         IF (.NOT. PRIMITIVE) KEEP_READING = .TRUE.
      ENDDO
      NN = FINDLOC(SPECIES%ID,ID,1)
      SPECIES(N_SPECIES - N_COPY_PRIMITIVE - N_SIMPLE_FUEL + N) = SPECIES(NN)
...
IF (N_TRACKED_SPECIES > N_PREDEFINED_SMIX) THEN
   N_AGGLOMERATION_SPECIES = 0
   REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
   N = N_PREDEFINED_SMIX+1
   DO WHILE (N<=N_TRACKED_SPECIES-N_COPY)
      KEEP_READING = .TRUE.
      DO WHILE (KEEP_READING)
         CALL SET_SPEC_DEFAULT
         READ(LU_INPUT,NML=SPEC,IOSTAT=IOS)
         IF (LUMPED_COMPONENT_ONLY .AND. N_BINS<=0) KEEP_READING = .FALSE.
      ENDDO
      IF (N_BINS>0) THEN
         N_AGGLOMERATION_SPECIES = N_AGGLOMERATION_SPECIES + 1
...
IF (N_COPY > 0) THEN
   N=N_TRACKED_SPECIES-N_COPY
   DO NN=1,N_COPY
      KEEP_READING = .TRUE.
      DO WHILE (KEEP_READING)
         CALL SET_SPEC_DEFAULT
         READ(LU_INPUT,NML=SPEC,IOSTAT=IOS)
         IF (COPY_LUMPED) KEEP_READING = .FALSE.
      ENDDO
      NNN = FINDLOC(SPECIES_MIXTURE%ID,SPEC_ID(1),1)
      SPECIES_MIXTURE(N+NN) = SPECIES_MIXTURE(NNN)
      SPECIES_MIXTURE(N+NN)%ID = ID
   ENDDO
ENDIF
...
IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
   DO NR=1,N_REACTIONS
      IF (.NOT. REACTION(NR)%SIMPLE_CHEMISTRY .OR. REACTION(NR)%PAIR_INDEX < NR) CYCLE
      IF (ANY(SPECIES_MIXTURE%ID==REACTION(NR)%FUEL)) THEN
         N = FINDLOC(SPECIES_MIXTURE%ID,REACTION(NR)%FUEL,1)
...
DO N=1,N_TRACKED_SPECIES
   SM => SPECIES_MIXTURE(N)
   WRITE(*,*) 'ID',TRIM(SM%ID)
   WRITE(*,*) 'MF',SM%MASS_FRACTION
   WRITE(*,*) 'VF',SM%VOLUME_FRACTION
   DO NN=1,N_SPECIES
...
ELSE
   ...
   IF ((SM%RAMP_CP/='null' .OR. SM%SPECIFIC_HEAT > 0._EB) .AND. &
       (SM%REFERENCE_ENTHALPY < -1.E20_EB .OR. SM%H_F < -1.E20_EB)) SM%REFERENCE_ENTHALPY = 0._EB
...
!IF (SM%H_F > -1.E20_EB) SM%H_F = SM%H_F/SM%MW*1000._EB !J/mol -> J/kg
...
DO N=1,N_SPECIES
   ...
   IF (SS%SPECIFIC_HEAT < 0._EB .AND. SS%RAMP_CP_INDEX < 0 .AND. .NOT. SS%LISTED) THEN
      CALL CALC_GAS_PROPS(1,N,D_TMP(N),MU_TMP(N),K_TMP(N),CP_TMP(N),H_TMP(N),SS%ISFUEL,G_F_TMP(N))
      SS%SPECIFIC_HEAT = CP_TMP(N)
   ENDIF
   IF (SS%SPECIFIC_HEAT > 0._EB) THEN
      ...
      ELSE
         ...
         SS%H_F = SS%REFERENCE_ENTHALPY + SS%SPECIFIC_HEAT*H_F_REFERENCE_TEMPERATURE*SS%MW*0.001_EB
      ENDIF
   ENDIF
   IF(SS%RAMP_CP_INDEX > 0) THEN
      IF (SS%H_F > -1.E20_EB) THEN
         CP2 = EVALUATE_RAMP(0._EB,SS%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
         H2 = 0._EB
         ...
         ENDDO
         SS%REFERENCE_ENTHALPY = SS%H_F/SS%MW*1000._EB - (H1 + (H2-H1)*(H_F_REFERENCE_TEMPERATURE-INT(H_F_REFERENCE_TEMPERATURE)))
      ELSE
         CP2 = EVALUATE_RAMP(0._EB,SS%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
         H2 = 0._EB
         DO J=1,INT(SS%REFERENCE_TEMPERATURE)+1
            ...
            H2 = H2 + 0.5_EB*(CP1+CP2)
         ENDDO
         SS%REFERENCE_ENTHALPY = SS%REFERENCE_ENTHALPY - (H1 + (H2-H1)*(SS%REFERENCE_TEMPERATURE-INT(SS%REFERENCE_TEMPERATURE)))
         CP2 = EVALUATE_RAMP(0._EB,SS%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
      ...
      ENDIF
   ENDIF
   SS%H_F = SS%H_F/SS%MW*1000._EB !J/kg
END DO
...
DO N=1,N_TRACKED_SPECIES
   ...
   IF (SM%SPECIFIC_HEAT > 0._EB) THEN
      IF (SM%H_F > -1.E20_EB) THEN
         SM%REFERENCE_ENTHALPY = SM%H_F/SS%MW*1000._EB - SM%SPECIFIC_HEAT*H_F_REFERENCE_TEMPERATURE
      ELSE
         SM%REFERENCE_ENTHALPY = SM%REFERENCE_ENTHALPY - SM%SPECIFIC_HEAT*SM%REFERENCE_TEMPERATURE
         SM%H_F = SM%REFERENCE_ENTHALPY + SM%SPECIFIC_HEAT*H_F_REFERENCE_TEMPERATURE*SM%MW*0.001_EB
      ENDIF
   ENDIF      
   IF(SM%RAMP_CP_INDEX > 0) THEN
      IF (SM%H_F > -1.E20_EB) THEN
         ...
         DO J=1,INT(H_F_REFERENCE_TEMPERATURE)+1
            ...
            H2 = H2 + 0.5_EB*(CP1+CP2)
         ENDDO
         SM%REFERENCE_ENTHALPY = SM%H_F - (H1 + (H2-H1)*(H_F_REFERENCE_TEMPERATURE-INT(H_F_REFERENCE_TEMPERATURE)))
      ELSE
         ...
         DO J=1,INT(SM%REFERENCE_TEMPERATURE)+1
            ...
            H2 = H2 + 0.5_EB*(CP1+CP2)
         ENDDO
         SM%REFERENCE_ENTHALPY = SM%REFERENCE_ENTHALPY - (H1 + (H2-H1)*(SM%REFERENCE_TEMPERATURE-INT(SM%REFERENCE_TEMPERATURE)))
         CP2 = EVALUATE_RAMP(1._EB,SM%RAMP_CP_INDEX,TAU=1._EB)*1000._EB
         ...
         DO J=1,INT(H_F_REFERENCE_TEMPERATURE)+1
            ...
            H2 = H2 + 0.5_EB*(CP1+CP2)
         ENDDO
         SM%H_F = H1 + (H2-H1)*(H_F_REFERENCE_TEMPERATURE-INT(H_F_REFERENCE_TEMPERATURE))*SM%MW*0.001_EB
      ENDIF
   ENDIF
   IF(SM%H_F <= -1.E20_EB) THEN
      SM%H_F = 0._EB
      ...
      ENDDO
   ENDIF
   SM%H_F = SM%H_F/SM%MW*1000._EB ! J/kg
END DO
...
REAL(EB) :: FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,FUEL_N_TO_HCN_FRACTION,SOOT_H_FRACTION
...
INTEGER :: NR,NS,NS2,NFR,PRIORITY,PROD_COUNTER=0, SIMPLE_COUNTER, NEW_REAC=0,N_SIMPLE_CHEMISTRY_REACTIONS
...
NAMELIST /REAC/ A,AIT_EXCLUSION_ZONE,AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                ...
                ID,IDEAL,LOWER_OXYGEN_LIMIT,N,NU,N_S,N_SIMPLE_CHEMISTRY_REACTIONS,N_T,O,PRIORITY,RADIATIVE_FRACTION,&
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   ELSE SIMPLE_IF
      ...
      ELSE ! RN%N_SIMPLE_CHEMISTRY_REACTIONS=2
         ...
         REACTION(1:N_REACTIONS) = REAC_TEMP(1:N_REACTIONS)
         ...
         RN2%N_SMIX = 3
         RN2%N_SPEC = 0
         ALLOCATE(RN2%NU_READ(RN2%N_SMIX))
         ALLOCATE(RN2%SPEC_ID_NU_READ(RN2%N_SMIX))
         IF (PROD_COUNTER ==1) THEN
...
INTEGER :: NS,NR,HF_COUNT,I_HFRT,F_SM_I,F_SM_I2
...
WRITE(*,*) 'IN',REACTION(1)%HEAT_OF_COMBUSTION,REACTION(2)%HEAT_OF_COMBUSTION,REACTION(1)%HOC_COMPLETE,REACTION(2)%HOC_COMPLETE,REACTION(1)%EPUMO2,REACTION(2)%EPUMO2
REAC_LOOP: DO NR=1,N_REACTIONS

   RN => REACTION(NR)
   F_SM_I = RN%FUEL_SMIX_INDEX
   SMF => SPECIES_MIXTURE(F_SM_I)
   ...
   ELSE SIMPLE_CHEMISTRY_IF ! Simple chemistry reaction
      ...
      ELSE
         RN2 => REACTION(RN%PAIR_INDEX)
         F_SM_I2 = RN2%FUEL_SMIX_INDEX
      ENDIF

      HOC_IF: IF (RN%HEAT_OF_COMBUSTION > -1.E21) THEN ! User specified heat of combustion
         IF (IDEAL) THEN
            RN2%HOC_COMPLETE = RN2%HEAT_OF_COMBUSTION * SMF%MW * 0.001 !J/kg -> J/mol
            RN2%HOC_COMPLETE = RN2%HOC_COMPLETE + ( &
                               RN2%NU_CO   *(SPECIES(CO2_INDEX)%H_F - SPECIES(CO_INDEX)%H_F) &
                             + RN2%NU_SOOT * SPECIES(CO2_INDEX)%H_F * (1._EB - SOOT_H_FRACTION_GLOBAL) &
                             + RN2%NU_SOOT * SPECIES(H2O_INDEX)%H_F * SOOT_H_FRACTION_GLOBAL * 0.5_EB &
                             + RN2%NU_HCN  *(SPECIES(CO2_INDEX)%H_F - SPECIES(HCN_INDEX)%H_F) &
                             + RN2%NU_HCN  *(SPECIES(H2O_INDEX)%H_F*0.5_EB - SPECIES(HCN_INDEX)%H_F))
            RN2%HOC_COMPLETE = RN2%HOC_COMPLETE / SMF%MW * 1000._EB !J/mol->J/kg
         ELSE
         ...
         ENDIF
         REDEFINE_H_F(RN%FUEL_SMIX_INDEX) = .TRUE.
         WRITE(*,*) 'A',HF_COUNT,REDEFINE_H_F
         IF (RN%N_SIMPLE_CHEMISTRY_REACTIONS==1) THEN
            RN%HEAT_OF_COMBUSTION = RN2%HOC_COMPLETE
            SM => SPECIES_MIXTURE(1)
            SMF%H_F = RN%HEAT_OF_COMBUSTION*ABS(RN%NU(F_SM_I))*SMF%MW*0.001_EB + RN%NU(1)*SM%H_F*SM%MW*0.001_EB
            SM => SPECIES_MIXTURE(RN%PROD_SMIX_INDEX)
            SMF%H_F = SM%H_F + RN%NU(RN%PROD_SMIX_INDEX)*SM%H_F*SM%MW*0.001_EB
            IF (SMF%SINGLE_SPEC_INDEX>0) SPECIES(SMF%SINGLE_SPEC_INDEX)%H_F = -SMF%H_F/ RN%NU(RN%FUEL_SMIX_INDEX)
            SMF%H_F = -SMF%H_F/ (RN%NU(RN%FUEL_SMIX_INDEX) * SMF%MW * 0.001_EB)
            RN%EPUMO2 = RN%HEAT_OF_COMBUSTION * SMF%MW * RN%NU(F_SM_I) / (RN%NU(1)*SM%MW*SM%MASS_FRACTION(O2_INDEX))
         ELSE
            RN%HOC_COMPLETE = RN2%HOC_COMPLETE
            RN2%HEAT_OF_COMBUSTION = 0._EB
            DO NS=1,N_TRACKED_SPECIES
               SM=>SPECIES_MIXTURE(NS)
               RN2%HEAT_OF_COMBUSTION = RN2%HEAT_OF_COMBUSTION - RN2%NU(NS) * SM%H_F * SM%MW
               WRITE(*,*) 'B',NS,RN2%HEAT_OF_COMBUSTION,RN2%NU(NS),SM%H_F,SM%MW
            ENDDO
            RN%HEAT_OF_COMBUSTION  = RN%HOC_COMPLETE - (1._EB+RN2%S)*RN2%HEAT_OF_COMBUSTION
            WRITE(*,*) RN%HEAT_OF_COMBUSTION,RN%HOC_COMPLETE,RN2%S
            SM => SPECIES_MIXTURE(1)
            RN%EPUMO2 = RN%HEAT_OF_COMBUSTION * SMF%MW * RN%NU(F_SM_I) / (RN%NU(1)*SM%MW*SM%MASS_FRACTION(O2_INDEX))
            SMF%H_F = RN%HEAT_OF_COMBUSTION*ABS(RN%NU(F_SM_I))*SMF%MW*0.001_EB + RN%NU(1)*SM%H_F*SM%MW*0.001_EB
            SM => SPECIES_MIXTURE(RN%PROD_SMIX_INDEX)
            ...
            SMF%H_F = -SMF%H_F/ (RN%NU(RN%FUEL_SMIX_INDEX) * SMF%MW * 0.001_EB)
            SM => SPECIES_MIXTURE(1)
            SMF => SPECIES_MIXTURE(RN2%FUEL_SMIX_INDEX)
            RN2%HEAT_OF_COMBUSTION =  RN%HEAT_OF_COMBUSTION / SMF%MW
            RN2%EPUMO2 = RN2%HEAT_OF_COMBUSTION * SMF%MW * RN2%NU(F_SM_I2) / (RN2%NU(1)*SM%MW*SM%MASS_FRACTION(O2_INDEX))
         ENDIF
      ...
      ELSE HOC_IF ! Heat of combustion not specified, use EPUMO2 or H_F is fuel is listed
         LISTED_FUEL_IF: IF (.NOT. LISTED_FUEL ) THEN
            ...
            ENDIF
            REDEFINE_H_F(RN%FUEL_SMIX_INDEX) = .TRUE.
            SMF%H_F = RN2%HOC_COMPLETE * ABS(RN%NU(RN%FUEL_SMIX_INDEX)) * SMF%MW * 0.001_EB
            DO NS = 1,N_TRACKED_SPECIES
               IF  (NS == RN%FUEL_SMIX_INDEX) CYCLE
                  SM=>SPECIES_MIXTURE(NS)
                  SMF%H_F = SMF%H_F + RN%NU(NS) * SM%H_F * SM%MW * 0.001_EB
...
WRITE(*,*) 'OUT',REACTION(1)%HEAT_OF_COMBUSTION,REACTION(2)%HEAT_OF_COMBUSTION,REACTION(1)%HOC_COMPLETE,REACTION(2)%HOC_COMPLETE,REACTION(1)%EPUMO2,REACTION(2)%EPUMO2


! Correct CP_BAR array for CONST_C_P_RATIO
IF (ANY(REDEFINE_H_F)) THEN
   ...
   ENDDO
ELSE
   IF (ANY(REACTION%SIMPLE_CHEMISTRY) .AND. CONSTANT_SPECIFIC_HEAT_RATIO) THEN
      H_F_OLD = 0._EB
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%HRRPUA>0._EB .OR. SF%MLRPUA>0._EB) THEN
      ...
      IF (N_REACTIONS > 1 .AND. .NOT. (N_REACTIONS==2 .AND. REACTION(1)%N_SIMPLE_CHEMISTRY_REACTIONS==2)) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
         IF (.NOT. ALL(REACTION%SIMPLE_CHEMISTRY)) THEN
            WRITE(MESSAGE,'(A)') 'ERROR: SURF '//TRIM(SF%ID)//&
                                 ' has HRRPUA or MLRPUA set and there are non-simple chemistry reactions.'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
      ENDIF
      BURNING = .TRUE.
