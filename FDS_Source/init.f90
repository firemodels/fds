...
REAL(EB) :: MU_N,ZZ_GET(0:N_TRACKED_SPECIES),VC,RTRM,CP
...
ENDIF
IF (DISSIPATION_SOURCE) THEN
   ALLOCATE(M%Q_DISSIPATION(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','Q_DISSIPATION',IZERO)
   M%Q_DISSIPATION=0._EB
ENDIF
!------------------------------------------------
...
IF (DYNSMAG .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ALLOCATE(M%CSD2_DYNSMAG(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','CSD2_DYNSMAG',IZERO)
ENDIF
...
IF (N_EVAP_INDICES>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   IF (ENTHALPY_TRANSPORT) THEN
      ALLOCATE(M%ENTHALPY_SOURCE_LAGRANGIAN(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
      CALL ChkMemErr('INIT','ENTHALPY_SOURCE_LAGRANGIAN',IZERO) 
      M%ENTHALPY_SOURCE_LAGRANGIAN = 0._EB
   ENDIF
ENDIF
...
IF (DYNSMAG .AND. .NOT.EVACUATION_ONLY(NM)) M%CSD2_DYNSMAG = 0._EB
...
IF (SOOT_DEPOSITION .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ALLOCATE(M%AWMSOOT(N_TOTAL_WALL_CELLS),STAT=IZERO)
