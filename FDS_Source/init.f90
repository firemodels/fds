...
IF (EMBEDDED_MESH) THEN
   ALLOCATE(M%SAVE3(-3:M%LSAVE),STAT=IZERO)
   CALL ChkMemErr('INIT','SAVE3',IZERO)
ENDIF
      ...
      SELECT_POISSON_SOLVER: SELECT CASE(M%IPS)
         ...
         CASE (0:1) SELECT_POISSON_SOLVER
            IF (.NOT.TWO_D) THEN
               ...
               IF (EMBEDDED_MESH)       CALL H3CZIS(XS,XF,IBAR,  3,YS,YF,JBAR,  3,ZS,ZF,KBAR,  3,M%HX,XLM,ITRN,JTRN,IERR,M%SAVE3)
            ENDIF
