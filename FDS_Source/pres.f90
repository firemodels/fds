...
PUBLIC PRESSURE_SOLVER,COMPUTE_VELOCITY_ERROR,COMPUTE_PRESSURE_ERROR !,BUILD_SPARSE_MATRIX_LAPLACE
...
SUBROUTINE COMPUTE_PRESSURE_ERROR(NM)

! This routine computes the maximum difference in the baroclinic torque term (FVX_B) from one iteration of the 
! pressure solution to the next.

USE COMP_FUNCTIONS, ONLY: SECOND
USE GLOBAL_CONSTANTS, ONLY: PRESSURE_ITERATIONS,PRESSURE_ERROR_MAX,T_USED
INTEGER, INTENT(IN) :: NM
REAL(EB) :: TNOW
REAL(EB), POINTER, DIMENSION(:,:,:) :: FVX_B_OLD

TNOW=SECOND()
CALL POINT_TO_MESH(NM)
FVX_B_OLD => WORK8

IF (PRESSURE_ITERATIONS>0) PRESSURE_ERROR_MAX(NM) = MAXVAL(ABS(FVX_B(0:IBAR,1:JBAR,1:KBAR)-FVX_B_OLD(0:IBAR,1:JBAR,1:KBAR)))

FVX_B_OLD = FVX_B

T_USED(5)=T_USED(5)+SECOND()-TNOW
END SUBROUTINE COMPUTE_PRESSURE_ERROR
