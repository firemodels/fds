...
LP%TMP_B             => OS%REALS(13,STORAGE_INDEX) ; IF (NEW) LP%TMP_B   = 0._EB
...
ELSE
   ...
   R_S(0:NWP) = ONE_D%X(NWP) - ONE_D%X(0:NWP)

   SURFACE_DENSITY = 0._EB
   DO II2=1,NWP
      IF (MODE==0) THEN
         WGT = (R_S(II2-1)**I_GRAD-R_S(II2)**I_GRAD)/(REAL(I_GRAD,EB)*SF%THICKNESS**(I_GRAD-1))
      ELSE
         WGT = (R_S(II2-1)**I_GRAD-R_S(II2)**I_GRAD)/SF%THICKNESS**I_GRAD
      ENDIF
...
H_V = 3.12463E+06_EB - MAX(MIN(TEMP,373.15_EB),273.15_EB)*2.31828E+03_EB ! J/kg, linear fit of JANAF table
X_SAT  = MIN(1._EB,EXP(H_V*MW_H2O/R0*(1._EB/373.15_EB-1._EB/ MIN(TEMP,373.15_EB))))
RELATIVE_HUMIDITY = 100._EB*Y_H2O*(MW_AIR/MW_H2O+(1._EB-MW_AIR/MW_H2O)*X_SAT)/X_SAT
