...
\end{equation}
In Eq.~(\ref{eq:A4}), $A$, represents the time updated value of the species concentrations in the mixed mode, $\hat{Y}_{\alpha}(\delta t)$, as found in Eq.~(\ref{eq:final_comp}). In addition to increasing the order of the error in the numerical scheme, Richardson extrapolation also provides an estimate of the error value. This estimate is calculated via a Taylor expansion:
...
\Delta t_{new}=\Delta t \left(\frac{err\_tol}{error}\right)^{\mathcal{O}^{-1}}
\end{equation}
