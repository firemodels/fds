...
INTEGER :: N,I,J,K,II,JJ,KK,IPTS,JPTS,KPTS,N_EDGES_DIM,IW,IC,SURF_INDEX,IOR,IOPZ,IERR,IPZ
...
ALLOCATE(M%EDGE_INDEX(1:12,0:M%CELL_COUNT),STAT=IZERO)
...
DO IW=M%N_EXTERNAL_WALL_CELLS+1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (SF%BACKING==EXPOSED) THEN
      ...
      IOR = WC%ONE_D%IOR
      IF (.NOT.M%SOLID(IC)) WC%BACK_INDEX = M%WALL_INDEX(IC,IOR)
      IF (     M%SOLID(IC)) THEN
         SELECT CASE(IOR)
            CASE(-1)
               II=II+1
         ...
         END SELECT
         IC = M%CELL_INDEX(II,JJ,KK)
         WC%BACK_INDEX = M%WALL_INDEX(IC,IOR)
      ENDIF
   ENDIF
ENDDO
...
M%POIS_PTB = 0._EB
M%SAVE1  = 0._EB
IF (MESH_REFINEMENT) M%SAVE2 = 0._EB
M%WORK  = 0._EB
M%PRHS  = 0._EB
M%BXS   = 0._EB
M%BXF   = 0._EB
...
SELECT_POISSON_SOLVER: SELECT CASE(M%IPS)
   ...
   CASE (0:1) SELECT_POISSON_SOLVER
      ...
      ENDIF
      IF (MESH_REFINEMENT .AND. M%MESH_LEVEL>0) THEN
...
INTEGER :: OBST_INDEX,IOR,IIG,JJG,KKG,IW_OLD,IERR
TYPE(WALL_TYPE), POINTER :: WC
...
IF (IW>N_EXTERNAL_WALL_CELLS) THEN
   ...
   IF (OBST_INDEX>0 .AND.      OBSTRUCTION(OBST_INDEX)%HIDDEN .AND. .NOT.SOLID(IC) ) WC%BOUNDARY_TYPE = NULL_BOUNDARY
   IF (OBST_INDEX>0 .AND. .NOT.OBSTRUCTION(OBST_INDEX)%HIDDEN .AND. .NOT.SOLID(ICG)) WC%BOUNDARY_TYPE = SOLID_BOUNDARY
