...
LOGICAL  :: ABSORBING,LUMPED_SPECIES_ONLY,BACKGROUND,BACKGROUND_ONLY,WATER_FLAG
...
COUNT_SPEC_LOOP: DO
   ...
   IF (ID=='WATER VAPOR') THEN
      WATER_FLAG = .TRUE.
      IF (WATER_EVAPORATION .AND. LUMPED_SPECIES_ONLY) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: A PART class evaporates to SPEC WATER VAPOR and LUMPED_SPECIES_ONLY is defined as TRUE'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      IF (H2O_INDEX > 0) THEN
         N_SPECIES = N_SPECIES - 1
...
IF (WATER_EVAPORATION .AND. .NOT. WATER_FLAG) THEN
   N_GAS_SPECIES = N_GAS_SPECIES + 1
   IF (H2O_INDEX == 0) THEN
      I_SKIP = 1
      N_SPECIES  = N_SPECIES + 1
...
DO N=N_SPECIES-N_MIX_SPECIES+1,N_SPECIES
   IF (N==I_BACKGROUND) CYCLE
   N2 = N2 + 1
   SS => SPECIES(N)
   WRITE(*,*) SS%ID,N,N2
   SS%Y_INDEX = N2
...
PART_CLASS_LOOP: DO IPC=1,N_PART
   ...
   IF (SIMPLE_CHEMISTRY) THEN
         IF (TRIM(PC%SPEC_ID)==TRIM(REACTION(1)%FUEL)) THEN
   ...
   ENDIF
   DO I=1,N_SPECIES
      IF (TRIM(SPECIES(I)%ID)==TRIM(PC%SPEC_ID)) THEN
...
REAL(EB) :: VOLUME_FRACTION(1:MAX_SPECIES),MASS_FRACTION(1:MAX_SPECIES)
LOGICAL :: BACKGROUND
...
REAL(EB) :: Y_F_INLET, MASS_REACTANT, MASS_PRODUCT, &
            H2_YIELD,SOOT_YIELD,CO_YIELD, Y_F_LFL,X_O2_LL,EPUMO2,BOF,&
            CRITICAL_FLAME_TEMPERATURE,HEAT_OF_COMBUSTION,MASS(MAX_SPECIES),NU(0:MAX_SPECIES),E,N_S(0:MAX_SPECIES),C,H,N,O,OTHER, &
            MAXIMUM_VISIBILITY,AUTO_IGNITION_TEMPERATURE,THRESHOLD_TEMPERATURE
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,EPUMO2,ID, N_S,&
                Y_O2_INFTY,Y_F_INLET, &
                H2_YIELD,SOOT_YIELD,CO_YIELD,Y_F_LFL,X_O2_LL,CRITICAL_FLAME_TEMPERATURE,NU,SOOT_H_FRACTION, &
                C,H,N,O,OTHER,MW_OTHER,IDEAL,MASS_EXTINCTION_COEFFICIENT,VISIBILITY_FACTOR,MAXIMUM_VISIBILITY, &
...
USE MATH_FUNCTIONS
INTEGER :: N,J,NS,NS2,NR,NNN,RN_SPEC_INDEX(MAX_SPECIES)
   ...
   FIND_SPECIES:IF (ANY(ML%NU_GAS > 0._EB)) THEN
      ...
      DO NS=1,N_SPECIES
         ...
         IF (RN_SPEC_INDEX(NS)==-1) THEN
            WRITE(MESSAGE,'(A,A,A,A)') 'ERROR: SPEC:',TRIM(MATL_SPEC_ID(N,NNN)),&
