...
IF (FLUX_LIMITER>=0 .AND. N_SPECIES>0) THEN
   ALLOCATE( M%FRHOYY(0:IBP1,0:JBP1,0:KBP1,1:N_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','FRHOYY',IZERO)
ENDIF
...
IF (FLUX_LIMITER>=0) THEN
   ALLOCATE(M%SCALAR_SAVE1(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','SCALAR_SAVE1',IZERO)
   IF (N_SPECIES>0) THEN
      ALLOCATE(M%SCALAR_SAVE2(0:IBP1,0:JBP1,0:KBP1,1:N_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','SCALAR_SAVE2',IZERO)
      ALLOCATE(M%SCALAR_SAVE3(0:IBP1,0:JBP1,0:KBP1,1:N_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','SCALAR_SAVE3',IZERO)
   ENDIF
   ALLOCATE(M%SMIN_SAVE(0:N_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','SMIN_SAVE',IZERO)
   ALLOCATE(M%SMAX_SAVE(0:N_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','SMAX_SAVE',IZERO)
ENDIF
...
M%RHO_AVG = RHOA
M%FRHO    = 0._EB
IF (FLUX_LIMITER>=0 .AND. N_SPECIES>0) M%FRHOYY  = 0._EB
M%U       = U0
M%V       = V0
M%W       = W0
M%US      = U0
...
IF (FLUX_LIMITER>=0) THEN
   ...
   M%SMIN_SAVE = 0._EB
   M%SMAX_SAVE = 0._EB ! obviously needs to be reset
ENDIF
...
M%UVW_SAVE = 0._EB
ALLOCATE(M%DUDT_AVG(M%NEWC),STAT=IZERO)
CALL ChkMemErr('INIT','DUDT_AVG',IZERO) 
ALLOCATE(M%DVDT_AVG(M%NEWC),STAT=IZERO)
CALL ChkMemErr('INIT','DVDT_AVG',IZERO) 
ALLOCATE(M%DWDT_AVG(M%NEWC),STAT=IZERO)
CALL ChkMemErr('INIT','DWDT_AVG',IZERO) 
...
! Poisson solver with stretching in the 1st coordinate
      ...
      SELECT_POISSON_SOLVER: SELECT CASE(M%IPS)
         ...
         CASE (0:1) SELECT_POISSON_SOLVER
            IF (.NOT.TWO_D) THEN
                                        CALL H3CZIS(XS,XF,IBAR,LBC,YS,YF,JBAR,MBC,ZS,ZF,KBAR,NBC,M%HX,XLM,ITRN,JTRN,IERR,M%SAVE1)
            ...
            IF (TWO_D .AND. .NOT.CYLINDRICAL) THEN
                                        CALL H2CZIS(XS,XF,IBAR,LBC,ZS,ZF,KBAR,NBC,M%HX,XLM,ITRN,IERR,M%SAVE1)
            ...
            IF (TWO_D .AND. CYLINDRICAL) THEN
               ...
               IF (XS==0._EB .AND. PRESSURE_CORRECTION) CALL H2CYIS(XS,XF,IBAR,  6,ZS,ZF,KBAR,  3,XLM,XMU,ITRN,IERR,M%SAVE2)
               IF (XS >0._EB .AND. PRESSURE_CORRECTION) CALL H2CYIS(XS,XF,IBAR,  3,ZS,ZF,KBAR,  3,XLM,XMU,ITRN,IERR,M%SAVE2)
            ENDIF
         CASE (2) SELECT_POISSON_SOLVER
                                     CALL H3CZIS(YS,YF,JBAR,MBC,XS,XF,IBAR,LBC,ZS,ZF,KBAR,NBC,M%HY,XLM,ITRN,JTRN,IERR,M%SAVE1)
            IF (PRESSURE_CORRECTION) CALL H3CZIS(YS,YF,JBAR,  3,XS,XF,IBAR,  3,ZS,ZF,KBAR,  3,M%HY,XLM,ITRN,JTRN,IERR,M%SAVE2)
         CASE (3) SELECT_POISSON_SOLVER
            IF (TWO_D) THEN
                                        CALL H2CZIS(ZS,ZF,KBAR,NBC,XS,XF,IBAR,LBC,M%HZ,XLM,ITRN,IERR,M%SAVE1)
               IF (PRESSURE_CORRECTION) CALL H2CZIS(ZS,ZF,KBAR,  3,XS,XF,IBAR,  3,M%HZ,XLM,ITRN,IERR,M%SAVE2)
            ELSE
                                        CALL H3CZIS(ZS,ZF,KBAR,NBC,YS,YF,JBAR,MBC,XS,XF,IBAR,LBC,M%HZ,XLM,ITRN,JTRN,IERR,M%SAVE1)
               IF (PRESSURE_CORRECTION) CALL H3CZIS(ZS,ZF,KBAR,  3,YS,YF,JBAR,  3,XS,XF,IBAR,  3,M%HZ,XLM,ITRN,JTRN,IERR,M%SAVE2)
            ENDIF
         CASE (4) SELECT_POISSON_SOLVER
            ...
            IF (PRESSURE_CORRECTION) &
                            CALL H3CSIS(XS,XF,IBAR,  3,YS,YF,JBAR,  3,ZS,ZF,KBAR,  3,XLM,ITRN,JTRN,IERR,M%SAVE2,M%WORK,M%HX,M%HY)
         CASE (5) SELECT_POISSON_SOLVER
            ...
            ELSE
               ...
               IF (PRESSURE_CORRECTION) &
                        CALL H3CSIS(XS,XF,IBAR,  3,ZS,ZF,KBAR,  3,YS,YF,JBAR,  3,XLM,ITRN,JTRN,IERR,M%SAVE2,M%WORK,M%HX,M%HZ)
            ENDIF
         CASE (6) SELECT_POISSON_SOLVER
            ...
            IF (PRESSURE_CORRECTION) &
                        CALL H3CSIS(ZS,ZF,KBAR,  3,YS,YF,JBAR,  3,XS,XF,IBAR,  3,XLM,ITRN,JTRN,IERR,M%SAVE2,M%WORK,M%HZ,M%HY)
...
INTEGER  :: II,N,NN,I_OBST,I,J,K,IBCX,IIG,JJG,KKG,IW,IIO,JJO,KKO,IC,ICG,IL,NOM_CHECK(0:1)
...
!! IF (IW<=M%NEWC .AND. .NOT.M%OBSTRUCTION(I_OBST)%HIDDEN) M%SOLID(IC) = .TRUE.
...
CHECK_MESHES: IF (IW<=M%NEWC .AND.  .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO ITER=0,1
      ...
      CALL SEARCH_OTHER_MESHES(NM,XIN,YIN,ZIN,NOM,IIO,JJO,KKO) 
      NOM_CHECK(ITER) = NOM
      IF (NOM/=0) THEN
