...
USE COMPLEX_GEOMETRY, ONLY: INTERSECT_CYLINDER_AABB,INTERSECT_SPHERE_AABB,ROTATION_MATRIX
...
REAL(EB) :: TRANSPARENCY,XB1,XB2,XB3,XB4,XB5,XB6,BULK_DENSITY,VOL_ADJUSTED,VOL_SPECIFIED,UNDIVIDED_INPUT_AREA(3),&
            INTERNAL_HEAT_SOURCE,SHAPE_LENGTH,SHAPE_RADIUS,SHAPE_XYZ(3),SHAPE_VEC(3),AABB(6),SHAPE_ROT(3,3),Z_VEC(3)
...
NAMELIST /OBST/ ALLOW_VENT,BNDF_FACE,BNDF_OBST,BULK_DENSITY,&
                ...
                OUTLINE,OVERLAY,PERMIT_HOLE,PROP_ID,REMOVABLE,RGB,&
                SHAPE_LENGTH,SHAPE_RADIUS,SHAPE_VEC,SHAPE_XYZ,&
                SURF_ID,SURF_ID6,SURF_IDS,TEXTURE_ORIGIN,THICKEN,&
                TRANSPARENCY,XB
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   READ_OBST_LOOP: DO NN=1,N_OBST_O
      ...
      SHAPE_RADIUS= -1._EB
      SHAPE_LENGTH= -1._EB
      SHAPE_VEC   = (/0._EB,0._EB,1._EB/)
      Z_VEC       = (/0._EB,0._EB,1._EB/)
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               IF (SHAPE_RADIUS>0._EB) THEN
                  AABB = (/XB1,XB2,XB3,XB4,XB5,XB6/)
                  SHAPE_TYPE = SPHERE_TYPE
                  IF (SHAPE_LENGTH>0._EB) THEN
                     SHAPE_TYPE = CYLINDER_TYPE
                  ENDIF
                  SELECT CASE (SHAPE_TYPE)
                     CASE (SPHERE_TYPE)
                        IS_INTERSECT = INTERSECT_SPHERE_AABB(SHAPE_XYZ,SHAPE_RADIUS,AABB)
                     CASE (CYLINDER_TYPE)
                        IS_INTERSECT = INTERSECT_CYLINDER_AABB(SHAPE_XYZ,SHAPE_LENGTH,SHAPE_RADIUS,SHAPE_ROT,AABB)
                  END SELECT
