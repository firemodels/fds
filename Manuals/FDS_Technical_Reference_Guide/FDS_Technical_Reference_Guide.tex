...
The Radiative Transport Equation (RTE) traduces the interaction of thermal radiation with an absorbing, emitting and scattering medium such as
\be \bs \cdot \nabla I_{\la}(\bx,\bs) =
- \Big[
...
\Big]+
\underbrace{   B(\bx,\la) }_{\textrm{Emission source term}} +
...
In the case of a non-scattering gas, the RTE is simplified such as
...
\section{Spatial, angular and spectral discretization}
...
In practical simulations, the spectral ($\la$) dependence of RTE cannot be solved
...
each band. For instance, the band specific RTE in a non-scattering gas is
...
is the appropriate mean absorption coefficient inside the band.
...
\section{Emission source term}

The emission term can be written in function to the dimensionless refractive index, an absorption coefficient and a source term :

\be
B(\bx,\la) = 
n_{n} \kappa_n(\bx) I_{b,n}(\bx) =
\kappa_n(\bx) I_{b,n}(\bx)\textrm{ for gaseous phase}
\ee

In a given spectral band $n$, the
...
\section{Absorption by the gaseous phase}
...
Based on \cite{Collin} and \cite{Maruyama}, the water fog inside a grid cell is modeled for radiation transport as a monodisperse fog whose diameter corresponds to the Sauter mean diameter of the polydisperse spray.
