...
!$OMP PRIVATE(WC,I,J,K,IOR,DUUDT,DVVDT,DWWDT,NOM,H_OTHER,KKO,JJO,IIO, &
...
SELECT CASE(IPS)
   ...
   CASE(:1,4,7)
      IF (CYLINDRICAL) THEN
         !$OMP DO SCHEDULE(STATIC) PRIVATE(TRM1,TRM3,TRM4)
         DO K=1,KBAR
            DO I=1,IBAR
      ...
      IF (.NOT.CYLINDRICAL) THEN
         !$OMP DO SCHEDULE(STATIC) PRIVATE(TRM1,TRM2,TRM3,TRM4)
         DO K=1,KBAR
            DO J=1,JBAR
               DO I=1,IBAR
