...
Both FDS and Smokeview live within a GitHub \emph{organization} called ``Fire Models''.  The current location of the organization is availabbe view the \emph{View on Github} button at \href{https://pages.nist.gov/fds/} {\textct{https://pages.nist.gov/fds/}}.  The repositories that are used by the FDS and Smokeview projects contain more than 6000 configuration items and are listed below along with a brief description:
...
fig & contains figures and other large files used in other repos under firemodels
\end{tabular}
\vskip\baselineskip
...
\item [1. Identify and document the need for a change:] The need for minor changes to FDS typically come from user inquiries. These may be reports of bugs in the software or requests for additional features or outputs from the model.
    ...
    Significant changes to FDS/Smokeview, guided by available research road maps \linebreak (see \href{https://pages.nist.gov/fds/} {\textct{https://pages.nist.gov/fds/}}) are made based on the following criteria, in no particular order:
\begin{description}
