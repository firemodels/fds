...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SF%SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         ...
         IF (WC%EW>TWO_EPSILON_EB) ONE_D%MASSFLUX(0:N_TRACKED_SPECIES) = ONE_D%MASSFLUX(0:N_TRACKED_SPECIES)*EXP(-WC%EW)
...
REAL(EB) :: RE,U2,V2,W2,H_NATURAL,H_FORCED,NUSSELT,ZZ_GET(0:N_TRACKED_SPECIES),CP,VELCON,FRICTION_VELOCITY,YPLUS,MU_DNS,KP,&
            RHO_G,TMP_G,TMP_F,DN
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU,VV,WW,RHOP
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP
INTEGER, PARAMETER :: LOGLAW=1,ABL=2
...
HTC_MODEL_SELECT: SELECT CASE(HT_MODEL)
   CASE DEFAULT
      RE = RHO_G*VELCON*CONV_LENGTH/MU_AIR_0
...
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_AEROSOL,RHOG,ZZ_GET(0:N_TRACKED_SPECIES),YDEP,K_AIR,TMP_FILM,KN,ALPHA,DTMPDX,&
            KN_EXP,TAU_PLUS,DN,TAU_PLUS_C,VEL_W,U2,V2,W2,U_GRAV,MASS_P,D_SOLID,SC_PART,R_PLUS,A_PART,B_PART,I_PART,&
            U_COMB,MASS_P_COMB,U_GRAV_COMB
...
SMIX_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (GRAVITATIONAL_DEPOSITION) THEN
         ...
         ENDIF
      ENDIF
      ! Combined Brownian, turbulent, and gravitational deposition mechanism (Lai and Nazaroff, 2000)
      IF (COMBINED_DEPOSITION) THEN
         MASS_P_COMB = 0.125_EB*FOTHPI*SM%MEAN_DIAMETER**3*SM%DENSITY_SOLID
         U_GRAV_COMB = -GVEC(ABS(IOR))*MASS_P_COMB*(1._EB+1.25_EB*KN+0.41_EB*KN*EXP(-0.88_EB/KN))/ &
                       (3._EB*CHI_D*MUGAS*SM%MEAN_DIAMETER)
         D_SOLID = K*TGAS*(1._EB+1.25_EB*KN+0.41_EB*KN*EXP(-0.88_EB/KN))/(3*PI*MUGAS*SM%MEAN_DIAMETER)
         SC_PART = (MUGAS/RHOG/D_SOLID)
         R_PLUS = SM%MEAN_DIAMETER*WC%U_TAU*(2._EB*MUGAS/RHOG)**(-1._EB)
         A_PART = 0.5_EB*LOG((10.92_EB*SC_PART**(-1._EB/3._EB)+4.3)**(3._EB)/(SC_PART**(-1._EB)+0.0609_EB))+ &
                  SQRT(3._EB)*ATAN((8.6_EB-10.92_EB*SC_PART**(-1._EB/3._EB))/(SQRT(3._EB)*10.92_EB*SC_PART**(-1._EB/3._EB)))
         B_PART = 0.5_EB*LOG((10.92_EB*SC_PART**(-1._EB/3._EB)+R_PLUS)**(3._EB)/(SC_PART**(-1._EB)+7.669E-4_EB*R_PLUS**(3._EB)))+ &
                  SQRT(3._EB)*ATAN((2*R_PLUS-10.92_EB*SC_PART**(-1._EB/3._EB))/(SQRT(3._EB)*10.92_EB*SC_PART**(-1._EB/3._EB)))
         I_PART = 3.64_EB*SC_PART**(2._EB/3._EB)*(A_PART-B_PART)+39._EB
         SELECT CASE(IOR)
            CASE( 1)
               U_COMB = WC%U_TAU/I_PART
            CASE(-1)
               U_COMB = WC%U_TAU/I_PART
            CASE( 2)
               U_COMB = WC%U_TAU/I_PART
            CASE(-2)
               U_COMB = WC%U_TAU/I_PART
            CASE( 3)
               U_COMB = U_GRAV_COMB/(1-EXP(-U_GRAV_COMB*I_PART/WC%U_TAU))
            CASE(-3)
               U_COMB = U_GRAV_COMB/(EXP(U_GRAV_COMB*I_PART/WC%U_TAU)-1)
         END SELECT
      ENDIF
      IF (TURBULENT_DEPOSITION) THEN
         ...
         ENDIF
      ENDIF
      IF (U_THERM+U_TURB+U_GRAV+U_COMB < 0._EB) CYCLE WALL_CELL_LOOP
      ZZ_GET = ZZ_GET * RHOG
      Y_AEROSOL = ZZ_GET(N)
      YDEP =Y_AEROSOL*MIN(1._EB,(U_THERM+U_TURB+U_GRAV+U_COMB)*DT*WC%RDN)
