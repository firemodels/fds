...
\begin{figure}[ht]
\centering
\includegraphics[width=.75\textwidth,trim={0 1in 0 1in},clip]{SCRIPT_FIGURES/obst_sphere}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.75\textwidth,trim={0 1in 0 1in},clip]{SCRIPT_FIGURES/obst_cylinder}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.75\textwidth,trim={0 1in 0 1in},clip]{SCRIPT_FIGURES/obst_cone}
...
\section{Special Topic: Particle verification test features}
\label{info:part-verification}

\subsection{Recycling or periodic particles}
\label{info:periodic-particles}

If periodic boundaries are being used, the particles by default will cease to exist when passing through a periodic boundary. Setting {\ct PERIODIC\_X=.TRUE.} on the {\ct PART} line will allow particles to pass through one side of the domain and emerge from the other in the $x$ direction. {\ct PERIODIC\_Y=.TRUE.} and {\ct PERIODIC\_Z=.TRUE.} allow the same for the $y$ and $z$ directions, respectively.

\subsection{Extra precision in {\ct .PRT} files}
\label{info:eb-part-file}

Setting {\ct EB\_PART\_FILE=.TRUE.} on the {\ct DUMP} line will change the {\ct .PRT} file to use 8-byte precision instead of 4-byte (the default). Smokeview will not run with 8-byte {\ct .PRT} files. The higher precision is necessary for verification testing of particles. The {\ct .PRT} files can be read in Matlab with the {\ct read\_prt5.m} script in the {\ct Utilities/Matlab/scripts/} directory of the FDS project.
...
{\ct EB\_PART\_FILE}                & Logical      & Section~\ref{info:eb-part-file}        &           & {\ct .FALSE.}                  \\ \hline
