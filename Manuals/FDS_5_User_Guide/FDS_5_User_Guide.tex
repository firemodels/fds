...
Note that this Guide does not provide the background theory for FDS. A companion document, called the FDS
...
Sylvain Desanghere                      & CTICM (Centre Technique Industriel de la Construction Métallique), France                  \\ \hline
Montu L. Das                            & Gage-Babcock \& Associates, USA and Canada                  \\ \hline
Franck Didieux                          & Laboratoire National de Métrologie et d'Essais (LNE), France                  \\ \hline
...
\item[{\bf Mac OS X}]
A Mac OS X FDS .zip archive is available for both the PowerPC and Intel architectures.
...
the flow field in each mesh is computed by a different ``process.''
Note the subtle difference between these
terms -- a ``process'' does not have the same meaning as a ``processor.'' The former can be thought of as a ``task'' that you would see in
the Windows Task Manager or by executing the ``top'' command on a Linux/Unix machine. The latter refers to the computer itself. A single
processor may run multiple processes, for example. The computation on a given FDS mesh is thought of as an individual ``process,'' and
MPI handles the transfer of information between these processes. Currently, each mesh is assigned its own process in a parallel calculation, although plans
are underway to relax this restriction, allowing multiple meshes to be handled via a single process. In this way, large meshes can be computed on dedicated processors, while
smaller meshes can be clustered together in a single process running on a single processor, without the need for MPI message passing between themselves.
...
0, 1, 2, and so on. Thus, Mesh 1 is assigned to Process 0; Mesh 2 to Process 1, and so on. As a user, you never actually number the meshes or the processes yourself, but
error statements from FDS or from MPI might refer to the meshes or processes by number. As an example, if a five mesh FDS case is run in parallel, the first printout (usually to the screen unless
...
\item[{\bf File Writing Errors}:] \index{Support!Error Statements!READONLY}
Occassionally, especially on Windows machines, FDS fails because it is not permitted to write to a file. A typical error statement reads:
\footnotesize
\begin{verbatim}
...
\begin{table}[ht]
\begin{center}
\caption{Namelist Group Reference Table}\index{Namelist Groups Table}
...
Note: {\ct TWFIN} will still work but it has been deprecated,
...
If you want the timeline to start at a number other than zero, you can use the parameter
...
{\ct T\_BEGIN}; otherwise, the are evaluated using the time from when the {\ct RAMP} activates.  Therefore, if you are
...
For example if you were testing a {\ct VENT} that is to open at 10 s whose {\ct SURF\_ID} uses a {\ct RAMP},
...
{\ct LOCK\_TIME\_STEP=.FALSE.} \\
on the {\ct TIME} line, in which case the specified
time step, {\ct DT}, will not be adjusted. This parameter is intended for diagnostic purposes only, for example, timing program execution. It can lead to
...
\subsubsection{Serial vs. Parallel Processing of Multiple Meshes}

To run FDS in parallel, you {\bf must} break up the computational domain into
...
in which case one CPU will serially process each mesh, one by one. Why do this? For one, if you set \\ {\ct SYNCHRONIZE=.FALSE.} \\ on the {\ct TIME} line,
...
\caption{\bf Rules concerning alignment of meshes.}
\label{fig:meshes}
\end{figure}
...
When running a case with multiple meshes in parallel, the efficiency of the \index{multi-mesh efficiency}
...
\includegraphics[width=3.1in]{FIGURES/piece_wise_linear.PDF}
\vspace{-.2in}
...
\hfill
\begin{minipage}[t]{3.1in}
\includegraphics[width=3.1in]{FIGURES/polygonal.PDF}
\vspace{-.2in}
...
The default is {\ct 'INERT'},non-reacting solid boundary fixed at TMPA. You do not need to define {\ct 'INERT'} via a {\ct SURF} line.
...
\index{MISC:PRESSURE\_CORRECTION}
\index{DUMP:CHECK\_VOLUME\_FLOW}
...
\ee
The first term on the right hand side of (\ref{eqn_baroclinic_torque}) is absorbed in $\nabla^2 {\cal H}$.  In the second term, which becomes part of $\nabla\cdot \mathbf{F}$, we must use the most recent value of the pressure $\tp^n$. The expression $\overline{\rho}$ is an average density, equal to $2 \rho_{\min} \rho_{\max} / (\rho_{\min} + \rho_{\max})$.
...
strategies before including any fire or HVAC \index{HVAC} functionality. Slowly build in complexity. See Section~\ref{stack_effect} for an example.
...
\noindent
If you want to prevent FDS from automatically changing the time step, set {\ct LOCK\_TIME\_STEP=.FALSE.} on the {\ct TIME} line, in which case the specified
time step, {\ct DT}, will not be adjusted. This parameter is intended for diagnostic purposes only, for example, timing program execution. It can lead to
numerical instabilities if the initial time step is set too high.
...
\begin{warning}
\noindent
Note that while single {\ct DEVC} or {\ct CTRL} can be used to control multiple items, a {\ct DEVC} or {\ct CTRL}
...
inert wall with the temperature fixed at TMPA, and is referred to as {\ct 'INERT'}. If only this
...
\subsection{Fans and other Velocity Boundary Conditions}
...
\subsubsection{Supply and Exhaust Vents}
...
\subsubsection{Tangential Velocity Boundary Conditions}
\index{tangential velocity}
...
In an LES calculation (default), the velocity at the wall
is set to be a fraction of its value in the mesh cell adjacent to the
wall. Only in a DNS (Direct Numerical Simulation) calculation is the velocity at the wall actually set to zero.
To alter these defaults, you can set a parameter called
...
The default {\ct SLIP\_FACTOR} is 0.5 for LES, -1.0 for DNS.

\begin{warning}
\noindent
It is not recommended that you change the default velocity boundary condition unless you are interested in
its effect on the overall flow. Sometimes people represent various degrees of {\em surface roughness} via the
{\ct SLIP\_FACTOR}. However, this should be done carefully using experimental data or empirical correlation as
guidance.
\end{warning}

\subsubsection{Louvered Vents}
...
\noindent
is a boundary condition for a louvered \index{louver}vent that pushes air into the
...
The above strategies for specifying various parts of a ventilation system have not considered the fact
that a real ventilation system is affected by pressure changes between compartments. This topic is taken up
in Section~\ref{info:Fan_Curves}.
...
\subsection{Special Topic: Pressure Boundary Conditions}
\label{info:pressure_boundary}
\index{VENT!DYNAMIC\_PRESSURE}
\index{VENT!PRESSURE\_RAMP}
...
\end{equationarray}
where $\rho_\infty$ is the ambient density and $\mathbf{u}$ is the most recent value of the velocity on the boundary. The {\ct PRESSURE\_RAMP} allows you to alter the pressure as a function of time. Note that you do not
...
drive a flow into the domain. However, a fire-driven flow of sufficient strength can push back against this incoming flow. A simple example demonstrating this
effect is given in Section~\ref{pressure_boundary}.
...
where $A_{\hbox{\footnotesize duct}}$ is the area of the duct~(m$^2$), $U_{\hbox{\footnotesize max}}$ is
the air velocity~(m/s), and $\Delta p_{\hbox{\footnotesize max}}$ is the maximum
pressure difference the fan can operate upon. Figure~(\ref{fig:Fan_Curve} displays a typical fan curve.

\begin{figure}[ht!]
\begin{center}
\includegraphics[width=3.5in]{FIGURES/Fan_Curve.PDF}
...
fan\_test}) is part of the FDS Verification Suite and is discussed in Section~\ref{fan_test}. In it, two simple
...
A solid object might contain multiple layers with multiple material
components per layer. The solid object is described by a {\ct SURF}
line which contains the names of the various {\ct MATL}s it is
composed of.  Each {\ct MATL} can undergo several reactions that may
occur at different temperatures and consume different amounts of heat.
Each individual reaction can produce a (single) solid {\ct RESIDUE},
water vapor, and/or fuel gas. For example, the evaporation of water
from a solid material is described by the ``reaction'' that converts
liquid water to water vapor. This reaction occurs close to 100~$^\circ$C and
...
that produces a solid {\ct RESIDUE}, water vapor, {\em and} fuel gas.

For each {\ct MATL} entry in the input file, decide how many reactions
it can undergo. It may not undergo any -- it may only just heat
up. However, if it is to change form via one or more reactions,
designate the number of reactions with the integer {\ct
N\_REACTIONS}. It is very important that you designate {\ct
N\_REACTIONS} or else FDS will ignore all parameters associated with
reactions. Note, that quite often the empirical observation of multiple
reactions does not imply {\ct N\_REACTIONS} > 1, but is caused by the
fact that the examined sample is a mixture of multiple materials
reacting at different temperatures.   Currently, the maximum number of
reactions for each material is 10 and the chain of consecutive
reactions may contain up to 20 steps.

Next, decide what each reaction produces: a (single) solid {\ct
RESIDUE}, water vapor, and/or fuel gas. This information is conveyed
to FDS via the {\em yields}: {\ct NU\_RESIDUE(j)}, {\ct NU\_WATER(j)},
and {\ct NU\_FUEL(j)}, respectively. Here, {\ct j} indicates which
reaction the parameters pertain to. If, like the evaporation of water,
only water vapor is produced, set {\ct NU\_WATER(j)=1.0} and the other
two to zero. The yields are all zero by default. If {\ct
NU\_RESIDUE(j)} is non-zero, then you {\em must} indicate what the
solid residue is via {\ct RESIDUE(j)}, the {\ct ID} of another {\ct
MATL} that is also listed in the input file. Ideally, the sum of the
...
Now you must specify at what temperature the reaction occurs, and how fast the reaction occurs at that temperature.
The reaction rate at the temperature, $T_s$, of the $i$th material ({\em i.e.} the {\ct MATL} that you are currently describing) undergoing its $j$th reaction is given by:
\be
  r_{ij} =
...
\ee
$\rho_{s,i}$ is the density of the $i$th material within that
particular layer, in the sense of the mass of the $i$th material
divided by the volume of the layer.  $\rho_{s0}$ is the initial
density of the layer. Thus, $\rho_{s,i}/\rho_{s0}$ is a quantity that
increases as the $i$th material is produced as a residue of some other
reaction, and decreases as the $i$th material decomposes.  If the
layer is composed of only one material, and if the reactions produce
no solid residues, then $\rho_{s,i}/\rho_{s0}$ is always 1. $n_{s,ij}$
is the reaction order and prescribed under the name {\ct N\_S(j)}, and
is 1 by default. If the value of $n_s$ is not known, it is a good
starting point to assume $n_s=1$.

The pre-exponential factor, $A_{ij}$, is prescribed under the name
{\ct A(j)}, with units of 1/s. $E_{ij}$, the activation energy, is
prescribed via {\ct E(j)} in units of kJ/kmol. Remember that 1~kcal is
4.184~kJ, and be careful with factors of 1000.  $A$ and $E$ are not
readily accessible for most real materials.  However, if they are
known, specify both. Avoid specifying just one because they act as a
pair.  If $A$ and $E$ are not known, which is usually the case,
specify {\ct REFERENCE\_RATE} (1/s) and {\ct REFERENCE\_TEMPERATURE}
($^\circ$C). This directs FDS to choose {\ct A} and {\ct E} so that
the reaction rate ({\ct REFERENCE\_RATE}) is achieved at the {\ct
REFERENCE\_TEMPERATURE}. The default value of {\ct REFERENCE\_RATE} is
0.1~s$^{-1}$.  It is suggested that unless you have information to the
contrary, leave {\ct REFERENCE\_RATE} at its default value and just
specify the {\ct REFERENCE\_TEMPERATURE}. Note that the {\ct
REFERENCE\_TEMPERATURE} is {\em not} the same as an ignition
temperature. Rather, it is simply the temperature at which the mass
fraction of the $i$th material decreases at a rate of
0.1~s$^{-1}$. You should check the sensitivity of these parameters
following the procedure explained in
Section~\ref{solid_phase_verification}.  \index{MATL!REFERENCE\_RATE}
...
thus, the last term of Equation~\ref{rr} does not affect the pyrolysis rate.
...
\begin{warning}
\noindent
Remember that all temperatures are specified in degrees Celsius, but are then converted to
degrees Kelvin within the program. Thus, the formulae in this section ought to be interpreted in terms of the absolute temperature.
\end{warning}

\noindent
One last issue before a few examples -- the most important one of all. Eq.~(\ref{rr}) describes the rate of the reaction as a function of temperature.
Most solid phase reactions require energy; that is, they are {\em endothermic}. The amount of energy consumed, per unit mass of reactant that is converted into
something else, is specified by the {\ct HEAT\_OF\_REACTION(j)}. Technically, this is the enthalpy
difference between the products and the reactant. A positive value indicates that the reaction is {\em endothermic}; that is, the reaction takes energy out of the
system. Usually the {\ct HEAT\_OF\_REACTION} is accurately known only
for simple phase change reactions like the vaporization of water. For
other reactions, it must be determined empirically.
\index{MATL!HEAT\_OF\_REACTION}


Here is an example of a material that burns in the neighborhood of 350~$^\circ$C,
converting all its mass to fuel gases ({\ct NU\_FUEL=1.}).
...
&MATL ID                       = 'My Fuel'
      ...
      N_REACTIONS              = 1
      NU_FUEL(1)               = 1.
      REFERENCE_TEMPERATURE(1) = 350.
      HEAT_OF_REACTION(1)      = 3000. /
\end{verbatim}
\normalsize

\noindent
Note that the {\ct (1)} has been added to the reaction parameters to emphasize the fact that these parameters are stored in arrays of length equal to {\ct N\_REACTIONS}. If there
is only one reaction, you need not include the {\ct (1)}, but it is a good habit to get into.
Note also that the {\ct HEAT\_OF\_COMBUSTION} is the energy released per unit mass of fuel gas that mixes with oxygen and combusts.  This has nothing to do with the pyrolysis
process, so why is it specified here? The answer is that there can be only one {\em gas phase} reaction of fuel and oxygen in FDS, but there can be dozens of different materials and
dozens of {\em solid phase} reactions. To ensure that the fuel vapors from
different materials combust to produce the proper amount of energy, it is very important to specify a {\ct HEAT\_OF\_COMBUSTION} for each material. That way, the mass loss rate
of fuel gases is automatically adjusted so that the effective mass loss rate multiplied by the single, global, gas phase heat of combustion produces the expected heat release rate.
\index{MATL!HEAT\_OF\_COMBUSTION}.  If, for example, the {\ct HEAT\_OF\_COMBUSTION} specified on the {\ct REAC} line is twice that
specified on the {\ct MATL} line, the mass of contained within wall cell will be decremented by that determined by the
pyrolysis model, but the mass added to gas phase would be reduced by 50 \%.

Several other examples of solid phase reactions can be found in various Verification examples. See Sections~\ref{thermoplastic} and \ref{charring_solid}.
...
materials making up the obstruction.

\begin{warning}
\noindent
Note also that if {\ct BURN\_AWAY} is prescribed, the {\ct SURF}
should be applied to the entire object, not just a face of the object
because it is unclear how to handle edges of solid obstructions that
have different {\ct SURF\_ID}s on different faces.
\end{warning}

\noindent
Also note that the amount of combustible fuel equals the {\ct DENSITY}
of the designated materials multiplied by the volume of the mesh
cell. If the volume of the obstruction changes because it has to
conform to the uniform mesh, FDS does {\bf not} adjust the burning
rate to account for this as it does with various quantities associated
with areas, like {\ct HRRPUA}.

Examples of the use of {\ct BURN\_AWAY} feature are available in
verification test box\_burn\_away (Section~\ref{box_burn_away})
and the couch sample fire (Section~\ref{couch}).

\index{SURF!SHRINK} If all the material components of the surface are reacting, and the pyrolysis reactions have no solid residue, the thickness of
the surface is going to shrink when the surface reacts. The shrinking behavior can be prevented in the numerical model by setting {\ct SHRINK=.FALSE.}
on the {\ct SURF} line.  However, setting {\ct SHRINK=.FALSE.} may introduce numerical problems in the wall solver. When all the material of
a shrinking surface is consumed but {\ct BURN\_AWAY} is not prescribed, the surface temperature is set to {\ct TMP\_BACK}, convective heat flux
...
By default the {\ct EPUMO2} value is combined with
...
{\ct C\_EDC} on the {\ct REAC} line. If you do not want to use this model, set {\ct EDDY\_DISSIPATION=.FALSE.} on the {\ct REAC} line, in which
...
Orloff and De Ris~\cite{Orloff:19th_Symposium} suggest a value of 1200~kW/m$^3$, and FDS uses by default a value of 2000~kW/m$^3$ as an upper bound ({\ct HRRPUV\_AVERAGE} on the
{\ct REAC} line).
...
possible to use a constant absorption coefficient by specifying {\ct
KAPPA0} on the {\ct RADI} line.
...
\chapter{Particles and Droplets: The \texorpdfstring{{\tt PART}}{PART} Namelist Group}
...
FDS, they are all Lagrangian particles; that is, point elements that are not bound by the structure of the underlying grid.} are used in FDS as water or liquid fuel
...
\section{Controlling Particles and Droplets}

Depending on how the particles or droplets are introduced into the computational domain, the following are important
parameters for controlling them:
\begin{description}
...
If more particles are desired, lower the input value of this parameter. The default value is 0.05~s.
...
\subsubsection{Particles Introduced at a Solid Surface}
...
\subsubsection{Droplets Introduced at a Sprinkler or Nozzle}
\index{PART!DROPLETS\_PER\_SECOND}

{\ct DROPLETS\_PER\_SECOND} is the  number of droplets inserted every
second per active sprinkler or nozzle. Its default value is 1000. Note that this parameter only affects sprinklers and nozzles.
...
liquid mass is distributed more uniformly over the droplets. If this parameter is too small, it can lead to a grainy, or uneven evaporation pattern, sometimes even to the
...
\subsubsection{Particles or Droplets Introduced Initially}
...
\subsubsection{Droplets that Strike Solid Surfaces}
...
\noindent
The following parameters pertain to the evaporation of liquid droplets. It is assumed by default that non-massless particles are
liquid droplets, but you can specify {\ct EVAPORATE=.FALSE.} to change this. Details are in the next section.
...
point in declaring anything else except maybe a {\ct COLOR} for Smokeview.
...
Also note that if {\ct MASSLESS=.TRUE.}, the {\ct SAMPLING\_FACTOR} is set to 1 unless you say otherwise, which would be silly since {\ct MASSLESS} particles are for visualization only.


\subsubsection{Static Particles or Droplets}
...
\subsubsection{Water Droplets}

{\ct WATER=.TRUE.} declares that the liquid droplets evaporate
into {\ct WATER VAPOR}, a separate gas phase species that is {\em automatically} added to the calculation by this command. By default, {\ct WATER=.FALSE.}, even though the default
properties of droplets are that of water. Setting {\ct WATER=.TRUE.} instructs FDS to add {\ct WATER VAPOR} as an explicitly defined species, and it also invokes appropriate constants
...
\subsubsection{Fuel Droplets}

{\ct FUEL=.TRUE.} indicates that the liquid droplets evaporate
into fuel gas and burn. In this case, add the {\ct HEAT\_OF\_COMBUSTION} (kJ/kg) of the fuel. Fuel droplets are colored yellow by default in Smokeview.
...
\subsubsection{Particles that do not Evaporate}

Unless you declare {\ct MASSLESS=.TRUE.} on the {\ct PART} line, it is assumed that the particle or droplet has mass and thermal properties that dictate its
heat up and evaporation. To prevent evaporation, set {\ct EVAPORATE=.FALSE.} The particles will still heat up due to convection, but they will not
shrink and no additional gaseous species need to be declared.

\begin{warning}
Note that the absorption of thermal radiation by water ({\ct WATER=.TRUE.}) or fuel droplets ({\ct FUEL=.TRUE.}) is handled in FDS with fairly well-established physical sub-models, the details
of which are contained in the FDS Technical Reference Guide~\cite{FDS_Tech_Guide_5}.
However, for arbitrary particles or droplets, there is no assumed radiative absorption.
\end{warning}


\section{Coloring Particles and Droplets}
...
in the same way as those for a sprinkler. Here is an example of a liquid fuel spray nozzle (also see Section~\ref{spray_burner}):
...
\noindent
which tells FDS to record the temperature at the given point as a function of time. The {\ct ID} is a label in the output file.
...
variables that are explicitly tracked.

Suppose, however, that you want to define your own ``smoke'' and that you want to specify its production rate independently of
the HRR (or even in lieu of an actual fire, like a smouldering source). You also want to define a mass extinction coefficient for the
...
\footnotesize
\begin{verbatim}
&SPEC ID='MY SMOKE', MW=29., MASS_EXTINCTION_COFFICIENT=7600. /
&SURF ID='SMOULDER', TMP_FRONT=1000., MASS_FLUX(1)=0.0001, COLOR='RED' /
&VENT XB=0.6,1.0,0.3,0.7,0.0,0.0, SURF_ID='SMOULDER' /

&PROP ID='Acme Smoke', QUANTITY='spot obscuration', SPEC_ID='MY SMOKE' /
...
will merely serve as a surrogate for smoke.
...
For any input that a  {\ct DEVC\_ID} can be specified for (such as {\ct OBST} or {\ct VENT}), a {\ct CTRL\_ID} can be
specified instead.
...
{\ct CTRL} will have the opposite value of its {\ct INITIAL\_STATE}. In the case below, the {\ct CUSTOM} \index{CTRL!FUNCTION\_TYPE!CUSTOM}
...
\item[{\ct FLUSH\_FILE\_BUFFERS}] By default, every 10 time steps FDS
purges the output file buffers and forces the data to be written out
into the respective output files. It has been noticed on Windows
machines that ocassionally a runtime error occurs because of file
...
\subsubsection{\ct VOLUME INTEGRAL}
...
\subsubsection{\ct AREA INTEGRAL}
...
{\ct MASS\_FLUX\_X'} along with {\ct SPEC\_ID='my gas'} is an appropriate output quantity if you want to know the mass flux of the gas species that
...
\subsubsection{\ct SURFACE INTEGRAL}
...
Two additional quantities are useful for monitoring the of reacting
surfaces: {\ct WALL\_THICKNESS} gives the overall thickness of the surface
element. {\ct SURFACE DENSITY} (no underscore) gives the oveall mass per unit area for the
surface element, computed as an integral of material density over wall
...
specify, for example, {\ct PBY=5.3} instead of {\ct XB} if it is desired\index{SLCF!PBX,PBY,PBZ}
...
Animated vectors can be created in Smokeview if a given {\ct SLCF} line has the attribute {\ct VECTOR=.TRUE.} \index{SLCF!VECTOR}If two {\ct SLCF}
...
Smokeview as realistic looking smoke and fire. By default, the output quantities {\ct 'soot mass fraction'} and {\ct 'HRRPUV'} (Heat Release Rate Per Unit Volume)
are used in the visualization. You have the option of rendering any other species mass fraction instead of {\ct 'soot mass fraction'}, so long as the
...
\footnotesize
\begin{verbatim}
&SPEC ID='MY SMOKE', MW=29., MASS_EXTINCTION_COEFFICIENT=7600. /
...
Devices that output a \% obscuration such as a {\ct DEVC} with a {\ct
QUANTITY} of {\ct aspiration}, {\ct spot obscuration} (smoke
...
suggested for flaming combustion of wood and plastics\footnote{For most flaming fuels, a suggested value for $\kappa$ is 8700~m$^2$/kg~$\pm$~1100~m$^2$/kg at a
...
\begin{warning}
Note that you can specify the output quantity {\ct 'VISIBILITY'} (capital letters matter) along with a {\ct SPEC\_ID} to produce an estimate of the visibility calculated using
the specified species instead of smoke. This does not require that you do a mixture fraction calculation; only that you have specified the given species. You can
...
surface, $\dq_c'' + \dq_r''$, use the {\ct QUANTITY} called {\ct
HEAT\_FLUX}. The individual components, the {\em net} convective and
radiative fluxes, are {\ct CONVECTIVE\_FLUX} and {\ct
RADIATIVE\_FLUX}, respectively. If you want to compare predicted heat
flux with a measurement, you often need to use {\ct
GAUGE\_HEAT\_FLUX}.
...
All of the above output quantities related to heat flux are defined at a solid surface. However,
there is a gas phase output quantity called {\ct RADIANT\_INTENSITY} that is used mainly for diagnosing problems with the
radiation solver. Even though its units are kW/m$^2$, it should not be interpreted as the heat flux to an object that would
occupy that particular point in space. Rather, it is the integral over all directions of the radiation intensity, $I(\bx,\bs)$, which is a function of both
space and direction ($\bs$ is a direction vector that ranges over $4\pi$ solid angles):
\be
   U(\bx) = \int I(\bx,\bs)\, d\bs
\ee
Note that $U$ is used in Eq.~(\ref{TC}) to get the effective radiative
flux to a small thermocouple bead so that its temperature, as opposed
to the gas temperature, can be calculated. If you divide $U$ by
$4\pi$, you get the average intensity at a particular point in space:
\be
   \overline{I}(\bx) = \frac{U(\bx)}{4 \pi}
\ee
The incident heat flux due to a constant intensity would be $\pi \, \overline{I}$, thus, the average incident radiative heat flux would then be:
\be
   \overline{\dq_{r,inc}''} = \pi \, \overline{I} = \pi \; \frac{U}{4 \pi} = \frac{U}{4}
\ee
Thus, you can use {\ct RADIATIVE\_INTENSITY}/4 as an approximation for the incident radiative heat flux. However, this number can not
take into account the non-uniform nature of radiation. Also note that this is the {\em incident} heat flux. To get the {\em net} heat flux to the
object or person whose surface temperature is $T_s$, you need to subtract $\epsilon \sigma T_s^4$, the outgoing heat flux.

There is a second output quantity available called {\ct RADIATIVE\_FLUX\_GAS} which uses the additional input
of {\ct ORIENTATION}.  This quantity integrates the incomming radiative flux over $2\pi$ solid angles around the
vector defined by {\ct ORIENTATION}.
...
{\ct 'MPUA'} is the \underline{M}ass \underline{P}er \underline{U}nit \underline{A}rea of the droplets named
{\ct PART\_ID}. Likewise, {\ct 'AMPUA'} is the \underline{A}ccumulated \underline{M}ass \underline{P}er \underline{U}nit \underline{A}rea. Both of these
...
period. The cooling of a solid surface by droplets of a given type is given by {\ct 'CPUA'}, the \underline{C}ooling \underline{P}er \underline{U}nit \underline{A}rea
in units of kW/m$^2$.
...
the air, in units of kg/m$^3$.  {\ct 'DROPLET\_FLUX\_X'},  {\ct 'DROPLET\_FLUX\_Y'}, and {\ct 'DROPLET\_FLUX\_Z'} produce {\em only} slice and Plot3D files
...
Note that lower case quantities are appropriate only for calculations involving the mixture fraction, $Z$.
For example, the quantity
{\ct visibility} pertains to the obscuration of visible light due to the implicitly defined smoke in a mixture fraction calculation, whereas
{\ct VISIBILITY} pertains to the obscuration of visible light due to the explicitly defined species denoted by {\ct SPEC\_ID}.
...
{\ct PART\_ID} on the same input line. % Also, the use of unnecessary underscores in output quantity names has been eliminated.
\end{warning}
...
{\ct ABSORPTION COEFFICIENT}                    & $\kappa$                                      & 1/m            & D,I,P,S      \\ \hline
...
{\ct BACK WALL TEMPERATURE}                     & $T_w$                                         & $^\circ$C      & B,D          \\ \hline
...
{\ct MPUV}$^{**}$                               & See Section~\ref{info:part_output}            & kg/m$^3$       & D,I,P,S      \\ \hline
{\ct nitrogen}                                  & $X_{N_2}(Z)$                                  & mol/mol        & D,I,P,S      \\ \hline
{\ct optical depth}                             & $OD=K/2.3$ (Section~\ref{info:visibility})    & 1/m            & D,I,P,S      \\ \hline
{\ct other}                                     & $X_{other}(Z)$                                & mol/mol        & D,I,P,S      \\ \hline
{\ct other mass fraction}                       & $Y_{other}(Z)$                                & mol/mol        & D,I,P,S      \\ \hline
...
{\ct RADIANT INTENSITY}                         & See Section~\ref{info:heat_flux}              & kW/m$^2$       & D,I,P,S      \\ \hline
...
{\ct SURFACE DENSITY}                           & $\sum\int\rho_i dx$                           & kg/m$^2$       & B,D          \\ \hline
...
{\ct WALL THICKNESS}                            & $\Delta x_w$                                  & m              & B,D          \\ \hline
{\ct water vapor}                               & $X_{H_2O}(Z)$                                 & mol/mol        & D,I,P,S      \\ \hline
{\ct water vapor mass fraction}                 & $Y_{H_2O}(Z)$                                 & kg/kg          & D,I,P,S      \\ \hline
\end{tabular}
\end{center}
\end{table}
\noindent
...
\begin{table}[H]
\caption[Boundary File Parameters]{For more information see Section~\ref{info:BNDF}.}\label{tbl:BNDF}
\noindent
...
\begin{table}[H]
\caption[MIN/MAX Clipping Parameters]{For more information see Section~\ref{info:CLIP}.}\label{tbl:CLIP}
\noindent
\begin{tabular*}{6.5in}{@{\extracolsep\fill}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Control Function Parameters]{For more information see Section~\ref{info:CTRL}.}\label{tbl:CTRL}
\noindent
\begin{tabular*}{6.5in}{@{\extracolsep\fill}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Device Parameters]{For more information see Section~\ref{info:DEVC}.}\label{tbl:DEVC}
\noindent
...
\begin{table}[H]
\caption[Output Parameters]{For more information see Section~\ref{info:DUMP}.}\label{tbl:DUMP}
\noindent
\begin{tabular*}{6.5in}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
{\ct CTRL\_DUMP\_LIMIT}         & Integer      & Number of columns in {\ct CTRL} file &           & 254                      \\ \hline
{\ct DEVC\_DUMP\_LIMIT}         & Integer      & Number of columns in {\ct DEVC} file &           & 254                      \\ \hline
...
\begin{table}[H]
\caption[Header Parameters]{For more information see Section~\ref{info:HEAD}.}\label{tbl:HEAD}
\noindent
\begin{tabular*}{6.5in}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Obstruction Cutout Parameters]{For more information see Section~\ref{info:HOLE}.}\label{tbl:HOLE}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Initial Conditions]{For more information see Section~\ref{info:INIT}.}\label{tbl:INIT}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Isosurface Parameters]{For more information see Section~\ref{info:ISOF}.}\label{tbl:ISOF}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Material Properties]{For more information see Section~\ref{info:MATL}.}\label{tbl:MATL}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Mesh Parameters]{For more information see Section~\ref{info:MESH}.}\label{tbl:MESH}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
{\ct FYI}         & Character                   & Comment String (has no effect)              &    &                  \\ \hline
%{\ct MPI\_PROCESS} & Integer                    & See Section~\ref{info:MPI}                &    &    \\ \hline
...
\begin{table}[H]
\caption[Obstruction Parameters]{For more information see Section~\ref{info:OBST}.}\label{tbl:OBST}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Lagrangian Particles/Droplets]{For more information see Section~\ref{info:PART}.}\label{tbl:PART}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
{\ct SIGMA\_D}                  & Real            & Parameter for size distribution   &           &               \\ \hline
...
{\ct XB(6)}                     & Real Sextuplet            & Initial particle placement region & m         &               \\ \hline
...
\begin{table}[H]
\caption[Wall Profile Parameters]{For more information see Section~\ref{info:PROF}.}\label{tbl:PROF}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Device Properties]{For more information see Section~\ref{info:PROP}.}\label{tbl:PROP}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Radiation Parameters]{For more information see Section~\ref{info:RADI}.}\label{tbl:RADI}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Ramp Function Parameters]{For more information see Section~\ref{info:RAMP}.}\label{tbl:RAMP}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Reaction Parameters]{For more information see Section~\ref{info:REAC}.}\label{tbl:REAC}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
{\ct HRRPUV\_AVERAGE}                     & Real        & See Section~\ref{info:turbulent_combustion}   & kW/m$^3$          & 2000.    \\ \hline
...
\begin{table}[H]
\caption[Slice File Parameters]{For more information see Section~\ref{info:SLCF}.}\label{tbl:SLCF}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Species Parameters]{For more information see Section~\ref{info:SPEC}.}\label{tbl:SPEC}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
{\ct MASS\_FRACTION(I)}               & Real Array      & For species {\ct I}               & kg/m$^2$ s          &                         \\ \hline
...
\begin{table}[H]
\caption[Table Parameters]{For more information see Section~\ref{info:TABL}.}\label{tbl:TABL}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Time Parameters]{For more information see Section~\ref{info:TIME}.}\label{tbl:TIME}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
%\begin{table}[H]
%\caption[Conical Tree Parameters]{For more information see Section~\ref{info:TIME}.}\label{tbl:TIME}
%\noindent
%\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
%\hline
...
\begin{table}[H]
\caption[MESH Transformations]{For more information see Section~\ref{info:TRNX}.}\label{tbl:TRNX}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Vent Parameters]{For more information see Section~\ref{info:VENT}.}\label{tbl:VENT}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
\begin{table}[H]
\caption[Pressure Zone Parameters]{For more information see Section~\ref{info:ZONE}.}\label{tbl:ZONE}
\noindent
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|l|l|l|l|}
\hline
...
code checking, and numerical tests. This chapter contains a discussion, and examples of,
...
\subsection{Pressure Rise in a Sealed Enclosure (\texorpdfstring{{\bf pressure\_rise}}{pressure\_rise})  }
\label{pressure_rise}
...
meshes each 1~m long with identical gridding.  We expect the pressure,
...
Thermodynamics and the equation of state.  The figures below show the results of this verification case.  As is seen density
matches exactly showing that FDS is injecting the appropriate quantity of mass and is properly initializing the domain.
Pressure rise and temperature rise; however, are over-predicted by 3~\% and 12~\% respectively.  Also a slight drop in pressure
...
\includegraphics[width=3.2in]{FIGURES/pressure_rise_T.pdf} &
\includegraphics[width=3.2in]{FIGURES/pressure_rise_P.pdf} \\
\includegraphics[width=3.2in]{FIGURES/pressure_rise_R.pdf} &
\includegraphics[width=3.2in]{FIGURES/pressure_rise_1000.jpg}
\end{tabular*}

\clearpage

\subsection{Leaks and Fans in a Sealed Enclosure (\texorpdfstring{{\bf leak\_test} and {\bf leak\_test\_2}}{leak\_test and leak\_test\_2})}
\label{leak_test}
...
\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.1in]{FIGURES/leak_test_Pressure_Case_1.PDF} &
\includegraphics[width=3.1in]{FIGURES/leak_test_Pressure_Case_2.PDF}
\end{tabular*}


\clearpage

\subsection{Fan Curves (\texorpdfstring{{\bf fan\_test}}{fan\_test})}
\label{fan_test}

In this example, two sealed
compartments share a common wall. Both compartments are considered as
separate ``pressure zones.'' Two fans are mounted in the Partition
...
\be \dot{V}_{\hbox{\footnotesize fan}} = A_{\hbox{\footnotesize duct}} U_{\hbox{\footnotesize max}} \;
   \hbox{sign} (\Delta p_{\hbox{\footnotesize max}}-\Delta p)
   \sqrt{ \frac{ |\Delta p - \Delta p_{\hbox{\footnotesize max}}|}{\Delta p_{\hbox{\footnotesize max}} } }  \ee
where $A_{\hbox{\footnotesize duct}}$ is the area of the duct~(both are 0.04~m$^2$), $U_{\hbox{\footnotesize max}}$ is
the air velocity~(0.4~m/s from Zone~1 to Zone~2 and 0.2~m/s from Zone~2 to Zone~1), and $\Delta p_{\hbox{\footnotesize max}}$ is the maximum
pressure difference the fan can operate upon (in this case both fans use 1000~Pa).

In steady state, the volume flow from compartment to compartment (or Zone to Zone) should be equal and opposite in sign.
\be (0.04 \; \hbox{m}^2) (0.4 \; \hbox{m/s})  \;
   \sqrt{ \frac{ |p_2-p_1-1000 \; \hbox{Pa}|}{1000 \; \hbox{Pa}}  } =
   (0.04 \; \hbox{m}^2) (0.2 \; \hbox{m/s})  \;
   \sqrt{ \frac{ |p_1-p_2-1000 \; \hbox{Pa}|}{1000 \; \hbox{Pa}}  } \ee
The solution is $p_2=300$~Pa and $p_1=-300$~Pa. Note that the sign of the Volume Flow in FDS has to do with whether the flow is moving in the plus or minus
coordinate direction. This convention can make these types of calculations a bit tricky.

\vspace{0.1in}
\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.1in]{FIGURES/fan_test_Pressure.PDF} &
\includegraphics[width=3.1in]{FIGURES/fan_test_Volume_Flow.PDF}
\end{tabular*}


\clearpage

\subsection{Stack Effect (\texorpdfstring{{\bf stack\_effect}}{stack\_effect})}
\label{stack_effect}
...
There are numerous examples in the heat transfer literature of exact solutions, for simple configurations of hot and cold objects, of the
radiation transport equation.


\subsection{Radiation inside a box (\texorpdfstring{{\bf radiation\_in\_a\_box}}{radiation\_in\_a\_box}) }
...
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{FIGURES/box_results_20.pdf} &
\includegraphics[width=3.2in]{FIGURES/box_results_100.pdf}
\end{tabular*}
...
\subsection{Radiation from a plane layer (\texorpdfstring{{\bf radiation\_plane\_layer}}{radiation\_plane\_layer})}
...
\clearpage
\subsection{Wall Internal Radiation (\texorpdfstring{{\bf wall\_internal\_radiation}}{wall\_internal\_radiation}) }
...
This section contains examples that test the one-dimensional heat
conduction solver in FDS, along with an example of a simple
two-step, solid phase reaction scheme.


\subsection{Simple Heat Conduction Through a Solid Slab (\texorpdfstring{{\bf heat\_conduction}}{heat\_conduction})}
...
\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{FIGURES/heat_conduction_Slab_A_Temperatures.PDF} &
\includegraphics[width=3.2in]{FIGURES/heat_conduction_Slab_B_Temperatures.PDF} \\
\includegraphics[width=3.2in]{FIGURES/heat_conduction_Slab_C_Temperatures.PDF} &
\includegraphics[width=3.2in]{FIGURES/heat_conduction_Slab_D_Temperatures.PDF}
\end{tabular*}


\clearpage

\subsection{Temperature-Dependent Thermal Properties (\texorpdfstring{{\bf heat\_conduction\_kc}}{heat\_conduction\_kc})}
...
\noindent
\begin{center}
\includegraphics[width=4.5in]{FIGURES/heat_conduction_kc.pdf}
\end{center}


\subsection{A Simple Two-Step Pyrolysis Example (\texorpdfstring{{\bf two\_step\_solid\_reaction}}{two\_step\_solid\_reaction})}
...
analytical solution is~\cite{Lautenberger:2006}%Kevin undefined in .bib file.
\begin{eqnarray}
    Y_a(t)    &=&    \exp(-K_{ab}t)    \nonumber   \\
...
\begin{center}
\includegraphics[width=4.5in]{FIGURES/two_step_solid_reaction_Mass_Fractions.PDF}
\end{center}


\chapter{Code Checking}
\label{Code Checking}
...
\subsection{Axially-Symmetric Helium Plume (\texorpdfstring{{\bf helium\_2d}}{helium\_2d}) }
\label{helium_2d}
...
\includegraphics[height=2in]{FIGURES/helium_2d.jpg}
\end{minipage}
\hfill
...
\subsection{Sawtooth (\texorpdfstring{{\bf sawtooth}}{sawtooth})}
\label{sawtooth}

Sometimes it is desired to have stair-stepped obstructions representing
curved or sloped geometry. A concern is that this may
change the flow pattern near the wall. To lessen the impact of
stair-stepped boundaries near the edges of the obstructions,
one may specify the option {\ct SAWTOOTH=.FALSE.} If {\ct SAWTOOTH}
is set to {\ct FALSE}, then the velocity boundary conditions will be applied in
such a way as to minimize the impact of the boundaries due to vortices
at sharp corners, as shown in the following example:
...
\subsection{Pressure Boundaries (\texorpdfstring{{\bf pressure\_boundary}}{pressure\_boundary})}
\label{pressure_boundary}


This case just demonstrates how to do a time-dependent pressure boundary. It is a 10~m long, 1~m wide, 1~m tall tunnel with a fire in the middle and a pressure boundary
imposed on the right side ($x=10$).

\scriptsize
\begin{verbatim}
&HEAD CHID='pressure_boundary', TITLE='Simple tunnel with pressure boundary, SVN $Revision$' /

&MESH IJK=100,10,10, XB=0.0,10.0,0.0,1.0,0.0,1.0 /

&TIME TWFIN=60. /

&SURF ID='FIRE', HRRPUA=2000., COLOR='RED' /
...
&RAMP ID='wind_ramp', T=30., F= 1. /
&RAMP ID='wind_ramp', T=31., F=-1. /
...
\noindent
This snapshot from Smokeview was taken during the time period when there was a positive pressure imposed at the right portal of the tunnel. The fire leans to the
left because of the preferential flow in that direction.
\begin{center}
\includegraphics[width=\textwidth]{FIGURES/pressure_boundary.jpg}
\end{center}
...
\subsection{A Simple Under-Ventilated Compartment Fire (\texorpdfstring{{\bf door\_crack}}{door\_crack}) }
\label{door_crack}

This example uses the same simple compartment that was used to test leakage and fan curves (Sections~\ref{leak_test} and~\ref{fan_test}. Now, we add a small (160~kW) fire, with the same
...
\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.1in]{FIGURES/door_crack_Pressure.PDF} &
\includegraphics[width=3.1in]{FIGURES/door_crack_HRR.PDF}
\end{tabular*}
...
\clearpage
\subsection{Testing the ``Burn-Away'' Feature (\texorpdfstring{{\bf box\_burn\_away}}{box\_burn\_away}) }
\label{box_burn_away}
...
\subsection{Aspiration Detector (\texorpdfstring{{\bf beam\_detector}}{beam\_detector})}
\label{beam_detectors}

A 10~m x 10~m x 4~m compartment is filled with 0.006~kg/kg of
{\ct MIXTURE\_FRACTION\_2} with the default soot yield
0.01~kg/kg.  This results in an initial soot density of 71.9~mg/m$^3$
which using the default extinction coefficient of
8700~m$^2$/kg results in an optical depth of 0.626~m$^{-1}$.  The
compartment has a series of obstructions placed at varying depths that are
multiples of 1~m.  Using the correlation for the output quantity
{\ct visibility}, one obtains a visibility distance of 4.8~m.
When viewing the smoke levels with Smokeview, one can just barely see
the fifth obstacle which is at a distance of 5~m.
Smokeview, therefore, is properly displaying the obscuration of the
initial soot density.  Three beam detectors are also
...
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{FIGURES/beam_detector_nosmoke.jpg} &
\includegraphics[width=3.2in]{FIGURES/beam_detector_smoke.jpg}
\end{tabular*}

\clearpage

\subsection{Aspiration Detector (\texorpdfstring{{\bf aspiration\_detector}}{aspiration\_detector}) }
\label{aspiration_detector}

A cubical compartment, 2~m on a side with a fire has a three
sampling location aspiration system.  The three locations
have flow rates of 0.1, 0.5, and 0.8~kg/s, respectively, and
transport times of 0.2, 0.1, and 0.3~s, respectively.  No bypass flow rate is
specified for the aspiration detector.  The input file fixes the
initial time step to 0.01~s so that the initial output
times in the {\bf aspiration\_detector\_devc.csv} file will line
up exactly with the transport times.  At 0.75~s, when
FDS begins reducing the time step below 0.01~s, the time delayed
soot densities at the three sampling locations are $7.4x10^{-6}$~kg/m$^3$,
$9.5x10^{-4}$~kg/m$^3$, and $1.6x10^{-18}$~kg/m$^3$, respectively.
Using these values along with the respective flow rates results
in a detector obscuration of 0.000823~\%/m which is the same obscuration as predicted by FDS.
...
\subsection{Water Droplet Evaporation (\texorpdfstring{{\bf water\_evaporation}}{water\_evaporation}) }
\label{water_evaporation}

The case called {\bf water\_evaporation} is nothing more than stationary
water droplets in an adiabatic box with dimensions of 1~m on a side. The air
within the box is stirred to maintain uniform conditions, and there are no
leaks or heat losses. The initial air temperature is 40~$^\circ$C.
Initially, the droplets have a median volumetric diameter of 100~$\mu$m,
a temperature of 90~$^\circ$C, and a total mass of 0.02~kg.
It is expected that a steady-state will be achieved after several minutes.
The initial energy content, sum of the air and water enthalpies, of the box
is 360,000~kJ.  After a short period of time, 0.0141 kg of water evaporate and the box
reaches an equilibrium temperature of 16.2~$^\circ$C, see the figure below.
At this point the energy content of the box is 372,000~kJ or a 3~\% error.
At 16.2~$^\circ$C, the expected evaporation is 0.0142~kg.

\begin{center}
\includegraphics[width=4.5in]{FIGURES/water_evaporation.jpg}
\end{center}
...
\subsection{Measuring Water Flux (\texorpdfstring{{\bf bucket\_test}}{bucket\_test}) }
\label{bucket_test}

A common way of measuring the spray distribution for any fire
sprinkler is called a ``bucket test.''  Usually, one or several
sprinklers is mounted a specified distance above an array of catch
bins, the water flows for a given period of time, and the water
flux distribution is calculated from the accumulated water mass in
each bin. In the test case {\bf bucket\_test}, a single sprinkler is
...
\subsection{Complex Spray Patterns (\texorpdfstring{{\bf bucket\_test\_2}}{bucket\_test\_2}) }
\label{bucket_test_2}
...
This section contains a variety of simple tests to check
the functionality of the code. These examples are good
demonstrations of how to make things happen in FDS.

\subsection{Creating and Removing \texorpdfstring{{\ct HOLE}s}{HOLEs} and \texorpdfstring{{\ct OBST}ructions}{OBSTructions} (\texorpdfstring{{\bf create\_remove}}{create\_remove})}
\label{create_remove}

It is often convenient to create or remove solid obstructions, or conversely,
remove or create empty holes. They are essentially the same thing as far as the logic in
FDS is concerned, but the input can be tricky. To avoid confusion, here
is an example of holes and obstructions being created and removed.

\vspace{0.2in}
\tiny
\noindent
\begin{minipage}{1.4in}
\includegraphics[height=1.4in]{FIGURES/create_remove.jpg}
\end{minipage}
\hfill
\begin{minipage}{5.5in}
\begin{verbatim}
...
\section{Testing Solid Phase Properties}
...
\clearpage
\section{Pyrolysis Samples}

\subsection{A Simple Thermoplastic (\texorpdfstring{{\bf thermoplastic}}{thermoplastic}) }
\label{thermoplastic}

The term ``thermoplastic'' is often applied to materials that
essentially heat up and gasify without leaving any solid residue. The
term is used here merely to indicate the class of materials for which
the pyrolysis can be modeled with a single reaction that converts
solid to gaseous fuel.


The purpose of this example is to demonstrate how the solid phase
pyrolysis model works in FDS. Essentially, the gas phase calculation
is shut off except for the imposition of a 50~kW/m$^2$ ``virtual''
heat flux. The solid in this example is a slab of plastic, similar in
composition to PMMA. The solid is described by the following {\ct
SURF} line:
...
\noindent
Notice that in addition to $k$, $\rho$ and $c$, there is one reaction
specified, the {\em yield} of which is 100~\% fuel gas ({\ct
NU\_FUEL=1.}).  The phase change from solid to gas {\em consumes}
energy at a rate of 1,578~kJ/kg. Although not relevant in this
example, the burning of these fuel gases would {\em produce} energy at
a rate of 25,200~kJ/kg. The reaction nominally takes place at
330~$^\circ$C, a necessary parameter for the pyrolysis model described
in Section~\ref{info:solid_pyrolysis}.

The plots below show the surface temperature and heat flux as
functions of time.  Note that the default emissivity of the material
surface is 0.9, meaning that only 45~kW/m$^2$ out of the initial
50~kW/m$^2$ is actually heating the solid. Some of this energy heats
up the solid to about 350~$^\circ$C at the surface, some is radiated
away (about 8~kW/m$^2$), some is used to convert the solid material
into a gas, and is plotted here as the product of the burning rate and
the heat of reaction.

\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{FIGURES/thermoplastic_Surface_Temperature.PDF} &
\includegraphics[width=3.2in]{FIGURES/thermoplastic_Fluxes.PDF}
\end{tabular*}
The slab is 0.025~m thick, with a density of 500~kg/m$^3$. This means there is 12.5~kg/m$^2$ of fuel present, which if divided by the
burning rate (about 0.018~kg/m$^2$/s) yields a burning time of about 700~s. After this, the material is ``burned away.''

\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{FIGURES/thermoplastic_Burning_Rate.PDF} &
\includegraphics[width=3.2in]{FIGURES/thermoplastic_Thickness.PDF}
\end{tabular*}



\clearpage
\subsection{A Charring Solid (\texorpdfstring{{\bf charring\_solid}}{charring\_solid}) }
...
of a 50~kW/m$^2$ ``virtual'' heat flux. The reaction mechanism is
fairly complicated, as it includes various solids like cellulose,
char, and ash. Each are input via {\ct MATL} lines as follows:
...
\noindent
Note the edition of the parameter {\ct EXTERNAL\_FLUX} on the {\ct
SURF} line. This produces a 50~kW/m$^2$ flux on the sample, without
any additional input lines in the file. It is just to test the solid
phase model and should not be copied into an actual fire simulation.

The figures below show the surface temperature and burning rate
...
phase, and then peaks again at the end due to presence of an external
backing material. The initial peak is typical of char-forming solids.

\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[width=3.2in]{FIGURES/charring_solid_Surface_Temperature.PDF} &
\includegraphics[width=3.2in]{FIGURES/charring_solid_Burning_Rate.PDF}
\end{tabular*}



\chapter{Sample Fires}

\section{Background}

All fire simulations must include a ``model'' of the fire itself.
Almost always, these models are simplifications of the actual burning item. The examples
in this chapter are merely starting points for more elaborate
fire simulations. The point of these examples is to demonstrate that any simulation of a fire ought
to first start with a model of just the fire itself and its
immediate surroundings. It is a tremendous waste of time to begin a project
by setting up a calculation of the fire, smoke and heat transport,
target/detector response, and so on, without first doing quick
simulations of the fire itself to ensure that it conforms to either a test burn or your own
conceptual model.



\subsection{A Liquid Pool Fire (\texorpdfstring{{\bf ethanol\_pan}}{ethanol\_pan}) }
\label{ethanol_pan}
...
\begin{center}
\includegraphics[width=4.5in]{FIGURES/ethanol_pan_HRR.PDF}
\end{center}



\clearpage
\subsection{A Couch Fire (\texorpdfstring{{\bf couch}}{couch}) }
\label{couch}
...
\clearpage
\subsection{Flame Spread along a Cable Tray (\texorpdfstring{{\bf cable\_tray}}{cable\_tray}) }
\label{cable_tray}
...
\subsection{A Liquid Fuel Spray Burner (\texorpdfstring{{\bf spray\_burner}}{spray\_burner}) }
\label{spray_burner}
...
The HRR curve for this example is given here:

\begin{center}
\includegraphics[width=4.5in]{FIGURES/spray_burner_HRR.PDF}
\end{center}

Note that in this example there is no need for a separate ignition source. The combustion model assumes fuel and oxygen burn
on contact, and so the small amount of evaporation at ambient temperature is enough to start the reaction.
...
with  a heat  release rate  of  1500~kW.   A similar analysis was performed
using  Alpert's ceiling  jet correlation~\cite{SFPE:Alpert}  that also
showed better  predictions   with smaller grid
...
Dissipation Function (see FDS Technical Reference Guide~\cite{FDS_Tech_Guide_5} for details). Related to the
...
discussed  in the  FDS  Validation Guide~\cite{FDS_Validation_Guide_5}%Kevin unreferenced.
