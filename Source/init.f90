...
IF (REMOVE .AND. ( (SF%THERMAL_BC_INDEX==THERMALLY_THICK.AND.(SF%VARIABLE_THICKNESS.OR.SF%HT_DIM>1)) &
   ...
   IF (MESHES(NM)%CELL(ICG_OLD)%EXTERIOR) THEN
      ...
      IF (NOM>0) THEN
         ...
         JJO = EWC%JJO_MIN
         KKO = EWC%KKO_MIN
         ICO = MESHES(NOM)%CELL_INDEX(IIO,JJO,KKO)
         IW_OLD = MESHES(NOM)%CELL(ICO)%WALL_INDEX(-IOR)
      ELSE
         IW_OLD = 0
      ENDIF
   ELSE
   ...
   IF (IW_OLD>0) THEN
      ...
      ELSEIF (.NOT.MESHES(NOM)%CELL(ICG_OLD)%SOLID .AND. .NOT.MESHES(NOM)%CELL(ICG)%SOLID .AND. MESHES(NOM)%CELL(IC)%SOLID .AND. &
