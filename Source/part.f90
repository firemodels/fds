   ...
   IF (INIT_INVOKED_BY_SURF) THEN
      ...
      DO IW=1,N_INTERNAL_WALL_CELLS+N_EXTERNAL_WALL_CELLS
         ...
         IF (BC%IOR==3 .AND. SF%INIT_INDICES(1)>0) THEN
            ...
            Z_OFFSET = BC%Z
            P1 => MESHES(NM)%BOUNDARY_PROP1(WC%P1_INDEX)
            INIT_CALLS = REAL(SF%INIT_PER_AREA*P1%AREA)
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   ...
   P1 => MESHES(NM)%BOUNDARY_PROP1(CFA%P1_INDEX)
...
INSERT_TYPE_LOOP: DO INSERT_TYPE = 1,2
   ...
   SELECT CASE (INSERT_TYPE)
      CASE (1) ! PART_ID on SURF
         ...
         IF (T < P1%T_IGN) CYCLE INSERT_TYPE_LOOP
         ...
         IF (ANY(SF%EMBER_GENERATION_HEIGHT>=0._EB)) THEN
            ...
            IF (.NOT. P1%M_DOT_G_PP_ADJUST(REACTION(1)%FUEL_SMIX_INDEX)>0._EB) CYCLE INSERT_TYPE_LOOP
         ENDIF
         ! If there is a flow ramp and the value is zero, return.
         IF (SF%PARTICLE_MASS_FLUX > 0._EB) THEN
            IF (ABS(P1%T_IGN-T_BEGIN)<=TWO_EPSILON_EB .AND. SF%RAMP(TIME_PART)%INDEX>=1) THEN
               TSI = T
            ELSE
               TSI = T - P1%T_IGN
            ENDIF
         ...
         ELSEIF (PRESENT(CFACE_INDEX)) THEN
            ...
            IF (INSERT_TYPE==1 .AND. LP%EMBER) THEN
               ...
               BC%Z = CFA_Z + SF%EMBER_GENERATION_HEIGHT(1) + &
                      (SF%EMBER_GENERATION_HEIGHT(2)-SF%EMBER_GENERATION_HEIGHT(1))*REAL(RN3,EB)
            ENDIF
            LP%U = DOT_PRODUCT(CFA%NVEC,(/-P1%U_NORMAL,SF%VEL_T(1),SF%VEL_T(2)/))
            LP%V = DOT_PRODUCT(CFA%NVEC,(/SF%VEL_T(1),-P1%U_NORMAL,SF%VEL_T(2)/))
            LP%W = DOT_PRODUCT(CFA%NVEC,(/SF%VEL_T(1),SF%VEL_T(2),-P1%U_NORMAL/))
         ...
         SELECT CASE (INSERT_TYPE)
               ...
               IF (MASS_SUM > 0._EB) THEN
                  IF (SF%PARTICLE_MASS_FLUX > 0._EB) THEN
                     DO I=1,SF%NPPC
                        ...
                        LP%PWT = LP%PWT * FLOW_RATE*P1%AREA_ADJUST*P1%AREA*SF%DT_INSERT/MASS_SUM
                     ENDDO
                  ELSEIF (SF%PARTICLE_SURFACE_DENSITY > 0._EB) THEN
                     DO I=1,SF%NPPC
                        LP => LAGRANGIAN_PARTICLE(LP_INDEX_LOOKUP(I))
                        LP%PWT = LP%PWT * SF%PARTICLE_SURFACE_DENSITY*P1%AREA_ADJUST*P1%AREA/MASS_SUM
                     ENDDO
                  ENDIF
               ENDIF
            ...
            CASE (2) ! PART_ID on MATL
               IF (MASS_SUM > 0._EB) THEN
                  DO I=1,SF%NPPC
                     LP => LAGRANGIAN_PARTICLE(LP_INDEX_LOOKUP(I))
                     LP%PWT = LP%PWT * ONE_D%PART_MASS(N_LPC)*P1%AREA_ADJUST*P1%AREA* &
                              MIN(1._EB,SF%DT_INSERT/P1%T_MATL_PART)/MASS_SUM
                  ENDDO
               ENDIF
   ...
   IF (INSERT_TYPE==2) THEN
      ...
      ONE_D%PART_MASS = MAX(0._EB,1-SF%DT_INSERT/(P1%T_MATL_PART+TINY_EB))*ONE_D%PART_MASS
      ONE_D%PART_ENTHALPY = MAX(0._EB,1-SF%DT_INSERT/(P1%T_MATL_PART+TINY_EB))*ONE_D%PART_ENTHALPY
      P1%T_MATL_PART = MAX(0._EB,P1%T_MATL_PART-SF%DT_INSERT)
   ENDIF
      ...
      IF (IN%ID==DV%INIT_ID) THEN ! the INIT line is referred to be the DEVC line
         IF (DV%QUANTITY(1)=='RADIATIVE HEAT FLUX' .OR. &
            ...
            IF (DV%PROP_INDEX>0) THEN
               P1 => MESHES(NM)%BOUNDARY_PROP1(LP%P1_INDEX)
               P1%EMISSIVITY = PROPERTY(DV%PROP_INDEX)%EMISSIVITY
               P1%Q_RAD_OUT  = PROPERTY(DV%PROP_INDEX)%EMISSIVITY*SIGMA*TMPA4
               IF (PROPERTY(DV%PROP_INDEX)%HEAT_TRANSFER_COEFFICIENT>0._EB) &
                  P1%HEAT_TRANS_COEF = PROPERTY(DV%PROP_INDEX)%HEAT_TRANSFER_COEFFICIENT
            ENDIF
         ENDIF
         EXIT
      ENDIF
...
IF (LPC%SOLID_PARTICLE) THEN
   ...
   SELECT CASE (LPC%DRAG_LAW)
      ...
      CASE (POROUS_DRAG)
         ...
         SELECT CASE (LP_SF%GEOMETRY)
            CASE (SURF_CARTESIAN)
               P1%AREA = LP_VOLUME/LP_SF%THICKNESS
               LPC%LENGTH = SQRT(P1%AREA )
               IF (LP_SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
                  DO N=1,LP_SF%N_LAYERS
                     LP%MASS = LP%MASS + LP_SF%LAYER_THICKNESS(N)*LP_SF%LAYER_DENSITY(N)
                  END DO
                  LP%MASS = LP%MASS*P1%AREA
                  LP%RADIUS = SUM(LP_SF%LAYER_THICKNESS(:))/2._EB
               ENDIF
            CASE (SURF_CYLINDRICAL)
               X1 = SUM(LP_SF%LAYER_THICKNESS)+LP_SF%INNER_RADIUS
               LP%RADIUS = X1
               P1%AREA = 2._EB*LP_VOLUME/X1
               ...
               ENDIF
            CASE (SURF_SPHERICAL)
               P1%AREA = 3._EB*LP_VOLUME/(LP_SF%THICKNESS+LP_SF%INNER_RADIUS)
               LP%PWT = P1%AREA/(4._EB*PI*LP_SF%THICKNESS**2)
               IF (LP_SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
                  X1 = SUM(LP_SF%LAYER_THICKNESS)+LP_SF%INNER_RADIUS
                  LP%RADIUS = X1
                  DO N=LP_SF%N_LAYERS,1,-1
      ...
      CASE DEFAULT
         ...
         IF (LP_SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
            SELECT CASE (LP_SF%GEOMETRY)
               CASE (SURF_CARTESIAN)
                  P1%AREA = 2._EB*LP_SF%LENGTH*LP_SF%WIDTH
                  ...
                  ENDDO
               CASE (SURF_CYLINDRICAL)
                  P1%AREA = 2._EB*PI*(LP_SF%THICKNESS+LP_SF%INNER_RADIUS)*LP_SF%LENGTH
                  ...
                  ENDDO
               CASE (SURF_SPHERICAL)
                  P1%AREA = 4._EB*PI*(LP_SF%THICKNESS+LP_SF%INNER_RADIUS)**2
                  X1 = (SUM(LP_SF%LAYER_THICKNESS)+LP_SF%INNER_RADIUS)*SCALE_FACTOR
                  LP%RADIUS = X1
                  DO N=1,LP_SF%N_LAYERS
                     X2 = X1 - LP_SF%LAYER_THICKNESS(N)*SCALE_FACTOR
...
IF (LP%OD_INDEX>0) THEN
   ...
   ELSEIF (LP_SF%THERMAL_BC_INDEX==THERMALLY_THICK .AND. LP_SF%TMP_INNER>0._EB) THEN
      LP_ONE_D%TMP = LP_SF%TMP_INNER
   ELSE
      LP_ONE_D%TMP = TMPA
   ENDIF
   P1%TMP_F = LP_ONE_D%TMP(1)
ELSE
   IF (LPC%TMP_INITIAL>0._EB) THEN
      P1%TMP_F = LPC%TMP_INITIAL
   ELSE
      P1%TMP_F = TMPA
   ENDIF
ENDIF
...
IF (LP_SF%FIRE_SPREAD_RATE>0._EB) THEN
   P1%T_IGN = T_BEGIN + SQRT((BC%X-LP_SF%XYZ(1))**2 + (BC%Y-LP_SF%XYZ(2))**2 + (BC%Z-LP_SF%XYZ(3))**2)/LP_SF%FIRE_SPREAD_RATE
ELSE
   P1%T_IGN = LP_SF%T_IGN
ENDIF
...
DRAG_LAW_SELECT: SELECT CASE (LPC%DRAG_LAW)
   ...
   CASE DEFAULT
      ...
      IF (LPC%DRAG_LAW==DISK_DRAG) LP%RE = RHO_G*QREL*SQRT(P1%AREA/2._EB)/MU_FILM
   ...
   ELSE
      SELECT CASE(SF%GEOMETRY)
         ...
         CASE(SURF_CARTESIAN)
            ...
            IF (LPC%DRAG_LAW==DISK_DRAG) A_DRAG = P1%AREA/2._EB
         CASE(SURF_CYLINDRICAL)
...
PARTICLE_NON_STATIC_IF: IF (.NOT.LPC%STATIC .OR. LP%EMBER) THEN ! Move airborne, non-stationary particles
   ...
   IF (BETA>TWO_EPSILON_EB) THEN
      ...
      M_DOT = SUM(P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES))*P1%AREA
...
IF (ACCUMULATE_WATER .AND. .NOT.LP%SPLAT .AND. LPC%ADHERE_TO_SOLID) THEN
   P2%A_LP_MPUA(LPC%ARRAY_INDEX) = P2%A_LP_MPUA(LPC%ARRAY_INDEX) + LP%PWT*LPC%FTPR*R_D**3/P1%AREA
   LP%SPLAT = .TRUE.
ENDIF

IF ( P1%U_NORMAL>SURFACE(SURF_INDEX)%PARTICLE_EXTRACTION_VELOCITY .OR. &
     (POROUS_FLOOR .AND. BC%Z<ZS .AND. LPC%LIQUID_DROPLET) ) THEN
   IF (P1%NODE_INDEX > 0) THEN
      IF (DUCTNODE(P1%NODE_INDEX)%TRANSPORT_PARTICLES) THEN
         SELECT CASE (BCX%IOR)
            CASE(1)
               IF(BCX%IIG-2 < 1) THEN
                  SET_EXTRACT = .TRUE.
...
TYPE(BOUNDARY_ONE_D_TYPE), POINTER :: ONE_D
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: LP_P1,P1
TYPE(BOUNDARY_PROP2_TYPE), POINTER :: P2
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE (SURFACE_TYPE), POINTER :: SF
   ...
   IF (LPC%ARRAY_INDEX>0) THEN
      IF (LPC%LIQUID_DROPLET) THEN
         DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
            WC => WALL(IW)
            P2 => BOUNDARY_PROP2(WC%P2_INDEX)
            P2%LP_TEMP(LPC%ARRAY_INDEX) = 0._EB
            P2%LP_CPUA(LPC%ARRAY_INDEX) = LPC%RUNNING_AVERAGE_FACTOR_WALL*P2%LP_CPUA(LPC%ARRAY_INDEX)
         ENDDO
         DO ICF = N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
            CFA => CFACE(ICF)
            P2 => BOUNDARY_PROP2(CFA%P2_INDEX)
      ...
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         WC => WALL(IW)
         P2 => BOUNDARY_PROP2(WC%P2_INDEX)
         P2%LP_MPUA(LPC%ARRAY_INDEX) = LPC%RUNNING_AVERAGE_FACTOR_WALL*P2%LP_MPUA(LPC%ARRAY_INDEX)
      ENDDO
      DO ICF = N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
         CFA => CFACE(ICF)
         P2 => BOUNDARY_PROP2(CFA%P2_INDEX)
...
SPECIES_LOOP: DO Z_INDEX = 1,N_TRACKED_SPECIES
   ...
   FILM_SUMMING_LOOP: DO IP=1,NLP
      ...
      ENDIF
      P2%WORK2 = P2%WORK2 + LP%PWT*LP%RADIUS**3/P1%AREA  ! FILM_THICKNESS
   ENDDO FILM_SUMMING_LOOP

   DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC => WALL(IW)
      P2 => BOUNDARY_PROP2(WC%P2_INDEX)
      P2%WORK2 = MAX(MINIMUM_FILM_THICKNESS,FOTHPI*P2%WORK2)
   ENDDO
   DO ICF = N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      CFA => CFACE(ICF)
      P2 => BOUNDARY_PROP2(CFA%P2_INDEX)
   ...
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      BC  => BOUNDARY_COORD(LP%BC_INDEX)
      LP_P1  => BOUNDARY_PROP1(LP%P1_INDEX)
      ...
      TIME_ITERATION_LOOP: DO WHILE (DT_SUM < DT)
         ...
         KILL_RADIUS_CHECK: IF (LP%RADIUS>LPC%KILL_RADIUS) THEN
            ...
            SOLID_OR_GAS_PHASE_1: IF (BC%IOR/=0 .AND. (LP%WALL_INDEX>0 .OR. LP%CFACE_INDEX>0) .AND. .NOT. SF_FIXED) THEN
               SELECT CASE(ABS(BC%IOR))
                  ...
                  CASE(3)
                     VEL = SQRT(U2**2+V2**2)
               END SELECT
               A_DROP = M_DROP/(P2%WORK2*LPC%DENSITY)  ! WORK2 is FILM_THICKNESS
               Q_DOT_RAD = MIN(A_DROP,P1%AREA/LP%PWT)*P1%Q_RAD_IN
               TMP_WALL = MAX(TMPMIN,P2%WORK1)
            ...
            BOIL_ALL: IF (Q_DOT_RAD*DT_SUBSTEP > M_DROP*H_V) THEN  ! boil it all
               ...
               LP_P1%M_DOT_G_PP_ACTUAL(Z_INDEX) = LP_P1%M_DOT_G_PP_ACTUAL(Z_INDEX) + M_VAP/(A_DROP*DT)
            ...
            ELSE BOIL_ALL  ! boil only some of the liquid
               ...
               IF (Y_DROP > Y_GAS) LP_P1%B_NUMBER = (Y_DROP - Y_GAS) / MAX(DY_MIN_BLOWING,1._EB-Y_DROP)
               ...
               SOLID_OR_GAS_PHASE_2: IF (BC%IOR/=0 .AND. (LP%WALL_INDEX>0 .OR. LP%CFACE_INDEX>0) .AND. .NOT. SF_FIXED) THEN
                  ...
                  IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
                     ...
                     ENDDO
                     MCBAR = RHOCBAR*P1%AREA*(ONE_D%X(1)-ONE_D%X(0))
                     ARRAY_CASE = 3
                  ...
                  ELSE
                     ...
                     SELECT CASE(LPC%EVAP_MODEL)
                        ...
                        CASE(RM_B,RM_LEWIS_B) !Sazhin M0 - M1
                           H_MASS   = MAX(2._EB,SHERWOOD)*D_FILM/LENGTH*LOG(1._EB+LP_P1%B_NUMBER)/LP_P1%B_NUMBER
                        CASE(RM_FL_LEWIS_B) !Sazhin M2
                           H_MASS   = MAX(2._EB,SHERWOOD)*D_FILM/LENGTH*LOG(1._EB+LP_P1%B_NUMBER)/ &
                                     (LP_P1%B_NUMBER*F_B(LP_P1%B_NUMBER))
                     END SELECT
                  ENDIF
               ELSE SOLID_OR_GAS_PHASE_2
                  ...
                  CALL DROPLET_H_MASS_H_HEAT_GAS(H_MASS,H_HEAT,D_FILM,K_FILM,CP_FILM,RHO_FILM,LENGTH,Y_DROP,Y_GAS,&
                                                 LP_P1%B_NUMBER,NU_FAC_GAS,SH_FAC_GAS,RE_L,TMP_FILM,ZZ_GET,Z_INDEX,&
                                                 LPC%EVAP_MODEL)
                  H_WALL   = 0._EB
                  TMP_WALL = TMPA
                  ARRAY_CASE = 1
               ...
               IF (BC%IOR/=0 .AND. (LP%WALL_INDEX>0 .OR. LP%CFACE_INDEX>0) .AND. .NOT. SF_FIXED) P2%WORK1 = TMP_WALL_NEW
      ...
      IF (LP%WALL_INDEX>0 .OR. LP%CFACE_INDEX>0) THEN
         ...
         P2%LP_TEMP(LPC%ARRAY_INDEX) = P2%LP_TEMP(LPC%ARRAY_INDEX) + A_DROP*0.5_EB*(TMP_DROP+TMP_DROP_NEW)
         P2%LP_CPUA(LPC%ARRAY_INDEX) = P2%LP_CPUA(LPC%ARRAY_INDEX) + &
                                       (1._EB-LPC%RUNNING_AVERAGE_FACTOR_WALL)*WGT*Q_CON_WALL/(P1%AREA*DT)
         P1%Q_RAD_IN = (P1%AREA*DT*P1%Q_RAD_IN - WGT*DT*Q_DOT_RAD) / (P1%AREA*DT)
      ENDIF
   ...
   PART_CLASS_SUM_LOOP: DO N_LPC = 1,N_LAGRANGIAN_CLASSES
      ...
      PARTICLE_LOOP_2: DO IP=1,NLP
         ...
         IF (BC%IOR==0 .OR. LPC%SOLID_PARTICLE) THEN
            ...
            DROP_TMP(II,JJ,KK)  = DROP_TMP(II,JJ,KK)  + DEN_ADD*LP_P1%TMP_F
         ...
         IF (BC%IOR/=0 .AND. (LP%WALL_INDEX>0 .OR. LP%CFACE_INDEX>0) .AND. .NOT. SF_FIXED) THEN
            ...
            ELSE
               M_DROP = LP%MASS
            ENDIF
            P2%LP_MPUA(LPC%ARRAY_INDEX) = P2%LP_MPUA(LPC%ARRAY_INDEX) + &
                                          (1._EB-LPC%RUNNING_AVERAGE_FACTOR_WALL)*LP%PWT*M_DROP/P1%AREA
         ENDIF
     ...
     ! Compute cumulative quantities for PARTICLE "clouds"
      ...
      IF (LPC%LIQUID_DROPLET) THEN
         ...
         DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
            WC => WALL(IW)
            P2 => BOUNDARY_PROP2(WC%P2_INDEX)
            IF (P2%WORK2 > 0._EB) THEN
               P2%LP_TEMP(LPC%ARRAY_INDEX) = P2%LP_TEMP(LPC%ARRAY_INDEX)/P2%WORK2
               P2%LP_TEMP(LPC%ARRAY_INDEX) = MAX(SPECIES(LPC%Y_INDEX)%TMP_MELT,P2%LP_TEMP(LPC%ARRAY_INDEX))
            ELSE
               P2%LP_TEMP(LPC%ARRAY_INDEX) = TMPA
            ENDIF
         ENDDO
         DO ICF = N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
            CFA => CFACE(ICF)
...
SUM_MOMENTUM_LOOP: DO I=1,NLP
   ...
   CALL GET_IJK(BC%X,BC%Y,BC%Z,NM,XI,YJ,ZK,II,JJ,KK)
   IC = CELL_INDEX(II,JJ,KK)
   IF (CELL(IC)%SOLID) CYCLE SUM_MOMENTUM_LOOP
   X_WGT = XI - FLOOR(XI)
   Y_WGT = YJ - FLOOR(YJ)
   Z_WGT = ZK - FLOOR(ZK)

   IW = CELL(IC)%WALL_INDEX(-1)
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   WEED_LOOP: DO
      ...
      P1  => M%BOUNDARY_PROP1(LP%P1_INDEX)
      SF  => SURFACE(LPC%SURF_INDEX)
...
ENDIF
IF (LP%P1_INDEX>0) THEN
   M%BOUNDARY_PROP1_OCCUPANCY(LP%P1_INDEX) = 0
   M%NEXT_AVAILABLE_BOUNDARY_PROP1_SLOT = MIN(LP%P1_INDEX,M%NEXT_AVAILABLE_BOUNDARY_PROP1_SLOT)
ENDIF
IF (LP%P2_INDEX>0) THEN
   M%BOUNDARY_PROP2_OCCUPANCY(LP%P2_INDEX) = 0
   M%NEXT_AVAILABLE_BOUNDARY_PROP2_SLOT = MIN(LP%P2_INDEX,M%NEXT_AVAILABLE_BOUNDARY_PROP2_SLOT)
ENDIF
...
M%LAGRANGIAN_PARTICLE(NLP-IKILL)%P1_INDEX = 0
M%LAGRANGIAN_PARTICLE(NLP-IKILL)%P2_INDEX = 0
