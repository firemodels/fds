...
REAL(EB) :: T,YY_G,RHO_G,UN,DD,MFT,TSI,RADIUS,AREA_SCALING,&
            RVC,RHO_NEW,M_DOT_PPP,CP,MW_RATIO,H_G,DELTA_H_G,TMP_WGT2,TMP_WGT,YY_GET(1:N_GAS_SPECIES),CP_BG,CP_BG2,CP_G,CP_G2
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   IF (N_GAS_SPECIES==0 .AND. SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX .AND. SF%MASS_FLUX(0) == 0._EB) CYCLE WALL_CELL_LOOP
...
IF (VIRTUAL_PARTICLES .AND. CORRECTOR) THEN
   DROPLET_LOOP: DO I=1,NLP
      ...
      DO N=1,N_GAS_SPECIES
         MW_RATIO = SPECIES(Y2SPEC(N))%RCON/RSUM(II,JJ,KK)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   POINT_LOOP1: DO I=1,NWP
      ...
      MATERIAL_LOOP1a: DO N=1,SF%N_MATL
         IF (WC%RHO_S(I,N)== 0._EB) CYCLE MATERIAL_LOOP1a
         ML  => MATERIAL(SF%MATL_INDEX(N))
