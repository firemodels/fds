...
\noindent {\bf Species and Combustion}
    ...
    \begin{itemize}
    \item Custom species mixtures (``lumped species'') can be defined with the new input group SMIX.
...
There are two namelist groups that describe gas species. The first is {\ct SPEC}, which describes each {\em primitive} gas species that is included in the simulation. The second is {\ct SMIX} which defines lumped species or mixtures of primitive {\ct SPEC}.
...
{\ct HUMIDITY} can be specified for {\ct WATER VAPOR} in place of {\ct MASS\_FRACTION\_0}. {\ct HUMIDITY} is the relative humidity of water vapor in units of \%. It is 40 \% by default.
...
In general only one species for a given {\ct ID} can be defined; however, you may wish to model multiple inlet streams of a species and be able to identify how well the streams are mixing.  This can be done by using the character string {\ct ALIAS} on the {\ct SPEC} line.  The {\ct ID} must exist on a previouslys given {\ct SPEC} input.  For example, the lines:
...
\noindent define two gases, both of which are {\ct CARBON DIOXIDE}.  Both will use the built in property data for CO$_2$.  The {\ct ALIAS} for each species is then used in the remainder of the input file.  The {\ct SMIX\_COMPNONENT\_ONLY} is given so that FDS only tracks {\ct CO2 1} and {\ct CO2 2} and not also {\ct CARBON DIOXIDE}.  Note that the name you provide for {\ct ALIAS} should not match the name of any other {\ct SPEC} or {\ct SMIX}.  When using {\ct ALIAS}, species outputs that reference the {\ct ALIAS} will only output the {\ct ALIAS} species and use of the original species name will sum over all applicable species.  For example, if the output quantities in the example above are changed as shown below, then {\ct Device 1} would just output the mass fraction of {\ct CO2 1} and {\ct Device 2} would output the sum of both {
...
\section{Specifying Species Mixtures: The \texorpdfstring{{\tt SMIX}}{SMIX} Namelist Group}
\label{info:SMIX}
...
\noindent{\bf Example 2: Defining a background {\ct SMIX}}
\vskip\baselineskip
\footnotesize
\begin{verbatim}
...
\item[{\ct ID}] Character string identifying the name of the species. You must provide this.  This cannot be the same as an {\ct ID} or an {\ct ALIAS} on a {\ct SPEC} input.
...
&REAC ID = 'R1'
      ...
      NU = -1,-5,3,4
      SPEC_ID_N = 'PROPANE','OXYGEN'
      N_S = 0.1,1.65
\end{verbatim} \normalsize

\noindent
The array {\ct SPEC\_ID\_NU} contains the list of tracked species participating in the reaction as a product or reactant.  Note that a primitive species with {\ct SMIX\_COMPONONENT\_ONLY=.TRUE.} cannot be used in a reaction since it is not tracked separately.  The array {\ct SPEC\_ID\_N} contains the list of species with the exponents, {\ct N\_S}, in Eq.~(\ref{eq:reac_rate}).  This array must contain only primitive species, i.e. no species mixtures. The array of stoichiometric coefficients, {\ct NU}, can be taken directly from Eq.~(\ref{eq:propane_1step}). If we extend Eq.~(\ref{eq:propane_1step}) to include reactions that account for carbon monoxide production, we obtain:
...
{\ct MASS\_EXTINCTION\_COEFFICIENT}, is the $K_m$ in Eq.~(\ref{mec}). It is defined on one or more of the {\ct SPEC} lines\footnote{When using the simple chemistry combustion model, you can change the default mass extinction coefficient by adding a line to the input file of the form: \tt{\footnotesize \&SPEC ID='SOOT', MASS\_EXTINCTION\_COEFFICIENT=..., SMIX\_COMPONENT\_ONLY=.TRUE. /} } for the various light absorbing gas species. Its default value is 8700~m$^2$/kg, a value
...
{\ct SPEC\_ID\_N(:)}                      & Char.~Array & Section~\ref{info:SMIX}                   &                   &     \\ \hline
{\ct SMIX\_ID\_NU(:)}                     & Char.~Array & Section~\ref{info:SMIX}                   &                   &     \\ \hline
