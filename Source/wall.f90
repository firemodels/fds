...
WALL_CELL_LOOP_0: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (CALL_HT_1D) B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=IW)
...
IF (SOLID_PARTICLES) THEN
   ...
   PARTICLE_LOOP_0: DO IP=1, NLP
      ...
      IF (CALL_HT_1D) B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,PARTICLE_INDEX_IN=IP)
...
IF (N_THIN_WALL_CELLS>0 .AND. CALL_HT_1D) THEN
   ...
   DO ITW=1,N_THIN_WALL_CELLS
      ...
      IF (TW%WALL_INDEX_M>0) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=TW%WALL_INDEX_M)
      ELSEIF (TW%WALL_INDEX_P>0) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=TW%WALL_INDEX_P)
      ELSE
...
CFACE_LOOP_0: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   IF (CALL_HT_1D) B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,CFACE_INDEX_IN=ICF)
ENDDO CFACE_LOOP_0
!$OMP END DO
...
IF (PRESENT(PARTICLE_INDEX_IN)) THEN
   ...
   DN = SFX%CONV_LENGTH
   R_DROP = LPX%RADIUS
   IF (R_DROP>TWO_EPSILON_EB .AND. SFX%GEOMETRY/=SURF_CARTESIAN) CONV_LENGTH = 2._EB*R_DROP
   TMP_G  = P1X%TMP_G
   RHO_G  = P1X%RHO_G
...
TMP_LOOP: DO I=1,N_TGA
   ...
   IF (TGA_WALL_INDEX>0) THEN
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=IW)
      CALL SOLID_HEAT_TRANSFER(NM,T_TGA,TGA_DT,WALL_INDEX=IW)
   ELSE
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,PARTICLE_INDEX_IN=IP)
