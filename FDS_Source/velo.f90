...
REAL(EB) :: DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,SS,S12,S13,S23,DELTA,CS,YY_GET(1:N_SPECIES), &
            DAMPING_FACTOR,MU_WALL,YPLUS,TMP_WGT,NU,RE_DELTA,F_EMC
...
IF (LES .OR. EVACUATION_ONLY(NM)) THEN
   CS = CSMAG
   !$OMP DO COLLAPSE(3) PRIVATE(K,J,I,DELTA,DUDX,DVDY,DWDZ,DUDY,DUDZ,DVDX,DVDZ,DWDX,DWDY,S12,S13,S23,SS,NU,RE_DELTA,F_EMC)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            EMC_IF: IF (ENSEMBLE_MEAN_CLOSURE) THEN
               ...
               IF (RE_DELTA>1._EB) THEN
                  F_EMC = C_EMC*( SQRT(RE_DELTA**2-1._EB) + ASIN(1._EB/RE_DELTA) - PIO2 )
               ELSE
                  F_EMC = 0._EB
               ENDIF
               CS = SQRT(F_EMC)*CSMAG
