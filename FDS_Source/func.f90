...
TYPE (LAGRANGIAN_PARTICLE_TYPE), TARGET, ALLOCATABLE, DIMENSION(:) :: DUMMY
INTEGER, INTENT(IN) :: CODE,NM,NOM,NEW_DROPS
INTEGER :: IZERO,I,K
TYPE (MESH_TYPE), POINTER :: M=>NULL()
TYPE(OMESH_TYPE), POINTER :: M2=>NULL()

MESH_OR_OMESH: SELECT CASE(CODE)
   CASE(1) MESH_OR_OMESH
      M=>MESHES(NM)
      K=M%NLPDIM
      ALLOCATE(DUMMY(1:M%NLPDIM),STAT=IZERO)
      CALL ChkMemErr('FUNC:RE_ALLOCATE_PARTICLES','DUMMY',IZERO)
      DUMMY = M%LAGRANGIAN_PARTICLE      

      DO I=1,M%NLPDIM
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%TMP)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%TMP)
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%LAYER_THICKNESS)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%LAYER_THICKNESS)
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%X)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%X)
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX)
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX_ACTUAL)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX_ACTUAL)
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%RHO)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%RHO)
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%ILW)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%ILW)
         IF (ALLOCATED(M%LAGRANGIAN_PARTICLE(I)%ONE_D%N_LAYER_CELLS)) DEALLOCATE(M%LAGRANGIAN_PARTICLE(I)%ONE_D%N_LAYER_CELLS)
      ENDDO

      DEALLOCATE(M%LAGRANGIAN_PARTICLE)
      ALLOCATE(M%LAGRANGIAN_PARTICLE(M%NLPDIM+NEW_DROPS),STAT=IZERO)
      CALL ChkMemErr('FUNC:RE_ALLOCATE_PARTICLES','M%LAGRANGIAN_PARTICLE',IZERO)
      
      !Do loop for stack reduction
      DO I=1,M%NLPDIM
         M%LAGRANGIAN_PARTICLE(I) = DUMMY(I)
      ENDDO
      M%NLPDIM = M%NLPDIM+NEW_DROPS
      
   CASE(2) MESH_OR_OMESH
      M2=>MESHES(NM)%OMESH(NOM)
      K=M2%N_DROP_ORPHANS_DIM
      ALLOCATE(DUMMY(1:M2%N_DROP_ORPHANS_DIM),STAT=IZERO)
      CALL ChkMemErr('FUNC:RE_ALLOCATE_PARTICLES','DUMMY',IZERO)
      DUMMY = M2%LAGRANGIAN_PARTICLE

      DO I=1,M2%N_DROP_ORPHANS_DIM
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%TMP)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%TMP)
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%LAYER_THICKNESS)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%LAYER_THICKNESS)
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%X)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%X)
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX)
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX_ACTUAL)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX_ACTUAL)
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%RHO)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%RHO)
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%ILW)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%ILW)
         IF (ALLOCATED(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%N_LAYER_CELLS)) DEALLOCATE(M2%LAGRANGIAN_PARTICLE(I)%ONE_D%N_LAYER_CELLS)
      ENDDO
          
      DEALLOCATE(M2%LAGRANGIAN_PARTICLE)
      ALLOCATE(M2%LAGRANGIAN_PARTICLE(M2%N_DROP_ORPHANS_DIM+NEW_DROPS),STAT=IZERO)
      CALL ChkMemErr('FUNC:RE_ALLOCATE_PARTICLES','M2%LAGRANGIAN_PARTICLE',IZERO)
      M2%LAGRANGIAN_PARTICLE(1:M2%N_DROP_ORPHANS_DIM) = DUMMY(1:M2%N_DROP_ORPHANS_DIM)
      M2%N_DROP_ORPHANS_DIM = M2%N_DROP_ORPHANS_DIM + NEW_DROPS
END SELECT MESH_OR_OMESH

DO I=1,K
   IF (ALLOCATED(DUMMY(I)%ONE_D%TMP)) DEALLOCATE(DUMMY(I)%ONE_D%TMP)
   IF (ALLOCATED(DUMMY(I)%ONE_D%LAYER_THICKNESS)) DEALLOCATE(DUMMY(I)%ONE_D%LAYER_THICKNESS)
   IF (ALLOCATED(DUMMY(I)%ONE_D%X)) DEALLOCATE(DUMMY(I)%ONE_D%X)
   IF (ALLOCATED(DUMMY(I)%ONE_D%MASSFLUX)) DEALLOCATE(DUMMY(I)%ONE_D%MASSFLUX)
   IF (ALLOCATED(DUMMY(I)%ONE_D%MASSFLUX_ACTUAL)) DEALLOCATE(DUMMY(I)%ONE_D%MASSFLUX_ACTUAL)
   IF (ALLOCATED(DUMMY(I)%ONE_D%RHO)) DEALLOCATE(DUMMY(I)%ONE_D%RHO)
   IF (ALLOCATED(DUMMY(I)%ONE_D%ILW)) DEALLOCATE(DUMMY(I)%ONE_D%ILW)
   IF (ALLOCATED(DUMMY(I)%ONE_D%N_LAYER_CELLS)) DEALLOCATE(DUMMY(I)%ONE_D%N_LAYER_CELLS)
ENDDO   
...
IF (RADIATION) THEN
   IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
      IF (ALLOCATED(ONE_D%ILW)) DEALLOCATE(ONE_D%ILW)
      ALLOCATE(ONE_D%ILW(ANGLE_INCREMENT,NSB),STAT=IZERO)
      CALL ChkMemErr('FUNC','ILW',IZERO)
   ELSE
      IF (ALLOCATED(ONE_D%ILW)) DEALLOCATE(ONE_D%ILW)
      ALLOCATE(ONE_D%ILW(NRA,NSB),STAT=IZERO)
      CALL ChkMemErr('FUNC','ILW',IZERO)
   ENDIF
   IF (WC%BOUNDARY_TYPE==VIRTUAL_BOUNDARY) THEN
      ONE_D%ILW = 0._EB
   ELSE
      ONE_D%ILW = SIGMA*TMPA4*RPI
   ENDIF
   ONE_D%QRADIN  = ONE_D%EMISSIVITY*SIGMA*TMPA4
   ONE_D%QRADOUT = ONE_D%EMISSIVITY*SIGMA*TMPA4
ELSE
      ALLOCATE(ONE_D%ILW(1,1),STAT=IZERO)
      CALL ChkMemErr('FUNC','ILW',IZERO)
      ONE_D%ILW = 0._EB
ENDIF
...
IF (SF%THERMALLY_THICK) THEN
   CALL ALLOCATE_ONE_D_XFER(NM,WC%SURF_INDEX,WALL_INDEX=IW)
   ONE_D%SHRINKING = .FALSE.
   ONE_D%BURNAWAY  = .FALSE.
   WC%TMP_F = ONE_D%TMP(0)
   WC%TMP_B = ONE_D%TMP(SF%N_CELLS+1)
ENDIF
...
SUBROUTINE ALLOCATE_ONE_D_XFER(NM,SURF_INDEX,WALL_INDEX,LAGRANGIAN_PARTICLE_INDEX)
INTEGER, INTENT(IN):: NM,SURF_INDEX
INTEGER, INTENT(IN), OPTIONAL:: WALL_INDEX,LAGRANGIAN_PARTICLE_INDEX
INTEGER:: II,IL,IZERO,N,NN,KK
REAL(EB) :: ZK
TYPE(SURFACE_TYPE), POINTER :: SF=>NULL()
TYPE(ONE_D_M_AND_E_XFER_TYPE), POINTER :: ONE_D=>NULL()

IF (PRESENT(WALL_INDEX)) THEN
   ONE_D=>MESHES(NM)%WALL(WALL_INDEX)%ONE_D
   KK = MESHES(NM)%WALL(WALL_INDEX)%KK
ELSEIF(PRESENT(LAGRANGIAN_PARTICLE_INDEX)) THEN
   ONE_D=>MESHES(NM)%LAGRANGIAN_PARTICLE(LAGRANGIAN_PARTICLE_INDEX)%ONE_D
   ZK = MESHES(NM)%CELLSK(FLOOR((MESHES(NM)%LAGRANGIAN_PARTICLE(LAGRANGIAN_PARTICLE_INDEX)%Z-MESHES(NM)%ZS)*MESHES(NM)%RDZINT))
   KK = FLOOR(ZK+1._EB)   
ELSE
   RETURN
ENDIF
...
ALLOCATE(ONE_D%X(0:SF%N_CELLS),STAT=IZERO)
CALL ChkMemErr('FUNC','ONE_D%X',IZERO)
ONE_D%X = SF%X_S
IF (ALLOCATED(ONE_D%TMP)) DEALLOCATE(ONE_D%TMP)
ALLOCATE(ONE_D%TMP(0:SF%N_CELLS+1),STAT=IZERO)
CALL ChkMemErr('FUNC','ONE_D%TMP',IZERO)
IF (ALLOCATED(ONE_D%RHO)) DEALLOCATE(ONE_D%RHO)
ALLOCATE(ONE_D%RHO(0:SF%N_CELLS+1,SF%N_MATL),STAT=IZERO)
CALL ChkMemErr('FUNC','ONE_D%RHO',IZERO)
ONE_D%RHO = 0._EB
DO II=0,SF%N_CELLS+1
   IL = SF%LAYER_INDEX(II)
   IF (SF%TMP_INNER(IL)>0._EB) THEN
      ONE_D%TMP(II) = SF%TMP_INNER(IL)
   ELSE
      ONE_D%TMP(II) = MESHES(NM)%TMP_0(KK)
   ENDIF
   DO NN=1,SF%N_LAYER_MATL(IL)
      DO N=1,SF%N_MATL
         IF (SF%LAYER_MATL_INDEX(IL,NN)==SF%MATL_INDEX(N)) &
            ONE_D%RHO(II,N) = SF%LAYER_MATL_FRAC(IL,NN)*SF%LAYER_DENSITY(IL)
      ENDDO
   ENDDO
ENDDO
...
IF (SF%SHRINK) THEN
   ...
   ALLOCATE(ONE_D%LAYER_THICKNESS(SF%N_LAYERS),STAT=IZERO)
   CALL ChkMemErr('FUNC','ONE_D%LAYER_THICKNESS',IZERO)
   ONE_D%LAYER_THICKNESS = SF%LAYER_THICKNESS(1:SF%N_LAYERS)
   IF (ALLOCATED(ONE_D%N_LAYER_CELLS)) DEALLOCATE(ONE_D%N_LAYER_CELLS)
   ALLOCATE(ONE_D%N_LAYER_CELLS(SF%N_LAYERS),STAT=IZERO)
   CALL ChkMemErr('FUNC','ONE_D%N_LAYER_CELLS',IZERO)
   ONE_D%N_LAYER_CELLS = SF%N_LAYER_CELLS
ELSE
   IF (ALLOCATED(ONE_D%LAYER_THICKNESS)) DEALLOCATE(ONE_D%LAYER_THICKNESS)
   ALLOCATE(ONE_D%LAYER_THICKNESS(1),STAT=IZERO)
   CALL ChkMemErr('FUNC','ONE_D%LAYER_THICKNESS',IZERO)
   ONE_D%LAYER_THICKNESS = SUM(SF%LAYER_THICKNESS)
   IF (ALLOCATED(ONE_D%N_LAYER_CELLS)) DEALLOCATE(ONE_D%N_LAYER_CELLS)
   ALLOCATE(ONE_D%N_LAYER_CELLS(1),STAT=IZERO)
   CALL ChkMemErr('FUNC','ONE_D%N_LAYER_CELLS',IZERO)
   ONE_D%N_LAYER_CELLS = SF%N_CELLS
ENDIF

END SUBROUTINE ALLOCATE_ONE_D_XFER
