...
SUBROUTINE MASS_FINITE_DIFFERENCES(T,NM)
...
USE MANUFACTURED_SOLUTIONS, ONLY: VD2D_MMS_RHO_SRC
...
! Manufactured solution mass source term

IF (PERIODIC_TEST==7) THEN
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            FRHO(I,J,K) = FRHO(I,J,K) - VD2D_MMS_RHO_SRC(XC(I),ZC(K),T)
         ENDDO
      ENDDO
   ENDDO
ENDIF
...
USE MANUFACTURED_SOLUTIONS, ONLY: VD2D_MMS_Z_OF_RHO,VD2D_MMS_RHO_SRC
