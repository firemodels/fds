...
USE PHYSICAL_FUNCTIONS, ONLY : GET_AVERAGE_SPECIFIC_HEAT,GET_SENSIBLE_ENTHALPY
REAL(EB) :: VC,TMP_N,RHO_N,U_N,ZZ_GET(1:N_TRACKED_SPECIES),E_DIFF,H_S,H_S_0,HDIFF,H_GRAD_J
INTEGER, INTENT(IN) :: NM
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY .AND. &
       WC%BOUNDARY_TYPE/=OPEN_BOUNDARY) CYCLE WALL_LOOP
   IIG = WC%IIG
   JJG = WC%JJG
   KKG = WC%KKG
   ...
   CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S_0,TMPA)
   IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) CHRR(NM) = CHRR(NM) + (WC%QCONF+WC%QRADIN-WC%QRADOUT)*WC%AW
   IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
      H_GRAD_J = 0._EB
      DO N=1,N_TRACKED_SPECIES
         ITMP  = MIN(5000,NINT(WC%TMP_F))
         HDIFF = H_S_Z(ITMP,N) - H_S_Z(ITMP,0)
         H_GRAD_J = H_GRAD_J + HDIFF*WC%RHODW(N)*(ZZ(IIG,JJG,KKG,N)-ZZ(WC%II,WC%JJ,WC%KK,N))*WC%RDN
      ENDDO
      E_DIFF = H_GRAD_J + WC%KW*(TMP(IIG,JJG,KKG)-TMP(WC%II,WC%JJ,WC%KK))*WC%RDN + WC%QRADIN-WC%QRADOUT
   ELSE
      E_DIFF = 0._EB
   ENDIF
   FHRR(NM) = FHRR(NM) + (U_N*RHO_N*(H_S-H_S_0)+E_DIFF)*WALL(IW)%AW
ENDDO WALL_LOOP

IF (N_REACTIONS > 0) THEN
   WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
      IF (WALL(IW)%BOUNDARY_TYPE/=SOLID_BOUNDARY .AND. &
         WALL(IW)%BOUNDARY_TYPE/=VIRTUAL_BOUNDARY) CYCLE WALL_LOOP2
         MLR(NM) = MLR(NM) + WALL(IW)%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)*WALL(IW)%AW*WALL(IW)%AREA_ADJUST
   ENDDO WALL_LOOP2
ENDIF
