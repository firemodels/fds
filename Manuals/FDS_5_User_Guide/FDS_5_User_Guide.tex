...
\end{tabular*}
\caption[Fan curve example.]{Pressure and volume flow for the {\bf fan\_test}.}
\label{fan_test}
\end{figure}
...
\subsection{Pyrolysis of Solid Fuels}
...
\subsubsection{Testing Solid Phase Properties}
...
330~$^\circ$C, a necessary parameter for the pyrolysis model described above.
...
burning rate (about 0.018~kg/m$^2$/s) yields a burning time of about 700~s. After this, the material is ``burned away.'' See Fig.~\ref{thermoplastic}.
...
Figure~\ref{charring_solid} shows the surface temperature and burning rate
...
\subsubsection{Example Case: couch}
...
{\ct SPECIFIC\_ENTHALPY}, and {\ct REFERENCE\_TEMPERATURE}.  If either {\ct SPECIFIC\_HEAT} or {\ct ENTHALPY} is specified, then both must be specified.   
 The {\ct REFERENCE\_TEMPERATURE} is the temperature that corresponds to the specified value of {\ct SPECIFIC\_ENTHALPY}.  In the absence of any of these parameters, the appropriate
...
The {\ct ORIFICE\_DIAMETER} input is not used, if either {\ct DROPLET\_VELOCITY} or {\ct SPRAY\_PATTERN\_TABLE} is given.
...
of 60~L/min. The first jet contains 0.2 of the total, the second, 0.8 of the total.
...
%{\ct LOCK\_TIME\_STEP}          & Logical       & Use constant time step                        &               & {\ct .FALSE.}     \\ \hline
...
%{\ct WERNER\_WENGLE\_WALL\_MODEL} & Logical       & Werner and Wengle model for viscous wall stress &               & {\ct .FALSE.}     \\ \hline
\end{longtable}
