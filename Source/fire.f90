...
PUBLIC COMBUSTION,COMBUSTION_BC
...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),CP,H_S_N,&
            REAC_SOURCE_TERM_TMP(N_TRACKED_SPECIES),Q_REAC_TMP(N_REACTIONS),RSUM_LOC,TRI_COR_TMP,TMP_FLAME_TMP
LOGICAL :: Q_EXISTS
...
REAL(EB) :: ERR_EST,ERR_TOL,A1(1:N_TRACKED_SPECIES),A2(1:N_TRACKED_SPECIES),A4(1:N_TRACKED_SPECIES),ZETA,ZETA_0,&
            ...
            VOL_FRAC_FLAME,ZZ_FLAME(1:N_TRACKED_SPECIES),PHI_TILDE
...
IF (TRI_MODEL .AND. Q_OUT>TWO_EPSILON_EB) THEN
   ...
   KAPPA_0     = GET_KAPPA(ZZ_0,TMP_IN,1)
   KAPPA_FLAME = GET_KAPPA(ZZ_FLAME,TMP_FLAME,1)

   TRI_NUMER = (1._EB-VOL_FRAC_FLAME)*KAPPA_0*TMP_IN**4 + VOL_FRAC_FLAME*KAPPA_FLAME*TMP_FLAME**4
   TRI_DENOM = KAPPA_0*TMP_IN**4

   TRI_COR_OUT = MAX(1._EB, MIN(TRI_NUMER/MAX(TRI_DENOM,TWO_EPSILON_EB), TRI_COR_MAX))
...
!IF (PRESENT(IIC)) THEN
!  IF (Q_OLD(IIC-1,JJC,KKC)>HRRPUV_SPARK .AND. U(IIC-1,JJC,KKC)>-LAMINAR_FLAME_SPEED) THEN ; EXTINCT = .FALSE. ; RETURN ; ENDIF
!  IF (Q_OLD(IIC+1,JJC,KKC)>HRRPUV_SPARK .AND. U(IIC  ,JJC,KKC)< LAMINAR_FLAME_SPEED) THEN ; EXTINCT = .FALSE. ; RETURN ; ENDIF
!  IF (Q_OLD(IIC,JJC-1,KKC)>HRRPUV_SPARK .AND. V(IIC,JJC-1,KKC)>-LAMINAR_FLAME_SPEED) THEN ; EXTINCT = .FALSE. ; RETURN ; ENDIF
!  IF (Q_OLD(IIC,JJC+1,KKC)>HRRPUV_SPARK .AND. V(IIC,JJC  ,KKC)< LAMINAR_FLAME_SPEED) THEN ; EXTINCT = .FALSE. ; RETURN ; ENDIF
!  IF (Q_OLD(IIC,JJC,KKC-1)>HRRPUV_SPARK .AND. W(IIC,JJC,KKC-1)>-LAMINAR_FLAME_SPEED) THEN ; EXTINCT = .FALSE. ; RETURN ; ENDIF
!  IF (Q_OLD(IIC,JJC,KKC+1)>HRRPUV_SPARK .AND. W(IIC,JJC,KKC  )< LAMINAR_FLAME_SPEED) THEN ; EXTINCT = .FALSE. ; RETURN ; ENDIF
!  IC = CELL_INDEX(IIC,JJC,KKC)
!  DO IOR=-3,3
!     IF (IOR==0) CYCLE
!     IW = WALL_INDEX(IC,IOR)
!     IF (IW==0) CYCLE
!     IF (WALL(IW)%ONE_D%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)>0._EB) THEN
!        EXTINCT = .FALSE.
!        EXIT
!     ENDIF
!  ENDDO
...
!CALL GET_ENTHALPY(ZZ_0,H_0,TMP_IN) ! H of reactants participating in reaction (includes chemical enthalpy)
!CALL GET_ENTHALPY(ZZ_IN,H_CRIT,AUTO_IGNITION_TEMPERATURE) ! H of products at the critical flame temperature
!IF (H_0 < H_CRIT) EXTINCT = .TRUE.
...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),CP,H_S_N,&
            REAC_SOURCE_TERM_TMP(N_TRACKED_SPECIES),Q_REAC_TMP(N_REACTIONS),VCELL,TRI_COR_TMP,TMP_FLAME_TMP
LOGICAL  :: Q_EXISTS_CC
TYPE (REACTION_TYPE), POINTER :: RN
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM
LOGICAL  :: DO_REACTION,REALIZABLE
