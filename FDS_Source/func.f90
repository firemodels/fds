...
SUBROUTINE GET_EQUILIBRIUM(Y_EQ,T_EQ,M_EQ,T_G,T_L,ZZ_G,M_G,M_L,SPEC_INDEX,SMIX_INDEX,Q_IN)
!For calling purposes PC%Y_INDEX = SPEC_INDEX, PC%Z_INDEX=SMIX_INDEX
REAL(EB), INTENT(IN) :: T_G,T_L,M_G,M_L,ZZ_G(0:N_TRACKED_SPECIES),Q_IN
...
REAL(EB) :: T_EQ_I,Y_G,MW_G,MW_RATIO,C_P_V,C_P_G,C_P_L,TMP_WGT,H_V,E_TOT,ZZ_NEW(0:N_TRACKED_SPECIES),X_EQ,&
            MW_V,DHOR,M_EQ_MAX,M_EQ_MIN,M_EQ_I,M_ZZ_I(0:N_TRACKED_SPECIES)
...
E_TOT = M_L*C_P_L*T_L + M_G*C_P_G*T_G+Q_IN
 
! Compute equilibrium vapor at maximum of gas or drop temperature (adjusted for radiation and convection)
T_EQ = MIN(SS%TMP_V,MAX(T_L+Q_IN/(M_L*C_P_L),T_G))
...
IF (Y_EQ >= 1._EB) THEN
   M_EQ_MAX = M_L
ELSE
   M_EQ_MAX = MAX(0._EB,(M_ZZ_I(SMIX_INDEX) - M_G*Y_EQ)/(Y_EQ-1._EB))
ENDIF

! Initial guess, evaporate 50 % of the maximum vapor
M_EQ = 0.5_EB*M_EQ_MAX
M_EQ_MIN = 0._EB
...
M_SEARCH: DO WHILE (ITER_M)
   ...
   ZZ_NEW = M_ZZ_I
   ZZ_NEW(SMIX_INDEX)=M_ZZ_I(SMIX_INDEX)+M_EQ
   ...
   ELSE
      EXIT M_SEARCH
   ENDIF
   IF (ABS(M_EQ_I-M_EQ)<TWO_EPSILON_EB) EXIT M_SEARCH ! exit if mass hasn't changed
ENDDO M_SEARCH
