...
\paragraph{{\ct VELOCITY\_TOLERANCE}}
...
\paragraph{{\ct GLMAT\_SOLVER}}

For non-overlapping meshes at same refinement level, setting the parameter \linebreak[4]{\ct GLMAT\_SOLVER} to {\ct .TRUE.} allows for the solution of the Poisson equation from the global discretization matrix. The solver used in this mode is Intel\textsuperscript{\textregistered} MKL Pardiso for serial targets, and Intel\textsuperscript{\textregistered} MKL Sparse Cluster Solver for MPI targets. Two solution modes are defined by the logical parameter {\ct PRES\_ON\_WHOLE\_DOMAIN}: If {\ct .TRUE.} (default), the $H$ equation is solved in cells covered by obstacles as well as gas phase cells in the spirit of FDS default solver. If {\ct .FALSE.} the pressure is solved only on gas cells, employing solid boundary conditions for pressure in immersed obstacles. This method is more computationally and memory intensive than the default solver with default options, but eliminates velocity penetration errors in obstacles as well as inter-mesh boundaries.

\paragraph{{\ct CHECK\_POISSON}}
...
In this example, air is pushed through a 30~cm long, two-dimensional channel at 0.5~m/s. A plate obstruction normal to the flow creates a Karman vortex street. The computational domain is divided into 4 meshes. Three simulations are performed: one in which the {\ct VELOCITY\_TOLERANCE} is set to the default value, one in which it is set to a relatively small value, and one in which the {\ct GLMAT\_SOLVER} is used. Figure~\ref{dancing_eddies} shows the downstream pressure histories for these cases compared to a simulation that uses only one mesh. The case with the tighter tolerance produces a better match to the single mesh solution, but at a higher computational cost---about a factor of 2.5 .  The {\ct GLMAT\_SOLVER} case takes just a little longer than the default---by a factor of 1.25---but the error is machine precision.  These cases used MPI for domain decomposition, but only a single OpenMP thread.  Note, however, that {\ct GLMAT\_SOLVER} is threaded, whereas the FFT solver (FDS default) is not.  
...
{\ct HT3D} can also be used in multiple meshes to transfer heat across mesh boundaries. An example that demonstrates its usage is provided in ({\ct ht3d\_2mesh}). Notice that we can also account for volumetric heat sources, below is an example from ({\ct ht3d\_source\_term}) input file:

\begin{lstlisting}
 &INIT XB=-.1,.1,-.1,.1,-.1,.1, HRRPUV=1/
\end{lstlisting}
 
{\ct HT3D} can use internal heat source as shown in the example below:
\begin{lstlisting}
 &MATL ID='orange', .../
 &SURF ID='SPHERE', HT3D=.TRUE./
 &OBST XB=..., HT3D=.TRUE., MATL_ID='orange', SURF_ID='SPHERE', INTERNAL_HEAT_SOURCE=200./
\end{lstlisting} 
Note that you should not specify the internal heat source at the {\ct SURF} line.
...
\ee
The Obukhov length, $L$, can be chosen from Table~\ref{OL}.
\begin{table}[!ht]
\centering
...
\label{OL}
\end{table}
Suggested values of the aerodynamic roughness length, $z_0$, are given in Table~\ref{z0}.
\begin{table}[!ht]
\centering
...
\noindent The scaling potential temperature, $\theta_*$, can be obtained from the relation:
\be
...
\subsection{Issues Related to Mean Forcing}
...
\label{info:INITIALIZATION_ONLY}
...
{\ct INITIALIZATION\_ONLY}                      & Logical       & Section~\ref{info:INITIALIZATION_ONLY}        &               & {\ct .FALSE.}     \\ \hline
