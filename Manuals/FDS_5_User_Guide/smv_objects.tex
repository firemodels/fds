...
giving underlying technical details may be found in the Smokeview User's Guide~\cite{Smokeview_Users_Guide_5}.
...
sphere with a fixed diameter.  Its appearance remains the same regardless of how an FDS input file is set up.  A
dynamic object is also defined using instructions found in {\tt objects.svo}\ but in addition uses data specified on
the {\tt \&PROP}\ namelist statement and/or data contained in a particle file.  As a result, the appearance of dynamic
objects depends on the particular FDS case that is run.   For example, the {\tt tsphere}\ object is dynamic.  The
diameter and an image used to cover the sphere (known as a texture map) is specified in an FDS input file.
...
or not.  When FDS determines that a device has activated it places a message in the {\tt .smv}\ file indicating the
...
indicate the origin, the part of the device displayed at the $(x,y,z)$ coordinate specified on the {\tt \&DEVC}\ input line.
...
Particle file data may be used to customize the appearance of Smokeview objects.  This data is accessed
in an object definition using object variable names that match shortened particle file quantity names -
those names that appear when the colorbar is displayed in Smokeview.  For example, {\tt U-VEL}, {\tt V-VEL},
{\tt W-VEL} and {\tt temp} are shortened quantity names that correspond to the full names {\tt U-VELOCITY},
{\tt V-VELOCITY}, {\tt W-VELOCITY} and {\tt TEMPERATURE}.  These full names are documented in Table \ref{tab:output}
in this guide.  Shortened particle file quantity names also appear in the Smokeview
...
OBJECTDEF // color with FDS quantity, stretch with velocity
 ...
 \end{verbatim}
The variables {\tt U-VEL}, {\tt V-VEL}, and {\tt W-VEL} are also particle file quantities (shortened version) that may be
...
velocity particle data respectively.  Table \ref{tab:devices_dynamic2} documents those objects that can be customized
...
\incgraphics{scriptfigures/velegg} \\ \hline
\end{longtable}
