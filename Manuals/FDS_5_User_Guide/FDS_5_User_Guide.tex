...
\item[{\ct FLUSH\_FILE\_BUFFERS}] By default, every 10 time steps FDS purges the output file buffers and forces the data to be written out into the respective output files. It has been
noticed on Windows machines that ocassionally a runtime error occurs because of file access problems related to the buffer flushing. If this happens, set this parameter to {\ct .FALSE.}, but
be aware that it may not be possible to look at output in Smokeview until after the calculation is finished.
\end{description}
