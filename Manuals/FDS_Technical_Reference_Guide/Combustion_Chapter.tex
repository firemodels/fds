...
\label{eq:tau_sgs}  \tau_{\rm u} &= \frac{\Delta}{\sqrt{(2/3)k_{\rm sgs}}} \\
...
\subsection{Turbulent Batch Reactor (TBR) Model}
...
For multiple reactions with infinitely fast chemistry, a more general boundedness correction algorithm is used. In this formulation, the change in fuel is simply the amount of the fuel present
...
\end{equation}
Using $A_i^{\prime}$, the reaction rate in becomes
...
To maintain realizable species mass fractions ($\hat{Y}_{\alpha}\ge0$ and $\sum\hat{Y}_{\alpha}=1$), a predictor-corrector scheme is used
\begin{align}
...
\beta = \left\{ \begin{array}{ll} \displaystyle
          \displaystyle \frac{0-\hat{Y}_{\alpha}^k}{\lvert \hat{Y}_{\alpha}^p\rvert + \hat{Y}_{\alpha}^k}  & \quad {\hat{Y}_\alpha^p \le 0} \\ [0.2in]
...
\subsubsection{Lumped Species Approach}
...
\end{align}
\subsubsection{Primitive Species Approach}
