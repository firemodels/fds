...
REAL(EB) :: PX,PY,PZ,X1,X2,Y1,Y2,Z1,Z2,T_ACTIVATE,XIN,YIN,ZIN,DIST,XW,YW,ZW,UW,RDN,AW,TSI,&
            ZZ_GET(1:N_TRACKED_SPECIES),RSUM_F,UN,R1,RR
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   IF (SF%PROFILE==BOUNDARY_LAYER_PROFILE) THEN
      ...
      SELECT CASE(ABS(IOR))
         CASE(1)
            IF (VT%RADIUS>0._EB) THEN
               IF ( SF%DISPLACEMENT_THICKNESS<TWO_EPSILON_EB ) THEN
                  SF%DISPLACEMENT_THICKNESS = VT%RADIUS - SQRT( VT%RADIUS**2*(2._EB*ABS(SF%VEL_BULK/SF%VEL)-1._EB) )
               ENDIF
               R1 = VT%RADIUS - SF%DISPLACEMENT_THICKNESS
               RR = SQRT( (M%YC(J)-VT%Y0)**2 + (M%ZC(K)-VT%Z0)**2 )
               IF (RR>R1 .AND. RR<=VT%RADIUS .AND. SF%DISPLACEMENT_THICKNESS>TWO_EPSILON_EB) THEN
                  WC%UW0 = WC%UW0*(1._EB - ((RR-R1)/SF%DISPLACEMENT_THICKNESS)**2 )
               ENDIF
            ENDIF
         CASE(2)
            IF (VT%RADIUS>0._EB) THEN
