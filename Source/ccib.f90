   ...
   DO JCC=1,CUT_CELL(ICC)%NCELL
      ...
      IFC_LOOP : DO IFC=1,CUT_CELL(ICC)%CCELEM(1,JCC)
         ...
         CASE(CC_FTYPE_CFINB) ! INBOUNDARY CUT FACE: Note here we add U_NORMAL with Transpiration velocity due to
            ...
            AF      = CUT_FACE(IFC2)%AREA(IFACE2)
            VELN    = CUT_FACE(IFC2)%VELS(IFACE2)
            MURDN   = MU_TMP * BOUNDARY_PROP1(CFACE(ICFA)%P1_INDEX)%RDN
...
IF (CHECK_HT) THEN
   DO IFACE=1,N_EXTERNAL_CFACE_CELLS+N_INTWALL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      ...
      VELN = (ABS(P1%Q_CON_F)/P1%RHO_F)**ONTH * 2._EB*P1%RDN
      CFLMAX_TMP = DT * VELN
      IF (CFLMAX_TMP > CFL) THEN
         UVWMAX = VELN
         CFL = CFLMAX_TMP
...
TYPE (EXTERNAL_WALL_TYPE), POINTER :: EWC
TYPE (CFACE_TYPE), POINTER :: CFA
TYPE (BOUNDARY_PROP1_TYPE), POINTER :: CFA_P1
...
CFACE_LOOP_1 : DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTWALL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   EXT_CFACE_IF : IF (ICF<=N_EXTERNAL_CFACE_CELLS) THEN
      ...
      OPEN_IF: IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
         ...
         IF (ABS(P1%T_IGN-T_BEGIN)<=TWO_EPSILON_EB .AND. VT%PRESSURE_RAMP_INDEX >=1) THEN
            TSI = T
         ELSE
            TSI = T - T_BEGIN
         ENDIF
         ...
         CFA_P1 => BOUNDARY_PROP1(CFA%P1_INDEX)
         ...
         SELECT CASE(IOR)
            CASE( 1)
               IF (UU(0,J,K)<0._EB) THEN
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + KRES(1,J,K)
               ELSE
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + H0
               ENDIF
               BXS(J,K) = CFA%PRES_BXN
            CASE(-1)
               IF (UU(IBAR,J,K)>0._EB) THEN
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + KRES(IBAR,J,K)
               ELSE
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + H0
               ENDIF
               BXF(J,K) = CFA%PRES_BXN
            CASE( 2)
               IF (VV(I,0,K)<0._EB) THEN
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + KRES(I,1,K)
               ELSE
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + H0
               ENDIF
               BYS(I,K) = CFA%PRES_BXN
            CASE(-2)
               IF (VV(I,JBAR,K)>0._EB) THEN
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + KRES(I,JBAR,K)
               ELSE
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + H0
               ENDIF
               BYF(I,K) = CFA%PRES_BXN
            CASE( 3)
               IF (WW(I,J,0)<0._EB) THEN
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + KRES(I,J,1)
               ELSE
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + H0
               ENDIF
               BZS(I,J) = CFA%PRES_BXN
            CASE(-3)
               IF (WW(I,J,KBAR)>0._EB) THEN
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + KRES(I,J,KBAR)
               ELSE
                  CFA%PRES_BXN = P_EXTERNAL/CFA_P1%RHO_F + H0
               ENDIF
               BZF(I,J) = CFA%PRES_BXN
...
DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   VEL_WALL = -P1%U_NORMAL*NVEC + SF%VEL_T(1)*TVEC1 + SF%VEL_T(2)*TVEC2
   ...
   CALL WALL_MODEL(SLIP_FACTOR,P2%U_TAU,P2%Y_PLUS,MU_WALL/RHO_WALL,SF%ROUGHNESS,0.5_EB*DN,U_STRM)
...
PREDICT_NORMALS: IF (PREDICTOR) THEN
   ...
   CFACE_LOOP: DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTWALL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      ...
      WALL_CELL_TYPE: SELECT CASE (CFA%BOUNDARY_TYPE)
         ...
         CASE (SOLID_BOUNDARY)
            ...
            IF (ABS(P1%T_IGN-T_BEGIN) < SPACING(P1%T_IGN) .AND. SF%RAMP(TIME_VELO)%INDEX>=1) THEN
               TSI = T + DT
            ELSE
               TSI = T + DT - P1%T_IGN
               IF (TSI<0._EB) THEN
            ...
            P1%U_NORMAL_S = TIME_RAMP_FACTOR*P1%U_NORMAL_0

            ! Special Cases
            ! NEUMANN_IF: IF (SF%SPECIFIED_NORMAL_GRADIENT) THEN
            ! TO DO, following PREDICT_NORMAL_VELOCITY.

            IF (ABS(SURFACE(CFA%SURF_INDEX)%MASS_FLUX_TOTAL)>=TWO_EPSILON_EB) P1%U_NORMAL_S = &
                                                                              P1%U_NORMAL_S*RHOA/P1%RHO_F
   ...
   DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTWALL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      CFA => CFACE(ICF)
      P1  => BOUNDARY_PROP1(CFA%P1_INDEX)
      CFA%DUNDT = RDT*(P1%U_NORMAL_S-P1%U_NORMAL)
   ENDDO
...
ELSE PREDICT_NORMALS
   ...
   DO ICF=1,N_EXTERNAL_CFACE_CELLS+N_INTWALL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
      CFA => CFACE(ICF)
      P1 => BOUNDARY_PROP1(CFA%P1_INDEX)
      CFA%DUNDT = CFA%DUNDT + 2._EB*RDT*(P1%U_NORMAL-P1%U_NORMAL_S)
   ENDDO
...
IF (INT_NPE_HI > 0) THEN
   ...
   ENDDO
   ! PRESS = PRESS + P1%RHO_F*U_NORM**2._EB
ELSE
   ! Underlying cell approximate value:
   ICC = CUT_FACE(IND1)%CELL_LIST(2,LOW_IND,IND2)
   IIG = CUT_CELL(ICC)%IJK(1)
...
SUBROUTINE CFACE_THERMAL_GASVARS(ICF,P1)

TYPE(BOUNDARY_PROP1_TYPE) :: P1
INTEGER, INTENT(IN) :: ICF
...
IF (CFACE_INTERPOLATE) THEN
   ...
   IF (INT_NPE_HI > 0) THEN
      ...
      P1%U_TANG = SQRT(V_TANG(IAXIS)**2._EB+V_TANG(JAXIS)**2._EB+V_TANG(KAXIS)**2._EB)
...
CASE(CC_FTYPE_CFGAS) ! Cut-cell -> use value from CUT_CELL data struct:
   ...
   P1%U_TANG = SQRT( U_CAVG(IAXIS)**2._EB + U_CAVG(JAXIS)**2._EB + U_CAVG(KAXIS)**2._EB )
...
MESH_LOOP_02 : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      ...
      DO IFACE=1,CF%NFACE
         ...
         IROW = CUT_CELL(ICC)%UNKZ(JCC) - UNKZ_IND(NM_START)
         BOUNDARY_PROP1(CFACE(CF%CFACE_INDEX(IFACE))%P1_INDEX)%RDN = 1._EB/DXN_UNKZ_LOC(IROW)
      ENDDO
   ENDDO
ENDDO MESH_LOOP_02
...
AXIS_DO : DO X1AXIS = IAXIS,KAXIS
   ...
   IFACE_DO : DO IFACE=1,MESHES(NM)%CC_NREGFACE_Z(X1AXIS)
      ...
      IF (REGFACE_Z(IFACE)%IWC > 0) THEN
         ...
         REGFACE_Z(IFACE)%FN_ZZ(N)=P1%RHO_F*P1%ZZ_F(N)
      ELSE
         SELECT CASE(X1AXIS)
         CASE(IAXIS)
   ...
   IFACE_DO : DO IFACE=1,MESHES(NM)%CC_NREGFACE_Z(X1AXIS)
      ...
      IF (REGFACE_Z(IFACE)%IWC > 0) THEN
         ...
         ELSE
            TMP_F_GAS = P1%TMP_F
         ENDIF
         ZZ_GET(1:N_TRACKED_SPECIES) = P1%ZZ_F(1:N_TRACKED_SPECIES)
         CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,TMP_F_GAS)
         REGFACE_Z(IFACE)%FN_H_S = P1%RHO_F*H_S ! bar{rho*hs}
      ELSE
         SELECT CASE(X1AXIS)
         CASE(IAXIS)
...
DO IFACE=1,MESHES(NM)%CC_NREGFACE_Z(X1AXIS)
   ...
   RG_ON_WC_IF_1 : IF((IW>0).AND. .NOT.ANY(WALL(IW)%BOUNDARY_TYPE==(/INTERPOLATED_BOUNDARY,PERIODIC_BOUNDARY/))) THEN
      ...
      FN_ZZ           = P1%RHO_F*P1%ZZ_F(N)
      SELECT CASE(WC%BOUNDARY_TYPE)
         CASE DEFAULT
            VELC = UU(I,J,K)
         CASE(SOLID_BOUNDARY)
            IF (PREDICTOR) VELC = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL_S
            IF (CORRECTOR) VELC = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL
      END SELECT
   ...
   CF_ON_WALL_CELL_IF : IF (IW > 0) THEN
      ...
      DO IFACE=1,CF%NFACE
         ...
         IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN ! Here used the CFA corresponding U_NORMAL.
            FCT  = 1._EB
            P1 => BOUNDARY_PROP1(CFACE(CF%CFACE_INDEX(IFACE))%P1_INDEX)
            VELC =         PRFCT*P1%U_NORMAL + (1._EB-PRFCT)*P1%U_NORMAL_S
...
DO IFACE=1,MESHES(NM)%CC_NBBREGFACE_Z(X1AXIS)
   ...
   VELC      = UU(I,J,K)
   VELC2     = PRFCT*P1%U_NORMAL + (1._EB-PRFCT)*P1%U_NORMAL_S
   TMP_F_GAS = P1%TMP_F
   ...
   ZZ_GET(1:N_TRACKED_SPECIES) = P1%ZZ_F(1:N_TRACKED_SPECIES)
   CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,TMP_F_GAS)
   FN_H_S = P1%RHO_F*H_S ! bar{rho*hs}
   ! Add: -(bar{rho*hs} u dot n - (rho*hs) u dot n) to corresponding cell DP:
   AF = DY(J)*DZ(K)
...
DO ICF = 1,MESHES(NM)%N_BBCUTFACE_MESH
   ...
   DO IFACE=1,CUT_FACE(ICF)%NFACE
      ...
      IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN ! Here used the CFA corresponding U_NORMAL.
         FCT  = 1._EB
         P1 => BOUNDARY_PROP1(CFACE(CUT_FACE(ICF)%CFACE_INDEX(IFACE))%P1_INDEX)
         VELC =         PRFCT*P1%U_NORMAL + (1._EB-PRFCT)*P1%U_NORMAL_S
      ELSEIF(WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) THEN
         VELC = CUT_FACE(ICF)%VEL_SAVE(IFACE)
      ELSE
...
CFACE_LOOP : DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   ! H_RHO_D_DZDN
   UN_P =  PRFCT*P1%U_NORMAL + (1._EB-PRFCT)*P1%U_NORMAL_S
   TMP_G = P1%TMP_F
   IF (UN_P>0._EB) TMP_G = CFA%TMP_G
   DO N=1,N_TOTAL_SCALARS
      CALL GET_SENSIBLE_ENTHALPY_Z(N,TMP_G,H_S)
      CUT_FACE(IND1)%H_RHO_D_DZDN(N,IND2) = H_S*P1%RHO_D_DZDN_F(N)
   ENDDO
   ...
   CUT_CELL(ICC)%DEL_RHO_D_DEL_Z_VOL(1:N_TOTAL_SCALARS,JCC) = &
   CUT_CELL(ICC)%DEL_RHO_D_DEL_Z_VOL(1:N_TOTAL_SCALARS,JCC) - P1%RHO_D_DZDN_F(1:N_TOTAL_SCALARS)*CFA%AREA
ENDDO CFACE_LOOP
...
DO IFACE=1,MESHES(NM)%CC_NBBREGFACE_Z(X1AXIS)
   ...
   DPVOL(IIG,JJG,KKG) = DPVOL(IIG,JJG,KKG) - ( P1%Q_CON_F ) * AF  + WC%Q_LEAK * (DX(IIG)*DY(JJG)*DZ(KKG))
ENDDO
...
DO IFACE=1,MESHES(NM)%CC_NBBRCFACE_Z
   ...
   CASE(CC_FTYPE_CFGAS) ! Cut-cell -> use Temperature value from CUT_CELL data struct:
      ...
      CUT_CELL(ICC)%DVOL(JCC) - ( P1%Q_CON_F ) * AF + WC%Q_LEAK * CUT_CELL(ICC)%VOLUME(JCC) ! Qconf +ve sign is
   ...
   ELSE
      ...
      DO IFACE=1,CUT_FACE(ICF)%NFACE
         ...
         CASE(CC_FTYPE_CFGAS) ! Cut-cell -> use Temperature value from CUT_CELL data struct:
            ...
            JCC = CUT_FACE(ICF)%CELL_LIST(3,ISIDE+2,IFACE)
            CUT_CELL(ICC)%DVOL(JCC) = &
            CUT_CELL(ICC)%DVOL(JCC) - ( P1%Q_CON_F ) * AF + WC%Q_LEAK * CUT_CELL(ICC)%VOLUME(JCC) ! Qconf +ve sign
...
IF (PREDICTOR) THEN
  DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
     ...
     IND1 = CFA%CUT_FACE_IND1;                         IND2 = CFA%CUT_FACE_IND2
     ICC  = CUT_FACE(IND1)%CELL_LIST(2,LOW_IND,IND2);  JCC  = CUT_FACE(IND1)%CELL_LIST(3,LOW_IND,IND2)
     CUT_CELL(ICC)%DVOL(JCC)=CUT_CELL(ICC)%DVOL(JCC)-( P1%Q_CON_F ) * CUT_FACE(IND1)%AREA(IND2) ! QCONF(+) into solid.
  ENDDO
ELSE
  DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
     CFA  => CFACE(ICF)
...
DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   CFA => CFACE(ICF)
   P1 => BOUNDARY_PROP1(CFA%P1_INDEX)
   N_ZZ_MAX_V(ICF)=MAXLOC(P1%ZZ_F(1:N_TRACKED_SPECIES),1)
ENDDO
...
DIFFUSIVE_FLUX_LOOP: DO N=1,N_TOTAL_SCALARS
   ...
   CFACE_LOOP : DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
      CFA => CFACE(ICF)
      P1 => BOUNDARY_PROP1(CFA%P1_INDEX)
      ! Use external Gas point data for ZZ_G estimation, consistent with CFA%P1%RDN in the finite difference.
      ! Flux fixing done here for CFACEs:
      RHO_D_DZDN = 2._EB*P1%RHO_D_F(N)*(CFA%ZZ_G(N)-P1%ZZ_F(N))*P1%RDN
      IF (N==N_ZZ_MAX_V(ICF)) THEN
         ZZ_GET(1:N_TRACKED_SPECIES) = CFA%ZZ_G(1:N_TRACKED_SPECIES)
         RHO_D_DZDN_GET(1:N_TRACKED_SPECIES) = &
         2._EB*P1%RHO_D_F(1:N_TRACKED_SPECIES)*( ZZ_GET(1:N_TRACKED_SPECIES) - P1%ZZ_F(1:N_TRACKED_SPECIES))*P1%RDN
         RHO_D_DZDN = -(SUM(RHO_D_DZDN_GET(1:N_TRACKED_SPECIES))-RHO_D_DZDN)
      ENDIF
      P1%RHO_D_DZDN_F(N) = RHO_D_DZDN
      ...
      CUT_FACE(CFA%CUT_FACE_IND1)%ZZ_FACE(N,   CFA%CUT_FACE_IND2) = P1%ZZ_F(N)
      CUT_FACE(CFA%CUT_FACE_IND1)%TMP_FACE(    CFA%CUT_FACE_IND2) = P1%TMP_F
   ENDDO CFACE_LOOP
...
KKG = BC%KKG
IOR = BC%IOR
N_ZZ_MAX = MAXLOC(P1%ZZ_F(1:N_TRACKED_SPECIES),1)
RHO_D_DZDN = 2._EB*P1%RHO_D_F(N)*(ZZP(IIG,JJG,KKG,N)-P1%ZZ_F(N))*P1%RDN
IF (N==N_ZZ_MAX) THEN
   RHO_D_DZDN_GET = 2._EB*P1%RHO_D_F(:)*(ZZP(IIG,JJG,KKG,:)-P1%ZZ_F(:))*P1%RDN
...
N_ZZ_MAX = MAXLOC(P1%ZZ_F(1:N_TRACKED_SPECIES),1)
RHO_D_DZDN = P1%RHO_D_F(N)*(ZZ_G-P1%ZZ_F(N))*IDX
IF (N==N_ZZ_MAX) THEN
   RHO_D_DZDN_GET = P1%RHO_D_F(:)*(ZZ_GV(:)-P1%ZZ_F(:))*IDX
   RHO_D_DZDN = -(SUM(RHO_D_DZDN_GET(:))-RHO_D_DZDN)
ENDIF

IF (IOR < 0) RHO_D_DZDN = -RHO_D_DZDN ! This is to switch the sign of the spatial derivative in high side boundaries.
DIFF_FACE = P1%RHO_D_F(N)/P1%RHO_F
ZZ_FACE   = P1%ZZ_F(N)
TMP_FACE  = P1%TMP_F
   ...
   IF (PREDICTOR) THEN
      ...
      WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         SELECT CASE(WC%BOUNDARY_TYPE)
            ...
            CASE(SOLID_BOUNDARY);        UN = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL
   ...
   ELSE
      ...
      WALL_LOOP_2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         SELECT CASE(WC%BOUNDARY_TYPE)
            ...
            ! SOLID_BOUNDARY is not currently functional here, but keep for testing
            CASE(SOLID_BOUNDARY);        UN = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL_S
            CASE(INTERPOLATED_BOUNDARY); UN = UVW_SAVE(IW)
         END SELECT
         SELECT CASE(IOR)
            CASE( 1); UU(IIG-1,JJG,KKG) = UN
...
MESH_LOOP_DBND : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   IFACE_LOOP_RCF1: DO IFACE=1,MESHES(NM)%CC_NRCFACE_Z
      ...
      CASE(KAXIS)
         ...
         VELC = WP(I,J,K)
      END SELECT
      FN_ZZ           = P1%RHO_F*P1%ZZ_F(N) ! These have been Flux limited in wall.f90
      SELECT CASE(WC%BOUNDARY_TYPE)
         CASE DEFAULT
            ! Already filled in previous X1AXIS select case.
         CASE(SOLID_BOUNDARY)
...
IF (PREDICTOR) THEN
   VELX1 = -P1%U_NORMAL * CFACE(ICF)%NVEC(X1AXIS)
ELSE
   VELX1 = -P1%U_NORMAL_S* CFACE(ICF)%NVEC(X1AXIS)
ENDIF
...
MESHES_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   CUT_FACE_LOOP : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
      ...
      CUTFACE_STATUS_IF : IF(CUT_FACE(ICF)%STATUS == CC_INBOUNDARY) THEN ! CUTFACE_STATUS_IF
         ...
         ! Here we define interpolation stencils that will be used to populate P1%H_G, TMP_G, RHO_G,
