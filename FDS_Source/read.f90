...
REAL(EB) :: VEG_SV,VEG_MOISTURE,VEG_CHAR_FRACTION,VEG_DRAG_COEFFICIENT,VEG_DENSITY,VEG_BULK_DENSITY, &
            ...
            VEG_FUEL_MPV_MIN,VEG_MOIST_MPV_MIN,USER_DRAG_COEFFICIENT(3),FREE_AREA_FRACTION
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (ANY(USER_DRAG_COEFFICIENT>=0._EB)) THEN
      DRAG_LAW = 'USER'
      IF (USER_DRAG_COEFFICIENT(1)>=0._EB) THEN
         IF(USER_DRAG_COEFFICIENT(2)<0._EB .AND. USER_DRAG_COEFFICIENT(3) <0._EB) THEN
            USER_DRAG_COEFFICIENT(2) = USER_DRAG_COEFFICIENT(1)
            USER_DRAG_COEFFICIENT(3) = USER_DRAG_COEFFICIENT(1)
         ELSE
            WRITE(MESSAGE,'(A)') 'ERROR: Set all three components of USER_DRAG_COEFFICIENT'
            CALL SHUTDOWN(MESSAGE)            
         ENDIF
      ELSE
         WRITE(MESSAGE,'(A)') 'ERROR: Set all three components of USER_DRAG_COEFFICIENT'
