...
NAMELIST /GEOM/ AZIM, AZIM_DOT, COMPONENT_ONLY, DAZIM, DELEV, DSCALE, DXYZ0, DXYZ, &
                ELEV, ELEV_DOT, FACES, GAXIS, GROTATE, GROTATE_DOT,&
                GEOM_IDS, ID, SCALE, SCALE_DOT, SURF_ID, VERTS, XYZ0, XYZ, XYZ_DOT
...
! use GROTATE/GAXIS or AZIM/ELEV but not both
   ...
   IF(ALL(GAXIS(1:3).LT.MAX_COORD).OR.GROTATE.LT.MAX_COORD.OR.GROTATE_DOT.LT.MAX_COORD)THEN
      AZIM = 0.0_EB
      ELEV = 0.0_EB
      AZIM_DOT = 0.0_EB
      ELEV_DOT = 0.0_EB
   ...
   IF (ABS(GROTATE_DOT).LT.MAX_COORD) THEN
      AZIM_DOT = 0.0_EB
      ELEV_DOT = 0.0_EB
   ENDIF    
