...
REAL(EB) :: DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,SS,S12,S13,S23,DELTA,CS,ZZ_GET(1:N_TRACKED_SPECIES), &
...
IF (N_TRACKED_SPECIES == 0 .OR. EVACUATION_ONLY(NM)) THEN
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            MU(I,J,K)=(Z2MU_C(ITMP)+TMP_WGT*(Z2MU_C(ITMP+1)-Z2MU_C(ITMP)))*SPECIES_MIXTURE(0)%MW
         ENDDO
      ENDDO
   ENDDO
   !$OMP END DO
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
            ZZ_GET(:) = ZZP(I,J,K,:)
            CALL GET_VISCOSITY(ZZ_GET,MU(I,J,K),TMP(I,J,K)) !INTENT:INOUT,OUT,IN
         ENDDO
      ENDDO
   ENDDO
   !$OMP END DO
...
!!!$OMP DO PRIVATE(IW,II,JJ,KK,IIG,JJG,KKG,ZZ_GET,ITMP,TMP_WGT,MU_WALL,YPLUS) FIRSTPRIVATE(DAMPING_FACTOR)
   ...
   SELECT CASE(BOUNDARY_TYPE(IW))
      CASE(SOLID_BOUNDARY)
         IF (N_TRACKED_SPECIES>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
            ...
            CALL GET_VISCOSITY(ZZ_GET,MU_WALL,TMP(IIG,JJG,KKG)) !INTENT:INOUT,OUT,IN
         ELSE
            ITMP = INT(TMP(IIG,JJG,KKG))
            IF (ITMP > 5000) THEN
               MU_WALL = Z2MU_C(5000)*SPECIES_MIXTURE(0)%MW            
            ELSE
               TMP_WGT = TMP(IIG,JJG,KKG) - AINT(TMP(IIG,JJG,KKG))
               MU_WALL = (Z2MU_C(ITMP)+TMP_WGT*(Z2MU_C(ITMP+1)-Z2MU_C(ITMP)))*SPECIES_MIXTURE(0)%MW
            ENDIF
         ENDIF
            ...
            BOUNDARY_CONDITION: SELECT CASE(VELOCITY_BC_INDEX)
               ...
               CASE (WALL_MODEL) BOUNDARY_CONDITION
                  ...
                  ELSE
                     ...
                     MU_WALL = Z2MU_C(ITMP)*SPECIES_MIXTURE(0)%MW
...
REAL(EB) :: UODX,VODY,WODZ,UVW,UVWMAX,R_DX2,MU_MAX,MUTRM,CP,ZZ_GET(1:N_TRACKED_SPECIES)
