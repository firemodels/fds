...
The instantaneous absorption coefficient $\kappa_n$ of the gas mixture at location $\bx$ is calculated as a weighted sum of individual gases' gray or band-mean absorption coefficients $\kappa_{n,i}$
\begin{equation}
\kappa_{n}(\bx) = \sum_i Y_i(\bx) \kappa_{n,i}(Y_i(\bx),T(\bx))
\end{equation}
...
\begin{equation}
\kappa_{n,i,{\rm P}}(P,T) = \dfrac{\pi}{\sigma T^4}
...
The second coefficient is the so-called {\em path mean} or {\em effective} absorption coefficient, $\kappa_{n,i,{\rm e}}(P,T,L)$ which is defined according to the following equation
\be
   \underbrace{\int_{\la_{\rm min}}^{\la_{\rm max}}I(\la,L,T,T_{\rm rad}) \; \d \la}_{I_{\rm tot}}
   = \frac{\sigma}{\pi}
...
\be
  \kappa_{n,i,{\rm e}}(P,T,L) = -\frac{1}{L}\ln\left(\frac{I_{\rm tot}-I_b(T)}{I_b(T_{\rm rad})-I_b(T)}\right)
\ee
...
\be
   \kappa_{n,i}=\min \Big( \kappa_{n,i,{\rm P}}(P,T),\kappa_{n,i,{\rm e}}(P,T,L) \Big)
\ee
