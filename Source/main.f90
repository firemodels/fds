...
INTEGER, INTENT(IN) :: CODE
INTEGER :: NM,NOM,I,II,JJ,KK,LL,LLL,N,RNODE,SNODE,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IJK_SIZE,RC,IC,ICC,LC
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF_RECEIVE_PARTICLES: IF (CODE==11 .AND. OS%N_ITEMS>0) THEN
         ...
         DO IP=M%NLP+1,M%NLP+OS%N_ITEMS
            ...
            LP%RVC = M%RDX(BC%IIG)*M%RRN(BC%IIG)*M%RDY(BC%JJG)*M%RDZ(BC%KKG)
            IF (CC_IBM) THEN
               ICC = M%CCVAR(BC%IIG,BC%JJG,BC%KKG,4)
               IF (ICC>0) LP%RVC = 1/SUM(M%CUT_CELL(ICC)%VOLUME(:))
            ENDIF
