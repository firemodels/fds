...
In FDS, the turbulent viscosity of the first off-wall cell is interpolated between $\nu_t(y=0)=0$ at the wall and $\nu_t(y = 3\delta y/2)$ from the second off-wall cell.  The interpolation function is based on the Van Driest damping function with $y^+ = (\delta y/2)/\delta_\nu$.  Considering that the viscosity scales with $\ell_{mix}^{\,2}$\,, we obtain
\begin{equation}
\nu_t(\delta y/2) = \nu_t(3\delta y/2) \left[ \frac{(1 - e^{-y^+/A}) }{( 1 - e^{-3y^+/A}) } \right]^2
\end{equation}
