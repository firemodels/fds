...
REAL(EB) :: PHI_RN,RN,FLOW_RATE,THETA_RN,SPHI,CPHI,MASS_SUM,D_PRES_FACTOR, &
            ...
            TRIGT1,TRIGT2,TNOW,TSI,PIPE_PRESSURE
REAL(EB), PARAMETER :: VENT_OFFSET=0.1
INTEGER :: I,KS,II,JJ,KK,IC,IL,IU,IPC,DROP_SUM,IIG,JJG,KKG,IW,IBC,IOR,STRATUM
INTEGER :: N_OPEN_NOZZLES
...
REAL(EB) :: RHO_G,RVC,RDS,RDC,QREL,SFAC,UREL,VREL,WREL,TMP_G,RN,THETA_RN, &
            ...
            T_BU_BAG,T_BU_STRIP,B_1,THROHALF
LOGICAL :: HIT_SOLID
...
!$OMP PARALLEL DO FIRSTPRIVATE(GRVT1,GRVT2,GRVT3,SURFACE_TENSION) &
!$OMP PRIVATE(I,DR,PC,XI,YJ,ZK,II,JJ,KK,IC,IIX,JJY,KKZ,UBAR,VBAR,WBAR,RVC,RD,RDS,RDC,UREL,VREL,WREL,QREL) &
!$OMP PRIVATE(TMP_G,RHO_G,MU_AIR,C_DRAG,WE_G,T_BU_BAG,T_BU_STRIP) &
...
DROPLET_LOOP: DO I=1,NLP  
   ...
   DRAG_CALC: IF (DR%IOR==0 .AND. DR%RE>0) THEN
      ...
      !Secondary break-up
      BREAKUP: IF (SECONDARY_BREAKUP) THEN
         WE_G=RHO(II,JJ,KK)*QREL**2*2._EB*RD/SURFACE_TENSION
         T_BU_BAG    = T_END-T_BEGIN
         T_BU_STRIP  = T_END-T_BEGIN
         ! Breakup conditions according to WAVE model by Reitz (1987)
         IF (WE_G >= 12.0_EB)             T_BU_BAG   = 1.72_EB*B_1*SQRT(PC%DENSITY*RDC/(2*SURFACE_TENSION))
         ...
         IF ((T-DR%T) > MIN(T_BU_BAG,T_BU_STRIP)) THEN
            IF (PC%MONODISPERSE) THEN
               RD       = THROHALF*RD
            ELSE
               DO WHILE (RD > THROHALF*DR%R)
                  CALL RANDOM_CHOICE(PC%CDF(:),THROHALF*PC%R_CDF(:),NDC,RD)
               END DO
            ENDIF
            DR%RE    = RHO_G*QREL*2._EB*RD/MU_AIR
            IF (PC%DRAG_LAW==USER_DRAG) THEN
    ...
    !!!  ENDDO GET_N
         ...
         TIME_ITERATION_LOOP: DO WHILE (DT_SUM < DT)
            ...
            IF (ABS(TMP_G_NEW/TMP_G - 1._EB) > 0.2_EB) THEN
               DT_SUBSTEP = DT_SUBSTEP * 0.5_EB            
               IF (DT_SUBSTEP <= 0.00001_EB*DT) STOP
               CYCLE TIME_ITERATION_LOOP
            ENDIF
