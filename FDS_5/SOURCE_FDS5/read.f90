...
REWIND(LU5)
COUNT_MESH_LOOP: DO
   CALL CHECKREAD('MESH',LU5,IOS)
   IF (IOS==1) EXIT COUNT_MESH_LOOP
   READ(LU5,MESH,END=15,ERR=16,IOSTAT=IOS)
   NMESHES = NMESHES + 1
...
REWIND(LU5)
TIME_LOOP: DO
   CALL CHECKREAD('TIME',LU5,IOS)
   IF (IOS==1) EXIT TIME_LOOP
...
REWIND(LU5)
DUMP_LOOP: DO 
   CALL CHECKREAD('DUMP',LU5,IOS)
   IF (IOS==1) EXIT DUMP_LOOP
   READ(LU5,DUMP,END=23,ERR=24,IOSTAT=IOS)
...
INTEGER  :: N_SPEC_READ,N_MIX,N,NN
LOGICAL  :: ABSORBING
...
LOGICAL :: TEMPERATURE_DEPENDENT,IDEAL
...
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,OXIDIZER,EPUMO2,ID, N_S,&
                ...
                H2_YIELD,SOOT_YIELD,CO_YIELD,TEMPERATURE_DEPENDENT,Y_F_LFL,X_O2_LL,CRITICAL_FLAME_TEMPERATURE,NU,SOOT_H_FRACTION, &
...
COUNT_REAC_LOOP: DO
   ID   = 'null'
   FUEL = 'null'      
   CALL CHECKREAD('REAC',LU5,IOS)
   IF (IOS==1) EXIT COUNT_REAC_LOOP
   READ(LU5,REAC,ERR=434,IOSTAT=IOS)
...
CHARACTER(25) :: COLOR
LOGICAL :: MASSLESS,STATIC,FUEL,WATER,TREE
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC
...
NAMELIST /PART/ NUMBER_INITIAL_DROPLETS,FYI,DROPLETS_PER_SECOND,MASS_PER_VOLUME, &
                ...
                MELTING_TEMPERATURE,DIAMETER,GAMMA_D,HEAT_OF_COMBUSTION, &
                AGE,SPEC_ID,INITIAL_TEMPERATURE,XB,RGB,QUANTITIES,DT_INSERT,COLOR, &
                VERTICAL_VELOCITY,HORIZONTAL_VELOCITY,GAMMA
...
COUNT_PART_LOOP: DO
   ...
   READ(LU5,PART,END=219,ERR=220,IOSTAT=IOS)
   N_PART = N_PART + 1
...
REWIND(LU5)
COUNT_PROP_LOOP: DO
   CALL CHECKREAD('PROP',LU5,IOS)
   IF (IOS==1) EXIT COUNT_PROP_LOOP
   READ(LU5,PROP,ERR=34,IOSTAT=IOS)
   N_PROP = N_PROP + 1
   34 IF (IOS>0) THEN
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   CALL CHECKREAD('PROP',LU5,IOS)  ! Look for PROP lines in the input file
   CALL SET_PROP_DEFAULTS          ! Reset PROP NAMELIST parameters to default values 
   IF (N > 0) READ(LU5,PROP) 
...
COUNT_MATL_LOOP: DO
   ...
   READ(LU5,MATL,ERR=34,IOSTAT=IOS)
   N_MATL = N_MATL + 1
   MATL_NAME(N_MATL) = ID
   34 IF (IOS>0) THEN
...
N_SURF = 0
COUNT_SURF_LOOP: DO
   CALL CHECKREAD('SURF',LU5,IOS)
   IF (IOS==1) EXIT COUNT_SURF_LOOP
   READ(LU5,SURF,ERR=34,IOSTAT=IOS)
   N_SURF = N_SURF + 1
   SURF_NAME(N_SURF) = ID
   34 IF (IOS>0) THEN
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
   ...
   IF (ANY(SF%MASS_FLUX    /=0._EB) .OR. BURNING) SF%SPECIES_BC_INDEX = SPECIFIED_MASS_FLUX
...
ELSE
   RHOMAX = 3.0_EB*P_INF*MW_MAX/(R0*TMPMIN)
ENDIF
DO N=1,N_SPECIES
...
COUNT_RAMP_POINTS: DO N=1,N_RAMP
   ...
   SEARCH_LOOP: DO
      CALL CHECKREAD('RAMP',LU5,IOS)
      IF (IOS==1) EXIT SEARCH_LOOP
      READ(LU5,NML=RAMP,ERR=56,IOSTAT=IOS)
...
COUNT_TABLE_POINTS: DO N=1,N_TABLE
   ...
   END SELECT
   SEARCH_LOOP: DO
      CALL CHECKREAD('TABL',LU5,IOS)
      IF (IOS==1) EXIT SEARCH_LOOP
      TABLE_DATA = -999._EB
      READ(LU5,NML=TABL,ERR=56,IOSTAT=IOS)
...
REWIND(LU5)
COUNT_ZONE_LOOP: DO
   CALL CHECKREAD('ZONE',LU5,IOS)
   IF (IOS==1) EXIT COUNT_ZONE_LOOP
   READ(LU5,NML=ZONE,END=11,ERR=12,IOSTAT=IOS)
   N_ZONE = N_ZONE + 1
   12 IF (IOS>0) THEN
...
REWIND(LU5)
COUNT_DEVC_LOOP: DO
   CALL CHECKREAD('DEVC',LU5,IOS)
   IF (IOS==1) EXIT COUNT_DEVC_LOOP
   READ(LU5,NML=DEVC,END=11,ERR=12,IOSTAT=IOS)
   N_DEVC = N_DEVC + 1
   12 IF (IOS>0) THEN
...
REWIND(LU5)
COUNT_CTRL_LOOP: DO
   CALL CHECKREAD('CTRL',LU5,IOS)
   IF (IOS==1) EXIT COUNT_CTRL_LOOP
   READ(LU5,NML=CTRL,END=11,ERR=12,IOSTAT=IOS)
   N_CTRL = N_CTRL + 1
   12 IF (IOS>0) THEN
...
REWIND(LU5)
COUNT_PROF_LOOP: DO
   CALL CHECKREAD('PROF',LU5,IOS)
   IF (IOS==1) EXIT COUNT_PROF_LOOP
   READ(LU5,NML=PROF,END=11,ERR=12,IOSTAT=IOS)
   N_PROF = N_PROF + 1
   12 IF (IOS>0) THEN
