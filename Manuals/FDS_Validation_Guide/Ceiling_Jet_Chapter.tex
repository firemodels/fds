...
\section{Back-Layering of Smoke in Tunnels}

This section presents small and large-scale experiments focused on the phenomenon known as ``back-layering;'' that is, where jet fans are installed near the ceiling of a tunnel to blow smoke from a fire in one direction. Propagation of smoke upwind of the fire is called back-layering.
...
This section contains the results of the simulations of the Wu and Bakar experiments described in Sec.~\ref{Wu_Bakar_Tunnels_Description}. Five simulations are conducted. In each, the heat release rate is stepped up from its lowest reported value to its highest, each step lasting 20~s. For each of the eight heat release rates, the tunnel velocity is set to the reported critical velocity. Temperature readings along the ceiling indicate where the smoke layer temperature drops below 30~\unit{\degreeCelsius}, 10~\unit{\degreeCelsius} above ambient. This is taken as the extent of the back-layer. Using an empirical correlation for the normalized back-layer length, $L_{\rm b}^*$, developed by Li~et al.~\cite{Li:FSJ2010}
\be
   L_{\rm b}^* = \left\{ \begin{array}{ll} 18.5 \, \ln (0.81 \, (\dot{Q}^*)^{1/3}/V^*) & \dot{Q}^*\le 0.15 \\
                                      18.5 \, \ln (0.43 /V^*)            & \dot{Q}^*>   0.15 \end{array} \right.
\ee
where
\be
   L_{\rm b}^* = \frac{L_{\rm b}}{\bar{H}} \quad ; \quad \bar{H}=\frac{4A}{P} \quad ; \quad \dot{Q}^*=\frac{\dot{Q}}{\rho_\infty c_p T_\infty \sqrt{g \bar{H}^5} } \quad ; \quad V^* = \frac{V}{\sqrt{g \bar{H}}}
\ee
...
\be
   V^* = \left\{ \begin{array}{ll} 0.4 \, (\dot{Q}^*/0.2)^{1/3} & \dot{Q}^* \le 0.2 \\ 0.4 & \dot{Q}^* > 0.2 \end{array} \right.
\ee
...
The figures on the following pages summarize the results of 17 simulations of the longitudinal fan experiments (Sequences 15, 17, and 18). For each experiment, the measured heat release rate is shown. The model heat release rate is specified to match, but some deviations between the two are evident. Next, the measured and predicted volume flow through the tunnel is presented. The measured and predicted values are taken at Loop~214, just inside the north (uphill) portal. Next shown are the near-ceiling temperature and velocity at Loop~305, approximately 10~m uphill of the fire. ``Back-layering'' of smoke is assumed if the temperature at this location rises significantly above ambient. The jet fans were positioned uphill of the fire and usually blew in the downhill direction, except for Tests 23B, 24B, and 25B, where the fans were positioned downhill of the fire, but still blew in the downhill direction. The fan flow direction was reversed during some of the experiments. The fifth figure indicates where a near-ceiling temperature of 50~\unit{\degreeCelsius} is located, as interpolated from the measurements made 0.9~m below the ceiling. This position varies in time as the jet fans are activated. The fire location is designated with the letter ``F''. In most cases, the fans blew air downhill from the north portal (left) toward the south portal (right). Back-layering is indicated when the location of 50~\unit{\degreeCelsius} falls to the left of the dashed vertical line. Finally, the contour plots show comparisons of measured and predicted gas temperature in the vertical centerline plane of the tunnel. The temperature contours are drawn from temperature values obtained along 13 vertical arrays of thermocouples. The fire location is designated with the letter ``F''.

A summary plot of the longitudinal ventilation cases is found in Fig.~\ref{critical_velocity_plot}. The points on the plot indicate the bulk tunnel velocity and heat release rate at various points in time when the back-layering of smoke is or is not controlled, as indicated by the non-filled and filled symbols, respectively. Super-imposed on the points are two empirical correlations, one from Wu and Bakar~\cite{Wu:FSJ2000} and the other from the 2014 edition of NFPA~502~\cite{NFPA_502}. It is not expected that the experimental or simulation points should match the correlations perfectly. Rather the correlations have been added to provide context.

\begin{figure}[h!]
\begin{center}
\includegraphics[height=2.15in]{SCRIPT_FIGURES/Memorial_Tunnel/Critical_Velocity}
\end{center}
\caption[Critical velocity for the Memorial Tunnel experiments]{Critical velocity for the Memorial Tunnel experiments.}
\label{critical_velocity_plot}
\end{figure}
...
\subsubsection{Transverse Ventilation}
...
The plots to the right of each figure indicate where a near-ceiling temperature of 50~\unit{\degreeCelsius} is located, as interpolated from the measurements made just below the ceiling. The fire location is designated with the vertical dashed line. The ventilation flow is from left to right; thus, back-layering is indicated when the location of the 50~\unit{\degreeCelsius} contour falls to the left of the dashed vertical line.
