...
TYPE(BOUNDARY_ONE_D_TYPE_TYPE), POINTER :: ONE_D,ONE_D_BACK
               ...
               CASE (NET_FLUX_BC) METHOD_OF_HEAT_TRANSFER
                  ...
                  ELSE
                     ...
                     ADLOOP: DO
                        ...
                        IF (ABS(QNET) > 0._EB .AND. ABS(DTMP) <TWO_EPSILON_EB) DTMP=1._EB
                        ONE_D%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,WALL_INDEX=IW)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. BOUNDARY_TYPE/=OPEN_BOUNDARY) RHOP(BC%II,BC%JJ,BC%KK) = 2._EB*ONE_D%RHO_F - ONE_D%RHO_G
ENDDO WALL_CELL_LOOP
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
   ...
   END SELECT
   HTCF = MIN(HTC_LIMIT , HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,PARTICLE_INDEX=PARTICLE_INDEX))
ENDIF
Q_CON_F = HTCF*DTMP
...
IF (PRESENT(PARTICLE_INDEX)) THEN
   LPX => LAGRANGIAN_PARTICLE(PARTICLE_INDEX)
