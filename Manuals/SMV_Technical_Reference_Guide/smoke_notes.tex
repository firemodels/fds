...
The radiation transport equation is used to calculate radiance due to one or more light sources within a region possibly containing a participating media such as smoke~\cite{Siegel:2001}. The change in radiance along a ray with direction $\omega$ at any one instant may be expressed using
...
\end{eqnarray}
The basic computational strategy then is to compute a color using (\ref{eq:color}) and an opacity using (\ref{eq:alpha2}) for each pixel in the 2D projected image.  Equations (\ref{eq:alpha2}) and ({\ref{eq:color}) are equivalent to the recursions presented in Ref.~\cite[Chapter 39]{gpugems} for performing volume rendering.
