...
[T,Y]=ode113(@EDM_spec_propane,tspan,y0, options);
...
filename1 = '../../Verification/Species/EDM_Propane_1Step_Species.csv';
...
filename1 = '../../Verification/Species/EDM_Propane_1Step_TempPres.csv';
fid = fopen(filename1,'wt');
fprintf(fid,'%s, %s, %s\n',header1{:});
for j=1:length(tss)
    fprintf(fid,'%f, %f, %f\n',tss(j),Tf(j),dP(j));
