...
DO NOM=1,NMESHES
   ...
   IF (CODE==6 .AND. EXCHANGE_EVACUATION .AND. MYID==EVAC_PROCESS .AND. .NOT.EVACUATION_ONLY(NOM)) THEN
      ...
      IF (N_TRACKED_SPECIES>0) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M4%ZZ(0,0,0,1),IWW*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
         ...
         DO KK=KMIN,KMAX
            DO JJ=JMIN,JMAX
               DO II=IMIN,IMAX
                  ...
                  IF (N_TRACKED_SPECIES>0) M3%REAL_SEND_PKG1(LL+4:LL+3+N_TRACKED_SPECIES) = M%ZZS(II,JJ,KK,1:N_TRACKED_SPECIES)
         ...
         DO KK=KMIN,KMAX
            DO JJ=JMIN,JMAX
               DO II=IMIN,IMAX
                  ...
                  IF (N_TRACKED_SPECIES>0) M3%REAL_SEND_PKG3(LL+4:LL+3+N_TRACKED_SPECIES) = M%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
                  LL = LL+3+N_TRACKED_SPECIES
               ENDDO
            ENDDO
         ENDDO
...
DO NM=1,NMESHES
   ...
   IF (USE_MPI .AND. CODE==6 .AND. EXCHANGE_EVACUATION .AND. MYID/=EVAC_PROCESS .AND. PROCESS(NM)==MYID .AND. &
      ...
      IF (N_TRACKED_SPECIES>0) THEN
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M%ZZ(0,0,0,1),IWW*N_TRACKED_SPECIES,MPI_DOUBLE_PRECISION,EVAC_PROCESS,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      ...
      DO KK=KMIN,KMAX
         DO JJ=JMIN,JMAX
            DO II=IMIN,IMAX
               ...
               IF (N_TRACKED_SPECIES>0) M2%ZZS(II,JJ,KK,1:N_TRACKED_SPECIES)= M2%REAL_RECV_PKG1(LL+4:LL+3+N_TRACKED_SPECIES)
               LL = LL+3+N_TRACKED_SPECIES
            ENDDO
         ENDDO
      ENDDO
      ...
      DO KK=KMIN,KMAX
         DO JJ=JMIN,JMAX
            DO II=IMIN,IMAX
               ...
               IF (N_TRACKED_SPECIES>0) M2%ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)= M2%REAL_RECV_PKG3(LL+4:LL+3+N_TRACKED_SPECIES)
               LL = LL+3+N_TRACKED_SPECIES
            ENDDO
         ENDDO
      ENDDO
...
USE HVAC_ROUTINES, ONLY: NODE_CP,NODE_P,NODE_TMP,NODE_RHO,NODE_RSUM,NODE_ZZ,LEAK_CP,LEAK_TMP,LEAK_RHO,LEAK_RSUM,LEAK_ZZ
...
REAL_BUFFER_7 = NODE_ZZ
CALL MPI_SCATTER(REAL_BUFFER_7(1,1,1),SIZE(NODE_ZZ),MPI_DOUBLE_PRECISION, &
