...
\index{SURF!SHRINK} If all the material components of the surface are
reacting, and the pyrolysis reactions have no solid residue, the
thickness of the surface is going to shrink when the surface
reacts. The shrinking behavior can be prevented in the numerical model
by setting {\ct SHRINK = .FALSE.} on the {\ct SURF} line.  However,
setting {\ct SHRINK = .FALSE.} may introduce numerical problems in the
wall solver. When all the material of a shrinking surface is consumed
but the {\ct BURN\_AWAY} is not prescribed, the surface
temperature is set to {\ct TMP\_BACK}, convective heat flux to zero and
burning rate to zero. 
