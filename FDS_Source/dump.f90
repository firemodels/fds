   ...
   ! write out slice file info to the .smv file
      ...
      IF (SL%GEOMETRY)THEN
         IF (.NOT.SL%TERRAIN_SLICE.AND..NOT.SL%CELL_CENTERED) THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,I6,A)') 'SLCGF',NM,TRIM(SLICELABEL)
         ENDIF
         IF (SL%CELL_CENTERED) THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,I6,A)') 'SLCGC',NM,TRIM(SLICELABEL)
         ENDIF
      ELSE
         IF (.NOT.SL%TERRAIN_SLICE.AND..NOT.SL%CELL_CENTERED) THEN
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   IF (.NOT.PLOT3D) THEN
      ...
      IF (.NOT. SL%GEOMETRY) THEN ! write out slice file using original slice file format
         ...
         CLOSE(LU_SLCF(IQ,NM))
      !ELSE
      ! write out slice file using new geometry file format 
      ENDIF
   ENDIF
