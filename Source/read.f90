...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (MINIMUM_DIAMETER<0._EB)     &
      LPC%MINNIMUM_DIAMETER             = 0.005_EB*DIAMETER
   IF (KILL_DIAMETER<0._EB) THEN
      IF (MONODISPERSE) THEN            ! Kill if mass <= 0.5 % of MINIMUM_DIAMETER (or DIAMETER if MONODISPERSE)
         LPC%KILL_RADIUS                = 0.5_EB*(0.005_EB*(0.5_EB*DIAMETER)**3)**ONTH
      ELSE
         LPC%KILL_RADIUS                = 0.5_EB*(0.005_EB*(0.5_EB*MINIMUM_DIAMETER)**3)**ONTH
      ENDIF
   ELSE
      IF (.NOT. MONODISPERSE .AND. KILL_DIAMETER >= MINIMUM_DIAMETER) THEN
