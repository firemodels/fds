...
in the presence of complex ventilation systems and the FDS implmentation of the MELCOR solver is largely based on the implementation found in FSSIM.  The coupling of the HVAC solver to the remainder of the FDS computation is in part based upon approaches taken in GOTHIC~\cite{GOTHIC}, another containment analysis code that couples CFD-like features for large containment volumens with a network model for piping and ventilation.
...
\be \frac{1}{\sqrt{f}} \; = \; -2 \log_{10} \left(\frac{\frac{\epsilon}{D}}{3.7}\; - \; \frac{4.518}{\mathrm{Re}_D} \log_{10} \left(
\frac{6.9}{\mathrm{Re}_D}+\left( \frac{\frac{\epsilon}{D}}{3.7} \right)^{1.11} \right) \right) \ee
