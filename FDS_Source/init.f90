...
NOT_EVAC_IF_1: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   ENDDO
   M%WALL%ONE_D%QRADIN = SIGMA*TMPA4
   M%WALL%ONE_D%QRADOUT = SIGMA*TMPA4
   M%WALL%ONE_D%QCONF = 0._EB
   M%WALL%ONE_D%HEAT_TRANS_COEF = 0._EB
...
NOT_EVAC_IF_2: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   DO IW=1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%ONE_D%MASSFLUX(0:N_TRACKED_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%ONE_D%MASSFLUX',IZERO)  
      M%WALL(IW)%ONE_D%MASSFLUX = 0._EB
      ALLOCATE(M%WALL(IW)%ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%ONE_D%MASSFLUX_ACTUAL',IZERO)  
      M%WALL(IW)%ONE_D%MASSFLUX_ACTUAL = 0._EB
   ENDDO
   M%WALL%NPPCW = 1
   M%WALL%BACK_INDEX = 0
   M%WALL%AW = 0._EB
...
IF (ACCUMULATE_WATER .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   DO IW = 1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%A_LP_MPUA(N_LP_ARRAY_INDICES),STAT=IZERO)
...
IF (.NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO IW = 1,N_TOTAL_WALL_CELLS
      ALLOCATE(M%WALL(IW)%LP_MPUA(N_LP_ARRAY_INDICES),STAT=IZERO)
      CALL ChkMemErr('INIT','WALL(IW)%LP_MPUA',IZERO) 
      M%WALL(IW)%LP_MPUA = 0._EB  
      ALLOCATE(M%WALL(IW)%LP_CPUA(N_LP_ARRAY_INDICES),STAT=IZERO)
...
WC%ONE_D%EMISSIVITY = SF%EMISSIVITY
