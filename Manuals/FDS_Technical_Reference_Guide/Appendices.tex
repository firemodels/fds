...
where $\kappa_{\lambda} \mbox{ }(m^{-1})$ is the absorption coefficient for wavelength $\lambda \mbox{ } (m)$.  Absorption coefficient can be determined from experiment by measuring the transmittance $\tau_{S}=\frac{I_{S,\lambda}}{I_{0,\lambda}}$ for a path length $S$:
...
Where data on absorption coefficients of liquids exists in the open literature, such as Coblentz Society data found on the NIST Chemistry WebBook \cite{Coblentz:1}, it usually  only contains data for wavelengths from approximately 2.5 mm upwards. A large part of the total energy in emission spectrum of flames may easily be contained in wavelengths shorter than 2.5 mm thus rendering these data useless for our purposes.

However absorption spectra that begin from 1 mm exists for a few liquids of which toluene (\cite{Bertie:JMS2005}, \cite{Bertie:AS1994b}, \cite{Bertie:AS1994a}), methanol \cite{Bertie:AS1993a}, benzene \cite{Bertie:AS1993b} and water \cite{Bertie:AS1996} are of interest for our current purposes. Furthermore, \cite{Suo-Anttila:PCT2009} included in their work on pool fires spectrally resolved transmission spectra of ethanol, heptane, JP-8 and an ethanol-toluene blend and calculations. Complex refractive index spectra for few diesel fuels were reported in \cite{Sazhin:IJHMT2004b}. Different diesel fuels have slightly different absorption spectra, due to differing additives. However the data reported in \cite{Sazhin:IJHMT2004b} can perhaps be used to obtain a order of magnitude estimate for the absorption coefficient of diesel fuel.
...
\begin{equation}
\label{eqn_totintens}
     \ \bar{I}_S = \int_{0}^{\infty} I_{\eta,S} d\eta \mbox{.}
\end{equation}
...
\begin{tabular}{l r}
\hline\hline
Liquid & Wavenumber Range $(cm^{-1})$ \\ [0.5ex]
\hline
JP-8 \cite{Suo-Anttila:PCT2009}                  &  - \\
Ethanol-Toluene blend \cite{Suo-Anttila:PCT2009} &  - \\
Ethanol \cite{Suo-Anttila:PCT2009}               &  - \\
Toluene \cite{Suo-Anttila:PCT2009}               &  - \\
Toluene \cite{Bertie:AS1994a}                    &  436-6500 \\
Methanol \cite{Bertie:AS1993a}                   & 2-8000 \\
Water \cite{Bertie:AS1996}                       & xxxx-xxxx \\
Benzene \cite{Bertie:AS1993b}                    & xxxx-xxxx \\ 
\hline
\end{tabular}
\label{tbl_wavnum}
\end{table}
...
\begin{table}[ht]
\caption{Effective absorption coefficients for selected liquids. Blackbody temperature 1450 K and path length 3 mm. Data from Suo-Anttila et al. 2009 \cite{Suo-Anttila:PCT2009}.}
\centering
\begin{tabular}{l r}
\hline\hline
Liquid & Effective Absorption Coefficient,$\kappa$ \\ [0.5ex]
\hline
JP-8                                 & 301.4\\
Ethanol-Toluene blend                & 680.1\\
Ethanol                              & 1534.3 \\
Toluene \cite{Suo-Anttila:PCT2009}   & 187.5 \\
Toluene \cite{Bertie:AS1994a}        & 160.8 \\
Methanol                             & 52 \\
Water                                & 1578 \\
Benzene                              & 123 \\ 
\hline
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=5.0in]{FIGURES/KAPPA_VS_TEMP.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=5.0in]{FIGURES/KAPPA_VS_PATH_LENGTH.pdf}
