...
A lumped species represents a mixture of gas species that transport together ({\em i.e.}, the lumped species has a single set of transport properties) and react together, and from the point of
...
\be \dod{ }{t}(\rho h_s) + \nabla\!\cdot (\rho h_s \bu) = \frac{\mbox{D}\bar{p}}{\mbox{D}t}  + \dq''' - \dq_b'''
...
As mentioned above, we do not solve Eq.~(\ref{energy}) explicitly. Instead, we factor out the divergence as follows:
\be
\label{eqn_simplediv1}
\Div\mathbf{u} = \frac{1}{\rho h_s} \left[ \frac{\mbox{D}}{\mbox{D} t}(\bar{p}-\rho h_s) + \dot{q}''' - \dq_b''' - \Div \dot{\mathbf{q}}'' \right]
\ee
...
\be \nabla^2 {\cal H} = - \left[ \dod{ }{t} (\nabla\!\cdot \bu) + \nabla\!\cdot \bF \right]   \label{simplephi2} \ee
...
\be \kappa \; I_b = \left\{ \begin{array}{cll}
    \kappa \, \sigma \, T^4/\pi                                           & \hbox{Outside flame zone}, & \dot{q}'''=0  \\ [0.15in]
...
\be \dt \; \max\left( \frac{|u|}{\dx}, \frac{|v|}{\dy}, \frac{|w|}{\dz} \right) < 1 \quad \ee
