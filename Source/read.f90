...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IF (AGL_SLICE>=0._EB .AND. CELL_CENTERED) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (QUANTITY=='FIRE ARRIVAL TIME' .AND. AGL_SLICE<0) THEN
         WRITE(MESSAGE,'(A)') 'ERROR(962): FIRE ARRIVAL TIME only permitted with AGL_SLICE.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (QUANTITY=='FIRE ARRIVAL TIME' .AND. FIRE_ARRIVAL_SLICE) THEN
         WRITE(MESSAGE,'(A)') 'ERROR(963): Only one FIRE ARRIVAL TIME slice allowed.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (PBX>-1.E5_EB .OR. PBY>-1.E5_EB .OR. PBZ>-1.E5_EB) THEN
         XB(1) = XS
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         IF (ITER == 1 .AND. AGL_SLICE > -1._EB) THEN
            ...
            IF (QUANTITY=='TIME OF ARRIVAL') FIRE_ARRIVAL_SLICE = .TRUE.
         ENDIF
