...
IF(RADIATION .AND. .NOT. ALLOW_RANDOM_RADIATION_ROTATION) THEN
   ...
   CALL ALLOCATE_RADIATION_RECV_PKG()
   CALL ALLOCATE_RADIATION_SEND_PKG()
ENDIF
...
MAIN_LOOP: DO
   ...
   DO ITER=1,RADIATION_ITERATIONS
      IF (RADIATION .AND. ALLOW_RANDOM_RADIATION_ROTATION .AND. ITER==1) THEN
         START_NEW_ANGLE_CYCLE = MOD(MAX(MESHES(LOWER_MESH_INDEX)%RAD_CALL_COUNTER-1,0), & 
                                 ANGLE_INCREMENT*TIME_STEP_INCREMENT)==0  ! -1 because already called once duing initialization
         IF (START_NEW_ANGLE_CYCLE) THEN
            CALL CALCULATE_DIRECTION_COEFFICIENTS()
            CALL INTERPOLATE_IL()
            CALL ALLOCATE_RADIATION_RECV_PKG()
...
USE MEMORY_FUNCTIONS, ONLY: PACK_PARTICLE,NULLIFY_PARTICLE,ALLOCATE_STORAGE
...
USE MEMORY_FUNCTIONS, ONLY: REALLOCATE_P_ZONE,REALLOCATE_CELL
...
USE MEMORY_FUNCTIONS, ONLY: PACK_PARTICLE,PACK_WALL,PACK_THIN_WALL,REALLOCATE_STORAGE_ARRAYS,ALLOCATE_STORAGE

REAL(EB) :: TNOW,XI,YJ,ZK,T_NOW_SUB
INTEGER, INTENT(IN) :: CODE
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF ((CODE==1.OR.CODE==4) .AND. M3%NIC_S>0) THEN
         ...
         IF (RNODE/=SNODE) THEN
            ...
            T_USED(12) = T_USED(12) + CURRENT_TIME() - T_NOW_SUB
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
            IF (CODE==1) THEN
...
SUBROUTINE ALLOCATE_RADIATION_RECV_PKG()
...
T_USED(11)=T_USED(11) + CURRENT_TIME() - TNOW
END SUBROUTINE ALLOCATE_RADIATION_RECV_PKG


! Determine the maximum number of radiation angles that are to be sent

SUBROUTINE ALLOCATE_RADIATION_SEND_PKG()
...
T_USED(11)=T_USED(11) + CURRENT_TIME() - TNOW
END SUBROUTINE ALLOCATE_RADIATION_SEND_PKG
...
ELSE
   ...
   WRITE(LU_CPU,'(A)') 'Rank,MAIN,DIVG,MASS,VELO,PRES,WALL,DUMP,PART,RADI,FIRE,COMM,PACK,HVAC,GEOM,VEGE,CHEM,Total T_USED (s)'
   DO N=0,N_MPI_PROCESSES-1
