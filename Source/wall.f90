...
REAL(EB), INTENT(IN) :: T
REAL(EB) :: DT_BC,DTMP,DT_BC_HT3D,TSI,UBAR,VBAR,WBAR,RAMP_FACTOR,RHO_G,ZZ_G(1:N_TRACKED_SPECIES),RSUM_G,MU_G
            ...
            METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
               ...
               CASE (NET_FLUX_BC) METHOD_OF_HEAT_TRANSFER
                  ...
                  ELSE
                     ...
                     IF (ASSUMED_GAS_TEMPERATURE > 0._EB) ONE_D%TMP_G = TMPA + &
                        EVALUATE_RAMP(T-T_BEGIN,I_RAMP_AGT)*(ASSUMED_GAS_TEMPERATURE-TMPA)
                     TMP_F = ONE_D%TMP_F
                     TMP_OTHER = TMP_F
                     DTMP = ONE_D%TMP_G - TMP_F
                     IF (ABS(ONE_D%T_IGN-T_BEGIN)<= SPACING(ONE_D%T_IGN) .AND. SF%RAMP_INDEX(TIME_HEAT)>=1) THEN
                        TSI = T
                     ELSE
                        TSI = T - ONE_D%T_IGN
               ...
               CASE (THERMALLY_THICK_HT3D) ! thermally thick, continuous heat flux
                  ...
                  JJG = BC%JJG
                  KKG = BC%KKG
                  ONE_D%TMP_G = TMP_NEW(IIG,JJG,KKG)
                  ! Special case where the gas temperature is fixed by the user
                  IF (ASSUMED_GAS_TEMPERATURE > 0._EB) ONE_D%TMP_G = TMPA + &
                     EVALUATE_RAMP(T-T_BEGIN,I_RAMP_AGT)*(ASSUMED_GAS_TEMPERATURE-TMPA)
                  ...
                  TMP_F_LOOP: DO ADCOUNT=1,3
                     ...
                     IF (RADIATION) THEN
                        ...
                        TMP_F = ( ONE_D%Q_RAD_IN + TH_EST4 + HTC*ONE_D%TMP_G + 2._EB*K_S*RDN*TMP_S ) / &
                                (                   FO_EST3 + HTC       + 2._EB*K_S*RDN       )
                     ELSE
                        TMP_F = ( HTC*ONE_D%TMP_G + 2._EB*K_S*RDN*TMP_S ) / &
                                ( HTC       + 2._EB*K_S*RDN       )
                     ENDIF
                     IF (OB%MATL_INDEX>0) THEN
                  ...
                  ENDDO TMP_F_LOOP
                  ONE_D%TMP_F = TMP_F
                  Q_CON_F_SUB(IW) = HTC*(ONE_D%TMP_G-TMP_F)
