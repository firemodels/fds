...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~xxxx, 52 pages (May 2009) \\
CODEN: NSPUE2 }
...
The tests in this chapter verify whether visualization types such as surface contours (boundary files), isosurfaces, particles, slice files {\em etc.} are working as intended.
These verifications use
the FDS input files {\tt plume5c.fds}\ (see Appendix \ref{FDSplume5c}) and {\tt fireline.fds} (see Appendix \ref{FDSfireline}) to generate the simulation data and
the Smokeview script files {\tt plume5c.ssf}\ (see Appendix \ref{SSFplume5c}) and {\tt fireline.ssf}\ (see Appendix \ref{SSFfireline}) to generate the verification figures. The {\tt plume5c.fds}\ test case is a simple plume fire with two blockages to make the smoke flow more complex.  A portion of the gas phase and the upper blockage are both initialized to 600~\degC (rather than 20~\degC) in order to test the conversion of a known temperature data to the correct color (as shown in the colorbar).  The {\tt fireline.ssf}\ test case is a terrain test case.  The center of the case has a {\em hill}.  The fire line data displayed conforms to this hill.
...
&cell averaged  data&cell centered data\\
 &&&\raisebox{0.5in}[0pt]{\includegraphics[height=7.5in]{figures/colorbar_20_620}}\\
  \end{tabular}
\end{center}
 \caption[Boundary file test.]{Boundary file test. The upper obstacle is initialized to 600.0~\degC.}
\label{figboundtest}%
\end{figure}
...
\begin{tabular}{rcc}
 ...
 &point view&outline view
  \end{tabular}
\end{center}
 \caption[Isosurface file test 1.]{Isosurface file test 1. A portion of the interior gas temperature is initialized to 600.0~\degC.}
\label{figisotest}%
\end{figure}
...
\begin{tabular}{cccp{1.0in}}
 \includegraphics[height=\figheightD]{scriptfigures/plume5c_slice_00}&
 \includegraphics[height=\figheightD]{scriptfigures/plume5c_slice_chop_00}&
 \includegraphics[height=\figheightD]{scriptfigures/plume5c_slice_cell_00}\\
...
\begin{tabular}{rccl}
 0.0 s&
 \includegraphics[height=\figheightD]{scriptfigures/plume5c_vslice_00}&
 \includegraphics[height=\figheightD]{scriptfigures/plume5c_vslicechop_00}\\
 10.0 s&
...
Proper smoke visualization requires that smoke be both computed and drawn correctly.
The FDS verification guide~\cite{FDS_Verification_Guide_5} addresses the question of correct
computation in terms of soot/smoke production, transport {\em etc.}  In this chapter it is presumed that this component of the problem is correct and considers the question is smoke drawn correctly.  More precisely, given a known density and distribution of soot does the 3D smoke drawn by Smokeview match how {\em theory}\ suggests it should look.  For now, we are mainly interested in how smoke obscures objects in the background.  Visualization effects due to light scattering are not modeled except to consider the smoke albedo when drawing its color.

The smoke drawing verification problem can be broken down into two main steps.  The first step is to verify that Smokeview can record or denote the correct grey level of smoke that is drawn.
...
This device behaves like other FDS devices but has the additional property that when displayed by Smokeview, it displays the grey level at the sensor location as viewed by the observer.  The grey level is simply Smokeview's computation of smoke level between the sensor location and the eye.  These computations are performed by the video card using OpenGL, the graphical library Smokeview uses to display FDS scenarios.  The user places a device of type {\tt smoke\_sensor}\ at a particular $(x, y, z)$ location.
...
Figure \ref{figgraysquare} shows a reference colorbar consisting of shade of grey.  The shades range from black to white.
The numerical values range from 0 (black) to 255 (white).  Of interest is the difficulty one has in distinguishing nearly equal shades.  This Figure gives a benchmark on ones ability to distinguish nearly equal shades of smoke.
\begin{figure}[\figoptions]
\begin{center}
 \centering
\begin{tabular}{c}
...
Figure \ref{figsmoketestgeom} shows a side view of the numerical smoke box used to test the smoke visualization.  In addition to the walls defining the box, this box consists of  six parallel walls spaced 1.0~m apart.  The widths are increasing from one wall to the next so that when looking at the box from the front a different distance or path length occurs between the observer and the portion of the wall that is visible.  The spacing between the walls, the distance between the walls and the observer and the initial soot densities is chosen so that the computed smoke obscurations work out to nice values.
...
Figure \ref{figsmoketest2} shows quantitative test of the smoke opacity calculation performed in Smokeview. It also shows at the bottom, the shades of grey that should be displayed.

Smoke grey level or opacity, $\alpha$ is computed using
\begin{eqnarray}
\label{eq:alpha}
...
\end{eqnarray}
Path lengths (smoke sensor locations) are chosen to obtain grey levels of $\alpha$ of 192, 128, 64, 32, 16 and 8.  These path lengths may be found by substituting these grey levels into equation (\ref{eq:deltaX}) to obtain $\Delta x$ values of 0.41~m, 1.0~m, 2.0~m, 3.0~m, 4.0~m and 5.0~m respectively.  The walls in {\tt smoketest2.fds}\ are placed at these distances from the front of the simulation domain.
