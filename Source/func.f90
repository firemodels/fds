...
M%N_WALL_CELLS_DIM     = N_SLOTS
M%N_BOUNDARY_COORD_DIM = N_SLOTS
M%N_BOUNDARY_PROPS_DIM = N_SLOTS
M%N_BOUNDARY_RADIA_DIM = N_SLOTS
M%N_BOUNDARY_ONE_D_DIM = N_SLOTS
M%N_BOUNDARY_THR_D_DIM = 10

ALLOCATE(M%WALL(0:M%N_WALL_CELLS_DIM))
...
ALLOCATE(M%BOUNDARY_THR_D(1:M%N_BOUNDARY_THR_D_DIM))
...
ALLOCATE(M%BOUNDARY_THR_D_OCCUPANCY(1:M%N_BOUNDARY_THR_D_DIM))  ; M%BOUNDARY_THR_D_OCCUPANCY = 0
...
USE GLOBAL_CONSTANTS, ONLY: THERMALLY_THICK,TMPM,SIGMA,TMPA,TMPA4,RHOA,N_TRACKED_SPECIES,N_SURFACE_DENSITY_SPECIES
...
ENDDO
DO NN=1,SF%N_SPEC
   ONE_D%SPEC_COMP(NN)%RHO_ZZ(0:ONE_D%N_CELLS_INI+1) = SF%PHIRHOZ_0(0:ONE_D%N_CELLS_INI+1,NN)
ENDDO
...
IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
   IF (SF%RAMP_T_I_INDEX > 0) THEN
      ...
      ENDDO
      ONE_D%TMP(ONE_D%N_CELLS_INI+1) = ONE_D%TMP(ONE_D%N_CELLS_INI)
      ONE_D%TMP_F = ONE_D%TMP(0)
      ONE_D%TMP_B = ONE_D%TMP(ONE_D%N_CELLS_INI+1)
   ELSE
      ONE_D%TMP(0) = SF%TMP_INNER(1)
      ONE_D%TMP(ONE_D%N_CELLS_INI+1) = SF%TMP_INNER(SF%N_LAYERS)
      NN=1
      DO N=1,SF%N_LAYERS
         ONE_D%TMP(NN:NN+ONE_D%N_LAYER_CELLS(N)-1) = SF%TMP_INNER(N)
         NN = NN + ONE_D%N_LAYER_CELLS(N)
      ENDDO
   ENDIF
ELSE
   ONE_D%TMP(0:ONE_D%N_CELLS_INI+1) = SF%TMP_INNER(1)
ENDIF
...
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER :: IC,RC,LC,BC_INDEX,OD_INDEX,BP_INDEX,BR_INDEX,TD_INDEX
...
! If we are reading (unpacking) a stored wall cell, set up the appropriate data structures
...
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),WC%THIN(1),UNPACK_IT)
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),WC%THIN(2),UNPACK_IT)
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),WC%THIN(3),UNPACK_IT)
...
!IF (SF%INCLUDE_BOUNDARY_THR_D_TYPE) CALL PACK_BOUNDARY_THR_D(NM2,IC,RC,OS,SF,STORAGE_INDEX,OD_INDEX,TD_INDEX,UNPACK_IT,COUNT_ONLY)
...
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%DX,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%DY,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%DZ,UNPACK_IT)
...
DO NN=1,SF%N_SPEC
   I1 = I2+1 ; I2 = I1 + (SF%N_CELLS_MAX+2) - 1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%SPEC_COMP(NN)%RHO_ZZ(0:I2-I1) , UNPACK_IT)
ENDDO
...
!> \brief Pack BOUNDARY_THR_D into REAL and INTEGER 1-D storage arrays for a WALL
!> \param NM Mesh index
!> \param IC Integer counter
!> \param RC Real counter
!> \param OS STORAGE_ARRAY name
...
!> \param STORAGE_INDEX Column in the STORAGE_ARRAY to hold the data
!> \param OD_INDEX Index of the BOUNDARY_ONE_D array
!> \param TD_INDEX Index of the BOUNDARY_THR_D array
!> \param UNPACK_IT Logical indicating whether the data is to be packed into the 1-D array or unpacked from it
!> \param Optional flag signifying that only a variable count is to be done; no packing

SUBROUTINE PACK_BOUNDARY_THR_D(NM,IC,RC,OS,SF,STORAGE_INDEX,OD_INDEX,TD_INDEX,UNPACK_IT,COUNT_ONLY)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,OD_INDEX,TD_INDEX
INTEGER, INTENT(INOUT) :: IC,RC
LOGICAL, INTENT(IN) :: UNPACK_IT,COUNT_ONLY
INTEGER :: I1,I2,I,NWP
TYPE(BOUNDARY_ONE_D_TYPE), POINTER :: ONE_D
...
IF (.NOT.ALLOCATED(MESHES(NM)%BOUNDARY_THR_D(TD_INDEX)%NODE)) RETURN

ONE_D => MESHES(NM)%BOUNDARY_ONE_D(OD_INDEX)
NWP = SUM(ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS))

IF (.NOT.COUNT_ONLY .AND. UNPACK_IT) THEN
!  ALLOCATE(MESHES(NM)%BOUNDARY_THR_D(TD_INDEX)%NODE(NWP))
   THR_D => MESHES(NM)%BOUNDARY_THR_D(TD_INDEX)
ELSE
   THR_D => MESHES(NM)%BOUNDARY_THR_D(TD_INDEX)
ENDIF
...
DO I=1,NWP
   I1 = I2+1 ; I2 = I1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(I1,STORAGE_INDEX) , THR_D%NODE(I)%ALTERNATE_WALL_VALUES , UNPACK_IT)
   IF (.NOT.COUNT_ONLY .AND. UNPACK_IT) THEN
      ALLOCATE(THR_D%NODE(I)%ALTERNATE_WALL_MESH(THR_D%NODE(I)%ALTERNATE_WALL_VALUES))
      ALLOCATE(THR_D%NODE(I)%ALTERNATE_WALL_INDEX(THR_D%NODE(I)%ALTERNATE_WALL_VALUES))
      ALLOCATE(THR_D%NODE(I)%ALTERNATE_WALL_NODE(THR_D%NODE(I)%ALTERNATE_WALL_VALUES))
      ALLOCATE(THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(THR_D%NODE(I)%ALTERNATE_WALL_VALUES))
   ENDIF
   I1 = I2+1 ; I2 = I1 + THR_D%NODE(I)%ALTERNATE_WALL_VALUES - 1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(I1:I2,STORAGE_INDEX) , THR_D%NODE(I)%ALTERNATE_WALL_INDEX(1:I2-I1+1) , UNPACK_IT)
   I1 = I2+1 ; I2 = I1 + THR_D%NODE(I)%ALTERNATE_WALL_VALUES - 1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(I1:I2,STORAGE_INDEX) , THR_D%NODE(I)%ALTERNATE_WALL_NODE(1:I2-I1+1) , UNPACK_IT)
   I1 = I2+1 ; I2 = I1 + THR_D%NODE(I)%ALTERNATE_WALL_VALUES - 1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(I1:I2,STORAGE_INDEX) , THR_D%NODE(I)%ALTERNATE_WALL_MESH(1:I2-I1+1) , UNPACK_IT)
ENDDO
...
DO I=1,NWP
   I1 = I2+1 ; I2 = I1 + THR_D%NODE(I)%ALTERNATE_WALL_VALUES - 1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(1:I2-I1+1) , UNPACK_IT)
ENDDO

RC = I2

END SUBROUTINE PACK_BOUNDARY_THR_D
...
IF (CELL_SIZE>0._EB) THEN
   N_CELLS = NINT(LAYER_THICKNESS/CELL_SIZE)
   DDSUM   = REAL(N_CELLS)
