...
\hat{Y}_\alpha^c &= \hat{Y}_\alpha^k + \Delta \hat{Y}_\alpha \beta \label{eq:bc_correct}
\end{align}
After computing the predicted values, a boundedness corrector ($\beta$) is applied to the change in species vector, $\Delta \hat{Y}_{\alpha}$, to determine the corrected values. Here $\beta$ is defined as
...
\end{equation}
{\bf Note:} For infinitely fast chemistry, $\Delta \hat{Y}_\alpha$ is still computed following Eq.~(\ref{eq:change_alpha}), but instead of finding the stoichiometric change in fuel based on the limiting reactant, the change in fuel is simply the amount of the fuel present, $\Delta \hat{Y}_{\rm F} = -\hat{Y}_{\rm F}$.

\subsubsection{Example: Infinitely Fast Chemistry, Oxygen-limited, Methane Combustion}
...
\end{equation}
where the initial mass fractions of species are 30\% methane, 30\% oxygen, and 40\% nitrogen by mass. Following the predictor-corrector scheme, the change in methane concentration, $\Delta \hat{Y}_{\rm F}$, is $-$0.3 and the corresponding change is oxygen is
\begin{equation}\label{eq:bc_o2_single}
\Delta \hat{Y}_{\rm O_2} = -0.3 \left( \frac{(-2)(32)}{(-1)(16)} \right) = -1.2
\end{equation}
The updated reaction concentrations are $\hat{Y}_{\rm F}^p$ = 0 and $\hat{Y}_{\rm O_2}^p$ = $-$0.9. The boundedness correction factor is then calculated:
\begin{equation}\label{eq:calc_beta_single}
\beta = \frac{0.3}{0.9 + 0.3} = 0.25
\end{equation}
Multiplying $\beta$ by the change in species results in an updated $\Delta \hat{Y}_{\rm F}$ of $-$0.075 and $\Delta \hat{Y}_{\rm O_2}$ of $-$0.3. The change in fuel is the same result as solving Eq.~(\ref{eq:stoich_fuel_single}) directly
...
\end{align}
The initial composition of a premixed reactor is 30\% methane, 40\% propane, and 30\% oxygen by mass. Based on the stoichiometry of the reactions, Eqs.~(\ref{eq:two_fuel_ox_limit1}) and (\ref{eq:two_fuel_ox_limit2}), and the composition of the reactor, the reactor is oxygen-limited. This means that the amount of fuel present requires more than the available oxygen to completely combust. The first step is to compute the change in fuel for each reaction
...
\Delta \hat{Y}_{\rm O_2} &= (-2.65)(0.113) = -0.3 \label{eq:del_ox_c}
\end{align}
and the corrected species concentrations of
\begin{align}\label{eq:mass_final}
\hat{Y}_{\rm F,1}^c &= 0.3 -0.0339 = 0.2661 \\
\hat{Y}_{\rm F,2}^c &= 0.4 -0.0452 = 0.3548 \\
\hat{Y}_{\rm O_2}^c &= 0.3 - 0.3 = 0
\end{align}
Alternatively, this problem could be solved by combining the two reactions, Eqs.~(\ref{eq:two_fuel_ox_limit1}) and (\ref{eq:two_fuel_ox_limit2}), into one lumped reaction. The lumped fuel reaction is determined from the initial volume fractions of each component fuel (0.673 and 0.327 for methane and propane, respectively) in the lumped fuel.
\begin{equation}\label{eq:lumped_reac}
\mathrm{C_{1.65}H_{5.31} + 2.98 \, O_2} \rightarrow  \mathrm{1.65 \, CO_2 + 2.66 \, H_2O}
\end{equation}
The molecular weight of this lumped fuel is 25.21 g/mol. One a mass basis, the reactor is 70 \% fuel, with with 43 \% of that mass methane and 57 \% propane. Following Eq.~(\ref{eq:stoich_fuel_single}) yields a change in fuel of
\begin{equation}\label{eq:edc_meth_single}
...
\end{align}
These results are the same as using the predictor-corrector scheme, Eqs.~(\ref{eq:del_meth_C}) and (\ref{eq:del_prop_c}).
