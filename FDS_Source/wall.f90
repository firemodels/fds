...
REAL(EB) :: AREA_ADJUST,ARO,FDERIV,QEXTRA,QNET,RAMP_FACTOR,RHO_G,RHO_G_2,RSUM_W,TMP_G,TMP_OTHER,TSI,UN, &
            RHO_ZZ_F(1:N_TRACKED_SPECIES),ZZ_GET(0:N_TRACKED_SPECIES),T_IGN,DUMMY, & !ZZ_G_ALL(MAX_SPECIES),
...
USE HVAC_ROUTINES, ONLY : NODE_AREA_EX,NODE_TMP_EX,DUCT_MF,LEAK_PATH,NODE_ZZ_EX
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%VENT_INDEX > 0 .AND. .NOT. ANY(SF%LEAK_PATH>0._EB)) THEN
      IF (VENTS(WC%VENT_INDEX)%NODE_INDEX > 0) THEN
         ...
         MFT = -DUCTNODE(DN)%DIR(1)*DUCT_MF(DU,1)/NODE_AREA_EX(DN,1)
         IF (DUCTNODE(DN)%DIR(1)*DUCT_MF(DU,1) > 0._EB) THEN
            WC%ONE_D%TMP_F = NODE_TMP_EX(DN,1)
         ELSE
   ...
   ELSE SPECIES_IF_1
      IF (UN > 0._EB) THEN
         WC%ONE_D%MASSFLUX(1:N_TRACKED_SPECIES) = -NODE_ZZ_EX(DN,1:N_TRACKED_SPECIES,1)*MFT
