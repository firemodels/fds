...
CHARACTER(50) :: TCFORM
LOGICAL :: EX
CHARACTER(255), PARAMETER :: dumpid='$Id: dump.f90,v 1.72 2007/04/30 21:31:39 mcgratta Exp $'
...
PUBLIC INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,DUMP_SLCF,TIMINGS,FLUSH_BUFFERS,READ_CORE,WRITE_DIAGNOSTICS, &
...
ELSE
   ...
   IF (T>=BNDF_CLOCK(NM)) THEN
      ...
      ENDDO
   ENDIF
   IF (T>=PL3D_CLOCK(NM) .OR. STOP_CODE>0) THEN
   ...
   IF (T>=PROF_CLOCK(NM)) THEN
      ...
      ENDDO
   ENDIF
   IF (T>=CORE_CLOCK(NM) .OR. STOP_CODE==2) THEN
      CALL DUMP_CORE(T,NM)
...
ALLOCATE(FNDUMP(1:4 + N_DEVC_FILES + N_CTRL_FILES))
LU_MASS  = 11
LU_HRR   = 12
LU_XYZ   = 13
LU_STATE = 14
LU_DEVC  = 14
LU_CTRL  = 14 + N_DEVC_FILES
WRITE(FNDUMP(1),'(A,A)') TRIM(CHID),'_mass.csv'
WRITE(FNDUMP(2),'(A,A)') TRIM(CHID),'_hrr.csv'
IF (N_DEVC_FILES == 1) THEN
   WRITE(FNDUMP(5),'(A,A)') TRIM(CHID),'_devc.csv'
ELSEIF (N_DEVC_FILES > 1) THEN
   DO I = 1, N_DEVC_FILES
      WRITE(FNDUMP(4+I),'(A,A,I0,A)') TRIM(CHID),'_',I,'_devc.csv'
   ENDDO         
ENDIF
IF (N_CTRL_FILES == 1) THEN
   WRITE(FNDUMP(5 + N_DEVC_FILES),'(A,A)') TRIM(CHID),'_ctrl.csv'
ELSEIF (N_CTRL_FILES > 1) THEN 
   DO I = 1, N_CTRL_FILES
      WRITE(FNDUMP(4 + N_DEVC_FILES),'(A,A,I0,A)') TRIM(CHID),'_',I,'_ctrl.csv'
   ENDDO         
ENDIF
...
IF (APPEND) THEN
   ...
   IF (EX) OPEN(LU6,FILE=FN06,FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
ELSE
...
IF (N_DEVC>0) THEN
   DO I = 1,N_DEVC_FILES
      IF (APPEND) THEN
         OPEN(LU_DEVC+I,FILE=FNDUMP(4+I),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
      ELSE
         OPEN(LU_DEVC+I,FILE=FNDUMP(4+I),FORM='FORMATTED',STATUS='REPLACE')
         WRITE(LU_DEVC+I,'(I5)') MIN(254, N_DEVC - 254 * (I - 1))
...
IF (N_CTRL>0) THEN
   DO I = 1,N_CTRL_FILES
      IF (APPEND) THEN
         OPEN(LU_CTRL+I,FILE=FNDUMP(4+N_DEVC_FILES+I),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
      ELSE
         OPEN(LU_CTRL+I,FILE=FNDUMP(4+N_DEVC_FILES+I),FORM='FORMATTED',STATUS='REPLACE')
         WRITE(LU_CTRL+I,'(I5)') MIN(254, N_CTRL - 254 * (I - 1))
         ...
         WRITE(TCFORM,'(A,I4.4,A)') "(",MIN(254, N_CTRL - 254 * (I - 1)),"(A,','),A)"
         WRITE(LU_CTRL+I,TCFORM) 'TIME',(' ',N=254 * (I - 1) + 1,MIN(N_CTRL, I * 254))
         WRITE(TCFORM,'(A,I4.4,A)') "(",MIN(254, N_CTRL - 254 * (I - 1)),"(A,','),A)"
...
IF (APPEND) THEN
   OPEN(LU_HRR,FILE=FNDUMP(2),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
ELSE
...
DO N=1,N_SPECIES
   IF (SPECIES(N)%MODE==MIXTURE_FRACTION_SPECIES) THEN
      DZZ = 1._EB/10000._EB
      WRITE(FNDUMP(4),'(A,A,I2.2,A)') TRIM(CHID),'_state_',N,'.csv'
      OPEN(LU_STATE,FILE=FNDUMP(4),FORM='FORMATTED',STATUS='REPLACE')
      WRITE(LU_STATE,'(I5)') 10
      WRITE(TCFORM,'(A,I4.4,A)') "(",10,"(A,','),A)"
      WRITE(LU_STATE,TCFORM) 'Z','Fuel','O2','N2','H2O','CO2','CO','H2','Soot','Other','Wgt'
      WRITE(LU_STATE,TCFORM) ('kg/kg',NN=1,10),'g/mol'
      WRITE(LU_STATE,'(10(ES12.4,","),ES12.4)') (K*DZZ,(SPECIES(N)%Y_MF(K,J),J=1,9),SPECIES(N)%MW_MF(K),K=0,10000,10)
      CLOSE(LU_STATE)
   ENDIF
ENDDO
...
IF_DUMP_SPECIES_INFO: IF (MASS_FILE) THEN
   IF (APPEND) THEN
      OPEN(LU_MASS,FILE=FNDUMP(1),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
   ELSE
      OPEN(LU_MASS,FILE=FNDUMP(1),FORM='FORMATTED',STATUS='REPLACE')
...
WRITE_XYZ_FILE: IF (WRITE_XYZ .AND..NOT.EVACUATION_ONLY(NM)) THEN
   IF (NMESHES >1) WRITE(FN_XYZ,'(A,A,I2.2,A)') TRIM(CHID),'_',NM,'.xyz'
   IF (NMESHES==1) WRITE(FN_XYZ,'(A,A)')        TRIM(CHID),       '.xyz'
   OPEN(LU_XYZ,FILE=FN_XYZ,FORM='UNFORMATTED',STATUS='REPLACE')
   DO K=0,KBAR
      DO J=0,JBAR
         DO I=0,IBAR
      ...
      ENDDO   
   ENDDO   
   WRITE(LU_XYZ) IBP1,JBP1,KBP1
   WRITE(LU_XYZ) (((M%XPLT(I),I=0,IBAR),J=0,JBAR),K=0,KBAR),(((M%YPLT(J),I=0,IBAR),J=0,JBAR),K=0,KBAR), &
   ...
   M%N_STRINGS = M%N_STRINGS + 1
   WRITE(M%STRING(M%N_STRINGS),'(A)') FN_XYZ  
...
IF (.NOT.APPEND .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO N=1,N_ISOF
      ...
      IF (NMESHES >1) WRITE(FNSF,'(A,A,I2.2,A,I2.2,A)') TRIM(CHID),'_',NM,'_',N,'.iso'
      IF (NMESHES==1) WRITE(FNSF,'(A,A,I2.2,A)')        TRIM(CHID),'_',N,'.iso'
      ...
      ERROR = 0
      IF (IS%INDEX2==0) CALL ISOHEADER(TRIM(FNSF)//CHAR(0),LABEL1,LABEL2,LABEL3, &
                                       IS%VALUE(1:IS%N_VALUES),IS%N_VALUES,ERROR)
      IF (IS%INDEX2/=0) CALL TISOHEADER(TRIM(FNSF)//CHAR(0),LABEL1,LABEL2,LABEL3, &
                                        IS%VALUE(1:IS%N_VALUES),IS%N_VALUES,ERROR)
   ENDDO
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D .AND..NOT.EVACUATION_ONLY(NM)) THEN
   ...
   IF (NMESHES >1) WRITE(SMOKE3D_FILE(1,NM),'(A,A,I2.2,A)') TRIM(CHID),'_',NM,'_01.s3d'
   IF (NMESHES==1) WRITE(SMOKE3D_FILE(1,NM),'(A,A)')        TRIM(CHID),       '_01.s3d'
   IF (.NOT.APPEND) THEN
      IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
      M%N_STRINGS = M%N_STRINGS + 1
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') OUTPUT_QUANTITY(148)%NAME
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') OUTPUT_QUANTITY(148)%SHORT_NAME
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') OUTPUT_QUANTITY(148)%UNITS
      CALL SMOKE3DHEADER(TRIM(SMOKE3D_FILE(1,NM))//CHAR(0),0,M%IBAR,0,M%JBAR,0,M%KBAR)
   ENDIF
    
   IF (NMESHES >1) WRITE(SMOKE3D_FILE(2,NM),'(A,A,I2.2,A)') TRIM(CHID),'_',NM,'_02.s3d'
   IF (NMESHES==1) WRITE(SMOKE3D_FILE(2,NM),'(A,A)')        TRIM(CHID),       '_02.s3d'
   IF (.NOT.APPEND) THEN
      IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A,1X,I4)') 'SMOKE3D',NM
      ...
      CALL SMOKE3DHEADER(TRIM(SMOKE3D_FILE(2,NM))//CHAR(0),0,M%IBAR,0,M%JBAR,0,M%KBAR)
   ENDIF
...
DO N=1,M%N_SLCF
   ...
   ENDIF
   LU_SLCF = 3000+100*NM + N
   IF (APPEND) THEN
      OPEN(LU_SLCF,FILE=FNSF,FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
   ELSE
      IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
      M%N_STRINGS = M%N_STRINGS + 1
      ...
      OPEN(LU_SLCF,FILE=FNSF,FORM='UNFORMATTED',STATUS='REPLACE')
      WRITE(LU_SLCF) OUTPUT_QUANTITY(SL%INDEX)%NAME
      WRITE(LU_SLCF) OUTPUT_QUANTITY(SL%INDEX)%SHORT_NAME
      WRITE(LU_SLCF) OUTPUT_QUANTITY(SL%INDEX)%UNITS
      WRITE(LU_SLCF) SL%I1,SL%I2,SL%J1,SL%J2,SL%K1,SL%K2
   ENDIF
ENDDO   
...
BOUNDARY_FILES: DO NF=1,N_BNDF
   ...
   RESTART: IF (APPEND) THEN
      OPEN(LU_BNDF,FILE=FNSF,FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
   ELSE RESTART
      IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
      M%N_STRINGS = M%N_STRINGS + 1
      ...
      OPEN(LU_BNDF,FILE=FNSF,FORM='UNFORMATTED',STATUS='REPLACE')
      WRITE(LU_BNDF) OUTPUT_QUANTITY(BF%INDEX)%NAME
      WRITE(LU_BNDF) OUTPUT_QUANTITY(BF%INDEX)%SHORT_NAME
      WRITE(LU_BNDF) OUTPUT_QUANTITY(BF%INDEX)%UNITS
      WRITE(LU_BNDF) M%NPATCH
      IF (M%INC(-3,0)==1) WRITE(LU_BNDF) 0,IBAR,0,JBAR,KBAR,KBAR,-3,0,NM
      IF (M%INC(-2,0)==1) WRITE(LU_BNDF) 0,IBAR,JBAR,JBAR,0,KBAR,-2,0,NM
      IF (M%INC(-1,0)==1) WRITE(LU_BNDF) IBAR,IBAR,0,JBAR,0,KBAR,-1,0,NM
      IF (M%INC( 1,0)==1) WRITE(LU_BNDF)    0,   0,0,JBAR,0,KBAR, 1,0,NM
      IF (M%INC( 2,0)==1) WRITE(LU_BNDF)    0,IBAR,   0,0,0,KBAR, 2,0,NM
      IF (M%INC( 3,0)==1) WRITE(LU_BNDF)    0,IBAR,   0,JBAR,0,0, 3,0,NM
      DO N=1,M%N_OBST
         OB=>M%OBSTRUCTION(N)
         I1 = OB%I1
         I2 = OB%I2
         ...
         K1 = OB%K1
         K2 = OB%K2
         IF (M%INC(-3,N)==1) WRITE(LU_BNDF) I1,I2,J1,J2,K1,K1,-3,N,NM
         IF (M%INC(-2,N)==1) WRITE(LU_BNDF) I1,I2,J1,J1,K1,K2,-2,N,NM
         IF (M%INC(-1,N)==1) WRITE(LU_BNDF) I1,I1,J1,J2,K1,K2,-1,N,NM
         IF (M%INC( 1,N)==1) WRITE(LU_BNDF) I2,I2,J1,J2,K1,K2, 1,N,NM
         IF (M%INC( 2,N)==1) WRITE(LU_BNDF) I1,I2,J2,J2,K1,K2, 2,N,NM
         IF (M%INC( 3,N)==1) WRITE(LU_BNDF) I1,I2,J1,J2,K2,K2, 3,N,NM
      ENDDO
   ENDIF RESTART
...
DROPLET_IF: IF ( (DROPLET_FILE .AND. .NOT.EVACUATION_ONLY(NM)).OR. (EVACUATION_ONLY(NM).AND.EVACUATION_GRID(NM)) ) THEN
   ...
   IF (NMESHES>1) THEN
      WRITE(FN10,'(A,I2.2,A)') TRIM(CHID)//'_',NM,'.prt5'
   ELSE
      WRITE(FN10,'(A,A)') TRIM(CHID),'.prt5'
   ENDIF
   ...
   APPEND_DROPLET_FILE: IF (APPEND) THEN
      ...
      OPEN(LU_PROF,FILE=FN10,FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
   ...
   ELSE APPEND_DROPLET_FILE
      ...
      EVAC_ONLY: IF (EVACUATION_ONLY(NM)) THEN
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A,I3)') 'EVA5',NM
         M%N_STRINGS = M%N_STRINGS + 1
         ...
         DO N=1,N_EVAC
            M%N_STRINGS = M%N_STRINGS + 1
            WRITE(M%STRING(M%N_STRINGS),'(I3)') N+N_PART
         ENDDO
      ELSE
         M%N_STRINGS = M%N_STRINGS + 1
      ...
      OPEN(LU_PROF,FILE=FN10,FORM='UNFORMATTED',STATUS='REPLACE')
      WRITE(LU_PROF) ONE_INTEGER                ! The number ONE, to indicate file Endian-ness
      WRITE(LU_PROF) NINT(VERSION_NUMBER*100.)  ! FDS version number
      EVAC_ONLY2: IF (EVACUATION_ONLY(NM)) THEN
         WRITE(LU_PROF) N_EVAC
         DO N=1,N_EVAC
            WRITE(LU_PROF) EVAC_N_QUANTITIES,ZERO_INTEGER  ! ZERO_INTEGER is a place holder for future INTEGER quantities
            DO NN=1,EVAC_N_QUANTITIES
               WRITE(LU_PROF) OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%NAME
               WRITE(LU_PROF) OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%UNITS
            ENDDO
         ENDDO
      ELSE
         WRITE(LU_PROF) N_PART
         DO N=1,N_PART
            PC => PARTICLE_CLASS(N)
            WRITE(LU_PROF) PC%N_QUANTITIES,ZERO_INTEGER  ! ZERO_INTEGER is a place holder for future INTEGER quantities
            DO NN=1,PC%N_QUANTITIES
               WRITE(LU_PROF) OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%NAME
               WRITE(LU_PROF) OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%UNITS
            ENDDO
         ENDDO
      ENDIF EVAC_ONLY2
...
PROF_LOOP: DO N=1,N_PROF
   ...
   WRITE(FN_PROF,CFORM) TRIM(CHID),'_prof_',N,'.csv'
   LU_PROF = 4000 + N
   IF (APPEND) THEN
      OPEN(LU_PROF,FILE=FN_PROF,FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
   ELSE
...
IF (APPEND) THEN
   OPEN(LU4,FILE=TRIM(CHID)//'.smv',FORM='FORMATTED', STATUS='OLD',POSITION='APPEND')
   RETURN
ENDIF
 
OPEN(LU4,FILE=TRIM(CHID)//'.smv',FORM='FORMATTED',STATUS='REPLACE')
...
WRITE(LU4,'(/A)') 'VERSION'
WRITE(LU4,'(F5.1,2X,A)') VERSION_NUMBER,VERSION_STRING

! Indicate the "endian-ness" of the output files
 
OPEN(LU2,FILE=TRIM(CHID)//'.end',FORM='UNFORMATTED',STATUS='REPLACE')
...
IF (INPUT_FILE=='null') INPUT_FILE = TRIM(CHID)//'.data'
WRITE(LU4,'(/A)') 'INPF'
WRITE(LU4,'(A)') TRIM(INPUT_FILE)

! Write out the CHID
WRITE(LU4,'(/A)') 'CHID'
WRITE(LU4,'(A)') TRIM(CHID)
...
WRITE(LU4,'(2F10.2,I6)') 0.0_EB,MAX(0.01_EB,T_END),MAX(2,NFRAMES)
...
WRITE(LU4,'(/A)') 'SURFDEF'
WRITE(LU4,'(A,A)') ' ',SURF_NAME(DEFAULT_SURF_INDEX)
 
DO N=0,N_SURF
   SF => SURFACE(N)
   WRITE(LU4,'(/A)') 'SURFACE'
   WRITE(LU4,'(A,A)') ' ',SURF_NAME(N)
   IF (SF%THERMALLY_THICK) THEN
      ML => MATERIAL(SF%LAYER_MATL_INDEX(1,1))
      WRITE(LU4,'(2F8.1)') TMPM,ML%EMISSIVITY
   ELSE
      WRITE(LU4,'(2F8.1)') 5000.,1.0
   ENDIF
   WRITE(LU4,'(I2,6F8.2)') SF%SURF_TYPE,SF%TEXTURE_WIDTH,SF%TEXTURE_HEIGHT,REAL(SF%RGB,FB)/255._FB,SF%TRANSPARENCY
   WRITE(LU4,'(A,A)') ' ',SF%TEXTURE_MAP
ENDDO
...
EVAC_ONLY3: IF (.NOT. ALL(EVACUATION_ONLY)) THEN
   DO N=1,N_PART
      PC => PARTICLE_CLASS(N)
      WRITE(LU4,'(/A)') 'CLASS_OF_PARTICLES'
      WRITE(LU4,'(A,A)') ' ',PC%CLASS_NAME
      WRITE(LU4,'(3F8.2)') REAL(PC%RGB,FB)/255._FB
      WRITE(LU4,'(I3)') PC%N_QUANTITIES
      DO NN=1,PC%N_QUANTITIES
         WRITE(LU4,'(A)') OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%NAME
         WRITE(LU4,'(A)') OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%SHORT_NAME
         WRITE(LU4,'(A)') OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%UNITS
      ENDDO
   ENDDO
ENDIF EVAC_ONLY3
...
EVAC_ONLY4: IF (ANY(EVACUATION_GRID)) THEN
   DO N=1,N_EVAC
      WRITE(LU4,'(/A)') 'CLASS_OF_HUMANS'
      WRITE(LU4,'(A,A)') ' ',EVAC_CLASS_NAME(N)
      WRITE(LU4,'(3F8.2)') REAL(EVAC_CLASS_RGB(:,N),FB)/255._EB
      WRITE(LU4,'(I3)') EVAC_N_QUANTITIES
      DO NN=1,EVAC_N_QUANTITIES
         WRITE(LU4,'(A)') OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%NAME
         WRITE(LU4,'(A)') OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%SHORT_NAME
         WRITE(LU4,'(A)') OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%UNITS
      ENDDO
   ENDDO
ENDIF EVAC_ONLY4
...
DO I=0,N_TICKS-1
   WRITE(LU4,'(A)') 'LABEL'
   WRITE(LU4,'(8F10.3)') XMIN+I*X_INT,YMIN-2._EB*TICK_LENGTH,ZMIN,-1.,-1.,-1.,0._EB,T_END
...
DO I=0,N_TICKS-1
   WRITE(LU4,'(A)') 'LABEL'
   WRITE(LU4,'(8F10.3)') XMIN-2._EB*TICK_LENGTH,YMIN+I*Y_INT,ZMIN,-1.,-1.,-1.,0.,T_END
...
DO I=0,N_TICKS-1
   WRITE(LU4,'(A)') 'LABEL'
   WRITE(LU4,'(8F10.3)') XMIN-2._EB*TICK_LENGTH,YMIN,ZMIN+I*Z_INT, -1.,-1.,-1.,0.,T_END
...
DO I=1,N
   SEG=>SEGMENT(I)
   WRITE(LU4,'(6F12.4)') SEG%X1,SEG%Y1,SEG%Z1,SEG%X2,SEG%Y2,SEG%Z2
ENDDO
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   WRITE(LU4,'(4I5)') M%IBAR,M%JBAR,M%KBAR,EVAC_CODE

   WRITE(LU4,'(/A)') 'PDIM'
   WRITE(LU4,'(9F12.5)') M%XS,M%XF,M%YS,M%YF,M%ZS,M%ZF,(REAL(M%RGB(I),FB)/255._FB,I = 1,3)

   WRITE(LU4,'(/A)') 'TRNX'
   WRITE(LU4,'(I5)') T%NOC(1)
   DO N=1,T%NOC(1)
      WRITE(LU4,'(I5,2F12.5)') T%IDERIVSTORE(N,1),T%CCSTORE(N,1),T%PCSTORE(N,1)
   ENDDO
   DO I=0,M%IBAR
   ...
   DO N=1,T%NOC(2)
      WRITE(LU4,'(I5,2F12.5)') T%IDERIVSTORE(N,2),T%CCSTORE(N,2),T%PCSTORE(N,2)
   ENDDO
   DO J=0,M%JBAR
   ...
   DO N=1,T%NOC(3)
      WRITE(LU4,'(I5,2F12.5)') T%IDERIVSTORE(N,3),T%CCSTORE(N,3),T%PCSTORE(N,3)
   ENDDO
   DO K=0,M%KBAR
   ...
   DO N=1,M%N_OBST
      ...
      IF (OB%TEXTURE(1)==-999._EB) THEN
         WRITE(LU4,'(6F12.5,I7,6I4)') OB%X1,OB%X2,OB%Y1,OB%Y2,OB%Z1,OB%Z2,OB%ORDINAL, &
                                    OB%IBC(-1),OB%IBC(1),OB%IBC(-2),OB%IBC(2),OB%IBC(-3),OB%IBC(3)
      ELSE
         WRITE(LU4,'(6F12.5,I7,6I4,3F8.2)') OB%X1,OB%X2,OB%Y1,OB%Y2,OB%Z1,OB%Z2,OB%ORDINAL, &
   ...
   DO N=1,M%N_OBST
      OB=>M%OBSTRUCTION(N)
      IF (OB%BCI/=-3) THEN
         WRITE(LU4,'(8I5)') OB%I1,OB%I2,OB%J1,OB%J2,OB%K1,OB%K2,OB%BCI,OB%BTI
      ELSE
         WRITE(LU4,'(8I5,4F6.2)') OB%I1,OB%I2,OB%J1,OB%J2,OB%K1,OB%K2,OB%BCI,OB%BTI, REAL(OB%RGB,FB)/255._FB, OB%TRANSPARENCY
      ENDIF
   ENDDO
   ...
   WRITE(LU4,'(/A)') 'VENT'
   WRITE(LU4,'(2I5)') M%N_VENT+NDV,NDV

   DO N=1,M%N_VENT
      VT=>M%VENTS(N)
      IF (VT%TEXTURE(1)==-999._EB) THEN
         WRITE(LU4,'(6F12.5,I6,I4)')  VT%X1,VT%X2,VT%Y1,VT%Y2,VT%Z1,VT%Z2,VT%ORDINAL,VT%IBC
      ELSE
         WRITE(LU4,'(6F12.5,I6,I4,3F8.2)') VT%X1,VT%X2,VT%Y1,VT%Y2,VT%Z1,VT%Z2,VT%ORDINAL,VT%IBC, &
                                           VT%TEXTURE(1),VT%TEXTURE(2),VT%TEXTURE(3)
      ENDIF
   ENDDO
   DO N=1,NDV
      WRITE(LU4,'(6F12.5,I6,I4)') M%X(IDV1(N)),M%X(IDV2(N)),M%Y(JDV1(N)),M%Y(JDV2(N)),  &
                                  M%Z(KDV1(N)),M%Z(KDV2(N)),M%N_VENT+N,DEFAULT_SURF_INDEX
   ENDDO

   DO N=1,M%N_VENT
      VT=>M%VENTS(N)
      IF (VT%BOUNDARY_TYPE==OPEN_BOUNDARY) INDX = -VT%VCI
      IF (VT%BOUNDARY_TYPE/=OPEN_BOUNDARY) INDX =  VT%VCI
      IF (VT%RGB(1)<0) THEN
         WRITE(LU4,'(8I5)') VT%I1,VT%I2,VT%J1,VT%J2,VT%K1,VT%K2,INDX,VT%VTI
      ELSE
         WRITE(LU4,'(8I5,4F6.2)') VT%I1,VT%I2,VT%J1,VT%J2,VT%K1,VT%K2,INDX,VT%VTI, REAL(VT%RGB,FB)/255._FB,VT%TRANSPARENCY
      ENDIF
   ENDDO
   DO N=1,NDV
      INDX = 99
      WRITE(LU4,'(8I5)') IDV1(N),IDV2(N),JDV1(N),JDV2(N),KDV1(N),KDV2(N),INDX,0
   ENDDO
...
DO NM=1,NMESHES
WRITE(LU4,'(F8.1)') HRRPUA_SHEET/(7._EB*MESHES(NM)%DXMIN)
ENDDO
...
WRITE(LU4,'(I5)') N_RAMP
DO N=1,N_RAMP
   WRITE(LU4,'(A,A)')  'RAMP: ',RAMP_ID(N)
   WRITE(LU4,'(I5)') RAMPS(N)%NUMBER_DATA_POINTS
   DO I=1,RAMPS(N)%NUMBER_DATA_POINTS
      WRITE(LU4,'(6F12.5)') RAMPS(N)%INDEPENDENT_DATA(I),RAMPS(N)%DEPENDENT_DATA(I)
   ENDDO
ENDDO
...
DO N=1,N_DEVC
   DV => DEVICE(N)
   WRITE(LU4,'(/A)') 'DEVICE'
   WRITE(LU4,'(A)') TRIM(PROPERTY(DV%PROP_INDEX)%SMOKEVIEW_ID)
   WRITE(LU4,'(3F12.5,3F6.2)') DV%X,DV%Y,DV%Z,DV%ORIENTATION(1:3)
ENDDO
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   WRITE(LU6,'(A,F9.3)')   '   Length (m)                    ',M%XF-M%XS
   WRITE(LU6,'(A,F9.3)')   '   Width  (m)                    ',M%YF-M%YS
   WRITE(LU6,'(A,F9.3)')   '   Height (m)                    ',M%ZF-M%ZS
   WRITE(LU6,'(A,F8.3)')   '   Initial Time Step (s)         ',M%DT
ENDDO MESH_LOOP
WRITE(LU6,'(//A/)')     ' Miscellaneous Parameters'
WRITE(LU6,'(A,F8.1)')   '   Simulation Start Time (s)     ',T_BEGIN
WRITE(LU6,'(A,F8.1)')   '   Simulation End Time (s)       ',T_END
IF (LES) THEN
   WRITE(LU6,'(A)')        '   LES Calculation'
   WRITE(LU6,'(A,F8.2)')   '   Smagorinsky Constant          ',CSMAG
   WRITE(LU6,'(A,F8.2)')   '   Turbulent Prandtl Number      ',PR
   IF (N_SPECIES>0._EB) WRITE(LU6,'(A,F8.2)')   '   Turbulent Schmidt Number      ',SC
ENDIF
WRITE(LU6,'(A,F8.2)')   '   Ambient Temperature (C)       ',TMPA-TMPM
...
SPEC_LOOP: DO N=0,N_SPECIES
   ...
   SS => SPECIES(N)
   WRITE(LU6,'(/3X,A)') SS%NAME
   SELECT CASE(SS%MODE)
      CASE (MIXTURE_FRACTION_SPECIES)
         WRITE(LU6,'( 3X,A)') 'Mixture Fraction Variable'
      CASE (GAS_SPECIES)
         WRITE(LU6,'( 3X,A)') 'Gas Species'
      CASE (AEROSOL_SPECIES)            
         WRITE(LU6,'( 3X,A)') 'Aerosol'
   END SELECT
   IF (N==0) WRITE(LU6,'( 3X,A)') 'Background Species'
   IF (SS%MW <  1000._EB .AND. SS%MODE==GAS_SPECIES) &    
      WRITE(LU6,'(A,F8.2)')   '   Molecular Weight (g/mol)      ',SS%MW
   IF (SS%MW >= 1000._EB .AND. SS%MODE==GAS_SPECIES) &    
      WRITE(LU6,'(A,F8.2)')   '   Density (kg/m^3)              ',SS%MW*P_INF/(TMPA*R0)
   WRITE(LU6,'(A,F8.3)')   '   Initial Mass Fraction         ',SS%YY0
   IF (DNS .AND. SS%MODE==GAS_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Viscosity (kg/m/s)   Ambient: ',SS%MU(NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                          500 C: ',SS%MU(77)
      WRITE(LU6,'(A,ES8.2)')  '                         1000 C: ',SS%MU(127)
      WRITE(LU6,'(A,ES8.2)')  '                         1500 C: ',SS%MU(177)
   ENDIF
   IF (DNS .AND. SS%MODE==MIXTURE_FRACTION_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Viscosity (kg/m/s)   Ambient: ',SS%MU_MF(10,NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T= 500 C: ',SS%MU_MF(10,77)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1000 C: ',SS%MU_MF(10,127)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1500 C: ',SS%MU_MF(10,177)
   ENDIF
   IF (.NOT.ISOTHERMAL .AND. DNS .AND. SS%MODE==GAS_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Therm. Cond. (W/m/K) Ambient: ',SS%K(NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                          500 C: ',SS%K(77)
      WRITE(LU6,'(A,ES8.2)')  '                         1000 C: ',SS%K(127)
      WRITE(LU6,'(A,ES8.2)')  '                         1500 C: ',SS%K(177)
   ENDIF
   IF (.NOT.ISOTHERMAL .AND. DNS .AND. SS%MODE==MIXTURE_FRACTION_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Therm. Cond. (W/m/K) Ambient: ',SS%K_MF(10,NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T= 500 C: ',SS%K_MF(10,77)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1000 C: ',SS%K_MF(10,127)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1500 C: ',SS%K_MF(10,177)
   ENDIF
   IF (.NOT.ISOTHERMAL .AND. DNS .AND. SS%MODE==GAS_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Spec. Heat (J/kg/K)  Ambient: ',SS%CP(NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                          500 C: ',SS%CP(77)
      WRITE(LU6,'(A,ES8.2)')  '                         1000 C: ',SS%CP(127)
      WRITE(LU6,'(A,ES8.2)')  '                         1500 C: ',SS%CP(177)
   ENDIF
   IF (.NOT.ISOTHERMAL .AND. DNS .AND. SS%MODE==MIXTURE_FRACTION_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Spec. Heat (J/kg/K)  Ambient: ',SS%CP_MF(10,NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T= 500 C: ',SS%CP_MF(10,77)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1000 C: ',SS%CP_MF(10,127)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1500 C: ',SS%CP_MF(10,177)
   ENDIF
   IF (N>0 .AND. DNS .AND. SS%MODE==GAS_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Diff. Coeff. (m^2/s) Ambient: ',SS%D(NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                          500 C: ',SS%D(77)
      WRITE(LU6,'(A,ES8.2)')  '                         1000 C: ',SS%D(127)
      WRITE(LU6,'(A,ES8.2)')  '                         1500 C: ',SS%D(177)
   ENDIF
   IF (N>0 .AND. DNS .AND. SS%MODE==MIXTURE_FRACTION_SPECIES) THEN
      WRITE(LU6,'(A,ES8.2)')  '   Diff. Coeff. (m^2/s) Ambient: ',SS%D_MF(10,NINT(0.1_EB*TMPA))
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T= 500 C: ',SS%D_MF(10,77)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1000 C: ',SS%D_MF(10,127)
      WRITE(LU6,'(A,ES8.2)')  '                Z=0.1, T=1500 C: ',SS%D_MF(10,177)
   ENDIF
ENDDO SPEC_LOOP
 
! Print out Stoichiometric parameters for mixture fraction model
 
IF (N_REACTIONS>0) WRITE(LU6,'(//A)') ' Gas Phase Reaction Information'
   ...
   SELECT CASE(RN%MODE)
      CASE(MIXTURE_FRACTION_REACTION)
         IF (CO_PRODUCTION) THEN
            SELECT CASE(N)
               CASE(1)
                  WRITE(LU6,'(/3X,A)') 'Incomplete (CO Production) Reaction'
               CASE(2)
                  WRITE(LU6,'(/3X,A)') 'Complete (CO2 Production) Reaction'
               CASE(3)
                  WRITE(LU6,'(/3X,A)') 'Null (Extinction) Reaction'               
             END SELECT
         ELSE
            SELECT CASE(N)
               CASE(1)
             ...
             END SELECT
         ENDIF
         WRITE(LU6,'(3X,A)')  'Mixture Fraction Reaction'
      CASE(FINITE_RATE_REACTION)
         WRITE(LU6,'(/3X,A)') RN%NAME
         WRITE(LU6,'(3X,A)')  'Finte Rate Reaction'
   END SELECT
   WRITE(LU6,'(A,F8.2)')  '   Molecular Weight, Fuel (g/mol)',RN%MW_FUEL
   WRITE(LU6,'(A,F8.0)')  '   Heat of Combustion (kJ/kg)    ',RN%HEAT_OF_COMBUSTION/1000._EB
   SELECT CASE (RN%MODE)
      CASE (MIXTURE_FRACTION_REACTION)
         WRITE(LU6,'(A,F8.3)')  '   Stoich. Coeff., O_2           ',RN%NU_O2
         WRITE(LU6,'(A,F8.3)')  '   Stoich. Coeff., CO_2          ',RN%NU_CO2
         WRITE(LU6,'(A,F8.3)')  '   Stoich. Coeff., H2O           ',RN%NU_H2O
         WRITE(LU6,'(A,F8.3)')  '   Stoich. Coeff., Soot          ',RN%NU_SOOT
         WRITE(LU6,'(A,F8.3)')  '   Stoich. Coeff., CO            ',RN%NU_CO  
         WRITE(LU6,'(A,F8.3)')  '   Stoich. Coeff., N_2           ',RN%NU_N2           
         WRITE(LU6,'(A,F8.3)')  '   Stoich. Coeff., Other         ',RN%NU_OTHER
         WRITE(LU6,'(A,F8.3)')  '   Stoichiometric Value of Z     ',RN%Z_F
      CASE (FINITE_RATE_REACTION)
         DO NN=1,N_SPECIES
            WRITE(LU6,'(A,A15,F8.3)')  '   Stoich. Coeff. ',TRIM(SPECIES(NN)%NAME),RN%NU(NN) 
         ENDDO
         DO NN=1,N_SPECIES
            IF(RN%N(NN)==-999._EB) THEN
               WRITE(LU6,'(A,A15,F8.3)')  '    Rate Exponent ',TRIM(SPECIES(NN)%NAME),0. 
            ELSE
               WRITE(LU6,'(A,A15,F8.3)')  '    Rate Exponent ',TRIM(SPECIES(NN)%NAME),RN%N(NN) 
            ENDIF
         ENDDO
   END SELECT
ENDDO REACTION_LOOP
...
WRITE(LU6,'(//A,I2)')  ' Material Information'
 
MATL_LOOP: DO N=1,N_MATL
   ML => MATERIAL(N)
   WRITE(LU6,'(/I4,1X,A)')    N,MATL_NAME(N)
   WRITE(LU6,'(A,F8.3)')    '     Emissivity                   ',ML%EMISSIVITY
   WRITE(LU6,'(A,F8.1)')    '     Density (kg/m3)              ',ML%RHO_S
   IF (ML%C_S>0._EB) THEN
      WRITE(LU6,'(A,F8.2)') '     Specific Heat (kJ/kg/K)      ',ML%C_S*0.001_EB
   ELSE
      NR = -NINT(ML%C_S)
      WRITE(LU6,'(A,F8.2)') '     Specific Heat (kJ/kg/K)      ',EVALUATE_RAMP(TMPA,0._EB,NR)
   ENDIF
   IF (ML%K_S>0._EB) THEN
      WRITE(LU6,'(A,F8.4)') '     Conductivity (W/m/K)         ',ML%K_S
   ELSE
      NR = -NINT(ML%K_S)
      WRITE(LU6,'(A,F8.4)') '     Conductivity (W/m/K)         ',EVALUATE_RAMP(TMPA,0._EB,NR)
   ENDIF
   IF (ML%KAPPA_S<1.0E4_EB) THEN
      WRITE(LU6,'(A,F8.2)') '     Absorption coefficient (1/m) ',ML%KAPPA_S
   ENDIF
...
SURFLOOP: DO N=0,N_SURF-1
   ...
   SF => SURFACE(N)
   IF (N==0) THEN
      WRITE(LU6,'(/I4,1X,A,A)')  N,SURF_NAME(N),' (DEFAULT)'
   ELSE
      WRITE(LU6,'(/I4,1X,A)')    N,SURF_NAME(N)
   ENDIF
   ...
   THICK: IF (SF%THERMAL_BC_INDEX == THERMALLY_THICK) THEN
      WRITE(LU6,'(A)')      '     Material List'
      DO NN=1,SF%N_MATL
         WRITE(LU6,'(8X,I3,2X,A)') NN,SF%MATL_NAME(NN)
      ENDDO
      IF (SF%SHRINK)          WRITE(LU6,'(A)')'     Shrinking wall'
      DO NL=1,SF%N_LAYERS
         WRITE(LU6,'(A,I2)')      '     Layer ',NL
         WRITE(LU6,'(A,F8.3)')    '        Thickness   (m): ',SF%LAYER_THICKNESS(NL)
         WRITE(LU6,'(A,F8.3)')    '        Density (kg/m3): ',SF%LAYER_DENSITY(NL)
         DO NN=1,SF%N_LAYER_MATL(NL)
            WRITE(LU6,'(8X,A,A,F7.2)') TRIM(SF%LAYER_MATL_NAME(NL,NN)),', Mass fraction: ',SF%LAYER_MATL_FRAC(NL,NN)
         ENDDO
      ENDDO
      WRITE(LU6,'(A)')         '     Solid Phase Nodes (m):      '
      DO I=0,SF%N_CELLS
         WRITE(LU6,'(10X,I3, F10.5)') I,SF%X_S(I)
      ENDDO
      IF (SF%GEOMETRY==SURF_CARTESIAN) THEN
         IF (SF%BACKING==VOID)      WRITE(LU6,'(A)') '     Backing to void'
         IF (SF%BACKING==INSULATED) WRITE(LU6,'(A)') '     Insulated Backing'
         IF (SF%BACKING==EXPOSED)   WRITE(LU6,'(A)') '     Exposed Backing'
      ENDIF
      IF (SF%GEOMETRY==SURF_CYLINDRICAL) WRITE(LU6,'(A)') '     Assumed cylindrical symmetry'
   ENDIF THICK
 
   IF (SF%THERMAL_BC_INDEX==SPECIFIED_TEMPERATURE) &
                                  WRITE(LU6,'(A,F8.1)') '     Wall or Vent Temperature (C)', SF%TMP_FRONT - TMPM
   IF (SF%VEL/=-999._EB)             WRITE(LU6,'(A,F8.3)') '     Normal Velocity (m/s)       ', SF%VEL
   IF (SF%MASS_FLUX_TOTAL/=-999._EB) WRITE(LU6,'(A,F8.3)') '     Total Mass Flux (kg/m^2/s)  ', SF%MASS_FLUX_TOTAL
   ...
   IF (MIXTURE_FRACTION) THEN
      IF (CO_PRODUCTION) THEN
         IF (SF%MASS_FLUX(I_FUEL)>0._EB) WRITE(LU6,'(A,F8.1)') '     HRR Per Unit Area (kW/m2)     ', &
...
IF (N_PROP > 0) WRITE(LU6,'(//A,I2)')  ' Device Properties'
...
PROPERTY_LOOP: DO N=1,N_PROP
   ...
   SELECT CASE(PY%QUANTITY)
      ...
      CASE('spot obscuration')
         ...
         WRITE(LU6,'(A,A   )') '     Detector Class              ', TRIM(PY%ID)                  
   END SELECT
   WRITE(LU6,'(A,A   )') '     Smokeview ID                ', TRIM(PY%SMOKEVIEW_ID)
ENDDO PROPERTY_LOOP
 
! Print out DEVICE locations and info    
 
IF (N_DEVC>0) THEN
   WRITE(LU6,'(//A/)')   ' Device Coordinates'
   DO N=1,N_DEVC
      DV => DEVICE(N)
      WRITE(LU6,'(I4,A,3F9.2,A,A,A,A,A,A)') N,' Coords:',DV%X,DV%Y,DV%Z, &
...
IF (DT_PL3D<T_END) THEN
   ...
   WRITE(LU6,'(A,F8.3/)')'   Sampling Interval (s)          ',DT_PL3D
   DO N=1,5
      WRITE(LU6,'(I4,A,A)') N,' Quantity: ',TRIM(PLOT3D_QUANTITY(N))
   ENDDO
ENDIF
...
IF (N_ISOF>0) THEN
   ...
   WRITE(LU6,'(A,F8.3/)')'   Sampling Interval (s)          ',DT_ISOF
   DO N=1,N_ISOF
      IS => ISOSURFACE_FILE(N)
      WRITE(LU6,'(I4,A,A,A,10F8.3)')N,' Quantity: ',OUTPUT_QUANTITY(IS%INDEX)%NAME(1:20), &
...
MESH_LOOP_4: DO NM=1,NMESHES
   ...
   IF (M%N_SLCF>0) THEN
      ...
      WRITE(LU6,'(A,F8.3/)')'   Sampling Interval (s)          ',DT_SLCF
      DO N=1,M%N_SLCF
         SL=> M%SLICE(N)
...
IF (N_BNDF>0) THEN
   ...
   WRITE(LU6,'(A,F8.3/)')'   Sampling Interval (s)          ',DT_BNDF
   DO N=1,N_BNDF
      BF => BOUNDARY_FILE(N)
      WRITE(LU6,'(I4,A,A)') N,' Quantity: ',TRIM(OUTPUT_QUANTITY(BF%INDEX)%NAME)
   ENDDO
ENDIF
...
WRITE_RADIATION: IF (RADIATION) THEN
   ...
   WRITE(LU6,'(A,I4)')   '   Time step increment      ', TIME_STEP_INCREMENT
   WRITE(LU6,'(A,I4)')   '   Angle increment          ', ANGLE_INCREMENT
   ...
   ELSE
      IF (MIXTURE_FRACTION) THEN
         ...
         WRITE(LU6,'(A,F6.3,A)')'   Mean beam length ',PATH_LENGTH,' m'
      ELSE
         WRITE(LU6,'(A,F6.2,A)')'   Constant absorption coeff. ',KAPPA0
      ENDIF
   ENDIF
   IF (FUEL_EVAPORATION)  WRITE(LU6,'(A)')     '   Extinction by fuel droplets.'
   IF (WATER_EVAPORATION) WRITE(LU6,'(A)')     '   Absorption and scattering by water droplets (Mie theory).'
...
WRITE(LU91) RSUM_W
WRITE(LU91) SOLID
WRITE(LU91) EDGE_INDEX
WRITE(LU91) IJKW
WRITE(LU91) IJKE
WRITE(LU91) BOUNDARY_TYPE
WRITE(LU91) INC
...
DO IW=1,NWC
   ...
   IF (SURFACE(IBC)%THERMALLY_THICK) THEN
      IF (SURFACE(IBC)%SHRINK) THEN
         WRITE(LU91) WALL(IW)%N_LAYER_CELLS,WALL(IW)%LAYER_THICKNESS,WALL(IW)%X_S
         IF (SUM(WALL(IW)%N_LAYER_CELLS)>0) WRITE(LU91) WALL(IW)%TMP_S,WALL(IW)%RHO_S
      ELSE
         IF (SURFACE(IBC)%N_CELLS>0) WRITE(LU91) WALL(IW)%TMP_S,WALL(IW)%RHO_S
      ENDIF
      IF (RADIATION .AND. BOUNDARY_TYPE(IW)/=OPEN_BOUNDARY) WRITE(LU91) WALL(IW)%ILW
   ENDIF
ENDDO
IF (N_SPECIES>0) WRITE(LU91) YY
IF (N_SPECIES>0) WRITE(LU91) DEL_RHO_D_DEL_Y
IF (N_SPECIES>0) WRITE(LU91) YY_W
...
DO IPC=1,N_PART
   ...
   WRITE(LU91) PC%INSERT_CLOCK(1:NMESHES)
ENDDO

WRITE(LU91) T,ICYC,PART_CLOCK(NM),CORE_CLOCK(NM),SLCF_CLOCK(NM), &
             PL3D_CLOCK(NM),BNDF_CLOCK(NM),DEVC_CLOCK,HRR_CLOCK,MINT_CLOCK, &
             ISOF_CLOCK(NM),BC_CLOCK,WALL_COUNTER,DTOLD,DTNEXT, &
             PBAR,D_PBAR_DT,N_EDGES
DO N=1,N_DEVC
   DV => DEVICE(N)
   WRITE(LU91) DV%INSTANT_VALUE,DV%VALUE,DV%T_CHANGE,DV%TMP_L,DV%Y_C,DV%COUNT,DV%CURRENT_STATE,DV%PRIOR_STATE
   IF (DV%QUANTITY=='spot obscuration') THEN
      WRITE(LU91) UBOUND(DV%T_E,1) 
      WRITE(LU91) DV%N_T_E,DV%T_E,DV%Y_E
   ENDIF
...
ENDDO
DO N=1,N_CTRL
   WRITE(LU91) CONTROL(N)%T_CHANGE,CONTROL(N)%CURRENT_STATE,CONTROL(N)%PRIOR_STATE
ENDDO
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (N_SPECIES>0) WRITE(LU91) M2%YY,M2%YYS
   WRITE(LU91) M2%IJKW
...
WRITE(FN90,'(A,A,I2.2,A)') TRIM(RESTART_CHID),'_',NM,'.restart'
INQUIRE(FILE=FN90,EXIST=EX)
IF (.NOT.EX) THEN
   WRITE(MESSAGE,'(A,A,A)') "ERROR: The file, ",TRIM(FN90),", does not exist in the current directory"
...
READ(LU90)  RSUM_W
READ(LU90)  SOLID
READ(LU90)  EDGE_INDEX
READ(LU90)  IJKW
READ(LU90)  IJKE
READ(LU90)  BOUNDARY_TYPE
READ(LU90)  INC
READ(LU90)  MASS_LOSS
READ(LU90)  NWC
READ(LU90)  NPATCH
...
DO IW=1,NWC
   ...
   IF (SURFACE(IBC)%THERMALLY_THICK) THEN
      IF (SURFACE(IBC)%SHRINK) THEN
         READ(LU90) WALL(IW)%N_LAYER_CELLS,WALL(IW)%LAYER_THICKNESS,WALL(IW)%X_S
         IF (SUM(WALL(IW)%N_LAYER_CELLS)>0) THEN
            NULLIFY(MESHES(NM)%WALL(IW)%TMP_S)
            ALLOCATE(MESHES(NM)%WALL(IW)%TMP_S(0:SURFACE(IBC)%N_CELLS+1))
            READ(LU90) MESHES(NM)%WALL(IW)%TMP_S,MESHES(NM)%WALL(IW)%RHO_S
         ENDIF
      ELSE
      ...
      ENDIF
   ENDIF
   IF (RADIATION .AND. BOUNDARY_TYPE(IW)/=OPEN_BOUNDARY) READ(LU90) WALL(IW)%ILW
ENDDO
IF (N_SPECIES>0) READ(LU90)  YY
IF (N_SPECIES>0) READ(LU90)  DEL_RHO_D_DEL_Y
IF (N_SPECIES>0) READ(LU90)  YY_W
IF (DROPLET_FILE) THEN
   READ(LU90) NLP,NLPDIM
   DEALLOCATE(MESHES(NM)%DROPLET)
   ALLOCATE(MESHES(NM)%DROPLET(1:NLPDIM))
   READ(LU90) MESHES(NM)%DROPLET(1:NLP)
ENDIF

DO IPC=1,N_PART
   PC => PARTICLE_CLASS(IPC)
   READ(LU90) PC%INSERT_CLOCK(1:NMESHES)
ENDDO

READ(LU90) T,ICYC,PART_CLOCK(NM),CORE_CLOCK(NM),SLCF_CLOCK(NM), &
             PL3D_CLOCK(NM),BNDF_CLOCK(NM),DEVC_CLOCK,HRR_CLOCK,MINT_CLOCK, &
             ISOF_CLOCK(NM),BC_CLOCK,WALL_COUNTER,DTOLD,DTNEXT, &
             PBAR,D_PBAR_DT,N_EDGES
DO N=1,N_DEVC
   DV => DEVICE(N)
   READ(LU90) DV%INSTANT_VALUE,DV%VALUE,DV%T_CHANGE,DV%TMP_L,DV%Y_C,DV%COUNT,DV%CURRENT_STATE,DV%PRIOR_STATE
   IF (DV%QUANTITY=='spot obscuration') THEN
      READ(LU90) N_T_E_MAX 
      DV%T_E => REALLOCATE(DV%T_E,-1,N_T_E_MAX) 
      DV%Y_E => REALLOCATE(DV%Y_E,-1,N_T_E_MAX) 
      READ(LU90) DV%N_T_E,DV%T_E,DV%Y_E
   ENDIF
   IF (DV%QUANTITY=='aspiration') THEN
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (N_SPECIES>0) READ(LU90) M2%YY,M2%YYS
   READ(LU90) M2%IJKW
ENDDO OTHER_MESH_LOOP
...
WRITE(LU0,'(1X,A,I7,A,F10.2,A)')  'Time Step:',ICYC,',    Simulation Time:',T(1),' s'
...
DO NM=1,NMESHES
   IF (NMESHES>1) WRITE(LU6,'(6X,A,I2,A,I6)') ' Mesh ',NM,', Cycle ',NTCYC(NM)
   M => MESHES(NM)
   IF (T_ACCUM(NM)<60._EB) WRITE(LU6,110) T_PER_STEP(NM),T_ACCUM(NM)
   IF (T_ACCUM(NM)>=60._EB .AND. T_ACCUM(NM)<3600._EB) WRITE(LU6,112) T_PER_STEP(NM),T_ACCUM(NM)/60._EB
   ...
   IF (ABS(RHRR(NM))>1._EB) WRITE(LU6,120) -RHRR(NM)/1000._EB
ENDDO
...
PARTICLE_CLASS_LOOP: DO N=1,N_PART
   ...
   ALLOCATE(YP(NPLIM))
   ALLOCATE(ZP(NPLIM))
   ALLOCATE(QP(NPLIM,PC%N_QUANTITIES))
   ...
   IF (PC%N_QUANTITIES > 0) WRITE(LU_PROF) ((QP(I,NN),I=1,NPLIM),NN=1,PC%N_QUANTITIES)
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   IF (IS%INDEX2==0) THEN
      ...
      CALL ISO2FILE(TRIM(FNSF)//CHAR(0),STIME, QQ(0:IBAR,0:JBAR,0:KBAR,1),IBLK,IS%VALUE(1:IS%N_VALUES), &
   ...
   ELSE
      ...
      CALL ISOT2FILE(TRIM(FNSF)//CHAR(0),STIME,QQ(0:IBAR,0:JBAR,0:KBAR,1),DATAFLAG, &
...
STIME   = T
EXTCOEF = MASS_EXTINCTION_COEFFICIENT/1000._EB
DXX     = DX(1)
FF => WORK3
...
DATA_LOOP: DO DATAFLAG=1,2
   ...
   SELECT CASE(DATAFLAG)
      CASE(1)
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
                  FF(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,151,T)
               ENDDO
            ENDDO
         ENDDO
      CASE(2)
   ...
   CALL SMOKE3DTOFILE(TRIM(SMOKE3D_FILE(DATAFLAG,NM))//CHAR(0), STIME,DXX,EXTCOEF,DATAFLAG, &
         PACK(QQ(0:IBAR,0:JBAR,0:KBAR,1),MASK=.TRUE.), IBP1,JBP1,KBP1)
...
IF (PLOT3D) THEN  ! Write out information to .smv file
   ...
   IF (T_END < 10000._EB) THEN
      IF (ITM < 0) THEN
         IF (NMESHES>1)  WRITE(FN14,'(A,A,I2.2,A1,I5.4,A,I2.2,A2)') TRIM(CHID),'_',NM,'_',ITM,'_',ITM1,'.q'
         IF (NMESHES==1) WRITE(FN14,'(A,A1,I5.4,A,I2.2,A2)')        TRIM(CHID),'_',ITM,'_',ITM1,'.q'
      ELSE
         IF (NMESHES>1)  WRITE(FN14,'(A,A,I2.2,A1,I4.4,A,I2.2,A2)') TRIM(CHID),'_',NM,'_',ITM,'_',ITM1,'.q'
         IF (NMESHES==1) WRITE(FN14,'(A,A1,I4.4,A,I2.2,A2)')        TRIM(CHID),'_',ITM,'_',ITM1,'.q'
      ENDIF
   ELSE
      IF (ITM <0 ) THEN
         IF (NMESHES>1)  WRITE(FN14,'(A,A,I2.2,A1,I6.5,A,I2.2,A2)') TRIM(CHID),'_',NM,'_',ITM,'_',ITM1,'.q'
         IF (NMESHES==1) WRITE(FN14,'(A,A1,I6.5,A,I2.2,A2)')        TRIM(CHID),'_',ITM,'_',ITM1,'.q'
      ELSE
         IF (NMESHES>1)  WRITE(FN14,'(A,A,I2.2,A1,I5.5,A,I2.2,A2)') TRIM(CHID),'_',NM,'_',ITM,'_',ITM1,'.q'
         IF (NMESHES==1) WRITE(FN14,'(A,A1,I5.5,A,I2.2,A2)')        TRIM(CHID),'_',ITM,'_',ITM1,'.q'
      ENDIF
   ENDIF
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   IF (.NOT.PLOT3D) THEN
      ...
      IF (.NOT.SL%RLE .AND. .NOT. SL%TWO_BYTE) THEN
         LU_SLCF = 3000+100*NM + IQ
         STIME = T
         WRITE(LU_SLCF) STIME
         WRITE(LU_SLCF) (((QQ(I,J,K,1),I=I1,I2),J=J1,J2),K=K1,K2)
      ENDIF
      IF (SL%TWO_BYTE) THEN
         LU_SLCF = 3000+100*NM + IQ
         STIME = T
         WRITE(LU_SLCF) STIME
         WRITE(LU_SLCF) (((TWO_BYTE_REAL(QQ(I,J,K,1)),I=I1,I2),J=J1,J2),K=K1,K2)
      ENDIF 
      IF (SL%RLE) THEN
         LU_SLCF = 3000+100*NM + IQ
...
IF (PLOT3D) THEN
   ...
   WRITE(LU14) ((((QQ(I,J,K,IQ),I=0,IBAR),J=0,JBAR),K=0,KBAR),IQ=1,5)
   CLOSE(LU14)
ENDIF
...
REAL(EB) FUNCTION EDGE_VALUE(A,B,S,INDX)
...
REAL(EB) :: FLOW,HMFAC,F1,F2,H_TC,SRAD,TMP_TC,MU_AIR,RE_D,K_AIR,NUSSELT,NU_FAC,PR,AREA,VEL, &
            Q_SUM,TMP_G,UU,VV,WW,VEL2,Y_MF_INT,DENS,EXT_COEF,UIIINT, &
            Z_1,Z_2,Z_3,Z_F,VELSR,WATER_VOL_FRAC,RHS,Y_E,DT_C,DT_E,T_RATIO,Y_E_LAG
...
SELECT CASE(IND)
   ...
   CASE(41:49,141:149,151:154) ! Mixture Fraction Related Quantities
      ...
      CALL GET_MASS_FRACTION(Z_1,Z_2,Z_3,INDX,Y_SUM(II,JJ,KK),Z_F,Y_MF_INT)
      SELECT CASE(IND)
   ...
   CASE(155) ! path obscuration
      ...
      DO NN=1,DV%N_PATH
         ...
         CALL GET_MASS_FRACTION(Z_1,Z_2,Z_3,SOOT_INDEX,Y_SUM(I,J,K),Z_F,Y_MF_INT)
         EXT_COEF = EXT_COEF + Y_MF_INT*RHO(I,J,K)*DV%D_PATH(NN)
      ENDDO
      GAS_PHASE_OUTPUT =  (1._EB - EXP(-1._EB*MASS_EXTINCTION_COEFFICIENT*EXT_COEF))*100._EB
   ...
   CASE(158) ! spot obscuration
      ...
      ELSE
         Z_2 = 0._EB
      ENDIF
      CALL GET_MASS_FRACTION(YY(I,J,K,I_FUEL),Z_2,YY(I,J,K,I_PROG_F),SOOT_INDEX,Y_SUM(I,J,K),Z_F,Y_E)
      VEL2 = 0.25_EB*( (U(I,J,K)+U(I-1,J,K))**2 + (V(I,J,K)+V(I,J-1,K))**2 + (W(I,J,K)+W(I,J,K-1))**2 )
...
SELECT CASE(INDX)
   CASE( 1) ! RADIATIVE_FLUX
      SOLID_PHASE_OUTPUT = QRAD(IWX)*0.001_EB
   ...
   CASE(10) ! HEAT_FLUX
      SOLID_PHASE_OUTPUT = (QRAD(IWX)+QCONF(IWX))*0.001_EB
   ...
   CASE(32) ! ADIABATIC_SURFACE_TEMPERATURE
      ...
      ADLOOP: DO
         ...
         IF ((SOLID_PHASE_OUTPUT_OLD - SOLID_PHASE_OUTPUT) / SOLID_PHASE_OUTPUT < 0.0001_EB) THEN
   ...
   CASE(33) ! WALL_THICKNESS
      IF (SURFACE(IJKW(5,IWX))%SHRINK) THEN
         SOLID_PHASE_OUTPUT = MAXVAL(WALL(IWX)%X_S)
      ELSE
...
DO I=1,N_DEVC_FILES
   ...
   WRITE(LU_DEVC+I,TCFORM) T,(DEVICE(N)%VALUE/REAL(DEVICE(N)%COUNT,EB), N=254*(I-1)+1, MIN(N_DEVC,254*I))
ENDDO
...
DO I=1,N_CTRL_FILES
   ...
   WRITE(LU_CTRL+I,TCFORM) T,(WRITE_VALUE(N), N=254*(I-1)+1, MIN(N_CTRL,254*I))
ENDDO
...
PROF_LOOP: DO N=1,N_PROF
   ...
   IF (PF%QUANTITY == 'TEMPERATURE') THEN
      WRITE(LU_PROF,TCFORM) T,NWP+1, &
      (X_S_NEW(I),I=0,NWP),&
      (WC%TMP_S(I)+DX_WGT_S(I)*(WC%TMP_S(I+1)-WC%TMP_S(I))-TMPM,I=0,NWP)
   ELSE
      ...
      WRITE(LU_PROF,TCFORM) T,NWP+1,&
      (X_S_NEW(I),I=0,NWP),&
      (RHO_S(I)+DX_WGT_S(I)*(RHO_S(I+1)-RHO_S(I)),I=0,NWP)
   ENDIF
ENDDO PROF_LOOP
...
WALL_LOOP: DO IW=1,NWC
   ...
   SELECT CASE(IOR)
      ...
      CASE(-3) 
         U_N =  W(IIG,JJG,KKG)
   END SELECT
   IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY) CHRR(NM) = CHRR(NM) - (QCONF(IW)+QRAD(IW))*AW(IW)
   FHRR(NM) = FHRR(NM) - U_N*RHO_N*CP_GAMMA*(TMP_N-TMPA)*AW(IW)
   MLR(NM)  = MLR(NM)  + ACTUAL_BURN_RATE(IW)*AW(IW)
ENDDO WALL_LOOP
...
WRITE(LU_HRR,TCFORM) T,0.001_EB*HRR_TOTAL, .001_EB*RHRR_TOTAL,.001_EB*FHRR_TOTAL,.001_EB*CHRR_TOTAL, & 
...
REAL(EB) :: VC,Z_1,Z_2,Z_F,Y_MF_INT
INTEGER, INTENT(IN) :: NM
INTEGER :: I,J,K,N,NN
   ...
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         ...
         IF (MIXTURE_FRACTION) THEN
            ...
            ENDIF               
            DO N=1,9
               CALL GET_MASS_FRACTION(Z_1,Z_2,YY(I,J,K,I_PROG_F),N, Y_SUM(I,J,K), Z_F, Y_MF_INT)            
               MINT(N,NM) = MINT(N,NM) + RHO(I,J,K)*Y_MF_INT*VC
            ENDDO
            NN = 9
            DO N=1,N_SPECIES
               IF (SPECIES(N)%MODE==GAS_SPECIES) THEN
...
IF (MIXTURE_FRACTION) THEN
   ...
   WRITE(LU_MASS,TCFORM) T,(MINT_TOTAL(N),N=0,NN)
ELSE
   WRITE(TCFORM,'(A,I4.4,A)') "(",N_SPECIES+1,"(ES15.7E3,','),ES15.7E3)"
   WRITE(LU_MASS,TCFORM) T,(MINT_TOTAL(N),N=0,N_SPECIES)
ENDIF
      ...
      SELECT CASE (IOR)
         CASE (1,-1)
            ...
            DO K=K1-1,K2
               ...
               ENDDO
            ENDDO
            WRITE(LU_BNDF) ((PPN(J,K),J=J1-1,J2),K=K1-1,K2)
         ...
         CASE (2,-2)
            ...
            DO K=K1-1,K2
               ...
               ENDDO
            ENDDO
            WRITE(LU_BNDF) ((PPN(I,K),I=I1-1,I2),K=K1-1,K2)
         ...
         CASE(3,-3)
            ...
            DO J=J1-1,J2
               ...
               ENDDO
            ENDDO
            WRITE(LU_BNDF) ((PPN(I,J),I=I1-1,I2),J=J1-1,J2)
      END SELECT
...
DO NM=1,NMESHES
   IF ( (DROPLET_FILE.AND..NOT.EVACUATION_ONLY(NM)) .OR. (EVACUATION_ONLY(NM).AND.EVACUATION_GRID(NM)) ) THEN
      LU_PROF = 7000 + NM
      CALL FLUSH_BUFFER(LU_PROF)
   ENDIF
ENDDO

DO I = 1, N_DEVC_FILES
   CALL FLUSH_BUFFER(LU_DEVC+I)
ENDDO

CALL FLUSH_BUFFER(LU_HRR)
IF (MASS_FILE) CALL FLUSH_BUFFER(LU_MASS)
...
DO NM=1,NMESHES
   DO N=1,MESHES(NM)%N_SLCF
      LU_SLCF = 3000+100*NM + N
      CALL FLUSH_BUFFER(LU_SLCF)
   ENDDO
   DO N=1,N_BNDF
      LU_BNDF = 1000+100*NM + N
      INQUIRE(UNIT=LU_BNDF,OPENED=EX)
      IF (EX) CALL FLUSH_BUFFER(LU_BNDF)
   ENDDO
ENDDO

IF (ANY(EVACUATION_GRID)) CALL FLUSH_BUFFER(LU121_EVAC)
IF (ANY(EVACUATION_GRID)) CALL FLUSH_BUFFER(LU122_EVAC)
IF (ANY(EVACUATION_GRID)) CALL FLUSH_BUFFER(LU123_EVAC)
...
IF (N_DEVC > 0) THEN
   WRITE(LU6,'(//A/)')   ' DEVICE Activation Times'
   DO N=1,N_DEVC
      DV => DEVICE(N)
     IF (DV%T_CHANGE < 100000._EB) WRITE(LU6,'(I4,2X,A,F8.1,A)') N,DV%ID,DV%T_CHANGE,' s'
!     IF (DV%T_CHANGE > 100000._EB) WRITE(LU6,'(I4,2X,A,A)')      N,DV%ID,'No Activation'
   ENDDO
ENDIF
...
WRITE(LU6,'(//A,F12.3)') ' Elapsed Wall Clock Time (s): ',WALL_CLOCK_END-WALL_CLOCK_START
