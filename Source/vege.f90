...
PRIVATE
PUBLIC INITIALIZE_LEVEL_SET_FIRESPREAD_1,INITIALIZE_LEVEL_SET_FIRESPREAD_2,LEVEL_SET_FIRESPREAD,UPDATE_FIRE_ARRIVAL
INTEGER :: IZERO
INTEGER  :: LIMITER_LS
...
AGL_ONLY: IF (LEVEL_SET_MODE==0) THEN
   ...
   Z_LS_OTHER = 0._EB
   IF (FIRE_ARRIVAL_SLICE) TOA_LS_OTHER = 0._EB
   DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
      DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
         Z_LS_OTHER = Z_LS_OTHER + OMESH(NOM)%Z_LS(IIO,JJO)
         IF (FIRE_ARRIVAL_SLICE) TOA_LS_OTHER = TOA_LS_OTHER + OMESH(NOM)%TOA(IIO,JJO)
      ENDDO
   ENDDO
   ...
   IF (USE_OTHER) THEN
      ...
      IF (FIRE_ARRIVAL_SLICE) TOA(II,JJ) = TOA_LS_OTHER/REAL(N_INT_CELLS,EB)
   ENDIF
...
ELSE
   ...
   QUANTITY_LOOP: DO IQ=1,N_SLCF
      ...
      IF (OUTPUT_QUANTITY(SL%INDEX)%NAME=='TIME OF ARRIVAL') EXIT QUANTITY_LOOP
   ENDDO QUANTITY_LOOP

   DO I=0,IBP1
      DO J=0,JBP1
         IF (Q(I,J,K_AGL_SLICE(I,J,NTSL))>HRRPUVCUT) THEN
            FIRE_PRESENT=.TRUE.
         ELSE
            FIRE_PRESENT=.FALSE.
         ENDIF
...
END SUBROUTINE UPDATE_FIRE_ARRIVAL
