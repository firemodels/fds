...
REAL :: ACTIVATION_TEMPERATURE,A_C,ALPHA,AREA,A_T,A_V,CHI_R,CONDUIT_DIAMETER,CONDUIT_THICKNESS,CUTOFF_TIME, C_S,D, &
        DELTA,DT,H,H_K,H_V,JACKET_THICKNESS,K_S,L,L_F,LEAK_AREA,MASS_PER_LENGTH,M_DOT,P,Q,Q_STAR,Q_STEP,R, &
        RADIATIVE_FRACTION,RHO_AIR,RHO_S,RTI,SCALING_FACTOR,T,t_activation,T_END,TMP_A,TMP_G,T_P,T_CLOCK,U_JET,V, &
        V_DOT,W,W_V,Z_H
REAL, DIMENSION(10) :: X,Z
REAL, DIMENSION(0:5) :: TMP_RAMP,T_RAMP
CHARACTER(20), DIMENSION(10) :: Z_LABEL
LOGICAL :: PROFILE=.FALSE.,ITER=.TRUE.,HEADER=.TRUE.
INTEGER :: I,IOS
...
NAMELIST /SPKACT/ ALPHA,CUTOFF_TIME,RTI,ACTIVATION_TEMPERATURE,H,R,TMP_A,RADIATIVE_FRACTION,SCALING_FACTOR,OUTPUT_FILE
NAMELIST /HESKESTAD/ T,Q,Z_H,A_C,TMP_A,RHO_AIR,RADIATIVE_FRACTION,OUTPUT_FILE,HEADER
...
! Process SPKACT (NUREG Sprinkler Activation) lines

DO
   READ(10,NML=SPKACT,END=106,ERR=99,IOSTAT=IOS)
   CALL COMPUTE_SPRINKLER_ACTIVATION
ENDDO
106 WRITE(0,*) 'Done Sprinkler Activation'
REWIND(10)
...
SUBROUTINE COMPUTE_SPRINKLER_ACTIVATION
...
END SUBROUTINE COMPUTE_SPRINKLER_ACTIVATION
...
IF (HEADER) THEN
   OPEN(11,FILE=TRIM(OUTPUT_FILE),FORM='FORMATTED',STATUS='REPLACE')
   WRITE(11,'(A)') 'Time,HRR,Height,Temperature'
ELSE
   OPEN(11,FILE=TRIM(OUTPUT_FILE),FORM='FORMATTED',STATUS='OLD')
ENDIF
...
Z_0 = -1.02*D + 0.083*(Q)**(2./5.)

! Compute plume centerline temperature

T_P = 9.1 * ((TMP_A+273)/(G*(C_P**2.)*(RHO_AIR)**2.))**(1./3.) * (Q_C)**(2./3.) * (Z_H-Z_0)**(-5./3.) + (TMP_A+273)

WRITE(11,'(F6.1,A1,F6.1,A1,F6.1,A1,F6.1)') T,',',Q,',',Z_H,',',T_P-273
