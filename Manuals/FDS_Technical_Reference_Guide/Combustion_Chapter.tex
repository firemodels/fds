...
\be
K(T) = \exp(-\Delta G_{\mathrm{rxn}}^\circ(T)/(RT))
\ee
where
\be
G_{\mathrm{rxn}}^\circ = \frac{d}{a} G_\mathrm{D}^\circ + \frac{c}{a} G_\mathrm{C}^\circ - \frac{b}{a} G_\mathrm{B}^\circ - G_\mathrm{A}^\circ
\ee
Note that the Gibbs free energy is tabulated for species in the FDS database as a function of temperature.  New species may be added with specified Gibbs free energy.
...
\end{equation}
The heat release per unit volume, define as $\dot{q}''' \equiv - \sum_\alpha \dot{m}_\alpha''' \, \Delta h_{{\rm f},\alpha}^0$, is then found by summing the species mass production rates times the respective heats of formation:
