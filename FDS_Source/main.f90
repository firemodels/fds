...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (OMESH_PARTICLES) THEN
      ...
      ALLOCATE(M%OMESH(NOM)%LAGRANGIAN_PARTICLE(M%OMESH(NOM)%N_DROP_ORPHANS_DIM), STAT=IZERO)
   ...
   INITIALIZATION_IF: IF (CODE==0) THEN
      ...
      IF (OMESH_PARTICLES) THEN
         IF (M3%NIC_S>0 .OR. M3%NIC_R>0) THEN
            ALLOCATE(M3%REAL_RECV_PKG6(  (22+(OM_LP_NODES+2)+OM_LP_LAYERS+OM_LP_NODES+1+2*(N_TRACKED_SPECIES+1)+&
                     (OM_LP_NODES+2)*OM_LP_MATL+(OM_LP_ILW_A*OM_LP_ILW_B))*N_DROP_ADOPT_MAX))
            ALLOCATE(M3%INTG_RECV_PKG1((9+OM_LP_LAYERS)*N_DROP_ADOPT_MAX))
            ALLOCATE(M3%LOGI_RECV_PKG1(7*N_DROP_ADOPT_MAX))
         ENDIF
      ENDIF
...
INTEGER :: NN1,NN2,NRA,NSB,INDEX1,INDEX2
...
SENDING_MESH_LOOP: DO NM=1,NMESHES
   ...
   RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
      ...
      INITIALIZE_SEND_IF: IF (CODE==0) THEN
         ...
         IF (OMESH_PARTICLES .AND.  (M3%NIC_S>0 .OR. M3%NIC_R>0) .AND. RNODE/=SNODE) THEN
            ALLOCATE(M3%REAL_SEND_PKG6( (22+(OM_LP_NODES+2)+OM_LP_LAYERS+OM_LP_NODES+1+2*(N_TRACKED_SPECIES+1)+&
                  (OM_LP_NODES+2)*OM_LP_MATL+(OM_LP_ILW_A*OM_LP_ILW_B))*N_DROP_ADOPT_MAX))
            ALLOCATE(M3%INTG_SEND_PKG1((9+OM_LP_LAYERS)*N_DROP_ADOPT_MAX))
            ALLOCATE(M3%LOGI_SEND_PKG1(7*N_DROP_ADOPT_MAX))     
         ENDIF 
      ...
      IF_SEND_PARTICLES: IF (CODE==6 .AND. OMESH_PARTICLES) THEN 
         ...
         NODE_CHECK_PARTICLE: IF (SNODE/=RNODE) THEN
            ...
            NC =  (22+(OM_LP_NODES+2)+OM_LP_LAYERS+OM_LP_NODES+1+2*(N_TRACKED_SPECIES+1)+&
                  (OM_LP_NODES+2)*OM_LP_MATL+(OM_LP_ILW_A*OM_LP_ILW_B))

            DO N=1,MIN(M3%N_DROP_ORPHANS,N_DROP_ADOPT_MAX)
               M3%REAL_SEND_PKG6((N-1)*NC+1)  = M3%LAGRANGIAN_PARTICLE(N)%TMP_F            
               M3%REAL_SEND_PKG6((N-1)*NC+2)  = M3%LAGRANGIAN_PARTICLE(N)%TMP_B            
               M3%REAL_SEND_PKG6((N-1)*NC+3)  = M3%LAGRANGIAN_PARTICLE(N)%X
               M3%REAL_SEND_PKG6((N-1)*NC+4)  = M3%LAGRANGIAN_PARTICLE(N)%Y
               M3%REAL_SEND_PKG6((N-1)*NC+5)  = M3%LAGRANGIAN_PARTICLE(N)%Z
               M3%REAL_SEND_PKG6((N-1)*NC+6)  = M3%LAGRANGIAN_PARTICLE(N)%U
               M3%REAL_SEND_PKG6((N-1)*NC+7)  = M3%LAGRANGIAN_PARTICLE(N)%V
               M3%REAL_SEND_PKG6((N-1)*NC+8)  = M3%LAGRANGIAN_PARTICLE(N)%W
               M3%REAL_SEND_PKG6((N-1)*NC+9)  = M3%LAGRANGIAN_PARTICLE(N)%PWT
               M3%REAL_SEND_PKG6((N-1)*NC+10) = M3%LAGRANGIAN_PARTICLE(N)%ACCEL_X
               M3%REAL_SEND_PKG6((N-1)*NC+11) = M3%LAGRANGIAN_PARTICLE(N)%ACCEL_Y
               M3%REAL_SEND_PKG6((N-1)*NC+12) = M3%LAGRANGIAN_PARTICLE(N)%ACCEL_Z
               M3%REAL_SEND_PKG6((N-1)*NC+13) = M3%LAGRANGIAN_PARTICLE(N)%T
               M3%REAL_SEND_PKG6((N-1)*NC+14)  = M3%LAGRANGIAN_PARTICLE(N)%RE            
               M3%REAL_SEND_PKG6((N-1)*NC+15)  = M3%LAGRANGIAN_PARTICLE(N)%MASS            
               M3%REAL_SEND_PKG6((N-1)*NC+16)  = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%AREA            
               M3%REAL_SEND_PKG6((N-1)*NC+17)  = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%RAREA            
               M3%REAL_SEND_PKG6((N-1)*NC+18)  = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%HEAT_TRANS_COEF            
               M3%REAL_SEND_PKG6((N-1)*NC+19)  = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%QCONF            
               M3%REAL_SEND_PKG6((N-1)*NC+20)  = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%QRADIN            
               M3%REAL_SEND_PKG6((N-1)*NC+21)  = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%QRADOUT            
               M3%REAL_SEND_PKG6((N-1)*NC+22)  = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%EMISSIVITY

               INDEX1=(N-1)*NC+23               
               INDEX2=INDEX1+N_TRACKED_SPECIES
               M3%REAL_SEND_PKG6(INDEX1:INDEX2) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(0:N_TRACKED_SPECIES)

               INDEX1=INDEX2+1
               INDEX2=INDEX1+N_TRACKED_SPECIES            
               M3%REAL_SEND_PKG6(INDEX1:INDEX2) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES)

               INDEX1=INDEX2+1
               INDEX2=INDEX1+OM_LP_NODES+1          
               M3%REAL_SEND_PKG6(INDEX1:INDEX2) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(0:OM_LP_NODES+1) 

               INDEX1=INDEX2+1
               INDEX2=INDEX1+OM_LP_LAYERS-1              
               M3%REAL_SEND_PKG6(INDEX1:INDEX2) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1:OM_LP_LAYERS)

               INDEX1=INDEX2+1
               INDEX2=INDEX1+OM_LP_NODES               
               M3%REAL_SEND_PKG6(INDEX1:INDEX2) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%X(0:OM_LP_NODES)

               INDEX1=INDEX2+1
               INDEX2=INDEX1+(OM_LP_MATL)*(OM_LP_NODES+2)-1        
               DO II = 0,OM_LP_NODES+1
                  DO JJ = 1,OM_LP_MATL
                      M3%REAL_SEND_PKG6(INDEX1) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(II,JJ)
                     INDEX1=INDEX1+1
                  ENDDO
               ENDDO
               ...
               DO II = 1,OM_LP_ILW_A
                  DO JJ = 1,OM_LP_ILW_B
                     INDEX1 = INDEX1 + 1
                     M3%REAL_SEND_PKG6(INDEX1) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(II,JJ)
                  ENDDO
               ENDDO              
            ENDDO  
            ...
            DO N=1,MIN(M3%N_DROP_ORPHANS,N_DROP_ADOPT_MAX)
               M3%INTG_SEND_PKG1((N-1)*NC+1) = M3%LAGRANGIAN_PARTICLE(N)%IOR
               M3%INTG_SEND_PKG1((N-1)*NC+2) = M3%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX
               M3%INTG_SEND_PKG1((N-1)*NC+3) = M3%LAGRANGIAN_PARTICLE(N)%SPLIT_IOR
               M3%INTG_SEND_PKG1((N-1)*NC+4) = M3%LAGRANGIAN_PARTICLE(N)%SURF_INDEX
               M3%INTG_SEND_PKG1((N-1)*NC+5) = M3%LAGRANGIAN_PARTICLE(N)%TAG
               M3%INTG_SEND_PKG1((N-1)*NC+6) = M3%LAGRANGIAN_PARTICLE(N)%WALL_INDEX
               M3%INTG_SEND_PKG1((N-1)*NC+7) = M3%LAGRANGIAN_PARTICLE(N)%IIG
               M3%INTG_SEND_PKG1((N-1)*NC+8) = M3%LAGRANGIAN_PARTICLE(N)%JJG
               M3%INTG_SEND_PKG1((N-1)*NC+9) = M3%LAGRANGIAN_PARTICLE(N)%KKG
               M3%INTG_SEND_PKG1((N-1)*NC+10:(N-1)*NC+9+OM_LP_LAYERS) = &
                   M3%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1:OM_LP_LAYERS)
            ENDDO
            BUFFER_SIZE = NC*MIN(M3%N_DROP_ORPHANS,N_DROP_ADOPT_MAX)
            BUFFER_SIZE = MAX(1,BUFFER_SIZE)
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            ...
            NC = 7
            DO N=1,MIN(M3%N_DROP_ORPHANS,N_DROP_ADOPT_MAX)
               M3%LOGI_SEND_PKG1((N-1)*NC+1) = M3%LAGRANGIAN_PARTICLE(N)%ALREADY_ALLOCATED
               M3%LOGI_SEND_PKG1((N-1)*NC+2) = M3%LAGRANGIAN_PARTICLE(N)%SHOW
               M3%LOGI_SEND_PKG1((N-1)*NC+3) = M3%LAGRANGIAN_PARTICLE(N)%SPLAT
               M3%LOGI_SEND_PKG1((N-1)*NC+4) = M3%LAGRANGIAN_PARTICLE(N)%IGNITOR
               M3%LOGI_SEND_PKG1((N-1)*NC+5) = M3%LAGRANGIAN_PARTICLE(N)%EVAPORATE
               M3%LOGI_SEND_PKG1((N-1)*NC+6) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%BURNAWAY
               M3%LOGI_SEND_PKG1((N-1)*NC+7) = M3%LAGRANGIAN_PARTICLE(N)%ONE_D%SHRINKING            
            ENDDO
            BUFFER_SIZE = NC*MIN(M3%N_DROP_ORPHANS,N_DROP_ADOPT_MAX)
            BUFFER_SIZE = MAX(1,BUFFER_SIZE)
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
         ...
         ELSE NODE_CHECK_PARTICLE
            M2=>MESHES(NOM)%OMESH(NM)
            DROP_ADOPT_CHECK: IF (M2%N_DROP_ADOPT>0) THEN
               DO N=M4%NLP+1,M4%NLP+M2%N_DROP_ADOPT
                  M4%LAGRANGIAN_PARTICLE(N)%IOR               = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%IOR
                  M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX       = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%CLASS_INDEX
                  M4%LAGRANGIAN_PARTICLE(N)%SPLIT_IOR         = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%SPLIT_IOR
                  M4%LAGRANGIAN_PARTICLE(N)%SURF_INDEX        = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%SURF_INDEX
                  M4%LAGRANGIAN_PARTICLE(N)%TAG               = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%TAG
                  M4%LAGRANGIAN_PARTICLE(N)%WALL_INDEX        = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%WALL_INDEX
                  M4%LAGRANGIAN_PARTICLE(N)%IIG               = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%IIG
                  M4%LAGRANGIAN_PARTICLE(N)%JJG               = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%JJG
                  M4%LAGRANGIAN_PARTICLE(N)%KKG               = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%KKG
                  M4%LAGRANGIAN_PARTICLE(N)%ALREADY_ALLOCATED = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ALREADY_ALLOCATED
                  M4%LAGRANGIAN_PARTICLE(N)%SHOW              = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%SHOW
                  M4%LAGRANGIAN_PARTICLE(N)%SPLAT             = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%SPLAT
                  M4%LAGRANGIAN_PARTICLE(N)%IGNITOR           = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%IGNITOR
                  M4%LAGRANGIAN_PARTICLE(N)%EVAPORATE         = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%EVAPORATE
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%BURNAWAY    = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%BURNAWAY
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%SHRINKING   = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%SHRINKING     
                  M4%LAGRANGIAN_PARTICLE(N)%TMP_F             = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%TMP_F
                  M4%LAGRANGIAN_PARTICLE(N)%TMP_B             = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%TMP_B
                  M4%LAGRANGIAN_PARTICLE(N)%X                 = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%X
                  M4%LAGRANGIAN_PARTICLE(N)%Y                 = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%Y
                  M4%LAGRANGIAN_PARTICLE(N)%Z                 = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%Z
                  M4%LAGRANGIAN_PARTICLE(N)%U                 = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%U
                  M4%LAGRANGIAN_PARTICLE(N)%V                 = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%V
                  M4%LAGRANGIAN_PARTICLE(N)%W                 = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%W
                  M4%LAGRANGIAN_PARTICLE(N)%PWT               = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%PWT
                  M4%LAGRANGIAN_PARTICLE(N)%ACCEL_X           = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ACCEL_X
                  M4%LAGRANGIAN_PARTICLE(N)%ACCEL_Y           = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ACCEL_Y
                  M4%LAGRANGIAN_PARTICLE(N)%ACCEL_Z           = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ACCEL_Z
                  M4%LAGRANGIAN_PARTICLE(N)%T                 = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%T
                  M4%LAGRANGIAN_PARTICLE(N)%RE                = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%RE       
                  M4%LAGRANGIAN_PARTICLE(N)%MASS              = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%MASS
               
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%AREA            = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%AREA
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RAREA           = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%RAREA
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%HEAT_TRANS_COEF = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%HEAT_TRANS_COEF
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%QCONF           = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%QCONF
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%QRADIN          = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%QRADIN
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%QRADOUT         = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%QRADOUT
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%EMISSIVITY      = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%EMISSIVITY   

                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX)
                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL)
                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP) 
                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS)
                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X)
                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS)
                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW)
                  IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO)) &
                     DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO)

                  SURF_INDEX_IF: IF(LAGRANGIAN_PARTICLE_CLASS(M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX)%SURF_INDEX > 0) THEN
                     SF => SURFACE(LAGRANGIAN_PARTICLE_CLASS(M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX)%SURF_INDEX)
                     THERMALLY_THICK_IF: IF (SF%THERMALLY_THICK) THEN
                        ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(0:SF%N_CELLS+1))
                        M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(0:SF%N_CELLS+1) = &
                           M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%TMP(0:SF%N_CELLS+1)
                        ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(0:SF%N_CELLS+1,SF%N_MATL))
                        M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(0:SF%N_CELLS+1,SF%N_MATL) = &
                           M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%RHO(0:SF%N_CELLS+1,SF%N_MATL)
                        IF (SF%SHRINK) THEN
                           ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(SF%N_LAYERS))
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(SF%N_LAYERS) = &
                              M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%LAYER_THICKNESS(SF%N_LAYERS)
                           ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(SF%N_LAYERS))
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(SF%N_LAYERS) = &
                              M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%N_LAYER_CELLS(SF%N_LAYERS)
                           ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(0:SF%N_CELLS))
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(0:SF%N_CELLS) = &
                              M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%X(0:SF%N_CELLS)
                        ELSE
                           ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1))
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1) = &
                              M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%LAYER_THICKNESS(1)
                           ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1))
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1) = &
                              M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%N_LAYER_CELLS(1)
                           ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1))
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1) = &
                              M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%X(1)
                        ENDIF
                     ELSE THERMALLY_THICK_IF
                        ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1))
                        M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%TMP(1)
                        ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1))
                        M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1) = &
                           M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%LAYER_THICKNESS(1) 
                        ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1))
                        M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%X(1)
                        ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1))
                        M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%N_LAYER_CELLS(1)
                        ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(1,1))
                        M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(1,1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%RHO(1,1)
                     ENDIF THERMALLY_THICK_IF
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(0:N_TRACKED_SPECIES))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(0:N_TRACKED_SPECIES) = & 
                        M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%MASSFLUX(0:N_TRACKED_SPECIES)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES) = &
                        M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(NRA,NSB))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(1:OM_LP_ILW_A,1:OM_LP_ILW_B) = &
                        M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%ILW(1:OM_LP_ILW_A,1:OM_LP_ILW_B)
                  ELSE SURF_INDEX_IF
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(1))                  
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%MASSFLUX(0)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%MASSFLUX_ACTUAL(0)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%TMP(1)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%LAYER_THICKNESS(1) 
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%X(1)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%N_LAYER_CELLS(1)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(1,1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(1,1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%ILW(1,1)
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(1,1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(1,1) = M3%LAGRANGIAN_PARTICLE(N-M4%NLP)%ONE_D%RHO(1,1)
                  ENDIF SURF_INDEX_IF
               ENDDO
               M4%NLP = M4%NLP + M2%N_DROP_ADOPT               
            ENDIF DROP_ADOPT_CHECK
...
RECV_MESH_LOOP: DO NM=1,NMESHES
   ...
   RECEIVE_RADIATION: IF ( CODE==6 .AND. M2%NIC_S>0 .AND. EXCHANGE_RADIATION .AND. RNODE/=SNODE) THEN
      ...
      UNPACK_REAL_RECV_PKG5: DO 
         ...
         DO NN2=1,NSB
            DO NN1=1,NRA
               LL = LL + 1
               M4%WALL(IW)%ONE_D%ILW(NN1,NN2) = M2%REAL_RECV_PKG5(LL)
            ENDDO
         ENDDO
   ...
   IF (CODE==6 .AND. OMESH_PARTICLES .AND. RNODE/=SNODE) THEN
   ...
   IF_RECEIVE_PARTICLES: IF (CODE==6 .AND. OMESH_PARTICLES .AND. RNODE/=SNODE) THEN 
      IF_PARTICLES_SENT: IF (M2%N_DROP_ADOPT>0) THEN
         NC = 9+OM_LP_LAYERS
         DO N=M4%NLP+1,M4%NLP+M2%N_DROP_ADOPT
            NN = N-M4%NLP-1
            M4%LAGRANGIAN_PARTICLE(N)%IOR                                = M2%INTG_RECV_PKG1((NN)*NC+1) 
            M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX                        = M2%INTG_RECV_PKG1((NN)*NC+2) 
            M4%LAGRANGIAN_PARTICLE(N)%SPLIT_IOR                          = M2%INTG_RECV_PKG1((NN)*NC+3) 
            M4%LAGRANGIAN_PARTICLE(N)%SURF_INDEX                         = M2%INTG_RECV_PKG1((NN)*NC+4) 
            M4%LAGRANGIAN_PARTICLE(N)%TAG                                = M2%INTG_RECV_PKG1((NN)*NC+5) 
            M4%LAGRANGIAN_PARTICLE(N)%WALL_INDEX                         = M2%INTG_RECV_PKG1((NN)*NC+6)             
            M4%LAGRANGIAN_PARTICLE(N)%IIG                                = M2%INTG_RECV_PKG1((NN)*NC+7) 
            M4%LAGRANGIAN_PARTICLE(N)%JJG                                = M2%INTG_RECV_PKG1((NN)*NC+8) 
            M4%LAGRANGIAN_PARTICLE(N)%KKG                                = M2%INTG_RECV_PKG1((NN)*NC+9) 
            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS)) DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS)

            IF (LAGRANGIAN_PARTICLE_CLASS(M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX)%SURF_INDEX > 0) THEN
               SF=>SURFACE(LAGRANGIAN_PARTICLE_CLASS(M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX)%SURF_INDEX)
               IF (SF%THERMALLY_THICK) THEN
                  IF (SF%SHRINK) THEN
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(SF%N_LAYERS))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS) = &
                        M2%INTG_RECV_PKG1((NN)*NC+10:(NN)*NC+9+SF%N_LAYERS)
                  ELSE
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1))                  
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1) = M2%INTG_RECV_PKG1((NN)*NC+10)                  
                  ENDIF
               ENDIF
            ELSE
               ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%N_LAYER_CELLS(1))                  
         ...
         DO N=M4%NLP+1,M4%NLP+M2%N_DROP_ADOPT
            NN = N-M4%NLP-1
            M4%LAGRANGIAN_PARTICLE(N)%ALREADY_ALLOCATED  = M2%LOGI_RECV_PKG1((NN)*NC+1)  
            M4%LAGRANGIAN_PARTICLE(N)%SHOW               = M2%LOGI_RECV_PKG1((NN)*NC+2)  
            M4%LAGRANGIAN_PARTICLE(N)%SPLAT              = M2%LOGI_RECV_PKG1((NN)*NC+3)  
            M4%LAGRANGIAN_PARTICLE(N)%IGNITOR            = M2%LOGI_RECV_PKG1((NN)*NC+4)  
            M4%LAGRANGIAN_PARTICLE(N)%EVAPORATE          = M2%LOGI_RECV_PKG1((NN)*NC+5)  
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%BURNAWAY     = M2%LOGI_RECV_PKG1((NN)*NC+6)  
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%SHRINKING    = M2%LOGI_RECV_PKG1((NN)*NC+7)  
         ENDDO         

         NC =  (22+(OM_LP_NODES+2)+OM_LP_LAYERS+OM_LP_NODES+1+2*(N_TRACKED_SPECIES+1)+&
               (OM_LP_NODES+2)*OM_LP_MATL+(OM_LP_ILW_A*OM_LP_ILW_B))
         REAL_DATA:DO N=M4%NLP+1,M4%NLP+M2%N_DROP_ADOPT
            NN = N-M4%NLP-1
            
            M4%LAGRANGIAN_PARTICLE(N)%TMP_F                 = M2%REAL_RECV_PKG6((NN)*NC+1)            
            M4%LAGRANGIAN_PARTICLE(N)%TMP_B                 = M2%REAL_RECV_PKG6((NN)*NC+2)
            M4%LAGRANGIAN_PARTICLE(N)%X                     = M2%REAL_RECV_PKG6((NN)*NC+3)
            M4%LAGRANGIAN_PARTICLE(N)%Y                     = M2%REAL_RECV_PKG6((NN)*NC+4)
            M4%LAGRANGIAN_PARTICLE(N)%Z                     = M2%REAL_RECV_PKG6((NN)*NC+5)
            M4%LAGRANGIAN_PARTICLE(N)%U                     = M2%REAL_RECV_PKG6((NN)*NC+6)
            M4%LAGRANGIAN_PARTICLE(N)%V                     = M2%REAL_RECV_PKG6((NN)*NC+7)
            M4%LAGRANGIAN_PARTICLE(N)%W                     = M2%REAL_RECV_PKG6((NN)*NC+8)
            M4%LAGRANGIAN_PARTICLE(N)%PWT                   = M2%REAL_RECV_PKG6((NN)*NC+9)
            M4%LAGRANGIAN_PARTICLE(N)%ACCEL_X               = M2%REAL_RECV_PKG6((NN)*NC+10)
            M4%LAGRANGIAN_PARTICLE(N)%ACCEL_Y               = M2%REAL_RECV_PKG6((NN)*NC+11)
            M4%LAGRANGIAN_PARTICLE(N)%ACCEL_Z               = M2%REAL_RECV_PKG6((NN)*NC+12)
            M4%LAGRANGIAN_PARTICLE(N)%T                     = M2%REAL_RECV_PKG6((NN)*NC+13)
            M4%LAGRANGIAN_PARTICLE(N)%RE                    = M2%REAL_RECV_PKG6((NN)*NC+14)
            M4%LAGRANGIAN_PARTICLE(N)%MASS                  = M2%REAL_RECV_PKG6((NN)*NC+15)
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%AREA            = M2%REAL_RECV_PKG6((NN)*NC+16)
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RAREA           = M2%REAL_RECV_PKG6((NN)*NC+17)
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%HEAT_TRANS_COEF = M2%REAL_RECV_PKG6((NN)*NC+18)
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%QCONF           = M2%REAL_RECV_PKG6((NN)*NC+19)
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%QRADIN          = M2%REAL_RECV_PKG6((NN)*NC+20)
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%QRADOUT         = M2%REAL_RECV_PKG6((NN)*NC+21)
            M4%LAGRANGIAN_PARTICLE(N)%ONE_D%EMISSIVITY      = M2%REAL_RECV_PKG6((NN)*NC+22)
            
            INDEX1=(NN)*NC+23
            INDEX2=INDEX1+N_TRACKED_SPECIES

            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX)) DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX)
            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL)) &
               DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL)
            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP)) DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP) 
            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS)) &
               DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS)
            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X)) DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X)
            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO)) DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO)
            IF (ALLOCATED(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW)) DEALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW)


            SURF_CHECK2:IF (LAGRANGIAN_PARTICLE_CLASS(M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX)%SURF_INDEX > 0) THEN
               SF=>SURFACE(LAGRANGIAN_PARTICLE_CLASS(M4%LAGRANGIAN_PARTICLE(N)%CLASS_INDEX)%SURF_INDEX) 
               ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(0:N_TRACKED_SPECIES))
               M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(0:N_TRACKED_SPECIES) = M2%REAL_RECV_PKG6(INDEX1:INDEX2)
               INDEX1=INDEX2+1
               INDEX2=INDEX1+N_TRACKED_SPECIES            
               ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES))
               M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES) = M2%REAL_RECV_PKG6(INDEX1:INDEX2) 
               INDEX1=INDEX2+1
               INDEX2=INDEX1+OM_LP_NODES+1          
               THERM_THICK2: IF (SF%THERMALLY_THICK) THEN                  
                  SHRINK_CHECK: IF (SF%SHRINK) THEN
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(0:SF%N_CELLS+1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(0:SF%N_CELLS+1) = M2%REAL_RECV_PKG6(INDEX1:INDEX1+SF%N_CELLS+1)
                     INDEX1=INDEX2+1
                     INDEX2=INDEX1+OM_LP_LAYERS-1   
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1:SF%N_LAYERS))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1:SF%N_LAYERS) = M2%REAL_RECV_PKG6(INDEX1:INDEX1+SF%N_LAYERS-1)
                     INDEX1=INDEX2+1
                     INDEX2=INDEX1+OM_LP_NODES    
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(0:SF%N_CELLS))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(0:SF%N_CELLS) = M2%REAL_RECV_PKG6(INDEX1:INDEX1+SF%N_CELLS)
                     INDEX1=INDEX2+1
                     INDEX2=INDEX1+(OM_LP_MATL)*(OM_LP_NODES+2)-1
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(0:SF%N_CELLS,SF%N_MATL))
                     DO II = 0,SF%N_CELLS
                        DO JJ = 1,SF%N_MATL                           
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(II,JJ) = M2%REAL_RECV_PKG6(INDEX1)  
                           INDEX1=INDEX1 + 1
                        ENDDO
                     ENDDO 
                     INDEX1=INDEX2
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(OM_LP_ILW_A,OM_LP_ILW_B))
                     DO II = 1, OM_LP_ILW_A
                        DO JJ = 1, OM_LP_ILW_A
                           INDEX1 = INDEX1 + 1
                           M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(II,JJ) =  M2%REAL_RECV_PKG6(INDEX1)
                        ENDDO
                     ENDDO
                  ELSE SHRINK_CHECK
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(0:SF%N_CELLS+1))
                     ...
                     INDEX2=INDEX1+OM_LP_LAYERS-1    
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1) = M2%REAL_RECV_PKG6(INDEX1)
                     INDEX1=INDEX2+1
                     INDEX2=INDEX1+OM_LP_NODES   
                     ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(0:SF%N_CELLS))
                     M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(0:SF%N_CELLS) = M2%REAL_RECV_PKG6(INDEX1:INDEX1+SF%N_CELLS)
               ...
               ELSE THERM_THICK2
                  ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1))
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1) = M2%REAL_RECV_PKG6(INDEX1+1)
                  INDEX1=INDEX2+1
                  INDEX2=INDEX1+OM_LP_LAYERS-1     
                  ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1))
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%LAYER_THICKNESS(1) = M2%REAL_RECV_PKG6(INDEX1)
                  ...
                  INDEX2=INDEX1+OM_LP_NODES    
                  ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1))
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%X(1) = M2%REAL_RECV_PKG6(INDEX1+1)
                  INDEX1=INDEX2+1
                  ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(1,1))
                  INDEX2=INDEX1+(OM_LP_MATL)*(OM_LP_NODES+2)-1               
                  M4%LAGRANGIAN_PARTICLE(N)%ONE_D%RHO(1,1) = M2%REAL_RECV_PKG6(INDEX1+OM_LP_MATL)
                  INDEX1=INDEX2
                  ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(OM_LP_ILW_A,OM_LP_ILW_B))
                  DO II = 1, OM_LP_ILW_A
                     DO JJ = 1, OM_LP_ILW_B
            ...
            ELSE SURF_CHECK2 
               ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(1))
               M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX(1) = M2%REAL_RECV_PKG6(INDEX1)
               INDEX1=INDEX2+1
               INDEX2=INDEX1+N_TRACKED_SPECIES  
               ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(1))
               M4%LAGRANGIAN_PARTICLE(N)%ONE_D%MASSFLUX_ACTUAL(1) = M2%REAL_RECV_PKG6(INDEX1) 
               INDEX1=INDEX2+1
               INDEX2=INDEX1+OM_LP_NODES+1        
               ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1))
               M4%LAGRANGIAN_PARTICLE(N)%ONE_D%TMP(1) = M2%REAL_RECV_PKG6(INDEX1+1)
               ...
               INDEX1=INDEX2+1
               ALLOCATE(M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(1,1))
               M4%LAGRANGIAN_PARTICLE(N)%ONE_D%ILW(1,1) =  M2%REAL_RECV_PKG6(INDEX1)
            ENDIF SURF_CHECK2
...
IF (USE_MPI) THEN
   ...
   REAL_BUFFER_1 = Q_DOT(1,:)
   CALL MPI_GATHERV(REAL_BUFFER_1(DISP),CNT,MPI_DOUBLE_PRECISION,Q_DOT(1,:),COUNTS,DISPLS,&
                    MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
   REAL_BUFFER_1 = Q_DOT(2,:)
   CALL MPI_GATHERV(REAL_BUFFER_1(DISP),CNT,MPI_DOUBLE_PRECISION,Q_DOT(2,:),COUNTS,DISPLS,&
                    MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF
...
IF_DUMP_HRR: IF (T>=HRR_CLOCK) THEN
   ...
   IF (MINVAL(HRR_TIME_INTERVAL,MASK=.NOT.EVACUATION_ONLY)>0._EB) THEN
      IF (USE_MPI) THEN
         DO N=1,N_Q_DOT
            REAL_BUFFER_1 = Q_DOT_SUM(N,:)
            CALL MPI_GATHERV(REAL_BUFFER_1(DISP),CNT,MPI_DOUBLE_PRECISION, &
                             Q_DOT_SUM(N,:),COUNTS,DISPLS,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
         ENDDO
         DO N=1,N_M_DOT
            REAL_BUFFER_1 = M_DOT_SUM(N,:)
            CALL MPI_GATHERV(REAL_BUFFER_1(DISP),CNT,MPI_DOUBLE_PRECISION, &
                             M_DOT_SUM(N,:),COUNTS,DISPLS,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
         ENDDO
      ENDIF
...
IF (N_DEVC>0) THEN
   ...
   DO CNT=1,N_DEVC
      IF (DEVICE(CNT)%PROP_INDEX > 0) THEN
