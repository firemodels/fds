...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,RELAXATION_FACTOR,FYI, &
                ...
                WIND_ONLY,TERRAIN_CASE,MATLAB_DLMREAD,PERIODIC_BC,TURB_INIT,COMPUTE_VISCOSITY_TWICE, &
                CONSTANT_PROPERTIES,FLUXMAX,DYNSMAG,DSMAG_FREQ,EMBEDDED_MESH
...
REAL(EB) :: EPSIJ,SIGMA2,AMW,OMEGA,TSTAR,D_N0,TE,EPSK_N,SIG_N,MW_N, &
            CP_N2,CP_F,CP_CO2,CP_O2,CP_H2O,CP_CO,CP_H2,D_TMP(N_STATE_SPECIES),MW_Z2,MW_Z3
REAL(EB), ALLOCATABLE, DIMENSION(:) :: MU_TMP,CP_TMP,K_TMP
REAL(EB) :: YYY(1:N_SPECIES)            
INTEGER :: IPC,NN,N
...
IF (MIXTURE_FRACTION) THEN
   ...
   DO N = 1,N_REACTIONS-1
      Y2Y(N2_INDEX,I_Z_MIN+N)    = REACTION(N)%Y_F_INLET / REACTION(N)%MW_FUEL * MW_N2 * REACTION(N)%NU(N2_INDEX) &
                                    -REACTION(N)%Y_N2_INFTY
      Y2Y(H2O_INDEX,I_Z_MIN+N)   = REACTION(N)%Y_F_INLET / REACTION(N)%MW_FUEL * MW_H2O * REACTION(N)%NU(H2O_INDEX)
      Y2Y(H2_INDEX,I_Z_MIN+N)    = REACTION(N)%Y_F_INLET / REACTION(N)%MW_FUEL * MW_H2 * REACTION(N)%NU(H2_INDEX)
      Y2Y(SOOT_INDEX,I_Z_MIN+N)  = REACTION(N)%Y_F_INLET / REACTION(N)%MW_FUEL * MW_SOOT * REACTION(N)%NU(SOOT_INDEX)
      Y2Y(OTHER_INDEX,I_Z_MIN+N) = REACTION(N)%Y_F_INLET / REACTION(N)%MW_FUEL * MW_OTHER * REACTION(N)%NU(OTHER_INDEX)
   ENDDO    
   IF (CO_PRODUCTION) THEN
   ...
   ELSE
      Y2Y(O2_INDEX,I_Z_MIN+1)  = -REACTION(1)%Y_F_INLET / REACTION(1)%MW_FUEL * MW_O2 * REACTION(1)%NU(O2_INDEX) &
                                 -REACTION(1)%Y_O2_INFTY
      Y2Y(CO_INDEX,I_Z_MIN+1)  = REACTION(1)%Y_F_INLET / REACTION(1)%MW_FUEL * MW_CO  * REACTION(1)%NU(CO_INDEX)
      Y2Y(CO2_INDEX,I_Z_MIN+1) = REACTION(1)%Y_F_INLET / REACTION(1)%MW_FUEL * MW_CO2 * REACTION(1)%NU(CO2_INDEX)      
   ENDIF
   DO N=1,N_SPECIES
      IF (SPECIES(N)%MODE /= MIXTURE_FRACTION_SPECIES) Y2Y(SPECIES(N)%INDEX,N) = 1._EB
...
ALLOCATE(Y2MW_C(N_Y_ARRAY),STAT=IZERO)
CALL ChkMemErr('READ','Y2MW_C',IZERO)    
Y2MW_C = 0._EB   
ALLOCATE(Y2MW(N_Y_ARRAY,N_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ','Y2MW',IZERO) 
Y2MW = 0._EB
...
IF (MIXTURE_FRACTION) THEN
   Y2MW_C(N2_INDEX) = REACTION(1)%Y_N2_INFTY/MW_N2   
   Y2MW_C(O2_INDEX) = REACTION(1)%Y_O2_INFTY/MW_O2
   MW_AVG_Y_C = REACTION(1)%Y_O2_INFTY/MW_O2 + REACTION(1)%Y_N2_INFTY/MW_N2
   Y2MW(FUEL_INDEX,:)  = Y2Y(FUEL_INDEX,:)  / RN%MW_FUEL
   Y2MW(N2_INDEX,:)    = Y2Y(N2_INDEX,:)    / MW_N2
   Y2MW(O2_INDEX,:)    = Y2Y(O2_INDEX,:)    / MW_O2
   Y2MW(CO_INDEX,:)    = Y2Y(CO_INDEX,:)    / MW_CO
   Y2MW(CO2_INDEX,:)   = Y2Y(CO2_INDEX,:)   / MW_CO2
   Y2MW(H2O_INDEX,:)   = Y2Y(H2O_INDEX,:)   / MW_H2O
   Y2MW(H2_INDEX,:)    = Y2Y(H2_INDEX,:)    / MW_H2
   Y2MW(SOOT_INDEX,:)  = Y2Y(SOOT_INDEX,:)  / MW_SOOT
   Y2MW(OTHER_INDEX,:) = Y2Y(OTHER_INDEX,:) / MW_OTHER
   ...
   IF (CO_PRODUCTION) &
      MW_AVG_Y(I_Z_MAX) = Y2Y(FUEL_INDEX,I_Z_MAX)/RN%MW_FUEL + Y2Y(N2_INDEX,I_Z_MAX)/MW_N2 +  Y2Y(O2_INDEX,I_Z_MAX)/MW_O2 + &
   ...
   DO N=1,N_SPECIES
      IF (SPECIES(N)%MODE /= MIXTURE_FRACTION_SPECIES) THEN
         IF(SPECIES(N)%INDEX > N_STATE_SPECIES) Y2MW(SPECIES(N)%INDEX,:) = Y2Y(SPECIES(N)%INDEX,:)/SPECIES(N)%MW
...
ELSE
   ...
   DO N=1,N_SPECIES
      Y2MW(SPECIES(N)%INDEX,:) = Y2Y(SPECIES(N)%INDEX,:) / SPECIES(N)%MW
      MW_AVG_Y(N) = Y2Y(SPECIES(N)%INDEX,N)/SPECIES(N)%MW-MW_AVG_Y_C
   ENDDO
ENDIF  
...
CALL GET_MOLECULAR_WEIGHT(YYY,RSUM0)
RSUM0 = R0/RSUM0
...
ALLOCATE(Y2CPBAR_C(500))   
CALL ChkMemErr('READ','Y2CPBAR_C',IZERO)    
Y2CPBAR_C = 0._EB   

ALLOCATE(Y2H_G_C(500))
...
ALLOCATE(Y2CP(500,N_SPECIES))     
CALL ChkMemErr('READ','Y2CC',IZERO)    
Y2CP = 0._EB   

ALLOCATE(Y2CPBAR(500,N_SPECIES))     
CALL ChkMemErr('READ','Y2CPBAR',IZERO)    
Y2CPBAR = 0._EB   

ALLOCATE(Y2H_G(500,N_SPECIES))           
CALL ChkMemErr('READ','YH_GC',IZERO)    
Y2H_G = 0._EB   

ALLOCATE(Y2K(500,N_SPECIES))      
CALL ChkMemErr('READ','Y2K',IZERO)    
Y2K = 0._EB   

ALLOCATE(Y2MU(500,N_SPECIES))      
...
IF (MIXTURE_FRACTION) THEN
   ...
   Y2D_C = 0._EB   
   ALLOCATE(Y2D(500,I_Z_MAX-I_Z_MIN+1))
   CALL ChkMemErr('READ','Y2D_C',IZERO)    
   Y2D = 0._EB   
ENDIF
...
T_LOOP_1: DO J=1,500
   ...
   ENDIF
   IF(MIXTURE_FRACTION) THEN
      CP_TMP(FUEL_INDEX)  = CP_F*1000._EB
      CP_TMP(O2_INDEX)    = CP_O2*1000._EB
      CP_TMP(N2_INDEX)    = CP_N2*1000._EB
      CP_TMP(OTHER_INDEX) = CP_N2*1000._EB
      CP_TMP(H2O_INDEX)   = CP_H2O*1000._EB
      CP_TMP(CO2_INDEX)   = CP_CO2*1000._EB
      CP_TMP(CO_INDEX)    = CP_CO*1000._EB
      CP_TMP(SOOT_INDEX)  = 904._EB !cp carbon
      CP_TMP(H2_INDEX)    = CP_H2*1000._EB
      SUB_SPECIES_LOOP2: DO NN=1,N_STATE_SPECIES
         ...
         D_TMP(NN) = 0.00266E-4_EB*AMW*(10*J)**1.5_EB/(SIGMA2*OMEGA)
      ENDDO SUB_SPECIES_LOOP2
      Y2CP_C(J)    = SUM(Y2Y_C(:) * CP_TMP(:))
      IF (J>1) THEN
         Y2H_G_C(J)   = Y2H_G_C(J-1) + Y2CP_C(J) * 10._EB
      ELSE
         Y2H_G_C(J)   =                Y2CP_C(J) * 10._EB
      ENDIF
      Y2CPBAR_C(J) = Y2H_G_C(J)*0.001/TE
   ...
   SPECIES_LOOP_1: DO N = 0,N_SPECIES
      ...
      SELECT CASE(SS%ID)
         CASE DEFAULT
            CP_TMP(SS%INDEX) = SS%RCON*GAMMA/(GAMMA-1._EB)
         CASE('AIR')
            CP_TMP(SS%INDEX) = (0.77_EB*CP_N2+0.23_EB*CP_O2)*1000._EB
         CASE('NITROGEN')
            CP_TMP(SS%INDEX) = CP_N2 *1000._EB
         CASE('OXYGEN')
            CP_TMP(SS%INDEX) = CP_O2 *1000._EB
         CASE('METHANE')
            CP_TMP(SS%INDEX) = CP_F  *1000._EB
         CASE('CARBON DIOXIDE')
            CP_TMP(SS%INDEX) = CP_CO2*1000._EB
         CASE('WATER VAPOR')
            CP_TMP(SS%INDEX) = CP_H2O*1000._EB
         CASE('CARBON MONOXIDE')
            CP_TMP(SS%INDEX) = CP_CO*1000._EB
         CASE('HYDROGEN')
            CP_TMP(SS%INDEX) = CP_H2*1000._EB
         CASE('ARGON')
      ...
      TSTAR = J*10/EPSK(N)
      OMEGA = 1.16145_EB*TSTAR**(-0.14874_EB) +  0.52487_EB*EXP(-0.77320_EB*TSTAR) + 2.16178_EB*EXP(-2.43787_EB*TSTAR)
      MU_TMP(SS%INDEX) = 26.69E-7_EB*(SS%MW*J*10)**0.5_EB/(SIGMA2*OMEGA)/SS%MW
      IF (MU_USER(N)>=0._EB .AND. .NOT.CONSTANT_PROPERTIES) MU_TMP(SS%INDEX) = MU_USER(N)*(J*10/TMPA)**0.75_EB/SS%MW
      ...
      IF (K_USER(N)>=0._EB .AND. .NOT.CONSTANT_PROPERTIES)  K_TMP(SS%INDEX) = K_USER(N)*(J*10/TMPA)**0.75_EB/SS%MW
      ...
      IF(SS%MODE /= MIXTURE_FRACTION_SPECIES) THEN
         ...
         OMEGA = 1.06036_EB/TSTAR**(0.15610_EB) + 0.19300_EB/EXP(0.47635_EB*TSTAR) + 1.03587_EB/EXP(1.52996_EB*TSTAR) &
               + 1.76474_EB/EXP(3.89411_EB*TSTAR)     
         D_N0 = 0.00266E-4_EB*AMW*(10*J)**1.5_EB/(SIGMA2*OMEGA)
         IF (D_USER(N)>=0._EB .AND. .NOT.CONSTANT_PROPERTIES) D_N0 = D_USER(N)*(J*10/TMPA)**1.75_EB
   ...
   SPECIES_LOOP_2: DO N=1,N_SPECIES
      ...
      IF (J>1) THEN
         Y2H_G(J,:)   = Y2H_G(J-1,:) + Y2CP(J,:) * 10._EB
      ELSE
         Y2H_G(J,:)   =                Y2CP(J,:) * 10._EB      
      ENDIF
      Y2CPBAR(J,:) = Y2H_G(J,:)*0.001/TE      
...
CHARACTER(30) :: PART_ID,SPEC_ID,QUANTITIES(1:10)
CHARACTER(25) :: COLOR
...
NAMELIST /TREE/ XYZ,XB,CROWN_WIDTH,CROWN_WIDTH_BOTTOM,CROWN_WIDTH_TOP,CROWN_BASE_HEIGHT,TREE_HEIGHT,PART_ID, &
                ...
                T_RAMPON_IGNITOR_ELEMENTS,FUEL_GEOM,RAMP_IGNELEM
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   SELECT CASE(BACKING)
      ...
      CASE('EXPOSED')
         SF%BACKING        = EXPOSED
   END SELECT
   SF%BOUNDARY_LAYER_THICKNESS = BOUNDARY_LAYER_THICKNESS
...
ADIABATIC               = .FALSE.
BACKING                 = 'VOID'
BOUNDARY_LAYER_THICKNESS= 0.01_EB
BURN_AWAY               = .FALSE.
...
RGB                     = -1
SHRINK                  = .TRUE.
IF (LES) SLIP_FACTOR    = -10._EB    ! Do not specify for LES
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
            ...
            IF (ML%N_REACTIONS > 0) THEN
               IF (I_FUEL /= 0) THEN
                  ADJUSTED_LAYER_DENSITY = ADJUSTED_LAYER_DENSITY + &
                     SF%LAYER_MATL_FRAC(NL,NN)/(MAXVAL(ML%ADJUST_BURN_RATE(1:ML%N_REACTIONS,I_FUEL))*ML%RHO_S)
               ELSE
                  ADJUSTED_LAYER_DENSITY = ADJUSTED_LAYER_DENSITY + &
                     SF%LAYER_MATL_FRAC(NL,NN)/(MAXVAL(ML%ADJUST_BURN_RATE(1:ML%N_REACTIONS,:))*ML%RHO_S)
               ENDIF
            ELSE
   ...
   IF (SF%VEL_T(1)/=-999._EB) SF%SPECIFIED_TANGENTIAL_VELOCITY = .TRUE. 
...
INIT_LOOP: DO N=1,N_INIT
   ...
   CALL GET_MOLECULAR_WEIGHT(YY_GET,RR_SUM)
   RR_SUM = R0/RR_SUM
...
LOGICAL :: INITIAL_STATE,LATCH,DRY
...
NAMELIST /DEVC/ DEPTH,FYI,IOR,ID,ORIENTATION,PROP_ID,QUANTITY,ROTATION,XB,XYZ,INITIAL_STATE,LATCH,TRIP_DIRECTION,CTRL_ID,& 
                SETPOINT,DEVC_ID,FLOWRATE,DELAY,BYPASS_FLOWRATE,SURF_ID,STATISTICS,PART_ID,MATL_ID,SPEC_ID,DRY
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   N = 0
   N_TERRAIN_SLCF = 0
   !! SLICE%TERRAIN_SLICE = .FALSE. !! set in type.f90
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         IF (ITER==2 .OR. ITER==3 .OR. ITER ==4) THEN
            ...
            ENDIF
         ENDIF
         SL%CELL_CENTERED_SLICE = CELL_CENTERED
      ENDDO VECTORLOOP
...
OUTPUT_QUANTITY(103)%NAME  = 'LEAKAGE'                   
OUTPUT_QUANTITY(103)%UNITS  = 'm3/s'                    
OUTPUT_QUANTITY(103)%SHORT_NAME  = 'leak'
OUTPUT_QUANTITY(103)%SLCF_APPROPRIATE = .FALSE.
OUTPUT_QUANTITY(103)%ISOF_APPROPRIATE = .FALSE.
OUTPUT_QUANTITY(103)%PART_APPROPRIATE = .FALSE.
...
IF (QUANTITY=='MASS_LOSS')                     QUANTITY = 'MASS LOSS'
