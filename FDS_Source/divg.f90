...
!   SPECIES_DIFFUSION: IF (.NOT.MIXTURE_FRACTION) THEN
   SPECIES_DIFFUSION: IF (SPECIES(N)%MODE/=MIXTURE_FRACTION_SPECIES) THEN
      ...
      !$OMP DO COLLAPSE(3) PRIVATE(K,J,I,TMP_G,YY_GET,H_G,HDIFF)
      DO K=0,KBAR
         DO J=0,JBAR
            DO I=0,IBAR
      ...
      !$OMP DO PRIVATE(IW,IIG,JJG,KKG,IOR,TMP_G,YY_GET,H_G,HDIFF,RHO_D_DYDN)
...
!   SPECIES_DIFFUSION_2: IF (.NOT.MIXTURE_FRACTION) THEN
   SPECIES_DIFFUSION_2: IF (SPECIES(N)%MODE/=MIXTURE_FRACTION_SPECIES) THEN
      !$OMP DO COLLAPSE(3) PRIVATE(K,J,I,YY_GET,H_G,HDIFF)
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
