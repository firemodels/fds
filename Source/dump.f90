...
MESH_LOOP: DO NM=1,NMESHES
   ...
   LU_SMOKE3D(3,NM) = -GET_FILE_NUMBER()
   IF (RESTART) LU_SMOKE3D(3,NM) = ABS(LU_SMOKE3D(3,NM))
   IF (NMESHES >1) WRITE(FN_SMOKE3D(3,NM),'(A,A,I4.4,A)') TRIM(CHID),'_',NM,'_03.s3d'
   IF (NMESHES==1) WRITE(FN_SMOKE3D(3,NM),'(A,A)')        TRIM(CHID),       '_03.s3d'

   LU_SMOKE3D(4,NM) = -GET_FILE_NUMBER()
   IF (RESTART) LU_SMOKE3D(4,NM) = ABS(LU_SMOKE3D(4,NM))
   IF (NMESHES >1) WRITE(FN_SMOKE3D(4,NM),'(A,A,I4.4,A)') TRIM(CHID),'_',NM,'_04.s3d'
   IF (NMESHES==1) WRITE(FN_SMOKE3D(4,NM),'(A,A)')        TRIM(CHID),       '_04.s3d'

   LU_SMOKE3D(5,NM) = -GET_FILE_NUMBER() ! size file unit number for LU_SMOKE3D(1,NM)
   IF (RESTART) LU_SMOKE3D(5,NM) = ABS(LU_SMOKE3D(5,NM))
   IF (NMESHES >1) WRITE(FN_SMOKE3D(5,NM),'(A,A,I4.4,A)') TRIM(CHID),'_',NM,'_01.s3d.sz'
   IF (NMESHES==1) WRITE(FN_SMOKE3D(5,NM),'(A,A)')        TRIM(CHID),       '_01.s3d.sz'

   LU_SMOKE3D(6,NM) = -GET_FILE_NUMBER() ! size file unit number for LU_SMOKE3D(2,NM)
   IF (RESTART) LU_SMOKE3D(6,NM) = ABS(LU_SMOKE3D(6,NM))
   IF (NMESHES >1) WRITE(FN_SMOKE3D(6,NM),'(A,A,I4.4,A)') TRIM(CHID),'_',NM,'_02.s3d.sz'
   IF (NMESHES==1) WRITE(FN_SMOKE3D(6,NM),'(A,A)')        TRIM(CHID),       '_02.s3d.sz'

   LU_SMOKE3D(7,NM) = -GET_FILE_NUMBER() ! size file unit number for LU_SMOKE3D(3,NM)
   IF (RESTART) LU_SMOKE3D(7,NM) = ABS(LU_SMOKE3D(7,NM))
   IF (NMESHES >1) WRITE(FN_SMOKE3D(7,NM),'(A,A,I4.4,A)') TRIM(CHID),'_',NM,'_03.s3d.sz'
   IF (NMESHES==1) WRITE(FN_SMOKE3D(7,NM),'(A,A)')        TRIM(CHID),       '_03.s3d.sz'

   LU_SMOKE3D(8,NM) = -GET_FILE_NUMBER() ! size file unit number for LU_SMOKE3D(4,NM)
   IF (RESTART) LU_SMOKE3D(8,NM) = ABS(LU_SMOKE3D(8,NM))
   IF (NMESHES >1) WRITE(FN_SMOKE3D(8,NM),'(A,A,I4.4,A)') TRIM(CHID),'_',NM,'_04.s3d.sz'
   IF (NMESHES==1) WRITE(FN_SMOKE3D(8,NM),'(A,A)')        TRIM(CHID),       '_04.s3d.sz'
...
! soot
   ...
   IF (SMOKE3D_QUANTITY_INDEX/=11) THEN
      IF (APPEND) THEN
         ...
         OPEN(ABS(LU_SMOKE3D(5,NM)),FILE=FN_SMOKE3D(5,NM),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
      ELSE
         OPEN(ABS(LU_SMOKE3D(1,NM)),FILE=FN_SMOKE3D(1,NM),FORM='UNFORMATTED',STATUS='REPLACE')
         OPEN(ABS(LU_SMOKE3D(5,NM)),FILE=FN_SMOKE3D(5,NM),FORM='FORMATTED',STATUS='REPLACE')
         IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
         M%N_STRINGS = M%N_STRINGS + 1
...
! hrrpuv
   ...
   IF (APPEND) THEN
      ...
      OPEN(ABS(LU_SMOKE3D(6,NM)),FILE=FN_SMOKE3D(6,NM),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
   ELSE
      OPEN(ABS(LU_SMOKE3D(2,NM)),FILE=FN_SMOKE3D(2,NM),FORM='UNFORMATTED',STATUS='REPLACE')
      OPEN(ABS(LU_SMOKE3D(6,NM)),FILE=FN_SMOKE3D(6,NM),FORM='FORMATTED',STATUS='REPLACE')
      IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A,I6)') 'SMOKF3D',NM
      M%N_STRINGS = M%N_STRINGS + 1
...
! temperature
   ...
   IF (SMOKE3D_CO2) THEN
      IF (APPEND) THEN
         OPEN(ABS(LU_SMOKE3D(3,NM)),FILE=FN_SMOKE3D(3,NM),FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
         OPEN(ABS(LU_SMOKE3D(7,NM)),FILE=FN_SMOKE3D(7,NM),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
      ELSE
         OPEN(ABS(LU_SMOKE3D(3,NM)),FILE=FN_SMOKE3D(3,NM),FORM='UNFORMATTED',STATUS='REPLACE')
         OPEN(ABS(LU_SMOKE3D(7,NM)),FILE=FN_SMOKE3D(7,NM),FORM='FORMATTED',STATUS='REPLACE')
         IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A,I6)') 'SMOKG3D',NM
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(FN_SMOKE3D(3,NM))
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(OUTPUT_QUANTITY(5)%NAME)
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(OUTPUT_QUANTITY(5)%SHORT_NAME)
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(OUTPUT_QUANTITY(5)%UNITS)
      ENDIF
   ENDIF

! co2

   IF (SMOKE3D_CO2.AND.CO2_INDEX>0) THEN
      IF (APPEND) THEN
         OPEN(ABS(LU_SMOKE3D(4,NM)),FILE=FN_SMOKE3D(4,NM),FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
         OPEN(ABS(LU_SMOKE3D(8,NM)),FILE=FN_SMOKE3D(8,NM),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
      ELSE
         OPEN(ABS(LU_SMOKE3D(4,NM)),FILE=FN_SMOKE3D(4,NM),FORM='UNFORMATTED',STATUS='REPLACE')
         OPEN(ABS(LU_SMOKE3D(8,NM)),FILE=FN_SMOKE3D(8,NM),FORM='FORMATTED',STATUS='REPLACE')
         IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(A,I6)') 'SMOKG3D',NM
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(FN_SMOKE3D(4,NM))
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM("CARBON DIOXIDE MASS FRACTION")
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM("Y_CO2")
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM("kg/kg")
      ENDIF
   ENDIF
...
DATA_FILE_LOOP: DO DATA_FILE_TYPE=1,4
   ...
   IF (.NOT.HAVE_SOOT.AND.DATA_FILE_TYPE==1) CYCLE
   IF (.NOT.HAVE_TEMP.AND.DATA_FILE_TYPE==3) CYCLE
   IF (.NOT.HAVE_CO2.AND.DATA_FILE_TYPE==4) CYCLE

   SELECT CASE(DATA_FILE_TYPE)
      CASE(1) ! soot
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
      ...
      CASE(4) ! co2
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1 ! need to make the following line general
                  FF(I,J,K) = RHO(I,J,K)*GAS_PHASE_OUTPUT(I,J,K,90,0,CO2_INDEX,0,0,0,0,0,0,0,T,DT,NM)
               ENDDO
            ENDDO
         ENDDO
   END SELECT
   ...
   CALL SMOKE3D_TO_FILE(LU_SMOKE3D(DATA_FILE_TYPE,NM),LU_SMOKE3D(DATA_FILE_TYPE+4,NM),&
             STIME,DXX,MASS_EXT_COEF,DATA_FILE_TYPE,QQ_PACK,IBP1,JBP1,KBP1,HRRPUV_MAX_SMV)
   DEALLOCATE(QQ_PACK)
