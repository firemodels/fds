...
PUBLIC PRESSURE_SOLVER_COMPUTE_RHS,PRESSURE_SOLVER_FFT,TUNNEL_POISSON_SOLVER,PRESSURE_SOLVER_CHECK_RESIDUALS, &
       COMPUTE_VELOCITY_ERROR,GET_BOUNDARY_PRESSURE
...
SUBROUTINE GET_BOUNDARY_PRESSURE(NM1,NM2,ORIENT_INDEX)

USE POIS, ONLY: H2CZIS,H2CZSS,H3CZIS,H3CZSS
USE GLOBAL_CONSTANTS, ONLY: PREDICTOR,TWO_D,T_USED,POISSON_PATCH_WIDTH
USE COMP_FUNCTIONS, ONLY: CURRENT_TIME
INTEGER, INTENT(IN) :: NM1,NM2,ORIENT_INDEX
TYPE(MESH_TYPE), POINTER :: M1,M2
REAL(EB), POINTER, DIMENSION(:,:,:) :: HP1,HP2
INTEGER :: IERR,FACE_INDEX
TYPE PETIT_POISSON_TYPE
   REAL(EB) :: XLM=0._EB,POIS_PERTURBATION,XS,XF,YS,YF,ZS,ZF
   REAL(EB), ALLOCATABLE, DIMENSION(:) :: SAVE,WORK,HX
   REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: BXS,BXF,BYS,BYF,BZS,BZF
   REAL(EB), ALLOCATABLE, DIMENSION(:,:,:) :: PRHS
   INTEGER :: IBAR,JBAR,KBAR,LSAVE,LWORK,LBC,MBC,NBC,N_LAYERS,NM1=0,NM2=0,IBMNL,JBMNL,KBMNL
   LOGICAL :: FIRST_CALL(6)=.TRUE.
END TYPE PETIT_POISSON_TYPE
TYPE(PETIT_POISSON_TYPE), SAVE, ALLOCATABLE, DIMENSION(:), TARGET :: PETIT_POISSON
TYPE(PETIT_POISSON_TYPE), POINTER :: PP
REAL(EB) :: TNOW

TNOW=CURRENT_TIME()

IF (.NOT.ALLOCATED(PETIT_POISSON)) ALLOCATE(PETIT_POISSON(6))

M1 => MESHES(NM1)
M2 => MESHES(NM2)

DO FACE_INDEX=1,6
   PP => PETIT_POISSON(FACE_INDEX)
   IF (NM1==PP%NM1 .AND. NM2==PP%NM2) EXIT
   IF (PP%NM1==0 .AND. PP%NM2==0) THEN
      PP%NM1 = NM1
      PP%NM2 = NM2
      EXIT
   ENDIF
...
IF (PP%FIRST_CALL(FACE_INDEX)) THEN
   PP%LBC = M1%LBC
   PP%MBC = M1%MBC
   PP%NBC = M1%NBC
   PP%IBAR = M1%IBAR
   PP%JBAR = M1%JBAR
   PP%KBAR = M1%KBAR
   PP%N_LAYERS = POISSON_PATCH_WIDTH
   PP%IBMNL = M1%IBAR - PP%N_LAYERS
   PP%JBMNL = M1%JBAR - PP%N_LAYERS
   PP%KBMNL = M1%KBAR - PP%N_LAYERS
   PP%XS = M1%XS
   PP%XF = M1%XF
   PP%YS = M1%YS
   PP%YF = M1%YF
   PP%ZS = M1%ZS
   PP%ZF = M1%ZF
   SELECT CASE(ORIENT_INDEX)
      CASE(1)
         PP%LBC  = 1
         PP%IBAR = 2*PP%N_LAYERS
         PP%XS = M1%XF - PP%N_LAYERS*M1%DX(M1%IBAR)
         PP%XF = M1%XF + PP%N_LAYERS*M2%DX(1)
      CASE(2)
         PP%MBC  = 1
         PP%JBAR = 2*PP%N_LAYERS
         PP%YS = M1%YF - PP%N_LAYERS*M1%DY(M1%JBAR)
         PP%YF = M1%YF + PP%N_LAYERS*M2%DY(1)
      CASE(3)
         PP%NBC  = 1
         PP%KBAR = 2*PP%N_LAYERS
         PP%ZS = M1%ZF - PP%N_LAYERS*M1%DZ(M1%KBAR)
         PP%ZF = M1%ZF + PP%N_LAYERS*M2%DZ(1)
   END SELECT
   PP%LSAVE = (PP%IBAR+1)*PP%JBAR*PP%KBAR+7*PP%IBAR+5*PP%JBAR+6*PP%KBAR+56
   PP%LWORK = (PP%IBAR+1)*PP%JBAR*PP%KBAR
   ALLOCATE(PP%HX(0:PP%IBAR+1)) ; PP%HX = 1._EB
   ALLOCATE(PP%SAVE(-3:PP%LSAVE))
   ALLOCATE(PP%WORK(   PP%LWORK))
   IF (TWO_D) THEN
      CALL H2CZIS(PP%XS,PP%XF,PP%IBAR,PP%LBC,PP%ZS,PP%ZF,PP%KBAR,PP%NBC,PP%HX,PP%XLM,PP%IBAR,IERR,PP%SAVE)
   ELSE
      CALL H3CZIS(PP%XS,PP%XF,PP%IBAR,PP%LBC,PP%YS,PP%YF,PP%JBAR,PP%MBC,PP%ZS,PP%ZF,PP%KBAR,PP%NBC, &
                  PP%HX,PP%XLM,PP%IBAR,PP%JBAR,IERR,PP%SAVE)
   ENDIF
   ALLOCATE(PP%BXS(PP%JBAR,PP%KBAR))
   ALLOCATE(PP%BXF(PP%JBAR,PP%KBAR))
   ALLOCATE(PP%BYS(PP%IBAR,PP%KBAR))
   ALLOCATE(PP%BYF(PP%IBAR,PP%KBAR))
   ALLOCATE(PP%BZS(PP%IBAR,PP%JBAR))
   ALLOCATE(PP%BZF(PP%IBAR,PP%JBAR))
   ALLOCATE(PP%PRHS(PP%IBAR,PP%JBAR,PP%KBAR))
   PP%FIRST_CALL(FACE_INDEX) = .FALSE.
ENDIF

SELECT CASE(ORIENT_INDEX)
   CASE(1) 
      PP%PRHS(          1:PP%IBAR/2,1:PP%JBAR,1:PP%KBAR) = M1%PRHS(PP%IBMNL+1:M1%IBAR    ,1:M1%JBAR,1:M1%KBAR)
      PP%PRHS(PP%IBAR/2+1:PP%IBAR  ,1:PP%JBAR,1:PP%KBAR) = M2%PRHS(1         :PP%N_LAYERS,1:M1%JBAR,1:M1%KBAR)
      PP%BXS(1:PP%JBAR,1:PP%KBAR) = 0.5_EB*(HP1(PP%IBMNL   ,1:M1%JBAR,1:M1%KBAR)+HP1(PP%IBMNL+1   ,1:M1%JBAR,1:M1%KBAR))
      PP%BXF(1:PP%JBAR,1:PP%KBAR) = 0.5_EB*(HP2(PP%N_LAYERS,1:M1%JBAR,1:M1%KBAR)+HP2(PP%N_LAYERS+1,1:M1%JBAR,1:M1%KBAR))
      PP%BYS(1:PP%IBAR/2        ,1:PP%KBAR) = M1%BYS(PP%IBMNL+1:M1%IBAR    ,1:M1%KBAR)
      PP%BYS(PP%IBAR/2+1:PP%IBAR,1:PP%KBAR) = M2%BYS(         1:PP%N_LAYERS,1:M2%KBAR)
      PP%BYF(1:PP%IBAR/2        ,1:PP%KBAR) = M1%BYF(PP%IBMNL+1:M1%IBAR    ,1:M1%KBAR)
      PP%BYF(PP%IBAR/2+1:PP%IBAR,1:PP%KBAR) = M2%BYF(         1:PP%N_LAYERS,1:M2%KBAR)
      PP%BZS(1:PP%IBAR/2        ,1:PP%JBAR) = M1%BZS(PP%IBMNL+1:M1%IBAR    ,1:M1%JBAR)
      PP%BZS(PP%IBAR/2+1:PP%IBAR,1:PP%JBAR) = M2%BZS(         1:PP%N_LAYERS,1:M2%JBAR)
      PP%BZF(1:PP%IBAR/2        ,1:PP%JBAR) = M1%BZF(PP%IBMNL+1:M1%IBAR    ,1:M1%JBAR)
      PP%BZF(PP%IBAR/2+1:PP%IBAR,1:PP%JBAR) = M2%BZF(         1:PP%N_LAYERS,1:M2%JBAR)
   CASE(2) 
      PP%PRHS(1:PP%IBAR,          1:PP%JBAR/2,1:PP%KBAR) = M1%PRHS(1:M1%IBAR,PP%JBMNL+1:M1%JBAR    ,1:M1%KBAR)
      PP%PRHS(1:PP%IBAR,PP%JBAR/2+1:PP%JBAR  ,1:PP%KBAR) = M2%PRHS(1:M1%IBAR,1         :PP%N_LAYERS,1:M1%KBAR)
      PP%BXS(1:PP%JBAR/2        ,1:PP%KBAR) = M1%BXS(PP%JBMNL+1:M1%JBAR    ,1:M1%KBAR)
      PP%BXS(PP%JBAR/2+1:PP%JBAR,1:PP%KBAR) = M2%BXS(         1:PP%N_LAYERS,1:M2%KBAR)
      PP%BXF(1:PP%JBAR/2        ,1:PP%KBAR) = M1%BXF(PP%JBMNL+1:M1%JBAR    ,1:M1%KBAR)
      PP%BXF(PP%JBAR/2+1:PP%JBAR,1:PP%KBAR) = M2%BXF(         1:PP%N_LAYERS,1:M2%KBAR)
      PP%BYS(1:PP%IBAR,1:PP%KBAR) = 0.5_EB*(HP1(1:M1%IBAR,PP%JBMNL   ,1:M1%KBAR)+HP1(1:M1%IBAR,PP%JBMNL+1   ,1:M1%KBAR))
      PP%BYF(1:PP%IBAR,1:PP%KBAR) = 0.5_EB*(HP2(1:M1%IBAR,PP%N_LAYERS,1:M1%KBAR)+HP2(1:M1%IBAR,PP%N_LAYERS+1,1:M1%KBAR))
      PP%BZS(1:PP%IBAR,1:PP%JBAR/2        ) = M1%BZS(1:M1%IBAR,PP%JBMNL+1:M1%JBAR    )
      PP%BZS(1:PP%IBAR,PP%JBAR/2+1:PP%JBAR) = M2%BZS(1:M1%IBAR,         1:PP%N_LAYERS)
      PP%BZF(1:PP%IBAR,1:PP%JBAR/2        ) = M1%BZF(1:M1%IBAR,PP%JBMNL+1:M1%JBAR    )
      PP%BZF(1:PP%IBAR,PP%JBAR/2+1:PP%JBAR) = M2%BZF(1:M1%IBAR,         1:PP%N_LAYERS)
   CASE(3) 
      PP%PRHS(1:PP%IBAR,1:PP%JBAR,          1:PP%KBAR/2) = M1%PRHS(1:M1%IBAR,1:M1%JBAR,PP%KBMNL+1:M1%KBAR    )
      PP%PRHS(1:PP%IBAR,1:PP%JBAR,PP%KBAR/2+1:PP%KBAR)   = M2%PRHS(1:M1%IBAR,1:M1%JBAR,1         :PP%N_LAYERS)
      PP%BXS(1:PP%JBAR,1:PP%KBAR/2        ) = M1%BXS(1:M1%JBAR,PP%KBMNL+1:M1%KBAR    )
      PP%BXS(1:PP%JBAR,PP%KBAR/2+1:PP%KBAR) = M2%BXS(1:M1%JBAR,         1:PP%N_LAYERS)
      PP%BXF(1:PP%JBAR,1:PP%KBAR/2        ) = M1%BXF(1:M1%JBAR,PP%KBMNL+1:M1%KBAR    )
      PP%BXF(1:PP%JBAR,PP%KBAR/2+1:PP%KBAR) = M2%BXF(1:M1%JBAR,         1:PP%N_LAYERS)
      PP%BYS(1:PP%IBAR,1:PP%KBAR/2        ) = M1%BYS(1:M1%IBAR,PP%KBMNL+1:M1%KBAR    )
      PP%BYS(1:PP%IBAR,PP%KBAR/2+1:PP%KBAR) = M2%BYS(1:M1%IBAR,         1:PP%N_LAYERS)
      PP%BYF(1:PP%IBAR,1:PP%KBAR/2        ) = M1%BYF(1:M1%IBAR,PP%KBMNL+1:M1%KBAR    )
      PP%BYF(1:PP%IBAR,PP%KBAR/2+1:PP%KBAR) = M2%BYF(1:M1%IBAR,         1:PP%N_LAYERS)
      PP%BZS(1:PP%IBAR,1:PP%JBAR) = 0.5_EB*(HP1(1:M1%IBAR,1:M1%JBAR,PP%KBMNL   )+HP1(1:M1%IBAR,1:M1%JBAR,PP%KBMNL+1   ))
      PP%BZF(1:PP%IBAR,1:PP%JBAR) = 0.5_EB*(HP2(1:M1%IBAR,1:M1%JBAR,PP%N_LAYERS)+HP2(1:M1%IBAR,1:M1%JBAR,PP%N_LAYERS+1))
END SELECT

IF (TWO_D) THEN
   CALL H2CZSS(PP%BXS,PP%BXF,PP%BZS,PP%BZF,PP%IBAR,PP%PRHS,PP%POIS_PERTURBATION,PP%SAVE,PP%WORK,PP%HX)
ELSE
   CALL H3CZSS(PP%BXS,PP%BXF,PP%BYS,PP%BYF,PP%BZS,PP%BZF,PP%IBAR,PP%JBAR,PP%PRHS,PP%POIS_PERTURBATION,PP%SAVE,PP%WORK,PP%HX)
ENDIF

SELECT CASE(ORIENT_INDEX)
   CASE(1) 
      M1%BXF(1:M1%JBAR,1:M1%KBAR) = 0.5_EB*(PP%PRHS(PP%N_LAYERS,1:PP%JBAR,1:PP%KBAR)+PP%PRHS(PP%N_LAYERS+1,1:PP%JBAR,1:PP%KBAR))
      M2%BXS(1:M1%JBAR,1:M1%KBAR) = 0.5_EB*(PP%PRHS(PP%N_LAYERS,1:PP%JBAR,1:PP%KBAR)+PP%PRHS(PP%N_LAYERS+1,1:PP%JBAR,1:PP%KBAR))
   CASE(2) 
      M1%BYF(1:M1%IBAR,1:M1%KBAR) = 0.5_EB*(PP%PRHS(1:PP%IBAR,PP%N_LAYERS,1:PP%KBAR)+PP%PRHS(1:PP%IBAR,PP%N_LAYERS+1,1:PP%KBAR))
      M2%BYS(1:M1%IBAR,1:M1%KBAR) = 0.5_EB*(PP%PRHS(1:PP%IBAR,PP%N_LAYERS,1:PP%KBAR)+PP%PRHS(1:PP%IBAR,PP%N_LAYERS+1,1:PP%KBAR))
   CASE(3) 
      M1%BZF(1:M1%IBAR,1:M1%JBAR) = 0.5_EB*(PP%PRHS(1:PP%IBAR,1:PP%JBAR,PP%N_LAYERS)+PP%PRHS(1:PP%IBAR,1:PP%JBAR,PP%N_LAYERS+1))
      M2%BZS(1:M1%IBAR,1:M1%JBAR) = 0.5_EB*(PP%PRHS(1:PP%IBAR,1:PP%JBAR,PP%N_LAYERS)+PP%PRHS(1:PP%IBAR,1:PP%JBAR,PP%N_LAYERS+1))
END SELECT

T_USED(12)=T_USED(12)+CURRENT_TIME()-TNOW
END SUBROUTINE GET_BOUNDARY_PRESSURE
...
IF (ITERATE_BAROCLINIC_TERM .OR. INSEPARABLE_POISSON) THEN
   ...
   P = RHOP*(HP-KRES)
   RESIDUAL => WORK8(1:IBAR,1:JBAR,1:KBAR)
   !$OMP PARALLEL PRIVATE(I,J,K,RHSS,LHSS)
...
INTEGER, PARAMETER :: NNZ_STENCIL_H =  7 ! 7 Point stencil in 3D.
...
USE CC_SCALARS_IBM, ONLY : GET_H_CUTFACES
INTEGER, INTENT(IN) :: NM
...
ZONE_MESH_LOOP_2: DO IPZ=0,N_ZONE
   ...
   DO ICC=1,M%N_CUTCELL_MESH
      ...
      IROW                  = IROW + 1
      ZM%MESH_IJK(1:3,IROW) = (/ I, J, K/)
      CUT_CELL(ICC)%UNKH(1:CUT_CELL(ICC)%NCELL) = IROW
   ENDDO
...
ZONE_MESH_LOOP_4: DO IPZ=0,N_ZONE
   ZM=>MESHES(NM)%ZONE_MESH(IPZ)
   IF (.NOT.ZM%ZONE_IN_MESH) CYCLE ZONE_MESH_LOOP_4

   ! 3.c Per pressure zone add PRES_ZONE value to REG, RC and cut-faces, etc.
...
USE CC_SCALARS_IBM, ONLY : GET_PRES_CFACE_BCS
...
! Local Variables:
INTEGER :: NRHS,MAXFCT,MNUM,ERROR,I,J,K,ICC,JCC,IIG,JJG,KKG,IOR,IW,IROW,NCELL,ICFACE,IFACE,JFACE
...
IF (.NOT.PRES_ON_CARTESIAN) THEN
   DO IROW=ZM%NUNKH_CART+1,ZM%NUNKH ! Cut-cells.
      I=ZM%MESH_IJK(IAXIS,IROW); J=ZM%MESH_IJK(JAXIS,IROW); K=ZM%MESH_IJK(KAXIS,IROW)
      ...
      ENDDO
      ! Add to F_H:
      ZM%F_H(IROW) = -(CUT_CELL(ICC)%DDDTVOL + DIV_FN_VOL)
   ENDDO
ELSE
   DO IROW=ZM%NUNKH_CART+1,ZM%NUNKH ! Cut-cells.
      I=ZM%MESH_IJK(IAXIS,IROW); J=ZM%MESH_IJK(JAXIS,IROW); K=ZM%MESH_IJK(KAXIS,IROW)
...
WALL_CELL_LOOP_1 : DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   IF(PRESSURE_ZONE(IIG,JJG,KKG)/=IPZ) CYCLE WALL_CELL_LOOP_1
   ! NEUMANN boundaries:
...
H_INDEFINITE_IF_1 : IF (ZM%MTYPE==SYMM_INDEFINITE ) THEN
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (MUNKH(I,J,K)<=0 .OR. PRESSURE_ZONE(I,J,K)/=IPZ) CYCLE ! Gasphase Cartesian cells.
   ...
   DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
      I = CUT_CELL(ICC)%IJK(IAXIS); J = CUT_CELL(ICC)%IJK(JAXIS); K = CUT_CELL(ICC)%IJK(KAXIS)
      IF (CUT_CELL(ICC)%UNKH(1)<=0 .OR. PRESSURE_ZONE(I,J,K)/=IPZ) CYCLE
...
WALL_CELL_LOOP_2 : DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   ! Gasphase cell indexes:
   IIG = BC%IIG; JJG = BC%JJG; KKG = BC%KKG; IOR = BC%IOR
   IF(PRESSURE_ZONE(IIG,JJG,KKG)/=IPZ) CYCLE WALL_CELL_LOOP_2
   ! NEUMANN boundaries:
...
DO X1AXIS=IAXIS,KAXIS
   ...
   DO IFACE=1,MESHES(NM)%NREGFACE_H(X1AXIS)
      ...
      IF(PRESSURE_ZONE(I,J,K)==IPZ) REGFACE_H(IFACE)%PRES_ZONE=IPZ
   ENDDO
ENDDO
...
DO IFACE=1,MESHES(NM)%IBM_NRCFACE_H
   ...
   IF(PRESSURE_ZONE(I,J,K)==IPZ) MESHES(NM)%IBM_RCFACE_H(IFACE)%PRES_ZONE=IPZ
ENDDO
...
DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
   ...
   IF(PRESSURE_ZONE(I,J,K)==IPZ) CUT_FACE(ICF)%PRES_ZONE=IPZ
ENDDO
...
USE CC_SCALARS_IBM, ONLY : GRADH_ON_CARTESIAN
...
IF(CHECK_POISSON) THEN
   MSGLVL = 1
   IF(MY_RANK==0) WRITE(LU_ERR,*) 'ULMAT : PARDISO factorization for MESH,ZONE=',NM,IPZ
ENDIF
ERROR     = 0 ! initialize error flag
