...
\be \nabla^2{\cal H}^{(n+1)_e} = - \left[ \frac{ (\nabla \cdot \bu)^{(n+1)_e} -
  (\nabla \cdot \bu)^n }{\dt} \right] - \nabla \cdot \bF^n  \ee
...
\be {\cal H}_{nx+1,jk} = 2 {\cal H}_{nx+\ha,jk} - {\cal H}_{nx,jk} \ee


\subsubsection{Multiple Mesh Considerations (On-Going Research)}

Here we provide an overview of the ``pressure correction'' scheme used in FDS to enforce global volume conservation in multiple mesh (i.e. parallel) computations (a ``mesh'' refers to the computational domain which is mapped to an individual processor).  There are three critical components of the scheme which we discuss in further detail below: (1) the averaging of the normal velocity component at a mesh interface, (2) the Dirichlet boundary value for the hydrodynamic pressure, and (3) the Neumann boundary condition for the pressure correction.  The term ``pressure correction'' actually best identifies only the third component of the scheme which is ultimately responsible for enforcing volume conservation from mesh to mesh (and thus globally).  However, the first two components of the scheme are perhaps even more important for maintaining stability of the solution.

The basic idea behind the scheme is the following.  First, we break up the hydrodynamic pressure $\cH$ into a fluctuating component $\cH^\prime$ and a small correction $\cal H^{\prime\prime}$.  We may then write the momentum equation as
\begin{equation}
\label{eqn_mom1}
\frac{\partial \mathbf{u}}{\partial t} = - \left[ \mathbf{F} + \nabla \cH^\prime + \nabla \cH^{\prime\prime} \right] \,\mbox{.}
\end{equation}
...
\begin{equation}
\label{eqn_poisson}
\nabla^2 \cH^\prime = - \left[ \frac{\partial}{\partial t} \left(\nabla\cdot \mathbf{u}\right) + \nabla\cdot \mathbf{F} \right] \,\mbox{,}
\end{equation}
and the pressure correction satisfies Laplace's equation,
\begin{equation}
\label{eqn_laplace}
\nabla^2 \cH^{\prime\prime} = 0 \,\mbox{.}
\end{equation}

Now, as opposed to conventional domain decomposition techniques where boundary conditions are prescribed only along the global boundary of the problem domain and the PDEs (\ref{eqn_poisson}) and (\ref{eqn_laplace}) are supposed to apply across mesh interfaces (a.k.a. patch boundaries), in the FDS pressure correction scheme we impose a Dirichlet condition on $\cH^\prime$ and we impose a Neumann condition on $\cH^{\prime\prime}$ at patch boundaries.  The Dirichlet condition for the pressure fluctuation is designed to provide stability and accuracy and the Neumann condition for the (ideally small) pressure correction field is designed to enforce volume conservation from mesh to mesh.  Once these boundary conditions are prescribed, calculations on each mesh may proceed independently, resulting in an algorithm which is embarrassingly parallel.  Note that this scheme may be viewed as a single iteration in an additive Schwarz method.

The time advancement scheme for multiple meshes must now include averaging of the velocity components at a patch boundary in order to obey angular momentum conservation and hence maintain stability. It is extremely important to note that -- because of the staggered grid storage arrangement -- two values of the numerical solution for the velocity exist at a given point on a mesh interface.  Consider meshes $m$ (left) and $om$ (right) which are joined side by side in the $x$ direction.  The values $u^{(m)}_{nx,jk}$ and $u^{(om)}_{0,jk}$ live at the same physical location, but are advanced separately (and by different processors) during the course of the algorithm.  Obviously, in the ideal circumstance these velocities are identical, as is the case if the PDEs (\ref{eqn_poisson}) and (\ref{eqn_laplace}) are enforced at the mesh interface.  To improve the speed of our parallel algorithm, our strategy is instead to keep the difference between these values small while still enforcing global volume conservation.  Our strategy recovers the ideal case in the limit $\delta t \rightarrow 0$.  In fact, we observe that for reasonable time steps (CFL $\le$ 1) the difference between the two coincident velocities is extremely small.

While the primitive variables $\mathbf{u}$ are indeed unique to a given mesh, for each mesh we may define the discrete ``patch-averaged'' field $\bar{\mathbf{u}}$ which includes ghost cell data and is identical at all overlapping mesh points.  To do this we simply average the coincident values of the velocity components at the mesh interfaces and inject the ghost cell values from the neighboring mesh.  For instance, considering the same side-by-side meshes $m$ and $om$ as before,
\begin{equation}
\label{eqn_patchave_ufield}
\bar{u}^{(m)}_{ijk} \equiv \left\{ \begin{array}{ll} u^{(m)}_{ijk} & \mbox{for}\quad 0<i<nx^{(m)} \vspace{0.2cm}\\
\frac{1}{2} \left[ u^{(m)}_{nx^{(m)},jk} + u^{(om)}_{0,jk} \right] & \mbox{for}\quad i=nx^{(m)} \vspace{0.2cm}\\
u^{(om)}_{1,jk} & \mbox{for}\quad i=nx^{(m)}+1 \end{array} \right.
\end{equation}
for all patch boundary cells $j$ and $k$. The values of $\bar{u}^{(m)}$ for the index ranges not considered in (\ref{eqn_patchave_ufield}) depend on the specific boundary conditions for the other faces of mesh $m$.  The same ideas are used to define the patch-averaged fields for $\bar{v}$ and $\bar{w}$.

To illustrate how the new the patch-averaged fields are used, we consider the predictor step in the time advancement, which may now be written as
\begin{equation}
\label{eqn_RK1}
\mathbf{u}^{(n+1)_e} = \bar{\mathbf{u}}^n - \delta t \left[ \mathbf{F}(\rho^n,\bar{\mathbf{u}}^n) + {\nabla\cH^\prime}^{(n+1)_e} + {\nabla\cH^{\prime\prime}}^{(n+1)_e} \right] \,\mbox{,}
\end{equation}
Note that (\ref{eqn_RK1}) updates the $\bar{\mathbf{u}}$ field to a $\mathbf{u}$ field.  Though this is perfectly legitimate, we should be aware of the following issues.  First, the patch-averaging procedure is slightly dissipative.  But as long as we are accurate, this numerical dissipation is minimal and acceptable (much smaller than physical dissipation).  Second, we must now think of the time update in terms of a projection scheme in order to repair the damage made to the divergence by the averaging procedure. In practical terms, this means that the Poisson equation for the pressure fluctuation in the predictor step, for example, is given by
\begin{equation}
\label{eqn_poisson_stg1}
\nabla^2 {\cH^\prime}^{(n+1)_e} = -\left[\frac{ (\nabla\cdot\mathbf{u})^{(n+1)_e} - (\nabla\cdot\bar{\mathbf{u}})^n}{\delta t}\right] -  \mathbf{F}(\rho^n,\bar{\mathbf{u}}^n) \,\mbox{.}
\end{equation}
In (\ref{eqn_poisson_stg1}), only the term $(\nabla\cdot\mathbf{u})^{(n+1)_e}$ is obtained from the equation of state.  The divergence of the patch-averaged field, $(\nabla\cdot\bar{\mathbf{u}})^n$, is obtained explicitly.  Similarly, in the corrector Poisson equation analogous to (\ref{eqn_corrector_poisson}), only the $(\nabla\cdot\mathbf{u})^{n+1}$ term is obtained from the equation of state; other divergences are obtained explicitly:
\begin{equation}
\label{eqn_poisson_stg2}
\nabla^2 {\cH^\prime}^{\,n+1} = -\left[\frac{ 2(\nabla\cdot\mathbf{u})^{n+1} - (\nabla\cdot\bar{\mathbf{u}})^{(n+1)_e} - (\nabla\cdot\bar{\mathbf{u}})^n}{\delta t}\right] -  \mathbf{F}(\rho^{(n+1)_e},\bar{\mathbf{u}}^{(n+1)_e}) \,\mbox{,}
\end{equation}
Finally, the velocity corrector step is
\be
\bu^{n+1} = \ha \left[ \bar{\bu}^n + \bar{\bu}^{(n+1)_e} - \dt \left( \mathbf{F}[\rho^{(n+1)_e},\bar{\mathbf{u}}^{(n+1)_e}] + \nabla{\cH^\prime}^{\,n+1} + \nabla{\cH^{\prime\prime}}^{\,n+1}\right) \right] \,\mbox{.}
\ee
The benefit of the averaging procedure is that, since all force terms are determined using the patch-averaged field, for a given location along a mesh interface $\mathbf{F}$ is the same for each mesh.  This also means that stress tensors computed at a mesh interface will be symmetric which is a requirement for angular momentum conservation.

At the interface of two abutting meshes the following Dirichlet boundary condition is applied (again, mesh $m$ is to the left of mesh $om$ in this example):
\be \cH_{nx+\ha,jk}^{(m)} = \cH_{1-\ha,jk}^{(om)} = \frac{1}{2}\left[ \cH_{nx,jk}^{(m),*} + \cH_{1,jk}^{(om),*} \right]  \ee
Again, the asterisk denotes the most recent value of the pressure.  The bracketed superscript indicates the mesh index.  Note that the pressure boundary values are identical for each mesh at the interface.  To a good approximation, this boundary condition maintains continuity of the pressure field from mesh to mesh without invoking an iterative procedure.  Roughly speaking, here we are committing the same level of error as we commit by using (\ref{sbc}) at solid obstacles within the mesh domain.

To get the boundary conditions for the Laplace equation, we first solve a discretized form of the Poisson equation for the entire domain using the individual
meshes themselves. This is much like the strategy adopted by zone and lumped parameter models, which assume that each compartment in a building has a uniform pressure found
by solving a system of equations based on mass balances at compartment boundaries.
This coarse Poisson equation is derived by integrating Eq.~(\ref{finepois}) over each mesh and applying the Divergence Theorem:
\be \int_{\partial \Omega_m} \nabla \cH' \cdot \dS = -\int_{\partial \Omega_m} \overline{\bG} \cdot \dS
    - \int_{\Omega_m} \dod{(\nabla \cdot \bu)}{t} \; dV  \quad ; \quad \bG = \bF + \nabla \cH^0  \label{poisdiv} \ee
The subscript $m$ refers to a given mesh. The overbar indicates that at mesh boundaries, the average of the quantity from the
abutting meshes is to be used. Next, introduce a single scalar quantity, $\widehat{\cH'}_m$, for each mesh such that at each mesh boundary
\be \int \nabla \cH' \cdot \dS = \frac{\widehat{\cH'}_{m_2}  - \widehat{\cH'}_{m_1} }{L_{12}} A_{12} \ee
where $L_{12}$ is the distance between the mesh centers in the coordinate direction normal to the mesh boundary, and $A_{12}$ is the area of the boundary. In discretized form,
Eq.~(\ref{poisdiv}) forms a sparse linear system of equations for $\widehat{\cH'}$ that can be solved
with a direct (non-iterative) solver.

Following the solution of the coarse pressure equation, at the interface between two
neighboring meshes, we now have an estimate of the rate of change of the volume flux. For example, assume that meshes 1 and 2 abut at a plane $x=x_0$, and that mesh 2 is
to the positive side of the interface. Then the change in volume flow with time is:
\be \frac{d\dot{V}_{12}}{dt}  = -\int_{x=x_0} \overline{G}_x \, dy \, dz
- \frac{\widehat{\cH'}_{m_2} - \widehat{\cH'}_{m_1}}{L_{12}} A_{12} \ee
...
such that at the interface the gradient of $\cH'$ is used to add or subtract the necessary volume flow to enforce conservation mesh to mesh.
For example, at the $x=x_0$ interface, the gradient of
$\cH'$ at the boundary of each mesh is assumed constant and chosen so that:
\be  \frac{d\dot{V}}{dt} =
    - \int_{x=x_0} G_x \; dy \, dz -  \int_{x=x_0} \dod{\cH'}{x} \; dy \, dz \label{constraint} \ee
In this way, the resulting volume flow at the next time step $\dot{V}=\int \bu \cdot \dS$ into and
...
\frac{P_0(\theta_d)}{(1-\mu^2)(1-\mu'^2)-(\mu_d-\mu\mu')2 }
\; d\mu_d \, d\mu \, d\mu'
\ee
