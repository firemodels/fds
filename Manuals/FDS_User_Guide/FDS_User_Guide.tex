...
\subsubsection{Example: A Single-Step, Mixing-Controlled Reaction with Deposition}

This example shows how to define a reaction that invokes an aerosol deposition model in FDS (e.g. soot deposition).  Note, that this model should be considered an expiremental feature. The deposition model is invoked by defining a species with the parameter {\ct AEROSOL=.TRUE.}.  For this example propane will be the fuel with a 5 \% soot yield.  The stoichometric equation is shown below.
...
In this mixing-controlled example, the user is responsible for defining the fuel, heat of combustion of that fuel, species participating the reaction, and the stoichiometric coefficients that describe the reaction. The heat of combustion for PRPOANE was found from the SFPE Handbook \cite{SFPE:Tewarson}. In the lumped species approach, the stoichiometric coefficients for the {\ct SMIX}s are the coefficients of the lumped species themselves.
\footnotesize
\begin{verbatim}
...
\begin{figure}[ht]
\centering \includegraphics[width=3.2in]{SCRIPT_FIGURES/Propane_flame_deposition}
...
{\ct PID}                         & PID Controller, see ~\ref{info:PID} \\ \hline
\end{tabular}
\end{center}
\end{table}
...
\subsection{PID Control Function: \texorpdfstring{{\tt PID}}{PID}}

\label{info:PID}

A common process control function is a PID controller (Proportional-Integral-Derivatve).  This control function is evaluated as:

\be
output(t) = K_p error(t) + K_i \int_0^t error(t) \,\mbox{d}t + K_d \frac{d error(t)}{dt}
\ee

Where K$_p$ is the {\ct PROPORTIONAL\_GAIN}, K$_i$ is the {\ct INTEGRAL\_GAIN}, and K$_d$ is the {\ct DIFFERENTIAL\_GAIN}.  The error is the difference between the input to the control function and it's {\ct TARGET\_VALUE}.
...
For any user defined {\ct RAMP}, the normal independent variable, for example time for {\ct RAMP\_V}, can be replaced by the output of a {\ct DEVC} or a {\ct CTRL}. This is done by specifying the input {\ct DEVC\_ID} or {\ct CTRL\_ID} one on of the
{\ct RAMP} input lines.  When this is done, the current output of the {\ct DEVC} or
{\ct CTRL} is used, see \ref{info:basic_control} as the independent variable for the {\ct RAMP}.
...
{\ct DIFFERENTIAL\_GAIN} & Real         & Section~\ref{info:basic_control}        &    &  0.                    \\ \hline
...
{\ct INTEGRAL\_GAIN}     & Real         & Section~\ref{info:basic_control}        &    &  0.                    \\ \hline
...
{\ct PROPORTIONAL\_GAIN} & Real         & Section~\ref{info:basic_control}        &    &  1.                    \\ \hline
{\ct RAMP\_ID}           & Character    & Section~\ref{info:CUSTOM}               &    &                        \\ \hline
{\ct SETPOINT(2)}        & Real         & Section~\ref{info:basic_control}        &    &                        \\ \hline
{\ct TARGET\_VALUE}      & Real         & Section~\ref{info:basic_control}        &    &  1.                    \\ \hline
...
%Undocumented: HRRPUVCUT_MAX, HRRPUV_MAX_SMV, NEW_EVAP, TENSOR_DIFFUSIVIRY,  TERRAIN_IMAGE, THERMOPHORETIC_DEPOSITION, TURBULENT_DEPOSITION, VEG_LEVEL_SET,  , WFDS
...
\vspace{\baselineskip}
% undefined AEROSOL,CONDUCTIVITY_SOLID,DENSITY_SOLID,MEAN_DIAMETER for testing particle deposition
\newpage
