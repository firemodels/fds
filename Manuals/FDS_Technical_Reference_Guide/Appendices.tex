...
In FDS, the particle position is advanced by a time step increment, $\Delta t_{\rm p}$, which is a fraction of the gas phase time step, $\Delta t$, determined so as to ensure that the particle does not travel more than the width of a grid cell in one sub-time step:
\be
   \Delta t_{\rm p} = \frac{\Delta t}{\lceil 0.9 \, \hbox{CFL} \rceil } \quad ; \quad \hbox{CFL} = \Delta t \max \left( \frac{|u_{\rm p}|}{\dx},\frac{|v_{\rm p}|}{\dy},\frac{|w_{\rm p}|}{\dz} \right)
\ee 
Note that $\lceil \hbox{CFL} \rceil$ denotes the ceiling operator.
\begin{align}
    \mathbf{u}^{n+1} &= \frac{\mathbf{u}^n}{1 + \beta_{\rm p} \Delta t_{\rm p}} + \frac{(\mathbf{u}^n + \alpha_{\rm p} \mathbf{U}^n)\beta_{\rm p} \Delta t_{\rm p}}{(1 + \beta_{\rm p} \Delta t_{\rm p})(1 + \alpha_{\rm p})} + \mathbf{g} \Delta t_{\rm p} - \frac{\alpha_{\rm p} \beta_{\rm p} (\Delta t_{\rm p})^2}{2 (1 + \alpha_{\rm p})} \left[\mathbf{u}_\text{r}^n \left(\frac{\mathbf{u}_\text{r}^n \cdot \mathbf{g}}{|\mathbf{u}_\text{r}^n|^2}\right) + \mathbf{g}\right] \\[.1in]
    \mathbf{x}^{n+1} &= \mathbf{x}^n + \left(\frac{\mathbf{u}^n + \alpha_{\rm p} \mathbf{U}^n}{1 + \alpha_{\rm p}}\right) \Delta t + \frac{\alpha_{\rm p} (\mathbf{u}^n - \mathbf{U}^n)}{\beta_{\rm p} (1 + \alpha_{\rm p})} \text{ln}(\beta_{\rm p} \Delta t_{\rm p} + 1) + \frac{\mathbf{g} (\Delta t_{\rm p})^2}{2} \\[.1in]
...
\end{align}
At first glance, the theoretical accuracy of the original equation for $\mathbf{x}^{n+1}$, Eq.~(\ref{mom_x_p_full}), appears to be $\mathit{O}(\Delta t_{\rm p}^3)$. But computation reveals it is actually $\mathit{O}(\Delta t_{\rm p}^2)$. This is due to the velocity error reducing the overall accuracy of the position solution. The $(\Delta t_{\rm p})^3$ term, therefore, may be dropped from Eq.~(\ref{mom_x_p_full}) without loss of accuracy.
