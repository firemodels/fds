...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,FVEC,PRESSURE_RELAX_FACTOR,RELAXATION_FACTOR,FYI, &
                ...
                CHECK_KINETIC_ENERGY,PP_BC_X,PP_BC_Y,PP_BC_Z,PROJECTION,CHECK_MU_DNS
...
T_LOOP_1: DO J=1,5000
   ...
   IF(MIXTURE_FRACTION) THEN
      ...
      H_TMP(FUEL_INDEX)   = 247.5E3_EB !data from Reynolds
      H_TMP(O2_INDEX)   = 182.4E3_EB
      H_TMP(N2_INDEX)   = 149.3E3_EB
      H_TMP(OTHER_INDEX)   = 149.3E3_EB
      H_TMP(H2O_INDEX)   = 1949.4E3_EB
      H_TMP(CO2_INDEX)   = 174.6E3_EB
      H_TMP(CO_INDEX)   = 1.620E3_EB
      H_TMP(H2_INDEX)   = 0._EB
   ...
   SPECIES_LOOP_1: DO N = 0,N_SPECIES
      ...
      SELECT CASE(SS%ID)
         ...
         CASE('AIR')
            ...
            H_TMP(SS%INDEX) = 0.77_EB*149.3E3_EB+0.23_EB*182.4E3_EB
         CASE('NITROGEN')
            CP_TMP(SS%INDEX) = CP_N2 *1000._EB
            H_TMP(SS%INDEX) = 149.3E3_EB
         CASE('OXYGEN')
            CP_TMP(SS%INDEX) = CP_O2 *1000._EB
            H_TMP(SS%INDEX) = 182.4E3_EB            
         CASE('METHANE')
            CP_TMP(SS%INDEX) = CP_F  *1000._EB
            H_TMP(SS%INDEX) = 247.5E3_EB            
         CASE('CARBON DIOXIDE')
            CP_TMP(SS%INDEX) = CP_CO2*1000._EB
            H_TMP(SS%INDEX) = 174.6E3_EB            
         CASE('WATER VAPOR')
            CP_TMP(SS%INDEX) = CP_H2O*1000._EB
            H_TMP(SS%INDEX) = 1949.4E3_EB            
         CASE('CARBON MONOXIDE')
            CP_TMP(SS%INDEX) = CP_CO*1000._EB
            H_TMP(SS%INDEX) = 1.620E3_EB            
         CASE('HYDROGEN')
   ...
   SPECIES_LOOP_2: DO N=1,N_SPECIES
      ...
      IF (J>1) THEN
         Y2H_G(J,:)   = Y2H_G(J-1,:) +0.5_EB*(Y2CP(J,:)+Y2CP(J-1,:)) 
      ELSE
         Y2H_G(J,:)   = SUM(Y2Y(:,N) * H_TMP(:)) + Y2CP(J,:)    
      ENDIF
      Y2CPBAR(J,:) = Y2H_G(J,:)/REAL(J,EB)      
...
OUTPUT_QUANTITY(52)%NAME = 'MOLECULAR VISCOSITY'
OUTPUT_QUANTITY(52)%UNITS = 'kg/m/s'
OUTPUT_QUANTITY(52)%SHORT_NAME = 'mu_dns'
