...
IF (VIRTUAL_PARTICLES) THEN
   DO IP = 1, NLP
      ...
      IF (LPC%SOLID_PARTICLE) THEN
         ...
         IF (SURFACE(SURF_INDEX)%THERMALLY_THICK .AND. CALL_PYROLYSIS) CALL PYROLYSIS(T,DT_BC,PARTICLE_INDEX=IP)
      ENDIF
   ENDDO
ENDIF
...
USE MATH_FUNCTIONS, ONLY : EVALUATE_RAMP
REAL(EB) :: ZZ_G,UN,DD,MFT,TSI,ZZ_GET(0:N_TRACKED_SPECIES),RSUM_F,MPUA_SUM,T_IGN,AREA_ADJUST,RHO_0
INTEGER :: N,ITER
INTEGER, INTENT(IN), OPTIONAL :: WALL_INDEX,PARTICLE_INDEX
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         ...
         DO ITER=1,5
            ...
            SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
               ...
               WC%ZZ_F(N) = ( ONE_D%MASSFLUX(N) + DD*ZZ_G ) / (DD + UN*WC%RHO_F + TWO_EPSILON_EB)
            ENDDO SPECIES_LOOP
...
USE HVAC_ROUTINES, ONLY : NODE_CP,NODE_TMP,NODE_RHO,DUCT_MF,LEAK_PATH,NODE_ZZ
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%VENT_INDEX > 0 .AND. .NOT. ANY(SF%LEAK_PATH>0._EB)) THEN
      IF (VENTS(WC%VENT_INDEX)%NODE_INDEX > 0) THEN
         ...
         MFT = -DUCTNODE(DN)%DIR(1)*DUCT_MF(DU,1)/VENTS(WC%VENT_INDEX)%FDS_AREA
         IF (DUCTNODE(DN)%DIR(1)*DUCT_MF(DU,1) > 0._EB) THEN
   ...
   ELSE !leakage
      ...
      ENDIF
      IF(DUCT_MF(DU,1) > 0._EB) THEN
         DN=DUCT(DU)%NODE_INDEX(1)
      ELSE
         DN=DUCT(DU)%NODE_INDEX(2)
      ENDIF
      MFT = -DUCTNODE(DN2)%DIR(1)*DUCT_MF(DU,1)/FDS_LEAK_AREA(IZ1,IZ2,1)
   ENDIF
   WC%RHO_F = PBAR_P(KK,WC%PRESSURE_ZONE)/(RSUM_F*WC%ONE_D%TMP_F)
   UN =  -MFT/WC%RHO_F
...
HTC_MODEL_SELECT: SELECT CASE(HT_MODEL)
   CASE DEFAULT
      ...
      END SELECT
      H_FORCED  = NUSSELT*K_Z(ITMP,0)/CONV_LENGTH
   CASE(H_LOGLAW)
      H_NATURAL = 0._EB
      CALL HEAT_FLUX_MODEL(H_FORCED,YPLUS,FRICTION_VELOCITY,K_Z(ITMP,0),RHO_G,CP_Z(ITMP,0),MU_Z(ITMP,0))
   CASE(H_ABL)
      H_NATURAL = 0._EB
      CALL ABL_HEAT_FLUX_MODEL(H_FORCED,FRICTION_VELOCITY,DN,ROUGHNESS,TMP_G,TMP_F,RHO_G,CP_AIR_0)
   CASE(H_EDDY)
      H_NATURAL = 0._EB
      H_FORCED = MU(IIG,JJG,KKG)*CP_Z(ITMP,0)/PR * (2._EB/DN)
   CASE(H_CUSTOM)
      RE = RHO_G*VELCON*CONV_LENGTH/MU_Z(ITMP,0)
