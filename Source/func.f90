...
INTEGER :: N_NEW_STORAGE_SLOTS,OD_INDEX,TD_INDEX,P1_INDEX,P2_INDEX,BR_INDEX,BC_INDEX,I,NAS
...
TYPE (BOUNDARY_PROP1_TYPE), ALLOCATABLE, DIMENSION(:) :: P1_DUMMY
TYPE (BOUNDARY_PROP2_TYPE), ALLOCATABLE, DIMENSION(:) :: P2_DUMMY
...
IF (PRESENT(LP_INDEX)) THEN
   ...
   P1_INDEX = M%LAGRANGIAN_PARTICLE(LP_INDEX)%P1_INDEX
   P2_INDEX = M%LAGRANGIAN_PARTICLE(LP_INDEX)%P2_INDEX
   ...
   M%LAGRANGIAN_PARTICLE(LP_INDEX)%P1_INDEX = P1_INDEX
   M%LAGRANGIAN_PARTICLE(LP_INDEX)%P2_INDEX = P2_INDEX
...
ELSEIF (PRESENT(WALL_INDEX)) THEN
   ...
   TD_INDEX = M%WALL(WALL_INDEX)%TD_INDEX
   P1_INDEX = M%WALL(WALL_INDEX)%P1_INDEX
   P2_INDEX = M%WALL(WALL_INDEX)%P2_INDEX
   ...
   M%WALL(WALL_INDEX)%TD_INDEX   = TD_INDEX
   M%WALL(WALL_INDEX)%P1_INDEX   = P1_INDEX
   M%WALL(WALL_INDEX)%P2_INDEX   = P2_INDEX
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   ENDIF
   BC_INDEX = M%THIN_WALL(THIN_WALL_INDEX)%BC_INDEX
   OD_INDEX = M%THIN_WALL(THIN_WALL_INDEX)%OD_INDEX
   TD_INDEX = M%THIN_WALL(THIN_WALL_INDEX)%TD_INDEX
   P1_INDEX = M%THIN_WALL(THIN_WALL_INDEX)%P1_INDEX
   IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) CALL ALLOCATE_BOUNDARY_COORD_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_PROP1_TYPE) CALL ALLOCATE_BOUNDARY_PROP1_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL ALLOCATE_BOUNDARY_ONE_D_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_THR_D_TYPE) CALL ALLOCATE_BOUNDARY_THR_D_ARRAYS
   M%THIN_WALL(THIN_WALL_INDEX)%THIN_WALL_INDEX = THIN_WALL_INDEX
   M%THIN_WALL(THIN_WALL_INDEX)%BC_INDEX   = BC_INDEX
   M%THIN_WALL(THIN_WALL_INDEX)%OD_INDEX   = OD_INDEX
   M%THIN_WALL(THIN_WALL_INDEX)%TD_INDEX   = TD_INDEX
   M%THIN_WALL(THIN_WALL_INDEX)%P1_INDEX   = P1_INDEX
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   ...
   P1_INDEX = M%CFACE(CFACE_INDEX)%P1_INDEX
   P2_INDEX = M%CFACE(CFACE_INDEX)%P2_INDEX
   ...
   M%CFACE(CFACE_INDEX)%P1_INDEX = P1_INDEX
   M%CFACE(CFACE_INDEX)%P2_INDEX = P2_INDEX
...
IF (.NOT.ALREADY_ALLOCATED) THEN
   ...
   ALLOCATE(ONE_D%N_LAYER_CELLS(SF%N_LAYERS))
ENDIF
...
IF (P1_INDEX==0) THEN
   ...
   ENDDO
   P1_INDEX = M%NEXT_AVAILABLE_BOUNDARY_PROP1_SLOT
ENDIF

IF (P1_INDEX>M%N_BOUNDARY_PROP1_DIM) THEN  ! There are no open slots for boundary coordinates
   ALLOCATE(P1_DUMMY(1:M%N_BOUNDARY_PROP1_DIM+N_NEW_STORAGE_SLOTS))
   P1_DUMMY(1:M%N_BOUNDARY_PROP1_DIM) = M%BOUNDARY_PROP1(1:M%N_BOUNDARY_PROP1_DIM)
   CALL MOVE_ALLOC(P1_DUMMY,M%BOUNDARY_PROP1)
...
M%BOUNDARY_PROP1_OCCUPANCY(P1_INDEX) = 1

P1 => M%BOUNDARY_PROP1(P1_INDEX)

IF (P1%SURF_INDEX==SURF_INDEX) THEN
   ALREADY_ALLOCATED = .TRUE.
ELSE
   ALREADY_ALLOCATED = .FALSE.
ENDIF

P1%SURF_INDEX = SURF_INDEX

IF (.NOT.ALREADY_ALLOCATED) THEN
   IF (ALLOCATED(P1%M_DOT_G_PP_ACTUAL)) DEALLOCATE(P1%M_DOT_G_PP_ACTUAL)
   ALLOCATE(P1%M_DOT_G_PP_ACTUAL(N_TRACKED_SPECIES))
   IF (ALLOCATED(P1%M_DOT_G_PP_ADJUST)) DEALLOCATE(P1%M_DOT_G_PP_ADJUST)
   ALLOCATE(P1%M_DOT_G_PP_ADJUST(N_TRACKED_SPECIES))
   IF (ALLOCATED(P1%ZZ_F)) DEALLOCATE(P1%ZZ_F)
   ALLOCATE(P1%ZZ_F(N_TRACKED_SPECIES))
   IF (ALLOCATED(P1%RHO_D_F)) DEALLOCATE(P1%RHO_D_F)
   ALLOCATE(P1%RHO_D_F(N_TRACKED_SPECIES))
   IF (ALLOCATED(P1%RHO_D_DZDN_F)) DEALLOCATE(P1%RHO_D_DZDN_F)
   ALLOCATE(P1%RHO_D_DZDN_F(N_TRACKED_SPECIES))
   IF (ALLOCATED(P1%AWM_AEROSOL)) DEALLOCATE(P1%AWM_AEROSOL)
   ALLOCATE(P1%AWM_AEROSOL(N_SURFACE_DENSITY_SPECIES))

   CALL INITIALIZE_BOUNDARY_PROP1(NM,P1_INDEX,SURF_INDEX)
ENDIF
...
IF (P2_INDEX==0) THEN
   ...
   ENDDO
   P2_INDEX = M%NEXT_AVAILABLE_BOUNDARY_PROP2_SLOT
ENDIF

IF (P2_INDEX>M%N_BOUNDARY_PROP2_DIM) THEN  ! There are no open slots for boundary coordinates
   ALLOCATE(P2_DUMMY(1:M%N_BOUNDARY_PROP2_DIM+N_NEW_STORAGE_SLOTS))
   P2_DUMMY(1:M%N_BOUNDARY_PROP2_DIM) = M%BOUNDARY_PROP2(1:M%N_BOUNDARY_PROP2_DIM)
   CALL MOVE_ALLOC(P2_DUMMY,M%BOUNDARY_PROP2)
...
M%BOUNDARY_PROP2_OCCUPANCY(P2_INDEX) = 1

P2 => M%BOUNDARY_PROP2(P2_INDEX)

IF (P2%SURF_INDEX==SURF_INDEX) THEN
   ALREADY_ALLOCATED = .TRUE.
ELSE
   ALREADY_ALLOCATED = .FALSE.
ENDIF
...
IF (.NOT.ALREADY_ALLOCATED) THEN
   IF (ALLOCATED(P2%A_LP_MPUA)) DEALLOCATE(P2%A_LP_MPUA)
   ALLOCATE(P2%A_LP_MPUA(N_LP_ARRAY_INDICES))
   IF (ALLOCATED(P2%LP_CPUA)) DEALLOCATE(P2%LP_CPUA)
   ALLOCATE(P2%LP_CPUA(N_LP_ARRAY_INDICES))
   IF (ALLOCATED(P2%LP_MPUA)) DEALLOCATE(P2%LP_MPUA)
   ALLOCATE(P2%LP_MPUA(N_LP_ARRAY_INDICES))
   IF (ALLOCATED(P2%LP_TEMP)) DEALLOCATE(P2%LP_TEMP)
   ALLOCATE(P2%LP_TEMP(N_LP_ARRAY_INDICES))
ENDIF
...
!> \param N_INTEGERS Number of integer variables per particle
!> \param N_REALS Number of real variables per particle
!> \param N_LOGICALS Number of logical variables per particle

SUBROUTINE PACK_PARTICLE(NM,LP_INDEX,LPC_INDEX,STORAGE_INDEX,UNPACK_IT,NOM,N_INTEGERS,N_REALS,N_LOGICALS)
...
INTEGER, OPTIONAL, INTENT(OUT) :: N_INTEGERS,N_REALS,N_LOGICALS
INTEGER :: IC,RC,LC,BC_INDEX,OD_INDEX,P1_INDEX,P2_INDEX,BR_INDEX
LOGICAL :: COUNT_ONLY
...
! COUNT_ONLY means that we only want to know the number of integers, reals, and logical variables for this particle class.

COUNT_ONLY = .FALSE.
IF (PRESENT(N_INTEGERS) .OR. PRESENT(N_REALS) .OR. PRESENT(N_LOGICALS)) COUNT_ONLY = .TRUE.
...
IF (UNPACK_IT .AND. .NOT.COUNT_ONLY) CALL ALLOCATE_STORAGE(NM,LP_INDEX=LP_INDEX,LPC_INDEX=LPC_INDEX,SURF_INDEX=LPC%SURF_INDEX)

LP => MESHES(NM)%LAGRANGIAN_PARTICLE(LP_INDEX)
BC_INDEX = LP%BC_INDEX
OD_INDEX = LP%OD_INDEX
P1_INDEX = LP%P1_INDEX
P2_INDEX = LP%P2_INDEX
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%TAG,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%CLASS_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%ORIENTATION_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%WALL_INDEX,UNPACK_IT)
IF (PRESENT(NOM) .AND. UNPACK_IT) LP%WALL_INDEX = 0
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%DUCT_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%DUCT_CELL_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%INIT_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%CFACE_INDEX,UNPACK_IT)
IF (PRESENT(NOM) .AND. UNPACK_IT) LP%CFACE_INDEX = 0
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),LP%PROP_INDEX,UNPACK_IT)

! Assign logical pointers and values

LC = 0

LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%SHOW,UNPACK_IT)
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%SPLAT,UNPACK_IT)
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%EMBER,UNPACK_IT)
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),LP%PATH_PARTICLE,UNPACK_IT)

! Assign real pointers and values

RC = 0

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%U,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%V,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%W,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%PWT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%ACCEL_X,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%ACCEL_Y,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%ACCEL_Z,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%RE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%MASS,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%T_INSERT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%DX,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%DY,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%DZ,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%C_DRAG,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),LP%RADIUS,UNPACK_IT)

! Boundary Coordinates

IF (LPC%INCLUDE_BOUNDARY_COORD_TYPE) CALL PACK_BOUNDARY_COORD(NM,IC,RC,OS,STORAGE_INDEX,BC_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,SF,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,STORAGE_INDEX,P1_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_PROP2_TYPE) CALL PACK_BOUNDARY_PROP2(NM,IC,RC,OS,1,P2_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_RADIA_TYPE) CALL PACK_BOUNDARY_RADIA(NM,RC,OS,1,BR_INDEX,UNPACK_IT,COUNT_ONLY)

IF (PRESENT(N_INTEGERS)) N_INTEGERS = IC
IF (PRESENT(N_REALS))    N_REALS    = RC
...
!> \param STORAGE_INDEX Index of the storage number
!> \param UNPACK_IT Logical indicating whether the data is to be packed into the 1-D array or unpacked from it
!> \param N_INTEGERS Number of integer variables per wall cell
!> \param N_REALS Number of real variables per wall cell
!> \param N_LOGICALS Number of logical variables per wall cell

SUBROUTINE PACK_WALL(NM,WALL_INDEX,SURF_INDEX,STORAGE_INDEX,UNPACK_IT,NOM,N_INTEGERS,N_REALS,N_LOGICALS)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER :: NM2
INTEGER, INTENT(IN) :: NM,SURF_INDEX,WALL_INDEX,STORAGE_INDEX
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER :: IC,RC,LC,BC_INDEX,OD_INDEX,P1_INDEX,P2_INDEX,BR_INDEX
INTEGER, OPTIONAL, INTENT(IN) :: NOM
INTEGER, OPTIONAL, INTENT(OUT) :: N_INTEGERS,N_REALS,N_LOGICALS
LOGICAL :: COUNT_ONLY
...
! COUNT_ONLY means that we only want to know the number of integers, reals, and logical variables for this wall cell.
...
IF (PRESENT(NOM) .AND. UNPACK_IT) THEN
   OS => MESHES(NM)%OMESH(NOM)%WALL_STORAGE_RECV(SURF_INDEX)
   NM2 = NOM
ELSEIF (PRESENT(NOM)) THEN
   OS => MESHES(NM2)%OMESH(NOM)%WALL_STORAGE_SEND(SURF_INDEX)
ELSE
   OS => SF%WALL_STORAGE
   IF (UNPACK_IT .AND. .NOT.COUNT_ONLY) CALL ALLOCATE_STORAGE(NM2,WALL_INDEX=WALL_INDEX,SURF_INDEX=SURF_INDEX)
ENDIF

! Point to the wall cell that was just set up and extract the various boundary indices

WC => MESHES(NM2)%WALL(WALL_INDEX)
BC_INDEX = WC%BC_INDEX
OD_INDEX = WC%OD_INDEX
P1_INDEX = WC%P1_INDEX
P2_INDEX = WC%P2_INDEX
BR_INDEX = WC%BR_INDEX

! Pack or unpack the integer values

IC = 0  ! Integer Counter

IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%BACK_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%BACK_MESH,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%BACK_SURF,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%BOUNDARY_TYPE,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%SURF_INDEX_ORIG,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%OBST_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%PRESSURE_BC_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%VENT_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%JD11_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%JD12_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%JD21_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%JD22_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),WC%CUT_FACE_INDEX,UNPACK_IT)

! Pack or unpack the real values

RC = 0  ! Real Counter

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),WC%DUNDT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),WC%Q_LEAK,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),WC%V_DEP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),WC%VEL_ERR_NEW,UNPACK_IT)

! Pack or unpack the logical values

LC = 0  ! Logical Counter

! Pack or unpack the appropriate derived type variables tied to this wall cell

IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) CALL PACK_BOUNDARY_COORD(NM2,IC,RC,OS,STORAGE_INDEX,BC_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM2,IC,RC,OS,SF,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM2,IC,RC,LC,OS,STORAGE_INDEX,P1_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) CALL PACK_BOUNDARY_PROP2(NM2,IC,RC,OS,STORAGE_INDEX,P2_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) CALL PACK_BOUNDARY_RADIA(NM2,RC,OS,STORAGE_INDEX,BR_INDEX,UNPACK_IT,COUNT_ONLY)
...
!> \param NM Mesh index
!> \param THIN_WALL_INDEX Index of the THIN_WALL cell
!> \param SURF_INDEX Index of the surface
!> \param STORAGE_INDEX Index of the storage number
!> \param UNPACK_IT Logical indicating whether the data is to be packed into the 1-D array or unpacked from it
!> \param N_INTEGERS Number of integer variables per wall cell
...
SUBROUTINE PACK_THIN_WALL(NM,THIN_WALL_INDEX,SURF_INDEX,STORAGE_INDEX,UNPACK_IT,NOM,N_INTEGERS,N_REALS,N_LOGICALS)

USE COMP_OPERATORS, ONLY: EQUATE
INTEGER :: NM2
INTEGER, INTENT(IN) :: NM,SURF_INDEX,THIN_WALL_INDEX,STORAGE_INDEX
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER :: IC,RC,LC,BC_INDEX,OD_INDEX
INTEGER, OPTIONAL, INTENT(IN) :: NOM
INTEGER, OPTIONAL, INTENT(OUT) :: N_INTEGERS,N_REALS,N_LOGICALS
LOGICAL :: COUNT_ONLY
...
IF (PRESENT(NOM) .AND. UNPACK_IT) THEN
   OS => MESHES(NM)%OMESH(NOM)%THIN_WALL_STORAGE_RECV(SURF_INDEX)
   NM2 = NOM
ELSEIF (PRESENT(NOM)) THEN
   OS => MESHES(NM2)%OMESH(NOM)%THIN_WALL_STORAGE_SEND(SURF_INDEX)
ELSE
   OS => SF%WALL_STORAGE
   IF (UNPACK_IT .AND. .NOT.COUNT_ONLY) CALL ALLOCATE_STORAGE(NM2,THIN_WALL_INDEX=THIN_WALL_INDEX,SURF_INDEX=SURF_INDEX)
ENDIF

! Point to the wall cell that was just set up and extract the various boundary indices

TW => MESHES(NM2)%THIN_WALL(THIN_WALL_INDEX)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%BACK_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%BACK_MESH,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%BACK_SURF,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%BOUNDARY_TYPE,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%OBST_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),TW%IEC,UNPACK_IT)
...
!> \param SURF_INDEX Index of the surface
!> \param UNPACK_IT Logical indicating whether the data is to be packed into the 1-D array or unpacked from it
!> \param N_INTEGERS Number of integer variables per cface
!> \param N_REALS Number of real variables per cface
!> \param N_LOGICALS Number of logical variables per cface

SUBROUTINE PACK_CFACE(NM,CFACE_INDEX,SURF_INDEX,UNPACK_IT,N_INTEGERS,N_REALS,N_LOGICALS)
...
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER, OPTIONAL, INTENT(OUT) :: N_INTEGERS,N_REALS,N_LOGICALS
INTEGER :: I1,I2,IC,RC,LC,BC_INDEX,OD_INDEX,P1_INDEX,P2_INDEX,BR_INDEX
LOGICAL :: COUNT_ONLY
TYPE(CFACE_TYPE), POINTER :: CFA
TYPE(STORAGE_TYPE), POINTER :: OS
TYPE(SURFACE_TYPE), POINTER :: SF

! COUNT_ONLY means that we only want to know the number of integers, reals, and logical variables for this cface cell.
...
IF (UNPACK_IT .AND. .NOT.COUNT_ONLY) CALL ALLOCATE_STORAGE(NM,CFACE_INDEX=CFACE_INDEX,SURF_INDEX=SURF_INDEX)

CFA => MESHES(NM)%CFACE(CFACE_INDEX)
BC_INDEX = CFA%BC_INDEX
OD_INDEX = CFA%OD_INDEX
P1_INDEX = CFA%P1_INDEX
P2_INDEX = CFA%P2_INDEX
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%VENT_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%BACK_MESH,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%BACK_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%BOUNDARY_TYPE,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%CUT_FACE_IND1,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),CFA%CUT_FACE_IND2,UNPACK_IT)

! Assign and initialize reals

RC = 0

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%AREA,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%NVEC(1),UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%NVEC(2),UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%NVEC(3),UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%VEL_ERR_NEW,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%V_DEP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%Q_LEAK,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%DUNDT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%RSUM_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%TMP_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%RHO_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%MU_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),CFA%PRES_BXN,UNPACK_IT)

I2 = RC
I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,1) , CFA%ZZ_G(1:I2-I1+1) , UNPACK_IT)
RC = I2
...
IF (SF%INCLUDE_BOUNDARY_COORD_TYPE) CALL PACK_BOUNDARY_COORD(NM,IC,RC,OS,1,BC_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,SF,1,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP1_TYPE) CALL PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,1,P1_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROP2_TYPE) CALL PACK_BOUNDARY_PROP2(NM,IC,RC,OS,1,P2_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_RADIA_TYPE) CALL PACK_BOUNDARY_RADIA(NM,RC,OS,1,BR_INDEX,UNPACK_IT,COUNT_ONLY)
...
SUBROUTINE PACK_BOUNDARY_COORD(NM,IC,RC,OS,STORAGE_INDEX,BC_INDEX,UNPACK_IT,COUNT_ONLY)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%II,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%JJ,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%KK,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%IIG,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%JJG,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%KKG,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BC%IOR,UNPACK_IT)

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%X,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Y,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Z,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%X1,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%X2,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Y1,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Y2,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Z1,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BC%Z2,UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_ONE_D(NM,IC,RC,OS,SF,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
...
IF (.NOT.COUNT_ONLY) ONE_D => MESHES(NM)%BOUNDARY_ONE_D(OD_INDEX)

! Integers

IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%N_CELLS_MAX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%N_CELLS_INI,UNPACK_IT)

I2 = IC

I1 = I2+1 ; I2 = I1 + SF%N_LAYERS - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(I1:I2,STORAGE_INDEX) , ONE_D%N_LAYER_CELLS(1:I2-I1+1) , UNPACK_IT)

IC = I2  ! Update Integer Counter

I2 = RC  ! I1 and I2 continue counting reals

I1 = I2+1 ; I2 = I1 + SF%N_MATL - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%M_DOT_S_PP(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_CELLS_MAX
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%X(0:I2-I1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_CELLS_MAX + 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%TMP(0:I2-I1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_LAYERS - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%LAYER_THICKNESS(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_CELLS_MAX - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%RHO_C_S(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_CELLS_MAX + 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%K_S(0:I2-I1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_LAYERS - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%DDSUM(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_LAYERS - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%SMALLEST_CELL_SIZE(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_LPC - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%PART_MASS(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_LPC - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%PART_ENTHALPY(1:I2-I1+1) , UNPACK_IT)

DO NN=1,SF%N_MATL
   I1 = I2+1 ; I2 = I1 + (SF%N_CELLS_MAX+2) - 1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%MATL_COMP(NN)%RHO(0:I2-I1) , UNPACK_IT)
ENDDO

DO NN=1,SF%N_MATL
   I1 = I2+1 ; I2 = I1 + (SF%N_CELLS_MAX+2) - 1
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%MATL_COMP(NN)%RHO_DOT(0:I2-I1) , UNPACK_IT)
ENDDO
...
SUBROUTINE INITIALIZE_BOUNDARY_PROP1(NM,P1_INDEX,SURF_INDEX)

USE GLOBAL_CONSTANTS, ONLY: SIGMA,TMPA,RHOA,TMPA4,N_TRACKED_SPECIES
INTEGER, INTENT(IN) :: NM,P1_INDEX,SURF_INDEX
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: P1
TYPE(SURFACE_TYPE), POINTER :: SF

P1 => MESHES(NM)%BOUNDARY_PROP1(P1_INDEX)
SF => SURFACE(SURF_INDEX)

P1%Q_RAD_IN        = SF%EMISSIVITY*SIGMA*TMPA4
P1%Q_RAD_OUT       = SF%EMISSIVITY*SIGMA*TMPA4
P1%EMISSIVITY      = SF%EMISSIVITY
P1%T_IGN           = SF%T_IGN
P1%TMP_F           = SF%TMP_FRONT
P1%TMP_G           = TMPA
P1%TMP_B           = SF%TMP_BACK
P1%RHO_F           = RHOA
P1%TMP_F_OLD       = SF%TMP_FRONT
P1%BURN_DURATION   = SF%BURN_DURATION

P1%M_DOT_G_PP_ACTUAL = 0._EB
P1%M_DOT_G_PP_ADJUST = 0._EB
P1%ZZ_F(1:N_TRACKED_SPECIES) = SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0
P1%RHO_D_F = 0._EB
P1%RHO_D_DZDN_F = 0._EB
P1%AWM_AEROSOL = 0._EB
P1%Q_IN_SMOOTH = 0._EB
...
!> \param OS STORAGE_ARRAY name
!> \param STORAGE_INDEX Column in the STORAGE_ARRAY to hold the data
!> \param P1_INDEX Index of the BOUNDARY_PROP1 array
!> \param UNPACK_IT Logical indicating whether the data is to be packed into the 1-D array or unpacked from it
!> \param Optional flag signifying that only a variable count is to be done; no packing

SUBROUTINE PACK_BOUNDARY_PROP1(NM,IC,RC,LC,OS,STORAGE_INDEX,P1_INDEX,UNPACK_IT,COUNT_ONLY)

USE COMP_OPERATORS, ONLY: EQUATE
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,N_SURFACE_DENSITY_SPECIES
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,P1_INDEX
...
IF (.NOT.COUNT_ONLY) P1 => MESHES(NM)%BOUNDARY_PROP1(P1_INDEX)

! Integers

IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),P1%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),P1%PRESSURE_ZONE,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),P1%NODE_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),P1%N_SUBSTEPS,UNPACK_IT)

! Logicals

LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),P1%BURNAWAY,UNPACK_IT)

! Reals

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%AREA,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%HEAT_TRANS_COEF,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_CON_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_RAD_IN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_RAD_OUT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%EMISSIVITY,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%AREA_ADJUST,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%T_IGN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%TMP_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%TMP_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%TMP_B,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%U_NORMAL,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%U_NORMAL_S,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%U_NORMAL_0,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%U_TANG,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%RHO_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%RDN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%K_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_DOT_G_PP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_DOT_O2_PP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_CONDENSE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%TMP_F_OLD,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%BURN_DURATION,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%T_SCALE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_SCALE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%T_MATL_PART,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%B_NUMBER,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%M_DOT_PART_ACTUAL,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P1%Q_IN_SMOOTH,UNPACK_IT)

I2 = RC  ! I1 and I2 continue counting reals

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , P1%M_DOT_G_PP_ACTUAL(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , P1%M_DOT_G_PP_ADJUST(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , P1%ZZ_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , P1%RHO_D_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , P1%RHO_D_DZDN_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_SURFACE_DENSITY_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , P1%AWM_AEROSOL(1:I2-I1+1) , UNPACK_IT)
...
!> \param P2_INDEX Index within BOUNDARY_PROP2 array
!> \param UNPACK_IT Logical indicating whether to pack or unpack
!> \param Optional flag signifying that only a variable count is to be done; no packing

SUBROUTINE PACK_BOUNDARY_PROP2(NM,IC,RC,OS,STORAGE_INDEX,P2_INDEX,UNPACK_IT,COUNT_ONLY)

USE COMP_OPERATORS, ONLY: EQUATE
USE GLOBAL_CONSTANTS, ONLY: N_LP_ARRAY_INDICES
INTEGER, INTENT(IN) :: NM,STORAGE_INDEX,P2_INDEX
...
IF (.NOT.COUNT_ONLY) P2 => MESHES(NM)%BOUNDARY_PROP2(P2_INDEX)

IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),P2%SURF_INDEX,UNPACK_IT)

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P2%U_TAU,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P2%Y_PLUS,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P2%Z_STAR,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P2%PHI_LS,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P2%WORK1,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P2%WORK2,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),P2%K_SUPPRESSION,UNPACK_IT)

I2 = RC

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),P2%A_LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),P2%LP_CPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),P2%LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),P2%LP_TEMP(1:I2-I1+1),UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_RADIA(NM,RC,OS,STORAGE_INDEX,BR_INDEX,UNPACK_IT,COUNT_ONLY)
...
DO NN=1,NUMBER_SPECTRAL_BANDS
   ...
   IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BR%BAND(NN)%ILW(1:I2-I1+1),UNPACK_IT)
ENDDO

I1 = I2+1 ; I2 = I1 + NUMBER_SPECTRAL_BANDS - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BR%IL(1:I2-I1+1),UNPACK_IT)
