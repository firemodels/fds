...
CHARACTER(255), PARAMETER :: gsmvid='$Id$'
CHARACTER(255), PARAMETER :: gsmvrev='$Revision$'
CHARACTER(255), PARAMETER :: gsmvdate='$Date$'
...
CALL REMOVE_DEGENERATE_FACES(NFACES,MAXFACES,FACES,WHICH_POLY)
IF(DEBUG==1)THEN
  DO I = 0, NVERTS-1
...
SUBROUTINE REMOVE_DEGENERATE_FACES(N_FACES,MAX_FACES,FACES,WHICH_POLY)
INTEGER, INTENT(INOUT) :: N_FACES
INTEGER, INTENT(IN) :: MAX_FACES
INTEGER, TARGET, INTENT(INOUT) :: FACES(3*MAX_FACES), WHICH_POLY(MAX_FACES)
...
DO IFROM = 1, N_FACES
   ...
   WHICH_POLY(N_FACES2) = WHICH_POLY(IFROM)
ENDDO
N_FACES=N_FACES2
...
SUBROUTINE MAKE_EDGELIST(FACES,NFACES,EDGES,NEDGES)
...
! sort (now o(n**2) replace with o(n*log(n)) once n is "large" )

FINISHED=1
DO WHILE (FINISHED.EQ.1) 
   DO I = 1, NEDGES-1
      IF(EDGES(I,1).LT.EDGES(I+1,1)) CYCLE
      IF(EDGES(I,1).EQ.EDGES(I+1,1).AND.EDGES(I,2).LT.EDGES(I+1,2)) CYCLE
      TEMP1=EDGES(I,1)
      TEMP2=EDGES(I,2)
...
ITO=0
DO I = 1, NEDGES-1
   IF(EDGES(I,1).EQ.EDGES(I+1,1).AND.EDGES(I+1,1).EQ.EDGES(I+1,2)) CYCLE
   ITO = ITO + 1
   EDGES(ITO,1) = EDGES(I,1)
   EDGES(ITO,2) = EDGES(I,2)
END DO
NEDGES = ITO

END SUBROUTINE MAKE_EDGELIST
