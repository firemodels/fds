...
IF (N_TRACKED_SPECIES>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   ! Allocate scalar diffusive fluxes

   ALLOCATE( M%FDX(0:IBP1,0:JBP1,0:KBP1,1:N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','FDX',IZERO)
   ALLOCATE( M%FDY(0:IBP1,0:JBP1,0:KBP1,1:N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','FDY',IZERO)
   ALLOCATE( M%FDZ(0:IBP1,0:JBP1,0:KBP1,1:N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','FDZ',IZERO)
   M%FDX = 0._EB
   M%FDY = 0._EB
   M%FDZ = 0._EB
ENDIF
...
IF (TEST_FULL_TRANSPORT) THEN
   ...
   ALLOCATE(M%SCALAR_WORK1(0:IBP1,0:JBP1,0:KBP1,0:N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','SCALAR_SAVE1',IZERO)
ENDIF

IF (EMBEDDED_MESH) THEN
   ALLOCATE(M%SCALAR_SAVE1(0:IBP1,0:JBP1,0:KBP1,0:N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','SCALAR_SAVE1',IZERO)
   ALLOCATE(M%SCALAR_SAVE2(0:IBP1,0:JBP1,0:KBP1,0:N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','SCALAR_SAVE2',IZERO)
   ALLOCATE(M%SCALAR_SAVE3(0:IBP1,0:JBP1,0:KBP1,0:N_TRACKED_SPECIES),STAT=IZERO)
