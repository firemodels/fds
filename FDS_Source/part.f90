...
REAL(EB) :: SURFACE_PARTICLE_DIAMETER,XI,YJ,ZK,RD,RDS,RDC,&
            X_OLD,Y_OLD,Z_OLD,THETA_RN,STEP_FRACTION(-3:3),P_UVWMAX
LOGICAL :: HIT_SOLID
...
PARTICLE_LOOP: DO I=1,NLP  
   ...
   SOLID_GAS_MOVE: IF (LP%ONE_D%IOR/=0) THEN
      ...
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)      
   ELSE SOLID_GAS_MOVE
      CALL MOVE_IN_GAS(I,NM,T,P_UVWMAX)
      LP => LAGRANGIAN_PARTICLE(I)
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
...
SUM_MOMENTUM_LOOP: DO I=1,NLP
   ...
   IW = WALL_INDEX(IC,1)
   IF (WALL(IW)%BOUNDARY_TYPE==NULL_BOUNDARY .OR. LPC%PERIODIC_X) THEN
      FVXS(IIX,JJ,KK) = FVXS(IIX,JJ,KK) - LP%ACCEL_X
      
      IF (LPC%PERIODIC_X) THEN
         IF (IIX == 0) THEN
            FVXS(IBAR,JJ,KK) = FVXS(IBAR,JJ,KK) - LP%ACCEL_X
         ELSEIF (IIX == IBAR) THEN
            FVXS(0,JJ,KK) = FVXS(0,JJ,KK) - LP%ACCEL_X
         ENDIF
      ENDIF
   ENDIF
   IC = CELL_INDEX(II,JJY,KK)
   IW = WALL_INDEX(IC,2) 
   IF (WALL(IW)%BOUNDARY_TYPE==NULL_BOUNDARY .OR. LPC%PERIODIC_Y) THEN
      FVYS(II,JJY,KK) = FVYS(II,JJY,KK) - LP%ACCEL_Y
      
      IF (LPC%PERIODIC_Y) THEN
         IF (JJY == 0) THEN
            FVYS(II,JBAR,KK) = FVYS(II,JBAR,KK) - LP%ACCEL_Y
         ELSEIF (JJY == JBAR) THEN
            FVYS(II,0,KK) = FVYS(II,0,KK) - LP%ACCEL_Y
         ENDIF
      ENDIF
   ENDIF
   IC = CELL_INDEX(II,JJ,KKZ)
   IW = WALL_INDEX(IC,3) 
   IF (WALL(IW)%BOUNDARY_TYPE==NULL_BOUNDARY .OR. LPC%PERIODIC_Z) THEN
      FVZS(II,JJ,KKZ) = FVZS(II,JJ,KKZ) - LP%ACCEL_Z
      
      IF (LPC%PERIODIC_Z) THEN
         IF (KKZ == 0) THEN
            FVZS(II,JJ,KBAR) = FVZS(II,JJ,KBAR) - LP%ACCEL_Z
         ELSEIF (KKZ == KBAR) THEN
            FVZS(II,JJ,0) = FVZS(II,JJ,0) - LP%ACCEL_Z
         ENDIF
      ENDIF
   ENDIF
