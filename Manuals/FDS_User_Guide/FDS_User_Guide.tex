...
\noindent The {\ct INIT} line designates the plane of the screen using the sextuplet {\ct XB}. A single particle is located in each cell with the parameter {\ct N\_PARTICLES\_PER\_CELL=1}.  The particles should be defined with a {\ct SURF\_ID} containing the material properties of the screen.  A special drag law for screens is used by setting {\ct DRAG\_LAW='SCREEN'}.  {\ct ORIENTATION} is the surface normal to the screen and {\ct FREE\_AREA\_FRACTION} is the fraction of the screen's surface area that is open.  The pressure drop across the screen is given by
