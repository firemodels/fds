...
DO NM=1,NMESHES
   ...
   ALLOCATE(M%NEIGHBORING_MESH(20))
   DO NM2=1,NMESHES
      M2 => MESHES(NM2)
      IF (M2%XS>M%XF+M%DXI   .OR. M2%XF<M%XS-M%DXI   .OR. &
          M2%YS>M%YF+M%DETA  .OR. M2%YF<M%YS-M%DETA  .OR. &
          M2%ZS>M%ZF+M%DZETA .OR. M2%ZF<M%ZS-M%DZETA) CYCLE
      M%N_NEIGHBORING_MESHES = M%N_NEIGHBORING_MESHES + 1
      M%NEIGHBORING_MESH(M%N_NEIGHBORING_MESHES) = NM2
   ENDDO
ENDDO
