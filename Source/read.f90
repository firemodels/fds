...
IF (SMOKE3D) THEN
   QUANTITY_LOOP: DO N=1,4
      CALL GET_QUANTITY_INDEX(SMOKE3D_SMOKEVIEW_LABEL(N),SMOKE3D_SMOKEVIEW_BAR_LABEL(N),SMOKE3D_QUANTITY_INDEX(N),I_DUM(1), &
                              ...
                              SMOKE3D_QUANTITY(N),'null',SMOKE3D_SPEC_ID(N),'null','null','null','null','null',I_DUM(6),I_DUM(7))
   ENDDO QUANTITY_LOOP
ENDIF
...
INTEGER  :: NN,NM,MESH_NUMBER,N_DEVC_READ,IOR,TRIP_DIRECTION,VELO_INDEX,POINTS,I_POINT,PIPE_INDEX,ORIENTATION_INDEX, &
            ORIENTATION_NUMBER,STATISTICS_LOCATION_INDEX,GHOST_CELL_IOR(3),CELL_N
...
NAMELIST /DEVC/ ABSOLUTE_VALUE,BYPASS_FLOWRATE,CONVERSION_FACTOR,CONVERSION_ADDEND,COORD_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,&
                ...
                TIME_AVERAGED,TIME_HISTORY,TRIP_DIRECTION,UNITS,VELO_INDEX,XB,XYZ,X_ID,Y_ID,Z_ID,XYZ_UNITS,CELL_N
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (BAD) THEN
         IF (CTRL_ID/='null' .OR. DUCT_ID/='null' .OR. NODE_ID(1)/='null' .OR. INIT_ID/='null' .OR. CELL_N>-1) THEN
            XYZ(1) = MESHES(1)%XS
            XYZ(2) = MESHES(1)%YS
            XYZ(3) = MESHES(1)%ZS
            MESH_NUMBER = 1
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   QUANTITY_IF: IF (DV%QUANTITY /= 'null') THEN
      ...
      IF (QUANTITY_INDEX < 0 .AND. (DV%STATISTICS=='MASS MEAN' .OR. DV%STATISTICS=='VOLUME MEAN' .OR. &
                                    ...
                                    DV%STATISTICS=='MASS' .OR. DV%STATISTICS=='VOLUME')) THEN
...
READ_ISOF_LOOP: DO N=1,N_ISOF
   ...
   CALL GET_QUANTITY_INDEX(IS%SMOKEVIEW_LABEL,IS%SMOKEVIEW_BAR_LABEL,IS%INDEX,I_DUM(1), &
                           ...
                           QUANTITY,'null',SPEC_ID,'null','null','null','null','null',I_DUM(7),I_DUM(8))
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         CALL GET_QUANTITY_INDEX(SL%SMOKEVIEW_LABEL,SL%SMOKEVIEW_BAR_LABEL,SL%INDEX,SL%INDEX2, &
                                 ...
                                 QUANTITY,QUANTITY2,SPEC_ID,PART_ID,'null','null',REAC_ID,MATL_ID,I_DUM(3),I_DUM(4))
...
READ_BNDF_LOOP: DO N=1,N_BNDF
   ...
   CALL GET_QUANTITY_INDEX(BF%SMOKEVIEW_LABEL,BF%SMOKEVIEW_BAR_LABEL,BF%INDEX,I_DUM(1), &
                           ...
                           QUANTITY,'null',SPEC_ID,PART_ID,'null','null','null','null',I_DUM(6),I_DUM(7))
...
SUBROUTINE GET_QUANTITY_INDEX(SMOKEVIEW_LABEL,SMOKEVIEW_BAR_LABEL,OUTPUT_INDEX,OUTPUT2_INDEX, &
                              ...
                              QUANTITY,QUANTITY2,SPEC_ID_IN,PART_ID,DUCT_ID,NODE_ID,REAC_ID,MATL_ID,CELL_N,DUCT_CELL_INDEX)
...
INTEGER, INTENT(OUT) :: OUTPUT_INDEX,Y_INDEX,Z_INDEX,PART_INDEX,DUCT_INDEX,NODE_INDEX,REAC_INDEX,OUTPUT2_INDEX,MATL_INDEX,&
                        DUCT_CELL_INDEX
INTEGER :: ND,NS,NN,NR,N_PLUS,N_MINUS,CELL_N
...
IF (CELL_N > 0) THEN
   IF (DUCT_ID == 'null') THEN
      ...
      CALL SHUTDOWN(MESSAGE)
   ELSEIF (CELL_N > DUCT(DUCT_INDEX)%N_CELLS) THEN
      WRITE(MESSAGE,'(A,A,A,A)')  'ERROR: CELL_N used for output QUANTITY ',TRIM(QUANTITY),' in outside of DUCT_ID ',TRIM(DUCT_ID)
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   DUCT_CELL_INDEX = CELL_N
ENDIF
...
QUANTITY_INDEX_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
   ...
   QUANTITY_IF:IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
      ...
      IF (OUTPUT_QUANTITY(ND)%CELL_N_REQUIRED .AND. DUCT_CELL_INDEX<1) THEN ! come back and add DUCT_ID errors
         IF (CELL_N<0) THEN
            WRITE(MESSAGE,'(3A)')  'ERROR: Output QUANTITY ',TRIM(QUANTITY),' requires a CELL_N'
         ELSE
            WRITE(MESSAGE,'(5A)')  'ERROR: Output QUANTITY ',TRIM(QUANTITY),'. CELL_N ',CELL_N,' not found.'
         ENDIF
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
