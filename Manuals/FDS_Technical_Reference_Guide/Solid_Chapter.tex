...
The deposition velocity to surfaces is determined by assuming the phenomena are independent,
computing a deposition velocity for each (u$_{th}$, the thermophoretic velocity, and u$_{dt}$,
a combined diffusion-turbulence velocity), and summing them~\cite{Bixler:1}. If the aerosol
is located in a gas-phase cell adjacent to a wall cell, then the aerosol is removed from
the gas phase at a rate proportional to its deposition velocity and deposited onto the surface.
If the aerosol is located in a gas-phase cell adjacent to an upward-facing wall cell
(i.e., a floor), then the aerosol is removed from the gas phase at a rate proportional
to its deposition velocity plus its gravitational settling velocity (u$_g$) and deposited onto the
flat surface. The accumulated surface density of deposited aerosols is tracked.
