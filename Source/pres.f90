   ...
   IF_DIRICHLET: IF (WC%PRESSURE_BC_INDEX==DIRICHLET) THEN
      ...
      OPEN_IF: IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
         ...
         IF (TEST_NEW_OPEN) THEN
            ...
            IF (VT%VEL > -1.E12) THEN
               SELECT CASE(ABS(IOR))
                  CASE( 1); UBAR = -SIGN(1._EB,REAL(IOR,EB))*VT%VEL
                  CASE( 2); VBAR = -SIGN(1._EB,REAL(IOR,EB))*VT%VEL
                  CASE( 3); WBAR = -SIGN(1._EB,REAL(IOR,EB))*VT%VEL
               END SELECT
            ENDIF
            IF (ICF>0) THEN
               H0 = 0.5_EB*((UBAR+VEL_EDDY)**2 + (UBAR+VEL_EDDY)**2 + (UBAR+VEL_EDDY)**2)
            ELSE
               SELECT CASE(IOR)
