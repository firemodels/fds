...
PRIMARY_WALL_LOOP_2: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.SF%HT3D .OR. WC%BACK_INDEX<=0) CYCLE PRIMARY_WALL_LOOP_2
...
PRIMARY_THIN_WALL_LOOP_2: DO ITW=1,M%N_THIN_WALL_CELLS

   TW => M%THIN_WALL(ITW)
   SF => SURFACE(TW%SURF_INDEX)

   IF (.NOT.SF%HT3D .OR. TW%BACK_INDEX<=0) CYCLE PRIMARY_THIN_WALL_LOOP_2
   ...
   PRIMARY_NODE_LOOP_2: DO I=1,NWP  ! Loop over nodes of primary wall cell
      ...
      ALTERNATE_WALL_LOOP_B: DO IW2=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS  ! Loop over potential alternate wall cells
         CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NM,WALL_CELL=IW2)
      ENDDO ALTERNATE_WALL_LOOP_B

      OTHER_MESH_LOOP_2B: DO NOM=1,NMESHES
         IF (NM==NOM) CYCLE
         ALTERNATE_WALL_LOOP_2B: DO IW2=1,M%OMESH(NOM)%N_WALL_CELLS_RECV
            CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM,WALL_CELL=IW2)
         ENDDO ALTERNATE_WALL_LOOP_2B
      ENDDO OTHER_MESH_LOOP_2B

      ALTERNATE_THIN_WALL_LOOP_B: DO IW2=1,M%N_THIN_WALL_CELLS  ! Loop over potential alternate wall cells
         CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NM,THIN_WALL_CELL=IW2)
      ...
      IF (ABS(SUM(THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(:))-2._EB)>0.001_EB) THEN
         ...
         ENDDO
         WRITE(0,'(A,I0,A,I0,A,I0,A,I0,A,I0,A,I0,A,I0,A,F6.3)') &
            'WARNING: Mesh=',NM,' THIN_WALL=',ITW,' IJK=',BC%IIG,',',BC%JJG,',',BC%KKG,' IOR=',BC%IOR,' NODE=',I,' WEIGHT=',&
            SUM(THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(:))
      ENDIF

   ENDDO PRIMARY_NODE_LOOP_2

ENDDO PRIMARY_THIN_WALL_LOOP_2

CONTAINS

SUBROUTINE SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM,WALL_CELL,THIN_WALL_CELL)
...
ELSE
   CELL = THIN_WALL_CELL
   TW2 => M2%THIN_WALL(THIN_WALL_CELL)
...
ALTERNATE_NODE_LOOP: DO I2=1,NWP2  ! Loop over nodes of alternate wall cell
   ...
   IF (IWA>DM) CALL REALLOCATE_ALTERNATE
   ...
   IF (PRESENT(THIN_WALL_CELL)) THR_D%NODE(I)%ALTERNATE_WALL_TYPE(IWA) = 1
