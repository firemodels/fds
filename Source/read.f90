...
LOGICAL :: DEFAULT,VEG_LSET_SPREAD,VEG_LSET_TAN2,TGA_ANALYSIS,COMPUTE_EMISSIVITY,&
           COMPUTE_EMISSIVITY_BACK,HT3D,THERM_THICK,BLOWING,ABL_MODEL,SET_H
...
NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BLOWING,BURN_AWAY,BURN_DURATION,&
                ...
                RENODE_DELTA_T,RGB,ROUGHNESS,SET_H,SHAPE_FACTOR,SPEC_ID,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%ADIABATIC .AND. (SF%NET_HEAT_FLUX < 1.E12_EB .OR. ABS(SF%CONVECTIVE_HEAT_FLUX)>TWO_EPSILON_EB)) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
   IF (SF%NET_HEAT_FLUX < 1.E12_EB) THEN
      IF (SET_H) THEN
            WRITE(MESSAGE,'(A)') 'ERROR: SURF '//TRIM(SF%ID)// ' cannot have both NET_HEAT_FLUX and SET_H=T'
            CALL SHUTDOWN(MESSAGE) ; RETURN
      ELSE
         IF (ABS(SF%CONVECTIVE_HEAT_FLUX)>TWO_EPSILON_EB) THEN
   ...
   ENDIF
   IF (ABS(SF%CONVECTIVE_HEAT_FLUX)>TWO_EPSILON_EB) THEN
      IF (.NOT. SET_H .AND. TMP_FRONT >= -TMPM) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: SURF '//TRIM(SF%ID)// ' cannot have TMP_FRONT with CONVECTIVE_HEAT_FLUX'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (SET_H .AND. TMP_FRONT < -TMPM) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: SURF '//TRIM(SF%ID)// ' need to set TMP_FRONT with SET_H=T and CONVECTIVE_HEAT_FLUX'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
   IF (HT3D) THEN
...
#ifndef NO_SCARC
USE SCRC, ONLY: SCARC_METHOD, SCARC_GRID, SCARC_ERROR_FILE, SCARC_ACCURACY, &
                ...
                SCARC_VERBOSE, SCARC_MULTIGRID_CYCLE, SCARC_MULTIGRID_LEVEL, SCARC_MULTIGRID_INTERPOL, &
...
NAMELIST /PRES/ BAROCLINIC,CHECK_POISSON,FISHPAK_BC,ITERATION_SUSPEND_FACTOR,INSEPARABLE_POISSON, &
                SCARC_METHOD, SCARC_GRID, SCARC_ERROR_FILE, SCARC_ACCURACY, &
                SCARC_KRYLOV_ITERATIONS, SCARC_KRYLOV_ACCURACY, SCARC_KRYLOV_INTERPOL, SCARC_MGM_BC, SCARC_MGM_INTERPOLATION, &
                SCARC_MGM_ITERATIONS, SCARC_MGM_ACCURACY, SCARC_MGM_EXACT_INITIAL, SCARC_MGM_LAPLACE_SOLVER, SCARC_MGM_USE_FFT, &
                SCARC_MGM_CHECK_LAPLACE, SCARC_MULTIGRID, SCARC_MULTIGRID_ITERATIONS, SCARC_MULTIGRID_ACCURACY, SCARC_TWOLEVEL, &
                SCARC_VERBOSE, SCARC_MULTIGRID_CYCLE, SCARC_MULTIGRID_LEVEL, SCARC_MULTIGRID_INTERPOL, &
