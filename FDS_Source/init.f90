...
REAL(EB) :: MU_N,YY_GET(1:N_GAS_SPECIES),VC,RTRM,CP,TMP_WGT
...
!!ALLOCATE(  M%RHO(-1:IBP1+1,-1:JBP1+1,-1:KBP1+1),STAT=IZERO)
CALL ChkMemErr('INIT','RHO',IZERO)
ALLOCATE( M%RHOS(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
!!ALLOCATE( M%RHOS(-1:IBP1+1,-1:JBP1+1,-1:KBP1+1),STAT=IZERO)
...
!! ALLOCATE( M%YY(0:IBP1,0:JBP1,0:KBP1,N_GAS_SPECIES),STAT=IZERO)
   ALLOCATE( M%YY(-1:IBP1+1,-1:JBP1+1,-1:KBP1+1,N_GAS_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','YY',IZERO)
   M%YY = 0._EB
!! ALLOCATE(M%YYS(0:IBP1,0:JBP1,0:KBP1,N_GAS_SPECIES),STAT=IZERO)
   ALLOCATE(M%YYS(-1:IBP1+1,-1:JBP1+1,-1:KBP1+1,N_GAS_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','YYS',IZERO)
   M%YYS = 0._EB   
   ALLOCATE(M%DEL_RHO_D_DEL_Y(0:IBP1,0:JBP1,0:KBP1,N_GAS_SPECIES),STAT=IZERO)
...
NOT_EVAC_IF_1: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   M%TMP_B = TMPA
   ALLOCATE(M%YY_F(N_TOTAL_WALL_CELLS,N_GAS_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','YY_F',IZERO)  
   DO IW=1,N_TOTAL_WALL_CELLS
      M%YY_F(IW,1:N_GAS_SPECIES) = YY_AMBIENT(1:N_GAS_SPECIES)
   ENDDO
...
NOT_EVAC_IF_2: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   M%KW = 1._EB
   ALLOCATE(M%RHODW(N_TOTAL_WALL_CELLS,N_GAS_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','RHODW',IZERO) 
   M%RHODW = 1._EB
   ALLOCATE(M%AREA_ADJUST(N_TOTAL_WALL_CELLS),STAT=IZERO)
   CALL ChkMemErr('INIT','AREA_ADJUST',IZERO)  
   M%AREA_ADJUST = 1._EB
   ALLOCATE(M%MASSFLUX(N_TOTAL_WALL_CELLS,0:N_GAS_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','MASSFLUX',IZERO)  
   M%MASSFLUX = 0._EB
   ALLOCATE(M%MASSFLUX_ACTUAL(N_TOTAL_WALL_CELLS,0:N_GAS_SPECIES),STAT=IZERO)
...
IF (N_GAS_SPECIES>0 .AND. M%BOUNDARY_TYPE(IW)/=VIRTUAL_BOUNDARY .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   M%RSUM(I,J,K) = M%RSUM(IIG,JJG,KKG)
   M%YY_F(IW,1:N_GAS_SPECIES)  = M%YY(IIG,JJG,KKG,1:N_GAS_SPECIES)
   M%YY(I,J,K,1:N_GAS_SPECIES) = M%YY(IIG,JJG,KKG,1:N_GAS_SPECIES)
ENDIF
   ...
   DO K=K1+1,K2
      DO J=J1+1,J2
         DO I=I1+1,I2
            ...
            RHO(I,J,K)  = RHOA
            IF (N_GAS_SPECIES>0) THEN
               YY(I,J,K,1:N_GAS_SPECIES)  = YY_AMBIENT(1:N_GAS_SPECIES)
               YYS(I,J,K,1:N_GAS_SPECIES) = YY_AMBIENT(1:N_GAS_SPECIES)
            ENDIF
         ENDDO
      ENDDO
   ENDDO
