...
% uncomment following paragraph after we start documenting dynamic devices

%With respect to visualization in Smokeview, 
%there are two types devices: static and dynamic. 
%This terminology refers to whether the look and behavior of a device can be %modified based upon data provided in an FDS input file.  For example, the {\tt %sensor}\ device is drawn as a small green sphere with a fixed diameter.  It is a %static device.  The {\em look}\ of this device is the same regardless of how an FDS %input file is set up.  The {\tt tsphere}\ device on the other hand is dynamic.  The %diameter and an image used to cover the sphere (known as a texture map) can be %specified in an FDS input file.  It is a dynamic device.
