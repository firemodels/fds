...
It is expected that a steady-state will be achieved after about 10~s.  During evaporation the gas temperature increases slightly as the water droplets are intially warmer than the gas.  The plot on the left below shows the enthalpy of the gas, the enthalpy of the liquid water, and the sum of the two.  The summed enthalpy should not change over the simulation time.  The plot on the left shows the relative humidity.  Ideally it should be equal to 100 \%, but since FDS does not currently compute condensation a slight overshoot is not unexpected.
