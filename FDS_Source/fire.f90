...
INTEGRATION_LOOP: DO TIME_ITER = 1,TIME_ITER_MAX
   ...
   TMP_MIXED_ZONE = ((ZETA-ZETA1)*CELL_MASS*TMP(I,J,K) + SUM(SMIX_MIX_MASS_0)*TMP_GUESS_2)/(TOTAL_MIX_MASS)
...
RECURSIVE SUBROUTINE DETERMINE_EXTINCTION(ZZ_MIXED_IN,TMP_0,EXTINCT)
   ...
   CASE(EXTINCTION_1)
      DO NRR = 1,N_REACTIONS
         ...
         IF (RNN%FAST_CHEMISTRY) THEN
            IF(EXTINCT_1(ZZ_MIXED_IN,TMP_0,NRR)) THEN
               EXTINCT(NRR) = .TRUE.
            ELSE
               EXTINCT(NRR) = .FALSE.   
            ENDIF
      ...
      ENDDO
   CASE(EXTINCTION_2)
      IF(EXTINCT_2(ZZ_MIXED_IN,TMP_0)) THEN
         EXTINCT(:) = .TRUE.
      ELSE
         EXTINCT(:) = .FALSE.
      ENDIF
...
LOGICAL FUNCTION EXTINCT_2(ZZ_MIXED_IN,TMP_0)
...
ELSE     
   ...
   CALL GET_SENSIBLE_ENTHALPY(ZZ_FUEL_EXTINCT,H_F_0,TMP_0) 
   ...
   CALL GET_SENSIBLE_ENTHALPY(ZZ_FUEL_EXTINCT,H_F_0,TMP_0) 
   CALL GET_SENSIBLE_ENTHALPY(ZZ_DIL_EXTINCT,H_G_0,TMP_0) 
   H_F_N = 0._EB
   H_G_N = 0._EB
   DO NRR = 1,N_REACTIONS
      RNN => REACTION(NRR)   
...
IF (MASS_OX_STOICH > MASS_OX) THEN ! Oxidizer limited exothermic
   ...
   DO NRR = 1,N_REACTIONS
      ...
      IF (.NOT. RNN%FAST_CHEMISTRY .AND. RNN%HEAT_OF_COMBUSTION > 0._EB) THEN
         ...
         DZ_FR(NRR) = AA(NRR)*RHO(I,J,K)**RNN%RHO_EXPONENT*EXP(-EE(NRR)/(R0*TMP_MIXED_ZONE))*TMP_MIXED_ZONE**RNN%N_T
