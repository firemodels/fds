...
IF_PRESSURE_ZONES: IF (N_ZONE>0) THEN
   ...
   WALL_LOOP4: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC => WALL(IW)
      IF (INTERPOLATED_MESH(WC%BOUNDARY_COORD%IIG,WC%BOUNDARY_COORD%JJG,WC%BOUNDARY_COORD%KKG)>0) CYCLE
   ...
   WALL_LOOP3: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      WALL_CELL_TYPE: SELECT CASE (WC%BOUNDARY_TYPE)
         ...
         CASE (SOLID_BOUNDARY)
            ...
            !    IF (ICF>0) THEN
            !       IF(CUT_FACE(ICF)%NFACE>0) WC%ONE_D%U_NORMAL_S = WC%ONE_D%U_NORMAL_S * &
            !                SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE)) / WC%ONE_D%AREA
            !    ENDIF
            ! ENDIF
