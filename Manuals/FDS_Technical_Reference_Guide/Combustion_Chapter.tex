...
Suppose we are considering $n_s$ species in our reactor model.  It is actually unnecessary to solve Eq.~(\ref{eq:iem}) for each species individually. Let us define the state of mixing in the cell by the \emph{unmixed} fraction, $\zeta(t)$, such that the local mass fraction of any species may be written as
\begin{equation}\label{eq:local_mass_fraction}
Y_{\alpha}(t)=\zeta(t)+(1-\zeta(t))\overline{Y}_{\alpha}(t)
\end{equation}

Plugging Eq.~(\ref{eq:local_mass_fraction}) into Eq.~(\ref{eq:iem}) and summing over species we find that the unmixed fraction is governed by the following simple ODE:
...
\end{equation}
We may think of an initial unmixed fraction of $\zeta_0=1$ as representing a diffusion flame and $\zeta_0=0$ as a premixed flame.
...
\end{equation}
The first term on the RHS represents chemical kinetics.  The second term accounts for mixing.  Note that in the second step we have utilized the fact that the ummixed composition remains constant troughout the time step. Using Eqs.~(\ref{eq:zeta}) and (\ref{eq:mixunmix_2}), the second term becomes
\begin{align}
...
\overline{Y}_{\alpha}(t)= \zeta(t) \, \overline{Y}_{\alpha,0} + (1-\zeta(t)) \, \hat{Y}_{\alpha}(t)
\end{equation}
...
Discussion on the extension of the partially-stirred reactor model to include multiple environments as a way to model subgrid variance is included in Appendix~{\ref{multi_env_edc}}.
