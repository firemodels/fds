...
CHARACTER(30) :: ID,SURF_DEFAULT,EVAC_SURF_DEFAULT,SPECIES_ID(1:MAX_SPECIES)='null',MATL_SPEC_ID(1:MAX_REACTIONS,1:MAX_SPECIES)
...
REAL(EB) :: C_HORIZONTAL,C_VERTICAL,MW,VISCOSITY,CONDUCTIVITY, &
            FORCE_VECTOR(3)=0._EB,SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,H_FIXED
CHARACTER(30) :: RAMP_GX,RAMP_GY,RAMP_GZ,BACKGROUND_SPECIES
...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,PRESSURE_RELAX_FACTOR,RELAXATION_FACTOR,FYI, &
                ...
                VAN_DRIEST,HRRPUVCUT_MAX,EMBEDDED_MESH,RUN_AVG_FAC,THERMOPHORETIC_DEPOSITION,TURBULENT_DEPOSITION, &
...
!Background parameters only set for error trapping
BACKGROUND_SPECIES = 'null'
VISCOSITY = -1._EB
CONDUCTIVITY = -1._EB
SPECIFIC_HEAT = -1._EB
SPECIFIC_ENTHALPY = -1._EB
REFERENCE_TEMPERATURE = -1._EB
MW      = -1._EB   
...
IF (TRIM(BACKGROUND_SPECIES)/='null' .OR. VISCOSITY > 0._EB .OR. CONDUCTIVITY  > 0._EB .OR. SPECIFIC_HEAT  > 0._EB .OR. &
    SPECIFIC_ENTHALPY > 0._EB .OR. REFERENCE_TEMPERATURE > 0._EB .OR. MW > 0._EB) THEN
   WRITE(MESSAGE,'(A)')  'ERROR: BACKGROUND species data moved from MISC to SPEC'
   CALL SHUTDOWN(MESSAGE)
ENDIF
...
INTEGER  :: N_SPEC_READ,N,I,IPC,MODE,N2,I_SKIP=0
...
COUNT_SPEC_LOOP: DO
   ...
   IF (LUMPED_SPECIES_ONLY) THEN
      IF (SIMPLE_CHEMISTRY) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Cannot define additional LUMPED_SPECIES_ONLY species when using the simple REAC input'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
   ...
   ELSE
      IF (BACKGROUND) THEN 
         IF (N_MIX_SPECIES > 0 .AND. DEFINED_BACKGROUND) THEN
            WRITE(MESSAGE,'(A)') 'ERROR: Cannot define both a BACKGROUND SPEC and a BACKGROUND SMIX'
            CALL SHUTDOWN(MESSAGE)
         ENDIF
...
IF (WATER_EVAPORATION .AND. .NOT. ANY(SPECIES_ID=='WATER VAPOR')) THEN
   N_GAS_SPECIES = N_GAS_SPECIES + 1
   I_SKIP = 1
   IF (H2O_INDEX == 0) THEN
      N_SPECIES  = N_SPECIES + 1
...
IF (I_BACKGROUND == 0) THEN
   ...
   BACKGROUND_ONLY = .TRUE.
ELSE
   IF (DEFINED_BACKGROUND) I_BACKGROUND = I_BACKGROUND + N_SPECIES
ENDIF

! Add lumped species variables to the species list
I_Z_MIN = -999
I_Z_MAX = -999
N2 = N_SPECIES
...
IF(N_MIX_SPECIES > 0) THEN
   ...
   DO N = 1,N_MIX_SPECIES
      IF (N>1 .OR. .NOT. DEFINED_BACKGROUND) N_GAS_SPECIES = N_GAS_SPECIES+1
      N_SPECIES = N_SPECIES + 1
      I_Z_MAX = N_GAS_SPECIES    
      IF (TRIM(SPECIES_MIXTURE(N)%ID)=='null') THEN
         IF (DEFINED_BACKGROUND .AND. I_BACKGROUND>N2) THEN
            WRITE(SPECIES_ID(N_SPECIES),'(A,I1.1)') 'LUMPED SPECIES ',N-1
         ELSE
            WRITE(SPECIES_ID(N_SPECIES),'(A,I1.1)') 'LUMPED SPECIES ',N         
         ENDIF
      ELSE
         SPECIES_ID(N_SPECIES) = SPECIES_MIXTURE(N)%ID      
      ENDIF
   ENDDO
   IF (SIMPLE_CHEMISTRY) THEN
      I_FUEL = I_Z_MIN
...
ALLOCATE(Y2SPEC(0:N_GAS_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ','Y2SPEC',IZERO)
Y2SPEC = 0
...
SPEC_LOOP: DO N=1,N_SPEC_READ
   ...
   IF (N<=N_SPEC_READ) THEN
      ...
      IF (ID=='CARBON MONOXIDE') THEN
         IF (CO_INDEX > 0) THEN
            ...
            IF (.NOT. LUMPED_SPECIES_ONLY .AND. SIMPLE_CHEMISTRY .AND. MASS_FRACTION_0 > 0) SPECIES(CO_INDEX)%YY0 = MASS_FRACTION_0    
            N2 = N2 + 1
            CYCLE
         ELSE
            CO_INDEX = N
      ...
      IF (ID=='HYDROGEN') THEN
         IF (H2_INDEX > 0) THEN
            ...
            IF (.NOT. LUMPED_SPECIES_ONLY .AND. SIMPLE_CHEMISTRY .AND. MASS_FRACTION_0 > 0) SPECIES(H2_INDEX)%YY0 = MASS_FRACTION_0    
            N2 = N2 + 1
            CYCLE
         ELSE
            H2_INDEX = N
      ...
      IF (ID=='SOOT') THEN
         IF (SOOT_INDEX > 0) THEN
            ...
            IF (.NOT. LUMPED_SPECIES_ONLY .AND. SIMPLE_CHEMISTRY .AND. MASS_FRACTION_0 > 0) SPECIES(SOOT_INDEX)%YY0 = MASS_FRACTION_0    
            N2 = N2 + 1
            CYCLE
         ELSE
            SOOT_INDEX = N
   ...
   IF (MASS_FRACTION_0<0._EB) THEN
      IF (ID=='WATER VAPOR' .AND. H2O_INDEX <= 0) THEN
   ...
   IF (SIMPLE_CHEMISTRY) THEN
      SS => SPECIES(N_LUMPED_SPECIES + I_SKIP + N - N2) 
   ELSE
      SS => SPECIES(N)
   ENDIF
...
IF (I_BACKGROUND > N_SPEC_READ .AND. N_MIX_SPECIES == 0 ) THEN
...
IF (WATER_EVAPORATION) SPECIES(H2O_INDEX)%LUMPED_SPECIES_ONLY = .FALSE.
...
PART_CLASS_LOOP: DO IPC=1,N_PART
   ...
   IF (SIMPLE_CHEMISTRY) THEN
         IF (PC%SPEC_ID==REACTION(1)%FUEL) THEN
         PC%SPEC_ID='LUMPED SPECIES 1'
         PC%FUEL = .TRUE.
         FUEL_EVAPORATION = .TRUE.
         PC%RGB= (/255,255,0/)
      ENDIF
   ENDIF
   DO I=1,N_SPECIES
      IF (SPECIES(I)%ID==PC%SPEC_ID) THEN
         IF (SPECIES(I)%Y_INDEX <=0) THEN
...
PROP_LOOP: DO IPC=1,N_PROP
   ...
   DO I=1,N_SPECIES
      IF (SPECIES(I)%ID==PY%SPEC_ID) THEN
         IF (SPECIES(I)%Y_INDEX <=0) THEN
            WRITE(MESSAGE,'(A,A,A)') 'ERROR: PROP SPEC_ID ',TRIM(PY%SPEC_ID),' not a tracked gas species'
            CALL SHUTDOWN(MESSAGE)
         ENDIF         
              PY%SPEC_INDEX = I
...
SUBROUTINE READ_SMIX_1
CHARACTER(30) :: SPEC_ID(1:MAX_SPECIES),ID,MIXTURE_ID(0:MAX_SPECIES)
...
COUNT_SMIX_LOOP: DO
   ...
   IF (BACKGROUND) THEN
      ...
      IF (.NOT. DEFINED_BACKGROUND) THEN
         ...
         I_BACKGROUND = N_MIX_SPECIES
      ELSE
         WRITE(MESSAGE,'(A)') 'ERROR: Can only define one background mixture'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
...
SPECIES_MIXTURE%N_SUB_SPECIES = 0
SPECIES_MIXTURE(1:N_MIX_SPECIES)%ID = MIXTURE_ID(1:N_MIX_SPECIES)

END SUBROUTINE READ_SMIX_1

SUBROUTINE READ_SMIX_2
USE PROPERTY_DATA,ONLY:GAS_PROPS
...
N = N_GAS_SPECIES - N_MIX_SPECIES
IF (DEFINED_BACKGROUND) N = N + 1
...
DO NS=1,N_MIX_SPECIES
   ...
   IF (N_SPECIES - N_MIX_SPECIES + NS == I_BACKGROUND) THEN
      SPECIES(0) = SPECIES(N_SPECIES - N_MIX_SPECIES + NS)
   ELSE
      N= N + 1
      SS%Y_INDEX = N
      Y2SPEC(N) = N_SPECIES - N_MIX_SPECIES + NS
   ENDIF
ENDDO

END SUBROUTINE READ_SMIX_2
...
COUNT_REAC_LOOP: DO
   ...
   IF ((BOF > 0._EB .OR. E > 0._EB) .AND. SIMPLE_CHEMISTRY) THEN
...
IF (FUEL_EVAPORATION) SIMPLE_CHEMISTRY = .TRUE.
IF (SIMPLE_CHEMISTRY .AND. N_REACTIONS==0) N_REACTIONS = 1
...
DO N=1,N_Y_ARRAY
   SPECIES(N)%INDEX = N
   IF (SPECIES(N)%Y_INDEX==I_SOOT .AND. SOOT_INDEX == 0) SOOT_INDEX = N
   IF (SPECIES(N)%Y_INDEX==I_N2 .AND. N2_INDEX == 0) N2_INDEX = N 
   IF (SPECIES(N)%Y_INDEX==I_O2 .AND. O2_INDEX == 0) O2_INDEX = N
   IF (SPECIES(N)%Y_INDEX==I_WATER .AND. H2O_INDEX == 0) H2O_INDEX = N
   IF (SPECIES(N)%Y_INDEX==I_H2 .AND. H2_INDEX == 0) H2_INDEX = N
   IF (SPECIES(N)%Y_INDEX==I_CO .AND. CO_INDEX == 0)  CO_INDEX = N
   IF (SPECIES(N)%Y_INDEX==I_CO2 .AND. CO2_INDEX == 0) CO2_INDEX = N      
ENDDO
...
PART_LOOP: DO N=1,N_PART
   ...
   IF (SIMPLE_CHEMISTRY) THEN
      IF(PC%SPEC_INDEX==Y2SPEC(I_FUEL)) SPEC_ID = REACTION(1)%FUEL
   ENDIF
   TMP_REF = -1._EB
   TMP_MELT = -1._EB
   TMP_V = -1._EB
   ...
   IF (PC%SPEC_INDEX>0) THEN
      ...
      YY_GET = 0._EB
      YY_GET(SPECIES(PC%SPEC_INDEX)%Y_INDEX) = 1._EB
...
READ_PART_LOOP: DO N=1,N_PART
   ...
   IF ((TRIM(SPEC_ID)=='FUEL' .OR. TRIM(SPEC_ID)=='fuel') .AND. SIMPLE_CHEMISTRY) THEN 
      SPEC_ID= 'LUMPED SPECIES 1'
      FUEL_EVAPORATION = .TRUE.
      PC%FUEL = .TRUE.
      IF (ANY(RGB<0) .AND. COLOR=='null') COLOR='YELLOW'
   ELSEIF (TRIM(SPEC_ID)=='WATER VAPOR' .OR. TRIM(SPEC_ID)=='water vapor') THEN
   ...
   IF (SIMPLE_CHEMISTRY) THEN
      IF (TRIM(SPEC_ID)==REACTION(1)%FUEL) THEN
         SPEC_ID= 'LUMPED SPECIES 1'
         FUEL_EVAPORATION = .TRUE.
         PC%FUEL = .TRUE.
         IF (ANY(RGB<0) .AND. COLOR=='null') COLOR='YELLOW'
   ...
   IF ((MELTING_TEMPERATURE > -300._EB .OR. SPECIFIC_HEAT > 0._EB .OR. HEAT_OF_VAPORIZATION > 0._EB .OR. &
      ...
      WRITE(MESSAGE,'(A,I,A)') 'ERROR: PART line ',N,' requires a SPEC_ID.'
      CALL SHUTDOWN(MESSAGE)
   ENDIF  
...
CHARACTER(30) :: CONDUCTIVITY_RAMP,SPECIFIC_HEAT_RAMP,SPEC_ID(1:MAX_REACTIONS,1:MAX_SPECIES)
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   IF (ANY(ML%NU_GAS > 0._EB)) THEN
      NU_GAS = ML%NU_GAS
      DO NR=1,ML%N_REACTIONS
         IF(TRIM(MATL_SPEC_ID(NR,1))=='null') THEN
            WRITE(MESSAGE,'(A,A,A,I3,A)') 'ERROR: MATL ',TRIM(MATL_NAME(N)),' reaction ',NR,' requires SPEC_ID'
            CALL SHUTDOWN(MESSAGE)                  
         ENDIF
         DO NN=1,N_GAS_SPECIES
            IF (TRIM(MATL_SPEC_ID(NR,NN))=='null') EXIT
            DO NNN=1,N_SPECIES
               IF (TRIM(MATL_SPEC_ID(NR,NN))==TRIM(SPECIES(NNN)%ID)) THEN
                  IF (SPECIES(NNN)%Y_INDEX<=0) THEN
                     WRITE(MESSAGE,'(A,A,A,A)') 'ERROR: SPEC:',TRIM(MATL_SPEC_ID(NN,NNN)),&
                                                ' is not a tracked species on MATL ',TRIM(MATL_NAME(N))
                     CALL SHUTDOWN(MESSAGE)                  
                  ENDIF
                  ML%NU_GAS(NR,SPECIES(NNN)%Y_INDEX) = NU_GAS(NR,NN)
                  EXIT
               ENDIF
               IF (NNN==N_SPECIES) THEN
                  WRITE(MESSAGE,'(A,A,A,A)') 'ERROR: SPEC:',TRIM(MATL_SPEC_ID(NN,NNN)),&
                                             ' is not found on MATL ',TRIM(MATL_NAME(N))
                  CALL SHUTDOWN(MESSAGE)                  
               ENDIF
            ENDDO
         ENDDO
...
SUBROUTINE READ_SURF(ENTRY_NUMBER)
 
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX
USE DEVICE_VARIABLES, ONLY : CABLE,N_CABL,PROPERTY_TYPE,PROPERTY
INTEGER, INTENT(IN) :: ENTRY_NUMBER
...
COUNT_SURF_LOOP: DO
   ...
   34 IF (IOS>0) THEN
         ...
         CALL SHUTDOWN(MESSAGE)
      ENDIF
   IF (ENTRY_NUMBER == 1 .AND. (HRRPUA>0._EB .OR. MLRPUA>0._EB)) SIMPLE_CHEMISTRY=.TRUE.
ENDDO COUNT_SURF_LOOP

!If 1st entry just check for HRRPUA and MLRPUA      
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   DO NN=1,SF%N_MATL
      ...
      DO J=1,ML%N_REACTIONS
         ...
         ENDDO
         IF (ML%NU_RESIDUE(J)==0._EB) SF%SHRINK = .TRUE.
      ENDDO
...
REAL(EB) :: TEMPERATURE,DENSITY,MASS_FRACTION(1:MAX_SPECIES),RR_SUM,YY_GET(1:N_GAS_SPECIES),MASS_PER_VOLUME,MASS_PER_TIME,DT_INSERT, &
...
INIT_LOOP: DO N=1,N_INIT_READ
   ...
   DT_INSERT                  = -1._EB
   MASS_FRACTION              =  0._EB
   IF (N_GAS_SPECIES > 0) MASS_FRACTION(1:N_GAS_SPECIES) = YY_AMBIENT(1:N_GAS_SPECIES)
   MASS_PER_TIME              = -1._EB
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            SPEC_INIT_IF:IF (N_GAS_SPECIES > 0._EB .AND. ANY(MASS_FRACTION > 0._EB)) THEN
               IF (SPEC_ID(1)=='null') THEN
...
IF (SPEC_ID(1:17)=='MIXTURE_FRACTION_' .OR. SPEC_ID(1:17)=='MIXTURE FRACTION ') SPEC_ID='LUMPED SPECIES '//TRIM(SPEC_ID(18:30))
...
IF (SPEC_ID/='null') THEN
   DO NS=1,N_SPECIES
      IF (SPEC_ID==SPECIES(NS)%ID) THEN
         IF (SPECIES(NS)%GAS_SPECIES_ONLY .OR. SPECIES(NS)%MODE==LUMPED_SPECIES) THEN
            SPEC_INDEX = SPECIES(NS)%Y_INDEX
         ELSE
            SPEC_INDEX = -NS
         ENDIF
      ENDIF
...
IF (QUANTITY=='VOLUME FRACTION' .AND. (SPEC_ID=='MIXTURE FRACTION' .OR. SPEC_ID(1:15)=='LUMPED SPECIES')) THEN
...
IF (SPEC_INDEX > 0) THEN
   IF (Y2SPEC(SPEC_INDEX)==I_BACKGROUND) SPEC_INDEX=-Y2SPEC(SPEC_INDEX)
ENDIF
...
QUANTITY_INDEX_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
  ...
  QUANTITY_IF:IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
     ...
     IF (QUANTITY=='RELATIVE HUMIDITY' .AND. ((.NOT. SIMPLE_CHEMISTRY .AND. I_WATER==0 .OR. &
                                               SIMPLE_CHEMISTRY .AND. H2O_INDEX==0))) THEN
        WRITE(MESSAGE,'(A)')  'ERROR: RELATIVE HUMIDTY requires water droplets, SPEC=WATER VAPOR, or SIMPLE_CHEMISTRY'
        CALL SHUTDOWN(MESSAGE)
     END IF
     ...
     IF (SPEC_INDEX>0) THEN
        SMOKEVIEW_LABEL = TRIM(SPECIES(Y2SPEC(SPEC_INDEX))%ID)//' '//TRIM(QUANTITY)
        SMOKEVIEW_BAR_LABEL = TRIM(OUTPUT_QUANTITY(ND)%SHORT_NAME)//'_'//TRIM(SPECIES(Y2SPEC(SPEC_INDEX))%FORMULA)
