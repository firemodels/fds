...
if strcmp(Stats_Output, 'Validation') && (exist('Output_File_Baseline','var') == 1)
    ...
    stats_baseline{5} = cellfun(@str2num, stats_baseline{5});
    stats_baseline{6} = cellfun(@str2num, stats_baseline{6});
    stats_current{5} = cellfun(@str2num, stats_current{5});
    stats_current{6} = cellfun(@str2num, stats_current{6});
