...
REAL(EB), ALLOCATABLE, DIMENSION(:) :: MU_TMP,CP_TMP,K_TMP,H_TMP,D_TMP,G_F_TMP,ZZ_GET, &
                                       MU_TMP_Z,CP_TMP_Z,K_TMP_Z,H_TMP_Z,D_TMP_Z
REAL(EB) :: CP1,CP2,H1
INTEGER :: NN,N
TYPE(SPECIES_TYPE), POINTER :: SS=>NULL()
TYPE(SPECIES_MIXTURE_TYPE), POINTER :: SM=>NULL()
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2), &
            TAU_EXTERNAL_FLUX,TAU_EF,E_COEFFICIENT,VOLUME_FLOW_RATE,TMP_FRONT,TMP_INNER(MAX_LAYERS),THICKNESS(MAX_LAYERS),VEL, &
...
NAMELIST /SURF/ ADIABATIC,BACKING,BURN_AWAY,CELL_SIZE_FACTOR,C_FORCED_CONSTANT,C_FORCED_PR_EXP,C_FORCED_RE,C_FORCED_RE_EXP,&
                ...
                VEL,VEL_GRAD,VEL_T,VOLUME_FLOW_RATE,WIDTH,XYZ,Z0,&
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   SF%VEG_LSET_QCON     = -VEG_LSET_QCON*1000._EB !convert to W
   ...
   SELECT CASE (PROFILE)
      ...
      CASE('1D-PARABOLIC')
         SF%PROFILE        = ONED_PARABOLIC_PROFILE
      CASE('RAMP')
   ...
   SF%VEL_T                = VEL_T
   SF%VOLUME_FLOW_RATE     = VOLUME_FLOW_RATE
   SF%WIDTH                = WIDTH
   SF%Z0                   = Z0
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
   ...
   BLOWING  = .FALSE.
   SUCKING  = .FALSE.
   IF (SF%VEL<0._EB .OR. SF%VOLUME_FLOW_RATE<0._EB .OR. SF%MASS_FLUX_TOTAL < 0._EB) BLOWING = .TRUE.
   IF (SF%VEL>0._EB .OR. SF%VOLUME_FLOW_RATE>0._EB .OR. SF%MASS_FLUX_TOTAL > 0._EB) SUCKING = .TRUE.
               ...
               IF (OB%THIN) THEN
                  FACE_LOOP_2: DO NNN=-3,3
                     ...
                     IF (ABS(SURFACE(OB%SURF_INDEX(NNN))%VEL)             >TWO_EPSILON_EB .OR. &
                         ABS(SURFACE(OB%SURF_INDEX(NNN))%VOLUME_FLOW_RATE)>TWO_EPSILON_EB .OR. &
