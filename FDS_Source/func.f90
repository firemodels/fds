...
USE GLOBAL_CONSTANTS, ONLY: RADIATION,SIGMA,TMPA4,OPEN_BOUNDARY,VIRTUAL_BOUNDARY,EVACUATION_ONLY,TMPA
...
CALL ALLOCATE_STORAGE(NM,WC%SURF_INDEX,WALL_INDEX=IW)
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,NRA=>NUMBER_RADIATION_ANGLES,NSB=>NUMBER_SPECTRAL_BANDS
...
SF%ONE_D_REALS_ARRAY_SIZE(7) = NRA*NSB                  ! ILW
...
USE GLOBAL_CONSTANTS, ONLY: SIGMA,TMPA4,TMPA,RPI,N_TRACKED_SPECIES,NRA=>NUMBER_RADIATION_ANGLES,NSB=>NUMBER_SPECTRAL_BANDS
...
IF (PRESENT(WALL_INDEX)) THEN
   ...
   OS => MESHES(NM)%ONE_D_WALL_STORAGE(SURF_INDEX)
   ARRAY_INDEX = WALL_INDEX
   REALS_DIMENSION    = SF%N_ONE_D_STORAGE_REALS
   INTEGERS_DIMENSION = SF%N_ONE_D_STORAGE_INTEGERS
   LOGICALS_DIMENSION = SF%N_ONE_D_STORAGE_LOGICALS
...
ELSEIF(PRESENT(LP_INDEX)) THEN
   ...
   IF (LP_INDEX>MESHES(NM)%NLPDIM) THEN
      ...
      ALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(MESHES(NM)%NLPDIM+1000))
      MESHES(NM)%LAGRANGIAN_PARTICLE(1:MESHES(NM)%NLPDIM) = LP_DUMMY
      DEALLOCATE(LP_DUMMY)
      MESHES(NM)%NLPDIM = MESHES(NM)%NLPDIM + 1000
   ENDIF
...
IF (STORAGE_INDEX_OLD>0) THEN
   ...
   NEW_ONE_D = .FALSE.
ELSEIF (STORAGE_INDEX_OPEN<1000000) THEN
...
!if (PRESENT(WALL_INDEX)) write(0,*) 're-allocate ONE_D wall storage arrays, SURF_INDEX=', SURF_INDEX,' slots=',OS%N_STORAGE_SLOTS
!if (PRESENT(LP_INDEX)) write(0,*) 're-allocate part storage arrays', OS%N_STORAGE_SLOTS,nm
   ...
   ENDIF
   ALLOCATE(OS%REALS(REALS_DIMENSION,OS%N_STORAGE_SLOTS+1000))
   ALLOCATE(OS%INTEGERS(INTEGERS_DIMENSION,OS%N_STORAGE_SLOTS+1000))
   ALLOCATE(OS%LOGICALS(LOGICALS_DIMENSION,OS%N_STORAGE_SLOTS+1000))
   OS%INTEGERS = 0
   IF (OS%N_STORAGE_SLOTS>0) THEN
      ...
      DO I=1,OS%N_STORAGE_SLOTS
         ...
         STORAGE_INDEX = I
         IF (PRESENT(WALL_INDEX)) CALL ONE_D_POINTERS(OLD_INDEX,.FALSE.,0,0,0)
   ...
   OS%N_STORAGE_SLOTS = OS%N_STORAGE_SLOTS+1000
ENDIF

! Assign values to the pointer arrays

IF (PRESENT(WALL_INDEX)) CALL ONE_D_POINTERS(ARRAY_INDEX,NEW_ONE_D,0,0,0)
IF (PRESENT(LP_INDEX)) CALL PARTICLE_POINTERS(ARRAY_INDEX,NEW_ONE_D)
...
LP%IOR                 => OS%INTEGERS( 5,STORAGE_INDEX) ; IF (NEW) LP%IOR = 0
LP%SPLIT_IOR           => OS%INTEGERS( 6,STORAGE_INDEX) ; IF (NEW) LP%SPLIT_IOR = 0
LP%SURF_INDEX          => OS%INTEGERS( 7,STORAGE_INDEX) ; IF (NEW) LP%SURF_INDEX = 0
LP%WALL_INDEX          => OS%INTEGERS( 8,STORAGE_INDEX) ; IF (NEW) LP%WALL_INDEX = 0
LP%IIG                 => OS%INTEGERS( 9,STORAGE_INDEX)
LP%JJG                 => OS%INTEGERS(10,STORAGE_INDEX)
LP%KKG                 => OS%INTEGERS(11,STORAGE_INDEX)
LP%VEG_N_TREE_OUTPUT   => OS%INTEGERS(12,STORAGE_INDEX) ; IF (NEW) LP%VEG_N_TREE_OUTPUT = 0

! Assign logical pointers and values

LP%ALREADY_ALLOCATED => OS%LOGICALS( 1,STORAGE_INDEX) ; IF (NEW) LP%ALREADY_ALLOCATED = .FALSE.
LP%SHOW              => OS%LOGICALS( 2,STORAGE_INDEX) ; IF (NEW) LP%SHOW              = .FALSE.
LP%SPLAT             => OS%LOGICALS( 3,STORAGE_INDEX) ; IF (NEW) LP%SPLAT             = .FALSE.
LP%IGNITOR           => OS%LOGICALS( 4,STORAGE_INDEX) ; IF (NEW) LP%IGNITOR           = .FALSE.
LP%EVAPORATE         => OS%LOGICALS( 5,STORAGE_INDEX) ; IF (NEW) LP%EVAPORATE         = .FALSE.

! Assign real pointers and values

LP%TMP_F             => OS%REALS( 1,STORAGE_INDEX) ; IF (NEW) LP%TMP_F   = TMPA 
LP%X                 => OS%REALS( 2,STORAGE_INDEX) ; IF (NEW) LP%X       = 0._EB
LP%Y                 => OS%REALS( 3,STORAGE_INDEX) ; IF (NEW) LP%Y       = 0._EB
LP%Z                 => OS%REALS( 4,STORAGE_INDEX) ; IF (NEW) LP%Z       = 0._EB
LP%U                 => OS%REALS( 5,STORAGE_INDEX) ; IF (NEW) LP%U       = 0._EB
LP%V                 => OS%REALS( 6,STORAGE_INDEX) ; IF (NEW) LP%V       = 0._EB
LP%W                 => OS%REALS( 7,STORAGE_INDEX) ; IF (NEW) LP%W       = 0._EB
LP%PWT               => OS%REALS( 8,STORAGE_INDEX) ; IF (NEW) LP%PWT     = 1._EB
LP%ACCEL_X           => OS%REALS( 9,STORAGE_INDEX) ; IF (NEW) LP%ACCEL_X = 0._EB
LP%ACCEL_Y           => OS%REALS(10,STORAGE_INDEX) ; IF (NEW) LP%ACCEL_Y = 0._EB
LP%ACCEL_Z           => OS%REALS(11,STORAGE_INDEX) ; IF (NEW) LP%ACCEL_Z = 0._EB
LP%T                 => OS%REALS(12,STORAGE_INDEX) ; IF (NEW) LP%T       = -1.E9_EB
LP%RE                => OS%REALS(13,STORAGE_INDEX) ; IF (NEW) LP%RE      = 0._EB
LP%MASS              => OS%REALS(14,STORAGE_INDEX) ; IF (NEW) LP%MASS    = 0._EB
LP%TMP_B             => OS%REALS(15,STORAGE_INDEX) ; IF (NEW) LP%TMP_B   = 0._EB
LP%VEG_KAPPA         => OS%REALS(16,STORAGE_INDEX)
LP%VEG_EMISS         => OS%REALS(17,STORAGE_INDEX)
LP%VEG_DIVQC         => OS%REALS(18,STORAGE_INDEX)
LP%VEG_PACKING_RATIO => OS%REALS(19,STORAGE_INDEX)
LP%VEG_FUEL_MASS     => OS%REALS(20,STORAGE_INDEX)
LP%VEG_MOIST_MASS    => OS%REALS(21,STORAGE_INDEX)
LP%VEG_IGN_TON       => OS%REALS(22,STORAGE_INDEX)
LP%VEG_IGN_TOFF      => OS%REALS(23,STORAGE_INDEX)
LP%VEG_IGN_TRAMPON   => OS%REALS(24,STORAGE_INDEX)
LP%VEG_IGN_TRAMPOFF  => OS%REALS(25,STORAGE_INDEX)
LP%VEG_CHAR_MASS     => OS%REALS(26,STORAGE_INDEX)
LP%VEG_SV            => OS%REALS(27,STORAGE_INDEX)
LP%VEG_MLR           => OS%REALS(28,STORAGE_INDEX)
LP%VEG_VOLFRACTION   => OS%REALS(29,STORAGE_INDEX)

CALL ONE_D_POINTERS(LP_INDEX_LOCAL,NEW,29,12,5)
...
I1 = IC+3 ; I2 = I1 + SF%ONE_D_INTEGERS_ARRAY_SIZE(1) - 1
...
I1 = RC+8 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(1) - 1
ONE_D%MASSFLUX_ACTUAL(0:)             => OS%REALS(I1:I2,STORAGE_INDEX) ; IF (NEW) ONE_D%MASSFLUX_ACTUAL = 0._EB

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(2) - 1
ONE_D%MASSFLUX(0:)                    => OS%REALS(I1:I2,STORAGE_INDEX) ; IF (NEW) ONE_D%MASSFLUX = 0._EB

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(3) - 1
ONE_D%X(0:)                           => OS%REALS(I1:I2,STORAGE_INDEX) ; IF (NEW) ONE_D%X = SF%X_S

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(4) - 1
ONE_D%TMP(0:)                         => OS%REALS(I1:I2,STORAGE_INDEX) ; IF (NEW) ONE_D%TMP(0:SF%N_CELLS+1) = SF%TMP_INNER(1)

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(5) - 1
DUMMY(1:)                             => OS%REALS(I1:I2,STORAGE_INDEX)
ONE_D%RHO(0:SF%N_CELLS+1,1:SF%N_MATL) => DUMMY(1:I2-I1+1)              ; IF (NEW) ONE_D%RHO = SF%RHO_0

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(6) - 1
ONE_D%LAYER_THICKNESS(1:)             => OS%REALS(I1:I2,STORAGE_INDEX) ; IF (NEW) ONE_D%LAYER_THICKNESS = SF%LAYER_THICKNESS

I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(7) - 1
DUMMY(1:)                             => OS%REALS(I1:I2,STORAGE_INDEX)
ONE_D%ILW(1:NRA,1:NSB)                => DUMMY(1:I2-I1+1)              ; IF (NEW) ONE_D%ILW = SIGMA*TMPA4*RPI
