...
\frac{\d Y}{\d t} = f(Y)
\end{equation}
where $f(Y)$ represents a reaction rate law and $Y$ is a species mass fraction.  The total time interval is the FDS time step, $\delta t$, and the iteration substep is $h$. The RK2 scheme is given by 
\begin{align}
\label{RK2}
Y^* &= Y^k + hf(Y^k) \notag\\
Y^{k+1}   &= \frac{1}{2}\,(Y^k + Y^* + hf(Y^*))
\end{align}
