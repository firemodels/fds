...
CHARACTER(30) :: ID,LABEL,MB,ODE_SOLVER,EXTINCTION_MODEL
CHARACTER(100) :: MESSAGE,FYI
CHARACTER(30) :: SURF_DEFAULT='INERT',EVAC_SURF_DEFAULT='INERT',FUEL_RADCAL_ID='null',LES_FILTER_WIDTH='null', &
                 SPEC_ALIAS_ID(MAX_SPECIES)='null'
...
REAL(EB) :: REAC_ATOM_ERROR,REAC_MASS_ERROR,SPEC_ALIAS_YY0(MAX_SPECIES)=0._EB
INTEGER  :: I,J,K,IZERO,IOS,BACKGROUND_SPEC_INDEX=-1,BACKGROUND_SMIX_INDEX=-1,SPEC_ALIAS_INDEX(MAX_SPECIES)=0,N_ALIAS=0
...
USE PHYSICAL_FUNCTIONS, ONLY: LES_FILTER_WIDTH_FUNCTION,LES_FILTER_WIDTH_2D_MAX,LES_FILTER_WIDTH_2D_MEAN,LES_FILTER_WIDTH_3D_MAX,&
                              LES_FILTER_WIDTH_3D_MEAN,LES_FILTER_WIDTH_2D_MIN,LES_FILTER_WIDTH_3D_MIN
...
ALLOCATE(LOOP_QRW(NMESHES),STAT=IZERO)
CALL ChkMemErr('READ','LOOP_QRW',IZERO)
LOOP_QRW = .FALSE.
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IF (TWO_D) THEN
               SELECT CASE (TRIM(LES_FILTER_WIDTH))
                  CASE ('MIN','min')
                     LES_FILTER_WIDTH_FUNCTION=>LES_FILTER_WIDTH_2D_MIN
                  CASE ('MAX','max','null')
                     LES_FILTER_WIDTH_FUNCTION=>LES_FILTER_WIDTH_2D_MAX
                  CASE ('MEAN','mean')
                     LES_FILTER_WIDTH_FUNCTION=>LES_FILTER_WIDTH_2D_MEAN
               END SELECT
            ELSE
               SELECT CASE (TRIM(LES_FILTER_WIDTH))
                  CASE ('MIN','min')
                     LES_FILTER_WIDTH_FUNCTION=>LES_FILTER_WIDTH_3D_MIN
                  CASE ('MAX','max','null')
                     LES_FILTER_WIDTH_FUNCTION=>LES_FILTER_WIDTH_3D_MAX
                  CASE ('MEAN','mean')
                     LES_FILTER_WIDTH_FUNCTION=>LES_FILTER_WIDTH_3D_MEAN
               END SELECT
            ENDIF            
...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                CDF_CUTOFF,CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,CHECK_HT,CHECK_VN,CLIP_MASS_FRACTION,&
                COMPUTE_VISCOSITY_TWICE,CONSTANT_SPECIFIC_HEAT,C_DEARDORFF,C_SMAGORINSKY,C_VREMAN,C_FORCED,&
                C_FORCED_CYLINDER,C_FORCED_SPHERE,C_HORIZONTAL,C_VERTICAL,COMBINED_DEPOSITION,DNS,DRIFT_FLUX,&
                EVACUATION_DRILL,EVACUATION_MC_MODE,EVAC_PRESSURE_ITERATIONS,EVAC_TIME_ITERATIONS,&
                EVAPORATION_DRAG,PARTICLE_CFL_MAX,PARTICLE_CFL,&
                FLUX_LIMITER,FORCE_VECTOR,FREEZE_VELOCITY,FYI,GAMMA,GRAVITATIONAL_DEPOSITION,&
                GRAVITATIONAL_DEPOSITION_CEILING,GROUND_LEVEL,GVEC,HRRPUV_MAX_SMV,&
                IMMERSED_BOUNDARY_METHOD,INCOMPRESSIBLE,INITIAL_UNMIXED_FRACTION,&
                KINETIC_ENERGY_SOURCE,LAPSE_RATE,LES_FILTER_WIDTH,&
                MAXIMUM_VISIBILITY,MEAN_FORCING,NEW_CP_WHILE,NEW_EVAP_DROP,NOISE,NOISE_VELOCITY,NO_EVACUATION,&
                OVERWRITE,PERIODIC_TEST,POROUS_FLOOR,PR,PROJECTION,P_INF,RAMP_GX,RAMP_GY,RAMP_GZ,RESEARCH_MODE,&
                RESTART,RESTART_CHID,RICHARDSON_ERROR_TOLERANCE,RUN_AVG_FAC,SC,SECOND_ORDER_PARTICLE_TRANSPORT,&
                SHARED_FILE_SYSTEM,SLIP_CONDITION,SMOKE_ALBEDO,SOLID_PHASE_ONLY,&
                STRATIFICATION,TEMPERATURE_DEPENDENT_REACTION,TENSOR_DIFFUSIVITY,TERRAIN_CASE,TERRAIN_IMAGE,TEXTURE_ORIGIN,&
                THERMOPHORETIC_DEPOSITION,THICKEN_OBSTRUCTIONS,TMPA,TURBULENCE_MODEL,TURBULENT_DEPOSITION,U0,UVW_FILE,V0,&
                VEG_LEVEL_SET,VISIBILITY_FACTOR,VN_MAX,VN_MIN,W0,WIND_ONLY,&
...
NAMELIST /DUMP/ COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,CUTCELL_DATA_FILE,&
                ...
                FLUSH_FILE_BUFFERS,MASS_FILE,MAXIMUM_PARTICLES,NFRAMES,PLOT3D_PART_ID,PLOT3D_QUANTITY,PLOT3D_SPEC_ID,&
                PLOT3D_VELO_INDEX,RENDER_FILE,SIG_FIGS,SIG_FIGS_EXP,SMOKE3D,SMOKE3D_QUANTITY,SMOKE3D_SPEC_ID,STATUS_FILES,&
...
INTEGER  :: N_SPEC_READ,N,NN,NR,N_SPEC_READ_2
LOGICAL  :: ABSORBING,SMIX_COMPONENT_ONLY,PREDEFINED(MAX_SPECIES),AEROSOL,BACKGROUND
CHARACTER(30) :: PREDEFINED_SPEC_ID(MAX_SPECIES),RAMP_CP,RAMP_CP_L,RAMP_K,RAMP_MU,RAMP_D,ALIAS,RADCAL_ID,SPEC_ID_READ(MAX_SPECIES)
CHARACTER(100) :: FORMULA
TYPE(SPECIES_TYPE), POINTER :: SS=>NULL()
NAMELIST /SPEC/ ABSORBING,AEROSOL,ALIAS,BACKGROUND,CONDUCTIVITY,CONDUCTIVITY_SOLID, &
                ...
                MASS_FRACTION_0,MEAN_DIAMETER,MELTING_TEMPERATURE,MW,RADCAL_ID,RAMP_CP,RAMP_CP_L,RAMP_D,RAMP_K,RAMP_MU,&
                REFERENCE_ENTHALPY,REFERENCE_TEMPERATURE,SIGMALJ,SMIX_COMPONENT_ONLY,SPECIFIC_HEAT,SPECIFIC_HEAT_LIQUID,&
                VAPORIZATION_TEMPERATURE,VISCOSITY
...
COUNT_SPEC_LOOP: DO
   ...
   IF (TRIM(ALIAS)/='null') THEN
      IF (BACKGROUND) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Cannot defined an aliased species as the BACKGROUND species'
         CALL SHUTDOWN(MESSAGE)         
      ENDIF
      CYCLE COUNT_SPEC_LOOP      
   ENDIF

   DO NN = 1,N_SPEC_READ-1
      IF (ID==SPEC_ID_READ(NN)) THEN
         WRITE(MESSAGE,'(A,I2,A,I2,A)') 'ERROR: Species ',N_SPEC_READ,' has the same ID as species ',NN, ' without using ALIAS'
         CALL SHUTDOWN(MESSAGE)              
      ENDIF
   ENDDO   
...
IF (.NOT. SIMPLE_CHEMISTRY .AND. BACKGROUND_SPEC_INDEX<1 .AND. .NOT.BACKGROUND_SMIX_DECLARED) THEN
   N_SPECIES = N_SPECIES + 1
   PREDEFINED(1)         = .TRUE.
...
N_SPEC_READ = 0
SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   N = N + 1
   ...
   IF (ALIAS/='null') THEN
      ...
      SPEC_ALIAS_ID(N_ALIAS)=ALIAS
      SPEC_ALIAS_YY0(N_ALIAS) = MASS_FRACTION_0
      DO NN=1,N
         IF (TRIM(ID)==SPECIES(NN)%ID) THEN
            SPEC_ALIAS_INDEX(N_ALIAS)=NN
         ENDIF
      ENDDO
      IF(SPEC_ALIAS_INDEX(N_ALIAS)==0) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: Species ',N_SPEC_READ, ' referenced ID for an ALIAS species must be defined first'
         CALL SHUTDOWN(MESSAGE)         
      ENDIF
   ...
   CALL GAS_PROPS(SS%DATA_ID,SS%SIG,SS%EPSK,SS%MW,SS%ABSORBING,SS%FORMULA,SS%LISTED,SS%ATOMS,SS%H_F,SS%RADCAL_ID)  
   CALL FED_PROPS(SS%DATA_ID,SS%FLD_LETHAL_DOSE,SS%FIC_CONCENTRATION)         
...
SUBROUTINE READ_SMIX(IMODE)

! Read SMIX lines. Note that SMIX Namelist is defined at the top of the module because it is used in several subroutines.

INTEGER, INTENT(IN) :: IMODE
CHARACTER(100) :: FORMULA
CHARACTER(30) :: SPEC_ID(1:MAX_SPECIES)
REAL(EB) :: CONVERSION,MASS_FRACTION_0,VOLUME_FRACTION(MAX_SPECIES),MASS_FRACTION(MAX_SPECIES)
INTEGER :: N,NS,NS2,N_SUB_SPECIES,N_SMIX_READ,SPEC_INDEX(0:MAX_SPECIES)=-1,Y_INDEX(N_SPECIES),ALIAS_INDEX(0:MAX_SPECIES)=0
LOGICAL :: PREDEFINED(0:MAX_SPECIES),ALIAS(0:MAX_SPECIES),BACKGROUND
...
! Look through the input file to see if there are any 'BACKGROUND' SMIX lines

REWIND(LU_INPUT)
SCAN_SMIX_LOOP: DO
   BACKGROUND = .FALSE.
   CALL CHECKREAD('SMIX',LU_INPUT,IOS)
   IF (IOS==1) EXIT SCAN_SMIX_LOOP
   READ(LU_INPUT,NML=SMIX,END=19,ERR=20,IOSTAT=IOS)
   IF (BACKGROUND) BACKGROUND_SMIX_DECLARED = .TRUE.
   20 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Problem with SMIX line'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
ENDDO SCAN_SMIX_LOOP
19 CONTINUE
...
COUNT_SMIX_LOOP: DO
   ...
   DO N=1,N_SPECIES
      IF (TRIM(ID)==SPECIES(N)%ID) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SMIX ',N_SMIX_READ,' has the same ID as a SPEC'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
   ENDDO
   DO N=1,N_ALIAS
      IF (TRIM(ID)==SPEC_ALIAS_ID(N)) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SMIX ',N_SMIX_READ,' has the same ID as a ALIAS'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
   ENDDO
...
IF (N_ALIAS>0) THEN
   DO N=1,N_ALIAS
      N_TRACKED_SPECIES = N_TRACKED_SPECIES + 1
      PREDEFINED(N_TRACKED_SPECIES) = .TRUE.
      ALIAS(N_TRACKED_SPECIES) = .TRUE.
      ALIAS_INDEX(N_TRACKED_SPECIES) = N
      SPEC_INDEX(N_TRACKED_SPECIES) = SPEC_ALIAS_INDEX(N)
   ENDDO
ENDIF
...
READ_SMIX_LOOP: DO N=0,N_TRACKED_SPECIES  
   ...
   IF (PREDEFINED(N)) THEN
      ...
      IF (ALIAS(N)) THEN
         ID = SPEC_ALIAS_ID(ALIAS_INDEX(N))
         MASS_FRACTION_0 = SPEC_ALIAS_YY0(ALIAS_INDEX(N))
      ENDIF
   ELSE
...
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX
CHARACTER(30) :: FUEL,RADCAL_ID='null'
CHARACTER(100) :: FORMULA
CHARACTER(30), TARGET  :: SPEC_ID_NU(MAX_SPECIES),SPEC_ID_N(MAX_SPECIES)
CHARACTER(255) :: EQUATION
CHARACTER(100) :: FWD_ID,A_RAMP,E_RAMP
INTEGER :: NR,NR2,NS,NS2
...
NAMELIST /REAC/ A,A_RAMP,AUTO_IGNITION_TEMPERATURE,BETA_EDC,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                ...
                REAC_MASS_ERROR,REVERSIBLE,SOOT_H_FRACTION,SOOT_YIELD,SPEC_ID_N,SPEC_ID_NU,SUPPRESSION,TAU_CHEM,TAU_FLAME,&
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (.NOT.SIMPLE_CHEMISTRY) THEN
      ...
      DO NS=1,MAX_SPECIES
         IF (TRIM(SPEC_ID_N(NS))/='null') THEN
            NS2=NS2+1
         ELSE
            EXIT
         ENDIF
   ...
   IF (RN%N_SPEC > 0) THEN
      ...
      ALLOCATE(RN%SPEC_ID_N_READ(RN%N_SPEC))
      RN%SPEC_ID_N_READ = 'null'
      RN%SPEC_ID_N_READ(1:RN%N_SPEC)=SPEC_ID_N(1:RN%N_SPEC)
   ENDIF
   ...
   !Get ramps
   IF (RN%A_RAMP/='null') THEN
      CALL GET_RAMP_INDEX(RN%A_RAMP,'PHI',NR2)
      RN%A_RAMP_INDEX = NR2
   ENDIF
   IF (RN%E_RAMP/='null') THEN
      CALL GET_RAMP_INDEX(RN%E_RAMP,'PHI',NR2)
      RN%E_RAMP_INDEX = NR2
   ENDIF      
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   ALLOCATE(RN%SPEC_ID_N(1:N_SPECIES))
   ...
   DO NS=1,RN%N_SPEC
      IF (TRIM(RN%SPEC_ID_N_READ(NS))=='null') CYCLE
      IF (RN%A < 0.0_EB) CYCLE
      NAME_FOUND = .FALSE.
      DO NS2=1,N_SPECIES
         IF (TRIM(RN%SPEC_ID_N_READ(NS))==TRIM(SPECIES(NS2)%ID)) THEN
            RN%SPEC_ID_N(NS2) = RN%SPEC_ID_N_READ(NS)
      ...
      IF (.NOT. NAME_FOUND) THEN
         WRITE(MESSAGE,'(A,I3,A,A,A)') 'ERROR: Problem with REAC ',NR,'. Primitive species ', &
                                       TRIM(RN%SPEC_ID_N_READ(NS)),' not found.'
         CALL SHUTDOWN(MESSAGE)
      ENDIF     
   ENDDO
...
CHARACTER(25) :: COLOR
LOGICAL :: MASSLESS,STATIC,MONODISPERSE,BREAKUP,CHECK_DISTRIBUTION,TURBULENT_DISPERSION,PERIODIC_X,PERIODIC_Y,PERIODIC_Z
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                N_STRATA,ORIENTATION,PERIODIC_X,PERIODIC_Y,PERIODIC_Z,PROP_ID,QUANTITIES,RADIATIVE_PROPERTY_TABLE,&
                REAL_REFRACTIVE_INDEX,RGB,SAMPLING_FACTOR,SIGMA_D,SPEC_ID,STATIC,SURFACE_TENSION,SURF_ID,&
                TURBULENT_DISPERSION,VERTICAL_VELOCITY ! Backward compatibility
   ...
   SURF_OR_SPEC: IF (LPC%SURF_INDEX==DROPLET_SURF_INDEX) THEN
      ...
      DO J = 1, 5000
         IF (SS%C_P_L(J) > 0._EB) THEN
            ...
            IF (J==1) THEN
               CALL JANAF_TABLE_LIQUID (J,CPBAR,H_V,H_L,TMP_REF,TMP_MELT,TMP_V,SS%DATA_ID,LPC%FUEL,DENSITY)
            ...
            ENDIF
         ELSE
            CALL JANAF_TABLE_LIQUID (J,SS%C_P_L(J),H_V,H_L,TMP_REF,TMP_MELT,TMP_V,SS%DATA_ID,LPC%FUEL,DENSITY)
            IF (J==1) THEN         
               IF (LPC%DENSITY < 0._EB) LPC%DENSITY = DENSITY
               IF (LPC%DENSITY < 0._EB) THEN
                  WRITE(MESSAGE,'(A,A,A)') 'ERROR: PARTicle class ',TRIM(SS%ID),' requires a density'
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BEAD_DENSITY,BEAD_DIAMETER,BEAD_EMISSIVITY,&
                ...
                PARTICLE_VELOCITY,PART_ID,PDPA_END,PDPA_HISTOGRAM, &
...
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE,CONDUCTIVITY,CONDUCTIVITY_RAMP,DENSITY,E,EMISSIVITY,FYI,&
                ...
                THRESHOLD_SIGN,THRESHOLD_TEMPERATURE,POROSITY,ALLOW_SHRINKING,ALLOW_SWELLING
...
LOGICAL :: ADIABATIC,BURN_AWAY,FREE_SLIP,NO_SLIP
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2), &
            ...
            IGNITION_TEMPERATURE,HEAT_OF_VAPORIZATION,NET_HEAT_FLUX,LAYER_DIVIDE,SURFACE_DENSITY, &
            ROUGHNESS,RADIUS,LENGTH,WIDTH,DT_INSERT,HEAT_TRANSFER_COEFFICIENT,TAU_PART,EMISSIVITY,EMISSIVITY_BACK,&
...
NAMELIST /SURF/ ADIABATIC,BACKING,BURN_AWAY,CELL_SIZE_FACTOR,COLOR,&
                CONVECTION_LENGTH_SCALE,CONVECTIVE_HEAT_FLUX,DEFAULT,&
                ...
                HRRPUA,HEAT_TRANSFER_COEFFICIENT,MASS_TRANSFER_COEFFICIENT,ID,IGNITION_TEMPERATURE,LAYER_DIVIDE,&
                ...
                RGB,ROUGHNESS,SPEC_ID,SPREAD_RATE,STRETCH_FACTOR,SURFACE_DENSITY,&
                ...
                VEG_LSET_ROS_FLANK,VEG_LSET_ROS_HEAD,VEG_LSET_WIND_EXP, &
                VEG_LSET_SIGMA,VEG_LSET_HT,VEG_LSET_BETA,VEG_LSET_ELLIPSE,VEG_LSET_TAN2,VEG_LSET_ELLIPSE_HEAD, & 
                VEL,VEL_GRAD,VEL_T,VOLUME_FLUX,WIDTH,XYZ,Z0,&
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   SELECT CASE(HEAT_TRANSFER_MODEL)
      CASE DEFAULT
         SF%HEAT_TRANSFER_MODEL = 0
      CASE('LOGLAW','LOG LAW')
         SF%HEAT_TRANSFER_MODEL = 1
      CASE('ABL')
         SF%HEAT_TRANSFER_MODEL = 2
   END SELECT
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,CH4_BANDS,KAPPA0,NMIEANG,NUMBER_RADIATION_ANGLES,PATH_LENGTH,&
...
IF (ANY(BAND_LIMITS>0._EB)) THEN
   ...
   IF (NUMBER_SPECTRAL_BANDS<2) CALL SHUTDOWN('ERROR: Need more spectral band limits.')
   IF (ANY((BAND_LIMITS(2:NUMBER_SPECTRAL_BANDS+1)-BAND_LIMITS(1:NUMBER_SPECTRAL_BANDS))<0._EB)) &
...
REAL(EB) :: TEMPERATURE,DENSITY,RR_SUM,ZZ_GET(0:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
...
NAMELIST /INIT/ CELL_CENTERED,CTRL_ID,DENSITY,DEVC_ID,DT_INSERT,DX,DY,DZ,HEIGHT,HRRPUV,ID,MASS_FRACTION,&
                MASS_PER_TIME,MASS_PER_VOLUME,MULT_ID,N_PARTICLES,N_PARTICLES_PER_CELL,PART_ID,RADIUS,SHAPE,&
                SPEC_ID,TEMPERATURE,UVW,XB,XYZ,&
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            SPEC_INIT_IF:IF (N_TRACKED_SPECIES > 0._EB .AND. ANY(MASS_FRACTION > TWO_EPSILON_EB)) THEN
               IF (SPEC_ID(1)=='null') THEN
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   IF (POINTS>1                     .AND. &
      ...
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (POINTS<2 .AND. (TRIM(STATISTICS)=='RMS'      .OR. &
                       TRIM(STATISTICS)=='COV'      .OR. &
                       TRIM(STATISTICS)=='CORRCOEF' .OR. &
                       TRIM(STATISTICS)=='TIME MIN' .OR. &
                       TRIM(STATISTICS)=='TIME MAX') ) THEN
   ...
   IF (DV%QUANTITY == 'PARTICLE RADIATION LOSS') THEN
      LOOP_QRW(MESH_NUMBER) = .TRUE.
      WRITE(LU_ERR,'(A, I4, A)') 'WARNING: The calculation of the DEVC Quantity PARTICLE RADIATION LOSS in mesh', &
      MESH_NUMBER, ' may increase the simulation time.'
   ENDIF
...
REAL(EB) :: VALUE(10)
CHARACTER(30) :: QUANTITY,SPEC_ID
INTEGER :: REDUCE_TRIANGLES,N,I_DUM,VELO_INDEX
TYPE(ISOSURFACE_FILE_TYPE), POINTER :: IS=>NULL()
NAMELIST /ISOF/ FYI,QUANTITY,REDUCE_TRIANGLES,SPEC_ID,VALUE,VELO_INDEX
...
CHARACTER(30) :: QUANTITY,SPEC_ID,PART_ID,QUANTITY2,PROP_ID
