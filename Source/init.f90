...
PUBLIC INITIALIZE_MESH_VARIABLES_1,INITIALIZE_MESH_VARIABLES_2,INITIALIZE_MESH_VARIABLES_3,INITIALIZE_GLOBAL_VARIABLES, &
       ...
       INITIALIZE_VARIABLE_THICKNESS_WALL_CELLS
...
USE MEMORY_FUNCTIONS, ONLY: PACK_WALL,PACK_CFACE
...
INTEGER, ALLOCATABLE, DIMENSION(:) :: CELL_COUNTER
...
ALLOCATE(CELL_COUNTER(NMESHES))      ; CELL_COUNTER = 0
ALLOCATE(CELL_INDICES(1000,NMESHES)) ; CELL_INDICES = 0
ALLOCATE(SURF_INDICES(1000,NMESHES)) ; SURF_INDICES = 0
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (NOM/=NM .AND. PROCESS(NOM)/=MY_RANK) THEN
      CELL_COUNTER(NOM) = CELL_COUNTER(NOM) + 1
      CELL_INDICES(CELL_COUNTER(NOM),NOM) = WC%BACK_INDEX
      SURF_INDICES(CELL_COUNTER(NOM),NOM) = WC%SURF_INDEX
      WC%BACK_INDEX = CELL_COUNTER(NOM)
   ENDIF
ENDDO
   ...
   DO IC=1,CELL_COUNT(NOM)
      DO IOR=-3,3
         ...
         IF (SURFACE(M4%SURF_INDEX(IC,IOR))%VARIABLE_THICKNESS) THEN
            CELL_COUNTER(NOM) = CELL_COUNTER(NOM) + 1
            CELL_INDICES(CELL_COUNTER(NOM),NOM) = M4%WALL_INDEX(IC,IOR)
            SURF_INDICES(CELL_COUNTER(NOM),NOM) = M4%SURF_INDEX(IC,IOR)
         ENDIF
      ENDDO
   ENDDO
ENDDO
...
DO NOM=1,NMESHES
   M%OMESH(NOM)%N_WALL_CELLS_RECV = 0
   IF (CELL_COUNTER(NOM)==0) CYCLE
   M%OMESH(NOM)%N_WALL_CELLS_RECV = CELL_COUNTER(NOM)
   ALLOCATE(M%OMESH(NOM)%WALL_CELL_INDICES_RECV(CELL_COUNTER(NOM)))
   ALLOCATE(M%OMESH(NOM)%WALL_SURF_INDICES_RECV(CELL_COUNTER(NOM)))
   M%OMESH(NOM)%WALL_CELL_INDICES_RECV(1:CELL_COUNTER(NOM)) = CELL_INDICES(1:CELL_COUNTER(NOM),NOM)
   M%OMESH(NOM)%WALL_SURF_INDICES_RECV(1:CELL_COUNTER(NOM)) = SURF_INDICES(1:CELL_COUNTER(NOM),NOM)
ENDDO

DEALLOCATE(CELL_COUNTER)
DEALLOCATE(CELL_INDICES)
DEALLOCATE(SURF_INDICES)
...
!> \brief Set up weighting arrays to transfer solid phase temperatures from one direction sweep to another.
...
INTEGER, INTENT(IN) :: NM
INTEGER :: I,IW,IW2,NWP,NWP2,I2,IWA,DM,NN,BACK_INDEX,IOR
...
REAL(EB) :: X1,X2,Y1,Y2,Z1,Z2,XX1,XX2,YY1,YY2,ZZ1,ZZ2,PRIMARY_VOLUME,OVERLAP_VOLUME,DXX,DYY,DZZ,DUM,WEIGHT_FACTOR,EXTRUDE,&
            SUM_WGT(3)
TYPE(WALL_TYPE), POINTER :: WC,WC2,WCB
...
PRIMARY_WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   BACK_INDEX = WC%BACK_INDEX
   OM  => MESHES(WC%BACK_MESH)
   WCB => OM%WALL(BACK_INDEX)
   BC  => M%BOUNDARY_COORD(WC%BC_INDEX)
   ONE_D => M%BOUNDARY_ONE_D(WC%OD_INDEX)
   SELECT CASE(ABS(BC%IOR))
      CASE(1) ; ONE_D%LAYER_THICKNESS(1) = ABS(BC%X - OM%BOUNDARY_COORD(WCB%BC_INDEX)%X)
      CASE(2) ; ONE_D%LAYER_THICKNESS(1) = ABS(BC%Y - OM%BOUNDARY_COORD(WCB%BC_INDEX)%Y)
      CASE(3) ; ONE_D%LAYER_THICKNESS(1) = ABS(BC%Z - OM%BOUNDARY_COORD(WCB%BC_INDEX)%Z)
   END SELECT
...
! Get the wall and node indices of the alternate directions of the 3D wall cells.
...
PRIMARY_WALL_LOOP_2: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   PRIMARY_NODE_LOOP: DO I=1,NWP  ! Loop over nodes of primary wall cell
      ...
      ALTERNATE_WALL_LOOP: DO IW2=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS  ! Loop over potential alternate wall cells
         ...
         IF (.NOT.SF2%VARIABLE_THICKNESS) CYCLE ALTERNATE_WALL_LOOP
         BC2 => M%BOUNDARY_COORD(WC2%BC_INDEX)
         IF (IOR_AVOID(BC2%IOR)) CYCLE ALTERNATE_WALL_LOOP
         XX1=M%X(BC2%IIG-1) ; XX2=M%X(BC2%IIG) ; YY1=M%Y(BC2%JJG-1) ; YY2=M%Y(BC2%JJG) ; ZZ1=M%Z(BC2%KKG-1) ; ZZ2=M%Z(BC2%KKG)
         IF (.NOT.ANY(WC%THIN) .AND. .NOT.ANY(WC2%THIN)) THEN
            IF (ABS(BC2%IOR)/=1) THEN ; DXX = MIN(XX2,X2)-MAX(XX1,X1) ; IF (DXX<=0._EB) CYCLE ALTERNATE_WALL_LOOP ; ENDIF
            IF (ABS(BC2%IOR)/=2) THEN ; DYY = MIN(YY2,Y2)-MAX(YY1,Y1) ; IF (DYY<=0._EB) CYCLE ALTERNATE_WALL_LOOP ; ENDIF
            IF (ABS(BC2%IOR)/=3) THEN ; DZZ = MIN(ZZ2,Z2)-MAX(ZZ1,Z1) ; IF (DZZ<=0._EB) CYCLE ALTERNATE_WALL_LOOP ; ENDIF
         ENDIF
         ONE_D2 => M%BOUNDARY_ONE_D(WC2%OD_INDEX)
         ...
         ALTERNATE_NODE_LOOP: DO I2=1,NWP2  ! Loop over nodes of alternate wall cell
            ...
            END SELECT
            IF (WC2%THIN(1)) THEN ; XX1=M%X(BC2%II) ; XX2=XX1+EXTRUDE*BC2%DX ; ENDIF
            IF (WC2%THIN(2)) THEN ; YY1=M%Y(BC2%JJ) ; YY2=YY1+EXTRUDE*BC2%DY ; ENDIF
            IF (WC2%THIN(3)) THEN ; ZZ1=M%Z(BC2%KK) ; ZZ2=ZZ1+EXTRUDE*BC2%DZ ; ENDIF
