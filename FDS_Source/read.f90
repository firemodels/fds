...
IF (SM_H2O > 0) THEN
   IF (SPECIES(H2O_INDEX)%YY0 > ZERO_P) THEN
      IF(HUMIDITY > 0._EB) THEN
         WRITE(LU_ERR,*) 'WARNING: Specified HUMIDTY on REAC will overide any initial SPEC value for WATER VAPOR.'
         SM%MASS_FRACTION(SM_H2O) = AMBIENT_WATER_VAPOR(HUMIDITY,TMPA)
      ELSE
         WRITE(LU_ERR,*) 'WARNING: Specified H2O MASS_FRACITON_0 on SPEC will overide default simple chemistry value of HUMIDTY.'   
         SM%MASS_FRACTION(SM_H2O)  = SPECIES(H2O_INDEX)%YY0  
      ENDIF
   ELSE
      IF (HUMIDITY < 0._EB) THEN
         XVAP = AMBIENT_WATER_VAPOR(40._EB,TMPA)
...
IF (SM_H2O>0)  THEN
   ...
   IF (SM_CO2>0) THEN
      ...
      SM%MASS_FRACTION(SM_N2) = SM%MASS_FRACTION(SM_N2) - SM%MASS_FRACTION(SM_CO2)
   ENDIF
   SM%MASS_FRACTION(1:SM_H2O-1) = SM%MASS_FRACTION(1:SM_H2O-1) * (1._EB-SM%MASS_FRACTION(SM_H2O))
ELSE
   IF (SM_CO2>0) THEN
      SM%MASS_FRACTION(SM_N2) = SM%MASS_FRACTION(SM_N2) - SM%MASS_FRACTION(SM_CO2)
   ENDIF
...
SM%MASS_FRACTION(SM_N2)  = SM%MASS_FRACTION(SM_N2) - SM%MASS_FRACTION(SM_O2)
