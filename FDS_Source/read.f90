...
USE MASS, ONLY: SCALAR_FACE_VALUE, SCALAR_FACE_CENTRAL,SCALAR_FACE_GODUNOV, SCALAR_FACE_SUPERBEE, SCALAR_FACE_MINMOD, &
                SCALAR_FACE_CHARM, SCALAR_FACE_MP5
...
CHARACTER(30) :: SURF_DEFAULT='INERT',EVAC_SURF_DEFAULT='INERT',FUEL_RADCAL_ID='null'
   ...
   ICLOOP: DO IC=1,3
      ...
      SELECT CASE (T%ITRAN(IC))
         ...
         CASE (2)  ! linear transformation
            ...
            DO N=1,T%NOC(IC)+1
               IF (T%C1(N,IC)-T%C1(N-1,IC)<TWO_EPSILON_EB) CALL SHUTDOWN('ERROR: Do not specify endpoints in linear grid transformation')
...
CHARACTER(30) :: RAMP_GX,RAMP_GY,RAMP_GZ,TURBULENCE_MODEL,LES_FILTER_WIDTH
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                ...
                C_FORCED_CYLINDER,C_FORCED_SPHERE,C_HORIZONTAL,C_VERTICAL,DNS,DRIFT_FLUX,EDC,ENTHALPY_TRANSPORT,&
                EVACUATION_DRILL,EVACUATION_MC_MODE,EVAC_PRESSURE_ITERATIONS,EVAC_TIME_ITERATIONS,&
                FLUX_LIMITER,FORCE_VECTOR,&
                ...
                MAXIMUM_VISIBILITY,MEAN_FORCING,MIXING_LAYER_H0,MIXING_LAYER_THETA0,MIXING_LAYER_U0,&
                NEW_CP_WHILE,NEW_EVAP_DROP,NOISE,NOISE_VELOCITY,NO_EVACUATION,&
                ...
                SHARED_FILE_SYSTEM,SMOKE_ALBEDO,SOLID_PHASE_ONLY,&
                STRATIFICATION,TENSOR_DIFFUSIVITY,TERRAIN_CASE,TERRAIN_IMAGE,TEXTURE_ORIGIN,THERMOPHORETIC_DEPOSITION,&
                THICKEN_OBSTRUCTIONS,TMPA,TURBULENCE_MODEL,TURBULENT_DEPOSITION,U0,UVW_FILE,V0,VEG_LEVEL_SET,VISIBILITY_FACTOR,&
...
SELECT CASE (FLUX_LIMITER)
   CASE(CENTRAL_LIMITER)
      SCALAR_FACE_VALUE=>SCALAR_FACE_CENTRAL
   CASE(GODUNOV_LIMITER)
      SCALAR_FACE_VALUE=>SCALAR_FACE_GODUNOV
   CASE(SUPERBEE_LIMITER)
      SCALAR_FACE_VALUE=>SCALAR_FACE_SUPERBEE
   CASE(MINMOD_LIMITER)
      SCALAR_FACE_VALUE=>SCALAR_FACE_MINMOD
   CASE(CHARM_LIMITER)
      SCALAR_FACE_VALUE=>SCALAR_FACE_CHARM
   CASE(MP5_LIMITER)
      SCALAR_FACE_VALUE=>SCALAR_FACE_MP5  
END SELECT
...
NAMELIST /DUMP/ COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,CUTCELL_DATA_FILE,&
                ...
                DT_GEOM,DT_HRR,DT_ISOF,DT_MASS,DT_PART,DT_PL3D,DT_PROF,DT_RESTART,DT_SL3D,DT_SLCF,DT_VEG,FLUSH_FILE_BUFFERS,&
...
CHARACTER(30) :: PREDEFINED_SPEC_ID(MAX_SPECIES),RAMP_CP,RAMP_CP_L,RAMP_K,RAMP_MU,RAMP_D,ALIAS,RADCAL_ID
...
REAL(EB) :: SOOT_YIELD,CO_YIELD,EPUMO2,A,A_LEAN, &
            ...
            AUTO_IGNITION_TEMPERATURE,THRESHOLD_TEMPERATURE,HUMIDITY,SOOT_H_FRACTION,N_T,X_O2_LL,Y_O2_LL,Y_F_LFL
...
NAMELIST /REAC/ A,A_LEAN,AUTO_IGNITION_TEMPERATURE,BETA_EDC,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                E,E_LEAN,EDCM,EPUMO2,EQUATION,EXTINCTION_MODEL,FIXED_MIX_TIME,FORMULA,FUEL,FUEL_RADCAL_ID, &
                FWD_ID,FYI,H,HEAT_OF_COMBUSTION,&
                HRRPUA_SHEET,HRRPUV_AVERAGE,HUMIDITY,ID,IDEAL,N,NU,N_S,N_T,O,ODE_SOLVER,PHI_DEPENDENCE,REAC_ATOM_ERROR,&
                REAC_MASS_ERROR,REVERSIBLE,SMIX_ID,SOOT_H_FRACTION,SOOT_YIELD,SPEC_ID,SUPPRESSION,TAU_CHEM,TAU_FLAME,&
                THRESHOLD_TEMPERATURE,Y_CO2_INFTY,Y_O2_INFTY,Y_P_MIN_EDC,X_O2_LL,Y_O2_LL,Y_F_LFL
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   RN%X_O2_LL                   = X_O2_LL
   RN%Y_O2_LL                   = RN%X_O2_LL*31.9988/ (RN%X_O2_LL*31.9988+(1._EB-RN%X_O2_LL)*28.0134)
   RN%Y_F_LFL                   = Y_F_LFL
...
CHARACTER(25) :: COLOR
LOGICAL :: MASSLESS,STATIC,MONODISPERSE,BREAKUP,CHECK_DISTRIBUTION,FUEL,WATER,&
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                DENSITY,DEVC_ID,DIAMETER,DISTRIBUTION,DRAG_COEFFICIENT,DRAG_LAW,FREE_AREA_FRACTION,FYI,GAMMA_D,HEAT_OF_COMBUSTION,&
                ...
                TURBULENT_DISPERSION,VERTICAL_VELOCITY,&
...
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,SPECIFIC_HEAT,DENSITY,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE, &
            INITIAL_VAPOR_FLUX,PEAK_REACTION_RATE,POROSITY
...
USE RADCONS
REAL(EB),DIMENSION(MAX_NUMBER_SPECTRAL_BANDS)::WL_BAND_LIMITS
NAMELIST /RADI/ ANGLE_INCREMENT,CH4_BANDS,KAPPA0,NEW_ABSORPTION,NMIEANG,NUMBER_RADIATION_ANGLES,PATH_LENGTH,&
...
IF(ANY(WL_BAND_LIMITS>0._EB)) THEN
   NUMBER_SPECTRAL_BANDS=COUNT(WL_BAND_LIMITS>0._EB)-1
   IF(NUMBER_SPECTRAL_BANDS<2) THEN
     CALL SHUTDOWN('ERROR: Need more spectral band limits.')
   ENDIF
   IF(ANY((WL_BAND_LIMITS(2:NUMBER_SPECTRAL_BANDS+1)-WL_BAND_LIMITS(1:NUMBER_SPECTRAL_BANDS))<0._EB)) THEN
   ...
   DO I=1,NUMBER_SPECTRAL_BANDS
     WL_LOW(I)=WL_BAND_LIMITS(I)
     WL_HIGH(I)=WL_BAND_LIMITS(I+1)
   ENDDO
...
INTEGER  :: N,NN,NNN,NM,QUANTITY_INDEX,QUANTITY2_INDEX,MAXCELLS,I,J,K,I_DUM,N_PLUS,N_MINUS
