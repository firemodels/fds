...
REAL(EB) :: MASS_FRACTION_0,MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,XVAP
INTEGER  :: N_SPEC_READ,N_SPEC_REALLOC,N,NN
...
READ_SMIX_LOOP: DO N=1,N_LUMPED_SPECIES   
   ...
   WRITE(SM%FORMULA,'(A,I1)') 'Z',N-SMIX_OFFSET   
   MASS_FRACTION = -1._EB
...
DO NS=0,N_TRACKED_SPECIES
   ...
   WRITE(SM%ID,'(A,I1)') 'LUMPED SPECIES ',NS-1
ENDDO
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (DEFAULT) SURF_DEFAULT ='ID'
   IF (EVAC_DEFAULT) EVAC_SURF_DEFAULT ='ID'
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   IF (ANY(MASS_FLUX(1:N_TRACKED_SPECIES)>0._EB) .OR. ANY(MASS_FRACTION(1:N_TRACKED_SPECIES)>0._EB)) THEN
