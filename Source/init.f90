   ...
   DO IC=1,CELL_COUNT(NOM)
      ...
      DO IOR=-3,3
         ...
         IF (SURFACE(M4%SURF_INDEX(IC,IOR))%HT_DIM>1) THEN
            IF (COUNT(WALL_SAVE(NOM)%CELL_INDICES(WALL_SAVE(NOM)%START:WALL_SAVE(NOM)%COUNTER)==M4%WALL_INDEX(IC,IOR))>0) CYCLE
            IF (WALL_SAVE(NOM)%COUNTER==WALL_SAVE(NOM)%DIMEN) CALL REALLOCATE_WALL_SAVE
            WALL_SAVE(NOM)%COUNTER = WALL_SAVE(NOM)%COUNTER + 1
      ...
      DO IOR=-3,3
         ...
         DO IEC=1,3
            IF (M4%THIN_WALL_INDEX(IC,IOR,IEC)>0) THEN
               ...
               THIN_WALL_SAVE(NOM)%CELL_INDICES(THIN_WALL_SAVE(NOM)%COUNTER) = M4%THIN_WALL_INDEX(IC,IOR,IEC)
               THIN_WALL_SAVE(NOM)%SURF_INDICES(THIN_WALL_SAVE(NOM)%COUNTER) = M4%THIN_SURF_INDEX(IC,IOR,IEC)
            ENDIF
         ENDDO
      ENDDO
   ENDDO
