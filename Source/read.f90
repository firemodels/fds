...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            AREA_ADJUST,PARTICLE_SURFACE_DENSITY,MOISTURE_FRACTION,SURFACE_VOLUME_RATIO,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   ! Convert SURFACE_VOLUME_RATIO

   IF (SURFACE_VOLUME_RATIO>0._EB) THEN
      SELECT CASE(GEOMETRY)
         CASE('CARTESIAN')   ; THICKNESS(1) = 1._EB/SURFACE_VOLUME_RATIO
         CASE('CYLINDRICAL') ; THICKNESS(1) = 2._EB/SURFACE_VOLUME_RATIO
         CASE('SPHERICAL')   ; THICKNESS(1) = 3._EB/SURFACE_VOLUME_RATIO
      END SELECT
   ENDIF
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   COUNT_LAYERS: DO NL=1,MAX_LAYERS
      ...
      EMISSIVITY = 0._EB
      IF (SF%MOISTURE_FRACTION>0._EB) THEN  ! Add MOISTURE as a MATL if the user specifies MOISTURE_FRACTION>0
         DO NN=1,MAX_MATERIALS
            IF (MATL_ID(NL,NN) == 'null') EXIT
         ENDDO
         MATL_ID(NL,NN) = 'MOISTURE'
         MATL_MASS_FRACTION(NL,NN) = SF%MOISTURE_FRACTION/(1._EB+SF%MOISTURE_FRACTION)
         ...
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            PART_ID_IF: IF (PART_ID/='null') THEN
               ...
               IF (IN%MASS_PER_TIME>0._EB .OR. IN%MASS_PER_VOLUME>0._EB .OR. PACKING_RATIO>0._EB) THEN
                  ...
                  ENDIF
                  IF (LPC%SURF_INDEX>0) THEN
                     MOIST_FRAC = SURFACE(LPC%SURF_INDEX)%MOISTURE_FRACTION
