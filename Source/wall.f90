...
CALL THERMAL_BC(T,NM,CALL_HT_1D)
...
CALL SPECIES_BC(T,DT,NM,CALL_HT_1D)
...
!> \param T Time (s)
!> \param NM Mesh number
!> \param CALL_HT_1D Indicator if 1-D conduction solver is called
...
SUBROUTINE THERMAL_BC(T,NM,CALL_HT_1D)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      CALL SURFACE_HEAT_TRANSFER(WALL_INDEX=IW)
...
CFACE_LOOP: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   MU_G  = CFA%MU_G
   IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      CALL SURFACE_HEAT_TRANSFER(CFACE_INDEX=ICF)
   ELSEIF (CALL_HT_1D) THEN
...
SOLID_PARTICLES_IF: IF (SOLID_PARTICLES) THEN
   DO IP = 1, NLP
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN  ! Target particles are included to get gas phase values
         ...
         IF (LPC%SOLID_PARTICLE .AND. .NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) CALL SURFACE_HEAT_TRANSFER(PARTICLE_INDEX=IP)
...
SUBROUTINE SURFACE_HEAT_TRANSFER(WALL_INDEX,CFACE_INDEX,PARTICLE_INDEX)
...
!> \param DT Current time step (s)
!> \param NM Mesh number
!> \param CALL_HT_1D Indicates whether 1-D conduction solver is called

SUBROUTINE SPECIES_BC(T,DT,NM,CALL_HT_1D)
