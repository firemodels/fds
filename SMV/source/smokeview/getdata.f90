...
REAL(EB), DIMENSION(0:2) :: DIFF1, DIFF2, ACROSSB
REAL(EB) :: VAL

DIFF1 = A - C
DIFF2 = B - C
CALL CROSS_PRODUCT(DIFF1, DIFF2, ACROSSB)
VAL = DOT_PRODUCT(ACROSSB,N)
...
REAL(EB), DIMENSION(:), POINTER :: VERT
INTEGER :: I
...
DO BP = 0, 5
   ...
   BOXNORMAL(0:2) => BOX_NORMALS(0:2,BP)
   VECDIFF = XYZ - BOXVERT
   IF (DOT_PRODUCT(BOXNORMAL,VECDIFF)>0.0) THEN
      IN_BOX=0
      RETURN
   ENDIF
END DO
...
DO TP = 0, 3
   ...
   IF (DOT_PRODUCT(TETRANORMAL,VECDIFF)>0.0) THEN
      IN_TETRA=0
      RETURN
   ENDIF
END DO
...
REAL(EB), DIMENSION(0:299) :: OUT_VERTS_EB
