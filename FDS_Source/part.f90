...
UVW = MAX( ABS(LP%U)*RDX(IIG),ABS(LP%V)*RDY(JJG),ABS(LP%W)*RDZ(KKG) )
...
PARTICLE_LOOP: DO I=1,NLP
   ...
   IF (T-LP%ONE_D%T>LPC%LIFETIME .OR. (T-LP%ONE_D%T>MAX_PART_LIFE .AND. IKILL <= INSERT_COUNT .AND. .NOT. LPC%STATIC)) THEN
      IKILL = IKILL + 1
      KILL_PARTICLE_INDEX(IKILL) = I
      CYCLE PARTICLE_LOOP
   ENDIF
