...
rectilinear meshes that need not coinside with the fire meshes, so
that the actual positions of the obstacles might be a little bit
different due to the different spatial discretations of the meshes.
...
cabability in the code to read the ``old'' evacuation input style.
...
\begin{itemize}
  ...
  \item Agent properties and initial postions (\Timtt{PERS} and
...
\Timtt{TARGET\_WHEN\_CLOSED} logical paramters in
\Timtt{EXIT}/\Timtt{DOOR} namelist.  The \Timtt{CORR} has paramerers
...
IDs to specify additional holes and obsts for these see-or-not meshes,
...
"outflow" vents.  No need to define additional obsts for the "outflow"
...
does not mean straght to that point.  The agents aim between the door
...
agents by Korhonen \emph{et al.}~\cite{Korhonen07a, Korhonen07b,
  Korhonen08a, Korhonen08b}.  Later the model was enchanced to deal
...
% Timo: Lis‰‰ t‰nne, ett‰ mitein eli jostakin kokouspaperista kuvaus eli
...
%!Timo: OK    eli ottaa huomioon t‰m‰n hetken target-v0:llan. Samalla myˆskin
%!Timo:       random forcea skaalattu henkilˆn v0:n mukaan, vertailukohtana
%!Timo:       1.3 m/s menev‰ henkilˆ.
...
and Heliˆvaara \emph{et al.}~\cite{Heliovaara10}.  The agents observe
...
default.  For the non-visible doors distance should bperhaps e
...
evacuation calculation should be the same (exept some minor bug fixes
...
% Simo Heliˆvaaraa, Timo Korhonen, Simo Hostikka, Harri Ehtamo
...
\item Run a short simulation (\Timtt{T\_END=0.1}).  Check the
  ...
  at the EXITs and DOORs.  There should also be no narrow paths, say
...
\item In addition to the user defined meshes, there is automatically
  ...
  defined by the user.  These meshes are located horizontally excatly
...
\item[\Timts{PERS\_ID}] This applies just for the \Timts{COUNT\_ONLY=
  ...
  If \Timts{EVAC\_ID} is also given then both IDs match if the agent
  is to be counted.
%
\item[\Timts{EVAC\_ID}] This applies just for the \Timts{COUNT\_ONLY=
  ...
  also given then both IDs should match if the agent is to be counted.
%
\end{description}
...
just as a sigle object.  This model was first introduced in the Evac
...
%
\item[\Timtt{IJK}] Like for the evacuation meshes, set how meny cells
  span the $x$ and $y$ directions. K should always be equal to one.
%
\item[\Timtt{EVAC\_Z\_OFFSET}] Like for the evacuation mehses.  This
...
\item[\Timtt{XB\_LANDINGS(ii,:)}] An array that list the positions of
  ...
  is not a landing, but it is counted as a landing. But the STRS
...
%
\bibitem{Rinne10} Rinne, T., Tillander, K., and Grˆnberg, P., ``Data
