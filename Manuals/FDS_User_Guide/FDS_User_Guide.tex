...
\begin{figure}[ht]
\includegraphics[width=3.1in]{SCRIPT_FIGURES/stack_effect.pdf}
...
\mathrm{C_2H_3Cl + 1.53462 \, (O_2+(^{.79}/_{.21}) \, N_2)} \rightarrow \\
\nonumber \mathrm{HCl +  H_2O + 0.140582\, CO + 0.96433\, CO_2 + 0.895087\, C + 1.53462\,(^{.79}/_{.21}) \, N_2}
\end{eqnarray}
The choice of fuel in this example, PVC, is not defined in Table {tab:gasspecies}, therefore its properties must be defined on a {\ct SPEC} line. In this example, we use the Lennard-Jones Potential parameters and molecular weight to define the fuel. The example will also use the lumped species formulation to minimize the number of scalar transport equations that need to be solved. Therefore, each species that does not transport by itself will have an SMIX\_COMPONENT\_ONLY = .TRUE. designation.
\footnotesize
\begin{verbatim}
&SPEC ID = 'NITROGEN',          SMIX_COMPONENT_ONLY = .TRUE. /
&SPEC ID = 'OXYGEN',            SMIX_COMPONENT_ONLY = .TRUE. /
...
&SPEC ID = 'PVC', MW = 62.5036, SIGMALJ = 4.898, EPSILONKLJ = 300/
\end{verbatim} \normalsize

Looking at Equation \ref{eq:PVC_reac} we can group the fuel, oxidizer, and products together:
\begin{eqnarray}\label{eq:PVC_reac3}
\mathrm{\underbrace{C_2H_3Cl}_\text{Fuel} + \underbrace{1.53462 \, (O_2+(^{.79}/_{.21}) \, N_2)}_\text{Oxidizer}} \rightarrow \\
\nonumber \mathrm{\underbrace{HCl +  H_2O + 0.140582\, CO + 0.96433\, CO_2 + 0.895087\, C + 1.53462\,(^{.79}/_{.21}) \, N_2}_\text{Products}}
\end{eqnarray}
For the oxidizer and products, which are both composed of multiple primitive species, {\ct SMIX} lines are needed to define the composition of the lumped species. As the user, you can define the {\ct SMIX} by either the MASS\_FRACTIONS of the component gases or the VOLUME\_FRACTIONS. For this case, VOLUME\_FRACTIONS will be used. To determine the volume fractions, we can find the total number of moles by summing the stoichiometric coefficients in the lumped species and then dividing each species' stoichiometric coefficient by that sum.
\footnotesize
\begin{verbatim}
&SMIX ID = 'AIR', SPEC_ID ='NITROGEN','OXYGEN', VOLUME_FRACTION =0.79,0.21, BACKGROUND=.TRUE. /
&SMIX ID = 'PRODUCTS', SPEC_ID ='NITROGEN','CARBON DIOXIDE','CARBON MONOXIDE',
           'WATER VAPOR','SOOT','HYDROGEN CHLORIDE', 
           VOLUME_FRACTION = 0.590713,0.098672,0.014384,0.102322,0.091587,0.102322 /
\end{verbatim} \normalsize
Note: the order of which the {\ct SPEC\_ID}s are defined must be the same order as which you defined your VOLUME\_FRACTIONS or MASS\_FRACTIONS. To set the initial concentration of fuel, an {\ct INIT} line is used:
\footnotesize
\begin{verbatim}
&INIT MASS_FRACTION(1)=0.228671,SPEC_ID(1)='PVC' /
\end{verbatim} \normalsize
In this mixing-controlled example, the user is responsible for defining the fuel, heat of combustion of that fuel, species participating the reaction, and the stoichiometric coefficients that describe the reaction. The heat of combustion for `PVC' was found from the SFPE Handbook \cite{SFPE:Tewarson}. If this reaction were all primitive species then the coefficients would be those in Equation \ref{eq:PVC_reac2}. In the lumped species approach, the stoichiometric coefficients for the {\ct SMIX}s become the sum of the coefficients of the primitive species.
\footnotesize
\begin{verbatim}
&REAC FUEL = 'PVC', HEAT_OF_COMBUSTION=16400,
      SMIX_ID = 'PVC','AIR','PRODUCTS', NU=-1,-7.30771,9.77309
      FIXED_MIX_TIME=0.1,
