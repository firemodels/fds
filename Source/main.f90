...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(3)
      ...
      IF (N_MPI_PROCESSES>1) THEN
         ...
         CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,CELL_COUNT_REALS,COUNTS,DISPLS,MPI_INTEGER,MPI_COMM_WORLD,IERR)
      ...
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         DO N=1,M%N_NEIGHBORING_MESHES
            ...
            IF (.NOT.ALLOCATED(M4%CELL))          ALLOCATE(M4%CELL(0:CELL_COUNT(NOM)))
            IF (.NOT.ALLOCATED(M4%CELL_REALS))    ALLOCATE(M4%CELL_REALS(1:CELL_COUNT_REALS(NOM)))
            IF (.NOT.ALLOCATED(M4%CELL_INTEGERS)) ALLOCATE(M4%CELL_INTEGERS(1:CELL_COUNT_INTEGERS(NOM)))
            IF (.NOT.ALLOCATED(M4%CELL_LOGICALS)) ALLOCATE(M4%CELL_LOGICALS(1:CELL_COUNT_LOGICALS(NOM)))
...
!   CALL MPI_BCAST(M%CELL_REALS,SIZE(M%CELL_REALS),MPI_DOUBLE_PRECISION,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
