...
INTEGER :: TAG,PNAMELEN=0,DISP,TAG_EVAC
...
DO NM=1,NMESHES
   ...
   DO NOM=1,NMESHES
      ...
      IF (USE_MPI .AND. NM/=NOM .AND. PROCESS(NOM)/=MYID) THEN
         CALL MPI_SEND(MESHES(NM)%OMESH(NOM)%I_MIN_R,1,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,IERR)
         CALL MPI_SEND(MESHES(NM)%OMESH(NOM)%I_MAX_R,1,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,IERR)
         CALL MPI_SEND(MESHES(NM)%OMESH(NOM)%J_MIN_R,1,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,IERR)
         CALL MPI_SEND(MESHES(NM)%OMESH(NOM)%J_MAX_R,1,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,IERR)
         CALL MPI_SEND(MESHES(NM)%OMESH(NOM)%K_MIN_R,1,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,IERR)
         CALL MPI_SEND(MESHES(NM)%OMESH(NOM)%K_MAX_R,1,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,IERR)
         CALL MPI_SEND(MESHES(NM)%OMESH(NOM)%NIC_S,  1,MPI_INTEGER,PROCESS(NOM),NM,MPI_COMM_WORLD,IERR)
      ELSE
...
DO NM=1,NMESHES
   ...
   DO NOM=1,NMESHES
      ...
      IF (USE_MPI .AND. NM/=NOM .AND. PROCESS(NM)/=MYID) THEN
         CALL MPI_RECV(MESHES(NOM)%OMESH(NM)%I_MIN_S,1,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
         CALL MPI_RECV(MESHES(NOM)%OMESH(NM)%I_MAX_S,1,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
         CALL MPI_RECV(MESHES(NOM)%OMESH(NM)%J_MIN_S,1,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
         CALL MPI_RECV(MESHES(NOM)%OMESH(NM)%J_MAX_S,1,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
         CALL MPI_RECV(MESHES(NOM)%OMESH(NM)%K_MIN_S,1,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
         CALL MPI_RECV(MESHES(NOM)%OMESH(NM)%K_MAX_S,1,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
         CALL MPI_RECV(MESHES(NOM)%OMESH(NM)%NIC_R,  1,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
      ENDIF
   ENDDO
ENDDO
...
MAIN_LOOP: DO  
   ...
   DO NM=1,NMESHES
      IF (PROCESS(NM)==MYID .AND. ACTIVE_MESH(NM)) THEN
         IF (.NOT. CALL_UPDATE_CONTROLS) CALL UPDATE_CONTROLS(T,MESHES(NM)%DT)
...
INTEGER :: SNODE,NRA,NSB,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IJK_SIZE,N,N_STORAGE_SLOTS
REAL(EB) :: TNOW
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (CODE==7 .AND. OMESH_PARTICLES .AND. (M3%NIC_S>0 .OR. M3%NIC_R>0)) THEN
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%N_PART_ADOPT,N_LAGRANGIAN_CLASSES,MPI_INTEGER,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==6 .AND. OMESH_PARTICLES .AND. (M3%NIC_S>0 .OR. M3%NIC_R>0)) THEN
      DO N=1,N_LAGRANGIAN_CLASSES
         ...
         CALL MPI_IRECV(M3%ADOPT_PARTICLE_STORAGE(N)%REALS(1,1),(LPC%N_STORAGE_REALS)*N_STORAGE_SLOTS, &
                        MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
...
INTEGER :: NN1,NN2,NRA,NSB,IPC,CNT,IBC,STORAGE_INDEX_SAVE
   ...
   RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
      ...
      IF (CODE==1 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG1(1),SIZE(M3%REAL_SEND_PKG1),MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,&
                           REQ(N_REQ),IERR)
         ELSE
      ...
      IF (CODE==2 .AND. M3%NIC_R>0 .AND. RNODE/=SNODE) THEN
         N_PREQ=N_PREQ+1
         CALL MPI_SEND_INIT(M3%REAL_SEND_PKG7(1),SIZE(M3%REAL_SEND_PKG7),MPI_DOUBLE_PRECISION,SNODE,TAG, &
                            MPI_COMM_WORLD,PREQ(N_PREQ),IERR)
      ENDIF
      ...
      IF (CODE==3 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG2(1),SIZE(M3%REAL_SEND_PKG2),MPI_DOUBLE_PRECISION,SNODE,TAG, &
      ...
      IF (CODE==4 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),SIZE(M3%REAL_SEND_PKG3),MPI_DOUBLE_PRECISION,SNODE,TAG,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      IF (CODE==6 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG4(1),SIZE(M3%REAL_SEND_PKG4),MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD, &
                           REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      SEND_RADIATION: IF ( CODE==6 .AND. M3%NIC_R>0 .AND. EXCHANGE_RADIATION) THEN
         ...
         ELSE
            DO IW=1,M4%N_EXTERNAL_WALL_CELLS
               IF (M4%WALL(IW)%NOM==NM) M4%WALL(IW)%ONE_D%ILW(1:NRA,1:NSB) = M3%WALL_ILW(IW)%ILW(1:NRA,1:NSB)
            ENDDO
         ENDIF
      ...
      IF (CODE==7 .AND. OMESH_PARTICLES) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%N_PART_ORPHANS,N_LAGRANGIAN_CLASSES,MPI_INTEGER,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
...
USE HVAC_ROUTINES, ONLY: NODE_TMP,NODE_RHO,DUCT_VF,NODE_ZZ
REAL(EB) :: TNOW
...
REAL_BUFFER_10 = DUCT_VF
CALL MPI_SCATTER(REAL_BUFFER_10(1,1),N_DUCTS,MPI_DOUBLE_PRECISION, &
                 DUCT_VF,N_DUCTS,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
IF (N_TRACKED_SPECIES>0) THEN
   REAL_BUFFER_7 = NODE_ZZ
