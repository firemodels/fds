...
OVERALL_INSERT_LOOP: DO  
   ...
   WALL_INSERT_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (SF%PARTICLE_MASS_FLUX > 0._EB) THEN
            IF (TW(IW)==T_BEGIN .AND. SF%RAMP_INDEX(TIME_PART)>=1) THEN
               TSI = T
            ELSE
               TSI = T - TW(IW)
            ENDIF
...
REAL(EB) :: R_DROP,NUSSELT,K_AIR,H_V,H_V_REF, H_L,&
            ...
            CP,H_NEW,YY_GET(1:N_GAS_SPECIES),YY_GET2(1:N_GAS_SPECIES),M_GAS_NEW,MW_GAS,CP2,VEL_REL,DELTA_H_G,TMP_G_I,H_G_OLD,H_L_REF,&
