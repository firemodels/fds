...
Note that by applying the natural log to the random variable, we can express its variance in terms of the
relative uncertainty, $\sigma_E/\theta$. This is convenient because we have assumed that the relative experimental
uncertainty is constant for a given quantity of interest.
Using a Bayesian argument\footnote{Bayes theorem states that the posterior distribution is the product of
the prior distribution, the likelihood function, and a normalizing constant: $f(\theta|E)=C \, f(\theta) \, f(E|\theta)$.}
...
In a similar manner, assume that the model predictions are normally distributed about the ``true'' value
...
This and the bias factor, $\delta$, are the parameters
that we seek to quantify.
Again, using the log transformation we obtain
\be \ln(M)|\theta \sim N \left(\ln(\delta)+\ln(\theta)-\frac{\omega_M^2}{2} \; , \;
    \omega_M^2 \right) \quad ; \quad \omega_M=\frac{\sigma_M}{\theta \, \delta} \ee
...
\be u(M) = \sqrt{\omega_M^2+\widetilde{\sigma}_E^2} \approx \sqrt{ \frac{1}{n-1} \sum_{i=1}^n \,
