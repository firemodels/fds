...
A damper can be placed in a duct by adding the keyword {\ct DAMPER} along with either a {\ct CTRL\_ID} or {\ct DEVC\_ID}.  When the control or device is {\ct .TRUE.} the damper will be open.  The sample below is a simple duct network of three ducts connected by a tee.  At damper is present in on of the ducts and linked to a {\ct DEVC} that will change state at 10 s.  With the damper open the respective duct flow speeds should be 3 m/s, 1 m/s and 2 m/s.  When the damper closes, the respective duct flow speeds should be 3 m/s, 3 m/s and 0 m/s.  Results of this are shown in \ref{HVAC_damper}.
