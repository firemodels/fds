...
In practice, the filtered mass and energy source term contributions to the velocity divergence constraint are collected in a single term denoted {\ct D\_LAGRANGIAN} within the code,
\begin{equation}
D_{\si{Lagrangian}} = \frac{1}{\rho} \sum_\alpha \frac{\overline{W}}{W_\alpha} \, \dot{m}_{{\rm b},\alpha}^{\ppp} + \frac{1}{\rho c_p T} \left( \dot{q}_{\rm b}^{\ppp} - \sum_\alpha h_{s,\alpha} \, \dot{m}_{{\rm b},\alpha}^{\ppp} \right)
\end{equation}
