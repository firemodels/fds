...
CHARACTER(80) :: TCFORM
LOGICAL :: EX,DRY
CHARACTER(255), PARAMETER :: dumpid='$Id: dump.f90 10097 2012-02-17 14:56:13Z randy.mcdermott $'
CHARACTER(255), PARAMETER :: dumprev='$Revision: 10097 $'
CHARACTER(255), PARAMETER :: dumpdate='$Date: 2012-02-17 09:56:13 -0500 (Fri, 17 Feb 2012) $'
...
PARTICLE_IF: IF ( (PARTICLE_FILE .AND. .NOT.EVACUATION_ONLY(NM)).OR. (EVACUATION_ONLY(NM).AND.EVACUATION_GRID(NM)) ) THEN
   ...
   ELSE APPEND_PARTICLE_FILE
      ...
      ELSE
         ...
         WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(FN_PART(NM))
         M%N_STRINGS = M%N_STRINGS + 1
         WRITE(M%STRING(M%N_STRINGS),'(I3)') N_LAGRANGIAN_CLASSES
!
...
REAL(EB), INTENT(IN) :: T
INTEGER :: SURF_INDEX,NOM,IW,N,N_TOTAL_WALL_CELLS
INTEGER, INTENT(IN) :: NM
...
N_TOTAL_WALL_CELLS = N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS + N_VIRTUAL_WALL_CELLS + N_GHOST_WALL_CELLS
WRITE(LU_CORE(NM)) U
WRITE(LU_CORE(NM)) V
WRITE(LU_CORE(NM)) W
WRITE(LU_CORE(NM)) D
...
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%UW
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%UWS
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%TMP_F
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%TMP_B
DO IW=1,N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS + N_VIRTUAL_WALL_CELLS + N_GHOST_WALL_CELLS
   WRITE(LU_CORE(NM)) WALL(IW)%ONE_D%MASSFLUX
   WRITE(LU_CORE(NM)) WALL(IW)%RHODW
   IF (N_TRACKED_SPECIES>0) WRITE(LU_CORE(NM)) WALL(IW)%ZZ_F
ENDDO
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%ONE_D%QRADIN
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%ONE_D%QRADOUT
IF (N_LP_ARRAY_INDICES>0) WRITE(LU_CORE(NM)) QR_W
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%TW
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%EW
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%KW
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%RHO_F
WRITE(LU_CORE(NM)) SOLID
WRITE(LU_CORE(NM)) EDGE_INDEX
WRITE(LU_CORE(NM)) IJKE
WRITE(LU_CORE(NM)) OME_E
WRITE(LU_CORE(NM)) TAU_E
WRITE(LU_CORE(NM)) WALL(1:N_TOTAL_WALL_CELLS)%BOUNDARY_TYPE
...
DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (SURFACE(SURF_INDEX)%THERMALLY_THICK) THEN
      IF (SURFACE(SURF_INDEX)%SHRINK) THEN
         WRITE(LU_CORE(NM)) WALL(IW)%ONE_D%N_LAYER_CELLS,WALL(IW)%ONE_D%LAYER_THICKNESS,WALL(IW)%ONE_D%X
         IF (SUM(WALL(IW)%ONE_D%N_LAYER_CELLS)>0) WRITE(LU_CORE(NM)) WALL(IW)%ONE_D%TMP,WALL(IW)%ONE_D%RHO
      ELSE
         IF (SURFACE(SURF_INDEX)%N_CELLS>0) WRITE(LU_CORE(NM)) WALL(IW)%ONE_D%TMP,WALL(IW)%ONE_D%RHO
      ENDIF
   ENDIF
   IF (RADIATION .AND. WALL(IW)%BOUNDARY_TYPE/=OPEN_BOUNDARY) WRITE(LU_CORE(NM)) WALL(IW)%ONE_D%ILW
ENDDO
IF (N_TRACKED_SPECIES>0) WRITE(LU_CORE(NM)) ZZ
IF (N_TRACKED_SPECIES>0) WRITE(LU_CORE(NM)) DEL_RHO_D_DEL_Z
!***** FIX RESTART FOR PARTICLES ONCE TYPE STRUCTURE IS FINALIZED
!IF (PARTICLE_FILE) THEN
!   WRITE(LU_CORE(NM)) NLP,NLPDIM
!   WRITE(LU_CORE(NM)) LAGRANGIAN_PARTICLE(1:NLP)
!ENDIF
DO N=1,N_INIT
   IN => INITIALIZATION(N)
...
INTEGER :: SURF_INDEX,NOM,IW,N,N_T_E_MAX,N_TOTAL_WALL_CELLS
INTEGER, INTENT(IN) :: NM
LOGICAL :: EX
CHARACTER(100) :: MESSAGE
TYPE(OMESH_TYPE), POINTER :: M2=>NULL()
...
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%UW
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%UWS
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%TMP_F
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%TMP_B
DO IW=1,N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS + N_VIRTUAL_WALL_CELLS + N_GHOST_WALL_CELLS
   READ(LU_RESTART(NM)) WALL(IW)%ONE_D%MASSFLUX
   READ(LU_RESTART(NM)) WALL(IW)%RHODW
   IF (N_TRACKED_SPECIES>0) READ(LU_RESTART(NM))  WALL(IW)%ZZ_F
ENDDO
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%ONE_D%QRADIN
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%ONE_D%QRADOUT
IF (N_LP_ARRAY_INDICES>0) READ(LU_RESTART(NM))  QR_W
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%TW
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%EW
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%KW
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%RHO_F
...
READ(LU_RESTART(NM))  WALL(1:N_TOTAL_WALL_CELLS)%BOUNDARY_TYPE
...
DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (SURFACE(SURF_INDEX)%THERMALLY_THICK) THEN
      IF (SURFACE(SURF_INDEX)%SHRINK) THEN
         READ(LU_RESTART(NM)) WALL(IW)%ONE_D%N_LAYER_CELLS,WALL(IW)%ONE_D%LAYER_THICKNESS,WALL(IW)%ONE_D%X
         IF (SUM(WALL(IW)%ONE_D%N_LAYER_CELLS)>0) THEN
            IF (ALLOCATED(WALL(IW)%ONE_D%TMP)) DEALLOCATE(WALL(IW)%ONE_D%TMP)
            ALLOCATE(WALL(IW)%ONE_D%TMP(0:SURFACE(SURF_INDEX)%N_CELLS+1))
            IF (ALLOCATED(WALL(IW)%ONE_D%RHO)) DEALLOCATE(WALL(IW)%ONE_D%RHO)
            ALLOCATE(WALL(IW)%ONE_D%RHO(0:SURFACE(SURF_INDEX)%N_CELLS+1,SURFACE(SURF_INDEX)%N_MATL))
            READ(LU_RESTART(NM)) WALL(IW)%ONE_D%TMP,WALL(IW)%ONE_D%RHO
         ENDIF
      ELSE
         IF (SURFACE(SURF_INDEX)%N_CELLS>0) THEN
            IF (ALLOCATED(WALL(IW)%ONE_D%TMP)) DEALLOCATE(WALL(IW)%ONE_D%TMP)
            ALLOCATE(WALL(IW)%ONE_D%TMP(0:SURFACE(SURF_INDEX)%N_CELLS+1))
            IF (ALLOCATED(WALL(IW)%ONE_D%RHO)) DEALLOCATE(WALL(IW)%ONE_D%RHO)
            ALLOCATE(WALL(IW)%ONE_D%RHO(0:SURFACE(SURF_INDEX)%N_CELLS+1,SURFACE(SURF_INDEX)%N_MATL))
      ...
      ENDIF
   ENDIF
   IF (RADIATION .AND. WALL(IW)%BOUNDARY_TYPE/=OPEN_BOUNDARY) READ(LU_RESTART(NM)) WALL(IW)%ONE_D%ILW
ENDDO
...
!   ALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(1:NLPDIM))
!   READ(LU_RESTART(NM)) MESHES(NM)%LAGRANGIAN_PARTICLE(1:NLP)
!ENDIF
...
REAL(EB), INTENT(IN) :: T
REAL(FB) :: STIME
INTEGER  :: NPP,NPLIM,I,N,NN,ILPC,IZERO,IBC,IW
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   DO I=1,NLP
      ...
      ILPC = LP%CLASS_INDEX
      IF (LP%SHOW .AND. ILPC==N) NPLIM = NPLIM + 1
   ENDDO
   ...
   ALLOCATE(QP(NPLIM,LPC%N_QUANTITIES),STAT=IZERO)
   ...
   LOAD_LOOP: DO I=1,NLP
      LP=>LAGRANGIAN_PARTICLE(I)
      ILPC = LP%CLASS_INDEX
      IF (.NOT.LP%SHOW .OR. ILPC/=N) CYCLE LOAD_LOOP
      NPP = NPP + 1
      ...
      DO NN=1,LPC%N_QUANTITIES
         SELECT CASE(LPC%QUANTITIES_INDEX(NN))
            ...
            CASE(434)  ! PARTICLE DIAMETER
               QP(NPP,NN) = 2.E6*MAXVAL(LP%ONE_D%X)
            CASE(435)  ! PARTICLE VELOCITY
               QP(NPP,NN) = SQRT(LP%U**2+LP%V**2+LP%W**2)
            CASE(436)  ! PARTICLE PHASE
               QP(NPP,NN) = LP%IOR
            CASE(437)  ! PARTICLE TEMPERATURE
               QP(NPP,NN) = LP%TMP_F - TMPM
            CASE(438)  ! PARTICLE MASS
               IBC = LP%SURF_INDEX
               IF (IBC<1) THEN
                  QP(NPP,NN) = 1.E3_EB*LP%ONE_D%RHO(1,1)*FOTHPI*MAXVAL(LP%ONE_D%X)**3
               ELSE
                  SF => SURFACE(IBC)
                  IW = LP%WALL_INDEX
                  SELECT CASE(SF%GEOMETRY)
                  CASE(SURF_CARTESIAN)
                     QP(NPP,NN) = 1.E3_EB*      SF%LENGTH*SF%WIDTH*MAXVAL(LP%ONE_D%X) * &
                                  SURFACE_DENSITY(NM,1,LAGRANGIAN_PARTICLE_INDEX=I) 
                  CASE(SURF_CYLINDRICAL)
                     QP(NPP,NN) = 1.E3_EB*PI*   SF%LENGTH         *MAXVAL(LP%ONE_D%X)**2 * &
                                  SURFACE_DENSITY(NM,1,LAGRANGIAN_PARTICLE_INDEX=I) 
                  CASE(SURF_SPHERICAL)
                     QP(NPP,NN) = 1.E3_EB*FOTH*PI                 *MAXVAL(LP%ONE_D%X)**3 * &
                                  SURFACE_DENSITY(NM,1,LAGRANGIAN_PARTICLE_INDEX=I) 
                  END SELECT
               ENDIF            
   ...
   IF (LPC%N_QUANTITIES > 0) WRITE(LU_PART(NM)) ((QP(I,NN),I=1,NPLIM),NN=1,LPC%N_QUANTITIES)
...
REAL(EB), INTENT(IN) :: T
REAL(EB) :: STIME
INTEGER  :: NPP,NPLIM,I,N,NN,ILPC,IZERO
...
INTEGER :: HAVE_TDATA, HAVE_IBLANK
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   IF (IS%INDEX2==0) THEN
      ...
      CALL ISO_TO_FILE(LU_ISOF(N,NM),STIME,QTEMP,HAVE_TDATA,QTEMP,HAVE_IBLANK,IBLK,&
...
!      CALL ISO_TO_FILE(LU_ISOF(N,NM),STIME,QTEMP,HAVE_TDATA,QTEMP2,HAVE_IBLANK,IBLK,&
...
INTEGER, INTENT(IN) :: NM
REAL(EB), INTENT(IN) :: T
REAL(EB) :: SUM,DROPMASS,RVC,TT
INTEGER :: IFRMT,I,J,K,II,JJ,KK,NQT,I1,I2,J1,J2,K1,K2,ITM,ITM1,IQ,IQQ,IND,IND2,II1,II2,JJ1,JJ2,KK1,KK2, &
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,&
            ...
            DHOR,X_EQUIL,MW_RATIO,Y_EQUIL,EXPON,Y_SPECIES,MEC,Y_SPECIES2,Y_H2O,R_Y_H2O,R_DN,SGN,Y_ALL(N_SPECIES),H_S
REAL(FB) :: TMPUP,TMPLOW,ZINT,TMP_WGT,DELTA,RHO2
INTEGER :: N,I,J,K,NN,IL,III,JJJ,KKK,IW,Y_INDEX,Z_INDEX,PART_INDEX,ITMP,IP,JP,KP,FLOW_INDEX
CHARACTER(100) :: MESSAGE
...
!      GAS_PHASE_OUTPUT = SUM(ZZ(II,JJ,KK,I_Z_MIN:I_Z_MAX))
   ...
   CASE(21)  ! RELATIVE HUMIDITY
      ...
      ELSE
         ...
         DHOR    = (SPECIES(H2O_INDEX)%H_V(ITMP)+TMP_WGT*(SPECIES(H2O_INDEX)%H_V(ITMP+1)-SPECIES(H2O_INDEX)%H_V(ITMP)))* &
                   SPECIES(H2O_INDEX)%MW/R0
         X_EQUIL  = MIN(1._EB,EXP(DHOR*(1._EB/SPECIES(H2O_INDEX)%TMP_V-1._EB/MIN(TMP(II,JJ,KK),SPECIES(H2O_INDEX)%TMP_V))))
         Y_MF_INT=0._EB
         ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
         ...
         ELSE
            ...
            MW_RATIO = MW_RATIO/SPECIES(H2O_INDEX)%MW 
   ...
   CASE(46:47)  ! SPECIFIC ENTHALPY and ENTHALPY
      ...
      IF (IND==46) GAS_PHASE_OUTPUT = CPBAR*TMP(II,JJ,KK)*0.001_EB
      IF (IND==47) GAS_PHASE_OUTPUT = RHO(II,JJ,KK)*CPBAR*TMP(II,JJ,KK)*0.001_EB
      
   CASE(48:49)  ! SPECIFIC SENSIBLE ENTHALPY and SENSIBLE ENTHALPY
      IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,TMP(II,JJ,KK))
      IF (IND==48) GAS_PHASE_OUTPUT = H_S*0.001_EB
      IF (IND==49) GAS_PHASE_OUTPUT = RHO(II,JJ,KK)*H_S*0.001_EB
   ...
   CASE(164) ! CABLE TEMPERATURE
      ...
      GAS_PHASE_OUTPUT = WALL(IW)%ONE_D%TMP(DV%I_DEPTH) - TMPM

   CASE(170) ! MPUV
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT = AVG_DROP_DEN(II,JJ,KK,LPC%ARRAY_INDEX)

   CASE(171) ! ADD
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT = AVG_DROP_RAD(II,JJ,KK,LPC%ARRAY_INDEX)*2.E6_EB

   CASE(172) ! ADT
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT = AVG_DROP_TMP(II,JJ,KK,LPC%ARRAY_INDEX) - TMPM
      ...
      IF ( ((PY%PDPA_START<=T) .AND.(PY%PDPA_END>=T)) .OR. .NOT.PY%PDPA_INTEGRATE ) THEN
         ...
         DLOOP: DO I=1,NLP
            ...
            SELECT CASE(PY%QUANTITY)
               ...
               CASE('ENTHALPY')
                  ITMP = INT(LP%TMP_F)
                  TMP_WGT = LP%TMP_F - AINT(LP%TMP_F)
                  VEL = (SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP)+TMP_WGT*&
                        (SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP+1)-SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP)))*LP%TMP_F
                  ITMP = INT(SPECIES(LPC%Y_INDEX)%TMP_MELT)
                  TMP_WGT = SPECIES(LPC%Y_INDEX)%TMP_MELT - AINT(SPECIES(LPC%Y_INDEX)%TMP_MELT)
                  VEL = VEL - (SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP)+TMP_WGT*&
                              (SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP+1)-SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP)))*&
                               SPECIES(LPC%Y_INDEX)%TMP_MELT
...
SELECT CASE(INDX)
   CASE( 1) ! RADIATIVE HEAT FLUX
      SOLID_PHASE_OUTPUT = (WC%ONE_D%QRADIN-WC%ONE_D%QRADOUT)*0.001_EB
   CASE( 2) ! CONVECTIVE HEAT FLUX
      SOLID_PHASE_OUTPUT = WC%ONE_D%QCONF*0.001_EB
   CASE( 3) ! NORMAL VELOCITY
      SOLID_PHASE_OUTPUT = WC%UW
   ...
   CASE( 5) ! WALL TEMPERATURE
      SOLID_PHASE_OUTPUT = WC%TMP_F - TMPM

   CASE( 6) ! INSIDE WALL TEMPERATURE
      IF (SURFACE(WC%SURF_INDEX)%SHRINK) THEN
         II2 = SUM(WC%ONE_D%N_LAYER_CELLS)
         IF (DV%DEPTH>WC%ONE_D%X(II2)) THEN
            SOLID_PHASE_OUTPUT = TMPA-TMPM
         ELSE
            DO II2=II2,1,-1
               IF (DV%DEPTH<=WC%ONE_D%X(II2)) II1 = II2
            ENDDO
            SOLID_PHASE_OUTPUT = WC%ONE_D%TMP(II1)-TMPM
         ENDIF
      ELSE
         SOLID_PHASE_OUTPUT = WC%ONE_D%TMP(DV%I_DEPTH) - TMPM
      ENDIF

   CASE( 7) ! BURNING RATE
      IF (N_REACTIONS>0) THEN
         SOLID_PHASE_OUTPUT = WC%ONE_D%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)
      ELSE
         SOLID_PHASE_OUTPUT = 0._EB
      ENDIF
   CASE( 8) ! HRRPUM
      SOLID_PHASE_OUTPUT = WC%ONE_D%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)*0.001*REACTION(1)%HEAT_OF_COMBUSTION/ &
                           SURFACE(WC%SURF_INDEX)%SURFACE_DENSITY
   CASE( 9) ! HRRPUA
      SOLID_PHASE_OUTPUT = WC%ONE_D%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)*0.001*REACTION(1)%HEAT_OF_COMBUSTION
   CASE(10) ! NET HEAT FLUX
      SOLID_PHASE_OUTPUT = (WC%ONE_D%QRADIN-WC%ONE_D%QRADOUT+WC%ONE_D%QCONF)*0.001_EB
   ...
   CASE(12) ! BACK WALL TEMPERATURE
      SOLID_PHASE_OUTPUT = WC%TMP_B - TMPM
   CASE(13) ! GAUGE HEAT FLUX
      CONCORR = WC%ONE_D%HEAT_TRANS_COEF*(WC%TMP_F-PY%GAUGE_TEMPERATURE)
      SOLID_PHASE_OUTPUT = (WC%ONE_D%QRADIN/(WC%ONE_D%EMISSIVITY+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4 + &
                            WC%ONE_D%QCONF + CONCORR)*0.001_EB
   CASE(19) ! INCIDENT HEAT FLUX
      SOLID_PHASE_OUTPUT = ( WC%ONE_D%QRADIN/(WC%ONE_D%EMISSIVITY+1.0E-10_EB) + WC%ONE_D%QCONF )*0.001_EB
   CASE(20) ! MASS FLUX
     IF (Z_INDEX >=0) THEN
        SOLID_PHASE_OUTPUT = WC%ONE_D%MASSFLUX(Z_INDEX)
     ELSEIF(Y_INDEX >0) THEN
        ZZ_GET(1:N_TRACKED_SPECIES) = WC%ZZ_F(1:N_TRACKED_SPECIES)
        CALL GET_MASS_FRACTION(ZZ_GET,Y_INDEX,Y_SPECIES)
        SOLID_PHASE_OUTPUT = SUM(WC%ONE_D%MASSFLUX)*Y_SPECIES
     ENDIF
   CASE(21) ! HEAT TRANSFER COEFFICENT
      SOLID_PHASE_OUTPUT = WC%ONE_D%HEAT_TRANS_COEF
   CASE(22) ! RADIOMETER
      SOLID_PHASE_OUTPUT = (WC%ONE_D%QRADIN/(WC%ONE_D%EMISSIVITY+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4)*0.001_EB
   CASE(23) ! ADIABATIC SURFACE TEMPERATURE
      SOLID_PHASE_OUTPUT_OLD = WC%TMP_F
      SOLID_PHASE_OUTPUT     = WC%TMP_F
      ADLOOP: DO
         CONCORR = WC%ONE_D%HEAT_TRANS_COEF*(WC%TMP_F-SOLID_PHASE_OUTPUT_OLD)      
         FLUX = WC%ONE_D%QCONF + CONCORR + WC%ONE_D%QRADIN - WC%ONE_D%EMISSIVITY * SIGMA * SOLID_PHASE_OUTPUT_OLD ** 4
         DFLUXDT = -4._EB*WC%ONE_D%EMISSIVITY*SIGMA*SOLID_PHASE_OUTPUT_OLD**3 - WC%ONE_D%HEAT_TRANS_COEF
   ...
   CASE(24) ! WALL THICKNESS
      IF (SURFACE(WC%SURF_INDEX)%SHRINK) THEN
         SOLID_PHASE_OUTPUT = SUM(WC%ONE_D%LAYER_THICKNESS)
      ELSE
         IF (SURFACE(WC%SURF_INDEX)%THERMALLY_THICK) THEN
            SOLID_PHASE_OUTPUT = MAXVAL(SURFACE(WC%SURF_INDEX)%X_S)
         ELSE
            SOLID_PHASE_OUTPUT = 0._EB
         ENDIF
      ENDIF
   ...
   CASE(26) ! SOLID DENSITY
      ...
      DO NN=1,SF%N_MATL
         IF (DV%MATL_ID==SF%MATL_NAME(NN)) THEN
            SOLID_PHASE_OUTPUT = WC%ONE_D%RHO(II1,NN)
            RETURN
         ENDIF
      ENDDO
      SOLID_PHASE_OUTPUT = 0._EB
   ...
   CASE(27) ! EMISSIVITY
      SOLID_PHASE_OUTPUT = WC%ONE_D%EMISSIVITY
   ...
   CASE(33) ! SOLID SPECIFIC HEAT
      ...
      SF => SURFACE(WALL(IWX)%SURF_INDEX)
      II1 = DV%I_DEPTH
      IF (SF%SHRINK) THEN
         II2 = SUM(WC%ONE_D%N_LAYER_CELLS)
         IF (DV%DEPTH>WC%ONE_D%X(II2)) THEN
      ...
      MATERIAL_LOOP_CP: DO NN=1,SF%N_MATL
         IF (WC%ONE_D%RHO(II1,NN)<=ZERO_P) CYCLE MATERIAL_LOOP_CP
         ML  => MATERIAL(SF%MATL_INDEX(NN))
         VOLSUM = VOLSUM + WC%ONE_D%RHO(II1,NN)/ML%RHO_S         
         IF (ML%C_S>0._EB) THEN
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%ONE_D%RHO(II1,NN)*ML%C_S/ML%RHO_S
         ELSE
            NR     = -NINT(ML%C_S)
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%ONE_D%RHO(II1,NN)*EVALUATE_RAMP(WC%ONE_D%TMP(II1),0._EB,NR)* &
   ...
   CASE(34) ! SOLID CONDUCTIVITY 
      ...
      MATERIAL_LOOP_K: DO NN=1,SF%N_MATL
         IF (WC%ONE_D%RHO(II1,NN)<=ZERO_P) CYCLE MATERIAL_LOOP_K
         ML  => MATERIAL(SF%MATL_INDEX(NN))
         VOLSUM = VOLSUM + WC%ONE_D%RHO(II1,NN)/ML%RHO_S         
         IF (ML%K_S>0._EB) THEN  
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%ONE_D%RHO(II1,NN)*ML%K_S/ML%RHO_S
         ELSE
            NR     = -NINT(ML%K_S)
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + WC%ONE_D%RHO(II1,NN)*EVALUATE_RAMP(WC%ONE_D%TMP(II1),0._EB,NR)/ML%RHO_S
         ENDIF
...
PART_DIST_LOOP: DO I=1,N_LAGRANGIAN_CLASSES
   ...
   IF (.NOT.LPC%CHECK_DISTRIBUTION .OR. LPC%MONODISPERSE .OR. LPC%DIAMETER<ZERO_P) CYCLE PART_DIST_LOOP
   LU = GET_FILE_NUMBER()
   ...
   DO J=1,NDC
      WRITE(LU,'(E16.9,A,E16.9)') LPC%R_CDF(J),',',LPC%CDF(J)
   ENDDO
   CLOSE(LU)
...
PROF_LOOP: DO N=1,N_PROF
   ...
   IF (.NOT.WC%ALREADY_ALLOCATED .OR. WC%BOUNDARY_TYPE==NULL_BOUNDARY) CYCLE PROF_LOOP
   ...
   ELSE
      WC%ONE_D%RHO(0,1:SF%N_MATL)     = WC%ONE_D%RHO(1,1:SF%N_MATL)
      WC%ONE_D%RHO(NWP+1,1:SF%N_MATL) = WC%ONE_D%RHO(NWP,1:SF%N_MATL)
      RHO_S = 0._EB
      DO NN=1,SF%N_MATL
...
REAL(EB) :: VC,TMP_N,RHO_N,U_N,ZZ_GET(0:N_TRACKED_SPECIES),H_S,HDIFF,H_S_J_ALPHA,CP,SPECIFIC_ENTHALPY_SUM_OLD
INTEGER, INTENT(IN) :: NM
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,TMP_N)
   CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,TMP_N)
   H_S_J_ALPHA = 0._EB
   DO N=1,N_TRACKED_SPECIES
      CALL GET_SENSIBLE_ENTHALPY_DIFF(N,WC%TMP_F,HDIFF)
      H_S_J_ALPHA = H_S_J_ALPHA + 2._EB*HDIFF*WC%RHODW(N)*(ZZ(IIG,JJG,KKG,N)-WC%ZZ_F(N))*WC%RDN
   ENDDO
   Q_DOT(3,NM) = Q_DOT(3,NM) - U_N*RHO_N*H_S*WC%AW
   Q_DOT(4,NM) = Q_DOT(4,NM) - WC%ONE_D%QCONF*WC%AW
   Q_DOT(5,NM) = Q_DOT(5,NM) - H_S_J_ALPHA*WC%AW
ENDDO WALL_LOOP

! Determine mass loss rate of fuel, M_DOT(1,NM)
...
IF (N_REACTIONS > 0) THEN
   WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
      WC => WALL(IW)
      IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY .AND. WC%BOUNDARY_TYPE/=VIRTUAL_BOUNDARY) CYCLE WALL_LOOP2
      M_DOT(1,NM) = M_DOT(1,NM) + WC%ONE_D%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)*WC%AW*WC%AREA_ADJUST
   ENDDO WALL_LOOP2
ENDIF

Q_DOT_SUM(:,NM) = Q_DOT_SUM(:,NM) + DT*Q_DOT(:,NM)
M_DOT_SUM(1,NM) = M_DOT_SUM(1,NM) + DT*SUM(M_DOT(:,NM))
...
REAL(FB) :: STIME
INTEGER :: NM,I
REAL(EB) :: Q_DOT_TOTAL(N_Q_DOT),M_DOT_TOTAL(N_M_DOT)
...
MESH_LOOP: DO NM=1,NMESHES
  ...
  IF (.NOT. TREE_MESH_OUT(NM)) CYCLE MESH_LOOP 
   VEG_PRESENT = .TRUE.
!  R_SVxPR = 1.0_EB/LPC%VEG_SV*LP%VEG_PACKING_RATIO
...
STATIC_IF: IF (ABS(T-T_BEGIN)<ZERO_P) THEN
   STATIC_LOOP: DO N=1,1
      ...
      WRITE(LU_GEOM(N)) N_VERT,N_FACE
      IF (N_VERT>0) WRITE(LU_GEOM(N)) (REAL(VERTEX(I)%X,FB),REAL(VERTEX(I)%Y,FB),REAL(VERTEX(I)%Z,FB),I=1,N_VERT)
      IF (N_FACE>0) THEN
        WRITE(LU_GEOM(N)) (FACET(I)%VERTEX,I=1,N_FACE)
        WRITE(LU_GEOM(N)) (FACET(I)%SURF_INDEX,I=1,N_FACE)
      ENDIF
   ENDDO STATIC_LOOP
ENDIF STATIC_IF
...
DO N=1,4
   ...
   IF (EX) THEN
      CLOSE(ABS(LU_SMOKE3D(N,NM)))
      IF(N.LE.2)THEN
         OPEN(ABS(LU_SMOKE3D(N,NM)),FILE=FN_SMOKE3D(N,NM),FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
      ELSE
         OPEN(ABS(LU_SMOKE3D(N,NM)),FILE=FN_SMOKE3D(N,NM),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
      ENDIF
   ENDIF
END DO  
