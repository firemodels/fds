...
REAL(EB) :: SUM,CONST,RHYMI,RHYPI,RHYMJ,RHYPJ,RHYMK,RHYPK,RHY0,YMI,YPI,YMJ,YPJ,YMK,YPK,Y00,YMIN,YMAX,YY_GET(1:N_SPECIES)
...
USE GLOBAL_CONSTANTS, ONLY: N_SPECIES,PREDICTOR,CORRECTOR,FLUX_LIMITER,NULL_BOUNDARY,POROUS_BOUNDARY,OPEN_BOUNDARY, &
                            LU_ERR,INTERPOLATED_BOUNDARY,PERIODIC_TEST,MYID,EMBEDDED_MESH
...
WALL_LOOP: DO IW=1,NWC
   ...
   SELECT CASE(IOR)
      CASE( 1)
         ...
         IF (BOUNDARY_TYPE(IW)/=INTERPOLATED_BOUNDARY .OR. EMBEDDED_MESH) THEN
