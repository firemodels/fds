...
USE PHYSICAL_FUNCTIONS, ONLY: GET_VISCOSITY,LES_FILTER_WIDTH
...
REAL(EB) :: ZZ_GET(0:N_TRACKED_SPECIES),NU_EDDY,DELTA,KSGS,U2,V2,W2,AA,A_IJ(3,3),BB,B_IJ(3,3),&
            DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,MU_DNS,VDF,DUMMY
   ...
   CASE (DEARDORFF) SELECT_TURB ! Deardorff (1980) eddy viscosity model (current default)
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
               DELTA = LES_FILTER_WIDTH(DX(I),DY(J),DZ(K))
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      CASE(SOLID_BOUNDARY)
         ...
         CALL WALL_MODEL(DUMMY,WC%U_TAU,WC%Y_PLUS,SQRT(2._EB*KRES(IIG,JJG,KKG)),&
...
HEAT_TRANSFER_IF: IF (CHECK_HT) THEN
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      UVW = WC%ONE_D%HEAT_TRANS_COEF/(WC%RHO_F*CP)
      IF (UVW>=UVWMAX) THEN
         UVWMAX = UVW
         ICFL=IIG
         JCFL=JJG
...
IF (IMMERSED_BOUNDARY_METHOD==2) THEN
   ...
   DELTA = LES_FILTER_WIDTH(DX(1),DY(1),DZ(1))
