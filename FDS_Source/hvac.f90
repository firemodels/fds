...
INTEGER :: IOS,IZERO,N_HVAC_READ,NS,N,ND,NN,I_DUCT=0,I_DUCTNODE=0,I_FAN=0 !, &
...
DO ND = 1, N_DUCTS
   ...
   IF (DU%NODE_INDEX(1) <= 0) THEN
      WRITE(MESSAGE,'(A,I2,A,A)') 'ERROR: Duct node 1 not located, DUCT ',ND,', DUCT ID:',TRIM(DU%ID)
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (DU%NODE_INDEX(2) <= 0) THEN
      WRITE(MESSAGE,'(A,I2,A,A)') 'ERROR: Duct node 2 not located, DUCT ',ND,', DUCT ID:',TRIM(DU%ID)
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (DU%NODE_INDEX(1) == DU%NODE_INDEX(2)) THEN
      WRITE(MESSAGE,'(A,I2,A,A)') 'ERROR: Duct node 1 cannot be the same as duct node 2, DUCT ',ND,', DUCT ID:',TRIM(DU%ID)
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (DUCT_FAN_A(ND)/='null') THEN
      ...
      IF (DU%FAN_INDEX <= 0) THEN
         WRITE(MESSAGE,'(A,I2,A,A)') 'ERROR: Fan not located for DUCT ',ND,', DUCT ID:',TRIM(DU%ID)
         CALL SHUTDOWN(MESSAGE)
      ENDIF
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   ...
   IF (DN%VENT_INDEX > 0 .AND. DN%AMBIENT) THEN
      WRITE(MESSAGE,'(A,I,A,A)') 'ERROR: DUCTNODE cannot be AMBIENT and have an assigned VENT_ID, DUCTNODE ',NN,&
                                  ', DUCTNODE ID:',TRIM(DN%ID)
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (DN%N_DUCTS == 1 .AND. DN%VENT_INDEX < 0 .AND. .NOT. DN%AMBIENT) THEN
      WRITE(MESSAGE,'(A,I,A,A)') 'ERROR: Internal DUCTNODE must have at least two attached ducts, DUCTNODE ',NN,&
                                  ', DUCTNODE ID:',TRIM(DN%ID)
      CALL SHUTDOWN(MESSAGE)
   ENDIF  
   IF (DN%N_DUCTS> 1 .AND. (DN%AMBIENT .OR. DN%VENT_INDEX > 0) ) THEN
      WRITE(MESSAGE,'(A,I,A,A)') 'ERROR: External DUCTNODE can only have one attached duct, DUCTNODE ',NN,&
                                  ', DUCTNODE ID:',TRIM(DN%ID)
      CALL SHUTDOWN(MESSAGE)
   ENDIF  
...
REAL(EB) :: TNOW,MTOT,ETOT,VTOT,ZZTOT(1:N_TRACKED_SPECIES),TGUESS,VFLOW,ZZ_GET(0:N_TRACKED_SPECIES)
...
ITER_LOOP: DO
   ...
   NODE_LOOP:DO NN = 1,NE%N_DUCTNODES
      ...
      DN%TMP = TGUESS
      CP_LOOP: DO
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,DN%CP,TGUESS)
         DN%TMP = ETOT /DN%CP
...
DO ND = 1, NE%N_DUCTS
   ...
   IF(.NOT. DU%LEAKAGE) THEN
      ...
      ENDIF
   ENDIF
   RHS(ARRAYLOC) = DU%VEL(OLD)-DT/DU%LENGTH*(HEAD+DU%DP_FAN)/DU%RHO_D+0.5_EB*DU%TOTAL_LOSS*ABS(DU%VEL(NEW))*DU%VEL(GUESS)*DT/DU%LENGTH
ENDDO
...
SELECT CASE (FA%FAN_TYPE)
   CASE(1) !Constant flow
      ...
      RETURN     
   CASE(2) !Quadratic
      VEL = -DU%VEL(NEW)*DU%AREA
      IF (DU%REVERSE) VEL = -VEL
      DEL_P = SQRT(ABS(VEL-FA%MAX_FLOW)/FA%MAX_FLOW)*SIGN(1._EB,VEL)*FA%MAX_PRES*EVALUATE_RAMP(T,FA%TAU,FA%SPIN_INDEX)
      !VEL = -FA%MAX_FLOW*SIGN(1._EB,DEL_P)*SQRT(ABS(DEL_P)/FA%MAX_PRES)/DU%AREA*EVALUATE_RAMP(T,FA%TAU,FA%SPIN_INDEX)
   CASE(3) !Fan curve
      VEL = -DU%VEL(NEW)*DU%AREA
      IF (DU%REVERSE) VEL = -VEL
      DEL_P = EVALUATE_RAMP(VEL,0._EB,FA%RAMP_INDEX)*EVALUATE_RAMP(T,FA%TAU,FA%SPIN_INDEX)      
END SELECT

IF (DU%REVERSE) DEL_P=-DEL_P
DU%DP_FAN = 0.4_EB*DEL_P+0.6_EB*DU%DP_FAN
...
REAL(EB) :: ZZ_SUM(1:N_TRACKED_SPECIES),ZZ_GET(0:N_TRACKED_SPECIES),RHO_SUM,P_SUM,AREA_SUM,AREA, &
            TMP_SUM,H_SUM,H_G,TMP_NEW,CPBAR,TNOW,P_AVE
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   ...
   VENT_LOOP: DO KK = K1,K2
      DO JJ = J1,J2
         DO II = I1, I2
            ...
            END SELECT
            P_SUM = P_SUM + P_AVE*AREA!(P_AVE+RHO(II,JJ,KK)*(HP(II,JJ,KK)-KRES(II,JJ,KK)))*AREA      
   ...
   IF (N_TRACKED_SPECIES>0) NODE_ZZ(NN,1:N_TRACKED_SPECIES,NM) = ZZ_SUM(1:N_TRACKED_SPECIES) / RHO_SUM
   H_SUM = H_SUM / RHO_SUM
   DO
      IF (N_TRACKED_SPECIES > 0) ZZ_GET(1:N_TRACKED_SPECIES) = DN%ZZ_V(1:N_TRACKED_SPECIES)
         ...
         ELSEIF (DU%FAN_INDEX > 0) THEN
            ...
            ELSEIF (DU%CTRL_INDEX > 0) THEN
               ...
               IF (DU%FAN_OPERATING .AND. CONTROL(DU%CTRL_INDEX)%CURRENT_STATE .NEQV. CONTROL(DU%CTRL_INDEX)%PRIOR_STATE) &
                  DU%FAN_ON_TIME = T
            ENDIF
            IF (FAN(DU%FAN_INDEX)%FAN_TYPE==1) DU%FIXED=0._EB
         ENDIF
...
REAL(EB) :: AREA_SUM,AREA,CPBAR,H_G,TMP_SUM,TMP_NEW,ZZ_GET(0:N_TRACKED_SPECIES)
TYPE(DUCTNODE_TYPE), POINTER :: DN=>NULL()
