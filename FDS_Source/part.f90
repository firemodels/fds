...
PART_CLASS_LOOP: DO IPC=1,N_PART
   ...
   IF_SIZE_DISTRIBUTION: IF (.NOT.PC%MONODISPERSE .AND. (PC%DIAMETER > 0._EB)) THEN
      IF(PC%CNF_RAMP_INDEX<0) THEN
         CALL DROPLET_SIZE_DISTRIBUTION(PC%DIAMETER,PC%R_CDF(:),PC%CDF(:),NDC,PC%GAMMA,PC%SIGMA,PC%DISTRIBUTION)
      ELSE
...
DROPLET_LOOP: DO I=1,NLP  
   ...
   PARTICLE_NON_STATIC_IF: IF (.NOT.PC%STATIC .AND. DR%IOR==0) THEN
      ...
      IF (BETA>ZERO_P) THEN
         ...
         ! semi-analytical solution for droplet position
         ALBO = ALPHA*LOG(OBDT)/BETA/OPA
         DR%X = X_OLD + (U_OLD+ALPHA*UBAR)*DTOPA + ALBO*(U_OLD-UBAR) + GVEC(1)*HALF_DT2
         DR%Y = Y_OLD + (V_OLD+ALPHA*VBAR)*DTOPA + ALBO*(V_OLD-VBAR) + GVEC(2)*HALF_DT2
         DR%Z = Z_OLD + (W_OLD+ALPHA*WBAR)*DTOPA + ALBO*(W_OLD-WBAR) + GVEC(3)*HALF_DT2
      ELSE
         ! no drag
         DR%A_X  = 0._EB
         DR%A_Y  = 0._EB
         DR%A_Z  = 0._EB
         DR%X = X_OLD + DT*U_OLD + GVEC(1)*HALF_DT2
         DR%Y = Y_OLD + DT*V_OLD + GVEC(2)*HALF_DT2
         DR%Z = Z_OLD + DT*W_OLD + GVEC(3)*HALF_DT2
      ENDIF
