...
REAL(EB) :: X1,X2,Y1,Y2,Z1,Z2,XX1,XX2,YY1,YY2,ZZ1,ZZ2,PRIMARY_VOLUME,OVERLAP_VOLUME,DXX,DYY,DZZ
...
PRIMARY_WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   CALL GET_WALL_NODE_COORDINATES(ONE_D%N_LAYER_CELLS(1),SF%N_CELLS_MAX+1,SF%N_LAYERS,ONE_D%N_LAYER_CELLS, &
                                  N_LAYER_CELLS_OLD(1),ONE_D%SMALLEST_CELL_SIZE(1:SF%N_LAYERS), &
                                  SF%STRETCH_FACTOR(1:SF%N_LAYERS),REMESH_LAYER(1:SF%N_LAYERS),ONE_D%X,X_S_OLD(0:SF%N_CELLS_MAX))
   DEALLOCATE(X_S_OLD)
...
PRIMARY_WALL_LOOP_2: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   PRIMARY_NODE_LOOP: DO I=1,NWP  ! Loop over nodes of primary wall cell
      ...
      SELECT CASE(BC%IOR)
         CASE( 1) ; X1=BC%X-ONE_D%X(I)   ; X2=BC%X-ONE_D%X(I-1) 
         CASE(-1) ; X1=BC%X+ONE_D%X(I-1) ; X2=BC%X+ONE_D%X(I) 
         CASE( 2) ; Y1=BC%Y-ONE_D%X(I)   ; Y2=BC%Y-ONE_D%X(I-1) 
         CASE(-2) ; Y1=BC%Y+ONE_D%X(I-1) ; Y2=BC%Y+ONE_D%X(I) 
         CASE( 3) ; Z1=BC%Z-ONE_D%X(I)   ; Z2=BC%Z-ONE_D%X(I-1) 
         CASE(-3) ; Z1=BC%Z+ONE_D%X(I-1) ; Z2=BC%Z+ONE_D%X(I) 
      END SELECT
      IF (WC%THIN(1)) THEN ; X1=M%X(BC%II) ; X2=X1+BC%DX ; ENDIF
      IF (WC%THIN(2)) THEN ; Y1=M%Y(BC%JJ) ; Y2=Y1+BC%DY ; ENDIF
      IF (WC%THIN(3)) THEN ; Z1=M%Z(BC%KK) ; Z2=Z1+BC%DZ ; ENDIF
      PRIMARY_VOLUME = (X2-X1)*(Y2-Y1)*(Z2-Z1)
if (bc%iig==10 .and. bc%jjg==12 .and. bc%ior==-1 .and. i==1) then
   write(0,*) 'primary: ',x1,x2,y1,y2,z1,z2,PRIMARY_VOLUME
endif
      IOR_AVOID = .FALSE.
      ...
      !  IF (ABS(BC2%IOR)/=3) THEN ; DZZ = MIN(ZZ2,Z2)-MAX(ZZ1,Z1) ; IF (DZZ<=0._EB) CYCLE ALTERNATE_WALL_LOOP ; ENDIF
         ...
         ALTERNATE_NODE_LOOP: DO I2=1,NWP2  ! Loop over nodes of alternate wall cell
            ...
            END SELECT
      IF (WC2%THIN(1)) THEN ; XX1=M%X(BC2%II) ; XX2=XX1+BC2%DX ; ENDIF
      IF (WC2%THIN(2)) THEN ; YY1=M%Y(BC2%JJ) ; YY2=YY1+BC2%DY ; ENDIF
      IF (WC2%THIN(3)) THEN ; ZZ1=M%Z(BC2%KK) ; ZZ2=ZZ1+BC2%DZ ; ENDIF
...
if (bc%iig==10 .and. bc%jjg==12 .and. bc%ior==-1 .and. i==1) then
   write(0,*) 'weight : ',xx1,xx2,yy1,yy2,zz1,zz2,OVERLAP_VOLUME/PRIMARY_VOLUME
endif
            IWA = IWA + 1
            ...
            IF (IWA>DM) THEN  ! If the primary node array does not have enough entries, add more
               ...
               CALL MOVE_ALLOC(INTEGER_DUMMY,THR_D%NODE(I)%ALTERNATE_WALL_INDEX) ; THR_D%NODE(I)%ALTERNATE_WALL_INDEX(DM+1:DM+8) = 0
               ALLOCATE(INTEGER_DUMMY(DM+8))
            ...
            THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(IWA) = OVERLAP_VOLUME/PRIMARY_VOLUME
            IOR_AVOID(-BC2%IOR) = .TRUE.  ! Do not use the opposite side wall
         ENDDO ALTERNATE_NODE_LOOP

      ENDDO ALTERNATE_WALL_LOOP

if (bc%iig<13 .and. bc%jjg<13) write(0,*) iw,bc%iig,bc%jjg,bc%kkg,bc%ior,i,sum(THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(:))
