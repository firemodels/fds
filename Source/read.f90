...
PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   ! Set surrogate dimensions when undefined for screen drag or porous media drag
   IF(SF%LENGTH < TWO_EPSILON_EB) THEN
      IF (SF%GEOMETRY==SURF_CARTESIAN) THEN
         IF (LPC%DRAG_LAW==SCREEN_DRAG) SF%LENGTH = SQRT(LPC%FREE_AREA_FRACTION)
         IF (LPC%DRAG_LAW==POROUS_DRAG) SF%LENGTH = SQRT(LPC%POROUS_VOLUME_FRACTION/SF%THICKNESS)
         IF(SF%WIDTH < TWO_EPSILON_EB) SF%WIDTH = SF%LENGTH
      ELSEIF (SF%GEOMETRY==SURF_CYLINDRICAL) THEN
         IF (LPC%DRAG_LAW==SCREEN_DRAG) SF%LENGTH = 0.5_EB*LPC%FREE_AREA_FRACTION/(SF%INNER_RADIUS + SF%THICKNESS)
         IF (LPC%DRAG_LAW==POROUS_DRAG) SF%LENGTH = LPC%POROUS_VOLUME_FRACTION/ &
               (PI*((SF%INNER_RADIUS + SF%THICKNESS)**2._EB))
      ENDIF
   ENDIF
   ...
   IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      ...
      DO NN=1,SF%N_CELLS_INI
         SELECT CASE (SF%GEOMETRY)
            CASE (SURF_CARTESIAN)
               MASS = MASS + 2._EB*SF%LENGTH*SF%WIDTH*(SF%X_S(NN)-SF%X_S(NN-1))*SUM(SF%RHO_0(NN,1:SF%N_MATL))
               VOLUME = VOLUME + 2._EB*SF%LENGTH*SF%WIDTH*(SF%X_S(NN)-SF%X_S(NN-1))
...
REAL(EB) :: EMBER_GENERATION_HEIGHT(2),EMBER_POWER_MEAN,EMBER_POWER_SIGMA,EMBER_TRACKING_RATIO,EMBER_YIELD
...
NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BLOWING,BURN_AWAY,BURN_DURATION,&
                ...
                EMBER_GENERATION_HEIGHT,EMBER_POWER_MEAN,EMBER_POWER_SIGMA,EMBER_TRACKING_RATIO,EMBER_YIELD,&
