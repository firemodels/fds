...
if [[ `grep -E "Error:|Fatal error|! LaTeX Error:" -I SMV_User_Guide.err | grep -v "xpdf supports version 1.5"` == "" ]]
   then
      # Continue along
      :
   else
      echo "LaTeX errors detected:"
      grep -E "Error:|Fatal error|! LaTeX Error:" -I SMV_User_Guide.err | grep -v "xpdf supports version 1.5"
      clean_build=0
fi
