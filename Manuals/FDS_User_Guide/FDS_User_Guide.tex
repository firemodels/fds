...
\noindent define two gases, both of which are {\ct 'CARBON DIOXIDE'}.  Both will use the built in property data for CO$_2$.  The {\ct ALIAS} for each species is then used in the remainder of the input file.  The {\ct SMIX\_COMPNONENT\_ONLY} is given so that FDS only tracks {\ct 'CO2 1'} and {\ct 'CO2 2'} and not also {\ct 'CARBON DIOXIDE'}.  Note that the name you provide for {\ct ALIAS} should not match the name of any other {\ct SPEC} or {\ct SMIX}.  When using {\ct ALIAS}, species outputs that reference the {\ct ALIAS} will only output the {\ct ALAIS} species and use of the original species name will sum over all applicable species.  For example, if the output quantities in the example above are changed as shown below, then {\ct Device 1} would just output the mass fraction of {\ct CO2 1} and {\ct Device 2} would output the sum of both {
...
\footnotesize
\begin{verbatim}
&REAC FUEL = 'PROPANE', HEAT_OF_COMBUSTION=44715., SPEC_ID_NU = 'PROPANE','AIR','PRODUCTS','SOOT',
