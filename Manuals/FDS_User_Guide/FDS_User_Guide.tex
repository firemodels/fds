...
associate the input file with the FDS executable, be careful not to accidental double-click on the input file when trying to edit it.
...
fThe parameters in the input file can be integers ({\ct T\_END=5400}),
...
Below are three example duct node inputs representing a typical tee-type connection (mutiple ducts being joined), a connection to the FDS domain, and a connection to the ambient outside the FDS domain.
\footnotesize
...
%Undocumented: HRRPUVCUT_MAX, HRRPUV_MAX_SMV, NEW_EVAP, TENSOR_DIFFUSIVIRY,  TERRAIN_IMAGE, VEG_LEVEL_SET, WFDS
...
% Undocumented:
% DENSE_VOLUME_FRACTION, TREE,
% VEG_BULK_DENSITY, VEG_BURNING_RATE_MAX,VEG_CHAR_FRACTION,VEG_CHAR_OXIDATION,
% VEG_DEGRADATION, VEG_DEHYDRATION_RATE_MAX, VEG_DENSITY, VEG_DRAG_COEFFICIENT,
% VEG_FUEL_MPV_MIN, VEG_INITIAL_TEMPERATURE, VEG_MOISTURE, VEG_MOIST_MPV_MIN,
% VEG_REMOVE_CHARRED, VEG_STEM, VEG_SV
...
{\ct RADCAL\_FUEL}                  & Character & Section~\ref{info:RadCal}                 &                   & {\ct 'METHANE'}   \\ \hline
...
{\ct RADCAL\_FUEL\_ID}                    & Character   & Section~\ref{info:simple_chemistry}       &                   &       \\ \hline
...
% Not listed: EDCM, FIXED_MIX_TIME, FWD_ID, ODE_SOLVER, REVERSIBLE, SOOT_DEPOSITION, TAU_CHEM, TAU_FLAME, TOL_INT
...
% Undocumented:  BOUNDARY_CONDITION_FILENAME, FIRELINE_MLR_MAX, SHRINK,
