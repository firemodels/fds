...
NAMELIST /REAC/ A,AUTO_IGNITION_TEMPERATURE,BETA_EDC,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                E,EDCM,EPUMO2,EQUATION,EXTINCTION_MODEL,FIXED_MIX_TIME,FORMULA,FUEL,FUEL_RADCAL_ID, &
...
NAMELIST /PRES/ CHECK_POISSON,MAX_PRESSURE_ITERATIONS, &
                PRESSURE_ACCELERATION_FACTOR,PRESSURE_RELAX_TIME,RELAXATION_FACTOR, &
...
DO NM=1,NMESHES
   IF (.NOT.EVACUATION_ONLY(NM)) THEN      
      ...
      ENDDO
   ELSE
      IF (.NOT.EVAC_FDS6) N_EVAC_ZONE = N_EVAC_ZONE + 1
      IF (EVAC_FDS6 .AND. EVACUATION_GRID(NM)) N_EVAC_ZONE = N_EVAC_ZONE + 1
   END IF
ENDDO
...
READ_EVACUATION_ZONE_LOOP: DO N=N_ZONE-N_EVAC_ZONE+1,N_ZONE
   ...
   EVAC_MESH_NUMBER: DO NM=1,NMESHES
      IF ((EVAC_FDS6.AND.EVACUATION_GRID(NM)).OR.(.NOT.EVAC_FDS6.AND.EVACUATION_ONLY(NM))) THEN      
