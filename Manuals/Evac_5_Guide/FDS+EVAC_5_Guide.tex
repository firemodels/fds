...
(NIST).  This manual applies to the FDS+Evac version is 2.1.1, which
is embedded in the current FDS version 5.3.0.  The most up to date
version of this manual can be obtained from the FDS+Evac web pages at
...
the theorethical model behind the agent movement algorithm and details
...
visualized using the Smokeview~\cite{SV_UserGuide} programme. Also,
...
as straigthforward as to use the default stairscase model.  There are
...
can add the evacuation meshes to the calculation and de-activate the
...
levels of smoke consentration needed for a smoke sensing may be below
...
explicitely define the evacuation geometry to take this in to account.
...
now used to give the coloring information instead of the
...
\noindent The underlaying fire simulation programme, Fire Dynamics Simulator
...
space and time, but the building geometry is fitted to the underlaying
...
Nilsson~\cite{Frantzich03}, where larger smoke consentrations were used
...
\caption{Unimpeded walking velocities and body dimensions in FDS+Evac.
   ...
   velocities of the agents are personalized by using them from uniform
...
Fig.~\ref{Fig_HumanBody}.  The default body dimensions and the
...
dioxide does not have toxic effects at consentrations of up to 5
...
These tests are refered below as ``IMO 1'' \emph{etc}.  Note, that the
...
\item Unimpeded walking speed vs smoke density: Smoke reduces the
  ...
  smokeclear environment was set to 1.0~m/s.  The result of this test
  is shown in Fig.~\ref{Fig_SmokeSpeedTest}.  In the figure, the line
...
Fig.~\ref{Fig_Geoms}. One of the geometries was used to study the flow
...
namelist, which is a crude model for stairs.  In each figure, several
simulated curves are shown corresponding to different values of the
staircase speed reduction parameter.  In Figure~\ref{Fig_StairType1},
the best results are obtained when the value for the speed reduction
parameter is 0.5, \emph{i.e.}, the movement speeds of the agents are
reduced to $v = 0.5v_0$ at the stairs.  The results using more
sophisticated way of defining staircases (type 2) are compared to the
observed values in Figure~\ref{Fig_StairType2}.  In these simulations,
the stairs are modelled as inclines (\Timtt{EVSS} namelist), where
agents move at reduced speed.  Reducing the walking speed by a factor
~0.7 gives a good agreement with the observations.  Whether the
anisotropy parameter $\lambda_i$ is 0.3 or 0.5 does not have
effect of any practical importance.
%
%
...
standard agent type ``Adult''.  The premovement times were generated
...
\item Assembly space: The second test case is a large fictitious
  ...
  Fig.~\ref{Fig_AssemblySnapshots}a.  The FDS+Evac results (``Evac'')
...
but you should be sure that all fire mesh definitions are preceeding
...
\item Define the main evacuation meshes with the \Timtt{MESH} namelist
  ...
  co-ordinates for these meshes should be at a level, where the most
...
\item Define your doors, exits, stairs, and entries using the
  ...
  matching to the underlaying computational mesh.  See
...
\item If you are doing more than one evacuation calculation per one
  ...
  because the FDS+Evac is not deternimistic model.  After the runs,
...
%
  \item The evacuation meshes are two dimensional, \emph{i.e.}, they should
    ...
    where N${}_x$ and N${}_y$ are the number of celss in the $x$ and
...
%
  \item Check your \Timtt{EVAC} namelists that you are not putting
    ...
    \Timtt{EVHO} namelists to exclude the initialization of agents at
...
pre-defined agent type and set the detection and reaction time
...
\item[\Timts{DIA\_MEAN,DIA\_PARA,DIA\_PARA2,DIA\_LOW,DIA\_HIGH}] The
  parameters of the distribution used for the iameter of the agent
...
the pre-defined person types, unless you know what you are doing.  The
...
\item[\Timtt{XB}] Defines the rectangle, where the agents are put, $z$
  ...
  co-ordinates \Timtt{XB} intersects many evacuation meshes then the
...
\item[\Timts{ANGLE}] By default the orientation of agents is random,
  ...
  and the positive direction of the angle is anticlockwise.
%
...
\item[\Timts{COLOR}] Color of the agents seen in Smokeview if
  ...
  for the list of available color names.
%
\item[\Timts{RGB}] Three integers (0--255) specifying a color of the
...
\item[\Timtt{ID}] ID string of the door. One can refer to this door by
  its name.
%
\item[\Timtt{XB}] Defines the postion of the door, should be a line in
...
flows in stairs, then you should model the landings explicitely and
...
just change the $z$ co-ordinates of the agents, when these are saved
on the hard disk to be plotted by Smokeview.  Thus, the internal agent
movement algorithm of FDS+Evac does not use the $z$ co-ordinates.  All
...
\item[\Timts{HEIGHT}] The height of the ``\Timtt{-IOR}'' edge of the
  ...
  by the $z$ of \Timtt{XB}.  This edge has $z = z_{\scriptscriptstyle
    XB} +$ \Timts{HEIGHT}.  \Timts{HEIGTH} can have a positive or a
...
\item[\Timts{HEIGHT0}] The height of the ``\Timtt{IOR}'' edge of the
  ...
  by the $z$ of \Timts{XB}.  This edge has $z = z_{\scriptscriptstyle
    XB} +$ \Timts{HEIGHT0}.  \Timts{HEIGTH0} can have a positive or a
  negative value.  If \Timts{HEIGHT0}=\Timts{HEIGHT} the \Timtt{EVSS}
  defines a horisontal plane and \Timtt{IOR} has no effect.  Default
  is 0~m.
%
...
democraphics of the agents.  One do not need to recalculate the
...
The next colums are counters for the exit selection algorithm.  They
...
initialization phase of FDS+Evac run, if the evacuation geometry is
...
the line edings to the DOS style.
...
\item More intelligence in the exit door selection algorithm,
  ...
  heading/queuing for the same exit door, which is already implemented
...
%
\item Sreading of the information: agent to agent communication like
  fire detection.
%
\item Easier generation of the input file.
%
\item Counterflows are implemented but need still V\&V work.
%
\item More optimal computer memory usage.
%
\item Parallelization of the evacuation calculation using MPI/OpenMPI.
%
\end{itemize}
%
