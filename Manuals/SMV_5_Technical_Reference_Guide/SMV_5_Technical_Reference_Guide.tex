...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~1017-2, 68 pages (July 2009) \\
CODEN: NSPUE2 }
...
\LaTeX\ packages available, including MiKTeX. The FDS-Smokeview developers edit the manuals as part of the day to day upkeep of the model. Different editions of
...
\section{Moving Objects} The previous sections discussed how appearance
...
\subsection{Movement - OpenGL Model}
...
\subsection{Movement - Smokeview Implementation}
...
stoichimetric mixture fraction.
...
\noindent where $\Delta T_1$ and $\Delta T_2$ are the uncertainty in $T_1$ and $T_2$.
...
Smokeview is a software tool used to gain insight into the results generated
by fire modeling simulations such as the Fire Dynamics Simulator.
There are two general areas that need to be addressed in order to improve this tool.
First, to be able to improve the visualizations themselves - in particular, to do a better job of visualizing smoke and fire.
The aim then of any work to add or improve visualization algorithms in Smokeview is allow one to provide more and better insight.
Second, to be able to visualize the larger cases now made possible by the more powerful computers (faster with more memory) now commonly available.   Some possible areas of future research are described in more detail in the following sections.


\section{Visualize Cases more Realistically}
Realism is a metric used to gauge qualitatively how accurate the smoke and fire is being modeled. Realism itself is not the primary goal.  By applying more physics to the problem of visualizing smoke and fire the presumption is that the results will not only be more realistic but more accurate.
\begin{itemize}
\item Investigate techniques for visualizing fire more realistically by using information about the fire such as its temperature to color it more realistically.

\item Investigate algorithms for modeling the interaction of light and smoke.
The {\em transport} of light into and out of the smoke/fire ({\em ie scattering}) is another important factor that effects its appearance.  Light sources could consist of either man made lights or from the fire itself.  Work also needs to be done to generalize the wavelength of light used to visualize the scene, {\em ie} implementing an algorithm in Smokeview to simulate a thermal imager or infrared viewer.  
\end{itemize}
...
\begin{itemize}
\item Investigate methods for making it easier to probe or mine data, {\em ie}\ to retrieve data of particular interest.  Now FDS data is stored sequentially.  For large data sets one needs to be able to retrieve data at a particular time and region without reading the entire data set that comes before.  Put another way, with 64 bit comes much larger data files and longer read times.  We need to figure out how to load just part of the data efficiently to shorten time required to visualize cases.
...
\item Investigate methods for using the video card to perform scientific compoutations.
