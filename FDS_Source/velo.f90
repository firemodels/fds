...
REAL(EB) :: DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,SS,S12,S13,S23,DELTA,CS,YY_GET(1:N_SPECIES), &
            DAMPING_FACTOR=1._EB,NU,YPLUS,YPOAP
...
WALL_LOOP: DO IW=1,NWC
   ...
   IF (VAN_DRIEST .AND. .NOT.DYNSMAG) THEN
      ...
      YPLUS = (0.5_EB/RDN(IW))*U_TAU(IW)/NU ! should be positive (if not, there is a bug somewhere)
      YPOAP = MIN(1._EB,YPLUS/APLUS) ! in [0,1]
      DAMPING_FACTOR = (YPOAP-(0.5_EB*(YPOAP**2)))**2 ! series expansion for (1._EB-EXP(-YPLUS/APLUS))**2
   ENDIF
   
   SELECT CASE(BOUNDARY_TYPE(IW))
      CASE(SOLID_BOUNDARY)
         MU(IIG,JJG,KKG) = DAMPING_FACTOR*MU(IIG,JJG,KKG)
