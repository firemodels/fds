...
REAL(FB) :: TMPUP,TMPLOW,ZINT,DELTA,RHO2,BBF
      ...
      IF ( ((PY%PDPA_START<=T) .AND.(PY%PDPA_END>=T)) .OR. .NOT.PY%PDPA_INTEGRATE ) THEN
         ...
         DLOOP: DO I=1,NLP
            ...
            SELECT CASE(PY%QUANTITY)
               ...
               CASE('ENTHALPY')
                  CALL INTERPOLATE1D_UNIFORM(SPECIES(LPC%Y_INDEX)%C_P_L_BAR,LP%ONE_D%TMP_F,CPBAR)
                  VEL = CPBAR*LP%ONE_D%TMP_F
                  CALL INTERPOLATE1D_UNIFORM(SPECIES(LPC%Y_INDEX)%C_P_L_BAR,SPECIES(LPC%Y_INDEX)%TMP_MELT,CPBAR)
