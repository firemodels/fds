...
IF (CHECK_VN) THEN
   ...
   M%D_Z_MAX=0._EB
ENDIF
IF (CFL_VELOCITY_NORM>=4) THEN
   ALLOCATE(M%DVX(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','DVX',IZERO)
   ALLOCATE(M%DVY(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','DVY',IZERO)
   ALLOCATE(M%DVZ(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','DVZ',IZERO)
   M%DVX=0._EB
   M%DVY=0._EB
   M%DVZ=0._EB
ENDIF
