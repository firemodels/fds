...
MAIN_LOOP: DO
   ...
   IF (T>=FLSH_CLOCK(FLSH_COUNTER) .AND. FLUSH_FILE_BUFFERS) THEN
      IF (MY_RANK==0) CALL FLUSH_GLOBAL_BUFFERS
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         CALL FLUSH_LOCAL_BUFFERS(NM)
      ENDDO
      FLSH_COUNTER = FLSH_COUNTER + 1
   ENDIF
   ...
   IF ( (T>=RSRT_CLOCK(RSRT_COUNTER) .OR. STOP_STATUS==USER_STOP) .AND. (T>=T_END .OR. ALL(RADIATION_COMPLETED)) ) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         CALL DUMP_RESTART(T,DT,NM)
      ENDDO
      RSRT_COUNTER = RSRT_COUNTER + 1
   ENDIF
      ...
      IF (CODE==6) THEN
         IF (RNODE/=SNODE) THEN
            ...
            DO II=1,M%OMESH(NOM)%N_WALL_CELLS_SEND
               ...
               M3%REAL_SEND_PKG6(LL+1) = ONE_D%Q_RAD_IN
               M3%REAL_SEND_PKG6(LL+2) = M%TMP(BC%IIG,BC%JJG,BC%KKG)
               LL = LL+2
            ENDDO
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
            DO II=1,M2%N_EXPOSED_WALL_CELLS
               ...
               BC => M%BOUNDARY_COORD(WC%BC_INDEX)
               ONE_D => M%BOUNDARY_ONE_D(WC%OD_INDEX)
               M2%EXPOSED_WALL(II)%Q_RAD_IN = ONE_D%Q_RAD_IN
               M2%EXPOSED_WALL(II)%TMP_GAS  = M%TMP(BC%IIG,BC%JJG,BC%KKG)
            ENDDO
         ENDIF
      ENDIF
...
IF (T>=HRR_CLOCK(HRR_COUNTER) .AND. N_MPI_PROCESSES>1) CALL GATHER_Q_AND_M

! Gather species mass integrals to node 0

IF (T>=MASS_CLOCK(MASS_COUNTER) .AND. N_MPI_PROCESSES>1) THEN
...
IF (T>=HRR_CLOCK(HRR_COUNTER)) THEN
   IF (MY_RANK==0) CALL DUMP_HRR(T,DT)
   HRR_COUNTER = HRR_COUNTER + 1
   Q_DOT_SUM = 0._EB
   M_DOT_SUM = 0._EB
   T_LAST_DUMP_HRR = T
ENDIF
...
IF (N_FACE>0 .AND. T>=GEOM_CLOCK(GEOM_COUNTER)) THEN
   IF (MY_RANK==0) THEN
...
IF (T>=MASS_CLOCK(MASS_COUNTER)) THEN
...
IF (T>=DEVC_CLOCK(DEVC_COUNTER) .AND. N_DEVC>0) THEN
...
IF (T>=CTRL_CLOCK(CTRL_COUNTER) .AND. N_CTRL>0) THEN
...
IF (T>=CPU_CLOCK(CPU_COUNTER)) THEN
   CALL DUMP_TIMERS
