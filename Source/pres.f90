...
IF (TUNNEL_PRECONDITIONER) THEN
   DO I=0,IBP1
      HP(I,0:JBP1,0:KBP1) = HP(I,0:JBP1,0:KBP1) + H_BAR((NM-1)*IBAR+I)
   ENDDO
ENDIF
...
!> \brief Solve a 1-D Poisson equation for an entire tunnel
...
DO I=1,IBAR
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         TP_CC(II) = TP_CC(II) + PRHS(I,J,K)*DY(J)*DZ(K)  ! RHS of 1-D Poisson equation
      ENDDO
   ENDDO
   TP_CC(II) = TP_CC(II)/((YF-YS)*(ZF-ZS))
   TP_DD(II) = -2._EB*RDX(I)**2  ! Diagonal of tri-diagonal matrix
   TP_AA(II) =  1._EB*RDX(I)**2  ! Upper band
   TP_BB(II) =  1._EB*RDX(I)**2  ! Lower band
...
IF (NM==1) THEN
   IF (LBC==FISHPAK_BC_NEUMANN_NEUMANN .OR. LBC==FISHPAK_BC_NEUMANN_DIRICHLET) THEN
      TP_CC(1) = TP_CC(1) + BXS_BAR*TP_BB(1)/RDXN(0)
...
IF (NM==NMESHES) THEN
   IF (LBC==FISHPAK_BC_NEUMANN_NEUMANN .OR. LBC==FISHPAK_BC_DIRICHLET_NEUMANN) THEN
      TP_CC(TUNNEL_NXP) = TP_CC(TUNNEL_NXP) - BXF_BAR*TP_AA(TUNNEL_NXP)/RDXN(TUNNEL_NXP)
...
CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,TP_AA,COUNTS_TP,DISPLS_TP,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,TP_BB,COUNTS_TP,DISPLS_TP,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,TP_CC,COUNTS_TP,DISPLS_TP,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
CALL MPI_ALLGATHERV(MPI_IN_PLACE,0,MPI_DATATYPE_NULL,TP_DD,COUNTS_TP,DISPLS_TP,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
...
IF (NM==1) THEN
   IF (LBC==FISHPAK_BC_NEUMANN_NEUMANN .OR. LBC==FISHPAK_BC_NEUMANN_DIRICHLET) THEN
      H_BAR(0) =  H_BAR(1) - BXS_BAR/RDXN(0)
   ELSE
...
IF (NM==NMESHES) THEN
   IF (LBC==FISHPAK_BC_NEUMANN_NEUMANN .OR. LBC==FISHPAK_BC_DIRICHLET_NEUMANN) THEN
      H_BAR(TUNNEL_NXP+1) =  H_BAR(TUNNEL_NXP) + BXF_BAR/RDXN(TUNNEL_NXP)
   ELSE
