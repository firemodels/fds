...
INTEGER :: GEOM_TYPE, NXB, NLAT, NLONG, IJK(3), FACE_OFFSET, VERT_OFFSET
...
NAMELIST /GEOM/ AZIM, AZIM_DOT, COMPONENT_ONLY, DAZIM, DELEV, DSCALE, DXYZ0, DXYZ, &
                ELEV, ELEV_DOT, FACES, GAXIS, GROTATE, GROTATE_DOT,GEOM_IDS, ID, IJK, NLAT, NLONG,  &
                SPHERE_RADIUS, SCALE, SCALE_DOT, SPHERE_ORIGIN, SURF_ID, &
                TEXTURE_ORIGIN, TEXTURE_SCALE, TEXTURE_MAPPING, &
...
READ_GEOM_LOOP: DO N=1,N_GEOMETRY
   ...
   ! check for terrain (horizontal surface of the form z=f(x,y) )
...
INTEGER, INTENT(OUT), DIMENSION(3*(NLAT-1)*(NLONG-1)*2) :: SPHERE_FACES
REAL(EB) :: LAT, LONG
...
END DO
DO I = 1, NLONG
   LONG = -PI + 2.0_EB*PI*REAL(I-1,EB)/REAL(NLONG-1,EB)
   COSLONG(I) = COS(LONG)
   SINLONG(I) = SIN(LONG)
END DO
...
DO I = 1, NLAT
   DO J = 1, NLONG
      SPHERE_VERTS(IJ)   = COSLONG(J)*COSLAT(I)
      SPHERE_VERTS(IJ+1) = SINLONG(J)*COSLAT(I)
      SPHERE_VERTS(IJ+2) = SINLAT(I)
