...
INTEGER, ALLOCATABLE, DIMENSION(:) :: REQ,REQ1,REQ2,REQ3,REQ4,REQ5,REQ7,REQ6,REQ14,REQ15,&
                                      ...
                                      MPI_NEIGHBORHOOD,MPI_NEIGHBORHOOD_ROOT
...
DO WHILE (ANY(SETUP_PRESSURE_ZONES_INDEX==0))
   IF (N_MPI_PROCESSES>1) THEN
      DO NM=1,NMESHES
         IF (MPI_NEIGHBORHOOD(NM)==MPI_COMM_NULL) CYCLE
         M => MESHES(NM)
         CALL MPI_BCAST(M%PRESSURE_ZONE,SIZE(M%PRESSURE_ZONE),MPI_INTEGER,MPI_NEIGHBORHOOD_ROOT(NM),MPI_NEIGHBORHOOD(NM),IERR)
      ENDDO
   ENDIF
...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(3)
      ...
      ! MPI_NEIGHBORHOOD(NM) is the name of the communicator for mesh NM
      ! MPI_NEIGHBORHOOD_ROOT(NM) is the rank within MPI_NEIGHBORHOOD(NM) of the process that controls mesh NM

      ALLOCATE(MPI_NEIGHBORHOOD(NMESHES))
      ALLOCATE(MPI_NEIGHBORHOOD_ROOT(NMESHES))
      ...
      DO NM=1,NMESHES
         ...
         DO N=1,M%N_NEIGHBORING_MESHES
            ...
            ELSE
               NN = NN - 1
            ENDIF
            IF (NM==NOM) MPI_NEIGHBORHOOD_ROOT(NM) = NN
         ENDDO
         CALL MPI_GROUP_INCL(GROUP_WORLD,NN+1,MEMBERS(0:NN),SUBGROUP,IERR)  ! Create the new sub-group of GROUP_WORLD
         CALL MPI_COMM_CREATE(MPI_COMM_WORLD,SUBGROUP,MPI_NEIGHBORHOOD(NM),IERR) ! Create the new communicator
      ENDDO
...
!> \brief Broadcast basic geometry info from each mesh, NM, to its neighbors in MPI_NEIGHBORHOOD(NM)
...
DO NM=1,NMESHES
   IF (MPI_NEIGHBORHOOD(NM)==MPI_COMM_NULL) CYCLE
   M => MESHES(NM)
   CALL MPI_BCAST(M%CELL_INDEX,SIZE(M%CELL_INDEX),MPI_INTEGER,MPI_NEIGHBORHOOD_ROOT(NM),MPI_NEIGHBORHOOD(NM),IERR)
   CALL MPI_BCAST(M%SOLID,SIZE(M%SOLID),MPI_INTEGER,MPI_NEIGHBORHOOD_ROOT(NM),MPI_NEIGHBORHOOD(NM),IERR)
   CALL MPI_BCAST(M%WALL_INDEX,SIZE(M%WALL_INDEX),MPI_INTEGER,MPI_NEIGHBORHOOD_ROOT(NM),MPI_NEIGHBORHOOD(NM),IERR)
   CALL MPI_BCAST(M%OBST_INDEX_C,SIZE(M%OBST_INDEX_C),MPI_INTEGER,MPI_NEIGHBORHOOD_ROOT(NM),MPI_NEIGHBORHOOD(NM),IERR)
ENDDO
