...
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(MAX_DUCTS,MAX_DUCTS),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW, &
            TMP_WGT,TAU_VF,CP,YY_GET(1:N_SPECIES)
...
NAMELIST /HVAC/ TYPE_ID,AREA,DIAMETER,ROUND,SQUARE,LENGTH,DAMPER,REVERSE,LOSS,DUCT_ID,NODE_ID,FAN_ID, &!FILTER_ID,AIRCOIL_ID, &
                VENT_ID,DEVC_ID,CTRL_ID,RAMP_ID,TABLE_ID,ID,VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,XYZ,AMBIENT,TAU_VF
...
IF (LEAK_DUCTS > 0) THEN
   ...
   IF (N_SPECIES > 0) THEN
      ALLOCATE(LEAK_YY(0:N_ZONE,0:N_ZONE,1:N_SPECIES,1:NMESHES))
      YY_GET(1:N_SPECIES) =  SPECIES(1:N_SPECIES)%YY0
      DO NS=1,N_SPECIES
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   ...
   IF (DN%LEAKAGE) THEN
      ...
      IF (N_SPECIES > 0) THEN
         ...
         CALL GET_AVERAGE_SPECIFIC_HEAT(DN%YY_V,DN%CP_V,TMPA)      
      ELSE
...
IF (N_SPECIES > 0) ALLOCATE(NODE_YY(1:N_DUCTNODES,1:N_SPECIES,1:NMESHES))
...
DO NN=1,N_DUCTNODES
   ...
   IF (N_SPECIES > 0) THEN   
      DO N=1,N_SPECIES
         NODE_YY(NN,N,1:NMESHES) = SPECIES(N)%YY0
      ENDDO
   ENDIF
ENDDO
...
REAL(EB) :: TNOW,MTOT,ETOT,VTOT,YYTOT(1:N_SPECIES),TGUESS,VFLOW,YY_N(1:N_SPECIES),TMP_WGT
INTEGER :: ITMP
...
!IF (N_SPECIES > 0) WRITE(*,*) (TRIM(DUCTNODE(NN)%ID),DUCTNODE(NN)%YY(:),NN=1,N_DUCTNODES)
...
END SELECT
DU%VEL(NEW) = DU%VEL(NEW)*EVALUATE_RAMP(T,DU%TAU,-1)
...
REAL(EB) :: YY_SUM(N_SPECIES),YY_N(1:N_SPECIES),RHO_SUM,P_SUM,AREA_SUM,AREA,TMP_SUM,H_SUM,H_G,TMP_NEW,CP,TNOW,&
           P_AVE,TMP_WGT!,VEL2
...
!            VEL2  = 0.25_EB*( (UP(II,JJ,KK)+UP(II-1,JJ,KK))**2+(VP(II,JJ,KK)+VP(II,JJ-1,KK))**2+(WP(II,JJ,KK)+WP(II,JJ,KK-1))**2)
   ...
   IF (N_SPECIES>0) NODE_YY(NN,1:N_SPECIES,NM) = YY_SUM(1:N_SPECIES) / RHO_SUM
   H_SUM = H_SUM / RHO_SUM
   DO
      IF (N_SPECIES > 0) THEN
         YY_N = DN%YY_V(:)
   ...
   IF (N_SPECIES>0) THEN
      YY_N = NODE_YY(NN,1:N_SPECIES,NM)
...
LEAKAGE_BC: IF (LEAK_DUCTS > 0) THEN
   ...
   IF (N_SPECIES > 0) LEAK_YY(:,:,:,NM) = 0._EB
   WALL_LOOP: DO IW=1,NWC
      ...
      IF (N_SPECIES > 0) THEN
         YY_N = YY(II,JJ,KK,:)
         LEAK_YY(IZ1,IZ2,1:N_SPECIES,NM) = LEAK_YY(IZ1,IZ2,1:N_SPECIES,NM)+YY_N*RHO(II,JJ,KK)*AREA
   ...
   DO IZ1 = 0,N_ZONE
      DO IZ2 = 0,N_ZONE
         ...
         IF (N_SPECIES>0) THEN
            LEAK_YY(IZ1,IZ2,1:N_SPECIES,NM) = LEAK_YY(IZ1,IZ2,1:N_SPECIES,NM) / LEAK_RHO(IZ1,IZ2,NM)
            YY_N = LEAK_YY(IZ1,IZ2,1:N_SPECIES,NM)
...
REAL(EB) :: AREA_SUM,AREA,CP,H_G,TMP_WGT,TMP_SUM,TMP_NEW,YY_N(1:N_SPECIES)
TYPE(DUCTNODE_TYPE), POINTER :: DN=>NULL()
...
DO NN=1,N_DUCTNODES
   ...
   IF (DN%VENT_INDEX > 0 .AND. .NOT. DN%LEAKAGE) THEN
      ...
      IF (N_SPECIES > 0) DN%YY_V(1:N_SPECIES) = NODE_YY(NN,1:N_SPECIES,NM)
   ENDIF
   LEAKAGE_IF: IF (DN%LEAKAGE) THEN
      ...
      DO NM=1,NMESHES
         ...
         IF (N_SPECIES > 0) THEN
            YY_N(1:N_SPECIES) = LEAK_YY(IZ1,IZ2,1:N_SPECIES,NM)
            DN%YY_V(1:N_SPECIES) = DN%YY_V(1:N_SPECIES) + YY_N(1:N_SPECIES) * LEAK_RHO(IZ1,IZ2,NM) * AREA
