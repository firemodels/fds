...
PRIVATE
INTEGER IZERO
CHARACTER(255), PARAMETER :: initid='$Id: init.f90,v 1.42 2007/04/30 19:04:39 mcgratta Exp $'
CHARACTER(100) MESSAGE
PUBLIC INITIALIZE_MESH_VARIABLES,INITIALIZE_GLOBAL_VARIABLES, OPEN_AND_CLOSE
...
IF (STRATIFICATION) THEN
   DO K=0,M%KBP1
      ...
      IF (LAPSE_RATE /= 0._EB) THEN
         M%P_0(K) = P_INF*(M%TMP_0(K)/TMPA)**(GVEC(3)/SPECIES(0)%RCON/LAPSE_RATE)
      ELSE
         M%P_0(K) = P_INF*EXP(GVEC(3)*M%ZC(K)/(SPECIES(0)%RCON*TMPA))
      ENDIF
   ENDDO
ELSE
   M%TMP_0(:) = TMPA
...
IF (TWO_D) THEN
   M%Q_UPPER = HRRPUA_SHEET*1000._EB/(M%DXMIN*M%DZMIN)**0.5_EB
ELSE
   M%Q_UPPER = HRRPUA_SHEET*1000._EB/(M%DXMIN*M%DYMIN*M%DZMIN)**ONTH
ENDIF
      ...
      DO J=0,JBP1
         DO I=0,IBP1
            IF (M%XC(I) > IN%X1 .AND. M%XC(I) < IN%X2 .AND. &
               ...
               M%YY(I,J,K,1:N_SPECIES) = IN%MASS_FRACTION(1:N_SPECIES)
...
CALL ChkMemErr('INIT','QRAD',IZERO) 
M%QRAD = 0._EB
...
M%MASS_LOSS = 0._EB
ALLOCATE(M%WALL_INDEX(0:M%NDBC,-3:3),STAT=IZERO)
...
INTEGER  :: NM,NOM,ICO,IBC,IOR
REAL(EB) :: PX,PY,PZ,X1,X2,Y1,Y2,Z1,Z2,T_ACTIVATE,XI,YJ,ZK, XIN,YIN,ZIN,DIST
INTEGER N,NN,I_OBST,I,J,K,IBCX,IIG,JJG,KKG,IW,II,JJ,KK,IC,ICG,IL
LOGICAL :: VENT_FOUND
...
CHECK_MESHES: IF (IW<=M%NEWC .AND.  .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   FIND_MESH: DO NOM=1,NMESHES
      IF (NOM==NM) CYCLE FIND_MESH
      IF (EVACUATION_ONLY(NOM)) CYCLE FIND_MESH
      MM=>MESHES(NOM)
      IF (XIN<MM%XS) CYCLE FIND_MESH
      IF (XIN>MM%XF) CYCLE FIND_MESH
      IF (YIN<MM%YS) CYCLE FIND_MESH
      IF (YIN>MM%YF) CYCLE FIND_MESH
      IF (ZIN<MM%ZS) CYCLE FIND_MESH
      IF (ZIN>MM%ZF) CYCLE FIND_MESH
      SELECT CASE(IOR)
         CASE( 1)  
            XIN = XIN - 0.05_EB*M%DX(0)
         CASE(-1) 
            XIN = XIN + 0.05_EB*M%DX(M%IBP1)
         CASE( 2) 
            YIN = YIN - 0.05_EB*M%DY(0)
         CASE(-2)
            YIN = YIN + 0.05_EB*M%DY(M%JBP1)
         CASE( 3)
            ZIN = ZIN - 0.05_EB*M%DZ(0)
         CASE(-3) 
            ZIN = ZIN + 0.05_EB*M%DZ(M%KBP1)
      END SELECT
      IF (XIN<MM%XS) CYCLE FIND_MESH
      IF (XIN>MM%XF) CYCLE FIND_MESH
      IF (YIN<MM%YS) CYCLE FIND_MESH
      ...
      IF (ZIN<MM%ZS) CYCLE FIND_MESH
      IF (ZIN>MM%ZF) CYCLE FIND_MESH
      XI = MM%CELLSI(NINT((XIN-MM%XS)*MM%RDXINT)) + 1._EB
      YJ = MM%CELLSJ(NINT((YIN-MM%YS)*MM%RDYINT)) + 1._EB
      ZK = MM%CELLSK(NINT((ZIN-MM%ZS)*MM%RDZINT)) + 1._EB
      II = FLOOR(XI)
      JJ = FLOOR(YJ)
      KK = FLOOR(ZK)
...
VLOOP: DO N=1,M%N_VENT
   ...
   VT => M%VENTS(N)
   IF (I_OBST>0 .AND. VT%BOUNDARY_TYPE==OPEN_BOUNDARY) CYCLE VLOOP
   IF (.NOT.OBSTRUCTION(I_OBST)%ALLOW_VENT) CYCLE VLOOP
   ...
   IF (VT%BOUNDARY_TYPE==OPEN_BOUNDARY)   M%BOUNDARY_TYPE(IW) = OPEN_BOUNDARY
   ...
   CASE (1) COMPONENT
      ...
      SELECT CASE(ABS(IOR))
         CASE( 2)
            ...
            IJKE(12,IE) = IIO
            IJKE(13,IE) = JJO
            IF (KK<KBAR) IJKE(14,IE) = KKO-1
            IF (KK==KBAR) IJKE(14,IE) = KKO
         CASE( 3)
            IF (IOR>0) IJKE( 7,IE) = -NOM
            IF (IOR<0) IJKE( 7,IE) =  NOM
            IJKE( 8,IE) = IIO
            IF (JJ<JBAR) IJKE( 9,IE) = JJO-1
            IF (JJ==JBAR) IJKE( 9,IE) = JJO
            IJKE(10,IE) = KKO
      END SELECT
   ...
   CASE (2) COMPONENT
      ...
      SELECT CASE(ABS(IOR))
         ...
         CASE( 3)
            IF (IOR>0) IJKE( 7,IE) = -NOM
            IF (IOR<0) IJKE( 7,IE) =  NOM
            IF (II<IBAR) IJKE( 8,IE) = IIO-1
            IF (II==IBAR) IJKE( 8,IE) = IIO
            IJKE( 9,IE) = JJO
            IJKE(10,IE) = KKO
      END SELECT 
   ...
   CASE (3) COMPONENT
      ...
      SELECT CASE(ABS(IOR))
         CASE( 1)
            ...
            IF (IOR<0) IJKE(11,IE) =  NOM
            IJKE(12,IE) = IIO
            IF (JJ<JBAR) IJKE(13,IE) = JJO-1
            IF (JJ==JBAR) IJKE(13,IE) = JJO
            IJKE(14,IE) = KKO
         CASE( 2)
            IF (IOR>0) IJKE( 7,IE) = -NOM
            IF (IOR<0) IJKE( 7,IE) =  NOM
