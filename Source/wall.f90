...
TMP_LOOP: DO I=1,N_TGA
   ...
   IF (TGA_WALL_INDEX>0) THEN
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T_TGA,B1%TMP_G-B1%TMP_F,SF,WALL_INDEX_IN=IW)
      CALL SOLID_HEAT_TRANSFER(NM,T_TGA,TGA_DT,WALL_INDEX=IW)
   ELSE
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T_TGA,B1%TMP_G-B1%TMP_F,SF,PARTICLE_INDEX_IN=IP)
   ...
   IF (I==1 .OR. B1%TMP_G >= TMP_DUMP) THEN
      ...
      WRITE(LU_TGA,TCFORM) REAL(T_TGA,FB), REAL(B1%TMP_G-TMPM,FB), &
                           ...
                           REAL(DSC_CONVERSION_FACTOR*B1%Q_CON_F*0.001_EB/SURF_DEN_0,FB), &
                           REAL(DSC_CONVERSION_FACTOR*B1%Q_CON_F*0.001_EB/SURF_DEN(0),FB)
   ENDIF
ENDDO TMP_LOOP
