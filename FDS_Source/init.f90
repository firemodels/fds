...
ALLOCATE(M%EXTERNAL_WALL(M%N_EXTERNAL_WALL_CELLS))
...
IF (.NOT.EVACUATION_ONLY(NM)) THEN
   ...
   ALLOCATE(M%Q_H_CORR(0:IBP1,0:JBP1,0:KBP1),STAT=IZERO)
   CALL ChkMemErr('INIT','Q_H_CORR',IZERO) ; M%Q_H_CORR = 0._EB
ENDIF
...
CHECK_MESHES: IF (IW<=M%N_EXTERNAL_WALL_CELLS .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   FOUND_OTHER_MESH: IF (NOM>0) THEN 
      ...
      IF ( SECOND_ORDER_INTERPOLATED_BOUNDARY .AND. &
           ABS(REAL((IIO_MAX-IIO_MIN)*(JJO_MAX-JJO_MIN)*(KKO_MAX-KKO_MIN),EB))>TWO_EPSILON_EB ) THEN
