...
LOGICAL :: ALPERT_PLUS_HGL_MQH=.FALSE.,ALPERT_PLUS_HGL_FPA=.FALSE., &
           ALPERT_PLUS_HGL_DB=.FALSE.,ALPERT_PLUS_HGL_BEYLER=.FALSE., &
...
NAMELIST /ALPERT/ Q,LOCATION_FACTOR,T_END,R_VALUES,H_VALUES,LABEL,TMP_A,OUTPUT_FILE,T_SQUARED,ALPHA, &
                  ALPERT_PLUS_HGL_MQH,ALPERT_PLUS_HGL_FPA,ALPERT_PLUS_HGL_DB,ALPERT_PLUS_HGL_BEYLER, &
...
INTEGER :: I,J,K,N_PTS
REAL :: HRR,K1,K2,M,Z,RHO_G,SIGMA
...
DO I=0,50
   ...
   ! Modified forms of the method of Alpert that use various HGL temperature correlations for TMP_A
   IF (ALPERT_PLUS_HGL_MQH) THEN
      SIGMA = 5.67e-11
      ...
      TMP_A = TMP_G
   ELSEIF (ALPERT_PLUS_HGL_FPA) THEN
      T_P = (RHO_S*C_S/K_S) * (DELTA/2.)**2
      A_T = 2.*L*W + 2.*L*H + 2.*W*H
      ...
      ! Set ambient temperature to HGL temperature
      TMP_A = TMP_G
   ELSEIF (ALPERT_PLUS_HGL_DB) THEN
      A_T = 2.*L*W + 2.*L*H + 2.*W*H
      V = L*W*H
      ...
      ! Set ambient temperature to HGL temperature
      TMP_A = TMP_G
   ELSEIF (ALPERT_PLUS_HGL_BEYLER) THEN
      A_T = 2.*L*W + 2.*L*H + 2.*W*H
      RHO_A = 353./(TMP_AMBIENT)
      M = L*W*H*RHO_A
      ...
      TMP_A = TMP_G
   ELSE
      ! Unmodified form of the method of Alpert
      TMP_A = TMP_AMBIENT
   ENDIF
