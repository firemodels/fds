...
REAL(EB) :: TMP_G,DELTA_TMP,U2,V2,W2,VELCON,H_NATURAL,H_FORCED,H_DNS,CP,YY_GET(1:N_SPECIES),DAMPING_FACTOR
...
ELSE DNS_IF
   H_EDDY_IF: IF (H_EDDY) THEN
      ...
      ELSE
         DAMPING_FACTOR = 0.0625_EB ! = (0.05/0.2)**2
      ENDIF
      HEAT_TRANSFER_COEFFICIENT = DAMPING_FACTOR*MU(IIG,JJG,KKG)*CP*2._EB*RDN(IW)
   ELSE H_EDDY_IF
      IF (PREDICTOR) THEN
         UU => U
         VV => V
