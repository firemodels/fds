...
are meaningful to certain computer operating systems.  If this is specified as:
\begin{verbatim}
CHID='%INPUTFILENAME%'
\end{verbatim}
then {\ct CHID} will be set to the name of the input file minus any suffix such as {\ct .fds}.
...
\end{verbatim} \normalsize
Note that an additional scalar {\tt QUANTITY2} is required for MSR and WEM.  {\tt QUANTITY2} may be any output quantity appropriate for {\tt SLCF}. Also, {\tt CELL\_CENTERED} is optional for any of the three metrics.
...
In versions of FDS prior to 5.5, the second term was included only as an option.
Starting with FDS~5.5, however, the baroclinic torque is included by default. It can be excluded by setting
{\ct BAROCLINIC=.FALSE.} on the {\ct MISC} line, but this is only recommended for diagnostic purposes. For example, in the simple helium plume test case below,
...
species called 'GAS'. In the third example (box\_burn\_away3), the
