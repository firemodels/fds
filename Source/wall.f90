...
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: P1,P1_BACK
TYPE(BOUNDARY_PROP2_TYPE), POINTER :: P2,P2_BACK
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   P1%TMP_G =  TMP(BC%IIG,BC%JJG,BC%KKG)
...
CFACE_LOOP: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   CALL CFACE_THERMAL_GASVARS(ICF,P1)
   P1%TMP_G = CFA%TMP_G
   RHO_G = CFA%RHO_G
   ZZ_G  = CFA%ZZ_G
   RSUM_G  = CFA%RSUM_G
...
IF (SOLID_PARTICLES) THEN
   DO IP = 1, NLP
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN  ! Target particles are included to get gas phase values
         ...
         P1%U_TANG = SQRT(UBAR**2+VBAR**2+WBAR**2)
         ...
         IF (LPC%MASSLESS_TARGET) THEN
            ...
            IF (PY%HEAT_TRANSFER_COEFFICIENT>0._EB) THEN
               P1%HEAT_TRANS_COEF = PY%HEAT_TRANSFER_COEFFICIENT
            ELSE
               P1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(P1%TMP_G-PY%GAUGE_TEMPERATURE,SF%H_FIXED,SURF_INDEX,&
                                       PARTICLE_INDEX_IN=IP)
            ENDIF
            P1%Q_CON_F = P1%HEAT_TRANS_COEF*(P1%TMP_G-PY%GAUGE_TEMPERATURE)
         ENDIF
      ENDIF
   ENDDO
ENDIF
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (NO_CONVECTION) METHOD_OF_HEAT_TRANSFER
      ...
      P1%TMP_F  = P1%TMP_G
      P1%Q_RAD_OUT = SIGMA*P1%EMISSIVITY*P1%TMP_F**4
      ...
      IF (UN>TWO_EPSILON_EB) THEN  ! Assume the flow is coming into the domain
         ...
         IF (WC%VENT_INDEX>0) THEN
            ...
            IF (VT%TMP_EXTERIOR>0._EB) THEN
               TSI = T - T_BEGIN
               P1%TMP_F = TMP_0(KK) + EVALUATE_RAMP(TSI,VT%TMP_EXTERIOR_RAMP_INDEX)*(VT%TMP_EXTERIOR-TMP_0(KK))
            ENDIF
         ENDIF
         P1%ZZ_F(1:N_TRACKED_SPECIES)=SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0
      ELSE
         P1%TMP_F = P1%TMP_G
         P1%ZZ_F(1:N_TRACKED_SPECIES) = ZZ_G(1:N_TRACKED_SPECIES)
      ENDIF

      ! Ghost cell values

      TMP(II,JJ,KK) = P1%TMP_F
      ZZP(II,JJ,KK,1:N_TRACKED_SPECIES) = P1%ZZ_F(1:N_TRACKED_SPECIES)
      ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,ZZP(II,JJ,KK,1:N_TRACKED_SPECIES))
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM(II,JJ,KK))
      RHOP(II,JJ,KK) = PBAR_P(KK,P1%PRESSURE_ZONE)/(RSUM(II,JJ,KK)*TMP(II,JJ,KK))

      P1%Q_CON_F = 2._EB*P1%K_G*(P1%TMP_G-P1%TMP_F)*P1%RDN
      P1%Q_RAD_OUT = SIGMA*P1%EMISSIVITY*P1%TMP_F**4

   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER

      IF (ABS(P1%T_IGN-T_BEGIN) <= SPACING(P1%T_IGN) .AND. SF%RAMP(TIME_TEMP)%INDEX>=1) THEN
         TSI = T
      ELSE
         TSI = T - P1%T_IGN
      ENDIF

      IF (P1%U_NORMAL>TWO_EPSILON_EB) THEN
         P1%TMP_F = P1%TMP_G
      ELSEIF (SF%TMP_FRONT>0._EB) THEN
         P1%TMP_F = TMP_0(KKG) + EVALUATE_RAMP(TSI,SF%RAMP(TIME_TEMP)%INDEX,TAU=SF%RAMP(TIME_TEMP)%TAU)*(SF%TMP_FRONT-TMP_0(KKG))
      ELSE
         P1%TMP_F = TMP_0(KKG)
      ENDIF

      DTMP = P1%TMP_G - P1%TMP_F
      IF (PRESENT(WALL_INDEX)) THEN
         P1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,WALL_INDEX_IN=WALL_INDEX)
      ELSEIF (PRESENT(CFACE_INDEX)) THEN
         P1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,CFACE_INDEX_IN=CFACE_INDEX)
      ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
         P1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SURF_INDEX,PARTICLE_INDEX_IN=PARTICLE_INDEX)
      ENDIF
      P1%Q_CON_F = P1%HEAT_TRANS_COEF*DTMP
      P1%Q_RAD_OUT = SIGMA*P1%EMISSIVITY*P1%TMP_F**4

   CASE (CONVECTIVE_FLUX_BC,NET_FLUX_BC) METHOD_OF_HEAT_TRANSFER

      IF (ABS(P1%T_IGN-T_BEGIN)<= SPACING(P1%T_IGN) .AND. SF%RAMP(TIME_HEAT)%INDEX>=1) THEN
         TSI = T
      ELSE
         TSI = T - P1%T_IGN
      ENDIF
      ...
      IF (SF%SET_H) THEN
         P1%Q_CON_F = -RAMP_FACTOR*SF%CONVECTIVE_HEAT_FLUX*P1%AREA_ADJUST
         P1%HEAT_TRANS_COEF = P1%Q_CON_F/(P1%TMP_G-TMP_OTHER+TWO_EPSILON_EB)
      ELSE
         IF (SF%THERMAL_BC_INDEX==NET_FLUX_BC) THEN
            QNET = -RAMP_FACTOR*SF%NET_HEAT_FLUX*P1%AREA_ADJUST
         ELSE
            QNET = -RAMP_FACTOR*SF%CONVECTIVE_HEAT_FLUX*P1%AREA_ADJUST
         ENDIF
         ADCOUNT = 0
         ADLOOP: DO
            ADCOUNT = ADCOUNT + 1
            DTMP = P1%TMP_G - TMP_OTHER
            ...
            IF (RADIATION .AND. SF%THERMAL_BC_INDEX/=CONVECTIVE_FLUX_BC) THEN
               QEXTRA =  P1%HEAT_TRANS_COEF*DTMP + P1%Q_RAD_IN - P1%EMISSIVITY * SIGMA * TMP_OTHER ** 4 - QNET
               FDERIV = -P1%HEAT_TRANS_COEF -  4._EB * P1%EMISSIVITY * SIGMA * TMP_OTHER ** 3
            ELSE
               QEXTRA = P1%HEAT_TRANS_COEF*DTMP - QNET
               FDERIV = -P1%HEAT_TRANS_COEF
            ENDIF
            IF (ABS(FDERIV) > TWO_EPSILON_EB) TMP_OTHER = TMP_OTHER - QEXTRA / FDERIV
            IF (ABS(QEXTRA) < 1.E-10_EB .OR. ADCOUNT > 20) THEN
               P1%TMP_F = MAX(TMPMIN,MIN(TMPMAX,TMP_OTHER))
               EXIT ADLOOP
            ELSE
               P1%TMP_F = MAX(TMPMIN,MIN(TMPMAX,TMP_OTHER))
               CYCLE ADLOOP
   ...
   CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER  ! Only for EXTERNAL_WALL_CELLs
      ...
      IF (ATMOSPHERIC_INTERPOLATION) THEN
         ...
         PBAR_G = PBAR_P(KKG,P1%PRESSURE_ZONE)
         ...
         RHOP(II,JJ,KK) = PBAR_P(KK,P1%PRESSURE_ZONE)/RSUM(II,JJ,KK)/DENOM
      ELSE
      ...
      SINGLE_SPEC_IF: IF (N_TOTAL_SCALARS > 1) THEN
         SPECIES_LOOP: DO N=1,N_TOTAL_SCALARS
            ...
            SELECT CASE(IOR)
               CASE( 1)
                  ...
                  PBAR_F = PBAR_P(KKG,P1%PRESSURE_ZONE)
               CASE(-1)
                  IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
                     RHO_ZZ_G_2 = RHOP(IIG-1,JJG,KKG)*ZZP(IIG-1,JJG,KKG,N)
                     RHO_ZZ_OTHER_2 = OM_RHOP(IIO+1,JJO,KKO)*OM_ZZP(IIO+1,JJO,KKO,N)
               ...
               CASE( 3)
                  ...
                  CALL GET_SCALAR_FACE_VALUE(U_TEMP,Z_TEMP,F_TEMP,1,1,1,1,1,1,3,I_FLUX_LIMITER)
                  RHO_ZZ_F(N) = F_TEMP(1,1,1)
                  PBAR_F = (PBAR_P(KK,P1%PRESSURE_ZONE)*DZ(KKG) + PBAR_P(KKG,P1%PRESSURE_ZONE)*DZ(KK)) / (DZ(KK)+DZ(KKG))
               CASE(-3)
                  IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
                     RHO_ZZ_G_2 = RHOP(IIG,JJG,KKG-1)*ZZP(IIG,JJG,KKG-1,N)
                     RHO_ZZ_OTHER_2 = OM_RHOP(IIO,JJO,KKO+1)*OM_ZZP(IIO,JJO,KKO+1,N)
         ...
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM(II,JJ,KK))
         TMP(II,JJ,KK) = PBAR_P(KK,P1%PRESSURE_ZONE)/(RSUM(II,JJ,KK)*RHOP(II,JJ,KK))

         ! face value of temperature
         IF (ATMOSPHERIC_INTERPOLATION) THEN
            P1%TMP_F = (TMP(II,JJ,KK)*DZ(KKG) + TMP(IIG,JJG,KKG)*DZ(KK)) / (DZ(KK)+DZ(KKG))
            P1%ZZ_F(1:N_TOTAL_SCALARS) = (ZZP(II,JJ,KK,1:N_TOTAL_SCALARS)*DZ(KKG) + ZZP(IIG,JJG,KKG,1:N_TOTAL_SCALARS)*DZ(KK)) &
                                          / (DZ(KK)+DZ(KKG))
            ZZ_GET(1:N_TRACKED_SPECIES) = P1%ZZ_F(1:N_TRACKED_SPECIES)
            CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
            P1%RHO_F = PBAR_F/(RSUM_F*P1%TMP_F)
         ELSE
            P1%ZZ_F(1:N_TOTAL_SCALARS) = MAX(0._EB,MIN(1._EB,RHO_ZZ_F(1:N_TOTAL_SCALARS)/P1%RHO_F))
            ZZ_GET(1:N_TRACKED_SPECIES) = P1%ZZ_F(1:N_TRACKED_SPECIES)
            CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
            P1%TMP_F = PBAR_F/(RSUM_F*P1%RHO_F)
         ENDIF
            ...
            SPECIES_LOOP_2: DO N=1,N_TOTAL_SCALARS
               SELECT CASE(SIM_MODE)
                  CASE(DNS_MODE,LES_MODE)
                     D_Z_N = D_Z(:,N)
                     CALL INTERPOLATE1D_UNIFORM(LBOUND(D_Z_N,1),D_Z_N,P1%TMP_G,D_Z_G)
                     IF (SIM_MODE==LES_MODE) CALL GET_VISCOSITY(ZZ_GET,MU_DNS_G,P1%TMP_G)
               END SELECT
               ...
               P1%RHO_D_DZDN_F(N) =  RHO_D_DZDN_OTHER
            ENDDO SPECIES_LOOP_2
            IF (SIM_MODE==DNS_MODE .OR. SIM_MODE==LES_MODE) THEN
               N=MAXLOC(P1%ZZ_F(1:N_TRACKED_SPECIES),1)
               P1%RHO_D_DZDN_F(N) = -(SUM(P1%RHO_D_DZDN_F(1:N_TRACKED_SPECIES))-P1%RHO_D_DZDN_F(N))
            ENDIF
         ENDIF COARSE_MESH_IF

      ELSE SINGLE_SPEC_IF
         P1%ZZ_F(1) = 1._EB
         TMP(II,JJ,KK) = PBAR_P(KK,P1%PRESSURE_ZONE)/(RSUM0*RHOP(II,JJ,KK))
         SELECT CASE(IOR)
            CASE DEFAULT
               PBAR_F = PBAR_P(KKG,P1%PRESSURE_ZONE)
            CASE (-3,3)
         ...
         IF (ATMOSPHERIC_INTERPOLATION) THEN
            P1%TMP_F = (TMP(II,JJ,KK)*DZ(KKG) + TMP(IIG,JJG,KKG)*DZ(KK)) / (DZ(KK)+DZ(KKG))
            P1%RHO_F = PBAR_F/(RSUM0*P1%TMP_F)
         ELSE
            P1%TMP_F = PBAR_F/(RSUM0*P1%RHO_F)
         ENDIF
      ENDIF SINGLE_SPEC_IF

      P1%Q_CON_F = 0._EB ! no convective heat transfer at interpolated boundary
      P1%Q_RAD_OUT = SIGMA*P1%EMISSIVITY*P1%TMP_F**4
...
SELECT CASE(SIM_MODE)
   CASE DEFAULT
      DO N=1,N_TRACKED_SPECIES
         P1%RHO_D_F(N) = MU_G*RSC*P1%RHO_F/RHO_G
      ENDDO
   CASE (LES_MODE)
      ITMP = MIN(I_MAX_TEMP-1,NINT(P1%TMP_F))
      IIG = BC%IIG
      JJG = BC%JJG
      KKG = BC%KKG
      DO N=1,N_TRACKED_SPECIES
         P1%RHO_D_F(N) = P1%RHO_F*( D_Z(ITMP,N) + (MU_G-MU_DNS(IIG,JJG,KKG))/RHO_G*RSC )
      ENDDO
   CASE (DNS_MODE)
      ITMP = MIN(I_MAX_TEMP-1,NINT(P1%TMP_F))
      DO N=1,N_TRACKED_SPECIES
         P1%RHO_D_F(N) = P1%RHO_F*D_Z(ITMP,N)
      ENDDO
END SELECT
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   IF (CALL_HT_1D) THEN
      P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES) = P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES)*AREA_SCALING
      P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES) = P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES)*AREA_SCALING
      P1%Q_DOT_G_PP                             = P1%Q_DOT_G_PP                            *AREA_SCALING
      P1%Q_DOT_O2_PP                            = P1%Q_DOT_O2_PP                           *AREA_SCALING
   ...
   DO NS=1,N_TRACKED_SPECIES
      IF (ABS(P1%M_DOT_G_PP_ADJUST(NS))<=TWO_EPSILON_EB) CYCLE
      MW_RATIO = SPECIES_MIXTURE(NS)%RCON/RSUM(BC%IIG,BC%JJG,BC%KKG)
      M_DOT_SINGLE = LP%PWT*P1%M_DOT_G_PP_ADJUST(NS)*P1%AREA
      ...
      ZZ_GET = 0._EB
      ZZ_GET(NS) = 1._EB
      CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S_B,P1%TMP_F)
      Q_DOT(3,NM) = Q_DOT(3,NM) + P1%M_DOT_G_PP_ADJUST(NS)*P1%AREA*H_S_B*LP%PWT    ! Q_CONV
   ENDDO
   ...
   CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,P1%TMP_G)
   H_G = CP*P1%TMP_G*M_GAS

   D_SOURCE(IIG,JJG,KKG) = D_SOURCE(IIG,JJG,KKG) - P1%Q_DOT_G_PP*P1%AREA/H_G * LP%PWT

   ! Calculate contribution to divergence term due to convective heat transfer from particle

   D_SOURCE(IIG,JJG,KKG) = D_SOURCE(IIG,JJG,KKG) - P1%Q_CON_F*P1%AREA/H_G * LP%PWT

   ! Add energy losses and gains to overall energy budget array

   Q_DOT(7,NM) = Q_DOT(7,NM) - (P1%Q_CON_F + P1%Q_RAD_IN - P1%Q_RAD_OUT)*P1%AREA*LP%PWT      ! Q_PART
   Q_DOT(2,NM) = Q_DOT(2,NM) + (P1%Q_RAD_IN-P1%Q_RAD_OUT)*P1%AREA*LP%PWT                        ! Q_RADI

   ! Calculate the mass flux of fuel gas from particles

   IF (CORRECTOR) THEN
      DO NS=1,N_TRACKED_SPECIES
         M_DOT(NS,NM) = M_DOT(NS,NM) + P1%M_DOT_G_PP_ACTUAL(NS)*P1%AREA*LP%PWT
      ENDDO
   ENDIF
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. .NOT.CELL(IC)%SOLID .AND. .NOT.CELL(ICG)%SOLID) &
       ZZP(II,JJ,KK,1:N_TRACKED_SPECIES) = 2._EB*P1%ZZ_F(1:N_TRACKED_SPECIES) - ZZ_G(1:N_TRACKED_SPECIES)
...
IF (N_TRACKED_SPECIES==1) THEN
   ...
   IF ( P1%NODE_INDEX < 0 .AND. .NOT.SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX ) THEN
      P1%ZZ_F(1) = 1._EB
      RETURN
   ENDIF
...
IF (CORRECTOR .AND. SF%E_COEFFICIENT>0._EB .AND. I_WATER>0 .AND. (PRESENT(WALL_INDEX).OR.PRESENT(CFACE_INDEX))) THEN
   IF (SPECIES_MIXTURE(I_WATER)%EVAPORATING) THEN
      ...
      DO N=1,N_LAGRANGIAN_CLASSES
         ...
         IF (LPC%Z_INDEX==I_WATER) MPUA_SUM = MPUA_SUM + P2%LP_MPUA(LPC%ARRAY_INDEX)
      ENDDO
      P2%K_SUPPRESSION = P2%K_SUPPRESSION + SF%E_COEFFICIENT*MPUA_SUM*DT
   ENDIF
ENDIF
...
IF (P1%NODE_INDEX > 0) THEN
   IF (-DUCTNODE(P1%NODE_INDEX)%DIR(1)*DUCT_MF(DUCTNODE(P1%NODE_INDEX)%DUCT_INDEX(1))>=0._EB) THEN
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FRACTION) METHOD_OF_MASS_TRANSFER
      ...
      IF (ABS(P1%T_IGN-T_BEGIN)<SPACING(P1%T_IGN) .AND. ANY(SF%RAMP(1:N_TRACKED_SPECIES)%INDEX>=1)) THEN
         IF (PREDICTOR) TSI = T + DT
         IF (CORRECTOR) TSI = T
      ELSE
         IF (PREDICTOR) TSI = T + DT - P1%T_IGN
         IF (CORRECTOR) TSI = T      - P1%T_IGN
      ENDIF

      IF (P1%U_NORMAL_S<0._EB) THEN  ! If there is a non-zero velocity into the domain, assign appropriate species
      ...
      IF (PERIODIC_TEST==13 .AND. (TRIM(SF%ID)=='inlet')) THEN
         P1%ZZ_F(2) = 0.5_EB*(1._EB + COS(4._EB*PI*XC(BC%II)))
         P1%ZZ_F(1) = 1._EB - P1%ZZ_F(2)
      ENDIF
      ...
      END SELECT
      DO N=1,N_TRACKED_SPECIES
         P1%M_DOT_G_PP_ADJUST(N) = SIGN(1._EB,REAL(IOR,EB))*( P1%RHO_F*P1%ZZ_F(N)*UN - P1%RHO_D_DZDN_F(N) )
         P1%M_DOT_G_PP_ACTUAL(N) = P1%M_DOT_G_PP_ADJUST(N)
      ENDDO
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (SF%REFERENCE_HEAT_FLUX > 0._EB) THEN
         ...
         P1%Q_IN_SMOOTH = (P1%Q_IN_SMOOTH *(TSI-DT) + DT*(P1%Q_CON_F+P1%Q_RAD_IN))/TSI
      ENDIF

      ! If the current time is before the "activation" time, T_IGN, apply simple BCs and get out

      IF (T < P1%T_IGN .OR. P1%T_IGN+P1%BURN_DURATION<T .OR. INITIALIZATION_PHASE) THEN
         P1%ZZ_F(1:N_TRACKED_SPECIES) = ZZ_G(1:N_TRACKED_SPECIES)
         IF (PREDICTOR) P1%U_NORMAL_S = 0._EB
         IF (CORRECTOR) P1%U_NORMAL  = 0._EB
         P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES) = 0._EB
         P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES) = 0._EB
         P1%M_DOT_PART_ACTUAL = 0._EB
         RETURN
      ENDIF
      ...
      IF (SF%REFERENCE_HEAT_FLUX > 0._EB .AND. N_REACTIONS>=1 .AND. PREDICTOR) THEN
         ...
         IF (SF%EMISSIVITY > 0._EB) THEN
            P1%T_SCALE = P1%T_SCALE + DT * MAX(0._EB,P1%Q_IN_SMOOTH) / (SF%REFERENCE_HEAT_FLUX * SF%EMISSIVITY)
         ELSE
            P1%T_SCALE = P1%T_SCALE + DT * MAX(0._EB,P1%Q_IN_SMOOTH) / (SF%REFERENCE_HEAT_FLUX)
         ENDIF
         CALL INTERPOLATE1D_UNIFORM(1,RP%INTERPOLATED_DATA(1:RP%NUMBER_INTERPOLATION_POINTS),P1%T_SCALE*RP%RDT,Q_NEW)
      ENDIF
      ...
      SUM_MASSFLUX_LOOP: DO N=1,N_TRACKED_SPECIES
         IF (ABS(SF%MASS_FLUX(N)) > TWO_EPSILON_EB) THEN  ! Use user-specified ramp-up of mass flux
            IF (ABS(P1%T_IGN-T_BEGIN) < SPACING(P1%T_IGN) .AND. SF%RAMP(N)%INDEX>=1) THEN
               IF (PREDICTOR) TSI = T + DT
               IF (CORRECTOR) TSI = T
            ELSE
               IF (PREDICTOR) TSI = T + DT - P1%T_IGN
            ...
            IF (SF%REFERENCE_HEAT_FLUX > 0._EB .AND. N_REACTIONS>=1) THEN
               IF (PREDICTOR) P1%M_DOT_G_PP_ACTUAL(N) = (Q_NEW-P1%Q_SCALE)/DT*SF%MASS_FLUX(N)
            ELSE
               P1%M_DOT_G_PP_ACTUAL(N) = EVALUATE_RAMP(TSI,SF%RAMP(N)%INDEX,TAU=SF%RAMP(N)%TAU)*SF%MASS_FLUX(N)
            ENDIF
            P1%M_DOT_G_PP_ADJUST(N) = SF%ADJUST_BURN_RATE(N)*P1%M_DOT_G_PP_ACTUAL(N)*P1%AREA_ADJUST
         ENDIF
         MFT = MFT + P1%M_DOT_G_PP_ADJUST(N)
      ENDDO SUM_MASSFLUX_LOOP
      
      IF (SF%REFERENCE_HEAT_FLUX > 0._EB .AND. N_REACTIONS>=1 .AND. PREDICTOR) P1%Q_SCALE = Q_NEW
      ...
      IF (PRESENT(WALL_INDEX) .OR. PRESENT(CFACE_INDEX)) THEN
         IF (P2%K_SUPPRESSION>TWO_EPSILON_EB) THEN
            P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES) = P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES)*EXP(-P2%K_SUPPRESSION)
            P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES) = P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES)*EXP(-P2%K_SUPPRESSION)
         ENDIF
      ENDIF
         ...
         IF (OTHER_MESH_OBST_INDEX>0) THEN
            ...
            IF (MESHES(EWC%NOM)%OBSTRUCTION(OTHER_MESH_OBST_INDEX)%CONSUMABLE) THEN
               ...
               OMESH(EWC%NOM)%REAL_SEND_PKG8(LL)   = &
                  (P1%M_DOT_PART_ACTUAL+SUM(P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES)))*DT*P1%AREA
            ENDIF
         ELSE
      ...
      IF (N_TRACKED_SPECIES==1) THEN  ! there is just the background species
         P1%RHO_F = PBAR_P(KK,P1%PRESSURE_ZONE)/(RSUM0*P1%TMP_F)
         P1%ZZ_F(1) = 1._EB
         UN = MFT/P1%RHO_F
      ELSEIF (PRESENT(WALL_INDEX) .AND. .NOT.CELL(IC)%SOLID .AND. .NOT.CELL(IC)%EXTERIOR) THEN  ! this is a thin obstruction
         ...
         IF (CORRECTOR) THEN  ! calculate the mass production rate of gases in the adjacent gas cell
            ...
            DO NS=1,N_TRACKED_SPECIES
               ...
               M_DOT_PPP_SINGLE = P1%M_DOT_G_PP_ADJUST(NS)*P1%AREA*RVC
            ...
            CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,P1%TMP_G)
            H_G = CP*P1%TMP_G
            D_SOURCE(IIG,JJG,KKG) = D_SOURCE(IIG,JJG,KKG) - P1%Q_DOT_G_PP*P1%AREA*RVC/(H_G*RHO_G)
         ENDIF
      ELSE
         RHO_F_PREVIOUS = P1%RHO_F
         DO ITER=1,3
            UN = MFT/P1%RHO_F
            SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
               P1%RHO_D_F(N) = P1%RHO_D_F(N)*P1%RHO_F/RHO_F_PREVIOUS
               DD = 2._EB*P1%RHO_D_F(N)*P1%RDN
               DENOM = DD + UN*P1%RHO_F
               IF ( ABS(DENOM) > TWO_EPSILON_EB ) THEN
                  P1%ZZ_F(N) = ( P1%M_DOT_G_PP_ADJUST(N) + DD*ZZ_G(N) ) / DENOM
               ELSE
                  P1%ZZ_F(N) = ZZ_G(N)
               ENDIF
            ENDDO SPECIES_LOOP
            CALL GET_REALIZABLE_MF(P1%ZZ_F)
            ZZ_GET(1:N_TRACKED_SPECIES) = P1%ZZ_F(1:N_TRACKED_SPECIES)
            CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
            RHO_F_PREVIOUS = P1%RHO_F
            P1%RHO_F = PBAR_P(KK,P1%PRESSURE_ZONE)/(RSUM_F*P1%TMP_F)
         ENDDO
      ENDIF

      IF (PREDICTOR) P1%U_NORMAL_S = -UN
      IF (CORRECTOR) P1%U_NORMAL  = -UN
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. BOUNDARY_TYPE/=OPEN_BOUNDARY) RHOP(BC%II,BC%JJ,BC%KK) = 2._EB*P1%RHO_F - RHO_G
ENDDO WALL_CELL_LOOP
...
ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,P1%ZZ_F(1:N_TRACKED_SPECIES))
CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
P1%RHO_F = PBAR_P(BC%KK,P1%PRESSURE_ZONE)/(RSUM_F*P1%TMP_F)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   WC => WALL(IW)
   P1 => BOUNDARY_PROP1(WC%P1_INDEX)
   IF (P1%NODE_INDEX == 0) CYCLE WALL_CELL_LOOP
   BC => BOUNDARY_COORD(WC%BC_INDEX)
   SURF_INDEX = WC%SURF_INDEX
   Q_LEAK => WC%Q_LEAK
   P1%TMP_G = TMP(BC%IIG,BC%JJG,BC%KKG)
...
CFACE_LOOP: DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   CFA => CFACE(ICF)
   P1 => BOUNDARY_PROP1(CFA%P1_INDEX)
   IF (P1%NODE_INDEX == 0) CYCLE CFACE_LOOP
   BC => BOUNDARY_COORD(CFA%BC_INDEX)
   SURF_INDEX = CFA%SURF_INDEX
   Q_LEAK => CFA%Q_LEAK
   P1%TMP_G = CFA%TMP_G
...
DU=DUCTNODE(P1%NODE_INDEX)%DUCT_INDEX(1)
MFT = -DUCTNODE(P1%NODE_INDEX)%DIR(1)*DUCT_MF(DU)/NODE_AREA_EX(P1%NODE_INDEX)
IF (.NOT. ANY(SF%LEAK_PATH>0)) THEN
   IF (DUCTNODE(P1%NODE_INDEX)%DIR(1)*DUCT_MF(DU) > 0._EB) THEN
      IF (SF%THERMAL_BC_INDEX==HVAC_BOUNDARY) THEN
         P1%TMP_F = NODE_TMP_EX(P1%NODE_INDEX)
         P1%HEAT_TRANS_COEF = 0._EB
         P1%Q_CON_F = 0._EB
      ELSE
         IF (DUCT(DU)%LEAK_ENTHALPY) THEN
            ZZ_GET(1:N_TRACKED_SPECIES) = NODE_ZZ_EX(P1%NODE_INDEX,1:N_TRACKED_SPECIES)
            CALL GET_ENTHALPY(ZZ_GET,H_G,P1%TMP_F)
            CALL GET_ENTHALPY(ZZ_GET,H_D,NODE_TMP_EX(P1%NODE_INDEX))
            Q_LEAK = -MFT*(H_D-H_G)*P1%RDN
         ENDIF
      ENDIF
   ELSE
      IF (SF%THERMAL_BC_INDEX==HVAC_BOUNDARY) THEN
...
IF (MFT >= 0._EB) THEN
   ZZ_GET(1:N_TRACKED_SPECIES) = ZZ_G(1:N_TRACKED_SPECIES)
   CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
   P1%RHO_F = PBAR_P(KK,P1%PRESSURE_ZONE)/(RSUM_F*P1%TMP_G)
   UN = MFT/P1%RHO_F
   IF (PREDICTOR) P1%U_NORMAL_S = UN
   IF (CORRECTOR) P1%U_NORMAL  = UN
ELSE
   P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES) = -NODE_ZZ_EX(P1%NODE_INDEX,1:N_TRACKED_SPECIES)*MFT
ENDIF
...
UNPACK_WALL_PARTICLE: IF (PRESENT(WALL_INDEX)) THEN
   ...
   IF (WC%BACK_INDEX>0 .AND. WC%BACK_MESH==NM) THEN
      ...
      P1_BACK => BOUNDARY_PROP1(WC_BACK%P1_INDEX)
      P2_BACK => BOUNDARY_PROP2(WC_BACK%P2_INDEX)
   ...
   ELSEIF (WC%BACK_INDEX>0 .AND. WC%BACK_MESH/=NM) THEN
      ...
      P1_BACK => MESHES(WC%BACK_MESH)%BOUNDARY_PROP1(WC_BACK%P1_INDEX)
      P2_BACK => MESHES(WC%BACK_MESH)%BOUNDARY_PROP2(WC_BACK%P2_INDEX)
   ...
   IF (NLP>0) THEN
      Q_WATER_F = -SUM(P2%LP_CPUA(:)) + P1%Q_CONDENSE
   ELSE
      Q_WATER_F = P1%Q_CONDENSE
   ENDIF
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   IF (TW%BACK_INDEX>0 .AND. TW%BACK_MESH==NM) THEN
      ...
      P1_BACK => BOUNDARY_PROP1(TW_BACK%P1_INDEX)
   ...
   ELSEIF (TW%BACK_INDEX>0 .AND. TW%BACK_MESH/=NM) THEN
      ...
      P1_BACK => MESHES(TW%BACK_MESH)%BOUNDARY_PROP1(TW_BACK%P1_INDEX)
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN UNPACK_WALL_PARTICLE
   ...
   IF (CFA%BACK_INDEX>0 .AND. CFA%BACK_MESH==NM) THEN
      ...
      P1_BACK => BOUNDARY_PROP1(CFA_BACK%P1_INDEX)
      P2_BACK => BOUNDARY_PROP2(CFA_BACK%P2_INDEX)
...
IF (SF%TMP_GAS_FRONT > 0._EB) P1%TMP_G = TMPA + EVALUATE_RAMP(T-T_BEGIN,SF%RAMP(TIME_TGF)%INDEX)*(SF%TMP_GAS_FRONT-TMPA)
...
P1%HEAT_TRANS_COEF = 0._EB
P1%Q_CON_F = 0._EB
IF (SF%INTERNAL_RADIATION) Q_RAD_OUT_OLD = P1%Q_RAD_OUT
P1%Q_RAD_OUT = 0._EB
...
SELECT CASE(BACKING)
   ...
   CASE(EXPOSED)  ! The backside is exposed to gas in current or adjacent mesh.
      ...
      IF (WC%BACK_MESH/=NM .AND. WC%BACK_MESH>0) THEN  ! Back side is in other mesh.
         TMP_GAS_BACK = P1_BACK%TMP_G
         DTMP = TMP_GAS_BACK - P1%TMP_B
         HTCB = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED_B,SURF_INDEX_IN=SURF_INDEX)
         Q_RAD_IN_B  = P1_BACK%Q_RAD_IN
      ELSE  ! Back side is in current mesh.
         ...
         P1_BACK%HEAT_TRANS_COEF = HTCB
         Q_RAD_IN_B  = P1_BACK%Q_RAD_IN
         IF (NLP>0) Q_WATER_B = -SUM(P2_BACK%LP_CPUA(:)) + P1%Q_CONDENSE
      ENDIF
      Q_CON_B = HTCB*DTMP
...
PYROLYSIS_PREDICTED_IF: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
   ...
   POINT_LOOP1: DO I=1,NWP
      ...
      IF (PRESENT(PARTICLE_INDEX)) THEN
         CALL PYROLYSIS(SF%N_MATL,SF%MATL_INDEX,SURF_INDEX,BC%IIG,BC%JJG,BC%KKG,ONE_D%TMP(I),P1%TMP_F,BC%IOR,&
                        ...
                        Q_DOT_PART,M_DOT_PART,T_BOIL_EFF,P1%B_NUMBER,LAYER_INDEX(I),SOLID_CELL_INDEX=I,&
...
ELSEIF (SF%PYROLYSIS_MODEL==PYROLYSIS_SPECIFIED) THEN PYROLYSIS_PREDICTED_IF
   ...
   Q_S(1) = Q_S(1) - P1%M_DOT_G_PP_ADJUST(REACTION(1)%FUEL_SMIX_INDEX)*SF%H_V/DX_S(1)
...
IF (SF%INTERNAL_RADIATION) THEN
   ...
   ENDDO
   ! solution inwards
   RFLUX_UP = P1%Q_RAD_IN + (1._EB-P1%EMISSIVITY)*Q_RAD_OUT_OLD/(P1%EMISSIVITY+1.0E-10_EB)
   DO I=1,NWP
   ...
   DO I=NWP,1,-1
      ...
      RFLUX_UP = RFLUX_DOWN
   ENDDO
   Q_RAD_OUT = P1%EMISSIVITY*RFLUX_DOWN
ENDIF
...
IF (ICYC>WALL_INCREMENT) THEN
   IF (SF%INTERNAL_RADIATION) THEN
      ...
      Q_NET_B = Q_CON_B
   ELSE
      Q_NET_F = P1%Q_RAD_IN - P1%EMISSIVITY*SIGMA*P1%TMP_F**4 + Q_CON_F
      Q_NET_B = Q_RAD_IN_B  - E_WALLB      *SIGMA*P1%TMP_B**4 + Q_CON_B
   ENDIF
   DO I=2,NWP-1
...
IF (SF%INTERNAL_RADIATION) THEN
   ...
   P1%Q_RAD_OUT = P1%Q_RAD_OUT + Q_RAD_OUT*DT_BC_SUB/DT_BC
ENDIF
...
IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED .AND. .NOT.PRESENT(THIN_WALL_INDEX)) THEN
   ...
   ENDIF
   P1%Q_DOT_G_PP              = P1%Q_DOT_G_PP              + Q_DOT_G_PP*DT_BC_SUB*RDT
   P1%Q_DOT_O2_PP             = P1%Q_DOT_O2_PP             + Q_DOT_O2_PP*DT_BC_SUB*RDT
   P1%M_DOT_G_PP_ADJUST       = P1%M_DOT_G_PP_ADJUST       + P1%AREA_ADJUST*M_DOT_G_PP_ADJUST*DT_BC_SUB*RDT
   P1%M_DOT_G_PP_ACTUAL       = P1%M_DOT_G_PP_ACTUAL       +                M_DOT_G_PP_ACTUAL*DT_BC_SUB*RDT
...
PYROLYSIS_PREDICTED_IF_2: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
   ...
   REMESH_GRID: IF (CHANGE_THICKNESS) THEN
      ...
      IF (THICKNESS <=TWO_EPSILON_EB) THEN
         ...
         P1%TMP_F        = MIN(TMPMAX,MAX(TMPMIN,TMP_GAS_BACK))
         P1%TMP_B        = MIN(TMPMAX,MAX(TMPMIN,TMP_GAS_BACK))
         P1%Q_CON_F        = 0._EB
         P1%Q_RAD_OUT      = 0._EB         
         P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES) = 0._EB
         P1%M_DOT_G_PP_ACTUAL(1:N_TRACKED_SPECIES) = 0._EB
      ...
      REMESH_IF: IF (ANY(REMESH_LAYER)) THEN
         ...
         ONE_D%TMP(0)         = 2._EB*P1%TMP_F-ONE_D%TMP(1)   !Make sure front surface temperature stays the same
         ONE_D%TMP(NWP_NEW+1) = 2._EB*P1%TMP_B-ONE_D%TMP(NWP_NEW) !Make sure back surface temperature stays the same
...
POINT_LOOP3: DO I=1,NWP
   ...
   MATERIAL_LOOP3: DO N=1,SF%N_MATL
      ...
      IF (.NOT.E_FOUND) P1%EMISSIVITY = P1%EMISSIVITY + ONE_D%MATL_COMP(N)%RHO(I)*ML%EMISSIVITY/SF%RHO_S(LAYER_INDEX(I),N)
      RHO_S(I) = RHO_S(I) + ONE_D%MATL_COMP(N)%RHO(I)
   ENDDO MATERIAL_LOOP3

   IF (VOLSUM > 0._EB) THEN
      ONE_D%K_S(I) = ONE_D%K_S(I)/VOLSUM
      IF (.NOT.E_FOUND) P1%EMISSIVITY = P1%EMISSIVITY/VOLSUM
   ENDIF
   IF (P1%EMISSIVITY>=0._EB) E_FOUND = .TRUE.
...
IF (SF%DIRICHLET_FRONT) THEN
   RFACF2 = -1._EB
   QDXKF  = 2._EB*P1%TMP_F
ELSEIF (.NOT.RADIATION .OR. SF%INTERNAL_RADIATION .OR. PRESENT(THIN_WALL_INDEX)) THEN
   RFACF  = 0.5_EB*HTCF
   RFACF2 = (KODXF-RFACF)/(KODXF+RFACF)
   QDXKF  = (HTCF*P1%TMP_G  + Q_WATER_F)/(KODXF+RFACF)
ELSE
   RFACF  = 0.5_EB*HTCF + 2._EB*P1%EMISSIVITY*SIGMA*P1%TMP_F**3
   RFACF2 = (KODXF-RFACF)/(KODXF+RFACF)
   QDXKF  = (HTCF*P1%TMP_G  + Q_WATER_F + P1%Q_RAD_IN + 3._EB*P1%EMISSIVITY*SIGMA*P1%TMP_F**4) / (KODXF+RFACF)
ENDIF

IF (SF%DIRICHLET_BACK) THEN
   RFACB2 = -1._EB
   QDXKB  = 2._EB*P1%TMP_B
ELSEIF (.NOT.RADIATION .OR. SF%INTERNAL_RADIATION .OR. PRESENT(THIN_WALL_INDEX)) THEN
   RFACB  = 0.5_EB*HTCB
   RFACB2 = (KODXB-RFACB)/(KODXB+RFACB)
   QDXKB  = (HTCB*TMP_GAS_BACK + Q_WATER_B)/(KODXB+RFACB)
ELSE
   RFACB  = 0.5_EB*HTCB + 2._EB*E_WALLB*SIGMA*P1%TMP_B**3
   RFACB2 = (KODXB-RFACB)/(KODXB+RFACB)
   QDXKB  = (HTCB*TMP_GAS_BACK + Q_WATER_B + Q_RAD_IN_B     + 3._EB*E_WALLB*SIGMA*P1%TMP_B**4         ) / (KODXB+RFACB)
ENDIF
...
P1%Q_CON_F = P1%Q_CON_F + HTCF*DT_BC_SUB*(P1%TMP_G-0.5_EB*P1%TMP_F)
P1%TMP_F_OLD = P1%TMP_F  ! Save this value for output of effective HTC

P1%TMP_F  = 0.5_EB*(ONE_D%TMP(0)+ONE_D%TMP(1))
P1%TMP_B  = 0.5_EB*(ONE_D%TMP(NWP)+ONE_D%TMP(NWP+1))

P1%Q_CON_F = P1%Q_CON_F - 0.5_EB*HTCF*DT_BC_SUB*P1%TMP_F
IF (.NOT.SF%INTERNAL_RADIATION) P1%Q_RAD_OUT = P1%Q_RAD_OUT + &
                                               DT_BC_SUB*(P1%TMP_F_OLD**4+2._EB*P1%TMP_F_OLD**3*(P1%TMP_F-P1%TMP_F_OLD))

! Clipping for excessively high or low temperatures

P1%TMP_F  = MIN(TMPMAX,MAX(TMPMIN,P1%TMP_F))
P1%TMP_B  = MIN(TMPMAX,MAX(TMPMIN,P1%TMP_B))
...
IF (SF%N_LPC > 0) THEN
   ...
   P1%T_MATL_PART = P1%T_MATL_PART + DT_BC_SUB
   P1%M_DOT_PART_ACTUAL = SUM(M_DOT_PART_S(1:SF%N_LPC))
ENDIF
...
P1%N_SUBSTEPS = P1%N_SUBSTEPS + 1

ENDDO SUB_TIMESTEP_LOOP

P1%Q_CON_F = P1%Q_CON_F / DT_BC
IF (.NOT. SF%INTERNAL_RADIATION) P1%Q_RAD_OUT = P1%Q_RAD_OUT / DT_BC * SIGMA * P1%EMISSIVITY
P1%HEAT_TRANS_COEF = HTCF
...
IF (P1%T_IGN > T) THEN
   IF (SUM(P1%M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES)) > 0._EB .OR. P1%M_DOT_PART_ACTUAL > 0._EB ) P1%T_IGN = T
   IF (P1%TMP_F>=SF%TMP_IGN) P1%T_IGN = T
ENDIF

! If the surface temperature is less than the extinction temperature, stop the burning

IF (SF%TMP_IGN<50000._EB .AND. P1%TMP_F<SF%TMP_EXT .AND. P1%T_IGN<T) P1%T_IGN = HUGE(1._EB)
...
IF (PRESENT(PARTICLE_INDEX_IN)) THEN
   ...
   P1X => BOUNDARY_PROP1(LPX%P1_INDEX)
...
ELSEIF (PRESENT(WALL_INDEX_IN)) THEN
   ...
   P1X => BOUNDARY_PROP1(WCX%P1_INDEX)
   P2X => BOUNDARY_PROP2(WCX%P2_INDEX)
...
ELSEIF (PRESENT(CFACE_INDEX_IN)) THEN
   ...
   P1X => BOUNDARY_PROP1(CFAX%P1_INDEX)
   P2X => BOUNDARY_PROP2(CFAX%P2_INDEX)
...
DO I=1,N_TGA
   ...
   IF (I==1 .OR. MOD(I,NINT(1._EB/(TGA_HEATING_RATE*DT_TGA)))==0) THEN
      ...
      IF (N_REACTIONS>0) THEN
         HRR = 0._EB
         DO NR=1,N_REACTIONS
            HRR = P1%M_DOT_G_PP_ADJUST(REACTION(NR)%FUEL_SMIX_INDEX)*0.001*REACTION(NR)%HEAT_OF_COMBUSTION / &
                  (P1%AREA_ADJUST*SURF_DEN_0)
         ENDDO
      ELSE
         HRR = 0._EB
      ENDIF
      WRITE(LU_TGA,TCFORM) REAL(T_TGA,FB), REAL(P1%TMP_F-TMPM,FB), (REAL(SURF_DEN(N)/SURF_DEN_0,FB),N=0,SF%N_MATL), &
                           REAL(-SUM(ONE_D%M_DOT_S_PP(1:SF%N_MATL))/SURF_DEN_0,FB), &
                           (REAL(-ONE_D%M_DOT_S_PP(N)/SURF_DEN_0,FB),N=1,SF%N_MATL), &
                           REAL(HRR,FB), REAL(P1%HEAT_TRANS_COEF*(SF%TMP_GAS_FRONT-P1%TMP_F)*0.001_EB/SURF_DEN_0,FB)
   ENDIF
ENDDO
