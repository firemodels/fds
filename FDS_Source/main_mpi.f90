...
MAIN_LOOP: DO  
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      IF (BAROCLINIC) THEN
         RHO_LOOP1: DO NM=1,NMESHES
            IF (PROCESS(NM)/=MYID)    CYCLE RHO_LOOP1
            IF (.NOT.ACTIVE_MESH(NM)) CYCLE RHO_LOOP1
            RHO_UPPER_LOCAL = MAX(RHO_UPPER_GLOBAL,MESHES(NM)%RHO_UPPER)
            RHO_LOWER_LOCAL = MIN(RHO_LOWER_GLOBAL,MESHES(NM)%RHO_LOWER)
         ENDDO RHO_LOOP1
   ...
   IF (BAROCLINIC) THEN
      RHO_LOOP2: DO NM=1,NMESHES
         IF (PROCESS(NM)/=MYID)    CYCLE RHO_LOOP2
         IF (.NOT.ACTIVE_MESH(NM)) CYCLE RHO_LOOP2
         RHO_UPPER_LOCAL = MAX(RHO_UPPER_GLOBAL,MESHES(NM)%RHO_UPPER)
         RHO_LOWER_LOCAL = MIN(RHO_LOWER_GLOBAL,MESHES(NM)%RHO_LOWER)
      ENDDO RHO_LOOP2
      CALL MPI_ALLREDUCE(RHO_UPPER_LOCAL,RHO_UPPER_GLOBAL,1,MPI_DOUBLE_PRECISION,MPI_MAX,MPI_COMM_WORLD,IERR)
