...
PUBLIC INITIALIZE_MESH_VARIABLES_1,INITIALIZE_MESH_VARIABLES_2,INITIALIZE_MESH_VARIABLES_3,INITIALIZE_GLOBAL_VARIABLES, &
       ...
       ADJUST_VARIABLE_THICKNESS_WALL_CELLS,INITIALIZE_VARIABLE_THICKNESS_WALL_CELLS
...
INTEGER :: N,NN,I,J,K,IW,IC,SURF_INDEX,IOR,IERR,IZERO,II,JJ,KK,I_OBST,N_EXTERNAL_CELLS,OB_I1,OB_I2,OB_J1,OB_J2,OB_K1,OB_K2, &
           THIN_INDEX
...
OBST_LOOP_2: DO N=1,M%N_OBST
   ...
   OB_I1=OB%I1 ; OB_I2=OB%I2 ; OB_J1=OB%J1 ; OB_J2=OB%J2 ; OB_K1=OB%K1 ; OB_K2=OB%K2

   IF (SURFACE(OB%SURF_INDEX(1))%VARIABLE_THICKNESS .OR. SURFACE(OB%SURF_INDEX(-1))%VARIABLE_THICKNESS .OR. &
       SURFACE(OB%SURF_INDEX(2))%VARIABLE_THICKNESS .OR. SURFACE(OB%SURF_INDEX(-2))%VARIABLE_THICKNESS .OR. &
       SURFACE(OB%SURF_INDEX(3))%VARIABLE_THICKNESS .OR. SURFACE(OB%SURF_INDEX(-3))%VARIABLE_THICKNESS) THEN
      IF (OB_I1==OB_I2) OB_I1 = OB_I1-1
      IF (OB_J1==OB_J2) OB_J1 = OB_J1-1
      IF (OB_K1==OB_K2) OB_K1 = OB_K1-1
   ENDIF
...
USE MEMORY_FUNCTIONS, ONLY: PACK_WALL
...
WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
  ...
  !      M%OMESH(NOM)%N_WALL_CELLS_RECV = M%OMESH(NOM)%N_WALL_CELLS_RECV + 1
   ...
   IF_THERM_THICK_EXPOSED: IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK .AND. SF%BACKING==EXPOSED) THEN ! search for the back side
      ...
      FIND_BACK_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
         ...
         IF (.NOT.SF%VARIABLE_THICKNESS .AND. ITER==2) CYCLE WALL_LOOP  ! 1-D heat transfer allowed only in 0 or 1 cells
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (NOM/=NM .AND. PROCESS(NOM)/=MY_RANK) THEN
      ...
      WALL_SAVE(NOM)%SURF_INDICES(WALL_SAVE(NOM)%COUNTER) = WC%SURF_INDEX
...
DO N=1,M%N_NEIGHBORING_MESHES
   ...
   DO IC=1,CELL_COUNT(NOM)
      DO IOR=-3,3
         IF (IOR==0) CYCLE
         IF (SURFACE(M4%SURF_INDEX(IC,IOR))%VARIABLE_THICKNESS) THEN
...
! Allocate 1-D arrays to store the variables of WALL and CFACE cells for restarts and MPI exchanges

DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   WC => M%WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)
   IF (SF%N_WALL_STORAGE_INTEGERS<1 .OR. SF%VARIABLE_THICKNESS) THEN
...
!> \brief Find WALL cells with VARIABLE_THICKNESS and adjust the 1-D internal noding
!> \param NM Mesh index

SUBROUTINE ADJUST_VARIABLE_THICKNESS_WALL_CELLS(NM)
...
PRIMARY_WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.SF%VARIABLE_THICKNESS .OR. WC%BACK_INDEX<=0) CYCLE PRIMARY_WALL_LOOP_1
   ...
   IF (ONE_D%LAYER_THICKNESS(1)<TWO_EPSILON_EB) THEN
      SELECT CASE(ABS(BC%IOR))
         CASE(1) ; ONE_D%LAYER_THICKNESS(1) = M%OBSTRUCTION(WC%OBST_INDEX)%X2 - M%OBSTRUCTION(WC%OBST_INDEX)%X1
         CASE(2) ; ONE_D%LAYER_THICKNESS(1) = M%OBSTRUCTION(WC%OBST_INDEX)%Y2 - M%OBSTRUCTION(WC%OBST_INDEX)%Y1
         CASE(3) ; ONE_D%LAYER_THICKNESS(1) = M%OBSTRUCTION(WC%OBST_INDEX)%Z2 - M%OBSTRUCTION(WC%OBST_INDEX)%Z1 
      END SELECT
   ENDIF

   IF (WC%THIN(1)) BC%DX = M%OBSTRUCTION(WC%OBST_INDEX)%X2 - M%OBSTRUCTION(WC%OBST_INDEX)%X1
   IF (WC%THIN(2)) BC%DY = M%OBSTRUCTION(WC%OBST_INDEX)%Y2 - M%OBSTRUCTION(WC%OBST_INDEX)%Y1
   IF (WC%THIN(3)) BC%DZ = M%OBSTRUCTION(WC%OBST_INDEX)%Z2 - M%OBSTRUCTION(WC%OBST_INDEX)%Z1
...
END SUBROUTINE ADJUST_VARIABLE_THICKNESS_WALL_CELLS


!> \brief Set up weighting arrays to transfer 3D solid phase temperatures from one direction sweep to another.
!> \param NM Mesh index

SUBROUTINE INITIALIZE_VARIABLE_THICKNESS_WALL_CELLS(NM)

INTEGER, INTENT(IN) :: NM
INTEGER :: I,IW,IW2,NWP,NWP2,I2,IWA,DM,IOR,NOM
...
! Loop over all wall cells looking for 3D cases. When found, compute the node coordinates.

PRIMARY_WALL_LOOP_2: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS

   WC => M%WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)

   IF (.NOT.SF%VARIABLE_THICKNESS .OR. WC%BACK_INDEX<=0) CYCLE PRIMARY_WALL_LOOP_2
   ...
   PRIMARY_NODE_LOOP: DO I=1,NWP  ! Loop over nodes of primary wall cell
      ...
      X1=M%X(BC%IIG-1) ; X2=M%X(BC%IIG) ; Y1=M%Y(BC%JJG-1) ; Y2=M%Y(BC%JJG) ; Z1=M%Z(BC%KKG-1) ; Z2=M%Z(BC%KKG)
      SELECT CASE(BC%IOR)
         CASE( 1) ; X1=BC%X-ONE_D%X(I)   ; X2=BC%X-ONE_D%X(I-1) ; EXTRUDE=-1._EB
         CASE(-1) ; X1=BC%X+ONE_D%X(I-1) ; X2=BC%X+ONE_D%X(I)   ; EXTRUDE= 1._EB
         CASE( 2) ; Y1=BC%Y-ONE_D%X(I)   ; Y2=BC%Y-ONE_D%X(I-1) ; EXTRUDE=-1._EB
         CASE(-2) ; Y1=BC%Y+ONE_D%X(I-1) ; Y2=BC%Y+ONE_D%X(I)   ; EXTRUDE= 1._EB
         CASE( 3) ; Z1=BC%Z-ONE_D%X(I)   ; Z2=BC%Z-ONE_D%X(I-1) ; EXTRUDE=-1._EB
         CASE(-3) ; Z1=BC%Z+ONE_D%X(I-1) ; Z2=BC%Z+ONE_D%X(I)   ; EXTRUDE= 1._EB
      END SELECT
      IF (WC%THIN(1)) THEN ; X1=M%X(BC%II) ; X2=X1+BC%DX ; EXTRUDE=1._EB ; ENDIF
      IF (WC%THIN(2)) THEN ; Y1=M%Y(BC%JJ) ; Y2=Y1+BC%DY ; EXTRUDE=1._EB ; ENDIF
      IF (WC%THIN(3)) THEN ; Z1=M%Z(BC%KK) ; Z2=Z1+BC%DZ ; EXTRUDE=1._EB ; ENDIF
      PRIMARY_VOLUME = (X2-X1)*(Y2-Y1)*(Z2-Z1)
      IOR_AVOID = .FALSE.
      IOR_AVOID(BC%IOR) = .TRUE. ; IOR_AVOID(-BC%IOR) = .TRUE.
      IWA = 0
      THR_D%NODE(I)%ALTERNATE_WALL_VALUES = 0

      ALTERNATE_WALL_LOOP: DO IW2=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS  ! Loop over potential alternate wall cells
         CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NM)
      ENDDO ALTERNATE_WALL_LOOP

      OTHER_MESH_LOOP: DO NOM=1,NMESHES
         ALTERNATE_WALL_LOOP_2: DO IW2=1,M%OMESH(NOM)%N_WALL_CELLS_RECV
            CALL SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM)
      ...
      IF (ABS(SUM(THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(:))-2._EB)>0.001_EB) THEN
         ...
         ENDDO
         WRITE(0,'(A,I0,A,I0,A,I0,A,I0,A,I0,A,I0,A,I0,A,F6.3)') &
            'WARNING: Mesh=',NM,' WALL=',IW,' IJK=',BC%IIG,',',BC%JJG,',',BC%KKG,' IOR=',BC%IOR,' NODE=',I,' WEIGHT=',&
            SUM(THR_D%NODE(I)%ALTERNATE_WALL_WEIGHT(:))
      ENDIF

   ENDDO PRIMARY_NODE_LOOP

ENDDO PRIMARY_WALL_LOOP_2

CONTAINS

SUBROUTINE SEARCH_FOR_ALTERNATE_WALL_CELLS(NOM)
...
IF (.NOT.SF2%VARIABLE_THICKNESS) RETURN
BC2 => M2%BOUNDARY_COORD(WC2%BC_INDEX)
IF (IOR_AVOID(BC2%IOR)) RETURN
XX1=M2%X(BC2%IIG-1) ; XX2=M2%X(BC2%IIG) ; YY1=M2%Y(BC2%JJG-1) ; YY2=M2%Y(BC2%JJG) ; ZZ1=M2%Z(BC2%KKG-1) ; ZZ2=M2%Z(BC2%KKG)
IF (.NOT.ANY(WC%THIN) .AND. .NOT.ANY(WC2%THIN)) THEN
...
ALTERNATE_NODE_LOOP: DO I2=1,NWP2  ! Loop over nodes of alternate wall cell
   SELECT CASE(BC2%IOR)
      CASE( 1) ; XX1=BC2%X-ONE_D2%X(I2)   ; XX2=BC2%X-ONE_D2%X(I2-1) 
      CASE(-1) ; XX1=BC2%X+ONE_D2%X(I2-1) ; XX2=BC2%X+ONE_D2%X(I2) 
      CASE( 2) ; YY1=BC2%Y-ONE_D2%X(I2)   ; YY2=BC2%Y-ONE_D2%X(I2-1) 
      CASE(-2) ; YY1=BC2%Y+ONE_D2%X(I2-1) ; YY2=BC2%Y+ONE_D2%X(I2) 
      CASE( 3) ; ZZ1=BC2%Z-ONE_D2%X(I2)   ; ZZ2=BC2%Z-ONE_D2%X(I2-1) 
      CASE(-3) ; ZZ1=BC2%Z+ONE_D2%X(I2-1) ; ZZ2=BC2%Z+ONE_D2%X(I2) 
   END SELECT
   IF (WC2%THIN(1)) THEN ; XX1=M2%X(BC2%II) ; XX2=XX1+EXTRUDE*BC2%DX ; ENDIF
   IF (WC2%THIN(2)) THEN ; YY1=M2%Y(BC2%JJ) ; YY2=YY1+EXTRUDE*BC2%DY ; ENDIF
   IF (WC2%THIN(3)) THEN ; ZZ1=M2%Z(BC2%KK) ; ZZ2=ZZ1+EXTRUDE*BC2%DZ ; ENDIF
   ...
   IF (ABS(BC2%IOR)==1 .AND. WC%THIN(1) .AND. BC%II==BC2%II) THEN ; XX1=M2%X(BC%II) ; XX2=XX1+EXTRUDE*BC%DX ; ENDIF
   IF (ABS(BC2%IOR)==2 .AND. WC%THIN(2) .AND. BC%JJ==BC2%JJ) THEN ; YY1=M2%Y(BC%JJ) ; YY2=YY1+EXTRUDE*BC%DY ; ENDIF
   IF (ABS(BC2%IOR)==3 .AND. WC%THIN(3) .AND. BC%KK==BC2%KK) THEN ; ZZ1=M2%Z(BC%KK) ; ZZ2=ZZ1+EXTRUDE*BC%DZ ; ENDIF
   ...
   ENDIF
   THR_D%NODE(I)%ALTERNATE_WALL_VALUES = THR_D%NODE(I)%ALTERNATE_WALL_VALUES + 1
   THR_D%NODE(I)%ALTERNATE_WALL_MESH(IWA) = NOM
   THR_D%NODE(I)%ALTERNATE_WALL_INDEX(IWA) = IW2
...
END SUBROUTINE INITIALIZE_VARIABLE_THICKNESS_WALL_CELLS
...
SUBROUTINE INIT_WALL_CELL(NM,I,J,K,OBST_INDEX,IW,IOR,SURF_INDEX,IERR,TT,THIN_INDEX)
...
INTEGER, INTENT(IN), OPTIONAL :: THIN_INDEX
...
IF (PRESENT(THIN_INDEX)) THEN
   IF (THIN_INDEX>0) WC%THIN(THIN_INDEX) = .TRUE.
ENDIF
...
BC%Y = YW
BC%Z = ZW
IF (ABS(BC%IOR)==1) THEN ; BC%DY=M%DY(J) ; BC%DZ=M%DZ(K) ; ENDIF
IF (ABS(BC%IOR)==2) THEN ; BC%DX=M%DX(I) ; BC%DZ=M%DZ(K) ; ENDIF
IF (ABS(BC%IOR)==3) THEN ; BC%DX=M%DX(I) ; BC%DY=M%DY(J) ; ENDIF
