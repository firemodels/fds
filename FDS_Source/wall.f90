...
USE PHYSICAL_FUNCTIONS, ONLY: GET_CPBAR
REAL(EB) :: DT_BC,T,TSI,TMP_G,RHO_G,DTMP,TMP_OTHER,CP_TERM,CP_TERM_F,RHOWAL,RAMP_FACTOR,QNET,FDERIV,TMP_EXTERIOR,UN,ARO,UWO,&
            CP,CP_SUM,YY_N(1:N_SPECIES),QEXTRA
...
HEAT_FLUX_LOOP: DO IW=1,NWC
   ...
   IF (SF%THERMAL_BC_INDEX /= NO_CONVECTION .AND. SF%THERMAL_BC_INDEX /= INTERPOLATED_BC) THEN
      ITMP = 0.05_EB*(TMP(IIG,JJG,KKG)+TMP_W(IW))
         ...
         IF (N_SPECIES > (I_Z_MAX-I_Z_MIN+1)) THEN
            ...
            DO N=1,N_SPECIES
               IF (SPECIES(N)%MODE/=MIXTURE_FRACTION_SPECIES) &
                  CP_SUM = CP_SUM + YY_N(N)*SPECIES(N)%CPBAR(ITMP)
            END DO
            CP = CP_SUM + (1._EB-Y_SUM(IIG,JJG,KKG))*CP
         ENDIF
      ELSEIF (.NOT.MIXTURE_FRACTION) THEN
         IF (N_SPECIES>0) THEN
            ...
            DO N=1,N_SPECIES
               CP = CP + YY_N(N)*(SPECIES(N)%CPBAR(ITMP)-SPECIES(0)%CPBAR(ITMP))
            END DO
         ELSE
            CP = SPECIES(0)%CPBAR(ITMP)
         ENDIF
   ...
   METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
      ...
      CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
         ...
         CP_TERM_F = MAX(0._EB,-UW(IW))*RHOWAL*SPECIES(0)%CPBAR(INT(0.1_EB*TMP_F(IW))) !! Temporary patch, Issue 567
         IF (FLUX_LIMITER<0) THEN
            TMP_W(IW) = ( (RDN(IW)*KW(IW)-0.5_EB*CP_TERM)*TMP_G + CP_TERM_F*TMP_F(IW) - QCONF(IW) )/(0.5_EB*CP_TERM+RDN(IW)*KW(IW))
         ELSE
...
! Determine ghost cell value of RSUM=R0*Sum(Y_i/M_i) for mixture fraction case
   ...
   IF (MIXTURE_FRACTION) THEN
      ...
      DO N=1,N_SPECIES
         ...
         ENDIF
      ENDDO
      Z_VECTOR = YY_W(IW,I_Z_MIN:I_Z_MAX)
      CALL GET_MOLECULAR_WEIGHT(Z_VECTOR,Y_SUM_W,RSUM_W(IW))
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_MOLECULAR_WEIGHT, GET_CPBAR
USE GEOMETRY_FUNCTIONS
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
REAL(EB) :: DTMP,QNETF,QDXKF,QDXKB,RR,TMP_G,T,RFACF,RFACB,RFACF2,RFACB2, &
            PPCLAUS,PPSURF,TMP_G_B,RHOWAL,CP_TERM,DT_BC, CP,CP_SUM,&
            DXKF,DXKB,REACTION_RATE,QCONB,DELTA_RHO_S,QRADINB,RFLUX_UP,RFLUX_DOWN,E_WALLB, &
            HVRG,Z_2,Y_MF_G,Y_MF_W,YY_S,Y_SUM_W, RSUM_W, RSUM_G, RSUM_S, MFLUX, MFLUX_S, VOLSUM, &
...
REAL(EB) :: SMALLEST_CELL_SIZE(MAX_LAYERS),THICKNESS,LAYER_THICKNESS_NEW(MAX_LAYERS),YY_G(1:N_SPECIES)
...
WALL_CELL_LOOP: DO IW=1,NWC+NVWC
   ...
   IF (N_SPECIES > 0) YY_G(:) = 0.5_EB*(YY(IIG,JJG,KKG,:)+YY_W(IW,:))
   IF (MIXTURE_FRACTION) THEN
      ...
      IF (N_SPECIES > (I_Z_MAX-I_Z_MIN+1)) THEN
         ...
         DO N=1,N_SPECIES
            IF (SPECIES(N)%MODE/=MIXTURE_FRACTION_SPECIES) &
               CP_SUM = CP_SUM + YY_G(N)*SPECIES(N)%CPBAR(ITMP)
         END DO
         CP = CP_SUM + (1._EB-Y_SUM(IIG,JJG,KKG))*CP
      ENDIF
   ELSEIF (.NOT.MIXTURE_FRACTION) THEN
      IF(N_SPECIES>0) THEN
         CP = SPECIES(0)%CPBAR(ITMP)
         DO N=1,N_SPECIES
            CP = CP + YY_G(N)*(SPECIES(N)%CPBAR(ITMP)-SPECIES(0)%CPBAR(ITMP))
         END DO
      ELSE
         CP = SPECIES(0)%CPBAR(ITMP)
      ENDIF
         ...
         DO J=1,ML%N_REACTIONS
            ...
            DO NS = 1,N_SPECIES
               ...
               IF (SPECIES(NS)%ISFUEL)  &
                  ACTUAL_BURN_RATE(IW) = ACTUAL_BURN_RATE(IW) +                           ML%NU_GAS(J,NS)*MFLUX_S
            ENDDO
...
!         IF (ML%N_REACTIONS.EQ.0 .OR. ANY(ML%NU_RESIDUE.GT.0._EB)) THEN
   ...
   MFLUX = MAX(0._EB,MFLUX - ACTUAL_BURN_RATE(IW)) ! Decrease 
   MATERIAL_LOOP2: DO N=1,SF%N_MATL
      ...
      Z_VECTOR = YY(IIG,JJG,KKG,I_Z_MIN:I_Z_MAX)
      CALL GET_MOLECULAR_WEIGHT(Z_VECTOR,Y_SUM(IIG,JJG,KKG),RSUM_G)
      ! wall values
      ...
      ENDDO            
      Z_VECTOR = YY_W(IW,I_Z_MIN:I_Z_MAX)
      CALL GET_MOLECULAR_WEIGHT(Z_VECTOR,Y_SUM_W,RSUM_W)
      ...
      IF (PPSURF/=PPCLAUS) THEN
         IF (MFLUX==0.AND.PPSURF<PPCLAUS) THEN
            MFLUX = 2.E-5*REACTION(1)%MW_FUEL/ML%ADJUST_BURN_RATE(1,I_FUEL)
            TW(IW)=T
         ELSE
