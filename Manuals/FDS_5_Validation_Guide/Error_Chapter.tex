...
The predictions of any mathematical model are subject to two forms of uncertainty. The first is associated with the uncertainty of the input parameters.
The second is associated with the various assumptions and approximations of the mathematical formulation itself.
...
to the mathematical formulation. The reason for this is two-fold. For one,
...
Our objective here is to outline a process by which to quantify the model error. Our strategy is {\em not} to decompose the model into its many parts and systematically assess
...
A fire model validation study typically consists of comparing point measurements from one or more fire experiments with corresponding model predictions.
...
measurements against which the model predictions are to be compared. This is the subject of the next section.
...
they are normally distributed. Given the complexity of both the model and the experiment, it is hard to justify
...
relative error of the model are both 0.1, or 10~\%. The scatter plot on the left side of Fig.~\ref{xx} displays the model predicted values compared to the
...
values are never known. Instead, the only way to present the data is via the scatter plot on the right side of Fig.~{xx}. Here, the measured values, $E_i$, are
...
We assume the model has a bias factor of 1.05 and its relative error is 0.15 in order to generate a set of hypothetical model predictions.
A graphical representation of the data is shown in Fig.~\ref{scatterplot_1000}. Note that the
...
values we used in creating the hypothetical model predictions. The results of six random trials are shown in Table~\ref{trials}. It is not expected that the
...
surface temperatures by 5~\%. Let us also say that the relative error has been calculated and it is 0.15~. These are the same values that were assumed in the second example
