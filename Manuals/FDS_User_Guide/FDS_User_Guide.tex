...
\subsection{Synthetic Turbulence Inflow Boundary Conditions (Experimental)}
...
The example input files called {\bf couch.fds} and {\bf room\_fire.fds} demonstrate how to model furniture.
...
\begin{description}
\item[{\ct REFERENCE\_ENTHALPHY}] $h(T)=h(T_{ref})+\int_{T_{ref}}^T c_p(T)dT$
