...
\be \frac{(\rho Z)_\alpha^*- \rho^n Z_\alpha^n}{\dt} + \nabla\!\cdot \rho^n \, Z_\alpha^n \, \bu^n = \nabla\!\cdot (\rho^n D_\alpha^n \nabla Z_\alpha^n) + \dot{m}_\alpha'''(\rho^n) \ee
The asterisk denotes a first order accurate estimate at the next time step. Note that the mass source term, $\dot{m}_\alpha'''(\rho^n)$, is computed in the previous time step using the corrected value of the density and species mass fractions.
...
\nabla\!\cdot (\rho^* D_\alpha^* \nabla Z_\alpha^*) + \dot{m}_\alpha'''(\rho^n)
\ee

The background pressure is corrected similarly. Note that the mass source term is the same as that added in the predictor step. It is only computed once per time step.
