...
!print *, DZ_AIR, 'air'
!
!print *, (DZ_FUEL*CPBAR_F_0 + DZ_AIR*CPBAR_G_0)*TMP(I,J,K), 'hs'
!print *, DZ_FUEL*RN%HEAT_OF_COMBUSTION, 'q'
!print *, (DZ_FUEL*CPBAR_F_N + DZ_AIR*CPBAR_G_N)*RN%CRIT_FLAME_TMP, 'hscr'
!print *, EXTINCTION
!pause
...
DO NRR=1,N_REACTIONS ! Calculate total Oxidizer needed to combust all fuel
   ...
   DO NS=0,N_TRACKED_SPECIES
      IF (RN%NU(NS)<-ZERO_P .AND. NS/=RN%FUEL_SMIX_INDEX) &
         MASS_OX_STOICH = MASS_OX_STOICH + PHI_IN(RNN%FUEL_SMIX_INDEX)*RNN%NU(NS)*SPECIES_MIXTURE(NS)%MW/&
         SPECIES_MIXTURE(RNN%FUEL_SMIX_INDEX)%MW
   ENDDO
ENDDO
IF (RNN%FAST_CHEMISTRY) THEN
   IF (MASS_OX_STOICH > MASS_OX) THEN ! Global oxygen limited fast chemistry reaction  
      DZ_F(:) = 1._EB    
