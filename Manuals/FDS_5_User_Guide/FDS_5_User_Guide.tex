...
This section provides a few suggestions to improve the performance of the pressure solver. Normally, these options are not used, but there are certain
applications that might require them.

\subsubsection{Solid Boundary Issues}

At solid surfaces that are not part of the boundary of the computational domain, the pressure solver enforces a no-flux boundary condition. However, it is not perfect, and
it is possible to have a non-zero normal velocity at a solid surface. For most applications, this velocity is so small that it has a negligible effect on the solution. But sometimes, you
might want to reduce this error. To do so, you can specify {\ct VELOCITY\_TOLERANCE} on the {\ct MISC} line to be the maximum allowable normal velocity component on the solid
boundary. It is in units of m/s. If you set this, experiment with different values, and monitor the number of pressure iterations required at each time step to achieve your
...
\subsubsection{Multiple-Mesh Issues}
...
{\ct H\_FIXED} on the {\ct MISC} line in units of W/m$^2$/K. Another option is to use a turbulent
...
Do not specify $A$ and $E$ if you specify {\ct REFERENCE\_TEMPERATURE}.
...
{\ct VELOCITY\_TOLERANCE}       & Real          & See Section~\ref{info:pressurecorrection}     &  m/s          &                   \\ \hline
