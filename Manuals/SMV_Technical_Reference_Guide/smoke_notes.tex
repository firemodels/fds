...
The RTE solution, (\ref{eq:rtesoln}), and the optical depth, (\ref{eq:optdepth}), may be approximated using a Riemann sum as in
\begin{eqnarray*}
\int_a^b f(x)dx \approx \sum_{i=0}^{N-1}f(x_i)\Delta x
\end{eqnarray*}
where $x_i=a+i\Delta x$ and $\Delta x=(b-a)/N$.

Figure \ref{fig:smokediscretesetup}\ illustrates one of possibly many paths through smoke for which a radiance line integral computation is required.  The coordinate system is set up so that the origin, $x_0$, is on right where radiance presumably has its full value.  The observer in this example is located on the left at $x_N$  where radiance is computed and displayed.  
