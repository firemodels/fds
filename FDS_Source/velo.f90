...
USE TURBULENCE, ONLY: VARDEN_DYNSMAG,TEST_FILTER,EX2G3D,WALL_MODEL
USE MATH_FUNCTIONS, ONLY:EVALUATE_RAMP
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
REAL(EB) :: ZZ_GET(0:N_TRACKED_SPECIES),NU_EDDY,DELTA,KSGS,U2,V2,W2,AA,A_IJ(3,3),BB,B_IJ(3,3),&
            DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,MU_DNS,SLIP_COEF,VEL_GAS,VEL_T,RAMP_T,TSI,&
            MU_VELOCITY_ERROR,MU_WALL,DUDN
...
!$OMP        DWDX,DWDY,II,JJ,KK,A_IJ,AA,B_IJ,BB,C_VREMAN,GVEC,IOR,MU_DNS) &
!$OMP PRIVATE(ZZ_GET)
...
!$OMP PRIVATE(IW,II,JJ,KK,IIG,JJG,KKG,DUMMY,ZPLUS,VDF)
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      CASE(SOLID_BOUNDARY)
         ...
         CALL EFFECTIVE_VISCOSITY(MU_WALL,DUDN,WC%U_TAU,WC%Y_PLUS,1._EB/WC%RDN)
         MU_WALL = RHO(IIG,JJG,KKG)*MU_WALL
         MU_VELOCITY_ERROR = RHO(IIG,JJG,KKG)*WC%VELOCITY_ERROR/WC%RDN
         MU(IIG,JJG,KKG) = MAX(MU_DNS,MU_WALL) + MU_VELOCITY_ERROR
         IF (SOLID(CELL_INDEX(II,JJ,KK))) MU(II,JJ,KK) = MU(IIG,JJG,KKG) ! Mirror viscosity into solid
...
SUBROUTINE EFFECTIVE_VISCOSITY(NU_WALL,DUDN,U_TAU,Y_PLUS,DY)

REAL(EB), INTENT(IN) :: U_TAU,Y_PLUS,DY
...
!$OMP PRIVATE(WOMY,VOMZ,RRHO,DVDY,DWDZ,TXXP,TXXM,DTXXDX,DTXYDY,DTXZDZ,VTRM)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=0,IBAR
...
!$OMP PRIVATE(WOMX,UOMZ,RRHO,DUDX,DWDZ,TYYP,TYYM,DTXYDX,DTYYDY,DTYZDZ,VTRM)
DO K=1,KBAR
   DO J=0,JBAR
      DO I=1,IBAR
...
!$OMP PRIVATE(UOMY,VOMX,RRHO,DUDX,DVDY,TZZP,TZZM,DTXZDX,DTYZDY,DTZZDZ,VTRM) 
DO K=0,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
...
MEAN_FORCING_X: IF (MEAN_FORCING(1)) THEN
   ...
   !$OMP END SINGLE
   !$OMP DO COLLAPSE(3) SCHEDULE(STATIC) &
   !$OMP PRIVATE(K,J,I,IC1,IC2,VC) REDUCTION(+:INTEGRAL,SUM_VOLUME)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=0,IBAR
            ...
            ELSE HVAC_IF
               ...
               BOUNDARY_CONDITION: SELECT CASE(VELOCITY_BC_INDEX)
                  ...
                  CASE (WALL_MODEL_BC) BOUNDARY_CONDITION
                     ...
                     CALL EFFECTIVE_VISCOSITY(MU_WALL,DUDN,U_TAU,Y_PLUS,DXX(ICD))
...
EXTERNAL_WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   IF (WC%VENT_INDEX>0) THEN
      ...
      IF (ABS(WC%ONE_D%T-T_BEGIN)<=SPACING(WC%ONE_D%T) .AND. VT%PRESSURE_RAMP_INDEX>=1) THEN
         TSI = T
      ELSE
         TSI = T - T_BEGIN
      ENDIF
