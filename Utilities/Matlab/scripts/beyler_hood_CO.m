...
% plot the results
i1 = strmatch('Exp CO (5 cm)',exp_header);
j1 = strmatch('FDS CO (5 cm)',fds_header);
K(1) = plot(exp_data(:,i1),fds_data(:,j1),'bd','MarkerFaceColor','b'); hold on

i2 = strmatch('Exp CO (-10 cm)',exp_header);
j2 = strmatch('FDS CO (-10 cm)',fds_header);
K(2) = plot(exp_data(:,i2),fds_data(:,j2),'rsq','MarkerFaceColor','r');

i3 = strmatch('Exp CO (0 cm)',exp_header);
j3 = strmatch('FDS CO (0 cm)',fds_header);
...
xlabel('Measured CO Volume Fraction (\%)','Interpreter','LaTeX','FontSize',Label_Font_Size)
ylabel('Predicted CO Volume Fraction (\%)','Interpreter','LaTeX','FontSize',Label_Font_Size)
...
print(gcf,'-dpdf',[repository,'/Manuals/FDS_5_Validation_Guide/FIGURES/Beyler_Hood/Beyler_CO'])
