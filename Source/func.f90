...
IF (MESHES(1)%N_UNIQUE_SLCF>0) THEN
   ...
   DO NM=1,NMESHES
      DO IPROC=1,N_MPI_PROCESSES-1
         IF (MY_RANK==IPROC.AND.IPROC==PROCESS(NM)) THEN
            IF (MY_RANK/=0) THEN
               ALLOCATE(CHARACTER(LEN=MESHES(1)%N_SLCF_O*200*3) :: STRING_BUF)
               DO I=1,MESHES(1)%N_SLCF_O*3
                  STRING_BUF((I-1)*200+1:I*200) = MESHES(1)%ALL_SLICE_QUANTITIES(I)
               ENDDO
               CALL MPI_SEND(STRING_BUF,MESHES(1)%N_SLCF_O*200*3,MPI_CHARACTER,0,PROCESS(NM),MPI_COMM_WORLD,IERR)
               DEALLOCATE(STRING_BUF)
            ENDIF
         ELSEIF (MY_RANK==0) THEN
            IF (PROCESS(NM)==0) CYCLE
            IF (PROCESS(NM)/=IPROC) CYCLE
            ALLOCATE(CHARACTER(MESHES(1)%N_SLCF_O*200*3) :: STRING_BUF)
            CALL MPI_RECV(STRING_BUF,MESHES(1)%N_SLCF_O*200*3,MPI_CHARACTER,IPROC,IPROC,MPI_COMM_WORLD,MPISTATUS,IERR)
            QUANTITY_LOOP: DO I=1,MESHES(1)%N_SLCF_O*3
               ALL_SLICE_QUANTITIES(I) = STRING_BUF((I-1)*200+1:I*200)
