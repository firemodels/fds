...
PUBLIC VERIFY_TETRABOX,GET_TETRABOX_VOLUME, GET_VERTS
...
END DO
V_CENTER(0:2) = V_CENTER(0:2)/NVERTS
...
DO I = 0, NFACES-1
   ...
   IBEG0 = FACESTART(I)
   IBEG1 = FACESTART(I)+1
   V0(0:2) => VERTS(3*IBEG0:3*IBEG0+2)
   V1(0:2) => VERTS(3*IBEG1:3*IBEG1+2)
   DO J = FACESTART(I)+2, FACESTART(I+1)-1
      V2(0:2) => VERTS(3*J:3*J+2)
...
DO F = 0, NFACES-1
   IF (FACENUM(F).GT.0) THEN
      WRITE(6,*)"   ",F+1
      DO V = FACESTART(F),FACESTART(F+1)-1
         WRITE(6,*)"    ",VERTS(3*V:3*V+2)
      END DO
   ENDIF
END DO
...
! determine orientation of vertices A, B, C
...
REAL(EB), DIMENSION(:), POINTER :: VV0, VV1, VV2
REAL(EB), DIMENSION(0:299), TARGET :: VERTS_EB
REAL(EB) :: VOL
INTEGER :: I,J, IBEG0, IBEG1
