...
IF (ANY(BAND_LIMITS>0._EB)) THEN
   ...
   IF (NUMBER_SPECTRAL_BANDS>1 .AND. .NOT.WIDE_BAND_MODEL) THEN ; CALL SHUTDOWN('ERROR: Set WIDE_BAND_MODEL=.TRUE. if defining band limits.') ; RETURN ; ENDIF
...
INTEGER :: N,NM,MESH_NUMBER,NN,N_PROFO,IOR,FORMAT_INDEX,I_DUM(10)
REAL(EB) :: XYZ(3)
CHARACTER(LABEL_LENGTH) :: QUANTITY,INIT_ID,STR_DUM1,STR_DUM2,SPEC_ID
LOGICAL :: CELL_CENTERED
TYPE (PROFILE_TYPE), POINTER :: PF
NAMELIST /PROF/ CELL_CENTERED,FORMAT_INDEX,FYI,ID,INIT_ID,IOR,QUANTITY,XYZ,SPEC_ID
...
PROF_LOOP: DO NN=1,N_PROFO
   ...
   ! Get species indices
   IF (SPEC_ID/='null') THEN
      CALL GET_QUANTITY_INDEX(STR_DUM1,STR_DUM2,I_DUM(1),I_DUM(2), &
           I_DUM(3),I_DUM(4),I_DUM(5),I_DUM(6),I_DUM(7),I_DUM(8),I_DUM(9),'PROF', &
           QUANTITY,'null',SPEC_ID,'null','null','null','null','null',-1._EB,I_DUM(10))
   ENDIF
   ...
   PF%Y_INDEX      = I_DUM(3)
   PF%Z_INDEX      = I_DUM(4)
   !write(*,*) FORMAT_INDEX,NN,MESH_NUMBER,ID,QUANTITY,PF%Y_INDEX,PF%Z_INDEX
   ...
   QUANTITY_IF:IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
      ...
      SELECT CASE (TRIM(OUTTYPE))
         ...
         CASE ('BNDF')
            ...
            IF (QUANTITY=='AMPUA' .OR. QUANTITY=='AMPUA_Z') ACCUMULATE_WATER = .TRUE.
         CASE('PROF')
            IF (.NOT. OUTPUT_QUANTITY(ND)%PROF_APPROPRIATE) THEN
               WRITE(MESSAGE,'(3A)') 'ERROR: ',TRIM(QUANTITY),' is not a profile output QUANTITY'
               CALL SHUTDOWN(MESSAGE) ; RETURN
            ENDIF
         CASE('ISOF')
