...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (XX>=M2%XS .AND. XX<=M2%XF .AND.  YY>=M2%YS .AND. YY<=M2%YF .AND. ZZ>=M2%ZS .AND. ZZ<=M2%ZF) THEN
      IF (ALLOCATED(M2%CELLSI)) THEN
         XI  = MAX( 1._EB , MIN( REAL(M2%IBAR,EB)+ALMOST_ONE_EB , M2%CELLSI(FLOOR((XX-M2%XS)*M2%RDXINT,EB)) + 1._EB ) )
         YJ  = MAX( 1._EB , MIN( REAL(M2%JBAR,EB)+ALMOST_ONE_EB , M2%CELLSJ(FLOOR((YY-M2%YS)*M2%RDYINT,EB)) + 1._EB ) )
         ZK  = MAX( 1._EB , MIN( REAL(M2%KBAR,EB)+ALMOST_ONE_EB , M2%CELLSK(FLOOR((ZZ-M2%ZS)*M2%RDZINT,EB)) + 1._EB ) )
         IIO = FLOOR(XI,EB)
         JJO = FLOOR(YJ,EB)
         KKO = FLOOR(ZK,EB)
...
XI  = MAX( 1._EB , MIN( REAL(M%IBAR,EB)+ALMOST_ONE_EB , M%CELLSI(NINT((XX-M%XS)*M%RDXINT,EB)) + 1._EB ) )
YJ  = MAX( 1._EB , MIN( REAL(M%JBAR,EB)+ALMOST_ONE_EB , M%CELLSJ(NINT((YY-M%YS)*M%RDYINT,EB)) + 1._EB ) )
ZK  = MAX( 1._EB , MIN( REAL(M%KBAR,EB)+ALMOST_ONE_EB , M%CELLSK(NINT((ZZ-M%ZS)*M%RDZINT,EB)) + 1._EB ) )
II  = FLOOR(XI,EB)
JJ  = FLOOR(YJ,EB)
KK  = FLOOR(ZK,EB)
...
XI = MESHES(NM)%CELLSI(  MIN( MESHES(NM)%CELLSI_HI, MAX(MESHES(NM)%CELLSI_LO,FLOOR((X-MESHES(NM)%XS)*MESHES(NM)%RDXINT,EB)) )  )
YJ = MESHES(NM)%CELLSJ(  MIN( MESHES(NM)%CELLSJ_HI, MAX(MESHES(NM)%CELLSJ_LO,FLOOR((Y-MESHES(NM)%YS)*MESHES(NM)%RDYINT,EB)) )  )
ZK = MESHES(NM)%CELLSK(  MIN( MESHES(NM)%CELLSK_HI, MAX(MESHES(NM)%CELLSK_LO,FLOOR((Z-MESHES(NM)%ZS)*MESHES(NM)%RDZINT,EB)) )  )
I = FLOOR(XI+1._EB,EB)
J = FLOOR(YJ+1._EB,EB)
K = FLOOR(ZK+1._EB,EB)
