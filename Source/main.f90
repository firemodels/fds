...
IF (OMESH_PARTICLES) THEN
   ...
   NEIGHBORING_MESH_LOOP: DO N=1,M%N_NEIGHBORING_MESHES
      ...
      ALLOCATE(OM%ORPHAN_PARTICLE_STORAGE%REALS(100))
      ALLOCATE(OM%ORPHAN_PARTICLE_STORAGE%INTEGERS(100))
      ALLOCATE(OM%ORPHAN_PARTICLE_STORAGE%LOGICALS(100))
      ALLOCATE(OM%ADOPT_PARTICLE_STORAGE%REALS(100))
      ALLOCATE(OM%ADOPT_PARTICLE_STORAGE%INTEGERS(100))
      ALLOCATE(OM%ADOPT_PARTICLE_STORAGE%LOGICALS(100))
...
INTEGER :: NOM,N_REALS,N_INTEGERS,N_LOGICALS,IW
TYPE(WALL_TYPE), POINTER :: WC
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   DO IW=1,M%N_WALL_CELLS
      WC => M%WALL(IW)
      N_REALS = MAX(N_REALS,WC%N_REALS)
      N_INTEGERS = MAX(N_INTEGERS,WC%N_INTEGERS)
      N_LOGICALS = MAX(N_LOGICALS,WC%N_LOGICALS)
   ENDDO
   ALLOCATE(M%WALL_STORAGE%REALS(N_REALS))
   ALLOCATE(M%WALL_STORAGE%INTEGERS(N_INTEGERS))
   ALLOCATE(M%WALL_STORAGE%LOGICALS(N_LOGICALS))
ENDDO
...
MESH_LOOP_1A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_1B: DO NOM=1,NMESHES
      ...
      M3 => M%OMESH(NOM)
      IF (M3%N_WALL_CELLS_SEND==0) CYCLE MESH_LOOP_1B
      N_REALS = 0
      N_INTEGERS = 0
      N_LOGICALS = 0
      ALLOCATE(M3%WALL_REAL_NUMBERS_SEND(M3%N_WALL_CELLS_SEND))
      ALLOCATE(M3%WALL_INTE_NUMBERS_SEND(M3%N_WALL_CELLS_SEND))
      ALLOCATE(M3%WALL_LOGI_NUMBERS_SEND(M3%N_WALL_CELLS_SEND))
      DO I=1,M3%N_WALL_CELLS_SEND
         WC => M%WALL(M3%WALL_CELL_INDICES_SEND(I))
         M3%WALL_REAL_NUMBERS_SEND(I) = WC%N_REALS
         M3%WALL_INTE_NUMBERS_SEND(I) = WC%N_INTEGERS
         M3%WALL_LOGI_NUMBERS_SEND(I) = WC%N_LOGICALS
         N_REALS    = N_REALS    + M3%WALL_REAL_NUMBERS_SEND(I)
         N_INTEGERS = N_INTEGERS + M3%WALL_INTE_NUMBERS_SEND(I)
         N_LOGICALS = N_LOGICALS + M3%WALL_LOGI_NUMBERS_SEND(I)
      ENDDO
      ALLOCATE(M3%WALL_STORAGE_SEND%REALS(N_REALS))
      ALLOCATE(M3%WALL_STORAGE_SEND%INTEGERS(N_INTEGERS))
      ALLOCATE(M3%WALL_STORAGE_SEND%LOGICALS(N_LOGICALS))
   ENDDO MESH_LOOP_1B
ENDDO MESH_LOOP_1A
...
MESH_LOOP_3A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_3B: DO NOM=1,NMESHES
      ...
      M3 => M%OMESH(NOM)
      IF (M3%N_THIN_WALL_CELLS_SEND==0) CYCLE MESH_LOOP_3B
      N_REALS = 0
      N_INTEGERS = 0
      N_LOGICALS = 0
      ALLOCATE(M3%THIN_WALL_REAL_NUMBERS_SEND(M3%N_THIN_WALL_CELLS_SEND))
      ALLOCATE(M3%THIN_WALL_INTE_NUMBERS_SEND(M3%N_THIN_WALL_CELLS_SEND))
      ALLOCATE(M3%THIN_WALL_LOGI_NUMBERS_SEND(M3%N_THIN_WALL_CELLS_SEND))
      DO I=1,M3%N_THIN_WALL_CELLS_SEND
         TW => M%THIN_WALL(M3%THIN_WALL_CELL_INDICES_SEND(I))
         M3%THIN_WALL_REAL_NUMBERS_SEND(I) = TW%N_REALS
         M3%THIN_WALL_INTE_NUMBERS_SEND(I) = TW%N_INTEGERS
         M3%THIN_WALL_LOGI_NUMBERS_SEND(I) = TW%N_LOGICALS
         N_REALS    = N_REALS    + M3%THIN_WALL_REAL_NUMBERS_SEND(I)
         N_INTEGERS = N_INTEGERS + M3%THIN_WALL_INTE_NUMBERS_SEND(I)
         N_LOGICALS = N_LOGICALS + M3%THIN_WALL_LOGI_NUMBERS_SEND(I)
      ENDDO
      ALLOCATE(M3%THIN_WALL_STORAGE_SEND%REALS(N_REALS))
      ALLOCATE(M3%THIN_WALL_STORAGE_SEND%INTEGERS(N_INTEGERS))
      ALLOCATE(M3%THIN_WALL_STORAGE_SEND%LOGICALS(N_LOGICALS))
   ENDDO MESH_LOOP_3B
ENDDO MESH_LOOP_3A

! Mesh NM sends M(NM)%OM(NOM)%WALL_REAL_NUMBERS_SEND, etc, to M(NOM)%OM(NM)%WALL_REAL_NUMBERS_RECV
...
MESH_LOOP_2A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_2B: DO NOM=1,NMESHES
      ...
      M3 => M%OMESH(NOM)
      IF (M3%N_WALL_CELLS_RECV==0) CYCLE MESH_LOOP_2B
      CALL ALLOCATE_BOUNDARY_TYPES(NOM,50)
      N_REALS = 0
      N_INTEGERS = 0
      N_LOGICALS = 0
      DO I=1,M3%N_WALL_CELLS_RECV
         N_REALS    = N_REALS    + M3%WALL_REAL_NUMBERS_RECV(I)
         N_INTEGERS = N_INTEGERS + M3%WALL_INTE_NUMBERS_RECV(I)
         N_LOGICALS = N_LOGICALS + M3%WALL_LOGI_NUMBERS_RECV(I)
      ENDDO
      ALLOCATE(M3%WALL_STORAGE_RECV%REALS(N_REALS))
      ALLOCATE(M3%WALL_STORAGE_RECV%INTEGERS(N_INTEGERS))
      ALLOCATE(M3%WALL_STORAGE_RECV%LOGICALS(N_LOGICALS))
      DO I=1,M3%N_WALL_CELLS_RECV
         CALL ALLOCATE_STORAGE(NOM,WALL_INDEX=I+M3%N_WALL_CELLS_RECV_START-1,SURF_INDEX=M3%WALL_SURF_INDICES_RECV(I))
      ENDDO
   ENDDO MESH_LOOP_2B
ENDDO MESH_LOOP_2A
...
MESH_LOOP_4A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_4B: DO NOM=1,NMESHES
      ...
      M3 => M%OMESH(NOM)
      IF (M3%N_THIN_WALL_CELLS_RECV==0) CYCLE MESH_LOOP_4B
      CALL ALLOCATE_BOUNDARY_TYPES(NOM,50)
      N_REALS = 0
      N_INTEGERS = 0
      N_LOGICALS = 0
      DO I=1,M3%N_THIN_WALL_CELLS_RECV
         N_REALS    = N_REALS    + M3%THIN_WALL_REAL_NUMBERS_RECV(I)
         N_INTEGERS = N_INTEGERS + M3%THIN_WALL_INTE_NUMBERS_RECV(I)
         N_LOGICALS = N_LOGICALS + M3%THIN_WALL_LOGI_NUMBERS_RECV(I)
      ENDDO
      ALLOCATE(M3%THIN_WALL_STORAGE_RECV%REALS(N_REALS))
      ALLOCATE(M3%THIN_WALL_STORAGE_RECV%INTEGERS(N_INTEGERS))
      ALLOCATE(M3%THIN_WALL_STORAGE_RECV%LOGICALS(N_LOGICALS))
      DO I=1,M3%N_THIN_WALL_CELLS_RECV
         CALL ALLOCATE_STORAGE(NOM,THIN_WALL_INDEX=I+M3%N_THIN_WALL_CELLS_RECV_START-1,SURF_INDEX=M3%THIN_WALL_SURF_INDICES_RECV(I))
      ENDDO
   ENDDO MESH_LOOP_4B
ENDDO MESH_LOOP_4A
...
RECEIVING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SENDING_MESH_LOOP: DO NN=1,M%N_NEIGHBORING_MESHES
      ...
      INITIALIZATION_IF: IF (CODE==0) THEN
         ...
         IF (OMESH_PARTICLES) THEN
            OS => M3%ADOPT_PARTICLE_STORAGE
            N_REQ2=N_REQ2+1 ; CALL MPI_RECV_INIT(M3%N_PART_ADOPT,1,MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ2(N_REQ2),IERR)
      ...
      IF (CODE==19 .AND. M3%N_WALL_CELLS_RECV>0) THEN
         ALLOCATE(M3%WALL_REAL_NUMBERS_RECV(M3%N_WALL_CELLS_RECV))
         ALLOCATE(M3%WALL_INTE_NUMBERS_RECV(M3%N_WALL_CELLS_RECV))
         ALLOCATE(M3%WALL_LOGI_NUMBERS_RECV(M3%N_WALL_CELLS_RECV))
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%WALL_REAL_NUMBERS_RECV(1),SIZE(M3%WALL_REAL_NUMBERS_RECV),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%WALL_INTE_NUMBERS_RECV(1),SIZE(M3%WALL_INTE_NUMBERS_RECV),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%WALL_LOGI_NUMBERS_RECV(1),SIZE(M3%WALL_LOGI_NUMBERS_RECV),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==19 .AND. M3%N_THIN_WALL_CELLS_RECV>0) THEN
         ALLOCATE(M3%THIN_WALL_REAL_NUMBERS_RECV(M3%N_THIN_WALL_CELLS_RECV))
         ALLOCATE(M3%THIN_WALL_INTE_NUMBERS_RECV(M3%N_THIN_WALL_CELLS_RECV))
         ALLOCATE(M3%THIN_WALL_LOGI_NUMBERS_RECV(M3%N_THIN_WALL_CELLS_RECV))
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%THIN_WALL_REAL_NUMBERS_RECV(1),SIZE(M3%THIN_WALL_REAL_NUMBERS_RECV),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%THIN_WALL_INTE_NUMBERS_RECV(1),SIZE(M3%THIN_WALL_INTE_NUMBERS_RECV),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%THIN_WALL_LOGI_NUMBERS_RECV(1),SIZE(M3%THIN_WALL_LOGI_NUMBERS_RECV),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==10 .AND. M3%N_WALL_CELLS_RECV>0) THEN
         IF (.NOT.ALLOCATED(M3%WALL_STORAGE_RECV%REALS)) CYCLE
         N_REQ6 = N_REQ6 + 1
         CALL MPI_RECV_INIT(M3%WALL_STORAGE_RECV%REALS,SIZE(M3%WALL_STORAGE_RECV%REALS),&
                            MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_RECV_INIT(M3%WALL_STORAGE_RECV%INTEGERS,SIZE(M3%WALL_STORAGE_RECV%INTEGERS),&
                            MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_RECV_INIT(M3%WALL_STORAGE_RECV%LOGICALS,SIZE(M3%WALL_STORAGE_RECV%LOGICALS),&
                            MPI_LOGICAL,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
      ENDIF

      IF (CODE==10 .AND. M3%N_THIN_WALL_CELLS_RECV>0) THEN
         IF (.NOT.ALLOCATED(M3%THIN_WALL_STORAGE_RECV%REALS)) CYCLE
         N_REQ6 = N_REQ6 + 1
         CALL MPI_RECV_INIT(M3%THIN_WALL_STORAGE_RECV%REALS,SIZE(M3%THIN_WALL_STORAGE_RECV%REALS),&
                            MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_RECV_INIT(M3%THIN_WALL_STORAGE_RECV%INTEGERS,SIZE(M3%THIN_WALL_STORAGE_RECV%INTEGERS),&
                            MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_RECV_INIT(M3%THIN_WALL_STORAGE_RECV%LOGICALS,SIZE(M3%THIN_WALL_STORAGE_RECV%LOGICALS),&
                            MPI_LOGICAL,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
      ENDIF

      ! PARTICLEs

      IF (CODE==11 .AND. OMESH_PARTICLES .AND. M3%N_PART_ADOPT>0) THEN
         OS => M3%ADOPT_PARTICLE_STORAGE
...
SENDING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      INITIALIZE_SEND_IF: IF (CODE==0) THEN
         ...
         IF (OMESH_PARTICLES) THEN
            OS => M3%ORPHAN_PARTICLE_STORAGE
            N_REQ2=N_REQ2+1 ; CALL MPI_SEND_INIT(M3%N_PART_ORPHANS,1,MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ2(N_REQ2),IERR)
      ...
      IF (CODE==19 .AND. M3%N_WALL_CELLS_SEND>0) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%WALL_REAL_NUMBERS_SEND,SIZE(M3%WALL_REAL_NUMBERS_SEND),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%WALL_INTE_NUMBERS_SEND,SIZE(M3%WALL_INTE_NUMBERS_SEND),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%WALL_LOGI_NUMBERS_SEND,SIZE(M3%WALL_LOGI_NUMBERS_SEND),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
            M2%WALL_REAL_NUMBERS_RECV = M3%WALL_REAL_NUMBERS_SEND
            M2%WALL_INTE_NUMBERS_RECV = M3%WALL_INTE_NUMBERS_SEND
            M2%WALL_LOGI_NUMBERS_RECV = M3%WALL_LOGI_NUMBERS_SEND
         ENDIF
      ENDIF

      IF (CODE==19 .AND. M3%N_THIN_WALL_CELLS_SEND>0) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%THIN_WALL_REAL_NUMBERS_SEND,SIZE(M3%THIN_WALL_REAL_NUMBERS_SEND),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%THIN_WALL_INTE_NUMBERS_SEND,SIZE(M3%THIN_WALL_INTE_NUMBERS_SEND),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%THIN_WALL_LOGI_NUMBERS_SEND,SIZE(M3%THIN_WALL_LOGI_NUMBERS_SEND),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
            M2%THIN_WALL_REAL_NUMBERS_RECV = M3%THIN_WALL_REAL_NUMBERS_SEND
            M2%THIN_WALL_INTE_NUMBERS_RECV = M3%THIN_WALL_INTE_NUMBERS_SEND
            M2%THIN_WALL_LOGI_NUMBERS_RECV = M3%THIN_WALL_LOGI_NUMBERS_SEND
         ENDIF
      ENDIF

      IF (CODE==10 .AND. M3%N_WALL_CELLS_SEND>0 .AND. RNODE/=SNODE) THEN
         IF (.NOT.ALLOCATED(M3%WALL_STORAGE_SEND%REALS)) CYCLE
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%WALL_STORAGE_SEND%REALS,SIZE(M3%WALL_STORAGE_SEND%REALS),&
                            MPI_DOUBLE_PRECISION,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%WALL_STORAGE_SEND%INTEGERS,SIZE(M3%WALL_STORAGE_SEND%INTEGERS),&
                            MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%WALL_STORAGE_SEND%LOGICALS,SIZE(M3%WALL_STORAGE_SEND%LOGICALS),&
                            MPI_LOGICAL,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
      ENDIF

      IF (CODE==10 .AND. M3%N_THIN_WALL_CELLS_SEND>0 .AND. RNODE/=SNODE) THEN
         IF (.NOT.ALLOCATED(M3%THIN_WALL_STORAGE_SEND%REALS)) CYCLE
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%THIN_WALL_STORAGE_SEND%REALS,SIZE(M3%THIN_WALL_STORAGE_SEND%REALS),&
                            MPI_DOUBLE_PRECISION,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%THIN_WALL_STORAGE_SEND%INTEGERS,SIZE(M3%THIN_WALL_STORAGE_SEND%INTEGERS),&
                            MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%THIN_WALL_STORAGE_SEND%LOGICALS,SIZE(M3%THIN_WALL_STORAGE_SEND%LOGICALS),&
                            MPI_LOGICAL,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
      ENDIF
      ...
      IF (CODE==6 .AND. RNODE/=SNODE .AND. M3%N_WALL_CELLS_SEND>0) THEN
         RC=0 ; IC=0 ; LC=0
         OS => M3%WALL_STORAGE_SEND
         DO I=1,M3%N_WALL_CELLS_SEND
            IW = M3%WALL_CELL_INDICES_SEND(I)
            WC => M%WALL(IW)
            CALL PACK_WALL(NM,OS,WC,M3%WALL_SURF_INDICES_SEND(I),RC,IC,LC,UNPACK_IT=.FALSE.,COUNT_ONLY=.FALSE.)
         ENDDO
      ENDIF

      IF (CODE==6 .AND. RNODE/=SNODE .AND. M3%N_THIN_WALL_CELLS_SEND>0) THEN
         RC=0 ; IC=0
         OS => M3%THIN_WALL_STORAGE_SEND
         DO I=1,M3%N_THIN_WALL_CELLS_SEND
            IW = M3%THIN_WALL_CELL_INDICES_SEND(I)
            TW => M%THIN_WALL(IW)
            CALL PACK_THIN_WALL(NM,OS,TW,M3%THIN_WALL_SURF_INDICES_SEND(I),RC,IC,UNPACK_IT=.FALSE.,COUNT_ONLY=.FALSE.)
         ENDDO
      ENDIF
      ...
      IF (CODE==7 .AND. OMESH_PARTICLES .AND. RNODE==SNODE) THEN
         M2=>MESHES(NOM)%OMESH(NM)
         OS=>M2%ADOPT_PARTICLE_STORAGE
         M2%N_PART_ADOPT = M3%N_PART_ORPHANS
         OS%N_REALS      = M3%ORPHAN_PARTICLE_STORAGE%N_REALS
         OS%N_INTEGERS   = M3%ORPHAN_PARTICLE_STORAGE%N_INTEGERS
         OS%N_LOGICALS   = M3%ORPHAN_PARTICLE_STORAGE%N_LOGICALS
      ENDIF

      ! Sending/Receiving PARTICLE Buffer Arrays

      IF_SEND_PARTICLES: IF (CODE==11 .AND. OMESH_PARTICLES .AND. M3%N_PART_ORPHANS>0) THEN
         OS => M3%ORPHAN_PARTICLE_STORAGE
         IF (SNODE/=RNODE) THEN
         ...
         ELSE
            M2 => MESHES(NOM)%OMESH(NM)
            M2%ADOPT_PARTICLE_STORAGE%REALS    = OS%REALS
            M2%ADOPT_PARTICLE_STORAGE%INTEGERS = OS%INTEGERS
            M2%ADOPT_PARTICLE_STORAGE%LOGICALS = OS%LOGICALS
         ENDIF
         M3%N_PART_ORPHANS = 0
         OS%N_REALS = 0
         OS%N_INTEGERS = 0
...
RECV_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      RECEIVE_BACK_WALL: IF (CODE==6 .AND. SNODE/=RNODE .AND. M2%N_WALL_CELLS_RECV>0) THEN
         RC=0 ; IC=0 ; LC=0
         OS => M2%WALL_STORAGE_RECV
         DO I=1,M2%N_WALL_CELLS_RECV
            IW = I+M2%N_WALL_CELLS_RECV_START-1
            WC => MESHES(NOM)%WALL(IW)
            CALL PACK_WALL(NOM,OS,WC,M2%WALL_SURF_INDICES_RECV(I),RC,IC,LC,UNPACK_IT=.TRUE.,COUNT_ONLY=.FALSE.)
         ENDDO
      ENDIF RECEIVE_BACK_WALL

      RECEIVE_BACK_THIN_WALL: IF (CODE==6 .AND. SNODE/=RNODE .AND. M2%N_THIN_WALL_CELLS_RECV>0) THEN
         RC=0 ; IC=0
         OS => M2%THIN_WALL_STORAGE_RECV
         DO I=1,M2%N_THIN_WALL_CELLS_RECV
            IW = I+M2%N_THIN_WALL_CELLS_RECV_START-1
            TW => MESHES(NOM)%THIN_WALL(IW)
            CALL PACK_THIN_WALL(NOM,OS,TW,M2%THIN_WALL_SURF_INDICES_RECV(I),RC,IC,UNPACK_IT=.TRUE.,COUNT_ONLY=.FALSE.)
         ENDDO
      ENDIF RECEIVE_BACK_THIN_WALL

      ! Sending/Receiving PARTICLE Buffer Arrays

      IF (CODE==7 .AND. OMESH_PARTICLES .AND. M2%N_PART_ADOPT>0) THEN
         OS => M2%ADOPT_PARTICLE_STORAGE
      ...
      IF_RECEIVE_PARTICLES: IF (CODE==11 .AND. OMESH_PARTICLES .AND. M2%N_PART_ADOPT>0) THEN
         RC=0 ; IC=0 ; LC=0
         OS => M2%ADOPT_PARTICLE_STORAGE
         DO IP=M%NLP+1,M%NLP+M2%N_PART_ADOPT
