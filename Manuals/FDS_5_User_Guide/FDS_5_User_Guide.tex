...
%
% ---------- section documenting device visualization --------------------------
%
...
For any user defined {\ct RAMP}, the normal independent variable, for example time for {\ct RAMP\_V}, can be replaced by the output of a {\ct DEVC}.  This is done by specifying the input {\ct DEVC\_ID} one on of the {\ct RAMP} input lines.  In the following example a blower is turned on when the temperature exceeds 100~$^\circ$C and is turned off when the temperature exceeds 200~$^\circ$C.  This is similar functionality to the {\ct CUSTOM} control function, but it allows for variable response rather than just on or off.
