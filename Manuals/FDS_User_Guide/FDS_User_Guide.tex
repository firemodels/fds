...
\vskip-\baselineskip
\begin{tabular}{ll}
{\ct 'CONSTANT SMAGORINSKY'}   & constant coefficient Smagorinsky model \cite{Smagorinsky:1}, additionally specify {\ct CSMAG}\\
{\ct 'DYNAMIC SMAGORINSKY'}   & dynamic Smagorinsky model \cite{Germano:1991,Moin:1991}\\
{\ct 'DEARDORFF'} & Deardorff model \cite{Deardorff:1980,Pope:2000} (default) \\
{\ct 'VREMAN'}    & Vreman's eddy viscosity model \cite{Vreman:2004}
\end{tabular}
\end{center}
...
\begin{warning}
\noindent
To return to the non-dynamic form of LES used in FDS versions 1-5, set {\ct TURBULENCE\_MODEL='CONSMAG'} on the {\ct MISC} line. You can change the empirical constant $C_s$ via {\ct CSMAG}. Its default value is 0.2.
\end{warning}
...
! static geometry
  ...
  WRITE(LU_GEOM) N_VERT
  IF (N_VERT>0) WRITE(LU_GEOM) (Xvert(I),Yvert(I),Zvert(I)),I=1,N_VERT)
  WRITE(LU_GEOM(N)) N_FACE
  IF (N_FACE>0) WRITE(LU_GEOM) (FACEX(I),FACEY(I),FACEZ(I)),I=1,N_FACE)
...
! dynamic geometry
  ...
  IF(GEOM_TYPE.EQ.0)THEN
    ...
    WRITE(LU_GEOM(N)) N_FACE
    IF (N_FACE>0) WRITE(LU_GEOM) (FACEX(I),FACEY(I),FACEZ(I)),I=1,N_FACE)
  ELSE IF(GEOM_TYPE.EQ.1.OR.GEOM_TYPE.EQ.2)THEN
    WRITE(LU_GEOM) Xtran, Ytran, Ztran, rot_az, rot_elev
  ENDIF
              .
              .
              .
...
{\ct VERSION}\ currently has value 0 and indicates the version number of this file format.
{\ct STIME} is the time.
{\ct N\_VERT}\ is the number of vertices.  
{\ct Xvert, Yvert, Zvert}\ are the vertex coordinates.
{\ct N\_FACE}\ is the number of faces.
{\ct FACEX, FACEY, FACEY}\ are the vertex numbers for each face (triangle).
...
If {\ct GEOM\_TYPE}\ is 1 then time dependent geometry is written out using only a translation
and two rotations.  These transformations are applied to the static geometry defined 
at the beginning of the file.
If {\ct GEOM\_TYPE}\ is 2 then time dependent geometry is again written out using only a translation
and two rotations.  These transformations are applied to the initial dynamic geometry defined
at the beginning of the file.
{\ct Xtran, Ytran, Ztran}\ is the translation applied to the static geometry.
{\ct rot\_az, rot\_elev}\ is the azimuthal and elevation rotations applied to the static geometry.
...
\begin{verbatim}
  ...
  WRITE(LU_GEOM_DATA) N_VERT_STATIC,N_FACE_STATIC
  IF (N_VERT_STATIC>0) WRITE(LU_GEOM_DATA) (ValVertStatic(I)),I=1,N_VERT_STATIC)
  IF (N_FACE_STATIC>0) WRITE(LU_GEOM_DATA) (ValFaceStatic(I)),I=1,N_VERT_STATIC)
  WRITE(LU_GEOM_DATA) N_VERT_DYNAMIC,NFACE_DYNAMIC
  IF (N_VERT_DYNAMIC>0) WRITE(LU_GEOM_DATA) (ValVertDynamic(I)),I=1,N_VERT_DYNAMIC)
  IF (N_FACE_DYNAMIC>0) WRITE(LU_GEOM_DATA) (ValFaceDynamic(I)),I=1,N_FACE_DYNAMIC)
              .
              .
              .
\end{verbatim}
...
{\ct VERSION}\ currently has value 0 and indicates the version number of this file format.
{\ct STIME} is the time.
{\ct N\_VERT\_STATIC, N\_FACE\_STATIC}\ is the number of static vertices and faces.  One can write out
data values located at nodes, located at the center of faces or both.
{\ct N\_VERT\_DYNAMIC, N\_FACE\_DYNAMIC}\ is the number of dynamic vertices and faces.  One can write out
data values located at nodes, located at the center of faces, both or neither (if there is no dynamic geometry).
{\ct ValVertStatic, ValFaceStatic}\ static vertex and face data.
{\ct ValVertDynamic, ValFaceDynamic}\ dynamic
