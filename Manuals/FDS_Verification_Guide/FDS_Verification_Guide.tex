...
\item[(Relatively) Opaque] particles with refractive index $\hbox{m} = 1.5+0.5 \,i$.
\end{description}
In addition, the different methods to specify the refractive index are tested:
For water and fuel, the FDS internal tables are used. For transparent droplets,
constant values are used. For opaque droplets, a radiative property table is used.
In each case, 10~g of non-evaporating, monodisperse, 100~$\mu$m particles with 1,000~kg/m$^3$ density are placed inside the cube.  The resulting heat
...
\ee
where $C_0$ is the initial gas-phase soot concentration (100~mg/m$^3$). In this example, the relevant surface area is the area of the lower surface where the soot deposits, and thus the surface area to volume ratio ($A/V$) is equal to \SI{1}{1/m}. For a soot particle with a diameter of 1~$\mu$m, the gravitational settling velocity is equal to 1.9747$\times 10^{-4}$~m/s. The governing equations for the gravitational settling velocity are discussed in more detail in the FDS Technical Reference Guide~\cite{FDS_Math_Guide}. Figure~\ref{fig:soot_gravitational_settling} shows a comparison of the analytical and simulated results for the time evolution of the gas-phase soot concentration.
