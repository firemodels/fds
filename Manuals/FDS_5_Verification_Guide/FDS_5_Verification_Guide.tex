...
%\item[Integrated MPUV] is the particle mass per unit volume integrated over the volume of the computational domain.
%For charring materials it should decrease from the initial value (twice the final value) to the analytical mass. For non-charring materials, it should
%decrease from the analytical value to zero.
\end{description}
...
%\subsection{SURF associated with PART}
%
%The analytical masses are obtained by multiplying the corresponding values of $m''$ by the particle area.
%For the cartesian surfaces, the particle area is two times the product of parameters {\ct LENGHT} and {\ct WIDTH} of the {\ct SURF} line, both of
%which were given a value 0.05 m. As a result, the analytical masses for particles with associated cartesian surfaces are
%$m_{char}$ = 1.8 $\cdot 2\cdot 0.05^2$ kg = 0.0090 kg, and $m''_{nchar}$ = 0.018 kg.
%
%With associated cylindrical surfaces, the particle area is determined by the {\ct LENGTH} parameter, which was given a value 0.1 m, and the surface
%{\ct THICKNESS} = 0.01 m. The analytical masses are thus
%$m_{char}$ = 0.9 $\cdot 2\pi\cdot 0.01\cdot 0.1$ kg = 0.00565 kg, and $m''_{nchar}$ = 0.0113 kg.
%
%With associated spherical surfaces, the particle area is $4\pi\cdot$ {\ct THICKNESS}$^2$.
%The analytical masses are thus
%$m_{char}$ = 0.6 $\cdot 4\pi\cdot 0.01^2$ kg = 7.54$\cdot10^{-4}$ kg, and $m''_{nchar}$ = 1.51$\cdot10^{-3}$ kg.
%
