...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            TMP_FRONT,TMP_INNER(MAX_LAYERS),THICKNESS(MAX_LAYERS),VEL,SPECIFIED_SOURCE_TERM(MAX_LAYERS),&
...
NAMELIST /SURF/ ADIABATIC,BACKING,BURN_AWAY,CELL_SIZE_FACTOR,C_FORCED_CONSTANT,C_FORCED_PR_EXP,C_FORCED_RE,C_FORCED_RE_EXP,&
                ...
                LEAK_PATH,LENGTH,MASS_FLUX,MASS_FLUX_TOTAL,MASS_FLUX_VAR,MASS_FRACTION,MATL_ID,MATL_MASS_FRACTION,&
                MINIMUM_LAYER_THICKNESS,MLRPUA,N_CELLS_MAX,N_LAYER_CELLS_MAX,NET_HEAT_FLUX,&
                ...
                RGB,ROUGHNESS,SPEC_ID,SPREAD_RATE,STRETCH_FACTOR,TGA_ANALYSIS,TGA_FINAL_TEMPERATURE,TGA_HEATING_RATE,&
                ...
                VEL,VEL_GRAD,VEL_T,VOLUME_FLOW,WIDTH,XYZ,Z0,SPECIFIED_SOURCE_TERM,&
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   IF (ANY(N_LAYER_CELLS_MAX<2)) THEN
      WRITE(MESSAGE,'(A,A,A)') 'ERROR: SURF ',TRIM(SF%ID),' N_LAYER_CELLS_MAX must be >= 2'
      CALL SHUTDOWN(MESSAGE) ; RETURN      
   ENDIF
   SF%N_LAYER_CELLS_MAX    = N_LAYER_CELLS_MAX
   ...
   IF (SF%LAYER_DENSITY(1) > 0._EB) THEN
      ...
      ALLOCATE(SF%SPECIFIED_SOURCE_TERM(SF%N_LAYERS))    ! Volumeetric source term set by the user
   ELSE
      SF%TMP_FRONT                  = TMP_FRONT + TMPM
   ...
   IF (SF%N_LAYERS>0) THEN
      SF%SPECIFIED_SOURCE_TERM(1:SF%N_LAYERS) = 1000._EB*SPECIFIED_SOURCE_TERM(1:SF%N_LAYERS)
   ENDIF
