...
REAL :: AREA,A_T,A_V,CHI_R,CONDUIT_DIAMETER,CONDUIT_THICKNESS,C_S,D,DELTA,DT,H,H_K,H_V,JACKET_THICKNESS,K_S, &
        L,L_F,LEAK_AREA,MASS_PER_LENGTH,M_DOT,P,Q,Q_STAR,R,RADIATIVE_FRACTION,RHO_S,RTI,T,T_ACTIVATION,T_END, &
        TMP_A,TMP_G,T_P,T_CLOCK,U_JET,V,V_DOT,W,W_V
...
NAMELIST /SPKACT/ Q,RTI,T_ACTIVATION,H,R,TMP_A,RADIATIVE_FRACTION,OUTPUT_FILE
...
t_activation = (RTI / SQRT(U_JET)) * LOG((T_JET - TMP_A)/(T_JET - T_ACTIVATION))

WRITE(*,*) t_activation

WRITE(11,'(A)') 'Activation time, Activation temperature'
WRITE(11,'(F6.1,A1,F6.1)') t_activation,',',T_JET-273.
