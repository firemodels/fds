...
REAL(EB), ALLOCATABLE, DIMENSION(:,:):: NODE_AREA,NODE_AREA_EX,NODE_H,NODE_P,NODE_RHO,NODE_X,NODE_Y,NODE_Z,NODE_TMP,DUCT_MF
...
INTEGER :: NN,II,JJ,KK,I1,I2,J1,J2,K1,K2,IC,IW,IOR,NODE_ZONE,IZ1,IZ2
REAL(EB) :: ZZ_GET(0:N_TRACKED_SPECIES),AREA,TMP_SUM,H_G,TMP_NEW,TNOW,P_AVE,FAC,CPBAR
...
NODE_RHO(:,NM) = 0._EB
NODE_AREA(:,NM) = 0._EB
IF (N_TRACKED_SPECIES > 0) NODE_ZZ(:,:,NM) = 0._EB
ZZ_GET = 0._EB
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   ...
   IF (.NOT. DN%IN_MESH(NM)) CYCLE NODE_LOOP
   ...
   VENT_LOOP: DO KK = K1,K2
      DO JJ = J1,J2
         DO II = I1, I2
            ...
            NODE_AREA(NN,NM) = NODE_AREA(NN,NM) + AREA
            NODE_RHO(NN,NM) = NODE_RHO(NN,NM) + RHOP(II,JJ,KK)*AREA

            IF (N_TRACKED_SPECIES > 0) THEN
               ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
               NODE_ZZ(NN,1:N_TRACKED_SPECIES,NM) = NODE_ZZ(NN,1:N_TRACKED_SPECIES,NM) + &
                                                    ZZ_GET(1:N_TRACKED_SPECIES)*AREA*RHO(II,JJ,KK)
            ENDIF
            CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_GET,CPBAR,TMP(II,JJ,KK))
            NODE_TMP(NN,NM) = NODE_TMP(NN,NM) + TMP(II,JJ,KK)*AREA
            NODE_H(NN,NM) = NODE_H(NN,NM) + CPBAR * TMP(II,JJ,KK) * RHOP(II,JJ,KK) * AREA
           
            SELECT CASE (IOR)
               CASE (1)
                  P_AVE = PBARP(KK,NODE_ZONE)
                  NODE_X(NN,NM) = NODE_X(NN,NM) + X(II-1)*AREA
                  NODE_Y(NN,NM) = NODE_Y(NN,NM) + YC(JJ)*AREA            
                  NODE_Z(NN,NM) = NODE_Z(NN,NM) + ZC(KK)*AREA
               CASE(-1)
                  P_AVE = PBARP(KK,NODE_ZONE)
                  NODE_X(NN,NM) = NODE_X(NN,NM) + X(II)*AREA
                  NODE_Y(NN,NM) = NODE_Y(NN,NM) + YC(JJ)*AREA            
                  NODE_Z(NN,NM) = NODE_Z(NN,NM) + ZC(KK)*AREA
               CASE (2)
                  P_AVE = PBARP(KK,NODE_ZONE)
                  NODE_X(NN,NM) = NODE_X(NN,NM) + XC(II)*AREA
                  NODE_Y(NN,NM) = NODE_Y(NN,NM) + Y(JJ-1)*AREA            
                  NODE_Z(NN,NM) = NODE_Z(NN,NM) + ZC(KK)*AREA
               CASE(-2)
                  P_AVE = PBARP(KK,NODE_ZONE)
                  NODE_X(NN,NM) = NODE_X(NN,NM) + XC(II)*AREA
                  NODE_Y(NN,NM) = NODE_Y(NN,NM) + Y(JJ)*AREA            
                  NODE_Z(NN,NM) = NODE_Z(NN,NM) + ZC(KK)*AREA
               CASE (3)
                  P_AVE = 0.5_EB*(PBARP(KK-1,NODE_ZONE)+PBARP(KK,NODE_ZONE))
                  NODE_X(NN,NM) = NODE_X(NN,NM) + XC(II)*AREA
                  NODE_Y(NN,NM) = NODE_Y(NN,NM) + YC(JJ)*AREA            
                  NODE_Z(NN,NM) = NODE_Z(NN,NM) + Z(KK-1)*AREA
               CASE (-3)
                  P_AVE = 0.5_EB*(PBARP(KK,NODE_ZONE)+PBARP(KK+1,NODE_ZONE))
                  NODE_X(NN,NM) = NODE_X(NN,NM) + XC(II)*AREA
                  NODE_Y(NN,NM) = NODE_Y(NN,NM) + YC(JJ)*AREA            
                  NODE_Z(NN,NM) = NODE_Z(NN,NM) + Z(KK)*AREA
            END SELECT
            NODE_P(NN,NM) = NODE_P(NN,NM) + (P_AVE+RHO(II,JJ,KK)*(HP(II,JJ,KK)-KRES(II,JJ,KK)))*AREA     
         ENDDO
      ENDDO
   ENDDO VENT_LOOP
...
USE PHYSICAL_FUNCTIONS, ONLY : GET_SPECIFIC_GAS_CONSTANT,GET_AVERAGE_SPECIFIC_HEAT
INTEGER:: NN,NM,NS,IZ1,IZ2,ITCOUNT
REAL(EB) :: AREA,AREA_SUM,RHO_SUM,CPBAR,H_G,TMP_SUM,TMP_NEW,ZZ_GET(0:N_TRACKED_SPECIES),CPBAR2,DCPDT
TYPE(DUCTNODE_TYPE), POINTER :: DN=>NULL()
...
DO NN=1,N_DUCTNODES
   ...
   VENT_NODE: IF (DN%VENT_INDEX > 0 .AND. .NOT. DN%LEAKAGE) THEN
      ...
      DN%P = 0.7_EB*DN%P + 0.3_EB*SUM(NODE_P(NN,:))/AREA
