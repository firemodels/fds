...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   IF_VEG_INCLUDED: IF (NLP>0 .AND. TREE_MESH(NM)) THEN
      VEG_ELEM_LOOP: DO ID = 1,NLP
         ...
         XID = CELLSI(FLOOR((PARTICLE(ID)%X-XS)*RDXINT))
         YJD = CELLSJ(FLOOR((PARTICLE(ID)%Y-YS)*RDYINT))
         ZKD = CELLSK(FLOOR((PARTICLE(ID)%Z-ZS)*RDZINT))
         IID = FLOOR(XID+1._EB)
         JJD = FLOOR(YJD+1._EB)
         KKD = FLOOR(ZKD+1._EB)
   ...
   IF (NLP>0 .AND. VIRTUAL_PARTICLES) THEN
      DO ID = 1,NLP
         ...
         XID = CELLSI(FLOOR((LP%X-XS)*RDXINT))
         YJD = CELLSJ(FLOOR((LP%Y-YS)*RDYINT))
         ZKD = CELLSK(FLOOR((LP%Z-ZS)*RDZINT))
         IID = FLOOR(XID+1._EB)
         JJD = FLOOR(YJD+1._EB)
         KKD = FLOOR(ZKD+1._EB)
