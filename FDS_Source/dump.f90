...
IF (MIXTURE_FRACTION .AND. STATE_FILE) THEN
   ...
   WRITE(TCFORM,'(A,I4.4,A)') "(",N_STATE_SPECIES+1,"(A,','),A)"
   WRITE(LU_STATE(1),TCFORM) ('kg/kg',NN=1,N_STATE_SPECIES+1),'g/mol'
   LABEL(1) = 'Z'
   LABEL(2:2+N_STATE_SPECIES-1) = MF_SPEC_FORMULA(1:N_STATE_SPECIES)
   LABEL(2+N_STATE_SPECIES) = 'Wgt'
   WRITE(LU_STATE(1),TCFORM) LABEL(1:N_STATE_SPECIES+2)
   Y_VECTOR=0._EB
   SOOT_DEP_IF: IF (SOOT_DEPOSITION) THEN
      IF (CO_PRODUCTION) THEN
         ...
         DO IZ=0,1000
            ...
            WRITE(LU_STATE(1),'(10(ES12.4,","),ES12.4)') ZZ,Y_MF(1:N_STATE_SPECIES),MW_MF
         ENDDO
      ELSE
         Z_F = REACTION(1)%Z_F
         DO IZ=0,1000
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D .AND..NOT.EVACUATION_ONLY(NM)) THEN
   ...
   IF (.NOT.APPEND .AND. SMOKE3D_QUANTITY_INDEX/=11) THEN
      IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
      M%N_STRINGS = M%N_STRINGS + 1
      IF(USE_HRRPUV_MAX_SMV.EQ.1)THEN
...
DO N=1,N_GEOM
   ...
   IF (G%ISHAPE==ICYLINDER) WRITE(LU_SMV,'(1X,A,1F12.5)') 'L=',2._EB*G%HL(2)
...
EVAC_ONLY5: IF (ANY(EVACUATION_GRID).AND.MYID==MAX(0,EVAC_PROCESS)) THEN
   ...
   NN = 0
   DO N=1,NPC_EVAC
      IF (.NOT.EVACUATION(N)%SHOW) CYCLE
      NN = NN + 1
      XX = EVACUATION(N)%X1
      YY = EVACUATION(N)%Y1
      ZZ = 0.5_EB*(EVACUATION(N)%Z1+EVACUATION(N)%Z2)+0.05_EB*(NN-1)
      WRITE(LU_SMV,'(/A)') 'DEVICE'
      WRITE(LU_SMV,'(A)') 'evacbox'
      WRITE(LU_SMV,'(6F12.5,2I2)') XX,YY,ZZ,0.0,0.0,1.0,0,6
      XX = ABS(EVACUATION(N)%X1-EVACUATION(N)%X2)
      YY = ABS(EVACUATION(N)%Y1-EVACUATION(N)%Y2)
      WRITE(LU_SMV,'(3I6,3F10.3)')  EVACUATION(N)%RGB(1:3), XX, YY, 0.05
   ENDDO
   NN = 0
   DO N=1,N_HOLES
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   VENT_LOOP: DO N=1,M%N_VENT
      ...
      IF (VT%BOUNDARY_TYPE==MIRROR_BOUNDARY .OR. VT%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
   ...
   ELSE   
      IF (DNS .AND. SS%MODE==GAS_SPECIES) THEN
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '   Viscosity (kg/m/s)   Ambient: ',&
                                           (Y2MU_C(NINT(TMPA))+Y2MU(NINT(TMPA),N))*SPECIES(N)%MW
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                          500 C: ',(Y2MU_C(773)+Y2MU(773,N))*SPECIES(N)%MW
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                         1000 C: ',(Y2MU_C(1273)+Y2MU(1273,N))*SPECIES(N)%MW
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                         1500 C: ',(Y2MU_C(1773)+Y2MU(1773,N))*SPECIES(N)%MW
      ENDIF
      IF (DNS .AND. SS%MODE==GAS_SPECIES) THEN
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '   Therm. Cond. (W/m/K) Ambient: ',&
                                           (Y2K_C(NINT(TMPA))+Y2K(NINT(TMPA),N))*SPECIES(N)%MW
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                          500 C: ',(Y2K_C( 773)+Y2K( 773,N))*SPECIES(N)%MW
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                         1000 C: ',(Y2K_C(1273)+Y2K(1273,N))*SPECIES(N)%MW
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                         1500 C: ',(Y2K_C(1773)+Y2K(1773,N))*SPECIES(N)%MW
      ENDIF
      IF (DNS .AND. SS%MODE==GAS_SPECIES) THEN
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '   Spec. Heat (J/kg/K)  Ambient: ',Y2CP_C(NINT(TMPA))+Y2CP(NINT(TMPA),N)
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                          500 C: ',Y2CP_C( 773)+Y2CP( 773,N)
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                         1000 C: ',Y2CP_C(1273)+Y2CP(1273,N)
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                         1500 C: ',Y2CP_C(1773)+Y2CP(1773,N)
      ENDIF
   ENDIF
...
REACTION_LOOP: DO N=1,N_REACTIONS
   ...
   SELECT CASE (RN%MODE)
      ...
      CASE (FINITE_RATE_REACTION)
         DO NN=1,N_SPECIES
            WRITE(LU_OUTPUT,'(A,A15,F8.3)')  '   Stoich. Coeff. ',TRIM(SPECIES(NN)%ID),RN%NU(NN) 
         ENDDO
         DO NN=1,N_SPECIES
            IF(RN%N(NN)==-999._EB) THEN
               WRITE(LU_OUTPUT,'(A,A15,F8.3)')  '    Rate Exponent ',TRIM(SPECIES(NN)%ID),0. 
            ELSE
               WRITE(LU_OUTPUT,'(A,A15,F8.3)')  '    Rate Exponent ',TRIM(SPECIES(NN)%ID),RN%N(NN) 
            ENDIF
         ENDDO
   END SELECT
ENDDO REACTION_LOOP
...
IF (N_SPECIES>0) WRITE(LU_CORE(NM)) YY_F
...
IF (N_SPECIES>0) READ(LU_RESTART(NM))  YY_F
IF (DROPLET_FILE) THEN
...
REAL(FB) :: TMPUP,TMPLOW,ZINT,TMP_WGT
...
IF (SPEC_INDEX<0) THEN
   ...
   IF (DRY .AND. H2O_INDEX > 0) THEN
      ...
      R_Y_H2O = RCON_MF(H2O_INDEX) * Y_H2O
...
SELECT CASE(IND)
   ...
   CASE(15)  ! MIXING TIME
      GAS_PHASE_OUTPUT = MAX(DT,C_EDC*SC*RHO(II,JJ,KK)*(DX(II)*DY(JJ)*DZ(KK))**TWTH/MU(II,JJ,KK))
   ...
   CASE(94)  ! VOLUME FRACTION
      IF (SPEC_INDEX<0) THEN
         GAS_PHASE_OUTPUT = RCON_MF(-SPEC_INDEX)*Y_SPECIES/RSUM(II,JJ,KK)/(1._EB-R_Y_H2O/RSUM(II,JJ,KK))
      ELSEIF (SPEC_INDEX>0) THEN
         GAS_PHASE_OUTPUT = SPECIES(SPEC_INDEX)%RCON*Y_SPECIES/RSUM(II,JJ,KK)
      ENDIF
   CASE(95)  ! VISIBILITY
...
REAL(EB) :: CONCORR,U2,V2,W2,HQ2,FLUX,DFLUXDT,SOLID_PHASE_OUTPUT_OLD,YY_GET(1:N_SPECIES),Y_SPECIES
...
SELECT CASE(INDX)
   ...
   CASE(20) ! MASS FLUX
      IF (SPEC_INDEX<0) THEN
         YY_GET(:) = ABS(MASSFLUX(IWX,1:N_SPECIES))
         FLUX = SUM(YY_GET)
         IF (FLUX > 0._EB) THEN
            YY_GET(:) = YY_GET/FLUX
            CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,Y_SPECIES)
   ...
   CASE(1)
      NORMAL_SELECT_1: SELECT CASE(IOR)
         CASE(+1)
            ...
            HP   = 0.5_EB*(   H(II,JJ,KK)+   H(II+1,JJ,KK))
            KP   = 0.5_EB*(KRES(II,JJ,KK)+KRES(II+1,JJ,KK))
            DIV  = 0.5_EB*(   D(II,JJ,KK)+   D(II+1,JJ,KK)) 
         ...
         CASE(-1)
            ...
            HP   = 0.5_EB*(   H(II-1,JJ,KK)+   H(II,JJ,KK))
            KP   = 0.5_EB*(KRES(II-1,JJ,KK)+KRES(II,JJ,KK))
            DIV  = 0.5_EB*(   D(II-1,JJ,KK)+   D(II,JJ,KK))
   ...
   CASE(3)
      NORMAL_SELECT_3: SELECT CASE(IOR)
         ...
         CASE(+3)
            ...
            RHOP = 0.5_EB*( RHO(II,JJ,KK)+ RHO(II,JJ,KK+1))
            MUA  = 0.5_EB*(  MU(II,JJ,KK)+  MU(II,JJ,KK+1))
            HP   = 0.5_EB*(   H(II,JJ,KK)+   H(II,JJ,KK+1))
            KP   = 0.5_EB*(KRES(II,JJ,KK)+KRES(II,JJ,KK+1))
            DIV  = 0.5_EB*(   D(II,JJ,KK)+   D(II,JJ,KK+1))
         ...
         CASE(-3)
            ...
            RHOP = 0.5_EB*( RHO(II,JJ,KK-1)+ RHO(II,JJ,KK))
            MUA  = 0.5_EB*(  MU(II,JJ,KK-1)+  MU(II,JJ,KK))
            HP   = 0.5_EB*(   H(II,JJ,KK-1)+   H(II,JJ,KK))
            KP   = 0.5_EB*(KRES(II,JJ,KK-1)+KRES(II,JJ,KK))
            DIV  = 0.5_EB*(   D(II,JJ,KK-1)+   D(II,JJ,KK))
...
REAL(EB) :: VC,TMP_N,RHO_N,U_N,H_G,YY_N(1:N_SPECIES),E_DIFF,TMP_WGT
INTEGER, INTENT(IN) :: NM
...
WALL_LOOP: DO IW=1,NWC
   ...
   ENDIF
   H_G = H_G * TMP_N
   IF (MIXTURE_FRACTION .AND. H2O_INDEX > 0) THEN
      H_G = H_G - YY_N(I_PROG_F) * H_G_H2O_FAC
      IF (CO_PRODUCTION)  H_G = H_G - YY_N(I_PROG_CO) * H_G_H2O_FAC
   ENDIF
...
IF(CENTERED_HRR_TIME)THEN
  ...
  IF(STIME.GE.T_BEGIN+DT_HRR)THEN
...
REAL(EB) :: VC,Y_MF_INT(1:N_Y_ARRAY),YY_GET(1:N_SPECIES)
INTEGER, INTENT(IN) :: NM
INTEGER :: I,J,K
      ...
      DO J=0,JBAR
         DO I=0,IBAR
            ...
            IF(I.NE.0.AND.J.NE.0)THEN
              ...
              ISUM = ISUM + 1
            ENDIF
            IF(I.NE.IBAR.AND.J.NE.0)THEN
              PPN(I,J) = PPN(I,J) + PP(I+1,J)
              ISUM = ISUM + 1
            ENDIF
            IF(I.NE.0.AND.J.NE.JBAR)THEN
              PPN(I,J) = PPN(I,J) + PP(I,J+1)
              ISUM = ISUM + 1
            ENDIF
            IF(I.NE.IBAR.AND.J.NE.JBAR)THEN
...
REAL(EB) :: T_TOT,T_LES,T_SGS,ZZ(3,3,3),ZZ_HAT(3,3,3)
INTEGER :: ARRAY_LO(3),ARRAY_HI(3),I,J,K,LL,MM,NN
...
CALL TEST_FILTER(ZZ_HAT,ZZ,ARRAY_LO,ARRAY_HI,ARRAY_LO,ARRAY_HI)
   
T_LES = ZZ(2,2,2)**2
T_SGS = (ZZ(2,2,2)-ZZ_HAT(2,2,2))**2
T_TOT = T_LES + T_SGS
