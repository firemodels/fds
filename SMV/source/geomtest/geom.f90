...
PRIVATE
PUBLIC :: READ_GEOM,WRITE_GEOM,ROTATE_VEC, SETUP_TRANSFORM
...
REAL(EB) :: AZIM_RATE, ELEV_RATE, SCALE_RATE(3), XYZ_RATE(3)
...
TYPE(GEOMETRY_TYPE), POINTER :: G=>NULL()
NAMELIST /GEOM/ AZIM, AZIM_RATE, COMPONENT_ONLY, DAZIM, DELEV, DSCALE, DXYZ0, DXYZ, &
                ELEV, ELEV_RATE, FACES, GEOM_IDS, ID, SCALE, SCALE_RATE, SURF_ID, VERTS, XYZ0, XYZ, XYZ_RATE
...
!   if an object is in a GEOM_IDS list then COMPONENT_ONLY is initially set to .TRUE. (is only drawn as part of a larger group)
!   if an object is not in any GEOM_IDS list then COMPONENT_ONLY is initially set to .FALSE. (is drawn by default)
...
READ_GEOM_LOOP: DO N=1,N_GEOMETRY
   ...
   G%AZIM_RATE = AZIM_RATE
   G%ELEV_RATE = ELEV_RATE
   G%SCALE_RATE(1:3) = SCALE_RATE(1:3)
   G%XYZ_RATE(1:3) = XYZ_RATE(1:3)
   IF (ABS(AZIM_RATE).LT.TWO_EPSILON_EB .AND. ABS(ELEV_RATE).LT.TWO_EPSILON_EB.AND.&
       ANY(ABS(SCALE_RATE(1:3)).LT.TWO_EPSILON_EB) .AND. ANY(ABS(XYZ_RATE(1:3) ).LT.TWO_EPSILON_EB) ) THEN 
      G%IS_DYNAMIC = .FALSE.
   ELSE
      G%IS_DYNAMIC = .TRUE.
...
SUBROUTINE SETUP_TRANSFORM(SCALE,AZ,ELEV,M)
...
REAL(EB) :: AXIS(3), M0(3,3), M1(3,3), M2(3,3), MTEMP(3,3)
...
! transform (scale, rotate and translate) vectors found on each &GEOM line
   ...
   DO I = 1, N_GEOMETRY
      ...
      G%AZIM = G%AZIM_BASE + DELTA_T*G%AZIM_RATE
      G%ELEV = G%ELEV_BASE + DELTA_T*G%ELEV_RATE
      G%XYZ = G%XYZ_BASE + DELTA_T*G%XYZ_RATE
      
      IF (IS_DYNAMIC.AND..NOT.G%IS_DYNAMIC.OR..NOT.IS_DYNAMIC.AND.G%IS_DYNAMIC) THEN
         G%N_VERTS = 0
         G%N_FACES = 0
      ELSE
   ...
   DO I = 1, N_GEOMETRY
      ...
      CALL EXPAND_GROUPS(I) ! create vertex and face list from geometries specified in GEOM_IDS list
      IF (G%N_VERTS.EQ.0) CYCLE
      CALL SETUP_TRANSFORM(G%SCALE,G%AZIM,G%ELEV,M)
...
IF(G%N_FACES>G%N_FACES_OLD)THEN
   IF (G%N_FACES_OLD.NE.0) THEN
      DEALLOCATE(G%FACES)
      DEALLOCATE(G%SURFS)
   ENDIF
   ...
   ALLOCATE(G%SURFS(N_FACES),STAT=IZERO)
   CALL ChkMemErr('READ_GEOM','SURFS',IZERO)
   G%N_FACES_OLD=G%N_FACES
ENDIF

IF(G%N_VERTS>G%N_VERTS_OLD)THEN
   IF (G%N_VERTS_OLD.NE.0) THEN
      DEALLOCATE(G%VERTS)
      IF(G%NSUB_GEOMS>0) DEALLOCATE(G%VERTS_BASE)
   ENDIF
   ALLOCATE(G%VERTS(3*N_VERTS),STAT=IZERO)
   CALL ChkMemErr('READ_GEOM','VERTS',IZERO)
   IF(G%NSUB_GEOMS>0)THEN
      ALLOCATE(G%VERTS_BASE(3*N_VERTS),STAT=IZERO)
      CALL ChkMemErr('READ_GEOM','VERTS_BASE',IZERO)
   ENDIF
   G%N_VERTS_OLD=G%N_VERTS
ENDIF
...
IF (G%HAS_SURF) THEN
   ...
   G%SURFS(1:N_FACES) = SURF_INDEX
ENDIF
G%VERTS_BASE(1:3*N_VERTS)=G%VERTS(1:3*N_VERTS)
...
SUBROUTINE OUTGEOM(LUNIT,IS_DYNAMIC,TIME)
   ...
   IF (N_VERTS.GT.0.0.AND.N_FACES.GT.0) THEN
