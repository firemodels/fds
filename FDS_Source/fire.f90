...
REAL(EB):: ZZ_GET(0:N_TRACKED_SPECIES),ZZ_MIN=1.E-10_EB,DZZ(0:N_TRACKED_SPECIES),CP,HDIFF,H_S
         ...
         IF (ANY(A1 < 0._EB)) THEN
            DO NSS=0,N_TRACKED_SPECIES
               IF (A1(NSS) < 0._EB .AND. ABS(DZZDT(NSS)+DZZDT2(NSS))>ZERO_P) THEN
                  DT_SUB = MIN(DT_SUB,ABS(-2._EB*ZZ_0(NSS)/(DZZDT(NSS)+DZZDT2(NSS))))
               ENDIF
            ENDDO
            A1 = ZZ_0 + 0.5_EB*(DZZDT+DZZDT2)*DT_SUB
         ENDIF 
...
REAL(EB), PARAMETER :: TAU_CHEM_FINITE_RATE = 1.E-4_EB
INTEGER :: NS
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
RN => REACTION(NR)
...
SELECT CASE (MODE)
   ...
   CASE(FINITE_RATE)
      ...
      ENDIF
      YY_F_LIM = MAX(YY_F_LIM,Y_F_MIN)
      RATE_CONSTANT = MIN(YY_F_LIM/TAU_CHEM_FINITE_RATE,RATE_CONSTANT)          
