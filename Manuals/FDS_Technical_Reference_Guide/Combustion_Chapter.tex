...
(\hat{Z}_{\rm F} + \hat{Z}_{\rm A} + \hat{Z}_{\rm P}) \, h_{\rm PFP}(T_{\rm LFT}) 
\end{align}
where $T$ is the initial mean cell temperature and PFP denotes a ``post-flame product'' mixture.  If the inequality Eq.~(\ref{eq:extinction}) is true, then combustion is suppressed---the heat of combustion is not sufficient to raise the product mixture above its critical flame temperature.
