...
INTEGER :: N_REQ0,SNODE,MEMBERS(0:NMESHES-1),NN,NOM,N_COMMUNICATIONS,JDIM
...
INTEGER :: NM_MAX_V,NM_MAX_P,NOM,NNN
   ...
   IF (POISSON_ITERATION_ACCELERATOR) THEN
      ...
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         DO NNN=1,M%N_NEIGHBORING_MESHES
            ...
            M2 => MESHES(NOM)
            IF (M%XS<M2%XS .OR. M%YS<M2%YS .OR. M%ZS<M2%ZS) THEN
               CALL GET_BOUNDARY_PRESSURE(NM,NOM)
            ELSE
               CALL GET_BOUNDARY_PRESSURE(NOM,NM)
            ENDIF
         ENDDO
      ENDDO
   ENDIF
...
DO NM=1,NMESHES
   IF (MPI_COMM_MESH(NM)==MPI_COMM_NULL) CYCLE
   M => MESHES(NM)
   CALL MPI_BCAST(M%H ,SIZE(M%H)     ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%HS,SIZE(M%HS)    ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%PRHS,SIZE(M%PRHS),MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%BXS,SIZE(M%BXS)  ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%BXF,SIZE(M%BXF)  ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%BYS,SIZE(M%BYS)  ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%BYF,SIZE(M%BYF)  ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%BZS,SIZE(M%BZS)  ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%BZF,SIZE(M%BZF)  ,MPI_DOUBLE_PRECISION,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%LBC,1            ,MPI_INTEGER         ,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%MBC,1            ,MPI_INTEGER         ,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
   CALL MPI_BCAST(M%NBC,1            ,MPI_INTEGER         ,MPI_COMM_MESH_ROOT(NM),MPI_COMM_MESH(NM),IERR)
ENDDO
