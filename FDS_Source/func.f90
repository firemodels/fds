...
IF (RADIATION) THEN
   ...
   ELSE
      ...
      CALL ChkMemErr('FUNC','ILW',IZERO)
   ENDIF
   IF (M%BOUNDARY_TYPE(IW)==VIRTUAL_BOUNDARY) THEN
      WC%ILW        = 0._EB
   ELSE
...
IF (SF%THERMALLY_THICK) THEN
   ...
   DO II=0,SF%N_CELLS+1
      ...
      ELSE
         WC%TMP_S(II) = M%TMP_0(M%IJKW(3,IW))
      ENDIF
   ...
   WC%SHRINKING = .FALSE.
   WC%BURNAWAY  = .FALSE.
   M%TMP_F(IW)  = WC%TMP_S(0)
   M%TMP_B(IW)  = WC%TMP_S(SF%N_CELLS+1)
ENDIF
...
Y_OUT = Z2Y_C(INDEX) + DOT_PRODUCT(Z2Y(INDEX,:), Z_IN)
...
DO I=1,N_SPECIES
   Y_OUT(I) = Z2Y_C(I) + DOT_PRODUCT(Z2Y(I,:),Z_IN)
ENDDO
...
MW_OUT =  1._EB/ (MW_AVG_Y_C  + DOT_PRODUCT(MW_AVG_Y,Z_IN))
...
RSUM_OUT =  R0 * (MW_AVG_Y_C  + DOT_PRODUCT(MW_AVG_Y,Z_IN))

END SUBROUTINE GET_SPECIFIC_GAS_CONSTANT



SUBROUTINE GET_SPECIFIC_HEAT_BG(CP_OUT,TMPG)
...
CP_OUT_A = Z2CP_C(ITMP)
CP_OUT_B = Z2CP_C(ITMP+1)

CP_OUT = CP_OUT_A + TMP_WGT * (CP_OUT_B-CP_OUT_A) 

END SUBROUTINE GET_SPECIFIC_HEAT_BG
...
CP_OUT_A = Z2CP_C(ITMP) + DOT_PRODUCT(Z2CP(ITMP,:),Z_IN)
CP_OUT_B = Z2CP_C(ITMP+1) + DOT_PRODUCT(Z2CP(ITMP+1,:),Z_IN)
...
SUBROUTINE GET_AVERAGE_SPECIFIC_HEAT_BG(CPBAR_OUT,TMPG)
...
CPBAR_OUT_A = Z2CPBAR_C(ITMP)
CPBAR_OUT_B = Z2CPBAR_C(ITMP+1)

CPBAR_OUT = CPBAR_OUT_A + TMP_WGT * (CPBAR_OUT_B-CPBAR_OUT_A) 

END SUBROUTINE GET_AVERAGE_SPECIFIC_HEAT_BG
...
CPBAR_OUT_A = Z2CPBAR_C(ITMP) + DOT_PRODUCT(Z2CPBAR(ITMP,:),Z_IN)
CPBAR_OUT_B = Z2CPBAR_C(ITMP+1) + DOT_PRODUCT(Z2CPBAR(ITMP+1,:),Z_IN)
...
CPBAR_DIFF_OUT = Z2CPBAR(ITMP,N)+TMP_WGT * (Z2CPBAR(ITMP+1,N)-Z2CPBAR(ITMP,N)) 

END SUBROUTINE GET_AVERAGE_SPECIFIC_HEAT_DIFF



SUBROUTINE GET_CONDUCTIVITY_BG(K_OUT,TMPG)
...
K_OUT_A = Z2K_C(ITMP) 
K_OUT_B = Z2K_C(ITMP+1)

K_OUT = (K_OUT_A + TMP_WGT * (K_OUT_B-K_OUT_A) )*SPECIES_MIXTURE(0)%MW

END SUBROUTINE GET_CONDUCTIVITY_BG
...
K_OUT_A = Z2K_C(ITMP) + DOT_PRODUCT(Z2K(ITMP,:),Z_IN)
K_OUT_B = Z2K_C(ITMP+1) + DOT_PRODUCT(Z2K(ITMP+1,:),Z_IN)
...
SUBROUTINE GET_VISCOSITY_BG(MU_OUT,TMPG)
...
MU_OUT_A = Z2MU_C(ITMP)
MU_OUT_B = Z2MU_C(ITMP+1)

MU_OUT = (MU_OUT_A + TMP_WGT * (MU_OUT_B-MU_OUT_A) )*SPECIES_MIXTURE(0)%MW

END SUBROUTINE GET_VISCOSITY_BG
...
MU_OUT_A = Z2MU_C(ITMP) + DOT_PRODUCT(Z2MU(ITMP,:),Z_IN)
MU_OUT_B = Z2MU_C(ITMP+1) + DOT_PRODUCT(Z2MU(ITMP+1,:),Z_IN)
...
SUBROUTINE SPRAY_ANGLE_DISTRIBUTION(XI,CDF,BETA,MU,THETA_MAX,THETA_MIN,DISTRIBUTION_TYPE,NPT)
INTEGER,INTENT(IN)::NPT
REAL(EB),INTENT(INOUT):: XI(0:NPT),CDF(0:NPT)
REAL(EB),INTENT(IN)::BETA,MU,THETA_MAX,THETA_MIN
CHARACTER(30),INTENT(IN):: DISTRIBUTION_TYPE

INTEGER::I
REAL(EB)::dTHETA,PDF(0:NPT)

dTHETA=(THETA_MAX-THETA_MIN)/(NPT-1)
XI=(/ (THETA_MIN+(I-1)*dTHETA,I=1,NPT) /)
SELECT CASE(DISTRIBUTION_TYPE)
   CASE("TRIANGLE")
      WHERE(XI<MU)
         PDF=BETA*(XI-THETA_MIN)
      ELSEWHERE
         PDF=BETA*(MU-THETA_MIN)-BETA*(XI-MU)
      END WHERE
      PDF=PDF*sin(XI)     
   CASE("GAUSSIAN")
      PDF=exp(-BETA*((XI-MU)/(THETA_MAX-THETA_MIN))**2)
      PDF=PDF*sin(XI)
...
CDF(0)=0._EB
DO I=1,NPT
   CDF(I)=CDF(I-1)+0.5*(PDF(I)+PDF(I-1))*dTHETA
ENDDO
CDF(:)=CDF(:)/CDF(NPT)
