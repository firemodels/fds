...
the evacuation module~\cite{Korhonen05, Korhonen07a, Korhonen07b,
  Korhonen08a, Korhonen08b} developed at VTT Technical Research Centre
of Finland.  This combined fire and evacuation simulation program is
called as FDS+Evac in this document.  FDS+Evac allows simultaneous
simulation both the fire and evacuation processes.  FDS+Evac can also
be run on a ``fire drill'' mode, \emph{i.e.}, used to calculate only
the human egress process.

FDS+Evac treats each evacuee as a spearate entity, which has its own
personal properties and escape strategies, \emph{i.e.}, FDS+Evac
models the egress process using autonomous 'agents' to represent each
escaping person.  The movement of the agents is treated using two
dimensional planes to represent the floors of buildings.  The basic
algorithm behind the egress movement is to follow each agent by an
equation motion in a continuous 2D space and time, \emph{i.e.}, do
some kind of an artificial molecular dynamics for the agents.  The
forces acting on the agents are both physical forces, like contact
forces and gravity, and psycholigical forces exerted by the
environment and other agents.  The model behind the movement algorithm
is the social force model introduced by Helbing's
group~\cite{Helbing95, Helbing00, Helbing02, Werner03} and the
modification of the model to a better description of a human body was
introduced by Langston \em et al. \em ~\cite{Langston06}.
...
compared to other evacuation calculation methods and experimental
...
first the manual of FDS and learn how to do fire simulations.  Even if
the user want to run the programme in the ``fire drill'' mode, she/he
should be acquainted with the FDS in order to build up the simulation
geometry. 
...
calculation can be run parallel by using the MPI version of FDS.
...
pages.  This document refers to the FDS version 5.2.0.
...
\section{Special Features}\label{Sec_SpecFeatures}
...
with spectator stands or concert halls can also be modelled if their
geometry is not too complicated, but the user should note that there
has not been any validation work done for these kind of inclines.  It
is also assumed that the different levels of the building are
separated from each other, \emph{i.e.}, they are connected to each
other by stairs, escalators, doors, and similar objects.  Note, that
FDS+Evac does not support the use of elevators during evacuation
process.  Wide stairs or inclines can also be used to connect
different floors, but this is not as straigthforward as to use the
default stairscase model.  There are no merging flows allowed in the
default staircase model, so FDS+Evac is not yet easy to use for tall
buildings.  The default staircase algorithm is still very simple and
it should not be trusted too much if the stairs get crowded.  At this
point this is not a major problem, because there are no merging flows
in the staircase model, \emph{i.e.}, the exit door restricts the flow
to the staircase quite efficiently.  If merging flows in staircases
are needed then the detailed geometry of the staircase should be
modelled, but this is not usually very practical way of doing things.
...
programme and see that there are not too narrow ``holes'' in the
geometry.

Note that the FDS fire calculation input could have time dependent
geometry elements, like obstacles and holes which are created and
removed by special control devices defined in the user input.  The
evacuation calculation geometry is not supporting time dependent
geometries.  It uses the initial geometry during the whole evacuation
simulation.
...
same evacuation mesh.  Usually, one main evacuation
mesh extends over a whole floor of the building.  There may be many
main evacuation meshes, \emph{e.g.}, different floors of the building,
in the FDS+Evac calculation and the total number of humans is not
restricted by the program, the only limit is the available computer
memory.  The user should know that if there is more than a couple of
thousands humans on an evacuation floor then the calculation is going
to be CPU expensive. 

The initial density of humans can not be much larger than 4 persons
per square metre.  Because the initial positions of humans are
generated randomly, a simulation could start after a couple of
unsuccessful initialization trials, if the human density is high.  If
FDS+Evac is not able to put humans on their initial positions, an
error message ``ERROR: FDS improperly set up.'' is printed on the
standard error channel and more information is printed on the
diagnostic output file of the evacuation calculation.
...
model.  Note that the effects of radiation and gas temperature on
humans are not yet implemented in the programme, so humans are not
avoiding a fire if the user does not explicitely define the evacuation
geometry to take this in to account.
...
will be more optimal.  Using computer memory efficiently means that
one defines the evacuation meshes as small as possible and put as few
obstacles in the calculation as possible.  For now the only thing the
user can do is to swicth from a 32 bit machine to a 64 bit machine in
order to increase the available memory, if the FDS+Evac calculation
takes too much memory.



\section{Intended User's of FDS+Evac}\label{Sec_IntUsers}
