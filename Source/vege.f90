...
! WC%ONE_D%Q_CON_F  = QCONF_FDS_WALL !W/m^2
...
!
  ...
  ENDDO
!
  WC%ONE_D%Q_CON_F = SUM(SF%VEG_DIVQNET_L) !*RDN(IW)*WC%VEG_HEIGHT
! qconf(iw) = 0.0_EB
!
! -----------------------------------------------
...
! Compute +/- radiation fluxes and their divergence due to incident fluxes on boundaries
    QRADM_INC = WALL(IW)%ONE_D%Q_RAD_IN/WALL(IW)%ONE_D%EMISSIVITY !sigma*Ta^4 + flame
!   QRADM_INC = WALL(IW)%ONE_D%Q_RAD_IN/WALL(IW)%ONE_D%EMISSIVITY + SIGMA*WALL(IW)%ONE_D%TMP_F**4 ! as done in FDS4
!   WRITE(LU_OUTPUT,*)'vege: WALL(IW)%ONE_D%Q_RAD_IN',WALL(IW)%ONE_D%Q_RAD_IN
