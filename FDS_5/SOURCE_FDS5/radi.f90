...
CHARACTER(255), PARAMETER :: radiid='$Id: radi.f90,v 1.22 2007/03/11 20:02:53 mcgratta Exp $'
...
!!!SUBROUTINE GET_KAPPA(Z_1,Z_2,Z_3,YY_SUM,KAPPA,TYY,IBND)
!!!! GET_KAPPA returns the radiative absorption
!!!USE PHYSICAL_FUNCTIONS, ONLY : GET_F,GET_F_C
!!!INTEGER :: IZ1,IZ2
!!!REAL(EB) :: Z_1,Z_2,Z_3,YY_SUM,F,C,Z_3_MAX,Z,Z_F,KAPPA,WGT,KAPPA_N(N_SPECIES)
!!!INTEGER :: IBND,TYY
!!!
...
!!!selectmethod: IF (SUPPRESSION .AND. (.NOT. CO_PRODUCTION)) THEN
!!!   
!!!   Z_F = REACTION(1)%Z_F
!!!   Z_1 = MAX(0._EB,Z_1)/(1._EB - YY_SUM)
!!!   Z_3 = MAX(0._EB,Z_3)/(1._EB - YY_SUM)
!!!   Z = Z_1 + Z_3
!!!   IF (Z > Z_F) THEN
!!!      IF (Z >= 1._EB) THEN
!!!         F = 1._EB
...
!!!ELSE selectmethod    
!!!   Z_1 = MAX(0._EB,Z_1)/(1._EB - YY_SUM)
!!!   Z_2 = MAX(0._EB,Z_2)/(1._EB - YY_SUM)         
!!!   Z_3 = MAX(0._EB,Z_3)/(1._EB - YY_SUM)           
!!!   Z = Z_1 + Z_2 + Z_3
