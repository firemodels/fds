...
REAL(EB) :: CONCORR,VOLSUM,MFT,ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,DEPTH,UN,H_S,RHO_D_DYDN,U_CELL,V_CELL,W_CELL,&
            AAA,BBB,CCC,ALP,BET,GAM,MMM,LTMP,ATMP,CTMP,H_W_EFF,MTMP,X0,X1,XC0,XC1,TMP_BAR,VOL,DVOL,DN,PRESS,&
...
!            write(*,*) ONE_D%LP_TEMP(LPC%ARRAY_INDEX)-273.15
         ENDIF
      ENDDO
      ! Note, LP_TEMP is not limited above TMPM
      LTMP = LTMP / (ATMP + TWO_EPSILON_EB)
      H_W_EFF = -CTMP / (ONE_D%TMP_F-LTMP+TWO_EPSILON_EB)
      H_W_EFF = MIN(10000._EB,max(0._EB,H_W_EFF));
      write(101,*) ONE_D%TMP_F-273.15_EB,LTMP-273.15_EB,H_W_EFF,ATMP,CTMP
