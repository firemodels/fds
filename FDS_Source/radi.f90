...
DROPLETS: IF (N_EVAP_INDICES>0) THEN
   GET_PC_RADI: DO J=1,N_PART
      ...
      IF (PC%FUEL) THEN  ! Tuntomo, Tien and Park, Comb. Sci. and Tech., 84, 133-140, 1992
         ...
         PC%R50(0) = 0._EB
         DO I=1,NDG
            PC%R50(I)=EXP(I/2.5_EB-4._EB)
         ENDDO
         PC%WQABS=PC%WQABS/10000._EB
         PC%R50=PC%R50/1.E6_EB
      ENDIF
...
!                     CALL INTERPOLATE1D(PC%R50,PC%WQABS(:,IBND), 0.95_EB*AVG_DROP_RAD(I,J,K,EVAP_INDEX),QVAL) 
                     CALL INTERPOLATE1D(PC%DMN/2._EB,PC%WQABS(:,IBND),AVG_DROP_RAD(I,J,K,EVAP_INDEX),QVAL) 
                     KAPPAW(I,J,K) = KAPPAW(I,J,K) + NCSDROP*QVAL
                     IF (PC%WATER) THEN
!                        CALL INTERPOLATE1D(PC%R50,PC%WQSCA(:,IBND),0.95_EB*AVG_DROP_RAD(I,J,K,EVAP_INDEX),QVAL)
                        CALL INTERPOLATE1D(PC%DMN/2._EB,PC%WQSCA(:,IBND),AVG_DROP_RAD(I,J,K,EVAP_INDEX),QVAL)
