...
NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BURN_AWAY,BURN_DURATION,&
                ...
                TMP_BACK,TMP_FRONT,TMP_INNER,TRANSPARENCY,VARIABLE_THICKNESS,&
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (ANY(MOISTURE_FRACTION>TWO_EPSILON_EB) .OR. &
      ...
      LAYER_LOOP_2: DO NL=1,MAX_LAYERS
         ...
         IF (SURFACE_VOLUME_RATIO(NL)>TWO_EPSILON_EB .AND. THICKNESS(NL)<0._EB .AND. .NOT.VARIABLE_THICKNESS) THEN
   ...
   LAYER_LOOP: DO IL=1,MAX_LAYERS
      IF ((ADIABATIC.OR.NET_HEAT_FLUX<1.E12_EB.OR.ABS(CONVECTIVE_HEAT_FLUX)>TWO_EPSILON_EB.OR.TMP_FRONT>-TMPM) &
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (THICKNESS(IL)<=0._EB .AND. .NOT.VARIABLE_THICKNESS .AND. MATL_ID(IL,1)/='null') THEN
   ...
   IF (SF%VARIABLE_THICKNESS) SF%INCLUDE_BOUNDARY_THR_D_TYPE = .TRUE.
   ...
   N_LIST = 0
   NAME_LIST = 'null'
   IF (SF%VARIABLE_THICKNESS) THICKNESS(1) = 1.0_EB  ! Dummy thickness
   SF%THICKNESS  = 0._EB
