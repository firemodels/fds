...
\end{eqnarray}
The basic computational strategy then is to compute a color using (\ref{eq:color}) and an opacity using (\ref{eq:alpha2}) for each pixel in the 2D projected image.  Equations (\ref{eq:alpha2}) and ({\ref{eq:color}) are equivalent to the recursions presented in~\cite[Chapter 39]{gpugems} for performing volume rendering.
