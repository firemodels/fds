...
USE VTK_FDS_INTERFACE, ONLY : WRITE_PARAVIEW_STATE_FILE, WRITE_VTK_GEOM_FILE, WRITE_VTKHDF_GEOM_FILE,&
                              INITIALIZE_VTKHDF_FILES
...
WRITE(*,*) "STARTING FIRST DUMP"
IF (.NOT.RESTART) THEN
   IF (VTK_HDF) CALL INITIALIZE_VTKHDF_FILES(T)
   N_WRITTEN=0
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      CALL UPDATE_GLOBAL_OUTPUTS(T,DT,NM)
      WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " STARTING TO WRITE"
      CALL DUMP_MESH_OUTPUTS(T,DT,NM,.FALSE.)
      N_WRITTEN = N_WRITTEN + 1
   ENDDO
   !WRITE(*,*) "RANK ", MY_RANK, " N_WRITTEN ", N_WRITTEN, " TARGET ", MAXVAL(MESHES_PER_PROCESS)
   DO NM=1,MAXVAL(MESHES_PER_PROCESS)
      IF (N_WRITTEN < MAXVAL(MESHES_PER_PROCESS)) THEN
         WRITE(*,*) "RANK ", MY_RANK, " MESH ", LOWER_MESH_INDEX, " FAKE WRITE"
...
WRITE(*,*) "RANK ", MY_RANK, "GOT PAST FIRST DUMP_MESH_OUTPUT"
...
MAIN_LOOP: DO
   ...
   WRITE(*,*) "STARTING SECOND DUMP_MESH_OUTPUT"
   
   IF (VTK_HDF) CALL INITIALIZE_VTKHDF_FILES(T)
   WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " PAST VTKHDF INIT"
   N_WRITTEN=0
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      CALL DUMP_MESH_OUTPUTS(T,DT,NM,.FALSE.)
      N_WRITTEN = N_WRITTEN + 1
   ...
   WRITE(*,*) "GOT PAST SECOND DUMP_MESH_OUTPUT"
