...
IMPLICIT NONE
PRIVATE
CHARACTER(255), PARAMETER :: readid='$Id: read.f90,v 1.103 2007/04/30 21:06:32 mcgratta Exp $'
...
IF (HUMIDITY < 0._EB) THEN
   ...
   HUMIDITY = 0.4_EB*MIN( 1._EB , X_H2O_40_C/X_H2O_TMPA )
ENDIF
...
IF (I_WATER>0) THEN
   ...
   SPECIES(I_WATER)%YY0 =HUMIDITY*XVAP/(MW_AIR/MW_H2O+(1._EB-MW_AIR/MW_H2O)*XVAP)
ENDIF
...
REAL(EB) :: Y_O2_INFTY,Y_F_INLET, &
            H2_YIELD,SOOT_YIELD,CO_YIELD, X_O2_LL,EPUMO2,BOF, SOOT_H_FRACTION,&
...
REAL(EB) :: TOTAL_MASS,C,FUEL_BURNED,GEN_TO_YLD,ZZ,DZZ,Y_F,FUEL_TO_CO2
...
REAL(EB) :: ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
            ...
            DROPLET_VELOCITY,FLOW_RATE,FLOW_TAU,GAUGE_TEMPERATURE,K_FACTOR,LENGTH,SPRAY_ANGLE(2), &
...
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX
CHARACTER(30) :: CONDUCTIVITY_RAMP,SPECIFIC_HEAT_RAMP,PYROLYSIS_MODEL
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,SPECIFIC_HEAT,DENSITY,HEAT_OF_COMBUSTION,REFERENCE_RATE,ABSORPTION_COEFFICIENT
REAL(EB), DIMENSION(1:MAX_REACTIONS) :: A,E,HEAT_OF_REACTION,NU_FUEL,NU_WATER,NU_RESIDUE,N_S,N_T,REFERENCE_TEMPERATURE,&
...
NAMELIST /MATL/ ID,FYI,SPECIFIC_HEAT,CONDUCTIVITY,CONDUCTIVITY_RAMP,SPECIFIC_HEAT_RAMP, &
                ...
                EMISSIVITY,HEAT_OF_REACTION,DENSITY,RESIDUE,PYROLYSIS_MODEL, &
                HEAT_OF_COMBUSTION,A,E,NU_FUEL,NU_WATER,NU_RESIDUE,N_S,N_T,N_REACTIONS,ABSORPTION_COEFFICIENT
...
! Pack MATL parameters into the MATERIAL derived type
   ...
   ML%H_V(:)               = 1000._EB*HEAT_OF_REACTION(:)
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%GEOMETRY == SURF_CYLINDRICAL) SF%BACKING   = INSULATED
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
   ...
   BLOWING  = .FALSE.
   SUCKING  = .FALSE.
   IF (SF%HRRPUA>0._EB .OR. SF%MLRPUA>0._EB) BURNING = .TRUE.
   IF (SF%N_LAYERS > 0) THEN
      DO NN=1,SF%N_MATL
         ML => MATERIAL(SF%MATL_INDEX(NN))
   ...
   DO NN=1,SF%N_MATL
      ML => MATERIAL(SF%MATL_INDEX(NN))
      IF (ANY(ML%TMP_IGN< 5000._EB))   SF%T_IGN = T_END
      IF (ML%N_REACTIONS>0)            SF%T_IGN = T_END
   ENDDO
...
! Allocate arrays to hold x_s, 1/dx_s (center to center, RDXN_W), 1/dx_s (edge to edge, RDX_W)
...
! Compute node coordinates 
   ...
   CALL GET_WALL_NODE_WEIGHTS(SF%N_CELLS,SF%N_LAYERS,SF%N_LAYER_CELLS,SF%THICKNESS,SF%GEOMETRY, &
         SF%X_S,SF%RDX,SF%RDXN,SF%DX_WGT,SF%DXF,SF%DXB,SF%LAYER_INDEX)
      ...
      OBST_LOOP: DO
         ...
         IF (DEVC_ID/='null' .OR. CTRL_ID/='null') THEN
            ...
            IF (DEVC_ID /='null') THEN
               OB%REMOVABLE = .TRUE.
               OB%HIDDEN = DEVICE(OB%DEVC_INDEX)%INITIAL_STATE               
               DEVICE(OB%DEVC_INDEX)%INITIAL_STATE = .NOT.DEVICE(OB%DEVC_INDEX)%INITIAL_STATE
            ENDIF
            IF (CTRL_ID /='null') THEN
               OB%REMOVABLE = .TRUE.
               OB%HIDDEN = CONTROL(OB%CTRL_INDEX)%INITIAL_STATE
               CONTROL(OB%CTRL_INDEX)%INITIAL_STATE = .NOT.CONTROL(OB%CTRL_INDEX)%INITIAL_STATE
            ENDIF
...
USE DEVICE_VARIABLES, ONLY : DEVICE_TYPE, DEVICE, N_DEVC, N_PROP, PROPERTY
USE CONTROL_VARIABLES
! Process the DEViCes
   ...
   SPECIAL_DETECTION_QUANTITIES: SELECT CASE (DV%QUANTITY)
      ...
      CASE ('CONTROL')
         ...
         IF (DV%CTRL_ID/='null' .AND. DV%CTRL_INDEX<=0) THEN
            ...
            CALL SHUTDOWN(MESSAGE)
         ENDIF
         DV%INITIAL_STATE = CONTROL(NN)%INITIAL_STATE
         DV%SETPOINT = 0.5
         DV%TRIP_DIRECTION = 1
