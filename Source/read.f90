...
DO N1 = 1, N_TRACKED_SPECIES
   IF (SPECIES_MIXTURE(N1)%SINGLE_SPEC_INDEX==O2_INDEX) NU_SOOT_OX(N1) = -(SS%ATOMS(1) * 0.25_EB + SS%ATOMS(6) - SS%ATOMS(8)*0.5_EB) * MW_O2 / SS%MW
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (TRIM(RN%EQUATION)/='null') THEN
      ...
      RN%N_SMIX = 0
      DO NS=1,MIN(MAX_SPECIES,N_TRACKED_SPECIES)
