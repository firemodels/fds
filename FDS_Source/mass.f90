...
!$OMP        UVW_SAVE,ENTHALPY_TRANSPORT,SURFACE,PREDICTOR,CORRECTOR, &
...
WLOOP_FL: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (ENTHALPY_TRANSPORT) THEN
       IF (SURFACE(SURF_INDEX)%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX .OR. &
           ...
           ANY(SURFACE(SURF_INDEX)%LEAK_PATH>0._EB)) THEN
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,TMPMAX,TMPMIN,EVACUATION_ONLY, &
                            ...
                            DEBUG_OPENMP,CLIP_MASS_FRACTION,ENTHALPY_TRANSPORT,N_REACTIONS
REAL(EB) :: DTRATIO,OMDTRATIO,TNOW,ZZ_GET(0:N_TRACKED_SPECIES),CP,H_S
INTEGER  :: I,J,K,N
INTEGER, INTENT(IN) :: NM
...
CASE(.TRUE.) PREDICTOR_STEP
   ...
   !$OMP PARALLEL DEFAULT(NONE) &
   !$OMP SHARED(CHANGE_TIME_STEP,NM,ENTHALPY_TRANSPORT,N_TRACKED_SPECIES,KBAR,JBAR,IBAR,SOLID,CELL_INDEX,ZZS,DT, &
   ...
   !       MASS_FINITE_DIFFERENCES.
      ...
      IF (ENTHALPY_TRANSPORT) TMP0 = TMP
...
CASE(.FALSE.) PREDICTOR_STEP
   ...
   !$OMP PARALLEL DEFAULT(NONE) &
   !$OMP SHARED(ENTHALPY_TRANSPORT,N_TRACKED_SPECIES,KBAR,JBAR,IBAR,SOLID,CELL_INDEX,ZZ,RHO,RHOS,RHO_H_S_OVER_PBAR, &
   ...
   IF (ENTHALPY_TRANSPORT .AND. N_REACTIONS==0) THEN
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
