...
USE VTK_FDS_INTERFACE, ONLY : BUILD_VTK_GAS_PHASE_GEOMETRY,BUILD_VTK_SOLID_PHASE_GEOMETRY,BUILD_VTK_SLICE_GEOMETRY,&
                ...
                ADD_DATA_TO_SMOKE3D_VTKHDF_FILE
...
IF (WRITE_VTK) THEN
   ...
   IF (VTK_HDF) THEN
      ...
      IF (T>=SLCF_VTK_CLOCK(SLCF_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
         !WRITE(*,*) "RANK ", MY_RANK, " INTO DUMP_SLCF_VTK 2D"
         CALL DUMP_SLCF_VTK(T,DT,NM,4,FAKEWRITE)
         !WRITE(*,*) "RANK ", MY_RANK, " OUT OF DUMP_SLCF_VTK 2D"
      ENDIF
      
      ! VTK 2-D slices
      IF (T>=SL3D_VTK_CLOCK(SL3D_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
         !WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " INTO DUMP_SLCF_VTK 3D"
         CALL DUMP_SLCF_VTK(T,DT,NM,3,FAKEWRITE)
         !WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " OUT OF DUMP_SLCF_VTK 3D"
      ENDIF
      
      ! VTK Smoke 3D slices
      IF (T>=SM3D_VTK_CLOCK(SM3D_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
         WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " INTO DUMP_SMOKE3D_VTKHDF"
         CALL DUMP_SMOKE3D_VTKHDF(T,DT,NM,1,FAKEWRITE)
         WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " OUT OF DUMP_SMOKE3D_VTKHDF"
      ENDIF
