...
practical for thermally-driven flow simulations, in particular fire. Some of the major features of the model
are:
\begin{itemize}
\item Low Mach number approximation
\item Unform gridding and a very simple form of the Immersed Boundary Method
...
\end{itemize}
These approximations and assumptions characterize the default operation of the model. However, the model is not limited to
...
of other special features. The more detailed physics incur increased computational cost, often with little increase in
accuracy. So why include them? Because we cannot conclude that a more detailed physical algorithm is of little value until we
...
features to keep, discard, or improve.
...
In practice, we do not solve transport equations for each gas species generated by the combustion process. Usually, it is assumed
...
and therefore solve transport equations for only the Fuel and Products.
