   ...
   DO N=1,M%N_OBST
      ...
      ELSE
         WRITE(LU_SMV,'(6F12.5,I7,6I4,3F13.5,1X,A33)') OB%X1,OB%X2,OB%Y1,OB%Y2,OB%Z1,OB%Z2,OB%ORDINAL, &
            OB%SURF_INDEX(-1),OB%SURF_INDEX(1),OB%SURF_INDEX(-2),OB%SURF_INDEX(2),OB%SURF_INDEX(-3),OB%SURF_INDEX(3), OB%TEXTURE(1),OB%TEXTURE(2),OB%TEXTURE(3),&
            TEMPCHAR
      ENDIF
   ENDDO
   DO N=1,M%N_OBST
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
      E_DIFF = WC%QRADIN-WC%QRADOUT
   ELSE
      E_DIFF = 0._EB
   ENDIF
   FHRR(NM) = FHRR(NM) + (U_N*RHO_N*H_G+E_DIFF)*WALL(IW)%AW
ENDDO WALL_LOOP
