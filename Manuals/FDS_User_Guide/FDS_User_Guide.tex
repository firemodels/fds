...
\begin{equation}
\mu_{\hbox{\tiny LES}} = C_k \Delta \sqrt{k_{sgs}}
\end{equation}
where $C_k=0.1$ and the subgrid scale (sgs) kinetic energy is taken from an algebraic relationship based on scale similarity (see the FDS Technical Reference Guide~\cite{FDS_Math_Guide}).

Options for the turbulent viscosity model specified using {\ct TURBULENCE\_MODEL} on {\ct MISC}:
\begin{center}
\begin{tabular}{ll}
{\ct 'CONSMAG'}   & constant coefficient Smagorinsky model \cite{Smagorinsky:1}, additionally specify {\ct CSMAG}\\
{\ct 'DYNSMAG'}   & dynamic Smagorinsky model \cite{Germano:1991}\\
{\ct 'DEARDORFF'} & Deardorff model \cite{Deardorff:1980} (default) \\
