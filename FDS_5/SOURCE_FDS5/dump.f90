...
IF (.NOT.APPEND .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO N=1,N_ISOF
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') TRIM(IS%SMOKEVIEW_LABEL)
      M%N_STRINGS = M%N_STRINGS + 1
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D .AND..NOT.EVACUATION_ONLY(NM)) THEN
   ...
   IF (.NOT.APPEND) THEN
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') TRIM(SMOKE3D_SMOKEVIEW_LABEL)
      M%N_STRINGS = M%N_STRINGS + 1
...
DO N=1,M%N_SLCF
   ...
   ELSE
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') TRIM(SL%SMOKEVIEW_LABEL)
      M%N_STRINGS = M%N_STRINGS + 1
      ...
      WRITE(LU_SLCF(N,NM)) TRIM(SL%SMOKEVIEW_LABEL)
...
BOUNDARY_FILES: DO NF=1,N_BNDF
   ...
   ELSE RESTART
      ...
      M%N_STRINGS = M%N_STRINGS + 1
      WRITE(M%STRING(M%N_STRINGS),'(A)') TRIM(BF%SMOKEVIEW_LABEL)
      M%N_STRINGS = M%N_STRINGS + 1
      ...
      WRITE(LU_BNDF(NF,NM)) TRIM(BF%SMOKEVIEW_LABEL)
      ...
      ELSE
         ...
         DO N=1,N_PART
            ...
            DO NN=1,PC%N_QUANTITIES
               WRITE(LU_PART(NM)) TRIM(PC%SMOKEVIEW_LABEL(NN))
...
EVAC_ONLY3: IF (.NOT. ALL(EVACUATION_ONLY)) THEN
   DO N=1,N_PART
      ...
      DO NN=1,PC%N_QUANTITIES
         WRITE(LU_SMV,'(A)') TRIM(PC%SMOKEVIEW_LABEL(NN))
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   DO N=1,M%N_OBST
      OB=>M%OBSTRUCTION(N)
      IF (OB%HIDDEN) THEN
         WRITE(LU_SMV,'(A,I6)') 'HIDE_OBST',NM
   ...
   DO N=1,M%N_VENT
      ...
      IF (.NOT.VT%ACTIVATED) THEN
         WRITE(LU_SMV,'(/A,I6)') 'CLOSE_VENT',NM
...
IF (N_ISOF>0) THEN
   ...
   DO N=1,N_ISOF
      IS => ISOSURFACE_FILE(N)
      WRITE(LU_OUTPUT,'(I4,A,A,A,10F8.3)')N,' Quantity: ',IS%SMOKEVIEW_LABEL,', VALUE(s):',(IS%VALUE(I),I=1,IS%N_VALUES)
   ENDDO
ENDIF
...
INTEGER :: IOR,II1,II2,II,JJ,KK,IIG,JJG,KKG,NWP,I_GRAD
...
SELECT CASE(INDX)
   ...
   CASE(34) ! SURFACE DENSITY
      ...
      IF (SURFACE(IJKW(5,IWX))%SHRINK) THEN
         ...
         X_S_NEW(0:NWP) = WALL(IWX)%X_S(0:NWP)
      ELSE
         THICKNESS = SUM(SURFACE(IJKW(5,IWX))%LAYER_THICKNESS)
         NWP = SURFACE(IJKW(5,IWX))%N_CELLS
         X_S_NEW(0:NWP) = SURFACE(IJKW(5,IWX))%X_S(0:NWP)
      ENDIF
      DO II2=1,NWP
         WGT = (X_S_NEW(II2)**(I_GRAD+1)-X_S_NEW(II2-1)**(I_GRAD+1))/SURFACE(IJKW(5,IWX))%THICKNESS**I_GRAD
