...
COUNT_SPEC_LOOP: DO
   ...
   IF (BACKGROUND) THEN 
      IF (LUMPED_SPECIES_ONLY) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Cannot define a LUMPED_SPECIES_ONLY as the BACKGROUND species'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
...
SPEC_READ_LOOP: DO N=1,N_SPEC_READ
   ...
   IF (.NOT. BACKGROUND_ONLY .AND. .NOT. LUMPED_SPECIES_ONLY) THEN
