...
INTEGER I,J,II,JJ,KK,IL,IU,IPC
...
!      DR%Z = 0.5_EB*(Z(KK-1)+Z(KK))
      ...
      IF (PC%DIAMETER>0._EB) THEN
         IF (PC%MONODISPERSE) THEN
            ...
            DR%PWT = 1._EB
         ELSE
            PC%STRATUM = PC%STRATUM + 1
            IF (PC%STRATUM > NSTRATA) PC%STRATUM = 1
            IL = PC%IL_CDF(PC%STRATUM)
            IU = PC%IU_CDF(PC%STRATUM)
            CALL RANDOM_CHOICE(PC%CDF(IL:IU),PC%R_CDF(IL:IU),IU-IL,DR%R)
            DR%PWT = PC%W_CDF(PC%STRATUM)
...
INTEGER :: I,KS,II,JJ,KK,IC,IL,IU,IPC,DROP_SUM,IIG,JJG,KKG,IW,IBC,IOR
