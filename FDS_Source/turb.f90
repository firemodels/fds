...
PUBLIC :: NS_ANALYTICAL_SOLUTION, INIT_TURB_ARRAYS, VARDEN_DYNSMAG, &
          GET_REV_turb, WERNER_WENGLE_WALL_MODEL, COMPRESSION_WAVE, &
...
REAL(EB) :: XX,YY,ZZ,SHAPE_FACTOR,VOLUME_WEIGHTING_FACTOR(3),EDDY_VOLUME(3)
...
RR = (/0._EB,0._EB,1._EB/) ! ray direction
FACET%RAY_TEST=.FALSE.

FACE_LOOP: DO I=1,N_FACE

   ! check that we have not already visited this facet (e.g. on an edge or vertex)
   IF (FACET(I)%RAY_TEST) CYCLE FACE_LOOP
   ...
   IF ( RAY_TRIANGLE_INTERSECT(I,XP,RR) ) THEN
      FACET(I)%RAY_TEST=.TRUE.
