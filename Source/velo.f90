...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      ...
      CASE(SOLID_BOUNDARY)
         ...
         ELSE
            CALL WALL_MODEL(SLIP_COEF,BP%U_TAU,BP%Y_PLUS,&
                            MU_DNS(IIG,JJG,KKG)/RHO(IIG,JJG,KKG),SF%ROUGHNESS,0.5_EB/ONE_D%RDN,VEL_GAS-VEL_T)
         ENDIF
...
ELSE FREEZE_VELOCITY_IF
   ...
   IF (CC_IBM) CALL CCIBM_PROJECT_VELOCITY(NM,DT,STORE_FLG=.FALSE.)
   SELECT CASE(PRES_FLAG)
...
ELSE FREEZE_VELOCITY_IF
   ...
   IF (CC_IBM) CALL CCIBM_PROJECT_VELOCITY(NM,DT,.TRUE.)
   SELECT CASE(PRES_FLAG)
   ...
   IF (CC_IBM) CALL CCIBM_PROJECT_VELOCITY(NM,DT,.FALSE.)
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,ULMAT_FLAG,USCARC_FLAG)
...
REAL(EB) :: MUA,TSI,WGT,T_NOW,RAMP_T,OMW,MU_WALL,RHO_WALL,SLIP_COEF,VEL_T, &
            ...
            TMP_F,TMP_G,K_G,CP_G,LOB,HTC,ZZ_GET(1:N_TRACKED_SPECIES),U_NORM
