...
The movement of droplets and particles within a time step is calculated using an analytical solution and remains stable regardless of the time step.
In some cases with extremely fast droplets or particles, however, the stability of the overall flow behavior may require setting an additional parameter
that limits the time step according to the speed of the fastest particle in the simulation. Setting {\ct PARTICLE\_CFL = .TRUE.} on the {\ct MISC} line
activates this constraint. The actual value of of the constraint is set using {\ct PARTICLE\_CFL\_MAX} parameter. Value 1 means that the fastest moving
particle can move a distance of one grid cell during the time step.
...
%{\ct INCOMPRESSIBLE}                            & Logical       &                                                       &               & {\ct .FALSE.}     \\ \hline
