...
IF (CALL_HT_1D) CALL HEAT_EXCHANGE_3D
...
SUBROUTINE HEAT_EXCHANGE_3D

INTEGER :: NWP,I_NODE,I,II,IWA
REAL(EB) :: TMP_AVG
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.SF%VARIABLE_THICKNESS .OR. WC%BACK_INDEX<=0) CYCLE WALL_LOOP

   BC => BOUNDARY_COORD(WC%BC_INDEX)
   ONE_D => BOUNDARY_ONE_D(WC%OD_INDEX)
   NWP = SUM(ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS))
   THR_D => BOUNDARY_THR_D(WC%TD_INDEX)

   NODE_LOOP: DO I=1,NWP
      THR_D%NODE(I)%TMP_AVG = ONE_D%TMP(I)
      DO II = 1,THR_D%NODE(I)%ADJACENT_WALL_VALUES
         ...
         THR_D%NODE(I)%TMP_AVG = THR_D%NODE(I)%TMP_AVG + THR_D%NODE(I)%ADJACENT_WALL_WEIGHT(II)*ONE_D2%TMP(I_NODE)
      ENDDO
      THR_D%NODE(I)%TMP_AVG = THR_D%NODE(I)%TMP_AVG*ONTH
   ENDDO NODE_LOOP
...
WALL_LOOP_2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.SF%VARIABLE_THICKNESS .OR. WC%BACK_INDEX<=0) CYCLE WALL_LOOP_2
   ...
   DO I=1,NWP
      ONE_D%TMP(I) = THR_D%NODE(I)%TMP_AVG
   ENDDO
ENDDO WALL_LOOP_2

END SUBROUTINE HEAT_EXCHANGE_3D
...
REAL(EB) :: D_Z_N(0:I_MAX_TEMP),D_Z_TEMP,D_Z_P(0:NWP_MAX+1), D_DRHOZDX(0:NWP_MAX),D_BAR,PHI_BAR,D_STAR_BAR,&
            RR_SUM,GAS_DENSITY,POROSITY(0:NWP_MAX+1),DDSUM,SMALLEST_CELL_SIZE(1:MAX_LAYERS),DIM_FACTOR
...
IF (E_WALLB < 0._EB .AND. BACKING /= INSULATED) THEN
   E_WALLB = 0._EB
   VOLSUM  = 0._EB
   IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED .OR. SF%VARIABLE_THICKNESS) THEN
      NWP = SUM(ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS))
   ELSE
      NWP = SF%N_CELLS_INI
   ENDIF
   DO N=1,SF%N_MATL
      ...
      VOLSUM  = VOLSUM  + ONE_D%MATL_COMP(N)%RHO(NWP)/SF%RHO_S(SF%LAYER_INDEX(NWP),N)
      E_WALLB = E_WALLB + ONE_D%MATL_COMP(N)%RHO(NWP)*ML%EMISSIVITY/SF%RHO_S(SF%LAYER_INDEX(NWP),N)
   ENDDO
...
COMPUTE_GRID: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED .OR. SF%VARIABLE_THICKNESS) THEN
   NWP = SUM(ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS))
   ...
   DO I=1,NWP
      IF (SF%KAPPA_S(LAYER_INDEX(I))<0._EB) THEN
         ...
         DO N=1,SF%N_MATL
            ...
            ML  => MATERIAL(SF%MATL_INDEX(N))
            VOLSUM = VOLSUM + ONE_D%MATL_COMP(N)%RHO(I)/SF%RHO_S(SF%LAYER_INDEX(I),N)
            KAPSUM = KAPSUM + ONE_D%MATL_COMP(N)%RHO(I)*ML%KAPPA_S/SF%RHO_S(SF%LAYER_INDEX(I),N)
         ENDDO
...
PYROLYSIS_PREDICTED_IF_2: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
   ...
   POINT_LOOP2: DO I=1,NWP
      ...
      MATERIAL_LOOP1a: DO N=1,SF%N_MATL
         ...
         REGRID_MAX = MAX(REGRID_MAX,ONE_D%MATL_COMP(N)%RHO(I)/SF%RHO_S(SF%LAYER_INDEX(I),N))
         REGRID_SUM = REGRID_SUM + ONE_D%MATL_COMP(N)%RHO(I)/SF%RHO_S(SF%LAYER_INDEX(I),N)
...
POINT_LOOP3: DO I=1,NWP
   ...
   MATERIAL_LOOP3: DO N=1,SF%N_MATL
      ...
      ML  => MATERIAL(SF%MATL_INDEX(N))
      VOLSUM = VOLSUM + ONE_D%MATL_COMP(N)%RHO(I)/SF%RHO_S(SF%LAYER_INDEX(I),N)
      ONE_D%K_S(I) = ONE_D%K_S(I) + ONE_D%MATL_COMP(N)%RHO(I)*ML%K_S(ITMP)/SF%RHO_S(SF%LAYER_INDEX(I),N)
      ONE_D%RHO_C_S(I) = ONE_D%RHO_C_S(I) + ONE_D%MATL_COMP(N)%RHO(I)*ML%C_S(ITMP)

      IF (.NOT.E_FOUND) ONE_D%EMISSIVITY = ONE_D%EMISSIVITY + ONE_D%MATL_COMP(N)%RHO(I)*ML%EMISSIVITY/SF%RHO_S(SF%LAYER_INDEX(I),N)
      RHO_S(I) = RHO_S(I) + ONE_D%MATL_COMP(N)%RHO(I)
      POROSITY(I) = POROSITY(I) + ONE_D%MATL_COMP(N)%RHO(I)*ML%POROSITY/SF%RHO_S(SF%LAYER_INDEX(I),N)
...
IF (SF%VARIABLE_THICKNESS) THEN
   DIM_FACTOR = ONTH
ELSE
   DIM_FACTOR = 1._EB
ENDIF

KODXF = DIM_FACTOR*ONE_D%K_S(0)/DXF
KODXB = DIM_FACTOR*ONE_D%K_S(NWP)/DXB
