...
\be
    {\bof_b} = \frac{1}{V} \sum_{d\in V} \left[ \ha \rho C_D \pi  r_d^2 (\bu_d-\bu) |\bu_d-\bu| - \frac{\d m_d}{\d t}(\bu_d-\bu) \right]
\ee
...
\be F(d) = \left\{ \begin{array}{ll}
   ...
   \mathrm{e}^{-\frac{[\ln(d'/d_m)]^2}{2\sigma^2}} \; \d d'    & (d \le d_m) \\ [0.2in]
   1 - \mathrm{e}^{-0.693 \left(\frac{d}{d_m}\right)^\gamma }  & (d_m < d)
   \end{array} \right.  \ee
...
\be f(d) = \frac{F'(d)}{d^3}  \left/ \int_0^\infty \, \frac{F'(d')}{d'^3}
     \, \d d' \right. \quad ; \quad F' \equiv \frac{\d F}{\d d}   \ee
...
\be
   Y_l^{n+1} \approx Y_l^n + \left( \frac{\d Y_l}{\d T_l} \right)^n \; \Big( T_l^{n+1}-T_l^n \Big)
\ee
where the derivative of $Y_l$ with respect to temperature is obtained via the chain rule:
\be
   \frac{\d Y_l}{\d T_l} = \frac{\d Y_l}{\d X_l} \, \frac{\d X_l}{\d T_l}  = \frac{W_a/W_l}{ (X_l (1-W_a/W_l) + W_a/W_l)^2 } \;
...
\be
   \delta m_l = -\dt \, A \, h_m \, \rho  \left[  Y_l^n + \ha \left( \frac{\d Y_l}{\d T_l} \right)^n \;
...
\be \dm_f''(t) = \dm_{f,0}''(t) \; \mathrm{e}^{-\int k(t) \, \d t}
\label{nistexting} \ee
...
\ee
The force vector $\bof_b$ in Eq.~(\ref{momentum}) represents the momentum transferred from the screen to the gas:
\be
   \bof_b = l \; \left( \frac{\mu}{K} + \rho \frac{Y}{\sqrt{K}} |\bu| \right) \left( \frac{u}{\dx} , \frac{v}{\dy} , \frac{w}{\dz} \right)
\ee
