...
DO NL=1,ONE_D%N_LAYERS
   ...
   CALL GET_N_LAYER_CELLS(ONE_D%MIN_DIFFUSIVITY(NL),ONE_D%SWELL_RATIO(NL)*ONE_D%LAYER_THICKNESS(NL),ONE_D%STRETCH_FACTOR(NL), &
...
IF (ALLOCATED(ONE_D%REMESH_NWP)) ONE_D%REMESH_NWP(1:ONE%N_LAYERS) = ONE_D%N_LAYER_CELLS(1:ONE%N_LAYERS)
...
IF (ALLOCATED(ONE_D%DX_OLD)) THEN
   DEALLOCATE(ONE_D%DX_OLD)
   ALLOCATE(ONE_D%DX_OLD(ONE_D%N_CELLS_MAX))
   DO II=1,ONE_D%N_CELLS_INI
      ONE_D%DX_OLD(II) = ONE_D%X(II) - ONE_D%X(II-1)
   ENDDO
ENDDO
IF ALLOCATED(ONE_D%LAYER_THICKNESS_OLD)) ONE_D%LAYER_THICKNESS_OLD(1:ONE%N_LAYERS) = ONE_D%LAYER_THICKNESS(1:ONE%N_LAYERS)
...
INTEGER :: II,JJ,KK,IC,ICG,IOR,NOM,ITER,OBST_INDEX,OBST_INDEX_PREVIOUS,NN,NNN,NL,N_LAYERS_OBST,&
           N_MATL_OBST,N_LAYERS,N_MATLS,IIF,JJF,KKF,N_MATL_OBST_TEMP,N_MATL_TEMP
INTEGER, DIMENSION(MAX_MATERIALS) :: MATL_INDEX_OBST,MATL_INDEX,MATL_OBST_TEMP,MATL_TEMP
...
REAL(EB), DIMENSION(MAX_LAYERS) :: LAYER_THICKNESS_OBST,HEAT_SOURCE,HEAT_SOURCE_OBST,&
                                   ...
                                   CELL_SIZE_FACTOR,CELL_SIZE_FACTOR_OBST,SWELL_RATIO
...
REAL(EB) :: XXC,YYC,ZZC,THICKNESS,OLD_THICKNESS,FRONT_LINING_THICKNESS,BACK_LINING_THICKNESS,LAYER_THICKNESS_OBST_TOTAL,&
            LAYER_DENSITY,MINIMUM_DENSITY
CHARACTER(MESSAGE_LENGTH) :: MESSAGE
LOGICAL :: THIN_OBSTRUCTION,OBST_REAC
TYPE (MESH_TYPE), POINTER :: M
TYPE (WALL_TYPE), POINTER :: WC
TYPE (BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE (BOUNDARY_ONE_D_TYPE), POINTER :: ONE_D
...
FIND_BACK_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
   ...
   VT_HT3D_IF: IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN 
...
IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN
   ...
   DO NL=1,SF%N_LAYERS
      ...
      SWELL_RATIO(N_LAYERS)       = SF%SWELL_RATIO(N_LAYERS)
   ...
   DO NL=1,N_LAYERS_OBST
      ...
      DO NN=1,N_MATL_OBST
         DO NNN=1,N_MATLS
            IF (MATL_INDEX_OBST(NN)==MATL_INDEX(NNN)) THEN
               MATL_MASS_FRACTION(N_LAYERS,NNN) = MATL_MASS_FRACTION_OBST(NL,NN)
               LAYER_DENSITY = LAYER_DENSITY + MATL_MASS_FRACTION_OBST(NL,NN)/MATERIAL(MATL_INDEX_OBST(NN))%RHO_S
               IF (ANY(MATERIAL(MATL_INDEX_OBST(NN))%N_RESIDUE > 0)) OBST_REAC = .TRUE.
               IF (MATL_MASS_FRACTION_OBST(NL,NN) > TWO_EPSILON_EB) THEN
                  N_MATL_OBST_TEMP = N_MATL_OBST_TEMP + 1
                  MATL_OBST_TEMP(N_MATL_OBST_TEMP) = MATL_INDEX_OBST(NN)
               ENDIF
            ENDIF
         ENDDO
      ENDDO
      IF (OBST_REAC) THEN  
         LAYER_DENSITY = 1._EB/LAYER_DENSITY
         MINIMUM_DENSITY = 10000000._EB
         CALL ADD_MATERIAL(N_MATL_OBST_TEMP,MATL_OBST_TEMP,N_MATL_TEMP,MATL_TEMP)
         DO NN =1,N_MATL_TEMP
            MINIMUM_DENSITY = MIN(MINIMUM_DENSITY,MATERIAL(MATL_TEMP(NN))%RHO_S)
         ENDDO
         SWELL_RATIO(N_LAYERS) = LAYER_DENSITY/MINIMUM_DENSITY
      ENDIF
   ENDDO
   ...
   IF (ALLOCATED(ONE_D%LAYER_THICKNESS_OLD)) THEN
      DEALLOCATE(ONE_D%LAYER_THICKNESS_OLD)
      ALLOCATE(ONE_D%LAYER_THIKNERSS_OLD(ONE_D%N_LAYERS)
   ...
   DEALLOCATE(ONE_D%SWELL_RATIO)       ; ALLOCATE(ONE_D%SWELL_RATIO(ONE_D%N_LAYERS))       ; ONE_D%SWELL_RATIO = 1._EB
   ...
   ONE_D%SWELL_RATIO(1:ONE_D%N_LAYERS)       = SWELL_RATIO(1:ONE_D%N_LAYERS)
...
INTEGER :: II,JJ,SEARCH_LIST(N_MATL)

MATL_INDEX_X = 0

DO JJ=1,N_MATLS_SEARCH
   SEARCH_LIST(MATL_INDEX_SEARCH(JJ)) = 1
ENDDO

DO JJ=1,N_MATL
   IF (SEARCH_LIST(JJ) > 0) SEARCH_LIST = SEARCH_LIST + MATERIAL(JJ)%CHILD_MATL
ENDDO

WHERE (SEARCH_LIST > 1) SEARCH_LIST=1

N_MATLS_X = SUM(SEARCH_LIST)
II = 0
MAKE_MATL_INDEX_X: DO JJ=1,N_MATL
   IF (SEARCH_LIST(JJ) > 0) THEN
      II = II + 1
      MATL_INDEX_X(II) = JJ
      IF (II==N_MATLS_X) EXIT MAKE_MATL_INDEX_X
   ENDIF
ENDDO MAKE_MATL_INDEX_X
