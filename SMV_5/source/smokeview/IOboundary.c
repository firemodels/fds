...
char IOboundary_revision[]="$Revision: 800 $";
...
//  FREEMEMORY(meshi->patchfacevis);
  ...
  if(meshi->npatches>0){
    if(
 //      NewMemory((void **)&meshi->patchfacevis,7*nbb*sizeof(int)   )==0||
...
//  for(n=0;n<nbb;n++){
//    meshi->patchfacevis[7*n+6]=0;
//    for(j=0;j<6;j++){
//      meshi->patchfacevis[7*n+6]+=meshi->patchfacevis[7*n+j];
//    }
//    if(meshi->patchfacevis[7*n+6]!=0)meshi->patchfacevis2=1;
//  }
  for(n=0;n<meshi->nbptrs;n++){
    bc=meshi->blockageinfoptrs[n];
    bc->patchvis[6]=0;
...
//            bc=meshi->blockageinfoptrs[iblock];
            ...
            surf_index = meshi->patch_surfindex[n];
            surfi = bc->surf[surf_index];
            if(surfi==NULL){
              chartemp_ign=1000000;
            }
            else{
              chartemp_ign=surfi->temp_ignition;
            }
            switch (loadpatchbysteps){
            case 0:
            case 1:
              for(j=0;j<nsize;j++){
                if(meshi->chartime[nn+j]<0.0&&meshi->pqqi[nn+j]>=chartemp_ign){
                  meshi->chartime[nn+j]=meshi->patchtimes[i];
                }
              }
              break;
            case 2:
              ...
              for(j=0;j<nsize;j++){
                ...
                if(meshi->chartime[nn+j]<0.0&&val>=chartemp_ign){
                  meshi->chartime[nn+j]=meshi->patchtimes[i];
                }
              }
              break;
...
void drawpatch_texture_char(const mesh *meshi){
  float r11, r12, r21, r22;
  int n,nn,nn1,nn2;
  int nrow, ncol, irow, icol;
  float *xyzp1, *xyzp2;
      ...
      for(irow=0;irow<nrow-1;irow++){
        ...
        for(icol=0;icol<ncol-1;icol++){
          if(*ip1==1&&*ip2==1&&*(ip1+1)==1&&*(ip2+1)==1){
            ...
            color21=clear_color;
            color22=clear_color;
            if(meshi->chartime[nn1+icol  ]>=0.0&&times[itime]>meshi->chartime[nn1+icol  ])color11=burn_color;
            if(meshi->chartime[nn1+icol+1]>=0.0&&times[itime]>meshi->chartime[nn1+icol+1])color12=burn_color;
            if(meshi->chartime[nn2+icol  ]>=0.0&&times[itime]>meshi->chartime[nn2+icol  ])color21=burn_color;
            if(meshi->chartime[nn2+icol+1]>=0.0&&times[itime]>meshi->chartime[nn2+icol+1])color22=burn_color;
      ...
      for(irow=0;irow<nrow-1;irow++){
        ...
        for(icol=0;icol<ncol-1;icol++){
          if(*ip1==1&&*ip2==1&&*(ip1+1)==1&&*(ip2+1)==1){
            ...
            color21 = NULL;
            color22 = NULL;
            if(meshi->chartime[nn1+icol  ]>=0.0&&times[itime]>meshi->chartime[nn1+icol  ]){
              color11=&char_color[0];
              nnulls--;
            }
            if(meshi->chartime[nn1+icol+1]>=0.0&&times[itime]>meshi->chartime[nn1+icol+1]){
              color12=&char_color[0];
              nnulls--;
            }
            if(meshi->chartime[nn2+icol  ]>=0.0&&times[itime]>meshi->chartime[nn2+icol  ]){
              color21=&char_color[0];
              nnulls--;
            }
            if(meshi->chartime[nn2+icol+1]>=0.0&&times[itime]>meshi->chartime[nn2+icol+1]){
...
//          patchfacevis[7*i+1]=0;
    ...
    for(i=0;i<meshi->nbptrs;i++){
      ...
      if(i1==bc->ijk[IMIN]&&i2==bc->ijk[IMAX]&&
         k1==bc->ijk[KMIN]&&k2==bc->ijk[KMAX]){
        if(j1==bc->ijk[JMIN]){
//          patchfacevis[7*i+2]=0;
          bc->patchvis[2]=0;
          *blockonpatch=i;
          *meshonpatch=meshi;
        }
        if(j1==bc->ijk[JMAX]){
//          patchfacevis[7*i+3]=0;
          bc->patchvis[3]=0;
          *blockonpatch=i;
          *meshonpatch=meshi;
        }
    ...
    for(i=0;i<meshi->nbptrs;i++){
      ...
      if(i1==bc->ijk[IMIN]&&i2==bc->ijk[IMAX]&&
        ...
        if(k1==bc->ijk[KMIN]){
//          patchfacevis[7*i+4]=0;
          bc->patchvis[4]=0;
          *blockonpatch=i;
          *meshonpatch=meshi;
        }
        if(k1==bc->ijk[KMAX]){
//          patchfacevis[7*i+5]=0;
          bc->patchvis[5]=0;
          *blockonpatch=i;
          *meshonpatch=meshi;
        }
