...
NAMELIST /MISC/ AEROSOL_AL2O3,AEROSOL_SCRUBBING,AGGLOMERATION,ALIGNMENT_TOLERANCE,ALLOW_SURFACE_PARTICLES, &
                ...
                TEST_NEW_OPEN,TEST_CLIP_STABILITY_CHECK,TEXTURE_ORIGIN,&
...
ELSEIF (RESEARCH_MODE .OR. SIMULATION_MODE=='LES') THEN
   ...
   IF (NEAR_WALL_TURBULENCE_MODEL=='null') NEAR_WALL_TURBULENCE_MODEL = 'WALE'
...
!                  write(*,*) NL,NN,NNN,INIT_SPEC_ID(NL,NN)
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (.NOT.ALLOCATED(SF%N_LAYER_CELLS)) THEN
      ...
      SF%TMP_INNER(:) = TMPA
!      IF (SF%MT1D) THEN
!         ALLOCATE(SF%SPEC_0(0:SF%N_CELLS_MAX+1,SF%N_SPEC))
!         SF%SPEC_0 = 0._EB
!      ENDIF
   ENDIF
...
!REAL(EB) :: RR_SUM,ZZ_GET(1:N_TRACKED_SPECIES)
...
SURF_GRID_LOOP: DO SURF_INDEX=0,N_SURF
   ...
   ! Initialize the gas densities inside the solid (porous media)
   IF (SF%MT1D) THEN
      SF%SPEC_0 = 0._EB
      DO NL=1,SF%N_LAYERS
         ...
         ELSE
            ...
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
!         ZZ_GET(1:N_TRACKED_SPECIES) = SF%LAYER_SPEC_FRAC(NL,1:N_TRACKED_SPECIES)
...
!            GAS_DENSITY = P_INF/(SF%TMP_INNER(IL)*RR_SUM)
            SF%SPEC_0(II,N) = SF%LAYER_SPEC_FRAC(IL,N)
         ENDDO
      ENDDO
   ENDIF
