...
SUBROUTINE GET_BOUNDARY_PRESSURE(NM1,NM2)

USE POIS, ONLY: H2CZIS,H2CZSS
USE GLOBAL_CONSTANTS, ONLY: PREDICTOR
...
TYPE PETIT_POISSON_TYPE
   ...
   INTEGER :: IBAR,JBAR,KBAR,ITRN,JTRN,KTRN,LSAVE,LWORK,LBC,MBC,NBC,N_LAYERS=3,NM1=0,NM2=0
...
IF (PP%FIRST_CALL(FACE_INDEX)) THEN
   ...
   PP%KBAR = M1%KBAR
   PP%ITRN = PP%IBAR + 1
   IF (PP%JBAR==1) THEN ; PP%JTRN=PP%JBAR ; ELSE ; PP%JTRN=PP%JBAR+1 ; ENDIF
   PP%KTRN = PP%KBAR + 1
   PP%LSAVE = (PP%ITRN+1)*PP%JTRN*PP%KTRN+7*PP%ITRN+5*PP%JTRN+6*PP%KTRN+56
   PP%LWORK = (PP%ITRN+1)*PP%JTRN*PP%KTRN
   ...
   PP%ZS = M1%ZS
   PP%ZF = M1%ZF
   CALL H2CZIS(PP%XS,PP%XF,PP%IBAR,PP%LBC,PP%ZS,PP%ZF,PP%KBAR,PP%NBC,PP%HX,PP%XLM,PP%ITRN,IERR,PP%SAVE)
   ALLOCATE(PP%BXS(PP%JTRN,PP%KTRN))
   ALLOCATE(PP%BXF(PP%JTRN,PP%KTRN))
   ALLOCATE(PP%BYS(PP%ITRN,PP%KTRN))
   ALLOCATE(PP%BYF(PP%ITRN,PP%KTRN))
   ALLOCATE(PP%BZS(PP%ITRN,PP%JTRN))
   ALLOCATE(PP%BZF(PP%ITRN,PP%JTRN))
   ALLOCATE(PP%PRHS(PP%ITRN,PP%JTRN,PP%KTRN))
   PP%FIRST_CALL(FACE_INDEX) = .FALSE.
ENDIF

PP%PRHS(            1:  PP%N_LAYERS,1:PP%JBAR,1:PP%KBAR) = M1%PRHS(M1%IBAR-PP%N_LAYERS+1:M1%IBAR    ,1:M1%JBAR,1:M1%KBAR)
PP%PRHS(PP%N_LAYERS+1:2*PP%N_LAYERS,1:PP%JBAR,1:PP%KBAR) = M2%PRHS(1                    :PP%N_LAYERS,1:M1%JBAR,1:M1%KBAR)

PP%BXS(1:PP%JBAR,1:PP%KBAR) = 0.5_EB*(HP1(M1%IBAR-PP%N_LAYERS,1:M1%JBAR,1:M1%KBAR)+HP1(M1%IBAR-PP%N_LAYERS+1,1:M1%JBAR,1:M1%KBAR))
PP%BXF(1:PP%JBAR,1:PP%KBAR) = 0.5_EB*(HP2(PP%N_LAYERS        ,1:M1%JBAR,1:M1%KBAR)+HP2(PP%N_LAYERS+1        ,1:M1%JBAR,1:M1%KBAR))

PP%BZS(            1:  PP%N_LAYERS,1:M1%JBAR) = M1%BZS(M1%IBAR-PP%N_LAYERS+1:M1%IBAR ,1:M1%JBAR)
PP%BZF(PP%N_LAYERS+1:2*PP%N_LAYERS,1:M2%JBAR) = M2%BZS(                    1:PP%N_LAYERS,1:M2%JBAR)

CALL H2CZSS(PP%BXS,PP%BXF,PP%BZS,PP%BZF,PP%ITRN,PP%PRHS,PP%POIS_PERTURBATION,PP%SAVE,PP%WORK,PP%HX)
