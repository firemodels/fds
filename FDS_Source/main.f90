...
INTEGER, ALLOCATABLE, DIMENSION(:) :: REQ,REQ1,REQ3,REQ4,REQ5,COUNTS,DISPLS,COUNTS2D,DISPLS2D,COUNTS_TIMERS,DISPLS_TIMERS, &
...
INTEGER :: N_REQ,N_REQ1,N_REQ3,N_REQ4,N_REQ5,N_COMMUNICATIONS,INTEGER_SEND_BUFFER(7),INTEGER_RECV_BUFFER(7)
...
WRITE(VERSION_STRING,'(A)') 'FDS 6.1.1'
...
MAIN_LOOP: DO  
   ...
   IF (USE_MPI) CALL MPI_ALLGATHERV(MPI_IN_PLACE,COUNTS(MYID),MPI_DOUBLE_PRECISION, &
   ...
   IF (SYNCHRONIZE) THEN
      ...
      IF (USE_MPI) CALL MPI_ALLGATHERV(MPI_IN_PLACE,COUNTS(MYID),MPI_DOUBLE_PRECISION,DT_SYNC,COUNTS,DISPLS, &
   ...
   IF (USE_MPI) CALL MPI_ALLGATHERV(MPI_IN_PLACE,COUNTS(MYID),MPI_LOGICAL, &
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      COMPUTE_DIVG_LOOP_A: DO NM=1,NMESHES
         IF (PROCESS(NM)/=MYID .OR. .NOT.ACTIVE_MESH(NM)) CYCLE COMPUTE_DIVG_LOOP_A
         CALL DIVERGENCE_PART_1(T(NM),NM)
      ENDDO COMPUTE_DIVG_LOOP_A
      ...
      PREDICT_VELOCITY_LOOP: DO NM=1,NMESHES
         ...
         IF (PERIODIC_TEST==8 .AND. MESHES(NM)%MESH_LEVEL>=0) CALL NS_ANALYTICAL_SOLUTION(NM,T(NM)+MESHES(NM)%DT,RK_STAGE=1)
   ...
   IF (MESH_REFINEMENT .AND. N_REACTIONS>0) THEN
      DO NM=NMESHES,1,-1
         DO NOM=NMESHES,1,-1
            IF (MESHES(NOM)%MESH_LEVEL<=MESHES(NM)%MESH_LEVEL) CYCLE
            CALL RESTRICT_MASS_EMB(NM,NOM,IERROR)
   ...
   CORRECT_VELOCITY_LOOP: DO NM=1,NMESHES
      ...
      IF (PERIODIC_TEST==8 .AND. MESHES(NM)%MESH_LEVEL>=0) CALL NS_ANALYTICAL_SOLUTION(NM,T(NM),RK_STAGE=2)
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (CODE==0 .OR. CODE==6) THEN
      N_REQ = MIN(N_REQ+1,SIZE(REQ))      
      CALL MPI_IRECV(M3%BOUNDARY_TYPE(0),SIZE(M3%BOUNDARY_TYPE),MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD, REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==6 .AND. EXCHANGE_RADIATION .AND. M3%NIC_S>0) THEN
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%REAL_RECV_PKG5(1),SIZE(M3%REAL_RECV_PKG5),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==7 .AND. OMESH_PARTICLES .AND. (M3%NIC_S>0 .OR. M3%NIC_R>0)) THEN
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%N_PART_ADOPT,SIZE(M3%N_PART_ADOPT),MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
   ...
   RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
      ...
      IF (CODE==0 .OR. CODE==6) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M5%BOUNDARY_TYPE(0),SIZE(M5%BOUNDARY_TYPE),MPI_INTEGER,SNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      SEND_RADIATION: IF ( CODE==6 .AND. M3%NIC_R>0 .AND. EXCHANGE_RADIATION) THEN
         ...
         IF (RNODE/=SNODE) THEN
            ...
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG5(1),SIZE(M3%REAL_SEND_PKG5),MPI_DOUBLE_PRECISION,SNODE,NM,MPI_COMM_WORLD,&
                           REQ(N_REQ),IERR)
         ELSE
      ...
      IF (CODE==7 .AND. OMESH_PARTICLES) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%N_PART_ORPHANS,SIZE(M3%N_PART_ORPHANS),MPI_INTEGER,SNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
...
IF (USE_MPI .AND. CODE/=1 .AND. CODE/=3 .AND. CODE/=4 .AND. CODE/=5 .AND. N_REQ>0) &
...
IF_DUMP_HRR: IF (T>=HRR_CLOCK) THEN
   IF (USE_MPI) CALL MPI_ALLGATHERV(MPI_IN_PLACE,CNT,MPI_DOUBLE_PRECISION, &
...
USE HVAC_ROUTINES, ONLY: NODE_TMP,NODE_RHO,DUCT_MF,NODE_ZZ
REAL(EB) :: TNOW
