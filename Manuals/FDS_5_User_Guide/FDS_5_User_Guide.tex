...
Example input files for FDS can be found in the FDS-SMV project Subversion repository.  There are a few ways to get a copy of these files but the two easiest methods will be covered here.  Some of these files are used for example input throughout this document, especially in Part \ref{part:samples}.
...
\item From the command line (Terminal) change to the directory where you want the new directory of files to appear and enter the following string into the command prompt. This command will create a directory for you named Test\_Case\_Files and download all of the example cases into directories within it.
\footnotesize
\begin{verbatim}
...
This method is not the most efficient way to get all of the test cases to your local computer. Instead, it is a convenient way to review the individual files online within the repository browser, and to selectively download only the files that you are interested in.
\begin{enumerate}
\item Go to the FDS-SMV Project Site \href{http://fds-smv.googlecode.com}{{\ct http://fds-smv.googlecode.com}}.  Navigate to the Source Tab and then click Browse between 'Checkout' and 'Changes' next to the 'Search Trunk' text box.  You can also click this \href{http://code.google.com/p/fds-smv/source/browse/trunk/FDS/trunk/Test_cases/}{{\ct Link to Test\_cases}}, to go directly to the Test\_cases directory in the repository.
