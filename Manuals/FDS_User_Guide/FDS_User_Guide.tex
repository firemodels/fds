...
In default mode, FDS uses the dynamic form of the Smagorinsky Large Eddy Simulation (LES) to model subgrid-scale turbulence.
...
where $C_s$ is a function of the local flow conditions and $\Delta$ is a length on the
...
$\PR_t=0.5$, and $\SC_t=0.5$ via the parameters {\ct PR}, and {\ct SC} on the {\ct MISC} line. A more detailed
discussion of these parameters is given in the FDS Technical Reference Guide~\cite{FDS_Math_Guide}.

The dynamic form of LES is more costly than the constant coefficient form. To save on CPU time, you can try setting
{\ct COMPUTE\_VISCOSITY\_TWICE} equal to {\ct .FALSE.} to force FDS to compute the viscosity only once per time step. You can
also reduce the number of times that the dynamic coefficient, $C_s$, is computed by setting {\ct DSMAG\_FREQ} equal to an
integer greater than or equal to 1. A value of 3, for example, means that $C_s$ should be computed every third time that the
...
{\ct SPEC} line with the {\ct BACKGROUND\_SPECIES} and if needed {\ct BACKGROUND\_SPECIES\_ONLY} keywords.
If this species is not listed in Table~\ref{tab:gasspecies}, specify its molecular weight, {\ct MW},
...
{\ct FORMALDEHYDE}       & 30   & 3.626     &481.8           &    Y   \\ \hline
...
{\ct NITROGEN DIOXIDE}   & 46   & 3.992     &204.88          &    Y   \\ \hline
{\ct NITROUS OXIDE}      & 44   & 3.828     &232.4           &    Y   \\ \hline
...
{\ct SULFUR HEXAFLUORIDE}&146   & 5.128     &146.0           &        \\ \hline
{\ct SULFUR DIOXIDE}     & 64   & 4.112     &335.4           &        \\ \hline
...
This parameter is independent of the inputs for the fuel chemistry, i.e. {\ct C, H, O, N, OTHER}. Table~\ref{tab:gasspecies} provides a listing of the available species.
...
&SMIX ID='AIR', SPEC_ID='NITROGEN','OXYGEN','CARBON DIOXIDE','WATER VAPOR',
      MASS_FRACTION=0.75778,0.21312,0.00577,0.00524, BACKGROUND=.TRUE. /
...
{\ct NUMBER\_INITIAL\_DROPLETS} to indicate the number of particles/droplets within the computational domain at the start of the simulation.
...
for {\ct XB} is the same as that used on the {\ct OBST} line. For the moment, you can only introduce a cloud of particles within a rectangular block. In future
versions, the parameter {\ct SHAPE} will be used to indicate other types of initial volumes.
...
\section{Special Topic: Suppression by Water (Mixing-Controlled Combustion Model Only)}
...
{\ct XB(6)}                     & Real Sextuplet    & Rectangular block coordinates                     & m             &               \\ \hline
\end{longtable}

% Undocumented: PROF_ID, ORIGIN, XYZ, UVW, DIAMETER
...
{\ct REDUCE\_TRIANGLES}     & Integer       & Sampling parameter to reduce size     &       & 1       \\ \hline
...
%{\ct CHECK\_GR}                 & Logical       &                                                           &               & {\ct .FALSE.}     \\ \hline
%{\ct CHECK\_VN}                 & Logical       &                                                           &               & {\ct .FALSE.}     \\ \hline
...
{\ct EVAC\_SURF\_DEFAULT}       & Character     & See FDS+Evac User's Guide                                 &               & {\ct 'INERT'}     \\ \hline
...
%{\ct STORE\_MU\_DNS}            & Logical       &                                                           &               & {\ct .FALSE.}      \\ \hline
{\ct STRATIFICATION}            & Logical       & See Section~\ref{info:stratification}                     &               & {\ct .TRUE.}      \\ \hline
{\ct SURF\_DEFAULT}             & Character     & See Section~\ref{info:MISC_Basics}                        &               & {\ct 'INERT'}     \\ \hline
...
%Undocumented: H_LOGLAW, H_CHILTON_COLBURN, LIMITING_DT_RATIO, VAN_DRIEST, HRRPUVCUT_MAX, RUN_AVG_FAC, THERMOPHORETIC_DEPOSITION, TURBULENT_DEPOSITION,
%               VEG_LEVEL_SET, CP_FTMP, HRRPUV_MAX_SMV, TERRAIN_IMAGE,N EW_EVAP, SCALAR_ENERGY_TOLERANCE, TKE_TOLERANCE, MEAN_FORCING, RFAC_FORCING, USE_MAX_FILTER_WIDTH,
%               WFDS,PATCH_VELOCITY,UVW_FILE
...
%Undocumented: REYNOLDS_STRESS, L_EDDY, N_EDDY, VEL_RMS
...
\hline
\endhead
{\ct BACKGROUND\_SPECIES}       & {\ct MISC}    & {\ct BACKGROUND=.TRUE.}                     &   {\ct SPEC}        & See Section~\ref{info:SPEC}                  \\ \hline
{\ct CONDUCTIVITY}              & {\ct MISC}    & {\ct CONDUCTIVITY}                          &   {\ct SPEC}        & See Section~\ref{info:SPEC}                  \\ \hline
{\ct CO\_PRODUCTION}            & {\ct MISC}    & {\ct CO\_PRODUCTION}                        &   {\ct REAC}        & See Section~\ref{info:CO}                    \\ \hline
{\ct HUMIDITY}                  & {\ct MISC}    & {\ct HUMIDITY}                              &   {\ct SPEC}        & See Section~\ref{info:SPEC}                  \\ \hline
{\ct ISOTHERMAL}                & {\ct MISC}    & Eliminated                                  &                     &                                              \\ \hline
{\ct MASS\_FRACTION}            & {\ct VENT}    & Eliminated                                  &                     &                                              \\ \hline
{\ct MAXIMUM\_MASS\_FRACTION}   & {\ct CLIP}    & Eliminated                                  &                     &                                              \\ \hline
{\ct MINIMUM\_MASS\_FRACTION}   & {\ct CLIP}    & Eliminated                                  &                     &                                              \\ \hline
{\ct MW}                        & {\ct MISC}    & {\ct MW}                                    &   {\ct SPEC}        & See Section~\ref{info:SPEC}                  \\ \hline
...
{\ct PRESSURE\_CORRECTION}      & {\ct MISC}    & Eliminated                                  &                     &                                              \\ \hline
{\ct RADIATION}                 & {\ct MISC}    & {\ct RADIATION}                             &   {\ct RADI}        & Same functionality                           \\ \hline
{\ct RESIDUE}                   & {\ct MATL}    & {\ct MATL\_ID}                              &   {\ct MATL}        & See Section~\ref{info:solid_pyrolysis}       \\ \hline
{\ct STATE\_FILE}               & {\ct DUMP}    & Eliminated                                  &                     &                                              \\ \hline
{\ct SUPPRESSION}               & {\ct MISC}    & {\ct SUPPRESSION}                           &   {\ct REAC}        & See Section~\ref{info:extinction}            \\ \hline
{\ct VISCOSITY}                 & {\ct MISC}    & {\ct VISCOSITY}                             &   {\ct SPEC}        & See Section~\ref{info:SPEC}                  \\ \hline
\end{longtable}
