...
REAL(EB) :: FUEL_C_TO_CO_FRACTION_GLOBAL       !< Fraction of C atoms in the fuel that are converted to CO
REAL(EB) :: FUEL_H_TO_H2_FRACTION_GLOBAL       !< Fraction of H atoms in the fuel that are converted to H2
REAL(EB) :: FUEL_N_TO_HCN_FRACTION_GLOBAL      !< Fraction of N atoms in the fuel that are converted to HCN
REAL(EB) :: SOOT_H_FRACTION_GLOBAL             !< Atom fraction of H in soot
INTEGER :: N_SIMPLE_CHEMISTRY_REACTIONS_GLOBAL !< Number of SIMPLE_CHEMISTRY reactions
...
INTEGER  :: N_SPEC_READ,N,NN,NNN,NS2,NR,N_SPEC_READ_2,N_SUB_SPECIES,NS,N_BINS,N_FOUND,Y_S,Z_S,SPECIES_ARRAY_DIMENSION,N_FUEL
INTEGER  :: N_COPY=0,N_TOTAL_BINS=0,N_LUMPED=0,N_CONDENSATION=0,N_PRIMITIVE=0
INTEGER, ALLOCATABLE, DIMENSION(:) :: Y_INDEX
LOGICAL  :: LUMPED_COMPONENT_ONLY,AEROSOL,BACKGROUND,&
            DEFINED_BACKGROUND,PRIMITIVE,COPY_LUMPED,FOUND
LOGICAL, ALLOCATABLE, DIMENSION(:) :: PREDEFINED,PREDEFINED_SMIX,NEW_PRIMITIVE,REAC_FUEL_READ
...
20 IF (IOS>0) THEN
   ...
   IF (COPY_LUMPED) N_COPY = N_COPY+1
   IF (N_BINS > 0) N_TOTAL_BINS = N_TOTAL_BINS + N_BINS  
   IF (BACKGROUND) THEN
      ...
      ENDIF
   ENDIF
   IF (SPEC_ID(1)=='null') N_PRIMITIVE = N_PRIMITIVE + 1
ENDDO COUNT_SPEC_LINES
19 REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0

N_TRACKED_SPECIES = N_LUMPED
...
IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
   IF (DEFINED_BACKGROUND) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Can not define a BACKGROUND SPECies when using simple chemistry.'
      CALL SHUTDOWN(MESSAGE) ; RETURN         
   ENDIF
   N_TRACKED_SPECIES = 1
   N_FUEL = 0
   DO NR=1,N_REACTIONS
      IF (REACTION(NR)%SIMPLE_CHEMISTRY .AND. REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS>0) THEN
         N_FUEL = N_FUEL+1
         IF (REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS==1) N_TRACKED_SPECIES = N_TRACKED_SPECIES + 2
         IF (REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS==2) N_TRACKED_SPECIES = N_TRACKED_SPECIES + 3
      ENDIF
   ENDDO
   SPECIES_ARRAY_DIMENSION = SPECIES_ARRAY_DIMENSION + N_TRACKED_SPECIES
ELSE
...
ALLOCATE(PREDEFINED_SMIX(1:PREDEFINED_SMIX_DIM))
...
ALLOCATE(REAC_FUEL_READ(SPECES_ARRAY_DIMENSION))
REAC_FUEL_READ=.FALSE.
...
IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
   ...
   PREDEFINED_SPEC_ID(6)   = 'SOOT'
   PREDEFINED_SPEC_ID(7)   = 'HYDROGEN'
   PREDEFINED_SPEC_ID(8)   = 'HYDROGEN CYANIDE'
   NN = 1
   NNN = 2
   DO NR=1,N_REACTIONS
      IF (REACTION(NR)%SIMPLE_CHEMISTRY .AND. REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS>0) THEN
         PREDEFINED_SPEC_ID(8+NN) = REACTION(NR)%FUEL
         NN = NN + 1
         IF (REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS==1) THEN
            PREDEFINED_SMIX(NNN:NNN+1) = .TRUE.
            REACTION(NR)%AIR_SMIX_INDEX = 1
            REACTION(NR)%PROD_SMIX_INDEX = NNN+2
            IF (REACTION(NR)%C <=0._EB .AND. REACTION(NR)%H <=0._EB) THEN
               PREDEFINED_SMIX(NNN)=.FALSE.
            ELSE
               REACTION(NR)%FUEL_SMIX_INDEX = NNN+1
            ENDIF
            NNN = NNN + 2
         ENDIF
         IF (REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS==2) THEN
            PREDEFINED_SMIX(NNN:NNN+2) = .TRUE.
            REACTION(NR)%AIR_SMIX_INDEX = 1
            REACTION(NR)%PROD_SMIX_INDEX = NNN+2
            REACTION(REACTION(NR)%PAIR_INDEX)%FUEL_SMIX_INDEX = NNN+2
            REACTION(REACTION(NR)%PAIR_INDEX)%PROD_SMIX_INDEX = NNN+3
            IF (REACTION(NR)%C <=0._EB .AND. REACTION(NR)%H <=0._EB) THEN
               PREDEFINED_SMIX(NNN)=.FALSE.
            ELSE
               REACTION(NR)%FUEL_SMIX_INDEX = NNN+1
...
COUNT_SPEC_LOOP: DO
   ...
   IF (ID=='AIR' .AND. ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
   ...
   IF (SPEC_ID(1)=='null' .OR. PRIMITIVE) THEN
      ...
      IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
         FOUND = .FALSE.
         DO NR=1,N_REACTIONS
            IF (REACTION(NR)%SIMPLE_CHEMISTRY .AND. TRIM(ID)==TRIM(REACTION(NR)%FUEL)) THEN
               FOUND = .TRUE.
               EXIT
            ENDIF
         ENDDO
         IF (.NOT. FOUND .AND. .NOT. LUMPED_COMPONENT_ONLY .AND. .NOT. BACKGROUND) N_TRACKED_SPECIES = N_TRACKED_SPECIES + 1
      ELSE
   ...
   ELSE
      IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
         ...
         DO NR=1,N_REACTIONS
            ...
            ENDIF
         ENDDO
         IF (.NOT. FOUND) N_TRACKED_SPECIES = N_TRACKED_SPECIES + 1
      ELSE
   ...
   PREDEFINED_SMIX_IF: IF (ANY(PREDEFINED_SMIX)) THEN
      CHECK_SPEC_LOOP: DO NN=1,N_SPECIES-1
         PREDEFINED_SPEC_IF: IF (TRIM(PREDEFINED_SPEC_ID(NN))==TRIM(ID)) THEN
            SIMPLE_CHEM_IF: IF (.NOT. ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
            ...
            ELSE SIMPLE_CHEM_IF
                  ...
                  DO NR=1,N_REACTIONS
                     IF (REACTION(NR)%SIMPLE_CHEMISTRY) THEN
                        IF (TRIM(ID)==TRIM(REACTION(NR)%FUEL)) THEN
                           ...
                           EXIT
                        ENDIF
                        IF (REACTION(NR%N_SIMPLE_CHEMISTRY_REACTIONS==1) NNN = NNN + 2
                        IF (REACTION(NR%N_SIMPLE_CHEMISTRY_REACTIONS==2) NNN = NNN + 3
                     ENDIF
                  ENDDO                  
                  IF (.NOT. FOUND) THEN
                     WRITE(MESSAGE,'(A,I0,A)') 'ERROR: Species ',N_SPEC_READ+1, &
               ...
               ELSE
                  ...
                  DO NR=1,N_REACTIONS
                     ...
                     ENDIF
                  ENDDO                  
                  IF (FOUND) REAC_FUEL_READ(NNN) = .TRUE.
               ENDIF
            ENDIF SIMPLE_CHEM_IF
            PREDEFINED(NN) = .FALSE.
            N_SPECIES = N_SPECIES - 1
            EXIT
         ENDIF PREDEFINED_SPEC_IF
      ENDDO CHECK_SPEC_LOOP
   ENDIF PREDEFINED_SMIX_IF
...
IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
   ...
   DO NR=1,N_REACTIONS
      IF (REACTION(NR)%SIMPLE_CHEMISTRY) THEN
         IF (.NOT. (REAC_FUEL_REAC(NNN+1) .OR. SIMPLE_FUEL_DEFINED(NR)) THEN
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   ELSE
      ...
      IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
         ...
         DO NR=1,N_REACTIONS
            IF (REACTION(NR)%SIMPLE_CHEMISTRY) THEN
               IF(TRIM(ID)==TRIM(REACTION(NR)%FUEL)) PREDEFINED_SMIX(NNN+1)=.FALSE.
               IF (REACTION(NR%N_SIMPLE_CHEMISTRY_REACTIONS==1) NNN = NNN + 2
               IF (REACTION(NR%N_SIMPLE_CHEMISTRY_REACTIONS==2) NNN = NNN + 3
            ENDIF
         ENDDO                  
   ...
   IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
      DO NR=1,N_REACTIONS
         IF (REACTION(NR)%SIMPLE_CHEMISTRY .AND. TRIM(SS%ID)==TRIM(REACTION(NR)%FUEL) .AND. .NOT. SS%LISTED) SS%H_F = H_F_IN
      ENDDO
   ENDIF
   ...
   IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
      DO NR=1,N_REACTIONS
         IF (TRIM(ID)==TRIM(REACTION(NR)%FUEL)) THEN
            REACTION(NR)%FUEL_INDEX = N
            WRITE(FORMULA,'(A,I0)') 'SPEC_',N
            IF (TRIM(SS%FORMULA)==TRIM(FORMULA)) SS%MW = REACTION(NR)%MW_FUEL
         ENDIF
      ENDDO
...
TRACKED_SPEC_LOOP_1: DO WHILE (N_FOUND <= N_TRACKED_SPECIES .OR. .NOT. DEFINED_BACKGROUND)
   ...
   ELSE
      ...
      IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
         ...
         DO NR=1,N_REACTIONS
            IF (REACTION(NR)%SIMPLE_CHEMISTRY .AND. REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS > 0 .AND. &
               TRIM(SM%ID)==TRIM(REACTION(1)%FUEL)) THEN
               REACTION(NR)%FUEL_SMIX_INDEX = N
               ...
               ENDIF
               IF (REACTION(NR)%C<=TWO_EPSILON_EB .AND. REACTION(1)%H<=TWO_EPSILON_EB) THEN
...
IF (ANY(PREDEFINED_SMIX)) THEN
   ...
   IF (ANY(REACTION%SIMPLE_CHEMISTRY)) THEN
      DO NR=1,N_REACTIONS
         IF (REACTION(NR)%SIMPLE_CHEMISTRY .AND. REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS > 0) CALL SETUP_PREDEFINED_SMIX(1,NR)
      ENDDO
   ENDOF
ENDIF
...
!**************** FIGURE OUT MULTIPLE SIMPLE CHEM
...
ELSE
   RN=>REACTION(NR)
   SELECT CASE (RN%N_SIMPLE_CHEMISTRY_REACTIONS)
      CASE (1)
         ...
         RN%NU_H2O          = 0.5_EB*REACTION(1)%H - 0.5_EB*(RN%NU_SOOT*SOOT_H_FRACTION_GLOBAL + RN%NU_HCN)
         IF (ABS(RN%NU_H2O) < TWO_EPSILON_EB) RN%NU_H2O = 0._EB
         RN%NU_CO2          = REACTION(1)%C - RN%NU_CO - RN%NU_HCN - RN%NU_SOOT*(1._EB-SOOT_H_FRACTION_GLOBAL)
      ...
      CASE (2)
         ...
         RN%NU_SOOT         = (REACTION(1)%C-RN%NU_CO-RN%NU_HCN)/(1._EB-SOOT_H_FRACTION_GLOBAL)
         ...
         RN%NU_H2O          = 0.5_EB*(REACTION(1)%H - 0.5_EB*RN%NU_H2 - RN%NU_HCN - RN%NU_SOOT*SOOT_H_FRACTION_GLOBAL)
         IF (ABS(RN%NU_H2O)<TWO_EPSILON_EB) RN%NU_H2O = 0._EB
         RN%NU_CO2          = REACTION(1)%C - RN%NU_CO - RN%NU_SOOT*(1._EB-SOOT_H_FRACTION_GLOBAL) - RN%NU_HCN
         IF (ABS(RN%NU_CO2)<TWO_EPSILON_EB) RN%NU_CO2 = 0._EB
         RN%NU_O2           = RN%NU_CO2 + 0.5_EB*(RN%NU_CO+RN%NU_H2O-REACTION(1)%O)
...
REAL(EB) :: FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,FUEL_N_TO_HCN_FRACTION
INTEGER :: N_SIMPLE_CHEMISTRY_REACTIONS
NAMELIST /COMB/ AIT_EXCLUSION_ZONE,AUTO_IGNITION_TEMPERATURE,CHECK_REALIZABILITY,EXTINCTION_MODEL,&
                ...
                ODE_SOLVER,RADIATIVE_FRACTION,RICHARDSON_ERROR_TOLERANCE,SOOT_H_FRACTION,SUPPRESSION,TAU_CHEM,TAU_FLAME

ODE_SOLVER         = 'null'
RADIATIVE_FRACTION = -1._EB
FUEL_C_TO_CO_FRACTION=0.6667_EB 
FUEL_H_TO_H2_FRACTION=0._EB     
FUEL_N_TO_HCN_FRACTION=0.2_EB   
...
IF (FUEL_C_TO_CO_FRACTION < 0._EB .OR. FUEL_C_TO_CO_FRACTION > 1._EB) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
FUEL_C_TO_CO_FRACTION_GLOBAL = FUEL_C_TO_CO_FRACTION
IF (FUEL_H_TO_H2_FRACTION < 0._EB .OR. FUEL_H_TO_H2_FRACTION > 1._EB) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
FUEL_H_TO_H2_FRACTION_GLOBAL = FUEL_H_TO_H2_FRACTION
IF (FUEL_N_TO_HCN_FRACTION < 0._EB .OR. FUEL_N_TO_HCN_FRACTION > 1._EB) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
FUEL_N_TO_HCN_FRACTION_GLOBAL = FUEL_N_TO_HCN_FRACTION
IF (N_SIMPLE_CHEMISTRY_REACTIONS < 1 .OR. N_SIMPLE_CHEMISTRY_REACTIONS > 2) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: N_SIMPLE_CHEMISTRY_REACTIONS must be 1 or 2.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
N_SIMPLE_CHEMISTRY_REACTIONS_GLOBAL = N_SIMPLE_CHEMISTRY_REACTIONS
IF (SOOT_H_FRACTION > 1._EB) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: SOOT_H_FRACTION must be between 0 and 1.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
ENDIF
...
INTEGER :: NR,NR2,NS,NS2,NFR,PRIORITY,INT_PROD_COUNTER, SIMPLE_COUNTER, NEW_REAC
...
NAMELIST /REAC/ A,AIT_EXCLUSION_ZONE,AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                ...
                FUEL_C_TO_CO_FRACTION,FUEL_H_TO_H2_FRACTION,FUEL_N_TO_HCN_FRACTION,N_SIMPLE_CHEMISTRY_REACTIONS
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (NR>1 .AND. ANY(REACTION%SIMPLE_CHEMISTTY) THEN
   ...
   IF (A < 0._EB .AND. E < 0._EB .AND. TRIM(SPEC_ID_NU(1))=='null' .AND. TRIM(EQUATION)=='null') THEN
      IF (N_REACTIONS > MAX_GAS_REACTIONS) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Too many simple chemistry reactions have been defined.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      RN%SIMPLE_CHEMISTRY = .TRUE.
      SIMPLE_COUNTER = SIMPLE_COUNTER + 1
      !****** Delete once working for one reaction
      IF (SIPMLE_COUNTER > 1) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: Can only define one simple chemistry reaction.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      ...
      ELSE
         RN%N_SIMPLE_CHEMISTRY_REACTIONS = N_SIMPLE_CHEMISTRY_REACTIONS_GLOBAL
      ENDIF
   ...
   IF (SOOT_H_FRACTION_GLOBAL > 0._EB .AND. SOOT_H_FRACTION > 0._EB) THEN
      SOOT_H_FRACTION_GLOBAL = SOOT_H_FRACTION
      WRITE(MESSAGE,'(A)') 'WARNING: Multiple SOOT_H_FRACTION values defined. Last value will be used for all reactions.'
      IF (MY_RANK==0) WRITE(LU_ERR,'(A)') TRIM(MESSAGE)
   ENDIF
...
! Set up paired reactions
INT_PROD_COUNTER = 0
IF (ANY(REACTION%N_SIMPLE_CHEMISTRY_REACTIONS==2)) THEN
   NEW_REAC = COUNT(REACTION%N_SIMPLE_CHEMISTRY_REACTIONS==2,1)
   ALLOCATE(REAC_TMP(N_REACTIONS),STAT=IZERO)
   REAC_TMP = REACTION
   DEALLOCATE(REACTION)
   ALLOCATE(REACTION(N_REACTIONS+NEW_REAC),STAT=IZERO)
   REACTION(1:N_REACTONS) = REAC_TEMP(1:N_REACTIONS)
   DEALLOCATE(REAC_TEMP)
   NR2 = N_REACTIONS
   DO NR = 1,N_REACTIONS
      IF (REACTION(NR)%N_SIMPLE_CHEMISTRY_REACTIONS/=2) CYCLE
      INT_PROD_COUNTER = INT_PROD_COUNTER +1
      NR2 = NR2 + 1
      RN => REACTION(NR2)
      REACTION(NR)%PAIR_INDEX = NR2
      RN%SIMPLE_CHEMISTRY = .TRUE.
      RN%N_SIMPLE_CHEMISTRY_REACTIONS = -2
      RN%PAIR_INDEX = NR
      RN%PRIORITY = 2
      IF (INT_PROD_COUNTER ==1) THEN
         RN%FUEL = 'INTERMEDIATE PRODUCTS'
      ELSE
         WRITE(RN%FUEL,'(A,I0)') 'INTERMEDIATE PRODUCTS ',INT_PROD_COUNTER
      ENDIF
   ENDDO
...
FORMULA                     = 'null'
FUEL                        = 'null'
FUEL_C_TO_CO_FRACTION       = -1.E10_EB
FUEL_H_TO_H2_FRACTION       = -1.E10_EB
FUEL_N_TO_HCN_FRACTION      = -1.E10_EB
FWD_ID                      = 'null'
FYI                         = 'null'
H                           = 0._EB
...
N_S                         = -999._EB
N_T                         = 0._EB
N_SIMPLE_CHEMISTRY_REACTIONS= -999 
O                           = 0._EB
PRIORITY                    = 1
RAMP_CHI_R                  = 'null'
RAMP_CFT                    = 'null'
...
INTEGER :: NS,NS2,NR,NSPEC,INT_PROD_COUNTER
...
DO NR=1,N_REACTIONS
   ...
   SIMPLE_CHEM_IF: IF (RN%SIMPLE_CHEMISTRY) THEN
      ...
      IF (RN%N_SIMPLE_CHEMISTRY_REACTIONS==-2) CYCLE
      ...
      IF (RN%N_SIMPLE_CHEMISTRY_REACTIONS==1) THEN
         ...
         ELSE
            WRITE(RN%SPEC_ID_NU_READ(3),'(A,I0)') 'PRODUCTS ',INT_PROD_COUNTER
         ENDIF
         RN%NU_READ(1)      = -1._EB
   ...
   ELSE ! RN%N_SIMPLE_CHEMISTRY_REACTIONS=2
      ...
      RN%SPEC_ID_NU_READ(1) = RN%FUEL
      RN%SPEC_ID_NU_READ(2) = 'AIR'
      RN%SPEC_ID_NU_READ(3) = REACTION(RN%PAIR_INDEX)%FUEL
      RN%NU_READ(1)      = -1._EB
      RN%NU_READ(2)      = -RN%NU_O2/SPECIES_MIXTURE(1)%VOLUME_FRACTION(O2_INDEX)
      RN%NU_READ(3)      = -(RN%NU_READ(1)*SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW+RN%NU_READ(2)*SPECIES_MIXTURE(1)%MW)/ &
      ...
      ELSE
         WRITE(RN2%SPEC_ID_NU_READ(3),'(A,I0)') 'PRODUCTS ',INT_PROD_COUNTER
      ENDIF
...
REAC_LOOP: DO NR=1,N_REACTIONS
      ...
      IF (RN%SIMPLE_CHEMISTRY) THEN
         IF (IDEAL) THEN
            ...
            RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION + ( &
                         ...
                         + RN%NU_SOOT * SPECIES(CO2_INDEX)%H_F * (1._EB - SOOT_H_FRACTION_GLOBAL) &
                         + RN%NU_SOOT * SPECIES(H2O_INDEX)%H_F * SOOT_H_FRACTION_GLOBAL * 0.5_EB &
   ...
   ELSE HOC_IF ! Heat of combustion not specified
      ...
      IF (RN%SIMPLE_CHEMISTRY) THEN ! Calculate heat of combustion based oxygen consumption
...
DO NR=1,N_REACTIONS
   ...
   IF (RN%SIMPLE_CHEMISTRY) THEN
      ...
      ELSEIF (RN%N_SIMPLE_CHEMISTRY_REACTIONS==2) THEN
         IF (RN%HOC_COMPLETE < 0._EB) THEN  ! Compute complete HoC using HoC for first and second reactions
            RN%HOC_COMPLETE = RN%HEAT_OF_COMBUSTION + (1._EB+RN%S)*REACTION(RN%PAIR_INDEX)%HEAT_OF_COMBUSTION
         ELSE  ! Change the HoC of reaction 1 to match user-specified value for complete reaction by changing the HoF of fuel
            R1_HOC_COMPUTED = RN%HEAT_OF_COMBUSTION  ! Heat of combustion for reaction 1 computed using heats of formation
            RN%HEAT_OF_COMBUSTION = RN%HOC_COMPLETE - (1._EB+RN%S)*REACTION(RN%PAIR_INDEX)%HEAT_OF_COMBUSTION
            REDEFINE_H_F(RN%FUEL_SMIX_INDEX) = .TRUE.
            SMF => SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)
            SMF%H_F = SMF%H_F - (R1_HOC_COMPUTED - RN%HEAT_OF_COMBUSTION)
         ENDIF
      ENDIF
   ELSE
       IF (RN%HOC_COMPLETE < 0._EB) RN%HOC_COMPLETE = RN%HEAT_OF_COMBUSTION
   ENDIF
ENDDO

! Correct CP_BAR array if H_F is redefined.
!****** Figure out multiple simple
IF (ANY(REDEFINE_H_F)) THEN
   ...
   ENDDO
ELSE
   IF (ANY(RN%SIMPLE_CHEMISTRY) .AND. CONSTANT_SPECIFIC_HEAT_RATIO) THEN
      H_F_OLD = 0._EB
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   ! Adjust RAMP_Q for REFERENCE_HEAT_FLUX
