...
!write(0,*) meshes(1)%one_d_wall_storage(1)%integers(3,1:10)
ENDDO
IF (USE_MPI) THEN
...
IF (NOISE .OR. PERIODIC_TEST>0) THEN
   DO NM=1,NMESHES
      ...
      IF (N_CSVF>=NM) THEN
         IF (TRIM(CSVFINFO(NM)%UVWFILE)/='null') THEN
            CALL UVW_INIT(NM,CSVFINFO(NM)%UVWFILE)
...
EVACUATION_MESH_IF: IF (.NOT.EVACUATION_ONLY(NM)) THEN
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (OMESH_PARTICLES) THEN
      ...
      DO N=1,N_LAGRANGIAN_CLASSES
         ...
         ALLOCATE(M%OMESH(NOM)%ORPHAN_PARTICLE_STORAGE(N)%REALS(LPC%N_STORAGE_REALS,1000))
         ALLOCATE(M%OMESH(NOM)%ORPHAN_PARTICLE_STORAGE(N)%INTEGERS(LPC%N_STORAGE_INTEGERS,1000))
         ALLOCATE(M%OMESH(NOM)%ORPHAN_PARTICLE_STORAGE(N)%LOGICALS(LPC%N_STORAGE_LOGICALS,1000))
         ALLOCATE(M%OMESH(NOM)%ADOPT_PARTICLE_STORAGE(N)%REALS(LPC%N_STORAGE_REALS,1000))
         ALLOCATE(M%OMESH(NOM)%ADOPT_PARTICLE_STORAGE(N)%INTEGERS(LPC%N_STORAGE_INTEGERS,1000))
         ALLOCATE(M%OMESH(NOM)%ADOPT_PARTICLE_STORAGE(N)%LOGICALS(LPC%N_STORAGE_LOGICALS,1000))
      ENDDO
   ENDIF
...
INTEGER :: SNODE,NRA,NSB,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IJK_SIZE,N
REAL(EB) :: TNOW
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (CODE==6 .AND. OMESH_PARTICLES .AND. (M3%NIC_S>0 .OR. M3%NIC_R>0)) THEN
      ...
      DO N=1,N_LAGRANGIAN_CLASSES
         LPC => LAGRANGIAN_PARTICLE_CLASS(N)
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M3%ADOPT_PARTICLE_STORAGE(N)%REALS(1,1),(LPC%N_STORAGE_REALS)*1000, &
                        MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M3%ADOPT_PARTICLE_STORAGE(N)%INTEGERS(1,1),(LPC%N_STORAGE_INTEGERS)*1000, &
                        MPI_INTEGER,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_IRECV(M3%ADOPT_PARTICLE_STORAGE(N)%LOGICALS(1,1),(LPC%N_STORAGE_LOGICALS)*1000, &
...
REAL(EB) :: TNOW
INTEGER, INTENT(IN) :: CODE
INTEGER :: NM,II,JJ,KK,LL,N,RNODE,SNODE,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IJK_SIZE
      ...
      IF_SEND_PARTICLES: IF (CODE==6 .AND. OMESH_PARTICLES) THEN 
         ...
         NODE_CHECK_PARTICLE: IF (SNODE/=RNODE) THEN
            ...
            DO IPC=1,N_LAGRANGIAN_CLASSES
               ...
               N_REQ=MIN(N_REQ+1,SIZE(REQ))
               CALL MPI_ISEND(M3%ORPHAN_PARTICLE_STORAGE(IPC)%REALS(1,1),LPC%N_STORAGE_REALS*1000,MPI_DOUBLE_PRECISION, &
                              SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
               N_REQ=MIN(N_REQ+1,SIZE(REQ))
               CALL MPI_ISEND(M3%ORPHAN_PARTICLE_STORAGE(IPC)%INTEGERS(1,1),LPC%N_STORAGE_INTEGERS*1000,MPI_INTEGER, &
                              SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
               N_REQ=MIN(N_REQ+1,SIZE(REQ))
               CALL MPI_ISEND(M3%ORPHAN_PARTICLE_STORAGE(IPC)%LOGICALS(1,1),LPC%N_STORAGE_LOGICALS*1000,MPI_LOGICAL, &
                              SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
            ENDDO
   ...
   RECV_MESH_LOOP: DO NM=1,NMESHES
      ...
      IF_RECEIVE_PARTICLES: IF (CODE==6 .AND. OMESH_PARTICLES) THEN 
         ...
         DO IPC=1,N_LAGRANGIAN_CLASSES
            ...
            PART_ADOPT_CHECK: IF (M2%N_PART_ADOPT(IPC)>0) THEN
               CNT = 0
