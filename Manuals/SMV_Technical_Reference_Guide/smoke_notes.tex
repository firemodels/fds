...
The $\alpha$ parameter in (\ref{eq:alpha}) is used by OpenGL to blend smoke planes with the current background.  The $\alpha$ parameter used here also represents an opacity, 0.0 for completely transparent, 1.0 for completely opaque.
...
\end{eqnarray*}
Therefore, when $\Delta x<\ln(254/255)/\ln(1/2)\approx 0.00567$ the opacity will truncate to zero.  Equivalently, the opacity truncates to zero when $N>\ln(1/2)/\ln(254/255)\approx 177$ where $N$ is the number of grid planes in the 1~m column of smoke.  Smokeview will not draw smoke in this situation even though smoke is present.
