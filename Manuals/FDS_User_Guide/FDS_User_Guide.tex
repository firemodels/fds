...
\end{lstlisting}
Note that you must also specify a time scale for relaxation, {\ct DT\_MEAN\_FORCING}. For an outdoor flow, all other boundaries (except the ground) should be set to {\ct OPEN}. You may omit a region of the flow domain from forcing using the {\ct HOLE} feature.
