...
SUBROUTINE GET_MASS_FRACTION(Z1,Z2,Z3,INDEX,YY_SUM,Y_MF)
! Y_MF returns the mass fraction of species INDEX
INTEGER, INTENT(IN) :: INDEX
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,Y_MF,OMYYSUM
...
ELSE
   ...
   Z_1 = MAX(0._EB,MIN(1._EB,Z1))/OMYYSUM
   Z_2 = MAX(0._EB,MIN(1._EB,Z2))/OMYYSUM
   Z_3 = MAX(0._EB,MIN(1._EB,Z3))/OMYYSUM
ENDIF

IF (CO_PRODUCTION) THEN
   Y_MF = Z2Y_C(INDEX)+ Z2Y(INDEX,1) * Z_1 + Z2Y(INDEX,2) * Z_2 + Z2Y(INDEX,3) * Z_3
ELSE
   Y_MF = Z2Y_C(INDEX)+ Z2Y(INDEX,1) * Z_1 + Z2Y(INDEX,2) * Z_3
ENDIF
...
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,Y_MF(9),OMYYSUM
...
IF (CO_PRODUCTION) THEN
   Y_MF(:) = Z2Y_C+ Z2Y(:,1) * Z_1 + Z2Y(:,2) * Z_2 + Z2Y(:,3) * Z_3
ELSE
   Y_MF(:) = Z2Y_C+ Z2Y(:,1) * Z_1 + Z2Y(:,2) * Z_3
ENDIF
...
IF (CO_PRODUCTION) THEN
   MW_MF = (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_2 + Y_MF_SUM_Z(3)*Z_3)/ &
           (MW_AVG_C  + MW_AVG_Z(1) * Z_1 + MW_AVG_Z(2) * Z_2 + MW_AVG_Z(3) * Z_3)
ELSE
   MW_MF = (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_3)/ &
           (MW_AVG_C  + MW_AVG_Z(1) * Z_1 + MW_AVG_Z(2) * Z_3)
ENDIF

END SUBROUTINE GET_MOLECULAR_WEIGHT


SUBROUTINE GET_MU(Z1,Z2,Z3,YY_SUM,MU_MF,ITMP)
! GET_MU returns the viscosity of the mixture fraction
INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,MU_MF,Y_MF(9),MW_MF,OMYYSUM
...
CALL GET_MOLECULAR_WEIGHT(Z_1,Z_2,Z_3,0._EB,MW_MF)

MU_MF = DOT_PRODUCT(Y_MF(:),Z2MU(:,ITMP))*MW_MF

END SUBROUTINE GET_MU

SUBROUTINE GET_D(Z1,Z2,Z3,YY_SUM,D_MF,ITMP)
! GET_D returns the diffusivity of the mixture fraction
INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,D_MF,OMYYSUM
...
IF (CO_PRODUCTION) THEN
   D_MF = (Z2D_C(ITMP) + Z2D(ITMP,1) * Z_1 + Z2D(ITMP,2) * Z_2+ Z2D(ITMP,3) * Z_3)/ &
             (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_2 + Y_MF_SUM_Z(3)*Z_3)
ELSE
   D_MF = (Z2D_C(ITMP) + Z2D(ITMP,1) * Z_1 + Z2D(ITMP,2) * Z_3)/ &
             (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_3)
ENDIF

END SUBROUTINE GET_D


SUBROUTINE GET_CP(Z1,Z2,Z3,YY_SUM,CP_MF,ITMP)

! GET_CP returns the specific heat of the gas mixture in the mixture fraction formulation

INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,CP_MF,OMYYSUM
...
IF (CO_PRODUCTION) THEN
   CP_MF = (Z2CP_C(ITMP) + Z2CP(ITMP,1) * Z_1 + Z2CP(ITMP,2) * Z_2+ Z2CP(ITMP,3) * Z_3)/ &
             (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_2 + Y_MF_SUM_Z(3)*Z_3)
ELSE
   CP_MF = (Z2CP_C(ITMP) + Z2CP(ITMP,1) * Z_1 + Z2CP(ITMP,2) * Z_3)/ &
             (Y_MF_SUM_C + Y_MF_SUM_Z(1)*Z_1 + Y_MF_SUM_Z(2)*Z_3)
ENDIF
...
SUBROUTINE GET_K(Z1,Z2,Z3,YY_SUM,K_MF,ITMP)

! Thermal conductivity of the gas mixture in the mixture fraction formulation

INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z_1,Z_2,Z_3,K_MF,MW_MF,OMYYSUM
...
CALL GET_MOLECULAR_WEIGHT(Z_1,Z_2,Z_3,0._EB,MW_MF)
IF (CO_PRODUCTION) THEN
   K_MF = (Z2K_C(ITMP) + Z2K(ITMP,1) * Z_1 + Z2K(ITMP,2) * Z_2+ Z2K(ITMP,3) * Z_3)*MW_MF
ELSE
   K_MF = (Z2K_C(ITMP) + Z2K(ITMP,1) * Z_1 + Z2K(ITMP,2) * Z_3) * MW_MF
ENDIF
