...
USE CONTROL_VARIABLES
INTEGER :: N,I,J,K,II,JJ,KK,IPTS,JPTS,KPTS,N_EDGES_DIM,IW,IC,IOR,IERR,IPZ,NOM,IZERO,ICF,NSLICE,ITW,IEC,ITW2
INTEGER, INTENT(IN) :: NM
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),VC,RTRM,CP,XXC,YYC,ZZC
INTEGER :: IBP1,JBP1,KBP1,IBAR,JBAR,KBAR,NN,NL
...
WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   CALL FIND_BACK_WALL_INDEX(NM,IW)
...
INTEGER :: NL,N_CELLS_MAX,II,NWP,N,I,ITMP
...
DO NL=1,ONE_D%N_LAYERS
   ...
   CALL GET_N_LAYER_CELLS(ONE_D%MIN_DIFFUSIVITY(NL),SF%SWELL_RATIO*ONE_D%LAYER_THICKNESS(NL),SF%STRETCH_FACTOR, &
                          SF%CELL_SIZE_FACTOR,SF%CELL_SIZE,SF%N_LAYER_CELLS_MAX,N_CELLS_MAX,&
   ...
   CALL GET_N_LAYER_CELLS(ONE_D%MIN_DIFFUSIVITY(NL),ONE_D%LAYER_THICKNESS(NL),SF%STRETCH_FACTOR, &
                          SF%CELL_SIZE_FACTOR,SF%CELL_SIZE,SF%N_LAYER_CELLS_MAX,ONE_D%N_LAYER_CELLS(NL),&
                          ONE_D%SMALLEST_CELL_SIZE(NL),ONE_D%DDSUM(NL))
...
SUBROUTINE FIND_BACK_WALL_INDEX(NM,IW)
...
END SUBROUTINE FIND_BACK_WALL_INDEX
...
IF (REMOVE .AND. (SF%THERMAL_BC_INDEX==THERMALLY_THICK .OR. SF%PYROLYSIS_MODEL==PYROLYSIS_SPECIFIED)) THEN
   ...
   IF (IW_OLD>0) THEN
      ...
      ELSEIF (.NOT.CELL(ICG_OLD)%SOLID .AND. .NOT.CELL(ICG)%SOLID .AND. CELL(IC)%SOLID .AND. &
         ...
         WC%BOUNDARY_TYPE = SOLID_BOUNDARY
         IF (WC_OLD%BACK_MESH>0 .AND. WC_OLD%BACK_MESH/=NM) THEN
            OS => OMESH(WC_OLD%BACK_MESH)%WALL_SEND_BUFFER
            DO II=1,OS%N_ITEMS
