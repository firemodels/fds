...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IS%INDEX,IS%SPEC_INDEX,0,T,NM)
         ENDDO
      ENDDO
   ENDDO
   ...
   ELSE
      ...
      DO K=0,KBP1
         DO J=0,JBP1
            DO I=0,IBP1
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IS%INDEX2,IS%SPEC_INDEX,0,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ...
   SELECT CASE(DATAFLAG)
      CASE(1)
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
                  FF(I,J,K) = RHO(I,J,K)*GAS_PHASE_OUTPUT(I,J,K,SMOKE3D_QUANTITY_INDEX,SMOKE3D_SPEC_INDEX,0,T,NM)
               ENDDO
            ENDDO
         ENDDO
      CASE(2)
      ...
      DO I=II1,II2
         DO J=JJ1,JJ2
            DO K=KK1,KK2
               ...
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,KTS,IND,SPEC_INDEX,PART_INDEX,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ENDIF
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   GAS_OR_SOLID_PHASE: IF (DV%OUTPUT_INDEX>0) THEN 
      ...
      GAS_STATS: IF (DV%STATISTICS=='null') THEN
         ...
         VALUE = GAS_PHASE_OUTPUT(DV%I,DV%J,DV%K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM)

      ELSEIF (DV%STATISTICS=='TIME INTEGRAL') THEN GAS_STATS
         VALUE = DV%TI_VALUE + (T-DV%TI_T)*GAS_PHASE_OUTPUT(DV%I,DV%J,DV%K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM)
         DV%TI_VALUE = VALUE
         DV%TI_T = T
         ...
         DO K=DV%K1,DV%K2
            DO J=DV%J1,DV%J2
               DO I=DV%I1,DV%I2
                  ...
                  SELECT CASE(DV%STATISTICS)
                     CASE('MAX')
                        STAT_VALUE = MAX(STAT_VALUE, GAS_PHASE_OUTPUT(I,J,K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM))
                     CASE('MIN')
                        STAT_VALUE = MIN(STAT_VALUE, GAS_PHASE_OUTPUT(I,J,K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM))
                     CASE('MEAN')
                        STAT_VALUE = STAT_VALUE + GAS_PHASE_OUTPUT(I,J,K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM)
                     ...
                     CASE('AREA INTEGRAL')
                        ...
                        IF (DV%IOR==2) STAT_VALUE = STAT_VALUE + &
                                       GAS_PHASE_OUTPUT(I,J,K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM)*DX(I)*DZ(K)
                        IF (DV%IOR==3) STAT_VALUE = STAT_VALUE + &
                                       GAS_PHASE_OUTPUT(I,J,K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM)*DX(I)*DY(J)
                     CASE('VOLUME MEAN')
                        VOL = DX(I)*DY(J)*DZ(K)
                        STAT_VALUE = STAT_VALUE + GAS_PHASE_OUTPUT(I,J,K,DV%OUTPUT_INDEX,DV%SPEC_INDEX,DV%PART_INDEX,T,NM)*VOL
...
REAL(EB) FUNCTION GAS_PHASE_OUTPUT(II,JJ,KK,IND,SPEC_INDEX,PART_INDEX,T,NM)
