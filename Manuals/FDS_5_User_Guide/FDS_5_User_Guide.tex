...
the temperature at which the mass fraction of the material
...
over what temperature range, the reaction takes place. First, the {\ct HEATING\_RATE} ($\dot{T}$) is the rate at which the
...
{\ct PYROLYSIS\_RANGE} ($\Delta T$) is the approximate width (in degrees Celsius or Kelvin) of the green curve, assuming its shape to be roughly triangular. Its default value
...
\begin{eqnarray}
   ...
   E &=& \frac{r_p}{Y_p} \, \frac{R \, T_p^2}{\dot{T}} \\
   A &=& \frac{r_p}{Y_p} \, e^{E/R \, T_p}
\end{eqnarray}
Note that $\nu_r$ is the yield of solid residue. 
Note also that $Y_p$ is the mass fraction (blue curve in Fig.~\ref{pyrolysis}) at the temperature where the reaction rate is its maximum. It is 0.4 by default and not
adjustable. 
