...
\section{Confined Space Experiments}

NRL performed a multi-year series of experiments inside of a four level, 23 compartment test facility with 20 doors and ceiling vents whose exterior boundaries were airtight~\cite{Confined_JFPE}.  Three HVAC systems were installed in the facility: a supply air system which takes suction from a fan room and discharges the air to the remainder of the facility, an exhaust sytem which takes suction from the facility and discharges it to the fan room, and a smoke control control system which takes suction from an upper level compartment and discharges it to the ambient, Figure~\ref{confined_HVAC}.  A second set of HVAC ducts directly connected the second level with the fourth level, Figure~\ref{confined_bypass}.
