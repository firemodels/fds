...
\Huge
\flushright{Fire Dynamics Simulator (Version 5) \\ User's Guide \\ (Draft, May 9, 2007)}
...
\subsection{Stack Effect}%Kevin - more information about how it relates to FDS.
\label{stack}
...
Sometimes it is desired that a fire spread radially at some specified rate. Rather than trying to design material properties
...
time history, {\ct RAMP\_Q} or {\ct TAU\_Q}, you can also specify {\ct XYZ} and {\ct SPREAD\_RATE} on the {\ct VENT} line.
Then the fire is directed to start at the point {\ct XYZ} and spread
radially at a rate of {\ct SPREAD\_RATE} (m/s). The ramp-up begins at the time when the fire arrives at a given point.
...
\section{Creating or Removing Obstructions; Opening or Closing Vents}
...
{\ct SPRINKLER LINK TEMPERATURE}                & See Section~\ref{info:sprinkler}              & $^\circ$C      & D            \\ \hline
