...
\be X_{\rm l} = \exp \left[ -\frac{ h_{\rm v} W_{\rm f}}{\cal R} \left(\frac{1}{T_{\rm s}}-\frac{1}{T_{\rm b}} \right) \right]
\label{CC_liquid}
\ee
...
\be
\dot{m}'' =h_{\rm m} \rho_{g} log\left(\frac{X_{\rm g}-1}{X_{\rm l}-1}\right).
\ee
Here $h_{\rm m}=\SH\mu_{\rm g}/\SC \Delta~x$ is the mass transfer coefficient and $\rho_{g}$ and $X_{\rm g}$ are the density of the fuel vapor and the volume fraction of fuel vapor in the grid cell adjacent to the pool surface. The Schmidt number of air $\SC$ is $0.6$ and the Sherwood numer is given by
\be
\SH=0.037~\SC^{\frac{1}{3}} \RE^{\frac{4}{5}} .
\ee
The Reynolds number $\RE=\rho~v~L/\mu$ is calculated based on conditions in the cell adjacent to the surface. The length scale used in calculating the Reynolds number is 1 unless otherwise specified and is the same length scale used for convective heat transfer calculation.
