...
PUBLIC GET_TETRABOX_VOLUME, GET_VERTS, TETRAHEDRON_VOLUME, REMOVE_DUPLICATE_VERTS, GET_POLYHEDRON_VOLUME
...
INTEGER :: NVERTS, NPOLYS, NFACES
INTEGER, DIMENSION(0:599) :: FACES
INTEGER, DIMENSION(0:199) :: WHICH_POLY
...
CALL GET_VERTS(BOX_BOUNDS,V0,V1,V2,V3,VERTS,NVERTS,FACES,NORMAL_INDEX,WHICH_POLY,NFACES,NPOLYS,BOX_STATE,VERT_STATE)
IF (VERT_STATE==1) THEN  ! box is inside tetrahedron
   GET_TETRABOX_VOLUME = (BOX_BOUNDS(1)-BOX_BOUNDS(0))*(BOX_BOUNDS(3)-BOX_BOUNDS(2))*(BOX_BOUNDS(5)-BOX_BOUNDS(4))
   RETURN
ELSE IF(VERT_STATE==2) THEN ! tetrahedron is inside box
   GET_TETRABOX_VOLUME = ABS(TETRAHEDRON_VOLUME(V0,V1,V2,V3))
   RETURN
ELSE IF(VERT_STATE==3) THEN ! tetrahedron and box do not intersect
   GET_TETRABOX_VOLUME = 0.0_EB
   RETURN
ENDIF

! compute volume using computed vertices

GET_TETRABOX_VOLUME = GET_POLYHEDRON_VOLUME(VERTS,NVERTS,FACES,NFACES)
...
REAL(EB) FUNCTION GET_POLYHEDRON_VOLUME(VERTS,NVERTS,FACES,NFACES)
...
DO I = 0, NFACES-1
   ...
   VOLUME = VOLUME + ABS(TETRAHEDRON_VOLUME(V0,V1,V2,V_CENTER))
ENDDO
...
SUBROUTINE GET_VERTS(BOX_BOUNDS,VV0,VV1,VV2,VV3,VERTS,NVERTS,FACES,NORMAL_INDEX,WHICH_POLY,NFACES,NPOLYS,BOX_STATE,VERT_STATE)
...
INTEGER, DIMENSION(0:199) :: POLYSTART, POLYNUM
INTEGER, DIMENSION(0:9), INTENT(OUT) :: NORMAL_INDEX
INTEGER, INTENT(OUT) :: NVERTS,NPOLYS,NFACES
INTEGER, INTENT(OUT) :: BOX_STATE(-1:5)
INTEGER, INTENT(OUT) :: VERT_STATE
...
! define verts and faces for a box
   ...
   NFACES=12
   WHICH_POLY(0:11) = (/2,2,1,1,3,3,0,0,5,5,4,4/)
   RETURN
ENDIF
...
! define verts and faces for a tetrahedron
   ...
   FACES(9:11)=(/0,2,1/)
   NFACES=4
   WHICH_POLY(0:7) = (/6,6,7,7,8,8,9,9/)
   VERT_STATE=2
   RETURN
ENDIF
...
DO BP = 0, 5 
   ...
   IF (BOX_STATE(BP) /= -1 ) THEN ! this box plane coincides with a tetrhedon plane
      ...
      IF (NVERTS2D>0) THEN
         ...
         POLYNUM(NPOLYS) = POLYNUM(NPOLYS) + NVERTS2D
      ENDIF
      IF (POLYNUM(NPOLYS)>0) THEN
         NORMAL_INDEX(NPOLYS) = BP
         NPOLYS = NPOLYS + 1
      ENDIF
      CYCLE
   ENDIF
...
!  add intersection of tetrahedron edge and box plane (if on box)
   ...
   DO TE = 0, 5 ! tetra edge
      ...
      POLYNUM(NPOLYS) = POLYNUM(NPOLYS) + 1
      NVERTS=NVERTS+1
   ENDDO
...
DO F = 1, NPOLYS
  POLYSTART(F) = POLYSTART(F-1) + POLYNUM(F-1)
ENDDO

! order vertices of each face clockwise

DO F = 0, NPOLYS-1
  IF (POLYNUM(F)<=3) CYCLE
  FACEVERTS(0:3*POLYNUM(F)-1) => VERTS(3*POLYSTART(F):3*POLYSTART(F+1)-1)
  CALL ORDER_VERTS(FACEVERTS,POLYNUM(F),NORMAL_INDEX(F))
ENDDO

NFACES=0
DO I = 0, NPOLYS-1
   IF (POLYNUM(I)<3) CYCLE
   DO J = POLYSTART(I)+1, POLYSTART(I+1)-2
      FACES(3*NFACES:3*NFACES+2) = (/POLYSTART(I),J,J+1/)
      WHICH_POLY(NFACES) = NORMAL_INDEX(I)
      NFACES = NFACES + 1
   ENDDO
ENDDO

CALL REMOVE_DUPLICATE_VERTS(NVERTS,NFACES,NVOLUS,MAXVERTS,MAXFACES,MAXVOLUS,VERTS,FACES,VOLUS)
...
INTEGER, TARGET, INTENT(INOUT) :: VOLUS(0:4*MAX_VOLUS-1)
REAL(EB), TARGET, INTENT(INOUT) :: VERTS(0:3*MAX_VERTS-1)
INTEGER, TARGET, INTENT(INOUT) :: FACES(0:3*MAX_FACES-1)

REAL(EB), POINTER, DIMENSION(:) :: VI, VJ
REAL(EB), DIMENSION(3) :: VIMVJ
REAL(EB) :: NORM_VI, NORM_VJ, NORM_VIMVJ
REAL(EB) :: VERT_EPS=0.000001_EB
...
DO WHILE (I<N_VERTS)
   ...
   DO WHILE (J<N_VERTS)
      ...
      IF (NORM_VIMVJ <= VERT_EPS*MAX(1.0_EB,NORM_VI,NORM_VJ)) THEN
         ...
         DO K = 0, 3*N_FACES-1    ! replace J with I in all faces
           IF (FACES(K)==J)FACES(K)=I
         ENDDO
         DO K = 0, 4*N_VOLUS-1 ! replace J with I in all volumes
           IF (VOLUS(K)==J)VOLUS(K)=I
         ENDDO
         VJ(1:3)=VERTS(3*(N_VERTS-1):3*(N_VERTS-1)+2)  ! move n'th vertex down to j'th position
         DO K = 0, 3*N_FACES-1   ! ! replace N with J in all faces
           IF (FACES(K)==N_VERTS-1)FACES(K)=J
         ENDDO
         DO K = 0, 4*N_VOLUS-1  ! replace N with J in all volumes
           IF (VOLUS(K)==N_VERTS-1)VOLUS(K)=J
         ENDDO
         N_VERTS=N_VERTS-1
         CYCLE
      ENDIF
