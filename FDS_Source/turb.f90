...
PUBLIC :: ANALYTICAL_SOLUTION, sandia_dat, INIT_TURB_ARRAYS, spectral_output, VARDEN_DYNSMAG, &
          ...
          INITIALIZE_MEAN_FORCING, COMPUTE_MEAN_FORCING
...
SUBROUTINE COMPUTE_MEAN_FORCING(NM)

INTEGER, INTENT(IN) :: NM
REAL(EB) :: INTEGRAL,DVOLUME,SUM_VOLUME
INTEGER :: I,J,K,IC1,IC2
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU=>NULL(),VV=>NULL(),WW=>NULL()

IF (MESHES(NM)%MESH_LEVEL/=0) RETURN
...
IF (MEAN_FORCING_U) THEN
   ...
   DO K=1,KBAR
      ...
      ENDDO
   ENDDO
   UMEAN = (UMEAN*FSUMU + INTEGRAL)/(FSUMU+SUM_VOLUME)
   FSUMU = FSUMU + SUM_VOLUME
ENDIF
...
IF (MEAN_FORCING_V) THEN
   ...
   DO K=1,KBAR
      ...
      ENDDO
   ENDDO
   VMEAN = (VMEAN*FSUMV + INTEGRAL)/(FSUMV+SUM_VOLUME)
   FSUMV = FSUMV + SUM_VOLUME
ENDIF
...
IF (MEAN_FORCING_W) THEN
   ...
   DO K=0,KBAR
      ...
      ENDDO
   ENDDO
   WMEAN = (WMEAN*FSUMW + INTEGRAL)/(FSUMW+SUM_VOLUME)
   FSUMW = FSUMW + SUM_VOLUME
ENDIF

END SUBROUTINE COMPUTE_MEAN_FORCING


SUBROUTINE INITIALIZE_MEAN_FORCING
UMEAN = 0._EB
VMEAN = 0._EB
WMEAN = 0._EB
FSUMU = 0._EB
FSUMV = 0._EB
FSUMW = 0._EB
END SUBROUTINE INITIALIZE_MEAN_FORCING
