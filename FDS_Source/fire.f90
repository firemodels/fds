...
LOGICAL, INTENT(IN) :: EXTINCT(1:N_REACTIONS)
REAL(EB) :: DZ_F,YY_PRIMITIVE(1:N_SPECIES),DG_RXN,MW,MOLPCM3,DT_TMP(1:N_TRACKED_SPECIES),DT_MIN,&
            ZZ_TMP(1:N_TRACKED_SPECIES),ZZ_NEW(1:N_TRACKED_SPECIES)
INTEGER :: I,NS,SUB_IT_USE,FAST_IT
LOGICAL :: FAST_REACTANTS_PRESENT
INTEGER, PARAMETER :: INFINITELY_FAST=1,FINITE_RATE=2
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
KINETICS_SELECT: SELECT CASE(KINETICS)
   ...
   CASE(INFINITELY_FAST)
      ...
      FAST_REACTANTS_LOOP: DO FAST_IT=1,100

         ZZ_TMP = ZZ_NEW
         DZZ = 0._EB
         FAST_REACTANTS_PRESENT = .FALSE.
         ...
         REACTION_LOOP_1: DO I=1,N_REACTIONS
            ...
            ENDIF
            IF (DZ_F > TWO_EPSILON_EB) FAST_REACTANTS_PRESENT = .TRUE.
            DZZ = DZZ + RN%A_PRIME_FAST * RHO_0**RN%RHO_EXPONENT_FAST * RN%NU_MW_O_MW_F * DZ_F
         ENDDO REACTION_LOOP_1

         IF (FAST_REACTANTS_PRESENT) THEN
            DT_TMP = HUGE_EB
            DO NS = 1,N_TRACKED_SPECIES
               IF (DZZ(NS) < 0._EB) DT_TMP(NS) = -ZZ_TMP(NS)/DZZ(NS)
            ENDDO
            ZZ_NEW = ZZ_TMP + DZZ*MINVAL(DT_TMP)
         ELSE
            EXIT FAST_REACTANTS_LOOP
         ENDIF

         IF (FAST_IT > 20) CALL SHUTDOWN('ERROR: Infinite loop in FAST CHEMISTRY')

      ENDDO FAST_REACTANTS_LOOP
