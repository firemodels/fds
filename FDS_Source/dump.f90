...
ELSE
   ...
   IF (T>=BNDF_CLOCK(NM)) THEN
      ...
      ENDDO
   ENDIF
   IF (T>=BNDE_CLOCK(NM)) THEN
      IF (SYNCHRONIZE) THEN
         CALL DUMP_BNDE(T,NM)
      ELSE
         CALL DUMP_BNDE(BNDE_CLOCK(NM),NM)
      ENDIF
      DO
         BNDE_CLOCK(NM) = BNDE_CLOCK(NM) + DT_BNDE
         IF (BNDE_CLOCK(NM)>=T) EXIT
      ENDDO
   ENDIF
...
ALLOCATE(FN_BNDE(N_BNDE,NMESHES))
ALLOCATE(LU_BNDE(N_BNDE,NMESHES))
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   DO N=1,N_BNDE
      LU_BNDE(N,NM) = GET_FILE_NUMBER()
      IF (NMESHES>1) THEN
         WRITE(FN_BNDE(N,NM),'(A,A,I4.4,A,I2.2,A)') TRIM(CHID),'_',NM,'_',N,'.be'
      ELSE
         WRITE(FN_BNDE(N,NM),'(A,A,I2.2,A)') TRIM(CHID),'_',N,'.be'
      ENDIF
   ENDDO
...
INTEGER, INTENT(IN) :: NM
INTEGER :: IOR,IZERO,I,J,K,N,ERROR,I1,I2,J1,J2,K1,K2,I1B,I2B,IW,NN,NF,NE
INTEGER :: NTSL
...
BOUNDARY_ELEMENT_FILES: DO NE=1,N_BNDE

   BE => BOUNDARY_ELEMENT_FILE(NE)
 
   IF (EVACUATION_ONLY(NM)) EXIT BOUNDARY_ELEMENT_FILES
 
   RESTART_BE: IF (APPEND) THEN
      OPEN(LU_BNDE(NE,NM),FILE=FN_BNDE(NE,NM),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
   ELSE RESTART_BE
      OPEN(LU_BNDE(NE,NM),FILE=FN_BNDE(NF,NM),FORM='FORMATTED',STATUS='REPLACE')
      WRITE(LU_BNDE(NE,NM),'(A)') BE%SMOKEVIEW_LABEL(1:30)
      WRITE(LU_BNDE(NE,NM),'(A)') BE%SMOKEVIEW_BAR_LABEL(1:30)
      WRITE(LU_BNDE(NE,NM),'(A)') OUTPUT_QUANTITY(BE%INDEX)%UNITS(1:30)
   ENDIF RESTART_BE
...
WRITE(LU_CORE(NM)) T,ICYC,PART_CLOCK(NM),CORE_CLOCK(NM),SLCF_CLOCK(NM), &
             PL3D_CLOCK(NM),BNDE_CLOCK(NM),BNDF_CLOCK(NM),DEVC_CLOCK,HRR_CLOCK,MINT_CLOCK, &
...
ENDIF
IF(HISTOGRAM_FILE)  OPEN(LU_HISTOGRAM,FILE=FN_HISTOGRAM,STATUS='REPLACE')! Overwrite Histogram file
...
ELOOP: DO NE=1,N_BNDE

   BE => BOUNDARY_ELEMENT_FILE(NE)
   PY => PROPERTY(BE%PROP_INDEX)
   
   WRITE(LU_BNDE(NE,NM),'(F10.4)') STIME
...
DO N=1,N_BNDE
   INQUIRE(FILE=FN_BNDE(N,NM),EXIST=EX)
   IF (EX) THEN
      CLOSE(LU_BNDE(N,NM))
      OPEN(LU_BNDE(N,NM),FILE=FN_BNDE(N,NM),FORM='FORMATTED',STATUS='OLD',POSITION='APPEND')
   ENDIF
ENDDO
