...
\leftline{\underline{Transport of Sensible Enthalpy} }
...
\be \rho \, \frac{\mbox{D}\bu}{\mbox{D}t} \cdot \bu = \rho  \, \frac{\mbox{D}\left(|\bu|^2/2\right)}{\mbox{D}t} = \rho \bof_b \cdot \bu - \nabla p \cdot \bu + \nabla \cdot (\btau_{ij} \cdot \bu) - \epsilon \ee
...
the convective terms, upwind biasing for the predictor step and downwind biasing for the corrector step, resulting in a second-order scheme which is consistent with the scheme used for the continuity equation.
\be
...
estimated at the next time step with an explicit Euler step. For
example, the density is estimated by
\be \rho^{(n+1)_e} = \rho^n - \dt (\bar{\mathbf{u}}^n \cdot \nabla \rho^n + \rho^n \nabla \cdot \bar{\mathbf{u}}^n) \ee
...
\be \nabla^2{\cal H'}^{(n+1)_e} = - \left[ \frac{ (\nabla \cdot \bu)^{(n+1)_e} -
  (\nabla \cdot \bar{\mathbf{u}})^n }{\dt} \right] - \nabla \cdot \bar{\bF}^n  \ee
...
\item The velocity is estimated at the next time step
\be
\bu^{(n+1)_e} = \bar{\mathbf{u}}^n - \dt \left( \bar{\bF}^n + \nabla {\cal H'}^{(n+1)_e} + \nabla {\cal H''}^{(n+1)_e}\right)
\ee
...
\be
\rho^{n+1} = \ha \left[\rho^n + \rho^{(n+1)_e} - \dt \left(\bar{\mathbf{u}}^{(n+1)_e} \cdot \nabla \rho^{(n+1)_e} + \rho^{(n+1)_e} \nabla \cdot \bar{\mathbf{u}}^{(n+1)_e}\right) \right]
\ee
Before boundary conditions for $\rho$ and $Y_\alpha$ are applied, values of these quantities at
interpolated mesh boundaries are exchanged via MPI calls.
...
\item The velocity is corrected
\be
\bu^{n+1} = \ha \left[ \bar{\mathbf{u}}^n + \bar{\mathbf{u}}^{(n+1)_e} - \dt \left( \bar{\mathbf{F}}^{(n+1)_e} + \nabla {\cal H'}^{n+1} + \nabla {\cal H''}^{n+1}\right) \right]
\ee
