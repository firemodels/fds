...
However, for the purposes of inputting into FDS, since M is neither a product nor a species it would not be specified in the chemical reaction using either {\ct REACTION} or {\ct NU}.  It would be given a rate exponent using {\ct N\_S} to indicate that its presence is required.
...
\end{lstlisting}
Note that each set of {\ct TABL} lines must have a unique {\ct ID}.  Also note that the {\ct TABL} lines can be specified in any order.
...
{\ct FLAME TEMPERATURE}                         & Section \ref{info:finite}                         & $^\circ$C      & D,I,P,S
\\ \hline
