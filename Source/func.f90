...
IF (LPC%INCLUDE_BOUNDARY_PROPERTY_TYPE)LPC%N_STORAGE_REALS = LPC%N_STORAGE_REALS + N_BOUNDARY_PROPERTY_STORAGE_REALS
...
IF (LPC%INCLUDE_BOUNDARY_PROPERTY_TYPE) LPC%N_STORAGE_INTEGERS = LPC%N_STORAGE_INTEGERS + N_BOUNDARY_PROPERTY_STORAGE_INTEGERS
...
IF (LPC%INCLUDE_BOUNDARY_PROPERTY_TYPE) LPC%N_STORAGE_LOGICALS = LPC%N_STORAGE_LOGICALS + N_BOUNDARY_PROPERTY_STORAGE_LOGICALS
...
TYPE (ONE_D_M_AND_E_XFER_TYPE), ALLOCATABLE, DIMENSION(:) :: OD_DUMMY
...
IF (PRESENT(LP_INDEX)) THEN
   ...
   IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE)    CALL ALLOCATE_ONE_D_ARRAYS
   IF (LPC%INCLUDE_BOUNDARY_PROPERTY_TYPE) CALL ALLOCATE_BOUNDARY_PROPERTY_ARRAYS
...
ELSEIF (PRESENT(WALL_INDEX)) THEN
   ...
   IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE)    CALL ALLOCATE_ONE_D_ARRAYS
   IF (SF%INCLUDE_BOUNDARY_PROPERTY_TYPE) CALL ALLOCATE_BOUNDARY_PROPERTY_ARRAYS
...
IF (MINVAL(M%BOUNDARY_COORD_OCCUPANCY)==1) THEN  ! There are no open slots for boundary coordinates
   N_NEW_STORAGE_SLOTS = 1000
...
IF (BC_INDEX==0) BC_INDEX = MINLOC(M%BOUNDARY_COORD_OCCUPANCY,DIM=1) ; M%BOUNDARY_COORD_OCCUPANCY(BC_INDEX) = 1
...
IF (MINVAL(M%BOUNDARY_ONE_D_OCCUPANCY)==1) THEN  ! There are no open slots for boundary coordinates
   N_NEW_STORAGE_SLOTS = 1000
...
IF (OD_INDEX==0) OD_INDEX = MINLOC(M%BOUNDARY_ONE_D_OCCUPANCY,DIM=1) ; M%BOUNDARY_ONE_D_OCCUPANCY(OD_INDEX) = 1
...
IF (MINVAL(M%BOUNDARY_PROPERTY_OCCUPANCY)==1) THEN  ! There are no open slots for boundary coordinates
   N_NEW_STORAGE_SLOTS = 1000
   ALLOCATE(BP_DUMMY(1:M%N_BOUNDARY_PROPERTY_DIM+N_NEW_STORAGE_SLOTS))
   BP_DUMMY(1:M%N_BOUNDARY_PROPERTY_DIM) = M%BOUNDARY_PROPERTY(1:M%N_BOUNDARY_PROPERTY_DIM)
   CALL MOVE_ALLOC(BP_DUMMY,M%BOUNDARY_PROPERTY)
   ALLOCATE(DUMMY(1:M%N_BOUNDARY_PROPERTY_DIM+N_NEW_STORAGE_SLOTS)) ; DUMMY = 0
   DUMMY(1:M%N_BOUNDARY_PROPERTY_DIM) = M%BOUNDARY_PROPERTY_OCCUPANCY(1:M%N_BOUNDARY_PROPERTY_DIM)
   CALL MOVE_ALLOC(DUMMY,M%BOUNDARY_PROPERTY_OCCUPANCY)
   M%N_BOUNDARY_PROPERTY_DIM = M%N_BOUNDARY_PROPERTY_DIM + N_NEW_STORAGE_SLOTS
ENDIF

IF (BP_INDEX==0) BP_INDEX = MINLOC(M%BOUNDARY_PROPERTY_OCCUPANCY,DIM=1) ; M%BOUNDARY_PROPERTY_OCCUPANCY(BP_INDEX) = 1
...
IF (UNPACK_IT) CALL ALLOCATE_STORAGE(NM,LP_INDEX=LP_INDEX,LPC_INDEX=LPC_INDEX)
...
IF (LP%BP_INDEX>0) CALL PACK_BOUNDARY_PROPERTY(NM,RC,OS,1,LP%BP_INDEX,UNPACK_IT)
