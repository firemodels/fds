...
The combustion routine is responsible for determining the mean chemical mass production rate of species $\alpha$ per unit volume, $\dot{m}^{\prime\prime\prime}_{\alpha}$, in the species transport equation, Eq.~(\ref{species}).  In the typical FDS problem the primitive species are lumped into reacting groups (more on this below) and we consider the simple reaction
\begin{equation}\label{eq:simple}
\mathrm{Fuel + Air \rightarrow Products}
\end{equation}
For the vast majority of FDS applications, the ``mixed is burnt'' approximation is adequate and the chemical source term for Fuel may be modeled by the Eddy Dissipation Concept (EDC) of Magnussen and Hjertager \cite{Magnussen:1,Poinsot:TNC}:
\begin{equation}
\label{eq:edc}
\dot{m}_F^{\prime\prime\prime} = -\rho \frac{\min(Z_F, s\,Z_A)}{\tau_{\rm mix}}
\end{equation}
Here, $Z_F$ and $Z_A$ are the mass fractions of Fuel and Air, respectively, and $s$ is the mass stoichiometric coefficient for Air.  The quantity $\tau_{\rm mix}$ is a time scale for mixing.  The EDC model therefore states that the rate of fuel consumption is proportional to both the local limiting reactant concentration and the local rate of mixing.  A model for the mixing time is discussed in Section \ref{sec:reac_time_scale}.  With the mass production (consumption) rate of Fuel known, the production rates for all other species in the system (e.g., \si{CO_2}, \si{H_2O}) may be found from stoichiometry.  Specifics for the default reaction system are given in Sections \ref{sec:simplechemistry} (Simple Chemistry) and \ref{sec:yield} (Specified CO and Soot Yield).

%In this chapter, we integrate the change in cell average species concentration over a time step for a single grid cell to obtain the mean chemical source term,
...
The remainder of this chapter describes our generalized combustion model, of which EDC is a limiting case. First, we discuss of our \emph{lumped species} approach, which reduces the computational burden of the full chemical system by combining species into groups that transport and react together.  We then discuss our approach to describing the chemical mixing process---we treat each computational cell as a partially-stirred batch reactor with a characteristic mixing time.  The model for the mixing time is discussed next.  Once reactants are mixed, the reaction rate depends on kinetics.  The last part of this chapter discusses available kinetic mechanisms within FDS, from infinitely fast chemistry (default) to Arrhenius rate laws and reversible reactions.
...
For the special case where the initial unmixed fraction is set to unity and the chemical kinetics are taken to be infinitely fast, our model reduces to the Eddy Dissipation Concept (EDC) model shown by Eq.~(\ref{eq:edc}) \cite{Magnussen:1,Poinsot:TNC}.
...
\label{eq:tau_sgs}  \tau_{\rm u} &= \frac{\Delta}{\sqrt{2k_{sgs}}} \\
...
At present, simple empirical rules---which ignore strain---are used to predict local extinction (within a certain grid cell) based on local resolved species concentrations and the local cell temperature.  The default FDS extinction model consists of two parts based on the concept of a \emph{critical flame temperature} \cite{SFPE:Beyler}. First, if the cell temperature is below the auto-ignition temperature for the fuel then combustion is suppressed. The auto-ignition temperature is zero by default so that the typical user does not need to specify an ignition source.  Next, if the potential combustion heat release cannot raise the local subgrid temperature above an empirically determined limiting flame temperature, then combustion is suppressed. If either criterion fails, then there is no chemical reaction and $\dot{q}^{\prime\prime\prime}=0$. The limiting flame temperature extinction algorithm is outlined below.
