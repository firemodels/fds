...
USE GLOBAL_CONSTANTS, ONLY: RADIATION,SIGMA,TMPA4,OPEN_BOUNDARY,NRA=>NUMBER_RADIATION_ANGLES,NSB=>NUMBER_SPECTRAL_BANDS
...
SUBROUTINE GET_WALL_NODE_WEIGHTS(N_CELLS,N_LAYERS,N_LAYER_CELLS, &
         LAYER_THICKNESS,GEOMETRY,X_S,L_DIVIDE,DX,RDX,RDXN,DX_WGT,DXF,DXB,LAYER_INDEX,MF_FRAC)

! Get the wall internal coordinates

INTEGER, INTENT(IN)     :: N_CELLS, N_LAYERS, N_LAYER_CELLS(N_LAYERS), GEOMETRY
REAL(EB), INTENT(IN)    :: X_S(0:N_CELLS),LAYER_THICKNESS(1:N_LAYERS), L_DIVIDE
...
! is added to the front
   ...
   IF (L_DIVIDE .GE. REAL(N_LAYERS,EB)) THEN
      MF_FRAC = 1.0_EB
   ELSE
      MF_FRAC = 0._EB

      II = INT(L_DIVIDE)
      X_DIVIDE  = LAYER_THICKNESS(II) + DMOD(L_DIVIDE,1.0_EB)*LAYER_THICKNESS(II+1)
...
Y_MF = Z2Y_C(INDEX) + DOT_PRODUCT(Z2Y(INDEX,:), Z)

Y_MF = MIN(1._EB,MAX(0._EB,Y_MF)) * OMYYSUM

END SUBROUTINE GET_MASS_FRACTION

SUBROUTINE GET_MASS_FRACTION_ALL(Z_IN,YY_SUM,Y_MF)
! Y_MF returns the mass fraction of all mixture fraction species
REAL(EB), INTENT(IN) :: Z_IN(1:I_Z_MAX - I_Z_MIN + 1),YY_SUM
REAL(EB) :: Z(1:I_Z_MAX - I_Z_MIN + 1),Y_MF(9),OMYYSUM
INTEGER :: I
...
DO I=1,9
   Y_MF(I) = Z2Y_C(I) + DOT_PRODUCT(Z2Y(I,:),Z)
ENDDO
...
SUBROUTINE GET_MOLECULAR_WEIGHT(Z_IN,YY_SUM, MW_MF)

! Molecular weight of the gas mixture in the mixture fraction formulation
...
MW_MF = (Y_MF_SUM_C + DOT_PRODUCT(Y_MF_SUM_Z,Z))/ (MW_AVG_C  + DOT_PRODUCT(MW_AVG_Z,Z))

END SUBROUTINE GET_MOLECULAR_WEIGHT


SUBROUTINE GET_MU(Z_IN,YY_SUM,MU_MF,ITMP)
! GET_MU returns the viscosity of the mixture fraction
INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z_IN(1:I_Z_MAX - I_Z_MIN + 1),YY_SUM
REAL(EB) :: Z(1:I_Z_MAX - I_Z_MIN + 1),MU_MF,Y_MF(9),MW_MF,OMYYSUM
...
CALL GET_MASS_FRACTION_ALL(Z,0._EB,Y_MF)
CALL GET_MOLECULAR_WEIGHT(Z,0._EB,MW_MF)

MU_MF = DOT_PRODUCT(Y_MF(:),Z2MU(:,ITMP))*MW_MF

END SUBROUTINE GET_MU


SUBROUTINE GET_D(Z_IN,YY_SUM,D_MF,ITMP)
! GET_D returns the diffusivity of the mixture fraction
INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z_IN(1:I_Z_MAX - I_Z_MIN + 1),YY_SUM
REAL(EB) :: Z(1:I_Z_MAX - I_Z_MIN + 1),D_MF,OMYYSUM
...
D_MF = (Z2D_C(ITMP) + DOT_PRODUCT(Z2D(ITMP,:),Z))/(Y_MF_SUM_C + DOT_PRODUCT(Y_MF_SUM_Z,Z))
...
! GET_CP returns the specific heat of the gas mixture in the mixture fraction formulation
...
SUBROUTINE GET_CPBAR(Z_IN,YY_SUM,CPBAR_MF,ITMP)

! GET_CPBAR returns the average specific heat of the gas mixture in the mixture fraction formulation

INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z_IN(1:I_Z_MAX - I_Z_MIN + 1),YY_SUM
REAL(EB) ::Z(1:I_Z_MAX - I_Z_MIN + 1),CPBAR_MF,OMYYSUM

IF (YY_SUM >=1._EB) THEN
   CPBAR_MF = SPECIES(0)%CPBAR(ITMP)
   RETURN
ELSE
   OMYYSUM = 1._EB-MAX(0._EB,YY_SUM)
   Z = MAX(0._EB,MIN(1._EB,Z_IN))/OMYYSUM
...
CPBAR_MF = (Z2CPBAR_C(ITMP) + DOT_PRODUCT(Z2CPBAR(ITMP,:),Z))/(Y_MF_SUM_C + DOT_PRODUCT(Y_MF_SUM_Z,Z))

END SUBROUTINE GET_CPBAR

SUBROUTINE GET_H_G(Z_IN,YY_SUM,H_G_MF,ITMP)

! GET_H_G returns the enthalpy of the gas mixture in the mixture fraction formulation

INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z_IN(1:I_Z_MAX - I_Z_MIN + 1),YY_SUM
REAL(EB) ::Z(1:I_Z_MAX - I_Z_MIN + 1),H_G_MF,OMYYSUM

IF (YY_SUM >=1._EB) THEN
   H_G_MF = SPECIES(0)%H_G(ITMP)
   RETURN
ELSE
   OMYYSUM = 1._EB-MAX(0._EB,YY_SUM)
   Z = MAX(0._EB,MIN(1._EB,Z_IN))/OMYYSUM
...
H_G_MF = (Z2H_G_C(ITMP) + DOT_PRODUCT(Z2H_G(ITMP,:),Z))/(Y_MF_SUM_C + DOT_PRODUCT(Y_MF_SUM_Z,Z))

END SUBROUTINE GET_H_G

SUBROUTINE GET_K(Z_IN,YY_SUM,K_MF,ITMP)

! Thermal conductivity of the gas mixture in the mixture fraction formulation

INTEGER, INTENT(IN) :: ITMP
REAL(EB), INTENT(IN) :: Z_IN(1:I_Z_MAX - I_Z_MIN + 1),YY_SUM
REAL(EB) :: Z(1:I_Z_MAX - I_Z_MIN + 1),K_MF,MW_MF,OMYYSUM
...
K_MF = (Z2K_C(ITMP) + DOT_PRODUCT(Z2K(ITMP,:),Z))*MW_MF
