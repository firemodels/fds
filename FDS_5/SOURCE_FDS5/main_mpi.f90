...
CHARACTER(255), PARAMETER :: mainmpiid='$Id: main_mpi.f90,v 1.28 2007/04/30 17:13:26 mcgratta Exp $'
...
WRITE(LU0,'(A,I2,A,I2,A,A)') 'Mesh ',MYID+1,' of ', NUMPROCS,' is alive on ',PNAME(1:PNAMELEN)
...
COMPILE_DATE   = 'April 30, 2007'
VERSION_STRING = '5_RC3_MPI'
VERSION_NUMBER = 5.0
...
MAIN_LOOP: DO  
   ...
   IF (EX) ISTOP = 2
   CALL MPI_ALLGATHER(ISTOP(MYID+1),1,MPI_INTEGER,ISTOP,1, MPI_INTEGER,MPI_COMM_WORLD,IERR)
   ...
   IF (SYNCHRONIZE) THEN
      ...
      ELSE
         ...
         IF (STOP_CODE>0) ACTIVE_MESH(MYID+1) = .TRUE.
      ENDIF
   ELSE
      ACTIVE_MESH = .FALSE.
      DO NM=1,NMESHES
...
!     ENDIF
      ...
      IF (SYNCHRONIZE) THEN
         ...
         CALL MPI_ALLGATHER(ISTOP(NM),1,MPI_INTEGER,ISTOP,1, MPI_INTEGER,MPI_COMM_WORLD,IERR)
         IF (ANY(ISTOP>0)) THEN
            STOP_CODE = 1
...
IF (MYID==0) THEN
   DO N=1,MESHES(1)%N_STRINGS
      WRITE(LU4,'(A)') TRIM(MESHES(1)%STRING(N))
   ENDDO
