...
IF (N_GEOMETRY>0) THEN
   ...
   DO I = 1, N_GEOMETRY
      ...
      IF (TRIM(G%SURF_ID(1))=='null') THEN
         WRITE(LU_SMV,'(1X,A,1X,3(E13.6,1X),I2)') TRIM(G%TEXTURE_MAPPING), G%TEXTURE_ORIGIN, IS_TERRAIN_INT
      ELSE
         WRITE(LU_SMV,'(1X,A,1X,3(E13.6,1X),I2,1X,A,1X,A)') TRIM(G%TEXTURE_MAPPING), G%TEXTURE_ORIGIN, &
                                                    IS_TERRAIN_INT, '%',TRIM(G%SURF_ID(1))
      ENDIF
   ENDDO
   DO N=1,N_TRNF
      DO NM=1,NMESHES
