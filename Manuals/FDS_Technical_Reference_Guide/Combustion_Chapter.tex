...
It is more convenient for FDS to work in terms of mass fractions, $Y_{\alpha}$.  The concentration (mol/L) and mass fractions (kg/kg) are related by $C_{\alpha}=Y_{\alpha} \rho/W_{\alpha}$.  Substituting for $C_{\alpha}$ and combining Eqs.~(\ref{eq:rate_f}) and (\ref{eq:rate_cons}) yields:
\begin{equation}\label{eq:finite_rate_unit}
r^{\prime}_{F,i} = -\frac{1000}{\rho}k_{i} \; \prod \left(\frac{Y_{\alpha}\,\rho}{W_{\alpha}}\right)^{a_{\alpha,i}} \end{equation}
...
\frac{\mbox{d}Y_{\alpha}}{\mbox{d}t} = \sum_{i} r^{\prime}_{\alpha,i}
\end{equation}
