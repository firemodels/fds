...
\section{Shrinking and swelling materials (\texorpdfstring{{\bf shring\_swell}}{shrink\_swell})}
...
Figure~\ref{water_evaporation_7_plot} displays the energy balance for this case. The heat release rate, {\ct HRR}, of the fire is \underline{expected} to be 320~kW. The rate at which the water droplets extract energy from the system, {\ct Q\_PART}, is \underline{expected} to be
\be - \left( 4.184 \; \hbox{kJ/kg/K} \times 80 \; \hbox{K} + 2260 \; \hbox{kJ/kg} \right) \times 1/60 \; \hbox{kg/s} = -43.2 \; \hbox{kW} \ee
The sum of all the terms, {\ct Q\_TOTAL}, is \underline{expected} to be zero. The three other quantities, {\ct Q\_COND}, {\ct Q\_RADI}, {\ct Q\_CONV}, all have plausible values, but there is no way to determine the exact values.
