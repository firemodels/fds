...
!$OMP PRIVATE(Y_O2_CORR,Y_F_CORR,DYF,DELTA2,Q_BOUND_1,Q_BOUND_2,Q_BOUND_3,Q_NEW,CRIT_FLAME_TMPA)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         IC = CELL_INDEX(I,J,K)
         ...
         IF (LES .AND. EDDY_DISSIPATION) THEN
            ...
            IF (SCALE_MIX_TIME) THEN
               RE_DELTA = MAX(1._EB,SQRT(DELTA2*KRES(I,J,K))*RHO(I,J,K)/MU_DNS(I,J,K))
               MIX_TIME(I,J,K) = MIX_TIME(I,J,K)/(RE_DELTA**0.8_EB)
            ENDIF
         ENDIF
...
CONVERT_CO: IF (CO_PRODUCTION) THEN 
   ...
   !$OMP DO COLLAPSE(3) PRIVATE(K,J,I,Y_O2_0,Y_CO_0,Z_2_MIN,Q_BOUND_2,DYCO,RHOX,X_FU_0,X_O2_0,X_FU,X_O2,ETRM) &
