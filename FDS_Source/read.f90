...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                KINETIC_ENERGY_SOURCE,LAPSE_RATE,LES_FILTER_WIDTH,LIMIT_BAROCLINIC_TERM,MAX_CHEMISTRY_ITERATIONS,&
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IF (TRIM(QUANTITY)=='BAROCLINIC LIMITER') THEN
         STORE_B_LIMITER = .TRUE.
         PRESSURE_TOLERANCE = HUGE_EB
         IF (.NOT.LIMIT_BAROCLINIC_TERM) THEN
            WRITE(MESSAGE,'(A,I3,A)') 'ERROR: Promblem with SCLF ',NN,&
               ', BAROCLINIC LIMITER requires LIMIT_BAROCLINIC_TERM=T on MISC'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
      ENDIF
