...
Each peak can be characterized by the maximum value of the heat release rate~($\dot{q}_p$), the temperature ($T_p$), and the relative fraction of the original 
sample mass that undergoes this particular reaction ($Y_0$). The area under the curve
...
where $\nu_r$ is the fraction of the original mass that remains as residue. Sometimes this is referred to as the ``char yield.''
 
The MCC measurement is similar to TGA in that it is possible to derive the kinetic parameters, $A$ and $E$, for the various reactions from
...
\be
   r_{p,j} = \beta \, \frac{\dot{q}_{p,j}}{ \Delta H'}   \quad ; \quad 
   \Delta H'=\int_0^\infty \dot{q}(T) \, dT   \label{r_formula}
\ee
where $\dot{q}_{p,j}$ is the value of the $j$th heat release rate peak. The values, $Y_{0,j}$, can be estimated from the
...
exercise. Note that {\ct REFERENCE\_TEMPERATURE} is $T_p$, but in units of $^\circ$C. {\ct REFERENCE\_RATE} is actually
$r_p/Y_0$, in units of s$^{-1}$. {\ct HEATING\_RATE} is $\beta$ in units of K/min.
