...
SPECIES_LOOP_0: DO N=1,N_SPECIES
   ...
   ELSEIF (SS%MODE==MIXTURE_FRACTION_SPECIES) THEN
      IF (CO_PRODUCTION) THEN
         Z_VECTOR(1) = 0._EB
         Z_VECTOR(2) = REACTION(1)%Z_F
         Z_VECTOR(3) = 0._EB
         CALL GET_MOLECULAR_WEIGHT(Z_VECTOR,0._EB,MW_Z2)
         Z_VECTOR(2) = 0._EB
         Z_VECTOR(3) = REACTION(1)%Z_F
         CALL GET_MOLECULAR_WEIGHT(Z_VECTOR,0._EB,MW_Z3)
      ELSE
         MW_Z2 = SS0%MW
         Z_VECTOR(1) = 0._EB
         Z_VECTOR(2) = REACTION(1)%Z_F
...
SPECIES_LOOP_2: DO N=0,N_SPECIES
   ...
   DO ITMP=1,500
      ...
      IF (MU_USER(N)>=0._EB .AND. .NOT.CONSTANT_PROPERTIES) MU_N = MU_USER(N)*(ITMP*10/TMPA)**0.75_EB
      IF (MU_USER(N)>=0._EB .AND. CONSTANT_PROPERTIES)      MU_N = MU_USER(N)
      
      K_N = MU_N*SS%CP(ITMP)/PR
      IF (K_USER(N)>=0._EB .AND. .NOT.CONSTANT_PROPERTIES)  K_N = K_USER(N)*(ITMP*10/TMPA)**0.75_EB
      IF (K_USER(N)>=0._EB .AND. CONSTANT_PROPERTIES)       K_N = K_USER(N)
...
CHARACTER(30) :: SPEC_ID,QUANTITIES(1:10)
CHARACTER(25) :: COLOR
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(0:MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2), &
            ...
            IGNITION_TEMPERATURE,HEAT_OF_VAPORIZATION,REGRID_FACTOR,NET_HEAT_FLUX,L_DIVIDE
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%L_DIVIDE < 0._EB) THEN
      IF (SF%BACKING==EXPOSED) THEN 
         SF%L_DIVIDE = 0.5_EB * REAL(SF%N_LAYERS,EB)
      ELSE
         SF%L_DIVIDE = REAL(SF%N_LAYERS+1)
      ENDIF
   ENDIF
  ...
  ! Determine if the surface is combustible/burning
   ...
   DO NL=1,SF%N_LAYERS
      ...
      MATL_LOOP:DO NN=1,SF%N_LAYER_MATL(NL)
         ...
         ML => MATERIAL(NNN)
         IF (ML%N_REACTIONS == 0) CYCLE MATL_LOOP
         IF (I_FUEL /= 0) THEN
...
! Compute node coordinates 
   ...
   CALL GET_WALL_NODE_WEIGHTS(SF%N_CELLS,SF%N_LAYERS,SF%N_LAYER_CELLS,SF%LAYER_THICKNESS,SF%GEOMETRY, &
         SF%X_S,SF%L_DIVIDE,SF%DX,SF%RDX,SF%RDXN,SF%DX_WGT,SF%DXF,SF%DXB,SF%LAYER_INDEX,SF%MF_FRAC)
