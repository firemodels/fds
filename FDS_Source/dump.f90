...
USE PHYSICAL_FUNCTIONS, ONLY: GET_MASS_FRACTION,FED,FIC,GET_SPECIFIC_HEAT,GET_AVERAGE_SPECIFIC_HEAT,RELATIVE_HUMIDITY, &
                              ...
                              LES_FILTER_WIDTH_FUNCTION, GET_VISCOSITY
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,&
            ...
            MU_DNS,DISSIPATION_RATE,S11,S22,S33,S12,S13,S23,DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,ONTHDIV,SS,ETA
...
INTEGER :: N,I,J,K,NN,IL,III,JJJ,KKK,Y_INDEX,Z_INDEX,PART_INDEX,ITMP,IP,JP,KP,FLOW_INDEX,IW,FED_ACTIVITY, &
           IP1,JP1,KP1,IM1,JM1,KM1,IIM1,JJM1,KKM1
CHARACTER(100) :: MESSAGE
...
SELECT CASE(IND)
   ...
   CASE(21)  ! RELATIVE HUMIDITY
      ...
      ELSE
         ...
         GAS_PHASE_OUTPUT_RES = RELATIVE_HUMIDITY(Y_ALL(H2O_INDEX),TMP(II,JJ,KK))
      ENDIF
   CASE(22)  ! HS
   ...
   CASE(61)  ! U MASK
      GAS_PHASE_OUTPUT_RES = U_MASK(II,JJ,KK)
   CASE(62)  ! V MASK
      GAS_PHASE_OUTPUT_RES = V_MASK(II,JJ,KK)
   CASE(63)  ! W MASK
      GAS_PHASE_OUTPUT_RES = W_MASK(II,JJ,KK)
   CASE(64)  ! P MASK
      GAS_PHASE_OUTPUT_RES = P_MASK(II,JJ,KK)  
   ...
   CASE(85)  ! KOLMOGOROV LENGTH SCALE
      ...
      IF (N_TRACKED_SPECIES > 0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_VISCOSITY(ZZ_GET,MU_DNS,TMP(II,JJ,KK))
      GAS_PHASE_OUTPUT_RES = ((MU_DNS/RHO(II,JJ,KK))**3/(DISSIPATION_RATE+EPS))**0.25_EB
   ...
   CASE(99) ! CHEMISTRY SUBITERATIONS
      GAS_PHASE_OUTPUT_RES = MAX_CHEM_SUBIT(NM)
      MAX_CHEM_SUBIT(NM)=0
   ...
   CASE(155) ! PATH OBSCURATION  *********
      ...
      ENDDO
      GAS_PHASE_OUTPUT_RES =  (1._EB - EXP(-1._EB*MASS_EXT_COEF*EXT_COEF))*100._EB
...
REAL(EB) :: CONCORR,FLUX,DFLUXDT,SOLID_PHASE_OUTPUT_OLD,VOLSUM,C_S_ADJUST_UNITS,TF_MEAN,MFT,&
            ZZ_GET(0:N_TRACKED_SPECIES),Y_SPECIES,KSGS,DEPTH
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE( 7) ! BURNING RATE
      ...
      ENDIF
   CASE( 8) ! HRRPUM
      SOLID_PHASE_OUTPUT = ONE_D%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)*0.001*REACTION(1)%HEAT_OF_COMBUSTION/ &
   ...
   CASE(25,26) ! SURFACE DENSITY, NORMALIZED SURFACE DENSITY
