...
The calculations described in this section check the conservation of mass for the gravitational settling algorithms. The gravitational settling algorithm involves both the settling of soot particles in the gas phase and the accumulation of soot on the condensed phase. For this example, the initial soot concentration in a 1~m$^3$ box is set as 100~mg/m$^3$ via the {\ct INIT} line, and the soot is allowed to deposit to the bottom of the box via gravitational settling only. By default, if a species is defined as an aerosol by setting {\ct AEROSOL=.TRUE.} on the {\ct SPEC} line, then all of the aerosol deposition mechanisms (gravitational, thermophoretic, and turbulent) will act upon that species. For this example, gravitational deposition is enabled and the other deposition mechanisms are disabled by using the follows inputs on the {\ct MISC} line:
...
In this example, the initial gas-phase soot concentration is specified, and the only removal term (i.e., active deposition mechanism) is gravitational deposition. The dynamic gas-phase soot concentration can then be represented as
...
where C$_0$ is the initial gas-phase soot concentration (100~mg/m$^3$). In this example, the relevant surface area is the area of the lower surface where the soot deposits, and thus the surface area to volume ratio (A/V) is equal to 1~m$^{-1}$. For a soot particle with a diameter of 1~$\mu$m, the terminal settling velocity is equal to 1.9747$\times 10^{-4}$~m/s. The governing equations for the gravitational settling algorithms and settling velocities are discussed in more detail in the FDS Technical Reference Guide~\cite{FDS_Math_Guide}. Figure~\ref{fig:deposition_mass} shows a comparison of the analytical and simulated results for the time evolution of the gas-phase soot concentration.
...
\begin{figure}[ht]
   \begin{center}
      \includegraphics[height=3.00in]{SCRIPT_FIGURES/HVAC_flow_loss}
...
\section{Cut-cell Algorithm (\texorpdfstring{\bf tri\_cube\_cut\_cell\_test\_}{tri\_cube\_cut\_cell\_test\_})}
