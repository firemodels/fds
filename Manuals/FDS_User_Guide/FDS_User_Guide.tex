...
\item The stoichiometry is given before each species and is separated by an asterisk. Real numbers are allowed but exponential notation is not ({\em i.e.}, {\ct 201.1} but not {\ct 2.011E2}).
...
\noindent In {\ct REAC ID = 'R2'}, {\ct WATER VAPOR} concentration impacts the reaction rate as shown by its inclusion on the {\ct SPEC\_ID} and {\ct N\_S} lines, however it does not explicitly participate in the reaction because it is not defined on the {\ct SMIX\_ID} or {\ct NU} lines.
