...
!Compute fuel heat of formation
      ...
      IF (RN%NU(H2O_INDEX) < 0._EB) THEN
         ...
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      RN%NU(O2_INDEX)  = RN%NU(CO2_INDEX) + 0.5_EB*(RN%NU(CO_INDEX) - 0)
...
!Compute fuel heat of formation
      ...
      IF (RN%NU(H2O_INDEX) < 0._EB) THEN
         ...
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      RN%NU(O2_INDEX)  = RN%NU(CO2_INDEX) + 0.5_EB - 0
...
IF(MIXTURE_FRACTION) THEN
   ...
   IF (H2O_INDEX > 0)   STATE_SPECIES(H2O_INDEX)   = 'WATER VAPOR'
   IF (CO_INDEX > 0)    STATE_SPECIES(CO_INDEX)    = 'CARBON MONOXIDE'
   IF (H2_INDEX > 0)    STATE_SPECIES(H2_INDEX)    = 'HYDROGEN'
   IF (SOOT_INDEX > 0)  STATE_SPECIES(SOOT_INDEX)  = 'SOOT'
   IF (OTHER_INDEX > 0) STATE_SPECIES(OTHER_INDEX) = 'NITROGEN'
ENDIF
