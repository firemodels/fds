...
//  keyframe *framejm1;
  ...
  for(i=0;i<ntours;i++){
    ...
    for(keyj=(touri->first_frame).next,j=0;keyj->next!=NULL;keyj=keyj->next,j++){
      ...
      *tourknotstourlist_copy++ = touri;
      ...
      if(touri->periodic==0&&j==0){
        ...
        keyj->d_eye[3]=key2->azimuth - key1->azimuth;
        ...
        keyj->s_eye[3]=key1->azimuth - key0->azimuth;
    ...
    for(j=0;j<view_ntimes;j++){
      ...
      pj->elevation = hermiteeye(f1,5,kf1,kf2,&dummy);
   ...
   // construct distance array based on number of points in each interval
    ...
    for(j=0;j<view_ntimes;j++){
      ...
      if(kf1->viewtype==0||kf2->viewtype==0){
        ...
        dz = tan(pj->elevation*pi/180.0)/10.0;
        oview[0]=eye[0]+dx2;
        oview[1]=eye[1]+dy2;
        oview[2]=eye[2]+dz;
      }
...
float hermiteeye(float t, int i, keyframe *kf1, keyframe *kf2, float *slope){
  ...
  switch (i) {
  case 5:
    p0=kf1->nodeval.elevation;
    p1=kf2->nodeval.elevation;
    break;
  case 3:
    p0 = kf1->azimuth;
    p1 = kf2->azimuth;
...
void adjustviewangle(keyframe *kf, float *azimuth, float *elevation){
  float dx, dy, dz;
  float dx2, dy2;
