...
SUBROUTINE UPDATE_CONTROLS(T,DT)
...
CONTROL_LOOP_2: DO NC=1,N_CTRL
   ...
   CALL EVALUATE_CONTROL(T,NC,DT)
END DO CONTROL_LOOP_2
   
END SUBROUTINE UPDATE_CONTROLS

RECURSIVE SUBROUTINE EVALUATE_CONTROL(T,ID,DT)
...
CONTROL_SELECT: SELECT CASE (CF%CONTROL_INDEX)
         ...
         SELECT CASE (CF%INPUT_TYPE(NC))
            ...
            CASE (CONTROL_INPUT)
               IF (.NOT. CONTROL(CF%INPUT(NC))%UPDATED) THEN
                  CALL EVALUATE_CONTROL(T,CF%INPUT(NC),DT) 
                   CF => CONTROL(ID)
               ENDIF
  ...
  CASE (OR_GATE)
      ...
      SELECT CASE (CF%INPUT_TYPE(1))
         ...
         CASE (CONTROL_INPUT)
            ...
            IF (.NOT. CONTROL(CF%INPUT(1))%UPDATED) THEN
               CALL EVALUATE_CONTROL(T,CF%INPUT(1),DT) 
               CF => CONTROL(ID)
            ENDIF
      ...
      SELECT CASE (CF%INPUT_TYPE(2))
         ...
         CASE (CONTROL_INPUT)
            IF (.NOT. CONTROL(CF%INPUT(2))%UPDATED) THEN
               CALL EVALUATE_CONTROL(T,CF%INPUT(2),DT)  
               CF => CONTROL(ID)
            ENDIF
   ...
   CASE (CF_PID)
      ...
      CF%INTEGRAL = 0.5_EB*DT*(CF%INSTANT_VALUE+CF%PREVIOUS_VALUE)
      CF%INSTANT_VALUE = CF%PROPORTIONAL_GAIN * CF%INSTANT_VALUE + &
                         CF%INTEGRAL_GAIN * CF%INTEGRAL + &
                         CF%DIFFERENTIAL_GAIN * (CF%INSTANT_VALUE - CF%PREVIOUS_VALUE) / DT
