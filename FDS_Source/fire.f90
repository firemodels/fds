...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_ITERATIONS
   ...
   IF (SUPPRESSION .AND. TIME_ITER == 1) EXTINCT = FUNC_EXTINCT(ZZ_MIXED,TMP_MIXED_ZONE)
   ...
   INTEGRATOR_SELECT: SELECT CASE (COMBUSTION_ODE)
      ...
      CASE (EXPLICIT_EULER) ! Simple chemistry
         ...
         DO SR=0,N_SERIES_REACTIONS
            ZZ_MIXED = FUNC_FE(ZZ_MIXED,DT_SUB,TMP_MIXED_ZONE,RHO_HAT)
         ENDDO         
      ...
      CASE (RK2_RICHARDSON) ! Finite-rate (or mixed finite-rate/fast) chemistry
         ...
         RICH_EX_LOOP: DO RICH_ITER =1,RICH_ITER_MAX
            ...
            A1 = FUNC_A(ZZ_MIXED,DT_SUB,1,TMP_MIXED_ZONE,RHO_HAT) ! FDS Tech Guide (E.3)
            A2 = FUNC_A(ZZ_MIXED,DT_SUB,2,TMP_MIXED_ZONE,RHO_HAT) ! FDS Tech Guide (E.4)
            A4 = FUNC_A(ZZ_MIXED,DT_SUB,4,TMP_MIXED_ZONE,RHO_HAT)
   ...
   Q_SUM = 0._EB
   IF (MAXVAL(ABS(ZZ_GET-ZZ_TEMP)) > ZZ_MIN_GLOBAL) THEN
...
FUNCTION FUNC_FE(ZZ_IN,DT_LOC,TMP_MIXED_ZONE,RHO_HAT)
USE COMP_FUNCTIONS, ONLY:SHUTDOWN
REAL(EB), INTENT(IN) :: RHO_HAT,TMP_MIXED_ZONE
REAL(EB), INTENT(IN) :: ZZ_IN(0:N_TRACKED_SPECIES),DT_LOC
REAL(EB) :: FUNC_FE(0:N_TRACKED_SPECIES),ZZ_0(0:N_TRACKED_SPECIES),ZZ_NEW(0:N_TRACKED_SPECIES),DZZ(0:N_TRACKED_SPECIES),&
            BOUNDEDNESS_CORRECTION
INTEGER :: SR
...
IF (ANY(REACTION(:)%FAST_CHEMISTRY)) THEN
   DO SR = 0,N_SERIES_REACTIONS
      CALL REACTION_RATE(DZZ,ZZ_0,DT_LOC,RHO_HAT,TMP_MIXED_ZONE,1)
      ZZ_NEW = ZZ_0 + DZZ ! test Forward Euler step
      BOUNDEDNESS_CORRECTION = FUNC_BCOR(ZZ_0,ZZ_NEW) ! Reaction rate boundedness correction
      ZZ_NEW = ZZ_0 + DZZ*BOUNDEDNESS_CORRECTION ! corrected FE step for all species
      ZZ_0 = ZZ_NEW
   ENDDO
ENDIF

IF (.NOT.ALL(REACTION(:)%FAST_CHEMISTRY)) THEN
   CALL REACTION_RATE(DZZ,ZZ_0,DT_LOC,RHO_HAT,TMP_MIXED_ZONE,2)
   ZZ_NEW = ZZ_0 + DZZ
...
ZZ_NEW = MAX(ZZ_NEW,0._EB)
ZZ_NEW = MIN(ZZ_NEW,1._EB)
...
DO NS=0,N_TRACKED_SPECIES
   IF (ZZ_NEW(NS)<0._EB) THEN
      ...
      BCOR = MIN( BCOR, DZ_IB/(DZ_IB + DZ_OB + TWO_EPSILON_EB) )
   ENDIF
...
FUNCTION FUNC_A(ZZ_IN,DT_SUB,N_INC,TMP_MIXED_ZONE,RHO_HAT)
...
DO N=1,N_INC
   ZZ_1 = FUNC_FE(ZZ_0,DT_LOC,TMP_MIXED_ZONE,RHO_HAT)
   ZZ_2 = FUNC_FE(ZZ_1,DT_LOC,TMP_MIXED_ZONE,RHO_HAT)
...
REAL(EB) :: DZ_F(1:N_REACTIONS),YY_PRIMITIVE(1:N_SPECIES),G
INTEGER :: I,NS
INTEGER, PARAMETER :: FAST=1,FINITE_RATE=2
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
...
KINETICS_SELECT: SELECT CASE(KINETICS)
   CASE(FAST)
      ...
      REACTION_LOOP_1: DO I=1,N_REACTIONS
         ...
         DZ_F(I) = MIN(ZZ_0(RN%FUEL_SMIX_INDEX),ZZ_0(RN%AIR_SMIX_INDEX)/RN%S)
   ...
   CASE(FINITE_RATE) 
      REACTION_LOOP_2: DO I=1,N_REACTIONS
         ...
         ENDDO
         DZ_F(I) = RN%A_PRIME*RHO_0**RN%RHO_EXPONENT*TMP_0**RN%N_T*EXP(-RN%E/(R0*TMP_0))
         DO NS=1,N_SPECIES
            IF(RN%N_S(NS)>= -998._EB)  DZ_F(I) = YY_PRIMITIVE(NS)**RN%N_S(NS)*DZ_F(I)
         ENDDO
         IF(RN%REVERSE) THEN ! compute equilibrium constant
            CALL GET_GIBBS_FREE_ENERGY(G,RN%NU,TMP_0)
            RN%K = EXP(-G/(R0*TMP_0))
         ENDIF
...
LOGICAL FUNCTION FUNC_EXTINCT(ZZ_MIXED_IN,TMP_MIXED_ZONE)
...
IF (ANY(REACTION(:)%FAST_CHEMISTRY)) THEN
   SELECT CASE (EXTINCT_MOD)
      CASE(EXTINCTION_1)
         FUNC_EXTINCT = EXTINCT_1(ZZ_MIXED_IN,TMP_MIXED_ZONE)
      CASE(EXTINCTION_2)
         FUNC_EXTINCT = EXTINCT_2(ZZ_MIXED_IN,TMP_MIXED_ZONE)
      CASE(EXTINCTION_3)
         FUNC_EXTINCT = .FALSE.
   END SELECT
ENDIF
...
LOGICAL FUNCTION EXTINCT_1(ZZ_IN,TMP_MIXED_ZONE)
