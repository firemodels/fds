...
then
  ...
  echo " -D s - delay s seconds [default: 0]"
  ...
  echo " -M mpi_dir  - MPI directory [default: defined in .bashrc_fds]"
...
# use value of MPIDIST defined in .bashrc_fds as default
MPIDIST_LOCAL=$MPIDIST
MPI_DIR=
MPI_FLAG=

# read in parameters from command line

while getopts 'bcD:d:e:f:j:M:m:n:o:p:q:rstv' OPTION
do
case $OPTION  in
  b)
   use_debug=1
...
if [ "$MPI_DIR" != "" ] ; then
   if [ -d "$MPI_DIR" ] ; then
     MPIDIST_LOCAL=$MPI_DIR
     MPI_FLAG="-x QMPIDIST=$MPIDIST_LOCAL"
   else
     echo "***Error: the MPI directory, $MPI_DIR, does not exist"
     exit
   fi
   if [ $use_repository -eq 1 ] ; then
     echo "***Error: an executable must be specified when"
     echo "          using the -M MPI directory option."
     exit
   fi
fi
...
if [ $nmpi_processes -gt 1 ] ; then
  MPIRUN="$MPIDIST_LOCAL/bin/mpirun $REPORT_BINDINGS $SOCKET_OPTION -np $nmpi_processes"
  TITLE="$infile(MPI)"
  if [ "$FDSNETWORK" == "infiniband" ]; then
    TITLE="$infile(MPI_IB)"
  fi
