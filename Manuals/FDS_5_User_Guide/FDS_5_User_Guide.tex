...
Boris Stock                             & BFT Cognos Gmb, Germany                  \\ \hline
...
This section contains examples that test the one-dimensional heat conduction solver in FDS, along with an example of a simple
...
\subsection{A Simple Thermoplastic ({\bf thermoplastic}) }
\label{thermoplastic}
...
All fire simulations must include a ``model'' of the fire itself. Almost always, these models are simplifications of the actual burning item. The examples
in this chapter are merely starting points for more elaborate fire simulations. The point of these examples is to demonstrate that any simulation of a fire ought
to first start with a model of just the fire itself and its immediate surroundings. It is a tremendous waste of time to begin a project
by setting up a calculation of the fire, smoke and heat transport,
target/detector response, and so on, without first doing quick simulations of the fire itself to ensure that it conforms to either a test burn or your own
conceptual model.
