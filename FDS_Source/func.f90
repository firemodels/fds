...
ELSE
   ...
   IF (SF%SHRINK) THEN
      ...
      X_S_NEW(0:NWP) = THICKNESS-ONE_D%X(0:NWP)
   ELSE
      THICKNESS = SUM(SF%LAYER_THICKNESS)
      NWP = SF%N_CELLS
      X_S_NEW(0:NWP) = THICKNESS-SF%X_S(0:NWP)
   ENDIF
   SURFACE_DENSITY = 0._EB
   DO II2=1,NWP
      IF (MODE==0) THEN
         WGT = (X_S_NEW(II2-1)**I_GRAD-X_S_NEW(II2)**I_GRAD)/(REAL(I_GRAD,EB)*SF%THICKNESS**(I_GRAD-1))
      ELSE
         WGT = (X_S_NEW(II2-1)**I_GRAD-X_S_NEW(II2)**I_GRAD)/SF%THICKNESS**I_GRAD
      ENDIF
