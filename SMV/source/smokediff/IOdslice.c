...
void diff_slices(FILE *stream_out){
  ...
  for(j=0;j<caseinfo->nsliceinfo;j++){
    ...
    FORTopenslice(fullfile1,&unit1,&caseinfo->endian,&is1a,&is2a,&js1a,&js2a,&ks1a,&ks2a, &error1 FDSLABELLEN,len1);
    unit2=12;
    FORTget_file_unit(&unit2,&unit2);
    len2=strlen(fullfile2);
    FORTopenslice(fullfile2,&unit2,&caseinfo->endian,&is1b,&is2b,&js1b,&js2b,&ks1b,&ks2b, &error2a FDSLABELLEN,len2);
