...
REAL(EB) :: H_L,H_V,ZZ_GET(0:N_TRACKED_SPECIES),CPBAR,H_G_S,H_G_S_REF,H_L_REF,TMP_REF,TMP_MELT,TMP_V,TMP_WGT,DENSITY
TYPE(PARTICLE_CLASS_TYPE), POINTER :: PC=>NULL()
TYPE(SPECIES_TYPE),POINTER:: SS=>NULL()
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   IF (PY%FLOW_RATE > 0._EB .OR. PY%OPERATING_PRESSURE > 0._EB) THEN
      ...
      DO NN=1,N_PART
         IF (TRIM(PARTICLE_CLASS(NN)%ID)==TRIM(PART_ID)) THEN
