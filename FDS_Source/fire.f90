...
REAL(EB) :: FUNC_FE(0:N_TRACKED_SPECIES),ZZ_0(0:N_TRACKED_SPECIES),ZZ_NEW(0:N_TRACKED_SPECIES),DZZDT(0:N_TRACKED_SPECIES),&
            RATE_CONSTANT(1:N_REACTIONS),BOUNDEDNESS_CORRECTION
INTEGER :: NR,NS
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
...
REACTION_LOOP: DO NR = 1,N_REACTIONS
   ...
   DZZDT = DZZDT + RN%NU_MW_O_MW_F*RATE_CONSTANT(NR)
ENDDO REACTION_LOOP

ZZ_NEW = ZZ_0 + DT_LOC*DZZDT ! test Forward Euler step

! Reaction rate boundedness correction

BOUNDEDNESS_CORRECTION = 1._EB
DO NS=0,N_TRACKED_SPECIES
   IF (ZZ_NEW(NS)>=0._EB .AND. ZZ_NEW(NS)<=1._EB) CYCLE
   BOUNDEDNESS_CORRECTION = MIN( BOUNDEDNESS_CORRECTION, ZZ_0(NS)/(ZZ_0(NS) + ABS(ZZ_NEW(NS)) + TWO_EPSILON_EB) )
ENDDO

ZZ_NEW = ZZ_0 + DT_LOC*DZZDT*BOUNDEDNESS_CORRECTION ! corrected FE step for all species
...
REAL(EB) :: YY_PRIMITIVE(1:N_SPECIES),DZ_F(1:N_REACTIONS),DZ_FR(1:N_REACTIONS),DZ_FRAC_F(1:N_REACTIONS),&
            ZZ_OX,ZZ_OX_STOICH,ZZ_MIXED_FR(0:N_TRACKED_SPECIES),GFE,DZ_F_SUM
REAL(EB), PARAMETER :: ZZ_MIN=1.E-10_EB
INTEGER :: NS,NR
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
...
DO NR = 1,N_REACTIONS
   ...
   IF (RN%FAST_CHEMISTRY .AND. RN%HEAT_OF_COMBUSTION > 0._EB) THEN
      ZZ_OX_STOICH = ZZ_OX_STOICH + ZZ_MIXED_IN(RN%FUEL_SMIX_INDEX)*RN%S !Stoich mass O2
      ZZ_OX = ZZ_MIXED_IN(RN%AIR_SMIX_INDEX) ! Mass O2 in cell
   ENDIF
ENDDO
...
ELSE   
      ...
      DO NR = 1,N_REACTIONS
         ...
         IF (RN%FAST_CHEMISTRY .AND. RN%HEAT_OF_COMBUSTION > 0._EB .AND. .NOT. EXTINCT(NR)) THEN
            DO NS = 0,N_TRACKED_SPECIES
               ...
               ZZ_MIXED_FR(NS) = MAX(0._EB,ZZ_MIXED_FR(NS)) ! remove stoich fast chemistry reactants
            ENDDO
         ENDIF
      ENDDO
   ENDIF
   DO NR = 1,N_REACTIONS
      ...
      IF (.NOT. RN%FAST_CHEMISTRY) THEN ! finite rate reactions
         ...
         ENDDO
      ELSE ! fast chemistry
         DZ_F(NR) = MIN(ZZ_MIXED_IN(RN%FUEL_SMIX_INDEX),ZZ_MIXED_IN(RN%AIR_SMIX_INDEX)/RN%S)       
      ENDIF
      IF (ZZ_OX_STOICH > ZZ_OX .AND. RN%HEAT_OF_COMBUSTION > 0._EB) THEN
