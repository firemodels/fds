...
def upload_find_auth(file_path, project_name, summary, labels=None,
                     config_dir=None, user_name=None, passwd_file_dir=None, tries=3):
  ...
  If passwd_file_dir is not None, use the first line from the file as the Google Code password.
  ...
  Args:
    ...
    user_name: Your Google account name.
    passwd_file_dir: The full path to the gc.passwd file on the local machine.
  ...
  if passwd_file_dir != 'none':
    # Try to load password from first line of gc.passwd file.
    if passwd_file_dir != None:
        #Create passwd_file_full_path
        passwd_file_full_path = passwd_file_dir+'gc.passwd'
        print 'Full path is:', passwd_file_full_path
        # Open the file itself
        f = open(passwd_file_full_path, 'r')
  ...
  while tries > 0:
    ...
    if password is None:
      ...
      print 'getpass result:', password
      
    status, reason, url = upload(file_path, project_name, user_name, password, summary, labels)
    # Returns 403 Forbidden instead of 401 Unauthorized for bad credentials as of 2007-07-17.
...
def main():
  ...
  parser.add_option('--passwd-file-dir', dest='passwd_file_dir',
                    help='The path to the password file \'gc.passwd\' stored locally on machine.')
  ...
  status, reason, url = upload_find_auth(file_path, options.project,
                                         ...
                                         options.config_dir, options.user, options.passwd_file_dir)
  if url:
