...
REAL(EB) :: Y_FU_0,Y_P_0,Y_LIMITER,A,ETRM,Y_O2_0,Y_CO_0,DYF,DX_FDT,HFAC_F,DTT,DELTA, & 
            ...
            H_F_0,H_F_N,H_G_0,H_G_N,DYAIR,DELTAH_F,TAU_D,TAU_U,TAU_G,EPSK,KSGS,&
...
O2_F_RATIO = RN%NU(0)*SPECIES_MIXTURE(1)%MW*SPECIES_MIXTURE(1)%VOLUME_FRACTION(O2_INDEX)/(RN%NU(1)*SPECIES_MIXTURE(2)%MW)
...
!$OMP PRIVATE(Y_O2_CORR,Y_F_CORR,DYF,DELTA,Q_BOUND_1,Q_BOUND_2,Q_NEW,CRIT_FLAME_TMPA,YY_GET,DYAIR,H_F_0,H_F_N,H_G_0,H_G_N) &
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         IC = CELL_INDEX(I,J,K)
         IF (SOLID(IC)) CYCLE
         Y_FU_0  = MAX(0._EB,MIN(1._EB,YY(I,J,K,I_FUEL)))*SPECIES_MIXTURE(2)%MASS_FRACTION(FUEL_INDEX)
         ...
         IF_SUPPRESSION: IF (SUPPRESSION) THEN  ! Get maximum O2 in the current and neighboring cells to see if flame viable
            ...
            IF (EXTINCTION2) THEN
               ...
               YY_GET = 0._EB
               YY_GET(I_FUEL) = 1._EB
               CALL GET_AVERAGE_SPECIFIC_HEAT(YY_GET,H_F_0,TMP(I,J,K)) !INTENT: IN,OUT,IN
               CALL GET_AVERAGE_SPECIFIC_HEAT(YY_GET,H_F_N,RN%CRIT_FLAME_TMP) !INTENT: IN,OUT,IN
               YY_GET = YY(I,J,K,:)
               YY_GET(I_FUEL) = 0._EB
               YY_GET = YY_GET / (1._EB - Y_FU_0)
               CALL GET_AVERAGE_SPECIFIC_HEAT(YY_GET,H_G_0,TMP(I,J,K)) !INTENT: IN,OUT,IN
               CALL GET_AVERAGE_SPECIFIC_HEAT(YY_GET,H_G_N,RN%CRIT_FLAME_TMP) !INTENT: IN,OUT,IN
            ...
            ELSE
               ...
               IF (Y_O2_MAX < Y_O2_CORR .OR. Y_FU_MAX**SPECIES_MIXTURE(1)%MASS_FRACTION(FUEL_INDEX) < Y_F_CORR) CYCLE
            ENDIF
...
CONVERT_CO: IF (CO_PRODUCTION) THEN 
   ...
   !$OMP SCHEDULE(DYNAMIC) DO COLLAPSE(3) PRIVATE(K,J,I,Y_O2_0,Y_CO_0,DYCO,RHOX,X_FU_0,X_O2_0,X_FU,X_O2,ETRM) &
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            IF ((TMP(I,J,K) < RN%THRESHOLD_TEMP .AND. Q(I,J,K) <= ZERO_P) .OR. Q(I,J,K)>=Q_UPPER) CYCLE
...
!$OMP PRIVATE (NS,DELTA,MIX_TIME,TAU_D,TAU_G,TAU_U,DUDX,DUDY,DUDZ,DVDDVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,SS2,S12,S13,S23,EPSK,KSGS,ETRM) &
!$OMP PRIVATE(DT_FAC,X_S,DT_FAC2,X_N,QFAC,YY_GET)
DO K=1,KBAR
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         CALC_ETRM = .FALSE.
