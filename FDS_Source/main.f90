...
OLD_PRESSURE_SCHEME_IF: IF (OLD_PRESSURE_SCHEME) THEN
   IF (ITERATE_PRESSURE) THEN
      DO NM=1,NMESHES
         ...
         IF (PREDICTOR) THEN
            DO IW=1,MESHES(NM)%N_EXTERNAL_WALL_CELLS
...
CALL MPI_GATHERV(NODE_RSUM(1,DISPLS(MYID)+1),COUNTS_HVAC(MYID),MPI_DOUBLE_PRECISION,NODE_RSUM, &
                    COUNTS_HVAC,DISPLS_HVAC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
CALL MPI_GATHERV(NODE_YY(1,1,DISPLS(MYID)+1),COUNTS_HVAC(MYID)*N_SPECIES,MPI_DOUBLE_PRECISION,NODE_YY, &
                    COUNTS_HVAC*N_SPECIES,DISPLS_HVAC*N_SPECIES,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
...
IF (LEAK_DUCTS > 0) THEN
   ...
   CALL MPI_GATHERV(FDS_LEAK_AREA(0,0,DISPLS(MYID)+1),COUNTS_LEAK(MYID),MPI_DOUBLE_PRECISION,FDS_LEAK_AREA, &
                     COUNTS_LEAK,DISPLS_LEAK,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)                     
   CALL MPI_GATHERV(LEAK_YY(0,0,1,DISPLS(MYID)+1),COUNTS_LEAK(MYID)*N_SPECIES,MPI_DOUBLE_PRECISION,LEAK_YY, &
                     COUNTS_LEAK*N_SPECIES,DISPLS_LEAK*N_SPECIES,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF
