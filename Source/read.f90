...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%MESH>0) THEN
      IF (PROCESS(DV%MESH)==MYID) THEN
         M => MESHES(DV%MESH)
         CALL GET_IJK(DV%X,DV%Y,DV%Z,DV%MESH,XI,YJ,ZK,DV%I,DV%J,DV%K)
         IF (OUTPUT_QUANTITY(QUANTITY_INDEX)%CELL_POSITION==CELL_CENTER) THEN
            DV%I = MAX(1,MIN(DV%I,M%IBAR))
            DV%J = MAX(1,MIN(DV%J,M%JBAR))
            DV%K = MAX(1,MIN(DV%K,M%KBAR))
         ELSEIF (OUTPUT_QUANTITY(QUANTITY_INDEX)%CELL_POSITION==CELL_FACE) THEN
            SELECT CASE(OUTPUT_QUANTITY(QUANTITY_INDEX)%IOR)
               CASE(1) ; DV%I = NINT(XI)
               CASE(2) ; DV%J = NINT(YJ)
               CASE(3) ; DV%K = NINT(ZK)
            END SELECT
         ELSEIF (OUTPUT_QUANTITY(QUANTITY_INDEX)%CELL_POSITION==CELL_EDGE) THEN
            SELECT CASE(OUTPUT_QUANTITY(QUANTITY_INDEX)%IOR)
               CASE(1) ; DV%J = NINT(YJ) ; DV%K = NINT(ZK)
               CASE(2) ; DV%I = NINT(XI) ; DV%K = NINT(ZK)
               CASE(3) ; DV%I = NINT(XI) ; DV%J = NINT(YJ)
            END SELECT
         ENDIF
      ELSE
