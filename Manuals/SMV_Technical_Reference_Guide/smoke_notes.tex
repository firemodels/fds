...
suitable for applications where realism is required such as in a fire fighting simulator.
...
This \paper\ describes two methods for visualizing smoke realistically both using a form of the radiation transport equation\cite{Siegel:2001} to model
...
account interactions between light and smoke such as absorption, emission, and scattering (both in and out)
requires the solution of the the volume rendering equation\cite{levoy:1988}
also called the radiation transport equation (RTE)\cite{Siegel:2001}.
This equation models how light appears to an observer after interacting with a participating media in this case smoke.
The equation solved here for visualization purposes is the same one used in a fire model such as FDS\cite{FDS_Tech_Guide}
...
They
differ in the path length over which they are integrated.
...
absorption and out-scattering while components increasing radiance along a ray are emission and in-scattering.}
\label{figRadiance}
\end{figure}
