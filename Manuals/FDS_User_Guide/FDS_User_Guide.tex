...
1.4, based on an assumption of a perfect diatomic molecule like nitrogen. If you want FDS to assume that the specific heats of the gases are independent of
temperature and equal to
\be c_{p,\alpha} = \frac{\gamma}{\gamma-1} \frac{\cal R}{W_\alpha} \ee
set {\ct CONSTANT\_SPECIFIC\_HEAT=.TRUE.}
...
To reduce the computational cost of the HVAC solver, a duct should be considered as any length of duct that connects either a terminal (e.g. a connection to the FDS domain or the ambient) to either a filter or a node with more than two ducts attached.  For example the top of Figure~\ref{fig:HVAC_Simplify} shows a segment of an HVAC system where flow from a tee goes through an expansion fitting, two elbows, an expansion fitting, and a straight length of duct before it terminates as a connection to the FDS domain.  This could be input as each individual fitting or duct with its associated area and loss as shown in the middle of the figure; however, this would result in five duct segments (one for each component) with six node connections resulting in eleven parameters (five velocities and six pressures) which must be solved for.  This is not needed since whatever the flow rate is in any one segment of the duct, that same flow rate exists in all other segments and, thus, the velocties in any segment can be found by taking the area ratios, $v_1/v_2=A_2/A_1$.  Since flow losses are proportional to the square of the velocity, an equivelant duct can be constructed using the total length of the duct, a representative diameter, and summing the losses. Similarly, the pressure drops through the entire duct can be found by summing all of the fitting, $K$, and wall losses, $f$, through the duct as follows:

\be K_{eff} = \sum_i {K_i \frac {A_{eff}}{A_i}} \ee
\be f_{eff} = \sum_j {f_j \frac {d_{eff} A_{eff}}{d_j A_j}} \ee

where i is a fitting, j is a duct segment, A is the area of the duct or fitting, and d is the diameter of the duct.
...
\noindent
It is also possible to initialize monodisperse particles pointwise by specifying the droplet position (m), velocity (m/s), diameter (microns), and temperature ($^\circ$C).  This feature allows the user to specify an arbitrary droplet distribution.  This is done by specifying a {\ct TABL} that defines all the particles to be inserted.
...
droplets less than {\ct MINIMUM\_DIAMETER} are assumed to evaporate in a single time step. The default {\ct MINIMUM\_DIAMETER} is set to 0.005 the value of {\ct DIAMETER}. The droplet distribution is divided into a series of bins for picking particle size.  To avoid very small particle weights, the distribution is clipped at the cummulative fractions of {\ct CDF\_CUTOFF} and 1 - {\ct CDF\_CUTOFF}.
...
{\ct GAMMA}                                     & Real          & Section~\ref{info:MISC_Basics}                        &               & 1.4               \\ \hline
...
{\ct GAMMA}                         & Real            & Section~\ref{thermal_part_props}        &           & 1.4           \\ \hline
...
{\ct CABLE\_DIAMETER}                   & Real          & Section~\ref{info:THIEF}                  & m                     & 0.02      \\ \hline
{\ct CABLE\_FAILURE\_TEMPERATURE}       & Real          & Section~\ref{info:THIEF}                  & $^\circ$C             & 400       \\ \hline
{\ct CABLE\_JACKET\_THICKNESS}          & Real          & Section~\ref{info:THIEF}                  & m                     & 0.002     \\ \hline
{\ct CABLE\_MASS\_PER\_LENGTH}          & Real          & Section~\ref{info:THIEF}                  & kg/m                  & 0.5       \\ \hline
...
! static geometry
  ...
  IF(N_FLOATS>0)WRITE(LU_GEOM) WRITE(LU_GEOM) (FLOAT_HEADER(I),I=1,N_FLOATS)
  WRITE(LU_GEOM) N_INTS
  IF(N_INTS>0)WRITE(LU_GEOM) WRITE(LU_GEOM) (INT_HEADER(I),I=1,N_INTS)
