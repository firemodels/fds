...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       ...
       DUMP_GEOM,UPDATE_DEVICES_2,WRITE_DEVC_CTRL_LOG,WRITE_STL_FILE,DUMP_FIRE_ARRIVAL
...
USE VEGE, ONLY: UPDATE_FIRE_ARRIVAL
USE COMP_FUNCTIONS, ONLY : CURRENT_TIME
...
IF (FIRE_ARRIVAL_SLICE) CALL UPDATE_FIRE_ARRIVAL(T,NM)
...
IF (.NOT.VTK3D) THEN
   QUANTITY_LOOP: DO IQ=1,NQT
      ...
      ELSE
         SL => SLICE(IQ)
         IND  = SL%INDEX
         ! TIME OF ARRIVAL slice not updated here
         IF (SL%INDEX==78) CYCLE QUANTITY_LOOP
         IND2 = SL%INDEX2
         Y_INDEX = SL%Y_INDEX
         Z_INDEX = SL%Z_INDEX
...
! \brief Write time of arrival AGL_SLICE to a file
!>
!> \param T Current simulation time (s)
!> \param NM Mesh number

SUBROUTINE DUMP_FIRE_ARRIVAL(T,NM)
...
REAL(EB) :: TOA_SUM
REAL(FB) :: SLICE_MIN,SLICE_MAX,STIME,TOA_COUNT
REAL(FB), ALLOCATABLE, DIMENSION(:,:) :: QQ_TOA
...
ALLOCATE(QQ_TOA(1:II2,1:JJ2),STAT=IZERO); CALL ChkMemErr('DUMP','QQ_TOA',IZERO)
QQ_TOA=REAL(T_END+1.,FB)

! Average (mask out points where no fire arrival detected)
DO J=J1,J2
   DO I=I1,I2
      TOA_SUM=SUM(TOA(I:I+1,J:J+1), MASK=TOA(I:I+1,J:J+1)<=T_END)
      TOA_COUNT=REAL(COUNT(TOA(I:I+1,J:J+1)<=T_END),FB)
      IF (TOA_COUNT>0) QQ_TOA(I-J1+1,J-J1+1) = REAL(TOA_SUM/TOA_COUNT,FB)
   ENDDO
ENDDO
...
WRITE(LU_SLCF(IQ,NM)) STIME
WRITE(LU_SLCF(IQ,NM)) ((QQ_TOA(I,J) ,I=1,II2),J=1,JJ2)
CLOSE(LU_SLCF(IQ,NM))

SLICE_MIN = MINVAL(QQ_TOA)
SLICE_MAX = MAXVAL(QQ_TOA,MASK=(QQ_TOA<=T_END))
...
END SUBROUTINE DUMP_FIRE_ARRIVAL
