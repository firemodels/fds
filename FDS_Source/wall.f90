...
REAL(EB) :: DT_BC,T,TSI,TMP_G,RHO_G,DTMP,TMP_OTHER,RAMP_FACTOR,QNET,FDERIV,TMP_EXTERIOR,UN,ARO,UWO,QEXTRA,RSUM_W, &
            YY_G_ALL(MAX_SPECIES),RHO_YY_F(MAX_SPECIES),YY_GET(1:N_SPECIES),YY_EXTERIOR(MAX_SPECIES)!,HTC2
   ...
   METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
      ...
      CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER 
         ...
         IF (INFLOW) THEN
            ...
            IF (N_SPECIES>0) YY_EXTERIOR(1:N_SPECIES) = SPECIES(1:N_SPECIES)%YY0
            ...
            QCONF(IW) = 0._EB
            IF (N_SPECIES>0) YY_F(IW,1:N_SPECIES)=YY_EXTERIOR(1:N_SPECIES)
         ELSE
            TMP_F(IW) = TMP(IIG,JJG,KKG)
            IF (N_SPECIES>0) YY_F(IW,1:N_SPECIES) = YYP(IIG,JJG,KKG,1:N_SPECIES)
         ENDIF
      ...
      CASE (NET_FLUX_BC) METHOD_OF_HEAT_TRANSFER
         ...
         ADLOOP: DO
            ...
            IF (ABS(QNET) > 0._EB .AND. DTMP == 0._EB) DTMP=1._EB
            HEAT_TRANS_COEF(IW) = HEAT_TRANSFER_COEFFICIENT(IW,IIG,JJG,KKG,IOR,TMP_G,DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH)
            IF (RADIATION) THEN
      ...
      CASE (CONVECTIVE_FLUX_BC) METHOD_OF_HEAT_TRANSFER
         ...
         !IF (QCONF(IW)==0._EB) THEN
         !   TMP_F(IW)=TMP_G
         !ELSE
         !   TMP_OTHER = TMP_F(IW)
         !   CONVLOOP: DO
         !      DTMP = TMP_G - TMP_OTHER
         !      IF (DTMP==0._EB) DTMP=1._EB
         !      HEAT_TRANS_COEF(IW) = HEAT_TRANSFER_COEFFICIENT(IW,IIG,JJG,KKG,IOR,TMP_G,DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH)
         !      HTC2 = HEAT_TRANSFER_COEFFICIENT(IW,IIG,JJG,KKG,IOR,TMP_G,DTMP-1._EB,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH)
         !      FDERIV = -HEAT_TRANS_COEF(IW) + DTMP * (HTC2 - HEAT_TRANS_COEF(IW))
         !      TMP_OTHER = TMP_OTHER - (DTMP*HEAT_TRANS_COEF(IW)-QCONF(IW))/FDERIV
         !      IF (ABS(TMP_OTHER - TMP_F(IW)) / TMP_F(IW) < 0.0001) THEN
         !         TMP_F(IW) = TMP_OTHER
         !         EXIT CONVLOOP
         !      ELSE
...
REAL(EB) :: T,YY_G,RHO_G,UN,DD,MFT,TSI,RADIUS,AREA_SCALING,&
            RVC,RHO_NEW,M_DOT_PPP,CP,MW_RATIO,H_G,DELTA_H_G,TMP_WGT2,TMP_WGT,YY_GET(1:N_SPECIES),CP_BG,CP_BG2,CP_G,CP_G2
INTEGER :: I,IBC,IIG,JJG,KKG,IOR,IW,II,JJ,KK,N,ITMP,ITMP2,NS
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   METHOD_OF_MASS_TRANSFER: SELECT CASE(SF%SPECIES_BC_INDEX)
      ...
      CASE (SPECIFIED_MASS_FRACTION) METHOD_OF_MASS_TRANSFER
         ...
         IF (UWS(IW)<0._EB) THEN
            ...
            ENDDO
         ELSE
            YY_F(IW,1:N_SPECIES) = YYP(IIG,JJG,KKG,1:N_SPECIES)
         ENDIF
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. N_SPECIES > 0 .AND. &
...
IF (VIRTUAL_PARTICLES .AND. CORRECTOR) THEN
   DROPLET_LOOP: DO I=1,NLP
      ...
      ITMP2 = MIN(4999,INT(TMP_F(IW)))
      TMP_WGT2 = TMP(II,JJ,KK) - REAL(ITMP2,EB)
      CP_BG = Y2CPBAR_C(ITMP)+TMP_WGT*(Y2CPBAR_C(ITMP+1)-Y2CPBAR_C(ITMP))
      CP_BG2 = Y2CPBAR_C(ITMP2)+TMP_WGT2*(Y2CPBAR_C(ITMP2+1)-Y2CPBAR_C(ITMP2))   
      DO N=1,N_SPECIES
         MW_RATIO = SPECIES(N)%RCON/RSUM(II,JJ,KK)
         M_DOT_PPP = MASSFLUX(IW,N)*AW(IW)*RVC
         CP_G = Y2CPBAR(ITMP,NS)+TMP_WGT*(Y2CPBAR(ITMP+1,NS)-Y2CPBAR(ITMP,NS))
         CP_G2 = Y2CPBAR(ITMP2,NS)+TMP_WGT2*(Y2CPBAR(ITMP2+1,NS)-Y2CPBAR(ITMP2,NS))      
         DELTA_H_G = (CP_G2+CP_BG2)*TMP_F(IW)-(CP_G+CP_BG)*TMP(II,JJ,KK)
...
REAL(EB) :: SMALLEST_CELL_SIZE(MAX_LAYERS),THICKNESS,YY_GET(1:N_SPECIES),ZZ(1:I_Z_MAX - I_Z_MIN + 1)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
   ...
   SELECT CASE(SF%BACKING)
      CASE(VOID)  ! Non-insulated backing to an ambient void
         ...
         HTCB = HEAT_TRANSFER_COEFFICIENT(IW,-1,-1,-1,IOR,SF%TMP_BACK,DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH)
...
REAL(EB) :: U2=0._EB,V2=0._EB,W2=0._EB,VELCON,H_NATURAL,H_FORCED,CP,YY_GET(1:N_SPECIES),NUSSELT,MU_G
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU=>NULL(),VV=>NULL(),WW=>NULL(),RHOP=>NULL()
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: YYP=>NULL()
...
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_SOOT,RHOG,YIN(1:N_SPECIES),YDEP,K_AIR,TMP_FILM
