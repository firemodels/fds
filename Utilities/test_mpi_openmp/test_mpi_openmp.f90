...
write(*,*) "OPENMP_AVAILABLE_THREADS = ", OPENMP_AVAILABLE_THREADS
     ...
     &         "insufficient threading support."
      ...
      if (rank .eq. 0) then
         ...
         !$OMP CRITICAL
         write(*,91) "Hello from thread ",threadID," of ",nthreads,     &
     &      " in rank ",rank," of ",nproc," on ",trim(name)
         !$OMP END CRITICAL
         ...
         do r=1,nproc-1
            ...
            call MPI_RECV(sNthreads, 1, MPI_INTEGER, r, 10*r,           &
     &         MPI_COMM_WORLD, stat, ierr)
            do t=0,sNthreads-1
               ! For each thread, get the rank ID, thread ID, and name
               call MPI_RECV(dRank, 1, MPI_INTEGER, r, 10*r+1,          &
     &            MPI_COMM_WORLD, stat, ierr)
               call MPI_RECV(dThread, 1, MPI_INTEGER, r, 10*r+2,        &
     &            MPI_COMM_WORLD, stat, ierr)
               call MPI_RECV(dNamelen, 1, MPI_INTEGER, r,               &
     &            1000*r+10*dThread, MPI_COMM_WORLD, stat, ierr)
               call MPI_RECV(dName, dNamelen, MPI_CHARACTER, r,         &
     ...
     &            dName(1:dNamelen)
         ...
         !$OMP MASTER
         call MPI_SEND(nthreads, 1, MPI_INTEGER, 0, 10*rank,            &
     &      MPI_COMM_WORLD, ierr);
         !$OMP END MASTER
         ...
         call MPI_SEND(rank, 1, MPI_INTEGER, 0, 10*rank+1,              &
     &      MPI_COMM_WORLD, ierr)
         call MPI_SEND(threadID, 1, MPI_INTEGER, 0, 10*rank+2,          &
     &      MPI_COMM_WORLD, ierr)
         call MPI_SEND(namelen, 1, MPI_INTEGER, 0,                      &
     &      1000*rank+10*threadID, MPI_COMM_WORLD, ierr)
         call MPI_SEND(name, namelen, MPI_CHARACTER, 0,                 &
     &      1000*rank+10*threadID+1, MPI_COMM_WORLD, ierr)
