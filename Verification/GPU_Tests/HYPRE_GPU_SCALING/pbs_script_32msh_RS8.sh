...
mpiexec -n ${NTOTRANKS} --ppn ${NRANKS_PER_NODE} --depth=${NDEPTH} --cpu-bind depth --env OMP_NUM_THREADS=${NTHREADS} -env OMP_PLACES=threads ./set_affinity_gpu_polaris.sh $FIREMODELS/fds/Build/ompi_gnu_linux/fds_ompi_gnu_linux test_32mesh_RS8.fds 
