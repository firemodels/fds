...
\Huge
\flushright{Fire Dynamics Simulator (Version 5) \\ User's Guide \\ (Draft, May 27, 2007)}
...
Properties such as a sprinkler spray pattern vary multi-dimensionally. In such cases, use
the {\ct TABL} function.  In the example below, the spray pattern is defined as two 10 m/s and 20 lpm jets
centered on 45 degrees from the vertical with a 180 degree seperation.
...
&TABL ID='TABLE1',TABLE_DATA=40,50,185,10,0.5/
\end{verbatim}
\normalsize
...
lines.  For a spray pattern, specify the total flow using {\ct FLOW\_RATE}, the name of the spray pattern using
{\ct SPRAY\_PARTTERN\_TABLE} and then or more {\ct TABL} lines of the format:

\footnotesize
\begin{verbatim}
&TABL ID='table_id', TABLE_DATA=L1,L2,L3,L4,V,F/
\end{verbatim} \normalsize
...
for a specified solid angle. {\ct L1} and {\ct L2} are longitude range in degree for the solid angle measured in degrees from 
the south pole (0 is the south pole and 90 is the equator, 180 is the north pole).  {\ct L3} and {\ct L4} are the
lattitude range in degrees 
where 0 or 360 is the {\ct -x} axis and 90 is the {\ct -y} axis.  {\ct V} and {\ct F} are respectively the velocity (m/s)
and the relative flow rate for that solid angle.  The {\ct TABL} lines can be specified in any order.

In the example below, the spray pattern is defined as two 10 m/s and 20 kg/s jets
centered on 45 degrees from the vertical with a 180 degree seperation.
...
&TABL ID='TABLE1',TABLE_DATA=40,50,85,95,10,20/
&TABL ID='TABLE1',TABLE_DATA=40,50,185,10,20/
\end{verbatim} \normalsize
...
{\ct MISC} line). \index{{\ct POROUS\_FLOOR}!output issues}
