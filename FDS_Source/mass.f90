...
!$    SHARED(KBAR,JBAR,IBAR,KBM1,JBM1,IBM1,RHOP,FX,FY,FZ,UU,VV,WW,FLUX_LIMITER,R, &
!$           N_EXTERNAL_WALL_CELLS,N_INTERNAL_WALL_CELLS,BOUNDARY_TYPE,IJKW,WALL_INDEX,CELL_INDEX, &
!$           RHO_F,UVW_SAVE, &
!$           FRHO,SOLID,RDX,RDY,RDZ,RRN, &
!$           N_TRACKED_SPECIES,ZZP,ZZ_F)
...
SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
      ...
      !$OMP PARALLEL DEFAULT(NONE) &
      !$    SHARED(N,KBAR,JBAR,IBAR,KBM1,JBM1,IBM1,RHOP,ZZP,FX,FY,FZ,UU,VV,WW,FLUX_LIMITER,R, &
      !$           N_EXTERNAL_WALL_CELLS,N_INTERNAL_WALL_CELLS,BOUNDARY_TYPE,IJKW, &
      !$           WALL_INDEX,CELL_INDEX,RHO_F,ZZ_F,UVW_SAVE, &
      !$           SURFACE,UWS,RHODW,RDN,MASSFLUX, &
      !$           SOLID,DEL_RHO_D_DEL_Z,RDX,RDY,RDZ,RRN)
     ...
     !$    PRIVATE(IW,II,JJ,KK,IOR,IBC,IIG,JJG,KKG,ZZZ,UN,RHO_D_DZDN)
...
CASE(.TRUE.) PREDICTOR_STEP
   ...
   !$OMP PARALLEL DEFAULT(NONE) &
   !$    SHARED(CHANGE_TIME_STEP,NM,N_TRACKED_SPECIES,KBAR,JBAR,IBAR,SOLID,CELL_INDEX,ZZS,RHO,ZZ,DT, &
   !$           DEL_RHO_D_DEL_Z,DTRATIO,DT_PREV,OMDTRATIO,RHOS, &
   !$           FRHO,CLIP_MASS_FRACTION,N_ZONE,PBAR_S,PBAR,D_PBAR_DT,KBP1,JBP1,IBP1,RSUM,TMP,PRESSURE_ZONE, &
   !$           TMPMIN,TMPMAX)
...
CASE(.FALSE.) PREDICTOR_STEP
   ...
   !$OMP PARALLEL DEFAULT(NONE) &
   !$    SHARED(N_TRACKED_SPECIES,KBAR,JBAR,IBAR,SOLID,CELL_INDEX,ZZ,RHO,RHOS,ZZS,DT,DEL_RHO_D_DEL_Z, &
   !$           FRHO,CLIP_MASS_FRACTION,N_ZONE,PBAR,PBAR_S,D_PBAR_S_DT,KBP1,JBP1,IBP1,RSUM,TMP,PRESSURE_ZONE, &
   !$           TMPMIN,TMPMAX)
...
!$OMP PARALLEL DEFAULT(NONE) &
!$    SHARED(TWO_D,KBAR,IBAR,JBAR,V_CELL,DX,DY,DZ, &
!$           RHODELTA,CELL_INDEX,SOLID,RHOMIN,RHOP,WALL_INDEX,RHOMAX)
...
!$    PRIVATE(K,J,I,IC,RMIN,SUM,ISUM,LC,RHO00,RHOMI,RHOPI,RHOMJ,RHOPJ,RHOMK,RHOPK,CONST,CONST2)
DO K=1,KBAR
   DO J=1,JBAR
...
!$OMP DO COLLAPSE(3) SCHEDULE(DYNAMIC) &
!$    PRIVATE(K,J,I,IC,RMAX,SUM,ISUM,LC,RHO00,RHOMI,RHOPI,RHOMJ,RHOPJ,RHOMK,RHOPK,CONST,CONST2)
DO K=1,KBAR
   DO J=1,JBAR
...
SPECIESLOOP: DO N=1,N_TRACKED_SPECIES
   ...
   !$OMP PARALLEL DEFAULT(NONE) &
   !$    SHARED(ZZDELTA,KBAR,JBAR,IBAR,CELL_INDEX,SOLID,WALL_INDEX,ZZP,ZZ_F,N,BOUNDARY_TYPE, &
   !$           DEL_RHO_D_DEL_Z,RHOP)
   ...
   !$OMP DO COLLAPSE(3) SCHEDULE(DYNAMIC) &
   !$    PRIVATE(K,J,I,IC,IW_A,Y00,SUM,ISUM,LC,ZMIN,ZMI,ZPI,ZMJ,ZPJ,ZMK,ZPK, &
   !$            RHY0,RHZPI,RHZMI,RHZPJ,RHZMJ,RHZPK,RHZMK,CONST)
   DO K=1,KBAR
      DO J=1,JBAR
         CHECK_LOOP: DO I=1,IBAR
            IC = CELL_INDEX(I,J,K)
   ...
   ! Do overshoots
   !$OMP DO COLLAPSE(3) SCHEDULE(DYNAMIC) &
   !$    PRIVATE(K,J,I,IC,IW_A,Y00,SUM,ISUM,LC,ZMIN,ZMI,ZPI,ZMK,ZPK,ZMJ,ZPJ,ZMAX, &
   !$    RHY0,RHZPI,RHZMI,RHZPJ,RHZMJ,RHZPK,RHZMK,CONST)
   DO K=1,KBAR
      DO J=1,JBAR
         CHECK_LOOP2: DO I=1,IBAR
