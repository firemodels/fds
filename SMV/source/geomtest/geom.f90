...
READ_GEOM_LOOP: DO N=1,N_GEOM
   ...
   IF (N_FACES.GT.0) THEN
      ...
      G%SURF_INDICES(1:N_FACES) = SURF_INDEX
   ENDIF
...
SUBROUTINE MERGE_GEOMS(VERTS,N_VERTS,FACES,SURF_IDS,N_FACES)
   ...
   DO I = 0, N_GEOM-1
      ...
      IF(G%N_FACES>0)THEN
         ...
         SURF_IDS(ISURF:ISURF + G%N_FACES - 1) = G%SURF_INDICES(1:G%N_FACES)
...
SUBROUTINE EXPAND_GROUPS
   ...
   DO I = 2, N_GEOM ! first geometry will not have any sub-geometries
      ...
      DO J = 1, G%N_GEOMS
        ...
        XIN(1:G2%N_VERTS) => G2%VERTS(IVERT:IVERT+G2%N_VERTS-1)
        XOUT(1:G2%N_VERTS) => G2%VERTS(IVERT:IVERT+G2%N_VERTS-1)
