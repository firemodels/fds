...
REAL(EB) :: ZZ_0(0:N_TRACKED_SPECIES),DZZDT(0:N_TRACKED_SPECIES),DZZDT2(0:N_TRACKED_SPECIES),RATE_CONSTANT(1:N_REACTIONS),&
            ERR_EST,TOL_INT_VECTOR(1:N_REACTIONS),ERR_TOL,Q_NR_1(1:N_REACTIONS),Q_NR2_1(1:N_REACTIONS),Q_NR_2(1:N_REACTIONS),&
...
REAL(EB), PARAMETER :: DT_MIN=1.E-6_EB
INTEGER :: I_TS,NR,NS,NSS,ITER,TVI
INTEGER, PARAMETER :: NODETS=20,SUB_DT1=1,SUB_DT2=2,SUB_DT4=4,TV_ITER_MIN=5,Q_ITER_MAX=10
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
   ...
   RICH_EX_LOOP: DO WHILE (ERR_EST > ERR_TOL)
      ...
      ODE_LOOP1: DO NS = 1, SUB_DT1
         ...
         IF (ANY(A1 < 0._EB)) THEN
            DO NSS=0,N_TRACKED_SPECIES
               IF (A1(NSS) < 0._EB .AND. ABS(DZZDT(NSS))>ZERO_P) THEN
         ...
         REACTION_LOOP2: DO NR = 1, N_REACTIONS   
            RN => REACTION(NR)      
            CALL COMPUTE_RATE_CONSTANT(NR,RN%MODE,I_TS,Q_OUT2,RATE_CONSTANT(NR),A1,I,J,K)
         ...
         IF (ANY(A1 < 0._EB)) THEN
            DO NSS=0,N_TRACKED_SPECIES
               IF (A1(NSS) < 0._EB .AND. ABS(DZZDT(NSS)+DZZDT2(NSS))>ZERO_P) THEN
                  DT_SUB = MIN(DT_SUB,-2._EB*ZZ_0(NSS)/(DZZDT(NSS)+DZZDT2(NSS)))
               ENDIF
            ENDDO
            A1 = ZZ_0 + 0.5_EB*(DZZDT+DZZDT2)*DT_SUB
         ENDIF 
      ...
      ODE_LOOP2: DO NS = 1, SUB_DT2
         ...
         IF (ANY(A2 < 0._EB)) THEN
            DO NSS=0,N_TRACKED_SPECIES
               IF (A2(NSS) < 0._EB .AND. ABS(DZZDT(NSS))>ZERO_P) THEN
                  DT_SUB = MIN(DT_SUB,-ZZ_0(NSS)/DZZDT(NSS))
               ENDIF
            ENDDO
            A2 = ZZ_0 + DZZDT*(DT_SUB/REAL(SUB_DT2,EB))
         ...
         REACTION_LOOP2_2: DO NR = 1, N_REACTIONS   
            RN => REACTION(NR)      
            CALL COMPUTE_RATE_CONSTANT(NR,RN%MODE,I_TS,Q_OUT2,RATE_CONSTANT(NR),A2,I,J,K)
      ...
      ODE_LOOP4: DO NS = 1, SUB_DT4
         ...
         IF (ANY(A4 < 0._EB)) THEN
            DO NSS=0,N_TRACKED_SPECIES
               IF (A4(NSS) < 0._EB .AND. ABS(DZZDT(NSS))>ZERO_P) THEN
                  DT_SUB = MIN(DT_SUB,-ZZ_0(NSS)/DZZDT(NSS))
               ENDIF
            ENDDO
            A4 = ZZ_0 + DZZDT*(DT_SUB/REAL(SUB_DT4,EB))
         ...
         REACTION_LOOP2_4: DO NR = 1, N_REACTIONS   
            RN => REACTION(NR)      
            CALL COMPUTE_RATE_CONSTANT(NR,RN%MODE,I_TS,Q_OUT2,RATE_CONSTANT(NR),A4,I,J,K)
         ...
         IF (ABS(Q4+Q_SUM_4*(DT_SUB/REAL(SUB_DT4,EB))) > Q_UPPER*DT) THEN
            ...
            EXIT ODE_LOOP4
         ENDIF  
         Q4 = Q4+Q_SUM_4*(DT_SUB/REAL(SUB_DT4,EB))
         I_TS = I_TS + 1
         ZZ_0 = A4
      ENDDO ODE_LOOP4
      DT_A4 = DT_SUB
      ...
      IF (ERR_EST <= 0.0_EB) THEN
         DT_SUB_NEW = DT
      ELSE
         DT_SUB_NEW = MAX(DT_MIN,DT_SUB*(ERR_TOL/(ERR_EST))**(0.25_EB)) ! Determine New Time Step
      ENDIF
