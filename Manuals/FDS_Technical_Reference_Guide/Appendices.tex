...
\label{eqn_DTDt2}
\end{eqnarray}
The following combination of terms may be rearranged in terms of the latent heat $\Delta h_{v,\alpha}$ and is therefore responsible for evaporative cooling:
\begin{eqnarray}
...
&=&  \dot{m}_{b}^{\tripleprime} \left[ h_{s,b} - \sum_\alpha h_{s,\alpha} Y_{b,\alpha} \right] \nonumber\\  [.3cm]
&=&  \dot{m}_{b}^{\tripleprime} \left[ \sum_\alpha Y_{b,\alpha} (h_{s,b,\alpha}-h_{s,\alpha}) \right] \nonumber\\  [.3cm]
&=&  \dot{m}_{b}^{\tripleprime} \left[ \sum_\alpha Y_{b,\alpha} \left(\int_{T_0}^{T_b} c_{l,\alpha}(T^\prime) \,\mbox{d}T^\prime \right. \right. \nonumber\\
& & \left. \left. \hspace{1.4cm} - \left\{\int_{T_0}^{T_b} c_{l,\alpha}(T^\prime) \,\mbox{d}T^\prime + \Delta h_{v,\alpha} + \int_{T_b}^{T} c_{p,\alpha}(T^\prime) \,\mbox{d}T^\prime \right\} \right) \right] \nonumber\\  [.3cm]
&=&  -\dot{m}_{b}^{\tripleprime} \left[ \sum_\alpha Y_{b,\alpha} \left( \Delta h_{v,\alpha} + \int_{T_b}^{T} c_{p,\alpha}(T^\prime) \,\mbox{d}T^\prime \right) \right] \,\mbox{.}
\end{eqnarray}
\vspace{0.3cm}\newpage

\noindent Note the minus sign on the RHS in the last step of (\ref{eqn_latent_heat}).  This group of terms says that the heat needed to vaporize $\alpha$ and raise its temperature from $T_b$ to $T$ must be subtracted from the local gas mixture.
...
& & \left. - \dot{q}_b^\tripleprime -\dot{m}_{b}^{\tripleprime} \left\{\sum_\alpha Y_{b,\alpha} \left( \Delta h_{v,\alpha} + \int_{T_b}^{T} c_{p,\alpha}(T^\prime) \,\mbox{d}T^\prime \right) + \mbox{$\frac{1}{2}$}\left|\mathbf{u}_b-\mathbf{u}\right|^2 \right\} \right] \,\mbox{.}
\end{eqnarray}
...
& & \left. \left. \hspace{0.5cm} - \dot{q}_b^\tripleprime -\dot{m}_{b}^{\tripleprime} \left\{\sum_\alpha Y_{b,\alpha} \left( \Delta h_{v,\alpha} + \int_{T_b}^{T} c_{p,\alpha}(T^\prime) \,\mbox{d}T^\prime \right) + \mbox{$\frac{1}{2}$}\left|\mathbf{u}_b-\mathbf{u}\right|^2 \right\} \right) \right] \,\mbox{.} 
\end{eqnarray}
\vspace{0.3cm}
...
&+&  \frac{1}{\rho c_p T} \left[ - \sum_\alpha \Delta h^\circ_\alpha \dot{m}_\alpha^{\tripleprime} - \Div \dot{\mathbf{q}}^{\prime\prime} - \dot{q}_b^\tripleprime -\dot{m}_{b}^{\tripleprime} \left\{\sum_\alpha Y_{b,\alpha} \left( \Delta h_{v,\alpha} + \int_{T_b}^{T} c_{p,\alpha}(T^\prime) \,\mbox{d}T^\prime \right) + \mbox{$\frac{1}{2}$}\left|\mathbf{u}_b-\mathbf{u}\right|^2 \right\} \right] \,\mbox{.} \nonumber\\ [0.3cm]
\end{eqnarray}
...
&+& \underbrace{\frac{\dot{m}_b^\tripleprime}{\rho}\frac{\overline{W}}{\overline{W}_b} + \frac{1}{\rho c_p T} \left[ - \dot{q}_b^\tripleprime -\dot{m}_{b}^{\tripleprime} \left\{\sum_\alpha Y_{b,\alpha} \left( \Delta h_{v,\alpha} + h_{s,\alpha}(T)-h_{s,\alpha}(T_b)\right) + \mbox{$\frac{1}{2}$}\left|\mathbf{u}_b-\mathbf{u}\right|^2 \right\}  \right]}_{\mbox{\ct D\_LAGRANGIAN}} \nonumber\\ [.3cm]
