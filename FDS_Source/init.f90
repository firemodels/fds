...
INTEGER :: N,I,J,K,II,JJ,KK,IPTS,JPTS,KPTS,N_EDGES_DIM,IW,IC,IBC,IOR,IOPZ,IERR,IB,JB,KB
...
ALLOCATE(M%PRESSURE_BC_INDEX(NEWC),STAT=IZERO)
...
M%IJKE  = 0
ALLOCATE(M%OME_E(0:N_EDGES_DIM,2),STAT=IZERO)
CALL ChkMemErr('INIT','OME_E',IZERO)  
M%OME_E = 0._EB
ALLOCATE(M%TAU_E(0:N_EDGES_DIM,2),STAT=IZERO)
...
IF (I_OBST>0) THEN
   ...
   IF (OBX%BULK_DENSITY>0._EB) OBX%MASS = OBX%BULK_DENSITY*(OBX%X2-OBX%X1)*(OBX%Y2-OBX%Y1)*(OBX%Z2-OBX%Z1)
ENDIF
...
PROCESS_VENT: IF (I_VENT>0) THEN
   ...
   IF (SURFACE(IBCX)%VOLUME_FLUX    /=-999._EB)   M%UW0(IW) = SURFACE(IBCX)%VOLUME_FLUX/VT%FDS_AREA
