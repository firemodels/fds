...
REAL(EB), DIMENSION(MAX_LAYERS) :: LAYER_THICKNESS_OBST,LAYER_THICKNESS,HEAT_SOURCE,HEAT_SOURCE_OBST,&
...
REAL(EB) :: XXC,YYC,ZZC,THICKNESS,OLD_THICKNESS,TOTAL_THICKNESS,UPPER_BOUND,LOWER_BOUND,&
...
FIND_BACK_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
   ...
   IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN 
      ...
      IF (OBST_INDEX>0) CALL ADD_MATERIAL(N_MATL_OBST,OB%MATL_INDEX,MATL_INDEX_OBST)
...
IF (SF%VARIABLE_THICKNESS .OR. SF%HT_DIM>1) THEN
   ...
   DO NL=1,N_LAYERS_OBST
      IF (NL>1) THEN ; LOWER_BOUND=SUM(LAYER_THICKNESS_OBST(1:NL-1)) ; ELSE ; LOWER_BOUND=0._EB ; ENDIF
      UPPER_BOUND = SUM(LAYER_THICKNESS_OBST(1:NL))
      IF (UPPER_BOUND<FRONT_LINING_THICKNESS .OR. LOWER_BOUND>=TOTAL_THICKNESS-BACK_LINING_THICKNESS) CYCLE  ! Lining covers layer
      N_LAYERS = N_LAYERS + 1
      LAYER_THICKNESS(N_LAYERS)  = MIN(TOTAL_THICKNESS-BACK_LINING_THICKNESS,UPPER_BOUND) - &
                                   MAX(SUM(LAYER_THICKNESS(1:N_LAYERS-1)),LOWER_BOUND)
   ...
   IF (TOTAL_THICKNESS<FRONT_LINING_THICKNESS+BACK_LINING_THICKNESS) THEN
