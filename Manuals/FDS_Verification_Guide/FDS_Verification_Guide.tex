...
The mixing-controlled (mixed is burned) reaction model with local extinction is the default reaction mechanism in FDS. The reaction rate is determined by the eddy dissipation concept (EDC). The EDC assumes that the consumption rate of fuel for each reaction is determined from the local fuel, oxidizer, and product mass fractions and the turbulent mixing time by the following equation \cite{Poinsot:TNC}:

\begin{equation}\label{eq:EDC}
k_{i} = \frac{C}{\tau_{mix}} \, \min \left( Y_F,\frac{Y_\OTWO}{s},\beta\frac{Y_P}{(1+s)} \right)
\end{equation}
...
\end{tabular*}
\caption[Species Evolution in Mixing Controlled Reaction]{Time evolution of species mass fraction for a one-step mixing controlled reaction with a fixed mixing time of $\tau_{mix}=0.1s$. (Left) Fuel-limited reaction. (Right) Oxygen-limited reaction.}
...
{k}_{i}=-A\prod(Y_{\alpha}^{a_{\alpha}})T^{n_{\alpha}}\exp(-E/RT)
\end{equation}
where A, E, $a_{\alpha}$, and $n_{\alpha}$ are user defined inputs based on the specific reaction. Note that since Equation \ref{eq:FR_Arr} has an exponential term, the integration of Equation Set \ref{eq:spec_ode} is solved using a two-step Runge-Kutta scheme as compared to an explicit Euler scheme used in the mixing-controlled test cases.
...
\end{equation}
In this case, $E$, $a_{\alpha}$, and $n_{\alpha}$ are set to zero, and $A$ is set such that ${k}_{i} = 1$. The resulting reaction rates simplify to a function of molecular weight and stoichiometric coefficients.
...
\end{equation}
$E$ and $n_{\alpha}$ are still set to zero, but $a_{\alpha}=[1,1,0,0]$  for propane, oxygen, carbon monoxide, and water vapor respectively, making the reaction second-order ($\mathcal{O}=\sum a_{\alpha}$).
...
Multi-step Arrhenius finite rate reactions are also tested. A two-step forward propane reaction:
\begin{eqnarray}\label{eq:2step_propane}
\mathrm{C_3H_8+3.5 \, O_2 \rightarrow  3 \, CO + 4 \, H_2O} \\
\nonumber \mathrm{CO+0.5 \, O_2 \rightarrow CO_2}
\end{eqnarray}
and a two-step reversible propane reaction:
\begin{eqnarray}\label{eq:2step_propane_rev}
\mathrm{C_3H_8+3.5 \, O_2 \rightarrow  3 \, CO+4 \, H_2O} \\
\nonumber \mathrm{CO + 0.5 \, O_2 \leftrightarrow CO_2}
\end{eqnarray}
 are considered. Equation \ref{eq:2step_propane_rev} includes the decomposition of $CO_2$ into $CO$ and $O_2$ which the forward only two-step mechanism does not. This reaction mechanism is represented by three separate reactions on the {\ct REAC} line in an FDS input file. The reaction order for the first two steps in both cases is 1.75. In the first reaction $a_{\alpha}=[0.1,1.65,0,0]$ for propane, oxygen, carbon monoxide, and water vapor respectively. In the forward carbon monoxide reaction $a_{\alpha}=[1,0.25,0,0.5]$ for carbon monoxide, oxygen, carbon dioxide, and water vapor respectively. Note that in the  forward second step, water vapor contributes to the reaction rate based on its local concentration despite not being a participant in the reaction. For the reverse reaction in Equation \ref{eq:2step_propane_rev}, $a_{\alpha}=[1,0,0]$ for carbon dioxide, carbon monoxide, and oxygen respectively.
...
\end{tabular*}
\caption[Species Evolution in Mixing Controlled Reaction]{Time evolution of species mass fraction for multi-step Arrhenius finite rate propane reactions. (Left) Two-step reaction. (Right) Two-step reaction with reversible CO reaction.}
