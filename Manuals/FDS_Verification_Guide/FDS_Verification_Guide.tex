...
An  extensive amount  of  verification and  validation  work with  FDS version 4  has been  performed by Hietaniemi {\em et al.}~\cite{Hietaniemi:1} at VTT,
...
Recently, Lautenberger {\em et al.}~\cite{Lautenberger:FSJ} developed a method to automate the
...
\begin{figure}[t]
   ...
   The case is run with constant properties, $\rho=1$ kg/m$^3$ and $\mu = 0.1$ kg/m/s, and a Courant-Friedrichs-Lewy (CFL) of 0.25.}
...
\end{equation}
In Figure \ref{fig_vort2d_pointvelocity}, we show the $u$-velocity at a single point, on the lower left fringe of the vortex, for two different mesh resolutions, $80^{2}$ and $160^{2}$. A mesh resolution of $320^{2}$ grid cells produces a plot (not shown) that is, to the eye, a perfect match out to four pass-through times.
...
density, pressure and temperature ought to rise to the same values regardless of the time of injection.  The results are plotted in Fig.~\ref{isentropic2}.
...
\be \rho = \frac{ \overline{W} \, p }{ \R T} \ee
...
\be \frac{\d p}{\d t} = \frac{\gamma \, \dot{m}'' \, \R T \, A}{\overline{W} \, V} \ee
...
In this example, two fictitious gases are forced through tiny ducts (1~mm by 1~mm) that meet at a T-joint. The cross sectional areas of all the ducts are the same. Gas~1 has an initial temperature of 606.3~$^\circ$C and a constant specific heat of 1~\si{kJ/(kg.K)}. Gas~2 has an initial temperature of 20~$^\circ$C and a constant specific heat of 10~\si{kJ/(kg.K)}. Both gases have a molecular weight of 29~g/mol and a mass flow of $1 \times 10^{-8}$~kg/s. Typically gases of the same molecular weight have comparable, but not necessarily the same, specific heats. This is an extreme case. In steady state, it is expected that the convection and diffusion of enthalpy at the boundaries will be equal:
...
This case tests the ability of FDS to properly account for temperature dependent specific heats.  A coarse 2D channel mixing case is presented where hot gas at 900 \si{\degree C} with a constant specific heat of $c_p=1$ \si{kJ/(kg.K)} enters from the bottom left vent with a flux of 2 \si{kg/(m^2.s)} and cold gas at 20 \si{\degree C} with a constant specific heat of $c_p=10$ \si{kJ/(kg.K)} enters from a top left vent with the same mass flux.  The gases mix in a center channel and exit the domain flowing to the right.

The enthalpy of the mixture at the outlet is $2 \times 1 \times (900+273.15) + 2 \times 10 \times (20+273.15) = 8209.3$ \si{kJ/(m^2.s)}.  The mixture heat capacity at the outlet is 5.5 \si{kJ/(kg.K)} and the flux is 4 \si{kg/(m^2.s)}.  Therefore the outlet temperature is $8209.3/4/5.5 - 273.15 = 100$ \si{\degree C}.
\begin{figure}[ht]
\centering
\includegraphics[width=3.2in]{SCRIPT_FIGURES/energy_budget_tmix}
\caption[Outlet temperature for {\bf energy\_budget\_tmix} case]{\label{fig_outlet_temperature} Outlet temperature for {\bf energy\_budget\_tmix} case.  Black circles represent the correct result of 100 \si{\degree C}.}
\end{figure}
