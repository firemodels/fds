...
\Huge \flushright{Fire Dynamics Simulator (Version 5) \\ User's Guide \\ (Draft, July 16, 2007)}
...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~1019-5, \pageref{LastPage} pages (July 2007) \\
CODEN: NSPUE2 }
...
can be prescribed via {\ct U0}, {\ct V0}, and {\ct W0} on the {\ct
MISC} line. 
...
ambient temperature. Use {\ct TMP\_INNER} at the {\ct SURF} line to 
specify different initial temperature.

Also, the backside temperature of the surfaces with heat transfer to
void can be set using the {\ct TMP\_BACK} keyword. It has no meaning
for surfaces with insulated or exposed backings.
...
&DEVC XYZ=4,1,3, PROP_ID='Acme Smoker', ID='SD_3' /
&DEVC XYZ=4,4,3, PROP_ID='Acme Smoker', ID='SD_4' /
&DEVC XYZ=2,2,3, PROP_ID='Acme Nozzle', QUANTITY='CONTROL', ID='NOZZLE 1', CTRL_ID='nozzle trigger' /
