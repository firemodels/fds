...
INTEGER, ALLOCATABLE, DIMENSION(:) :: REQ,REQ1,REQ2,REQ3,REQ4,REQ5,REQ7,REQ6,REQ8,COUNTS,DISPLS,&
                                      ...
                                      COUNTS_QM_DOT,DISPLS_QM_DOT,COUNTS_TEN,DISPLS_TEN
...
REAL(EB), DIMENSION(NMESHES) :: CFL_VALUES
INTEGER :: NM_MAX

DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   REAL_BUFFER_2(1,NM) = MESHES(NM)%CFL
   REAL_BUFFER_2(2,NM) = MESHES(NM)%ICFL
   REAL_BUFFER_2(3,NM) = MESHES(NM)%JCFL
   REAL_BUFFER_2(4,NM) = MESHES(NM)%KCFL
   REAL_BUFFER_2(5,NM) = MESHES(NM)%US(MESHES(NM)%ICFL-1,MESHES(NM)%JCFL  ,MESHES(NM)%KCFL  )
   REAL_BUFFER_2(6,NM) = MESHES(NM)%US(MESHES(NM)%ICFL  ,MESHES(NM)%JCFL  ,MESHES(NM)%KCFL  )
   REAL_BUFFER_2(7,NM) = MESHES(NM)%VS(MESHES(NM)%ICFL  ,MESHES(NM)%JCFL-1,MESHES(NM)%KCFL  )
   REAL_BUFFER_2(8,NM) = MESHES(NM)%VS(MESHES(NM)%ICFL  ,MESHES(NM)%JCFL  ,MESHES(NM)%KCFL  )
   REAL_BUFFER_2(9,NM) = MESHES(NM)%WS(MESHES(NM)%ICFL  ,MESHES(NM)%JCFL  ,MESHES(NM)%KCFL-1)
   REAL_BUFFER_2(10,NM)= MESHES(NM)%WS(MESHES(NM)%ICFL  ,MESHES(NM)%JCFL  ,MESHES(NM)%KCFL  )
ENDDO

CALL MPI_GATHERV(REAL_BUFFER_2(1,DISPLS(MYID)+1),COUNTS_TEN(MYID),MPI_DOUBLE_PRECISION, &
                 REAL_BUFFER_3,COUNTS_TEN,DISPLS_TEN,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)

IF (MYID==0 .AND. .NOT.ALL(EVACUATION_ONLY)) THEN
   CFL_VALUES(:) = REAL_BUFFER_3(1,:)
   NM_MAX  = MAXLOC(CFL_VALUES,DIM=1)
   WRITE(LU_CFL,'(I7,A,ES12.4,A,ES12.4,A,I4,A,I3,A,I3,A,I3,A,F6.3,6(A,F7.3))') ICYC,',',T,',',DT,',',NM_MAX,',',&
         NINT(REAL_BUFFER_3(2,NM_MAX)),',',NINT(REAL_BUFFER_3(3,NM_MAX)),',',NINT(REAL_BUFFER_3(4,NM_MAX)),',',MAXVAL(CFL_VALUES),&
         ',',REAL_BUFFER_3(5,NM_MAX),',',REAL_BUFFER_3(6,NM_MAX),',',REAL_BUFFER_3(7,NM_MAX),',',REAL_BUFFER_3(8,NM_MAX),&
         ',',REAL_BUFFER_3(9,NM_MAX),',',REAL_BUFFER_3(10,NM_MAX)
ENDIF
