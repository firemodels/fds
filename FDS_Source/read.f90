...
IF (SM_H2O>0)  THEN
   SM%MASS_FRACTION(SM_O2) = SM%MASS_FRACTION(SM_O2) * (1._EB-SM%MASS_FRACTION(SM_H2O))
    SM%MASS_FRACTION(SM_N2)  = 1._EB - SM%MASS_FRACTION(SM_O2)
   IF (SM_CO2>0) THEN
  ...
  SUBROUTINE DEFINE_EVACUATION_HOLES(IMODE)
    ...
    IMODE_3_IF: IF (IMODE==3) THEN
       ...
       NSTRS_LOOP_3: DO I = 1, N_STRS
          ...
          XB(1) = XB(1) - 0.001_EB*MESHES(EMESH_STAIRS(I)%IMESH)%DX(1)
          XB(2) = XB(2) + 0.001_EB*MESHES(EMESH_STAIRS(I)%IMESH)%DX(1)
          XB(3) = XB(3) - 0.001_EB*MESHES(EMESH_STAIRS(I)%IMESH)%DY(1)
          XB(4) = XB(4) + 0.001_EB*MESHES(EMESH_STAIRS(I)%IMESH)%DY(1)
          XB(5) = XB(5) - 0.001_EB*MESHES(EMESH_STAIRS(I)%IMESH)%DZ(1)
          XB(6) = XB(6) + 0.001_EB*MESHES(EMESH_STAIRS(I)%IMESH)%DZ(1)
          EXIT NSTRS_LOOP_3
       END DO NSTRS_LOOP_3
    END IF IMODE_3_IF
