...
IF (MASS_FILE) THEN
   ...
   ELSE
      ...
      LABEL(3+N_SPECIES:3+N_SPECIES+N_TRACKED_SPECIES-1) = SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ID
...
WRITE(LU_CORE(NM)) Q_DOT_SUM(1:N_Q_DOT,NM),M_DOT_SUM(1:N_M_DOT,NM),MINT_SUM(0:N_SPECIES+N_TRACKED_SPECIES,NM)
DO N=1,N_DEVC
   DV => DEVICE(N)
...
READ(LU_RESTART(NM)) T,ICYC,PART_CLOCK(NM),RESTART_CLOCK,SLCF_CLOCK(NM),SMOKE3D_CLOCK(NM),RADF_CLOCK(NM), &
             ...
             RTE_SOURCE_CORRECTION_FACTOR,RAD_Q_SUM,KFST4_SUM,ENTHALPY_SUM(NM),&
             BC_CLOCK_HT3D,WALL_COUNTER_HT3D
READ(LU_RESTART(NM)) Q_DOT_SUM(1:N_Q_DOT,NM),M_DOT_SUM(1:N_M_DOT,NM),MINT_SUM(0:N_SPECIES+N_TRACKED_SPECIES,NM)
ICYC_RESTART = ICYC
DO N=1,N_DEVC
   DV => DEVICE(N)
         ...
         IF (CC_IBM) THEN
            ...
            IF (CCVAR(I,J,K,IBM_IDCC) > 0) THEN ! we have a cutcell
               ...
               DO JCC=1,NCELL
                  ...
                  MINT(N_SPECIES+1:N_SPECIES+N_TRACKED_SPECIES,NM) = MINT(N_SPECIES+1:N_SPECIES+N_TRACKED_SPECIES,NM) &
...
REAL(EB), INTENT(IN) :: T,DT
REAL(FB) :: STIME
REAL(EB) :: MINT_TOTAL(0:N_SPECIES+N_TRACKED_SPECIES)
...
DO NM=1,NMESHES
   MINT_TOTAL(0:N_TOTAL_SPECIES) = MINT_TOTAL(0:N_TOTAL_SPECIES) + MINT_SUM(0:N_TOTAL_SPECIES,NM)/MAX(DT,T-T_LAST_DUMP_MASS)
ENDDO

WRITE(TCFORM,'(A,I0,5A)') "(",N_TOTAL_SPECIES+1,"(",FMT_R,",','),",FMT_R,")"
WRITE(LU_MASS,TCFORM) STIME,(MINT_TOTAL(N),N=0,N_TOTAL_SPECIES)
