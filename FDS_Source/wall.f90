...
IF (N_TRACKED_SPECIES==1 .AND. (.NOT. SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX) .OR. &
   (WC%NODE_INDEX < 0 .AND. SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX .AND. ABS(SF%MASS_FLUX(1))<=TWO_EPSILON_EB)) THEN
   WC%ZZ_F(1) = 1._EB
   RETURN
ENDIF
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (MFT >= 0._EB) THEN
      ...
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
      WC%RHO_F = PBAR_P(KK,WC%PRESSURE_ZONE)/(RSUM_F*WC%ONE_D%TMP_F)
      WC%SURF_INDEX = HVAC_OUT_SURF_INDEX
      UN = MFT/WC%RHO_F
      IF (PREDICTOR) WC%ONE_D%UWS = UN
      IF (CORRECTOR) WC%ONE_D%UW  = UN
   ELSE
      WC%SURF_INDEX = HVAC_IN_SURF_INDEX
