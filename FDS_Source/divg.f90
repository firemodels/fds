...
REAL(EB) :: DELKDELT,VC,DTDX,DTDY,DTDZ,TNOW,ZZ_GET(0:N_TRACKED_SPECIES), &
            ...
            TMP_G,TMP_WGT,DIV_DIFF_HEAT_FLUX,H_S,UN,D_RHSOP_DT_0
   ...
   IF (PREDICTOR) THEN
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
               IF (.NOT.CHANGE_TIME_STEP(NM)) D_RHSOP_DT_S(I,J,K) = 3._EB*D_RHSOP_DT(I,J,K) - 2._EB*D_RHSOP_DT_S(I,J,K)
               PBAR_D_RHSOP_DT(I,J,K) = PBAR_P(K,PRESSURE_ZONE(I,J,K))*D_RHSOP_DT_S(I,J,K)
            ENDDO
         ENDDO 
      ENDDO
   ELSE
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
               D_RHSOP_DT_0 = ( RHO_H_S_P(I,J,K)/PBAR_P(K,PRESSURE_ZONE(I,J,K)) - RHO_H_S_OVER_PBAR(I,J,K) )/DT
               D_RHSOP_DT(I,J,K) = 1.25_EB*D_RHSOP_DT_0 + 0.375_EB*D_RHSOP_DT_S(I,J,K) - 0.625_EB*D_RHSOP_DT(I,J,K)
               D_RHSOP_DT_S(I,J,K) = D_RHSOP_DT_0
               PBAR_D_RHSOP_DT(I,J,K) = PBAR_P(K,PRESSURE_ZONE(I,J,K))*D_RHSOP_DT(I,J,K)
            ENDDO
         ENDDO 
      ENDDO
   ENDIF
...
REAL(EB) :: RDT,TNOW,P_EQ,SUM_P_PSUM,SUM_USUM,SUM_DSUM,SUM_PSUM,D_PBAR_DT_0
LOGICAL :: OPEN_ZONE
...
PRESSURE_ZONE_LOOP: DO IPZ=1,N_ZONE
   ...
   IF (ABS(PSUM(IPZ,NM)) > ZERO_P) THEN
      IF (ENTHALPY_TRANSPORT) THEN
         IF (PREDICTOR) THEN
            IF (.NOT.CHANGE_TIME_STEP(NM)) D_PBAR_DT_S(IPZ) = 3._EB*D_PBAR_DT(IPZ) - 2._EB*D_PBAR_DT_S(IPZ)
         ELSE
            D_PBAR_DT_0 = (DSUM(IPZ,NM) - USUM(IPZ,NM))/PSUM(IPZ,NM)
            D_PBAR_DT(IPZ) = 1.25_EB*D_PBAR_DT_0 + 0.375_EB*D_PBAR_DT_S(IPZ) - 0.625_EB*D_PBAR_DT(IPZ)
            D_PBAR_DT_S(IPZ) = D_PBAR_DT_0
         ENDIF
      ELSE
