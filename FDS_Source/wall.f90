...
CALL DIFFUSIVITY_BC
CALL THERMAL_BC(T)
IF (ANY(SPECIES_MIXTURE%DEPOSITING) .AND. .NOT.INITIALIZATION_PHASE) CALL CALC_DEPOSITION(NM)
CALL SPECIES_BC(T,NM)
CALL DENSITY_BC
IF (HVAC_SOLVE .AND. .NOT.INITIALIZATION_PHASE) CALL HVAC_BC
IF (N_FACE>0 .AND. .NOT.INITIALIZATION_PHASE)   CALL GEOM_BC(NM)
...
REAL(EB) :: AREA_ADJUST,ARO,FDERIV,QEXTRA,QNET,RAMP_FACTOR,RHO_G,RHO_G_2,RSUM_W,TMP_G,TMP_OTHER,TSI,UN, &
            ZZ_G_ALL(MAX_SPECIES),RHO_ZZ_F(MAX_SPECIES),ZZ_GET(0:N_TRACKED_SPECIES),T_IGN,DUMMY, &
   ...
   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         ONE_D%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,SF%HEAT_TRANSFER_MODEL,&
                                                           SF%ROUGHNESS,WALL_INDEX=WALL_INDEX)
      ELSE
         ONE_D%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,SF%HEAT_TRANSFER_MODEL,&
                                                           SF%ROUGHNESS,PARTICLE_INDEX=PARTICLE_INDEX)
      ENDIF
...
REAL(EB) :: RADIUS,AREA_SCALING,RVC,RHO_NEW,M_DOT_PPP,CP,CPBAR,MW_RATIO,H_G,DELTA_H_G,ZZ_GET(0:N_TRACKED_SPECIES), &
            CPBAR2,MOMENTUM_SINK_FACTOR,XI,YJ,ZK,UBAR,VBAR,WBAR,QREL
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
INTEGER :: II,JJ,KK,IIG,JJG,KKG,IW,NS,IP,IIX,JJY,KKZ
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   ELSE
      RADIUS = SF%INNER_RADIUS + SF%RADIUS
   ENDIF
   ...
   IF (EVAPORATION_DRAG) THEN
      ...
      M_DOT_PPP = LP%PWT*SUM(ONE_D%MASSFLUX(0:N_TRACKED_SPECIES))*ONE_D%AREA*RVC
      MOMENTUM_SINK_FACTOR = (1._EB - EXP(-M_DOT_PPP*DT/RHO(IIG,JJG,KKG)))/DT
      ...
      IF (KINETIC_ENERGY_SOURCE) THEN
         ...
         D_LAGRANGIAN(IIG,JJG,KKG) = D_LAGRANGIAN(IIG,JJG,KKG) + M_DOT_PPP * QREL / (RHO(IIG,JJG,KKG) * H_G)
      ENDIF
   ENDIF

   ! Update temperature

   IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZS(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
   CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM(IIG,JJG,KKG))
   TMP(IIG,JJG,KKG) = PBAR(KKG,PRESSURE_ZONE(IIG,JJG,KKG)) / RHO(IIG,JJG,KKG) / RSUM(IIG,JJG,KKG)
...
USE MATH_FUNCTIONS, ONLY : EVALUATE_RAMP
REAL(EB) :: ZZ_G,UN,DD,MFT,TSI,ZZ_GET(0:N_TRACKED_SPECIES),RSUM_F,MPUA_SUM,T_IGN,AREA_ADJUST
INTEGER :: N,ITER
INTEGER, INTENT(IN), OPTIONAL :: WALL_INDEX,PARTICLE_INDEX
...
IF (N_TRACKED_SPECIES==0 .AND. SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX .AND. ABS(SF%MASS_FLUX(0))<=TWO_EPSILON_EB) RETURN
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         ...
         IF (WC%EW>TWO_EPSILON_EB) THEN
            ONE_D%MASSFLUX(0:N_TRACKED_SPECIES) = ONE_D%MASSFLUX(0:N_TRACKED_SPECIES)*EXP(-WC%EW)
            ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES) = ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES)*EXP(-WC%EW)
         ENDIF
   ...
   CUTCELL_LOOP: DO
      ...
      METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
         ...
         CASE(SPECIFIED_TEMPERATURE)
            ...
            ENDIF
            DTMP = TMP_G - TMP_F
            HTC = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,SF%HEAT_TRANSFER_MODEL,SF%ROUGHNESS,&
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN UNNPACK_WALL_PARTICLE
   ...
   ONE_D%HEAT_TRANS_COEF = &
      HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,SF%HEAT_TRANSFER_MODEL,SF%ROUGHNESS,&
...
SELECT CASE(SF%BACKING)
   CASE(VOID)  ! Non-insulated backing to an ambient void
      ...
      ENDIF
      DTMP = TMP_BACK - TMP_B
      HTCB = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,HT_MODEL=0,ROUGHNESS=0._EB,WALL_INDEX=-1)
   ...
   CASE(EXPOSED)
      IF (IWB >= 0) THEN  
         ...
         IF (WALL(IWB)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
            ...
            DTMP = TMP_BACK - TMP_B
            HTCB = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,HT_MODEL=0,ROUGHNESS=0._EB,WALL_INDEX=IWB)
...
PYROLYSIS_MATERIAL_IF: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_MATERIAL) THEN
   ...
   IF (T_IGN > T) THEN
      IF (SUM(ONE_D%MASSFLUX(0:N_TRACKED_SPECIES)) > 0._EB) T_IGN = T
   ENDIF
...
REAL(EB) FUNCTION HEAT_TRANSFER_COEFFICIENT(DELTA_TMP,H_FIXED,GEOMETRY,CONV_LENGTH,HT_MODEL,ROUGHNESS,&
                                            WALL_INDEX,PARTICLE_INDEX,FACE_INDEX,CUTCELL_INDEX)

! Compute the convective heat transfer coefficient

USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_HEAT,GET_VISCOSITY,GET_CONDUCTIVITY
...
INTEGER  :: IIG,JJG,KKG,IOR,GEOMETRY,HT_MODEL,RE_INDEX
REAL(EB) :: RE,U2,V2,W2,H_NATURAL,H_FORCED,NUSSELT,ZZ_GET(0:N_TRACKED_SPECIES),CP,VELCON,FRICTION_VELOCITY,YPLUS,MU_DNS,KP,&
            RHO_G,TMP_G,TMP_F,DN,TMP_FILM,RE_CYL_EXP,C_FORCED_CYLINDER
REAL(EB), POINTER, DIMENSION(:,:,:) :: UU,VV,WW,RHOP
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP
INTEGER, PARAMETER :: LOGLAW=1,ABL=2,CUSTOM_HT=-1
...
HTC_MODEL_SELECT: SELECT CASE(HT_MODEL)
   CASE DEFAULT
      ...
      IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
      CALL GET_VISCOSITY(ZZ_GET,MU_DNS,TMP_FILM)
      CALL GET_CONDUCTIVITY(ZZ_GET,KP,TMP_FILM)
      RE = RHO_G*VELCON*CONV_LENGTH/MU_DNS
      SELECT CASE(GEOMETRY)
         ! Natural - Holman (Hilpert) Table 7-2
         CASE (SURF_CARTESIAN)
            SELECT CASE(ABS(IOR))
               CASE(0:2)
            ...
            END SELECT
            ! Forced - Holman, 1990, eq 5-84   
            NUSSELT = (0.037_EB*RE**(0.8_EB)-871._EB)*PR_ONTH 
            
         CASE (SURF_CYLINDRICAL)
            ! Natural - Holman, 1990, Table 7-2
            H_NATURAL = HCV*ABS(DELTA_TMP)**ONTH

            ! Forced - Holman, 1990, Table 6-2
            RE_INDEX = INT(LOG10(0.25_EB*RE))
            SELECT CASE(RE_INDEX) 
               CASE (:-1)
                  C_FORCED_CYLINDER = 0.989_EB
                  RE_CYL_EXP = 0.33_EB
               CASE (0)
                  C_FORCED_CYLINDER = 0.911_EB
                  RE_CYL_EXP = 0.385_EB
               CASE (1,2)
                  C_FORCED_CYLINDER = 0.683_EB
                  RE_CYL_EXP = 0.466_EB
               CASE (3)
                  C_FORCED_CYLINDER = 0.193_EB
                  RE_CYL_EXP = 0.618_EB
               CASE (4:)
                  C_FORCED_CYLINDER = 0.0266_EB
                  RE_CYL_EXP = 0.805_EB
            END SELECT
            NUSSELT =  C_FORCED_CYLINDER*RE**RE_CYL_EXP*PR_ONTH 

         CASE (SURF_SPHERICAL)
            ! Incropera and DeWitt            
            NUSSELT = 2._EB +0.6_EB*RE**(0.5_EB)*PR_ONTH
            H_NATURAL = 0._EB

      END SELECT
      H_FORCED  = NUSSELT*KP/CONV_LENGTH
   CASE(LOGLAW)      
      SELECT CASE(ABS(IOR))
         CASE(0:2)
            H_NATURAL = HCV*ABS(DELTA_TMP)**ONTH
         CASE(3)
            H_NATURAL = HCH*ABS(DELTA_TMP)**ONTH
      END SELECT   
      CALL GET_VISCOSITY(ZZ_GET,MU_DNS,TMP_G)
      CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,TMP_G)
      CALL GET_CONDUCTIVITY(ZZ_GET,KP,TMP_G)
      H_NATURAL = 0._EB
      CALL HEAT_FLUX_MODEL(H_FORCED,YPLUS,FRICTION_VELOCITY,KP,RHO_G,CP,MU_DNS)
   CASE(ABL)
      SELECT CASE(ABS(IOR))
         CASE(0:2)
            H_NATURAL = HCV*ABS(DELTA_TMP)**ONTH
      ...
      CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,TMP(IIG,JJG,KKG))
      H_NATURAL = 0._EB
      CALL ABL_HEAT_FLUX_MODEL(H_FORCED,FRICTION_VELOCITY,DN,ROUGHNESS,TMP_G,TMP_F,RHO_G,CP)
   CASE(CUSTOM_HT)
      IF (PRESENT(PARTICLE_INDEX)) THEN
         SF=>SURFACE(LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)%SURF_INDEX)
      ELSEIF (PRESENT(WALL_INDEX)) THEN
...
REAL(EB), PARAMETER :: CM3=3._EB*CM,CS2=CS*2._EB,CT2=2._EB*CT,K=1.3806488E-23_EB
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_AEROSOL,RHOG,ZZ_GET(0:N_TRACKED_SPECIES),YDEP,K_AIR,TMP_FILM,KN,ALPHA,DTMPDX,&
            KN_EXP,TAU_PLUS,DN,TAU_PLUS_C,VEL_W,U2,V2,W2,U_GRAV,MASS_P,D_SOLID
...
SMIX_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (GRAVITATIONAL_DEPOSITION) THEN
         ...
         IF (.NOT. GRAVITATIONAL_DEPOSITION_CEILING) THEN
      ...
      IF (TURBULENT_DEPOSITION) THEN
         ...
         IF (TAU_PLUS < 0.2_EB) THEN ! Diffusion regime
            D_SOLID = K*TGAS*(1._EB+1.25_EB*KN+0.41_EB*KN*EXP(-0.88_EB/KN))/(3._EB*PI*MUGAS*SM%MEAN_DIAMETER)
