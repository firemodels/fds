...
#ifdef WITH_HDF5
USE HDF5
USE VTK_FDS_INTERFACE, ONLY : WRITE_VTKHDF_GEOM_FILE, INITIALIZE_VTKHDF_FILES
#endif
...
! Define Resource Sets Communication information:

CALL DEFINE_RS_COMM_INFO
...
IF (MY_RANK==0 .AND. WRITE_SMV) THEN
...
! Count number of meshes in each PROCESS
ALLOCATE(MESHES_PER_PROCESS(N_MPI_PROCESSES))
...
! MPI call to exchange N_PATCH for VTK boundary output
CALL EXCHANGE_NPATCH_INFO

! MPI call to exchange slice info for VTK output
CALL EXCHANGE_NSLICE_INFO
...
IF (.NOT.RESTART) THEN
   ...
   DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      CALL DUMP_MESH_OUTPUTS(T,DT,NM,.FALSE.)
      N_WRITTEN = N_WRITTEN + 1
   ENDDO
#ifdef WITH_HDF5
   IF ((VTK_HDF).AND.(WRITE_VTK)) THEN
      DO NM=1,MAXVAL(MESHES_PER_PROCESS)
         IF (N_WRITTEN < MAXVAL(MESHES_PER_PROCESS)) THEN
            CALL DUMP_MESH_OUTPUTS(T,DT,LOWER_MESH_INDEX,.TRUE.)
            N_WRITTEN = N_WRITTEN + 1
         ENDIF
      ENDDO
   ENDIF
...
IF (WRITE_SMV) CALL WRITE_STRINGS
...
#ifdef WITH_HDF5
   CALL H5CLOSE_F(ERROR) ! Close HDF5 interface
#endif
...
INTEGER, PARAMETER :: PADDING = 128
CHARACTER(LEN=LINE_LENGTH+PADDING) :: LINE
CHARACTER(LEN=LINE_LENGTH+PADDING), DIMENSION(0:N_MPI_PROCESSES-1) :: LINE_ARRAY
CHARACTER(30) :: FRMT
...
ELSE
   LINE_ARRAY(0) = LINE
   DO N=1,N_MPI_PROCESSES-1
      !-- Clear out buffer first by writing empty string
      WRITE(LINE, FMT='(a)') ''
      CALL MPI_RECV(LINE,LINE_LENGTH,MPI_CHARACTER,N,N,MPI_COMM_WORLD,STATUS,IERR)
      LINE_ARRAY(N) = LINE
   ENDDO
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   IF (MY_RANK>0) THEN
      ...
      IF (MESHES(NM)%N_STRINGS>0) THEN
        ! Pack the strings:
        ALLOCATE(CHARACTER(MESHES(NM)%N_STRINGS * (MESH_STRING_LENGTH+PADDING)) :: &
                 STRING_BUF)
        DO N=1,MESHES(NM)%N_STRINGS
          ASSOCIATE ( MST => MESH_STRING_LENGTH )
          STRING_BUF ((N-1)*(MST+PADDING)+1:(N-1)*(MST+PADDING)+MST) = MESHES(NM)%STRING(N)
          END ASSOCIATE
        END DO
        CALL MPI_SEND(STRING_BUF,MESHES(NM)%N_STRINGS*(MESH_STRING_LENGTH+PADDING),MPI_CHARACTER,0,NM,MPI_COMM_WORLD,IERR)
   ...
   OTHER_MESH_LOOP: DO NOM=2,NMESHES
      IF (PROCESS(NOM)>0) THEN
         ...
         IF (N_STRINGS_DUM>0) THEN
            ALLOCATE(CHARACTER(LEN=N_STRINGS_DUM*(MESH_STRING_LENGTH+PADDING)) :: STRING_BUF )
            CALL MPI_RECV(STRING_BUF,N_STRINGS_DUM*(MESH_STRING_LENGTH+PADDING),MPI_CHARACTER,&
                          PROCESS(NOM),NOM,MPI_COMM_WORLD,STATUS,IERR)
            ! Unpack the strings:
            ALLOCATE(STRING_DUM(N_STRINGS_DUM))
            ASSOCIATE ( MST => MESH_STRING_LENGTH )
            DO N=1,N_STRINGS_DUM
              STRING_DUM(N) = STRING_BUF((N-1)*(MST+PADDING)+1:(N-1)*(MST+PADDING)+MST)
            END DO
            END ASSOCIATE
            DEALLOCATE (STRING_BUF)
         ENDIF
...
INTEGER :: NM,IERR,IPROC,INT_EXC(3)
...
! Send obst info from all ranks to rank 0
DO NM=1,NMESHES
   DO IPROC=1,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC.AND.IPROC==PROCESS(NM)) THEN
         IF (MY_RANK/=0) THEN
            INT_EXC = (/MESHES(NM)%N_OBST, NP(NM), NC(NM)/)
            CALL MPI_SEND(INT_EXC,3,MPI_INTEGER,0,PROCESS(NM),MPI_COMM_WORLD,IERR)
         ENDIF
      ELSEIF (MY_RANK==0) THEN
         ...
         CALL MPI_RECV(INT_EXC,3,MPI_INTEGER,IPROC,IPROC,MPI_COMM_WORLD,MPISTATUS,IERR)
         IF (MESHES(NM)%N_OBST == 0) MESHES(NM)%N_OBST = INT_EXC(1)
         IF (MESHES(NM)%NP == 0) MESHES(NM)%NP = INT_EXC(2)
         IF (MESHES(NM)%NC == 0) MESHES(NM)%NC = INT_EXC(3)
      ENDIF
   ENDDO
ENDDO
! Send obst info from rank 0 to all ranks

DO NM=1,NMESHES
   DO IPROC=1,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC) THEN
         IF (MY_RANK/=0) THEN
            CALL MPI_RECV(INT_EXC,3,MPI_INTEGER,0,0,MPI_COMM_WORLD,MPISTATUS,IERR)
            MESHES(NM)%N_OBST = INT_EXC(1)
            MESHES(NM)%NP = INT_EXC(2)
            MESHES(NM)%NC = INT_EXC(3)
         ENDIF
      ELSEIF (MY_RANK==0) THEN
         INT_EXC = (/MESHES(NM)%N_OBST, MESHES(NM)%NP, MESHES(NM)%NC/)
         CALL MPI_SEND(INT_EXC,3,MPI_INTEGER,IPROC,0,MPI_COMM_WORLD,IERR)
      ENDIF
   ENDDO
ENDDO
...
INTEGER, PARAMETER :: PADDING = 256
LOGICAL, ALLOCATABLE, DIMENSION(:) :: EMPTY_UNIQUE_SLICE, UNIQUE_SLICE_IS_SL3D, UNIQUE_SLICE_IN_DOMAIN
CHARACTER(200), ALLOCATABLE, DIMENSION(:) :: ALL_SLICE_QUANTITIES
CHARACTER(:), ALLOCATABLE :: STRING_BUF
LOGICAL :: SLICE_EXISTS
...
IF (MESHES(1)%N_UNIQUE_SLCF>0) THEN
   ...
   IF (MY_RANK==0) THEN
      DO I=1,MESHES(1)%N_UNIQUE_SLCF
         SLICE_EXISTS=.FALSE.
         DO NM=1,NMESHES
            IF (.NOT.MESHES(NM)%EMPTY_UNIQUE_SLICE(I)) THEN
               SLICE_EXISTS = .TRUE.
               EXIT
            ENDIF
         ENDDO
        IF (SLICE_EXISTS) THEN
           MESHES(1)%UNIQUE_SLICE_IN_DOMAIN(I)=.TRUE.
        ENDIF
      ENDDO
   ENDIF
   ! Send slice existance info from rank 0 to other ranks
   DO IPROC=1,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC) THEN
         IF (MY_RANK/=0) THEN
            IF (.NOT.ALLOCATED(MESHES(1)%UNIQUE_SLICE_IN_DOMAIN)) THEN
               ALLOCATE(MESHES(1)%UNIQUE_SLICE_IN_DOMAIN(MESHES(1)%N_UNIQUE_SLCF))
            ENDIF
            ALLOCATE(UNIQUE_SLICE_IN_DOMAIN(MESHES(1)%N_UNIQUE_SLCF))
            CALL MPI_RECV(UNIQUE_SLICE_IN_DOMAIN,MESHES(1)%N_UNIQUE_SLCF,MPI_LOGICAL,0,IPROC,MPI_COMM_WORLD,MPISTATUS,IERR)
            MESHES(1)%UNIQUE_SLICE_IN_DOMAIN = UNIQUE_SLICE_IN_DOMAIN
            DEALLOCATE(UNIQUE_SLICE_IN_DOMAIN)
         ENDIF
      ELSEIF (MY_RANK==0) THEN
         CALL MPI_SEND(MESHES(1)%UNIQUE_SLICE_IN_DOMAIN,MESHES(1)%N_UNIQUE_SLCF,MPI_LOGICAL,IPROC,IPROC,MPI_COMM_WORLD,IERR)
      ENDIF
   ENDDO
   ...
   DO IPROC=1,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC) THEN
         IF (MY_RANK/=0) THEN
            ...
            CALL MPI_RECV(UNIQUE_SLICE_IS_SL3D,MESHES(1)%N_UNIQUE_SLCF,MPI_LOGICAL,0,IPROC,MPI_COMM_WORLD,MPISTATUS,IERR)
            MESHES(1)%UNIQUE_SLICE_IS_SL3D = UNIQUE_SLICE_IS_SL3D
            DEALLOCATE(UNIQUE_SLICE_IS_SL3D)
         ENDIF
      ELSEIF (MY_RANK==0) THEN
         CALL MPI_SEND(MESHES(1)%UNIQUE_SLICE_IS_SL3D,MESHES(1)%N_UNIQUE_SLCF,MPI_LOGICAL,IPROC,IPROC,MPI_COMM_WORLD,IERR)
      ENDIF
   ENDDO
   ...
   DO IPROC=1,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC) THEN
         ALLOCATE(CHARACTER(MESHES(1)%N_SLCF_O*(200+PADDING)*3) :: STRING_BUF)
         CALL MPI_RECV(STRING_BUF,MESHES(1)%N_SLCF_O*(200+PADDING)*3,MPI_CHARACTER,0,IPROC,MPI_COMM_WORLD,MPISTATUS,IERR)
      ...
      ELSEIF (MY_RANK==0) THEN
         ...
         DO I=1,MESHES(1)%N_SLCF_O*3
            STRING_BUF((I-1)*(200+PADDING)+1:(I-1)*(200+PADDING)+200) = MESHES(1)%ALL_SLICE_QUANTITIES(I)
         ENDDO
         CALL MPI_SEND(STRING_BUF,MESHES(1)%N_SLCF_O*(200+PADDING)*3,MPI_CHARACTER,IPROC,IPROC,MPI_COMM_WORLD,IERR)
         DEALLOCATE(STRING_BUF)
      ENDIF
   ENDDO
ENDIF
...
TYPE (MPI_STATUS) :: MPISTATUS
#endif
INTEGER :: NM,IERR,IPROC,N_PATCH

IF (N_MPI_PROCESSES==1) RETURN

! Copy BNDF N_PATCH to rank 0 for VTK output
DO NM=1,NMESHES
   !CALL POINT_TO_MESH(NM)
   DO IPROC=1,N_MPI_PROCESSES-1
      IF (MY_RANK==IPROC.AND.IPROC==PROCESS(NM)) THEN
         IF (MY_RANK/=0) THEN
            CALL MPI_SEND(MESHES(NM)%N_PATCH,1,MPI_INTEGER,0,PROCESS(NM),MPI_COMM_WORLD,IERR)
         ENDIF
      ELSEIF (MY_RANK==0) THEN
         IF (PROCESS(NM)==0) CYCLE
         IF (PROCESS(NM)/=IPROC) CYCLE
         CALL MPI_RECV(N_PATCH,1,MPI_INTEGER,IPROC,IPROC,MPI_COMM_WORLD,MPISTATUS,IERR)
         MESHES(NM)%N_PATCH = N_PATCH
      ENDIF
   ENDDO
ENDDO
...
SUBROUTINE DEFINE_RS_COMM_INFO

CHARACTER(3) :: RANKS_GPU_CHAR
INTEGER :: STATUS_RPG_READ,IERR,COMM_COLOR,I
#ifdef WITHOUT_MPIF08
INTEGER :: GROUP_WORLD
#else
...
! Here read of define FDS_RANKS_PER_GPU and communicators MPI_COMM_RS (with master rank MASTER_RS), MPI_COMM_RS_MASTERS.
! RS stands for Resource Set, taken from Summit architecture, represents a GPU and associated MPI processes.
CALL GET_ENVIRONMENT_VARIABLE(NAME="FDS_RANKS_PER_GPU",VALUE=RANKS_GPU_CHAR,STATUS=STATUS_RPG_READ)

IF(STATUS_RPG_READ/=0) THEN ! Cases -1 (FDS_RANKS_PER_GPU>3 chars),
                           !        1 (FDS_RANKS_PER_GPU not defined),
                           !        2 (Env Var reading not supported).
   IF(MY_RANK==0 .AND. STATUS_RPG_READ==1) WRITE(LU_ERR,*) 'Note : Environment variable FDS_RANKS_PER_GPU not present,',&
                                 ' proceeding as if all MPI ranks on Resource Set are linked to GPU.'
   FDS_RANKS_PER_GPU = 1
ELSE ! Read correctly RANKS_GPU_CHAR
   READ(RANKS_GPU_CHAR(1:3),'(I3)') FDS_RANKS_PER_GPU
ENDIF

IF(MY_RANK==0) WRITE(LU_ERR,*) 'Running with ',FDS_RANKS_PER_GPU,' MPI ranks per Resource Set.'

! Split communicator in RS communicators:
COMM_COLOR=MY_RANK/FDS_RANKS_PER_GPU
CALL MPI_COMM_SPLIT(MPI_COMM_WORLD,COMM_COLOR,MY_RANK,MPI_COMM_RS,IERR)
CALL MPI_COMM_RANK(MPI_COMM_RS,MY_RANK_RS,IERR)
CALL MPI_ALLREDUCE(MY_RANK_RS,N_MPI_RS,1,MPI_INTEGER,MPI_MAX,MPI_COMM_RS,IERR); N_MPI_RS=N_MPI_RS+1 ! Highest rank + 1

! Create communicator for RS masters:
N_MPI_RS_MASTERS=N_MPI_PROCESSES/FDS_RANKS_PER_GPU
IF(MOD(N_MPI_PROCESSES,FDS_RANKS_PER_GPU)>0) N_MPI_RS_MASTERS=N_MPI_RS_MASTERS+1

CALL MPI_COMM_GROUP(MPI_COMM_WORLD,GROUP_WORLD,IERR)
IF(ALLOCATED(MASTERS_RS)) DEALLOCATE(MASTERS_RS); ALLOCATE(MASTERS_RS(0:N_MPI_RS_MASTERS-1))
IN_MASTERS_RS=.FALSE.
DO I=0,N_MPI_RS_MASTERS-1
   MASTERS_RS(I) = I*FDS_RANKS_PER_GPU
   IF(MY_RANK==MASTERS_RS(I)) IN_MASTERS_RS=.TRUE.
ENDDO
CALL MPI_GROUP_INCL(GROUP_WORLD,N_MPI_RS_MASTERS,MASTERS_RS(0:N_MPI_RS_MASTERS-1),GROUP_RS_MASTERS,IERR)
CALL MPI_COMM_CREATE(MPI_COMM_WORLD,GROUP_RS_MASTERS,MPI_COMM_RS_MASTERS,IERR)
MY_RANK_RS_MASTERS=-1; IF(IN_MASTERS_RS) CALL MPI_COMM_RANK(MPI_COMM_RS_MASTERS,MY_RANK_RS_MASTERS,IERR)
! IF(IN_MASTERS_RS) &
! WRITE(LU_ERR,*) MY_RANK,'FDS_RANKS_PER_GPU,N_MPI_RS,N_MPI_RS_MASTERS,MY_RANK_RS,MY_RANK_RS_MASTERS=',&
!                 FDS_RANKS_PER_GPU,N_MPI_RS,N_MPI_RS_MASTERS,MY_RANK_RS,MY_RANK_RS_MASTERS
