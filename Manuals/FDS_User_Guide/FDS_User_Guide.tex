...
Particulates in the flowfield, such as soot, can deposit on surfaces removing them from the flow.  For fire simulations this could result in a delay in detection time or an improvement in the visbility for egress.  The deposition of soot on surfaces results in visible patterns and being able to simulate deposition would allow for the evaluation of a specific cause and origin hypothesis against observed patterns at a fire scene.  Lastly a number of toxicants occur as aerosols (for examle radionuclides or chemical or biological agents), and surface deposition is an important phenomena for evaluating dispersion.  FDS contains an experimental model for computing the deposition of aerosols onto surfaces.

Aerosol deposition is determined by computing a deposition velocity, u$_dep$, and applying that velocity to the aerosol next to a wall cell to determine a deposition rate.  There are a number of phenomena that result in the deposition velocity: thermophoresis (where temperature gradients push the aerosol towards or away from the wall), gravitional settling, diffusive deposition (where the aerosols move along the boundary layer concentration gradient), and turbulent deposition (essentially impact deposition due to a turbulent boundary layer).  Other phenomena such eletrical fields can also result in deposition but are not considered in FDS.  The deposition velocity is determined by assuming the phenomena are independent, computing a deposition velocity for each (respectively u$_{th}$, u$_g$, and u$_{dt}$ which is a combined diffusion and turbluence velocity), and summing them~\cite{Bixler:1} .  By default FDS will compute all of the phenomena, but they can be selectively disabled with the logical flags {\ct GRAVITATIONAL\_DEPOSITION}, {\ct THERMOPHORETIC\_DEPOSITION}, and {\ct TURBULENT\_DEPOSITION}.


The gravitational velocity is given by~\cite{Davies_Charles}

\be
u_g = g m_a \frac{Cn}{6 \pi {\chi}_d \mu r_a}
\ee
...
\be
Cn = 1 + 1.25 Kn + 0.41 Kn e^{-\frac{0.88}{Kn}}
\ee
...
\be
Kn = \frac{0.0065~m~at~25~ ^\circ C}{r_a} \frac{T_g}{298.15 K}
\ee
...
K$_{th}$ is the thermophoretic velocity coefficient, and it is calculated using the following correlation~\cite{Brock:1}

\be
 K_{th} = \frac{2 C_s \left(\alpha + C_t Kn \right) Cn}{\left(1 + 3 C_m Kn \right) \left(1 + 2 \alpha + 2 C_t Kn \right) }
\ee
...
The diffusive and turbulent deposition velocity depends upon the flow regime (diffusion, diffusion-impaction, or intertia-moderated).  The deposition velocity for these regimes are given below ~\cite{McCoy_Hanratty}.   

\begin{eqnarray}
u_{dt} &=& 0.086 Sc^{-0.7} u_{\tau} \hspace{1 cm} \mbox{for} \quad \tau^+ < 0.2 \\
u_{dt} &=& 3.5 \times 10^{-4} {\tau^+}^2 u_{\tau} \hspace{0.62 cm} \mbox{for} \quad 0.2 < \tau^+ < 30 \\
u_{dt} &=&  0.17 u_{\tau}  \hspace{2.12 cm} \mbox{for} \quad \tau^+ < 30 \\
\end{eqnarray}
...
1\,\underbrace{\mathrm{(C_3H_8)}}_\text{Fuel} + 1\,\underbrace{\mathrm{(4.82 \, O_2+4.82 \,(^{.79}/_{.21}) \, N_2})}_\text{Oxidizer} \rightarrow \\
\nonumber \underbrace{1\,\mathrm{(4\, H_2O + 2.82\, CO_2 + 4.82\,(^{.79}/_{.21}) \, N_2)}+0.1835\,\mathrm{C}}_\text{Products}
\end{eqnarray}
...
\footnotesize
\begin{verbatim}
&SPEC ID = 'PROPANE', FORMULA = 'C2H3Cl' /
&SPEC ID = 'OXYGEN',            SMIX_COMPONENT_ONLY = .TRUE. /
&SPEC ID = 'NITROGEN',          SMIX_COMPONENT_ONLY = .TRUE. /
&SPEC ID = 'WATER VAPOR',       SMIX_COMPONENT_ONLY = .TRUE. /
&SPEC ID = 'CARBON DIOXIDE',    SMIX_COMPONENT_ONLY = .TRUE. /
...
&SMIX ID = 'AIR', SPEC_ID ='NITROGEN','OXYGEN',
      VOLUME_FRACTION = 18.11896,4.816432, BACKGROUND = .TRUE. /
&SMIX ID = 'PRODUCTS', SPEC_ID ='NITROGEN','CARBON DIOXIDE','WATER VAPOR',
      VOLUME_FRACTION = 18.11896,2.816432,4/
\end{verbatim} \normalsize
...
&REAC FUEL = 'PROPANE', HEAT_OF_COMBUSTION=46000,
      SMIX_ID = 'PROPANE','AIR','PRODUCTS','SOOT', NU=-1.,-1,1,0.183568/
\end{verbatim} \normalsize

\noindent
Note: The sign on NU corresponds to whether that species is consumed (-) or produced (+). The results, Figure \ref{soot_deposition}, show a boundary file plot, of deposition in kg/m$^2$.  This boundary quantity is given by the input below.
