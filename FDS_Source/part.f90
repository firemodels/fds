...
REAL(EB) :: PHI_RN,FLOW_RATE,THETA_RN,SPHI,CPHI,MASS_SUM,D_PRES_FACTOR, &
            STHETA,CTHETA,PWT0,PARTICLE_SPEED,XI,YJ,ZK,SHIFT1,SHIFT2,XTMP,YTMP,ZTMP,VLEN, &
...
!      IF (T < PY%PARTICLE_INSERT_CLOCK(NM)) CYCLE SPRINKLER_INSERT_LOOP
      ...
      PARTICLE_INSERT_LOOP: DO I=1,N_INSERT
         ...
         LP%A_X    = 0._EB                  ! x component of drag on the gas in units of m/s2
         LP%A_Y    = 0._EB                  ! y component of drag on the gas in units of m/s2
         LP%A_Z    = 0._EB                  ! z component of drag on the gas in units of m/s2
   ...
   WALL_INSERT_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IOR = IJKW(4,IW)
      MASS_SUM = 0._EB
      PARTICLE_INSERT_LOOP2: DO I=1,NPPCW(IW) 
      ...
      IF (MASS_SUM > 0._EB) THEN
         IF (SF%PARTICLE_MASS_FLUX > 0._EB) THEN
            ...
            PARTICLE(NLP-NPPCW(IW)+1:NLP)%PWT = &
            PARTICLE(NLP-NPPCW(IW)+1:NLP)%PWT*FLOW_RATE*AREA_ADJUST(IW)*AW(IW)*SF%DT_INSERT/MASS_SUM
         ENDIF
      ENDIF
      ...
      INSERT_PARTICLE_LOOP: DO I=1,N_INITIAL
         ...
         ELSE POINTWISE_IF
           ...
           !!  CALL RANDOM_CONE(LP%X,LP%Y,LP%Z,0.5_EB*(X2+X1),0.5_EB*(Y2+Y1),Z1,0.25_EB,0.5_EB)
               ...
               II = CELLSI(FLOOR((LP%X-XS)*RDXINT)) + 1._EB
               JJ = CELLSJ(FLOOR((LP%Y-YS)*RDYINT)) + 1._EB
               KK = CELLSK(FLOOR((LP%Z-ZS)*RDZINT)) + 1._EB
         ...
         IF (PC%DIAMETER>0._EB .AND. .NOT.IN%POINTWISE_PARTICLE_INIT) THEN
            IF (PC%MONODISPERSE) THEN
               LP%R   = 0.5_EB*PC%DIAMETER
               LP%PWT = 1._EB
            ELSE
         ...
         IF (PC%SURF_INDEX>0) THEN
            ...
            ENDIF
            LP%PWT = 1._EB
            XW(LP%WALL_INDEX) = LP%X
            YW(LP%WALL_INDEX) = LP%Y
            ZW(LP%WALL_INDEX) = LP%Z
            IJKW(1,LP%WALL_INDEX) = II
            IJKW(2,LP%WALL_INDEX) = JJ
            IJKW(3,LP%WALL_INDEX) = KK
            IJKW(4,LP%WALL_INDEX) = 0
            IJKW(5,LP%WALL_INDEX) = PC%SURF_INDEX
            IJKW(6,LP%WALL_INDEX) = II
            IJKW(7,LP%WALL_INDEX) = JJ
            IJKW(8,LP%WALL_INDEX) = KK
         ENDIF
...
REAL     :: RN
REAL(EB) :: RHO_G,RVC,RDS,RDC,QREL,UREL,VREL,WREL,TMP_G,THETA_RN, &
            RD,T,C_DRAG,XI,YJ,ZK,MU_AIR,WE_G,DROP_VOL_FRAC,DROP_DEN, &
            UBAR,VBAR,WBAR, &
            UVW,X_OLD,Y_OLD,Z_OLD,STEP_FRACTION(-3:3),SURFACE_PARTICLE_DIAMETER, &
            T_BU_BAG,T_BU_STRIP,B_1,THROHALF,P_UVWMAX,UVWMAX, &
            ALPHA,BETA,DR_MASS,FP_MASS,OBDT,OPA,BDTOA,U_OLD,V_OLD,W_OLD,MPOM,RDT,&
            WAKE_VEL,RE_WAKE,SFAC,HALF_DT2,ALBO,DTOPA !,HAB,PARACOR
...
DO I=1,NLP
   ...
   JJ  = FLOOR(YJ+1._EB)
   KK  = FLOOR(ZK+1._EB)
   IF (LP%PWT>0._EB) NDPC(II,JJ,KK)=NDPC(II,JJ,KK)+LP%PWT
ENDDO
...
PARTICLE_LOOP: DO I=1,NLP  
   ...
   UVW = MAX( ABS(LP%U)*RDX(II),ABS(LP%V)*RDY(JJ),ABS(LP%W)*RDZ(KK) )
   ...
   IF (PC%MASSLESS .OR. LP%PWT<=ZERO_P) THEN
      LP%U = UBAR
      LP%V = VBAR
      LP%W = WBAR 
      LP%X = LP%X + LP%U*DT
   ...
   IF (PC%SURF_INDEX<1) THEN
      ...
      IF (DROP_VOL_FRAC > PC%DENSE_VOLUME_FRACTION) CALL WAKE_REDUCTION(DROP_VOL_FRAC,LP%RE,C_DRAG,WAKE_VEL)
   ENDIF
   ...
   BREAKUP: IF (PC%BREAKUP) THEN
      ...
      AGE_IF: IF ((T-LP%T) > MIN(T_BU_BAG,T_BU_STRIP)) THEN
         IF (PC%MONODISPERSE) THEN
            RD    = THROHALF*RD
         ELSE
            DO WHILE (RD >= LP%R)
               CHILD_RADIUS = PC%BREAKUP_CHILD_DIAMETER*LP%R*PC%CHILD_R_CDF(:)
   ...
   PARTICLE_NON_STATIC_IF: IF (.NOT.PC%STATIC .AND. LP%IOR==0) THEN

      DR_MASS = PC%DENSITY*FOTH*PI*RDC     ! PARTICLE mass
      ...
      IF (BETA>ZERO_P) THEN
         ...
         MPOM = LP%PWT*DR_MASS/(RHO_G/RVC)
         LP%A_X = MPOM*(U_OLD-LP%U)*RDT 
         LP%A_Y = MPOM*(V_OLD-LP%V)*RDT
         LP%A_Z = MPOM*(W_OLD-LP%W)*RDT
   ...
   PARTICLE_STATIC_IF: IF (PC%STATIC .AND. LP%IOR==0 .AND. .NOT. PC%TREE) THEN
      BETA = 0.5_EB*RVC*C_DRAG*(LP%PWT*PI*RDS)*QREL
      OBDT = 1._EB+BETA*DT
      LP%A_X = UBAR*(1._EB/OBDT-1._EB)*RDT 
      LP%A_Y = VBAR*(1._EB/OBDT-1._EB)*RDT
      LP%A_Z = WBAR*(1._EB/OBDT-1._EB)*RDT
   ENDIF PARTICLE_STATIC_IF

   TREE_PARTICLES: IF (LP%IOR==0 .AND. PC%TREE) THEN
      SFAC   = PC%VEG_DRAG_COEFFICIENT*PC%VEG_SV*LP%VEG_PACKING_RATIO*QREL*C_DRAG
      LP%A_X  = SFAC*UREL - LP%VEG_MLR*UBAR/RHO_G
      LP%A_Y  = SFAC*VREL - LP%VEG_MLR*VBAR/RHO_G
      LP%A_Z  = SFAC*WREL - LP%VEG_MLR*WBAR/RHO_G
   ENDIF TREE_PARTICLES

   ! Move particles/PARTICLEs attached to solids

   SOLID_IF: IF (LP%IOR/=0) THEN
      LP%A_X = 0._EB 
      LP%A_Y = 0._EB 
      LP%A_Z = 0._EB 
      LP%X = X_OLD + U_OLD*DT
   ...
   IF (PC%STATIC) CYCLE PARTICLE_LOOP
   ...
   IF (POROUS_FLOOR .AND. LP%Z<ZS .AND. PC%EVAP_INDEX>0) THEN
      ...
      IW = WALL_INDEX(IC,-3)
      IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY .AND. ACCUMULATE_WATER .AND. .NOT.LP%SPLAT) THEN
         AWMPUA(IW,PC%EVAP_INDEX) = AWMPUA(IW,PC%EVAP_INDEX) + LP%PWT*PC%FTPR*RDC*RAW(IW)
         LP%SPLAT = .TRUE.
      ENDIF
      CYCLE PARTICLE_LOOP
   ENDIF
   ...
   IF (LP%X<XS .AND. BOUNDARY_TYPE(WALL_INDEX(IC,-1))/=SOLID_BOUNDARY) CYCLE PARTICLE_LOOP
   IF (LP%X>XF .AND. BOUNDARY_TYPE(WALL_INDEX(IC, 1))/=SOLID_BOUNDARY) CYCLE PARTICLE_LOOP
   IF (LP%Y<YS .AND. BOUNDARY_TYPE(WALL_INDEX(IC,-2))/=SOLID_BOUNDARY) CYCLE PARTICLE_LOOP
   IF (LP%Y>YF .AND. BOUNDARY_TYPE(WALL_INDEX(IC, 2))/=SOLID_BOUNDARY) CYCLE PARTICLE_LOOP
   IF (LP%Z<ZS .AND. BOUNDARY_TYPE(WALL_INDEX(IC,-3))/=SOLID_BOUNDARY) CYCLE PARTICLE_LOOP
   IF (LP%Z>ZF .AND. BOUNDARY_TYPE(WALL_INDEX(IC, 3))/=SOLID_BOUNDARY) CYCLE PARTICLE_LOOP
   ...
   AIR_TO_SOLID: IF (II/=IIN .OR. JJ/=JJN .OR. KK/=KKN) THEN
      ...
      IF_HIT_SOLID: IF (HIT_SOLID) THEN
         ...
         IF (ACCUMULATE_WATER .AND. HIT_SOLID .AND. .NOT.LP%SPLAT) THEN
            AWMPUA(LP%WALL_INDEX,PC%EVAP_INDEX)=AWMPUA(LP%WALL_INDEX,PC%EVAP_INDEX)+LP%PWT*PC%FTPR*LP%R**3*RAW(LP%WALL_INDEX)
            LP%SPLAT = .TRUE.
         ENDIF
   ...
   IF (LP%IOR/=0 .AND. BOUNDARY_TYPE(IW)/=SOLID_BOUNDARY) THEN
      LP%IOR = 0
      LP%WALL_INDEX = 0
...
USE OUTPUT_DATA, ONLY: FUEL_PARTICLE_MLR
...
EVAP_INDEX_LOOP: DO EVAP_INDEX = 1,N_EVAP_INDICES
   ...
   PART_CLASS_LOOP: DO N_PC = 1,N_PART
      ...
      FILM_SUMMING_LOOP: DO I=1,NLP
         ...
         IW = LP%WALL_INDEX
         FILM_THICKNESS(IW) = FILM_THICKNESS(IW) + LP%PWT*LP%R**3/AW(IW)
         ...
         TIME_ITERATION_LOOP: DO WHILE (DT_SUM < DT)
            ...
            IF (N_REACTIONS>0) THEN
               IF (PC%Z_INDEX==REACTION(1)%FUEL_SMIX_INDEX) THEN
                  FUEL_PARTICLE_MLR(NM) = FUEL_PARTICLE_MLR(NM) + WGT*M_VAP/DT_SUBSTEP
   ...
   PART_CLASS_SUM_LOOP: DO N_PC = 1,N_PART
      ...
      PARTICLE_LOOP_2: DO I=1,NLP
         ...
         IF (LP%IOR/=0 .AND. LP%WALL_INDEX>0) THEN
            IW = LP%WALL_INDEX
            WMPUA(IW,EVAP_INDEX) = WMPUA(IW,EVAP_INDEX) + OMRAF*LP%PWT*M_DROP*RAW(IW)
         ENDIF
...
SUM_MOMENTUM_LOOP: DO I=1,NLP
   ...
   IW = WALL_INDEX(IC,1)
   IF (BOUNDARY_TYPE(IW)==NULL_BOUNDARY) THEN
      FVXS(IIX,JJ,KK) = FVXS(IIX,JJ,KK) - LP%A_X
   ENDIF
   IC = CELL_INDEX(II,JJY,KK)
   IW = WALL_INDEX(IC,2) 
   IF (BOUNDARY_TYPE(IW)==NULL_BOUNDARY) THEN
      FVYS(II,JJY,KK) = FVYS(II,JJY,KK) - LP%A_Y
   ENDIF
   IC = CELL_INDEX(II,JJ,KKZ)
   IW = WALL_INDEX(IC,3) 
   IF (BOUNDARY_TYPE(IW)==NULL_BOUNDARY) THEN
      FVZS(II,JJ,KKZ) = FVZS(II,JJ,KKZ) - LP%A_Z
   ENDIF
...
IF (NLP + INSERT_COUNT > MAXIMUM_PARTICLES) THEN
   MAX_DROP_LIFE = REAL(MAXIMUM_PARTICLES,EB) / INSERT_RATE
ELSE
   MAX_DROP_LIFE = 1.E6
ENDIF
IKILL = 0
DROP_LOOP: DO I=1,NLP
   WEED_LOOP: DO
      ...
      IF ((T-LP%T>PC%LIFETIME) .OR. (T-LP%T>MAX_DROP_LIFE .AND. IKILL <= INSERT_COUNT)) THEN
         ...
         CYCLE WEED_LOOP
      ENDIF
      IF (LP%X>XS .AND. LP%X<XF .AND.LP%Y>YS .AND. LP%Y<YF .AND. LP%Z>ZS .AND. LP%Z<ZF)  CYCLE DROP_LOOP
      CALL REPLACE
   ENDDO WEED_LOOP
ENDDO DROP_LOOP
