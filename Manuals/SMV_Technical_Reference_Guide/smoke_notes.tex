...
the fire.  The RTE used by Smokeview to model smoke and fire
appearance is identical to that used by FDS to model radiative
heat transfer.  Smokeview and FDS use this equation in different
ways, however.  FDS solves it at infrared wavelengths of light.
Generally, Smokeview solves the RTE for visible wavelengths of
light.  With the proper extinction coefficient, however, Smokeview
can also view smoke at other wavelengths, simulating a thermal
imager, for example. Smokeview solves the RTE assuming a gray gas
environment.  This is the default solution method for FDS.  One
other important difference is that Smokeview requires a solution
at only one point at a time (any arbitrary point though), the
observer's viewpoint, while FDS requires a radiation field, a
solution at all points within the solution domain.  Even so, the
solution of this integro-differential equation for visualization
requires significant computation and memory  resources.
Approximations are required in order to display smoke and fire at
interactive frame rates.  The primary approximation is to take
advantage of the low albedo character of smoke allowing one to
either simplify or eliminate scattering terms in the RTE.
...
\item form a weighted average of these these colors using either
...
This section discusses splitting the the Radiation Transport
...
sight.  For example, as illustrated in Figure
\ref{figsmokesetup3}, consider an interval $[x_0,x_N]$ that is
split at $D$ into two sub-intervals $[x_0,D]$ and $[D,x_N]$.  The
goal then is to compute a radiance and opacity for $[x_0,x_N]$
using radiances and opacities computed on  the two sub-intervals.
...
planes (see Figure \ref{figray}).  The absorption coefficient also
...
Three example colormaps  are illustrated in Figure
\ref{fig:colormaps}. The black/orange colormap in Figure
...
white colormap in Figure \ref{fig:colormaps}b along with an
...
colormap in Figure \ref{fig:colormaps}c produces non-realistic
