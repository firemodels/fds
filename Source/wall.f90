...
REAL(EB) :: H_S_B,SPYRO_HF_FACTOR,SPYRO_TH_FACTOR
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (SF%REFERENCE_HEAT_FLUX > 0._EB .AND. N_REACTIONS>=1 .AND. PREDICTOR .AND. B1%T_IGN <=T) THEN
         ...
         IF (SF%EMISSIVITY > 0._EB) THEN
            SPYRO_HF_FACTOR =  MAX(0._EB,B1%Q_IN_SMOOTH) / (SF%REFERENCE_HEAT_FLUX * SF%EMISSIVITY)
         ELSE
            SPYRO_HF_FACTOR =  MAX(0._EB,B1%Q_IN_SMOOTH) / (SF%REFERENCE_HEAT_FLUX)
         ENDIF
         IF (SF%REFERENCE_CONE_THICKNESS > 0._EB) THEN
            SPYRO_TH_FACTOR = SF%REFERENCE_CONE_THICKNESS / SF%LAYER_THICKNESS(1)
         ELSE
            SPYRO_TH_FACTOR = 1.
         ENDIF
         B1%T_SCALE = B1%T_SCALE + DT * SPYRO_HF_FACTOR * SPYRO_TH_FACTOR
         CALL INTERPOLATE1D_UNIFORM(1,RP%INTERPOLATED_DATA(1:RP%NUMBER_INTERPOLATION_POINTS),B1%T_SCALE*RP%RDT,Q_NEW)
         WRITE(*,*) SF%REFERENCE_CONE_THICKNESS, SF%LAYER_THICKNESS(1), SPYRO_TH_FACTOR, SPYRO_HF_FACTOR, B1%T_SCALE, B1%Q_SCALE, Q_NEW
      ENDIF
      ...
      SUM_MASSFLUX_LOOP: DO N=1,N_TRACKED_SPECIES
         IF (ABS(SF%MASS_FLUX(N)) > TWO_EPSILON_EB) THEN  ! Use user-specified ramp-up of mass flux
            ...
            IF (SF%REFERENCE_HEAT_FLUX > 0._EB .AND. N_REACTIONS>=1) THEN
               IF (PREDICTOR) THEN
                  IF (B1%T_IGN <=T) THEN
                     B1%M_DOT_G_PP_ACTUAL(N) = (Q_NEW-B1%Q_SCALE)/DT*SF%MASS_FLUX(N)/SPYRO_TH_FACTOR
                  ELSE
