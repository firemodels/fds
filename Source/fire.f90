...
CO_EXTINCT: DO CO_PASS = 1,2
   ...
   INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_ITERATIONS
      ...
      IF (SUPPRESSION .AND. TIME_ITER==1) CALL GET_EXTINCT(EXTINCT,ZZ_MIXED,TMP_MIXED,AIT_LOC)
...
SUBROUTINE GET_EXTINCT(EXTINCT,ZZ_MIXED_IN,TMP_MIXED,AIT_LOC)
...
IF (ANY(REACTION(:)%FAST_CHEMISTRY)) THEN
   SELECT CASE (EXTINCT_MOD)
      ...
      CASE(EXTINCTION_3) ! experimental
         EXTINCT = EXTINCT_3_AIT(TMP_MIXED,AIT_LOC)
   END SELECT
ENDIF
...
FUNCTION EXTINCT_3_AIT(TMP_MIXED,AIT_IN)
...
REACTION_LOOP: DO NR=1, N_REACTIONS
   ...
   IF ( TMP_MIXED < AIT_LOC ) EXTINCT_3_AIT(NR) = .TRUE.
