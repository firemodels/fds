...
! Build deterministic key from (vent, eddy, cycle, component, seed)
! Constants based on the SplitMix64 algorithm
K = BASE_SEED
...
K = IEOR(K,ISHFT(K,-31))
! Map top 53 bits of mixed K to [0,1)
...
SUBROUTINE FIND_WALL_INDEX(II,JJ,KK,NN,IOR,IW_FOUND,NN_FOUND,MESH_CUTS)

INTEGER, INTENT(IN) :: II,JJ,KK,NN,IOR
INTEGER, INTENT(OUT) :: IW_FOUND,NN_FOUND,MESH_CUTS
INTEGER :: IIN,JJN,KKN,NNN,IC
LOGICAL :: SEARCH
REAL(EB) :: XX,YY,ZZ

IIN=II ; JJN=JJ ; KKN=KK ; NNN=NN
MESH_CUTS = 0

SEARCH_LOOP: DO
   SEARCH = .FALSE.
   SELECT CASE(IOR)
      CASE( 1) ; IIN = IIN + 1 ; IF (IIN>MESHES(NNN)%IBAR) SEARCH = .TRUE.
      CASE(-1) ; IIN = IIN - 1 ; IF (IIN<1               ) SEARCH = .TRUE.
      CASE( 2) ; JJN = JJN + 1 ; IF (JJN>MESHES(NNN)%JBAR) SEARCH = .TRUE.
      CASE(-2) ; JJN = JJN - 1 ; IF (JJN<1               ) SEARCH = .TRUE.
      CASE( 3) ; KKN = KKN + 1 ; IF (KKN>MESHES(NNN)%KBAR) SEARCH = .TRUE.
      CASE(-3) ; KKN = KKN - 1 ; IF (KKN<1               ) SEARCH = .TRUE.
   END SELECT
   IF (SEARCH) THEN
      MESH_CUTS = MESH_CUTS + 1
      XX = MESHES(NNN)%XC(IIN)
      YY = MESHES(NNN)%YC(JJN)
      ZZ = MESHES(NNN)%ZC(KKN)
      CALL SEARCH_OTHER_MESHES(XX,YY,ZZ,NNN,IIN,JJN,KKN)
   ENDIF
   IF (NNN>0) THEN
      IC = MESHES(NNN)%CELL_INDEX(IIN,JJN,KKN)
      IF (MESHES(NNN)%CELL(IC)%SOLID) CYCLE SEARCH_LOOP
      IW_FOUND = MESHES(NNN)%CELL(IC)%WALL_INDEX(-IOR)
      NN_FOUND = NNN
      EXIT SEARCH_LOOP
   ELSE
      IW_FOUND = 0
...
END SUBROUTINE FIND_WALL_INDEX
