...
INTEGER  :: I_FUEL
REAL(EB) :: BT,FB_TIME_FCTR,GRIDCELL_FRACTION,GRIDCELL_TIME,RFIREBASE_TIME,RGRIDCELL_TIME,ROS_MAG,SMF, &
            TAN_THETA_X_MAX,TAN_THETA_Y_MAX,THETA_X,THETA_Y,TOTAL_FUEL_LOAD
...
!       GRIDCELL_TIME = SQRT(DX(IIG)**2 + DY(JJG)**2)/ROS_MAG !no accounting for spread direction
        NORMAL_FIRELINE(1) = SR_X_LS(IIG,JJG)/ROS_MAG        
        NORMAL_FIRELINE(2) = SR_Y_LS(IIG,JJG)/ROS_MAG        
        TAN_THETA_X_MAX = DY(JJG)*RDX(IIG)
        TAN_THETA_Y_MAX = DX(IIG)*RDY(JJG)

        IF(NORMAL_FIRELINE(1) == 0._EB) GRIDCELL_TIME = DY(JJG)/ROS_MAG
        IF(NORMAL_FIRELINE(2) == 0._EB) GRIDCELL_TIME = DX(IIG)/ROS_MAG

!x >= 0 quadrants
        IF(NORMAL_FIRELINE(1) > 0._EB) THEN
          THETA_X = ACOS(NORMAL_FIRELINE(1))
          IF(TAN(THETA_X) <= TAN_THETA_X_MAX) GRIDCELL_TIME = DX(IIG)/ABS(NORMAL_FIRELINE(1))/ROS_MAG
          IF(TAN(THETA_X) >  TAN_THETA_X_MAX) THEN
            THETA_Y = 0.5_EB*PI - THETA_X
            GRIDCELL_TIME = DY(JJG)/COS(THETA_Y)/ROS_MAG
          ENDIF
        ENDIF
...
!x <= 0 quadrants
        IF(NORMAL_FIRELINE(1) < 0._EB) THEN
          ...
          IF(TAN(THETA_X) >  TAN_THETA_X_MAX) THEN
            THETA_Y = 0.5_EB*PI - THETA_X
            GRIDCELL_TIME = DY(IIG)/COS(THETA_Y)/ROS_MAG
          ENDIF
        ENDIF

        RGRIDCELL_TIME = 1.0_EB/GRIDCELL_TIME
        GRIDCELL_FRACTION = MIN(1.0_EB,SF%VEG_LSET_FIREBASE_TIME*RGRIDCELL_TIME)
      ENDIF
...
! Fuel mass flux kg/s/m^2
        IF (SF%VEG_LSET_FUEL_INDEX > 0) THEN
          SMF = SF%MASS_FLUX(I_FUEL) !From Rothermel model 
        ELSE
          SMF = (1.0_EB-SF%VEG_LSET_CHAR_FRACTION)*TOTAL_FUEL_LOAD*RFIREBASE_TIME
          SF%MASS_FLUX(I_FUEL) = SMF
        ENDIF

! Case with grid cell > fire depth      
...
!       Fire has left cell
          IF (BT > GRIDCELL_TIME + SF%VEG_LSET_FIREBASE_TIME) LSET_FIRE = .FALSE.
          BURN_TIME_LS(IIG,JJG) = BURN_TIME_LS(IIG,JJG) + DT
          ONE_D%M_DOT_G_PP_ACTUAL(I_FUEL) = SMF*FB_TIME_FCTR
          ONE_D%M_DOT_G_PP_ADJUST(I_FUEL) = ONE_D%M_DOT_G_PP_ACTUAL(I_FUEL)
!         ONE_D%M_DOT_G_PP_ADJUST(I_FUEL) = ONE_D%M_DOT_G_PP_ACTUAL(I_FUEL)*ONE_D%AREA_ADJUST
        ENDIF
...
IF (VEG_LEVEL_SET_COUPLED) THEN
   ...
   SF%VEG_LSET_FIREBASE_TIME = 756._EB/SF%VEG_LSET_SIGMA
