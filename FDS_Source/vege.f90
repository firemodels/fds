...
REAL(EB) :: LX,SR_X_MAX,SR_Y_MAX,SR_MAX,T_FINAL,U_AMBIENT,UMAG,UMAX_LS, V_AMBIENT,VMAX_LS,XMAX_LS,XMIN_LS, &
            YMIN_LS,YMAX_LS
...
ROS_FLANK        = 0.4_EB    
ROS_BACKU        = 0.1_EB   !wind  dependent back fire ROS
...
!  IF(COS_THETA_WIND >= 0._EB) COS_THETA_WIND_H = COS_THETA_WIND
!  IF(COS_THETA_WIND <= 0._EB) COS_THETA_WIND_B = COS_THETA_WIND
...
!  IF(COS_THETA_SLOPE >= 0._EB) COS_THETA_SLOPE_H = COS_THETA_SLOPE
!  IF(COS_THETA_SLOPE <= 0._EB) COS_THETA_SLOPE_B = COS_THETA_SLOPE
...
! Spread against the wind and upslope
   IF(COS_THETA_WIND <  0._EB .AND. COS_THETA_SLOPE >= 0._EB) THEN
    IF(DEGREES_SLOPE >= 5._EB .AND. DEGREES_SLOPE < 10._EB)  ROS_BACKS = -0.33_EB*ROS_BACKU
    IF(DEGREES_SLOPE >= 10._EB .AND. DEGREES_SLOPE < 20._EB) ROS_BACKS =         -ROS_BACKU
    IF(DEGREES_SLOPE >= 20._EB)                              ROS_BACKS = -3.0_EB*ROS_BACKU
    MAG_SR = ROS_FLANK(I,J)*(1._EB + COS_THETA_WIND*COS_THETA_SLOPE) + &
             (ROS_FLANK(I,J) - ROS_BACKU)*COS_THETA_WIND + &
...
! Spread against the wind and downslope
   IF(COS_THETA_WIND <  0._EB .AND. COS_THETA_SLOPE < 0._EB) THEN
    IF(DEGREES_SLOPE >= 5._EB .AND. DEGREES_SLOPE < 10._EB)  ROS_BACKS = 0.33_EB*ROS_BACKU
    IF(DEGREES_SLOPE >= 10._EB .AND. DEGREES_SLOPE < 20._EB) ROS_BACKS = 0.50_EB*ROS_BACKU
    IF(DEGREES_SLOPE >= 20._EB)                              ROS_BACKS = 0.75_EB*ROS_BACKU
    MAG_SR = ROS_FLANK(I,J)*(1._EB + COS_THETA_WIND*COS_THETA_SLOPE) + &
             (ROS_FLANK(I,J) - ROS_BACKU)*COS_THETA_WIND + &
             (ROS_FLANK(I,J) - ROS_BACKS)*COS_THETA_SLOPE  !magnitude of spread rate
   ENDIF
...
!   F_SOUTH = 0.5*( PHI0_LS(I,J) + PHI_MAX_LS )
   ...
   MAG_SR = 0.0_EB
   IF(COS_THETA_WIND_H >= 0._EB .AND. COS_THETA_SLOPE_H >= 0._EB) THEN
    MAG_SR = ROS_FLANK(I,J)*(1._EB + 3._EB*COS_THETA_WIND_H*COS_THETA_SLOPE_H) + &
             (ROS_HEAD(I,J) - ROS_FLANK(I,J))*COS_THETA_WIND_H + &
             (ROS_HEADS     - ROS_FLANK(I,J))*COS_THETA_SLOPE_H  !magnitude of spread rate
   ENDIF

   IF(COS_THETA_WIND_H >= 0._EB .AND. COS_THETA_SLOPE_B < 0._EB) THEN
    MAG_SR = ROS_FLANK(I,J)*(1._EB - 3._EB*COS_THETA_WIND_H*COS_THETA_SLOPE_B) + &
             (ROS_HEAD(I,J) - ROS_FLANK(I,J))*COS_THETA_WIND_H + &
             (ROS_HEADS     - ROS_FLANK(I,J))*COS_THETA_SLOPE_B  !magnitude of spread rate
   ENDIF
!  MAG_SR = ROS_FLANK(I,J) + ROS_HEAD(I,J)*COS_THETA_WIND**1.5 !magnitude of spread rate
!  MAG_SR = ROS_FLANK(I,J) + ROS_HEAD(I,J)*MAG_U*COS_THETA_WIND**1.5 !magnitude of spread rate
!  MAG_SR = R_FLANK + ROS_HEAD*WIND_DOT_NORMAL_FIRELINE !magnitude of spread rate
