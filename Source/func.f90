...
USE GLOBAL_CONSTANTS, ONLY: THERMALLY_THICK,TMPM,SIGMA,TMPA4,RHOA,N_TRACKED_SPECIES,N_SURFACE_DENSITY_SPECIES
...
SUBROUTINE PACK_WALL(NM,WALL_INDEX,SURF_INDEX,UNPACK_IT,N_INTEGERS,N_REALS,N_LOGICALS)
...
IF (UNPACK_IT .AND. .NOT.COUNT_ONLY) CALL ALLOCATE_STORAGE(NM,WALL_INDEX=WALL_INDEX,SURF_INDEX=SURF_INDEX)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%SURF_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%BACK_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%BACK_MESH,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%BOUNDARY_TYPE,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%SURF_INDEX_ORIG,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%OBST_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%PRESSURE_BC_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%VENT_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%JD11_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%JD12_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%JD21_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%JD22_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,1),WC%CUT_FACE_INDEX,UNPACK_IT)

! Pack or unpack the real values

RC = 0  ! Real Counter

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),WC%DUNDT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),WC%Q_LEAK,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),WC%V_DEP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,1),WC%VEL_ERR_NEW,UNPACK_IT)

! Pack or unpack the logical values

LC = 0  ! Logical Counter

LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,1),WC%THIN(1),UNPACK_IT)
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,1),WC%THIN(2),UNPACK_IT)
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,1),WC%THIN(3),UNPACK_IT)
...
IF (SF%INCLUDE_BOUNDARY_THR_D_TYPE) CALL PACK_BOUNDARY_THR_D(NM,IC,RC,OS,SF,1,OD_INDEX,TD_INDEX,UNPACK_IT,COUNT_ONLY)
