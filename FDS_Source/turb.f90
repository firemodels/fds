...
PUBLIC :: NS_ANALYTICAL_SOLUTION, INIT_TURB_ARRAYS, VARDEN_DYNSMAG, &
          ...
          SURFACE_HEAT_FLUX_MODEL, MEASURE_SCALAR_RESOLUTION, WAVELET_ERROR_MEASURE, &
...
REAL(EB) FUNCTION MEASURE_SCALAR_RESOLUTION(II,JJ,KK,SPEC_INDEX,NM)
USE PHYSICAL_FUNCTIONS, ONLY: GET_MASS_FRACTION
IMPLICIT NONE

INTEGER, INTENT(IN) :: II,JJ,KK,SPEC_INDEX,NM
REAL(EB) :: T_TOT,T_LES,T_SGS,ZZ(3,3,3),ZZ_HAT(3,3,3),YY_GET(1:N_SPECIES)
...
SPEC_INDEX_IF: IF (SPEC_INDEX<0) THEN
   DO K=KK-1,KK+1
      ...
      DO J=JJ-1,JJ+1
         ...
         DO I=II-1,II+1
            ...
            YY_GET(:) = MESHES(NM)%YY(LL,MM,NN,:)
            CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,ZZ(I-II+2,J-JJ+2,K-KK+2))
         ENDDO
      ENDDO
   ENDDO
ELSEIF (SPEC_INDEX>0) THEN SPEC_INDEX_IF
   ZZ = MESHES(NM)%YY(II-1:II+1,JJ-1:JJ+1,KK-1:KK+1,SPEC_INDEX)
ENDIF SPEC_INDEX_IF
...
REAL(EB) FUNCTION WAVELET_ERROR_MEASURE(II,JJ,KK,SPEC_INDEX,NM)
USE PHYSICAL_FUNCTIONS, ONLY: GET_MASS_FRACTION
IMPLICIT NONE

INTEGER, INTENT(IN) :: II,JJ,KK,SPEC_INDEX,NM
REAL(EB) :: SS(4),YY_GET(1:N_SPECIES)

! wavelet error measure
WAVELET_ERROR_MEASURE = 0._EB

SPEC_ID_IF: IF (SPEC_INDEX<0) THEN

   YY_GET(:) = MESHES(NM)%YY(MAX(0,II-2),JJ,KK,:);               CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(1))
   YY_GET(:) = MESHES(NM)%YY(MAX(0,II-1),JJ,KK,:);               CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(2))
   YY_GET(:) = MESHES(NM)%YY(II,JJ,KK,:);                        CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(3))
   YY_GET(:) = MESHES(NM)%YY(MIN(MESHES(NM)%IBP1,II+1),JJ,KK,:); CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(4))
   WAVELET_ERROR_MEASURE = WAVELET_ERROR(SS)
   
   IF (.NOT.TWO_D) THEN
      YY_GET(:) = MESHES(NM)%YY(II,MAX(0,JJ-2),KK,:);               CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(1))
      YY_GET(:) = MESHES(NM)%YY(II,MAX(0,JJ-1),KK,:);               CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(2))
      YY_GET(:) = MESHES(NM)%YY(II,JJ,KK,:);                        CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(3))
      YY_GET(:) = MESHES(NM)%YY(II,MIN(MESHES(NM)%JBP1,JJ+1),KK,:); CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(4))
      WAVELET_ERROR_MEASURE = WAVELET_ERROR(SS)
   ENDIF
   
   YY_GET(:) = MESHES(NM)%YY(II,JJ,MAX(0,KK-2),:);               CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(1))
   YY_GET(:) = MESHES(NM)%YY(II,JJ,MAX(0,KK-1),:);               CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(2))
   YY_GET(:) = MESHES(NM)%YY(II,JJ,KK,:);                        CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(3))
   YY_GET(:) = MESHES(NM)%YY(II,JJ,MIN(MESHES(NM)%KBP1,KK+1),:); CALL GET_MASS_FRACTION(YY_GET,-SPEC_INDEX,SS(4))
   WAVELET_ERROR_MEASURE = WAVELET_ERROR(SS)
   
ELSEIF (SPEC_INDEX>0) THEN SPEC_ID_IF

   SS(1) = MESHES(NM)%YY(MAX(0,II-2),JJ,KK,SPEC_INDEX)
   SS(2) = MESHES(NM)%YY(MAX(0,II-1),JJ,KK,SPEC_INDEX)
   SS(3) = MESHES(NM)%YY(II,JJ,KK,SPEC_INDEX)
   SS(4) = MESHES(NM)%YY(MIN(MESHES(NM)%IBP1,II+1),JJ,KK,SPEC_INDEX)
   WAVELET_ERROR_MEASURE = WAVELET_ERROR(SS)

   IF (.NOT.TWO_D) THEN
      SS(1) = MESHES(NM)%YY(II,MAX(0,JJ-2),KK,SPEC_INDEX)
      SS(2) = MESHES(NM)%YY(II,MAX(0,JJ-1),KK,SPEC_INDEX)
      SS(3) = MESHES(NM)%YY(II,JJ,KK,SPEC_INDEX)
      SS(4) = MESHES(NM)%YY(II,MIN(MESHES(NM)%JBP1,JJ+1),KK,SPEC_INDEX)
      WAVELET_ERROR_MEASURE = MAX(WAVELET_ERROR_MEASURE,WAVELET_ERROR(SS))
   ENDIF

   SS(1) = MESHES(NM)%YY(II,JJ,MAX(0,KK-2),SPEC_INDEX)
   SS(2) = MESHES(NM)%YY(II,JJ,MAX(0,KK-1),SPEC_INDEX)
   SS(3) = MESHES(NM)%YY(II,JJ,KK,SPEC_INDEX)
   SS(4) = MESHES(NM)%YY(II,JJ,MIN(MESHES(NM)%KBP1,KK+1),SPEC_INDEX)
   WAVELET_ERROR_MEASURE = MAX(WAVELET_ERROR_MEASURE,WAVELET_ERROR(SS))
