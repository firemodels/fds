...
IF (N_DEVC > 0) THEN
   ...
   DO N=1,N_DEVC
      ...
      IF (ABS(DV%T_CHANGE) < 1._EB)        WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.5,A)') N,TRIM(DV%ID),DV%CURRENT_STATE,DV%T_CHANGE,' s'
      IF (ABS(DV%T_CHANGE) < 100._EB)      WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.3,A)') N,TRIM(DV%ID),DV%CURRENT_STATE,DV%T_CHANGE,' s'
      IF (ABS(DV%T_CHANGE) < 10000._EB)    WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.1,A)') N,TRIM(DV%ID),DV%CURRENT_STATE,DV%T_CHANGE,' s'
      IF (ABS(DV%T_CHANGE) < 1000000._EB)  WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.0,A)') N,TRIM(DV%ID),DV%CURRENT_STATE,DV%T_CHANGE,' s'
      IF (ABS(DV%T_CHANGE) >= 1000000._EB) WRITE(LU_OUTPUT,'(I4,2X,A,L1,A)')      N,TRIM(DV%ID),DV%CURRENT_STATE,' No Activation'
   ENDDO
ENDIF
...
IF (N_CTRL > 0) THEN
   ...
   DO N=1,N_CTRL
      CF => CONTROL(N)
      IF (ABS(CF%T_CHANGE) < 1._EB)        WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.5,A)') N,TRIM(CF%ID),CF%CURRENT_STATE,CF%T_CHANGE,' s'
      IF (ABS(CF%T_CHANGE) < 100._EB)      WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.3,A)') N,TRIM(CF%ID),CF%CURRENT_STATE,CF%T_CHANGE,' s'
      IF (ABS(CF%T_CHANGE) < 10000._EB)    WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.1,A)') N,TRIM(CF%ID),CF%CURRENT_STATE,CF%T_CHANGE,' s'
      IF (ABS(CF%T_CHANGE) < 1000000._EB)  WRITE(LU_OUTPUT,'(I4,2X,A,L1,F8.0,A)') N,TRIM(CF%ID),CF%CURRENT_STATE,CF%T_CHANGE,' s'
      IF (ABS(CF%T_CHANGE) >= 1000000._EB) WRITE(LU_OUTPUT,'(I4,2X,A,L1,A)')      N,TRIM(CF%ID),CF%CURRENT_STATE,' No Activation'
   ENDDO
ENDIF
