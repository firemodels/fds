...
INTEGER, INTENT(IN) :: NM
INTEGER :: I,J,K,IC,N
REAL(EB) :: VC,Q_DOT_GEOM,HTC,ZZ_GET(0:N_TRACKED_SPECIES),CP,DTMP
REAL(EB), POINTER, DIMENSION(:,:,:) :: RHOP=>NULL()
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP=>NULL()
   ...
   CUTCELL_LOOP: DO
      ...
      SELECT CASE(SF%THERMAL_BC_INDEX)
         ...
         CASE(CONVECTIVE_FLUX_BC)
            ...
            HTC = HEAT_TRANSFER_COEFFICIENT(DTMP,SF%H_FIXED,SF%GEOMETRY,SF%CONV_LENGTH,SF%HEAT_TRANSFER_MODEL,&
                                            FACE_INDEX=N,CUTCELL_INDEX=IC)
            FACE%TMP_F = TMP(I,J,K) + SF%CONVECTIVE_HEAT_FLUX/HTC
...
IF (DNS) THEN
   IF (PRESENT(WALL_INDEX)) HEAT_TRANSFER_COEFFICIENT = 2._EB*WC%KW*WC%RDN
   IF (PRESENT(FACE_INDEX)) HEAT_TRANSFER_COEFFICIENT = 2._EB*FC%KW*FC%RDN
   RETURN
ENDIF
