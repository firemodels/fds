...
IF (WRITE_VTK) THEN
   ...
   IF (T>=SM3D_VTK_CLOCK(SM3D_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
      CALL DUMP_SMOKE3D_VTKHDF(T,DT,NM,FAKEWRITE)
      IF (.NOT.FAKEWRITE) THEN
...
ENDIF
IF (N_RADF_MAX>0) THEN
   ALLOCATE(FN_RADF(N_RADF_MAX,NMESHES))
   ALLOCATE(LU_RADF(N_RADF_MAX,NMESHES))
ENDIF
...
! VTK Files
ALLOCATE(FN_SL3D_VTK(MESHES(1)%N_UNIQUE_SLCF,NMESHES+1))
ALLOCATE(FN_SMOKE3D_VTK(NMESHES+1))
ALLOCATE(FN_BNDF_VTK(NMESHES+1))
ALLOCATE(FN_OBST_VTK(NMESHES+1))
ALLOCATE(FN_GEOM_VTK(N_GEOMETRY))
...
IF (PARAVIEW_PROJECT) THEN
   LU_PARAVIEW = GET_FILE_NUMBER()
   FN_PARAVIEW = TRIM(CHID)//'_paraview.py'
ENDIF
...
WRITE_XYZ_FILE: IF (WRITE_XYZ.AND.WRITE_SMV) THEN
...
IF (TERRAIN_CASE.AND.WRITE_SMV) THEN
...
SMOKE3D_INITIALIZATION: IF (SMOKE3D.AND.WRITE_SMV) THEN
   ...
   DO N=1,N_SMOKE3D
      ...
      IF (.NOT. APPEND) THEN
         ...
         IF (SMOKE3D_VERSION==2) THEN
            SELECT CASE(SMOKE3D_FILE(N)%DISPLAY_TYPE)
               CASE('GAS')         ; WRITE(LU_SMOKE3D(N,NM)) 0._FB,1._FB
               CASE('FIRE')        ; WRITE(LU_SMOKE3D(N,NM)) 0._FB,REAL(HRRPUV_MAX_SMV,FB)
               CASE('TEMPERATURE') ; WRITE(LU_SMOKE3D(N,NM)) REAL(TEMP_MIN_SMV,FB),REAL(TEMP_MAX_SMV,FB)
            END SELECT
         ENDIF
         ...
         M%N_STRINGS = M%N_STRINGS + 1
         IF (SMOKE3D_VERSION==2) THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,I6,F11.3)') 'SMOKH3D',NM,SMOKE3D_FILE(N)%MASS_EXTINCTION_COEFFICIENT
         ELSE
...
IF_BOUNDARY_FILES: IF ((N_BNDF>0 .AND. M%BNDF_DUMP).OR.(WRITE_VTK)) THEN
...
PARTICLE_IF: IF (PARTICLE_FILE.AND.WRITE_SMV) THEN
...
INTEGER :: FACE_INDEX  ! NM is host-associated from WRITE_SMOKEVIEW_FILE so DUMMY_VENTS/REALLOCATE_PATCH see the current mesh
! Create EXTERIOR_PATCHes with which Smokeview colors, textures, or contours exterior mesh boundaries.
...
END SUBROUTINE ADD_EXTERIOR_VENTS
...
WRITE(LU_CORE(NM)) DT_BNDF,DT_CPU,DT_CTRL,DT_DEVC,DT_FLUSH,DT_GEOM,DT_HRR,DT_ISOF,DT_MASS,DT_PART,DT_PL3D,DT_PROF,DT_RADF,&
                   DT_SLCF,DT_SL3D,DT_SMOKE3D,DT_UVW,DT_VTK
...
IF (N_BNDF>0 .AND. BNDF_DUMP .OR. WRITE_VTK) THEN
...
IF (DT_VTK_SPECIFIED    > 0._EB) DT_VTK     = DT_VTK_SPECIFIED
...
SUBROUTINE DUMP_PART_VTKHDF(T,NM,IFRMT,FAKEWRITE)
...
INTEGER(IB32), DIMENSION(1:NMESHES) :: NPOINTS
...
INTEGER(HID_T) :: DSET_ID_PTS, DSET_ID_NPTS, DSET_ID    ! Dataset identifiers
INTEGER :: NMNM, N_WRITTEN, ERROR
INTEGER :: NPOINTS_ACCUM, NPOINTS_START, NPOINTS_TOTAL
...
ENDDO
DO NMNM=1,NMESHES
   MESHES_PER_PROCESS(PROCESS(NMNM)+1) = MESHES_PER_PROCESS(PROCESS(NMNM)+1) + 1
ENDDO
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   WRITE(FILENAME,'(A,A,A,A,A,I8.8,I2.2,A)') "",TRIM(VTK_DIR)//TRIM(CHID),'_PART_',TRIM(LPC%ID),'_',ITM,ITM1,'.vtkhdf'

   ! Read information from other meshes
   CALL OPEN_PART_VTKHDF(FILENAME, FILE_ID, PLIST_ID, GROUP_ID1,GROUP_ID2,GROUP_ID3,GROUP_ID4)
   ...
   NPOINTS_TOTAL = MAX(1,MAXVAL(NPOINTS))

   IF (FAKEWRITE) THEN

      ! Write point data to file
      START2(1:2) = INT((/3, NPOINTS_TOTAL/),HSIZE_T)
      EXTENT2(1:2) = INT((/3, SUM(NPOINTS)/),HSIZE_T)
      ...
      EXTENT2(1:2) = INT((/0, 0/),HSIZE_T)
      FDATA_OUT3 = 0
      CALL PARALLEL_WRITE_F32(2, EXTENT2, DSET_ID_PTS, PLIST_ID, START2, EXTENT2, FDATA_OUT3)
      CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)

      ! Write tag to file
      START1 = NPOINTS_TOTAL
      EXTENT1 = SUM(NPOINTS)
      CALL PARALLEL_INIT_I32(GROUP_ID4, TRIM("TAG"), CRP_LIST, 1, START1, EXTENT1, DSET_ID_PTS, PLIST_ID) ! Points
      START1 = 0
      EXTENT1 = 0
      IDATA_OUT1 = 0
      CALL PARALLEL_WRITE_I32(1, EXTENT1, DSET_ID_PTS, PLIST_ID, START1, EXTENT1, IDATA_OUT1)
      CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ...
      DO NN=1,LPC%N_QUANTITIES
         ...
         CALL PARALLEL_INIT_F32(GROUP_ID4, TRIM(LPC%SMOKEVIEW_LABEL(NN)), CRP_LIST, 1,&
            START1, EXTENT1, DSET_ID_PTS, PLIST_ID) ! Points
         START1 = 0
         EXTENT1 = 0
         FDATA_OUT3 = 0
         CALL PARALLEL_WRITE_F32(1, EXTENT1, DSET_ID_PTS, PLIST_ID, START1, EXTENT1, FDATA_OUT3(1))
         CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ENDDO
      ...
      CYCLE LAGRANGIAN_PARTICLE_CLASS_LOOP
   ENDIF
   ...
   MESH_LOOP_HDF: DO NMNM=1,NMESHES
      ...
      ! Allocate some temporary 4 byte arrays just to hold the data that is to be dumped to the file
      ALLOCATE(VERTICES(3,NPLIM))
      ALLOCATE(TA(NPLIM),STAT=IZERO)                  ; CALL ChkMemErr('DUMP','TA',IZERO)
      ALLOCATE(QP(NPLIM,LPC%N_QUANTITIES),STAT=IZERO) ; CALL ChkMemErr('DUMP','QP',IZERO)
      IF (IFRMT.EQ.1) THEN ; ALLOCATE(COLORS(3,NPLIM),STAT=IZERO) ; CALL ChkMemErr('DUMP','COLOR',IZERO) ; ENDIF
      ! Load particle data into single precision arrays
      ...
      LOAD_LOOP: DO IP=1,NLP
         ...
         BC=>BOUNDARY_COORD(LP%BC_INDEX)
         IF (.NOT.LP%SHOW .OR. LP%CLASS_INDEX/=N) CYCLE LOAD_LOOP
         !IF (LP%CLASS_INDEX/=N) CYCLE LOAD_LOOP
         NPP = NPP + 1
         IF (NPP > NPLIM) EXIT LOAD_LOOP
         TA(NPP) = LP%TAG
         VERTICES(:,NPP) = REAL((/BC%X,BC%Y,BC%Z/),FB)
         DO NN=1,LPC%N_QUANTITIES
      ...
      EXTENT2(1:2) = INT((/3, NPOINTS(NM)/),HSIZE_T)
      CALL PARALLEL_WRITE_F32(2, EXTENT2, DSET_ID_PTS,&
         PLIST_ID, START2, EXTENT2, VERTICES)
      CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ...
      CALL PARALLEL_INIT_I32(GROUP_ID4, TRIM("TAG"), CRP_LIST, 1,&
         START1, EXTENT1, DSET_ID_PTS, PLIST_ID) ! Points
      START1 = NPOINTS_START
      EXTENT1 = NPOINTS(NM)
      CALL PARALLEL_WRITE_I32(1, EXTENT1, DSET_ID_PTS, PLIST_ID, START1, EXTENT1, TA)
      CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ...
      EXTENT2(1:2) = INT((/3, NPOINTS(NM)/),HSIZE_T)
      CALL PARALLEL_WRITE_F32(2, EXTENT2, DSET_ID_PTS,&
         PLIST_ID, START2, EXTENT2, COLORS)
      CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ...
      DO NN=1,LPC%N_QUANTITIES
         ...
         START1 = NPOINTS_START
         EXTENT1 = NPOINTS(NM)
         CALL PARALLEL_WRITE_F32(1, EXTENT1, DSET_ID_PTS,&
            PLIST_ID, START1, EXTENT1, QP(1:NPP,NN))
         CALL H5DCLOSE_F(DSET_ID_PTS, ERROR)
      ENDDO
...
SUBROUTINE DUMP_SMOKE3D_VTKHDF(T,DT,NM,FAKEWRITE)
...
SUBROUTINE DUMP_SLCF_VTK(T,DT,NM,IFRMT,FAKEWRITE)
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: B,S,QUANTITY
REAL(FB) :: STIME
LOGICAL :: PLOT3D,SLCF3D,VTK3D,SL3D
LOGICAL :: AGL_TERRAIN_SLICE,CC_CELL_CENTERED,CC_INTERP2FACES
REAL(FB), ALLOCATABLE, DIMENSION(:) :: QQ_PACK
TYPE (MESH_TYPE), POINTER :: M2
...
IF(FAKEWRITE) THEN
   UNIQUE_LOOPF: DO IQ=1,MESHES(1)%N_UNIQUE_SLCF
      ...
      IF (.NOT.MESHES(1)%UNIQUE_SLICE_IN_DOMAIN(IQ)) CYCLE UNIQUE_LOOPF
      ...
      NCELLS=0
      NCONNECTIONS=0
      ALL_SLCF_LOOPF: DO II=1,MESHES(1)%N_SLCF_VTK
         IF(TRIM(MESHES(1)%ALL_SLICE_NAMES(II))/=TRIM(MESHES(1)%UNIQUE_SLICE_NAMES(IQ))) CYCLE ALL_SLCF_LOOPF
         QTY = MESHES(1)%ALL_SLICE_QUANTITIES(II)
         ! If this unique slice is not present in this mesh, parallel write no data
         ALLOCATE(QQ_PACK(0))
         CALL WRITE_VTKHDF_SLICE_DATA_FILE(FILENAME,QTY,NM,NCELLS,NPOINTS,QQ_PACK)
         DEALLOCATE(QQ_PACK)
      ENDDO ALL_SLCF_LOOPF
   ENDDO UNIQUE_LOOPF
   RETURN
ENDIF
...
UNIQUE_LOOPA: DO IQ=1,MESHES(1)%N_UNIQUE_SLCF
   IF (.NOT.MESHES(1)%UNIQUE_SLICE_IN_DOMAIN(IQ)) CYCLE UNIQUE_LOOPA
   SL3D = MESHES(1)%UNIQUE_SLICE_IS_SL3D(IQ)
...
SUBROUTINE DUMP_BNDF_VTKHDF(T,DT,NM,FAKEWRITE)
INTEGER(HID_T) :: FILE_ID, PLIST_ID, CRP_LIST       ! Identifiers
INTEGER(HID_T) :: GROUP_ID1,GROUP_ID2,GROUP_ID3,GROUP_ID4 ! Group identifier
...
LOGICAL, INTENT(IN) :: FAKEWRITE
REAL(FB) :: STIME
INTEGER :: NF,IND,IC,IW,IP,NC,I1,I2,J1,J2,K1,K2,ITM,ITM1
INTEGER :: IFACT,NFACES_CUTCELLS,NVERTS_CUTCELLS,NVERTS,NFACES
INTEGER, INTENT(IN) :: NM
TYPE(PATCH_TYPE), POINTER :: PA
INTEGER :: NMNM, NMNM2, NM1, NM2, N_WRITTEN, ERROR, PA_NCELLS, PA_NPOINTS
INTEGER(IB32), DIMENSION(1:2*NMESHES) :: NCELLS, NPOINTS
INTEGER, DIMENSION(1:N_MPI_PROCESSES) :: MESHES_PER_PROCESS
INTEGER :: NCELLS_ACCUM, NCELLS_TOTAL, NCELLS_START
INTEGER :: NPOINTS_ACCUM, NPOINTS_TOTAL, NPOINTS_START
...
MESH_LOOP_HDF: DO NMNM=1,NMESHES
   ...
   DATA_FILE_LOOP: DO NF=1,N_BNDF
      ...
      IF ((MESHES(NM)%N_PATCH>0).AND.(.NOT.FAKEWRITE)) THEN
         ...
         IF (BF%CELL_CENTERED) THEN
            ...
            CALL PARALLEL_INIT_F32(GROUP_ID2, BF%SMOKEVIEW_LABEL(1:30), CRP_LIST, 1,&
               START1, EXTENT1, DSET_ID, PLIST_ID) ! Open previously initialized cell centered for writing
            START1=NCELLS_START
            EXTENT1=NCELLS(NM1)
            CALL PARALLEL_WRITE_F32(1, EXTENT1, DSET_ID, PLIST_ID, START1, EXTENT1, ALL_DATA)
         ELSE
            START1=0
            EXTENT1=0
            CALL PARALLEL_INIT_F32(GROUP_ID4, BF%SMOKEVIEW_LABEL(1:30), CRP_LIST, 1,&
               START1, EXTENT1, DSET_ID, PLIST_ID) ! Open previously initialized nodal for writing
            START1=NPOINTS_START
            EXTENT1=NPOINTS(NM1)
            CALL PARALLEL_WRITE_F32(1, EXTENT1, DSET_ID, PLIST_ID, START1, EXTENT1, ALL_DATA)
         ENDIF
      ...
      IF ((MESHES(NM)%N_INTERNAL_CFACE_CELLS>0).AND.(.NOT.FAKEWRITE)) THEN
         CALL GET_GEOMSIZES('INBOUND_FACES',0,0,0,0,0,0,NVERTS,NVERTS_CUTCELLS,NFACES,NFACES_CUTCELLS)
         IF (NVERTS>0 .AND. NFACES>0) THEN
