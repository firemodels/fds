...
Julio Cesar                             & Federal University of Rio de Janeiro, Brazil                              \\ \hline
...
In some cases with extremely fast droplets or particles, however, the stability of the overall flow behaviour may require setting an additional parameter
...
In general only one species for a given {\ct ID} can be defined; however, you may wish to model multiple inlet streams of a species and be able to identify how well the streams are mixing.  This can be done by defining a lumped species, see Chapter~\ref{lumped}, with a single component.  For example, the lines:
...
Alternatively, we can define a ``lumped species'' that represents the air mixture and save on CPU time as FDS will the three trasnport equations would no longer be needed as the ``lumped'' air becomes the background species.
...
The ambient mass fractions for the consituents of air are specified on {\ct MISC} using the inputs:
\begin{description}
...
\noindent
Note that the stoichiometry is specfied using the inputs {\ct NU} and {\ct SPEC\_ID\_NU}.  All species listed for {\ct NU} need to be tracked species (i.e. a species with {\ct LUMPED\_COMPONENT\_ONLY} cannot be specified for {\ct NU}). \footnote{There are several parameters on the {\ct REAC} line that control the specification of the stoichiometry.
\begin{description}
...
\end{multline}
and the reaction for wood with a soot yield of $y_s$ = 0.01 and a CO yield of $y_{\mathrm{CO}}$= 0.004 \cite{SFPE:Tewarson} is:
...
\noindent
The array {\ct SPEC\_ID\_NU} contains the list of tracked species participating in the reaction as a product or reactant.  Note that a primitive species with {\ct LUMPED\_COMPONONENT\_ONLY=.TRUE.} cannot be used in a reaction since it is not tracked separately.  The array {\ct SPEC\_ID\_N\_S} contains the list of species with the exponents, {\ct N\_S}, in Eq.~(\ref{eq:reac_rate}).  This array must contain only primitive species, i.e. no species mixtures. The array of stoichiometric coefficients, {\ct NU}, can be taken directly from Eq.~(\ref{eq:propane_1step}). If we extend Eq.~(\ref{eq:propane_1step}) to include reactions that account for carbon monoxide production, we obtain:
...
{\ct MIE\_MAXIMUM\_DIAMETER}. Also, the size of the Mie cofficient tables can be specified using {\ct MIE\_NDG} parameter.
...
{\ct LUMPED\_COMPONENT\_ONLY}       & Logical     & Section~\ref{info:SMIX}                 &                   & {\ct .FALSE.} \\ \hline
