...
MAIN_LOOP: DO
   ...
   IF ( ((T>=RSRT_CLOCK(RSRT_COUNTER(1)) .OR. STOP_STATUS==USER_STOP) .AND.  (T>=T_END .OR. ALL(RADIATION_COMPLETED))) &
      .OR. STOP_STATUS==CTRL_STOP ) THEN
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
...
IF (STOP_STATUS==NO_STOP .OR. STOP_STATUS==USER_STOP .OR. STOP_STATUS==CTRL_STOP) CALL DUMP_TIMERS
