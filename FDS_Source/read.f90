...
CHARACTER(30) :: ASSUMED_GAS_TEMPERATURE_RAMP,EXTINCTION_MODEL,RAMP_GX,RAMP_GY,RAMP_GZ,TURBULENCE_MODEL
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,ASSUMED_GAS_TEMPERATURE_RAMP,&
                ...
                COMPUTE_VISCOSITY_TWICE,CONSTANT_SPECIFIC_HEAT_RATIO,C_DEARDORFF,C_SMAGORINSKY,C_VREMAN,DNS,ENTHALPY_TRANSPORT,&
                ...
                IMMERSED_BOUNDARY_METHOD,INITIAL_UNMIXED_FRACTION,ISOTROPIC_PARTICLE_FORCE,ISOTHERMAL,&
...
G_F_Z = 0._EB
   ...
   ! Check if each  SPEC_ID is listed in data.f90 and write out a warning if not.

DO N=1,N_SPECIES
    LISTED=.TRUE.
    SS => SPECIES(N)
    CALL CALC_GAS_PROPS(1,N,D_TMP(N),MU_TMP(N),K_TMP(N),CP_TMP(N),H_TMP(N),SS%ISFUEL,G_F_TMP(N),LISTED)
    IF(.NOT.LISTED .AND. .NOT. CONSTANT_SPECIFIC_HEAT_RATIO) WRITE(LU_ERR,*) "Gas phase species ",TRIM(SS%ID)," not found."
ENDDO
...
TABLE_LOOP: DO J=1,5000
   ...
   DO N=1,N_SPECIES
      SS => SPECIES(N)
      CALL CALC_GAS_PROPS(J,N,D_TMP(N),MU_TMP(N),K_TMP(N),CP_TMP(N),H_TMP(N),SS%ISFUEL,G_F_TMP(N),LISTED)
      IF (SS%RAMP_CP_INDEX>0) THEN
