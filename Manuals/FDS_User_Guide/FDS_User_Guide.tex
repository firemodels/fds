...
First inntall the FDS-SMV package on all of the machines you plan to use. The new version of FDS will overwrite all existing versions unless they are moved or renamed.
...
\end{lstlisting}
where 4 indicates that 4 FDS processes will be created. Note that as a default each FDS process will use 4 OpenMP threads. It is advisable to set the environmental variable {\ct OMP\_NUM\_THREADS} so that the total number of threads ({\ct n $\times$ OMP\_NUM\_THREADS}) is less than or equal to the total number of cores on the machine 
