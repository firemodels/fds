...
See Section~\ref{info:adiabatic} for more information.
...
Note that the {\ct MULT}iplication functionality works for {\ct MESH} and {\ct INIT} lines. For a {\ct MESH}, it only applies to the
...
example (box\_burn\_away1), the released gas is fuel (mixture fraction)
...
\section{Special Topic: HVAC Systems: The {\ct HVAC} Namelist Group (\ref{hvacinputs})}
\label{info:HVAC}

There are occasions where simply defining fixed flow and fixed species boundary conditions is not sufficient to model the behavior of an HVAC system.  If the ability to transport heat and comubstion products through a duct network or the ability to fully account for the pressurization of a compartment due to a fire on the flows in a duct network is important, one can make use of a coupled HVAC network solver.  The solver computes the flows through a duct network described as a mapping of duct segments and nodes where a node is either the joining of two or more ducts or where a duct segment connects to the FDS computational domain.  The current HVAC solver does not allow for mass storage in the duct network (i.e. what goes in during a timestep, goes out during a timestep).  HVAC components such as fans and binary dampers (fully open or fully closed) can be included in the HVAC network and coupled to the FDS control function capability.  The
user can select from three fan models.

The HVAC solver is invoked if there is an {\ct HVAC} namelist group present in the input file.  An HVAC network is defined by providing inputs for the ducts, duct nodes, and any fans or dampers present in the system.  Additionally the user must define the locations where the HVAC network joins the computational domain.  The basic syntax for an {\ct HVAC} component is:
...
\begin{description}
\item[{\ct TYPE\_ID}] Is a chacracter string the indicates the type of component that namelist group is defining.   {\ct TYPE\_ID} can be {\ct DUCT}, {\ct NODE}, or {\ct FAN}.
...
\begin{description}
\item[{\ct AREA}] Is the area in $m^2$ of the duct.  One of {\ct AREA} or {\ct DIAMETER} must be defined.
...
\item[{\ct DAMPER}] Is a logical variable indicating the precense of a damper in the duct.  The state of the damper is controlled by function named by {\ct CTRL\_ID} or {\ct DEVC\_ID}.  Only one of either {\ct DAMPER} or {\ct FAN\_ID} can be specified for a single duct.
...
\item[{\ct DIAMETER}]  Is the diameter in $m$ of the duct.  One of {\ct AREA} or {\ct DIAMETER} must be defined.
...
\item[{\ct ROUGHNESS}]  Is the absolute roughness in $m$ of the duct which is used in computing the friction factor for the duct.
\item[{\ct VOLUME\_FLOW}] Defines a fixed volumetric flow rate in $m^3/s$ in the duct.  {\ct VOLUME\_FLOW} cannot be used in conjunction with {\ct FAN\_ID}.
\end{description}
...
\begin{description}
\item[{\ct AMBIENT}] Is a logical value.  If {\ct .TRUE.}, then the node is connected to the ambient (i.e. it is equivalent to the {\ct OPEN} boundary condition on {\ct SURF}).
\item[{\ct DUCT\_ID}] Gives the {\ct ID}s of the ducts connected to the node.  Up to 10 ducts can be connected to a node.
\item[{\ct LOSS}] Is an $nxn$ array of real numbers giving the flow losses for the node.  {\ct LOSS(x,y)} is the loss from duct x to duct y.
\item[{\ct VENT\_ID}] If the node connects to the FDS computational domain.  {\ct VENT\_ID} is the name of the {\ct VENT} where the connection occurs.
\item[{\ct XYZ}] Is a triplet of real numbers giving the coordinates of the node.  This location is used to compute bouyancy heads.  If the node is connected to the FDS domain, then do not specify {\ct XYZ}.  FDS will computed it as the centriod of the {\ct VENT}.
\end{description}
...
\item[{\ct MAX\_PRESSURE}] the stall pressure of the fan in $Pa$.  This input activates a quadratic fan model.
\item[{\ct RAMP\_ID}] Is the name of a {\ct RAMP} that contains a fan curve table of fan volumetric flow in $m^3/s$ vs the pressure drop across the fan in $Pa$.  node connects to the FDS computational domain.
{\ct VOLUME\_FLOW} is the fixed volumetric flow of the fan in $m^3/s$.
\end{description}

Note that only one set of fan model inputs ({\ct VOLUME\_FLOW}, {\ct RAMP\_ID}, or {\ct MAX\_FLOW + MAX\_PRESSURE}) should be specified.

Note that {\ct FAN} defines a class of fans rather than one specific fan; therefore, more than one duct can reference a single {\ct FAN}.
...
The surface roughness (in meters) is set by {\ct ROUGHNESS} on {\ct SURF}. See the FDS Technical Reference Guide~\cite{FDS_Math_Guide_5} for wall model details.
...
\begin{warning}
\noindent
The parameter {\ct SLIP\_FACTOR} is no longer used to set the tangential velocity boundary condition, as of FDS version 5.4.0.
\end{warning}
...
\item[{\ct Y\_O2\_INFTY}] Ambient mass fraction of oxygen (Mixture Fraction only, default 0.232428)
...
\be \dq''' = \min \, \left( \dq_{\max}''' \; , \; \frac{ \rho \min \left( Y_\F , Y_\OTWO/s \right) }{\tau} \, \Delta H  \right) \quad ; \quad s=\frac{W_\F}{\nu_{\OTWO} W_{\OTWO} }  \ee
...
on the {\ct REAC} line, in which case $\tau$ becomes the time step, $\dt$.
There is an additional bound on the local heat release rate per unit volume,
based on an empirical estimate of the average volumetric heat release rate of a fire.
Orloff and De Ris~\cite{Orloff:19th_Symposium} suggest a value of 1200~kW/m$^3$ for the entire fire.
FDS uses by default a value of
\be \dq_{\max}'''=\frac{\dq_{\max}''}{\dx} + \overline{\dq_{\max}'''} \quad \hbox{kW/m}^3 \ee
as a local upper bound. The term, $\dq_{\max}''$, is the maximum heat release rate per unit area of flame sheet.
It is specified via {\ct HRRPUA\_SHEET} on the {\ct REAC} line. It is 0~kW/m$^2$ by default for LES; 200 for DNS.
The term, $\overline{\dq_{\max}'''}$, is the maximum heat release rate per unit volume.
It is specified via {\ct HRRPUV\_AVERAGE} on the {\ct REAC} line. It is 2500~kW/m$^3$ by default for LES; 0 for DNS.
Further discussion is found in the FDS Technical Reference Guide, Volume 1.
...
{\ct ETHYLENE}         & 28   & 4.163     &224.7           &        \\ \hline
{\ct HELIUM}           &  4   & 2.551     & 10.22          &        \\ \hline
{\ct HYDROGEN}         &  2   & 2.827     & 59.7           &        \\ \hline
{\ct HYDROGEN BROMIDE} & 81   & 3.353     &449.0           &        \\ \hline
{\ct HYDROGEN CHLORIDE}& 36   & 3.339     &344.7           &        \\ \hline
{\ct HYDROGEN CYANIDE} & 26   & 3.63      &569.1           &        \\ \hline
{\ct HYDROGEN FLOURIDE}& 20   & 3.148     &330.0           &        \\ \hline
{\ct METHANE}          & 16   & 3.758     &148.6           &        \\ \hline
{\ct METHANOL}         & 32   & 3.626     &481.8           &    Y   \\ \hline
{\ct N-HEXANE}         & 86   & 4.524     &199.41          &    Y   \\ \hline
{\ct N-HEPTANE}        &100   & 4.701     &205.75          &    Y   \\ \hline
{\ct N-OCTANE}         &114   & 4.892     &231.16          &        \\ \hline
...
{\ct TOLUENE}          & 92   & 5.698     &480.0           &        \\ \hline
...
\noindent
In a file called {\bf CHID\_line.csv}, there will be 4 columns of data (3 spatial coordinates and the value)
with the 20 temperature points presented as a running average. The
...
The bead {\ct BEAD\_DIAMETER}, {\ct BEAD\_EMISSIVITY}, {\ct BEAD\_DENSITY}, and {\ct BEAD\_SPECIFIC\_HEAT} are given on the associated {\ct PROP} line.
...
Table~\ref{tab:hvacoutput} summarizes the various Output Quantities for HVAC system.  Quantities for a duct require the specification of a {\ct DUCT\_ID}, and quantities for a nodet require the specification of a {\ct NODE\_ID}.  Mass and volume fraction outputs also require the specification of a {\ct SPEC\_ID}
...
\hline \hline
\endhead
{\ct DUCT DENSITY}                              & Desnsity of the flow in a duct     &  kg/$m^3$             \\ \hline
...
{\ct NODE DENSITY}                              & Desnsity of the flow through a node     &  kg/$m^3$             \\ \hline
{\ct NODE MASS FRACTION}                        & Mass fraction of a species in a node                 &  kg/kg             \\ \hline
{\ct NODE Pressume}                             & Pressure of a node                 &       Pa           \\ \hline
...
\section{\texorpdfstring{{\tt HVAC}}{HVAC} (HVAC System Defintion)}
...
\label{tbl:hvacinputs} \\
\hline
\multicolumn{5}{|c|}{{\ct HVAC} (HVAC System Defintion)} \\
\hline \hline
\endfirsthead
\caption[]{Continued} \\
\hline
...
\hline \hline
\endhead
{\ct AMBIENT}                   & Logical           & Node is connected to the ambient                   &               &  .FALSE.      \\ \hline
{\ct AREA}                      & Real              & Area of a duct                          & $m^2$         &               \\ \hline
{\ct CTRL\_ID}                  & Character         & Name of {\ct CTRL} controlling a fan or damper   &               &  'null'       \\ \hline
{\ct DAMPER}                    & Logical           & Damper is present in a duct                      &               &  .FALSE.      \\ \hline
{\ct DEVC\_ID}                  & Character         & Name of {\ct DEVC} controlling a fan or damper   &               &  'null'       \\ \hline
{\ct DUCT\_ID}                  & Character Array   & Name of ducts connected to a node         &               &               \\ \hline
{\ct DIAMETER}                  & Real              & Diameter of a duct                      &  m            &               \\ \hline
{\ct FAN\_ID}                   & Character         & Name of a fan in a duct                          &               &  'null'       \\ \hline
...
{\ct MAX\_FLOW}                 & Real              & Maximum volumetric flow for a quadratic fan                 &  $m^3$        &               \\ \hline
...
{\ct RAMP\_ID}                  & Character         & Name of {\ct RAMP} containing a fan curve  &               &  'null'       \\ \hline
{\ct ROUGHNESS}                 & Real              & Absolute roughness of a duct                        &  m            &               \\ \hline
{\ct TYPE\_ID}                  & Character         & Type of component                          &               &               \\ \hline
{\ct VENT\_ID}                  & Character         & Name of the {\ct VENT} the node is connected to  &               &  'null'       \\ \hline
{\ct VOLUME\_FLOW}              & Real              & Fixed volumetric flow of a duct                        &  $m^3$        &               \\ \hline
...
%moved to SURF%{\ct H\_FIXED}                  & Real          & See Section~\ref{info:convection}             & W/m$^2$/K     &                   \\ \hline
...
{\ct BEAD\_DENSITY}                     & Real          & Density of TC bead                        & kg/m$^3$              & 8908.     \\ \hline
{\ct BEAD\_DIAMETER}                    & Real          & Diameter of TC bead                       & m                     & 0.001     \\ \hline
{\ct BEAD\_EMISSIVITY}                  & Real          & Emissivity of TC bead                     &                       & 0.85      \\ \hline
{\ct BEAD\_SPECIFIC\_HEAT}              & Real          & Specific Heat of TC bead                  & kJ/kg/K               & 0.44      \\ \hline
