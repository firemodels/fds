...
This chapter describes a process by which we quantify the error of FDS. Note the use of the word ``error.'' The reason for using it is to distinguish it from the
word ``uncertainty.'' Whenever a fire model is used to make a prediction of what will happen in the event of a fire, people want to know how reliable the prediction is.
There are two reasons why a model prediction might be less than reliable -- error in the model itself or uncertainty in the measured or postulated input parameters. We use
the term ``uncertainty'' to describe the latter because the word has a connotation that ranges from guesswork to insufficient information. For example, the heat release rate of
a specified design fire is not necessarily wrong, but rather uncertain in the sense that it is impossible to say precisely how big the fire is going to be in any given
building. Model error, on the other hand, is more easily quantified because it involves the laws of physics and mathematical equations. However, as will be shown in this chapter,
quantifying model error cannot be done directly. Only via comparisons with a considerable number of small and full-scale experiments is it possible to assign some measure of
accuracy.

\section{Model Error}

First, we need to define a few terms. Unfortunately, there is no consensus on the meaning
of various terms used to describe the error and uncertainty associated with fire model predictions. To simplify matters a bit, we will use the word
``error'' when describing anything to do with a mathematical model, and we will use the word ``uncertainty'' when describing anything to do with an experimental measurement.
Why the distinction? Models like FDS do not actually solve the Navier-Stokes equations, but rather an approximate form of these equations. The approximation involves simplifying
physical assumptions, like the low-Mach number approximation, and the discretization of the governing equations. For example, the partial derivative of the density, $\rho$,
with respect to the coordinate direction, $x$, is written in approximate form like:
\be \frac{\partial \rho}{\partial x} = \frac{\rho_{i+1} - \rho_{i-1}}{2 \, \dx} + \mathcal{O}(\dx^2) \ee
The second term on the right stands for all the terms of order $\dx^2$, also known as the {\em discretization error}. These extra terms are simply dropped from
...
some extent, but what is called the ``subgrid-scale'' turbulence model, which is yet another approximation of the real thing. What effect do these approximations have on
...
question like, ``What is the error of an FDS prediction of the gas temperature at a particular point at a particular time?''

To make matters worse, there are literally dozens of subroutines that make up FDS, from its radiation solver, solid phase heat transfer routines, pyrolysis model,
...
model error is to quantify the uncertainty of the measurements against which the model predictions are to be compared.


\section{Experimental Uncertainty}

The documentation of the experiments described in the previous chapter have varying descriptions of the uncertainty in the reported measurements.
...
In the US Nuclear Regulatory Commission Verification and Validation study~\cite{NUREG_1824}, Hamins estimates the combined
...
At this point in the analysis, we have a set of model predictions and a corresponding set of experimental measurements,
$M_i$ and $E_i$, respectively, where $i$ ranges from 1 to $n$. Without loss of generality, assume that the
ambient value of these quantities is zero. The other piece of information we have is an estimate of the uncertainty of the experimental measurements. With
this information, {\em and only this information}, we seek to quantify the model error.
...
consideration. Essentially, we are assuming that the experimental measurement has no bias and its standard deviation, $\sigma_E$, has been determined following the procedure described in the
...
Again using the log transformation we obtain $\ln(M)|\theta \sim N(\ln(\delta)+\ln(\theta),\tilde{\sigma}_M^2)$
...
(that is for any $\theta$) of $\ln(M) \sim N(\ln(\delta)+\ln(E),\tilde{\sigma}_M^2+\tilde{\sigma}_E^2)$.

What we need now is a way to estimate $\delta$, the bias, and $\tilde{\sigma}_M$, the relative uncertainty, of the model predictions. First, define:
\be \bar{M} = \frac{1}{n} \, \sum_{i=1}^n \, \ln(M_i)  \quad ; \quad \bar{E} = \frac{1}{n} \, \sum_{i=1}^n \, \ln(E_i) \ee
...
Now define
\be \hat{M}_i = \bar{M}-\bar{E} + \ln(E_i) \ee
The least squares estimate of the standard deviation of $\ln(M_i)$, that is of $\sqrt{\tilde{\sigma}_M^2+\tilde{\sigma}_E^2}$, is
\be u(M) = \sqrt{ \frac{1}{n-1} \sum_{i=1}^n \, \left[ \ln(M_i)-\hat{M}_i \right]^2 } \ee
Note that this quantity is the {\em total relative uncertainty} of $M$.
The model-intrinsic component, $\tilde{\sigma}_M$, can be estimated from
\be u(M)^2 = \tilde{\sigma}_M^2 + \tilde{\sigma}_E^2 \ee

To verify this procedure, we generate 1000 random numbers between 0 and 1000. We say that these numbers represent the ``true'' temperature, $\theta_i$, from 1000 fire
experiments. Of course, we would never know the true temperature -- this is only a hypothetical exercise. Next, for each $\theta_i$, we randomly choose an
experimental measurement, $E_i$, from a normal distribution whose mean is $\theta_i$ and whose relative uncertainty is 0.07, based on Table~\ref{Uncertainty},
``HGL Temperature.'' Finally, for each $\theta_i$ we randomly choose a model prediction, $M_i$, from a normal distribution whose mean is $1.2 \, \theta_i$
and whose relative uncertainty is 0.3~. Given 1000 pairs of $(E_i,M_i)$ and the experimental uncertainty, {\em and nothing else},
the procedure above ought to estimate the values of $\delta=1.2$ and $\tilde{\sigma}_M=0.3$~.
