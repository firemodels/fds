...
is {\ct DT\_HRR} on the {\ct DUMP} line. The columns in this file record the time history of various terms in the enthalpy transport equation.
Neglecting the mass and energy gains and losses from particles, the transport equation for the sensible enthalpy is
\begin{equation}
\label{eqn_enthalpytransport}
\frac{\partial (\rho h_s) }{\partial t} + \nabla \cdot \left( \rho \bu h_s - k \nabla T - \sum_\alpha h_{s,\alpha} \, \rho D_\alpha \nabla Y_\alpha +
     \dot{\mathbf{q}}^{\prime\prime}_{r} \right) = \dot{q}'''  + \frac{d \overline{p} }{dt}
\end{equation}
The first column in the file contains the time in seconds. The second
through sixth columns contain integrated energy gains and losses, all
in units of kW. The second column, {\ct HRR}, contains the total heat release
rate, the volume integral of $\dot{q}'''$. The third column, {\ct Q\_RAD}, contains the radiative heat loss to all the boundaries
(solid and open), the volume integral of $\nabla \cdot \dot{\mathbf{q}}^{\prime\prime}_{r}$. The fourth and fifth columns, {\ct Q\_OPEN} and {\ct Q\_WALL}, 
contains the volume integral of conduction and diffusion terms, calculated as a surface integral over the open and solid parts of the boundary, respectively. 
The sixth column, {\ct Q\_TOTAL}, is just the sum of {\ct Q\_RAD}, {\ct Q\_OPEN}, and {\ct Q\_WALL}. Ideally, this should equal {\ct HRR}.

The seventh column contains the total burning rate of
...
\footnotesize
\begin{verbatim}
s,kW,kW,kW,kW,kW,kg/s,Pa,Pa, ...
Time,HRR,Q_RAD,Q_OPEN,Q_WALL,Q_TOTAL,BURN_RATE,ZONE_01,ZONE_02, ...
...
\noindent
{\ct HRR} is the total heat release rate, {\ct Q\_RAD} is the rate of thermal radiation lost to the boundaries,
{\ct Q\_OPEN} is the sensible enthalpy flow rate out of (positive) or into (negative) the computational domain,
{\ct Q\_WALL} is the sensible enthalpy flow into (positive) or out of (negative) the solid surfaces, {\ct Q\_TOTAL} is the
sum of {\ct Q\_RAD}, {\ct Q\_OPEN}, and {\ct Q\_WALL}, and typically equals {\ct HRR}.  
