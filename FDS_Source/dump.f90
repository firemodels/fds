...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   OUTPUT_INDEX_SELECT: SELECT CASE(DV%OUTPUT_INDEX)
      ...
      CASE(1:299,500:502) OUTPUT_INDEX_SELECT ! gas phase
...
!       GAS_PHASE_OUTPUT_RES = MEASURE_SCALAR_RESOLUTION(II,JJ,KK,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,NM)
   ...
   CASE(503) ! AUTO IGNITION TEMPERATURE
      IF (REIGNITION_MODEL) THEN
         GAS_PHASE_OUTPUT_RES = MIN( AIT(II,JJ,KK)-TMPM, REACTION(1)%AUTO_IGNITION_TEMPERATURE )
      ELSE
         GAS_PHASE_OUTPUT_RES = MIN( 1.E20_EB,           REACTION(1)%AUTO_IGNITION_TEMPERATURE )
      ENDIF
...
REAL(EB) :: CONCORR,VOLSUM,C_S_ADJUST_UNITS,MFT,ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,KSGS,DEPTH,UN,H_S,EMISSIVITY
...
DO NM=1,NMESHES
   Q_DOT_TOTAL(:) = Q_DOT_TOTAL(:) + Q_DOT_SUM(:,NM)/(T-T_LAST_DUMP_HRR)
   M_DOT_TOTAL(1) = M_DOT_TOTAL(1) + M_DOT_SUM(1,NM)/(T-T_LAST_DUMP_HRR)
   M_DOT_TOTAL(2) = M_DOT_TOTAL(2) + M_DOT_SUM(2,NM)/(T-T_LAST_DUMP_HRR)
ENDDO
...
DO NM=1,NMESHES
   MINT_TOTAL(0:N_SPECIES) = MINT_TOTAL(0:N_SPECIES) + MINT_SUM(0:N_SPECIES,NM)/(T-T_LAST_DUMP_MASS)
ENDDO
