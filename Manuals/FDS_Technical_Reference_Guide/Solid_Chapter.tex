...
Our goal now is to determine the vapor phase mole fractions which then feed into the mass flux driving force (Fick's law).  Typically, we will assume the liquid phase mole fractions to be known.  Then, the unknowns in the problem are the $n$ gas phase mole fractions and the temperature $T_{\rm s}$.  The equations we have are $n$ vapor pressure relations plus one constraint on the mole fractions $\sum X_{\alpha,{\rm g}}=1$.
