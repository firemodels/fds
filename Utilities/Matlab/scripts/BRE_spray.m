...
dmax = max(exp_data(:,exp_col_d));
wmax = max(exp_data(:,exp_col_w));
...
for n = 1:3
   ...
   hl(n) = legend(hd(n,:),'Exp.','FDS','Location','NorthEast');
end
...
for n = 1:3
   ...
   hl(n) = legend(hw(n,:),'Exp.','FDS','Location','NorthWest');
end
