...
WALL_CELL_LOOP_0: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (SF%THERMAL_BC_INDEX/=THERMALLY_THICK) CYCLE WALL_CELL_LOOP_0
   BC => BOUNDARY_COORD(WC%BC_INDEX)
   B1 => BOUNDARY_PROP1(WC%B1_INDEX)
   CALL NEAR_SURFACE_GAS_VARIABLES(T,SF,BC,B1,WALL_INDEX=IW)
   IF (CALL_HT_1D) B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=IW)
ENDDO WALL_CELL_LOOP_0
!$OMP END DO

IF (SOLID_PARTICLES) THEN
   !$OMP DO SCHEDULE(DYNAMIC)
   PARTICLE_LOOP_0: DO IP=1, NLP
      LP => LAGRANGIAN_PARTICLE(IP)
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      IF (.NOT.LPC%SOLID_PARTICLE .AND. .NOT.LPC%MASSLESS_TARGET) CYCLE PARTICLE_LOOP_0
      SF => SURFACE(LPC%SURF_INDEX)
      IF (SF%THERMAL_BC_INDEX/=THERMALLY_THICK) CYCLE PARTICLE_LOOP_0
      ...
      IF (CALL_HT_1D) B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,PARTICLE_INDEX_IN=IP)
   ENDDO PARTICLE_LOOP_0
   !$OMP END DO
ENDIF
...
IF (N_THIN_WALL_CELLS>0 .AND. CALL_HT_1D) THEN
   ...
   DO ITW=1,N_THIN_WALL_CELLS
      ...
      IF (TW%WALL_INDEX_M>0) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=TW%WALL_INDEX_M)
      ELSEIF (TW%WALL_INDEX_P>0) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=TW%WALL_INDEX_P)
      ELSE
...
!$OMP DO SCHEDULE(DYNAMIC)
CFACE_LOOP_0: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   CFA => CFACE(ICF)
   IF (CFA%BOUNDARY_TYPE==NULL_BOUNDARY) CYCLE CFACE_LOOP_0
   SF => SURFACE(CFA%SURF_INDEX)
   IF (.NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) CYCLE CFACE_LOOP_0
   BC => BOUNDARY_COORD(CFA%BC_INDEX)
   B1 => BOUNDARY_PROP1(CFA%B1_INDEX)
   CALL CFACE_THERMAL_GASVARS(ICF,SF,B1,T)
   IF (CALL_HT_1D) B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,CFACE_INDEX_IN=ICF)
ENDDO CFACE_LOOP_0
!$OMP END DO
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
   ...
   IF (LPC%MASSLESS_TARGET) THEN  ! the particle's sole purpose is to record a heat flux
      ...
      ELSE  ! the HTC will be computed using the surrounding gas phase environment
         SF_HTC = -1._EB
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,B1%TMP_G-PY%GAUGE_TEMPERATURE,SF_HTC,SF,&
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,DTMP,SF%H_FIXED,SF,WALL_INDEX_IN=WALL_INDEX)
      ELSEIF (PRESENT(CFACE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,DTMP,SF%H_FIXED,SF,CFACE_INDEX_IN=CFACE_INDEX)
      ELSEIF (PRESENT(PARTICLE_INDEX)) THEN
         B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T,DTMP,SF%H_FIXED,SF,PARTICLE_INDEX_IN=PARTICLE_INDEX)
      ENDIF
...
SUB_TIMESTEP_LOOP: DO
   ...
   SELECT CASE(BACKING)
      ...
      CASE(VOID)  ! Non-insulated backing to an ambient void
         ...
         HTCB = HEAT_TRANSFER_COEFFICIENT(NM,T,DTMP,SF%H_FIXED_B,SF)
         Q_CON_B = HTCB*DTMP
   ...
   IF (SF%BOUNDARY_FUEL_MODEL) THEN
      ...
      DO I=1,NWP
         ...
         IF (PRESENT(WALL_INDEX)) THEN
            HTCF = HEAT_TRANSFER_COEFFICIENT(NM,T,DTMP,SF%H_FIXED,SF,WALL_INDEX_IN=WALL_INDEX)
         ELSEIF(PRESENT(CFACE_INDEX)) THEN
            HTCF = HEAT_TRANSFER_COEFFICIENT(NM,T,DTMP,SF%H_FIXED,SF,CFACE_INDEX_IN=CFACE_INDEX)
         ENDIF
...
REAL(EB) FUNCTION HEAT_TRANSFER_COEFFICIENT(NM,T,DELTA_N_TMP,H_FIXED,SF,WALL_INDEX_IN,CFACE_INDEX_IN,PARTICLE_INDEX_IN)
...
REAL(EB), INTENT(IN) :: DELTA_N_TMP,H_FIXED,T
INTEGER, INTENT(IN) :: NM
...
TMP_FILM = B1%TMP_F+FILM_FAC*(B1%TMP_G-B1%TMP_F)
      ...
      HTC_MODEL_SELECT: SELECT CASE(SF%HEAT_TRANSFER_MODEL)
         CASE(DEFAULT_HTC_MODEL,IMPINGING_JET_HTC_MODEL)
            ...
            IF (SF%HEAT_TRANSFER_MODEL==IMPINGING_JET_HTC_MODEL) THEN
               ...
               NUSSELT_IMPINGE = HEAT_TRANSFER_COEFFICIENT*EXP(-0.5_EB * (XX / SF%HTC_SIGMA)**2) * CONV_LENGTH/K_G
            ENDIF
            IF (PRESENT(PARTICLE_INDEX_IN)) THEN
...
TMP_LOOP: DO I=1,N_TGA
   ...
   IF (TGA_WALL_INDEX>0) THEN
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T_TGA,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,WALL_INDEX_IN=IW)
      CALL SOLID_HEAT_TRANSFER(NM,T_TGA,TGA_DT,WALL_INDEX=IW)
   ELSE
      B1%HEAT_TRANS_COEF = HEAT_TRANSFER_COEFFICIENT(NM,T_TGA,B1%TMP_G-B1%TMP_F,SF%H_FIXED,SF,PARTICLE_INDEX_IN=IP)
