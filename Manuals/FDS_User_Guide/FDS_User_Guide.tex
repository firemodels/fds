...
\item[Parallel Processing] It is possible to run an FDS calculation on more than one computer using the Message Passing Interface (MPI). Details can be found in Section~\ref{info:parallelprocessing}.
...
FDS uses OpenMP (Open Multi-Processing) to allow a single computer to run a single FDS job on one or more cores/threads. The use of OpenMP does not require the FDS domain to be broken up into multiple meshes, but will still work with cases that have multiple meshes defined.

By default, FDS uses the maximum number of cores/threads available on a single computer. To specify the number of cores/threads that are available to FDS jobs, the {\ct OMP\_NUM\_THREADS} environment variable can be used. For example, to specify 4 available cores/threads on a Windows machine, type the following at the command prompt:
\begin{lstlisting}
set OMP_NUM_THREADS=4
\end{lstlisting}
To specify 4 available cores/threads on a Mac OS~X, Linux, or Unix machine, type the following at the command prompt:
\begin{lstlisting}
export OMP_NUM_THREADS=4
\end{lstlisting}
When the job is started, FDS will print the number of cores/threads that will be used for that job. Note that this setting will only apply until you log out or restart your machine. To set the default value of available cores/threads upon startup, the {\ct OMP\_NUM\_THREADS} environment variable can also be set in the startup configuration scripts on a machine. Refer to the documentation for your operating system for more information on how to configure environment variables upon startup.
...
The parameters in the input file can be integers, reals, character strings, or logical parameters. A logical parameter is either {\ct .TRUE.} or {\ct .FALSE.} -- the periods are a Fortran convention. Character strings that are listed in this User's Manual must be copied exactly as written -- the code is case sensitive and underscores {\em do} matter. The maximum length of most character input parameters is 60.
...
Some text file encodings may not work on all systems. If file reading errors occur and no typographical errors can be found in the input file, try saving the input file using a different encoding. For example, the text file editor Notepad works fine on a Windows PC, but a file edited in Notepad may not work under linux or Mac OSX because of the difference in line endings under the DOS and unix/linux operating systems. The editor Wordpad typically works better, but try a simple case first.
...
\item[{\ct LOSS}] invokes a linear flow loss model where the flow loss, $K$, is given as a linear function of the total loading, $K_{\si{FILTER}}=K_{\si{CLEAN\_LOSS}}+K_{\si{LOSS}} \sum \left( L_i M_i \right)$, where $L_i$ is the species loading and $M_i$ is a multiplier.  Only one of {\ct LOSS} or {\ct RAMP\_ID} should be specified.
...
\begin{lstlisting}
&DEVC XB=..., QUANTITY='MASS FRACTION', SPEC_ID='METHANE', STATISTICS='MASS INTEGRAL',
QUANTITY_RANGE=0.03,0.15/
\end{lstlisting}
