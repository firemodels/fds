...
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(MAX_DUCTS,MAX_DUCTS),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW, &
            TMP_WGT,TAU_FAN,TAU_VF,CP,YY_GET(1:N_GAS_SPECIES)
   ...
   SELECT CASE (TYPE_ID)
      CASE('DUCT')
         ...
         IF (N_GAS_SPECIES > 0) THEN
            ALLOCATE(DU%YY(N_GAS_SPECIES))
            DU%YY(1:N_GAS_SPECIES) = YY_AMBIENT(1:N_GAS_SPECIES)
         ENDIF
...
IF (LEAK_DUCTS > 0) THEN
   ...
   IF (N_GAS_SPECIES > 0) THEN
      ALLOCATE(LEAK_YY(0:N_ZONE,0:N_ZONE,1:N_GAS_SPECIES,1:NMESHES))
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   ...
   IF (DN%LEAKAGE) THEN
      ...
      DN%RSUM = RSUM0
      IF (N_GAS_SPECIES > 0) THEN
         ALLOCATE(DN%YY(N_GAS_SPECIES))
         DN%YY(1:N_GAS_SPECIES) = YY_AMBIENT(1:N_GAS_SPECIES)
         ALLOCATE(DN%YY_V(N_GAS_SPECIES))            
         DN%YY_V(1:N_GAS_SPECIES) = YY_AMBIENT(1:N_GAS_SPECIES)
...
IF (N_GAS_SPECIES > 0) ALLOCATE(NODE_YY(1:N_DUCTNODES,1:N_GAS_SPECIES,1:NMESHES))
...
REAL(EB) :: TNOW,MTOT,ETOT,VTOT,YYTOT(1:N_GAS_SPECIES),TGUESS,VFLOW,YY_N(1:N_GAS_SPECIES),TMP_WGT
INTEGER :: ITMP
...
ITER_LOOP: DO
   ...
   DUCT_LOOP:DO ND = 1,NE%N_DUCTS
      ...
      IF (DN%UPDATED) THEN
         ...
         DU%CP_D   = DN%CP         
         DU%UPDATED = .TRUE.
         IF (N_GAS_SPECIES > 0) DU%YY(:) = DN%YY(:)
      ENDIF
   ENDDO DUCT_LOOP
   NODE_LOOP:DO NN = 1,NE%N_DUCTNODES
      ...
      MTOT = 0._EB
      ETOT = 0._EB
      IF(N_GAS_SPECIES > 0) YYTOT = 0._EB
      TGUESS = 0._EB
      DO ND = 1,DN%N_DUCTS
         ...
         IF(N_GAS_SPECIES > 0) YYTOT = YYTOT + VFLOW * DU%RHO_D * DU%YY
...
!IF (N_GAS_SPECIES > 0) WRITE(*,*) (TRIM(DUCTNODE(NN)%ID),DUCTNODE(NN)%YY(:),NN=1,N_DUCTNODES)
...
INTEGER :: NN,II,JJ,KK,I1,I2,J1,J2,K1,K2,IC,IW,IOR,NODE_ZONE,ITMP,IZ1,IZ2,IBC
REAL(EB) :: YY_SUM(N_GAS_SPECIES),YY_N(1:N_GAS_SPECIES),RHO_SUM,P_SUM,AREA_SUM,AREA,TMP_SUM,H_SUM,H_G,TMP_NEW,CP,TNOW,&
           P_AVE,TMP_WGT!,VEL2
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   IF (N_GAS_SPECIES>0) YY_SUM=0._EB
   RHO_SUM=0._EB
   P_SUM=0._EB
   AREA_SUM=0._EB
   TMP_SUM=0._EB
      ...
      DO JJ = J1,J2
         DO II = I1, I2
            ...
            IF (N_GAS_SPECIES > 0) THEN
               ...
               YY_N = YY(II,JJ,KK,:)
               CALL GET_AVERAGE_SPECIFIC_HEAT(YY_N,H_G,TMP(II,JJ,KK))
               H_G = H_G * TMP(II,JJ,KK)
            ELSE
               ITMP =MIN(4999,INT(TMP(II,JJ,KK)))
               TMP_WGT = TMP(II,JJ,KK) - ITMP
               H_G = TMP(II,JJ,KK)*(Y2CP_C(ITMP)+TMP_WGT*(Y2CP_C(ITMP+1)-Y2CP_C(ITMP)))
            ENDIF
...
!            VEL2  = 0.25_EB*( (UP(II,JJ,KK)+UP(II-1,JJ,KK))**2+(VP(II,JJ,KK)+VP(II,JJ-1,KK))**2+(WP(II,JJ,KK)+WP(II,JJ,KK-1))**2)
   ...
   IF (N_GAS_SPECIES>0) NODE_YY(NN,1:N_GAS_SPECIES,NM) = YY_SUM(1:N_GAS_SPECIES) / RHO_SUM
   H_SUM = H_SUM / RHO_SUM
   DO
      IF (N_GAS_SPECIES > 0) THEN
         YY_N = DN%YY_V(:)
         CALL GET_AVERAGE_SPECIFIC_HEAT(YY_N,CP,TMP_SUM)      
      ELSE
         ITMP =MIN(4999,INT(TMP_SUM))
         TMP_WGT = TMP_SUM - ITMP
   ...
   IF (N_GAS_SPECIES>0) THEN
      YY_N = NODE_YY(NN,1:N_GAS_SPECIES,NM)
...
LEAKAGE_BC: IF (LEAK_DUCTS > 0) THEN
   ...
   IF (N_GAS_SPECIES > 0) LEAK_YY(:,:,:,NM) = 0._EB
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (N_GAS_SPECIES > 0) THEN
         YY_N = YY(II,JJ,KK,:)
         LEAK_YY(IZ1,IZ2,1:N_GAS_SPECIES,NM) = LEAK_YY(IZ1,IZ2,1:N_GAS_SPECIES,NM)+YY_N*RHO(II,JJ,KK)*AREA
         CALL GET_AVERAGE_SPECIFIC_HEAT(YY_N,CP,TMP(II,JJ,KK))
         H_G = CP * TMP(II,JJ,KK)
      ELSE
         ITMP =MIN(4999,INT(TMP(II,JJ,KK)))
         TMP_WGT = TMP(II,JJ,KK) - ITMP
   ...
   DO IZ1 = 0,N_ZONE
      DO IZ2 = 0,N_ZONE
         ...
         IF (N_GAS_SPECIES>0) THEN
            LEAK_YY(IZ1,IZ2,1:N_GAS_SPECIES,NM) = LEAK_YY(IZ1,IZ2,1:N_GAS_SPECIES,NM) / LEAK_RHO(IZ1,IZ2,NM)
            YY_N = LEAK_YY(IZ1,IZ2,1:N_GAS_SPECIES,NM)
         ...
         DO
            ...
            ELSE
               ...
               CP = Y2CP_C(ITMP)+TMP_WGT*(Y2CP_C(ITMP+1)-Y2CP_C(ITMP))
            ENDIF
            TMP_NEW = LEAK_CP(IZ1,IZ2,NM) / CP
            IF (ABS(TMP_SUM - TMP_NEW) < ZERO_P .OR. ABS(TMP_SUM - TMP_NEW)/TMP_NEW < 0.0005_EB) EXIT
            TMP_SUM = TMP_NEW
         ENDDO    
         LEAK_TMP(IZ1,IZ2,NM) = TMP_NEW
         LEAK_CP(IZ1,IZ2,NM) = CP 
      ENDDO
   ENDDO     
ENDIF LEAKAGE_BC
...
DUCTLOOP: DO ND=1,NE%N_DUCTS
   ...
   IF (DU%VEL(PREVIOUS)>=0._EB) THEN
      ...
      IF(DN%VENT_INDEX > 0 .OR. DN%AMBIENT .OR. DN%LEAKAGE) THEN
         DN%RHO= DN%RHO_V
         DN%TMP= DN%TMP_V
         IF (N_GAS_SPECIES>0) DN%YY = DN%YY_V
         DN%RSUM = DN%RSUM_V
         DN%CP = DN%CP_V                  
      ENDIF
   ENDIF
...
INTEGER:: NN,NM,IZ1,IZ2,ITMP
REAL(EB) :: AREA_SUM,AREA,CP,H_G,TMP_WGT,TMP_SUM,TMP_NEW,YY_N(1:N_GAS_SPECIES)
TYPE(DUCTNODE_TYPE), POINTER :: DN=>NULL()
...
DO NN=1,N_DUCTNODES
   ...
   IF (DN%VENT_INDEX > 0 .AND. .NOT. DN%LEAKAGE) THEN
      ...
      IF (N_GAS_SPECIES > 0) DN%YY_V(1:N_GAS_SPECIES) = NODE_YY(NN,1:N_GAS_SPECIES,NM)
   ENDIF
   LEAKAGE_IF: IF (DN%LEAKAGE) THEN
      ...
      DN%CP_V = 0._EB
      DN%RSUM_V = 0._EB
      IF (N_GAS_SPECIES > 0) DN%YY_V = 0._EB       
      AREA_SUM = 0._EB
      DO NM=1,NMESHES
         ...
         IF (N_GAS_SPECIES > 0) THEN
            YY_N(1:N_GAS_SPECIES) = LEAK_YY(IZ1,IZ2,1:N_GAS_SPECIES,NM)
            DN%YY_V(1:N_GAS_SPECIES) = DN%YY_V(1:N_GAS_SPECIES) + YY_N(1:N_GAS_SPECIES) * LEAK_RHO(IZ1,IZ2,NM) * AREA
            CALL GET_AVERAGE_SPECIFIC_HEAT(YY_N,CP,LEAK_TMP(IZ1,IZ2,NM))
            H_G = CP * LEAK_TMP(IZ1,IZ2,NM)
         ELSE
            ITMP =MIN(4999,INT(LEAK_TMP(IZ1,IZ2,NM)))
            TMP_WGT = LEAK_TMP(IZ1,IZ2,NM) - ITMP
            H_G = LEAK_TMP(IZ1,IZ2,NM)*(Y2CP_C(ITMP)+TMP_WGT*(Y2CP_C(ITMP+1)-Y2CP_C(ITMP)))
         ENDIF
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_AVERAGE_SPECIFIC_HEAT
REAL(EB) :: YY_GET(1:N_GAS_SPECIES),TMP_WGT
INTEGER :: I_DUCT,I_DUCTNODE,NZ1,NZ2,ITMP
...
DO NZ1 = 0, N_ZONE
   DO NZ2 = 0, N_ZONE
      IF (LEAK_PATH(NZ1,NZ2) == 1) THEN
         ...
         IF (N_GAS_SPECIES > 0) THEN
            ALLOCATE(DU%YY(N_GAS_SPECIES))
            YY_GET = YY_AMBIENT(1:N_GAS_SPECIES)
            DU%YY(1:N_GAS_SPECIES) = YY_GET
            CALL GET_AVERAGE_SPECIFIC_HEAT(YY_GET,DU%CP_D,TMPA)
         ELSE
            ITMP =MIN(4999,INT(TMPA))
            TMP_WGT = TMPA - ITMP
            DU%CP_D = Y2CP_C(ITMP)+TMP_WGT*(Y2CP_C(ITMP+1)-Y2CP_C(ITMP))
         ENDIF         
