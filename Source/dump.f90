...
INTEGER :: N,NN,I,J,K,NM,NX,NY,NZ,NIN,NXL,NYL,NZL,NDV,NDVDIM,COLOR_INDEX,IZERO,STATE_INDEX,SURF_INDEX,&
           TYPE_INDEX,HI1,HI2,VI1,VI2,FACE_INDEX,VRGB(3),N_CVENT,N_NODE_OUT=0,N_DUCT_OUT=0
...
! Write out HVAC info to .smv file
DO N=1,N_DUCTNODES
   IF (DUCTNODE(N)%LEAKAGE .OR. DUCT(DUCTNODE(N)%DUCT_INDEX(1))%LOCALIZED_LEAKAGE) CYCLE
   N_NODE_OUT = N_NODE_OUT + 1
ENDDO
DO N=1,N_DUCTS
   IF (DUCT(N)%LEAKAGE .OR. DUCT(N)%LOCALIZED_LEAKAGE) CYCLE
   N_DUCT_OUT = N_DUCT_OUT + 1
ENDDO
WRITE(LU_SMV,'(/A)') 'HVAC'
WRITE(LU_SMV,'(/A)') 'NODE'
WRITE(LU_SMV,'(I5)') N_NODE_OUT
DO N=1,N_DUCTNODES
   IF (DUCTNODE(N)%LEAKAGE .OR. DUCT(DUCTNODE(N)%DUCT_INDEX(1))%LOCALIZED_LEAKAGE) CYCLE
   WRITE(LU_SMV,'(I5')) N
   WRITE(LU_SMV,'(1X,A)') TRIM(DUCTNODE(N)%ID)
   WRITE(LU_SMV,'(1X,A)') TRIM(DUCTNODE(N)%NETWORK_ID)
   WRITE(LU_SMV,'1X,3F12.6') DUCTNODE(N)%XYZ
   WRITE(LU_SMV,'(1X,A)') TRIM(DUCTNODE(N)%VENT_ID)
...
ENDDO
WRITE(LU_SMV,'(/A)') 'DUCT'
WRITE(LU_SMV,'(I5)') N_DUCT_OUT
DO N=1,N_DUCTNODES
   IF (DUCT(N)%LEAKAGE .OR. DUCT(N)%LOCALIZED_LEAKAGE) CYCLE
   WRITE(LU_SMV,'(I5')) N
   WRITE(LU_SMV,'(1X,A)') TRIM(DUCT(N)%ID)
   WRITE(LU_SMV,'(1X,A)') TRIM(DUCT(N)%NETWORK_ID)
   WRITE(LU_SMV,'(1X,I5,1X,I5)') DUCT(N)%NODE_INDEX(1),DUCT(N)%NODE_INDEX(2)
   WRITE(LU_SMV,'(1X,A)') TRIM(DUCTNODE(N)%VENT_ID)
   IF (DUCT(N)%FAN_INDEX > 3) THEN
      WRITE(LU_SMV,'(1X,A)') 'F'
   ELSEIF(DUCT(N)%DAMPER)
      WRITE(LU_SMV,'(1X,A)') 'D'
   ELSEIF(DUCT(N)%AIRCOIL_INDEX)
      WRITE(LU_SMV,'(1X,A)') 'A'
   ELSE
      WRITE(LU_SMV,'(1X,A)') '-'
   ENDIF
   WRITE(LU_SMV,'1X,I5') MAX(1,DUCT(N)%N_CELLS)
   WRITE(LU_SMV,'1X,I5') DUCT(N)%N_WAYPOINTS
   DO NN=1,DUCT(N)%N_WAPOINTS
      WRITE(LU_SMV,'1X,3F12.6') DUCT(N)%WAYPOINTS(NN,1),DUCT(N)%WAYPOINTS(NN,2),DUCT(N)%WAYPOINTS(NN,3)
   ENDDO
ENDDO
