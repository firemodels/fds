...
Recall that the default {\ct SURF} for an {\ct OBST} is {\ct 'INERT'}, which is a Dirichlet condition for the surface temperature equivalent to {\ct TMP\_FRONT=TMPA}.  If two-way coupling with the gas phase is desired, then the {\ct {SURF}} associated with the {\ct {OBST}} face should have {\ct {HT3D=.TRUE.}} and no other specified thermal boundary condition. For example,
\begin{lstlisting}
&MATL ID='steel', .../
