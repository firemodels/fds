...
REAL(EB), INTENT(IN) :: Y_IN(1:N_TRACKED_SPECIES)
...
ENDIF
D_OUT_A = (Y2D_C(ITMP) + DOT_PRODUCT(Y2D(ITMP,1:I_Z_MAX - I_Z_MIN + 1),Z))
D_OUT_B = (Y2D_C(ITMP+1) + DOT_PRODUCT(Y2D(ITMP+1,1:I_Z_MAX - I_Z_MIN + 1),Z))
...
CP_OUT_A = Y2CP_C(ITMP)
CP_OUT_B = Y2CP_C(ITMP+1)
...
INTEGER :: ITMP
REAL :: TMP_WGT,CP_OUT_A,CP_OUT_B
REAL(EB), INTENT(IN) :: Y_IN(1:N_TRACKED_SPECIES),TMPG
REAL(EB), INTENT(OUT) :: CP_OUT
...
CPBAR_OUT_A = Y2CPBAR_C(ITMP)
CPBAR_OUT_B = Y2CPBAR_C(ITMP+1)
...
K_OUT_A = Y2K_C(ITMP) 
K_OUT_B = Y2K_C(ITMP+1)

K_OUT = (K_OUT_A + TMP_WGT * (K_OUT_B-K_OUT_A) )*SPECIES(0)%MW
...
MU_OUT_A = Y2MU_C(ITMP)
MU_OUT_B = Y2MU_C(ITMP+1)

MU_OUT = (MU_OUT_A + TMP_WGT * (MU_OUT_B-MU_OUT_A) )*SPECIES(0)%MW
...
DO INDEX=1,N_SPECIES
   IF (SPECIES(INDEX)%FLD_LETHAL_DOSE > 0._EB) THEN
      Call GET_MASS_FRACTION(Y_IN,INDEX,Y_MF_INT)
      TMP_1 = TMP_1 + SPECIES(INDEX)%RCON*Y_MF_INT/RSUM / SPECIES(INDEX)%FLD_LETHAL_DOSE
   ENDIF
ENDDO
...
FIC = 0._EB
DO INDEX=1,N_SPECIES
   IF (SPECIES(INDEX)%FIC_CONCENTRATION > 0._EB) THEN
      Call GET_MASS_FRACTION(Y_IN,INDEX,Y_MF_INT)
