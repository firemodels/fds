...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,ASSUMED_GAS_TEMPERATURE_RAMP,&
                ...
                MAX_LEAK_PATHS,MAX_SPECIES,MAXIMUM_VISIBILITY,MEAN_FORCING,NOISE,NOISE_VELOCITY,NO_EVACUATION,&
                OVERWRITE,PERIODIC_TEST,POROUS_FLOOR,PR,PROJECTION,P_INF,&
...
REAL(EB) :: MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            ...
            CONVERSION
REAL(EB):: MASS_FRACTION(MAX_SPECIES_NL),VOLUME_FRACTION(MAX_SPECIES_NL)
...
CHARACTER(LABEL_LENGTH) :: RAMP_CP,RAMP_CP_L,RAMP_K,RAMP_MU,RAMP_D,RADCAL_ID,RAMP_G_F,SPEC_ID(MAX_SPECIES_NL)
...
ALLOCATE(PREDEFINED(MAX_SPECIES))
ALLOCATE(PREDEFINED_SMIX(0:MAX_SPECIES))
ALLOCATE(NEW_PRIMITIVE(0:MAX_SPECIES))
NEW_PRIMITIVE=.FALSE.
ALLOCATE(PREDEFINED_SPEC_ID(MAX_SPECIES))
ALLOCATE(SPEC_ID_READ(MAX_SPECIES))
...
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX
CHARACTER(LABEL_LENGTH) :: FUEL,RADCAL_ID='null',SPEC_ID_NU(MAX_SPECIES_NL),SPEC_ID_N_S(MAX_SPECIES_NL)
CHARACTER(100) :: FORMULA
...
REAL(EB) :: SOOT_YIELD,CO_YIELD,EPUMO2,A, &
            ...
            AUTO_IGNITION_TEMPERATURE,SOOT_H_FRACTION,N_T,K,NU(MAX_SPECIES_NL),N_S(MAX_SPECIES_NL)
...
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: NU_SPEC
...
ALLOCATE(SPEC_ID(MAX_SPECIES,MAX_REACTIONS))
ALLOCATE(NU_SPEC(MAX_SPECIES,MAX_REACTIONS))
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   ALLOCATE(ML%NU_SPEC(MAX_SPECIES,MAX_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','MATERIAL',IZERO)
   ALLOCATE(ML%NU_GAS(MAX_SPECIES,MAX_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','MATERIAL',IZERO)
   ALLOCATE(ML%ADJUST_BURN_RATE(MAX_SPECIES,MAX_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','MATERIAL',IZERO)
   ALLOCATE(ML%SPEC_ID(MAX_SPECIES,MAX_REACTIONS),STAT=IZERO)
   CALL ChkMemErr('READ','MATERIAL',IZERO)
...
CHARACTER(LABEL_LENGTH) :: PART_ID,RAMP_MF(MAX_SPECIES_NL),RAMP_Q,RAMP_V,RAMP_T,MATL_ID(MAX_LAYERS,MAX_MATERIALS),&
                 PROFILE,BACKING,GEOMETRY,NAME_LIST(MAX_MATERIALS),EXTERNAL_FLUX_RAMP,RAMP_EF,RAMP_PART,SPEC_ID(MAX_SPECIES_NL),&
...
CHARACTER(25) :: COLOR
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES_NL),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2), &
            TAU_EXTERNAL_FLUX,TAU_EF,E_COEFFICIENT,VOLUME_FLOW,VOLUME_FLUX,&
            TMP_FRONT,TMP_INNER(MAX_LAYERS),THICKNESS(MAX_LAYERS),VEL,&
            MASS_FLUX(MAX_SPECIES_NL),Z0,PLE,CONVECTIVE_HEAT_FLUX,PARTICLE_MASS_FLUX,&
            ...
            MASS_FRACTION(MAX_SPECIES_NL),MASS_TRANSFER_COEFFICIENT,&
...
REAL(EB) :: DIAMETER,TEMPERATURE,DENSITY,RR_SUM,ZZ_GET(0:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
            MASS_PER_TIME,DT_INSERT,UVW(3),HRRPUV,XYZ(3),DX,DY,DZ,HEIGHT,RADIUS,MASS_FRACTION(MAX_SPECIES_NL), &
...
CHARACTER(LABEL_LENGTH) :: ID,CTRL_ID,DEVC_ID,PART_ID,SHAPE,MULT_ID,SPEC_ID(1:MAX_SPECIES_NL)
