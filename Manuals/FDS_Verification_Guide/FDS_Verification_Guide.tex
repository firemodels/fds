...
\dot{m}^{\prime\prime\prime}_{fuel,i} = \frac{C_{EDC}}{\tau_{mix}} \, \min \left( Y_F,\frac{Y_\OTWO}{s},\beta_{EDC}\frac{Y_P}{(1+s)} \right)
\end{equation}
where $C_{EDC}$ is a model constant related to the turbulent subgrid model, $\beta_{EDC}$ is a model constant related to combustion, and $s$ is the mass stoichiometric coefficient. This model can be considered a deficient model as the consumption rate is found by the limiting species. To determine species evolution for the reaction, a system of ordinary differential equations is solved for each participating species ($\alpha$):
