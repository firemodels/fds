...
REAL(EB) :: FXYZ,PMDT,UDRHODN,TNOW,ZZ(4),UN
...
NOT_ISOTHERMAL_IF: IF (.NOT.ISOTHERMAL) THEN
   ...
   ELSE FLUX_LIMITER_IF
      ...
      !$OMP SINGLE PRIVATE(IW,II,JJ,KK,IOR,UN)
      !!$OMP DO PRIVATE(IW,II,JJ,KK,IOR,UN)
      WLOOP_FL: DO IW=1,NWC
         IF (BOUNDARY_TYPE(IW)/=SOLID_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=INTERPOLATED_BOUNDARY) CYCLE WLOOP_FL
         II  = IJKW(1,IW) 
         JJ  = IJKW(2,IW)
         KK  = IJKW(3,IW)
         IOR = IJKW(4,IW)
         SELECT CASE(IOR)
            CASE( 1)
               IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY)        UN = UU(II,JJ,KK)
               IF (BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY) UN = UVW_SAVE(IW)
               FX(II,JJ,KK)   = UN*RHO_F(IW)*R(II)
            CASE(-1)
               IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY)        UN = UU(II-1,JJ,KK)
               IF (BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY) UN = UVW_SAVE(IW)
               FX(II-1,JJ,KK) = UN*RHO_F(IW)*R(II-1)
            CASE( 2)
               IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY)        UN = VV(II,JJ,KK)
               IF (BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY) UN = UVW_SAVE(IW)
               FY(II,JJ,KK)   = UN*RHO_F(IW)
            CASE(-2)
               IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY)        UN = VV(II,JJ-1,KK)
               IF (BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY) UN = UVW_SAVE(IW)
               FY(II,JJ-1,KK) = UN*RHO_F(IW)
            CASE( 3)
               IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY)        UN = WW(II,JJ,KK)
               IF (BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY) UN = UVW_SAVE(IW)
               FZ(II,JJ,KK)   = UN*RHO_F(IW)
            CASE(-3)
               IF (BOUNDARY_TYPE(IW)==SOLID_BOUNDARY)        UN = WW(II,JJ,KK-1)
               IF (BOUNDARY_TYPE(IW)==INTERPOLATED_BOUNDARY) UN = UVW_SAVE(IW)
...
SPECIES_LOOP: DO N=1,N_SPECIES
   ...
   ELSE FLUX_LIMITER_IF2
      ...
      WLOOP2_FL: DO IW=1,NWC
         IF (BOUNDARY_TYPE(IW)/=SOLID_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=INTERPOLATED_BOUNDARY) CYCLE WLOOP2_FL
         II  = IJKW(1,IW) 
         JJ  = IJKW(2,IW)
         KK  = IJKW(3,IW)
         IOR = IJKW(4,IW)
