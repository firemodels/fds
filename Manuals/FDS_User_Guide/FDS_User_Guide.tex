...
{\ct A(j)} on the {\ct MATL} line of the $i$th material, with units of 1/s. $E_{ij}$, the activation energy,
...
{\ct REFERENCE\_RATE} is the reaction rate, in units of 1/s, at the given {\ct REFERENCE\_TEMPERATURE} divided by
...
\begin{align*}
   ...
   r_{p,1,1} &= 0.0016 \; \si{1/s}           & r_{p,2,1}   &= 0.0012 \;\si{1/s} \\
   \nu_{s,1,1} &= 0                          & \nu_{s,2,1} &= 0.2 \\
   \dot{T} &= 5 \; \mathrm{K/min} & &
\end{align*}
\end{minipage} \normalsize
...
factor $A_{ij}$ should then be given a value that is close or slightly smaller than the specific heat (\si{kJ/(kg.K)}) of the material mixture at
...
a solid surface. However, you may wish to control the total mass flow rate per unit area (\si{kg/(m$^2$.s)}) via the parameter {\ct MASS\_FLUX\_TOTAL}. This parameter uses the same sign convention as {\ct VEL} above.  In fact, the value entered for
...
\noindent
indicates that argon and nitrogen are to be injected at rates of 0.1~\si{kg/(m$^2$.s)} and 0.3~\si{kg/(m$^2$.s)} over time periods of approximately 5~s and 10~s, respectively. It does not
...
Conductivity can be specified in one of two ways.  First, it can be explicitly defined using {\ct CONDUCTIVITY} (\si{W/(m.K)}) with optional temperature dependence by using {\ct RAMP\_K}.  Second, it can be computed by FDS using {\ct MW}, {\ct PR} on {\ct MISC}, and the Lennard-Jones potential parameters $\sigma$ ({\ct SIGMALJ}) and $\epsilon/k$ ({\ct EPSILONKLJ}).  If no inputs are specified, FDS will compute the conductivity using {\ct MW} and the Lennard-Jones parameters for air.
...
\ee
where $c_p$ is the {\ct SPECIFIC\_HEAT} (\si{kJ/(kg.K)}) with optional temperature dependence using {\ct RAMP\_CP}.  The (optional) {\ct REFERENCE\_TEMPERATURE}, $T_{ref}$ ($^\circ$C), is the temperature that corresponds to
...
\begin{align}
\dot{m}^{\prime \prime}_{poly} &= \frac{600~\mathrm{kW}}{1~\mathrm{m^2}}\frac{1}{26200~\si{kJ/kg}} = 0.022901~\si{kg/(m^2.s)}\\
\dot{m}^{\prime \prime}_{wood} &= \frac{600~\mathrm{kW}}{1~\mathrm{m^2}}\frac{1}{16400~\si{kJ/kg}} = 0.036585~\si{kg/(m^2.s)}
\end{align}
...
\end{equation}
\begin{description}
\item[{\ct A}] is the pre-exponential factor in units of \si{mol/(cm$^3$.s)},
...
\noindent
The THIEF model assumes that the cable plastic material has a thermal conductivity of 0.2~\si{W/(m.K)} and a specific heat of 1.5~\si{kJ/(kg.K)}. If you change these
...
\ee
The parameter {\ct E\_COEFFICIENT} must be obtained experimentally, and it is expressed in units of \si{m$^2$/(kg.s)}. Usually, this type of suppression algorithm is invoked when the fuel is complicated, like a cartoned commodity.  The example case {\ct e\_coefficient} demonstrates the use of this parameter.  A sprinkler is placed over a burner defined with an {\ct E\_COEFFICIENT} as shown below.  The sprinker is set to operate at 5~s.  Figure~\ref{e_coef_fig} shows the heat release rate and burning rate.
...
\begin{description}
\item[{\ct RTI}] Response Time Index in units of $\sqrt{\si{m.s}}$. (Default 100.)
...
\hline \hline
\endhead
{\ct A}                                   & Real        & Section~\ref{info:finite}                 & \si{cm$^3$/(mol.s)} &     \\ \hline
