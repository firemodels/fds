...
{\bf Caution:} Note that specifying a combination of {\ct VEL} and {\ct MASS\_FRACTION} is \emph{not} numerically equivalent to specifying {\ct MASS\_FLUX}---\emph{even if {\small MASS\_FRACTION=1}!}  For example, suppose you supply a {\ct VENT} with 0.01 \si{m^3/s} of ``pure'' methane.  The total mass flux of methane will be made up of the volume flow \emph{plus} a diffusive component.  The diffusion velocity is approximated using Fick's Law with mixture-averaged diffusivities.  As the grid resolution decreases the species concentration in the near-wall cell may become diluted and the diffusion driving force (gradient of pure minus dilute) may increase.  These errors can be larger than one might expect.  So if a precise mass flux is required of the problem it is important to use a {\ct MASS\_FLUX} boundary condition.
