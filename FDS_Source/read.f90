...
REAL(EB) :: VEL_CHAR,DT
INTEGER :: NM
LOGICAL :: COMPUTE_INITIAL_TIME_STEP
...
IF (DT<=0._EB) THEN
   COMPUTE_INITIAL_TIME_STEP = .TRUE.
   DT = 1.E6_EB
ELSE
   COMPUTE_INITIAL_TIME_STEP = .FALSE.
ENDIF
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   IF (COMPUTE_INITIAL_TIME_STEP) THEN
      ...
      DT = MIN(DT,M%CELL_SIZE/VEL_CHAR*CFL_MAX)
   ENDIF
...
NAMELIST /MISC/ AGGLOMERATION,AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                N_FIXED_CHEMISTRY_SUBSTEPS,NEW_OPEN_BOUNDARY,NOISE,NOISE_VELOCITY,NO_EVACUATION,&
