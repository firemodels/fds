...
IF(ICF > SIZE_CUT_FACE) THEN
   ...
   DO ICF1=1,ICF-1
      ...
      CALL MOVE_ALLOC(FROM=MESHES(NM)%CUT_FACE(ICF1)%SHARED,              TO=CUT_FACE_AUX(ICF1)%SHARED)
      ...
      CALL MOVE_ALLOC(FROM=MESHES(NM)%CUT_FACE(ICF1)%FN_B,                TO=CUT_FACE_AUX(ICF1)%FN_B)
...
IF (ICC > SIZE_CUT_CELL) THEN
   ...
   DO ICC1=1,ICC-1
      ...
      CALL MOVE_ALLOC(FROM=MESHES(NM)%CUT_CELL(ICC1)%IJK_LINK         ,TO=CUT_CELL_AUX(ICC1)%IJK_LINK)
      ...
      CALL MOVE_ALLOC(FROM=MESHES(NM)%CUT_CELL(ICC1)%KRES             ,TO=CUT_CELL_AUX(ICC1)%KRES)
...
DEALLOCATE(MESHES(NM)%CUT_CELL(ICC)%FACE_LIST,MESHES(NM)%CUT_CELL(ICC)%IJK_LINK)
...
ALLOCATE(MESHES(NM)%CUT_CELL(ICC)%IJK_LINK(IAXIS:KAXIS+2,1:NCELL))
