...
REAL(EB) :: X1,X2,Y1,Y2,Z1,Z2,XX1,XX2,YY1,YY2,ZZ1,ZZ2,ORIGINAL_VOLUME,OVERLAP_VOLUME,DXX,DYY,DZZ
...
! Get the node indices of the variable thickness wall cells

WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS

   WC => M%WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)

   IF (.NOT.SF%VARIABLE_THICKNESS .OR. WC%BACK_INDEX<=0) CYCLE WALL_LOOP_1
...
WALL_LOOP_2: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   DO I=1,NWP
      ALLOCATE(THR_D%NODE(I)%ADJACENT_WALL_INDEX(8))  ; THR_D%NODE(I)%ADJACENT_WALL_INDEX = 0
      ALLOCATE(THR_D%NODE(I)%ADJACENT_WALL_NODE(8))   ; THR_D%NODE(I)%ADJACENT_WALL_NODE = 0
      ALLOCATE(THR_D%NODE(I)%ADJACENT_WALL_WEIGHT(8)) ; THR_D%NODE(I)%ADJACENT_WALL_WEIGHT = 0._EB
   ENDDO

   NODE_LOOP_1: DO I=1,NWP
      ...
      END SELECT
      ORIGINAL_VOLUME = (X2-X1)*(Y2-Y1)*(Z2-Z1)
      IOR_AVOID = .FALSE.
      IOR_AVOID(BC%IOR) = .TRUE. ; IOR_AVOID(-BC%IOR) = .TRUE.
      IWA = 0
      THR_D%NODE(I)%ADJACENT_WALL_VALUES = 0
      WALL_LOOP_2B: DO IW2=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
         WC2 => M%WALL(IW2)
         BC2 => M%BOUNDARY_COORD(WC2%BC_INDEX)
         IF (IOR_AVOID(BC2%IOR)) CYCLE WALL_LOOP_2B
         XX1=M%X(BC2%IIG-1) ; XX2=M%X(BC2%IIG) ; YY1=M%Y(BC2%JJG-1) ; YY2=M%Y(BC2%JJG) ; ZZ1=M%Z(BC2%KKG-1) ; ZZ2=M%Z(BC2%KKG)
         IF (ABS(BC2%IOR)/=1) THEN ; DXX = MIN(XX2,X2)-MAX(XX1,X1) ; IF (DXX<=0._EB) CYCLE WALL_LOOP_2B ; ENDIF
         IF (ABS(BC2%IOR)/=2) THEN ; DYY = MIN(YY2,Y2)-MAX(YY1,Y1) ; IF (DYY<=0._EB) CYCLE WALL_LOOP_2B ; ENDIF
         IF (ABS(BC2%IOR)/=3) THEN ; DZZ = MIN(ZZ2,Z2)-MAX(ZZ1,Z1) ; IF (DZZ<=0._EB) CYCLE WALL_LOOP_2B ; ENDIF
         SF2 => SURFACE(WC2%SURF_INDEX)
    !    IF (SF%ID/=SF2%ID) CYCLE WALL_LOOP_2B
         ONE_D2 => M%BOUNDARY_ONE_D(WC2%OD_INDEX)
         NWP2 = SUM(ONE_D2%N_LAYER_CELLS(1:SF2%N_LAYERS))
         NODE_LOOP_2: DO I2=1,NWP2
            SELECT CASE(BC2%IOR)
            ...
            END SELECT
            DXX = MIN(XX2,X2)-MAX(XX1,X1) ; IF (DXX<=0._EB) CYCLE NODE_LOOP_2
            DYY = MIN(YY2,Y2)-MAX(YY1,Y1) ; IF (DYY<=0._EB) CYCLE NODE_LOOP_2
            DZZ = MIN(ZZ2,Z2)-MAX(ZZ1,Z1) ; IF (DZZ<=0._EB) CYCLE NODE_LOOP_2
            OVERLAP_VOLUME = DXX*DYY*DZZ
            IWA = IWA + 1
            DM = SIZE(THR_D%NODE(I)%ADJACENT_WALL_INDEX)
            IF (IWA>DM) THEN
               ALLOCATE(INTEGER_DUMMY(DM+8))
               INTEGER_DUMMY(1:DM) = THR_D%NODE(I)%ADJACENT_WALL_INDEX(1:DM)
               CALL MOVE_ALLOC(INTEGER_DUMMY,THR_D%NODE(I)%ADJACENT_WALL_INDEX) ; THR_D%NODE(I)%ADJACENT_WALL_INDEX(DM+1:DM+8) = 0
               ALLOCATE(INTEGER_DUMMY(DM+8))
               INTEGER_DUMMY(1:DM) = THR_D%NODE(I)%ADJACENT_WALL_NODE(1:DM)
               CALL MOVE_ALLOC(INTEGER_DUMMY,THR_D%NODE(I)%ADJACENT_WALL_NODE)
               ALLOCATE(REAL_DUMMY(DM+8))
               REAL_DUMMY(1:DM) = THR_D%NODE(I)%ADJACENT_WALL_WEIGHT(1:DM)
               CALL MOVE_ALLOC(REAL_DUMMY,THR_D%NODE(I)%ADJACENT_WALL_WEIGHT)
            ENDIF
            THR_D%NODE(I)%ADJACENT_WALL_VALUES = THR_D%NODE(I)%ADJACENT_WALL_VALUES + 1
            THR_D%NODE(I)%ADJACENT_WALL_INDEX(IWA) = IW2
            THR_D%NODE(I)%ADJACENT_WALL_NODE(IWA) = I2
            THR_D%NODE(I)%ADJACENT_WALL_WEIGHT(IWA) = OVERLAP_VOLUME/ORIGINAL_VOLUME
