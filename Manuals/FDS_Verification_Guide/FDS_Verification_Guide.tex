...
In FDS, $\tau_w$ is obtained only from a wall model such as Werner-Wengle. The resolution of the wall-normal streamwise velocity gradient increases with  mesh resolution; the value of $y^+$ decreases accordingly. In this verification case, a wall device is used to output $y^+$ for different mesh resolutions in the $z$-direction. $y^+$ is outputted for a given $\delta z$, height of a cell, $\delta z$ = [1/8, 1/16, 1/32]. Then, the device values obtained are compared with the exact value of $y^+$ for a given mesh resolution. An FDS wall device outputs the mid-cell value of the first cell next to the wall, hence, the $z$ value ($y$ in Eq.~(\ref{eqn_yplus})) used to determine the exact $y^+$ varies with $\delta z$ $(z = \delta z/2)$.
...
\caption[Outlet temperature for {\bf energy\_budget\_tmix} case]{\label{fig_outlet_temperature} Outlet temperature for energy\_budget\_tmix case.  Black circles represent the correct result of 100 $^\circ$C.}
\end{figure}
...
\caption[The {\bf energy\_budget\_solid} case]{\label{fig_solid_energy} Outputs of temperature [left] and pressure [right] for the energy\_budget\_solid case.  Circles represent the correct result.}
\end{figure}
...
\section{Reaction Rates (\texorpdfstring{\bf ReactionRate\_}{ReactionRate\_})}
...
\subsection{Mixing-Controlled Reactions (\texorpdfstring{\bf ReactionRate\_EDC\_}{ReactionRate\_EDC})}
...
\caption[Species Evolution in a One-Step Methane EDC Reaction]{Time evolution of species mass fraction for a one-step methane EDC reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
\label{fig:EDC_1Step_meth_spec}
\end{center}
\end{figure}

In addition to species, we want to compare FDS predictions for temperature and pressure. For this reacting system, the internal energy remains constant, so temperature can be found from:
...
\end{tabular*}
\caption[Temperature and Pressure Evolution in a One-Step Methane EDC Reaction]{Time evolution of temperature and pressure for a one-step methane EDC reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\caption[Species Evolution in a One-Step Propane EDC Reaction]{Time evolution of species mass fraction for a one-step propane EDC reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\end{tabular*}
\caption[Temperature and Pressure Evolution in a One-Step Propane EDC Reaction]{Time evolution of temperature and pressure for a one-step propane EDC reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\end{tabular*}
\caption[Temperature and Pressure Evolution in a One-Step Methane EDC Reaction]{Temperature (left) and pressure (right) evolution for a one-step methane EDM reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\caption[Species Evolution in Two-Step Methane EDC Reaction]{Time evolution of species mass fraction for a two-step, fuel-limited EDC reaction with a fixed mixing time of $\tau_{mix}=0.1$~s.}
...
\subsection{Finite Rate Reactions (\texorpdfstring{\bf ReactionRate\_Arrhenius\_}{ReactionRate\_Arrhenius\_})}

The main difference between Arrhenius rate reactions and the mixing-controlled reactions is that the reaction rate in finite rate reactions is found from local parameters such as species concentration and temperature as well as rate constants. The values of $a_{\alpha}$ in Equation (\ref{eq:Arrheniusratemode}) are specific to each reaction and for non-zero values, $\frac{\mbox{d}\phi_{F}}{\mbox{d}t}$ becomes  a non-linear differential equation. We consider both linear and non-linear cases with one and two-step reaction mechanisms. First we examine two one-step cases; a zero-order and second-order reaction. Equation (\ref{eq:1step_CO}) shows a one-step, zero-order test case of carbon monoxide oxidation:
...
\end{center}
\caption[Species Evolution in a Zero-Order 1 Step Finite Rate Reaction]{Time evolution of species mass fraction for a one-step zero-order Arrhenius finite rate reaction.}
...
\end{center}
\caption[Species Evolution in a Second-Order 1 Step Finite Rate Reaction]{Time evolution of species mass fraction for a one-step second-order Arrhenius finite rate reaction.}
...
\mathrm{C_3H_8+3.5 \, O_2} &\rightarrow \mathrm{3 \, CO+4 \, H_2O} \\
\nonumber \mathrm{CO + 0.5 \, O_2} &\leftrightarrow \mathrm{CO_2}
\end{align}
with the following reaction input parameters:
...
\end{center}
\caption[Mixture fraction state relation for species]{Lumped species mass fractions variation as function of mixture fraction.}
...
\end{equation}
The molecular weight of the species were not set to be uniform, so the average molecular weight and therefore $c_v$, will change as a function of the mixture fraction. As a result, the traditional linear temperature profile is not expected.
\begin{figure}[h!]
\begin{center}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/Burke_Schumann_Temp}
\end{center}
\caption[Mixture fraction state relation for temperature]{Temperature variation as function of mixture fraction.}
...
Here, we examine a 10 $\times$ 10 array of cases: varying initial temperature from 300 K to 1875 K and initial fuel (methane) mass fraction from 0.05 to 0.95. The oxidizer is air which is assumed to be 23 \% oxygen  and 77 \% nitrogen by mass. A simple one-step methane reaction is used to govern the combustion:
\begin{equation}\label{eq:methane_ex}
\mathrm{CH_4 + 2\, O_2 \rightarrow  CO_2 + 2\, H_2O}
\end{equation}
Based on the combination of initial oxygen concentration, methane concentration, and temperature each case can either be able to sustain burning for a finite period of time or not - Fig.~\ref{fig:extinct}
\begin{figure}[h!]
\begin{center}
...
Note: This test case is designed to test the implementation of the extinction model over a wide temperature and composition space, not whether the model has the appropriate delineation between cases that burn and cases that do not.
...
\end{tabular}
\end{center}
\caption[Soot mass conservation for {\bf propane\_flame\_deposition} case]{Time evolution of soot mass components for deposition cases. (Left) Mass of soot in gas phase. (Right) Mass of soot deposited on condensed phase walls. (Bottom) Total mass of soot in gas phase and condensed phase.}
...
\end{tabular}
\end{center}
\caption[Soot mass concentration for {\bf soot\_gravitational\_settling} case]{Time evolution of soot mass concentration for the soot gravitational settling case.}
...
\subsection{Evaporation of liquid surface}
...
\end{center}
\caption[The {bf surf\_mass\_vent\_liquid\_fuel} test case.]{Comparison of analytical and predicted mass change for liquid surface.}
...
where A$_{cx}$ is the 1 m$^2$ cross-sectional area, the pressure drops in each sub channel is 3.77, 7.54, and 15.1 Pa.  Comparisons of computed and analytical results are shown in Figure~\ref{particle_drag_plots}. It is seen that the pressure drop converges to the analytic solution as the time step decreases.
...
\end{tabular*}
\caption[Convergence analysis of the {\bf terminal\_velocity} case]{(Left) FDS convergence for the particle velocity in the terminal velocity case. (Right) FDS convergence for the particle position in the terminal velocity case.}
...
In this case, a gas burner lies near the bottom of a 1~m by 1~m by 5~m high vertical channel whose walls are made of a thin sheet of insulated steel. Air at ambient temperature is forced into the bottom of the channel at 1~m/s. Monodisperse water droplets with a diameter of 2000~$\mu$m are
...
\ee 
where $A_{\hbox{\scriptsize leak}}=0.0001$~m$^2$ and $\rho$ is the density at the discharge side. Equating the fan and leak volume flow rates, we can solve for $\Delta_p$. The results are shown in Fig.~\ref{leak_test}.  
