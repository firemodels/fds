...
REAL(EB) :: MU_N,ZZ_GET(1:N_TRACKED_SPECIES),CS,DELTA,DUMMY,INTEGRAL
...
IF (STRATIFICATION .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO K=0,M%KBP1
      ...
      IF (K<2) THEN
         M%P_0(K) = P_INF
      ELSE
         INTEGRAL = INTEGRAL + (GVEC(3)/(RSUM0*0.5_EB*(M%TMP_0(K-1)+M%TMP_0(K))))*(M%ZC(K)-M%ZC(K-1))
         M%P_0(K) = P_INF *EXP(INTEGRAL)
      ENDIF
   ENDDO
ELSE
   M%TMP_0(:) = TMPA
