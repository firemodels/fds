...
\be \dod{ }{t}(\rho h_s) + \nabla\!\cdot (\rho h_s \bu) = \frac{\mbox{D}\bp}{\mbox{D}t}  + \dq''' - \dq_b'''
...
\Div\mathbf{u} = \frac{1}{\rho h_s} \left[ \frac{\mbox{D}}{\mbox{D} t}(\bp-\rho h_s) + \dq^\ppp + \dq_r^\ppp - \dq_b^\ppp - \Div \dot{\mathbf{q}}^\pp \right]
\ee
