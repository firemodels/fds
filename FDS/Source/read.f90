...
LOGICAL :: SUCCESS,EX,THICKEN_OBSTRUCTIONS,BAD,BACKGROUND_SMIX_DECLARED=.FALSE.,IDEAL=.FALSE.,&
...
DO NM=1,NMESHES
   ...
   DO NM2=1,NMESHES
      ...
      IF (M2%XS>M%XF+TWO_EPSILON_EB .OR. M2%XF<M%XS-TWO_EPSILON_EB .OR. &
          ...
          M2%ZS>M%ZF+TWO_EPSILON_EB .OR. M2%ZF<M%ZS-TWO_EPSILON_EB) CYCLE
...
NAMELIST /TIME/ DT,EVAC_DT_FLOWFIELD,EVAC_DT_STEADY_STATE,FYI,LIMITING_DT_RATIO,LOCK_TIME_STEP,RESTRICT_TIME_STEP, &
                T_BEGIN,T_END,T_END_GEOM,TIME_SHRINK_FACTOR,WALL_INCREMENT, &
...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                BAROCLINIC,BNDF_DEFAULT,CC_IBM,CNF_CUTOFF,CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,&
                ...
                DNS,DRAG_CFL_MAX,DT_MEAN_FORCING,ENTHALPY_TRANSPORT,EQUILIBRIUM_MODEL,&
                ...
                GRAVITATIONAL_SETTLING,GROUND_LEVEL,GVEC,DT_HVAC,H_F_REFERENCE_TEMPERATURE,HRRPUV_MAX_SMV,&
                HUMIDITY,HVAC_MASS_TRANSPORT,&
                ...
                N_FIXED_CHEMISTRY_SUBSTEPS,NEW_OPEN_BOUNDARY,NOISE,NOISE_VELOCITY,NO_EVACUATION,NO_RAMPS,NORTHANGLE,&
                OVERWRITE,PARTICLE_CFL_MAX,PARTICLE_CFL_MIN,PARTICLE_CFL,PERIODIC_TEST,PROFILING,POROUS_FLOOR,&
                POTENTIAL_TEMPERATURE_CORRECTION,PR,PROJECTION,P_INF,&
...
PLOOP: DO N=1,5
   CALL GET_QUANTITY_INDEX(PLOT3D_SMOKEVIEW_LABEL(N),PLOT3D_SMOKEVIEW_BAR_LABEL(N),PLOT3D_QUANTITY_INDEX(N),I_DUM, &
                           PLOT3D_Y_INDEX(N),PLOT3D_Z_INDEX(N),PLOT3D_PART_INDEX(N),I_DUM,I_DUM,I_DUM,'PLOT3D', &
...
IF (SMOKE3D) THEN
   CALL GET_QUANTITY_INDEX(SMOKE3D_SMOKEVIEW_LABEL,SMOKE3D_SMOKEVIEW_BAR_LABEL,SMOKE3D_QUANTITY_INDEX,I_DUM, &
                           SMOKE3D_Y_INDEX,SMOKE3D_Z_INDEX,I_DUM,I_DUM,I_DUM,I_DUM,'SMOKE3D', &
...
REAL(EB) :: CP1,CP2,H1,H2,H_REF_SENSIBLE(1:N_TRACKED_SPECIES)
INTEGER :: NN,N
TYPE(SPECIES_TYPE), POINTER :: SS=>NULL()
TYPE(SPECIES_MIXTURE_TYPE), POINTER :: SM=>NULL()
...
! Adjust CP_AVG_Z to 0 at the H_F_REFERENCE_TEMPERATURE
J = INT(H_F_REFERENCE_TEMPERATURE)
H_REF_SENSIBLE(:) = H_SENS_Z(J,:)+(H_F_REFERENCE_TEMPERATURE-REAL(J,EB))*(H_SENS_Z(J+1,:)-H_SENS_Z(J,:))
...
CHARACTER(LABEL_LENGTH) :: FUEL,RADCAL_ID='null',SPEC_ID_NU(MAX_SPECIES),SPEC_ID_N_S(MAX_SPECIES),RAMP_FS,TABLE_FS,ALT_REAC_ID,&
                           RAMP_CHI_R
CHARACTER(100) :: FORMULA
...
! Alternate reaction pathway due to extinction

DO NR = 1,N_REACTIONS
   NRR_LOOP: DO NRR = NR+1,N_REACTIONS
      REACTION(NRR)%ALT_INDEX = -1
      IF ( TRIM(REACTION(NR)%ID) == 'null' ) CYCLE NRR_LOOP
      IF ( TRIM(REACTION(NRR)%ALT_REAC_ID) == TRIM(REACTION(NR)%ID) ) THEN
         REACTION(NRR)%ALT_INDEX = NR
         EXIT NRR_LOOP
      ENDIF
   ENDDO NRR_LOOP
ENDDO
...
INTEGER :: NS,NR,HF_COUNT
REAL(EB) :: H_F_OLD(1:N_TRACKED_SPECIES)
...
INTEGER :: SAMPLING_FACTOR,N,NN,ILPC,IPC,RGB(3),I_DUMMY(7),N_STRATA,N_LAGRANGIAN_CLASSES_READ
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   LPC%KILL_RADIUS        = MINIMUM_DIAMETER*0.5_EB*1.E-6_EB*0.171_EB
   ...
   IF (ANY(LPC%QUANTITIES/='null')) THEN
      QUANTITIES_LOOP: DO N=1,10
         ...
         CALL GET_QUANTITY_INDEX(LPC%SMOKEVIEW_LABEL(LPC%N_QUANTITIES),LPC%SMOKEVIEW_BAR_LABEL(LPC%N_QUANTITIES), &
                                 ...
                                 I_DUMMY(4),I_DUMMY(5),I_DUMMY(6),I_DUMMY(7),'PART', &
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BEAD_DENSITY,BEAD_DIAMETER,BEAD_EMISSIVITY,&
                ...
                CHARACTERISTIC_VELOCITY,C_FACTOR,FLOW_RAMP,FLOW_RATE,FLOW_TAU,GAUGE_EMISSIVITY,GAUGE_TEMPERATURE,ID,&
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   IF (SF%THERMALLY_THICK_HT3D) THEN
      IF ( SF%NET_HEAT_FLUX < 1.E12_EB .OR. ABS(SF%CONVECTIVE_HEAT_FLUX) > TWO_EPSILON_EB .OR. TMP_FRONT >= -TMPM ) THEN
         WRITE(MESSAGE,'(A,I3)') 'ERROR: SURF '//TRIM(SF%ID)// ' cannot have HT3D with specified TMP or FLUX bc'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
...
SURF_GRID_LOOP: DO SURF_INDEX=0,N_SURF
   ...
   LAYER_LOOP: DO NL=1,SF%N_LAYERS
      ...
      DO N = 1,SF%N_MATL
         ...
         DO NN = 1,SF%N_LAYER_MATL(NL)
            IF (ML%PYROLYSIS_MODEL==PYROLYSIS_SOLID .AND. SF%LAYER_MATL_INDEX(NL,NN)==SF%MATL_INDEX(N)) THEN
      ...
      SWELL_RATIO = 1._EB
      IF (SF%PYROLYSIS_MODEL==PYROLYSIS_MATERIAL .AND. DENSITY_MIN>TWO_EPSILON_EB) SWELL_RATIO = DENSITY_MAX/DENSITY_MIN
...
NAMELIST /PRES/ CHECK_POISSON,FISHPAK_BC,LAPLACE_PRESSURE_CORRECTION, &
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,INITIAL_RADIATION_ITERATIONS,KAPPA0,NMIEANG,NUMBER_RADIATION_ANGLES,PATH_LENGTH,&
                RADIATION,RADIATION_ITERATIONS,RADIATIVE_FRACTION,RADTMP,RTE_SOURCE_CORRECTION,TIME_STEP_INCREMENT,&
...
CHARACTER(LABEL_LENGTH) :: QUANTITY,QUANTITY2,PROP_ID,CTRL_ID,DEVC_ID,INIT_ID,SURF_ID,STATISTICS,PART_ID,MATL_ID,SPEC_ID,UNITS, &
                 DUCT_ID,NODE_ID(2),X_ID,Y_ID,Z_ID,NO_UPDATE_DEVC_ID,NO_UPDATE_CTRL_ID,REAC_ID
...
NAMELIST /DEVC/ BYPASS_FLOWRATE,CONVERSION_FACTOR,COORD_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,DUCT_ID,EVACUATION,FLOWRATE,FYI,&
                ...
                REAC_ID,RELATIVE,ROTATION,SETPOINT,SMOOTHING_FACTOR,SPEC_ID,STATISTICS,STATISTICS_START,SURF_ID,&
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   QUANTITY_IF: IF (DV%QUANTITY /= 'null') THEN
      ...
      IF (TRIM(DV%QUANTITY)=='NODE PRESSURE DIFFERENCE') THEN
         CALL GET_QUANTITY_INDEX(DV%SMOKEVIEW_LABEL,DV%SMOKEVIEW_BAR_LABEL,QUANTITY_INDEX,I_DUM, &
                                 DV%Y_INDEX,DV%Z_INDEX,DV%PART_INDEX,DV%DUCT_INDEX,DV%NODE_INDEX(2),I_DUM,'DEVC', &
...
READ_ISOF_LOOP: DO N=1,N_ISOF
   ...
   CALL GET_QUANTITY_INDEX(IS%SMOKEVIEW_LABEL,IS%SMOKEVIEW_BAR_LABEL,IS%INDEX,I_DUM, &
                           IS%Y_INDEX,IS%Z_INDEX,I_DUM,I_DUM,I_DUM,I_DUM,'ISOF', &
...
REAL(EB) :: AGL_SLICE
INTEGER :: N,NN,NM,MESH_NUMBER,N_SLCF_O,NITER,ITER,VELO_INDEX,I_DUM,IOR
...
NAMELIST /SLCF/ AGL_SLICE,CELL_CENTERED,EVACUATION,FACE_CENTERED,FIRE_LINE,FYI,ID,IOR,LEVEL_SET_FIRE_LINE,MAXIMUM_VALUE,&
                MESH_NUMBER,MINIMUM_VALUE,PART_ID,PBX,PBY,PBZ,PROP_ID,QUANTITY,QUANTITY2,REAC_ID,SLICETYPE,SPEC_ID,VECTOR,&
                VELO_INDEX,XB
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         CALL GET_QUANTITY_INDEX(SL%SMOKEVIEW_LABEL,SL%SMOKEVIEW_BAR_LABEL,SL%INDEX,SL%INDEX2, &
                                 SL%Y_INDEX,SL%Z_INDEX,SL%PART_INDEX,I_DUM,I_DUM,SL%REAC_INDEX,'SLCF', &
...
READ_BNDF_LOOP: DO N=1,N_BNDF
   ...
   CALL GET_QUANTITY_INDEX(BF%SMOKEVIEW_LABEL,BF%SMOKEVIEW_BAR_LABEL,BF%INDEX,I_DUM, &
                           BF%Y_INDEX,BF%Z_INDEX,BF%PART_INDEX,I_DUM,I_DUM,I_DUM,'BNDF', &
...
READ_BNDE_LOOP: DO N=1,N_BNDE
   ...
   CALL GET_QUANTITY_INDEX(BE%SMOKEVIEW_LABEL,BE%SMOKEVIEW_BAR_LABEL,BE%INDEX,I_DUM, &
                           BE%Y_INDEX,BE%Z_INDEX,BE%PART_INDEX,I_DUM,I_DUM,I_DUM,'BNDE', &
                           QUANTITY,'null',SPEC_ID,PART_ID,'null','null','null')
...
INTEGER :: I
REAL(EB) :: CP_G,CP_G_O,CP_L,CP_L_O,H_G,H_L,H_G_0,H_L_0,G_F,RCON,H_V,T_R,T_M,T_B,DENSITY
LOGICAL :: FUEL

DO I=0,5000
   CALL JANAF_TABLE (I,CP_G,H_G_0,WATER_VAPOR,RCON,FUEL,G_F)
   CALL JANAF_TABLE_LIQUID (I,CP_L,H_V,H_L_0,T_R,T_M,T_B,WATER_VAPOR,FUEL,DENSITY)
   IF (I==0) THEN
      H_G = H_G_0
      H_L = H_L_0
   ELSE
