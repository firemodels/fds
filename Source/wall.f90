...
REAL(EB), INTENT(IN) :: T,DT
REAL(EB) :: DT_BC,DTMP,DT_BC_HT3D,TMP_FRONT,WGT,VEL_T,TSI
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   VEL_T = 0._EB
   IF (SF%SPECIFIED_TANGENTIAL_VELOCITY) THEN
      IF (ABS(SF%T_IGN-T_BEGIN)<=SPACING(SF%T_IGN) .AND. SF%RAMP_INDEX(TIME_VELO)>=1) THEN
         TSI = T
      ELSE
         TSI = T-SF%T_IGN
      ENDIF
      VEL_T = SQRT(SF%VEL_T(1)**2 + SF%VEL_T(2)**2) * EVALUATE_RAMP(TSI,SF%TAU(TIME_VELO),SF%RAMP_INDEX(TIME_VELO))
   ENDIF
   ONE_D%U_TANG  = SQRT(2._EB*KRES(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG)) - VEL_T
...
IF (SOLID_PARTICLES) THEN
   DO IP = 1, NLP
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN  ! Target particles are included to get gas phase values
         ...
         ELSE
            ONE_D%U_TANG  = SQRT(2._EB*KRES(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG)) - 0.5_EB*(LP%U**2+LP%V**2+LP%W**2)
         ENDIF
