...
REAL(EB), POINTER, DIMENSION(:,:,:) :: RHOP=>NULL(),UP=>NULL(),VP=>NULL(),WP=>NULL(), &
                                       ...
                                       UU=>NULL(),VV=>NULL(),WW=>NULL(),MU_DNS=>NULL()
...
!$OMP        LES,NM,C_SMAGORINSKY,TWO_D,DX,DY,DZ,USE_MAX_FILTER_WIDTH,RDX,RDY,RDZ,UU,VV,WW,RHOP,CSD2, &
!$OMP        N_EXTERNAL_WALL_CELLS,N_INTERNAL_WALL_CELLS,BOUNDARY_TYPE,IJKW,KRES, &
!$OMP        IBP1,JBP1,KBP1) &
!$OMP PRIVATE(ZZ_GET,CSD2)
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   IF (BOUNDARY_TYPE(IW)==NULL_BOUNDARY) CYCLE WALL_LOOP
   II  = IJKW(1,IW)
   JJ  = IJKW(2,IW)
   KK  = IJKW(3,IW)
   IOR = IJKW(4,IW)
   ...
   SELECT CASE(BOUNDARY_TYPE(IW))
      CASE(SOLID_BOUNDARY)
         IF (LES) THEN
            SELECT CASE (IOR)
               CASE ( 1); MU(IIG,JJG,KKG) = MAX(MU_DNS(IIG,JJG,KKG),ONTH*MU(IIG+1,JJG,KKG))
               CASE (-1); MU(IIG,JJG,KKG) = MAX(MU_DNS(IIG,JJG,KKG),ONTH*MU(IIG-1,JJG,KKG))
               CASE ( 2); MU(IIG,JJG,KKG) = MAX(MU_DNS(IIG,JJG,KKG),ONTH*MU(IIG,JJG+1,KKG))
               CASE (-2); MU(IIG,JJG,KKG) = MAX(MU_DNS(IIG,JJG,KKG),ONTH*MU(IIG,JJG-1,KKG))
               CASE ( 3); MU(IIG,JJG,KKG) = MAX(MU_DNS(IIG,JJG,KKG),ONTH*MU(IIG,JJG,KKG+1))
               CASE (-3); MU(IIG,JJG,KKG) = MAX(MU_DNS(IIG,JJG,KKG),ONTH*MU(IIG,JJG,KKG-1))
            END SELECT
         ENDIF
...
INTEGER :: I,J,K,IOR,IIG,JJG,KKG,IW,IBC
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   FREE_SLIP_IF: IF (SURFACE(IBC)%VELOCITY_BC_INDEX==FREE_SLIP_BC) THEN
      SELECT CASE(ABS(IOR))
         CASE(1)
            DVDX = 0._EB
            DWDX = 0._EB
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (NOM/=0 .OR. BOUNDARY_TYPE(IW)==SOLID_BOUNDARY) THEN
      SELECT CASE(IOR)
         CASE( 1) 
...
EXTERNAL_WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS
   ...
   U_GHOST(IW) = U_GHOST(IW)/REAL((IJKW(13,IW)-IJKW(10,IW)+1)*(IJKW(14,IW)-IJKW(11,IW)+1)*(IJKW(15,IW)-IJKW(12,IW)+1),EB)
   V_GHOST(IW) = V_GHOST(IW)/REAL((IJKW(13,IW)-IJKW(10,IW)+1)*(IJKW(14,IW)-IJKW(11,IW)+1)*(IJKW(15,IW)-IJKW(12,IW)+1),EB)
   W_GHOST(IW) = W_GHOST(IW)/REAL((IJKW(13,IW)-IJKW(10,IW)+1)*(IJKW(14,IW)-IJKW(11,IW)+1)*(IJKW(15,IW)-IJKW(12,IW)+1),EB)
...
UNSTRUCTURED_GEOMETRY_LOOP: DO TRI_INDEX=1,N_FACE
   ...
   P_CELL_LOOP: DO
      ...
      TMP_SUM = TMP_SUM + TMP(I,J,K) ! for testing, set face tmp to gas cell tmp
      N_CELLS = N_CELLS + 1
