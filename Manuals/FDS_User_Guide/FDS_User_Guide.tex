...
By default, FDS will suspend the pressure iterations if the error is not reduced by at least 25~\% at each step. If you do not want FDS to suspend the pressure iteration, set {\ct SUSPEND\_PRESSURE\_ITERATIONS} to {\ct .FALSE.} on the {\ct PRES} line. This will be done automatically if you change the default values of {\ct VELOCITY\_TOLERANCE} or {\ct MAX\_PRESSURE\_ITERATIONS}.
...
The method is invoked by adding {\ct {HT3D=T}} to an {\ct {OBST}} line.  The {\ct {OBST}} line must then also have a {\ct {MATL\_ID}} associated.  If two-way coupling with the gas phase is desired, then the {\ct {SURF}} associated with the {\ct {OBST}} should also have {\ct {HT3D=T}}.  Note that if the FDS time step is too large then the accuracy of two-way coupling can be poor.  You may need to set {\ct {DT}} on the {\ct {TIME}} line to be on the same order as the {\ct VN} time step restriction based on the thermal diffusivity of the solid (this is problem dependent, see Sec.~\ref{info:CFL}).  Below is an example input file:

\begin{lstlisting}
&MATL ID='steel', .../
&SURF ID='s1', HT3D=T/
&OBST XB=..., HT3D=T, MATL_ID='steel', SURF_ID='s1'/
\end{lstlisting}
