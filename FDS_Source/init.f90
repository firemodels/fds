...
IF (OBST_INDEX>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   WC%ONE_D%AREA_ADJUST = OB%INPUT_AREA(ABS(IOR))/OB%FDS_AREA(ABS(IOR))
   IF (WC%ONE_D%AREA_ADJUST<=TWO_EPSILON_EB) WC%ONE_D%AREA_ADJUST = 1._EB
   IF (OB%MASS>1.E5_EB) THEN
      OB%MASS = SF%SURFACE_DENSITY*WC%AW*WC%ONE_D%AREA_ADJUST
      IF (OB%BULK_DENSITY>0._EB) OB%MASS = OB%BULK_DENSITY*(OB%X2-OB%X1)*(OB%Y2-OB%Y1)*(OB%Z2-OB%Z1)/OB%VOLUME_ADJUST
   ENDIF
ENDIF
...
IF (OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLOW)>=TWO_EPSILON_EB .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   OB=>M%OBSTRUCTION(OBST_INDEX)
   WC%UW0 = SF%VOLUME_FLOW*(OB%INPUT_AREA(ABS(IOR))/OB%UNDIVIDED_INPUT_AREA(ABS(IOR))) / OB%FDS_AREA(ABS(IOR))
ENDIF
