...
\begin{figure}[ht]
\center{\includegraphics[width=4.0in]{FIGURES/pyrolysis}}
...
Two additional parameters can be specified along with {\ct REFERENCE\_TEMPERATURE} to better define the range of temperatures over
which pyrolysis occurs. First, the {\ct TGA\_HEATING\_RATE} is the rate at which the temperature of the TGA (or equivalent) test apparatus was increased, in units
...
{\ct TGA\_HEATING\_RANGE} is the approximate width (in degrees Celsius or Kelvin) of the green curve, assuming its shape to be roughly triangular. Its default value
is 80~$^\circ$C. These two parameters merely help to fine tune the shape of the curve, assuming some sort of experiment has been done to indicate at what temperature, and
over what temperature range, the reaction takes place. As with $A$ and $E$ above, if this information is not available, only specify the 
{\ct REFERENCE\_TEMPERATURE}.
