  ...
  *maxfullbuffer=0;
  ...
  for(;;){
    ...
    for(i=0;i<*nisolevels;i++){
      if(fgets(buffer,255,isosizestream)==NULL)break;
      if(istep%isoframestep!=0||(settmin_i==1&&time<tmin_i)||(settmax_i==1&&time>tmax_i)){
      }
      else{
...
void drawstaticiso(const isosurface *asurface,int surfacetype, int smoothnorm){
  int j,k;
  float vv1[3],vv2[3],vv3[3];
  float vv1n[3],vv2n[3],vv3n[3];
  short *norm=NULL;
...
int isocompare( const void *arg1, const void *arg2 ){
  ...
  if(strcmp(isoi->label.longlabel,isoj->label.longlabel)<0)return -1;
  if(strcmp(isoi->label.longlabel,isoj->label.longlabel)>0)return 1;
...
void updateisomenulabels(void){
  ...
  if(niso>0){
    ...
    for(i=0;i<niso;i++){
      isoi = isoinfo + i;
      STRCPY(isoi->menulabel,isoi->label.longlabel);
      if(nmeshes>1){
