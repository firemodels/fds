...
$\theta$ to assume anything other than a uniform\footnote{A uniform distribution means that there is an equal likelihood that the random variable takes on any value
within the range of possibilities.} distribution. This is equivalent to saying that the modeler has not biased the model input parameters to compensate for a known
bias in the model output. For example, if a model has been shown to over-predict the compartment temperature, and the modeler has reduced the specified heat release
rate as a result, then it can no longer be assumed that the prior distribution of the true temperature is uniform, and the
methodology for estimating the true value is no longer valid. Still another way to understand this is by analogy to target shooting. If a given rifle 
has a defect such that, on average, it shoots 10~cm to the left of a given target, then it must be assumed that a marksman without this knowledge was aiming 
10~cm to the right of the bullet hole. 
