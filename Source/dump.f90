...
REAL(EB) :: Q_CON,VOLSUM,MFT,ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,DEPTH,UN,H_S,RHO_D_DYDN,U_CELL,V_CELL,W_CELL,&
            ...
            AAA,DDD,EEE,BETA_Q,GAMMA_Q,PPP,QQQ,RRR,YYY,UUU,WWW
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(23) ! ADIABATIC SURFACE TEMPERATURE (Ferrari's Method for Solving the Quartic)
      ...
      IF (ONE_D%HEAT_TRANS_COEF+H_W_EFF>1.E-5_EB .AND. ABS(ONE_D%Q_RAD_IN-AAA*TMP(BC%IIG,BC%JJG,BC%KKG)**4)>5.E-3_EB) THEN
         DDD = ONE_D%HEAT_TRANS_COEF + H_W_EFF
         EEE = -ONE_D%Q_RAD_IN - ONE_D%HEAT_TRANS_COEF*TMP(BC%IIG,BC%JJG,BC%KKG) - H_W_EFF*LTMP
         BETA_Q = DDD/AAA
         GAMMA_Q = EEE/AAA
         PPP = -GAMMA_Q
         QQQ = -0.125_EB*BETA_Q**2
         RRR = -0.5_EB*QQQ+SQRT(0.25*QQQ**2+PPP**3/27._EB)
         IF (ABS(RRR) <TWO_EPSILON_EB) THEN
            YYY = QQQ**ONTH
         ELSE
            UUU = RRR**ONTH
         ...
         ENDIF
         WWW = SQRT(2._EB*YYY)
         SOLID_PHASE_OUTPUT = 0.5_EB*(-WWW+SQRT(2._EB*(BETA_Q/WWW-YYY))) - TMPM
      ELSE
