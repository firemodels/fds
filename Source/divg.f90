...
USE CC_SCALARS, ONLY : ADD_CUTCELL_PSUM,ADD_LINKEDCELL_PSUM,SET_EXIMDIFFLX_3D,SET_EXIMRHOHSLIM_3D,&
                       SET_EXIMRHOZZLIM_3D,CC_DIVERGENCE_PART_1,CFACE_PREDICT_NORMAL_VELOCITY
         ...
         BOUNDARY_TYPE_SELECT: SELECT CASE(WC%BOUNDARY_TYPE)
            ...
            CASE(OPEN_BOUNDARY,INTERPOLATED_BOUNDARY)
               ...
               IF (EWC%NIC>1) THEN
                  ...
                  SELECT CASE(IOR)
                     CASE( 1); RHO_D_DZDX(IIG-1,JJG,KKG,N) =  P1%RHO_D_DZDN_F(N)
                     CASE(-1); RHO_D_DZDX(IIG,JJG,KKG,N)   = -P1%RHO_D_DZDN_F(N)
                     CASE( 2); RHO_D_DZDY(IIG,JJG-1,KKG,N) =  P1%RHO_D_DZDN_F(N)
                     CASE(-2); RHO_D_DZDY(IIG,JJG,KKG,N)   = -P1%RHO_D_DZDN_F(N)
                     CASE( 3); RHO_D_DZDZ(IIG,JJG,KKG-1,N) =  P1%RHO_D_DZDN_F(N)
                     CASE(-3); RHO_D_DZDZ(IIG,JJG,KKG,N)   = -P1%RHO_D_DZDN_F(N)
                  END SELECT
               ELSE
                  ! store computed flux for output
                  SELECT CASE(IOR)
                     CASE( 1); P1%RHO_D_DZDN_F(N) =  RHO_D_DZDX(IIG-1,JJG,KKG,N)
                     CASE(-1); P1%RHO_D_DZDN_F(N) = -RHO_D_DZDX(IIG,JJG,KKG,N)
                     CASE( 2); P1%RHO_D_DZDN_F(N) =  RHO_D_DZDY(IIG,JJG-1,KKG,N)
                     CASE(-2); P1%RHO_D_DZDN_F(N) = -RHO_D_DZDY(IIG,JJG,KKG,N)
                     CASE( 3); P1%RHO_D_DZDN_F(N) =  RHO_D_DZDZ(IIG,JJG,KKG-1,N)
                     CASE(-3); P1%RHO_D_DZDN_F(N) = -RHO_D_DZDZ(IIG,JJG,KKG,N)
                  END SELECT
               ENDIF
...
CORRECTION_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
      P1%K_G = 0.5_EB*(KP(IIG,JJG,KKG)+KP(II,JJ,KK))
      CYCLE CORRECTION_LOOP
   ELSE
      P1%K_G = KP(IIG,JJG,KKG)
   ENDIF
   IOR = BC%IOR
   SELECT CASE(IOR)
      CASE( 1)
   ...
   DP(IIG,JJG,KKG) = DP(IIG,JJG,KKG) - ( P1%AREA_ADJUST*P1%Q_CON_F*P1%RDN - WC%Q_LEAK )
...
IF_PRESSURE_ZONES: IF (N_ZONE>0) THEN
   ...
   WALL_LOOP4: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      P1 => BOUNDARY_PROP1(WC%P1_INDEX)
      IPZ = P1%PRESSURE_ZONE
      IF (IPZ<1) CYCLE WALL_LOOP4
      IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE WALL_LOOP4
      IF (PREDICTOR) USUM(IPZ,NM) = USUM(IPZ,NM) + P1%U_NORMAL_S*P1%AREA
      IF (CORRECTOR) USUM(IPZ,NM) = USUM(IPZ,NM) + P1%U_NORMAL  *P1%AREA
   ENDDO WALL_LOOP4
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      ...
      CASE(SOLID_BOUNDARY)
         IF (PREDICTOR) UN = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL_S
         IF (CORRECTOR) UN = -SIGN(1._EB,REAL(IOR,EB))*P1%U_NORMAL
      CASE(INTERPOLATED_BOUNDARY)
         UN = UVW_SAVE(IW)
   END SELECT

   DU = (P1%RHO_F*H_S - RHO_H_S_P(IIG,JJG,KKG))*UN
   U_DOT_DEL_RHO_H_S(IIG,JJG,KKG) = U_DOT_DEL_RHO_H_S(IIG,JJG,KKG) - SIGN(1._EB,REAL(IOR,EB))*DU*P1%RDN
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   DU = (P1%RHO_F*P1%ZZ_F(N) - RHO_Z_P(IIG,JJG,KKG))*UN
   U_DOT_DEL_RHO_Z(IIG,JJG,KKG) = U_DOT_DEL_RHO_Z(IIG,JJG,KKG) - SIGN(1._EB,REAL(IOR,EB))*DU*P1%RDN
...
PREDICT_NORMALS: IF (PREDICTOR) THEN
      ...
      WALL_CELL_TYPE: SELECT CASE (WC%BOUNDARY_TYPE)
         ...
         CASE (SOLID_BOUNDARY)
            ...
            NEUMANN_IF: IF (SF%SPECIFIED_NORMAL_GRADIENT) THEN
               SELECT CASE(BC%IOR)
                  CASE( 1); P1%U_NORMAL_S =-(U(BC%IIG,BC%JJG,BC%KKG)   + SF%VEL_GRAD*P1%RDN)
                  CASE(-1); P1%U_NORMAL_S = (U(BC%IIG-1,BC%JJG,BC%KKG) + SF%VEL_GRAD*P1%RDN)
                  CASE( 2); P1%U_NORMAL_S =-(V(BC%IIG,BC%JJG,BC%KKG)   + SF%VEL_GRAD*P1%RDN)
                  CASE(-2); P1%U_NORMAL_S = (V(BC%IIG,BC%JJG-1,BC%KKG) + SF%VEL_GRAD*P1%RDN)
                  CASE( 3); P1%U_NORMAL_S =-(W(BC%IIG,BC%JJG,BC%KKG)   + SF%VEL_GRAD*P1%RDN)
                  CASE(-3); P1%U_NORMAL_S = (W(BC%IIG,BC%JJG,BC%KKG-1) + SF%VEL_GRAD*P1%RDN)
               END SELECT
            ENDIF NEUMANN_IF
            IF (ABS(SURFACE(WC%SURF_INDEX)%MASS_FLUX_TOTAL)>=TWO_EPSILON_EB) P1%U_NORMAL_S = &
            ...
            VENT_IF: IF (WC%VENT_INDEX>0) THEN
               ...
               IF (VT%N_EDDY>0) THEN ! Synthetic Eddy Method
                  IF (SF%PROFILE/=0 .AND. ABS(SF%VEL)>TWO_EPSILON_EB) THEN
                     PROFILE_FACTOR = ABS(P1%U_NORMAL_0/SF%VEL)
                  ELSE
                     PROFILE_FACTOR = 1._EB
                  ENDIF
                  SELECT CASE(BC%IOR)
                     CASE( 1); P1%U_NORMAL_S = P1%U_NORMAL_S - TIME_RAMP_FACTOR*VT%U_EDDY(BC%JJ,BC%KK)*PROFILE_FACTOR
                     CASE(-1); P1%U_NORMAL_S = P1%U_NORMAL_S + TIME_RAMP_FACTOR*VT%U_EDDY(BC%JJ,BC%KK)*PROFILE_FACTOR
                     CASE( 2); P1%U_NORMAL_S = P1%U_NORMAL_S - TIME_RAMP_FACTOR*VT%V_EDDY(BC%II,BC%KK)*PROFILE_FACTOR
                     CASE(-2); P1%U_NORMAL_S = P1%U_NORMAL_S + TIME_RAMP_FACTOR*VT%V_EDDY(BC%II,BC%KK)*PROFILE_FACTOR
                     CASE( 3); P1%U_NORMAL_S = P1%U_NORMAL_S - TIME_RAMP_FACTOR*VT%W_EDDY(BC%II,BC%JJ)*PROFILE_FACTOR
                     CASE(-3); P1%U_NORMAL_S = P1%U_NORMAL_S + TIME_RAMP_FACTOR*VT%W_EDDY(BC%II,BC%JJ)*PROFILE_FACTOR
                  END SELECT
               ENDIF
            ENDIF VENT_IF
         ...
         CASE(OPEN_BOUNDARY,INTERPOLATED_BOUNDARY)
            SELECT CASE(BC%IOR)
               CASE( 1); P1%U_NORMAL_S = -U(BC%II,BC%JJ,BC%KK)
               CASE(-1); P1%U_NORMAL_S =  U(BC%II-1,BC%JJ,BC%KK)
               CASE( 2); P1%U_NORMAL_S = -V(BC%II,BC%JJ,BC%KK)
               CASE(-2); P1%U_NORMAL_S =  V(BC%II,BC%JJ-1,BC%KK)
               CASE( 3); P1%U_NORMAL_S = -W(BC%II,BC%JJ,BC%KK)
               CASE(-3); P1%U_NORMAL_S =  W(BC%II,BC%JJ,BC%KK-1)
            END SELECT
   ...
   DO IW=1,N_EXTERNAL_WALL_CELLS
      WC => WALL(IW)
      P1 => BOUNDARY_PROP1(WC%P1_INDEX)
      WC%DUNDT = RDT*(P1%U_NORMAL_S-P1%U_NORMAL)
   ENDDO
...
ELSE PREDICT_NORMALS
   ...
   DO IW=1,N_EXTERNAL_WALL_CELLS
      WC => WALL(IW)
      P1 => BOUNDARY_PROP1(WC%P1_INDEX)
      WC%DUNDT = WC%DUNDT + 2._EB*RDT*(P1%U_NORMAL-P1%U_NORMAL_S)
   ENDDO
