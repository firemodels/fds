...
by:
\be
   \hat{Y}_{\OTWO,lim} = \frac{ \overline{c_p} (T_{f,lim}-T_m) }{\Delta H/r_\OTWO} \approx  \frac{ 1.2 \, (1700-T_m) }{ 13100}  \label{extinction_model}
\ee
...
Let $m_{\rm p}$ denote the particle mass, $\mathbf{u}_{\rm p}$ the particle velocity, $A_{\rm p}$ the particle cross-sectional area, $C_{\text{d},p}$ the particle drag coefficient, $\rho_\text{a}$ the fluid mass density, $\mathbf{U}_{\rm p}$ the fluid velocity around the particle, $V_\text{g}$ is the volume occupied by the fluid in a cell, and $M \equiv \rho_\text{a} V_\text{g}$ the fluid mass of a cell, $n_\text{p}$ is the number of particles in a cell, $M_\text{p} \equiv M/n_\text{p}$ is the average fluid mass per particle in a cell, and $\mathbf{g}$ is the gravitational acceleration vector.

The equations of motion of the particles and fluid are formulated as follows from Newton's second law,
\begin{align}
    \label{particle_eom}
    m_{\rm p} \frac{\d \mathbf{u}_{\rm p}}{\d t} &= - \frac{1}{2} \rho_\text{a} C_{\text{d},p} A_{\rm p} (\mathbf{u}_{\rm p} - \mathbf{U}_{\rm p}) |\mathbf{u}_{\rm p} - \mathbf{U}_{\rm p}| + m_{\rm p} \mathbf{g} \\
    \label{fluid_eom}
    M_{\rm p} \frac{\d \mathbf{U}_{\rm p}}{\d t} &= \frac{1}{2} \rho_\text{a} C_{\text{d},p} A_{\rm p} (\mathbf{u}_{\rm p} - \mathbf{U}_{\rm p}) |\mathbf{u}_{\rm p} - \mathbf{U}_{\rm p}| \,.
\end{align}
...
If we define $\mathbf{u}_\text{r} \equiv \mathbf{u}_{\rm p} - \mathbf{U}_{\rm p}$ as the relative velocity between the fluid and the particle, we can find a single equation for the relative velocity by dividing both equations by their respective masses (i.e., $m_{\rm p}$ and $M_{\rm p}$) and then subtracting the second from the first. This result is
\begin{align}
    \frac{\d \mathbf{u}_\text{r}}{\d t} = -\frac{1}{2} \rho_\text{a} C_{\text{d},p} A_{\rm p} \left(\frac{1}{m_{\rm p}} + \frac{1}{M_{\rm p}} \right) \mathbf{u}_\text{r} |\mathbf{u}_\text{r}| + \mathbf{g} \,.
\end{align}
The equation above can be written in short as
\begin{align}
    \frac{\d \mathbf{u}_\text{r}}{\d t} = -K_{\rm p} \mathbf{u}_\text{r} |\mathbf{u}_\text{r}| + \mathbf{g} \quad ; \quad K_{\rm p} \equiv \frac{1}{2} \rho_\text{a} C_{d_{\rm p}} A_{\rm p} \left(\frac{1}{m_{\rm p}} + \frac{1}{M_{\rm p}} \right) \,.
\end{align}
...
\begin{align}
    \label{ud_exact}
    \mathbf{u}_\text{d} = \frac{\mathbf{u}_{p,0} - \mathbf{U}_{p,0}}{1 + \beta_{\rm p} t} \quad ; \quad \beta_{\rm p} \equiv K_{\rm p} |\mathbf{u}_{\text{r},0}| \,.
\end{align}
...
\begin{align}
    ...
    %\label{ur_expanded}
    %\mathbf{u}_{\rm p} - \mathbf{U}_{\rm p} &= \frac{\mathbf{u}_{p,0} - \mathbf{U}_{p,0}}{1 + \beta_{\rm p} t} + \mathbf{g} t - \frac{\beta_{\rm p} t^2}{2} \left[(\mathbf{u}_{p,0} - \mathbf{U}_{p,0}) \left(\frac{(\mathbf{u}_{p,0} - \mathbf{U}_{p,0}) \cdot \mathbf{g}}{|\mathbf{u}_{p,0} - \mathbf{U}_{p,0}|^2}\right) + \mathbf{g}\right] + \mathcal{O}(t^3) \quad ; \quad \beta_{\rm p} \equiv K_{\rm p} |\mathbf{u}_{p,0} - \mathbf{U}_{p,0}|
\end{align}

\subsubsection*{Particle velocities and positions}

The results of the previous section are not directly useful unless $\mathbf{U}_{\rm p}(t)$ is known. $\mathbf{U}_{\rm p}(t)$ can be found via the conservation of momentum and this leads to a solution for the particle velocities and positions.

The fluid and particles can gain or lose momentum due to gravity. Drag forces exchange momentum between the fluid and particle, which does not change the total momentum of the system. As gravity is the only force that can change momentum, the time derivative of the fluid-particle system momentum is $m_{\rm p} \mathbf{g}$ by Newton's second law, so we can write
\begin{align}
    m_{\rm p} \mathbf{u}_{\rm p} + M_{\rm p} \mathbf{U}_{\rm p} &= m_{\rm p} \mathbf{u}_{p,0} + M_{\rm p} \mathbf{U}_{p,0} + m_{\rm p} \mathbf{g} t \,, \\
    \label{part_mom_cons}
    \mathbf{u}_{\rm p} + \alpha_{\rm p} \mathbf{U}_{\rm p} &= \mathbf{u}_{p,0} + \alpha_{\rm p} \mathbf{U}_{p,0} + \mathbf{g} t \quad ; \quad \alpha_{\rm p} \equiv \frac{M_{\rm p}}{m_{\rm p}} \,.
\end{align}
%Eq. (\ref{part_mom_cons}) can be combined with Eq. (\ref{ur_short}) to get solution for $\mathbf{u}_{\rm p}$.
Solving for $\mathbf{U}_{\rm p}$ leads to
\begin{align*}
    \mathbf{U}_{\rm p} + \frac{\mathbf{u}_{\text{r},0}}{1 + \beta_{\rm p} t} + \mathbf{u}_\text{g} + \alpha_{\rm p} \mathbf{U}_{\rm p} = \mathbf{u}_{p,0} + \alpha_{\rm p} \mathbf{U}_{p,0} + \mathbf{g} t \,,
\end{align*}
\begin{align}
    \label{mom_U_p}
    \mathbf{U}_{\rm p} = \frac{\mathbf{u}_{p,0} + \alpha_{\rm p} \mathbf{U}_{p,0} + \mathbf{g} t - \mathbf{u}_\text{g}}{1 + \alpha_{\rm p}} - \frac{\mathbf{u}_{\text{r},0}}{(1 + \beta_{\rm p} t)(1 + \alpha_{\rm p})} \,.
\end{align}
Eq. (\ref{mom_U_p}) can be substituted into Eq. (\ref{ur_short}) to get the solution for $\mathbf{u}_{\rm p}$.
\begin{align*}
    \mathbf{u}_{\rm p} &= \mathbf{U}_{\rm p} + \frac{\mathbf{u}_{p,0} - \mathbf{U}_{p,0}}{1 + \beta_{\rm p} t} + \mathbf{u}_\text{g} \\
    &= \frac{\mathbf{u}_{p,0} + \alpha_{\rm p} \mathbf{U}_{p,0} + \mathbf{g} t - \mathbf{u}_\text{g}}{1 + \alpha_{\rm p}} - \frac{\mathbf{u}_{p,0} - \mathbf{U}_{p,0}}{(1 + \beta_{\rm p} t)(1 + \alpha_{\rm p})} + \frac{\mathbf{u}_{p,0} - \mathbf{U}_{p,0}}{1 + \beta_{\rm p} t} + \mathbf{u}_\text{g} \\
    &= \frac{\mathbf{u}_{p,0}}{1 + \beta_{\rm p} t} + \frac{(\mathbf{u}_{p,0} + \alpha_{\rm p} \mathbf{U}_{p,0})\beta_{\rm p} t}{(1 + \beta_{\rm p} t)(1 + \alpha_{\rm p})} + \frac{\mathbf{g} t + \alpha_{\rm p} \mathbf{u}_\text{g}}{1 + \alpha_{\rm p}}
\end{align*}
\begin{align}
    \label{mom_u_p_full}
    \mathbf{u}_{\rm p} = \frac{\mathbf{u}_{p,0}}{1 + \beta_{\rm p} t} + \frac{(\mathbf{u}_{p,0} + \alpha_{\rm p} \mathbf{U}_{p,0})\beta_{\rm p} t}{(1 + \beta_{\rm p} t)(1 + \alpha_{\rm p})} + \mathbf{g} t - \frac{\alpha_{\rm p} \beta_{\rm p} t^2}{2 (1 + \alpha_{\rm p})} \left[\mathbf{u}_{\text{r},0} \left(\frac{\mathbf{u}_{\text{r},0} \cdot \mathbf{g}}{|\mathbf{u}_{\text{r},0}|^2}\right) + \mathbf{g}\right] + \mathcal{O}(t^3)
\end{align}
Integrating Eq. (\ref{mom_u_p_full}) leads to an equation for the particle positions,
\begin{align}
    \label{mom_x_p_full}
    \mathbf{x}_{\rm p} = \mathbf{x}_{p,0} + \left(\frac{\mathbf{u}_{p,0} + \alpha_{\rm p} \mathbf{U}_{p,0}}{1 + \alpha_{\rm p}}\right) t - \frac{\alpha_{\rm p} (\mathbf{u}_{p,0} - \mathbf{U}_{p,0})}{\beta_{\rm p} (1 + \alpha_{\rm p})} \text{ln}(\beta_{\rm p} t + 1) + \frac{\mathbf{g} t^2}{2} - \frac{\alpha_{\rm p} \beta_{\rm p} t^3}{6 (1 + \alpha_{\rm p})} \left[\mathbf{u}_{\text{r},0} \left(\frac{\mathbf{u}_{\text{r},0} \cdot \mathbf{g}}{|\mathbf{u}_{\text{r},0}|^2}\right) + \mathbf{g}\right] + \mathcal{O}(t^4) \,.
\end{align}
...
\begin{align}
    \mathbf{u}_{\rm p}^{n+1} &= \frac{\mathbf{u}_{\rm p}^n}{1 + \beta_{\rm p} \delta t} + \frac{(\mathbf{u}_{\rm p}^n + \alpha_{\rm p} \mathbf{U}_{\rm p}^n)\beta_{\rm p} \delta t}{(1 + \beta_{\rm p} \delta t)(1 + \alpha_{\rm p})} + \mathbf{g} \delta t - \frac{\alpha_{\rm p} \beta_{\rm p} (\delta t)^2}{2 (1 + \alpha_{\rm p})} \left[\mathbf{u}_\text{r}^n \left(\frac{\mathbf{u}_\text{r}^n \cdot \mathbf{g}}{|\mathbf{u}_\text{r}^n|^2}\right) + \mathbf{g}\right] \\
    & \notag \\
    \mathbf{x}_{\rm p}^{n+1} &= \mathbf{x}_{\rm p}^n + \left(\frac{\mathbf{u}_{\rm p}^n + \alpha_{\rm p} \mathbf{U}_{\rm p}^n}{1 + \alpha_{\rm p}}\right) \delta t + \frac{\alpha_{\rm p} (\mathbf{u}_{\rm p}^n - \mathbf{U}_{\rm p}^n)}{\beta_{\rm p} (1 + \alpha_{\rm p})} \text{ln}(\beta_{\rm p} \delta t + 1) + \frac{\mathbf{g} (\delta t)^2}{2}
\end{align}
