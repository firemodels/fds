...
REAL(EB) FUNCTION FLAME_SPEED(TMP,EQ,NR)
...
INTEGER :: I,J,K,AIR_INDEX,FUEL_INDEX,PROD_INDEX,IIG,JJG,KKG,IOR,IW,II,JJ,KK
REAL(EB) :: S,Z_F,DENOM,ZZZ(1:4),DZDX,DZDY,DZDZ
...
! Mass stoichiometric coefficient, F+A->P

AIR_INDEX  = REACTION(1)%AIR_SMIX_INDEX
FUEL_INDEX = REACTION(1)%FUEL_SMIX_INDEX
PROD_INDEX = FUEL_INDEX+1

S = SPECIES_MIXTURE(AIR_INDEX)%MW*REACTION(1)%NU(AIR_INDEX) / &
       ( SPECIES_MIXTURE(FUEL_INDEX)%MW*REACTION(1)%NU(FUEL_INDEX) )

!$OMP PARALLEL PRIVATE(ZZZ,FUEL_INDEX,PROD_INDEX,S)
!$OMP DO SCHEDULE(STATIC)
DO K=0,KBP1
   DO J=0,JBP1
      DO I=0,IBP1
         ZZP(I,J,K) = ZZ(I,J,K,FUEL_INDEX) !+ ZZ(I,J,K,PROD_INDEX)/(1._EB + S) ! scalar mixture fraction
      ENDDO
   ENDDO
ENDDO
!$OMP END DO
...
WALL_LOOP_2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   Z_F = WC%ZZ_F(FUEL_INDEX) !+ WC%ZZ_F(PROD_INDEX)/(1._EB + S)
