...
def buildInputFile(testnum, HRR, H, dx):
    ...
    txt = txt + "&SURF ID='CEILING', EMISSIVITY=0.8, MATL_ID='CERAMIC-FIBER', THICKNESS=0.0254, COLOR = 'GRAY 80', %s /\n"%(suffix2)
    ...
    for ID, QTY in zip(['GHF_profile','HTC_profile','CHF_profile'], ['GAUGE HEAT FLUX', 'HEAT TRANSFER COEFFICIENT', 'CONVECTIVE HEAT FLUX']):
        txt = txt + "&DEVC ID='%s', XB=-2.35,-0.05,0.0,0.0,2.1,2.1, IOR=-3, POINTS=101, QUANTITY='%s', STATISTICS_START=50. /\n"%(ID, QTY)
    txt = txt + "\n"
