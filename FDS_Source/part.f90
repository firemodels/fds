...
WALL_INSERT_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (MASS_SUM > 0._EB) THEN
      ...
      IF (SF%PARTICLE_MASS_FLUX > 0._EB) THEN
         ...
         DO I=NLP-WALL(IW)%NPPCW+1,NLP
            LAGRANGIAN_PARTICLE(I)%PWT = LAGRANGIAN_PARTICLE(I)%PWT*FLOW_RATE*WALL(IW)%AREA_ADJUST*WALL(IW)%AW*SF%DT_INSERT/MASS_SUM
         ENDDO
      ENDIF
   ENDIF
...
ELSEIF (.NOT.LPC%MASSLESS) THEN

   IF (PRESENT(TEMPERATURE)) THEN
      IF (TEMPERATURE > 0._EB) THEN
         ONE_D%TMP(0:SF%N_CELLS+1) = TEMPERATURE
      ELSE
...
IF (LPC%MASSLESS .OR. LP%PWT<=ZERO_P) THEN
   IF (LPC%TURBULENT_DISPERSION) THEN
      DD_X = RHO(LP%IIG+1,LP%JJG,LP%KKG)*MU(LP%IIG+1,LP%JJG,LP%KKG)-RHO(LP%IIG-1,LP%JJG,LP%KKG)*MU(LP%IIG-1,LP%JJG,LP%KKG)
      DD_Y = RHO(LP%IIG,LP%JJG+1,LP%KKG)*MU(LP%IIG,LP%JJG+1,LP%KKG)-RHO(LP%IIG,LP%JJG-1,LP%KKG)*MU(LP%IIG,LP%JJG-1,LP%KKG)
      DD_Z = RHO(LP%IIG,LP%JJG,LP%KKG+1)*MU(LP%IIG,LP%JJG,LP%KKG+1)-RHO(LP%IIG,LP%JJG,LP%KKG-1)*MU(LP%IIG,LP%JJG,LP%KKG-1)
      LP%U = UBAR + DD_X*RDX(LP%IIG)/RHO(LP%IIG,LP%JJG,LP%KKG)
      LP%V = VBAR + DD_Y*RDY(LP%JJG)/RHO(LP%IIG,LP%JJG,LP%KKG)
      LP%W = WBAR + DD_Z*RDZ(LP%KKG)/RHO(LP%IIG,LP%JJG,LP%KKG)
      DD = SQRT(2._EB*MU(LP%IIG,LP%JJG,LP%KKG)*RSC*DT)
         ...
         AGE_IF: IF ((T-LP%T) > MIN(T_BU_BAG,T_BU_STRIP)) THEN
            ...
            ELSE
               DO WHILE (RD >= LP%ONE_D%X(1))
                  CHILD_RADIUS = LPC%BREAKUP_CHILD_DIAMETER*LP%ONE_D%X(1)*LPC%CHILD_R_CDF(:)
...
INTEGER, INTENT(IN) :: NM
INTEGER :: IKILL,I,LOWER_INDEX,UPPER_INDEX,ARRAY_INDEX,ILPC
REAL(EB) :: T,MAX_PART_LIFE
INTEGER, DIMENSION(:), ALLOCATABLE :: KILL_PARTICLE_INDEX
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
...
MAX_PART_LIFE = 1.E6
IF (NLP+INSERT_COUNT>MAXIMUM_PARTICLES .AND. INSERT_RATE>ZERO_P) MAX_PART_LIFE = REAL(MAXIMUM_PARTICLES,EB) / INSERT_RATE
IKILL = 0

ALLOCATE(KILL_PARTICLE_INDEX(1:NLP))
KILL_PARTICLE_INDEX = -1
...
PARTICLE_LOOP: DO I=1,NLP
   ...
   IF (T-LP%T>LPC%LIFETIME .OR. (T-LP%T>MAX_PART_LIFE .AND. IKILL <= INSERT_COUNT .AND. .NOT. LPC%STATIC)) THEN
      IKILL = IKILL + 1
      KILL_PARTICLE_INDEX(IKILL) = I
      CYCLE PARTICLE_LOOP
   ENDIF
...
IF (IKILL > 0) THEN 
   ...
   ! Create a temporary array to hold the particles that are to be shifted in the particle array
   ...
   DO I=1,IKILL
      ...
      LP=>MESHES(NM)%LAGRANGIAN_PARTICLE(KILL_PARTICLE_INDEX(I))  
      LPC=>LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)

      ! Free up the storage arrays holding the particle information

      MESHES(NM)%PARTICLE_STORAGE(LP%CLASS_INDEX)%INTEGERS(:,LP%STORAGE_INDEX) = 0

      ! Determine indices such that the non-rejected particles shift to lower indices but retain same order

      UPPER_INDEX = KILL_PARTICLE_INDEX(I)
   ...
   ! Copy the temporary particle array back into the actual one, then reset pointers.

   NLP = NLP - IKILL

   DO I=1,NLP
      MESHES(NM)%LAGRANGIAN_PARTICLE(I) = TEMP_LAGRANGIAN_PARTICLE(I)
      ILPC = TEMP_LAGRANGIAN_PARTICLE(I)%CLASS_INDEX
      CALL ALLOCATE_STORAGE(NM,LAGRANGIAN_PARTICLE_CLASS(ILPC)%SURF_INDEX,LPC_INDEX=ILPC,LP_INDEX=I,&
                            TAG=TEMP_LAGRANGIAN_PARTICLE(I)%TAG)
   ENDDO
...
DEALLOCATE (KILL_PARTICLE_INDEX)
