...
PART_CLASS_LOOP: DO ILPC=1,N_LAGRANGIAN_CLASSES
   ...
   IF_SIZE_DISTRIBUTION: IF (.NOT.LPC%MONODISPERSE .AND. (LPC%DIAMETER > 0._EB)) THEN
      ...
      ENDIF
      BIN_SIZE = LPC%R_CDF(NDC)/REAL(NSTRATA,EB)
   ...
   IF (LPC%BREAKUP .AND. .NOT.LPC%MONODISPERSE) THEN
      IF(LPC%CHILD_CNF_RAMP_INDEX<0) THEN
         CALL PARTICLE_SIZE_DISTRIBUTION(1._EB,LPC%CHILD_R_CDF(:),LPC%CHILD_CDF(:),NDC, &
                                        LPC%BREAKUP_CHILD_GAMMA,LPC%BREAKUP_CHILD_SIGMA,LPC%CHILD_DISTRIBUTION)
      ELSE
         RM=>RAMPS(LPC%CHILD_CNF_RAMP_INDEX)
         DD=RM%SPAN/NDC
         DO I=1,NDC
               DI=RM%T_MIN+(I-0.5_EB)*DD
               LPC%CHILD_R_CDF(I) = 0.5_EB*DI
               LPC%CHILD_CDF(I)   = EVALUATE_RAMP(DI,0._EB,LPC%CHILD_CNF_RAMP_INDEX) 
         ENDDO
         LPC%CHILD_CDF=LPC%CHILD_CDF/LPC%CHILD_CDF(NDC)
      ENDIF
   ENDIF
...
!      IF (T < PY%PARTICLE_INSERT_CLOCK(NM)) CYCLE SPRINKLER_INSERT_LOOP
   ...
   PARTICLE_INSERT_LOOP: DO I=1,N_INSERT
      ...
      LP%CLASS_INDEX = PY%PART_INDEX     ! The class of particles
      LP%TAG    = PARTICLE_TAG           ! A unique identifying integer
      IF (MOD(NLP,LPC%SAMPLING)==0) THEN ! Decide whether to show the PARTICLE in Smokeview
         LP%SHOW = .TRUE.    
      ELSE
         LP%SHOW = .FALSE.
      ENDIF
      ...
      IF (LPC%SURF_INDEX < 0) THEN
         LP%ONE_D%X = LP%ONE_D%X*D_PRES_FACTOR
   ...
   IF (DROP_SUM > 0) THEN
      ...
      LAGRANGIAN_PARTICLE(NLP-DROP_SUM+1:NLP)%PWT = LAGRANGIAN_PARTICLE(NLP-DROP_SUM+1:NLP)%PWT*PWT0
   ENDIF
...
WALL_INSERT_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   PARTICLE_INSERT_LOOP2: DO I=1,WALL(IW)%NPPCW
      ...
      SELECT CASE (ABS(IOR)) ! Assign particle position
         CASE(1)
      ...
      LP%CLASS_INDEX = ILPC
      LP%TAG         = PARTICLE_TAG
      IF (MOD(NLP,LPC%SAMPLING)==0) THEN
         LP%SHOW = .TRUE.
      ELSE
         LP%SHOW = .FALSE.
      ENDIF
      ...
      CALL MAKE_PARTICLE
      LP=>LAGRANGIAN_PARTICLE(NLP)      
      IF (.NOT. LPC%MASSLESS) MASS_SUM = MASS_SUM + LP%PWT*LPC%FTPR*LP%ONE_D%X(1)**3
   ...
   IF (MASS_SUM > 0._EB) THEN
      IF (SF%PARTICLE_MASS_FLUX > 0._EB) THEN
         ...
         LAGRANGIAN_PARTICLE(NLP-WALL(IW)%NPPCW+1:NLP)%PWT = &
         LAGRANGIAN_PARTICLE(NLP-WALL(IW)%NPPCW+1:NLP)%PWT*FLOW_RATE*WALL(IW)%AREA_ADJUST*WALL(IW)%AW*SF%DT_INSERT/MASS_SUM
      ENDIF
   ENDIF
...
VOLUME_INSERT_LOOP: DO IB=1,N_INIT
   ...
   POINTWISE_IF: IF (IN%TABL_INDEX > 0) THEN
      INSERT_POINTWISE: DO I=1,TABLES(IN%TABL_INDEX)%NUMBER_ROWS
         XYZ = TABLES(IN%TABL_INDEX)%TABLE_DATA(I,1:3)
         IF (XYZ(1)>XF .OR. XYZ(1)<XS .OR. XYZ(2)>YF .OR. XYZ(2)<YS .OR. XYZ(3)>ZF .OR. XYZ(3)<ZS) CYCLE INSERT_POINTWISE
         NLP = NLP + 1
         PARTICLE_TAG = PARTICLE_TAG + NMESHES
         IF (NLP>NLPDIM) THEN
            CALL RE_ALLOCATE_PARTICLES(1,NM,0,1000)
         ...
         IF (.NOT. LPC%STATIC) THEN
            LP%U = TABLES(IN%TABL_INDEX)%TABLE_DATA(I,4)
            LP%V = TABLES(IN%TABL_INDEX)%TABLE_DATA(I,5)
            LP%W = TABLES(IN%TABL_INDEX)%TABLE_DATA(I,6)
         ELSE
            LP%U = 0._EB
            LP%V = 0._EB
            LP%W = 0._EB
         ENDIF
         MASS_PER_TIME = TABLES(IN%TABL_INDEX)%TABLE_DATA(I,9)
         CALL MAKE_PARTICLE(DIAMETER=TABLES(IN%TABL_INDEX)%TABLE_DATA(I,7),TEMPERATURE=TABLES(IN%TABL_INDEX)%TABLE_DATA(I,8))
         LP=>LAGRANGIAN_PARTICLE(NLP)         
         IF (LP%ONE_D%X(1)>0._EB .AND. MASS_PER_TIME > 0._EB) THEN
            LP%PWT = IN%DT_INSERT*MASS_PER_TIME/(FOTHPI*LP%ONE_D%RHO(1,1)*LP%ONE_D%X(1)**3)
         ELSE
            LP%PWT = 1._EB
         ENDIF     
         LP%T   = T                         ! Insertion time is current time
         LP%IOR = 0                         ! Orientation of solid surface (0 means the PARTICLE/particle is not attached)
         LP%CLASS_INDEX = ILPC              ! Class identifier
         LP%TAG   = PARTICLE_TAG            ! Unique integer tag
         IF (MOD(NLP,LPC%SAMPLING)==0) THEN  ! Decide whether to show or not show in Smokeview
            LP%SHOW = .TRUE.    
         ELSE
            LP%SHOW = .FALSE.    
         ENDIF
   ...
   ELSE POINTWISE_IF
      ...
      INSERT_PARTICLE_LOOP: DO I=1,N_INITIAL
         INSERT_RATE = INSERT_RATE + N_INITIAL/IN%DT_INSERT
         NLP = NLP + 1
         PARTICLE_TAG = PARTICLE_TAG + NMESHES
         IF (NLP>NLPDIM) THEN
            CALL RE_ALLOCATE_PARTICLES(1,NM,0,1000)
         ...
         IF (LPC%N_SPLIT>1) THEN
            LP%SPLIT_IOR = MOD(I-1,LPC%N_SPLIT)+1
            LP%PWT = LP%PWT/REAL(LPC%N_SPLIT,EB)
         ENDIF            
      ...
      IF (MASS_PER_VOLUME>0._EB) LAGRANGIAN_PARTICLE(NLP-N_INITIAL+1:NLP)%PWT = & 
                                 LAGRANGIAN_PARTICLE(NLP-N_INITIAL+1:NLP)%PWT*MASS_PER_VOLUME*BLOCK_VOLUME/MASS_SUM
      IN%ALREADY_INSERTED = .TRUE.                                             
   ENDIF POINTWISE_IF
...
SUBROUTINE MAKE_PARTICLE(DIAMETER,TEMPERATURE)
USE MEMORY_FUNCTIONS, ONLY: ALLOCATE_ONE_D_XFER
REAL(EB), INTENT(IN), OPTIONAL :: DIAMETER,TEMPERATURE
REAL(EB) :: X1,X2
INTEGER :: N
...
IF (LPC%SURF_INDEX > 0) THEN
   ...
   IF (SF%THERMALLY_THICK) THEN
      CALL ALLOCATE_ONE_D_XFER(NM,LPC%SURF_INDEX,LAGRANGIAN_PARTICLE_INDEX=NLP)
      LP => LAGRANGIAN_PARTICLE(NLP)
      ONE_D => LP%ONE_D
   ELSE
      ALLOCATE(ONE_D%TMP(1))
      ONE_D%TMP = SF%TMP_FRONT
      ALLOCATE(ONE_D%LAYER_THICKNESS(1))
      ONE_D%LAYER_THICKNESS=0._EB
      ALLOCATE(ONE_D%X(1))
      ONE_D%X = 0._EB
      ALLOCATE(ONE_D%RHO(1,1))
      ONE_D%RHO(1,1) = 0._EB
      ALLOCATE(ONE_D%N_LAYER_CELLS(1))   
      ONE_D%N_LAYER_CELLS = 0      
   ENDIF
   LP%PWT = 1
   LP%TMP_F = ONE_D%TMP(1)   
   LP%TMP_B = ONE_D%TMP(SF%N_CELLS+1)   
   LP%MASS = 0._EB
   SELECT CASE (SF%GEOMETRY)
      ...
      CASE (SURF_SPHERICAL)
         IF (SF%THERMALLY_THICK) THEN
            ...
            DO N=1,SF%N_LAYERS
               X2 = X1 - SF%LAYER_THICKNESS(N)
               LP%MASS = LP%MASS + SF%LENGTH*SF%LAYER_DENSITY(N)*FOTHPI*(X1**3-X2**3)
               X1 = X2
            END DO      
         ELSE
            ONE_D%X(1) = SF%RADIUS
   ...
   ONE_D%QRADIN  = ONE_D%EMISSIVITY*SIGMA*TMPA4
   ONE_D%QRADOUT = ONE_D%EMISSIVITY*SIGMA*LP%TMP_F**4   
   ALLOCATE(ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES))
   ONE_D%MASSFLUX_ACTUAL = 0._EB
   ALLOCATE(ONE_D%MASSFLUX(0:N_TRACKED_SPECIES))
   ONE_D%MASSFLUX = 0._EB
   IF (RADIATION) THEN      
      ALLOCATE(ONE_D%ILW(NUMBER_RADIATION_ANGLES,NUMBER_SPECTRAL_BANDS))
   ELSE
      ALLOCATE(ONE_D%ILW(1,1))      
   ENDIF
   ONE_D%MASSFLUX = 0._EB      
ELSEIF (LPC%MASSLESS) THEN
   LP => LAGRANGIAN_PARTICLE(NLP)
   LP%PWT = 1
   ...
   ALLOCATE(ONE_D%X(1))
   ONE_D%X = 0._EB
   ALLOCATE(ONE_D%MASSFLUX_ACTUAL(1))
   ONE_D%MASSFLUX_ACTUAL = 0._EB
   ALLOCATE(ONE_D%MASSFLUX(1))
   ONE_D%MASSFLUX = 0._EB
   ALLOCATE(ONE_D%RHO(1,1))
   ONE_D%RHO = 0._EB
   ALLOCATE(ONE_D%ILW(1,1))
...
ELSE
   ...
   IF (PRESENT(TEMPERATURE)) THEN
      IF (TEMPERATURE > 0._EB) THEN
         ONE_D%TMP = TEMPERATURE
      ELSE
         ONE_D%TMP = LPC%TMP_INITIAL
      ENDIF
   ELSE
      ONE_D%TMP = LPC%TMP_INITIAL
   ENDIF
   LP%TMP_F = ONE_D%TMP(1)   
   ALLOCATE(LP%ONE_D%RHO(1,1))
   LP%ONE_D%RHO(1,1) = LPC%DENSITY
   ALLOCATE(ONE_D%N_LAYER_CELLS(1))
   ONE_D%N_LAYER_CELLS = 1
   ALLOCATE(ONE_D%X(1))
   IF (PRESENT(DIAMETER)) THEN
      IF (DIAMETER > 0) THEN
         ONE_D%X(1) = 0.5_EB*DIAMETER
      ELSE
   ...
   ONE_D%QRADIN  = ONE_D%EMISSIVITY*SIGMA*TMPA4
   ONE_D%QRADOUT = ONE_D%EMISSIVITY*SIGMA*TMPA4
ENDIF
...
LOGICAL :: HIT_SOLID
INTEGER :: ICN,I,IIN,JJN,KKN,IW,IWP1,IWM1,IWP2,IWM2,IWP3,IWM3,IOR_OLD,IC,IOR_FIRST,IML
INTEGER, INTENT(IN) :: NM
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC=>NULL()
...
DO I=1,NLP
   ...
   CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XI,YJ,ZK,LP%IIG,LP%JJG,LP%KKG)  
   IF (LP%PWT>0._EB .AND. LP%IOR==0) NDPC(LP%IIG,LP%JJG,LP%KKG)=NDPC(LP%IIG,LP%JJG,LP%KKG)+LP%PWT
ENDDO
...
PARTICLE_LOOP: DO I=1,NLP  
   ...
   AIR_TO_SOLID: IF (LP%IIG/=IIN .OR. LP%JJG/=JJN .OR. LP%KKG/=KKN) THEN
      ...
      IF (KKN>LP%KKG .AND. WALL(IWP3)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         LP%IOR=-3
         HIT_SOLID = .TRUE.
         STEP_FRACTION(LP%IOR) = MAX(0._EB,(Z(LP%KKG)-Z_OLD-0.05_EB*DZ(LP%KKG))/(LP%Z-Z_OLD))
      ENDIF
      IF (KKN<LP%KKG .AND. WALL(IWM3)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         LP%IOR= 3
         HIT_SOLID = .TRUE.
         STEP_FRACTION(LP%IOR) = MAX(0._EB,(Z(LP%KKG-1)-Z_OLD+0.05_EB*DZ(LP%KKG-1))/(LP%Z-Z_OLD))
      ENDIF
      IF (IIN>LP%IIG .AND. WALL(IWP1)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         LP%IOR=-1
         HIT_SOLID = .TRUE.
         STEP_FRACTION(LP%IOR) = MAX(0._EB,(X(LP%IIG)-X_OLD-0.05_EB*DX(LP%IIG))/(LP%X-X_OLD))
      ENDIF
      IF (IIN<LP%IIG .AND. WALL(IWM1)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         LP%IOR= 1
         HIT_SOLID = .TRUE.
         STEP_FRACTION(LP%IOR) = MAX(0._EB,(X(LP%IIG-1)-X_OLD+0.05_EB*DX(LP%IIG-1))/(LP%X-X_OLD))
      ENDIF
      IF (JJN>LP%JJG .AND. WALL(IWP2)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         LP%IOR=-2
         HIT_SOLID = .TRUE.
         STEP_FRACTION(LP%IOR) = MAX(0._EB,(Y(LP%JJG)-Y_OLD-0.05_EB*DY(LP%JJG))/(LP%Y-Y_OLD))
      ENDIF
      IF (JJN<LP%JJG .AND. WALL(IWM2)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         LP%IOR= 2
         HIT_SOLID = .TRUE.
         STEP_FRACTION(LP%IOR) = MAX(0._EB,(Y(LP%JJG-1)-Y_OLD+0.05_EB*DY(LP%JJG-1))/(LP%Y-Y_OLD))
      ENDIF

      IF (LP%IOR/=0 .AND. .NOT.ALLOW_SURFACE_PARTICLES) THEN
         LP%ONE_D%X = 0.9_EB*LPC%KILL_RADIUS
         CYCLE PARTICLE_LOOP
      ENDIF
      ...
      IF_HIT_SOLID: IF (HIT_SOLID) THEN
         ...
         IF (ACCUMULATE_WATER .AND. HIT_SOLID .AND. .NOT.LP%SPLAT) THEN
            WALL(LP%WALL_INDEX)%A_LP_MPUA(LPC%ARRAY_INDEX)=WALL(LP%WALL_INDEX)%A_LP_MPUA(LPC%ARRAY_INDEX)+&
               LP%PWT*LPC%FTPR*MAXVAL(LP%ONE_D%X)**3*WALL(LP%WALL_INDEX)%RAW
            LP%SPLAT = .TRUE.
         ENDIF

         ! Adjust the size of the PARTICLE and weighting factor 

         LP%ONE_D%X   = MIN(0.5_EB*SURFACE_PARTICLE_DIAMETER,(LP%PWT*RDC)**ONTH)
         LP%PWT = LP%PWT*RDC/MAXVAL(LP%ONE_D%X)**3
         ...
         CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XI,YJ,ZK,LP%IIG,LP%JJG,LP%KKG)         
         ICN = CELL_INDEX(LP%IIG,LP%JJG,LP%KKG)
   ...
   IF (WALL(IW)%BOUNDARY_TYPE/=SOLID_BOUNDARY) THEN
      SELECT CASE(LP%IOR)
         CASE( 1)
            LP%X = LP%X - 0.2_EB*DX(LP%IIG)
            LP%W = -LP%W
         CASE(-1)
            LP%X = LP%X + 0.2_EB*DX(LP%IIG)
            LP%W = -LP%W
         CASE( 2)
            LP%Y = LP%Y - 0.2_EB*DY(LP%JJG)
            LP%W = -LP%W
         CASE(-2)
            LP%Y = LP%Y + 0.2_EB*DY(LP%JJG)
            LP%W = -LP%W
...
REAL(EB) XI,YJ,ZK,UBAR,VBAR,WBAR,C_DRAG,RVC,UREL,VREL,WREL,QREL,TMP_G,RHO_G,RD,RDS,RDC,X_OLD,Y_OLD,Z_OLD,&
         ...
         WE_G,T_BU_BAG,T_BU_STRIP,FP_MASS,HALF_DT2,BETA,OBDT,ALPHA,OPA,DTOPA,BDTOA,MPOM,ALBO,SFAC,CHILD_RADIUS(0:NDC)
INTEGER IIX,JJY,KKZ
TYPE(LAGRANGIAN_PARTICLE_TYPE),POINTER::LP=>NULL()
TYPE(LAGRANGIAN_PARTICLE_CLASS_TYPE),POINTER::LPC=>NULL()
...
UVW = MAX( ABS(LP%U)*RDX(LP%IIG),ABS(LP%V)*RDY(LP%JJG),ABS(LP%W)*RDZ(LP%KKG) )
...
JJY  = FLOOR(YJ+.5_EB)
KKZ  = FLOOR(ZK+.5_EB)
UBAR = AFILL2(U,LP%IIG-1,JJY,KKZ,(LP%X-X(LP%IIG-1))*RDX(LP%IIG),YJ-JJY+.5_EB,ZK-KKZ+.5_EB)
VBAR = AFILL2(V,IIX,LP%JJG-1,KKZ,XI-IIX+.5_EB,(LP%Y-Y(LP%JJG-1))*RDY(LP%JJG),ZK-KKZ+.5_EB)
WBAR = AFILL2(W,IIX,JJY,LP%KKG-1,XI-IIX+.5_EB,YJ-JJY+.5_EB,(LP%Z-Z(LP%KKG-1))*RDZ(LP%KKG))
...
RVC   = RDX(LP%IIG)*RRN(LP%IIG)*RDY(LP%JJG)*RDZ(LP%KKG)
RHO_G = RHO(LP%IIG,LP%JJG,LP%KKG)
UREL   = LP%U - UBAR
VREL   = LP%V - VBAR
WREL   = LP%W - WBAR
...
DRAG_LAW_SELECT: SELECT CASE (LPC%DRAG_LAW)
   ...
   CASE DEFAULT
      TMP_G  = MAX(TMPMIN,TMP(LP%IIG,LP%JJG,LP%KKG))
      IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(LP%IIG,LP%JJG,LP%KKG,1:N_TRACKED_SPECIES)
      ...
      WAKE_VEL=1.0_EB
      IF (LPC%SURF_INDEX<1) THEN
         DROP_DEN      = AVG_DROP_DEN_ALL(LP%IIG,LP%JJG,LP%KKG) 
      ...
      BREAKUP: IF (LPC%BREAKUP) THEN
         ...
         AGE_IF: IF ((T-LP%T) > MIN(T_BU_BAG,T_BU_STRIP)) THEN
            IF (LPC%MONODISPERSE) THEN
               RD    = THROHALF*RD
            ELSE
               DO WHILE (RD >= MAXVAL(LP%ONE_D%X))
                  CHILD_RADIUS = LPC%BREAKUP_CHILD_DIAMETER*MAXVAL(LP%ONE_D%X)*LPC%CHILD_R_CDF(:)
                  CALL RANDOM_CHOICE(LPC%CHILD_CDF(:),CHILD_RADIUS,NDC,RD)
               END DO
            ...
            WAKE_VEL = 1.0_EB
            IF (LPC%SURF_INDEX<1) THEN
               DROP_DEN      = AVG_DROP_DEN(LP%IIG,LP%JJG,LP%KKG,LPC%ARRAY_INDEX) 
               DROP_VOL_FRAC = MIN(1._EB,DROP_DEN/LPC%DENSITY)
               IF (DROP_VOL_FRAC > LPC%DENSE_VOLUME_FRACTION) CALL WAKE_REDUCTION(DROP_VOL_FRAC,LP%RE,C_DRAG,WAKE_VEL)
            ENDIF
...
PARTICLE_NON_STATIC_IF: IF (.NOT.LPC%STATIC) THEN
   FP_MASS = (RHO_G/RVC)/NDPC(LP%IIG,LP%JJG,LP%KKG) ! fluid parcel mass
...
REAL(EB) :: R_DROP,NUSSELT,K_AIR,H_V,H_V_REF, H_L,&
            ...
            M_GAS_NEW,MW_GAS,CP2,DELTA_H_G,TMP_G_I,H_G_OLD,H_D_OLD,H_L_NEW, &
            H_L_REF,TMP_G_NEW,DT_SUM,DCPDT,TMP_WGT,X_EQUIL,Y_EQUIL,Y_ALL(1:N_SPECIES),H_S_B
...
SPECIES_LOOP: DO Z_INDEX = 1,N_TRACKED_SPECIES
   ...
   FILM_SUMMING_LOOP: DO I=1,NLP
      LP => LAGRANGIAN_PARTICLE(I)
      LPC => LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)
      IF (LPC%CONSTANT_TEMPERATURE) CYCLE FILM_SUMMING_LOOP
      IF (LPC%Z_INDEX/=Z_INDEX) CYCLE FILM_SUMMING_LOOP
      IF (LP%IOR==0)                  CYCLE FILM_SUMMING_LOOP
      IF (SUM(LP%ONE_D%N_LAYER_CELLS) > 1)  CYCLE FILM_SUMMING_LOOP
      IF (LP%WALL_INDEX==0)           CYCLE FILM_SUMMING_LOOP
      IF (LP%ONE_D%X(1)<=0._EB)       CYCLE FILM_SUMMING_LOOP
      IW = LP%WALL_INDEX
      FILM_THICKNESS(IW) = FILM_THICKNESS(IW) + LP%PWT*MAXVAL(LP%ONE_D%X)**3/WALL(IW)%AW
   ...
   PARTICLE_LOOP: DO I=1,NLP
      ...
      IF (LPC%CONSTANT_TEMPERATURE) CYCLE PARTICLE_LOOP
      LP => LAGRANGIAN_PARTICLE(I)

      IF (LP%ONE_D%X(1)<=0._EB)      CYCLE PARTICLE_LOOP
      IF (SUM(LP%ONE_D%N_LAYER_CELLS) > 1) CYCLE PARTICLE_LOOP
      ...
      TIME_ITERATION_LOOP: DO WHILE (DT_SUM < DT)
         ...
         IF (LPC%EVAPORATE .AND. R_DROP<0.5_EB*LPC%MINIMUM_DIAMETER) THEN
         ...
         IF (LPC%EVAPORATE .AND. TMP_DROP_NEW<TMP_MELT) TMP_DROP_NEW = TMP_MELT

         ! If the PARTICLE temperature reaches boiling, use only enough energy from gas to vaporize liquid
         IF (LPC%EVAPORATE .AND. TMP_DROP_NEW>=TMP_BOIL) THEN  
         ...
         H_L_NEW  = (SS%C_P_L_BAR(ITMP)+TMP_WGT*(SS%C_P_L_BAR(ITMP+1)-SS%C_P_L_BAR(ITMP)))*TMP_DROP_NEW-H_L_REF
         H_NEW = H_G_OLD + H_D_OLD*WGT + Q_RAD - M_DROP*H_L*WGT
         IF (H_NEW > 0._EB) THEN
            ZZ_GET2 = ZZ_GET * M_GAS/M_GAS_NEW               
            IF (Z_INDEX>0) ZZ_GET2(Z_INDEX) = ZZ_GET2(Z_INDEX) + WGT*M_VAP/M_GAS_NEW
            TMP_G_I = TMP_G
            TEMPITER = .TRUE.
            ITCOUNT = 0
         ...
         IF (ENTHALPY_TRANSPORT) THEN
            ZZ_GET = 0._EB
            IF (LPC%Z_INDEX>0) ZZ_GET(LPC%Z_INDEX) = 1._EB
            CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S_B,TMP_DROP)
            D_LAGRANGIAN(II,JJ,KK) =  D_LAGRANGIAN(II,JJ,KK) + (M_VAP*H_S_B - Q_CON_GAS) * RVC * WGT / DT_SUBSTEP
         ELSE
         ...
         IF (MAXVAL(LP%ONE_D%X)<=0._EB) CYCLE PARTICLE_LOOP
...
SUM_PART_QUANTITIES: IF (N_LP_ARRAY_INDICES > 0) THEN
   ...
   PART_CLASS_SUM_LOOP: DO N_LPC = 1,N_LAGRANGIAN_CLASSES
      ...
      PARTICLE_LOOP_2: DO I=1,NLP
         ...
         R_DROP = MAXVAL(LP%ONE_D%X)
         IF (R_DROP<=0._EB)      CYCLE PARTICLE_LOOP_2
         II = LP%IIG
         JJ = LP%JJG
         KK = LP%KKG
         RVC = RDX(LP%IIG)*RRN(II)*RDY(JJ)*RDZ(KK)
   ...
   DO I=1,NLP
      ...
      RVC = RDX(LP%IIG)*RRN(LP%IIG)*RDY(LP%JJG)*RDZ(LP%KKG)
      DROP_DEN_ALL(LP%IIG,LP%JJG,LP%KKG)  = DROP_DEN(LP%IIG,LP%JJG,LP%KKG)  + LP%PWT*LP%MASS*RVC
   ENDDO
...
USE MEMORY_FUNCTIONS, ONLY : RE_ALLOCATE_PARTICLES 
INTEGER :: IKILL,I,NM,LOWER_INDEX,UPPER_INDEX,ARRAY_INDEX
REAL(EB) :: T,MAX_DROP_LIFE
INTEGER, DIMENSION(:), ALLOCATABLE :: KILL_PARTICLE
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
TYPE (LAGRANGIAN_PARTICLE_TYPE), ALLOCATABLE, DIMENSION(:), TARGET :: TEMP_LAGRANGIAN_PARTICLE
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC=>NULL()
...
PARTICLE_LOOP: DO I=1,NLP
   ...
   IF (LPC%SURF_INDEX > 0) THEN
      IF (LP%ONE_D%BURNAWAY) THEN
         IKILL = IKILL + 1
         KILL_PARTICLE(IKILL) = I
         CYCLE PARTICLE_LOOP
      ENDIF
   ELSE
      IF (MAXVAL(LP%ONE_D%X) <= LPC%KILL_RADIUS .AND. .NOT. LPC%MASSLESS) THEN
         IKILL = IKILL + 1
         KILL_PARTICLE(IKILL) = I
         CYCLE PARTICLE_LOOP
      ENDIF      
   ...
   IF (T-LP%T>LPC%LIFETIME .OR. (T-LP%T>MAX_DROP_LIFE .AND. IKILL <= INSERT_COUNT .AND. .NOT. LPC%STATIC)) THEN
      IKILL = IKILL + 1
      KILL_PARTICLE(IKILL) = I
      CYCLE PARTICLE_LOOP
   ENDIF
   ...
   IF (LP%X<MESHES(NM)%XS .OR. LP%X>MESHES(NM)%XF .OR. LP%Y<MESHES(NM)%YS .OR. LP%Y>MESHES(NM)%YF .OR. &
       LP%Z<MESHES(NM)%ZS .OR. LP%Z>MESHES(NM)%ZF) THEN
      IKILL = IKILL + 1
      KILL_PARTICLE(IKILL) = I
      CALL CHECK_MESH_PARTICLE
   ENDIF
...
IF (IKILL > 0) THEN 
   ALLOCATE (TEMP_LAGRANGIAN_PARTICLE(NLP-IKILL))
   ARRAY_INDEX = 1
   LOWER_INDEX = 1
   DO I=1,IKILL
      UPPER_INDEX = KILL_PARTICLE(I)
      IF (UPPER_INDEX==LOWER_INDEX) THEN
         LOWER_INDEX = LOWER_INDEX + 1
         CYCLE
      ENDIF
      TEMP_LAGRANGIAN_PARTICLE(ARRAY_INDEX:ARRAY_INDEX+UPPER_INDEX-LOWER_INDEX-1) = &
         MESHES(NM)%LAGRANGIAN_PARTICLE(LOWER_INDEX:UPPER_INDEX-1)
      ARRAY_INDEX = ARRAY_INDEX+UPPER_INDEX-LOWER_INDEX
      LOWER_INDEX = UPPER_INDEX + 1
   ENDDO
   IF (ARRAY_INDEX <= NLP-IKILL) TEMP_LAGRANGIAN_PARTICLE(ARRAY_INDEX:NLP-IKILL) = MESHES(NM)%LAGRANGIAN_PARTICLE(LOWER_INDEX:NLP)

   !Individual deallocation to prevent memory leaks
   DO I=1,NLP
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%TMP)
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%LAYER_THICKNESS)
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%X)
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX)
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX_ACTUAL)
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%RHO)
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%ILW)
      DEALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(I)%ONE_D%N_LAYER_CELLS)
   ENDDO
  
   NLP = NLP - IKILL
   
   !Do loop for stack reduction
   DO I=1,NLP
      MESHES(NM)%LAGRANGIAN_PARTICLE(I) = TEMP_LAGRANGIAN_PARTICLE(I)
   ENDDO
   
   LAGRANGIAN_PARTICLE=>MESHES(NM)%LAGRANGIAN_PARTICLE
   !Individual deallocation to prevent memory leaks
   DO I=1,NLP
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%TMP)
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%X)
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%RHO)
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%ILW)
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%N_LAYER_CELLS)
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%LAYER_THICKNESS)
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX)
      DEALLOCATE(TEMP_LAGRANGIAN_PARTICLE(I)%ONE_D%MASSFLUX_ACTUAL)
   ENDDO
   
   DEALLOCATE (TEMP_LAGRANGIAN_PARTICLE)

ENDIF

DEALLOCATE (KILL_PARTICLE)

CONTAINS
 
SUBROUTINE CHECK_MESH_PARTICLE
 
INTEGER :: OM,NOM
TYPE (MESH_TYPE), POINTER :: M=>NULL()
TYPE (OMESH_TYPE), POINTER :: M2=>NULL()
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP_OM=>NULL()
TYPE (ONE_D_M_AND_E_XFER_TYPE), POINTER :: ONE_D=>NULL()
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
IF (NOM/=0) THEN
   ...
   LP_OM => M2%LAGRANGIAN_PARTICLE(M2%N_DROP_ORPHANS)
 
   ONE_D => M2%LAGRANGIAN_PARTICLE(M2%N_DROP_ORPHANS)%ONE_D
   IF (.NOT. ALLOCATED(ONE_D%TMP)) ALLOCATE(ONE_D%TMP(0:OM_LP_NODES+1))
   ONE_D%TMP = 0._EB
   IF (.NOT. ALLOCATED(ONE_D%RHO)) ALLOCATE(ONE_D%RHO(0:OM_LP_NODES+1,OM_LP_MATL))
   ONE_D%RHO = 0._EB
   IF (.NOT. ALLOCATED(ONE_D%LAYER_THICKNESS)) ALLOCATE(ONE_D%LAYER_THICKNESS(OM_LP_LAYERS))
   ONE_D%LAYER_THICKNESS = 0._EB
   IF (.NOT. ALLOCATED(ONE_D%N_LAYER_CELLS)) ALLOCATE(ONE_D%N_LAYER_CELLS(OM_LP_LAYERS))
   ONE_D%N_LAYER_CELLS = 0
   IF (.NOT. ALLOCATED(ONE_D%X)) ALLOCATE(ONE_D%X(0:OM_LP_NODES))
   ONE_D%X = 0._EB
   IF (.NOT. ALLOCATED(ONE_D%MASSFLUX)) ALLOCATE(ONE_D%MASSFLUX(0:N_TRACKED_SPECIES))
   ONE_D%MASSFLUX = 0._EB
   IF (.NOT. ALLOCATED(ONE_D%MASSFLUX_ACTUAL)) ALLOCATE(ONE_D%MASSFLUX_ACTUAL(0:N_TRACKED_SPECIES))
   ONE_D%MASSFLUX_ACTUAL = 0._EB
   IF (.NOT. ALLOCATED(ONE_D%ILW)) ALLOCATE(ONE_D%ILW(OM_LP_ILW_A,OM_LP_ILW_B))
   ONE_D%ILW = 0._EB
   
   IF (LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)%SURF_INDEX <= 0) THEN
      ONE_D%TMP(1) = LP%ONE_D%TMP(1)
      ONE_D%RHO(1,1) = LP%ONE_D%RHO(1,1)
      ONE_D%LAYER_THICKNESS(1) = LP%ONE_D%LAYER_THICKNESS(1)   
      ONE_D%N_LAYER_CELLS(1) = LP%ONE_D%N_LAYER_CELLS(1)
      ONE_D%X(1) = LP%ONE_D%X(1)
   ELSEIF (LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)%SURF_INDEX > 0) THEN
      SF => SURFACE(LAGRANGIAN_PARTICLE_CLASS(LP%CLASS_INDEX)%SURF_INDEX)
      IF (SF%THERMALLY_THICK) THEN
         ONE_D%TMP(0:SF%N_CELLS+1) = LP%ONE_D%TMP(0:SF%N_CELLS+1)
         ONE_D%RHO(0:SF%N_CELLS+1,SF%N_LAYERS) = LP%ONE_D%RHO(0:SF%N_CELLS+1,SF%N_MATL)
         IF (SF%SHRINK) THEN
            ONE_D%LAYER_THICKNESS(1:SF%N_LAYERS) = LP%ONE_D%LAYER_THICKNESS(1:SF%N_LAYERS)   
            ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS) = LP%ONE_D%N_LAYER_CELLS(1:SF%N_MATL)
            ONE_D%X(0:SF%N_CELLS+1) = LP%ONE_D%X(0:SF%N_CELLS+1)
         ELSE
            ONE_D%LAYER_THICKNESS(1) = LP%ONE_D%LAYER_THICKNESS(1)   
            ONE_D%N_LAYER_CELLS(1) = LP%ONE_D%N_LAYER_CELLS(1)
            ONE_D%X(1) = LP%ONE_D%X(1)
      ...
      ELSE
         ...
         ONE_D%X(1) = LP%ONE_D%X(1)
      ENDIF
      ONE_D%MASSFLUX = LP%ONE_D%MASSFLUX
      ONE_D%MASSFLUX_ACTUAL = LP%ONE_D%MASSFLUX_ACTUAL
      ONE_D%ILW = LP%ONE_D%ILW      
   ENDIF
   ONE_D%AREA = LP%ONE_D%AREA   
   ONE_D%RAREA = LP%ONE_D%RAREA
   ONE_D%HEAT_TRANS_COEF = LP%ONE_D%HEAT_TRANS_COEF
   ONE_D%QCONF = LP%ONE_D%QCONF
   ONE_D%QRADIN = LP%ONE_D%QRADIN
   ONE_D%QRADOUT = LP%ONE_D%QRADOUT
   ONE_D%EMISSIVITY = LP%ONE_D%EMISSIVITY
   ONE_D%BURNAWAY = LP%ONE_D%BURNAWAY
   ONE_D%SHRINKING = LP%ONE_D%SHRINKING   
   LP_OM%TMP_F = LP%TMP_F
   LP_OM%X = LP%X
   LP_OM%Y = LP%Y
   LP_OM%Z = LP%Z
   LP_OM%U = LP%U
   ...
   LP_OM%W = LP%W
   LP_OM%PWT = LP%PWT
   LP_OM%ACCEL_X = LP%ACCEL_X
   LP_OM%ACCEL_Y = LP%ACCEL_Y
   LP_OM%ACCEL_Z = LP%ACCEL_Z
   LP_OM%T = LP%T
   LP_OM%RE = LP%RE
   LP_OM%MASS = LP%MASS
   LP_OM%TMP_B = LP%TMP_B
   LP_OM%IOR = LP%IOR
   LP_OM%CLASS_INDEX = LP%CLASS_INDEX
   LP_OM%SPLIT_IOR = LP%SPLIT_IOR
   LP_OM%SURF_INDEX = LP%SURF_INDEX
   LP_OM%TAG = LP%TAG
   LP_OM%WALL_INDEX = LP%CLASS_INDEX
   LP_OM%IIG = LP%IIG
   LP_OM%JJG = LP%JJG
   LP_OM%KKG = LP%KKG
   LP_OM%ALREADY_ALLOCATED = LP%ALREADY_ALLOCATED
   LP_OM%SHOW = LP%SHOW
   LP_OM%SHOW =  LP%SHOW
   LP_OM%IGNITOR = LP%IGNITOR
   LP_OM%EVAPORATE = LP%EVAPORATE
ENDIF
 
END SUBROUTINE CHECK_MESH_PARTICLE
