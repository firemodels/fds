...
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) THEN
   IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
      CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,OD_INDEX,UNPACK_IT,COUNT_ONLY,.TRUE.)
   ELSE
      CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,OD_INDEX,UNPACK_IT,COUNT_ONLY,.FALSE.)
   ENDIF
ENDIF
...
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) THEN
   IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
      CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,OD_INDEX,UNPACK_IT,COUNT_ONLY,.TRUE.)
   ELSE
      CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,OD_INDEX,UNPACK_IT,COUNT_ONLY,.FALSE.)
...
!> \param UNPACK_IT Flag indicating whether the data is to be packed into the 1-D array or unpacked from it
!> \param COUNT_ONLY Flag signifying that only a variable count is to be done; no packing
!> \param DX_OLD_GLAG Flag signifying to allocate DX_OLD and LAYER_THICKNESS_OLD

SUBROUTINE PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,OD_INDEX,UNPACK_IT,COUNT_ONLY,DX_OLD_FLAG)

USE GLOBAL_CONSTANTS, ONLY: CHECK_BOUNDARY_ONE_D_ARRAYS
USE COMP_OPERATORS, ONLY: EQUATE
INTEGER, INTENT(IN) :: NM,OD_INDEX
INTEGER, INTENT(INOUT) :: IC,RC,LC
LOGICAL, INTENT(IN) :: UNPACK_IT,COUNT_ONLY,DX_OLD_FLAG
INTEGER :: I1,NN,NL
TYPE(BOUNDARY_ONE_D_TYPE), POINTER :: ONE_D
TYPE(STORAGE_TYPE), POINTER :: OS
...
DO NL=1,ONE_D%N_LAYERS
   ...
   IF (DX_OLD_FLAG) IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC) , ONE_D%REMESH_NWP(NL) , UNPACK_IT)
...
DO NL=1,ONE_D%N_LAYERS
   ...
   RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC) , ONE_D%SWELL_RATIO(NL) , UNPACK_IT)
...
IF (ALLOCATED(ONE_D%SWELL_RATIO)) DEALLOCATE(ONE_D%SWELL_RATIO)               ; ALLOCATE(ONE_D%SWELL_RATIO(ONE_D%N_LAYERS))
...
IF (ALLOCATED(ONE_D%DX_OLD)) THEN
   ONE_D%DX_OLD=0._EB
...
ONE_D%SWELL_RATIO(1:ONE_D%N_LAYERS)  = SF%SWELL_RATIO(1:SF%N_LAYERS)
...
ONE_D%SWELL_RATIO(1:SF%N_LAYERS) = 1._EB
