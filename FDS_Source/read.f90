...
LOGICAL  :: LUMPED_COMPONENT_ONLY,PREDEFINED(MAX_SPECIES),PREDEFINED_SMIX(0:MAX_SPECIES),AEROSOL,BACKGROUND,&
            DEFINED_BACKGROUND,REAC_FUEL_READ=.FALSE.
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   CALL GAS_PROPS(SS%ID,SS%SIG,SS%EPSK,SS%MW,SS%FORMULA,SS%LISTED,SS%ATOMS,SS%H_F,SS%RADCAL_ID)
...
LOGICAL :: L_TMP,CHECK_ATOM_BALANCE,FAST_CHEMISTRY
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,KAPPA0,NMIEANG,NUMBER_RADIATION_ANGLES,PATH_LENGTH,&
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   IF (.NOT. TEMPERATURE_DEPENDENT_REACTION .AND. DV%QUANTITY == 'FLAME TEMPERATURE') THEN
      WRITE(MESSAGE,'(A,A,A)')  'ERROR: DEVC ',TRIM(ID),', TEMPERATURE_DEPENDENT_REACTION must be set on MISC.'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
...
INTEGER :: N,I_DUM
LOGICAL :: CELL_CENTERED
CHARACTER(30) :: QUANTITY,PROP_ID,SPEC_ID,PART_ID,STATISTICS
