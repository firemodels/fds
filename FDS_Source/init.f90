...
USE PHYSICAL_FUNCTIONS, ONLY: GET_VISCOSITY,GET_SPECIFIC_GAS_CONSTANT,GET_SPECIFIC_HEAT,GET_SPECIFIC_HEAT_BG
...
REAL(EB) :: MU_N,YY_GET(1:N_TRACKED_SPECIES),VC,RTRM,CP,TMP_WGT
...
IF (N_TRACKED_SPECIES>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ALLOCATE( M%YY(0:IBP1,0:JBP1,0:KBP1,N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','YY',IZERO)
   M%YY = 0._EB
   ALLOCATE(M%YYS(0:IBP1,0:JBP1,0:KBP1,N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','YYS',IZERO)
   M%YYS = 0._EB   
   ALLOCATE(M%DEL_RHO_D_DEL_Y(0:IBP1,0:JBP1,0:KBP1,N_TRACKED_SPECIES),STAT=IZERO)
...
IF (EVACUATION_ONLY(NM)) THEN
   ...
   M%H       = 0._EB
ENDIF
IF (N_TRACKED_SPECIES > 0 .AND. .NOT.EVACUATION_ONLY(NM)) M%DEL_RHO_D_DEL_Y = 0._EB
...
ELSE
   ...
   MU_N = (Y2MU_C(ITMP)-TMP_WGT*(Y2MU_C(ITMP+1)-Y2MU_C(ITMP)))*SPECIES(0)%MW
ENDIF
...
DO N=1,N_INIT
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            IF (M%XC(I) > IN%X1 .AND. M%XC(I) < IN%X2 .AND. &
               ...
               IF (N_TRACKED_SPECIES>0) M%YY(I,J,K,1:N_TRACKED_SPECIES) = IN%MASS_FRACTION(1:N_TRACKED_SPECIES)
      ...
      ENDDO
   ENDDO
   ! Assign user-specified velocity profile to INIT region
   PATCH_VELOCITY_IF: IF (PATCH_VELOCITY) THEN
      PROF_LOOP: DO NPF=1,N_PROF
         IF (TRIM(IN%PROF_ID)==TRIM(PROFILE(NPF)%ID)) THEN
            IN%PROF_INDEX=NPF
            EXIT PROF_LOOP
         ENDIF
      ENDDO PROF_LOOP
...
NOT_EVAC_IF_1: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   M%TMP_B = TMPA
   ALLOCATE(M%YY_F(N_TOTAL_WALL_CELLS,N_TRACKED_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','YY_F',IZERO)  
   DO IW=1,N_TOTAL_WALL_CELLS
      M%YY_F(IW,1:N_TRACKED_SPECIES) = YY_AMBIENT(1:N_TRACKED_SPECIES)
   ENDDO
      ...
      DO K=1,M%KBAR
         DO J=1,M%JBAR
            DO I=1,M%IBAR
               ...
               IF (N_TRACKED_SPECIES==0) THEN
                  CALL GET_SPECIFIC_HEAT_BG(CP,M%TMP(I,J,K))
                  RTRM = RSUM0/(CP*M%PBAR(K,IPZ))
               ELSE
                  YY_GET(:) = M%YY(I,J,K,:)
                  CALL GET_SPECIFIC_HEAT(YY_GET,CP,M%TMP(I,J,K))
...
IF (N_TRACKED_SPECIES>0 .AND. M%BOUNDARY_TYPE(IW)/=VIRTUAL_BOUNDARY .AND. &
   ...
   M%YY_F(IW,1:N_TRACKED_SPECIES)  = M%YY(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
   M%YY(I,J,K,1:N_TRACKED_SPECIES) = M%YY(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
ENDIF
   ...
   DO K=K1+1,K2
      DO J=J1+1,J2
         DO I=I1+1,I2
            ...
            RHO(I,J,K)  = RHOA
            IF (N_TRACKED_SPECIES>0) THEN
               YY(I,J,K,1:N_TRACKED_SPECIES)  = YY_AMBIENT(1:N_TRACKED_SPECIES)
               YYS(I,J,K,1:N_TRACKED_SPECIES) = YY_AMBIENT(1:N_TRACKED_SPECIES)
            ENDIF
         ENDDO
      ENDDO
   ENDDO
