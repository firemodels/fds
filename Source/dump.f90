   ...
   IF (.NOT.PLOT3D) THEN
      ...
      ELSE
         ...
         ! geometry and data file at first time step
            ...
            OPEN(LU_SLCF(IQ+N_SLCF_MAX,NM),FILE=FN_SLCF(IQ+N_SLCF_MAX,NM),FORM='UNFORMATTED',STATUS='REPLACE')
         ...
         ! data file at subsequent time steps
            OPEN(LU_SLCF(IQ,NM),FILE=FN_SLCF(IQ,NM),FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
            OPEN(LU_SLCF(IQ+N_SLCF_MAX,NM),FILE=FN_SLCF(IQ+N_SLCF_MAX,NM),FORM='UNFORMATTED',STATUS='OLD',POSITION='APPEND')
            CALL DUMP_SLICE_GEOM_DATA(NM,LU_SLCF(IQ,NM),LU_SLCF(IQ+N_SLCF_MAX,NM),SL%FACE_CENTERED,SL%CELL_CENTERED,SL%SLICETYPE, &
...
IF (CC_IBM) THEN
   FILE_LOOP2 : DO NF=1,N_BNDF
      ...
      IF (ABS(STIME-T_BEGIN)<TWO_EPSILON_EB) THEN
         ...
         CALL DUMP_SLICE_GEOM_DATA(NM,LU_BNDG(NF,NM),LU_BNDG(NF+N_BNDF,NM),.FALSE.,.FALSE.,"INBOUND_FACES",1,STIME,I1,I2,J1,J2,K1,K2, &
      ...
      ELSE
         ...
         CALL DUMP_SLICE_GEOM_DATA(NM,LU_BNDG(NF,NM),LU_BNDG(NF+N_BNDF,NM),.FALSE.,.FALSE.,"INBOUND_FACES",0,STIME,I1,I2,J1,J2,K1,K2, &
                                   IND,BF%Y_INDEX,BF%Z_INDEX,BF%DEBUG)
      ENDIF
      CLOSE(LU_BNDG(NF,NM))
