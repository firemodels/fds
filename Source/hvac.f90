...
PUBLIC HVAC_CALC,READ_HVAC,PROC_HVAC,HVAC_BC_IN,COLLAPSE_HVAC_BC,SET_INIT_HVAC,INIT_DUCT_NODE,LEAKAGE_HVAC
...
USE GLOBAL_CONSTANTS, ONLY: LU_INPUT,INPUT_FILE_LINE_NUMBER,STOP_STATUS,SETUP_STOP,HVAC_DEBUG,N_DUCTS,N_DUCTNODES,&
                            N_FILTERS,N_FANS,N_AIRCOILS,N_ZONE,N_SURF,HVAC_SOLVE,DUCT_DR,DUCTNODE_DR,&
                            RHOA,TMPA,N_TRACKED_SPECIES,T_BEGIN,TANH_RAMP,TSQR_RAMP,HVAC_MASS_TRANSPORT,XS_MIN,YS_MIN,ZS_MIN,&
                            TMPM,NODE_Z_MIN,NODE_Z_MAX,T_USED,TAU_DEFAULT,HVAC_SOLVE,N_FANS,N_AIRCOILS
...
REAL(EB) :: CURVE_TEMP !< Temperature for fan curve
...
NAMELIST /HVAC/ AIRCOIL_ID,AMBIENT,AREA,CLEAN_LOSS,COOLANT_SPECIFIC_HEAT,COOLANT_MASS_FLOW,COOLANT_TEMPERATURE,CTRL_ID,&
                CURVE_TEMP,DAMPER,DEBUG,DEVC_ID,DIAMETER,DISCHARGE_COEFFICIENT,DRY,DUCT_ID,&
...
DO NN=1,N_HVAC_READ
   ...
   SELECT CASE (TYPE_ID)
      ...
      CASE('FAN')
         ...
         IF (FAN(I_FAN)%OFF_LOSS<TWENTY_EPSILON_EB) FAN(I_FAN)%OFF_LOSS = 1._EB
         ...
         ENDIF
         FAN%CURVE_TEMP = CURVE_TEMP+TMPM
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,N_DUCTNODES,N_DUCTS,HVAC_SOLVE,N_FANS,N_AIRCOILS,N_LAGRANGIAN_CLASSES,RSUM0,&
                            TMPA,RHOA,P_INF,NMESHES,PROCESS,MY_RANK,HVAC_SURF_INDEX,HVAC_BOUNDARY,PYROLYSIS_NONE,N_MPI_PROCESSES,&
                            STOP_STATUS,N_FILTERS,T_USED
USE MPI_F08
            ...
            IF(MESHES(NM)%VENTS(NV)%ID == DN%VENT_ID) THEN
               ...
               IF (MESHES(NM)%VENTS(NV)%BOUNDARY_TYPE/=HVAC_BOUNDARY) THEN
                  ...
                  IF (ABS(SF%VEL)>TWENTY_EPSILON_EB .OR. ABS(SF%VOLUME_FLOW)>TWENTY_EPSILON_EB .OR. &
                      ABS(SF%MASS_FLUX_TOTAL)>TWENTY_EPSILON_EB .OR. SF%PYROLYSIS_MODEL/= PYROLYSIS_NONE) THEN
...
ALLOCATE(LHS(N_DUCTS+N_DUCTNODES,N_DUCTS+N_DUCTNODES))
ALLOCATE(RHS(N_DUCTS+N_DUCTNODES))

ALLOCATE(NETWORK%DUCT_INDEX(N_DUCTS))
ALLOCATE(NETWORK%NODE_INDEX(N_DUCTNODES))
ALLOCATE(NETWORK%MATRIX_INDEX(N_DUCTS+N_DUCTNODES))
...
USE GLOBAL_CONSTANTS, ONLY: N_DUCTNODES,N_CONNECTIVITY_INDICES,N_DUCTS
INTEGER :: NN,NN3,ND
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,N_DUCTNODES,STRATIFICATION,TMPA,I_RAMP_TMP0_Z,I_RAMP_P0_Z,RSUM0,P_INF,RHOA,N_INIT,&
                            T_USED
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES) !< Species mass fraction array.
...
USE GLOBAL_CONSTANTS, ONLY: N_ZONE,N_DUCTS,CORRECTOR,OLD,NEW,N_DUCTNODES,HVAC_MASS_TRANSPORT,PREVIOUS,GUESS,HVAC_QFAN,P_INF,&
                            N_AIRCOILS,T_USED
INTEGER :: NN,ND
...
SUBROUTINE MATRIX_SOLVE

USE MATH_FUNCTIONS, ONLY: GAUSSJ
USE GLOBAL_CONSTANTS, ONLY: NEW,N_DUCTS
INTEGER :: IERR,ND,NN
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: LHS_GAUSSJ
REAL(EB), ALLOCATABLE, DIMENSION(:) :: RHS_GAUSSJ
TYPE(NETWORK_TYPE), POINTER :: NE
TYPE(DUCT_TYPE), POINTER :: DU
TYPE(DUCTNODE_TYPE), POINTER :: DN

NE =>NETWORK
ALLOCATE(LHS_GAUSSJ(1:NE%N_MATRIX,1:NE%N_MATRIX))
LHS_GAUSSJ(1:NE%N_MATRIX,1:NE%N_MATRIX) = LHS(1:NE%N_MATRIX,1:NE%N_MATRIX)
ALLOCATE(RHS_GAUSSJ(1:NE%N_MATRIX))
RHS_GAUSSJ(1:NE%N_MATRIX) = RHS(1:NE%N_MATRIX)
CALL GAUSSJ(LHS_GAUSSJ,NE%N_MATRIX,NE%N_MATRIX,RHS_GAUSSJ,1,1,IERR)
...
DO NN = 1,NE%N_DUCTNODES
   ...
   DN%P = RHS(NE%MATRIX_INDEX(N_DUCTS+NE%NODE_INDEX(NN)))
ENDDO
   ...
   UPDATE_DUCTRUNS: DO NR = 1,N_DUCTRUNS
      ...
      SUBSTEP_LOOP: DO NSS = 1, N_SUBSTEPS
         ...
         DO NN = 1,DR%N_DUCTNODES
            ...
            IF (DN%N_DUCTS==2) THEN
               IF (ABS(DUCT(DN%DUCT_INDEX(1))%VEL(NEW)) < TWENTY_EPSILON_EB .AND. &
                   ABS(DUCT(DN%DUCT_INDEX(2))%VEL(NEW)) < TWENTY_EPSILON_EB) N_UPDATED(DR%NODE_INDEX(NN)) = .TRUE.
            ENDIF
         ENDDO
         ...
         ITER_LOOP_1: DO
            ...
            DUCT_LOOP_1:DO ND = 1,DR%N_DUCTS
               ...
               CYCLE_FLAG = .TRUE.
               IF (DU%VEL(NEW) > TWENTY_EPSILON_EB) THEN
                  DN => DUCTNODE(DU%NODE_INDEX(1))
                  NN = DU%NODE_INDEX(1)
               ELSEIF (DU%VEL(NEW) < -TWENTY_EPSILON_EB) THEN
...
DO NN = 1,NE%N_DUCTNODES
   ...
   IF (DN%N_DUCTS==2) THEN
      IF (ABS(DUCT(DN%DUCT_INDEX(1))%VEL(NEW)) < TWENTY_EPSILON_EB .AND. &
          ABS(DUCT(DN%DUCT_INDEX(2))%VEL(NEW)) < TWENTY_EPSILON_EB) N_UPDATED(NE%NODE_INDEX(NN)) = .TRUE.
   ENDIF
ENDDO
...
ITER_LOOP_2: DO
   ...
   NODE_LOOP_2:DO NN = 1,NE%N_DUCTNODES
      ...
      N_UPDATED(NE%NODE_INDEX(NN)) = .TRUE.
      IF (ABS(MTOT)<=TWENTY_EPSILON_EB) CYCLE NODE_LOOP_2
      ZZ_GET = 0._EB
      DN%ZZ(:)  = ZZTOT/MTOT
      ZZ_GET(1:N_TRACKED_SPECIES) = DN%ZZ(1:N_TRACKED_SPECIES)
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,DN%RSUM)
...
SUBROUTINE RHSNODE
USE GLOBAL_CONSTANTS,ONLY : N_DUCTS
...
DO NN = 1, NE%N_DUCTNODES
   DN => DUCTNODE(NE%NODE_INDEX(NN))
   IF (DN%VENT .OR. DN%AMBIENT .OR. DN%LEAKAGE .OR. DN%GEOM) CYCLE
   ARRAYLOC = NE%MATRIX_INDEX(N_DUCTS+NE%NODE_INDEX(NN))
   DO ND = 1,DN%N_DUCTS
      DU => DUCT(DN%DUCT_INDEX(ND))
...
DO NN = 1, NE%N_DUCTNODES
   ...
   ARRAYLOC1 = NE%MATRIX_INDEX(N_DUCTS+NE%NODE_INDEX(NN))
   DO ND = 1,DN%N_DUCTS
      DU => DUCT(DN%DUCT_INDEX(ND))
      IF (DU%FIXED .OR. DU%AREA <=TWENTY_EPSILON_EB) CYCLE
      ARRAYLOC2 = NE%MATRIX_INDEX(DN%DUCT_INDEX(ND))
...
USE GLOBAL_CONSTANTS, ONLY: N_ZONE,PSUM,CONNECTED_ZONES,OLD,NEW
INTEGER :: NN,IPZ,IOPZ
TYPE(DUCT_TYPE), POINTER::DU
TYPE(DUCTNODE_TYPE), POINTER::DN
...
USE GLOBAL_CONSTANTS, ONLY: HVAC_LOCAL_PRESSURE,OLD,GAMMA,N_ZONE,STRATIFICATION,GVEC,PREVIOUS,GUESS
...
DO ND = 1, NE%N_DUCTS
   ...
   HEAD = 0._EB
   RGZ = 0._EB
   ARRAYLOC = NE%MATRIX_INDEX(NE%DUCT_INDEX(ND))
   DN=>DUCTNODE(DU%NODE_INDEX(1))
   IF (DN%AMBIENT) THEN
      HEAD = HEAD + DN%P
...
USE GLOBAL_CONSTANTS, ONLY: PREVIOUS,GUESS,HVAC_LOCAL_PRESSURE,GAMMA,N_DUCTS
INTEGER :: NN,ND, ARRAYLOC1,ARRAYLOC2
TYPE(NETWORK_TYPE), POINTER :: NE
...
DUCT_LOOP: DO ND = 1, NE%N_DUCTS
   DU => DUCT(NE%DUCT_INDEX(ND))
   IF (DU%FIXED .OR. DU%AREA < TWENTY_EPSILON_EB) CYCLE DUCT_LOOP
   ARRAYLOC1 = NE%MATRIX_INDEX(NE%DUCT_INDEX(ND))
   ...
   IF (.NOT. (DN%VENT .OR. DN%GEOM)) THEN
      IF (.NOT. DN%AMBIENT .AND. .NOT. DN%LEAKAGE) THEN
         ARRAYLOC2 = NE%MATRIX_INDEX(N_DUCTS+DU%NODE_INDEX(1))
      ...
      IF (DN%LEAKAGE .AND. .NOT. DN%AMBIENT) THEN
         ...
         DO NN=1,PZ%N_DUCTNODES
            ...
            IF (DU2%AREA < TWENTY_EPSILON_EB .OR. DU2%FIXED) CYCLE
            ARRAYLOC2 = NE%MATRIX_INDEX(DN2%DUCT_INDEX(1))
   ...
   IF (.NOT. (DN%VENT .OR. DN%GEOM)) THEN
      IF (.NOT. DN%AMBIENT .AND. .NOT. DN%LEAKAGE) THEN
         ARRAYLOC2 = NE%MATRIX_INDEX(N_DUCTS+DU%NODE_INDEX(2))
         LHS(ARRAYLOC1,ARRAYLOC2) = DT_HV/(DU%RHO_D*DU%LENGTH)
      ENDIF
      IF (DN%LEAKAGE .AND. .NOT. DN%AMBIENT) THEN
         !IF (HVAC_LOCAL_PRESSURE) &
...
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE GLOBAL_CONSTANTS, ONLY: PREVIOUS,OLD,NEW,R0,MW_AIR,P_STP
INTEGER :: FAN_ITER,DR_INDEX,DR_NF,DR_DU
INTEGER, INTENT(IN) :: ND
REAL(EB), INTENT(IN) :: T
REAL(EB) :: DEL_P,VDOT,RHO_CURVE
...
RHO_CURVE = MW_AIR*P_STP/R0/FA%CURVE_TEMP
...
QFAN_IF: IF (DU%QFAN_INDEX < 0) THEN
   SELECT CASE (FA%FAN_TYPE)
      ...
      CASE(2) ! Quadratic
         ...
         DEL_P = DEL_P*EVALUATE_RAMP(TSI,FA%SPIN_INDEX,TAU=FA%TAU)*DU%RHO_D/RHO_CURVE
      CASE(3) !Fan curve
         VDOT = 0.5_EB*(DU%VEL(NEW)+DU%VEL(OLD))*DU%AREA
         IF (DU%REVERSE) VDOT = -VDOT
         DEL_P = EVALUATE_RAMP(VDOT,FA%RAMP_INDEX)*EVALUATE_RAMP(TSI,FA%SPIN_INDEX,TAU=FA%TAU)*DU%RHO_D/RHO_CURVE
   END SELECT
ELSE QFAN_IF
   DR_INDEX = DU%DUCTRUN
   DR_NF = DU%QFAN_INDEX
   ...
   IF (RAMP_FAC < TWENTY_EPSILON_EB) THEN
      DU%DP_FAN(NEW) = 0._EB
      RETURN
   ENDIF
   ...
   SELECT CASE (FA%FAN_TYPE)
      CASE(2) ! Quadratic
         MAX_PRES = FA%MAX_PRES*RAMP_FAC*DU%RHO_D/RHO_CURVE
      ...
      CASE(3) !Fan curve
         ...
         MAX_PRES = MAXVAL(RP%DEPENDENT_DATA)*RAMP_FAC*DU%RHO_D/RHO_CURVE
         ...
         IF (DUCTRUN(DR_INDEX)%VEL(DR_DU,0,1)*REV_FAC > RP%T_MAX*RAMP_FAC) THEN
            DEL_P=RP%DEPENDENT_DATA(RP%NUMBER_DATA_POINTS)*RAMP_FAC*DU%RHO_D/RHO_CURVE
         ELSEIF (DUCTRUN(DR_INDEX)%VEL(DR_DU,DR_NF,1)*REV_FAC <= RP%T_MIN) THEN
            DEL_P=RP%DEPENDENT_DATA(1)*RAMP_FAC*DU%RHO_D/RHO_CURVE
         ELSE
            FAN_ITER = 0
            FAN_LOOP2: DO
               FAN_ITER = FAN_ITER + 1
               P_SYS = CONST * (FLOWGUESS - DUCTRUN(DR_INDEX)%VEL(DR_DU,0,1)*REV_FAC*DU%AREA)**2
               DEL_P = EVALUATE_RAMP(FLOWGUESS/RAMP_FAC,FA%RAMP_INDEX)*RAMP_FAC*DU%RHO_D/RHO_CURVE
               IF (P_SYS > DEL_P) THEN
                  FLOW2 = FLOWGUESS
               ...
               ELSE
                  EXIT FAN_LOOP2
               ENDIF
               IF (ABS(FLOW3 - FLOWGUESS)/(FLOWGUESS+TWENTY_EPSILON_EB) < ERROR) EXIT FAN_LOOP2
               IF (FAN_ITER==20) THEN
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,PREDICTOR,NULL_BOUNDARY,T_USED
...
USE GLOBAL_CONSTANTS, ONLY: CC_IBM,HVAC_LOCAL_PRESSURE,P_INF
...
DUCT_LOOP: DO ND=1,N_DUCTS
   ...
   IF (DU%VOLUME_FLOW_INITIAL<1.E6_EB) THEN
      ...
      IF(DU%AREA > TWENTY_EPSILON_EB) DU%VEL(NEW) = DU%VOLUME_FLOW/DU%AREA
      DU%FIXED = .TRUE.
      CYCLE DUCT_LOOP
   ENDIF
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
      ...
      DO ND = 1,DN%N_DUCTS
         ...
         ENDIF
         IF (DU%AREA < TWENTY_EPSILON_EB) THEN
            COUNTER = COUNTER + 1
            ZERO_AREA = .TRUE.
         ENDIF
      ENDDO
      IF (COUNTER==DN%N_DUCTS) THEN
         IF (ZERO_AREA .AND. ABS(VOLUME_FLOW)<TWENTY_EPSILON_EB) CYCLE NODE_LOOP
...
USE GLOBAL_CONSTANTS, ONLY: N_DUCTS,N_DUCTNODES,N_ZONE
INTEGER:: NZ,NN,ND,NODE_COUNTER(N_DUCTNODES),COUNTER,ZONE_COUNTER(N_ZONE)
...
NETWORK%N_DUCTS=0
NETWORK%DUCT_INDEX=0
NETWORK%NODE_INDEX=0
NETWORK%MATRIX_INDEX = 0

COUNTER = 0
DO ND = 1, N_DUCTS
   IF (DUCT(ND)%AREA > TWENTY_EPSILON_EB) THEN
      NETWORK%N_DUCTS = NETWORK%N_DUCTS+1
      NETWORK%DUCT_INDEX(NETWORK%N_DUCTS) = ND
   ENDIF
ENDDO
...
DO NN = 1, N_DUCTNODES
   ...
   IF (ZERO_AREA) CYCLE
   NODE_COUNTER(NN) = NN
   NETWORK%N_DUCTNODES = NETWORK%N_DUCTNODES+1
   NETWORK%NODE_INDEX(NETWORK%N_DUCTNODES) = NN
   
   NZ = DUCTNODE(NN)%ZONE_INDEX

   IF (NZ>=1) ZONE_COUNTER(NZ) = ZONE_COUNTER(NZ) + 1
ENDDO
...
DO ND=1,NE%N_DUCTS
   ...
   IF (DU%FIXED .OR. DU%AREA<=TWENTY_EPSILON_EB) CYCLE
   COUNTER = COUNTER + 1
   NE%MATRIX_INDEX(NE%DUCT_INDEX(ND))=COUNTER
ENDDO
...
DO NN=1,NE%N_DUCTNODES
   ...
   IF (DN%VENT .OR. DN%AMBIENT .OR. DN%LEAKAGE .OR. DN%GEOM) CYCLE
   COUNTER = COUNTER + 1
   NE%MATRIX_INDEX(N_DUCTS+NE%NODE_INDEX(NN))=COUNTER
ENDDO
...
SUBROUTINE UPDATE_LOSS(T,DT)

USE PHYSICAL_FUNCTIONS, ONLY: GET_VISCOSITY
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,PREVIOUS
...
NODELOOP : DO NN=1,NE%N_DUCTNODES
   ...
   IF (DN%FILTER_INDEX > 0) THEN
      ...
      IF (DUCT(DN%DUCT_INDEX(1))%AREA < TWENTY_EPSILON_EB .OR. DUCT(DN%DUCT_INDEX(2))%AREA < TWENTY_EPSILON_EB) CYCLE
   ...
   NODECLASS: IF (DN%VENT .OR. DN%AMBIENT .OR. DN%GEOM) THEN
      ...
      IF (DUCT(DN%DUCT_INDEX(1))%AREA < TWENTY_EPSILON_EB .OR. DUCT(DN%DUCT_INDEX(1))%LOCALIZED_LEAKAGE) CYCLE
...
SUBROUTINE SET_GUESS(T)

USE GLOBAL_CONSTANTS, ONLY: GUESS,PREVIOUS,NEW
REAL(EB), INTENT(IN):: T
INTEGER :: ND
TYPE(DUCT_TYPE),POINTER :: DU
TYPE(NETWORK_TYPE), POINTER :: NE
...
DUCTLOOP: DO ND=1,NE%N_DUCTS
   DU=>DUCT(NE%DUCT_INDEX(ND))
   IF (DU%AREA < TWENTY_EPSILON_EB) CYCLE DUCTLOOP
   RHOLAST = DU%RHO_D
   TMPLAST = DU%TMP_D
...
USE GLOBAL_CONSTANTS, ONLY: PREVIOUS,NEW
INTEGER :: NN, ND, COUNT
LOGICAL :: CONVERGED
...
DO ND=1,NE%N_DUCTS
   ...
   IF (DU%VEL(PREVIOUS) < 0._EB .EQV. DU%VEL(NEW) < 0._EB) THEN ! check for flow reversal
         IF (ABS(DU%VEL(PREVIOUS))<=TWENTY_EPSILON_EB) THEN
            CONVERGED = .FALSE.
            EXIT
         ELSE
...
DO NN=1,NE%N_DUCTNODES
   ...
   IF (DN%FILTER_INDEX > 0) THEN
      ...
      MTOT = MTOT + MFLOW
   ENDIF
   IF(ABS(MSUM)< 1.E-6 * MTOT .OR. MTOT < TWENTY_EPSILON_EB .OR. COUNT >= DN%N_DUCTS-1) CYCLE
   CONVERGED = .FALSE.
ENDDO
...
VENT_CUSTOM_AMBIENT: DO NN=1,N_DUCTNODES
   ...
   INTERNAL_NODE_IF: IF (((DN%VENT .OR. DN%LEAKAGE .OR. DN%GEOM) .AND. .NOT. DN%AMBIENT) .OR. &
      ...
      ZZ_GET = 0._EB
      AREA = NODE_AREA(NN)
      IF (AREA<=TWENTY_EPSILON_EB) THEN
      ...
      IF (DN%LEAKAGE) THEN
         IF(ABS(T-T_BEGIN) < TWENTY_EPSILON_EB) DN%P_OLD = NODE_P(NN)/AREA+P_INF
      ENDIF
...
AMBIENT_LEAK: DO NN=1,N_DUCTNODES
   DN => DUCTNODE(NN)
    IF (DN%AMBIENT .AND. NODE_AREA(NN)<=TWENTY_EPSILON_EB) THEN
...
USE GLOBAL_CONSTANTS, ONLY: N_DUCTNODES,N_DUCTS,HVAC_MASS_TRANSPORT_CELL_L,N_TRACKED_SPECIES,RHOA,TMPA,N_ZONE,T_USED
REAL(EB) :: TNOW !< Current CPU time (s) used in computing length of time spent in HVAC routines.
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,N_DUCTS
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_ENTHALPY
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,N_ZONE,N_DUCTS,N_DUCTNODES,RHOA,TMPA,RSUM0,T_USED
...
USE GLOBAL_CONSTANTS, ONLY: N_DUCTS,NEW
REAL(EB), INTENT(IN) :: T
INTEGER :: ND
TYPE(DUCT_TYPE),POINTER::DU
...
COIL_LOOP: DO ND = 1,N_DUCTS
   ...
   IF (DU%AREA < TWENTY_EPSILON_EB) CYCLE COIL_LOOP
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,I_MAX_TEMP
...
IF (AC%FIXED_Q > -1.E9_EB) THEN
   ...
   ELSEIF (TMP_GUESS < TWENTY_EPSILON_EB) THEN
...
ELSE
   ...
   IF (MDOT_DU < TWENTY_EPSILON_EB) RETURN
   ...
   DO WHILE (ITER <= 10)
      ...
      IF (ABS(TMP_OUT-TMP_GUESS) < TWENTY_EPSILON_EB) EXIT
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,NEW
REAL(EB), INTENT(IN) :: DT
...
DUCT_LOOP: DO ND = 1,DUCTRUN(NR)%N_DUCTS
   ...
   IF (ABS(DU%VEL(NEW))<=TWENTY_EPSILON_EB .OR. DU%AREA<=TWENTY_EPSILON_EB) CYCLE DUCT_LOOP
...
USE GLOBAL_CONSTANTS, ONLY: N_DUCTNODES,N_DUCTS
...
USE GLOBAL_CONSTANTS, ONLY: N_DUCTRUNS,GUESS,N_AIRCOILS,PREVIOUS,OLD,NEW
REAL(EB), INTENT(IN) :: T
INTEGER :: NR,NF
TYPE(DUCTRUN_TYPE), POINTER :: DR
...
IF (DR%FAN_OPERATING(1)) THEN
   FAN_P = FAN_MAX(DUCT(DR%DUCT_INDEX(1))%FAN_INDEX,T-DUCT(DR%DUCT_INDEX(1))%FAN_ON_TIME,DUCT(DR%DUCT_INDEX(1))%RHO_D)
...
DO ND=1,N_DUCTS
   ...
   IF (DR_INDEX > 0) THEN
      ...
      IF (.NOT. DUCT(ND)%FIXED .AND. DUCT(ND)%AREA > TWENTY_EPSILON_EB) &
   ...
   QFAN_IF: IF (DUCTRUN(NR)%N_QFANS > 0) THEN
      IF (DUCTRUN(NR)%N_M_DUCTS > 0) THEN
         ...
         DO ND=1,DUCTRUN(NR)%N_DUCTS
            IF (DUCT(DUCTRUN(NR)%DUCT_INDEX(ND))%FIXED .OR. DUCT(DUCTRUN(NR)%DUCT_INDEX(ND))%AREA < TWENTY_EPSILON_EB) CYCLE
            NN3 = NN3 + 1
...
ALLOCATE(LHS_GAUSSJ(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES,1:DR%N_M_DUCTS+DR%N_M_DUCTNODES))
LHS_GAUSSJ(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES,1:DR%N_M_DUCTS+DR%N_M_DUCTNODES) = &
   LHS(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES,1:DR%N_M_DUCTS+DR%N_M_DUCTNODES)
ALLOCATE(RHS_GAUSSJ(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES))
RHS_GAUSSJ(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES) = RHS(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES)
CALL GAUSSJ(LHS_GAUSSJ,DR%N_M_DUCTS+DR%N_M_DUCTNODES,DR%N_M_DUCTS+DR%N_M_DUCTNODES,RHS_GAUSSJ,1,1,IERR)
RHS(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES) = RHS_GAUSSJ(1:DR%N_M_DUCTS+DR%N_M_DUCTNODES)
DEALLOCATE(LHS_GAUSSJ)
DEALLOCATE(RHS_GAUSSJ)
...
USE GLOBAL_CONSTANTS, ONLY: HVAC_LOCAL_PRESSURE,OLD,GAMMA,STRATIFICATION,GVEC,PREVIOUS,GUESS
INTEGER, INTENT(IN) :: DUCTRUN_INDEX,NF
REAL(EB), INTENT(IN) :: T
INTEGER :: ND
REAL(EB) :: HEAD !< DUCT pressure head (Pa)
...
DO ND = 1, DR%N_M_DUCTS
   ...
   IF (DU%QFAN_INDEX==NF) THEN
      IF ( DU%REVERSE) THEN
         DP_FAN = -FAN_MAX(DU%FAN_INDEX,T-DU%FAN_ON_TIME,DU%RHO_D)
      ELSE
         DP_FAN = FAN_MAX(DU%FAN_INDEX,T-DU%FAN_ON_TIME,DU%RHO_D)
      ENDIF
   ELSE
...
USE GLOBAL_CONSTANTS, ONLY: PREVIOUS,GUESS,HVAC_LOCAL_PRESSURE,GAMMA
INTEGER, INTENT(IN) :: DUCTRUN_INDEX,NF
INTEGER :: ND
TYPE(DUCTRUN_TYPE), POINTER :: DR
TYPE(DUCT_TYPE), POINTER :: DU
...
REAL(EB) FUNCTION FAN_MAX(FAN_INDEX,TSI,RHO)
USE GLOBAL_CONSTANTS, ONLY: R0,MW_AIR,P_STP
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
INTEGER, INTENT(IN) :: FAN_INDEX
REAL(EB),INTENT(IN) :: TSI,RHO
REAL(EB) :: RHO_CURVE

RHO_CURVE = MW_AIR*P_STP/R0/FAN(FAN_INDEX)%CURVE_TEMP
...
FAN_MAX = FAN_MAX * EVALUATE_RAMP(TSI,FAN(FAN_INDEX)%SPIN_INDEX,TAU=FAN(FAN_INDEX)%TAU)*RHO/RHO_CURVE
...
DO ND=1,DR%N_M_DUCTS
   ...
   IF (ABS(DR%VEL(ND,NF,NEW)-DR%VEL(ND,NF,PREVIOUS))/(ABS(DR%VEL(ND,NF,PREVIOUS))+TWENTY_EPSILON_EB) > 0.01_EB) CONVERGED = .FALSE.
ENDDO
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_VISCOSITY
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,PREVIOUS,OLD,NEW
...
DUCTLOOP: DO ND=1,DR%N_DUCTS
   ...
   ENDIF
   IF (ABS(DVEL) > TWENTY_EPSILON_EB) THEN
      IF (DVEL>0._EB) THEN
         NN = DU%NODE_INDEX(1)
      ELSE
         NN = DU%NODE_INDEX(2)
...
USE GLOBAL_CONSTANTS, ONLY: N_TRACKED_SPECIES,N_DUCTS,N_DUCTNODES,STRATIFICATION,GVEC,P_INF,NEW
...
ITER_LOOP: DO
   ...
   DUCT_LOOP:DO ND = 1,DR%N_DUCTS
      ...
      ENDIF
      IF (ABS(DVEL) > TWENTY_EPSILON_EB) THEN
         IF (DVEL > TWENTY_EPSILON_EB) NN = DU%NODE_INDEX(1)
         IF (DVEL < TWENTY_EPSILON_EB) NN = DU%NODE_INDEX(2)
         DN => DUCTNODE(NN)
   ...
   NODE_LOOP:DO NN = 1,DR%N_DUCTNODES
      ...
      N_UPDATED(DR%NODE_INDEX(NN)) = .TRUE.
      IF (ABS(MTOT)<=TWENTY_EPSILON_EB) CYCLE NODE_LOOP
...
USE OUTPUT_DATA, ONLY: OUTPUT_QUANTITY
