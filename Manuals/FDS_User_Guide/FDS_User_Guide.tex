...
{\ct NUMBER\_INITIAL\_PARTICLES} to indicate the number of particles/droplets within the computational domain at the start of the simulation. 
...
specifies the particle/droplet mass per unit volume (Default 1~kg/m$^3$). Do not confuse this parameter with {\ct DENSITY}, explained in the next section. 
...
Also, to limit the particles/droplets to a certain region of the domain, add the real sextuplet {\ct XB} to designate the coordinates of a rectangular volume. 
...
and the volume of the block is calculated according to the dimensions {\ct XB}, regardless of whether there are solid obstructions within this volume.

\footnotesize
\begin{verbatim}
&PART ID='drops', DIAMETER=750., SAMPLING_FACTOR=1, COLOR='RED' /
...
\noindent
If you want to introduce droplets or particles within a given volume periodically in time and not just at the initial time, set {\ct DT\_INSERT} on 
