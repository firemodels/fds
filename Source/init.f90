...
WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF_THERM_THICK_EXPOSED: IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK .AND. SF%BACKING==EXPOSED) THEN ! search for the back side
      ...
      ZERO_OR_ONE_CELL_THICK: DO  ! Look for the back wall cell face if the obstruction is either zero or one cell thick.
...
DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (SF%N_WALL_STORAGE_INTEGERS<1 .OR. SF%VARIABLE_THICKNESS) THEN
      CALL PACK_WALL(NM,IW,WC%SURF_INDEX,.FALSE.,N_INTEGERS=N_INT,N_REALS=N_RLS,N_LOGICALS=N_LOG)
...
INTEGER :: IW2,NWP,NWP2,I2,IWA,DM,NN
...
PRIMARY_WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(ABS(BC%IOR))
      CASE(1) ; ONE_D%LAYER_THICKNESS(1) = ABS(BC%X - M%BOUNDARY_COORD(M%WALL(WC%BACK_INDEX)%BC_INDEX)%X)
      CASE(2) ; ONE_D%LAYER_THICKNESS(1) = ABS(BC%Y - M%BOUNDARY_COORD(M%WALL(WC%BACK_INDEX)%BC_INDEX)%Y)
      CASE(3) ; ONE_D%LAYER_THICKNESS(1) = ABS(BC%Z - M%BOUNDARY_COORD(M%WALL(WC%BACK_INDEX)%BC_INDEX)%Z)
   END SELECT
