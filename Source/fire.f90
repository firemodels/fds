...
DO K=1,KBAR
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         ...
         DEBUG = .FALSE.
         IF (I==10 .AND. J==1 .AND. K==52) DEBUG = .TRUE.
         !***************************************************************************************
         ! Call combustion integration routine for Cartesian cell (I,J,K)
         IF (TWO_REAC_EXTINCT_TEST .AND. N_SIMPLE_CHEMISTRY_REACTIONS==2) THEN
            CALL COMBUSTION_2STEP( T,DT,ZZ_GET,Q(I,J,K),MIX_TIME(I,J,K),CHI_R(I,J,K),&
                                  CHEM_SUBIT_TMP,REAC_SOURCE_TERM_TMP,Q_REAC_TMP,&
                                  TMP(I,J,K),RHO(I,J,K),MU(I,J,K),&
                                  ZETA_P(I,J,K),AIT_P(I,J,K),&
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,MU_IN,ZETA_INOUT,AIT_IN,DELTA,CELL_VOLUME)
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_SUBSTEPS
   ...
   IF (SUPPRESSION) CALL CHECK_AUTO_IGNITION(EXTINCT,TMP_MIXED,AIT_LOC)
...
SUBROUTINE COMBUSTION_2STEP(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,&
                            TMP_IN,RHO_IN,MU_IN,ZETA_INOUT,AIT_IN,DELTA,CELL_VOLUME)
USE COMP_FUNCTIONS, ONLY: SHUTDOWN
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
USE PHYSICAL_FUNCTIONS, ONLY: GET_AVERAGE_SPECIFIC_HEAT,GET_SPECIFIC_GAS_CONSTANT,GET_ENTHALPY,GET_SPECIFIC_HEAT,GET_REALIZABLE_MF
REAL(EB), INTENT(IN) :: T,DT,TMP_IN,RHO_IN,MU_IN,AIT_IN,DELTA,CELL_VOLUME
REAL(EB), INTENT(OUT) :: Q_OUT,MIX_TIME_OUT,CHI_R_OUT,REAC_SOURCE_TERM_OUT(N_TRACKED_SPECIES),Q_REAC_OUT(N_REACTIONS)
INTEGER, INTENT(OUT) :: CHEM_SUBIT_OUT
REAL(EB), INTENT(INOUT) :: ZZ_GET(1:N_TRACKED_SPECIES),ZETA_INOUT
REAL(EB) :: ZETA,ZETA_0,DZZ(1:N_TRACKED_SPECIES),&
            CELL_MASS,MIXED_MASS(1:N_TRACKED_SPECIES),&
            ZZ_MIXED(1:N_TRACKED_SPECIES),ZZ_UNMIXED(1:N_TRACKED_SPECIES),&
            ZZ_MIXED_NEW(1:N_TRACKED_SPECIES),TAU_D,TAU_G,TAU_U,TAU_MIX,TMP_MIXED,TMP_NEW,TMP_UNMIXED,RHO_HAT,&
            VEL_RMS,ZZ_0(1:N_TRACKED_SPECIES),&
            Q_REAC_SUM(1:N_REACTIONS),CHI_R_SUM,TIME_RAMP_FACTOR,&
            TOTAL_MIXED_MASS,&
            AIT_LOC,D_F,ETOT,TGUESS,CP,CP2,ITCOUNT,DCPDT,ZZ_NEW(1:N_TRACKED_SPECIES),ZZ_NEW_2(1:N_TRACKED_SPECIES),Q_NEW
INTEGER :: NR
INTEGER, PARAMETER :: TV_ITER_MIN=5
LOGICAL :: EXTINCT
INTEGER, PARAMETER :: INFINITELY_FAST=1
TYPE(REACTION_TYPE), POINTER :: RN=>NULL()
REAL(EB), PARAMETER :: C_U = 0.4_EB*0.1_EB*SQRT(1.5_EB) ! C_U*C_DEARDORFF/SQRT(2/3)
...
ZETA = MAX(0._EB,ZETA_0*EXP(-DT/TAU_MIX)) ! FDS Tech Guide (5.18)
TOTAL_MIXED_MASS = (1._EB-ZETA)*CELL_MASS      ! FDS Tech Guide (5.23)
MIXED_MASS = MAX(0._EB,ZZ_0*(1._EB-ZETA_0)*CELL_MASS - (ZETA - ZETA_0)*ZZ_UNMIXED*CELL_MASS) ! FDS Tech Guide (5.26)
ZZ_MIXED = MIXED_MASS/MAX(TOTAL_MIXED_MASS,TWO_EPSILON_EB) ! FDS Tech Guide (5.27)
!!!!!!IF (DEBUG) WRITE(*,*) ZETA,ZETA_0,CELL_MASS
!!!IF (DEBUG) WRITE(*,*) '     AIR           METHANE       PRODUCTS      INTERMEDIATE  N2 AGENT'
!!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZIN:',ZZ_0
!!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZMI:',ZZ_MIXED
!!!!!!IF (DEBUG) WRITE(*,*) 'TMM:',TOTAL_MIXED_MASS
!!!IF (DEBUG) WRITE(*,*) 'T0 RH0:',TMP_IN,RHO_IN
...
! First Reaction
IF (AIT_LOC > 1.E10_EB) AIT_LOC = REACTION(1)%AUTO_IGNITION_TEMPERATURE

!!!IF (DEBUG) WRITE(*,*) 'AIT1:',TMP_MIXED,AIT_LOC
IF ((SUPPRESSION .AND. TMP_MIXED >AIT_LOC) .OR. (.NOT. SUPPRESSION)) THEN

   Q_REAC_OUT(1) = 0._EB
   CALL REACTION_RATE(DZZ,ZZ_MIXED,DT,RHO_HAT,TMP_MIXED,INFINITELY_FAST,Q_REAC_OUT,1,PRIORITY=1)
!!!   !!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'DZZ1:',DZZ
!!!   !!!IF (DEBUG) WRITE(*,*) 'QR1:',Q_REAC_OUT(1)
   Q_NEW = Q_REAC_OUT(1)
   ZZ_MIXED_NEW = ZZ_MIXED + DZZ
   !!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZZMN:',ZZ_MIXED_NEW
   Q_REAC_OUT(1) = Q_REAC_OUT(1)*TOTAL_MIXED_MASS

   CALL GET_REALIZABLE_MF(ZZ_MIXED_NEW)

   IF (SUPPRESSION) CALL EXTINCT_2A(EXTINCT,ZZ_MIXED,ZZ_MIXED_NEW,TMP_MIXED,1)

   EXTNCT_1: IF (EXTINCT) THEN
      CHEM_SUBIT_OUT = 0
      Q_REAC_OUT(1) = 0._EB
   ELSE EXTNCT_1
      IF (ZZ_MIXED_NEW(1) > 0._EB .AND. ZZ_MIXED_NEW(4) > 0._EB) THEN
         ZZ_NEW(2) = 0._EB
         ZZ_NEW(4) = ZZ_MIXED_NEW(4)
         ZZ_NEW(1) = MIN(ZZ_MIXED_NEW(1),-REACTION(2)%NU_MW_O_MW_F(1)*ZZ_NEW(4))
         ZZ_NEW(3) = ZZ_NEW(1)/ZZ_MIXED_NEW(1)*ZZ_MIXED(3)
         ZZ_NEW(5) = ZZ_NEW(1)/ZZ_MIXED_NEW(1)*ZZ_MIXED(5)-DZZ(1)*ZZ_MIXED(5)/ZZ_MIXED(1)
!!!         !!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZZN :',ZZ_NEW
!!!         !!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'NUMW:',REACTION(2)%NU_MW_O_MW_F
         ZZ_NEW_2 = ZZ_NEW / SUM(ZZ_NEW)
!!!         !!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZZN2:',ZZ_NEW_2
         CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_NEW_2,CP,TMP_MIXED)
!!!         !!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'CP Q:',CP,TMP_MIXED,Q_NEW,SUM(ZZ_NEW)
         ETOT = CP*TMP_MIXED + Q_NEW/SUM(ZZ_NEW)
         ITCOUNT = 0
         CP_LOOP: DO
            ITCOUNT = ITCOUNT + 1
            TGUESS = TMP_MIXED 
            CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_NEW_2,CP,TGUESS)
            IF (TGUESS>1._EB) THEN
               CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_NEW_2,CP2,TGUESS-1._EB)
               DCPDT = CP - CP2
            ELSE
               CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_NEW_2,CP2,TGUESS+1._EB)
               DCPDT = CP2- CP
            ENDIF
            TMP_MIXED =TGUESS+(ETOT-CP*TGUESS)/(CP+TGUESS*DCPDT)
            IF (ABS(TMP_MIXED - TGUESS) < TWO_EPSILON_EB) EXIT CP_LOOP
            IF ((TMP_MIXED - TGUESS)/TMP_MIXED < 0.0005_EB) EXIT CP_LOOP
            IF (ITCOUNT > 10) THEN
               TMP_MIXED = 0.5_EB*(TMP_MIXED+TGUESS)
               EXIT CP_LOOP
            ENDIF
            TMP_MIXED = MAX(0._EB,TMP_MIXED)
         ENDDO CP_LOOP
         !!!IF (DEBUG) WRITE(*,*) 'TMP',TMP_MIXED
      ENDIF
      ZZ_MIXED = ZZ_MIXED_NEW
   ENDIF EXTNCT_1
ENDIF
...
IF (ZZ_MIXED(1) > 0._EB) THEN

   IF (AIT_LOC > 1.E10_EB) AIT_LOC = REACTION(2)%AUTO_IGNITION_TEMPERATURE

   !!!IF (DEBUG) WRITE(*,*) 'AIT2:',TMP_MIXED,AIT_LOC
   IF ((SUPPRESSION .AND. TMP_MIXED >AIT_LOC) .OR. (.NOT. SUPPRESSION)) THEN
      Q_REAC_OUT(2) = 0._EB
      CALL REACTION_RATE(DZZ,ZZ_MIXED,DT,RHO_HAT,TMP_MIXED,INFINITELY_FAST,Q_REAC_OUT,2,PRIORITY=2)
      ZZ_MIXED_NEW = ZZ_MIXED + DZZ
      Q_REAC_OUT(2) = Q_REAC_OUT(2)*TOTAL_MIXED_MASS
!!!      !!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'DZZ2:',DZZ
!!!      !!!IF (DEBUG) WRITE(*,*) 'QR2:',Q_REAC_OUT(2)

      CALL GET_REALIZABLE_MF(ZZ_MIXED_NEW)

      IF (SUPPRESSION .AND. Q_REAC_OUT(2) > 0._EB) CALL EXTINCT_2A(EXTINCT,ZZ_MIXED,ZZ_MIXED_NEW,TMP_MIXED,2)
...
IF (OUTPUT_CHEM_IT) CHEM_SUBIT_OUT = 1
...
IF (SUM(Q_REAC_OUT)>TWO_EPSILON_EB) THEN
   CHI_R_SUM=0._EB
   DO NR=1,N_REACTIONS
      RN=>REACTION(NR)
      TIME_RAMP_FACTOR = EVALUATE_RAMP(T,0._EB,RN%RAMP_CHI_R_INDEX)
...
IF (REAC_SOURCE_CHECK) THEN
   REAC_SOURCE_TERM_OUT = RHO_IN*(ZZ_GET-ZZ_0)/DT
   Q_REAC_OUT = Q_REAC_OUT/CELL_VOLUME/DT
ENDIF

END SUBROUTINE COMBUSTION_2STEP


SUBROUTINE CHECK_AUTO_IGNITION(EXTINCT,TMP_MIXED,AIT_IN)
...
! Mowrer model, Tech Guide, Appendix D.1
...
REACTION_LOOP: DO NR=1,N_REACTIONS
   ...
   CALL GET_AVERAGE_SPECIFIC_HEAT(ZZ_IN,CPBAR,TMP_IN)
   Y_O2 = ZZ_IN(RN%AIR_SMIX_INDEX)
   Y_O2_CRIT = CPBAR*(RN%CRIT_FLAME_TMP-TMP_IN)/RN%EPUMO2
   IF (Y_O2 < Y_O2_CRIT) EXTINCT = .TRUE.
ENDDO REACTION_LOOP
...
SUBROUTINE EXTINCT_2A(EXTINCT,ZZ_0_IN,ZZ_IN,TMP_IN,I_RN)

! Default model, FDS Tech Guide, Section 5.3

USE PHYSICAL_FUNCTIONS, ONLY: GET_ENTHALPY
REAL(EB),INTENT(IN) :: TMP_IN,ZZ_0_IN(1:N_TRACKED_SPECIES),ZZ_IN(1:N_TRACKED_SPECIES)
INTEGER, INTENT(IN) :: I_RN
LOGICAL, INTENT(INOUT) :: EXTINCT
REAL(EB) :: ZZ_HAT_0(1:N_TRACKED_SPECIES),ZZ_HAT(1:N_TRACKED_SPECIES),H_0,H_CRIT,PHI_TILDE
INTEGER :: NS
TYPE(REACTION_TYPE), POINTER :: R1=>NULL()
...
!!!!!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZZ_0:',ZZ_0_IN
!!!!!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZZ  :',ZZ_IN
PHI_TILDE = (ZZ_0_IN(R1%AIR_SMIX_INDEX) - ZZ_IN(R1%AIR_SMIX_INDEX)) / ZZ_0_IN(R1%AIR_SMIX_INDEX)  ! FDS Tech Guide (5.45)
!!!IF (DEBUG) WRITE(*,*) PHI_TILDE
! Define the modified pre and post mixtures (ZZ_HAT_0 and ZZ_HAT) in which excess air and products are excluded.
...
DO NS=1,N_TRACKED_SPECIES
   ...
   ENDIF
ENDDO
!!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZZH0:',ZZ_HAT_0
!!!IF (DEBUG) WRITE(*,'(A,5(1X,E13.6))') 'ZZ_H:',ZZ_HAT
! Normalize the modified pre and post mixtures
...
CALL GET_ENTHALPY(ZZ_HAT,H_CRIT,R1%CRIT_FLAME_TMP) ! H of products at the critical flame temperature
IF (H_0 < H_CRIT) EXTINCT = .TRUE. ! FDS Tech Guide (5.46)
!!!IF (DEBUG) WRITE(*,*) 'H H CFT',H_0,H_CRIT,R1%CRIT_FLAME_TMP
!!!!!!IF (DEBUG) WRITE(*,*) 'CPB',CPBAR_Z(1731,4)*R1%CRIT_FLAME_TMP,CPBAR_Z(1731,5)*R1%CRIT_FLAME_TMP
!!!IF (DEBUG) WRITE(*,*) 'EX',EXTINCT
END SUBROUTINE EXTINCT_2A
...
INC_LOOP: DO N=1,N_INC
   ...
   CALL FIRE_FORWARD_EULER(ZZ_3,ZZ_2,ZZ_UNMIXED,ZETA_3,ZETA_2, DT_LOC, TMP_MIXED,TMP_UNMIXED,RHO_HAT,CELL_MASS,TAU_MIX,&
                           Q_REAC_3,SUB_IT,TOTAL_MIXED_MASS_3)
...
WALL_LOOP_2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   Z_F = WC%ONE_D%ZZ_F(REACTION(1)%FUEL_SMIX_INDEX)
...
! This is for plotting regular slices:
DO ICC=1,MESHES(NM)%N_CUTCELL_MESH
   I     = CUT_CELL(ICC)%IJK(IAXIS)
   J     = CUT_CELL(ICC)%IJK(JAXIS)
   K     = CUT_CELL(ICC)%IJK(KAXIS)
