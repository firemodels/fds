...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),CP,H_S_N,&
            REAC_SOURCE_TERM_TMP(N_TRACKED_SPECIES),Q_REAC_TMP(N_REACTIONS),RSUM_LOC,TRI_COR_TMP
LOGICAL :: Q_EXISTS
   ...
   DO J=1,JBAR
      ILOOP: DO I=1,IBAR
         ...
         CALL COMBUSTION_MODEL( T,DT,ZZ_GET,Q(I,J,K),MIX_TIME(I,J,K),CHI_R(I,J,K),&
                                CHEM_SUBIT_TMP,REAC_SOURCE_TERM_TMP,Q_REAC_TMP,TRI_COR_TMP,&
         ...
         DZZ_IF: IF ( ANY(ABS(DZZ) > TWO_EPSILON_EB) ) THEN
            IF (ABS(Q(I,J,K)) > TWO_EPSILON_EB) THEN
               Q_EXISTS = .TRUE.
               IF (TRI_MODEL) TRI_COR(I,J,K) = TRI_COR_TMP
            ENDIF
            ! Divergence term
...
SUBROUTINE COMBUSTION_MODEL(T,DT,ZZ_GET,Q_OUT,MIX_TIME_OUT,CHI_R_OUT,CHEM_SUBIT_OUT,REAC_SOURCE_TERM_OUT,Q_REAC_OUT,TRI_COR_OUT,&
...
REAL(EB), INTENT(OUT) :: Q_OUT,MIX_TIME_OUT,CHI_R_OUT,REAC_SOURCE_TERM_OUT(N_TRACKED_SPECIES),Q_REAC_OUT(N_REACTIONS),TRI_COR_OUT
...
IF ( TMP_IN > AUTO_IGNITION_TEMPERATURE ) EXTINCT = .FALSE.

IF (.NOT.EXTINCT) RETURN

IF (PRESENT(IIC)) THEN
   IF (Q_OLD(IIC-1,JJC,KKC)>HRRPUV_SPARK .AND. U(IIC-1,JJC,KKC)>-LAMINAR_FLAME_SPEED) EXTINCT = .FALSE.
   IF (Q_OLD(IIC+1,JJC,KKC)>HRRPUV_SPARK .AND. U(IIC  ,JJC,KKC)< LAMINAR_FLAME_SPEED) EXTINCT = .FALSE.
   IF (Q_OLD(IIC,JJC-1,KKC)>HRRPUV_SPARK .AND. V(IIC,JJC-1,KKC)>-LAMINAR_FLAME_SPEED) EXTINCT = .FALSE.
   IF (Q_OLD(IIC,JJC+1,KKC)>HRRPUV_SPARK .AND. V(IIC,JJC  ,KKC)< LAMINAR_FLAME_SPEED) EXTINCT = .FALSE.
   IF (Q_OLD(IIC,JJC,KKC-1)>HRRPUV_SPARK .AND. W(IIC,JJC,KKC-1)>-LAMINAR_FLAME_SPEED) EXTINCT = .FALSE.
   IF (Q_OLD(IIC,JJC,KKC+1)>HRRPUV_SPARK .AND. W(IIC,JJC,KKC  )< LAMINAR_FLAME_SPEED) EXTINCT = .FALSE.
ENDIF
...
REAL(EB) :: ZZ_GET(1:N_TRACKED_SPECIES),DZZ(1:N_TRACKED_SPECIES),CP,H_S_N,&
            REAC_SOURCE_TERM_TMP(N_TRACKED_SPECIES),Q_REAC_TMP(N_REACTIONS),VCELL,TRI_COR_TMP
LOGICAL  :: Q_EXISTS_CC
TYPE (REACTION_TYPE), POINTER :: RN
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM
LOGICAL  :: DO_REACTION,REALIZABLE
