...
SPEC_LOOP: DO N = 1, N_AGGLOMERATION_SPECIES
   ...
   DO I=1,N_PARTICLE_BINS(N)
      DO II=1,N_PARTICLE_BINS(N)
         ...
         BINDO:DO III=2,N_PARTICLE_BINS(I)
            IF (PARTICLE_MASS(N,I,II) > BIN_X(N,N_PARTICLE_BINS(I))) THEN
               BIN_ETA_INDEX(N,I,II,:) = N_PARTICLE_BINS
               BIN_ETA(N,I,II,:) = 0.5_EB*BIN_X(N,N_PARTICLE_BINS(I))/PARTICLE_MASS(N,I,II)
               EXIT BINDO
            ELSE
   ...
   BIN_ETA(N,N_PARTICLE_BINS,N_PARTICLE_BINS,1) = 1._EB
   BIN_ETA_INDEX(N,N_PARTICLE_BINS,N_PARTICLE_BINS,1) = N_PARTICLE_BINS(N)
   BIN_ETA(N,N_PARTICLE_BINS,N_PARTICLE_BINS,2) = 0._EB
   BIN_ETA_INDEX(N,N_PARTICLE_BINS,N_PARTICLE_BINS,2) = N_PARTICLE_BINS(N)
...
WALL_CELL_LOOP: DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (SS%AGGLOMERATING) THEN
      ...
      NS2 = INT(SUM(FINDLOC(AGGLOMERATION_SPEC_INDEX,SOOT_INDEX)))
...
SUBROUTINE DROPLET_SCRUBBING(IP,NM)
USE PHYSICAL_FUNCTIONS, ONLY: GET_VISCOSITY
USE GLOBAL_CONSTANTS, ONLY: GVEC
INTEGER, INTENT(IN) :: IP,NM
INTEGER :: IIG,JJG,KKG,NS
REAL(EB) :: VEL,VREL, R_D, FRAC, EFF, EFF_IN, EFF_IM, STK, MU_G, ZZ_GET(1:N_TRACKED_SPECIES), KN, KN_FAC, CN, GRAV_FAC, GRAV_VEL
REAL(EB) :: R_RATIO, EFF_IN_VIS, EFF_IN_POT, EFF_IM_VIS, EFF_IM_POT, RATE
...
KN_FAC = SQRT(2._EB*PI/(PBAR(KKG,PRESSURE_ZONE(IIG,JJG,KKG))*RHO(IIG,JJG,KKG)))*MU_G

VEL = SQRT(LP%U**2 + LP%V**2 + LP%W**2)
R_D = LP%ONE_D%X(1)

FRAC = MIN(1._EB,VEL*LP%PWT*PI*R_D**2 * RDX(IIG)*RRN(IIG)*RDY(JJG)*RDZ(KKG))

!IF (IIG==25 .AND. JJG==25 .AND. KKG==48) WRITE(*,*) 'A',FRAC,VEL,SPECIES_MIXTURE(3)%DEPOSITING

DO NS=1, N_TRACKED_SPECIES
   SM => SPECIES_MIXTURE(NS)
   IF (.NOT. SM%DEPOSITING) CYCLE
   R_RATIO = 0.5_EB*SM%MEAN_DIAMETER/R_D
   EFF_IN_VIS = (1._EB + R_RATIO)**2 * (1._EB - 3._EB/(2._EB*(1._EB + R_RATIO)) + 1._EB/(2._EB*(1._EB + R_RATIO)**3))
   EFF_IN_POT = (1._EB + R_RATIO)**2 - (1._EB + R_RATIO)
   EFF_IN = (EFF_IN_VIS + EFF_IN_POT * LP%RE/60._EB)/(1._EB + LP%RE/60._EB)
   !IF (IIG==25 .AND. JJG==25 .AND. KKG==48) WRITE(*,*) 'B1',EFF_IN_VIS,EFF_IN_POT,EFF_IN
   KN = KN_FAC/SM%MEAN_DIAMETER
   CN = CUNNINGHAM(KN)
   GRAV_FAC = SM%MEAN_DIAMETER**2*SM%DENSITY_SOLID/18._EB
   GRAV_VEL = GRAV_FAC*CN/MU_G
   VREL = SQRT((LP%U - GVEC(1)*GRAV_VEL)**2 + (LP%V - GVEC(2)*GRAV_VEL)**2 + (LP%W  - GVEC(3)*GRAV_VEL)**2)
   STK = 2._EB * SM%MEAN_DIAMETER * SM%DENSITY_SOLID / (9._EB*MU_G*R_D)
   !IF (IIG==25 .AND. JJG==25 .AND. KKG==48) WRITE(*,*) 'STK', STK
   IF (STK > 0.2_EB) THEN
      EFF_IM_POT = 0._EB
   ELSE
      EFF_IM_POT = (STK / (STK + 0.5_EB))**2._EB
      IF (STK > 0.0834) EFF_IM_POT = (1._EB-(STK-0.0834_EB)/(0.2_EB-0.0834_EB))*EFF_IM_POT
   ENDIF
   IF (STK > 1.214_EB) THEN
      EFF_IM_VIS = (1._EB+0.75_EB*LOG(2._EB*STK)/(STK-1.214))**(-2_EB)
   ELSE
      EFF_IM_VIS = 0._EB
   ENDIF
   EFF_IM = (EFF_IM_VIS + EFF_IM_POT * LP%RE/60._EB)/(1._EB + LP%RE/60._EB)
   !IF (IIG==25 .AND. JJG==25 .AND. KKG==48) WRITE(*,*) 'B2',EFF_IM_VIS,EFF_IM_POT,EFF_IM
   EFF = 1._EB - (1._EB - EFF_IN) * (1._EB - EFF_IM)
   RATE = EFF * FRAC * ZZ_GET(NS) * RHO(IIG,JJG,KKG)
   !IF (IIG==25 .AND. JJG==25 .AND. KKG==48) WRITE(*,*) 'C',RATE,EFF,ZZ_GET(NS) * RHO(IIG,JJG,KKG)
