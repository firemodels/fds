!> \brief Routines related to unstructured geometry and computational geometry engine.
...
! REAL(EB), PARAMETER :: MIN_VOL_FACTOR = 0.001_EB ! Threshold cut-cell volume ratio used to block small cut-cells.
...
SUBROUTINE RANDOM_CFACE_XYZ(NM,CFA,CFA_X,CFA_Y,CFA_Z)
...
N_TRI= MESHES(NM)%CUT_FACE(IND1)%CFELEM(1,IND2)
...
CFTRI_LOOP : DO ITRI=1,N_TRI
   ...
   INOD_2 = MESHES(NM)%CUT_FACE(IND1)%CFELEM(1+INOD_2,IND2)
   INOD_3 = MESHES(NM)%CUT_FACE(IND1)%CFELEM(1+INOD_3,IND2)

   ! Compute triangles Area / AreaTOT for CFACE polygon:
   V12(IAXIS:KAXIS) = (/ MESHES(NM)%CUT_FACE(IND1)%XYZVERT(IAXIS,INOD_2)-BOUNDARY_COORD(CFA%BC_INDEX)%X, &
                         MESHES(NM)%CUT_FACE(IND1)%XYZVERT(JAXIS,INOD_2)-BOUNDARY_COORD(CFA%BC_INDEX)%Y, &
                         MESHES(NM)%CUT_FACE(IND1)%XYZVERT(KAXIS,INOD_2)-BOUNDARY_COORD(CFA%BC_INDEX)%Z /)
   V13(IAXIS:KAXIS) = (/ MESHES(NM)%CUT_FACE(IND1)%XYZVERT(IAXIS,INOD_3)-BOUNDARY_COORD(CFA%BC_INDEX)%X, &
                         MESHES(NM)%CUT_FACE(IND1)%XYZVERT(JAXIS,INOD_3)-BOUNDARY_COORD(CFA%BC_INDEX)%Y, &
                         MESHES(NM)%CUT_FACE(IND1)%XYZVERT(KAXIS,INOD_3)-BOUNDARY_COORD(CFA%BC_INDEX)%Z /)
...
CFA_X = E1*BC%X+E2*MESHES(NM)%CUT_FACE(IND1)%XYZVERT(IAXIS,INOD_2)+E3*MESHES(NM)%CUT_FACE(IND1)%XYZVERT(IAXIS,INOD_3)
CFA_Y = E1*BC%Y+E2*MESHES(NM)%CUT_FACE(IND1)%XYZVERT(JAXIS,INOD_2)+E3*MESHES(NM)%CUT_FACE(IND1)%XYZVERT(JAXIS,INOD_3)
CFA_Z = E1*BC%Z+E2*MESHES(NM)%CUT_FACE(IND1)%XYZVERT(KAXIS,INOD_2)+E3*MESHES(NM)%CUT_FACE(IND1)%XYZVERT(KAXIS,INOD_3)
...
MAIN_MESH_LOOP : DO NM=1,NMESHES
   ...
   CALL GET_CCREGION_FACE_CELL_LIST_INFO(NM)
   ...
   ! Block cut-cells whose volume factor is less than MIN_VOL_FACTOR:
   ! CALL BLOCK_SMALL_CUTCELLS(NM)

   ! Here: 1,2. Define Linking information for cut-cells. Block cells that can't be linked,
   ! 3. Define regular faces in cut-cell region, 4. Define RC faces in cut-cell region.
   CALL GET_CELL_LINK_INFO(NM)
   !CALL BLOCK_UNLINKED_CUTCELLS(NM)
   ...
   IF (GET_CUTCELLS_VERBOSE) THEN
      IF(MESHES(NM)%N_CUTCELL_MESH > 0) THEN
         ...
         DO ICC1=1,MESHES(NM)%N_CUTCELL_MESH
            IF (MESHES(NM)%CUT_CELL(ICC1)%NCELL==0) CYCLE
            ...
            ENDDO
         ENDDO
         IF(SUM_CCELL > TWO_EPSILON_EB) MEAN_FACES_PER_CUTCELL = SUM_FACE / SUM_CCELL
         ! Write to file:
   ...
   IF (DEBUG_SET_CUTCELLS) THEN
      CUTFACE_TEST_LOOP : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
         ...
         DO I=1,NFACE
            ...
            WRITE(LU_SETCC,*) 'ZERO AREA=',MESHES(NM)%CUT_FACE(ICF)%STATUS,ICF,I,NFACE,MESHES(NM)%CUT_FACE(ICF)%AREA(I)
            WRITE(LU_SETCC,*) 'CFELEM   =',MESHES(NM)%CUT_FACE(ICF)%CFELEM(1:MESHES(NM)%CUT_FACE(ICF)%CFELEM(1,I)+1,I)
            DO J=1,MESHES(NM)%CUT_FACE(ICF)%CFELEM(1,I)
            WRITE(LU_SETCC,*) J,'=',MESHES(NM)%CUT_FACE(ICF)%XYZVERT(IAXIS:KAXIS,MESHES(NM)%CUT_FACE(ICF)%CFELEM(J+1,I))
            ENDDO
            ENDIF
         ENDDO
      ...
      CUTCELL_TEST_LOOP : DO ICF=1,MESHES(NM)%N_CUTCELL_MESH
         ...
         DO I=1,NCELL
            ...
            WRITE(LU_SETCC,*) 'ZERO VOLU=',ICF,I,NCELL,MESHES(NM)%CUT_CELL(ICF)%VOLUME(I)
            WRITE(LU_SETCC,*) 'CCELEM   =',MESHES(NM)%CUT_CELL(ICF)%CCELEM(1:MESHES(NM)%CUT_CELL(ICF)%CCELEM(1,I)+1,I)
            ENDIF
         ENDDO
      ENDDO CUTCELL_TEST_LOOP
   ENDIF

   ! Compute cut-face apertures for gasphase cut-faces:
   IF (PROCESS(NM)==MY_RANK) THEN
      DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
         ...
         X1AXIS = MESHES(NM)%CUT_FACE(ICF)%IJK(KAXIS+1)
         SELECT CASE(X1AXIS)
         CASE(IAXIS); MESHES(NM)%CUT_FACE(ICF)%ALPHA_CF = SUM(MESHES(NM)%CUT_FACE(ICF)%AREA(1:NFACE))/(DY(J)*DZ(K))
         CASE(JAXIS); MESHES(NM)%CUT_FACE(ICF)%ALPHA_CF = SUM(MESHES(NM)%CUT_FACE(ICF)%AREA(1:NFACE))/(DZ(K)*DX(I))
         CASE(KAXIS); MESHES(NM)%CUT_FACE(ICF)%ALPHA_CF = SUM(MESHES(NM)%CUT_FACE(ICF)%AREA(1:NFACE))/(DX(I)*DY(J))
         END SELECT
      ENDDO
   ENDIF
...
SUBROUTINE GET_CCREGION_FACE_CELL_LIST_INFO(NM)

INTEGER, INTENT(IN) :: NM

! Local Vars:
INTEGER :: ICC,JCC,IFC,IFACE,ICF1,ICF2
...
RETURN
END SUBROUTINE GET_CCREGION_FACE_CELL_LIST_INFO
...
! Local Vars:
INTEGER :: ICF,JCF,ICC,JCC,IBOD
REAL(EB):: AREA_CF_T,FCT,AREA_CRT,VOL_CC_T
LOGICAL, ALLOCATABLE, DIMENSION(:,:,:) :: EXPAND_CC
TYPE(MESH_TYPE), POINTER :: M=>NULL()
LOGICAL :: FLG

M=>MESHES(NM)

ALLOCATE(EXPAND_CC(0-NGUARD:M%IBAR+NGUARD,0-NGUARD:M%JBAR+NGUARD,0-NGUARD:M%KBAR+NGUARD)); EXPAND_CC = .FALSE.

DO K=-CCGUARD,M%KBAR+CCGUARD
   DO J=-CCGUARD,M%JBAR+CCGUARD
      DO I=-CCGUARD,M%IBAR+CCGUARD
         ICF = M%CCVAR(I,J,K,IBM_IDCF)
         IF (ICF > 0) THEN
            FLG=.FALSE.
            DO JCF = 1,M%CUT_FACE(ICF)%NFACE ! Loop over INBOUNDARY CFACEs for the cell.
               IBOD = M%CUT_FACE(ICF)%BODTRI(1,JCF) ! Associated GEOMETRY.
               IF(GEOMETRY(IBOD)%EXPAND_CUTCELLS) THEN
                  FLG=.TRUE.
                  EXIT
               ENDIF
            ENDDO
...
CUTFACE_LOOP : DO ICF=1,M%N_CUTFACE_MESH+M%N_GCCUTFACE_MESH
  IF(M%CUT_FACE(ICF)%STATUS/=IBM_GASPHASE) CYCLE CUTFACE_LOOP
  NFACE  = M%CUT_FACE(ICF)%NFACE
  I      = M%CUT_FACE(ICF)%IJK(IAXIS)
  J      = M%CUT_FACE(ICF)%IJK(JAXIS)
  K      = M%CUT_FACE(ICF)%IJK(KAXIS)
  X1AXIS = M%CUT_FACE(ICF)%IJK(KAXIS+1)
  SELECT CASE(X1AXIS)
     CASE(IAXIS); FLG = ANY(EXPAND_CC(I:I+1,J,K))
     CASE(JAXIS); FLG = ANY(EXPAND_CC(I,J:J+1,K))
     CASE(KAXIS); FLG = ANY(EXPAND_CC(I,J,K:K+1))
  END SELECT
  IF(.NOT.FLG) CYCLE CUTFACE_LOOP
  ! First Areas:
  AREA_CF_T = SUM(M%CUT_FACE(ICF)%AREA(1:NFACE))
  DO JCF=1,NFACE
     FCT = M%CUT_FACE(ICF)%AREA(JCF)/AREA_CF_T
     SELECT CASE(X1AXIS)
     CASE(IAXIS); AREA_CRT = DYCELL(J)*DZCELL(K)
     CASE(JAXIS); AREA_CRT = DXCELL(I)*DZCELL(K)
     CASE(KAXIS); AREA_CRT = DXCELL(I)*DYCELL(J)
     END SELECT
     M%CUT_FACE(ICF)%AREA(JCF) = FCT*AREA_CRT
  ENDDO
  ! Then face centroids:
  M%CUT_FACE(ICF)%XYZCEN(IAXIS,1:NFACE) = XCELL(I)
  M%CUT_FACE(ICF)%XYZCEN(JAXIS,1:NFACE) = YCELL(J)
  M%CUT_FACE(ICF)%XYZCEN(KAXIS,1:NFACE) = ZCELL(K)
  SELECT CASE(X1AXIS)
  CASE(IAXIS); M%CUT_FACE(ICF)%XYZCEN(IAXIS,1:NFACE) = XFACE(I)
  CASE(JAXIS); M%CUT_FACE(ICF)%XYZCEN(JAXIS,1:NFACE) = YFACE(J)
  CASE(KAXIS); M%CUT_FACE(ICF)%XYZCEN(KAXIS,1:NFACE) = ZFACE(K)
  END SELECT
ENDDO CUTFACE_LOOP

CUTCELL_LOOP : DO ICC=1,M%N_CUTCELL_MESH+M%N_GCCUTCELL_MESH
   I      = M%CUT_CELL(ICC)%IJK(IAXIS)
   J      = M%CUT_CELL(ICC)%IJK(JAXIS)
   K      = M%CUT_CELL(ICC)%IJK(KAXIS)
   IF(.NOT.EXPAND_CC(I,J,K)) CYCLE CUTCELL_LOOP
   NCELL  = M%CUT_CELL(ICC)%NCELL
   ! First volumes:
   VOL_CC_T = SUM(M%CUT_CELL(ICC)%VOLUME(1:NCELL))
   DO JCC=1,NCELL
      FCT = M%CUT_CELL(ICC)%VOLUME(JCC)/VOL_CC_T
      M%CUT_CELL(ICC)%VOLUME(JCC) = FCT*DXCELL(I)*DYCELL(J)*DZCELL(K)
   ENDDO
   ! Then cell centroids:
   M%CUT_CELL(ICC)%XYZCEN(IAXIS,1:NCELL) = XCELL(I)
   M%CUT_CELL(ICC)%XYZCEN(JAXIS,1:NCELL) = YCELL(J)
   M%CUT_CELL(ICC)%XYZCEN(KAXIS,1:NCELL) = ZCELL(K)
ENDDO CUTCELL_LOOP
...
IF (GET_CUTCELLS_VERBOSE) THEN
   DO ICF=1,M%N_CUTFACE_MESH+M%N_GCCUTFACE_MESH
      ...
      END SELECT
      AREA_CF_T = SUM(M%CUT_FACE(ICF)%AREA(1:NFACE))
      FCT       = ABS(AREA_CRT-AREA_CF_T)
      IF(FCT>TWO_EPSILON_EB) WRITE(LU_SETCC,*) 'CUT FACE ICF,NFACE,AREAT,ERR=',ICF,NFACE,AREA_CF_T,FCT
   ENDDO
   DO ICC=1,M%N_CUTCELL_MESH+M%N_GCCUTCELL_MESH
      ...
      K      = M%CUT_CELL(ICC)%IJK(KAXIS)
      NCELL  = M%CUT_CELL(ICC)%NCELL
      FCT    = ABS(SUM(M%CUT_CELL(ICC)%VOLUME(1:NCELL))-DXCELL(I)*DYCELL(J)*DZCELL(K))
      IF(FCT>TWO_EPSILON_EB) &
      WRITE(LU_SETCC,*) 'CUT CELL ICC,NCELL,VOLT,ERR=',ICC,NCELL,SUM(M%CUT_CELL(ICC)%VOLUME(1:NCELL)),FCT
   ENDDO
ENDIF

DEALLOCATE(EXPAND_CC)
      ...
      DO J=JLO,JHI
         DO I=ILO,IHI
            ...
            X1AXIS_LOOP3 : DO X1AXIS=IAXIS,KAXIS
               ...
               DO SIDE=LOW_IND,HIGH_IND
                  ...
                  IF ( FSID_XYZ(SIDE,X1AXIS) == IBM_GASPHASE ) THEN
                     ...
                     FACE_LIST(1:IBM_NPARAM_CCFACE,NFACE_CELL) = (/ IBM_FTYPE_RGGAS, SIDE, X1AXIS, 0, 0, IBM_UNDEFINED/)
                  ...
                  ELSEIF (FSID_XYZ(SIDE,X1AXIS) == IBM_CUTCFE ) THEN
                     ...
                     DO ICF=1,MESHES(NM)%CUT_FACE(CEI)%NFACE
                        NFACE_CELL = NFACE_CELL + 1
                        FACE_LIST(1:IBM_NPARAM_CCFACE,NFACE_CELL)=(/ IBM_FTYPE_CFGAS,SIDE,X1AXIS,CEI,ICF,IBM_UNDEFINED/)
                        ! IBM_FTYPE_CFGAS=1
                     ENDDO
                  ENDIF
               ENDDO
            ...
            IF ( CEI > 0 ) THEN
               DO ICF=1,MESHES(NM)%CUT_FACE(CEI)%NFACE
                  NFACE_CELL = NFACE_CELL + 1
                  FACE_LIST(1:IBM_NPARAM_CCFACE,NFACE_CELL) = (/ IBM_FTYPE_CFINB, 0, 0, CEI, ICF, IBM_UNDEFINED /)
...
! -------------------------- BLOCK_SMALL_CUTCELLS -------------------------------

! SUBROUTINE BLOCK_SMALL_CUTCELLS(NM)
!
! INTEGER, INTENT(IN) :: NM
!
! INTEGER :: ICC,JCC,I,J,K,II,JJ,KK,IFC,IFACE,ILH,X1AXIS,I_LNK,J_LNK,K_LNK
! REAL(EB):: VOL
! TYPE(MESH_TYPE), POINTER :: M=>NULL()
! M => MESHES(NM)
!
! DO K=-CCGUARD+1,M%KBAR+CCGUARD-1
!    KK = MIN(M%KBAR,MAX(1,K))
!    DO J=-CCGUARD+1,M%JBAR+CCGUARD-1
!       JJ = MIN(M%JBAR,MAX(1,J))
!       DO I=-CCGUARD+1,M%IBAR+CCGUARD-1
!          ICC = M%CCVAR(I,J,K,IBM_IDCC)
!          IF(ICC<1) CYCLE ! No Cut-cell.
!          II = MIN(M%IBAR,MAX(1,I))
!          VOL= M%DX(II)*M%DY(JJ)*M%DZ(KK)
!          JCC_LOOP : DO JCC=1,M%CUT_CELL(ICC)%NCELL
!             IF(M%CUT_CELL(ICC)%VOLUME(JCC)/VOL > MIN_VOL_FACTOR) CYCLE JCC_LOOP
!             ! Check that cut-cell not next to a REGULAR CELL:
!             IFC_LOOP : DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
!                IFACE = M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
!                IF ( M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)/=IBM_FTYPE_RGGAS ) CYCLE IFC_LOOP
!                ILH     = 2*M%CUT_CELL(ICC)%FACE_LIST(2,IFACE) - 3
!                X1AXIS  = M%CUT_CELL(ICC)%FACE_LIST(3,IFACE)
!                I_LNK = I; J_LNK = J; K_LNK = K
!                SELECT CASE(X1AXIS)
!                CASE(IAXIS); I_LNK = I+ILH
!                CASE(JAXIS); J_LNK = J+ILH
!                CASE(KAXIS); K_LNK = K+ILH
!                END SELECT
!                IF(M%CCVAR(I_LNK,J_LNK,K_LNK,IBM_CGSC)==IBM_GASPHASE) CYCLE JCC_LOOP
!             ENDDO IFC_LOOP
!
!             CALL BLOCK_CUT_CELL(NM,ICC,JCC)
!          ENDDO JCC_LOOP
!       ENDDO
!    ENDDO
! ENDDO
!
! ! Clean up CUT_CELL, CUT_FACE arrays:
! CALL CUT_CELL_FACE_ARRAYS_CLEANUP(NM)
!
! RETURN
! END SUBROUTINE BLOCK_SMALL_CUTCELLS


! -------------------------- BLOCK_UNLINKED_CUTCELLS -------------------------------

! SUBROUTINE BLOCK_UNLINKED_CUTCELLS(NM)
!
! INTEGER, INTENT(IN) :: NM
!
! INTEGER :: ICC,JCC,I,J,K,II,JJ,KK
! REAL(EB):: VOL
! TYPE(MESH_TYPE), POINTER :: M=>NULL()
! !TYPE(IBM_CUTFACE_TYPE), POINTER :: CTF=>NULL()
! M => MESHES(NM)
!
! DO K=-CCGUARD+1,M%KBAR+CCGUARD-1
!    KK = MIN(M%KBAR,MAX(1,K))
...
!          VOL= M%DX(II)*M%DY(JJ)*M%DZ(KK)
!          DO JCC=1,M%CUT_CELL(ICC)%NCELL
!             IF(M%CUT_CELL(ICC)%IJK_LINK(1,JCC)/=IBM_SOLID) CYCLE
!             CALL BLOCK_CUT_CELL(NM,ICC,JCC)
!          ENDDO
!       ENDDO
!    ENDDO
...
! CALL CUT_CELL_FACE_ARRAYS_CLEANUP(NM)
!
! ! Check that INBOUNDARY faces have SURF_INDEX, IBOD and ITRI (used for NVEC) associated:
! ! DO K=-CCGUARD,M%KBAR+CCGUARD
! !    DO J=-CCGUARD,M%JBAR+CCGUARD
! !       DO I=-CCGUARD,M%IBAR+CCGUARD
! !          ICF = M%CCVAR(I,J,K,IBM_IDCF); IF(ICF<1) CYCLE
! !          CTF => M%CUT_FACE(ICF)
! !          IF (CTF%STATUS/=IBM_INBOUNDARY) CYCLE
! !          DO JCF=1,CTF%NFACE
! !             IF(ANY(CTF%BODTRI(1:2,JCF) < 1)) THEN
! !                WRITE(LU_ERR,*) 'INB CF=',ICF,JCF,I,J,K,':',CTF%BODTRI(1:2,JCF),CTF%SURF_INDEX(JCF)
! !             ENDIF
! !          ENDDO
! !       ENDDO
! !    ENDDO
...
!
! RETURN
! END SUBROUTINE BLOCK_UNLINKED_CUTCELLS


! ---------------------- CUT_CELL_FACE_ARRAYS_CLEANUP -----------------------------

! SUBROUTINE CUT_CELL_FACE_ARRAYS_CLEANUP(NM)
!
! INTEGER, INTENT(IN) :: NM
!
! INTEGER, ALLOCATABLE, DIMENSION(:) :: CCIND,CFIND
! INTEGER :: I,J,K,X1AXIS,ICC,JCC,IFC,IFACE,ICF,JCF,IFC1,CT,CTC,CTF,ILH,&
!            N_CUTCELL_MESH_NEW,N_GCCUTCELL_MESH_NEW,N_CUTFACE_MESH_NEW,N_GCCUTFACE_MESH_NEW,N_BBCUTFACE_MESH_NEW
! TYPE(MESH_TYPE), POINTER :: M
! M => MESHES(NM)
! ALLOCATE(CCIND(M%N_CUTCELL_MESH+M%N_GCCUTCELL_MESH),CFIND(M%N_CUTFACE_MESH+M%N_GCCUTFACE_MESH)); CCIND=0; CFIND=0
!
! ! Cout cut-cells and face entries with NCELL, NFACE > 0:
! CTC=0; N_CUTCELL_MESH_NEW=0; N_GCCUTCELL_MESH_NEW=0
! DO ICC=1,M%N_CUTCELL_MESH+M%N_GCCUTCELL_MESH
!    IF(M%CUT_CELL(ICC)%NCELL<1) CYCLE
!    CTC=CTC+1
!    CCIND(ICC) = CTC
!    IF (ICC<=M%N_CUTCELL_MESH) THEN; N_CUTCELL_MESH_NEW   = N_CUTCELL_MESH_NEW   + 1
!    ELSE;                            N_GCCUTCELL_MESH_NEW = N_GCCUTCELL_MESH_NEW + 1; ENDIF
! ENDDO
! CTF=0; N_CUTFACE_MESH_NEW=0; N_GCCUTFACE_MESH_NEW=0; N_BBCUTFACE_MESH_NEW=0
! DO ICF=1,M%N_CUTFACE_MESH+M%N_GCCUTFACE_MESH
!    IF(M%CUT_FACE(ICF)%NFACE<1) CYCLE
!    CTF=CTF+1
!    CFIND(ICF) = CTF
!    IF (ICF<=M%N_BBCUTFACE_MESH) N_BBCUTFACE_MESH_NEW     = N_BBCUTFACE_MESH_NEW + 1
!    IF (ICF<=M%N_CUTFACE_MESH) THEN; N_CUTFACE_MESH_NEW   = N_CUTFACE_MESH_NEW   + 1
!    ELSE;                            N_GCCUTFACE_MESH_NEW = N_GCCUTFACE_MESH_NEW + 1; ENDIF
! ENDDO
!
! ! Move Cut-cells to new location, NCELL=0 entries are dropped:
! DO ICC=1,M%N_CUTCELL_MESH+M%N_GCCUTCELL_MESH
!    IF(M%CUT_CELL(ICC)%NCELL<1 .OR. ICC==CCIND(ICC)) CYCLE
!    CALL CUT_CELL_MOVE(M%CUT_CELL(ICC),M%CUT_CELL(CCIND(ICC)))
! ENDDO
! M%N_CUTCELL_MESH   = N_CUTCELL_MESH_NEW
! M%N_GCCUTCELL_MESH = N_GCCUTCELL_MESH_NEW
!
! ! Now Cut-faces:
! DO ICF=1,M%N_CUTFACE_MESH+M%N_GCCUTFACE_MESH
!    IF(M%CUT_FACE(ICF)%NFACE<1 .OR. ICF==CFIND(ICF)) CYCLE
!    CALL CUT_FACE_MOVE(M%CUT_FACE(ICF),M%CUT_FACE(CFIND(ICF)))
! ENDDO
! M%N_CUTFACE_MESH   = N_CUTFACE_MESH_NEW
! M%N_GCCUTFACE_MESH = N_GCCUTFACE_MESH_NEW
! M%N_BBCUTFACE_MESH = N_BBCUTFACE_MESH_NEW
!
! ! Finally fix ICC and ICF in CCVAR, FCVAR, CELL_LIST and FACE_LIST arrays
! DO ICC=1,M%N_CUTCELL_MESH+M%N_GCCUTCELL_MESH
!    I = M%CUT_CELL(ICC)%IJK(IAXIS); J = M%CUT_CELL(ICC)%IJK(JAXIS); K = M%CUT_CELL(ICC)%IJK(KAXIS);
!    M%CCVAR(I,J,K,IBM_IDCC) = ICC
!    DO JCC=1,M%CUT_CELL(ICC)%NCELL
!       DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
!          IFACE = M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
!          IF ( .NOT.(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_CFINB .OR. &
!                     M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_CFGAS) ) CYCLE
!          IFC1  = M%CUT_CELL(ICC)%FACE_LIST(4,IFACE)
!          M%CUT_CELL(ICC)%FACE_LIST(4,IFACE) = CFIND(IFC1)
!       ENDDO
!    ENDDO
! ENDDO
!
! DO ICF=1,M%N_CUTFACE_MESH+M%N_GCCUTFACE_MESH
!    CT     = HIGH_IND
!    I      = M%CUT_FACE(ICF)%IJK(IAXIS); J = M%CUT_FACE(ICF)%IJK(JAXIS); K = M%CUT_FACE(ICF)%IJK(KAXIS)
!    X1AXIS = M%CUT_FACE(ICF)%IJK(KAXIS+1)
!    SELECT CASE(M%CUT_FACE(ICF)%STATUS)
!    CASE(IBM_INBOUNDARY)
!       CT = LOW_IND
!       M%CCVAR(I,J,K,IBM_IDCF) = ICF
!    CASE(IBM_GASPHASE)
!       M%FCVAR(I,J,K,IBM_IDCF,X1AXIS) = ICF
!    END SELECT
!    DO JCF=1,M%CUT_FACE(ICF)%NFACE
!       DO ILH=LOW_IND,CT
!          IF (M%CUT_FACE(ICF)%CELL_LIST(1,ILH,JCF)==IBM_FTYPE_CFGAS) THEN
!             ICC = M%CUT_FACE(ICF)%CELL_LIST(2,ILH,JCF)
!             M%CUT_FACE(ICF)%CELL_LIST(2,ILH,JCF) = CCIND(ICC)
!          ENDIF
!       ENDDO
!    ENDDO
! ENDDO
...
!
! RETURN
! END SUBROUTINE CUT_CELL_FACE_ARRAYS_CLEANUP

! ---------------------------- BLOCK_CUT_CELL -------------------------------------

! SUBROUTINE BLOCK_CUT_CELL(NM,ICC,JCC)
!
! INTEGER, INTENT(IN) :: NM,ICC,JCC
!
! INTEGER :: I,J,K,II,JJ,KK,IFC,IFC1,JFC1,IFACE,LOHI,ILH,X1AXIS,NSVERT,NSFACE,NVERTFACE_NEW,COUNT,DUM,IBOD,ITRI,&
!            HILO,ILHF,ICC2,JCC2,IFC2,IFACE2,IFCX,JFCX,IV,IVERT,MAXVERTS,INOD,INDFC(1:4)
! REAL(EB):: XYZV(IAXIS:KAXIS)
! INTEGER, ALLOCATABLE, DIMENSION(:,:) :: BODTRI
! INTEGER, ALLOCATABLE, DIMENSION(:)   :: CFELEM
! REAL(EB),ALLOCATABLE, DIMENSION(:)   :: AREA
! LOGICAL :: REALLOC_FLG, NEW_FACE_FLG
! TYPE(MESH_TYPE), POINTER :: M
! M => MESHES(NM)
!
! IBOD = 0; ITRI = 0
...
! DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
!    IFACE   =   M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
!    IF (M%CUT_CELL(ICC)%FACE_LIST(1,IFACE) == IBM_FTYPE_CFINB) COUNT = COUNT + 1
! ENDDO
! IF (COUNT>0) THEN
...
!       IF (M%CUT_CELL(ICC)%FACE_LIST(1,IFACE) /= IBM_FTYPE_CFINB) CYCLE
!       DO DUM=1,COUNT
...
! IFC_LOOP : DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
!    IFACE   =   M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
!    LOHI    =   M%CUT_CELL(ICC)%FACE_LIST(2,IFACE)
...
!    X1AXIS  =   M%CUT_CELL(ICC)%FACE_LIST(3,IFACE)
!    IFCX    =   M%CUT_CELL(ICC)%FACE_LIST(4,IFACE)
!    JFCX    =   M%CUT_CELL(ICC)%FACE_LIST(5,IFACE)
!
!    FACE_TYPE_IF : IF(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_RGGAS .OR. &
!                      M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_CFGAS) THEN
!       ! If needed reallocate CUT_FACE to accomodate INBOUNDARY face in neighbor cell.
!       SELECT CASE(X1AXIS)
!       CASE(IAXIS); II=I+ILH; JJ=J; KK=K
!       CASE(JAXIS); II=I; JJ=J+ILH; KK=K
!       CASE(KAXIS); II=I; JJ=J; KK=K+ILH
!       END SELECT
!       IF(M%CCVAR(II,JJ,KK,IBM_CGSC)/=IBM_CUTCFE) CYCLE IFC_LOOP
!       IFC1 = M%CCVAR(II,JJ,KK,IBM_IDCF) ! INBOUNDARY cut-faces in neighbor cartesian cell.
!       NEW_FACE_FLG = .FALSE.
!       IF (IFC1 < 1) THEN
!          ! Insert IFC1:
!          CALL INSERT_CUT_FACE(NM,II,JJ,KK,0,IFC1); M => MESHES(NM) ! Make space for INBOUNDARY cut-face
!          NEW_FACE_FLG = .TRUE.
!       ENDIF
!       REALLOC_FLG = .FALSE.
!       NSVERT = 0; NSFACE = 0;
!       IF(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_RGGAS) NVERTFACE_NEW = 5
!       IF(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_CFGAS) NVERTFACE_NEW = M%CUT_FACE(IFCX)%CFELEM(1,JFCX)+1
!       IF(SIZE(M%CUT_FACE(IFC1)%CFELEM, DIM=1) < NVERTFACE_NEW) REALLOC_FLG = .TRUE.
!       IF(SIZE(M%CUT_FACE(IFC1)%XYZVERT,DIM=2) < M%CUT_FACE(IFC1)%NVERT+NVERTFACE_NEW-1) THEN
!          REALLOC_FLG = .TRUE.
!          NSVERT      = NVERTFACE_NEW-1
!       ENDIF
!       IF(SIZE(M%CUT_FACE(IFC1)%AREA,DIM=1) < M%CUT_FACE(IFC1)%NFACE+1) THEN
!          REALLOC_FLG = .TRUE.
!          NSFACE      = 1
!       ENDIF
!       JFC1 = M%CUT_FACE(IFC1)%NFACE+1
!
!       ! Reallocate CUT_FACE(IFC1) entry:
!       IF(NEW_FACE_FLG) THEN
!          CALL FACE_DEALLOC(NM,IFC1); CALL NEW_FACE_ALLOC(NM,IFC1,NSVERT,NSFACE,NVERTFACE_NEW)
!       ELSEIF(REALLOC_FLG) THEN
!          CALL FACE_REALLOC(NM,IFC1,M%CUT_FACE(IFC1)%NVERT,M%CUT_FACE(IFC1)%NFACE,NSVERT,NSFACE,NVERTFACE_NEW)
!       ENDIF
!       M=>MESHES(NM)
!
!       ! Provide GEOM surface information to newly created INBOUNDARY face:
!       M%CUT_FACE(IFC1)%BODTRI(1:2,JFC1)         = (/ IBOD, ITRI /)
!       M%CUT_FACE(IFC1)%SURF_INDEX(JFC1)         = 0 ! Default surf.
!       IF(IBOD>0) M%CUT_FACE(IFC1)%SURF_INDEX(JFC1)         = GEOMETRY(IBOD)%SURFS(ITRI)
!       M%CUT_FACE(IFC1)%NFACE = JFC1
!    ENDIF FACE_TYPE_IF
!
!    SELECT CASE(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE))
!    CASE(IBM_FTYPE_RGGAS) ! This regular face has to be connecting two small cut-cells.
!       ! Scheme:
!       ! 1. Add INBOUNDARY cut-face with size of RGGAS in CUT_FACE for this face (IFC1,JFC1).
!       DUM = M%CUT_FACE(IFC1)%NVERT + 1
!       SELECT CASE(X1AXIS)
!       CASE(IAXIS)
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I+ILHF), M%Y(J-1), M%Z(K-1) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I+ILHF), M%Y(J  ), M%Z(K-1) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I+ILHF), M%Y(J  ), M%Z(K  ) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I+ILHF), M%Y(J-1), M%Z(K  ) /)
!          M%CUT_FACE(IFC1)%AREA(JFC1)               = M%DY(J)*M%DZ(K)
!          M%CUT_FACE(IFC1)%XYZCEN(IAXIS:KAXIS,JFC1) = (/ M%X(I+ILHF), M%YC(J), M%ZC(K) /)
!       CASE(JAXIS)
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I-1), M%Y(J+ILHF), M%Z(K-1) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I-1), M%Y(J+ILHF), M%Z(K  ) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I  ), M%Y(J+ILHF), M%Z(K  ) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I  ), M%Y(J+ILHF), M%Z(K-1) /)
!          M%CUT_FACE(IFC1)%AREA(JFC1)               = M%DX(I)*M%DZ(K)
!          M%CUT_FACE(IFC1)%XYZCEN(IAXIS:KAXIS,JFC1) = (/ M%XC(I), M%Y(J+ILHF), M%ZC(K) /)
!       CASE(KAXIS)
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I-1), M%Y(J-1), M%Z(K+ILHF) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I  ), M%Y(J-1), M%Z(K+ILHF) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I  ), M%Y(J  ), M%Z(K+ILHF) /); DUM = DUM + 1
!          M%CUT_FACE(IFC1)%XYZVERT(IAXIS:KAXIS,DUM) = (/ M%X(I-1), M%Y(J  ), M%Z(K+ILHF) /)
!          M%CUT_FACE(IFC1)%AREA(JFC1)               = M%DX(I)*M%DY(J)
!          M%CUT_FACE(IFC1)%XYZCEN(IAXIS:KAXIS,JFC1) = (/ M%XC(I), M%YC(J), M%Z(K+ILHF) /)
!       END SELECT
!       INDFC(1:4) = (/ 1, 2, 3, 4 /); INDFC = INDFC + M%CUT_FACE(IFC1)%NVERT
!       M%CUT_FACE(IFC1)%NVERT = DUM
!
!       ! All faces connectivities:         (/ NNODS, NOD1, NOD2, NOD3, NOD4 /) ! Conn. into gas region of new cell.
!       IF (LOHI==LOW_IND) THEN; M%CUT_FACE(IFC1)%CFELEM(1:5,JFC1)= (/ 4, INDFC(1), INDFC(2), INDFC(3), INDFC(4) /)
!       ELSE;                    M%CUT_FACE(IFC1)%CFELEM(1:5,JFC1)= (/ 4, INDFC(1), INDFC(4), INDFC(3), INDFC(2) /); ENDIF
!
!       ! 2. Find cut-cell sharing this RGGAS face, and where in FACE_LIST this face is.
!       ! 3. Change in FACE_LIST -> (/IBM_FTYPE_RGGAS,SIDE,MYAXIS,0,0/) to (/IBM_FTYPE_CFINB,0,0,IFC1,JFC1/).
!       ! 4. Make FCVAR(I,J,K,IBM_CGSC,X1AXIS)=IBM_SOLID;
!       ICC2 = M%CCVAR(II,JJ,KK,IBM_IDCC)
!       JCC2_LOOP_1 : DO JCC2=1,M%CUT_CELL(ICC2)%NCELL
!          DO IFC2=1,M%CUT_CELL(ICC2)%CCELEM(1,JCC2)
!             IFACE2 = M%CUT_CELL(ICC2)%CCELEM(IFC2+1,JCC2)
!             IF( M%CUT_CELL(ICC2)%FACE_LIST(1,IFACE2)==IBM_FTYPE_RGGAS .AND. &
!                 M%CUT_CELL(ICC2)%FACE_LIST(2,IFACE2)==HILO            .AND. &
!                 M%CUT_CELL(ICC2)%FACE_LIST(3,IFACE2)==X1AXIS) THEN
!                M%CUT_CELL(ICC2)%FACE_LIST(1:5,IFACE2) = (/ IBM_FTYPE_CFINB, 0, 0, IFC1, JFC1 /)
!                M%CUT_FACE(IFC1)%CELL_LIST(1:4,LOW_IND,JFC1) =(/IBM_FTYPE_CFGAS, ICC2, JCC2, IFC2 /)
!                SELECT CASE(X1AXIS)
!                CASE(IAXIS); M%FCVAR(I+ILHF,J,K,IBM_FGSC,X1AXIS) = IBM_SOLID
!                CASE(JAXIS); M%FCVAR(I,J+ILHF,K,IBM_FGSC,X1AXIS) = IBM_SOLID
!                CASE(KAXIS); M%FCVAR(I,J,K+ILHF,IBM_FGSC,X1AXIS) = IBM_SOLID
!                END SELECT
!                EXIT JCC2_LOOP_1
!             ENDIF
!          ENDDO
...
!
!       ! Scheme:
!       ! 1. Add INBOUNDARY cut-face in CUT_FACE for this face (IFC1,JFC1).
!       ! Add XYZVERT, AREA, XYZCEN and CFELEM entry in CUT_FACE(IFC1) for this (IFCX,JFCX) CFGAS face.
!       M%CUT_FACE(IFC1)%CFELEM(1,JFC1) = M%CUT_FACE(IFCX)%CFELEM(1,JFCX)
!       MAXVERTS = SIZE(M%CUT_FACE(IFC1)%XYZVERT,DIM=2)
!       COUNT=1
!       DO IVERT=1,M%CUT_FACE(IFCX)%CFELEM(1,JFCX)
!          IV=M%CUT_FACE(IFCX)%CFELEM(IVERT+1,JFCX)
!          XYZV(IAXIS:KAXIS) =M%CUT_FACE(IFCX)%XYZVERT(IAXIS:KAXIS,IV)
!          CALL INSERT_FACE_VERT_LOC(MAXVERTS,XYZV,M%CUT_FACE(IFC1)%NVERT,INOD,M%CUT_FACE(IFC1)%XYZVERT)
!          COUNT=COUNT+1
!          M%CUT_FACE(IFC1)%CFELEM(COUNT,JFC1)=INOD
!       ENDDO
!       IF (HILO==HIGH_IND) THEN ! Mirror the connectivity, s.t. normal pointing inside:
!          COUNT=M%CUT_FACE(IFC1)%CFELEM(1,JFC1)
!          ALLOCATE(CFELEM(COUNT)); CFELEM(1:COUNT) = M%CUT_FACE(IFC1)%CFELEM(COUNT+1:2:-1,JFC1)
!          M%CUT_FACE(IFC1)%CFELEM(2:COUNT+1,JFC1) = CFELEM(1:COUNT)
!          DEALLOCATE(CFELEM)
!       ENDIF
!       M%CUT_FACE(IFC1)%AREA(JFC1)     = M%CUT_FACE(IFCX)%AREA(JFCX)
!       M%CUT_FACE(IFC1)%XYZCEN(:,JFC1) = M%CUT_FACE(IFCX)%XYZCEN(:,JFCX)
!
!       ! 2. Find cut-cell sharing this CFGAS face (IFCX,JFCX), find where in saids cell FACE_LIST this face is.
!       ! 3. Change in FACE_LIST -> (/IBM_FTYPE_CFGAS,SIDE,MYAXIS,IFCX,JFCX/) to (/IBM_FTYPE_CFINB,0,0,IFC1,JFC1/)
!       ! 4. Drop (IFCX,JFCX) from CUT_FACE(IFCX), if new CUT_FACE(IFCX)%NFACE=0 then:
!       !    a. Make FCVAR(I,J,K,IBM_CGSC,X1AXIS)=IBM_SOLID; FCVAR(I,J,K,IBM_IDCF,X1AXIS)=IBM_UNDEFINED;
!       !    b. Make CUT_FACE(IFCX)%STATUS=IBM_SOLID.
!       !    Note this changes the face arrays, so FACE_LIST face indexes for cut-cells associated to CUT_FACE(IFCX)
!       !    remaining cut faces need to be updated.
!       ICC2 = M%CCVAR(II,JJ,KK,IBM_IDCC)
!       JCC2_LOOP_2 : DO JCC2=1,M%CUT_CELL(ICC2)%NCELL
!          DO IFC2=1,M%CUT_CELL(ICC2)%CCELEM(1,JCC2)
!             IFACE2 = M%CUT_CELL(ICC2)%CCELEM(IFC2+1,JCC2)
!             IF( M%CUT_CELL(ICC2)%FACE_LIST(1,IFACE2)==IBM_FTYPE_CFGAS .AND. &
!                 M%CUT_CELL(ICC2)%FACE_LIST(4,IFACE2)==IFCX            .AND. &
!                 M%CUT_CELL(ICC2)%FACE_LIST(5,IFACE2)==JFCX) THEN
!                M%CUT_CELL(ICC2)%FACE_LIST(1:5,IFACE2) = (/ IBM_FTYPE_CFINB, 0, 0, IFC1, JFC1 /)
!                M%CUT_FACE(IFC1)%CELL_LIST(1:4,LOW_IND,JFC1) =(/IBM_FTYPE_CFGAS, ICC2, JCC2, IFC2 /)
!                ! Drop (IFCX,JFCX) from CUT_FACE(IFCX):
!                CALL DROP_CUTFACE(NM,IBM_FTYPE_CFGAS,I,J,K,ILHF,X1AXIS,IFCX,JFCX)
!                EXIT JCC2_LOOP_2
!             ENDIF
!          ENDDO
!       ENDDO JCC2_LOOP_2
...
!
!       ! Scheme:
!       ! 1. Drop (IFC2,JFC2) from CUT_FACE(IFC2). Note this changes the face arrays, so FACE_LIST face indexes
!       !    for cut-cells on this CUT_CELL(ICC) entry need to be updated.
!       CALL DROP_CUTFACE(NM,IBM_FTYPE_CFINB,I,J,K,ILHF,X1AXIS,IFCX,JFCX)
!
!    END SELECT
!
! ENDDO IFC_LOOP
!
! ! At this point all faces defining the ICC,JCC cut-cell have been dropped in the CUT_FACE, CUT_CELL trees.
! ! We can drop JCC from CUT_CELL(ICC)%CCELEM, etc.
! CALL DROP_CUTCELL(NM,ICC,JCC)
!
!
! RETURN
! END SUBROUTINE BLOCK_CUT_CELL

! --------------------------- INSERT_CUT_FACE -----------------------------------------------

! SUBROUTINE INSERT_CUT_FACE(NM,I,J,K,AXIS,ICF)
!
! ! This routine add a cut-face entry ICF in the CUT_FACE array:
! ! 1. IF AXIS = 0 INBOUNDARY face:
! !    ICF = MESHES(NM)%N_CUTFACE_MESH+1                               if II,JJ,KK is an interior cell.
! !    ICF = MESHES(NM)%N_CUTFACE_MESH+MESHES(NM)%N_GCCUTFACE_MESH+1   if II,JJ,KK is a guard cell.
! ! 2. IF AXIS = 1,2,3 GASPHASE face:
! !    ICF = MESHES(NM)%N_BBCUTFACE_MESH+1                             if II,JJ,KK,AXIS is a boundary face.
! !    ICF = MESHES(NM)%N_CUTFACE_MESH+1                               if II,JJ,KK,AXIS is an interior face.
! !    ICF = MESHES(NM)%N_CUTFACE_MESH+MESHES(NM)%N_GCCUTFACE_MESH+1   if II,JJ,KK,AXIS is a guard face.
!
! INTEGER, INTENT(IN) :: NM,I,J,K,AXIS
! INTEGER, INTENT(OUT):: ICF
!
!
! INTEGER :: ICC,JCC,IFC,IFACE,IFCX,DUM,IDUM,JDUM,KDUM,X1AXIS
! TYPE(IBM_CUTFACE_TYPE), ALLOCATABLE, DIMENSION(:) :: CUT_FACE_AUX
!
! IF(AXIS==0) THEN
!    IF( 0<I .AND. I<MESHES(NM)%IBP1 .AND. 0<J .AND. J<MESHES(NM)%JBP1 .AND. 0<K .AND. K<MESHES(NM)%KBP1 ) THEN
!       MESHES(NM)%N_CUTFACE_MESH = MESHES(NM)%N_CUTFACE_MESH + 1
!       ICF = MESHES(NM)%N_CUTFACE_MESH
!    ELSE
!       MESHES(NM)%N_GCCUTFACE_MESH = MESHES(NM)%N_GCCUTFACE_MESH+1
!       ICF = MESHES(NM)%N_CUTFACE_MESH + MESHES(NM)%N_GCCUTFACE_MESH
!    ENDIF
! ELSE
!    SELECT CASE(AXIS)
!    CASE(IAXIS)
!       IF(0<J .AND. J<MESHES(NM)%JBP1 .AND. 0<K .AND. K<MESHES(NM)%KBP1) THEN
!          IF (0<I .AND. I<MESHES(NM)%IBAR) THEN ! Internal
!             MESHES(NM)%N_CUTFACE_MESH = MESHES(NM)%N_CUTFACE_MESH + 1
!             ICF = MESHES(NM)%N_CUTFACE_MESH
!          ELSEIF(I<0 .OR. I>MESHES(NM)%IBAR) THEN ! External
!             MESHES(NM)%N_GCCUTFACE_MESH = MESHES(NM)%N_GCCUTFACE_MESH+1
!             ICF = MESHES(NM)%N_CUTFACE_MESH + MESHES(NM)%N_GCCUTFACE_MESH
!          ELSEIF(I==0 .OR. I==MESHES(NM)%IBAR) THEN ! Block boundary
!             MESHES(NM)%N_BBCUTFACE_MESH = MESHES(NM)%N_BBCUTFACE_MESH+1
!             MESHES(NM)%N_CUTFACE_MESH   = MESHES(NM)%N_CUTFACE_MESH + 1
!             ICF = MESHES(NM)%N_BBCUTFACE_MESH
!          ENDIF
!       ELSE ! External
!          MESHES(NM)%N_GCCUTFACE_MESH = MESHES(NM)%N_GCCUTFACE_MESH+1
!          ICF = MESHES(NM)%N_CUTFACE_MESH + MESHES(NM)%N_GCCUTFACE_MESH
!       ENDIF
!    CASE(JAXIS)
!       IF(0<I .AND. I<MESHES(NM)%IBP1 .AND. 0<K .AND. K<MESHES(NM)%KBP1) THEN
!          IF (0<J .AND. J<MESHES(NM)%JBAR) THEN ! Internal
!             MESHES(NM)%N_CUTFACE_MESH = MESHES(NM)%N_CUTFACE_MESH + 1
!             ICF = MESHES(NM)%N_CUTFACE_MESH
!          ELSEIF(J<0 .OR. J>MESHES(NM)%JBAR) THEN ! External
!             MESHES(NM)%N_GCCUTFACE_MESH = MESHES(NM)%N_GCCUTFACE_MESH+1
!             ICF = MESHES(NM)%N_CUTFACE_MESH + MESHES(NM)%N_GCCUTFACE_MESH
!          ELSEIF(J==0 .OR. J==MESHES(NM)%JBAR) THEN ! Block boundary
!             MESHES(NM)%N_BBCUTFACE_MESH = MESHES(NM)%N_BBCUTFACE_MESH+1
!             MESHES(NM)%N_CUTFACE_MESH   = MESHES(NM)%N_CUTFACE_MESH + 1
!             ICF = MESHES(NM)%N_BBCUTFACE_MESH
!          ENDIF
...
!       ENDIF
!    CASE(KAXIS)
!       IF(0<J .AND. J<MESHES(NM)%JBP1 .AND. 0<I .AND. I<MESHES(NM)%IBP1) THEN
!          IF (0<K .AND. K<MESHES(NM)%KBAR) THEN ! Internal
!             MESHES(NM)%N_CUTFACE_MESH = MESHES(NM)%N_CUTFACE_MESH + 1
!             ICF = MESHES(NM)%N_CUTFACE_MESH
!          ELSEIF(K<0 .OR. K>MESHES(NM)%KBAR) THEN ! External
!             MESHES(NM)%N_GCCUTFACE_MESH = MESHES(NM)%N_GCCUTFACE_MESH+1
!             ICF = MESHES(NM)%N_CUTFACE_MESH + MESHES(NM)%N_GCCUTFACE_MESH
!          ELSEIF(K==0 .OR. K==MESHES(NM)%KBAR) THEN ! Block boundary
!             MESHES(NM)%N_BBCUTFACE_MESH = MESHES(NM)%N_BBCUTFACE_MESH+1
!             MESHES(NM)%N_CUTFACE_MESH   = MESHES(NM)%N_CUTFACE_MESH + 1
!             ICF = MESHES(NM)%N_BBCUTFACE_MESH
!          ENDIF
...
!
! ! Reallocate CUT_FACE:
! ALLOCATE(CUT_FACE_AUX( MAX(SIZE(MESHES(NM)%CUT_FACE,DIM=1),MESHES(NM)%N_CUTFACE_MESH+MESHES(NM)%N_GCCUTFACE_MESH) ))
! DO DUM=1,ICF-1
!    CALL CUT_FACE_MOVE(MESHES(NM)%CUT_FACE(DUM),CUT_FACE_AUX(DUM))
! ENDDO
! DO DUM=ICF,MESHES(NM)%N_CUTFACE_MESH+MESHES(NM)%N_GCCUTFACE_MESH-1
!    CALL CUT_FACE_MOVE(MESHES(NM)%CUT_FACE(DUM),CUT_FACE_AUX(DUM+1))
! ENDDO
! CALL MOVE_ALLOC(FROM=CUT_FACE_AUX,TO=MESHES(NM)%CUT_FACE)
!
! ! Reset FACE_LIST, FCVAR and CCVAR (IBM_IDCF):
! DO KDUM=-CCGUARD,MESHES(NM)%KBAR+CCGUARD
!    DO JDUM=-CCGUARD,MESHES(NM)%JBAR+CCGUARD
!       DO IDUM=-CCGUARD,MESHES(NM)%IBAR+CCGUARD
!          IF(MESHES(NM)%CCVAR(IDUM,JDUM,KDUM,IBM_IDCF)>=ICF) &
!          MESHES(NM)%CCVAR(IDUM,JDUM,KDUM,IBM_IDCF)=MESHES(NM)%CCVAR(IDUM,JDUM,KDUM,IBM_IDCF) + 1
!          DO X1AXIS=IAXIS,KAXIS
!             IF(MESHES(NM)%FCVAR(IDUM,JDUM,KDUM,IBM_IDCF,X1AXIS)>=ICF) &
!             MESHES(NM)%FCVAR(IDUM,JDUM,KDUM,IBM_IDCF,X1AXIS) = MESHES(NM)%FCVAR(IDUM,JDUM,KDUM,IBM_IDCF,X1AXIS) + 1
!          ENDDO
!       ENDDO
!    ENDDO
! ENDDO
...
!       DO IFC=1,MESHES(NM)%CUT_CELL(ICC)%CCELEM(1,JCC)
!          IFACE   =   MESHES(NM)%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
!          IFCX    =   MESHES(NM)%CUT_CELL(ICC)%FACE_LIST(4,IFACE)
!          IF(IFCX >= ICF) MESHES(NM)%CUT_CELL(ICC)%FACE_LIST(4,IFACE) = IFCX+1
!       ENDDO
!    ENDDO
! ENDDO
!
! IF(AXIS==0) THEN
!     MESHES(NM)%CCVAR(I,J,K,IBM_IDCF) = ICF
!     MESHES(NM)%CUT_FACE(ICF)%STATUS  = IBM_INBOUNDARY
! ELSE
!     MESHES(NM)%FCVAR(I,J,K,IBM_FGSC,AXIS) = IBM_CUTCFE
!     MESHES(NM)%FCVAR(I,J,K,IBM_IDCF,AXIS) = ICF
!     MESHES(NM)%CUT_FACE(ICF)%STATUS       = IBM_GASPHASE
! ENDIF
!
! RETURN
! END SUBROUTINE INSERT_CUT_FACE

! ----------------------------- DROP_CUTFACE --------------------------------------

! SUBROUTINE DROP_CUTFACE(NM,FTYPE,I,J,K,ILHF,X1AXIS,IFC,JFC)
!
! INTEGER, INTENT(IN) :: NM,FTYPE,I,J,K,ILHF,X1AXIS,IFC,JFC
!
! INTEGER :: CT,DUM,ILH,ICC1,JCC1,IFACE,IFC1
! INTEGER, ALLOCATABLE, DIMENSION(:) :: IND
! TYPE(MESH_TYPE), POINTER :: M
! M => MESHES(NM)
!
! NFACE_IF_1 : IF(M%CUT_FACE(IFC)%NFACE>1) THEN
!    ALLOCATE(IND(M%CUT_FACE(IFC)%NFACE)); IND = 0
!    CT=0;
!    DO DUM=1,M%CUT_FACE(IFC)%NFACE
!       IF(DUM==JFC) CYCLE
!       CT = CT + 1
!       IND(DUM) = CT
!    ENDDO
!    ! Collapse NFACE variables:
!    DO DUM=1,M%CUT_FACE(IFC)%NFACE
!       IF(DUM==JFC) CYCLE
!       M%CUT_FACE(IFC)%CFELEM(     :,IND(DUM)) = M%CUT_FACE(IFC)%CFELEM(     :,DUM)
!       M%CUT_FACE(IFC)%AREA(         IND(DUM)) = M%CUT_FACE(IFC)%AREA(         DUM)
!       M%CUT_FACE(IFC)%XYZCEN(     :,IND(DUM)) = M%CUT_FACE(IFC)%XYZCEN(     :,DUM)
!       M%CUT_FACE(IFC)%SHARED(       IND(DUM)) = M%CUT_FACE(IFC)%SHARED(       DUM)
!       M%CUT_FACE(IFC)%CELL_LIST(:,:,IND(DUM)) = M%CUT_FACE(IFC)%CELL_LIST(:,:,DUM)
!       ! Finally change FACE_LIST of involved cells:
!       CT = HIGH_IND; IF(FTYPE==IBM_FTYPE_CFINB) CT = LOW_IND
!       DO ILH=LOW_IND,CT
!          ICC1 = M%CUT_FACE(IFC)%CELL_LIST(2,ILH,IND(DUM))
!          JCC1 = M%CUT_FACE(IFC)%CELL_LIST(3,ILH,IND(DUM))
!          IFC1 = M%CUT_FACE(IFC)%CELL_LIST(4,ILH,IND(DUM))
!          IFACE= M%CUT_CELL(ICC1)%CCELEM(IFC1+1,JCC1)
!          M%CUT_CELL(ICC1)%FACE_LIST(5,IFACE) = IND(DUM)
!       ENDDO
!    ENDDO
!    DEALLOCATE(IND)
! ENDIF NFACE_IF_1
!
! M%CUT_FACE(IFC)%NFACE = M%CUT_FACE(IFC)%NFACE - 1
! IF(M%CUT_FACE(IFC)%NFACE < 1) THEN
!    M%CUT_FACE(IFC)%STATUS = IBM_SOLID
!    IF (FTYPE == IBM_FTYPE_CFGAS) THEN
!       SELECT CASE(X1AXIS)
!       CASE(IAXIS)
!       M%FCVAR(I+ILHF,J,K,IBM_FGSC,X1AXIS) = IBM_SOLID; M%FCVAR(I+ILHF,J,K,IBM_IDCF,X1AXIS) = IBM_UNDEFINED
!       CASE(JAXIS)
!       M%FCVAR(I,J+ILHF,K,IBM_FGSC,X1AXIS) = IBM_SOLID; M%FCVAR(I,J+ILHF,K,IBM_IDCF,X1AXIS) = IBM_UNDEFINED
!       CASE(KAXIS)
!       M%FCVAR(I,J,K+ILHF,IBM_FGSC,X1AXIS) = IBM_SOLID; M%FCVAR(I,J,K+ILHF,IBM_IDCF,X1AXIS) = IBM_UNDEFINED
!       END SELECT
!    ELSEIF (FTYPE == IBM_FTYPE_CFINB) THEN
!       M%CCVAR(I,J,K,IBM_IDCF) = IBM_UNDEFINED
!    ENDIF
! ENDIF
!
! RETURN
! END SUBROUTINE DROP_CUTFACE


! ----------------------------- DROP_CUTCELL --------------------------------------

! SUBROUTINE DROP_CUTCELL(NM,ICC,JCC)
!
! INTEGER, INTENT(IN) :: NM,ICC,JCC
!
! ! Local Variables
...
! I = M%CUT_CELL(ICC)%IJK(IAXIS); J = M%CUT_CELL(ICC)%IJK(JAXIS); K = M%CUT_CELL(ICC)%IJK(KAXIS)
!
! ! Check if JCC is the only cut-cell in CUT_CELL(ICC):
! IF (JCC==1 .AND. M%CUT_CELL(ICC)%NCELL==1) THEN
!    ! Set cut-cell to solid
!    M%CCVAR(I,J,K,IBM_CGSC) = IBM_SOLID
!    M%CCVAR(I,J,K,IBM_IDCC) = IBM_UNDEFINED
!    M%CUT_CELL(ICC)%NCELL = 0
!    ! Then drop INBOUNDARY cut-faces in I,J,K if there are any left:
!    IFC=M%CCVAR(I,J,K,IBM_IDCF)
!    IF (IFC>0) THEN
!       M%CUT_FACE(IFC)%STATUS = IBM_SOLID
!       M%CUT_FACE(IFC)%NFACE  = 0
!    ENDIF
!    M%CCVAR(I,J,K,IBM_IDCF) = IBM_UNDEFINED
!    RETURN
! ENDIF
!
! ! First count:
! ALLOCATE(IND(1:M%CUT_CELL(ICC)%NCELL)); IND=0
! CT=0
...
! DO JCC2=1,M%CUT_CELL(ICC)%NCELL
!    IF (JCC2==JCC) CYCLE
!    M%CUT_CELL(ICC)%CCELEM(:,IND(JCC2))   = M%CUT_CELL(ICC)%CCELEM(:,JCC2)
!    M%CUT_CELL(ICC)%IJK_LINK(:,IND(JCC2)) = M%CUT_CELL(ICC)%IJK_LINK(:,JCC2)
!    M%CUT_CELL(ICC)%LINK_LEV(IND(JCC2))   = M%CUT_CELL(ICC)%LINK_LEV(JCC2)
!    M%CUT_CELL(ICC)%VOLUME(IND(JCC2))     = M%CUT_CELL(ICC)%VOLUME(JCC2)
!    M%CUT_CELL(ICC)%XYZCEN(:,IND(JCC2))   = M%CUT_CELL(ICC)%XYZCEN(:,JCC2)
! ENDDO
!
! M%CUT_CELL(ICC)%NCELL = M%CUT_CELL(ICC)%NCELL - 1
!
! DEALLOCATE(IND)
!
! RETURN
! END SUBROUTINE DROP_CUTCELL
...
INTEGER :: ICC,JCC,ICC2,JCC2,JCC_LNK,I,J,K,I_LNK,J_LNK,K_LNK,IFC,IFC2,IFACE,IFACE2,IFACE3,IBOD,IWSEL,VAL_UNKZ,&
           LINK_ITER,INGH,JNGH,KNGH,X1AXIS,ILH,INRM(1:3),DUM,LNK_LEV,ULINK_COUNT,LU_UNLNK,LINK_LEV_UP
...
M%CCVAR(:,:,:,IBM_UNKZ) = IBM_UNDEFINED
DO ICC=1,M%N_CUTCELL_MESH+M%N_GCCUTCELL_MESH
   M%CUT_CELL(ICC)%UNKZ(:) = IBM_UNDEFINED
ENDDO
...
DO K=0,M%KBP1
   DO J=0,M%JBP1
      DO I=0,M%IBP1
         IF (  M%CCVAR(I,J,K,IBM_CGSC) /= IBM_CUTCFE ) CYCLE
         ...
         ! around Cartesian cell I,J,K of type IBM_CUTCFE:
         DO KNGH=K-1,K+1
            IF ( (KNGH < 1) .OR. (KNGH > M%KBAR) ) CYCLE
            DO JNGH=J-1,J+1
               IF ( (JNGH < 1) .OR. (JNGH > M%JBAR) ) CYCLE
               DO INGH=I-1,I+1
                  ! Either not GASPHASE or already counted:
                  IF ((M%CCVAR(INGH,JNGH,KNGH,IBM_CGSC)/=IBM_GASPHASE) .OR. (M%CCVAR(INGH,JNGH,KNGH,IBM_UNKZ)>0)) CYCLE
                  IF ( (INGH < 1) .OR. (INGH > M%IBAR) ) CYCLE
                  IF (M%SOLID(CELL_INDEX(INGH,JNGH,KNGH))) CYCLE
                  M%CCVAR(INGH,JNGH,KNGH,IBM_UNKZ) = 1
               ENDDO
            ENDDO
         ENDDO
...
! 1. Try linking them to adjacent regular cell with UNKZ > 0. Attempt going in surface normal direction first.
! 2. Try linking to adjacent cut-cell with UNKZ > 0. Attempt going in surface normal direction first.
! 3. If cut-cell could not be linked after N_LINK_ATTMP, block it.
LINK_ITER = 0; LINK_LEV_UP = 0
LINK_LOOP : DO ! Cut-cell linking loop for small cells. -> Algo defined by CCVOL_LINK.
   ...
   ICC_LOOP_1 : DO ICC=1,M%N_CUTCELL_MESH
      ...
      JCC_LOOP_1 : DO JCC=1,M%CUT_CELL(ICC)%NCELL
         IF ( M%CUT_CELL(ICC)%UNKZ(JCC) > 0 ) CYCLE
         CRTCELL_FLG = .FALSE.

         ! Find area averaged body surface normal:
         NRML(IAXIS:KAXIS) = 0._EB; AREA = 0._EB
         DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
            IFACE   = M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
            IF(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)/=IBM_FTYPE_CFINB) CYCLE
         ...
         AREA_IF_1 : IF (AREA > TWO_EPSILON_EB) THEN
            ...
            INRM(3) = MAXLOC(ABS(NRML(IAXIS:KAXIS)),MASK=MASK,DIM=1)
            AXIS_LOOP_1 : DO DUM=IAXIS,KAXIS
               X1AXIS=INRM(DUM)
               IFC_LOOP_1 : DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
                  IFACE   = M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
                  ILH     = 2*M%CUT_CELL(ICC)%FACE_LIST(2,IFACE) - 3 ! -1 for LOW_IND, 1 for HIGH_IND
                  IF( (X1AXIS /= M%CUT_CELL(ICC)%FACE_LIST(3,IFACE)) .OR. &
                      (M%CUT_CELL(ICC)%FACE_LIST(1,IFACE) /= IBM_FTYPE_RGGAS) .OR. &
                      (ILH /= INT(SIGN(1._EB,NRML(X1AXIS)))) ) CYCLE IFC_LOOP_1
                  SELECT CASE(X1AXIS)
                  CASE(IAXIS)
                     I_LNK = I+ILH; J_LNK = J; K_LNK = K
                     IF(M%CCVAR(I_LNK,J_LNK,K_LNK,IBM_UNKZ) > 0) THEN ! Regular Cartesian Cell
                        VAL_UNKZ = M%CCVAR(I_LNK,J_LNK,K_LNK,IBM_UNKZ)
                        CRTCELL_FLG = .TRUE.
                     ENDIF
                  CASE(JAXIS)
                  ...
                  CASE(KAXIS)
                     ...
                     ENDIF
                  END SELECT
                  IF ( CRTCELL_FLG ) EXIT AXIS_LOOP_1
               ENDDO IFC_LOOP_1
            ENDDO AXIS_LOOP_1
         ENDIF AREA_IF_1

         ! If not successful try any Regular Gasphase face.
         ! Small cells, get IBM_UNKZ from a large cell neighbor:
         IF (.NOT. CRTCELL_FLG) THEN
            IFC_LOOP_2 : DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
               IFACE   = M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
               ILH     = 2*M%CUT_CELL(ICC)%FACE_LIST(2,IFACE) - 3
               IF(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)/=IBM_FTYPE_RGGAS) CYCLE IFC_LOOP_2
               X1AXIS  = M%CUT_CELL(ICC)%FACE_LIST(3,IFACE)
               SELECT CASE(X1AXIS)
               CASE(IAXIS)
                  I_LNK = I+ILH; J_LNK = J; K_LNK = K
               ...
               CASE(KAXIS)
                  ...
                  ENDIF
               END SELECT
               IF ( CRTCELL_FLG ) EXIT IFC_LOOP_2
            ENDDO IFC_LOOP_2
         ENDIF
         IF (VAL_UNKZ>0) THEN
            M%CUT_CELL(ICC)%FACE_LIST(6,IFACE)      = INTEGER_ONE ! This face is shared with master.
            M%CUT_CELL(ICC)%UNKZ(JCC) = VAL_UNKZ     !(/    Cell Type,     I,     J,     K, JCC_LNK /)
            M%CUT_CELL(ICC)%IJK_LINK(1:KAXIS+2,JCC) = (/ IBM_GASPHASE, I_LNK, J_LNK, K_LNK,       0 /)
            M%CUT_CELL(ICC)%LINK_LEV(JCC)           = -1 ! One link hierarchy level below regular cells (at LNK_LEV=0).
         ENDIF
      ENDDO JCC_LOOP_1
   ENDDO ICC_LOOP_1
   ENDIF
   ...
   ICC_LOOP_2 : DO ICC=1,M%N_CUTCELL_MESH
      ...
      JCC_LOOP_2 : DO JCC=1,M%CUT_CELL(ICC)%NCELL
         IF ( M%CUT_CELL(ICC)%UNKZ(JCC) > 0 ) CYCLE
         VAL_UNKZ = IBM_UNDEFINED
         VAL_CVOL = -GEOMEPS
         ...
         AREA_IF_2 : IF (AREA > TWO_EPSILON_EB) THEN
            ...
            INRM(2) = MAXLOC(ABS(NRML(IAXIS:KAXIS)),MASK=MASK,DIM=1); MASK(INRM(2))=.FALSE.
            INRM(3) = MAXLOC(ABS(NRML(IAXIS:KAXIS)),MASK=MASK,DIM=1)
            AXIS_LOOP_2 : DO DUM=IAXIS,KAXIS
               X1AXIS=INRM(DUM)
               IFC_LOOP_3 : DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
                  IFACE   = M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
                  IF((M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_CFINB) .OR. &
                     (M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_SVERT)) CYCLE IFC_LOOP_3
                  ILH     = 2*M%CUT_CELL(ICC)%FACE_LIST(2,IFACE) - 3 ! -1 for LOW_IND, 1 for HIGH_IND
                  IF( (X1AXIS /= M%CUT_CELL(ICC)%FACE_LIST(3,IFACE)) .OR. &
                      (ILH /= INT(SIGN(1._EB,NRML(X1AXIS)))) ) CYCLE IFC_LOOP_3
                  SELECT CASE(X1AXIS)
                  CASE(IAXIS)
                     IF( (I+ILH < 1) .OR. (I+ILH > M%IBAR) ) CYCLE IFC_LOOP_3 ! Drop if outside the mesh.
                  CASE(JAXIS)
                     IF( (J+ILH < 1) .OR. (J+ILH > M%JBAR) ) CYCLE IFC_LOOP_3
                  CASE(KAXIS)
                     IF( (K+ILH < 1) .OR. (K+ILH > M%KBAR) ) CYCLE IFC_LOOP_3
                  END SELECT
                  SELECT CASE(M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)) ! 1. Check if a surrounding cell is a regular cell:
                  CASE(IBM_FTYPE_RGGAS) ! REGULAR GASPHASE
                     SELECT CASE(X1AXIS)
                     CASE(IAXIS)
                        IF(M%CCVAR(I+ILH,J,K,IBM_UNKZ) <= 0) THEN ! Cut - cell. Array IBM_RCFACE_VEL is used.
                           CALL GET_ICC2_JCC2(ICC,IFACE,I+ILH,J,K,ICC2,JCC2)
                           IF( ANY((/ ICC2, JCC2 /) == 0) .OR. M%CUT_CELL(ICC2)%UNKZ(JCC2)<1)CYCLE IFC_LOOP_3
                           IF(M%CUT_CELL(ICC2)%LINK_LEV(JCC2)/=LINK_LEV_UP)CYCLE IFC_LOOP_3
                           I_LNK = I+ILH; J_LNK = J; K_LNK = K; JCC_LNK = JCC2
                           VAL_UNKZ = M%CUT_CELL(ICC2)%UNKZ(JCC2); LNK_LEV=M%CUT_CELL(ICC2)%LINK_LEV(JCC2);
                           EXIT AXIS_LOOP_2
                        ENDIF
                     CASE(JAXIS)
                        IF(M%CCVAR(I,J+ILH,K,IBM_UNKZ) <= 0) THEN ! Cut - cell. Array IBM_RCFACE_VEL is used.
                        ...
                        ENDIF
                     CASE(KAXIS)
                        IF(M%CCVAR(I,J,K+ILH,IBM_UNKZ) <= 0) THEN ! Cut - cell. Array IBM_RCFACE_VEL is used.
                  ...
                  CASE(IBM_FTYPE_CFGAS) ! 2. Check for large surrounding cut-cells:
                     IFC2    = M%CUT_CELL(ICC)%FACE_LIST(4,IFACE)
                     IFACE2  = M%CUT_CELL(ICC)%FACE_LIST(5,IFACE)
                     ICC2    = M%CUT_FACE(IFC2)%CELL_LIST(2,M%CUT_CELL(ICC)%FACE_LIST(2,IFACE),IFACE2)
                     JCC2    = M%CUT_FACE(IFC2)%CELL_LIST(3,M%CUT_CELL(ICC)%FACE_LIST(2,IFACE),IFACE2)
                     IF (M%CUT_CELL(ICC2)%UNKZ(JCC2)<1) CYCLE IFC_LOOP_3
                     IF (M%CUT_CELL(ICC2)%LINK_LEV(JCC2)/=LINK_LEV_UP)CYCLE IFC_LOOP_3
                     I_LNK    = M%CUT_CELL(ICC2)%IJK(IAXIS); J_LNK   = M%CUT_CELL(ICC2)%IJK(JAXIS);
                     K_LNK    = M%CUT_CELL(ICC2)%IJK(KAXIS); JCC_LNK = JCC2
                     VAL_UNKZ = M%CUT_CELL(ICC2)%UNKZ(JCC2); LNK_LEV=M%CUT_CELL(ICC2)%LINK_LEV(JCC2); EXIT AXIS_LOOP_2
                  END SELECT
               ENDDO IFC_LOOP_3
            ENDDO AXIS_LOOP_2
            IF (VAL_UNKZ > 0) THEN
               M%CUT_CELL(ICC)%FACE_LIST(6,IFACE)      = INTEGER_ONE ! This face is shared with master.
               M%CUT_CELL(ICC)%UNKZ(JCC)               = VAL_UNKZ
               M%CUT_CELL(ICC)%IJK_LINK(1:KAXIS+2,JCC) = (/ IBM_CUTCFE, I_LNK, J_LNK, K_LNK, JCC_LNK /)
               M%CUT_CELL(ICC)%LINK_LEV(JCC)           = LNK_LEV-1 ! One link hierarchy level below master cell.
               CYCLE JCC_LOOP_2
            ENDIF
         ENDIF AREA_IF_2

         ! Small cells, get IBM_UNKZ from a large cell neighbor:
         IFACE3 = IBM_UNDEFINED
         IFC_LOOP_4 : DO IFC=1,M%CUT_CELL(ICC)%CCELEM(1,JCC)
            IFACE   = M%CUT_CELL(ICC)%CCELEM(IFC+1,JCC)
            IF((M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_CFINB) .OR. &
               (M%CUT_CELL(ICC)%FACE_LIST(1,IFACE)==IBM_FTYPE_SVERT)) CYCLE IFC_LOOP_4
            ILH     = 2*M%CUT_CELL(ICC)%FACE_LIST(2,IFACE) - 3 ! -1 for LOW_IND, 1 for HIGH_IND
            ...
            SELECT CASE(X1AXIS)
            CASE(IAXIS)
               IF( (I+ILH < 1) .OR. (I+ILH > M%IBAR) ) CYCLE IFC_LOOP_4
            CASE(JAXIS)
               IF( (J+ILH < 1) .OR. (J+ILH > M%JBAR) ) CYCLE IFC_LOOP_4
            CASE(KAXIS)
               IF( (K+ILH < 1) .OR. (K+ILH > M%KBAR) ) CYCLE IFC_LOOP_4
            END SELECT
            ...
            CASE(IBM_FTYPE_RGGAS) ! REGULAR GASPHASE
               ...
               CASE(IAXIS)
                  IF(M%CCVAR(I+ILH,J,K,IBM_UNKZ) <= 0) THEN ! Cut - cell. Array IBM_RCFACE_VEL is used.
                     ...
                     I_LNK = I+ILH; J_LNK = J; K_LNK = K; JCC_LNK = JCC2
                     VAL_CVOL = M%CUT_CELL(ICC2)%VOLUME(JCC2); VAL_UNKZ = M%CUT_CELL(ICC2)%UNKZ(JCC2)
                     LNK_LEV  = M%CUT_CELL(ICC2)%LINK_LEV(JCC2); IFACE3 = IFACE
                  ENDIF
               CASE(JAXIS)
                  IF(M%CCVAR(I,J+ILH,K,IBM_UNKZ) <= 0) THEN ! Cut - cell. Array IBM_RCFACE_VEL is used.
                     CALL GET_ICC2_JCC2(ICC,IFACE,I,J+ILH,K,ICC2,JCC2)
            ...
            CASE(IBM_FTYPE_CFGAS) ! 2. Check for large surrounding cut-cells:
               ...
               IF(M%CUT_CELL(ICC2)%VOLUME(JCC2)<VAL_CVOL .OR. M%CUT_CELL(ICC2)%UNKZ(JCC2)<1) CYCLE IFC_LOOP_4
               IF(M%CUT_CELL(ICC2)%LINK_LEV(JCC2)/=LINK_LEV_UP)CYCLE IFC_LOOP_4
               I_LNK=M%CUT_CELL(ICC2)%IJK(IAXIS); J_LNK=M%CUT_CELL(ICC2)%IJK(JAXIS); K_LNK=M%CUT_CELL(ICC2)%IJK(KAXIS)
               JCC_LNK  = JCC2; VAL_CVOL = M%CUT_CELL(ICC2)%VOLUME(JCC2);
               VAL_UNKZ = M%CUT_CELL(ICC2)%UNKZ(JCC2); LNK_LEV  = M%CUT_CELL(ICC2)%LINK_LEV(JCC2); IFACE3 = IFACE
            END SELECT
         ENDDO IFC_LOOP_4
         ...
         ! neighbors:
         IF (VAL_UNKZ > 0) THEN
            M%CUT_CELL(ICC)%FACE_LIST(6,IFACE3)     = INTEGER_ONE ! This face is shared with master.
            M%CUT_CELL(ICC)%UNKZ(JCC)               = VAL_UNKZ
            M%CUT_CELL(ICC)%IJK_LINK(1:KAXIS+2,JCC) = (/ IBM_CUTCFE, I_LNK, J_LNK, K_LNK, JCC_LNK /)
            M%CUT_CELL(ICC)%LINK_LEV(JCC)           = LNK_LEV-1 ! One link hierarchy level below master cell.
         ELSE
   ...
   IF(.NOT. CC_UNSTRUCTURED_PROJECTION) THEN
      DO ICC=1,M%N_CUTCELL_MESH
         ...
         ! with UNKZ > 0:
         DO JCC=1,M%CUT_CELL(ICC)%NCELL
            IF ( M%CUT_CELL(ICC)%UNKZ(JCC) > 0 ) EXIT
         ENDDO
         JCC_LNK = JCC
         IF (JCC_LNK <= M%CUT_CELL(ICC)%NCELL) THEN
            DO JCC=1,M%CUT_CELL(ICC)%NCELL
               IF(JCC==JCC_LNK) CYCLE
               M%CUT_CELL(ICC)%UNKZ(JCC)               = M%CUT_CELL(ICC)%UNKZ(JCC_LNK)
               M%CUT_CELL(ICC)%IJK_LINK(1:KAXIS+2,JCC) = (/ IBM_CUTCFE, I, J, K, JCC_LNK /)
               M%CUT_CELL(ICC)%LINK_LEV(JCC)           = M%CUT_CELL(ICC)%LINK_LEV(JCC_LNK) - 1
            ENDDO
         ENDIF
      ENDDO
   ENDIF
   ...
   LINK_LEV_UP = LINK_LEV_UP - 1

   LINK_ITER = LINK_ITER + 1
   BLOCK_CELL_IF : IF (LINK_ITER > N_LINK_ATTMP) THEN
       ...
       IF (GET_CUTCELLS_VERBOSE) THEN
          ...
          DO ICC=1,M%N_CUTCELL_MESH
             ...
             DO JCC=1,M%CUT_CELL(ICC)%NCELL
                ...
                ULINK_COUNT,', I,J,K,ICC,JCC=',I,J,K,ICC,JCC,', X,Y,Z,CCVOL,CCVOL_CRT=',M%X(I),M%Y(J),M%Z(K), &
                M%CUT_CELL(ICC)%VOLUME(JCC),M%DX(I)*M%DY(J)*M%DZ(K)
             ENDDO
          ENDDO
          CLOSE(LU_UNLNK)
       ENDIF

       ! Unlinked cells get blocked, inboundary cut-faces are dropped, shared gas cut-faces are made inboundary faces
       ! for neighbors. If no cut-cells left in location I,J,K => CCVAR(I,J,K,IBM_CGSC) is set to IBM_SOLID.
       DO ICC=1,M%N_CUTCELL_MESH
          DO JCC=1,M%CUT_CELL(ICC)%NCELL
             IF ( M%CUT_CELL(ICC)%UNKZ(JCC) > 0 ) CYCLE
             M%CUT_CELL(ICC)%IJK_LINK(1,JCC) = IBM_SOLID ! Flag for Blocking after main mesh loop in SET_CUTCELLS_3D
          ENDDO
       ENDDO
       ...
       DO ICC=1,M%N_CUTCELL_MESH
          ...
          IF (M%SOLID(M%CELL_INDEX(I,J,K))) CYCLE
          DO JCC=1,M%CUT_CELL(ICC)%NCELL
             IF ( M%CUT_CELL(ICC)%UNKZ(JCC) > 0 .OR. M%CUT_CELL(ICC)%IJK_LINK(1,JCC)==IBM_SOLID) CYCLE
             ULINK_COUNT = ULINK_COUNT + 1
          ENDDO
       ENDDO
       ...
       IF (GET_CUTCELLS_VERBOSE) THEN
          ...
          OPEN(UNIT=LU_UNLNK,FILE=TRIM(UNLINKED_FILE),STATUS='OLD',POSITION='APPEND')
          WRITE(LU_UNLNK,*) ' '
          WRITE(LU_UNLNK,*) 'STATUS AFTER BLOCKING SMALL UNLINKED CUT-CELLS:'
          WRITE(LU_UNLNK,'(A,I4,A,I4)') ' Mesh NM=',NM,', number of unlinked cells after blocking=',ULINK_COUNT
...
! Finally compute M%FINEST_LINK_LEV:
DO ICC=1,M%N_CUTCELL_MESH
   DO JCC=1,M%CUT_CELL(ICC)%NCELL
      IF(M%CUT_CELL(ICC)%LINK_LEV(JCC) < M%FINEST_LINK_LEV) M%FINEST_LINK_LEV = M%CUT_CELL(ICC)%LINK_LEV(JCC)
   ENDDO
ENDDO
...
INTEGER :: IFC, IFACE2
ICC2=M%CCVAR(INXT,JNXT,KNXT,IBM_IDCC); IF (ICC2<=0) RETURN
DO JCC2=1,M%CUT_CELL(ICC2)%NCELL
   ! Loop faces and test:
   DO IFC=1,M%CUT_CELL(ICC2)%CCELEM(1,JCC2)
      IFACE2 = M%CUT_CELL(ICC2)%CCELEM(IFC+1,JCC2)
      ! If face type in face_list is not IBM_FTYPE_RGGAS, drop:
      IF(M%CUT_CELL(ICC2)%FACE_LIST(1,IFACE2) /= IBM_FTYPE_RGGAS) CYCLE
      ! Does X1AXIS match and LOWHIGH are different?
      IF(    M%CUT_CELL(ICC2)%FACE_LIST(3,IFACE2) /= M%CUT_CELL(ICC)%FACE_LIST(3,IFACE)) CYCLE ! X1AXIS is different.
      IF(ABS(M%CUT_CELL(ICC2)%FACE_LIST(2,IFACE2)  - M%CUT_CELL(ICC)%FACE_LIST(2,IFACE)) < 1) CYCLE ! Same LOWHIGH.
...
MESH_LOOP : DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   EXTERNAL_WALL_LOOP : DO IW=1,N_EXTERNAL_WALL_CELLS
      ...
      ELSE
         ! Here we might need to add other EXT wall cell types.
         IF (.NOT.(WC%BOUNDARY_TYPE==OPEN_BOUNDARY .OR. WC%BOUNDARY_TYPE==SOLID_BOUNDARY)) CYCLE EXTERNAL_WALL_LOOP
      ENDIF
      II  = BC%II
      JJ  = BC%JJ
      KK  = BC%KK
...
! Local Variables:
INTEGER :: NVERTFACE
REAL(EB), ALLOCATABLE, DIMENSION(:,:)     :: XYZVERT, XYZCEN, REAL2D
REAL(EB), ALLOCATABLE, DIMENSION(:)       :: AREA, REAL1D
INTEGER,  ALLOCATABLE, DIMENSION(:)       :: INT1D
INTEGER,  ALLOCATABLE, DIMENSION(:,:)     :: CFELEM, INT2D ! Cut-faces connectivities.
INTEGER,  ALLOCATABLE, DIMENSION(:,:,:)   :: INT3D
INTEGER,  ALLOCATABLE, DIMENSION(:,:,:,:) :: INT4D
...
IF(SIZE(MESHES(NM)%CUT_FACE(ICF)%XYZVERT,DIM=2) < NVERT+NSVERT) THEN
   ! Allocate and initialize NVERT related fields:
   ALLOCATE(XYZVERT(IAXIS:KAXIS,1:NVERT+NSVERT)); XYZVERT = 0._EB
   XYZVERT(IAXIS:KAXIS,1:NVERT)=MESHES(NM)%CUT_FACE(ICF)%XYZVERT(IAXIS:KAXIS,1:NVERT)
   CALL MOVE_ALLOC(FROM=XYZVERT,TO=MESHES(NM)%CUT_FACE(ICF)%XYZVERT)
ENDIF

IF(SIZE(MESHES(NM)%CUT_FACE(ICF)%AREA,DIM=1)<NFACE+NSFACE) THEN
   ! Allocate and initialize NFACE, NVERTFACE related fields:
   ALLOCATE(CFELEM(1:NVERTFACE_NEW,1:NFACE+NSFACE)); CFELEM = IBM_UNDEFINED
   CFELEM(1:NVERTFACE,1:NFACE)=MESHES(NM)%CUT_FACE(ICF)%CFELEM(1:NVERTFACE,1:NFACE)
   CALL MOVE_ALLOC(FROM=CFELEM,TO=MESHES(NM)%CUT_FACE(ICF)%CFELEM)

   ALLOCATE(AREA(1:NFACE+NSFACE),XYZCEN(IAXIS:KAXIS,1:NFACE+NSFACE)); AREA = 0._EB; XYZCEN = 0._EB
   ...
   CALL MOVE_ALLOC(FROM=AREA,TO=MESHES(NM)%CUT_FACE(ICF)%AREA)
   CALL MOVE_ALLOC(FROM=XYZCEN,TO=MESHES(NM)%CUT_FACE(ICF)%XYZCEN)

   ! This is only used for reallocation of INB faces when blocking cut-cells:
   IF (MESHES(NM)%CUT_FACE(ICF)%STATUS == IBM_INBOUNDARY) THEN

      ALLOCATE(SHARED(1:NFACE+NSFACE)); SHARED = .FALSE.
      SHARED(1:NFACE) = MESHES(NM)%CUT_FACE(ICF)%SHARED(1:NFACE)
      CALL MOVE_ALLOC(FROM=SHARED,TO=MESHES(NM)%CUT_FACE(ICF)%SHARED)

      ALLOCATE(INT2D(1:2,1:NFACE+NSFACE)); INT2D=IBM_UNDEFINED
      INT2D(1:2,1:NFACE)=MESHES(NM)%CUT_FACE(ICF)%BODTRI(1:2,1:NFACE)
      CALL MOVE_ALLOC(FROM=INT2D,TO=MESHES(NM)%CUT_FACE(ICF)%BODTRI)

      ALLOCATE(INT2D(LOW_IND:HIGH_IND,1:NFACE+NSFACE)); INT2D = IBM_UNDEFINED
      INT2D(LOW_IND:HIGH_IND,1:NFACE) = MESHES(NM)%CUT_FACE(ICF)%UNKH(LOW_IND:HIGH_IND,1:NFACE)
      CALL MOVE_ALLOC(FROM=INT2D,TO=MESHES(NM)%CUT_FACE(ICF)%UNKH)
      ALLOCATE(INT2D(LOW_IND:HIGH_IND,1:NFACE+NSFACE)); INT2D = IBM_UNDEFINED
      INT2D(LOW_IND:HIGH_IND,1:NFACE) = MESHES(NM)%CUT_FACE(ICF)%UNKZ(LOW_IND:HIGH_IND,1:NFACE)
      CALL MOVE_ALLOC(FROM=INT2D,TO=MESHES(NM)%CUT_FACE(ICF)%UNKZ)

      ALLOCATE(REAL2D(IAXIS:KAXIS,1:NFACE+NSFACE)); REAL2D = 0._EB
      REAL2D(IAXIS:KAXIS,1:NFACE) = MESHES(NM)%CUT_FACE(ICF)%XCENLOW(IAXIS:KAXIS,1:NFACE)
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%XCENLOW)
      ALLOCATE(REAL2D(IAXIS:KAXIS,1:NFACE+NSFACE)); REAL2D = 0._EB
      REAL2D(IAXIS:KAXIS,1:NFACE) = MESHES(NM)%CUT_FACE(ICF)%XCENHIGH(IAXIS:KAXIS,1:NFACE)
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%XCENHIGH)

      ALLOCATE(REAL2D(LOW_IND:HIGH_IND,1:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%RHO)

      ALLOCATE(REAL2D(1:MAX_SPECIES,1:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%RHO_D)
      ALLOCATE(REAL2D(1:MAX_SPECIES,1:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%RHO_D_DZDN)
      ALLOCATE(REAL2D(1:MAX_SPECIES,1:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%H_RHO_D_DZDN)
      ALLOCATE(REAL2D(1:MAX_SPECIES,1:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%ZZ_FACE)

      ALLOCATE(REAL1D(1:NFACE+NSFACE)); REAL1D=0._EB; CALL MOVE_ALLOC(FROM=REAL1D,TO=MESHES(NM)%CUT_FACE(ICF)%TMP_FACE)
      ALLOCATE(REAL1D(1:NFACE+NSFACE)); REAL1D=0._EB; CALL MOVE_ALLOC(FROM=REAL1D,TO=MESHES(NM)%CUT_FACE(ICF)%VEL)
      ALLOCATE(REAL1D(1:NFACE+NSFACE)); REAL1D=0._EB; CALL MOVE_ALLOC(FROM=REAL1D,TO=MESHES(NM)%CUT_FACE(ICF)%VELS)
      ALLOCATE(REAL1D(1:NFACE+NSFACE)); REAL1D=0._EB; CALL MOVE_ALLOC(FROM=REAL1D,TO=MESHES(NM)%CUT_FACE(ICF)%FN)
      ALLOCATE(REAL1D(1:NFACE+NSFACE)); REAL1D=0._EB; CALL MOVE_ALLOC(FROM=REAL1D,TO=MESHES(NM)%CUT_FACE(ICF)%FN_B)
      ALLOCATE(REAL1D(1:NFACE+NSFACE)); REAL1D=0._EB; CALL MOVE_ALLOC(FROM=REAL1D,TO=MESHES(NM)%CUT_FACE(ICF)%VELINT)

      ALLOCATE(INT3D(1:2,1:2,1:NFACE+NSFACE)); INT3D = IBM_UNDEFINED
      CALL MOVE_ALLOC(FROM=INT3D,TO=MESHES(NM)%CUT_FACE(ICF)%JDH)
      ALLOCATE(INT3D(MAX_DIM+1,LOW_IND:HIGH_IND,1:NFACE+NSFACE)); INT3D = IBM_UNDEFINED
      INT3D(1:MAX_DIM+1,LOW_IND:HIGH_IND,1:NFACE) = &
      MESHES(NM)%CUT_FACE(ICF)%CELL_LIST(1:MAX_DIM+1,LOW_IND:HIGH_IND,1:NFACE)
      CALL MOVE_ALLOC(FROM=INT3D,TO=MESHES(NM)%CUT_FACE(ICF)%CELL_LIST)

      ALLOCATE(INT2D(IAXIS:KAXIS,(NFACE+NSFACE+1)*DELTA_INT)); INT2D = IBM_UNDEFINED
      CALL MOVE_ALLOC(FROM=INT2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_IJK)

      ALLOCATE(REAL1D((NFACE+NSFACE+1)*DELTA_INT)); REAL1D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL1D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_COEF)

      ALLOCATE(REAL2D(IAXIS:KAXIS,0:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_XYZBF)

      ALLOCATE(REAL2D(IAXIS:KAXIS,0:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_NOUT)

      ALLOCATE(INT2D(1:3,0:NFACE+NSFACE)); INT2D = IBM_UNDEFINED
      CALL MOVE_ALLOC(FROM=INT2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_INBFC)

      ALLOCATE(INT4D(LOW_IND:HIGH_IND,0:KAXIS,1:INT_N_EXT_PTS,0:NFACE+NSFACE)); INT4D = 0
      CALL MOVE_ALLOC(FROM=INT4D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_NPE)

      ALLOCATE(REAL2D(0:INT_N_EXT_PTS,0:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_XN)
      ALLOCATE(REAL2D(0:INT_N_EXT_PTS,0:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_CN)
      ALLOCATE(REAL2D(1:N_INT_FVARS,(NFACE+NSFACE+1)*DELTA_INT)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_FVARS)

      ALLOCATE(INT2D(LOW_IND:HIGH_IND,(NFACE+NSFACE+1)*DELTA_INT)); INT2D = IBM_UNDEFINED
      CALL MOVE_ALLOC(FROM=INT2D,TO=MESHES(NM)%CUT_FACE(ICF)%INT_NOMIND)

      ALLOCATE(REAL2D(-1:0,1:NFACE+NSFACE)); REAL2D = 0._EB
      CALL MOVE_ALLOC(FROM=REAL2D,TO=MESHES(NM)%CUT_FACE(ICF)%RHOPVN)

      ALLOCATE(INT1D(1:NFACE+NSFACE)); INT1D=IBM_UNDEFINED
      CALL MOVE_ALLOC(FROM=INT1D,TO=MESHES(NM)%CUT_FACE(ICF)%CFACE_INDEX)
      ALLOCATE(INT1D(1:NFACE+NSFACE)); INT1D=IBM_UNDEFINED
      INT1D(1:NFACE) = MESHES(NM)%CUT_FACE(ICF)%SURF_INDEX(1:NFACE)
      CALL MOVE_ALLOC(FROM=INT1D,TO=MESHES(NM)%CUT_FACE(ICF)%SURF_INDEX)
...
IF(ICF > SIZE_CUT_FACE) THEN
   ...
   DO ICF1=1,ICF-1
      CALL CUT_FACE_MOVE(MESHES(NM)%CUT_FACE(ICF1),CUT_FACE_AUX(ICF1))
   ENDDO
   CALL MOVE_ALLOC(FROM=CUT_FACE_AUX,TO=MESHES(NM)%CUT_FACE)
ENDIF
...
SUBROUTINE CUT_FACE_MOVE(CUT_FACE_FROM,CUT_FACE_TO)

TYPE(IBM_CUTFACE_TYPE), INTENT(INOUT) :: CUT_FACE_FROM, CUT_FACE_TO

CUT_FACE_TO%IWC       = CUT_FACE_FROM%IWC
CUT_FACE_TO%PRES_ZONE = CUT_FACE_FROM%PRES_ZONE
CUT_FACE_TO%NVERT     = CUT_FACE_FROM%NVERT
CUT_FACE_TO%NSVERT    = CUT_FACE_FROM%NSVERT
CUT_FACE_TO%NFACE     = CUT_FACE_FROM%NFACE
CUT_FACE_TO%NSFACE    = CUT_FACE_FROM%NSFACE
CUT_FACE_TO%STATUS    = CUT_FACE_FROM%STATUS
CUT_FACE_TO%IJK       = CUT_FACE_FROM%IJK

CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%XYZVERT,          TO=CUT_FACE_TO%XYZVERT)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%CFELEM,           TO=CUT_FACE_TO%CFELEM)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%AREA,             TO=CUT_FACE_TO%AREA)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%XYZCEN,           TO=CUT_FACE_TO%XYZCEN)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%SHARED,           TO=CUT_FACE_TO%SHARED)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INXAREA,          TO=CUT_FACE_TO%INXAREA)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INXSQAREA,        TO=CUT_FACE_TO%INXSQAREA)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%JNYSQAREA,        TO=CUT_FACE_TO%JNYSQAREA)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%KNZSQAREA,        TO=CUT_FACE_TO%KNZSQAREA)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%BODTRI,           TO=CUT_FACE_TO%BODTRI)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%UNKH,             TO=CUT_FACE_TO%UNKH)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%UNKZ,             TO=CUT_FACE_TO%UNKZ)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%XCENLOW,          TO=CUT_FACE_TO%XCENLOW)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%XCENHIGH,         TO=CUT_FACE_TO%XCENHIGH)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%RHO,              TO=CUT_FACE_TO%RHO)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%ZZ_FACE,          TO=CUT_FACE_TO%ZZ_FACE)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%RHO_D,            TO=CUT_FACE_TO%RHO_D)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%TMP_FACE,         TO=CUT_FACE_TO%TMP_FACE)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%RHO_D_DZDN,       TO=CUT_FACE_TO%RHO_D_DZDN)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%H_RHO_D_DZDN,     TO=CUT_FACE_TO%H_RHO_D_DZDN)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%VEL,              TO=CUT_FACE_TO%VEL)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%VELS,             TO=CUT_FACE_TO%VELS)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%FN,               TO=CUT_FACE_TO%FN)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%FN_B,             TO=CUT_FACE_TO%FN_B)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%VELINT,           TO=CUT_FACE_TO%VELINT)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%JDH,              TO=CUT_FACE_TO%JDH)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%CELL_LIST,        TO=CUT_FACE_TO%CELL_LIST)

CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_IJK,          TO=CUT_FACE_TO%INT_IJK)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_COEF,         TO=CUT_FACE_TO%INT_COEF)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_DCOEF,        TO=CUT_FACE_TO%INT_DCOEF)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_XYZBF,        TO=CUT_FACE_TO%INT_XYZBF)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_NOUT,         TO=CUT_FACE_TO%INT_NOUT)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_INBFC,        TO=CUT_FACE_TO%INT_INBFC)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_NPE,          TO=CUT_FACE_TO%INT_NPE)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_XN,           TO=CUT_FACE_TO%INT_XN)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_CN,           TO=CUT_FACE_TO%INT_CN)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_FVARS,        TO=CUT_FACE_TO%INT_FVARS)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_NOMIND,       TO=CUT_FACE_TO%INT_NOMIND)

CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%INT_CVARS,        TO=CUT_FACE_TO%INT_CVARS)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%RHOPVN,           TO=CUT_FACE_TO%RHOPVN)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%CFACE_INDEX,      TO=CUT_FACE_TO%CFACE_INDEX)
CALL MOVE_ALLOC(FROM=CUT_FACE_FROM%SURF_INDEX,       TO=CUT_FACE_TO%SURF_INDEX)

CUT_FACE_TO%NOMICF = CUT_FACE_FROM%NOMICF

RETURN
END SUBROUTINE CUT_FACE_MOVE
...
ENDIF
IF(ALLOCATED(MESHES(NM)%CUT_FACE(ICF)%SHARED))  DEALLOCATE(MESHES(NM)%CUT_FACE(ICF)%SHARED)
...
DEALLOCATE(MESHES(NM)%CUT_FACE(ICF)%VEL, MESHES(NM)%CUT_FACE(ICF)%VELS, &
           MESHES(NM)%CUT_FACE(ICF)%FN,  MESHES(NM)%CUT_FACE(ICF)%FN_B, MESHES(NM)%CUT_FACE(ICF)%VELINT)

IF(ALLOCATED(MESHES(NM)%CUT_FACE(ICF)%JDH)) DEALLOCATE(MESHES(NM)%CUT_FACE(ICF)%JDH)
...
ALLOCATE(MESHES(NM)%CUT_FACE(ICF)%SHARED(1:NFACE)); MESHES(NM)%CUT_FACE(ICF)%SHARED = .FALSE.
...
ALLOCATE(MESHES(NM)%CUT_FACE(ICF)%VEL(1:NFACE), MESHES(NM)%CUT_FACE(ICF)%VELS(1:NFACE), &
         MESHES(NM)%CUT_FACE(ICF)%FN(1:NFACE),  MESHES(NM)%CUT_FACE(ICF)%FN_B(1:NFACE), &
         MESHES(NM)%CUT_FACE(ICF)%VELINT(1:NFACE))
MESHES(NM)%CUT_FACE(ICF)%VEL    = 0._EB; MESHES(NM)%CUT_FACE(ICF)%VELS   = 0._EB
MESHES(NM)%CUT_FACE(ICF)%FN     = 0._EB
MESHES(NM)%CUT_FACE(ICF)%FN_B   = 0._EB; MESHES(NM)%CUT_FACE(ICF)%VELINT = 0._EB

ALLOCATE(MESHES(NM)%CUT_FACE(ICF)%JDH(1:2,1:2,1:NFACE)); MESHES(NM)%CUT_FACE(ICF)%JDH = IBM_UNDEFINED
         ...
         MYAXIS_LOOP : DO MYAXIS=IAXIS,KAXIS
            ...
            DO SIDE=LOW_IND,HIGH_IND
               ...
               IF ( FSID_XYZ(SIDE,MYAXIS) == IBM_GASPHASE ) THEN
                  ...
                  FACE_LIST(1:IBM_NPARAM_CCFACE,NFACE_CELL) = (/ IBM_FTYPE_RGGAS, SIDE, MYAXIS, 0, 0, IBM_UNDEFINED /)
                  ! IBM_FTYPE_RGGAS=0, regular face.
               ...
               ELSEIF (FSID_XYZ(SIDE,MYAXIS) == IBM_CUTCFE ) THEN
                  ...
                  DO ICF=1,MESHES(NM)%CUT_FACE(CEI)%NFACE
                     ...
                     FACE_LIST(1:IBM_NPARAM_CCFACE,NFACE_CELL) = (/ IBM_FTYPE_CFGAS,SIDE,MYAXIS,CEI,ICF,IBM_UNDEFINED /)
                     ! IBM_FTYPE_CFGAS=1
            ...
            IDCF_COND : IF (IDCF > 0) THEN
                ...
                AXIS_LOOP : DO MYAXIS=IAXIS,KAXIS
                    ...
                    SIDE_LOOP : DO SIDE=LOW_IND,HIGH_IND
                       ...
                       NFACE_CELL = N_GAS_CFACES + NIBFACE
                       FACE_LIST(1:IBM_NPARAM_CCFACE,NFACE_CELL) = (/ IBM_FTYPE_CFINB,0,0,IDCF, NIBFACE,IBM_UNDEFINED /)
                    ENDDO SIDE_LOOP
                ENDDO AXIS_LOOP
...
IF (ICC > SIZE_CUT_CELL) THEN
   ...
   DO ICC1=1,ICC-1
      CALL CUT_CELL_MOVE(MESHES(NM)%CUT_CELL(ICC1),CUT_CELL_AUX(ICC1))
   ENDDO
...
SUBROUTINE CUT_CELL_MOVE(CUT_CELL_FROM,CUT_CELL_TO)

TYPE(IBM_CUTCELL_TYPE), INTENT(INOUT) :: CUT_CELL_FROM,CUT_CELL_TO

CUT_CELL_TO%NCELL      = CUT_CELL_FROM%NCELL
CUT_CELL_TO%NFACE_CELL = CUT_CELL_FROM%NFACE_CELL
CUT_CELL_TO%IJK        = CUT_CELL_FROM%IJK

CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%CCELEM           ,TO=CUT_CELL_TO%CCELEM)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%FACE_LIST        ,TO=CUT_CELL_TO%FACE_LIST)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%IJK_LINK         ,TO=CUT_CELL_TO%IJK_LINK)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%LINK_LEV         ,TO=CUT_CELL_TO%LINK_LEV)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%VOLUME           ,TO=CUT_CELL_TO%VOLUME)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%XYZCEN           ,TO=CUT_CELL_TO%XYZCEN)

CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%RHO              ,TO=CUT_CELL_TO%RHO)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%RHOS             ,TO=CUT_CELL_TO%RHOS)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%RSUM             ,TO=CUT_CELL_TO%RSUM)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%TMP              ,TO=CUT_CELL_TO%TMP)
...
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%CHI_R            ,TO=CUT_CELL_TO%CHI_R)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%MIX_TIME         ,TO=CUT_CELL_TO%MIX_TIME)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%Q_REAC           ,TO=CUT_CELL_TO%Q_REAC)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%REAC_SOURCE_TERM ,TO=CUT_CELL_TO%REAC_SOURCE_TERM)
...
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%KRES             ,TO=CUT_CELL_TO%KRES)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%H                ,TO=CUT_CELL_TO%H)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%HS               ,TO=CUT_CELL_TO%HS)

CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%RTRM             ,TO=CUT_CELL_TO%RTRM)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%R_H_G            ,TO=CUT_CELL_TO%R_H_G)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%RHO_0            ,TO=CUT_CELL_TO%RHO_0)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%WVEL             ,TO=CUT_CELL_TO%WVEL)

CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_IJK         ,TO=CUT_CELL_TO%INT_IJK   )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_COEF        ,TO=CUT_CELL_TO%INT_COEF  )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_XYZBF       ,TO=CUT_CELL_TO%INT_XYZBF )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_NOUT        ,TO=CUT_CELL_TO%INT_NOUT  )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_INBFC       ,TO=CUT_CELL_TO%INT_INBFC )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_NPE         ,TO=CUT_CELL_TO%INT_NPE   )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_XN          ,TO=CUT_CELL_TO%INT_XN    )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_CN          ,TO=CUT_CELL_TO%INT_CN    )
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_CCVARS      ,TO=CUT_CELL_TO%INT_CCVARS)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%INT_NOMIND      ,TO=CUT_CELL_TO%INT_NOMIND)
...
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%USE_CC_VOL           ,TO=CUT_CELL_TO%USE_CC_VOL)
CALL MOVE_ALLOC(FROM=CUT_CELL_FROM%NOMICC               ,TO=CUT_CELL_TO%NOMICC)

RETURN
END SUBROUTINE CUT_CELL_MOVE
...
DEALLOCATE(MESHES(NM)%CUT_CELL(ICC)%FACE_LIST,MESHES(NM)%CUT_CELL(ICC)%IJK_LINK,MESHES(NM)%CUT_CELL(ICC)%LINK_LEV)
DEALLOCATE(MESHES(NM)%CUT_CELL(ICC)%VOLUME,   MESHES(NM)%CUT_CELL(ICC)%RHO,      &
           ...
           MESHES(NM)%CUT_CELL(ICC)%MIX_TIME,                                    &
           MESHES(NM)%CUT_CELL(ICC)%KRES,     MESHES(NM)%CUT_CELL(ICC)%H,        &
...
ALLOCATE(MESHES(NM)%CUT_CELL(ICC)%IJK_LINK(IAXIS:KAXIS+2,1:NCELL),MESHES(NM)%CUT_CELL(ICC)%LINK_LEV(1:NCELL))
MESHES(NM)%CUT_CELL(ICC)%CCELEM    = IBM_UNDEFINED
MESHES(NM)%CUT_CELL(ICC)%FACE_LIST = IBM_UNDEFINED
MESHES(NM)%CUT_CELL(ICC)%IJK_LINK  = IBM_UNDEFINED
...
ALLOCATE(MESHES(NM)%CUT_CELL(ICC)%VOLUME(1:NCELL),   MESHES(NM)%CUT_CELL(ICC)%RHO(1:NCELL),       &
         ...
         MESHES(NM)%CUT_CELL(ICC)%KRES(1:NCELL))
...
MESHES(NM)%CUT_CELL(ICC)%KRES     = 0._EB
...
LOGICAL :: SNAP_TO_GRID=.FALSE., DONE, EXPAND_CUTCELLS
...
NAMELIST /GEOM/ AUTO_TEXTURE,BNDF_GEOM,BINARY_FILE,COLOR,CYLINDER_ORIGIN,CYLINDER_AXIS,&
                ...
                VERTS,VOLUS,XB,ZMIN,ZVALS,ZVAL_HORIZON,EXPAND_CUTCELLS
...
221   IF(IOS > 0) THEN
   ...
   G%EXPAND_CUTCELLS = EXPAND_CUTCELLS
