...
INTEGER, ALLOCATABLE, DIMENSION(:,:) :: INTEGER_SEND_BUFFER,INTEGER_RECV_BUFFER
INTEGER, ALLOCATABLE, DIMENSION(:) :: BSEND_BUFFER
INTEGER :: BSEND_BUFFER_BYTE_SIZE
...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(4)
      ...
      IF (N_MPI_PROCESSES>1) THEN
         ALLOCATE(INTEGER_SEND_BUFFER(7,NMESHES))
         ALLOCATE(INTEGER_RECV_BUFFER(7,NMESHES))
         ALLOCATE(BSEND_BUFFER(NMESHES*(7*C_SIZEOF(EB)+MPI_BSEND_OVERHEAD)))
         BSEND_BUFFER_BYTE_SIZE = C_SIZEOF(BSEND_BUFFER)  ! Size of BSEND_BUFFER in bytes
         CALL MPI_BUFFER_ATTACH(BSEND_BUFFER,BSEND_BUFFER_BYTE_SIZE,IERR)
      ENDIF
      ...
      DO NM=1,NMESHES
         ...
         DO NOM=1,NMESHES
            ...
            IF (N_MPI_PROCESSES>1 .AND. NM/=NOM .AND. PROCESS(NOM)/=MYID) THEN
               INTEGER_SEND_BUFFER(1,NOM) = MESHES(NM)%OMESH(NOM)%I_MIN_R
               INTEGER_SEND_BUFFER(2,NOM) = MESHES(NM)%OMESH(NOM)%I_MAX_R
               INTEGER_SEND_BUFFER(3,NOM) = MESHES(NM)%OMESH(NOM)%J_MIN_R
      ...
      DO NM=1,NMESHES
         ...
         DO NOM=1,NMESHES
            ...
            IF (N_MPI_PROCESSES>1 .AND. NM/=NOM .AND. PROCESS(NM)/=MYID) THEN
               CALL MPI_RECV(INTEGER_RECV_BUFFER(1,NM),7,MPI_INTEGER,PROCESS(NM),NM,MPI_COMM_WORLD,STATUS,IERR)
               MESHES(NOM)%OMESH(NM)%I_MIN_S = INTEGER_RECV_BUFFER(1,NM)
               MESHES(NOM)%OMESH(NM)%I_MAX_S = INTEGER_RECV_BUFFER(2,NM)
               MESHES(NOM)%OMESH(NM)%J_MIN_S = INTEGER_RECV_BUFFER(3,NM)
      ...
      IF (N_MPI_PROCESSES>1) THEN
         CALL MPI_BUFFER_DETACH(BSEND_BUFFER(1),BSEND_BUFFER_BYTE_SIZE,IERR)
