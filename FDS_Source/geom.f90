...
!    I11   I21      
   ...
   G%HAS_TEXTURE=0
   IF (SURF_INDEX>=1 .AND. SURF_INDEX<=N_SURF) THEN
...
DO IFACE=NLONG, N_FACES-NLONG-1
   ...
   IF (MOD(IFACE-NLONG,2*NLONG)==0)THEN
      TFACE(1)=0.0_EB
      TFACE(3)=0.0_EB
   ENDIF
   IF (MOD(IFACE-NLONG,2*NLONG)==1) TFACE(3)=0.0_EB
   
   IF (MOD(IFACE-NLONG,2*NLONG)==2*NLONG-2) TFACE(5)=1.0_EB
   IF (MOD(IFACE-NLONG,2*NLONG)==2*NLONG-1)THEN
      TFACE(1)=1.0_EB
      TFACE(5)=1.0_EB
   ENDIF
ENDDO
...
IF (ABS(TFACE1(2)-1.0_EB)<10.0_EB*TWO_EPSILON_EB) THEN
   TF12(1)=TFACE1(3)
   TF13(1)=TFACE1(5)
ENDIF
IF (ABS(TFACE1(2))<10.0_EB*TWO_EPSILON_EB) THEN
   TF13(1)=TFACE1(5)
   TF12(1)=TFACE1(3)
ENDIF
...
DO I = 0, N_GEOMETRY
   ...
   IF (G%HAS_SURF==1) HAS_SURF=1
   IF (G%HAS_TEXTURE==1) HAS_TEXTURE=1
   IF (G%HAS_MATL==1) HAS_MATL=1
...
G%N_FACES = IFACE
G%N_VOLUS = IVOLUS
IF (G%HAS_SURF==1 .AND. IFACE>0) THEN
   SURF_INDEX = GET_SURF_INDEX(G%SURF_ID)
   G%SURFS(1:G%N_FACES) = SURF_INDEX
ENDIF
IF (G%HAS_MATL==1 .AND. IVOLUS>0) THEN
...
SUBROUTINE OUTGEOM(LUNIT,IS_DYNAMIC,TIME)
   ...
   IF (N_FACES>0) THEN
      ...
      IF(HAS_SURF==1)WRITE(LUNIT) (SURF_IDS(I), I=1,N_FACES)
      IF(HAS_TEXTURE==1)WRITE(LUNIT) (REAL(TFACES(I),FB), I=1,6*N_FACES)
   ENDIF
   IF (N_VOLUS>0) THEN
      WRITE(LUNIT) (VOLUS(I), I=1,4*N_VOLUS)
      IF(HAS_MATL==1)WRITE(LUNIT) (MATL_IDS(I), I=1,N_VOLUS)
   ENDIF
