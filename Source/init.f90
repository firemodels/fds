...
THIN_WALL_LOOP: DO ITW=1,M%N_THIN_WALL_CELLS
   ...
   FIND_BACK_THIN_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
      ...
      IC = OM%CELL_INDEX(II,JJ,KK)
      ITW2 = OM%THIN_WALL_INDEX(IC,-IOR,IEC)

      IF (ITW2>0 .AND. OM%THIN_OBST_INDEX(IC,-IOR,IEC)==OB%ORDINAL) THEN ! the back thin wall is found
...
! Use the recently populated THIN_WALL_INDEX(IC,IOR,IEC) and count the number of
...
M%THIN_OBST_INDEX(M%CELL_INDEX(I,J,K),IOR,IEC) = OB%ORDINAL
