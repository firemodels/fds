...
\begin{titlepage}
  ...
  \begin{center}
    {\bf \huge Fire Dynamics Simulator with Evacuation}\\[0.5em]

%    {\bf \huge FDS+Evac, version 5.}\\[0.2em]

    {\bf \huge Technical Reference and User's Guide}\\[0.5em]

    {\large (FDS 5.3.0, Evac 2.1.1, SVN Revision 3193)}
  \end{center}
  \vspace{30pt}
  \begin{center}
...
(FDS) developed at the National Institute of Standards and Technology
(NIST).  The current FDS+Evac version is 2.1.1 and it is embedded in
the current FDS version, which is 5.3.
...
visualization programme for FDS.  Their full capabilities are
...
intended only to supplement the informed judgment of the qualified
...
commercial software does not constitute endorsement by VTT, nor does
it indicate that the products are necessarily those best suited for
...
Dynamics Simulator (FDS)~\cite{FDS_Manual, FDS_UserGuide,
  FDS_VVGuide1, FDS_VVGuide2} developed at National Institute of
Standards and Technology (NIST), and in particular, the evacuation
module~\cite{Korhonen05, Korhonen07a, Korhonen07b, Korhonen08a,
  Korhonen08b} developed at VTT Technical Research Centre of Finland.
This combined fire and evacuation simulation program is called
FDS+Evac in this document.  FDS+Evac allows simultaneous simulation of
the fire and evacuation processes.  FDS+Evac can also be used to
simulate only the human egress process without any fire effects.
...
how to do fire simulations.  Even if the user wants to run the
programme without fire, she/he should be acquainted with the FDS in
order to build up the simulation geometry.


\section{Getting Started}\label{Sec_GetStart}

\noindent The evacuation module is implemented as
...
``Stairs.fds'' on the FDS+Evac web pages.
...
meshes and FDS+Evac can be run in an ``evacuation drill'' mode.
...
agents towards the exits.  This dificulty can be addressed by an
experience user, but an easier way is always to use evacuation meshes,
which are not too fine.  Usually mesh cell sizes 0.25~m or larger can
be used without any problems.  For example, if you have 1.2~m wide
doors you could use 0.3~m, 0.6~m, or 1.2~m cell spacing depending on
how detailed geometry is needed.  Note, that Smokeview does not yet
...
Smokeview.  Note also that the spaces, where agents are allowed to
move, should be at least about 0.7~m wide, because FDS+Evac is not
able to move agents in narrower exit paths correctly.  User should
check the evacuation geometry by using the Smokeview programme and
make sure that the ``holes'' in the geometry are not too narrow.  The
...
geometry is always used for the duration of the simulation, but the
user can give time dependent information on the usability of the doors
for the door selection algorithm.
...
more than 10000 agents are tried to place on the same evacuation mesh.
...
require a couple of initialization trials because the initial
positions of agents are generated randomly. If FDS+Evac cannot place
agents on their initial positions, an error message ``ERROR: FDS
...
density is used both to slow down the walking speeds of agents and to
affect the exit selection algorithm of agents.  Smoke density can also
...
agents do not try to avoid a fire if the user does not explicitely
...
of agents.  In addition, there is a small random force on each agent's
...
evacuation flow fields used to guide agent movement need to be
...
the amount of available memory.  This may require swicthing from a 32
...
consult the \Timtt{Readme.txt} file on the FDS+Evac web page.
...
their \Timtt{XB} fitted to the underlaying grid just like the ordinary
geometric objects in FDS fire calculation meshes, e.g., \Timtt{OBST}
and \Timtt{VENT} namelists.  Note, that the \Timtt{EVAC},
...
is slightly inside a wall, see Fig.~\ref{Fig_DoorGeom}.  One had to
define a small hole in the wall, where an exit (or a door) was placed.
This is not anymore nesessary in version 2.1.0.  The ``outflow'' vent
and the exit door can be placed directly at the wall surface.  But the
user should still note, that because the obstacles and vents are
aligned with the mesh but the exits and doors are not, one should be
sure that exits (or doors) a positioned correctly.  Use Smokeview and
activate mesh lines to see the actual position of the ``outflow'' vent
and use these coordinates to define the exit door.
...
to give the color of the agents shown in Smokeview on the \Timtt{EVAC}
...
used to give the coloring information instead of the
...
\noindent FDS+Evac is primarly a research tool for studying evacuation processes
...
visualize the results of FDS and FDS+Evac calculations and information
...
\noindent The underlaying fire simulation program, Fire Dynamics Simulator
...
and co-workers are cited in the Acknowledgments.
...
time step.  The movement of agents can be visualized using the
...
the diagnostic evacuation text file including the initial positions
...
\begin{figure}[tb]
  ...
  % T‰nne myˆs selitys soc-voimasta, jos sellainen kuva laitetaan
  % mukaan.
\end{figure}
%
...
Eq.~\ref{Eq_contactforce} contains also a physical damping
force~\cite{Langston06} with a damping parameter $c_d$, which the
original model by Helbing {\em et al.} does not have.  This parameter
reflects the fact that the collision of two humans is not an elastic
one.  The physical wall--agent interaction, $ \mathbf{f}^{c}_{iw}$, is
treated similarly and same force constants are used.
...
agents, e.g., describing a fire fighter pair entering the building.
All of the force terms in Eq.~\ref{Eq_force} are relatively short
ranged and they need a line-of-sight connection.  Longer ranged forces
could be taken in to account by changing the preferred walking
velocity field $ \mathbf{v}_i^0 $.
...
\end{equation}
%
where $\omega^0 $ is the maximum target angular speed of a turning
...
agents have same elastic constants then $k_{i} = 2 k_{ij}$.  And the
friction coefficient between two agents is simply $\kappa_{ij} =
\kappa_i$.
...
truncated Gaussian with mean zero, standard deviation of $ {\xi}_i/m_i
= 0.1 ~\mathrm{ \textrm{m} \, \textrm{s}^\textrm{-2}}$, and it is
truncated at three times of the standard deviation.  The random torque
...
%s‰‰t‰minen, satunnaisvoiman lauseke.  T‰nne myˆskin se, ett‰ mitein
...
\item Make a FDS fire input file for your project and do the
  ...
  calculation utilizes just one processor.
%
...
%
\item Define the main evacuation meshes (\Timtt{EVAC\_HUMANS=.TRUE.}
  and \Timtt{EVACUATION=.TRUE.}).  Each floor should have its own
  mesh.  The $z$ co-ordinates for these meshes should be at a level,
  where the most obstacles for the movement are, usually about one
  meter above the floor.  The \Timtt{EVAC\_Z\_OFFSET} parameter
  defines the floor level of this mesh measured from the mid $z$ level
  of the mesh.  See Fig.~\ref{Fig_EvacGrid} and
  Sec.~\ref{Sec_EvacGrid}.  Note, that the smoke and other fire
  related quantities are taken at the level defined by the
  \Timtt{HUMAN\_SMOKE\_HEIGHT} parameter on (some) \Timtt{PERS}
  namelist.  Try to make the mesh division such that the mesh cell
  sizes in the $x$ and $y$ are nice round numbers, like 0.25~m,
  0.35~m, 0.5~m, \emph{etc}, depending on the widths of the egress
  paths.
%
...
\item Define evacuation flow field vents (\Timtt{EVACUATION=.TRUE.},
  ...
  on each flow field.  Many flow fields per a floor is needed when the
...
\item Define your doors, exits, stairs, and entries, the \Timtt{DOOR},
  ...
  \Timtt{VENT}s to match the mesh cell boundaries.  Note, that the
  Evac version 2.1.1 also moves the \Timtt{DOOR}, \Timtt{EXIT}, and
  \Timtt{ENTR} to match the mesh cell boundaries.  The older Evac
  versions were not doing this matching to the underlaying
  computational mesh.  See Sec.~\ref{Sec_SpecFeatures} and
...
\item The evacuation input should be matched to the evacuation floor
  ...
  \Timtt{DOOR}, \Timtt{EXIT}, \Timtt{ENTR}, and \Timtt{EVAC}, are
  moved to the closest mesh points starting from the Evac version
  2.1.1.  For now, Smokeview does not show these evacuation related
  objects.
%
...
%
  \item The positions of doors and exits are not checked, \emph{i.e.},
    ...
    meshes is solid by default, if there is no vents specified.  Thus,
    it is nice to add \Timtt{SLCF} output for velocity vectors at the
    $z$-level of the evacuation meshes and see these vectors in
    Smokeview.
%
  \item Check your \Timtt{EVAC} lines that you are not putting agents
    in areas, where humans should not be.  You can use \Timtt{EVHO}
    namelists to exclude the initialization of agents at some place.
    If you want that the agents never go somewhere, then you should
    use evacuation \Timtt{OBST}, not \Timtt{EVHO}.  Check also that
    agents can not go 'out of bounds', \emph{i.e.}, that there are no
    openings in the evacuation geometry where no door nor exit is
    defined.
%
  \item Check your \Timtt{EVAC} lines that you are not trying to put
    ...
    is somewhere between 0.5--0.6~m, so you can not put more than
    about four persons per a square meter.  If you are trying to
    populate the floor more densly, the program will stop after the
    ...
    the diagnostic evacuation output file CHID\_evac.out to see the
    position of the agent, which could not be placed correctly at the
    initialization.
%
\end{itemize}
...
going to be using same exit routes.  If you have a floor, where two or
more parts are not directly connected by open routes, then the flow
field of the main evacuation mesh is not nice.  Note also that the
evacuation calculation is faster if you define different meshes for
the parts, which do not interact with each other, because the
...
makes less mistakes this way.  The earlier FDS+Evac versions did not
move the evacuation objects to the closest mesh points, but the
present version (2.1.1) and onwards the namelists \Timtt{DOOR},
\Timtt{EXIT}, and \Timtt{ENTR} are move to closest mesh points.

Evacuation meshes, which are only used to calculate (additional)
evacuation movement flow fields towards different doos/exits should
...
T_\textrm{start}$.  The product $t_\mathrm{flow} =
...
fields should become 'steady-state' during this time.  Note, that the
...
%
\item[\Timtt{EVACUATION}] Should be \Timtt{.TRUE.} for all evacuation
  flow field vents.  (Default is \Timtt{.FALSE.})  There should be an
  evacuation \Timtt{VENT} defined at each exit/door on the main
  evacuation meshes, or otherwise the agents will not be able to use
  these doors.
%
...
\Timtt{DET}, or \Timtt{PRE}.  See Eqs.~ \ref{Eq_GammaDist} -
\ref{Eq_GumbelDist} for the definition for the
distributions.}\label{Table_StatDists}
...
$\sigma_{\ln(x)}$, $\ln(x)$ is normally distributed)\\  
...
Table~\ref{Table_DefaultHumans} for their properties.  The default
values of the other agent related parameters are listed at the end of
Ch.~\ref{Sec_MoveModel}.  This namelist is also used to set some
miscellaneous (global) parameters for evacuation calculation.
...
\item[\Timtt{PRE\_MEAN,PRE\_PARA,PRE\_PARA2,PRE\_LOW,PRE\_HIGH}] The
  parameters of the reaction time distribution (is equal to
...
\item[\Timts{DIA\_MEAN,DIA\_PARA,DIA\_PARA2,DIA\_LOW,DIA\_HIGH}]
  Diameter of the agent circle ($2R_d$), the parameters of the
...
\Timtt{PERS} lines are used.  So, it is nice practice to give these
keywords just on one \Timtt{PERS} group or have exactly same values on
every \Timtt{PERS} group.  Some of these keywords set some model
parameters for all the agents and some are used to specify the outputs
and the mode of operation.
%
\begin{description}
%
\item[\Timts{FAC\_A\_WALL}] Social force constant $A_w$ for
  wall--agent interaction is \Timtt{FAC\_A\_WALL}$*A_i$.
%
\item[\Timts{FAC\_B\_WALL}] Social force constant $B_w$ for
  wall--agent interaction is \Timtt{FAC\_B\_WALL}$*B_i$.
%
...
\item[\Timts{HUMAN\_SMOKE\_HEIGHT}] Specifies the level above the
  ...
  parameter \Timts{EVAC\_Z\_OFFSET} and the co-ordinates \Timtt{XB} on
  the evacuation mesh lines define the floor levels.  (Default is
  1.6~m.)
%
...
\begin{equation}\label{Eq_NormalDist}
 f(x) = \frac{1}{ \sigma \sqrt{2\pi}} ~ \exp \left\{ -0.5(
 \frac{x-\bar{x}}{\sigma} )^2 \right\}   ~,~ \sigma > 0
\end{equation}
%
%Log-normal 
%%
...
\noindent Places agents in the evacuation meshes, \emph{i.e.}, the initial positions of
the agents.
%
\begin{description}
%
...
\noindent Defines an exit.  Note, that an 'outflow' vent is not automatically
...
\item[\Timtt{XB}] Co-ordinates, should be a line in the $(x,y)$ plane,
  ...
  belongs to many evacuation meshes then the keyword \Timts{MESH\_ID}
  should be given.  Note, that the Evac versions 2.1.1. and later are
  moving the corners of the rectangle defined by \Timtt{XB} to the
  closest grid cell corners.
%
\item[\Timts{MESH\_ID}] If there are overlapping main evacuation
  meshes then this parameter could be used to specify the mesh where
...
\item[\Timts{TIME\_START}] The time (s) when this entry starts adding
  agents to the calculation.  The default is $T_\textrm{start}$.
%
...
\item[\Timts{FLOW\_FIELD\_ID}] Flow field in the 'room/floor', which
  the agents are following, \emph{i.e.}, specifies to which door an
...
\noindent Defines stairs (or a horizontal corridor).  One uses \Timtt{CORR} to
move agents from one floor to the next one, \emph{i.e.}, from one main
evacuation mesh to some other one.  The corridor (actually stairs)
...
\item[\Timtt{MAX\_HUMANS\_INSIDE}] how many agents fit inside the
  corridor.  A rule of thumb: 2 persons per square metre.
%
...
\item[\Timtt{XB}, \Timtt{XB1}, \Timtt{XB2}] Used to specify the
  points, where the smoke and FED data is taken for this
  corridor/stairs.  If only one value is used for the corridor/stairs,
  give \Timtt{XB}.  If the values at the beginning and at the end of
  the corridor/stair is used, give both \Timtt{XB1} and \Timtt{XB2},
  respectively, and the FED data values are linearly interpolated
  between the start and the end of the corridor/stairs.
%
\item[\Timtt{EFF\_LENGTH}] The effective length of the
  corridor/stairs, movement time for agents inside the stairs is
  calculated as \Timtt{EFF\_LENGTH}/(\Timts{FAC\_SPEED}$\times
  v_i^0$).
%
...
flows in stairs, then you should model the ladings explicitely and you
can use \Timtt{CORR} to move the agents from one landing to the next
one.
...
\noindent Defines an incline (\emph{e.g.}, stairs or a spectator stand) or an
...
\Timtt{EVSS} constructions are only used to plot the agents in
'correct' positions when they are viewed by Smokeview.  The agent
movement algorithm uses only the $x$ and $y$.
...
\item[\Timtt{IOR}] Direction of the incline, \emph{e.g.}, +1 means
  ...
  the $z$ of \Timtt{XB} and the -$x$ edge is a horizontal line a
  height \Timts{HEIGHT} above the main evacuation plane.
%
\item[\Timts{MESH\_ID}] If there are overlapping main evacuation
  meshes then this parameter could be used to specify the mesh where
...
\item[\Timts{HEIGHT0}] The height of the base line of the incline
  ...
  \Timts{HEIGHT0}=\Timts{HEIGHT} the \Timtt{EVSS} defines a horisontal
  plane and \Timtt{IOR} has no effect.
%
\item[\Timts{FAC\_V0\_UP}] The unimpeded speed of an agent is
...
\item[\Timts{FAC\_V0\_HORI}] The unimpeded speed of an agent
  horisontally is $\Timts{FAC\_V0\_HORI} \times
  v_i^0$.
%
\item[\Timts{ESC\_SPEED}] The speed of an escalator (m/s).  One can
  use \Timtt{EVSS} as a simple model of an escalator, agents are all
  moving with the velocity \Timts{ESC\_SPEED} along the escalator,
  \emph{i.e.}, they are not overtaking each others nor walking on the
  escalator.
%
%
\end{description}
...
calculation.  This file depends only on the evacuation meshes.  If
this file exist and there are no fire meshes in the FDS input file, it
is read in.  Otherwise it is (re)calculated and saved on the disk.
Because FDS+Evac is a stochastic, one should always run the same
FDS+Evac simulation a dozen or so times to see the stochastic
variability in the results.  This file is also useful when one is
doing many egress calculations using exactly the same geometry but
with different human input, \emph{e.g.}, varying the number of humans
and the democraphics of humans.  This is very useful when doing Monte
Carlo calculations in a quite complex buildings.  One do not need to
calculate the evacuation flow fields and, thus, one is saving some CPU
seconds.  This file need not to be recalculated, if only the namelists
\Timtt{EVAC}, \Timtt{EVHO}, \Timtt{PERS}, and \Timtt{ENTR} are
changed in the FDS input file.  Also the simulation time
\Timtt{T\_END} on the \Timtt{TIME} namelist may be changed together
with any only fire mesh related changes.
...
one, which wrote the files.  Note, that the fed-file depends on the
namelists \Timtt{DOOR}, \Timtt{EXIT}, and \Timtt{CORR}, whereas the
eff-file does not.
...
columns are printed only if the FED is used, \emph{i.e.}, the smoke
and toxic gas information is available.  The first of these columns
reports the number of dead agents, \emph{i.e.}, those whose FED values
are larger than unity.  The next column is the maximum value of FED
among all agents, dead or alive.  Note, that the FED value of the
``dead'' agents will continue to build up as if they would be alive.
Finally, the last column is the maximum FED value of the agents which
...
simulation of the egress scenario, see Ch.~\ref{Sec_UpdatingInput} and
Ch.~\ref{Sec_InFiles}.
...
The development work of FDS+Evac is nicely overviewed on the VTT's
...
additional work is presented on the FDS+Evac web pages.
...
\item Fractional Effective Dose is calculated and used to
  ...
  O${}_\mathrm{2}$ consentrations are used.
%
...
\item A simple stair algorithm without merging flows is implemented.
  ...
  file containing the landings and stairs using \Timtt{EVSS}
  namelists.
%
...
to the programme source code but these are not discussed on the
manual.  These featrures are still work-on-progress stage.
%
\begin{itemize}
%
\item Merging flows in stairs: An easy to use staircase model with
  merging flows and smoke information.  
%
\item A more realistic \Timtt{CORR} object: Record the position of
  ...
  motion to model the movement in the corridor/stairs or use some kind
  of density vs speed correlation.
%
...
\item More intelligence in the exit door selection algorithm,
  ...
  heading/queuing for the same exit door.  Better treatment of the
  non-visible known doors.
%
\item Social interactions like herding, small groups, \emph{etc.}
%
...
%
\item Parallelization of the evacuation calculation using MPI.
%
\end{itemize}
%
...
\begin{itemize}
%
\item Restrictions on the geometry and the calculational mesh due to
  the rectanglular nature of the FDS geometry, \emph{i.e.}, objects,
...
results and example cases than this manual.
