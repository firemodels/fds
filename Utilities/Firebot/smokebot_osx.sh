...
compile_fds_db()
{
   ...
   cd $FDS_SVNROOT/FDS_Compilation/intel_$platform$S$DB
   rm -f fds_intel_$platform$S$DB
   make --makefile ../makefile clean &> /dev/null
...
compile_fds_mpi_db()
{
   ...
   cd $FDS_SVNROOT/FDS_Compilation/mpi_intel_$platform$S$IB$DB
   rm -f fds_mpi_intel_$platform$S$IB$DB
   make --makefile ../makefile clean &> /dev/null
...
check_compile_fds_db()
{
   ...
   # if the executable does not exist then an email has already been sent
      if [ -e "fds_intel_$platform$S$DB" ] ; then
        THIS_FDS_FAILED=1
      fi
   fi
}
...
check_compile_fds_mpi_db()
{
   # Check for errors in FDS debug compilation
   cd $FDS_SVNROOT/FDS_Compilation/mpi_intel_$platform$S$IB$DB
   if [ -e "fds_mpi_intel_$platform$S$IB$DB" ]
   then
   ...
   then
      ...
      echo "" >> $WARNING_LOG
   # if the executable does not exist then an email has already been sent
      if [ -e "fds_mpi_intel_$platform$S$IB$DB" ] ; then
        THIS_FDS_FAILED=1
      fi
   fi
}
...
compile_fds()
{
   ...
   cd $FDS_SVNROOT/FDS_Compilation/intel_$platform$S
   rm -f fds_intel_$platform$S
   make --makefile ../makefile clean &> /dev/null
...
{
   # Clean and compile FDS
   cd $FDS_SVNROOT/FDS_Compilation/mpi_intel_$platform$S$IB
   rm -f fds_mpi_intel_$platformSS$IB
   make --makefile ../makefile clean &> /dev/null
...
check_compile_fds_mpi()
{
   # Check for errors in FDS compilation
   cd $FDS_SVNROOT/FDS_Compilation/mpi_intel_$platform$S$IB
   if [ -e "fds_mpi_intel_$platform$S$IB" ]
   then
...
{  
   # smokeview libraries
   cd $FDS_SVNROOT/SMV/Build/LIBS/lib_$platform\_intel$SS
   echo 'Building Smokeview libraries:' > $FIREBOT_DIR/output/stage5pre 2>&1
   ./makelibs.sh >> $FIREBOT_DIR/output/stage5pre 2>&1

   # smokezip:
   cd $FDS_SVNROOT/Utilities/smokezip/intel_$platform$S
   rm -f *.o smokezip_$platform$S
   ...
   # smokediff:
   cd $FDS_SVNROOT/Utilities/smokediff/intel_$platform$S
   rm -f *.o smokediff_$platform$S
  ...
  # wind2fds:
   cd $FDS_SVNROOT/Utilities/wind2fds/intel_$platform$S
   rm -f *.o wind2fds_$platform$S
...
check_smv_utilities()
{
   ...
   cd $FDS_SVNROOT
   if [ -e "$FDS_SVNROOT/Utilities/smokezip/intel_$platform$S/smokezip_$platform$S" ]  && \
      [ -e "$FDS_SVNROOT/Utilities/smokediff/intel_$platform$S/smokediff_$platform$S" ]  && \
      [ -e "$FDS_SVNROOT/Utilities/wind2fds/intel_$platform$S/wind2fds_$platform$S" ]  && \
...
compile_smv_db()
{
   ...
   cd $FDS_SVNROOT/SMV/Build/intel_$platform$S
   rm -f smokeview_$platform$S$DB
