...
REAL(EB) :: XB(6), DX
INTEGER :: FACES(3*MAX_FACES), VOLUS(4*MAX_VOLUS)
...
READ_GEOM_LOOP: DO N=1,N_GEOMETRY
   ...
   ! check for general object (FACES, VERTS and VOLUS keywords)
   
   N_VERTS=0
   DO I = 1, MAX_VERTS
      IF (VERTS(3*I-2).GE.MAX_VAL.OR.VERTS(3*I-1).GE.MAX_VAL.OR.VERTS(3*I).GE.MAX_VAL) EXIT
      N_VERTS = N_VERTS+1
   END DO
   ...
   IF (N_ZVALS.GT.0) THEN
      ...
      CALL ChkMemErr('READ_GEOM','ZVALS',IZERO)
...
!    I11   I21      
 ...
 ! make IJK(1), IJK(2), IJK(3) consistent with grid resolution (if not specified on &GEOM line)
   ...
   IF (SPHERE_RADIUS.LT.MAX_VAL.AND. &
      ...
      IF (N_LEVELS==0) N_LEVELS = INT(LOG(2.0_EB*PI*SPHERE_RADIUS/(5.0_EB*DX))/LOG(2.0_EB))
      N_LEVELS = MIN(5,MAX(1,N_LEVELS))
      CALL INIT_SPHERE(N_LEVELS,N_VERTS,N_FACES,MAX_VERTS,MAX_FACES,VERTS,FACES)

      DO I = 1, N_VERTS
         VERTS(3*I-2) = SPHERE_ORIGIN(1) + SPHERE_RADIUS*VERTS(3*I-2)
         VERTS(3*I-1) = SPHERE_ORIGIN(2) + SPHERE_RADIUS*VERTS(3*I-1)
         VERTS(3*I)   = SPHERE_ORIGIN(3) + SPHERE_RADIUS*VERTS(3*I)
      END DO
   ENDIF
  ...
  ! wrap up
   ...
   IF (N_VOLUS.GT.0) THEN
      ...
      CALL ChkMemErr('READ_GEOM','MATLS',IZERO)
...
IF(N_FACES+3.GT.MAX_FACES.OR.N_VERTS+3.GT.MAX_VERTS)RETURN ! prevent memory overwrites
...
SUBROUTINE MERGE_GEOMS(VERTS,N_VERTS,FACES,SURF_IDS,N_FACES,VOLUS,MATL_IDS,N_VOLUS,IS_DYNAMIC)
...
ENDIF
IF (G%HAS_MATL.AND.IVOLUS>0) THEN
...
SUBROUTINE CONVERTGEOM(IS_DYNAMIC,TIME)
   ...
   IF (N_VERTS.GT.0.AND.(N_FACES.GT.0.OR.N_VOLUS.GT.0)) THEN
      ...
      CALL MERGE_GEOMS(VERTS,N_VERTS,FACES,SURF_IDS,N_FACES,VOLUS,MATL_IDS,N_VOLUS,IS_DYNAMIC)
   ENDIF
...
! Allocate FACET array
   ...
   IF (N_VOLU.GT.0) THEN
      ...
      CALL ChkMemErr('READ_GEOM','VOLU',IZERO)
...
SUBROUTINE OUTGEOM(LUNIT,IS_DYNAMIC,TIME)
   ...
   N_FACES = 0
   N_VOLUS = 0
   VOLLIST = (/1,2,4,  2,3,4,  1,4,3,  1,3,2 /)
   ...
   ALLOCATE(VOLUS(MAX(1,4*N_VOLUS)),STAT=IZERO)
   CALL ChkMemErr('WRITE_GEOM_TO_SMV','VOLUS',IZERO)

   ALLOCATE(MATL_IDS(MAX(1,N_VOLUS)),STAT=IZERO)
   CALL ChkMemErr('WRITE_GEOM_TO_SMV','MATL_IDS',IZERO)
   ...
   IF( N_VOLUS.GT.0) THEN
      WRITE(LUNIT) N_VERTS, 4*N_VOLUS
   ELSE
      WRITE(LUNIT) N_VERTS, N_FACES
   ENDIF
   IF (N_VERTS>0) WRITE(LUNIT) (REAL(VERTS(I),FB), I=1,3*N_VERTS)
   IF (N_VOLUS.GT.0) THEN
      WRITE(LUNIT) ((VOLUS(4*I+VOLLIST(K)),K=1,12),I=0,N_VOLUS-1)
      WRITE(LUNIT) (MATL_IDS(I),MATL_IDS(I),MATL_IDS(I),MATL_IDS(I), I=1,N_VOLUS)
   ELSE
      IF (N_FACES>0) THEN
