...
REAL(EB) :: ZZ_GET(0:N_TRACKED_SPECIES),ZZ_G(1:N_TRACKED_SPECIES),UN,MFT,DD,RSUM_F,EFT_LEAK,EFT_WALL,CP_WALL
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   ELSE !leakage
      ...
      ENDIF 
      IF(DUCT_U(DU,NM) > 0._EB) THEN
         DN2=DUCT(DU)%NODE_INDEX(1)
      ELSE
         DN2=DUCT(DU)%NODE_INDEX(2)      
      ENDIF        
      MFT = -DUCTNODE(DN)%DIR(1)*DUCT_U(DU,NM)*NODE_RHO(DN2,NM)/FDS_LEAK_AREA(IZ1,IZ2,1)
   ENDIF
   ...
   ELSE SPECIES_IF_1
      IF (UN > 0._EB) THEN
         MASSFLUX(IW,:) = 0._EB
         MASSFLUX(IW,1:N_TRACKED_SPECIES) = -NODE_ZZ(DN2,1:N_TRACKED_SPECIES,NM)*MFT
