...
DO N=1,M%N_SLCF
   ...
   ! write out slice file info to the .smv file
      ...
      IF (SL%ID/='null') THEN
         WRITE(SLICELABEL,'(A,A,A,A,A,A,A,I6)') ' # ',TRIM(SL%SLICETYPE),' %',TRIM(SL%ID),TRIM(SLICEPARMS),TRIM(SLICEOFFSET),' ! ',SL%SLCF_INDEX
      ELSE
         WRITE(SLICELABEL,'(A,A,A,A,A,I6)') ' # ',TRIM(SL%SLICETYPE),TRIM(SLICEPARMS),TRIM(SLICEOFFSET),' ! ',SL%SLCF_INDEX
      ENDIF
      IF (SL%SLICETYPE=='STRUCTURED') THEN
