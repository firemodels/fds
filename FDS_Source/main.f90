...
IF (ANY(EVACUATION_GRID)) THEN
   ...
   IF (MYID==MAX(0,EVAC_PROCESS)) CALL INIT_EVAC_GROUPS
ENDIF
...
DO NM=1,NMESHES
   IF (MYID==MAX(0,EVAC_PROCESS) .AND. .NOT.EVACUATION_ONLY(NM)) THEN
      M=>MESHES(NM)
...
T_EVAC_SAVE = T_EVAC
IF (ANY(EVACUATION_GRID) .AND. MYID==MAX(0,EVAC_PROCESS)) CALL INITIALIZE_EVAC_DUMPS
...
IF (MYID/=MAX(0,EVAC_PROCESS)) CALL EVAC_MESH_EXCHANGE(T_EVAC,T_EVAC_SAVE,I_EVAC,ICYC,EXCHANGE_EVACUATION,1)
IF (MYID==MAX(0,EVAC_PROCESS)) CALL EVAC_MESH_EXCHANGE(T_EVAC,T_EVAC_SAVE,I_EVAC,ICYC,EXCHANGE_EVACUATION,2)
...
EVAC_TIME_STEP_LOOP: DO WHILE (T_EVAC < T_FIRE)
   ...
   IF (MYID==MAX(0,EVAC_PROCESS)) CALL PREPARE_TO_EVACUATE(ICYC)
   DO NM=1,NMESHES
      IF (EVACUATION_ONLY(NM)) THEN
         ACTIVE_MESH(NM) = .FALSE.
