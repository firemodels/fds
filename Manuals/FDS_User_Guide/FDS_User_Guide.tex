...
\item[{\ct AUTO\_IGNITION\_TEMPERATURE}] The temperature below which combustion will not occur.  Note that if this parameter is used, then some form of ignition source must be present in order for combution to begin. (Mixture Fraction only, default 0.0 K)
\end{description}
...
can be specified using the parameters {\ct NU\_GAS(j,k)} and {\ct SPEC\_ID(j,k)}, where {\ct j} refers to the {\ct j}'th reaction
...
&MATL ID = 'WOOD'
      ...
      N_S                     = 1.0
      NU_RESIDUE              = 0.18
      SPEC_ID(1,1:4) = 'PYROLYZATE','OXYGEN','WATER VAPOR','CARBON DIOXIDE'
...
\footnotesize
\begin{verbatim}
&SPEC ID = 'NITROGEN',BACKGROUND=.TRUE.,BACKGROUND_ONLY=.TRUE./
