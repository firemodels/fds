...
\be
  h_{\alpha}(T) = h_{\alpha,ref} + \int_{T_{ref}}^{T_{R}} c_{p,\alpha}(T) \, \mbox{d}T
\ee
...
\chapter{Scaling Based Pyrolysis}
...
\be
   \dq_{net}''=\dm_f''\Delta H_g
   \label{Spyro1}
\ee
where $\dq_{net}''$ is the net heat flux into the surface and $\Delta H_{g}$ is the heat of gasification. Similarly, the heat release rate per unit area of a material, $\dQ''$, is related to the pyrolysis rate of the material through the effective heat of combustion, $\Delta H_c$, as
\be
   \dQ''=\dm_f''\Delta H_c
   \label{Spyro2}
\ee
The ratio of Eq.~(\ref{Spyro1}) and Eq.~(\ref{Spyro2}) relate $\dot{q}_{net}''$ and $\dot{Q}''$ as
\be
   \frac{\dQ''}{\dq_{net}''}=\frac{\Delta H_c}{\Delta H_g}
   \label{Spyro3}
\ee
Assuming $\Delta H_g$ and $\Delta H_c$ are invariant with $\dq_{net}''$, Eq.~(\ref{Spyro3}) can be related to predict the change in magnitude of $\dQ''$ due to changes in thermal exposure as
\be
   \dQ_1'' = \frac{\dq_{net,1}''}{\dq_{net,2}''}\dQ_2''
   \label{Spyro4}
\ee
...
\be
   E_{\Delta t}''=\dq_{net}''\frac{\Delta H_c}{\Delta H_g}\Delta t
   \label{Spyro5}
\ee
where $E_{\Delta t}$ is the total energy released over $\Delta t$. Relating Eq.~(\ref{Spyro5}) at two thermal exposure levels yields
\be
   \Delta t_{1}=\frac{\dq_{net,2}''}{\dq_{net,1}''}\Delta t_2
   \label{Spyro6}
\ee
Because $\dq_{net}''$ is difficult to measure during burning, an approximate reference heat flux, $\dq''_{ref}$, is introduced as
\be
   \dq''_{ref} = \dq''_{cone}(1-\Gamma) + \dq''_{flame}
   \label{Spyro7}
\ee
where $\dq''_{cone}$ is the set exposure flux to a cold surface in the cone calorimeter testing, $\Gamma$ is the fraction of that flux absorbed by the flame, and $\dq''_{flame}$ is the heat feedback from the flame to the surface. For the input data from a cone test, the values of $\Gamma$ and $\dq''_{ref}$ are not readily obtained, and the empirical approach discussed in Section~\ref{Spyro_q_flame} is used to estimate them when determining $\dq_{net,2}''$ in Eq.~\ref{Spyro4}. For $\dq_{net,1}''$, FDS uses the sum of the incident radiation and the net convective heat flux in the calculation at each time step. 
...
\ee
where $h$ is the heat transfer coefficient. The traditional formulation of $\hbox{Bi}$ assumes that the external exposure of the solid is related primarily to surface convection and thus uses the convection heat transfer coefficient, $h_c$, for $h$. However, this assumption is not valid in the case of burning solid fuels where external radiation (such as the applied surface heat flux in a cone calorimeter experiment) and flame radiation from the combustion of pyrolyzate above the surface drive the heat transfer to the surface. This is accounted for with an alternative Biot number, $\hbox{Bi}^*$, which combines the effect of surface convection and radiation:
\be
\hbox{Bi}^*=\frac{h_c+h_r}{k/\Delta}
\label{bi_eq}
\ee
where $h$ has been split into two components for convection, $h_c$, and radiation, $h_r$. The radiation heat transfer coefficient is calculated based on the linearized formulation~\cite{Incropera:1}:
\be
h_r=\varepsilon\sigma\left(T_r+T_s\right)\left(T_r^2+T_s^2\right)
\ee
where $\varepsilon$ is the surface emissivity, $\sigma$ is the Stefan-Boltzmann constant, $T_{r}$ is the radiative temperature, and $T_s$ is the surface temperature. The radiative temperature is an effective temperature representative of the energy absorbed at the surface assuming a view factor of 1:
\be
T_{r}=\left[\frac{\dq_r''}{\varepsilon \sigma}\right]^\of
\label{eq:tr}
\ee
where $\dq_r''$ is the incident radiation to the surface. 

As discussed in Section~\ref{spyro_appendix:fixed}, $\Delta H_g$ is assumed to be constant in the prior approach. However, a time-varying effective $\Delta H_g$ can be calculated based on Eq.~(\ref{Spyro3}) using the time-varying measurement of $\dQ''$ and estimating the time-varying $\dq''_{flame}$. It is assumed in this model that the variability in burning intensity and duration observed in experiments can be related to the impact of the material thickness and thermal exposure on these non-dimensional groups, $\Delta H_g=f\left(\hbox{Bi}^{*},\hbox{Fo}\right)$.  $\Delta H_g$ was found to collapse at the non-dimensional time-scale $\hbox{FoBi}^{*}$. A scaling relationship for $\hbox{FoBi}^{*}$ can be established by equating this non-dimensional time across two exposure levels,
\be
...
\be
\Delta t = \frac{\Delta}{\Delta_{ref}}\frac{\dq''_{ref}}{\dq''}\Delta t_{ref}
...
When using Eq.~\ref{Spyro7} to obtain $\dq_{net,2}''$ in Eq.~\ref{Spyro4}, an approach is needed to determine $\Gamma$ and $\dq_{flame}''$. In an actual cone test these will depend upon the material being burned as the heat of combustion, burning rate, and soot yield will all influence the flame heat flux and the absorption of the cone radiation. As there is no simple analytical model to predict these, an empirical approach was developed using FDS simulations of a cone calorimeter. 

The simulation geometry is shown in Fig.~\ref{fig:cone_ref_geom} and includes the radiant cone, the sample holder, and a volume above the cone to capture the flame height contribution to the heat feedback. A set of 30 simulations were run where the heat of combustion (10, 20, 30, 40, and 50~\si{MJ/kg}) and soot yield (0, 1, 2, 5, 10, and 20~\%) were varied (CO yield was assumed to equal the soot yield). Each simulation consisted of three phases: the sample burning at a prescribed rate with the cone off, the sample burning at a prescribed rate with the cone on, and no burning with the cone on ($\dq_{cone}''=50$~\si{kW/m^2}). The prescribed burning was varied over a range of HRRPUA (100, 200, 400, 800, 1200, 1600, and 2000~\si{kW/m^2}). The sample was given a fixed temperature of 300~$^\circ$C representing a notional ignition temperature. The cone off phase gives 
$\dq_{flame}''$, the delta between burning with the cone on and burning with the cone off and on gives the cone flux reaching the sample, and the cone with no burning phase gives $\dq_{cone}''$. The difference between the cone flux reaching the sample and $\dq_{cone}''$ gives $\Gamma$.
...
If $\dq_{flame}''$ for each heat of combustion is normalized by the maximum value for all HRRPUA, the results collapse as shown in the left of Fig~\ref{fig:Spyro_hoc}. A quadratic fit was then done for the maximum $\dq_{flame}''$ for each heat of combustion as shown in the right of Fig~\ref{fig:Spyro_hoc}.  
...
\begin{figure}
	...
	\end{tabular*}
	\caption[Empirical flame heat flux for Spyro]{(Left) Normalized flame heat flux as a function of HRRPUA, (Right) Maximum flame heat flux as a function of $\Delta H_c$.}
	\label{fig:Spyro_hoc}
\end{figure}

Various simple combinations of the heat of combustion, soot yield, and HRRPUA were evaluated to determine a relationship for $\Gamma$. The functional form of $A + B \times \hbox{HRRPUA} / \Delta H_c + C \times \hbox{soot yield}$ gave a fit with the lowest RMS value. To determine the time-dependent $\dq_{ref}''$ for a cone test first the maximum $\dq_{flame}''$ is determined using $\Delta H_c$ at the fit shown in Fig~\ref{fig:Spyro_hoc}, second  $\dq_{flame}''$ is adjusted for the current HRRPUA by interpolating the average normalized curve in Fig~\ref{fig:Spyro_hoc}, thirdly $\Gamma$ is determined and used to adjust  $\dq_{cone}''$, and finally $\dq_{ref}''$ is determined via Eq.~\ref{Spyro7}. The resulting fit has a 2~\% error compared to the FDS predictions, see Fig.~\ref{fig:Spyro_cone_error}.
