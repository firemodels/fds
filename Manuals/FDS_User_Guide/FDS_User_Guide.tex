...
For liquid droplets, particle sizes follow a given the Cumulative Volume Function (CVF) of droplet sizes.  Distribution can be selected by setting the {\ct DISTRIBUTION} parameter on the PART line. The default is {\ct ROSIN-RAMMLER-LOGNORMAL}. Other choices are {\ct LOGNORMAL} and {\ct ROSIN-RAMMLER}. Note that the specified {\ct DIAMETER} (in units of $\mu$m) on the {\ct PART} line is the median volumetric diameter of the droplets or particles. 
...
\ee
The width of the distribution is controlled by parameter {\ct SIGMA\_D} on the {\ct PART} line. The {\ct ROSIN-RAMMLER} distribution is given by
\be
...
User can also define the particle size distribution by defining the Cumulative Number Function of the distribution (CNF). This is done by setting the {\ct USER\_CNF\_RAMP} on the {\ct PART} line and including a {\ct RAMP} that gives the CNF. For example, a particles with diameters following the normal distribution can be defined as follows.
\begin{verbatim}
&PART ID='USER',
