...
\paragraph{A{\footnotesize NSYS} and Legacy Formats}~\\ 
...
\subparagraph{nodes.dat and elements.dat (ANSYS/Legacy Format)}~\\
...
\end{tabular*}
\caption[The \textct{h\_profile\_geom} parity results]{Diagnostic parity plots comparing mapped boundary conditions ($T_{\hbox{\tiny AST}}$ and $h$) between Legacy and GEOM extraction methods.}
\label{geom_parity_plots}
\end{figure}

Furthermore, the transient thermal evolution and the resulting thermomechanical displacements are tracked and compared. Figure~\ref{geom_thermal} shows the thermal history comparison for points A and B, confirming that the new mapping methodology perfectly overlaps with the legacy results.
