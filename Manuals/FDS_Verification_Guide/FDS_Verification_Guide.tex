...
\dot{m}^{\prime\prime\prime}_{fuel,i} = \frac{C_{mag}}{\tau_{mix}} \, \min \left( Y_F,\frac{Y_\OTWO}{s},\beta\frac{Y_P}{(1+s)} \right)
\end{equation}
where $C_{mag}$ is a model constant related to the turbulent subgrid model, $\beta$ is a model constant related to combustion, and $s$ is the mass stoichiometric coefficient. This model can be considered a deficient model as the consumption rate is found by the limiting species. To determine species evolution for the reaction, a system of ordinary differential equations is solved for each participating species ($\alpha$):
...
\end{equation}
is considered for both a fuel-limiting and oxygen-limiting case with a fixed mixing time, $\tau_{mix}=0.1$~s (Figure \ref{fig:MixCtrl_1Step}).
...
The EDM reaction implementation is also tested for a two-step methane reaction:

\begin{eqnarray}\label{eq:2step_methane}
\mathrm{CH_4 + 1.5 \, O_2 \rightarrow  CO + 2 \, H_2O} \\
\nonumber \mathrm{CO + 0.5 \, O_2 \rightarrow CO_2}
\end{eqnarray}
for both fuel-limited and oxygen-limited conditions with a fixed mixing time, $\tau_{mix}=0.1$~s (Figure \ref{fig:MixCtrl_2Step}).
...
\dot{m}^{\prime\prime\prime}_{\alpha,i}=k_{i}\,Y_{\alpha}^{a_{\alpha,i}}
\end{equation} 
...
\frac{dY_{\alpha}}{dt}=\frac{\nu_{\alpha} \, W_{\alpha}}{W_F} \,\displaystyle\sum_{i=1}^{NR} \dot{m}^{\prime\prime\prime}_{\alpha,i} 
\end{equation}
...
\hline Reaction & $\nu_{\alpha}$ & E & A & $a_{\alpha}$ & $n_{\alpha}$  \\ \hline \hline
