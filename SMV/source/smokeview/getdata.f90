...
REAL(EB), DIMENSION(0:299), INTENT(OUT), TARGET :: OUT_VERTS
...
!  add intersection of tetrahedron edge and box plane (if on box)
   ...
   DO TE = 0, 5 ! tetra edge
      ...
      IF ( PLANE_EDGE_INTERSECTION( BOXVERT,BOXNORMAL,TETRAVERT0,TETRAVERT1,OUT_VERT).EQ.1) THEN
         IF (IN_BOX(OUT_VERT, BP).EQ.1) THEN
             OUT_VERTS(3*NVERTS:3*NVERTS+2)=OUT_VERT
   ...
   DO BE = 0, 3 ! box plane edges
      ...
      DO TP = 0, 3 ! tetra plane
         ...
         IF ( PLANE_EDGE_INTERSECTION( TETRAVERT,TETRANORMAL,BOXVERT0,BOXVERT1,OUT_VERT).EQ.1) THEN
            IF (IN_TETRA(OUT_VERT, TP) .EQ. 1) THEN
               OUT_VERTS(3*NVERTS:3*NVERTS+2)=OUT_VERT
               FACENUM(NFACES) = FACENUM(NFACES) + 1
               NVERTS=NVERTS+1
            ENDIF
...
! add box plane vertices if inside tetrahedron
   ...
   DO BV = 0, 3
      ...
      IF (IN_TETRA(BOXVERT, -1) .EQ. 1) THEN
         OUT_VERTS(3*NVERTS:3*NVERTS+2)=BOXVERT
         FACENUM(NFACES) = FACENUM(NFACES) + 1
         NVERTS=NVERTS+1
      ENDIF
   END DO
...
DO F = 0, NFACES-1
  ...
  VBEG = 0
  VEND = FACENUM(F)-1
  FACEVERTS(0:3*VEND+2) => OUT_VERTS(3*FACESTART(F):3*FACESTART(F)+3*VEND+2)
...
DO F = 0, NFACES-1
   IF (FACENUM(F).GT.0) THEN
      WRITE(6,*)"   ",F+1
      DO V = FACESTART(F),FACESTART(F)+FACENUM(F)-1
         WRITE(6,*)"    ",OUT_VERTS(3*V:3*V+2)
      END DO
   ENDIF
END DO
...
FACEVERTS_TEMP(0:3*(NVERTS-1)+2) = FACEVERTS(0:3*(NVERTS-1)+2)
DO I = 0, NVERTS-1
...
INTEGER FUNCTION PLANE_EDGE_INTERSECTION(X0,N0,V0,V1,OUT_VERT)
...
REAL(EB), DIMENSION(0:2), INTENT(OUT) :: OUT_VERT
...
SUBROUTINE GETVERTS2(BOX_BOUNDS,V0,V1,V2,V3,OUT_VERTS,NVERTS,FACESTART,FACENUM,NFACES,VOLUME)
USE PRECISION_PARAMETERS
...
REAL(FB), DIMENSION(0:299), INTENT(OUT) :: OUT_VERTS
INTEGER, DIMENSION(0:99), INTENT(OUT) :: FACESTART, FACENUM
INTEGER, INTENT(OUT) :: NVERTS,NFACES
...
REAL(EB), DIMENSION(0:299), TARGET :: OUT_VERTS_EB
REAL(EB) :: VOL
...
CALL GETVERTS(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB,OUT_VERTS_EB,NVERTS,FACESTART,FACENUM,NFACES)

IF(NVERTS.GT.0)OUT_VERTS(0:3*NVERTS-1) = OUT_VERTS_EB(0:3*NVERTS-1)

V_CENTER=0.0_EB
DO I = 0, NVERTS - 1
   V_CENTER(0:2) = V_CENTER(0:2) + OUT_VERTS(3*I:3*I+2)
END DO
...
DO I = 0, NFACES-1
   ...
   IBEG0 = FACESTART(I)
   IBEG1 = FACESTART(I)+1
   VV0(0:2) => OUT_VERTS_EB(3*IBEG0:3*IBEG0+2)
   VV1(0:2) => OUT_VERTS_EB(3*IBEG1:3*IBEG1+2)
   DO J = FACESTART(I)+2, FACESTART(I) + FACENUM(I)-1
      VV2(0:2) => OUT_VERTS_EB(3*J:3*J+2)
