...
READ_GEOM_LOOP: DO N=1,N_GEOMETRY
   ...
   IF (ABS(AZIM_DOT).LT.TWO_EPSILON_EB .AND. ABS(ELEV_DOT).LT.TWO_EPSILON_EB.AND.&
       ANY(ABS(SCALE_DOT(1:3)).LT.TWO_EPSILON_EB) .AND. ANY(ABS(XYZ_DOT(1:3) ).LT.TWO_EPSILON_EB) ) THEN 
      G%IS_DYNAMIC = .FALSE.
   ELSE
      G%IS_DYNAMIC = .TRUE.
