...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZATION_IF: IF (CODE==0) THEN
      ...
      IF (NIC(NM,NOM)>0) THEN
         ALLOCATE(M3%REAL_RECV_PKG1(MAX(NIC(NM,NOM),NIC(NOM,NM))*(8+N_SPECIES)+1))
         ALLOCATE(M3%REAL_RECV_PKG2(MAX(NIC(NM,NOM),NIC(NOM,NM))*(8          )+1))
         ALLOCATE(M3%REAL_RECV_PKG3(MAX(NIC(NM,NOM),NIC(NOM,NM))*(8+N_SPECIES)+1))
...
DO NOM=1,NMESHES
   SNODE = PROCESS(NOM)
   IF (CODE==4 .AND. EXCHANGE_EVACUATION .AND. MYID==EVAC_PROCESS .AND. .NOT.EVACUATION_ONLY(NOM)) THEN
      M4=>MESHES(NOM)
...
RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZE_SEND_IF: IF (CODE==0) THEN
      ...
      IF (NIC(NOM,NM)>0 .AND. RNODE/=SNODE) THEN
         ALLOCATE(M3%REAL_SEND_PKG1(MAX(NIC(NOM,NM),NIC(NM,NOM))*(8+N_SPECIES)+1))
         ALLOCATE(M3%REAL_SEND_PKG2(MAX(NIC(NOM,NM),NIC(NM,NOM))*(8          )+1))
         ALLOCATE(M3%REAL_SEND_PKG3(MAX(NIC(NOM,NM),NIC(NM,NOM))*(8+N_SPECIES)+1))
            ...
            DO KK=M3%IJKW(12,IW),M3%IJKW(15,IW)
               DO JJ=M3%IJKW(11,IW),M3%IJKW(14,IW)
                  DO II=M3%IJKW(10,IW),M3%IJKW(13,IW)
                     ...
                     M3%REAL_SEND_PKG1(LL+3) = M%FVX(II,JJ,KK)
                     M3%REAL_SEND_PKG1(LL+4) = M%FVY(II,JJ,KK)
                     M3%REAL_SEND_PKG1(LL+5) = M%FVZ(II,JJ,KK)
                     M3%REAL_SEND_PKG1(LL+6) = M%FVX(II-1,JJ,KK)
                     M3%REAL_SEND_PKG1(LL+7) = M%FVY(II,JJ-1,KK)
                     M3%REAL_SEND_PKG1(LL+8) = M%FVZ(II,JJ,KK-1)
                     IF (N_SPECIES>0) M3%REAL_SEND_PKG1(LL+9:LL+8+N_SPECIES) = M%YYS(II,JJ,KK,1:N_SPECIES)
                     LL = LL+8+N_SPECIES
                  ENDDO
               ENDDO
            ENDDO
         ENDDO PACK_REAL_SEND_PKG1
         M3%REAL_SEND_PKG1(IWW*(8+N_SPECIES)+1) = -999.0_EB
         N_REQ=N_REQ+1
         CALL MPI_ISEND(M3%REAL_SEND_PKG1(1),IWW*(8+N_SPECIES)+1,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
            ...
            DO KK=M3%IJKW(12,IW),M3%IJKW(15,IW)
               DO JJ=M3%IJKW(11,IW),M3%IJKW(14,IW)
                  DO II=M3%IJKW(10,IW),M3%IJKW(13,IW)
                     ...
                     M3%REAL_SEND_PKG3(LL+3) = M%FVX(II,JJ,KK)
                     M3%REAL_SEND_PKG3(LL+4) = M%FVY(II,JJ,KK)
                     M3%REAL_SEND_PKG3(LL+5) = M%FVZ(II,JJ,KK)
                     M3%REAL_SEND_PKG3(LL+6) = M%FVX(II-1,JJ,KK)
                     M3%REAL_SEND_PKG3(LL+7) = M%FVY(II,JJ-1,KK)
                     M3%REAL_SEND_PKG3(LL+8) = M%FVZ(II,JJ,KK-1)
                     IF (N_SPECIES>0) M3%REAL_SEND_PKG3(LL+9:LL+8+N_SPECIES) = M%YY(II,JJ,KK,1:N_SPECIES)
                     LL = LL+8+N_SPECIES
                  ENDDO
                ENDDO
             ENDDO
         ENDDO PACK_REAL_SEND_PKG3
         M3%REAL_SEND_PKG3(IWW*(8+N_SPECIES)+1) = -999.0_EB
         N_REQ=N_REQ+1
         CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),IWW*(8+N_SPECIES)+1,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
         M2=>MESHES(NOM)%OMESH(NM)
...
DO NM=1,NMESHES
   IF (CODE==4 .AND. EXCHANGE_EVACUATION .AND. MYID/=EVAC_PROCESS .AND. PROCESS(NM)==MYID .AND. .NOT.EVACUATION_ONLY(NM)) THEN
      M => MESHES(NM)
...
RECV_MESH_LOOP: DO NM=1,NMESHES
         ...
         DO KK=M4%IJKW(12,IW),M4%IJKW(15,IW)
            DO JJ=M4%IJKW(11,IW),M4%IJKW(14,IW)
               DO II=M4%IJKW(10,IW),M4%IJKW(13,IW)
                  ...
                  M2%FVX(II,JJ,KK)   = M2%REAL_RECV_PKG1(LL+3)
                  M2%FVY(II,JJ,KK)   = M2%REAL_RECV_PKG1(LL+4)
                  M2%FVZ(II,JJ,KK)   = M2%REAL_RECV_PKG1(LL+5)
                  M2%FVX(II-1,JJ,KK) = M2%REAL_RECV_PKG1(LL+6)
                  M2%FVY(II,JJ-1,KK) = M2%REAL_RECV_PKG1(LL+7)
                  M2%FVZ(II,JJ,KK-1) = M2%REAL_RECV_PKG1(LL+8)
                  IF (N_SPECIES>0) M2%YYS(II,JJ,KK,1:N_SPECIES)= M2%REAL_RECV_PKG1(LL+9:LL+8+N_SPECIES)
                  LL = LL+8+N_SPECIES
               ENDDO
            ENDDO
         ENDDO
   ...
   IF (CODE==2 .AND. NIC(NOM,NM)>0 .AND. RNODE/=SNODE) THEN
      LL = 0
         ...
         DO KK=M4%IJKW(12,IW),M4%IJKW(15,IW)
            DO JJ=M4%IJKW(11,IW),M4%IJKW(14,IW)
               DO II=M4%IJKW(10,IW),M4%IJKW(13,IW)
                  ...
                  M2%FVX(II,JJ,KK)   = M2%REAL_RECV_PKG3(LL+3)
                  M2%FVY(II,JJ,KK)   = M2%REAL_RECV_PKG3(LL+4)
                  M2%FVZ(II,JJ,KK)   = M2%REAL_RECV_PKG3(LL+5)
                  M2%FVX(II-1,JJ,KK) = M2%REAL_RECV_PKG3(LL+6)
                  M2%FVY(II,JJ-1,KK) = M2%REAL_RECV_PKG3(LL+7)
                  M2%FVZ(II,JJ,KK-1) = M2%REAL_RECV_PKG3(LL+8)
                  IF (N_SPECIES>0) M2%YY(II,JJ,KK,1:N_SPECIES)= M2%REAL_RECV_PKG3(LL+9:LL+8+N_SPECIES)
                  LL = LL+8+N_SPECIES
               ENDDO
            ENDDO
         ENDDO
   ...
   RECEIVE_RADIATION: IF ( CODE==4 .AND. NIC(NOM,NM)>0 .AND. EXCHANGE_RADIATION .AND. RNODE/=SNODE) THEN
      LL = 0
   ...
   IF (CODE==4 .AND. DROPLET_FILE .AND. RNODE/=SNODE) THEN
   ...
   IF_RECEIVE_DROPLETS: IF (CODE==4 .AND. DROPLET_FILE .AND. RNODE/=SNODE) THEN 
...
USE MATH_FUNCTIONS, ONLY : GAUSSJ
REAL(EB) :: A_LOC(NCGC,NCGC),A(NCGC,NCGC),B_LOC(NCGC),B(NCGC),VDOT(NMESHES,NMESHES)
TYPE (MESH_TYPE), POINTER :: M
INTEGER :: IERROR,NM,II,JJ,KK,IOR,IW,NOM
