...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                HUMIDITY,IBLANK_SMV,IMMERSED_BOUNDARY_METHOD,INITIAL_UNMIXED_FRACTION,ISOTROPIC_PARTICLE_FORCE,&
...
ELSE
   ...
   IF (PRESSURE_TOLERANCE<TWO_EPSILON_EB) PRESSURE_TOLERANCE = 1.0_EB/CHARACTERISTIC_CELL_SIZE**2
ENDIF
...
REAL(EB) :: TRANSPARENCY,XB1,XB2,XB3,XB4,XB5,XB6,BULK_CONDUCTIVITY,BULK_DENSITY,BULK_SPECIFIC_HEAT,&
            VOL_ADJUSTED,VOL_SPECIFIED,UNDIVIDED_INPUT_AREA(3)
...
NAMELIST /OBST/ ALLOW_VENT,BNDF_FACE,BNDF_OBST,BULK_CONDUCTIVITY,BULK_DENSITY,BULK_SPECIFIC_HEAT,&
                COLOR,CTRL_ID,DEVC_ID,EVACUATION,FYI,HT3D,ID,MESH_ID,MULT_ID,NOTERRAIN,&
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               ENDIF
               OB%HT3D               = HT3D
               OB%BULK_CONDUCTIVITY  = BULK_CONDUCTIVITY
               OB%BULK_DENSITY       = BULK_DENSITY
               OB%BULK_SPECIFIC_HEAT = BULK_SPECIFIC_HEAT
               IF (ABS(OB%VOLUME_ADJUST)<TWO_EPSILON_EB .AND. OB%BULK_DENSITY>0._EB) THEN
                  OB%HT3D               = .FALSE.
                  OB%BULK_CONDUCTIVITY  = -1._EB
                  OB%BULK_DENSITY       = -1._EB
                  OB%BULK_SPECIFIC_HEAT = -1._EB
               ENDIF
               IF (OB%HT3D .AND. &
                  (OB%BULK_CONDUCTIVITY<0._EB .OR. OB%BULK_DENSITY<0._EB .OR. OB%BULK_SPECIFIC_HEAT<0._EB)) THEN
                  WRITE(MESSAGE,'(A,I5,A)')  'ERROR: Problem with OBST number',NN,&
                                             ', HT3D requires BULK_CONDUCTIVITY, BULK_DENSITY, BULK_SPECIFIC_HEAT.'
                  CALL SHUTDOWN(MESSAGE) ; RETURN
               ENDIF
