...
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: ASUM_ALL,ASUM_ALL_LOCAL
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZATION_IF: IF (CODE==0) THEN
      ...
      IF (NIC(NM,NOM)>0) THEN
         ALLOCATE(M3%REAL_RECV_PKG1(MAX(NIC(NM,NOM),NIC(NOM,NM))*(3+N_SPECIES)+1))
         ALLOCATE(M3%REAL_RECV_PKG2(MAX(NIC(NM,NOM),NIC(NOM,NM))*(8          )+1))
         ALLOCATE(M3%REAL_RECV_PKG3(MAX(NIC(NM,NOM),NIC(NOM,NM))*(3+N_SPECIES)+1))
...
RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
   ...
   INITIALIZE_SEND_IF: IF (CODE==0) THEN
      ...
      IF (NIC(NOM,NM)>0 .AND. RNODE/=SNODE) THEN
         ALLOCATE(M3%REAL_SEND_PKG1(MAX(NIC(NOM,NM),NIC(NM,NOM))*(3+N_SPECIES)+1))
         ALLOCATE(M3%REAL_SEND_PKG2(MAX(NIC(NOM,NM),NIC(NM,NOM))*(8          )+1))
         ALLOCATE(M3%REAL_SEND_PKG3(MAX(NIC(NOM,NM),NIC(NM,NOM))*(3+N_SPECIES)+1))
   ...
   IF (CODE==1 .AND. NIC(NOM,NM)>0) THEN
      IF (RNODE/=SNODE) THEN
         ...
         M3%REAL_SEND_PKG1(IWW*(3+N_SPECIES)+1) = -999.0_EB
         N_REQ=N_REQ+1
         CALL MPI_ISEND(M3%REAL_SEND_PKG1(1),IWW*(3+N_SPECIES)+1,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
   ...
   IF (CODE==4 .AND. NIC(NOM,NM)>0) THEN
      IF (RNODE/=SNODE) THEN
         ...
         M3%REAL_SEND_PKG3(IWW*(3+N_SPECIES)+1) = -999.0_EB
         N_REQ=N_REQ+1
         CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),IWW*(3+N_SPECIES)+1,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
         M2=>MESHES(NOM)%OMESH(NM)
