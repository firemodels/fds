...
DO N=1,N_TRACKED_SPECIES
   ...
   !$OMP PARALLEL DO PRIVATE(HDIFF) SCHEDULE(static)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
...
LIMITER_SELECT: SELECT CASE (FLUX_LIMITER)
   ...
   CASE DEFAULT LIMITER_SELECT
      ...
      !$OMP PARALLEL
      !$OMP DO PRIVATE(ZZZ) SCHEDULE(static)
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBM1
...
!$OMP PARALLEL DO PRIVATE(WC, II, JJ, KK, IIG, JJG, KKG, IOR, &
!$OMP& ZZZ, UN, DU_M, DU_P) &
!$OMP& SCHEDULE(static)
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   WC=>WALL(IW)
   IF (WC%BOUNDARY_TYPE==NULL_BOUNDARY) CYCLE WALL_LOOP
