...
IF (USE_OPENMP) THEN
   WRITE(LU_ERR,'(A,A,I3,A)') PNAME(1:PNAMELEN),' has ',OPENMP_AVAILABLE_THREADS,' threads available'
ENDIF
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
...
DO NM=1,NMESHES
   ...
   CALL MPI_SEND(MESHES(NM)%PRESSURE_ITERATIONS,     1,MPI_INTEGER,          0,1,MPI_COMM_WORLD,IERR)
ENDDO
...
DO NOM=1,NMESHES
   ...
   CALL MPI_RECV(MESHES(NOM)%PRESSURE_ITERATIONS,     1,MPI_INTEGER,         PROCESS(NOM),1,MPI_COMM_WORLD,STATUS,IERR)
ENDDO
...
IF_DUMP_HRR: IF (T>=HRR_CLOCK) THEN
   CALL MPI_ALLGATHERV(HRR_TIME_INTERVAL(DISP),CNT,MPI_DOUBLE_PRECISION,HRR_TIME_INTERVAL,COUNTS,DISPLS, &
                       MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
...
IF (N_TREES_OUT > 0 .AND. T > VEG_CLOCK) THEN
      ...
      CALL MPI_GATHERV(TREE_OUTPUT_DATA(1,1,DISPLS(MYID)+1),COUNTS_TREE(MYID),MPI_DOUBLE_PRECISION, &
...
IF_DUMP_MASS: IF (T>=MINT_CLOCK) THEN
   CALL MPI_ALLGATHERV(MINT_TIME_INTERVAL(DISP),CNT,MPI_DOUBLE_PRECISION,MINT_TIME_INTERVAL,COUNTS,DISPLS, &
                       MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
