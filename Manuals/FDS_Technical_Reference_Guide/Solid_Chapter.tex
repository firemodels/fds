...
\delta = 
   \begin{cases}
 	\max_{\alpha}\left(\frac{\rho_{s,\alpha}}{\rho_\alpha}\right) & \text{if }\max_{\alpha}\left(\frac{\rho_{s,\alpha}}{\rho_\alpha}\right) \geq 1 \\
 	\sum_{\alpha}\left(\frac{\rho_{s,\alpha}}{\rho_\alpha}\right) & \text{if }\max_{\alpha}\left(\frac{\rho_{s,\alpha}}{\rho_\alpha}\right) < 1 
   \end{cases}
\ee
...
The total aerosol deposition velocity to surfaces, $u_{dep}$, is determined by assuming the deposition
phenomena are independent, computing a deposition velocity for each mechanism, and then summing
them~\cite{Bixler:1}
\be
u_{dep} = u_g+u_{th}+u_{dt}
\ee
where u$_g$ is the gravitational settling velocity (for cells near upward-facing surfaces), u$_{th}$ is
the thermophoretic velocity, and u$_{dt}$ is the combined diffusion-turbulence velocity. If the aerosol
is located in a gas-phase cell adjacent to a wall, then the aerosol represented by species $\alpha$
is removed from the gas-phase and deposited onto the wall surface by the amount calculated by
\be
\Delta \rho_{\alpha,dep} = \rho_{\alpha} u_{dep} \frac{\Delta t}{\Delta x}
\ee
where $\Delta \rho_{\alpha,dep}$ is the amount of aerosol deposited, $\rho_{\alpha}$ is the ambient density
of the aerosol, $\Delta t$ is the size of the time step, and $\Delta x$ is the size of the grid cell.
The accumulated aerosol density that deposits to surfaces,
$m_{\alpha,dep}''$, is tracked by calculating
\be
m_{\alpha,dep}'' = m_{\alpha,dep}'' + \Delta \rho_{\alpha,dep} \Delta x
\ee
at each time step. Finally, the amount of aerosol that deposits to a surface is removed from
the gas-phase cell by calculating
\be
Y_{\alpha} = Y_{\alpha} - Y_{\alpha,dep}
\ee
where $Y_{\alpha}$ is the aerosol mass fraction, and $Y_{\alpha,dep}$ is the mass fraction of aerosol
that deposits in a given time step.
...
\be
u_g = g m_a \frac{\text{Cn}}{6 \pi \, \chi_d \, \mu \, r_a}
\ee

\noindent where $m_a$ is the particle mass, $\chi_d$ is a shape factor, $\mu$ is the dynamic viscosity of air,
...
\noindent where Kn is the particle Knudsen number given by the ratio of the mean free path of the gas
...
\be
\text{Kn} = \frac{\lambda}{r_a} \frac{T_g}{T_\infty}
\ee

\noindent where $\lambda$ is the mean free path of gas molecules and is 0.065 $\mu$m at a temperature of 25 $^\circ$C and atmospheric pressure.
...
\noindent This requires the wall temperature gradient, which is only resolved in a DNS simulation.
For an LES simulation, the temperature gradient is computed from the wall heat transfer coefficient.

\be
 \frac{\d T}{\d x} = \frac{h \left( T_g - T_w \right)}{k_g}
\ee

\noindent $K_{th}$ is the thermophoretic velocity coefficient and is calculated using the following correlation~\cite{Brock:1}

\be
 K_{th} = \frac{2 \, C_s \left(\alpha + C_t \; \text{Kn} \right) \; \text{Cn}}{\left(1 + 3 \, C_m \; \text{Kn} \right) \left(1 + 2 \, \alpha + 2 \, C_t \, \text{Kn} \right) }
\ee

\noindent where $C_s=1.17$ is the thermal slip coefficient, $\alpha$ is the ratio of the gas
...
\noindent where Sc is the particle Schmidt number, or the ratio of the kinematic viscosity to the
...
\be
 \tau^+=\frac{\rho_a \, (2 r_a)^2}{18 \, \mu^2}  \; u_{\tau}^2 \; \rho_g
\ee
