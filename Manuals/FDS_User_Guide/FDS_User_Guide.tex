...
\ee
where $t_1$ is the CPU time for the case with 1 mesh (MPI process), and $t_N$ is the CPU time for the case with $N$ meshes (MPI processes).
...
\ee
Figure~\ref{fig:scaling_tests} shows the results of a weak and strong scaling study. In the weak study, meshes with dimension 50 by 50 by 50 are lined up side by side, ranging from 1 to 288. The efficiency of the resulting simulations is plotted at left. In the strong study, a single mesh of dimension 180 by 160 by 80 is divided into a range of smaller meshes, with the smallest partitioning being 288 meshes of dimension 20 by 20 by 20. The resulting decrease in CPU time is shown in the plot. Ideally, the CPU time should be inversely proportional to the number of meshes (MPI processes).
\begin{figure}[!ht]
...
Additioally to the mesh assignment to individual MPI processes, the number of OpenMP threads for each MPI process may be specified. The parameter {\ct N\_THREADS} instructs FDS to set this number of threads and must be consistent for all meshes on the same MPI process, but may vary between them:
\begin{lstlisting}
&MESH ID='mesh1', IJK=..., XB=..., MPI_PROCESS=0 /
...
{\ct SOOT}               & 12.010700  & C$_{0.9}$H$_{0.1}$ & 3.798    & 71.4     &          &  {\ct SOOT}                \\ \hline
...
\be
   \Delta p =  \Delta x \; \left(\frac{\mu}{K} u + \rho \frac{Y}{\sqrt{K}} u^2 \right)
\ee
