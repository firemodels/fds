...
CHARACTER(30) :: ID,SHAPE,TFILE,SURF_ID
...
READ_GEOM_LOOP: DO NG=1,N_GEOM
   ...
   IF (TRIM(G%BNDC_FILENAME)/='null') THEN
      DO N=1,N_SURF
         SF => SURFACE(N)
         IF (TRIM(SF%ID) == TRIM(G%SURF_ID)) SF%THERMAL_BC_INDEX = SPECIFIED_TEMPERATURE_FROM_FILE
      ENDDO
   ENDIF
...
REAL(EB) :: BB(6),V1(3),V2(3),V3(3),AREA,PC(18),XPC(27),V_POLYGON_CENTROID(3),VC
LOGICAL :: EX,OP
CHARACTER(60) :: FN
...
GEOM_LOOP: DO N=1,N_GEOM
   ...
   IF (ABS(T-T_BEGIN)<ZERO_P .AND. LU_GEOC<0) THEN
      ...
      READ(LU_GEOC) DUMMY_INTEGER ! one
      READ(LU_GEOC) DUMMY_INTEGER ! version
      READ(LU_GEOC) DUMMY_INTEGER ! n_floats
      IF (DUMMY_INTEGER>0) READ(LU_GEOC) (DUMMY_FB_REAL, I=1,DUMMY_INTEGER)
      READ(LU_GEOC) DUMMY_INTEGER ! n_ints
      IF (DUMMY_INTEGER>0) READ(LU_GEOC) (DUMMY_INTEGER2, I=1,DUMMY_INTEGER)

      READ(LU_GEOC) DUMMY_INTEGER, DUMMY_INTEGER2 ! n_vert_s, n_face_s
      
   ENDIF

   READ(LU_GEOC) DUMMY_FB_REAL,DUMMY_INTEGER ! stime,gem_type

   IF (T>=REAL(DUMMY_FB_REAL,EB)) THEN
      SELECT CASE(DUMMY_INTEGER)
         CASE(0)
            READ(LU_GEOC) DUMMY_INTEGER2,DUMMY_INTEGER3 ! n_vert_d,n_face_d
            ...
            IF (DUMMY_INTEGER2>0) READ(LU_GEOC) ((FB_REAL_VERT_ARRAY((I-1)*3+J),J=1,3),I=1,DUMMY_INTEGER2)
...
!                IF (T>30 .AND. T<60) VERTEX(I)%Y = VERTEX(I)%Y-0.05
            ENDDO
            IF (DUMMY_INTEGER3>0) READ(LU_GEOC) ((INT_FACE_VALS_ARRAY((I-1)*3+J),J=1,3),I=1,DUMMY_INTEGER3)
            IF (DUMMY_INTEGER3>0) READ(LU_GEOC) (INT_SURF_VALS_ARRAY(I),I=1,DUMMY_INTEGER3)
            ! assuming the surface triangles have the same vertices
         CASE(1)
            READ(LU_GEOC) DUMMY_FB_REAL2,DUMMY_FB_REAL2,DUMMY_FB_REAL2,DUMMY_FB_REAL2,&
...
! Set up any face parameters related to cut cells
...
DO N=1,N_FACE
   ...
   CUTCELL_LOOP: DO
      ...
      FC%RDN = FC%RDN + CL%AREA*VC

      CL=>CL%NEXT ! point to the next index in the linked list
   ENDDO CUTCELL_LOOP

   IF (FC%RDN /= 0._EB) FC%RDN = 2._EB/(FC%RDN/FC%AW)**ONTH ! reciprocal of wall normal length scale

ENDDO

! Read boundary condition from file

GEOM_BNDC_LOOP: DO N=1,N_GEOM
   IF (TRIM(GEOMETRY(N)%BNDC_FILENAME)=='null') CYCLE
   
   IF (ABS(T-T_BEGIN)<ZERO_P .AND. LU_BNDC<0) THEN
      ...
      READ(LU_BNDC) DUMMY_INTEGER ! one
      READ(LU_BNDC) DUMMY_INTEGER ! version
   ...
   READ(LU_BNDC) DUMMY_FB_REAL ! stime

   IF (T>=REAL(DUMMY_FB_REAL,EB)) THEN
      ! n_vert_s_vals,n_vert_d_vals,n_face_s_vals,n_face_d_vals
      READ(LU_BNDC) DUMMY_INTEGER,DUMMY_INTEGER,DUMMY_INTEGER,DUMMY_INTEGER
...
REAL(EB), INTENT(IN) :: V1(3),V2(3),V3(3),BB(6)
REAL(EB) :: P0(3),P1(3),Q(3),PC_TMP(27)
