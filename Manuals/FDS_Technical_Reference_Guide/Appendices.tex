...
\be
\Delta t = \frac{\Delta}{\Delta_{ref}}\frac{\bar{\dq}''_{ref}}{\dq''}\Delta t_{ref}
...
\begin{figure}
	...
	\end{tabular*}
	\caption[Empirical flame heat flux for Spyro]{(Left) Normalized flame heat flux as a function of HRRPUA, (Right) Maximum flame heat flux as a function of .}
	\label{fig:Spyro_hoc}
\end{figure}

Various simple combinations of the heat of combustion, soot yield, and HRRPUA were evaluated to determine a relationship for $\Gamma$. The functional form of $A + B \hbox{HRRPUA} / \Delta H_c + C \hbox{soot yield}$ gave a fit with the lowest RMS value. To determine the time-dependent $\dq_{ref}''$ for a cone test first the maximum $\dq_{flame}''$ is determined using $\Delta H_c$ at the fit shown in Fig~\ref{fig:Spyro_hoc}, second  $\dq_{flame}''$ is adjusted for the current HRRPUA by interpolating the average normalized curve in Fig~\ref{fig:Spyro_hoc}, thirdly $\Gamma$ is determined and used to adjust  $\dq_{cone}''$, and finally $\dq_{ref}''$ is determined via Eq.~\ref{Spyro7}. The resulting fit has a 2~\% error compared to the FDS predictions, see Fig.~\ref{fig:Spyro_cone_error}.
