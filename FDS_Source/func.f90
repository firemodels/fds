...
SUBROUTINE OPENMP_PRINT_STATUS
  ...
  !$OMP CRITICAL
  IF (USE_OPENMP .AND. USE_MPI .AND. OPENMP_USED_THREADS>1) WRITE(LU_ERR,91) " OpenMP thread ",THREAD_ID," of ",&
  ...
  IF (USE_OPENMP .AND. .NOT.USE_MPI .AND. OPENMP_USED_THREADS>1) WRITE(LU_ERR,93) " OpenMP thread ",THREAD_ID," of ",&
     OPENMP_USED_THREADS-1," is running"
  !$OMP END CRITICAL
