...
IF (WRITE_VTK) THEN
   ...
   IF (VTK_HDF) THEN
      ...
      IF (T>=BNDF_VTK_CLOCK(BNDF_VTK_COUNTER(NM))) THEN
         WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " INTO DUMP_BNDF_VTKHDF"
         CALL DUMP_BNDF_VTKHDF(T,DT,NM,FAKEWRITE)
         WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " OUT OF DUMP_BNDF_VTKHDF"
      ENDIF
   ...
   IF (VTK_OUT) THEN
      ...
      IF (S3%DISPLAY_TYPE=='GAS') THEN
         ...
         DO I=1,NP
            ...
            QQ_PACK(I) = VAL_SMV
            !QQ_PACK_INT(I) = INT(NINT(VAL_SMV),IB8)
            !WRITE(*,*) FACTOR, QQ_PACK(I), VAL_FDS, VAL_SMV, QQ_PACK_INT(I)
         ENDDO
...
INTEGER(HID_T) :: DSET_ID_NCELLS, DSET_ID_NPTS, DSET_ID    ! Dataset identifiers
REAL(EB), INTENT(IN) :: T,DT
LOGICAL, INTENT(IN) :: FAKEWRITE
REAL(FB) :: STIME
...
MESH_LOOP_HDF: DO NMNM=1,NMESHES
   ...
   DATA_FILE_LOOP: DO NF=1,N_BNDF
      ...
      BF => BOUNDARY_FILE(NF)
      WRITE(*,*) "RANK", MY_RANK, "MESH", NM, "NF", NF, "OCS", NCELLS_START
      ! If parallel write called with empty data, write empty
      !IF(FAKEWRITE) THEN
      !   WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " N ", N, " START "
      !   ALLOCATE(QQ_PACK(0))
      !   CALL WRITE_VTKHDF_BNDF_CELL_FILE(FILENAME,NM,NCELLS,NPOINTS,NCONNECTIONS,.TRUE.)
      !   !CALL ADD_DATA_TO_SMOKE3D_VTKHDF_FILE(FILENAME, S3%SMOKEVIEW_LABEL(1:30), QQ_PACK, NM, .TRUE.) !REAL(QQ_PACK, FB))
      !   DEALLOCATE(QQ_PACK)
      !   WRITE(*,*) "RANK ", MY_RANK, " MESH ", NM, " N ", N
      !   CYCLE DATA_FILE_LOOP
      ...
      IF ((MESHES(NM)%N_PATCH>0).AND.(.NOT.FAKEWRITE)) THEN
         ...
         IF (BF%CELL_CENTERED) THEN
            WRITE(*,*) "RANK", MY_RANK, "MESH", NM, "NF", NF, "OCS", NCELLS_START, "OCE", NCELLS_START+NCELLS(NM1)
         ...
         ELSE
            WRITE(*,*) "RANK", MY_RANK, "MESH", NM, "NF", NF, "OPS", NPOINTS_START, "OPE", NPOINTS_START+NPOINTS(NM1)
      ...
      IF ((MESHES(NM)%N_INTERNAL_CFACE_CELLS>0).AND.(.NOT.FAKEWRITE)) THEN
         ...
         IF (NVERTS>0 .AND. NFACES>0) THEN
            ...
            ! Write ALL_DATA to file
            IF (BF%CELL_CENTERED) THEN
               WRITE(*,*) "RANK", MY_RANK, "MESH", NM, "NF", NF, "GCS", NCELLS_ACCUM, "GCE", NCELLS_ACCUM+NCELLS(NM2)
               CALL PARALLEL_INIT_F32(GROUP_ID2, BF%SMOKEVIEW_LABEL(1:30), CRP_LIST, 1,&
                  INT8((/0/)),INT8((/0/)), DSET_ID, PLIST_ID) ! Open previously initialized cell centered for writing
               CALL PARALLEL_WRITE_F32(1, INT8((/NCELLS(NM2)/)), DSET_ID,&
                  PLIST_ID, INT8((/NCELLS_ACCUM/)), INT8((/NCELLS(NM2)/)), ALL_DATA)
            ELSE
               WRITE(*,*) "RANK", MY_RANK, "MESH", NM, "NF", NF, "GPS", NPOINTS_ACCUM, "GPE", NPOINTS_ACCUM+NPOINTS(NM2)
               CALL PARALLEL_INIT_F32(GROUP_ID4, BF%SMOKEVIEW_LABEL(1:30), CRP_LIST, 1,&
                  INT8((/0/)),INT8((/0/)), DSET_ID, PLIST_ID) ! Open previously initialized nodal for writing
