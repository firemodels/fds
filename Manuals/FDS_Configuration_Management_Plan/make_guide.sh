...
if [[ `grep -E "Error:|Fatal error|! LaTeX Error:" -I FDS_Configuration_Management_Plan.err` == "" ]]
   then
      # Continue along
      :
   else
      echo "LaTeX errors detected:"
      grep -E "Error:|Fatal error|! LaTeX Error:" -I FDS_Configuration_Management_Plan.err
      clean_build=0
fi
