...
NAMELIST /HESKESTAD/ T,Q,Z,A_C,TMP_A,RHO_AIR,RADIATIVE_FRACTION,OUTPUT_FILE,Z_LABEL,HEADER
...
DO I=1,N_Z
   ...
   T_PLUME(I) = 9.1 * ((TMP_A+273)/(G*(C_P**2.)*(RHO_AIR)**2.))**(1./3.) * (Q_C)**(2./3.) * (Z(I)-Z_0)**(-5./3.) + (TMP_A+273)

ENDDO

WRITE(FMT,'(A,I1.1,5A)') "(",N_Z,"(","F7.2",",','),","F7.2",")"
WRITE(11,FMT) T, (T_PLUME(K)-273,K=1,N_Z)
