...
ZONE_LEAK_IF: IF (ALL(SF%LEAK_PATH < 0)) THEN
   ...
   IF (HVAC_LOCAL_PRESSURE) THEN
      ...
      END SELECT
      NODE_P(NODE_INDEX) = NODE_P(NODE_INDEX) + (P_AVE-101325._EB+RHO(II,JJ,KK)*(HP(II,JJ,KK)-KRES(II,JJ,KK)))*AREA
   ELSE
      SELECT CASE (IOR)
         CASE (1)
            NODE_P(NODE_INDEX) = NODE_P(NODE_INDEX) + &
                                       (PBARP(KK,B1%PRESSURE_ZONE)-101325._EB-RHO(II,JJ,KK) * &
...
ELSE ZONE_LEAK_IF
   ...
   SELECT CASE (IOR)
      CASE (3)
         P_AVE = 0.5_EB*(PBARP(KK-1,B1%PRESSURE_ZONE)+PBARP(KK,B1%PRESSURE_ZONE))-101325._EB
         NODE_Z(NODE_INDEX) = NODE_Z(NODE_INDEX) + Z(KK-1)*AREA
      CASE (-3)
         P_AVE = 0.5_EB*(PBARP(KK,B1%PRESSURE_ZONE)+PBARP(KK+1,B1%PRESSURE_ZONE))-101325._EB
         NODE_Z(NODE_INDEX) = NODE_Z(NODE_INDEX) + Z(KK)*AREA
      CASE DEFAULT
         P_AVE = PBARP(KK,B1%PRESSURE_ZONE)-101325._EB
         NODE_Z(NODE_INDEX) = NODE_Z(NODE_INDEX) + ZC(KK)*AREA
   END SELECT
...
VENT_CUSTOM_AMBIENT: DO NN=1,N_DUCTNODES
   ...
   INTERNAL_NODE_IF: IF (((DN%VENT .OR. DN%LEAKAGE) .AND. .NOT. DN%AMBIENT) .OR. &
      ...
      IF (DN%LEAKAGE) THEN
         IF(ABS(T-T_BEGIN) < TWO_EPSILON_EB) DN%P_OLD = NODE_P(NN)/AREA+101325._EB
      ENDIF

      DN%P = HVAC_PRES_RELAX*(NODE_P(NN)/AREA+101325._EB)+(1._EB-HVAC_PRES_RELAX)*DN%P_OLD
...
DUCT_LOOP: DO ND = 1, DR%N_M_DUCTS
   ...
   IF (DN%VENT .AND. HVAC_LOCAL_PRESSURE) LHS(ND,ND) = LHS(ND,ND)-DN%DIR(1)*DU%AREA/(DU%LENGTH*GAMMA*NODE_AREA_EX(DU%NODE_INDEX(1)))
   DN=>DUCTNODE(DU%NODE_INDEX(2))
   IF (DN%DUCTRUN_M_INDEX > 0) LHS(ND,DR%N_M_DUCTS + DN%DUCTRUN_M_INDEX) =  DT_QF/(DR%RHO_D(ND,NF)*DU%LENGTH)
   IF (DN%VENT .AND. HVAC_LOCAL_PRESSURE) LHS(ND,ND) = LHS(ND,ND)+DN%DIR(1)*DU%AREA/(DU%LENGTH*GAMMA*NODE_AREA_EX(DU%NODE_INDEX(2)))
ENDDO DUCT_LOOP
