...
MESH_LOOP_1A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_1B: DO NOM=1,NMESHES
      ...
      DO I=1,M3%N_WALL_CELLS_SEND
         ...
         OS%N_REALS_DIM    = OS%N_REALS_DIM    + M3%WALL_REAL_NUMBERS_SEND(I)
         OS%N_INTEGERS_DIM = OS%N_INTEGERS_DIM + M3%WALL_INTE_NUMBERS_SEND(I)
         OS%N_LOGICALS_DIM = OS%N_LOGICALS_DIM + M3%WALL_LOGI_NUMBERS_SEND(I)
      ENDDO
      ALLOCATE(OS%REALS(OS%N_REALS_DIM))
      ALLOCATE(OS%INTEGERS(OS%N_INTEGERS_DIM))
      ALLOCATE(OS%LOGICALS(OS%N_LOGICALS_DIM))
...
MESH_LOOP_3A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_3B: DO NOM=1,NMESHES
      ...
      DO I=1,M3%N_THIN_WALL_CELLS_SEND
         ...
         OS%N_REALS_DIM    = OS%N_REALS_DIM    + M3%THIN_WALL_REAL_NUMBERS_SEND(I)
         OS%N_INTEGERS_DIM = OS%N_INTEGERS_DIM + M3%THIN_WALL_INTE_NUMBERS_SEND(I)
         OS%N_LOGICALS_DIM = OS%N_LOGICALS_DIM + M3%THIN_WALL_LOGI_NUMBERS_SEND(I)
      ENDDO
      ALLOCATE(OS%REALS(OS%N_REALS_DIM))
      ALLOCATE(OS%INTEGERS(OS%N_INTEGERS_DIM))
      ALLOCATE(OS%LOGICALS(OS%N_LOGICALS_DIM))
...
MESH_LOOP_2A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_2B: DO NOM=1,NMESHES
      ...
      DO I=1,M3%N_WALL_CELLS_RECV
         OS%N_REALS_DIM    = OS%N_REALS_DIM    + M3%WALL_REAL_NUMBERS_RECV(I)
         OS%N_INTEGERS_DIM = OS%N_INTEGERS_DIM + M3%WALL_INTE_NUMBERS_RECV(I)
         OS%N_LOGICALS_DIM = OS%N_LOGICALS_DIM + M3%WALL_LOGI_NUMBERS_RECV(I)
      ENDDO
      ALLOCATE(OS%REALS(OS%N_REALS_DIM))
      ALLOCATE(OS%INTEGERS(OS%N_INTEGERS_DIM))
      ALLOCATE(OS%LOGICALS(OS%N_LOGICALS_DIM))
...
MESH_LOOP_4A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   MESH_LOOP_4B: DO NOM=1,NMESHES
      ...
      DO I=1,M3%N_THIN_WALL_CELLS_RECV
         OS%N_REALS_DIM    = OS%N_REALS_DIM    + M3%THIN_WALL_REAL_NUMBERS_RECV(I)
         OS%N_INTEGERS_DIM = OS%N_INTEGERS_DIM + M3%THIN_WALL_INTE_NUMBERS_RECV(I)
         OS%N_LOGICALS_DIM = OS%N_LOGICALS_DIM + M3%THIN_WALL_LOGI_NUMBERS_RECV(I)
      ENDDO
      ALLOCATE(OS%REALS(OS%N_REALS_DIM))
      ALLOCATE(OS%INTEGERS(OS%N_INTEGERS_DIM))
      ALLOCATE(OS%LOGICALS(OS%N_LOGICALS_DIM))
   ...
   SENDING_MESH_LOOP: DO NN=1,M%N_NEIGHBORING_MESHES
      ...
      INITIALIZATION_IF: IF (CODE==0) THEN
         ...
         IF (OMESH_PARTICLES) THEN
            ...
            N_REQ2=N_REQ2+1 ; CALL MPI_RECV_INIT(M3%N_PARTICLES_RECV,1,MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ2(N_REQ2),IERR)
      ...
      IF (CODE==8) THEN
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%N_WALL_CELLS_SEND,1,MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%N_THIN_WALL_CELLS_SEND,1,MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==9 .AND. M3%N_WALL_CELLS_SEND>0) THEN
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%WALL_CELL_INDICES_SEND(1),SIZE(M3%WALL_CELL_INDICES_SEND),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%WALL_SURF_INDICES_SEND(1),SIZE(M3%WALL_SURF_INDICES_SEND),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==9 .AND. M3%N_THIN_WALL_CELLS_SEND>0) THEN
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%THIN_WALL_CELL_INDICES_SEND(1),SIZE(M3%THIN_WALL_CELL_INDICES_SEND),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%THIN_WALL_SURF_INDICES_SEND(1),SIZE(M3%THIN_WALL_SURF_INDICES_SEND),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      ...
      IF (CODE==19 .AND. M3%N_WALL_CELLS_RECV>0) THEN
         ...
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%WALL_REAL_NUMBERS_RECV(1),SIZE(M3%WALL_REAL_NUMBERS_RECV),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%WALL_INTE_NUMBERS_RECV(1),SIZE(M3%WALL_INTE_NUMBERS_RECV),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%WALL_LOGI_NUMBERS_RECV(1),SIZE(M3%WALL_LOGI_NUMBERS_RECV),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      ...
      IF (CODE==19 .AND. M3%N_THIN_WALL_CELLS_RECV>0) THEN
         ...
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%THIN_WALL_REAL_NUMBERS_RECV(1),SIZE(M3%THIN_WALL_REAL_NUMBERS_RECV),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%THIN_WALL_INTE_NUMBERS_RECV(1),SIZE(M3%THIN_WALL_INTE_NUMBERS_RECV),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1 ; CALL MPI_IRECV(M3%THIN_WALL_LOGI_NUMBERS_RECV(1),SIZE(M3%THIN_WALL_LOGI_NUMBERS_RECV),MPI_INTEGER,&
                                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF
      ...
      IF (CODE==11 .AND. OMESH_PARTICLES .AND. M3%N_PARTICLES_RECV>0) THEN
         OS => M3%PARTICLE_RECV_BUFFER
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      INITIALIZE_SEND_IF: IF (CODE==0) THEN
         ...
         IF (OMESH_PARTICLES) THEN
            ...
            N_REQ2=N_REQ2+1 ; CALL MPI_SEND_INIT(M3%N_PARTICLES_SEND,1,MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ2(N_REQ2),IERR)
      ...
      IF (CODE==10 .AND. M3%N_WALL_CELLS_SEND>0 .AND. RNODE/=SNODE) THEN
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%WALL_SEND_BUFFER%REALS,SIZE(M3%WALL_SEND_BUFFER%REALS),&
                            MPI_DOUBLE_PRECISION,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%WALL_SEND_BUFFER%INTEGERS,SIZE(M3%WALL_SEND_BUFFER%INTEGERS),&
                            MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%WALL_SEND_BUFFER%LOGICALS,SIZE(M3%WALL_SEND_BUFFER%LOGICALS),&
                            MPI_LOGICAL,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
      ENDIF

      IF (CODE==10 .AND. M3%N_THIN_WALL_CELLS_SEND>0 .AND. RNODE/=SNODE) THEN
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%THIN_WALL_SEND_BUFFER%REALS,SIZE(M3%THIN_WALL_SEND_BUFFER%REALS),&
                            MPI_DOUBLE_PRECISION,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%THIN_WALL_SEND_BUFFER%INTEGERS,SIZE(M3%THIN_WALL_SEND_BUFFER%INTEGERS),&
                            MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         N_REQ6 = N_REQ6 + 1
         CALL MPI_SEND_INIT(M3%THIN_WALL_SEND_BUFFER%LOGICALS,SIZE(M3%THIN_WALL_SEND_BUFFER%LOGICALS),&
                            MPI_LOGICAL,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
      ENDIF
      ...
      IF (CODE==7 .AND. OMESH_PARTICLES .AND. RNODE==SNODE) THEN
         ...
         M2%N_PARTICLES_RECV = M3%N_PARTICLES_SEND
      ...
      IF_SEND_PARTICLES: IF (CODE==11 .AND. OMESH_PARTICLES .AND. M3%N_PARTICLES_SEND>0) THEN
         OS => M3%PARTICLE_SEND_BUFFER
         IF (SNODE/=RNODE) THEN
...
RECV_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (CODE==7 .AND. OMESH_PARTICLES .AND. M2%N_PARTICLES_RECV>0) THEN
         OS => M2%PARTICLE_RECV_BUFFER
      ...
      IF_RECEIVE_PARTICLES: IF (CODE==11 .AND. OMESH_PARTICLES .AND. M2%N_PARTICLES_RECV>0) THEN
         RC=0 ; IC=0 ; LC=0
         OS => M2%PARTICLE_RECV_BUFFER
         DO IP=M%NLP+1,M%NLP+M2%N_PARTICLES_RECV
            ...
            ENDIF
         ENDDO
         M%NLP = M%NLP + M2%N_PARTICLES_RECV
         M2%N_PARTICLES_RECV = 0
