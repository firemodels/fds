...
ALLOCATE(T_USED(N_TIMERS)) ; T_USED = 0._EB

! Assign a compilation date (All Nodes)

WRITE(VERSION_STRING,'(A)') 'FDS 6.3.2'
...
DO NM=1,NMESHES
   IF (PROCESS(NM)==MYID) CALL INITIALIZE_MESH_VARIABLES_1(DT,NM)
ENDDO
CALL STOP_CHECK(1)
...
DO NM=1,NMESHES
   IF (PROCESS(NM)/=MYID .OR. TGA_SURF_INDEX>0 .OR. EVACUATION_ONLY(NM)) CYCLE
...
DO NM=1,NMESHES
   IF (RESTART .AND. PROCESS(NM)==MYID) CALL READ_RESTART(T,DT,NM)
ENDDO
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
...
IF (.NOT.RESTART) THEN
   DO NM=1,NMESHES
      IF (PROCESS(NM)==MYID) CALL OPEN_AND_CLOSE(T,NM)
   ENDDO
ENDIF
...
MAIN_LOOP: DO
   ...
   COMPUTE_FINITE_DIFFERENCES_1: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE COMPUTE_FINITE_DIFFERENCES_1
      CALL INSERT_PARTICLES(T,NM)
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      COMPUTE_DIVERGENCE_LOOP: DO NM=1,NMESHES
         IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE COMPUTE_DIVERGENCE_LOOP
      ...
      COMPUTE_WALL_BC_LOOP_A: DO NM=1,NMESHES
         IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE COMPUTE_WALL_BC_LOOP_A
      ...
      FINISH_DIVERGENCE_LOOP: DO NM=1,NMESHES
         IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE FINISH_DIVERGENCE_LOOP
      ...
      PREDICT_VELOCITY_LOOP: DO NM=1,NMESHES
         ...
         IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE PREDICT_VELOCITY_LOOP
   ...
   DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE
      CALL MATCH_VELOCITY(NM)
   ENDDO
   ...
   VELOCITY_BC_LOOP: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE VELOCITY_BC_LOOP
   ...
   COMPUTE_DIVERGENCE_2: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE COMPUTE_DIVERGENCE_2
      CALL COMPUTE_VELOCITY_FLUX(T,DT,NM,2)
      IF (AGGLOMERATION_INDEX>0) CALL CALC_AGGLOMERATION(DT,NM)
   ...
   COMPUTE_WALL_BC_2A: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE COMPUTE_WALL_BC_2A
      CALL UPDATE_PARTICLES(T,DT,NM)
      CALL WALL_BC(T,DT,NM)
      CALL PARTICLE_MOMENTUM_TRANSFER(NM)
   ...
   FINISH_DIVERGENCE_LOOP_2: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE FINISH_DIVERGENCE_LOOP_2
      CALL DIVERGENCE_PART_2(DT,NM)
   ...
   CORRECT_VELOCITY_LOOP: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM))    CYCLE CORRECT_VELOCITY_LOOP
   ...
   VELOCITY_BC_LOOP_2: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. EVACUATION_SKIP(NM)) CYCLE VELOCITY_BC_LOOP_2
      CALL VELOCITY_BC(T,NM)
      CALL UPDATE_GLOBAL_OUTPUTS(T,DT,NM)
...
DO NM=1,NMESHES
   ...
   IF (PROCESS(NM)==MYID) MESHES(NM)%WALL_WORK1 = 0._EB
   IF (PROCESS(NM)==MYID) CALL MATCH_VELOCITY_FLUX(NM)
ENDDO
...
PRESSURE_ITERATION_LOOP: DO
   ...
   DO NM=1,NMESHES
      IF (EVACUATION_ONLY(NM) .OR. EVACUATION_SKIP(NM)) CYCLE
      IF (PROCESS(NM)==MYID) CALL COMPUTE_VELOCITY_ERROR(DT,NM)
   ENDDO
...
IF_DUMP_HRR: IF (T>=HRR_CLOCK) THEN
   IF (T-T_LAST_DUMP_HRR>0._EB) THEN
      IF (N_MPI_PROCESSES>1) THEN
...
IF_DUMP_MASS: IF (T>=MINT_CLOCK) THEN
   IF (T-T_LAST_DUMP_MASS>0._EB) THEN
      IF (N_MPI_PROCESSES>1) THEN
