...
LOGICAL :: POSSIBLY_PERIODIC,CYLINDRICAL_OLD_VALUE,PERIODIC_FOUND_IN_FILE
REAL(EB) :: XB1,XB2,XB3,XB4,XB5,XB6,OVERLAP_X,OVERLAP_Y,OVERLAP_Z
CHARACTER(25) :: COLOR
...
DO NM=1,NMESHES
   ...
   DO NM2=1,NMESHES
      ...
      OVERLAP_X = MIN(M%XF,M2%XF) - MAX(M%XS,M2%XS)
      OVERLAP_Y = MIN(M%YF,M2%YF) - MAX(M%YS,M2%YS)
      OVERLAP_Z = MIN(M%ZF,M2%ZF) - MAX(M%ZS,M2%ZS)
      IF (((ABS(M2%XS-XS_MIN)<MESH_SEPARATION_DISTANCE .AND. ABS( M%XF-XF_MAX)<MESH_SEPARATION_DISTANCE)  .OR.  &
          ...
          OVERLAP_Y>CHARACTERISTIC_CELL_SIZE .AND. OVERLAP_Z>CHARACTERISTIC_CELL_SIZE) POSSIBLY_PERIODIC = .TRUE.
      IF (((ABS(M2%YS-YS_MIN)<MESH_SEPARATION_DISTANCE .AND. ABS( M%YF-YF_MAX)<MESH_SEPARATION_DISTANCE)  .OR.  &
          ...
          OVERLAP_X>CHARACTERISTIC_CELL_SIZE .AND. OVERLAP_Z>CHARACTERISTIC_CELL_SIZE) POSSIBLY_PERIODIC = .TRUE.
      IF (((ABS(M2%ZS-ZS_MIN)<MESH_SEPARATION_DISTANCE .AND. ABS( M%ZF-ZF_MAX)<MESH_SEPARATION_DISTANCE)  .OR.  &
          ...
          OVERLAP_X>CHARACTERISTIC_CELL_SIZE .AND. OVERLAP_Y>CHARACTERISTIC_CELL_SIZE) POSSIBLY_PERIODIC = .TRUE.
      IF (.NOT.PERIODIC_FOUND_IN_FILE) POSSIBLY_PERIODIC = .FALSE.
      IF ((OVERLAP_X>=0._EB .AND. OVERLAP_Y>CHARACTERISTIC_CELL_SIZE .AND. OVERLAP_Z>CHARACTERISTIC_CELL_SIZE) .OR. &
          (OVERLAP_Y>=0._EB .AND. OVERLAP_X>CHARACTERISTIC_CELL_SIZE .AND. OVERLAP_Z>CHARACTERISTIC_CELL_SIZE) .OR. &
          (OVERLAP_Z>=0._EB .AND. OVERLAP_X>CHARACTERISTIC_CELL_SIZE .AND. OVERLAP_Y>CHARACTERISTIC_CELL_SIZE) .OR. &
          POSSIBLY_PERIODIC) THEN
...
NAMELIST /PRES/ BAROCLINIC,CHECK_POISSON,FISHPAK_BC,ITERATION_SUSPEND_FACTOR,INSEPARABLE_POISSON, &
                ...
                SCARC_PRECON, SCARC_PRECON_SCOPE, SCARC_PRECON_ITERATIONS, SCARC_PRECON_ACCURACY, SCARC_PRECON_OMEGA, &
                SCARC_SMOOTH, SCARC_SMOOTH_SCOPE, SCARC_SMOOTH_ITERATIONS, SCARC_SMOOTH_ACCURACY, SCARC_SMOOTH_OMEGA, &
                MAX_PRESSURE_ITERATIONS,PRESSURE_RELAX_TIME,PRESSURE_TOLERANCE,POISSON_ITERATION_ACCELERATOR,POISSON_PATCH_WIDTH, &
