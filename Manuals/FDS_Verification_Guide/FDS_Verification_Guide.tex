...
In this example, the initial gas-phase soot concentration is specified, and the only active deposition mechanism (i.e., soot removal term) is gravitational deposition. A simple HVAC setup (one supply vent and one exhaust vent) is used to stir the air and create well-mixed conditions. The transient gas-phase soot concentration for a well-mixed compartment can then be represented as

\be
\frac{\d C}{\d t} = -\frac{A}{V} (V_d C)
\label{eq:gravitational_settling}
\ee

where $C$ is the soot concentration (mg/m$^3$), $A/V$ is the surface area to volume ratio (m$^{-1}$), and $V_d$ is the gravitational settling velocity (m/s). Equation~\ref{eq:gravitational_settling} has an analytical solution for the dynamic gas-phase soot concentration of the form
\be
C(t) = C_0 \exp \left[-\frac{A}{V}(V_d t)\right]
\ee

where $C_0$ is the initial gas-phase soot concentration (100~mg/m$^3$). In this example, the relevant surface area is the area of the lower surface where the soot deposits, and thus the surface area to volume ratio ($A/V$) is equal to 1~m$^{-1}$. For a soot particle with a diameter of 1~$\mu$m, the terminal settling velocity is equal to 1.9747$\times 10^{-4}$~m/s. The governing equations for the gravitational settling algorithm and settling velocity are discussed in more detail in the FDS Technical Reference Guide~\cite{FDS_Math_Guide}. Figure~\ref{fig:deposition_mass} shows a comparison of the analytical and simulated results for the time evolution of the gas-phase soot concentration.

\begin{figure}[ht]
\begin{center}
\begin{tabular}{c}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/soot_gravitational_settling}
\end{tabular}
\end{center}
...
\section{Heat Conduction through Insulated Steel (\texorpdfstring{{\bf insulated\_steel\_plate}}{insulated\_steel\_plate})}

In this example, a 1~cm thick steel plate is coated on each side with 2~cm thick insulation material. The steel and insulation have constant thermal properties and the heat transfer coefficient is fixed at 10~W/m$^2$/K. One side of the plate is exposed to a 480~$^\circ$C gas while the other side is exposed to ambient temperature (20~$^\circ$C). The plate is heated for 10~h and the steady-state temperature profile is shown in Fig.~\ref{insulated_steel_plate}. The heat flux through the plate is given by:
\be 
   \dot{q}'' = \frac{ T_{\hbox{\tiny hot}} - T_{\hbox{\tiny cold}} }{ 2/h + \sum_{i=1}^3 L_i/k_i } 
\ee
where $h$ is the heat transfer coefficient, $L_i$ is the layer thickness, and $k_i$ is the thermal conductivity of the layer. 
The heat flux through each layer is the same, and the steady-state solution can be obtained by solving for the temperature drop across each. Note that the position of the symbols in Fig.~\ref{insulated_steel_plate} indicates the default finite difference noding of the solid.
...
\end{center}
\caption[The {\bf insulated\_steel\_plate} test case]{Steady-state temperature profile within an insulated steel plate.}
