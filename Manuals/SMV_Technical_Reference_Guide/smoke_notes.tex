...
mesh to optical properties color and opacity.
\item
Forming a 2D projected image by casting a ray, one ray for each pixel
...
This can be implemented \cite[Chapter 39]{gpugems} using the forward recursion
...
are the color and opacity at the $i$'th step.  A general transfer function is used to map data values to color and opacity. To produce a realistic image, the strategy is to perform this mixing in the same way that light would behave by solving an approximate form of the radiation transport equation.  The solution
...
The RTE solution in (\ref{eq:rtesoln}) and the optical depth in (\ref{eq:optdepth}) may be approximated using a Riemann sum.  Figure \ref{fig:smokediscretesetup}\ illustrates one of possibly many paths through smoke for which a radiance line integral computation is required.  The coordinate system is set up so that the origin, $x_0$, is on right where radiance presumably has its full value.  The observer in this example is located on the left at $x_N$  where radiance has is computed and displayed.  
...
where $\tau_j=\exp\left(-\sigma_t(s_j)\Delta s\right)$ represents the transparency over one interval.
...
where $\sigma_{a,i}=\sigma_t(x_i)$, $\Lei{i}=L_e(x_i)$, $x_i=i\Delta x$ and $\Delta x=D/N$ and $\htau{i}$ is obtained from (\ref{eq:tauhat_discrete}).
...
Therefore, to compute $\hat{L}_0$ recursively,
let $\hat{L}_j$ and $\hat{L}_{j-1}$ be intermediate results at nodes $j$ and $j-1$ given by
