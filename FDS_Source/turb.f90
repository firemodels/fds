...
CALL TEST_FILTER(BETAHAT11,BETA11,CODE=1)
CALL TEST_FILTER(BETAHAT22,BETA22,CODE=1)
CALL TEST_FILTER(BETAHAT33,BETA33,CODE=1)
CALL TEST_FILTER(BETAHAT12,BETA12,CODE=1)
CALL TEST_FILTER(BETAHAT13,BETA13,CODE=1)
CALL TEST_FILTER(BETAHAT23,BETA23,CODE=1)
...
CALL TEST_FILTER(MLHAT,ML,CODE=1)
CALL TEST_FILTER(MMHAT,MM,CODE=1)
...
CALL TEST_FILTER(RUU_HAT,RUU,CODE=1)
CALL TEST_FILTER(RVV_HAT,RVV,CODE=1)
CALL TEST_FILTER(RWW_HAT,RWW,CODE=1)
CALL TEST_FILTER(RUV_HAT,RUV,CODE=1)
CALL TEST_FILTER(RUW_HAT,RUW,CODE=1)
CALL TEST_FILTER(RVW_HAT,RVW,CODE=1)
...
CALL TEST_FILTER(RU_HAT,RU,CODE=1)
CALL TEST_FILTER(RV_HAT,RV,CODE=1)
CALL TEST_FILTER(RW_HAT,RW,CODE=1)
...
SUBROUTINE TEST_FILTER(HAT,ORIG,CODE)

! Tophat filter

REAL(EB), INTENT(IN), TARGET :: ORIG(0:IBP1,0:JBP1,0:KBP1)
REAL(EB), INTENT(OUT) :: HAT(0:IBP1,0:JBP1,0:KBP1)
INTEGER, INTENT(IN), OPTIONAL :: CODE ! CODE=1 allows overwrite of ORIG memory space using a temporary work array
INTEGER :: I, J, K, L, M, N
...
REAL(EB), POINTER, DIMENSION(:,:,:) :: PHI=>NULL()
   ...
   DO J = 1,JBP1-1
      DO I = 1,IBP1-1
         ...
         DO N = -1,1
            DO M = -1,1
               DO L = -1,1
                  HAT(I,J,K) = HAT(I,J,K) + PHI(I+L,J+M,K+N) * K3D(L,M,N)
               ENDDO
            ENDDO
         ENDDO
