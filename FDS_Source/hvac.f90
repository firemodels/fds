...
INTEGER :: N_HVAC_READ,N,ND,ND2,NM,NN,NF,NV,I_DUCT=0,I_DUCTNODE=0,I_FILTER=0,I_FAN=0,I_AIRCOIL=0,FAN_INDEX,AIRCOIL_INDEX, &
           INDEX,IOS,IZERO
...
ITER_LOOP: DO
   ...
   NODE_LOOP:DO NN = 1,NE%N_DUCTNODES
      ...
      IF(N_SPECIES > 0) THEN
         DN%YY(:)  = YYTOT/MTOT
         CALL GET_SPECIFIC_GAS_CONSTANT(DN%YY,DN%RSUM)
      ENDIF
      TGUESS = TGUESS / MTOT      
      ETOT = ETOT/ MTOT
      DN%TMP = TGUESS
...
IF (N_SPECIES > 0) WRITE(*,*) (DUCTNODE(NN)%YY(2),NN=1,N_DUCTNODES),' YY'
...
USE GLOBAL_CONSTANTS
INTEGER, INTENT(IN)::NETWORK_INDEX
INTEGER :: NN,ND, ARRAYLOC,IPZ,NM
REAL :: HEAD,XYZ(3),D_PBAR_DT_EST
TYPE(NETWORK_TYPE), POINTER::NE
...
DO ND = 1, NE%N_DUCTS
   ...
   ELSEIF (DN%VENT_INDEX > 0) THEN
      ...
      IF (N_ZONE > 0) THEN
         ...
         IF (IPZ > 0) THEN
            PZ => P_ZONE(IPZ)
            D_PBAR_DT_EST = (DSUM(IPZ,NM) - USUM(IPZ,NM))/PSUM(IPZ,NM)
            HEAD = HEAD + D_PBAR_DT_EST*DT
            WRITE(*,*) 'HEAD1 ',HEAD , D_PBAR_DT_EST*DT
            DO NN=1,PZ%N_DUCTNODES
               DN2=>DUCTNODE(PZ%NODE_INDEX(NN))
               DU2=>DUCT(DN%DUCT_INDEX(1))
               HEAD = HEAD + DN2%DIR(1) * DU2%AREA * DU2%VEL(OLD) * DT/PSUM(IPZ,NM)
               WRITE(*,*) 'HEAD3 ',HEAD ,PSUM(IPZ,NM),DN2%DIR(1),DU2%AREA*DU2%VEL(OLD),DT         
            ENDDO
         ENDIF
      ENDIF
   ENDIF
...
!   LHS(ARRAYLOC1,ARRAYLOC1) = 1._EB+0.5_EB*DU%TOTAL_LOSS*ABS(DU%VEL(NEW)-DU%VEL(GUESS))*DT/DU%LENGTH
   ...
   WRITE(*,*) 'LHS1 ',LHS(ARRAYLOC1,ARRAYLOC1)
   DN=>DUCTNODE(DU%NODE_INDEX(1))
   IF (DN%VENT_INDEX < 0) THEN
   ...
   ELSE
      IF (DN%ZONE_INDEX >0) THEN
         ...
         DO NN=1,PZ%N_DUCTNODES
            ...
            WRITE(*,*) 'LHS2 ',LHS(ARRAYLOC1,ARRAYLOC2),PSUM(DN%ZONE_INDEX,DN%MESH_INDEX)
         ENDDO
      ENDIF
   ENDIF
   DN=>DUCTNODE(DU%NODE_INDEX(2))
...
DO NN = 1, N_DUCTNODES
   ...
   IF (NM==1 .AND. DN%AMBIENT) THEN
      ...
      IF (N_SPECIES > 0) THEN
         ...
         CALL GET_SPECIFIC_GAS_CONSTANT(DN%YY(:),DN%RSUM_V)
      ELSE
   ...
   DN%CP_V  = CP
   DN%TMP_V = TMP_SUM
   IF (N_SPECIES>0) CALL GET_SPECIFIC_GAS_CONSTANT(DN%YY_V(:),DN%RSUM_V)
