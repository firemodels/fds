...
REAL(EB) :: T, RAP, AX, AXU, AXD, AY, AYU, AYD, AZ, VC, RU, RD, RP, &
            ILXU, ILYU, ILZU, QVAL, BBF, BBFA, NCSDROP, RSA_RAT,COSINE,KFST4_ALTERNATIVE,EFLUX
...
!                     NCSDROP = THFO*AVG_DROP_DEN(I,J,K,EVAP_INDEX)/ (PC%DENSITY*AVG_DROP_RAD(I,J,K,EVAP_INDEX))
   ...
   IF (NLP>0 .AND. VIRTUAL_PARTICLES) THEN
      DO ID = 1,NLP
         ...
         END SELECT
         KAPPA_PART = 0.25_EB*SURFACE_AREA*RDX(IID)*RRN(IID)*RDY(JJD)*RDZ(KKD)
...
IF (VIRTUAL_PARTICLES) THEN
   DROPLET_LOOP: DO I=1,NLP
      ...
      IF (PC%SURF_INDEX>0) THEN
         ...
         SF => SURFACE(PC%SURF_INDEX)
         EFLUX = EVALUATE_RAMP(T,SF%TAU(TIME_EFLUX),SF%RAMP_INDEX(TIME_EFLUX))*SF%EXTERNAL_FLUX
   !!!   QRADIN(IW) = E_WALL(IW)*(DR%ILW + EFLUX)
