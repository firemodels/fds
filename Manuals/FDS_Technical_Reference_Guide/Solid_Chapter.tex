...
%% alternate model that does not rely on arbitrary time scale
%\begin{equation}
%\label{eq:pendepth2}
%\delta = \left( \frac{\rho_{\rm s} \alpha_{\rm s}^3}{\dot{q}_w^{\prime\prime}} \right)^{1/3}
%\end{equation}
%where $\alpha_{\rm s} = k_{\rm s}/( \rho_{\rm s} c_{\rm s} )$.
...
% \subsection{Solid Pyrolysis 3D (Beta) {\color{red}[UNDER CONSTRUCTION]}}

% In three dimensions, the mass transport equation for solid material component $\alpha$ in conservative form is given by
% \begin{align}
% \label{eq:3dmatlsolid}
% \frac{\partial \rho_{\rm s,\alpha}}{\partial t} + \Div{(\rho_{\rm s,\alpha} \mathbf{u}_{\rm s})} &= \rho_{\rm s}(0) \left[S_\alpha - \sum_{\beta=1}^{N_{\rm r,\alpha}} r_{\alpha \beta}\right] =  \dot{m}_{\alpha}^{\ppp}
% \end{align}

% Expanding Eq.~(\ref{eq:3dmatlsolid}) and integrating over a cell volume leads to a transport equation for the mean cell density of solid material $\alpha$,
% \begin{align}
% \frac{1}{V_{\rm c}} \int_{V_{\rm c}} \left[ \frac{\partial \rho_{\rm s,\alpha}}{\partial t} + \rho_{\rm s,\alpha} \Div{\mathbf{u}_{\rm s}} + \mathbf{u}_{\rm s} \cdot \nabla \rho_{\rm s,\alpha} \right] \,\mbox{d}V &=  \frac{1}{V_{\rm c}} \int_{V_{\rm c}} \dot{m}_{\alpha}^{\ppp} \,\mbox{d}V \\
% \frac{\partial \overline{\rho}_{\rm s,\alpha}}{\partial t} + \overline{\rho}_{\rm s,\alpha} \Div{\mathbf{u}_{\rm s}} + ... &= \overline{\dot{m}_{\alpha}^{\ppp}}
% \end{align}
% where the overline represents a cell average value.

% In our present formulation movement of the solid material is not tracked explicitly---we do not have a representation of $\mathbf{u}_{\rm s}$ in the model.  Instead, a volume expansion (or contraction) within an Eulerian cell is imagined.  When the volume falls below a threshold (10 \%) of the original cell volume, the local cell mass is merged with its neighbor and the cell is ``burned away''.

% When pyrolyzate gas is produced via Eq.~(\ref{eq:pyrolyzate}) it evolves by
% \begin{equation}
% \label{eq:pryo3dgas}
% \DoD{\rho_\gamma}{t} = - \nabla\cdot \mathbf{J}_\gamma + \dot{m}_{\gamma}^{\ppp}
% \end{equation}
% where $\mathbf{J}_\gamma$ is the diffusive flux for gas species $\gamma$.

% For completeness, we rewrite the heat equation in this section since the shrinking and swelling of materials has important consequences for the heat flux between Eulerian cells on the Cartesian grid.  Additionally, to account for internal radiation, we apply the optically thick (diffusion) approximation \cite{Modest:2003}.  The ``radiative conductivity'' is given by
% \begin{equation}
% \label{eq:krad}
% k_{\rm r} = \frac{16 n^2 \sigma T^3}{3 \beta_{\rm r}}
% \end{equation}
% and the thermal energy equation for the solid becomes
% \begin{equation}
%   \rho_{\rm s} c_{\rm s} \; \DoD{T_{\rm s}}{t} = \nabla\cdot \left( [k_{\rm s}+k_{\rm r}] \nabla T_{\rm s} \right) + \dot{q}_{\rm s}'''
%   \label{eq:3dheatGibbs}
% \end{equation}
...
% \caption{Solid to cell volume ratio, $V_{\rm s}/V_{\rm c}$.}
% \label{fig:vsrvc}
% \end{figure}
...
\ee
\be \frac{1}{{\rm Fu}(m,\omega)} = \epsilon_s \frac{r_m + r_{\omega}}{k_{\rm b} T \left({\rm B}(m) + {\rm B}(\omega) \right)} \sqrt {\frac{8 k_{\rm b} T}{\pi} \left(\frac{1}{m}-\frac{1}{\omega} \right)} +
		 1+\frac{2 \sqrt{(\tilde{a}_m^2+\tilde{a}_{\omega}^2)}}{r_m+r_{\omega}}
\ee
\be
...
\ee
\be
\epsilon_{\rm PK}(m,\omega) = \frac {{\rm min} \left(r_m,r_{\omega} \right)^2}{2 \left( r_m^2+r_{\omega}^2 \right)}
\ee
\be
u_g(m) = g m {\rm B}(m)
\ee
