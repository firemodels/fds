   ...
   CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER
      ...
      SELECT CASE(IOR)
         CASE( 1) 
            ...
            WC%RHO_F = SCALAR_FACE_VALUE(UU(II,JJ,KK),ZZZ)
         CASE(-1)
            ZZZ(1:4) = (/RHO_G_2,RHO_G,RHO_OTHER,RHO_OTHER_2/)
            WC%RHO_F = SCALAR_FACE_VALUE(UU(II-1,JJ,KK),ZZZ)
         CASE( 2) 
            ZZZ(1:4) = (/RHO_OTHER_2,RHO_OTHER,RHO_G,RHO_G_2/)
            WC%RHO_F = SCALAR_FACE_VALUE(VV(II,JJ,KK),ZZZ)
         CASE(-2) 
            ZZZ(1:4) = (/RHO_G_2,RHO_G,RHO_OTHER,RHO_OTHER_2/)
            WC%RHO_F = SCALAR_FACE_VALUE(VV(II,JJ-1,KK),ZZZ)
         CASE( 3) 
            ZZZ(1:4) = (/RHO_OTHER_2,RHO_OTHER,RHO_G,RHO_G_2/)
            WC%RHO_F = SCALAR_FACE_VALUE(WW(II,JJ,KK),ZZZ)
         CASE(-3) 
            ZZZ(1:4) = (/RHO_G_2,RHO_G,RHO_OTHER,RHO_OTHER_2/)
            WC%RHO_F = SCALAR_FACE_VALUE(WW(II,JJ,KK-1),ZZZ)
      END SELECT
         ...
         SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
            ...
            SELECT CASE(IOR)
               CASE( 1) 
                  ...
                  RHO_ZZ_F(N) = SCALAR_FACE_VALUE(UU(II,JJ,KK),ZZZ)
               CASE(-1)
                  ZZZ(1:4) = (/RHO_ZZ_G_2,RHO_ZZ_G,RHO_ZZ_OTHER(N),RHO_ZZ_OTHER_2/)
                  RHO_ZZ_F(N) = SCALAR_FACE_VALUE(UU(II-1,JJ,KK),ZZZ)
               CASE( 2) 
                  ZZZ(1:4) = (/RHO_ZZ_OTHER_2,RHO_ZZ_OTHER(N),RHO_ZZ_G,RHO_ZZ_G_2/)
                  RHO_ZZ_F(N) = SCALAR_FACE_VALUE(VV(II,JJ,KK),ZZZ)
               CASE(-2) 
                  ZZZ(1:4) = (/RHO_ZZ_G_2,RHO_ZZ_G,RHO_ZZ_OTHER(N),RHO_ZZ_OTHER_2/)
                  RHO_ZZ_F(N) = SCALAR_FACE_VALUE(VV(II,JJ-1,KK),ZZZ)
               CASE( 3) 
                  ZZZ(1:4) = (/RHO_ZZ_OTHER_2,RHO_ZZ_OTHER(N),RHO_ZZ_G,RHO_ZZ_G_2/)
                  RHO_ZZ_F(N) = SCALAR_FACE_VALUE(WW(II,JJ,KK),ZZZ)
               CASE(-3) 
                  ZZZ(1:4) = (/RHO_ZZ_G_2,RHO_ZZ_G,RHO_ZZ_OTHER(N),RHO_ZZ_OTHER_2/)
                  RHO_ZZ_F(N) = SCALAR_FACE_VALUE(WW(II,JJ,KK-1),ZZZ)
            END SELECT
         ENDDO SPECIES_LOOP
