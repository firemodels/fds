...
WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (SF%HT1D .OR. SF%HT_DIM>1) THEN
      ONE_D%LAYER_THICKNESS(ONE_D%N_LAYERS) = 0._EB
   ...
   FIND_BACK_WALL_CELL: DO  ! Look for the back wall cell; that is, the wall cell on the other side of the obstruction
      ...
      IF (SF%HT1D .OR. SF%HT_DIM>1) THEN 
         ...
         ONE_D%LAYER_THICKNESS(ONE_D%N_LAYERS) = ONE_D%LAYER_THICKNESS(ONE_D%N_LAYERS) + THICKNESS - OLD_THICKNESS

         MATL_LOOP: DO NN=1,MAX_MATERIALS
            MI = OB%MATL_INDEX(NN)
            IF (MI<1) EXIT
            DO NNN=1,ONE_D%N_MATL
               IF (MI==MATL_INDEX(NNN)) EXIT MATL_LOOP
            ENDDO
            ONE_D%N_MATL = ONE_D%N_MATL + 1
            MATL_INDEX(ONE_D%N_MATL) = MI
         ENDDO MATL_LOOP
         ...
         IF (OBST_INDEX/=OBST_INDEX_PREVIOUS .AND. OBST_INDEX_PREVIOUS>0 .AND. OBST_INDEX>0) THEN
            ...
            IF ( (ANY(OB%MATL_MASS_FRACTION(:)/=OB_PREV%MATL_MASS_FRACTION(:),DIM=1)) .OR. &
               ...
               ONE_D%N_LAYERS = ONE_D%N_LAYERS + 1
               CALL REALLOCATE_REAL_ARRAY(ONE_D%LAYER_THICKNESS,1,ONE_D%N_LAYERS-1,ONE_D%N_LAYERS)
               ONE_D%LAYER_THICKNESS(ONE_D%N_LAYERS) = 0._EB
            ENDIF
         ENDIF
         ...
         IF (OBST_INDEX>0) THEN
            DO NN=1,ONE_D%N_MATL
               DO NNN=1,MAX_MATERIALS
                     IF (OB%MATL_INDEX(NNN)==MATL_INDEX(NN)) MATL_MASS_FRACTION(ONE_D%N_LAYERS,NN) = OB%MATL_MASS_FRACTION(NNN)
               ENDDO
            ENDDO
         ENDIF
      ...
      IF (.NOT.SF%HT1D .AND. SF%HT_DIM==1 .AND. THICKNESS>SUM(SF%LAYER_THICKNESS)) CYCLE WALL_LOOP
...
USE MEMORY_FUNCTIONS, ONLY: PACK_WALL,PACK_THIN_WALL,REALLOCATE_REAL_ARRAY,REALLOCATE_INTEGER_ARRAY
INTEGER, INTENT(IN) :: NM
INTEGER :: IW,ITW,NN
...
PRIMARY_WALL_LOOP_1: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   WC => M%WALL(IW)
   SF => SURFACE(WC%SURF_INDEX)
   IF (.NOT.SF%HT1D .AND. SF%HT_DIM==1 .OR. WC%BACK_INDEX<=0) CYCLE PRIMARY_WALL_LOOP_1
...
DO NL=1,ONE_D%N_LAYERS
   ...
   MIN_DIFFUSIVITY = HUGE_EB
   DO NN=1,ONE_D%N_MATL
      IF (ONE_D%MATL_COMP(NN)%MASS_FRACTION(NL)>TWO_EPSILON_EB) &
         MIN_DIFFUSIVITY = MIN(MIN_DIFFUSIVITY,MATERIAL(ONE_D%MATL_INDEX(NN))%THERMAL_DIFFUSIVITY)
   ENDDO

   ! Get the new N_CELLS_MAX for this wall cell

   CALL GET_N_LAYER_CELLS(MIN_DIFFUSIVITY,SF%SWELL_RATIO*ONE_D%LAYER_THICKNESS(NL),SF%STRETCH_FACTOR(NL), &
                          SF%CELL_SIZE_FACTOR(NL),SF%CELL_SIZE,SF%N_LAYER_CELLS_MAX(NL),N_CELLS_MAX,&
   ...
   CALL GET_N_LAYER_CELLS(MIN_DIFFUSIVITY,ONE_D%LAYER_THICKNESS(NL),SF%STRETCH_FACTOR(NL), &
                          SF%CELL_SIZE_FACTOR(NL),SF%CELL_SIZE,SF%N_LAYER_CELLS_MAX(NL),ONE_D%N_LAYER_CELLS(NL),&
                          ONE_D%SMALLEST_CELL_SIZE(NL),ONE_D%DDSUM(NL))
...
CALL GET_WALL_NODE_COORDINATES(ONE_D%N_CELLS_INI,1,ONE_D%N_LAYERS,ONE_D%N_LAYER_CELLS, &
                               ONE_D%N_LAYER_CELLS,ONE_D%SMALLEST_CELL_SIZE, &
