...
LOGICAL :: SUCCESS,EX,THICKEN_OBSTRUCTIONS,BAD,DEFINED_BACKGROUND=.FALSE.,SPECIFIED_HUMIDITY=.FALSE.
REAL(EB) :: XB(6),TEXTURE_ORIGIN(3)
REAL(EB) :: PBX,PBY,PBZ,HUMIDITY
REAL(EB) :: MW_MIN,MW_MAX
INTEGER  :: I,J,K,IZERO,IOS,N_SPEC_READ
...
IF (CYLINDRICAL.AND.PRES_METHOD=='SCARC') THEN
   WRITE(MESSAGE,'(A)')  'ERROR: Cannot set CYLINDRICAL=.TRUE. when ScaRC pressure solver is used'
   CALL SHUTDOWN(MESSAGE)
ENDIF
...
IF (CHECK_POISSON.AND.PRES_METHOD=='SCARC') THEN
   WRITE(MESSAGE,'(A)')  'ERROR: Cannot set CHECK_POISSON=.TRUE. when ScaRC pressure solver is used'
   CALL SHUTDOWN(MESSAGE)
ENDIF
...
REAL(EB) :: MASS_FRACTION_0,MW,SIGMALJ,EPSILONKLJ,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,XVAP,FLD_LETHAL_DOSE,FIC_CONCENTRATION
INTEGER  :: N_SPEC_READ,N
LOGICAL  :: ABSORBING,SMIX_COMPONENT_ONLY,BACKGROUND
...
NAMELIST /SPEC/ MASS_FRACTION_0,MW,FYI,ID,SIGMALJ,EPSILONKLJ,CONDUCTIVITY,VISCOSITY,DIFFUSIVITY,ABSORBING,BACKGROUND, &
                ...
                HUMIDITY,FLD_LETHAL_DOSE,FIC_CONCENTRATION
...
! IF no SPEC, set default background and exit

IF (I_BACKGROUND==0) N_SPECIES=N_SPECIES+1
...
SPEC_READ_LOOP: DO N=1,N_SPEC_READ
   ...
   IF (FLD_LETHAL_DOSE   > 0._EB) SS%FLD_LETHAL_DOSE   = FLD_LETHAL_DOSE/1000000._EB
   IF (FIC_CONCENTRATION > 0._EB) SS%FIC_CONCENTRATION = FIC_CONCENTRATION/1000000._EB
   ...
   IF (.NOT. BACKGROUND .AND. .NOT. SMIX_COMPONENT_ONLY) THEN
      N_TRACKED_SPECIES = N_TRACKED_SPECIES + 1
      SS%Y_INDEX = N_TRACKED_SPECIES
   ENDIF
   SS%MODE = GAS_SPECIES
   SELECT CASE (ID)
      CASE('WATER VAPOR')
      ...
      CASE('CARBON DIOXIDE')
         CO2_INDEX = N
         IF (.NOT. SMIX_COMPONENT_ONLY) I_CO2   = SS%Y_INDEX
      CASE('CARBON MONOXIDE')
         CO_INDEX = N
         IF (.NOT. SMIX_COMPONENT_ONLY) I_CO    = SS%Y_INDEX
      CASE('OXYGEN')
         O2_INDEX = N
         IF (.NOT. SMIX_COMPONENT_ONLY) I_O2    = SS%Y_INDEX
      CASE('NITROGEN')
         N2_INDEX = N
         IF (.NOT. SMIX_COMPONENT_ONLY) I_N2    = SS%Y_INDEX
      CASE('HYDROGEN')
         H2_INDEX = N
         IF (.NOT. SMIX_COMPONENT_ONLY) I_H2    = SS%Y_INDEX
      CASE('SOOT')
         SOOT_INDEX = N
         IF (.NOT. SMIX_COMPONENT_ONLY) I_SOOT  = SS%Y_INDEX
...
READ_SMIX_LOOP: DO N=1,N_MIX_SPECIES   
   ...
   IF (BACKGROUND) THEN
      ...
      ELSEIF (I_BACKGROUND <= N_SPEC_READ) THEN
         WRITE(MESSAGE,'(A)') 'ERROR: BACKGROUND species defined on both SPEC and SMIX'
         CALL SHUTDOWN(MESSAGE)
      ELSEIF (I_BACKGROUND == N_SPECIES) THEN
...
DO NS=1,N_MIX_SPECIES
   ...
   CALL GAS_PROPS(SS%ID,SS%SIG,SS%EPSK,SS%MW,SS%ABSORBING,SS%FORMULA)    
   IF (DEFINED_BACKGROUND .AND. NS==1) CYCLE
   N_TRACKED_SPECIES = N_TRACKED_SPECIES + 1
   I_Z_MIN = MIN(N_TRACKED_SPECIES,I_Z_MIN)
   I_Z_MAX = MAX(N_TRACKED_SPECIES,I_Z_MAX)
   SS%Y_INDEX = N_TRACKED_SPECIES
ENDDO

IF (DEFINED_BACKGROUND) SPECIES(0) = SPECIES(N_SPECIES - N_MIX_SPECIES + 1)
...
REAL(EB) :: Y_F_INLET, MASS_REACTANT, MASS_PRODUCT, Y_CO2_INFTY,&
            ...
            MAXIMUM_VISIBILITY,AUTO_IGNITION_TEMPERATURE,SUM1,THRESHOLD_TEMPERATURE
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM=>NULL()
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,EPUMO2,ID, N_S,EXTINCTION2,&
                ...
                C,H,N,O,IDEAL,VISIBILITY_FACTOR,MAXIMUM_VISIBILITY, &
...
COUNT_REAC_LOOP: DO
   ...
   IF ((BOF > 0._EB .OR. E > 0._EB) .AND. (C>0._EB .OR. H>0._EB)) THEN
      ...
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (SIMPLE_CHEMISTRY .AND. I_BACKGROUND<=N_SPEC_READ) THEN
   ...
   IF (SIMPLE_CHEMISTRY .AND. C<0._EB .AND. H<0._EB) THEN
      ...
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (SIMPLE_CHEMISTRY .AND. N_MIX_SPECIES > 0) THEN
...
INTEGER :: SM_N2=0,SM_O2=0,SM_FUEL=0,SM_H2O=0,SM_CO2=0,SM_CO=0,SM_SOOT=0,SM_H2=0,SM_PTR=0,SM2SPEC(1:MAX_SPECIES)
INTEGER :: NR2,NS,NS2
...
IF     (     CO_PRODUCTION) THEN
   N_REACTIONS = 2
   N_MIX_SPECIES = 4
   I_Z_MIN = N_TRACKED_SPECIES+1
   I_Z_MAX = N_TRACKED_SPECIES+3
...
DO NS=1,N_SPECIES
   IF (TRIM(FUEL)==TRIM(SPECIES(NS)%ID)) THEN
      FUEL_INDEX = NS
      SM2SPEC(1)= NS
      EXIT
   ENDIF
...
IF (FUEL_INDEX==0)  THEN
   ADD_NAME = FUEL
   CALL ADD_SPECIES(ADD_NAME)
   FUEL_INDEX = N_SPECIES
   SPECIES(FUEL_INDEX)%MW = MW_FUEL
ENDIF
SM2SPEC(SM_FUEL) = FUEL_INDEX
SM_FUEL_INDEX = SM_FUEL
...
IF (N2_INDEX == 0) THEN
   ...
   N2_INDEX = N_SPECIES  
ENDIF
SM2SPEC(SM_N2)=N2_INDEX  

IF (C>0) THEN
   SM_PTR = SM_PTR + 1
   SM_CO2 = SM_PTR
   IF(CO2_INDEX == 0) THEN
      ADD_NAME = 'CARBON DIOXIDE'
      CALL ADD_SPECIES(ADD_NAME)
      CO2_INDEX = N_SPECIES            
   ENDIF
   SM2SPEC(SM_CO2)=CO2_INDEX
   IF(CO_YIELD > 0._EB .OR. CO_PRODUCTION) THEN
      SM_PTR = SM_PTR + 1
      SM_CO = SM_PTR   
      IF(CO_INDEX == 0) THEN
         ADD_NAME = 'CARBON MONOXIDE'
         CALL ADD_SPECIES(ADD_NAME)
         CO_INDEX = N_SPECIES
      ENDIF
      SM2SPEC(SM_CO)=CO_INDEX      
   ENDIF  
ENDIF

IF (H > 0._EB .OR. HUMIDITY > 0._EB) THEN
   SM_PTR = SM_PTR + 1
   SM_H2O = SM_PTR
   IF (H2O_INDEX == 0) THEN
      ADD_NAME = 'WATER VAPOR'
      CALL ADD_SPECIES(ADD_NAME)
      H2O_INDEX = N_SPECIES
   ENDIF
   SM2SPEC(SM_H2O)=H2O_INDEX
   IF (H2_YIELD > 0._EB) THEN
      ...
      IF (H2_INDEX == 0) THEN
         ...
         H2_INDEX = N_SPECIES
      ENDIF
      SM2SPEC(SM_H2)=H2_INDEX
   ENDIF
ENDIF

IF (SOOT_YIELD > 0) THEN
   SM_PTR = SM_PTR + 1
   SM_SOOT = SM_PTR
   IF (SOOT_INDEX == 0) THEN
      ADD_NAME = 'SOOT'
      CALL ADD_SPECIES(ADD_NAME)
      SOOT_INDEX = N_SPECIES
      SPECIES(SOOT_INDEX)%MASS_EXTINCTION_COEFFICIENT = MASS_EXTINCTION_COEFFICIENT
   ELSE
      IF ((SPECIES(SOOT_INDEX)%MASS_EXTINCTION_COEFFICIENT - MASS_EXTINCTION_COEFFICIENT)>ZERO_P) &
         WRITE(LU_ERR,*) 'WARNING: SOOT MASS_EXINCTION_COEFFICIEJNT specified on SPEC will override default simple chemistry value'
      
   ENDIF
   SPECIES(SOOT_INDEX)%MW = MW_SOOT
   SM2SPEC(SM_SOOT)=SOOT_INDEX  
ENDIF
...
DO NS=1,N_MIX_SPECIES
   SM => SPECIES_MIXTURE(NS)
   SM%N_SUB_SPECIES=SM_PTR
   ALLOCATE (SM%SPEC_ID(SM_PTR))
   ALLOCATE (SM%SPEC_INDEX(SM_PTR))
   DO NS2 = 1,SM_PTR
      SM%SPEC_INDEX(NS2) = SM2SPEC(NS2)
      SM%SPEC_ID(NS2) = SPECIES(SM2SPEC(NS2))%ID
   ENDDO
   ALLOCATE (SM%MASS_FRACTION(SM_PTR))
   SM%MASS_FRACTION = 0._EB
   ALLOCATE (SM%VOLUME_FRACTION(SM_PTR))
   SM%VOLUME_FRACTION = 0._EB
   WRITE(SM%ID,'(A,I1)') 'LUMPED SPECIES ',NS-1
ENDDO


I_FUEL = N_TRACKED_SPECIES + 1
IF (CO_PRODUCTION) THEN
   I_PROG_CO = I_FUEL + 1
...
IF (SPECIES(O2_INDEX)%YY0 > ZERO_P) THEN
   ...
   SM%MASS_FRACTION(SM_O2)  = SPECIES(O2_INDEX)%YY0  
ELSE
   SM%MASS_FRACTION(SM_O2)  = Y_O2_INFTY
ENDIF
...
IF (SM_CO2 > 0) THEN
   IF (SPECIES(CO2_INDEX)%YY0 > ZERO_P) THEN
      ...
      SM%MASS_FRACTION(SM_CO2)  = SPECIES(CO2_INDEX)%YY0  
   ELSE
      SM%MASS_FRACTION(SM_CO2)  = Y_CO2_INFTY
   ENDIF
ENDIF
...
IF (SM_H2O > 0) THEN
   ...
   ELSE
      ...
      IF (HUMIDITY < 0._EB) THEN
         ...
         ENDIF
      ENDIF
      SM%MASS_FRACTION(SM_H2O) = AMBIENT_WATER_VAPOR(HUMIDITY,TMPA)
   ENDIF
ENDIF

IF (SM_H2O>0)  THEN
   SM%MASS_FRACTION(SM_O2) = SM%MASS_FRACTION(SM_O2) * (1._EB-SM%MASS_FRACTION(SM_H2O))    
   IF (SM_CO2>0) THEN
      SM%MASS_FRACTION(SM_CO2) = SM%MASS_FRACTION(SM_CO2) * (1._EB-SM%MASS_FRACTION(SM_H2O))
   ENDIF
ENDIF

SM%MASS_FRACTION(SM_N2) = 1._EB-SUM(SM%MASS_FRACTION)

IF (SM%MASS_FRACTION(SM_N2) < 0._EB) THEN
   WRITE(MESSAGE,*) 'ERROR: Computed N2 mass fraction for simple chemistry is less  than zero.'
   CALL SHUTDOWN(MESSAGE)
ENDIF

SM%VOLUME_FRACTION(SM_N2) = SM%MASS_FRACTION(SM_N2)/MW_N2
SM%VOLUME_FRACTION(SM_O2) = SM%MASS_FRACTION(SM_O2)/MW_O2
IF (SM_CO2>0) SM%VOLUME_FRACTION(SM_CO2) = SM%MASS_FRACTION(SM_CO2)/MW_CO2
...
SM%VOLUME_FRACTION(SM_FUEL) = SM%MASS_FRACTION(SM_FUEL) / MW_FUEL
SM%MASS_FRACTION(SM_N2) = 1._EB - Y_F_INLET
SM%VOLUME_FRACTION(SM_N2) = SM%MASS_FRACTION(SM_N2) / MW_N2
SM%MW = 1._EB / SUM(SM%VOLUME_FRACTION)
SM%RSUM = R0/SM%MW
SM%VOLUME_FRACTION = SM%VOLUME_FRACTION / SUM(SM%VOLUME_FRACTION)
...
!Compute fuel heat of formation
      NU(SM_O2) = C + 0.5_EB * H - 0.5_EB * O
      IF (HEAT_OF_COMBUSTION < 0._EB) HEAT_OF_COMBUSTION = EPUMO2*MW_O2*NU(SM_O2)/MW_FUEL
   ENDIF

   NU(SM_N2) = 0.5_EB * N   
   NU(SM_CO) = C
   IF (SM_H2O > 0) NU(SM_H2O) = 0.5_EB * H
   IF (H2_YIELD > 0._EB) THEN
      NU(SM_H2)  = H2_YIELD * MW_FUEL / MW_H2
      NU(SM_H2O) = NU(SM_H2O) - NU(SM_H2)
   ENDIF
   IF (SOOT_YIELD > 0._EB) THEN
      NU(SM_SOOT) = SOOT_YIELD * MW_FUEL / MW_SOOT
      NU(SM_CO)  = NU(SM_CO) - NU(SM_SOOT) * (1._EB - SOOT_H_FRACTION)
      IF (H > 0._EB) NU(SM_H2O) = NU(SM_H2O)- 0.5_EB * NU(SM_SOOT) * SOOT_H_FRACTION
   ENDIF
   NU(SM_O2)  = 0.5_EB*(NU(SM_CO) - O)
   IF (H>0._EB) NU(SM_O2) = NU(SM_O2) + 0.5*NU(SM_H2O)

   IF (H > 0._EB) THEN
      IF (NU(SM_H2O) < 0._EB) THEN
   ...
   SM%VOLUME_FRACTION(SM_N2) = NU(SM_N2) +& !N2 from fuel molecule
                    SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_N2)*NU(SM_O2)/SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2)+&!N2 air
                    SPECIES_MIXTURE(2)%VOLUME_FRACTION(SM_N2)/SPECIES_MIXTURE(2)%VOLUME_FRACTION(SM_FUEL) !N2 fuel stream
   IF (SPECIES_MIXTURE(2)%VOLUME_FRACTION(2)>0)  SM%VOLUME_FRACTION(SM_N2) =  SM%VOLUME_FRACTION(SM_N2) + &
                    SPECIES_MIXTURE(2)%VOLUME_FRACTION(1)/SPECIES_MIXTURE(2)%VOLUME_FRACTION(2) !N2 as fuel stream diluent
   SM%MASS_FRACTION(SM_N2) = SM%VOLUME_FRACTION(SM_N2) * MW_N2
   SM%VOLUME_FRACTION(SM_CO2) = SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_CO2)*NU(SM_O2)/&
                                 SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2)!CO2 air
   SM%MASS_FRACTION(SM_CO2) = SM%VOLUME_FRACTION(SM_CO2) * MW_CO2   
   SM%VOLUME_FRACTION(SM_CO) = NU(SM_CO)
   SM%MASS_FRACTION(SM_CO) =  SM%VOLUME_FRACTION(SM_CO) * MW_CO
   IF (SM_H2O > 0) THEN
      SM%VOLUME_FRACTION(SM_H2O) = NU(SM_H2O)  +&!H2O from fuel
                     SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_H2O)*NU(SM_O2)/SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2)!H2O air
      SM%MASS_FRACTION(SM_H2O) = SM%VOLUME_FRACTION(SM_H2O) * MW_H2O
   ENDIF
   IF (SOOT_YIELD > 0._EB)  THEN
      SM%VOLUME_FRACTION(SM_SOOT) = NU(SM_SOOT)
      SM%MASS_FRACTION(SM_SOOT) = SM%MASS_FRACTION(SM_SOOT) * MW_SOOT      
   ENDIF
   IF (H2_YIELD > 0._EB)  THEN
      SM%VOLUME_FRACTION(SM_H2) = NU(SM_H2)
      SM%MASS_FRACTION(SM_H2) = SM%VOLUME_FRACTION(SM_H2) * MW_H2
   ENDIF
   ...
   RN%SPECIES(1) = 2
   RN%NU(1) = -1._EB
   RN%NU(0) = -NU(SM_O2)/SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2)*SPECIES_MIXTURE(2)%VOLUME_FRACTION(SM_FUEL) 
   RN%SPECIES(2) = 3
   RN%NU(2) = SUM1 * SPECIES_MIXTURE(2)%VOLUME_FRACTION(SM_FUEL) 

   IF (.NOT. RN%IDEAL) THEN
      IF(HEAT_OF_COMBUSTION < 0._EB) HEAT_OF_COMBUSTION = EPUMO2 * (NU(SM_O2)+0.5_EB*(NU(SM_CO)-CO_YIELD*MW_FUEL/MW_CO)) *&
                                                          MW_O2/MW_FUEL

   ELSE
! Correct heat of combustion for minor products of combustion
      HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION - (C-NU(SM_CO)) * CO2_HEAT_OF_FORMATION * MW_CO2/MW_FUEL
      IF (H > 0._EB)  HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION - (0.5_EB*H-NU(SM_H2O)) * H2O_HEAT_OF_FORMATION * MW_H2O/MW_FUEL
   ENDIF
! Correct heat of combustion for incomplete reaction   
   RN%HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION*1000._EB - NU(SM_CO)*(CO2_HEAT_OF_FORMATION-CO_HEAT_OF_FORMATION)*1.E3_EB/MW_FUEL
...
!*****
   ...
   NU(SM_CO2) = NU(SM_CO) - CO_YIELD * MW_FUEL/MW_CO
   NU(SM_CO)  = CO_YIELD * MW_FUEL/MW_CO
   IF (NU(SM_CO2) < 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Values for SOOT_YIELD, CO_YIELD, and SOOT_H_FRACTION result in negative CO2 yield'
      CALL SHUTDOWN(MESSAGE)
   ENDIF

   NU(SM_O2) = NU(SM_O2) + 0.5_EB * NU(SM_CO2)
   ...
   SM%VOLUME_FRACTION(SM_N2) = NU(SM_N2) +& !N2 from fuel molecule
                    ...
                    SPECIES_MIXTURE(2)%VOLUME_FRACTION(SM_N2)/SPECIES_MIXTURE(2)%VOLUME_FRACTION(SM_FUEL) !N2 fuel stream
   SM%MASS_FRACTION(SM_N2) = SM%VOLUME_FRACTION(SM_N2) * MW_N2
   SM%VOLUME_FRACTION(SM_CO2) = NU(SM_CO2) +&!CO2 from fuel
                    SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_CO2)*NU(SM_O2)/SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2)!CO2 air
   SM%MASS_FRACTION(SM_CO2) =SM%VOLUME_FRACTION(SM_CO2) * MW_CO2
   SM%VOLUME_FRACTION(SM_CO) = NU(SM_CO)
   SM%MASS_FRACTION(SM_CO) = SM%VOLUME_FRACTION(SM_CO) * MW_CO
   SM%VOLUME_FRACTION(SM_H2O) = NU(SM_H2O) +&!H2O from fuel
   ...
   IF (SOOT_YIELD > 0._EB)  THEN
      SM%VOLUME_FRACTION(SM_SOOT) = NU(SM_SOOT)
      SM%MASS_FRACTION(SM_SOOT) = SM%VOLUME_FRACTION(SM_SOOT) * MW_SOOT      
   ENDIF
   IF (H2_YIELD > 0._EB)  THEN
      SM%VOLUME_FRACTION(SM_H2) = NU(SM_H2)
      SM%MASS_FRACTION(SM_H2) = SM%VOLUME_FRACTION(SM_H2) * MW_H2
   ...
   RN%SPECIES(1) = 3
   RN%NU(1) = -1._EB
   RN%NU(0) = -0.5_EB*SPECIES_MIXTURE(3)%VOLUME_FRACTION(SM_CO)*NU(SM_CO2)/(NU(SM_CO2)+NU(SM_CO))/&
               SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2)
   RN%SPECIES(2) = 4
   RN%NU(2) = 1._EB-RN%NU(0)*(1._EB-SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2))
...
!*****
   ...
   REACTION%HOC_COMPLETE = REACTION(1)%HEAT_OF_COMBUSTION + NU(SM_CO2) * MW_CO * RN%HEAT_OF_COMBUSTION/MW_FUEL
...
!Compute fuel heat of formation
   ...
   IF (SM_CO2>0) NU(SM_CO2) = C
   IF (CO_YIELD > 0._EB) THEN
      NU(SM_CO)  = CO_YIELD * MW_FUEL / MW_CO
      NU(SM_CO2) = NU(SM_CO2) - NU(SM_CO)
   ENDIF
   IF (H > 0._EB) NU(SM_H2O) = 0.5_EB * H
   IF (H2_YIELD > 0._EB) THEN
      NU(SM_H2)  = H2_YIELD * MW_FUEL / MW_H2
      NU(SM_H2O) = NU(SM_H2O) - NU(SM_H2)
   ENDIF
   IF (SOOT_YIELD > 0._EB) THEN
      NU(SM_SOOT) = SOOT_YIELD * MW_FUEL / MW_SOOT
      NU(SM_CO2)  = NU(SM_CO2) - NU(SM_SOOT) * (1._EB - SOOT_H_FRACTION)
      IF (H > 0._EB) NU(SM_H2O) = NU(SM_H2O)- 0.5_EB * NU(SM_SOOT) * SOOT_H_FRACTION
   ENDIF
   IF (SM_CO2>0) THEN
      IF (NU(SM_CO2) < 0._EB) THEN
   ...
   IF (H > 0._EB) THEN
      ...
      ENDIF
   ENDIF
   IF (SM_CO2>0) NU(SM_O2)  = NU(SM_CO2) - 0.5_EB * O
   IF (CO_YIELD > 0._EB) NU(SM_O2) = NU(SM_O2) + 0.5_EB * NU(SM_CO)
   IF (H>0._EB) NU(SM_O2) = NU(SM_O2) + 0.5*NU(SM_H2O)
   NU(SM_N2) = 0.5_EB * N
   ...
   IF (SOOT_YIELD > 0._EB)  THEN
      IF (SOOT_DEPOSITION) THEN
         SM%VOLUME_FRACTION(SM_SOOT) = 0._EB
         SM%MASS_FRACTION(SM_SOOT) = 0._EB
         SPECIES_MIXTURE(4)%VOLUME_FRACTION(SM_SOOT) = 1._EB
         SPECIES_MIXTURE(4)%MASS_FRACTION(SM_SOOT) = 1._EB
   ...
   IF (.NOT. RN%IDEAL) THEN
      IF(HEAT_OF_COMBUSTION < 0._EB) HEAT_OF_COMBUSTION = &
                                     -EPUMO2*RN%NU(0)*MW_O2*SPECIES_MIXTURE(1)%VOLUME_FRACTION(SM_O2)/&
                                     (SPECIES_MIXTURE(2)%VOLUME_FRACTION(SM_FUEL)*MW_FUEL)
   ELSE
! Correct heat of combustion for minor products of combustion
      IF (SOOT_YIELD > 0._EB) HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION - NU(SM_SOOT)*(1._EB-SOOT_H_FRACTION)*&
                                                   CO2_HEAT_OF_FORMATION/MW_FUEL
      IF (CO_YIELD > 0._EB) HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION - NU(SM_CO)*&
                                                 (CO2_HEAT_OF_FORMATION - CO_HEAT_OF_FORMATION)/MW_FUEL
      IF (H > 0._EB)  HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION - (0.5_EB*H-NU(SM_H2O))*H2O_HEAT_OF_FORMATION/MW_FUEL
   ENDIF
...
I_BACKGROUND = N_SPECIES + 1

DO NS=1,N_MIX_SPECIES
   SM=>SPECIES_MIXTURE(NS)
   CALL ADD_SPECIES(SM%ID)
   SS=>SPECIES(N_SPECIES)
   WRITE(SS%FORMULA,'(A,I1)') 'Z',NS-1   
   SS%ABSORBING = .TRUE.
   SS%MODE = LUMPED_SPECIES
   SS%MW = SPECIES_MIXTURE(NS)%MW  
   SS%ID = SPECIES_MIXTURE(NS)%ID
...
DO NR2=1,N_REACTIONS
   RN => REACTION(NR2)
   DO NS=1,RN%N_SPECIES
      RN%SPECIES(NS) = N_TRACKED_SPECIES - N_MIX_SPECIES + RN%SPECIES(NS)
   ENDDO
ENDDO
...
!Remove last species fromo spec array
INTEGER :: IZERO

TYPE(SPECIES_TYPE),POINTER,DIMENSION(:):: SPECIES_TEMP
TYPE(SPECIES_TYPE),POINTER:: SS=>NULL()

ALLOCATE(SPECIES_TEMP(0:N_SPECIES-1),STAT=IZERO)
CALL ChkMemErr('ADD SPECIES','SPECIES_TEMP',IZERO) 
SPECIES_TEMP(0:N_SPECIES-1) = SPECIES(0:N_SPECIES-1)
DEALLOCATE(SPECIES)
N_SPECIES = N_SPECIES - 1
ALLOCATE(SPECIES(0:N_SPECIES),STAT=IZERO)
CALL ChkMemErr('ADD SPECIES','SPECIES',IZERO) 
SPECIES(0:N_SPECIES)=SPECIES_TEMP(0:N_SPECIES)
DEALLOCATE(SPECIES_TEMP)  

END SUBROUTINE REMOVE_SPECIES 
 
SUBROUTINE ADD_SPECIES(SPEC_ID)
USE PROPERTY_DATA,ONLY:GAS_PROPS
!Add species to spec array
INTEGER :: IZERO
...
ALLOCATE(SPECIES_TEMP(0:N_SPECIES),STAT=IZERO)
CALL ChkMemErr('ADD SPECIES','SPECIES_TEMP',IZERO) 
SPECIES_TEMP(0:N_SPECIES) = SPECIES(0:N_SPECIES)
DEALLOCATE(SPECIES)
N_SPECIES = N_SPECIES + 1
ALLOCATE(SPECIES(0:N_SPECIES),STAT=IZERO)
CALL ChkMemErr('ADD SPECIES','SPECIES',IZERO) 
SPECIES(0:N_SPECIES-1)=SPECIES_TEMP(0:N_SPECIES-1)
DEALLOCATE(SPECIES_TEMP)
...
SS%YY0                         = 0._EB
SS%MW                          = -1._EB
SS%REFERENCE_TEMPERATURE       = 25._EB + TMPM
SS%SIG                         = -1._EB
SS%SPECIFIC_HEAT               = -1._EB*1000._EB
SS%SPECIFIC_ENTHALPY           = -1._EB*1000._EB
SS%MU_USER                     = -1._EB
...
DO NR=1,N_REACTIONS
   RN => REACTION(NR)
   ALLOCATE(RN%NU_SPECIES(1:N_SPECIES-N_MIX_SPECIES),STAT=IZERO)
...
REAL(EB) :: YYY(1:N_TRACKED_SPECIES)
...
ALLOCATE(Y2SPEC(0:N_TRACKED_SPECIES))
...
N_Y_ARRAY = N_SPECIES - N_MIX_SPECIES

ALLOCATE(Y2Y(N_Y_ARRAY,N_TRACKED_SPECIES),STAT=IZERO)
...
ALLOCATE(YY_AMBIENT(N_TRACKED_SPECIES),STAT=IZERO)
...
ALLOCATE(Y2CP(0:5000,N_TRACKED_SPECIES))     
CALL ChkMemErr('READ','Y2CC',IZERO)    
Y2CP = 0._EB   

ALLOCATE(Y2CPBAR(0:5000,N_TRACKED_SPECIES))     
CALL ChkMemErr('READ','Y2CPBAR',IZERO)    
Y2CPBAR = 0._EB   

ALLOCATE(Y2K(0:5000,N_TRACKED_SPECIES))      
CALL ChkMemErr('READ','Y2K',IZERO)    
Y2K = 0._EB   

ALLOCATE(Y2MU(0:5000,N_TRACKED_SPECIES))      
...
INTEGER :: N, J, ITMP
REAL(EB) :: H_L,H_V,YY_GET(1:N_TRACKED_SPECIES),CPBAR,H_G_S,H_G_S_REF,H_L_REF,TMP_REF,TMP_MELT,TMP_V,TMP_WGT
...
INTEGER :: N,J,NS,NS2,NR,SPEC_INDEX(MAX_SPECIES,MAX_REACTIONS)
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   DO NR=1,ML%N_REACTIONS
      DO NS=1,N_SPECIES
         ...
         DO NS2=1,N_TRACKED_SPECIES
            IF (TRIM(ML%SPEC_ID(NS,NR))==TRIM(SPECIES(Y2SPEC(NS2))%ID)) THEN
               SPEC_INDEX(NS,NR) = NS2
               ML%NU_GAS(SPEC_INDEX(NS,NR),NR) = ML%NU_SPEC(NS,NR)
               EXIT
            ENDIF
         ENDDO
         IF (SPEC_INDEX(NS,NR)==-1) THEN
  ...
  ! Determine if the surface is combustible/burning
   ...
   BURNING_IF: IF (BURNING .AND. .NOT.ALL(EVACUATION_ONLY)) THEN
      IF (SF%HRRPUA>0._EB) THEN
         ...
         ELSE
            RN => REACTION(1)
         ENDIF
         SF%MASS_FLUX(I_FUEL) = SF%HRRPUA/ (RN%HOC_COMPLETE*SPECIES_MIXTURE(2)%MASS_FRACTION(SM_FUEL_INDEX))
      ENDIF
   ...
   IF (SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FRACTION) THEN
      IF (ALL(SF%MASS_FRACTION< 0._EB)) THEN
         SF%MASS_FRACTION(1:N_TRACKED_SPECIES) = YY_AMBIENT(1:N_TRACKED_SPECIES)
      ELSE
         SPECIES_LOOP: DO NSPC=1,N_TRACKED_SPECIES
            IF (SF%MASS_FRACTION(NSPC)<0._EB) SF%MASS_FRACTION(NSPC) = 0._EB
         ENDDO SPECIES_LOOP
         IF (SUM(SF%MASS_FRACTION(1:N_TRACKED_SPECIES)) > 1._EB) THEN
...
REAL(EB) :: TEMPERATURE,DENSITY,MASS_FRACTION(1:MAX_SPECIES),RR_SUM,YY_GET(1:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
         ...
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            SPEC_INIT_IF:IF (N_TRACKED_SPECIES > 0 .AND. ANY(MASS_FRACTION > ZERO_P)) THEN
               ...
               ELSE 
                  ...
                  YY_GET = IN%MASS_FRACTION(1:N_TRACKED_SPECIES)
                  CALL GET_SPECIFIC_GAS_CONSTANT(YY_GET,RR_SUM)
               ENDIF            
            ELSE SPEC_INIT_IF
                IF (N_TRACKED_SPECIES > 0) IN%MASS_FRACTION(1:N_TRACKED_SPECIES) = YY_AMBIENT(1:N_TRACKED_SPECIES)            
                RR_SUM = RSUM0
            ENDIF SPEC_INIT_IF
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%QUANTITY /= 'null') THEN
      CALL GET_QUANTITY_INDEX(DV%SMOKEVIEW_LABEL,DV%SMOKEVIEW_BAR_LABEL,QUANTITY_INDEX,I_DUM, &
                              DV%SPEC_INDEX,DV%PART_INDEX,DV%DUCT_INDEX,DV%NODE_INDEX(1),'DEVC', &
      ...
      IF (TRIM(DV%QUANTITY)=='NODE PRESSURE DIFFERENCE') THEN
         CALL GET_QUANTITY_INDEX(DV%SMOKEVIEW_LABEL,DV%SMOKEVIEW_BAR_LABEL,QUANTITY_INDEX,I_DUM, &
                                 DV%SPEC_INDEX,DV%PART_INDEX,DV%DUCT_INDEX,DV%NODE_INDEX(2),'DEVC', &
...
IF (TRIM(SPEC_ID)/='null' .AND. SPEC_INDEX==0) THEN
   IF (SIMPLE_CHEMISTRY) THEN
