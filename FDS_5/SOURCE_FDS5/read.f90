...
REAL(EB) :: ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
            BEAD_DIAMETER,BEAD_EMISSIVITY,CABLE_MASS_PER_LENGTH,CABLE_DIAMETER,CABLE_FAILURE_TEMPERATURE, &
...
USE DEVICE_VARIABLES, ONLY : DEVICE, N_DEVC, GAS_CELL_RAD_FLUX, GAS_CELL_RAD_DEVC, N_GAS_CELL_RAD_DEVC
USE CONTROL_VARIABLES
INTEGER :: NC,NN,NNN
TYPE (CONTROL_TYPE), POINTER :: CF
...
USE DEVICE_VARIABLES, ONLY : DEVICE_TYPE, DEVICE, N_DEVC, PROPERTY, N_PROP, &
                             GAS_CELL_RAD_FLUX, GAS_CELL_RAD_DEVC, N_GAS_CELL_RAD_DEVC
USE CONTROL_VARIABLES
! Process the DEViCes
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
  ...
  ! Assign properties to the DEVICE array
   ...
   IF (DV%QUANTITY == 'RADIATIVE_FLUX_GAS') THEN 
      GAS_CELL_RAD_FLUX = .TRUE.
      N_GAS_CELL_RAD_DEVC = N_GAS_CELL_RAD_DEVC + 1
   ENDIF
ENDDO PROC_DEVC_LOOP

IF (GAS_CELL_RAD_FLUX) ALLOCATE (GAS_CELL_RAD_DEVC(N_GAS_CELL_RAD_DEVC))
NNN = 0
DO NN = 1, N_DEVC
   IF (DEVICE(NN)%QUANTITY == 'RADIATIVE_FLUX_GAS') THEN 
      NNN = NNN + 1
      GAS_CELL_RAD_DEVC(NNN) = NN
   ENDIF
ENDDO
...
DO N = 1, N_EVAP_INDICIES
   ...
   OUTPUT_QUANTITY(230+N)%NAME = TRIM(ID)//'_MEDIAN_DIAMETER'
   OUTPUT_QUANTITY(230+N)%UNITS = 'mu-m'
   OUTPUT_QUANTITY(230+N)%SHORT_NAME = 'diam'

   OUTPUT_QUANTITY(231:239)%ISOF_APPROPRIATE = .FALSE.
   OUTPUT_QUANTITY(231:239)%PART_APPROPRIATE = .FALSE.
   OUTPUT_QUANTITY(231:239)%SLCF_APPROPRIATE = .FALSE.
   OUTPUT_QUANTITY(231:239)%BNDF_APPROPRIATE = .FALSE.
