...
REAL(EB) :: ZZ_0(0:N_TRACKED_SPECIES),ERR_EST,ERR_TOL,ZZ_TEMP(0:N_TRACKED_SPECIES),&
            ZZ_NEW(0:N_TRACKED_SPECIES),A1(0:N_TRACKED_SPECIES),A2(0:N_TRACKED_SPECIES),A4(0:N_TRACKED_SPECIES),Q_SUM,Q_CALC,&
...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_ITERATIONS
   ...
   RK2_IF: IF (COMBUSTION_ODE /= RK2_RICHARDSON) THEN ! Explicit Euler 
      ZZ_0 = ZZ_MIXED
      DO SR=0,SERIES_REAC
         CALL FIRE_FORWARD_EULER (ZZ_MIXED,DT_SUB,1,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
      ENDDO 
   ...
   ELSE RK2_IF ! RK2 w/ Richardson
      ...
      RICH_EX_LOOP: DO RICH_ITER =1,RICH_ITER_MAX
         ...
         ZZ_NEW = ZZ_MIXED
         DO SR=0,SERIES_REAC
            CALL FIRE_FORWARD_EULER(ZZ_NEW,DT_SUB,1,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
            A1 = ZZ_NEW
            IF (ALL(REACTION(:)%FAST_CHEMISTRY) .AND. SR == SERIES_REAC) THEN ! All fast reactions solve explicit Euler
               ...
               EXIT RICH_EX_LOOP
            ENDIF
            CALL FIRE_FORWARD_EULER(A1,DT_SUB,1,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
            A1 = 0.5_EB*(ZZ_0 + A1)
         ENDDO
         !--------------------
         ! Calculate A2 term
         ...
         ZZ_0 = ZZ_MIXED
         ZZ_NEW = ZZ_MIXED
         ODE_LOOP2: DO NSS = 1,2
            DO SR=0,SERIES_REAC
               CALL FIRE_FORWARD_EULER(ZZ_NEW,DT_SUB,2,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
               A2 = ZZ_NEW
               CALL FIRE_FORWARD_EULER(A2,DT_SUB,2,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
               A2 = 0.5_EB*(ZZ_0 + A2)
               ZZ_0 = A2
               ZZ_NEW = A2
            ENDDO
         ...
         ZZ_0 = ZZ_MIXED
         ZZ_NEW = ZZ_MIXED
         ODE_LOOP4: DO NSS = 1,4
            DO SR=0,SERIES_REAC
               CALL FIRE_FORWARD_EULER(ZZ_NEW,DT_SUB,4,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
               A4 = ZZ_NEW
               CALL FIRE_FORWARD_EULER(A4,DT_SUB,4,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
               A4 = 0.5_EB*(ZZ_0 + A4)
               ZZ_0 = A4
               ZZ_NEW = A4
            ENDDO
...
REAL(EB),INTENT(IN)::ZZ_MIXED_IN(0:N_TRACKED_SPECIES),TMP_MIXED_ZONE
REAL(EB):: H_F_0,H_F_N,H_A_0,H_A_N,H_P_0,H_P_N,Z_OX_STOICH(0:N_TRACKED_SPECIES),DZ_FUEL_TOT,DZ_P_TOT,DZ_AIR_TOT,&
           DZ_AIR(0:N_TRACKED_SPECIES),Z_OX(0:N_TRACKED_SPECIES),Z_P(0:N_TRACKED_SPECIES),DZ_FUEL(0:N_TRACKED_SPECIES),&
           Z_F(0:N_TRACKED_SPECIES),DZ_F(0:N_TRACKED_SPECIES),DZ_FRAC_F(1:N_REACTIONS),&
           DZ_P(0:N_TRACKED_SPECIES),HOC_EXTINCT,AIT_EXTINCT,CFT_EXTINCT,DZ_F_SUM
INTEGER :: NS,NR,OX_COUNT(0:N_TRACKED_SPECIES),FUEL_COUNT(0:N_TRACKED_SPECIES)
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
   ...
   IF (RN%FAST_CHEMISTRY .AND. RN%HEAT_OF_COMBUSTION > 0._EB) THEN
      DO NS = 0,N_TRACKED_SPECIES
         IF (RN%NU(NS) < 0._EB .AND. NS /= RN%FUEL_SMIX_INDEX) THEN
            Z_OX_STOICH(NS) = Z_OX_STOICH(NS) + ZZ_MIXED_IN(RN%FUEL_SMIX_INDEX)*RN%S !Stoich mass O2
            Z_OX(NS) = ZZ_MIXED_IN(NS) ! Mass O2 in cell
            OX_COUNT(NS) = OX_COUNT(NS) + 1
            Z_F(RN%FUEL_SMIX_INDEX) = ZZ_MIXED_IN(RN%FUEL_SMIX_INDEX)
            DZ_F(RN%FUEL_SMIX_INDEX) = DZ_F(RN%FUEL_SMIX_INDEX) + MIN(ZZ_MIXED_IN(RN%FUEL_SMIX_INDEX),ZZ_MIXED_IN(NS)/RN%S)
            FUEL_COUNT(RN%FUEL_SMIX_INDEX) = FUEL_COUNT(RN%FUEL_SMIX_INDEX) + 1
         ENDIF
      ENDDO
   ENDIF
ENDDO
IF (SUM(Z_OX_STOICH) > SUM(Z_OX)) DZ_F_SUM = SUM(DZ_F)
DO NR = 1,N_REACTIONS
   RN => REACTION(NR) 
   IF (SUM(Z_OX_STOICH) > SUM(Z_OX) .AND. RN%HEAT_OF_COMBUSTION > 0._EB) THEN 
      DZ_FRAC_F(NR) = DZ_F(RN%FUEL_SMIX_INDEX)/MAX(DZ_F_SUM,TWO_EPSILON_EB)
      DO NS = 0,N_TRACKED_SPECIES
         IF (RN%NU(NS)< 0._EB .AND. NS/=RN%FUEL_SMIX_INDEX) THEN
            DZ_FUEL(RN%FUEL_SMIX_INDEX) = DZ_F(RN%FUEL_SMIX_INDEX)*DZ_FRAC_F(NR)/REAL(OX_COUNT(NS),EB)
            DZ_AIR(NS) = RN%S*DZ_FUEL(RN%FUEL_SMIX_INDEX)/REAL(FUEL_COUNT(RN%FUEL_SMIX_INDEX),EB)
         ENDIF
      ENDDO
   ELSE
      DO NS = 0,N_TRACKED_SPECIES
         IF (RN%NU(NS) < 0._EB .AND. NS/=RN%FUEL_SMIX_INDEX) THEN
            DZ_FUEL(RN%FUEL_SMIX_INDEX) = DZ_F(RN%FUEL_SMIX_INDEX)/REAL(OX_COUNT(NS),EB)
            DZ_AIR(NS) = RN%S*DZ_FUEL(RN%FUEL_SMIX_INDEX)/REAL(FUEL_COUNT(RN%FUEL_SMIX_INDEX),EB)
         ENDIF
      ENDDO
   ENDIF
ENDDO
Z_P = Z_P - DZ_FUEL - DZ_AIR
DZ_FUEL_TOT = SUM(DZ_FUEL)
DZ_AIR_TOT = SUM(DZ_AIR)
DZ_P = Z_P*MIN(DZ_AIR_TOT/SUM(Z_OX),DZ_FUEL_TOT/SUM(Z_F))
DZ_P_TOT = SUM(DZ_P)

! Normalize compositions
DZ_FUEL = DZ_FUEL/(SUM(DZ_FUEL)+TWO_EPSILON_EB)
DZ_AIR = DZ_AIR/(SUM(DZ_AIR)+TWO_EPSILON_EB)
DZ_P = DZ_P/(SUM(DZ_P)+TWO_EPSILON_EB)

DO NR = 1,N_REACTIONS
   RN => REACTION(NR)
   HOC_EXTINCT = HOC_EXTINCT+DZ_FUEL(RN%FUEL_SMIX_INDEX)/REAL(FUEL_COUNT(RN%FUEL_SMIX_INDEX),EB)*RN%HEAT_OF_COMBUSTION
   AIT_EXTINCT = AIT_EXTINCT+DZ_FUEL(RN%FUEL_SMIX_INDEX)/REAL(FUEL_COUNT(RN%FUEL_SMIX_INDEX),EB)*RN%AUTO_IGNITION_TEMPERATURE
   CFT_EXTINCT = CFT_EXTINCT+DZ_FUEL(RN%FUEL_SMIX_INDEX)/REAL(FUEL_COUNT(RN%FUEL_SMIX_INDEX),EB)*RN%CRIT_FLAME_TMP
ENDDO
...
ELSE     
   ...
   CALL GET_SENSIBLE_ENTHALPY(DZ_FUEL,H_F_0,TMP_MIXED_ZONE)
   CALL GET_SENSIBLE_ENTHALPY(DZ_AIR,H_A_0,TMP_MIXED_ZONE)
   CALL GET_SENSIBLE_ENTHALPY(DZ_P,H_P_0,TMP_MIXED_ZONE)  
   CALL GET_SENSIBLE_ENTHALPY(DZ_FUEL,H_F_N,CFT_EXTINCT)
   CALL GET_SENSIBLE_ENTHALPY(DZ_AIR,H_A_N,CFT_EXTINCT)
   CALL GET_SENSIBLE_ENTHALPY(DZ_P,H_P_N,CFT_EXTINCT)
   
   ! See if enough energy is released to raise the fuel and required "air" temperatures above the critical flame temp.
   IF (DZ_FUEL_TOT*H_F_0 + DZ_AIR_TOT*H_A_0 + DZ_P_TOT*H_P_0 + DZ_FUEL_TOT*HOC_EXTINCT < &
      DZ_FUEL_TOT*H_F_N + DZ_AIR_TOT*H_A_N + DZ_P_TOT*H_P_N) EXTINCT_2 = .TRUE.
ENDIF

END FUNCTION EXTINCT_2


SUBROUTINE FIRE_FORWARD_EULER (ZZ_NEW,DT_SUB,DT_RICH,TMP_MIXED_ZONE,EXTINCT,RHO_HAT,SR)
REAL(EB), INTENT(IN) :: DT_SUB,TMP_MIXED_ZONE,RHO_HAT
LOGICAL, INTENT(IN) :: EXTINCT(1:N_REACTIONS)
REAL(EB), INTENT(INOUT) :: ZZ_NEW(0:N_TRACKED_SPECIES)
INTEGER, INTENT(IN) :: DT_RICH,SR
INTEGER :: NR,NS
REAL(EB) :: DZZDT_TEMP(0:N_TRACKED_SPECIES),DZZDT(0:N_TRACKED_SPECIES),RATE_CONSTANT(1:N_REACTIONS),ZZ_0(0:N_TRACKED_SPECIES)
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
...
DZZDT_TEMP = 0._EB
RATE_CONSTANT = 0._EB
CALL COMPUTE_RATE_CONSTANT(RATE_CONSTANT,ZZ_0,RHO_HAT,DT_SUB,TMP_MIXED_ZONE,EXTINCT)
REACTION_LOOP: DO NR = 1,N_REACTIONS
   RN => REACTION(NR)
   IF (.NOT.RN%FAST_CHEMISTRY .AND. SR < SERIES_REAC) RATE_CONSTANT(NR) = 0._EB
   DZZDT_TEMP = RN%NU_MW_O_MW_F*RATE_CONSTANT(NR)
   ZZ_NEW = ZZ_0 + (DZZDT_TEMP+DZZDT)*DT_SUB/REAL(DT_RICH,EB)
   IF (ANY(ZZ_NEW < 0._EB)) THEN
      DO NS=0,N_TRACKED_SPECIES
         IF (ZZ_NEW(NS) < 0._EB .AND. ABS(DZZDT_TEMP(NS))>TWO_EPSILON_EB) &
            RATE_CONSTANT(NR) = MIN(RATE_CONSTANT(NR),ZZ_0(NS)/(ABS(RN%NU_MW_O_MW_F(NS))*DT_SUB/REAL(DT_RICH,EB)))
      ENDDO
      DZZDT_TEMP = RN%NU_MW_O_MW_F*RATE_CONSTANT(NR)
   ENDIF
   DZZDT = DZZDT+DZZDT_TEMP
ENDDO REACTION_LOOP
ZZ_NEW = ZZ_0 + DZZDT*DT_SUB/REAL(DT_RICH,EB)
