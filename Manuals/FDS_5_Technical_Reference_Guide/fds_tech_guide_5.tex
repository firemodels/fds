...
heats of reaction and the differences in latent heats between the original
material and the products
\be
\dot{q}_{s,c}'''(T) = \rho_{s0}\;
    \sum_{\alpha=1}^{N_m} \sum_{\beta=1}^{N_{r,\alpha}}
    r_{\alpha \beta} \;
    \left[H_{v,\alpha \beta}-
    \int_{T_0}^T \Delta c(\theta)d\theta \right]
\ee
