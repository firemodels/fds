...
{\ct SUPPRESS\_DIAGNOSTICS}         & Logical      & Section~\ref{out:file}                 &           & {\ct .FALSE.}$^*$              \\ \hline
...
$^*$ {\ct SUPPRESS\_DIAGNOSTICS} is {\ct .TRUE.} if the number of meshes is 10 or above.
...
For jobs involving multiple meshes, the diagnostic output file can become very large and potentially impact the efficiency of the parallel processing. For this reason, the full diagnostic output is suppressed for jobs involving 10 or more meshes. Instead of the diagnostic output shown above, only the Time Step number, Total Time, and Time Step size are printed out. To return to full diagnostics, set {\ct SUPPRESS\_DIAGNOSTICS} to {\ct .FALSE.} on the {\ct DUMP} line.  
