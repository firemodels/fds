...
PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   ! Set initial LPC%LENGTH for screen drag or porous media drag, will be re-scaled later
   IF (SF%GEOMETRY==SURF_CARTESIAN) THEN
      IF (LPC%DRAG_LAW==SCREEN_DRAG) LPC%LENGTH = SQRT(LPC%FREE_AREA_FRACTION)
      IF (LPC%DRAG_LAW==POROUS_DRAG) LPC%LENGTH = SQRT(0.5_EB*LPC%POROUS_VOLUME_FRACTION/SF%THICKNESS)
   ELSEIF (SF%GEOMETRY==SURF_CYLINDRICAL) THEN
      IF (LPC%DRAG_LAW==SCREEN_DRAG) LPC%LENGTH = 0.5_EB*LPC%FREE_AREA_FRACTION/(SF%INNER_RADIUS + SF%THICKNESS)
      IF (LPC%DRAG_LAW==POROUS_DRAG) LPC%LENGTH = LPC%POROUS_VOLUME_FRACTION/ &
   ...
   IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      ...
      DO NN=1,SF%N_CELLS_INI
         SELECT CASE (SF%GEOMETRY)
            CASE (SURF_CARTESIAN)
               AREA_FACTOR = 2._EB*SF%LENGTH*SF%WIDTH
               IF (LPC%DRAG_LAW==SCREEN_DRAG .OR. LPC%DRAG_LAW==POROUS_DRAG) AREA_FACTOR = 2._EB*LPC%LENGTH**2._EB
            ...
            CASE (SURF_CYLINDRICAL)
               AREA_FACTOR = PI*SF%LENGTH
               IF (LPC%DRAG_LAW==SCREEN_DRAG .OR. LPC%DRAG_LAW==POROUS_DRAG) AREA_FACTOR = PI*LPC%LENGTH
