...
Forced convection uses a Nusselt number correlation using a Reynolds number, $\RE$, computed using the local gas conditions and the characteristic length of the surface or particle.  The Nusselt number, $\NU$, depends on the shape of the obstruction~\cite{Holman:1, Incropera:1}:
...
\begin{equation}
h = \frac{\dot{q}''_{\rm w}}{\Delta T} = \frac{\rho_{\rm w} \, c_{\rm p} \, u_\tau}{T^+}
\end{equation}
...
where
\be
  r_{ij} = A_{ij} \; Y_{{\rm s},i}^{n_{{\rm s},ij}} \; \exp \left(-\frac{E_{ij}}{R \, T_{\rm s}} \right) X_{\rm O_2,g}^{n_{{\rm O_2},ij}}
...
The fourth term of the reaction rate equation (\ref{rr}) can be used to simulate oxidation reactions. $X_{\rm O_2,g}$ is the oxygen volume 
fraction in the gas phase. If the heterogeneous reaction order $n_{{\rm O_2},ij}$ is greater than zero, the reaction rate is affected 
by the presence of oxygen in the first gas phase cell next to the solid surface. $n_{{\rm O_2},ij}$ is prescribed under the name {\ct N\_O2(j)} on the 
{\ct MATL} line of the $i$th material. It is zero by default.
