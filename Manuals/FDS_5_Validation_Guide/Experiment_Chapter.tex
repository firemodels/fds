...
At this point in the analysis, we have a set of model predictions and a set experimental measurements. We will refer to the model predictions as
$M_i$, where $i$ ranges from 1 to $n$. The experimental measurements are likewise referred to as $E_i$. Without loss of generality, assume that the
ambient value of the quantity under consideration is zero. First, assume that $E|\theta \sim N(\theta,\sigma_E^2)$, meaning that the probability distribution of $E$
...
In the analysis to follow, it is convenient to work with $\ln(E_i)$ because the standard deviation of $\ln(E)$ is approximately $\tilde{\sigma}_E$. 
...
Now, we actually have a number of pairs   , one for each experiment. Denote this by a subscript i, that is, there are now n pairs  . The formulation from above can be extended to assume that

  and  for i=1,…,n. This will lead to   for i=1,…,n.
...
The usual least squares estimate of   is   and the estimate of  is
...
Then the least squares estimate of the standard deviation  of   , that is of   , is


 .                                       (5)


Note that this quantity is the total relative uncertainty of  .

The model-intrinsic component   can be estimated by setting  and solving for   as
