...
../configure  --enable-fortran --enable-parallel CXX="$(which mpiicpc) -cc=$(which icpx)" CC="$(which mpiicc) -cc=$(which icx)" FC="$(which mpiifx) -fc=$(which ifx)" --prefix=$HDF5_INSTALL_PREFIX
else
../configure --enable-fortran --enable-parallel CXX="$(which mpicxx)" CC="$(which mpicc)" FC="$(which mpifort)" --prefix=$HDF5_INSTALL_PREFIX
fi
