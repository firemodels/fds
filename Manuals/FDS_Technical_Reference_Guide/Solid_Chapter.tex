...
\be
\dot{m}'' =h_{\rm m} \rho_{f,g} log\left(\frac{X_{\rm g}-1}{X_{\rm f}-1}\right).
\ee
Here $h_{\rm m}=Sh\mu_{\rm g}/Sc\Delta~x$ is the mass transfer coefficient and $\rho_{f,g}$ and $X_{\rm g}$ are the density of the fuel vapour and the volume fraction of fuel vapour in the grid cell adjacent to the pool surface. The Schmidt number of air $Sc$ is  $0.6$ and the Sherwood numer is given by
\be
Sh=0.037~Sc^{\frac{1}{3}} Re^{\frac{4}{5}} .
\ee
The Reynolds number $Re=\rho_{\rm g} v_{\rm g}/L~x\mu_{\rm g}$ is calculated based on conditions in the cell adjacent to the surface. The length scale used in calculating the Reynolds number is 1 unless otherwise specified and is the same length scale used for convective heat transfer calculation.
