...
{\ct A(j)}, with units of s$^{-1}$. $E_{ij}$, the activation energy, is
...
specify {\ct REFERENCE\_RATE} (s$^{-1}$) and {\ct REFERENCE\_TEMPERATURE}($^\circ$C). This directs FDS to calculate the activation energy, {\ct E}, using
the specified reference rate, reference temperature, and an assumed value of $A=1\times 10^{13}$~s$^{-1}$. Essentially, this guarantees that
the {\ct REFERENCE\_RATE} is achieved at the {\ct REFERENCE\_TEMPERATURE}. The default value of {\ct REFERENCE\_RATE} is
...
specify the {\ct REFERENCE\_TEMPERATURE}. Note that the {\ct REFERENCE\_TEMPERATURE} is {\em not} the same as an ignition
...
{\ct REFERENCE\_RATE}         & Real        & Reaction rate at ref.~temp.    & s$^{-1}$         & 0.1    \\ \hline
