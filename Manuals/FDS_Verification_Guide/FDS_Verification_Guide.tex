...
\begin{table}
\begin{center}
\caption{\label{tab_zplus} \small Case matrix and friction factor results for turbulent channel flow with smooth walls.
...
\begin{figure}[ht]
   ...
   \end{tabular}
         \caption{(Left) Time series of $\rho$ at the position $(x,y) = (3\pi/2, 3\pi/2)$ for several grid resolutions using the Superbee limiter. (Right) Convergence plot for central differencing, Superbee, and CHARM.  All schemes are second-order accurate.}
...
\end{tabular*}
\caption[The symmetry\_test case.]{Flow field (left) and velocity component plot (right) for the symmetry\_test case.}
\label{symmetry_test}
\end{figure}
...
This case tests the conservation of energy that is absorbed by liquid droplets. We want to make sure that the energy that is taken away from
the thermal radiation field is accurately converted into increased temperature of the droplets. Considering the total mass of
absorbing droplets, the average droplet temperature $\bar{T_d}$ is increased by the absorbed radiation $\dot{Q}_r$ according to the following ODE
\be
m c_p \frac{d \bar{T_d}}{dt} = \dot{Q}_r
\ee
where $m$ and $c_p$ are the total mass and specific heat of the droplets, respectively. Here we have assumed that the convective heat transfer between the
gas and droplets is small. (This is implemented by increasing the gas phase Prandtl number into a artificially high value.)
If the absorbed power remains constant during the time step, the average droplet temperature in the end should be
\be
\bar{T_d} = T_0 + dt \int\dot{Q}_r dV / (m c_p)
\ee
By setting $T_0=$ 0$^\circ$C, $dt =$ 0.01~s, $m =$ 0.01~kg and $c_p =$ 1.0~kJ/kg, the value of $\bar{T_d}$ should be equal to the volumetric integral of
$\dot{Q}_r$. The following two plots show comparison of the predicted average droplet temperature and the analytical value in cartesian and cylindrical
co-ordinate systems.
\noindent
\begin{figure}[h]
\begin{tabular*}{\textwidth}{lr}
...
\end{tabular*}
\caption[Radiation absorption by liquid droplets]{Conservation of absorbed thermal radiation energy into increased average droplet temperature.}
...
\section{Fractional Effective Dose (\texorpdfstring{{\bf FED\_Device}}{FED\_Device}) }
...
\noindent
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/FED_Device_O2} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/FED_Device_CO} \\
\includegraphics[width=3.2in]{SCRIPT_FIGURES/FED_Device_CO2_CO} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/FED_Device_All}
\end{tabular*}
...
The expected and computed results for charring material are compared in~\ref{surf_mass_vent_char}.
The expected and computed results for non-charring material are compared in~\ref{surf_mass_vent_nonchar}.
\begin{figure}[ht!]
\noindent
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
...
\end{tabular*}
\caption[The {\bf surf\_mass\_part\_X\_char\_X} test cases.]{Comparison of analytical mass change and simulated mass changes for charring surfaces associated with particles.}
...
\end{tabular*}
\caption[The {\bf surf\_mass\_part\_X\_nonchar\_X} test cases.]{Comparison of analytical mass change and simulated mass changes for non-charring surfaces associated with particles.}
...
This is a verification test case for a phase change problem where a semi-infinite layer of liquid in the region $x>0$
is cooled down by setting the surface $x=0$ to a constant temperature $T_0$
below the freezing temperature $T_f$. Initially, the material is at temperature $T_i > T_f$. The position of the boundary between frozen and melt layers
is denoted by $x_f$. The heat conduction equations for the two regions are
\begin{eqnarray}
\dod{T_s}{t} & = & \alpha_s \dsods{T_s}{x}\mbox{,} \;\; (0 < x < x_f,\; t > 0) \\
\dod{T_s}{t} & = & \alpha_l \dsods{T_s}{x}\mbox{,} \;\; (x > x_f ,\; t > 0)
\end{eqnarray}
...
\ee
\begin{eqnarray}
\frac{T_s(x,t)-T_0}{T_f - T_0} &=& \frac{\erf(x/2\sqrt{\alpha t})} {\erf (x_f/2\sqrt{\alpha t})}\mbox{,}\;\; (0 < x < x_f) \\
\frac{T_i-T_s(x,t)}{T_i - T_f} &=& \frac{\erfc(x/2\sqrt{\alpha t})}{\erfc(x_f/2\sqrt{\alpha t})}\mbox{,}\;\; (x > x_f)
\end{eqnarray}
where $\lambda = x_f /2\sqrt{\alpha t}$ is obtained as a root of the (complicated)
trancendential equation, given in \cite{Carslaw:1}.

In the test case, the thickness of the layer cannot be infinite. For practical purposes, a 50 cm thick layer was found to be sufficient.
The fixed-temperature boundary condition, which is formally not available for the condenced-phase heat transfer solver,
is implemented as convection to $T_0$ ambient with very high convection coefficient.
For material properties, we choose $\rho_s = \rho_l =$ 1000 kg/m$^3$,
$k_s = k_l = 2$ W/Km, $c_s = c_l = 2$ kJ/kgK, $H_{r,\alpha\beta}=300$ kJ/kg, $T_f = 0^\circ$C, $T_i = +15^\circ$C,
...
Analytical and simulated results are compared in the figures below. Left side shows the position of the phase boundary.
From FDS results, two estimates of $x_f$ are determined because in the nummerical solution, the boundary of the two phases is not perfectly sharp.
''FDS min'' is the largest $x$ for which $\rho_s > 990$ kg/m$^3$ and ''FDS max'' is the largest $x$ for which $\rho_s > 0$ kg/m$^3$. The right hand side
figure shows temperatures at three depths from the surface: 1 cm, 5 cm and 10 cm.
\begin{figure}[ht!]
\noindent
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
...
\end{tabular*}
\caption[The {\bf pcm\_slab} test cases.]{Comparison of analytical and simulated results for the two-region Neuman problem with phase change.}
\label{pcm_slab}
\end{figure}
...
The test case called {\bf water\_evaporation} involves stationary water droplets in a box with dimensions of 1~m on a side. The walls of the box are
assumed adiabatic, meaning that there are no leaks or heat losses. The air within the box is stirred to maintain uniform conditions. Initially, the
...
mass of water droplets is 0.01~kg. The initial mass fraction of water vapor is 0. It is expected that the droplets will all evaporate within about 10~s. Figure~\ref{water_evaporation_plots} displays
the average enthalpy, humidity, density, pressure, temperature and mass of water vapor of the gas within the box. The solid horizontal lines denote the expected
steady-state values. The plot of enthalpy displays the change in enthalpy of both the droplets and the gas. The change in the enthalpy of the water droplets should equal the change in the enthalpy of the gas minus the work
...
\be de = dh - \frac{1}{\rho} \; dp - p \; d \left(\frac{1}{\rho}
\right) = dh - v \; dp - p \; dv \ee
...
\noindent
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/water_evaporation_enthalpy} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/water_evaporation_humidity} \\
\includegraphics[width=3.2in]{SCRIPT_FIGURES/water_evaporation_density} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/water_evaporation_temperature} \\
\includegraphics[width=3.2in]{SCRIPT_FIGURES/water_evaporation_pressure} &
\includegraphics[width=3.2in]{SCRIPT_FIGURES/water_evaporation_water_mass}
\end{tabular*}
...
\begin{figure}[ht]
   \begin{center}
     ...
     \caption{\label{fig_HVAC_loss} \small Duct velocities with varied duct flow loss.}
   \end{center}
\end{figure}
...
A 1~m by 1~m by 1~m box is divided into two equal compartments by a vertical wall, see Figure~\ref{fig_HVAC_e_p}.  The ambient species is defined with a molecular weight of 28 g/mol, a specific heat of 1 kJ/kg/K and a specific enthalpy of 298.15 kJ/kg at the reference temperature of 298.15~K.  The left side of the compartment is initialized to twice the ambient temperature.  A simple HVAC system of two 0.1 m$^2$ ducts is created.  The first duct takes suction from the lower left half and discharges into the lower right half, and this duct is given a fixed volume flow rate 0.1 m$^3$/s.  The second duct is placed in the upper half of the compartment and is assigned a flow loss of 1 and an area of 0.01 m$^2$.  It is expected that the top and bottom duct velocities will be 10 m/s and 1 m/s respectively, the total energy will be conserved, that at the start of the computation hot gas will be seen entering the lower right and cold gas entering the upper left, and that at the end of the computation the pressure difference between the two sides will be 43.6 Pa based on Eq.~\ref{HVAC_steady_state}
...
\section{Leakage: {\bf leak\_test} and {\bf leak\_test\_2}}
...
Two compartments are defined: A 5 m x 4 m x 3. 2 m compartment containing a second 3.6 m x 2.4 m x 2.4 m compartment, see Figure~\ref{fig_HVAC_leak_fan}.  The interior compartment is connected to the exterior compartment both by a fan (quadratic fan curve with a maximum volumetric flow of 0.005 m$^3$/s and a maximum pressure of 1000 Pa) and by an 0.0001 m$^2$ leakage path.  The interior compartment is also connected to the ambient by an 0.0001 m$^2$ leakage path.  The fan is defined with a 1 s tanh ramp to reach full power.  When the simulation starts the fan will begin to remove air from the outer compartment and discharge it into the inner compartment.  As the pressure rises, the leakage flows will slowly increase until the leakage flow is equal to the flow from the fan.  Since some air from the interior compartment is being discharged to the ambient, the average pressure of the two compartments will decrease until the pressure of the interior compartment is equal to that of the ambient.  At this point in time, no more leakage flow will occur to the ambient and the fan flow will equal intra-compartment leakage flow.  Given the leakage area and the fan curve, this will occur when the pressure difference between the two compartments is 598 Pa.
...
ASRHAE Sample Problem 7~\cite{ASHRAE} is a simple HVAC system that represents an exhaust system for a machine shop where particulate matter from three pieces of equipment are removed by a dust collector.  The network, shown in Figure~\ref{ASHRAE7_schematic} consists of three inlets, two tees, the dust collector, and a fan.  The lengths, diameters, and friction losses for all the components were defined as well as the fan flow rate.  Pressure drops and flow rates in the remaining components can be computed by hand.


\begin{figure}[ht]
\begin{center}
\begin{tabular}{l}
\includegraphics[width=5.0in]{SCRIPT_FIGURES/ashrae_7_dotplot.pdf}
\end{tabular}
\end{center}
