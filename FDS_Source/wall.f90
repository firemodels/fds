...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   DO N=1,N_TRACKED_SPECIES
      IF (LES) THEN
         WC%RHODW(N) = MU(WC%ONE_D%IIG,WC%ONE_D%JJG,WC%ONE_D%KKG)*RSC*WC%RHO_F/RHOP(WC%ONE_D%IIG,WC%ONE_D%JJG,WC%ONE_D%KKG)
      ELSE
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   CALL HEAT_TRANSFER_BC(SURF_INDEX,T,WC%ONE_D%TMP_F,WALL_INDEX=IW) 
   WC=>WALL(IW)
...
IF (VIRTUAL_PARTICLES) THEN      
   DO II = 1, NLP         
      ...
      IF (SURF_INDEX/=DROPLET_SURF_INDEX .AND. SURF_INDEX/=MASSLESS_PARTICLE_SURF_INDEX) THEN
         CALL HEAT_TRANSFER_BC(SURF_INDEX,T,LP%ONE_D%TMP_F,PARTICLE_INDEX=II)
...
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
INTEGER :: I,II,JJ,KK,IIG,JJG,KKG,IW,NS,N,NWP,SURF_INDEX
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
IF (VIRTUAL_PARTICLES) THEN
   ...
   PARTICLE_LOOP: DO I=1,NLP
      ...
      CALL CALC_SPECIES_BC(T,PARTICLE_INDEX=I)
      ...
      DO NS=0,N_TRACKED_SPECIES
         ...
         IF (ENTHALPY_TRANSPORT) THEN
            CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S_B,LP%ONE_D%TMP_F)
...
USE MATH_FUNCTIONS, ONLY : EVALUATE_RAMP
REAL(EB), INTENT(IN) :: T
REAL(EB) :: ZZ_G,UN,DD,MFT,TSI,RADIUS,AREA_SCALING,&
...
CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
   ...
   IF (PRESENT(WALL_INDEX)) THEN
      ...
      DO ITER=1,5
         ...
         WC%RHO_F = PBAR_S(KK,WC%PRESSURE_ZONE_WALL)/(RSUM_F*WC%ONE_D%TMP_F)
      ENDDO
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) WC%RHO_F = PBAR_P(KK,WC%PRESSURE_ZONE_WALL)/(RSUM_F*WC%ONE_D%TMP_F) 
...
IF (PRESENT(PARTICLE_INDEX)) THEN
   IIG = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%ONE_D%IIG
   JJG = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%ONE_D%JJG
   KKG = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%ONE_D%KKG
   ...
   IOR = LAGRANGIAN_PARTICLE(PARTICLE_INDEX)%ONE_D%IOR
ELSEIF (PRESENT(WALL_INDEX)) THEN
   IF (WALL_INDEX ==-1) THEN !Void
      IIG = -1 
      IOR = 0
   ELSE   
      IIG = WALL(WALL_INDEX)%ONE_D%IIG
      JJG = WALL(WALL_INDEX)%ONE_D%JJG
      KKG = WALL(WALL_INDEX)%ONE_D%KKG
      IOR = WALL(WALL_INDEX)%ONE_D%IOR
...
IF (DNS) THEN
   IF (PRESENT(PARTICLE_INDEX)) THEN
      ...
      HEAT_TRANSFER_COEFFICIENT = 2._EB*KP*RDN
   ELSEIF (PRESENT(WALL_INDEX)) THEN
