...
INTEGER :: IOS,IZERO,N_HVAC_READ,N,ND,ND2,NM,NN,NF,NV,ITMP,I_DUCT=0,I_DUCTNODE=0,I_FAN=0 !, &
!            AIRCOIL_INDEX,I_AIRCOIL=0,FAN_INDEX, INDEX,I_FILTER=0
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(MAX_DUCTS,MAX_DUCTS),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW,RHOA,TMP_WGT
...
INTEGER :: NN,II,JJ,KK,I1,I2,J1,J2,K1,K2,IC,IW,IOR,NODE_ZONE,ITMP
REAL(EB) :: YY_SUM(N_SPECIES),YY_N(1:N_SPECIES),RHO_SUM,P_SUM,AREA_SUM,AREA,TMP_SUM,H_SUM,H_G,TMP_NEW,CP,TNOW,VEL2,P_AVE,TMP_WGT
...
DO NN = 1, N_DUCTNODES
   ...
   IF (NM==1 .AND. DN%AMBIENT) THEN
      ...
      IF (N_SPECIES > 0) THEN
         DN%YY(1:N_SPECIES) = SPECIES(1:N_SPECIES)%YY0
         YY_N = DN%YY
         CALL GET_AVERAGE_SPECIFIC_HEAT(YY_N,DN%CP,DN%TMP)
         CALL GET_SPECIFIC_GAS_CONSTANT(YY_N,DN%RSUM_V)
      ELSE
         ITMP = MIN(4999,INT(DN%TMP))
         TMP_WGT = DN%TMP - ITMP
         DN%CP_V = Y2CP_C(ITMP)+TMP_WGT*(Y2CP_C(ITMP+1)-Y2CP_C(ITMP))
         DN%RSUM_V = RSUM0
      ENDIF
