...
ELSE
   ...
   VV   => VS
   WW   => WS
   IF (.NOT.EVACUATION_ONLY(NM)) ZZP => ZZS
ENDIF

! Compute viscosity for DNS using primitive species/mixture fraction

IF (EVACUATION_ONLY(NM)) ZZ_GET(1:N_TRACKED_SPECIES) = 0._EB
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         IF (.NOT.EVACUATION_ONLY(NM)) ZZ_GET(1:N_TRACKED_SPECIES) = ZZP(I,J,K,1:N_TRACKED_SPECIES)
