...
\noindent
define a brick wall that is 4.9~m long, 1~m high, and 20~cm thick. Note that the thickness of the wall indicated by the {\ct OBST} line is indepentdent of the {\ct THICKNESS} specified by the {\ct SURF} line.  The {\ct OBST} line defines the geometry of the obstruction (i.e. how the obstruction is seen by the flow solver).  The {\ct SURF} line defines the heat transfer characterstics of the the obstruction (i.e. how the obstruction is seen by the 1D solid phase solver).  This allows an obstruction to snap to the local grid but still have the heat trasnfer solution reflect the actual thickness.
...
By default in an LES calculation, the convective heat flux to a surface or a particle is obtained from a combination of natural and forced convection correlations.

The natural convection correlation used is:
\be \dq_c'' = h \; \Delta T
...
and 1.31 for a vertical surface, by default).  You can change the empirical coefficients for the natural convection model by using {\ct C\_HORIZONTAL} or {\ct C\_VERTICAL} for $C$ which are input on the {\ct SURF} line.

Forced convection uses a Nusselt number correlation using a Reynolds number, $\RE$, computed using the local gas conditons and the characteristic length of the surface or particle.  The Nusselt number, $\NU$, depends on the shape of the obstruction~\cite{Holman:1}, ~\cite{Incropera:1}:
...
\end{center}
\end{table}
$L$ is a characteristic length related to the size of the physical obstruction; $k$ is the thermal conductivity of the gas, and the Reynolds $\RE$ and Prandtl $\PR$ numbers are based on the gas flowing past the obstruction (i.e. for a wall cell the conditions in gas cell immeadiately adjacent to the wall and for a particle the gas cell containing the particle). Since the Reynolds number is proportional to the characteristic length, $L$, the heat transfer coefficient is weakly related to $L$ (for high $\RE$, $h \sim L^{-1/5}$). For this reason, for a wall cell $L$ is taken to be 1~m for most calculations.  For cylindrical or spherical objects $L$ is take to be the diameter. The length scale can be changed for a specific surface by using {\ct CONVECTION\_LENGTH\_SCALE} on the {\ct SURF} line. You can change the empirical coefficients for the forced convection model by using {\ct C\_FORCED\_CONSTANT}, {\ct C\_FORCED\_RE}, {\ct C\_FORCED\_RE\_EXP}, and {\ct C\_FORCED\_PR\_EXP} for the constants $C_1$ through $C_4$ in the Nusselt number correlation, all of which are input on the {\ct MISC} line.  
...
If you have an {\ct OBST} that is one cell thick with gas cells on both sides (i.e. the obstruction is not at the edge of the domain) and you apply the attribute {\ct BACKING='EXPOSED'}, then FDS calculates the heat conduction through the entire {\ct THICKNESS} and it uses the gas phase temperature and heat flux on the front and back sides for boundary conditions. A redundant calculation is performed on the opposite side of the obstruction.  FDS always applies a {\ct SURF} to an obstruction by having the first layer be the exposed surface of the face and the last layer as the opposite face.  Take for example the {\ct SURF} defintion below and assume the the grid spacing is 10 cm.  On the -x side of the {\ct OBST} layer 1 will be {\ct MATERIAL A}, layer 2 will be {\ct MATERIAL B}, and layer 3 will be the last {\ct MATERIAL A}.  On the +x side the {\ct SURF} will be applied in the same manner.
...
Now for example take the  {\ct SURF} defintion below and assume the the grid spacing is 10 cm.  On the -x side of the {\ct OBST} layer 1 will be {\ct MATERIAL A}, layer 2 will be {\ct MATERIAL B}, and layer 3 will be {\ct MATERIAL C}.  On the +x side the {\ct SURF} will be applied in the same manner, layer 1 will be {\ct MATERIAL A}, layer 2 will be {\ct MATERIAL B}, and layer 3 will be {\ct MATERIAL C}.  This means that both sides of the {\ct OBST} will compute heat transfer assuming {\ct MATERIAL A} is the first layer.
...
You need only specify the chemical formula of the fuel along with the yields of CO and soot, and the volume fraction of hydrogen in the soot, $X_\Hy$.
...
\nu_s          &=& \frac{W_{\rm F}}{W_{\rm S}} \, y_{\rm S}  \\
\nu_{\NTWO}    &=& \frac{\rm v}{2}  \\
...
&REAC FUEL = 'METHANE' /
\end{lstlisting}
In this case, there is no need for a {\ct FORMULA} or atom count because the {\ct FUEL} is listed in Table~\ref{tab:gasspecies}. It is assumed that the soot and CO yields are zero.
\begin{lstlisting}
&REAC FUEL               = 'PROPANE'
      SOOT_YIELD         = 0.01
      CO_YIELD           = 0.02
...
\end{lstlisting}
In this case, the heat of combustion is specified explicitly rather than calculated. The soot yield is specified as 0.01 and the CO yield is specified as 0.02.
...
\end{lstlisting}
In this case, the {\ct FORMULA} and heat of combustion are known, but the product yields are not.
...
\be
   \dq'''_{\hbox{\tiny max}} = \frac{ \hbox{\ct HRRPUA\_SHEET} }{\dx} + \hbox{\ct HRRPUV\_AVERAGE}
\ee
...
If a more complex spray pattern is desired than one characterized by a {\ct SPRAY\_ANGLE}, then a \linebreak[4]{\ct SPRAY\_PATTERN\_TABLE} can be specified using the {\ct TABL} (Section~\ref{info:TABL})
...
\noindent
where n is the time step, $x$ is the instantaneous device output and $\bar{x}$ is the smoothed output.  The \linebreak[4]{\ct SMOOTHING\_FACTOR} defaults to 0 which means no smoothing is performed.
...
\be D \equiv - \frac{1}{L} \, \log_{10} \left( \frac{I}{I_0} \right) = K \, \log_{10} {\rm e}   \ee
...
surface, $\dq_{\rm c}'' + \dq_{\rm r}''$, use the {\ct QUANTITY} called {\ct 'NET HEAT FLUX'}. The individual components, the {\em net} convective and
...
the latter is the amount of energy that would be absorbed if the surface were cold (or some specified temperature $T_{\rm G}$):
\be
...
than ambient, set {\ct GAUGE\_TEMPERATURE} ($T_{\rm G}$) on the {\ct PROP} line
...
\be \dot{q}_{\rm r}'' + \dot{q}_{\rm c}'' = \epsilon \sigma \, \left(
...
\begin{eqnarray}
\mbox{MTR}(\mathbf{x},t) = \frac{k_{\rm sgs}}{k_{\rm res} + k_{\rm sgs}}
\end{eqnarray}
where
\vskip-.6cm
\begin{eqnarray}
k_{\rm res} &=& \mbox{$\frac{1}{2}$}\tilde{u}_i \tilde{u}_i \\
k_{\rm sgs} &=& \mbox{$\frac{1}{2}$}(\tilde{u}_i - \hat{\tilde{u}}_i)(\tilde{u}_i - \hat{\tilde{u}}_i)
\end{eqnarray}
...
\begin{eqnarray}
\mbox{MSR}(\mathbf{x},t) = \frac{T_{\rm sgs}}{T_{\rm res} + T_{\rm sgs}}
\end{eqnarray}
where
\vskip-.6cm
\begin{eqnarray}
T_{\rm res} &=& \tilde{\varphi}^2 \\
T_{\rm sgs} &=& (\tilde{\varphi} - \hat{\tilde{\varphi}})^2
\end{eqnarray}
