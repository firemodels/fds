...
IMPLICIT NONE
PRIVATE
CHARACTER(255), PARAMETER :: wallid='$Id: wall.f90,v 1.37 2007/04/26 12:50:34 mcgratta Exp $'
PUBLIC WALL_BC
...
HEAT_FLUX_LOOP: DO IW=1,NWC
   ...
   METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
      ...
      CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
         ...
         DTMP = TMP_G - TMP_F(IW)
         CALL GET_HEAT_TRANSFER_COEFFICIENT(IW,IIG,JJG,KKG,IOR,TMP_G,DTMP)
...
LOGICAL :: POINT_SHRINK, RECOMPUTE
...
WALL_CELL_LOOP: DO IW=1,NWC
   ...
   SELECT CASE(SF%BACKING)
      CASE(VOID)  ! Non-insulated backing to an ambient void
         ...
         IF (DNS) THEN
            HTCB   = 2._EB*KW(IW)*RDN(IW)
         ELSE
            IF (ABS(IOR)==3) THEN
               HTCB   = HCH*ABS(DTMP)**ONTH
            ELSE
               HTCB   = HCV*ABS(DTMP)**ONTH               
            ENDIF
         ENDIF
      ...
      CASE(EXPOSED)  
         ...
         IF (BOUNDARY_TYPE(IWB)==SOLID_BOUNDARY) THEN
            ...
            ELSE
               QINB = 0._EB
            ENDIF
            CALL GET_HEAT_TRANSFER_COEFFICIENT(IWB,IIB,JJB,KKB,IOR,TMP_G_B,TMP_G_B-TMP_B(IW))
            HTCB = HEAT_TRANS_COEF(IWB)
         ELSE
            QINB = 0._EB
            TMP_G_B  = TMPA
...
!      RSUM_S  = 0.5*(RSUM_W+RSUM_G)
   ...
   RECOMPUTE_GRID: IF (WC%SHRINKING) THEN
      ...
      IF (RECOMPUTE) THEN
         ...
         CALL GET_WALL_NODE_WEIGHTS(NWP_NEW,SF%N_LAYERS,N_LAYER_CELLS_NEW,THICKNESS,SF%GEOMETRY, &
            X_S_NEW(0:NWP_NEW),RDX_S(0:NWP_NEW+1),RDXN_S(0:NWP_NEW),DX_WGT_S(0:NWP_NEW),DXF,DXB,LAYER_INDEX)
         ...
         CALL GET_INTERPOLATION_WEIGHTS(SF%N_LAYERS,NWP,NWP_NEW,WC%N_LAYER_CELLS,N_LAYER_CELLS_NEW, &
                                    WC%X_S(0:NWP),X_S_NEW(0:NWP_NEW),RDX_S(1:NWP_NEW),INT_WGT(1:NWP_NEW,1:NWP))
   ...
   DO I=1,NWP
      CCS(I) = WC%TMP_S(I) - AAS(I)*(WC%TMP_S(I+1)-WC%TMP_S(I)) + BBS(I)*(WC%TMP_S(I)-WC%TMP_S(I-1)) + DT_BC*Q_S(I)/RHOCBAR(I)
   ENDDO
   IF (SF%INTERNAL_RADIATION) THEN
   ...
   TMP_F(IW)       = 0.5_EB*(WC%TMP_S(0) +WC%TMP_S(1))
...
SUBROUTINE GET_HEAT_TRANSFER_COEFFICIENT(IW,IIG,JJG,KKG,IOR,TMP_G,DELTA_TMP)
...
IF (H_FIXED >= 0._EB) THEN
   HEAT_TRANS_COEF(IW) = H_FIXED
   RETURN
ENDIF

IF (DNS) THEN
   HEAT_TRANS_COEF(IW) = 2._EB*KW(IW)*RDN(IW)
ELSE
   SELECT CASE(ABS(IOR))
      CASE(1)
...
END SUBROUTINE GET_HEAT_TRANSFER_COEFFICIENT
