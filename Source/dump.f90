...
DO N=1,M%N_SLCF
   ...
   ! write out slice file info to the .smv file
      ...
      IF (SL%ID/='null') THEN
         WRITE(SLICELABEL,'(A,A,A,A,A,A)') ' # ',TRIM(SL%SLICETYPE),' %',TRIM(SL%ID),TRIM(SLICEPARMS),SLICEOFFSET
      ELSE
         WRITE(SLICELABEL,'(A,A,A,A)') ' # ',TRIM(SL%SLICETYPE),TRIM(SLICEPARMS),SLICEOFFSET
      ENDIF
      IF (SL%SLICETYPE=='STRUCTURED') THEN
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(40) ! PRESSURE COEFFICIENT TEST
      ...
      JJG = ONE_D%JJG
      KKG = ONE_D%KKG
      KSGS = (MU(IIG,JJG,KKG)*WC%ONE_D%RDN/(RHO(IIG,JJG,KKG)*C_DEARDORFF))**2
