...
REAL(EB) XLOC,YLOC,ZLOC
INTEGER NCT,NLP_TREE,NLP_RECT_VEG,NXB,NYB,NZB,IPC
...
! Other gas quantities
 ...
 MU_AIR   = Y2MU_C(MIN(500,NINT(0.1_EB*TMP_GAS)))*SPECIES(0)%MW
...
!print*,rho_gas,qrel,sv_veg,mu_air
 ...
 QCON_VEG = SV_VEG*DR%VEG_PACKING_RATIO*QCON_VEG
...
! Volitalization of vegetation
  IF_VOLITALIZATION: IF(MPV_MOIST <= MPV_MOIST_MIN) THEN
    ...
    IF_MD_VOLIT: IF(MPV_VEG > MPV_VEG_MIN .AND. TMP_VEG_NEW >= 400._EB) THEN !Morvan & Dupuy volitalization
     ...
     Q_VOLIT      = CHAR_FCTR*Q_FOR_VOLIT*0.01*(TMP_VEG-400.)
     MPV_VOLIT    = Q_VOLIT*0.00000239
...
!Enthalpy of volatiles using Cp,volatiles(T) from Ritchie
     H_SENS_VEG_VOLIT = 0.0445*(TMP_VEG_NEW**1.5 - TMP_GAS**1.5) - 0.136*(TMP_VEG_NEW - TMP_GAS)
...
! WRITE(9998,'(7(ES12.4))')T,TMP_GAS,TMP_VEG,QCON_VEG,QRAD_VEG,DR%VEG_MOIST_MASS,DR%VEG_FUEL_MASS
!ENDIF
