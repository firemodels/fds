...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,ASSUMED_GAS_TEMPERATURE_RAMP,&
                ...
                COMPUTE_VISCOSITY_TWICE,CONSTANT_SPECIFIC_HEAT,C_DEARDORFF,C_SMAGORINSKY,C_VREMAN,DNS,ENTHALPY_TRANSPORT,&
                ...
                KINETIC_ENERGY_SOURCE,LAPSE_RATE,LES_FILTER_WIDTH,LIMIT_MU,&
                ...
                TEMPERATURE_DEPENDENT_REACTION,TENSOR_DIFFUSIVITY,TERRAIN_CASE,TERRAIN_IMAGE,TEXTURE_ORIGIN,&
...
LOGICAL :: L_TMP,CHECK_ATOM_BALANCE,FAST_CHEMISTRY,REVERSE
NAMELIST /REAC/ A,A_RAMP,AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                E,E_RAMP,EPUMO2,EQBM_CONS,EQUATION,EXTINCTION_MODEL,FIXED_MIX_TIME,FORMULA,FUEL,FUEL_RADCAL_ID, &
                FWD_ID,FYI,H,HEAT_OF_COMBUSTION,&
                HRRPUA_SHEET,HRRPUV_AVERAGE,ID,IDEAL,N,NU,N_S,N_T,O,ODE_SOLVER,REAC_ATOM_ERROR,&
                REAC_MASS_ERROR,REVERSE,SOOT_H_FRACTION,SOOT_YIELD,SPEC_ID_N_S,SPEC_ID_NU,SUPPRESSION,TAU_CHEM,TAU_FLAME,&
                Y_P_MIN_EDC
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (RN%A == -1._EB .OR. RN%A > 1.E15_EB) THEN
      IF (RN%E == -1000._EB .OR. RN%E == 0._EB) THEN
...
INTEGER :: NS,NS2,NR,NRR,NSPEC,HF_COUNT,NFR,R_COUNT,F_COUNT
LOGICAL :: NAME_FOUND,SKIP_ATOM_BALANCE
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM
TYPE(REACTION_TYPE), POINTER :: RNN=>NULL(),RR=>NULL(),FR=>NULL()
...
REVERSE_LOOP: DO NRR=1,N_REACTIONS
   ...
   R_COUNT=0
   F_COUNT=0
   IF (.NOT. RR%REVERSE) CYCLE REVERSE_LOOP
   R_COUNT = 1
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   IF (DV%QUANTITY == 'CHEMISTRY SUBITERATIONS') THEN
      OUTPUT_CHEM_IT = .TRUE.
   ENDIF
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IF (TRIM(QUANTITY) == 'CHEMISTRY SUBITERATIONS') THEN
         OUTPUT_CHEM_IT = .TRUE.
      ENDIF
      
      IF (.NOT. TEMPERATURE_DEPENDENT_REACTION .AND. TRIM(QUANTITY) == 'FLAME TEMPERATURE') THEN
         WRITE(MESSAGE,'(A,A,A)')  'ERROR: SLCF ',TRIM(ID),', TEMPERATURE_DEPENDENT_REACTION must be set on MISC.'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
