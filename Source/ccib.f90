...
! Local variables:
INTEGER :: ICF,JCF,ICC,JCC,I,J,K,X1AXIS,II1,II2,JJ1,JJ2,KK1,KK2
REAL(EB):: PRFCT,WG1,WG2,IDX,X1F,RHO1,RHO2,P1,P2,RHOF,AT
LOGICAL :: FLG
...
CF_LOOP : DO ICF=1,MESHES(NM)%N_CUTFACE_MESH
   ...
   II2 = I; JJ2 = J; KK2 = K
   SELECT CASE(X1AXIS)
   CASE(IAXIS); IDX = 1/DXN(I); II2 = II2+1; AT = CF%ALPHA_CF * (DY(J)*DZ(K))
   CASE(JAXIS); IDX = 1/DYN(J); JJ2 = JJ2+1; AT = CF%ALPHA_CF * (DZ(K)*DX(I))
   CASE(KAXIS); IDX = 1/DZN(K); KK2 = KK2+1; AT = CF%ALPHA_CF * (DX(I)*DY(J))
   END SELECT
   DO JCF=1,CF%NFACE
      IF(.NOT.GRADH_ON_CARTESIAN) THEN
      ...
      CF%FN_B(JCF) = IDX * ( P2*(1._EB/RHOF-1._EB/RHO2) - P1*(1._EB/RHOF-1._EB/RHO1) )
   ENDDO
   ...
   IF (ICF<=MESHES(NM)%N_BBCUTFACE_MESH) THEN
      ...
      IF(WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY .OR. WC%BOUNDARY_TYPE==PERIODIC_BOUNDARY) THEN
         SELECT CASE(X1AXIS)
         CASE(IAXIS); FVX(I,J,K)=FVX(I,J,K)-FVX_B(I,J,K); FVX_B(I,J,K)=CF%FV_B; FVX(I,J,K)=FVX(I,J,K)+FVX_B(I,J,K)
         CASE(JAXIS); FVY(I,J,K)=FVY(I,J,K)-FVY_B(I,J,K); FVY_B(I,J,K)=CF%FV_B; FVY(I,J,K)=FVY(I,J,K)+FVY_B(I,J,K)
         CASE(KAXIS); FVZ(I,J,K)=FVZ(I,J,K)-FVZ_B(I,J,K); FVZ_B(I,J,K)=CF%FV_B; FVZ(I,J,K)=FVZ(I,J,K)+FVZ_B(I,J,K)
         END SELECT
      ENDIF
   ENDIF
...
REAL(EB):: IDX,HN,TSI,VEL_EDDY,TIME_RAMP_FACTOR,P_EXTERNAL,H0
...
IF(.NOT.CC_UNSTRUCTURED_PROJECTION .AND. .NOT.DO_IBEDGES) RETURN
