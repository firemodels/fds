...
REAL(EB) :: MU_N,YY_GET(1:N_GAS_SPECIES),VC,RTRM,CP_MF,TMP_WGT
      ...
      DO K=1,M%KBAR
         DO J=1,M%JBAR
            DO I=1,M%IBAR
               ...
               IF (N_GAS_SPECIES==0) THEN
                  ITMP = MIN(4999,INT(M%TMP(I,J,K)))
                  TMP_WGT = M%TMP(I,J,K) - ITMP
                  CP_MF = Y2CP_C(ITMP) + TMP_WGT*(Y2CP_C(ITMP+1)-Y2CP_C(ITMP))
                  RTRM = RSUM0/(CP_MF*M%PBAR(K,IPZ))
               ELSE
                  YY_GET(:) = M%YY(I,J,K,:)
                  CALL GET_SPECIFIC_HEAT(YY_GET,CP_MF,M%TMP(I,J,K))
