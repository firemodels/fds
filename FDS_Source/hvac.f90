...
WALL_LOOP: DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (VENTS(WC%VENT_INDEX)%BOUNDARY_TYPE/=HVAC_BOUNDARY) CYCLE WALL_LOOP
   ...
   KK = WC%ONE_D%KKG
      ...
      SELECT CASE (IOR)
         CASE (1)
            P_AVE = PBARP(KK,NODE_ZONE(WC%NODE_INDEX,NM))
         ...
         CASE(-2)
            ...
            NODE_Z(WC%NODE_INDEX,NM) = NODE_Z(WC%NODE_INDEX,NM) + ZC(KK)*AREA
         CASE (3)
            P_AVE = 0.5_EB*(PBARP(KK-1,NODE_ZONE(WC%NODE_INDEX,NM))+PBARP(KK,NODE_ZONE(WC%NODE_INDEX,NM)))
            NODE_X(WC%NODE_INDEX,NM) = NODE_X(WC%NODE_INDEX,NM) + XC(II)*AREA
            NODE_Y(WC%NODE_INDEX,NM) = NODE_Y(WC%NODE_INDEX,NM) + YC(JJ)*AREA            
            NODE_Z(WC%NODE_INDEX,NM) = NODE_Z(WC%NODE_INDEX,NM) + Z(KK-1)*AREA
         CASE (-3)
            P_AVE = 0.5_EB*(PBARP(KK,NODE_ZONE(WC%NODE_INDEX,NM))+PBARP(KK+1,NODE_ZONE(WC%NODE_INDEX,NM)))
            NODE_X(WC%NODE_INDEX,NM) = NODE_X(WC%NODE_INDEX,NM) + XC(II)*AREA
            NODE_Y(WC%NODE_INDEX,NM) = NODE_Y(WC%NODE_INDEX,NM) + YC(JJ)*AREA            
...
LEAKAGE_BC: IF (LEAK_DUCTS > 0) THEN
   ...
   WALL_LOOP_2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC=>WALL(IW)
      IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE WALL_LOOP_2
      SF => SURFACE(WC%SURF_INDEX)
      IF (SF%LEAK_PATH(1) /= WC%PRESSURE_ZONE .AND. SF%LEAK_PATH(2) /= WC%PRESSURE_ZONE) CYCLE WALL_LOOP_2
...
DO NN=1,N_DUCTNODES
   ...
   IF (SUM(NODE_ZONE(NN,:)) > DN%ZONE_INDEX) THEN
   ...
   VENT_NODE: IF (DN%VENT .AND. .NOT. DN%LEAKAGE) THEN
