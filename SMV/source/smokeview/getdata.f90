...
INTEGER :: NVERTS, NFACES, IBEG0, IBEG1
INTEGER, DIMENSION(0:99) :: FACESTART, FACENUM
REAL(EB), DIMENSION(0:299), TARGET :: VERTS
REAL(EB), DIMENSION(:), POINTER :: VV0, VV1, VV2
REAL(EB) :: VOLUME

CALL GETVERTS(BOX_BOUNDS,V0,V1,V2,V3,VERTS,NVERTS,FACESTART,FACENUM,NFACES)
...
DO I = 0, NFACES-1
   ...
   IBEG0 = FACESTART(I)
   IBEG1 = FACESTART(I)+1
   VV0(0:2) => VERTS(3*IBEG0:3*IBEG0+2)
   VV1(0:2) => VERTS(3*IBEG1:3*IBEG1+2)
   DO J = FACESTART(I)+2, FACESTART(I+1)-1
      VV2(0:2) => VERTS(3*J:3*J+2)
      VOLUME = VOLUME + ABS(VOLUME_VERTS(VV0,VV1,VV2,V_CENTER))
   END DO
END DO

GET_TETRABOX_VOLUME = VOLUME
RETURN
END FUNCTION GET_TETRABOX_VOLUME

!  ------------------ GETVERTS ------------------------ 

SUBROUTINE GETVERTS(BOX_BOUNDS,V0,V1,V2,V3,VERTS,NVERTS,FACESTART,FACENUM,NFACES)
REAL(EB), DIMENSION(0:5), INTENT(IN) :: BOX_BOUNDS
REAL(EB), DIMENSION(0:2), INTENT(IN) :: V0, V1, V2, V3
...
VOLUME_VERTS = DOT_PRODUCT(ACROSSB,D)
...
CALL GETVERTS(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB,VERTS_EB,NVERTS,FACESTART,FACENUM,NFACES)

IF(NVERTS.GT.0)VERTS(0:3*NVERTS-1) = VERTS_EB(0:3*NVERTS-1)
...
DO I = 0, NFACES-1
   ...
   IBEG0 = FACESTART(I)
   IBEG1 = FACESTART(I)+1
   VV0(0:2) => VERTS_EB(3*IBEG0:3*IBEG0+2)
   VV1(0:2) => VERTS_EB(3*IBEG1:3*IBEG1+2)
   DO J = FACESTART(I)+2, FACESTART(I) + FACENUM(I)-1
      VV2(0:2) => VERTS_EB(3*J:3*J+2)
      VOL = VOL + ABS(VOLUME_VERTS(VV0,VV1,VV2,V_CENTER))
   END DO
END DO
