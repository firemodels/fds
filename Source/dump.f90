...
IF (WRITE_VTK) THEN
   ...
   IF (VTK_HDF) THEN
      ...
      IF (T>=SL3D_VTK_CLOCK(SL3D_VTK_COUNTER(NM)) .OR. STOP_STATUS==INSTABILITY_STOP) THEN
         WRITE(*,*) "RANK ", MY_RANK, " INTO DUMP_SLCF_VTK 3D"
         CALL DUMP_SLCF_VTK(T,DT,NM,3,FAKEWRITE)
         WRITE(*,*) "RANK ", MY_RANK, " OUT OF DUMP_SLCF_VTK 3D"
      ENDIF
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   IF (VTK_OUT) THEN
      ...
      IF (VTK_HDF) THEN
         CALL ADD_1F32_DATA_TO_VTKHDF_RECT_GAS_FILE(FILENAME, S3%SMOKEVIEW_LABEL(1:30), QQ_PACK) !REAL(QQ_PACK, FB))
      ELSE
...
SUBROUTINE DUMP_SMOKE3D_VTKHDF(T,DT,NM,IFRMT)
