...
!> \brief Re-allocate arrays of derived types related to WALLs, CFACE, or LAGRANGIAN_PARTICLEs
!> \param NM Index to the current mesh
!> \param LP_INDEX Index within the LAGRANGIAN_PARTICLE array
!> \param LP_INDEX Index within the LAGRANGIAN_PARTICLE_CLASS array
...
INTEGER, INTENT(IN):: NM
INTEGER, INTENT(IN), OPTIONAL :: LP_INDEX,LPC_INDEX,SURF_INDEX,WALL_INDEX,CFACE_INDEX
INTEGER :: N_NEW_STORAGE_SLOTS,OD_INDEX,BP_INDEX,BC_INDEX
...
TYPE (BOUNDARY_PROPERTY_TYPE), ALLOCATABLE, DIMENSION(:) :: BP_DUMMY
TYPE (ONE_D_M_AND_E_XFER_TYPE), ALLOCATABLE, DIMENSION(:) :: ONE_D_DUMMY
...
IF (PRESENT(LP_INDEX)) THEN
   ...
   SF => SURFACE(LAGRANGIAN_PARTICLE_CLASS(LPC_INDEX)%SURF_INDEX)
   ...
   ENDIF
   CALL ALLOCATE_LAGRANGIAN_PARTICLE_ARRAYS
   IF (M%LAGRANGIAN_PARTICLE(LP_INDEX)%LP_INDEX==0) THEN
      M%N_BOUNDARY_COORD = M%N_BOUNDARY_COORD + 1    
      M%N_BOUNDARY_ONE_D = M%N_BOUNDARY_ONE_D + 1
      BC_INDEX = M%N_BOUNDARY_COORD
      OD_INDEX = M%N_BOUNDARY_ONE_D
   ...
   CALL ALLOCATE_BOUNDARY_COORD_ARRAYS
   CALL ALLOCATE_ONE_D_ARRAYS
...
ELSEIF (PRESENT(WALL_INDEX)) THEN
   ...
   ENDIF
   IF (M%WALL(WALL_INDEX)%WALL_INDEX==0) THEN
      M%N_BOUNDARY_COORD    = M%N_BOUNDARY_COORD + 1    ; BC_INDEX = M%N_BOUNDARY_COORD
      M%N_BOUNDARY_ONE_D    = M%N_BOUNDARY_ONE_D + 1    ; OD_INDEX = M%N_BOUNDARY_ONE_D
      M%N_BOUNDARY_PROPERTY = M%N_BOUNDARY_PROPERTY + 1 ; BP_INDEX = M%N_BOUNDARY_PROPERTY
   ...
   CALL ALLOCATE_BOUNDARY_COORD_ARRAYS
   CALL ALLOCATE_ONE_D_ARRAYS
   CALL ALLOCATE_BOUNDARY_PROPERTY_ARRAYS
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN
   ...
   ENDIF
   IF (M%CFACE(CFACE_INDEX)%CFACE_INDEX==0) THEN
      M%N_BOUNDARY_COORD    = M%N_BOUNDARY_COORD + 1    ; BC_INDEX = M%N_BOUNDARY_COORD
      M%N_BOUNDARY_ONE_D    = M%N_BOUNDARY_ONE_D + 1    ; OD_INDEX = M%N_BOUNDARY_ONE_D
      M%N_BOUNDARY_PROPERTY = M%N_BOUNDARY_PROPERTY + 1 ; BP_INDEX = M%N_BOUNDARY_PROPERTY
...
IF (BC_INDEX>M%N_BOUNDARY_COORD_DIM) THEN
   N_NEW_STORAGE_SLOTS = 1000
   ALLOCATE(BC_DUMMY(0:M%N_BOUNDARY_COORD_DIM))
   BC_DUMMY(0:M%N_BOUNDARY_COORD_DIM) = M%BOUNDARY_COORD(0:M%N_BOUNDARY_COORD_DIM)
   DEALLOCATE(M%BOUNDARY_COORD)
   ALLOCATE(M%BOUNDARY_COORD(0:M%N_BOUNDARY_COORD_DIM+N_NEW_STORAGE_SLOTS))
   M%BOUNDARY_COORD(0:M%N_BOUNDARY_COORD_DIM) = BC_DUMMY(0:M%N_BOUNDARY_COORD_DIM)
   DEALLOCATE(BC_DUMMY)
...
SUBROUTINE ALLOCATE_ONE_D_ARRAYS

USE GLOBAL_CONSTANTS, ONLY: THERMALLY_THICK,TMPM,SIGMA,TMPA4,RSUM0,TMPA,RHOA
TYPE(ONE_D_M_AND_E_XFER_TYPE), POINTER :: ONE_D
REAL(EB) :: X_POS
INTEGER :: NN,N

IF (OD_INDEX>M%N_BOUNDARY_ONE_D_DIM) THEN
   N_NEW_STORAGE_SLOTS = 1000
   ALLOCATE(ONE_D_DUMMY(0:M%N_BOUNDARY_ONE_D_DIM))
   ONE_D_DUMMY(0:M%N_BOUNDARY_ONE_D_DIM) = M%BOUNDARY_ONE_D(0:M%N_BOUNDARY_ONE_D_DIM)
   DEALLOCATE(M%BOUNDARY_ONE_D)
   ALLOCATE(M%BOUNDARY_ONE_D(0:M%N_BOUNDARY_ONE_D_DIM+N_NEW_STORAGE_SLOTS))
   M%BOUNDARY_ONE_D(0:M%N_BOUNDARY_ONE_D_DIM) = ONE_D_DUMMY(0:M%N_BOUNDARY_ONE_D_DIM)
   DEALLOCATE(ONE_D_DUMMY)
...
ONE_D%Q_CONDENSE      = 0._EB
ONE_D%TMP_F_OLD       = SF%TMP_FRONT
ONE_D%BURN_DURATION   = SF%BURN_DURATION
ONE_D%T_SCALE         = 0._EB
ONE_D%Q_SCALE         = 0._EB
ONE_D%T_MATL_PART     = 0._EB
ONE_D%B_NUMBER        = 0._EB

IF (ALLOCATED(ONE_D%M_DOT_G_PP_ACTUAL)) DEALLOCATE(ONE_D%M_DOT_G_PP_ACTUAL)
ALLOCATE(ONE_D%M_DOT_G_PP_ACTUAL(SF%ONE_D_REALS_ARRAY_SIZE(1))) ; ONE_D%M_DOT_G_PP_ACTUAL = 0._EB
IF (ALLOCATED(ONE_D%M_DOT_S_PP)) DEALLOCATE(ONE_D%M_DOT_S_PP)
ALLOCATE(ONE_D%M_DOT_S_PP(SF%ONE_D_REALS_ARRAY_SIZE(2)))        ; ONE_D%M_DOT_S_PP = 0._EB
IF (ALLOCATED(ONE_D%M_DOT_G_PP_ADJUST)) DEALLOCATE(ONE_D%M_DOT_G_PP_ADJUST)
ALLOCATE(ONE_D%M_DOT_G_PP_ADJUST(SF%ONE_D_REALS_ARRAY_SIZE(3))) ; ONE_D%M_DOT_G_PP_ADJUST = 0._EB
IF (ALLOCATED(ONE_D%X)) DEALLOCATE(ONE_D%X)
ALLOCATE(ONE_D%X(0:SF%ONE_D_REALS_ARRAY_SIZE(4)-1)) ; ONE_D%X=0._EB ; ONE_D%X(0:SF%N_CELLS_INI) = SF%X_S(0:SF%N_CELLS_INI)
IF (ALLOCATED(ONE_D%TMP)) DEALLOCATE(ONE_D%TMP)
ALLOCATE(ONE_D%TMP(0:SF%ONE_D_REALS_ARRAY_SIZE(5)-1))           ; ONE_D%TMP = SF%TMP_INNER(1)
...
ALLOCATE(ONE_D%LAYER_THICKNESS(SF%ONE_D_REALS_ARRAY_SIZE(7)))
         ONE_D%LAYER_THICKNESS(1:SF%N_LAYERS) = SF%LAYER_THICKNESS(1:SF%N_LAYERS)
IF (ALLOCATED(ONE_D%ZZ_G)) DEALLOCATE(ONE_D%ZZ_G)
ALLOCATE(ONE_D%ZZ_G(SF%ONE_D_REALS_ARRAY_SIZE(8))) ; ONE_D%ZZ_G = 0._EB
IF (ALLOCATED(ONE_D%ZZ_F)) DEALLOCATE(ONE_D%ZZ_F)
ALLOCATE(ONE_D%ZZ_F(SF%ONE_D_REALS_ARRAY_SIZE(9))) ; ONE_D%ZZ_F = 0._EB
IF (ALLOCATED(ONE_D%RHO_D_F)) DEALLOCATE(ONE_D%RHO_D_F)
ALLOCATE(ONE_D%RHO_D_F(SF%ONE_D_REALS_ARRAY_SIZE(10))) ; ONE_D%RHO_D_F = 0._EB
IF (ALLOCATED(ONE_D%RHO_D_DZDN_F)) DEALLOCATE(ONE_D%RHO_D_DZDN_F)
ALLOCATE(ONE_D%RHO_D_DZDN_F(SF%ONE_D_REALS_ARRAY_SIZE(11))) ; ONE_D%RHO_D_DZDN_F = 0._EB
IF (ALLOCATED(ONE_D%RHO_C_S)) DEALLOCATE(ONE_D%RHO_C_S)
ALLOCATE(ONE_D%RHO_C_S(SF%ONE_D_REALS_ARRAY_SIZE(12))) ; ONE_D%RHO_C_S = 1.E6_EB
IF (ALLOCATED(ONE_D%K_S)) DEALLOCATE(ONE_D%K_S)
ALLOCATE(ONE_D%K_S(0:SF%ONE_D_REALS_ARRAY_SIZE(13)-1)) ; ONE_D%K_S = 0._EB
IF (ALLOCATED(ONE_D%AWM_AEROSOL)) DEALLOCATE(ONE_D%AWM_AEROSOL)
ALLOCATE(ONE_D%AWM_AEROSOL(SF%ONE_D_REALS_ARRAY_SIZE(14))) ; ONE_D%AWM_AEROSOL = 0._EB
...
ALLOCATE(ONE_D%DDSUM(SF%ONE_D_REALS_ARRAY_SIZE(17))) ; ONE_D%DDSUM(1:SF%N_LAYERS) = SF%DDSUM(1:SF%N_LAYERS)
IF (ALLOCATED(ONE_D%SMALLEST_CELL_SIZE)) DEALLOCATE(ONE_D%SMALLEST_CELL_SIZE)
ALLOCATE(ONE_D%SMALLEST_CELL_SIZE(SF%ONE_D_REALS_ARRAY_SIZE(18)))
...
ALLOCATE(ONE_D%PART_MASS(SF%ONE_D_REALS_ARRAY_SIZE(19))) ; ONE_D%PART_MASS = 0._EB
IF (ALLOCATED(ONE_D%PART_ENTHALPY)) DEALLOCATE(ONE_D%PART_ENTHALPY)
ALLOCATE(ONE_D%PART_ENTHALPY(SF%ONE_D_REALS_ARRAY_SIZE(20))) ; ONE_D%PART_ENTHALPY = 0._EB
...
END SUBROUTINE ALLOCATE_ONE_D_ARRAYS


SUBROUTINE ALLOCATE_BOUNDARY_PROPERTY_ARRAYS

USE GLOBAL_CONSTANTS, ONLY: TMPA,TMPA4,SIGMA,PI,NUMBER_SPECTRAL_BANDS,NUMBER_RADIATION_ANGLES
TYPE(BOUNDARY_PROPERTY_TYPE), POINTER :: BP
INTEGER :: NN

IF (BP_INDEX>M%N_BOUNDARY_PROPERTY_DIM) THEN
   ALLOCATE(BP_DUMMY(0:M%N_BOUNDARY_PROPERTY_DIM))
   BP_DUMMY(0:M%N_BOUNDARY_PROPERTY_DIM) = M%BOUNDARY_PROPERTY(0:M%N_BOUNDARY_PROPERTY_DIM)
   DEALLOCATE(M%BOUNDARY_PROPERTY)
   ALLOCATE(M%BOUNDARY_PROPERTY(0:M%N_BOUNDARY_PROPERTY_DIM+N_NEW_STORAGE_SLOTS))
   M%BOUNDARY_PROPERTY(0:M%N_BOUNDARY_PROPERTY_DIM) = BP_DUMMY(0:M%N_BOUNDARY_PROPERTY_DIM)
   DEALLOCATE(BP_DUMMY)
   M%N_BOUNDARY_PROPERTY_DIM = M%N_BOUNDARY_PROPERTY_DIM + N_NEW_STORAGE_SLOTS
ENDIF

BP => M%BOUNDARY_PROPERTY(BP_INDEX)
...
BP%WORK1         = 0._EB
BP%WORK2         = 0._EB
BP%K_SUPPRESSION = 0._EB
BP%L_OBUKHOV     = 0._EB

IF (ALLOCATED(BP%A_LP_MPUA)) DEALLOCATE(BP%A_LP_MPUA)
ALLOCATE(BP%A_LP_MPUA(BOUNDARY_PROPERTY_REALS_ARRAY_SIZE(1))) ; BP%A_LP_MPUA = 0._EB
IF (ALLOCATED(BP%LP_CPUA)) DEALLOCATE(BP%LP_CPUA)
ALLOCATE(BP%LP_CPUA(BOUNDARY_PROPERTY_REALS_ARRAY_SIZE(2))) ; BP%LP_CPUA = 0._EB
IF (ALLOCATED(BP%LP_MPUA)) DEALLOCATE(BP%LP_MPUA)
ALLOCATE(BP%LP_MPUA(BOUNDARY_PROPERTY_REALS_ARRAY_SIZE(3))) ; BP%LP_MPUA = 0._EB
IF (ALLOCATED(BP%LP_TEMP)) DEALLOCATE(BP%LP_TEMP)
ALLOCATE(BP%LP_TEMP(BOUNDARY_PROPERTY_REALS_ARRAY_SIZE(4))) ; BP%LP_TEMP = TMPA
IF (ALLOCATED(BP%BAND)) DEALLOCATE(BP%BAND)
ALLOCATE(BP%BAND(NUMBER_SPECTRAL_BANDS))
DO NN=1,NUMBER_SPECTRAL_BANDS
   IF (ALLOCATED(BP%BAND(NN)%ILW)) DEALLOCATE(BP%BAND(NN)%ILW)
   ALLOCATE(BP%BAND(NN)%ILW(NUMBER_RADIATION_ANGLES)) ; BP%BAND(NN)%ILW = 0._EB
ENDDO
IF (ALLOCATED(BP%IL)) DEALLOCATE(BP%IL)
ALLOCATE(BP%IL(BOUNDARY_PROPERTY_REALS_ARRAY_SIZE(6))) ; BP%IL = SIGMA*TMPA4/PI

END SUBROUTINE ALLOCATE_BOUNDARY_PROPERTY_ARRAYS


SUBROUTINE ALLOCATE_LAGRANGIAN_PARTICLE_ARRAYS

USE GLOBAL_CONSTANTS, ONLY: TMPA4,SIGMA,PI,NUMBER_SPECTRAL_BANDS,NUMBER_RADIATION_ANGLES
INTEGER :: NN
TYPE(LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP

IF (LAGRANGIAN_PARTICLE_CLASS(LPC_INDEX)%N_ORIENTATION==0) RETURN
...
DO NN=1,NUMBER_SPECTRAL_BANDS
   IF (ALLOCATED(LP%BAND(NN)%ILW)) DEALLOCATE(LP%BAND(NN)%ILW)
   ALLOCATE(LP%BAND(NN)%ILW(1:NUMBER_RADIATION_ANGLES)) ; LP%BAND(NN)%ILW = 0._EB
ENDDO
IF (ALLOCATED(LP%IL)) DEALLOCATE(LP%IL)
ALLOCATE(LP%IL(NUMBER_SPECTRAL_BANDS)) ; LP%IL = SIGMA*TMPA4/PI

END SUBROUTINE ALLOCATE_LAGRANGIAN_PARTICLE_ARRAYS

END SUBROUTINE ALLOCATE_STORAGE


!> \brief Pack LAGRANGIAN_PARTICLE variables into REAL, INTEGER, and LOGICAL 1-D arrays
!> \param NM Mesh index
!> \param NOM Index of the mesh that is to receive the particles
!> \param LP_INDEX Index of the particle
!> \param LPC_INDEX Class index of the particle
!> \param STORAGE_INDEX Slot in ORPHAN_PARTICLE_STORAGE in which to store the variables

SUBROUTINE PACK_PARTICLE(NM,NOM,LP_INDEX,LPC_INDEX,STORAGE_INDEX)

USE GLOBAL_CONSTANTS, ONLY: NUMBER_SPECTRAL_BANDS,NUMBER_RADIATION_ANGLES
INTEGER, INTENT(IN) :: NM,NOM,LP_INDEX,LPC_INDEX,STORAGE_INDEX
INTEGER :: NN,IC,RC,LC,I1,I2
TYPE(LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP
TYPE(LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC
...
LPC => LAGRANGIAN_PARTICLE_CLASS(LPC_INDEX)
SF => SURFACE(LPC%SURF_INDEX)
OS => M%OMESH(NOM)%ORPHAN_PARTICLE_STORAGE(LPC_INDEX)

! Assign integer pointers and values

IC = 0

IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%BC_INDEX              
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%OD_INDEX              
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%TAG                   
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%LP_INDEX              
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%CLASS_INDEX           
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%ORIENTATION_INDEX     
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%WALL_INDEX            
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%DUCT_INDEX            
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%DUCT_CELL_INDEX       
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%INIT_INDEX            
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%CFACE_INDEX           
IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=LP%PROP_INDEX            

! Assign logical pointers and values

LC = 0

LC=LC+1 ; OS%LOGICALS(LC,STORAGE_INDEX)=LP%SHOW               
LC=LC+1 ; OS%LOGICALS(LC,STORAGE_INDEX)=LP%SPLAT              
LC=LC+1 ; OS%LOGICALS(LC,STORAGE_INDEX)=LP%EMBER              
LC=LC+1 ; OS%LOGICALS(LC,STORAGE_INDEX)=LP%PATH_PARTICLE      

! Assign real pointers and values

RC = 0

RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%U                  
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%V                  
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%W                  
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%PWT                
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%ACCEL_X            
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%ACCEL_Y            
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%ACCEL_Z            
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%RE                 
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%MASS               
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%T_INSERT           
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%DX                 
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%DY                 
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%DZ                 
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%M_DOT              
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%HTC_LIMIT          
RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=LP%C_DRAG             
...
IF (LPC%N_ORIENTATION>0) THEN
   ...
   DO NN=1,NUMBER_SPECTRAL_BANDS
      ...
      OS%REALS(I1:I2,STORAGE_INDEX) = LP%BAND(NN)%ILW(1:NUMBER_RADIATION_ANGLES)
   ENDDO

   I1 = I2+1 ; I2 = I1 + NUMBER_SPECTRAL_BANDS - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = LP%IL(1:I2-I1+1)
...
IF (LP%BC_INDEX>0) THEN
   ...
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=BC%II               
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=BC%JJ               
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=BC%KK               
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=BC%IIG              
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=BC%JJG              
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=BC%KKG              
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=BC%IOR              

   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=BC%X                
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=BC%Y                
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=BC%Z                

ENDIF

! 1-D heat transfer variables

IF (LP%OD_INDEX>0) THEN

   ONE_D => M%BOUNDARY_ONE_D(LP%OD_INDEX)

   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=ONE_D%PRESSURE_ZONE    
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=ONE_D%NODE_INDEX       
   IC=IC+1 ; OS%INTEGERS(IC,STORAGE_INDEX)=ONE_D%N_SUBSTEPS       
   
   I2 = IC
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_INTEGERS_ARRAY_SIZE(1) - 1
   OS%INTEGERS(I1:I2,STORAGE_INDEX) = ONE_D%N_LAYER_CELLS(1:I2-I1+1)
   
   IC = I2  ! Update Integer Counter
   
   LC=LC+1 ; OS%LOGICALS(LC,STORAGE_INDEX)=ONE_D%BURNAWAY
   
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%AREA             
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%HEAT_TRANS_COEF  
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%Q_CON_F          
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%Q_RAD_IN         
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%Q_RAD_OUT        
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%EMISSIVITY       
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%AREA_ADJUST      
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%T_IGN            
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%TMP_F            
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%TMP_B            
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%U_NORMAL         
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%U_NORMAL_S       
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%U_NORMAL_0       
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%RSUM_G           
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%TMP_G            
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%RHO_G            
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%U_TANG           
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%RHO_F            
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%RDN              
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%MU_G             
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%K_G              
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%Q_DOT_G_PP       
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%Q_DOT_O2_PP      
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%Q_CONDENSE       
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%TMP_F_OLD        
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%BURN_DURATION    
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%T_SCALE          
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%Q_SCALE          
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%T_MATL_PART      
   RC=RC+1 ; OS%REALS(RC,STORAGE_INDEX)=ONE_D%B_NUMBER         
   
   I2 = RC  ! I1 and I2 continue counting reals
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(1) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%M_DOT_G_PP_ACTUAL(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(2) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%M_DOT_S_PP(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(3) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%M_DOT_G_PP_ADJUST(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(4) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%X(0:I2-I1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(5) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%TMP(0:I2-I1)
   
   DO NN=1,SF%N_MATL
      I1 = I2+1 ; I2 = I1 + (SF%N_CELLS_MAX+2) - 1
      OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%MATL_COMP(NN)%RHO(0:SF%N_CELLS_MAX+1)
   ENDDO
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(7) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%LAYER_THICKNESS(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(8) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%ZZ_G(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(9) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%ZZ_F(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(10) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%RHO_D_F(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(11) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%RHO_D_DZDN_F(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(12) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%RHO_C_S(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(13) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%K_S(0:SF%N_CELLS_INI+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(14) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%AWM_AEROSOL(1:I2-I1+1)
   
   DO NN=1,SF%N_MATL
      I1 = I2+1 ; I2 = I1 + (SF%N_CELLS_MAX+2) - 1
      OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%MATL_COMP(NN)%RHO_DOT(0:SF%N_CELLS_MAX+1)
   ENDDO
   
   DO NN=1,SF%N_SPEC
      I1 = I2+1 ; I2 = I1 + (SF%N_CELLS_MAX+2) - 1
      OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%SPEC_COMP(NN)%RHO_ZZ(0:SF%N_CELLS_MAX+1)
   ENDDO
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(17) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%DDSUM(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(18) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%SMALLEST_CELL_SIZE(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(19) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%PART_MASS(1:I2-I1+1)
   
   I1 = I2+1 ; I2 = I1 + SF%ONE_D_REALS_ARRAY_SIZE(20) - 1
   OS%REALS(I1:I2,STORAGE_INDEX) = ONE_D%PART_ENTHALPY(1:I2-I1+1)
...
SUBROUTINE UNPACK_PARTICLE(NM,NOM,LP_INDEX,LPC_INDEX,STORAGE_INDEX)
...
LPC => LAGRANGIAN_PARTICLE_CLASS(LPC_INDEX)
SF => SURFACE(LPC%SURF_INDEX)
OS => M%OMESH(NOM)%ADOPT_PARTICLE_STORAGE(LPC_INDEX)

CALL ALLOCATE_STORAGE(NM,LP_INDEX=LP_INDEX,LPC_INDEX=LPC_INDEX)
...
IC=IC+1 ; LP%BC_INDEX             = LP%BC_INDEX
IC=IC+1 ; LP%OD_INDEX             = LP%OD_INDEX
IC=IC+1 ; LP%TAG                  = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%LP_INDEX             = LP_INDEX
IC=IC+1 ; LP%CLASS_INDEX          = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%ORIENTATION_INDEX    = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%WALL_INDEX           = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%DUCT_INDEX           = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%DUCT_CELL_INDEX      = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%INIT_INDEX           = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%CFACE_INDEX          = OS%INTEGERS(IC,STORAGE_INDEX)
IC=IC+1 ; LP%PROP_INDEX           = OS%INTEGERS(IC,STORAGE_INDEX)

! Assign logical pointers and values

LC = 0

LC=LC+1 ; LP%SHOW              = OS%LOGICALS(LC,STORAGE_INDEX)
LC=LC+1 ; LP%SPLAT             = OS%LOGICALS(LC,STORAGE_INDEX)
LC=LC+1 ; LP%EMBER             = OS%LOGICALS(LC,STORAGE_INDEX)
LC=LC+1 ; LP%PATH_PARTICLE     = OS%LOGICALS(LC,STORAGE_INDEX)

! Assign real pointers and values

RC = 0

RC=RC+1 ; LP%U                 = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%V                 = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%W                 = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%PWT               = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%ACCEL_X           = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%ACCEL_Y           = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%ACCEL_Z           = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%RE                = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%MASS              = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%T_INSERT          = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%DX                = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%DY                = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%DZ                = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%M_DOT             = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%HTC_LIMIT         = OS%REALS(RC,STORAGE_INDEX)
RC=RC+1 ; LP%C_DRAG            = OS%REALS(RC,STORAGE_INDEX)
...
IF (LP%BC_INDEX>0) THEN
   ...
   IC=IC+1 ; BC%II              = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; BC%JJ              = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; BC%KK              = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; BC%IIG             = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; BC%JJG             = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; BC%KKG             = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; BC%IOR             = OS%INTEGERS(IC,STORAGE_INDEX)
...
IF (LP%OD_INDEX>0) THEN
   ...
   IC=IC+1 ; ONE_D%PRESSURE_ZONE   = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; ONE_D%NODE_INDEX      = OS%INTEGERS(IC,STORAGE_INDEX)
   IC=IC+1 ; ONE_D%N_SUBSTEPS      = OS%INTEGERS(IC,STORAGE_INDEX)
   ...
   LC=LC+1 ; ONE_D%BURNAWAY        = OS%LOGICALS(LC,STORAGE_INDEX)
   
   RC=RC+1 ; ONE_D%AREA            = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%HEAT_TRANS_COEF = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%Q_CON_F         = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%Q_RAD_IN        = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%Q_RAD_OUT       = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%EMISSIVITY      = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%AREA_ADJUST     = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%T_IGN           = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%TMP_F           = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%TMP_B           = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%U_NORMAL        = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%U_NORMAL_S      = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%U_NORMAL_0      = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%RSUM_G          = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%TMP_G           = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%RHO_G           = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%U_TANG          = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%RHO_F           = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%RDN             = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%MU_G            = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%K_G             = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%Q_DOT_G_PP      = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%Q_DOT_O2_PP     = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%Q_CONDENSE      = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%TMP_F_OLD       = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%BURN_DURATION   = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%T_SCALE         = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%Q_SCALE         = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%T_MATL_PART     = OS%REALS(RC,STORAGE_INDEX)
   RC=RC+1 ; ONE_D%B_NUMBER        = OS%REALS(RC,STORAGE_INDEX)
