...
USE VTK_FDS_INTERFACE, ONLY : BUILD_VTK_GAS_PHASE_GEOMETRY,BUILD_VTK_SOLID_PHASE_GEOMETRY,BUILD_VTK_SLICE_GEOMETRY,&
                ...
                ADD_DATA_TO_SMOKE3D_VTKHDF_FILE,WRITE_VTKHDF_BNDF_CELL_FILE
...
TYPE(PATCH_TYPE), POINTER :: PA
INTEGER :: N, N_WRITTEN
INTEGER(IB32), DIMENSION(1:NMESHES) :: NCELLS, NPOINTS, NCONNECTIONS
INTEGER :: NCELLS_ACCUM, NCONNECTIONS_ACCUM, NPOINTS_ACCUM
INTEGER, ALLOCATABLE, DIMENSION(:) :: LOCATIONS,FACES,SURFIND,GEOMIND
REAL(FB), ALLOCATABLE, DIMENSION(:) :: VALS, VERTS
REAL(FB), ALLOCATABLE, DIMENSION(:) :: QQ_PACK
REAL(FB), ALLOCATABLE, DIMENSION(:) :: X_PTS, Y_PTS, Z_PTS
...
! Determine points per patch

CALL POINT_TO_MESH(NM)

DATA_FILE_LOOP: DO N=1,N_BNDF
...
IF (MESHES(NM)%N_PATCH > 0) THEN
   ...
   PATCH_LOOP1A: DO IP=1,N_PATCH
      PA => PATCH(IP)
      ! Initialize piece
      CALL BUILD_VTK_SOLID_PHASE_GEOMETRY(NM, PA, NCELLS(NM), NPOINTS(NM), &
                                          X_PTS, Y_PTS, Z_PTS, CONNECT, OFFSETS, VTKC_TYPE)
      VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_PIECE(NP=NPOINTS(NM), NC=NCELLS(NM))
      VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_GEO(NP=NPOINTS(NM), NC=NCELLS(NM), X=X_PTS, Y=Y_PTS, Z=Z_PTS)
      VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_CONNECTIVITY(NC=NCELLS(NM), CONNECTIVITY=CONNECT, OFFSET=OFFSETS, VTKC_TYPE=VTKC_TYPE)
      CALL DEALLOCATE_VTK_GAS_PHASE_GEOMETRY(X_PTS,Y_PTS,Z_PTS,OFFSETS,VTKC_TYPE,CONNECT)
   ...
   IF (CC_IBM) THEN
      ...
      IF (NVERTS>0 .AND. NFACES>0) THEN
         ...
         VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_PIECE(NP=NPOINTS(NM), NC=NCELLS(NM))
         VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_GEO(NP=NPOINTS(NM), NC=NCELLS(NM), X=X_PTS, Y=Y_PTS, Z=Z_PTS)
         VTK_ERROR = A_VTK_FILE%XML_WRITER%WRITE_CONNECTIVITY(&
            NC=NCELLS(NM), CONNECTIVITY=CONNECT, OFFSET=OFFSETS, VTKC_TYPE=VTKC_TYPE)
