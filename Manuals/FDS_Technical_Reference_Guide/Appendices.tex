...
In the current formulation of the reactor model there are two environments: the mean species concentrations which FDS currently transports and the amount of mixing within a cell (unmixed fraction) which FDS models. Since FDS does not transport the mixing variable, at the start of a time step, the mixing environment is assumed to be in the unmixed state. We would like to extend the reactor model to more environments so that we can eventually construct PDFs of reactor concentration. The difficulty is how to construct/transport the necessary information to build the PDF.
...
y_{n+1}     &= y_{n}+h\,f(y^*_{n+1/2},t_{n+1/2})
\end{align}
The implementation of RK2 in FDS is as follows:
\begin{align}
\label{RK2}
y^*_{n+1} &= y_n+h\,f(y_{n},t_{n}) \notag\\
y^*_{n+2} &= y^*_{n+1}+h\,f(y^*_{n+1},t_{n+1}) \notag\\
y_{n+1}   &= \frac{1}{2}\,(y_{n}+y^*_{n+2})
\end{align}
