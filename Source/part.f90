...
DO N_LPC=1,N_LAGRANGIAN_CLASSES
   ...
   IF (LPC%ARRAY_INDEX>0) THEN
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         WALL(IW)%ONE_D%LP_TEMP(LPC%ARRAY_INDEX) = LPC%RUNNING_AVERAGE_FACTOR_WALL*WALL(IW)%ONE_D%LP_TEMP(LPC%ARRAY_INDEX)
      ...
      ENDDO
      DO ICF = 1,N_CFACE_CELLS
         CFACE(ICF)%ONE_D%LP_TEMP(LPC%ARRAY_INDEX) = LPC%RUNNING_AVERAGE_FACTOR_WALL*CFACE(ICF)%ONE_D%LP_TEMP(LPC%ARRAY_INDEX)
...
SUM_PART_QUANTITIES: IF (N_LP_ARRAY_INDICES > 0) THEN
   ...
   PART_CLASS_SUM_LOOP: DO N_LPC = 1,N_LAGRANGIAN_CLASSES
      ...
      ! Compute mean particle temperature on the wall for liquid droplets
...
!         KK = LP%ONE_D%KKG
            ...
            IF (LP%ONE_D%IOR/=0 .AND. (LP%WALL_INDEX>0 .OR. LP%CFACE_INDEX>0)) THEN
               ...
               AREA_ADD = AREA_ADD + A_DROP
               write(*,*) IP, LP%ONE_D%TMP_F-273.15_EB, ONE_D%LP_TEMP(LPC%ARRAY_INDEX)-273.15_EB, &
                          LP%PWT*(M_DROP/ONE_D%AREA)/(ONE_D%LP_MPUA(LPC%ARRAY_INDEX)+TWO_EPSILON_EB)
               TMP_DROP_NEW = TMP_DROP_NEW + A_DROP*LP%ONE_D%TMP_F
!               ONE_D%LP_TEMP(LPC%ARRAY_INDEX) = ONE_D%LP_TEMP(LPC%ARRAY_INDEX) + &
!                            (1._EB-LPC%RUNNING_AVERAGE_FACTOR_WALL) * &
!                            LP%PWT*(M_DROP/ONE_D%AREA)*LP%ONE_D%TMP_F/(ONE_D%LP_MPUA(LPC%ARRAY_INDEX)+TWO_EPSILON_EB)
            ENDIF
