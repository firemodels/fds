...
INTEGER :: NR,I_TS,NODETS=20,NODETSMAX=100,NS,ITER,COUNT
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
...
ELSE
   ...
   ITER = MIN(MAX(ITER,NODETS),NODETSMAX)
...
ODE_LOOP: DO WHILE (DT_SUM < DT)
   ...
   DT_SUM = DT_SUM + DT_NEW
   TOL_CALC=1._EB
   IF (DT_NEW < DT/REAL(ITER,EB)) DT_NEW = DT/REAL(ITER,EB)
   IF (DT_NEW + DT_SUM > DT) DT_NEW = DT - DT_SUM
   I_TS = I_TS + 1
ENDDO ODE_LOOP
