...
\begin{align}
\label{eq:dmdt_1} m_\alpha^* &= m_{\alpha}(t^k) + \overline{Y}_{\alpha,0} \, U(t^k) \, [1 - \exp(-\Delta t^k/\tau_{mix})]  \\
...
In FDS, the rate expression (on a fuel basis) for both infinitely fast and Arrhenius rate chemistry are coded with the same general form:
