...
LOGICAL  :: EX,EX_GLOBAL,DIAGNOSTICS,EXCHANGE_RADIATION=.TRUE.,EXCHANGE_EVACUATION=.FALSE.,FIRST_PASS
...
MAIN_LOOP: DO  
   ...
   ! Exchange minimum and maximum density needed for BAROCLINIC torque correction

   IF (BAROCLINIC) THEN
      RHO_UPPER_LOCAL = RHO_UPPER
      RHO_LOWER_LOCAL = RHO_LOWER
      CALL MPI_ALLREDUCE(RHO_UPPER_LOCAL,RHO_UPPER,1,MPI_DOUBLE_PRECISION,MPI_MAX,MPI_COMM_WORLD,IERR)
      CALL MPI_ALLREDUCE(RHO_LOWER_LOCAL,RHO_LOWER,1,MPI_DOUBLE_PRECISION,MPI_MIN,MPI_COMM_WORLD,IERR)
   ENDIF
...
IF (RECOMPUTE_A) THEN
   A = TRANSPOSE(A)
   CALL MPI_GATHERV(A(1,DISPLS(MYID)+1),COUNTS2D(MYID),MPI_DOUBLE_PRECISION,A,COUNTS2D,DISPLS2D, &
...
CALL MPI_GATHERV(B(DISPLS(MYID)+1),COUNTS(MYID),MPI_DOUBLE_PRECISION,B,COUNTS,DISPLS, &
                 MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
