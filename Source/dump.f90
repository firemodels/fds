...
ALLOCATE(LU_SMOKE3D(N_SMOKE3D*3,NMESHES)) ! and the vol3d files (16 bit integers)
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   DO N=1,N_SMOKE3D
      ...
      WRITE(FN_SMOKE3D(N+2*N_SMOKE3D,NM),  '(A,A,I0,A,I0,A)') TRIM(CHID),'_',NM,'_',N,'.v3d'
   ENDDO
...
INTEGER, ALLOCATABLE, DIMENSION(:) :: NODE_INDS, DUCT_INDS
CHARACTER(80) :: FN
...
! Open heat HVAC file (CHID.hvac)

IF (HVAC_SOLVE) THEN
   IF (APPEND) THEN
      OPEN(LU_HVAC,FILE=FN_HVAC,FORM='UNFORMATTED',STATUS='OLD')
   ELSE
     IF(N_DUCTNODES>0)ALLOCATE(NODE_INDS(N_DUCTNODES))
     IF(N_DUCTS>0)ALLOCATE(DUCT_INDS(N_DUCTS))
      ...
      DO N=1,N_DUCTS
         ...
         DUCT_INDS(N_DUCT_OUT) = N
      ENDDO
      WRITE(LU_HVAC)N_NODE_OUT, N_NODE_VARS, N_DUCT_OUT, N_DUCT_VARS
      WRITE(LU_HVAC)(NODE_INDS(N),N=1,N_NODE_OUT)
      WRITE(LU_HVAC)(DUCT_INDS(N),N=1,N_DUCT_OUT)
   ENDIF
ENDIF
...
INTEGER :: CC_VAL
INTEGER :: VOL3D_COMPRESS, VOL3D_VERSION
   ...
   DO N=1,N_SMOKE3D
      ...
      ELSE
         ...
         IF (VOL3D) THEN
            ...
            WRITE(LU_SMOKE3D(N+2*N_SMOKE3D,NM)) INTEGER_ONE,VOL3D_VERSION,VOL3D_COMPRESS,M%IBP1,M%JBP1,M%KBP1
         ENDIF
         IF (M%N_STRINGS+5>M%N_STRINGS_MAX) CALL RE_ALLOCATE_STRINGS(NM)
         M%N_STRINGS = M%N_STRINGS + 1
...
IF (HVAC_SOLVE) THEN
   ...
   ENDDO
   N_NODE_VARS = 3 ! These values are output to the CHID.hvac file.
   N_DUCT_VARS = 2 ! The number of NODE and DUCT entries below needs to
                   ! be consistent with these values
   WRITE(LU_SMV,'(/A)') 'HVACVALS'
   WRITE(LU_SMV,'(1X,A)')  FN_HVAC
   WRITE(LU_SMV,'(1X,I0)') N_NODE_VARS  ! number of node variables output
   WRITE(LU_SMV,'(1X,A)') 'TEMPERATURE' ! long name (for smokeview menus)
   WRITE(LU_SMV,'(1X,A)') 'Temp'        ! short name (for smokeview colorbar)
   WRITE(LU_SMV,'(1X,A)') 'C'           ! unit (for smokeview colorbar)
   WRITE(LU_SMV,'(1X,A)') 'DENSITY'
   WRITE(LU_SMV,'(1X,A)') 'Dens'
   WRITE(LU_SMV,'(1X,A)') 'kg/m3'
   WRITE(LU_SMV,'(1X,A)') 'PRESSURE'
   WRITE(LU_SMV,'(1X,A)') 'Pres'
   WRITE(LU_SMV,'(1X,A)') 'Pa'
   WRITE(LU_SMV,'(1X,I0)') N_DUCT_VARS
   WRITE(LU_SMV,'(1X,A)') 'UPSTREAM TEMPERATURE'
   WRITE(LU_SMV,'(1X,A)') 'UTemp'
   WRITE(LU_SMV,'(1X,A)') 'C'
   WRITE(LU_SMV,'(1X,A)') 'UPSTREAM DENSITY'
   WRITE(LU_SMV,'(1X,A)') 'UDen'
   WRITE(LU_SMV,'(1X,A)') 'kg/m3'
ENDIF
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   CALL SMOKE3D_TO_FILE(NM,STIME,DXX,N,QQ_PACK,IBP1,JBP1,KBP1,VOL3D)
   DEALLOCATE(QQ_PACK)
...
DO N=1,N_DUCTS
   IF (DUCT(N)%LEAKAGE) CYCLE
   WRITE(LU_HVAC) REAL(DUCT(N)%TMP_D,FB), REAL(DUCT(N)%RHO_D,FB)
ENDDO
