...
\subsection{Simple Vents, Fans and Heaters}
...
\caption[Example of a fan curve.]{Fan curve corresponding to {\ct VOLUME\_FLUX=10} and {\ct MAX\_PRESSURE=500}. Note that a volume flux greater than
...
default value of {\ct REFERENCE\_RATE} is 0.001~s$^{-1}$.  It is
...
decreases at a rate of 0.001~s$^{-1}$. You should check the sensitivity
...
Here is an example of a steel pan filled with a thin layer of ethanol. Note that the material
...
\section{Special Topic: Finite-Rate Combustion}
\label{info:finite}
...
Since a single {\ct DEVC} cannot span more than one mesh, having a beam detector that crosses multiple meshes will require post processing by the user.  Break the beam detector into multiple {\ct DEVC} inputs, one for each mesh that the beam crosses.  The total obscuration is given by

\be
  \hbox{Obscuration}  = \left(1 -  \prod_{i=1}^N {1-output_i/100};  \right) \times 100  \; \; \hbox{\%}
\ee

where $output_i$ is the FDS output for the beam detector {\ct DEVC}.
...
{\ct INITIAL\_VAPOR\_FLUX}    & Real        & Initial evaporation rate       & m$^2$/(sm$^2$)   & 5E-4   \\ \hline
{\ct ID     }                 & Character   & IDentifier                     &                  &        \\ \hline
...
{\ct REFERENCE\_RATE}         & Real array  & Reaction rates at ref.~temp.   & s$^{-1}$         & 0.001  \\ \hline
...
{\ct CABLE\_DIAMETER}                   & Real          & See Section {info:THIEF}                  & m                     & 0.02      \\ \hline
{\ct CABLE\_FAILURE\_TEMPERATURE}       & Real          & See Section {info:THIEF}                  & $^\circ$C             & 400       \\ \hline
{\ct CABLE\_JACKET\_THICKNESS}          & Real          & See Section {info:THIEF}                  & m                     & 0.002     \\ \hline
{\ct CABLE\_MASS\_PER\_LENGTH}          & Real          & See Section {info:THIEF}                  & kg/m                  & 0.5       \\ \hline
