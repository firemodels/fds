...
REAL(EB) :: T,YY_WALL,DENOM,YY_OTHER(MAX_SPECIES),RHO_YY_OTHER(MAX_SPECIES),YY_EXTERIOR(MAX_SPECIES), &
            YY_G,YY_G_ALL(MAX_SPECIES),RHO_G,UN,DD,EPSB,MFT,TSI,ARO,UWO,RADIUS,AREA,RVC,RHO_NEW,MASS_ADDED,CP
...
IF (VIRTUAL_PARTICLES .AND. CORRECTOR) THEN
   DROPLET_LOOP: DO I=1,NLP
      ...
      CP = Y2CP_C(ITMP)
      D_LAGRANGIAN(II,JJ,KK) =  D_LAGRANGIAN(II,JJ,KK) - QCONF(IW)*AREA*RVC/(RHO(II,JJ,KK)*CP*TMP(II,JJ,KK)) * DR%PWT
   ENDDO DROPLET_LOOP
ENDIF
