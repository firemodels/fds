...
!> \brief Determines the size of the BOUNDARY_PROPS type structure for a surface type
...
IF (MINVAL(M%BOUNDARY_PROPS_OCCUPANCY)==1) THEN  ! There are no open slots for boundary coordinates
   N_NEW_STORAGE_SLOTS = 1000
...
IF (BP_INDEX==0) BP_INDEX = MINLOC(M%BOUNDARY_PROPS_OCCUPANCY,DIM=1) ; M%BOUNDARY_PROPS_OCCUPANCY(BP_INDEX) = 1
...
ALLOCATE(BP%A_LP_MPUA(BOUNDARY_PROPS_REALS_ARRAY_SIZE(1))) ; BP%A_LP_MPUA = 0._EB
IF (ALLOCATED(BP%LP_CPUA)) DEALLOCATE(BP%LP_CPUA)
ALLOCATE(BP%LP_CPUA(BOUNDARY_PROPS_REALS_ARRAY_SIZE(2))) ; BP%LP_CPUA = 0._EB
IF (ALLOCATED(BP%LP_MPUA)) DEALLOCATE(BP%LP_MPUA)
ALLOCATE(BP%LP_MPUA(BOUNDARY_PROPS_REALS_ARRAY_SIZE(3))) ; BP%LP_MPUA = 0._EB
IF (ALLOCATED(BP%LP_TEMP)) DEALLOCATE(BP%LP_TEMP)
ALLOCATE(BP%LP_TEMP(BOUNDARY_PROPS_REALS_ARRAY_SIZE(4))) ; BP%LP_TEMP = TMPA
...
IF (LP%BP_INDEX>0) CALL PACK_BOUNDARY_PROPS(NM,RC,OS,1,LP%BP_INDEX,UNPACK_IT)
...
IF (WC%BP_INDEX>0) CALL PACK_BOUNDARY_PROPS(NM,RC,OS,1,WC%BP_INDEX,UNPACK_IT)
...
IF (CFA%BP_INDEX>0) CALL PACK_BOUNDARY_PROPS(NM,RC,OS,1,CFA%BP_INDEX,UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_PROPS(NM,RC,OS,STORAGE_INDEX,BP_INDEX,UNPACK_IT)
