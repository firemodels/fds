...
IF (N_GEOMETRY>0) THEN
   ...
   DO I = 1, N_GEOMETRY
      ...
      IF (TRIM(G%SURF_ID(1))=='null') THEN
         WRITE(LU_SMV,'(1X,A,1X,3(E13.6,1X),I2,1X,A,1X,3(I3,1X),1X,E13.6,1x,I7)') TRIM(G%TEXTURE_MAPPING), G%TEXTURE_ORIGIN, &
                                                    IS_TERRAIN_INT, '!', G%RGB, G%TRANSPARENCY, G%N_FACES
      ELSE
         WRITE(LU_SMV,'(1X,A,1X,3(E13.6,1X),I2,1X,A,1X,A,1X,A,1X,3(I3,1X),1X,E13.6,1x,I7)') TRIM(G%TEXTURE_MAPPING), G%TEXTURE_ORIGIN, &
                                                    IS_TERRAIN_INT, '%',TRIM(G%SURF_ID(1)), '!', G%RGB, G%TRANSPARENCY, G%N_FACES
      ENDIF
   ENDDO
