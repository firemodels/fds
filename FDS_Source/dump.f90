...
CHARACTER(LABEL_LENGTH) :: QUANTITY,DATE,ODE_SOLVER,EXTINCTION_MODEL
...
DO NM=1,NMESHES
   ...
   IF (M%DT>0.00001_EB) THEN
      WRITE(LU_OUTPUT,111) M%DT,T(NM)
   ELSE
      WRITE(LU_OUTPUT,114) M%DT,T(NM)
   ENDIF
   WRITE(LU_OUTPUT,115) M%CFL,M%ICFL,M%JCFL,M%KCFL, M%DIVMX,M%IMX,M%JMX,M%KMX, M%DIVMN,M%IMN,M%JMN,M%KMN
...
115 FORMAT(6X,' Max CFL number: ',E9.2,' at (',I3,',',I3,',',I3,')'/ &
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,&
            ...
            DISSIPATION_RATE,S11,S22,S33,S12,S13,S23,DUDX,DUDY,DUDZ,DVDX,DVDY,DVDZ,DWDX,DWDY,DWDZ,ONTHDIV,SS,ETA
...
SELECT CASE(IND)
   ...
   CASE(99) ! EQUILIBRIUM VAPOR FRACTION
      GAS_PHASE_OUTPUT_RES = EQ_VAP_FRAC(II,JJ,KK,Z_INDEX)          
