...
\be
  \mu_{\hbox{\tiny LES}} =  \rho \,  (C_{\rm s}\, \Delta)^2  \, |S|
\ee
where $C_{\rm s}$  is an empirical constant, $\Delta$ is a length on the order of the size of a grid  cell,  and  the  deformation   term  $|S|$  is
...
Thus, $C_{\rm s}$, $\PR_t$  and $\SC_t$ are  a set  of  empirical constants.  Most  FDS users
...
Smagorinsky parameters,  and suggested the current  default values. Of the  three parameters,  the  Smagorinsky constant  $C_{\rm s}$  is the  most
...
\section{2D Analytical Solution to Navier-Stokes (\texorpdfstring{{\bf ns2d}}{ns2d})}
\label{ns2d}
...
\section{2D Vortex Simulation (\texorpdfstring{{\bf vort2d}}{vort2d})}
\label{two_dimensional_vortex}

{\bf Max Gould, NIST SURF student}\\
{\bf Ragini Acharya, United Technologies Research Center}\\
...
\begin{figure}[h]
   ...
   \end{tabular*}
   \caption[Time history of a velocity profile showing qualitative convergence]{$u$-velocity along the $z$-axis at $x=0$ plotted for each of the vortex's first four loops through the computational domain. (Left) $80^{2}$ grid cell model. (Right) $160^{2}$ grid cell model.}
...
\begin{figure}[h!]
   \centering
    ...
    \caption[Two-dimensional vortex convergence study]{RMS error between simulated and analytical $u$-velocity values along the $z$-axis plotted for each grid resolution at each of three subsequent passes of the vortex through the computational domain.}
    \label{fig_vort2d_error}
\end{figure}

The rate with which the simulated profiles converge to the analytical one defines the order of accuracy of the numerical scheme. In Figure~\ref{fig_vort2d_error} we plot the rms error of the numerical solution as a function of the grid resolution. The three colored curves represent the rms error at three different pass-through times. The broken and solid black lines represent the plot gradient corresponding to first and second order error respectively. While the error increases with each flow-through time, the gradients of the lines are roughly parallel to the solid black line, indicating second-order accuracy of the numerical scheme.
...
\end{equation}
In Figure \ref{fig_vort2d_pointvelocity}, we show the $u$-velocity at a single point, on the lower left fringe of the vortex, for two different mesh resolutions, $80^{2}$ and $160^{2}$. A mesh resolution of $320^{2}$ grid cells produces a plot that is, to the eye, a perfect match out to four pass-through times.
...
\subsection{Constant Smagorinsky (\texorpdfstring{{\bf csmag}}{csmag})}
...
\subsection{Dynamic Smagorinsky (\texorpdfstring{{\bf dsmag}}{dsmag})}
...
\begin{figure}[h]
   ...
   \end{tabular*}
   \caption[Integrated and spectral energy decay for CBC experiment using Deardorff's model]{\label{fig_deardorff_cbc} Deardorff model results (analogous to Figure \ref{fig_cbc_energy}) for integrated kinetic energy (left) and spectra (right).}
\end{figure}

\clearpage

\subsection{Vreman (\texorpdfstring{{\bf vreman}}{vreman})}
\label{Vreman_model}
...
\begin{figure}[h]
   ...
   \end{tabular*}
   \caption[Integrated and spectral energy decay for CBC experiment using Vreman's model]{\label{fig_vreman_cbc} Vreman model results (analogous to Figure \ref{fig_cbc_energy}) for integrated kinetic energy (left) and spectra (right).}
\end{figure}

\clearpage

\section{Jet Centerline Velocity Decay (\texorpdfstring{{\bf jet}}{jet})}

{\bf Gregor J\"ager, BFT Cognos GmbH, Germany}\\

\noindent The {\bf jet\_} series in the Validation/Turbulent\_Jet directory tests the ability of subgrid stress closure (turbulence model) to correctly capture the transition from laminar to turbulent flow and subsequent momentum decay for an incompressible jet.
...
\subsection{The FDS Moody Chart (\texorpdfstring{{\bf poiseuille}}{poiseuille}, \texorpdfstring{{\bf moody}}{smoothwall}, and \texorpdfstring{{\bf z0}}{z0})}
...
With the same grid set up as described above the smooth walls, a series of cases labeled as {\bf z0}\footnote{The atmospheric community typically uses $z_0$ to denote the roughness height. However, we also use $s$ interchangeably with $z_0$ to conform to the notation used in Pope \cite{Pope:2000}.} in the repository were run at various roughness heights, grid resolutions, and Reynolds numbers. The results are presented together with the smooth wall cases in Fig.~\ref{fig_fds_moody_chart}. The laminar cases provide accurate results for two different Reynolds numbers. Both the smooth wall and rough wall treatments behave well over the range tested.
...
\subsection{Blasius boundary layer (\texorpdfstring{{\bf blasius}}{blasius})}

{\bf Hyun-Wook Park and Jung-il Choi, Yonsei University, Seoul, South Korea}\\

\noindent Consider a steady flow over a semi-infinite flat plate aligned with the flow with free stream velocity $u_0$ . The Prandtl boundary layer equations (see \cite{Panton:1}) are:
\begin{equation}
...
Famously, Paul Richard Heinrich Blasius found a solution to Eqs.~(\ref{eqn_BL_momentum}) and (\ref{eqn_BL_continuity}) by introducing a similarity variable:
\begin{equation}
\label{eta}
\eta\equiv\frac{z}{\sqrt{\nu x/u_0}} \,\mbox{.}
\end{equation}
By substituting $\eta$ and the stream function ($u = \partial \psi/\partial z$, $w = -\partial \psi/\partial x$) into Eq.~(\ref{eqn_BL_momentum}), we get the following ODEs:
\begin{eqnarray}
\label{BL_ODE}
\frac{1}{2}f f^{''}+f^{'''}&=&0 \\
u&=&u_0 f^{'} \\
...
\subsection{Near-Wall Grid Resolution (\texorpdfstring{{\bf yplus}}{yplus})}

{\bf Abbas Jaber, NIST SURF student}\\
...
\subsection{Pulsating 2D solution (\texorpdfstring{{\bf pulsating}}{pulsating})}
\label{soln_2d}
...
\subsection{Stationary compression wave in 2D (\texorpdfstring{{\bf compression\_wave}}{compression\_wave})}
\label{stationary_2d}
...
\section{Scalar Transport (\texorpdfstring{\bf move\_slug}{move\_slug})}
...
The specification of the fuel mass flux is probably the most commonly used boundary condition for specifying a fire.  When the user specifies a \textct{HRRPUA} (heat release rate per unit area), FDS divides this input by the heat of combustion for the specified fuel (propane by default) and uses this values as the specified mass flux $\dot{m}_{\alpha}^{\prime\prime}$ at the \textct{VENT}.  This seems simple enough.  But the problem is complicated by the fact that the total species flux is the sum of the advective and diffusive fluxes,
\begin{equation}
...
In the test case, we inject propane into a cube with 1 m$^2$ vents on all six faces.  We specify the \textct{MASS\_FLUX} of propane to be 0.0001 kg/(m$^2$-s) and the temperature of the face of the \textct{VENT} (\textct{TMP\_FRONT}) is specified to be 500 $^\circ$C.  The box is initially filled with air at standard conditions.  We \textct{DUMP} a \textct{MASS\_FILE} and compare the accumulation of propane with the specified rate.  The results are shown in Figure \ref{fig_low_flux_hot_gas_filling}.
...
\end{tabular*}
\caption[Low flux of hot gas into a closed domain]{Test of specified mass flux boundary condition for low mass flux and specified temperature on the face of the \textct{VENT}. (Left) Image of the domain showing contours of propane mass fraction. (Right) Comparison of the accumulated mass of propane with the FDS values reported in the \textct{\_mass} file.}
...
This example checks that if nitrogen is added via an external vent to a sealed compartment with adiabatic (i.e., no heat loss) walls, that the
...
\be \frac{\d p}{\d t} = \frac{\gamma \, p \, u \, A}{V} \ee
where $u$ is the injection velocity and $A$ is the area of the vent. The vent area is 0.04~m$^2$. The density of the incoming N$_2$ is found from the equation of state:
\be \rho = \frac{ \overline{W} \, p }{ \calR T} \ee
...
\be \frac{\d p}{\d t} = \frac{\gamma \, \dot{m}'' \, \calR T \, A}{\overline{W} \, V} \ee
Note that the pressure rise is constant. In both the fast and slow injection cases, the pressure is expected to rise 64,282~Pa above ambient. The density
...
The enthalpy of the mixture at the outlet is $2 \times 1 \times (900+273.15) + 2 \times 10 \times (20+273.15) = 8209.3$ kJ/(m$^2$-s).  The mixture heat capacity at the outlet is 5.5 kJ/(kg-K) and the flux is 4 kg/(m$^2$-s).  Therefore the outlet temperature is $8209.3/4/5.5 - 273.15 = 100$ $^\circ$C.
\begin{figure}[ht]
\centering
\includegraphics[width=3.2in]{SCRIPT_FIGURES/energy_budget_tmix}
\caption[Outlet temperature for {\bf energy\_budget\_tmix} case]{\label{fig_outlet_temperature} Outlet temperature for {\bf energy\_budget\_tmix} case.  Black circles represent the correct result of 100 $^\circ$C.}
\end{figure}

\clearpage

\subsection{Solid phase energy conservation (\texorpdfstring{\bf energy\_budget\_solid}{energy\_budget\_solid})}

This case tests the ability of FDS to properly conserve the total of the solid phase and the gas phase energy.  A 1 m$^3$ box is filled with 1 kg of a gas with a specific heat of 1 kJ/(kg-K), a molecular weight of 28 g/mol, and an initial temperature of 20 $^\circ$C.  All the walls, except the floor, are adiabatic.  The floor is defined as a 1 mm thick layer of a material with a conductivity of 100 W/(m-K),  a specific heat of 1 kJ/(kg-K), an initial temperature of 150 $^\circ$C, and a density of 1,000 kg/m$^3$.  The initial enthalpy is $1 \times 1 \times (20+273.15) + 1 \times 1 \times (150+273.15)  =  716.3$ kJ.  At the end state, the gas will have risen in temperature, the solid will have decreased in temperature, and the pressure will have risen.  Since energy is conserved this occurs at a temperature of 96.3 $^\circ$C.  The final enthalpy is $1 \times 1 \times (96.3+273.15) + 1 \times 1 \times (96.3+273.15) + 22670 / 1000  =  716.3$ kJ.
...
\caption[The {\bf energy\_budget\_solid} case]{\label{fig_solid_energy} Outputs of temperature (left) and pressure (right) for the {\bf energy\_budget\_solid} case.  Circles represent the correct result.}
\end{figure}
...
\subsection{Finite difference errors (\texorpdfstring{{\bf symmetry\_test}}{symmetry\_test})}
...
\end{tabular*}
\caption[The {\bf velocity\_bc\_test} case]{Centerline time-averaged pressure (left) and velocity (right) profiles for the {\bf velocity\_bc\_test} case.}
\label{velocity_bc_test}
\end{figure}


\subsection{Checking the divergence constraint (\texorpdfstring{{\bf divergence\_test}}{divergence\_test})}
...
\end{center}
\caption[Results for the {\bf divergence\_test} case]{Minimum and maximum divergence for the {\bf divergence\_test} case.}
...
\be \bs \cdot \nabla I_{\la}(\bx,\bs) =
 -\Big[ \kappa(\bx,\la) + \sigma_{\rm s}(\bx,\la) \Big] \;
I_\la(\bx,\bs) +B(\bx,\la) + \frac{\sigma_{\rm s}(\bx,\la)}{4\pi}
...
in different co-ordinate systems. The radiating surface is at 1,000~$^\circ$C temperature and has emissivity of 1.0.  The exact values are
...
A comparison of exact values and FDS predictions at three different angular resolutions is shown below.
\begin{center}
...
These cases check that energy is conserved when radiation is absorbed by liquid droplets. We want to make sure that the absorbed thermal radiation is accurately converted into internal energy of the droplets. The rate of increase of the average droplet temperature, $\overline{T_{\rm p}}$, is related to the rate of radiation absorption of all the droplets, $\dot{Q}_{\rm r}$, according to the following equation:
\be
m \, c_p \frac{\d \overline{T_{\rm p}}}{\d t} = \dot{Q}_{\rm r}
\ee
...
\be
\overline{T_{\rm p}} = T_0 + \frac{\Delta t \, \dot{Q}_{\rm r} }{m \, c_p}
\ee
By setting $T_0=0$~$^\circ$C, $\Delta t =0.01$~s, $m =0.01$~kg, and $c_p =1.0$~kJ/(kg$\cdot$K), the final value of $\overline{T_{\rm p}}$ should equal $\dot{Q}_{\rm r}$. Figure~\ref{droplet_absorption_figures} shows comparisons of the predicted average droplet temperature and the expected value in Cartesian and cylindrical coordinate systems.
...
\section{Radiation Absorbed by Solid Particles (\texorpdfstring{{\bf particle\_absorption}}{particle\_absorption}) }
\label{particle_absorption}

Tests similar to those in Section~\ref{droplet_absorption} are performed for solid particles with Cartesian, cylindrical and spherical shapes. All the properties are the same, except the particle mass ($m_{\rm cart} = 6.26\times 10^{-4}$~kg, $m_{\rm cyl} = 1.96\times 10^{-3}$~kg, $m_{\rm sph} = 6.54\times 10^{-4}$~kg). It is also necessary to define a high thermal conductivity $k = 1\times 10^{9}$~W/(m$\cdot$K) because the particles are not assumed to be thermally-thin like liquid droplets. Figure~\ref{droplet_absorption_surface_figures} shows comparisons of the predicted average droplet temperature and the expected value. By default, the radiation is expressed in terms of an integrated average (0 angles), but there is an option to ``split'' the particles into multiple parts that absorb radiation impinging from different directions. In this case, radiation is split into opposite $x$, $y$ and $z$ directions (except for Cartesian particles; it is only in the $z$ direction).
...
\caption[The {\bf radiation\_shield} test case]{(Left) Heat flux to a cold plate separated from a hot plate by a heat shield. At 400~s, the shield is removed. (Right)
...
\section{Product Yields (\texorpdfstring{\bf methane\_flame}{methane\_flame})}
...
\section{Reaction Rates (\texorpdfstring{\bf reactionrate}{reactionrate})}
...
\subsection{Mixing-Controlled Reactions (\texorpdfstring{\bf reactionrate\_EDC}{reactionrate\_EDC})}

The cases presented in the section are designed to examine species evolutions for reactions of varying complexity using the EDC model with infinitely fast chemistry. The first test case is a one-step methane reaction that occurs in a constant volume, adiabatic ``reactor'' with a fixed mixing time of $\tau_{mix}=0.125$~s. Each computational cell contains a stoichiometric mixture of fuel and air. In this problem ``air'' is defined to be 21\% oxygen and 79\% nitrogen by volume.
...
\caption[Species evolution in a 1-step methane EDC reaction]{Time evolution of species mass fraction for a one-step methane EDC reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\end{equation}
where $P$ represents the products, $R$ represents the reactants, and $\overline{u}_{\alpha}=\overline{h}_{\alpha}-\overline{R}T$. The total enthalpy, $\overline{h}_{\alpha}$, is defined as the sum of the enthalpy of formation ($\overline{h}_{f}^{\circ}$) and sensible enthalpy ($\Delta \overline{h}$) for each of the $\alpha$ species. Assuming the products behave like an ideal gas, the final pressure can be found using the equation of state for an ideal gas:
...
\end{tabular*}
\caption[Temperature and pressure evolution in a 1-step methane EDC reaction]{Time evolution of temperature and pressure for a one-step methane EDC reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\caption[Species evolution in a 1-step propane EDC reaction]{Time evolution of species mass fraction for a one-step propane EDC reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\end{tabular*}
\caption[Temperature and pressure evolution in a 1-step Methane EDC reaction]{Temperature (left) and pressure (right) evolution for a one-step methane EDM reaction with a fixed mixing time of $\tau_{mix}=0.125$~s.}
...
\end{align}
Each cell in the model reaction is initialized with an oxygen rich mixture and the mixing time, $\tau_{mix}$, is fixed at $0.1$~seconds (Figure \ref{fig:EDC_2Step_fuel}).

\begin{figure}[h!]
\begin{center}
\includegraphics[height=2.2in]{SCRIPT_FIGURES/reactionrate_EDC_flim_2step}
\caption[Species evolution in 2-step methane EDC reaction]{Time evolution of species mass fraction for a two-step, fuel-limited EDC reaction with a fixed mixing time of $\tau_{mix}=0.1$~s.}
...
\end{align}
The species evolutions are compared to expected values in Figure \ref{fig:EDC_2Step_ox}.
\begin{figure}[h!]
\begin{center}
\includegraphics[height=2.2in]{SCRIPT_FIGURES/reactionrate_EDC_O2lim_2step}
\caption[Species evolution in two competing EDC reactions]{Time evolution of species mass fraction for a two-step, oxygen-limited EDC reaction with a fixed mixing time of $\tau_{mix}=0.1$~s.}
...
\subsection{Finite Rate Reactions (\texorpdfstring{\bf reactionrate\_arrhenius}{reactionrate\_arrhenius})}

The main difference between Arrhenius rate reactions and the mixing-controlled reactions is that the reaction rate in finite rate reactions is found from local parameters such as species concentration and temperature as well as rate constants. The values of $a_{\alpha}$ in Equation (\ref{eq:finite_rate1}) are specific to each reaction and for non-zero values, $\d Y_{\alpha}/ \d t$ becomes  a non-linear differential equation. We consider both linear and non-linear cases with one and two-step reaction mechanisms. First we examine two one-step cases; a zero-order and second-order reaction. Equation (\ref{eq:1step_CO}) shows a one-step, zero-order test case of carbon monoxide oxidation:
...
\begin{align}\label{eq:2step_propane}
...
\end{center}
The species evolutions for the two-step 1.75 order reaction mechanism are shown in Figure \ref{fig:Arrhenius_175Order_2step}. We also consider a two-step reversible propane reaction:
\begin{align}\label{eq:2step_propane_rev}
\mathrm{C_3H_8+3.5 \, O_2} &\rightarrow \mathrm{3 \, CO+4 \, H_2O} \\
...
$\alpha$ is defined as [$\mathrm{C_3H_8}$ $\mathrm{O_2}$ $\mathrm{CO}$ $\mathrm{H_2O}$ $\mathrm{CO_2}$].
\end{center}
Equation (\ref{eq:2step_propane_rev}) includes the decomposition of $\mathrm{CO_2}$ into $\mathrm{CO}$ and $\mathrm{O_2}$ whereas the forward-only two-step mechanism, Equation (\ref{eq:2step_propane}), does not. This reverse reaction mechanism is represented by three separate reactions on the \textct{REAC} line in an FDS input file as indicated by rows $\mathrm{a}$, $\mathrm{b}$ and $\mathrm{c}$ in the preceding table. Species evolutions for the 2 step reverse reaction are shown in Figure \ref{fig:Arrhenius_175Order_2stepR}. In the forward CO step, (row $\mathrm{b}$ in the two prior tables), we see that water vapor contributes to the reaction rate based on its local concentration despite not being a participant in the reaction.
\vskip8pt
...
\section{Mixture Fraction ({\bf burke\_schumann})}
\label{Mix_Frac}
...
\section{Extinction Model ({\bf extinction})}
\label{Extinct}
...
\end{center}
\caption[{\bf extinction} model test cases]{Array of initial temperatures and oxygen concentrations that can can sustain burning.}
\label{fig:extinct}
\end{figure}
...
\section{Ideal and Non-Ideal Heat of Combustion ({\bf extinction})}
\label{HoC}
...
\end{tabular*}
\caption[{\bf HoC\_Ideal} and {\bf HoC\_NonIdeal} test cases]{Demonstration of correct adjust to heat of combustion for minor products of combustion.}
\label{fig:HoC}
\end{figure}
...
\ee
where $t$ is time (minutes), and $C_\mathrm{CO}$ is the CO concentration (ppm). The fraction of an incapacitating dose of CN is calculated as
\be
\mathrm{FED}_\mathrm{CN} = \int_0^t \left( \frac{\exp\left( \frac{C_\mathrm{CN}(t)}{43} \right)}{220} - 0.0045 \right) \, \d t
\ee
where $t$ is time (minutes), and $C_\mathrm{CN}$ is the concentration (ppm) of HCN corrected for the protective effect of NO$_\mathrm{2}$. $C_\mathrm{CN}$ is calculated as
\be
...
\ee
where $t$ is time (minutes), and $C_\mathrm{NO_x}$ is the sum of NO and NO$_\mathrm{2}$ concentrations (ppm).
...
\mathrm{FLD}_\mathrm{irr} = \int_0^t \left(
    ...
    \right) \, \d t
\ee
where $t$ is time (minutes), the numerators are the instantaneous concentrations (ppm) of each irritant, and
...
\ee
where $t$ is time (minutes), and $C_\mathrm{O_2}$ is the O${}_2$ concentration (volume percent).
...
\ee
where $t$ is time (minutes), and $C_\mathrm{CO_2}$ is the CO${}_2$ concentration (percent).
...
\end{tabular*}
\caption[The FED and FIC values in {\bf FED\_FIC\_SMIX} test cases]{Comparison of FED and FIC predictions with expected values using \textct{SMIX}.}
\label{FED_FIC_SMIX_plot}
\end{figure}

\clearpage

\section{Smoke Detector Model (\texorpdfstring{{\bf smoke\_detector}}{smoke\_detector}) }
...
Figure~\ref{fig:smoke_detector} shows a comparison of the analytical and simulated results for the time evolution of the smoke detector chamber obscuration.
...
\end{tabular}
\end{center}
\caption[Smoke detector chamber obscuration for {\bf smoke\_detector} case]{Time evolution of smoke detector chamber obscuration for {\bf smoke\_detector} case}
...
\subsection{Surface Deposition of Aerosols (\texorpdfstring{\bf propane\_flame\_deposition}{propane\_flame\_deposition})}
...
\end{tabular}
\end{center}
\caption[Soot mass conservation for {\bf propane\_flame\_deposition} case]{Time evolution of soot mass components for {\bf propane\_flame\_deposition} cases. (Top left) Mass of soot in gas phase. (Top right) Mass of soot deposited on walls. (Bottom) Total mass of soot in gas phase and deposited on walls.}
\label{fig:deposition_mass}
\end{figure}

\clearpage

\subsection{Gravitational Settling of Aerosols (\texorpdfstring{\bf soot\_gravitational\_settling}{soot\_gravitational\_settling})}

The calculations described in this section check the conservation of mass for the gravitational settling algorithm. The gravitational settling algorithm involves both the settling of soot particles in the gas phase and the accumulation of soot on the condensed phase. For this example, the initial soot concentration in a 1~m$^3$ box is set as 100~mg/m$^3$ via the \textct{INIT} line, and the soot deposits to the bottom of the box via gravitational settling only. By default, if a species is defined as an aerosol by setting \textct{AEROSOL=.TRUE.} on the \textct{SPEC} line, then all of the aerosol deposition mechanisms (gravitational, thermophoretic, and turbulent) will act upon that species. For this example, gravitational deposition is enabled and the other deposition mechanisms are disabled by using the following inputs on the \textct{MISC} line:
...
\end{tabular}
\end{center}
\caption[Soot mass concentration for {\bf soot\_gravitational\_settling} case]{Time evolution of soot mass concentration for the {\bf soot\_gravitational\_settling} case.}
...
\section{Heat Conduction Through a Solid Slab (\texorpdfstring{{\bf heat\_conduction}}{heat\_conduction})}
...
The sample of homogeneous material is initially at 0~$^\circ$C and at $t>0$ exposed to a gas at 700~$^\circ$C. A fixed heat transfer coefficient of
10~W/m$^2$/K is assumed. The density of the material is 10,000~kg/m$^3$. The conductivity and specific heat are functions of temperature with the
...
\section{Convective Cooling (\texorpdfstring{{\bf convective\_cooling}}{convective\_cooling})}

This example tests the convective cooling algorithm. A 1~m thick solid slab that is initially at 1000~$^\circ$C is suddenly exposed to air at 0~$^\circ$C. The back of the slab is insulated. Its density is 1000~kg/m$^3$, its specific heat is 0.001~kJ/(kg$\cdot$K), its conductivity is 1~W/(m$\cdot$K), and its emissivity is zero, meaning there is no radiative loss from the surface. The convective heat transfer coefficient is 1~W/(m$^2\cdot$K). The predicted and exact temperature at the back side of the slab are displayed in Fig.~\ref{convective_cooling}.
...
\end{center}
\caption[The {\bf convective\_cooling} test case]{Predicted vs.~exact back side temperatures of a thick slab cooled by convection alone.}
...
\section{Heat Conduction through Insulated Steel (\texorpdfstring{{\bf insulated\_steel\_x}}{insulated\_steel\_x})}

In these examples, a 1~cm thick steel plate and pipe are coated on each side with 2~cm thick insulation material. The conductivity, specific heat, and density of the steel are 50~W/m/K, 0.5~kJ/kg/K, and 7500~kg/m$^3$, respectively. The corresponding values for the insulation material are 0.2~W/m/K, 1~kJ/kg/K, and 200~kg/m$^3$, respectively. The heat transfer coefficient is 10~W/m$^2$/K. The outer surface of the composite is exposed to air at 480~$^\circ$C while the inner surface is exposed to air at 20~$^\circ$C. The assemblies are heated for 10~h and the steady-state temperature profiles are shown in Fig.~\ref{insulated_steel_plate}. Neglecting thermal radiation, the heat flux through the plate is given by~\cite{Incropera:1}:
\be
...
\end{tabular*}
\caption[The {\bf insulated\_steel} test cases]{Steady-state temperature profile within an insulated steel plate and pipe.}
...
\section{Mass conservation of pyrolyzed mass (\texorpdfstring{{\bf surf\_mass\_conservation}}{surf\_mass\_conservation})}

The calculations described in this section check the conservation of
mass produced by the pyrolysis algorithm. Parameters describing the geometric configuration of the solid are input via a \textct{SURF} line,  and
the reacting materials are described on respective \textct{MATL} lines. In the tests, four independent modeling options are varied:
\begin{enumerate}
\item The \textct{SURF} line can be associated with either a solid surface as designated by a \textct{VENT} line, or by solid particles as
described by a \textct{PART} line.
...
mixture fraction model, or an additional gas species, defined by a \textct{SPEC} line.
\end{enumerate}
...
The analytical mass losses are calculated by multiplying the mass per unit area by the \textct{VENT} area,
...
\end{tabular*}
\caption[The {\bf  surf\_mass\_vent\_char} test cases]{Comparison of analytical and predicted mass change for charring surfaces that are introduced as \textct{VENT}s.}
...
\end{tabular*}
\caption[The {\bf surf\_mass\_vent\_nonchar} test cases]{Comparison of analytical and predicted mass change for non-charring surfaces that are introduced as \textct{VENT}s.}
...
For Cartesian surfaces, the particle area is two times the product of the parameters \textct{LENGTH} and \textct{WIDTH} on the \textct{SURF} line, both of
...
specified on the \textct{SURF} line as \textct{THICKNESS}.
For cylindrical particles, the \textct{LENGTH} is 0.1~m, and the radius, $r$
(specified via \textct{THICKNESS}), is 0.01~m. The expected masses are thus
...
\end{tabular*}
\caption[The {\bf surf\_mass\_part\_char} test cases]{Comparison of analytical and predicted mass change for charring particle surfaces.}
...
\end{tabular*}
\caption[The {\bf surf\_mass\_part\_nonchar} test cases]{Comparison of analytical and predicted mass change for non-charring particle surfaces.}
...
\end{center}
\caption[The {\bf surf\_mass\_part\_char\_cyl\_gas\_advanced} test cases]{Comparison of analytical and predicted mass change for a charring, cylindrical surface
...
%\end{tabular*}
%\caption[The \textct{surf\_mass\_part\_fuel\_split} test cases.]{Comparison of analytical and predicted mass change for splitted particle surfaces.}
...
%\end{tabular*}
%\caption[The \textct{surf\_mass\_part\_fuel\_split} test cases.]{Comparison of analytical and predicted total energy release for splitted particle surfaces.}
...
\subsection{Burning Rate of a Small Object (\texorpdfstring{{\bf cell\_burn\_away}}{cell\_burn\_away})}

In FDS, it is assumed that heat conduction and pyrolysis within solids are functions of depth only. This can be problematic in cases where entire solid obstructions burn away. In this example, a single grid cell, 4~cm on a side, is made up of a material with density $\rho_{\rm s}=50$~kg/m$^3$ and constant reaction rate, $r_{\rm s}=0.05$~s$^{-1}$. The burning rate of the single grid cell, $\dm$, is given by:
\be
...
fuel was exposed to a 50 kW/m$^2$ heat flux and the evolution of gas phase fuel mass and surface density were tracked. Results are shown in the
figure below.
\begin{figure}[ht]
\begin{center}
\includegraphics[width=3.5in]{SCRIPT_FIGURES/surf_mass_vent_liquid_fuel}
\end{center}
\caption[The {\bf surf\_mass\_vent\_liquid\_fuel} test case.]{Comparison of analytical and predicted mass change for a liquid surface.}
...
\section{Shrinking and swelling materials (\texorpdfstring{{\bf shrink\_swell}}{shrink\_swell})}
...
a static matrix structure (\textct{ALLOW\_SHRINKING=.FALSE.} or \textct{ALLOW\_SWELLING=.FALSE.}) the layer thickness should not change. The parameters of the
six test cases are shown in the following table. The figures below show the surface densities, which obviously should not change, and the surface
...
Case     & \multicolumn{2}{|c|}{Reactant}  &  \multicolumn{3}{|c|}{Inert} & Product & \multicolumn{4}{|c|}{Layer} \\ \hline
         ...
         & (kg/m$^3$)&       & (kg/m$^3$)&       & shrink/swell & (kg/m$^3$)& (kg/m$^3$)  & (kg/m$^3$)    & (m)         & (m) \\ \hline
...
\end{tabular*}
\caption[The {\bf shrink\_swell} test case]{Testing the shrinking and swelling surfaces.}
\label{shrink_swell2}
\end{figure}
...
30~kg/m$^3$ and 10~W/m/K, respectively. The emissivity of the front and back is 1.  The specific heat of material A changes from
...
For material properties, we choose $\rho_{\rm s} = \rho_l =1,000$~kg/m$^3$,
$k_{\rm s} = k_l = 2$~W/m/K, $c_{\rm s} = c_l = 2$~kJ/kg/K, $H_{r,\alpha\beta}=300$~kJ/kg, $T_f = 0$~$^\circ$C, $T_i = +15$~$^\circ$C,
...
``FDS min'' is the largest value of $x$ for which $\rho_{\rm s} > 990$~kg/m$^3$ and ``FDS max'' is the largest $x$ for which $\rho_{\rm s} > 0$~kg/m$^3$. The plot on the right
...
\be
   \frac{\d Y_i}{\d T} = -\frac{A_i}{\beta} \, Y_i \, \exp \left( -\frac{E_i}{RT} \right)    \quad ; \quad Y(T=T_0) = Y_{0,i}  \label{dYdT}
\ee
...
\be
   \frac{\d^2 Y_i}{\d T^2} = -\frac{A_i}{\beta} \, \frac{\d Y_i}{\d T} \, \exp \left( -\frac{E_i}{RT_{p,i}} \right) -
   \frac{A_i}{\beta} \, Y_i \, \exp \left( -\frac{E_i}{RT_{p,i}} \right) \, \frac{E_i}{R T_{p,i}^2}
   = -\frac{\d Y_i}{\d T} \left[ \frac{A_i}{\beta} \, \exp \left( -\frac{E_i}{RT_{p,i}} \right) - \frac{E_i}{R T_{p,i}^2} \right] = 0  \label{d2YdT2}
\ee
Next, Eq.~(\ref{dYdT}) can be integrated from $Y_{0,i}$ to $Y_{p,i}$ (the value of $Y_i$ at the peak), and $T_0$ to $T_{p,i}$:
\be
   \int_{Y_{0,i}}^{Y_{p,i}} \left( \frac{\d Y_i'}{Y_i'} \right) = - \frac{A_i}{\beta} \int_{T_0}^{T_{p,i}} \exp \left( -\frac{E_i}{RT'} \right) \; dT'
...
exercise. Note that \textct{REFERENCE\_TEMPERATURE} is $T_{p,i}$, but in units of $^\circ$C. \textct{REFERENCE\_RATE} is actually
$r_{p,i}/Y_{0,i}$, in units of s$^{-1}$. \textct{HEATING\_RATE} is $\beta$ in units of K/min.
\textct{NU\_RESIDUE} is $\nu_r$ and \textct{NU\_FUEL} is ($1-\nu_r$).
...
\section{Fluid-Particle Momentum Transfer (\texorpdfstring{\bf fluid\_part\_mom\_x, \_y, \_z}{fluid\_part\_mom\_x, \_y, \_z})}
...
channel with periodic boundary conditions on the $x$-faces and \textct{FREE\_SLIP}
...
\begin{table}[ht]
\begin{center}
\caption[Parameters for {\bf particle\_drag} cases]{Parameters used in the {\bf particle\_drag} cases.}
...
\section{Pressure Drop through Stationary Particles (\texorpdfstring{\bf drag\_dt}{drag\_dt})}

The pressure drop test cases consider a 1 m wide by 1 m tall by 2 m long channel with a 2 m/s fixed velocity boundary condition on one end, an \textct{OPEN} boundary condition on the other end, and \textct{FREE\_SLIP} on the y- and z-faces.  Three channels are stacked vertically (each with its own \textct{MESH}, and a plane of particles is placed across each channel at 1 m.  The particles are 0.01 m in diameter and 10 particles are placed per cell.  A \textct{USER\_DRAG\_COEFFICIENT} of 5, 10, and 20 is set for the particles in the channels.  According to
\be \Delta P=\frac{1}{2} \rho \frac{\sum \, C_{D} \, \pi \, r_{d}^{2}} {A} \ee
where A$_{cx}$ is the 1 m$^2$ cross-sectional area, the pressure drops in each sub channel are 3.77, 7.54, and 15.1~Pa.  Comparisons of computed and analytical results are shown in Figure~\ref{particle_drag_plots}. The pressure drop converges to the analytical solution as the time step decreases.
...
\\
\end{tabular*}
\caption[Sample cases {\bf drag}]{Outputs of the {\bf drag\_dt} test cases compared with analytical solutions.}
...
\section{Mesh Screens (\texorpdfstring{\bf screen\_drag}{screen\_drag})}
...
\caption[Sample cases {\bf screen\_drag}]{Outputs of the {\bf screen\_drag} test cases compared with analytical solutions.  Left 2~m/s inlet, right 20~m/s inlet.}
\label{screen_drag_plots}
\end{figure}

\section{Particles in 1D (\texorpdfstring{\bf terminal\_velocity}{terminal\_velocity})}
...
Droplet fluid density & $\rho_d$ & 1,000~kg/m$^3$ \\
\hline
\end{tabular}
\end{center}
As shown in Fig.~\ref{fig_terminal_velocity_convergence}, FDS exhibits second-order convergence in the {\bf terminal\_velocity} case for both the velocities and the positions.
...
\end{tabular*}
\caption[Convergence analysis of the {\bf terminal\_velocity} case]{(Left) FDS convergence for the particle velocity in the {\bf terminal\_velocity} case. (Right) FDS convergence for the particle position in the {\bf terminal\_velocity} case.}
...
\section{2D Flat Fire Trajectory (\texorpdfstring{\bf flat\_fire}{flat\_fire})}
...
\ee
\be
    u = \frac{\d x}{\d t} = \frac{V_0}{V_0 K t + 1} \quad ; \quad w = \frac{\d z}{\d t} = \frac{g}{2 V_0 K (V_0 K t + 1)} - \frac{g t}{2} - \frac{g}{2 V_0 K}
\ee
...
\end{tabular*}
\caption[The {\bf flat\_fire} case]{Output of test case {\bf flat\_fire}.}
...
\begin{equation} \d e = \d h - \frac{1}{\rho} \; \d p - p \; d \left(\frac{1}{\rho}
\right) = \d h - v \; \d p - p \; \d v \end{equation}
...
\end{tabular*}
\caption[Sample case {\bf water\_evaporation\_1}]{Output of the {\bf water\_evaporation\_1} test case.}
...
\end{tabular*}
\caption[Sample case {\bf water\_evaporation\_2}]{Output of the {\bf water\_evaporation\_2} test case.}
...
\end{tabular*}
\caption[Sample case {\bf water\_evaporation\_3}]{Output of the {\bf water\_evaporation\_3} test case.}
...
This case tests the conservation of energy between drops, surfaces, and  the gas.  A 1 m$^3$ box is filled with 1 kg of gas ($c_p$ = 1~kJ/(kg-K) and W = 28~g/mol) at an initial temperature of 100~$^\circ$C.  The box is filled with 0.1~kg of droplets ($c_p$ = 2~kJ/(kg-K)) that evaporate to the gas.  The droplets have an initial temperature of 20~$^\circ$C, a freezing point of 0~$^\circ$C, a boiling point of 100~$^\circ$C,  and a median diameter of 0.2~mm. All walls except the floor are adiabatic.  The floor is thermally thin (e.g., high conductivity compared to its thickness) has a total mass of 1 kg, a specific heat of 1~kJ/(kg-K), and an initial temperature of 250~$^\circ$C.  At steady-state all the drops will have evaporated, the wall and the gas will be at 172.0~$^\circ$C and the pressure rise will be 34,600 Pa (see Fig.~\ref{water_evaporation_6_plots}).
...
\end{tabular*}
\caption[Sample case {\bf water\_evaporation\_6}]{Output of {\bf water\_evaporation} case 6.}
...
introduced via a nozzle in the middle of the channel at a rate of 1~L/min, starting at 60~s. The water temperature is 20~$^\circ$C, and the spray is directed at the walls with an initial velocity of 5~m/s.
The water completely evaporates before it drips down to the bottom of the channel.
Figure~\ref{water_evaporation_7_plot} displays the energy balance for this case. The heat release rate, \textct{HRR}, of the fire is expected to be 320~kW. The rate at which the water droplets extract energy from the system, \textct{Q\_PART}, is expected to be
\be - \left( 4.184 \; \hbox{kJ/kg/K} \times 80 \; \hbox{K} + 2260 \; \hbox{kJ/kg} \right) \times 1/60 \; \hbox{kg/s} = -43.2 \; \hbox{kW} \ee
...
\end{center}
\caption[Sample case {\bf water\_evaporation\_7}]{Output of {\bf water\_evaporation} case 7.}
...
\end{tabular*}
\caption[The {\bf flow\_rate\_2} case]{Output of the {\bf flow\_rate\_2} test case.}
...
Two cases are set up to test the turbulent dispersion of tracer particles.  Tracer particles are injected into a channel with a steady laminar flow with mean velocity 1 m/s.  In the first case ({\bf random\_walk\_1}), the diffusivity is set to $D=0.0834$~m$^2$/s (this value results from setting the dynamic viscosity to $\mu = 0.1$~kg/(m$\cdot$s) with an ambient density of $\rho=1.199$~kg/m$^3$ and a Schmidt number of unity).  In the second case ({\bf random\_walk\_2}), the diffusivity is set to $D=0.00834$ m$^2$/s.  The results are compared with Einstein's classic result for Brownian motion \cite{Gardiner:2004}:
\begin{equation}
\label{eqn_number_density}
f(z,t) = \frac{n}{4\pi D} \frac{\exp \left( -z^2/4Dt \right)}{\sqrt{t}}
\end{equation}
where $f(z,t)$ is the particle number density at lateral distance $z$ from the source at time $t$, $n$ is the total number of particles in the system.  A phase doppler particle analyzer (PDPA) is set up to measure the number concentration at the outlet of the channel as a function of height, $z$.  The outlet is 15 m downstream of the point source of tracer particles.  With a mean velocity of 1 m/s, the time is taken as $t = 15$ s.  The FDS results are compared with Eq.~(\ref{eqn_number_density}) in Fig.~\ref{random_walk}.
...
\section{Duct Flow Losses (\texorpdfstring{\bf HVAC\_flow\_loss}{HVAC\_flow\_loss})}
...
where $K$ is the loss coefficient for the duct, $\rho$ is the gas density, and $u$ is the mean velocity. In this example, a duct is split at a tee joint into two ducts. All ducts have the same cross sectional area.  The primary duct is assigned a constant flow rate of 0.3~m$^3$/s.  The other two ducts are assigned loss coefficients of 4 and 16 and outlets at the same elevation.  From Eq.~(\ref{HVAC_steady_state}), it can be shown that since the two branch ducts have the same pressure at their outlets, the steady-state velocities in the three ducts should be 3~m/s, 2~m/s, and 1~m/s.

\begin{figure}[ht]
   \begin{center}
      \includegraphics[width=3.2in]{SCRIPT_FIGURES/HVAC_flow_loss}
     \caption[The {\bf HVAC\_flow\_loss} test case]{\label{fig_HVAC_loss} Duct velocities in three ducts with different loss coefficients.}
   \end{center}
\end{figure}

\clearpage

\section{Node Flow Losses (\texorpdfstring{\bf HVAC\_tee\_loss\_1}{HVAC\_tee\_loss\_1} and \texorpdfstring{\bf HVAC\_tee\_loss\_2}{HVAC\_tee\_loss\_2})}

These two cases use the same geometry as in Section~\ref{HVAC_flow_loss}. However, instead of losses assigned to the ducts, the losses are assigned to the duct nodes.  In the first case, {\bf HVAC\_tee\_loss\_1}, the flow goes into the primary leg of the tee. The secondary branches are assigned loss coefficients of 15 and 3 and the discharge nodes are assigned losses of 1.  This gives total losses of 16 and 4. In the second case, {\bf HVAC\_tee\_loss\_2}, the flow goes out the primary leg of the tee. Duct directions are reversed so the sign on velocity remains the same as in the first case.  The same flow losses are assigned.  It is expected that the duct velocities in both cases should be 3~m/s, 2~m/s, and 1~m/s.
...
\begin{figure}[ht]
   ...
   \end{tabular*}
   \caption[The {\bf HVAC\_tee\_loss} test cases]{Duct velocities with varied node loss. {\bf HVAC\_tee\_loss\_1} (left) and {\bf HVAC\_tee\_loss\_2} (right).}
   \label{fig_HVAC_node}
\end{figure}
...
\section{HVAC Mass Conservation (\texorpdfstring{\bf HVAC\_mass\_conservation}{HVAC\_mass\_conservation})}

A 1~m by 1~m by 1~m compartment is initialized with the lower half of the compartment being filled with a species called \textct{SPEC1} having the same molecular weight as air, see Figure~\ref{fig_HVAC_mass}.   An HVAC system of two ducts is created with one duct taking suction from the lower half (red vent) and discharging into the upper half (blue vent), and the second duct taking suction from the upper half (green vent) and discharging into the lower half (yellow vent).  The two ducts are assigned the same volume flow rate.  It is expected that the total mass in the compartment will be conserved, that the mass of the second species will be conserved (at 50 \% of the total mass), and that at the start of the computation a slice file of species will show pure ambient being discharged in the lower half and pure \textct{SPEC1} being discharged in the upper half.
...
\begin{figure}[ht]
   ...
   \end{tabular*}
   \caption[The {\bf HVAC\_mass\_conservation} case]{SPEC1 mass fraction at 1 s (left) and SPEC1 and total mass (right).}
   \label{fig_HVAC_mass}
\end{figure}
...
\section{HVAC Energy Conservation and Pressure (\texorpdfstring{\bf HVAC\_energy\_pressure}{HVAC\_energy\_pressure})}
...
\end{tabular*}
\caption[The {\bf HVAC\_energy\_pressure} case]{Clockwise from top left: Temperature at 1 s, duct velocities, total enthalpy in the domain, and duct node pressures for the top duct.}
\label{fig_HVAC_e_p}
\end{figure}

\clearpage

\section{HVAC Dampers (\texorpdfstring{\bf HVAC\_damper}{HVAC\_damper})}
...
\section{HVAC Fans (\texorpdfstring{\bf fan\_test}{fan\_test})}

Two compartments share a common wall. Both compartments are considered as
separate ``pressure zones.'' Two, 0.04~m$^2$ HVAC ducts are defined.  One has a quadratic fan with a maximum volumetric flow of 0.16~m$^3$/s and a stall pressure of 10~Pa.
...
$\dot{V}_{\hbox{\footnotesize max}}=0.16$~m$^3$/s is the maximum output of the fan from
...
\be  \dot{V}_{\hbox{\footnotesize fan}}=A_{\hbox{\footnotesize duct}} \sqrt{\frac{2 \Delta p}{\rho K}} \ee
Setting the above equation equal to Eq.~(\ref{fan_curve}), the pressure drop can be determined:
\be  \sqrt{\frac {2 |p_2-p_1|}{ (1.2 \; \hbox{kg/m$^3$}) 10}} (0.04 \; \hbox{m}^2) =
   (0.16 \; \hbox{m$^3$/s})  \;
...
\subsection{Single Zone Leakage (\texorpdfstring{\bf leak\_test}{leak\_test})}
...
\begin{figure}[ht]
   ...
   \end{tabular*}
   \caption[The {\bf leak\_test\_2} test case]{Vector plot of pressure at 10 s with geometry clipped to show compartment interiors (left) and a plot of zone pressures (right).}
...
\section{ASHRAE Sample Problem 7\\(\texorpdfstring{{\bf ashrae\_7\_fixed\_flow}}{ashrae\_7\_fixed\_flow}, \texorpdfstring{{\bf ashrae\_7\_quadratic}}{ashrae\_7\_quadratic}, and \texorpdfstring{{\bf ashrae\_7\_table}}{ashrae\_7\_table})}
...
\caption[Results of {\bf ashrae\_7} test cases]{Schematic of ASHRAE Sample Problem 7, and tabulated pressure drops for a fixed flow fan curve, quadratic fan curve, and a table defined fan curve.}
\label{ASHRAE7}
\end{figure}
...
\section{Cut-cell Algorithm (\texorpdfstring{\bf tri\_cube\_cut\_cell\_test}{tri\_cube\_cut\_cell\_test})}
...
\IfFileExists{SCRIPT_FIGURES/verification_statistics.tex}{\input{SCRIPT_FIGURES/verification_statistics.tex}}{\typeout{Error: Missing file SCRIPT_FIGURES/verification_statistics.tex}}
...
The use of computer software for safety purposes is sometimes regulated under a software quality assurance (SQA) program by the Authority Having Jurisdiction.  For example, in the United States both the Department of Energy (DOE) via DOE G 414.1-4~\cite{DOE_G_414} and the Nuclear Regulatory Commission (NRC) via NUREG-1737~\cite{NUREG_1737} impose specific requirements on software used for certain safety purposes.  In addition to imposing general requirements on configuration management and verification and validation, SQA programs often impose an acceptance testing requirement.  For simulation software this involves test program in which the software is used to execute a number of sample cases with results compared to accepted values.  The purpose of this testing is to ensure that the software installation was successful and that no corruption of the software executable or libraries has occurred.

The acceptance test procedure is as follows:

\begin{enumerate}
\item Obtain and installed the version of FDS you will be running.  Note that this procedure will need to be repeated for each version of the FDS executable being used (i.e. if you will be running both serial and mpi versions, then the procedure will be done twice).
\item Either checkout the entire FDS repository by following the link from the FDS-SMV webpage or go to the repository and download the files shown in Table~\ref{tbl:installation_test}.  The folder identified is a subfolder of the Verification folder.
\item Execute each of the FDS input files listed in Table~\ref{tbl:installation_test}.
...
\item Verify that the expected results match the current results using the criteria shown in Appendix~\ref{info:verification_statistics}.
\end{enumerate}
...
\hline
Folder  & Input File & Output File & Expected Results File \\ \hline
