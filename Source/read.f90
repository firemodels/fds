...
DO N=1,N_SPECIES - N_COPY_PRIMITIVE
   ...
   IF (H_F_IN > -1.E20_EB .OR. SS%REFERENCE_ENTHALPY > -1.E20_EB .OR. SS%LISTED) SS%EXPLICIT_H_F=.TRUE.
   IF (H_F_IN > -1.E20_EB) SS%H_F = H_F_IN
   IF (SS%REFERENCE_ENTHALPY > -1.E20_EB) SS%H_F = -1.E33_EB
...
IF (N_TRACKED_SPECIES > N_PREDEFINED_SMIX) THEN
   ...
   DO WHILE (N<=N_TRACKED_SPECIES-N_COPY)
      ...
      IF (N_BINS>0) THEN
         N_AGGLOMERATION_SPECIES = N_AGGLOMERATION_SPECIES + 1
         AGGLOMERATION_SMIX_INDEX(N_AGGLOMERATION_SPECIES) = (N_DEFINE(N))
         SPECIES_MIXTURE((N_DEFINE(N)))%AGGLOMERATION_INDEX=N_AGGLOMERATION_SPECIES
         SPEC_ID(1) = ID
         DO NNN=1,N_PARTICLE_BINS(N_AGGLOMERATION_SPECIES)
            ...
            CALL DEFINE_MIXTURE(N_DEFINE(N))
            N = N + 1
         ENDDO
      ELSE
         IF (TRIM(ID)=='WATER VAPOR') H2O_SMIX_INDEX = (N_DEFINE(N))
         CALL DEFINE_MIXTURE(N_DEFINE(N))
         IF (SPECIES_MIXTURE((N_DEFINE(N)))%SINGLE_SPEC_INDEX > 0) THEN
            IF (SPECIES(SPECIES_MIXTURE((N_DEFINE(N)))%SINGLE_SPEC_INDEX)%CONDENSABLE) THEN
               SPECIES_MIXTURE(N)%CONDENSATION_SMIX_INDEX = (N_DEFINE(N+1))
               SPECIES_MIXTURE((N_DEFINE(N)))%DEPOSITING=.FALSE.
               SPEC_ID(1) = SPECIES_MIXTURE(N)%ID
               MASS_FRACTION(1)=1._EB
               N = N + 1
               WRITE(ID,'(A,A)') TRIM(ID),'_COND'
               CALL DEFINE_MIXTURE(N_DEFINE(N))
               SPECIES_MIXTURE((N_DEFINE(N)))%DEPOSITING=.TRUE.
               SPECIES_MIXTURE((N_DEFINE(N)))%EVAPORATION_SMIX_INDEX = (N_DEFINE(N-1))
               SPECIES_MIXTURE((N_DEFINE(N)))%SINGLE_SPEC_INDEX=SPECIES_MIXTURE(N-1)%SINGLE_SPEC_INDEX
...
IF (SM%H_F > -1.E23_EB .OR. SM%REFERENCE_ENTHALPY > -1.E23_EB .OR. ALL_H_F) SM%EXPLICIT_H_F = .TRUE.
...
DO N=1,N_SPECIES
   SS => SPECIES(N)
   WRITE(*,*) 'PROC SMIX SS ',TRIM(SS%ID),SS%SPECIFIC_HEAT,SS%RAMP_CP_INDEX,SS%LISTED,SS%H_F
...
DO N=1,N_TRACKED_SPECIES
   ...
   IF(SM%RAMP_CP_INDEX > 0) THEN
      ...
      ENDIF
   ENDIF
   WRITE(*,*) 'PROC SMIX SM',TRIM(SM%ID),SM%H_F,SM%MW
END DO
...
DO N=1,N_TRACKED_SPECIES
   SM=>SPECIES_MIXTURE(N)
   IF (SM%H_F <=-1.E20_EB) THEN
      WRITE(*,*) TRIM(SM%ID),CPBAR_Z(0,N),CPBAR_Z(INT(H_F_REFERENCE_TEMPERATURE),N)*REAL(INT(H_F_REFERENCE_TEMPERATURE),EB),&
         CPBAR_Z(INT(H_F_REFERENCE_TEMPERATURE+1),N)*REAL(INT(H_F_REFERENCE_TEMPERATURE+1),EB)
...
SPEC_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   I_MELT = MAX(0,INT(SS%TMP_MELT) - 1)
   I_BOIL = MIN(I_MAX_TEMP,INT(SS%TMP_V) + 1)
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   IF (RN%SIMPLE_CHEMISTRY) THEN
      ...
      LISTED_FUEL = .FALSE.
      IF (SMF%EXPLICIT_H_F) LISTED_FUEL = .TRUE.
      DO NS=1,N_SPECIES
   ...
   DO NS = 1,N_TRACKED_SPECIES
      IF (RN%NU(NS) /= 0._EB) THEN
         IF (.NOT. SPECIES_MIXTURE(NS)%EXPLICIT_H_F) HF_COUNT = HF_COUNT +1
      ENDIF
   ENDDO
   ...
   ELSE SIMPLE_CHEMISTRY_IF ! Simple chemistry reaction
      ...
      ELSE HOC_IF ! Heat of combustion not specified, use EPUMO2 or H_F is fuel is listed
         LISTED_FUEL_IF: IF (.NOT. LISTED_FUEL ) THEN
            ...
            WRITE(*,*) RN2%HOC_COMPLETE,RN2%EPUMO2,RN%NU_O2,SPECIES(O2_INDEX)%MW , SMF%MW
            IF (RN%N_SIMPLE_CHEMISTRY_REACTIONS==1) THEN
               RN%HEAT_OF_COMBUSTION = RN2%HOC_COMPLETE
               SMF%H_F = RN%HEAT_OF_COMBUSTION - RN%S*SM%H_F + (1._EB+RN%S)*SPECIES_MIXTURE(RN%PROD_SMIX_INDEX)%H_F
               IF (SMF%SINGLE_SPEC_INDEX>0) SPECIES(SMF%SINGLE_SPEC_INDEX)%H_F = SMF%H_F
         ...
         ELSE LISTED_FUEL_IF ! Listed Fuel
            WRITE(*,*) 'B'
            RN%HEAT_OF_COMBUSTION  = SMF%H_F+RN%S*SPECIES_MIXTURE(1)%H_F-(1._EB+RN%S)*SPECIES_MIXTURE(RN%PROD_SMIX_INDEX)%H_F
            WRITE(*,*) RN%HEAT_OF_COMBUSTION,SMF%H_F,SPECIES_MIXTURE(1)%H_F,SPECIES_MIXTURE(RN%PROD_SMIX_INDEX)%H_F,RN%S
            SM => SPECIES_MIXTURE(1)
            RN%EPUMO2 = RN%HEAT_OF_COMBUSTION*SMF%MW*RN%NU(RN%FUEL_SMIX_INDEX)/(RN%NU(1)*SM%MW*SM%MASS_FRACTION(O2_INDEX))
            IF (RN%N_SIMPLE_CHEMISTRY_REACTIONS==2) THEN
               RN2%HEAT_OF_COMBUSTION=SMF2%H_F+RN2%S*SPECIES_MIXTURE(1)%H_F-(1._EB+RN2%S)*SPECIES_MIXTURE(RN2%PROD_SMIX_INDEX)%H_F
               SM => SPECIES_MIXTURE(1)
               RN2%EPUMO2 = RN2%HEAT_OF_COMBUSTION*SMF2%MW*RN2%NU(RN2%FUEL_SMIX_INDEX)/(RN2%NU(1)*SM%MW*SM%MASS_FRACTION(O2_INDEX))
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            CONE_HEAT_FLUX,REFERENCE_HEAT_FLUX,REFERENCE_HEAT_FLUX_TIME_INTERVAL,&
...
NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BURN_AWAY,BURN_DURATION,&
                ...
                CONE_HEAT_FLUX,VOLUME_FLUX ! Backwards compatability??
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (CONE_HEAT_FLUX>0._EB) REFERENCE_HEAT_FLUX = CONE_HEAT_FLUX  ! Backward compatibility
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
   ...
   !***** figure this out for multiple fuels
