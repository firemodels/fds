...
IF (HVAC_SOLVE) THEN
   ...
   DO N=1,N_DUCTS
      IF (DUCT(N)%LEAKAGE) CYCLE
      WRITE(LU_SMV,'(1X,3I0,A,A,A,A)') N,DUCT(N)%NODE_INDEX,' % ',TRIM(DUCT(N)%ID),' % ','null' !TRIM(DUCT(N)%NETWORK_ID)
      ...
      ELSE
         HVAC_LABEL = '-'
      ENDIF
      WRITE(LU_SMV,'(1X,A,I0,A,A)') 'MT_CELLS ',MAX(1,DUCT(N)%N_CELLS),' % ',TRIM(HVAC_LABEL)
     ! IF (ALLOCATED(DUCT(N)%HT_INDEX) THEN
      ...
      !ENDIF
      WRITE(LU_SMV,'(1X,A,I0)') 'WAYPOINTS ',DUCT(N)%N_WAYPOINTS
