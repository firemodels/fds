...
USE MATH_FUNCTIONS, ONLY: GET_RAMP_INDEX,NORMAL,SPECIAL_RAMP_FUNCTIONS
...
IF (SIGMA_THETA>0._EB) THEN
   CALL SPECIAL_RAMP_FUNCTIONS(SIGMA_THETA,TAU_THETA,0._EB,'RANDOM')
...
USE MATH_FUNCTIONS, ONLY: SPECIAL_RAMP_FUNCTIONS
REAL(EB) :: X,Z,T,F,TM,PERIOD,T_OFFSET
INTEGER  :: I,II,NN,N,NUMBER_INTERPOLATION_POINTS,N_RES_RAMP
CHARACTER(LABEL_LENGTH) :: DEVC_ID,CTRL_ID,FUNCTION_TYPE
TYPE(RAMPS_TYPE), POINTER :: RP
TYPE(RESERVED_RAMPS_TYPE), POINTER :: RRP
NAMELIST /RAMP/ CTRL_ID,DEVC_ID,F,FUNCTION_TYPE,FYI,ID,NUMBER_INTERPOLATION_POINTS,PERIOD,T,T_OFFSET,X,Z
...
COUNT_RAMP_POINTS: DO N=1,N_RAMP
   ...
   ! A few default values related to special time ramp functions
   ...
   ELSE
      ...
      SEARCH_LOOP: DO
         ...
         RP%PERIOD = PERIOD
         RP%T_OFFSET = T_OFFSET
         RP%FUNCTION_TYPE = FUNCTION_TYPE
   ...
   IF (RP%NUMBER_DATA_POINTS==0) THEN
      ...
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ELSEIF (RP%NUMBER_DATA_POINTS==1 .AND. RP%PERIOD<0._EB) THEN
      WRITE(MESSAGE,'(A,A,A)') 'ERROR: RAMP ',TRIM(RAMP_ID(N)), ' has only one point'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
...
READ_RAMP_LOOP: DO N=1,N_RAMP
   ...
   ! Special case where the user is asking for a special time function with a given period. This function is computed and then
   ! added to the RESERVED_RAMP list.

   IF (RP%PERIOD>0._EB) THEN
      CALL SPECIAL_RAMP_FUNCTIONS(1._EB,RP%PERIOD,RP%T_OFFSET,RP%FUNCTION_TYPE)
      RP%RESERVED_RAMP_INDEX = N_RESERVED_RAMPS
      RP%NUMBER_DATA_POINTS = RESERVED_RAMPS(N_RESERVED_RAMPS)%NUMBER_DATA_POINTS
   ENDIF
