...
if [[ `grep -E "Undefined control sequence|Error:|Fatal error|! LaTeX Error:|Paragraph ended before|Missing \\\$ inserted|Misplaced" -I FDS_Validation_Guide.err | grep -v "xpdf supports version 1.5"` == "" ]]
   then
      ...
      grep -E "Undefined control sequence|Error:|Fatal error|! LaTeX Error:|Paragraph ended before|Missing \\\$ inserted|Misplaced" -I FDS_Validation_Guide.err | grep -v "xpdf supports version 1.5"
      clean_build=0
fi
