...
IF (DEPOSITION .AND. .NOT.INITIALIZATION_PHASE .AND.CORRECTOR .AND. .NOT.SOLID_PHASE_ONLY) CALL DEPOSITION_BC(DT)
...
CFACE_LOOP: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   B1 => BOUNDARY_PROP1(CFA%B1_INDEX)
   BC => BOUNDARY_COORD(CFA%BC_INDEX)
   CALL CALCULATE_RHO_D_F(B1,BC,CFACE_INDEX=ICF)
ENDDO CFACE_LOOP
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   IF (PREDICTOR) CYCLE PARTICLE_LOOP  ! Only do basic boundary conditions during the PREDICTOR stage of time step.
   CALL DEPOSIT_PARTICLE_MASS(NM,IP,LP,LPC,CALL_HT_1D)  ! Add the particle off-gas to the gas phase mesh
ENDDO PARTICLE_LOOP
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   WC => WALL(IW)
   IF (WC%BOUNDARY_TYPE==NULL_BOUNDARY .OR. WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) CYCLE WALL_CELL_LOOP
   B1 => BOUNDARY_PROP1(WC%B1_INDEX)
   BC => BOUNDARY_COORD(WC%BC_INDEX)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SF=>SURFACE(WC%SURF_INDEX)
   IF (ABS(SF%VEL)>TWO_EPSILON_EB .OR. ANY(ABS(SF%MASS_FLUX)>TWO_EPSILON_EB) .OR. ABS(SF%VOLUME_FLOW)>TWO_EPSILON_EB) &
