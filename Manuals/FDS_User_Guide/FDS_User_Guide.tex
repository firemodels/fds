...
By default FDS uses the dynamic procedure \cite{Germano:1991} to compute the Smagorinsky constant in the turbulent viscosity, which is modeled as
\be \mu_{\hbox{\tiny LES}} = \rho \, (C_s\, \Delta)^2 \,
   \left(2 \; \overline{\bS}_{ij} : \overline{\bS}_{ij} - \frac{2}{3} (\nabla \cdot \overline{\bu})^2 \right)^\ha \ee
where $C_s$ is a function of the local flow conditions and $\Delta$ is a length scale on the order of the size of a grid cell.  FDS takes the local maximum cell dimension as the filter width, $\Delta = \max(\delta x, \delta y, \delta z)$. The bar above the various quantities denotes that these are the resolved, or {\em filtered}, values, meaning that they are computed on a numerical grid. The other diffusive parameters, the thermal conductivity and material diffusivity, are related to the turbulent viscosity by
...
The number of iterations are written out to the file {\bf CHID.out}. If you use a value that is too small, the CPU time required might be prohibitive.
...
stability of the solid phase solution may be improved by making
...
the yields of CO and soot, and the amount of hydrogen in the soot, $\mathrm{H_{frac}}$.
...
\item[{\ct SOOT\_H\_FRACTION}] The fraction of the atoms in the soot that are hydrogen.  Note that this input only applies to simple chemistry {\ct REAC} inputs.
...
\end{verbatim}
\normalsize
\noindent Here, the {\ct FORMULA} is known, but the product yields are not. Also, the user prefers to specify the {\ct HEAT\_OF\_COMBUSTION} rather than have it calculated. See
...
\be \dm_f''' = -\rho \min \left( Y_\F , \frac{Y_\OTWO}{s}, \beta \frac{Y_{\hbox{\tiny P}}}{1+s} \right) \; \left( 1 - e^{-\dt/\tau} \right)   \quad ; \quad
   s=\frac{W_\F}{\nu_{\OTWO} W_{\OTWO} }  \ee
Here, $\tau$ is a mixing time scale and $\beta$ is an empirical parameter equal to 1. You can change the value of $\beta$ by specifying {\ct BETA\_EDC}. Its default value
...
\noindent But, you could also explicitly list the individual gas species (and track them too in the calculation):
...
\subsection{Reaction Rate Limiters}
...
\begin{description}
\item[{\ct CHECK\_ATOM\_BALANCE}] If chemical formulas are provide for all {\ct SPEC} that participate in a reaction, then FDS will
check the stoichiometry to ensure that atoms are conserved.  Setting this flag to {\ct .FALSE.} will bypass this check.  (default is {\ct .TRUE.})
\item{\ct REAC\_ATOM\_ERROR} This is the error tolerance in atoms for the reaction stoichiometry check.  (default is 1.E-5)
\item{\ct REAC\_MASS\_ERROR} This is the relative error tolerance computed as (mass of products - mass of reactants)/(mass of products)
for the reaction stoichiometry mass balance check.  (default is 1.E-4)
\end{description}
...
{\ct REAL\_REFRACTIVE\_INDEX} and {\ct COMPLEX\_REFRACTIVE\_INDEX}. Alternatively, wavelength dependent values of these two quantities can be tabulated in a {\ct TABLE} and called using the {\ct RADIATIVE\_PROPERTY\_TABLE}. More details can be found in Section~\ref{radiative_part_props}.

Other parameters affecting the computations of particle-radiation interaction are: {\ct RADTMP} is the assumed radiative source temperature.
...
\be
\bar{x}^n=\bar{x}^{n-1} {\ct SMOOTHING\_FACTOR} + x (1-{\ct SMOOTHING\_FACTOR})
\ee
...
floor yields a total of 90 kg (Figure \ref{bucket_test_fig}). In some cases an in balance in mass of water may occur. Some droplets evaporate, and some droplets
...
\caption[Output of {\ct bucket\_test} test case.]{Output of {\ct bucket\_test} test cases.}
...
\hline \hline
\endhead
{\ct AIRCOIL\_ID}               & Character         & Name of an aircoil in a duct    &               &  {\ct 'null'}         \\ \hline
{\ct AMBIENT}                   & Logical           & Node is connected to the ambient                  &               &  {\ct .FALSE.}        \\ \hline
{\ct AREA}                      & Real              & Area of a duct                                    & m$^2$         &                       \\ \hline
{\ct CLEAN\_LOSS}               & Real              & Flow loss for a clean filter                                    &               &                       \\ \hline
{\ct COOLANT\_CP}               & Real              & Specific heat for the working fluid of an aircoil   & kJ/kg/K       &                       \\ \hline
{\ct COOLANT\_MDOT}             & Real              & Flow rate for the working fluid of an aircoil   & kg/s          &                       \\ \hline
{\ct COOLANT\_TEMPERATURE}      & Real              &Inlet temperature for the working fluid of an aircoil   & $^\circ$C             &                       \\ \hline
...
{\ct EFFICIENCY}                & Real Array        & Efficiency of a filter or aircoil                                    &               & 1.                     \\ \hline
{\ct FAN\_ID}                   & Character         & Name of a fan in a duct                           &               &  {\ct 'null'}         \\ \hline
{\ct FILTER\_ID}                & Character         & Name of a fan in a node                           &               &  {\ct 'null'}         \\ \hline
{\ct FIXED\_Q}                  & Real              & Fixed heat exchange (positive is heat addition) for an aircoil        & kW            &                       \\ \hline
{\ct ID}                        & Character         & Name of component                                 &               &                       \\ \hline
{\ct LENGTH}                    & Real              & Length of a duct                                  &  m            &                       \\ \hline
{\ct LOADING}                   & Real Array        & The initial species loading of a filter         & kg            & 0                     \\ \hline
{\ct LOADING\_MULTIPLIER}       & Real Array        & Filter flow loss multiplier for species         & kg            & 0                     \\ \hline
{\ct LOSS}                      & Real Array        & Flow loss coefficients for a duct, node, or filter   &               &                       \\ \hline
...
{\ct RAMP\_ID}                  & Character         & Name of {\ct RAMP} containing a fan or volume flow curve &               &  {\ct 'null'}         \\ \hline
{\ct REVERSE}                   & Logical           & Indicates fan or {\ct VOLUME\_FLOW} direction is from second node to first node &               &  {\ct 'null'}         \\ \hline
{\ct ROUGHNESS}                 & Real              & Absolute roughness of a duct                      &  m            &                       \\ \hline
{\ct SPEC\_ID}                  & Character         & List of tracked species           &               &                       \\ \hline
...
% Undocumented:  SHRINK, REGRID_FACTOR
