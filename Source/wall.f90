...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (LES .AND. .NOT. RESEARCH_MODE) THEN
      DO N=1,N_TRACKED_SPECIES
         ...
         JJG = WC%ONE_D%JJG
         KKG = WC%ONE_D%KKG
         WC%ONE_D%RHO_D_F(N) = MU(IIG,JJG,KKG)*RSC*WC%ONE_D%RHO_F/RHOP(IIG,JJG,KKG)
      ENDDO
   ELSE
      DO N=1,N_TRACKED_SPECIES
         ITMP = MIN(4999,NINT(WC%ONE_D%TMP_F))
         WC%ONE_D%RHO_D_F(N) = WC%ONE_D%RHO_F*D_Z(ITMP,N)
      ENDDO
   ENDIF
ENDDO WALL_LOOP
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER  ! Only for WALL cells
      ...
      ONE_D%QCONF = 2._EB*WC%KW*(ONE_D%TMP_G-ONE_D%TMP_F)*ONE_D%RDN
   ...
   CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER  ! Only for EXTERNAL_WALL_CELLs
      ...
      SINGLE_SPEC_IF: IF (N_TOTAL_SCALARS > 1) THEN
         ...
         ONE_D%TMP_F = PBAR_P(KK,ONE_D%PRESSURE_ZONE)/(RSUM_W*WC%ONE_D%RHO_F)
      ELSE SINGLE_SPEC_IF
         WC%ONE_D%ZZ_F(1) = 1._EB
         ONE_D%TMP_F = PBAR_P(KK,ONE_D%PRESSURE_ZONE)/(RSUM0*WC%ONE_D%RHO_F)
...
REAL(EB) :: UN,DD,MFT,TSI,ZZ_GET(1:N_TRACKED_SPECIES),RSUM_F,MPUA_SUM,RHO_0,RN1,RN2,TWOMFT
INTEGER :: N,ITER
      ...
      ELSE
         DO ITER=1,3
            ...
            SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
               ONE_D%RHO_D_F(N) = ONE_D%RHO_D_F(N)*ONE_D%RHO_F/RHO_0
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (BOUNDARY_TYPE==NULL_BOUNDARY) CYCLE WALL_CELL_LOOP
   ONE_D => WC%ONE_D
   KK = ONE_D%KK
   CALL CALC_DENSITY
...
IF (BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) THEN
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (DUCTNODE(WC%NODE_INDEX)%DIR(1)*DUCT_MF(DU,1) > 0._EB) THEN
         ...
         ELSE
            IF (DUCT(DU)%LEAK_ENTHALPY) THEN
               ...
               WC%Q_LEAK = -MFT*(CP_D*NODE_TMP_EX(WC%NODE_INDEX,1)-CP_G*TMP(IIG,JJG,KKG))*WC%ONE_D%RDN
            ENDIF
         ENDIF
      ELSE
         IF (SF%THERMAL_BC_INDEX==HVAC_BOUNDARY) THEN
   ...
   IF (MFT >= 0._EB) THEN
      ...
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
      WC%ONE_D%RHO_F = PBAR_P(KK,WC%ONE_D%PRESSURE_ZONE)/(RSUM_F*WC%ONE_D%TMP_F)
...
IF (DNS .AND. PRESENT(WALL_INDEX)) THEN
   HEAT_TRANSFER_COEFFICIENT = 2._EB*WCX%KW*ONE_DX%RDN
   RETURN
ENDIF
