...
SPEC_LOOP: DO NS = 1, N_TRACKED_SPECIES
   ...
   WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC=>WALL(IW)
      P1 => BOUNDARY_PROP1(WC%P1_INDEX)
      IF (WC%BOUNDARY_TYPE /= SOLID_BOUNDARY .OR. P1%NODE_INDEX > 0) CYCLE WALL_LOOP
      P2 => BOUNDARY_PROP2(WC%P2_INDEX)
      BC => BOUNDARY_COORD(WC%BC_INDEX)
      I = BC%IIG
      J = BC%JJG
      K = BC%KKG
      IF (P1%AWM_AEROSOL(SS%AWM_INDEX) < TWO_EPSILON_EB .AND. ZZ_INTERIM(I,J,K,NS) < ZZ_MIN_GLOBAL) CYCLE WALL_LOOP
      ...
      CALL INTERPOLATE1D_UNIFORM(LBOUND(SS%H_V,1),SS%H_V,P1%TMP_F,H_V)
      CALL INTERPOLATE1D_UNIFORM(LBOUND(SS%H_V,1),SS%H_V,T_BOIL_EFF,H_V_B)
      H_V_A = 0.5_EB*(H_V+H_V_B)
      ...
      M_WALL = P1%AWM_AEROSOL(SS%AWM_INDEX) * P1%AREA
      RVC = RDX(I)*RRN(I)*RDY(J)*RDZ(K)
      ...
      IF(Y_GAS > Y_WALL) THEN
         ...
         Y_N = 0.5_EB*(Y_1+Y_2)
      ELSE
         IF (P1%AWM_AEROSOL(SS%AWM_INDEX) < TWO_EPSILON_EB) CYCLE WALL_LOOP
         Y_1 = Y_GAS
      ...
      Y_PLUS = RHO_G*MAX(0.0001_EB,P2%U_TAU)*0.5_EB/(P1%RDN*MU_AIR) ! FDS Tech Guide (8.36)
      ...
      IF (Y_GAS > Y_WALL) THEN
         M_DOT = RHO_G*MAX(0.0001_EB,P2%U_TAU)*(Y_GAS-Y_WALL)/Y_S_PLUS
         M_VAP = -MIN((Y_GAS-Y_N)*RHO_G/(RVC*(1._EB-Y_N)),M_DOT*P1%AREA*DT)
      ELSE
         M_DOT = RHO_G*MAX(0.0001_EB,P2%U_TAU)*(Y_WALL-Y_GAS)/Y_S_PLUS
         M_VAP = MIN((Y_N-Y_GAS)*RHO_G/(RVC*(1._EB-Y_N)),M_DOT*P1%AREA*DT)
      ENDIF
      ...
      IF (SM2%AWM_INDEX > 0) P1%AWM_AEROSOL(SM2%AWM_INDEX) = M_WALL2/P1%AREA
      IF (SS%AWM_INDEX > 0)  P1%AWM_AEROSOL(SS%AWM_INDEX)  = M_WALL2/P1%AREA
      P1%Q_CONDENSE = -M_VAP * (H_V+H_L_1-H_L_2)/(P1%AREA*DT)
   ENDDO WALL_LOOP
ENDDO SPEC_LOOP
