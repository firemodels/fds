...
USE PHYSICAL_FUNCTIONS, ONLY: GET_VISCOSITY,GET_CONDUCTIVITY
USE GLOBAL_CONSTANTS, ONLY: EVACUATION_ONLY,SOLID_PHASE_ONLY,SOLID_BOUNDARY,N_TRACKED_SPECIES,K_BOLTZMANN,&
...
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MU_G,Y_AEROSOL,RHOG,ZZ_GET(1:N_TRACKED_SPECIES),YDEP,K_G,TMP_FILM,ALPHA,DTMPDX,&
            TAU_PLUS,TAU_PLUS_C,U_GRAV,D_SOLID,MW_RATIO,KN,KN_FAC
INTEGER  :: IIG,JJG,KKG,IW,N,ICF
...
SMIX_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   CFACE_LOOP: DO ICF=1,N_CFACE_CELLS
      CFA=>CFACE(ICF)
      IF (CFA%BOUNDARY_TYPE/=SOLID_BOUNDARY .OR. CFA%ONE_D%UW < 0._EB) CYCLE CFACE_LOOP
...
REAL(EB) :: NVEC(3)
REAL, PARAMETER :: ZZ_MIN_DEP=1.E-14_EB
...
IF (PRESENT(CFACE_INDEX)) THEN
   CFA%V_DEP = MAX(0._EB,U_THERM+U_TURB+U_GRAV)
...
USE GLOBAL_CONSTANTS, ONLY : R0,SOLID_BOUNDARY,MW_O2,MW_H2O,MW_CO2,H2O_INDEX,O2_INDEX,CO2_INDEX,SOOT_INDEX,PREDICTOR
USE PHYSICAL_FUNCTIONS, ONLY: GET_MOLECULAR_WEIGHT, GET_SPECIFIC_GAS_CONSTANT, GET_SPECIFIC_HEAT,GET_SENSIBLE_ENTHALPY_Z
INTEGER,INTENT(IN) :: NM
REAL(EB), INTENT(IN) :: DT
REAL(EB) :: M_SOOT,MW,RHOG,TMPG,E=-211000._EB*1000._EB/R0,A=4.7E10_EB,DMDT,DM,ZZ_GET(1:N_TRACKED_SPECIES),VOL
REAL(EB) :: DZZ(1:N_TRACKED_SPECIES),H_S_N,NU_O2_MW,NU_H2O_MW,NU_CO2_MW,RSUM_LOC,CP,ZZ_O2,X_O2
INTEGER :: IW,NS,IIG,JJG,KKG
TYPE(SPECIES_MIXTURE_TYPE), POINTER :: SM=>NULL()
TYPE(SPECIES_TYPE), POINTER :: SS=>NULL()
TYPE(WALL_TYPE), POINTER :: WC=>NULL()
...
SM => SPECIES_MIXTURE(SOOT_INDEX)
SS => SPECIES(SM%SINGLE_SPEC_INDEX)

NU_O2_MW = (SS%ATOMS(1) * 0.25_EB + SS%ATOMS(12)) * MW_O2/SM%MW
NU_H2O_MW = SS%ATOMS(1) * 0.5_EB * MW_H2O / SM%MW
NU_CO2_MW = SS%ATOMS(12) * MW_CO2 / SM%MW

WALL_CELL_LOOP: DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC=>WALL(IW)
      IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY) CYCLE WALL_CELL_LOOP
      IF (WC%AWM_AEROSOL(SM%AWM_INDEX)<TWO_EPSILON_EB) CYCLE WALL_CELL_LOOP
      M_SOOT = WC%AWM_AEROSOL(SM%AWM_INDEX)*WC%ONE_D%AREA
      IIG = WC%ONE_D%IIG
      JJG = WC%ONE_D%JJG
      KKG = WC%ONE_D%KKG
      RHOG = RHO(IIG,JJG,KKG)
      TMPG = TMP(IIG,JJG,KKG)
      ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
      ZZ_O2 = ZZ_GET(O2_INDEX)
      IF (ZZ_O2 < TWO_EPSILON_EB) CYCLE
      CALL GET_MOLECULAR_WEIGHT(ZZ_GET,MW)
      X_O2 = ZZ_O2*MW/MW_O2

      DZZ = 0._EB
      DMDT = A*M_SOOT*X_O2*EXP(E/WC%ONE_D%TMP_F)
      DM = MIN(M_SOOT,DMDT*DT,ZZ_O2*RHOG*VOL/NU_O2_MW)
      VOL = DX(IIG)*RC(IIG)*DY(JJG)*DZ(KKG)

      DZZ(O2_INDEX) = -NU_O2_MW * DM / (VOL*RHOG)
      DZZ(H2O_INDEX) = NU_H2O_MW * DM / (VOL * RHOG)
      DZZ(CO2_INDEX) = NU_CO2_MW * DM / (VOL * RHOG)

      Q(IIG,JJG,KKG) = -RHOG*SUM(SPECIES_MIXTURE%H_F*DZZ)/DT

      DM = DM/WC%ONE_D%AREA

      WC%AWM_AEROSOL(SM%AWM_INDEX) = WC%AWM_AEROSOL(SM%AWM_INDEX) - DM
      ...
      DO NS=1,N_TRACKED_SPECIES
         ...
         CALL GET_SENSIBLE_ENTHALPY_Z(NS,TMPG,H_S_N)
         D_SOURCE(IIG,JJG,KKG) = D_SOURCE(IIG,JJG,KKG) + ( SM%RCON/RSUM_LOC - H_S_N/(CP*TMPG) )*DZZ(NS)/DT
         M_DOT_PPP(IIG,JJG,KKG,NS) = M_DOT_PPP(IIG,JJG,KKG,NS) + RHOG*DZZ(NS)/DT
      ENDDO
