...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (LES .AND. .NOT. RESEARCH_MODE) THEN
      DO N=1,N_TRACKED_SPECIES
         ...
         JJG = WC%ONE_D%JJG
         KKG = WC%ONE_D%KKG
         WC%RHODW(N) = MU(IIG,JJG,KKG)*RSC*WC%ONE_D%RHO_F/RHOP(IIG,JJG,KKG)
      ENDDO
   ELSE
      DO N=1,N_TRACKED_SPECIES
         ITMP = MIN(4999,NINT(WC%ONE_D%TMP_F))
         WC%RHODW(N) = WC%ONE_D%RHO_F*D_Z(ITMP,N)
      ENDDO
   ENDIF
ENDDO WALL_LOOP
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   ONE_D%ZZ_G(:) = ZZP(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG,:)
   ...
   CALL HEAT_TRANSFER_BC(WALL_INDEX=IW)
...
CFACE_LOOP: DO ICF=1,N_CFACE_CELLS
 ...
 ! ONE_D%RSUM_G  = 
 ! ONE_D%U_TANG  = 
   CALL HEAT_TRANSFER_BC(CFACE_INDEX=ICF)
   ...
   DO IP = 1, NLP
      ...
      IF (LPC%SOLID_PARTICLE) THEN
         ...
         ONE_D%U_TANG  = 0.25_EB*SQRT ( (UU(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG)+UU(ONE_D%IIG-1,ONE_D%JJG,ONE_D%KKG))**2 + &
                                        (VV(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG)+VV(ONE_D%IIG,ONE_D%JJG-1,ONE_D%KKG))**2 + &
                                        (WW(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG)+WW(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG-1))**2 )
         CALL HEAT_TRANSFER_BC(PARTICLE_INDEX=IP)
...
SUBROUTINE HEAT_TRANSFER_BC(WALL_INDEX,CFACE_INDEX,PARTICLE_INDEX)

USE MASS, ONLY: SCALAR_FACE_VALUE

INTEGER, INTENT(IN), OPTIONAL :: WALL_INDEX,CFACE_INDEX,PARTICLE_INDEX
REAL(EB) :: ARO,FDERIV,QEXTRA,QNET,RAMP_FACTOR,RHO_G_2,RSUM_W,TMP_OTHER,TSI,UN, &
...
LOGICAL :: INFLOW,SECOND_ORDER_INTERPOLATED_BOUNDARY,SOLID_OTHER
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (INFLOW_OUTFLOW) METHOD_OF_HEAT_TRANSFER  ! Only for WALL cells
      ...
      IF (INFLOW) THEN
         ...
         ENDIF
         WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES)=SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%ZZ0
      ELSE
         ONE_D%TMP_F = ONE_D%TMP_G
         WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES)=ONE_D%ZZ_G(1:N_TRACKED_SPECIES)
      ENDIF

      ! Ghost cell values

      TMP(II,JJ,KK) = ONE_D%TMP_F
      ZZP(II,JJ,KK,1:N_TRACKED_SPECIES) = WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES)
      ...
      ONE_D%QCONF = 2._EB*WC%KW*(ONE_D%TMP_G-ONE_D%TMP_F)*WC%RDN
   ...
   CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER  ! Only for EXTERNAL_WALL_CELLs
      ...
      RHO_G_2 = ONE_D%RHO_G ! first-order (default)
      ...
      SELECT CASE(IOR)
         CASE( 1)
            ...
            WC%ONE_D%RHO_F = SCALAR_FACE_VALUE(UU(II,JJ,KK),ZZZ,FLUX_LIMITER)
         CASE(-1)
            IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
               RHO_G_2 = RHOP(IIG-1,JJG,KKG)
               RHO_OTHER_2 = OM_RHOP(IIO+1,JJO,KKO)
            ...
            WC%ONE_D%RHO_F = SCALAR_FACE_VALUE(UU(II-1,JJ,KK),ZZZ,FLUX_LIMITER)
         CASE( 2)
            IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
            ...
            ENDIF
            ZZZ(1:4) = (/RHO_OTHER_2,RHO_OTHER,ONE_D%RHO_G,RHO_G_2/)
            WC%ONE_D%RHO_F = SCALAR_FACE_VALUE(VV(II,JJ,KK),ZZZ,FLUX_LIMITER)
         CASE(-2)
            IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
               RHO_G_2 = RHOP(IIG,JJG-1,KKG)
               RHO_OTHER_2 = OM_RHOP(IIO,JJO+1,KKO)
            ENDIF
            ZZZ(1:4) = (/RHO_G_2,ONE_D%RHO_G,RHO_OTHER,RHO_OTHER_2/)
            WC%ONE_D%RHO_F = SCALAR_FACE_VALUE(VV(II,JJ-1,KK),ZZZ,FLUX_LIMITER)
         CASE( 3)
            IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
            ...
            ENDIF
            ZZZ(1:4) = (/RHO_OTHER_2,RHO_OTHER,ONE_D%RHO_G,RHO_G_2/)
            WC%ONE_D%RHO_F = SCALAR_FACE_VALUE(WW(II,JJ,KK),ZZZ,FLUX_LIMITER)
         CASE(-3)
            IF (SECOND_ORDER_INTERPOLATED_BOUNDARY) THEN
               RHO_G_2 = RHOP(IIG,JJG,KKG-1)
               RHO_OTHER_2 = OM_RHOP(IIO,JJO,KKO+1)
            ENDIF
            ZZZ(1:4) = (/RHO_G_2,ONE_D%RHO_G,RHO_OTHER,RHO_OTHER_2/)
            WC%ONE_D%RHO_F = SCALAR_FACE_VALUE(WW(II,JJ,KK-1),ZZZ,FLUX_LIMITER)
      END SELECT
      ...
      SINGLE_SPEC_IF: IF (N_TOTAL_SCALARS > 1) THEN
         ...
         WC%ONE_D%ZZ_F(1:N_TOTAL_SCALARS) = MAX(0._EB,MIN(1._EB,RHO_ZZ_F(1:N_TOTAL_SCALARS)/WC%ONE_D%RHO_F))
         ZZ_GET(1:N_TRACKED_SPECIES) = WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES)
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_W)
         ONE_D%TMP_F = PBAR_P(KK,WC%PRESSURE_ZONE)/(RSUM_W*WC%ONE_D%RHO_F)
      ELSE SINGLE_SPEC_IF
         WC%ONE_D%ZZ_F(1) = 1._EB
         ONE_D%TMP_F = PBAR_P(KK,WC%PRESSURE_ZONE)/(RSUM0*WC%ONE_D%RHO_F)
...
REAL(EB) :: DT_SUB,T_LOC,RHO_S,K_S,C_S,TMP_G,TMP_F,TMP_S,RDN,HTC,K_S_M,K_S_P,AREA_ADJUST,TMP_OTHER,RAMP_FACTOR,&
...
SUBSTEP_LOOP: DO WHILE ( ABS(T_LOC-DT_BC_HT3D)>TWO_EPSILON_EB )
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=0,IBAR
            ICM = CELL_INDEX(MAX(I,1),J,K)
            ICP = CELL_INDEX(MIN(I+1,IBAR),J,K)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. .NOT.SOLID(CELL_INDEX(II,JJ,KK)) .AND. .NOT.SOLID(CELL_INDEX(IIG,JJG,KKG))) &
       ZZP(II,JJ,KK,1:N_TRACKED_SPECIES) = 2._EB*WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES) - ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)

ENDDO WALL_CELL_LOOP

CONTAINS

SUBROUTINE CALC_SPECIES_BC(WALL_INDEX)
...
REAL(EB) :: UN,DD,MFT,TSI,ZZ_GET(1:N_TRACKED_SPECIES),RSUM_F,MPUA_SUM,AREA_ADJUST,RHO_0,RN1,RN2,TWOMFT
INTEGER :: N,ITER
INTEGER, INTENT(IN), OPTIONAL :: WALL_INDEX

! Special cases for N_TRACKED_SPECIES==1

IF (N_TRACKED_SPECIES==1 .AND. PRESENT(WALL_INDEX)) THEN
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (NO_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (.NOT.SOLID(CELL_INDEX(IIG,JJG,KKG)) .AND. PRESENT(WALL_INDEX)) &
         WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES) = ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
   ...
   CASE (SPECIFIED_MASS_FRACTION) METHOD_OF_MASS_TRANSFER
      ...
      IF (ONE_D%UWS<=0._EB) THEN
         ...
         IF (PRESENT(WALL_INDEX)) WC%ONE_D%ZZ_F = ZZ_GET
      ELSE
         IF (PRESENT(WALL_INDEX)) WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES) = ZZP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)
      ENDIF
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (PRESENT(WALL_INDEX)) THEN
         ...
         RHO_0 = WC%ONE_D%RHO_F
         IF (N_TRACKED_SPECIES==1) THEN
            WC%ONE_D%RHO_F = PBAR_P(KK,WC%PRESSURE_ZONE)/(RSUM0*WC%ONE_D%TMP_F)
            WC%ONE_D%ZZ_F(1) = 1._EB
            UN = MFT/WC%ONE_D%RHO_F
         ELSE
            DO ITER=1,3
               UN = MFT/WC%ONE_D%RHO_F
               SPECIES_LOOP: DO N=1,N_TRACKED_SPECIES
                  WC%RHODW(N) = WC%RHODW(N)*WC%ONE_D%RHO_F/RHO_0
                  DD = 2._EB*WC%RHODW(N)*WC%RDN
                  DENOM = DD + UN*WC%ONE_D%RHO_F
                  IF ( ABS(DENOM) > TWO_EPSILON_EB ) THEN
                     WC%ONE_D%ZZ_F(N) = ( ONE_D%MASSFLUX(N) + DD*ONE_D%ZZ_G(N) ) / DENOM
                  ELSE
                     WC%ONE_D%ZZ_F(N) = ONE_D%ZZ_G(N)
                  ENDIF
               ENDDO SPECIES_LOOP
               CALL GET_REALIZABLE_MF(WC%ONE_D%ZZ_F)
               ZZ_GET(1:N_TRACKED_SPECIES) = WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES)
               CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
               RHO_0 = WC%ONE_D%RHO_F
               WC%ONE_D%RHO_F = PBAR_P(KK,WC%PRESSURE_ZONE)/(RSUM_F*WC%ONE_D%TMP_F)
            ENDDO
         ENDIF
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) THEN
      ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES))
      CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM_F)
      WC%ONE_D%RHO_F = PBAR_P(WC%ONE_D%KK,WC%PRESSURE_ZONE)/(RSUM_F*WC%ONE_D%TMP_F)
      IF (IW<=N_EXTERNAL_WALL_CELLS .AND. WC%BOUNDARY_TYPE/=OPEN_BOUNDARY) &
         RHOP(WC%ONE_D%II,WC%ONE_D%JJ,WC%ONE_D%KK) = 2._EB*WC%ONE_D%RHO_F - WC%ONE_D%RHO_G
   ENDIF
   ...
   IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
      ...
      ENDIF
      IF (UWP>0._EB) THEN
         WC%ONE_D%ZZ_F(1:N_TRACKED_SPECIES) = ZZP(WC%ONE_D%IIG,WC%ONE_D%JJG,WC%ONE_D%KKG,1:N_TRACKED_SPECIES)
         WC%ONE_D%RHO_F = RHOP(WC%ONE_D%IIG,WC%ONE_D%JJG,WC%ONE_D%KKG)
         WC%ONE_D%TMP_F = TMP(WC%ONE_D%IIG,WC%ONE_D%JJG,WC%ONE_D%KKG)
      ENDIF
   ENDIF
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   BOUNDARY_TYPE_SELECT: SELECT CASE(WC%BOUNDARY_TYPE)
      ...
      CASE DEFAULT
         ...
         WC%ONE_D%ZZ_F(ZETA_INDEX) = INITIAL_UNMIXED_FRACTION

      CASE(SOLID_BOUNDARY)

         IF (WC%ONE_D%UWS<=0._EB) THEN
            WC%ONE_D%ZZ_F(ZETA_INDEX) = SF%ZETA_FRONT
         ELSE
            WC%ONE_D%ZZ_F(ZETA_INDEX) = ZZP(IIG,JJG,KKG,ZETA_INDEX)  ! Use nearest gas cell value if the wall is sucking in gas
         ENDIF
      ...
      CASE(OPEN_BOUNDARY)
         ...
         IF (INFLOW) THEN
            WC%ONE_D%ZZ_F(ZETA_INDEX)=INITIAL_UNMIXED_FRACTION
         ELSE
            WC%ONE_D%ZZ_F(ZETA_INDEX)=ZZP(IIG,JJG,KKG,ZETA_INDEX)
         ENDIF
         ZZP(II,JJ,KK,ZETA_INDEX) = WC%ONE_D%ZZ_F(ZETA_INDEX) ! Ghost cell values
   ...
   IF (IW<=N_EXTERNAL_WALL_CELLS .AND. .NOT.SOLID(CELL_INDEX(II,JJ,KK)) .AND. .NOT.SOLID(CELL_INDEX(IIG,JJG,KKG))) &
       ZZP(II,JJ,KK,ZETA_INDEX) = WC%ONE_D%ZZ_F(ZETA_INDEX)
...
INTEGER, INTENT(IN), OPTIONAL :: WALL_INDEX,PARTICLE_INDEX,CFACE_INDEX
INTEGER  :: ITMP
REAL(EB) :: RE,H_NATURAL,H_FORCED,NUSSELT,FRICTION_VELOCITY,YPLUS,DN,TMP_FILM,MU_G,K_G,CP_G,ZZ_GET(1:N_TRACKED_SPECIES)
...
SELECT CASE(SFX%GEOMETRY)
   CASE (SURF_CARTESIAN)
      SELECT CASE(ABS(ONE_D%IOR))
         CASE(0:2)
            H_NATURAL = SFX%C_VERTICAL*ABS(DELTA_TMP)**ONTH
         CASE(3)
            H_NATURAL = SFX%C_HORIZONTAL*ABS(DELTA_TMP)**ONTH
         END SELECT
...
HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
   CASE DEFAULT
      ...
      RE = ONE_D%RHO_G*ONE_D%U_TANG*SFX%CONV_LENGTH/MU_G
      SELECT CASE(SFX%GEOMETRY)
         CASE (SURF_CARTESIAN)
   ...
   CASE(H_LOGLAW)
      ...
      CALL HEAT_FLUX_MODEL(H_FORCED,YPLUS,FRICTION_VELOCITY,K_G,ONE_D%RHO_G,CP_G,MU_G)
   CASE(H_ABL)
      H_NATURAL = 0._EB
      CALL GET_SPECIFIC_HEAT(ZZ_GET,CP_G,TMP_FILM)
      CALL ABL_HEAT_FLUX_MODEL(H_FORCED,FRICTION_VELOCITY,DN,SFX%ROUGHNESS,ONE_D%TMP_G,ONE_D%TMP_F,ONE_D%RHO_G,CP_G)
   CASE(H_CUSTOM)
      CALL GET_VISCOSITY(ZZ_GET,MU_G,TMP_FILM)
      CALL GET_CONDUCTIVITY(ZZ_GET,K_G,TMP_FILM)
      RE = ONE_D%RHO_G*ONE_D%U_TANG*SFX%CONV_LENGTH/MU_G
      NUSSELT = SFX%C_FORCED_CONSTANT+SFX%C_FORCED_RE*RE**SFX%C_FORCED_RE_EXP*PR_AIR**SFX%C_FORCED_PR_EXP
      H_FORCED = NUSSELT*K_G/SFX%CONV_LENGTH
