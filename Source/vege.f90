...
IF (CC_IBM) THEN
   ...
   DO ICF=1,M%N_CUTFACE_MESH
      ...
      IF (CFA%NVEC(KAXIS)>-TWO_EPSILON_EB .AND. CFA%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         ...
         Z_LS(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG) = DOT_PRODUCT(CUT_FACE(ICF)%XYZCEN(KAXIS,1:CUT_FACE(ICF)%NFACE), &
                                                                           CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))     / &
                                                                       SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))
         IF (CFA%BOUNDARY_COORD%KKG < LS_KLO_TERRAIN(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG)) &
            LS_KLO_TERRAIN(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG) = CFA%BOUNDARY_COORD%KKG
         IF (CFA%BOUNDARY_COORD%KKG > K_LS(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG)) &
            K_LS(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG) = CFA%BOUNDARY_COORD%KKG
         LS_SURF_INDEX(CFA%BOUNDARY_COORD%IIG,CFA%BOUNDARY_COORD%JJG) = CFA%SURF_INDEX
      ENDIF
   ENDDO
   DO J=1,JBAR
      DO I=1,IBAR
...
ELSE
   ...
   DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC => WALL(IW)
      IF (WC%BOUNDARY_COORD%IOR==3 .AND. WC%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
         Z_LS(WC%BOUNDARY_COORD%IIG,WC%BOUNDARY_COORD%JJG) = Z(WC%BOUNDARY_COORD%KKG-1)
         K_LS(WC%BOUNDARY_COORD%IIG,WC%BOUNDARY_COORD%JJG) = WC%BOUNDARY_COORD%KKG
         LS_SURF_INDEX(WC%BOUNDARY_COORD%IIG,WC%BOUNDARY_COORD%JJG)= WC%SURF_INDEX
      ENDIF
   ENDDO
...
DO JJG=1,JBAR
   DO IIG=1,IBAR
      ...
      ELSE IF_CFD_COUPLED  ! The wind velocity is specified by the user
         ...
         U_LS(IIG,JJG) = U0*EVALUATE_RAMP(T,I_RAMP_U0_T)
         V_LS(IIG,JJG) = V0*EVALUATE_RAMP(T,I_RAMP_V0_T)
