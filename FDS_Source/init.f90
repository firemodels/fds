...
INTEGER :: N,I,J,K,II,JJ,KK,NN,IPTS,JPTS,KPTS,N_EDGES_DIM,IW,IC,IBC,IOR,IOPZ,IERR,IB,JB,KB
INTEGER, INTENT(IN) :: NM
REAL(EB) :: MU_N,YY_GET(1:N_SPECIES)
...
DO N=1,N_PART
   PC => PARTICLE_CLASS(N)
   IF (PC%SURF_INDEX>0) M%NVWC = M%NVWC + PC%N_INITIAL
ENDDO
...
PARTICLE_LOOP: DO N=1,N_PART
   PC =>PARTICLE_CLASS(N)
   IF (PC%SURF_INDEX>0) THEN
      PC%WALL_INDEX_START = IW + 1
      DO NN=1,PC%N_INITIAL
         IW = IW + 1
         I = 1
         J = 1
         K = 1
...
CHECK_MESHES: IF (IW<=M%NEWC .AND.  .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO ITER=0,1
      ...
      IF (ABS(IOR)/=1) XIN = M%XW(IW) + (ITER*0.9_EB-0.45_EB)*(M%X(I)-M%X(I-1))
      IF (ABS(IOR)/=2) YIN = M%YW(IW) + (ITER*0.9_EB-0.45_EB)*(M%Y(J)-M%Y(J-1))
      IF (ABS(IOR)/=3) ZIN = M%ZW(IW) + (ITER*0.9_EB-0.45_EB)*(M%Z(K)-M%Z(K-1))
...
IF (I_OBST>0 .AND. SURFACE(IBCX)%VOLUME_FLUX/=0._EB) THEN
   OBX=>M%OBSTRUCTION(I_OBST)
   M%UW0(IW) = SURFACE(IBCX)%VOLUME_FLUX/OBX%FDS_AREA(ABS(IOR))
ENDIF
IF (I_OBST>0 .AND. SURFACE(IBCX)%MASS_FLUX_TOTAL/=0._EB) THEN
   OBX=>M%OBSTRUCTION(I_OBST)
...
PROCESS_VENT: IF (I_VENT>0) THEN
   ...
   IF (SURFACE(IBCX)%MASS_FLUX_TOTAL/=0._EB)   M%UW0(IW) = SURFACE(IBCX)%MASS_FLUX_TOTAL/RHOA*M%AREA_ADJUST(IW)   
   ...
   IF (VT%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
      IF (.NOT.M%SOLID(ICG)) M%BOUNDARY_TYPE(IW) = OPEN_BOUNDARY
      IF (M%PRESSURE_ZONE_WALL(IW)>0 .AND. .NOT.ALLOW_ZONE_BREAKS) THEN
