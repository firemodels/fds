...
\item[{\ct DAMPER}] is a logical parameter indicating the presence of a damper in the duct. The state of the damper is controlled by either a device or a control function (see Section~\ref{HVAC:damper}.
...
\subsubsection{HVAC Dampers}
\label{info:HVACdamper}

A damper can be placed in a duct by adding the keyword {\ct DAMPER} along with either a {\ct CTRL\_ID} or {\ct DEVC\_ID}.  When the control or device is {\ct .TRUE.} the damper will be open.  The sample below is a simple duct network of three ducts connected by a tee.  A damper is present in one of the ducts and linked to a {\ct DEVC} that will change state at 10~s.  With the damper open, the respective duct flow speeds should be 3~m/s, 1~m/s and 2~m/s.  When the damper closes, the respective duct flow speeds should be 3~m/s, 3~m/s and~0 m/s.  Results of this are shown in \ref{HVAC_damper}.

\footnotesize
\begin{verbatim}
&HVAC TYPE_ID='NODE',ID='SUPPLY 1',VENT_ID='SUPPLY 1',DUCT_ID='SUPPLY 1',LOSS=0,0/

&HVAC TYPE_ID='NODE',ID='EXHAUST 1',VENT_ID='EXHAUST 1',DUCT_ID='EXHAUST 1',LOSS=1,1/
&HVAC TYPE_ID='NODE',ID='EXHAUST 2',VENT_ID='EXHAUST 2',DUCT_ID='EXHAUST 2',LOSS=1,1/

&HVAC TYPE_ID='NODE',ID='TEE',DUCT_ID='SUPPLY 1','EXHAUST 1','EXHAUST 2',XYZ(3)=0.0,
 LOSS(1,2)=15,LOSS(1,3)=3,LOSS(2,1)=15,LOSS(2,3)=3,LOSS(3,2)=15,LOSS(3,1)=3/


&HVAC TYPE_ID='DUCT',ID='SUPPLY 1',NODE_ID='SUPPLY 1','TEE',AREA=0.01,
      VOLUME_FLOW=0.03,LENGTH=1,LOSS=0,0/
&HVAC TYPE_ID='DUCT',ID='EXHAUST 1',NODE_ID='TEE','EXHAUST 1',AREA=0.01,
      LENGTH=1.0,LOSS=0,0/
...
%{\ct PRESSURE\_ACCELERATION\_FACTOR}& Real          & Section~\ref{info:PRES}           &               & 0.                        \\ \hline
