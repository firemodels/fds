...
NAMELIST /SPEC/ AEROSOL,BACKGROUND,CONDUCTIVITY,CONDUCTIVITY_SOLID, &
                ...
                MASS_EXTINCTION_COEFFICIENT,MASS_FRACTION,MASS_FRACTION_0,MEAN_DIAMETER,MELTING_TEMPERATURE,MW,PRIMITIVE,&
                RADCAL_ID,RAMP_CP,RAMP_CP_L,RAMP_D,RAMP_G_F,RAMP_K,RAMP_MU,REFERENCE_ENTHALPY,REFERENCE_TEMPERATURE,SIGMALJ,&
                SPEC_ID,SPECIFIC_HEAT,SPECIFIC_HEAT_LIQUID,VAPORIZATION_TEMPERATURE,VISCOSITY,VOLUME_FRACTION
...
COUNT_SPEC_LOOP: DO
   ...
   IF (SPEC_ID(1)=='null' .OR. (SPEC_ID(1)/='null' .AND. PRIMITIVE)) THEN
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   IF (SPEC_ID(1)/='null' .AND. .NOT. PRIMITIVE) THEN
      N = N - 1
   ...
   ENDIF 
   PROP_ID_S = SS%ID
   IF (SPEC_ID(1)/='null') PROP_ID_S=SPEC_ID(1)
   CALL GAS_PROPS(PROP_ID_S,SS%SIG,SS%EPSK,SS%MW,SS%FORMULA,SS%LISTED,SS%ATOMS,SS%H_F,SS%RADCAL_ID)
   CALL FED_PROPS(PROP_ID_S,SS%FLD_LETHAL_DOSE,SS%FIC_CONCENTRATION)
   ...
   IF (PROP_ID_S=='SOOT' .AND. AL2O3) SS%DENSITY_SOLID = 4000.
