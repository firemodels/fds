...
INTEGER :: IOS,IZERO,N_HVAC_READ,NS,N,ND,NN,I_DUCT=0,I_DUCTNODE=0,I_FAN=0 ,I_FILTER=0!,I_AIRCOIL=0
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(MAX_DUCTS,MAX_DUCTS),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW, &
            TAU_FAN,TAU_VF,CLEAN_LOSS,LOADING(1:MAX_SPECIES),EFFICIENCY(1:MAX_SPECIES),LOADING_MULTIPLIER(1:MAX_SPECIES)
...
NAMELIST /HVAC/ AMBIENT,AREA,CLEAN_LOSS,CTRL_ID,DAMPER,DEVC_ID,DIAMETER,DUCT_ID,EFFICIENCY,FAN_ID,FILTER_ID,ID,LENGTH,&
                LOADING,LOADING_MULTIPLIER,TABLE_ID,LOSS,MAX_FLOW,MAX_PRESSURE,NODE_ID,REVERSE, &
                RAMP_ID,ROUGHNESS,TAU_FAN,TAU_VF,TYPE_ID,VENT_ID,VOLUME_FLOW,XYZ!AIRCOIL_ID,ROUND, SQUARE,TABLE_ID
...
!         ENDIF
      ...
      CASE('FILTER')         
         ...
         FILTER(I_FILTER)%MULTIPLIER =LOADING_MULTIPLIER
...
NODE_LOOP: DO NN = 1, N_DUCTNODES
   ...
   IF(TRIM(NODE_FILTER_A(NN))/='null') THEN
      DO N = 1,N_FILTERS
         IF(NODE_FILTER_A(NN)==TRIM(FILTER(N)%ID)) THEN
            DN%FILTER_INDEX = N
            EXIT
         ENDIF
         IF(N==N_FILTERS) THEN
      ...
      IF (DN%VENT_INDEX > 0 .OR. DN%AMBIENT .OR. DN%LEAKAGE) THEN
         ...
         ELSEIF (DU%FAN_INDEX > 0) THEN
            ...
            IF (DU%FAN_OPERATING .AND. FAN(DU%FAN_INDEX)%FAN_TYPE==1) THEN
               DU%FIXED=.TRUE.
               CALL UPDATE_FAN(T-DU%FAN_ON_TIME,DN%DUCT_INDEX(1))
            ENDIF            
         ENDIF
         CYCLE NODE_LOOP
      ENDIF
...
DN%FILTER_LOADING(:,3) = ABS(DU%VEL(GUESS))*DN2%RHO*DOT_PRODUCT(DN2%ZZ,FI%EFFICIENCY)
