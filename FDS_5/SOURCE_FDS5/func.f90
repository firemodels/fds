...
SELECT CASE(CODE)
   CASE(1)
      ...
      ALLOCATE(M%DROPLET(M%NLPDIM+1000))
   ...
   CASE(2)
      ...
      ALLOCATE(M2%DROPLET(M2%N_DROP_ORPHANS_DIM+1000))
...
SELECT CASE(INDEX)
   ...
   CASE(N2_INDEX)
      Y_MF = (1._EB - Z) * RN%Y_N2_INFTY + Z * RN%Y_N2_INLET + (Z_2 + Z_3) * RN%Y_F_INLET / RN%MW_FUEL * MW_N2 * RN%NU_N2
   CASE(O2_INDEX)
      Y_MF = (1._EB - Z) * RN%Y_O2_INFTY - (Z_3 * RN%NU_CO2 + &
              0.5_EB *(Z_3 * RN%NU_CO + (Z_2 + Z_3) * RN%NU_H2O)) * MW_O2 * RN%Y_F_INLET / RN%MW_FUEL
      IF (CO_PRODUCTION) Y_MF = Y_MF - 0.5_EB * Z_2 * REACTION(1)%NU_CO * MW_O2 * RN%Y_F_INLET / RN%MW_FUEL  
   CASE(CO_INDEX)
      Y_MF = Z_3 * RN%Y_F_INLET / RN%MW_FUEL * MW_CO * RN%NU_CO
      IF (CO_PRODUCTION) Y_MF = Y_MF + Z_2 * RN%Y_F_INLET / RN%MW_FUEL * MW_CO * REACTION(1)%NU_CO
   CASE(CO2_INDEX)
...
Y_MF(N2_INDEX) = (1._EB - Z) * RN%Y_N2_INFTY + Z * RN%Y_N2_INLET + (Z_2 + Z_3) * RN%Y_F_INLET / RN%MW_FUEL * MW_N2 * RN%NU_N2

Y_MF(O2_INDEX) = (1._EB - Z) * RN%Y_O2_INFTY - (Z_3 * RN%NU_CO2 + &
                 0.5_EB *(Z_3 * RN%NU_CO + (Z_2 + Z_3) * RN%NU_H2O)) * MW_O2 * RN%Y_F_INLET / RN%MW_FUEL
IF (CO_PRODUCTION) Y_MF(O2_INDEX) = Y_MF(O2_INDEX) - 0.5_EB * Z_2 * REACTION(1)%NU_CO * MW_O2 * RN%Y_F_INLET / RN%MW_FUEL  

Y_MF(CO_INDEX) = Z_3 * RN%Y_F_INLET / RN%MW_FUEL * MW_CO * RN%NU_CO
IF (CO_PRODUCTION) Y_MF(CO_INDEX) = Y_MF(CO_INDEX) + Z_2 * RN%Y_F_INLET / RN%MW_FUEL * MW_CO * REACTION(1)%NU_CO
...
SUBROUTINE GET_MASS_FRACTION(Z1,Z2,Z3,INDEX, YY_SUM, Z_F, Y_MF)
 
! Y_MF returns the mass fraction of species INDEX
 
INTEGER, INTENT(IN) :: INDEX
INTEGER :: IZ1,IZ2
REAL(EB) :: Z_1,Z_2,Z_3,YY_SUM,Y_N(N_SPECIES),F,C,Z,Z_F,Y_MF,WGT,Z1,Z2,Z3
...
selectmethod: IF (.NOT. CO_PRODUCTION) THEN
   ...
   Y_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%Y_MF(IZ1,INDEX)   + WGT*SPECIES(I_FUEL)%Y_MF(IZ2,INDEX)
   Y_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%Y_MF(IZ1,INDEX) + WGT*SPECIES(I_PROG_F)%Y_MF(IZ2,INDEX)  
   Y_MF = (1._EB - F) * Y_N(I_FUEL) + F * Y_N(I_PROG_F)    
ELSE selectmethod
   CALL GET_F_C(Z_1,Z_2,Z_3,F,C,Z_F)
   IF (INDEX==0) RETURN
   Y_N(I_FUEL)    = (1._EB-WGT)*SPECIES(I_FUEL)%Y_MF(IZ1,INDEX)    + WGT*SPECIES(I_FUEL)%Y_MF(IZ2,INDEX)
   Y_N(I_PROG_CO) = (1._EB-WGT)*SPECIES(I_PROG_CO)%Y_MF(IZ1,INDEX) + WGT*SPECIES(I_PROG_CO)%Y_MF(IZ2,INDEX)
   Y_N(I_PROG_F)  = (1._EB-WGT)*SPECIES(I_PROG_F)%Y_MF(IZ1,INDEX)  + WGT*SPECIES(I_PROG_F)%Y_MF(IZ2,INDEX)     
   Y_MF = (1._EB - C) * Y_N(I_FUEL) + C * Y_N(I_PROG_CO)    
   IF (Z > REACTION(2)%Z_F .AND. Z <REACTION(1)%Z_F) THEN
      IF (Z < Z_F) THEN
         Y_MF = Y_MF +        C * ((1._EB-WGT)*Y_CORR_FUEL(IZ1,INDEX)+WGT*Y_CORR_FUEL(IZ2,INDEX))
      ELSE
         Y_MF = Y_MF + (1._EB-C)* ((1._EB-WGT)*Y_CORR_O2(IZ1,INDEX)+WGT*Y_CORR_O2(IZ2,INDEX))
      ENDIF
   ENDIF
   Y_MF = (1._EB - F) * Y_MF + F * Y_N(I_PROG_F)
ENDIF selectmethod

Y_MF = MIN(1._EB,MAX(0._EB,Y_MF*(1._EB-YY_SUM)))
...
! Y_MF returns the mass fraction of species INDEX
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z,Z_1,Z_2,Z_3,MW_MF,Y_MF(9)
TYPE(REACTION_TYPE), POINTER :: RN
...
! GET_MOLECULAR_WEIGHT returns the molecular weight
 
INTEGER :: IZ1,IZ2
REAL(EB) :: Z_1,Z_2,Z_3,YY_SUM,F,C,Z,Z_F,MW_MF,WGT,MW_N(N_SPECIES),Z1,Z2,Z3
...
selectmethod: IF (.NOT. CO_PRODUCTION) THEN
   CALL GET_F(Z_1,Z_3,F,Z_F)
   MW_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%MW_MF(IZ1)   + WGT*SPECIES(I_FUEL)%MW_MF(IZ2)
   MW_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%MW_MF(IZ1) + WGT*SPECIES(I_PROG_F)%MW_MF(IZ2)
   MW_MF = 1./((1._EB - F) / MW_N(I_FUEL) + F / MW_N(I_PROG_F))
ELSE selectmethod
   CALL GET_F_C(Z_1,Z_2,Z_3,F,C,Z_F)
   MW_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%MW_MF(IZ1)   + WGT*SPECIES(I_FUEL)%MW_MF(IZ2)
   MW_N(I_PROG_CO) = (1._EB-WGT)*SPECIES(I_PROG_CO)%MW_MF(IZ1) + WGT*SPECIES(I_PROG_CO)%MW_MF(IZ2)
   MW_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%MW_MF(IZ1) + WGT*SPECIES(I_PROG_F)%MW_MF(IZ2)
   MW_MF = (1._EB - C) / MW_N(I_FUEL) + C / MW_N(I_PROG_CO)
   IF (Z > REACTION(2)%Z_F .AND. Z <REACTION(1)%Z_F) THEN
      IF (Z < Z_F) THEN
         MW_MF = MW_MF +        C / ((1._EB-WGT)*MW_MF_CORR_FUEL(IZ1) + WGT*MW_MF_CORR_FUEL(IZ2))
      ELSE
         MW_MF = MW_MF + (1._EB-C)/ ((1._EB-WGT)*MW_MF_CORR_O2(IZ1)   + WGT*MW_MF_CORR_O2(IZ2))
      ENDIF
   ENDIF
   MW_MF = (1._EB - F) * MW_MF + F / MW_N(I_PROG_F)
   MW_MF = 1._EB / MW_MF
ENDIF selectmethod
...
INTEGER :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z,Z_1,Z_2,Z_3,MU_MF,Y_MF(9),MW_MF
...
INTEGER :: IZ1,IZ2,ITMP
REAL(EB) :: Z_1,Z_2,Z_3,YY_SUM,F,C,Z,Z_F,MU_MF,WGT,MU_N(N_SPECIES),Z1,Z2,Z3

IF (YY_SUM >=1._EB) THEN
   MU_MF = SPECIES(I_FUEL)%MU_MF(0,ITMP)
   RETURN
ENDIF
YY_SUM = MAX(0._EB,YY_SUM)
Z_1 = MAX(0._EB,Z1)/(1._EB - YY_SUM)
...
selectmethod: IF (.NOT. CO_PRODUCTION) THEN
   CALL GET_F(Z_1,Z_3,F,Z_F)
   MU_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%MU_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%MU_MF(IZ2,ITMP)
   MU_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%MU_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%MU_MF(IZ2,ITMP)
   MU_MF = (1._EB - F) * MU_N(I_FUEL) + F * MU_N(I_PROG_F)
ELSE selectmethod
   CALL GET_F_C(Z_1,Z_2,Z_3,F,C,Z_F)
   MU_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%MU_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%MU_MF(IZ2,ITMP)
   MU_N(I_PROG_CO) = (1._EB-WGT)*SPECIES(I_PROG_CO)%MU_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_CO)%MU_MF(IZ2,ITMP)
   MU_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%MU_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%MU_MF(IZ2,ITMP)
   MU_MF = (1._EB - F) * ((1._EB - C) * MU_N(I_FUEL) + C * MU_N(I_PROG_CO)) + F * MU_N(I_PROG_F)
ENDIF selectmethod
...
INTEGER :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z,Z_1,Z_2,Z_3,D_MF,Y_MF(9)
TYPE(SPECIES_TYPE), POINTER :: SS
...
INTEGER :: IZ1,IZ2,ITMP
REAL(EB) :: Z_1,Z_2,Z_3,YY_SUM,F,C,Z,Z_F,D_MF,WGT,D_N(N_SPECIES),Z1,Z2,Z3

IF (YY_SUM >=1._EB) THEN
   D_MF = SPECIES(I_FUEL)%D_MF(0,ITMP)
   RETURN
ENDIF
YY_SUM = MAX(0._EB,YY_SUM)
Z_1 = MAX(0._EB,Z1)/(1._EB - YY_SUM)
...
selectmethod: IF (.NOT. CO_PRODUCTION) THEN
   CALL GET_F(Z_1,Z_3,F,Z_F)
   D_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%D_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%D_MF(IZ2,ITMP)
   D_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%D_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%D_MF(IZ2,ITMP)
   D_MF = (1._EB - F) * D_N(I_FUEL) + F * D_N(I_PROG_F)
ELSE selectmethod
   CALL GET_F_C(Z_1,Z_2,Z_3,F,C,Z_F)
   D_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%D_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%D_MF(IZ2,ITMP)
   D_N(I_PROG_CO) = (1._EB-WGT)*SPECIES(I_PROG_CO)%D_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_CO)%D_MF(IZ2,ITMP)
   D_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%D_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%D_MF(IZ2,ITMP)
   D_MF = (1._EB - F) * ((1._EB - C) * D_N(I_FUEL) + C * D_N(I_PROG_CO)) + F * D_N(I_PROG_F)
ENDIF selectmethod
...
INTEGER :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z,Z_1,Z_2,Z_3,CP_MF,Y_MF(9)
TYPE(SPECIES_TYPE), POINTER :: SS
...
INTEGER :: IZ1,IZ2,ITMP
REAL(EB) :: Z_1,Z_2,Z_3,YY_SUM,F,C,Z,Z_F,CP_MF,WGT,CP_N(N_SPECIES),Z1,Z2,Z3

IF (YY_SUM >=1._EB) THEN
   CP_MF = SPECIES(I_FUEL)%CP_MF(0,ITMP)
   RETURN
ENDIF
YY_SUM = MAX(0._EB,YY_SUM)
Z_1 = MAX(0._EB,Z1)/(1._EB - YY_SUM)
...
selectmethod: IF (.NOT. CO_PRODUCTION) THEN
   CALL GET_F(Z_1,Z_3,F,Z_F)
   CP_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%CP_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%CP_MF(IZ2,ITMP)
   CP_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%CP_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%CP_MF(IZ2,ITMP)
   CP_MF = (1._EB - F) * CP_N(I_FUEL) + F * CP_N(I_PROG_F)
ELSE selectmethod
   CALL GET_F_C(Z_1,Z_2,Z_3,F,C,Z_F)
   CP_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%CP_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%CP_MF(IZ2,ITMP)
   CP_N(I_PROG_CO) = (1._EB-WGT)*SPECIES(I_PROG_CO)%CP_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_CO)%CP_MF(IZ2,ITMP)
   CP_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%CP_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%CP_MF(IZ2,ITMP)
   CP_MF = (1._EB - F) * ((1._EB - C) * CP_N(I_FUEL) + C * CP_N(I_PROG_CO)) + F * CP_N(I_PROG_F)
ENDIF selectmethod
...
INTEGER :: ITMP
REAL(EB), INTENT(IN) :: Z1,Z2,Z3,YY_SUM
REAL(EB) :: Z,Z_1,Z_2,Z_3,K_MF,Y_MF(9),MW_MF
TYPE(SPECIES_TYPE), POINTER :: SS
TYPE(REACTION_TYPE), POINTER :: RN
...
INTEGER :: IZ1,IZ2,ITMP
REAL(EB) :: Z_1,Z_2,Z_3,YY_SUM,F,C,Z,Z_F,K_MF,WGT,K_N(N_SPECIES),Z1,Z2,Z3

IF (YY_SUM >=1._EB) THEN
   K_MF = SPECIES(I_FUEL)%K_MF(0,ITMP)
   RETURN
ENDIF
YY_SUM = MAX(0._EB,YY_SUM)
Z_1 = MAX(0._EB,Z1)/(1._EB - YY_SUM)
...
selectmethod: IF (.NOT. CO_PRODUCTION) THEN
   CALL GET_F(Z_1,Z_3,F,Z_F)
   K_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%K_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%K_MF(IZ2,ITMP)
   K_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%K_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%K_MF(IZ2,ITMP)
   K_MF = (1._EB - F) * K_N(I_FUEL) + F * K_N(I_PROG_F)
ELSE selectmethod
   CALL GET_F_C(Z_1,Z_2,Z_3,F,C,Z_F)
   K_N(I_FUEL)   = (1._EB-WGT)*SPECIES(I_FUEL)%K_MF(IZ1,ITMP)   + WGT*SPECIES(I_FUEL)%K_MF(IZ2,ITMP)
   K_N(I_PROG_CO) = (1._EB-WGT)*SPECIES(I_PROG_CO)%K_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_CO)%K_MF(IZ2,ITMP)
   K_N(I_PROG_F) = (1._EB-WGT)*SPECIES(I_PROG_F)%K_MF(IZ1,ITMP) + WGT*SPECIES(I_PROG_F)%K_MF(IZ2,ITMP)
   K_MF = (1._EB - F) * ((1._EB - C) * K_N(I_FUEL) + C * K_N(I_PROG_CO)) + F * K_N(I_PROG_F)
ENDIF selectmethod
