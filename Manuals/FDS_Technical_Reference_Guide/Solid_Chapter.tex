...
where $m_a$ is the particle mass, $\chi_d$ is a shape factor, $r_a$ is the particle radius,
and Cn is the Cunningham slip factor given by~\cite{Cunningham:1}

\be
\text{Cn} = 1 + 1.25 \; \text{Kn} + 0.41 \; \text{Kn} \; e^{-0.88/\text{Kn}}
\ee
...
where $C_s$ is the thermal slip coefficient (1.17), $\alpha$ is the ratio of the gas
conductivity to the particle conductivity, $C_m$ is the momentum accommodation
coefficient (1.14), and $C_t$ is the thermal accommodation coefficient (2.18).
...
where $u_{\tau}$ is the wall friction velocity computed by the wall model,
