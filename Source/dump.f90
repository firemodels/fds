...
WRITE(LU_CORE(NM)) Q_DOT_SUM(1:N_Q_DOT,NM),M_DOT_SUM(1:N_M_DOT,NM),MINT_SUM(0:N_SPECIES,NM),U_DOT_SUM(1:N_U_DOT,NM)
DO N=1,N_DEVC
   DV => DEVICE(N)
...
READ(LU_RESTART(NM)) T,ICYC,PART_CLOCK(NM),RESTART_CLOCK,SLCF_CLOCK(NM),RADF_CLOCK(NM), &
             ...
             RTE_SOURCE_CORRECTION_FACTOR,RAD_Q_SUM,KFST4_SUM,SPECIFIC_ENTHALPY_SUM(NM),&
             BC_CLOCK_HT3D,WALL_COUNTER_HT3D,MOM_CLOCK
READ(LU_RESTART(NM)) Q_DOT_SUM(1:N_Q_DOT,NM),M_DOT_SUM(1:N_M_DOT,NM),MINT_SUM(0:N_SPECIES,NM),U_DOT_SUM(1:N_U_DOT,NM)
ICYC_RESTART = ICYC
DO N=1,N_DEVC
   DV => DEVICE(N)
...
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
         ...
         ENDIF
         VC = DX(I)*RC(I)*DY(J)*DZ(K)
         U_DOT(1,NM) = U_DOT(1,NM) + VC*U(I,J,K)
      ENDDO
   ENDDO
ENDDO

U_DOT_SUM(:,NM) = U_DOT_SUM(:,NM) + DT*U_DOT(:,NM)
...
DO NM=1,NMESHES
   !U_DOT_TOTAL(:) = U_DOT_TOTAL(:) + U_DOT_SUM(:,NM)/MAX(DT,T-T_LAST_DUMP_MOM)
ENDDO

DUMMY = DT

WRITE(TCFORM,'(A,I4.4,5A)') "(",1,"(",FMT_R,",','),",FMT_R,")"
WRITE(LU_MOM,TCFORM) STIME,DUMMY
