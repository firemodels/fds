...
INTEGER :: NR,NS,NSS,ITER,TVI,RICH_ITER,TMP_ITER,TIME_ITER
INTEGER, PARAMETER :: SUB_DT1=1,SUB_DT2=2,SUB_DT4=4,TV_ITER_MIN=5,RICH_ITER_MAX=50,TIME_ITER_MAX=1000
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
...
INTEGRATION_LOOP: DO TIME_ITER = 1,TIME_ITER_MAX
   RK2_IF: IF (COMBUSTION_ODE /= EDCM_RK2) THEN ! Explicit Euler
      ZETA1 = ZETA_0*EXP(-DT_SUB/MIX_TIME(I,J,K))
      TOTAL_MIX_MASS(1) = (1-ZETA1)*CELL_MASS
      SMIX_MIX_MASS(:,1) = SMIX_MIX_MASS(:,0) + (ZETA_0-ZETA1)*CELL_MASS*ZZ_GET_0
      ...
      REACTION_LOOP1: DO NR = 1, N_REACTIONS
         RN => REACTION(NR)
         CALL COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT(NR),ZZ_0,I,J,K,DT_SUB,ITER,TMP_0)
         DZZDT = RN%NU*SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT(NR)
         IF (ANY(ZZ_0 + (DZZDT1_1+DZZDT)*DT_SUB < -TWO_EPSILON_EB)) THEN
            DO NSS=0,N_TRACKED_SPECIES
               IF (ZZ_0(NSS) + (DZZDT(NSS)+DZZDT1_1(NSS))*DT_SUB < 0._EB .AND. ABS(DZZDT(NSS))>TWO_EPSILON_EB) THEN
   ...
   ELSE RK2_IF ! RK2 w/ Richardson
      ...
      RICH_EX_LOOP: DO RICH_ITER =1,RICH_ITER_MAX
         ...
         TOTAL_MIX_MASS(1) = (1-ZETA1)*CELL_MASS
         SMIX_MIX_MASS(:,1) = SMIX_MIX_MASS(:,0) + (ZETA-ZETA1)*CELL_MASS*ZZ_GET_0
         ZZ_MIXED = SMIX_MIX_MASS(:,1)/(TOTAL_MIX_MASS(1))
         !--------------------
         ! Calculate A1 term
         ! Time step = DT_SUB
         ...
         ODE_LOOP1: DO NS = 1, SUB_DT1
            ...
            REACTION_LOOP1_2: DO NR = 1, N_REACTIONS
               RN => REACTION(NR)
               CALL COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT2(NR),A1,I,J,K,DT_SUB,ITER,TMP_0)
               DZZDT = RN%NU*SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT2(NR)
               IF (ANY(A1 + (DZZDT1_2+DZZDT)*DT_SUB < -TWO_EPSILON_EB)) THEN
                  DO NSS=0,N_TRACKED_SPECIES
                     IF (A1(NSS) + (DZZDT(NSS)+DZZDT1_2(NSS))*DT_SUB < 0._EB .AND. ABS(DZZDT(NSS))>TWO_EPSILON_EB) THEN
                        RATE_CONSTANT2(NR) = MIN(RATE_CONSTANT2(NR),A1(NSS)/&
                        ((ABS(RN%NU(NSS))*SPECIES_MIXTURE(NSS)%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW)*DT_SUB))
         ...
         ODE_LOOP2: DO NS = 1, SUB_DT2
            ...
            REACTION_LOOP2_1: DO NR = 1, N_REACTIONS
               ...
               CALL COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT(NR),ZZ_0,I,J,K,DT_SUB,ITER,TMP_0)
               DZZDT = RN%NU*SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT(NR)
               IF (ANY(ZZ_0 + (DZZDT2_1+DZZDT)*(DT_SUB*0.5_EB) < -TWO_EPSILON_EB)) THEN
                  DO NSS=0,N_TRACKED_SPECIES
            ...
            REACTION_LOOP2_2: DO NR = 1, N_REACTIONS
               RN => REACTION(NR)
               CALL COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT2(NR),A2,I,J,K,DT_SUB,ITER,TMP_0)
               DZZDT = RN%NU*SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT2(NR)
         ...
         ODE_LOOP4: DO NS = 1, SUB_DT4
            ...
            REACTION_LOOP4_1: DO NR = 1, N_REACTIONS
               ...
               CALL COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT(NR),ZZ_0,I,J,K,DT_SUB,ITER,TMP_0)
               DZZDT = RN%NU*SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT(NR)
               IF (ANY(ZZ_0 + (DZZDT4_1+DZZDT)*(DT_SUB*0.25_EB) < -TWO_EPSILON_EB)) THEN
                  DO NSS=0,N_TRACKED_SPECIES
            ...
            REACTION_LOOP4_2: DO NR = 1, N_REACTIONS
               RN => REACTION(NR)
               CALL COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT2(NR),A4,I,J,K,DT_SUB,ITER,TMP_0)
               DZZDT = RN%NU*SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT2(NR)
...
RECURSIVE SUBROUTINE COMPUTE_RATE_CONSTANT_EDCM1(NR,RATE_CONSTANT,ZZ_MIXED_IN,I,J,K,DT_SUB,ITER,TMP_0)
USE PHYSICAL_FUNCTIONS, ONLY : GET_MASS_FRACTION_ALL
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
REAL(EB), INTENT(IN) :: ZZ_MIXED_IN(0:N_TRACKED_SPECIES),DT_SUB,TMP_0
INTEGER, INTENT(IN) :: NR,I,J,K,ITER
REAL(EB), INTENT(INOUT) :: RATE_CONSTANT
REAL(EB) :: YY_PRIMITIVE(1:N_SPECIES),ZZ_MIN=1.E-10_EB,RATE_CONSTANT_LIMIT,DZ_F(1:N_REACTIONS),DF_R(1:N_REACTIONS),&
            MASS_OX,MASS_OX_STOICH,AA,EE,EQ_RATIO
INTEGER :: NS,NRR
...
IF(RN%FAST_CHEMISTRY .OR. RN%A_RAMP /= 'null' .OR. RN%E_RAMP /= 'null') THEN
   IF(RN%HEAT_OF_COMBUSTION > -TWO_EPSILON_EB) THEN
      DO NS = 0,N_TRACKED_SPECIES
         IF (RN%NU(NS) < -TWO_EPSILON_EB .AND. NS /= RN%FUEL_SMIX_INDEX) MASS_OX = ZZ_MIXED_IN(NS) ! Mass O2 in cell
      ENDDO
   ...
   ENDIF
ENDIF
IF (RN%A_RAMP /= 'null' .OR. RN%E_RAMP /= 'null') THEN
   EQ_RATIO = 0._EB
   EQ_RATIO = MASS_OX_STOICH/(MASS_OX + TWO_EPSILON_EB)
   AA = RN%A*EVALUATE_RAMP(EQ_RATIO,0._EB,RN%A_RAMP_INDEX)
   EE = RN%E*EVALUATE_RAMP(EQ_RATIO,0._EB,RN%E_RAMP_INDEX) 

   IF (AA > 1.E15_EB .AND. ABS(EE) < TWO_EPSILON_EB) THEN
      RN%FAST_CHEMISTRY = .TRUE.
   ELSE
      RN%FAST_CHEMISTRY = .FALSE.
   ENDIF
ENDIF

IF (RN%FAST_CHEMISTRY) THEN
   RATE_CONSTANT_LIMIT = 1.E10_EB
   IF (MASS_OX_STOICH >= MASS_OX) THEN ! Oxygen limited fast chemistry reactions  
      DZ_F(:) = 1._EB
      DO NRR = 1,N_REACTIONS
         RNN => REACTION(NRR)
         DO NS = 0,N_TRACKED_SPECIES
            IF (RNN%NU(NS) < -TWO_EPSILON_EB) & 
            DZ_F(NRR) = MIN(DZ_F(NRR),-ZZ_MIXED_IN(NS)*SPECIES_MIXTURE(RNN%FUEL_SMIX_INDEX)%MW/(RNN%NU(NS)*SPECIES_MIXTURE(NS)%MW))
         ENDDO
      ENDDO 
      DF_R(NR) = DZ_F(NR)/SUM(DZ_F)
      RATE_CONSTANT_LIMIT = DZ_F(NR)*DF_R(NR)/DT_SUB 
   ELSE ! Fuel limited fast chemistry reaction
      RATE_CONSTANT_LIMIT = ZZ_MIXED_IN(RN%FUEL_SMIX_INDEX)/DT_SUB
   ENDIF
   RATE_CONSTANT = RATE_CONSTANT_LIMIT
   RETURN
ENDIF
...
RATE_CONSTANT = AA*RHO(I,J,K)**RN%RHO_EXPONENT*EXP(-EE/(R0*TMP_0))*TMP_0**RN%N_T
IF (ALL(RN%N_S<-998._EB)) THEN
   DO NS=0,N_TRACKED_SPECIES
