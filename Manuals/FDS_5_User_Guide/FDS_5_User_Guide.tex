...
have been added since the release of FDS 5.0. The current version is 5.3.
...
MPI handles the transfer of information between these processes. Usually, each mesh is assigned its own process in a parallel calculation, although it is also possible
assign multiple meshes to a single process.\footnote{Assigning multiple meshes to a single process was introduced in FDS version 5.3.} 
In this way, large meshes can be computed on dedicated processors, while
...
{\ct T\_BEGIN} (Time Begin)\index{TIME!T\_BEGIN} to specify the time written to file for the first time step.
This would be useful for matching time lines of experimental data or or video recordings.
...
0 through 3. Note that the processes need to be invoked in ascending order, starting with 0. Why would you do this? Suppose you only have four processors available for this
job. By starting only four processes instead of six, you can save time because 'mesh2' and 'mesh3' can communicate directly with each other without having to transmit data
...
Most users of FDS assume that the acceleration of gravity\index{gravity} points in the negative $z$ direction, or more simply, downward.
...
\begin{figure}[t]
\includegraphics[width=\textwidth]{FIGURES/pyramid_s0000}
\caption{\bf An example of the multiplier functionality.}
\label{fig:mult}
\end{figure}

A slightly more complicated use of this feature is demonstrated by the following short input file:
...
\begin{notebox}
\noindent
Avoid using a Smokeview .smv file keyword for the {\ct ID}.  See the ``User's Guide for
Smokeview Version~5''~\cite{Smokeview_Users_Guide_5} for a listing of these keywords.
\end{notebox}
...
wall. Consider a solid surface forming the lower boundary of the computational domain ($z=0$). The horizontal velocity components $u_{ij,1}$ and
...
Prior to FDS 5.3, the default boundary condition for LES calculations
was where $u_{ij,0}$ and $v_{ij,0}$ were set to one-half the values of
$u_{ij,1}$ and $v_{ij,1}$, respectively. Now, however, FDS assumes a default value of 0.01~m for $d$. This parameter can be reset using
{\ct BOUNDARY\_LAYER\_THICKNESS} on the {\ct SURF} line. The boundary condition is now a function of the grid resolution.
...
Note that it is still possible to specify the tangential velocity conditions the old way.
To alter the default mode, you can set a parameter called
{\ct SLIP\_FACTOR} which ranges
...
Prior to FDS~5.3, the default values for {\ct SLIP\_FACTOR} were 0.5 for LES and -1.0 for DNS.
...
domain to have their own ``background'' pressures, and these zones can be connected via leakage and duct paths. The basic syntax
for a pressure {\ct ZONE} is:

\footnotesize
\begin{verbatim}
&ZONE XB=0.3,1.2,0.4,2.9,0.3,4.5 /
&ZONE XB=2.3,5.8,1.4,2.9,6.8,9.7 /
\end{verbatim}
\normalsize

\noindent
There are several restrictions to assigning pressure zones. First, the pressure zones must be completely surrounded by
obstructions (an external boundary also suffices). It is possible to ``break'' pressure zones by removing
...
Second, the pressure zones {\bf can} span multiple meshes,
...
zones abut (i.e. touch) even if the volume includes the solid region between the zones. FDS recognizes that a zone boundary has been removed when
two adjacent cells belong to two different zones but have no solid obstruction between them.
...
\begin{warning}
\noindent
When a {\ct SURF} is applied to the face of an {\ct OBST}, the first {\ct MATL\_ID} is at the face of the {\ct OBST}, with the other {\ct MATL\_ID}s being applied in succession with the final {\ct MATL\_ID} being applied on the opposite face of the {\ct OBST}.  If in the example above, {\ct BRICK WALL} was applied to the entire {\ct OBST} using {\ct SURF\_ID}, then when doing a heat transfer calculation from the +x face to the -x face, FDS would consider the {\ct OBST} to be {\ct BRICK} followed by {\ct INSULATOR} and the same for a heat transfer calculation from the -x face to the +x face.  To avoid this the user would need to specify a second {\ct SURF} that has the reverse {\ct MATL\_ID} and use {\ct SURF\_ID6} to apply the two {\ct SURF} definitions to opposite faces of the {\ct OBST}.
\end{warning}
...
Now you must specify at what temperature the reaction occurs, and how
fast the reaction occurs at that temperature.  The reaction rate at
the temperature, $T_s$, of the $i$th material ({\em i.e.} the {\ct
MATL} that you are currently describing) undergoing its $j$th reaction
is given by:
\be
  r_{ij} =
...
{\ct A(j)}, with units of s$^{-1}$. $E_{ij}$, the activation energy,
is prescribed via {\ct E(j)} in units of kJ/kmol. Remember that 1~kcal
is 4.184~kJ, and be careful with factors of 1000.  $A$ and $E$ are not
...
specify {\ct REFERENCE\_RATE} (s$^{-1}$) and {\ct
REFERENCE\_TEMPERATURE}($^\circ$C). This directs FDS to calculate the
activation energy, {\ct E}, using the specified reference rate,
reference temperature, and an assumed value of $A=1\times
10^{13}$~s$^{-1}$. Essentially, this guarantees that the {\ct
REFERENCE\_RATE} is achieved at the {\ct REFERENCE\_TEMPERATURE}. The
default value of {\ct REFERENCE\_RATE} is 0.1~s$^{-1}$.  It is
suggested that unless you have information to the contrary, leave {\ct
REFERENCE\_RATE} at its default value and just specify the {\ct
REFERENCE\_TEMPERATURE}. Note that the {\ct REFERENCE\_TEMPERATURE} is
{\em not} the same as an ignition temperature. Rather, it is simply
the temperature at which the mass fraction of the $i$th material
decreases at a rate of 0.1~s$^{-1}$. You should check the sensitivity
of these parameters following the procedure explained in
...
\noindent
One last issue before a few examples -- the most important one of
all. Equation~(\ref{rr}) describes the rate of the reaction as a function
...
\index{MATL!HEAT\_OF\_COMBUSTION}  If, for example, the {\ct
HEAT\_OF\_COMBUSTION} specified on the {\ct REAC} line is twice that
...
each gaseous species using the notation {\ct
HEAT\_OF\_COMBUSTION(j,s)}, as explained in Section~\ref{nu_gas}.

Several other examples of solid phase reactions can be found in
various examples. See Sections~\ref{thermoplastic} and
\ref{charring_solid}.
...
\noindent
The inclusion of {\ct BOILING\_TEMPERATURE} \index{MATL!BOILING\_TEMPERATURE} on the {\ct MATL} line tells FDS to use its liquid
...
\subsection{Special topic: Two-Sided Burning}
\label{info:EXPOSED}
...
$$ T_W(t) = {\ct TAMB} + f(t) \left( {\ct TMP\_FRONT} - {\ct TAMB} \right) $$
...
\index{Pipe Pressure}
\index{Sprinkler!Pipe Pressure}

In real sprinkler systems, the pipe pressure is affected by the number of open nozzles. Typically,
...
{\ct OPERATING\_PRESSURE}. At leat two of the following three parameters affecting the flow rate should then be specified: {\ct FLOW\_RATE},
{\ct K\_FACTOR} and {\ct OPERATING\_PRESSURE}. The flow rate is then given by $ \dm_w = K \sqrt{p}$ and droplet velocity
...
and $d_m(p_o)$ is specified by parameter {\ct DIAMETER} at the corresponding {\ct PART} line.
...
This control function behaves like an HVAC thermostat.  It can operating in one of two modes analagous to heating or cooling.  The function is provided with an {\ct INPUT\_ID} which is the {\ct DEVC} whose value is used by the function, a {\ct DEADBAND}, and the mode of operation by {\ct ON\_BOUND}.  If  {\ct ON\_BOUND='LOWER'}, the function changes state from its {\ct INITIAL\_VALUE} when the value of the {\ct INPUT\_ID} drops below the lower value in {\ct DEADBAND} and reverts when it increases past the upper value, i.e. like a heating system.  The reverse will occur if {\ct ON\_BOUND='UPPER'}, i.e. like a cooling system.  
...
is to disappear ({\ct POROUS\_FLOOR=.TRUE.} on the {\ct MISC} line). \index{POROUS\_FLOOR!output issues}
...
The PDPA device output is computed as
\be
f(t) = \frac{1}{\min(t,t_e)-t_s} \int_{t_s}^{\min(t,t_e)} \left( \frac{\sum_i n_i d_i^m}{\sum_i n_i d_i^n}\right)^{1/(m-n)}x  dt
\ee
The summation goes over all the particles within a sphere with radius {\ct PDPA\_RADIUS}.
...
{\ct ENTHALPY}                                  & $\sum Y_\alpha \int_0^T c_{p,\alpha}(T') \, dT'$  & kJ/kg      & D,I,P,S      \\ \hline
...
{\ct MASS LOSS}                                 & Integrated mass loss at surface               & kg/m$^2$       & B,D          \\ \hline
...
\hline \hline
\endhead
{\ct A}                       & Real array  & Pre-exponential factors        &    1/s           & 1E13   \\ \hline
...
{\ct ID     }                 & Character   & IDentifier                     &                  &        \\ \hline
{\ct THRESHOLD\_TEMPERATURE}  & Real array  & Threshold temperatures         & $^\circ$C        & -273.15 \\ \hline
{\ct N\_REACTIONS}            & Character   & Number of Reactions            &                  & 0      \\ \hline
{\ct N\_S}                    & Real array  & Exponents of mass fraction     &                  & 1.     \\ \hline
{\ct N\_T}                    & Real array  & Exponents of temperature       &                  & 0.     \\ \hline
{\ct NU\_FUEL}                & Real array  & Fuel yields                    & kg/kg            & 0.     \\ \hline
...
{\ct REFERENCE\_RATE}         & Real array  & Reaction rates at ref.~temp.   & s$^{-1}$         & 0.1    \\ \hline
{\ct REFERENCE\_TEMPERATURE}  & Real array  & Reference temperatures         & $^\circ$C        &        \\ \hline
...
{\ct BOUNDARY\_LAYER\_THICKNESS}      & Real            & See Section~\ref{info:Velocity_BC}&  m                  & 0.01                    \\ \hline
...
\section{Pressure Boundaries (\texorpdfstring{{\bf pressure\_boundary}}{pressure\_boundary})}
...
This chapter contains examples of simple calculations that demonstrate various features of FDS related to the conservation of mass and energy via the
equation of state.
...
935.2~Pa. The two cases are summarized in the following plots:
...
\section{Opening Pressure ZONEs (\texorpdfstring{{\bf zone\_break}}{zone\_break})}
\label{zone_break}

A {\em pressure zone} is a term used in FDS to designate a volume within the computational domain that has a background pressure that is distinct from
other volumes. Air movement through cracks, fans, ducts, and so on, is often a function of the overall pressure difference between the zones. In the
...
\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[height=2.2in]{FIGURES/door_crack_Pressure} &
\includegraphics[height=2.2in]{FIGURES/door_crack_HRR}
\end{tabular*}
...
which using the default mass extinction coefficient of
...
specified for the aspiration detector.  1~kg/s of combustion products with {\ct SOOT\_YIELD=0.001} is input into
the bottom of the compartment and 1~kg/s of total mass is removed from the top of the compartment.
...
\noindent
\begin{tabular*}{\textwidth}{lr}
\includegraphics[height=2.2in]{FIGURES/aspiration_detector} &
\end{tabular*}
...
is 344,369~kJ.  After a short period of time, 0.0131 kg of water evaporates, a mass fraction of 0.0115~kg/kg and the box
reaches an equilibrium temperature of 12.8~$^\circ$C, see the figure below.
At this point the energy content of the box is 336,867~kJ or a 3~\% error.
At 12.8~$^\circ$C, the expected mass fraction is 0.0115~kg/kg.
...
Note that in this example there is no need for a separate ignition source. The combustion model assumes fuel and oxygen burn on contact, and so the small amount of evaporation at ambient temperature is enough to start the reaction.
...
\section{How to Acquire Example Input Files}
\index{Download Verification Files}

Example input files for FDS can be found in the FDS-SMV project Subversion repository.  There are a few ways to get a copy of these files, two different approaches will be covered here. The first is via the SVN Command Line Client and the second is to Download Individual Files from the Web Interface.  Note: Some of these files are used for example input throughout this document, especially in Part \ref{part:samples}.

\begin{description}
\item[{\bf Command Line SVN Client}]
There are two methods for using the command line client, both methods have an optional step to keep the files up to date with the latest version in the repository over time.  The first method using SVN Checkout (co) is useful if you want to able to easily update the files on your local machine.  The downside of this method is that there will be '.svn' files within each directory that are used to check the local versions when the optional Update command is later used.  The SVN Export command will make a 'clean' copy the files and folders from the repository to your local computer, where 'clean' in this context means that there will not be '.svn' files in the directories.  The disadvantage of this method is that you cannot use the standard 'svn update' command to get the latest versions from the repository after check out.  Instead you will have to either force an overwrite of the existing files or remove the Verification directory and run the export command again.  Both methods will require that you download and install the \href{http://subversion.tigris.org}{{Subversion}} command line client for your Operating System.

{\bf METHOD 1: SVN Checkout}
\begin{enumerate}
\item From the command line (Terminal) change to the directory where you want the new directory of files to appear and enter the following command into the shell prompt.
\footnotesize
\begin{verbatim}
svn co http://fds-smv.googlecode.com/svn/trunk/FDS/trunk/Verification Verification
\end{verbatim}
\normalsize
\item {\bf Optional}: Run the following svn command from within the Verification directory when you want to update your local copy of the files to the latest versions of the test cases in the repository.
\footnotesize
\begin{verbatim}
svn update
\end{verbatim}
...
{\bf METHOD 2: SVN Export}
\begin{enumerate}
\item From the command line (Terminal) change to the directory where you want the new directory of files to appear and enter the following command into the shell prompt.
\footnotesize
\begin{verbatim}
svn export http://fds-smv.googlecode.com/svn/trunk/FDS/trunk/Verification
\end{verbatim}
\normalsize
\item {\bf Optional}: Run the following svn command from the directory where the Verification directory exists when you want to overwrite the local copy of the files to the latest versions of the test cases in the repository.  You could also just delete the local Verification directory and run the command above again.  Note the '--force' option in the export command.
\footnotesize
\begin{verbatim}
svn export --force http://fds-smv.googlecode.com/svn/trunk/FDS/trunk/Verification
\end{verbatim}
\normalsize
\end{enumerate}

\item[{\bf Download Individual Files From Web Interface}]
This method is not the most efficient way to get all of the test cases to your local computer.
Instead, it is a convenient way to review the individual files online within the repository browser, and to selectively download only the files that you are interested in.
\begin{enumerate}
\item Go to the FDS-SMV Project Site \href{http://fds-smv.googlecode.com}{{\ct http://fds-smv.googlecode.com}}.
Navigate to the Source Tab and then click Browse between 'Checkout' and 'Changes' next to the 'Search Trunk' text box.
You can also click this \href{http://code.google.com/p/fds-smv/source/browse/trunk/FDS/trunk/Verification/}{{\ct Link to Verification}}, to go directly to the Verification directory in the repository.
\item Navigate through the repository directories and click on the 'casename'.fds files that you want to review and download.
\item To download a file you are interested in, right click the 'View raw file' link in the 'File info' section to the right of the page and save it to your local machine.
\end{enumerate}
\end{description}
