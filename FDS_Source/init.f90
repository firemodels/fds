...
SELECT_POISSON_SOLVER: SELECT CASE(M%IPS)
   ...
   CASE (0:1) SELECT_POISSON_SOLVER
      ...
      IF (TWO_D .AND. CYLINDRICAL) THEN
         IF (ABS(XS)<=ZERO_P .AND. LBC==FISHPAK_BC_DIRICHLET_DIRICHLET) LBC = 5
         IF (ABS(XS)<=ZERO_P .AND. LBC==FISHPAK_BC_DIRICHLET_NEUMANN)   LBC = 6
         IF (ABS(XS)<=ZERO_P .AND. LBC==FISHPAK_BC_NEUMANN_NEUMANN)     LBC = 6
         IF (ABS(XS)<=ZERO_P .AND. LBC==FISHPAK_BC_NEUMANN_DIRICHLET)   LBC = 5
...
IF (OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLUX)>=ZERO_P .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
   WC%UW0 = SF%VOLUME_FLUX/OBX%FDS_AREA(ABS(IOR))
ENDIF
IF (OBST_INDEX>0 .AND. ABS(SF%MASS_FLUX_TOTAL)>=ZERO_P .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   IF (ABS(SF%VOLUME_FLUX)    >=ZERO_P)   THEN
      WC%UW0 = SF%VOLUME_FLUX/VT%FDS_AREA
   ENDIF
   IF (ABS(SF%MASS_FLUX_TOTAL)>=ZERO_P .AND. .NOT.EVACUATION_ONLY(NM)) THEN
      WC%UW0 = SF%MASS_FLUX_TOTAL/RHOA*WC%ONE_D%AREA_ADJUST
   ENDIF
 
   ! Special velocity profiles
 
   IF (SF%PROFILE==PARABOLIC) THEN 
      SELECT CASE(ABS(IOR))
         CASE(1)
            Y1 = M%Y(VT%J1)
            Y2 = M%Y(VT%J2)
   ...
   IF (SF%PROFILE==ONED_PARABOLIC) THEN 
      SELECT CASE(ABS(IOR))
         CASE(1)
            Y1 = M%Y(VT%J1)
            Y2 = M%Y(VT%J2)
