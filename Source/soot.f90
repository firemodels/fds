...
REAL(EB), PARAMETER :: CS=1.17_EB,CT=2.2_EB,CM=1.146_EB
REAL(EB), PARAMETER :: CM3=3._EB*CM,CS2=CS*2._EB,CT2=2._EB*CT
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_AEROSOL,RHOG,ZZ_GET(1:N_TRACKED_SPECIES),YDEP,K_AIR,TMP_FILM,ALPHA,DTMPDX,&
            TAU_PLUS,TAU_PLUS_C,VEL_W,U2,V2,W2,U_GRAV,D_SOLID,MW_RATIO,KN,KN_FAC
...
SMIX_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      KN_FAC = MUGAS*SQRT(2._EB*PI/(PBAR(KKG,PRESSURE_ZONE(IIG,JJG,KKG))*RHO(IIG,JJG,KKG)))
      RHOG = RHO(IIG,JJG,KKG)
      ALPHA = K_AIR/SM%CONDUCTIVITY_SOLID
      IF (SIM_MODE/=DNS_MODE) THEN
         DTMPDX = WC%ONE_D%HEAT_TRANS_COEF*(TGAS-TWALL)/K_AIR
      ELSE
         DTMPDX = 2._EB*(TGAS-TWALL)*WC%ONE_D%RDN
      ENDIF
      IF (THERMOPHORETIC_DEPOSITION) THEN
         KN = KN_FAC/SM%THERMOPHORETIC_DIAMETER
         U_THERM = CS2*(ALPHA+CT*KN)*CUNNINGHAM(KN)/((1._EB+CM3*KN)*(1+2*ALPHA+CT2*KN)) * MUGAS/(TGAS*RHOG)*DTMPDX
      ENDIF
