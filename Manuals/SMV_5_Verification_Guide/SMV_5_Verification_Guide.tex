...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~1017-3, 58 pages (July 2009) \\
CODEN: NSPUE2 }
...
the Smokeview script files {\tt plume5c.ssf}\ (see Appendix \ref{SSFplume5c}) and {\tt fireline.ssf}\ (see Appendix \ref{SSFfireline}) to generate the verification figures. The {\tt plume5c.fds}\ test case is a simple fire plume with two blockages to make the smoke flow more complex.  A portion of the gas phase and the upper blockage are both initialized to 600~\degC\ (rather than 20~\degC) in order to test the conversion of a known temperature data to the correct color (as shown in the colorbar).  The {\tt fireline.ssf}\ test case is a terrain test case.  The center of the case has a {\em hill}.  The fire line data displayed conforms to this hill.
...
\begin{figure}[\figoptions]
\begin{center}
\includegraphics[height=4.0in]{figures/linear_interpolation}
\end{center}
...
of fire modeling simulations such as FDS.  Future work with Smokeview then involves adding or improving visualization algorithms that allow one to provide more and better insight, in particular to remove the limitations discussed at the beginning of this report.  Work needs to be done to visualize the fire more realistically, perhaps by using its temperature as an indicator of color.
The {\em transport} of light into and out of the smoke/fire is another important factor that effects its appearance.  Light sources could consist of either man made lights or from the fire itself.  Work also needs to be done to generalize the wavelength of light used to visualize the scene, {\em ie} implementing an algorithm in Smokeview to simulate a thermal imager or infrared viewer.  The GPU (graphical processing unit) is already being used in Smokeview to perform limited 3D smoke computations.  It will need to be exploited even more in order to make the proposed fire coloring and smoke lighting computations practical.
