...
INTEGER :: N
NAMELIST /DUMP/ RENDER_FILE,SMOKE3D,SMOKE3D_COMPRESSION,SMOKE3D_QUANTITY,FLUSH_FILE_BUFFERS,MASS_FILE, &
...
SMOKE3D_QUANTITY   = 'soot mass fraction'
...
! Check Plot3D and SMOKE3D QUANTITIES
...
NAMELIST /PART/ NUMBER_INITIAL_DROPLETS,FYI,DROPLETS_PER_SECOND,MASS_PER_VOLUME, &
                ...
                VERTICAL_VELOCITY,HORIZONTAL_VELOCITY,GAMMA,MONODISPERSE,EVAPORATE
...
REAL(EB) :: SPREAD_RATE,TRANSPARENCY,DUMMY,XYZ(3),TMP_EXTERIOR,MASS_FRACTION(MAX_SPECIES) 
...
NAMELIST /VENT/ XB,IOR,MB,PBX,PBY,PBZ,SURF_ID,FYI,RGB,TRANSPARENCY,COLOR, &
                ...
                XYZ,EVACUATION,MESH_ID,SPREAD_RATE,TMP_EXTERIOR,MASS_FRACTION
  ...
  IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
     ...
     SELECT CASE (TRIM(OUTTYPE))
        ...
        CASE ('PLOT3D')
            ...
            ENDIF
        CASE ('SMOKE3D')
            IF (.NOT.OUTPUT_QUANTITY(ND)%MASS_FRACTION) THEN
