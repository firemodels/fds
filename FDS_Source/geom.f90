...
G%N_FACES = IFACE
G%N_VOLUS = IVOLUS
IF (IFACE>0) G%SURFS(1:G%N_FACES) = GET_SURF_INDEX(G%SURF_ID)
IF (IVOLUS>0) G%MATLS(1:G%N_VOLUS) = GET_MATL_INDEX(G%MATL_ID)
...
SUBROUTINE OUTGEOM(LUNIT,IS_DYNAMIC,TIME)
   ...
   IF (N_VERTS>0 .AND. (N_FACES>0 .OR. N_VOLUS>0)) THEN
      CALL MERGE_GEOMS(VERTS,N_VERTS, &
