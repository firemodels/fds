...
USE EMBEDDED_MESH_METHOD, ONLY: SCALARF_EMB,RESTRICT_MASS_EMB,RESTRICT_DIV_EMB,SCALAR_GHOST_EMB, &
...
INTEGER NOM,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IW,IRANK,JRANK
...
MAIN_LOOP: DO  
   ...
   IF (EMBEDDED_MESH) THEN
      CALL SORT_MESH_LEVEL
   
      !CALL MATCH_VELOCITY_EMB(1,2)
      !CALL PROJECT_VELOCITY(2)
      
      !CALL MATCH_VELOCITY_EMB(1,3)
      !CALL MATCH_VELOCITY_EMB(2,3)
      !CALL PROJECT_VELOCITY(3)
      
      DO JRANK=NMESHES,1,-1
         NOM=MESH_LIST_EMB(JRANK)
         IF (NOM==1) CYCLE
         DO IRANK=NMESHES,1,-1
            NM=MESH_LIST_EMB(IRANK)
            IF (IRANK<=JRANK) CYCLE
            !print *,NM,NOM
            CALL MATCH_VELOCITY_EMB(NM,NOM)
         ENDDO
         !print *,NOM
         CALL PROJECT_VELOCITY(NOM)
      ENDDO
   ...
   IF (EMBEDDED_MESH) THEN
      !CALL SCALARF_EMB(2,3)
      !CALL SCALARF_EMB(1,2)
      !CALL SCALARF_EMB(1,3)
      DO IRANK=1,NMESHES
         NM=MESH_LIST_EMB(IRANK)
         DO JRANK=1,NMESHES
            NOM=MESH_LIST_EMB(JRANK)
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      IF (EMBEDDED_MESH) THEN
         !CALL SCALAR_GHOST_EMB(1,2)
         !CALL SCALAR_GHOST_EMB(1,3)
         !CALL SCALAR_GHOST_EMB(2,3)
         DO IRANK=NMESHES,1,-1
            NM=MESH_LIST_EMB(IRANK)
            DO JRANK=NMESHES,1,-1
               NOM=MESH_LIST_EMB(JRANK)
               IF (IRANK<=JRANK) CYCLE
               CALL SCALAR_GHOST_EMB(NM,NOM)
            ENDDO
         ENDDO
      ENDIF
      ...
      IF (EMBEDDED_MESH) THEN
         !CALL RESTRICT_DIV_EMB(2,3)
         !CALL RESTRICT_DIV_EMB(1,2)
         !CALL RESTRICT_DIV_EMB(1,3)
         DO IRANK=1,NMESHES
            NM=MESH_LIST_EMB(IRANK)
            DO JRANK=1,NMESHES
               NOM=MESH_LIST_EMB(JRANK)
               IF (IRANK<=JRANK) CYCLE
               CALL RESTRICT_DIV_EMB(NM,NOM)
            ENDDO
         ENDDO
      ENDIF
   ...
   IF (EMBEDDED_MESH .AND. N_REACTIONS>0) THEN
      !CALL RESTRICT_MASS_EMB(2,3)
      !CALL RESTRICT_MASS_EMB(1,2)
      !CALL RESTRICT_MASS_EMB(1,3)
      DO IRANK=1,NMESHES
         NM=MESH_LIST_EMB(IRANK)
         DO JRANK=1,NMESHES
            NOM=MESH_LIST_EMB(JRANK)
            IF (IRANK<=JRANK) CYCLE
            CALL RESTRICT_MASS_EMB(NM,NOM)
         ENDDO
      ENDDO
   ENDIF
