...
IMPLICIT NONE 
CHARACTER(255), PARAMETER :: funcid='$Id: func.f90,v 1.21 2007/04/16 12:49:33 shostikk Exp $'
...
USE GLOBAL_CONSTANTS, ONLY: INPUT_FILE
IF (INPUT_FILE=='null') CALL GETARG(1,INPUT_FILE)
...
IF (THICKNESS.EQ.0._EB) THEN
   ...
   RETURN
ENDIF
SHRINK_LOOP: DO N=1,999,2
   DSUM = 0._EB
   SUM_LOOP: DO I=1,(N-1)/2
      DSUM = DSUM + 2._EB*STRETCH_FACTOR**(I-1)
   ENDDO SUM_LOOP
   DSUM = DSUM + STRETCH_FACTOR**((N-1)/2)
...
REAL(EB) DX_S
   ...
   DO NL=1,N_LAYERS
      DO I=1,N_LAYER_CELLS(NL)
         II = II+1
         IF (I <= (N_LAYER_CELLS(NL)+1)/2) THEN
            DX_S = SMALLEST_CELL_SIZE(NL)*STRETCH_FACTOR**(I-1)
         ELSE
            DX_S = SMALLEST_CELL_SIZE(NL)*STRETCH_FACTOR**(N_LAYER_CELLS(NL)-I)
         ENDIF
...
SELECT CASE(RAMP_INDEX)
   CASE(-2)
      EVALUATE_RAMP = TANH(RAMP_INPUT/TAU)
   CASE(-1)
