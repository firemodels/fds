...
IF (N_SPECIES>0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   !!ALLOCATE( M%YY(-1:IBP1+1,-1:JBP1+1,-1:KBP1+1,N_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','YY',IZERO)
   M%YY = 0._EB
   ALLOCATE(M%YYS(0:IBP1,0:JBP1,0:KBP1,N_SPECIES),STAT=IZERO)
   !!ALLOCATE(M%YYS(-1:IBP1+1,-1:JBP1+1,-1:KBP1+1,N_SPECIES),STAT=IZERO)
...
NOT_EVAC_IF_1: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   M%TMP_B = TMPA
   ALLOCATE(M%YY_F(N_TOTAL_WALL_CELLS,N_SPECIES),STAT=IZERO)
...
NOT_EVAC_IF_2: IF (.NOT.EVACUATION_ONLY(NM)) THEN 
   ...
   M%KW = 1._EB
   ALLOCATE(M%RHODW(N_TOTAL_WALL_CELLS,N_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','RHODW',IZERO) 
   M%RHODW = 1._EB
   ALLOCATE(M%AREA_ADJUST(N_TOTAL_WALL_CELLS),STAT=IZERO)
   CALL ChkMemErr('INIT','AREA_ADJUST',IZERO)  
   M%AREA_ADJUST = 1._EB
   ALLOCATE(M%MASSFLUX(N_TOTAL_WALL_CELLS,0:N_SPECIES),STAT=IZERO)
   CALL ChkMemErr('INIT','MASSFLUX',IZERO)  
   M%MASSFLUX = 0._EB
   ALLOCATE(M%MASSFLUX_ACTUAL(N_TOTAL_WALL_CELLS,0:N_SPECIES),STAT=IZERO)
