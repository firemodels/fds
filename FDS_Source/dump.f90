...
EVAC_ONLY4: IF (ANY(EVACUATION_GRID)) THEN
   DO N=1,N_EVAC
      ...
      WRITE(LU_SMV,'(1X,A)') EVAC_CLASS_NAME(N)
      WRITE(LU_SMV,'(3F13.5)') REAL(EVAC_CLASS_RGB(:,N),FB)/255._EB
      WRITE(LU_SMV,'(I3)') EVAC_N_QUANTITIES
      DO NN=1,EVAC_N_QUANTITIES
         WRITE(LU_SMV,'(1X,A)') OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%NAME
         WRITE(LU_SMV,'(1X,A)') OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%SHORT_NAME
         WRITE(LU_SMV,'(1X,A)') OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%UNITS
      ENDDO
   ENDDO
