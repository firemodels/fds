...
CALL ChkMemErr('READ','TUSED',IZERO) ; TUSED = 0._EB
...
SIG_FIGS           = 8
SIG_FIGS_EXP       = 3
IF (NMESHES>=10) SUPPRESS_DIAGNOSTICS = .TRUE.
UVW_TIMER          = 1.E10_EB
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   IF (SS%SPECIFIC_HEAT > 0._EB) THEN
      ...
      ELSEIF (SS%REFERENCE_ENTHALPY < -1.E20_EB) THEN
         ...
         SS%H_F = SS%REFERENCE_ENTHALPY + SS%SPECIFIC_HEAT * (H_F_REFERENCE_TEMPERATURE - SS%REFERENCE_TEMPERATURE)
      ENDIF
   ENDIF
...
DO N=1,N_TRACKED_SPECIES
   ...
   IF(SM%H_F <= -1.E22_EB) THEN
      SM%H_F = 0._EB
      DO J=1,N_SPECIES
         SM%H_F = SM%H_F + SM%VOLUME_FRACTION(J) * SPECIES(J)%H_F*SPECIES(J)%MW ! Calculate H_F of mixtures
      ENDDO
      SM%H_F = SM%H_F/SM%MW
   ENDIF
END DO
...
LOGICAL :: REDEFINE_H_F(1:N_TRACKED_SPECIES)
TYPE (SPECIES_MIXTURE_TYPE), POINTER :: SM
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   HOC_IF: IF (RN%HEAT_OF_COMBUSTION > -1.E21) THEN ! User specified heat of combustion
      IF (SIMPLE_CHEMISTRY) THEN
         IF (IDEAL) THEN
            ...
            RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION + ( &
                           RN%NU_CO*(SPECIES(CO2_INDEX)%H_F*SPECIES(CO2_INDEX)%MW*0.001 - &
                                     SPECIES(CO_INDEX)%H_F*SPECIES(CO_INDEX)%MW*0.001) &
                         + RN%NU_SOOT*SPECIES(CO2_INDEX)%H_F*SPECIES(CO2_INDEX)%MW*0.001*(1._EB-RN%SOOT_H_FRACTION) &
                         + RN%NU_SOOT*SPECIES(H2O_INDEX)%H_F*SPECIES(H2O_INDEX)%MW*0.001*RN%SOOT_H_FRACTION*0.5_EB)
            RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION/SPECIES(FUEL_INDEX)%MW*1000._EB !J/mol->J/kg
         ENDIF
         RN%EPUMO2 = RN%HEAT_OF_COMBUSTION*SPECIES(FUEL_INDEX)%MW/(RN%NU_O2*SPECIES(O2_INDEX)%MW)
      ENDIF
      HF_COUNT = 0
      DO NS = 1,N_TRACKED_SPECIES
      ...
      WRITE(*,*) RN%HEAT_OF_COMBUSTION,RN%FUEL_SMIX_INDEX,RN%NU(RN%FUEL_SMIX_INDEX),SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW
      SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%H_F = &
         RN%HEAT_OF_COMBUSTION*RN%NU(RN%FUEL_SMIX_INDEX)*SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*0.001_EB
      DO NS = 1,N_TRACKED_SPECIES
         IF  (NS == RN%FUEL_SMIX_INDEX) CYCLE
            SM=>SPECIES_MIXTURE(NS)
            SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%H_F = SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%H_F - RN%NU(NS)*SM%H_F*SM%MW*0.001_EB
            WRITE(*,*) NS,SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%H_F,RN%NU(NS),SM%H_F,SM%MW
      ENDDO
      SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%H_F = SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%H_F/ &
                                                (RN%NU(RN%FUEL_SMIX_INDEX)*SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*0.001_EB)
      IF (SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%SINGLE_SPEC_INDEX>0) &
         SPECIES(SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%SINGLE_SPEC_INDEX)%H_F = SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%H_F
