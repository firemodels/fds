...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,ASSUMED_GAS_TEMPERATURE_RAMP,&
                ...
                INITIAL_UNMIXED_FRACTION,ISOTROPIC_PARTICLE_FORCE,&
                KINETIC_ENERGY_SOURCE,LAPSE_RATE,LES_FILTER_WIDTH,MAX_CHEMISTRY_ITERATIONS,&
                MAX_LEAK_PATHS,MAXIMUM_VISIBILITY,MEAN_FORCING,MPI_TIMEOUT,NOISE,NOISE_VELOCITY,NO_EVACUATION,&
...
NAMELIST /DUMP/ COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,&
...
NAMELIST /SPEC/ AEROSOL,BACKGROUND,CONDUCTIVITY,CONDUCTIVITY_SOLID, &
                ...
                MIN_DIAMETER,MW,N_BINS,PRIMITIVE,RADCAL_ID,&
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            TMP_FRONT,TMP_INNER(MAX_LAYERS),THICKNESS(MAX_LAYERS),VEL,INTERNAL_HEAT_SOURCE(MAX_LAYERS),&
...
REAL(EB) :: DEPTH,ORIENTATION(3),ROTATION,SETPOINT,FLOWRATE,BYPASS_FLOWRATE,DELAY,XYZ(3),CONVERSION_FACTOR,SMOOTHING_FACTOR,&
            OR_TEMP(3),QUANTITY_RANGE(2),STATISTICS_DT,STATISTICS_START,COORD_FACTOR
...
NAMELIST /DEVC/ BYPASS_FLOWRATE,CONVERSION_FACTOR,COORD_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,DUCT_ID,EVACUATION,FLOWRATE,FYI,&
                ...
                RELATIVE,ROTATION,SETPOINT,SMOOTHING_FACTOR,SPEC_ID,STATISTICS,STATISTICS_DT,STATISTICS_START,SURF_ID,&
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%LINE == 0 .AND. (DV%STATISTICS=='RMS' .OR. DV%STATISTICS=='COV' .OR. DV%STATISTICS=='CORRCOEF')) THEN
      ...
      IF (DV%STATISTICS_DT > T_END -DV%STATISTICS_START) DV%STATISTICS_DT = T_END -DV%STATISTICS_START
      DV%DT = 0.01_EB*DV%STATISTICS_DT
      DV%T = DV%STATISTICS_START + DV%DT
      DV%TIME_MAX_VALUE = DV%STATISTICS_START + DV%STATISTICS_DT
      ALLOCATE(DV%TIME_ARRAY(100))
      DV%TIME_ARRAY=0._EB
      DV%TIME_ARRAY(100) = TWO_EPSILON_EB
      SELECT CASE (DV%STATISTICS)
         CASE('RMS')
            ALLOCATE(DV%AVERAGE_ARRAY(100))
            DV%AVERAGE_ARRAY=0._EB
            ALLOCATE(DV%RMS_ARRAY(100))
            DV%RMS_ARRAY=0._EB
         CASE('COV')
            ALLOCATE(DV%AVERAGE_ARRAY(100))
            DV%AVERAGE_ARRAY=0._EB
            ALLOCATE(DV%AVERAGE2_ARRAY(100))
            DV%AVERAGE2_ARRAY=0._EB
            ALLOCATE(DV%COV_ARRAY(100))
            DV%COV_ARRAY=0._EB
         CASE('CORRCOEF')
            ALLOCATE(DV%AVERAGE_ARRAY(100))
            DV%AVERAGE_ARRAY=0._EB
