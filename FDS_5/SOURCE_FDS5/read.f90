...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,RELAXATION_FACTOR,FYI, &
                ...
                FLUX_LIMITER,FREEZE_VELOCITY,ADJUST_HOR,CFL_VELOCITY_NORM,PERIODIC_TEST
...
NAMELIST /DUMP/ RENDER_FILE,SMOKE3D,SMOKE3D_COMPRESSION,SMOKE3D_QUANTITY,FLUSH_FILE_BUFFERS,MASS_FILE,STATE_FILE, &
                ...
                PLOT3D_QUANTITY,UL_PAN_DATA,STATUS_FILES,DEVC_COLUMN_LIMIT,CTRL_COLUMN_LIMIT
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   READ_OBST_LOOP: DO NN=1,N_OBST_O
      ...
      IF (DEVC_ID /='null') THEN
         ...
         DEVICE(OB%DEVC_INDEX)%CURRENT_STATE = DEVICE(OB%DEVC_INDEX)%INITIAL_STATE
         DEVICE(OB%DEVC_INDEX)%PRIOR_STATE   = DEVICE(OB%DEVC_INDEX)%INITIAL_STATE
         OB%REMOVABLE = .TRUE.
      ENDIF
      IF (CTRL_ID /='null') THEN
         IF (.NOT.CONTROL(OB%CTRL_INDEX)%INITIAL_STATE) OB%HIDDEN = .TRUE.
         CONTROL(OB%CTRL_INDEX)%CURRENT_STATE = CONTROL(OB%CTRL_INDEX)%INITIAL_STATE
         CONTROL(OB%CTRL_INDEX)%PRIOR_STATE   = CONTROL(OB%CTRL_INDEX)%INITIAL_STATE
         OB%REMOVABLE = .TRUE.
      ENDIF
...
!      IF ((.NOT.EVACUATION .AND. EVACUATION_ONLY(NM)) .OR. (EVACUATION .AND. .NOT.EVACUATION_ONLY(NM))) CYCLE MESH_LOOP
      ...
      OBST_LOOP: DO
         ...
         DEVC_OR_CTRL: IF (DEVC_ID/='null' .OR. CTRL_ID/='null') THEN
            ...
            IF (DEVC_ID /='null') THEN
               OB%REMOVABLE = .TRUE.
               IF (DEVICE(OB%DEVC_INDEX)%SPECIFIED_STATE) OB%HIDDEN = .TRUE.
               DEVICE(OB%DEVC_INDEX)%INITIAL_STATE = .NOT.DEVICE(OB%DEVC_INDEX)%SPECIFIED_STATE
               DEVICE(OB%DEVC_INDEX)%CURRENT_STATE = DEVICE(OB%DEVC_INDEX)%INITIAL_STATE
               DEVICE(OB%DEVC_INDEX)%PRIOR_STATE   = DEVICE(OB%DEVC_INDEX)%INITIAL_STATE
            ENDIF
            IF (CTRL_ID /='null') THEN
               OB%REMOVABLE = .TRUE.
               IF (CONTROL(OB%CTRL_INDEX)%SPECIFIED_STATE) OB%HIDDEN = .TRUE.
               CONTROL(OB%CTRL_INDEX)%INITIAL_STATE = .NOT.CONTROL(OB%CTRL_INDEX)%SPECIFIED_STATE
               CONTROL(OB%CTRL_INDEX)%CURRENT_STATE = CONTROL(OB%CTRL_INDEX)%INITIAL_STATE
               CONTROL(OB%CTRL_INDEX)%PRIOR_STATE   = CONTROL(OB%CTRL_INDEX)%INITIAL_STATE
            ENDIF
...
!  IF (PROCESS(NM)/=MYID) CYCLE MESH_LOOP_1
   ...
   READ_VENT_LOOP: DO NN=1,NVO
      ...
      IF (DEVC_ID /= 'null') THEN
         ...
         DEVICE(VT%DEVC_INDEX)%CURRENT_STATE = DEVICE(VT%DEVC_INDEX)%INITIAL_STATE
         DEVICE(VT%DEVC_INDEX)%PRIOR_STATE   = DEVICE(VT%DEVC_INDEX)%INITIAL_STATE
      ENDIF
      IF (CTRL_ID /= 'null') THEN
         IF (.NOT.CONTROL(VT%CTRL_INDEX)%INITIAL_STATE) VT%ACTIVATED = .FALSE.
         CONTROL(VT%CTRL_INDEX)%CURRENT_STATE = CONTROL(VT%CTRL_INDEX)%INITIAL_STATE      
         CONTROL(VT%CTRL_INDEX)%PRIOR_STATE   = CONTROL(VT%CTRL_INDEX)%INITIAL_STATE
      ENDIF
...
READ_DEVC_LOOP: DO NN=1,N_DEVCO
   ...
   DV%SPECIFIED_STATE  = INITIAL_STATE
...
READ_CTRL_LOOP: DO NC=1,N_CTRL
   ...
   CF%SPECIFIED_STATE = INITIAL_STATE
...
OUTPUT_QUANTITY(18)%NAME = 'RADIANT INTENSITY'       
...
OUTPUT_QUANTITY(90:95)%SPEC_ID_REQUIRED = .TRUE.
...
OUTPUT_QUANTITY(-16)%NAME= 'MASS LOSS'               
...
IF (QUANTITY=='RADIANT_INTENSITY')             QUANTITY = 'RADIANT INTENSITY'
...
DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
  ...
  IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
     ...
     IF (OUTPUT_QUANTITY(ND)%SPEC_ID_REQUIRED .AND. SPEC_INDEX<1) THEN
