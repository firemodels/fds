...
INTEGER  :: NRDMIE, NLMBDMIE, NDG=50	!EBl perhaps needs to be increased	
...
IMPLICIT NONE
REAL(EB), ALLOCATABLE :: RDMIE(:),  LMBDMIE(:),LMBDWGHT(:) !EBl , RDWGHT(:)
...
!EBl USE PHYSICAL_FUNCTIONS, ONLY: DROPLET_SIZE_DISTRIBUTION 
USE TYPES, ONLY: PARTICLE_CLASS_TYPE, PARTICLE_CLASS 
INTEGER  :: NSB,I,J,IBND,IZERO,NX,NLAMBDALOW(1),NLAMBDAHIGH(1),ND ,CLASS_NUMBER  !EBl ,NRDINT
REAL(EB) :: RMMAX,RMMIN,RDTMP,IB,IBSUM,AVAL,BVAL,ASUM,BSUM  !EBl ,GAMMA,SIGMA
!EBl REAL(EB), ALLOCATABLE, DIMENSION(:) :: FDDIST, RDDIST
...
!
!EBl ALLOCATE(FDDIST(0:NRDINT),STAT=IZERO)
!EBl CALL ChkMemErr('INIT','FDDIST',IZERO)
!EBl ALLOCATE(RDDIST(0:NRDINT),STAT=IZERO)
!EBl CALL ChkMemErr('INIT','RDDIST',IZERO)
!EBl ALLOCATE(RDWGHT(0:NRDINT),STAT=IZERO)
!EBl CALL ChkMemErr('INIT','RDWGHT',IZERO)
...
BANDLOOP: DO IBND = 1,NSB
   ...
   DRGROUPLOOP: DO ND = 1, NDG	!EBl NDG perhaps needs to be increased

      PC%R50(ND) = EXP(DGROUP_A*REAL(ND,EB) + DGROUP_B)
      !EBl CALL DROPLET_SIZE_DISTRIBUTION(2._EB*PC%R50(ND),RDDIST,FDDIST,NRDINT, GAMMA,SIGMA)     

      !     Calculate integration weights using trapezoidal rule 
      !     rdwght(n) = 0.5*f(n)*[r(n+1)-r(n-1)] = 0.5*{[f(n+1)-f(n-1)]/[r(n+1)-r(n-1)]}*[r(n+1)-r(n-1)]
      !               = 0.5*[f(n+1)-f(n-1)]
	  
	  PC%DMN(ND) = 2._EB * PC%R50(ND)		!EBl
	   
      !EBl RDWGHT(0) = 0.5_EB*(FDDIST(1)-FDDIST(0))
      !EBl DO I = 1,NRDINT-1
      !EBl    RDWGHT(I) = 0.5_EB*(FDDIST(I+1)-FDDIST(I-1))
      !EBl ENDDO
      !EBl RDWGHT(NRDINT) = 0.5_EB*(FDDIST(NRDINT)-FDDIST(NRDINT-1))
      ...
      !EBl PC%DMN(ND) = 0._EB
      !EBl DO I = 0,NRDINT
      !EBl    PC%DMN(ND) = PC%DMN(ND) + RDWGHT(I)*PI*(2._EB*RDDIST(I))**3
      !EBl    ASUM       = ASUM       + RDWGHT(I)*PI*(2._EB*RDDIST(I))**2
      !EBl ENDDO
      !EBl PC%DMN(ND) = PC%DMN(ND)/ASUM
...
DEALLOCATE(RDMIE)
!EBl DEALLOCATE(RDWGHT)
DEALLOCATE(QSCA)
DEALLOCATE(QABS)
DEALLOCATE(CHI_F)
DEALLOCATE(LMBDMIE)
DEALLOCATE(LMBDWGHT)
!EBl DEALLOCATE(FDDIST)
!EBl DEALLOCATE(RDDIST)
