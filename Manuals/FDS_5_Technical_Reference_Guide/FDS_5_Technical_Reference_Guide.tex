...
The strategy of approximating the solution of the Poisson equation on multiple meshes is to decompose the pressure term such that the velocity is
updated according to:
\be \dod{\bu}{t} + \bF + \nabla \cH^0 + \nabla \cH' = 0 \label{finepois} \ee
such that
\be \nabla^2 {\cal H}^0 = -\nabla \cdot \bF - \dod{}{t} (\nabla \cdot \bu) \quad ; \quad  \nabla^2 \cH' = 0  \ee
The equation on the left is the same as that described above, with the same boundary conditions, as well.
To get the boundary conditions for the second equation, we first solve a discretized form of the Poisson equation for the entire domain using the individual
meshes themselves. This coarse Poisson equation is derived by integrating Eq.~(\ref{finepois}) over each mesh and applying the Divergence Theorem:
\be \int_{\partial \Omega_m} \nabla \cH' \cdot \dS = -\int_{\partial \Omega_m} \left(\overline{\bF}+\nabla \overline{\cH^0} \right) \cdot \dS
    - \int_{\Omega_m} \dod{(\nabla \cdot \bu)}{t} \; dV \ee
The subscript $m$ refers to individual cells of the coarse mesh. The overbar indicates that at mesh boundaries, the average of the quantity is to be used.

Next, define
\be \bG = \bF + \nabla \cH^0 \ee
and introduce a single scalar quantity, $\tilde{\cH}_m$, for each mesh that satisfies:
\be \frac{\overline{\cH}_{m+}'  - \overline{\cH}_{m}' }{\dx} A_{x^+} -
    \frac{\overline{\cH}_{m }' - \overline{\cH}_{m-}'}{\dx} A_{x^-} + \; \cdots =
    -\int_{x^+} \overline{G_x} \, dy \, dz  + \int_{x^-} G_x \, dy \, dz  + \;  \cdots
    - \int_{\Omega_m} \dod{(\nabla \cdot \bu)}{t} dV   \label{coarse_pois} \ee
The subscripts $m^+$ and $m^-$ refer to neighboring meshes, and $A_{x^+}$ and $A_{x^-}$ and so on
refer to the area of the coarse cell interfaces.
Solve the coarse grid Poisson equation (\ref{coarse_pois}) with a direct (non-iterative) solver. Note that at open boundaries,
the ``ghost'' value of the coarse pressure is simply minus the value of the coarse pressure in the mesh adjacent to
the open boundary.

Following the solution of the coarse pressure equation, at the interface between two
neighboring meshes, we now have an estimate of the rate of change of the volume flux. For example, at the $x=x_{\max}$ interface:
\be \frac{d\dot{V}}{dt}  = -\int_{x=x_{\max}} \overline{G}_x \, dy \, dz
...
such that at the interface the gradient of $\cH'$ is constant. For example, at the $x=x_{\max}$ interface, the gradient of
$\cH'$ on the mesh to the left must satisfy:
\be  \frac{d\dot{V}}{dt} =
    - \int_{x=x_{\max}} G_{x,I,jk,m} \; dy \, dz -
   \int_{x=x_{\max}} \dod{\cH'}{x} \; dy \, dz \label{constraint} \ee
