...
NEW_ELIZ_TRANSFER = .FALSE.
IF (CORRECTOR) CALL MOVE_PARTICLES(T,NM)
IF (CORRECTOR .AND. .NOT. NEW_ELIZ_TRANSFER) CALL PARTICLE_MASS_ENERGY_TRANSFER(T,NM)
IF (CORRECTOR .AND. NEW_ELIZ_TRANSFER) CALL PARTICLE_MASS_ENERGY_TRANSFER_ELIZ(T,NM)
...
SUBROUTINE PARTICLE_MASS_ENERGY_TRANSFER_ELIZ(T,NM)
...
! New variables ELIZABETH
REAL(EB):: Y_EQUIL_MIN
! NB : must be optimized by pointing to work target
REAL(EB), DIMENSION(:,:,:), ALLOCATABLE :: Q_CON_GAS_SUM, Q_EVAP_SUM
...
! Elizabeth (could disappear with optimization)
ALLOCATE(Q_CON_GAS_SUM(1:IBAR,1:JBAR,1:KBAR))
ALLOCATE(Q_EVAP_SUM(1:IBAR,1:JBAR,1:KBAR))
Y_EQUIL_MIN= 1._EB
...
SPECIES_LOOP: DO Z_INDEX = 1,N_TRACKED_SPECIES
   ...
   PARTICLE_LOOP: DO I=1,NLP
     ...
     !DT_SUM = 0._EB
      ...
      !TIME_ITERATION_LOOP: DO WHILE (DT_SUM < DT)
		 ...
		 ! Elizabeth keep track of total energy for evaporating particles in cell ! must be corrected (Jason' email)
		 Q_CON_GAS_SUM(II,JJ,KK) = Q_CON_GAS_SUM(II,JJ,KK) + WGT*Q_CON_GAS
		 Q_EVAP_SUM(II,JJ,KK) = Q_EVAP_SUM(II,JJ,KK) + WGT*M_VAP*H_V
         ...
         X_EQUIL  = MIN(1._EB,EXP(DHOR*(1._EB/TMP_BOIL-1._EB/MIN(TMP_DROP_NEW,TMP_BOIL))))
         Y_EQUIL = X_EQUIL/(MW_RATIO + (1._EB-MW_RATIO)*X_EQUIL)
		 Y_EQUIL_MIN = MIN(Y_EQUIL_MIN,Y_EQUIL)
   ...
   !New structure Elizabeth 
   ! Update gas temperature and determine new subtimestep

   DO KK=1,KBAR
      DO JJ=1,JBAR
         DO II=1,IBAR
			IF ((Q_CON_GAS_SUM(II,JJ,KK) .NE. 0._EB) .AND. (Q_EVAP_SUM(II,JJ,KK) .NE. 0._EB)) THEN
				...
				DELTA_H_G = -Q_CON_GAS_SUM(II,JJ,KK) + Q_EVAP_SUM(II,JJ,KK)
				...
				! Limit super-saturation ! keep temporarely
				...
				D_LAGRANGIAN(II,JJ,KK) = D_LAGRANGIAN(II,JJ,KK) + &
				(MW_RATIO*MVAP_TOT(II,JJ,KK)/M_GAS + (MVAP_TOT(II,JJ,KK)*DELTA_H_G - &
				Q_CON_GAS_SUM(II,JJ,KK))/H_G_OLD) !* WGT / DT_SUBSTEP
					
				Q_DOT(7,NM) = Q_DOT(7,NM) + (MVAP_TOT(II,JJ,KK)*H_S_B - Q_CON_GAS_SUM(II,JJ,KK)) !*WGT/DT_SUBSTEP ! need to be checked
				!Q_DOT(7,NM) = Q_DOT(7,NM) + (M_VAP*H_S_B - Q_CON_GAS) !*WGT/DT_SUBSTEP
...
! Elizabeth ! could disappear afetr optimization

DEALLOCATE(Q_CON_GAS_SUM)
DEALLOCATE(Q_EVAP_SUM)
...
END SUBROUTINE PARTICLE_MASS_ENERGY_TRANSFER_ELIZ
