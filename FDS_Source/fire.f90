...
!   TMP_MIXED_ZONE = TMP_FLAME(I,J,K)
!ELSE
   ...
   IF (N_REACTIONS > 1) THEN
      ...
      IF (ITER > 3) THEN
         REACTION_LOOP_TV: DO NR = 1, N_REACTIONS
            ...
            ENDDO
            IF (SUM(TV) > 0.0_EB .AND. SUM(TV) >= ABS(2.9_EB*SUM(ZZ_DIFF)) .AND. ITER >= TV_ITER_MIN) EXIT INTEGRATION_LOOP
...
DO NRR = 1,N_REACTIONS
   ...
   IF (.NOT. RNN%FAST_CHEMISTRY) THEN ! finite rate reactions
      ...
      DO NS = 0,N_TRACKED_SPECIES
         IF (RNN%NU(NS) < 0._EB) THEN
            DZ_FR(NRR) =(1._EB/RNN%EQBM_CONS)*MIN(DZ_FR(NRR),-ZZ_MIXED_FR(NS)/RNN%NU_MW_O_MW_F(NS)/DT_SUB)
         ENDIF
      ENDDO
   ELSE ! fast chemistry
      DZ_F(NRR) = 1.E10_EB
