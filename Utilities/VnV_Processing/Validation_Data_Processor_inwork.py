...
def extract_comp_data(comp_file_info):
    ...
    for scatter_label in combined_scatter_data_labels[0]:
        ...
        if comp_file_info['Quantity'] == str(0):
            ...
            # More statistical metrics can be set here with new 'if' statements.
            if  metric == 'max':
                if diagnostic_level >= 2:
                    print "*** Compute Rise ***"
                ...
                try:
                    metric_value = ((d2_rise_value-d1_rise_value)/d1_rise_value)
                    if diagnostic_level >= 2:
                        print "Rise Relative Difference is:", metric_value
                except:
                    print "!!! Computation of Rise metric_value failed. !!!\nCheck source data for columns listed above."
                    exit()
                ...
                scatter_data_dict[combined_scatter_data_labels[0][scatter_counter]] = [d1_rise_value,d2_rise_value,metric_value]
            elif metric == 'min':
                if diagnostic_level >= 2:
                    print "*** Compute Drop ***"
                temp_d1_data_values = [x for x in d1_data_values_comp if x != 'Null']
                ...
                try:
                    metric_value = ((d2_drop_value-d1_drop_value)/d1_drop_value)
                    if diagnostic_level >= 2:
                        print "Min Relative Difference is:", metric_value
                except:
                    print "!!! Computation of Min metric_value failed. !!!\nCheck source data for columns listed above."
                    exit()
                #Append Drop Values to Global Scatter Data Dictionary.
                scatter_data_dict[combined_scatter_data_labels[0][scatter_counter]] = [d1_drop_value,d2_drop_value,metric_value]
            else:
...
def comparison_plot(group_info,plot_data,d1_data,d2_data):
    ...
    # length of d1 and d2 lists can be different.
    if diagnostic_level >= 3:
        print "Data Set 1, Data:",d1_data
        print "Data Set 2, Data:",d2_data
    ...
    if len(d1_data) > 1: # or len(d2_data) > 1:
        ...
        # Loop through and plot Data Set 2, data
        ...
        # Handle single value d1_data or d2_data.
        if len(d1_data) == 1:
            if diagnostic_level >= 3:
                print "Data Set 1, Data to Plot:", d1_data[0]
            #Extract Dataset
            d1_data_item = d1_data[0]
            # Set Plot Key Text.
            d1_key = plot_data['d1_Key']
            #Get Plot Type for line or symbol definitions. 
            d1_plot_type = plot_data['d1_Plot_Type']
            
            # Check if line or symbol is requested.  A value of '0' means that a line is needed,
            # any number greater than 0 relates to a symbol as defined in the group 'g' section of the config file.
            if d1_plot_type == '0':
                # Plot Data Set 1, data
                g.plot(graph.data.points(d1_data_item, title=d1_key, x=ind_axis_index, y=dep_axis_index),
                    [d1PlotStyle])
            else:
                group_number = int(d1_plot_type)
                # Pull group symbol specifications from config file.
                config_group_symbol = group_info[group_number]["Symbol"]
                if diagnostic_level >= 3:
                    print "Group Symbol:", config_group_symbol
                group_symbol = "graph.style.symbol."+config_group_symbol
                
                config_group_symbol_color = group_info[group_number]["Color"]
                if diagnostic_level >= 3:
                    print "Group Symbol Color:", config_group_symbol_color
                config_group_symbol_filled = group_info[group_number]["Filled"]
                if diagnostic_level >= 3:
                    print "Group Symbol Filled:", config_group_symbol_filled
                ...
                #Create temporary symbol style.
                tempstyle = "graph.style.symbol("+group_symbol+", size=0.1*unit.v_cm, symbolattrs=["+fillstyle+"])"
                d1_plotstyle = eval(tempstyle)
                g.plot(graph.data.points(d1_data_item, title=d1_key, x=ind_axis_index, y=dep_axis_index),[d1_plotstyle])
                
        if len(d2_data) == 1:
            if diagnostic_level >= 3:
                print "Data Set 2, Data to Plot:", d2_data[0]
            #Extract Dataset
            d2_data_item = d2_data[0]
            # Set Plot Key Text.
            d2_key = plot_data['d2_Key']
            #Get Plot Type for line or symbol definitions. 
            d2_plot_type = plot_data['d2_Plot_Type']
            
            # Check if line or symbol is requested.  A value of '0' means that a line is needed,
            # any number greater than 0 relates to a symbol as defined in the group 'g' section of the config file.
            if d2_plot_type == '0':
                # Plot Data Set 2, data
                g.plot(graph.data.points(d2_data_item, title=d2_key, x=ind_axis_index, y=dep_axis_index),
                    [d2PlotStyle])
            else:
                group_number = int(d2_plot_type)
                # Pull group symbol specifications from config file.
                config_group_symbol = group_info[group_number]["Symbol"]
                if diagnostic_level >= 3:
                    print "Group Symbol:", config_group_symbol
                ...
                #Create temporary symbol style.
                tempstyle = "graph.style.symbol("+group_symbol+", size=0.1*unit.v_cm, symbolattrs=["+fillstyle+"])"
                d2_plotstyle = eval(tempstyle)
                g.plot(graph.data.points(d2_data_item, title=d2_key, x=ind_axis_index, y=dep_axis_index),[d2_plotstyle])
...
for data_record in group_quantity_data_dicts[2]:
    ...
    if process_set == 1 or process_set == 2:
        comparison_plot(group_quantity_data_dicts[0],group_quantity_data_dicts[2][data_record],d1_plot_data,d2_plot_data)
    d_count = d_count + 1
