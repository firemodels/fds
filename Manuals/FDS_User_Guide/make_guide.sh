...
if [[ `grep -E "Undefined control sequence|Error:|Fatal error|! LaTeX Error:|Paragraph ended before|Missing \\\$ inserted|Misplaced" -I FDS_User_Guide.err | grep -v "xpdf supports version 1.5"` == "" ]]
   then
      ...
      grep -A 10 -E "Undefined control sequence|Error:|Fatal error|! LaTeX Error:|Paragraph ended before|Missing \\\$ inserted|Misplaced" -I FDS_User_Guide.err | grep -v "xpdf supports version 1.5"
      clean_build=0
fi
