   ...
   DO N = 1,N_DEVC
      IF (ABS(DEVICE(N)%T_CHANGE-T) < TWO_EPSILON_EB) THEN
         ...
         WRITE(LU_DEVC_CTRL,'(ES12.5,",",A,",",A,",",L1,",",ES12.5,",",A)') & 
            DEVICE(N)%T_CHANGE,'DEVC',ID,DEVICE(N)%CURRENT_STATE,DEVICE(N)%SMOOTHED_VALUE,TRIM(DEVICE(N)%UNITS)
      ENDIF
   ENDDO
ENDIF
         ...
         SELECT CASE (CONTROL(N)%CONTROL_INDEX)
            CASE(1,4:99)
               WRITE(LU_DEVC_CTRL,'(ES12.5,",",A,",",A,",",L1)') & 
                  CONTROL(N)%T_CHANGE,'CTRL',ID,CONTROL(N)%CURRENT_STATE
            CASE(2,3,100:)
               WRITE(LU_DEVC_CTRL,'(ES12.5,",",A,",",A,",",L1,",",ES12.5)') & 
                  CONTROL(N)%T_CHANGE,'CTRL',ID,CONTROL(N)%CURRENT_STATE,CONTROL(N)%INSTANT_VALUE
         END SELECT
      ENDIF
   ENDDO
ENDIF
