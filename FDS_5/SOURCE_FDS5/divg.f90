...
SPECIES_LOOP: DO N=1,N_SPECIES
   ...
   WALL_LOOP: DO IW=1,NWC
      IF (BOUNDARY_TYPE(IW)/=SOLID_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=MIRROR_BOUNDARY) CYCLE WALL_LOOP
      IIG = IJKW(6,IW) 
      JJG = IJKW(7,IW) 
      KKG = IJKW(8,IW) 
   ...
   SPECIES_DIFFUSION: IF (.NOT.MIXTURE_FRACTION) THEN
      ...
      WALL_LOOP2: DO IW=1,NWC
         IF (BOUNDARY_TYPE(IW)/=SOLID_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=MIRROR_BOUNDARY) CYCLE WALL_LOOP2
         IIG = IJKW(6,IW)
         JJG = IJKW(7,IW)
         KKG = IJKW(8,IW)
         IOR  = IJKW(4,IW)
...
ENERGY: IF (.NOT.ISOTHERMAL) THEN
   ...
   CORRECTION_LOOP: DO IW=1,NWC
      IF (BOUNDARY_TYPE(IW)/=SOLID_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=MIRROR_BOUNDARY) CYCLE CORRECTION_LOOP
      II  = IJKW(1,IW) 
      JJ  = IJKW(2,IW) 
      KK  = IJKW(3,IW) 
      IIG = IJKW(6,IW) 
