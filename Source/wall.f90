...
CFACE_LOOP: DO ICF=INTERNAL_CFACE_CELLS_LB+1,INTERNAL_CFACE_CELLS_LB+N_INTERNAL_CFACE_CELLS
   ...
   IF (N_TRACKED_SPECIES>1) CALL CALCULATE_RHO_D_F(B1,BC)
...
IF (PRESENT(WALL_INDEX)) THEN
   ...
   SELECT CASE(BC%IOR)
      CASE(1,-1)
         ...
         VBAR = 0.5_EB*(VV(BC%IIS,BC%JJS,BC%KKS)+VV(BC%IIS,BC%JJS-1,BC%KKS))
         WBAR = 0.5_EB*(WW(BC%IIS,BC%JJS,BC%KKS)+WW(BC%IIS,BC%JJS,BC%KKS-1))
         B1%U_TANG_FS = SQRT(VBAR**2+WBAR**2)
      CASE(2,-2)
         UBAR = 0.5_EB*(UU(BC%IIG,BC%JJG,BC%KKG)+UU(BC%IIG-1,BC%JJG,BC%KKG)) - SF%VEL_T(1)*RAMP_FACTOR
         WBAR = 0.5_EB*(WW(BC%IIG,BC%JJG,BC%KKG)+WW(BC%IIG,BC%JJG,BC%KKG-1)) - SF%VEL_T(2)*RAMP_FACTOR
         B1%U_TANG = SQRT(UBAR**2+WBAR**2)
         UBAR = 0.5_EB*(UU(BC%IIS,BC%JJS,BC%KKS)+UU(BC%IIS-1,BC%JJS,BC%KKS))
         WBAR = 0.5_EB*(WW(BC%IIS,BC%JJS,BC%KKS)+WW(BC%IIS,BC%JJS,BC%KKS-1))
         B1%U_TANG_FS = SQRT(UBAR**2+WBAR**2)
      CASE(3,-3)
         UBAR = 0.5_EB*(UU(BC%IIG,BC%JJG,BC%KKG)+UU(BC%IIG-1,BC%JJG,BC%KKG)) - SF%VEL_T(1)*RAMP_FACTOR
         ...
         UBAR = 0.5_EB*(UU(BC%IIS,BC%JJS,BC%KKS)+UU(BC%IIS-1,BC%JJS,BC%KKS))
         VBAR = 0.5_EB*(VV(BC%IIS,BC%JJS,BC%KKS)+VV(BC%IIS,BC%JJS-1,BC%KKS))
         B1%U_TANG_FS = SQRT(UBAR**2+VBAR**2)
   END SELECT
...
B1%MU_G = MU(BC%IIG,BC%JJG,BC%KKG)
B1%RSUM_G = RSUM(BC%IIG,BC%JJG,BC%KKG)

B1%TMP_FS =  TMP(BC%IIS,BC%JJS,BC%KKS)
B1%RHO_FS =  RHO(BC%IIS,BC%JJS,BC%KKS)
...
REAL(EB) :: ARO,QNET,RAMP_FACTOR,RHO_G_2,RSUM_F,PBAR_F,TSI,UN, &
            ...
            BBB,CCC,PPP,QQQ,RRR,UUU,YYY,WWW,HTC_OLD,RSC_LOC,DTMP
...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (INTERPOLATED_BC) METHOD_OF_HEAT_TRANSFER  ! Only for EXTERNAL_WALL_CELLs
      ...
      IF (ATMOSPHERIC_INTERPOLATION) THEN
         ...
         CALL GET_SPECIFIC_GAS_CONSTANT(ZZ_GET,RSUM(BC%II,BC%JJ,BC%KK))
         DENOM = PBAR_G/B1%RHO_G/B1%RSUM_G + DDO*(PBAR_OTHER/RHO_OTHER/RSUM(BC%II,BC%JJ,BC%KK) - PBAR_G/B1%RHO_G/B1%RSUM_G)
                  ...
                  JJO_LOOP: DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
                     IIO_LOOP: DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
                        ...
                        MODE_SELECT: SELECT CASE(SIM_MODE)
                           CASE DEFAULT
                              RHO_D = MAX(0._EB, 0.5_EB*(MU_OTHER+B1%MU_G) )*RSC
                           CASE(DNS_MODE,LES_MODE)
                              D_Z_N = D_Z(:,N)
                              ...
                              IF (SIM_MODE==LES_MODE) THEN
                                 ...
                                 RHO_D_TURB = 0.5_EB*(MU_OTHER-MU_DNS_OTHER + B1%MU_G-MU_DNS_G)*RSC_LOC
                              ENDIF
...
SUBROUTINE CALCULATE_RHO_D_F(B1,BC)
...
SELECT CASE(SIM_MODE)
   CASE DEFAULT
      DO N=1,N_TRACKED_SPECIES
         B1%RHO_D_F(N) = B1%MU_G*RSC*B1%RHO_F/B1%RHO_G
      ENDDO
   CASE (LES_MODE)
      ...
      DO N=1,N_TRACKED_SPECIES
         RSC_LOC = RSC
         IF (SPECIES_MIXTURE(N)%SC_T_USER>TWO_EPSILON_EB) RSC_LOC=1._EB/SPECIES_MIXTURE(N)%SC_T_USER
         B1%RHO_D_F(N) = B1%RHO_F*( D_Z(ITMP,N) + (B1%MU_G-MU_DNS(BC%IIG,BC%JJG,BC%KKG))/B1%RHO_G*RSC_LOC )
      ENDDO
   CASE (DNS_MODE)
      ITMP = MIN(I_MAX_TEMP-1,NINT(B1%TMP_F))
      DO N=1,N_TRACKED_SPECIES
...
REAL(EB) :: U_THERM,U_TURB,Y_AEROSOL,ZZ_GET(1:N_TRACKED_SPECIES),YDEP,K_G,TMP_FILM,ALPHA,DTMPDX,&
            TAU_PLUS,U_GRAV,D_SOLID,MW_RATIO,KN,KN_FAC,ZZ_G(1:N_TRACKED_SPECIES),TAU_PLUS_C
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE(BOUNDARY_PROP1_TYPE), POINTER :: B1
TYPE(BOUNDARY_PROP2_TYPE), POINTER :: B2
...
SMIX_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   MW_RATIO = SPECIES_MIXTURE(N)%RCON/B1%RSUM_G
   TMP_FILM = 0.5_EB*(B1%TMP_G+B1%TMP_F)
   CALL GET_CONDUCTIVITY(ZZ_GET,K_G,TMP_FILM)
   KN_FAC = B1%MU_G*SQRT(0.5_EB*PI/(PBAR(BC%KKG,PRESSURE_ZONE(BC%IIG,BC%JJG,BC%KKG))*B1%RHO_G))
   ...
   IF (THERMOPHORETIC_DEPOSITION) THEN
      ...
      U_THERM = CS2*(ALPHA+CT*KN)*CUNNINGHAM(KN)/((1._EB+CM3*KN)*(1+2*ALPHA+CT2*KN)) * B1%MU_G/(B1%TMP_G*B1%RHO_G)*DTMPDX
   ENDIF

   IF (GRAVITATIONAL_DEPOSITION) THEN
      KN = KN_FAC/SM%MEAN_DIAMETER
      U_GRAV = - DOT_PRODUCT(GVEC,BC%NVEC)*CUNNINGHAM(KN)*SM%MEAN_DIAMETER**2*SM%DENSITY_SOLID/(18._EB*B1%MU_G)
   ...
   IF (TURBULENT_DEPOSITION) THEN
      ...
      TAU_PLUS = TAU_PLUS_C/B1%MU_G**2*B2%U_TAU**2*B1%RHO_G
      IF (TAU_PLUS < 0.2_EB) THEN ! Diffusion regime
         D_SOLID = K_BOLTZMANN*B1%TMP_G*CUNNINGHAM(KN)/(3._EB*PI*B1%MU_G*SM%MEAN_DIAMETER)
         U_TURB = B2%U_TAU * 0.086_EB*(B1%MU_G/B1%RHO_G/D_SOLID)**(-0.7_EB)
...
REAL(EB) :: REACTION_RATE,Y_O2,X_O2,MW(N_MATS),Y_GAS(N_MATS),Y_TMP(N_MATS),Y_SV(N_MATS),X_SV(N_MATS),X_L(N_MATS),&
            ...
            RHO_DOT,RHO_DOT_REAC(MAX_REACTIONS),RHO_DOT_REAC_SUM
...
REAL(EB) :: RE,H_NATURAL,H_FORCED,FRICTION_VELOCITY=0._EB,YPLUS=0._EB,ZSTAR,DN,CP_G,&
            R_DROP,CONV_LENGTH,GR,RA,NUSSELT_FORCED,NUSSELT_FREE,NUSSELT_IMPINGE,&
            FILM_FAC,PHI,C0_IMP,C1_IMP,C2_IMP,M_IMP,MU_G,K_G,TMP_FILM
...
TMP_FILM = 0.5_EB*(P1X%TMP_F+P1X%TMP_FS)
...
ELSE H_FIXED_IF
   ...
   HTC_TYPE_IF: IF ( (SIM_MODE==DNS_MODE .OR. SOLID_PHASE_ONLY) .AND. (PRESENT(WALL_INDEX_IN) .OR. PRESENT(CFACE_INDEX_IN)) ) THEN
      ...
      HEAT_TRANSFER_COEFFICIENT = 2._EB * K_G * P1X%RDN
      IF (SIM_MODE==DNS_MODE) RETURN

   ELSE HTC_TYPE_IF

      CALL GET_VISCOSITY(P1X%ZZ_G,MU_G,TMP_FILM)
      CALL GET_CONDUCTIVITY(P1X%ZZ_G,K_G,TMP_FILM)

      HTC_MODEL_SELECT: SELECT CASE(SFX%HEAT_TRANSFER_MODEL)
         CASE(DEFAULT_HTC_MODEL,IMPINGING_JET_HTC_MODEL)
            RE = P1X%RHO_FS*P1X%U_TANG_FS*CONV_LENGTH/MU_G
            GR = GRAV*ABS(DELTA_N_TMP)*CONV_LENGTH**3*(P1X%RHO_FS/MU_G)**2/P1X%TMP_FS
            ...
            IF (SFX%HEAT_TRANSFER_MODEL==IMPINGING_JET_HTC_MODEL) THEN
               ...
               RE = P1X%RHO_G*P1X%U_IMPACT*CONV_LENGTH/MU_G
         ...
         CASE(LOGLAW_HTC_MODEL)
            CALL GET_SPECIFIC_HEAT(P1X%ZZ_G,CP_G,P1X%TMP_G)
            FRICTION_VELOCITY = P2X%U_TAU
            YPLUS = P2X%Y_PLUS
            CALL LOGLAW_HEAT_FLUX_MODEL(H_FORCED,YPLUS,FRICTION_VELOCITY,K_G,P1X%RHO_G,CP_G,MU_G)
            HEAT_TRANSFER_COEFFICIENT = H_FORCED
         CASE(RAYLEIGH_HTC_MODEL)
            ! not appropriate for a particle, used with SURF and CFACE only
            CALL GET_SPECIFIC_HEAT(P1X%ZZ_G,CP_G,P1X%TMP_G)
            CALL RAYLEIGH_HEAT_FLUX_MODEL(H_NATURAL,ZSTAR,HTR,DN,P1X%TMP_F,P1X%TMP_G,K_G,P1X%RHO_G,CP_G,MU_G,&
                                          P1X%U_TANG,P1X%U_IMPACT)
            P2X%Z_STAR = ZSTAR
            P2X%HEAT_TRANSFER_REGIME = HTR
            HEAT_TRANSFER_COEFFICIENT = H_NATURAL
         CASE(FM_HTC_MODEL)
            CALL GET_SPECIFIC_HEAT(P1X%ZZ_G,CP_G,P1X%TMP_G)
            CALL FM_HEAT_FLUX_MODEL(H_NATURAL,DN,P1X%TMP_F,P1X%TMP_G,K_G,P1X%RHO_G,CP_G)
            HEAT_TRANSFER_COEFFICIENT = H_NATURAL
