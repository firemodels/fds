...
HEAT_FLUX_LOOP: DO IW=1,NWC
   ...
   IF (SF%THERMAL_BC_INDEX /= NO_CONVECTION .AND. SF%THERMAL_BC_INDEX /= INTERPOLATED_BC) THEN
      ITMP = MAX(500,NINT(0.05_EB*(TMP(IIG,JJG,KKG)+TMP_W(IW))))
      IF (N_SPECIES > 0) THEN
         YY_N = 0.5*(YYP(IIG,JJG,KKG,:)+YY_W(IW,:))
         CALL GET_CPBAR(YY_N,CP,ITMP)
      ELSE
         CP   = Y2CP_C(ITMP)
      ENDIF
   ENDIF
   METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
      ...
      CASE (SPECIFIED_TEMPERATURE) METHOD_OF_HEAT_TRANSFER
         ...
         RHOWAL    = 0.5_EB*(RHOP(IIG,JJG,KKG)+RHO_W(IW))
         IF (N_SPECIES > 0) THEN
            CALL GET_CPBAR(YY_N,CP_F,NINT(0.1_EB*TMP_F(IW)))
         ELSE
            CP_F   = Y2CP_C(NINT(0.1_EB*TMP_F(IW)))
         ENDIF
...
! Determine ghost cell value of RSUM=R0*Sum(Y_i/M_i) 
   IF (N_SPECIES>0) THEN
      CALL GET_MOLECULAR_WEIGHT(YY_W(IW,:),RSUM_W(IW))
...
REAL(EB) :: DTMP,QNETF,QDXKF,QDXKB,RR,TMP_G,T,RFACF,RFACB,RFACF2,RFACB2, &
            ...
            HVRG,Y_MF_G,Y_MF_W,YY_S, RSUM_W, RSUM_G, RSUM_S, MFLUX, MFLUX_S, VOLSUM, &
...
WALL_CELL_LOOP: DO IW=1,NWC+NVWC
   ...
   IF (N_SPECIES > 0) THEN
      YY_G(:) = 0.5_EB*(YY(IIG,JJG,KKG,:)+YY_W(IW,:))
      CALL GET_CPBAR(YY_G,CP,ITMP)
   ELSE
      CP = Y2CP_C(ITMP)
   ENDIF
...
!         IF (ML%N_REACTIONS.EQ.0 .OR. ANY(ML%NU_RESIDUE.GT.0._EB)) THEN
   ...
   MATERIAL_LOOP2: DO N=1,SF%N_MATL
      ...
      CALL GET_MOLECULAR_WEIGHT(YY(IIG,JJG,KKG,:),RSUM_G)
      ! wall values
      Y_MF_W = YY_W(IW,I_FUEL)
      CALL GET_MOLECULAR_WEIGHT(YY_W,RSUM_W)
