...
\item Max Gould of Reed College helped develop and produce the ``2D Vortex'' test case.
\end{itemize}
...
\begin{figure}[h!]
   ...
   \begin{picture}(7300,3420)
      ...
      \put(3450,-250){
         \scalebox{.498}{\includegraphics{FIGURES/vort2d_diagram}}
      }
   \end{picture}
...
\subsection{The FDS Moody Chart (\texorpdfstring{{\bf moody}}{moody}, \texorpdfstring{{\bf poiseuille}}{poiseuille}, and \texorpdfstring{{\bf z0}}{z0})}
\label{fds_moody_chart}

At a minimum, a wall model should accurately reproduce the mean wall stress for flow in a straight channel.  We verify that this is true for FDS by reproducing the Moody chart, a plot of friction factor versus Reynolds number for pipe flow \cite{Moody:1944}.
...
\end{equation}
where $\tau_{ij}^{r}$ is the deviatoric part of the residual stress tensor, here modeled by gradient diffusion with dynamic Smagorinsky \cite{Germano:1991} used for the eddy viscosity.  In this work we specify a constant pressure drop $\mbox{d}p/\mbox{d}x$ in the streamwise direction to drive the flow.  The hydrodynamic pressure $\tilde{p}$ is obtained from a Poisson equation which enforces (\ref{eqn_cont}).
