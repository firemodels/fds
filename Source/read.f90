...
IF (ANY(RN%SIMPLE_CHEMISTRY)) THEN
...
COUNT_SPEC_LOOP: DO
   ...
   IF (ID=='AIR' .AND. ANY(RN%SIMPLE_CHEMISTRY)) THEN
   ...
   IF (PREDEFINED_SMIX(1)) THEN
      DO NN=1,N_SPECIES-1
         IF (TRIM(PREDEFINED_SPEC_ID(NN))==TRIM(ID)) THEN
            !****** LOOPS for SIMPLE?
            IF (.NOT. ANY(RN%SIMPLE_CHEMISTRY)) THEN
...
!***** LOOP for SIMPLE
IF (ANY(RN%SIMPLE_CHEMISTRY) .AND. .NOT. (REAC_FUEL_READ .OR. SIMPLE_FUEL_DEFINED)) THEN
...
IF (.NOT. ANY(RN%SIMPLE_CHEMISTRY)) SM%ADJUST_NU = SUM(SM%VOLUME_FRACTION)
...
SELECT CASE(N)
   ...
   CASE(2)
      ...
      MASS_FRACTION(1) = 1._EB
   CASE(3)
      IF (REACION(1)%N_SIMPLE_CHEMISTRY_REACTIONS==1) RN=>REACTION(1)
      IF (REACION(1)%N_SIMPLE_CHEMISTRY_REACTIONS==2) RN=>REACTION(2)
      ID                 = 'PRODUCTS'
      FORMULA            = 'Z2'
