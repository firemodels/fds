...
ELSE FREEZE_VELOCITY_IF
   ...
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,USCARC_FLAG); CALL WALL_VELOCITY_NO_GRADH(DT,.FALSE.)
   END SELECT
...
ELSE FREEZE_VELOCITY_IF
   ...
   SELECT CASE(PRES_FLAG)
      CASE(GLMAT_FLAG,UGLMAT_FLAG,USCARC_FLAG)
...
PREDICTOR_COND : IF (PREDICTOR) THEN
   ...
   WALL_CELL_LOOP_1: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS_AUX
      ...
      IF (CC_UNSTRUCTURED_FDIV .AND. IW<=N_EXTERNAL_WALL_CELLS) THEN
         ...
         IF (ICF>0) THEN
            ONE_D => BOUNDARY_ONE_D(WC%OD_INDEX)
            DWSCL = SUM(CUT_FACE(ICF)%AREA(1:CUT_FACE(ICF)%NFACE))/ONE_D%AREA
         ENDIF
      ENDIF
