...
This chapter describes in detail the finite difference approximation of the mass and species conservation equations.
Due to the use of the low Mach number approximation, the energy conservation equation is defined implicitly via the divergence of the flow field, which contains the combustion and radiation source terms.

\section{Discretizing the Convective and Diffusive Transport Terms}
...
\subsection{Boundary Conditions for Density and Species Mass Fraction}

Gas phase temperatures are defined at cell centers; solid surfaces lie at the
interface of the gas and solid cell. The numerical expression for the heat lost to the boundary is
equated with the empirical convective heat transfer
\be k_{\hbox{\tiny LES}} \frac{T_g - T_s}{\ha \dn} = h \; (T_g-T_s)  \label{ebal} \ee
where $\ha \dn$ is the distance between the center of the gas cell
and the surface. The convective heat flux coefficient, $h$, is described in Section~\ref{conflux}.

At solid walls there is no transfer of mass, thus the boundary condition
for the species mixture $\alpha$ at a wall is simply
\be Z_{\alpha,s} = Z_{\alpha,gas} \ee
The species mixture mass fraction, like temperature, is defined at the cell center.
At forced flow boundaries, either the species mass fraction or
the mass flux may be specified.
For specified mass fraction, the boundary value for $Z_\alpha$ is as specified.
For specified mass flux, the following equation must be solved iteratively
\be \dot{m}_\alpha'' = u_n \rho_s Z_{\alpha,s}
  - \rho D_\alpha \frac{Z_{\alpha,g}-Z_{\alpha,s}}{\ha \dn} \ee
where $\dot{m}_\alpha''$ is the specified mass flux of species mixture $\alpha$,
$u_n$ is the normal component of velocity at the wall pointing into
the flow domain, and $\ha \dn$ is the distance between the center of the gas
cell and the wall. The reason for the iteration is that the density at the surface, $\rho_s$, is a function of
the species mixture at the surface.

Once the temperature and species mass fractions have been defined at the boundary, the density there is computed from the equation of
state
\be  \rho_s = \frac{\overline{p}_m}{ {\cal R} \, T_s \, \sum_\alpha (Z_{\alpha,s}/W_\alpha) }  \ee
