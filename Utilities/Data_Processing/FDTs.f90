...
REAL :: ACTIVATION_TEMPERATURE,A_C,ALPHA,AREA,A_T,A_V,C_I,CHI_R,CONDUIT_DIAMETER,CONDUIT_THICKNESS,CUTOFF_TIME,C_PL,C_CJ, &
        ...
        M_DOT,P,Q,Q_STAR,Q_STEP,R,RADIATIVE_FRACTION,RHO_AIR,RHO_I,RHO_S,RTI,SCALING_FACTOR,T,t_activation,T_END,T_P, &
...
DO I=1,9999
   ...
   DO J=1,20
      ...
      IF (STEEL_UNPROTECTED) THEN
         ...
         T_STEEL = 293
         DO M=2,TIME_RAMP(I)
            T_STEEL(M) = (F_V * (1/(RHO_S*C_STEEL)) * ((H_C * ((T_PLUME(J))-(T_STEEL(M-1)))) + ((SIGMA*1000) * EPSILON * ((T_PLUME(J))**4 - (T_STEEL(M-1))**4))) * DELTA_T) + (T_STEEL(M-1))
         ENDDO
