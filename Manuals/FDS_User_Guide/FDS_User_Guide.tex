...
&MESH ID='mesh6', IJK=..., XB=..., MPI_PROCESS=3 /
\end{lstlisting}
Notes: An unspecified value will result in the default number of threads (see \ref{single_mesh}) on the MPI process. Some cluster systems do not support an heterogenous distribution of resources, here computational cores, per MPI process. Therefore, the number of OpenMP threads may change, but not the amount of cores assigned to the MPI process.
...
be overriden by specifying them on the {\ct PART} line.
...
FDS creates a Lagrangian particle for the device that records the AST at a particular point with a particular {\ct ORIENTATION}. FDS does this by internally creating an {\ct INIT} line to position the particle. For other quantities, the new {\ct DEVC} line need the same ID of the {\ct INIT} line with {\ct INIT\_ID} as the {\ct DEVC} line for the {\ct ADIABATIC SURFACE TEMPERATURE}. For example, you can output the heat transfer coefficient using the following lines:  
\begin{lstlisting}
