...
DO KK=1,KBAR
   DO JJ=1,JBAR
      DA = R(II)*DY(JJ)*DZ(KK)
         ...
         ENDIF
         IF (WC%BOUNDARY_TYPE==SOLID_BOUNDARY .OR. WC%BOUNDARY_TYPE==NULL_BOUNDARY) B(NM) = B(NM) + WC%DUWDT*DA
   ...
   DO JJ=1,JBAR
      DA = R(II)*DY(JJ)*DZ(KK)
         ...
         IF (WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) THEN
            ...
            B(NM)         = B(NM) + DA*DUDT_AVG    
         ENDIF
         IF (WALL(IW)%BOUNDARY_TYPE==SOLID_BOUNDARY .OR. WALL(IW)%BOUNDARY_TYPE==NULL_BOUNDARY) B(NM) = B(NM) + WC%DUWDT*DA
