...
PRIVATE
PUBLIC INITIALIZE_LEVEL_SET_FIRESPREAD_1,INITIALIZE_LEVEL_SET_FIRESPREAD_2,LEVEL_SET_FIRESPREAD,UPDATE_AGL_TIMERS
INTEGER :: IZERO
INTEGER  :: LIMITER_LS
...
! Setup only what is needed if AGL_SLICE without a level set calculation
AGL_ONLY: IF (LEVEL_SET_MODE==0) THEN
...
ELSE AGL_ONLY
   ...
   ALLOCATE(M%PHI_LS(0:IBP1,0:JBP1)) ; CALL ChkMemErr('VEGE:LEVEL SET','PHI_LS',IZERO)
   PHI_LS  => M%PHI_LS  ; PHI_LS = PHI_LS_MIN
...
ENDIF
IF (AGL_TIMERS(2)) THEN
   ALLOCATE(M%T_RES(0:IBP1,0:JBP1),STAT=IZERO) ; CALL ChkMemErr('VEGE:T_RES','T_RES',IZERO)
   T_RES  => M%T_RES  ; T_RES = 0._EB
ENDIF
...
REAL(EB) :: PHI_LS_OTHER,U_LS_OTHER,V_LS_OTHER,Z_LS_OTHER,T_ARR_LS_OTHER,T_RES_LS_OTHER
TYPE (EXTERNAL_WALL_TYPE), POINTER :: EWC
LOGICAL :: SOLID_CELL,USE_OTHER=.FALSE.
...
V_LS_OTHER = 0._EB
Z_LS_OTHER = 0._EB
T_ARR_LS_OTHER = 0._EB
T_RES_LS_OTHER = 0._EB
...
AGL_ONLY: IF (LEVEL_SET_MODE==0) THEN
   ...
   DO JJO=EWC%JJO_MIN,EWC%JJO_MAX
      DO IIO=EWC%IIO_MIN,EWC%IIO_MAX
         ...
         IF (AGL_TIMERS(1)) T_ARR_LS_OTHER = T_ARR_LS_OTHER + OMESH(NOM)%T_ARR(IIO,JJO)
         IF (AGL_TIMERS(2)) T_RES_LS_OTHER = T_RES_LS_OTHER + OMESH(NOM)%T_RES(IIO,JJO)
      ENDDO
   ENDDO
   ...
   SELECT CASE(IOR)
      ...
      CASE(-3)  ! only grab a PHI_LS value from the other mesh if the (II,JJ) cell of the current mesh has no terrain surface
         ...
         ENDIF
         IF (.NOT.SOLID_CELL) USE_OTHER = .TRUE.
   END SELECT
   IF (USE_OTHER) THEN
      Z_LS(II,JJ)     = Z_LS_OTHER/REAL(N_INT_CELLS,EB)
      IF (AGL_TIMERS(1)) T_ARR(II,JJ) = T_ARR_LS_OTHER/REAL(N_INT_CELLS,EB)
      IF (AGL_TIMERS(2)) T_RES(II,JJ) = T_RES_LS_OTHER/REAL(N_INT_CELLS,EB)
   ENDIF
   ...
   SELECT CASE(IOR)
      ...
      CASE(3)  ! only grab a PHI_LS value from the other mesh if the (II,JJ) cell of the current mesh has no terrain surface
         ...
         ELSE
            IF (CELL(CELL_INDEX(II,JJ,KBAR))%SOLID) SOLID_CELL = .TRUE.
         ENDIF
         IF (.NOT.SURFACE(LS_SURF_INDEX(II,JJ))%VEG_LSET_SPREAD .AND. SOLID_CELL) USE_OTHER = .TRUE.
      CASE(-3)  ! only grab a PHI_LS value from the other mesh if the (II,JJ) cell of the current mesh has no terrain surface
         SOLID_CELL = .FALSE.
         IF (CC_IBM) THEN
            IF (CCVAR(II,JJ,1,CC_CGSC)==CC_SOLID .OR. CCVAR(II,JJ,1,CC_CGSC)==CC_CUTCFE) SOLID_CELL = .TRUE.
         ELSE
            IF (CELL(CELL_INDEX(II,JJ,1))%SOLID) SOLID_CELL = .TRUE.
         ENDIF
         IF (.NOT.SURFACE(LS_SURF_INDEX(II,JJ))%VEG_LSET_SPREAD .AND. .NOT.SOLID_CELL) USE_OTHER = .TRUE.
   END SELECT
   IF (USE_OTHER) THEN
...
!> \brief Compute a value for the arrival time (T_ARR) and residence time (T_RES)
!>
...
END SUBROUTINE UPDATE_AGL_TIMERS
