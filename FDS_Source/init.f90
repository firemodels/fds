...
IF (OBST_INDEX>0 .AND. ABS(SF%VOLUME_FLOW_RATE)>=TWO_EPSILON_EB .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   OBX=>M%OBSTRUCTION(OBST_INDEX)
   WC%UW0 = SF%VOLUME_FLOW_RATE / OBX%FDS_AREA(ABS(IOR))
ENDIF
...
PROCESS_VENT: IF (WC%VENT_INDEX>0) THEN
   ...
   IF (ABS(SF%VOLUME_FLOW_RATE)>=TWO_EPSILON_EB)   THEN
      WC%UW0 = SF%VOLUME_FLOW_RATE/VT%FDS_AREA
   ENDIF
