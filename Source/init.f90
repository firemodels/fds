...
USE MEMORY_FUNCTIONS, ONLY: COMPUTE_ONE_D_STORAGE_DIMENSIONS,&
                            ...
                            COMPUTE_BOUNDARY_PROPERTY_STORAGE_DIMENSIONS
...
M%N_BOUNDARY_COORD_DIM = 1000
M%N_BOUNDARY_PROPERTY_DIM = 1000
M%N_BOUNDARY_ONE_D_DIM = 1000

ALLOCATE(M%WALL(0:M%N_WALL_CELLS_DIM),STAT=IZERO) ; CALL ChkMemErr('INIT','WALL',IZERO)

ALLOCATE(M%BOUNDARY_COORD(0:M%N_BOUNDARY_COORD_DIM),STAT=IZERO) ; CALL ChkMemErr('INIT','WALL',IZERO)
ALLOCATE(M%BOUNDARY_PROPERTY(0:M%N_BOUNDARY_PROPERTY_DIM),STAT=IZERO) ; CALL ChkMemErr('INIT','WALL',IZERO)
ALLOCATE(M%BOUNDARY_ONE_D(0:M%N_BOUNDARY_ONE_D_DIM),STAT=IZERO) ; CALL ChkMemErr('INIT','WALL',IZERO)
...
WALL_LOOP_0: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (.NOT.DO_EVACUATION) ONE_D%PRESSURE_ZONE = M%PRESSURE_ZONE(BC%IIG,BC%JJG,BC%KKG)
...
BC => M%BOUNDARY_COORD(WC%BC_INDEX)
BP => M%BOUNDARY_PROPERTY(WC%BP_INDEX)
ONE_D => M%BOUNDARY_ONE_D(WC%OD_INDEX)
...
IF (DO_EVACUATION) ONE_D%U_NORMAL = 0._EB
...
IF (OBST_INDEX>0 .AND. .NOT.DO_EVACUATION) THEN
   ...
   IF (OBX%MASS>1.E5_EB) THEN
      OBX%MASS = SF%SURFACE_DENSITY*ONE_D%AREA*ONE_D%AREA_ADJUST
...
IF (REMOVE .AND. SURFACE(WC%SURF_INDEX)%PYROLYSIS_MODEL==PYROLYSIS_SPECIFIED) THEN
   ...
   IF (IW_OLD>0) THEN
      ONE_D => MESHES(NM)%BOUNDARY_ONE_D(WC%OD_INDEX)
      IF (WALL(IW_OLD)%SURF_INDEX==WC%SURF_INDEX) ONE_D%T_IGN = MESHES(NM)%BOUNDARY_ONE_D(WALL(IW_OLD)%OD_INDEX)%T_IGN
   ENDIF
ENDIF
