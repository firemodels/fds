...
METHOD_OF_HEAT_TRANSFER: SELECT CASE(SF%THERMAL_BC_INDEX)
   ...
   CASE (NO_CONVECTION) METHOD_OF_HEAT_TRANSFER
      ...
      B1%TMP_F  = B1%TMP_G
      B1%Q_RAD_OUT = SIGMA*B1%EMISSIVITY*B1%TMP_F**4
...
METHOD_OF_MASS_TRANSFER: SELECT CASE(SPECIES_BC_INDEX)
   ...
   CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
      ...
      IF (SF%REFERENCE_HEAT_FLUX > 0._EB .AND. B1%T_IGN <=T) THEN
         ...
         B1%Q_IN_SMOOTH = MIN(MAX(SF%MINIMUM_SCALING_HEAT_FLUX,B1%Q_IN_SMOOTH),SF%MAXIMUM_SCALING_HEAT_FLUX,B1%Q_IN_SMOOTH)
      ENDIF
...
IF (.NOT.SF%INTERNAL_RADIATION) B1%Q_RAD_OUT = B1%Q_RAD_OUT + &
...
IF (.NOT. SF%INTERNAL_RADIATION) B1%Q_RAD_OUT = B1%Q_RAD_OUT / DT_BC * SIGMA * B1%EMISSIVITY
