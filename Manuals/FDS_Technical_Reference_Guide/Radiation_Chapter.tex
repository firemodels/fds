...
\section{Absorption and Scattering of Thermal Radiation by Droplets}
...
The local absorption and scattering coefficients are in theory calculated
...
\end{align}
where $r$ is the droplet radius and $Q_a$ and $Q_s$ are absorption and
scattering efficiencies, respectively, given by Mie theory.
Based on \cite{Collin} and \cite{Maruyama}, the water fog inside a grid cell is modeled for radiation transport as a mono-disperse fog
whose diameter corresponds to the Sauter mean diameter of the poly-disperse spray.
...
These expressions are function of the total cross sectional area per unit volume of the droplets $A_d$. $A_d$ is computed simply by summing the cross sectional areas of all the droplets within a cell and divided by the cell volume.
For practical reasons, a relaxation factor of 0.5 is used to smooth slightly
the temporal variation of $A_d$.\\
...
look-up using the local mean droplet diameter.
