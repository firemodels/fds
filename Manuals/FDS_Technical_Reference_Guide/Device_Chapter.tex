...
\be
   \hbox{Obscuration}  = \left( 1 - e^{-K_m \rho Y_c l} \right) \times 100 \; \; \hbox{\% per length} \; l
\ee
...
The SFPE Handbook~\cite{SFPE} has references to various works on smoke detection and suggested values for the
...
the path:  
\be
  \hbox{Obscuration}  = \left(1 - e^{-K_m \int \rho Y_s \, \d l }  \right) \times 100  \; \; \hbox{\%}
\ee
