...
N_TOTAL_WALL_CELLS = N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS + N_GHOST_WALL_CELLS
WRITE(LU_CORE(NM)) U
WRITE(LU_CORE(NM)) V
WRITE(LU_CORE(NM)) W
WRITE(LU_CORE(NM)) D
...
!DO IW=1,N_EXTERNAL_WALL_CELLS + N_INTERNAL_WALL_CELLS + N_GHOST_WALL_CELLS
...
INTEGER :: NOM,N,N_T_E_MAX,N_TOTAL_WALL_CELLS
INTEGER, INTENT(IN) :: NM
LOGICAL :: EX
CHARACTER(100) :: MESSAGE
TYPE(OMESH_TYPE), POINTER :: M2=>NULL()
      ...
      ELSE SOLID_STATS
         ...
         WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
            ...
            IF (DV%SURF_ID=='null' .OR. SURFACE(SURF_INDEX)%ID==DV%SURF_ID) THEN
               ...
               SELECT CASE(DV%STATISTICS)
                  CASE('MAX')
                     STAT_VALUE = MAX(STAT_VALUE,SOLID_PHASE_OUTPUT(NM,ABS(DV%OUTPUT_INDEX),DV%Z_INDEX,&
                                                                    DV%PART_INDEX,OPT_WALL_INDEX=IW))
                  CASE('MIN')
                     STAT_VALUE = MIN(STAT_VALUE,SOLID_PHASE_OUTPUT(NM,ABS(DV%OUTPUT_INDEX),DV%Z_INDEX,&
                                                                    DV%PART_INDEX,OPT_WALL_INDEX=IW))
                  CASE('MEAN')
                     STAT_VALUE = STAT_VALUE + SOLID_PHASE_OUTPUT(NM,ABS(DV%OUTPUT_INDEX),DV%Z_INDEX,DV%PART_INDEX,&
                                                                  OPT_WALL_INDEX=IW)
                     STAT_COUNT = STAT_COUNT + 1
                  CASE('SURFACE INTEGRAL')
                     STAT_VALUE = STAT_VALUE + SOLID_PHASE_OUTPUT(NM,ABS(DV%OUTPUT_INDEX),DV%Z_INDEX,DV%PART_INDEX,&
                                                                  OPT_WALL_INDEX=IW)*WALL(IW)%AW 
               END SELECT
            ENDIF
