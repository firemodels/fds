...
INTEGER :: CVODE_TASK, NS, NTRY, MAXTRY, CVODE_FILE_UNIT
CHARACTER(LEN=256) :: OUTFILENAME
CHARACTER(LEN=20)  :: STR_EQUIV, STR_TAU_MIX, STR_ZETA_0
REAL(EB) :: H_G
...
ELSE IF(CVODE_TASK == CV_ONE_STEP) THEN
   ...
   ! This WRITE_SUBSTEPS is only true for a verification case.
   IF (WRITE_SUBSTEPS) THEN
      ...
      WRITE(STR_EQUIV, '(F6.4)') EQUIV
      WRITE(STR_TAU_MIX, '(F6.4)') TAU_MIX
      WRITE(STR_ZETA_0, '(F4.2)') ZETA0
      OUTFILENAME = 'CVODE_MIXING_SUBSTEPS' // '_PHI_' // TRIM(ADJUSTL(STR_EQUIV)) // &
                        '_TAUMIX_' // TRIM(ADJUSTL(STR_TAU_MIX)) // &
                        '_ZETA0_' // TRIM(ADJUSTL(STR_ZETA_0)) //'.CSV'
      CVODE_FILE_UNIT=1101
      OPEN(UNIT=CVODE_FILE_UNIT, FILE=OUTFILENAME, STATUS="REPLACE", ACTION="WRITE")
      WRITE(CVODE_FILE_UNIT, '(A)', ADVANCE="NO") "TIME,TMP,PR,ENTHALPY,"
      DO NS = 1, N_TRACKED_SPECIES
         IF (NS < N_TRACKED_SPECIES) THEN
            WRITE(CVODE_FILE_UNIT, '(A, A)', ADVANCE="NO") TRIM(SPECIES_MIXTURE(NS)%ID), ","
         ELSE
            WRITE(CVODE_FILE_UNIT, '(A)') TRIM(SPECIES_MIXTURE(NS)%ID)
         ENDIF
      ENDDO
   ENDIF
   ...
   DO WHILE (TCUR_C(1) < TEND)
      ...
      IF (WRITE_SUBSTEPS) THEN
         ...
         WRITE(CVODE_FILE_UNIT, '(4(F18.5, A))', ADVANCE="NO") TCUR_C(1)*1000, ",", CVEC_C(N_TRACKED_SPECIES+1), ",", & 
                              CVEC_C(N_TRACKED_SPECIES+2), ", ",H_G, ", "
         DO NS = 1, N_TRACKED_SPECIES
            IF (NS < N_TRACKED_SPECIES) THEN
               WRITE(CVODE_FILE_UNIT, '(F12.5, A)', ADVANCE="NO") ZZ(NS), ","
            ELSE
               WRITE(CVODE_FILE_UNIT, '(F12.5)') ZZ(NS)  ! Last value moves to new line
            ENDIF   
         ENDDO
      ENDIF  
