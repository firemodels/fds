...
HVAC_IN_SURF_INDEX           = N_SURF + 5
HVAC_OUT_SURF_INDEX          = N_SURF + 6
MASSLESS_TRACER_SURF_INDEX   = N_SURF + 7
DROPLET_SURF_INDEX           = N_SURF + 8
VEGETATION_SURF_INDEX        = N_SURF + 9
EVACUATION_SURF_INDEX        = N_SURF + 10
MASSLESS_TARGET_SURF_INDEX   = N_SURF + 11
...
SURFACE(HVAC_IN_SURF_INDEX)%ID           = 'HVAC'
SURFACE(HVAC_OUT_SURF_INDEX)%ID          = 'HVAC2'
...
MESH_LOOP_1: DO NM=1,NMESHES
   ...
   COUNT_VENT_LOOP: DO
      ...
      N_VENT_NEW = 0
      IF (SURF_ID=='HVAC2') THEN
            WRITE(MESSAGE,'(A,I4,A,I4)') 'ERROR: Cannot use HVAC2 as a SURF_ID for a VENT, VENT ', N_VENT+1,&
                                         ', line number',INPUT_FILE_LINE_NUMBER
            CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (MULT_ID=='null') THEN
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               IF (VT%SURF_INDEX==HVAC_IN_SURF_INDEX)  VT%BOUNDARY_TYPE = HVAC_BOUNDARY
               VT%IOR = IOR
               VT%ORDINAL = NN
...
MESH_LOOP_2: DO NM=1,NMESHES
   ...
   VENT_LOOP_3: DO N=1,N_VENT
      ...
      IF (VT%SURF_INDEX==HVAC_IN_SURF_INDEX .AND. N>1) THEN
         DO NN=1,N-1
            IF (TRIM(VT%ID)==TRIM(VENTS(NN)%ID) .AND. VENTS(NN)%SURF_INDEX==HVAC_IN_SURF_INDEX) THEN
