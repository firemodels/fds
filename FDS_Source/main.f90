...
INTEGER :: N_REQ,N_PREQ,N_COMMUNICATIONS,INTEGER_SEND_BUFFER(7),INTEGER_RECV_BUFFER(7)
...
WRITE(VERSION_STRING,'(A)') 'FDS 6, Release Candidate 2' 
...
OTHER_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF (CODE==3 .AND. M3%NIC_S>0) THEN
      N_REQ = MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%REAL_RECV_PKG2(1),SIZE(M3%REAL_RECV_PKG2),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==4 .AND. M3%NIC_S>0) THEN
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%REAL_RECV_PKG3(1),SIZE(M3%REAL_RECV_PKG3),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==0 .OR. CODE==6) THEN
      N_REQ = MIN(N_REQ+1,SIZE(REQ))      
      CALL MPI_IRECV(M3%BOUNDARY_TYPE(0),M4%N_EXTERNAL_WALL_CELLS+1,MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD, REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==6 .AND. M3%NIC_S>0) THEN
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%REAL_RECV_PKG4(1),SIZE(M3%REAL_RECV_PKG4),MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==6 .AND. EXCHANGE_RADIATION .AND. M3%NIC_S>0) THEN
      ...
      IWW = M3%NIC_S
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%REAL_RECV_PKG5(1),(NRA*NSB+1)*IWW+1,MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
   ...
   IF (CODE==7 .AND. OMESH_PARTICLES .AND. (M3%NIC_S>0 .OR. M3%NIC_R>0)) THEN
      N_REQ=MIN(N_REQ+1,SIZE(REQ))
      CALL MPI_IRECV(M3%N_PART_ADOPT,N_LAGRANGIAN_CLASSES,MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
   ENDIF
...
SENDING_MESH_LOOP: DO NM=1,NMESHES
   ...
   RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
      ...
      INITIALIZE_SEND_IF: IF (CODE==0) THEN
         ...
         IF (RNODE/=SNODE) THEN
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M%OMESH(NM)%IJKW(1,1),15*M%N_EXTERNAL_WALL_CELLS, MPI_INTEGER,SNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
      ...
      IF (CODE==1 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG1(1),SIZE(M3%REAL_SEND_PKG1),MPI_DOUBLE_PRECISION,SNODE,NM,MPI_COMM_WORLD,&
                           REQ(N_REQ),IERR)
         ELSE
      ...
      IF (CODE==3 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG2(1),SIZE(M3%REAL_SEND_PKG2),MPI_DOUBLE_PRECISION,SNODE,NM, &
      ...
      IF (CODE==4 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG3(1),SIZE(M3%REAL_SEND_PKG3),MPI_DOUBLE_PRECISION,SNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      IF (CODE==0 .OR. CODE==6) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            CALL MPI_ISEND(M5%BOUNDARY_TYPE(0),M%N_EXTERNAL_WALL_CELLS+1,MPI_INTEGER,SNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      IF (CODE==6 .AND. M3%NIC_R>0) THEN
         IF (RNODE/=SNODE) THEN
            ...
            ENDDO
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG4(1),SIZE(M3%REAL_SEND_PKG4),MPI_DOUBLE_PRECISION,SNODE,NM,MPI_COMM_WORLD, &
                           REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
      ...
      SEND_RADIATION: IF ( CODE==6 .AND. M3%NIC_R>0 .AND. EXCHANGE_RADIATION) THEN
         ...
         IF (RNODE/=SNODE) THEN
            ...
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%REAL_SEND_PKG5(1),(NRA*NSB+1)*IWW+1,MPI_DOUBLE_PRECISION,SNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
      ...
      IF (CODE==7 .AND. OMESH_PARTICLES) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=MIN(N_REQ+1,SIZE(REQ))
            CALL MPI_ISEND(M3%N_PART_ORPHANS,N_LAGRANGIAN_CLASSES,MPI_INTEGER,SNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
