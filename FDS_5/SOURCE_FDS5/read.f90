...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,RELAXATION_FACTOR,FYI, &
                ...
                FLUX_LIMITER, FREEZE_VELOCITY, ADJUST_HOR
...
NAMELIST /DUMP/ RENDER_FILE,SMOKE3D,SMOKE3D_COMPRESSION,SMOKE3D_QUANTITY,FLUSH_FILE_BUFFERS,MASS_FILE,STATE_FILE, &
                ...
                PLOT3D_QUANTITY,UL_PAN_DATA,STATUS_FILES,CHECK_BOUNDEDNESS
...
REAL(EB) :: ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
            BEAD_DIAMETER,BEAD_EMISSIVITY,CABLE_MASS_PER_LENGTH,CABLE_DIAMETER,CABLE_JACKET_THICKNESS,CABLE_FAILURE_TEMPERATURE, &
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
                ...
                CABLE_MASS_PER_LENGTH,CABLE_DIAMETER,CABLE_JACKET_THICKNESS,CABLE_FAILURE_TEMPERATURE, &
                C_FACTOR,CHARACTERISTIC_VELOCITY, &
...
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX
USE DEVICE_VARIABLES, ONLY : CABLE,N_CABL,PROPERTY
...
ALLOCATE(MATERIAL(1:N_MATL+N_CABL),STAT=IZERO)
...
READ_SURF_LOOP: DO N=0,N_SURF
   ...
   IF (SF%CABL_INDEX>0) THEN
      ...
      THICKNESS(1)   = 0.5_EB*PROPERTY(SF%PROP_INDEX)%CABLE_DIAMETER
      STRETCH_FACTOR = 1._EB
      MATL_ID(1,1)   = PROPERTY(SF%PROP_INDEX)%ID
   ENDIF
...
INTEGER  :: N,NN,NM,MESH_NUMBER,N_DEVCO,IOR,TRIP_DIRECTION,PDPA_M,PDPA_N,PDPA_GROUP
REAL(EB) :: DEPTH,ORIENTATION(3),ROTATION,SETPOINT,FLOWRATE,BYPASS_FLOWRATE,DELAY,XYZ(3),PDPA_RADIUS,PDPA_START,PDPA_END
CHARACTER(30) :: QUANTITY,PROP_ID,CTRL_ID,DEVC_ID,SURF_ID,STATISTICS,PART_ID
LOGICAL :: INITIAL_STATE,LATCH,PDPA_U,PDPA_V,PDPA_W,PDPA_COUNT
...
NAMELIST /DEVC/ DEPTH,FYI,IOR,ID,ORIENTATION,PROP_ID,QUANTITY,ROTATION,XB,XYZ,INITIAL_STATE,LATCH,TRIP_DIRECTION,CTRL_ID,& 
                ...
                PART_ID,PDPA_START,PDPA_END,PDPA_RADIUS,PDPA_U,PDPA_V,PDPA_W,PDPA_COUNT,PDPA_M,PDPA_N,PDPA_GROUP
...
READ_DEVC_LOOP: DO NN=1,N_DEVCO
   ...
   DV%Y                = XYZ(2)
   DV%Z                = XYZ(3)
   DV%PDPA_START       = PDPA_START
   DV%PDPA_END         = PDPA_END
   DV%PDPA_RADIUS      = PDPA_RADIUS
   DV%PDPA_GROUP       = PDPA_GROUP
   DV%PDPA_COUNT       = PDPA_COUNT
   DV%PDPA_M           = PDPA_M
   DV%PDPA_N           = PDPA_N
   DV%PDPA_U           = PDPA_U
   DV%PDPA_V           = PDPA_V
   ...
   SPECIAL_QUANTITIES: SELECT CASE (DV%QUANTITY)
      ...
      CASE ('CABLE TEMPERATURE')
         ...
         IF (DV%PROP_INDEX<1) THEN
            ...
            CALL SHUTDOWN(MESSAGE)
         ENDIF
         DV%SETPOINT = PROPERTY(DV%PROP_INDEX)%CABLE_FAILURE_TEMPERATURE
         DV%TMP_L    = PROPERTY(DV%PROP_INDEX)%INITIAL_TEMPERATURE
         GAS_CELL_RAD_FLUX   = .TRUE.
         N_GAS_CELL_RAD_DEVC = N_GAS_CELL_RAD_DEVC + 1
         DV%GAS_CELL_RAD_FLUX = .TRUE.
         DV%DEPTH             = PROPERTY(DV%PROP_INDEX)%CABLE_JACKET_THICKNESS
         DO NNN=1,N_SURF
            IF (SURF_NAME(NNN)==PROPERTY(DV%PROP_INDEX)%ID) DV%SURF_INDEX = NNN
         ENDDO
...
DO N = 1, N_EVAP_INDICIES
   ...
   ! Special droplet statistics for DEVC only
