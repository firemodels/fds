...
REAL(EB), INTENT(IN) :: T,DT
INTEGER :: NM,II,JJ,KK
CHARACTER(40) :: STEP_STRING,SIM_TIME_STRING,CPU_TIME_STRING,TIME_STEP_STRING
...
REAL(EB),ALLOCATABLE, DIMENSION(:) :: CONST,CUMSUM,COORD_FACTOR
...
IF (N_PDPA_HISTOGRAM>0) THEN
   ...
   ALLOCATE(CONST(N_PDPA_HISTOGRAM),CUMSUM(N_PDPA_HISTOGRAM),COORD_FACTOR(N_PDPA_HISTOGRAM))
   CUMSUM(:)=0._EB
   ...
   DO J=1,N_PDPA_HISTOGRAM
      ...
      IF (PY%PDPA_HISTOGRAM) THEN
         ...
         CONST(N)=SUM(DV%PDPA_HISTOGRAM_COUNTS(:))
         SELECT CASE(PY%QUANTITY)
         CASE('DIAMETER') ! m -> mu-m
            COORD_FACTOR(N)=1E6_EB
         CASE DEFAULT
            COORD_FACTOR(N)=1
         END SELECT
      ENDIF
   ...
   DO J=1,N_DEVC
      ...
      IF (PY%PDPA_HISTOGRAM) THEN
         ...
         DO NN =1,MAX_PDPA_HISTOGRAM_NBINS
            IF(NN>PY%PDPA_HISTOGRAM_NBINS) CYCLE
            CUMSUM(N) = CUMSUM(N) + DV%PDPA_HISTOGRAM_COUNTS(NN)
            VALUE = DV%PDPA_HISTOGRAM_COUNTS(NN)
            DI=PY%PDPA_HISTOGRAM_LIMITS(1)+(REAL(NN,EB)-0.5_EB)*DD
            IF(PY%PDPA_HISTOGRAM_CUMULATIVE) VALUE = CUMSUM(N)
            IF(PY%PDPA_NORMALIZE) VALUE = VALUE / CONST(N)
            WRITE(TCFORM,'(5A)') "(1(",FMT_R,",A),",FMT_R,")"
            WRITE(PDPA_HISTOGRAM_VALUE(N,NN),TCFORM) DI*COORD_FACTOR(N),',',VALUE
         ENDDO
      ENDIF
   ENDDO
