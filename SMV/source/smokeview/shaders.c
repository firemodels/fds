...
int setZoneSmokeShaders() {
  ...
  const GLchar *FragmentShaderSource[]={
    ...
    "  vec3 dboxmin,dboxmax,deyepos;"
    "  L=distance(fragpos,eyepos)*xyzmaxdiff;"
    "  alpha_min=1000000.0;"
    "  deyepos=eyepos-fragpos;"
    "  dboxmin=boxmin-fragpos;"
    "  dboxmax=boxmax-fragpos;"
    "  alpha_zlay = -(zlay-fragpos.z)/deyepos.z;"
    "  if(zoneinside==0){"
    "    if(zonedir!=-1){"
    "      alpha = -dboxmin.x/deyepos.x;"
    "      if(alpha>0.0&&alpha<alpha_min){"
    "        alpha_min=alpha;"
    "      }"
    "    }"
    "    if(zonedir!=1){"
    "      alpha = -dboxmax.x/deyepos.x;"
    "      if(alpha>0.0&&alpha<alpha_min){"
    "        alpha_min=alpha;"
    "      }"
    "    }"
    "    if(zonedir!=-2){"
    "      alpha = -dboxmin.y/deyepos.y;"
    "      if(alpha>0.0&&alpha<alpha_min){"
    "        alpha_min=alpha;"
    "      }"
    "    }"
    "    if(zonedir!=2){"
    "      alpha = -dboxmax.y/deyepos.y;"
    "      if(alpha>0.0&&alpha<alpha_min){"
    "        alpha_min=alpha;"
    "      }"
    "    }"
    "    if(zonedir!=-3){"
    "      alpha = -dboxmin.z/deyepos.z;"
    "      if(alpha>0.0&&alpha<alpha_min){"
    "        alpha_min=alpha;"
    "      }"
    "    }"
    "    if(zonedir!=3){"
    "      alpha = -dboxmax.z/deyepos.z;"
    "      if(alpha>0.0&&alpha<alpha_min){"
    "        alpha_min=alpha;"
    "      }"
    "    }"
