...
SPRINKLER_INSERT_LOOP: DO KS=1,N_DEVC
   ...
   PARTICLE_INSERT_LOOP: DO I=1,N_INSERT
      ...
      CALL ALLOCATE_STORAGE(NM,LP_INDEX=NLP,LPC_INDEX=PY%PART_INDEX)
...
ILPC_IF: IF (ILPC > 0) THEN
   ...
   RF_IF: IF (.NOT. RETURN_FLAG) THEN
      ...
      PARTICLE_INSERT_LOOP2: DO I=1,SF%NPPC
         ...
         CALL ALLOCATE_STORAGE(NM,LP_INDEX=NLP,LPC_INDEX=ILPC)
...
N_LPC_IF: IF (SF%N_LPC>0) THEN
   ...
   LPC_LOOP: DO N_LPC = 1, SF%N_LPC
      ...
      PARTICLE_INSERT_LOOP2A: DO I=1,SF%NPPC
         ...
         CALL ALLOCATE_STORAGE(NM,LP_INDEX=NLP,LPC_INDEX=SF%MATL_PART_INDEX(N_LPC))
...
INTEGER :: N,I
TYPE(ONE_D_M_AND_E_XFER_TYPE), POINTER :: LP_ONE_D
...
REAL(EB) :: UBAR,VBAR,WBAR,RVC,UREL,VREL,WREL,QREL,RHO_G,TMP_G,MU_FILM, &
            ...
            GX_LOC,GY_LOC,GZ_LOC,DRAG_MAX(3)=0._EB,SUM_RHO,K_SGS,U_P,KN
REAL(EB), SAVE :: BETA
...
TYPE(ONE_D_M_AND_E_XFER_TYPE), POINTER :: ONE_D,LP_ONE_D
TYPE(BOUNDARY_PROPERTY_TYPE), POINTER :: BP
TYPE(BOUNDARY_COORD_TYPE), POINTER :: BC
TYPE (SURFACE_TYPE), POINTER :: SF
...
PARTICLE_LOOP: DO IP=1,NLP
   ...
   WEED_LOOP: DO
      ...
      IF (LPC%SOLID_PARTICLE .AND. SF%THERMAL_BC_INDEX==THERMALLY_THICK .AND. ONE_D%BURNAWAY) THEN
         CALL PARTICLE_ORPHANAGE
         CYCLE WEED_LOOP
      ENDIF

      IF (LPC%LIQUID_DROPLET .AND. ONE_D%X(1)<=LPC%KILL_RADIUS) THEN
         CALL PARTICLE_ORPHANAGE
         CYCLE WEED_LOOP
      ENDIF
...
INTEGER :: MM,NOM,N_NEW_STORAGE_SLOTS,BC_INDEX_SAVE,OD_INDEX_SAVE
...
IF (NOM/=0 .AND. NOM/=NM) THEN
   ...
   CALL PACK_PARTICLE(NM,IP,LP%CLASS_INDEX,OM%N_PART_ORPHANS(LP%CLASS_INDEX),.FALSE.,NOM=NOM)
...
IF (IP<NLP-IKILL) THEN
   BC_INDEX_SAVE = M%LAGRANGIAN_PARTICLE(IP)%BC_INDEX
   OD_INDEX_SAVE = M%LAGRANGIAN_PARTICLE(IP)%OD_INDEX
   M%LAGRANGIAN_PARTICLE(IP) = M%LAGRANGIAN_PARTICLE(NLP-IKILL)
   M%LAGRANGIAN_PARTICLE(IP)%LP_INDEX = IP
   M%LAGRANGIAN_PARTICLE(IP)%BC_INDEX = BC_INDEX_SAVE
   M%LAGRANGIAN_PARTICLE(IP)%OD_INDEX = OD_INDEX_SAVE
   M%BOUNDARY_COORD(BC_INDEX_SAVE) = M%BOUNDARY_COORD(M%LAGRANGIAN_PARTICLE(NLP-IKILL)%BC_INDEX)
   M%BOUNDARY_ONE_D(OD_INDEX_SAVE) = M%BOUNDARY_ONE_D(M%LAGRANGIAN_PARTICLE(NLP-IKILL)%OD_INDEX)
ENDIF
