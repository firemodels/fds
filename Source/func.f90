...
WC%VEG_HEIGHT         => OS%REALS( 6,STORAGE_INDEX)
WC%VEL_ERR_NEW        => OS%REALS( 7,STORAGE_INDEX) ; IF (NEW) WC%VEL_ERR_NEW = 0._EB
WC%V_DEP              => OS%REALS( 8,STORAGE_INDEX) ; IF (NEW) WC%V_DEP = 0._EB
WC%Q_LEAK             => OS%REALS( 9,STORAGE_INDEX) ; IF (NEW) WC%Q_LEAK = 0._EB
...
REAL (EB) :: X_SAT,X_H2O,H_V_B,H_V,DHOR
REAL(EB),PARAMETER :: T_BOIL=373.15_EB

H_V_B = H_V_H2O(373)+0.15_EB*(H_V_H2O(374)-H_V_H2O(373))

IF (TEMP >= T_BOIL) THEN
   H_V = H_V_B
   X_SAT = 1._EB
ELSE
   H_V = H_V_H2O(INT(TEMP))+(TEMP-INT(TEMP))*(H_V_H2O(INT(TEMP)+1)-H_V_H2O(INT(TEMP)))
   DHOR = 0.5_EB*(H_V_B+H_V)*MW_H2O/R0
   X_SAT  = MIN(1._EB,P_STP/PZONE*EXP(DHOR*(1._EB/T_BOIL-1._EB/TEMP)))
ENDIF
