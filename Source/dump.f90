...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   ELSEIF (IFRMT.EQ.1) THEN
      WRITE(FN_PART_VTK(N,NM),'(A,A,A,A,A,I0,A,I8.8,I2.2,A)') "",TRIM(RESULT_DIR)//TRIM(CHID),'_PART_',TRIM(LPC%ID),'_',&
                               NM,'_',ITM,ITM1,'.vtp'
      IF (VTK_BINARY) THEN
...
DATA_FILE_LOOP: DO N=1,N_SMOKE3D
   ...
   IF (VTK_OUT) THEN
      ...
      IF (ITM1==100) THEN
         ...
         ITM1 = 0
      ENDIF
      WRITE(FN_SMOKE3D_VTK(NM),'(A,A,A,I0,A,I8.8,I2.2,A)') "",TRIM(RESULT_DIR)//TRIM(CHID),'_SM3D_',NM,'_',ITM,ITM1,'.vtu'
      I2 = MESHES(NM)%IBAR
      J2 = MESHES(NM)%JBAR
      K2 = MESHES(NM)%KBAR
      NX = I2 + 1 - I1
   ...
   UNIQUE_LOOPA: DO II=1,MESHES(1)%N_UNIQUE_SLCF
      ...
      QUANTITY_LOOPA: DO IQ=1,NQT
         ...
         IF (.NOT.VTK_INITIALIZED) THEN
            WRITE(FN_SL3D_VTK(II,NM),'(A,A,A,A,I0,A,I8.8,I2.2,A)') TRIM(RESULT_DIR)//TRIM(CHID),'_',&
...
ELSEIF (IFRMT.EQ.1) THEN
   ...
   IF (ITM1==100) THEN
      ...
      ITM1 = 0
   ENDIF
   WRITE(FN_BNDF_VTK(NM),'(A,A,A,I0,A,I8.8,I2.2,A)') "",TRIM(RESULT_DIR)//TRIM(CHID),'_BNDF_',NM,'_',ITM,ITM1,'.vtu'
   IF (VTK_BINARY) THEN
