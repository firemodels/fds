...
INTEGER :: N,I,J,K,NN,IL,III,JJJ,KKK,IP,JP,KP,IW,FED_ACTIVITY,IP1,JP1,KP1,IM1,JM1,KM1,IIM1,JJM1,KKM1,NR,NS,RAM,&
...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(111:113)  ! ENTHALPY FLUX
      IP=II ; JP=JJ ; KP=KK
      SELECT CASE(IND)
         CASE(111) ! ENTHALPY FLUX X
            IP   = II+1
            VEL  = U(II,JJ,KK)
            R_DN = RDXN(II)
            IW   = MAX(WALL_INDEX(CELL_INDEX(II,JJ,KK),1),WALL_INDEX(CELL_INDEX(IP,JP,KP),-1))
         CASE(112) ! ENTHALPY FLUX Y
            JP   = JJ+1
            VEL  = V(II,JJ,KK)
            R_DN = RDYN(JJ)
            IW   = MAX(WALL_INDEX(CELL_INDEX(II,JJ,KK),2),WALL_INDEX(CELL_INDEX(IP,JP,KP),-2))
         CASE(113) ! ENTHALPY FLUX Z
            KP   = KK+1
            VEL  = W(II,JJ,KK)
            R_DN = RDZN(KK)
            IW   = MAX(WALL_INDEX(CELL_INDEX(II,JJ,KK),3),WALL_INDEX(CELL_INDEX(IP,JP,KP),-3))
      END SELECT

      IF (IW>0) THEN
         IF (WALL(IW)%BOUNDARY_TYPE==SOLID_BOUNDARY) THEN
            WRITE(MESSAGE,'(A)') "ERROR: Output QUANTITY 'ENTHALPY FLUX' not appropriate at solid boundary"
            CALL SHUTDOWN(MESSAGE,PROCESS_0_ONLY=.FALSE.)
          ENDIF
      ENDIF
