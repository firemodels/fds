...
!$OMP SHARED(N_EXTERNAL_WALL_CELLS,BXS,BXF,BYS,BYF,BZS,BZF,HX,HY,HZ,FVX,FVY,FVZ, &
!$OMP        IBP1,JBP1,KBP1, &
...
!$OMP PRIVATE(IW,WC,I,J,K,IOR,DUUDT,DVVDT,DWWDT,NOM,H_OTHER,KKO,JJO,IIO, &
!$OMP         N_INT_CELLS,DX_OTHER,DY_OTHER,DZ_OTHER,VT,TSI,TIME_RAMP_FACTOR,P_EXTERNAL,WALL) 
   ...
   IF_DIRICHLET: IF (WC%PRESSURE_BC_INDEX==DIRICHLET) THEN
      ...
      OPEN_IF: IF (WC%BOUNDARY_TYPE==OPEN_BOUNDARY) THEN
         ...
         IF (WC%VENT_INDEX>0) THEN
            ...
            IF (ABS(WC%ONE_D%T-T_BEGIN)<=TWO_EPSILON_EB .AND. VT%PRESSURE_RAMP_INDEX >=1) THEN
               TSI = T
            ELSE
               TSI = T - T_BEGIN
            ENDIF
