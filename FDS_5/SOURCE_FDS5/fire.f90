...
CONVERT_CO: IF (CO_PRODUCTION) THEN 
   ...
   Z_2_MIN_FAC = F_TO_CO * REACTION(1)%CO_YIELD / REACTION(1)%Y_F_INLET
   A  = RN%BOF 
   NODETS = 20
...
INTEGER :: II,JJ,KK,IIG,JJG,KKG,IW,IOR,NOM,IIO,JJO,KKO,N_INT_CELLS
REAL(EB) :: Q_OTHER
      ...
      DO KKO=IJKW(12,IW),IJKW(15,IW)
         DO JJO=IJKW(11,IW),IJKW(14,IW)
            DO IIO=IJKW(10,IW),IJKW(13,IW)
               Q_OTHER = Q_OTHER + MESHES(NOM)%Q(IIO,JJO,KKO)
            ENDDO
         ENDDO
      ENDDO
