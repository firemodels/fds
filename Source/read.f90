...
LOGICAL :: EX,THICKEN_OBSTRUCTIONS,BAD,IDEAL=.FALSE.,SIMPLE_FUEL_DEFINED=.FALSE.
...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                BAROCLINIC,BNDF_DEFAULT,CC_IBM,CHECK_MASS_CONSERVE,CCVOL_LINK,CNF_CUTOFF,CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,&
                ...
                COUPLED_1D3D_HEAT_TRANSFER,C_DEARDORFF,C_RNG,C_RNG_CUTOFF,C_SMAGORINSKY,C_VREMAN,&
                ...
                RADIATION,RESEARCH_MODE,RESTART,RESTART_CHID,RICHARDSON_ERROR_TOLERANCE,RUN_AVG_FAC,&
...
NAMELIST /DEVC/ BYPASS_FLOWRATE,CONVERSION_FACTOR,COORD_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,DUCT_ID,EVACUATION,FLOWRATE,FYI,&
                GHOST_CELL,HIDE_COORDINATES,ID,INITIAL_STATE,INIT_ID,IOR,LATCH,MATL_ID,NODE_ID, &
   ...
   QUANTITY_IF: IF (DV%QUANTITY /= 'null') THEN
      ...
      IF (DV%GHOST_CELL) THEN
         IF (DV%STATISTICS/='null') THEN
            WRITE(MESSAGE,'(A,A)') 'ERROR: Statistics not appropriate for GHOST_CELL DEVC ',TRIM(DV%ID)
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
         IF (ABS(DV%IOR)<=0) THEN
            WRITE(MESSAGE,'(A,A)') 'ERROR: GHOST_CELL requires IOR for DEVC ',TRIM(DV%ID)
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
      ENDIF
