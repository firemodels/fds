...
\huge \flushright{NIST Special Publication 1017-1B}
...
\huge
\flushright{NIST Special Publication 1017-1B}
...
\large
\flushright{\bf National Institute of Standards and Technology Special Publication 1017-1B\\
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~1017-1B, 68 pages (May 2009) \\
CODEN: NSPUE2 }
...
This report then describes these algorithms giving some of the technical and programmatic details.

Additional software libraries are used for implementing dialogs, rendering images and decompressing data files.
A small but important part of Smokeview is written in Fortran 90.  This part is used to read data
...
Currently, Smokeview is maintained by the Building and Fire Research Laboratory (BFRL) of National Institute of Standards and Technology.
...
Smoke and other attributes of fire is visualized by Smokeview using both quantitative and realistic techniques.  Smokeview uses quantitative display techniques such as 2D and 3D contouring.  Realistic display of data refers to the intent of presenting the data in a form as it would actually appear.
...
\end{enumerate}
\item Triangle decimation.  Eliminate any triangle that has two or more vertices that are {\em closest}\ to the same underlying FDS mesh node.
Remove this triangle's vertices, replacing them with a vertex with coordinates that are the average of the coordinates of the three removed vertices.  Then re-triangulate using this new vertex.  This process is illustrated in Figure \ref{figdecimate}.
