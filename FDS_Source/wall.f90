...
REAL(EB) :: T,YY_WALL,DENOM,YY_OTHER(MAX_SPECIES),RHO_YY_OTHER(MAX_SPECIES),YY_EXTERIOR(MAX_SPECIES), &
            YY_G(MAX_SPECIES),RHO_G,UN,DD,EPSB,MFT,TSI,ARO,UWO,RADIUS,AREA,RVC 
...
IF (VIRTUAL_PARTICLES .AND. CORRECTOR .AND. N_SPECIES>0) THEN
   DROPLET_LOOP: DO I=1,NLP
      ...
      IF (IBC>0) THEN
         ...
         YYP(II,JJ,KK,1:N_SPECIES) = (RHO(II,JJ,KK)*YYP(II,JJ,KK,1:N_SPECIES) + MASSFLUX(IW,1:N_SPECIES)*AREA*DT*RVC)/RHO(II,JJ,KK)
      ENDIF
   ENDDO DROPLET_LOOP
ENDIF
...
WALL_CELL_LOOP: DO IW=1,NWC+NVWC
   ...
   IF (N_SPECIES > 0) THEN
      ...
      CALL GET_AVERAGE_SPECIFIC_HEAT(YY_G,CP,ITMP)
   ELSE
      CP = Y2CP_C(ITMP)
   ENDIF
