...
\noindent {\bf Species and Combustion}
    ...
    \item It is now possible to transport, produce, and consume product species such as CO and soot. Chemical mechanisms must be provided by the you and may include reversible reactions.
...
There are two namelist groups that describe gas species. The first is {\ct SPEC}, which describes each gas species that is included in the simulation. The second is {\ct SMIX} which defines lumped species or mixtures of individual {\ct SPEC}.
...
the gas that is present at the start of the simulation.  Note that no transport equation is needed for the background species as it is whatever mass remains after all other tracked species have been accounted for. Or, a gas species might be one component of a mixture of gases that are transported together. FDS exploits the idea that the products of combustion from a fire mix and travel together; you only need to solve
...
the name of the species via a character string, {\ct ID}. Next, if the ambient (initial) mass fraction\footnote{{\ct HUMIDITY} can be specified for {\ct WATER VAPOR} in place of {\ct MASS\_FRACTION\_0}. {\ct HUMIDITY} is the relative humidity of water vapor in units of \%. It is 40 \% by default.} of the gas is something other than 0, then the parameter {\ct MASS\_FRACTION\_0} is used to specify it. There are a number of gasses with predefined properties available to you.  These gasses are listed in Table~\ref{tab:gasspecies}.  When using one of these gasses you need only specify the correct {\ct ID} and provide, if needed, the initial mass fraction.  You do not need to specify any of the additional gas properties discussed in the next section.  Here is an example:
...
the {\ct REFERENCE\_ENTHALPY}, $h(T_{ref})$ (kJ/kg).  The default value of the  {\ct REFERENCE\_TEMPERATURE} is 25~$^\circ$C. 

If {\ct SPECIFIC\_HEAT} is specified and the {\ct REFERENCE\_ENTHALPY} is not, the {\ct REFERENCE\_ENTHALPY} will be set to $h(T_{ref})=c_p T_{ref}$.
