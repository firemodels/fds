...
Natl.~Inst.~Stand.~Technol.~Spec.~Publ.~xxxx, 142 pages (May 2002) \\
CODEN: NSPUE2 }
...
This guide presents a series of images generated by first running FDS and then running Smokeview, designed to verify that Smokeview is working properly.
...
a computational fluid dynamics (CFD) model of fire-driven fluid
flow\cite{FDS_Tech_Guide_5} or CFAST, a zone model of compartment fire phenomena\cite{Jones:2004A}. Details on setting up and
...
be found in the Smokeview User's guide\cite{SMV_Users_Guide_5}.

This document presents a series of images generated by first running FDS and then running Smokeview, designed to verify that Smokeview is working properly.  These images are produced using the scripting
capability of Smokeview guaranteeing that the same images
...
\section{Slices}
Figures \ref{figslicetest} and \ref{figvslicetest} present images verifying the display of slices and vector slices.
The FDS input file for this test is {\tt plume5c.fds}\ (see Appendix \ref{FDSplume5c}).
The images for this test were created automatically by running the smokeview script, {\tt plume5c.ssf}\ (see Appendix \ref{SSFplume5c}).
...
\section{Fire Lines}
Figures \ref{figfirelinetest} present images verifying the display of a fire line.  A fire line is drawn as a special case of a temperature slice file.
