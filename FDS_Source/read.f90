...
NAMELIST /MISC/ AGGLOMERATION,AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                EXTINCTION_MODEL,HVAC_PRES_RELAX,&
                FDS5_OPTIONS,FLUX_LIMITER,FORCE_VECTOR,FREEZE_VELOCITY,FYI,GAMMA,GRAVITATIONAL_DEPOSITION,&
                GRAVITATIONAL_SETTLING,GROUND_LEVEL,GVEC,H_F_REFERENCE_TEMPERATURE,HRRPUV_MAX_SMV,&
                HUMIDITY,IMMERSED_BOUNDARY_METHOD,INITIAL_UNMIXED_FRACTION,ISOTROPIC_PARTICLE_FORCE,&
                ...
                OVERWRITE,PARTICLE_CFL_MAX,PARTICLE_CFL_MIN,PARTICLE_CFL,PERIODIC_TEST,POROUS_FLOOR,&
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   IF (SS%SPECIFIC_HEAT > 0._EB) THEN
      ...
      IF (SS%H_F > -2.E23_EB) THEN
         SS%REFERENCE_ENTHALPY = SS%H_F - SS%SPECIFIC_HEAT*H_F_REFERENCE_TEMPERATURE
      ELSEIF (SS%REFERENCE_ENTHALPY < -1.E20_EB) THEN
...
NAMELIST /REAC/ A,ALT_REAC_ID,AUTO_IGNITION_TEMPERATURE,C,CHECK_ATOM_BALANCE,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                ...
                ID,IDEAL,N,NU,N_S,N_T,O,ODE_SOLVER,RADIATIVE_FRACTION,RAMP_FS,REAC_ATOM_ERROR,&
...
REAC_READ_LOOP: DO NR=1,N_REACTIONS
   ...
   RN%FLAME_SPEED_TEMPERATURE   = FLAME_SPEED_TEMPERATURE+273.15_EB
...
DO NR = 1,N_REACTIONS
   NRR_LOOP: DO NRR = NR,N_REACTIONS
...
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,SPECIFIC_HEAT,DENSITY,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE, &
            PEAK_REACTION_RATE,POROSITY,KAI_A,KAI_B,KAI_C,KAI_D,SLOPE
REAL(EB), DIMENSION(MAX_MATERIALS,MAX_REACTIONS) :: NU_MATL
REAL(EB), DIMENSION(MAX_REACTIONS) :: A,E,PYROLYSIS_RANGE,HEAT_OF_REACTION, &
          N_S,N_T,N_O2,THRESHOLD_TEMPERATURE,HEAT_OF_COMBUSTION, &
          THRESHOLD_SIGN,GAS_DIFFUSION_DEPTH
REAL(EB), DIMENSION(MAX_REACTIONS,MAX_HEATING_RATES) :: REFERENCE_TEMPERATURE,HEATING_RATE,REFERENCE_RATE
...
INTEGER :: N,NN,NNN,IOS,NR,N_REACTIONS,N_HEATING_RATES
NAMELIST /MATL/ A,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE,COLOR,CONDUCTIVITY,CONDUCTIVITY_RAMP,DENSITY,E,EMISSIVITY,FYI,&
                ...
                THRESHOLD_SIGN,THRESHOLD_TEMPERATURE,POROSITY,ALLOW_SHRINKING,ALLOW_SWELLING,GAS_DIFFUSION_DEPTH,N_HEATING_RATES
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   NOT_BOILING: IF (BOILING_TEMPERATURE>4000._EB) THEN
      ...
      DO NR=1,N_REACTIONS
         IF (REFERENCE_TEMPERATURE(NR,1)<-TMPM  .AND. (E(NR)< 0._EB .OR. A(NR)<0._EB)) THEN
   ...
   DO NR=1,ML%N_REACTIONS
      IF (N_HEATING_RATES==1) THEN
         IF (ML%TMP_REF(NR,1) > 0._EB) THEN
            IF (ML%RATE_REF(NR,1) > 0._EB) THEN
               PEAK_REACTION_RATE = ML%RATE_REF(NR,1)
            ELSE
               PEAK_REACTION_RATE = 2._EB*ML%HEATING_RATE(NR,1)*(1._EB-SUM(ML%NU_RESIDUE(:,NR)))/ML%PYROLYSIS_RANGE(NR)
            ENDIF
            ML%E(NR) = EXP(1._EB)*PEAK_REACTION_RATE*R0*ML%TMP_REF(NR,1)**2/ML%HEATING_RATE(NR,1)
            ML%A(NR) = EXP(1._EB)*PEAK_REACTION_RATE*EXP(ML%E(NR)/(R0*ML%TMP_REF(NR,1)))
         ENDIF
         
      ELSEIF (N_HEATING_RATES>1) THEN
         KAI_A = 0._EB
         KAI_B = 0._EB
         KAI_C = 0._EB
         KAI_D = 0._EB
         DO NN=1,N_HEATING_RATES
            IF (ML%TMP_REF(NR,NN) > 0._EB) THEN
               KAI_A = KAI_A + (1._EB/ML%TMP_REF(NR,NN))**2
               KAI_B = KAI_B + 1._EB/ML%TMP_REF(NR,NN)
               KAI_C = KAI_C + (1._EB/ML%TMP_REF(NR,NN))*LOG(ML%HEATING_RATE(NR,NN)/ML%TMP_REF(NR,NN)**2)
               KAI_D = KAI_D + LOG(ML%HEATING_RATE(NR,NN)/ML%TMP_REF(NR,NN)**2)
            ENDIF 
         ENDDO
         SLOPE = (KAI_C*REAL(N_HEATING_RATES,EB) - KAI_B*KAI_D) / (KAI_A*REAL(N_HEATING_RATES,EB)-KAI_B**2)
!         ADD_FITTNESS CALCULATION and STORE in ML%FITNESS(NR)
         ML%E(NR) = -1._EB*SLOPE*R0

         NNN = 0
         ! Average A and N_S over heating rates
         ML%A(NR) = 0._EB
         DO NN=1,N_HEATING_RATES
            ML%A(NR) = ML%A(NR) + ML%HEATING_RATE(NR,NN)*ML%E(NR)/(EXP(-ML%E(NR)/(R0*ML%TMP_REF(NR,NN)))*R0*ML%TMP_REF(NR,NN)**2)
            IF(ML%RATE_REF(NR,NN) > 0._EB) THEN
               PEAK_REACTION_RATE = ML%RATE_REF(NR,NN)
               ML%N_S(NR) = ML%N_S(NR) + ML%HEATING_RATE(NR,NN)*ML%E(NR)/(EXP(1._EB)*R0*ML%TMP_REF(NR,NN)**2 *PEAK_REACTION_RATE)
               NNN = NNN+1
            ENDIF
         ENDDO
         ML%A(NR) = ML%A(NR)/REAL(N_HEATING_RATES,EB)
         ML%N_S(NR) = ML%N_S(NR)/REAL(NNN,EB)
         IF (NNN==0) ML%N_S(NR)=1._EB
      ENDIF    
...
LOGICAL :: VECTOR,CELL_CENTERED, FIRE_LINE, EVACUATION,LEVEL_SET_FIRE_LINE,GEOMETRY
CHARACTER(LABEL_LENGTH) :: QUANTITY,SPEC_ID,PART_ID,QUANTITY2,PROP_ID,REAC_ID
REAL(EB), PARAMETER :: TOL=1.E-10_EB
TYPE (SLICE_TYPE), POINTER :: SL=>NULL()
NAMELIST /SLCF/ AGL_SLICE,CELL_CENTERED,EVACUATION,FIRE_LINE,GEOMETRY,FYI,ID,LEVEL_SET_FIRE_LINE,MAXIMUM_VALUE,MESH_NUMBER,&
                MINIMUM_VALUE,PART_ID,PBX,PBY,PBZ,PROP_ID,QUANTITY,QUANTITY2,REAC_ID,SPEC_ID,VECTOR,VELO_INDEX,XB
