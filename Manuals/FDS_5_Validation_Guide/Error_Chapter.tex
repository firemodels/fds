...
\ee
Subtracting the random variable $\ln E$ in Eq.~(\ref{eeq}) from $\ln M$ in Eq.~(\ref{meq}) yields:
\be
...
\ee
The least squares estimate of the standard deviation is
\be
   \widehat{ \widetilde{\sigma}_M^2+\widetilde{\sigma}_E^2 } = \frac{1}{n-1} \sum_{i=1}^n \,
   \left[ \left(\ln M_i - \ln E_i \right) - \left( \overline{\ln M} - \overline{\ln E} \right)  \right]^2
\ee
...
\be
   \hat{\delta} = \exp \left( \overline{\ln M}-\overline{\ln E}+\frac{\widetilde{\sigma}_M^2}{2}-\frac{\widetilde{\sigma}_E^2}{2} \right)
\ee
...
\be \delta \, \theta \; | \; M \sim N \left( M \; , \; \sigma_M^2 \right) \label{thetaeq} \ee
...
\be \theta \sim N \left( \frac{M}{\hat{\delta}} \; , \; \widetilde{\sigma}_M^2 \left( \frac{M}{\hat{\delta}} \right)^2 \right) \label{truth} \ee
...
\be \mu = 20 + \frac{M}{\delta} = 20 + \frac{330}{1.03} = 340.4 \; ^\circ \hbox{C}  \quad ; \quad  \sigma = \widetilde{\sigma}_M \, \frac{M}{\hat{\delta}} = 0.10 \times \frac{330}{1.03} = 32 \; ^\circ \hbox{C}  \ee
