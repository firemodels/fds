...
NAMELIST /MISC/ PR,SC,TMPA,GVEC,PRESSURE_RELAX_FACTOR,RELAXATION_FACTOR,FYI, &
                ...
                ALLOW_UNDERSIDE_DROPLETS,POROUS_FLOOR,TEXTURE_ORIGIN,NSTRATA, &
                ...
                FDS6,COMBUSTION2,CLIP_MASS_FRACTION,STORE_MU_DNS,CHECK_VN, &
                SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,IMMERSED_BOUNDARY_METHOD, &
                MEAN_FORCING,RFAC_FORCING,H_EDDY,VELOCITY_TOLERANCE
...
NAMELIST /DUMP/ RENDER_FILE,SMOKE3D,SMOKE3D_COMPRESSION,SMOKE3D_QUANTITY,FLUSH_FILE_BUFFERS,MASS_FILE,STATE_FILE, &
                ...
                PLOT3D_PART_ID,CENTERED_HRR_TIME,CENTERED_DEVC_TIME,HEADERS,PLOT3D_VELO_INDEX,RAW_DATA_FILE
...
PLOOP: DO N=1,5
   CALL GET_QUANTITY_INDEX(PLOT3D_SMOKEVIEW_LABEL(N),PLOT3D_SMOKEVIEW_BAR_LABEL(N),PLOT3D_QUANTITY_INDEX(N),PLOT3D_SPEC_INDEX(N), &
                           PLOT3D_PART_INDEX(N),I_DUM,I_DUM,'PLOT3D',PLOT3D_QUANTITY(N),PLOT3D_SPEC_ID(N),PLOT3D_PART_ID(N),&
                           'null','null')
ENDDO PLOOP
...
IF (SMOKE3D) THEN
   CALL GET_QUANTITY_INDEX(SMOKE3D_SMOKEVIEW_LABEL,SMOKE3D_SMOKEVIEW_BAR_LABEL,SMOKE3D_QUANTITY_INDEX,SMOKE3D_SPEC_INDEX,I_DUM, &
                           I_DUM,I_DUM,'SMOKE3D',SMOKE3D_QUANTITY,SMOKE3D_SPEC_ID,'null','null','null')
ENDIF
...
USE PHYSICAL_FUNCTIONS, ONLY: GAS_PROPS
...
INTEGER  :: N_SPEC_READ,N_MIX,N,I,J,IPC,MODE,N_STATE_READ
LOGICAL  :: ABSORBING,MIXTURE_FRACTION_ONLY
CHARACTER(30) :: FORMULA,SPECIES_ID(0:MAX_SPECIES)
TYPE (PARTICLE_CLASS_TYPE), POINTER :: PC
TYPE (PROPERTY_TYPE), POINTER :: PY
TYPE (SPECIES_TYPE), POINTER, DIMENSION(:) :: SPECIES_TMP
NAMELIST /SPEC/ MASS_FRACTION_0,MW,FYI,ID,SIGMALJ,EPSILONKLJ,CONDUCTIVITY,VISCOSITY,DIFFUSIVITY,ABSORBING, &
                MASS_EXTINCTION_COEFFICIENT,FORMULA,SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,MIXTURE_FRACTION_ONLY
...
COUNT_SPEC_LOOP: DO
   ...
   IF (ID(1:16)=='MIXTURE_FRACTION' .OR. ID(1:16)=='MIXTURE FRACTION') THEN
      WRITE(MESSAGE,'(A,I2,A)') 'ERROR: Species ',N_SPECIES_TOTAL+1,' cannot be named MIXTURE_FRACTION'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   N_SPEC_READ = N_SPEC_READ + 1   
   N_SPECIES_TOTAL   = N_SPECIES_TOTAL  + 1   
   IF (.NOT. MIXTURE_FRACTION_ONLY) THEN
      IF (ID=='WATER VAPOR')     I_WATER = N_SPECIES_TOTAL
      IF (ID=='CARBON DIOXIDE')  I_CO2   = N_SPECIES_TOTAL
      IF (ID=='CARBON MONOXIDE') I_CO    = N_SPECIES_TOTAL      
      IF (ID=='OXYGEN')          I_O2    = N_SPECIES_TOTAL            
      IF (ID=='SOOT')            I_SOOT  = N_SPECIES_TOTAL            
      IF (ID=='NITROGEN')        I_N2    = N_SPECIES_TOTAL        
      IF (ID=='HYDROGEN')        I_H2    = N_SPECIES_TOTAL
   ELSE
      N_STATE_READ = N_STATE_READ + 1
   ENDIF
   30 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I2)') 'ERROR: Problem with SPECies number',N_SPEC_READ+1
         CALL SHUTDOWN(MESSAGE)
      ENDIF
ENDDO COUNT_SPEC_LOOP
29 REWIND(LU_INPUT)

N_SPECIES = N_SPECIES_TOTAL - N_STATE_READ
...
IF (WATER_EVAPORATION .AND. I_WATER==0) THEN
   N_SPECIES  = N_SPECIES_TOTAL + 1
   I_WATER    = N_SPECIES_TOTAL
...
IF (MIXTURE_FRACTION) THEN
   I_Z_MIN = N_SPECIES_TOTAL+1
   I_Z_MAX = N_SPECIES_TOTAL+N_MIX_SPECIES
   N_SPECIES_TOTAL = N_SPECIES_TOTAL + N_MIX_SPECIES
   N_SPECIES = N_SPECIES + N_MIX_SPECIES
   DO N = 1,N_MIX_SPECIES
      WRITE(SPECIES_ID(N_SPECIES),'(A,I1.1)') 'MIXTURE_FRACTION_',N
   ENDDO
   YYMIN(I_Z_MIN:I_Z_MAX) = 0.0_EB
ENDIF

! Allocate species-related arrays

ALLOCATE(SPECIES(0:N_SPECIES_TOTAL),STAT=IZERO)
...
N_MIX=0
REWIND(LU_INPUT)
SPEC_LOOP: DO N=0,N_SPECIES_TOTAL
   ...
   ELSE
      ...
      MW = MW_AIR
      IF (N-N_SPEC_READ < 10) THEN
         WRITE(FORMULA,'(A1,I1)') 'Z',N-N_SPEC_READ
      ELSE
         WRITE(FORMULA,'(A1,I2)') 'Z',N-N_SPEC_READ      
      ENDIF
   ENDIF
   ...
   IF (MASS_FRACTION_0<0._EB) THEN
      IF (ID=='WATER VAPOR' .AND. .NOT. MIXTURE_FRACTION_ONLY) THEN
   ...
   ! Special case for soot
     ...
     IF (ID=='SOOT' .AND. .NOT. MIXTURE_FRACTION_ONLY) MODE = AEROSOL_SPECIES
   ...
   SS%MIXTURE_FRACTION_ONLY       = MIXTURE_FRACTION_ONLY
   SS%MW                          = MW
   SS%MODE                        = MODE
   ...
   IF (MIXTURE_FRACTION_ONLY) THEN
      N_MIX = N_MIX + 1
      IF (MW <= ZERO_P) THEN
         CALL GAS_PROPS(ID,SIGMALJ,EPSILONKLJ,MW,ABSORBING,FORMULA)
         IF (MW <= ZERO_P) THEN
            WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPECies ',N,': Specify MW'
            CALL SHUTDOWN(MESSAGE)
         ENDIF
      ENDIF
      MF_MW(N_MIX) = MW
      MF_SPEC_ID(N_MIX) = ID
      MF_SPEC_FORMULA(N_MIX) = FORMULA
   ENDIF
...
PART_CLASS_LOOP: DO IPC=1,N_PART
   ...
   DO I=1,N_SPECIES_TOTAL
      IF (SPECIES(I)%ID==PC%SPEC_ID .AND. .NOT. SPECIES(I)%MIXTURE_FRACTION_ONLY) THEN
         PC%SPEC_INDEX = I
...
PROP_LOOP: DO IPC=1,N_PROP
   ...
   DO I=1,N_SPECIES
      IF (SPECIES(I)%ID==PY%SPEC_ID .AND. .NOT. SPECIES(I)%MIXTURE_FRACTION_ONLY) THEN
         PY%SPEC_INDEX = I
         CYCLE PROP_LOOP
      ENDIF
   ENDDO
...
IF (N_STATE_READ > 0) THEN
   ALLOCATE(SPECIES_TMP(0:N_SPECIES_TOTAL),STAT=IZERO)
   CALL ChkMemErr('READ','SPECIES_TMP',IZERO)
   I = 0
   J = N_SPECIES
   DO N=1,N_SPECIES_TOTAL
      IF (SPECIES_TMP(N)%MIXTURE_FRACTION_ONLY) THEN
         J = J + 1
         SPECIES(J) = SPECIES_TMP(N)
      ELSE
         I = I + 1
         SPECIES(I) = SPECIES_TMP(N)
      ENDIF
   ENDDO
   DEALLOCATE(SPECIES_TMP)
ENDIF

END SUBROUTINE READ_SPEC

SUBROUTINE READ_SPEC_MF
!Pre-process species to identify number of mixture fraction components 
REAL(EB) :: MASS_FRACTION_0,MW,SIGMALJ,EPSILONKLJ,XVAP,VISCOSITY,CONDUCTIVITY,DIFFUSIVITY,MASS_EXTINCTION_COEFFICIENT, &
            SPECIFIC_HEAT,SPECIFIC_ENTHALPY,REFERENCE_TEMPERATURE,H_V
...
N_STATE_SPECIES = 0
REWIND(LU_INPUT)
COUNT_STATE_SPEC_LOOP: DO
   CALL CHECKREAD('SPEC',LU_INPUT,IOS) 
   IF (IOS==1) EXIT COUNT_STATE_SPEC_LOOP
   MIXTURE_FRACTION_ONLY = .FALSE.
   ID = 'null'
   READ(LU_INPUT,NML=SPEC,END=29,ERR=30,IOSTAT=IOS)
   IF (ID=='null') THEN
   ...
   IF (ID(1:16)=='MIXTURE_FRACTION' .OR. ID(1:16)=='MIXTURE FRACTION') THEN
      ...
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (MIXTURE_FRACTION_ONLY) N_STATE_SPECIES = N_STATE_SPECIES + 1
   30 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I2)') 'ERROR: Problem with SPECies number',N_SPECIES+1
         CALL SHUTDOWN(MESSAGE)
      ENDIF
ENDDO COUNT_STATE_SPEC_LOOP
29 REWIND(LU_INPUT)

IF (N_STATE_SPECIES==0) RETURN
...
MF_SPEC_ID(1:N_STATE_SPECIES) = 'null'
ALLOCATE(MF_SPEC_FORMULA(N_STATE_SPECIES),STAT=IZERO)
MF_SPEC_FORMULA(1:N_STATE_SPECIES) = 'null'
CALL ChkMemErr('READ','MF_SPEC_FORMULA',IZERO)

END SUBROUTINE READ_SPEC_MF

 
SUBROUTINE READ_MIXF
!Read defintions of mixture fraction variables
REAL(EB) :: NU(MAX_SPECIES)
INTEGER :: NZ
NAMELIST /MIXF/ NU
...
COUNT_MIXF_LOOP: DO
   NU = 0._EB
   CALL CHECKREAD('MIXF',LU_INPUT,IOS)
   IF (IOS==1) EXIT COUNT_MIXF_LOOP
   READ(LU_INPUT,MIXF,ERR=4213,IOSTAT=IOS)  
   IF (ABS(MINVAL(NU))<ZERO_P .AND. ABS(MAXVAL(NU)) < ZERO_P) EXIT
   N_MIX_SPECIES = N_MIX_SPECIES + 1
   4213 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I3)') 'ERROR: Problem with MIXF line ',N_MIX_SPECIES
      CALL SHUTDOWN(MESSAGE)
   ENDIF
END DO COUNT_MIXF_LOOP

IF (N_MIX_SPECIES<=0) RETURN

MIXTURE_FRACTION = .TRUE.

ALLOCATE(MF_NU(N_STATE_SPECIES,0:N_MIX_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ','MF_NU',IZERO)
MF_NU = 0._EB
REWIND(LU_INPUT)

DO NZ = 0,N_MIX_SPECIES
   NU = 0._EB
   READ(LU_INPUT,MIXF)  
   MF_NU(1:N_STATE_SPECIES,NZ) = NU(1:N_STATE_SPECIES)
ENDDO
  
RETURN

END SUBROUTINE READ_MIXF 
 
 
SUBROUTINE READ_REAC
 
CHARACTER(30) :: FUEL,OXIDIZER
LOGICAL :: IDEAL,HRRPUA_REAC,MIXING_CONTROLLED
INTEGER :: NN,N_REAC_READ
REAL(EB) :: Y_O2_INFTY,Y_F_INLET, &
            H2_YIELD,SOOT_YIELD,CO_YIELD, Y_F_LFL,X_O2_LL,EPUMO2,BOF,&
            CRITICAL_FLAME_TEMPERATURE,HEAT_OF_COMBUSTION,COMPLETE_HEAT_OF_COMBUSTION,E,N_S(MAX_SPECIES),C,H,N,O,OTHER, &
            FUEL_HEAT_OF_FORMATION,MAXIMUM_VISIBILITY,AIT, MW_FUEL,NU_Z(MAX_SPECIES,MAX_SPECIES),NU(MAX_SPECIES)
NAMELIST /REAC/ E,BOF,HEAT_OF_COMBUSTION,FYI,FUEL,OXIDIZER,EPUMO2,ID, N_S,&
                Y_O2_INFTY,Y_F_INLET, COMPLETE_HEAT_OF_COMBUSTION,&
                ...
                EDDY_DISSIPATION,C_EDC,HRRPUA_SHEET,HRRPUV_AVERAGE,HRRPUV_AVERAGE_TIME,AIT,SUPPRESSION,SUPPRESSION_SEARCH, &
                CO_PRODUCTION,SOOT_DEPOSITION,HRRPUA_REAC,NU_Z
...
COUNT_REAC_LOOP: DO
   ...
   IF ((BOF > 0._EB .OR. E > 0._EB) .AND. MIXTURE_FRACTION) THEN
      ...
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (MIXTURE_FRACTION .AND. N_MIX_SPECIES==0 .AND. N_REACTIONS > 1) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: can not have more than one reaction when using simple mixture fraction (C,H,O,N,Other)'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (N_MIX_SPECIES > 0 .AND. (C >=0._EB .OR. H>=0._EB .OR. O>0._EB .OR. N>0._EB .OR. OTHER>0._EB))  THEN
      WRITE(MESSAGE,'(A)') 'ERROR: can not use C,H,O,N,Other with &MIXF inputs'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   434 IF (IOS>0) THEN
...
IF (MIXTURE_FRACTION .AND. N_MIX_SPECIES<=0) THEN 
   N_REAC_READ = 1
ELSE
...
READ_REACTION_LOOP: DO NN=1,N_REAC_READ
   ...
   ELSE
      ...
      RN%COMPLETE_HEAT_OF_COMBUSTION = COMPLETE_HEAT_OF_COMBUSTION*1000._EB
      RN%HRRPUA_REAC        = HRRPUA_REAC
      RN%NAME               = ID
      RN%FUEL               = FUEL        
      RN%OXIDIZER           = OXIDIZER
      REACTION%MIXING_CONTROLLED = MIXING_CONTROLLED
      IF (MIXTURE_FRACTION .AND. N_MIX_SPECIES > 0) THEN
         ALLOCATE(RN%NU_Z(N_STATE_SPECIES,N_MIX_SPECIES))
         RN%NU_Z = NU_Z
      ENDIF
...
SUBROUTINE SET_MIXF
! Set up REAC and MIXF arrays when C,H,O, etc. options are used
REAL(EB) :: FUEL_STREAM_C
INTEGER :: I,J,NNN
...
REACTION%MIXING_CONTROLLED = .TRUE.
...
WRITE(*,*) 'STATE MIX REAC ',N_STATE_SPECIES,N_MIX_SPECIES,N_REACTIONS
...
IF (H > 0._EB) THEN
   NNN = NNN + 1
   MF_SPEC_ID(NNN) = 'WATER VAPOR'
   MF_SPEC_FORMULA(NNN) = 'H2O'
   MF_MW(NNN) = MW_H2O
   H2O_INDEX = NNN
ENDIF     
IF (CO_PRODUCTION .OR. CO_YIELD > 0._EB)THEN
   NNN = NNN + 1
   MF_SPEC_ID(NNN) = 'CARBON MONOXIDE'
   MF_SPEC_FORMULA(NNN) = 'CO'
   MF_MW(NNN) = MW_CO
   CO_INDEX = NNN
ENDIF     
IF (SOOT_DEPOSITION .OR. SOOT_YIELD > 0._EB)THEN
   NNN = NNN + 1
   MF_SPEC_ID(NNN) = 'SOOT'
   MF_SPEC_FORMULA(NNN) = 'Soot'
   MF_MW(NNN) = MW_SOOT
   SOOT_INDEX = NNN
ENDIF     
IF (H2_YIELD > 0._EB)THEN
   NNN = NNN + 1
   MF_SPEC_ID(NNN) = 'HYDROGEN'
   MF_SPEC_FORMULA(NNN) = 'H2'
   MF_MW(NNN) = MW_H2
   H2_INDEX = NNN
ENDIF     
IF (OTHER > 0._EB) THEN
   NNN = NNN + 1
   MF_SPEC_ID(NNN) = 'OTHER'
   MF_SPEC_FORMULA(NNN) = 'Other'
   MF_MW(NNN) = MW_OTHER
   OTHER_INDEX = NNN
ENDIF     

DO NNN = 1,N_REACTIONS
   ALLOCATE(REACTION(NNN)%NU_Z(N_STATE_SPECIES,N_MIX_SPECIES))
   REACTION(NNN)%NU_Z = 0._EB
   ALLOCATE(REACTION(NNN)%Z2Z(N_MIX_SPECIES))   
   REACTION(NNN)%Z2Z = 0._EB
ENDDO

MW_AVG_C = Y_O2_INFTY/MW_O2 + (1._EB-Y_O2_INFTY)/MW_N2

MF_NU(O2_INDEX,0) = Y_O2_INFTY/MW_O2/MW_AVG_C
MF_NU(N2_INDEX,0) = 1._EB - MF_NU(O2_INDEX,0)
...
REACTION(1)%NU_Z(FUEL_INDEX,1) = -1._EB

FUEL_STREAM_C = Y_F_INLET/MW_FUEL + (1._EB - Y_F_INLET)/MW_N2
MF_NU(FUEL_INDEX,1) = Y_F_INLET/MW_FUEL/FUEL_STREAM_C
MF_NU(N2_INDEX,1) = 1._EB - MF_NU(FUEL_INDEX,1)

IF (IDEAL) THEN
!Compute fuel heat of formation
   IF (HEAT_OF_COMBUSTION < 0._EB) HEAT_OF_COMBUSTION = EPUMO2*(MW_O2*(C + 0.5_EB * H - 0.5_EB * O))/MW_FUEL
   FUEL_HEAT_OF_FORMATION =  HEAT_OF_COMBUSTION * MW_FUEL - (C * CO2_HEAT_OF_FORMATION + 0.5_EB * H * H2O_HEAT_OF_FORMATION)
ENDIF
...
SET_THREE_PARAMETER: IF (CO_PRODUCTION) THEN !Set reaction variables for three parameter mixture fraction 
   ...
   RN => REACTION(1) 
   RN%FUEL = 'FUEL'
   RN%I_FUEL = FUEL_INDEX
   RN%OXIDIZER = 'OXYGEN'
   RN%I_OXIDIZER = O2_INDEX   

   RN%NU_Z(CO2_INDEX,2) = 0._EB

   IF (H > 0._EB) RN%NU_Z(H2O_INDEX,2) = 0.5_EB * H
   IF (H2_YIELD > 0._EB) THEN
      RN%NU_Z(H2_INDEX,2)  = H2_YIELD * MW_FUEL / MW_H2
      MF_NU(H2_INDEX,2) = RN%NU_Z(H2_INDEX,2)
      RN%NU_Z(H2O_INDEX,2) = RN%NU_Z(H2O_INDEX,2) - RN%NU_Z(H2_INDEX,2)
   ENDIF

   IF (SOOT_YIELD > 0._EB) THEN
      RN%NU_Z(SOOT_INDEX,2) = SOOT_YIELD * MW_FUEL / MW_SOOT
      MF_NU(SOOT_INDEX,2) = RN%NU_Z(SOOT_INDEX,2)
      IF (H > 0._EB) RN%NU_Z(H2O_INDEX,2) = RN%NU_Z(H2O_INDEX,2)- 0.5_EB * RN%NU_Z(SOOT_INDEX,2) * SOOT_H_FRACTION
   ENDIF
   
   RN%NU_Z(N2_INDEX,2) = 0.5_EB*N
   
   IF (OTHER > 0._EB) THEN 
      RN%NU_Z(OTHER_INDEX,2) = OTHER
      MF_NU(OTHER_INDEX,2) = OTHER
   ENDIF
  
   RN%NU_Z(CO_INDEX,2) = C
   IF (SOOT_YIELD > 0._EB)  RN%NU_Z(CO_INDEX,2) = RN%NU_Z(CO_INDEX,2) - RN%NU_Z(SOOT_INDEX,2) * (1._EB - SOOT_H_FRACTION) 
   RN%NU_Z(O2_INDEX,2) = 0.5_EB*(O - RN%NU_Z(CO_INDEX,2))

   IF (H > 0._EB) RN%NU_Z(O2_INDEX,2) = RN%NU_Z(O2_INDEX,2) - 0.5_EB*RN%NU_Z(H2O_INDEX,2)

   MF_NU(O2_INDEX,2) = RN%NU_Z(O2_INDEX,2)
   MF_NU(N2_INDEX,2) = RN%NU_Z(N2_INDEX,2) +  MF_NU(N2_INDEX,1)/ MF_NU(FUEL_INDEX,1)
   MF_NU(CO_INDEX,2) = RN%NU_Z(CO_INDEX,2)
   
   IF (H>0._EB) MF_NU(H2O_INDEX,2) = RN%NU_Z(H2O_INDEX,2)

   IF (.NOT. IDEAL) THEN
      IF(HEAT_OF_COMBUSTION < 0._EB) HEAT_OF_COMBUSTION = -EPUMO2*RN%NU_Z(O2_INDEX,2)*MW_O2/MW_FUEL
      RN%HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION*1000._EB
   ELSE
   ! Correct heat of combustion for minor products of combustion
      RN%HEAT_OF_COMBUSTION = FUEL_HEAT_OF_FORMATION + RN%NU_Z(CO_INDEX,2) * CO_HEAT_OF_FORMATION 
      IF (H > 0._EB)  RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION + RN%NU_Z(H2O_INDEX,2) * H2O_HEAT_OF_FORMATION
      RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION * 1000._EB /MW_FUEL
   ENDIF
   
   RN%Z2Z(1) = -1._EB
   RN%Z2Z(2) = (DOT_PRODUCT(MF_NU(:,2),MF_MW)-MF_NU(O2_INDEX,2)*MW_O2)/(DOT_PRODUCT(MF_NU(:,1),MF_MW)/MF_NU(FUEL_INDEX,1))
   ...
   RN%OXIDIZER = 'OXYGEN'
   RN%I_OXIDIZER = O2_INDEX   
   RN%HEAT_OF_COMBUSTION = (CO2_HEAT_OF_FORMATION-CO_HEAT_OF_FORMATION)/MW_CO*1.E6_EB
   RN%NU_Z(CO_INDEX,2)  = -1._EB
   RN%NU_Z(CO2_INDEX,3) = 1._EB
   RN%NU_Z(O2_INDEX,3)  = -0.5_EB      

   IF (RN%NU_Z(CO2_INDEX,3) < 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Values for SOOT_YIELD, CO_YIELD, and SOOT_H_FRACTION result in negative CO2 yield'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (RN%NU_Z(H2O_INDEX,3) < 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Values for SOOT_YIELD, H2_YIELD, and SOOT_H_FRACTION result in negative H2O yield'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   
   MF_NU(N2_INDEX,3) = MF_NU(N2_INDEX,2)
   IF (SOOT_YIELD > 0._EB) MF_NU(SOOT_INDEX,3) = MF_NU(SOOT_INDEX,2)
   IF (H2_YIELD > 0._EB) MF_NU(H2_INDEX,3) = MF_NU(H2_INDEX,2)
   IF (H>0._EB) MF_NU(H2O_INDEX,3) = MF_NU(H2O_INDEX,2)
   IF (OTHER>0._EB) MF_NU(OTHER_INDEX,3) = MF_NU(OTHER_INDEX,2)
   MF_NU(CO_INDEX,3) = MW_FUEL*CO_YIELD/MW_CO
   MF_NU(CO2_INDEX,3) = MF_NU(CO_INDEX,2 )- MF_NU(CO_INDEX,3)
   MF_NU(O2_INDEX,3) = MF_NU(O2_INDEX,2) - 0.5*MF_NU(CO2_INDEX,3)
   RN%Z2Z(2) = -1._EB
   RN%Z2Z(3) = (DOT_PRODUCT(MF_NU(:,3),MF_MW)-MF_NU(O2_INDEX,3)*MW_O2)/(DOT_PRODUCT(MF_NU(:,2),MF_MW)-MF_NU(O2_INDEX,2)*MW_O2)

   MF_NU(:,2) = MF_NU(:,2)/(SUM(MF_NU(:,2))-MF_NU(O2_INDEX,2)) 
   MF_NU(:,3) = MF_NU(:,3)/(SUM(MF_NU(:,3))-MF_NU(O2_INDEX,3))  
...
!Set reaction variables for complete reaction
   ...
   RN%OXIDIZER = 'OXYGEN'
   RN%I_OXIDIZER = O2_INDEX   
   RN%NU_Z(CO2_INDEX,2) = C

   IF (H > 0._EB) RN%NU_Z(H2O_INDEX,2) = 0.5_EB * H

   IF (CO_YIELD > 0._EB) THEN
      RN%NU_Z(CO_INDEX,2)  = CO_YIELD * MW_FUEL / MW_CO
      RN%NU_Z(CO2_INDEX,2) = RN%NU_Z(CO2_INDEX,2) - RN%NU_Z(CO_INDEX,2) 
      MF_NU(CO_INDEX,2) = RN%NU_Z(CO_INDEX,2)      
   ENDIF
   ...
   IF (SOOT_DEPOSITION) THEN
      RN%NU_Z(SOOT_INDEX,3) = SOOT_YIELD * MW_FUEL / MW_SOOT
      IF (H > 0._EB) RN%NU_Z(H2O_INDEX,2) = RN%NU_Z(H2O_INDEX,2)- 0.5_EB * RN%NU_Z(SOOT_INDEX,3) * SOOT_H_FRACTION
      RN%NU_Z(CO2_INDEX,2) = RN%NU_Z(CO2_INDEX,2) - RN%NU_Z(SOOT_INDEX,3)*(1._EB-SOOT_H_FRACTION)
      MF_NU(SOOT_INDEX,3) = 1._EB
   ELSE
      IF (SOOT_YIELD > 0._EB) THEN
         RN%NU_Z(SOOT_INDEX,2) = SOOT_YIELD * MW_FUEL / MW_SOOT
         IF (H > 0._EB) RN%NU_Z(H2O_INDEX,2) = RN%NU_Z(H2O_INDEX,2)- 0.5_EB * RN%NU_Z(SOOT_INDEX,2) * SOOT_H_FRACTION
         RN%NU_Z(CO2_INDEX,2) = RN%NU_Z(CO2_INDEX,2) - RN%NU_Z(SOOT_INDEX,2)*(1._EB-SOOT_H_FRACTION)         
         MF_NU(SOOT_INDEX,2) = RN%NU_Z(SOOT_INDEX,2)
      ENDIF
   ENDIF

   MF_NU(CO2_INDEX,2) = RN%NU_Z(CO2_INDEX,2)

   RN%NU_Z(N2_INDEX,2) = 0.5_EB*N
   
   IF (OTHER > 0._EB) RN%NU_Z(OTHER_INDEX,2) = OTHER
   
   RN%NU_Z(O2_INDEX,2) = -RN%NU_Z(CO2_INDEX,2)+0.5_EB*O
   IF (CO_YIELD > 0._EB)  RN%NU_Z(O2_INDEX,2) = RN%NU_Z(O2_INDEX,2) - 0.5_EB*RN%NU_Z(CO_INDEX,2)
   IF (H > 0._EB)  RN%NU_Z(O2_INDEX,2) = RN%NU_Z(O2_INDEX,2) - 0.5_EB*RN%NU_Z(H2O_INDEX,2)   
   MF_NU(O2_INDEX,2) = RN%NU_Z(O2_INDEX,2)

   IF (RN%NU_Z(CO2_INDEX,2) < 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Values for SOOT_YIELD, CO_YIELD, and SOOT_H_FRACTION result in negative CO2 yield'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   IF (RN%NU_Z(H2O_INDEX,2) < 0._EB) THEN
      WRITE(MESSAGE,'(A)') 'ERROR: Values for SOOT_YIELD, H2_YIELD, and SOOT_H_FRACTION result in negative H2O yield'
      CALL SHUTDOWN(MESSAGE)
   ENDIF
   ...
   RN%Z2Z(1) = -1
   RN%Z2Z(2) = (DOT_PRODUCT(MF_NU(:,2),MF_MW)-MF_NU(O2_INDEX,2)*MW_O2)/(DOT_PRODUCT(MF_NU(:,1),MF_MW)/MF_NU(FUEL_INDEX,1))
   IF (SOOT_DEPOSITION) RN%Z2Z(3) = RN%NU_Z(SOOT_INDEX,3)*MW_SOOT/MW_FUEL

   MF_NU(:,2) = MF_NU(:,2)/(SUM(MF_NU(:,2))-MF_NU(O2_INDEX,2))   

   WRITE(*,*) IDEAL,EPUMO2,R
   IF (.NOT. IDEAL) THEN
      IF(HEAT_OF_COMBUSTION < 0._EB) HEAT_OF_COMBUSTION = -EPUMO2*RN%NU_Z(O2_INDEX,2)*MW_O2/MW_FUEL
      RN%HEAT_OF_COMBUSTION = HEAT_OF_COMBUSTION*1000._EB
   ELSE
   ! Correct heat of combustion for minor products of combustion
      RN%HEAT_OF_COMBUSTION = FUEL_HEAT_OF_FORMATION + RN%NU_Z(CO2_INDEX,2) * CO2_HEAT_OF_FORMATION
      IF (CO_YIELD > 0._EB)  RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION + &
                                                         RN%NU_Z(CO_INDEX,2) * CO_HEAT_OF_FORMATION    
      IF (H > 0._EB)  RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION + &
                                                         RN%NU_Z(H2O_INDEX,2) * H2O_HEAT_OF_FORMATION
      RN%HEAT_OF_COMBUSTION = RN%HEAT_OF_COMBUSTION * 1000._EB /MW_FUEL
   ENDIF
...
WRITE(*,*) MF_SPEC_ID
WRITE(*,*) MF_SPEC_FORMULA
WRITE(*,*) MF_MW
DO I=0,N_MIX_SPECIES
   WRITE(*,*) 'Z_',I
   WRITE(*,*) MF_NU(:,I)
ENDDO
DO I=1,N_REACTIONS
   WRITE(*,*) 'REACTION ',I
   WRITE(*,*) REACTION(I)%Z2Z
   DO J=1,N_MIX_SPECIES
      WRITE(*,*) J
      WRITE(*,*) REACTION(I)%NU_Z(:,J)
   ENDDO
ENDDO

END SUBROUTINE SET_MIXF 
...
HRRPUA_REAC                 = .FALSE.
H2_YIELD                    = 0._EB
COMPLETE_HEAT_OF_COMBUSTION = -1._EB
...
REAL(EB) :: EPSIJ,SIGMA2,AMW,OMEGA,TSTAR,D_N0,EPSK_N,SIG_N,MW_N,D_TMP(N_STATE_SPECIES),MW_Z,MASS_SUM
...
DO N=1,N_SPECIES
   ...
   IF (SPECIES(N)%ID=='MIXTURE_FRACTION') &
      SPECIES%MW = DOT_PRODUCT(MF_NU(1:N_STATE_SPECIES,N),MF_MW(1:N_STATE_SPECIES))/SUM(MF_NU(1:N_STATE_SPECIES,N))
...
IF (MIXTURE_FRACTION) THEN
   Y2Y_C(1:N_STATE_SPECIES) = MF_NU(1:N_STATE_SPECIES,0)*MF_MW(1:N_STATE_SPECIES)
   WRITE(*,*) Y2Y_C,N_STATE_SPECIES
   Y2Y_C = Y2Y_C / SUM(Y2Y_C)
   DO N = 1,N_SPECIES
      IF (N >= I_Z_MIN) THEN
         Y2Y(1:N_STATE_SPECIES,N) = MF_NU(1:N_STATE_SPECIES,N)*MF_MW(1:N_STATE_SPECIES)
         MASS_SUM = 0._EB
         DO NN=1,N_STATE_SPECIES
            IF (Y2Y(NN,N) > 0._EB) MASS_SUM = MASS_SUM + Y2Y(NN,N)
         ENDDO
         Y2Y(:,N) = Y2Y(:,N) / MASS_SUM 
         Y2Y(:,N) = Y2Y(:,N) - Y2Y_C * SUM(Y2Y(:,N))
      ELSE
         Y2Y(SPECIES(N)%INDEX,N) = 1._EB - Y2Y_C(SPECIES(N)%INDEX)
      ENDIF
   END DO
...
IF (MIXTURE_FRACTION) THEN
   MW_AVG_Y_C = SUM(Y2Y_C(1:N_STATE_SPECIES)/MF_MW(1:N_STATE_SPECIES))
   DO N=1,N_SPECIES
      IF (N >= I_Z_MIN) THEN
         MW_AVG_Y(N)   = MW_AVG_Y(N)   + SUM(Y2Y(1:N_STATE_SPECIES,N)/MF_MW(1:N_STATE_SPECIES))
      ELSE
         MW_AVG_Y(N) = 1._EB/SPECIES(N)%MW - MW_AVG_Y_C
      ENDIF
   ENDDO
ELSE
...
IF (MIXTURE_FRACTION) THEN
   ...
   SS0%MW = MF_MW(N2_INDEX)*MF_NU(N2_INDEX,0)+MF_MW(O2_INDEX)*MF_NU(O2_INDEX,0)
ENDIF
...
IF (MIXTURE_FRACTION) THEN
   DO N=1,N_MIX_SPECIES
      MW_Z = DOT_PRODUCT(MF_NU(1:N_STATE_SPECIES,N),MF_MW(1:N_STATE_SPECIES))
      MW_MIN = MIN(MW_MIN,MW_Z)
      MW_MAX = MAX(MW_MAX,MW_Z)
   ENDDO
ENDIF
...
T_LOOP_1: DO J=1,5000
   IF (MIXTURE_FRACTION) THEN
      SUB_SPECIES_LOOP2: DO NN=1,N_STATE_SPECIES
         CALL JANAF_TABLE(J,CP_TMP(NN),H_TMP(NN),MF_SPEC_ID(NN),0._EB)   
         SIG_N = -1._EB
         EPSK_N = -1._EB 
         MW_N = -.1_EB
         CALL GAS_PROPS(MF_SPEC_ID(NN),SIG_N,EPSK_N,MW_N,ABSORBING,MF_SPEC_FORMULA(NN))
         !Conductivity
         SIGMA2 = SIG_N**2
         TSTAR = J/EPSK_N
...
CHARACTER(25) :: COLOR
LOGICAL :: MASSLESS,STATIC,FUEL,WATER,TREE,MONODISPERSE,EVAPORATE,RADIATION,BREAKUP
...
NAMELIST /PART/ FYI,DROPLETS_PER_SECOND,SAMPLING_FACTOR,ID,STATIC,MASSLESS,FUEL,WATER,TREE, &
                ...
                VEG_MOIST_MPV_MIN,VEG_REMOVE_CHARRED,VEG_STEM, &
...
PLOOP2: DO IPC=1,N_PART
   PC=>PARTICLE_CLASS(IPC)
   IF (PC%MASSLESS) PC%RADIATION=.FALSE.
   IF (PC%RADIATION) THEN
   ...
   IF (ANY(PC%QUANTITIES/='null')) THEN
      ...
      QUANTITIES_LOOP: DO NN=1,10
         ...
         CALL GET_QUANTITY_INDEX(PC%SMOKEVIEW_LABEL(PC%N_QUANTITIES),PC%SMOKEVIEW_BAR_LABEL(PC%N_QUANTITIES), &
                                 PC%QUANTITIES_INDEX(PC%N_QUANTITIES),I_DUMMY,I_DUMMY,I_DUMMY,I_DUMMY,'PART',PC%QUANTITIES(NN),&
                                 SPEC_ID,PART_ID,'null','null')
...
REAL(EB) :: EMISSIVITY,CONDUCTIVITY,SPECIFIC_HEAT,DENSITY,ABSORPTION_COEFFICIENT,BOILING_TEMPERATURE, &
            INITIAL_VAPOR_FLUX,PEAK_REACTION_RATE,PEAK_MASS_FRACTION
REAL(EB), DIMENSION(1:MAX_REACTIONS) :: A,E,HEATING_RATE,PYROLYSIS_RANGE,HEAT_OF_REACTION,NU_FUEL,NU_WATER,NU_RESIDUE, &
                                        N_S,N_T,REFERENCE_RATE,REFERENCE_TEMPERATURE,THRESHOLD_TEMPERATURE
...
READ_MATL_LOOP: DO N=1,N_MATL
   ...
   DO NN=1,ML%N_REACTIONS
      IF (ML%TMP_REF(NN) > 0._EB) THEN
         ...
         PEAK_MASS_FRACTION = 0.4_EB ! Approximate value of Y(T_p)
         ML%E(NN) = (PEAK_REACTION_RATE/PEAK_MASS_FRACTION)*R0*ML%TMP_REF(NN)**2/ML%HEATING_RATE(NN)
         ML%A(NN) = (PEAK_REACTION_RATE/PEAK_MASS_FRACTION)*EXP(ML%E(NN)/(R0*ML%TMP_REF(NN)))
      ENDIF
   ENDDO
      ...
      IF (MIXTURE_FRACTION) THEN
         IF (N_REACTIONS > 0) THEN
            ...
            IF (ML%HEAT_OF_COMBUSTION(J,I_FUEL)>0._EB .AND. RN%HEAT_OF_COMBUSTION>0._EB)  &
             ML%ADJUST_BURN_RATE(J,I_FUEL) = ML%HEAT_OF_COMBUSTION(J,I_FUEL)/(Y2Y(FUEL_INDEX,1)*RN%HEAT_OF_COMBUSTION)
         ENDIF
...
CHARACTER(30) :: PART_ID,RAMP_MF(0:MAX_SPECIES),RAMP_Q,RAMP_V,RAMP_T,MATL_ID(MAX_LAYERS,MAX_MATERIALS),&
                 PROFILE,BACKING,GEOMETRY,NAME_LIST(MAX_MATERIALS),EXTERNAL_FLUX_RAMP
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(0:MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2), &
            TAU_EXTERNAL_FLUX,E_COEFFICIENT,VOLUME_FLUX,TMP_FRONT,TMP_INNER,THICKNESS(MAX_LAYERS),VEL, &
            ...
            ROUGHNESS,RADIUS,LENGTH,WIDTH,DT_INSERT
...
NAMELIST /SURF/ TMP_FRONT,TMP_INNER,THICKNESS,MASS_FRACTION,VEL,VEL_T,NPPC, &
                E_COEFFICIENT,CONVECTIVE_HEAT_FLUX,TAU_Q,TAU_V,TAU_T,RAMP_Q,RAMP_T,TAU_MF,TAU_EXTERNAL_FLUX, &
                ...
                IGNITION_TEMPERATURE,HEAT_OF_VAPORIZATION,REGRID_FACTOR,NET_HEAT_FLUX,LAYER_DIVIDE,SURFACE_DENSITY, &
                ROUGHNESS,FREE_SLIP,NO_SLIP,EXTERNAL_FLUX,EXTERNAL_FLUX_RAMP,RADIUS,LENGTH,WIDTH,DT_INSERT
  ...
  ! Determine if the surface is combustible/burning
   ...
   BURNING_IF: IF (BURNING) THEN
      IF (SF%HRRPUA>0._EB) THEN
         RN => REACTION(1)
         WRITE(*,*) SF%HRRPUA,RN%HEAT_OF_COMBUSTION,Y2Y(FUEL_INDEX,I_Z_MIN),FUEL_INDEX,I_Z_MIN
         SF%MASS_FLUX(I_FUEL) = SF%HRRPUA/ (RN%HEAT_OF_COMBUSTION*Y2Y(FUEL_INDEX,I_Z_MIN))
      ENDIF
...
CHARACTER(30) :: PART_ID,SHAPE,MULT_ID
...
NAMELIST /INIT/ XB,TEMPERATURE,DENSITY,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,NUMBER_INITIAL_DROPLETS, &
                PART_ID,SHAPE,DT_INSERT,MULT_ID
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%QUANTITY /= 'null') THEN
      CALL GET_QUANTITY_INDEX(DV%SMOKEVIEW_LABEL,DV%SMOKEVIEW_BAR_LABEL,QUANTITY_INDEX,DV%SPEC_INDEX,DV%PART_INDEX, &
                              DV%DUCT_INDEX,DV%NODE_INDEX,'DEVC',DV%QUANTITY,DV%SPEC_ID,DV%PART_ID,DV%DUCT_ID,DV%NODE_ID)
  ...
  ! Assign properties to the DEVICE array
   ...
   IF (DV%STATISTICS/='null') CALL CHANGE_UNITS(DV%QUANTITY,DV%UNITS,DV%STATISTICS)
...
NAMELIST /ISOF/ QUANTITY,FYI,VALUE,REDUCE_TRIANGLES,COLOR_QUANTITY,SPEC_ID,COLOR_SPEC_ID,VELO_INDEX
...
READ_ISOF_LOOP: DO N=1,N_ISOF
   ...
   CALL GET_QUANTITY_INDEX(IS%SMOKEVIEW_LABEL,IS%SMOKEVIEW_BAR_LABEL,IS%INDEX,IS%SPEC_INDEX,I_DUM,I_DUM,I_DUM,'ISOF',QUANTITY,&
                           SPEC_ID,'null','null','null')
   IF (COLOR_QUANTITY/='null') &
      CALL GET_QUANTITY_INDEX(IS%SMOKEVIEW_LABEL2,IS%SMOKEVIEW_BAR_LABEL2,IS%INDEX2,IS%SPEC_INDEX2,I_DUM,I_DUM,I_DUM,'ISOF', &
                              COLOR_QUANTITY,COLOR_SPEC_ID,'null','null','null')
...
NAMELIST /SLCF/ XB,QUANTITY,FYI,PBX,PBY,PBZ,VECTOR,MESH_NUMBER,RLE,MAXIMUM_VALUE,MINIMUM_VALUE,TWO_BYTE,SPEC_ID, &
                AGL_SLICE,PART_ID,CELL_CENTERED,FIRE_LINE,EVACUATION,VELO_INDEX
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      VECTORLOOP: DO ITER=1,NITER
         ...
         CALL GET_QUANTITY_INDEX(SL%SMOKEVIEW_LABEL,SL%SMOKEVIEW_BAR_LABEL,SL%INDEX,SL%SPEC_INDEX,SL%PART_INDEX,I_DUM,I_DUM, &
                                 'SLCF',QUANTITY,SPEC_ID,PART_ID,'null','null')
...
READ_BNDF_LOOP: DO N=1,N_BNDF
   ...
   CALL GET_QUANTITY_INDEX(BF%SMOKEVIEW_LABEL,BF%SMOKEVIEW_BAR_LABEL,BF%INDEX,BF%SPEC_INDEX,BF%PART_INDEX,I_DUM,I_DUM, &
                           'BNDF',QUANTITY,SPEC_ID,PART_ID,'null','null')
...
OUTPUT_QUANTITY(52)%NAME = 'UBAR'
OUTPUT_QUANTITY(52)%UNITS = 'm/s'
OUTPUT_QUANTITY(52)%SHORT_NAME = 'ubar'
...
SUBROUTINE GET_QUANTITY_INDEX(SMOKEVIEW_LABEL,SMOKEVIEW_BAR_LABEL,OUTPUT_INDEX,SPEC_INDEX,PART_INDEX,DUCT_INDEX,NODE_INDEX,OUTTYPE,&
                              QUANTITY,SPEC_ID_IN,PART_ID,DUCT_ID,NODE_ID)
...
INTEGER, INTENT(OUT) :: OUTPUT_INDEX,SPEC_INDEX,PART_INDEX,DUCT_INDEX,NODE_INDEX
INTEGER :: ND,NS,NN
...
CHARACTER(30) :: SHAPE
REAL(EB) :: XYZ(3),XYZOR(3),XB(6),RADIUS,VELOCITY(3),OMEGA(3),RGB(3),PIXELS
LOGICAL :: HOLE,TWO_SIDED
INTEGER :: IOS,IZERO,N
TYPE(GEOMETRY_TYPE), POINTER :: G
NAMELIST /GEOM/ XB,XYZ,XYZOR,RADIUS,SHAPE,VELOCITY,OMEGA,HOLE,RGB,PIXELS,TWO_SIDED
