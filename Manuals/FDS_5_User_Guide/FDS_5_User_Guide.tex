...
In the model, each reaction can produce a (single) solid {\ct RESIDUE},
...
Each reaction produces a (single) solid {\ct RESIDUE}, water vapor, and/or fuel gas. This information is conveyed
...
For each reaction that each material component undergoes you must specify kinetic parameters of the reaction rate.  The reaction rate at
the temperature, $T_s$, of the $i$th material undergoing its $j$th reaction is given by:
\be
  r_{ij} =
...
It is very important to keep in mind that $A$ and $E$ are not available for most real materials. If $A$ and $E$ are not known, there are several parameters that can
be used by FDS to derive effective values. The most important parameter to specify in place of $A$ and $E$ is
the  {\ct REFERENCE\_TEMPERATURE} ($^\circ$C). To understand this parameter, consider the plots shown in Fig.~\ref{pyrolysis}. These curves represent
\begin{figure}[ht]
\center{\includegraphics[width=4.0in]{FIGURES/pyrolysis_1}}
\caption[Simple demonstration of pyrolysis model.]{The curve at left (blue) shows the mass loss for a solid material
undergoing a slow heating. At right, the green curve shows the reaction rate, which is the first derivative of the
mass loss curve.}
\label{pyrolysis}
\end{figure}
the results of a hypothetical TGA experiment.\footnote{The blue curve (Mass Fraction) in Fig.~\ref{pyrolysis} is the solution, $Y(t)$, of the ordinary differential
equation
\be \frac{dY}{dt} = -A \, Y \, \exp(-E/RT) \quad ; \quad Y(0)=1  \ee
The temperature, $T(t)$ is a linearly increasing function of time, in this case 5~K/min, typical of a TGA experiment. The green
curve (Reaction Rate) represents the function, $-dY/dt$.}
...
FDS.}  Note that the {\ct REFERENCE\_TEMPERATURE} is
{\em not} the same as an ignition temperature, nor is it necessarily the surface temperature of the burning solid. Rather, it is simply
...
Two additional parameters can be specified along with {\ct REFERENCE\_TEMPERATURE} ($T_p$) to help to fine tune the shape of the reaction rate curve,
assuming some sort of experiment has been done to indicate at what temperature, and
over what temperature range, the reaction takes place. First, the {\ct TGA\_HEATING\_RATE} ($\dot{T}$) is the rate at which the
temperature of the TGA (or equivalent) test apparatus was increased, in units
of K/min. Its default value is 5~K/min. In Fig.~\ref{pyrolysis}, the area under the green curve (Reaction Rate) is equal to the heating rate (in units of K/s). Second, the
{\ct TGA\_PYROLYSIS\_RANGE} ($\Delta T$) is the approximate width (in degrees Celsius or Kelvin) of the green curve, assuming its shape to be roughly triangular. Its default value
is 80~$^\circ$C. Using these input parameters, an estimate is made of the peak reaction rate, $r_p$, from which $E$, then $A$, can be calculated to ensure that
the peak reaction rate occurs at the specified {\ct REFERENCE\_TEMPERATURE}.
\begin{eqnarray}
   r_p &=& \frac{2 \, \dot{T}} {\Delta T} \\
   E &=& \frac{r_p \, R \, T_p^2}{Y_p \, \dot{T}} \\
   A &=& \frac{E \, \dot{T}}{R \, T_p^2} \, e^{E/R \, T_p}
\end{eqnarray}
Note that $Y_p$ is the mass fraction (blue curve in Fig.~\ref{pyrolysis}) at the temperature where the reaction rate is its maximum. It is 0.4 by default and not
adjustable.
...
The solid phase reaction represented by Fig.~\ref{pyrolysis} is very simple -- a single, homogenous material is heated and gasified completely. In general, real materials are not
so simple. First, they consist of more than one material, each of which react over a particular temperature interval, and some of which leave behind a solid residue.
Some material components may even undergo multiple reactions that form different residues, like woods that form various amounts of tar, char, and ash, depending on the
rate of heating. Figure~\ref{pyrolysis_2}
demonstrates a more complicated material than the one previously described. It is a hypothetical material that contains 10~\% (by mass) water and 90~\% combustible material.
The water evaporates in the neighborhood of 100~$^\circ$C and the rest pyrolyzes in the neighborhood of 300~$^\circ$C, leaving 20~\% of its mass behind in the form of a
solid residue.

\begin{figure}[ht]
\center{\includegraphics[width=4.0in]{FIGURES/pyrolysis_2}}
\caption[A more complicated demonstration of the pyrolysis model.]{The mass loss (blue curve) and reaction rate (green curve) for a material that
contains 10~\% water (by mass) that evaporates off near 100~$^\circ$C, and 90~\% combustible that pyrolyzes near 300~$^\circ$C, leaving a 20~\% (by mass)
residue behind.}
\label{pyrolysis_2}
\end{figure}

The input lines for this reaction would consist of the parameters listed below.
...
The thermal properties of each material would also be needed for
an actual FDS simulation, but are not relevant in the context of a TGA or similar experiment that involves a slow heating of a small sample
of material.
...
\noindent The full set
of parameters for these cases are listed in {\bf pyrolysis\_1.fds} and {\bf pyrolysis\_2.fds}. Those interested in testing potential solid phase reaction
mechanisms ought to use these test cases as templates.
...
The term ``thermoplastic'' is often used to describe materials that
...
is shut off except for the imposition of a 50~kW/m$^2$ ``external''
heat flux. The solid in this example is a slab of plastic, similar in
composition to PMMA. The solid is described by the following {\ct SURF} line:
...
specified, the {\em yield} of which is 100~\% fuel gas ({\ct NU\_FUEL=1.}).
The phase change from solid to gas {\em consumes}
energy at a rate of 1,500~kJ/kg. Although not relevant in this
example, the burning of these fuel gases would {\em produce} energy at
a rate of 25,000~kJ/kg. The reaction nominally takes place at
...
simple theory should be regarded as producing an upper bound on the burning rate.
...
The slab is originally 0.025~m thick, with a density of 500~kg/m$^3$.
This means there is 12.5~kg/m$^2$ of fuel present, which if divided by the
burning rate (about 0.018~kg/m$^2$/s) yields a burning time of about 700~s.
...
{\ct TGA\_HEATING\_RATE}      & Real array  & See Section~\ref{info:solid_pyrolysis}    & $^\circ$C/min     & 5.     \\ \hline
{\ct TGA\_PYROLYSIS\_RANGE}   & Real array  & See Section~\ref{info:solid_pyrolysis}    & $^\circ$C         & 80.    \\ \hline
\end{longtable}
