...
and 1.31 for a vertical surface, by default) and $C_2$ is the coefficient for forced convection (0.037 by default)~\cite{Holman:1};
...
The Mass Fraction (blue curve) is the normalized density of the material which decreases as the sample is slowly heated,
in this case at a rate
of 5~K/min. The Reaction Rate (green curve) is the rate of change of the mass fraction as a function of time.
...
decreases at its maximum rate within the context of a TGA or similar experimental apparatus. Next, the
{\ct REFERENCE\_RATE} is the reaction rate, in units of s$^{-1)$, corresponding to the {\ct REFERENCE\_TEMPERATURE}. The
kinetic constants for the reaction are then found from the formulae\footnote{These formulas have been derived from an analysis that considers a first-order reaction. When using the proposed method,
do not specify non-unity value for the reaction order {\ct N\_S} on the {\ct MATL} line.}:
\begin{align}
   E &= \frac{e \, r_p}{Y_0} \, \frac{R \, T_p^2}{\dot{T}} \label{E} \\
   A &= \frac{e \, r_p}{Y_0} \, e^{E/R \, T_p} \label{A}
\end{align}
where $T_p$ and $r_p$ are the reference temperature and rate, respectively. The parameter $Y_0$ represents the
mass fraction corresponding to the particular material component undergoing reaction. For a single component, single
reaction material, it is 1. Further discussion of $Y_0$ is found in the next example. The {\ct HEATING\_RATE} ($\dot{T}$) is
the rate at which the temperature of the TGA (or equivalent) test
apparatus was increased, in units of K/min (in the formula, it is expressed in K/s).
...
the formulae in Eqs.~(\ref{E}) and (\ref{A}).}
\label{pyrolysis}
\end{figure} \normalsize
...
\ee
The parameter, $\nu_r$, is the yield of solid residue. When in doubt about the values of
