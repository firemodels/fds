...
REAL(EB) :: XX,YY,ZZ,SHAPE_FACTOR,VOLUME_WEIGHTING_FACTOR(3),EDDY_VOLUME(3),PROFILE_FACTOR,RAMP_T,TSI,&
            VEL_NORMAL,VEL_TANG_1,VEL_TANG_2
...
VENT_LOOP: DO NV=1,N_VENT
   ...
   IF ( VT%BOUNDARY_TYPE==OPEN_BOUNDARY .AND. OPEN_WIND_BOUNDARY ) THEN
      ...
      SELECT CASE(ABS(VT%IOR))
         CASE(1)
            RAMP_T = EVALUATE_RAMP(T,I_RAMP_U0_T)
            VEL_NORMAL = -U0*RAMP_T
            VEL_TANG_1 =  V0*RAMP_T
            VEL_TANG_2 =  W0*RAMP_T
         CASE(2)
            RAMP_T = EVALUATE_RAMP(T,I_RAMP_V0_T)
            VEL_NORMAL = -V0*RAMP_T
            VEL_TANG_1 =  U0*RAMP_T
            VEL_TANG_2 =  W0*RAMP_T
         CASE(3)
            RAMP_T = EVALUATE_RAMP(T,I_RAMP_W0_T)
   ...
   IOR_SELECT: SELECT CASE(ABS(VT%IOR))
      CASE(1)
         EDDY_LOOP_1: DO NE=1,VT%N_EDDY ! loop over eddies
            ...
            IF (I_RAMP_U0_Z>0)                PROFILE_FACTOR = EVALUATE_RAMP(VT%Z_EDDY(NE),I_RAMP_U0_Z)
            IF (SF%RAMP_INDEX(VELO_PROF_Z)>0) PROFILE_FACTOR = EVALUATE_RAMP(VT%Z_EDDY(NE),SF%RAMP_INDEX(VELO_PROF_Z))
      ...
      CASE(2)
         EDDY_LOOP_2: DO NE=1,VT%N_EDDY
            PROFILE_FACTOR = 1._EB
            IF (I_RAMP_V0_Z>0)                PROFILE_FACTOR = EVALUATE_RAMP(VT%Z_EDDY(NE),I_RAMP_V0_Z)
            IF (SF%RAMP_INDEX(VELO_PROF_Z)>0) PROFILE_FACTOR = EVALUATE_RAMP(VT%Z_EDDY(NE),SF%RAMP_INDEX(VELO_PROF_Z))
      ...
      CASE(3)
         EDDY_LOOP_3: DO NE=1,VT%N_EDDY
            PROFILE_FACTOR = 1._EB
            IF (I_RAMP_W0_Z>0)                PROFILE_FACTOR = EVALUATE_RAMP(VT%Z_EDDY(NE),I_RAMP_W0_Z)
            IF (SF%RAMP_INDEX(VELO_PROF_Z)>0) PROFILE_FACTOR = EVALUATE_RAMP(VT%Z_EDDY(NE),SF%RAMP_INDEX(VELO_PROF_Z))
            VT%X_EDDY(NE) = VT%X_EDDY(NE) + DT*VEL_TANG_1*PROFILE_FACTOR
