...
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
INTEGER :: I,SURF_INDEX,IIG,JJG,KKG,IW,NS,N,NWP
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
IF (VIRTUAL_PARTICLES) THEN
   ...
   PARTICLE_LOOP: DO I=1,NLP
      ...
      CALC_LP_MASS:IF (SF%THERMALLY_THICK) THEN
         LP%MASS = 0._EB
         NWP = SUM(LP%ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS))
         SELECT CASE (SF%GEOMETRY)
            CASE (SURF_CARTESIAN)               
               DO N=1,NWP
                  LP%MASS = LP%MASS + SF%LENGTH*SF%WIDTH*(LP%ONE_D%X(N)-LP%ONE_D%X(N-1))*SUM(LP%ONE_D%RHO(N,1:SF%N_MATL))
               ENDDO
             CASE (SURF_CYLINDRICAL)
               DO N=1,NWP
                  LP%MASS = LP%MASS + SF%LENGTH*PI*(LP%ONE_D%X(N)**2-LP%ONE_D%X(N-1)**2)*SUM(LP%ONE_D%RHO(N,1:SF%N_MATL))
               END DO      
            CASE (SURF_SPHERICAL)
               DO N=1,NWP
                  LP%MASS = LP%MASS + FOTHPI*(LP%ONE_D%X(N)**3-LP%ONE_D%X(N-1)**3)*SUM(LP%ONE_D%RHO(N,1:SF%N_MATL))
               END DO      
         END SELECT
...
IF (SF%SHRINK) THEN
   X(0:) => ONE_D%X(0:SF%N_CELLS)
   LAYER_THICKNESS(1:) => ONE_D%LAYER_THICKNESS(1:SF%N_LAYERS)
   N_LAYER_CELLS(1:) => ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS)
ELSE
   X(0:) => SF%X_S(0:SF%N_CELLS)
   LAYER_THICKNESS(1:) => SF%LAYER_THICKNESS(1:SF%N_LAYERS)
   N_LAYER_CELLS(1:) => SF%N_LAYER_CELLS(1:SF%N_LAYERS)
ENDIF
...
COMPUTE_GRID: IF (SF%SHRINK) THEN
   NWP = SUM(ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS))
   CALL GET_WALL_NODE_WEIGHTS(NWP,SF%N_LAYERS,ONE_D%N_LAYER_CELLS(1:SF%N_LAYERS),LAYER_THICKNESS,SF%GEOMETRY, &
