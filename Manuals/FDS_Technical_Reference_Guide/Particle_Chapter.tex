...
\be
   ...
   \end{array} \right.
\ee
where $\NU_0$ is the low mass flux Nusselt number, $\NU$ is the Nusselt number with blowing, $k$ is the thermal conductivity of the gas, and $\PR$ is the molecular Prandtl number, $\PR=\mu_{\rm f} \, c_{\rm p,f}/k_{\rm f}$, with properties evaluated at the film temperature, $T_{\rm f} = T_{\rm s} + \frac{1}{3}(T_{\rm g}-T_{\rm s})$.

In cases where the droplet is actually a portion of the liquid film attached to a solid surface, the surface area of the drop used for heat and mass transfer is calculated using the drop mass, drop density, and film  thickness. 
