...
RSQ_MW_Y(1:N_SPECIES)=1._EB/SQRT(SPECIES(1:N_SPECIES)%MW)
...
ALLOCATE(RSQ_MW_Z(1:N_TRACKED_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ','RSQ_MW_Z',IZERO)

MWR_Z(1:N_TRACKED_SPECIES) = 1._EB/SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%MW
RSQ_MW_Z(1:N_TRACKED_SPECIES) = 1._EB/SQRT(SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%MW)
...
TABLE_LOOP: DO J=1,5000
   ...
   DO N=1,N_TRACKED_SPECIES
      ...
      IF (MU_TMP_Z(N) > 0._EB) THEN
         MU_Z(J,N) = MU_TMP_Z(N)
      ELSE
         MU_Z(J,N) = SUM(Z2Y(:,N) * MU_TMP(:)) / SUM(Z2Y(:,N) * RSQ_MW_Y(:)) * RSQ_MW_Z(N)
      ENDIF
      IF (K_TMP_Z(N) > 0._EB) THEN
         K_Z(J,N)  = K_TMP_Z(N)         
      ELSE
         K_Z(J,N)  = SUM(Z2Y(:,N) * K_TMP(:)) / SUM(Z2Y(:,N) * RSQ_MW_Y(:)) * RSQ_MW_Z(N)
      ENDIF
