...
VOLUME_INSERT_LOOP: DO IB=1,N_INIT
   ...
   ELSEIF (IN%N_PARTICLES_PER_CELL > 0) THEN TOTAL_OR_PER_CELL
      ...
      N_INSERT = 0
      INSERT_VOLUME = 0._EB
      CALL GET_IJK(MIN(X1+MICRON_EB,X2),MIN(Y1+MICRON_EB,Y2),MIN(Z1+MICRON_EB,Z2),NM,XI,YJ,ZK,I1,J1,K1)
      CALL GET_IJK(MAX(X2-MICRON_EB,X1),MAX(Y2-MICRON_EB,Y1),MAX(Z2-MICRON_EB,Z1),NM,XI,YJ,ZK,I2,J2,K2)
      I2 = MIN(I2,IBAR)
      J2 = MIN(J2,JBAR)
      K2 = MIN(K2,KBAR)
...
REAL(EB) :: UBAR,VBAR,WBAR,RVC,UREL,VREL,WREL,QREL,RHO_G,TMP_G,MU_FILM, &
            ...
            GX_LOC,GY_LOC,GZ_LOC,DRAG_MAX(3)=0._EB,SUM_RHO,K_SGS,U_P,L_WAKE,KAPPA_E,KN
REAL(EB), SAVE :: BETA
...
IIX  = FLOOR(XI+.5_EB,EB)
JJY  = FLOOR(YJ+.5_EB,EB)
KKZ  = FLOOR(ZK+.5_EB,EB)
...
SUM_MOMENTUM_LOOP: DO I=1,NLP
   ...
   X_WGT = XI - FLOOR(XI,EB)
   Y_WGT = YJ - FLOOR(YJ,EB)
   Z_WGT = ZK - FLOOR(ZK,EB)
