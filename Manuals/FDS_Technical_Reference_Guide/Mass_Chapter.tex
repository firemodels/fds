...
\be \bp_m  =  \rho T \R \sum_\alpha \frac{Y_\alpha}{W_\alpha} = \frac{\rho T \R}{ \bW }  \label{state} \ee
...
\be  \frac{(\rho Y_\alpha)_{ijk}^{*}-(\rho Y_\alpha)_{ijk}^n}{\dt}
  + \nabla\!\cdot(\overline{\rho Y_\alpha}^{FL} \mathbf{u})_{ijk}^n
  = \nabla\!\cdot (\rho D_\alpha \nabla Y_\alpha)_{ijk}^n \ee
at the predictor step, and
\be \frac{(\rho Y_\alpha)_{ijk}^{n+1}-\ha\left((\rho Y_\alpha)_{ijk}^n
     +(\rho Y_\alpha)_{ijk}^{*}\right)} {\ha \dt}
    + \nabla\!\cdot(\overline{\rho Y_\alpha}^{FL} \mathbf{u})_{ijk}^*
    = \nabla\!\cdot (\rho D_\alpha \nabla Y_\alpha)_{ijk}^{*} \ee
...
\ee
The values for $h_{s,\alpha}$ and $c_{p,\alpha}$ are obtained by table lookup from the NIST-JANAF tables~\cite{NIST_JANAF}.
