...
INTEGRATION_LOOP: DO TIME_ITER = 1,MAX_CHEMISTRY_ITERATIONS
   ...
   ELSE RK2_IF ! RK2 w/ Richardson
      ...
      RICH_EX_LOOP: DO RICH_ITER =1,RICH_ITER_MAX
         ...
         A1 = FUNC_A(ZZ_MIXED,DT_SUB,1,TMP_MIXED_ZONE,EXTINCT,RHO_HAT)
         A2 = FUNC_A(ZZ_MIXED,DT_SUB,2,TMP_MIXED_ZONE,EXTINCT,RHO_HAT)
...
REACTION_LOOP: DO NR = 1,N_REACTIONS
   ...
   RN => REACTION(NR)
   IF (.NOT.RN%FAST_CHEMISTRY .AND. SR < SERIES_REAC) CYCLE REACTION_LOOP
...
DO N=1,N_INC
   DO SR=0,SERIES_REAC ! repeat for the number of serier reactions
   ...
   IF (RN%FAST_CHEMISTRY .AND. RN%HEAT_OF_COMBUSTION > 0._EB) THEN
      DO NS = 0,N_TRACKED_SPECIES
         IF (RN%NU(NS) < 0._EB .AND. NS /= RN%FUEL_SMIX_INDEX) THEN
            ...
            Z_OX(NS) = ZZ_MIXED_IN(NS)/OX_COUNT(NS) ! Mass O2 in cell
            Z_F(RN%FUEL_SMIX_INDEX) = ZZ_MIXED_IN(RN%FUEL_SMIX_INDEX)/FUEL_COUNT(RN%FUEL_SMIX_INDEX)
