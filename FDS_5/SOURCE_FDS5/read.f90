...
CHARACTER(30), INTENT(IN) :: QUANTITY
CHARACTER(6), INTENT(IN) :: OUTTYPE
     ...
     SELECT CASE (TRIM(OUTTYPE))
        ...
        CASE ('PLOT3D')
            IF (OUTPUT_QUANTITY(ND)%SOLID_PHASE) THEN
               WRITE(MESSAGE,'(5A)') 'ERROR: ',TRIM(OUTTYPE),' quantity ',TRIM(QUANTITY), ' not appropriate for gas phase'
               CALL SHUTDOWN(MESSAGE)
            ENDIF
            IF (OUTPUT_QUANTITY(ND)%PART_APPROPRIATE) THEN
               WRITE(MESSAGE,'(5A)') 'ERROR: ',TRIM(OUTTYPE),' quantity ',TRIM(QUANTITY), ' not appropriate as Plot3D'
               CALL SHUTDOWN(MESSAGE)
            ENDIF
        CASE DEFAULT
     END SELECT
...
USE DEVICE_VARIABLES
CHARACTER(30), INTENT(IN) :: PROP_ID
CHARACTER(6), INTENT(IN) :: OUTTYPE
