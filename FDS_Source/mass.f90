...
ENTHALPY_IF: IF (ENTHALPY_TRANSPORT) THEN
   ...
   !$OMP PARALLEL DEFAULT(NONE) &
   !$OMP SHARED(KBAR,JBAR,IBAR,KBM1,JBM1,IBM1,EE,FX,FY,FZ,UU,VV,WW,FLUX_LIMITER,TMP_F,R, &
   !$OMP        N_EXTERNAL_WALL_CELLS,N_INTERNAL_WALL_CELLS,BOUNDARY_TYPE,IJKW,WALL_INDEX,CELL_INDEX, &
   !$OMP        RHO_F,UVW_SAVE, &
   !$OMP        SOLID,RDX,RDY,RDZ,RRN, &
   !$OMP        N_TRACKED_SPECIES,ZZP,ZZ_F,ENTHALPY_SOURCE)
   ...
   !$OMP DO PRIVATE(IW,II,JJ,KK,IOR,IBC,IIG,JJG,KKG,ZZZ,UN,ZZ_GET,CP,E_F)
