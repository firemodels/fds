...
Notice that we can account for volumetric heat sources, below is an example from ({\ct ht3d\_source\_term} file:
\begin{lstlisting}
&INIT XB=-.1,.1,-.1,.1,-.1,.1, HRRPUV=1/
\end{lstlisting}

HT3d can also use internal heat source as shown in the example below:
\begin{lstlisting}
&MATL ID='orange', .../
...
Note that you should not specify the internal heat source at the SURF line.
