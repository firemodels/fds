...
CHARACTER(60) :: INPUT_FILE,OUTPUT_FILE,T2_RAMP
REAL :: ACTIVATION_TEMPERATURE,ALPHA,AREA,A_T,A_V,CHI_R,CONDUIT_DIAMETER,CONDUIT_THICKNESS,C_S,D,DELTA,DT,H, &
        H_K,H_V,JACKET_THICKNESS,K_S,L,L_F,LEAK_AREA,MASS_PER_LENGTH,M_DOT,P,Q,Q_STAR,Q_STEP,R, &
        RADIATIVE_FRACTION,RHO_S,RTI,SCALING_FACTOR,T,t_activation,T_END,TMP_A,TMP_G,T_P,T_CLOCK,U_JET,V,V_DOT,W,W_V
REAL, DIMENSION(10) :: X,Z
REAL, DIMENSION(0:5) :: TMP_RAMP,T_RAMP
CHARACTER(20), DIMENSION(10) :: Z_LABEL
LOGICAL :: PROFILE=.FALSE.,ITER=.TRUE.
INTEGER :: I,IOS
...
NAMELIST /SPKACT/ Q,T2_RAMP,RTI,ACTIVATION_TEMPERATURE,H,R,TMP_A,RADIATIVE_FRACTION,OUTPUT_FILE,SCALING_FACTOR
...
DO WHILE (ITER)
   Q_C = ALPHA * T**2 * (1 - RADIATIVE_FRACTION) * SCALING_FACTOR
   ...
   IF ((((T_JET-273 - TMP_A)/(T_JET-273 - ACTIVATION_TEMPERATURE))<=0) .OR. (t_activation<=0)) THEN
      WRITE(11,'(F6.1,A1,F6.1,A5,F6.1)') T,',',T_JET-273.,',NaN,',Q_C
   ELSE
