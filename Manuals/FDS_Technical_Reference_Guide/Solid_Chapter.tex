...
{\bf Kiyoung Moon and Jung-il Choi, Yonsei University, Seoul, South Korea}\\
...
computing a deposition velocity for each (respectively u$_{th}$, u$_g$, and u$_{dt}$,
a combined diffusion-turbulence velocity), and summing them~\cite{Bixler:1}. If the aerosol
is located in a gas-phase cell adjacent to an upward-facing wall cell (i.e., a flat surface),
then the aerosol is removed from the gas phase at a rate proportional to its deposition
velocity and deposited on the flat surface. The accumulated surface density of deposited
aerosols is tracked.
...
\noindent where $m_a$ is the particle mass, $\chi_d$ is a shape factor, $r_a$ is the particle radius,
and Cn is the Cunningham slip factor given by~\cite{Cunningham:1}
...
\be
\text{Kn} = \frac{0.0065}{r_a} \frac{T_g}{T_\infty}
\ee
...
is also used in the total deposition velocity to deposit aerosols onto flat surfaces,
as described above.
...
u_{dt} = \left\{ \begin{array}{r@{\quad \quad}l}
         ...
         3.5 \times 10^{-4} \; {\tau^+}^2 \; u_{\tau} &  0.2 < \tau^+ < 30 \\
         0.17 \; u_{\tau}                             &  \tau^+ < 30
         \end{array} \right.
\ee

\noindent where $u_{\tau}$ is the wall friction velocity computed by the wall model,
and $\tau^+$ is the dimensionless stopping distance given by~\cite{Ludwig_ICONE}

\be
 \tau^+=\frac{2 \, \rho_a \, r_a}{18 \, \mu^2}  \; u_{\tau}^2 \; \rho_g
\ee
