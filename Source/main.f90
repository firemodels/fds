...
USE LOCMAT_SOLVER, ONLY : ULMAT_SOLVER,ULMAT_SOLVER_SETUP
...
SELECT CASE(TASK_NUMBER)
   ...
   CASE(3)
      ...
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         ALLOCATE(M%SURF_INDEX(0:CELL_COUNT(NM),-3:3),STAT=IZERO) ; M%SURF_INDEX = 0
         ALLOCATE(M%THIN_WALL_INDEX(0:CELL_COUNT(NM),-3:3,1:3),STAT=IZERO) ; M%THIN_WALL_INDEX = 0
         ALLOCATE(M%THIN_SURF_INDEX(0:CELL_COUNT(NM),-3:3,1:3),STAT=IZERO) ; M%THIN_SURF_INDEX = 0
         ALLOCATE(M%THIN_OBST_INDEX(0:CELL_COUNT(NM),-3:3,1:3),STAT=IZERO) ; M%THIN_OBST_INDEX = 0
      ENDDO
      ...
      DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         DO N=1,M%N_NEIGHBORING_MESHES
            ...
            IF (.NOT.ALLOCATED(M4%THIN_WALL_INDEX))    ALLOCATE(M4%THIN_WALL_INDEX(0:CELL_COUNT(NOM),-3:3,1:3))
            IF (.NOT.ALLOCATED(M4%THIN_SURF_INDEX))    ALLOCATE(M4%THIN_SURF_INDEX(0:CELL_COUNT(NOM),-3:3,1:3))
            IF (.NOT.ALLOCATED(M4%THIN_OBST_INDEX))    ALLOCATE(M4%THIN_OBST_INDEX(0:CELL_COUNT(NOM),-3:3,1:3))
            IF (.NOT.ALLOCATED(M4%SURF_INDEX))    ALLOCATE(M4%SURF_INDEX(0:CELL_COUNT(NOM),-3:3))
...
DO NM=1,NMESHES
   ...
   CALL MPI_BCAST(M%THIN_WALL_INDEX,SIZE(M%THIN_WALL_INDEX),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
   CALL MPI_BCAST(M%THIN_SURF_INDEX,SIZE(M%THIN_SURF_INDEX),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
   CALL MPI_BCAST(M%THIN_OBST_INDEX,SIZE(M%THIN_OBST_INDEX),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
   CALL MPI_BCAST(M%SURF_INDEX,SIZE(M%SURF_INDEX),MPI_INTEGER,MPI_COMM_NEIGHBORS_ROOT(NM),MPI_COMM_NEIGHBORS(NM),IERR)
...
INTEGER :: IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,NOM,IOR,IW,N,N_STORAGE_SLOTS,IIO,JJO,KKO,NIC_R,ILC
...
IF (OMESH_PARTICLES) THEN
   ...
   DO ILC=1,N_LAGRANGIAN_CLASSES
      ...
      CALL PACK_PARTICLE(NM,1,ILC,1,.FALSE.,N_INTEGERS=LPC%N_STORAGE_INTEGERS,N_REALS=LPC%N_STORAGE_REALS,&
                         N_LOGICALS=LPC%N_STORAGE_LOGICALS)
      IF (.NOT.ALLOCATED(LPC%PARTICLE_STORAGE%REALS))    ALLOCATE(LPC%PARTICLE_STORAGE%REALS(LPC%N_STORAGE_REALS,1))
      IF (.NOT.ALLOCATED(LPC%PARTICLE_STORAGE%INTEGERS)) ALLOCATE(LPC%PARTICLE_STORAGE%INTEGERS(LPC%N_STORAGE_INTEGERS,1))
      IF (.NOT.ALLOCATED(LPC%PARTICLE_STORAGE%LOGICALS)) ALLOCATE(LPC%PARTICLE_STORAGE%LOGICALS(LPC%N_STORAGE_LOGICALS,1))
   ENDDO
...
DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   DO NOM=1,NMESHES
      ...
      IF (M3%N_WALL_CELLS_SEND>0) ALLOCATE(M3%WALL_SURF_INDICES_SEND(M3%N_WALL_CELLS_SEND))
      IF (M3%N_THIN_WALL_CELLS_SEND>0) ALLOCATE(M3%THIN_WALL_CELL_INDICES_SEND(M3%N_THIN_WALL_CELLS_SEND))
      IF (M3%N_THIN_WALL_CELLS_SEND>0) ALLOCATE(M3%THIN_WALL_SURF_INDICES_SEND(M3%N_THIN_WALL_CELLS_SEND))
   ENDDO
ENDDO
...
DO SURF_INDEX=1,N_SURF
   ...
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,SF%N_WALL_STORAGE_INTEGERS,INTEGER_ONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,SF%N_WALL_STORAGE_LOGICALS,INTEGER_ONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,SF%N_THIN_WALL_STORAGE_REALS   ,INTEGER_ONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,SF%N_THIN_WALL_STORAGE_INTEGERS,INTEGER_ONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
   CALL MPI_ALLREDUCE(MPI_IN_PLACE,SF%N_THIN_WALL_STORAGE_LOGICALS,INTEGER_ONE,MPI_INTEGER,MPI_MAX,MPI_COMM_WORLD,IERR)
ENDDO
...
DO N=0,N_SURF
   ...
   IF (.NOT.ALLOCATED(SF%THIN_WALL_STORAGE%REALS))    ALLOCATE(SF%THIN_WALL_STORAGE%REALS(SF%N_THIN_WALL_STORAGE_REALS,1))
   IF (.NOT.ALLOCATED(SF%THIN_WALL_STORAGE%INTEGERS)) ALLOCATE(SF%THIN_WALL_STORAGE%INTEGERS(SF%N_THIN_WALL_STORAGE_INTEGERS,1))
   IF (.NOT.ALLOCATED(SF%THIN_WALL_STORAGE%LOGICALS)) ALLOCATE(SF%THIN_WALL_STORAGE%LOGICALS(SF%N_THIN_WALL_STORAGE_LOGICALS,1))
ENDDO
...
MESH_LOOP_1A: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   M => MESHES(NM)
   MESH_LOOP_2A: DO NOM=1,NMESHES
      IF (PROCESS(NM)==PROCESS(NOM)) CYCLE MESH_LOOP_2A
      M3 => M%OMESH(NOM)
      IF (M3%N_WALL_CELLS_SEND==0) CYCLE MESH_LOOP_2A
      ALLOCATE(M3%WALL_STORAGE_SEND(N_SURF))
      CALL ALLOCATE_BOUNDARY_TYPES(NOM,50)
      DO SURF_INDEX=1,N_SURF
         SF => SURFACE(SURF_INDEX)
         SLOTS = COUNT(M3%WALL_SURF_INDICES_SEND==SURF_INDEX)
         IF (SLOTS>0) THEN
            ALLOCATE(M3%WALL_STORAGE_SEND(SURF_INDEX)%REALS(SF%N_WALL_STORAGE_REALS,SLOTS))
            ALLOCATE(M3%WALL_STORAGE_SEND(SURF_INDEX)%INTEGERS(SF%N_WALL_STORAGE_INTEGERS,SLOTS))
            ALLOCATE(M3%WALL_STORAGE_SEND(SURF_INDEX)%LOGICALS(SF%N_WALL_STORAGE_LOGICALS,SLOTS))
         ENDIF
      ENDDO
   ENDDO MESH_LOOP_2A
ENDDO MESH_LOOP_1A

MESH_LOOP_1R: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   M => MESHES(NM)
   MESH_LOOP_2R: DO NOM=1,NMESHES
      IF (PROCESS(NM)==PROCESS(NOM)) CYCLE MESH_LOOP_2R
      M3 => M%OMESH(NOM)
      IF (M3%N_WALL_CELLS_RECV==0) CYCLE MESH_LOOP_2R
      ALLOCATE(M3%WALL_STORAGE_RECV(N_SURF))
      CALL ALLOCATE_BOUNDARY_TYPES(NOM,50)
      DO SURF_INDEX=1,N_SURF
         SF => SURFACE(SURF_INDEX)
         SLOTS = COUNT(M3%WALL_SURF_INDICES_RECV(1:M3%N_WALL_CELLS_RECV)==SURF_INDEX)
         IF (SLOTS>0) THEN
            ALLOCATE(M3%WALL_STORAGE_RECV(SURF_INDEX)%REALS(SF%N_WALL_STORAGE_REALS,SLOTS))
            ALLOCATE(M3%WALL_STORAGE_RECV(SURF_INDEX)%INTEGERS(SF%N_WALL_STORAGE_INTEGERS,SLOTS))
            ALLOCATE(M3%WALL_STORAGE_RECV(SURF_INDEX)%LOGICALS(SF%N_WALL_STORAGE_LOGICALS,SLOTS))
            DO I=1,M3%N_WALL_CELLS_RECV
               IF (M3%WALL_SURF_INDICES_RECV(I)/=SURF_INDEX) CYCLE
               CALL ALLOCATE_STORAGE(NOM,WALL_INDEX=I+M3%N_WALL_CELLS_RECV_START-1,SURF_INDEX=SURF_INDEX)
            ENDDO
         ENDIF
      ENDDO
   ENDDO MESH_LOOP_2R
...
MESH_LOOP_1B: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   M => MESHES(NM)
   MESH_LOOP_2B: DO NOM=1,NMESHES
      IF (PROCESS(NM)==PROCESS(NOM)) CYCLE MESH_LOOP_2B
      M3 => M%OMESH(NOM)
      IF (M3%N_THIN_WALL_CELLS_SEND==0 .AND. M3%N_THIN_WALL_CELLS_RECV==0) CYCLE MESH_LOOP_2B
      IF (M3%N_THIN_WALL_CELLS_SEND>0) ALLOCATE(M3%THIN_WALL_STORAGE_SEND(N_SURF))
      IF (M3%N_THIN_WALL_CELLS_RECV>0) ALLOCATE(M3%THIN_WALL_STORAGE_RECV(N_SURF))
      IF (M3%N_THIN_WALL_CELLS_SEND>0 .OR. M3%N_THIN_WALL_CELLS_RECV>0) CALL ALLOCATE_BOUNDARY_TYPES(NOM,50)
      DO SURF_INDEX=1,N_SURF
         SF => SURFACE(SURF_INDEX)
         SLOTS = COUNT(M3%THIN_WALL_SURF_INDICES_SEND==SURF_INDEX)
         IF (SLOTS>0) THEN
            ALLOCATE(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%REALS(SF%N_THIN_WALL_STORAGE_REALS,SLOTS))
            ALLOCATE(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%INTEGERS(SF%N_THIN_WALL_STORAGE_INTEGERS,SLOTS))
            ALLOCATE(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%LOGICALS(SF%N_THIN_WALL_STORAGE_LOGICALS,SLOTS))
         ENDIF
         SLOTS = COUNT(M3%THIN_WALL_SURF_INDICES_RECV(1:M3%N_THIN_WALL_CELLS_RECV)==SURF_INDEX)
         IF (SLOTS>0) THEN
            ALLOCATE(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%REALS(SF%N_THIN_WALL_STORAGE_REALS,SLOTS))
            ALLOCATE(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%INTEGERS(SF%N_THIN_WALL_STORAGE_INTEGERS,SLOTS))
            ALLOCATE(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%LOGICALS(SF%N_THIN_WALL_STORAGE_LOGICALS,SLOTS))
            DO I=1,M3%N_THIN_WALL_CELLS_RECV
               IF (M3%THIN_WALL_SURF_INDICES_RECV(I)/=SURF_INDEX) CYCLE
               CALL ALLOCATE_STORAGE(NOM,THIN_WALL_INDEX=I+M3%N_THIN_WALL_CELLS_RECV_START-1,SURF_INDEX=SURF_INDEX)
            ENDDO
         ENDIF
      ENDDO
   ENDDO MESH_LOOP_2B
...
INTEGER :: RNODE,SNODE,IJK_SIZE,N,NRA,NRA_MAX,LL,AIC,NN,NOM,SURF_INDEX
REAL(EB) :: TNOW
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC
...
RECEIVING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SENDING_MESH_LOOP: DO NN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (CODE==8) THEN
         ...
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%N_THIN_WALL_CELLS_SEND,1,MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==9 .AND. M3%N_WALL_CELLS_SEND>0) THEN
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%WALL_CELL_INDICES_SEND(1),SIZE(M3%WALL_CELL_INDICES_SEND),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%WALL_SURF_INDICES_SEND(1),SIZE(M3%WALL_SURF_INDICES_SEND),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==9 .AND. M3%N_THIN_WALL_CELLS_SEND>0) THEN
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%THIN_WALL_CELL_INDICES_SEND(1),SIZE(M3%THIN_WALL_CELL_INDICES_SEND),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         N_REQ=N_REQ+1
         CALL MPI_IRECV(M3%THIN_WALL_SURF_INDICES_SEND(1),SIZE(M3%THIN_WALL_SURF_INDICES_SEND),MPI_INTEGER,&
                        SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ENDIF

      IF (CODE==10 .AND. M3%N_WALL_CELLS_RECV>0) THEN
         DO SURF_INDEX=1,N_SURF
            IF (.NOT.ALLOCATED(M3%WALL_STORAGE_RECV(SURF_INDEX)%REALS)) CYCLE
            N_REQ6 = N_REQ6 + 1
            CALL MPI_RECV_INIT(M3%WALL_STORAGE_RECV(SURF_INDEX)%REALS,SIZE(M3%WALL_STORAGE_RECV(SURF_INDEX)%REALS),&
                               MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_RECV_INIT(M3%WALL_STORAGE_RECV(SURF_INDEX)%INTEGERS,SIZE(M3%WALL_STORAGE_RECV(SURF_INDEX)%INTEGERS),&
                               MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_RECV_INIT(M3%WALL_STORAGE_RECV(SURF_INDEX)%LOGICALS,SIZE(M3%WALL_STORAGE_RECV(SURF_INDEX)%LOGICALS),&
                               MPI_LOGICAL,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         ENDDO
      ENDIF

      IF (CODE==10 .AND. M3%N_THIN_WALL_CELLS_RECV>0) THEN
         DO SURF_INDEX=1,N_SURF
            IF (.NOT.ALLOCATED(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%REALS)) CYCLE
            N_REQ6 = N_REQ6 + 1
            CALL MPI_RECV_INIT(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%REALS,SIZE(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%REALS),&
                               MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_RECV_INIT(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%INTEGERS,SIZE(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%INTEGERS),&
                               MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_RECV_INIT(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%LOGICALS,SIZE(M3%THIN_WALL_STORAGE_RECV(SURF_INDEX)%LOGICALS),&
                               MPI_LOGICAL,SNODE,NOM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         ENDDO
      ENDIF
      ...
      IF (CODE==11 .AND. OMESH_PARTICLES) THEN
         DO N=1,N_LAGRANGIAN_CLASSES
            ...
            IF (LPC%STATIC .AND. .NOT.LPC%EMBER_PARTICLE) CYCLE
            N_REQ=N_REQ+1
            CALL MPI_IRECV(M3%ADOPT_PARTICLE_STORAGE(N)%REALS(1,1),LPC%N_STORAGE_REALS*M3%N_PART_ADOPT(N), &
                           MPI_DOUBLE_PRECISION,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_IRECV(M3%ADOPT_PARTICLE_STORAGE(N)%INTEGERS(1,1),LPC%N_STORAGE_INTEGERS*M3%N_PART_ADOPT(N), &
                           MPI_INTEGER,SNODE,NOM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_IRECV(M3%ADOPT_PARTICLE_STORAGE(N)%LOGICALS(1,1),LPC%N_STORAGE_LOGICALS*M3%N_PART_ADOPT(N), &
...
INTEGER, INTENT(IN) :: CODE
INTEGER :: NM,NOM,I,II,JJ,KK,LL,LLL,N,RNODE,SNODE,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,IJK_SIZE,N_NEW_STORAGE_SLOTS
INTEGER :: NNN,NN1,NN2,IPC,CNT,II1,II2,JJ1,JJ2,KK1,KK2,NQT2,NN,IOR,NRA,NRA_MAX,AIC,OBST_INDEX,IP,SURF_INDEX,IW
...
SENDING_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   RECEIVING_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      IF (CODE==8) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%N_WALL_CELLS_RECV,1,MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%N_THIN_WALL_CELLS_RECV,1,MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
            M2%N_WALL_CELLS_SEND = M3%N_WALL_CELLS_RECV
            M2%N_THIN_WALL_CELLS_SEND = M3%N_THIN_WALL_CELLS_RECV
         ENDIF
      ENDIF

      IF (CODE==9 .AND. M3%N_WALL_CELLS_RECV>0) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%WALL_CELL_INDICES_RECV,SIZE(M3%WALL_CELL_INDICES_RECV),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%WALL_SURF_INDICES_RECV,SIZE(M3%WALL_SURF_INDICES_RECV),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
            M2%WALL_CELL_INDICES_SEND = M3%WALL_CELL_INDICES_RECV
            M2%WALL_SURF_INDICES_SEND = M3%WALL_SURF_INDICES_RECV
         ENDIF
      ENDIF

      IF (CODE==9 .AND. M3%N_THIN_WALL_CELLS_RECV>0) THEN
         IF (RNODE/=SNODE) THEN
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%THIN_WALL_CELL_INDICES_RECV,SIZE(M3%THIN_WALL_CELL_INDICES_RECV),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
            N_REQ=N_REQ+1
            CALL MPI_ISEND(M3%THIN_WALL_SURF_INDICES_RECV,SIZE(M3%THIN_WALL_SURF_INDICES_RECV),MPI_INTEGER,RNODE,NM,&
                           MPI_COMM_WORLD,REQ(N_REQ),IERR)
         ELSE
            M2=>MESHES(NOM)%OMESH(NM)
            M2%THIN_WALL_CELL_INDICES_SEND = M3%THIN_WALL_CELL_INDICES_RECV
            M2%THIN_WALL_SURF_INDICES_SEND = M3%THIN_WALL_SURF_INDICES_RECV
         ENDIF
      ENDIF

      IF (CODE==10 .AND. M3%N_WALL_CELLS_SEND>0 .AND. RNODE/=SNODE) THEN
         DO SURF_INDEX=1,N_SURF
            IF (.NOT.ALLOCATED(M3%WALL_STORAGE_SEND(SURF_INDEX)%REALS)) CYCLE
            N_REQ6 = N_REQ6 + 1
            CALL MPI_SEND_INIT(M3%WALL_STORAGE_SEND(SURF_INDEX)%REALS,SIZE(M3%WALL_STORAGE_SEND(SURF_INDEX)%REALS),&
                               MPI_DOUBLE_PRECISION,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_SEND_INIT(M3%WALL_STORAGE_SEND(SURF_INDEX)%INTEGERS,SIZE(M3%WALL_STORAGE_SEND(SURF_INDEX)%INTEGERS),&
                               MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_SEND_INIT(M3%WALL_STORAGE_SEND(SURF_INDEX)%LOGICALS,SIZE(M3%WALL_STORAGE_SEND(SURF_INDEX)%LOGICALS),&
                               MPI_LOGICAL,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         ENDDO
      ENDIF

      IF (CODE==10 .AND. M3%N_THIN_WALL_CELLS_SEND>0 .AND. RNODE/=SNODE) THEN
         DO SURF_INDEX=1,N_SURF
            IF (.NOT.ALLOCATED(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%REALS)) CYCLE
            N_REQ6 = N_REQ6 + 1
            CALL MPI_SEND_INIT(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%REALS,SIZE(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%REALS),&
                               MPI_DOUBLE_PRECISION,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_SEND_INIT(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%INTEGERS,SIZE(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%INTEGERS),&
                               MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
            N_REQ6 = N_REQ6 + 1
            CALL MPI_SEND_INIT(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%LOGICALS,SIZE(M3%THIN_WALL_STORAGE_SEND(SURF_INDEX)%LOGICALS),&
                               MPI_LOGICAL,RNODE,NM,MPI_COMM_WORLD,REQ6(N_REQ6),IERR)
         ENDDO
      ENDIF
      ...
      IF (CODE==6 .AND. RNODE/=SNODE .AND. M3%N_WALL_CELLS_SEND>0) THEN
         DO SURF_INDEX=1,N_SURF
            II = 0
            DO I=1,M3%N_WALL_CELLS_SEND
               IF (M3%WALL_SURF_INDICES_SEND(I)/=SURF_INDEX) CYCLE
               IW = M3%WALL_CELL_INDICES_SEND(I)
               II = II + 1
               CALL PACK_WALL(NM,IW,SURF_INDEX,II,UNPACK_IT=.FALSE.,NOM=NOM)
            ENDDO
         ENDDO
      ENDIF

      IF (CODE==6 .AND. RNODE/=SNODE .AND. M3%N_THIN_WALL_CELLS_SEND>0) THEN
         DO SURF_INDEX=1,N_SURF
            II = 0
            DO I=1,M3%N_THIN_WALL_CELLS_SEND
               IF (M3%THIN_WALL_SURF_INDICES_SEND(I)/=SURF_INDEX) CYCLE
               IW = M3%THIN_WALL_CELL_INDICES_SEND(I)
               II = II + 1
               CALL PACK_THIN_WALL(NM,IW,SURF_INDEX,II,UNPACK_IT=.FALSE.,NOM=NOM)
            ENDDO
         ENDDO
      ENDIF
      ...
      IF_SEND_PARTICLES: IF (CODE==11 .AND. OMESH_PARTICLES) THEN
         DO IPC=1,N_LAGRANGIAN_CLASSES
            ...
            IF (SNODE/=RNODE) THEN
               N_REQ=N_REQ+1
               CALL MPI_ISEND(M3%ORPHAN_PARTICLE_STORAGE(IPC)%REALS(1,1),LPC%N_STORAGE_REALS*M3%N_PART_ORPHANS(IPC),&
                              MPI_DOUBLE_PRECISION,RNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
               N_REQ=N_REQ+1
               CALL MPI_ISEND(M3%ORPHAN_PARTICLE_STORAGE(IPC)%INTEGERS(1,1),LPC%N_STORAGE_INTEGERS*M3%N_PART_ORPHANS(IPC),&
                              MPI_INTEGER,RNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
               N_REQ=N_REQ+1
               CALL MPI_ISEND(M3%ORPHAN_PARTICLE_STORAGE(IPC)%LOGICALS(1,1),LPC%N_STORAGE_LOGICALS*M3%N_PART_ORPHANS(IPC),&
                              MPI_LOGICAL,RNODE,NM,MPI_COMM_WORLD,REQ(N_REQ),IERR)
            ELSE
               M2 => MESHES(NOM)%OMESH(NM)
...
RECV_MESH_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
   ...
   SEND_MESH_LOOP: DO NNN=1,M%N_NEIGHBORING_MESHES
      ...
      RECEIVE_BACK_WALL: IF (CODE==6 .AND. SNODE/=RNODE .AND. M2%N_WALL_CELLS_RECV>0) THEN
         DO SURF_INDEX=1,N_SURF
            II = 0
            DO I=1,M2%N_WALL_CELLS_RECV
               IF (M2%WALL_SURF_INDICES_RECV(I)/=SURF_INDEX) CYCLE
               II = II + 1
               CALL PACK_WALL(NM,I+M2%N_WALL_CELLS_RECV_START-1,SURF_INDEX,II,UNPACK_IT=.TRUE.,NOM=NOM)
            ENDDO
         ENDDO
      ENDIF RECEIVE_BACK_WALL

      RECEIVE_BACK_THIN_WALL: IF (CODE==6 .AND. SNODE/=RNODE .AND. M2%N_THIN_WALL_CELLS_RECV>0) THEN
         DO SURF_INDEX=1,N_SURF
            II = 0
            DO I=1,M2%N_THIN_WALL_CELLS_RECV
               IF (M2%THIN_WALL_SURF_INDICES_RECV(I)/=SURF_INDEX) CYCLE
               II = II + 1
               CALL PACK_THIN_WALL(NM,I+M2%N_THIN_WALL_CELLS_RECV_START-1,SURF_INDEX,II,UNPACK_IT=.TRUE.,NOM=NOM)
            ENDDO
         ENDDO
      ...
      IF_RECEIVE_PARTICLES: IF (CODE==11 .AND. OMESH_PARTICLES) THEN
         DO IPC=1,N_LAGRANGIAN_CLASSES
            ...
            IF (LPC%STATIC .AND. .NOT.LPC%EMBER_PARTICLE) CYCLE
            CNT = 0
            DO IP=M%NLP+1,M%NLP+M2%N_PART_ADOPT(IPC)
               CNT = CNT + 1
               CALL PACK_PARTICLE(NM,IP,IPC,CNT,UNPACK_IT=.TRUE.,NOM=NOM)
