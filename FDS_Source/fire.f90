...
REAL(EB) :: ZZ_0(0:N_TRACKED_SPECIES),ZZ_I(0:N_TRACKED_SPECIES),ZZ_N(0:N_TRACKED_SPECIES),DZZDT(0:N_TRACKED_SPECIES),&
            ...
            RATE_CONSTANTE(1:N_REACTIONS),DZZDTE(0:N_TRACKED_SPECIES),DZZDTS(0:N_TRACKED_SPECIES),ZZ_1(0:N_TRACKED_SPECIES),&
            ZZ_2(0:N_TRACKED_SPECIES),DIFF_ZZ(0:N_TRACKED_SPECIES),DIFF_DT,ERR,TOL_INT_VECTOR(1:N_REACTIONS)
INTEGER :: NR,I_TS,NODETS=20,NODETSMAX=10000,NS,ITER,COUNT
TYPE(REACTION_TYPE),POINTER :: RN=>NULL()
...
! Setting sub timestep from error tolerance
DO NR = 1, N_REACTIONS   
   RN => REACTION(NR)
   TOL_INT_VECTOR(NR)=RN%TOL_INT
ENDDO

IF (ALL(TOL_INT_VECTOR < -998._EB)) THEN
   DT_NEW = DT_ODE
ELSE
   DO NR = 1, N_REACTIONS   
      RN => REACTION(NR)    
      CALL COMPUTE_RATE_CONSTANT(NR,RN%MODE,I_TS,Q_OUT,RATE_CONSTANT(NR),ZZ_0,I,J,K)
      DZZDTS = DZZDTS + RN%NU * SPECIES_MIXTURE%MW/SPECIES_MIXTURE(RN%FUEL_SMIX_INDEX)%MW*RATE_CONSTANT(NR)
   END DO 
   ZZ_1 = ZZ_0 + DZZDTS * DT
   ZZ_2 = ZZ_0 + DZZDTS * DT/2._EB
   DIFF_ZZ = ZZ_1 - ZZ_2
   DIFF_DT = DT-DT/2._EB
   ERR = MAXVAL(ABS(DIFF_ZZ/DIFF_DT))
   ITER = CEILING(DT/(MINVAL(ABS(TOL_INT_VECTOR)/ERR)))
   ITER = MIN(MAX(ITER,5),NODETSMAX)
   DT_NEW = DT/REAL(ITER,EB)
ENDIF
...
ODE_LOOP: DO WHILE (DT_SUM < DT)
   ...
   TOLERANCE_LOOP: DO WHILE (TOL_CALC > TOL)
      ...
      REACTION_LOOP: DO NR = 1, N_REACTIONS   
         RN => REACTION(NR)    
         CALL COMPUTE_RATE_CONSTANT(NR,RN%MODE,I_TS,Q_OUT,RATE_CONSTANT(NR),ZZ_I,I,J,K)
         CALL COMPUTE_RATE_CONSTANT(NR,RN%MODE,I_TS,Q_OUT,RATE_CONSTANTE(NR),ZZ_0,I,J,K)
         IF (RATE_CONSTANT(NR) < ZERO_P) CYCLE REACTION_LOOP
      ...
      ZZ_N = ZZ_0 + 0.5_EB * (DZZDT + DZZDTE) * DT_NEW ! Updates species vector - trapezoid     
      Q_SUM = SUM(0.5_EB*(Q_NR+Q_NRE)) * DT_NEW

      IF (ANY(ZZ_N < 0._EB)) THEN !Shrinks time step if negative mass fractions
         DO NS=0,N_TRACKED_SPECIES
            !IF (ZZ_N(NS) < 0._EB .AND. ABS(DZZDT(NS))>ZERO_P) DT_NEW = MIN(DT_NEW,-ZZ_N(NS)/DZZDT(NS))
            IF (ZZ_N(NS) < 0._EB .AND. ABS(DZZDT(NS))>ZERO_P) DT_NEW = 0.95_EB*DT_NEW
         ENDDO
      ELSE
      ...
      IF (COUNT > 500._EB) EXIT TOLERANCE_LOOP
   ...
   !IF (DT_NEW < DT_ODE) DT_NEW = DT_ODE
   !IF (DT_NEW + DT_SUM > DT) DT_NEW = DT - DT_SUM
   IF (DT_NEW < DT/REAL(MIN(MAX(ITER,NODETS),NODETSMAX),EB)) DT_NEW = DT/REAL(MIN(MAX(ITER,NODETS),NODETSMAX),EB)
   IF (DT_NEW + DT_SUM > DT) DT_NEW = DT - DT_SUM
