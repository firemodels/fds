...
The term ``thermoplastic'' is often applied to materials that essentially heat up and gasify without leaving any solid
residue. The term is used here merely to indicate the class of materials for which the pyrolysis can be modeled with a single
reaction that converts solid to gaseous fuel.


\subsection{A Simple Thermoplastic (\texorpdfstring{{\bf thermoplastic}}{thermoplastic}) }
\label{thermoplastic}

The purpose of this example is to demonstrate how the solid phase pyrolysis model works in
FDS. Essentially, the gas phase calculation is shut off except for the imposition
of a 50~kW/m$^2$ ``virtual'' heat flux. The solid in this example is a
slab of plastic, similar in composition to PMMA. The solid is described by the following {\ct SURF} line:
...
\noindent
The {\ct COLOR} is meaningless except to distinguish it in Smokeview. The {\ct EXTERNAL\_FLUX} is a virtual heat source that is only used for these types of
diagnostic exercises. The properties of the material are conveyed via the {\ct MATL} line:
...
The phase change from solid to gas {\em consumes} energy at a rate of 1,578~kJ/kg. Although not relevant in this example, the burning of these fuel gases would {\em produce}
energy at a rate of 25,200~kJ/kg. The reaction nominally takes place at 330~$^\circ$C, a necessary parameter for the pyrolysis model described in Section~\ref{info:solid_pyrolysis}.

The plots below show the surface temperature and heat flux as functions of time.
Note that the default emissivity of the material surface is 0.9, meaning that only
45~kW/m$^2$ out of the initial 50~kW/m$^2$ is actually heating the solid. Some of this energy
heats up the solid to about 350~$^\circ$C at the surface, some is radiated away (about 8~kW/m$^2$), some is
used to convert the solid material into a gas, and is plotted here as the product of the burning rate and the heat of reaction.
