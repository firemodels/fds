...
computed. A two-step chemical reaction with an additional lumped species can also be used with the first step being oxidation of fuel to
carbon monoxide and the second step the oxidation of carbon monoxide to carbon dioxide.
The four lumped species for the two step reaction are air, fuel plus an optional diluent, and products of incomplete combustion
({\em i.e.} products of the first step), and products of complete combustion (i.e. products of the second step).  The mass fractions of all of the major
...
for Information Interchange. There are 256 characters that make up the standard ASCII text.} text file containing parameters organized into {\em namelist}
...
a better strategy is to coarsen the grid resolution while keeping the CFL $\approx 1$.
...
model the interaction of the entire HVAC system with the fire, read Section~\ref{info:HVAC} for a description of the parameters that
describe the components of the entire HVAC system.
...
If the louvered vent is part of an HVAC system, please see \ref{info:HVAClouvers} for details on how to specify the louver.
...
HVAC components such as fans and binary dampers (fully open or fully closed) can be included in the HVAC network and are coupled to the FDS control function capability.  The
...
\item[{\ct ID}] is a character string giving a name to the component.  The name must be unique amongst all other components of that type; however, the same name can be given to components of different types ({\em i.e.} a duct and a node could have the same name but two ducts could not).
\end{description}
...
\item[{\ct CTRL\_ID}] is the {\ct ID} of a {\ct CTRL} function for a damper (open/closed) or fan (on/off) in the duct.  Only one of {\ct CTRL\_ID} or {\ct DEVC\_ID} can be defined.
...
\item[{\ct FAN\_ID}] is the {\ct ID} of a fan located in the duct. Only one of either {\ct DAMPER} or {\ct FAN\_ID} can be specified for a single duct.
...
\item[{\ct RAMP\_ID}] gives the {\ct ID} of a {\ct RAMP} giving a time-dependent multiplier to {\ct VOLUME\_FLOW}.
\item[{\ct REVERSE}]  is a logical variable that when {\ct .TRUE.} indicates that the specified {\ct FAN\_ID} or {\ct VOLUME\_FLOW} blows from the second node to the first node.
...
separate ``pressure zones.'' Two HVAC ducts are defined.  One is a quadratic fan with a maximum volumetric flow of 0.16 $m^3$/s and a stall pressure of 10 Pa.  The second is open duct with a flow loss of 10. The relevant input lines are:
...
&HVAC ID='LEFT',TYPE_ID='DUCT',NODE_ID='LEFT 1','LEFT 2',LENGTH=1,AREA=0.04,FAN_ID='LEFT',REVERSE=.TRUE.,LOSS=0,0/
...
The solution is $p_2=4.5$~Pa and $p_1=-4.5$~Pa (see Fig.~\ref{fan_test}). Note that the sign of the Volume Flow of a duct in FDS has to do with whether the flow is moving from the first node to the second (postive) or the second node to the first (negative). This convention can make these types of calculations a bit tricky.
...
is blown into Zone~1 at a constant rate for 5~s, increasing its pressure approximately 9500~Pa. At 10~s, Zone~1 is opened to Zone~2, decreasing the
...
\footnotesize
\begin{verbatim}
&PART ID='particles', DIAMETER=750., DENSITY=1000., EVAPORATE=.FALSE. /
&SURF ID='SLOT', PART_ID='particles', VEL=-5., PARTICLE_MASS_FLUX=0.1 /
&OBST XB=-0.2,0.2,-0.2,0.2,4.0,4.4, SURF_IDS='INERT','SLOT','INERT' /
\end{verbatim} \normalsize

\noindent
creates an obstruction that ejects non-evaporating particles with a mean volumetric diameter of 750~$\mu$m and density of 1000~kg/m$^3$ out of its sides at a rate
...
The radiative properties of the water and fuel droplets are determined automatically.
...
The real part of the refractive index should be a positive number. If it is greater than 10.0, the particles are treated as perfectly reflecting spheres. The complex part should be a non-negative number. Values less than 1.0E-6 are treated as non-absorbing.
Below is an example of the use of spectral property table, listing the properties at wavelengths 1, 5 and 10 $\mu$m.

\footnotesize
\begin{verbatim}
&PART ID='particles', DIAMETER=750., DENSITY=1000.,
      EVAPORATE=.FALSE., RADIATIVE_PROPERTY_TABLE = 'table' /
...
In this case the user should also specify {\ct SURFACE\_TENSION} of the liquid and breakup distribution parameters {\ct BREAKUP\_CHILD\_DIAMETER} and {\ct BREAKUP\_CHILD\_GAMMA\_D}.
...
{\ct 'PARTICLE\_TEMPERATURE'} for a non-massless particle refers to the temperature of the particle itself
...
&PART ID='heptane droplets', SPEC_ID='N-HEPTANE',
      QUANTITIES(1:2)='DIAMETER','PARTICLE_TEMPERATURE',
...
in buckets\footnote{Be aware of the fact that the default behavior for droplets hitting the ``floor,'' that is, the plane $z=\hbox{\ct ZMIN}$,
...
the simulation for the purpose of saving CPU time.} and the total mass determined at the end of a given time period. The cooling of a solid surface by droplets of a
...
{\ct PARTICLE FLUX X}$^{**}$                     & See Section~\ref{info:part_output}            & kg/m$^2$/s     & P,S          \\ \hline
{\ct PARTICLE FLUX Y}$^{**}$                     & See Section~\ref{info:part_output}            & kg/m$^2$/s     & P,S          \\ \hline
{\ct PARTICLE FLUX Z}$^{**}$                     & See Section~\ref{info:part_output}            & kg/m$^2$/s     & P,S          \\ \hline
...
{\ct EVACUATION}     & Logical     & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} &    & {\ct .FALSE.}    \\ \hline
...
{\ct OUTPUT}                & Logical         & See Section~\ref{info:out:DEVC}                                  &       & {\ct .TRUE.}  \\ \hline
{\ct PART\_ID}              & Character       & See Section~\ref{info:outputquantities}                          &       &               \\ \hline
...
{\ct ROTATION}              & Real            & See Section~\ref{info:DEVC}                                      & deg.  & 0             \\ \hline
{\ct SETPOINT}              & Real            & See Section~\ref{info:basic_control}                             &       &               \\ \hline
{\ct SMOOTHING\_FACTOR}     & Real            & See Section~\ref{info:basic_control}                             &       & 0             \\ \hline
{\ct SPEC\_ID}              & Character       & See Section~\ref{info:outputquantities}                          &       &               \\ \hline
...
{\ct MAXIMUM\_PARTICLES}             & Integer      & See Section~\ref{info:DUMP}       &           & 500000                         \\ \hline
...
{\ct SIG\_FIGS}                     & Integer      & See Section~\ref{info:SIG_FIGS}   &           & 8                              \\ \hline
{\ct SIG\_FIGS\_EXP}                & Integer      & See Section~\ref{info:SIG_FIGS}   &           & 3                              \\ \hline
...
{\ct EVACUATION}   & Logical           & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide}     &       &           \\ \hline
{\ct MESH\_ID }    & Character         & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide}     &       &           \\ \hline
...
\hline \hline
\endhead
{\ct AIRCOIL\_ID}               & Character         & See Section~\ref{info:HVACduct} &               &        \\ \hline
{\ct AMBIENT}                   & Logical           & See Section~\ref{info:HVACnode} &               & {\ct .FALSE.}  \\ \hline
{\ct AREA}                      & Real              & See Section~\ref{info:HVACduct} & m$^2$         &        \\ \hline
{\ct CLEAN\_LOSS}               & Real              & See Section ~\ref{info:HVACfilter}                                    &               &        \\ \hline
{\ct COOLANT\_CP}               & Real              & See Section ~\ref{info:HVACaircoil}   & kJ/kg/K       &        \\ \hline
{\ct COOLANT\_MDOT}             & Real              & See Section ~\ref{info:HVACaircoil}   & kg/s          &        \\ \hline
{\ct COOLANT\_TEMPERATURE}      & Real              & See Section ~\ref{info:HVACaircoil}    & $^\circ$C             &         \\ \hline
{\ct CTRL\_ID}                  & Character         & See Sections ~\ref{info:HVACduct}, ~\ref{info:HVACfan}, and~\ref{info:HVACfilter}  &               &        \\ \hline
{\ct DAMPER}                    & Logical           & See Section~\ref{info:HVACduct}                       &               & {\ct .FALSE.}  \\ \hline
{\ct DEVC\_ID}                  & Character         & See Sections ~\ref{info:HVACduct}, ~\ref{info:HVACfan}, and~\ref{info:HVACfilter}   &               &        \\ \hline
{\ct DIAMETER}                  & Real              & See Section~\ref{info:HVACduct}                                &  m            &        \\ \hline
{\ct DUCT\_ID}                  & Character Array   & See Section~\ref{info:HVACnode}                 &               &        \\ \hline
{\ct EFFICIENCY}                & Real Array        &  See Sections ~\ref{info:HVACfilter} and ~\ref{info:HVACaircoil}         &               & 1.0    \\ \hline
{\ct FAN\_ID}                   & Character         & See Section~\ref{info:HVACduct}                           &               &        \\ \hline
{\ct FILTER\_ID}                & Character         & See Section~\ref{info:HVACnode}                           &               &        \\ \hline
{\ct FIXED\_Q}                  & Real              & See Section ~\ref{info:HVACaircoil}     & kW            &        \\ \hline
{\ct ID}                        & Character         & Name of component                                 &               &        \\ \hline
{\ct LENGTH}                    & Real              & See Section~\ref{info:HVACduct}                                  &  m            &        \\ \hline
{\ct LOADING}                   & Real Array        &  See Section ~\ref{info:HVACfilter}         & kg            & 0.0    \\ \hline
{\ct LOADING\_MULTIPLIER}       & Real Array        &  See Section ~\ref{info:HVACfilter}         & kg            & 0.0    \\ \hline
{\ct LOSS}                      & Real Array        & See Sections ~\ref{info:HVACduct}, ~\ref{info:HVACnode}, ~\ref{info:HVACfan}, and~\ref{info:HVACfilter}  &               & 0.0    \\ \hline
{\ct MAX\_FLOW}                 & Real              & See Section ~\ref{info:HVACfan}   &  m$^3$/s      &        \\ \hline
{\ct MAX\_PRESSURE}             & Real              &  See Section ~\ref{info:HVACfan}                 &  Pa           &        \\ \hline
{\ct NODE\_ID}                  & Character Doublet & See Section~\ref{info:HVACduct}                 &               &         \\ \hline
{\ct RAMP\_ID}                  & Character         & See Sections ~\ref{info:HVACduct}, ~\ref{info:HVACfilter}, and~\ref{info:HVACfan}  &               &          \\ \hline
{\ct REVERSE}                   & Logical           & See Section~\ref{info:HVACduct} &               & {\ct .FALSE.}  \\ \hline
{\ct ROUGHNESS}                 & Real              & See Section~\ref{info:HVACduct}                      &  m            & 0.0    \\ \hline
{\ct SPEC\_ID}                  & Character         & See Section ~\ref{info:HVACfilter}            &               &        \\ \hline
{\ct TAU\_FAN}                  & Real              & See Section ~\ref{info:HVACfan}                                & s             & 1.0    \\ \hline
{\ct TAU\_VF}                   & Real              & See Section~\ref{info:HVACduct}                  & s             & 1.0    \\ \hline
{\ct TYPE\_ID}                  & Character         & See Section~\ref{info:HVAC}                                 &               &        \\ \hline
{\ct VENT\_ID}                  & Character         & See Section~\ref{info:HVACnode}   &               &        \\ \hline
{\ct VOLUME\_FLOW}              & Real              & See Section~\ref{info:HVACduct} and ~\ref{info:HVACfan} &  m$^3$/s      &                       \\ \hline
{\ct XYZ}                       & Real Triplet      & See Section~\ref{info:HVACnode}                             &  m            &        \\ \hline
\end{longtable}
...
{\ct MULT\_ID }                 & Character         & See Section~\ref{info:MULT}                       &               &               \\ \hline
{\ct NUMBER\_INITIAL\_PARTICLES} & Integer           & See Section~\ref{info:initial_droplets}           &               & 0             \\ \hline
...
{\ct EVAC\_HUMANS}      & Logical                       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} &    & {\ct .FALSE.}    \\ \hline
{\ct EVACUATION}        & Logical                       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} &    & {\ct .FALSE.}    \\ \hline
{\ct EVAC\_Z\_OFFSET}   & Real                          & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} & m  & 1                \\ \hline
{\ct ID}                & Character                     & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} &    &                  \\ \hline
...
{\ct \footnotesize ALLOW\_SURFACE\_PARTICLES}  & Logical & See Section~\ref{info:surface_droplets}           &               & {\ct .TRUE.}    \\ \hline
{\ct \footnotesize ALLOW\_UNDERSIDE\_PARTICLES}  & Logical & See Section~\ref{info:surface_droplets}         &               & {\ct .FALSE.}    \\ \hline
...
{\ct C\_DEARDORFF}              & Real          & See Section~\ref{info:LES}                                &               & 0.1               \\ \hline
{\ct C\_SMAGORINSKY}            & Real          & See Section~\ref{info:LES}                                &               & 0.20              \\ \hline
...
{\ct C\_G}                      & Real          & See Section~\ref{info:LES}                                &               & 0.04              \\ \hline
...
{\ct C\_VREMAN}                 & Real          & See Section~\ref{info:LES}                                &               & 0.07              \\ \hline
...
{\ct EVACUATION\_DRILL}         & Logical       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide}     &               & {\ct .FALSE.}     \\ \hline
{\ct EVACUATION\_MC\_MODE}      & Logical       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide}     &               & {\ct .FALSE.}     \\ \hline
{\ct EVAC\_PRESSURE\_ITERATIONS}& Integer       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide}     &               & 50                \\ \hline
{\ct EVAC\_TIME\_ITERATIONS}    & Integer       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide}     &               & 50                \\ \hline
{\ct FLUX\_LIMITER}             & Integer       & See FDS Tech Guide~\cite{FDS_Tech_Guide}                  &               & 4                 \\ \hline
...
{\ct MAXIMUM\_VISIBILITY}       & Real          & See Section~\ref{info:visibility}                         &  m            &  30               \\ \hline
...
{\ct NO\_EVACUATION}            & Logical       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide}     &               & {\ct .FALSE.}     \\ \hline
...
{\ct TURBULENCE\_MODEL}         & Character     & See Section~\ref{info:LES}                                &               & {\ct 'DEARDORFF'} \\ \hline
{\ct U0,V0,W0}                  & Reals         & See Section~\ref{info:MISC_Basics}                        & m/s           & 0.                \\ \hline
{\ct VISIBILITY\_FACTOR}        & Real          & See Section~\ref{info:visibility}                         &               & 3                 \\ \hline
...
{\ct COMPLEX\_REFRACTIVE\_INDEX}    & Real            & See Section~\ref{radiative_part_props}      &           & 0.01          \\ \hline
...
{\ct RADIATIVE\_PROPERTY\_TABLE}    & Real            & See Section~\ref{radiative_part_props}      &           &               \\ \hline
{\ct REAL\_REFRACTIVE\_INDEX}       & Real            & See Section~\ref{radiative_part_props}      &           & 1.33          \\ \hline
...
% Undocumented: SURFACE_TENSION,BREAKUP,BREAKUP_CHILD_DIAMETER,BREAKUP_CHILD_GAMMA_D,BREAKUP_CHILD_SIGMA_D,
...
{\ct PARTICLES\_PER\_SECOND}             & Integer       & See Section~\ref{info:sprinklers}             &                       & 5000      \\ \hline
{\ct PARTICLE\_VELOCITY}                 & Real          & See Section~\ref{info:sprinklers}             & m/s                   & 0.       \\ \hline
...
{\ct NUMBER\_RADIATION\_ANGLES}     & Integer   & See Section~\ref{info:RADI_Resolution}        &                   & 100           \\ \hline
...
\hline \hline
\endhead
{\ct A}                                   & Real        & See Section~\ref{info:finite}                 &   cm$^3$/mol/s    &     \\ \hline
{\ct AUTO\_IGNITION\_TEMPERATURE}         & Real        & See Section~\ref{info:extinction}             &   K               &  0  \\ \hline
{\ct BETA\_EDC}                           & Real        & See Section~\ref{info:turbulent_combustion}   &                   & 1.0    \\ \hline
{\ct C}                                   & Real        & See Section~\ref{info:simple_chemistry}       &                   & 3    \\ \hline
{\ct CHECK\_ATOM\_BALANCE}                & Logical     & See Section~\ref{info:REAC_Diagnostics}       &                   & {\ct .TRUE.}    \\ \hline
%{\ct CO\_PRODUCTION}                      & Logical     & See Section~\ref{info:CO}                     &                   & {\ct .FALSE.}     \\ \hline
{\ct CO\_YIELD}                           & Real        & See Section~\ref{info:simple_chemistry}       & kg/kg             & 0        \\ \hline
{\ct CRITICAL\_FLAME\_TEMPERATURE}        & Real        & See Section~\ref{info:extinction}             &   $^\circ$C       & 1427    \\ \hline
{\ct E}                                   & Real        & See Section~\ref{info:finite}                 &   kJ/kmol         &     \\ \hline
{\ct EPUMO2}                              & Real        & See Section~\ref{info:heat_of_combustion}     &   kJ/kg           & 13100    \\ \hline
{\ct EQUATION}                            & Character   & See Section~\ref{info:EQUATION}               &                   &          \\ \hline
{\ct FORMULA}                             & Character   & See Section~\ref{info:simple_chemistry}       &                   &  \\ \hline
{\ct FUEL}                                & Character   & See Section~\ref{info:simple_chemistry}       &                   &   \\ \hline
{\ct H}                                   & Real        & See Section~\ref{info:simple_chemistry}       &                   & 8    \\ \hline
{\ct HEAT\_OF\_COMBUSTION}                & Real        & See Section~\ref{info:heat_of_combustion}     & kJ/kg             &         \\ \hline
{\ct HRRPUA\_SHEET}                       & Real        & See Section~\ref{info:REAC_Limiters}          & kW/m$^2$          &         \\ \hline
{\ct HRRPUV\_AVERAGE}                     & Real        & See Section~\ref{info:REAC_Limiters}          & kW/m$^3$          &         \\ \hline
{\ct HUMIDITY}                            & Real        & See Section~\ref{info:simple_chemistry}       & \%                & 40.               \\ \hline
{\ct ID}                                  & Character   & See Section~\ref{info:simple_chemistry}       &                   &     \\ \hline
{\ct IDEAL}                               & Logical     & See Section~\ref{info:simple_chemistry}       &                   & {\ct .FALSE.}    \\ \hline
{\ct N}                                   & Real        & See Section~\ref{info:simple_chemistry}       &                   & 0    \\ \hline
{\ct NU(:)}                               & Real Array  & See Section~\ref{info:finite}                 &                   &     \\ \hline
{\ct N\_S(:)}                             & Real Array  & See Section~\ref{info:finite}                 &                   &     \\ \hline
{\ct N\_T}                                & Real        & See Section~\ref{info:finite}                 &                   &     \\ \hline
{\ct O}                                   & Real        & See Section~\ref{info:simple_chemistry}       &                   & 0    \\ \hline
{\ct REAC\_ATOM\_ERROR}                   & Real        & See Section~\ref{info:REAC_Diagnostics}       & atoms             & 1.E-5    \\ \hline
{\ct REAC\_MASS\_ERROR}                   & Real        & See Section~\ref{info:REAC_Diagnostics}       & kg/kg             & 1.E-4    \\ \hline
{\ct SMIX\_ID(:)}                         & Char.~Array & See Section~\ref{info:SMIX}                   &                   &     \\ \hline
{\ct SPEC\_ID(:)}                         & Char.~Array & See Section~\ref{info:SMIX}                   &                   &     \\ \hline
{\ct SOOT\_H\_FRACTION}                   & Real        & See Section~\ref{info:simple_chemistry}       &                   & 0.1    \\ \hline
{\ct SOOT\_YIELD}                         & Real        & See Section~\ref{info:simple_chemistry}       & kg/kg             & 0.01    \\ \hline
{\ct SUPPRESSION}                         & Logical     & See Section~\ref{info:extinction}             &                   & {\ct .TRUE.}      \\ \hline
{\ct THRESHOLD\_TEMPERATURE}              & Real        & See Section~\ref{info:mixedmode}              &   $^\circ$C       & 1427    \\ \hline
{\ct Y\_CO2\_INFTY}                       & Real        & See Section~\ref{info:simple_chemistry}       &  kg/kg            & 0.0058 \\ \hline
{\ct Y\_O2\_INFTY}                        & Real        & See Section~\ref{info:simple_chemistry}       &  kg/kg            & 0.232428\\ \hline
...
{\ct QUANTITY}          & Character         & See Section~\ref{info:outputquantities}   &           &                   \\ \hline
{\ct QUANTITY2}         & Character         & See Section~\ref{info:outputquantities}   &           &                   \\ \hline
{\ct SPEC\_ID}          & Character         & See Section~\ref{info:outputquantities}   &           &                   \\ \hline
...
{\ct DENSITY\_LIQUID}               & Real        & See Section~\ref{thermal_part_props}         & kg/m$^3$          &               \\ \hline
...
{\ct HUMIDITY}                      & Real        & See Section~\ref{info:SPEC_Basics}          & \%                & 40.           \\ \hline
...
{\ct REFERENCE\_ENTHALPY}            & Real        & See Section~\ref{gas_species_props}         & kJ/kg             &               \\ \hline
...
{\ct VAPORIZATION\_TEMPERATURE}     & Real            & See Section~\ref{thermal_part_props}        & K         &               \\ \hline
...
{\ct EVAC\_DEFAULT}                   & Logical         & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} &             & {\ct .FALSE.}           \\ \hline
...
{\ct REGRID\_FACTOR}                  & Real            & See Section~\ref{info:solid_phase_stability}  &                     & 0.9                     \\ \hline
{\ct ROUGHNESS}                       & Real            & See Section~\ref{info:WALL_MODEL}             & m                   & 0.                      \\ \hline
{\ct MINIMUM\_LAYER\_THICKNESS}       & Real            & See Section~\ref{info:solid_phase_stability}  & m                   & 1.E-6                   \\ \hline
...
{\ct EVAC\_DT\_FLOWFIELD}       & Real       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} & s   &  0.01           \\ \hline
{\ct EVAC\_DT\_STEADY\_STATE}   & Real       & See FDS+Evac User's Guide~\cite{FDS_Evac_Users_Guide} & s   &  0.05           \\ \hline
...
{\ct UVW(3) }               & Real Triplet      & See Section~\ref{info:HVAClouvers}                                 & m/s           &                     \\ \hline
...
{\ct PART}    & {\ct PARTICLES\_PER\_SECOND}         &  {\ct PROP}  & {\ct PARTICLES\_PER\_SECOND}                 & See Section~\ref{info:sprinkler_droplets}    \\ \hline
...
{\ct PART}    & {\ct NUMBER\_INITIAL\_PARTICLES}     &  {\ct PROP}  & {\ct NUMBER\_INITIAL\_PARTICLES}             & See Section~\ref{info:initial_droplets}      \\ \hline
...
The geometry file contains a description of immersed geometrical objects.  These objects are
defined in terms of nodes and triangles.  A node consists of an $(x,y,z)$ coordinate.
A triangle
consists of 3 nodes.  The file format allows one to specify objects that change with time.
...
These files are written out from {\bf dump.f} with lines equivalent to the following:
...
! dynamic geometry
  ...
  IF(GEOM_TYPE.EQ.0)THEN
    ...
    IF (N_VERT_D>0) WRITE(LU_GEOM) (Xvert_D(I),Yvert_D(I),Zvert_D(I),SURF_S(I),I=1,NVERT_D)
    IF (N_FACE_D>0) WRITE(LU_GEOM) (FACE1_D(I),FACE2_D(I),FACE3_D(I),I=1,NFACE_D)
    IF (N_FACE_D>0) WRITE(LU_GEOM) (SURF_D(I),I=1,NFACE_D)
...
\begin{verbatim}
  ...
  WRITE(LU_GEOM_DATA) N_VERT_S_VALS,N_VERT_D_VALS,N_FACE_S_VALS,NFACE_D_VALS
