...
SPECIES_LOOP: DO N=1,N_TOTAL_SCALARS
   ...
   !$OMP PARALLEL DO PRIVATE(IW,WC,BC,P1,II,JJ,KK,IIG,JJG,KKG,IOR,U_TEMP,Z_TEMP,F_TEMP)
   WALL_LOOP_2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      ELSE
         SELECT CASE(IOR)
            CASE( 1); FX(IIG-1,JJG,KKG,N) = P1%RHO_F*P1%ZZ_F(N)
            CASE(-1); FX(IIG,JJG,KKG,N)   = P1%RHO_F*P1%ZZ_F(N)
            CASE( 2); FY(IIG,JJG-1,KKG,N) = P1%RHO_F*P1%ZZ_F(N)
            CASE(-2); FY(IIG,JJG,KKG,N)   = P1%RHO_F*P1%ZZ_F(N)
            CASE( 3); FZ(IIG,JJG,KKG-1,N) = P1%RHO_F*P1%ZZ_F(N)
            CASE(-3); FZ(IIG,JJG,KKG,N)   = P1%RHO_F*P1%ZZ_F(N)
         END SELECT
      ENDIF
