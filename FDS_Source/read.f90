...
NAMELIST /PART/ FYI,SAMPLING_FACTOR,ID,STATIC,MASSLESS,TREE, &
                DIAMETER,DENSITY,MAXIMUM_DIAMETER,MINIMUM_DIAMETER,GAMMA_D,HEAT_OF_COMBUSTION, &
                ...
                DRAG_LAW,USER_DRAG_COEFFICIENT, &
...
READ_PART_LOOP: DO N=1,N_PART
   ...
   IF (PC%SPEC_ID/='null') THEN
      ...
      DO NN=1,N_SPECIES
         ...
         ENDIF
      ENDDO
      DENSITY=SPECIES(PC%Y_INDEX)%DENSITY_LIQUID
   ENDIF
...
INTEGER :: N, J, ITMP,Y_INDEX
LOGICAL :: SPECIES_COMPUTED(1:N_SPECIES)
...
PART_LOOP: DO N=1,N_PART
   ...
   IF (SPECIES_COMPUTED(PC%Y_INDEX)==.TRUE.) CYCLE PART_LOOP
   ...
   DO J = 1, 5000
      ...
      ELSE
         CALL JANAF_TABLE_LIQUID (J,SS%C_P_L(J),H_V,H_L,TMP_REF,TMP_MELT,TMP_V,SS%ID,PC%FUEL)
         IF (J==1) THEN         
