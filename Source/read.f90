...
NAMELIST /MISC/ AGGLOMERATION,AEROSOL_AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,&
                ...
                IBLANK_SMV,IMMERSED_BOUNDARY_METHOD,IBM_USE_OUTER_PLANE,&
...
NAMELIST /DEVC/ ABSOLUTE_VALUE,BYPASS_FLOWRATE,CONVERSION_FACTOR,CONVERSION_ADDEND,COORD_FACTOR,CELL_L,CTRL_ID,DELAY,DEPTH,&
                ...
                SPATIAL_STATISTIC,SPEC_ID,STATISTICS,STATISTICS_START,SURF_ID,TEMPORAL_STATISTIC,&
...
TEMPORAL_STATISTIC= 'TIME AVERAGE'
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   SPECIAL_QUANTITIES: SELECT CASE (DV%QUANTITY)
      ...
      CASE ('LAYER HEIGHT','UPPER TEMPERATURE','LOWER TEMPERATURE')
         ...
         IF (DV%MESH>0) THEN
            M  => MESHES(DV%MESH)
            DV%SUBDEVICE(1)%K1 = MAX(1     ,DV%SUBDEVICE(1)%K1)
            DV%SUBDEVICE(1)%K2 = MIN(M%KBAR,DV%SUBDEVICE(1)%K2)
         ENDIF
      ...
      CASE ('FED')
         IF (DV%SPATIAL_STATISTIC/='null' .OR. (DV%TEMPORAL_STATISTIC/='null' .AND. DV%TEMPORAL_STATISTIC/='TIME INTEGRAL')) THEN
            WRITE(MESSAGE,'(A)') 'ERROR: Only TIME INTEGRAL statistic can be used with FED devices'
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
   ...
   IF ((DV%TEMPORAL_STATISTIC=='RMS' .OR. &
        DV%TEMPORAL_STATISTIC=='COV' .OR. &
        DV%TEMPORAL_STATISTIC=='CORRCOEF') .AND. DV%LINE==0) THEN
      DV%TIME_AVERAGED=.FALSE.
      IF (DV%STATISTICS_START < T_BEGIN) DV%STATISTICS_START = T_BEGIN
   ENDIF

   IF (DV%TEMPORAL_STATISTIC=='TIME AVERAGE' .AND. .NOT.OUTPUT_QUANTITY(DV%OUTPUT_INDEX)%TIME_AVERAGED) DV%TEMPORAL_STATISTIC='null'
...
INTEGER :: N,VELO_INDEX,SKIP
TYPE(ISOSURFACE_FILE_TYPE), POINTER :: IS=>NULL()
NAMELIST /ISOF/ FYI,QUANTITY,SPEC_ID,SKIP,VALUE,VELO_INDEX
