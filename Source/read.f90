...
MESH_LOOP: DO NM=1,NMESHES
   ...
   ! compute maximum grid aspect ratio

   DO K=1,M%KBAR
      DO J=1,M%JBAR
         DO I=1,M%IBAR
            M%MAX_ASPECT_RATIO = MAX( M%MAX_ASPECT_RATIO, MAX(M%DX(I),M%DY(J),M%DZ(K))/MIN(M%DX(I),M%DY(J),M%DZ(K)) )
         ENDDO
      ENDDO
   ENDDO

   ! numerical experiments suggest that aspect ratios > 4:1 require a tighter CFL criterion

   IF (M%MAX_ASPECT_RATIO>3.99_EB .AND. .NOT.CFL_VELOCITY_NORM_USER_SPECIFIED) CFL_VELOCITY_NORM=1
