...
END DO
COUNT=COUNT+1
IF(ABS(VOL_TETRA-VOL).GT.1.0E-13_EB*ABS(VOL_TETRA)+1.0E-13_EB)THEN
   !WRITE(6,*)VOL_TETRA,VOL
...
END DO
TAFTER=SECOND()
WRITE(6,*)"total CPU time=",TAFTER-TBEFORE,"CPU time per calc=",(TAFTER-TBEFORE)/(REAL(COUNT,EB)*4.0_EB**3)
WRITE(6,*)"error count=",ERROR_COUNT," total count=",COUNT
RETURN
...
DO BP = 0, 5
  VERTS(0:3) => BOX_PLANE2VERT(0:2,BP)
