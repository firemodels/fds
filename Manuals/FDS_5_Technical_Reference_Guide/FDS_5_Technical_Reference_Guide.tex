...
\be
   \bp_0(z) = p_\infty  \left( \frac{T_0(z)}{T_\infty} \right)^{\frac{\overline{W}g}{\R \Gamma}}
   \label{pstrat}
\ee
...
\nabla \cdot \bu = {\cal D} + {\cal P}\dod{\bp_m}{t} \,\mbox{,}
\end{equation}
where
\begin{equation}
\label{eqn_fdsP1}
...
\frac{\overline{W}}{\rho} \sum_\alpha \Div \left( \rho D_\alpha \nabla[Y_\alpha/W_\alpha] \right) + \frac{1}{\rho}\sum_\alpha \left( \frac{\overline{W}}{W_\alpha} -
\frac{h_{s,\alpha}}{c_p T} \right) \dot{m}_{\alpha}^\tripleprime  - \mathcal{P} w \rho_i g \nonumber \vspace{0.3cm} \\
&+ \frac{\mathcal{R}}{\overline{W} c_p \overline{p}_i} \left[ \dot{q}^\tripleprime - \dq_b''' -
\Div \dot{\mathbf{q}}^{\prime\prime} - \sum_\alpha h_{s,\alpha} \Div \rho D_\alpha \nabla Y_\alpha +
\dot{m}_{b}^{\tripleprime} \sum_\alpha Y_{b,\alpha} c_{p,\alpha}(T_b-T) + \frac{\dot{m}_{b}^{\tripleprime}}{2}\left|\mathbf{u}_b-\mathbf{u}\right|^2  \right] \nonumber \vspace{0.3cm} \\
\end{align}
...
The size of any solid phase mesh cell and the number of solid phase mesh cells 
can reduce during the course of a calculation. As the solid material is consumed (converted into gaseous fuel), its density 
can become less than the virgin material density. The volume of each solid phase cell is reduced such that the cell density is 
equal to the density of the virgin material. 
The cell size is not changed if some of the components of a cell are not reacting at all, or if the component is
leaving a solid residue. 
If the cell size gets below a pre-defined threshold (1.0E-6 m) then the cell is completely removed. 
Cell removal can result in a reduction in the number of solid phase mesh cells during the course of a simulation. Following cell shrinking
or cell removal, the solid phase mesh is re-gridded and the mass and enthalpy values are interpolated from the old mesh to the new mesh.
