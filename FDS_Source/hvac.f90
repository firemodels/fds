...
INTEGER :: N_HVAC_READ,N,ND,ND2,NM,NN,NV,I_DUCT=0,I_DUCTNODE=0,I_FILTER=0,I_FAN=0,I_AIRCOIL=0,FAN_INDEX,AIRCOIL_INDEX, &
           INDEX,IOS,IZERO
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(2),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW,RHOA
LOGICAL :: ROUND, SQUARE, DAMPER, REVERSE, AMBIENT
CHARACTER(30) :: TYPE_ID,SUBTYPE,DEVC_ID,CTRL_ID,FILTER_ID,DUCT_ID(10),NODE_ID(2),VENT_ID,TABLE_ID,FAN_ID,AIRCOIL_ID,RAMP_ID,ID
...
NAMELIST /HVAC/ TYPE_ID,AREA,DIAMETER,ROUND,SQUARE,LENGTH,DAMPER,REVERSE,LOSS,FILTER_ID,DUCT_ID,NODE_ID,FAN_ID,AIRCOIL_ID, &
...
!            CALL SHUTDOWN(MESSAGE)
!         ENDIF
         IF (LOSS(2)<0._EB) LOSS(2)=LOSS(1)
         DU%LOSS(1:2) = LOSS(1:2)
         ...
         IF (DAMPER .OR. FAN_ID /='null') THEN
            IF (DEVC_ID=='null' .AND. CTRL_ID=='null') THEN
               ...
               ENDIF
            ELSE
               CALL SEARCH_CONTROLLER('HVAC',CTRL_ID,DEVC_ID,DU%DEVC_INDEX,DU%CTRL_INDEX,N)
               IF (DEVC_ID /='null') THEN
                  IF (.NOT.DEVICE(DU%DEVC_INDEX)%INITIAL_STATE) DU%DAMPER_OPEN = .FALSE.
               ENDIF
               IF (CTRL_ID /='null') THEN
                  IF (.NOT.CONTROL(DU%CTRL_INDEX)%INITIAL_STATE) DU%DAMPER_OPEN = .FALSE.
               ENDIF
            ENDIF
         ENDIF         
...
DO ND = 1, N_DUCTS
   ...
   IF (DUCT_FAN_A(ND)/='null') THEN
      DO NN = 1, N_FANS
         IF(FAN(NN)%ID == DUCT_FAN_A(ND)) THEN
            DU%FAN_INDEX = NN
            EXIT
         ENDIF
      ENDDO
...
DO NN = 1, N_DUCTNODES
   DN => DUCTNODE(NN)
   DO ND = 1, 10
      IF (NODE_DUCT_A(NN,ND) == 'null') EXIT
      DN%N_DUCTS=ND
   ENDDO  
   IF (DN%VENT_ID /= 'null') THEN
      MESH_LOOP: DO NM = 1, NMESHES
         DO NV = 1, MESHES(NM)%N_VENT
            WRITE(*,*) MESHES(NM)%VENTS(NV)%ID,DN%VENT_ID
   ...
   IF (DN%VENT_INDEX> 0 .OR. DN%AMBIENT) THEN
      ALLOCATE(DN%LOSS_ARRAY(2,1))   
   ELSE
      ALLOCATE(DN%LOSS_ARRAY(DN%N_DUCTS,DN%N_DUCTS))   
   ENDIF
   DN%LOSS_ARRAY = 0._EB
...
WRITE(*,*) 'ENTER HVAC_CALC ',N_NETWORKS
...
NE =>NETWORK(NNE)
WRITE(*,'(2X,3(E16.9,1X),A)') (LHS(ND,1),ND=1,NE%N_MATRIX),' LHS'
WRITE(*,'(2X,3(E16.9,1X),A)') (LHS(ND,2),ND=1,NE%N_MATRIX),' LHS'
WRITE(*,'(2X,3(E16.9,1X),A)') (LHS(ND,3),ND=1,NE%N_MATRIX),' LHS'
WRITE(*,'(2X,3(E16.9,1X),A)') RHS,' RHS_IN'
...
DO NN = 1,NE%N_DUCTNODES
   ...
   IF (DN%FIXED) CYCLE
   DN%P = RHS(NE%MATRIX_INDEX(NE%N_DUCTS+ND))
ENDDO
WRITE(*,'(2X,3(E16.9,1X),A)') RHS,' RHS_OUT'
...
REAL(EB) :: TNOW,MTOT,ETOT,VTOT,YYTOT(1:N_SPECIES),TGUESS,VFLOW
...
NE => NETWORK(NNE)
WRITE(*,*) DUCTNODE%TMP,' T IN'
DUCT%UPDATED = .FALSE.
DO NN = 1,NE%N_DUCTNODES
   DN=>DUCTNODE(NE%NODE_INDEX(NN))
   DN%UPDATED = .FALSE.
   IF (DN%VENT_INDEX > 0 .AND. DUCT(DN%DUCT_INDEX(1))%VEL(NEW)*DN%DIR(1) > 0._EB) DN%UPDATED = .TRUE.
ENDDO
...
ITER_LOOP: DO
   ...
   NODE_LOOP:DO NN = 1,NE%N_DUCTNODES
      ...
      DN%UPDATED = .TRUE.
      DN%RHO = MTOT / VTOT
      IF(N_SPECIES > 0) DN%YY(:)  = YYTOT/MTOT
      TGUESS = TGUESS / MTOT      
      ETOT = ETOT/ MTOT
      DN%TMP = TGUESS
      CP_LOOP: DO
         IF (N_SPECIES>0) THEN
            CALL GET_AVERAGE_SPECIFIC_HEAT(DN%YY(:),DN%CP,MIN(5000,NINT(TGUESS)))
         ELSE
            DN%CP = Y2CP_C(MIN(5000,NINT(TGUESS)))
         ENDIF
         DN%TMP = ETOT /DN%CP
         IF (DN%TMP == TGUESS) EXIT CP_LOOP
...
IF (N_SPECIES > 0) WRITE(*,*) (DUCTNODE(NN)%YY(1),NN=1,N_DUCTNODES),' YY'
WRITE(*,*) 'EXIT CALC'
...
DO ND = 1, NE%N_DUCTS
   ...
   DN=>DUCTNODE(DU%NODE_INDEX(1))
   IF (DN%VENT_INDEX > 0 .OR. DN%AMBIENT) HEAD = HEAD + DN%P
   XYZ = DN%XYZ
   DN=>DUCTNODE(DU%NODE_INDEX(2))
   IF (DN%VENT_INDEX > 0  .OR. DN%AMBIENT) HEAD = HEAD - DN%P
   XYZ = DN%XYZ - XYZ
   HEAD = HEAD + (GVEC(1)*XYZ(1)+GVEC(2)*XYZ(2)+GVEC(3)*XYZ(3))*DU%RHO_D
   RHS(ARRAYLOC) = DU%VEL(OLD)+DT/(DU%RHO_D*DU%LENGTH)*(HEAD+DU%DP_FAN)!+&
                   !DT/DU%LENGTH*0.5_EB*DU%TOTAL_LOSS*ABS(DU%VEL(NEW))*DU%VEL(GUESS)
   !WRITE(*,*) 'RHSD ',DT/DU%LENGTH*0.5_EB*DU%TOTAL_LOSS*ABS(DU%VEL(NEW))*DU%VEL(GUESS),HEAD,RHS(ARRAYLOC),DU%VEL(NEW),DU%VEL(GUESS)
ENDDO
...
USE PHYSICAL_FUNCTIONS, ONLY : GET_MOLECULAR_WEIGHT,GET_AVERAGE_SPECIFIC_HEAT,GET_SPECIFIC_ENTHALPY
INTEGER, INTENT(IN) :: NM
INTEGER :: NN,II,JJ,KK,I1,I2,J1,J2,K1,K2,IC,IW,IOR
REAL(EB) :: YY_SUM(N_SPECIES),RHO_SUM,P_SUM,AREA_SUM,AREA,TMP_SUM,H_SUM,H_G,TMP_NEW,CP,TNOW
...
DO NN = 1, N_DUCTNODES
   ...
   IF (NM==1 .AND. DN%AMBIENT) THEN
      ...
      IF (N_SPECIES > 0) THEN
         DN%YY(1:N_SPECIES) = SPECIES(1:N_SPECIES)%YY0
         CALL GET_AVERAGE_SPECIFIC_HEAT(DN%YY(:),DN%CP,MIN(5000,NINT(DN%TMP)))
         CALL GET_MOLECULAR_WEIGHT(DN%YY(:),DN%MW)
      ELSE
         DN%CP_V = Y2CP_C(NINT(DN%TMP))
         DN%MW_V = SPECIES(0)%MW
      ENDIF
   ENDIF
   ...
   DO KK = K1,K2
      DO JJ = J1,J2
         DO II = I1, I2
            ...
            AREA = AW(IW)
            RHO_SUM = RHO_SUM + RHO(II,JJ,KK)*AW(IW)
            IF (N_SPECIES > 0) THEN
               YY_SUM = YY_SUM+YY(II,JJ,KK,:)*RHO(II,JJ,KK)*AREA
               CALL GET_AVERAGE_SPECIFIC_HEAT(YY(II,JJ,KK,:),H_G,MIN(5000,NINT(TMP(II,JJ,KK))))
               H_G = H_G * TMP(II,JJ,KK)
            ELSE
               H_G = TMP(II,JJ,KK)*Y2CP_C(MIN(5000,NINT(TMP(II,JJ,KK))))
            ENDIF
            H_SUM = H_SUM+H_G*AREA*RHO(II,JJ,KK)
            TMP_SUM = TMP_SUM + TMP(II,JJ,KK)*AREA
            P_SUM = P_SUM + PBAR_S(KK,PRESSURE_ZONE(II,JJ,KK))*AREA
   ...
   DO
      IF (N_SPECIES > 0) THEN
         CALL GET_AVERAGE_SPECIFIC_HEAT(DN%YY_V(:),CP,MIN(5000,NINT(TMP_SUM)))      
      ELSE
         CP = Y2CP_C(MIN(5000,NINT(TMP_SUM)))
      ENDIF
      TMP_NEW = H_SUM / CP
      IF (TMP_SUM == TMP_NEW .OR. ABS(TMP_SUM - TMP_NEW)/TMP_NEW < 0.0005_EB) EXIT
      TMP_SUM = TMP_NEW
   ENDDO
   DN%CP_V  = CP
   DN%TMP_V = TMP_SUM
   IF (N_SPECIES>0) CALL GET_MOLECULAR_WEIGHT(DN%YY_V(:),DN%MW)
ENDDO
...
INTEGER:: NN,ND,DUCT_COUNTER(N_DUCTS),NODE_COUNTER(N_DUCTNODES),COUNTER,NETWORK_COUNTER(N_DUCTS)
...
DO ND = 1, N_DUCTS
   ...
   IF (DU%DEVC_INDEX > 0) THEN
      IF((DEVICE(DU%DEVC_INDEX)%CURRENT_STATE .AND. .NOT. DU%DAMPER_OPEN).OR.&
         (DEVICE(DU%DEVC_INDEX)%CURRENT_STATE .AND. .NOT. DU%DAMPER_OPEN)) THEN
      ...
      ENDIF
   ELSE
      IF((CONTROL(DU%CTRL_INDEX)%CURRENT_STATE .AND. .NOT. DU%DAMPER_OPEN).OR.&
         (CONTROL(DU%CTRL_INDEX)%CURRENT_STATE .AND. .NOT. DU%DAMPER_OPEN)) THEN
...
REAL(EB) :: FRICTION_FACTOR
INTEGER, INTENT(IN) :: NNE
INTEGER :: ND,ND2, NN, ARRAYLOC
TYPE(DUCT_TYPE), POINTER :: DU
TYPE(DUCTNODE_TYPE), POINTER :: DN
TYPE(NETWORK_TYPE), POINTER :: NE
...
NODELOOP : DO NN=1,NE%N_DUCTNODES
  ...
  ELSEIF(DN%VENT_INDEX > 0) THEN NODECLASS
     IF(DUCT(DN%DUCT_INDEX(1))%VEL(GUESS)*DN%DIR(1) > 0._EB) THEN
        DN%BRANCH_LOSS(1) = DN%LOSS_ARRAY(1,1)
     ELSE
        ...
        DO ND=1,DN%N_DUCTS
           ...
           DO ND2=1,DN%N_DUCTS
             IF(DN%BRANCH_LOSS(ND2) < ZERO_P) CYCLE
               DN%BRANCH_LOSS(ND) = DN%BRANCH_LOSS(ND) - DN%BRANCH_LOSS(ND2)*DN%LOSS_ARRAY(ND2,ND)
           ENDDO
        ENDDO
        WHERE (DN%BRANCH_LOSS >= ZERO_P)
           DN%BRANCH_LOSS = 0._EB
        ELSEWHERE
           DN%BRANCH_LOSS = -DN%BRANCH_LOSS
        END WHERE
     ENDIF
  ENDIF NODECLASS
ENDDO NODELOOP
...
DO ND = 1, NE%N_DUCTS
   ...
   IF (DU%VEL(GUESS)>0._EB) THEN
      DN => DUCTNODE(DU%NODE_INDEX(1))
      ARRAYLOC = MINLOC(DN%DUCT_INDEX,1,DN%DUCT_INDEX==ND)
      DU%TOTAL_LOSS = DU%LOSS(1)+DN%BRANCH_LOSS(ARRAYLOC)
   ELSEIF (DU%VEL(GUESS)<0._EB) THEN
      DN => DUCTNODE(DU%NODE_INDEX(2))
      ARRAYLOC = MINLOC(DN%DUCT_INDEX,1,DN%DUCT_INDEX==ND)
      DU%TOTAL_LOSS = DU%LOSS(2)+DN%BRANCH_LOSS(ARRAYLOC)
   ELSE
      DN => DUCTNODE(DU%NODE_INDEX(1))
      ARRAYLOC = MINLOC(DN%DUCT_INDEX,1,DN%DUCT_INDEX==ND)
      DU%TOTAL_LOSS = 0.5_EB*(DU%LOSS(1)+DN%BRANCH_LOSS(ARRAYLOC))
      DN => DUCTNODE(DU%NODE_INDEX(2))
      ARRAYLOC = MINLOC(DN%DUCT_INDEX,1,DN%DUCT_INDEX==ND)
      DU%TOTAL_LOSS = 0.5_EB*(DU%LOSS(1)+DN%BRANCH_LOSS(ARRAYLOC)) + DU%TOTAL_LOSS
   ENDIF
