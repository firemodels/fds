...
of the computational domain. Create {\ct OPEN} vents on all other
faces.
\item Finally, add solid phase output devices to the solid surface. For example, {\ct 'HRRPUM'} (Heat Release Rate Per Unit Mass) in units of kW/kg is appropriate for MCC; and {\ct 'NORMALIZED HEAT FLOW'} in units of kW/kg is appropriate for DSC.
\end{enumerate}
...
\noindent
The logical parameter {\ct LUMPED\_COMPONENT\_ONLY} denotes that the species is only present as part of a lumped species. When {\ct .TRUE.}, FDS will not allocate space to track that species individually. The parameters to defined a lumped species are::
...
\noindent The addition of lumped species to FDS has changed the meaning of {\ct SPEC\_ID} on some FDS inputs.  For {\ct INIT},
...
\frac{\mbox{d}\hat{C}_{\mathrm{C_3H_8}}}{\mbox{d}t}= -k\; \prod \hat{C}_{\alpha}\,^{N_S}
\end{equation}
...
\frac{\mbox{d}\hat{C}_{\mathrm{C_3H_8}}}{\mbox{d}t} = -8.6 \times 10^{11} \; T^{0} \; e^{-125520/RT} \; \hat{C}_{\mathrm{C_3H_8}}\,^{0.1} \; \hat{C}_{\mathrm{O_2}}\,^{1.65}
\end{equation}
...
For liquid droplets, particle sizes follow a given the Cumulative Volume Function (CVF) of droplet sizes. Distributions can be selected by setting the {\ct DISTRIBUTION} parameter on the PART line. The default is {\ct ROSIN-RAMMLER-LOGNORMAL}. Other choices are {\ct LOGNORMAL} and {\ct ROSIN-RAMMLER}. Note that the specified {\ct DIAMETER} (in units of $\mu$m) on the {\ct PART} line is the median volumetric diameter of the droplets or particles.

The {\ct LOGNORMAL} distribution is given by
\be
\frac{1}{\sqrt{2\pi}} {\displaystyle \int_0^d} \, \frac{1}{\sigma\, d'} \,
   e^{-\frac{[\ln(d'/d_m)]^2}{2\sigma^2}}.
\ee
The width of the distribution is controlled by the parameter {\ct SIGMA\_D} on the {\ct PART} line. The {\ct ROSIN-RAMMLER} distribution is given by
\be
   1 - e^{-0.693 \left(\frac{d}{d_m}\right)^\gamma. }
\ee
The width of the distribution is controlled by the parameter {\ct GAMMA\_D}. And finally the {\ct ROSIN-RAMMLER-LOGNORMAL} distribution is given by
\be F(d) = \left\{ \begin{array}{ll}
   ...
   \end{array} \right.
\ee
The width of the distribution is controlled by the parameter {\ct GAMMA\_D} (default 2.4). Note that the parameter $\sigma$ is given the value
...
over-ride this feature. The larger the value of $\gamma$, the narrower the droplet size is
distributed about the median value.

The particle size distribution can also defined using the Cumulative Number Function of the distribution (CNF). This is done by setting the {\ct USER\_CNF\_RAMP} on the {\ct PART} line and including a {\ct RAMP} that gives the CNF. For example, particles with diameters following the normal distribution can be defined as follows.
\begin{verbatim}
&PART ID='USER',
      CNF_RAMP_ID='USER_CNF',
      ..../

&RAMP ID="USER_CNF"   T= 0   F= 0.000000/
&RAMP ID="USER_CNF"   T= 200 F= 0.000003/
      ....
&RAMP ID="USER_CNF"   T= 1800 F= 0.999968/
&RAMP ID="USER_CNF"   T= 2000 F= 1.000000/
\end{verbatim}
Note that the {\ct RAMP} variable {\ct T} is given in micrometers.

 Note that you can prevent droplets or particles from
...
\noindent
The {\ct PART} line establishes the class of particles. In this case, the presence of a {\ct SURF\_ID} indicates that the particles are solids with the properties given by the {\ct SURF} line {\ct 'HOT'}. {\ct STATIC} is a logical parameter whose default is {\ct .FALSE.} that indicates if the particles are stationary. The {\ct PROP\_ID} references a {\ct PROP} (property) line that has additional information about this class of particles.

The {\ct SURF} line {\ct 'HOT'} describes the geometry and other boundary conditions of the particles. The {\ct GEOMETRY} options are {\ct 'SPHERICAL'}, {\ct 'CYLINDRICAL'}, or {\ct 'CARTESIAN'}. If the particles are thermally-thick; that is, heat is to be conducted in depth, then provide one or more values of the layer {\ct THICKNESS} and corresponding {\ct MATL\_ID}'s. If the {\ct SURF} line includes a {\ct MATL\_ID}, then the particle mass will be based upon the value or values of {\ct DENSITY} of the referenced {\ct MATL} lines. By default, the {\ct GEOMETRY} is {\ct 'CARTESIAN'}, in which case you also need to provide a {\ct LENGTH} and {\ct WIDTH} of the rectangular plate of the given {\ct THICKNESS}. If there is to be no heat conduction calculation in depth, do not specify a {\ct THICKNESS} or {\ct MATL\_ID}. Instead, you can specify, for example, the surface temperature, {\ct TMP\_FRONT}, or the heat release rate per unit area, {\ct HRRPUA} (kW/m$^2$). For {\ct 'CYLINDRICAL'} particles, specify the {\ct LENGTH}. For  {\ct 'CYLINDRICAL'} or {\ct 'SPHERICAL'} particles, specify the {\ct INNER\_RADIUS} and {\ct THICKNESS} for thermally-thick materials, or just specify the {\ct RADIUS} otherwise. The default value of {\ct INNER\_RADIUS} is 0~m, which means that the radius of the cylinder or sphere is given by the {\ct THICKNESS} values unless you specify an {\ct INNER\_RADIUS} as well.

The {\ct PROP} line, {\ct 'ball'}, just tells Smokeview that the particles are to be drawn as spheres of radius 0.01~m. See Section~\ref{info:SMOKEVIEW_PART} for details and options.

The {\ct INIT} line randomly fills the given volume with 10 of these hot spheres. See Section~\ref{info:initial_droplets} for details.
...
You may also use {\ct DEVC} (device) lines to output data for a given particle. For example, the lines:
...
\noindent The {\ct INIT} line designates the plane of the screen using the sextuplet {\ct XB}. A single particle is inserted into each cell by specifying the parameter {\ct N\_PARTICLES\_PER\_CELL=1}.  The particles should be defined with a {\ct SURF\_ID} containing the material properties of the screen.  A special drag law for screens is specified via the {\ct DRAG\_LAW}.  {\ct ORIENTATION} is the direction normal to the screen and {\ct FREE\_AREA\_FRACTION} is the fraction of the screen's surface area that is open. In the example, an aluminum screen with a 40~\% free area and an 0.0003~m wire diameter is placed normal to the $x$-axis.  Note that the {\ct LENGTH} on the {\ct SURF} line, when multiplied by the diameter, results in 60~\% of the cell area. It represents the length of wire that makes up the section of the screen that spans a single grid cell. The pressure drop across the screen is given by
\be
...
\end{eqnarray}
Note that the {\ct CONVECTION\_LENGTH\_SCALE}, which in this case is set equal to the wire diameter, is used in the convective heat transfer calculation.
...
algorithm is invoked when the fuel is complicated, like a cartoned commodity.  The example case {\ct e\_coefficient} demonstrates the use of this parameter.  A sprinkler is placed over a burner defined with an {\ct E\_COEFFICIENT} as shown below.  The sprinker is set to operate at 5 s.  Figure~\ref{e_coef_fig} shows the heat release rate and burning rate.
...
\item[{\ct FUNCTION\_TYPE}] The type of control function.  The possible types are shown in~\ref{tab:funcvalues}.
...
The full profile data may be more than is needed. Sometimes, all you want to know is the temperature at a certain depth. To get an inner wall temperature, you can also just use a device as follows:
...
The third parameter, {\ct VISIBILITY\_FACTOR} on the {\ct MISC} line, is the
...
{\ct AEROSOL VOLUME FRACTION}$^*$               & $\rho Y_a/\rho_a$ (Section~\ref{info:soot})   & mol/mol        & D,I,P,S      \\ \hline
...
{\ct HRRPUM}                                    & Section~\ref{info:TGA_DSC_MCC}                & kW/kg          & D            \\ \hline
...
{\ct NORMALIZED HEAT FLOW}                      & Section~\ref{info:TGA_DSC_MCC}                & kW/kg          & D            \\ \hline
...
%{\ct PRESSURE COEFFICIENT TEST}                 & $C_p$ (Section~\ref{info:wind})               &                & B,D          \\ \hline
