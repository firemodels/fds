...
The user should also be aware that the {\ct OPEN} pressure boundary condition is most stable for flows that are predominantly normal  to the vent, either mostly in or mostly out.  The reason is that the prescribed pressure at this boundary has a strong discontinuity if the flow is parallel to the vent.  Suppose for a moment that the flow is 10 m/s in the $x$ direction and $\pm .001$ m/s in the $z$ direction for an {\ct OPEN} top boundary in an outdoor flow.  The kinetic energy of this flow is roughtly $k=50$ m$^2$/s$^2$.  When the vertical velocity is positive (+0.001 m/s) then the prescibed boundary condition for the stagnation pressure is set to ${\cal H} = k = 50$ m$^2$/s$^2$.  But when the vertical velocity is negative (-0.001 m/s) then ${\cal H} = 0$ (see \cite{FDS_Tech_Guide}).  For this reason, {\ct OPEN} vents are not recommended for the top boundary in an outdoor flow.  See Section \ref{info:mean_forcing} for a better alternative.
