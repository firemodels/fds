...
REAL(EB), INTENT(IN) :: T
REAL(EB) :: STIME,DI,DD,SUM_FAC=0._EB
REAL(EB),ALLOCATABLE::CONST(:),CUMSUM(:)
...
STIME = T_BEGIN + (T-T_BEGIN)*TIME_SHRINK_FACTOR
DEVC_TIME = STIME
ALLOCATE(CONST(N_PDPA_HISTOGRAM),CUMSUM(N_PDPA_HISTOGRAM))
...
IF (N_PDPA_HISTOGRAM>0) THEN 
   ...
   CUMSUM(:)=0._EB
   IF(PROPERTY(DV%PROP_INDEX)%PDPA_HISTOGRAM_CUMULATIVE) SUM_FAC=1._EB
      ...
      DO J=1,N_DEVC
         ...
         IF(PROPERTY(DV%PROP_INDEX)%PDPA_HISTOGRAM .AND. DV%OUTPUT) THEN
            ...
            IF(PROPERTY(DV%PROP_INDEX)%PDPA_HISTOGRAM_NBINS>=I) THEN
               DI=(I-0.5_EB)*DD
               CUMSUM(NN)=SUM_FAC*CUMSUM(NN)+DV%PDPA_HISTOGRAM_COUNTS(I)
