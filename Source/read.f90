...
USE COMPLEX_GEOMETRY, ONLY: READ_GEOM, GET_TRAN_INDEX
USE MPI
...
REAL(EB) :: AGL_SLICE
INTEGER :: N,NN,NM,MESH_NUMBER,N_SLCF_O,NITER,ITER,VELO_INDEX,IOR,IG,TRAN_INDEX,GEOM_INDEX
LOGICAL :: VECTOR,CELL_CENTERED, FACE_CENTERED, FIRE_LINE, EVACUATION,LEVEL_SET_FIRE_LINE
CHARACTER(LABEL_LENGTH) :: QUANTITY,SPEC_ID,PART_ID,QUANTITY2,PROP_ID,REAC_ID,SLICETYPE,MATL_ID,TRAN_ID
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IF (TRAN_ID/='null') THEN
         CALL GET_TRAN_INDEX(TRAN_ID, TRAN_INDEX)
         IF ( TRAN_INDEX == -1) THEN
            WRITE(MESSAGE,'(A,A,A)') "ERROR: The &TRAN namelist, ",TRAN_ID," does not exist"
            CALL SHUTDOWN(MESSAGE) ; RETURN
         ENDIF
         GEOM_INDEX=TRANSFORM(TRAN_INDEX)%GEOM_INDEX
      ENDIF
