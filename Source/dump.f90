...
ALLOCATE(LU_GEOM(1))
IF (N_TRNF>0) THEN
  ALLOCATE(FN_GEOM_TRAN(N_TRNF,NMESHES))
ENDIF
...
IF (N_FACE>0 .OR. N_GEOMETRY>0) THEN
   ...
   IF (N_TRNF>0) THEN
      ...
      DO N=1,N_TRNF
         DO NM=1,NMESHES
            WRITE(FN_GEOM_TRAN(N,NM),'(A,A,I2.2,A,I2.2,A)') TRIM(CHID),'_tran_',N,'_',NM,'.ge'
         ENDDO
      ENDDO
   ENDIF
ENDIF
   ...
   ! write out slice file info to the .smv file
      ...
      IF (SL%SLICETYPE/='STRUCTURED') THEN
         ...
         IF (SL%SLICETYPE=='TRANSFORM_GEOM') THEN
            WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(TRANSFORM(N,NM)%FILE)
         ELSE
            WRITE(M%STRING(M%N_STRINGS),'(1X,A)') TRIM(FN_SLCF_GEOM(N,NM))
         ENDIF
      ENDIF
...
IF (N_GEOMETRY>0) THEN
   ...
   DO N=1,N_TRNF
      DO NM=1,NMESHES
         ...
         WRITE(LU_SMV,'(1X,A)') TRIM(FN_GEOM_TRAN(N,NM))
      ENDDO
   ENDDO
ENDIF
