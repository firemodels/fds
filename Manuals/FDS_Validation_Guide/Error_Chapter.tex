...
Alpert's ceiling jet correlation~\cite{SFPE:Alpert} predicts the temperature rise, $T-T_\infty$ (\unit{\degreeCelsius}), as a function of the HRR, $\dot{Q}$ (kW), and radial distance, $r$ (m), from the plume centerline:
\be
...
Damage is assumed to occur when the surface temperature of any cable reaches 200~\unit{\degreeCelsius}.
What is the likelihood that the cables would be damaged if the
model predicts that the maximum surface temperature of the cables is 175~\unit{\degreeCelsius}. Assuming that the input parameters are not in question, the following
...
The vertical lines indicate the ``critical'' temperature at which damage is assumed to occur ($T_c=200$~\unit{\degreeCelsius}), and the temperature predicted by the
model (175~\unit{\degreeCelsius}). Given an ambient temperature of 20~\unit{\degreeCelsius}, the predicted temperature rise, $M$, is 155~\unit{\degreeCelsius}.
...
critical value, $T_c=200$~\unit{\degreeCelsius}, which can be expressed via the {\em complimentary error function}:
...
away because of the fact that temperatures tend to stabilize at about 1000~\unit{\degreeCelsius} within the fire itself, but then rapidly decrease away from the
