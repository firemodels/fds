...
IF (SF%TMP_GAS_FRONT > 0._EB) ONE_D%TMP_G = TMPA + EVALUATE_RAMP(T-T_BEGIN,SF%RAMP_INDEX(TIME_TGF))*(SF%TMP_GAS_FRONT-TMPA)
...
SELECT CASE(BACKING)
   ...
   CASE(VOID)  ! Non-insulated backing to an ambient void
      ...
      IF (SF%TMP_GAS_BACK>0._EB) THEN
         TMP_GAS_BACK = TMP_0(BC%KK) + EVALUATE_RAMP(T-T_BEGIN,SF%RAMP_INDEX(TIME_TGB))*(SF%TMP_GAS_BACK-TMP_0(BC%KK))
      ELSE
...
IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED .AND. .NOT.PRESENT(THIN_WALL_INDEX)) THEN
   ...
   IF (PRESENT(WALL_INDEX)) THEN
      IF (OBSTRUCTION(I_OBST)%THIN .AND. .NOT.SOLID(CELL_INDEX(BC%II,BC%JJ,BC%KK))) RDT = 1._EB/DT_BC
   ENDIF
