...
USE GLOBAL_CONSTANTS, ONLY: CELL_COUNT,CELL_COUNT_REALS,CELL_COUNT_INTEGERS,CELL_COUNT_LOGICALS
...
INTEGER :: RC,IC,LC,CELL_COUNT_REALS_OLD,CELL_COUNT_INTEGERS_OLD,CELL_COUNT_LOGICALS_OLD
...
IF (.NOT.ALLOCATED(M%CELL)) THEN
   ...
   CELL_COUNT(NM) = N1
   CALL PACK_CELL(NM,.TRUE.,N_REALS=RC,N_INTEGERS=IC,N_LOGICALS=LC)
   CELL_COUNT_REALS(NM)    = RC
   CELL_COUNT_INTEGERS(NM) = IC
   CELL_COUNT_LOGICALS(NM) = LC
   ALLOCATE(M%CELL_REALS(1:RC))
...
CELL_COUNT_REALS_OLD    = SIZE(M%CELL_REALS)
CELL_COUNT_INTEGERS_OLD = SIZE(M%CELL_INTEGERS)
CELL_COUNT_LOGICALS_OLD = SIZE(M%CELL_LOGICALS)

CALL PACK_CELL(NM,.TRUE.,N_REALS=RC,N_INTEGERS=IC,N_LOGICALS=LC)

ALLOCATE(REAL_DUMMY(1:RC))
REAL_DUMMY(1:CELL_COUNT_REALS_OLD) = M%CELL_REALS(1:CELL_COUNT_REALS_OLD)
CALL MOVE_ALLOC(REAL_DUMMY,M%CELL_REALS)
CELL_COUNT_REALS(NM) = RC
...
IF (M%N_BOUNDARY_PROPS_DIM==0)  M%N_BOUNDARY_PROPS_DIM = N_SLOTS
...
IF (.NOT.ALLOCATED(M%BOUNDARY_PROPS)) ALLOCATE(M%BOUNDARY_PROPS(1:M%N_BOUNDARY_PROPS_DIM))
...
ENDIF
IF (.NOT.ALLOCATED(M%BOUNDARY_PROPS_OCCUPANCY)) THEN
   ALLOCATE(M%BOUNDARY_PROPS_OCCUPANCY(1:M%N_BOUNDARY_PROPS_DIM))
   M%BOUNDARY_PROPS_OCCUPANCY = 0
...
INTEGER :: N_NEW_STORAGE_SLOTS,OD_INDEX,TD_INDEX,BP_INDEX,BR_INDEX,BC_INDEX,I,NAS
...
USE GLOBAL_CONSTANTS, ONLY: SIGMA,TMPA,TMPA4,RHOA,N_TRACKED_SPECIES,N_SURFACE_DENSITY_SPECIES
...
IF (.NOT.ALREADY_ALLOCATED) THEN
   ...
   ALLOCATE(ONE_D%M_DOT_G_PP_ACTUAL(N_TRACKED_SPECIES))
   ...
   ALLOCATE(ONE_D%M_DOT_G_PP_ADJUST(N_TRACKED_SPECIES))
   ...
   ALLOCATE(ONE_D%ZZ_F(N_TRACKED_SPECIES))
   IF (ALLOCATED(ONE_D%RHO_D_F)) DEALLOCATE(ONE_D%RHO_D_F)
   ALLOCATE(ONE_D%RHO_D_F(N_TRACKED_SPECIES))
   IF (ALLOCATED(ONE_D%RHO_D_DZDN_F)) DEALLOCATE(ONE_D%RHO_D_DZDN_F)
   ALLOCATE(ONE_D%RHO_D_DZDN_F(N_TRACKED_SPECIES))
   ...
   ALLOCATE(ONE_D%AWM_AEROSOL(N_SURFACE_DENSITY_SPECIES))
...
IF (.NOT.ALREADY_ALLOCATED) THEN
   ...
   ALLOCATE(BP%A_LP_MPUA(N_LP_ARRAY_INDICES))
   IF (ALLOCATED(BP%LP_CPUA)) DEALLOCATE(BP%LP_CPUA)
   ALLOCATE(BP%LP_CPUA(N_LP_ARRAY_INDICES))
   IF (ALLOCATED(BP%LP_MPUA)) DEALLOCATE(BP%LP_MPUA)
   ALLOCATE(BP%LP_MPUA(N_LP_ARRAY_INDICES))
   IF (ALLOCATED(BP%LP_TEMP)) DEALLOCATE(BP%LP_TEMP)
   ALLOCATE(BP%LP_TEMP(N_LP_ARRAY_INDICES))
ENDIF
...
INTEGER :: IC,RC,LC,BC_INDEX,OD_INDEX,BP_INDEX,BR_INDEX
LOGICAL :: COUNT_ONLY
...
IF (LPC%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,SF,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (LPC%INCLUDE_BOUNDARY_PROPS_TYPE) CALL PACK_BOUNDARY_PROPS(NM,IC,RC,OS,1,BP_INDEX,UNPACK_IT,COUNT_ONLY)
...
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM2,IC,RC,LC,OS,SF,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) CALL PACK_BOUNDARY_PROPS(NM2,IC,RC,OS,STORAGE_INDEX,BP_INDEX,UNPACK_IT,COUNT_ONLY)
...
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER, OPTIONAL, INTENT(OUT) :: N_INTEGERS,N_REALS,N_LOGICALS
INTEGER :: I1,I2,IC,RC,LC,BC_INDEX,OD_INDEX,BP_INDEX,BR_INDEX
LOGICAL :: COUNT_ONLY
...
IF (SF%INCLUDE_BOUNDARY_ONE_D_TYPE) CALL PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,SF,1,OD_INDEX,UNPACK_IT,COUNT_ONLY)
IF (SF%INCLUDE_BOUNDARY_PROPS_TYPE) CALL PACK_BOUNDARY_PROPS(NM,IC,RC,OS,1,BP_INDEX,UNPACK_IT,COUNT_ONLY)
...
SUBROUTINE PACK_BOUNDARY_ONE_D(NM,IC,RC,LC,OS,SF,STORAGE_INDEX,OD_INDEX,UNPACK_IT,COUNT_ONLY)
...
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%PRESSURE_ZONE,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%NODE_INDEX,UNPACK_IT)
IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),ONE_D%N_SUBSTEPS,UNPACK_IT)
...
LC=LC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%LOGICALS(LC,STORAGE_INDEX),ONE_D%BURNAWAY,UNPACK_IT)

! Reals

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%AREA,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%HEAT_TRANS_COEF,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_CON_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_RAD_IN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_RAD_OUT,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%EMISSIVITY,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%AREA_ADJUST,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%T_IGN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%TMP_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%TMP_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%TMP_B,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_NORMAL,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_NORMAL_S,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_NORMAL_0,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%U_TANG,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%RHO_F,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%RDN,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%K_G,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_DOT_G_PP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_DOT_O2_PP,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_CONDENSE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%TMP_F_OLD,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%BURN_DURATION,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%T_SCALE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_SCALE,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%T_MATL_PART,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%B_NUMBER,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%M_DOT_PART_ACTUAL,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),ONE_D%Q_IN_SMOOTH,UNPACK_IT)

I2 = RC  ! I1 and I2 continue counting reals

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%M_DOT_G_PP_ACTUAL(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + SF%N_MATL - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%M_DOT_S_PP(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%M_DOT_G_PP_ADJUST(1:I2-I1+1) , UNPACK_IT)
...
I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%ZZ_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%RHO_D_F(1:I2-I1+1) , UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_TRACKED_SPECIES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%RHO_D_DZDN_F(1:I2-I1+1) , UNPACK_IT)
...
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX) , ONE_D%AWM_AEROSOL(1:I2-I1+1) , UNPACK_IT)
...
SUBROUTINE PACK_BOUNDARY_PROPS(NM,IC,RC,OS,STORAGE_INDEX,BP_INDEX,UNPACK_IT,COUNT_ONLY)
...
IF (.NOT.COUNT_ONLY) BP => MESHES(NM)%BOUNDARY_PROPS(BP_INDEX)

IC=IC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%INTEGERS(IC,STORAGE_INDEX),BP%SURF_INDEX,UNPACK_IT)

RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%U_TAU,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%Y_PLUS,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%Z_STAR,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%PHI_LS,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%WORK1,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%WORK2,UNPACK_IT)
RC=RC+1 ; IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(RC,STORAGE_INDEX),BP%K_SUPPRESSION,UNPACK_IT)

I2 = RC

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%A_LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%LP_CPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%LP_MPUA(1:I2-I1+1),UNPACK_IT)

I1 = I2+1 ; I2 = I1 + N_LP_ARRAY_INDICES - 1
IF (.NOT.COUNT_ONLY) CALL EQUATE(OS%REALS(I1:I2,STORAGE_INDEX),BP%LP_TEMP(1:I2-I1+1),UNPACK_IT)
...
!> \param NM Mesh number
!> \param UNPACK_IT Logical indicating whether to pack or unpack
!> \param N_REALS Number of reals packed
!> \param N_INTEGERS Number of integers packed
!> \param N_LOGICALS Number of logicals packed

SUBROUTINE PACK_CELL(NM,UNPACK_IT,N_REALS,N_INTEGERS,N_LOGICALS)

USE COMP_OPERATORS, ONLY: EQUATE
USE GLOBAL_CONSTANTS, ONLY: CELL_COUNT
INTEGER, INTENT(IN) :: NM
LOGICAL, INTENT(IN) :: UNPACK_IT
INTEGER, INTENT(OUT), OPTIONAL :: N_REALS,N_INTEGERS,N_LOGICALS
INTEGER :: IC,LC,RC,ICC,N
LOGICAL :: COUNT_ONLY
TYPE(MESH_TYPE), POINTER :: M

M => MESHES(NM)

IF (PRESENT(N_REALS) .OR. PRESENT(N_INTEGERS) .OR. PRESENT(N_LOGICALS)) THEN
   COUNT_ONLY = .TRUE.
ELSE
   COUNT_ONLY = .FALSE.
ENDIF
...
DO ICC=1,CELL_COUNT(NM)
   ...
   RC=RC+1  ; IF (.NOT.COUNT_ONLY) CALL EQUATE(M%CELL_REALS(RC)         ,M%CELL(ICC)%U_EDGE_Y,UNPACK_IT)
   RC=RC+1  ; IF (.NOT.COUNT_ONLY) CALL EQUATE(M%CELL_REALS(RC)         ,M%CELL(ICC)%U_EDGE_Z,UNPACK_IT)
   RC=RC+1  ; IF (.NOT.COUNT_ONLY) CALL EQUATE(M%CELL_REALS(RC)         ,M%CELL(ICC)%V_EDGE_X,UNPACK_IT)
   RC=RC+1  ; IF (.NOT.COUNT_ONLY) CALL EQUATE(M%CELL_REALS(RC)         ,M%CELL(ICC)%V_EDGE_Z,UNPACK_IT)
   RC=RC+1  ; IF (.NOT.COUNT_ONLY) CALL EQUATE(M%CELL_REALS(RC)         ,M%CELL(ICC)%W_EDGE_X,UNPACK_IT)
   RC=RC+1  ; IF (.NOT.COUNT_ONLY) CALL EQUATE(M%CELL_REALS(RC)         ,M%CELL(ICC)%W_EDGE_Y,UNPACK_IT)
...
USE GLOBAL_CONSTANTS, ONLY: N_MPI_PROCESSES,MY_RANK,LOWER_MESH_INDEX,UPPER_MESH_INDEX,PROCESS,NMESHES,MPI_COMM_NEIGHBORS,&
                            MPI_COMM_NEIGHBORS_ROOT,CELL_COUNT,CELL_COUNT_INTEGERS,CELL_COUNT_REALS,CELL_COUNT_LOGICALS,&
                            COUNTS,DISPLS
USE MPI_F08
