...
square meter per unit solid angle~(W/(sr$\cdot$m$^2$)).  The solid
...
$(W/\mbox{m}^2)$ on Mars due to the increased distance from the
sun is exactly offset by the reduced solid angle ($sr$) that the
...
W/(m$^2$~sr). The coefficients $\sigma_a$ and $\sigma_s$ have
units of 1/m and specify the time and location dependent change
