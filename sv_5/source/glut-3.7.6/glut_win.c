...
__glutCreateWindow(GLUTwindow * parent,
  int x, int y, int width, int height, int gameMode, int noCLOSE)
{
  GLUTwindow *window;
  ...
  if(noCLOSE==1)style = WS_DLGFRAME; // gpf 
...
/* CENTRY */
int APIENTRY
glutCreateNoCloseWindow(const char *title)
{
  ...
  window = __glutCreateWindow(NULL,
    ...
    /* not game mode */ 0,/* no close box */1);
  win = window->win;
...
int APIENTRY
glutCreateNoCloseSubWindow(int win, int x, int y, int width, int height)
{
  GLUTwindow *window;

  window = __glutCreateWindow(__glutWindowList[win - 1],
    x, y, width, height, /* not game mode */ 0,/* no close box */1);
#if !defined(_WIN32)
  {
    GLUTwindow *toplevel;
...
glutCreateSubWindow(int win, int x, int y, int width, int height)
{
  ...
  window = __glutCreateWindow(__glutWindowList[win - 1],
    x, y, width, height, /* not game mode */ 0,/* close box */0);
#if !defined(_WIN32)
  {
    GLUTwindow *toplevel;
