...
LOGICAL :: DEFAULT,VEG_LSET_SPREAD,VEG_LSET_TAN2,TGA_ANALYSIS,COMPUTE_EMISSIVITY,&
           COMPUTE_EMISSIVITY_BACK,THERM_THICK,HT3D
...
NAMELIST /SURF/ ADIABATIC,AREA_MULTIPLIER,BACKING,BURN_AWAY,BURN_DURATION,&
                CELL_SIZE,CELL_SIZE_FACTOR,COLOR,CONE_HEAT_FLUX,&
...
!> \brief copy the input file to CHID.inp

SUBROUTINE COPY_INPF
   INTEGER :: LU_COPY, PER

   IF (MY_RANK==0) THEN
      PER = INDEX(TRIM(FN_INPUT),".",.TRUE.)
      IF (PER>0) THEN
      ...
      CALL COPY_INPUT_FILE(LU_INPUT, LU_COPY)
      CLOSE(LU_COPY)
   ENDIF
CONTAINS

SUBROUTINE COPY_INPUT_FILE(LU_INFILE,LU_OUTFILE)

INTEGER, INTENT(IN) :: LU_INFILE,LU_OUTFILE
CHARACTER(2*MESSAGE_LENGTH+1) :: BUFFER2, ERR_BUFFER ! This size should be the same as MESSAGE_LENGTH_EXT+1.

REWIND(LU_INFILE)
COPY_FILE_LOOP: DO
   READ(LU_INFILE,'(A)',ADVANCE='NO',EOR=11,END=10) BUFFER2
   WRITE(ERR_BUFFER,'(A,I3,A)') 'ERROR: Input file '//TRIM(CHID)//'.fds has line that is too long. Split it.'
   CALL SHUTDOWN(ERR_BUFFER); RETURN
11 CONTINUE
   BACKSPACE(LU_INFILE)
   READ(LU_INFILE,'(A)') BUFFER2
   WRITE(LU_OUTFILE,'(A)') TRIM(BUFFER2)
ENDDO COPY_FILE_LOOP
10 RETURN

END SUBROUTINE COPY_INPUT_FILE
END SUBROUTINE COPY_INPF
