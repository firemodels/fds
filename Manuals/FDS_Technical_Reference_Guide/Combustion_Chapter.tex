...
In this section we discuss the basic extinction model for simple chemistry without additional diluents.  Details of the general case considering multiple, infinitely fast reactions with added diluents are discussed in Sec.~\ref{General_Extinction}.
...
\item If the potential combustion heat release from a \emph{local pocket of stoichiometric fuel-air-product mixture} cannot raise the temperature of that mixture above an empirically determined limiting flame temperature, $T_{\rm LFT}$, then combustion is suppressed.  Consider the simple reaction $\mbox{Fuel} + \mbox{Air} \rightarrow \mbox{Products}$. The local mass fractions of lumped species Fuel, Air, and Products in the cell at the beginning of the combustion time step are $[Z_{\rm F}, Z_{\rm A}, Z_{\rm P}]$ and $s$ is the mass stoichiometric coefficient for Air (mass of Air required per mass of Fuel consumed). Based on the limiting reactant, the masses in the local pocket of stoichiometric mixture are defined to be
\begin{align}
\label{eq:dzf} m_{\rm F,st} &= \rho V_{\rm c} \min(Z_{\rm F},Z_{\rm A}/s) \\
\label{eq:dza} m_{\rm A,st} &= s \, m_{\rm F,st}                          \\
\label{eq:dzp} m_{\rm P,st} &= m_{\rm A,st} Z_{\rm P}/Z_{\rm A}           
\end{align}
If the following inequality is true, then combustion is suppressed:
\begin{multline}
\label{eq:extinction}
m_{\rm F,st} \, \left( h[\overline{\mathbf{Z}}_{\rm F,st},T]+\Delta h_{\rm c}[\overline{\mathbf{Z}}_{\rm F,st}] \right) + m_{\rm A,st} \, h(\overline{\mathbf{Z}}_{\rm A,st},T) + m_{\rm P,st} \, h(\overline{\mathbf{Z}}_{\rm P,st},T)  < \\
(m_{\rm F,st} + m_{\rm A,st} + m_{\rm P,st}) \, h(\overline{\mathbf{Z}}_{\rm PFP},T_{\rm LFT}) 
\end{multline}
where $T$ is the initial mean cell temperature.  The respective mixture composition vectors are denoted by an overline.  For example, $\overline{\mathbf{Z}}_{\rm F,st}$ is the composition of the stoichiometric fuel mixture; $\Delta h_{\rm c}(\overline{\mathbf{Z}}_{\rm F,st})$ is the heat of combustion of the stoichiometric fuel mixture.  The composition vector for the Post-Flame Products is $\overline{\mathbf{Z}}_{\rm PFP}$.

For the simple chemistry case, the Fuel, Air, and Product mixtures are ``pure''.  But Eq.~(\ref{eq:extinction}) is general enough to account for multiple fuels and multiple fast reactions.  In that case, the mixture compositions and the limiting flame temperature are adjusted as discussed in Sec.~\ref{General_Extinction}.
\end{enumerate}
...
\begin{figure}
\centering
\includegraphics[width=4.in]{FIGURES/reaction_time_scale.pdf}
\vskip-.2cm
...
Using the stoichiometric change in fuel concentration, the change in each species $\alpha$ is given by
...
\end{align}
If both Eqs.~(\ref{eq:series_reac_1}) and (\ref{eq:series_reac_2}) are infinitely fast and the mixed state is a stoichiometric mixture of methane and oxygen (for \emph{complete} combustion) then only H$_2$O and CO$_2$ should be present at the end of a time step. Note that if these reactions are processed in parallel in a single step then we are guaranteed to have CO remaining, which of course should not happen if sufficient O$_2$ is present. To properly process series reactions, FDS will identify the number of infinitely fast series reactions, $n$, that the user has specified. FDS will then update the species $n$ times before updating the time step. This ensures that the reactions proceed as far as the mixed state stoichiometry permits.

\subsubsection{Multiple Infinitely Fast Reactions: Extinction}
\label{General_Extinction}

Here we extend the basic extinction model discussed in Sec.~\ref{extinction} to account for multiple fuels and reactions.  The scheme depends on the cell composition, not the total cell mass, so we take a cell mass of 1 kg as a basis ($\rho V_{\rm c} = 1 \; \si{kg}$) to simplify the mathematical development.

Given the cell composition vector $\mathbf{Z}$, the mass fractions of Fuel, Air, and Products (including additional diluents) are
\begin{align}
&&&&&& Z_{\rm F} &= \sum_i Z_{{\rm F},i}      &&\mbox{for all infinitely fast reactions $i$}   &&&&\\
&&&&&& Z_{\rm A} &= \sum_i Z_{{\rm A},i}      &&  \mbox{for all infinitely fast reactions $i$} &&&&\\
&&&&&& Z_{\rm P} &= 1 - Z_{\rm F} - Z_{\rm A} &&                                               &&&&
\end{align}


The stoichiometric masses of Fuel, Air, and Products depend on the limiting reactants computed in Sec.~\ref{fast_parallel_reactions}, which discusses fast parallel reactions:
\begin{align}
m_{\rm F,st} &= \sum_i |\Delta \hat{Y}_{{\rm F},i}| \quad \mbox{for fast reactions $i$}\\
m_{\rm A,st} &= \sum_i \left|\left(\frac{\nu_{{\rm A},i} \,W_{{\rm A},i}}{\nu_{{\rm F},i} \,W_{{\rm F},i}}\right)\Delta \hat{Y}_{{\rm F},i} \right| \quad \mbox{for fast reactions $i$} \\
m_{\rm P,st} &= m_{\rm A,st} \, Z_{\rm P}/Z_{\rm A}
\end{align}

The components of the mixture composition vectors are given by
\begin{align}
\overline{Z}_{{\rm F,st},\alpha} &= \frac{\sum_i |\Delta \hat{Y}_{\alpha,i}| \quad (\mbox{for $\alpha$=Fuel in fast reaction $i$}) }{m_{\rm F,st}} \\
\overline{Z}_{{\rm A,st},\alpha} &= \frac{\sum_i \left|\left(\frac{\nu_{\alpha,i} \,W_\alpha}{\nu_{{\rm F},i} \,W_{{\rm F},i}}\right)\Delta \hat{Y}_{{\rm F},i} \right| \quad (\mbox{for $\alpha$=Air in fast reaction $i$}) }{m_{\rm A,st}} \\
\overline{Z}_{{\rm P,st},\alpha} &= \frac{GQ}{m_{\rm P,st}} \\
\overline{Z}_{{\rm PFP,st},\alpha} &= \frac{\sum_i \left|\left(\frac{\nu_{\alpha,i} \,W_\alpha}{\nu_{{\rm F},i} \,W_{{\rm F},i}}\right)\Delta \hat{Y}_{{\rm F},i} \right| \quad \mbox{(for $\nu_{\alpha,i}>0$)} \quad + \quad GQ}{m_{\rm F,st} + m_{\rm A,st} + m_{\rm P,st}} 
\end{align}

The limiting flame temperature is taken as a mass-weighted average based on the stoichiometric fuel mixture composition:
\begin{equation}
T_{\rm LFT} = \frac{\sum_i |\Delta \hat{Y}_{\alpha,i}| \, T_{{\rm LFT},i} \quad (\mbox{for $\alpha$=Fuel in fast reaction $i$}) }{m_{\rm F,st}} 
\end{equation}
where $T_{{\rm LFT},i}$ is the LFT of reaction $i$.
