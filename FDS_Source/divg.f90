...
DP  = 0._EB
IF (N_SPECIES > 0 .AND. .NOT.EVACUATION_ONLY(NM)) DEL_RHO_D_DEL_Y = 0._EB
...
IF (NLP>0 .AND. N_EVAP_INDICIES > 0 .AND. .NOT.EVACUATION_ONLY(NM)) THEN
...
DO IPZ=1,N_ZONE
   ...
   DO IOPZ=N_ZONE,0,-1
      ...
      ENDIF
   ENDDO
   USUM_ADD(IPZ) = USUM_ADD(IPZ) + RDT*PSUM(IPZ,NM)*(PBAR(1,IPZ)-P_EQUILIBRIUM_NUM/P_EQUILIBRIUM_DEN)
ENDDO
      ...
      IF (CONNECTED_ZONES(IPZ,IOPZ,NM)) THEN
         DO IOPZ2=N_ZONE,0,-1
            ...
            IF (CONNECTED_ZONES(IOPZ,IOPZ2,NM)) THEN
               ...
               IF (ASUM(IOPZ,IOPZ2,NM)>0._EB) RF = MIN(RF,PRESSURE_RELAX_FACTOR,10._EB*ASUM(IOPZ,IOPZ2,NM)/ABS(USUM_ADD(IOPZ)))
            ENDIF
         ENDDO
      ENDIF
   ENDDO
