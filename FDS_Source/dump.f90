...
USE COMP_FUNCTIONS, ONLY : SECOND
USE EVAC, ONLY : EMESH_NFIELDS, EVAC_FDS6
REAL(EB) :: TNOW, TTMP
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: NM
CHARACTER(30) :: FN_UVW
...
! Dump the EVAC flowfieds for all EVAC meshes
   IF (EVAC_FDS6 .AND. EVACUATION_GRID(NM) .AND. ABS(ICYC)==EVAC_TIME_ITERATIONS) THEN
      TTMP = EVAC_DT_FLOWFIELD*EMESH_NFIELDS(NM)*EVAC_TIME_ITERATIONS/MAXVAL(EMESH_NFIELDS)
   ELSE
      TTMP = EVAC_DT_FLOWFIELD*EVAC_TIME_ITERATIONS
   END IF
   IF (EVAC_FDS6 .AND. EVACUATION_GRID(NM)) CALL DUMP_SLCF(T-TTMP,NM,0)
