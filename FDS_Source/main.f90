...
USE EVAC
USE TURBULENCE, ONLY: ANALYTICAL_SOLUTION,SANDIA_DAT,INIT_TURB_ARRAYS,SPECTRAL_OUTPUT,COMPRESSION_WAVE
USE OPENMP
USE MPI
USE EMBEDDED_MESH_METHOD, ONLY: SCALARF_EMB,VELOCITY_EMB,RESTRICT_MASS_EMB,RESTRICT_DIV_EMB,SCALAR_GHOST_EMB, &
                                PROJECT_VELOCITY,MATCH_VELOCITY_EMB,SORT_MESH_LEVEL
...
INTEGER  :: LO10,NM,IZERO,REVISION_NUMBER,IOS,IERROR
...
MAIN_LOOP: DO  
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      IF (FLUX_LIMITER>=0) THEN
         ...
         ENDDO COMPUTE_FLUX_LOOP
      ENDIF
      
      IF (EMBEDDED_MESH .AND. (.NOT.ISOTHERMAL .OR. N_SPECIES>0)) THEN
         COARSE_MESH_LOOP: DO NM=NMESHES,1,-1
            IF (PROCESS(NM)/=MYID .OR. .NOT.ACTIVE_MESH(NM)) CYCLE COARSE_MESH_LOOP
            FINE_MESH_LOOP: DO NOM=NMESHES,1,-1
               IF (PROCESS(NOM)/=MYID .OR. .NOT.ACTIVE_MESH(NOM)) CYCLE FINE_MESH_LOOP
               IF (MESHES(NOM)%MESH_LEVEL/=MESHES(NM)%MESH_LEVEL+1) CYCLE FINE_MESH_LOOP
               CALL SCALARF_EMB(NM,NOM,IERROR)
            ENDDO FINE_MESH_LOOP
      ...
      IF (EMBEDDED_MESH) THEN
         DO NM=1,NMESHES
            ...
            DO NOM=1,NMESHES
               IF (PROCESS(NOM)/=MYID .OR. .NOT.ACTIVE_MESH(NOM)) CYCLE
      ...
      COMPUTE_DIVERGENCE_LOOP_A: DO NM=1,NMESHES
         IF (PROCESS(NM)/=MYID .OR. .NOT.ACTIVE_MESH(NM)) CYCLE COMPUTE_DIVERGENCE_LOOP_A
         !!IF (.NOT.ISOTHERMAL .OR. N_SPECIES>0) CALL WALL_BC(T(NM),NM)
         CALL DIVERGENCE_PART_1(T(NM),NM)
      ENDDO COMPUTE_DIVERGENCE_LOOP_A
   ...
   IF (EMBEDDED_MESH) THEN
      DO NM=1,NMESHES
         ...
         DO NOM=1,NMESHES
            IF (PROCESS(NOM)/=MYID .OR. .NOT.ACTIVE_MESH(NOM)) CYCLE
            IF (MESHES(NOM)%MESH_LEVEL/=MESHES(NM)%MESH_LEVEL+1) CYCLE
            CALL MATCH_VELOCITY_EMB(NM,NOM,IERROR)
   ...
   COMPUTE_DIVERGENCE_2A: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID .OR. .NOT.ACTIVE_MESH(NM)) CYCLE COMPUTE_DIVERGENCE_2A
      IF (.NOT.ISOTHERMAL .OR. N_SPECIES>0) THEN
         CALL COMPUTE_RADIATION(T(NM),NM)
      ENDIF
      CALL DIVERGENCE_PART_1(T(NM),NM)
   ENDDO COMPUTE_DIVERGENCE_2A
      ...
      DO NM=1,NMESHES
         ...
         IF (PREDICTOR) THEN
            DO IW=1,MESHES(NM)%NEWC
               ...
               MESHES(NM)%DH(IW) = (MESHES(NM)%H(IIG,JJG,KKG) - MESHES(NM)%HS(IIG,JJG,KKG))/MESHES(NM)%DT
               MESHES(NM)%HS(IIG,JJG,KKG) = MESHES(NM)%H(IIG,JJG,KKG) + PREDICTOR_RELAX_FACTOR*MESHES(NM)%DHS(IW)*MESHES(NM)%DT
            ENDDO
         ENDIF
         IF (CORRECTOR) THEN
            DO IW=1,MESHES(NM)%NEWC
               ...
               JJG = MESHES(NM)%IJKW(7,IW)
               KKG = MESHES(NM)%IJKW(8,IW)
               MESHES(NM)%DHS(IW) = (MESHES(NM)%HS(IIG,JJG,KKG) - MESHES(NM)%H(IIG,JJG,KKG))/MESHES(NM)%DT
               MESHES(NM)%H(IIG,JJG,KKG) = MESHES(NM)%HS(IIG,JJG,KKG) + CORRECTOR_RELAX_FACTOR*MESHES(NM)%DH(IW)*MESHES(NM)%DT
            ENDDO
         ENDIF
      ENDDO
   ELSE
...
IF (ICYC > 0) THEN
   IF (.NOT.ALL(EVACUATION_ONLY)) THEN
      ...
      IF (USE_MPI) CALL MPI_ALLGATHERV(DT_NEXT_SYNC(DISPLS(MYID)+1),COUNTS(MYID),MPI_DOUBLE_PRECISION,DT_NEXT_SYNC,COUNTS,DISPLS, &
                                       MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
