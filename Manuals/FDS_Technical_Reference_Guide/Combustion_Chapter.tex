...
\end{equation}
To generate lumped species, the number of moles of each primitive species gets divided by the sum of the moles of the primitive species. In this case O$_{2}$ and N$_{2}$ will be lumped to form Air and CO$_{2}$, H$_{2}$O, and N$_{2}$ will be lumped to form Products. The volume fractions of the primitive species must sum to unity and the stoichiometric coefficient of the lumped species is the sum of the moles of the component species.
...
\paragraph{Remark}
In Eq.~(\ref{eq:lumped_expand}), 9.52 $\times$ 0.21 is only approximately equal to 2. In practice the atom balance requires machine precision. To alleviate this issue, FDS normalizes the lumped species volume fractions and makes any necessary adjustments to the specified lumped stoichiometric coefficients.
...
In FDS, the rate expression is on a fuel basis where only the concentrations of species in the mixed state (denoted by the {\em hat} symbol) are allowed to react. Consider a set of $N_r$ reactions with fuel, $F$, and $\alpha$ species. The reaction rate for $F$ in the $i$th reaction, $\hat{r}_{F,i}$, in mol/liter/s, is:
\begin{equation}\label{eq:rate_f}
\hat{r}_{F,i}= -k_{i}\; \prod \hat{C}_{\alpha}\,^{a_{\alpha,i}}
\end{equation}
For an Arrhenius reaction $i$, the rate constant, $k_{i}$, is a function of temperature ($T^{n_i}$), the pre-exponential ($A_{i}$), and the activation energy ($E_{i}$):
\begin{equation}\label{eq:rate_cons}
k_{i} = A_{i}\;T^{n_i}\;e^{-E_{i}/RT}
\end{equation}
...
It is more convenient for FDS to work in terms of mass fractions $\hat{Y}_{\alpha}$.  The concentration and mass fractions are related by $\hat{C}_{\alpha}=\hat{Y}_{\alpha} \rho/W_{\alpha}$.  Substituting for $\hat{C}_{\alpha}$ and combining Eqs.~(\ref{eq:rate_f}) and (\ref{eq:rate_cons}) yields:
...
\hat{r}^{\prime}_{F,i} = -A_i^{\prime}\;T^{n_i}\;e^{-E_i/RT}\;\prod \hat{Y}_{\alpha}\,^{a_{\alpha,i}}
\end{equation}
...
\end{equation}
Last, the change in composition for species $\alpha$ in the mixed reactor zone becomes:
