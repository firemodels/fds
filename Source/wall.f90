...
! For thermally-thick boundary conditions, set the flag, CALL_HT_1D, to call the subroutine SOLID_HEAT_TRANSFER_1D.
! This routine is called every WALL_INCREMENT time steps.
! Also, if any 3D heat transfer calculations are done, set a new "sweep direction" (1, 2, or 3) 
...
SOLID_PARTICLES_IF: IF (SOLID_PARTICLES) THEN
   DO IP = 1, NLP
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN  ! Target particles are included to get gas phase values
         ...
         IF (LPC%SOLID_PARTICLE .AND. .NOT.SF%THERMAL_BC_INDEX==THERMALLY_THICK) CALL CALCULATE_TMP_F(PARTICLE_INDEX=IP)
...
write(0,*) T,RADIUS,B1%M_DOT_G_PP_ACTUAL
...
!> \brief Compute density at wall from wall temperatures and mass fractions
...
REAL(EB) :: RDT,DTMP,QDXKF,QDXKB,RR,RFACF,RFACB,RFACF2,RFACB2,Q_RAD_IN_B,RFLUX_UP,RFLUX_DOWN,E_WALLB, &
            ...
            RADIUS,HTC_LIMIT,CP1,CP2,DENOM,SF_HTC_F,SF_HTC_B
...
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: RHO_DOT
...
POINT_LOOP1: DO I=1,NWP
   ...
   IF (PRESENT(PARTICLE_INDEX)) THEN
      CALL PYROLYSIS(ONE_D%N_MATL,ONE_D%MATL_INDEX,SURF_INDEX,BC%IIG,BC%JJG,BC%KKG,ONE_D%TMP(I),B1%TMP_F,BC%IOR,&
                     RHO_DOT(1:ONE_D%N_MATL,I),RHO_TEMP(1:ONE_D%N_MATL),ONE_D%X(I-1),DT_BC_SUB,&
                     M_DOT_G_PPP_ADJUST,M_DOT_G_PPP_ACTUAL,M_DOT_S_PPP,Q_S(I),Q_DOT_G_PPP,Q_DOT_O2_PPP,&
                     Q_DOT_PART,M_DOT_PART,T_BOIL_EFF,B1%B_NUMBER,LAYER_INDEX(I),REMOVE_LAYER,SOLID_CELL_INDEX=I,&
...
SUBROUTINE PYROLYSIS(N_MATS,MATL_INDEX,SURF_INDEX,IIG,JJG,KKG,TMP_S,TMP_F,IOR,RHO_DOT_OUT,RHO_S,DEPTH,DT_BC,&
                     ...
                     Q_DOT_PART,M_DOT_PART,T_BOIL_EFF,B_NUMBER,LAYER_INDEX,REMOVE_LAYER,SOLID_CELL_INDEX,&
                     R_DROP,LPU,LPV,LPW)
