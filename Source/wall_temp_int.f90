   ...
   K_LOOP_2: DO K=OB%K1+1,OB%K2
      J_LOOP_2: DO J=OB%J1+1,OB%J2
         I_LOOP_2: DO I=OB%I1+1,OB%I2
            ...
            CALL PYROLYSIS(MS%N_MATL,MS%MATL_INDEX,OB%MATL_SURF_INDEX,IIG,JJG,KKG,1._EB,TMP_S,1._EB,1._EB,TMP_F,IOR,&
...
REAL(EB) :: DTMP,QDXKF,QDXKB,RR,RFACF,RFACB,RFACF2,RFACB2, &
            ...
            ZZ_G(1:N_TRACKED_SPECIES),TMP_F_PYRO,TMP_B_PYRO
...
PYROLYSIS_PREDICTED_IF: IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
   ...
   POINT_LOOP1: DO I=1,NWP
      ...
      ENDDO
      IF (I==1) THEN
         TMP_F_PYRO = ONE_D%TMP_F
      ELSE
         TMP_F_PYRO = ONE_D%TMP(I-1)
      ENDIF
      IF (I==NWP) THEN
         TMP_B_PYRO = ONE_D%TMP_B
      ELSE
         TMP_B_PYRO = ONE_D%TMP(I+1)
      ENDIF
      IF (PRESENT(PARTICLE_INDEX)) THEN
         CALL PYROLYSIS(SF%N_MATL,SF%MATL_INDEX,SURF_INDEX,BC%IIG,BC%JJG,BC%KKG,TMP_B_PYRO,ONE_D%TMP(I),TMP_F_PYRO,&
                        DX_S(I),R_S(I-1),BC%IOR,&
                        ...
                        Q_DOT_PART,M_DOT_PART,T_BOIL_EFF,ONE_D%B_NUMBER,NWP,SOLID_CELL_INDEX=I,&
...
SUBROUTINE PYROLYSIS(N_MATS,MATL_INDEX,SURF_INDEX,IIG,JJG,KKG,TMP_B,TMP_S,TMP_F,DX,R,IOR,RHO_DOT_OUT,RHO_S,DEPTH,DT_BC,&
                     M_DOT_G_PPP_ADJUST,M_DOT_G_PPP_ACTUAL,M_DOT_S_PPP,Q_DOT_S_PPP,Q_DOT_G_PPP,Q_DOT_O2_PPP,&
                     Q_DOT_PART,M_DOT_PART,T_BOIL_EFF,B_NUMBER,NODE_MAX,SOLID_CELL_INDEX,&
                     R_DROP,LPU,LPV,LPW)
...
INTEGER, INTENT(IN) :: N_MATS,SURF_INDEX,IIG,JJG,KKG,IOR,NODE_MAX
INTEGER, INTENT(IN), OPTIONAL :: SOLID_CELL_INDEX
REAL(EB), INTENT(OUT), DIMENSION(:,:) :: RHO_DOT_OUT(N_MATS)
REAL(EB), INTENT(IN) :: TMP_B,TMP_S,TMP_F,DT_BC,DEPTH,R,DX
...
INTEGER :: N,NN,J,NS,SMIX_INDEX(N_MATS),NWP,NP,NP2,ITMP,NN_MAX
TYPE(MATERIAL_TYPE), POINTER :: ML
TYPE(SURFACE_TYPE), POINTER :: SF
REAL(EB) :: REACTION_RATE,Y_O2,X_O2,Q_DOT_S_PPP,MW(N_MATS),Y_GAS(N_MATS),Y_TMP(N_MATS),Y_SV(N_MATS),X_SV(N_MATS),X_L(N_MATS),&
            ...
            SCALE,TMP_1,TMP_2,TMP_NN,RR_NN,R_NN_MAX,RATIO,TFAC
   ...
   REACTION_LOOP: DO J=1,ML%N_REACTIONS  ! Loop over the reactions (beta subscript)
      ...
      SELECT CASE (ML%PYROLYSIS_MODEL)
         ...
         CASE (PYROLYSIS_SOLID)
            ! If a 1D surface integrate the REACTION_RATE over the node
            HT3D_IF: IF (.NOT. SF%HT3D) THEN
               IF (SOLID_CELL_INDEX==1) THEN
                  TMP_1 = TMP_F
                  TMP_2 = 0.5_EB*(TMP_S+TMP_B)
               ELSEIF (SOLID_CELL_INDEX==NODE_MAX) THEN
                  TMP_1 = 0.5_EB*(TMP_F+TMP_S)                 
                  TMP_2 = TMP_B
               ELSE
                  TMP_1 = 0.5_EB*(TMP_F+TMP_S)                 
                  TMP_2 = 0.5_EB*(TMP_S+TMP_B)                 
               ...
               RATIO = EXP(-ML%E(J)/(R0*MAX(TMP_1,TMP_2)))/EXP(-ML%E(J)/(R0*MIN(TMP_1,TMP_2))) 
               RATIO_IF: IF ( RATIO <1.4_EB) THEN
                  ...
                  WRITE(*,'(2(I4,1X),3(F12.5,1X),E12.5)') SOLID_CELL_INDEX,1,TMP_F,TMP_S,TMP_B,REACTION_RATE
               ELSE RATIO_IF
                  NN_MAX = NINT(LOG(RATIO) + 2._EB)*2
                  R_NN_MAX = 1._EB/REAL(NN_MAX,EB)
                  REACTION_RATE = 0._EB

                  SELECT CASE (SF%GEOMETRY)
                     CASE (SURF_CARTESIAN)
                        SCALE = 1._EB/REAL(NN_MAX,EB)
                     CASE (SURF_CYLINDRICAL)
                        SCALE = 1._EB/(R**2-(R-DX)**2)
                     CASE (SURF_SPHERICAL)
                        SCALE = 1._EB/(R**3-(R-DX)**3)
                  END SELECT   
               
                  DO NN=1,NN_MAX
                     TFAC = (REAL(NN,EB)-0.5)*R_NN_MAX
                     IF( TFAC <=0.5_EB) THEN
                        TMP_NN = TMP_1+(TMP_S-TMP_1)*TFAC*2._EB
                     ELSE
                        TMP_NN = TMP_S+(TMP_2-TMP_S)*(TFAC-0.5_EB)*2._EB
                     ENDIF
                     ! Tech Guide: r_alpha,beta (1/s)
                     RR_NN = ML%A(J)*(RHO_S(N))**ML%N_S(J)*EXP(-ML%E(J)/(R0*TMP_NN))
                     ! power term
                     IF (ABS(ML%N_T(J))>=TWO_EPSILON_EB) RR_NN = RR_NN * TMP_NN**ML%N_T(J)
                     SELECT CASE (SF%GEOMETRY)
                        CASE (SURF_CARTESIAN)
                           REACTION_RATE = REACTION_RATE + RR_NN
                        CASE (SURF_CYLINDRICAL)
                           REACTION_RATE = REACTION_RATE + RR_NN*((R-REAL(NN-1,EB)*R_NN_MAX*DX)**2-(R-REAL(NN,EB)*R_NN_MAX*DX)**2)
                        CASE (SURF_SPHERICAL)
                           REACTION_RATE = REACTION_RATE + RR_NN*((R-REAL(NN-1,EB)*R_NN_MAX*DX)**3-(R-REAL(NN,EB)*R_NN_MAX*DX)**3)
                     END SELECT
                  ENDDO
                  REACTION_RATE = REACTION_RATE * SCALE   
                  WRITE(*,'(2(I4,1X),3(F12.5,1X),E12.5)') SOLID_CELL_INDEX,NN_MAX,TMP_F,TMP_S,TMP_B,REACTION_RATE
               ENDIF RATIO_IF
            ! For an HT3D OBST just use the cell temperature
            ELSE HT3D_IF
               REACTION_RATE = ML%A(J)*(RHO_S(N))**ML%N_S(J)*EXP(-ML%E(J)/(R0*TMP_S))
               IF (ABS(ML%N_T(J))>=TWO_EPSILON_EB) REACTION_RATE = REACTION_RATE * TMP_S**ML%N_T(J)               
            ENDIF HT3D_IF
