...
INTEGER :: N,NN,I,J,K,NM,NX,NY,NZ,NIN,NXL,NYL,NZL,NDV,NDVOLD,NDVDIM,INDX,IZERO,EVAC_CODE,STATE_INDEX
...
DO N=1,N_TREES
   IF (VEG_FUEL_GEOM(N) == 'CONE' .OR. VEG_FUEL_GEOM(N) == 'CYLINDER') THEN
      ...
      WRITE(LU_SMV,'(/A)') 'DEVICE'
      WRITE(LU_SMV,'(A)') TRIM(VEG_DEVICE)
...
EVAC_ONLY5: IF (ANY(EVACUATION_GRID).AND.MYID==MAX(0,EVAC_PROCESS)) THEN
   ...
   DO N=1,N_SSTANDS
      ...
      ENDIF
      WRITE(LU_SMV,'(3F6.2,3I6)') XX, 0.1, YY, EVAC_SSTANDS(N)%RGB(1:3)
   ENDDO
END IF EVAC_ONLY5
...
DO NM=1,NMESHES
   ...
   IF (M%PRESSURE_ITERATIONS>1)  WRITE(LU_OUTPUT,'(A,I6)')     '       Pressure Iters: ',M%PRESSURE_ITERATIONS
...
IF (COLUMN_DUMP_LIMIT) THEN
   DO I=1,N_DEVC_FILES
      ...
      WRITE(LU_DEVC(I),TCFORM) DEVC_TIME,(DEVICE(N)%VALUE/DEVICE(N)%TIME_INTERVAL, &
                               N=DEVC_COLUMN_LIMIT*(I-1)+1, MIN(N_DEVC,DEVC_COLUMN_LIMIT*I))
   ENDDO
ELSE
   WRITE(TCFORM,'(A,I4.4,A)') "(",N_DEVC,"(ES15.7E3,','),ES15.7E3)"
   WRITE(LU_DEVC(1),TCFORM) DEVC_TIME,(DEVICE(N)%VALUE/DEVICE(N)%TIME_INTERVAL, N=1,N_DEVC)
ENDIF
