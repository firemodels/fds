...
REAL(EB) :: GROTATE, GROTATE_RATE, GAXIS(3)
...
NAMELIST /GEOM/ AZIM, AZIM_RATE, COMPONENT_ONLY, DAZIM, DELEV, DSCALE, DXYZ0, DXYZ, &
                ELEV, ELEV_RATE, FACES, GAXIS, GROTATE, GROTATE_RATE,&
                GEOM_IDS, ID, SCALE, SCALE_RATE, SURF_ID, VERTS, XYZ0, XYZ, XYZ_RATE
...
READ_GEOM_LOOP: DO N=1,N_GEOMETRY
   ...
   GROTATE_RATE = 1.001_EB*MAX_COORD
...
! use GROTATE/GAXIS or AZIM/ELEV but not both
   ...
   IF(ALL(GAXIS(1:3).LT.MAX_COORD).OR.GROTATE.LT.MAX_COORD.OR.GROTATE_RATE.LT.MAX_COORD)THEN
      AZIM = 0.0_EB
      ELEV = 0.0_EB
      AZIM_RATE = 0.0_EB
      ELEV_RATE = 0.0_EB
      ...
      IF(GROTATE_RATE.GT.MAX_COORD) GROTATE_RATE = 0.0_EB
      
      IF(ALL(ABS(GAXIS(1:3)).LT.TWO_EPSILON_EB).OR.ANY(GAXIS(1:3).GT.MAX_COORD))THEN
         GAXIS(1) = 0.0_EB
         GAXIS(2) = 0.0_EB
         GAXIS(3) = 1.0_EB
      ELSE
         VECNORM = SQRT(DOT_PRODUCT(GAXIS,GAXIS))
         GAXIS = GAXIS/VECNORM
      ENDIF
   ELSE
      GAXIS(1) = 0.0_EB
   ...
   IF (ABS(GROTATE_RATE).LT.MAX_COORD) THEN
      AZIM_RATE = 0.0_EB
      ELEV_RATE = 0.0_EB
   ENDIF    
   ...
   IF (ABS(AZIM_RATE).GT.TWO_EPSILON_EB .OR. ABS(ELEV_RATE).GT.TWO_EPSILON_EB.OR.&
       ANY(ABS(SCALE_RATE(1:3)).GT.TWO_EPSILON_EB) .OR. ANY(ABS(XYZ_RATE(1:3) ).GT.TWO_EPSILON_EB) ) THEN 
      G%IS_DYNAMIC = .TRUE.
...
! transform (scale, rotate and translate) vectors found on each &GEOM line
   ...
   DO I = 0, N_GEOMETRY
      ...
      G%GROTATE = G%GROTATE_BASE + DELTA_T*G%GROTATE_RATE
