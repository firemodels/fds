...
In this section we present another case that demonstrates the second-order accuracy of the FDS transport algorithm. Consider an analytically stable flow field consisting of a single vortex advected by a uniform flow. Maintaining the geometry of the vortex over time provides a good measure of the order of accuracy of the transport scheme.
...
\end{equation}
Figure \ref{fig_vort2d_pointvelocity} shows the $u$-velocity at a single point, on the lower left fringe of the vortex, for two different mesh resolutions, $80^{2}$ and $160^{2}$. A mesh resolution of $320^{2}$ grid cells produces a plot that is, to the eye, a perfect match out to four pass-through times.
...
\paragraph{Near-Wall Grid Resolution (YPLUS)}
...
\begin{figure}[h!]
\begin{center}
   \scalebox{1.0}{ \includegraphics[width=3.2in]{SCRIPT_FIGURES/yplus} }
...
\subsection{Solid phase energy conservation {energy\_budget\_solid})}
...
\subsection{Surface Deposition of Aerosols (Propane\_flame\_deposition)}
...
\subsection{Gravitational Sedimentation of Aerosols (soot\_gravitational\_sedimentation)}

The calculations described in this section check the conservation of mass for the gravitational sedimentation algorithms. The gravitational settling algorithm involves both the settling of soot particles in the gas phase and the accumulation of soot on the condensed phase. This example involves an initial mass concentration of soot in the gas phase and the aerosol deposition model in FDS. For this example, the initial soot concentration in a 1 m$^3$ box is set as 100~mg/m$^3$ via the {\ct INIT} line, and the soot is allowed to deposit to the bottom of the box via gravitational settling only. By default, if a species is defined as an aerosol by setting {\ct AEROSOL=.TRUE.} on the {\ct SPEC} line, then all of the aerosol deposition mechanisms (gravitational, thermophoretic, and turbulent) will act upon that species. For this example, gravitational deposition is enabled and the other deposition mechanisms are disabled by using the follows inputs on the {\ct MISC} line:
...
In this example, the initial gas-phase soot concentration is specifies, and the only removal term (i.e., active deposition mechanism) is gravitational deposition. The dynamic gas-phase soot concentration can then be represented as
\be
...
In this example, the relevant surface area is the area of the lower surface where the soot deposits, and thus the surface area to volume ratio (A/V) is equal to 1~m$^{-1}$. For a soot particle with a diameter of 1 $\mu$m, the terminal settling velocity is equal to 1.9747$\times 10^{-4}$~m/s. The governing equations for the gravitational settling algorithms and settling velocities are discussed in more detail in the FDS Technical Reference Guide~\cite{FDS_Math_Guide}. Figure~\ref{fig:deposition_mass} shows a comparison of the analytical and simulated results for the time evolution of the gas-phase soot concentration.

\begin{figure}[ht]
\begin{center}
\begin{tabular}{c}
\includegraphics[width=3.2in]{SCRIPT_FIGURES/soot_gravitational_sedimentation}
\end{tabular}
\end{center}
\caption[Soot mass concentration for soot\_gravitational\_sedimentation case]{Time evolution of soot mass concentration for the soot gravitational sedimentation case.}
\label{fig:soot_gravitational_sedimentation}
\end{figure}
