...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   WC%ONE_D%RHO_F = RHOP(IIG,JJG,KKG)
   IF (TRANSPORT_UNMIXED_FRACTION) WC%ONE_D%ZZ_F(ZETA_INDEX) = ZZP(IIG,JJG,KKG,ZETA_INDEX)
...
WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   IF (WC%BOUNDARY_TYPE/=INTERPOLATED_BOUNDARY) THEN
      ...
      IF (IW<=N_EXTERNAL_WALL_CELLS .AND. WC%BOUNDARY_TYPE/=OPEN_BOUNDARY) THEN
         II = WC%ONE_D%II
         JJ = WC%ONE_D%JJ
         RHOP(II,JJ,KK) = 2._EB*WC%ONE_D%RHO_F - WC%ONE_D%RHO_G
      ENDIF
   ENDIF
