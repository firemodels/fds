...
\end{eqnarray}
where the resolved kinetic energy per unit mass is
\vskip-.6cm
\begin{align}
k_{\rm res} &= \mbox{$\frac{1}{2}$}\tilde{u}_i \tilde{u}_i
\end{align}
and the subgrid kinetic energy is estimated from Deardorff's eddy viscosity model
\begin{align}
...
If you wish to output an estimate of the subgrid kinetic energy per unit mass only, this may be done with
\begin{lstlisting}
&SLCF PBY=0, QUANTITY='SUBGRID KINETIC ENERGY' /
\end{lstlisting}
