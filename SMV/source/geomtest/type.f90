...
IMPLICIT NONE
CHARACTER(255), PARAMETER :: typeid='$Id: type.f90 10043 2012-02-10 18:46:10Z drjfloyd $'
CHARACTER(255), PARAMETER :: typerev='$Revision: 10043 $'
CHARACTER(255), PARAMETER :: typedate='$Date: 2012-02-10 13:46:10 -0500 (Fri, 10 Feb 2012) $'
...
TYPE LAGRANGIAN_PARTICLE_CLASS_TYPE
   ...
   LOGICAL :: STATIC=.FALSE.,MASSLESS,MONODISPERSE=.FALSE.,EVAPORATE=.TRUE., &
...
TYPE ONE_D_M_AND_E_XFER_TYPE
   REAL(EB), ALLOCATABLE, DIMENSION(:) :: TMP,LAYER_THICKNESS,X,MASSFLUX_ACTUAL,MASSFLUX
   REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: RHO,ILW
   INTEGER, ALLOCATABLE, DIMENSION(:) :: N_LAYER_CELLS
   REAL(EB) :: AREA=0._EB,RAREA=0._EB,HEAT_TRANS_COEF=0._EB,&
               QCONF=0._EB,QRADIN=0._EB,QRADOUT=0._EB, EMISSIVITY=0._EB
   LOGICAL :: BURNAWAY,SHRINKING
END TYPE ONE_D_M_AND_E_XFER_TYPE


TYPE LAGRANGIAN_PARTICLE_TYPE
   TYPE (ONE_D_M_AND_E_XFER_TYPE) :: ONE_D
   LOGICAL :: ALREADY_ALLOCATED=.FALSE.,SHOW=.FALSE.,SPLAT=.FALSE.,IGNITOR=.FALSE.,EVAPORATE=.FALSE.
   REAL(EB) :: TMP_F=0._EB, X=0._EB,Y=0._EB,Z=0._EB, U=0._EB,V=0._EB,W=0._EB,PWT,ACCEL_X=0._EB,ACCEL_Y=0._EB,ACCEL_Z=0._EB,&
               T=-1.E9_EB,RE=0._EB, MASS=0._EB, TMP_B=0._EB
   INTEGER :: IOR=0,CLASS_INDEX=0,SPLIT_IOR=0,SURF_INDEX=0,TAG=0,WALL_INDEX=0,IIG,JJG,KKG
   
   !WUI Variables 
   INTEGER :: VEG_N_TREE_OUTPUT=0             
...
TYPE WALL_TYPE
   TYPE (ONE_D_M_AND_E_XFER_TYPE) :: ONE_D
   LOGICAL :: ALREADY_ALLOCATED=.FALSE.
   REAL(EB), POINTER, DIMENSION(:) :: A_LP_MPUA=>NULL(),AWM_AEROSOL=>NULL(),LP_CPUA=>NULL(),LP_MPUA=>NULL(),&
                                      RHODW=>NULL(),VEG_FUELMASS_L=>NULL(),VEG_MOISTMASS_L=>NULL(),&
                                      VEG_TMP_L=>NULL(),ZZ_F=>NULL()
   REAL(EB) :: VEG_HEIGHT
   REAL(EB) :: AREA_ADJUST=1._EB,AW=0._EB,DUWDT=0._EB,EW=0._EB,KW=1._EB, &
               RAW=0._EB,RDN=1._EB,RHO_F=1._EB,TMP_B=0._EB,TMP_F=0._EB, &
...
TYPE OMESH_TYPE
   ...
   TYPE(LAGRANGIAN_PARTICLE_TYPE), POINTER, DIMENSION(:) :: LAGRANGIAN_PARTICLE=>NULL()
   ...
   REAL(EB), POINTER, DIMENSION(:) :: &
         ...
         REAL_SEND_PKG6=>NULL(),REAL_SEND_PKG7=>NULL(),&
         REAL_RECV_PKG1=>NULL(),REAL_RECV_PKG2=>NULL(),REAL_RECV_PKG3=>NULL(),REAL_RECV_PKG4=>NULL(),REAL_RECV_PKG5=>NULL(),&
         REAL_RECV_PKG6=>NULL(),REAL_RECV_PKG7=>NULL()
   INTEGER , POINTER, DIMENSION(:) :: INTG_SEND_PKG1=>NULL(),INTG_RECV_PKG1=>NULL()
   INTEGER , POINTER, DIMENSION(:,:) :: INTG_SEND_PKGW   =>NULL()
   LOGICAL , POINTER, DIMENSION(:) :: LOGI_SEND_PKG1=>NULL(),LOGI_RECV_PKG1=>NULL()
END TYPE OMESH_TYPE
...
TYPE DUCTNODE_TYPE
   ...
   REAL(EB), DIMENSION(:), POINTER :: ZZ=>NULL(), DIR=>NULL(), BRANCH_LOSS=>NULL(), ZZ_V=>NULL()
