...
\section{Mass conservation of pyrolyzed mass 
	(\texorpdfstring{{\bf surf\_mass\_conservation}}
			     {surf\_mass\_concervation})}

The following tests are used to check the conservation of pyrolyzed
mass produced by the pyrolysis routine embedded in the 1D heat
transfer routine. This solver is envoked when a {\ct SURF} is used
with reacting materials {\ct MATL}. In the tests, four independent
parameters or modelling options are varied:
\begin{enumerate}
...
mixture fraction model, or an additional gas species, defined using
{\ct SPEC}.
\end{enumerate}
