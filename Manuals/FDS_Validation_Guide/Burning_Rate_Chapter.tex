...
\section{CHRISTIFIRE}
In this section, the modeling process of a PVC cable is presented. The sample cable is General Cable BICC~$\sharp$701 from the CHRISTIFIRE campaign~\cite{CHRISTIFIRE} (GENERAL CABLE BICC BRAND SUBSTATION CONTROL CABLE 7/C~$\sharp$12AWG 600V) that has seven conductors and 14~mm diameter. The sheath material is PVC and insulation PE. The mass fractions are 0.24 and 0.18, respectively, and the rest of the mass is incombustible metal conductor.

TGA experiments at 10~K/min and MCC experiments at 60~K/min were performed to sheath and insulation materials separately. Cone calorimeter experiments were performed for sample of seven parallel, 10~cm long pieces of cable at 25, 50 and 75~kW/m$^2$. The experimental methods are explained in Section~\ref{sec_FAA_Polymers}.

\subsection{Modelling process}
The components of the cable are first separated and their densities measured. 

The pyrolysis kinetics are estimated from the TGA results. The kinetic parameters depend on the choice of the reaction path, and in this demonstration two different reaction paths are used. The first one (v1) consists of several parallel, independent reactions that each yield both fuel (combustible) and inert (incombustible) gas. Only the last reaction yields also residue. The second reaction path is more complex and represents a more realistic reaction path. For PVC material, this means that the model consists of three components: pure PVC, plasticizer and CaCO$_3$. Pure PVC degrades in two steps, first yielding inert gas, little amount of fuel and residue, which degrades further in the second reaction step yielding fuel gas and residue. In air, the residual char is also oxidized, which contributes to the greater mass loss and release of fuel at high temperatures. The plasticizer degrades simultaneously with the first reaction of PVC yielding fuel gas. CaCO$_3$ degrades at high temperatures yielding only inert gas and residue. The initial mass fraction of these three components, the reaction yields and the heats of combustion are determined by comparing the mass loss results of TGA analysis to the heat release results of the MCC.

When the kinetic and reaction path parameters are estimated, they are fixed and not modified any more.  

Next step is to combine the small scale results for components and make the cone calorimeter model. In this example the cable is modeled as rectangular obstacle, which means that the geometry has to be simplified. The layer structure of the cable is symmetrical, backing is insulated by a calcium silicate board:
\begin{enumerate}
\item sheath (2.1 mm)
...
\item insulation (2.1 mm)
\item sheath (2.1 mm)
\item insulating calcium silicate board (20 mm)
\end{enumerate}
The layer thicknesses are calculated using the component mass fractions and densities. The missing parameters are estimated from the cone calorimeter results at 50~kW/m$^2$. The parameters are estimated by fitting a model to experimental heat release rate and mass loss rate curves.

After the parameters are estimated and the fit of the result is acceptable, the parameters are fixed. The model is validated by comparing the prediction of FDS and the experimental results at two other heat fluxes, 25 and 75~kW/m$^2$.
 
\subsection{Small scale results}
The fuel gas is~\textct{PROPANE} and inert gas~\textct{WATER VAPOR}. The parameters are listed in Tables~\ref{christifire_kin_parameters_v1} and~\ref{christifire_kin_parameters_v2} and the comparison of experimental and FDS results are shown visually in Fig.~\ref{christifire_small_scale_results}.

\begin{table}[h!]
\caption[Parameters from small scale results of cable~$\sharp$701: v1.]{Reaction path, kinetic parameters and heat of combustion for sheath and insulation material of cable~$\sharp$701 in v2. The values are provided in the same units as in FDS input file.}
\begin{center}
...
\begin{table}[h!]
\caption[Parameters from small scale results of cable~$\sharp$701: v2.]{Reaction path, kinetic parameters and heat of combustion for sheath and insulation material of cable~$\sharp$701 in v2. The values are provided in the same units as in FDS input file.}
\begin{center}
...
\end{tabular}
\caption{Small scale results of cable 701 sheath and insulation.}
\label{christifire_small_scale_results}
\end{figure}

\subsection{Bench scale results and validation}
The rest of the parameters were estimated by fitting the calculated cone calorimeter results to the experimental ones at 50~kW/m$^2$ (\ref{christifire_cone_results_50}). The parameters were then fixed, and the predictions of the FDS at other heat fluxes were validated at 25 and 75~kW/m$^2$ (\ref{christifire_cone_results_other_fluxes}). The remaining parameters for v1 and v2 are listed in the following tables.

\begin{table}
\begin{center}
\caption{Thermal parameters for cable 701 model: v1.}
...
\begin{table}
\begin{center}
\caption{Thermal parameters for cable 701 model: v2.}
\begin{tabular}{|l|c|c|c|c|c|}
 \hline
 & \textct{DENSITY} & \textct{CONDUCTIVITY} & \textct{SPECIFIC\_HEAT} & \textct{HEAT\_OF\_REACTION} & \textct{EMISSIVITY} \\
 \hline
...
\end{tabular}
\caption{Experimental and the fitted FDS results of cone calorimeter at 50~kW/m$^2$.}
...
\includegraphics[height=2.8in]{SCRIPT_FIGURES/CHRISTIFIRE/CHRISTIFIRE_C701_mlr_75} \\
 \includegraphics[height=2.8in]{SCRIPT_FIGURES/CHRISTIFIRE/CHRISTIFIRE_C701_ehc_75} &  \includegraphics[height=2.8in]{SCRIPT_FIGURES/CHRISTIFIRE/CHRISTIFIRE_C701_ehc_25}\\
\end{tabular}
\caption{Validation of the cone calorimeter fit at 25 and 75~kW/m$^2$.}
