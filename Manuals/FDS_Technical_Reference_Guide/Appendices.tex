...
The solid phase heat conduction equation can be solved in 3-D by applying the 1-D equation~\ref{crank-nicolson} in the following three-step alternating direction implicit (ADI) scheme:
\begin{align}
...
\section{Energy and Mass Conservation for Solid Phase Reactions}
\label{solid_energy_mass}

A solid material in FDS can undergo one or more reactions (see Section~\ref{pyrosection}) that produce new solid materials, gases, or Lagrangian particles (for example, melting or brand formation). These reactions should conserve both mass and energy. 

Mass conservation for reaction $\beta$ for solid material $\alpha$ is given by:
\be
	\sum_{\alpha'=1}^{N_{\rm m}} \nu_{\alpha',\alpha \beta} = 0 
\ee
where $N_{\rm m}$ is the number of all types of substances in FDS (solid, gas, or particle) and $\nu_{\alpha',\alpha \beta}$ is the yield of $\alpha'$ in the reaction where $\alpha'$ could be a new material, a gas, or a type of Lagrangian particle. In FDS input processing, the yield of the original material, $\nu_{\alpha,\alpha \beta}$, is taken as -1, and the prior equation written in terms of the yields as specified in an input file becomes:
\be
\sum_{\alpha'=1,\alpha' \neq \alpha}^{N_{\rm m}} \nu_{\alpha',\alpha \beta} = 1 
\ee

If the left hand side were to sum to larger than 1, this would mean mass is being created. This will result in an error message. If the left hand side sums to less than 1, this would mean mass is being destroyed. This, however, only leads to a warning message. Loosing mass is allowed to represent phenomena where mass is ejected from the solid but the user does not wish to track that mass once ejected.

For energy conservation, the enthalpy prior to the reaction must equal the enthalpy after the reaction plus the heat of reaction, $H_{\rm r,\alpha \beta}$. This in fact, defines the heat of reaction as the difference in enthalpy between the products and reactants:
\be
\sum_{\alpha'=1,\nu_{\alpha',\alpha \beta}<0}^{N_{\rm m}} -\nu_{\alpha',\alpha \beta} h_{\alpha'} \, = 
\sum_{\alpha'=1,\nu_{\alpha',\alpha \beta}>0}^{N_{\rm m}} \nu_{\alpha',\alpha \beta} h_{\alpha'} + H_{\rm r,\alpha \beta}
\label{eqn_solid_e_cons}
\ee

The equation suggests the heat of reaction is a constant; however, the heat of reaction can only be constant with temperature if all the products and reactants have the same temperature dependent specific heat. For example, consider a solid reaction that combines equal masses of two reactants each with a constant specific heat of 1~\si{kJ/(kg.K)} to produce a single product with a specific heat of 2~\si{kJ/(kg.K)}. If the solid increases in temperature by 20~K, then for 1~kg of products, the reactant enthalpy will increase by 20~\si{kJ/kg} but the product enthalpy will increase by 40~\si{kJ/kg}. For energy to be conserved, the heat of reaction at the higher temperature must decrease by 20~\si{kJ/kg}. 

Ensuring energy conservation requires that two things be performed as part of handling solid phase reactions. First, the temperature dependence of the heat of reaction must be determined. Second, in reactions combining gasses and solids, enthalpy differences due to temperature differences must be correctly assessed and apportioned to the gas and solid phase. A challenge for the first item is that for many real world materials, detailed enthalpy data is not known. Enthalpy is known for many pure solid substances (such as NaCl or SiO$_2$), but not for other types of solids like wood or many polymers (plastics and foams). If enthalpies and specific heats are known for all the components of a reaction, then a temperature dependent heat of reaction can be easily generated as the temperature dependent enthalpy difference between products and reactants. For many pyrolysis reactions of interest to fire, the heat of reaction is hundreds of kJ/kg, differences in the specific heat of products and reactants are generally less than 1~\si{kJ/(kg.K)}, and most of the mass undergoes reaction in a narrow temperature range of a few tens of K. Splitting that temperature range around the temperature of the peak reaction rate means that over the temperature range where a reaction takes place that the heat of reaction is often being adjusted by under 10~\%. 

For the first step, reference enthalpies for all the components in a reaction must be determined. For basic atmospheric gases and some common toxicants, FDS uses predefined reference enthalpies. In a typical simulation any other gases are fuels participating in combustion reactions where the reference enthalpy can be determined from the specific heat of combustion. Therefore, gas reference enthalpies are treated as known quantities for the solid phase. A similar rational applies to liquid particles produced by a solid phase reaction. Definition of a liquid particle includes specifying the gas it evaporates to and the heat of vaporization which means a reference enthalpy for the liquid can be determined. This leaves finding reference enthalpies for an solid material that is a reactant in a solid phase reaction or a product in a solid phase reaction. Products would include solid materials that remain part of the reacting surface and solid materials that leave the surface in the form of particles (i.e., a fire brand). Finding these enthalpies is done by solving a system of linear equations where there is one equation for every material reaction and one unknown for every solid material whose reference enthalpy is known.  Eq.~\ref{eqn_solid_e_cons} is applied to every material reaction. The enthalpy of material $\alpha$, $h_{\alpha}$, is given by:
\be
  h_{\alpha} = h_{\alpha,ref} + \int_0^T c_{p,\alpha}(T') \, \mbox{d}T'
\ee

The specific heats, $c_{p,\alpha}(T')$, are defined in the inputs, and evaluating the integral requires picking the upper bound temperature. This is taken as the reference temperature for the reaction. This may be user defined as part of the input. If not, it can be determined from the reaction $A_{\alpha\beta}$ and $E_{\alpha\beta}$ by performing a TGA simulation material $\alpha$, reaction $\beta$ and setting the reference temperature to the temperature at the peak reaction rate. Once the upper bound temperature is determined, the various $\nu_{\alpha',\alpha \beta} h_{\alpha,ref}$ terms can be collected on the left hand side, and the heats of reaction and the various $\nu_{\alpha',\alpha \beta} \int_0^T c_{p,\alpha}(T') \, \mbox{d}T'$ terms can be collected on the right hand side. The result is a system of linear equations $A \bar{x} = \bar{b}$ where the unknowns, $\bar{x}$, are the values of $h_{\alpha,ref}$; however, it is not guaranteed that there are an equal number of reactions and unknown material reference enthalpies.
