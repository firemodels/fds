...
TYPE :: USERDATA
    ...
    REAL(EB) :: U_IN ! Enthalpy of the intial mass fraction
END TYPE USERDATA
...
REAL(EB) :: ZETA, MIXING_FACTOR, VOL_CHANGE_TERM, SUM_OMEGA_DOT, SUM_CC, MW0, MW, VOL_MIX, SUM_H_ZZ0
...
!WRITE(LU_ERR,*)"ZETA0, TAU_MIX, CELL_MASS, U_IN=",USER_DATA%ZETA0, USER_DATA%TAU_MIX, USER_DATA%CELL_MASS, USER_DATA%U_IN
!WRITE(LU_ERR,*)"CC0=",CVEC
!WRITE(LU_ERR,*)"ZZ0=",USER_DATA%ZZ_0

ZETA = USER_DATA%ZETA0*EXP(-TN/USER_DATA%TAU_MIX)
!WRITE(LU_ERR,*)"TN, ZETA0,ZETA, TAU_MIX=",TN, USER_DATA%ZETA0,ZETA, USER_DATA%TAU_MIX
...
VOL_MIX = (1-ZETA)*USER_DATA%CELL_MASS/RHO
!WRITE(LU_ERR,*)"TN,Mix zone total volume, specific volume=", TN,(1-ZETA)*USER_DATA%CELL_MASS/RHO, 1._EB/RHO, TMP
...
FVEC(N_TRACKED_SPECIES+1)=-(FVEC(N_TRACKED_SPECIES+1) + (RHO*(SUM_H_ZZ0-USER_DATA%U_IN)-PR)*MIXING_FACTOR)/RHO/CP
...
DO NS=1,N_TRACKED_SPECIES
   !DIFF = RHO * USER_DATA%ZZ_0(NS)/SPECIES_MIXTURE(NS)%MW*MIXING_FACTOR - CVEC(NS)*MW/MW0*MIXING_FACTOR
   !WRITE(LU_ERR,*)"NS, ID, term1-term2=",NS, SPECIES_MIXTURE(NS)%ID, DIFF
...
REAL(EB) :: ZETA, MIXING_FACTOR, SUM_OMEGA_DOT, SUM_CC, SUM_OMEGA_DOT_BY_CC, SUM_CC_I, ENRG_TERM, DUMMY1, DUMMY2, DUMMY3, DUMMY4
...
! DTDOT/D[X]
ENRG_TERM = SUM_H_ZZ0-PR*RHOI-USER_DATA%U_IN
DUMMY3 = PR*RHOI
DUMMY4 = ENRG_TERM*CPI*CPI*RHOI
DO NS=1,N_TRACKED_SPECIES
   SUM_OMEGA_DOT_BY_CC= SUM(JMAT(NS,1:N_TRACKED_SPECIES))
   DUMMY1 = DUMMY3*SPECIES_MIXTURE(NS)%MW
   DUMMY1 = DUMMY1*CPI*RHOI
   DUMMY2 = DUMMY4*(CP*SPECIES_MIXTURE(NS)%MW-CP_I(NS))
   JMAT(NS, N_TRACKED_SPECIES+1) = JMAT(NS, N_TRACKED_SPECIES+1) - MIXING_FACTOR*(DUMMY1+DUMMY2)
ENDDO

! DTDOT/DT
DUMMY1 = SUM_CP_ZZ0+PR*RHOI*TMPI
DUMMY2 = ENRG_TERM*CPI*DCPDT
JMAT(N_TRACKED_SPECIES+1, N_TRACKED_SPECIES+1) = JMAT(N_TRACKED_SPECIES+1, N_TRACKED_SPECIES+1) &
         - MIXING_FACTOR*CPI*(DUMMY1+DUMMY2)
...
REAL(EB) :: ZZ(N_TRACKED_SPECIES), EQUIV, U_IN, MW0
INTEGER :: NS, NTRY, MAXTRY
...
USER_DATA%ZZ_0 = ZZ_0
CALL GET_ENTHALPY(ZZ_0,U_IN,TMP_IN)
CALL GET_MOLECULAR_WEIGHT(USER_DATA%ZZ_0,MW0)
USER_DATA%U_IN = U_IN - R0/MW0*TMP_IN
