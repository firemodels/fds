...
REAL(EB) :: LOADING(MAX_SPECIES_NL),EFFICIENCY(MAX_SPECIES_NL),LOADING_MULTIPLIER(MAX_SPECIES_NL)
...
CHARACTER(LABEL_LENGTH) :: SPEC_ID(MAX_SPECIES_NL)
...
TYPE(DUCTNODE_TYPE), POINTER :: DN=>NULL()
TYPE(DUCT_TYPE), POINTER :: DU=>NULL()
REAL(EB), ALLOCATABLE, DIMENSION(:) :: RAMP_DATA_TMP
TNOW=SECOND()
...
DO NF=1,N_FANS
   IF (FAN(NF)%FAN_TYPE/=3) CYCLE
   N = FAN(NF)%RAMP_INDEX
   ALLOCATE(RAMP_DATA_TMP(RAMPS(N)%NUMBER_DATA_POINTS))
   DO NN=1,RAMPS(N)%NUMBER_DATA_POINTS
      RAMP_DATA_TMP(NN)=RAMPS(N)%DEPENDENT_DATA(RAMPS(N)%NUMBER_DATA_POINTS+1-NN)
      RAMPS(N)%DEPENDENT_DATA(RAMPS(N)%NUMBER_DATA_POINTS+1-NN) = RAMPS(N)%INDEPENDENT_DATA(NN)
   ENDDO
   RAMPS(N)%INDEPENDENT_DATA(1:RAMPS(N)%NUMBER_DATA_POINTS) = RAMP_DATA_TMP(1:RAMPS(N)%NUMBER_DATA_POINTS)
   DEALLOCATE(RAMP_DATA_TMP)
ENDDO
...
DO NNE = 1, N_NETWORKS
   ...
   IF (NE%N_MATRIX > 0) THEN 
      ...
      DO WHILE (ITER < ITER_MAX)
         ...
         ITER = ITER + 1
         IF (ITER < ITER_MAX) CALL SET_GUESS(NNE,T,ITER)        
      ENDDO
      DEALLOCATE(LHS)
      DEALLOCATE(RHS)
   ELSE
...
SELECT CASE (FA%FAN_TYPE)
   ...
   CASE(2) !Quadratic
      ...
      VDOT = FA%MAX_FLOW*SIGN(1._EB,FA%MAX_PRES-DEL_P)*SQRT(ABS(FA%MAX_PRES-DEL_P)/FA%MAX_PRES)
      IF (DU%REVERSE) VDOT = -VDOT
      DU%VOLUME_FLOW = 0.8_EB*DU%VOLUME_FLOW+0.2_EB*VDOT
   ...
   CASE(3) !Fan curve
      DEL_P = DUCTNODE(DU%NODE_INDEX(2))%P - DUCTNODE(DU%NODE_INDEX(1))%P
      IF (DU%REVERSE) DEL_P = - DEL_P
      CALL INTERPOLATE1D(RAMPS(FA%RAMP_INDEX)%INDEPENDENT_DATA,RAMPS(FA%RAMP_INDEX)%DEPENDENT_DATA,DEL_P,VDOT)
      IF (DU%REVERSE) VDOT = -VDOT
      DU%VOLUME_FLOW = 0.8_EB*DU%VOLUME_FLOW+0.2_EB*VDOT
      DU%VEL(NEW) = DU%VOLUME_FLOW/DU%AREA      
!      VDOT = DU%VEL(NEW)*DU%AREA
...
DUCT_LOOP: DO ND=1,N_DUCTS
   DU=>DUCT(ND)
   IF (.NOT. DU%FAN_INDEX>0) DU%VOLUME_FLOW = 0._EB
   ...
   IF (DU%FAN_INDEX>0) THEN
      ...
      ELSEIF (DU%CTRL_INDEX > 0) THEN
         ...
         IF (DU%FAN_OPERATING .AND. (CONTROL(DU%CTRL_INDEX)%CURRENT_STATE .NEQV. CONTROL(DU%CTRL_INDEX)%PRIOR_STATE)) &
            DU%FAN_ON_TIME = T
      ENDIF
      IF (DU%FAN_OPERATING) THEN! .AND. FAN(DU%FAN_INDEX)%FAN_TYPE==1) THEN
         DU%FIXED=.TRUE.
         WRITE(*,*) 'FA',DU%FIXED,DU%FAN_OPERATING
         CALL UPDATE_FAN(T,ND)
      ELSE!IF (.NOT. DU%FAN_OPERATING .AND. FAN(DU%FAN_INDEX)%FAN_TYPE==1) THEN
         DU%FIXED = .FALSE.
         DU%VOLUME_FLOW = 0._EB
      ENDIF              
   ENDIF
...
SUBROUTINE SET_GUESS(NNE,T,ITER)
