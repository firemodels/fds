...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_DROPLETS,ALLOW_UNDERSIDE_DROPLETS,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,CHECK_GR,CHECK_HT,CHECK_VN,CLIP_MASS_FRACTION,CP_FTMP,CSMAG,C_FORCED, &
                C_FORCED_CYLINDER,C_FORCED_SPHERE,C_HORIZONTAL,C_VERTICAL,DEBUG_OPENMP,&
                DNS,&
                EVACUATION_DRILL,EVACUATION_MC_MODE,EVAC_PRESSURE_ITERATIONS,EVAC_TIME_ITERATIONS,FLUX_LIMITER,FORCE_VECTOR,&
                FREEZE_VELOCITY,FYI,GAMMA,GROUND_LEVEL,GVEC,GRAV_VISC,HRRPUVCUT_MAX,HRRPUV_MAX_SMV,IMMERSED_BOUNDARY_METHOD,&
                INCLUDE_NUMERICAL_DIFFUSION,LAPSE_RATE,&
                LES,LIMITING_DT_RATIO,MAXIMUM_VISIBILITY,MEAN_FORCING,NEW_EVAP,NOISE,NO_EVACUATION,NSTRATA,&
                ...
                STRATIFICATION,TERRAIN_CASE,TERRAIN_IMAGE,TEXTURE_ORIGIN,THERMOPHORETIC_DEPOSITION,THICKEN_OBSTRUCTIONS,TMPA,&
                TURBULENT_DEPOSITION,TURBULENCE_MODEL,U0,UVW_FILE,V0,VEG_LEVEL_SET,VISIBILITY_FACTOR,VN_MAX,VN_MIN,W0,&
                WFDS,WIND_ONLY
...
NAMELIST /DUMP/ COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEBUG,DEVC_COLUMN_LIMIT,DT_BNDF,DT_CTRL,DT_DEVC,DT_DEVC_LINE,DT_FLUSH,&
                DT_HRR,DT_ISOF,DT_MASS,DT_PART,DT_PL3D,DT_PROF,DT_RESTART,DT_SL3D,DT_SLCF,DT_VEG,EB_PART_FILE,FLUSH_FILE_BUFFERS,&
                MASS_FILE,MAXIMUM_DROPLETS,NFRAMES,PLOT3D_PART_ID,PLOT3D_QUANTITY,PLOT3D_SPEC_ID,PLOT3D_VELO_INDEX,&
...
LOGICAL :: A_TMP,L_TMP,CHECK_ATOM_BALANCE
NAMELIST /REAC/ A,AUTO_IGNITION_TEMPERATURE,BETA_EDC,C,CHECK_ATOM_BALANCE,CO_PRODUCTION,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                E,EPUMO2,EQUATION,FIXED_MIX_TIME,FORMULA,FUEL,FYI,H,HEAT_OF_COMBUSTION,HRRPUA_SHEET,HRRPUV_AVERAGE,HUMIDITY,&
                ID,IDEAL,N,NU,N_S,N_T,O,ODE_SOLVER,REAC_ATOM_ERROR,REAC_MASS_ERROR,SMIX_ID,SOOT_DEPOSITION,SOOT_H_FRACTION,&
                SOOT_YIELD,SPEC_ID,SUPPRESSION,TAU_CHEM,TAU_FLAME,THRESHOLD_TEMPERATURE,Y_CO2_INFTY,Y_O2_INFTY,Y_P_MIN_EDC
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CHILD_DIAMETER,BREAKUP_CHILD_GAMMA_D,BREAKUP_CHILD_SIGMA_D,COLOR,COMPLEX_REFRACTIVE_INDEX,&
                CTRL_ID,DENSE_VOLUME_FRACTION,DENSITY,DEVC_ID,DIAMETER,DRAG_LAW,EVAPORATE,FYI,GAMMA_D,HEAT_OF_COMBUSTION,&
...
REAL(EB) :: ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BETA_C,BETA_E, &
            ...
            DROPLET_VELOCITY,FLOW_RATE,FLOW_TAU,GAUGE_TEMPERATURE,INITIAL_TEMPERATURE,K_FACTOR,LENGTH,SPRAY_ANGLE(2,2), &
...
NAMELIST /PROP/ ACTIVATION_OBSCURATION,ACTIVATION_TEMPERATURE,ALPHA_C,ALPHA_E,BEAD_DENSITY,BEAD_DIAMETER,BEAD_EMISSIVITY,&
                ...
                CABLE_MASS_PER_LENGTH,CHARACTERISTIC_VELOCITY,CONDUIT_DIAMETER,CONDUIT_THICKNESS,C_FACTOR,DROPLETS_PER_SECOND,&
                DROPLET_VELOCITY,FLOW_RAMP,FLOW_RATE,FLOW_TAU,GAUGE_TEMPERATURE,ID,INITIAL_TEMPERATURE,K_FACTOR,LENGTH,OFFSET,&
                OPERATING_PRESSURE,ORIFICE_DIAMETER,P0,PART_ID,PDPA_END,PDPA_INTEGRATE,PDPA_M,PDPA_N,PDPA_NORMALIZE,PDPA_RADIUS,&
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   PY%DROPLETS_PER_SECOND      = DROPLETS_PER_SECOND
   PY%OFFSET                   = OFFSET
...
NAMELIST /SURF/ ADIABATIC,BACKING,BURN_AWAY,CELL_SIZE_FACTOR,COLOR,CONVECTION_LENGTH_SCALE,CONVECTIVE_HEAT_FLUX,DEFAULT,&
...
NAMELIST /CLIP/ MAXIMUM_DENSITY,MAXIMUM_TEMPERATURE,MINIMUM_DENSITY,MINIMUM_TEMPERATURE,FYI
...
REAL(EB) :: SPREAD_RATE,TRANSPARENCY,XYZ(3),TMP_EXTERIOR,DYNAMIC_PRESSURE, &
            REYNOLDS_STRESS(3,3),L_EDDY,VEL_RMS,L_EDDY_IJ(3,3)
...
NAMELIST /INIT/ CTRL_ID,DENSITY,DEVC_ID,DIAMETER,DT_INSERT,HRRPUV,MASS_FRACTION,MASS_PER_TIME,MASS_PER_VOLUME,MULT_ID,&
                NUMBER_INITIAL_DROPLETS,ORIGIN,PART_ID,PROF_ID,SHAPE,SPEC_ID,TEMPERATURE,UNIFORM,UVW,XB,XYZ
...
USE GLOBAL_CONSTANTS, ONLY: LU_INPUT,N_NODE
REAL(EB) :: XYZ(3)=0._EB
INTEGER :: I
NAMELIST /NODE/ XYZ
...
REWIND(LU_INPUT)
COUNT_NODE_LOOP: DO
   CALL CHECKREAD('NODE',LU_INPUT,IOS)
   IF (IOS==1) EXIT COUNT_NODE_LOOP
   READ(LU_INPUT,NML=NODE,END=14,ERR=15,IOSTAT=IOS)
   N_NODE=N_NODE+1
   15 IF (IOS>0) CALL SHUTDOWN('ERROR: problem with NODE line')
ENDDO COUNT_NODE_LOOP
14 REWIND(LU_INPUT)

IF (N_NODE==0) RETURN

! Allocate TRINODES array

ALLOCATE(TRINODES(N_NODE),STAT=IZERO)
CALL ChkMemErr('READ','NODE',IZERO)

READ_NODE_LOOP: DO I=1,N_NODE
   
   CALL CHECKREAD('NODE',LU_INPUT,IOS)
   IF (IOS==1) EXIT READ_NODE_LOOP
   
   ! Read the NODE line
   
   READ(LU_INPUT,NODE,END=36)
   
   TRINODES(I)%X = XYZ(1)
   TRINODES(I)%Y = XYZ(2)
   TRINODES(I)%Z = XYZ(3)

ENDDO READ_NODE_LOOP
36 REWIND(LU_INPUT)

END SUBROUTINE READ_NODE


SUBROUTINE READ_TRIS

USE GLOBAL_CONSTANTS, ONLY: LU_INPUT,N_TRIS
USE COMPLEX_GEOMETRY, ONLY: INIT_TRIS
INTEGER :: NODES(3),I
CHARACTER(30) :: SURF_ID
TYPE(TRIANGULAR_SURFACE_TYPE), POINTER :: TRI=>NULL()
NAMELIST /TRIS/ NODES,SURF_ID

N_TRIS=0
REWIND(LU_INPUT)
COUNT_TRIS_LOOP: DO
   CALL CHECKREAD('TRIS',LU_INPUT,IOS)
   IF (IOS==1) EXIT COUNT_TRIS_LOOP
   READ(LU_INPUT,NML=TRIS,END=16,ERR=17,IOSTAT=IOS)
   N_TRIS=N_TRIS+1
   16 IF (IOS>0) CALL SHUTDOWN('ERROR: problem with TRIS line')
ENDDO COUNT_TRIS_LOOP
17 REWIND(LU_INPUT)

IF (N_TRIS==0) RETURN

! Allocate TRISURF array

ALLOCATE(TRIANGULAR_SURFACE(N_TRIS),STAT=IZERO)
CALL ChkMemErr('READ','TRIS',IZERO)

READ_TRIS_LOOP: DO I=1,N_TRIS
   TRI=>TRIANGULAR_SURFACE(I)
   
   CALL CHECKREAD('TRIS',LU_INPUT,IOS)
   IF (IOS==1) EXIT READ_TRIS_LOOP
   
   ! Read the TRIS line
   
   READ(LU_INPUT,TRIS,END=37)
   
   TRI%NODE(1) = NODES(1)
   TRI%NODE(2) = NODES(2)
   TRI%NODE(3) = NODES(3)
   TRI%SURF_ID = TRIM(SURF_ID)

ENDDO READ_TRIS_LOOP
37 REWIND(LU_INPUT)

CALL INIT_TRIS

END SUBROUTINE READ_TRIS
