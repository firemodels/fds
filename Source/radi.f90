...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         INRAD_W(IW) = SUM(-DLN(WALL(IW)%ONE_D%IOR,:)* &
            WALL(IW)%BOUNDARY_PROPERTY%BAND(IBND)%ILW(:), 1, DLN(WALL(IW)%ONE_D%IOR,:)<0._EB)
      ENDDO
                     ...
                     CASE(1)
                        ...
                        IF (COS_DL>0._EB) THEN
                           IF (LPC%MASSLESS_TARGET) THEN
                              LP%BAND(IBND)%ILW(N) = COS_DL * IL(LP%ONE_D%IIG,LP%ONE_D%JJG,LP%ONE_D%KKG)
                              IF (N==NEAREST_RADIATION_ANGLE(LP%ORIENTATION_INDEX)) &
                                 LP%IL(IBND) = IL(LP%ONE_D%IIG,LP%ONE_D%JJG,LP%ONE_D%KKG)
                           ELSE
                              ! IL_UP does not account for the absorption of radiation within the cell occupied by the particle
                              LP%BAND(IBND)%ILW(N) = COS_DL * IL_UP(LP%ONE_D%IIG,LP%ONE_D%JJG,LP%ONE_D%KKG)
                           ENDIF
                        ENDIF
                     CASE(2:)
