...
INTEGER, ALLOCATABLE, DIMENSION(:) :: REQ,COUNTS,DISPLS,COUNTS2D,DISPLS2D,COUNTS_TIMERS,DISPLS_TIMERS,COUNTS_MASS,DISPLS_MASS
INTEGER, ALLOCATABLE, DIMENSION(:,:) :: ARRAY_OF_STATUSES
INTEGER :: N_REQ
CHARACTER(MPI_MAX_PROCESSOR_NAME) PNAME
REAL(EB) :: MPI_WALL_TIME,MPI_WALL_TIME_START,TNOW2
...
MAIN_LOOP: DO  
   ...
   !Timo:IF (ANY(EVACUATION_GRID)) THEN
   !Timo:   IF (MOD(ICYC,10)==0) EXCHANGE_EVACUATION = .TRUE.
   !Timo:ENDIF
...
!TUSED(16,:) = TUSED(16,:) + MPI_WTIME() - TMPI
!TMPI = MPI_WTIME()

IF (RECOMPUTE_A) THEN
   A_LOC = TRANSPOSE(A_LOC)
   CALL MPI_GATHERV(A_LOC(1,DISPLS(MYID)+1),COUNTS2D(MYID),MPI_DOUBLE_PRECISION,A_LOC,COUNTS2D,DISPLS2D, &
                    MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF

!TUSED(17,:) = TUSED(17,:) + MPI_WTIME() - TMPI
!TMPI = MPI_WTIME()

CALL MPI_GATHERV(B_LOC(DISPLS(MYID)+1),COUNTS(MYID),MPI_DOUBLE_PRECISION,B_LOC,COUNTS,DISPLS, &
                 MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)

!TUSED(18,:) = TUSED(18,:) + MPI_WTIME() - TMPI
!TMPI = MPI_WTIME()

!IF (MYID==0) THEN
!   IF (ICYC==1) A = TRANSPOSE(A_LOC)
!   IF (ICYC==1) AINV = A
!   IF (ICYC >1) A = AINV
!   B = B_LOC
...
!TUSED(20,:) = TUSED(20,:) + MPI_WTIME() - TMPI
!TMPI = MPI_WTIME()
...
!TUSED(21,:) = TUSED(21,:) + MPI_WTIME() - TMPI
...
IF (ICYC > 0) THEN
   IF (.NOT.ALL(EVACUATION_ONLY)) THEN
      ...
      DO NM=1,NMESHES
         IF (PROCESS(NM)==MYID) DTNEXT_SYNC(NM) = MESHES(NM)%DTNEXT
      ENDDO

      CALL MPI_ALLGATHERV(DTNEXT_SYNC(DISPLS(MYID)+1),COUNTS(MYID),MPI_DOUBLE_PRECISION,DTNEXT_SYNC,COUNTS,DISPLS, &
           MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
