...
\subsection{Eddy Dissipation Model (EDM) Reactions (\texorpdfstring{\bf ReactionRate\_EDM\_}{ReactionRate\_EDM})}
...
\end{equation}
where $A$ is the pre-exponential, $E$ is the activation energy, and $n_{\alpha}$ determines the temperature dependence of the reaction. $A$, $E$, and $n_{\alpha}$ are user defined inputs based on a specifically defined reaction. The species mass flux for each reaction is defined by:
...
\subsection{Mixed mode reactions (\texorpdfstring{\bf ReactionRate\_Mixed\_}{ReactionRate\_Mixed\_})}
