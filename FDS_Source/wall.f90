...
REAL(EB) :: DT_BC,T,TSI,TMP_G,RHO_G,DTMP,TMP_OTHER,CP_TERM,RHOWAL,RAMP_FACTOR,QNET,FDERIV,TMP_EXTERIOR,UN,ARO,UWO,&
            CP,CP_F,YY_N(1:N_SPECIES),QEXTRA
INTEGER  :: IOR,II,JJ,KK,IBC,IIG,JJG,KKG,IW,NOM,IIO,JJO,KKO,ITMP
...
HEAT_FLUX_LOOP: DO IW=1,NWC+NVWC
   ...
   IF (SF%THERMAL_BC_INDEX /= NO_CONVECTION .AND. SF%THERMAL_BC_INDEX /= INTERPOLATED_BC) THEN
      ...
      IF (N_SPECIES > 0) THEN
         YY_N = MAX(0._EB,0.5*(YYP(IIG,JJG,KKG,:)+YY_W(IW,:)))
...
REAL(EB) :: T,YY_WALL,DENOM,YY_OTHER(MAX_SPECIES),RHO_YY_OTHER(MAX_SPECIES),YY_EXTERIOR(MAX_SPECIES), &
            YY_G,YY_G_ALL(MAX_SPECIES),RHO_G,UN,DD,EPSB,MFT,TSI,ARO,UWO,RADIUS,AREA,RVC,RHO_NEW,M_DOT_PPP,CP,MW_RATIO,H_G,DELTA_H_G
INTEGER :: I,IBC,IIG,JJG,KKG,IOR,IW,II,JJ,KK,N,IIO,JJO,KKO,NOM,ITMP
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
TYPE (VENTS_TYPE), POINTER :: VT=>NULL()
TYPE (OMESH_TYPE), POINTER :: OM=>NULL()
TYPE (MESH_TYPE), POINTER :: MM=>NULL()
...
WALL_CELL_LOOP: DO IW=1,NWC+NVWC
   ...
   ! Special cases
   IF (N_SPECIES==0 .AND. .NOT. SF%SPECIES_BC_INDEX==SPECIFIED_MASS_FLUX) &
       CYCLE WALL_CELL_LOOP
   ...
   METHOD_OF_MASS_TRANSFER: SELECT CASE(SF%SPECIES_BC_INDEX)
      ...
      CASE (SPECIFIED_MASS_FLUX) METHOD_OF_MASS_TRANSFER
         ...
         SPECIES_LOOP: DO N=1,N_SPECIES
            DD    = RHODW(IW,N)*RDN(IW)
            YY_G  = YYP(IIG,JJG,KKG,N)
            DENOM = DD + (0.5_EB*UN+EPSB)*RHO_W(IW)
            YY_W(IW,N) = ( MASSFLUX(IW,N) + YY_G*(DD + (EPSB-0.5_EB*UN)*RHO_G) ) / DENOM
         ENDDO SPECIES_LOOP
...
REAL(EB) :: YY_GET(1:N_SPECIES),YY_G(1:N_SPECIES),UN,MFT,TMP_G,RHO_G,CP,CP_TERM, RHOWAL,YY_N(1:N_SPECIES),EPSB,DD,DENOM
...
WALL_CELL_LOOP: DO IW=1,NWC
   ...
   DO WHILE (ITER)
      ...
      IF (UN > 0._EB .AND. VENTS(VENT_INDEX(IW))%NODE_INDEX > 0) THEN
         ...
         RHOWAL    = 0.5_EB*(RHOP(IIG,JJG,KKG)+RHO_W(IW))
         IF (N_SPECIES > 0) THEN
            YY_N(:) = MAX(0._EB,0.5_EB*(YY_G(:) + YY_W(IW,:)))
            CALL GET_AVERAGE_SPECIFIC_HEAT(YY_N,CP,MIN(5000,NINT(TMP_F(IW))))
         ELSE
            CP   = Y2CPBAR_C(MIN(5000,NINT(TMP_F(IW))))
         ENDIF
         CP_TERM   = MAX(0._EB,-UW(IW))*RHOWAL*CP
         TMP_W(IW) = ( (RDN(IW)*KW(IW)-0.5_EB*CP_TERM)*TMP_G + CP_TERM*TMP_F(IW))/(0.5_EB*CP_TERM+RDN(IW)*KW(IW))
         TMP_W(IW) = MAX(TMPMIN,TMP_W(IW))        
      ELSE
         TMP_F(IW) = TMP(IIG,JJG,KKG)
         TMP_W(IW) = TMP_F(IW)
      ...
      ELSE
         ...
         ELSE
            ...
            DO N=1,N_SPECIES
               DD = RHODW(IW,N)*RDN(IW)
               DENOM = DD + (0.5_EB*UN+EPSB)*RHO_W(IW)
               YY_W(IW,N) = ( MASSFLUX(IW,N) + YY_G(N)*(DD + (EPSB-0.5_EB*UN)*RHO_G) ) / DENOM
               IF (YY_W(IW,N) > 1.E-10_EB) YY_ERR = MAX(YY_ERR,ABS(YY_W(IW,N)-YY_0(N))/YY_0(N))
            ENDDO
         ENDIF
         IF (COUNTER > 5) YY_W(IW,:) = 0.4_EB*YY_W(IW,:)+0.6_EB*YY_0(:)
         YY_GET = MAX(0._EB,YY_W(IW,:))
         CALL GET_SPECIFIC_GAS_CONSTANT(YY_GET,RSUM_W(IW))
         IF (YY_ERR > 1.E-6_EB) ITER = .TRUE.
      ENDIF
      RHO_W(IW) = PBAR_P(KK,PRESSURE_ZONE_WALL(IW))/(RSUM_W(IW)*TMP_W(IW))
      IF (ABS(RHO_0 - RHO_W(IW))/RHO_0 > 1.E-6_EB .OR. &
          ABS(TMP_0 - TMP_W(IW))/TMP_0 > 1.E-6_EB) ITER = .TRUE.
...
! Actually set the ghost cell value of density in the ghost cell if it is a solid wall
   ...
   IF ( (SOLID(CELL_INDEX(II,JJ,KK)) .AND. .NOT.SOLID(CELL_INDEX(IIG,JJG,KKG))) .OR.  &
      ...
      RHOP(II,JJ,KK) = RHO_W(IW)
      TMP(II,JJ,KK) = TMP_W(IW)
       IF (N_SPECIES>0) YYP(II,JJ,KK,:) = YY_W(IW,:)
      IF (N_SPECIES>0) RSUM(II,JJ,KK) = RSUM_W(IW)
   ENDIF
...
WALL_CELL_LOOP: DO IW=1,NWC+NVWC
   ...
   ITMP = MIN(5000,NINT(0.5_EB*(TMP(IIG,JJG,KKG)+TMP_W(IW))))
   IF (N_SPECIES > 0) THEN
      YY_G_ALL(:) = MAX(0._EB,0.5_EB*(YY(IIG,JJG,KKG,:)+YY_W(IW,:)))
...
REAL(EB) :: TMP_G,DELTA_TMP,U2=0._EB,V2=0._EB,W2=0._EB,VELCON,H_NATURAL,H_FORCED,H_DNS,H_FIXED,CP,YY_GET(1:N_SPECIES),MU_G
...
! Calculate the HTC for a DNS calculation. This may be used even if DNS=.FALSE.

H_DNS = 2._EB*KW(IW)*RDN(IW)

! If this is only a DNS calculation, set HTC and return

IF (DNS) THEN
   HEAT_TRANSFER_COEFFICIENT = H_DNS
   RETURN
ENDIF
...
NO_GAS_CELL_IF: IF (IIG<0) THEN  
   ...
   H_DNS = 0._EB  ! Just use natural convection if there is no gas phase cell
...
ELSE NO_GAS_CELL_IF
   ...
   SELECT CASE (IHTMODEL)
      ...
      CASE (ICHILTON)
         ...
         IF (VELCON > 0._EB) H_FORCED = U_TAU(IW)**2/VELCON*0.5_EB*(RHOP(IIG,JJG,KKG)+RHO_W(IW))*CP*RPR**TWTH           
      CASE DEFAULT
...
REAL(EB) :: U_THERM,U_TURB,TGAS,TWALL,MUGAS,Y_SOOT,RHOG,TNOW,YIN(1:N_SPECIES),YDEP,K_AIR
...
WALL_CELL_LOOP: DO IW=1,NWC+NVWC
   ...
   U_THERM = 0.55_EB*HEAT_TRANS_COEF(IW)*(TGAS-TWALL)*MUGAS/(TGAS*RHOG*K_AIR)
   U_TURB = 0.037_EB*U_TAU(IW)
