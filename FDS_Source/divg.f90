...
INTEGER :: IW,N,IOR,II,JJ,KK,IIG,JJG,KKG,ITMP,IBC,I,J,K,IPZ,IOPZ,IOPZ2
...
DO IW=1,NWC
   IF (BOUNDARY_TYPE(IW)/=NULL_BOUNDARY .AND. BOUNDARY_TYPE(IW)/=OPEN_BOUNDARY) CYCLE
   II  = IJKW(1,IW)
   JJ  = IJKW(2,IW)
   KK  = IJKW(3,IW)
   IIG = IJKW(6,IW)
   ...
   IF (PRESSURE_ZONE(II,JJ,KK)/=PRESSURE_ZONE(IIG,JJG,KKG)) THEN
      CONNECTED_ZONES(PRESSURE_ZONE(II,JJ,KK),PRESSURE_ZONE(IIG,JJG,KKG)) = .TRUE.
      CONNECTED_ZONES(PRESSURE_ZONE(IIG,JJG,KKG),PRESSURE_ZONE(II,JJ,KK)) = .TRUE.
   ENDIF
   IF (BOUNDARY_TYPE(IW)==OPEN_BOUNDARY) THEN
      CONNECTED_ZONES(0,PRESSURE_ZONE(IIG,JJG,KKG)) = .TRUE.
      CONNECTED_ZONES(PRESSURE_ZONE(IIG,JJG,KKG),0) = .TRUE.
   ENDIF
ENDDO
...
DO IPZ=1,N_ZONE
   DO IOPZ=1,N_ZONE
      ...
      IF (CONNECTED_ZONES(IPZ,IOPZ)) THEN
         DO IOPZ2=0,N_ZONE
            IF (IOPZ==IOPZ2) CYCLE
            IF (CONNECTED_ZONES(IOPZ,IOPZ2)) CONNECTED_ZONES(IPZ,IOPZ2) = .TRUE.
            IF (CONNECTED_ZONES(IOPZ,IOPZ2)) CONNECTED_ZONES(IOPZ2,IPZ) = .TRUE.
         ENDDO
      ENDIF
   ENDDO
ENDDO

IF (.NOT.ALLOW_ZONE_BREAKS) CONNECTED_ZONES = .FALSE.
...
SPECIES_LOOP: DO N=1,N_SPECIES
   ...
   SPECIES_DIFFUSION: IF (.NOT.MIXTURE_FRACTION) THEN
      ...
      WALL_LOOP2: DO IW=1,NWC
         ...
         KKG = IJKW(8,IW)
         IOR  = IJKW(4,IW)
         ITMP = MIN(500,NINT(0.1_EB*TMP_F(IW)))
         HDIFF = Y2H_G(ITMP,N)-Y2H_G_C(ITMP)
         RHO_D_DYDN = RHODW(IW,N)*(YYP(IIG,JJG,KKG,N)-YY_W(IW,N))*RDN(IW)
         SELECT CASE(IOR)
            CASE( 1) 
...
PRESSURE_ZONE_LOOP: DO IPZ=1,N_ZONE
   ...
   OTHER_ZONE_LOOP: DO IOPZ=0,N_ZONE
      ...
      IF (FDS_LEAK_AREA(IPZ,IOPZ) > 0._EB) THEN
         VDOT_LEAK = SIGN(1._EB,DELTA_P)*MIN(LEAK_AREA(IPZ,IOPZ)*SQRT(2._EB*ABS(DELTA_P)/RHOA), &
...
REAL(EB) :: RDT,TNOW,P_EQUILIBRIUM_NUM,P_EQUILIBRIUM_DEN
...
DO IPZ=1,N_ZONE
   ...
   DO IOPZ=N_ZONE,0,-1
      ...
      ENDIF
   ENDDO
   USUM(IPZ,NM) = USUM(IPZ,NM) + PRESSURE_RELAX_FACTOR*RDT*PSUM(IPZ,NM)*(PBAR(1,IPZ)-P_EQUILIBRIUM_NUM/P_EQUILIBRIUM_DEN)
ENDDO
