...
PARTICLES: IF (N_LP_ARRAY_INDICES>0) THEN
   GET_PC_RADI: DO J=1,N_LAGRANGIAN_CLASSES
      IF (.NOT.LAGRANGIAN_PARTICLE_CLASS(J)%MASSLESS .AND. LAGRANGIAN_PARTICLE_CLASS(J)%SURF_INDEX<=0 ) CALL MEAN_CROSS_SECTIONS(J)
   ENDDO GET_PC_RADI
ENDIF PARTICLES
...
REAL(EB), PARAMETER :: Q_MINIMUM=1._EB
...
IF (UPDATE_INTENSITY) THEN
   WALL%ONE_D%QRADIN = 0._EB
   IF(NLP>0) LAGRANGIAN_PARTICLE%ONE_D%QRADIN=0._EB
ENDIF
...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
            ...
            ZLOOPM: DO I=1,IBAR
               IF (SOLID(CELL_INDEX(I,J,K))) CYCLE ZLOOPM
                  PC_LOOP: DO N = 1,N_LAGRANGIAN_CLASSES
                     ...
                     IF (LPC%TREE .OR. LPC%SURF_INDEX>0) CYCLE PC_LOOP
                     ARRAY_INDEX = LPC%ARRAY_INDEX
                     IF (ARRAY_INDEX==0) CYCLE PC_LOOP
                     IF (ABS(AVG_DROP_AREA(I,J,K,ARRAY_INDEX))<ZERO_P) CYCLE PC_LOOP
   ...
   IF_VEG_INCLUDED: IF (NLP>0 .AND. TREE_MESH(NM)) THEN
      VEG_ELEM_LOOP: DO ID = 1,NLP
         ILPC = LAGRANGIAN_PARTICLE(ID)%CLASS_INDEX
         IF (.NOT. LAGRANGIAN_PARTICLE_CLASS(ILPC)%TREE) CYCLE VEG_ELEM_LOOP
         CALL GET_IJK(LP%X,LP%Y,LP%Z,NM,XID,YJD,ZKD,IID,JJD,KKD)
         KAPPAW(IID,JJD,KKD) = KAPPAW(IID,JJD,KKD) + LAGRANGIAN_PARTICLE(ID)%VEG_KAPPA
         KFST4W(IID,JJD,KKD) = KFST4W(IID,JJD,KKD) + LAGRANGIAN_PARTICLE(ID)%VEG_EMISS
      ENDDO VEG_ELEM_LOOP
      QR_W = 0._EB
      ...
      DO ID = 1,NLP
         ...
         SELECT CASE(SF%GEOMETRY)
            ...
            CASE(SURF_CYLINDRICAL)
               SURFACE_AREA = SF%LENGTH*TWOPI*MAXVAL(LP%ONE_D%X)
            CASE(SURF_SPHERICAL)
               SURFACE_AREA = 4._EB*PI*MAXVAL(LP%ONE_D%X)**2
         END SELECT
         KAPPA_PART = 0.25_EB*LP%PWT*SURFACE_AREA*RDX(IID)*RRN(IID)*RDY(JJD)*RDZ(KKD)
         KAPPAW(IID,JJD,KKD) = KAPPAW(IID,JJD,KKD) + KAPPA_PART
         KFST4W(IID,JJD,KKD) = KFST4W(IID,JJD,KKD) + FOUR_SIGMA*KAPPA_PART*LP%TMP_F**4
      ENDDO
      QR_W = 0._EB
   ENDIF
   ...
   IF (KFST4_SUM>0._EB) THEN
      RTE_SOURCE_CORRECTION_FACTOR = RAD_Q_SUM/KFST4_SUM
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               IF (SOLID(CELL_INDEX(I,J,K))) CYCLE
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS+N_VIRTUAL_WALL_CELLS
         ...
         ELSE
            ...
            IF (.NOT. SF%INTERNAL_RADIATION) WALL(IW)%ONE_D%QRADOUT = WALL(IW)%ONE_D%EMISSIVITY*SIGMA*WALL(IW)%TMP_F**4
         ENDIF
      ...
      DO IW = 1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         INRAD_W(IW) = SUM(-DLN(WALL(IW)%IOR,:)* WALL(IW)%ONE_D%ILW(:,IBND),1, DLN(WALL(IW)%IOR,:)<0._EB)
      ENDDO
         ...
         ANGLE_LOOP: DO N = NSTART,NEND,NSTEP  ! Sweep through control angles
            ...
            IF (VIRTUAL_PARTICLES) THEN
               PARTICLE_RADIATION_LOOP: DO I_DROP=1,NLP
                  ...
                  IF (LPC%N_SPLIT < 2) CYCLE PARTICLE_RADIATION_LOOP
                  SURF_INDEX = LPC%SURF_INDEX
                  IF (SURF_INDEX<1) CYCLE PARTICLE_RADIATION_LOOP
                  COSINE = LPC%ORIENTATION(LP%SPLIT_IOR,1)*DLX(N) + &
                           LPC%ORIENTATION(LP%SPLIT_IOR,2)*DLY(N) + &
                           LPC%ORIENTATION(LP%SPLIT_IOR,3)*DLZ(N)
                  IF (COSINE <0._EB) LP%ONE_D%ILW(N,IBND) = -COSINE * IL_UP(LP%IIG, LP%JJG, LP%KKG)
...
IF (UPDATE_INTENSITY) THEN
   ...
   DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      WALL(IW)%ONE_D%QRADIN  = SUM(WALL(IW)%ONE_D%ILW)
   ENDDO 
...
IF (.NOT. WIDE_BAND_MODEL) THEN
   ...
   IF_VEG_INCLUDED2: IF (NLP>0 .AND. TREE_MESH(NM)) THEN
      VEG_ELEM_LOOP2: DO ID = 1,NLP
         IF (.NOT. LAGRANGIAN_PARTICLE_CLASS(LAGRANGIAN_PARTICLE(ID)%CLASS_INDEX)%TREE) CYCLE VEG_ELEM_LOOP2
         LAGRANGIAN_PARTICLE(ID)%VEG_DIVQR = 0.0_EB
         CALL GET_IJK(LAGRANGIAN_PARTICLE(ID)%X,LAGRANGIAN_PARTICLE(ID)%Y,LAGRANGIAN_PARTICLE(ID)%Z,NM,XID,YJD,ZKD,IID,JJD,KKD)
         LAGRANGIAN_PARTICLE(ID)%VEG_DIVQR = LAGRANGIAN_PARTICLE(ID)%VEG_KAPPA*UII(IID,JJD,KKD) - &
                                             LAGRANGIAN_PARTICLE(ID)%VEG_EMISS
...
IF (GAS_CELL_RAD_FLUX) THEN
   DEVC_LOOP2: DO NN = 1, N_GAS_CELL_RAD_DEVC
      ...
      IF (IW>0) THEN
         WALL(IW)%ONE_D%QRADIN = WALL(IW)%ONE_D%EMISSIVITY*WEIGH_CYL*SUM(DV%ILW)
      ENDIF
   ENDDO DEVC_LOOP2
ENDIF
   ...
   PARTICLE_LOOP: DO I=1,NLP
      ...
      IF (LPC%SURF_INDEX>0) THEN
         ...
         IF(LPC%N_SPLIT>1) THEN
            LP%ONE_D%QRADIN = LP%ONE_D%EMISSIVITY*(WEIGH_CYL*SUM(LP%ONE_D%ILW) + EFLUX)
         ELSE
            LP%ONE_D%QRADIN = LP%ONE_D%EMISSIVITY*(0.25_EB*UII(LP%IIG,LP%JJG,LP%KKG) + EFLUX)
         ENDIF   
      ENDIF
   ENDDO PARTICLE_LOOP
ENDIF
