...
INTEGER :: ERROR_COUNT
REAL(EB), DIMENSION(0:10,0:10,0:10) :: VOL2
...
V1(1) = 0.0_EB + REAL(J1,EB)*0.2_EB/REAL(N-1,EB)
V1(2) = 0.0_EB + REAL(K1,EB)*0.2_EB/REAL(N-1,EB)
DO I2 = 0, N-1
DO J2 = 0, N-1
DO K2 = 0, N-1
V2(0) = 0.0_EB + REAL(I2,EB)*1.0_EB/REAL(N-1,EB)
V2(1) = 0.8_EB + REAL(J2,EB)*0.2_EB/REAL(N-1,EB)
V2(2) = 0.0_EB + REAL(K2,EB)*0.2_EB/REAL(N-1,EB)
...
DO J = 0, NBOX-1
DO K = 0, NBOX-1
BOXBOUNDS(0)=XMIN +   REAL(I,EB)*(XMAX-XMIN)/REAL(NBOX,EB)
BOXBOUNDS(1)=XMIN + REAL(I+1,EB)*(XMAX-XMIN)/REAL(NBOX,EB)
BOXBOUNDS(2)=YMIN +   REAL(J,EB)*(YMAX-YMIN)/REAL(NBOX,EB)
BOXBOUNDS(3)=YMIN + REAL(J+1,EB)*(YMAX-YMIN)/REAL(NBOX,EB)
BOXBOUNDS(4)=ZMIN +   REAL(K,EB)*(ZMAX-ZMIN)/REAL(NBOX,EB)
BOXBOUNDS(5)=ZMIN + REAL(K+1,EB)*(ZMAX-ZMIN)/REAL(NBOX,EB)
VOL2(I,J,K) = GET_TETRABOX_VOLUME(BOXBOUNDS,V0,V1,V2,V3)
VOL = VOL + VOL2(I,J,K)
END DO
END DO
END DO
...
IF(ABS(VOL_TETRA-VOL).GT.1.0E-5_EB*ABS(VOL_TETRA)+1.0E-5_EB)THEN
   ...
   DO K = 0, NBOX-1
   DO J = 0, NBOX-1 
   WRITE(6,*)"VV",(VOL2(I,J,K),I=0,NBOX-1)
   END DO
   END DO
