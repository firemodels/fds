...
\begin{equationarray}{rcll}
\label{eqn_wwlam}  u^+ &=& z^+       & \mbox{for} \quad z^+ \le 11.81 \\
...
\begin{equationarray}{rcll}
\label{eqn_tauwlam} |\tau_w| &=& \frac{2 \bar{\mu} |\tilde{u}|}{\Delta z} & \mbox{for} \quad z^+ \le 11.81 \vspace{0.2cm}\\
\label{eqn_tauwturb}|\tau_w| &=& \bar{\rho}\left[\alpha\left(\frac{\bar{\mu}}{\bar{\rho}\Delta z}\right)^\beta + \eta\left(\frac{\bar{\mu}}{\bar{\rho}\Delta z}\right)^B |\tilde{u}|\right]^\gamma                           & \mbox{for} \quad z^+ > 11.81 \,\mbox{,}
\end{equationarray}
where
...
Let $q \equiv |\mathbf{u}|$. By definition, ${\cal H} = \frac{1}{2}q^2 + \frac{\tilde{p}}{\rho_\infty}$.
...
\begin{equation}
\tilde{p}_{ext} + \mbox{$\frac{1}{2}$}\rho_1 q_1^2 = \rho_\infty({\cal H}_2 - \mbox{$\frac{1}{2}$} q_2^2) +
\mbox{$\frac{1}{2}$} \rho_2 q_2^2
\end{equation}
which rearranges to
\begin{equation}
{\cal H}_2 = \underbrace{\frac{\tilde{p}_{ext}}{\rho_\infty}}_{{\cal H}_{ext}} +
\underbrace{\frac{1}{2}q_1^2 \frac{\rho_1}{\rho_\infty}}_{{\cal H}_0} +   \frac{1}{2} q_2^2 \left(1-\frac{\rho_2}{\rho_\infty}\right)
\end{equation}
The density $\rho_2$ (denoted $\rho_w$ in Eq. (\ref{eqn_pressbc})) is taken as the average density
between the gas-phase and ghost cells adjacent to the boundary.

At open boundaries (say $i=I$), $\cal H$ is prescribed depending on whether the flow is incoming or outgoing.
Define $q^2 \equiv u_{I,jk}^2 + \bar{v}_{I,jk}^2 + \bar{w}_{I,jk}^2$ where $I$ is the index of the last gas phase cell in the $x$
direction and $u_{I,jk}$ is the $x$ component of velocity at the boundary.
The density at the boundary is denoted $\rho_w$ (for ``wall'').
The background density used in defining ${\cal H}$ is $\rho_\infty$.
The overbars on $v$ and $w$ indicate an interpolation of
these components to the boundary location.  The boundary condition on ${\cal H}$ is
\be
\label{eqn_pressbc}
\begin{array}{lll}
     {\cal H}_{I+\ha,jk} = {\cal H}_{ext} + \frac{1}{2}q^2  & \mbox{for} & u_{I,jk} > 0    \\
     {\cal H}_{I+\ha,jk} = {\cal H}_{ext} + {\cal H}_0 + \frac{1}{2}q^2(1-\frac{\rho_w}{\rho_\infty})  & \mbox{for} & u_{I,jk} \le 0
     \end{array}
\ee
The external dynamic pressure, ${\cal H}_{ext} = \tilde{p}_{ext}/\rho_\infty$, as well as the initial
dynamic pressure, ${\cal H}_0 = \frac{1}{2}(u_0^2 + v_0^2 + w_0^2)\rho_0/\rho_\infty$,
are specified by the user ($\tilde{p}_{ext}$ is set with {\ct DYNAMIC\_PRESSURE} for
a specific {\ct VENT}). The condition (\ref{eqn_pressbc}) follows from prescribing the
fluctuating hydrodynamic pressure $\tilde{p}_{ext}$ at an outflow vent (zero by default)
and assuming the Bernoulli equation applies for an inflow vent where the fluid accelerates from the
state \{$\tilde{p}_{ext},\rho_0,u_0,v_0,w_0$\} along a streamline.
Note that the background density $\rho_\infty$ is usually equal to the initial ambient density $\rho_0$ at
the start of a calculation, but may change in time if
the baroclinic correction term is included.
...
\be \dq''' = \frac{ \min \left( \rho Y_\F , s \rho Y_\OTWO \right) }{\tau} \, \Delta H  \quad ; \quad s=\frac{W_\F}{\nu_{\OTWO} W_{\OTWO} }   \ee
...
\be \dq_{\max,1}''' = \frac{ \dq_{\max}''}{(\dx \, \dy \, \dz)^\ot }  \quad ; \quad  \dq_{\max}'' = 200 \; \; \hbox{kW/m}^2  \ee
...
\be \dq_{\max,2}''' = \frac{ \overline{\dq_{\max}'''} - (1-\dt/\tau') \; \overline{\dq'''} }{\dt/\tau'} \quad ; \quad  \overline{\dq_{\max}'''}=3000 \; \; \hbox{kW/m}^3   \ee
...
\newpage
\section{Finite-Rate Combustion Model}
...
\section{Radiation Heat Transfer to Solid Objects}
...
($T_{thr,\alpha\beta}=0$ K). Appendix~\ref{solid_kinetics_appendix} describes a method
...
\section{Droplet Transport on a Surface}
...
\chapter{Solid Phase Kinetics}
\label{solid_kinetics_appendix}

This section describes a method of determining the values of the kinetic parameters for the thermal decomposition of a solid,
following the methodology described by Lyon~\cite{Lyon:FM2000}.

Consider a multi-component solid. The mass fraction of any component, $Y(t)$, changes in time according to the equation:
\be
...
rate curve corresponds to the number of reactions. For TGA data, the values of $r_p$ can be obtained directly from inspection of
the reaction
rate curve, assuming the units are s$^{-1}$. For micro-combustion calorimetry, assuming that the results are presented in units of
W/g, the values of $r_p$ can be found from:
\be
   r_p = \frac{q_p}{\Delta H} \label{r_formula}
\ee
where $q_p$ are the peaks of the heat release rate curve and $\Delta H$ is the heat of combustion, which is obtained by integrating
the heat release rate curve over the temperature and then multiplying by the heating rate.
It is important to check the units of all of these quantities because the
results of these experiments are often presented in different ways depending on the particular application.

As an example of how to apply this procedure, consider the two plots shown in Fig.~\ref{mcc_plots}. The dashed curves in the figures
...
cable.
\begin{figure}[ht]
\includegraphics[width=3in]{FIGURES/I11_2}
\includegraphics[width=3in]{FIGURES/J11_2}
\caption{Results of a micro-calorimetry analysis of a sample of cable insulation (left) and jacket material (right).}
\label{mcc_plots}
\end{figure}
The insulation material exhibits two fairly well-defined peaks, whereas the jacket material exhibits three. For each reaction,
the kinetic parameters were calculated using the formulae (\ref{E_formula}) and (\ref{A_formula}). The choices of
$T_p$, $r_p$, and $Y_0$ for each reaction are shown on the plots\footnote{Note that $r_p$ has been calculated using
Eq.~(\ref{r_formula}), where the heat of combustion, $\Delta H$, is the area beneath the curve multiplied by the
heating rate, $\beta$, which in this case is 1~K/s. Note also that the ``peak'' temperatures, $T_p$, are expressed
in units of Kelvin in the formulae. They are presented here in degrees Celsius because this is the unit typically used for
FDS input data and the unit of which MCC data is typically presented.}. These were all chosen by inspection from the original
micro-calorimetry data. Nothing has been assumed about the materials themselves. It has to be assumed that each reaction
liberates fuel gases that burn at the computed heat of combustion. It must also be assumed that each reaction yields the
same fraction of residue.

The black curves in the plots of Fig.~\ref{mcc_plots} are the results of numerically integrating Eq.~(\ref{dYdt}) for each
material component. It is assumed that the insulation and jacket materials are composed of multiple components, with each
having a single reaction that produces fuel gas and residue. In this example, the residue yield for the insulation material is
6~\%; for the jacket 49~\%, obtained simply by weighing the sample before and after the micro-calorimetry measurement.
Although the residue yields do not play a role in the kinetic constants, they are necessary
inputs to FDS. The residue is also important when interpreting the original data. Sometimes the heat release rate is
expressed in units of W/g of the {\em total} sample, and sometimes in units of W/g of pyrolyzed gases. The latter
is preferable because the heat of combustion obtained by integrating under the plot of the HRR is interpreted in the
conventional manner. In fact, the HRR data shown here was originally reported in units of W/g {\em total}. It was then
divided by the factor $(1-\nu_r)$ to modify its interpretation.
