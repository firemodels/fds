...
This appendix summarizes the formulation, discretization, boundary conditions,
and numerical solution of the Poisson equation for the pressure variable $H$
used in the FDS projection method. The emphasis is on consistency between the
discrete Poisson operator, velocity correction, and mesh coupling, particularly
when using global matrix solvers with mesh refinement and immersed boundaries.

%=============================================================================
\section{Pressure Projection and Definition of \texorpdfstring{$H$}{H}}
%=============================================================================

FDS advances the velocity field using a fractional--step (projection) method.
Rather than solving directly for pressure, the scalar variable $H$ is defined as
\begin{equation}
...
At each time step (and, when required, within a fixed--point iteration), $H$ is
...
\label{eq:poisson_H}
\end{equation}
where $\mathbf{F}_A$ represents known body forces and $\mathbf{F}_B$ includes
density--dependent and immersed boundary forcing terms.
...
\end{equation}
This formulation ensures that the discrete pressure gradient used in the velocity
update is consistent with the Poisson operator.
...
FDS employs a staggered Cartesian grid in which scalar quantities such as $H$ and
$\rho$ are stored at cell centers, while velocity components are stored at cell
faces. This arrangement avoids pressure–velocity decoupling and allows consistent
discretization of divergence and gradient operators.

\subsection*{Discrete Laplacian}

For a two--dimensional uniform grid with $\delta x = \delta y = h$, the discrete
Laplacian at cell $(i,j)$ is
\begin{equation}
\nabla^2 H_{i,j}
\approx
\frac{1}{h^2}
...
In three dimensions, the operator extends to a seven--point stencil. Discretization
over $N = N_x N_y (N_z)$ cells yields a sparse linear system
\begin{equation}
\mathbf{A}\,\mathbf{H} = \mathbf{R},
\label{eq:linear_system}
\end{equation}
where $\mathbf{A}$ is symmetric and contains at most 5 (2D) or 7 (3D) nonzero
entries per row.
...
The Laplacian of $H$ is constructed by applying the divergence operator to the
cell–centered gradient. On a uniform staggered grid with spacings
$\delta x$ and $\delta y$, the gradient components are first evaluated at cell
faces and then differenced back to the cell center.
...
{$\dfrac{H_{i+1,j}-H_{i,j}}{\delta x}$};

\node[anchor=east] (gxL) at (0,2.5)
{$\dfrac{H_{i,j}-H_{i-1,j}}{\delta x}$};

\node[anchor=south] (gyT) at (2.9,2.3)
{$\dfrac{H_{i,j+1}-H_{i,j}}{\delta y}$};

\node[anchor=north] (gyB) at (-0.9,-0.3)
{$\dfrac{H_{i,j}-H_{i,j-1}}{\delta y}$};
...
\end{tikzpicture}
\caption{Construction of the discrete Laplacian of $H$ on a two–dimensional
staggered grid. Pressure gradients are evaluated at cell faces and differenced
back to the cell center to form the five–point stencil.}
\label{fig:laplacian_staggered}
\end{figure}



Applying the divergence of the gradient yields
\begin{equation}
\nabla^2 H_{i,j}
\approx
\frac{1}{\delta x}
...
-
\frac{H_{i,j}-H_{i,j-1}}{\delta y}
\right),
\label{eq:laplacian_staggered}
\end{equation}
which reduces to the standard five–point stencil when
$\delta x = \delta y = h$.
...
Prescribed values of $H$ are enforced by eliminating ghost cell values through
linear extrapolation, resulting in a modification of the diagonal entry of
$\mathbf{A}$ and a corresponding adjustment to the right--hand side.

\subsection*{Neumann Boundary Conditions}

Normal gradient conditions are imposed by constraining the ghost cell such that
\begin{equation}
\frac{\partial H}{\partial n} = 0.
\end{equation}
This results in a change to the diagonal coefficient of the corresponding matrix
row. Homogeneous Neumann conditions lead to a singular operator and require an
additional constraint to fix the pressure reference.

%=============================================================================
\section{Immersed Boundary Treatment}
%=============================================================================

In the presence of immersed solid boundaries, FDS solves the Poisson equation for
$H$ throughout the gas phase while enforcing velocity boundary conditions through
additional forcing terms.

At forcing points adjacent to an immersed boundary, the normal velocity is
constrained to a prescribed value $u^d$. The combined forcing term is written as
\begin{equation}
(\mathbf{F}_A + \mathbf{F}_B)
=
-
\frac{u^d - u^n}{\Delta t}
-
\frac{\Delta H}{\Delta x},
\end{equation}
with the pressure gradient term treated implicitly through the Poisson solve.
This formulation allows velocity boundary conditions to be satisfied while
retaining a standard Laplacian operator for $H$.
...
On single, uniform, structured meshes with separable boundary conditions, the
Poisson equation is solved using a fast trigonometric method based on discrete
sine and cosine transforms. This approach exploits the separability of the
Laplacian operator, yielding an $\mathcal{O}(N \log N)$ solution with minimal
memory overhead.
...
For general geometries involving mesh refinement, immersed boundaries, or
multiple meshes, FDS assembles the global linear system
\eqref{eq:linear_system} and solves it using the UGLMAT framework, which interfaces
with the HYPRE library.

The resulting matrix is sparse and symmetric, incorporates boundary conditions
through local row modifications, and includes explicit coarse--fine coupling
terms at refinement interfaces. Iterative solvers with multigrid
preconditioning are used to obtain $H$, after which velocities are corrected
locally on each mesh.
...
The guard cell interpolation and flux--matching procedure described below ensures
that the discrete gradient $\nabla H$ used in the velocity update is consistent
with the fluxes represented in the global Poisson matrix. This consistency is
essential for maintaining velocity continuity across refinement interfaces when
using the UGLMAT solver.
