...
IF (ANY(REACTION(:)%FAST_CHEMISTRY)) THEN
   SELECT CASE (EXTINCT_MOD)
      ...
      CASE(EXTINCTION_2)
         FUNC_EXTINCT = EXTINCT_2(ZZ_MIXED_IN,TMP_MIXED)
      CASE(EXTINCTION_3)
         FUNC_EXTINCT = EXTINCT_3(ZZ_MIXED_IN,TMP_MIXED)
      CASE(EXTINCTION_4)
         FUNC_EXTINCT = EXTINCT_4(ZZ_MIXED_IN,TMP_MIXED,TAU_RES)
   END SELECT
ENDIF

IF (FUNC_EXTINCT) THEN
   DO I = 1,N_REACTIONS
      RN=>REACTION(I)
      SELECT CASE(RN%SUPPRESSION_INDEX)
         CASE(1); EXTINCT(I)=.FALSE.
         CASE(2); EXTINCT(I)=.TRUE.
         CASE(3); EXTINCT(I)=.FALSE.
      END SELECT
   ENDDO
ELSE
   DO I=1,N_REACTIONS
      ...
      SELECT CASE(RN%SUPPRESSION_INDEX)
         CASE(1); EXTINCT(I)=.FALSE.
         CASE(2); EXTINCT(I)=.FALSE.
         CASE(3); EXTINCT(I)=.TRUE.
      END SELECT
   ENDDO
ENDIF
...
REACTION_LOOP: DO NR=1,N_REACTIONS
   ...
   ELSE AIT_IF
      ...
      ! See if enough energy is released to raise the fuel and required "air" temperatures above the critical flame temp.
      IF ( ZZ_HAT_F*(H_F_0 + RN%HEAT_OF_COMBUSTION) + ZZ_HAT_A*H_A_0 + ZZ_HAT_P*H_P_0 < &
         ZZ_HAT_F*H_F_N  + ZZ_HAT_A*H_A_N + ZZ_HAT_P*H_P_N ) EXTINCT_3 = .TRUE. ! FDS Tech Guide (5.19)
   ENDIF AIT_IF
