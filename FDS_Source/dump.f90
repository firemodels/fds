...
PART_DIST_LOOP: DO I=1,N_LAGRANGIAN_CLASSES
   ...
   CLOSE(LU)
   ! Changmin output
   LU_CHANGMIN = GET_FILE_NUMBER()
   WRITE(FN_CHANGMIN,'(A,A,A,A)') TRIM(CHID),'_',TRIM(LPC%ID),'_cdf_actual.csv'
   OPEN (LU_CHANGMIN,FILE=FN_CHANGMIN,FORM='FORMATTED',STATUS='REPLACE',POSITION='APPEND')
   WRITE(LU_CHANGMIN,'(A)') 'd (mu-m),pwt'
