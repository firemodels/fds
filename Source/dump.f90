...
SMOKE3D_INITIALIZATION: IF (SMOKE3D .AND. .NOT.EVACUATION_ONLY(NM)) THEN
   ...
   DO N=1,4
      ...
      ELSE
         IF (N==1 .OR. N==2) SMLAB = 'SMOKF3D'
         IF (N==3 .OR. N==4) SMLAB = 'SMOKG3D'
         ...
         IF (USE_HRRPUV_MAX_SMV==1) THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,A,I6,1X,E13.6)') 'V',TRIM(SMLAB),NM,HRRPUV_MAX_SMV
         ELSE
...
DO N=1,N_DEVC
   ...
   WRITE(LU_CORE(NM)) DV%RMS_VALUE,DV%RMS_VALUE2,DV%COV_VALUE,DV%AVERAGE_VALUE,DV%AVERAGE_VALUE2,&
                      ...
                      DV%TIME_INTERVAL,DV%CURRENT_STATE,DV%PRIOR_STATE,DV%LP_TAG,DV%PART_CLASS_INDEX,DV%SMOOTHED_VALUE
...
DATA_FILE_LOOP: DO N=1,4
   ...
   CALL SMOKE3D_TO_FILE(LU_SMOKE3D(N,NM),LU_SMOKE3D(N+4,NM),STIME,DXX,MASS_EXT_COEF,N,QQ_PACK,IBP1,JBP1,KBP1,HRRPUV_MAX_SMV)
   DEALLOCATE(QQ_PACK)
         ...
         SOLID_STATS_SELECT: SELECT CASE(DV%STATISTICS)
            ...
            CASE('null','RMS','TIME MIN','TIME MAX') SOLID_STATS_SELECT
               ...
               ELSEIF (DV%LP_TAG>0) THEN
                  VALUE = SOLID_PHASE_OUTPUT(NM,ABS(DV%OUTPUT_INDEX),DV%Y_INDEX,DV%Z_INDEX,DV%PART_CLASS_INDEX,OPT_LP_INDEX=LP_INDEX)
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,MU_G,DUMMY,&
            ...
            UVW,UODX,VODY,WODZ,XHAT,ZHAT,BBF,RHO2,TMPUP,TMPLOW,ZINT,RHO_S,GAMMA_LOC,VC
   ...
   ELSE
      ...
      DO NN=1,SF%N_MATL
         IF (PF%QUANTITY==SF%MATL_NAME(NN) .OR. PF%QUANTITY=='DENSITY') THEN
            DO I=1,NWP
               RHO_S(I) = RHO_S(I) + ONE_D%RHO(I,NN)
            ENDDO
         ENDIF
      ENDDO
      RHO_S(0)     = RHO_S(1)
