...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            MAXIMUM_SCALING_HEAT_FLUX,REFERENCE_THICKNESS,&
...
READ_SURF_LOOP: DO N=0,N_SURF
      ...
      IF (RAMP_Q(NN)/='null' .AND. REFERENCE_HEAT_FLUX(NN) > 0._EB) THEN
         IF (NN>1) THEN
            IF (REFERENCE_HEAT_FLUX(NN) < REFERENCE_HEAT_FLUX(NN-1)) THEN
               WRITE(MESSAGE,'(A,A,A)') 'ERROR(X02): SURF ',TRIM(SF%ID),' REFERENCE_HEAT_FLUX array must monotonically increase.'
               CALL SHUTDOWN(MESSAGE) ; RETURN
            ENDIF
         ENDIF
   ...
   ALLOCATE(SF%REFERENCE_THICKNESS(SF%N_QDOTPP_REF))
   ...
   IF (REFERENCE_THICKNESS > 0._EB) SF%SPYRO_TH_FACTOR = REFERENCE_THICKNESS/THICKNESS(1)
   ...
   IF (SF%REFERENCE_HEAT_FLUX(1) > 0._EB) THEN
...
DO N=0,N_SURF
   ...
   DO NR=-N_SURF_RAMPS,N_TRACKED_SPECIES
      ...
      IF (SF%RAMP(NR)%ID/='null') THEN
         ...
         IF (SF%REFERENCE_HEAT_FLUX(1) > 0._EB .AND. NR <= TIME_HEAT .AND. .NOT. DUPLICATE(SF%RAMP(NR)%INDEX)) THEN 
