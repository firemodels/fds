...
ELSE
   ...
   IF (T>=BNDF_CLOCK(NM)) THEN
      ...
      ENDDO
   ENDIF
   IF (T>=PL3D_CLOCK(NM) .OR. GLOBAL_STOP_STATUS/=NO_STOP) THEN
   ...
   IF (T>=PROF_CLOCK(NM)) THEN
      ...
      ENDDO
   ENDIF
   IF (T>=CORE_CLOCK(NM) .OR. GLOBAL_STOP_STATUS==USER_STOP) THEN
      CALL DUMP_CORE(T,NM)
...
DROPLET_IF: IF ( (DROPLET_FILE .AND. .NOT.EVACUATION_ONLY(NM)).OR. (EVACUATION_ONLY(NM).AND.EVACUATION_GRID(NM)) ) THEN
   ...
   ELSE APPEND_DROPLET_FILE
      ...
      OPEN(LU_PART,FILE=FN10,FORM='UNFORMATTED',STATUS='REPLACE')
      WRITE(LU_PART) ONE_INTEGER                ! The number ONE, to indicate file Endian-ness
      WRITE(LU_PART) NINT(VERSION_NUMBER*100.)  ! FDS version number
      EVAC_ONLY2: IF (EVACUATION_ONLY(NM)) THEN
         WRITE(LU_PART) N_EVAC
         DO N=1,N_EVAC
            WRITE(LU_PART) EVAC_N_QUANTITIES,ZERO_INTEGER  ! ZERO_INTEGER is a place holder for future INTEGER quantities
            DO NN=1,EVAC_N_QUANTITIES
               WRITE(LU_PART) OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%NAME
               WRITE(LU_PROF) OUTPUT_QUANTITY(EVAC_QUANTITIES_INDEX(NN))%UNITS
            ENDDO
         ENDDO
      ELSE
         WRITE(LU_PART) N_PART
         DO N=1,N_PART
            PC => PARTICLE_CLASS(N)
            WRITE(LU_PART) PC%N_QUANTITIES,ZERO_INTEGER  ! ZERO_INTEGER is a place holder for future INTEGER quantities
            DO NN=1,PC%N_QUANTITIES
               WRITE(LU_PART) OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%NAME
               WRITE(LU_PART) OUTPUT_QUANTITY(PC%QUANTITIES_INDEX(NN))%UNITS
            ENDDO
         ENDDO
      ENDIF EVAC_ONLY2
...
WRITE(LU91) T,ICYC,PART_CLOCK(NM),CORE_CLOCK(NM),SLCF_CLOCK(NM), &
             ...
             ISOF_CLOCK(NM),BC_CLOCK,PART_COUNTER,WALL_COUNTER,DTOLD,DTNEXT, &
   ...
   IF (SURFACE(IBC)%THERMALLY_THICK) THEN
      IF (SURFACE(IBC)%SHRINK) THEN
         ...
         IF (SUM(WALL(IW)%N_LAYER_CELLS)>0) THEN
            ...
            READ(LU90) WALL(IW)%TMP_S,WALL(IW)%RHO_S
         ENDIF
      ELSE
...
PARTICLE_CLASS_LOOP: DO N=1,N_PART
   ...
   WRITE(LU_PART) NPLIM
   WRITE(LU_PART) (XP(I),I=1,NPLIM),(YP(I),I=1,NPLIM),(ZP(I),I=1,NPLIM)
   WRITE(LU_PART) (TA(I),I=1,NPLIM)
   IF (PC%N_QUANTITIES > 0) WRITE(LU_PART) ((QP(I,NN),I=1,NPLIM),NN=1,PC%N_QUANTITIES)
...
SELECT CASE(INDX)
   ...
   CASE(51:59) ! Particle Output
      SOLID_PHASE_OUTPUT = WMPUA(IWX,INDX-50)/MAX(1,PART_COUNTER)
   CASE(61:69) 
      SOLID_PHASE_OUTPUT = WCPUA(IWX,INDX-60)*0.001_EB/MAX(1,PART_COUNTER)
   CASE(71:79) 
