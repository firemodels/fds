...
USE CONTROL_VARIABLES
INTEGER :: N,I,J,K,II,JJ,KK,IPTS,JPTS,KPTS,N_EDGES_DIM,IW,IC,ICG,IOR,IERR,IPZ,NOM,ITER,IZERO,IIG,JJG,KKG,ICF
INTEGER, INTENT(IN) :: NM
...
CFACE_LOOP_0: DO ICF=1,N_CFACE_CELLS
   CFA => CFACE(ICF)
...
IF (.NOT.MOMENTUM_FILE) MOM_CLOCK = 1.E10_EB
...
IF (MOMENTUM_FILE) THEN
   ALLOCATE(MOMENTUM_SUM(3,NMESHES),STAT=IZERO)
   CALL ChkMemErr('INIT','MOMENTUM_SUM',IZERO)
   MOMENTUM_SUM = 0._EB
   ALLOCATE(U_DOT_SUM(3,N_U_DOT,NMESHES),STAT=IZERO)
   CALL ChkMemErr('INIT','U_DOT_SUM',IZERO)
   U_DOT_SUM = 0._EB
   ALLOCATE(U_DOT(3,N_U_DOT,NMESHES),STAT=IZERO)
   CALL ChkMemErr('INIT','U_DOT_SUM',IZERO)
   U_DOT = 0._EB
ENDIF
