...
!$OMP PARALLEL DO PRIVATE(ZZ_GET) SCHEDULE(guided)
DO K=1,KBAR
   DO J=1,JBAR
      DO I=1,IBAR
      ...
      DO K=1,KBAR
         DO J=1,JBAR
            DO I=1,IBAR
               ...
               MU_DNS = MU(I,J,K)
               DELTA = LES_FILTER_WIDTH_FUNCTION(DX(I),DY(J),DZ(K))
               CALL RNG_EDDY_VISCOSITY(MU_EFF,MU_DNS,RHOP(I,J,K),STRAIN_RATE(I,J,K),DELTA)
               MU(I,J,K) = MU_EFF
            ENDDO
         ENDDO
      ENDDO
...
!$OMP& PRIVATE(WC, II, JJ, KK, IOR, IIG, JJG, KKG, SF, ZZ_GET, MU_DNS, &
!$OMP& U2, V2, W2, VEL_GAS, SLIP_COEF, TSI, RAMP_T, VEL_T, DELTA, VDF, NU_EDDY)
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      CASE(SOLID_BOUNDARY)
         ...
         SELECT CASE (ABS(IOR))
            CASE (1)
               V2 = 0.25_EB*(VV(IIG,JJG-1,KKG)+VV(IIG,JJG,KKG))**2
               W2 = 0.25_EB*(WW(IIG,JJG,KKG-1)+WW(IIG,JJG,KKG))**2
               VEL_GAS = SQRT(V2+W2)
            CASE (2)
               U2 = 0.25_EB*(UU(IIG-1,JJG,KKG)+UU(IIG,JJG,KKG))**2
               W2 = 0.25_EB*(WW(IIG,JJG,KKG-1)+WW(IIG,JJG,KKG))**2
               VEL_GAS = SQRT(U2+W2)
            CASE (3)
               U2 = 0.25_EB*(UU(IIG-1,JJG,KKG)+UU(IIG,JJG,KKG))**2
         ...
         IF (LES) THEN
            ...
            MU(IIG,JJG,KKG) = MU_DNS + RHOP(IIG,JJG,KKG)*NU_EDDY
         ENDIF
