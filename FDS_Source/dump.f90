...
EVAC_ONLY3: IF (.NOT. ALL(EVACUATION_ONLY)) THEN
   DO N=1,N_PART
      ...
      IF (PC%SMOKEVIEW_ID=='CYLINDER') WRITE(LU_SMV,'(A,A,A,A)') ' ',PC%ID,'  % ',TRIM(PC%SMOKEVIEW_ID)
      IF (PC%SMOKEVIEW_ID/='CYLINDER') WRITE(LU_SMV,'(A,A)')     ' ',PC%ID
      ...
      ENDDO
      IF (PC%SMOKEVIEW_ID=='CYLINDER') WRITE(LU_SMV,'(4F9.3)') PC%DIAMETER,PC%LENGTH,PC%AZIMUTH,PC%ELEVATION
   ENDDO
ENDIF EVAC_ONLY3
...
EVAC_ONLY5: IF (ANY(EVACUATION_GRID)) THEN
   DO N=1,N_DOORS
      ...
      IF (ABS(EVAC_DOORS(N)%IOR) <= 1) XX = EVAC_DOORS(N)%IOR*(EVAC_DOORS(N)%X - XX)
      IF (ABS(EVAC_DOORS(N)%IOR) > 1 ) XX = EVAC_DOORS(N)%IOR*(EVAC_DOORS(N)%Y - YY)/2.0_EB
   ...
   DO N=1,N_EXITS
      ...
      ! x y z translate for the arrow
      ! x: upwards, y: right, z: towards the door
      IF (ABS(EVAC_EXITS(N)%IOR) <= 1) XX = EVAC_EXITS(N)%IOR*(EVAC_EXITS(N)%X - XX)
      IF (ABS(EVAC_EXITS(N)%IOR) > 1 ) XX = EVAC_EXITS(N)%IOR*(EVAC_EXITS(N)%Y - YY)/2.0_EB
...
SPEC_LOOP: DO N=0,N_SPECIES
   ...
   ELSE   
      ...
      IF (.NOT.ISOTHERMAL .AND. DNS .AND. SS%MODE==GAS_SPECIES) THEN
         ...
         WRITE(LU_OUTPUT,'(A,ES8.2)')  '                          500 C: ',Y2CP_C(7733)+Y2CP(773,N)
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_MASS_FRACTION, FED,GET_SPECIFIC_ENTHALPY,GET_SPECIFIC_HEAT,GET_CONDUCTIVITY,GET_MOLECULAR_WEIGHT
...
SELECT CASE(IND)
   ...
   CASE(46:47)  ! SPECIFIC ENTHALPY and ENTHALPY
      ...
      IF (N_SPECIES>0) THEN
         YY_G = YY(II,JJ,KK,:)
         CALL GET_SPECIFIC_ENTHALPY(YY_G,H_G,ITMP)
      ELSE
         H_G = Y2H_G_C(ITMP)
      ENDIF
            ...
            DO I=DV%I1,DV%I2
               ...
               IF (IND==113 .OR. IND==116 .OR. IND==119) THEN
                  ...
                  IF (N_SPECIES>0) THEN
                     ...
                     CALL GET_SPECIFIC_ENTHALPY(YY_G,H_G_SUM,ITMP)
                     CALL GET_SPECIFIC_ENTHALPY(YY_G,H_G,NINT(TMPA))
                  ELSE
      ...
      IF ( (PY%PDPA_START<=T) .AND.(PY%PDPA_END>=T) ) THEN
         ...
         IF (PY%QUANTITY == 'ENTHALPY') THEN
            DLOOP: DO I=1,NLP
               ...
               GAS_PHASE_OUTPUT = GAS_PHASE_OUTPUT + FOTH*PI*DR%R**3*PC%DENSITY*PC%C_P*(DR%TMP-PC%TMP_MELT)*DR%PWT
            ENDDO DLOOP          
...
REAL(EB) :: VC,TMP_N,RHO_N,U_N,H_G,YY_N(1:N_SPECIES)
INTEGER, INTENT(IN) :: NM
...
WALL_LOOP: DO IW=1,NWC
   ...
   IF (N_SPECIES>0) THEN
      ...
      CALL GET_SPECIFIC_ENTHALPY(YY_N,H_G,ITMP)
   ELSE
      H_G = Y2H_G_C(ITMP)
   ENDIF
