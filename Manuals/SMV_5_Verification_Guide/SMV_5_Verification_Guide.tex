...
The tests in this chapter verify whether visualization types such as surface contours (boundary files), isosurfaces, particles, slice files, 3D smoke files, PLOT3D files and fire lines (for WUI simulations) are working as intended.
These verifications use
the FDS input files {\tt plume5c.fds}\ (see Appendix \ref{FDSplume5c}) and {\tt fireline.fds} (see Appendix \ref{FDSfireline}) to generate the simulation data.  The verification figures are generated automatically using the Smokeview script files {\tt plume5c.ssf}\ (see Appendix \ref{SSFplume5c}) and {\tt fireline.ssf}\ (see Appendix \ref{SSFfireline}).  The use of scripting allows the figures and hence this document to be updated easily as changes are made in FDS, Smokeview or the FDS input data files.  This allows the verification process to be ongoing.

The {\tt plume5c.fds}\ test case is a simple fire plume with two blockages to make the smoke flow more complex.  A portion of the gas phase and the upper blockage are both initialized to 600~\degC\ (rather than 20~\degC) in order to test the conversion of a known temperature data to the correct color (as shown in the colorbar).  The {\tt fireline.fds}\ test case is a terrain test case.  The center of the case has a {\em hill}.  The fire line data displayed conforms to this hill.
...
of fire modeling simulations.  Future work with Smokeview then involves adding or improving visualization algorithms that allow one to provide more and better insight, in particular to remove the limitations discussed at the beginning of this report.  Work needs to be done to visualize the fire more realistically, perhaps by using its temperature as an indicator of color.
