...
REAL(EB) :: DELKDELT,VC,DTDX,DTDY,DTDZ,TNOW,ZZ_GET(0:N_TRACKED_SPECIES), &
            ...
            TMP_G,TMP_WGT,DIV_DIFF_HEAT_FLUX,H_S,PBAR_D_RHO_H_S_DT
...
ENERGY: IF (.NOT.EVACUATION_ONLY(NM)) THEN
   ...
   CORRECTION_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      WC => WALL(IW)
      IF (WC%BOUNDARY_TYPE==NULL_BOUNDARY .OR. &
          WC%BOUNDARY_TYPE==OPEN_BOUNDARY .OR. WC%BOUNDARY_TYPE==INTERPOLATED_BOUNDARY) CYCLE CORRECTION_LOOP
      II  = WC%II
      JJ  = WC%JJ
      KK  = WC%KK
      IIG = WC%IIG
   ...
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            IF (PREDICTOR) THEN
               PBAR_D_RHO_H_S_DT = ( RHO_H_S_P(I,J,K)/PBAR_P(K,PRESSURE_ZONE(I,J,K)) - RHO_H_S_OVER_PBAR(I,J,K) )/DT
            ELSE
               PBAR_D_RHO_H_S_DT = ( RHO_H_S_P(I,J,K)/PBAR_P(K,PRESSURE_ZONE(I,J,K)) - RHO_H_S_OVER_PBAR(I,J,K) )/(0.5_EB*DT)
            ENDIF
            PBAR_D_RHO_H_S_DT = PBAR_P(K,PRESSURE_ZONE(I,J,K))*PBAR_D_RHO_H_S_DT
...
DO IPZ=1,N_ZONE
   IF (EVACUATION_ONLY(NM)) CYCLE
   P_EQ_NUM = PBAR_P(1,IPZ)*PSUM(IPZ,NM)
   ...
   DO IOPZ=N_ZONE,0,-1
      ...
      IF (CONNECTED_ZONES(IPZ,IOPZ,NM)) THEN
         IF (IOPZ==0) THEN
            ...
            P_EQ_DEN = 1._EB
         ELSE
            P_EQ_NUM = P_EQ_NUM + PBAR_P(1,IOPZ)*PSUM(IOPZ,NM)
            P_EQ_DEN = P_EQ_DEN + PSUM(IOPZ,NM)
         ENDIF
      ENDIF
   ENDDO
   P_EQ = P_EQ_NUM/P_EQ_DEN
   USUM_ADD(IPZ) = RDT*PSUM(IPZ,NM)*(PBAR_P(1,IPZ)-P_EQ)
ENDDO
