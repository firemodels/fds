...
USE MATH_FUNCTIONS
INTEGER :: N,J,NS,NS2,NR,SPEC_INDEX(MAX_SPECIES)
...
PROC_MATL_LOOP: DO N=1,N_MATL
   ...
   FIND_SPECIES:IF (ANY(ML%NU_GAS > 0._EB)) THEN
      ...
      DO NS=1,N_SPECIES
         ...
         ENDDO
         IF (SIMPLE_CHEMISTRY) THEN
            IF (TRIM(MATL_SPEC_ID(N,NS))=='FUEL' .OR. TRIM(MATL_SPEC_ID(N,NS))==REACTION(1)%FUEL) SPEC_INDEX(NS)= I_FUEL
         ENDIF
         IF (SPEC_INDEX(NS)==-1) THEN
      ...
      REACTION_SEARCH: DO NR=1,ML%N_REACTIONS
         ...
         SPEC_LOOP_1: DO NS=1,N_SPECIES
            ...
            IF(SPEC_INDEX(NS)==-1) THEN
               ...
               CALL SHUTDOWN(MESSAGE)                  
            ENDIF
            ML%NU_GAS(NR,SPEC_INDEX(NS))=NU_GAS(NR,NS)
          ENDDO SPEC_LOOP_1
