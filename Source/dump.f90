...
REAL(EB) :: CONCORR,VOLSUM,MFT,ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,KSGS,DEPTH,UN,H_S,RHO_D_DYDN,RHO_D_DZDN(1:N_TRACKED_SPECIES)
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(60) ! MASS FLUX WALL CELL (deprecated)
      SOLID_PHASE_OUTPUT = -ONE_D%RHO_F*ONE_D%UW
   ...
   CASE(64) ! MASS FLUX WALL
      ...
      IF (Z_INDEX > 0) THEN
         Y_SPECIES = ONE_D%ZZ_F(Z_INDEX)
         RHO_D_DYDN = -ONE_D%RHO_D_F(Z_INDEX)*(ONE_D%ZZ_G(Z_INDEX)-ONE_D%ZZ_F(Z_INDEX))*2._EB*ONE_D%RDN
      ELSEIF (Y_INDEX > 0) THEN
         ZZ_GET(1:N_TRACKED_SPECIES) = ONE_D%ZZ_F(1:N_TRACKED_SPECIES)
         CALL GET_MASS_FRACTION(ZZ_GET,Y_INDEX,Y_SPECIES)
         RHO_D_DZDN = -ONE_D%RHO_D_F*(ONE_D%ZZ_G-ONE_D%ZZ_F)*2._EB*ONE_D%RDN
         RHO_D_DYDN = DOT_PRODUCT(Z2Y(Y_INDEX,1:N_TRACKED_SPECIES),RHO_D_DZDN)
      ELSE
         Y_SPECIES = 1._EB
         RHO_D_DYDN = 0._EB
      ENDIF
      ! convention here is: inflow is positive (adds mass to domain), outflow is negative (subtracts mass)
      SOLID_PHASE_OUTPUT = SIGN(1._EB,REAL(DV%IOR,EB))*(Y_SPECIES*WC%ONE_D%RHO_F*UN + RHO_D_DYDN)
