...
DO NN = 1,NE%N_DUCTNODES
   ...
   IF (DN%DUCTRUN > 0) THEN
      ...
      ENDIF
   ENDIF
   IF ((DN%LEAKAGE .OR. DN%VENT .OR. DN%AMBIENT) .AND. &
...
INTEGER:: ND, ND2, NN, COUNTER, COUNTER2, I
LOGICAL, DIMENSION(:):: LOSS_D(N_DUCTS,2), LOSS_N(N_DUCTNODES)
LOGICAL, DIMENSION(:):: LOSS_D_OLD(N_DUCTS,2), LOSS_N_OLD(N_DUCTNODES)
LOGICAL, ALLOCATABLE, DIMENSION(:,:,:) :: LOSS_D_TEMP
TYPE(DUCTNODE_TYPE), POINTER :: DN=>NULL()
TYPE(DUCT_TYPE), POINTER :: DU=>NULL(),DU2=>NULL()
...
DO ND = 1, N_DUCTS
   ...
   IF (DUCT(ND)%ROUGHNESS > 0._EB) LOSS_D(ND,:) = .TRUE.
   IF (DUCT(ND)%LOSS(1) > 0._EB) LOSS_D(ND,1) = .TRUE.
   IF (DUCT(ND)%LOSS(2) > 0._EB) LOSS_D(ND,2) = .TRUE.
   IF (DUCT(ND)%FAN_INDEX > 0) THEN
      IF (FAN(DUCT(ND)%FAN_INDEX)%FAN_TYPE==1) LOSS_D(ND,:) = .TRUE.
   ENDIF
   IF (DUCT(ND)%LEAKAGE) LOSS_D(ND,:) = .TRUE.
   WRITE(*,*) 'LOSS_D',ND,LOSS_D(ND,1),LOSS_D(ND,2)
ENDDO

DO NN =1, N_DUCTNODES
   IF (DUCTNODE(NN)%LEAKAGE) LOSS_N(NN) = .TRUE.
ENDDO
WRITE(*,*) 'LOSS_N',(LOSS_N(NN),NN=1,N_DUCTNODES)
CHANGE_LOOP: DO
   LOSS_D_OLD = LOSS_D
   LOSS_N_OLD = LOSS_N
   NODELOOP: DO NN = 1,N_DUCTNODES
      ...
      DN => DUCTNODE(NN)
      END_NODE_IF: IF (DN%N_DUCTS==1) THEN
         ! For a terminal ductnode with 1 duct check each flow direction for a ductnode loss_array or a duct loss
         DU => DUCT(DN%DUCT_INDEX(1))
         COUNTER = 0
         COUNTER2 = 0
         IF (DN%LOSS_ARRAY(1,2) > 0._EB) THEN
            COUNTER = 1
            IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
            IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
         ENDIF
         IF (DN%LOSS_ARRAY(2,1) > 0._EB) THEN
            COUNTER2 = 1
            IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
            IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
         ENDIF
         IF (DU%NODE_INDEX(1)==NN) THEN
            IF (LOSS_D(DN%DUCT_INDEX(1),1)) COUNTER = 1
            IF (LOSS_D(DN%DUCT_INDEX(1),2)) COUNTER2 = 1
         ENDIF
         IF (DU%NODE_INDEX(2)==NN) THEN
            IF (LOSS_D(DN%DUCT_INDEX(1),2)) COUNTER = 1
            IF (LOSS_D(DN%DUCT_INDEX(1),1)) COUNTER2 = 1
         ENDIF
         IF (COUNTER + COUNTER2 == 2) LOSS_N(NN) = .TRUE.
         CYCLE NODELOOP
      ENDIF END_NODE_IF

      TWO_DUCTS_IF: IF (DN%N_DUCTS==2)  THEN
         ! For a ductnode with 2 ducts check either loss_array(1,2) and loss_array(2,1) are non zero or
         ! that at least one duct has a loss for each flow direction
         ...
         COUNTER = 0
         COUNTER2 = 0
         !Check flow from duct 1 to duct 2
         IF (DN%LOSS_ARRAY(1,2) > 0._EB) THEN
            COUNTER = 1
            IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
            IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
            IF (DU2%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(2),1) = .TRUE.
            IF (DU2%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(2),2) = .TRUE.
         ELSE
            IF (DU%NODE_INDEX(1)==NN .AND. LOSS_D(DN%DUCT_INDEX(1),2)) THEN
               COUNTER = 1
               IF (DU2%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(2),1) = .TRUE.
               IF (DU2%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(2),2) = .TRUE.
            ENDIF
            IF (DU%NODE_INDEX(2)==NN .AND. LOSS_D(DN%DUCT_INDEX(1),1)) THEN
               COUNTER = 1
               IF (DU2%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(2),1) = .TRUE.
               IF (DU2%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(2),2) = .TRUE.
            ENDIF
            IF (DU2%NODE_INDEX(1)==NN .AND. LOSS_D(DN%DUCT_INDEX(2),1))  THEN
               COUNTER = 1
               IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(2),2) = .TRUE.
               IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(2),1) = .TRUE.
            ENDIF
            IF (DU2%NODE_INDEX(2)==NN .AND. LOSS_D(DN%DUCT_INDEX(2),2))  THEN
               COUNTER = 1
               IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(2),2) = .TRUE.
               IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(2),1) = .TRUE.
            ENDIF
         ...
         !Check flow from duct 2 to duct 1
         IF (DN%LOSS_ARRAY(2,1) > 0._EB) THEN
            COUNTER2 = 1
            IF (DU2%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(2),1) = .TRUE.
            IF (DU2%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(2),2) = .TRUE.
            IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
            IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
         ELSE
            IF (DU2%NODE_INDEX(2)==NN .AND. LOSS_D(DN%DUCT_INDEX(2),1)) THEN
               COUNTER2 = 1
               IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
               IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
            ENDIF
            IF (DU2%NODE_INDEX(1)==NN .AND. LOSS_D(DN%DUCT_INDEX(2),2)) THEN
               COUNTER2 = 1
               IF (DU%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
               IF (DU%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
            ENDIF
            IF (DU%NODE_INDEX(2)==NN .AND. LOSS_D(DN%DUCT_INDEX(1),2))  THEN
               COUNTER2 = 1
               IF (DU2%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
               IF (DU2%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
            ENDIF
            IF (DU%NODE_INDEX(1)==NN .AND. LOSS_D(DN%DUCT_INDEX(1),1))  THEN
               COUNTER2 = 1
               IF (DU2%NODE_INDEX(2)==NN) LOSS_D(DN%DUCT_INDEX(1),1) = .TRUE.
               IF (DU2%NODE_INDEX(1)==NN) LOSS_D(DN%DUCT_INDEX(1),2) = .TRUE.
            ENDIF
      ...
      ! For 3 or more ducts attached to a node,for each possible inlet need losses for each associated outlet
      COUNTER = 0
      ALLOCATE (LOSS_D_TEMP(DN%N_DUCTS,DN%N_DUCTS,2))
      DO ND=1, DN%N_DUCTS
         LOSS_D_TEMP(ND,:,1) = LOSS_D(:,1)
         LOSS_D_TEMP(ND,:,2) = LOSS_D(:,2)
         IF (DUCT(DN%DUCT_INDEX(ND))%NODE_INDEX(1)==NN) THEN
            LOSS_D_TEMP(ND,ND,2) = .TRUE.
         ELSE
            LOSS_D_TEMP(ND,ND,1) = .TRUE.
         ENDIF
      ENDDO
      DO ND=1, DN%N_DUCTS
         COUNTER2 = 0
         WRITE(*,*) 'D1:',ND,(LOSS_D_TEMP(ND,I,1),LOSS_D_TEMP(ND,I,2),I=1,N_DUCTS)
         INNERNODE: DO ND2=1, DN%N_DUCTS
            IF (ND==ND2) CYCLE INNERNODE

            ! If the DN LOSS for a path exists, then increment the counter and set the downstream duct loss flags
            IF (DN%LOSS_ARRAY(ND,ND2) > 0._EB) THEN
               WRITE(*,*) 'A:',ND,ND2,DN%DUCT_INDEX(ND2),DUCT(DN%DUCT_INDEX(ND2))%NODE_INDEX(1)
               COUNTER2 = COUNTER + 1
               IF (DUCT(DN%DUCT_INDEX(ND2))%NODE_INDEX(1)==NN) THEN
                  LOSS_D_TEMP(ND,ND2,:) = .TRUE.
               ELSE
                  LOSS_D_TEMP(ND,ND2,:) = .TRUE.
               ENDIF
               WRITE(*,*) 'D2A:',(LOSS_D_TEMP(ND,I,1),LOSS_D_TEMP(ND,I,2),I=1,N_DUCTS)
            ELSE
               WRITE(*,*) 'B:',ND,ND2,DN%DUCT_INDEX(ND2),DUCT(DN%DUCT_INDEX(ND2))%NODE_INDEX(1)
               IF (DUCT(DN%DUCT_INDEX(ND2))%NODE_INDEX(1)==NN) THEN
                  IF (LOSS_D_TEMP(ND,ND2,1)) THEN
                     COUNTER2 = COUNTER2 + 1
                     LOSS_D_TEMP(ND,ND2,2) = .TRUE.
                  ENDIF
               ELSE
                  IF (LOSS_D_TEMP(ND,ND2,2)) THEN
                     COUNTER2 = COUNTER2 + 1
                     LOSS_D_TEMP(ND,ND2,1) = .TRUE.
                  ENDIF
               ENDIF
               WRITE(*,*) 'D2B:',(LOSS_D_TEMP(ND,I,1),LOSS_D_TEMP(ND,I,2),I=1,N_DUCTS)
            ENDIF
         ENDDO INNERNODE
         WRITE(*,*) 'C2',NN,ND,COUNTER2
         IF (COUNTER2 >= DN%N_DUCTS - 2) THEN
            COUNTER = COUNTER + 1
            IF (DUCT(DN%DUCT_INDEX(ND))%NODE_INDEX(1)==NN) THEN
               IF (COUNTER2 == DN%N_DUCTS-1) LOSS_D_TEMP(ND,ND,2) = .TRUE.
            ELSE
               IF (COUNTER2 == DN%N_DUCTS-1) LOSS_D_TEMP(ND,ND,1) = .TRUE.
            ENDIF
            DO ND2=1,DN%N_DUCTS
               IF (DUCT(DN%DUCT_INDEX(ND2))%NODE_INDEX(1)==NN) THEN
                  LOSS_D_TEMP(ND,ND2,1) = .TRUE.
               ...
               ENDIF
            ENDDO
            WRITE(*,*) 'D2:',ND,(LOSS_D_TEMP(ND,ND2,1),LOSS_D_TEMP(ND,ND2,2),ND2=1,N_DUCTS)
         ENDIF
      ENDDO
      DO ND=1,DN%N_DUCTS
         IF (ALL(LOSS_D_TEMP(:,ND,1))) LOSS_D(ND,1) = .TRUE.
         IF (ALL(LOSS_D_TEMP(:,ND,2))) LOSS_D(ND,2) = .TRUE.
      ENDDO
      DEALLOCATE(LOSS_D_TEMP)
      WRITE(*,*) 'C1',NN,COUNTER
      IF (COUNTER == DN%N_DUCTS) LOSS_N(NN) = .TRUE.
   ENDDO NODELOOP
   WRITE(*,*) 'D3:',(LOSS_D(ND,1),LOSS_D(ND,2),ND=1,N_DUCTS)
   WRITE(*,*) 'N2:',(LOSS_N(NN),NN=1,N_DUCTNODES)
   IF (ALL(LOSS_D .EQV. LOSS_D_OLD) .AND. ALL(LOSS_N .EQV. LOSS_N_OLD)) EXIT CHANGE_LOOP

ENDDO CHANGE_LOOP

IF (.NOT. ALL(LOSS_D) .OR. .NOT. ALL(LOSS_N)) THEN
   WRITE(MESSAGE,'(A,I5,A,I5)') 'ERROR: HVAC network has insufficient LOSS or ROUGHNESS defintions for DUCTs and DUCTNODEs'
   CALL SHUTDOWN(MESSAGE); RETURN
ENDIF
...
DO NR = 1, N_DUCTRUNS
   QFAN_IF: IF (DUCTRUN(NR)%N_QFANS > 0) THEN
      ...
      DO NN = 1,DUCTRUN(NR)%N_M_DUCTNODES
         DUCTRUN(NR)%P(NN,:,:) = DUCTNODE(DUCTRUN(NR)%NODE_M_INDEX(NN))%P - 101325._EB
      ENDDO
...
DO ND = 1, DR%N_M_DUCTS
   ...
   IF (DN%VENT .AND. HVAC_LOCAL_PRESSURE) &
...
DUCT_LOOP: DO ND = 1, DR%N_M_DUCTS
   ...
   IF (DN%VENT) LHS(ND,ND) = LHS(ND,ND)-DN%DIR(1)*DU%AREA/(DU%LENGTH*GAMMA*NODE_AREA_EX(DU%NODE_INDEX(1)))
   DN=>DUCTNODE(DU%NODE_INDEX(2))
   IF (DN%DUCTRUN_M_INDEX > 0) LHS(ND,DR%N_M_DUCTS + DN%DUCTRUN_M_INDEX) =  DT_QF/(DR%RHO_D(ND,NF)*DU%LENGTH)
   IF (DN%VENT) LHS(ND,ND) = LHS(ND,ND)+DN%DIR(1)*DU%AREA/(DU%LENGTH*GAMMA*NODE_AREA_EX(DU%NODE_INDEX(2)))
ENDDO DUCT_LOOP
