...
USE LOCMAT_SOLVER, ONLY : ULMAT_SOLVER_H
...
! IF (PRES_FLAG == SCARC_FLAG .OR. PRES_FLAG == USCARC_FLAG) THEN
!    CALL SCARC_SETUP
!    CALL STOP_CHECK(1)
! ENDIF
...
MAIN_LOOP: DO
   ...
   CORRECT_VELOCITY_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      IF ((DIAGNOSTICS .OR. STORE_CARTESIAN_DIVERGENCE) .AND. .NOT.EVACUATION) CALL CHECK_DIVERGENCE(NM)
...
SELECT CASE(PRES_FLAG)
   CASE(GLMAT_FLAG,UGLMAT_FLAG); CALL FINISH_GLMAT_SOLVER_H
END SELECT
