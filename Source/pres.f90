...
IF (PREDICTOR) THEN
   HP => H
   RHOP => RHO
   IF (TUNNEL_PRECONDITIONER) H_BAR_P => H_BAR
ELSE
   HP => HS
   RHOP => RHOS
   IF (TUNNEL_PRECONDITIONER) H_BAR_P => H_BAR_S
ENDIF
...
IF (CHECK_POISSON) THEN
   ...
   !$OMP PARALLEL DO PRIVATE(II,I,J,K,RHSS,LHSS) SCHEDULE(STATIC)
   DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            IF (TUNNEL_PRECONDITIONER) THEN
               II = I_OFFSET(NM) + I
               LHSS = LHSS + (TP_RDXN(II)*(H_BAR_P(II+1)-H_BAR_P(II)) - TP_RDXN(II-1)*(H_BAR_P(II)-H_BAR_P(II-1)))*RDX(I)
            ENDIF
...
IF (ITERATE_BAROCLINIC_TERM) THEN
   ...
   IF (TUNNEL_PRECONDITIONER) THEN
      ...
      DO I=0,IBP1
         II = I_OFFSET(NM) + I
         P(I,0:JBP1,0:KBP1) = P(I,0:JBP1,0:KBP1) + RHOP(I,0:JBP1,0:KBP1)*H_BAR_P(II)
      ENDDO
      !$OMP END PARALLEL DO
   ENDIF
