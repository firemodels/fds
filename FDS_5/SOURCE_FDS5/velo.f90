...
PRIVATE
CHARACTER(255), PARAMETER :: veloid='$Id: velo.f90,v 1.14 2007/03/22 22:14:53 mcgratta Exp $'
...
PARABOLIC_IF: IF (DNS .OR. DXMIN<0.005_EB) THEN
   ...
   VN = 2._EB*DT*R_DX2*MUTRM
 
ENDIF PARABOLIC_IF
 
! Adjust time step size if necessary
 
IF (CFL<1.0_EB .AND. VN<1.0_EB) THEN
   DTNEXT = DT
   IF (CFL<=0.8_EB .AND. VN<0.8_EB) DTNEXT = MIN(1.1_EB*DT,DTINT)
ELSE
   IF (UVWMAX==0._EB) UVWMAX = 1._EB
   DT    = 0.9_EB*MIN( 1.0_EB/UVWMAX , 1.0_EB/(2._EB*R_DX2*MUTRM) )
