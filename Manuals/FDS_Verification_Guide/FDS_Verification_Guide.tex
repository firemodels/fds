...
\item Joonas Ryynänen of VTT, Finland, designed and documented the FED/FIC test cases.
...
\end{tabular*}
\caption[Yield of combustion products using three alternative definitions of the same chemistry.]{The cumulation of combustion product mass using three alternative definitions of the combustion chemistry. (Left) Simple chemistry vs. primitive species. (Right) Simple chemistry vs. lumped spcies. }
...
The three test cases called {\bf water\_evaporation\_x} involve stationary water droplets in a cubic box
The walls of the box are
assumed adiabatic, meaning that there are no leaks or heat losses. The air within the box is stirred to maintain uniform conditions. In each case, the numerical solutions are compared to analytical solutions.
...
Figures~\ref{water_evaporation_1_plots} to~\ref{water_evaporation_3_plots}  displays
the average enthalpy, humidity, density, pressure, temperature and mass of water vapor of the gas within the box.
For the two last cases, the liquid water concentration is also plotted. The solid horizontal lines denote the expected steady-state values.

In the first case, the analytical solution for the stationary state derives from the first principle of thermodynamics. The plot of enthalpy in~\ref{water_evaporation_1_plots} displays the change in enthalpy of both the droplets and the gas. The change in the enthalpy of the water droplets should equal the change in the enthalpy of the gas minus the work
...
In the second case, the dimensions of the box are 40~cm on a side, the initial
air temperature is 500~$^\circ$C. There is only one water droplet in the box whose diameter is equal to 100000~$\mu$m and temperature  is 20~$^\circ$C. The initial mass fraction of water vapor is 0. It is expected to reach the saturation state. The analytical solution for the stationary state derives from the first principle of thermodynamics and the Clausius-Clapeyron equation by assuming that the liquid water temperature is equal to air temperature at the stationary state.
...
air temperature is 500~$^\circ$C, the  initial diameter of all the droplets is 200~$\mu$m, the water temperature is 20~$^\circ$C, and the total mass of water droplets is 0.1~kg. The initial mass fraction of water vapor is 0. It is expected to reach the saturation state. As in the second case, the analytical solution for the stationary state derives from the first principle of thermodynamics and the Clausius-Clapeyron equation by assuming that the liquid water temperature is equal to air temperature at the stationary state.
...
\includegraphics[width=3.2in]{SCRIPT_FIGURES/water_evaporation_3_W_temperature}
\end{tabular*}
...
\subsection{Rate of evaporation}

The test consists in one single evaporating water droplet in an ambient air. The computational results are evaluated upon the 
the rate of evaporation and more precisely the droplet diameter versus time. The comparison is done with measurements extracted from the reference~\cite{Ranz}.

\begin{figure}[ht]
\begin{center}
\includegraphics[height=3.00in]{SCRIPT_FIGURES/rate_of_evaporation_diameter}
\caption[Sample case {\bf rate_of_evaporation}.]{Droplet diameter versus time}
\end{center}
\label{fig_rate_of_evaporation}
\end{figure}
