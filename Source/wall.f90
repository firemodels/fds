...
SELECT CASE(BACKING)
   ...
   CASE(EXPOSED)  ! The backside is exposed to gas in current or adjacent mesh.
      ...
      IF (BACK_MESH/=NM .AND. BACK_MESH>0) THEN  ! Back side is in other mesh.
         ...
         DTMP = TMP_GAS_BACK - B1%TMP_B
         IF (.NOT.ISOLATED_THIN_WALL) THEN
            HTCB = HEAT_TRANSFER_COEFFICIENT(BACK_MESH,DTMP,SF%H_FIXED_B,SURF_INDEX_IN=SURF_INDEX,WALL_INDEX_IN=BACK_WALL_INDEX)
         ELSE
            HTCB = 0._EB
         ENDIF
