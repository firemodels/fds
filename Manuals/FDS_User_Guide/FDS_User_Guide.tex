...
Additioally to the mesh assignment to individual MPI processes, the number of OpenMP threads for each MPI process may be specified. The parameter {\ct N_THREADS} instructs FDS to set this number of threads and must be consistent for all meshes on the same MPI process, but may vary between them:
\begin{lstlisting}
&MESH ID='mesh1', IJK=..., XB=..., MPI_PROCESS=0 /
