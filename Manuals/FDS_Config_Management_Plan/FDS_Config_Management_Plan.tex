...
\section{GitHub Actions: Repository Checks}

GitHub Actions provide automated checks of changes submitted to the \texttt{fds} and \texttt{out} repositories. Workflow definitions reside in each repository's \path{.github/workflows} directory, and results are reported on GitHub. These checks complement the verification and validation testing performed by Firebot. The workflows are summarized below.

\begin{description}
\item[Platform builds:] \path{linux.yml}, \path{osx.yml}, and \path{windows.yml} compile and launch debug and release versions of FDS using the supported Intel or GNU compiler and MPI combinations. The Linux workflow also checks Python package imports with \path{hello_world.py} and runs regression tests of the PR security logic.

\item[CMake builds:] \path{cmake.yml} builds FDS on Linux, macOS, and Windows across compiler, debug/release, and OpenMP configurations. CTest checks that the resulting executable starts. The platform and CMake workflows run for relevant source, build, or workflow changes.

\item[PR security:] \path{pr-security.yml} coordinates admission and build checks for FDS pull requests, while \path{virus-scan.yml} uses ClamAV to scan the submitted and proposed merge snapshots. Outside contributions must pass scanning before build checks begin; approved developers' scans run independently, including after merging. Section~\ref{sec:pr-virus-scanning} describes this process and its repository settings.

\item[Line endings:] \path{Line_Endings.yml} in both repositories reports an error if text files contain DOS-style line endings.

\item[Validation revisions:] \path{Git_Revisions.yml} in \texttt{out} checks only tracked \path{*_git.txt} files and reports an error if a revision string contains \texttt{dirty}, indicating uncommitted changes in the source used to generate the results.
\end{description}
...
Scripts use the results from Stage~5 to generate plots, regression information, and timing statistics for the FDS User~\cite{FDS_Users_Guide}, Verification~\cite{FDS_Verification_Guide}, and Validation~\cite{FDS_Validation_Guide} Guides, as well as the Smokeview User and Verification Guides.

The file \path{validation_scatterplot_output_baseline.csv} provides a version-controlled baseline for validation statistics. The script \path{validation_git_stats.py} compares each predicted quantity with this baseline and adds signed relative differences in model relative standard deviation, $\tilde{\sigma}_M$, and bias factor, $\delta$, to \path{validation_scatterplot_output.csv}.

Changes greater than 10\,\% in magnitude produce an \texttt{Error} when $\tilde{\sigma}_M$ increases or $\delta$ moves farther from unity. Improvements exceeding the same threshold produce a \texttt{Warning}. Smaller changes are recorded without a threshold message.

Small deviations from a baseline may occur with code changes. As model improvements reduce scatter and bring bias closer to unity, the baseline may be updated to reflect the improved statistics. These reviewed updates establish progressively tighter reference values for subsequent changes, encouraging development of the model physics toward tighter validation statistics over time. Version control preserves the history of these baseline changes.
...
\section{Virus Scanning of Pull Requests}
\label{sec:pr-virus-scanning}

GitHub Actions use ClamAV to scan pull requests to FDS and participating developers' forks. These checks examine repository content and do not replace release-bundle scans. Security code and approved account IDs come from the receiving repository's trusted revision. When both the author and event sender are approved developers, scanning proceeds in the background, including after merging; other pull requests must pass scanning before admission to build checks.

Disposable GitHub-hosted runners update signatures, test detection with EICAR, and scan the submitted and merge snapshots. Files are verified against Git's file list and hashes. Scanner jobs use read-only permissions, receive no project secrets, and never execute submitted code. Detections, errors, timeouts, and incomplete scans block outside contributions.

Each receiving repository must require \texttt{FDS / PR admission} and \texttt{FDS / PR builds} through branch protection, require workflow approval for all outside collaborators, and require maintainer review of security changes. The separate \texttt{FDS / ClamAV scan} status reports scan results. External testing systems must enforce admission independently. A clean scan does not establish that code is benign; code review and isolated testing remain necessary.
...
To verify a download, find the SHA-256 digest for that exact file under \emph{Assets} on the official GitHub release page. In the download directory, run the appropriate command below, replacing \texttt{bundle-file} with the full filename:
\begin{center}
\begin{tabular}{ll}
Windows (PowerShell) & \verb|Get-FileHash -Algorithm SHA256 "bundle-file"| \\
macOS (Terminal) & \verb|shasum -a 256 "bundle-file"| \\
Linux (Terminal) & \verb|sha256sum "bundle-file"| \\
\end{tabular}
\end{center}
Compare all 64 hexadecimal digits, ignoring letter case and any \texttt{sha256:} prefix. A match confirms that the download is identical to the published GitHub asset; do not install the file if the digests differ.
