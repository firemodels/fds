...
REAL :: ACTIVATION_TEMPERATURE,A_C,ALPHA,AREA,A_T,A_V,C_I,CONDUIT_DIAMETER,CONDUIT_THICKNESS,CUTOFF_TIME,C_PL,C_CJ, &
        ...
        RHO_I,RHO_S,RHO_STEEL,RTI,T,t_activation,T_END,T_P,TMP_A,TMP_AMBIENT,TMP_G,T_CLOCK,U_JET,V,V_ENT, &
...
LOGICAL :: USE_MQH_AS_TMPA=.FALSE.,USE_FPA_AS_TMPA=.FALSE., &
           USE_DB_AS_TMPA=.FALSE.,USE_BEYLER_AS_TMPA=.FALSE., &
           ITER=.TRUE.,PROFILE=.FALSE.,STEEL_PROTECTED=.FALSE., &
...
NAMELIST /ALPERT/ Q,LOCATION_FACTOR,T_END,R_VALUES,H_VALUES,LABEL,TMP_A,OUTPUT_FILE,T_SQUARED,ALPHA, &
                  USE_MQH_AS_TMPA,USE_FPA_AS_TMPA,USE_DB_AS_TMPA,USE_BEYLER_AS_TMPA, &
                  L,W,H,H_V,W_V,C_S,K_S,RHO_S,DELTA,M_DOT
...
DO I=0,50
   ...
   ! Use various HGL temperature correlations as TMP_A value
   CALL USE_CORRELATION_FOR_TMPA
...
T_SQUARED=.FALSE.
USE_MQH_AS_TMPA=.FALSE.
USE_FPA_AS_TMPA=.FALSE.
USE_DB_AS_TMPA=.FALSE.
USE_BEYLER_AS_TMPA=.FALSE.
...
SUBROUTINE USE_CORRELATION_FOR_TMPA
! Use various HGL temperature correlations as TMP_A value

REAL :: K1,K2,M,SIGMA

IF (USE_MQH_AS_TMPA) THEN
   SIGMA = 5.67e-11
   ...
   TMP_G = TMP_AMBIENT + 6.85*( Q**2/(A_V*SQRT(H_V)*A_T*H_K) )**(1./3.)

   ! Set ambient temperature to HGL temperature
   TMP_A = TMP_G
ELSEIF (USE_FPA_AS_TMPA) THEN
   T_P = (RHO_S*C_S/K_S) * (DELTA/2.)**2
   A_T = 2.*L*W + 2.*L*H + 2.*W*H
   ...
   TMP_G = TMP_AMBIENT*(1. + 0.63*(Q/(M_DOT*C_P*TMP_AMBIENT))**0.72 * (H_K*A_T/(M_DOT*C_P))**-0.36)

   ! Set ambient temperature to HGL temperature
   TMP_A = TMP_G
ELSEIF (USE_DB_AS_TMPA) THEN
   A_T = 2.*L*W + 2.*L*H + 2.*W*H
   V = L*W*H
   RHO_A = 353./(TMP_AMBIENT)
   ...
   IF (T>0.) THEN
      H_K = 0.4*MAX( SQRT(K_S*RHO_S*C_S/(T+0.000000001)) , K_S/DELTA )
      TMP_G = TMP_AMBIENT + (Q/(M_DOT*C_P + H_K*A_T))
   ELSE
      H_K = 0.
      TMP_G = TMP_AMBIENT
   ENDIF
   ...
   ! Set ambient temperature to HGL temperature
   TMP_A = TMP_G
ELSEIF (USE_BEYLER_AS_TMPA) THEN
   A_T = 2.*L*W + 2.*L*H + 2.*W*H
   RHO_A = 353./(TMP_AMBIENT)
   M = L*W*H*RHO_A
   ...
   TMP_G = TMP_AMBIENT + (2.*K2/K1**2)*(K1*SQRT(T) - 1. + EXP(-K1*SQRT(T)))
   
   ! Set ambient temperature to HGL temperature
   TMP_A = TMP_G
ELSE
   ! Use specified ambient temperature
   TMP_A = TMP_AMBIENT
ENDIF
