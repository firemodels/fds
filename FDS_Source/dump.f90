...
SELECT CASE(IND)
   ...
   CASE(50)  ! RKE (resolved kinetic energy)
      IF (.NOT.CHECK_KINETIC_ENERGY) THEN
         WRITE(MESSAGE,'(A)') "ERROR: Cannot DUMP RESOLVED_KINETIC_ENERGY, CHECK_KINETIC_ENERGY=.FALSE."
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      GAS_PHASE_OUTPUT = RKE(II,JJ,KK)
   CASE(51)  ! TKE (turbulent kinetic energy)
      IF (.NOT.CHECK_KINETIC_ENERGY) THEN
         WRITE(MESSAGE,'(A)') "ERROR: Cannot DUMP SUBGRID_KINETIC_ENERGY, CHECK_KINETIC_ENERGY=.FALSE."
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      GAS_PHASE_OUTPUT = TKE(II,JJ,KK)
   CASE(52)  ! MTR (Pope measure of turbulence resolution)
      IF (.NOT.CHECK_KINETIC_ENERGY) THEN
         WRITE(MESSAGE,'(A)') "ERROR: Cannot DUMP TURBULENCE_RESOLUTION, CHECK_KINETIC_ENERGY=.FALSE."
         CALL SHUTDOWN(MESSAGE)
      ENDIF
...
IF (COLUMN_DUMP_LIMIT) THEN
   DO I=1,N_DEVC_FILES
      ...
      WRITE(LU_DEVC(I),TCFORM) STIME,(DEVICE(N)%VALUE/REAL(DEVICE(N)%COUNT,EB), &
