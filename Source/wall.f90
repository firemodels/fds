...
WALL_LOOP: DO IW=1,M%N_EXTERNAL_WALL_CELLS+M%N_INTERNAL_WALL_CELLS
   ...
   IF (SF%HT_DIM==1) CYCLE WALL_LOOP
...
INTEGER :: IIB,JJB,KKB,NWP,I,NL,N,I_OBST,N_LAYER_CELLS_NEW(MAX_LAYERS),N_CELLS,EXPON,ITMP,ITER
...
UNPACK_WALL_PARTICLE: IF (PRESENT(WALL_INDEX)) THEN
   ...
   IF (ONE_D%BACK_INDEX>0 .AND. ONE_D%BACK_MESH==NM) THEN
      WC_BACK => WALL(ONE_D%BACK_INDEX)
   ...
   ELSEIF (ONE_D%BACK_INDEX>0 .AND. ONE_D%BACK_MESH/=NM) THEN
      WC_BACK => MESHES(ONE_D%BACK_MESH)%WALL(ONE_D%BACK_INDEX)
      B1_BACK => MESHES(ONE_D%BACK_MESH)%BOUNDARY_PROP1(WC_BACK%B1_INDEX)
      B2_BACK => MESHES(ONE_D%BACK_MESH)%BOUNDARY_PROP2(WC_BACK%B2_INDEX)
      BC_BACK => MESHES(ONE_D%BACK_MESH)%BOUNDARY_COORD(WC_BACK%BC_INDEX)
      SURF_INDEX_BACK = WC_BACK%SURF_INDEX
   ENDIF
   BACKING = SF%BACKING
...
ELSEIF (PRESENT(THIN_WALL_INDEX)) THEN
   ...
   IF (ONE_D%BACK_INDEX>0 .AND. ONE_D%BACK_MESH==NM) THEN
      TW_BACK => THIN_WALL(ONE_D%BACK_INDEX)
      B1_BACK => BOUNDARY_PROP1(TW_BACK%B1_INDEX)
      BC_BACK => BOUNDARY_COORD(TW_BACK%BC_INDEX)
      SURF_INDEX_BACK = TW_BACK%SURF_INDEX
   ELSEIF (ONE_D%BACK_INDEX>0 .AND. ONE_D%BACK_MESH/=NM) THEN
      TW_BACK => MESHES(ONE_D%BACK_MESH)%THIN_WALL(ONE_D%BACK_INDEX)
      B1_BACK => MESHES(ONE_D%BACK_MESH)%BOUNDARY_PROP1(TW_BACK%B1_INDEX)
      BC_BACK => MESHES(ONE_D%BACK_MESH)%BOUNDARY_COORD(TW_BACK%BC_INDEX)
      SURF_INDEX_BACK = TW_BACK%SURF_INDEX
   ENDIF
   BACKING = SF%BACKING
   B1%TMP_G = TMP(BC%IIG,BC%JJG,BC%KKG)
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN UNPACK_WALL_PARTICLE
   ...
   Q_WATER_F = B1%Q_CONDENSE
   IF (ONE_D%BACK_INDEX>0 .AND. ONE_D%BACK_MESH==NM) THEN
      CFA_BACK => CFACE(ONE_D%BACK_INDEX)
...
SELECT CASE(BACKING)
   ...
   CASE(EXPOSED)  ! The backside is exposed to gas in current or adjacent mesh.
      ...
      IF (ONE_D%BACK_MESH/=NM .AND. ONE_D%BACK_MESH>0) THEN  ! Back side is in other mesh.
         TMP_GAS_BACK = B1_BACK%TMP_G
         DTMP = TMP_GAS_BACK - B1%TMP_B
         IF (PRESENT(WALL_INDEX)) THEN
            HTCB = HEAT_TRANSFER_COEFFICIENT(ONE_D%BACK_MESH,DTMP,SF%H_FIXED_B,SURF_INDEX_IN=SURF_INDEX,&
                                             WALL_INDEX_IN=ONE_D%BACK_INDEX)
         ELSE
            HTCB = 0._EB
         ENDIF
