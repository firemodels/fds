...
NAMELIST /HVAC/ AIRCOIL_ID,AMBIENT,AREA,CLEAN_LOSS,COOLANT_SPECIFIC_HEAT,COOLANT_MASS_FLOW,COOLANT_TEMPERATURE,CTRL_ID,&
                ...
                RAMP_ID,RAMP_LOSS,REVERSE,ROUGHNESS,SPEC_ID,TAU_AC,TAU_FAN,TAU_VF,TYPE_ID,VENT_ID,VENT2_ID,VOLUME_FLOW,XYZ
...
DO ND = 1, NE%N_DUCTS
   ...
   IF(.NOT. DU%LEAKAGE) THEN
    ...
    !  ENDIF
   ENDIF
   RHS(ARRAYLOC) = DU%VEL(OLD)+DT_HV/DU%LENGTH*(HEAD+DU%DP_FAN)/DU%RHO_D + &
                   0.5_EB*DU%TOTAL_LOSS*ABS(DU%VEL(NEW))*DU%VEL(GUESS)*DT_HV/DU%LENGTH
ENDDO
...
DUCT_LOOP: DO ND = 1, NE%N_DUCTS
   ...
   LHS(ARRAYLOC1,ARRAYLOC1) = 1._EB+0.5_EB*DU%TOTAL_LOSS*ABS(DU%VEL(NEW)+DU%VEL(GUESS))*DT_HV/DU%LENGTH
   DN=>DUCTNODE(DU%NODE_INDEX(1))
   IF (.NOT. DN%VENT) THEN
   ...
   DUCT_LOOP:DO ND = 1, DR%N_DUCTS
      ...
      SELECT CASE (DU%DUCT_INTERP_TYPE_INDEX)
         CASE (NODE1) ! duct cells and ductnodes adopt values from node 1 of lowest duct_index duct in ductrun
            DU%RHO_C(:) = DUCTNODE(DUCT(DR%DUCT_INDEX(1))%NODE_INDEX(1))%RHO
            ...
            ENDDO
            DO NN = 1, 2
               IF (DUCTNODE((DU%NODE_INDEX(NN)))%VENT) CYCLE
               DUCTNODE(DU%NODE_INDEX(NN))%RHO = DUCTNODE(DUCT(DR%DUCT_INDEX(1))%NODE_INDEX(1))%RHO
         ...
         CASE (NODE2) ! duct cells and ductnodes adopt values from node 2 of highest duct_index duct in ductrun
            DU%RHO_C = DUCTNODE(DUCT(DR%DUCT_INDEX(DR%N_DUCTS))%NODE_INDEX(2))%RHO
            ...
            DO NN = 1, 2
               IF (DUCTNODE((DU%NODE_INDEX(NN)))%VENT) CYCLE
               DUCTNODE(DU%NODE_INDEX(NN))%RHO = DUCTNODE(DUCT(DR%DUCT_INDEX(DR%N_DUCTS))%NODE_INDEX(2))%RHO
            ...
            END DO
         ! CASE (LINEAR_INTERPOLATION) ! linear interpolation between node 1 and 2 [commented out for now]
