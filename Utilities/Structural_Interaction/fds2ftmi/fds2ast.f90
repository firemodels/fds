               ...
               DO K=K1B(II),K2B(II),NSAM
                  DO J=J1B(II),J2B(II),NSAM
                     DO I=I1B(II),I2B(II),NSAM
                        ...
                        IF (VAR.EQ.1 .AND. TIME.EQ.TBEG .AND. Q(I,J,K,NV).EQ.20) VCOUNT(IOR_LOOP)=VCOUNT(IOR_LOOP)+1
                     ENDDO 
                  ENDDO
               ENDDO
             ENDDO PATCHES
...
!    IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') WRITE (100+VAR,'(E12.5)', ADVANCE='YES') 20.0
!    IF (VARIABLE_KIND==' HEAT TRANSFER COEFFICIENT    ') WRITE (100+VAR,'(E12.5)', ADVANCE='YES') TAST(V)
    IF (VARIABLE_KIND==' ADIABATIC SURFACE TEMPERATURE') THEN
