   ...
   DO TE = 0, 5 ! tetra edge
      ...
      IF ( PLANE_EDGE_INTERSECTION( BOXVERT,BOXNORMAL,TETRAVERT0,TETRAVERT1,VERT).EQ.1) THEN
         IF (IN_BOX(VERT, BP).EQ.1) THEN
             VERTS(3*NVERTS:3*NVERTS+2)=VERT
...
! add box plane vertices if inside tetrahedron
   ...
   DO BV = 0, 3
      ...
      IF (IN_TETRA(BOXVERT, -1) .EQ. 1) THEN
         VERTS(3*NVERTS:3*NVERTS+2)=BOXVERT
         FACENUM(NFACES) = FACENUM(NFACES) + 1
         NVERTS=NVERTS+1
      ENDIF
   END DO
...
! add tetrahedron plane vertices if inside box
   ...
   DO TV = 0, 2
      ...
      IF (IN_BOX(TETRAVERT, -1) .EQ. 1) THEN
         VERTS(3*NVERTS:3*NVERTS+2)=TETRAVERT
         FACENUM(NFACES) = FACENUM(NFACES) + 1
         NVERTS=NVERTS+1
      ENDIF
   END DO
...
DO J0 = 0, N-1
DO K0 = 0, N-1
V0(0) = 0.01 + I0*0.48/(N-1.0)
V0(1) = 0.01 + J0*0.48/(N-1.0)
V0(2) = 0.01 + K0*0.48/(N-1.0)
DO I1 = 0, N-1
DO J1 = 0, N-1
DO K1 = 0, N-1
V1(0) = 0.51 + I1*0.48/(N-1.0)
V1(1) = 0.01 + J1*0.48/(N-1.0)
V1(2) = 0.01 + K1*0.48/(N-1.0)
DO I2 = 0, N-1
DO J2 = 0, N-1
DO K2 = 0, N-1
V2(0) = 0.01 + I2*0.98/(N-1.0)
V2(1) = 0.51 + J2*0.48/(N-1.0)
V2(2) = 0.01 + K2*0.48/(N-1.0)
DO I3 = 0, N-1
DO J3 = 0, N-1
DO K3 = 0, N-1
V3(0) = 0.01 + I3*0.98/(N-1.0)
V3(1) = 0.01 + J3*0.98/(N-1.0)
V3(2) = 0.51 + K3*0.49/(N-1.0)
XMIN = MIN(V0(0),V1(0),V2(0),V3(0))-0.1
YMIN = MIN(V0(1),V1(1),V2(1),V3(1))-0.1
ZMIN = MIN(V0(2),V1(2),V2(2),V3(2))-0.1
XMAX = MAX(V0(0),V1(0),V2(0),V3(0))+0.1
YMAX = MAX(V0(1),V1(1),V2(1),V3(1))+0.1
ZMAX = MAX(V0(2),V1(2),V2(2),V3(2))+0.1
VOL_TETRA = ABS(VOLUME_VERTS(V0,V1,V2,V3))
VOL=0.0
DO I = 0, 3
DO J = 0, 3
DO K = 0, 3
BOXBOUNDS(0)=XMIN +     I*(XMAX-XMIN)/4.0
BOXBOUNDS(1)=XMIN + (I+1)*(XMAX-XMIN)/4.0
BOXBOUNDS(2)=YMIN +     J*(YMAX-YMIN)/4.0
BOXBOUNDS(3)=YMIN + (J+1)*(YMAX-YMIN)/4.0
BOXBOUNDS(4)=ZMIN +     K*(ZMAX-ZMIN)/4.0
BOXBOUNDS(5)=ZMIN + (K+1)*(ZMAX-ZMIN)/4.0
...
END DO
VERT_CENTER = VERT_CENTER/NVERTS
...
V2_EB=V2
V3_EB=V3
BOX_BOUNDS_EB=BOX_BOUNDS

CALL GET_VERTS(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB,VERTS_EB,NVERTS,FACESTART,FACENUM,NFACES)

VERTS(0:3*NVERTS-1) = VERTS_EB(0:3*NVERTS-1)

VOLUME = GET_TETRABOX_VOLUME(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB)
...
XXMIN = XMIN - (XMAX-XMIN)/3.0
XXMAX = XMAX + (XMAX-XMIN)/3.0
YYMIN = YMIN - (YMAX-YMIN)/3.0
YYMAX = YMAX + (YMAX-YMIN)/3.0
ZZMIN = ZMIN - (ZMAX-ZMIN)/3.0
ZZMAX = ZMAX + (ZMAX-ZMIN)/3.0
...
DO J = 0, NY-1
DO I = 0, NX-1
BOX_BOUNDS_EB(0) = XXMIN + I*(XXMAX-XXMIN)/NX
BOX_BOUNDS_EB(1) = XXMIN + (I+1)*(XXMAX-XXMIN)/NX
BOX_BOUNDS_EB(2) = YYMIN + J*(YYMAX-YYMIN)/NY
BOX_BOUNDS_EB(3) = YYMIN + (J+1)*(YYMAX-YYMIN)/NY
BOX_BOUNDS_EB(4) = ZZMIN + K*(ZZMAX-ZZMIN)/NZ
BOX_BOUNDS_EB(5) = ZZMIN + (K+1)*(ZZMAX-ZZMIN)/NZ
VOL = VOL + GET_TETRABOX_VOLUME(BOX_BOUNDS_EB,V0_EB,V1_EB,V2_EB,V3_EB)
END DO 
END DO
END DO
