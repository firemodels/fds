...
DRAG_LAW_SELECT: SELECT CASE (LPC%DRAG_LAW)
   ...
   CASE (SCREEN_DRAG)
      IF (QREL > ZERO_P .AND. LPC%FREE_AREA_FRACTION < 1.0_EB ) THEN
         ...
         C_DRAG = 2._EB*LP%ONE_D%X(1)*(K_SCREEN+Y_SCREEN)*ABS(LPC%ORIENTATION(1,1)*UREL)
         A_DRAG = DX(IIG)*DZ(KKG)
         C_DRAG = 2._EB*LP%ONE_D%X(1)*(K_SCREEN+Y_SCREEN)*ABS(LPC%ORIENTATION(1,2)*VREL)
         A_DRAG = DX(IIG)*DY(KKG)
         C_DRAG = 2._EB*LP%ONE_D%X(1)*(K_SCREEN+Y_SCREEN)*ABS(LPC%ORIENTATION(1,3)*WREL)
