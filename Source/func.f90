...
VOL_NEW = (THICKNESS+INNER_RADIUS-XLOW_NEW)**I_GRAD-(THICKNESS+INNER_RADIUS-XUP_NEW)**I_GRAD

   OLD_POINT_LOOP: DO
      XLOW = X_S(I_OLD-1)
      XUP = X_S(I_OLD)
      VOL = (THICKNESS+INNER_RADIUS-MAX(XLOW_NEW,XLOW))**I_GRAD-(THICKNESS+INNER_RADIUS-MIN(XUP_NEW,XUP))**I_GRAD
      IF (VOL_NEW >=0._EB) INT_WGT(I_NEW,I_OLD) = MAX(0._EB,MIN(1._EB,VOL/VOL_NEW))
      IF (XUP >= XUP_NEW) EXIT OLD_POINT_LOOP
      I_OLD = I_OLD+1
