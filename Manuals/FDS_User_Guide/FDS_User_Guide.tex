...
Another application of this would be if you wanted to track the water that evaporated from sprinklers, seperately from the water 
...
\noindent The above inputs create a species called {\ct WATER VAPOR SPK} that has all the same properties as {\ct WATER VAPOR}.  The first device will give only water that resulted from droplet evaproration, and the second device would give water that came from both sprinklers and combustion.
...
{\ct 'PARTICLE MASS'} (kg) \\
{\ct 'PARTICLE AGE'} (s) \\
For particles with a specified {\ct SURF\_ID}, you may also specify any of the solid phase output quantities. If the specified quantity is associated with a species, use the parameter {\ct QUANTITIES\_SPEC\_ID(N)} to specify the species. Here {\ct N} refers to the order of the specified output quantities on the {\ct PART} line.

By default, if no {\ct QUANTITIES} are specified and none are selected in Smokeview, then
...
\noindent
specifies that half the particle is facing upwards and half is facing downwards.   Each orientation is modeled in practise by a separate particle that sees the radiation that is directed to a plane orthogonally to the orientation. The mass calculations and the surface that receives the radiation are scaled by the number of orientations. The splitting along the coordinate axis is demonstrated for all geometries in Figures~\ref{cart_split} -~\ref{sphere_split}. The orientation can naturally be any direction between the axis too. For cartesian particle only orientations that are perpendicular to the length-width -plane make physical sense.
