...
!$OMP        DWDX,DWDY,II,JJ,KK,A_IJ,AA,B_IJ,BB,C_VREMAN,C_G,GVEC,IOR,MU_DNS) &
!$OMP PRIVATE(ZZ_GET)
...
SELECT_TURB: SELECT CASE (TURB_MODEL_TMP)
   ...
   CASE (DEARDORFF) SELECT_TURB ! Deardorff (1980) eddy viscosity model (current default)
      ...
      !! extrapolate to ghost cells (this probably is not needed)
      !
   ...
   SELECT CASE(WC%BOUNDARY_TYPE)
      CASE(SOLID_BOUNDARY)
         IF (LES) THEN
            ...
            CALL WERNER_WENGLE_WALL_MODEL(DUMMY,WC%U_TAU, &
                 SQRT(KRES(IIG,JJG,KKG)),MU_DNS/RHO(IIG,JJG,KKG),1._EB/WC%RDN,SURFACE(WC%SURF_INDEX)%ROUGHNESS)
...
REAL(EB) :: U_IBM,V_IBM,W_IBM,DN, &
            ...
            MU_WALL,RRHO,MUA,DUUDT,DVVDT,DWWDT,DELTA,WT,XV(3),TMP_SUM
INTEGER :: I,J,K,NG,IJK(3),I_VEL,IP1,IM1,JP1,JM1,KP1,KM1,ITMP,TRI_INDEX,IERR,N_CELLS,IC
