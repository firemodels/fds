...
Consider a liquid consisting of multiple components. According to the Clausius-Clapeyron relation, the volume fraction of component $\alpha$ in a thin film layer above the pool surface is given by:
\be X_{\alpha,{\rm film}} = X_{\alpha,{\rm liq}} \, \exp \left[ -\frac{ h_{\rm v,\alpha} W_{\alpha}}\R \left(\frac{1}{T_{\rm s}}-\frac{1}{T_{\rm b,\alpha}} \right) \right]
\label{CC_liquid}
\ee
where $X_{\alpha,{\rm liq}}$ is the volume fraction of component $\alpha$ in the liquid, $h_{\rm v,\alpha}$ is its heat of vaporization, $W_{\rm \alpha}$ is its molecular weight, $T_{\rm b,\alpha}$ is its boiling temperature, and $T_{\rm s}$ is the surface temperature. The evaporation rate is governed by Stefan diffusion~\cite{Taylor&Krishna}:
\be
\dot{m}_\alpha'' = h_{\rm m,\alpha} \, \rho \, \ln \left(\frac{1-X_{\alpha,{\rm gas}}}{1-X_{\alpha,{\rm film}}}\right) \quad ; \quad h_{\rm m,\alpha}= \frac{\SH \, D_{\alpha}}{L}
\ee
where $\rho$ is the gas density, $X_{\alpha,{\rm gas}}$ is the volume fraction of component $\alpha$ outside of the thin film layer, and $D_\alpha$ is its gas phase diffusivity. The Sherwood number is given by
\be
\SH=0.037~\SC^{\frac{1}{3}} \RE^{\frac{4}{5}} \quad ; \quad {\rm Sc}=0.6 \quad ; \quad \RE= \frac{\rho~||\bu||~L}{\mu}
\ee
The Reynolds number is calculated based on conditions in the cell adjacent to the surface. The length scale, $L$, used in calculating the Reynolds number is 1~m unless otherwise specified and is the same length scale used for the convective heat transfer calculation.
...
\subsection{Multi-component Liquid Evaportaion (Distillation)}
\label{sec:distillation}

Thermodynamic equilibrium at a liquid-vapor interface is determined by an obscure thermodynamic quantity called \emph{fugacity}.  At equilibrium, the fugacity of the liquid and vapor phases are the same for each component independently.  Fugacity is a bit like pressure, and so the analogy of a force balance at equilibrium for each component of the mixture is appropriate.  To formalize this equilibrium state, we equate the vapor phase, ${\rm g}$, and liquid phase, $\ell$, fugacities as follows:
\be
\label{eq:fugacity}
f_{\alpha,{\rm g}} = f_{\alpha,\ell}
\ee

The usual approximation for the vapor phase fugacity is to simply use the partial pressure of the component in the gas phase.  The approximation of the liquid phase fugacity involves computing (or assuming) an \emph{activity coefficient}, $\gamma_\alpha$, and liquid \emph{equilibriun vapor pressure}, $p^*_\alpha$.  A common simplification is to assume the activity coefficient is unity.  This leads to ``Raoult's law'':
\begin{align}
\label{eq:raoult}
X_{\alpha,{\rm g}} \, \bp_m &= X_{\alpha,\ell} \, p^*_\alpha
\end{align}
where $X_{\alpha,{\rm g}}$ is the \emph{mole} fraction of $\alpha$ in the vapor phase and $X_{\alpha,\ell}$ is the \emph{mole fraction} of $\alpha$ in the liquid phase.  Of course, if the liquid is pure, then $X_{\alpha,\ell}=1$, and we have the vapor pressure equal to the equilibrium vapor pressure, by construction.  Thus, as written above in Eq.~(\ref{CC_liquid}), the Clausius-Clapeyron equation is providing the equilibrium vapor mole fraction as the ratio of the equilibrium vapor pressure (of the fuel species, assumed pure in the liquid phase) to the local total thermodynamic pressure, $X_{\rm F,g} = p^*_{\rm F}/\bp_m$.

For distillation, we need to first rearrange the Clausius-Clapeyron equation to give the equilibrium vapor pressure for component $\alpha$:
\be
\label{eq:CC_eqbm_vap_press}
p^*_\alpha = \bp_m \exp \left[ -\frac{ h_{\rm v,\alpha} W_\alpha}{\R} \left(\frac{1}{T_{\rm s}}-\frac{1}{T_{\rm b,\alpha}} \right) \right]
\ee

Our goal now is to determine the vapor phase mole fractions which then feed into the mass flux driving force (Fick's law).  Typically, we will assume the liquid phase mole fractions to be known.  The surface temperature is also known from the energy balance---this may or may not over-constrain the problem.  Let $N_g$ represent the number of tracked species in the gas phase.  And let $N_\ell$ represent the number of liquid phase species. Note that $N_\ell \le N_g$.

Plugging Eq.~(\ref{eq:CC_eqbm_vap_press}) into Eq.~(\ref{eq:raoult}) gives an expression for the gas phase mole fraction at the surface of the liquid, assuming we know the surface temperature, $T_{\rm s}$.
\be
\label{eq:CC_eqbm_Xg}
X_{\alpha,{\rm g}} = X_{\alpha,\ell} \, \exp \left[ -\frac{ h_{\rm v,\alpha} W_\alpha}{\R} \left(\frac{1}{T_{\rm s}}-\frac{1}{T_{\rm b,\alpha}} \right) \right]
\ee
If $\sum_{\alpha=1}^{N_\ell} X_{\alpha,{\rm g}} \le 1$, then we set the remaining tracked species composition such that the surface composition sums to one.  If $\sum_{\alpha=1}^{N_\ell} X_{\alpha,{\rm g}} > 1$, then we either need to rescale the mole fractions accordingly such that they sum to 1 or we need to adjust the surface temperature.
