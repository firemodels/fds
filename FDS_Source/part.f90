...
USE MATH_FUNCTIONS, ONLY: AFILL2,INTERPOLATE1D_UNIFORM
USE TRAN, ONLY: GET_IJK
...
INTEGER :: II,JJ,KK,IW,NS,Y_INDEX,IP
INTEGER, INTENT(IN) :: NM
TYPE (LAGRANGIAN_PARTICLE_TYPE), POINTER :: LP=>NULL()
TYPE (LAGRANGIAN_PARTICLE_CLASS_TYPE), POINTER :: LPC=>NULL()
TYPE (SURFACE_TYPE), POINTER :: SF=>NULL()
...
Y_EQ_Z_LOOP: DO NS = 1,N_TRACKED_SPECIES
   ...
   CALL INTERPOLATE1D_UNIFORM(SS%C_P_L_BAR,TMP_MELT,H_L_REF)
   ...
   PART_EQ_Z_LOOP: DO IP = 1,NLP
      ...
      CALL INTERPOLATE1D_UNIFORM(SS%C_P_L_BAR,TMP_DROP,H_L)
...
SPECIES_LOOP: DO Z_INDEX = 1,N_TRACKED_SPECIES
   ...
   TMP_BOIL = SS%TMP_V
   MW_DROP  = SS%MW
   CALL INTERPOLATE1D_UNIFORM(SS%H_V,SS%H_V_REFERENCE_TEMPERATURE,H_V_REF)
   CALL INTERPOLATE1D_UNIFORM(SS%C_P_L_BAR,SS%TMP_MELT,H_L_REF)
   H_L_REF= H_L_REF*TMP_MELT
   I_MELT   = INT(TMP_MELT)
   FILM_THICKNESS => WALL_WORK2
   FILM_THICKNESS =  0._EB
...
!$OMP        TMPA,AVG_DROP_DEN,PI,OMRAF,NU_FAC_WALL,SH_FAC_WALL,QR_W) &
   ...
   PARTICLE_LOOP: DO IP=1,NLP
      ...
      TIME_ITERATION_LOOP: DO WHILE (DT_SUM < DT)
         ...
         TMP_DROP = LP%ONE_D%TMP(1)
         CALL INTERPOLATE1D_UNIFORM(SS%H_V,TMP_DROP,H_V)
         IF (H_V < 0._EB) THEN
            WRITE(MESSAGE,'(A,A)') 'Numerical instability in particle energy transport, H_V for ',TRIM(SS%ID)
            CALL SHUTDOWN(MESSAGE)
         ENDIF
         CALL INTERPOLATE1D_UNIFORM(SS%C_P_L,TMP_DROP,C_DROP)         
         CALL INTERPOLATE1D_UNIFORM(SS%C_P_L_BAR,TMP_DROP,H_L)                  
         H_L = H_L*TMP_DROP+H_L_REF
         H_D_OLD  = H_L*M_DROP
         WGT      = LP%PWT
...
!               NUSSELT  = NU_FAC_WALL*RE_L**0.8_EB
         ...
         ELSE
            ...
            CYCLE TIME_ITERATION_LOOP
         ENDIF
         CALL INTERPOLATE1D_UNIFORM(SS%H_V,TMP_DROP_NEW,H_V)
         DHOR     = H_V*MW_DROP/R0 
