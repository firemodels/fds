...
where the subscript infinity generally refers to the ground. A linear temperature stratification of the atmosphere may
specified by the user such that $T_0(z) = T_\infty + \Gamma z$ where $\Gamma$ is the ``lapse rate'' (e.g., $\Gamma = -0.0098$ K/m).  In this case $\bp_0$ and $\rho_0$ are derived from Eqs.~(\ref{pstrat}) and (\ref{state}), respectively.  It can then be shown that the pressure stratification becomes
\be
   \bp_0(z) = p_\infty\left( \frac{T_0[z]}{T_\infty} \right)^{\frac{\overline{W}g}{\R \Gamma}} \,\mbox{.}
   \label{pstrat}
\ee
...
$\nu_s$ and $\nu_{g,s}$, respectively.  These coefficients should
