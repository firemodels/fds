...
MAIN_LOOP: DO  
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      IF (BAROCLINIC .AND. .NOT.BAROCLINIC2) THEN
         ...
         ELSE
            RHO_UPPER_GLOBAL = RHO_UPPER_LOCAL
            RHO_LOWER_GLOBAL = RHO_LOWER_LOCAL
         ENDIF
      ENDIF
      ...
      COMPUTE_DIVERGENCE_LOOP: DO NM=1,NMESHES
         ...
         IF (.NOT.ISOTHERMAL .OR. N_SPECIES>0) THEN
            IF (HVAC_SOLVE .AND. .NOT. USE_MPI) THEN
               CALL HVAC_BC_IN(NM)
      ...
      FINISH_DIVERGENCE_LOOP: DO NM=1,NMESHES
         IF (PROCESS(NM)/=MYID)    CYCLE FINISH_DIVERGENCE_LOOP
         IF (.NOT.ACTIVE_MESH(NM)) CYCLE FINISH_DIVERGENCE_LOOP
   ...
   FINISH_DIVERGENCE_LOOP_2: DO NM=1,NMESHES
      IF (PROCESS(NM)/=MYID)    CYCLE FINISH_DIVERGENCE_LOOP_2
      IF (.NOT.ACTIVE_MESH(NM)) CYCLE FINISH_DIVERGENCE_LOOP_2
      CALL DIVERGENCE_PART_2(NM)
...
DO NM=1,NMESHES
   IF (ITERATE_PRESSURE .AND. PROCESS(NM)==MYID .AND. ACTIVE_MESH(NM)) MESHES(NM)%WALL_WORK1 = 0._EB
ENDDO
...
RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
   ...
   IF ((CODE==2 .OR. CODE==5) .AND. NIC(NOM,NM)>0) THEN
      IF (RNODE/=SNODE) THEN
            ...
            DO KK=M3%IJKW(12,IW),M3%IJKW(15,IW)
               DO JJ=M3%IJKW(11,IW),M3%IJKW(14,IW)
                  DO II=M3%IJKW(10,IW),M3%IJKW(13,IW)
                     ...
                     END SELECT
                     M3%REAL_SEND_PKG7(LL+3) = M%H(II,JJ,KK)
                     LL = LL+3
                  ENDDO
               ENDDO
            ENDDO
         ...
         N_REQ=MIN(N_REQ+1,SIZE(REQ))
         CALL MPI_ISEND(M3%REAL_SEND_PKG7(1),IWW*3+1,MPI_DOUBLE_PRECISION,SNODE,TAG,MPI_COMM_WORLD,REQ(N_REQ),IERR)
      ELSE
         M2=>MESHES(NOM)%OMESH(NM)
         DO IW=1,M4%NEWC
...
IF (USE_MPI) CALL MPI_WAITALL(N_REQ,REQ(1:N_REQ),ARRAY_OF_STATUSES,IERR)
         ...
         DO KK=M4%IJKW(12,IW),M4%IJKW(15,IW)
            DO JJ=M4%IJKW(11,IW),M4%IJKW(14,IW)
               DO II=M4%IJKW(10,IW),M4%IJKW(13,IW)
                  ...
                  END SELECT
                  M2%H(II,JJ,KK) = M2%REAL_RECV_PKG7(LL+3)
                  LL = LL+3
               ENDDO
            ENDDO
         ENDDO
...
ELSE
   RECOMPUTE_A_GLOBAL = RECOMPUTE_A
ENDIF
...
IF (RECOMPUTE_A) THEN
   A = TRANSPOSE(A)
   IF (USE_MPI) CALL MPI_GATHERV(A(1,DISPLS_PC(MYID)+1),COUNTS2D_PC(MYID),MPI_DOUBLE_PRECISION,A,COUNTS2D_PC,DISPLS2D_PC, &
                                 MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
ENDIF

IF (USE_MPI) CALL MPI_GATHERV(B(DISPLS_PC(MYID)+1),COUNTS_PC(MYID),MPI_DOUBLE_PRECISION,B,COUNTS_PC,DISPLS_PC, &
                              MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
...
IF (USE_MPI) CALL MPI_BCAST(B,NCGC,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,IERR)
...
IF (.NOT.USE_MPI .OR. (USE_MPI .AND. MYID==EVAC_PROCESS)) CALL EVAC_MESH_EXCHANGE(T_EVAC,T_EVAC_SAVE,I_EVAC,ICYC,EXCHANGE_EVACUATION,2)
