...
CHARACTER(255), PARAMETER :: ctrlid='$Id: ctrl.f90,v 1.9 2007/03/23 19:36:10 mcgratta Exp $'
...
USE GLOBAL_CONSTANTS, ONLY : STOP_CODE,CORE_CLOCK
! Update the value of all sensing DEVICEs and associated output quantities
REAL(EB), INTENT(IN) :: T(NMESHES)
  ...
  CASE (OR_GATE)
   ...
   CASE (XOR_GATE)
      ...
      DO NC = 1, CF%N_INPUTS
         SELECT CASE (CF%INPUT_TYPE(NC))
            CASE (DEVICE_INPUT)
               IF (DEVICE(CF%INPUT(NC))%CURRENT_STATE) THEN
                  COUNTER = COUNTER + 1
                  IF (T(DV%MESH) >T_CHANGE) THEN
                     CF%MESH = DV%MESH
                     T_CHANGE = T(DV%MESH)
