...
\ee
The same case is tested in 2D ({\ct box\_burn\_away\_2D} and {\ct box\_burn\_away\_2D\_residue}). In the latter only half of the mass is converted to fuel, yielding 50 \% residue. The box is made burn away by setting {\ct BULK\_DENSITY = 10}. These two cases also plot the remaining mass of the solid box as a function of time, demonstrating the artificial increase of the solid mass when new, unburned surface is explosed to the surface. However, despite the instantaneous increases of the solid mass, both the final solid mass and the gaseous degradation products should match the expected values in the end of the simulation.
\begin{figure}[ht]
...
\item[{\ct VOLUME\_FLOW}] is a fixed flow rate (m$^3$/s) through the duct.  If you specify {\ct VOLUME\_FLOW}, you can change its value in time either using the characteristic time, {\ct TAU\_VF},  to define a tanh ({\ct TAU\_VF}>0) or t$^2$ ramp ({\ct TAU\_VF}<0); or you can specify a {\ct RAMP\_ID}.  This cannot be controlled by a device or control function; however, a constant volume flow {\ct FAN} can be.
\end{description}
...
{\ct CONSTANT\_SPECIFIC\_HEAT}                  & Real          & Section~\ref{info:Enthalpy}                           &               & {\ct .FALSE.}    \\ \hline
