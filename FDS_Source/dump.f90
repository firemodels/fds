...
USE CONTROL_VARIABLES
REAL(EB) :: TNOW
INTEGER :: NN,I,N,N_OUT,N_ZONE_TMP
CHARACTER(30), DIMENSION(42) :: LABEL='null'
CHARACTER(30), DIMENSION(:), ALLOCATABLE ::  P_ZONE_ID
LOGICAL :: END_OF_LIST
...
CUTCELL_DATA: IF (CUTCELL_DATA_FILE) THEN
   IF (N_FACE<=0) CALL SHUTDOWN('ERROR: no cutcell data')
   LU_CUTCELL = GET_FILE_NUMBER()
   WRITE(FN_CUTCELL,'(A,A)') TRIM(CHID),'_cc.csv'
   OPEN(LU_CUTCELL,FILE=FN_CUTCELL,FORM='FORMATTED',STATUS='REPLACE')
   WRITE(LU_CUTCELL,'(A)') 'FACET, CELL INDEX, CUTCELL AREA'
   FACE_LOOP: DO I=1,N_FACE
      CL=>FACET(I)%CUTCELL_LIST
      IF ( ASSOCIATED(CL) ) THEN
         END_OF_LIST=.FALSE.
         DO WHILE (.NOT.END_OF_LIST)
            WRITE(LU_CUTCELL,'(I5,A,I5,A,E15.8)') I,',',CL%INDEX,',',CL%AREA
            CL=>CL%NEXT
...
DO N=1,N_GEOM
   ...
   IF (G%ISHAPE==ICYLINDER) THEN
      ...
      IF (ABS(G%XOR-1._EB) <TWO_EPSILON_EB) WRITE(LU_SMV,'(1X,A,1F12.5)') 'L=',G%X2-G%X1
      IF (ABS(G%YOR-1._EB) <TWO_EPSILON_EB) WRITE(LU_SMV,'(1X,A,1F12.5)') 'L=',G%Y2-G%Y1
      IF (ABS(G%ZOR-1._EB) <TWO_EPSILON_EB) WRITE(LU_SMV,'(1X,A,1F12.5)') 'L=',G%Z2-G%Z1
   ENDIF
...
IF (NLPDIM>0) ALLOCATE(MESHES(NM)%LAGRANGIAN_PARTICLE(NLPDIM))
DO N=1,N_LAGRANGIAN_CLASSES
   LPC => LAGRANGIAN_PARTICLE_CLASS(N)
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   DO K=0,KBAR
      ...
      ENDDO
   ENDDO
   CALL ISO_TO_FILE(LU_ISOF(N,NM),IS%REDUCE_TRIANGLES,STIME,QQ,&
...
REAL(EB), INTENT(IN) :: T
REAL(EB) :: VALUE,VALUE2,STAT_VALUE,SUM_VALUE,VOL,WGT,T_TMP,DVAL,DVAL2,DEV2,WGT_UNBIASED
...
WGT_UNBIASED = 1._EB/WGT-1._EB
IF (WGT_UNBIASED>TWO_EPSILON_EB) THEN
   WGT_UNBIASED = 1._EB/WGT_UNBIASED
ELSE
   WGT_UNBIASED = WGT
ENDIF
...
DEVICE_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%LINE>0) THEN
      ...
      SELECT CASE(TRIM(DV%STATISTICS))
         ...
         CASE('COV') ! covariance
            DV%AVERAGE_VALUE2 = (1._EB-WGT)*DV%AVERAGE_VALUE2 + WGT*VALUE2
            DVAL  = VALUE -DV%AVERAGE_VALUE
            DVAL2 = VALUE2-DV%AVERAGE_VALUE2
            DV%COV_VALUE = (1._EB-WGT_UNBIASED)*DV%COV_VALUE + WGT_UNBIASED*DVAL*DVAL2
         ...
         CASE('CORRCOEF') ! correlation coefficient
            ...
            DVAL2 = VALUE2-DV%AVERAGE_VALUE2
            DV%COV_VALUE  = (1._EB-WGT_UNBIASED)*DV%COV_VALUE  + WGT_UNBIASED*DVAL*DVAL2
            DV%RMS_VALUE  = (1._EB-WGT_UNBIASED)*DV%RMS_VALUE  + WGT_UNBIASED*(VALUE -DV%AVERAGE_VALUE )**2
            DV%RMS_VALUE2 = (1._EB-WGT_UNBIASED)*DV%RMS_VALUE2 + WGT_UNBIASED*(VALUE2-DV%AVERAGE_VALUE2)**2
...
REAL(EB) :: FLOW,HMFAC,H_TC,TMP_TC,RE_D,NUSSELT,AREA,VEL,K_G,&
            ...
            EXPON,Y_SPECIES,MEC,Y_SPECIES2,Y_H2O,R_Y_H2O,R_DN,SGN,Y_ALL(N_SPECIES),H_S
REAL(FB) :: TMPUP,TMPLOW,ZINT,TMP_WGT,DELTA,RHO2,BBF
INTEGER :: N,I,J,K,NN,IL,III,JJJ,KKK,Y_INDEX,Z_INDEX,PART_INDEX,ITMP,IP,JP,KP,FLOW_INDEX,IW,FED_ACTIVITY
CHARACTER(100) :: MESSAGE
...
SELECT CASE(IND)
   ...
   CASE(13) ! MIXTURE FRACTION
      GAS_PHASE_OUTPUT = ZZ(II,JJ,KK,REACTION(1)%FUEL_SMIX_INDEX)+ZZ(II,JJ,KK,Z_INDEX)* &
   ...
   CASE(21)  ! RELATIVE HUMIDITY
      ...
      ELSE
         ...
         GAS_PHASE_OUTPUT = RELATIVE_HUMIDITY(Y_ALL(H2O_INDEX),TMP(II,JJ,KK))
      ENDIF
   CASE(22)  ! HS
   ...
   CASE(53)  ! U CELL
      III = MAX(1,MIN(II,IBAR))
      GAS_PHASE_OUTPUT = 0.5_EB*(U(III,JJ,KK)+U(MAX(1,III-1),JJ,KK))
   CASE(54)  ! V CELL
      JJJ = MAX(1,MIN(JJ,JBAR))
      GAS_PHASE_OUTPUT = 0.5_EB*(V(II,JJJ,KK)+V(II,MAX(1,JJJ-1),KK))
   CASE(55)  ! W CELL
      KKK = MAX(1,MIN(KK,KBAR))
      GAS_PHASE_OUTPUT = 0.5_EB*(W(II,JJ,KKK)+W(II,JJ,MAX(1,KKK-1)))
   ...
   CASE(80)  ! I
      GAS_PHASE_OUTPUT = REAL(II,EB)
   CASE(81)  ! J
      GAS_PHASE_OUTPUT = REAL(JJ,EB)
   CASE(82)  ! K
      GAS_PHASE_OUTPUT = REAL(KK,EB)
   ...
   CASE(91:93) ! MASS FLUX
      SELECT CASE(IND)
         CASE(91) ! MASS FLUX X
            IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II+1,JJ,KK,1:N_TRACKED_SPECIES)
            RHO2      = RHO(II+1,JJ,KK)
            VEL       = U(II,JJ,KK)
            IW        = MAX(WALL_INDEX(CELL_INDEX(II,JJ,KK),1),WALL_INDEX(CELL_INDEX(II+1,JJ,KK),-1))
         CASE(92) ! MASS FLUX Y
            IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II,JJ+1,KK,1:N_TRACKED_SPECIES)
            RHO2      = RHO(II,JJ+1,KK)
            VEL       = V(II,JJ,KK)
            IW        = MAX(WALL_INDEX(CELL_INDEX(II,JJ,KK),2),WALL_INDEX(CELL_INDEX(II,JJ+1,KK),-2))
         CASE(93) ! MASS FLUX Z
            IF (N_TRACKED_SPECIES>0) ZZ_GET(1:N_TRACKED_SPECIES) = ZZ(II,JJ,KK+1,1:N_TRACKED_SPECIES)
            RHO2      = RHO(II,JJ,KK+1)
            VEL       = W(II,JJ,KK)
   ...
   CASE(96)  ! AEROSOL VOLUME FRACTION
      GAS_PHASE_OUTPUT = Y_SPECIES*RHO(II,JJ,KK)/SPECIES(SPECIES_MIXTURE(Z_INDEX)%SINGLE_SPEC_INDEX)%DENSITY_SOLID
   ...
   CASE(102)  ! INCAPACITATION TIME, minutes
      ...
      ELSE
         FED_ACTIVITY = 2
      ENDIF
      GAS_PHASE_OUTPUT = 60.0_EB*FED(ZZ_GET,RSUM(II,JJ,KK),FED_ACTIVITY) ! [1/s] => [1/min]
      GAS_PHASE_OUTPUT = 1.0_EB/MAX(.00069444444444_EB,GAS_PHASE_OUTPUT) ! max is 24 h
   ...
   CASE(109)  ! FED
      ...
      ELSE
         FED_ACTIVITY = 2
      ENDIF
      GAS_PHASE_OUTPUT = FED(ZZ_GET,RSUM(II,JJ,KK),FED_ACTIVITY)
   ...
   CASE(159) ! CONTROL
      GAS_PHASE_OUTPUT = CONTROL(DV%CTRL_INDEX)%INSTANT_VALUE
      
   CASE(160) ! CONTROL
      GAS_PHASE_OUTPUT = 0._EB
      IF (CONTROL(DV%CTRL_INDEX)%CURRENT_STATE) GAS_PHASE_OUTPUT = 1._EB
   ...
   CASE(173) ! ADA
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
      GAS_PHASE_OUTPUT = AVG_DROP_AREA(II,JJ,KK,LPC%ARRAY_INDEX)
   ...
   CASE(174) ! QABS
      ...
      IF (ABS(AVG_DROP_AREA(II,JJ,KK,LPC%ARRAY_INDEX))>TWO_EPSILON_EB) THEN
         DO N = 1,NUMBER_SPECTRAL_BANDS
            ...
            GAS_PHASE_OUTPUT = GAS_PHASE_OUTPUT + BBF*Q_SUM
         ENDDO
         GAS_PHASE_OUTPUT = GAS_PHASE_OUTPUT/REAL(NUMBER_SPECTRAL_BANDS,EB)
      ENDIF
      ...
      IF ( ((PY%PDPA_START<=T) .AND.(PY%PDPA_END>=T)) .OR. .NOT.PY%PDPA_INTEGRATE ) THEN
         ...
         DLOOP: DO I=1,NLP
            ...
            SELECT CASE(PY%QUANTITY)
               ...
               CASE('ENTHALPY')
                  ITMP = INT(LP%ONE_D%TMP_F)
                  TMP_WGT = LP%ONE_D%TMP_F - AINT(LP%ONE_D%TMP_F)
                  VEL = (SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP)+TMP_WGT*&
                        (SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP+1)-SPECIES(LPC%Y_INDEX)%C_P_L_BAR(ITMP)))*LP%ONE_D%TMP_F
...
ELSEIF (PRESENT(OPT_LP_INDEX)) THEN
   ...
   IF (INDX == 5) THEN ! If wall temperature of particles
      LPC => LAGRANGIAN_PARTICLE_CLASS(PART_INDEX)
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE( 6) ! INSIDE WALL TEMPERATURE
      IF (ABS(SUM(ONE_D%LAYER_THICKNESS)-SUM(SURFACE(SURF_INDEX)%LAYER_THICKNESS))>TWO_EPSILON_EB) THEN
         ...
         IF (DV%DEPTH>ONE_D%X(II2)) THEN
            SOLID_PHASE_OUTPUT = TMPA-TMPM
         ELSE
            DO II2=II2,1,-1
               IF (DV%DEPTH<=ONE_D%X(II2)) II1 = II2
            ENDDO
   ...
   CASE( 8) ! HRRPUM
      SOLID_PHASE_OUTPUT = ONE_D%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)*0.001*REACTION(1)%HEAT_OF_COMBUSTION/ &
                           SURFACE(SURF_INDEX)%SURFACE_DENSITY
   CASE( 9) ! HRRPUA
      SOLID_PHASE_OUTPUT = ONE_D%MASSFLUX(REACTION(1)%FUEL_SMIX_INDEX)*0.001*REACTION(1)%HEAT_OF_COMBUSTION
   ...
   CASE(28) ! SURFACE DEPOSITION
      SOLID_PHASE_OUTPUT = WC%AWM_AEROSOL(SPECIES_MIXTURE(Z_INDEX)%AWM_INDEX)
...
DO I=1,N_CTRL_FILES
   N_OUT = MIN(CTRL_COLUMN_LIMIT, N_CTRL - CTRL_COLUMN_LIMIT * (I - 1))
   WRITE(TCFORM,'(3A,I4.4,A)') "(",FMT_R,",",N_OUT,"(',',I2))"
...
IF (N_REACTIONS > 0) THEN
   WALL_LOOP2: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      M_DOT(1,NM) = M_DOT(1,NM) + WC%ONE_D%MASSFLUX_ACTUAL(REACTION(1)%FUEL_SMIX_INDEX)*WC%AW*WC%ONE_D%AREA_ADJUST
   ENDDO WALL_LOOP2
ENDIF
...
IF (N_ZONE>0) THEN 
   ...
   DO I=1,N_ZONE
      IF (.NOT.P_ZONE(I)%EVACUATION) THEN
         N_ZONE_TMP = N_ZONE_TMP + 1
         P_ZONE_P(N_ZONE_TMP) = MESHES(1)%PBAR(1,I)
      ENDIF
   ENDDO
ENDIF
...
WRITE(LU_HRR,TCFORM) STIME,0.001_EB*Q_DOT_TOTAL(1:N_Q_DOT),0.001_EB*SUM(Q_DOT_TOTAL(1:N_Q_DOT-1)),&
                     M_DOT_TOTAL(1),(MESHES(1)%PBAR(1,I),I=1,N_ZONE_TMP)
...
GEOM_LOOP: DO N=1,1 ! placeholder for multiple geometry files

   STATIC_IF: IF (ABS(T-T_BEGIN)<TWO_EPSILON_EB) THEN 
      WRITE(LU_GEOM(N)) 0,0  
   ENDIF STATIC_IF

   WRITE(LU_GEOM(N)) STIME,0
   ...
   IF (N_FACE>0) WRITE(LU_GEOM(N)) (FACET(I)%SURF_INDEX,I=1,N_FACE)
...
IF (PERIODIC_TEST==2) THEN
! Eventually we will migrate PERIODIC_TEST=2 over to new init scheme,
! but maintain backward compatibility for now.
   IMIN=1
   JMIN=1
   KMIN=1
ELSE
