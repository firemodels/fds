...
CHARACTER(30) :: SURF_DEFAULT='INERT',EVAC_SURF_DEFAULT='INERT'
LOGICAL :: SUCCESS,EX,THICKEN_OBSTRUCTIONS,BAD,SIMPLE_CHEMISTRY=.FALSE.,BACKGROUND,BACKGROUND_SMIX_DECLARED=.FALSE.,IDEAL=.FALSE.
...
REAL(EB) :: VOLUME_FRACTION(MAX_SPECIES),MASS_FRACTION(MAX_SPECIES),MASS_FRACTION_0
...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                ...
                C_FORCED_CYLINDER,C_FORCED_SPHERE,C_HORIZONTAL,C_VERTICAL,DNS,EDC,ENTHALPY_TRANSPORT,&
                ...
                NEW_EVAP,NEW_EVAP_DROP,NEW_CP_WHILE,NOISE,NOISE_VELOCITY,NO_EVACUATION,&
...
LOGICAL  :: ABSORBING,SMIX_COMPONENT_ONLY,PREDEFINED(MAX_SPECIES),AEROSOL
...
NAMELIST /SPEC/ ABSORBING,AEROSOL,BACKGROUND,CONDUCTIVITY,CONDUCTIVITY_SOLID,DENSITY_LIQUID,DENSITY_SOLID,DIFFUSIVITY,&
                ENTHALPY_OF_FORMATION,EPSILONKLJ,FIC_CONCENTRATION,FLD_LETHAL_DOSE, &
...
SPEC_READ_LOOP: DO N=1,N_SPECIES
   ...
   CALL GAS_PROPS(SS%ID,SS%SIG,SS%EPSK,SS%MW,SS%ABSORBING,SS%FORMULA,SS%LISTED,SS%ATOMS,SS%H_F)  
...
LOGICAL :: PREDEFINED(0:MAX_SPECIES)
...
NAMELIST /REAC/ A,A_LEAN,AUTO_IGNITION_TEMPERATURE,BETA_EDC,C,CHECK_ATOM_BALANCE,CO_PRODUCTION,CO_YIELD,CRITICAL_FLAME_TEMPERATURE,&
                E,E_LEAN,EDCM,EPUMO2,EQUATION,EXTINCTION_MODEL,FIXED_MIX_TIME,FORMULA,FUEL,FWD_ID,FYI,H,HEAT_OF_COMBUSTION,&
   ...
   IF (SIMPLE_CHEMISTRY) THEN
      IF(C<=ZERO_P .AND. H<=ZERO_P) THEN
         IF (TRIM(FORMULA)=='null') THEN
            CALL GAS_PROPS(FUEL,S_TMP,E_TMP,MW_FUEL,A_TMP,FORMULA,L_TMP,ATOM_COUNTS,H_F)   
         ELSE         
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2), &
            ...
            SPREAD_RATE,XYZ(3),MINIMUM_LAYER_THICKNESS,VEL_GRAD
...
REAL(EB) :: TEMPERATURE,DENSITY,RR_SUM,ZZ_GET(0:N_TRACKED_SPECIES),MASS_PER_VOLUME, &
            MASS_PER_TIME,DT_INSERT,UVW(3),HRRPUV,XYZ(3),DX,DY,DZ,HEIGHT,RADIUS
