...
Enthalpy, $h$, and specific heat, $c_p$ are computed by doing a mass weighted average of the values for $h$ or $c_p$ for the individual species present.  The species values are obtained by table lookup using the nearest integer to the local temperature.  Values for $h$ and $c_p$ were obtained from the NIST JANAF tables $T$~\cite{NIST JANAF:1}.
