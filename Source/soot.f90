...
SMIX_LOOP: DO N=1,N_TRACKED_SPECIES
   ...
   WALL_CELL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
      ...
      IF (WC%BOUNDARY_TYPE/=SOLID_BOUNDARY .OR. WC%ONE_D%NODE_INDEX /= 0) CYCLE WALL_CELL_LOOP
...
KN_FAC = MU_G*SQRT(0.5_EB*PI/(PBAR(KKG,PRESSURE_ZONE(IIG,JJG,KKG))*RHOG))
   ...
   GEOMETRY_LOOP:DO K=1,KBAR
      DO J=1,JBAR
         DO I=1,IBAR
            ...
            MFP = MUG*SQRT(0.5_EB*PI/(PBAR(K,PRESSURE_ZONE(I,J,K))*RHOG))
            IM1 = MAX(0,I-1)
            JM1 = MAX(0,J-1)
            KM1 = MAX(0,K-1)
            IM2 = MAX(1,I-1)
            ...
            DO N=1,N_PARTICLE_BINS(NS)
               DO NN=1,N_PARTICLE_BINS(NS)
                  ...
                  PHI_S = PHI_S_FAC(NS,N,NN)*SQRT(RHOG/MUG*DISSIPATION_RATE)
...
CFACE_LOOP: DO ICF=N_EXTERNAL_CFACE_CELLS+1,N_EXTERNAL_CFACE_CELLS+N_INTERNAL_CFACE_CELLS
   ...
   IIG = CFA%BOUNDARY_COORD%IIG
   JJG = CFA%BOUNDARY_COORD%JJG
   KKG = CFA%BOUNDARY_COORD%KKG
   RHOG = ONE_D%RHO_G
   TMPG = ONE_D%TMP_G
   ZZ_GET(1:N_TRACKED_SPECIES) = MAX(0._EB,ONE_D%ZZ_G(1:N_TRACKED_SPECIES)- M_DOT_PPP(IIG,JJG,KKG,1:N_TRACKED_SPECIES)*DT/RHOG) 
   CALL GET_MASS_FRACTION(ZZ_GET,O2_INDEX,Y_O2)
...
IIG = LP%BOUNDARY_COORD%IIG
JJG = LP%BOUNDARY_COORD%JJG
KKG = LP%BOUNDARY_COORD%KKG
