...
WRITE(LU_CORE(NM)) N_BOUNDARY_COORD_DIM,N_BOUNDARY_ONE_D_DIM,N_BOUNDARY_PROPERTY_DIM
...
READ(LU_RESTART(NM)) N_BOUNDARY_COORD_DIM,N_BOUNDARY_ONE_D_DIM,N_BOUNDARY_PROPERTY_DIM
...
IF (ALLOCATED(MESHES(NM)%BOUNDARY_PROPERTY_OCCUPANCY)) DEALLOCATE(MESHES(NM)%BOUNDARY_PROPERTY_OCCUPANCY)
...
ALLOCATE(MESHES(NM)%BOUNDARY_PROPERTY_OCCUPANCY(N_BOUNDARY_PROPERTY_DIM)) ; MESHES(NM)%BOUNDARY_PROPERTY_OCCUPANCY = 0
...
IF (ALLOCATED(MESHES(NM)%BOUNDARY_PROPERTY)) DEALLOCATE(MESHES(NM)%BOUNDARY_PROPERTY)
ALLOCATE(MESHES(NM)%BOUNDARY_COORD(N_BOUNDARY_COORD_DIM))
ALLOCATE(MESHES(NM)%BOUNDARY_ONE_D(N_BOUNDARY_ONE_D_DIM))
ALLOCATE(MESHES(NM)%BOUNDARY_PROPERTY(N_BOUNDARY_PROPERTY_DIM))

READ(LU_RESTART(NM)) N_WALL_CELLS,N_WALL_CELLS_DIM
IF (ALLOCATED(MESHES(NM)%WALL)) DEALLOCATE(MESHES(NM)%WALL) ; ALLOCATE(MESHES(NM)%WALL(N_WALL_CELLS_DIM))
DO IW=1,N_WALL_CELLS
   ...
   CASE(231) ! PDPA
      ...
      IF ( ((PY%PDPA_START<=T) .AND. (PY%PDPA_END>=T)) .OR. .NOT.PY%PDPA_INTEGRATE ) THEN
         ...
         DLOOP: DO I=1,NLP
               ...
               CASE('ENTHALPY')
                  ...
                  ELSEIF (LPC%SURF_INDEX>0) THEN
                     ...
                     IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
                        ...
                        IF (MAXVAL(ONE_D%X)>TWO_EPSILON_EB) PHI = PHI / (2._EB*MAXVAL(ONE_D%X))**PY%PDPA_M
                     ENDIF
                  ENDIF
               CASE DEFAULT
                  PHI = 1._EB
            ...
            DV%PDPA_NUMER = DV%PDPA_NUMER + LP%PWT*(2._EB*MAXVAL(ONE_D%X))**PY%PDPA_M * PHI
            IF (PY%HISTOGRAM)  CALL UPDATE_HISTOGRAM(PY%HISTOGRAM_NBINS,PY%HISTOGRAM_LIMITS,DV%HISTOGRAM_COUNTS,&
                                              (2._EB*MAXVAL(ONE_D%X))**PY%PDPA_M * PHI,LP%PWT*MAXVAL(ONE_D%X)**PY%PDPA_N)
            ...
            IF ((PY%QUANTITY /= 'NUMBER CONCENTRATION') .AND. &
               ...
               DV%PDPA_DENUM = DV%PDPA_DENUM + LP%PWT*(2._EB*MAXVAL(ONE_D%X))**PY%PDPA_N
            ENDIF
         ENDDO DLOOP
            ...
            DV_I_LOOP: DO I=SDV%I1,SDV%I2
               ...
               SELECT CASE(INDX)
                  CASE(50,51,52) ! VOLUME FLOW WALL (+,-)
                     SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + UN*ONE_D%AREA
                  CASE(53,54,55) ! MASS FLOW WALL (+,-)
                     ...
                     ELSE
                        Y_SPECIES = 1._EB
                     ENDIF
                     SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + Y_SPECIES*ONE_D%RHO_F*UN*ONE_D%AREA
                  CASE(56,57,58) ! HEAT FLOW WALL (+,-)
                     ZZ_GET(1:N_TRACKED_SPECIES) = ONE_D%ZZ_F(1:N_TRACKED_SPECIES)
                     CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,ONE_D%TMP_F)
                     SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + ONE_D%RHO_F*H_S*UN*ONE_D%AREA*0.001_EB
               END SELECT
            ENDDO DV_I_LOOP
         ENDDO DV_J_LOOP
      ENDDO DV_K_LOOP
...
WALL_LOOP: DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
   ...
   ZZ_GET(1:N_TRACKED_SPECIES) = ONE_D%ZZ_F(1:N_TRACKED_SPECIES)
   CALL GET_SENSIBLE_ENTHALPY(ZZ_GET,H_S,ONE_D%TMP_F)
   CALL GET_SPECIFIC_HEAT(ZZ_GET,CP,ONE_D%TMP_F)
   H_S_J_ALPHA = 0._EB
   ...
   Q_DOT(1,NM) = Q_DOT(1,NM) + ONE_D%Q_DOT_O2_PP*ONE_D%AREA
ENDDO WALL_LOOP
